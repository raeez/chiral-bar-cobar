...
alpha_g = 2*rank + 4*dim*h^v (universal Hilbert-series growth, all simple types)
...
| C | **PROVED unconditionally on Koszul locus** (C0, C1); C2 Lagrangian conditional on BV package | 9 strengthenings (2026-04-16): (T1) `lem:derived-center-koszul-equivalence`; (T2) `prop:perfectness-standard-landscape` UNCONDITIONAL family-by-family; (T3) C1 reflexivity unconditional Koszul locus; (T4) `thm:theorem-C-g0` for M-bar_{0,3}=pt (resolves +3 shift); (T5) C2 hypotheses pinned (BV + Verdier + bar-chart lift); (T6) C0 unconditional; (T7) class-M C2(iii) weight-completed; (T8) delta_F_2^cross(W_3) = (c+204)/(16c); (T9) `thm:C-PTVV-alternative` genuinely independent (RMap(M-bar_g, BG_A) + PTVV + Verdier). |
...
| Topologization | **PROVED chain-level original complex for G/L/C/critical; weight-completed for M** (`thm:topologization-tower`) | (1) G: E_3^top immediate (Dunn on F^{H_k}); (2) L (V_k(g), k!=-h^v): E_3^top ORIGINAL via Dunn rerouted through CG factorization F^{CS}_k on RxC + explicit eta_1^{(i)}, eta_1^{(ii)} making [Q, tilde G_1] = T_Sug STRICT chain-level; (3) C: E_3^top via FMS bosonization to Heis; (4) M: weight-completed chain-level E_3^top via half-BRST + MC5 pattern; (5) critical k=-h^v: E_2^top (not E_3). Original chain-level class M single remaining open. ALT H08 no longer conjectural. |
| E_3 identification | **PROVED simple; gl_N; semisimple; reductive; nilpotent (Heis); solvable** | Z^der_ch(V_k(g)) ~= A^lambda. 9 strengthenings (2026-04-16): `prop:e3-gl-N` 2-param (B_tr, B_ab); `thm:e3-identification-semisimple` via Cartan-Whitehead + Kunneth; `thm:e3-identification-reductive` N=r+C(d+1,2); `prop:e3-heisenberg` 4-dim Sym^2(h^*)^h; `thm:e3-identification-solvable` UNCONDITIONAL; chain-level qi-model via Fresse Vol II Thm 16.2.1; CY_4 p_1-twisted double current (pi_4(BU)=Z obstruction group). |
...
| CY_4 p_1-twisted | PROVED as Y_{p_1}(X), c(x,y) = <x cup y cup p_1(T_X), [X]>/24 | From pi_4(BU)=Z obstruction. 7d hCS. K3xK3: N(X)=0 unobstructed E_4. |
...
| Chiral QG equiv | **PROVED on ORDERED Koszul locus (E_1-chiral input included); elliptic + toroidal formal-disk PROVED** | 8 strengthenings (2026-04-16): `def:ordered-koszul-chiral-algebra`; `prop:yangian-ordered-koszul` via Drinfeld second + Etingof-Kazhdan; `thm:chiral-qg-equiv-ordered` E_1-chiral input; `prop:sl2-yangian-triangle-concrete` explicit order-hbar w/ triangle coherence; `thm:glN-drinfeld-double-internal` bypasses JKL26 via Miura + cross-arity Stasheff + U(gl_hat_1) screenings; `thm:w-infty-chiral-qg-completed` weight-completed; `thm:grt1-rigidity` (trivial H^0, homotopic cochains); `thm:chiral-qg-equiv-elliptic` Felder dynamical R(z,tau) + Fay; `thm:chiral-qg-equiv-toroidal-formal-disk` SV CoHA + Miki DIM. |
| Non-principal W | PROVED hook-type (r <= N-3) via parabolic screening (KRW03) | Subregular via (principal W_{N-2}) x sl_2 x betagamma. Minimal via coset. Log W(p) Massey <Omega,Omega,Omega> obstruction explicit. |
| gl_N chiral QG | **PROVED UNCONDITIONAL all N>=2 via Feigin-Frenkel screening** (JKL26 phantom eliminated) | W_N = int ker(Q_{alpha_i}) inside rank-(N-1) Heis. Drinfeld coproduct descends to intersection; RTT from abelian parent. (qdet-central-all-N) internal via Molev antisymmetriser. Psi=0 (free abelian, identity QG) and Psi=1 (free-boson, OPE obstruction vanishes) inscribed. Two-param gl_N RTT matches `rem:e3-non-simple-gl-N`. DS intertwining via independent sl_3 RTT. |
