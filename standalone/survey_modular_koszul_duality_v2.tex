...
$K(\Vir)=26$, $K(\cW_3)=100$, $K(\mathrm{BP})=196$, $K(V_k(\fg))=2\dim\fg$.
...
On the Koszul locus, in the generic/PBW-completed regime,
$\mathrm{ChirHoch}^*(\cA)$ has degree-$\leq2$ polynomial Hilbert
series and is concentrated in cohomological degrees $\{0,1,2\}$.
This is a Hilbert-growth and amplitude statement, not a
free-polynomial cup-product statement. Any multiplicative freeness
claim requires a separate product computation; the critical-level
affine lane is separate.
...
series). With Sugawara conformal vector at non-critical level, and
...
one-way chiral Hochschild consequence on the Koszul locus, one
...
at genus~$0$ only.
\item $\ChirHoch^*(\cA)$ has degree-$\leq2$ Hilbert polynomial and
curve-level amplitude; no free-polynomial cup-product assertion is
included without a separate multiplicative computation.
...
ChirHoch is one-way on the Koszul locus; the Lagrangian criterion is
...
chiral Hochschild consequence on the Koszul locus), the Lagrangian
...
algebraic closed sector whose image is the bulk sector of a 3d gauge
