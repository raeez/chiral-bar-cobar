...
self-dual at $k = -3$ with $\kappa(-3) = 49/3$ and Koszul conductor
$K = 98/3$. The conformal weight range is $\{1, 3/2, 2\}$
...
surfaces); for $d = 3$ it is conditional on chain-level
$\mathbb{S}^3$-framing (the antisymmetric Euler form produces an
$E_1$ obstruction, not a topological one).
...
\item \emph{CY-A at $d = 3$ unconditional}: chain-level
 $\mathbb{S}^3$-framing $+$ BV compatibility.
...
categories (proved for $d = 2$; conditional on chain-level
$\mathbb S^3$-framing for $d = 3$). The CY quantum groups
