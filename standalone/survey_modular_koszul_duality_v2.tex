...
\providecommand{\BRST}{\ensuremath{\mathrm{BRST}}}
\providecommand{\ord}{\ensuremath{\mathrm{ord}}}
...
characteristic, and family-supported chiral Hochschild cohomology
under \(H_H(\cA;S)\). The shadow obstruction tower
...
structure as the algebraic closed-sector actor; physical realisation
as the bulk algebra of a 3d holomorphic--topological gauge theory
requires the open--closed/HT comparison datum. Line operators,
spectral braiding, and the closed-sector--boundary--line triangle then
form the holographic surface, with 3d quantum gravity as the climax.
Calabi--Yau quantum groups (CoHA,
...
\noindent
The level-$2$ object before averaging is the \emph{ordered bar
...
carries the collision kernels and their represented braiding data;
homotopy coinvariants retain equivariant operator classes.  The
normalized residue/trace functionals produce the scalar coordinates
studied by the five theorems, while the represented Yangian and
braiding structures belong to the $E_1$ frontier.
...
normal factor along \(D_{ij}\), leaving no second normal direction for
the same divisor; the desuspension sign records the ordered-bar
orientation.
...
quasi-periodicity from the prime form, and the fiberwise model becomes
curved: the raw identity is $m_1^2=[m_0^{(g)},-]$, while the scalar
diagonal projection of $m_0^{(g)}$ is
$\kappa(\cA)\cdot\omega_g$.

\subsection*{1.4.\enspace The collision kernel and represented $R$-matrix}
...
\text{pole order of }K_\cA^{\mathrm{coll}}(z)
\;=\; \text{pole order of OPE} - 1.
\end{equation}
The result is the ordered collision kernel
\[
K_\cA^{\mathrm{coll}}(z)
...
of the ordered bar. Under $H_{\mathrm{CYBE}}^{\mathrm{rep}}$, a
representation sends this kernel to $r_\cA(z)$, and the Arnold
three-insertion relation maps to the Yang--Baxter equation
...
The collision kernel is an $E_1$ invariant. The symmetric bar carries
$q_2(K_\cA^{\mathrm{coll}})$ as an operator class. Its normalized
trace is $\kappa$ for abelian and Virasoro-type families and
$\kappa_{\mathrm{dp}}$ for non-abelian affine Kac--Moody, where the
Sugawara term $\dim(\fg)/2$ completes $\kappa$. This projection is the subject
of~\S1.7.
...
\end{equation}
Koszul dual:
$\cH_k^!=(\operatorname{Sym}^{\mathrm{ch}}(V^*[1]),m_0=-k\,\omega)$,
...
The reflected level is the parameter of the Chevalley--Eilenberg chiral
target,
\[
(\widehat\fg_k)^!_{\ch}
\simeq \mathrm{CE}^{\ch}(\widehat\fg_{-k-2h^\vee}),
\]
not a second affine vertex algebra. This is the Feigin--Frenkel
reflection of the invariant bilinear form followed by Verdier duality
on the Ran space. Complementarity:
...
\begin{aligned}
\text{degree }2\colon&\quad
\tau_{\cA,2}^{\mathrm{res}}q_2(K_\cA^{\mathrm{coll}})
+\kappa_{\mathrm{sp}}=\kappa(\cA)
 &&\text{(Theorem D; $\kappa_{\mathrm{sp}}=0$ for abelian, $\dim\fg/2$ for KM)},\\
\text{degree }3\colon&\quad
\tau_{\cA,3}^{\mathrm{res}}q_3((\Theta_\cA^{E_1})_{0,3})
 =\mathfrak{C}
 &&\text{(cubic shadow)},\\
\text{degree }4\colon&\quad
\tau_{\cA,4}^{\mathrm{res}}q_4((\Theta_\cA^{E_1})_{0,4})
 =\mathfrak{Q}
...
nothing. For Kac--Moody, $r(z)=k\,\Omega_{\mathrm{tr}}/z$ carries the Casimir
...
$\barB_X^\Sigma(\cA)$ to
$K_X(\cA)=\mathbb D_{\Ran}\barB_X^\Sigma(\cA)$.  A chosen Koszul
pair supplies $\nu_\cA\colon K_X(\cA)\to\cA^!$, an equivalence on
its finite-type or completed pair locus.  Bar--cobar reconstruction
and Ran--Verdier duality are the two typed branches issuing from the
full bar coalgebra.
...
twelve tests and consequences (\S9).
...
yields the scalar complementarity $\kappa(\cA)+\kappa(\cA^!)=\varkappa(\cA)$,
where $\varkappa=0$ for Kac--Moody and free fields, $\varkappa=13$ for Virasoro.
The scalar $\varkappa$ is distinct from the Trinity ghost-charge conductor
$K(\cA):=c(\cA)+c(\cA^!)=-c_{\mathrm{ghost}}(\BRST)$, values
$K(\Vir)=26$, $K(\cW_3)=100$, $K(\mathrm{BP})=50$, $K(V_k(\fg))=2\dim\fg$.

\medskip
\noindent\textbf{Theorem D} (four-stage modular trace).  For
$g\geq2$, its native class is
...
compares the deformation class with the perfect object.  Under
$H_D^{\mathrm{graph}}$,
$F_g=\kappa\lambda_g^{\mathrm{FP}}+\delta F_g^{\mathrm{cross}}$.

\medskip
\noindent\textbf{Theorem H} (chiral Hochschild).
For each finite support set \(S\) and family datum \(H_H(\cA;S)\), a
strong deformation retract identifies
\(C^\bullet_{\mathrm{ch}}(\cA,\cA)\) with a complete model
\(K_{\cA,S}\) supported in~\(S\).  Hence
\(\operatorname{Supp}\ChirHoch^\bullet(\cA)\subseteq S\).
The BDSK superboson bounded model has support \(\{0,1\}\) and
dimensions \((2,1)\); the Virasoro bounded model has support
\(\{0,2,3\}\), with one-dimensional groups, and reaches the chart
through a named bounded-to-chart quasi-isomorphism.
...
$m_1^2=[m_0^{(g)},-]$ arises from failure to extend across nodes; its
scalar diagonal is $\kappa \cdot \omega_g$.
...
$E_2$ input (higher Deligne). For a family datum \(H_H(\cA;S)\),
Theorem~H gives
\(\operatorname{Supp}\ChirHoch^\bullet(\cA)\subseteq S\). The
$\SCchtop$ structure lives on the pair
$(\cZ^{\der}_{\ch}(\cA),\, \cA)$: the algebraic closed sector acting
on the boundary. A physical bulk action requires the open--closed
comparison datum. The bar complex $\barB(\cA)$ is a single-coloured
$E_1$ coalgebra, NOT an $\SCchtop$-coalgebra.
...
interprets these constructions physically by an open--closed
comparison: the derived centre is the algebraic closed sector whose
OCA image is the physical bulk sector of a 3d gauge theory.
...
differential is curved with scalar diagonal projection
...
Two conventions coexist. The period-corrected total bar differential is
square-zero at every genus. The fiberwise differential belongs to a
curved $A_\infty$/CDG package with $m_1^2=[m_0,-]$; the displayed
formula is only the scalar diagonal projection on a single curve
$\Sigma_g$. On that fibre, the Arnold relations fail by a
$(1,1)$-correction from the period matrix. The total
...
\]
The derivation proceeds by the normalized trace. In the
trace-form convention, the collision kernel is
$K_{k,\fg}^{\mathrm{coll}}(z)=k\Omega_{\mathrm{tr}}/z$,
where $\Omega=\sum_a t^a\otimes t^a$ is the Casimir of the Killing
form. The double-pole channel gives
$\tau_{k,\fg}^{\mathrm{res}}q_2(K_{k,\fg}^{\mathrm{coll}})
=k\dim\fg/(2h^\vee)$. The simple-pole
...
$K_{\Vir_c}^{\mathrm{coll}}(z)=(c/2)/z^3+2T/z$.
The normalized functional extracts the central scalar channel:
\[
\kappa(\mathrm{Vir}_c)=\tau_{\Vir_c,2}^{\mathrm{res}}
q_2(K_{\Vir_c}^{\mathrm{coll}})=c/2.
\]
...
The scalar curvature~\eqref{eq:pref-curvature} is the projected shadow
of the fiberwise curved identity $m_1^2=[m_0,-]$, but it does not
obstruct the total $D_g^2=0$. Integration against $A$-cycle periods
...
$D_g^2=0$: the projected $(1,1)$-curvature of the fibrewise curved
package is absorbed by the base-period correction. The fibrewise
scalar class $\kappa\cdot\omega_g\in H^{1,1}(\mathcal C_g)$ is identified
...
Then
\begin{align*}
D_g^2&=\operatorname{tr}_{\mathrm{diag}}(m_0^{(g)})+\{d_{\mathrm{fib}},\nabla^{\GM}\}+(\nabla^{\GM})^2\\
...
$\overline{\mathcal M}_g$.  For $g\geq2$, Theorem~D begins with
$\operatorname{Obs}^{\mathrm{def}}_g(\cA)\in
H^2(\operatorname{Def}_g(\cA))$.  Under~$H_D^K$, its perfect
virtual realization and signed Hodge character are
\begin{equation}\label{eq:obs-uniform}
\mathfrak O_g^K(\cA)=\kappa(\cA)\lambda_{-1}(\mathbb E_g),\qquad
\operatorname{ch}_g(\mathfrak O_g^K(\cA))=(-1)^g\kappa(\cA)\lambda_g,
\qquad \end{equation}
where $\mathbb E=\pi_*\omega_\pi$ is the Hodge bundle: the vector
bundle on $\overline{\mathcal M}_g$ whose fibre over $[C]$ is the
$g$-dimensional space $H^0(C,\omega_C)$.  The package~$H_D^1$ gives
$\operatorname{tr}_1\operatorname{Obs}^{\mathrm{def}}_{1,1}
=\kappa(\cA)\lambda_1$, and $H_D^{\mathrm{tr}}$ supplies the
deformation-to-$K$ comparison.  Under~$H_D^{\mathrm{graph}}$ the
mixed-propagator boundary graphs give the numerical decomposition
\begin{equation}\label{eq:obs-multiweight}
F_g(\cA)=\kappa(\cA)\lambda_g^{\mathrm{FP}}
+\delta F_g^{\mathrm{cross}}(\cA).
\end{equation}
(multi-weight genus expansion theorem, full monograph).  The
scalar-diagonal graph package sets $\delta F_g^{\mathrm{cross}}=0$.
...
generating function of the scalar-diagonal graph functional is
\[
\sum_{g\ge 1}F_g^{\mathrm{sc}}(\cA)x^{2g}
=\kappa(\cA)\bigl(\hat A(ix)-1\bigr).
\]
...
For $\cH_k$, the Koszul dual is
$\cH_k^!=(\Sym^{\mathrm{ch}}(V^*[1]),m_0=-k\omega)$
...
\subsection*{2.6.\enspace Feigin--Frenkel centre and the critical-level reflection}
...
\(\iota_{\mathrm{crit}}(k)=-k-2h^\vee\) is the scalar level transform
visible on the affine Koszul lane. It is not the Feigin--Frenkel
centre theorem and not, by itself, an identification of the chiral
Koszul dual object. The actual Feigin--Frenkel theorem enters at the
fixed critical level, where the centre is the oper algebra for the
Langlands dual group. The chiral Koszul dual still passes through the
Verdier-dual bar and chiral CE construction.

\medskip
\noindent\textbf{The critical-level reflection in coordinates.}\enspace
For $\fg=\fsl_2$, $h^\vee=2$, and the reflection is $k\mapsto -k-4$.
...
For $\fg=\fsl_N$, $h^\vee=N$, and the reflection is $k\mapsto -k-2N$.
The fixed point $k=-N$ is the self-dual level; the Sugawara central
charge at $k=-N$ is $c=(-N)\cdot(N^2-1)/(-N+N)$, which is undefined
(the fixed point of the reflection lies at the critical level for
$\fsl_N$). This is not a coincidence: the reflection has no fixed
...
\item The degree-$2$ ordered chiral homology
$H^1_{\mathrm{ord}, 2}(\widehat\fg_{-h^\vee})$ on $V \otimes V$
doubles in dimension: for $\fsl_2$, from $4$ (at generic $k$) to $8$
(at $k=-2$). This is a finite computation on $V \otimes V$ conserving
$\chi_{\mathrm{ord}, 2} = -4$. The FULL symmetric bar cohomology at
critical level becomes $\Omega^*(\mathrm{Op}_{{}^L\fg}(D))$, which is
infinite-dimensional in every degree (Frenkel--Teleman 2006).
...
$\operatorname{tr}_{\mathrm{diag}}(m_0^{(g)})=\kappa\cdot\omega_g$
is a statement about the $E_\infty$-chiral bar complex. At the $E_1$
(ordered) level, the same curved package appears but the additional structure of the
...
the Ran space $\Ran(X)$ (unordered configurations). The scalar diagonal
curvature $\kappa\cdot\omega_g$ is a scalar multiple of
...
algebraic closed-sector colour, $\cA$ is the boundary/open colour, and
the closed-sector action on the boundary is by chiral Hochschild
evaluation. A physical bulk reading requires the open--closed
comparison datum.

For \(E_\infty\)-chiral input satisfying \(H_H(\cA;S)\), Theorem~H
identifies \(\ChirHoch^\bullet(\cA)\) with the cohomology of the
finite-support model \(K_{\cA,S}\). With Sugawara conformal vector at non-critical level, and
after the stress-tensor homotopy data have been supplied, $\SCchtop$
descends to an $E_3^{\mathrm{top}}$ object. This is proved for
affine KM at non-critical level and for the principal-Casimir lanes
where the antighost homotopies are constructed; arbitrary conformal
vectors do not by themselves topologise the theory.

For $E_1$-chiral input (Yangians): the symmetric bar $\barB^\Sigma$
is unavailable until an \(R\)-twisted descent datum has been supplied
(the D-module does not descend to symmetric configurations by
forgetting order). The ordered derived centre carries only the
ordinary brace/\(E_2\) structure attached to the chosen associative
model; the symmetric Ran Deligne--Tamarkin package is not transported
from this data. Any passage to \(E_3\) requires a separate
topologization or Drinfeld-centre comparison theorem.
...
$\omega_g$, producing a curvature element whose scalar diagonal is
$\kappa\cdot\omega_g$.
...
$\Omega(\barB(\cA))\to\cA$ is a quasi-isomorphism. Twelve tests and
consequences (seven genuinely independent bidirectional equivalences
\textup{(i)--(v), (ix)--(x)}, one listed Barr--Beck--Lurie
consequence, one conditional factorization-homology comparison, one
one-way $H_H(\cA;S)$-conditional chiral Hochschild support consequence, one
shifted-symplectic-package-conditional criterion, one one-directional
implication) characterise this locus. The Koszulness programme
(\S9) proves the core equivalences from PBW concentration, bar-cobar
inversion, and Koszul-complex acyclicity.  Theorem~H is the
family-supported one-way consequence. All standard families are Koszul; shadow depth classifies
the complexity of Koszul algebras, not Koszulness itself.

\medskip
\noindent\textbf{The twelve Koszul characterisations (overview).}\enspace
The characterisation of chirally Koszul algebras involves twelve
numbered conditions, organised as $8 + 1 + 1 + 1 + 1$: eight
genuinely independent bidirectional equivalences, one
Barr--Beck--Lurie monadicity listed as a consequence of the
bar-cobar counit, one one-way chiral Hochschild consequence on
the Koszul locus, one conditional Lagrangian criterion, and one
one-directional $\mathcal{D}$-module purity:
\begin{enumerate}[label=(\roman*)]
...
\item A declared family datum \(H_H(\cA;S)\) supplies a strong
deformation retract onto \(K_{\cA,S}\) and finite support
\(\operatorname{Supp}\ChirHoch^\bullet(\cA)\subseteq S\).
...
at genus~$0$ only.
\item The family support model \(K_{\cA,S}\) computes
\(\ChirHoch^\bullet(\cA)\) under the strong deformation retract of
\(H_H(\cA;S)\).
...
extensions are Theorem~B.  Theorem~H supplies the one-way
family-support consequence recorded in~\textup{(iv)} under
\(H_H(\cA;S)\).
...
chiral Hochschild cohomology with its declared finite-support model.
...
is uncurved; the tangent cohomology $H^1$ jumps in dimension from
$1$ at generic~$k$ to $1+\dim\mathfrak z(\widehat\fg_{-h^\vee})$
at critical level. Since $\mathfrak z(\widehat\fg_{-h^\vee}) \cong
\Fun(\Op_{{}^L\fg}(D))$ is infinite-dimensional, this is not a
"doubling" but a jump from finite to infinite dimension; the
additional tangent directions correspond to the Feigin--Frenkel
center, so the deformation theory at $k=-h^\vee$ sees the
full space of opers.
...
\]
Under~$H_D^1$ this scalar multiplies $\lambda_1$ in the pointed
genus-one trace.  Under~$H_D^K$ it multiplies
$\lambda_{-1}(\mathbb E_g)$ in the perfect object, while
$H_D^{\mathrm{graph}}$ gives
$F_g=\kappa\lambda_g^{\mathrm{FP}}+\delta F_g^{\mathrm{cross}}$.
...
\]
The factor $k^2$ from two propagator insertions cancels against
...
The categorical logarithm of \S1 is the logarithmic normal form
\(\eta_{ij}\) along collision diagonals, represented by
\(d\log(z_i-z_j)\) only on affine/formal genus-zero screens. It is the
local integral kernel entering the residue part of \(D_\cA\), while
projective and positive-genus screens use the corresponding
projective-linear and period/KZB replacement data. Three structure
theorems emerge. \emph{Algebraicity}: on
...
data. The $E_1$ structure is ordered; the symmetric
...
$k\Omega_{\mathrm{tr}}/z$ (affine) or $(c/2)/z^3 + 2T/z$ (Virasoro) or the
...
$\widehat{\mathfrak{sl}}_2$ & $k\Omega_{\mathrm{tr}}/z$ & $2n{+}1$
 & linear & rational & $\frac{3(k{+}2)}{4}$ & $\mathbf L$\\
$\widehat{\mathfrak{sl}}_3$ & $k\Omega_{\mathrm{tr}}/z$ & $\{8,36,204,\ldots\}$
...
\medskip\noindent\textbf{Collision kernel and represented $R$-matrix.}\enspace
$K_{\cH}^{\mathrm{coll}}(z)=k\Omega_{\cH}/z$ (Tier~(a)). Spectral $R$-matrix
$R(z)=\exp(k/z)$; Yang--Baxter trivial.

\medskip\noindent\textbf{Coinvariant and scalar trace.}\enspace
$q_2(K_{\cH}^{\mathrm{coll}})$ is the rank-one operator class and
$\tau_{\cH,2}^{\mathrm{res}}q_2(K_{\cH}^{\mathrm{coll}})=k
=\kappa(\cH_k)$. Koszul dual:
$\cH_k^!=(\Sym^{\mathrm{ch}}(V^*[1]),m_0=-k\omega)$ (curved).
...
$r^{\mathrm{KM}}(z)=k\,\Omega_{\mathrm{tr}}/z$ with
...
Sugawara shift:
\[
\mathrm{av}(k\,\Omega_{\mathrm{tr}}/z)=\frac{k\dim\fg}{2h^\vee}=\kappa_{\mathrm{dp}},
\qquad
...
\simeq \mathrm{CE}^{\ch}(\widehat\fg_{-k-2h^\vee}),
\]
not a second affine vertex algebra; the reflection
$k+h^\vee\mapsto-(k+h^\vee)$ is followed by Verdier duality on the Ran
space.
...
\medskip\noindent\textbf{Collision kernel.}\enspace
$K_{\Vir_c}^{\mathrm{coll}}(z)=(c/2)/z^3+2T/z$: the quartic and quadratic OPE poles
drop by one order each under $d\log$ absorption. Tier~(b),
class~$\mathbf{M}$.

\medskip\noindent\textbf{Coinvariant and scalar trace.}\enspace
$\tau_{\Vir_c,2}^{\mathrm{res}}
q_2(K_{\Vir_c}^{\mathrm{coll}})=c/2=\kappa(\Vir_c)$. The stress-tensor
...
% ----- 7.4 W_3 -----
\subsection*{7.4.\quad \texorpdfstring{$\boldsymbol{\cW_3}$}{W3}}
...
\medskip\noindent\textbf{Collision kernel.}\enspace
The ordered collision kernel has Casimir, $T$-, and
$W$-mediated channels; the normalized channel coordinates are
$\kappa_T=c/2$ (from $T$) and $\kappa_W=c/3$ (from $W$).
Tier~(b), class~$\mathbf{M}$.

\medskip\noindent\textbf{Coinvariant and scalar trace.}\enspace
$\tau_{\cW_3,2}^{\mathrm{res}}q_2(K_{\cW_3}^{\mathrm{coll}})
=c/2+c/3=5c/6=\kappa(\cW_3)$:
...
% ----- 7.5 beta-gamma -----
\subsection*{7.5.\quad \texorpdfstring{$\boldsymbol{\beta\gamma}$}{beta-gamma} system}
...
$r(z)$ & $k\,\Omega_{\mathrm{tr},\mathfrak{sl}_3}/z$ & $r_c^{W_3}(z)$\\
$\kappa$ & $4(k+3)/3$ & $5c(k)/6$\\
depth & $3$ & $\infty$\\
$\Theta^{(2)}_{\mathrm{pf}}$ & $0$ & $\neq 0$
\end{tabular}
\end{center}
The trace-form Casimir $r$-matrix
$k\Omega_{\mathrm{tr}}/z$ specialises to
...
the critical-level reflection; the master commutative square
...
At integer level $k$, the ordered/TUY comparison lane recovers the
Verlinde formula: $Z_g = \sum_j S_{0j}^{2-2g}$ is the rank of the
TUY/Hitchin conformal-block sheaf after integrable truncation, boundary
sewing, projectively flat connection, and determinant-anomaly matching,
with the ordered chiral chain complex entering through the comparison
morphism and $n = k+2$. Closed-form
...
the Casimir $r$-matrix $k\,\Omega_{\mathrm{tr}}/z$ specialises to the
...
$4$ & $?$ & $?$ & $\mathbf M$\\
BP & $25/3$ (conditional) & $k = -3$ & $\mathbf M$
\end{tabular}
\end{center}
The Koszul conductor is a global duality invariant, not a local
ghost constant: $K^c_{\mathrm{BP}} = 50$ records the full
...
$V_\Lambda$ & $\mathrm{rk}(\Lambda)$ & $0$ & $N/z$
 & $\mathbf G$ & $2$ & $2$\\
$\mathrm{BP}_k$ & $\mathrm{BP}(k)$ & $25/3$ (cond.) & $r_k^{\mathrm{BP}}(z)$
 & $\mathbf M$ & $\infty$ & $?$\\
...
$c(\mathrm{BP}_k)$ has a simple pole; the value $\kappa(-3) = 25/6$
...
identity $c(k) + c(-k-6) \equiv 50$ in $\mathbb{Q}(k)$, and the induced
conditional $\kappa$-conductor is $K = \varrho \cdot 50 = 25/3$ with
...
Hecke--Newton closure gives the conditional chain:
finite-Hecke-span stability + prime-locality + MC moment recursion
$\Rightarrow$ prime-local CG closure $\Rightarrow$ CPS automorphy
$\Rightarrow \Sym^{r-1}(f)$ $\Rightarrow$ Ramanujan bound.
Deligne's theorem supplies the bound itself; the MC framework
supplies the comparison from chiral bar-cobar moments to the finite
Hecke span, not an independent proof of the bound.
...
This is an anomaly-accounting dictionary. It becomes a string-critical
statement only after the determinant line, \(bc\)-ghost complex, BRST
current, and nilpotence equation \(Q^2=0\) have been supplied.
...
Conformal anomaly $(c/12)R$ & modular characteristic $\kappa$ on the
algebraic shadow lane\\
$\det'_\zeta \Delta$ & Fredholm $\det(1 - K)$\\
Critical dimension $c_{\mathrm{matter}} = 26$
& BRST cancellation
\(c_{\mathrm{matter}}+c_{\mathrm{ghost}}=0\) for the chosen ghost
complex\\
Ghost system $bc$ & external BRST ghost resolution, compared with
$\cA^!$ only through a named BRST/Koszul map\\
...
non-perturbative completions are comparison targets for
$\Theta_\cA \in \MC(\gAmod)$ once the corresponding determinant-line
and BRST packages are fixed. The four depth classes
...
\Lambda^{\mathrm{bar}}}\|\ell\|^{-2s}$
restricts the Epstein series to the sublattice
$\Lambda^{\mathrm{bar}} \subset \Lambda$ determined by the bar complex.
...
decomposition, one for each degree stratum beyond
...
At integer level $k$, the ordered/TUY comparison lane recovers the
Verlinde formula
$Z_g = \sum_j S_{0j}^{2-2g}$ as the TUY/Hitchin conformal-block rank
after the comparison package is supplied for
~$V_k(\mathfrak{sl}_2)$ with $n = k+2$.
...
$d - 1$ constituent centres to the decomposition.
\end{enumerate}
...
Epstein analogue of the MC complementarity
$\kappa + \kappa^! = \varrho_\cA \cdot K$ (with $\varrho_\cA$
family-specific: $\varrho_{\mathrm{KM}} = \varrho_{\mathrm{free}} = 0$,
...
(twelve tests and consequences: seven genuinely independent
bidirectional equivalences \textup{(i)--(v), (ix)--(x)}, with
\textup{(vi)} a proved consequence of~\textup{(v)},
\textup{(vii)} a conditional factorization-homology comparison,
\textup{(viii)} a one-way chiral Hochschild consequence,
\textup{(xi)} conditional on the shifted-symplectic package, and
\textup{(xii)} one-directional). The fourteen-chart associator atlas
is the separate moduli-chart refinement.
...
(Lagrangian criterion). Seven are independent bidirectional
equivalences; Barr--Beck--Lurie is a listed consequence; the
factorization-homology row is conditional on comparison and detection;
the ChirHoch row records the family datum (H_H(\cA;S)); the
Lagrangian criterion is
...
following are organised by the status split above:}
\end{center}
...
complex.  Item~(viii) follows separately from the strong deformation
retract in (H_H(\cA;S)) and records its family-indexed support.
...
\item \emph{PBW universality} (sufficient). Every freely strongly generated vertex algebra is chirally Koszul (Proposition~\ref{prop:pbw-universality}). Applied to universal affine algebras, this is the non-critical PBW/Koszul lane; the critical Feigin--Frenkel centre boundary is a separate Sugawara/KZ degeneration surface, and ``admissible'' is a condition on simple quotient levels rather than an extra universal-PBW theorem. For simple quotients $L_k(\fg)$ at admissible levels, rationality is not a Koszulness criterion: the $L_k(\mathfrak{sl}_2)$ lane has finite Shapovalov/character evidence and is conditional on the quotient-bar package, while at $\operatorname{rk}(\fg)\ge 2$ with denominator $q\ge 3$, Koszulness fails via a $\operatorname{rk}(\fg)$-dimensional bar obstruction from the abelian Cartan
(Theorem~\ref{thm:admissible-sl3-non-koszul-qge3} at $\fg = \mathfrak{sl}_3$; Conjecture~\ref{conj:admissible-koszul-rank-obstruction} at $\operatorname{rk}(\fg) \geq 3$). PBW universality is one-directional: not every Koszul algebra is freely strongly generated.
...
& (xi) Lagrangian & conditional on shifted-symplectic package & perfectness input verified\\
...
$K^c_{\mathrm{BP}}=50$ (a global duality invariant, not
...
cohomology]\label{thm:svy-theorem-h-v2}
Fix a finite set \(S\subset\mathbb Z_{\ge0}\) and a family datum
\(H_H(\cA;S)\).  The datum supplies a complete chart model, a model
\(K_{\cA,S}\) supported in~\(S\), and a strong deformation retract
\[
C^\bullet_{\mathrm{ch}}(\cA,\cA)
\ \underset{i}{\overset{p}{\rightleftarrows}}\
K_{\cA,S},
\qquad ip-\mathrm{id}=dh+hd,
\]
together with filtration convergence and ordered-to-symmetric
comparison.  Then
\[
\ChirHoch^n(\cA)\cong H^n(K_{\cA,S}),\qquad
\operatorname{Supp}\ChirHoch^\bullet(\cA)\subseteq S.
\]
A bar presentation additionally carries
\(\operatorname{Conv}(B_X(\cA),\cA)
\xrightarrow{\sim}C^\bullet_{\mathrm{ch}}(\cA,\cA)\).
\end{theorem}

The BDSK superboson bounded calculation has support \(\{0,1\}\) and
dimensions \((2,1)\).  The bounded Virasoro calculation has support
\(\{0,2,3\}\), with a one-dimensional group in each supported degree.
Its chart interpretation uses the declared bounded-to-chart
quasi-isomorphism.  A perfect degree-\(d\) pairing is a separate
duality package.  The chiral derived centre is the algebraic
closed-sector object; physical bulk and Volume~II identifications are
supplied by their respective comparison theorems.
...
ordered bar complex is therefore the level-$2$ object before averaging; the
...
\subsection*{9.8.\enspace The twelve-characterisation circuit}

The seven genuinely independent bidirectional equivalences among
the twelve items --- namely \textup{(i)--(v)} and
\textup{(ix)--(x)} --- together with \textup{(vi)}
(consequence of \textup{(v)}), \textup{(vii)} (conditional
factorization-homology comparison), and \textup{(viii)} (one-way
$H_H(\cA;S)$-conditional chiral Hochschild support consequence), the Lagrangian
criterion (xi), the $\cD$-module purity implication (xii), and the six
...
(v) connects the homological and homotopical hubs.
Each link is an independent theorem:
\begin{itemize}[nosep]
...
inversion refining its cohomological shadow).
\end{itemize}
Under the additional family datum \(H_H(\cA;S)\), Theorem~H gives the
one-way consequence
\(\operatorname{Supp}\ChirHoch^\bullet(\cA)\subseteq S\).
...
The rank-$1$ case is the strongest evidence lane, not a free
consequence of the quotient map. Finite Shapovalov/character
computations for $L_k(\mathfrak{sl}_2)$ are compatible with
Koszulness at admissible levels $k = -2 + p/q$, but a theorem still
requires the quotient bar spectral sequence, PBW/Shapovalov detection
of higher differentials, finite-window exactness, and strong
convergence. The surjection
$\barB(V_k) \twoheadrightarrow \barB(L_k)$ alone does not transfer
bar concentration.
...
$(\cZ^{\der}_{\ch}(\cA),\, \cA)$. Volume~II supplies the typed
open--closed comparison under which the derived chiral centre is the
algebraic closed sector whose image may be interpreted as the bulk
algebra of a real $3$d holomorphic-topological gauge theory. The
passage from algebraic geometry to physics is through that comparison,
not through the bare derived centre alone.
...
Three objects lie in distinct ambients:
\begin{enumerate}[label=\textup{(\roman*)},nosep]
...
$\cZ^{\der}_{\ch}(\cC) =
R\Hom_{\Fun(\cC,\cC)}(\Id, \Id)$ is the universal closed sector,
...
invariant; $A_b$ depends on $b$; $\barB$ records twisting data,
not closed-sector or physical-bulk data; $\Theta_\cA$ is the
closed-sector MC element;
...
are proved locally; the global extension is a descent statement. In
Stage~4 (modularisation), the open/closed MC
...
acting on the fibre above a topological interval; there is no operation
with topological input and holomorphic output. Operadic composition is the boundary stratification
...
R\Hom_{\Fun(\cC,\cC)}(\Id, \Id)$: the universal closed sector, Morita
...
\medskip\noindent\emph{Closed sector $=$ derived centre.} Let
...
$E_2$-algebra. The Swiss-cheese structure maps the algebraic
closed-sector actor to this $E_2$-algebra; identifying a holomorphic
physical bulk with it requires the open--closed comparison datum.
...
uniqueness. For reductive non-simple $\fg$, the formal-disk
$\Pthree$ base is $\Sym^2(\fg^*)^\fg$, not merely
$H^3(\fg)$; the $\mathfrak{gl}_N$ case is controlled by the
two independent forms $B_{\mathrm{tr}}$ and
$B_{\mathrm{ab}}$, while arbitrary reductive cases require
componentwise matching of all invariant bilinear forms.
Solvable cases are governed by the filtration criterion rather
than by a one-dimensional uniqueness argument.
...
the Kodaira--Spencer class; under a specified chart comparison,
Theorem~H is the strong-deformation-retract support theorem for the
Lagrangian Hochschild chart.
...
(scalar diagonal $\kappa\cdot\omega_g$, Arakelov, global).
\end{enumerate}
...
$\simeq C^\bullet_{\ch}(A_b, A_b)$ is the universal closed sector.
\textsc{Proved.}
...
from the bar resolution. Under a family datum \(H_H(\cA;S)\),
Theorem~H identifies its cohomology with the finite-support model
\(K_{\cA,S}\); the chiral Gerstenhaber bracket
...
$k\,\Omega_{\mathrm{tr}}/z$ at degree $2$ and the Lie bracket at degree $3$;
...
$\cH_k$, closed-sector descriptor $\cZ^{\der}_{\ch}(\cH_k)$, and
$R(z) = e^{k\hbar/z}$ (scalar, YBE trivial), with $\kappa = k$.
The physical bulk comparison is extra OCA data. Class~$\mathbf{G}$:
the scalar shadow collapses to the single value~$k$.

\smallskip
\noindent\emph{Non-abelian CS} ($SL_2$, level $k$): boundary
$\widehat{\mathfrak{sl}}_2{}_k$, line dual
$\Ydg(\mathfrak{sl}_2)$, $r(z) = k\,\Omega_{\mathrm{tr}}/z$ (vanishing at
...
\]
The PVA Jacobi identity gives the classical master equation for this
Poisson sigma model. Topologisation is an additional datum: the
conformal vector must be inner in the quantised BRST complex, with
$T=[Q_{\mathrm{tot}},G]$ and the analytic SDR hypotheses needed to
transport this identity.
...
The scope splits. Affine Kac--Moody algebras at non-critical level and
principal $\cW$-algebras are proved through the affine/DS
gauge-theoretic lanes. For finite-type freely generated finite-jet
PVAs with conformal vector and inner holomorphic translation, the
Khan--Zeng construction supplies the classical BV action; the all-loop
boundary quantization and the $E_3$-topological lift require the KZ
analytic SDR package and a quantized homotopy
$T=[Q_{\mathrm{tot}},G]$.
...
commutator with the curvature, not to zero. At genus $g$, the
fiberwise statement remains $m_1^2=[m_0^{(g)},-]$; the scalar diagonal
of $m_0^{(g)}$ is $\kappa \cdot \omega_g$. The
...
structure, and the $\SCchtop$ datum on the pair
$(\cZ^{\der}_{\ch}(\cA),\, \cA)$. The derived chiral centre is the
closed-colour algebraic object; its interpretation as the bulk of a
real $3$d holomorphic-topological gauge theory requires the typed
open--closed comparison datum of Section~10. The modular PVA
quantisation programme (Section~11) supplies the obstruction theory.
The holographic modular Koszul datum packages the algebraic
comparison surface into a seven-entry structure: boundary algebra,
presentation coalgebra, strict Koszul dual, derived closed sector,
spectral $R$-matrix, universal MC element, and modular shadow
connection. The line category is recovered on the line-comparison
surface attached to the strict dual, while the derived centre occupies
the closed-sector entry.  The full-bar Verdier algebra
$K_X(\cA)=\mathbb D_{\Ran}\bar B_X(\cA)$ is the typed comparison
object between the presentation coalgebra and the strict dual. Every
ingredient constructed in Sections~1--9 is a typed
projection of this datum.
...
\;=\;
(\cA,\; \cA^{\mathrm i},\; \cA^!,\;
 \cZ^{\der}_{\ch}(\cA),\; K_\cA^{\mathrm{coll}}(z),\;
 \Theta_\cA,\; \nabla^{\hol}),
\]
where:
...
$H^\bullet(q_\cA)$ identifies its cohomology with
$H^\bullet\barB_X(\cA)$.
\item Verdier duality forms
$K_X(\cA)=\mathbb D_{\Ran}\barB_X(\cA)$, and the chosen pair map
$\nu_\cA\colon K_X(\cA)\to\cA^!$ supplies the strict chiral Koszul
partner.  Its module category becomes the line-operator category under
the line-comparison theorem.
\item $\cZ^{\der}_{\ch}(\cA)$ is the derived chiral centre. The
$\SCchtop$ datum is the pair
$(\cZ^{\der}_{\ch}(\cA),\,\cA)$ of closed and open colours; the bar
complex itself is not the closed colour.
\item $K_\cA^{\mathrm{coll}}(z)=
\Res^{\coll}_{0,2}(\Theta_\cA^{E_1})\in
\End_\cA(2)\widehat\otimes\cO(*\Delta)$ is the binary genus-$0$
collision kernel. Under $H_{\mathrm{CYBE}}^{\mathrm{rep}}(\cA;W)$,
$r_\cA(z)=\rho_\cA(K_\cA^{\mathrm{coll}}(z))$ is the represented
classical $r$-matrix.
...
the Sugawara formula at $k = -h^\vee$), and the degree-$2$
ordered chiral homology $H^1_{\mathrm{ord}, 2}$ on $V \otimes V$
doubles from $4$ to $8$ dimensions for $\mathfrak{sl}_2$
(a finite computation conserving $\chi_{\mathrm{ord}, 2} = -4$).
The spectral sequence $E_1^{p,q}$ has $d_1 = \lambda \cdot [\delta]$
vanishing at $\lambda = 0$ (the critical specialisation); the
Feigin--Frenkel centre
$\mathfrak{z}(\widehat\fg_{-h^\vee}) \cong \Fun(\Op_{{}^L\fg}(D))$
emerges as $H^0$ of the FULL symmetric bar, and
$H^n(\barB^\Sigma(\widehat\fg_{-h^\vee})) \cong \Omega^n(\Op_{{}^L\fg}(D))$
for all $n \geq 0$ (infinite-dimensional; Frenkel--Teleman 2006).
...
\item \emph{Yangian-shadow.} On a locus carrying
$H_{\mathrm{CYBE}}^{\mathrm{rep}}$ and the Yangian comparison package,
the represented collision kernel supplies the dg-shifted Yangian
$r$-operator together with its higher $A_\infty$ homotopies.
...
\]
The PVA Jacobi identity gives the classical master equation. A
topological theory requires the inner relation
$T=[Q_{\mathrm{tot}},G]$ in the quantised BRST complex, together with
the analytic SDR and all-loop QME hypotheses; the modular full-genus
statement belongs to that conditional lane.
The boundary carries a $(-1)$-shifted Poisson bracket and generates the
genus-$0$ quantisation problem. The conditional modular lane couples the
bulk theory to stable curves at $g \ge 1$ after the all-loop QME and
image-choice/Stokes hypotheses are fixed. The pipeline:
\[
...
one-loop-exact half-space BV theorem. For finite-type nonlinear KZ
PVAs, the classical BV action is constructed, while the all-loop QME,
reflected weight identity, and boundary vertex algebra require the KZ
analytic SDR package with image-choice Stokes pushforward. The affine
Poisson--vertex sigma model satisfies the physics bridge hypotheses,
and Drinfeld--Sokolov reduction preserves the logarithmic
$\SCchtop$ shadow. The proved standard landscape enters through these
two gauge-theoretic gates.
...
Stable graphs, automorphism weights, and genus weights form the
...
\begin{align*}
\cA &= \cH_k, & \cA^! &= (\Sym^{\ch}(V^*[1]),m_0=-k\omega),
 & \cC &= \mathsf{Vect},\\
r(z) &= k/z, & R(z) &= \exp(k/z),
...
Yang's $r(z) = k\,\Omega_{\mathrm{tr}}/z$, line-operator category
...
\medskip\noindent\textbf{The seven entries in full.}\enspace
\begin{enumerate}[label=(\roman*),nosep]
...
\item The Virasoro presentation
$\Vir_c=T_X(V_c)/(R_c)$ supplies
$\cA^{\mathrm i}=C_X(s^{-1}V_c,s^{-2}R_c)$ and
$q_c\colon\cA^{\mathrm i}\to\barB_X(\Vir_c)$.  Its bar cohomology is
$H^\bullet\barB_X(\Vir_c)$, identified with
$H^\bullet(\cA^{\mathrm i})$ by $H^\bullet(q_c)$ on the Koszul locus.

\item $K_X(\Vir_c)=\mathbb D_{\Ran}\barB_X(\Vir_c)$ is the Verdier
algebra, and the Virasoro pair map
$\nu_c\colon K_X(\Vir_c)\to\Vir_{26-c}$ supplies
$\cA^!=\Vir_{26-c}$ on the companion lane. Koszul
...
\item $\cZ^{\der}_{\ch}(\Vir_c)$: the derived centre occupies the
closed-sector entry. The line-operator category is attached to the
preceding strict dual, for example by $\Vir_{26-c}$-modules on the
line-comparison surface.

\item $K_{\Vir_c}^{\mathrm{coll}}(z)=(c/2)/z^3+2T/z$: the ordered
collision kernel. A classical $r$-matrix is its represented image
...
tower $\{m_k\}_{k \ge 2}$ and the collision kernel
$K_\cA^{\mathrm{coll}}(z)$;
...
\begin{align*}
\cA &= \cA_{M2,\infty},
 & \cA^{\mathrm i} &= C_X(s^{-1}V_{M2},s^{-2}R_{M2}),\\
H^\bullet\barB_X(\cA)&=H^\bullet\barB_X(\cA_{M2,\infty}),
 &K_X(\cA)&=\mathbb D_{\Ran}\barB_X(\cA),\\
\cA^! &= Y^{\dg}_\hbar(\mathfrak{psl}(2|2)),
 & \cZ^{\der}_{\ch}(\cA) &= C^\bullet_{\ch}(\cA,\cA),\\
...
The modular Koszul datum $\cH(M2)$ packages the algebraic
comparison surface into a seven-entry MC-controlled tuple. The
physical holographic correspondence additionally requires the
open--closed and line-comparison maps.
...
is a conditional algebraic encoding, not a physical bulk theorem
without the comparison maps.
...
the algebraic open/closed comparison surface into a seven-entry
...
The holographic datum of Section~12 packages a chiral algebra, its
bar-cohomology coalgebra, its strict Koszul dual, its derived closed
sector, the spectral $R$-matrix, the universal MC element, and the
shadow connection into a single algebraic object. The line-operator
category is read on the separate line-comparison surface. The frontier consists of
...
BRST/bar comparison is conditional at
...
genus~$1$, the curved bar complex with scalar diagonal curvature
$\kappa\cdot\omega_1$ supplies the
...
Curvature forces this direction. At $g \ge 1$ the bar complex is
curved, with scalar diagonal $\kappa\cdot\omega_g$;
...
\item \emph{Boundary bar duality} (conjectural): genus-$0$
completed boundary modules are expected to be analytic comodules
over $B^{\mathrm{an}}(\cA_\partial)$; curved genus uses analytic
contramodules over the curved dual. This is not a physical-bulk
identification without the open--closed comparison datum.
\end{enumerate}
...
\end{align*}
Theorems~A--D occupy their respective reconstruction, recognition,
complementarity, and modular-trace levels; Theorem~H occupies the
derived-centre support level. The holographic datum
...
\item Theorem~D separates
$\operatorname{Obs}^{\mathrm{def}}_g\in H^2(\operatorname{Def}_g)$ for
$g\geq2$,
the $H_D^1$ trace $\kappa\lambda_1$, the $H_D^K$ object
$\mathfrak O_g^K=\kappa\lambda_{-1}(\mathbb E_g)$, its signed Hodge
character, and the $H_D^{\mathrm{graph}}$ functional
$F_g=\kappa\lambda_g^{\mathrm{FP}}+\delta F_g^{\cross}$
...
under the derived-centre map; genus creation in the cochain-level
...
of Sections~1--9 receives its geometric instantiation: a CY
category first gives a holomorphic factorisation algebra, and
specialisation along the auxiliary cycle produces the chiral
shadow. Thus $\Phi_d^{(\Sigma_{d-1},C)}$ is $E_2$-chiral in the CY$_2$ lane and
$E_1$-chiral in the CY$_3$ BPS/CoHA lane; the $E_2$ structure for
CY$_3$ appears on the Drinfeld/derived centre, not on the
positive Hall algebra itself. The resulting chiral shadows carry
...
$d$-Calabi--Yau structure), the two-stage functor
$\Phi_d^{(\Sigma_{d-1},C)}$
produces a chiral shadow $\Phi_d^{(\Sigma_{d-1},C)}(\cC)$ whose:
\begin{itemize}[nosep]
\item central charge is $\kappa_{\ch}(\Phi_d^{(\Sigma_{d-1},C)}(\cC))$, a function of
...
\item the shadow tower supplies the scalar bar-Euler face of the
automorphic correction of the associated BKM superalgebra, when
such a BKM denominator exists.
\end{itemize}
The CY$_2$ lane is the K3/abelian-surface $E_2$ lane. The
CY$_3$ examples below use the $E_1$ BPS/CoHA shadow supplied by
the 6d holomorphic Chern--Simons construction; higher centre
structures are separate.

\medskip\noindent\textbf{Example: $K3 \times E$.}\enspace
The product of a K3 surface and an elliptic curve is a CY$_3$,
but it does not carry a single scalar $\kappa$. The four
load-bearing values are
\[
\bigl\{
\kappa_{\mathrm{cat}}(K3\times E),\,
\kappa_{\ch}^{\mathrm{Heis}}(K3\times E),\,
\kappa_{\mathrm{BKM}}(\Delta_5),\,
\kappa_{\mathrm{fiber}}(K3)
\bigr\}
=\{0,3,5,24\}.
\]
Here $\kappa_{\mathrm{cat}}(K3\times E)=0$ by K\"unneth
multiplicativity for $\chi(\cO_X)$, the free-field chiral shadow
has $\kappa_{\ch}^{\mathrm{Heis}}=3$, and the primitive K3
Borcherds denominator is the Gritsenko--Nikulin form
$\Delta_5$ of weight~$5$, so
$\kappa_{\mathrm{BKM}}(\Delta_5)=5$. The number
$\chi(K3)=24$ is the fibre moonshine multiplier, not
$\kappa_{\ch}(K3\times E)$.

The unnormalised Igusa square is
$\Phi_{10}^{\mathrm{un}}=\Delta_5^2$. In the
Oberdieck--Pandharipande reduced-DT normalisation one uses the
monic Borcherds product
$D_5=64^{-1}\Delta_5$ and therefore
\[
\Phi_{10}^{\mathrm{OP}}=D_5^2=4096^{-1}\Delta_5^2,\qquad
Z_{\mathrm{OP/DT}}=-D_5^{-2}=-4096\,\Delta_5^{-2}.
\]
Thus the BKM algebra is $\fg_{\Delta_5}$ on the primitive
half-BPS denominator; the compact reduced-DT scalar uses the
normalised square. The shadow tower records local bar-Euler and
Taylor data of the automorphic correction. It does not by itself
produce the full Siegel form; the shadow--Siegel gap is precisely
the genus-$2$ information missing from the scalar tower.
...
$\mathrm{CoHA}(\bC^3)=Y^+(\widehat{\fgl}_1)$, the positive half
of the affine Yangian. The algebra $\cW_{1+\infty}$ enters only
after passing through the Drinfeld double, centre, Fock
realisation, or evaluation slice.
The shadow matches the scalar constant-map GW sector for $\bC^3$ under
the MNOP normalisation. A full Gromov--Witten partition function
interpretation requires the comparison datum identifying scalar shadows
with the physical path-integral sector.
...
Drinfeld-centre passage to categorical half-braidings, and higher
Deligne/topologisation (derived centre). The circle closes only on the
named comparison loci: one needs the Drinfeld-centre/derived-centre
identification and the topologization package, proved chain-level on
the affine Kac--Moody non-critical locus and open on the general
class-$M$ original complex. On the abelian CY surface locus, the
Drinfeld-centre comparison
$\cZ(\Rep^{E_1}(Y^+(\widehat{\fgl}_1)))
...
\subsection*{13.10.\enspace The deepest open problems}

Periodic-CDG compatibility holds for admissible Kazhdan--Lusztig,
the constructed iterated-Sugawara lanes and their homotopy-coherent
completion, the
chiral higher-Deligne theorem, the SC$^{\mathrm{ch,top}}$ heptagon,
the CY-D dimension stratification, and the Francis--Gaitsgory
$(\infty,2)$-equivalence at properad level. The remaining frontier is
the following list of residual problems.

\smallskip\noindent\textbf{Residual frontiers.}
\begin{enumerate}[nosep]
\item \emph{$W(p)$ triplet tempering}: the Gurarie~1993 and
 Flohr~1996 logarithmic-CFT amplitudes exhibit unbounded
 Massey products despite finite-dimensional Zhu algebra. The
 correct tempered scope is principal, non-logarithmic,
 non-minimal standard landscape; tempering of $W(p)$ is
 open pending an Adamovi\'c--Milas character-amplitude
 bound.
\item \emph{Chain-level class~M on the original complex}:
 the weight-completed category is proved (Milnor/
 Mittag-Leffler via the explicit SDR of $h\cdot m_0^{j-1}$);
 the direct-sum chain-level identification is genuinely
 false, since $S_4(\Vir_c) = 10/[c(5c + 22)] \neq 0$ at
 generic~$c$. Whether the original complex admits chain-level
 $E_3$ through a different homotopy transfer remains open.
\item \emph{Exact closed form of $\beta_N$ at $N \geq 4$}:
 $W_N$ tempering is unconditional via $\beta$-independent
 Stirling dominance. The two candidate closed forms
 $(N{+}1)(N{+}2)/2$ and $N^2 - N + 4$ diverge at $N = 4$ (15
 vs.~16). A Fateev--Lukyanov explicit structure-constant
 derivation for $\cW_4$ is the outstanding computation.
\item \emph{Super-complementarity canonical Verdier pairing}:
 the super-Yangian complementarity sum splits by normalisation.
 The super-trace form
 $\kappa^{\str} + \kappa^{\str,!} = 0$ is PROVED for
 $Y(\mathfrak{sl}(m|n))$ on the sub-Sugawara line $m\neq n$ via
 super-Sugawara rational-identity cancellation. The Berezinian
 form $\kappa^{\sBer} + \kappa^{\sBer,!} = \max(m,n)$ is
 CONJECTURED at Sugawara-shifted dual level, conditional on a
 closed-form derivation of the $\tfrac{1}{2}\max(m,n)$ shift
 magnitude induced by the central automorphism $\sigma_{\sBer}$
 (Nazarov~1991 and Gow~2006 supply centrality of $\sBer(T(u))$
 and non-degeneracy of the Berezinian pairing, not the
 shift magnitude in closed form). Two admissible pairings
 (super-trace and Berezinian) coexist without
 programme-level canonicalisation; the Verdier pairing
 inscription is pending.
\end{enumerate}

\smallskip\noindent\textbf{Structural frontier.}
\begin{enumerate}[nosep,start=5]
\item \emph{Closing the $E_n$ circle}
 (Conjecture~\ref{conj:v1-drinfeld-center-equals-bulk}):
 $\cZ(\mathbf U_\cA) \simeq \cZ^{\der}_{\ch}(\cA)$.
\item \emph{Drinfeld double at $E_1$-chiral level}: construct
 $\cA \bowtie \cA^!$ as a Hopf object and identify the
 antipode with orientation reversal.
\item \emph{A genuinely new $E_1$ invariant}: a number from
 $E_1$ theory that $E_\infty$ cannot compute.
\item \emph{Equivalence of five $E_1$-chiral notions}:
 Notions A--E potentially define different categories.
...
\smallskip\noindent\textbf{Closed since 2026-04-16.}\enspace
CY-A at $d = 3$ ($\infty$-categorical, $\mathrm{HH}^{-2}_{E_1} = 0$);
the periodic-CDG conjecture (FM251) for admissible KL; the
modular-family upgrade of Theorem~A over $\overline{\mathcal M}_{g,n}$;
topologization on the constructed affine and principal-Casimir
Sugawara lanes via the iterated $E_n$ ladder; the chiral coproduct for
non-gauge-theoretic families via the unified
$Q_g^{k,f,\mu}$ construction.
...
automorphic correction. At the toric point
$\mathrm{CoHA}(\bC^3)=Y^+(\widehat{\fgl}_1)$; the
$\cW_{1+\infty}$ algebra enters after
...
\subsection*{13.12.\enspace The organizing ambient problem}
...
examples from CY geometry. The two-stage functor
$\Phi_d^{(\Sigma_{d-1},C)}$
produces $E_2$-chiral shadows in the CY$_2$ lane and $E_1$-chiral
BPS/CoHA shadows in the CY$_3$ lane; centre constructions carry
the higher $E_2$ structure. The CY quantum groups
...
algebra supplies the scalar bar-Euler face of the BKM
automorphic correction; the primitive K3 denominator is
$\Delta_5$, with $\Phi_{10}^{\mathrm{un}}=\Delta_5^2$ only after
squaring. At $\kappa_{\mathrm{ch}} = \Psi$ the shadow produces
the constant-map Gromov--Witten free energies of $\bC^3$, while
$\mathrm{CoHA}(\bC^3)=Y^+(\widehat{\mathfrak{gl}}_1)$ and
$\cW_{1+\infty}$ appears after double/centre/Fock/evaluation.
The Drinfeld centre
...
physically by an open--closed comparison: the derived centre is the
algebraic closed sector whose OCA image is the physical bulk sector
of a 3d gauge theory; 3d quantum gravity is the climax. Volume~III
