...
shadow. Thus $\Phi_d$ is $E_2$-chiral in the CY$_2$ lane and
...
$d$-Calabi--Yau structure), the two-stage functor $\Phi_d$
produces a chiral shadow $\Phi_d(\cC)$ whose:
\begin{itemize}[nosep]
\item central charge is $\kappa_{\ch}(\Phi_d(\cC))$, a function of
...
iterated Sugawara and the $E_\infty$-topologization ladder, the
...
topologization for general chiral with conformal vector via the
iterated-Sugawara $E_\infty$ ladder; the chiral coproduct for
...
examples from CY geometry. The two-stage functor $\Phi_d$
