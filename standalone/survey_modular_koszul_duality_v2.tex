...
augmentation ideal (using $\bar\cA$ rather than $\cA$ is essential:
AP132), and $s^{-1}$ is desuspension with
...
Beilinson--Ginzburg--Soergel, Loday--Vallette) is recovered: the affine
line deformation-retracts to a point, the FM chain cooperad becomes
quasi-isomorphic to the associative cooperad, and the bar construction
collapses to the classical bar complex of the underlying Lie algebra.
...
twisting datum.  The $R$-matrix is the arity-$2$ collision residue of
...
Applying $\mathrm{av}$ at arity~$2$ collapses the profile to a single
scalar,
\begin{equation}\label{eq:av-organizing}
\mathrm{av}\bigl(r(z)\bigr)\;=\;\kappa(\cA),
\end{equation}
the modular characteristic of Theorem~D.  At arity~$3$ the KZ
associator projects to the cubic shadow~$\mathfrak C$; at arity~$4$
the Yangian higher coproduct projects to the quartic resonance
class~$\mathfrak Q$.  Each arity-$n$ component of the ordered
Maurer--Cartan element projects under $\mathrm{av}$ to the arity-$n$
...
$m_1^2(a)=[m_0,a]$ (AP22: bar $d^2=0$ always; curvature appears as
$m_1^2\neq 0$).  The curvature is the bar-dual of the OPE coefficient.
...
$k/(z-w)^2$ drops by one order under the $d\log$ kernel (AP19: pole
absorbs one order; AP27: bar propagator weight is always~$1$,
independent of field weight).  Applying \eqref{eq:av-organizing},
...
\]
(AP126: the level~$k$ survives the $d\log$ absorption; at $k=0$ the
$r$-matrix vanishes, as required).  Averaging collapses the Casimir
to its trace:
\[
...
is a full $g\times g$ matrix (AP118: write the period matrix in full
form, never collapse to a scalar even if $g=1$).  The price:
...
The bar propagator absorbs one pole order (AP19), so the collision
residue is $r(z)=(c/2)/z^3+2T/z$ (cubic, not quartic: AP19, AP21).
...
\emph{fails} (AP32): a cross-channel correction from mixed-propagator
...
$F_1=\kappa/24$ as sanity check (AP120: Cauchy normalization
$1/(2\pi i)$, not $1/(2\pi)$).
...
rule is family-dependent (never universal; AP24):
\[
...
theory.  \emph{Over a point}: $B(A)=(T^c(s^{-1}\bar A),d_B)$ determines
...
the complexity of Koszul algebras, not Koszulness itself (AP14).
...
$\delta F_g^{\mathrm{cross}}$, AP32), and Theorem~H (chiral
...
(completion tower): proved (AP131: generating depth, the arity at
...
$\delta F_g^{\mathrm{cross}}$ (AP32).  The Mumford relation
...
\end{tabular}
\end{center}
$H_N=\sum_{j=1}^N 1/j$ is the harmonic number (AP116 and AP136:
evaluate at smallest $N$; $H_N-1\neq H_{N-1}$).  The sum rule
...
\]
(AP129: verify the ratio by substituting a known value.  At $c=1$,
$Q_{\mathrm{Vir}}=10/27$, not its reciprocal.)  For Heisenberg the
...
escapes via stratum separation.  No classical invariant of an algebra
over a point sees $\Delta$: it is visible only on curves of positive
genus, where the Arnold relation acquires a defect.

Classical Koszul duality (Priddy, Beilinson--Ginzburg--Soergel)
occupies genus~$0$, arity~$2$, $\Delta=0$: the formal, class-G case.
...
must (AP126).  The CYBE is the Arnold relation on $\FM_3(\bC)$
...
= -(1/6\pi)\int(|\nabla\sigma|^2 + R\sigma)$ is the arity-$2$
...
coefficient $\kappa/(6\pi)$.  The higher-arity corrections
...
(UNIFORM-WEIGHT formula fails; ALL-WEIGHT
requires cross-channel term).
...
The contact archetype.  First nonlinearity at arity $4$ (not $3$);
...
$K_{\mathrm{BP}}=196$ (AP140: this is a global duality invariant, not
...
invariant (AP94).  For $\mathfrak g=\mathfrak{sl}_2$ (Virasoro):
...
$\kappa\cdot\lambda_g^{\mathrm{FP}}$ by genus~$4$ (AP32: multi-weight
...
and mixed ($\dim=(k-1)!\binom{k+m}{m}$, AP89).  The mixed sector
...
Base case: shadow class $\mathbf G$, depth $2$, trivial
...
First nontrivial example: class $\mathbf L$, depth $3$,
