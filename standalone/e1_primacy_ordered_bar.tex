...
D, together with the Hochschild theorem~H) lift to the ordered side,
and each symmetric theorem is the $\Sigma_n$-coinvariant image of its
ordered counterpart. The information lost under $\av$ is geometric,
...
$\Sym^c$ by external $\Sigma_n$-coinvariants; it does not factor
...
under external $\Sigma_n$-coinvariants; this is the
Getzler--Kapranov comparison between ribbon and commutative Feynman
transforms~\cite{GK98}. Compatibility with the Lie bracket:
...
under $\Sigma_n$-coinvariants, and $\av(d_0) = d_0$.
Therefore
\[
  \av(\Theta^{\Eone}_\cA)
  = \av(D^{\Eone}_\cA - d_0)
...
the genus-$g$ ordered coHochschild complex of the ordered bar
...
$(s^{-1}\bar\cA)^{\otimes n}$ and taking $\Sigma_n$-coinvariants
...
\item \textup{(Spectral coproduct.)}
  ...
  on the ordered Koszul dual Yangian, satisfying the spectral
...
At genus~$0$: the $S$-matrix unitarity
$\sum_l S_{jl}^2 = \delta_{j0}$ gives $Z_0 = 1$.
