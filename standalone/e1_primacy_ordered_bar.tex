...
This paper establishes $\Eone$ primacy at level~$2$: the ordered bar
is the chain object before symmetrisation; the symmetric bar is
derived by averaging; and the five theorems are the invariants that
survive the quotient. The level-$0$ primitive remains the open
factorization category with its chosen vacuum.
...
$\Barord(\cA) = T^c(s^{-1}\bar\cA)$ is the level-$2$ chain object
before symmetrisation in modular Koszul duality, and that the symmetric bar
...
second is representation-theoretic: the degree-$2$ genus-$0$ ordered
Maurer--Cartan element contains $K_\cA^{\mathrm{coll}}$. Its homotopy
coinvariant remains operator-valued, while
$\tau_{\cA,2}^{\mathrm{res}}q_2(K_\cA^{\mathrm{coll}})$ gives
$\kappa$ for Heisenberg and
$\kappa_{\mathrm{dp}}=k\dim(\fg)/(2h^\vee)$ for non-abelian affine
Kac--Moody, with the full
...
\item The \emph{averaging map} $\av_R\colon \gEone
  \twoheadrightarrow \gmod$ as a surjective dg~Lie morphism under the
  strong-unitary descent and Reynolds-kernel package
...
\emph{averaging map} is the chain projection
...
\to \cM_\Ass(g,n)$
is a chosen finite-window chain section of the ribbon-forgetting
comparison from Theorem~\ref{thm:fass-to-fcom}. The result is
$\Sigma_n$-invariant by construction. Changing the section changes
the representative by a boundary only on the fixed comparison window;
without that window and homotopy the displayed average is merely a
linear projection. This is the Reynolds operator for the external
$\Sigma_n$-action: it takes external coinvariants after pullback
along~$\iota^{\mathrm{rib}}$.
When the completed Reynolds kernel satisfies the bracket-ideal
condition $[\ker(\av_R),\ker(\av_R)]\subseteq\ker(\av_R)$, this chain
projection is a dg~Lie morphism.
\end{definition}
...
convolution presentation, Beilinson levels~$2$ and~$5$, hypothesis
package: finite-window chain section of the ribbon-forgetting
comparison, strong-unitary \(R\)-twisted descent, completed
Reynolds-kernel bracket ideal, and the coefficient-multiplication
package of the universal conductor\textup{)}.
Under the fixed strong-unitary \(R\)-twisted descent datum and the
completed Reynolds-kernel bracket-ideal condition, the averaging map
$\av_R\colon \gEone \twoheadrightarrow \gmod$
is a surjective morphism of dg~Lie algebras. Without this descent
datum, section/homotopy datum, coefficient multiplication, and kernel
condition there is only the ordered dg Lie algebra together with, at
most, a chain projection to coinvariants; no dg~Lie morphism to
\(\gmod\) is asserted.
\end{theorem}

\begin{proof}
Surjectivity is a finite-window statement. On the chosen comparison
window, the chain section
\(\iota^{\mathrm{rib}}_{g,n}\) lifts a symmetric
\(\Sigma_n\)-invariant homomorphism from \(\cM_\Com(g,n)\) to a
ribbon representative in \(\cM_\Ass(g,n)\), and the Reynolds average
returns the original invariant class. A different section gives the
same target class only after the stated chain homotopy.
...
compatibility. Compatibility with the Lie bracket: equivariance of
operad composition in $\FMAss$ gives the descended bracket on symmetric
invariants after the conductor's coefficient-multiplication package is
supplied, and the Reynolds-kernel bracket-ideal condition is exactly
what prevents a bracket of two killed ordered components from
reappearing in the symmetric quotient. This is the finite-window
criterion of Theorem~\ref{thm:uc-r-twisted-dg-lie-descent}, written in
the \(E_1\)-primacy notation.
\end{proof}
...
\item Heisenberg $\cH_k$:
  $K_{\cH_k}^{\mathrm{coll}}(z)=k\Omega_{\mathcal H}/z$,
  $\kappa(\cH_k) = k$.

\item Affine Kac--Moody $\widehat{\fg}_k$:
  $K_{k,\fg}^{\mathrm{coll}}(z)=k\Omega_\fg/z$,
...
Etingof--Kazhdan quantisation theorem~\cite{EK96} after the source
Lie bialgebra, \(\hbar\)-adic topology, associator, and
QUE/quasi-Hopf target are fixed.
\end{proof}
...
\item \textup{(Surjectivity.)}
  ...
  \(R\)-twisted descent datum and the completed Reynolds-kernel
  bracket-ideal condition have been fixed.
...
\item \textup{(Bracket preservation.)}
  $\av_R$ preserves the dg~Lie bracket by the combination of
  $\Sigma_n$-equivariance of the operad composition maps and the
...
Equivariance of operad composition in $\FMAss$ gives the chain-level
coinvariant descent; the Reynolds-kernel bracket-ideal condition is the
extra hypothesis that makes $\av_R$ a dg~Lie morphism. Together they
convert $T^c$-convolution to $\Sym^c$-convolution
...
restatement. The $\Eone$/ordered structure is the level-$2$ chain
surface before averaging. The $\Einf$/symmetric/modular structure is its
...
analogue of the Voronov Swiss-cheese Deligne
conjecture~\cite{Voronov99} and the Kontsevich--Soibelman Deligne
conjecture~\cite{KS00}; the Swiss-cheese operad itself is not
formal (Livernet), and the argument routes around formality. Concretely, $\Phi(b)$ is the chiral
Hochschild cochain
\[
...
$\tau_{\cA,2}^{\mathrm{res}}q_2(r_0(z))=\kappa(\cA)$ on abelian families, while for
non-abelian affine Kac--Moody
$\tau_{\cA,2}^{\mathrm{res}}q_2(r_0(z))=\kappa_{\mathrm{dp}}(\cA)$ and
$\kappa(\cA) = \kappa_{\mathrm{dp}}(\cA) + \dim(\fg)/2$.
The coefficientwise normalized trace
$\tau_{\cA,2}^{\mathrm{res}}q_2(R^{\Eone,\mathrm{bin}})
=\sum \hbar^{2g}F_g^{(2)}$
...
identity $\tau_{\cA,2}^{\mathrm{res}}q_2(K_\cA^{\mathrm{coll}})
=\kappa$ holds for Heisenberg. On non-abelian affine Kac--Moody the
trace is $\kappa_{\mathrm{dp}}$ and the Sugawara term
$\dim(\fg)/2$ completes $\kappa$. The full operator
$q_2(K_\cA^{\mathrm{coll}})$ carries information about the braiding, the
...
\begin{itemize}
\item $K_{\cH_k}^{\mathrm{coll}}(z)=k\Omega_{\mathcal H}/z$
  (equation~\eqref{eq:heis-r-matrix}).
\item $\tau_{\cH_k,2}^{\mathrm{res}}q_2(K_{\cH_k}^{\mathrm{coll}})
  =\kappa(\cH_k)=k$
...
\begin{itemize}
\item $K_{k,\fg}^{\mathrm{coll}}(z)=k\Omega_\fg/z$
  (equation~\eqref{eq:km-r-matrix}; trace-form convention).

\item $\tau_{k,\fg}^{\mathrm{res}}q_2(K_{k,\fg}^{\mathrm{coll}})
  = \kappa_{\mathrm{dp}}
...
Verlinde formula through the ordered/TUY comparison lane.
...
$\Phi$. For $\mathfrak{sl}_2$, the cohomological bar-shadow is
associator-independent: the $\GRT_1$ action shifts the degree-$3$
obstruction class by coboundaries. For general $\fg$ this is a
comparison hypothesis, not a theorem forced by the existence of the
associator torsor. Bar-side invariants computed without the
coproduct, such as $\kappa$ and the finite shadow tower, are
associator-free; cobar-side and coproduct-side data depend on~$\Phi$.
No categorical modular datum or Drinfeld-centre object is claimed to
be $\GRT_1$-trivial from this argument. See~\cite[Chapter~9]{Lor26}
for the full proof in the verified cases.
\end{proof}
...
$V_k(\mathfrak{sl}_2)$ on a genus-$g$ curve reaches the
TUY/Hitchin conformal-block sheaf only through the ordered-to-TUY
comparison morphism.  After integrable truncation, boundary sewing,
projectively flat connection, and determinant-anomaly matching are
supplied, that target has the Verlinde rank.
...
level~$k$.
\begin{enumerate}[label=\textup{(\roman*)}]
\item \textup{(Genus~$0$.)} Under the ordered-chain-to-TUY
  comparison, the target is the unique vacuum:
...
\item \textup{(Genus~$1$.)} Under the same comparison, the target
  rank is the number of integrable representations:
...
\item \textup{(Handle attachment.)}
  TUY non-separating factorisation, compared with ordered sewing
  under the stated package, gives the TQFT handle-attachment formula:
  each
...
\begin{proof}
On $\Sigma_g$, the ordered chain model attached to
$V_k(\mathfrak{sl}_2)$ supplies the KZ local-system side of the comparison in
Section~\ref{sec:heis-km}. At integer level, the integrable quotient
of the root-of-unity \(U_q(\mathfrak{sl}_2)\) theory has
$k + 1$ simple channels $V_0, \ldots, V_k$.  The TUY/Hitchin target is
finite rank, and ordered sewing agrees with TUY sewing only under the
comparison package stated above.
...
$g - 1$ times from $Z_1 = k + 1$, verifying TUY factorisation and the
ordered sewing comparison under the separating and non-separating
sewing maps.
\end{proof}
...
\item \emph{Genuinely $\Eone$-chiral (Yangians, EK quantum
  ...
  analogue of the Etingof--Kazhdan quantisation theorem only after
  the source Lie bialgebra, \(\hbar\)-adic topology, associator, and
  QUE/quasi-Hopf target are fixed~\cite{EK96, EK00}.
\end{enumerate}
...
$\kappa(\cA)$ while the formal conductor fibre records the
non-coinvariant part of the $R$-matrix, the KZ associator, and every
braid-group representation that makes the $\Eone$ theory strictly
richer. The averaging map $\av_R$ is the typed information-loss
descent in the passage from the $\Eone$ primitive to the
