...
\begin{abstract}
Modular Koszul duality for chiral algebras is $E_1$--$E_1$ operadic
Koszul duality transported into the homotopical modular chiral realm
on algebraic curves. This first section of the programme summary
establishes the bar construction on an algebraic curve: the
logarithmic propagator $\eta_{12} = d\log(z_1 - z_2)$, the Arnold
relation on three points, and the ordered bar complex
$\barB^{\mathrm{ord}}(\cA) = T^c(s^{-1}\bar\cA)$ with deconcatenation
coproduct, together with the proof that $d^2 = 0$ at genus zero
through cancellation of Arnold with the Borcherds identity. The
symmetric bar $\barB^{\Sigma}(\cA)$ appears as the $\Sigma_n$-coinvariant
shadow obtained by the lossy averaging map; the ordered bar is the
primitive object and all five theorems of the programme extract
$\Sigma_n$-invariant content from it.
\end{abstract}
...
$\Sigma_n$-coinvariant image.  These six objects occupy distinct typed
ambients, and every identification among them is represented by a
comparison morphism.
\end{remark}
...
Three constructions applied to~$B(\cA)$ produce three typed outputs:
...
\mathbb{D}_{\Ran}(B(\cA))
 &=& K_X(\cA)
 & \text{(Verdier duality: the chain-level Koszul algebra)},
 \\[3pt]
\cA^{\mathrm i}=C_X(s^{-1}V,s^{-2}R)
 &\xrightarrow{\ q_\cA\ }&B_X(\cA)
 & \text{(quadratic presentation comparison)}.
\end{array}
\end{equation}
Cobar inversion is the round-trip to the original algebra~$\cA$.
Verdier duality on $\Ran(X)$ transposes the bar coproduct directly to
the multiplication of the chain-level algebra $K_X(\cA)$.  Bar
cohomology is the separate graded coalgebra $H^\bullet B_X(\cA)$.
When $q_\cA$ is a quasi-isomorphism and local finite duality applies,
\[
\mathbb D(q_\cA)\colon K_X(\cA)
\xrightarrow{\sim}\mathbb D_{\Ran}(\cA^{\mathrm i}),
\qquad
\nu_\cA\colon K_X(\cA)\xrightarrow{\sim}\cA^!
\]
give the presentation-dual and chosen-partner realizations.  Thus
\eqref{eq:three-functors} separates reconstruction, chain-level
duality, and strict quadratic duality by source and target.
