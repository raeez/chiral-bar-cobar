...
\title{The primary-state Virasoro Ward factor:\\
closed form and limits of the scalar quotient}
...
\begin{abstract}
The highest-weight-line quotient of the Virasoro collision-residue
connection has scalar Ward factor
$R^{\mathrm{prim}}_{h,c}(z)=z^{2h}\exp(-c/(4z^2))$.  After the
leading factor $z^{2h}$ is removed, only even powers of $1/z$ appear.
This note records the scalar primary-state computation and its
normalisation.  It is not a construction of the full completed
Virasoro spectral $R$-operator, and the scalar coefficient
$R_2=-c/4$ is not by itself a proof of class~$M$ non-formality.
\end{abstract}
...
binary genus-$0$ ordered collision kernel
...
$q_2(K_\cA^{\mathrm{coll}})$ remains operator-valued, and the modular
characteristic is
$\kappa(\cA)=\tau_{\cA,2}^{\mathrm{res}}
q_2(K_\cA^{\mathrm{coll}})$. A spectral $r$-matrix requires a
representation $\rho_\cA$ and
$H_{\mathrm{CYBE}}^{\mathrm{rep}}(\cA;W)$.

The collision kernel encodes the leading
...
scalar primary-state Ward factor, which is a function of the spectral
parameter~$z$.

For the Virasoro algebra $\Vir_c$, this primary-state quotient closes
into an elementary function. This note derives the closed form and
expands it in series. The full class~$M$ statement belongs to the
completed Virasoro obstruction tower, not to this scalar quotient
alone.
...
\begin{theorem}[Primary-state Virasoro Ward factor]
\label{thm:vir-R}
...
conformal weight $h$, the scalar primary-state Ward factor is
...
S_3(\Vir_c) = 2
\end{equation}
for $c\neq0$ in the BPZ-normalized vacuum shadow.
\end{theorem}
...
The identity $S_3(\Vir_c)=2$ supplies the nonzero cubic seed.
Shadow class~$M$ follows after the residue comparison and the
infinite weighted recurrence are supplied. The critical discriminant
$\Delta_{\Vir} = 8\kappa S_4 = 40/(5c+22) \neq 0$ for generic $c$,
and the single-line dichotomy forces $r_{\max} = \infty$.
\end{corollary}

% ================================================================
\section{What the scalar factor does not prove}
\label{sec:non-formality}
% ================================================================

The scalar factor
$R^{\mathrm{prim}}_{h,c}(z)=z^{2h}\exp(-c/(4z^2))$ records the
highest-weight-line projection of the Virasoro collision residue. The
first correction beyond the leading power is
\begin{equation}\label{eq:R2}
R_2 = -\frac{c}{4},
\end{equation}
which is forced by the central collision coefficient \(c/2\). This is
a necessary normalisation check, not a sufficient non-formality
theorem.  The class~$M$ statement requires the completed Virasoro
obstruction tower and the survival of the transferred wheel classes;
it cannot be inferred from the scalar primary quotient alone.
...
The primary scalar factors provide a quick normalisation comparison
between the three main families; they are not the full line-operator
comparison.
...
\begin{example}[Virasoro: primary scalar factor]\label{ex:virasoro}
The scalar factor
$R^{\mathrm{prim}}_{h,c}(z)=z^{2h}\exp(-c/(4z^2))$ has infinitely
many nonzero coefficients. This reflects the irregular scalar
highest-weight connection. The non-termination of the full Virasoro
shadow tower is a separate completed-bar statement.
\end{example}
...
\begin{tabular}{@{}llccc@{}}
\toprule
\textbf{Family} & scalar factor & $S_3$ & $r_{\max}$ & \textbf{Class}\\
\midrule
...
% ================================================================
\section{Level-prefix check: $k=0$ and convention bridge}
\label{sec:level-prefix-check-virr}
% ================================================================
...
\]
carries the central charge only in the cubic central pole. At $c=0$
the cubic pole vanishes, but the simple pole $2T/z$ remains in
$\Vir_0$ unless one further passes to the trivial quotient or to a
vacuum scalar projection where $T$ acts by zero. Thus $c=0$ is a
degenerate Virasoro point, not a trace-form vanishing check.

The \emph{cubic + simple} pole profile is the correct one:
$d\log$-absorption sends the fourth-order OPE pole $c/2
\cdot (z-w)^{-4}$ to the third-order $r$-matrix pole
$(c/2)/z^3$, and the second-order OPE pole $2T \cdot (z-w)^{-2}$ to
the simple $r$-matrix pole $2T/z$. The simple OPE pole
$\partial T/(z-w)$ is absorbed entirely and does not contribute.
The common mistakes $r^{\Vir}(z) = (c/2)/z^4$ (fourth-order,
un-absorbed) and $r^{\Vir}(z) = (c/2)/z^2$ (double pole,
over-absorbed) are both excluded by the pole-order calculation.

\begin{remark}[Trace-form vs $\lambda$-bracket convention
bridge]\label{rem:convention-bridge-virr}
Two conventions for the Virasoro $r$-matrix coexist in the
framework:
\begin{enumerate}[label=\textup{(\roman*)}]
 \item Trace-form (Vol~I): $r^{\Vir}(z) = (c/2)/z^3 + 2T/z$.
 \item $\lambda$-bracket (Vol~II): $\{T_\lambda T\} = (\lambda^3/6)\cdot c +
 2\lambda T + T'$, equivalent to writing the cubic coefficient as
 $c/12$ in divided-power convention.
\end{enumerate}
The bridge is $S^{\mathrm{Vol\,I}}_2 = \kappa^{\Vir} = c/2 = 6
S^{\lambda\text{-bracket}}_2$: the $\lambda$-bracket
$c/12$ coefficient is related to the Vol~I $c/2$ shadow invariant by
the divided-power factor $3! = 6$. Bare $S_2 = c/12$ is the
$\lambda$-bracket coefficient; the shadow invariant in Vol~I
notation is $S_2 = c/2$.
\end{remark}

\begin{remark}[Connection to $\kappa$ and the shadow
tower]\label{rem:vir-to-kappa}
The simple-pole coefficient $2T$ of $r^{\Vir}(z)$ averages (under
$\mathrm{av}$) to $2 \cdot \kappa(\Vir_c) = c$ on the primary vacuum
(since $\kappa^{\Vir} = c/2$, we have $2\kappa = c$). The cubic-pole
coefficient $c/2$ is already a scalar: $\mathrm{av}(c/2) = c/2 =
\kappa(\Vir_c)$. Both projections recover the modular
characteristic, confirming the $E_1$ $\to$ shadow consistency:
the cubic and simple poles project to the \emph{same} $\kappa$ scalar
up to the factor $2$ coming from the linear-in-$T$ structure of the
simple pole.
\end{remark}
