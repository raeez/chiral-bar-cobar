...
% ================================================================
\section{HZ-1 audit: level prefix, $k=0$ check, convention bridge}
\label{sec:hz1-audit-virr}
% ================================================================
...
\]
carries the central charge $c$ as explicit level prefix. The HZ-1
audit is: substitute $c = 0$. Both terms vanish: the $c/2$ central
term and the $T$ insertion (with $T \equiv 0$ on the $c=0$
triviality). The $c=0$ check passes: $r^{\Vir}(z)|_{c=0} = 0$, as
required for any trace-form $r$-matrix.

The \emph{cubic + simple} pole profile is the correct one (AP19,
AP21): $d\log$-absorption sends the fourth-order OPE pole $c/2
...
The common mistakes $r^{\Vir}(z) = (c/2)/z^4$ (AP19, fourth-order,
un-absorbed) and $r^{\Vir}(z) = (c/2)/z^2$ (AP27, double pole,
over-absorbed) are both excluded by this audit.
...
S^{\lambda\text{-bracket}}_2$ (AP180): the $\lambda$-bracket
...
(AP177: $\kappa^{\Vir} = c/2$, so $2\kappa = c$). The cubic-pole
