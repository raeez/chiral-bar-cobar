...
\end{equation}
is the spectral $r$-matrix of $\cA$.  It encodes the leading
...
\label{comp:vir-R}
For $\Vir_c$ acting on a primary state $|h\rangle$ of conformal
weight $h$, the spectral $R$-matrix is
...
\begin{proof}
The cubic shadow coefficient is the ratio of the linear
$T$-mode contribution to the constant central-term contribution
in the depth-$2$ collision residue:
\begin{equation}\label{eq:s3-def}
S_3 = \frac{(T_{(1)}T)_{\mathrm{lin}}}{(T_{(3)}T)_{\mathrm{const}}}.
\end{equation}
From the OPE~\eqref{eq:vir-ope}, the relevant modes are
$T_{(3)}T = c/2$ (the central term) and $T_{(1)}T = 2T$ (the
linear term).  The shadow coefficient is
\[
S_3 = \frac{2\kappa(\Vir_c)}{\kappa(\Vir_c)} = 2,
\]
where $\kappa(\Vir_c) = c/2$ appears in both numerator and
denominator and cancels identically.
\end{proof}
...
primary line.  Then $\cA$ is $A_\infty$-formal (all higher
products $m_n = 0$ for $n \geq 3$) if and only if $R_2 = 0$.
\end{proposition}

\begin{proof}
The coefficient $R_2$ is the leading non-trivial shadow coefficient:
$R_2 = -\kappa S_3/(2\kappa) \cdot \kappa/2 = -c/4$ for Virasoro.
The vanishing of $R_2$ is equivalent to $S_3 = 0$, which by the
shadow-formality identification is equivalent to the vanishing of
all transferred $A_\infty$ operations at arity $3$ on the bar
cohomology.  By the shadow obstruction tower recurrence, $S_3 = 0$
and the resulting vanishing of the critical discriminant force all
higher shadow coefficients $S_r = 0$ for $r \geq 3$, which is
precisely $A_\infty$-formality.
\end{proof}
...
The tower terminates at arity $2$: class $G$.
\end{example}
...
$f^{ab}_c$), so the tower extends to arity $3$; but the quartic
...
tower captures the full $R$-matrix: every arity contributes, and
