...
& quadratic coalgebra and comparison with the full bar
...
& Verdier algebra of the symmetric bar and chosen-pair comparison
...
& binary genus-$0$ collision kernel; its represented image is a
...
& scalar modular characteristic on abelian and scalar lanes; on the
affine non-abelian lane this is $\kappa_{\mathrm{dp}}$ and the
...
$Z^{\mathrm{der}}_{\mathrm{ch}}(\cA) = \operatorname{ChirHoch}^*(\cA,\cA)$
& derived chiral center; the universal closed-sector object computed by chiral Hochschild cochains on the curve
& \path{main.tex:787} \\
...
& Theorem~D perfect object under $H_D^K$; its top Hodge character is
...
& Vol III subscripted modular characteristics. $\kappa_{\mathrm{ch}}$ records the stage-$2$ chiral shadow $\mathrm{Sp}^{\mathrm{ch}}_{\Sigma_{d-1}, C}(\Phi^{\mathrm{FA}}_d(\cC))$ of the CY-to-chiral two-stage factorisation, $\kappa_{\mathrm{cat}}$ the categorified Euler trace, $\kappa_{\mathrm{BKM}}$ the Borcherds weight $c_N(0)/2$, and $\kappa_{\mathrm{fiber}}$ the lattice/fiber correction
& \path{Vol III cy_d_kappa_stratification.tex} \\
...
$\mathrm{THH},\ \mathrm{HH}^\bullet_{\mathrm{alg}},\ \ChirHoch^*,\ \mathrm{HH}^*_{\mathrm{cat}}$
& the Hochschild crosswalk: spectral topological Hochschild homology $\mathrm{THH}$ for $E_1$-ring spectra, algebraic Hochschild cochains $\mathrm{HH}^\bullet_{\mathrm{alg}}$ for dg algebras in chain complexes, chiral Hochschild $\ChirHoch^*$ for $E_\infty$-chiral input on a curve (Theorem~H), and categorical Hochschild for dg categories. The geometry determines which
...
& ordered bar (level-$2$ $E_1$-coalgebra with deconcatenation, carrying $K_A^{\mathrm{coll}}$ and its represented $R$-operators) versus symmetric bar (homotopy-coinvariant operator classes); normalized functionals $\tau_{A,n}^{\mathrm{res}}$ produce the scalar coordinates $\kappa,S_3,S_4,\ldots$
& \path{chapters/theory/e1_primacy_ordered_bar.tex} \\

$\mathrm{av}\colon \mathfrak{g}^{E_1}_\cA \twoheadrightarrow \mathfrak{g}^{\mathrm{mod}}_\cA$
& $\Sigma_n$-homotopy-coinvariant projection from ordered to equivariant modular operator data. At degree~$2$, $\tau_{A,2}^{\mathrm{res}}q_2(K_A^{\mathrm{coll}})=\kappa_{\mathrm{dp}}$ on non-abelian affine Kac--Moody; the full $\kappa$ adds the Sugawara contribution $\dim(\fg)/2$
& \path{chapters/theory/introduction.tex:1245} \\
...
\texttt{q\_convention\_bridge.md}); Vol~III deploys
the CY-to-chiral two-stage factorisation
...
locality), stage~$2$ factorisation homology along a closed
$(d-1)$-cycle $\Sigma_{d-1}$ followed by restriction to a reference
curve $C$ --- and the universal trace identity
