...
(cohomological, with model-level $(\Ethree)_\infty$). The $\En$ operadic circle
...
$f \in \fg$ any nilpotent element, and $k \neq -h^\vee$.
...
by~$f$. The $3$d bulk is independent of~$f$.
...
$T = [Q, G]$. When the chiral algebra carries a \emph{tower} of
inner higher-spin Casimir fields $\{T^{(n)}\}_{2 \leq n \leq
N+1}$, each admitting its own BRST primitive, the topologisation
iterates: a depth-$k$ inner stress tower promotes $\SCchtop$ to
$E_{k+2}^{\mathrm{top}}$. The principal $\cW_N$-algebra
$\cW_N$ realises exactly a depth-$(N-1)$ tower, and the limit
$\cW_\infty$ realises $\Einf$-topologisation.
...
them\textup{)}. Suppose each $T^{(n)}$ admits a BRST primitive
$G^{(n)} \in \cA$ with
$T^{(n)} = [Q_{\mathrm{tot}}, G^{(n)}]$ on the BRST cohomology
of the $3$d holomorphic--topological theory associated with
$\cA$. Then the depth-$k$ sub-tower $\{T^{(2)}, \ldots,
...
$\infty$\textup{)}: $\Einf^{\mathrm{top}}$.
\end{corollary}

\begin{remark}[The operadic spiral]
\label{rem:operadic-spiral-sa}
The iterated-Sugawara ladder and the $\En$ operadic circle
\eqref{eq:e-circle} together realise an infinite bidirectional
spiral. The bar construction descends the operadic level by
one; the derived centre ascends it by one. The iterated
Sugawara collapses any number of holomorphic directions into
topological ones. Meeting at $\Einf$, the spiral records that
$3$d quantum gravity in Vol~II is the $N = 2$ shadow of a
$3d + \infty$ topological theory.
\end{remark}
...
The operadic circle closes through the \emph{chiral higher
Deligne theorem}: the derived chiral centre
$\cZ^{\mathrm{der}}_{\mathrm{ch}}(\cA)$ is the universal
$\Ethree$-chiral algebra on which the Swiss-cheese operad
$\SCchtop$ acts.
...
The derived chiral centre $\cZ^{\mathrm{der}}_{\mathrm{ch}}(\cA)
= \mathrm{ChirHoch}^\bullet(\cA, \cA)$ is the universal
$\Ethree$-chiral algebra acted on by $\SCchtop$ through the
heptagon edges $(3) \leftrightarrow (4) \leftrightarrow (5)$ of
the five-presentation pentagon of $\SCchtop$-algebras. The
concentration theorem for $\mathrm{ChirHoch}^\bullet(\cA)$ in
degrees $\{0,1,2\}$ is a consequence of $\Ethree$-rigidity at a
point together with PBW collapse.
\end{theorem}
...
Composing Steps~1--3:
\[
Z(H_k\text{-mod}^{\mathrm{compl}})
\;\simeq\;
Z(C\text{-comod}^{\mathrm{conil}})
\;\simeq\;
\mathrm{Coext}_{C^e}^*(C_\Delta, C_\Delta)
\;\simeq\;
\ChirHoch^*(H_k, H_k)
...
$\En$ operadic circle closes: there is an equivalence of
$\Ethree^{\mathrm{top}}$-algebras
...
cohomology to vanish). The Drinfeld centre
$Z(U_{H_k})$ also has trivial $\Pthree$ bracket
...
\begin{lemma}[Stalk-wise reduction for class L]
\label{lem:e-km-stalkwise}
Let $V_k(\fg)$ be the universal affine vertex algebra at
non-critical level $k \neq -h^\vee$, with $\fg$ simple. The
Drinfeld centre $Z(V_k(\fg)\textup{-mod})$ is computed by
the stalk-wise centre at each point of $\Ran(X)$: no
$\Ainf$ correction at degree $\ge 4$ contributes.
\end{lemma}
...
\begin{theorem}[Closing conjecture for class L]
\label{thm:e-closing-class-l}
For $V_k(\fg)$ with $\fg$ simple and $k \neq -h^\vee$,
the $\En$ operadic circle closes: there is an equivalence
...
$Z(V_k(\fg)\text{-mod})$ with the chiral Hochschild
...
on $Z(U_{V_k(\fg)})$ is determined by the braiding of
