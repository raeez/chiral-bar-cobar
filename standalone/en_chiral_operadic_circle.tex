...
% level-shift check: r(z)|_{k=0} = 0 (abelian limit). Y.
...
family used in the heptagon comparison. The concentration theorem for
\(\mathrm{ChirHoch}^\bullet(\cA)\) in degrees \(\{0,1,2\}\) is not a
formal consequence of \(\Ethree\)-rigidity alone; it also uses the
PBW-collapse hypothesis.
\end{theorem}
...
The Koszul dual $H_k^! = \Sym^{\mathrm{ch}}(V^*)$
...
$r(z) = k\Omega/z$ (trace-form convention).
The Drinfeld centre
