...
pair of pants), and prove that, on the stated Koszul and
topologisation loci, the derived chiral centre
...
$\SCchtop + \text{inner conformal-vector datum} \simeq \Ethree^{\mathrm{top}}$ for
affine Kac--Moody at non-critical level (chain-level) and, via
Drinfeld--Sokolov reduction, for Virasoro and all $\cW$-algebras
(cohomological, with model-level $(\Ethree)_\infty$ under the stated
DS hypotheses). Under the intrinsic-formality and local-to-global
hypotheses of Theorem~\ref{thm:e-e3-identification}, the $\En$
operadic circle closes for simple $\fg$ at the level of formal
\(\Ethree\)-deformation families. Five notions of
...
\begin{proposition}[FM operad \textup{\cite{FultonMacPherson94}}]
...
the double-pole OPE. The ordered collision kernel is
\begin{equation}\label{eq:e-heis-r}
K_{H_k}^{\mathrm{coll}}(z)=k\Omega_{\mathcal H}/z,
\end{equation}
with $\tau_{H_k,2}^{\mathrm{res}}q_2(K_{H_k}^{\mathrm{coll}})
=\kappa(H_k)=k$. Its Fock representation supplies the classical
$r$-matrix. The specialization $k=0$ gives the zero operator.
\end{example}
...
Koszul dual is
$H_k^!=(\Sym^{\mathrm{ch}}(V^*[1]),m_0=-k\omega)$, the
curved second-kind symmetric chiral branch. The Koszul
complementarity is
...
The topological foundation is factorisation homology of manifolds
with $\En$ coefficients, developed by Ayala--Francis~\cite{AF15}
and extended to structured manifolds in
Ayala--Francis~\cite{AF19}.  In the topological setting, the
bar complex of an $\En$-algebra $A$ computes the
factorisation homology $\int_M A$ when $M$ is an
$n$-framed manifold; the chiral bar complex of the present
paper is the holomorphic refinement of this construction.
Higher enveloping algebras, introduced by
Knudsen~\cite{Knudsen18}, provide a Lie-algebraic model for
factorisation homology of free $\En$-algebras: the higher
enveloping algebra $U_n(\fg)$ of a Lie algebra computes the
factorisation homology of the free $\En$-algebra on $\fg$.
The operadic bar-cobar machinery underlying the Koszul duality
between $\En$-algebras and $\En$-coalgebras is developed in
full generality in Fresse~\cite{Fresse17}, where the homotopy
theory of operads is constructed using Grothendieck--Teichm\"uller
methods.  The intrinsic formality of $\En$-operads, proved by
Fresse--Willwacher~\cite{FresseWillwacher20}, is the technical
input for arrow~$(4)$ of the operadic
circle~\eqref{eq:e-circle}: it implies that the higher Deligne
conjecture is controlled by the rational homotopy type of
$\En$-operads, without auxiliary choices.  Higher Hochschild
homology, developed by Ginot--Tradler--Zeinalian~\cite{GTZ14},
computes the factorisation homology of $\Einf$-algebras over
arbitrary simplicial sets; the chiral Hochschild theory of
Section~\ref{sec:derived-centre} is the algebro-geometric
...
bar complex is uncurved at every genus. Under~$H_D^K$ the Hodge
character is $\operatorname{ch}_g(\mathfrak O_g^K)=(-1)^gk\lambda_g$;
under~$H_D^{\mathrm{graph}}$ the scalar trace gives
$F_1=k/24$ on
$\overline{\cM}_{1,1}$.
\end{example}
...
$A = \cA|_{\{z_0\}}$ the fibre at a basepoint, after choosing a
formal-disk trivialisation near \(z_0\). The local
...
of \(Z_{\SCchtop}^{\mathrm{loc}}(A)\) at degree \((k,m)\) is a map
\[
\phi_{k,m}\colon
\overline{\FM}_{k,m}(\bH;\bR) \;\longrightarrow\;
\Hom(A^{\otimes m}, A),
\]
with the symmetric equivariance on the interior labels and the ordered
boundary incidence inherited from the mixed Swiss-cheese sector. The
boundary inputs provide the module variables; the interior points
provide the holomorphic chiral insertions. Taking the equaliser over
the mixed-sector composition maps, and then gluing these local
formalisations over \(X\), gives the chiral Hochschild cochain
complex~\eqref{eq:e-chirHoch}.
...
\subsection{Theorem H: family-indexed cohomological support}

\begin{theorem}[Theorem H: support from a family retract]
\label{thm:e-theorem-H}
Fix a finite set $S\subset\mathbb Z$.  Let $Q_\cA$ be a complete
filtered model of $C^\bullet_{\mathrm{ch}}(\cA,\cA)$, and suppose
there are complete complexes and continuous maps
\[
 ...
 dh+hd=\id-\iota p,
\]
where the cochain terms of $K_{\cA,S}$ occur in degrees belonging
to~$S$.  Include in this datum the chart quasi-isomorphism
$Q_\cA\xrightarrow{\sim}C^\bullet_{\mathrm{ch}}(\cA,\cA)$,
the finite-window inverse-limit comparison, and the
ordered-to-symmetric averaging quasi-isomorphism.  Then
\begin{equation}\label{eq:e-thmH}
 \ChirHoch^n(\cA)\cong H^n(K_{\cA,S}),
 \qquad
 \operatorname{Supp}\ChirHoch^\bullet(\cA)\subseteq S.
\end{equation}
If these groups have finite graded pieces, their Hilbert series is
$P_\cA(t)=\sum_{n\in S}\dim H^n(K_{\cA,S})t^n$.  A palindromic
relation with $\cA^!$ follows from a separately specified perfect
degree-$d$ pairing.
\end{theorem}

\begin{remark}[Three independent data]
\label{rem:e-amplitude}
The finite set~$S$ records cohomological support.  The dimensions
$\dim H^n(K_{\cA,S})$ record the Hilbert series.  A perfect shifted
pairing records duality.  The Gelfand--Fuchs cohomology of the mode
Lie algebra and the chiral Hochschild cohomology of the curve chart
are functors on different ambient objects.
\end{remark}

\begin{example}[Rank-one even superboson]
\label{ex:e-heis-chirhoch}
Bakalov--De Sole--Kac compute the bounded vertex cohomology of the
rank-one even superboson $B_{\mathfrak h}$:
\begin{equation}\label{eq:e-heis-hoch0}
 \bigl(\dim H^n_{\mathrm{ch},b}
 (B_{\mathfrak h},B_{\mathfrak h})\bigr)_{n\geq0}
 =(2,1,0,0,\ldots).
\end{equation}
Thus its bounded support is $\{0,1\}$.  A bounded-to-chart
quasi-isomorphism
\[
 \chi^{\mathrm{bd}}_{B_{\mathfrak h}}\colon
 C^\bullet_{\mathrm{ch},b}(B_{\mathfrak h},B_{\mathfrak h})
 \xrightarrow{\sim}C^\bullet_{\mathrm{ch}}(\cA_{B_{\mathfrak h}},
 \cA_{B_{\mathfrak h}})
\]
transports precisely these dimensions to the curve chart
\cite[Theorem~7.4]{BDSK21}.
\end{example}

\begin{example}[Affine Kac--Moody: $\ChirHoch^*(V_k(\fg))$]
\label{ex:e-km-chirhoch}
For the affine Kac--Moody algebra $V_k(\fg)$ with
$\fg$ simple, Bakalov--De Sole--Kac conjecture the bounded estimates
\[
 \dim H^n_{\mathrm{ch},b}(V_k(\fg),V_k(\fg))
 \leq
 \dim\bigl(\Lambda^n\fg\oplus\Lambda^{n+1}\fg\bigr)
\]
away from the critical level \cite[Conjecture~7.5]{BDSK21}.  A
bounded-to-chart quasi-isomorphism transports these estimates to
$\ChirHoch^n(\cA_{V_k(\fg)})$.  The adjoint~$\fg$ belongs to the
zero-mode derivation prequotient and the bar-dual generator count.
\end{example}
...
\begin{enumerate}[label=\textup{(\roman*)}]
 ...
 $\Etwo$ structure (chiral braces), with support computed by
 a family datum $H_H(\cA;S)$ (Theorem~\ref{thm:e-theorem-H}).
...
\item \emph{Mixed sector} ($k$ closed and $m$ open inputs,
 ...
 \;=\; \overline{\FM}_{k,m}(\bH;\bR).
 \]
 This is the Fulton--MacPherson space of \(k\) interior points and
 \(m\) boundary-ordered points in the upper half-plane, with its
 incidence strata. Its open stratum is
 \(\Conf_k(\bH)\times\Conf_m(\bR)\); the product
 \(\FM_k(\bC)\times\Eone(m)\) is only the decoupled open-stratum
 approximation.
 No mixed operations with closed output: the closed colour
 receives no information from the open colour.
\end{enumerate}
Composition inserts interior configurations into interior points,
boundary configurations into boundary points, and records collisions
where interior points approach the boundary. The no-open-to-closed rule
...
\emph{single-coloured} operads. The decoupled open-stratum product
...
The Koszul dual
$H_k^!=(\Sym^{\mathrm{ch}}(V^*[1]),m_0=-k\omega)$ has
...
The answer requires extra structure: an inner conformal-vector datum.
Its antighost contraction makes holomorphic translations $Q$-exact
...
vector $T(z) \in \cA$ together with an antighost contraction
$G(z)$ in the algebra of bulk observables such
that
...
\emph{Critical level.}
At $k = -h^\vee$, the Sugawara denominator vanishes and the
coefficient has a pole: the inner conformal-vector datum is undefined,
$\partial_z$ is not $Q$-exact,
...
Without an inner conformal-vector datum, the $\SCchtop$-algebra produces a
...
BRST primitive of translation is the bridge. The $\Ethree$ is
$\Ethree^{\mathrm{top}}$ (topological), not
$\Ethree$-chiral: the primitive null-homotopes the chiral
translation.
\end{remark}
...
For Virasoro and $\cW$-algebras, the cohomological
$\Ethree^{\mathrm{top}}$ is now proved via DS reduction
(Theorem~\ref{thm:e-topologisation-DS}), and the
model-level $(\Ethree)_\infty$ follows
(Proposition~\ref{prop:e-chain-level-virasoro}).
The original-complex chain-level lift remains open for
class~$M$:
\end{remark}

\begin{conjecture}[Original-complex topologisation for class~$M$]
\label{conj:e-topologisation-general}
Let $\cA$ be a chiral algebra with conformal vector $T(z)$ at
non-critical level, and suppose the associated
holomorphic--topological theory on $\bC \times \bR$ has a BRST
differential $Q$ with $T = [Q, G]$ for some antighost
contraction $G$. Then:
\begin{enumerate}[label=\textup{(\roman*)}]
\item \textup{(Cohomological.)} $H_Q^\bullet(\cZ^{\mathrm{der}}_{\mathrm{ch}}(\cA))$
carries an $\Ethree^{\mathrm{top}}$-algebra structure.
This is now a theorem for all $\cW$-algebras obtained by
DS reduction \textup{(}Theorem~\ref{thm:e-topologisation-DS}\textup{)}
and all conformal vertex algebras with freely generated PVA
associated graded \textup{(}Khan--Zeng~\cite{KhanZeng25}\textup{)}.
\item \textup{(Original-complex.)} The $\Ethree^{\mathrm{top}}$
structure lifts from BRST cohomology to the original cochain
complex $\cZ^{\mathrm{der}}_{\mathrm{ch}}(\cA)$.
This remains open for class~$M$ \textup{(}Virasoro, $\cW_N$\textup{)}:
the $\Ainf$-coherence tower is infinite and the gauge
rectification does not terminate.
The coderived category is expected to absorb the obstruction.
\end{enumerate}
\end{conjecture}
...
\subsection{Topologisation via Drinfeld--Sokolov reduction}
\label{subsec:topologisation-DS}

The cohomological topologisation of
Theorem~\ref{thm:e-topologisation} was stated for affine
Kac--Moody. For Virasoro and $\cW$-algebras, the conjecture
(Conjecture~\ref{conj:e-topologisation-general}) asked for the
$3$d HT BRST complex. The Costello--Gaiotto
theorem~\cite{CostelloGaiotto} provides exactly this: every
$\cW$-algebra $\cW^k(\fg, f)$ arises as the boundary chiral
algebra of holomorphic Chern--Simons with Drinfeld--Sokolov
boundary conditions. The $3$d \emph{bulk} is the same
holomorphic Chern--Simons for~$\fg$, independent of the
nilpotent~$f$. The DS stress tensor decomposes as
$T_{\mathrm{DS}} = T_{\mathrm{Sug}} + T_{\mathrm{imp}}$,
where the improvement term involves only Cartan currents:
$T_{\mathrm{imp}} = \sum_{i,j}(x_0)_i\,
(\kappa^{-1})^{h_i h_j}\,\partial J_{h_j}$
with $x_0 = \tfrac{1}{2}h_0 \in \fh$ the neutral element of the
$\mathfrak{sl}_2$-triple $(e, h_0, f)$.
Since each $J_a = [Q_{\mathrm{CS}}, \bar c_a]$ on
$Q_{\mathrm{CS}}$-cohomology (by the BRST transformation of the
antighost), both summands are $Q_{\mathrm{CS}}$-exact in the $3$d
bulk.

\begin{theorem}[Cohomological topologisation via DS reduction]
\label{thm:e-topologisation-DS}
Let $\fg$ be a finite-dimensional simple Lie algebra,
$f \in \fg$ a nilpotent element admitting the standard good integral
grading used in Drinfeld--Sokolov reduction, and $k \neq -h^\vee$.
The $\cW$-algebra $\cW = \cW^k(\fg, f)$ obtained by
Drinfeld--Sokolov reduction from $V_k(\fg)$ admits an
$\Ethree^{\mathrm{top}}$-structure on the BRST cohomology of
its derived chiral centre
$\cZ^{\mathrm{der}}_{\mathrm{ch}}(\cW)$:
\begin{equation}\label{eq:e-DS-topologisation}
H_Q^\bullet\bigl(
\cZ^{\mathrm{der}}_{\mathrm{ch}}(\cW)\bigr)
\;\text{ is an }\;
\Ethree^{\mathrm{top}}\text{-algebra.}
\end{equation}
In particular, this applies to: Virasoro
$\mathrm{Vir}_c = \cW^k(\mathfrak{sl}_2, f_{\mathrm{prin}})$,
principal $\cW_N = \cW^k(\mathfrak{sl}_N, f_{\mathrm{prin}})$
for all $N$, and the Bershadsky--Polyakov algebra
$\cW^k(\mathfrak{sl}_3, f_{\min})$.
\end{theorem}

\begin{proof}
The Costello--Gaiotto theorem~\cite{CostelloGaiotto} provides the
$3$d holomorphic--topological theory: holomorphic Chern--Simons
for~$\fg$ at level~$k$, with DS boundary conditions determined
by~$f$. The statement is made on the locus where those boundary
conditions are represented by the standard BRST complex associated
to the chosen good grading. The $3$d bulk is independent of~$f$.

The DS-transported antighost is
$G' = G_{\mathrm{Sug}} + G_{\mathrm{imp}}$, where
$G_{\mathrm{Sug}} = (1/2(k+h^\vee))\sum_a {:}J^a\,\bar c_a{:}$
and
$G_{\mathrm{imp}} =
-\sum_{i,j}\,(x_0)_i\,
(\kappa^{-1})^{h_i h_j}\,\partial\bar c_{h_j}$.
Both are well-defined bulk operators.

\emph{The Sugawara term:}
$[Q_{\mathrm{CS}}, G_{\mathrm{Sug}}] = T_{\mathrm{Sug}}$
on $Q_{\mathrm{CS}}$-cohomology
(the same identity as in
Theorem~\ref{thm:e-topologisation}).

\emph{The improvement term:}
Since $x_0 = \tfrac{1}{2}h_0 \in \fh$
(the Jacobson--Morozov theorem places $h_0$ in the Cartan
subalgebra for any nilpotent), the improvement involves
only Cartan currents.
The identity $J_a = [Q_{\mathrm{CS}}, \bar c_a]$ on
$Q_{\mathrm{CS}}$-cohomology implies
$\partial J_{h_j} = [Q_{\mathrm{CS}}, \partial\bar c_{h_j}]$.
Hence
$T_{\mathrm{imp}} = [Q_{\mathrm{CS}}, G_{\mathrm{imp}}]$.

\emph{BRST identity:}
$[Q_{\mathrm{CS}}, G'] = T_{\mathrm{Sug}} + T_{\mathrm{imp}}
= T_{\mathrm{DS}}$ on $Q_{\mathrm{CS}}$-cohomology.
By the argument of Theorem~\ref{thm:e-topologisation}(i):
holomorphic translations are $Q$-exact, the factorisation
algebra on $\bC$ is locally constant on $Q$-cohomology, and
Dunn additivity produces
$\Etwo^{\mathrm{top}} \otimes \Eone^{\mathrm{top}} =
\Ethree^{\mathrm{top}}$.
\end{proof}
...
\label{rem:e-DS-sl2}
For $\fg = \mathfrak{sl}_2$ with $h^\vee = 2$,
$\mathrm{rk} = 1$, $\rho^\vee = 1/2$, and $\kappa^{hh} = 1/2$,
the DS-transported antighost reduces to
\[
G'(z)
\;=\;
\frac{1}{2(k+2)}\bigl({:}J^e\,\bar c_f{:}
+ {:}J^f\,\bar c_e{:}
+ \tfrac{1}{2}{:}J^h\,\bar c_h{:}\bigr)
...
on $Q_{\mathrm{CS}}$-cohomology.
The improvement antighost $\frac{1}{4}\partial\bar c_h$ is the
source of the $\frac{1}{2}\partial J^h$ term in the Miura
transformation.
\end{remark}

\begin{remark}[The proof is cohomological]
\label{rem:e-DS-cohomological}
The BRST identity is proved on $Q_{\mathrm{CS}}$-cohomology:
the ghost bilinear in $Q_{\mathrm{CS}} \cdot \bar c_a$ is
$Q$-exact but not zero, so at the cochain level
$[Q_{\mathrm{CS}}, G']$ differs from~$T_{\mathrm{DS}}$
by $Q$-exact terms.
For topologisation, the cohomological identity suffices.
The chain-level question is addressed in
Proposition~\ref{prop:e-chain-level-virasoro} below.
\end{remark}

\subsection{Chain-level $(\Ethree)_\infty$ for Virasoro
and $\cW$-algebras}
\label{subsec:chain-level-E3-virasoro}

The cohomological topologisation of
Theorem~\ref{thm:e-topologisation-DS} does not address
whether the $\Ethree^{\mathrm{top}}$-structure lifts from
BRST cohomology to the cochain complex itself.
For affine Kac--Moody (class~$L$), such a lift is proved in
Volume~I: the gauge rectification procedure terminates in
finitely many steps because the shadow depth
$r_{\max} = 3$ is finite.
For Virasoro (class~$M$), the shadow depth is infinite,
and the gauge rectification does \emph{not} terminate.
The chain-level $\Ethree$ on the \emph{original} brace
deformation complex of $\cZ^{\mathrm{der}}_{\mathrm{ch}}
(\mathrm{Vir}_c)$ is a genuinely harder problem.

There are, however, two unconditional chain-level results
that can be extracted from the existing machinery.

\begin{proposition}[Chain-level $(\Ethree)_\infty$
for Virasoro and $\cW$-algebras]
\label{prop:e-chain-level-virasoro}
Let $\cW = \cW^k(\fg, f)$ be a $\cW$-algebra obtained
by DS reduction from $V_k(\fg)$ at non-critical level
$k \neq -h^\vee$, including $\mathrm{Vir}_c =
\cW^k(\mathfrak{sl}_2, f_{\mathrm{prin}})$. The derived
chiral centre
$\cZ^{\mathrm{der}}_{\mathrm{ch}}(\cW)$ admits a
chain-level $(\Ethree)_\infty^{\mathrm{top}}$-algebra
structure on a quasi-isomorphic model, in two
independent ways.
\begin{enumerate}[label=\textup{(\roman*)}]
\item \textup{(From cohomological topologisation.)}
  Theorem~\textup{\ref{thm:e-topologisation-DS}} produces
  the cohomological $\Ethree^{\mathrm{top}}$ on
  $M = H_Q^\bullet(\cZ^{\mathrm{der}}_{\mathrm{ch}}(\cW))$.
  Over~$\bC$, choose a splitting of cycles and boundaries in
  the cochain complex to obtain a strong deformation retract
  from $\cZ^{\mathrm{der}}_{\mathrm{ch}}(\cW)$ onto~$M$.
  The homotopy transfer theorem for $\Ethree$-algebras
  produces a transferred $(\Ethree)_\infty^{\mathrm{top}}$
  structure on~$M$ \textup{(}zero differential\textup{)},
  unique up to $\infty$-quasi-isomorphism.

\item \textup{(From the $3$d bulk.)}
  The $3$d holomorphic Chern--Simons bulk for~$\fg$
  at level~$k$ has local observables that carry a
  chain-level $\Ethree^{\mathrm{top}}$-structure, independent
  of the boundary condition: for affine Kac--Moody boundary,
  this is the content of the gauge rectification in Volume~I.
  The DS boundary condition determines a
  BRST-type differential $Q_{\mathrm{DS}}$ on the bulk
  cochain complex~$C$ that is a derivation of the
  $\Ethree^{\mathrm{top}}$ structure on~$C$,
  because $Q_{\mathrm{DS}}$ is a component of the full
  BV-BRST differential $Q_{\mathrm{CS}}$ and the
  $\Ethree$ structure was constructed to be compatible
  with $Q_{\mathrm{CS}}$.
  The BRST cohomology $H^\bullet(C, Q_{\mathrm{DS}}) \cong
  \cZ^{\mathrm{der}}_{\mathrm{ch}}(\cW)$
  inherits $(\Ethree)_\infty^{\mathrm{top}}$ by homotopy
  transfer through the strong deformation retract from
  $(C, Q_{\mathrm{DS}})$ to its cohomology.

  This second route is independent of the proof of
  Theorem~\textup{\ref{thm:e-topologisation-DS}}: it
  derives the $\Ethree$ structure on the $\cW$-algebra
  derived centre from the \emph{already proved} chain-level
  $\Ethree$ on the Kac--Moody-side bulk, without passing
  through the DS-transported antighost.
\end{enumerate}
\end{proposition}

\begin{proof}
Part~(i) is a direct application of the homotopy transfer
theorem for operadic algebras~\cite{LV12}: the
$\Ethree^{\mathrm{top}}$ operations on the cohomology
complex $M$ are strict, the strong deformation retract
transfers them to $(\Ethree)_\infty^{\mathrm{top}}$ on the
zero-differential model, and the transferred structure is
unique up to $\infty$-quasi-isomorphism by the standard
contractibility of the space of choices.

Part~(ii) requires two inputs.

\emph{Input~1: Chain-level $\Ethree$ on the bulk.}
The gauge rectification for $V_k(\fg)$ at non-critical
level produces a corrected antighost contraction
$\widetilde{G}_1$ satisfying
$[Q_{\mathrm{CS}}, \widetilde{G}_1] = T_{\mathrm{Sug}}$
on the original BRST complex, and the brace defect
$D^{(1)} = [m, \widetilde{G}_1] - \partial_z$ vanishes
because the shadow depth $r_{\max} = 3$ is finite
(class~$L$). This gives chain-level $\Ethree^{\mathrm{top}}$
on the local bulk observables $C$ as a factorisation algebra
on $\bC \times \bR$.

\emph{Input~2: $Q_{\mathrm{DS}}$ is an $\Ethree$-derivation.}
The DS boundary condition is implemented at the $3$d level
by restricting to a sub-BV complex determined by the
nilpotent~$f$. The differential $Q_{\mathrm{DS}}$ is a
component of the full BV-BRST differential $Q_{\mathrm{CS}}$
of the $3$d bulk. The chain-level $\Ethree$ structure on~$C$
was constructed to be compatible with $Q_{\mathrm{CS}}$
(the brace equation $[m, \widetilde{G}_1] = \partial_z$
holds in the complex of $Q_{\mathrm{CS}}$-cochains).
Since $Q_{\mathrm{DS}}$ is a summand of~$Q_{\mathrm{CS}}$,
compatibility descends: $Q_{\mathrm{DS}}$ acts as an
$\Ethree$-derivation on~$C$.

With Inputs~1 and~2, the homotopy transfer theorem
for operadic algebras produces a transferred
$(\Ethree)_\infty^{\mathrm{top}}$-structure on the
$Q_{\mathrm{DS}}$-cohomology
$H^\bullet(C, Q_{\mathrm{DS}}) \cong
\cZ^{\mathrm{der}}_{\mathrm{ch}}(\cW)$.
The transferred structure is unique up to
$\infty$-quasi-isomorphism.
\end{proof}

\begin{remark}[Three layers of chain-level control]
\label{rem:e-three-layers}
Proposition~\ref{prop:e-chain-level-virasoro} gives
chain-level $(\Ethree)_\infty$ on the zero-differential
cohomology complex. This is the model-level statement:
sufficient for applications that depend only on the
$\infty$-quasi-isomorphism class (existence of an
$\Ethree$ enhancement, factorisation homology up to
equivalence, deformation functors).

A stronger statement would be chain-level $\Ethree$ on the
\emph{original} cochain complex
$\cZ^{\mathrm{der}}_{\mathrm{ch}}(\mathrm{Vir}_c)$ itself.
This requires an $\Ainf$-coherent null-homotopy of
$\partial_z$ in the brace deformation complex, which for
class~$M$ involves an infinite coherence tower (all
$m_k^{\SCchtop}$ are nontrivial). The tower does not
terminate, and the coderived category is the expected
framework for absorbing it.

The three layers are therefore:
\begin{enumerate}[label=\textup{(\alph*)}]
\item \emph{Cohomological $\Ethree^{\mathrm{top}}$}:
 proved unconditionally
 \textup{(}Theorem~\ref{thm:e-topologisation-DS}\textup{)}.
\item \emph{Model-level $(\Ethree)_\infty^{\mathrm{top}}$}:
 proved unconditionally
 \textup{(}Proposition~\ref{prop:e-chain-level-virasoro}\textup{)}.
\item \emph{Original-complex $\Ethree^{\mathrm{top}}$}:
 open, obstructed by the infinite class~$M$ coherence tower.
 The coderived category is expected to absorb this obstruction.
\end{enumerate}
For affine Kac--Moody (class~$L$), all three layers are proved,
because the coherence tower terminates at depth~$3$.
\end{remark}

\begin{remark}[The two obstructions to original-complex $\Ethree$
for Virasoro]
\label{rem:e-virasoro-two-obstructions}
For $\mathrm{Vir}_c$ specifically, the obstruction to
layer~(c) has two components:
\begin{enumerate}[label=\textup{(\roman*)}]
\item \emph{The infinite shadow tower.}
 Virasoro is class~$M$: all $\Ainf$ operations $m_k$ for
 $k \geq 2$ are nontrivial, producing an infinite coherence
 tower in the brace deformation complex. The gauge
 rectification procedure generates corrections at every
 degree, none of which vanishes on the ordinary
 cochain complex.
\item \emph{The class~$M$ BV/bar discrepancy.}
 The chain-level BV${}={}$bar comparison fails for class~$M$
 on ordinary chain complexes (the quartic discrepancy
 $\delta_4$ is not a coboundary). In the coderived category,
 $\delta_4$ is absorbed.
\end{enumerate}
These are distinct obstructions: (i) is about the $\Ainf$
coherence of the antighost homotopy, and (ii) is about the
comparison between BV and bar constructions. Both point
to the coderived category as the resolution.
\end{remark}

\subsection{The iterated-Sugawara ladder and
$\Einf$-topologisation}
\label{subsec:iterated-sugawara-ladder}

Theorem~\textup{\ref{thm:e-topologisation}} promotes
$\SCchtop$ to $\Ethree^{\mathrm{top}}$ by a single inner
conformal-vector datum $(T(z),G(z))$ satisfying
$T = [Q, G]$. When the chiral algebra carries a finite \emph{tower}
of inner higher-spin Casimir fields
$\{T^{(n)}\}_{2 \leq n \leq N+1}$, each with a compatible BRST
primitive, the topologisation iterates on that finite rung: a
depth-$k$ inner stress tower promotes $\SCchtop$ to
$E_{k+2}^{\mathrm{top}}$. For principal $\cW_N$-algebras this is a
finite-rung criterion. The $\cW_\infty$ limit requires a completed
tower with controlled derived inverse-limit obstruction; it is not a
formal consequence of the finite theorem.

\begin{theorem}[Iterated Sugawara construction]
\label{thm:iterated-sugawara-standalone}
Let $\cA$ be a chiral algebra carrying a higher-spin Casimir
tower $\{T^{(n)}\}_{2 \leq n \leq N+1}$ of inner fields
\textup{(}so each $T^{(n)} \in \cA$ and the OPE closes among
them\textup{)}. Suppose the first \(k\) fields in this tower admit
BRST primitives
$G^{(n)} \in \cA$ with
$T^{(n)} = [Q_{\mathrm{tot}}, G^{(n)}]$ on the BRST cohomology
of the associated \(3\)d holomorphic--topological theory, and suppose
the resulting \(E_1\)-directions commute up to the specified coherent
homotopies. Then the depth-$k$ sub-tower $\{T^{(2)}, \ldots,
T^{(k+1)}\}$ topologises $\SCchtop$ to $E_{k+2}^{\mathrm{top}}$:
\[
H_Q^\bullet\bigl(\cZ^{\mathrm{der}}_{\mathrm{ch}}(\cA)\bigr)
 \;\text{ is an }\;
...
\begin{proof}
For each $n$ the pair $(T^{(n)}, G^{(n)})$ makes the
corresponding $C$-translation $\partial_{C,n}$ equal to
$[Q, G^{(n)}]$, hence $Q$-exact on cohomology. A single
$Q$-exact translation collapses one holomorphic direction into
a topological one (Theorem~\ref{thm:e-topologisation}); $k$
independent such collapses collapse $k$ holomorphic directions.
Dunn additivity for the orthogonal $E_1^{\mathrm{top}}$ factors
then assembles
$E_2^{\mathrm{chiral}} \otimes
 \bigl(E_1^{\mathrm{top}}\bigr)^{\otimes k}
 \simeq E_{k+2}^{\mathrm{top}}$,
giving the claimed structure.
\end{proof}

\begin{corollary}[Specialisations]
\label{cor:iterated-sugawara-specialisations-sa}
\textup{(i)} Virasoro $\mathrm{Vir}_c$ \textup{(}$N=2$,
depth $1$\textup{)}: $\Ethree^{\mathrm{top}}$.
\textup{(ii)} Principal $\cW_N$ \textup{(}depth $N-1$\textup{)}:
$E_{N+1}^{\mathrm{top}}$ when the finite-rung hypotheses of
Theorem~\ref{thm:iterated-sugawara-standalone} are satisfied.
\textup{(iii)} Principal $\cW_\infty[\Psi]$ \textup{(}depth
$\infty$\textup{)}: $\Einf^{\mathrm{top}}$ only after passage to a
completed tower with vanishing derived inverse-limit obstruction.
\end{corollary}

\begin{remark}[The operadic spiral]
\label{rem:operadic-spiral-sa}
The iterated-Sugawara ladder and the $\En$ operadic circle
\eqref{eq:e-circle} have opposite finite-rung effects. The bar
construction descends the operadic level by one; the derived centre
ascends it by one. A compatible tower of inner higher-spin fields adds
topological directions one at a time. The infinite-rung limit is a
completion problem, not a formal consequence of the finite ladder.
\end{remark}

\subsection[Chiral higher Deligne and the SC ch,top heptagon]{Chiral higher Deligne and the \SCchtop\ heptagon}
\label{subsec:chiral-higher-deligne}

The operadic circle closes only after the Deligne, Swiss-cheese, and
topologisation comparisons are placed in the same model. The derived
chiral centre \(\cZ^{\mathrm{der}}_{\mathrm{ch}}(\cA)\) carries the
unconditional \(\Etwo\) structure supplied by chiral Deligne. The
\(\Ethree\) statement below is the formal-family comparison under the
listed rigidity and PBW hypotheses.

\begin{theorem}[Chiral higher Deligne comparison]
\label{thm:chiral-higher-Deligne-sa}
Let $\cA$ be an $\Eone$-chiral algebra on a smooth curve $X$.
The derived chiral centre
\(\cZ^{\mathrm{der}}_{\mathrm{ch}}(\cA)
= \mathrm{ChirHoch}^\bullet(\cA,\cA)\) carries the \(\Etwo\)-structure
of chiral Deligne. If, in addition, the \(\SCchtop\)-action, PBW
collapse, and intrinsic-formality hypotheses of
Theorem~\ref{thm:e-e3-identification} hold in the chosen model, then
this \(\Etwo\)-structure extends to the formal \(\Ethree\)-deformation
family used in the heptagon comparison.  The family datum
$H_H(\cA;S)$ supplies the independent deformation retract that
computes the cohomological support of the same derived centre.
\end{theorem}

\begin{remark}[The seven faces of $\SCchtop$]
\label{rem:sc-heptagon-sa}
The pentagon of five $\SCchtop$-algebra presentations
\textup{(}operadic, Koszul, factorisation, BV--BRST,
convolution\textup{)} extends to a heptagon when two further
faces are included: \textup{(6)} the Drinfeld-centre face
$Z(\mathrm{Rep}_{\mathrm{fact}}(\cA)) \simeq
 \mathrm{Rep}_{\mathrm{fact}}(\cZ^{\mathrm{der}}_{\mathrm{ch}}(\cA))^{\Etwo}$
through categorified bar--cobar with half-braiding, and
\textup{(7)} the derived-algebraic-geometry face through PTVV
on $\mathrm{Map}(X \times \mathbb{R}_{\geq 0},
 \mathrm{B\,SC\text{-}Alg})$. The seven-edge closed heptagon
records that $\SCchtop$ is the generic $\SCchtop$-algebra
presentation; topologisation to $\Ethree^{\mathrm{top}}$ is
proved for affine Kac--Moody at non-critical level, and is
conjectural in general pending the chain-level lift of
Conjecture~\textup{\ref{conj:e-topologisation-general}}.
\end{remark}

\subsection{Classification by topologisation status}

\begin{proposition}[Topologisation of standard families]
...
$V_k(\fg)$, $k \neq -h^\vee$ &
 $T_{\mathrm{Sug}}$ & $\Ethree^{\mathrm{top}}$ &
 proved (chain-level) \\
...
\end{tabular}
\end{center}
The column ``Status'' records the strongest chain-level
control:
\emph{proved~\textup{(}chain-level\textup{)}} means the
$\Ethree^{\mathrm{top}}$ structure lifts to the original
BRST complex
\textup{(}affine Kac--Moody, class~$L$\textup{)};
\emph{proved~\textup{(}cohom.~$+$~model\textup{)}} means
cohomological $\Ethree^{\mathrm{top}}$
\textup{(}Theorem~\ref{thm:e-topologisation-DS}\textup{)}
and $(\Ethree)_\infty^{\mathrm{top}}$ on a
quasi-isomorphic model
\textup{(}Proposition~\ref{prop:e-chain-level-virasoro}\textup{)},
but the original-complex lift remains open
\textup{(}class~$M$: infinite coherence tower\textup{)}.
\end{proposition}
...
\item \emph{Drinfeld centre.}
 ...
 data. It is the right adjoint to forgetting the braiding from
 \(\Etwo\)-monoidal categories to \(\Eone\)-monoidal categories.
 The Drinfeld centre is the source of non-symmetric half-braiding in
 this circle: direct restriction from
...
\begin{theorem}[Arrow 3 as the source of non-symmetric braiding]
...
\item The non-symmetric braiding in the Drinfeld centre is
 ...
 restriction; it enters through arrow~$3$. For
...
This is the precise sense in which arrow~$3$ contributes the
non-symmetric braiding in the circle.
\end{proof}
...
\begin{theorem}[Formality bridge for symmetric chiral algebras]
\label{thm:e-formality-bridge}
For an $\Einf$-chiral algebra $\cA$ with genuinely symmetric
factorization and trivial spectral kernel \textup{(}for instance,
abelian class~G examples\textup{)}, the averaging
...
\BarSig(\cA).
\]
The locality symmetry is trivial and the averaging map loses no
ordered spectral data. For affine Kac--Moody, Virasoro, and
$\cW_N$, averaging retains only the $G/L/C/M$ shadow invariants and
forgets ordered $r$-matrix, Hopf, and spectral data.
\end{theorem}
...
extract normalized traces of the homotopy-coinvariant ordered bar.
At degree~$2$ the scalar is
$\tau_{\cA,2}^{\mathrm{res}}q_2(K_\cA^{\mathrm{coll}})$; it equals
$\kappa$ in abelian and scalar families and $\kappa_{\mathrm{dp}}$
on the non-abelian affine row, where $\dim(\fg)/2$ completes
$\kappa$. They also extract the $H_D^K$ Hodge character
$\operatorname{ch}_g(\mathfrak O_g^K)=(-1)^g\kappa\lambda_g$,
the $H_D^{\mathrm{graph}}$ functional, and the shadow tower. For $\Einf$-chiral
...
H_*(\Ethree) \;=\; \Pthree.
\]
	This is operad formality. It does not imply intrinsic formality of
	an arbitrary $\Ethree$-algebra; recovering an algebra from its
	cohomology $\Pthree$-structure requires a separate rigidity or
	deformation-complex vanishing theorem.
\end{theorem}
...
\begin{theorem}[Conditional identification of $\Ethree$-deformation families
for simple $\fg$]
\label{thm:e-e3-identification}
Assume intrinsic formality of the relevant $\Ethree$-algebra and the
local-to-global extension for its deformation complex. Let $\fg$ be a simple finite-dimensional Lie algebra,
$V_k(\fg)$ the universal affine vertex algebra at non-critical
level $k \neq -h^\vee$, and
...
$k = -h^\vee$, the Sugawara vector is not defined in the affine vacuum
module, topologisation fails, and the $\Ethree$ identification does
not apply.
\end{remark}
...
On the verified $\mathfrak{sl}_2$ BRST shadow the induced
$\Pthree$ obstruction class changes by a coboundary, so the
cohomology class is associator-independent. For general simple
$\fg$ this independence is an additional comparison hypothesis, not
a formal consequence of the $\GRT_1$ torsor, and it says nothing by
itself about categorical modular data. At chain level, the
associator dependence is genuine.
\end{theorem}
...
\begin{remark}[Each notion has its own derived centre]
...
At integer level $k \ge 1$, the TUY/Hitchin conformal-block target
reached through the ordered-chain-to-TUY comparison has Verlinde
dimension:
...
at genus $g$ and level $k$. The ordered chiral chain complex over
the ordered configuration space is compared with this target only
after the integrable-level truncation, TUY sewing, projectively flat
connection, and determinant-anomaly packages are supplied.
Handle attachment (sewing along a pair of pants) and separating
factorisation are TUY identities, with the ordered pair-of-pants
fusion product (Definition~\ref{def:e-pants}) matching them under the
comparison package. The closed-form polynomials are obtained
...
$(\kappa, S_4, \Delta, \mathrm{class})$ and the genus package
$(\operatorname{Obs}^{\mathrm{def}}_g,\mathfrak O_g^K,
\operatorname{obs}^{\mathrm{Hdg}}_g,F_g)$. The three standard families
...
% level-shift trace-form check: r(z)|_{k=0} = 0. Y.
...
\textup{Koszul dual} & $H_k^!=(\Sym^{\mathrm{ch}}(V^*[1]),m_0=-k\omega)$,
 $\kappa^! = -k$ & $K = k + (-k) = 0$ \\
...
\textup{Hodge character} &
 $\operatorname{ch}_g(\mathfrak O_g^K)=(-1)^gk\lambda_g$
 & $H_D^K$; $d_{\mathrm{fib}}^2 = k\,\omega_g$
\end{tabular}
\end{center}
\end{proposition}
...
The bounded rank-one superboson cohomology has dimension vector
$(2,1,0,\ldots)$; its transport to the curve chart uses
$\chi^{\mathrm{bd}}_{B_{\mathfrak h}}$
...
\textup{OPE} &
 ...
 & defining \\
% level-shift trace-form: r(z)|_{k=0} = 0 for trace-form convention. Y.
\textup{collision kernel} &
 $K_{k,\fg}^{\mathrm{coll}}(z)=k\,\Omega_{\mathrm{tr}}/z$
 \textup{(trace-form)} &
 $d\log$ absorption \\
\textup{$\kappa$} &
 $\kappa = d(k + h^\vee)/(2h^\vee)$ &
 $\tau_{k,\fg}^{\mathrm{res}}q_2(K_{k,\fg}^{\mathrm{coll}})+d/2$ \\
\textup{Class} & L &
...
\textup{Yangian} &
 ...
 $\Einf$-chiral \\
\textup{Koszul dual} &
 $V_k(\fg)^!$, $\varkappa = 0$ & $\varkappa = \kappa + \kappa' = 0$ \\
\textup{$\ChirHoch^*$} &
...
\begin{proof}
The collision kernel uses the trace-form convention:
$K_{k,\fg}^{\mathrm{coll}}(z)=k\Omega_{\mathrm{tr}}/z$, where
$\Omega$ is the inverse Killing-form Casimir. Its homotopy coinvariant
retains the Casimir operator and the normalized functional gives
$\tau_{k,\fg}^{\mathrm{res}}q_2(K_{k,\fg}^{\mathrm{coll}})
=k\dim(\fg)/(2h^\vee)$ (the double-pole channel contribution).
The full $\kappa$ includes the
...
Under a family datum $H_H(V_k(\fg);S_{k,\fg})$, the curve-level
quotient is supported in $S_{k,\fg}$ and is computed by
$K_{V_k(\fg),S_{k,\fg}}$.  The bounded affine estimates enter through
the bounded-to-chart comparison; the adjoint~$\fg$ belongs to the
zero-mode derivation prequotient and bar-dual generator count.
\end{proof}

\begin{remark}[The Sugawara shift]
\label{rem:e-sugawara-shift}
For the Heisenberg row,
$\tau_{\mathcal H,2}^{\mathrm{res}}q_2(K_{\mathcal H}^{\mathrm{coll}})
=\kappa$. For non-abelian Kac--Moody, the same trace gives
$\kappa_{\mathrm{dp}}$ and the full modular characteristic is
$\kappa=\kappa_{\mathrm{dp}}+\dim(\fg)/2$. The term
...
\textup{Koszul dual} &
 ...
 self-dual at $c = 13$ \\
\textup{$\ChirHoch^*$} &
 bounded support $\{0,2,3\}$; chart after $\chi^{\mathrm{bd}}$ &
 \cite[Theorem~7.2]{BDSK21}; Theorem~\ref{thm:e-theorem-H} \\
\textup{$\Ethree^{\mathrm{top}}$} &
 proved \textup{(}cohom.\ $+$ model\textup{)} &
 Theorem~\ref{thm:e-topologisation-DS},
 Proposition~\ref{prop:e-chain-level-virasoro} \\
\textup{SC-formality} & not SC-formal & class M \\
\textup{Hodge character} &
 $\operatorname{ch}_g(\mathfrak O_g^K)=(-1)^g(c/2)\lambda_g$
 & $H_D^K$
\end{tabular}
\end{center}
\end{proposition}
...
The scalar $\kappa = c/2$ here is the Virasoro-family specialization,
not a universal rule for Heisenberg or affine Kac--Moody input. The
Koszul dual is
$\mathrm{Vir}_{26-c}$ with $\kappa' = (26-c)/2$, giving the scalar
complementarity $\varkappa := \kappa + \kappa' = c/2 + (26-c)/2 = 13$
(distinct from the Trinity ghost-charge conductor
$K(\Vir) := c + c^! = 26 = -c_{\mathrm{ghost}}(\BRST_{\Vir})$; related
by $\varkappa = \varrho_{\Vir}\cdot K$, $\varrho_{\Vir} = 1/2$).
Self-duality at $c = 13$: $\varkappa = 13$ and
$\kappa = \kappa' = 13/2$.
...
The cohomological $\Ethree^{\mathrm{top}}$ structure for
Virasoro is proved by Theorem~\ref{thm:e-topologisation-DS}:
the Costello--Gaiotto theorem provides $3$d holomorphic
Chern--Simons with DS boundary conditions, and the
DS-transported antighost $G'$ satisfies
$T_{\mathrm{DS}} = [Q_{\mathrm{CS}}, G']$ on
$Q_{\mathrm{CS}}$-cohomology.
The chain-level $(\Ethree)_\infty$ on a quasi-isomorphic
model follows from
Proposition~\ref{prop:e-chain-level-virasoro}.
The original-complex lift remains open (class~$M$:
infinite coherence tower;
Conjecture~\ref{conj:e-topologisation-general}(ii)).
\end{proof}
...
$\Ethree$ & trivial & nontrivial (proved) &
 conjectural \\
SC-formal & yes & no & no \\
$\ChirHoch$ profile & bounded $(2,1,0,\ldots)$; chart after $\chi^{\mathrm{bd}}$ & family support $S_{V_k(\fg)}$ after the bounded-to-chart comparison & bounded Virasoro support $\{0,2,3\}$; chart after $\chi^{\mathrm{bd}}$ \\
$K$ & $0$ & $0$ & $13$
\end{tabular}
\end{center}
...
and let \(U_\cA = \cA \bowtie \cA^!\) denote the completed chiral
Drinfeld double on the Koszul locus.
...
are defined. Then the expected comparison is an equivalence of
$\Ethree^{\mathrm{top}}$-algebras:
\begin{equation}\label{eq:e-closing}
\mathcal Z\!\left(\mathrm{Mod}^{\mathrm{compl}}_{U_\cA}\right)
\;\simeq\;
\cZ^{\mathrm{der}}_{\mathrm{ch}}(\cA),
\end{equation}
where \(\mathcal Z\) denotes the Drinfeld centre of the completed
module category, not the ordinary centre of the algebra object
\(U_\cA\), and \(\cZ^{\mathrm{der}}_{\mathrm{ch}}\) denotes the
...
\item \emph{Pointwise reduction for class M.}
 ...
 the Drinfeld centre
 \(\mathcal Z(\mathrm{Mod}^{\mathrm{compl}}_{U_\cA})\) can be computed
 stalk-wise:
...
stalk-wise for all classes, so
\(\mathcal Z(\mathrm{Mod}^{\mathrm{compl}}_{U_\cA})\) is computed as
a chiral Hochschild cochain complex. The comparison with
...
\(\mathcal Z(\mathrm{Mod}^{\mathrm{compl}}_{U_\cA})\) is a
well-defined $\Etwo$-braided monoidal category. The derived centre
...
\item \emph{Heisenberg $H_k$, $k \neq 0$
 ...
 completed module Drinfeld centre
 \(\mathcal Z(\mathrm{Mod}^{\mathrm{compl}}_{U_{H_k}})\simeq \bC^3\)
 (Proposition~\ref{prop:e-heis-complete}), matching the derived centre
...
\subsection{Proof for class G}

The closing conjecture is proved for the Heisenberg algebra
by resolving obstruction~(III) through the chiral
Hochschild--coHochschild duality.

\begin{lemma}[Chiral Morita--Hochschild comparison for $H_k$]
\label{lem:e-heis-morita-hochschild}
Let $C = \Barord(H_k)$ and $C^e = C \otimes C^{\mathrm{op}}$. The
chiral Drinfeld centre of the $\Eone$-monoidal category of complete
$H_k$-modules admits an identification
\begin{equation}\label{eq:e-heis-morita}
\mathcal Z(H_k\textup{-mod}^{\mathrm{compl}})
\;\simeq\;
\mathrm{Coext}_{C^e}^*(C_\Delta, C_\Delta)
\;\simeq\;
\ChirHoch^*(H_k, H_k)
\;=\;
\cZ^{\mathrm{der}}_{\mathrm{ch}}(H_k),
\end{equation}
...
\emph{Step~1: Module--comodule equivalence.}
The Heisenberg algebra $H_k$ is Koszul with
finite-type grading (each weight space is
finite-dimensional). By the module Koszul
duality theorem, the bar-cobar adjunction
restricts to an equivalence of derived
categories:
\[
D(\mathrm{Mod}^{\Eone,\,\mathrm{compl}}_{H_k})
\;\simeq\;
D(\mathrm{CoMod}^{\Eone,\,\mathrm{conil}}_{C}).
\]
This equivalence is monoidal with respect to the fusion product on
$H_k$-modules and the cotensor product on $C$-comodules: the fusion
$M_1 \otimes_{H_k}^{\mathbf{L}} M_2$ corresponds to the derived
cotensor $\mathcal{K}(M_1) \mathbin{\square}_C^{\mathbf{R}} \mathcal{K}(M_2)$.
The monoidality follows because the bar
resolution is a two-sided coalgebra resolution:
the two-pointed bar complex
$\Barord(H_k;\, M_1, M_2)$ computes the fusion,
and under the equivalence this is exactly the
cotensor of the corresponding comodules.

\emph{Step~2: Drinfeld centre as Hochschild self-extension.}
The Drinfeld centre of an $\Eone$-monoidal category $\cC$
is the category of pairs $(M, \gamma)$ where $\gamma$
is a half-braiding. For a module category $\cC = A\text{-mod}$,
the endomorphism algebra of the identity functor is
$\mathcal Z(\cC) = \mathrm{Nat}(\id_\cC, \id_\cC)
= \HH^*(A, A)$.
More precisely, the Hochschild cochain complex
$C^*(A, A)$ computes the derived endomorphisms
of the identity bimodule:
$\mathcal Z(\cC) \simeq R\!\Hom_{A^e}(A, A) = \HH^*(A, A)$.
The monoidal equivalence of Step~1 transports this to
the coalgebra side: the Drinfeld centre of
$C$-comod is the coHochschild self-extension
$\mathrm{Coext}_{C^e}^*(C_\Delta, C_\Delta)$.

\emph{Step~3: Hochschild--coHochschild duality.}
The chiral Hochschild--coHochschild duality
(cohomological version) gives
\[
\ChirHoch^*(H_k, H_k)
\;\simeq\;
\mathrm{Coext}_{C^e}^*(C_\Delta, C_\Delta).
...
Composing Steps~1--3:
\[
\mathcal Z(H_k\text{-mod}^{\mathrm{compl}})
\;\simeq\;
\mathcal Z(C\text{-comod}^{\mathrm{conil}})
\;\simeq\;
\mathrm{Coext}_{C^e}^*(C_\Delta, C_\Delta)
\;\simeq\;
\ChirHoch^*(H_k, H_k)
\;=\;
\cZ^{\mathrm{der}}_{\mathrm{ch}}(H_k). \qedhere
\]
\end{proof}

\begin{theorem}[Closing conjecture for class G]
\label{thm:e-closing-class-g}
For the Heisenberg algebra $H_k$ at level $k \neq 0$, the
$\En$ operadic circle closes on the completed module-category model:
there is an equivalence of $\Ethree^{\mathrm{top}}$-algebras
\begin{equation}\label{eq:e-closing-class-g}
\mathcal Z(\mathrm{Mod}^{\mathrm{compl}}_{U_{H_k}})
\;\simeq\;
\cZ^{\mathrm{der}}_{\mathrm{ch}}(H_k).
\end{equation}
Both sides are isomorphic to $\bC^3$ as graded vector spaces,
with trivial $\Pthree$ bracket.
\end{theorem}

\begin{proof}
We verify the hypotheses of the closing conjecture
and resolve all three obstructions simultaneously.

\emph{Obstruction~(I): Pointwise reduction.}
The Heisenberg is class~G: the shadow tower
terminates at degree~$2$ ($S_4 = 0$, $\Delta = 0$).
The $\Ainf$ corrections at degree $\ge 3$ are
absent (all higher $m_k$ vanish for $k \ge 3$
because the OPE has only a double pole). The
stalk-wise centre at every point of $\Ran(X)$
agrees with the full derived centre. Pointwise
reduction holds unconditionally.

\emph{Obstruction~(II): Factorisation for $H_k^!$.}
The Koszul dual
$H_k^!=(\Sym^{\mathrm{ch}}(V^*[1]),m_0=-k\omega)$
is a free-field chiral algebra generated by $V^*$
with the OPE
$\alpha^*(z)\alpha^*(w) \sim -k/(z-w)^2$.
As a free field, it carries a canonical factorisation
structure on $\Ran(X)$: the factorisation product is
the symmetric monoidal structure of the symmetric
algebra, and the $D$-module descent to the Ran space
is standard for free-field algebras. The Drinfeld
double $U_{H_k} = H_k \otimes H_k^!$ is therefore a
well-defined chiral algebra with braiding given by the
classical $r$-matrix $r(z) = k/z$.

\emph{Obstruction~(III): Bar-cobar vs Hochschild.}
This is the content of
Lemma~\ref{lem:e-heis-morita-hochschild}: the
module--comodule equivalence for $H_k$ transports the
Drinfeld centre to the coHochschild self-extension of
$\Barord(H_k)$, which is identified with the chiral
Hochschild cohomology by Hochschild--coHochschild
duality. The result:
\[
\mathcal Z(H_k\text{-mod}^{\mathrm{compl}})
\;\simeq\;
...
\emph{Comparison of $\Ethree$ structures.}
The derived chiral centre
$\cZ^{\mathrm{der}}_{\mathrm{ch}}(H_k) = \bC \oplus \bC[-1] \oplus \bC[-2]$
carries trivial $\Pthree$ bracket (the Gerstenhaber
bracket vanishes because $H_k$ is commutative: the Lie
bracket $[J, J] = 0$ forces all brackets on Hochschild
cohomology to vanish). The completed module Drinfeld centre
\(\mathcal Z(\mathrm{Mod}^{\mathrm{compl}}_{U_{H_k}})\) also has
trivial $\Pthree$ bracket
(the $R$-matrix $S(z) = \id$ gives symmetric braiding,
hence trivial bracket on the centre). By $\Ethree$
formality (Theorem~\ref{thm:e-e3-formality}), the
$\Ethree$-structure is determined by the $\Pthree$-structure
on cohomology. Since both $\Pthree$-structures are trivial
on the same underlying graded vector space $\bC^3$, the
$\Ethree$-algebras are isomorphic.

The topologisation hypotheses are satisfied for
$H_k$ at $k \neq 0$: the conformal vector is
$T = (1/(2k)){:}JJ{:}$, and the Sugawara antighost contraction
makes $\bC$-translations $Q$-exact.
\end{proof}

\begin{remark}[The abelian simplicity]
\label{rem:e-abelian-simplicity}
For the Heisenberg, all three obstructions dissolve
simultaneously because the algebra is abelian. Obstruction~(I)
is resolved by the finiteness of the shadow tower (class~G).
Obstruction~(II) is resolved by the free-field character of
$H_k^!$, which gives it canonical factorisation. Obstruction~(III)
is resolved by Koszulness, which makes the module--comodule
equivalence exact. The three obstructions are genuinely independent
for general algebras: class~M algebras (Virasoro) fail
obstruction~(I), non-Koszul algebras fail obstruction~(III), and
algebras whose Koszul duals have non-standard factorisation fail
obstruction~(II). The Heisenberg is the unique intersection where
all three vanish.
\end{remark}

\subsection{Proof for class L}

The closing conjecture for affine Kac--Moody algebras at
non-critical level follows from obstruction~(I) resolution
combined with the $\Ethree$ identification theorem.

\begin{lemma}[Conditional stalk-wise reduction for class L]
\label{lem:e-km-stalkwise}
Let $V_k(\fg)$ be the universal affine vertex algebra at
non-critical level $k \neq -h^\vee$, with $\fg$ simple. Assume the
Ran-space assembly of the completed module Drinfeld centre is
compatible with the PBW completion and with the degree-\(3\)
associator data. Then
\(\mathcal Z(V_k(\fg)\textup{-mod}^{\mathrm{compl}})\) is computed by
the stalk-wise centre at each point of \(\Ran(X)\): no \(\Ainf\)
correction at degree \(\ge 4\) contributes on this locus.
\end{lemma}

\begin{proof}
The shadow tower of $V_k(\fg)$ terminates at degree~$3$
(class~L: the OPE has a double pole and a simple pole,
giving $S_4 = 0$ and $\Delta = 0$). The $\Ainf$ structure
on the bar complex has $m_1$ (the bar differential),
$m_2$ (the deconcatenation coproduct), and $m_3$ (the triple
collision residue from the simple pole). All $m_k = 0$ for
$k \ge 4$. The stalk-wise Drinfeld centre at degree~$n$
is computed by the universal enveloping algebra of the
truncated $\Ainf$ operations $m_1, m_2, m_3$. Since no
operation at degree $\ge 4$ exists, the stalk-wise
computation at each point of $\Ran(X)$ sees the complete
$\Ainf$ data: no information is lost in the passage from
global to stalk-wise. The global assembly is a limit
indexed by the Ran space, and each stalk contributes the
full centre data.
\end{proof}

\begin{theorem}[Conditional closing comparison for class L]
\label{thm:e-closing-class-l}
For $V_k(\fg)$ with $\fg$ simple and $k \neq -h^\vee$, assume the
stalk-wise reduction, PBW-completed bar--cobar module comparison, and
CFG factorization-algebra identification hypotheses. Then the
\(\En\) operadic circle closes on this locus: there is an equivalence
of \(\Ethree^{\mathrm{top}}\)-algebras
\begin{equation}\label{eq:e-closing-class-l}
\mathcal Z(\mathrm{Mod}^{\mathrm{compl}}_{U_{V_k(\fg)}})
\;\simeq\;
\cZ^{\mathrm{der}}_{\mathrm{ch}}(V_k(\fg)).
\end{equation}
Both sides are isomorphic to the Costello--Francis--Gwilliam
$\Ethree^{\mathrm{top}}$-algebra from perturbative
Chern--Simons theory with gauge algebra $\fg$ at coupling
$\lambda = k + h^\vee$.
\end{theorem}

\begin{proof}
The proof proceeds via obstruction resolution and two
independent comparison paths.

\emph{Step~1: Obstruction~(I) resolution.}
By Lemma~\ref{lem:e-km-stalkwise}, the stalk-wise
centre computes the full Drinfeld centre for class~L.
The chiral centre theorem
(Theorem~\ref{thm:e-center-hochschild}) then identifies
\(\mathcal Z(V_k(\fg)\text{-mod}^{\mathrm{compl}})\) with the chiral Hochschild
cochains $C^\bullet_{\mathrm{ch}}(V_k(\fg), V_k(\fg))$.
By the theorem on resolution of any one obstruction
(Theorem~\ref{thm:e-one-obstruction}), the closing
conjecture reduces to a comparison of $\Etwo$-algebras
with matching $\Pthree$-structures.

\emph{Step~2: $\Pthree$ comparison.}
The $\Pthree$-structure on
$H^*(\cZ^{\mathrm{der}}_{\mathrm{ch}}(V_k(\fg)))
\cong C^*(\fg)$ is determined by the Gerstenhaber
bracket $\{\phi_a, \phi_b\} = \lambda\,f_{ab}{}^c\,\phi_c$
(Remark~\ref{rem:e-e3-to-p3}). The $\Pthree$-structure
on \(\mathcal Z(\mathrm{Mod}^{\mathrm{compl}}_{U_{V_k(\fg)}})\) is
determined by the braiding of
the Drinfeld double. For affine Kac--Moody at non-critical
level, the braiding is the KZ braiding, whose classical
limit is the $r$-matrix
$r(z) = k\Omega_{\mathrm{tr}}/z$ (trace-form convention).
The Drinfeld centre
carries the half-braiding
$\gamma_N \colon M \otimes N \to N \otimes M$ induced by
the $R$-matrix $R(z) = 1 + \lambda\,\Omega/z + O(\lambda^2)$,
whose first-order $\Pthree$ bracket on the centre
is $\{\phi_a, \phi_b\} = \lambda\,f_{ab}{}^c\,\phi_c$.
The two $\Pthree$ brackets agree.

\emph{Step~3: $\Ethree$ formality closes the circle.}
By $\Ethree$ formality
(Theorem~\ref{thm:e-e3-formality}), the $\Ethree$-structure
is determined by the $\Pthree$-structure on cohomology. For
simple $\fg$, $\dim H^3(\fg) = 1$, so the formal deformation
family over $\lambda H^3(\fg)[[\lambda]]$ is one-dimensional
at each order (Step~2 of the proof of
Theorem~\ref{thm:e-e3-identification}). Since both
$\Pthree$-brackets are controlled by the same single
parameter $\lambda = k + h^\vee$ and agree at each order,
the two $\Ethree$-algebras are isomorphic as formal
...
\cA^\lambda.
\]
The last identification is
Theorem~\ref{thm:e-e3-identification}.
\end{proof}

\begin{remark}[The two paths to the closing for class L]
\label{rem:e-two-paths-class-l}
The proof above resolves obstruction~(I) and then uses
$\Ethree$ formality. An alternative path resolves
obstruction~(III) directly: the bimodule--bicomodule
equivalence for $V_k(\fg)$ at non-critical level
transports the Drinfeld centre to the coalgebraic
coHochschild complex, which is identified with
$\ChirHoch^*(V_k(\fg))$ by the same
Hochschild--coHochschild duality used in the class~G
proof (Lemma~\ref{lem:e-heis-morita-hochschild}).
The obstruction to this path is that the bar--cobar
module equivalence for non-abelian $V_k(\fg)$ requires
the PBW completion technology of the ordered chiral
Koszul duality theory: the conilpotent comodules over
$\Barord(V_k(\fg))$ must be compared with complete
modules over $V_k(\fg)$ in the pro-nilpotent topology.
On the Koszul locus (which includes all standard affine
Kac--Moody algebras at non-critical level), this comparison
is exact and the two paths agree.
\end{remark}

\begin{remark}[Obstruction landscape after class G and L]
\label{rem:e-obstruction-after-gl}
With classes G and L proved, the closing conjecture
remains open only for class~M (Virasoro,
$\cW_N$) and for algebras beyond the standard
landscape. The surviving obstruction is the
conjunction of~(I) and the general topologisation
conjecture:
\begin{enumerate}[label=\textup{(\roman*)}]
\item For class~M, obstruction~(I) is
 genuine: the infinite shadow tower means the
 stalk-wise centre does not see the $\Ainf$
 corrections at degree $\ge 4$, and the global
 assembly requires coherent families of higher
 homotopies. Resolving this requires understanding
 the chain-level $\Ainf$-structure of the
 Drinfeld centre for algebras with infinite shadow
 tower: a problem equivalent to the chain-level
 topologisation problem.
\item For class~M, cohomological topologisation is
 now proved
 (Theorem~\ref{thm:e-topologisation-DS}),
 and the model-level $(\Ethree)_\infty$ follows
 (Proposition~\ref{prop:e-chain-level-virasoro}).
 The remaining gap is the original-complex
 chain-level lift
 (Conjecture~\ref{conj:e-topologisation-general}(ii)):
 the infinite $\Ainf$-coherence tower does not
 terminate, and applications requiring rigid
 chain-level data on the specific BRST complex
 remain conditional on the coderived resolution.
\end{enumerate}
The depth gap theorem ($d_{\mathrm{alg}} \in \{0, 1, 2, \infty\}$,
gap at $3$) implies there is no intermediate class between
L and M: the jump from finite shadow tower (classes G, L, C
with $d_{\mathrm{alg}} \le 2$) to infinite tower
(class M with $d_{\mathrm{alg}} = \infty$) is discontinuous.
The closing conjecture is therefore proved for all Koszul
$\Einf$-chiral algebras with finite shadow tower
(classes~G, L, C) that satisfy the topologisation
hypotheses, and remains open precisely for class~M.
\end{remark}

\subsection{Resolution of obstruction (III) on the Koszul locus}

The Hochschild--coHochschild strategy used for class~G
extends to all Koszul chiral algebras, providing a general
resolution of obstruction~(III).

\begin{proposition}[Obstruction (III) resolved on the Koszul locus]
\label{prop:e-obstruction-iii-koszul}
Let $\cA$ be a Koszul $\Einf$-chiral algebra with finite-type
grading. Set $C = \Barord(\cA)$. Then the chiral
Drinfeld centre of complete $\cA$-modules is isomorphic to
the derived chiral centre as $\Etwo$-algebras:
\begin{equation}\label{eq:e-obstruction-iii}
Z(\cA\textup{-mod}^{\mathrm{compl}})
\;\simeq\;
\cZ^{\mathrm{der}}_{\mathrm{ch}}(\cA).
\end{equation}
\end{proposition}
...
\begin{proof}
The argument follows the three-step structure of
Lemma~\ref{lem:e-heis-morita-hochschild}, with each
step valid on the Koszul locus.

\emph{Step~1.} The module Koszul duality theorem gives
a monoidal equivalence
$D(\mathrm{Mod}^{\mathrm{compl}}_\cA) \simeq
D(\mathrm{CoMod}^{\mathrm{conil}}_C)$
for Koszul algebras with finite-type grading. The
monoidality holds because the two-pointed bar complex
computes fusion on both sides.

\emph{Step~2.} The Drinfeld centre of
$\cA\text{-mod}^{\mathrm{compl}}$ is transported across
the monoidal equivalence to
$\mathrm{Coext}_{C^e}^*(C_\Delta, C_\Delta)$: the
coalgebraic self-extension of the diagonal bicomodule
in the coderived category of $C$-bicomodules.

\emph{Step~3.} The chiral Hochschild--coHochschild
duality (cohomological version) identifies
$\mathrm{Coext}_{C^e}^*(C_\Delta, C_\Delta)
\simeq \ChirHoch^*(\cA, \cA)
= \cZ^{\mathrm{der}}_{\mathrm{ch}}(\cA)$.

The $\Etwo$-structure is preserved at each step:
Step~1 preserves the monoidal structure (hence the
centre), Step~2 preserves the derived endomorphisms of
the identity (hence the half-braidings), and Step~3
preserves the Gerstenhaber bracket (the brace operations
on Hochschild cochains correspond to the composition product
on coalgebraic coextensions).
\end{proof}

\begin{remark}[Scope of the Koszul locus]
\label{rem:e-koszul-scope}
All standard chiral algebras in the landscape are Koszul:
Heisenberg, affine Kac--Moody (all levels), Virasoro,
$\cW_N$, $\beta\gamma$, $bc$, lattice vertex algebras.
Proposition~\ref{prop:e-obstruction-iii-koszul} therefore
resolves obstruction~(III) for the entire standard
landscape. The remaining obstructions for class~M are
(I)~the stalk-wise reduction and the general topologisation
conjecture: both are problems about the target
$\Ethree^{\mathrm{top}}$-algebra, not about the comparison
map.
\end{remark}
...
\bibitem{AF19} D.~Ayala and J.~Francis, \emph{Poincar\'e/Koszul
 duality}, Comm.\ Math.\ Phys.\ \textbf{365} (2019), 847--933.

\bibitem{BD04} A.~Beilinson and V.~Drinfeld, \emph{Chiral Algebras},
 AMS Colloquium Publications, vol.~51, 2004.

\bibitem{BDSK21} B.~Bakalov, A.~De Sole, and V.~G.~Kac,
...
\bibitem{CostelloGaiotto} K.~Costello and D.~Gaiotto, \emph{Vertex
 algebras and $3$d $\cN=4$ gauge theories}, J.\ High Energy Phys.\
 \textbf{2019} (2019), 018.
...
\bibitem{FultonMacPherson94} W.~Fulton and R.~MacPherson, \emph{A compactification
...
\bibitem{GTZ14} G.~Ginot, T.~Tradler, and M.~Zeinalian,
 \emph{Higher Hochschild homology, topological chiral homology
 and factorization algebras}, Comm.\ Math.\ Phys.\
 \textbf{326} (2014), 635--686.
...
\bibitem{KhanZeng25} A.~Z.~Khan and K.~Zeng, \emph{Poisson Vertex
 Algebras and Three-Dimensional Gauge Theory}, arXiv:2502.13227, 2025.

\bibitem{Knudsen18} B.~Knudsen, \emph{Higher enveloping algebras},
 Geom.\ Topol.\ \textbf{22} (2018), 4013--4066.
