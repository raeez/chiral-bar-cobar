...
families and by $\mathrm{av}(r(z))+\dim(\fg)/2$ for
non-abelian affine Kac--Moody, and its higher-degree
extensions. The ordered bar is the level-$2$ object before averaging; the
...
For Bershadsky--Polyakov: $\varkappa = 98/3$.
...
$K(\mathrm{BP}) = 196$, $K(V_k(\fg)) = 2\dim\fg$,
...
(values $K(\Vir) = 26$, $K(\cW_3) = 100$, $K(\mathrm{BP}) = 196$,
