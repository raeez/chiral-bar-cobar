...
\providecommand{\ClaimStatusConditional}{\textsuperscript{\textsc{conditional}}}
...
mediates between them; and the Koszul property says that bar
cohomology lies on the Koszul diagonal, equivalently that the
minimal resolution is linear. The theory is the
...
The shadow obstruction tower has scalar coordinates
$S_n=\tau_{\cA,n}^{\mathrm{res}}q_n((\Theta_\cA^{E_1})_{0,n})$.
At degree~$2$ this trace is $\kappa$ for abelian algebras and
$\kappa_{\mathrm{dp}}$ for non-abelian affine Kac--Moody, where the
Sugawara contribution $\dim(\fg)/2$ completes $\kappa$.
\end{theorem}
...
(Theorem~\ref{thm:intro-e1}) identifies the shadow tower as a traced
projection of the ordered Maurer--Cartan tower.  In arity~$n$, the
averaging map gives the equivariant operator
$q_n((\Theta_\cA^{E_1})_{0,n})$; the scalar shadow is obtained by
$\tau_{\cA,n}^{\mathrm{res}}$.  Represented $R$-matrices and Yangian
operators belong to the loci carrying their named representation and
flatness packages.
...
The level-$2$ object before averaging is the ordered bar complex
$B^{\mathrm{ord}}(\cA) = T^c(s^{-1}\bar{\cA})$ with its
deconcatenation coproduct and collision kernel
$K_\cA^{\mathrm{coll}}(z)\in\mathfrak{g}^{E_1}_\cA$. The shadow tower
$\{S_r\}$ and the algebraicity relation
$H(t)^2 = t^4\, Q_L(t)$ are statements about the
$\Sigma_n$-homotopy-coinvariant image.  At degree~$2$ the operator
class is $q_2(K_\cA^{\mathrm{coll}})$ and the scalar coordinate is
$\tau_{\cA,2}^{\mathrm{res}}q_2(K_\cA^{\mathrm{coll}})$; this equals
$\kappa$ in abelian and scalar families and $\kappa_{\mathrm{dp}}$
on the non-abelian affine row, where $\dim(\fg)/2$ completes
$\kappa$. Higher coordinates use $\tau_{\cA,n}^{\mathrm{res}}$ after
$\mathrm{av}\colon \mathfrak{g}^{E_1}_\cA \to
\mathfrak{g}^{\mathrm{mod}}_\cA$.

The full $E_1$ tower carries the spectral parameter~$z$ and, on the
named representation packages, the $R$-matrix and Yangian structure.
Homotopy coinvariants retain the equivariant operators. The scalar
tower in this paper applies $\tau_{\cA,n}^{\mathrm{res}}$ to those
operators. The ordered bar is the
...
scalar: $F_1(\cA) = \kappa(\cA)/24$. The Hodge class used in the
scalar genus tower is the top Chern class
\[
\lambda_g := c_g(\mathbb{E}_g) \in H^{2g}(\Mbar_g,\mathbb{Q}).
\]
The first Chern class of the Hodge line is written
\(\lambda_1=c_1(\det\mathbb E_g)\).
...
(coassociative, with ordered deconcatenation) and supports the
collision kernel $K_\cA^{\mathrm{coll}}(z)
\in\mathfrak{g}^{E_1}_\cA$. The shadow
obstruction tower $\{S_r\}$ and the algebraicity relation
$H(t)^2 = t^4\, Q_L(t)$ are statements about the
$\Sigma_n$-homotopy-coinvariant image. Its operator classes admit
scalar coordinates
$S_n=\tau_{\cA,n}^{\mathrm{res}}q_n((\Theta_\cA^{E_1})_{0,n})$.
In degree~$2$, this is $\kappa$ in abelian and scalar families and
$\kappa_{\mathrm{dp}}$ on the non-abelian affine row, where the
Sugawara term completes $\kappa$. The ordered bar is the level-$2$ object before averaging; the
...
% ================================================================
%
% This standalone gives the canonical presentation of the
% shadow obstruction tower. Six mathematical anchors fix the scope:
%
...
\begin{proof}
The verified Koszul hypothesis gives diagonal bar cohomology and
identifies the relevant weight-two obstruction component with an
off-diagonal summand. Hence
$H^2(\cA^{\mathrm{sh}}_{r,0}) = 0$ for that component. The
vanishing of this second cohomology group means that every
cocycle there is a coboundary, so the obstruction class vanishes.
...
vanishes only on the primary line. The global contact invariant is
$S_4^{\mathrm{cross}}=-5/12$ for the standard
$\beta\gamma_\lambda$ family, including the symplectic-boson point
$\lambda=1/2$. The
...
$S_4 = -5/12$, $S_r = 0$ for $r \ge 5$.
The internal weight-changing line is shadow-trivial, so the
class-$\mathbf{C}$ witness is the standard conformal-weight
family itself (Definition~\ref{def:class-C}).
\end{proof}
...
$\beta\gamma_\lambda$ (equivalently $bc_\lambda$) supplies the
unique finite nonlinear case beyond classes $\mathbf{G}$ and
$\mathbf{L}$:
$S_3 = 0$, $S_4 = -5/12$, and $S_r = 0$ for $r \ge 5$.
Hence $d_{\mathrm{alg}}$ can equal $2$, but no finite value
greater than $2$ occurs.
\end{remark}
...
$\mathbf{L}$ & $1$ & $m_3 \ne 0$, $m_k = 0$ for $k \ge 4$
 & $1$-formal \\
$\mathbf{C}$ & $2$ & $m_4 \ne 0$ on standard conformal-weight family
 & $2$-formal \\
$\mathbf{M}$ & $\infty$ & $m_k \ne 0$ for all $k$
 & non-formal \\
\bottomrule
...
The shadow obstruction tower $\{S_r\}_{r \ge 2}$ is obtained from
the homotopy-coinvariant ordered MC tower under the averaging
...
\item For non-abelian affine Kac--Moody $V_k(\fg)$
 (trace-form convention $K_\cA^{\mathrm{coll}}(z)=k\Omega_{\mathrm{tr}}/z$, where $\Omega$
...
$\Theta_\cA^{E_1}$ is the collision kernel
$K_\cA^{\mathrm{coll}}(z)$. Its homotopy coinvariant remains the
operator $q_2(K_\cA^{\mathrm{coll}})$, while the normalized trace is
the modular characteristic~$\kappa$ on abelian families. For
non-abelian affine Kac--Moody this trace is $\kappa_{\mathrm{dp}}$
and the full~$\kappa$ adds the
Sugawara correction from the simple-pole self-contraction of the
ordered two-point function. At higher degree the scalar coordinate is
$\tau_{\cA,r}^{\mathrm{res}}q_r((\Theta_\cA^{E_1})_{0,r})$.
\end{proof}
...
$\mathfrak{g}^{E_1}_\cA$ carries
$K_\cA^{\mathrm{coll}}(z)$ at degree~$2$. A spectral $R$-operator
arises on a represented CYBE/monodromy surface. The homotopy
coinvariant retains the binary operator and the normalized trace is
$\kappa$ for abelian families; for non-abelian affine Kac--Moody it
is~$\kappa_{\mathrm{dp}}$ and the full
...
\item \emph{Degree-$2$ content.}\enspace
 At degree~$2$, $q_2(K_\cA^{\mathrm{coll}})$ is the operator class.
 Its normalized trace is $\kappa$ for abelian algebras and
 $\kappa_{\mathrm{dp}}$ for non-abelian KM; the Sugawara term
 $\dim(\fg)/2$ completes the latter $\kappa$.
\end{enumerate}
\end{proposition}
...
The Koszul scalar complementarity $\kappa + \kappa' = 13$
(Virasoro-specific; not universal) reflects this fixed-point
structure. The scalar quantity $\kappa + \kappa^!$ is distinct from
the Trinity ghost-charge conductor $K(\Vir)=c+c^!=26$.
\end{remark}
...
\item \emph{Heisenberg.}\enspace
 ...
 (the curved second-kind chiral symmetric branch).
...
\begin{remark}[Family-dependence of the scalar complementarity]\label{rem:koszul-conductor}
The scalar complementarity invariant
$\varkappa(\cA) := \kappa(\cA) + \kappa(\cA^!)$
is \emph{not} universal: it depends on the family.
For affine Kac--Moody and free fields: $\varkappa = 0$.
For Virasoro: $\varkappa = 13$.
For $\cW_3$: $\varkappa = 250/3$.
For Bershadsky--Polyakov: conditionally $\varkappa = 25/3$.
The statement ``$\kappa + \kappa' = 0$'' without a family
qualifier is false for Virasoro and all $\cW_N$ with
$N \ge 2$.
The scalar quantity $\varkappa = \kappa + \kappa^!$ is distinct
from the Trinity ghost-charge conductor
$K(\cA) := c(\cA) + c(\cA^!) = -c_{\mathrm{ghost}}(\BRST)$, which
takes per-family values $K(\Vir) = 26$, $K(\cW_3) = 100$,
$K(\mathrm{BP}) = 50$, $K(V_k(\fg)) = 2\dim\fg$,
$K(\cW_N) = 4N^3 - 2N - 2$. The two are related by
$\varkappa = \varrho_{\cA}\cdot K$ with $\varrho_{\cW_N} = H_N - 1$,
...
correction; \ClaimStatusConditional]\label{prop:planted-forest}
In the signed planted-forest chain model, the predicted genus-$2$
planted-forest correction on a primary line is
\begin{equation}\label{eq:planted-forest}
 \delta_{\mathrm{pf}}^{(2,0)}
 \;=\;
 \frac{S_3(10\, S_3 - \kappa)}{48},
\end{equation}
provided the determinant-line signs make the planted-forest boundary
operator square to zero and the residue-pushforward transport from the
codimension-$2$ logarithmic Fulton--MacPherson boundary has been fixed.
With that orientation package in place, the expression measures the
contribution of codimension-$2$ boundary strata to the genus-$2$
amplitude. This correction vanishes for class~$\mathbf{G}$ (where
$S_3 = 0$). For Virasoro:
...
correspond to stable graphs with two internal edges. After choosing
the determinant-line orientation for the two possible contraction
orders, each edge carries the propagator $P = 1/\kappa$ and connects
two cubic vertices contributing $S_3$. The planted-forest graphs
(two disjoint trees planted on a genus-$1$ surface) contribute a
quadratic expression in $S_3$. The linear correction
$-\kappa\, S_3/48$ comes from the interaction between the planted tree
and the genus-$1$ curvature $\kappa$. The displayed formula is the
resulting chain-level computation; without the signed orientation and
residue-pushforward package it is a conditional model statement, not a
standalone geometric consequence of the logarithmic compactification.
\end{proof}
...
trace-form convention is $r(z) = k\Omega_{\mathrm{tr}}/z$ (with level
prefix $k$; at $k = 0$, $r(z) = 0$). Averaging:
$\mathrm{av}(k\Omega_{\mathrm{tr}}/z) = k\,\dim(\fg)/(2h^\vee)$.
...
class~$\mathbf{C}$: the standard conformal-weight family has
$S_3 = 0$, $S_4 = -5/12$, and $S_r = 0$ for $r \ge 5$.
Hence $r_{\max} = 4$,
$d_{\mathrm{alg}} = 2$.
...
$S_3 = 0$, $Q^{\mathrm{cont}} = S_4 = -5/12$, and
$S_r = 0$ for $r \ge 5$.
Thus the full family is class~$\mathbf{C}$, whereas the
weight-changing primary line by itself is shadow-trivial.
Hence the tower terminates at degree~$4$.
...
\begin{theorem}[Transferred Virasoro weighted-Riccati
coefficients]\label{thm:virasoro-coefficients}
\ClaimStatusConditional
Equip $\Vir_c$ with an ordered residue package
$\mathsf H_{\mathrm{res}}(\Vir_c;X)$ and a quadratic-transfer package
whose chain comparison identifies the transferred scalar brackets with
the one-dimensional weighted-Riccati master equation.  The resulting
geometric coefficients through degree~$10$ are
\begin{align*}
 S_2 &\;=\; c/2, \\[3pt]
 S_3 &\;=\; 2, \\[3pt]
...
$\beta\gamma$ family supplies the standard conformal-weight
class-$\mathbf{C}$ witness. The classification is:
\begin{itemize}
\item $\kappa = 0$: critical (not chirally Koszul in
 standard sense).
\item $S_4 = 0$, $\alpha = 0$: class $\mathbf{G}$.
\item $S_4 = 0$, $\alpha \ne 0$: class $\mathbf{L}$.
\item $S_3 = 0$, $S_4 \ne 0$, $S_r = 0$ for $r \ge 5$:
 class $\mathbf{C}$.
...
$\Vir_c$ & $13$ & no \\
$\cW_3$ & $250/3$ & no \\
Bershadsky--Polyakov & $25/3$ (conditional) & no \\
\bottomrule
\end{tabular}
\end{center}
...
$\kappa^! = -\kappa$. For Virasoro, the scalar complementarity
$\varkappa = 13$ is the self-dual value: $\kappa(c) + \kappa(26-c)
= c/2 + (26-c)/2 = 13$.

\begin{remark}[The scalar complementarity is not the Trinity
conductor]\label{rem:conductor-not-c}
The scalar complementarity $\varkappa = \kappa + \kappa'$ is an
invariant of the Koszul \emph{pair} $(\cA, \cA^!)$. It is
distinct from the Trinity ghost-charge conductor
$K(\cA) := c(\cA) + c(\cA^!) = -c_{\mathrm{ghost}}(\BRST)$
(values $K(\Vir) = 26$, $K(\cW_3) = 100$, $K(\mathrm{BP}) = 50$,
$K(V_k(\fg)) = 2\dim\fg$), and distinct again from the central
charge~$c$ (an invariant of the algebra alone). The three
quantities are related by $\varkappa = \varrho_{\cA}\cdot K$ and
$K = c + c^!$.
\end{remark}
...
The genus-$g$ scalar coefficient is a sum over stable graphs
...
\[
 ...
 -24\alpha S_4.
\]
The cancellation at $m=4$ is therefore not a standalone numerical
verification from the displayed unsymmetrized terms. The valid proof
is the symmetrized convolution argument of
Section~\ref{sec:riccati-proof}: the MC recursion is defined as the
negative of the convolution coefficient at each order, after the
same symmetry factors used in the Riccati equation.
...
Gevrey-$1$/Borel-summable structure of the three-dimensional gravity
comparison theorem and supersedes any
