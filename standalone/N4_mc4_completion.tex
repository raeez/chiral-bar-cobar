...
hypothesis we prove an arity cutoff lemma showing that each
...
is a sum over all bar arities, and a priori each component is
...
(i) the arity cutoff lemma (\S\ref{sec:cutoff}), which reduces
...
arity cutoff lemma then holds trivially and the completion is
...
proves the arity cutoff lemma. Section~\ref{sec:ml} proves
...
\]
involves only arities $r \le N$: on $\cA_{\le N}$ the bar
...
\item \emph{Completed MC element.} The componentwise limit
  ...
  being a finite sum by the arity cutoff lemma.
...
Lemma~\ref{lem:arity-cutoff}, modulo $F^{N+1}\cA$ only arities
...
Heisenberg-type contribution at bar arity $2$ and no
higher-arity operations. Under the conformal grading, the
...
hence higher-arity bar operations than Virasoro. The strong
