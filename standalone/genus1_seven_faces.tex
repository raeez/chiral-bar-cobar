...
\(\hbar_{\mathrm{ref}}=(k+h^\vee)^{-1}\), the KL half-monodromy
parameter is
\(q_{\mathrm{KL}}=\exp(\pi i\hbar_{\mathrm{ref}})\), while the DK
full pure-braid parameter is
\(q_{\mathrm{DK}}=q_{\mathrm{KL}}^2
=\exp(2\pi i\hbar_{\mathrm{ref}})\). For class~$M$ algebras (Virasoro,
...
The collision kernel $K_\cA^{\mathrm{coll}}(z)$ is the degree-$2$ component of
...
coalgebra with deconcatenation coproduct. Its homotopy coinvariant
$q_2(K_\cA^{\mathrm{coll}})$ retains the equivariant operator and its
spectral parameter.  The normalized functional
$\tau_{\cA,2}^{\mathrm{res}}$ gives $\kappa$ in abelian and scalar
families and $\kappa_{\mathrm{dp}}$ on the non-abelian affine row;
the Sugawara contribution $\dim(\fg)/2$ completes the latter modular
characteristic. At genus~$1$, the spectral parameter acquires
...
\(q_{\mathrm{KL}}=\exp(\pi i/(k+h^\vee))\) on the half-braid cover,
with full-cycle DK factor
\(q_{\mathrm{DK}}=q_{\mathrm{KL}}^2
=\exp(2\pi i/(k+h^\vee))\). The Verlinde partition function
at genus~$1$ is
...
eigenvalues are written on the KL half-braid cover with
\(q_{\mathrm{KL}}=e^{\pi i/(k+2)}\); the full pure-braid cycle is
the DK parameter \(q_{\mathrm{DK}}=q_{\mathrm{KL}}^2\). The passage from the
$E_1$ collision residue $r^{(1)}(z,\tau)$ to the quantum group
category is through this KL/DK bridge for the $B$-cycle monodromy of the ordered bar complex:
