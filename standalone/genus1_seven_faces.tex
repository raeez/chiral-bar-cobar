...
projections of the same $\Theta_\cA$. AP156: specify Weierstrass
$\zeta_\tau$ vs $\theta_1'/\theta_1$ convention; we use $\zeta_\tau$
throughout with Legendre identity $\eta_1\tau - \eta_\tau = \pi i$.
FM24 guard: $q = e^{2\pi i \hbar}$, not $e^{\pi i \hbar}$; substitute
integer $k$ to verify $|q|=1$.
\end{remark}
