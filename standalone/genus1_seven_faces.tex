...
\item The FULL symmetric bar cohomology at critical level computes the
algebra of differential forms on the space of $\fg^\vee$-opers:
$H^n(B^\Sigma(\widehat{\fg}_{-h^\vee})) \cong \Omega^n(\mathrm{Op}_{\fg^\vee}(D))$
in every degree $n \geq 0$ \textup{(}infinite-dimensional;
Feigin--Frenkel 1992, Frenkel--Teleman 2006\textup{)}.
...
FULL symmetric bar cohomology at critical level, the $d_1$ vanishing
lets the entire de~Rham algebra $\Omega^*(\mathrm{Op}_{\fsl_2}(D))$
survive as bar cohomology, which is infinite-dimensional.
\end{remark}
