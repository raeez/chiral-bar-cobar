...
\textbf{Family-indexed $\Etwo$ model.}
For a finite set $S\subset\mathbb Z$, a family datum
$H_H(\cA;S)$ identifies $\ChirHoch^n(\cA)$ with
$H^n(K_{\cA,S})$ and supports it in~$S$.  The $\Etwo$-formality
of this model is an additional hypothesis.  A perfect degree-$d$
pairing with $\cA^!$ is an additional duality datum.
...
and, with the additional package~$H_D^K$, the surviving Hodge
character at $g\ge1$ is
...
trace-form classical $r$-matrix
$r(z)=k\Omega_{\mathrm{tr}}/z$ admits no nontrivial infinitesimal
deformations.
...
filtration. Suppose condition~(v) is represented by a family datum
$H_H(\cA;S)$ and its cohomology is formal as an $\Etwo$-algebra.
Assume in addition:
\begin{enumerate}[label=\textup{(\alph*)}]
...
\item the transferred higher braces $\mathrm{br}_n$, $n\geq3$,
  carry specified null-homotopies compatible with the chiral cap
  product.
\end{enumerate}
...
\emph{Step 2: Cobar of the formal $\Etwo$-algebra.} The
...
ambient. The specified null-homotopies trivialize the transferred
brace operations $\mathrm{br}_n$ for $n\geq3$ in this model.
...
\begin{remark}[The typed Koszul objects]
\label{rem:kfc-four-objects}
Fix a quadratic presentation
$\cA=T_{\mathrm{ch}}(V)/(R)$.  Chiral Koszul duality uses the following
typed objects:
\begin{enumerate}
\item $\barBch(\cA) = T^c(s^{-1}\bar\cA)$: the bar
  \emph{coalgebra}, with deconcatenation coproduct and bar
  differential.
\item $\cA^i=C_X(s^{-1}V,s^{-2}R)$: the quadratic presentation
  \emph{coalgebra}, with canonical comparison
  $q_\cA\colon\cA^i\to\barBch(\cA)$.
\item $H^\bullet(\barBch(\cA))$: bar cohomology.  On the Koszul
  locus $H^\bullet(q_\cA)$ gives its identification with the
  cohomology of~$\cA^i$.
\item $K_X(\cA)=\mathbb D_{\Ran}\barBch(\cA)$: the Verdier-dual
  \emph{algebra}, with
  $\mathbb D(q_\cA)\colon K_X(\cA)\to\mathbb D_{\Ran}(\cA^i)$.
\item $\cA^!$: the chosen Koszul-partner algebra, reached by
...
\item $\cZ^{\mathrm{der}}_{\mathrm{ch}}(\cA) =
  ...
  resolution.
\end{enumerate}
The bar complex is the $\Eone$ engine;
$\Omega(\barBch(\cA))\xrightarrow{\sim}\cA$ is its inversion
counit.  Verdier duality forms~$K_X(\cA)$, and chiral Hochschild
cochains form the derived center.
\end{remark}
...
degree~$0$.  With the additional package~$H_D^K$, the Hodge character
is $\operatorname{ch}_g(\mathfrak O_g^K(\cA))
=(-1)^g\kappa(\cA)\lambda_g$.
...
trace-form classical $r$-matrix
$r(z)=k\Omega_{\mathrm{tr}}/z$ via the
...
For non-abelian affine KM, the normalized trace and Sugawara term give
\[
\tau_{k,\fg}^{\mathrm{res}}q_2(K_{k,\fg}^{\mathrm{coll}})
+\dim(\fg)/2=\kappa(V_k(\fg)),
\]
where the first term is $k\dim(\fg)/(2h^\vee)$, the
...
Scalar saturation is a statement about normalized traces of the
$\Sigma_n$-homotopy-coinvariant quotient. The full $\Eone$ ordered
bar carries the operator kernel
$K_\cA^{\mathrm{coll}}(z)\in\mathrm{End}_\cA(2)[\![z^{-1}]\!]$,
and $\kappa=\tau_{\cA,2}^{\mathrm{res}}
q_2(K_\cA^{\mathrm{coll}})$. Under
$H_{\mathrm{CYBE}}^{\mathrm{rep}}$, its represented image $r_\cA$
satisfies the classical Yang--Baxter equation
$[r_{12}, r_{13}] + [r_{12}, r_{23}] + [r_{13}, r_{23}] = 0$
and determines the Yangian structure when the quantization package
$H_Y$ is supplied.
...
$r^{\KM}(z) = k\Omega_{\mathrm{tr}}/z$, where
$\Omega_{\mathrm{tr}}$ is the inverse Killing-form Casimir and the
level prefix~$k$ is mandatory: at $k = 0$
the $r$-matrix vanishes (abelian limit).

The averaging map $\av_R \colon \fg_\cA^{\Eone} \to
\fg_\cA^{\mathrm{mod}}$ is the universal-conductor descent: in the
untwisted characteristic-zero splitting it is represented by the
$\Sigma_n$-coinvariant projection, and with nontrivial braiding it is
the $L_R$-twisted derived completed coinvariant descent. It is lossy:
the full $r$-matrix determines $\kappa$, but $\kappa$ does not
determine the $r$-matrix; the missing data live in the formal
conductor fibre. The five theorems of the parent monograph extract the
$\Sigma_n$-invariant content; the present paper works at this
invariant level, where scalar saturation gives full control at degree
two and Koszulness extends control to all degrees.
...
Lattice $V_L$ & (ii) PBW, lattice & G & $\mathrm{rk}(L)$ & $0$ \\
...
$\cW^{(2)}_k(\mathfrak{sl}_3)$ & (xii) Ext diag. & M & $c(k)/6$ & $25/3$ \\
\bottomrule
\end{tabular}
...
$\mathcal{H}_k^!=(\Symch(V^*[1]),m_0=-k\omega)$
...
and central charge, in the standard Fehily--Kawasetsu--Ridout
conformal-vector convention:
\[
 c(\mathrm{BP}_k) \;=\; -\frac{(2k+3)(3k+1)}{k+3}.
\]
The formula is singular at the critical level $k = -3 = -h^\vee(\mathfrak{sl}_3)$, where the Sugawara denominator $k+h^\vee$ of the parent $V_k(\mathfrak{sl}_3)$ vanishes; the self-dual point $k=-3$ of the involution $k \mapsto -k - 6$ coincides with this critical locus, and the value $\kappa(\mathrm{BP}_{-3}) = 25/6$ quoted below is the \emph{principal value} of the symmetric limit across the pole, not a regular function value. The polynomial identity $K_{\mathrm{BP}}(k) := c(k) + c(-k-6) \equiv 50$ holds in $\mathbb{Q}(k)$, and $c(k) - 25$ is an odd function of $(k+3)$. \textup{(}The explicitly shifted conformal vector $2 - 24(k+1)^2/(k+3)$ has reflection sum $196$; it belongs to a separate secondary lane.\textup{)}

At generic level, the proved input used here is the canonical
strictness/OPE normal form together with the computed BP bar witnesses.
The stronger assertion that this diagonal Ext condition is transported
from $V_k(\mathfrak{sl}_3)$ by non-principal DS reduction is conditional
on the subregular DS--bar transport package: DS reduction is a filtered
functor on primitive triples, not an equality of all structures, and
the level-shifted same-family duality remains conditional until the
DS--bar and Verdier-exchange hypotheses are supplied.

The modular characteristic is $\kappa(\cW^{(2)}_k) = c(k)/6$,
with anomaly ratio $\varrho = 1/6$, conditionally on the ratio lane
(the genus-one curvature computation remains open). The Koszul
complementarity conductor is then
...
\begin{proposition}[$\mathfrak{sl}_2$ at admissible level under the
quotient-bar package]
\label{prop:kfc-sl2-admissible}
Assume the rank-one admissible quotient-bar package: the quotient bar
spectral sequence for $L_k(\mathfrak{sl}_2)$ is constructed in a
finite-window complete ambient, PBW/Shapovalov data detect all higher
differentials, and the finite windows converge strongly. Then the
simple quotient $L_k(\mathfrak{sl}_2)$ is chirally Koszul at every
admissible level $k = -2 + p/q$ with $p \geq 2$, $q \geq 2$,
$\gcd(p,q) = 1$.
\end{proposition}

\begin{proof}[Proof sketch]
The finite Shapovalov and character computations show the expected
rank-one pattern: the first null vector is single-weight, the quotient
agrees with the universal algebra below the null weight, and the
checked finite windows have the same model cohomology as
$V_k(\mathfrak{sl}_2)$. These facts are evidence. The stated
proposition uses the extra quotient-bar package to rule out hidden
higher differentials and to pass from finite windows to the completed
simple quotient.
\end{proof}

\begin{remark}[Mechanism]
\label{rem:kfc-sl2-mechanism}
The rank-one evidence uses a small-rank accident: the null-vector
structure has only one Cartan direction and no higher-rank Cartan
plane. This is not, by itself, a dimensional proof of spectral-sequence
collapse; hidden differentials and nilradical effects are exactly what
the quotient-bar package must exclude.
\end{remark}
