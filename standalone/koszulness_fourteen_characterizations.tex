...
\item[(iii)] The fourteen atlas characterizations $\{U_j\}_{j=1}^{14}$ of Theorem~\ref{thm:kfc-fourteen} form a finite affine atlas covering $M_{\mathrm{Kosz}}(\mathcal A)$, each chart $U_j$ cut out by the associator coordinate $\Phi = \Phi_j$ (Kontsevich, Alekseev-Torossian, Hodge-Betti, Enriquez elliptic, Kontsevich integral).
...
\ClaimStatusProvedHere
The property $[\mathcal A \text{ chirally Koszul}] = [M_{\mathrm{Kosz}}(\mathcal A) \ne \emptyset]$ is $\mathrm{GRT}_1$-invariant. The chosen associator $\Phi$ controls only the \emph{characterization}, namely the witnessing predicate $\Pi_\Phi$. The fourteen classical characterizations are equivalent to Koszulness on their own $\Phi$-charts; four chart-dependent restrictions disappear after transfer to the appropriate associator.
\end{corollary}
...
\begin{remark}[The fourteen characterizations]
\label{rem:fourteen-characterizations-status}
