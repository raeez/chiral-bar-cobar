...
\subsection{The seven-fold TFAE at genus zero: hub-and-spoke
reconstitution}
\label{subsec:seven-fold-tfae-hub-spoke}

The fourteen characterizations above form an atlas on the
Koszulness moduli scheme. At genus zero, a strictly smaller
sub-atlas of \emph{seven} characterizations admits a sharper
presentation: the seven are mutually equivalent through a single
hub, so that the ostensibly $\binom{7}{2} = 21$ bidirections
reduce to six proved spokes plus transitivity.

\begin{theorem}[Seven-fold TFAE at genus zero, hub-and-spoke
reconstitution]\label{thm:seven-fold-tfae-hub-spoke-sa}
\ClaimStatusProvedHere
...
Corollary~\textup{\ref{cor:kfc-three-bijections}}. The following
seven conditions are equivalent \emph{through the PBW
$\Etwo$-collapse hub}:
...
For each $i \in \{1,2,3,4,5,7\}$ there is a single proved
bidirection $i \Leftrightarrow \textup{\ref{sv6}}$; the
seven-fold TFAE follows by transitivity from these six spokes.
No $\binom{7}{2} = 21$ independent bidirections are needed.

\textup{(Load-bearing non-tautology.)} The spoke
$\textup{\ref{sv4}} \Leftrightarrow \textup{\ref{sv6}}$
(twisted-tensor $\Leftrightarrow$ PBW) is the unique
non-tautological bidirection: both forward and reverse require
...
Warning~\textup{\ref{warn:koszulness-not-sc-formality}})}.
...
$\barBch$; spoke \textup{\ref{sv4}} by the non-tautological
bidirection with PBW cited above; spoke \textup{\ref{sv5}} by
Priddy's criterion in its chiral avatar; spoke \textup{\ref{sv7}}
by Proposition~\textup{\ref{prop:sc-formal-iff-class-g}} on
class~G. The transitivity closure is the elementary observation
