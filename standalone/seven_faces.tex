...
\begin{theorem}[Elliptic chiral QG equivalence
leading order $\hbar$]\label{thm:chiral-qg-equiv-elliptic-sf}
\ClaimStatusProvedElsewhere
For $\fsl_2$ on a smooth elliptic curve $E_\tau$, the Felder dynamical
$R$-matrix $R(z, \lambda, \tau)$ (built from the Jacobi theta
derivative $\theta_1'/\theta_1$, not the Weierstrass zeta; FM30) and
the Knizhnik--Zamolodchikov--Bernard connection determine each other
to leading order in $\hbar$. The Fay trisecant identity (equivalently
the Szegő three-term identity at $g=0,1$) provides the modular
compatibility that replaces the pentagon; the role of the Drinfeld
associator is played by the modular correction at the heat-equation
prefactor $1/(4\pi i)$ on the diagonal and $1/(2\pi i)$ off-diagonal.
\end{theorem}

\begin{theorem}[Toroidal chiral QG equivalence on the formal
disk]\label{thm:chiral-qg-equiv-toroidal-sf}
\ClaimStatusProvedElsewhere
The Drinfeld--Ginzburg--Miki toroidal algebra $U_{q,t}(\ddot{\fsl}_N)$
...
algebra on the instanton moduli space. Global extension to
$\PP^1 \times \PP^1$ is conditional on the class-$M$ chain-level
topologization frontier.
\end{theorem}
