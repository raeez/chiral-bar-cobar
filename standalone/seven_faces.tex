...
$r(z) = k\,\Omega_{\mathrm{tr}}/z \;\rightleftharpoons\; \Omega_{\mathrm{KZ}}/((k+h^\vee)z)$
...
$K_\cA^{\mathrm{coll}}(z)
=\Res^{\mathrm{coll}}_{0,2}(\Theta_{\cA}^{E_1})$ is the
degree-$2$ component of the $E_1$ Maurer--Cartan element
$\Theta^{E_1}_{\cA}$ in the ordered convolution algebra
$\mathfrak{g}^{E_1}_\cA$. Its homotopy coinvariant remains an
operator class. The normalized trace is $\kappa(\cA)$ in the abelian
and scalar families and $\kappa_{\mathrm{dp}}$ on the non-abelian
affine row, where $\dim(\fg)/2$ completes $\kappa$:
...
the simple-pole self-contraction channel). The operator
$q_2(K_\cA^{\mathrm{coll}})$ retains the full spectral-parameter
dependence; the seven faces are seven ways of reading this
$E_1$ data.
...
\label{rem:three-invariants-depth-gap}
...
\begin{theorem}[Seven typed comparison maps]\label{thm:seven-faces}
Let $\cA$ be a chirally Koszul chiral algebra in the standard
landscape, and let
$K_\cA^{\mathrm{coll}}(z)
=\Res^{\mathrm{coll}}_{0,2}(\Theta_{\cA}^{E_1})$ be its collision
kernel in $\operatorname{End}_\cA(2)\otimes\cO(*\Delta)$.  Assume
the representation, flatness, Yangian, PVA, Sklyanin, and Gaudin
comparison packages named in the seven clauses below.  The following
maps have common source $K_\cA^{\mathrm{coll}}$ and compatible images:
\begin{enumerate}[label=\textup{\textbf{(F\arabic*)}}]
 \item \emph{Twisting morphism}\/ ($E_1$ data):
 $K_\cA^{\mathrm{coll}}(z)$ is the
 ...
 \item \emph{Line operator $R$-twist}\/ ($E_1$ data):
 $\rho_{\mathrm{line}}(K_\cA^{\mathrm{coll}})$ is
 ...
 \item \emph{Classical PVA $r$-matrix}\/ ($E_\infty$ shadow):
 $\rho_{\mathrm{PVA}}(K_\cA^{\mathrm{coll}})$
 ...
 the coefficients of $\rho_{\mathrm{GZ}}(K_\cA^{\mathrm{coll}})$ generate the
 ...
 \item \emph{Yangian $r$-matrix}\/ ($E_1$ data):
 $\rho_Y(K_\cA^{\mathrm{coll}})$ is the
 ...
 \item \emph{Sklyanin bracket}\/ ($E_\infty$ shadow):
 $\rho_{\mathrm{Sk}}(K_\cA^{\mathrm{coll}})$ is
 ...
 \item \emph{Gaudin generator}\/ ($E_\infty$ shadow):
 $\rho_{\mathrm{Gau}}(K_\cA^{\mathrm{coll}})$ is
...
is the degree-$2$ datum of its Maurer--Cartan element. The faces do
not all sit at the same operadic level. F1 records the ordered bar
coalgebra, F2 and F5 record spectral exchange/coproduct realizations
of the same binary residue, F3 is the PVA infinitesimal shadow, F4 is
a flat-connection/commuting-differential realization, and F6--F7 are
simple-pole classical Hamiltonian shadows. Their common datum is the
binary collision residue after the named comparison maps; the
face-space statement is not a closed $E_n$-cycle. The averaging map
$\mathrm{av}\colon\mathfrak{g}^{E_1}_\cA\to\mathfrak{g}^{\mathrm{mod}}_\cA$
projects the collision kernel to its homotopy-coinvariant operator.
The normalized functional gives $\kappa$ in abelian and scalar
families and $\kappa_{\mathrm{dp}}$ on the non-abelian affine row,
where $\dim(\fg)/2$ completes $\kappa$. At higher arity,
$S_n=\tau_{\cA,n}^{\mathrm{res}}q_n((\Theta_\cA^{E_1})_{0,n})$.

The $E_n$ passage has arrow-by-arrow status. The codimension-$2$
restriction is proved on the Costello--Gwilliam/Costello--Gaiotto
Kac--Moody locus, not for an arbitrary class-$M$ algebra. The ordered
bar arrow is algebraic on the Koszul locus. The Drinfeld-centre arrow
constructs categorical half-braidings; it is not the averaging map.
The higher-Deligne/topologisation arrow gives an $E_3$-topological
structure for affine Kac--Moody at non-critical level, while the
general class-$M$ chain-level lift remains conjectural. Thus the
round trip closes only on the named comparison loci; elsewhere it is
an open corridor, not a constructed circle. The seven faces live at
the $E_1$ collision-residue stage, before the $E_2$/$E_3$
reconstruction hypotheses are invoked.
...
\begin{enumerate}[label=\textup{(\roman*)}]
 ...
 $k\,\Omega_{\mathrm{tr}}/z \;\rightleftharpoons\;\Omega_{\mathrm{KZ}}/((k+h^\vee)z)$ as the
...
\begin{remark}[GRT-torsor resolution of the normalization mismatch]
The attempted chain treats
$k\,\Omega_{\mathrm{tr}}/z = \Omega_{\mathrm{KZ}}/((k+h^\vee)z)$ as a numerical identity and
finds it false (at $k=1$, $h^\vee=2$ for $\fsl_2$:
...
%%% ================================================================
%%% 3.9 ORDERED-KOSZUL ANCHORS: CHIRAL QG EQUIVALENCE ON THE ORDERED KOSZUL LOCUS
%%% ================================================================
...
\begin{theorem}[Conditional $\mathrm{GRT}_1$ rigidity on the bar
...
--- are $\mathrm{GRT}_1(\bQ)$-trivial on $H^0$ only under the
fixed comparison package used for the verified affine checks:
changing $\Phi$ changes the associator representative, while the
listed finite bar-shadow classes remain equal. This is not a theorem
that categorical modular data, the Drinfeld centre, or a
root-of-unity BKM $S$-matrix is $\mathrm{GRT}_1$-trivial. At the
cochain level the action is homotopic only after the relevant gauge
datum is supplied, and it is not trivial: associator-dependence
concentrates in the coproduct/cobar side, not in the finite bar
shadow.
\end{theorem}
...
The formal-disk inscription is the Vol~III proposition on the algebra
side of toroidal Kazhdan--Lusztig for $\mathfrak{sl}_2$ on the formal
disk, extended to $\fsl_N$ via the
...
CoHA. On the formal disk the equivalence is bialgebra-level
	(the vertex-algebraic antipode lift is a separately proved
...
\begin{remark}[Coefficient check]\label{rem:miura-retraction-sf}
The coefficient is $(\Psi-1)/\Psi$, not $1/\Psi$. At
$\Psi = 2$ the two expressions agree numerically. At
$\Psi = 3$ they separate, with $(\Psi-1)/\Psi = 2/3$ and
$1/\Psi = 1/3$.
\end{remark}
...
$G$ (Gaussian) & $2$ & quadratic
 & Heisenberg, lattice VOAs, free fermion \\
\hline
$L$ (Lie/tree) & $3$ & quadratic
 & Affine $\widehat{\fg}_k$ all simple types, non-critical \\
\hline
$C$ (Contact) & $4$ & simple-pole
 & $\beta\gamma$, $bc$ ghosts, symplectic fermions, matrix factorizations \\
\hline
...
\hline
$\beta\gamma$, $bc$ ghosts & $1$ & $0$ & $4$ & trivial & $C$ \\
\hline
...
because it would require $p_{\max}=3$, which is excluded by
...
because the pole-valued collision \(r_{\mathrm{coll}}\) of
\(\Theta_{\beta\gamma}\) is zero after \(d\log\)-absorption. The raw
ordered contraction \(\beta(z)\gamma(w)\sim(z-w)^{-1}\) is nonzero,
and the regular contact transport
\(\Theta_{\beta\gamma}^{\mathrm{ord}}
=\beta\otimes\gamma-\gamma\otimes\beta\)
is the surviving ordered datum. The shadow depth $r_{\max}=4$ of
$\beta\gamma$ comes not
...
%%% ================================================================
%%% 6. SCOPE AND CONJECTURAL EXTENSIONS
%%% ================================================================

\section{Scope and remaining conjectural extensions}\label{sec:status}
...
\subsection{Remaining conjectural extensions}

Three conjectural extensions remain in the seven-face programme:
...
These extensions do not affect the master theorem on the standard
landscape; they precisely record the scope boundaries.

\subsection{Type distinctions required in applications}

Three type distinctions govern applications of the seven-face programme:
...
We close with the observation that the seven-face programme is a
\emph{consistency test}: every time a new face (an eighth framework,
