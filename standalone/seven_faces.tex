...
\begin{remark}[Four Yangian types, discipline
AP159]\label{rem:four-yangian-types-sf}
...
\end{enumerate}
Conflating these is the signature of AP159. The chiral QG
...
an associator gauge $\Phi \in \mathrm{GRT}_1(\bQ)$ (AP171); the bar
side is associator-free on $H^0$ by
Theorem~\ref{thm:grt1-rigidity-sf}.
\end{theorem}

\begin{remark}[Scope, AP174]\label{rem:chiral-qg-scope-sf}
...
remaining two arrows factor through the operadic hub (AP175). This
...
\begin{theorem}[Miura cross-universality at all spins,
AP128/FM31 corrected]\label{thm:miura-cross-universality-sf}
\ClaimStatusProvedElsewhere
...
1/\Psi = 1/2$): this coincidental agreement was the source of AP128,
and the Beilinson discipline (verify at three parameter values
AP186) diagnosed it by substituting $\Psi = 3$, where $(\Psi-1)/\Psi
= 2/3$ but $1/\Psi = 1/3$. The universal coefficient is
$(\Psi-1)/\Psi$.
\end{remark}
