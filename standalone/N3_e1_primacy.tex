...
is the arity-$2$ coinvariant of a meromorphic $r$-matrix $r(z)$ that
lives natively on the ordered side. The Drinfeld associator is the
arity-$3$ coinvariant of the KZ connection. The five foundational
...
splittings, and the cross-arity leakage that obstructs a dg Lie
...
arity $n$, the coshuffle structure with $2^{n}$ terms, and the
...
A through D, together with the Hochschild theorem H) lift to the
...
is the content of the arity-$2$ genus-$0$ Maurer--Cartan element
...
\item The \emph{Harrison coLie} (or co-Lie) cooperad
  $\Lie^{c}$, whose arity-$n$ component has dimension $(n-1)!$ and
  whose coproduct decomposes a Lie word into pairs of sub-Lie-words
  using the Harrison differential.
\item The \emph{coshuffle cooperad} $\Sym^{c}$, whose arity-$n$
...
\item The \emph{deconcatenation cooperad} $T^{c}$, whose arity-$n$
...
of information. At arity $3$ the Harrison differential sees
...
structures: they have different arity-$n$ term counts
...
desuspended generator has degree $|s^{-1}J| = 0$. At arity $2$
...
a single pole $k/(z_{1}-z_{2})$ (AP126: the level $k$ survives
the $d\log$ absorption; at $k=0$ the $r$-matrix vanishes).
\end{example}
...
\begin{theorem}[Arity-$2$ averaging: $\av(r(z)) = \kappa$]
\label{thm:av-arity-2}
Let $r(z) \in \End(V \otimes V) \otimes \cO(*\Delta)$ be the
genus-$0$ arity-$2$ $E_{1}$ shadow of $\cA$: the arity-$2$
...
\begin{proof}
The arity-$2$ Maurer--Cartan component is, by definition, the
arity-$2$ $E_{1}$ shadow
...
limiting case (at $k=0$, where $r$ vanishes by AP126), and
cross-family consistency against the landscape census.
\end{proof}

\begin{theorem}[Arity-$3$ averaging: associator to cubic shadow]
\label{thm:av-arity-3}
Let $\Phi_{\mathrm{KZ}}(\cA) \in \End(V^{\otimes 3})$ be the
genus-$0$ arity-$3$ Maurer--Cartan component, which for affine
...
\end{equation}
does not split as dg Lie algebras. At each fixed arity $n$, the
...
arity-$n$ extension $2$-cocycle vanishes. The obstruction is
cross-arity: the bar differential reduces arity by one and is
...
scalar modular characteristic $\kappa$ is the arity-$2$
...
At each genus $g$ and arity $n$ with $2g-2+n > 0$, the ordered
...
\begin{theorem}[Theorem~$\mathrm{D}^{E_{1}}$: arity-$2$ package]
...
\end{equation}
obtained from the genus-refined arity-$2$ projection of the
$E_{1}$ Maurer--Cartan element is the $E_{1}$ arity-$2$
...
the content of Theorem~\ref{thm:av-arity-2}. A full higher-genus
...
number per family (AP1: $\kappa^{\KM}$, $\kappa^{\Vir}$, and
$\kappa^{W_{N}} = c(H_{N}-1)$ are distinct per family, never to be
copied; the harmonic number is $H_{N} = \sum_{j=1}^{N} 1/j$,
AP116). Its arity-$2$ lift is a meromorphic function of a single
...
interest. At arity $2$, $\ker(\av)$ is the antisymmetric part of
...
on the bar complex. At arity $3$, $\ker(\av)$ contains the
...
corresponding to how the arity-$2$ Maurer--Cartan component
$r(z)$ is constructed. The tiers are a refinement of the
$E_{n}$-hierarchy restricted to the arity-$2$ slice; only the
...
singularity for any $a, b \in \cA$), then the arity-$2$ collision
...
poles in its OPE), then $r(z)$ is determined by the arity-$2$
...
$\widehat\fg_{k}$ with $r(z) = k\Omega_{\fg}/z$ (AP126: the
level $k$ must survive the $d\log$ absorption; the $k=0$ limit
must return zero), Virasoro $\Vir_{c}$ with $r(z) = (c/2)/z^{3}
...
structure; the $E_{1}$ refinement adds no new data at arity $2$
...
arity-$2$ Maurer--Cartan element is the rational Yang--Baxter
...
$\cA^{!}$; Hochschild cochains give the bulk. The averaging map
...
the arity-$2$ coinvariant of the spectral $r$-matrix $r(z)$, and
...
Theorem~\ref{thm:e1-theorem-d} packages the ordered arity-$2$
shadow into a formal series
$R^{E_{1},\mathrm{bin}}(z;\hbar)$ whose $g$-th coefficient is the
ordered arity-$2$ correction at genus $g$. Whether this formal
...
Swiss-cheese arity $(k,m)$. The averaging map restricted to the
