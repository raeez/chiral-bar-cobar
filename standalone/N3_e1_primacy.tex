...
quasi-triangular deformation data only after the source Lie bialgebra,
\(\hbar\)-adic topology, associator, and QUE/quasi-Hopf target are
fixed~\cite{EtingofKazhdan96}.
\end{proof}
...
The scalar $\kappa$ is one number per family, while its ordered source
is the meromorphic collision kernel $K_\cA^{\mathrm{coll}}(z)$.  For
Heisenberg,
$\tau_{\cH,2}^{\mathrm{res}}q_2(K_\cH^{\mathrm{coll}})=\kappa$; for
non-abelian affine Kac--Moody the corresponding trace is
$\kappa_{\mathrm{dp}}$ and the Sugawara channel adds $\dim(\fg)/2$.
The coinvariant $q_2(K_\cA^{\mathrm{coll}})$ remains operator-valued.
\end{remark}
...
\item \emph{Derived centre.} The chiral Hochschild cochain
 ...
 In the open-closed picture this is the cochain-level closed-sector
 actor: the centre of the open (line-operator) algebra $\cA$ is the
 closed-sector entry. A physical bulk interpretation requires an
 open--closed comparison datum. This functor is \emph{not} the
 bar--cobar composition; it is a separate construction, the subject
 of Theorem H.
\end{enumerate}
...
formal (Livernet), and the argument routes around formality. Concretely, $\Phi(b)_n$ is the chiral
Hochschild cochain
...
scalar degree-$2$ shadow is
$\tau_{\cA,2}^{\mathrm{res}}q_2(K_\cA^{\mathrm{coll}})$, giving $\kappa$
for abelian families and $\kappa_{\mathrm{dp}}$ for non-abelian
affine Kac--Moody before the Sugawara shift $\dim(\fg)/2$ is
added.  The intermediate coinvariant retains its equivariant operator
class.
