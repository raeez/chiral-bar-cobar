...
$\Sigma_n$-coinvariant image under an averaging map
...
and ordinary only in the pole-free locus. It is a surjective morphism
of dg Lie algebras; all
...
yet proved in the form stated here. A proved
...
bar is its $R$-twisted coinvariant shadow when descent exists; the five theorems
...
% ================================================================
\appendix
\section{Ordered Koszul chiral algebras, Yangians, and chiral QG}
\label{sec:n3-note-added}
% ================================================================

Three $E_1$-chiral results answer the open problems of
\S\ref{sec:open-problems}: ordered Koszul chiral algebras, the
Yangian instance, and the ordered chiral quantum-group equivalence.

\subsection*{Definition of ordered Koszul chiral algebra}

\begin{definition}[Ordered Koszul chiral algebra; N3-NIP]
\label{def:n3-ordered-koszul}
A chiral algebra $\cA$ on a smooth curve $X$ is \emph{ordered
Koszul} if:
\begin{enumerate}[label=\textup{(\alph*)},leftmargin=2.2em]
\item the ordered bar complex
$\Barord(\cA)=T^c(s^{-1}\bar\cA)$ with deconcatenation coproduct
carries a deformation-retract onto its cohomology as a coalgebra
over $(\mathrm{ChirAss})^!$;
\item the counit $\Cobar\bigl(\Barord(\cA)\bigr)\to\cA$ is a
quasi-isomorphism in the Francis--Gaitsgory factorization ambient;
\item the $R$-twisted descent from ordered to symmetric bar
(Vol~I \verb|lem:R-twisted-descent|, under Yang--Baxter plus
unitarity $R(z)\,R^{op}(-z)=\id$; automatic only after scalar
normalization for rational Yangians and generic-$q$ trigonometric
$U_q(\widehat{\fg})$, vacuous when
$R(z)=\tau$) is compatible with the chiral
twisting morphism.
\end{enumerate}
\end{definition}

This is the formal counterpart of \texttt{def:ordered-koszul-chiral-algebra}
in Vol~I \verb|chapters/theory/chiral_qg_equiv_ordered.tex|.

\subsection*{Yangian ordered Koszul (proposition)}

\begin{proposition}[$Y_\hbar(\fg)^{\mathrm{ch}}$ is ordered Koszul]
\label{prop:n3-yangian-ordered-koszul}
The chiral Yangian $Y_\hbar(\fg)^{\mathrm{ch}}$, defined via the
Drinfeld second presentation and Etingof--Kazhdan quantisation
flatness, is an ordered Koszul chiral algebra in the sense of
Definition~\textup{\ref{def:n3-ordered-koszul}}.
\end{proposition}

\begin{proof}[Proof sketch]
By Vol~I \verb|prop:yangian-ordered-koszul|
(\verb|chapters/theory/chiral_qg_equiv_ordered.tex|). The
Drinfeld second-presentation generators assemble an ordered bar
complex whose cohomology is computed by pure-braid
Orlik--Solomon (Shelton--Yuzvinsky Koszulity), and
Etingof--Kazhdan flatness supplies the chiral twisting morphism.
\end{proof}

This upgrades the open problem ``The full $E_1$ Koszul duality
theorem'' of \S\ref{sec:open-problems} to: \emph{Koszul duality
holds on the ordered Koszul locus, and the Yangian is a
concrete instance}.

\subsection*{Chiral QG equivalence on the ordered Koszul locus}

\begin{theorem}[Chiral quantum-group equivalence, ordered Koszul locus]
\label{thm:n3-chiral-qg-equiv-ordered}
On the ordered Koszul locus (Definition~\textup{\ref{def:n3-ordered-koszul}}),
three structures on a chiral algebra~$\cA$ determine each other
up to Drinfeld associator:
\begin{enumerate}[label=\textup{(\roman*)},leftmargin=2.2em]
\item the classical $R$-matrix $r_\cA(z)$;
\item the $A_\infty$-structure on $\Barord(\cA)$;
\item the deconcatenation coproduct on $\Barord(\cA)$ with
 compatible chiral twisting morphism.
\end{enumerate}
Cohomologically-derived Center is associator-INDEPENDENT (verified
for $\mathfrak{sl}_2$ at order~$\hbar$); cochain-level data is
associator-DEPENDENT with the dependence classified by
$\mathrm{GRT}_1$.
\end{theorem}

\begin{proof}[Proof sketch]
Vol~I \verb|thm:chiral-qg-equiv-ordered|
(\verb|chapters/theory/chiral_qg_equiv_ordered.tex|), with
explicit $\mathfrak{sl}_2$ Yangian triangle
(\verb|prop:sl2-yangian-triangle-concrete|) demonstrating triangle
coherence $\alpha\circ\gamma\circ\beta=\mathrm{id}$ at
order~$\hbar$.
\end{proof}

\subsection*{Two further ordered-locus theorems}

\textbf{(N3-NIP.a) $\mathrm{gl}_N$ Drinfeld-double internal.}
Vol~I \verb|thm:glN-drinfeld-double-internal|
(\verb|chapters/theory/chiral_qg_equiv_ordered.tex|) bypasses the
JKL26 phantom dependency: the $\mathrm{gl}_N$ chiral Drinfeld double
is constructed internally via Miura + cross-arity Stasheff +
$U(\hat{\mathfrak{gl}}_1)$ screenings. Two boundary parameters:
degenerate $\Psi=0$ recovers the free abelian chiral algebra
(identity quantum group); $\Psi=1$ is the free-boson point where
the OPE obstruction vanishes.

\textbf{(N3-NIP.b) $\cW_\infty$ chiral QG weight-completed.}
\verb|thm:w-infty-chiral-qg-completed| resolves the class-$\mathsf{M}$
contradiction through the weight-completed category, in parallel
with the MC5 weight-completed route.

\textbf{(N3-NIP.c) $\mathrm{GRT}_1$-rigidity of cohomological
center.}
\verb|thm:grt1-rigidity| makes associator-dependence explicit:
\emph{trivial on $H^0$; homotopic on cochains}. This sharpens
Proposition~\ref{prop:nonsplitting} of the present note:
the $\mathrm{GRT}_1$-torsor of splittings acts trivially on
the derived center at the $H^0$ level but non-trivially at the
cochain level.

\subsection*{Elliptic and toroidal extensions}

\textbf{(N3-NIP.d) Elliptic.}
\verb|thm:chiral-qg-equiv-elliptic| covers the genus-$1$ case with
Felder dynamical $R(z,\tau)$ and Fay trisecant replacing the
pentagon identity. Fay trisecant extends to all $g\ge 0$
universally; the distinct theta-nulls quartic identity on the
theta divisor is a separate statement at $g\ge 2$.

\textbf{(N3-NIP.e) Toroidal formal disk.}
\verb|thm:chiral-qg-equiv-toroidal-sf| of
\texttt{standalone/seven\_faces.tex} (inscribed; not an
\texttt{\textbackslash input} of \texttt{main.tex}, hence a
standalone inscription whose canonical formal-disk home is
Vol~III \texttt{quantum\_groups\_foundations.tex}) gives, via
Schiffmann--Vasserot CoHA plus Ding--Iohara--Miki presentation of
$U_{q,t}(\ddot{\mathfrak{sl}}_N)$ with two spectral parameters
$(u,v)$, chiral QG equivalence at the formal-disk level. The
earlier \texttt{thm:chiral-qg-equiv-toroidal-formal-disk} label
was never inscribed as a \texttt{\textbackslash label} anywhere in
Vol~I; the canonical standalone label is the \texttt{-sf} form
above. The global $\mathbb{P}^1\times\mathbb{P}^1$ case is
conditional on the class-$\mathsf{M}$ chain-level topologization
frontier on the original bar complex in $\Ch(\Vect)$.

\subsection*{Averaging and the Sugawara shift: precise statement}

For clarity we repeat the Sugawara-shift correction at degree~$2$
(already present in the main text, here aggregated for reference):
\begin{itemize}[leftmargin=1.8em]
\item \emph{Heisenberg $\cH_k$:}
$\av(r(z))=\kappa(\cH_k)=k$ (no shift; the $r$-matrix is scalar).
\item \emph{Non-abelian affine Kac--Moody $V_k(\fg)$:}
$\av(r(z))=\kappa_{\mathrm{dp}}(V_k(\fg))=k\dim(\fg)/(2h^\vee)$;
the full
$\kappa(V_k(\fg))=\av(r(z))+\dim(\fg)/2=\dim(\fg)(k+h^\vee)/(2h^\vee)$
adds the Sugawara shift $\dim(\fg)/2$.
\item \emph{Virasoro $\Vir_c$:}
$r(z)=(c/2)/z^3+2T/z$ (cubic and simple poles); averaging
gives $\kappa(\Vir_c)=c/2$ without a further shift.
\item \emph{Principal $\cW_N$:}
$\kappa(\cW_N)=c\cdot(H_N-1)$ with $H_N=\sum_{j=1}^N 1/j$
(NOT $H_{N-1}$; at $N=2$, $H_2-1=1/2$ recovers $\kappa(\Vir_c)=c/2$).
\end{itemize}
