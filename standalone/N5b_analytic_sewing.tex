...
\begin{abstract}
A sufficient analytic sewing criterion is established for chiral
algebras. The sewing envelope $\cA^{\mathrm{sew}}$ is the locally
convex completion of an algebraic chiral core determined by
coefficient seminorms before convergence and by compact-open matrix
coefficient seminorms on convergence domains. A Hilbert--Schmidt
estimate on the sector multiplication gives trace-class closed loop
amplitudes after $q$-damping. In the Heisenberg case, the disk
completion is modeled by
the symmetric algebra of the Bergman space; the second-quantized
Fredholm determinant formula is valid under the usual trace-class
implementability hypotheses. For interacting families the criterion is
conditional on a positive Hilbert model and polynomial OPE bounds.
\end{abstract}
...
For a general chiral algebra, the bar complex
...
The Hilbert--Schmidt condition is a single analytic hypothesis
strong enough to imply the closed-loop estimates needed in sewing
and concrete enough to be checked from OPE data. Let
$H = \widehat{\bigoplus}_{n \geq 0}
...
\;<\; \infty
\end{equation}
for some $0 < q < 1$. Proposition
\ref{prop:closed-trace-class} gives the corresponding trace-class
criterion for the \(q\)-damped kernel: closed sewing loops with the
specified Hilbert--Schmidt factors are trace-class on the weighted
completion $H_q := \widehat{\bigoplus}_n q^n H_n$. Genus-$g$
partition functions and correlators are then genuine numbers once
the remaining open-amplitude bounds are supplied.

The Heisenberg case verifies the criterion in the sharpest available
form. Moriwaki~\cite{Moriwaki26b}
identifies the Heisenberg sewing envelope with the symmetric
algebra $\Sym A^2(D)$ of the Bergman space of the disk. The
pair-of-pants amplitude is the second quantization
$\Gamma(R)$ of a one-particle Bergman restriction operator $R$,
and the Fredholm determinant expression follows whenever the bosonic
second quantization is trace-class. In the standard free-boson
normalisation, the genus-$g$ partition function for a rank-$r$
Heisenberg VOA is
\begin{equation}\label{eq:heisenberg-partition-intro}
Z_g(\cH^{(r)};\,\Omega)
\;=\;
(\det\operatorname{Im}\Omega)^{-r/2}
\cdot (\det{}'_\zeta \Delta_{\Sigma_g})^{-r/2},
\end{equation}
in agreement with the Belavin--Knizhnik~\cite{BK86} string measure.

The final criterion turns~\eqref{eq:hs-intro} into a checkable
hypothesis. A positive-energy chiral algebra with subexponential
...
checked from character formulas and OPE tables. The criterion holds
on the standard landscape for the Heisenberg $\cH_k$,
affine Kac--Moody $V_k(\fg)$ at integer level, Virasoro
$\Vir_c$, the $\cW_N$ algebras, and lattice vertex algebras
$V_\Lambda$, subject in the interacting cases to a positive Hilbert
model satisfying the stated OPE bounds.
...
in $\operatorname{IndHilb}$. In the present formulation it appears
as a one-particle reduction of the pair-of-pants operator, which is
the source of the Fredholm expression
\eqref{eq:heisenberg-partition-intro}.
...
Let $\Sigma$ be a finite conformally flat bordism whose sewing
parameters lie in a convergence polydisc $U$. For every compact
$K\Subset U$ and every choice of boundary vectors
$\xi_i \in \cA_{\mathrm{alg}}$, $\eta \in \cA_{\mathrm{alg}}^\vee$,
define the compact-open seminorm
\begin{equation}\label{eq:sewing-seminorm}
p_{\Sigma,K,\xi,\eta}(a)
\;=\;
\sup_{u\in K}
\bigl|\langle \eta,\,
A_\Sigma^{\mathrm{alg}}(u)
(\xi_1 \otimes \cdots \otimes a \otimes \cdots \otimes \xi_m)
\rangle\bigr|.
\end{equation}
Before convergence on $U$ is proved, use the corresponding
coefficient seminorms of the formal series.
The \emph{sewing envelope} $\cA^{\mathrm{sew}}$ is the Hausdorff
completion of $\cA_{\mathrm{alg}}$ for the locally convex topology
generated by these coefficient and compact-open seminorms.
\end{definition}

The seminorms~\eqref{eq:sewing-seminorm} require every matrix
coefficient of every convergent algebraic amplitude to extend to a
continuous function of the state, and record formal coefficients
before convergence has been established. The sewing envelope is the
smallest completion that remembers every local-to-global operation
at once.
It is the minimal target for analytic bootstrap arguments that use
all sewing amplitudes as continuous matrix coefficients.
...
$A_\Sigma^{\mathrm{alg}}$ extends continuously on the chosen
compact convergence domains, and every formal coefficient seminorm
is continuous. Then the identity on $\cA_{\mathrm{alg}}$ extends
uniquely to a continuous map
$E \to \cA^{\mathrm{sew}}$ with dense image, provided
$\cA_{\mathrm{alg}}$ is dense in~$E$.
\end{proposition}

\begin{proof}
By hypothesis, each seminorm $p_{\Sigma,K,\xi,\eta}$ and each
coefficient seminorm is continuous on $E$. Hence the identity
$(\cA_{\mathrm{alg}},E\text{-topology}) \to
(\cA_{\mathrm{alg}},\text{sewing topology})$ is continuous. Since
$\cA^{\mathrm{sew}}$ is complete, it extends uniquely from the dense
algebraic core to~$E$.
\end{proof}

\begin{remark}[Algebraic versus analytic completions]
The sewing envelope can differ from elementary completions
...
These completions compare to $\cA^{\mathrm{sew}}$ by continuous maps
once their amplitudes are continuous; embeddings or quotient
descriptions require additional topological hypotheses. The
HS-sewing criterion of the next section constructs trace-class
closed amplitudes on a concrete Hilbert completion~$H_q$.
A dense comparison map $H_q\to\cA^{\mathrm{sew}}$ exists only after
the open sewing seminorms are separately shown to be continuous on
$H_q$.
\end{remark}
...
the sewing operators satisfy composable norm estimates. The
Hilbert--Schmidt criterion supplies such a completion.
...
\end{equation}
	...
	for $v, w \in H_n$.
	The \emph{$q$-damped pair-of-pants} is the sectorwise map
	\[
	m_q\big|_{H_a\otimes H_b\to H_c}
	\;:=\;
	q^{(3a+3b-c)/2}\,m_{a,b}^c ,
	\]
	viewed as an operator $H_q\widehat\otimes H_q\to H_q$.
	\end{definition}

	The damping is part of the sewing kernel. The raw multiplication on
	the Hilbert space with inner product
	$q^{2n}\langle-,-\rangle_{H_n}$ contributes the sector factor
	$q^{2(c-a-b)}$ to the Hilbert--Schmidt norm, not the factor in
	\eqref{eq:hs-sewing}.

\begin{proposition}[Closed loop amplitudes are trace-class]
\label{prop:closed-trace-class}
Suppose $\cA$ satisfies HS-sewing at parameter $q$. Then
\begin{enumerate}[label=\textup{(\roman*)}]
\item the $q$-damped pair-of-pants
...
\]
\item under bounded collar transport and the heat-trace condition
$\sum_n q^n\dim H_n<\infty$, every closed stable amplitude built
from a finite pants decomposition with at least two
Hilbert--Schmidt pair-of-pants factors is trace class on $H_q$;
\item if every open sewing seminorm
$p_{\Sigma,K,\xi,\eta}$ is separately continuous on $H_q$, then
the weighted Hilbert completion~$H_q$ admits a continuous linear
map $H_q \to \cA^{\mathrm{sew}}$ whose image is dense.
\end{enumerate}
\end{proposition}
...
with the weight-$(a+b)$ subspace of $H_q^{\widehat{\otimes} 2}$,
	the raw sector component $m_{a,b}^c \colon H_a \otimes H_b \to H_c$
	contributes $q^{2(c-a-b)}\|m_{a,b}^c\|_{\HS}^2$. The damping
	$q^{(3a+3b-c)/2}$ multiplies this by $q^{3a+3b-c}$, so the sector
	contribution is $q^{a+b+c}\|m_{a,b}^c\|_{\HS}^2$. Summing gives
$\|m_q\|_{\HS}^2 = \sum q^{a+b+c} \|m_{a,b}^c\|_{\HS}^2 < \infty$.

Claim~(ii) is the standard fact that a composition of two
Hilbert--Schmidt operators is trace class, together with the stated
bounded collar transport and heat-trace assumptions. Thus a closed
stable sewing graph with the controlled factors has a well-defined
trace. Open amplitudes and loops with only one controlled factor
require separate uniform boundedness assumptions.

Claim~(iii) is a conditional comparison statement. If the open
sewing seminorms of Definition~\ref{def:sewing-envelope} are
continuous on $H_q$, then the identity
$(\cA_{\mathrm{alg}},H_q\text{-topology}) \to
(\cA_{\mathrm{alg}},\text{sewing topology})$ follows, and completion
gives $H_q \to \cA^{\mathrm{sew}}$.
\end{proof}

\begin{corollary}[Conditional positivity of the sewing Gram matrix]
\label{cor:hs-positivity}
Under HS-sewing and a reflection-positive Hilbert model whose
pair-of-pants operators are positive in the relevant channel, the
connected sewing amplitudes
...
trace, which is positive when applied to a positive trace-class
operator. The additional positivity hypothesis supplies positivity
of the pair-of-pants factors; the trace-class criterion alone does
not.
\end{proof}
...
\item The Gaussian collision kernel on Bergman tensors represents
the Wick contraction in the sewing amplitude.
...
\item Every closed genus-$g$ amplitude whose bosonic second
quantization is trace-class is represented by a Fredholm determinant
...
Claim~(ii) is the analytic form of the Wick contraction. Under
$\Theta$, the Heisenberg two-point contraction is the Gaussian collision
operator
$G(z, w) = k/(1 - z\bar w)^2$ on $A^2(D) \otimes A^2(D)$,
which is the reproducing kernel of the Bergman space; the bar
sewing amplitude factors through this densely defined Gaussian kernel
...
\begin{theorem}[Conditional Heisenberg one-particle sewing]
...
\|T_q\|_1 \;=\; \frac{q}{1 - q}.
\]
When the bosonic second quantization is trace-class, the genus-$g$
partition function of a rank-$r$ Heisenberg VOA is the Fredholm
determinant
...
\end{equation}
At genus~$1$,
$Z_1 = (\operatorname{Im}\tau)^{-r/2}\,|\eta(\tau)|^{-2r}$.
\end{theorem}
...
Consequently $R$ is Hilbert--Schmidt. The additional
implementability hypothesis that $\Gamma(R)$ is trace-class gives
$\operatorname{Tr}_{\Sym H}\Gamma(T)=\det(1-T)^{-1}$ for a
trace-class one-particle sewing operator $T$ with spectral radius
$<1$; for rank $r$ the determinant is raised to the power $-r$.

The Polyakov formula for the Heisenberg partition function
identifies
$\det(1 - K_g)^{-r}
= (\det\operatorname{Im}\Omega)^{-r/2}
(\det{}'_\zeta \Delta_{\Sigma_g})^{-r/2}$,
...
so $Z_1 = (\operatorname{Im}\tau)^{-r/2}\cdot |\eta(\tau)|^{-2r}$
for rank $r$ after combining holomorphic and antiholomorphic
sectors. For a single current at level $k$, this exponent is $1$;
the level affects the two-point normalization, not the number of
oscillator colours.
\end{proof}

\begin{remark}
For the families treated here, the Heisenberg algebra is the case in
which the sewing construction is completely explicit: the sewing
envelope is a Bergman Fock space, the pair-of-pants amplitude is a
second quantization, and the closed amplitudes covered by
Theorem~\ref{thm:heisenberg-one-particle} are Fredholm
determinants. The non-abelian cases below are controlled by
Hilbert--Schmidt estimates rather than by a one-particle model.
\end{remark}
...
\begin{enumerate}[label=\textup{(\roman*)}]
\item (\textbf{Hardy--Ramanujan sector growth})
...
\begin{corollary}[Trace-class stable-graph expansion]
\label{cor:trace-class-stable-graph}
Suppose $\cA$ satisfies the hypotheses of
Theorem~\ref{thm:general-hs-sewing}. Then every closed amplitude
associated to a finite stable sewing graph is a trace of a
trace-class operator on $H_q$. This gives a norm-convergent
stable-graph expansion
\[
\langle A_{\Sigma_{g,n}}\rangle
\;=\;
\sum_{\Gamma}
\operatorname{Tr}(m_q^{\Gamma}),
\]
where $\Gamma$ ranges over the stable graphs appearing in the
chosen sewing decomposition. It is a Fredholm determinant only in
Gaussian one-particle situations, such as the Heisenberg and
oscillator part of lattice VOAs.
\end{corollary}
...
The standard families below are finitely strongly generated in the
sense that a finite set of fields produces the entire algebra under
normal-ordered product and derivation. For nonunitary or degenerate
quotients, the polynomial OPE bound is a normed-model hypothesis. For
...
Theorem MC4 of~\cite{LorgatVolI}.
\end{remark}

\section{Verification on the standard landscape}\label{sec:examples}

The criterion applies to any chosen positive Hilbert realization for
which the heat trace, bounded collar transport, and polynomial
matrix-coefficient bounds have been verified. The data required is
a character formula, to control $\dim H_n$, and a normed OPE bound,
to determine $N$.
...
insertion of derivatives. In a positive Hilbert model satisfying
this normed estimate and bounded collar transport, HS-sewing at
every $0<q<1$ follows. The sector sum
$\sum_n q^n\dim H_n$ is the Kac--Moody character $\chi_{V_k}(q)$.
\end{example}

\begin{example}[Virasoro]\label{ex:virasoro}
The generic Virasoro vacuum VOA has character
\[
q^{-c/24}\prod_{m\ge2}(1-q^m)^{-1}.
\]
Thus its sector dimensions have the same subexponential growth as
the partition function with the $L_{-1}\mathbf 1$ generator removed.
The OPE
...
$0 < q < 1$ in the PBW monomial norm, and in a genuine Hilbert
model only after the same polynomial estimate is verified. The
criterion applies without rationality, but positivity of the
Hilbert form is a separate condition.
\end{example}
...
$|C| \leq K(n+1)^{2N}$ in the PBW monomial norm; applying
Theorem~\ref{thm:general-hs-sewing} with this $N$ gives
HS-sewing at every $0 < q < 1$ in any positive Hilbert
realization satisfying the same normed bound and bounded collar
transport, and sector summation
...
has sectors whose dimensions are the coefficients of
\[
\frac{\Theta_\Lambda(q)}{\eta(q)^r}.
\]
The discriminant group indexes simple coset modules in the full
module envelope, not the charge summands of the vacuum VOA. The OPE
is a finite sum of
...
$\Vir_c$ & $[q^n]\,q^{-c/24}\prod_{m\ge2}(1-q^m)^{-1}$
& $\leq K(n+1)^2$ & $2$
& $q^{-c/24}\prod_{m\ge2}(1-q^m)^{-1}$ \\
$\cW_N$ & $\leq Ce^{\alpha\sqrt n}$
& $\leq K(n+1)^{2N}$ & $2N$
& $\chi_{\cW_N}(q)$ \\
$V_\Lambda$ & $[q^n]\Theta_\Lambda(q)/\eta(q)^r$
& $\leq K \cdot r$ & $r$
& $\Theta_\Lambda(q)/\eta(q)^r$
\end{tabular}
\end{center}
In the indicated positive Hilbert models, these estimates verify
the hypotheses of Theorem~\ref{thm:general-hs-sewing}. For affine,
Virasoro, and $\cW_N$ families, the row records the required normed
model estimate unless a cited Hilbert-model proof is supplied. The
resulting HS-sewing envelopes have closed loop amplitudes controlled
by Proposition~\ref{prop:closed-trace-class}.
\end{example}

\section{Analytic extensions}\label{sec:conclusion}

The Hilbert--Schmidt criterion realizes the algebraic MC element of
the chiral bar construction as trace-class analytic sewing data on
$H_q$. For the Heisenberg algebra this realization is completely
explicit in the Bergman Fock model.

Three extensions remain.

\paragraph{Completed algebras.}
Inverse-limit algebras such as $\cW_{1+\infty}$ and completed
Yangians require a quantitative HS estimate in the completed
topology. The algebraic input is the MC4 comparison cited in
Remark~\ref{rem:completed}.

\paragraph{Fredholm determinants beyond Heisenberg.}
For non-abelian Kac--Moody and Virasoro, the closed amplitudes
are trace class under the stated hypotheses. No one-particle
Fredholm model is constructed here.
...
$SL_2(\mathbb{Z})$ covariance of genus-$1$ characters. A modular
statement requires compatibility with Zhu's genus-$1$
theorem~\cite{Zhu96} and Huang's higher-genus sewing
theorem~\cite{Huang05}.
