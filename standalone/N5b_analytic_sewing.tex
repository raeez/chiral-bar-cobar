...
$H_q$, for every $0 < q < 1$, after inserting the \(q\)-damping
into the sewing kernel. Second, for the Heisenberg algebra
...
positive-energy chiral algebra with Hardy--Ramanujan sector growth
and polynomial OPE coefficient growth satisfies the HS-sewing
condition. The criterion verifies for Heisenberg, positive
integer-level affine Kac--Moody, lattice vertex algebras, and for
Virasoro and principal $\mathcal{W}_N$ after a positive Hilbert model
with polynomial OPE bounds is fixed.
\end{abstract}
...
delivers the sewing package for the \(q\)-damped kernel: every
closed amplitude is trace-class on the weighted completion
...
HS-sewing criterion of the next section constructs a concrete Hilbert
completion~$H_q$ with a dense comparison map
$H_q \to \cA^{\mathrm{sew}}$ and verifies that amplitudes extend
continuously to it.
\end{remark}
...
linear map $H_q \to \cA^{\mathrm{sew}}$ whose image is dense.
\end{enumerate}
\end{proposition}
...
\begin{theorem}[Heisenberg sewing]\label{thm:heisenberg-sewing-vol1}
...
model only after the same polynomial estimate is verified; the sector sum
$\sum_n q^n p(n) = q^{(1-c)/24}/\eta(q)$
is the Virasoro vacuum character. The criterion applies without
rationality, but positivity of the Hilbert form is a separate
condition.
\end{example}
...
realization satisfying the same bound, and sector summation
