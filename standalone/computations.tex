...
\begin{remark}[Independent computation anchors]\label{rem:computation-anchors}
This note records the graph-by-graph computation that produces the
genus-$2$ cross-channel correction
$\delta F_2(W_3) = (c+204)/(16c)$ and the gravitational-coproduct
primitivity for $\mathrm{Vir}$. Two canonical compute modules carry
the independent verification paths: \texttt{w3\_genus2.py} (six-graph
tabulation at seven test central charges) and
\texttt{ds\_hpl\_primitivity.py} (ghost-number obstruction check).
The exceptional-Lie dimensions used elsewhere in the monograph are
not re-hardcoded here; when an exceptional dimension appears it is
imported from \texttt{compute/lib/bc\_exceptional\_categorical\_zeta\_engine.py}
at \texttt{FUNDAMENTAL\_DIMS[\textquotesingle E8\textquotesingle]}. Cross-reference to Vol~I
theorem status: \textup{\textsf{thm:multi-weight-genus-expansion}}
gives $\delta F_2(W_3) = (c+204)/(16c)$ explicit (proved, not
conjectural); \textup{\textsf{thm:grav-primitivity-standalone}} is
the ghost-number-obstruction argument repeated here in three steps.
The conjectural extension to general principal $\mathcal{W}(\fg)$ is
Conjecture~\textup{\ref{conj:general-ds-primitivity}}.
\end{remark}
...
$W \mapsto -W$, which forces all odd-degree shadows on the $W$-line
...
$r^{\mathrm{aff}}(z) = \Omega/((k{+}2)z)$. The HPL transfer through the
BRST SDR produces:
...
The affine $r$-matrix $r^{\mathrm{aff}}(z) = \Omega/((k{+}2)z)$ has a
...
\begin{theorem}[Virasoro coproduct primitivity: verified lane]
...
\begin{proof}
The proof uses the tensor-factor projection formulation of the
Virasoro DS complex, proceeding in three steps.
...
\emph{Step~2.} For the Virasoro transfer, every contributing HPL
tree at degree $k \geq 2$ has at least
...
tree topology at degree $k \geq 2$ always contains one
...
Since the output of each degree-$k$ tree has ghost number~$+1$,
...
at all degrees. The ghost-number obstruction of the BRST complex
...
\end{center}
	...
	The conjectural pattern is: \emph{principal Drinfeld--Sokolov
	reductions produce primitive coproducts}. The BRST ghost sector is
	expected to provide the grading obstruction that kills all
	higher-degree components.
Theorem~\ref{thm:grav-primitivity-standalone} proves this for
$\widehat{\mathfrak{sl}}_2 \to \mathrm{Vir}$; the same argument
	is expected to apply to any principal DS reduction
	$\widehat{\fg} \to \cW(\fg)$ after controlling all transferred
	coproduct terms, yielding
...
\item The \emph{leading coefficient} $c/2 = \kappa(\mathrm{Vir}_c)$
 ...
 $F_g = (c/2) \int_{\overline{\mathcal{M}}_g} \lambda_g$ (uniform-weight) at all
...
gauge theory, the affine $r$-matrix $r^{\mathrm{aff}}(z) = \Omega/((k{+}h^\vee)z)$
...
it takes the affine Casimir $\Omega/((k{+}h^\vee)z)$ to the gravitational
