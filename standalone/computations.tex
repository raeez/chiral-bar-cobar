...
\subsection{The six stable graphs at genus 2}
\label{subsec:genus2-graphs}

The moduli space $\overline{\mathcal{M}}_{2,0}$ has a
stratification by six stable graphs $\Gamma_0, \ldots, \Gamma_5$:
...
\begin{itemize}
\item A \emph{$c$-dependent} piece $15/(2c)$, arising from
  genus-0 vertex factors (the banana and theta graphs).  This is
...
At $c = 4$: the ratio is approximately~$478$, and the cross-channel
correction \emph{dominates} the scalar term.  At $c = 50$: the
ratio is approximately~$4.2$.
...
$\kappa_i/24$ without $R$-matrix corrections.  Whether the
CohFT $R$-matrix produces compensating terms that cancel
$\delta F_2$ is the content of the multi-generator universality
problem (\textbf{op:multi-generator-universality}).  If
universality holds, the $R$-matrix corrections at genus~$1$ must
produce exactly $-(c+120)/(16c)$, a nontrivial identity on the
CohFT structure.  The computation stands as an explicit
prediction that any proof of multi-generator universality must
reproduce.
\end{remark}
...
= \frac{c+120}{16c} + \frac{220-c}{16(100-c)}
= -\,\frac{c^2 - 100c - 6000}{8c(100-c)}.
\]
