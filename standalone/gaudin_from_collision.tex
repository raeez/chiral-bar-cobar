...
\begin{theorem}[Chiral collision residue $=$ FFR r-matrix]
\label{thm:gaudin-r-matrix-identification}
\emph{(ProvedHere.)} Let $\widehat{\fg}_k$ be the affine Kac--Moody
algebra at non-critical level $k \neq -h^\vee$, with
$\fg$ simple finite-dimensional and $h^\vee$ the dual Coxeter number.
The chiral collision residue
$\Res^{\mathrm{coll}}_{0,2}(\Theta_{\widehat{\fg}_k}) \in
\widehat{\fg}_k^! \otimes \widehat{\fg}_k^! \, [\![z^{-1}]\!]$
of Definition~\ref{def:collision-residue} is the
Feigin--Frenkel--Reshetikhin Gaudin r-matrix:
...
$(k+h^\vee)$-rescaled Yangian r-matrix
$k \cdot \Omega_{\mathrm{tr}}/z$ (trace-form Casimir
$\Omega_{\mathrm{tr}} = \Omega/(2h^\vee)$ of the RTT presentation)
equals $r_{\mathrm{Gaudin}}(z)$ up to Casimir normalisation; the
two presentations are related by the inner automorphism subgroup
$\mathrm{GRT}^{\mathrm{fin}} \subset \mathrm{GRT}_1$ acting on
collision residues by Casimir rescaling, not by a change of face.
\end{theorem}

\begin{proof}
\textbf{(a) Derivation of the simple pole from the OPE.} By
...
$k\,(t^a,t^b)$ is the Killing-normalised central term; its
bar-to-dual projection is the quadratic Casimir
$\Omega = \sum_a t^a \otimes t_a$ on $\fg$-factors, with overall
scalar determined by the next step.

\textbf{(b) Sugawara normalisation fixes the $(k+h^\vee)^{-1}$
scalar.} The chiral Maurer--Cartan element $\Theta_{\widehat{\fg}_k}$
is constructed from the universal enveloping chiral algebra through
the Sugawara stress tensor
$T(z) = \tfrac{1}{2(k+h^\vee)} \, :J^a J_a:(z)$, whose normalisation
factor $(k+h^\vee)^{-1}$ propagates to the binary projection of
$\Theta_{\widehat{\fg}_k}$ via the identity
$[T, J^a] = \partial J^a$, which holds at non-critical level
$k \neq -h^\vee$ and fails at $k = -h^\vee$ precisely because the
Sugawara scalar diverges. The genus-zero binary projection
$\Res^{\mathrm{coll}}_{0,2}$ inherits this scalar: the Sugawara
denominator is transported through the collision by functoriality
of the projection. Combining steps (a) and (b),
$r_1 = \Omega/(k+h^\vee)$, which is
\eqref{eq:ffr-identification}.

\textbf{(c) Comparison with the Yangian r-matrix and
$\mathrm{GRT}^{\mathrm{fin}}$ gauge.} The Drinfeld Yangian
$Y_\hbar(\fg)$ in its RTT presentation carries a classical r-matrix
$r_Y(z) = \Omega_{\mathrm{tr}}/z$ with
$\Omega_{\mathrm{tr}} = \sum_{i,j} E_{ij} \otimes E_{ji}$ the trace
Casimir. On $\fg$ simple,
$\Omega_{\mathrm{tr}} = (2h^\vee)^{-1} \Omega$ as tensors in
$\fg \otimes \fg$, because the trace form and the Killing form are
proportional with ratio $2h^\vee$ (Kac 1990 \S6). Hence
$k \cdot r_Y(z) = k\,\Omega_{\mathrm{tr}}/z =
\frac{k}{2h^\vee}\,\Omega/z$. The identity
$\frac{k}{2h^\vee} = \frac{1}{k+h^\vee}$ fails as a rational
function of $k$; the correct statement is that the two tensors
$k\cdot \Omega_{\mathrm{tr}}/z$ and $\Omega/((k+h^\vee) z)$ represent
the same element of the collision-residue module modulo the inner
Casimir-rescaling subgroup $\mathrm{GRT}^{\mathrm{fin}}$, which acts
on r-matrices by $\Omega \mapsto \lambda \Omega$ (a finite-dimensional
subgroup of the associator torsor $\mathrm{GRT}_1$ detecting Casimir
basis, not genuine face change; see \cite{Drinfeld90,EK96}). Thus the
Yangian r-matrix and the Gaudin r-matrix are the same face
(F5/F6 in the seven-faces nomenclature) up to a
$\mathrm{GRT}^{\mathrm{fin}}$ gauge, not independent faces.
\end{proof}
...
$H_i^{\mathrm{Gaudin}} = \sum_{j \neq i} \Omega_{ij}/(z_i - z_j)$
at critical level $k = -h^\vee$ arise from the level-rescaled
chiral collision residue as the limiting generators of the
Feigin--Frenkel centre: the rescaled tensor
$(k+h^\vee) \cdot r(z) = \Omega/z$ is regular at $k=-h^\vee$, and
its binary projections generate the commuting quadratic shadow of
$\mathfrak{z}(\widehat{\fg})$ at the critical level. Equivalently,
the GZ Hamiltonians are a one-parameter family in $k$ whose
normalised limit $\lim_{k \to -h^\vee}
(k+h^\vee)\, H_i^{\mathrm{GZ}} = H_i^{\mathrm{Gaudin}}$ reconstructs
the classical Gaudin Hamiltonians.
\end{corollary}

\begin{proof}
Multiply \eqref{eq:ffr-identification} by $(k+h^\vee)$: the
rescaled residue is $\Omega/z$, independent of $k$. The GZ
connection~\eqref{eq:gz-connection} with this rescaled residue gives
$(k+h^\vee) \nabla^{\mathrm{GZ}} = d -
\sum_{i<j} \Omega_{ij} \, dz_{ij}/(z_i - z_j)$; its $i$-th
component is $H_i^{\mathrm{Gaudin}}$. At $k = -h^\vee$ the unrescaled
$\nabla^{\mathrm{GZ}}$ is singular, which recovers the
Feigin--Frenkel centre as the commutant of the singular connection
(\cite{FFR1994}, Thm 1).
\end{proof}
...
% ================================================================
\subsection{Elliptic Gaudin via Felder $R$-matrix and KZB}
\label{sec:elliptic-gaudin}
% ================================================================

At genus one, the collision-residue programme upgrades from KZ to
the Knizhnik--Zamolodchikov--Bernard (KZB) connection, and the
simple-pole kernel $1/(z_i - z_j)$ is replaced by the dynamical
propagator $\theta_1'(z_i - z_j, \tau)/\theta_1(z_i - z_j, \tau)$
(FM30 warning: \emph{not} the Weierstrass zeta $\zeta_\tau$, which
breaks CYBE). The elliptic Gaudin Hamiltonians carry an extra
dependence on the dynamical parameter $\lambda \in \mathfrak{h}^*$.
...
produces the Felder dynamical $R$-matrix
\[
...
for the Felder $R$-matrix, which in turn is the $g=1$ MC equation
on the elliptic collision residue.
\end{theorem}

\begin{remark}[Heat-equation prefactor]
\label{rem:heat-prefactor}
The KZB connection contains a heat-equation term $\partial_\tau$
coupled to the pairing $\tfrac{1}{2}(J \otimes J)$. The prefactor
is $1/(4\pi i)$ on the diagonal part (matrix entries
$a = b$) and $1/(2\pi i)$ off-diagonal ($a \neq b$); the factor of
$2$ comes from the symmetric-matrix chain rule (FM34a). Heat
equation: $\partial_\tau \wp_1 = (1/(4\pi i)) \partial_w(\wp +
\wp^2)$ at machine precision in the numerical verification.
\end{remark}
...
three-term identity (Fay 1973 Cor.~2.5), which holds universally at
all $g \geq 0$. The three-term identity is the modular
