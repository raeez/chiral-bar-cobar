...
dependence. The Gaudin Hamiltonians are constructed after a represented
operator comparison. The homotopy coinvariant
$q_2(K_\cA^{\mathrm{coll}})$ remains operator-valued; its normalized
trace is $\kappa$ in abelian and scalar families and
$\kappa_{\mathrm{dp}}$ on the non-abelian affine row, where
$\dim(\fg)/2$ completes $\kappa$.

\begin{proposition}[Pole structure]\label{prop:pole-structure}
The collision kernel $K_\cA^{\mathrm{coll}}(z)$ has poles of order at most
$k_{\max} = p_{\max} - 1$, where $p_{\max}$ is the maximal OPE pole
order among generating fields. The leading coefficient $K_{k_{\max}}$
...
\begin{theorem}[KZ-normalized collision kernel and FFR $r$-matrix]
...
Assume the KZ representation and $H_{\mathrm{CYBE}}^{\mathrm{rep}}$.
The trace-form collision kernel and its KZ-normalized represented image
are
\[
K^{\mathrm{coll}}_{k,\fg}(z)=k\,\Omega_{\mathrm{tr}}/z,
\qquad
...
\begin{equation}\label{eq:gz-connection}
 \nabla^{\mathrm{GZ}} = d - \sum_{i < j}
 \rho_{\mathrm{KZ}}(K_\cA^{\mathrm{coll}}(z_i-z_j)) \,
 dz_{ij}
\end{equation}
on $\mathcal{M}_{0,n}$.  Under the stated representation package,
the Maurer--Cartan relation and the Arnold relation give flatness of
$\nabla^{\mathrm{GZ}}$, hence $[H_i,H_j]=0$.

For $\widehat{\fg}_k$, substituting
$\rho_{\mathrm{KZ}}(K_{k,\fg}^{\mathrm{coll}}(z))
=\Omega/((k+h^\vee)z)$
(Theorem~\ref{thm:gaudin-r-matrix-identification})
...
\begin{remark}[Normalised Szeg\H{o}/Fay identity as universal modular
identity]\label{rem:szego-fay}
The Fay trisecant identity, originally for $g \geq 2$ theta
functions, specializes at $g = 0$ and $g = 1$ to the normalised
Szeg\H{o}/prime-form identity \cite[Cor.~2.5]{Fay1973ThetaFunctions},
which holds universally at all $g \geq 0$ after the theta-prime and
prime-form normalisations are included. This normalised identity is
the modular compatibility that replaces the pentagon in the elliptic
and higher genus monodromy computations; at $g = 1$ it controls the
