...
\]
Thus the KZ-normalized chiral collision residue
$\Res^{\mathrm{coll}}_{0,2}(\Theta_{\widehat{\fg}_k}) \in
\widehat{\fg}_k^! \otimes \widehat{\fg}_k^! \, [\![z^{-1}]\!]$
of Definition~\ref{def:collision-residue} gives the
...
Sugawara scalar diverges. Passing from the trace-form collision tensor
...
At genus one, KZ regularizes to the KZB connection. The
non-dynamical elliptic classical $r$-matrix is the Belavin--Drinfeld
matrix, with the full Weierstrass $\zeta_\tau$ in the Cartan channel
and theta-function ratios in the root channels. The Felder
$R(z,\lambda,\tau)$ is a distinct dynamical IRF/face-model object
satisfying the dynamical Yang--Baxter equation; for $\mathfrak{sl}_2$
it is related to the Belavin matrix by a vertex--IRF gauge transform,
not identical to the bare collision residue.
