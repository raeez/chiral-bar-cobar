...
data), and the Yangian $Y_z^\hbar(\mathfrak{sl}_2)$, which
...
chiral homology
$\int_{E_\tau}^{\mathrm{ord}} Y_z^\hbar(\mathfrak{sl}_2)$
...
$Y_z^\hbar(\fg)$, the Etingof--Kazhdan quantum vertex
...
of $\mathrm{Rep}(Y_z^\hbar(\mathfrak{sl}_2))$.
...
the Yangian $Y_z^\hbar(\mathfrak{sl}_2)$, which
...
$\mathrm{Rep}(Y_z^\hbar(\mathfrak{sl}_2))$
...
\begin{example}[Yangian $Y_z^\hbar(\mathfrak{sl}_2)$
as a chiral quantum group: three structures and the
ordered chiral center]
\label{ex:yangian}
The Yangian $Y_z^\hbar(\mathfrak{sl}_2)$ is the first fully
...
$\mathrm{Mod}_{Y_z^\hbar(\mathfrak{sl}_2)}$ a braided
...
$m_k^{\mathrm{ch}} \colon Y_z^\hbar^{\otimes k} \to Y_z^\hbar$
...
$U(\mathfrak{sl}_2)$ inside $Y_z^\hbar(\mathfrak{sl}_2)$;
the PBW filtration has
$\operatorname{gr} Y_z^\hbar(\mathfrak{sl}_2)
\cong U(\mathfrak{sl}_2[t])$.

\medskip
\noindent
\textbf{Structure~(III): the chiral coproduct.}
The Drinfeld coproduct on $Y_z^\hbar(\mathfrak{sl}_2)$ is
...
product in the $Y_z^\hbar$ factor. The spectral parameter $z$
...
The ordered bar complex $\Barord(Y_z^\hbar(\mathfrak{sl}_2))
...
$Y_z^\hbar(\mathfrak{sl}_2)$ as the $\Eone$-chiral Koszul
dual.
...
(Y_z^\hbar(\mathfrak{sl}_2),\,Y_z^\hbar(\mathfrak{sl}_2))$
...
$Z(Y_z^\hbar(\mathfrak{sl}_2))$: those elements $a$ with
$a_{(n)}b = 0$ for all $b$ and $n \geq 0$.
The quantum determinant
$\operatorname{qdet}\,T(u) = A(u)\,D(u - \hbar)
- B(u)\,C(u - \hbar)$
is central in $Y_z^\hbar(\mathfrak{sl}_2)$~\cite{Molev07}.
...
form
\[
  P^{\mathrm{ord}}_{Y_z^\hbar}(t)
  = P^{\Sigma}_{Y_z^\hbar}(t) + \Delta P(t),
\]
...
Arity $n$ & Ordered datum & $\ker(\av_n)$
  & Symmetric shadow \\
\midrule
$0$ & $Z(Y_z^\hbar) = \CC[\operatorname{qdet}]$ & $0$
  & $Z(Y_z^\hbar)$ \\
...
producing the Yangian $Y_z^\hbar(\fg)$ as the
...
coproduct. For the Yangian $Y_z^\hbar(\fg)$, the $R$-matrix
...
\item \textup{(Chiral coproduct.)} The Drinfeld
  coproduct on $Y_z^\hbar(\fg)$ is the paradigmatic example.
...
For the Yangian $Y_z^\hbar(\fg)$, structure~(I) is the
...
$\Omega(\Barord(Y_z^\hbar(\fg))) \xrightarrow{\sim}
Y_z^\hbar(\fg)$ identifies these operations with the
...
\label{ex:ek-qvoa}
The Yangian $Y_z^\hbar(\mathfrak{sl}_2)$ of
...
$Y_z^\hbar(\mathfrak{sl}_2)$, confirming the identification
$U_0(R) \cong Y_z^\hbar(\mathfrak{sl}_2)$ as vector spaces.
...
$Y_z^\hbar(\mathfrak{sl}_2)$ on an elliptic curve
...
For $\cA = Y_z^\hbar(\mathfrak{sl}_2)$:
...
For $Y_z^\hbar(\mathfrak{sl}_2)$ viewed through its
...
the bar spectral sequence). Since $Y_z^\hbar(\mathfrak{sl}_2)$
...
\begin{equation}\label{eq:ell-arity2-cx}
  \cC_2^{\mathrm{ord}}(E_\tau, Y_z^\hbar)
  = \Bigl(
  \Omega^*_{\log}\bigl(
...
$H^*(\cC_2^{\mathrm{ord}}(E_\tau, Y_z^\hbar))$ is the de~Rham
