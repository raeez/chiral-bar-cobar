...
Proposition~\ref{prop:ker-av-schur-weyl} proves via
Schur--Weyl duality that $\dim\ker(\av_n) = d^n -
\binom{n+d-1}{d-1}$ on $V^{\otimes n}$ for
$V = \CC^d$, with generating function
...
$\kappa = 3(k + 2)/4$ (equation~\eqref{comp:sl2-kappa}).
...
Let $\fg = \mathfrak{gl}_r$ with fundamental representation
$V = \CC^r$, and let
...
\begin{equation}\label{eq:ker-av-schur-weyl}
  ...
  r^n - \binom{n+r-1}{r-1}.
\end{equation}
In particular, for $\fg = \mathfrak{sl}_2$ $(r = 2)$:
$\dim \ker(\av_n) = 2^n - (n+1)$.
\end{proposition}

\begin{proof}
Schur--Weyl duality decomposes the tensor power into
...
acts on the $\Sigma_n$-factor: it projects onto the trivial
isotypic component $S_{(n)} = \CC$ (the trivial
representation, corresponding to the single-row
partition $\lambda = (n)$) and annihilates every
...
\[
  ...
  \Sym^n(\CC^r),
\]
of dimension $\binom{n+r-1}{r-1}$ (the number of
degree-$n$ monomials in $r$ variables). The kernel is the
complement:
\[
  \ker(\av_n)
  \;\cong\;
...
$\dim(V^{\otimes n}) = r^n$,
$\dim(\Sym^n(\CC^r)) = \binom{n+r-1}{r-1}$
gives~\eqref{eq:ker-av-schur-weyl}.

For $r = 2$, the binomial coefficient evaluates to
$\binom{n+1}{1} = n + 1$, yielding
$\dim\ker(\av_n) = 2^n - (n+1)$.
\end{proof}
...
at low degrees for $\mathfrak{sl}_2$ through
$\mathfrak{sl}_5$.
...
representation-theoretic bound $r^n - \binom{n+r-1}{r-1}$.
...
$\mathfrak{sl}_4$ $(r{=}4)$
  & $6$ & $44$ & $221$ & $968$ & $4012$ \\
$\mathfrak{sl}_5$ $(r{=}5)$
  & $10$ & $90$ & $555$ & $2999$ & $15415$ \\
\midrule
\textit{Formula}
  & \multicolumn{5}{c}{$r^n - \binom{n+r-1}{r-1}$} \\
\bottomrule
\end{tabular}
\caption{Dimension of $\ker(\av_n\big|_{V^{\otimes n}})$ for
$V = \CC^r$ at degrees $n = 2, \ldots, 6$
...
$\av_2$ is $\kappa = 3(k+2)/4$ (equation~\eqref{comp:sl2-kappa}),
...
$R$-matrix of $Y(\widehat{\mathfrak{gl}}_N)$ is no longer
scalar, and the OPE compatibility at each truncation level
requires the full vertex bialgebra verification. The
coderivation argument (argument~A of step~6) applies to
$\cW_N$ at generic level by the same Koszul-locus reasoning,
provided the Drinfeld--Sokolov reduction intertwines the bar
complexes; this compatibility is established by
Theorem~\ref{thm:chiral-qg-equiv} applied to $\cW_N$
directly. The JKL argument (argument~B) applies to
...
\item \textup{(Formal deformation families.)}
  ...
  Writing $\lambda = k + h^\vee$ for the CFG coupling:
...
\item \textup{(Sugawara constraint.)}
  ...
  identification in the formal neighbourhood of critical level holds
  order-by-order in $(k + h^\vee)$, since the Sugawara element
  exists at any nonzero departure from critical level.
\item \textup{(What is proved.)} The deformation
  \emph{spaces} agree
  \textup{(}both $(k+h^\vee) H^3(\fg)[[k+h^\vee]]$\textup{)},
...
\begin{equation}\label{comp:sl2-kappa}
  \kappa = 3(k + 2)/4.
\end{equation}
...
The scalar $\kappa = 3(k+2)/4$ (equation~\eqref{comp:sl2-kappa})
...
proportional to $\kappa(\cA)$ (equation~\eqref{comp:sl2-kappa}).
\end{proof}
...
(both one-dimensional at each order in $(k+h^\vee)$ for
...
$\dim\ker(\av_n) = d^n - \binom{n+d-1}{d-1}$ via
Schur--Weyl duality; the open content is the passage from
...
$c \neq 0$ (conformal anomaly obstruction). The remaining
questions (Remark~\ref{rem:w-infty-vertex-gap}) are:
(i)~the vertex-algebraic antipode (does
$S(T(u)) = T(u)^{-1}$ on the Yangian lift to a
vertex Hopf algebra structure on $\cW_{1+\infty}$?);
(ii)~the descent to $\cW_N$ at finite~$N$, where the
$R$-matrix is no longer scalar; (iii)~the Stokes
...
genus-$1$ theory
$\int_{E_\tau}^{\mathrm{ord}} Y_\hbar(\mathfrak{sl}_2)$
explicitly, computing the degree-by-degree decomposition
through degrees~$0$, $1$, and~$2$. The curvature
