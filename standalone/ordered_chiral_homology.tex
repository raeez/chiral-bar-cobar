...
algebraic $\Etwo$ closed-sector object, and together with the
...
$\Etwo$ on the ordered derived centre; passage to a physical bulk then
requires the categorical Drinfeld-centre step and an open--closed
comparison.
...
with its deconcatenation coproduct is the level-$2$ object before
symmetrisation. Every construction here begins with this $\Eone$
chart datum; the symmetric theory is recovered by applying the
...
% level-shift check: at hbar=0, i.e. k -> infty, r = 0.
...
\item \textup{(Infinite-dimensionality.)}
  ...
  For an $\Einf$-chiral curve chart carrying a family datum
  $H_H(\cA;S)$, Theorem~H identifies its derived-centre
  cohomology with $H^\bullet(K_{\cA,S})$ and hence supports it
  in the prescribed finite set~$S$.  Finite-dimensional graded
  pieces are a separate finiteness hypothesis.
...
This is the Drinfeld-centre step in the $\En$ operadic circle
($\Eone \to \Etwo \to \Ethree$): the centre is the
right adjoint to forgetting the braiding and constructs
half-braiding data. The Reynolds/coinvariant averaging map is the
different, lossy $E_2\to E_\infty$ scalar-shadow projection.
The passage from $\Etwo$ to $\Ethree$ requires working at the
categorical rather than algebraic level. Concretely, the Drinfeld centre
...
\emph{Koszul locus} if the bar-cobar counit is a quasi-isomorphism:
...
\item \textup{(Holonomicity and singularity type.)}
  ...
  \item \textup{Class $L$ \textup{(}affine KM\textup{)}:}
    $r(z) = k\Omega_{\mathrm{tr}}/z$ \textup{(}trace-form\textup{)},
...
chiral-homology presentation, Beilinson levels~$2$ and~$5$,
hypothesis package: \(\Einf\)-commutative chiral input, finite
ordered arity, \(\Sigma_n\)-equivariant holonomic \(\cD\)-module,
finite direct image along \(X^n\to X^{(n)}\), Reynolds summand
identifying the Beilinson--Drinfeld symmetric \(\cD\)-module, and
regular extension across diagonals\textup{)}.
...
This is not an equivalence between $\cD$-modules on the quotient stack
and arbitrary $\cD$-modules on the coarse symmetric power
$X^{(n)}$.  Stabilizers along diagonals retain equivariant data.
The coarse Beilinson--Drinfeld object is obtained by the finite direct
image
\[
\pi_{n,+}\cF_n^{\mathrm{ord}},
\qquad
\pi_n\colon X^n\longrightarrow X^{(n)},
\]
followed by the Reynolds summand fixed by the symmetric descent datum:
\[
\cF_n^{\Sigma}(\cA)
\;\simeq\;
\operatorname{Re}_{\Sigma_n}
\bigl(\pi_{n,+}\cF_n^{\mathrm{ord}}(\cA)\bigr).
\]
The regular-extension hypothesis across the diagonals is exactly what
makes this finite direct-image description compatible with the
collision strata.

The de~Rham functor commutes with finite direct image.  After this
passage to global de~Rham complexes, Maschke's theorem applies to the
finite symmetric-group action in characteristic zero, so
derived coinvariants agree with ordinary coinvariants.  Thus
$R\Gamma_{\mathrm{dR}}(X^n,
\cF_n^{\mathrm{ord}})_{\Sigma_n}
\simeq
R\Gamma_{\mathrm{dR}}(X^{(n)},
...
symmetric in the BD sense: the two ordered collision representatives
are identified by the commutative chiral operation, with the Koszul
sign already built into the symmetric structure. This ensures that
the total complex, not just the fixed-degree pieces, descends to the
Reynolds symmetric summand.
\end{proof}
...
Type signature: \textup{(}Open quadrant, ordered-to-symmetric
chiral-homology presentation, Beilinson levels~$2$ and~$5$,
hypothesis package: fixed finite arity window, characteristic zero,
completed \(\Sigma_n\)- or \(R\)-twisted coinvariant descent, and
continuous ordered homology carrier\textup{)}.
...
Its kernel is exactly the completed relation subspace
\[
\ker(\av_n)
=
\overline{
...
\sigma\in\Sigma_n,\;
c\in\int_{X,n}^{\mathrm{ord}}\cA
\right\rangle}.
\]
In the \(R\)-twisted case the same formula is replaced by the
relations \(r\sigma\otimes c-r\otimes\sigma c\) in
\[
R_n\widehat\otimes_{\mathbb Q[\Sigma_n]}
\int_{X,n}^{\mathrm{ord}}\cA .
\]
Consequently \(\av_n\) is non-injective precisely when the ordered
arity-\(n\) carrier has a nonzero nontrivial
\(\Sigma_n\)-isotypic, or \(R\)-twisted, component.  When that
component vanishes, the descent is lossless in arity~\(n\).  On the
standard nontrivial windows, the kernel contains the following
witnesses:
\begin{enumerate}[label=\textup{(\roman*)}]
\item At $n=2$: the anti-invariant part of
  $K_\cA^{\mathrm{coll}}$ lies in $\ker(q_2)$, while
  $q_2(K_\cA^{\mathrm{coll}})$ retains the invariant operator part.
  The normalized trace gives $\kappa$ on scalar abelian windows and
  $\kappa_{\mathrm{dp}}$ on non-abelian KM, where the Sugawara term
  $\dim(\fg)/2$ completes $\kappa$.
\item At $n = 3$: the nontrivial Reynolds complement of the
  Drinfeld associator $\Phi$ appears in $\ker(\av_3)$ when the
  associator class survives in the chosen window. The space of
  sections of $\av_3$ compatible with the Maurer--Cartan equation is
  a $\GRT_1$-torsor.
\item At $n \geq 4$: whenever the corresponding nontrivial
  isotypic component survives, the kernel carries higher-degree
...
\begin{proof}
The coinvariant quotient of a completed \(\Sigma_n\)-complex is the
cokernel of the continuous maps \(\sigma-\id\). Hence the quotient map
is surjective and its kernel is the completed span of the displayed
relations. This proves the injectivity criterion. The \(R\)-twisted
statement is the same cokernel calculation for
\(R_n\widehat\otimes_{\mathbb Q[\Sigma_n]}(-)\), using the relations
defining the balanced tensor product.

The listed classes are then witnesses, not extra axioms.  In arity
\(2\), the complementary Reynolds projector
\((1-s)/2\) extracts the anti-invariant part of the ordered
\(r\)-matrix; it is zero on scalar abelian windows and nonzero on
the non-abelian windows where the ordered matrix part survives.
In arity \(3\), a nontrivial associator component contributes through
its Reynolds complement; Drinfeld's associator theorem and
Etingof--Kazhdan quantisation describe the torsor of compatible
sections once the source Lie bialgebra, topology, associator, and
target category are fixed. Higher arities are identical: only the
surviving nontrivial isotypic components lie in the kernel.
\end{proof}
...
invariant $\kappa(\cA)$ (the scalar
$\kappa(\cA)=\tau_{\cA,2}^{\mathrm{res}}q_2(K_\cA^{\mathrm{coll}})$
in the abelian and scalar families, while non-abelian affine
Kac--Moody has
$\kappa(V_k(\fg))=\kappa_{\mathrm{dp}}+\dim(\fg)/2$; computed
...
\item \textup{($R$-matrix.)} The vertex $R$-matrix is the
  ...
  % level-shift/trace-form: trace-form convention; k=0 -> r=0. Verified.
  $\nabla = d - k\Omega_{\mathrm{tr}}/(z_1 - z_2) \cdot d(z_1 - z_2)$
  around $\Delta_{12}$. The classical $r$-matrix
  (trace-form) is
  $r(z) = k\Omega_{\mathrm{tr}}/z$; at $k = 0$, $r = 0$; at
...
\item \textup{($R$-matrix.)}
  ...
  classical limit is
  % level-shift: r-matrix with level prefix Psi; Psi=0 -> r=0. Verified.
...
% level-shift: level prefix Psi mandatory; Psi=0 -> R = uI_4 (trivial). Verified.
...
% level-shift: Psi=0 -> r=0. Verified.
...
\item \textup{(Yang's rational $R$-matrix.)}
  ...
  \begin{equation}\label{eq:glN-yang-r}
    % level-shift: level prefix Psi mandatory; Psi=0 -> R = uI (trivial). Verified.
    R(u)
    = u\, I_{N^2} + \Psi\, P
...
\]
% level-shift: level prefix Psi; Psi=0 -> R = u I (trivial). Verified.
...
\item \textup{(Derived centre and open/closed package.)}
  ...
  \begin{equation}\label{eq:e3-hdc}
    ...
    \quad \text{is the universal closed-sector $\Etwo$-algebra.}
  \end{equation}
  Together with the boundary algebra $\cA$, it forms the
  universal $\mathrm{SC}^{\mathrm{ch,top}}$ pair; a physical bulk is
  obtained only after an open--closed comparison with a field theory.
\item \textup{(Scope and topologization boundary.)} For
  ...
  admits an inner conformal-vector datum whose antighost contraction
  makes holomorphic translations $Q$-exact, the affine topologization theorem
...
The KZ connection
% level-shift: trace-form; k=0 -> r=0. Verified.
...
\sum_{a} {:} J^a J_a {:}$
has an antighost contraction making holomorphic translations BRST-exact:
...
\begin{remark}[Closed-sector $\Etwo$ versus topological $\Ethree$]
\label{rem:two-e3}
The algebraic closed-sector $\Etwo$ (chiral Hochschild cohomology of
the $\Etwo$-coalgebra $\BarSig$, no conformal vector needed) and the
topological $\Ethree^{\mathrm{top}}$ (Sugawara
...
identification remains open, and a physical bulk interpretation
...
$\Omega = e \otimes f + f \otimes e
+ \tfrac{1}{2}h \otimes h$.
% kappa-normalization: kappa from landscape_census.tex:
...
$r(z) = k\Omega_{\mathrm{tr}}/z$ (at $k = 0$, $r = 0$);
in KZ normalisation,
$r(z) = \Omega_{\mathrm{KZ}}/((k + 2)z)$.
...
Drinfeld associator class. After fixing an associator gauge,
$\Phi_{\mathrm{KZ}}$ supplies a distinguished generator of this
one-dimensional shadow. The $\GRT_1$ action changes the associator
representative in the torsor; it does not upgrade this scalar
shadow into an associator-independent chain-level object.
...
% Virasoro-coproduct: r^Vir(z) = (c/2)/z^3 + 2T/z.
...
% kappa-normalization: kappa(V_k(sl_2)) = dim(sl_2)(k+h^v)/(2h^v)
...
The degree-$2$ projection of the genus-$1$ ordered Maurer--Cartan
element $\Theta_\cA^{(1)}\in\gEone$ is the elliptic collision kernel
$K_\cA^{(1),\mathrm{coll}}(z,\tau)$. Under the KZB package
$H_{\mathrm{CYBE}}^{\mathrm{rep}}$, its represented image is the
Belavin elliptic $r$-matrix
$r^{\mathrm{ell}}(z,\tau)=k\,r^{\mathrm{Belavin}}(z,\tau)$.
It satisfies the classical Yang--Baxter equation with spectral parameter (verified
numerically in \texttt{elliptic\_rmatrix\_shadow.py}),
and its normalized trace gives
$\tau_{\cA,2}^{\mathrm{res}}q_2(K_\cA^{(1),\mathrm{coll}})
=\kappa_{\mathrm{dp}}$; adjoining the Sugawara contribution gives
$\kappa=3(k+2)/4$
...
The Tsuchiya--Ueno--Yamada/Hitchin conformal-block sheaf is
then finite rank.  The ordered chiral chain complex enters this
statement only through the ordered-to-TUY comparison morphism:
after the integrable level, boundary sewing, projectively flat
connection, and determinant-anomaly packages are supplied, its
symmetric coinvariant shadow identifies with that finite-rank
sheaf (Tsuchiya--Ueno--Yamada~\cite{TUY89}).
...
\ClaimStatusConditional
Type signature: \textup{(}Open quadrant, ordered-to-symmetric
chiral-homology/conformal-block comparison presentation, Beilinson
levels~$2$ and~$5$, hypothesis package: positive integral
\(\widehat{\mathfrak{sl}}_2\) level, TUY/Hitchin finite-rank
conformal-block sheaf, integrable-module truncation, KZB
projectively flat connection, boundary sewing, determinant-anomaly
matching, and the ordered-chain-to-TUY comparison morphism\textup{)}.
...
\begin{enumerate}[label=\textup{(\roman*)}]
\item \textup{(Genus~$0$.)}
  Under the ordered-chain-to-TUY comparison, the genus-$0$
  no-marked-point target is the unique vacuum:
...
\item \textup{(Genus~$1$.)}
  Under the same comparison, the genus-$1$ target has rank equal
  to the number of integrable representations:
  ...
  On the ordered/TUY comparison lane:
  Proposition~\textup{\ref{prop:ell-degree0}} computes the
  degree-$0$ center at integrable level as $\CC^{k+1}$
...
\item \textup{(Handle attachment.)}
  TUY non-separating factorization, compared with the ordered
  sewing map under the stated package, gives the TQFT
  handle-attachment formula:
...
\item \textup{(Separating factorization.)}
  ...
  TUY factorization and the ordered sewing comparison give
...
Under the ordered-chain-to-TUY comparison, at genus~$0$ with no
...
\item \textup{(Ordered/TUY comparison.)}
  ...
  the integrable quotient). Under the comparison morphism this is
  the TUY genus-$1$ block space. The averaging map $\av_0$ is
...
Through the ordered-chain-to-TUY comparison, the symmetric
coinvariant shadow is identified with the TUY conformal-block sheaf.
The
...
surface with two extra marked points. TUY sewing reglues these
marked points, and the ordered sewing map agrees with it only under
the stated comparison package. The channel sum runs over the $k+1$
...
\textup{(v)} Separating factorization: TUY factorization under
separating degeneration, together with the ordered sewing comparison
...
\end{equation}
% kappa-normalization: kappa(V_k(sl_2)) = 3(k+2)/4 from landscape_census.tex;
...
the degree-$2$ ordered chiral homology on $V \otimes V$
has $\dim H^1_{\mathrm{ord}, 2}$ doubling from $4$ to $8$
\textup{(}a finite computation conserving $\chi_{\mathrm{ord}, 2} = -4$\textup{)},
Koszulness fails, and the FULL symmetric bar cohomology spreads
to $\Omega^*(\operatorname{Op}_{\mathfrak{sl}_2}(D))$, which is
infinite-dimensional in every cohomological degree
\textup{(}Feigin--Frenkel 1992, Frenkel--Teleman 2006; distinct from the finite
degree-$2$ ordered slice\textup{)}.
