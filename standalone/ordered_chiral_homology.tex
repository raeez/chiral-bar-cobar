...
\title[Ordered Chiral Homology]
{Ordered Chiral Homology, the $\Ethree$ Algebra\\
of Chern--Simons Theory, and the Holomorphic\\
Refinement of Factorisation Homology}
...
\begin{abstract}
We define ordered chiral homology $\int_X^{\mathrm{ord}} \cA$
for an $\Eone$-chiral algebra on a smooth curve~$X$ as the
derived pushforward of the ordered factorisation
$\cD$-module $\cF^{\mathrm{ord}}(\cA)$ on the ordered Ran
space $\Ran^{\mathrm{ord}}(X) = \coprod_n U_n(X)$: a
single $\cD$-module encoding the OPE, the KZ connection,
and the factorisation product across all arities
simultaneously.
The Beilinson--Drinfeld Ran space
$\Ran(X)$ is the $\Sigma_\bullet$-coinvariant quotient of
$\Ran^{\mathrm{ord}}(X)$, and the averaging map
$\av \colon R\Gamma(\Ran^{\mathrm{ord}}(X),
\cF^{\mathrm{ord}}) \to R\Gamma(\Ran(X), \cF^{\Sigma})$
recovers the Beilinson--Drinfeld chiral homology; for
genuinely $\Eone$-chiral algebras the descent is lossy,
carrying $\GRT_1$-dependent data from the Drinfeld
associator.
For $V_k(\fg)$, we construct a filtered $\Ethree$-chiral
algebra $\CE^{\mathrm{ch}}_\hbar(\fg)$ in $\cD$-modules
on $\Ran(X)$: its $\Pthree$ bracket is
$\{X,Y\} = h_{\mathrm{KZ}} \cdot (X,Y)$,
where $(X,Y)$ is the Killing form and
$h_{\mathrm{KZ}} = 1/(k + h^\vee)$ is the KZ coupling.
The $\Ethree$-deformation space is
$\hbar H^3(\fg)[[\hbar]]$; formal disk
restriction recovers the Costello--Francis--Gwilliam
perturbative Chern--Simons $\Ethree$-algebra, and the
Etingof--Kazhdan classification of quantum vertex
$R$-matrices gives a sixth identification, all in
natural bijection.
Five computations span the $G/L/C/M$ classification:
Heisenberg ($P(t) = 1 + t + t^2$, regular KZ),
$V_k(\mathfrak{sl}_2)$ (KZ conformal blocks with all
$\Ethree$ operations exhibited),
$\beta\gamma$ (nilpotent $R$-matrix, class~$C$),
Virasoro (irregular KZ connection, Stokes data,
class~$M$), and the Yangian $Y_\hbar(\mathfrak{sl}_2)$
(the first example where ordered chiral homology is
strictly richer than symmetric).
The Etingof--Kazhdan quantum vertex algebra is exhibited
with all five braided VOA axioms verified, and
$\cW_{1+\infty}$ is realised as a class~$M$ chiral
quantum group via the cohomological Hall algebra.
The chiral quantum group equivalence theorem identifies
three structures: vertex $R$-matrix, chiral
$\Ainf$-structure, and chiral coproduct, forming an
equivalence triangle mediated by the ordered bar complex
$\Barord(\cA) = T^c(s^{-1}\bar{\cA})$ and its
deconcatenation coproduct.
The correct setting for holomorphic Chern--Simons
invariants is the embedding of complex curves
$C \hookrightarrow Y$ into Calabi--Yau $3$-folds~$Y$
(complex codimension~$2$, two normal directions,
two spectral parameters); this connects the chiral
$\Ethree$-algebra to the Costello--Paquette programme
on $6$d holomorphic Chern--Simons and universal defect
algebras with $\ZZ \times \ZZ$-graded generators from
the rank-$2$ normal bundle.
\end{abstract}
...
\item \textup{(Ordered factorisation $\cD$-module on
  ...
  \end{equation}
  where $j \colon U_n = X^n \setminus \bigcup_{i < j}
  \Delta_{ij} \hookrightarrow X^n$ is the inclusion of
...
Section~\ref{sec:chiral-quantum-groups} closes with the
...
\item \textup{(The KZ connection.)}
  ...
  For affine Kac--Moody at level $k$ in the trace-form
  ...
  \begin{equation}\label{eq:kz-km}
    ...
    \, d(z_i - z_j),
  \end{equation}
  which has a \emph{simple pole} along $\Delta_{ij}$:
...
\item At $n = 2$: the full spectral $r$-matrix $r(z)$
  versus the scalar $\kappa = \av(r(z))$.
...
\end{enumerate}
Operad formality \textup{(a)} is the strongest general
result; it guarantees that ordered and symmetric
give the \emph{same answer as chain complexes} for
$\Einf$-chiral algebras. It does \emph{not} guarantee
that the answer is simple: the transferred
$\Ainf$-structure may carry non-trivial higher operations
\textup{(}levels (b)--(d) fail\textup{)}.
\end{definition}
...
\[
  ...
  \;\otimes\; [\omega_{ij}].
\]
This is precisely the Hochschild differential $d_{\HH}$
...
\item \textup{(Dimension count.)}
  ...
  The ratio $n!$ measures the additional holomorphic
  data carried by the ordered chiral complex beyond the
  topological Hochschild complex.
\end{enumerate}
\end{theorem}
...
\item \textup{(Higher Deligne gives $\Ethree$.)}
  ...
  structure. Applied to $\BarSig(\cA)$:
...
\item The $\Ethree$ structure exists for \emph{any}
  chiral algebra $\cA$ on a curve
  \textup{(}$\Einf$-chiral or $\Eone$-chiral\textup{)}.
  It does not require a Virasoro element, conformal
  symmetry, or any additional hypothesis beyond the
  curve geometry.
\end{enumerate}
\end{proposition}
...
$\{X, Y\}$ is the contraction of $\omega_3$ with the
KZ coupling $h_{\mathrm{KZ}} = 1/\hbar$:
\[
  \{X, Y\}
  = h_{\mathrm{KZ}} \cdot \omega_3(X, Y, -)
  \bigl|_{\HH^2 \to \HH^0}
  = h_{\mathrm{KZ}} \cdot (X, Y).
\]
The last equality uses the fact that the contraction
$\omega_3(X, Y, -) \in \mathfrak{sl}_2^*$ is paired with the
identity map $\mathfrak{sl}_2 \to \mathfrak{sl}_2$ to produce
a scalar, and the Killing form absorbs the structure
constants:
$\omega_3(X, Y, Z) = (X, [Y, Z])$ contracts to
$(X, Y)$ after tracing over $Z$ with the Casimir.
The $\Pthree$ bracket on $Z^{\mathrm{der}}_{\mathrm{ch}}$
is the binary shadow of the ternary Cartan form;
the normalisation $h_{\mathrm{KZ}} = 1/(k + h^\vee)$ is
fixed by the $r$-matrix residue
...
\item \textup{(Failure at critical level.)}
  ...
  genuinely holomorphic, not topological: the $\Etwo$ from
  the curve geometry depends essentially on the complex
  structure of~$X$.
  Perturbatively, at
...
deconcatenation coproduct (the topological color of the
Swiss-cheese operad); the $R$-matrix and the chiral
...
algebra $\cA$ determine each other, up to the choice of a
...
exactly $n$ facets, one for each consecutive collision
$z_i \to z_{i+1}$ ($1 \leq i \leq n$). Each facet is
...
dg coalgebra in the
$\mathsf{SC}^{\mathrm{ch,top}}$-coalgebra sense
(\cite[Theorem~1.2]{Lorgat26I}): the differential
$d_{\Barord}$ is the closed (holomorphic) color and the
deconcatenation $\Delta$ is the open (topological) color.
...
\item \textup{($R$-matrix.)} The vertex $R$-matrix is the
  ...
  around $\Delta_{12}$. At the classical level
  ($\hbar \to 0$, i.e.\ $k \to \infty$), this reduces to
  $R = 1 + \hbar\, r(z) + O(\hbar^2)$ where
  $r(z) = k\Omega/z$ is the classical $r$-matrix
  (trace-form; at $k = 0$, $r = 0$).
...
homology at genus~$0$ and describes the curvature
obstruction $d^2_{\mathrm{fib}} = \kappa \cdot \omega_g$
at genus $g \geq 1$, where $\omega_g = c_1(\lambda)$ is
the Hodge class on $\overline{\cM}_g$.
The full genus-$g$ theory requires a period-corrected
differential whose coefficients involve the
$(g \times g)$-matrix $(\mathrm{Im}\, \Omega)^{-1}$
of the Siegel period matrix, not the scalar
$1/\mathrm{Im}(\tau)$
of the genus-$1$ degeneration.
Does the ordered chiral chain complex at genus~$g$
...
This paper treats the rational Yang $R$-matrix
$R(u) = 1 + \hbar\, r(u)/u + O(\hbar^2)$ with
$r(z) = k\Omega/z$ (trace-form convention; at $k = 0$,
$r = 0$).
The trigonometric $R$-matrix
$R^{\mathrm{trig}}(u) = q^{\Omega/u}$ (quantum affine
algebras) and the elliptic Belavin $R$-matrix
$R^{\mathrm{ell}}(u, \tau)$ produce richer ordered chiral
homology with qualitatively different analytic behaviour:
higher-order Stokes phenomena, theta-function monodromy,
and flat sections valued in elliptic hypergeometric
integrals.
What is the ordered factorisation $\cD$-module
$\cF_n^{\mathrm{ord}}(\cA)$ for an $\Eone$-chiral algebra
whose vertex $R$-matrix is trigonometric or elliptic,
and does the regular/irregular dichotomy of
...
\item \textup{(The Khan--Zeng quantum extension.)}
Khan--Zeng~\cite{KhanZeng25} prove the topological
enhancement of the holomorphic-topological
$3$d gauge theory for the classical Poisson vertex
algebra (Proposition~\ref{prop:khan-zeng-topological}).
The extension to the full quantum vertex algebra
$V_k(\fg)$ at non-critical level is used in
Theorem~\ref{thm:e3-cs}(iv) but remains conjectural.
Does the Sugawara Virasoro element at non-critical level
provide a topological $\Ethree$-structure on the quantum
chiral $\Ethree$-algebra
$\CE^{\mathrm{ch}}_\hbar(\fg)$, or does the
quantum correction to the Sugawara element obstruct the
enhancement at finite $\hbar$?
