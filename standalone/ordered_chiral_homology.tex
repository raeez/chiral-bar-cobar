...
filtered affine bulk package
$\CE^{\mathrm{ch}}_k(\fg)$: a factorisation $\cD$-module on
$\Ran(X)$ whose formal-disk restriction matches the
associated-graded and $\Pthree$ data of the CFG
$\Ethree$-algebra $\cA^\lambda$.
...
The degree-$(-2)$ operation relevant for the affine
topologized bulk is not produced directly by the curve
geometry. To make it explicit, one needs a concrete
bracket. The Poisson vertex algebra (PVA)
...
% ----------------------------------------------------------------
\subsection{The filtered affine bulk package}
...
\end{equation}
On the curve itself this is a filtered factorisation
$\cD$-module with a $\Pthree$ operation, not a chiral
$\Ethree$-algebra; the $\Ethree^{\mathrm{top}}$ structure
enters only after affine topologization of the derived
centre.
\end{definition}

\begin{theorem}[Structure of the affine bulk package]
\label{thm:chiral-e3-structure}
Let $\fg$ be a simple Lie algebra.
The affine bulk package
...
\item \textup{(The $\Pthree$ package on the curve.)}
  The chiral $\Pthree$ bracket
  \textup{(}Proposition~\textup{\ref{prop:chiral-p3-structure}}%
  \textup{)} equips the critical-level commutative
  factorisation algebra with the degree-$(-2)$ operation
  that becomes the $\Pthree$ bracket of the affine
  $\Ethree^{\mathrm{top}}$ model after topologization.
  On the curve itself, this data is a filtered
  holomorphic-topological package in $\cD$-modules rather
  than a chiral $\Ethree$-algebra.
\item \textup{(Curvature and the deformation parameter.)}
  ...
  commutative structure to the filtered affine bulk
  package.
...
\item \textup{(Factorisation property.)}
  ...
  $\cD$-modules. The $\Pthree$ bracket is compatible with
  the factorisation isomorphism, and after non-critical
  affine topologization it matches the degree-$2$ operation
  in the resulting $\Ethree^{\mathrm{top}}$ model.
\end{enumerate}
\end{theorem}
...
\begin{remark}[Functoriality of ordered chiral homology]
\label{rem:ordered-ch-functoriality}
The ordered chiral homology functor
$X \mapsto \int_X^{\mathrm{ord}} \cA$ admits three
natural functoriality properties under geometric operations
on the curve $X$, each with a distinctive ordered refinement
absent from the symmetric (Beilinson--Drinfeld) theory.

\textup{(1) Degeneration (handle attachment and
factorisation).}
Let $\pi \colon \cX \to \Delta$ be a one-parameter family
of curves with smooth generic fibre $X_t$
($t \neq 0$) and nodal special fibre $X_0$.
The ordered bar complex carries a factorisation constraint
at each boundary stratum of $\overline{\cM}_{g,n}$.
Under \emph{separating} degeneration
$X_0 = X_1 \cup_p X_2$, the ordered chiral homology
admits a restriction map
\[
  \int_{X_t}^{\mathrm{ord}} \cA
  \;\xrightarrow{\;\mathrm{res}\;}\;
  \int_{X_1}^{\mathrm{ord}} \cA
  \;\otimes\;
  \int_{X_2}^{\mathrm{ord}} \cA
\]
that decomposes along isospin channels, each weighted by the
propagator $S_{0j}^{-2}$
(Proposition~\ref{prop:verlinde-ordered}).
Under \emph{non-separating} degeneration
(a single cycle pinches, reducing genus by one), the
handle-attachment formula~\eqref{eq:handle-attachment}
exhibits the ordered chain complex at genus $g+1$ as
a trace over the genus-$g$ complex weighted by the handle
operator $H_j = S_{0j}^{-2}$.
In both cases, the ordered theory refines the symmetric
Verlinde factorisation: the kernel of the averaging map
$\av$ at each degree provides additional data invisible
to conformal blocks, and the compatibility of $\av$
with the factorisation maps ensures that the symmetric
shadow of the ordered degeneration recovers the
Tsuchiya--Ueno--Yamada
sewing~\cite{TUY89}.

\textup{(2) Covering maps.}
Let $f \colon Y \to X$ be a finite morphism of smooth
proper curves of degree $d$.
The pullback $f^* \cA$ is a chiral algebra on $Y$
(the $\cD$-module pullback), and $f$ induces a map on
ordered configuration spaces
$f^n \colon Y^n \to X^n$
that is finite of degree $d^n$, equivariant for the
$\Sigma_n$-action on both sides when $f$ is Galois.
The pullback of the ordered factorisation $\cD$-module
satisfies
\[
  (f^n)^* \cF_n^{\mathrm{ord}}(\cA)
  \;\simeq\;
  \cF_n^{\mathrm{ord}}(f^* \cA)
\]
on $U_n(Y) = Y^n \setminus \bigcup \Delta_{ij}$, because
the OPE of $f^* \cA$ at points $y_i, y_j \in Y$ with
$f(y_i) \neq f(y_j)$ is the pullback of the OPE of $\cA$
at $f(y_i), f(y_j)$.
Along the diagonals, the identification extends
meromorphically provided the ramification locus of $f$
is disjoint from the colliding points, since the
$*$-extension commutes with pullback along unramified
morphisms.
At ramification points $y$ of $f$, the local coordinate
change $z \mapsto z^e$ ($e$ the ramification index)
transforms the $r$-matrix pole:
$r(z_1 - z_2) \, dz_{12}$ pulls back to
$r(w_1^e - w_2^e) \, d(w_1^e - w_2^e)$, which has
higher-order singularity along the diagonal in the
$w$-coordinates.
The resulting transfer map on ordered chiral homology
\[
  f_* \colon
  \int_Y^{\mathrm{ord}} f^* \cA
  \;\longrightarrow\;
  \int_X^{\mathrm{ord}} \cA
\]
(pushforward of de~Rham cohomology along a finite map)
is a chain map of degree zero.
When $f$ is a Galois cover with group $G$,
the $G$-equivariant ordered chiral homology of $f^*\cA$
on $Y$ maps to the ordered chiral homology of $\cA$
on $X$ via the $G$-coinvariant quotient; the interaction
between $G$-averaging and $\Sigma_n$-averaging produces
a bigraded descent spectral sequence
\[
  E_2^{p,q}
  = H^p\bigl(G,\;
    \textstyle\int_{Y,q}^{\mathrm{ord}} f^* \cA\bigr)
  \;\Longrightarrow\;
  \textstyle\int_{X,q}^{\mathrm{ord}} \cA.
\]
For the $\Einf$ (symmetric) theory this spectral sequence
collapses (the $\Sigma_n$-coinvariants absorb $G$), but for
genuinely $\Eone$-chiral algebras the two averaging
procedures are independent and the spectral sequence
is non-trivial.

\textup{(3) Subdisk inclusion.}
Let $D = \Spec\, \CC[[z]]$ be the formal disk and
$\iota \colon D \hookrightarrow X$ an embedding with
$\iota(0) = p \in X$.
The restriction of the ordered factorisation $\cD$-module
to $D^n \subset X^n$ recovers the ordered bar complex:
the de~Rham cohomology
$R\Gamma_{\mathrm{dR}}(D^n, \cF_n^{\mathrm{ord}}(\cA))$
computes the $n$-th degree of the ordered bar construction
$B_n^{\mathrm{ord}}(\cA_p)$
on the fibre $\cA_p = \iota^* \cA$, with the
KZ connection~\eqref{eq:kz-conn} restricting to the
standard Koszul differential.
The passage from the local (disk) to the global (curve)
ordered chiral homology is NOT a quasi-isomorphism in
general: the curve $X$ has nontrivial topology
(Arnold relations, non-trivial $H^1(X)$,
period matrix at $g \geq 1$) that the disk does not see.
The comparison takes the form of a localisation spectral
sequence
\[
  E_1^{p,q}
  = \bigoplus_{\alpha}
    H^q\bigl(B_p^{\mathrm{ord}}(\cA_{x_\alpha})\bigr)
  \;\Longrightarrow\;
  H^{p+q}\Bigl(
    \textstyle\int_X^{\mathrm{ord}} \cA
  \Bigr)
\]
running from the local bar cohomology at finitely many
points $\{x_\alpha\}$ covering $X$ to the global ordered
chiral homology.
The KZB connection
(Section~\ref{sec:elliptic-ordered}) is the datum
of a flat connection on the local system of bar
complexes over the configuration space of $X$;
the global cohomology is its space of flat sections.
At genus $0$, the spectral sequence collapses
(the formal disk retracts onto a point, and
$\PP^1 \setminus \{0, \infty\}$ is homotopy equivalent
to $S^1$, contributing only a rank-one local system).
At genus $g \geq 1$, the $E_2$-differentials
are non-trivial, sourced by the monodromy of the KZB
connection around the $A$- and $B$-cycles of $X$;
the $B$-cycle monodromy at genus $1$ produces
$q = e^{2\pi i \hbar}$
(Proposition~\ref{prop:b-cycle-monodromy}).
This local-to-global spectral sequence is the ordered
refinement of the \v{C}ech-to-derived spectral sequence
for the Beilinson--Drinfeld chiral homology on $\Ran(X)$.
\end{remark}
