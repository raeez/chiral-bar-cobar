...
$\CE^{\mathrm{ch}}_k(\fg)$ in $\cD$-modules on
$\Ran(X)$ whose $\Ethree$-deformation space is
$(k+h^\vee) H^3(\fg)[[k+h^\vee]]$. The same space classifies
...
\begin{conjecture}[Universal Miura cross-term coefficient]
\label{conj:miura-cross-universality}
...
Conjecture~\textup{\ref{conj:miura-cross-universality}}]
\label{rem:miura-cross-evidence}
The conjecture is verified at spins $s = 2, 3, 4$
by explicit computation
(\texttt{miura\_coproduct\_universal\_engine.py},
51~tests). At each spin, the coefficient arises from
a cancellation of two contributions:
\begin{enumerate}[label=\textup{(\roman*)}]
\item The Drinfeld formula for
  $\Delta_z(\psi_s)$ produces a
  $\psi_1 \otimes \psi_{s-1}$ cross-term with
  coefficient~$1$ (from
  $\binom{s-2}{s-2} z^0 = 1$ in the general
  formula~\eqref{eq:gl1-coprod-general}).
  Since $\psi_{s-1} = W_{s-1} +{}$~composites,
  this contributes $+1$ to the
  $J \otimes W_{s-1}$ coefficient.
\item The Miura relation
  $\psi_s = W_s + (1/\Psi)\,{:}J \cdot W_{s-1}{:}
  + \cdots$ (where the coefficient of
  ${:}J \cdot W_{s-1}{:}$ is $1/\Psi$ at every
  spin) requires subtracting
  $(1/\Psi)\,\Delta_z({:}J \cdot W_{s-1}{:})$.
  The term $(J \otimes 1)(1 \otimes W_{s-1})$
  in $\Delta_z({:}J \cdot W_{s-1}{:})$ contributes
  $-1/\Psi$.
\item All remaining Miura corrections (normal-ordered
  products of fields of spin ${}<s-1$) cannot produce
  $W_{s-1}$ in either tensor factor. Their contribution
  is zero.
\end{enumerate}
The net coefficient is $1 - 1/\Psi = (\Psi - 1)/\Psi$.
The mechanism is manifestly spin-independent: at each
spin, the Drinfeld formula contributes $+1$ (a
combinatorial identity), and the Miura inversion
subtracts $1/\Psi$ (from the universal coefficient of
${:}J \cdot W_{s-1}{:}$ in the Miura relation). The
conjecture amounts to the assertion that this structural
cancellation persists at all spins $s \geq 2$.

Limiting cases: at $\Psi = 1$ (free boson) the
coefficient vanishes, consistent with the triviality
of the Miura transform; as $\Psi \to \infty$
(classical limit) the coefficient tends to~$1$,
recovering the classical coproduct.
\end{remark}
...
The $\Etwo$-coalgebra structure on $\BarSig(\cA)$
(Warning~\ref{warn:e1-vs-e2-source}) and the Higher
Deligne Conjecture together equip the derived chiral centre
$Z^{\mathrm{der}}_{\mathrm{ch}}(\cA)$ with a natural
$\Ethree$-algebra structure ($\Etwo + 1 = \Ethree$).
The comparison with perturbative Chern--Simons theory
...
\item \emph{Topological $\Ethree$}
  ...
  $\Etwo$ (Warning~\ref{warn:e1-vs-e2-source}), producing a topological
...
Theorem~\textup{\ref{thm:e3-identification}} identifies
the two as families over $(k+h^\vee) H^3(\fg)[[k+h^\vee]]$.
\end{remark}
