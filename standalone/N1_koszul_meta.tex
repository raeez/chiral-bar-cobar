...
free-polynomial cup-product description is not asserted without a
separate multiplicative computation.
The Lagrangian criterion is conditional on the shifted-symplectic
hypothesis package, and $\cD$-module purity is used in the proved
...
one-way support theorem for a supplied family datum $H_H(\cA;S)$.
The Lagrangian and
...
Given a family datum $H_H(\cA;S)$ on the Koszul locus,
$\operatorname{Supp}\ChirHoch^\bullet(\cA)\subseteq S$ and
$\ChirHoch^n(\cA)\cong H^n(K_{\cA,S})$.  A perfect degree-$d$
pairing with $\cA^!$ supplies Hochschild duality as an additional
structure.
...
one-way $H_H$-conditional chiral Hochschild consequence.
...
\item If $\mathsf{FH}_{\Sigma}(\cA)$ holds, chiral Koszulness
 ...
 With the additional package~$H_D^K$, the signed Hodge character is
 $\operatorname{ch}_g(\mathfrak O_g^K(\cA))
 =(-1)^g\kappa(\cA)\lambda_g$.
\item Given $H_H(\cA;S)$ on the Koszul locus,
 $\operatorname{Supp}\ChirHoch^\bullet(\cA)\subseteq S$ and
 $\ChirHoch^n(\cA)\cong H^n(K_{\cA,S})$.  Finite-dimensional
 graded pieces give
 $P_\cA(t)=\sum_{n\in S}\dim H^n(K_{\cA,S})t^n$.
 A perfect degree-$d$ pairing with $\cA^!$ is a separate hypothesis.
\end{enumerate}
...
\subsubsection*{The family datum attached to \textup{(viii)}}

The quasi-isomorphism $\Omega^{\mathrm{ch}}(\barBgeom(\cA))
\xrightarrow{\sim}\cA$ supplies a bar resolution of~$\cA$.
The support assertion uses the further maps
\[
...
\]
together with the chart, inverse-limit, and averaging
quasi-isomorphisms.  These maps give
$\ChirHoch^n(\cA)\cong H^n(K_{\cA,S})$ and support in~$S$.
The bar--cobar counit and the family retract are independent inputs.
A perfect degree-$d$ pairing with the Verdier dual algebra~$\cA^!$
adds the palindromic Hilbert-series relation.
...
degree~$0$, hence
$H^k(\int_{\Sigma}\cA)=0$ for $k\neq 0$.  With the additional
package~$H_D^K$, the surviving Hodge character is
$\operatorname{ch}_g(\mathfrak O_g^K(\cA))
=(-1)^g\kappa(\cA)\lambda_g$.
...
graded is a free polynomial Poisson vertex algebra, and the oscillator
pairing is nondegenerate. Condition~(ix) holds, and
...
(depth $r_{\max} = 2$).  The bounded rank-one even-superboson model
has dimensions $(2,1)$ in degrees $(0,1)$.  A complete chart statement
uses a family datum $H_H(\Heis_k;S)$ and its bounded-to-chart
comparison.  The $A_\infty$-structure is strictly associative, and
the bar--cobar counit is a quasi-isomorphism by direct computation.
...
\]
The polynomial trace-form invariant extends to the critical value with
$\kappa^{\KM}(-h^\vee)=0$.  The Sugawara, KZ, and DS comparison
packages retain their critical poles: the Shapovalov form degenerates
and the Feigin--Frenkel centre enters the completed
centre/Hochschild ambient.  At generic level the PBW criterion applies
with $\kappa^{\KM}\neq0$.
...
coefficient from the OPE~\cite[\S3.1]{LorgatMKD1}.  At the critical
specialisation $k=-h^\vee=-2$, this scalar is zero and the
centre-enlarged companion lane governs the remaining structures. At
level $k=0$ the trace-form
...
and Priddy bar-weight concentration.  A supplied datum
$H_H(\cA;S)$ gives the separate Hochschild support consequence.
Swiss-cheese formality coincides with this hub only
...
$\mathsf{FH}^{\mathrm{det}}$ are available.  Under~$H_D^K$ the
surviving Hodge character is
$\operatorname{ch}_g(\mathfrak O_g^K(\cA))
=(-1)^g\kappa(\cA)\lambda_g$.  Under~$H_D^{\mathrm{graph}}$ the
multi-weight families carry the numerical term
$\delta F_g^{\mathrm{cross}}$.

The Hochschild condition is one-way.  The family datum
$H_H(\cA;S)$ gives support in~$S$ on the Koszul locus.  A perfect
degree-$d$ pairing gives duality, and a multiplicative computation
determines the cup-product algebra.
