...
\title[The Koszulness moduli: meta as moduli-level]
{The Koszulness moduli: reinterpreting the meta-theorem as
a $\mathrm{GRT}_1$-torsor of charts}
...
\begin{abstract}
The \emph{meta} in ``meta-theorem of chiral Koszulness'' is not
a linguistic decoration: it is the
Koszulness moduli scheme $\cM_{\mathrm{Kosz}}(\cA)$, a
$\mathrm{GRT}_1$-torsor whose points parameterize the
equivalent characterizations. The classical list of twelve
equivalences is the $\mathrm{GRT}_1$-coordinate atlas of this
moduli.

Precisely: for a chiral algebra $\cA$ on a smooth projective
curve~$X$ equipped with its PBW filtration, the moduli scheme
$\cM_{\mathrm{Kosz}}(\cA)$ over $\bQ$ has points
$(\Phi, c_\Phi)$ with $\Phi$ a Drinfeld associator and $c_\Phi$
a chart coordinate detecting a Koszulness predicate.
\textup{(A1)} $\cM_{\mathrm{Kosz}}(\cA) \neq \emptyset$ if and
only if $\cA$ is chirally Koszul.
\textup{(A2)} The projection to the associator torsor is a
$\mathrm{GRT}_1$-torsor, transitions between charts are Tamarkin
$\Phi$-transfer maps.
\textup{(A3)} The ten classical characterizations sit as open
charts $U_1, \ldots, U_{10}$ at the KZ associator
$\Phi_{\mathrm{KZ}}$; four supplementary charts $U_{11}$ (AT,
Lagrangian), $U_{12}$ (de~Rham--Betti, Hodge--Betti), $U_{13}$
(Enriquez elliptic), $U_{14}$ (Kontsevich integral) extend the
atlas unconditionally at their own associators.
\textup{(A4)} The shadow-class stratification $G/L/C/M/FF$
selects a distinguished chart on each stratum.

The reformulation makes the Kontsevich--Tamarkin principle
operational: chiral Koszulness is a $\mathrm{GRT}_1$-invariant
\emph{property}, and the twelve classical equivalences are
$\mathrm{GRT}_1$-coordinate \emph{charts}. Two consequences
follow. The Hodge--Betti chart $U_{12}$ closes the Virasoro
circularity \textup{} by a direct Feigin--Fuchs BGG
argument, without bar--cobar detour. The Yangian chart
$U_1^{\mathrm{assoc}} \hookrightarrow U_1^{\mathrm{chiral}}$
reformulates \textup{} as chart-surjectivity.

The core circuit $(i) \Leftrightarrow \cdots \Leftrightarrow
(viii)$ of the classical meta-theorem is now the transition
manifold between the ten KZ-centred charts; the supplementary
circuits (factorization, boundary, monadic, Kac--Shapovalov, Ext)
are chart inclusions. The meta-theorem, read off the scheme, is
verified on the standard landscape: Heisenberg, affine
Kac--Moody at generic level, Virasoro in the Koszul locus, and
the $\beta\gamma$ system.
\end{abstract}
...
\subsection{The meta is the moduli}

The word ``meta'' in \emph{meta-theorem of chiral Koszulness}
is operational: it names the Koszulness moduli scheme
$\cM_{\mathrm{Kosz}}(\cA)$, a $\mathrm{GRT}_1$-torsor. The
classical equivalences listed below are \emph{charts} on this
scheme; the equivalences among them are \emph{transition maps}
induced by the Tamarkin $\Phi$-transfer. We record the moduli
formulation first, then the classical presentation it organizes.

\begin{definition}[Koszulness moduli scheme, N1 form]
\label{def:koszulness-moduli-N1}
For $\cA$ a chiral algebra on~$X$ with PBW filtration, the
Koszulness moduli scheme is the affine $\bQ$-scheme
\[
\cM_{\mathrm{Kosz}}(\cA)
\;=\; \bigl\{(\Phi, c_\Phi) :
\Phi \in \mathrm{Assoc},\;
c_\Phi \in \mathrm{Chart}_\Phi(\cA)\bigr\},
\]
where $\mathrm{Assoc}$ is the Drinfeld associator torsor and
$c_\Phi$ is a chart coordinate: a predicate $\Pi_\Phi$ on
$(\cA, \Phi)$, a witness $\mathrm{wit}_\Phi(\cA)$, and a
Tamarkin transition cocycle $\gamma_\Phi$.
\end{definition}

\begin{theorem}[Meta as moduli]
\label{thm:meta-as-moduli-N1}
$\cM_{\mathrm{Kosz}}(\cA)$ is nonempty iff $\cA$ is chirally
Koszul, is a $\mathrm{GRT}_1$-torsor, and carries a finite
affine atlas $\{U_j\}_{j=1}^{14}$ consisting of
$U_1,\ldots,U_{10}$ at $\Phi_{\mathrm{KZ}}$, $U_{11}$ at
$\Phi_{\mathrm{AT}}$, $U_{12}$ at $\Phi_{\mathrm{dR,B}}$,
$U_{13}$ at $\Phi_{\mathrm{ell}}$, $U_{14}$ at
$\Phi_{\mathrm{Kon}}$, with the shadow-class stratification
selecting a distinguished chart on each stratum.
\end{theorem}

The proof is a direct reading of the classical meta-theorem as a
chart atlas; it is given in
\cite[Theorem~2.2]{LorgatMKD1_standalone}.

\subsection{The meta-theorem (classical presentation)}
...
the meta-theorem converts it to all eight genuinely independent
bidirectional equivalences (among (i)--(x), with (vi) a proved
consequence of (v) and (viii) a one-way chiral Hochschild consequence
on the Koszul locus) at once. The Lagrangian criterion~(xi) holds unconditionally for
...
% ================================================================
\appendix
\section{Note added in proof: 2026-04-16/17 strengthenings}
\label{sec:n1-note-added}
% ================================================================

Between the first circulation of this note and the 2026-04-17
Beilinson-rectification audit, the twelve-equivalence meta-theorem
acquired a sharper internal structure at genus~$0$ and refined
scope qualifiers at higher genus.

\subsection*{Seven-fold \textsc{tfae} hub-and-spoke at genus~$0$}

The core circuit
$(\mathrm{i})\Leftrightarrow(\mathrm{ii})\Leftrightarrow(\mathrm{iii})
\Leftrightarrow(\mathrm{v})\Leftrightarrow(\mathrm{viii})$
of Theorem~\ref{thm:koszul-equivalences-meta} is reorganised, at
genus~$0$, as a hub-and-spoke \textsc{tfae}
(Vol~I \verb|chapters/theory/ftm_seven_fold_tfae_platonic.tex|,
\verb|thm:ftm-seven-fold-tfae-via-hub-spoke|). Each spoke is
bidirected to the PBW $E_2$-collapse \emph{hub}:
\begin{enumerate}[label=\textup{(S\arabic*)},leftmargin=2.6em]
\item Koszul morphism (condition~(i));
\item bar-cobar counit quasi-isomorphism (condition~(v));
\item bar-cobar unit weak equivalence;
\item twisted tensor product acyclicity (equivalent to PBW);
\item bar cohomology concentrated in weight~$1$
 (the Priddy condition, condition~(ii));
\item Swiss-cheese formality \textup{(class-$G$-scoped: equivalent
to the hub on the class-$G$ stratum; class-$L$, $C$, $M$ members
of the Koszul locus are Koszul but not SC-formal)};
\item $\ChirHoch^*$ polynomial in $\{0,1,2\}$ (condition~(viii)).
\end{enumerate}
Spokes~(S1)--(S5) and (S7) are hub-equivalent on the full Koszul
locus; Spoke~(S6) is hub-equivalent only on class~$G$.
Pairwise bidirection count on the Koszul locus reduces to six
proved spokes; the remaining equivalences follow by transitivity.
Spoke~(S4) is the load-bearing non-tautology, witnessed on
$V_k(\mathfrak{sl}_2)$ at generic level.

\subsection*{Scope qualifiers}

\textbf{(N1-Q1) Condition~(vi) Barr--Beck--Lurie monadicity.}
The monadicity equivalence is stated at genus~$0$ level; the
all-genera extension is contingent on the Francis--Gaitsgory
$(\infty,2)$-descent and inherits the scope of Theorem~$\mathrm{A}^{\infty,2}$
(\verb|thm:A-infinity-2|, Vol~I
\verb|chapters/theory/theorem_A_infinity_2.tex|).

\textbf{(N1-Q2) Condition~(vii) factorization homology.}
The concentration-in-degree-$0$ statement of~(vii) is
uniform-weight at $g\ge 2$; multi-weight families
($\cW_N$,~$N\ge 3$) carry a cross-channel correction
$\delta F_g^{\mathrm{cross}}$ at $g\ge 2$, explicit at $(W_3,g=2)$:
$\delta F_2^{\mathrm{cross}}(W_3)=(c+204)/(16c)$
(\verb|prop:delta-f-cross-w3-g2|). The meta-theorem
(vii) is stated on the uniform-weight lane; outside the
uniform-weight lane the equivalence with~(i) still holds,
with a multi-weight interpretation of the concentration.

\textbf{(N1-Q3) Condition~(viii) $\ChirHoch^*$ concentration.}
The Step~3 circularity risk in the original proof of Theorem~H,
used at the $(\mathrm{v})\Rightarrow(\mathrm{viii})$ and
$(\mathrm{viii})\Rightarrow(\mathrm{v})$ arrows, is rerouted
through \verb|thm:pbw-koszulness-criterion| (Vol~I
\verb|chapters/theory/chiral_pbw_koszulness.tex|): the
Shelton--Yuzvinsky Koszulity of braid-arrangement Orlik--Solomon
algebras plus the chiral quadratic-Koszul lemma make bar-concentration
and FM-tower collapse PARALLEL consequences of PBW, not sequential,
retiring the earlier sequential routing.

\textbf{(N1-Q4) Condition~(xi) Lagrangian criterion.}
Perfectness, previously a hypothesis, is now a theorem on the
standard landscape (\verb|prop:perfectness-standard-landscape|),
verified family-by-family: Heisenberg, affine Kac--Moody at
non-critical level via Kac--Kazhdan, Virasoro at generic~$c$,
$\cW_N$ via Fateev--Lukyanov, lattice via theta/eta, $\beta\gamma$
via character. Consequently~(xi) becomes an UNCONDITIONAL
equivalence on the standard landscape; off-landscape, it remains
conditional on perfectness.

\textbf{(N1-Q5) Condition~(xii) $\cD$-module purity.}
One direction ((xii)$\Rightarrow$(x)) remains PROVED; the
converse (x)$\Rightarrow$(xii) was proved in Vol~I for the
affine Kac--Moody family; it is not extended in the 2026-04-16
wave and remains open in general.

\subsection*{$\mathrm{GRT}_1$-torsor structure: heptagon closure}

The five presentations of the Koszulness moduli are organised,
in Vol~II \verb|chapters/theory/sc_chtop_heptagon.tex|
(\verb|thm:sc-chtop-heptagon|), as a heptagon of
seven named edge theorems: five classical faces plus
face~(6) Drinfeld-centre
$Z(\mathrm{Rep}_{\mathrm{fact}}(\cA))\simeq
\mathrm{Rep}_{\mathrm{fact}}(Z^{\mathrm{der}}_{\mathrm{ch}}(\cA))^{E_2}$
via categorified bar-cobar with half-braiding, plus
face~(7) derived-algebraic-geometry via PTVV on
$\mathrm{Map}(X\times\mathbb{R}_{\ge 0},B\mathrm{SC\text{-}Alg})$.
The $\mathrm{GRT}_1$-torsor of this paper is the automorphism
torsor of this heptagon; the chart atlas of
Theorem~\ref{thm:meta-as-moduli-N1} is compatible with the
heptagon's seven-fold global structure.

\subsection*{Admissible Kazhdan--Lusztig periodic~CDG}

For a chiral algebra at admissible level $k+h^\vee=p/q$, the
meta-theorem extends via the periodic~CDG filtration
$F^n=\ker(Q^n_{\mathrm{adm}})$ on $\mathrm{KL}_k^{\mathrm{adm}}$
(\verb|thm:periodic-cdg-is-koszul-compatible|,
Vol~I \verb|chapters/theory/periodic_cdg_admissible.tex|):
periodic~CDG is Koszul-duality-compatible, and the bar-cobar
adjunction $\Omega^{\mathrm{ch}}\dashv B^{\mathrm{ch}}$ descends
UNCONDITIONALLY to $\mathrm{KL}_k^{\mathrm{adm}}
\rightleftarrows(\mathrm{KL}_{k^!}^{\mathrm{adm}})^{\mathrm{op}}$.
The class-$\mathsf{M}$ admissible minimal model
$\mathrm{KL}^{\mathrm{adm}}_{\mathrm{Vir}}(c_{p,q})$ has
$(p-1)(q-1)/2$ simples, all finite-length
(\verb|cor:class-M-admissible-minimal-model|). This closes
the admissible-level sector of the meta-theorem's coverage on
