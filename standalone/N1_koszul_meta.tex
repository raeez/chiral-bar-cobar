...
genus~$0$, as a seven-fold \textsc{tfae} hub-and-spoke
(Vol~I \verb|chapters/theory/ftm_seven_fold_tfae_platonic.tex|,
\verb|thm:ftm-seven-fold-tfae-via-hub-spoke|). Six spokes are
each bidirected to the PBW $E_2$-collapse \emph{hub}:
...
\item Swiss-cheese formality;
\item $\ChirHoch^*$ polynomial in $\{0,1,2\}$ (condition~(viii)).
\end{enumerate}
Pairwise bidirection count reduces from twenty-one to six:
each spoke is proved equivalent to the PBW hub directly, and the
remaining equivalences follow by transitivity. Spoke~(S4) is the
load-bearing non-tautology, witnessed on~$V_k(\mathfrak{sl}_2)$
at generic level.
