...
equivalent characterizations. The ten KZ-centred core equivalences,
enlarged below by auxiliary criteria, form the
$\mathrm{GRT}_1$-coordinate atlas of this
moduli.
...
\item factorization homology
 $\int_{\Sigma_g}\cA$ is concentrated in degree~$0$ at genus~$0$,
 and for $g\ge2$ on the uniform-weight scalar lane; off that lane
 one must include the cross-channel correction;
...
(failure mode~132 of~\cite{LorgatMKD1}).
\end{remark}
...
\begin{theorem}[Ten core equivalences with two auxiliary criteria]
...
by PBW collapse.
For $g\ge2$ this argument is the uniform-weight scalar lane; for
multi-weight algebras the concentration statement is replaced by the
cross-channel-corrected genus identity.
...
The twelve-equivalence meta-theorem has a sharper internal
structure at genus~$0$ and refined scope qualifiers at higher
genus.
...
affine Kac--Moody family; the general case remains open.
