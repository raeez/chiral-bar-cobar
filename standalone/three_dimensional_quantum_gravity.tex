...
expansion is the perturbative partition-function shadow) constitutes
a conditional algebraic MC model for the gravitational theory.
...
gravitational S-duality; self-duality at $c = 13$. An algebraic
avatar of the Page curve emerges from complementarity: the two
Koszul-dual saddles exchange dominance at
Page time $t_P = 3S_{\mathrm{BH}}/13$, $c$-independent
because $\kappa(\Vir_c) + \kappa(\Vir_{26-c}) = 13$; the crossing
structure is a structural shadow of the Page transition, while the
full information-theoretic content requires the island mechanism
(Penington, AEMM). BTZ black holes
...
% ================================================================
\section{The Page-curve shadow from complementarity}
\label{sec:page-curve}

Under the island-saddle and entropy-rate hypotheses, the
...
The derivation below is an algebraic Page-curve shadow from the
Lagrangian decomposition
of the genus expansion evaluated on the BTZ background,
derives the evaporation rate from the genus-$1$ free energy,
and identifies the Page time as the crossing of the two
saddle-point contributions. The $c$-independent Page time
$t_P = 3S_{\mathrm{BH}}/13$ is a consequence of the
complementarity constant, not an assumption.
...
Three-dimensional quantum gravity is not a motivational appendix
to the modular Koszul programme. It is the first nontrivial
gravitational realization of the iterated Sugawara
$E_\infty$-topologisation ladder, the chiral higher-Deligne
identification of the bulk, universal celestial holography, and the
topologization tower across the four shadow classes.
...
\begin{remark}[The climax is the theorem, not the motivation]
\label{rem:climax-identity}
The relationship ``3d quantum gravity IS the platonic theorem'' is
precise: the universal Maurer--Cartan element $\Theta_{\Vir_c}$ of
the modular convolution algebra encodes, via its MC projections to
successive degrees, the complete perturbative gravitational genus
expansion. Koszul duality $\Vir_c \leftrightarrow \Vir_{26-c}$ is
gravitational S-duality; Page-time complementarity
$t_P = 3 S_{\mathrm{BH}}/13$ is the structural shadow of the Page
transition (the full information-theoretic content requires the
island mechanism, which is supplied by the bulk $E_3$-topological
structure of $Z^{\mathrm{der}}_{\mathrm{ch}}(\Vir_c)$). BTZ black
holes are MC elements; de~Sitter entropy emerges from analytic
continuation $\kappa \to -|\kappa|$. The iterated Sugawara ladder
(Theorem~\ref{thm:einfty-ladder}) reveals this gravity as
the $N = 2$ \emph{shadow} of an $N \to \infty$ topological
tower: the full platonic endpoint is
$E_\infty^{\mathrm{top}}$ gravity in $3+\infty$ dimensions, of
which 3d quantum gravity is the lowest truncation.
\end{remark}
