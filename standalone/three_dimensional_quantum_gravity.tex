...
% ================================================================
% Paper M: Algebraic Holographic HT Sector with Virasoro
%          Boundary and Derived Chiral Centre Bulk
% TARGET: Annals of Mathematics (or Inventiones)
% LENGTH: 80-100pp
% ================================================================
%
% This paper identifies the ALGEBRAIC SECTOR of 3d gravity with
% negative cosmological constant. It constructs the open Beilinson
% tower with boundary $\partial = \Vir_c$, bulk
% $Z^{\mathrm{der}}_{\mathrm{ch}}(\Vir_c)$, and interaction the
% $\mathsf{SC}^{\mathrm{ch,top}}$-brace structure. It does NOT
% construct a compact dynamical-metric path integral; that is the
% Vol I open frontier F12 and requires BTZ/Cardy hypotheses
% (modular invariance + vacuum dominance + saddle stability) plus
% the Maloney--Witten + Manschot--Moore Farey-tail modular
% completion. scalar-shadow discipline observed throughout: scalar shadow
% projections do not equal the operator algebra; the trace
% direction (operators $\to$ scalar) is well-defined; the inverse
% direction is the named Pfaffian / square-root open problem.
% ================================================================
...
\title[Algebraic Holographic HT Sector for 3d Gravity]
{Algebraic Holographic HT Sector with Virasoro Boundary\\
 and Derived Chiral Centre Bulk}
...
constant is $SL(2,\mathbb{R}) \times SL(2,\mathbb{R})$ Chern--Simons
(Ach\'ucarro--Townsend, Witten); its boundary symmetry algebra is
the Virasoro algebra at central charge $c = 3\ell/(2G)$
(Brown--Henneaux). We identify the \emph{algebraic holographic
HT~sector} of this theory: the open Beilinson tower with
boundary~$\partial = \Vir_c$, bulk
$Z^{\mathrm{der}}_{\mathrm{ch}}(\Vir_c)
=C^\bullet_{\mathrm{ch}}(\Vir_c,\Vir_c)$,
and interaction the $\mathsf{SC}^{\mathrm{ch,top}}$-brace structure
on the pair $(Z^{\mathrm{der}}_{\mathrm{ch}}(\Vir_c), \Vir_c)$. The
ordered bar complex $\barB^{\mathrm{ord}}(\Vir_c)$ on
$\mathbb{C} \times \mathbb{R}$ is the twisting coalgebra; the
derived chiral centre carries $E_3^{\mathrm{top}}$ structure via
Sugawara topologisation (cohomological + weight-completed scope);
the universal Maurer--Cartan element $\Theta_{\Vir_c}$ generates the
shadow obstruction tower $\{S_r\}_{r \ge 2}$ recording the
boundary-to-scalar trace of the algebraic sector.

The trace direction (operators $\to$ scalar) is well-defined and
unconditional on the Virasoro Koszul locus: $\kappa(\Vir_c) = c/2$
is the Brown--Henneaux central charge; the quartic shadow
$10/[c(5c{+}22)]$ matches the level-$4$ Kac determinant; the
shadow free energies $F_g = \kappa\,\lambda_g^{\mathrm{FP}}$
reproduce the perturbative one-loop Hodge-integral expansion. The
inverse direction (scalar $\to$ operator), in particular the
construction of a compact dynamical-metric path integral from the
algebraic sector, is the Vol~I open frontier~F12: it requires the
BTZ / Cardy hypothesis package (modular invariance, vacuum
dominance, saddle stability) together with the Maloney--Witten
plus Manschot--Moore Farey-tail modular completion. Koszul duality
$\Vir_c \leftrightarrow \Vir_{26-c}$ is the algebraic shadow of
S-duality; the self-dual point sits at $c = 13$. The algebraic Page
curve emerges from complementarity:
$\kappa(\Vir_c) + \kappa(\Vir_{26-c}) = 13$ forces saddle exchange
at $t_P = 3S_{\mathrm{BH}}/13$, $c$-independent. The crossing is the
structural shadow of the Page transition; the full
information-theoretic content remains controlled by the island
mechanism (Penington, AEMM). BTZ black holes are Maurer--Cartan
elements of the gravitational convolution algebra; de~Sitter entropy
arises by analytic continuation $\kappa \to -|\kappa|$.
\end{abstract}
...
\begin{remark}[Scope: algebraic sector versus dynamical-metric path integral]
\label{rem:scope-algebraic-sector}
The Ach\'ucarro--Townsend--Witten equivalence holds at the classical
level: both theories have the same equations of motion and the same
moduli of flat connections. At the quantum level, the Chern--Simons
path integral is a topological invariant (the Reshetikhin--Turaev
functor), while a compact dynamical-metric gravitational path
integral must also sum over topologies and saddle geometries.

The \emph{algebraic holographic HT sector} of the gravitational theory
is the open Beilinson tower
$\partial = \Vir_c$, bulk
$Z^{\mathrm{der}}_{\mathrm{ch}}(\Vir_c)
=C^\bullet_{\mathrm{ch}}(\Vir_c,\Vir_c)$,
interaction the $\mathsf{SC}^{\mathrm{ch,top}}$-brace structure on
$(Z^{\mathrm{der}}_{\mathrm{ch}}(\Vir_c), \Vir_c)$. This sector is
the level-3 / level-4 / level-5 Beilinson rungs of the gravitational
theory; it is unconditional on the Virasoro Koszul locus (generic
$c$).

The \emph{trace direction} (operators $\to$ scalar) descends from
the algebraic sector to modular and partition-function shadows by
trace + clutching on the open category; this is the content of
Sections~\ref{sec:shadow-as-gravity}--\ref{sec:koszul-s-duality}
and is unconditional. The \emph{inverse direction} (scalar $\to$
operator), in particular the construction of a compact
dynamical-metric path integral from the algebraic sector, is the
Vol~I open frontier~F12 and requires a distinct hypothesis package:
the BTZ / Cardy hypotheses (modular invariance + vacuum dominance +
saddle stability) together with the Maloney--Witten
plus Manschot--Moore Farey-tail modular completion. The reverse
direction is a separate conjectural inverse problem, stated in
Section~\ref{sec:btz} and Section~\ref{sec:f12-frontier}.
\end{remark}
...
% kappa-normalization: kappa(Vir) from C2; c=0 -> 0, c=13 -> 13/2 verified
...
% augmentation-ideal: augmentation ideal present
% desuspension: desuspension s^{-1}, |s^{-1}v| = |v|-1
...
\subsection{The collision kernel and its represented $r$-matrix}

The collision kernel is obtained from the $\lambda$-bracket by the
binary residue construction.  A represented classical $r$-matrix
arises under $H_{\mathrm{CYBE}}^{\mathrm{rep}}$.
...
K_{\Vir_c}^{\mathrm{coll}}(z) \;=\;
\frac{c/2}{z^3} + \frac{2T}{z},
\end{equation}
% level-shift: level prefix c/2 present; at c=0, scalar part vanishes
...
\begin{proposition}[Represented Yang--Baxter equation]
\label{prop:vir-ybe}
Assume a representation~$\rho$ and
$H_{\mathrm{CYBE}}^{\mathrm{rep}}(\Vir_c,\rho)$.  Then
The CYBE
...
= 0
\end{equation}
holds for $r=\rho(K_{\Vir_c}^{\mathrm{coll}})$ from
\eqref{eq:vir-r-matrix}. It is the
...
% ================================================================
\section{The derived chiral centre and the gravitational comparison}
\label{sec:gravitational-bulk}

The bar complex computes the boundary data. The level-$3$
closed-sector object is the derived chiral centre
...
parameters from $\mathrm{FM}_k(\mathbb{C})$. A gravitational bulk is
obtained from it only after the Virasoro open--closed comparison map
has been supplied.
...
\textbf{Type signature.} Quadrant: Open.
Presentation: bounded vertex cochains followed by a
weight-completed curve chart.
Level: 3 (derived chiral centre as universal closed sector).
Hypothesis package: the Bakalov--De Sole--Kac bounded complex and a
bounded-to-chart quasi-isomorphism
\[
 \chi^{\mathrm{bd}}_{\mathrm{Vir}_c}\colon
...
\begin{equation}\label{eq:hhn}
 ...
 \end{cases}
\end{equation}
Hence $P^{\mathrm{bd}}_{\mathrm{Vir}_c}(t)=1+t^2+t^3$.
The map $\chi^{\mathrm{bd}}_{\mathrm{Vir}_c}$ transports this
dimension vector to the curve-chart chiral Hochschild complex.
\end{theorem}

\begin{proof}
The bounded calculation is \cite[Theorem~7.2]{BDSK21}.  Cohomology
is invariant under the displayed quasi-isomorphism, which proves the
chart statement.
\end{proof}

\begin{corollary}[The Virasoro universal closed sector]
\label{cor:bulk-gravity}
Under $\chi^{\mathrm{bd}}_{\mathrm{Vir}_c}$, the cohomology of the
universal closed-sector dg algebra
\begin{equation}\label{eq:bulk}
 Z^{\mathrm{der}}_{\mathrm{ch}}(\mathrm{Vir}_c)
 =C^\bullet_{\mathrm{ch}}(\mathrm{Vir}_c,\mathrm{Vir}_c)
\end{equation}
has Hilbert series $1+t^2+t^3$.  A Virasoro open--closed
quasi-isomorphism
$\Psi_{\mathrm{OCA}}\colon\cO^{\mathrm{phys}}_{\mathrm{bulk}}
\xrightarrow{\sim}Z^{\mathrm{der}}_{\mathrm{ch}}(\mathrm{Vir}_c)$
transports this dg algebra to physical bulk observables.  The
classical absence of local propagating gravitons follows separately
from the Chern--Simons presentation of three-dimensional gravity.
\end{corollary}

\begin{proof}
Theorem~\ref{thm:vir-hochschild} gives the cohomology statement.
The second assertion is transport along $\Psi_{\mathrm{OCA}}$.
Ach\'ucarro--Townsend and Witten identify the classical gravitational
field equations with flatness of the Chern--Simons connection
\cite{AT86,Wit88}.
\end{proof}
...
\textbf{Type signature.} Quadrant: Open.
Presentation: cohomological + weight-completed (chain-level on the
original direct-sum complex is Frontier~F3, conditional on the
Felder BRST Mittag--Leffler tower).
Level: 3 (derived chiral centre).
Hypothesis package: non-critical level $k \ne -2$ on the affine
side; Sugawara primitive $G_{\mathrm{Sug}}$ with
$[Q_{\mathrm{tot}}, G_{\mathrm{Sug}}] = T_{\mathrm{Sug}}$ on
cohomology; $A_\infty$ coherence of the topologisation primitives
in the chain-level lift (open at Class~$\mathbf M$).
...
\end{equation}
These constructions form the algebraic
closed-sector/boundary/line comparison diagram.  The bar complex
supports three typed functors:
\begin{enumerate}[label=(\roman*)]
\item $\Omega(\bar{B}) \simeq \mathrm{Vir}_c$
  (cobar recovers the boundary; bar-cobar adjunction),
\item
  \(K_X(\mathrm{Vir}_c)
    :=\mathbb D_{\mathrm{Ran}}\bar B_X^{\mathrm{ch}}(\mathrm{Vir}_c)\)
  is the full-bar Verdier algebra, with a chosen comparison
  \(\nu_c\colon K_X(\mathrm{Vir}_c)\to\mathrm{Vir}_c^!\);
\item $\mathrm{RHom}_{\mathrm{Vir}_c^e}
  (\mathrm{Vir}_c,\mathrm{Vir}_c)
  =C^\bullet_{\mathrm{ch}}(\mathrm{Vir}_c,\mathrm{Vir}_c)$
  is the universal algebraic closed sector.  An OCA realization
  \(\Psi_{\mathrm{OCA}}\) supplies its physical bulk interpretation.
\end{enumerate}

% ================================================================
\section{The shadow tower as algebraic-sector trace}
\label{sec:shadow-as-gravity}

The universal Maurer--Cartan element $\Theta_{\mathrm{Vir}_c}$ in
the modular convolution algebra
$\mathfrak{g}^{\mathrm{mod}}_{\mathrm{Vir}_c}$ satisfies the MC
equation $D\Theta + \frac{1}{2}[\Theta, \Theta] = 0$ and encodes
the algebraic-sector trace of the perturbative expansion of 3d
gravity (operator $\to$ scalar trace direction; level-4 $\to$
level-5 of the Beilinson tower). Its projections to successive
degrees constitute the \emph{shadow obstruction tower}, a sequence
of invariants $\{S_r\}_{r \ge 2}$ that record the perturbative
corrections to the semiclassical gravitational dictionary at the
level of the algebraic sector. The inverse construction (from the
$\{S_r\}$ to a compact dynamical-metric path integral) is the
named open obstruction F12; here we compute the trace.
...
scalar genus shadow of the gravitational partition function through the
...
% ================================================================
\section{Koszul duality as the algebraic shadow of S-duality}
...
algebraic duality is the level-2 / level-5 algebraic shadow of
gravitational S-duality, computed by trace + clutching on the
algebraic sector; the self-dual point $c = 13$ is the fixed point
of the Koszul involution. Lifting this algebraic shadow to a
nonperturbative S-duality on the dynamical-metric path integral
is part of F12 (Section~\ref{sec:f12-frontier}).

\subsection{The Koszul dual}

\begin{conjecture}[Virasoro reflection comparison]
\label{thm:vir-koszul}
Fix a completed Virasoro presentation and its Verdier/Koszul
comparison \(\nu_c\colon K_X(\mathrm{Vir}_c)\to\mathrm{Vir}_c^!\).
The proposed reflection comparison is a quasi-isomorphism
\begin{equation}\label{eq:vir-koszul}
\chi_c\colon \mathrm{Vir}_c^! \xrightarrow{\;\sim\;}
\mathrm{Vir}_{26-c}.
\end{equation}
Its scalar consequence is
...
\simeq \mathrm{Vir}_{13}$ after choosing the coherence of
\(\chi_{13}\).
\end{conjecture}

\begin{remark}[Comparison obligation]
The scalar identity in~\eqref{eq:complementarity} follows directly
from the two displayed central-charge parameters.  The categorical
statement~\eqref{eq:vir-koszul} requires a chain map on the completed
Virasoro bar model, compatibility with the Virasoro relations, and a
quasi-isomorphism proof.  These three constructions constitute the
reflection-comparison obligation.
\end{remark}
...
Koszul datum}: a seven-entry algebraic package.  It becomes a
physical bulk datum only after the open--closed comparison and the
line-comparison maps are supplied.

\subsection{The seven-entry package}
...
$c = 3\ell/(2G)$ is the seven-entry package
...
\item $\mathrm{Vir}_c$: the \textbf{boundary algebra}, the
  ...
  $\kappa(\mathrm{Vir}_c) = c/2$;
  % kappa-normalization: kappa(Vir) from C2; c=0 -> 0, c=13 -> 13/2 verified
\item Fix a quadratic-linear presentation
  \(\mathrm{Vir}_c=T_X(V_c)/(R_c)\).  Its
  \textbf{presentation coalgebra} is
  \(\mathrm{Vir}_c^{\mathrm i}=C_X(s^{-1}V_c,s^{-2}R_c)\), with
  \(q_c\colon\mathrm{Vir}_c^{\mathrm i}\to
  \bar B_X^{\mathrm{ch}}(\mathrm{Vir}_c)\).
\item The \textbf{strict Koszul partner} \(\mathrm{Vir}_c^!\)
  is reached from
  \(K_X(\mathrm{Vir}_c)=\mathbb D_{\Ran}
  \bar B_X^{\mathrm{ch}}(\mathrm{Vir}_c)\)
  through the chosen pair map
  \(\nu_c\colon K_X(\mathrm{Vir}_c)\to\mathrm{Vir}_c^!\).
  Conjecture~\textup{\ref{thm:vir-koszul}} formulates the further
  reflection comparison with \(\mathrm{Vir}_{26-c}\).
\item $Z^{\mathrm{der}}_{\mathrm{ch}}(\mathrm{Vir}_c)$: the
  \textbf{algebraic closed-sector object}.  The open--closed map
  \(\Psi_{\mathrm{OCA}}\) realizes it in the physical bulk when the
  OCA comparison package holds.  The line category is
  $\mathrm{Vir}_c^!\text{-}\mathsf{mod}$ on the strict
...
\begin{remark}[The algebraic closed/open/line triangle]
\label{rem:koszul-triangle}
Entries \textup{(i)}, \textup{(iii)}, and \textup{(iv)} form the
\emph{algebraic closed/open/line triangle}:
\begin{center}
\begin{tabular}{@{}rcl@{}}
\textbf{Closed sector} & $=$ & $Z^{\mathrm{der}}_{\mathrm{ch}}(\mathrm{Vir}_c)
  =C^\bullet_{\mathrm{ch}}(\mathrm{Vir}_c,\mathrm{Vir}_c)$ \\
\textbf{Boundary} & $=$ & $\mathrm{Vir}_c$ \\
\textbf{Lines} & $=$ & $\mathrm{Vir}_c^!\text{-mod}$
  (on Koszul locus)
\end{tabular}
\end{center}
The closed-to-boundary edge is the Swiss-cheese restriction
\(\rho_\partial\colon
Z^{\mathrm{der}}_{\mathrm{ch}}(\mathrm{Vir}_c)\to\mathrm{Vir}_c\).
The boundary-to-lines edge is the composite of \(q_c\), Ran-space
Verdier duality, and \(\nu_c\).  The OCA comparison
\(\Psi_{\mathrm{OCA}}\) maps the algebraic closed sector to the
physical bulk observables.
\end{remark}

\subsection{Explicit computation of the seven-entry package}

The seven-entry package $\mathcal{H}(\mathrm{Vir}_c)$ is determined
by $c$. Its nonformal numerical projections admit closed-form
expressions.

\begin{theorem}[The holographic datum: explicit form]
\label{thm:holographic-explicit}
The seven entries of\/ $\mathcal{H}(\mathrm{Vir}_c)$ are determined
by the single datum $c$; their numerical projections include:
\begin{enumerate}[label=\textup{(\roman*)}]
...
\begin{proof}
Each listed projection was computed in the preceding sections:
...
\subsection{The genus-$1$ formal coefficient and the Weinberg soft theorem}

\begin{theorem}[Genus-$1$ formal coefficient]
\label{thm:F1-derivation}
The genus-$1$ formal scalar coefficient of\/ $\mathrm{Vir}_c$ is
(UNIFORM-WEIGHT):
...
\subsection{The genus-$2$ formal coefficient}

\begin{theorem}[Genus-$2$ formal coefficient]
\label{thm:F2-derivation}
The genus-$2$ formal scalar coefficient is (UNIFORM-WEIGHT):
...
\frac{7c}{11520}.
\end{equation}
% kappa-normalization: kappa(Vir) from C2; verified: c=0 -> 0
...
$2{+}1$-dimensional gravity with $\Lambda < 0$. We identify it as a
Maurer--Cartan element of the gravitational convolution algebra of
the algebraic sector. The identification is unconditional at the
algebraic level (boundary $A_b = \Vir_c$, MC element
$\alpha_{\mathrm{BTZ}}$); the lift of $\alpha_{\mathrm{BTZ}}$ to a
saddle of a compact dynamical-metric path integral, and the
recovery of microstate counts from a sum over geometries, requires
the BTZ / Cardy hypothesis package and the Maloney--Witten +
Manschot--Moore Farey-tail completion (Section~\ref{sec:f12-frontier}).

\begin{quote}
\textbf{BTZ / Cardy hypothesis package} (named for use throughout
this section and Section~\ref{sec:page-curve}):
\begin{enumerate}[label=\textup{(BC\arabic*)}]
\item \emph{Modular invariance.} The torus partition function
  $Z_1(\tau)$ is invariant under $\mathrm{SL}_2(\mathbb{Z})$, and in
  particular under the $S$-transform $\tau \mapsto -1/\tau$ used in
  the Cardy saddle-point.
\item \emph{Vacuum dominance.} As $\tau \to i 0^+$ along the
  imaginary axis, $Z_1(\tau)$ is dominated by the Virasoro vacuum
  representation: $Z_1(\tau) \sim e^{-2\pi i (-c/24)\tau} \cdot (1 +
  O(q))$.
\item \emph{Saddle stability.} The saddle-point approximation to
  the modular $S$-transform is uncorrected at leading order in
  $h \gg c$, with subleading corrections enumerated by the
  Hardy--Ramanujan circle method (Faulkner--Lewkowycz subleading;
  Manschot--Moore Farey expansion).
\item \emph{Maloney--Witten Farey-tail completion.} The sum over
  $\mathrm{SL}_2(\mathbb{Z})/\mathbb{Z}$ images of the BTZ saddle
  produces the Maloney--Witten partition function
  $Z_{\mathrm{MW}}(\tau)$; the Manschot--Moore Farey-tail
  prescription regularises this (otherwise non-convergent /
  negatively-graded) sum into a holomorphic mock-modular
  completion.
\end{enumerate}
The identification of $\alpha_{\mathrm{BTZ}}$ as an MC element is
unconditional. The Cardy entropy formula
(Theorem~\ref{thm:cardy-bh}) below uses (BC1)--(BC3); the
genus-$g$ saddle expansion (Proposition~\ref{prop:btz-genus}) uses
(BC3); the construction of a compact gravitational partition
function from the algebraic sector uses all four hypotheses and is
the F12 open problem.
\end{quote}
...
\label{thm:btz-mc}
\textbf{Type signature.} Quadrant: Open.
Presentation: chain-level (Maurer--Cartan in the modular
convolution algebra of the algebraic sector).
Level: 4 (operator-level: line / flat-connection encoding).
Hypothesis package: $\Lambda < 0$ (AdS$_3$); flat $A_\pm$
connection on the solid torus $D \times S^1$ with prescribed
holonomy; identification of holonomy weights with conformal
weights $(h, \bar h)$ via Brown--Henneaux. The lift to a saddle
of a compact dynamical-metric path integral requires (BC1)--(BC4)
and is the F12 frontier.
...
\label{thm:cardy-bh}
\textbf{Type signature.} Quadrant: Open.
Presentation: chain-level + Cardy saddle.
Level: 5 (modular trace of level-$4$ operators).
Hypothesis package: BTZ / Cardy (BC1)--(BC3) above; trace
direction (operators $\to$ scalar) only.
...
% ================================================================
\section{The algebraic Page-curve shadow from complementarity}
\label{sec:page-curve}

Under the BTZ / Cardy hypothesis package
(Section~\ref{sec:btz}, (BC1)--(BC3)) and the island-saddle
mechanism upstream (Penington~\cite{Pen19},
Almheiri--Engelhardt--Marolf--Maxfield~\cite{AEMM19,AMMZ19}), the
complementarity constant $K = \kappa(\mathrm{Vir}_c)
+ \kappa(\mathrm{Vir}_{26-c}) = 13$ controls the transition
between the two Koszul-dual phases of the algebraic-sector
saddle competition. The derivation below is the algebraic Page
shadow of the gravitational Page curve: the Lagrangian
decomposition of the genus expansion evaluated on the BTZ
Maurer--Cartan element, the evaporation rate from the genus-$1$
free energy, and the Page time as the crossing of the two
saddle-point contributions on the algebraic sector. The
$c$-independent Page time $t_P = 3S_{\mathrm{BH}}/13$ is a
consequence of complementarity, not an assumption. The full
information-theoretic Page curve requires the island mechanism on
the dynamical-metric path integral (a distinct construction); the
present theorem computes the algebraic-sector saddle exchange.

\subsection{The Lagrangian decomposition on the BTZ background}

The complementarity theorem (Theorem~\ref{thm:vir-koszul})
decomposes the genus-$g$ bar complex into Koszul-dual halves.
We evaluate this decomposition on the BTZ Maurer--Cartan element
$\alpha_{\mathrm{BTZ}} \in \mathfrak{g}^{\mathrm{mod}}$
(Theorem~\ref{thm:btz-mc}), producing a two-saddle
partition function parametrised by the black hole mass.

\begin{construction}[Lorentzian BTZ genus expansion]
\label{con:lorentzian-btz}
Let $\alpha_{\mathrm{BTZ}}$ be the MC element at conformal weight
$h$ (non-rotating, $J = 0$). The genus-summed free energy
(Proposition~\ref{prop:btz-genus}) is:
\begin{equation}\label{eq:btz-genus-sum}
\mathcal{F}(h;\,\hbar)
\;=\;
S_{\mathrm{BH}}(h)\,\hbar^{-2}
\;+\;
\sum_{g \ge 1} F_g(\alpha_{\mathrm{BTZ}})\,
\hbar^{2g-2},
\end{equation}
where $S_{\mathrm{BH}}(h) = 2\pi\sqrt{ch/6}$ and
$\hbar = S_{\mathrm{BH}}^{-1}$.
The Lorentzian continuation replaces the Euclidean modular
parameter $\tau = i\beta/(2\pi\ell)$ by
$\tau \mapsto t/(2\pi\ell) + i\epsilon$, where $t$ is
Lorentzian time and $\epsilon \to 0^+$ selects the retarded
contour.
For an evaporating black hole, the conformal weight is
time-dependent: $h(t) = h_0 - \dot{h}\,t$ with
$\dot{h} > 0$, so that
$S_{\mathrm{BH}}(t) = 2\pi\sqrt{c\,h(t)/6}$ decreases
monotonically.
\end{construction}

\begin{theorem}[Lagrangian decomposition on the BTZ background]
\label{thm:lagrangian-btz}
The complementarity decomposition
(Theorem~\textup{\ref{thm:vir-koszul}}, Theorem~C) applied to
the genus expansion~\eqref{eq:btz-genus-sum} produces:
\begin{equation}\label{eq:lagrangian-btz}
\mathcal{F}(h;\,\hbar)
\;=\;
\mathcal{F}_+(\kappa;\,h;\,\hbar)
\;+\;
\mathcal{F}_-(\kappa';\,h;\,\hbar),
\end{equation}
...
\kappa\,\hbar^{-2}
\;+\;
\sum_{g \ge 1}\kappa\,\lambda_g^{\mathrm{FP}}\,
\hbar^{2g-2},
\qquad
\kappa = c/2,
\label{eq:F-plus}\\[4pt]
...
\begin{equation}\label{eq:Cg-decomp-btz}
\mathbf{C}_g(\mathrm{Vir}_c)\big|_{\alpha_{\mathrm{BTZ}}}
\;=\;
\mathbf{Q}_g^+(\kappa)
\;\oplus\;
\mathbf{Q}_g^-(\kappa'),
\end{equation}
with fiberwise differentials satisfying
$(d_{\mathrm{fib}}^+)^2 = \kappa\cdot\omega_g$ and
$(d_{\mathrm{fib}}^-)^2 = \kappa'\cdot\omega_g$ on the two
summands.
\end{theorem}

\begin{proof}
Theorem~C applied to the Virasoro algebra gives the
Lagrangian eigenspace decomposition of the genus-$g$ bar complex
under the Koszul involution $c \mapsto 26-c$. The involution has
period~$2$; its two eigenvalues on the fiberwise differential
$d_{\mathrm{fib}}^2 = \kappa\cdot\omega_g$ are $\kappa = c/2$
and $\kappa' = (26-c)/2$, the curvatures of the two eigenspaces.
Since $F_g = \kappa\cdot\lambda_g^{\mathrm{FP}}$
(UNIFORM-WEIGHT, Proposition~\ref{prop:uniform-weight}), each
summand contributes proportionally to its curvature eigenvalue.
The genus-summed partition function decomposes accordingly. The
evaluation on $\alpha_{\mathrm{BTZ}}$ preserves the
decomposition: the MC equation
$D\alpha + \frac{1}{2}[\alpha,\alpha] = 0$ is satisfied
independently on each Lagrangian half because the Koszul
involution commutes with the MC operator (it acts on the
parameter $c$, not on the MC variable $\alpha$).
\end{proof}

\subsection{The radiation entropy from saddle-point competition}

The two Lagrangian halves compete as the black hole evaporates.
The radiation entropy is determined by the saddle-point
prescription (the gravitational path integral selects the
geometry minimising the generalised entropy), and the Page
curve emerges from the exchange of dominance.

\begin{definition}[Evaporation rate from the genus expansion]
\label{def:evaporation-rate}
The entanglement production rate of a $2$d CFT at central charge
$c$ and Hawking temperature $T_H$ is determined by the genus-$1$
free energy. In thermal time units $\beta/(2\pi)$:
\begin{equation}\label{eq:evaporation-rate}
\dot{S}_{\mathrm{rad}}
\;=\;
\frac{4\pi\,F_1}{\beta/(2\pi)}
\;=\;
4\pi \cdot \frac{c}{48}
\;=\;
\frac{\pi c}{12}
\;=\;
\frac{c}{6}\;\text{per unit}\;\frac{\beta}{2\pi},
\end{equation}
using $F_1 = \kappa/24 = c/48$
(Theorem~\ref{thm:F1-derivation}).
% kappa-normalization: kappa(Vir) from C2; c=0 -> 0, c=13 -> 13/2 verified
The Koszul dual boundary, at central charge $26-c$, has
entanglement production rate $(26-c)/6$.
\end{definition}

\begin{theorem}[The algebraic Page curve]
\label{thm:page-curve}
\textbf{Type signature.} Quadrant: Open.
Presentation: scalar shadow on the algebraic sector (operator
$\to$ scalar trace direction).
Level: 5 (modular trace + saddle competition; scalar shadow
output, not operator-level reconstruction).
Hypothesis package: BTZ / Cardy (BC1)--(BC3); semiclassical
$S_{\mathrm{BH}} \gg 1$; Lagrangian decomposition under the
Koszul involution (Theorem~\ref{thm:lagrangian-btz}); island
mechanism upstream (Penington, AEMM) supplies the
information-theoretic content; the algebraic sector supplies the
saddle-exchange shadow only.
...
where:
\begin{itemize}
\item $S_+(t) = (c/6)\,t$ is the no-island contribution
  (entanglement growth at the boundary rate $\dot{S}_+ = c/6$,
  controlled by $\kappa = c/2$);
\item $S_-(t) = ((26-c)/6)\,t$ is the island contribution
  (entanglement growth at the dual rate $\dot{S}_- = (26-c)/6$,
  controlled by $\kappa' = (26-c)/2$);
\item $S_{\mathrm{BH}}(0)$ is the initial Bekenstein--Hawking
  entropy.
\end{itemize}
...
\item \textbf{Page time.}
  ...
  $S_+(t_P) = S_{\mathrm{BH}}(0) - S_-(t_P)$ gives:
  \begin{equation}\label{eq:page-time}
  t_P \;=\;
  \frac{3\,S_{\mathrm{BH}}(0)}{K}
  \;=\;
  \frac{3\,S_{\mathrm{BH}}(0)}{13},
  \end{equation}
  independent of~$c$.
\item \textbf{Page entropy.}
  $S_{\mathrm{Page}} = S_+(t_P)
  = \dfrac{c\,S_{\mathrm{BH}}(0)}{26}$.
\item \textbf{Hawking phase} ($t < t_P$):
  the boundary saddle $\barB(\mathrm{Vir}_c)$ with curvature
  $\kappa = c/2$ dominates; radiation
  entropy grows at rate $c/6$.
\item \textbf{Island phase} ($t > t_P$):
  the Koszul dual saddle $\barB(\mathrm{Vir}_{26-c})$ with
  $\kappa' = (26-c)/2$ dominates; radiation entropy
  decreases.
\item \textbf{Self-dual point} ($c = 13$):
  $\kappa = \kappa' = 13/2$, the two saddles have equal
  weight at all times,
...
\begin{proof}
The proof has three steps: the Lagrangian decomposition of the
replica partition function, the identification of the radiation
entropy from saddle dominance, and the computation of the
crossing time.

\medskip
\noindent\textit{Step~1: The replica partition function.}
The radiation entropy is computed from the replica partition
function $Z(n)$ on the $n$-sheeted BTZ geometry:
$S_{\mathrm{rad}}
= \lim_{n \to 1}(1-n)^{-1}\log(Z(n)/Z(1)^n)$.
The Lagrangian decomposition
(Theorem~\ref{thm:lagrangian-btz}) gives
$Z(n) = Z_+(n) + Z_-(n)$, corresponding to the boundary and
dual saddles. The $n$-sheeted replica of the BTZ geometry is
itself a BTZ geometry with conical excess at the replica branch
point; Theorem~\ref{thm:btz-mc} identifies it as an MC
element, so the genus expansion retains the same two-saddle
decomposition at every replica number.

\medskip
\noindent\textit{Step~2: The $\min$ formula from the generalised
entropy.}
In the semiclassical regime $S_{\mathrm{BH}} \gg 1$, the
gravitational path integral is dominated by the saddle with
smallest generalised entropy (the quantum extremal surface
prescription). Two saddles contribute:
\begin{itemize}
\item The \emph{no-island saddle}: the boundary-side partition
  function $Z_+$, contributing
  $S_{\mathrm{rad}} = S_+(t)$ to the radiation entropy.
  The rate $\dot{S}_+ = c/6$ is the genus-$1$
  entanglement rate
  (Definition~\ref{def:evaporation-rate}).
\item The \emph{island saddle}: the Koszul dual partition
  function $Z_-$, contributing
  $S_{\mathrm{rad}} = S_{\mathrm{BH}}(0) - S_-(t)$.
  The subtraction arises because the island saddle computes
  the complement: the radiation entropy equals the total entropy
  minus the interior entropy, and the interior is controlled
  by $\kappa' = (26-c)/2$.
\end{itemize}
The dominant saddle is whichever gives the
\emph{smaller} radiation entropy:
$S_{\mathrm{rad}} = \min(S_+,\,S_{\mathrm{BH}}(0) - S_-)$.

\medskip
\noindent\textit{Step~3: The crossing time.}
The crossing condition
$S_+(t_P) = S_{\mathrm{BH}}(0) - S_-(t_P)$ reads:
\[
\frac{c}{6}\,t_P
\;=\;
S_{\mathrm{BH}}(0) - \frac{26-c}{6}\,t_P.
\]
Adding the two rates:
$t_P\!\left(\frac{c}{6} + \frac{26-c}{6}\right)
= t_P \cdot \frac{26}{6} = S_{\mathrm{BH}}(0)$.
Solving:
\[
t_P
\;=\;
\frac{6\,S_{\mathrm{BH}}(0)}{26}
\;=\;
\frac{3\,S_{\mathrm{BH}}(0)}{13}
\;=\;
\frac{3\,S_{\mathrm{BH}}(0)}{K}.
\]
The $c$-independence is exact: the rates $c/6$ and $(26-c)/6$
are individually $c$-dependent, but their sum
$26/6 = \varkappa/3$ depends only on the scalar complementarity
invariant $\varkappa := \kappa + \kappa' = 13$ (distinct from the
Trinity ghost-charge conductor $K(\Vir) := c + c^! = 26$; related
by $\varkappa = \varrho_{\Vir}\cdot K$, $\varrho_{\Vir} = 1/2$).
\end{proof}

\begin{remark}[The entanglement rate is not an external input]
\label{rem:rate-from-genus}
The Calabrese--Cardy entanglement rate $c/6$ is not imported
from an independent argument. It is \emph{derived} from the
bar complex: the genus-$1$ free energy
$F_1 = \kappa/24 = c/48$
(Theorem~\ref{thm:F1-derivation})
determines the Hawking radiation rate via
$\dot{S}_{\mathrm{rad}} = 4\pi F_1/(\beta/(2\pi)) = c/6$
(Definition~\ref{def:evaporation-rate}).
The rate for the Koszul dual is $(26-c)/6$, and their sum
$26/6 = K/3$ is the quantity controlling the Page time.
The bar complex provides both the two saddles and their
relative growth rates; the entire Page curve is internal
to the genus expansion.
\end{remark}

\begin{remark}[$c$-independence of the Page time]
\label{rem:c-independence}
The coefficient $3/13$ in
$t_P = 3S_{\mathrm{BH}}/13$ is purely algebraic:
$3$ is the entanglement normalisation ($c/6$ rather than
$c/2$; equivalently, $\kappa/3$ rather than $\kappa$), and $13$
is the Virasoro complementarity constant $K = 13$.
The dependence on the specific black hole enters only through
$S_{\mathrm{BH}}$.
For any Koszul pair with complementarity constant $K$, the
Page time is $t_P = 3S_{\mathrm{BH}}/K$.
\end{remark}

\subsection{The island as Koszul dual dominance}

The island of the quantum extremal surface prescription has a
precise algebraic incarnation: it is the locus in the genus
expansion where the Koszul dual saddle dominates. The
complementarity theorem fixes the number of saddles (exactly two),
their curvatures ($\kappa$ and $\kappa'$), and the transition
time ($t_P$).
...
curvature of the second is $\kappa' = (26-c)/2$.
There is no third saddle: the Koszul involution has period~$2$,
so the pair $(\mathrm{Vir}_c, \mathrm{Vir}_{26-c})$ exhausts
the decomposition.

The transition is sharp: at $t = t_P$, the dominance switches
at every genus simultaneously because the ratio
$\kappa/\kappa' = c/(26-c)$ is genus-independent (the
$\lambda_g^{\mathrm{FP}}$ factors cancel in the ratio of
Boltzmann weights).
\end{theorem}

\begin{proof}
The Lagrangian eigenspace decomposition is
Theorem~C applied to the Virasoro algebra
(Theorem~\ref{thm:lagrangian-btz}). The curvatures
$\kappa$ and $\kappa'$ are the eigenvalues of
$d_{\mathrm{fib}}^2/\omega_g$ on the two summands. The
ratio of Boltzmann weights at genus $g$ is
\[
\frac{Z_+^{(g)}}{Z_-^{(g)}}
\;=\;
\exp\!\bigl((\kappa - \kappa')\,
\lambda_g^{\mathrm{FP}}\,\hbar^{2g-2}\bigr)
\;=\;
\exp\!\left((c-13)\,
\lambda_g^{\mathrm{FP}}\,\hbar^{2g-2}\right)\!,
\]
which depends on $g$ through the common factor
$\lambda_g^{\mathrm{FP}}\,\hbar^{2g-2}$ but whose sign
($c > 13$ vs $c < 13$) is genus-independent.
At $c = 13$: the ratio is $1$ at all genera and all times.
\end{proof}

\begin{remark}[Comparison with Penington and AEMM]
\label{rem:island}
The gravitational island of
Penington~\cite{Pen19} and
Almheiri--Engelhardt--Marolf--Maxfield~\cite{AEMM19,AMMZ19}
is the locus where the quantum extremal surface dominates the
gravitational path integral. The Koszul dual saddle
$\barB(\mathrm{Vir}_{26-c})$ is the algebraic counterpart:
the two-saddle structure, the exchange of dominance, and the
$c$-independent Page time all emerge from the complementarity
constant $K = 13$ alone.
The identification:
\begin{center}
\small
\renewcommand{\arraystretch}{1.2}
\begin{tabular}{@{}l@{\;\;}l@{}}
\textbf{QES / replica wormhole} & \textbf{Bar complex} \\
\hline
no-island saddle & $\mathbf{Q}_g(\mathrm{Vir}_c)$ \\
island saddle & $\mathbf{Q}_g(\mathrm{Vir}_{26-c})$ \\
quantum extremal surface & Koszul involution fixed locus \\
island region & $\kappa'$-eigenspace dominance \\
Page time & saddle crossing \\
two saddles, no more & period-$2$ involution \\
\end{tabular}
\end{center}
The number of saddles (exactly two) is determined by the Koszul
involution, not by a choice of replica geometry. The bar complex
provides the mechanism; the Penington--AEMM programme provides
the physical context and the fine-grained interpretation.
\end{remark}

\subsection{The transgression algebra and the Page transition}

The transgression algebra $B_\Theta$ provides a second,
independent characterisation of the Page transition. The
degeneration of $B_\Theta$ at $u = \eta^2 = 0$ is the algebraic
mechanism underlying the saddle exchange: the Morita-invisible
Hawking phase degenerates into the information-carrying island
phase.

\begin{proposition}[The transgression algebra at the BTZ saddle]
\label{prop:transgression-btz}
Let $B_\Theta$ be the transgression algebra of the bar complex
$\barB(\mathrm{Vir}_c)$ with MC element
$\Theta = \Theta_{\mathrm{Vir}_c}$
(Section~\textup{\ref{sec:shadow-as-gravity}}). Set
$u := \eta^2$ where $\eta$ is the degree-one generator with
$d\eta = \Theta$. At the BTZ saddle:
\begin{enumerate}[label=\textup{(\roman*)}]
\item For $u \ne 0$ (the generic locus): the genus-$g$
  Clifford completion satisfies
  $\mathfrak{G}_g(B_\Theta)[u^{-1}]
  \cong \mathrm{Mat}_{2^g}(B_\Theta[u^{-1}])$.
  Higher genus is Morita-invisible: the matrix algebra carries
  no new invariants beyond the genus-$0$ data.
  This is the \textbf{Hawking phase}: the boundary saddle
  dominates, and the black hole appears thermal.
\item For $u = 0$ (the degenerate locus):
  $\mathfrak{G}_g(B_\Theta)/(\eta^2)
  \cong B/(\Theta) \otimes \Lambda^{2g+1}$,
  the exterior algebra degeneration. The
  $\binom{2g+1}{k}$ generators in $\Lambda^k$ resolve the
  Morita-invisible matrix algebra into topological sectors of the
  genus-$g$ partition function.
  This is the \textbf{island phase}: the Koszul dual saddle
  dominates, and the radiation carries fine-grained information.
\item The \textbf{Page transition} is the passage from $u \ne 0$
  to $u = 0$. The secondary anomaly
  $u = \eta^2$ vanishes when the two Lagrangian saddles reach
  equal weight. The locus $u = 0$ is the
  codimension-one wall in the space of MC deformations where
  the matrix algebra degenerates to the exterior algebra.
\end{enumerate}
\end{proposition}

\begin{proof}
Parts~(i) and~(ii) are the two regimes of the Clifford
completion. The Clifford algebra
$\mathrm{Cl}_{2g+1}(u)$ is the matrix algebra
$\mathrm{Mat}_{2^g}$ for $u \ne 0$ (non-degenerate quadratic
form) and the exterior algebra $\Lambda^{2g+1}$ for $u = 0$
(zero form).

Part~(iii): the BTZ MC element $\alpha_{\mathrm{BTZ}} = h\,e_h$
determines the curvature $\Theta|_{\alpha} = \kappa\cdot\omega_1$
at genus~$1$. As the black hole evaporates,
$h(t) \to 0$ and $\alpha_{\mathrm{BTZ}}(t) \to 0$;
correspondingly, $u(t) = \eta^2|_{\alpha(t)} \to 0$. The Page
time $t_P$ is the time at which $u(t_P) = 0$ to leading
semiclassical order, which coincides with the saddle
crossing~\eqref{eq:page-time} because $u = 0$ is the locus
where neither Lagrangian half dominates.
\end{proof}

\begin{remark}[Morita invisibility is thermality]
\label{rem:morita-unitarity}
The Morita equivalence
$\mathfrak{G}_g \cong \mathrm{Mat}_{2^g}(B_\Theta[\eta^{-2}])$
for $u \ne 0$ is the algebraic expression of Hawking's
argument: a thermal state carries no fine-grained information
regardless of the number of replicas ($g$). The breakdown at
$u = 0$ is the algebraic expression of unitarity restoration:
the exterior algebra $\Lambda^{2g+1}$ resolves the thermal
degeneracy. The Page curve interpolates between these two
regimes; the passage is controlled by the scalar complementarity
$\varkappa := \kappa + \kappa' = 13$ (the Trinity
$K(\Vir) = 26$; $\varkappa = K/2$).
\end{remark}

\subsection{The Borel singularity and Stokes phenomenon}

Conditionally on the genus-expansion model, the shadow free energies $F_g = \kappa \cdot
...
Remark~\ref{rem:convergence}). The Page transition has a
third incarnation in this divergent structure: the Stokes
phenomenon of the Borel-summed genus expansion.

\begin{theorem}[Borel singularity from the Koszul dual]
\label{thm:page-borel}
...
satisfies:
\begin{enumerate}[label=\textup{(\roman*)}]
\item The series diverges factorially:
  $|F_g^{\mathrm{full}}| \sim C^g \cdot (2g)!$,
  forced by class~$\mathbf{M}$ (all $m_k \ne 0$).
\item The Borel transform
  $\hat{Z}(\zeta) = \sum_{g \ge 0}
  F_g^{\mathrm{full}}\,\zeta^{2g-2}/(2g{-}2)!$ has a
  singularity at the instanton action
  \begin{equation}\label{eq:instanton-action}
  A_{\mathrm{inst}}
  \;=\;
  \frac{|\kappa' - \kappa|}{K}
  \cdot(2\pi)^2
  \;=\;
  \frac{|13 - c|}{13}\cdot 4\pi^2.
  \end{equation}
  The singularity corresponds to the Koszul dual saddle
  $\barB(\mathrm{Vir}_{26-c})$.
\item At $c = 13$ (self-dual): $A_{\mathrm{inst}} = 0$.
  The Borel singularity sits at the origin, the two saddles
  have equal action, and the Stokes phenomenon is maximally
  strong.
\item The Stokes wall of the Borel summation is the Page time:
  the exponentially suppressed correction
  $\sim e^{-A_{\mathrm{inst}}/\hbar}$
  becomes of order unity at $t = t_P$ and
  dominates thereafter.
\end{enumerate}
\end{theorem}

\begin{proof}
Part~(i): the shadow depth $r_{\max} = \infty$ (all $m_k \ne 0$,
Theorem~\ref{thm:non-truncation}) forces the field-sector
contributions to grow as $\|m_k|_T\| \sim C^k\cdot k!$ by
the Virasoro OPE recursion. The cumulative genus-$g$ free
energy includes the sum over stable graph topologies at genus
$g$ with vertices decorated by $\{m_k\}$; the factorial growth
of $m_k$ combined with the graph-counting factor produces
$|F_g^{\mathrm{full}}| \sim C^g\cdot(2g)!$ (Gevrey-$1$).
The scalar shadow $\kappa\cdot\lambda_g^{\mathrm{FP}}
\sim \kappa\cdot(2\pi)^{-2g}$ converges (Bernoulli decay)
but the field sector dominates for $g \gg 1$.

Part~(ii): in the resurgence framework, the Borel singularity
nearest to the origin corresponds to the non-perturbative saddle
closest in action. Complementarity provides exactly one such
saddle: $\barB(\mathrm{Vir}_{26-c})$ with curvature $\kappa'$.
The tree-level free energies are
$\mathcal{F}_+^{(0)} = \kappa\,\hbar^{-2}$ and
$\mathcal{F}_-^{(0)} = \kappa'\,\hbar^{-2}$; the action
difference is
$|\mathcal{F}_+^{(0)} - \mathcal{F}_-^{(0)}|
= |\kappa - \kappa'|\,\hbar^{-2}
= |c - 13|\,\hbar^{-2}/1$.
Normalising by the scalar instanton action $(2\pi)^2$ (the
leading pole of the $\hat{A}$-genus at $x = 2\pi i$) gives
$A_{\mathrm{inst}} = (2\pi)^2|\kappa'-\kappa|/K$.

Part~(iii): at $c = 13$, $\kappa = \kappa'$, so
$A_{\mathrm{inst}} = 0$.

Part~(iv): the Stokes wall is crossed when the Borel contour
passes through the singularity. On one side:
$\barB(\mathrm{Vir}_c)$ dominates. On the other:
$\barB(\mathrm{Vir}_{26-c})$ dominates. The Stokes multiplier
is $\pm 1$ (the involution has period~$2$). The wall is
crossed when the evaporating black hole reaches the critical
value $\hbar^{-1} = S_{\mathrm{BH}}(t_P)$; at leading order
this reproduces $t_P = 3S_0/13$.
\end{proof}

\begin{remark}[Three incarnations of the Page transition]
\label{rem:three-threads}
The Page transition has three equivalent descriptions, each
originating from a different structure on the bar complex:
\begin{enumerate}[label=\textup{(\arabic*)}]
\item \textbf{Lagrangian:} the crossing of the two Koszul-dual
  saddle-point contributions to the gravitational partition
  function (Theorem~\ref{thm:page-curve}).
\item \textbf{Transgression:} the degeneration
  $u = \eta^2 \to 0$ in the transgression algebra $B_\Theta$,
  where the Morita-invisible matrix algebra degenerates to the
  exterior algebra
  (Proposition~\ref{prop:transgression-btz}).
\item \textbf{Resurgent:} the Stokes wall of the Borel-summed
  genus expansion, where the exponentially suppressed Koszul dual
  contribution becomes dominant
  (Theorem~\ref{thm:page-borel}).
\end{enumerate}
All three are controlled by the single datum
$\varkappa := \kappa + \kappa' = 13$ (the scalar complementarity;
the Trinity $K(\Vir) = c + c^! = 26$ sits upstream through
$\varkappa = \varrho_{\Vir}\cdot K = K/2$): it
determines the crossing time, the degeneration locus, and the
instanton action. The number of saddles is fixed by
complementarity (period-$2$ involution), the degeneration type
by the Clifford structure (matrix vs.\ exterior), and the Stokes
multiplier is $\pm 1$ (the involution sign).
\end{remark}
...
The algebraic Page curve exploits the Koszul pair $(\mathrm{Vir}_c,
...
\item The central charge continues to\/
  ...
  % kappa-normalization: kappa(Vir) from C2
...
\item \textbf{Self-duality.}
  ...
  % kappa-normalization: kappa(Vir) from C2; c=13 -> 13/2 verified
...
\item \textbf{$R$-matrix at self-duality.}
  ...
  % level-shift: level prefix 13/2 present; at c=0, scalar part vanishes
...
\item $c = 13$: \emph{Koszul self-duality}.
  ...
  complementarity constant is $K = 13$. The algebraic Page curve
  is symmetric. The gravitational Yangian has full self-dual structure.
...
the algebraic Page curve has maximal symmetry (equal entropies),
and the Yangian has maximal structure (self-dual braiding).
At $c = 26$,
...
$\mathcal{H}(\mathrm{Vir}_c)$: at $c = 13$, all seven entries
are Koszul self-dual (invariant under $c \mapsto 26 - c$),
the algebraic Page curve is symmetric, the genus tower has
maximal symmetry, and the Yangian structure organises the
infinite
$A_\infty$ tower into a representation-theoretic framework.
The holographic datum at generic $c$ carries the same seven
entries but without the Koszul self-duality (specific to
$c = 13$) that enables the Yangian interpretation.
\end{remark}

% ================================================================
% 3d gravity, the E_infty-topologization ladder, and celestial holography
% ================================================================

\section{Gravity, topologization, and celestial holography}
\label{sec:3dqg-climax}

The algebraic-sector identification of 3d gravity is not a
motivational appendix to the modular Koszul programme. The
algebraic sector is the first nontrivial gravitational realisation
of the homotopy-completed iterated Sugawara topologisation ladder
(conditional, scope-tagged), the chiral higher-Deligne
identification of the bulk, the algebraic shadow of celestial
holography, and the topologization tower across the four shadow
classes. None of these constructions builds the compact
dynamical-metric path integral; that lift is the F12 frontier
(Section~\ref{sec:f12-frontier}).

\begin{theorem}[Conditional iterated Sugawara topologisation ladder]
\label{thm:einfty-ladder}
For a chiral algebra $\cA$ with a tower of inner higher-spin
Casimirs $\{T^{(n)}\}_{2 \le n \le N+1}$, each admitting a BRST
primitive $G^{(n)}$ with $T^{(n)} = [Q_{\mathrm{tot}}, G^{(n)}]$ on
$Q$-cohomology, assume in addition that the primitives are
BRST-commuting up to coherent homotopy,
$[G^{(n)},G^{(m)}]=Q$-exact, and determine independent topological
directions. Then the transferred derived chiral centre carries an
$E_{N+2}$-topological structure via iterated Dunn additivity.
Specialisations:
\begin{itemize}
\item $\cA = \Vir_c$: one inner stress tensor, cohomological and
  weight-completed $E_3^{\mathrm{top}}$;
\item $\cA = \cW_N$: inner tower of depth $N-1$,
  $E_{N+1}^{\mathrm{top}}$ under the same coherent-homotopy package;
\item $\cA = \cW_\infty$: infinite depth,
  $E_\infty^{\mathrm{top}}$ only under convergence and completion
  hypotheses.
\end{itemize}
Consequently, 3d quantum gravity is modeled by the $N = 2$ shadow of
this conditional topological tower.
\end{theorem}

\begin{theorem}[Chiral higher Deligne and holographic shadow]
\label{thm:chiral-higher-deligne-holography}
The derived chiral centre $Z^{\mathrm{der}}_{\mathrm{ch}}(\Vir_c)$
is acted on, in the cohomological and weight-completed sense, by
$\mathsf{SC}^{\mathrm{ch,top}}$ through the heptagon edges
$(3) \leftrightarrow (4) \leftrightarrow (5)$ of the Vol~II
$\mathsf{SC}^{\mathrm{ch,top}}$ heptagon. On the C$_2$-cofinite
non-logarithmic standard landscape, this gives the algebraic shadow
of celestial holography: the celestial chiral algebra
$\cA^{\mathrm{cel}}$ of boundary soft modes, the SC$^{\mathrm{ch,top}}$
structure on $(\cA^{\mathrm{cel}}, Z^{\mathrm{der}}_{\mathrm{ch}}(\cA^{\mathrm{cel}}))$,
and the shadow-tower coefficients reproducing the soft-factor
hierarchy. Coverage: self-dual gauge (KM), gauge+matter (DS),
gravity (Virasoro + $w_{1+\infty}$).
\end{theorem}

\begin{remark}[Topologization tower: scope per class]
\label{rem:topologization-tower}
Topologization of $Z^{\mathrm{der}}_{\mathrm{ch}}(\cA)$ has four
regimes, with scope tags attached for every reference in this
paper:
\begin{itemize}
\item Class~G (Heisenberg): $E_3^{\mathrm{top}}$ immediate via
  Dunn on $F^{H_k}$ (commutative $\Rightarrow$ Dunn applies).
\item Class~L ($V_k(\fg)$, $k \ne -h^\vee$):
  $E_3^{\mathrm{top}}$ on the \emph{original} complex via
  Costello--Gwilliam factorization on $\bR \times \bC$ with
  explicit topologizing primitives
  $\eta_1^{(i)},\eta_1^{(ii)}$ making
  $[Q, \widetilde G_1] = T_{\mathrm{Sug}}$ strict chain-level.
\item Class~C ($\beta\gamma$, $bc$): $E_3^{\mathrm{top}}$ via
  Friedan--Martinec--Shenker bosonisation, reducing to Heisenberg.
\item Class~M ($\Vir$, $\cW_N$): $E_3^{\mathrm{top}}$
  weight-completed via half-BRST + MC5 pattern; original-complex
  chain-level \emph{open} (sole remaining direction of the
  topologization tower theorem).
\item Critical level $k = -h^\vee$: $E_2^{\mathrm{top}}$ (dimension
  drop, not failure).
\end{itemize}
Theorem~\ref{thm:e3-topologisation} is the
class~M line of this tower, attached with the cohomological /
weight-completed hypothesis package.
\end{remark}

\begin{remark}[Class~M scope and logarithmic exceptions]
\label{rem:class-m-scope}
Class~M chain-level topologization on the \emph{original}
complex remains open; the weight-completed statement is proved.
The scope qualifications are: (i)~W$(p)$ triplet tempering is conjectural
pending an Adamovi\'c--Milas character-amplitude bound
(Gurarie 1993 + Flohr 1996 counterexamples); (ii)~non-tempered
stratum is \emph{empty} only on the non-logarithmic subset;
(iii)~the conditional harmonic $\beta_N$ law separates the two
polynomial candidates at $N = 4$ ($13$ versus $15$ and $16$),
a parameter-question not structural; (iv)~super-Yangian canonical
pairing is sub-Sugawara. 3d quantum gravity with $\Lambda < 0$,
the Virasoro climax at $c = 3\ell/(2G)$, and the
$c$-linearity/self-duality structure are unconditional on the
Virasoro Koszul locus (generic~$c$); the class~M chain-level
conditional affects only original-complex chain-level bar/BV
comparison, not the genus expansion computed in
Section~\ref{sec:shadow-as-gravity}.
\end{remark}

\begin{remark}[Algebraic identification, not gravitational construction]
\label{rem:climax-identity}
The relationship between the Virasoro Beilinson tower and 3d
gravity is an \emph{identification of the algebraic sector}, not a
construction of a compact dynamical-metric path integral. The
universal Maurer--Cartan element $\Theta_{\Vir_c}$ of the modular
convolution algebra encodes, via its MC projections to successive
degrees, the complete \emph{algebraic-sector trace} of the
perturbative gravitational genus expansion. Koszul duality
$\Vir_c \leftrightarrow \Vir_{26-c}$ is the algebraic shadow of
S-duality; Page-time complementarity $t_P = 3 S_{\mathrm{BH}}/13$ is
the structural shadow of the Page transition. The full
information-theoretic Page curve (Penington, AEMM) requires the
island mechanism on the dynamical-metric path integral; the
complementarity decomposition computes the saddle-exchange shadow
on the algebraic sector. BTZ black holes are MC elements of the
algebraic convolution algebra; de~Sitter entropy emerges from
analytic continuation $\kappa \to -|\kappa|$.

The trace direction is unconditional on the Virasoro Koszul locus.
The inverse direction (algebraic sector $\to$ compact
dynamical-metric path integral) is open frontier F12: it requires
the BTZ / Cardy hypothesis package (modular invariance + vacuum
dominance + saddle stability) plus the Maloney--Witten +
Manschot--Moore Farey-tail completion, and it remains the named
operator-level Pfaffian / square-root reconstruction problem (Vol~I
F11 + F12). The iterated Sugawara ladder
(Theorem~\ref{thm:einfty-ladder}) embeds the present algebraic
sector as the $N = 2$ truncation of a conditional $E_\infty$
topologisation tower; the unconditional content is the $N = 2$
algebraic identification proved here.
\end{remark}


% ================================================================
\section{The F12 frontier: from algebraic sector to compact dynamical-metric path integral}
\label{sec:f12-frontier}

The preceding sections constructed the algebraic holographic
HT~sector of 3d gravity at $\Lambda < 0$: the open Beilinson tower
with boundary $\Vir_c$, bulk $Z^{\mathrm{der}}_{\mathrm{ch}}(\Vir_c)
=C^\bullet_{\mathrm{ch}}(\Vir_c,\Vir_c)$, and the
$\mathsf{SC}^{\mathrm{ch,top}}$-brace structure on the pair. The
trace direction (operators $\to$ scalar shadow) is unconditional on
the Virasoro Koszul locus: the genus tower
$F_g = \kappa\,\lambda_g^{\mathrm{FP}}$, the algebraic Page shadow
$t_P = 3 S_{\mathrm{BH}}/13$, the Cardy / Bekenstein--Hawking entropy,
the de~Sitter analytic continuation, and the gravitational Yangian
at $c = 13$ are all level-5 scalar shadows of the level-4 operator
algebra, computed via modular trace and clutching on the open
category.

The \emph{inverse direction}, namely the reconstruction of a
compact dynamical-metric path integral from the algebraic sector,
is the open frontier F12. Stating this precisely:

\begin{conjecture}[F12: algebraic sector $\to$ dynamical-metric path integral]
\label{conj:f12-frontier}
\textbf{Type signature.} Quadrant: Open.
Presentation: operator-level Pfaffian / square-root reconstruction.
Level: 5 $\to$ 4 (inverse modular reconstruction; scalar-shadow to
operator-algebra comparison).
Hypothesis package: BTZ / Cardy (BC1)--(BC4) of
Section~\ref{sec:btz}; Maloney--Witten partition function
$Z_{\mathrm{MW}}(\tau)$ defined via the
$\mathrm{SL}_2(\mathbb{Z})/\mathbb{Z}$ Farey sum over BTZ images;
Manschot--Moore mock-modular Farey-tail completion; Hall--Borcherds
residual + virtual $K_0$-determinant package (Vol I F11).

There exists a compact holomorphic-topological saddle structure on
the gravitational path integral
\begin{equation}\label{eq:f12-target}
Z_{3\mathrm{d\,QG, Vir}}(S^2 \times S^1_\tau)
\;=\; q^{(1{-}c)/24}\,(1-q)/\eta(\tau),
\end{equation}
extending the algebraic-sector partition function of the present
construction, such that:
\begin{enumerate}[label=\textup{(\roman*)}]
\item $Z_{3\mathrm{d\,QG, Vir}}$ equals the Maloney--Witten
  Farey-tail-completed sum over BTZ saddles plus thermal
  $\mathrm{AdS}_3$;
\item the inverse trace map sends the level-5 scalar tower
  $\{S_r\}_{r \ge 2}$ to the level-4
  operator content of the BTZ ensemble;
\item the algebraic Page shadow $t_P = 3 S_{\mathrm{BH}}/13$ lifts
  to the full information-theoretic Page curve of
  Penington--AEMM via the island mechanism applied to the
  dynamical-metric path integral.
\end{enumerate}
\end{conjecture}

\begin{remark}[F12 comparison problem]
\label{rem:f12-heal-path}
F12 is the conjunction of the following three constructions:
\begin{enumerate}[label=\textup{(\arabic*)}]
\item \emph{Operator-level Pfaffian} for the Virasoro family,
  analogous to the Igusa cusp form $\Delta_5$ on K3 $\times$ E
  (Vol I F11). The construction would identify the
  Manschot--Moore Farey-tail completion as a virtual
  $K_0$-determinant of an explicit operator-level object whose
  protected Pfaffian is the Maloney--Witten partition function.
\item \emph{Cross-volume bridge to Vol II / III.} Vol II's
  $\mathsf{SC}^{\mathrm{ch,top}}$-brace bulk is the operator-level
  companion of the algebraic sector identified here; the
  dynamical-metric path integral would emerge as the
  gravity-line completion of the Vol II climax. Vol III's
  Drinfeld double $G(X) = D(Y^+(X))$ supplies the line / brane
  operator data.
\item \emph{Resolution of the Maloney--Witten negativity problem.}
  The naive Maloney--Witten sum has negative Fourier coefficients
  in the partition function; Manschot--Moore Farey-tail completion
  is the candidate cure, but its compatibility with the
  Beilinson-tower reconstruction at level 4 is unverified. The
  resolution is the F12 endpoint.
\end{enumerate}
Until F12 is closed, every statement of the form ``the algebraic
sector \emph{is} the gravitational path integral'' or ``the
modular form $Z_{3\mathrm{d\,QG}}$ \emph{constructs} the BPS
Hilbert space'' has the wrong type: scalar shadow $\ne$ operator
algebra. The trace direction is unconditional; the inverse direction
remains the open F12 comparison problem with its own hypothesis
package.
\end{remark}

\begin{remark}[The cross-volume vertical]
\label{rem:cross-volume-vertical}
The algebraic holographic HT sector identified here sits at
levels 3--5 of the Vol~I Beilinson tower:
\begin{itemize}
\item Level 1 (chart): $A_b = \Vir_c$.
\item Level 2 (twisting): $\barB^{\mathrm{ord}}(\Vir_c)$.
\item Level 3 (bulk):
  $Z^{\mathrm{der}}_{\mathrm{ch}}(\Vir_c)
  =C^\bullet_{\mathrm{ch}}(\Vir_c,\Vir_c)$, with
  $E_3^{\mathrm{top}}$ structure
  (cohomological + weight-completed scope; Class~$\mathbf M$).
\item Level 4 (operator): BTZ MC elements, Wilson lines,
  $\Vir_{26-c}$-modules.
\item Level 5 (scalar shadow): genus tower
  $F_g = \kappa\,\lambda_g^{\mathrm{FP}}$, Cardy entropy,
  algebraic Page shadow.
\end{itemize}
Vertical equivalences: level 3 of Vol~I corresponds to the
Vol~II $\mathsf{SC}^{\mathrm{ch,top}}$-brace bulk (climax of
Vol~II); level 5 corresponds to the gravity-line completion
(Vol~II) and to the Borcherds-weight identity
$\kappa_{\mathrm{BKM}}(\Phi_N) = c_N(0)/2$ (Vol~III). F12 is
precisely the vertical equivalence at level 4 in the gravitational
direction: identifying the BTZ ensemble of the algebraic sector
with the saddle ensemble of the compact dynamical-metric path
integral.
\end{remark}
...
\textit{Computation of cohomology of vertex algebras},
...
\bibitem{Pen19}
G.~Penington,
\textit{Entanglement wedge reconstruction and the information problem},
J.\ High Energy Phys.\ \textbf{2020} (2020), no.~9, 002;
arXiv:1905.08255.

\bibitem{AEMM19}
A.~Almheiri, N.~Engelhardt, D.~Marolf, and H.~Maxfield,
\textit{The entropy of bulk quantum fields and the entanglement
wedge of an evaporating black hole},
J.\ High Energy Phys.\ \textbf{2019} (2019), no.~12, 063;
arXiv:1905.08762.

\bibitem{AMMZ19}
A.~Almheiri, R.~Mahajan, J.~Maldacena, and Y.~Zhao,
\textit{The Page curve of Hawking radiation from semiclassical geometry},
J.\ High Energy Phys.\ \textbf{2020} (2020), no.~3, 149;
arXiv:1908.10996.

\bibitem{MW07}
A.~Maloney and E.~Witten,
\textit{Quantum gravity partition functions in three dimensions},
J.\ High Energy Phys.\ \textbf{2010} (2010), no.~2, 029;
arXiv:0712.0155.

\bibitem{MM08}
J.~Manschot and G.~W.~Moore,
\textit{A modern Farey tail},
Commun.\ Number Theory Phys.\ \textbf{4} (2010), no.~1, 103--159;
arXiv:0712.0573.
