...
$Z^{\mathrm{der}}_{\mathrm{ch}}(\Vir_c) \simeq \mathbb{C}[\![c]\!]$,
...
Presentation: weight-completed / pro-object ambient (Class~$\mathbf M$
chain-level on the original direct-sum complex remains conditional;
see Frontier~F3).
Level: 3 (derived chiral centre as universal closed sector).
Hypothesis package: generic $c$ (Koszul locus); finite-dimensional
graded augmented Virasoro vacuum module; Hochschild--Serre
spectral-sequence convergence in the completed ambient.
...
\item $\mathrm{RHom}(\Omega(\bar{B}), \mathrm{Vir}_c)
  ...
  the universal closed sector; a physical bulk requires the open--closed comparison).
\end{enumerate}
...
(iii)~the $\beta_N$ closed form differs at $N = 4$ (15 vs 16),
parameter-question not structural; (iv)~super-Yangian canonical
...
\item Level 3 (bulk): $Z^{\mathrm{der}}_{\mathrm{ch}}(\Vir_c) \simeq
  \mathbb{C}[\![c]\!]$, with $E_3^{\mathrm{top}}$ structure
