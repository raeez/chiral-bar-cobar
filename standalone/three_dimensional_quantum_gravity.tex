...
+ \kappa(\mathrm{Vir}_{26-c}) = 13$ produces an algebraic
shadow of the Page curve: the two Koszul-dual saddles
exchange dominance at a crossing time determined by $K$
alone. This is a \emph{structural shadow} of the Page
transition, not a derivation from first principles.
The full information-theoretic Page curve requires the
gravitational island mechanism
(Penington~\cite{Pen19}, Almheiri--Engelhardt--Marolf--Maxfield~\cite{AEMM19},
Almheiri--Mahajan--Maldacena--Zhao~\cite{AMMZ19}),
which computes the
entanglement entropy via quantum extremal surfaces. The
bar-complex computation recovers the Page \emph{time}
$t_P = 3S_{\mathrm{BH}}/13$ and the crossing structure,
but not the full information-theoretic content (the
entropy of Hawking radiation as a function of the
quantum state). The algebraic avatar captures the
\emph{kinematic} structure forced by Koszul complementarity;
the island mechanism provides the \emph{dynamical} content.

\subsection{The radiation entropy}

\begin{theorem}[Algebraic Page curve from complementarity]
...
$\kappa' = (26-c)/2$.
The crossing of the two Koszul-dual saddles produces an algebraic
avatar of the Page curve:
...
\begin{remark}[Relation to the island mechanism]
\label{rem:island}
The gravitational island programme of
Penington~\cite{Pen19} and
Almheiri--Engelhardt--Marolf--Maxfield~\cite{AEMM19,AMMZ19}
derives the Page curve from quantum extremal surfaces in the
gravitational path integral.
The Koszul-dual saddle
$\bar{B}(\mathrm{Vir}_{26-c})$ is an algebraic avatar of
the island saddle: the region of the genus expansion where the
dual bar complex provides the larger contribution to
$\mathbf{C}_g$ is the algebraic counterpart of the island
region where the quantum extremal surface dominates.
The bar-complex computation recovers the crossing time
$t_P = 3S_{\mathrm{BH}}/13$ and the two-saddle structure
from the complementarity constant $K = 13$ alone; it does
not reproduce the full information-theoretic content of the
island mechanism (the fine-grained entropy of Hawking
radiation as a function of the quantum state).
The number of saddles (exactly two) is determined by the
Koszul involution (period~$2$), not by a choice of replica
geometry.
\end{remark}
