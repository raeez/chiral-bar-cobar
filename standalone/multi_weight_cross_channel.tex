...
G & dumbbell & $3$ & $8$ & $8$ & \textbf{mixed $\neq 0$} \\
\bottomrule
\end{tabular}
\end{center}

Graphs A, B, C contribute zero cross-channel correction:
\begin{itemize}
...
Graphs D, E, F, G carry the entire cross-channel correction.
...
\emph{Dumbbell \textup{(}graph G\textup{)}.}
Two self-loops are connected by a bridge. The mixed barbell
...
This proposition fixes the multi-weight scope of Theorem~C(2) in
Vol~I: the correction is explicit at
...
D (sunset), E (bridge-loop), F (theta), G (dumbbell) from the catalogue
