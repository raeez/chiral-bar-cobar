...
(UNIFORM-WEIGHT).
For the $\Walg_3$ algebra the scalar formula fails at genus
\(g=2\), and the failure is computed explicitly.
...
A closed-form gravitational lower-bound formula for
\(\delta F_2(\Walg_N)\) is computed for \(N\geq4\); it is exact for
\(\Walg_3\) and vanishes at the Virasoro point \(N=2\).
The scalar and cross-channel sectors have distinct large-order
signals in the computed range: the scalar ordinary genus series
has nearest poles at \(u=(2\pi)^2\) in \(u=\hbar^2\), while the
cross-channel coefficients acquire increasing central-charge degree.
The computed cross-channel ratios quantify the loss of information
under averaging and give a numerical expression
...
Modular Koszul duality attaches to each chiral algebra
...
The scalar formula encodes a strong universality: the genus-$g$
...
The cross-channel correction is defined by the mixed-channel graph
sum. Its basic consequences are:
...
measures the information in the ordered bar complex
 $B^{\ord}(\cA)$ that is lost under the coinvariant
...
% 2. A^i = C_X(s^{-1}V,s^{-2}R), with q_A:A^i -> B_X(A)
% 3. H^*(B_X(A)) = bar cohomology; H(q_A) is the Koszul comparison
% 4. K_X(A) = D_Ran B_X(A), with nu_A:K_X(A) -> A^!
% 4. Z^der_ch(A) = derived chiral center = Hochschild cochains = closed sector
...
The ordered bar $B^{\ord}(\cA)$ is the level-$2$ $E_1$-chiral
coalgebra before averaging; the symmetric bar $B^{\Sigma}(\cA)$ is its
$\Sigma_n$-coinvariant shadow.
The degree-$2$ collision kernel is $K_\cA^{\mathrm{coll}}$.
Its homotopy coinvariant remains operator-valued and
$\kappa(\cA)=\tau_{\cA,2}^{\mathrm{res}}
q_2(K_\cA^{\mathrm{coll}})$ on the scalar families. A classical
$r$-matrix is a represented CYBE image of this kernel.
The Koszul scalar complementarity is $\varkappa(\cA) := \kappa(\cA) + \kappa(\cA^!)$:
$\varkappa = 0$ for Kac--Moody and free fields, $\varkappa = 13$ for Virasoro,
$\varkappa = 250/3$ for $\Walg_3$. This scalar invariant is distinct from the
Trinity ghost-charge conductor $K(\cA) := c(\cA) + c(\cA^!) = -c_{\mathrm{ghost}}(\BRST)$
(values $K(\Vir)=26$, $K(\Walg_3)=100$, $K(\mathrm{BP})=50$, $K(V_k(\fg))=2\dim\fg$);
there is no single universal ratio between them. The affine line has
$K(V_k(\fg))=2\dim\fg$ but $\varkappa=0$.

Every displayed graph formula involving $F_g$ carries one of the two
tags below.  Class-valued formulas use
$\operatorname{Obs}^{\mathrm{def}}_g$, $\mathfrak O_g^K$, or
$\operatorname{obs}^{\mathrm{Hdg}}_g$ according to their target:
...
Assume the channel-decomposition and diagonal-metric stable-graph
calculus for \(\cA\). The scalar cohomological part is proved on the
scalar-diagonal component; the all-weight cross-channel decomposition below
is conditional on this graph calculus.
...
\item \emph{Per-channel universality.}
 ...
 \kappa(\cA) \cdot \lambda_g^{\FP}
 \qquad\text{for all } g \geq 1
 \quad\text{\textup{(}uniform-weight scalar component\textup{)}}.
 \end{equation}
...
\item \emph{Scalar-diagonal universality.}
 If $\cA$ lies on the uniform-weight scalar-diagonal component, then
 $\delta F_g^{\cross} = 0$ for all $g \geq 1$.

\item \emph{$R$-matrix scope.}
 The scalar diagonal term and the genus-$1$ correction are
 independent of the $R$-matrix representative. No higher-genus
 mixed-channel $R$-independence is asserted.
...
\smallskip\noindent
(iv) Under the scalar-diagonal hypothesis, all nonzero graph
assignments reduce to a single diagonal channel. Therefore
\(F_g^{\diag}=\kappa\lambda_g^{\FP}\) and the cross-channel sum is
empty.

\smallskip\noindent
(v) The assertion is restricted to the scalar diagonal term and genus
\(1\). The mixed-channel higher-genus correction can depend on the
channel-level representative; that dependence is part of the
all-weight graph calculus.
...
The three mechanisms explain why free fields form a multi-weight
family on which the scalar formula survives.
\end{proof}
...
stable graph types.
The following table records the canonical graph names and the
mixed-channel contribution used in the body computation.
...
Graphs A, B, D contribute zero cross-channel correction:
\begin{itemize}
...
\item Graph~D (dumbbell) has zero mixed contribution by the
 genus-$1$ single-edge purity relation.
\end{itemize}

Graphs C, E, F, G carry the entire cross-channel correction.
...
The four contributing graphs give:

\emph{Banana \textup{(}graph C\textup{)}.}
Two parallel boundary edges meet a single genus-$0$ vertex.
...
\emph{Theta \textup{(}graph E\textup{)}.}
...
\emph{Lollipop \textup{(}graph F\textup{)}.}
...
\emph{Barbell \textup{(}graph G\textup{)}.}
Two loop components are connected by a bridge. The mixed barbell
...
\begin{proposition}[Explicit $\delta F_2^{\cross}(\Walg_3)$]
\label{prop:delta-f-cross-w3-g2}
...
\[
 ...
 \qquad\text{\textup{(}ALL-WEIGHT\textup{)}},
\]
computed as a graph sum over the seven canonical stable graph terms of
$\overline{\cM}_{2,0}$ with rank-$2$ channel assignments.
The banana (C), theta (E), lollipop (F), and barbell (G) graphs
contribute nonzero cross-channel amplitude; the smooth, figure-eight,
and dumbbell terms vanish.
For $(\Walg_3,g=2)$ the scalar term
$\kappa\lambda_2^{\FP}$ is not the complete genus-$2$ free energy;
the missing term is exactly the displayed cross-channel correction.
...
\begin{proof}
The banana, theta, lollipop, and barbell amplitudes are computed
in Computation~\ref{comp:w3-genus2}. Summing:
\[
...
The graph enumeration reduces the genus-$3$ correction to a rational
function with denominator $138{,}240\,c^2$ and the displayed cubic
numerator.
...
The ratio to the scalar part is:
...
\begin{proposition}[Computed cross-channel dominance]\label{prop:cross-dominance}
For the computed genera \(g=2,3,4\), the full genus-$g$ free energy
of $\Walg_3$ satisfies
\textup{(ALL-WEIGHT)}:
...
\end{equation}
In the large-\(c\) limit the ratios
\[
\frac{\delta F_g^{\cross}}{\kappa \cdot \lambda_g^{\FP}}
\]
are \(0\), \(42/31\), and \(9184/381\) for \(g=2,3,4\),
respectively. Thus the cross-channel sector dominates the scalar
sector in the computed higher-genus range.
\end{proposition}

\begin{proof}
The genus \(2\) and genus \(3\) ratios follow from
Computation~\ref{comp:w3-genus2} and
Computation~\ref{comp:w3-genus3}. The genus \(4\) value is the
corresponding computed graph sum. These data prove dominance in the
computed range and make no factorial assertion in \(g\).
\end{proof}
...
% ================================================================
\section{Scalar singularities and cross-channel growth}\label{sec:instanton}

The scalar genus expansion
\(F_g^{\scal}=\kappa\lambda_g^{\FP}\), under the uniform-weight
hypothesis, has the ordinary generating function
\[
 ...
 \right).
\]
Equivalently, in the variable \(u=\hbar^2\), the nearest ordinary
singularities occur at \(u=(2\pi)^2\).

\begin{proposition}[Scalar Borel transform]\label{prop:scalar-borel}
With the factorial normalization
\[
 \mathcal{B}[Z](\xi)
 \;:=\;
 \sum_{g \geq 1}
...
\]
the scalar Borel transform is entire under the uniform-weight
hypothesis.
\end{proposition}

\begin{proof}
The Bernoulli asymptotics
\(|B_{2g}|\sim 2(2g)!/(2\pi)^{2g}\) give
\(|F_g^{\scal}|=O((2\pi)^{-2g})\). Dividing by \((g-1)!\) gives
superexponential decay, so the root test gives infinite radius of
convergence.
\end{proof}

\begin{proposition}[Computed cross-channel growth]\label{prop:instanton-hierarchy}
For \(\Walg_3\), the computed cross-channel data satisfy
\[
\delta F_2^{\cross}(\Walg_3)=O(1),\qquad
...
\[
 ...
 \qquad(c\to\infty).
\]
The scalar ordinary singularity at \(u=(2\pi)^2\) does not control the
available cross-channel data.
\end{proposition}

\begin{proof}
The first assertion is the large-\(c\) expansion of
Computation~\ref{comp:w3-genus2} and
Computation~\ref{comp:w3-genus3}:
\[
\delta F_2^{\cross}\sim \frac1{16},\qquad
\delta F_3^{\cross}\sim \frac{c}{27648}.
\]
Dividing gives \(c/1728\). This is a statement about the computed
genus \(2\) and \(3\) graph sums, not a high-genus resurgence theorem.
\end{proof}
...
summing over the four contributing stable graphs
C (banana), E (theta), F (lollipop), G (barbell) from the catalogue
...
The banana cross-channel contribution involves mixed
...
\emph{Barbell (graph G).}
The two loop factors contribute through the genus-$1$
characteristics and the bridge propagator. Its contribution is
included in the same even-parity channel expansion.

The closed-form coefficients
$B(N) = (N{-}2)(N{+}3)/96$ and
$A(N) = (N{-}2)(3N^3 + 14N^2 + 22N + 33)/24$
are obtained by expanding the four graph sums and applying the
standard power-sum identities for \(\sum_{j=2}^N j^m\).
...
The primitive object in modular Koszul duality is the
ordered bar complex
$B^{\ord}(\cA) = T^c(s^{-1}\bar{\cA})$
with deconcatenation coproduct and differential from the
$E_1$-chiral product.
The $E_1$ convolution algebra $\fg^{E_1}_{\cA}$ carries the collision
kernel $K_\cA^{\mathrm{coll}}(z)$ as the genus-$0$ binary component of
the $E_1$-framed MC element $\Theta^{E_1}_{\cA}$.  A classical
$r$-matrix appears after a representation satisfying
...
The five theorems use the homotopy-coinvariant ordered bar together
with its normalized scalar functionals. The degree-$2$ trace is
$\kappa(\cA)=\tau_{\cA,2}^{\mathrm{res}}
q_2(K_\cA^{\mathrm{coll}})$ in the abelian and scalar families; on
non-abelian affine Kac--Moody it is $\kappa_{\mathrm{dp}}$, and
$\kappa(V_k(\fg))=\kappa_{\mathrm{dp}}+\dim(\fg)/2$.
The scalar formula
...
component contributes only $4\%$.

This is the quantitative content of $E_1$ primacy for the computed
multi-weight examples.
The ordered bar $B^{\ord}(\cA)$ is the primary object carrying
most of the genus-$g$ information.
The symmetric bar, and the scalar formula that comes with it,
captures only the tip of the genus expansion.
The cross-channel correction measures the part of the ordered-bar
graph sum lost under coinvariant projection. In the computed range its
ratio to the scalar part increases with genus.
...
\begin{remark}[Free-field scalar survival]\label{rem:free-field-unique}
The free-field algebras (Heisenberg, $\beta\gamma$, $bc$,
and lattice algebras) form a multi-weight family for which the
...
Line operators in 3d holomorphic QFT: meromorphic tensor
