...
established at all degrees by induction from explicit base cases
at $r=2,3,4$. As a corollary the operadic complexity conjecture
is proved: the shadow termination degree, the transferred
...
\cite{Lorgat26}*{Thm.~H}. Its finite-degree truncations
...
\begin{theorem}[Shadow--formality identification; verified scope]
\label{thm:main}
For every verified chirally Koszul standard algebra in the
genus-zero cyclic setting, and for every degree $r\ge 2$,
\[
\Sh_r(\cA)\;=\;\ell_r^{(0),\mathrm{tr}}
(\Theta^{\le r-1}_{\cA},\dots,\Theta^{\le r-1}_{\cA}),
\]
where $\ell_r^{(0),\mathrm{tr}}$ is the transferred genus-zero
$\Linf$ bracket of degree $r$ on the minimal model of
...
filtered by tensor degree, $F^r\barB(\cA)=\bigoplus_{n\le
...
$\Sh_2(\cA)=\kappa(\cA)$ is the degree-$2$ shadow on the scalar
...
three-channel graph sum at degree three, matches this three-tree
...
obstruction tower]
\label{thm:main-body}
For every chiral algebra $\cA$ and every degree $r\ge 2$,
...
\begin{proof}
Induction on the degree $r$. The base cases $r=2,3,4$ are
...
The degree-$r$ shadow $\Sh_r(\cA)$, by
...
internal vertices decorated by lower-degree shadows
...
\eqref{eq:main-identity} through degree $r-1$, the graph sum for
...
\begin{corollary}[Operadic complexity in the verified scope]
\label{cor:operadic-body}
$r_{\max}(\cA)=d_\infty(\cA)=f_\infty(\cA)$ for the verified
chirally Koszul standard families in the genus-zero cyclic setting.
\end{corollary}
...
$\ell_4^{(0),\mathrm{tr}}$ is nonzero. Rank-one rigidity kills
degree-five graphs, giving
...
$:\!T^n\!:$ generates operations at every degree. Class
$\textbf{M}$.
\end{example}
