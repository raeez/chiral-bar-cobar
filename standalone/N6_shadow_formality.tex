...
\begin{example}[Virasoro weighted primary-line model; class \textbf{M}]
The Virasoro vertex algebra $\Vir_c$ has $\kappa(\Vir_c)=c/2$.
The local seeds and the weighted-Riccati construction give
\begin{align*}
R_2&=\tfrac{c}{2},&
R_3&=2\ \text{(}c\text{-independent)},&
R_4&=\tfrac{10}{c(5c+22)},\\
R_5&=-\tfrac{48}{c^2(5c+22)},&
R_6^{\mathrm{Ricc}}&=\tfrac{80(45c+193)}{3c^3(5c+22)^2}.
\end{align*}
The separate formal relation
$2R_2C_6^{\mathrm{rel}}+2R_3R_5+R_4^2=0$ gives
\[
C_6^{\mathrm{rel}}=\frac{4(240c+1031)}{c^3(5c+22)^2}.
\]
An ordered residue package $\mathsf H_{\mathrm{res}}(\Vir_c;X)$ and
its normalized projection define the transferred coordinates
$S_k(\Vir_c;\mathsf H_{\mathrm{res}})$.  Equality
$S_k=R_k$ is the corresponding comparison theorem.  A level-six
radical vector and its decoupling map supply the singular-vector
interpretation of $C_6^{\mathrm{rel}}$.  The weighted formal series
has infinite support for generic $c$, giving the class-$\mathbf M$
primary-line model.
\end{example}
