...
\[
 ...
 \colon \CY_d\text{-}\mathrm{Cat} \longrightarrow
 \En\text{-}\mathrm{ChirAlg}
\]
factors in two stages. Stage~1, $\Phi^{\mathrm{FA}}_d$, produces a
canonical holomorphic $E_d$-factorisation algebra
$\Phi^{\mathrm{FA}}_d(\cC)$ on a CY$_d$ variety~$X$, pinned up to
contractible choice by Kontsevich--Tamarkin $E_d$-formality on the
Gerstenhaber bracket of degree $1 - d$ on $\HH^\bullet(\cC)$ together
with Costello--Gwilliam--Li locality on~$X$. Stage~2,
$\SpCh_{\Sigma_{d-1}, C}$, is factorisation homology over a
closed $(d-1)$-cycle $\Sigma_{d-1} \subset X$ followed by restriction
to a reference curve $C \subset X$. The four computational steps --
Lie conformal algebra $\fL_\cC$ from the Gerstenhaber bracket,
factorisation envelope $\Fact_C(\fL_\cC)$, $\Sd$-framing
enhancement, CY-trace quantisation -- are the explicit realisation
of the composite. The functor is proved at $d = 2$
(Theorem~CY-A$_2$, $\Etwo$-chiral on any curve~$C$) and at $d = 3$
(Theorem~CY-A$_3$, $\Eone$-chiral output in the $\infty$-categorical
formulation). A single CY$_d$ category admits a family of
...
The CY-to-chiral functor factorises as
\[
...
algebra $\Phi^{\mathrm{FA}}_d(\cC)$ on the CY$_d$ variety~$X$:
Kontsevich--Tamarkin $E_d$-formality acts on the Gerstenhaber
bracket of degree $1 - d$ on $\HH^\bullet(\cC)$, and
Costello--Gwilliam--Li locality pins the factorisation cosheaf
up to contractible choice. Stage~2 takes the factorisation
...
%%%   CY-dimension stratification, universal trace identity
%%% ================================================================

\section{$\Phi$ scope, CY-dimension stratification, universal
  trace}
\label{sec:phi-scope-stratification-trace}

Three consolidated structural theorems pin the CY-to-chiral
functor to its precise scope: the native $\En$-output level on the
reference curve, the dimension-stratified
$\kappa_{\mathrm{ch}}$ spectrum, and the universal trace identity
connecting the Vol~I conductor $K = -c_{\mathrm{ghost}}$ with the
Vol~III Borcherds weight $\kappa_{\mathrm{BKM}} = c_N(0)/2$.

\begin{theorem}[Native $\En$-output scope of the two-stage $\Phi_d$]
\label{thm:phi-native-en-scope}
Let $\Phi_d = \SpCh_{\Sigma_{d-1}, C} \circ
\Phi^{\mathrm{FA}}_d \colon \CY_d\text{-}\mathrm{Cat}
\longrightarrow \En\text{-}\mathrm{ChirAlg}(C)$ be the CY-to-chiral
functor on a reference curve $C \subset X$ and closed
$(d-1)$-cycle $\Sigma_{d-1} \subset X$. Its native operadic level
is
\[
...
At $d \ge 3$ the $E_d$-structure on stage~1 becomes symmetric
under stage-2 specialisation ($\pi_1(\Conf_2(\bR^d)) = \bZ/2$),
and the functor output on~$C$ is natively $\Eone$-chiral; the
braided monoidal structure is recovered \emph{categorically} via
the Drinfeld centre, closing the $\En$ operadic circle on
$Z^{\mathrm{der}}_{\mathrm{ch}}$ rather than on~$\Phi_d(\cC)$.
\end{theorem}
...
\kappa_{\mathrm{ch}}\bigl(\Phi_d(D^b\Coh(X))\bigr)
...
\begin{theorem}[Universal trace identity: $K = -c_{\mathrm{ghost}} =
 2\,\kappa_{\mathrm{BKM}}$]\label{thm:phi-universal-trace}
...
charge, and the Borcherds weight of the specialisation
$\SpCh_{\Sigma_2, C}\bigl(\Phi^{\mathrm{FA}}_3(\cC_{X})\bigr)$
unify in a single identity:
\begin{equation}\label{eq:phi-universal-trace}
K(\cA) \;=\; -\,c_{\mathrm{ghost}}(\cA) \;=\; 2\,\kappa_{\mathrm{BKM}}(\Phi_N),
\end{equation}
where $\cA = \Phi_3(\cC_X)$ on the reference curve $C$ and
...
\begin{remark}[Status of $\Phi_d$ across CY dimensions]
\label{rem:phi-status-dimensional}
Stage~1 $\Phi^{\mathrm{FA}}_d$ is unconditional for $d \in \{1, 2, 3\}$
by Kontsevich--Tamarkin $E_d$-formality applied to the Gerstenhaber
bracket of degree $1 - d$ on $\HH^\bullet(\cC)$ together with
Costello--Gwilliam--Li locality on~$X$. Stage~2
$\SpCh_{\Sigma_{d-1}, C}$ is factorisation homology followed by
curve restriction, well-defined for any closed $(d-1)$-cycle
$\Sigma_{d-1}$ and reference curve $C \subset X$.
$\Phi_d$ at $d = 1$: trivial ($\Ainf = \mathrm{comm}$).
$d = 2$: proved (Theorem~\textup{CY-A$_2$}), $\Etwo$-chiral
output. $d = 3$: proved (Theorem~\textup{CY-A$_3$},
$\infty$-categorical), $\Eone$-chiral output on any reference
curve~$C$; the family of specialisations
...
$d = 4$: stage~1 obstructed by $\pi_4(BU) = \bZ$; the
$p_1$-twisted double current algebra is proved where the
Pontryagin obstruction vanishes (e.g.\ $K3 \times K3$); elsewhere
conjectural. $d = 5, 6$: open; conjectural Hodge supertrace
formula recovers $\kappa_{\mathrm{ch}} = 0$ for generic smooth CY.
The unconditional range is $d \in \{1, 2, 3\}$; $d = 4$ is
family-conditional.
\end{remark}
