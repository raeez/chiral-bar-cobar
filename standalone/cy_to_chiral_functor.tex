...
\[
 \Phi_d^{(\Sigma_{d-1},C)} \;=\; \SpCh_{\Sigma_{d-1}, C} \circ \Phi^{\mathrm{FA}}_d
...
Volume~II supplies the physical comparison surface: the derived chiral
center $\Zder(A) = C^\bullet_{\mathrm{ch}}(A,A)$ is the universal
closed-sector cochain object of~$A$. It becomes the bulk observable
algebra of a 3d holomorphic-topological gauge theory only after the
open--closed comparison map has been constructed; the
$\SCchtop$ pair $(C^\bullet_{\mathrm{ch}}(A,A), A)$
is the closed-sector--boundary algebraic system. It is a physical
boundary--bulk system only after that comparison.
...
The answer is the CY-to-chiral specialization
$\Phi_d^{(\Sigma_{d-1},C)} = \SpCh_{\Sigma_{d-1}, C} \circ \Phi^{\mathrm{FA}}_d$.
...
characteristic, boundary chiral algebra, Borcherds denominator weight,
and lattice frame.  The chiral-holographic pair
$(\kappa_{\mathrm{ch}}^{\mathrm{Heis}}, \kappa_{\mathrm{BKM}}) = (3, 5)$ governs
the boundary--Borcherds comparison axis of Volumes~I--II.  The
...
\[
 \Phi_d^{(\Sigma_{d-1},C)} \;=\; \SpCh_{\Sigma_{d-1}, C} \circ
 \Phi^{\mathrm{FA}}_d
 \colon \CY_d^{\mathrm{wit}}\text{-}\mathrm{Cat} \longrightarrow
 \En\text{-}\mathrm{ChirAlg}.
\]
...
\begin{enumerate}[label=(\roman*)]
 ...
 $A_\cC=T_X(V_\cC)/(R_\cC)$, the presentation coalgebra is
 $A_\cC^i=C_X(s^{-1}V_\cC,s^{-2}R_\cC)$, with
 $q_{A_\cC}\colon A_\cC^i\to B(A_\cC)$.  On the Koszul locus,
 $H^\bullet(q_{A_\cC})$ identifies its cohomology with
 $H^\bullet B(A_\cC)$.  A BPS comparison datum may then identify
 the latter coalgebra with the corresponding BPS state space.
 \item Verdier duality forms
 $K_X(A_\cC)=D_{\Ran}B(A_\cC)$, and a chosen pair map
 $\nu_{A_\cC}\colon K_X(A_\cC)\to A_\cC^!$ reaches the Koszul partner.
 For CY$_2$ categories, the mirror statement is the corresponding
 CY-to-chiral comparison theorem.
 \item $\Zder(A_\cC)$ (the derived chiral center):
 the chiral Hochschild cochain complex encoding the algebraic
 closed-sector slot; a physical bulk interpretation requires the
 open--closed comparison datum.
\end{enumerate}
The bar complex is the $\Eone$ engine; the derived center is the
$\SCchtop/E_3$ output.  The counit
$\Omega(B(A_\cC))\to A_\cC$ performs reconstruction, Verdier duality
forms $K_X(A_\cC)$, and categorical Hochschild cochains form the
derived center.
\end{remark}
...
An unsubscripted $\kappa$ has no invariant meaning on this surface.
...
\begin{enumerate}[label=(\roman*)]
 ...
 \item $\kappa_{\mathrm{BKM}}=c_N(0)/2$ is the Borcherds weight of
 the automorphic BKM denominator in the Vol~III normalization.
...
\subsection{The CoHA as ordered Hall input}
...
\begin{itemize}
\item The CoHA is the positive half: an ordered associative Hall
 algebra before chiralisation.
 ...
 \item The ordered bar $B^{\mathrm{ord}}(A_\cC)$ carries the
 collision kernel and its represented $R$-operator.  The symmetric
 bar $B^\Sigma(A_\cC)$ carries the homotopy-coinvariant operator
 $q_2(K_{A_\cC}^{\mathrm{coll}})$.  A normalized functional
 $\tau_{A_\cC,2}^{\mathrm{res}}$ gives $\kappa_{\mathrm{ch}}$ in
 abelian and scalar families and $\kappa_{\mathrm{dp}}$ on the
 non-abelian affine row; the Sugawara contribution $\dim(\fg)/2$
 completes $\kappa_{\mathrm{ch}}$ there.
\end{itemize}
...
identification requires the uniform-weight and $\Phi_3^{(\Sigma_2,C)}$ hypotheses,
...
The Drinfeld center is the categorical-centre step in the Volume~I
circle: it is the right adjoint to forgetting the braiding and
recovers non-trivial half-braidings at the categorical level. The
averaging map
$\mathrm{av} \colon \fg^{\Eone} \to \fg^{\mathrm{mod}}$
is a different, lossy projection from ordered $R$-matrix data to the
scalar $\kappa_{\mathrm{ch}}$ shadow in the $\kappa_\bullet$-spectrum.
...
\begin{theorem}[Native $\En$-output scope of the two-stage $\Phi_d^{(\Sigma_{d-1},C)}$]
...
\kappa_{\mathrm{ch}}^{\mathrm{Hodge}}\bigl(\Phi_d^{(\Sigma_{d-1},C)}(D^b\Coh(X))\bigr)
...
\]
Thus $c_N(0)=(10,6,4,3,2)$ \textup{(}Jatkar--Sen /
Govindarajan--Krishna; the fourth rung half-integral\textup{)} gives
\[
 \kappa_{\mathrm{BKM}}=(5,3,2,\tfrac32,1).
\]
...
\mathrm{BKM}, \mathrm{fiber}\}$. The identifications
...
\begin{remark}[Dimension range of $\Phi_d^{(\Sigma_{d-1},C)}$]
...
$(\Sigma_{d-1},C)$.
 $\Phi_d^{(\Sigma_0,C)}$ at $d = 1$: trivial ($\Ainf = \mathrm{comm}$).
