...
models at $d \geq 3$, the relevant Schouten--Nijenhuis bracket
vanishes. General CY categories are $\Eone$ by stage and degree, not
by abelianity.
\end{abstract}
...
filtration, the positive-half Hall multiplication and the hCS
extension correspondence define the same ordered $\Eone$ operation.
The compact Hall, quasi-NCCR, and Hall-double comparisons are separate
global identifications and remain conditional.
...
the reference curve.  In this local invariant lane the relevant
Schouten--Nijenhuis bracket on polyvector fields vanishes on the CY
sector, and the CoHA extension correspondence imposes a preferred
ordering (sub before quotient).  For general CY categories the
$\Eone$ level is forced by Stage~2 and degree, not by a universal
abelianity statement.  The braided monoidal structure is recovered
...
Pontryagin obstruction vanishes (e.g.\ $K3 \times K3$). Away from
that vanishing locus, the construction requires the missing
$p_1$-twisted factorisation datum. For $d = 5, 6$ no general
...
Thus the unconditional range is $d \in \{1, 2, 3\}$, and the $d=4$
range is exactly the Pontryagin-vanishing family.
\end{remark}
