...
trace.  The functor is proved unconditionally at $d = 2$
(Theorem~CY-A$_2$); at $d = 3$, the construction is conditional
on the chain-level $\bS^3$-framing.
...
\begin{enumerate}[(i)]
 ...
 $K3 \times E$.  The functor $\Phi$ at $d = 3$ is conditional
 on the chain-level $\bS^3$-framing.
...
trace on its Hochschild homology:
\[
...
\begin{proposition}[Hochschild homology and the Gerstenhaber bracket]
\label{prop:v3-st-gerstenhaber}
The Hochschild cohomology $\HH^\bullet(\cC)$ of any dg category
...
\begin{proof}
The Gerstenhaber bracket is a standard structure on Hochschild
...
Hochschild complex by the $d$-sphere.  The $\bS^1$-action
...
$\bS^3$-framing compatible with the BV structure is the open core
of the $d = 3$ programme.
...
assuming the $d$-dimensional CY-to-chiral functor $\Phi$ exists,
\[
 \kappa_{\mathrm{ch}}(\Phi(\cC)) \;=\; \chi^{\CY}(\cC).
\]
\end{conjecture}
...
\begin{enumerate}[label=(\roman*)]
 ...
 Hochschild data.  This is the multiplier in the BPS degeneracy
