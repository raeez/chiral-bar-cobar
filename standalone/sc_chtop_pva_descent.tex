...
First, the classical Swiss-cheese operad is Koszul and the closed
colour of $\SCchtop$ is formal; the full two-colour homotopy-Koszul
statement requires a mixed-sector quasi-isomorphism.
...
these descend to the shifted PVA shadow.
...
\begin{equation}\label{eq:intro-SC-pair}
  ...
  \;=\;
  (\text{closed sector},\; \text{boundary})
\end{equation}
is the datum governed by the two-coloured Swiss-cheese operad
$\SCchtop$.  The closed colour is the chiral Hochschild object;
it becomes a physical bulk only after the open--closed comparison
quasi-isomorphism has been chosen.  The open colour is the boundary.
The closed sector acts on the boundary; the boundary does not act on
the closed sector.  This directionality is the empty open-to-closed
...
\item $B$ is an algebra over the FM-operad
  $\{\FM_k(\CC)\}_{k \geq 1}$ (the closed-sector algebra);
...
$\SCchtop$-algebra.  The closed colour is the chiral Hochschild
sector; the open colour is the boundary.  The bar complex $\barB(\cA)$ is used
...
\begin{theorem}[Classical Swiss-cheese Koszulity and local product-formal shadow]
\label{thm:homotopy-koszul-SC}
\ClaimStatusConditional{}
The classical Swiss-cheese operad is Koszul, and the closed colour of
$\SCchtop$ is formal. The full two-coloured statement is conditional
on a mixed-sector quasi-isomorphism. Under that additional hypothesis,
the canonical map
\begin{equation}\label{eq:bar-cobar-SC}
  ...
  \SCchtop
\end{equation}
is a quasi-isomorphism; only then does the bar-cobar adjunction on
$\SCchtop$-algebras give the asserted Quillen equivalence.
\end{theorem}
...
$\cQ = \SC$ requires a mixed-sector quasi-isomorphism in addition to
the closed-colour formality quasi-isomorphism and the identity on the
open colour.
...
between the closed sector and the line side: the closed colour is governed by
...
\begin{definition}[Shifted Poisson vertex algebra shadow]
\label{def:PVA}
A \emph{$(-1)$-shifted Poisson vertex algebra shadow} over $\Bbbk$ is a
...
\begin{theorem}[Shifted PVA descent]
\label{thm:pva-descent}
\ClaimStatusConditional{}
Let $\cA$ be a logarithmic $\SCchtop$-algebra on a smooth curve $X$,
with the regular and singular kernels separated, and let
...
\item The pair $(H, \cdot, \{-_\lambda -\})$ is a
  $(-1)$-shifted Poisson vertex algebra shadow in the sense of
  Definition~\textup{\ref{def:PVA}}, with shifted Koszul signs.

\item The higher transferred operations $m_k^H$ for $k \geq 3$
  encode the homotopy corrections to the chain-level shadow
  formality problem. Class~G is the strict chain-level vanishing
  case, not the general cohomological PVA-descent statement.
\end{enumerate}
\end{theorem}
...
bootstrap: the minimal-model propagator from cohomology $H$ into the
chain algebra $\cA$ is determined self-consistently from the free
...
\item Classical $r$-matrix: $r(z) = k\,\Omega_{\mathrm{tr}}/z$ in the
  trace-form convention, where $\Omega_{\mathrm{tr}} = \sum_a J^a \otimes J_a$ is
  the split Casimir.
...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
%% Heptagon embedding, topologization scope, and PVA descent
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%

\section{Heptagon, topologization scope, and PVA descent}
\label{sec:schtop-heptagon}

The operad $\SCchtop$ is controlled by three tests: the Vol~II
$\SCchtop$ heptagon (seven edges routing bar, cobar, Drinfeld
centre, derived centre, Swiss-cheese, PTVV, and celestial
holography), the four-class topologisation scope tree, and the
Khan--Zeng PVA descent for freely-generated Poisson vertex algebras.

\begin{theorem}[The $\SCchtop$ heptagon]
\label{thm:schtop-heptagon}
...
\begin{theorem}[Sugawara topologization by shadow class]
\label{thm:topologization-scope}
...
\label{thm:khan-zeng-pva-descent}
...
\begin{remark}[The bar complex is not an $\SCchtop$-coalgebra]
\label{rem:bar-not-schtop-coalgebra}
...
$\SCchtop$-structure. The obstruction is formal: the bar
differential is not a closed colour, deconcatenation is not an open
colour, and two structures on one coalgebra do not produce the two
colours of $\SCchtop$. The $\SCchtop$ structure emerges in
$\Hom(B(\cA),\cA)$, not in $B(\cA)$.
\end{remark}

\begin{remark}[$\SCchtop$ is Koszul, not self-dual]
\label{rem:schtop-not-selfdual}
...
\begin{remark}[Status of the comparison theorems]
\label{rem:schtop-epistemic}
Theorem~\ref{thm:schtop-heptagon}: the seven heptagon edges are
\emph{proved} in the Vol~II heptagon chapter; the
operadic hub (edges 1--3) is proved directly.
Theorem~\ref{thm:topologization-scope}: class~G, L, C
are unconditional on the original complex; class~M is
weight-completed. Theorem~\ref{thm:khan-zeng-pva-descent}:
