...
as Conjecture~\ref{conj:general-ds-primitivity} and prove it
...
(Theorem~\ref{thm:grav-primitive}) and its connection to the
...
theorem and derives the shadow metric, the closed-form
generating function, and the shadow connection.
Section~\ref{sec:classification} proves the four-class
classification and the universal factorization theorem.
Section~\ref{sec:landscape} computes $(\kappa, \alpha, S_4)$
for every standard family and presents explicit shadow towers.
Section~\ref{sec:genus2} carries out the genus-$2$ graph sum,
including the multi-channel computation for~$\mathcal{W}_3$
and the planted-forest correction.
Section~\ref{sec:complementarity} develops the complementarity
constraints and duality identities.
Section~\ref{sec:gravitational} proves the gravitational
...
\section{The shadow metric and Riccati algebraicity}
...
\textit{Proof.  (To be written.)} \qed

\input{classification}

\input{computations}

\section{The gravitational coproduct}\label{sec:gravitational}

\begin{theorem}[Gravitational primitivity]
\label{thm:grav-primitive}
Let\/ $\Delta_z^{\mathrm{aff}}$ be the coproduct of the
dg-shifted Yangian $Y^{\mathrm{dg}}(\widehat{\mathfrak{sl}}_2)$,
and let $(i, p, h)$ be the SDR from the BRST complex to the
Virasoro cohomology.  The HPL-transferred coproduct satisfies
\[
  \Delta_z^{\Vir}(T)
  \;=\;
  \tau_z(T) \otimes 1 + 1 \otimes T.
\]
All higher-arity coproduct corrections vanish:
$\Delta_{z,n}^{\Vir} = 0$ for $n \ge 2$.
\end{theorem}
