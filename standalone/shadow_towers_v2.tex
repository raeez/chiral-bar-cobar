...
\begin{abstract}
Let $\cA$ be a chirally Koszul vertex algebra with modular
characteristic~$\kappa$, cubic shadow~$\alpha$, and quartic
shadow~$S_4$. We prove four results.
First, the weighted shadow generating function
...
and every $S_r$ for $r \ge 5$ is determined by the three seeds
$(\kappa, \alpha, S_4)$.
Second, the critical discriminant $\Delta = 8\kappa S_4$
partitions the standard landscape into four depth classes:
Gaussian ($r_{\max} = 2$), Lie ($r_{\max} = 3$), contact
($r_{\max} = 4$), and mixed ($r_{\max} = \infty$).
Third, the genus-$2$ free energy is
$F_2 = 7\kappa/5760$ for single-generator algebras; the first
multi-channel computation for~$\cW_3$ exhibits a cross-channel
correction $\delta F_2 = (c{+}204)/(16c)$, resolving the
multi-generator universality question negatively.
Fourth, the gravitational coproduct obtained by BRST reduction
from $\widehat{\mathfrak{sl}}_2$ to Virasoro is strictly primitive
at all degrees; the mechanism applies to all principal
...
% ================================================================
% Intermediate shadow-tower statement; canonical refinements are in v3.
% ================================================================
% This v2 file is weaker than the canonical standalone
%   standalone/shadow_towers_v3.tex
% v3 carries the following anchors: pole-doubling for all k on
% Sigma_Vir^{(k)} (thm:pole-doubling-all-k), C_A = 6 universal
% (thm:universal-class-M-C-is-6), 6 = r_0 . S_{r_0} = 3 . 2, large-c
% large-c asymptotic 2/c^2, cross-channel delta F_2(W_3) = (c+204)/(16c)
% on the seven stable graphs of M-bar_{2,0}, and beta_N = (N+1)(N+2)/2
% sub-subleading coefficient.  Preserve this file for archive only.
% ================================================================

\begin{remark}[Relationship to companion papers]
The $c$-independent identity $S_3(\mathrm{Vir}_c) = 2$ is now proved
as a theorem; see~\cite{LorgatVirR}. The seven-face
programme connecting the shadow tower to Gaudin models, Sklyanin
brackets, and GZ26 commuting Hamiltonians is developed
in~\cite{LorgatSevenFaces}.
\end{remark}
...
At genus~$0$, the classical theory embeds via restriction to
the formal disk (the embedding requires specifying a retraction
$\mathbb{A}^1 \to \mathrm{pt}$ and its homotopy transfer;
the $E_1/E_\infty$ bar distinction, the Arnol'd algebra
of~$\mathrm{Conf}_n(\mathbb{A}^1)$, and the Fulton--MacPherson
compactifications are already absent over a bare point). At
genus~$g \ge 1$, the topology of the curve forces further
...
truncating the degree filtration at level~$r$, form the
\emph{shadow obstruction tower}. At degree~$2$, the tower
produces the modular characteristic~$\kappa$. At degree~$3$,
a cubic shadow~$\alpha$ (the structure constant of the transferred
$L_\infty$ bracket $\ell_3$). At degree~$4$, a quartic
shadow~$S_4$ (the first genuinely nonlinear invariant, measuring
the contact term in the four-point collision residue).

The obstruction to truncating $\Theta_\cA$ at finite degree is
measured by classes
...
at every degree, and the shadow coefficients $S_r$ decay or grow at
...
Classical Koszul duality over a field embeds into the genus-$0$,
degree-$2$, $\Delta = 0$ stratum (the Gaussian case where the tower
terminates immediately) via restriction to the formal disk;
the embedding is not an equivalence, since the configuration-space
geometry and the $E_1/E_\infty$ distinction are already absent
over a bare point. Everything beyond class~$\mathbf{G}$ is new.
...
tadpole, and chain), weighted by automorphism factors
...
be \emph{strictly primitive at all degrees}. The mechanism
...
\[
 ...
 \quad\text{for } n \ge 2.
\]
The ghost-number mechanism kills all higher-degree coproduct
...
\item \textbf{Planted-forest formula \textup{(}conditional\textup{)}.}
 Under the determinant-line sign and residue-pushforward transport
 package, the genus-$2$ planted-forest correction on a primary
...
Part~(v) is conditional on the orientation and pushforward package
...
depth. The operadic complexity theorem identifies
...
identification is proved at all degrees by induction on~$r$
\textup{(thm:shadow-formality-identification)}.
...
degrees $r \ge 12$, the numerators may be divisible by
...
\subsection{The genus-$1$ amplitude Mellin transform is Eisenstein}
...
it is an Eisenstein $L$-function, not cuspidal. (The shadow
$L$-function $L_\cA^{\mathrm{sh}}(s) := \sum_{r \geq 2}
S_r\,r^{-s}$, which extracts degree-level constant terms, is
a \emph{different} object that does not equal
$-\kappa\,\zeta(s)\,\zeta(s{-}1)$.)
All connections between the shadow obstruction tower
