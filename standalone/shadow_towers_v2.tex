...
\subsection{The answer: the shadow tower}
...
is that the shadow tower is controlled by a single quadratic
polynomial.
...
complex is formal in the classical sense: the shadow tower
...
Class~$\mathbf{M}$ is the generic class: the shadow tower is
...
of the shadow metric.  When $\rho < 1$, the shadow tower converges
...
changes sign around a simple zero, and the shadow tower inherits
...
The shadow tower produces invariants of moduli spaces by
...
contributions from the shadow tower.
...
shadow tower appear to be new.

\subsection{Structure of the paper}

Section~\ref{sec:shadow-metric} reviews the shadow Postnikov
...
for every standard family, and presents explicit shadow towers,
...
$\delta F_2 = (c+120)/(16c)$ to the scalar approximation
$F_2^{\mathrm{scal}} = \kappa \cdot \lambda_2^{\mathrm{FP}}$.
Whether the CohFT $R$-matrix produces compensating terms that
restore the identity $F_g = \kappa \cdot \lambda_g^{\mathrm{FP}}$
at genus~$g \ge 2$ for multi-generator algebras remains open.
At genus~$1$, the identity holds unconditionally for all families.
...
\subsection{Pixton's ideal and the shadow tower}

For class~$\mathbf{M}$ algebras (infinite shadow tower), the
...
and it is natural to ask whether the shadow tower of a single
...
a more intricate grading, and the transferred coproduct may be
nontrivial.
