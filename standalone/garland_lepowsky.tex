...
$\dim H^n = 2n{+}1$ for $\fg = \fsl_2$ and, more
...
Feigin--Frenkel complementarity on the affine Kac--Moody line (the
complementarity sum vanishes family-specifically; the universal
form is $\kappa + \kappa^! = \varrho_\cA \cdot K$ with
$\varrho_{\mathrm{KM}} = 0$, which reduces to~$0$ here
independently of the value of $K_{\ghat}$). The shadow obstruction tower is class~$L$
with depth $r_{\max} = 3$: a nonzero cubic shadow from the Lie
bracket, killed at the quartic level by the Jacobi identity.
\end{abstract}

\begin{remark}[Wave-14 reconstitution anchor]\label{rem:wave14-anchor-GL}
This standalone inscribes the Platonic content of the chiral
Garland--Lepowsky theorem. The 2026-04-16 closure wave upgraded two
downstream infrastructures that this note now cites rather than
re-proves: (i) the periodic-CDG admissible-KL bar-cobar adjunction
...
compactification~\cite{} of the configuration space
...
\begin{proof}
Garland--Lepowsky~\cite{GL76} prove single-degree concentration
for every simply-laced affine Kac--Moody algebra by the Kostant
mechanism (Remark~\ref{rem:GL-mechanism}): the Whitehead lemma
kills all $E_1^{p,q}$ with $p \geq 1$, leaving only the
$\fg$-invariant column $E_1^{0,q} =
S^q(\fn_-^{\mathrm{loop}})^{\fg}$. For simply-laced $\fg$
