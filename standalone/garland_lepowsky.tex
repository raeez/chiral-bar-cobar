...
Feigin--Frenkel complementarity on the affine Kac--Moody line:
...
\begin{remark}[Admissible-KL and Positselski inputs]\label{rem:gl-inputs}
This paper uses two downstream inputs rather than re-proving them:
(i) the periodic-CDG admissible-KL bar-cobar adjunction
$\Omega^{\mathrm{ch}} \dashv B^{\mathrm{ch}}$
(\textup{\textsf{thm:periodic-cdg-is-koszul-compatible}},
\textup{\textsf{thm:admissible-kl-bar-cobar-adjunction}}, Vol~I
\verb|chapters/theory/periodic_cdg_admissible.tex|), which descends
the chiral bar-cobar adjunction unconditionally to
$KL_k^{\mathrm{adm}} \rightleftarrows (KL_{k^!}^{\mathrm{adm}})^{\mathrm{op}}$
at $k + h^\vee = p/q$ via Arakawa $C_2$-cofiniteness + finite length;
and (ii) the chiral Positselski weight-completed equivalence and
the chiral co-contra correspondence
(\textup{\textsf{thm:chiral-positselski-weight-completed}} at
\verb|chapters/theory/theorem_B_scope_platonic.tex|,
\textup{\textsf{thm:chiral-co-contra-correspondence}} at
\verb|chapters/theory/bar_cobar_adjunction_inversion.tex|), which
give respectively the conilpotent counit isomorphism on the
weight-completed bar coalgebra and the co-contra correspondence
$D^{\mathrm{co}}_{\mathrm{chi}} \simeq D^{\mathrm{ctr}}_{\mathrm{chi}}$.
The Garland--Lepowsky concentration is the generic-level witness of a
structure that now extends to the admissible locus; at the critical
level $k=-h^\vee$, Koszulness fails and the Feigin--Frenkel centre
appears (Warning~\ref{warn:bar-vs-ce} + \S\ref{sec:critical}).
\end{remark}
...
Garland--Lepowsky is an imported affine nilradical homology
calculation, not a consequence of the finite-dimensional Whitehead
lemma. The PBW filtration in the chiral bar complex identifies the
associated graded with the Chevalley--Eilenberg complex of
$\mathfrak u^-$; Garland--Lepowsky then supplies the homology
decomposition. The remaining local work is the PBW comparison between
the chiral bar differential and this affine nilradical complex.
\end{remark}
...
The affine Weyl decomposition in
Theorem~\ref{thm:garland-lepowsky} gives the bidegree support of
$H^*_{\CE}(\fn_-^{\mathrm{loop}};\mathbb C)$. The PBW comparison
identifies this $E_2$ page with the associated graded bar cohomology;
no Whitehead-lemma substitute proves the Garland--Lepowsky
calculation.
...
% Simply-laced ADE case, per Garland-Lepowsky 1976.
\begin{theorem}[Concentration and Hilbert series, simply-laced case]
...
a single cohomological degree at each conformal weight, and
the total graded Hilbert series of the $\fg$-invariant
$E_\infty$-page equals
\begin{equation}\label{eq:bar-hilbert-ADE}
\sum_{n \geq 0}\dim H^n\bigl(\widehat{\fg}_k^-;\,\mathbb{C}\bigr)\,
q^{\mathrm{wt}}
\;=\; \eta(q)^{-\rk(\fg)}
\end{equation}
up to the universal prefactor $q^{-\rk(\fg)/24}$.
\end{theorem}

\begin{proof}
Garland--Lepowsky~\cite{GL76} compute the affine nilradical homology
by affine Weyl representatives. For simply-laced $\fg$
the invariant ring of the adjoint representation on each
negative mode $t^{-m}$ is a polynomial ring on the $r$
fundamental Casimirs at degrees equal to the exponents
$d_1,\ldots,d_r$ of $\fg$; summing over $m \geq 1$ and
passing to the $\fg$-invariant Hilbert series gives
$\prod_{m \geq 1}(1-q^m)^{-r} = \eta(q)^{-r} \cdot q^{-r/24}$.
\end{proof}

\begin{corollary}[$\fsl_2$ specialization]\label{cor:bar-dim-formula}
For $\fg = \fsl_2$ ($r = 1$), Theorem~\ref{thm:bar-hilbert-ADE}
specializes to
