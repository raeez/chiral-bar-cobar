...
$k \neq -h^\vee$, the PBW spectral sequence is controlled by
Garland--Lepowsky affine nilradical homology. For $\fg=\fsl_2$ the
verified Koszul-dual dimension sequence is $\dim H^n=2n+1$; more
generally, polynomial growth is governed by the Weyl group of~$\fg$.
This diagonal PBW control is equivalent to chiral Koszulness of~$V_k(\fg)$
via the PBW spectral sequence. The Koszul dual is the chiral
Chevalley--Eilenberg algebra at the Verdier-reflected level
$k' = -k - 2h^\vee$, with modular characteristic
$\kappa(\ghat_k) = \dim(\fg)(k{+}h^\vee)/(2h^\vee)$ satisfying the
Verdier complementarity on the affine Kac--Moody line:
$\kappa(\ghat_k)+\kappa(\ghat_{-k-2h^\vee})=0$. This scalar
complementarity is distinct from the central-charge conductor
$K_c(V_k(\fg))=2\dim\fg$. The shadow obstruction tower is class~$L$
with depth $r_{\max} = 3$: a nonzero cubic shadow from the Lie
bracket, killed at the quartic level by the Jacobi identity.
\end{abstract}

\begin{remark}[Bar--cobar inputs at admissible level]\label{rem:gl-inputs}
Two structural results from~\cite{Lor-MKD} are used. First, the
periodic-CDG admissible-KL bar--cobar adjunction
\[
\Omega^{\mathrm{ch}} \dashv B^{\mathrm{ch}}\colon
KL_k^{\mathrm{adm}} \rightleftarrows
(KL_{k^!}^{\mathrm{adm}})^{\mathrm{op}}
\]
holds for the universal admissible non-critical PBW ambient at
\(k+h^\vee=p/q\), using Arakawa \(C_2\)-cofiniteness and finite
length. Simple quotients require the separate admissible quotient
analysis. Second, the
weight-completed chiral Positselski theorem gives the conilpotent
counit isomorphism on the completed bar coalgebra and the co-contra
equivalence
\[
D^{\mathrm{co}}_{\mathrm{ch}}\simeq D^{\mathrm{ctr}}_{\mathrm{ch}}.
\]
Thus Garland--Lepowsky concentration controls the generic level and
extends, through these structural inputs, to the admissible
non-critical locus. At \(k=-h^\vee\) the Feigin--Frenkel centre
changes the bar complex; see \S\ref{sec:critical}.
\end{remark}
...
At non-critical level \(k\neq -h^\vee\), the chiral bar complex
\(\barB(V_k(\fg))\) has bar cohomology on the PBW Koszul diagonal. The
PBW spectral sequence has \(E_1\)-page the Chevalley--Eilenberg complex
of the negative loop algebra with trivial coefficients, and the
Garland--Lepowsky concentration forces \(E_2=E_\infty\).
...
Let
\[
\mathfrak u^-=\fg\otimes t^{-1}\mathbb C[t^{-1}]
\]
be the nilradical opposite to the standard affine parabolic, and let
$W_{\mathrm{aff}}^0$ be the minimal representatives for
$W_{\mathrm{fin}}\backslash W_{\mathrm{aff}}$. Then
\[
H_p(\mathfrak u^-;\mathbb C)
\cong
\bigoplus_{\substack{w\in W_{\mathrm{aff}}^0\\ \ell(w)=p}}
L_{\fg}\!\left(\overline{w(\widehat\rho)-\widehat\rho}\right)
\otimes \mathbb C_{\deg_\delta(w)}.
\]
Dualizing gives the corresponding cohomology statement for
$H^p(\fn_-^{\mathrm{loop}};\mathbb C)$ with the opposite weight
convention.
\end{theorem}

\begin{remark}\label{rem:GL-mechanism}
Garland--Lepowsky computes affine nilradical homology independently of
Whitehead's finite-dimensional lemma. Under the PBW filtration, the
associated graded chiral bar complex is the Chevalley--Eilenberg
complex of \(\mathfrak u^-\). The PBW comparison identifies the
\(E_2\)-page with the Garland--Lepowsky homology decomposition.
\end{remark}
...
compactification~\cite{FultonMacPherson94} of the configuration space
...
\textup{(}the vertex product \(J^a_{(1)}J^b\)\textup{)}; the second
from the simple pole \textup{(}the product \(J^a_{(0)}J^b\)\textup{)}.
\end{proposition}

\begin{proof}
The residue extraction is governed by the vertex products in the OPE
\[
J^a(z_1) J^b(z_2)
...
+ \text{regular}.
\]
The product \(J^a_{(1)}J^b=k(J^a,J^b)\mathbf 1\) contributes the
curvature term, while \(J^a_{(0)}J^b=f^{ab}{}_cJ^c\) contributes the
collision bracket. The logarithmic form records the collision
divisor; it does not identify the curvature channel with the bracket
channel.
\end{proof}
...
$m_0$ is the element controlling the first curved Stasheff relation
\(m_1^2(a)=[m_0,a]\) on the generating space:
\begin{equation}\label{eq:curvature-km}
m_0 \;=\; \frac{k + h^\vee}{2h^\vee} \cdot \kappa_{\fg},
\end{equation}
...
and the induced flat relation is $m_1^2=0$.
\end{proof}
...
if the bar cohomology $H^*(\barB(\mathcal{A}))$ lies on the
Koszul diagonal determined by the PBW/conformal grading:
\begin{equation}
H^n(\barB(\mathcal{A}))_h \;\neq\; 0
\quad \Longrightarrow \quad h=h(n)
\end{equation}
for the diagonal function fixed by the PBW grading of the family.
...
\begin{proof}
The PBW filtration on \(V_k(\fg)\) gives a bar spectral sequence whose
\(E_1\)-page is the Chevalley--Eilenberg complex
\[
  C^*_{\CE}(\fn_-^{\mathrm{loop}};\mathbb C).
\]
Garland--Lepowsky compute the \(E_2\)-page by affine Weyl
representatives. Their calculation supplies the diagonal bidegree
support needed for collapse; no finite-dimensional Whitehead-lemma
substitute proves this affine nilradical statement.

The same conclusion is recovered from PBW generation. The universal
affine vertex algebra is freely strongly generated by the currents
\(\{J^a\}_{a=1}^{\dim\fg}\), and its associated graded chiral algebra
is \(\Sym^{\mathrm{ch}}(\fg_k)\). The symmetric algebra is Koszul by
Priddy~\cite{Priddy70}; the PBW universality criterion
(Proposition~5.3.1 of~\cite{Lor-MKD}) therefore identifies the
filtered chiral bar complex with its Koszul associated graded and
gives chiral Koszulness.
\end{proof}
...
\begin{remark}[Chiral bar cohomology vs.\ Chevalley--Eilenberg
...
\subsection{The affine nilradical mechanism in detail}

The PBW spectral sequence collapse in the proof of
Theorem~\ref{thm:chiral-koszul-km} is the mechanism by which the
classical Garland--Lepowsky theorem enters the chiral bar complex.
...
modulo the weight-$1$ stratum. The finite-dimensional Whitehead
lemma is not the Garland--Lepowsky calculation. It only controls
finite-dimensional $\fg$-module pieces. The required affine
nilradical homology is imported from
Theorem~\ref{thm:garland-lepowsky}. In the finite-dimensional pieces,
for a semisimple Lie algebra $\fg$ and a nontrivial
finite-dimensional simple $\fg$-module~$M$,
\begin{equation}\label{eq:whitehead}
H^p(\fg; M) \;=\; 0 \qquad \text{for all } p \geq 0.
\end{equation}
The spectral sequence comparison therefore uses Garland--Lepowsky for
the affine nilradical page; it is not a Whitehead-lemma substitute.
...
\begin{theorem}[Garland--Lepowsky nilradical concentration]
\label{thm:bar-hilbert-ADE}
Let $\widehat{\fg}_k$ be the affine Kac--Moody algebra
associated to a simply-laced simple Lie algebra $\fg$ of type
ADE and rank $r = \rk(\fg)$, at generic (non-critical) level
$k \neq -h^\vee$. Then the Lie-algebra cohomology
$H^*(\widehat{\fg}_k^-;\,\mathbb{C})$ of the negative part
$\widehat{\fg}_k^- = \fn_-^{\mathrm{loop}}$ is concentrated in
a single cohomological degree at each conformal weight. The affine
Weyl decomposition of Theorem~\ref{thm:garland-lepowsky} gives the
\(E_2\)-page of the PBW spectral sequence for the chiral bar complex.
It does not by itself give a closed Hilbert series for the full chiral
bar cohomology in every rank.
\end{theorem}

\begin{proof}
Garland--Lepowsky~\cite{GL76} compute the affine nilradical homology
by affine Weyl representatives. Under Proposition~\ref{prop:pbw-e1}
this cohomology is the \(E_2\)-page of the PBW spectral sequence. The
diagonal support is the concentration input needed for Koszulness; any
rank-by-rank Hilbert series for the chiral bar cohomology requires the
additional Orlik--Solomon form factor and is recorded only where
explicitly computed.
\end{proof}

\begin{corollary}[$\fsl_2$ specialization]\label{cor:bar-dim-formula}
For $\fg = \fsl_2$ ($r = 1$), the verified chiral bar cohomology
dimensions are
...
The Garland--Lepowsky affine nilradical computation identifies the
PBW spectral sequence page with the relevant $\fsl_2$-invariant
subalgebra, generated by one Casimir per mode $t^{-m}$, $m \geq 1$ (the
...
Part~(ii): at bar degree~$1$, the reduced bar complex is
\(s^{-1}\overline{V}_k(\fg)\). Its weight-\(1\) subspace is
\(s^{-1}\fg\). The reduced bar differential has no unary term on
these indecomposable current generators, and no bar-degree-\(2\)
boundary has weight \(1\). Hence the surviving cohomology is
\(\fg\), the generator space of the Koszul dual.
...
Computation~7.2.1 and Remark~\ref{warn:bar-vs-ce}).
\end{proof}
...
\end{tabular}
\end{center}
Here \(p(n)\) is the partition function and \(M(n)\) denotes Motzkin
numbers. The \(\widehat{\fsl}_3\) row records the expected exponential
rate beyond the computed low-degree terms. The proved statement here is
the class-\(L\) shadow-depth classification; the precise asymptotic
growth for every rank \(r\ge2\) remains a separate bar-cohomology
computation.
\end{remark}
...
\begin{proof}
The modular characteristic is the pointed genus-$1$ trace
coefficient:
$\operatorname{tr}_1\operatorname{Obs}^{\mathrm{def}}_{1,1}(V_k(\fg))
=\kappa\lambda_1$, where $\lambda_1$ is the first
...
\item The higher obstruction classes vanish in the strict
 class-\(\mathsf L\) representative supplied by the quadrichotomy
 theorem.
\end{enumerate}
\end{theorem}
...
The vanishing of the remaining obstruction tower is the class-\(L\)
strict-representative statement: after the quadrichotomy theorem has
put the affine Kac--Moody MC element into Lie/tree normal form, no
new independent higher shadow remains beyond the cubic Lie tensor.
\end{proof}

\begin{remark}[The collision kernel]\label{rem:r-matrix}
The trace-form collision kernel (binary genus-$0$ component)
of \(V_k(\fg)\) is
\begin{equation}\label{eq:km-rmatrix}
K_{k,\fg}^{\mathrm{coll}}(z) \;=\; k\,\Omega_{\mathrm{tr}}/z.
\end{equation}
The KZ conformal-block connection uses the separate Sugawara-normalised
kernel
\[
r^{\mathrm{KZ}}(z)=\Omega_{\mathrm{KZ}}/((k+h^\vee)z).
\]
At critical level the KZ coefficient is undefined, while the
trace-form collision residue remains finite. The comparison between
these two kernels is a normalization bridge, not equality as rational
functions of \(k\).
\end{remark}
...
\item \emph{$r$-matrix}:
 ...
 \(r^{\mathrm{coll}}(z)=k\,\Omega_{\mathrm{tr}}/z\). The KZ kernel is
 \(r^{\mathrm{KZ}}(z)=\Omega_{\mathrm{KZ}}/((k+2)z)\).
...
\item \emph{Bar cohomology at low degree}:
 ...
 The computed initial segment of the generating function is
 \begin{equation}\label{eq:sl3-gf}
 P_{\widehat{\fsl}_3}(x)
 \;=\; 8x + 36 x^2 + O(x^3).
 \end{equation}
 A closed form and the full asymptotic remain conjectural.
\item \emph{Shadow class}: $L$, $r_{\max} = 3$.
 The cubic Lie/tree shadow is \(\mathfrak C(x,y,z)=(x,[y,z])\);
 it is governed by the structure constants \(f^{abc}\).
\item \emph{$r$-matrix}:
 \(r^{\mathrm{coll}}(z)=k\,\Omega_{\mathrm{tr}}/z\). The KZ kernel is
 \(r^{\mathrm{KZ}}(z)=\Omega_{\mathrm{KZ}}/((k+3)z)\).
...
\begin{enumerate}[(i)]
\item The curvature vanishes: \(m_0=0\)
 \textup{(}Proposition~\textup{\ref{prop:bar-curvature}}
 with $k + h^\vee = 0$\textup{)}.
 Thus the induced flat first relation is \(m_1^2=0\), and the bar
 complex is a strict dg coalgebra \textup{(}uncurved\textup{)}.
\item The modular characteristic vanishes:
 $\kappa(\ghat_{-h^\vee}) = 0$. The genus-$1$
 pointed trace
 $\operatorname{tr}_1\operatorname{Obs}^{\mathrm{def}}_{1,1}=0$, and the
...
\item Under the local completed bar--Ext comparison, the critical
 completed bar--Ext groups identify with de Rham forms on the local
 oper space:
 \[
 H^n_{\mathrm{bExt}}\;\cong\;
 \Omega^n(\mathrm{Op}_{\fg^\vee}(D)).
 \]
 This is not a statement about raw bar cohomology.
\end{enumerate}
\end{proposition}
...
Item~(v) is the completed bar--Ext comparison combined with the
fundamental local equivalence at the critical level, which identifies
the relevant completed critical category with the Whittaker category
over \(\mathrm{Op}_{\fg^\vee}(D)\).
\end{proof}

\begin{remark}[Critical level and level reflection]
\label{rem:critical-self-dual}
The critical level is the unique self-dual point of
the Verdier level reflection:
$(-h^\vee)' = -(-h^\vee) - 2h^\vee = -h^\vee$.
Full Koszulness fails at this point; self-duality is only the scalar
level reflection, not an identification of the raw critical vertex
algebra with its strict Koszul dual.
...
\label{rem:langlands}
The critical-level completed bar--Ext complex lies on the oper side of
local geometric Langlands. Feigin--Frenkel identifies
\[
\mathfrak z(\widehat{\fg})\cong \mathrm{Fun}(\mathrm{Op}_{\fg^\vee}(D)).
\]
Together with the completed bar--Ext comparison and Raskin's local
equivalence, this identifies critical-level completed bar--Ext groups
with de Rham forms on the local oper space. The global curve \(X\)
replaces the formal disc by the Ran-space factorisation algebra; this
is the local input, not by itself the global Langlands kernel.
\end{remark}
...
\section{The affine Kac--Moody point in the Koszul landscape}
\label{sec:programme}

The affine Kac--Moody calculation occupies the class-\(L\) vertex of
the modular Koszul landscape. The PBW universality criterion identifies
\(V_k(\fg)\) as chirally Koszul because it is freely strongly generated
and its associated graded is classically Koszul. Verdier duality gives
the Koszul dual
\[
(V_k(\fg))^! \simeq \CE^{\mathrm{ch}}(V_{-k-2h^\vee}(\fg)).
\]
The genus-one obstruction coefficient is
\[
\kappa(\widehat{\fg}_k)=\frac{\dim\fg\,(k+h^\vee)}{2h^\vee},
\qquad
\kappa(\widehat{\fg}_k)+\kappa(\widehat{\fg}_{-k-2h^\vee})=0.
\]
The cubic shadow is the invariant form \((x,[y,z])\), and Jacobi
annihilates the quartic obstruction; hence the shadow depth is
\(r_{\max}=3\).
...
\item The modular characteristic formula
 ...
 free-field families. For $\mathcal{W}$-algebras the
 scalar complementarity is nonzero and is computed by the
 family-specific conductor normalization, not by a universal
 affine ratio.
...
\item The trace-form collision residue \(r^{\mathrm{coll}}(z)
 = k\Omega_{\mathrm{tr}}/z\)
 (Remark~\ref{rem:r-matrix}) is the binary genus-$0$
 projection of the universal Maurer--Cartan element
 \(\Theta_{\ghat_k}\). The KZ coefficient
 \(\Omega_{\mathrm{KZ}}/((k+h^\vee)z)\) belongs to the
 conformal-block normalization. The affine modular characteristic is
 recovered from the double-pole channel together with the Sugawara
 shift:
 \[
 \kappa_{\mathrm{dp}}+\dim(\fg)/2=\kappa(\ghat_k).
 \]
\end{enumerate}
