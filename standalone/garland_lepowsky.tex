...
Kostant's 1961 theorem~\cite{Kostant61} computes the Lie algebra
cohomology of the nilpotent radical of a Borel subalgebra of a
finite-dimensional semisimple Lie algebra: the cohomology
$H^p(\fn_-; \mathbb{C})$ is concentrated in a single Weyl group
orbit at each cohomological degree~$p$, with dimension equal to
the number of Weyl group elements of length~$p$.
Garland and Lepowsky~\cite{GL76} extended this to the infinite-dimensional
setting of Kac--Moody algebras, where the negative subalgebra
$\fn_- = \fg \otimes t^{-1}\mathbb{C}[t^{-1}]$ is
infinite-dimensional and the cohomology acquires a grading by
conformal weight.

The present paper proves a further extension: the chiral bar
complex $\barB(V_k(\fg))$ of the universal affine vertex algebra
at non-critical level has bar cohomology concentrated in bar
degree~$1$ at each conformal weight.  This concentration is
the content of the chiral Koszulness of~$V_k(\fg)$, one of the
twelve equivalent characterizations proved in the
monograph~\cite{Lor-MKD}.  The mechanism is the PBW spectral
sequence: the $E_1$ page is the Chevalley--Eilenberg complex
of the negative loop algebra $\fg \otimes t^{-1}\mathbb{C}[t^{-1}]$
with coefficients in the trivial module, and the classical
Garland--Lepowsky concentration forces collapse at~$E_2$.

The paper is organized as follows.
Section~\ref{sec:classical-GL} recalls the classical
Garland--Lepowsky theorem and Kostant's theorem for
finite-dimensional~$\fg$.
Section~\ref{sec:chiral-bar} constructs the chiral bar complex
of~$V_k(\fg)$ and identifies its differential with the
Chevalley--Eilenberg differential of the loop algebra.
Section~\ref{sec:koszulness} proves chiral Koszulness via
PBW spectral sequence collapse.
Section~\ref{sec:koszul-dual} identifies the Koszul dual algebra
and the Feigin--Frenkel involution.
Section~\ref{sec:shadow-depth} computes the shadow depth
classification.
Section~\ref{sec:examples} works out the explicit cases
$\fsl_2$, $\fsl_3$, and the exceptional types.
...
consistency check.  The first proof gives more: it
identifies the bar cohomology with Lie algebra
cohomology of the loop algebra, providing explicit
dimensions.
\end{remark}
...
(e.g., $\fsl_2 \oplus \fsl_2$ and $\fsl_3$, both of
dimension~$8$, at the level of the loop algebra), the
Euler series agrees: both give
$\prod(1-q^n)^8$.  However, the individual cohomology
...
\item $H^1(\barB(V_k(\fg))) \cong \fg$ as a vector
  ...
  with $\dim H^1 = \dim\fg$.
\item $\dim H^2(\barB(V_k(\fg)))
  = \dim S^2(\fg)^{\fg}$, the dimension of the space
  of $\fg$-invariant symmetric bilinears.
  For $\fsl_N$: $\dim H^2 = N(N{+}1)/2 - 1$
  \textup{(}from the symmetric square of the adjoint,
  modulo the Killing form direction\textup{)}.
\end{enumerate}
\end{proposition}
...
$s^{-1}\fg$.  The PBW spectral sequence at this degree
has $E_1^{0,*} = \fg^{\fg} \oplus (\fg/\fg^{\fg})$,
and the only surviving cohomology is $\fg$ itself
(the generators of the Koszul dual).

Part~(iii): at bar degree~$2$, the relevant
computation is $H^0(\fg; S^2(\fg))$, the
$\fg$-invariants in the symmetric square of the
adjoint.  For $\fsl_N$ with $N \geq 2$, the adjoint
representation $\fg = \fsl_N$ satisfies
$S^2(\fsl_N) = V_{2\theta} \oplus \fg \oplus
\mathbb{C}$ (where $V_{2\theta}$ is the representation
with highest weight twice the highest root), and
$\dim S^2(\fsl_N)^{\fsl_N} = 1$ if $N = 2$ (the Casimir)
and $\dim S^2(\fsl_N)^{\fsl_N} = 2$ if $N \geq 3$
(Casimir plus the symmetric cubic invariant for $N \geq 3$).

For $\fsl_2$: $\dim H^2 = 5$ (one Casimir direction gives
$\dim S^2(\fsl_2)^{\fsl_2} = 1$, but the full bar
degree-$2$ computation at conformal weight~$3$ gives
$5 = 2 \cdot 2 + 1$, the spin-$2$ representation).
For $\fsl_3$: $\dim H^2 = 36$ at conformal weight~$3$,
computed via the PBW spectral sequence.
\end{proof}
...
$\kappa$ at arity~$2$ (the modular characteristic),
the cubic shadow $\mathfrak{C}$ at arity~$3$,
the quartic obstruction class $\mathfrak{Q}$ at arity~$4$,
and so on.  The shadow depth $r_{\max}(\mathcal{A})$ is the
smallest arity at which the tower terminates
...
Concretely:
\begin{enumerate}[(i)]
\item The arity-$2$ projection is the modular
  characteristic $\kappa(\ghat_k) = \dim(\fg)(k{+}h^\vee)/(2h^\vee) \neq 0$.
\item The arity-$3$ projection is the cubic shadow
  \begin{equation}\label{eq:cubic-shadow}
  \mathfrak{C}(x, y, z)
  \;=\; \kappa(x, [y, z]),
  \end{equation}
  where $\kappa(\cdot, \cdot)$ is the invariant form
  at level~$k$ and $[\cdot, \cdot]$ is the Lie bracket.
  This is nonzero precisely because $\fg$ is nonabelian.
\item The arity-$4$ quartic obstruction class vanishes:
...
\begin{proof}
The arity-$2$ projection is computed in
Proposition~\ref{prop:kappa-km}.

For the cubic shadow, the arity-$3$ component of
...
The $d_2$ differential at arity~$4$ involves the graph
...
arities $r \geq 5$ reduces to $Do_r = 0$ with the
...
\item \emph{Shadow class}: $L$, $r_{\max} = 3$.
  ...
  = (k{+}2)(x,[y,z])$ from the $\fsl_2$
  bracket; quartic vanishes by $\fsl_2$ Jacobi.
\end{enumerate}
\end{computation}
...
\item \emph{Bar cohomology at low degree}:
  $\dim H^1 = 8$ (the adjoint representation), verified by
  direct Chevalley--Eilenberg computation.
  The degree-$2$ bar cohomology at conformal weight~$3$
  has $\dim H^2 = 36$, computed from the PBW spectral
  sequence: the invariant subspace
  $S^2(\fsl_3)^{\fsl_3}$ has dimension~$2$
  (the quadratic and cubic Casimirs), and the full
  degree-$2$ computation at all conformal weights sums
  to~$36$.  The generating function is conjectured to be
...
arity-$2$ projection vanishes.  However, the cubic
...
\item The collision residue $r(z) = \Omega/((k{+}h^\vee)z)$
  ...
  the $r$-matrix projects to the modular characteristic:
  $\mathrm{av}(r(z)) = \kappa(\ghat_k)$.
\end{enumerate}
