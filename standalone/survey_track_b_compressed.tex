...
$\cH_k^! = (\Sym^{\ch}(V^*[1]), m_0 = -k\omega)$, not a flat VOA.
...
$r$-matrix is $r_{\cl}(z) = k\,\Omega_{\mathrm{tr}}/z$ with
$\Omega_{\mathrm{tr}} = \sum_a J^a \otimes J_a$; it vanishes identically at $k = 0$, as it
must. The CYBE is the Arnold relation on $\FM_3(\bC)$
evaluated on the Casimir. The Koszul dual is the
chiral Chevalley--Eilenberg object at the Feigin--Frenkel dual level:
$\widehat\fg_k^!
\simeq \mathrm{CE}^{\mathrm{ch}}(\widehat\fg_{-k-2h^\vee})$.
Its scalar characteristic implements the involution
...
modular Koszul datum packages the algebraic comparison surface into
a seven-entry object: boundary algebra, presentation coalgebra,
strict Koszul dual, derived closed sector,
spectral $R$-matrix,
universal MC element, and modular shadow connection. The line
category is recovered from the strict dual on the line-comparison
surface, while the derived centre occupies the closed-sector entry.
The full-bar Verdier algebra
$K_X(\cA)=\mathbb D_{\Ran}\bar B_X(\cA)$ is the typed comparison
object between the presentation coalgebra and the strict dual. Every ingredient
constructed so far (the bar complex, the shadow tower, the
Swiss-cheese structure, the PVA descent, the genus expansion) is a
typed projection of this datum.
...
$H^\bullet(q_\cA)$ identifies presentation-coalgebra cohomology with
bar cohomology.
\item Verdier duality forms
$K_X(\cA)=\mathbb D_{\Ran}\barB_X(\cA)$, and a chosen pair map
$\nu_\cA\colon K_X(\cA)\to\cA^!$ supplies the strict partner.
Its module category becomes the line-operator category under the
line-comparison theorem.
...
Virasoro element promotes HT to a topological theory only when it is
inner in the quantised BRST complex, $T=[Q_{\mathrm{tot}},G]$, with
analytic SDR, all-loop QME, and image-choice/Stokes control. The
boundary carries a $(-1)$-shifted Poisson bracket and generates the
genus-$0$ quantisation problem. The conditional modular lane couples the
bulk theory to stable curves at $g \ge 1$. The pipeline:
\[
...
$\nu_{M2}\colon K_X(\cA)\to\cA^!$ are part of the M2 comparison
package.
...
algebraic comparison surface into a seven-entry MC-controlled tuple;
the physical holographic correspondence additionally requires the
...
$\mathfrak O_g^K=\kappa\lambda_{-1}(\mathbb E_g)$ and its signed
Hodge character, their $H_D^{\mathrm{tr}}$ comparison, and the
$H_D^{\mathrm{graph}}$ functional
