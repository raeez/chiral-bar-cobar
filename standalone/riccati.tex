...
\let\ric@oldref\ref
\renewcommand{\ref}[1]{%
 \@ifundefined{r@#1}{\textit{full monograph}}{\ric@oldref{#1}}%
}
\makeatother

\title{The shadow metric, the Riccati algebraicity theorem, and the depth classification}
\author{Raeez Lorgat}
\date{}
...
The Virasoro spectral R-matrix
$R(z) = z^{2h}\exp(-(c/4)/z^2)$ and its connection to the Riccati
shadow structure are developed in the companion paper \emph{The
Virasoro spectral R-matrix} (\texttt{virasoro\_r\_matrix.tex}).
\end{remark}
...
Let $\mathcal{A}$ be a chirally Koszul vertex algebra with modular
...
The complex carries a degree filtration $F^{\bullet}$ with
...
\begin{remark}
The shadow metric $Q_L$ is a quadratic polynomial in~$t$,
determined by three OPE invariants: $\kappa$, $\alpha$, $S_4$.
The term ``metric'' refers to $Q_L$'s role as the quadratic form
governing the tower's geometry (branch points, monodromy,
convergence radius), not to a Riemannian metric.
\end{remark}

\subsection{The recursion on a primary line}

On a primary line $L$, the all-degree master equation
...
the MC equation at degree $r \geq 5$ is
\begin{equation}
...
is determined by $\{S_2, S_3, S_4, \ldots, S_{r-1}\}$.
The degree-$4$ shadow $S_4$ is an intrinsic OPE invariant (the quartic
contact term in the four-point collision residue), not determined by
the recursion; it serves as initial data alongside $\kappa = S_2$
and $\alpha = S_3$.
\end{proposition}
...
$\mathrm{Sh}_k = S_k x^k$ to produce a term of degree $j + k - 2$
with coefficient $jk\,P\,S_j S_k$. The eigenvalue identity
$\nabla_H(x^r) = 2r \, x^r$ separates the MC equation at
degree $r$ into the linear term $2r \, S_r$ on the left and the
...
Then the all-degree master equation on $L$ is equivalent to the
algebraic relation
\begin{equation}
...
\eqref{eq:single-line-recursion} at degree $r = m + 2$.
...
the full MC recursion is therefore established at all degrees.
...
at degrees $2$, $3$, $4$ only. All shadow coefficients $S_r$
...
\item The flat section with $\Phi(0) = 1$ is
 ...
 the curvature along the degree parameter, weighted by $t^2$.
\end{enumerate}
\end{proposition}
...
reflecting the degree offset (the tower starts at degree $2$,
not degree $0$).
\end{remark}
...
of chirally Koszul algebras includes a fourth class $\mathbf{C}$
...
\cite[Theorem~VI.1]{FlajoletSedgewick} gives
...
The shadow depth of a chirally Koszul algebra $\mathcal{A}$
decomposes as
\begin{equation}
...
is $1$ (the curvature $\kappa$ at degree $2$), the maximum
...
contributes a functional-equation centre at $\operatorname{Re}(s)=k/2$;
Maass cusp-form projections contribute no new constituent centres.
\end{proof}
...
Affine $\widehat{\mathfrak{g}}_k$
 ...
 & $0$ & $\mathbf{L}$ & $0$ \\
$\beta\gamma$ (weight $\lambda$) & $6\lambda^2{-}6\lambda{+}1$ & $0^*$ & $0^*$ & $0^*$
 & $\mathbf{C}$ & $0$ \\
...
\vspace{2pt}
\noindent
${}^*$\,The table reports the weight-changing primary line,
where $\kappa = S_3 = S_4 = 0$. The quartic contact
invariant $Q^{\mathrm{contact}} \neq 0$ lives on this
charged stratum; the shadow metric requires $\kappa \neq 0$
and does not apply. The class~$\mathbf{C}$ designation
($r_{\max} = 4$) comes from rank-one rigidity, not the
single-line dichotomy. The T-line (stress tensor) carries
the Virasoro shadow data $(\kappa, \alpha, S_4) = (c/2, 2,
10/(c(5c{+}22)))$ with $c = 2(6\lambda^2{-}6\lambda{+}1)$
and is class~$\mathbf{M}$.
...
\begin{remark}[Large-$c$ asymptotic of $S_4$]
\label{rem:s4-large-c-asymptotic}
The Virasoro quartic shadow
\[
...
\[
 S_4(\mathrm{Vir}_c)
 \;=\; \frac{10}{5c^2}\Bigl(1 - \tfrac{22}{5c}
  + O(c^{-2})\Bigr)
 \;=\; \frac{2}{c^2} + O(c^{-3}),
\]
\emph{not} $2/(5c^2)$; the factor of $5$ in the denominator of
$10/[c(5c+22)]$ is absorbed when expanding $1/(5c+22)$ at
large~$c$.  Pairing this with $\kappa = c/2$ gives the
critical discriminant
$\Delta = 8\kappa S_4 = 40/(5c+22) \to 8/c$ as $c \to \infty$,
confirming class~$\mathbf{M}$ mixing persists to leading order.

The identity $S_2(\mathrm{Vir}_c) = \kappa = c/2$ is
convention-independent.  The occasional error
$S_2 = c/12$ conflates the shadow invariant with the Vol II
lambda-bracket divided-power coefficient $c/12 = (c/2)/3!$,
which is a different normalisation; the Vol I--II bridge is
$S_2^{\mathrm{Vol\,I}} = 6\,S_2^{\lambda}$.

The Riccati seed on $\mathrm{Vir}_c$ is
$(r_0, S_{r_0}) = (3, 2)$: $S_3(\mathrm{Vir}_c) = 2$ is the
$c$-independent cubic shadow (Lorgat, \textit{The Virasoro
spectral R-matrix}), and the shadow-exponential base
$C_A = 6$ on the non-logarithmic class~$\mathbf{M}$ locus
factorises as $6 = r_0 \cdot S_{r_0} = 3 \cdot 2$.
\end{remark}
