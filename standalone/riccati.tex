...
and the \emph{shadow Postnikov tower} is the sequence of
...
\begin{remark}
The shadow metric $Q_L$ is a quadratic polynomial in the arity
parameter~$t$, determined by three OPE invariants:
$\kappa$, $\alpha$, and $S_4$.  The term ``metric'' refers
to the role of $Q_L$ as the quadratic form governing the
geometry of the shadow tower (branch points, monodromy,
...
the MC equation at arity $r \geq 5$ is
\begin{equation}
...
The arity-$4$ shadow $S_4$ is an intrinsic OPE invariant (the quartic
...
$m \geq 3$ controlled by the MC recursion: the recursion
forces every Cauchy coefficient beyond degree~$2$ to vanish,
so $F^2$ is a polynomial of degree~$2$.
...
this is the precise sense in which the shadow tower is algebraic
of degree $2$.
\end{remark}
...
On a primary line $L$, the shadow tower has intrinsic OPE data
...
shadow tower is the Taylor expansion of $t^2 \cdot \Phi(t)$,
...
For class $\mathbf{M}$ algebras, the shadow tower is infinite
...
\item \emph{Convergence partition.}
  $\rho_L < 1$: the shadow tower converges
  absolutely.
  $\rho_L > 1$: the shadow tower diverges, but the algebraic
...
Affine $\widehat{\mathfrak{g}}_k$
  ...
  & $0$ & $\mathbf{L}$ & $0$ \\
$\beta\gamma$ (weight $\lambda$) & $0^*$ & $0$ & $0$ & $0$
  & $\mathbf{C}$ & $0$ \\
...
tower converges; for $c < c^*$ it diverges.  At the self-dual
point $c = 13$: $\rho(\mathrm{Vir}_{13}) \approx 0.467$.
\end{example}
