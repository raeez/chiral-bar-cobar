...
\begin{abstract}
The problem is the bar construction of a chiral algebra on a curve.
Over a point it is classical. On an algebraic curve of genus~$g\ge 1$
the propagator acquires curvature $\kappa(\cA)\cdot\omega_g$, the bar
differential ceases to square to zero in the fibre direction, and the
classical construction stalls. The programme resolves the stall: a
single universal Maurer--Cartan element $\Theta_\cA$ in the modular
convolution algebra controls the genus tower; five theorems
describe its projections. Theorem~A records bar--cobar adjunction
with Verdier intertwining; Theorem~B, the inversion
$\Omega\barB(\cA)\xrightarrow{\sim}\cA$ on the Koszul locus;
	Theorem~C, complementarity with conditional shifted-symplectic Lagrangian
geometry; Theorem~D, the native deformation class, its pointed trace,
its perfect $K$-theory realization and signed Hodge character, and the
stable-graph functional on their named packages; Theorem~H,
family-dependent finite support
$\operatorname{Supp}\ChirHoch^\bullet(\cA)\subseteq S$ from a
strong deformation retract supplied by $H_H(\cA;S)$. The ordered bar
$\barB^{\mathrm{ord}}(\cA)=T^c(s^{-1}\bar\cA)$ is the primitive
$E_1$-coalgebra; the symmetric bar $\barB^{\Sigma}(\cA)$ is its
$\Sigma_n$-coinvariant shadow. The averaging map
$\mathrm{av}\colon\mathfrak{g}^{E_1}_\cA\twoheadrightarrow
\mathfrak{g}^{\mathrm{mod}}_\cA$ is lossy; at degree~$2$ on
non-abelian affine Kac--Moody the full modular characteristic adds
the Sugawara shift $\dim(\fg)/2$. The shadow obstruction tower
partitions the standard landscape into four depth classes
(G/L/C/M); the universal class-M coefficient-growth constant is
$C_\cA=6$.
...
$(\infty,2)$-equivalence at properad level. The scope is the
non-logarithmic $C_2$-cofinite standard landscape plus
irrational cosets: $W(p)$ triplet tempering is open pending an
Adamovi\'c--Milas character amplitude bound; the CY-C pentagon
stratifies generator rank $\rho^{R_i}$ rather than $\kappa_{\ch}$
(Hodge supertrace route-independent); super-Yangian
complementarity splits by normalisation:
$\kappa^{\str}+\kappa^{\str,!}=0$ is proved via super-Sugawara
rational-identity cancellation, while the Berezinian form
$\kappa^{\sBer}+\kappa^{\sBer,!}=\max(m,n)$ is conjectural at
Sugawara-shifted dual level, conditional on a closed-form
derivation of the $\tfrac{1}{2}\max(m,n)$ shift magnitude
induced by the central automorphism $\sigma_{\sBer}$
(Nazarov~1991 + Gow~2006 supply centrality and non-degeneracy,
not magnitude); the primes $\{1423, 3067, 23, 43,
419\}$ are not Kummer-irregular at the primary source and survive
only as Riccati-arithmetic characteristic primes of the shadow
tower.

The programme spans three volumes with ${\sim}4{,}800$ pages and
$\sim 177{,}000$ computational tests across $\sim 4{,}200$ engines.
Residual open frontiers: chain-level class~M on the original
complex (versus the weight-completed category, where it is
proved); the exact closed form of $\beta_N$ at $N\ge 4$;
super-complementarity canonical Verdier pairing inscription;
and $W(p)$ triplet tempering.
\end{abstract}
...
strata~\cite{FultonMacPherson94}. Points of
...
normal factor along \(D_{ij}\).  After restriction to that divisor there
is no second normal direction for the same diagonal; the desuspension
sign only records the ordered-bar orientation.
\end{enumerate}
...
In the untwisted finite-arity window, the $\Sigma_n$-coinvariant quotient
...
$E_\infty$-coalgebra.
With nontrivial braiding the symmetric bar is instead the
...
and conilpotent completion.

The ordered bar is the level-$2$ chain object before averaging. The symmetric bar
...
deformation. The part not represented in the descended symmetric
target is the formal ordered information-loss conductor, not an
informal metaphor for ignorance.
...
$\overline{\cM}_g$, where the fiberwise differential becomes curved
with $m_1^2=[m_0^{(g)},-]$ and scalar diagonal
...
$L_R$-twisted completed $\Sigma_n$-coinvariant image in the braided
case, and its ordinary $\Sigma_n$-coinvariant image in the untwisted
case. No two of these six objects
...
Under the completed Eilenberg--Zilber comparison, the bar construction
is comonoidal:
$B(\cA \otimes \cA') \simeq B(\cA) \otimes B(\cA')$.
The form $\eta_{ij}=d\log(z_i-z_j)$ is a local logarithmic generator
with residue~$1$; Arnold's relation controls the codimension-two
boundary compatibility of its residue contractions.
...
\mathbb{D}_{\Ran}(B(\cA))
 &=& K_X(\cA)
 & \text{(Verdier duality: the homotopy Koszul-dual algebra)},
 \\[3pt]
\nu_\cA\colon K_X(\cA)&\longrightarrow&\cA^!
 & \text{(chosen Koszul-pair comparison)}.
\end{array}
\end{equation}
Cobar inversion recovers the original algebra~$\cA$. Verdier duality
converts the bar coalgebra into the algebra
$K_X(\cA):=\mathbb D_{\Ran}B(\cA)$, retaining the full chain-level
data.  For a quadratic presentation, the transpose
$\mathbb D(q_\cA)\colon K_X(\cA)\to
\mathbb D_{\Ran}(\cA^{\mathrm i})$ compares the full bar with the
presentation dual.  A chosen Koszul pair supplies
$\nu_\cA\colon K_X(\cA)\to\cA^!$.  Thus
\eqref{eq:three-functors} records three typed operations.
...
differential becomes curved with scalar diagonal projection
...
connection absorbs the projected fibrewise curvature, and the total
differential satisfies $D_g^2 = 0$.

Genus~$0$ is uncurved algebra. Genus $\ge 1$ is curved
geometry. On the uniform-weight scalar diagonal, the visible genus
tower is controlled by a single scalar: the projected curvature of the
bar complex on a family of curves.
...
Arnold relation fails. The fibrewise bar differential becomes curved
with scalar diagonal projection
...
the Pantev--To\"en--Vaqui\'e--Vezzosi~\cite{PTVV13}
shifted-symplectic package, cyclic closedness, skew-adjointness,
modular-bracket compatibility, twisted-tensor acyclicity, and the
stated BV/QME anomaly-killing inputs.
...
For each family datum \(H_H(\cA;S)\), where
\(S\subset\mathbb Z_{\ge0}\) is finite, a strong deformation retract
identifies the complete chart complex
\(C^\bullet_{\mathrm{ch}}(\cA,\cA)\) with a model
\(K_{\cA,S}\) supported in~\(S\).  Hence
\[
\ChirHoch^n(\cA)\cong H^n(K_{\cA,S}),\qquad
\operatorname{Supp}\ChirHoch^\bullet(\cA)\subseteq S.
\]
The BDSK superboson bounded calculation has support
\(\{0,1\}\) and dimensions \((2,1)\).  A Virasoro bounded model has
support \(\{0,2,3\}\), with one-dimensional groups in those degrees;
its chart statement follows from a named bounded-to-chart
quasi-isomorphism.  A perfect degree-\(d\) pairing is an additional
duality package.
\end{theorem}
...
the open--closed comparison datum. Theorem~H
constrains its cohomology.  Under the Virasoro bounded-to-chart
quasi-isomorphism just stated, its support is \(\{0,2,3\}\) and each
supported group is one-dimensional.
...
Koszulness: for a chosen quadratic presentation, the comparison
$q_\cA\colon\cA^{\mathrm i}\to B_X(\cA)$ is a quasi-isomorphism.
The meta-theorem organises this condition by
...
under the family package $H_H(\cA;S)$: a complete chart model, a
finite support model~$K_{\cA,S}$, a strong deformation retract,
filtration convergence, and ordered-to-symmetric comparison.  A bar
presentation additionally carries
$\operatorname{Conv}(B_X(\cA),\cA)\xrightarrow{\sim}
...
\item Under $H_H(\cA;S)$,
 ...
 for the declared finite family support set~$S$.
...
\End_\cA(2)[\![z^{-1}]\!]\,.
\end{equation}
The collision residue is the degree-$2$, genus-$0$ projection
...
$1$ & Kac--Moody $\widehat{\fg}_k$ & $k\,\Omega_{\mathrm{tr}}/z$ & $\mathbf{L}$ \\
...
\end{equation}
while for non-abelian affine Kac--Moody in the trace-form
convention
\[
\mathrm{av}(k\,\Omega_{\mathrm{tr}}/z)
\;=\;
...
\textbf{Physics} & Closed-sector operators & Boundary operators \\
\bottomrule
\end{tabular}
\end{center}
...
closed-sector operators act on the boundary, while physical-bulk
action requires an open--closed comparison.

The directionality of the Swiss-cheese operad is strict:
\emph{no open inputs produce closed outputs}. Closed-sector operators
restrict to boundary operators through the Swiss-cheese action; a
physical-bulk restriction exists only after OCA data. The open
...
primitive; the symmetric bar is its lossy coinvariant shadow, and
the Francis--Gaitsgory bar is the zeroth-pole associated-graded
shadow inside the symmetric bar, not a second name for either
of them.
...
chiral derived centre computes the universal closed sector; bar-cobar
...
algebra~$\cA$ is the seven-entry package
...
\end{equation}
where a chosen presentation $\cA=T_X(V)/(R)$ supplies
$\cA^{\mathrm i}=C_X(s^{-1}V,s^{-2}R)$ and
$q_\cA\colon\cA^{\mathrm i}\to B_X(\cA)$.
The full-bar Verdier algebra is
$K_X(\cA)=\mathbb D_{\Ran}B_X(\cA)$, with
$\mathbb D(q_\cA)$ and a chosen pair map
$\nu_\cA\colon K_X(\cA)\to\cA^!$ on the finite-type or completed
continuous surface.
$Z^{\mathrm{der}}_{\mathrm{ch}}(\cA)$ is the algebraic closed
sector, $K_\cA^{\mathrm{coll}}(z)$ is the collision kernel,
$\Theta_\cA$ is the universal MC element, and
$\nabla^{\mathrm{hol}}_{g,n} = d -
\mathrm{Sh}_{g,n}(\Theta_\cA)$ is the modular shadow
connection. The line-operator category
$\cA^!\text{-}\mathsf{mod}$ is attached to the third entry after
the line comparison; a physical bulk requires the open--closed
comparison map.
\end{definition}

The datum $\cH(\cA)$ is reconstructed from the boundary OPE
on the named comparison surface: $\cA$ supplies $B(\cA)$,
$\cA^{\mathrm i}$, the Verdier/Koszul dual, the chiral derived
centre, and the MC residue data.  Its hypothesis package comprises
finite-type or completed duality, the line comparison, and the
...
\item \emph{Gravitational theories} (classes~$\mathbf{C}$,
 $\mathbf{M}$): the shadow tower extends to degree~$4$
...
\item The $\mathbb{S}^d$-framing on categorical Hochschild homology
...
For $d = 3$, the functor is now proved ($\infty$-categorical
argument: $\mathrm{HH}^{-2}_{E_1} = 0$ implies the $E_3$-lifting
is contractible). The passage from $d = 2$ to $d = 3$
...
On toric $GL(d)$-invariant local charts the relevant
Schouten--Nijenhuis bracket on polyvector fields of $\Bbbk^d$
vanishes in the scalar channel; general CY categories are
$E_1$ by the two-stage specialization and degree bookkeeping,
not by Lie-level abelianity.
...
identity. The Heisenberg-level modular characteristic of the chiral
de Rham complex is $\kappa_{\mathrm{ch}}^{\mathrm{Heis}}(K3 \times E) = 3
= \dim_\Bbbk$, verified by six independent paths (additivity
under Cartesian product: $2 + 1$; distinct from the total-space
Hodge supertrace $\Xi(K3\times E)=\chi(\mathcal{O}_{K3\times E})$,
which is K\"unneth-multiplicative and vanishes for $K3 \times E$; see Volume~III
Remark~\ref{V3-rem:beauville-kappa-formula-subscript-split} for the
Route~A / Route~B adjudication). The BKM
...
\;=\; \{0, 3, 5, 24\}
\end{equation}
records four total-space and fibre-normalized values:
$\kappa_{\mathrm{cat}}=\chi(\mathcal{O}_{K3\times E})=0$,
$\kappa_{\mathrm{ch}}^{\mathrm{Heis}}=3$ (the complex dimension,
Route~B), $\kappa_{\mathrm{BKM}}=5$ (the Borcherds weight), and
$\kappa_{\mathrm{fiber}}=24$ (the K3 lattice rank). The value
$\chi(\mathcal{O}_{K3})=2$ belongs to the K3 fibre lane; it is not
...
\item \emph{Categorical}: the shadow tower produces Taylor
 ...
 on $\cH_2$.
\item \emph{$\kappa_\bullet$-mismatch}:
 $\kappa_{\mathrm{ch}}^{\mathrm{Heis}} = 3 \ne 5 =
 \kappa_{\mathrm{BKM}}$.
\item \emph{Second quantization}: the physical DT partition
 function is the OP-reduced $K3\times E$ invariant, with the
 elliptic translation factor removed, and is the DMVV symmetric
 product of single-copy data; the bar complex is the single copy.
...
$\kappa_\bullet$-spectrum is the first invariant that separates CY
...
critical CoHA $\mathrm{CoHA}(Q_X)$, identified with the positive
half $Y^+(\widehat{\fg}_{Q_X})$ of an affine super Yangian. For
$\Bbbk^3$, the Jordan quiver gives
$\mathrm{CoHA}(\Bbbk^3)\simeq Y^+(\widehat{\mathfrak{gl}}_1)$;
the full $Y(\widehat{\mathfrak{gl}}_1)\simeq \cW_{1+\infty}$
appears only after the Drinfeld double or the corresponding
...
$\cW_N$. The collision kernel $K_\cA^{\mathrm{coll}}(z)$ is scalar for
...
generation~\cite{FF92,FBZ04}). For simple quotients $L_k(\fg)$ at
admissible levels, even the rank-one $\mathfrak{sl}_2$ lane remains
conditional on the quotient-bar spectral sequence and convergence
package; higher rank carries the Cartan obstruction described in the
main text.
...
closed-sector algebra (chiral derived centre), the collision
...
% ====================================================================
\section{The organizing ambient problem}
...
on a curve. The three-dimensional extension is governed by one
organizing ambient problem, but several proof obligations remain
separate.

\begin{theorem}[CY-A at $d = 3$]\label{thm:cy-a3}
The Calabi--Yau/$A_\infty$ correspondence extends from
$d = 2$ to $d = 3$: the $\infty$-categorical obstruction
$\mathrm{HH}^{-2}_{E_1}(\mathcal{C}) = 0$ (for smooth proper
CY$_3$ categories) implies that the $E_3$-lifting of the
$S^3$-framing is contractible, producing an
$A_\infty$-structure on the boundary algebra compatible
with the chiral bar complex.
\end{theorem}

At $d = 2$, the CY-A correspondence was the first proved
case: the Fukaya category of a symplectic surface carries an
...
At $d = 3$, the $\infty$-categorical proof resolves the
chain-level compatibility of the $S^3$-framing with
the factorization structure on the $\mathbb{C}$-direction
by showing that the relevant obstruction group vanishes.

The two-dimensional core is proved in the stated Koszul,
coderived, and weight-completed ambients. The remaining obligations
include the MC5 partial boundary scope, the DK-4/DK-5 residue, the
chain-level BV/BRST--bar comparison off the completed lane, and the
projected higher-dimensional conjectures.
...
CY-A at $d = 3$ was the last unresolved input; its proof
completes the algebraic foundation.
