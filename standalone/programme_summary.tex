...
Theorem~C, complementarity with shifted-symplectic Lagrangian
...
theorem, the SC$^{\mathrm{ch,top}}$ heptagon, the CY-D
dimension stratification, and the Francis--Gaitsgory
$(\infty,2)$-equivalence at properad level. The 2026-04-17
Beilinson-rectified audit scope-qualifies the programme climax to
the non-logarithmic $C_2$-cofinite standard landscape plus
...
$\kappa^{\sBer}+\kappa^{\sBer,!}=\max(m,n)$ is CONJECTURED at
...
419\}$ previously labelled Kummer-irregular are retracted at the
primary source and survive only as Riccati-arithmetic
characteristic primes of the shadow tower.
...
modular pre-Koszul inputs at $g \ge 1$, the extension uses
the modular axiom; on the standard CFT-type landscape it is
unconditional except for integer-spin $\beta\gamma$.
\end{theorem}
...
For the generic Virasoro and principal $\cW$-algebra
families, the chiral Hochschild cohomology is concentrated
in degrees $\{0,1,2\}$ with Hilbert polynomial
$P(t)=1+t^2$. For affine Kac--Moody at generic level,
\[
\ChirHoch^1(V_k(\fg)) \cong \fg.
\]
At critical level, the cohomology is identified with the
continuous Lie algebra cohomology by the
Beilinson--Drinfeld comparison theorem.
\end{theorem}
...
under Cartesian product: $2 + 1$; distinct from the Hodge-supertrace
$\Xi = \kappa_{\mathrm{ch}}$ which is K\"unneth-multiplicative and
vanishes for $K3 \times E$; see Volume~III
...
characteristic), $\kappa_{\mathrm{ch}}^{\mathrm{Heis}} = 3$
(the complex dimension, Route~B), $\kappa_{\mathrm{BKM}} = 5$
(the Borcherds weight), and $\kappa_{\mathrm{fiber}} = 24$
(the lattice rank). Four algebras see four aspects of the
same manifold.
...
No open conjecture in the programme has unresolved
...
is constructed at all degrees; the arithmetic invariants
