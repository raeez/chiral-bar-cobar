...
$F_g = \kappa \cdot \lambda_g^{\mathrm{FP}}$ governed by
...
\fg \otimes \fg\,,
\end{equation}
where $\Omega$ is the Casimir tensor.  The averaging map
collapses $\Omega$ to a scalar:
...
algebra~$A$.  A quadratic algebra $A = T(V)/(R)$ has quadratic
...
$1$ & Kac--Moody $\widehat{\fg}_k$ & $\Omega/z$ & $\mathbf{L}$ \\
...
$\mathrm{av}(\Omega/z) = \dim(\fg)(k + h^\vee)/(2h^\vee)$.
