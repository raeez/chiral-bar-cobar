...
geometry; Theorem~D, the uniform-weight genus expansion
$\mathrm{obs}_g = \kappa\cdot\lambda_g$ with explicit cross-channel
correction $\delta F_g^{\mathrm{cross}}$ off that lane; Theorem~H,
polynomial chiral Hochschild cohomology concentrated in degrees
$\{0,1,2\}$. The ordered bar
...
For a Koszul pair $(\cA, \cA^!)$ on~$X$ and each $g \ge 0$, the
...
\end{equation}
If the cyclic pairing is perfect and nondegenerate, then for
$g \ge 1$ the summands $\mathbf{Q}_g(\cA) =
\operatorname{fib}(\sigma - \mathrm{id})$ and
$\mathbf{Q}_g(\cA^!)=\operatorname{fib}(\sigma + \mathrm{id})$
are Lagrangian for the $(-1)$-shifted symplectic pairing
on\/~$\mathbf{C}_g$.
\end{theorem}
...
On the Koszul locus, in the generic/PBW-completed regime, the
chiral Hochschild cohomology of the Virasoro and principal
$\cW$-algebra families is concentrated in degrees $\{0,1,2\}$
with degree-$\leq2$ Hilbert polynomial. This does not assert a
free-polynomial cup-product algebra. For affine
Kac--Moody at generic level,
\[
\ChirHoch^1(V_k(\fg)) \cong \fg.
\]
At critical level, the comparison belongs to the separate
critical-level lane and the cohomology is not bounded by this
generic concentration statement.
\end{theorem}
...
detection package. Item \textup{(viii)} is a one-way
consequence on the Koszul locus. Under the shifted-symplectic
hypothesis package, \textup{(xi)} is equivalent. Condition
\textup{(xii)} implies \textup{(x)}; the converse is open.
