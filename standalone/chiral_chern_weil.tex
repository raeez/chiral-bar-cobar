...
Koszul-complementary pairs. Two distinct but linked invariants
live on every Koszul-complementary pair:
\begin{equation}\label{eq:wave14-universal-conductor}
K(\cA) \;:=\; c(\cA) + c(\cA^!)
 \;=\; -\,c_{\mathrm{ghost}}\bigl(\mathrm{BRST}(\cA)\bigr)
\qquad\text{(Trinity conductor, Vol~I Thm.~Trinity)}
\end{equation}
and the scalar $\kappa$-complementarity
$K^{\kappa}(\cA) := \kappa(\cA) + \kappa(\cA^{!})$, related by
$K^{\kappa}(\cA) = \varrho_\cA \cdot K(\cA)$ with
$\varrho_\cA := \kappa(\cA)/c(\cA)$ the anomaly ratio.
Trinity values: $K = 0$ for classes~G and~L (Heisenberg, free,
abelian $\mathrm{KM}$, $K = 2\dim(\fg)$ for non-abelian $V_k(\fg)$);
$K = 26$ for Virasoro (so $K^{\kappa} = 13$ at self-dual
$c = 13$, with $\varrho_{\mathrm{Vir}} = 1/2$); $K = 196$ for the
...
$K^{\kappa}_{\mathrm{BP}} = 98/3$ and $\varrho_{\mathrm{BP}} = 1/6$);
$K = 100$ for principal $\cW_3$ (so $K^{\kappa} = 250/3$ with
$\varrho_{\cW_3} = 5/6 = H_3 - 1$). The Friedan--Martinec--Shenker
...
shift included for~$N \ge 3$). Then the Koszul
conductor satisfies
...
\begin{proof}[Proof sketch]
The Arnold form $d\log z$ is weight~$1$, so the residue
...
output with the Vol~III Borcherds characteristic. By the
cross-volume propagation discipline, every cross-volume citation is
verified by grep in all three repositories within the same session;
the identity is labelled with the uniform-weight scope tag and the
class~M chain-level conditional on topologization, since the
Virasoro half of the pair is class~M.
\end{remark}
