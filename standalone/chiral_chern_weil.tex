...
% ================================================================
% Universal Koszul conductor and scalar complementarity
% ================================================================
\section{The universal Koszul conductor and scalar complementarity}
\label{sec:conductor-climax}
...
single universal ratio converting $K_c$ to $K^\kappa$: the affine
...
normalisations.
	...
	$c = 13$, with $\varrho_{\mathrm{Vir}} = 1/2$); $K_c = 196$ for the
...
\begin{theorem}[Koszul conductor: central and scalar normalisations]
...
shift included for~$N \ge 3$). Then the central-charge conductor
satisfies
\begin{equation}\label{eq:conductor-identity}
K_c(\cA) \;=\; -\,c_{\mathrm{ghost}}(\cA).
\end{equation}
The scalar modular conductor $K^\kappa(\cA)$ is obtained from
$K_c(\cA)$ by the family-specific $\kappa$ normalisation.
Equivalently, the level shift~$k \mapsto k + h^\vee$ entering the
affine Sugawara construction is the Chern--Weil trace of
...
=0.
\]
The FMS ghost calculation records the central-charge lane; the
...
convention matches the central-charge conductor
$K_c^{\mathrm{BP}}=196$ at every~$k$; the scalar conductor is
$K^\kappa_{\mathrm{BP}}=196/6=98/3$, and the fixed point
$k = -h^\vee(\mathfrak{sl}_3) = -3$ is a pole with
principal-value half $49/3$.
\end{proof}

\begin{theorem}[Scalar complementarity at genus zero]
\label{thm:climax-genus-zero}
On the Koszul locus of the canonical standard landscape, the
scalar conductor~$K^\kappa(\cA)$ equals the residue of the genus-zero chiral
bar propagator against the identity insertion:
\begin{equation}\label{eq:climax-g0}
K^\kappa(\cA) \;=\; -\,\Res_{z=0}\,\tr\!\bigl[r(z)\,\omega_{\mathrm{Arnold}}(z)\bigr]
\Big|_{\cA \oplus \cA^!},
\end{equation}
...
genus-zero data: the scalar identity lifts to all genera through
the Hodge class
($\mathrm{obs}_g(\cA \oplus \cA^!) = K^\kappa(\cA)\cdot\lambda_g$), and
the genus-zero identity~\eqref{eq:climax-g0}
...
$k \mapsto -k - 2h^\vee$ on the sum of the trace-form
	contributions) and produces $K^\kappa(\cA)$. The lift to higher genus
...
The scalar conductor values $\{0, 13, 98/3, 250/3\}$ are verified
against two disjoint sources under the independent verification
protocol: direct computation of $\kappa + \kappa^!$, and FMS
ghost tabulation after conversion from $K_c$ to $K^\kappa$.
\end{remark}

\begin{remark}[Cross-volume bridge]
\label{rem:conductor-cross-volume}
The central identity~\eqref{eq:conductor-identity} and scalar
identity~\eqref{eq:climax-g0} are separate Vol~I manifestations of
the trace formalism. In Vol~III, the scalar normalization reappears
...
$K^\kappa(\cA)=\kappa_{\mathrm{BKM}}$ on the
Borcherds--admissible locus relates the Vol~I chiral Chern--Weil
output to the Vol~III Borcherds characteristic. The identity
carries the uniform-weight scope tag and the class~M chain-level
conditional on topologization, since the Virasoro half of the pair is
class~M.
\end{remark}
