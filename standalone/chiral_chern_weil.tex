...
$r(z)$. The averaging comparison from the ordered to the symmetric
convolution algebra plays the role of the invariant polynomial where
the descent datum exists: it kills the $\Eone$ operadic indices, and
the scalar shadow data is then extracted by family-specific
moment-residue, Sugawara, and composite-sector rules. As the curve~$X$
varies over $\Mbar_g$, the fiberwise bar differential has a fiber
Arakelov curvature term, while the base obstruction class is
$\obs_g(\cA)=\kappa(\cA)\lambda_g$.
The degree-by-degree images
$(\kappa, \mathfrak{C}, \mathfrak{Q}, \ldots)
= \av(r(z), r_3, r_4, \ldots)$
form the shadow tower. The four standard depth types (G/L/C/M) are
...
$r(z) = (c/2)/z^3 + 2T/z$ mixes $c$-number and
...
$\av\colon \gEone \to \gmod$ (the
$\Sigma_n$-coinvariant comparison on the descent locus), which kills
the $\Eone$ operadic indices. The final scalar extraction is
family-specific.
...
\]
After $d\log$-absorption the cross-contraction has pole order~$0$,
so $r_{\beta\gamma}^{\mathrm{coll}}(z)=0$ in the collision-residue
...
\item $\ker(\av)$ records the $R$-matrix structure, the braiding,
 and the Yangian coherences invisible to the symmetric shadow.
\end{enumerate}
\end{theorem}
...
\[
\Res_{z=0}[r_{bc}^{\mathrm{coll}}(z)] = 0.
\]
...
the collision $r$-matrix has no residue, and
$\kappa$ is not extracted by a naive trace-residue formula.
...
The averaging comparison extracts the symmetric degree-$2$ shadow
from the ordered $r$-matrix. This is $\kappa$ on abelian and scalar
families after the appropriate moment-residue rule; for non-abelian
affine Kac--Moody the averaged scalar is $\kappa_{\mathrm{cl}}$ and
the full modular characteristic is
...
fiber curve. The corresponding class on the moduli base is obtained
by the GRR/Deligne-pairing pushforward and is
...
The answer is no. At genus~$1$ the formula remains exact, while at
genus~$\ge 2$ a cross-channel
...
$\av\colon\gEone\to\gmod$ (which kills the
$\Eone$ operadic indices on the descent locus),
...
\kappa(\cA)
\;=\;
\sum_i \Res_{z=0}\!\bigl(z^{2h_i-2}r^{\mathrm{vac}}_{ii}(z)\,dz\bigr)
\;+\;\kappa_{\mathrm{sp}},
\end{equation}
