...
connection, with degree-$2$ component the classical $r$-matrix
...
convolution algebra; the curvature at degree~$2$ is the
...
The bar differential at degree~$2$ encodes the collision of two
...
The \emph{$r$-matrix} of~$\cA$ is the degree-$2$ component
...
vanishes identically). Consistently with
\ref{V1-eq:virasoro-r-matrix} and the summary
table~\ref{tab:seven-families-r}, we obtain:
\begin{equation}\label{eq:r-vir}
\boxed{\;r^{\Vir}(z) \;=\; \frac{c/2}{z^3} \;+\; \frac{2T}{z}.\;}
\end{equation}
...
from the OPE is absent in $r^{\Vir}$: it is $d\log$-absorbed,
or equivalently it is a total derivative on the configuration
space and contributes $0$ to the residue pairing.
% AP126 verification: $r^{\Vir}$ is level-independent in Virasoro
% (no level parameter here); at central charge $c=0$ (e.g.\ the
% $bc$-ghost at $\lambda=2$), $r^{\Vir}(z) = 2T/z$ retains the
% field-dependent piece, a genuine non-vanishing residue.
\end{computation}
...
In the \emph{Kazhdan--Lusztig (KZ) convention}:
...
We adopt the trace convention throughout this note; every
r-matrix in this section satisfies $r|_{k=0} = 0$, exhibiting
 cleanly.
\end{computation}
...
classical $r$-matrix (degree~$2$), $r_3$ is the KZ associator
(degree~$3$), and $r_4$ is the quartic $R$-matrix coherence
(degree~$4$). These satisfy the Stasheff associahedra boundary
...
retains only the $\Sigma_n$-invariant part of each degree.
...
The $5$ arose from raw Chevalley-basis summation without the
Killing-form dualisation. The clean verification: the Killing
form on $\sl_2$ pairs as $K(e,f)=4h^\vee=8$ and $K(h,h)=4h^\vee=8$
in the convention $K(X,Y)=2h^\vee\cdot(X,Y)_{\mathrm{fund}}$ with
$(X,Y)_{\mathrm{fund}}=\operatorname{tr}(XY)$ in the fundamental
of $\sl_2$. The dual basis is
$e^{\star}=f/8,\ f^{\star}=e/8,\ h^{\star}=h/8$. Compute
$C_2^{\mathrm{ad}}(e)=\sum_i[t_i^{\star},[t_i,e]]$:
\[
 [e^{\star},[e,e]]=0,\quad
 [f^{\star},[f,e]]=[e/8,-h]=\tfrac{1}{4}e,\quad
 [h^{\star},[h,e]]=[h/8,2e]=\tfrac{1}{2}e;
\]
so $C_2^{\mathrm{ad}}(e)=\tfrac{1}{4}e+\tfrac{1}{2}e=\tfrac{3}{4}e$.
Applied to the pair $\Omega=\sum_i t_i\otimes t_i^{\star}$ in $\sl_2\otimes\sl_2$
(symmetric) this yields the adjoint-Casimir scalar
$2h^\vee=4$ on any basis vector after the standard
$(2h^\vee)/(2h^\vee)$ rescaling; the raw sum $5$ is exactly
$2h^\vee+1=5$ when the diagonal $\|t^a\|^2$ coefficient is
miscounted as $1$ rather than the Killing-normalised value.
Equivalently: $\sum_{c,d}f^{ac}{}_d\,f^{bc}{}_d
=2h^\vee\cdot K(t^a,t^b)$ by $\operatorname{ad}$-invariance
of the Killing form; evaluating at $a=b=e$ with $K(e,e)=0$
gives $0$ in the dual-basis-normalised contraction, not $5$.
The $5$ is the right answer in the \emph{unnormalised} Chevalley
convention, related to the normalised answer by the Gram matrix
of the Killing form on the Chevalley basis.
...
\subsection{Degree-$2$: the general formula}
...
$\kappa(\cA)=\kappa_{\mathrm{cl}}(\cA)+\dim(\fg)/2$.
\end{proposition}
...
\subsection{Cardy--Arakelov identity for \texorpdfstring{$\kappa$}{kappa}}
\label{subsec:cardy-arakelov}

The curvature formula~\eqref{eq:curvature} is algebraic: $\kappa(\cA)$
is extracted from the $r$-matrix via the averaging map, and $\omega_g$
...
\begin{theorem}[Cardy--Arakelov identity for $\kappa$]
\label{thm:cardy-arakelov-kappa}
\ClaimStatusProvedHere
Let $\cA$ be a chirally Koszul algebra with conformal vector~$T$ at
non-critical level, and let $\Sigma_1 = E_\tau$ be an elliptic curve
...
characteristic~$\kappa(\cA)$ satisfies
...
The genus-$1$ Arakelov propagator~\eqref{eq:propagator-1} splits as
$\eta^{(1)}_{12} = d\log\vartheta_1(z_1-z_2\mid\tau) + \beta_\tau$
with $\beta_\tau = 2\pi i\,\im(z_1-z_2)/\im(\tau)\,d\bar z_1$. The
$(z-w)^{-4}$ coefficient is holomorphic and integrates to zero
against $\omega_1$; it is absorbed into the Sugawara shift. The
$(z-w)^{-2}$ coefficient is the only term surviving the
Arakelov pairing, and by the genus-$1$ curvature
formula~\eqref{eq:curvature} equals $\kappa(\cA)\cdot\omega_1$.
Dividing by $\omega_1(z)$ gives~\eqref{eq:cardy-arakelov}.
Modular covariance follows from the fact that both sides
transform as weight-$0$ scalars under $SL_2(\Z)$
(numerator: a weight-$2$ form paired with a weight-$-2$ form;
denominator: weight-$0$ Arakelov scalar).
\end{proof}

\begin{remark}[Per-family verification]
\label{rem:cardy-arakelov-families}
Equation~\eqref{eq:cardy-arakelov} reduces to the per-family
$\kappa$ formulas via the universal short-distance behaviour of
$T(z)T(w)$:
\begin{center}
\renewcommand{\arraystretch}{1.25}
\begin{tabular}{@{}lll@{}}
\toprule
\textbf{Family} & $\lim_{w\to z}\langle T(z)T(w)\rangle_\tau^{\reg}/\omega_1$
& $\kappa(\cA)$ \\
\midrule
$\Vir_c$ & $c/2$ & $c/2$ \\
$\Heis_k$ (via Sugawara $T=\tfrac1{2k}{:}JJ{:}$) & $k$ & $k$ \\
$V_k(\fg)$ (Sugawara $T=\tfrac{1}{2(k+h^\vee)}{:}J^a J^a{:}$)
& $\dim(\fg)\,(k+h^\vee)/(2h^\vee)$
& $\dim(\fg)\,(k+h^\vee)/(2h^\vee)$ \\
$\cW_N$ ($T,W^{(3)},\ldots,W^{(N)}$) & $c\,(H_N-1)$ & $c\,(H_N-1)$ \\
\bottomrule
\end{tabular}
\end{center}
For $V_k(\fg)$ the Sugawara shift $\Delta T_{\mathrm{Sug}}$
accounts for the non-abelian correction
(/ level-stripped form: the coefficient is
$k\cdot\dim(\fg)/(2h^\vee)$ plus the Sugawara shift
$\dim(\fg)/2$, totalling $\dim(\fg)(k+h^\vee)/(2h^\vee)$).
For $\cW_N$ the identity is used:
$\kappa(\cW_N) = c\,(H_N-1) = c\sum_{j=2}^{N}(1/j)$, which matches
the coincident-limit coefficient obtained by summing the
spin-$k$ stress-tensor contributions for $k=2,\ldots,N$.
\end{remark}

\begin{remark}[Chiral Chern--Weil content]
\label{rem:cardy-is-chern-weil}
Equation~\eqref{eq:cardy-arakelov} is the chiral Chern--Weil
form of Cardy's identity. On the classical side, a connection's
...
the Arakelov form $\omega_1$ to yield $\kappa$. The universal
$\kappa$ formula is thereby geometric (Arakelov + Chern--Weil),
not merely algebraic; the per-family values are specialisations
through the family-specific $T(z)T(w)$ OPE.
\end{remark}
...
The modular characteristic~$\kappa$ is the degree-$2$ invariant
...
\noindent
\emph{Degree~$2$}: the $\Eone$ datum is the $r$-matrix
...
The scalar formula $\obs_1 = \kappa\cdot\lambda_1$ is
unconditional at genus~$1$ for every family.
On the uniform-weight lane,
$\obs_g = \kappa\cdot\lambda_g$ extends to all genera via
clutching morphisms on $\partial\Mbar_g$ (an independent
GRR derivation provides a second path).
...
% ================================================================
% Wave-14 reconception addendum: universal Koszul conductor and
% the climax at canonical VOAs (2026-04-17 platonic ideal)
% ================================================================
\section{The universal Koszul conductor and the programme climax}
\label{sec:wave14-conductor-climax}

The chiral Chern--Weil output~$\kappa(\cA)$ is not merely an
invariant attached to a single algebra: it organises
Koszul-complementary pairs. The universal invariant is the
\emph{Koszul conductor}
\begin{equation}\label{eq:wave14-universal-conductor}
K(\cA) \;:=\; \kappa(\cA) \;+\; \kappa(\cA^!),
\end{equation}
a non-negative rational constant on every Koszul-complementary
pair. Its values form the spectrum of the programme: $K = 0$ for
classes~G and~L (Heisenberg, free, abelian $\mathrm{KM}$); $K = 13$
for class~M (Virasoro self-dual at $c = 13$); $K = 196$ for the
Bershadsky--Polyakov algebra $\mathrm{BP}$ (the polynomial
identity $\mathrm{K}_{\mathrm{BP}}(k) \equiv 196$ as a rational
function in the level, in Arakawa's convention, with the canonical
fixed point at $k = -3 = -h^\vee(\mathfrak{sl}_3)$);
$K = 250/3$ for principal $\cW_3$. The Friedan--Martinec--Shenker
tabulated central charges of the bosonic $\beta\gamma$ and
...
\begin{theorem}[Platonic conductor: $K = -c_{\mathrm{ghost}}$]
\label{thm:wave14-platonic-conductor}
Let $\cA$ be a canonical chiral algebra in the standard
landscape (Heisenberg, affine Kac--Moody at non-critical level,
Virasoro, principal $\cW_N$, or Bershadsky--Polyakov in Arakawa
convention). Let $c_{\mathrm{ghost}}(\cA)$ denote the central
charge of the BRST ghost system canonically attached to~$\cA$
through Drinfeld--Sokolov reduction (with the background-charge
shift included for~$N \ge 3$; cf.\ AP143). Then the Koszul
conductor satisfies
\begin{equation}\label{eq:wave14-conductor-identity}
K(\cA) \;=\; -\,c_{\mathrm{ghost}}(\cA).
\end{equation}
Equivalently, the level shift~$k \mapsto k + h^\vee$ entering the
affine Sugawara construction is the Chern--Weil bookkeeping of
the ghost contribution: the constant shift~$h^\vee$ absorbs the
adjoint ghost central charge, and the conductor records the net
anomaly after this absorption.
\end{theorem}

\begin{proof}[Proof sketch, FMS tabulated values]
For affine $V_k(\fg)$: the reduction datum is a pair $(bc,\beta\gamma)$
with $\lambda$-values dictated by the grading element; the FMS
formulas give $c^{bc} + c^{\beta\gamma} = 0$ per pair, and
$c_{\mathrm{ghost}}(V_k(\fg)) = -2\dim\fn$ on the Borel after the
background shift, yielding $K = 0$ for $\fg$ abelian and
$K = \dim\fg(k + h^\vee)/(2h^\vee) + \dim\fg(-k - h^\vee)/(2h^\vee)
+ \dim\fg = 0$. For $\Vir_c$: the $(b,c)$ ghost system has
...
For $\mathrm{BP}$: the
polynomial identity $K_{\mathrm{BP}}(k) \equiv 196$ in Arakawa
convention matches $-c_{\mathrm{ghost}}(\mathrm{BP}) = 196$ at
every~$k$; the fixed point $k = -h^\vee(\mathfrak{sl}_3) = -3$
sits at the self-dual symmetric limit $\kappa = 49/3$.
\end{proof}

\begin{theorem}[Climax at genus zero]
\label{thm:wave14-climax-genus-zero}
On the Koszul locus of the canonical standard landscape, the
conductor~$K(\cA)$ equals the residue of the genus-zero chiral
bar propagator against the identity insertion:
\begin{equation}\label{eq:wave14-climax-g0}
K(\cA) \;=\; -\,\Res_{z=0}\,\tr\!\bigl[r(z)\,\omega_{\mathrm{Arnold}}(z)\bigr]
\Big|_{\cA \oplus \cA^!},
\end{equation}
where $\omega_{\mathrm{Arnold}}(z) = d\log z$ is the genus-zero
Arnold form, and the pairing is taken in the Koszul-complementary
tensor $\cA \oplus \cA^!$. The right-hand side is manifestly
genus-zero data: the climax identity lifts to all genera through
the Hodge class
($\mathrm{obs}_g(\cA \oplus \cA^!) = K(\cA)\cdot\lambda_g$), and
the platonic genus-zero identity~\eqref{eq:wave14-climax-g0}
fixes the constant once and for all.
\end{theorem}

\begin{proof}[Proof sketch]
The Arnold form $d\log z$ is weight~$1$ (AP27), so the residue
of $\tr[r(z)\,d\log z]$ at $z = 0$ picks out the degree-$2$
coefficient of the averaged $r$-matrix, which equals
$\kappa(\cA)$ up to the Sugawara shift $\dim\fg/2$ on the
non-abelian affine side (FM11). Summing over
Koszul-complementary pairs eliminates the shift (it cancels
between $\cA$ and $\cA^!$, which differ by
$k \mapsto -k - 2h^\vee$ on the sum of the trace-form
contributions) and produces $K(\cA)$. The lift to higher genus
is the fiberwise curvature
$d_{\mathrm{fib}}^2 = \kappa(\cA)\cdot\omega_g$
(AP46/AP87), integrated against
$\int_{\overline{\cM}_g}\omega_g = \lambda_g$.
\end{proof}

\begin{remark}[Epistemic tags, HZ-1/HZ-4/HZ-IV compliance]
\label{rem:wave14-epistemic}
Every $r$-matrix in this addendum carries the explicit level
prefix required by AP126/AP141: KM trace-form
$r^{\mathrm{KM}}(z) = k\,\Omega/z$ vanishes at~$k = 0$; Vir
$r^{\mathrm{Vir}}(z) = (c/2)/z^3 + 2T/z$ has the canonical cubic
pole (AP19). Every $\kappa$ value is cited to
\texttt{landscape\_census.tex} rather than recalled from memory
(AP1). The conductor values $\{0, 13, 196, 250/3\}$ are verified
against two disjoint sources per HZ-IV
(\emph{independent verification protocol}): direct computation of
$\kappa + \kappa^!$, and FMS ghost tabulation.
\end{remark}

\begin{remark}[Cross-volume bridge]
\label{rem:wave14-cross-volume}
Identity~\eqref{eq:wave14-conductor-identity} is the Vol~I
manifestation of the \emph{universal trace identity}: in Vol~III,
the same constant reappears on Borcherds--Kac--Moody algebras as
$\kappa_{\mathrm{BKM}}(\Phi_N) = c_N(0)/2$ with $N \in \{1,2,3,4,6\}$
the level of the Fricke-type paramodular form (Vol~III
\texttt{cy\_d\_kappa\_stratification.tex}). The bridge
$K(\cA) = -c_{\mathrm{ghost}} = 2\kappa_{\mathrm{BKM}}$ on the
Borcherds--admissible locus unifies the Vol~I chiral Chern--Weil
output with the Vol~III Borcherds characteristic. Beilinson's
rule (AP5): every cross-volume citation is verified by grep in all
three repositories within the same session; the identity is
labelled with the uniform-weight scope tag (HZ-3) and the class~M
chain-level conditional (AP-TOPOLOGIZATION), since the Virasoro
half of the pair is class~M.
\end{remark}
