...
The Virasoro local seeds and weighted-Riccati construction give
\begin{align}
R_2 &= \tfrac{c}{2}\,, &
R_3 &= 2\,, &
R_4 &= \frac{10}{c(5c+22)}\,,
...
\end{align}
\noindent
The separate formal relation
$2R_2C_6^{\mathrm{rel}}+2R_3R_5+R_4^2=0$ gives
\[
C_6^{\mathrm{rel}}
=\frac{4(240c+1031)}{c^3(5c+22)^2}.
\]
The identity $R_3=2$ is the Virasoro OPE structure constant.
An ordered residue package $\mathsf H_{\mathrm{res}}(\Vir_c;X)$ and
its normalized projection define the geometric coefficients
$S_r(\Vir_c;\mathsf H_{\mathrm{res}})$.  A level-six radical vector
and its decoupling map supply the singular-vector interpretation of
$C_6^{\mathrm{rel}}$.
...
\begin{table}[ht]
\centering
\caption{Formal weighted-Riccati coefficients $R_r$ for
$r = 2, \dotsc, 10$}
...
\noindent
Each $R_r$ is a rational function of $c$ with poles at $c = 0$ and
$c = -22/5$; these are the two values where the quadratic Casimir
$c(5c+22)$ vanishes. The sign pattern
$R_r > 0$ for even $r$ and $R_r < 0$ for odd $r \geq 5$
...
The residue comparison attached to
$\mathsf H_{\mathrm{res}}(\Vir_c;X)$ determines the locus on which
