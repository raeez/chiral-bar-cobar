...
\]
These are conventionally related normalizations, not an identity as
rational functions of~$k$. The tree-level graph sum terminates at
trivalent vertices.
...
self-bracket exits the cyclic deformation complex at degree~$5$, by
...
$\kappa(\cA) = 0$. The shadow depth $r_{\max}(\cA)$ is then
unconstrained by the single-line dichotomy and is read from the
Feigin--Frenkel coset data $\mathrm{coset}_{FF}(\cA)$.
\end{definition}
...
\noindent\textbf{Characterization.}
At $\kappa = 0$ the conformal vector degenerates: the Sugawara
...
(Definition~\ref{def:canonical-fingerprint} in the monograph)
...
Class~FF is not a shadow-depth class in the sense of classes
$G, L, C, M$: it is the locus on which the ambient shadow invariants
degenerate and the classification is controlled by coset data rather
than by~$(\alpha, \Delta)$.

\begin{remark}[Why FF is a fifth class, not a refinement]
...
Promoting~FF to a fifth companion class is therefore not a refinement
of the quadrichotomy but a structural completion:
the five classes exhaust the full fingerprint stratum
(Theorem~\ref{thm:five-class-stratum} in the monograph).
\end{remark}
...
standard landscape. Two further strata appear once the landscape is
enlarged: logarithmic $\cW(p)$ and Wakimoto free-field
realisations.

\begin{definition}[Class~$M^\ast$: logarithmic companion]
\label{def:class-M-star}
A chirally Koszul algebra $\cA$ belongs to class~$M^\ast$ if it
is logarithmic in the sense of Gurarie--Flohr, i.e.\ the Zhu
...
analysis of Theorem~\ref{thm:single-line-dichotomy} applies on
the formal-weight line; logarithmic corrections do not affect
...
kernel inside $\cH^{\oplus (N-1)}$); affine $\widehat{\mathfrak{sl}}_2$
through the $\beta\gamma\,\phi$ Wakimoto realisation; admissible
affine algebras through parabolic screenings (KRW03, Arakawa07).

\noindent\textbf{Characterization.}
Class~$W$ is not disjoint from $G,L,C,M$: it is a presentational
overlay. The ambient Heisenberg is class~$G$, and the kernel
inherits chiral Koszulity from the screening-kernel preservation
theorem. The utility of the class~$W$ label is that it
canonicalises the Koszul dual: the Koszul-locus preservation is
witnessed explicitly by the screening integrand, and Drinfeld
coproducts descend to the intersection from the rank-$(N-1)$
free-field parent.

\begin{remark}[Why the full stratification is G, L, C, M, FF,
$M^\ast$, W]
...
$\kappa = 0$ boundary. Class~$M^\ast$ records logarithmic
amplitude behaviour orthogonal to $(\alpha, \Delta)$: a
Zhu-finite class-M algebra is class-M, class-$M^\ast$, or both,
depending on Massey boundedness (Gurarie 1993, Flohr 1996).
Class~$W$ records a presentation overlay: a class-L or class-M
algebra may simultaneously be class-$W$ when realised as a
screening kernel. The seven-stratum fingerprint is exhaustive on
the standard landscape.
\end{remark}

\subsection{Universal shadow-series exponential base}
\label{ssec:universal-C-A-6}

The asymptotic behaviour of the shadow coefficients on
non-logarithmic class~M algebras containing a Virasoro
subalgebra admits a sharp universal description, independent of
the specific family.

\begin{theorem}[Universal exponential base $C_\cA = 6$]
\label{thm:universal-C-A-6-sa}
Let $\cA$ be a non-logarithmic class~M chirally Koszul algebra
with Virasoro subalgebra at generic central charge. The shadow
series
$\Sigma_\cA(z) = \sum_{r \geq 2} S_r(\cA)\, z^r$
has radius of convergence exactly $1/6$; equivalently,
\[
C_\cA \;:=\; \limsup_{r \to \infty}
\bigl|S_{r+1}(\cA)/S_r(\cA)\bigr| \;=\; 6.
\]
The closed form on the Virasoro primary line is
\[
\Sigma_{\Vir}(z)
\;=\;
\tfrac{4z^3}{9} - \tfrac{z^2}{9} + \tfrac{z}{27}
 - \tfrac{\log(1 + 6z)}{162}\,.
\]
Pole-doubling for all $k$:
$\Sigma_\Vir^{(k)}(z)
   = [P_k + Q_k \log(1{+}6z)] / (1{+}6z)^{2k}$.
The constant~$6$ decomposes as $6 = r_0 \cdot S_{r_0}$
$= 3 \cdot 2$: the bar-depth seed times the Riccati seed
coefficient.
\end{theorem}

The constant~$6$ is programme-universal on class~M non-log with
Virasoro subalgebra: the same $1/6$ radius appears for
$\cW_N$-algebras at leading $1/c$, and the Vol~II tempering rate
$\beta_\cA$ coincides with~$C_\cA$ at leading $1/c$. The higher
sub-leading $\cW_N$ corrections $\beta_N = (N+1)(N+2)/2$ absorb
into $C_{\cW_N} = 6$ asymptotically.
...
\begin{proposition}[Picard--Fuchs for the shadow series]
\label{prop:picard-fuchs-sa}
Let $\cA$ be class~M with Virasoro subalgebra at generic
central charge. The shadow series $\Sigma_\cA(z)$ satisfies a
second-order Picard--Fuchs equation
\[
(1 + 6z)^2 \Sigma_\cA''(z)
 + P_1(z)\, \Sigma_\cA'(z)
 + P_0(z)\, \Sigma_\cA(z)
...
determined by $(\kappa, \alpha, \Delta)$. The regular-singular
point $z = -1/6$ is the universal branch point; the monodromy
around this point is the reflection sending $z \mapsto -z/(1+6z)$,
realising the Borel-plane involution on the shadow tower.
\end{proposition}

\subsection{The depth gap $d_{\mathrm{alg}} \in \{0,1,2,\infty\}$}
\label{ssec:depth-gap-standalone}

\begin{proposition}[Depth gap]
\label{prop:depth-gap-standalone-new}
\ClaimStatusProvedHere
For every chirally Koszul algebra in the standard landscape, the
algebraic depth $d_{\mathrm{alg}}$ takes values in
$\{0,1,2,\infty\}$. The value~$3$ is impossible.
\end{proposition}

\noindent\textbf{Witness table.} $d_{\mathrm{alg}} = 0$: free
fields, $bc$ ghosts (shadow tower trivial). $d_{\mathrm{alg}} =
1$: Heisenberg and lattice (class~G). $d_{\mathrm{alg}} = 2$:
$\beta\gamma$ on the conformal-weight line (class~C) and affine
Kac--Moody (class~L). $d_{\mathrm{alg}} = \infty$: Virasoro,
$\cW_N$ principal (class~M).

\noindent\textbf{Mechanism.}
The gap at~$3$ is structural. If $d_{\mathrm{alg}} = 3$ there is
a nonzero degree-$4$ MC obstruction with no degree-$5$ follow-up;
the Riccati equation $\Delta = 8\kappa S_4$ combined with the
shadow Lie Jacobi identity forces either $\Delta = 0$ (which
brings $d_{\mathrm{alg}} \leq 2$) or an infinite tower
($d_{\mathrm{alg}} = \infty$). No intermediate value survives.
...
that the tower could stop at degree~$5$, or~$7$, or~$100$, by
some suitable choice of OPE data. This does not happen. The
tower terminates at degree~$3$ (class~$\mathbf{G}$), at
degree~$4$ (class~$\mathbf{L}$), at degree~$5$
...
tower at degree~$4$ by a mechanism external to the
