...
charge of the $\cW_{1+\infty}$ algebra at $N=1$ (single $\fgl_1$
current) is computed from the Sugawara construction
$T = (1/(2\Psi)) :J^2:$ applied to the level-$\Psi$ Heisenberg
current $J(z)J(w) \sim \Psi/(z-w)^2$; this gives $c = 1$ for every
nonzero $\Psi$, independent of the specific value.  For $N$ bosons
the central charge is $c = N$; the 6d $\fgl_1$ gauge reduction
produces $N = 1$.
\end{proof}
...
\]
% Level prefix Psi visible; at Psi = 0 (the degenerate locus
% sigma_2 = 0 where two of the h_i coincide up to sign), the
% r-matrix vanishes identically and the Sugawara denominator
% 1/(2*Psi) becomes undefined.
...
full $\cW_{1+\infty}$ algebra, the $r$-matrix has additional
components at higher spins, with the universal Miura coefficient
$(\Psi - 1)/\Psi$ on the leading cross-term
$J \otimes W_{s-1}$ at every spin $s \geq 2$ (Vol~I
\texttt{thm:miura-cross-universality}).  The central charge
$c = 1$ is the $N = 1$ rank of the underlying free boson and does
not track $\Psi$; Prochazka--Rapcak
parametrise $\cW_{1+\infty}$ equivalently by $(\lambda_1, \lambda_2,
\lambda_3)$ with $\sum_i 1/\lambda_i = 0$: the identification with
the 6d equivariant parameters is $\lambda_i^{-1} \propto h_i$ up to
the homogeneity that makes $\Psi$ projective in the $h_i$; the
$\Psi = -\sigma_2$ presentation used here fixes a normalisation
consistent with Costello's 6d action and with the
\texttt{hcs\_codim2\_defect\_ope} engine's
$J(z)J(w) \sim \Psi/(z-w)^2$ OPE.
\end{remark}
...
\subsection{The CoHA carries $\Eone$-chiral bialgebra data}

The CoHA is the natural $\Eone$-primitive of the CY quantum group.
Its algebraic structure matches the $\Eone$-chiral bialgebra axiom
system of Volume~I; the chiral antipode does not lift
(Remark~\ref{rem:v3-qg-antipode}), so the object is a chiral bialgebra
in the sense of that volume, not a chiral Hopf algebra.

\begin{proposition}[CoHA $\Eone$-chiral bialgebra data]
\label{prop:v3-qg-coha-hopf}
The critical CoHA $\CoHA(Q, W)$ of a toric CY$_3$ carries the data
of an $\Eone$-chiral bialgebra, with a classical Yangian antipode
that does not lift to a vertex-algebraic antipode:
\begin{enumerate}[label=\textup{(H\arabic*)}]
 ...
 On the classical Yangian fibre, the antipode
 ...
 formalism.  This classical antipode does not lift to a
 vertex-algebraic antipode on $\CoHA(Q, W)$ (OPE obstruction
 $S(T)_{(3)}S(T) \neq c/2$ at generic $\Psi$, and the universal
 Hopf-axiom residual; see
 Remark~\ref{rem:v3-qg-antipode} for the precise obstruction
 and Vol~I
 Proposition~\ref{prop:w-infty-antipode-obstruction}
 for the proved negative).  The CoHA is therefore a chiral
 bialgebra, not a chiral Hopf algebra.
\end{enumerate}
\end{proposition}
...
Drinfeld double. The anti-pattern
 is categorical: the string ``$\CY_4$ Yangian'' must be
...
$O(\hbar^2)$ correction absent in classical Yangians. forbids
...
\begin{conjecture}[7d hCS boundary algebra]
\label{conj:7d-hcs-boundary-algebra}
...
%%% ================================================================
%%% Wave-14 reconception addendum: codim-2 defect + CoHA positivity
%%%    and the toroidal formal-disk identification (2026-04-17)
%%% ================================================================

\section{Wave-14 reconception: codim-$2$ defect, $\CoHA$, toroidal}
\label{sec:wave14-6dhCS}

The 6d holomorphic Chern--Simons hierarchy inscribed in the body of
this paper is updated against the 2026-04-17 platonic ideal at
three precise points: the codimension-$2$ defect algebra on a curve
...
\begin{theorem}[Codim-$2$ defect at non-critical level]
\label{thm:wave14-codim2-defect}
Let $Y = \bC^3$ with holomorphic volume form
$\Omega_Y = dh_1\wedge dh_2\wedge dh_3$ and let
$C = \bC \subset Y$ be a codim-$2$ holomorphic curve. The 6d
holomorphic Chern--Simons defect algebra on~$C$ is the chiral
algebra $\cW_{1+\infty}$ at coupling
$\Psi = -\sigma_2(h_1,h_2,h_3)$, with central charge $c = 1$
supplied by the Sugawara construction for the $\widehat\fgl_1$
current; the normal bundle $N_{C/Y} = \cO \oplus \cO$ contributes
the two spectral parameters $(h_1,h_2)$ subject to the CY
constraint $h_1 + h_2 + h_3 = 0$.
\end{theorem}
...
parameters $h_1,h_2$ are the transverse directions; their elementary
symmetric polynomial $\sigma_2$ is the Miki coupling. The vanishing
$\sigma_1 = 0$ is the CY condition. Non-criticality
$\Psi \ne 0,\infty$ places the theory on the Koszul locus
(AP-TOPOLOGIZATION: proved for affine KM at non-critical level;
the passage to $\cW_{1+\infty}$ is cohomological, not chain-level,
in class~M).
\end{proof}

\begin{theorem}[$\CoHA(\bC^3)$ is the positive half, not the full
$\cW_{1+\infty}$]
\label{thm:wave14-CoHA-positive-half}
...
$\widehat\fgl_1$:
\begin{equation}\label{eq:wave14-CoHA-positive}
...
\begin{proof}[Proof sketch, \cite{SV20}]
...
derived Drinfeld double. The slogan ``$\CoHA = Y$'' in the
informal literature is therefore the Drinfeld-double extension,
not the bare $\CoHA$; the precise identification is
$\CoHA \simeq Y^+$, matching the Vol~III Wave-14
\texttt{phi\_functor\_volIII} reconstitution
(character coincidence $\ne$ bar equivalence).
\end{proof}

\begin{theorem}[Toroidal formal-disk identification]
\label{thm:wave14-toroidal-formal-disk}
On the formal disk $D = \Spec\Bbbk[[z]]$, the quantum toroidal
algebra $U_{q,t}(\widehat{\widehat\fgl}_1)$ carries the structure
of an $\Eone$-chiral quantum group: (i)~the Miki $S_3$-automorphism
is the $\Omega$-background Weyl symmetry permuting the three
equivariant parameters $q_1,q_2,q_3$ with $q_1 q_2 q_3 = 1$;
(ii)~the spectral $R$-matrix from the SV vertex
$\CoHA$-coproduct satisfies the additive Yang--Baxter equation;
(iii)~the $\Eone$-chiral quantum group equivalence
(\emph{Chiral QG equiv theorem, Vol~I}) holds at the formal-disk
...
\label{rem:wave14-toroidal-global}
The extension of Theorem~\ref{thm:wave14-toroidal-formal-disk}
...
conditional (AP-TOPOLOGIZATION) and the 2026-04-17 open frontier
note regarding modular-family base-change (proved cohomologically;
chain-level via Vol~II \texttt{theorem\_A\_infinity\_2}, conditional
on the Francis--Gaitsgory base-change).
\end{remark}

\begin{remark}[Geometric vs algebraic model discipline]
\label{rem:wave14-geom-vs-alg}
Throughout this addendum: spectral parameters
$(h_1,h_2,h_3)$ are \emph{formal algebraic} variables in the chiral
endomorphism operad $\End^{\mathrm{ch}}_\cA$, not
parameters on~$\FM_k(\bC)$; the identification
$\CoHA(\bC^3) \simeq Y^+$ is an identity of algebraic Hopf algebras,
not a geometric identification on a moduli of sheaves;
and $BZFN$-style centre comparisons refer to different algebras
$(\cA^{\mathrm{ch}}$ vs $\cA_{\mathrm{mode}})$, not different
spectral parameters. Where the text switches between
models, a bridge remark names both.
\end{remark}

\begin{remark}[Epistemic tags]
\label{rem:wave14-6d-epistemic}
Theorem~\ref{thm:wave14-codim2-defect} rests on \cite{CY}-style
BRST calculation at non-critical level (\emph{proved
cohomologically for affine KM; cohomological, not chain-level;
class~M cases via DS reduction conditional}, per
AP-TOPOLOGIZATION). Theorem~\ref{thm:wave14-CoHA-positive-half} is
\emph{proved} per \cite{SV20} with the positive-half correction
installed (previous ``$\CoHA = \cW_{1+\infty}$'' folklore
retracted as character-level coincidence).
Theorem~\ref{thm:wave14-toroidal-formal-disk} is \emph{proved}
at the formal disk; global extension conditional per
Remark~\ref{rem:wave14-toroidal-global}.
\end{remark}
