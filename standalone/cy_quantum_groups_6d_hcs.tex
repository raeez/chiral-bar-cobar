...
function via the MNOP correspondence in the non-compact degree-zero
normalization. This is not the OP-reduced $K3\times E$ invariant,
where the elliptic translation factor is removed before comparison
with the Borcherds denominator.
\end{proposition}
...
Drinfeld double. The obstruction
...
Obstruction theory for Eone-to-$E_4$ promotion locates this first
scalar class in $\pi_4(BU)$. Its vanishing removes only this
...
homotopy group is non-trivial; it does not by itself decide the full
higher obstruction tower.
\end{remark}
...
for affine KM at non-critical level; the passage to
$\cW_{1+\infty}$ is cohomological, not chain-level, in class~M.
\end{proof}
...
$\cW_{1+\infty}$ is not a bar equivalence.
\end{proof}
...
Theorem~\ref{thm:codim2-defect} rests on \cite{CY}-style
