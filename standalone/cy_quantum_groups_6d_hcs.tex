...
and $\Phi_3^{(\Sigma_2,C)}$ hypotheses, and the motivic equality remains
...
collision kernel and its represented $R$-operator. The symmetric bar
$B^\Sigma(A)$ carries the homotopy-coinvariant operator
$q_2(K_A^{\mathrm{coll}})$.  The normalized functional gives
$\kappa_{\mathrm{ch}}$ in abelian and scalar families and
$\kappa_{\mathrm{dp}}$ on the non-abelian affine row; the Sugawara
contribution $\dim(\fg)/2$ completes $\kappa_{\mathrm{ch}}$ there.
...
$\Etwo \to \Ethree$ is higher Deligne plus topologization.  The round
trip closes only after the Drinfeld-centre/derived-centre comparison
and the topologization package are both available.  This is proved on
the affine Kac--Moody non-critical locus by the Sugawara BRST
primitive; for general class-$M$ inputs the original-complex
chain-level comparison remains open.  Without that primitive the
structure remains $\SCchtop$.

The CY quantum groups of this paper enter the circle at the
$\Eone$ vertex: the CoHA is the ordered associative Hall input, and the
...
\begin{remark}[Level-prefixed collision kernel and represented $r$-matrix]
\label{rem:v3-qg-level-r-matrix}
The collision kernel of the $\widehat\fgl_1$ current subalgebra
derived from the 6d reduction carries level $\Psi=-\sigma_2$:
\[
 K_{H_\Psi}^{\mathrm{coll}}(z)
 = \frac{\Psi\Omega_{\mathcal H}}{z}
 = \frac{-\sigma_2(h_1,h_2,h_3)\Omega_{\mathcal H}}{z}.
\]
Its Fock representation has commuting tensor insertions and supplies
$H_{\mathrm{CYBE}}^{\mathrm{rep}}$ and the associated classical
$r$-matrix.
The specialization $\Psi=0$ gives the zero collision operator and the
singular Sugawara coordinate; this is the degenerate locus adjacent to
the non-critical Koszul locus.  The homotopy coinvariant remains the
rank-one operator and
$\tau_{H_\Psi,2}^{\mathrm{res}}q_2(K_{H_\Psi}^{\mathrm{coll}})
=\Psi=\kappa_{\mathrm{ch}}(H_\Psi)$. For the
...
%%% ================================================================
%%% 4. CoHA AS ORDERED HALL INPUT
%%% ================================================================

\section{The CoHA as ordered Hall input}
\label{sec:coha-e1}
...
categorical levels: the $\Ethree$-action is on the algebraic
closed-sector actor $\Zder(A_\cC)$, while a physical-bulk reading
requires the OCA/HT comparison datum.  The Miki automorphism is on
the boundary algebra (the chiral algebra itself).  The relationship
...
the underlying-monoidal functor; its right adjoint, the Drinfeld
center, equips objects with half-braidings.  This is the
categorical-centre step, not the lossy averaging map to the scalar
shadow.
\end{theorem}
...
\begin{enumerate}[(i)]
 ...
 Borcherds lift.
 \item \emph{Virtual}: under the uniform-weight and $\Phi_3^{(\Sigma_2,C)}$
...
the CoHA with $\cW_{1+\infty}$ gives at most a character identity
until the Drinfeld double is included.
