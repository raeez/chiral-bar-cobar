...
These invariants are distinct projections of common ordered data. The
collision kernel $K_\cA^{\mathrm{coll}}(z)$ lives on the ordered bar complex
...
dependence. Its homotopy coinvariant
$q_2(K_\cA^{\mathrm{coll}})$ retains an operator class, and the
normalized functional gives
$\tau_{\cA,2}^{\mathrm{res}}q_2(K_\cA^{\mathrm{coll}})=\kappa$ for
abelian and Virasoro-type families. On the non-abelian affine row this
trace is $\kappa_{\mathrm{dp}}$, with Sugawara contribution
$\dim(\fg)/2$. This scalar trace,
...
\label{sec:disambiguation-trichotomy-quadrichotomy}
% ================================================================
...
quadrichotomy $G, L, C, M$ by shadow depth
...
\emph{The $r_{\max}$-quadrichotomy} is a four-cell partition
...
($k_{\max} = 0$) with the free fermion, whereas the quadrichotomy
places~$\beta\gamma$ in class~$C$ and the free fermion in class~$G$;
conversely, the quadrichotomy groups~$\mathrm{Vir}_c$ with~$\Walg_N$
...
quadrichotomy, and a fifth class~$\mathrm{FF}$ at $\kappa = 0$
...
A chirally Koszul algebra $\cA$ is \emph{contact (class $C$)} if its
shadow tower has finite quartic depth:
\[
S_3(\cA)=0,\qquad S_4(\cA)\neq0,\qquad S_r(\cA)=0\quad(r\geq5).
\]
Equivalently, the averaged Maurer--Cartan projection has no cubic
component, has a nonzero quartic contact component, and has no higher
shadow components. The standard witnesses are $bc_\lambda$ and
$\beta\gamma_\lambda$, for which $S_4=-5/12$.
\end{definition}

\begin{remark}[Why class $C$ is not a pole-order condition]
\label{rem:class-C-not-pole-order}
The tempting identification ``class~$C = $ triple-pole~$R(z)$''
fails on the $\beta\gamma$ witness: $(p_{\max}, k_{\max}, r_{\max}) =
(1, 0, 4)$ has a simple OPE pole, vanishing collision residue, and
nonzero quartic shadow. Class~$C$ is not detected at bar degree~$2$;
it is detected by the averaged quartic contact component.
\end{remark}
...
$C$ & $4$ & $\beta\gamma$, $bc$ ghosts, matrix factorizations
...
\begin{remark}[Class $M$ membership is finitely witnessed]
\label{rem:finite-cert}
For the Virasoro algebra, the finite computations
$S_3(\Vir_c)=2$ and
$S_4(\Vir_c)=10/[c(5c+22)]$ at generic~$c$ give
$\Delta=8\kappa S_4\neq0$. The single-line dichotomy then forces
infinite shadow depth. The implication $S_3\neq0\Rightarrow S_4\neq0$
is not a general formal rule; here both coefficients are read directly
from the Virasoro OPE and the norm of
$\Lambda={:}TT{:}-(3/10)\partial^2T$.
\end{remark}

% ================================================================
\section{Companion strata: $M^\ast$ logarithmic and $W$ Wakimoto}
\label{sec:companion-strata}
% ================================================================

The four-class fingerprint $(G,L,C,M)$ stratifies the
non-logarithmic non-critical standard landscape by the algebraic
invariants $(\alpha, \Delta)$ of the shadow metric. Two further
strata surface once the landscape is enlarged: logarithmic
triplet algebras and screening-kernel free-field realisations.

\begin{definition}[Class~$M^\ast$]\label{def:class-M-star-sa}
A chirally Koszul algebra $\cA$ belongs to class~$M^\ast$ if
\textup{(i)}~the Zhu algebra $A(\cA)$ is finite-dimensional and
\textup{(ii)}~the Hochschild Massey products
$\langle \Omega, \Omega, \ldots, \Omega \rangle$ carry
amplitude on arbitrarily high tensor powers of the vacuum.
\end{definition}

Canonical witnesses are the triplet algebras $\cW(p)$ for $p
\geq 2$ and the symplectic-fermion pair at $c = -2$. Class~$M^\ast$
shares the mixed-shadow algebraic invariants of class~$M$ but is
distinguished by Gurarie--Flohr logarithmic amplitudes
(Gurarie 1993 arXiv:hep-th/9303160; Flohr 1996
arXiv:hep-th/9605151): Zhu-finite yet Massey-unbounded. The
non-logarithmic single-line dichotomy applies to the algebraic
invariants, which are unaffected by logarithmic amplitudes;
tempering of the shadow tower is blocked.

\begin{definition}[Class~$W$]\label{def:class-W-sa}
A chirally Koszul algebra $\cA$ belongs to class~$W$ if it is
realised as a screening kernel
$\cA = \bigcap_i \ker Q_{\alpha_i}$
inside a rank-$(N-1)$ Heisenberg algebra, for some
finite family of screening operators $\{Q_{\alpha_i}\}$.
\end{definition}

Canonical witnesses are the principal $\cW_N$-algebras as
Feigin--Frenkel--Wakimoto kernels inside $\cH^{\oplus(N-1)}$,
the $\beta\gamma\phi$ Wakimoto realisation of
$\widehat{\mathfrak{sl}}_2$, and admissible affine algebras
through parabolic screenings (KRW03, Arakawa07). Class~$W$ is
not disjoint from $G,L,C,M$: a class~$M$ algebra realised as a
Wakimoto kernel inherits Koszulity of the ambient class~$G$
parent, and the Drinfeld coproduct descends from the free-field
parent to the intersection.

\begin{remark}[Seven-stratum fingerprint]
\label{rem:seven-stratum-fingerprint}
The stratification is $\{G, L, C, M, \mathrm{FF}, M^\ast, W\}$.
Classes~$G,L,C,M$ are determined by the shadow metric on the
non-logarithmic non-critical locus. FF is the $\kappa = 0$
boundary. $M^\ast$ records Massey amplitude behaviour
orthogonal to $(\alpha, \Delta)$. $W$ records a presentation
overlay: a class-L or class-M algebra may also be class-W.
\end{remark}

% ================================================================
\section{The universal shadow-series exponential base}
\label{sec:universal-exponential-base}
% ================================================================

\begin{theorem}[Universal $C_\cA = 6$]
\label{thm:universal-C-A-6-trichotomy}
\ClaimStatusProvedHere
Let $\cA$ be non-logarithmic class~M chirally Koszul with a
Virasoro subalgebra at generic central charge. The shadow series
$\Sigma_\cA(z) = \sum_{r \geq 2} S_r(\cA)\, z^r$
has radius of convergence exactly $1/6$:
$C_\cA := \limsup_r |S_{r+1}/S_r| = 6$.
...
\tfrac{4z^3}{9} - \tfrac{z^2}{9} + \tfrac{z}{27}
 - \tfrac{\log(1 + 6z)}{162}.
\]
Pole-doubling at all derivatives:
$\Sigma_\Vir^{(k)}(z) = [P_k + Q_k \log(1+6z)]/(1+6z)^{2k}$.
\end{theorem}

\begin{remark}[Decomposition $6 = 3 \cdot 2$]
\label{rem:6-decomposition}
The base~$6$ factors as the product of the bar-depth seed
$r_0 = 3$ (first non-vanishing shadow degree beyond $\kappa$)
and the Riccati seed coefficient $S_{r_0} = 2$
(equation~\eqref{eq:s3-vir}). The same universality extends to
$\cW_N$ at leading~$1/c$; sub-leading corrections
$\beta_N = (N+1)(N+2)/2$ asymptote into the universal base.
\end{remark}

\begin{proposition}[Picard--Fuchs for the shadow series]
\label{prop:picard-fuchs-shadow-trichotomy}
$\Sigma_\cA(z)$ satisfies a second-order Picard--Fuchs equation
with regular-singular point at $z = -1/6$; the local monodromy
is the reflection $z \mapsto -z/(1+6z)$, realising the
Borel-plane involution on the shadow tower.
\end{proposition}
