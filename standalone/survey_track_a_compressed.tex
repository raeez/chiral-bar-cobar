...
\chapter*{Track A: The Bar Construction and the Genus Tower}
\addcontentsline{toc}{chapter}{Track A}
...
logarithmic normal form along each collision diagonal, represented by
\(d\log(z_i-z_j)\) only on affine/formal genus-zero screens, and the
bar differential becomes an integral transform on the Fulton--MacPherson
...
The ordered bar is the level-$2$ chain object before averaging. The
...
Place sections \(a_1,\dots,a_n\in\cA\) at distinct points of \(X\).
On an affine/formal genus-zero screen with coordinates
\(z_1,\dots,z_n\), represent the logarithmic normal form along
\(D_{ij}\) by \(\eta_{ij}=d\log(z_i-z_j)\). For a spanning tree \(T\) of the
...
quasi-periodicity and the fiberwise model becomes curved:
$m_1^2=[m_0^{(g)},-]$, while the scalar diagonal projection of
$m_0^{(g)}$ is $\kappa(\cA)\cdot\omega_g$. Genus~$0$ is uncurved algebra;
...
property of the programme and is organised by twelve tests and
consequences in~\S9.

A quadratic presentation $\cA=T_X(V)/(R)$ supplies the coalgebra
\[
\cA^{\mathrm i}=C_X(s^{-1}V,s^{-2}R)
\]
and the canonical comparison
$q_\cA\colon\cA^{\mathrm i}\to B_X(\cA)$.  Quadratic Koszulness is the
quasi-isomorphism of~$q_\cA$; its cohomology map identifies
$H^\bullet(\cA^{\mathrm i})$ with bar cohomology.

The geometric dual operation acts on the full bar coalgebra. Verdier
duality on $\mathrm{Ran}(X)$ forms the factorization algebra
\[
K_X(\cA):=\mathbb D_{\Ran}\barB_X^\Sigma(\cA).
\]
The transpose
$\mathbb D(q_\cA)\colon K_X(\cA)\to\mathbb D_{\Ran}(\cA^{\mathrm i})$
compares it with the finite presentation dual, and a chosen Koszul-pair
map $\nu_\cA\colon K_X(\cA)\to\cA^!$ reaches the strict partner.
Self-dual fixed points, such as $\mathrm{Vir}_{c=13}$, are loci where
the named pair equivalence identifies that partner with the original
family member.
...
\[
\mathcal H_k^!=\bigl(\operatorname{Sym}^{\mathrm{ch}}(V^*[1]),\;m_0=-k\,\omega\bigr),
\]
...
The ordered bar carries the collision kernel
$K_{\mathcal H}^{\mathrm{coll}}(z)=k\Omega_{\mathcal H}/z$: the second-order OPE pole
$k/(z-w)^2$ drops by one order under the $d\log$ kernel. Applying \eqref{eq:av-organizing},
$q_2(K_{\mathcal H}^{\mathrm{coll}})$ remains the rank-one operator
and $\tau_{\mathcal H,2}^{\mathrm{res}}
q_2(K_{\mathcal H}^{\mathrm{coll}})=k=\kappa(\mathcal H_k)$.
Its Fock representation supplies the scalar $R$-matrix. For the
affine Kac--Moody algebra $\widehat{\mathfrak g}_k$, the ordered
collision kernel has the level-full Casimir form
\[
K_{k,\mathfrak g}^{\mathrm{coll}}(z)
\;=\;k\,\Omega_{\mathrm{tr}}/z,\qquad
\Omega=\sum_a J^a\otimes J_a.
\]
The homotopy coinvariant retains the Casimir operator and the
normalized functional gives its trace:
\[
\tau_{k,\mathfrak g}^{\mathrm{res}}
q_2(K_{k,\mathfrak g}^{\mathrm{coll}})
...
\end{equation}
(Theorem~\ref{thm:multi-weight-genus-expansion}).  The scalar-diagonal
graph package sets $\delta F_g^{\mathrm{cross}}=0$.
...
\textup{(i)--(v), (ix)--(x)}, with \textup{(vi)} a proved consequence
of \textup{(v)}, \textup{(vii)} a conditional factorization-homology
comparison, and \textup{(viii)} a one-way chiral Hochschild
consequence on the Koszul locus; one shifted-symplectic-package
conditional criterion; one one-directional implication) characterise
this locus. The Koszulness programme
(\S9) proves the core equivalences from PBW concentration, bar-cobar
inversion, and Koszul-complex acyclicity.  Theorem~H is a
...
\[
\mathrm B^{\log}_{\mathrm{mod}}(\cA)(g,n)=\bigoplus_{\Gamma\in\mathsf{Gr}^{\mathrm{st}}_{g,n}}\left(\bigotimes_{v\in V(\Gamma)}\bar B^{\mathrm{ch}}(A^{\mathrm{ch}}_\infty;\operatorname{In}(v))\otimes C_\bullet(\mathrm{FM}^{\log}_\Gamma)\otimes\overline{E_{\mathrm{int}}(\Gamma)}\right)_{\!\operatorname{Aut}(\Gamma)}.
\]
...
$\overline{E_{\mathrm{int}}(\Gamma)}=\det(\Bbbk E(\Gamma))\otimes\det(H_1(\Gamma;\Bbbk))^{-1}$;
...
(the four stages $\operatorname{Obs}^{\mathrm{def}}_g$,
$\mathfrak O_g^K$, $\operatorname{ch}_g(\mathfrak O_g^K)$, and
$F_g$, related by $H_D^1,H_D^K,H_D^{\mathrm{tr}},H_D^{\mathrm{graph}}$),
and Theorem~H (chiral
Hochschild support
$\operatorname{Supp}\ChirHoch^\bullet(\cA)\subseteq S$ under the
family datum $H_H(\cA;S)$) are the five
...
adds $\delta F_g^{\mathrm{cross}}$). Theorem~H identifies chiral
Hochschild cohomology with its declared finite-support model.
...
In three dimensions, an OCA comparison datum extends $\Theta_\cA$ to
the open/closed Maurer--Cartan element
$\Theta^{\mathrm{oc}}_\cA=\Theta_\cA+\sum_j\mu^{M_j}$
of the holomorphic-topological QFT on
...
(Volume~II).  The open--closed comparison map sends this algebraic
closed sector to the physical bulk observables.
...
local kernel entering the residue part of \(D_\cA\), while projective
and positive-genus screens use the corresponding replacement data.
Three structure theorems emerge. \emph{Algebraicity}: on
...
\textup{(vi)} a consequence of \textup{(v)}, \textup{(vii)} a
conditional factorization-homology comparison, \textup{(viii)} a
one-way $H_H(\cA;S)$-conditional chiral Hochschild support consequence,
\textup{(xi)} conditional on the shifted-symplectic package, and
\textup{(xii)} one-directional).
...
formality), operadic algebra (FM acyclicity), modular geometry
(conditional FH concentration), and symplectic topology (conditional
Lagrangian criterion). Seven are independent bidirectional
equivalences; Barr--Beck--Lurie is a listed consequence; ChirHoch is
one-way on the Koszul locus; $\mathcal D$-module purity is
...
& (viii) chiral Hochschild support in $S$ &
one-way consequence under $H_H(\cA;S)$ & proved\\
...
The programme extends to non-principal $\mathcal W$-algebras with an
explicit status split. The Bershadsky--Polyakov algebra
$\mathrm{BP}_k$ has proved canonical strictness and computed
OPE/bar witnesses at generic level ($k\neq-3$). Its same-family
Koszul companion $\mathrm{BP}_{-k-6}$ and conductor
$K^c_{\mathrm{BP}}=50$ are the conditional output of the subregular
DS--bar transport package, not a consequence of DS reduction alone.
Nilpotent transport from hook-type seeds gives computed graph evidence
through $\mathfrak{sl}_8$; the all-orbit Koszul-duality transport
remains conditional.
...
cohomology]\label{thm:track-a-theorem-h}
Fix a finite set \(S\subset\mathbb Z_{\ge0}\) and a family datum
\(H_H(\cA;S)\).  A strong deformation retract
\[
...
\]
onto a complete model supported in~\(S\) gives
\[
\ChirHoch^n(\cA)\cong H^n(K_{\cA,S}),\qquad
\operatorname{Supp}\ChirHoch^\bullet(\cA)\subseteq S.
\]
The package includes filtration convergence and ordered-to-symmetric
comparison; a bar presentation includes
\(\operatorname{Conv}(B_X(\cA),\cA)\to
C^\bullet_{\mathrm{ch}}(\cA,\cA)\).
\end{theorem}

The BDSK superboson bounded model has support \(\{0,1\}\) and
dimensions \((2,1)\).  The bounded Virasoro model has support
\(\{0,2,3\}\), with one-dimensional supported groups, and reaches the
complete chart through a named bounded-to-chart quasi-isomorphism.
A perfect degree-\(d\) pairing is a separate duality package.
