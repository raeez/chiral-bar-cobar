...
monodromy valued in the braid group.  The quantum loop algebra
$U_{q_{\mathrm{KL}}}(L\fg)$ has a universal $R$-matrix that also furnishes
braid representations.  That these representations coincide
on evaluation modules is the Drinfeld--Kohno theorem.  The ordered
chiral bar complex $B^{\mathrm{ord}}(V^k(\fg))$ provides the bridge:
its collision residue
$r^{\mathrm{coll}}(z) = k\Omega_{\mathrm{tr}}/z$ (trace-form
convention; the level prefix $k$ is essential and
$r^{\mathrm{coll}}|_{k=0} = 0$) is the ordered OPE datum, while the
KZ connection uses the separately normalised spectral datum
$r^{\mathrm{KZ}}(z)=\Omega_{\mathrm{KZ}}/((k+h^\vee)z)$.  These are
two normalisations of the affine monodromy datum, not the same
rational function of $k$.  The bar differential encodes the chiral
...
The comparison has four stages.
...
associative structure with spectral parameter on the stated
evaluation-generated core.  At DK-2 and DK-3, the generated-core
comparison identifies the dg-shifted Yangian surface and verifies the
strictification obstruction on simple types.  The full
factorisation/category extension remains a named proof obligation.
We verify the bridge on the Heisenberg algebra (where the
...
\begin{remark}[Convention for $q$: Kazhdan--Lusztig vs.\ Drinfeld]
\label{rem:dk-q-convention}
This paper uses two closely related but distinct deformation
parameters, and we fix the convention once and for all.
\begin{itemize}
\item \textbf{Half-braiding (Kazhdan--Lusztig).} We set
\[
  q_{\mathrm{KL}} \;:=\; \exp\!\bigl(\pi i/(k+\hv)\bigr).
\]
Half-braiding computations and eigenvalue tables use
$q_{\mathrm{KL}}$.
\item \textbf{Full pure-loop monodromy.} We set
\[
  q_{\mathrm{DK}} \;:=\; q_{\mathrm{KL}}^2
  = \exp\!\bigl(2\pi i/(k+\hv)\bigr).
\]
Full pure-braid monodromy displays use \(q_{\mathrm{DK}}\).
\end{itemize}
Thus every occurrence of a \(q\)-parameter below is explicitly
subscripted when the distinction matters.
\end{remark}
...
Independently, the quantum group $U_{q_{\mathrm{KL}}}(\fg)$ at
$q_{\mathrm{KL}} = e^{\pi i/(\kp)}$ produces braid group representations
from its universal $R$-matrix
$\cR \in U_{q_{\mathrm{KL}}}(\fg)^{\widehat{\otimes}\,2}$.  On any
collection of finite-dimensional $\fg$-modules
$V_1, \ldots, V_n$, the braided monoidal structure of
$\Rep_{q_{\mathrm{KL}}}(\fg)$ defines a representation
$\rho_n^{U_{q_{\mathrm{KL}}}} \colon B_n
\to \GL(V_1 \otimes \cdots \otimes V_n)$.
...
\subsection{The four bridge assertions}

The comparison consists of four assertions: DK-0 identifies KZ
monodromy with the quantum-group $R$-matrix on evaluation modules;
DK-1 strictifies the spectral $\Ainf$ bar product; DK-2 identifies the
strict algebra with the dg-shifted Yangian; DK-3 proves type-uniform
vanishing of the strictification obstruction.

\subsection{The pure-braid Orlik--Solomon route to Koszulity}
\label{sec:dk-os-route}

Arnold~\cite{Arnold69} and Orlik--Solomon~\cite{OS80} identify the
cohomology algebra of the pure-braid arrangement, and
Shelton--Yuzvinsky~\cite{SheltonYuzvinsky97} prove Koszulity for
supersolvable arrangements. The pure-braid arrangement
$\mathcal{A}_n^{\mathrm{PB}}$ (hyperplanes $z_i = z_j$) is
supersolvable by induction on $n$, hence the pure-braid
Orlik--Solomon algebra $\mathrm{OS}(\mathfrak{t}_n)$ is Koszul. This
is the purely combinatorial input on which the chiral
Drinfeld--Kohno programme rests.

\begin{theorem}[Pure-braid Orlik--Solomon flatness input
route]\label{thm:dk-os-koszul}
Let $\fg$ be simple and $V^k(\fg)$ the universal affine vertex
algebra at non-critical level. Then:
\begin{enumerate}[label=\textup{(\roman*)}]
 \item $\mathrm{OS}(\mathfrak{t}_n)$ controls the configuration-space
 $d\log$ forms entering the ordered bar differential;
 \item the collision residue $r(z) = k\Omega_{\mathrm{tr}}/z$
 (trace-form) solves CYBE in the Knizhnik--Drinfeld sense;
 \item affine bar concentration requires a separate PBW/Koszul input.
\end{enumerate}
The OS algebra supplies the flatness/CYBE input; it does not by itself
compute \(H^\bullet(B^{\mathrm{ord}}(V^k(\fg)))\).
\end{theorem}

\begin{lemma}[$R$-twisted $\Sigma_n$-descent]\label{lem:dk-r-twist-descent}
Let $\cA$ be an ordered-Koszul chiral algebra with $R$-matrix
$R(z)$ satisfying the quantum Yang--Baxter equation and normalized
unitarity $R(z)R^{\mathrm{op}}(-z)=\id$. The ordered bar complex
$B^{\mathrm{ord}}(\cA)$ descends to
the symmetric bar $B^\Sigma(\cA)$ via the $R$-twisted
$\Sigma_n$-coinvariant functor
\[
 \mathrm{av}^R \colon B^{\mathrm{ord}}(\cA) \;\longrightarrow\;
 B^\Sigma(\cA), \qquad
 a_1 \otimes \cdots \otimes a_n \;\mapsto\;
 \frac{1}{n!}\sum_{\sigma \in \Sigma_n} R_\sigma(z) \cdot
 \sigma(a_1 \otimes \cdots \otimes a_n),
\]
where $R_\sigma(z)$ is the spectral-$R$-matrix cocycle associated to
the permutation $\sigma$. The plain averaging map $\mathrm{av}$
(with $R_\sigma = 1$) is the classical limit $\hbar \to 0$. The
$R$-twist factor is necessary for the descent to produce a chain map
at $\hbar \neq 0$; setting $R = 1$ collapses the quantum-group
structure.
\end{lemma}

\begin{corollary}[Chiral Kac--Kazhdan formal smoothness]
\label{cor:dk-chiral-kk-smooth}
Assume the Francis--Gaitsgory bar-cobar $(\infty,2)$-equivalence has
been lifted to the properad level and that the affine PBW/Koszul
concentration input holds.  Then the pure-braid Orlik--Solomon
flatness input (Theorem~\ref{thm:dk-os-koszul}) and the $R$-twisted
$\Sigma_n$-descent (Lemma~\ref{lem:dk-r-twist-descent}) give formal
smoothness for conilpotent-complete factorization deformations on the
Koszul locus.
\end{corollary}

\begin{remark}[Four Yangian types must be kept distinct]
\label{rem:four-yangian-types-dk}
The Drinfeld--Kohno bridge separates four Yangian incarnations:
\begin{itemize}
 \item DK-0 is a statement about $Y_\hbar(\fg)$ (classical).
 \item DK-1 and DK-2 produce $Y^{\mathrm{dg}}_\hbar(\fg)$ (dg-shifted).
 \item The chiral-bar incarnation along this paper is
 $Y_\hbar(\fg)^{\mathrm{ch}}$ (chiral; $E_1$-chiral on $X$).
 \item $Y_\hbar(\fg)^{\mathrm{spec}}$ (spectral) is the
 transposed-coordinate presentation used in the toroidal and CoHA
 incarnations; not touched by DK-0 through DK-3.
\end{itemize}
...
own derived centre and its own $R$-matrix datum.
\end{remark}
...
\item \emph{Trace-form collision residue:}
...
\item \emph{KZ spectral normalization:}
  $r^{\mathrm{KZ}}(z) = \Omega_{\mathrm{KZ}}/((\kp)z)$.
  At $k = 0$, the $r$-matrix is $\Omega/(\hv z) \neq 0$
  for non-abelian $\fg$ (the Lie bracket persists).
  At $k = -\hv$, this normalization has a pole; the trace-form
  $r$-matrix is separate, and the critical Sugawara vector is undefined.
\end{enumerate}
There is no identity \(k\Omega_{\mathrm{tr}}=\Omega_{\mathrm{KZ}}/\kp\)
as rational functions of \(k\).  Passing from one display to the other
means changing the normalised invariant form and the monodromy
normalisation.
...
collision kernel $K_\cA^{\mathrm{coll}}(z)$ has pole of order $p - 1$.
...
For the affine algebra $V^k(\fg)$, the KZ-normalized Kohno
connection is
...
\end{equation}
where \(\Omega_{ij}\) is the KZ-normalised Casimir.  The trace-form
collision residue remains \(r^{\mathrm{coll}}(z)=k\Omega_{\mathrm{tr}}/z\).
\end{proposition}

\begin{proof}
The trace-form collision residue gives the ordered OPE connection
\[
\nabla_{\mathrm{coll}}
\;=\;
d
...
\frac{dz_{ij}}{z_{ij}}\,.
\]
The KZ equation uses the separately normalized spectral datum
\(\Omega_{\mathrm{KZ}}/\kp\), giving~\eqref{eq:dk-kz}.  This is the
Knizhnik--Zamolodchikov connection~\cite{KZ84}.  The $d\tau$ term of the
...
\Bigr)
\;=\;
\exp\!\left(\frac{2\pi i}{\kp}\,\Omega_{12}\right)\,,
\end{equation}
...
\]
The braid relations are obtained from flat KZ monodromy together with
...
and $R_{12} = e^{2\pi i k\Omega_{12}}$ in the trace
normalization where $r_{\mathrm{tr}}(z) = k\Omega_{\mathrm{tr}}/z$.
In the KZ normalization where
$r_{\mathrm{KZ}}(z) = \Omega/((\kp) z)$, the monodromy reads
\[
 \Mon_\gamma^{\mathrm{KZ}}
 \;=\;
 \exp\!\left(\frac{2\pi i}{\kp}\,\Omega_{12}\right)
 \;=\;
 q_{\mathrm{DK}}^{\Omega_{12}},
 \qquad
 q_{\mathrm{DK}} := \exp\!\left(\frac{2\pi i}{\kp}\right).
\]
The factor of $2\pi i$ in the exponent is \emph{structural},
not a contour normalization: it governs the braiding monodromy
and identifies the full pure-loop parameter \(q_{\mathrm{DK}}\).
The half-braiding parameter is
\(q_{\mathrm{KL}}=\exp(\pi i/\kp)\), so
\(q_{\mathrm{DK}}=q_{\mathrm{KL}}^2\).

The Yang--Baxter equation is a monodromy consequence of the flat KZ
connection and the Drinfeld associator.  It is not obtained by a bare
formal exponentiation of the CYBE.
\end{proof}
...
${\mathrm{Core}}_{\mathrm{eval}}(\widehat{\mathfrak{g}}_k)$,
...
monoidal structure of $\Rep_{q_{\mathrm{KL}}}(\fg)$ via the universal
$R$-matrix of $U_{q_{\mathrm{KL}}}(\fg)$:
\begin{equation}\label{eq:dk0}
\rho_n^{\mathrm{KZ}}
\;\simeq\;
\rho_n^{U_{q_{\mathrm{KL}}}(\fg)},
\qquad q_{\mathrm{KL}}=e^{\pi i/\kp}.
\end{equation}
\end{theorem}
...
(with KZ braiding) and $\Rep_{q_{\mathrm{KL}}}(\fg)$ (with $R$-matrix
braiding) at
\[
q_{\mathrm{KL}}=\exp(\pi i\hbar_{\mathrm{ref}}),
\qquad
\hbar_{\mathrm{ref}}=\kp^{-1}.
\]
...
operations $m_k$ ($k \geq 3$) vanish on the
evaluation-generated core
${\mathrm{Core}}_{\mathrm{eval}}(\widehat{\mathfrak{g}}_k)$.
(This is evaluation-core one-loop exactness of the BV--BRST
...
$\rho_n^{\mathrm{KZ}} \simeq \rho_n^{U_{q_{\mathrm{KL}}}(\fg)}$.
\end{proof}
...
commutes on the evaluation-generated core
${\mathrm{Core}}_{\mathrm{eval}}(\widehat{\mathfrak{g}}_k)$:
...
& D^b(\Rep_{\mathrm{fd}}^{\mathrm{eval}}(Y_{-\hbar}(\fg)))
 \ar[d, "\Phi_K"]
\\
D^b(\Rep_{\mathrm{fd}}(U_{q_Y}(L\fg)))
 \ar[r, "{q_Y\mapsto q_Y^{-1},\,z\mapsto z^{-1}}"']
& D^b(\Rep_{\mathrm{fd}}(U_{q_Y^{-1}}(L\fg)))
\end{tikzcd}
\end{equation}
where:
\begin{enumerate}[label=(\roman*)]
\item $\Phi$ is the Verdier/linear-dual functor on the ordered Koszul
  coalgebra; bar-cobar supplies the separate inversion witness. It reverses
  the braiding for the additive Yangian parameter:
  $R(u; \hbar_Y) \mapsto R^{-1}(u; \hbar_Y)
  = R(u; -\hbar_Y)$.
\item $\Phi_K$ is the evaluation-module Kazhdan--Drinfeld comparison
  $\Rep_{\mathrm{fd}}(Y_{\hbar_Y}(\fg))
  \xrightarrow{\sim}
  \Rep_{\mathrm{fd}}(U_{q_Y}(L\fg))\big|_{q_Y = \exp(\hbar_Y)}$,
  the monoidal equivalence that sends the rational
  $R$-matrix of the Yangian to the trigonometric
  $R$-matrix of the quantum loop algebra.
\item The bottom arrow is the Hopf algebra involution with loop
  inversion, sending
  $\cR_{q_Y}(z) \mapsto \cR_{q_Y}^{-1}(z)
  = \cR_{q_Y^{-1}}(z^{-1})$.
\end{enumerate}
...
$q_Y \mapsto q_Y^{-1}$ with $z\mapsto z^{-1}$ is realized by Verdier duality, which
...
The left vertical sends $V(a)_\hbar$ to the quantum-loop evaluation
module $V_{q_Y}(e^a)$.  The bottom arrow sends
$V_{q_Y}(e^a) \mapsto V_{q_Y^{-1}}(e^{-a})$.

Following the right vertical first:
$V(-a)_{-\hbar} \mapsto V_{q_Y^{-1}}(e^{-a})$.
Thus the square commutes before forgetting the loop evaluation point.
...
\item quantum Casimir eigenvalues \(q_{\mathrm{KL}}^{C_2}\) at
  \(q_{\mathrm{KL}} = e^{\pi i/(k+2)}\);
...
$R_1 = q_{\mathrm{KL}}^{1/2}$, where
$q_{\mathrm{KL}} = e^{i\pi/(k+2)}$.
...
For any simple Lie algebra $\fg$, on the
evaluation-generated core
${\mathrm{Core}}_{\mathrm{eval}}(\widehat{\mathfrak{g}}_k)$
the spectral Drinfeld
...
\begin{remark}[Full-category extension]
\label{rem:dk-eval-core-full-category}
The eval-core restriction in
Theorems~\ref{thm:dk0}, \ref{thm:dk-square},
\ref{thm:dk-strictification}, and~\ref{thm:dk-yangian-koszul}
is part of the statement: the bar-cobar identifications are proved on
${\mathrm{Core}}_{\mathrm{eval}}(\widehat{\mathfrak{g}}_k)$,
where the MC3 cocycle conditions are unconditional.
The full smooth-module category requires a separate thick-generation
theorem.
\end{remark}
...
Yangian $Y_{\hbar_Y}(\fg)$ at the additive Yangian parameter
\[
\hbar_Y:=\frac{\pi i}{\kp}.
\]
...
$m_{\hbar_Y}(u)$, and the monodromy of the strictified
connection is the Yangian $R$-matrix $R^Y(u)$.
\end{corollary}

\begin{proof}
The strictified product satisfies the spectral associativity
$(m_{\hbar_Y} \otimes \id) \circ m_{\hbar_Y}
= (\id \otimes m_{\hbar_Y}) \circ m_{\hbar_Y}$
...
Yangian $Y_{\hbar_Y}(\fg)$.  The identification with the
standard Yangian follows from Drinfeld's new
realization~\cite{Drinfeld88}.
\end{proof}
...
For $\fg$ simple and $k \neq -\hv$ generic, on the
evaluation-generated core
${\mathrm{Core}}_{\mathrm{eval}}(\widehat{\mathfrak{g}}_k)$
the $\Eone$-chiral Koszul duality functor applied to the
...
\textbf{Yangian side} ($\Ydg_{\hbar_Y}(\fg)$) \\
\midrule
Chiral OPE ($\Einf$-chiral) &
Spectral product ($\Eone$-topological) \\
Level $k$ &
Additive Yangian parameter $\hbar_Y = \pi i/(\kp)$ \\
KZ monodromy &
$R$-matrix \\
Affine Weyl group &
Quantum Weyl group \\
...
\begin{proof}
The $\Eone$-chiral Koszul coalgebra of $\cA$ is
$(V^k(\fg))^{i,\Eone}=H^*(B^{\mathrm{ord}}(V^k(\fg)))$.
The dual algebra $(V^k(\fg))^{!,\Eone}$ is obtained only after
finite/Verdier duality and spectral strictification, using the
algebra structure from the deconcatenation coproduct and the spectral
product from
Theorem~\ref{thm:dk-strictification}.
...
\;\simeq\;
\operatorname{Fact}_{\Eone}(U_{q_{\mathrm{KL}}}(\fg))^{\op},
\]
...
$U_{q_{\mathrm{KL}}}(\mathrm{Heis})$ is commutative, the $R$-matrix is
...
$\kappa(\cH_k)=k$. The homotopy coinvariant
$q_2(K_{\cH_k}^{\mathrm{coll}})$ retains the rank-one operator, and
$\tau_{\cH_k,2}^{\mathrm{res}}q_2(K_{\cH_k}^{\mathrm{coll}})=k$.
The Fock representation supplies the scalar $r$- and $R$-operators.
...
The trace-form collision residue for $\widehat{\mathfrak{sl}}_2$ at
level~$k$ is
\[
r^{\mathrm{coll}}(z)
\;=\;
\frac{k\,\Omega}{z}
\]
in the trace form.  The KZ connection below uses the separately
normalised spectral datum $\Omega/((k+2)z)$.
...
fundamental representation, the KZ Casimir exponent acts with
eigenvalue \(1/2\) on the symmetric triplet and \(-3/2\) on the
antisymmetric singlet.  The full pure-loop KZ monodromy is
\begin{equation}\label{eq:dk-sl2-r}
\Mon^{\mathrm{KZ}}
\;=\;
\exp\!\left(\frac{2\pi i}{k+2}\,\Omega\right)
\;=\;
q_{\mathrm{KL}}^{2\Omega},
\qquad
q_{\mathrm{KL}} \;=\; \exp\!\left(\frac{\pi i}{k+2}\right),
\end{equation}
so that on the singlet-triplet decomposition
\begin{equation}\label{eq:dk-sl2-r-matrix}
\Mon^{\mathrm{KZ}}
\;=\;
\operatorname{diag}(
\underbrace{q_{\mathrm{KL}}^{1/2}, q_{\mathrm{KL}}^{1/2},
q_{\mathrm{KL}}^{1/2}}_{\text{triplet}},\;
\underbrace{q_{\mathrm{KL}}^{-3/2}}_{\text{singlet}}
).
\end{equation}
The braid operator includes the permutation:
\begin{equation}\label{eq:dk-sl2-eigenvalues}
P\,q_{\mathrm{KL}}^{C_{12}}
\;=\;
\operatorname{diag}(
\underbrace{q_{\mathrm{KL}}^{1/2},q_{\mathrm{KL}}^{1/2},
q_{\mathrm{KL}}^{1/2}}_{\text{triplet}},\;
...
\end{equation}
where \(C_{12}=C_2(j)-C_2(1/2)-C_2(1/2)\) on the \(j\)-channel.
The minus sign is the antisymmetric action of \(P\), not a KZ
monodromy eigenvalue.  The relative Hecke-braiding phase is
\(-q_{\mathrm{KL}}^{-2}\).
...
$q_{\mathrm{KL}} = e^{i\pi/(k+2)}$, and the trace requires the Markov
...
(writhe correction via \(q_{\mathrm{KL}}^{\pm w(K) \cdot C_2}\) and
division by the quantum dimension
$[n]_{q_{\mathrm{KL}}}
= (q_{\mathrm{KL}}^{n/2} - q_{\mathrm{KL}}^{-n/2})/
(q_{\mathrm{KL}}^{1/2} - q_{\mathrm{KL}}^{-1/2})$)
...
eigenvalues are \(q_{\mathrm{KL}}^{1/2}\) and
\(-q_{\mathrm{KL}}^{-3/2}\), the associator
...
collision residue $r^{\mathrm{coll}}(z) = k\Omega_{\mathrm{tr}}/z$ is the degree-$2$
...
Invent.\ Math.\ \textbf{56} (1980), 167--189.
