...
\begin{itemize}
\item \textbf{Canonical (Kazhdan--Lusztig).} We set
\[
  q \;:=\; \exp\!\bigl(2\pi i/(k+\hv)\bigr).
\]
All $R$-matrices, braiding monodromies, quantum-group
identifications ($U_q(\fg)$, $\Rep_q(\fg)$, Kazhdan--Lusztig
equivalence) and the default statements of DK-0, DK-1, DK-2, DK-3
use this $q$ throughout.
\item \textbf{Drinfeld.} Where the half-monodromy is displayed
directly (singlet/triplet eigenvalue tables, half-braiding
computations), we write
\[
  q_{\mathrm{D}} \;:=\; \exp\!\bigl(\pi i/(k+\hv)\bigr),
  \qquad
  q \;=\; q_{\mathrm{D}}^{2}.
\]
The Drinfeld variable $q_{\mathrm{D}}$ appears ONLY in displays
explicitly marked with the subscript $\mathrm{D}$.
\end{itemize}
\textbf{Bridge to \texttt{en\_koszul\_duality.tex}.} Vol I's
\texttt{en\_koszul\_duality} uses the Kazhdan--Lusztig convention
throughout; earlier drafts of the present paper had used both.
After this note, every new occurrence of $q$ defaults to
Kazhdan--Lusztig unless explicitly marked $q_{\mathrm{D}}$.
\end{remark}
...
The Shelton--Yuzvinsky theorem asserts that the Orlik--Solomon
algebra $\mathrm{OS}(\mathcal{A})$ of any supersolvable arrangement
$\mathcal{A}$ is Koszul. The pure-braid arrangement
...
\begin{theorem}[Pure-braid Orlik--Solomon Koszulity
...
\begin{enumerate}[label=\textup{(\roman*)}]
 \item $H^\bullet(B^{\mathrm{ord}}(V^k(\fg))) \cong
 \mathrm{OS}(\mathfrak{t}_n) \otimes V^k(\fg)^{\otimes n}$ as
 $\Sigma_n$-twisted graded objects (the ordered bar concentrates
 in weight $1$ on cohomology; PBW collapse);
 \item the collision residue $r(z) = k\Omega_{\mathrm{tr}}/z$
 (trace-form) solves CYBE in the Knizhnik--Drinfeld sense;
 \item the Cherednik monodromy representation of the pure braid
 group factors through $H^\bullet(B^{\mathrm{ord}}(V^k(\fg)))$.
\end{enumerate}
Consequence: the ordered-Koszul property of $V^k(\fg)$ is
witnessed combinatorially by $\mathrm{OS}(\mathfrak{t}_n)$
Koszulity, without analytic input beyond $d\log$ absorption.
\end{theorem}
...
Combined with the Francis--Gaitsgory bar-cobar $(\infty,2)$-equivalence
lifted to the properad level, the pure-braid Orlik--Solomon
Koszulity route (Theorem~\ref{thm:dk-os-koszul}) and the $R$-twisted
$\Sigma_n$-descent (Lemma~\ref{lem:dk-r-twist-descent}) imply that
the chiral Kac--Kazhdan category is formally smooth at the properad
level: every conilpotent-complete factorization deformation of a
Koszul-locus chiral algebra extends to all orders.
\end{corollary}

\begin{remark}[Four Yangian types must be kept distinct]
\label{rem:four-yangian-types-dk}
The Drinfeld--Kohno bridge relates three of the four Yangian notions
(Definition~\ref{def:four-yangian-types-3hbar} in the companion
paper on three-parameter $\hbar$):
\begin{itemize}
...
In the Kazhdan--Lusztig (KZ) normalization where
...
\[
 ...
 q^{\Omega_{12}},
 \qquad
 q := \exp\!\left(\frac{2\pi i}{\kp}\right).
\]
The factor of $2\pi i$ in the exponent is \emph{structural},
not a contour normalization: it governs the braiding monodromy
and identifies the Kazhdan--Lusztig root of unity $q$ at integer
level. In particular, at level $k$ a positive integer, $q$ is
a primitive $(\kp)$-th root of unity and the braiding restricts
to the quantum-group $\mathrm{Rep}_q(\mathfrak{g})$ on the
evaluation-generated core (Kazhdan--Lusztig equivalence).
...
${\mathrm{Core}}_{\mathrm{eval}}(\widehat{\mathfrak{g}}_k)$.
(This is the one-loop exactness of the BV--BRST
differential on the eval core: only the quadratic part of
the chiral action contributes; see Vol~I,
Theorem~\ref{V1-thm:chiral-mc3-eval-core} in
\texttt{five\_theorems\_modular\_koszul.tex}.)  The full superconnection on the bar complex
...
is : the bar-cobar identifications are proved on
${\mathrm{Core}}_{\mathrm{eval}}(\widehat{\mathfrak{g}}_k)$,
where the MC3 cocycle conditions are unconditional.
Extension to the full category of smooth $V^k(\fg)$-modules
(beyond evaluation modules and their tensor generates) is
conjectural; the downstream DK-4 and DK-5 identifications
are correspondingly conditional on this extension.
\end{remark}
...
$j = 0$) component.  The KZ monodromy is
...
\;=\;
\operatorname{diag}(
\underbrace{q_{\mathrm{D}}^{1/2}, q_{\mathrm{D}}^{1/2}, q_{\mathrm{D}}^{1/2}}_{\text{triplet}},\;
\underbrace{-q_{\mathrm{D}}^{-3/2}}_{\text{singlet}}
),
\end{equation}
written in the Drinfeld normalization $q_{\mathrm{D}} = e^{i\pi/(k+2)}$
so that $q_{\mathrm{D}}^2 = q$ equals the Kazhdan--Lusztig root of
unity used in equations~\eqref{eq:dk-sl2-r}--\eqref{eq:dk-sl2-r-matrix}.
The relative phase between the singlet and triplet is $-q_{\mathrm{D}}^{-2} = -q^{-1}$,
which is the fundamental building block of the colored Jones polynomial.
