...
and boundary algebra, not on the bar complex. We construct the
...
$r(z) = k\,\Omega/z$ in trace-form convention, where $\Omega$ is
the inverse Killing form Casimir. At $k = 0$: $r = 0$ (abelian
limit). In KZ normalisation: $r(z) = \Omega/((k + h^\vee)z)$.
Bridge identity: $k\,\Omega_{\mathrm{tr}} = \Omega/(k+h^\vee)$
at generic $k$. At $k = -h^\vee = -2$: trace-form finite,
KZ diverges (Sugawara singularity at critical level).
...
\item \emph{Bulk}: the chiral derived centre
  ...
  \end{equation}
  computed by chiral Hochschild cochains (not topological
  Hochschild; the geometry of the curve determines which
  Hochschild theory applies).
...
on $\cA$ produces a coalgebra; the Hochschild cochain complex of
