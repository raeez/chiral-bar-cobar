...
\begin{abstract}
For the modular-Koszul boundary sector of a 3d
holomorphic-topological gauge theory $T$ on $\bR_+ \times X$, the
comparison data form a seven-component package
\[
\cH(T)=(\cA,\cA^{\mathrm{i}},\cA^!,
\cZ^{\mathrm{der}}_{\mathrm{ch}}(\cA),r(z),\Theta_{\cA},
\nabla^{\hol}_{\bullet,\bullet}).
\]
The entries are the boundary chiral algebra, bar cohomology coalgebra,
Verdier-dual Koszul algebra, closed-sector derived centre, spectral
$r$-matrix, universal Maurer--Cartan element, and modular shadow
connection. We define each component from the bar complex
$\barB(\cA) = T^c(s^{-1}\bar{\cA})$ and its single
structural identity $D_{\cA}^2 = 0$, and compute the datum in
three model families: Heisenberg $\cH_k$ (abelian
...
$\mathbf{M}$). The algebraic closed-sector--boundary--line triangle is
recovered on the compact-generator comparison surface; its physical
BBL reading requires the open--closed comparison datum. The
closed-sector vertex is the chiral derived centre
$\cZ^{\mathrm{der}}_{\mathrm{ch}}(\cA)$, the boundary is
$\cA$ itself, and the line category is modelled by
$\cA^!\text{-mod}$. The
$\SCchtop$ structure emerges on the pair
$(\cZ^{\mathrm{der}}_{\mathrm{ch}}(\cA), \cA)$ of derived centre
and boundary algebra. The central
...
constructed via PTVV on $\mathrm{Map}(X \times \bR_{\geq 0},
...
\begin{remark}[The holographic-datum heptagon]\label{rem:holographic-heptagon}
The seven presentations of the $\SCchtop$ datum are the operadic,
Koszul, factorization, BV--BRST, convolution, Drinfeld-centre, and
derived-geometric presentations. The Drinfeld-centre face identifies
the factorizable representation centre with the representation theory
of the chiral derived centre through categorified bar-cobar and
half-braiding. The derived-geometric face carries the PTVV
1-shifted symplectic form on
\(\mathrm{Map}(X \times \bR_{\geq 0},B\,\mathrm{SC\text{-}Alg})\).
All seven presentations factor through the chiral Hochschild cochain
complex. The topologisation from $\SCchtop$ to
\(\Ethree^{\mathrm{top}}\) is proved cohomologically for affine
Kac--Moody algebras at non-critical level, where the Sugawara
conformal vector is explicit. The general chain-level lift for
conformal-vector chiral algebras is conjectural; the affine critical
level is excluded because Sugawara topologisation is singular there.
\end{remark}
...
The problem is to organise the algebraic data visible from the
modular-Koszul boundary sector of a 3d holomorphic-topological (HT)
quantum field theory. A 3d HT
theory on the half-space $\bR_+ \times X$, where $X$ is a smooth
algebraic curve, produces three observation algebras: a boundary
chiral algebra $\cA$ at $\{0\} \times X$, a physical bulk
factorisation algebra in the interior, and a category of topological
line operators along $\bR_+$.
...
\cH(T) \;=\;
(\cA,\; \cA^{\mathrm{i}},\; \cA^!,\;
\cZ^{\mathrm{der}}_{\mathrm{ch}}(\cA),\; r(z),\;
\Theta_{\cA},\; \nabla^{\hol}_{\bullet,\bullet}).
\end{equation}
It is extracted from a single Maurer--Cartan element by a sequence of
projections. The components are:
\begin{enumerate}[label=(\roman*),nosep]
\item the boundary chiral algebra $\cA$;
\item for a chosen quadratic presentation
  $\cA=T_{\mathrm{ch}}(V)/(R)$, the presentation coalgebra
  $\cA^{\mathrm{i}}=C_X(s^{-1}V,s^{-2}R)$ and the comparison
  $q_\cA\colon\cA^{\mathrm i}\to\barB(\cA)$;
\item the Verdier bar algebra
  $K_X(\cA)=\mathbb D_{\Ran}\barB(\cA)$ and the chosen chiral
  Koszul partner $\nu_\cA\colon K_X(\cA)\to\cA^!$;
\item the closed-string algebra
...
structure). The $\SCchtop$ structure emerges on the chiral derived centre
$\cZ^{\mathrm{der}}_{\mathrm{ch}}(\cA)
= C^\bullet_{\mathrm{ch}}(\cA,\cA)$, where the pair
$(\cZ^{\mathrm{der}}_{\mathrm{ch}}(\cA), \cA)$ is the SC datum
with the algebraic closed sector acting on the boundary. A physical
bulk action requires the open--closed comparison datum.
...
the line category is the Fock-charge sector of the abelian theory,
collapsing to $\mathsf{Vect}$ only after imposing the neutral
$k=0$ quotient; the higher-genus shadow connection has scalar
curvature \(k\,\omega_g\). This is the scalar base case.
...
$r(z) = k\,\Omega_{\mathrm{tr}}/z$ (trace-form; vanishing at $k=0$). The line
...
The closed-sector--boundary--line algebraic triangle is not an ad hoc
diagram. The ordered bar coalgebra is the level-$2$ twisting input.
The boundary-to-closed-sector trace passes through the derived chiral
centre, the boundary-to-lines passage uses Koszul duality, and the
closed-sector-to-lines edge is the complementarity cofibre. Physical
BBL language requires the open--closed comparison datum. Commutativity
of the algebraic triangle is Theorem~A on the boundary-linear
...
The ordered bar is the level-$2$ chain object before averaging; the symmetric bar is
its coinvariant shadow.

\begin{remark}[SC structure lives on the derived centre]
\label{rem:bar-not-sc}
The bar complex $\barB(\cA)$ is an $\Eone$-chiral coassociative
coalgebra. The $\SCchtop$ structure emerges on the chiral derived centre
$\cZ^{\mathrm{der}}_{\mathrm{ch}}(\cA) =
...
$\cA=T_{\mathrm{ch}}(V)/(R)$.  Its presentation coalgebra and full-bar
comparison are
...
$K_X(\cA):=\mathbb D_{\Ran}\barB(\cA)$ and the dual comparison
$\mathbb D(q_\cA)\colon K_X(\cA)\to
\mathbb D_{\Ran}(\cA^{\mathrm i})$.  A chiral Koszul-pair datum is a
chosen algebra map
$\nu_\cA\colon K_X(\cA)\to\cA^!$, required to be an equivalence on
the finite-type Koszul locus.
\end{definition}

The passage from $\cA$ to $\cA^!$ involves a quadratic presentation,
the full bar construction, Verdier duality, and a chosen Koszul-pair
comparison.  On the Koszul locus $q_\cA$ is a quasi-isomorphism, and
$H^\bullet(q_\cA)$ identifies the presentation-coalgebra cohomology
with bar cohomology.  The resulting typed objects and maps are
\begin{equation}\label{eq:four-objects}
\cA^{\mathrm i}\xrightarrow{\ q_\cA\ }\barB(\cA),
\qquad
...
\end{equation}
The cobar counit
$\Omega(\barB(\cA))\xrightarrow{\sim}\cA$ records inversion.


\subsection{Line-operator category}\label{subsec:lines}

\begin{definition}[Line-operator category]\label{def:lines}
Under the compact-generator open--closed hypotheses, the
\emph{line-operator category} is modelled by
...
Under $H_{\mathrm{CYBE}}^{\mathrm{rep}}(\cA;W)$, the represented
kernel $r_\cA=\rho_\cA(K_\cA^{\mathrm{coll}})$ satisfies the classical
Yang--Baxter equation. The representation carries the Arnold
relation on $\overline{M}_{0,4}$ to CYBE: the
...
The ordered kernel $K_\cA^{\mathrm{coll}}$ lives in the $\Eone$
convolution algebra $\mathfrak{g}^{\Eone}_{\cA}$. Its homotopy
coinvariant $q_2(K_\cA^{\mathrm{coll}})$ remains operator-valued.
The normalized functional $\tau_{\cA,2}^{\mathrm{res}}$ gives
$\kappa$ for abelian and scalar families. For non-abelian affine
Kac--Moody it gives $\kappa_{\mathrm{dp}}$, and the Sugawara
contribution completes the modular characteristic:
...
\cH(\cH_k) \;=\;
\bigl(\cH_k,\; H^*(\barB(\cH_k)),\;
(\Sym^{\mathrm{ch}}(V^*[1]),m_0=-k\omega),\;
\cZ^{\mathrm{der}}_{\mathrm{ch}}(\cH_k),\; k/z,\;
k \cdot \eta \otimes \Lambda,\; d-k\,\omega_g\bigr).
\end{equation}
\end{proposition}

\begin{proof}
The entries are computed directly from the OPE, the bar differential,
and the shadow projection.
...
\(\cC\) is the Fock-charge line category of the abelian theory,
equivalently the rank-one abelian Yangian/Fock module sector. It
collapses to \(\mathsf{Vect}\) only at \(k=0\) after imposing the
neutral quotient.
...
\(\nabla^{\hol}_{0,n}=d\) at genus \(0\). At positive genus the
shadow connection has scalar curvature \(k\,\omega_g\); it becomes
trivial only at \(k=0\) or after cancellation by the dual curvature.
\end{proof}

\begin{remark}[Modular characteristic]\label{rem:heis-kappa}
The modular characteristic is $\kappa(\cH_k) = k$. This is the
abelian prototype:
$\tau_{\cH_k,2}^{\mathrm{res}}q_2(K_{\cH_k}^{\mathrm{coll}})
=k=\kappa$.
...
\Bigl(
\widehat{\fsl}_2{}_k,\;
H^*(\barB(\widehat{\fsl}_2{}_k)),\;
\mathrm{CE}^{\mathrm{ch}}(\widehat{\fsl}_2{}_{-k-4}),\;
\cZ^{\mathrm{der}}_{\mathrm{ch}}(\widehat{\fsl}_2{}_k),\;
\frac{k\,\Omega}{z},\;
\Theta_k,\;
...
\smallskip\noindent\emph{(ii) Koszul dual.}\enspace
\(\cA^!=\mathrm{CE}^{\mathrm{ch}}(\widehat{\fsl}_2{}_{-k-4})\)
after the PBW rectification of the affine bar coalgebra. The level
parameter is reflected by
\(k\mapsto -k-2h^\vee\), with \(h^\vee(\fsl_2)=2\). The
complementarity of scalar modular characteristics is
...
$r^{\mathrm{coll}}(z)=k\,\Omega_{\mathrm{tr}}/z$ is the trace-form
collision parameter. The KZ conformal-block normalization uses
\(\Omega_{\mathrm{KZ}}/((k+h^\vee)z)\). On evaluation modules, the
same spectral variable becomes the additive spectral parameter of the
Yangian. The full quantised
$R$-matrix $R(z) = 1 + \hbar\,r(z) + O(\hbar^2)$ satisfies the
quantum Yang--Baxter equation.
\end{remark}

\begin{remark}[Critical level]\label{rem:critical}
At $k = -h^\vee = -2$, the KZ normalization is singular and
\(\kappa=0\) on the affine scalar line. The trace-form current OPE
does not vanish. The Feigin--Frenkel centre replaces the non-critical
Sugawara construction, and the critical Gaudin algebra is recovered
from the centre rather than from a regular value of the KZ connection.
The degree-$2$ ordered chiral homology $H^1_{\mathrm{ord}, 2}$
doubles from $4$ to $8$ for $\fsl_2$ \textup{(}finite computation on
$V \otimes V$, $\chi_{\mathrm{ord}, 2} = -4$ conserved\textup{)}, and
Koszulness fails for the full symmetric bar, whose cohomology
becomes $\Omega^*(\mathrm{Op}_{\fsl_2}(D))$
\textup{(}infinite-dimensional; Frenkel--Teleman 2006; distinct object from the
degree-$2$ ordered slice\textup{)}. The Feigin--Frenkel centre is the algebra of
...
\cH(\Vir_c)
\;=\;
\bigl(\Vir_c,\; H^*(\barB(\Vir_c)),\; \Vir_{26-c},\;
\cZ^{\mathrm{der}}_{\mathrm{ch}}(\Vir_c),\; r_c(z),\;
\Theta_c,\; \nabla^{\hol}_c\bigr),
\end{equation}
with
\begin{align}
...
% ====================================================================
\section{The closed-sector--boundary--line triangle}\label{sec:bbl}
% ====================================================================

The closed-sector--boundary--line triangle is the algebraic comparison
surface underlying the physical BBL diagram of a 3d
holomorphic-topological gauge theory. The ordered bar complex is the
common source: its trace produces the boundary-to-closed-sector map,
its Verdier dual produces the boundary-to-lines map, and its
complementarity cofibre produces the closed-sector-to-lines map. A
physical bulk vertex is obtained only after the open--closed comparison
datum is supplied.


\subsection{The three vertices}\label{subsec:vertices}

\begin{definition}[Closed-sector--boundary--line triangle]\label{def:bbl}
Let $\cA$ be a chirally Koszul vertex algebra. The
\emph{closed-sector--boundary--line triangle} of $\cA$ consists of:
\begin{enumerate}[label=(\roman*),nosep]
\item \emph{Boundary}: the chiral algebra $\cA$ itself, the
  input to the bar construction.
\item \emph{Closed sector}: the chiral derived centre
  \begin{equation}\label{eq:closed-sector}
  \cA_{\mathrm{cl}}
  \;\simeq\;
  \cZ^{\mathrm{der}}_{\mathrm{ch}}(\cA)
  \;=\;
  ...
  chiral/topological/categorical Hochschild theory applies). A physical
  bulk interpretation requires the open--closed comparison datum.
\item \emph{Lines}: under compact-generation and line-comparison
...
bracket, and the boundary algebra $\cA$ is acted upon by the
algebraic closed sector via the universal brace. The SC datum is
two-coloured with directionality: closed-sector-to-boundary only, no
boundary-to-closed-sector map. A physical bulk-to-boundary reading
...
\item \emph{Boundary $\to$ closed sector} (the bar map).
...
\item \emph{Closed sector $\to$ lines} (the complementarity cofibre).
...
The full bar coalgebra gives
$K_X(\cA)=\mathbb D_{\Ran}\barB(\cA)$, and the chosen pair map
$\nu_\cA\colon K_X(\cA)\to\cA^!$ gives the line algebra.  On a
quadratic Koszul chart, $H^\bullet(q_\cA)$ and
$\mathbb D(q_\cA)$ compare this construction with the presentation
coalgebra~$\cA^{\mathrm i}$.  The
module category $\cA^!\text{-}\mathsf{mod}$ is the categorical
output.
\end{enumerate}

Commutativity of the triangle is Theorem~A on the
boundary-linear modular-Koszul locus.


\subsection{Reconstruction from the datum}\label{subsec:reconstruction}

The datum $\cH(\cA)$ is complete on the boundary-linear
modular-Koszul comparison surface.
...
Boundary $= \cH_k$. Closed sector
$= \cZ^{\mathrm{der}}_{\mathrm{ch}}(\cH_k)$.  Bakalov--De
Sole--Kac compute the bounded rank-one even superboson dimensions as
$(2,1,0,\ldots)$; a bounded-to-chart quasi-isomorphism transports
this vector to $\cZ^{\mathrm{der}}_{\mathrm{ch}}(\cH_k)$
\cite[Theorem~7.4]{BDSK21}. Lines
are the abelian Fock-charge sector, with a neutral \(\mathsf{Vect}\)
quotient at \(k=0\). The triangle degenerates only after this neutral
quotient; for generic \(k\) the scalar curvature records the abelian
monodromy.
\end{remark}

\begin{remark}[BBL for Kac--Moody]\label{rem:bbl-km}
Boundary $= \widehat{\fsl}_2{}_k$. Closed sector
$= \cZ^{\mathrm{der}}_{\mathrm{ch}}(\widehat{\fsl}_2{}_k)$, whose
support is computed by the family datum
$H_H(V_k(\fsl_2);S_{V_k(\fsl_2)})$.  The adjoint
$\fsl_2$ belongs to the zero-mode derivation prequotient.  The
bounded affine groups satisfy the Bakalov--De Sole--Kac conjectural
exterior-power bounds; their passage to the curve chart uses a
bounded-to-chart quasi-isomorphism. Lines $= \Rep(U_q(\fsl_2))$. The
Drinfeld--Kohno theorem makes the boundary-to-lines edge
constructive: the monodromy of the KZ connection generates
$\Rep(U_q(\fsl_2))$. The closed-sector-to-lines edge is the
complementarity cofibre:
$\mathbf{Q}_g(\widehat{\fsl}_2{}_{-k-4})$ controls the line-operator
anomaly.
\end{remark}

\begin{remark}[BBL for Virasoro]\label{rem:bbl-vir}
Boundary $= \Vir_c$. Closed sector
$= \cZ^{\mathrm{der}}_{\mathrm{ch}}(\Vir_c)$.  Its bounded vertex
cohomology has one-dimensional groups in degrees $\{0,2,3\}$
\cite[Theorem~7.2]{BDSK21}; a bounded-to-chart quasi-isomorphism
transports this support to the chiral curve chart. Lines
$= \cC_c$: the infinite-depth line category, governed by the BPZ
equations. The triangle is fully active at all three algebraic
vertices; the closed-sector-to-lines edge is controlled by the Virasoro
...
\item When $u \neq 0$: the Clifford algebra
  $\mathrm{Cl}(H_1(\Sigma_g); u)$ is a matrix algebra
  \(M_{2^g}(\bC)\), of vector-space dimension \(2^{2g}\), after
  inverting \(u\). Genus-$g$ is Morita equivalent to
...
matter contribution cancels the \(bc\)-ghost anomaly in the combined
matter-ghost system. The pure Virasoro gravity partition function
still has its own collision tower. At generic \(c\):
...
$B_\Theta$ controls the modular-Koszul boundary-linear sector of the
3d HT correspondence: the seven components determine the genus-$0$
comparison data, and the secondary anomaly \(u\) governs the genus
tower inside this sector. The entire structure descends from a single
identity: $D_{\cA}^2 = 0$.
...
\begin{multline*}
\Ethree^{\mathrm{top}}(\text{physical bulk})
\;\to\;
...
\end{multline*}
The arrows are restriction to codimension-$2$ defect, ordered bar
complex, Drinfeld-centre passage to categorical half-braidings, and
higher Deligne passage to the derived centre. The Drinfeld centre is
the right adjoint to the forgetful functor from braided to monoidal
categories; the averaging map is the later lossy
$E_2\to E_\infty$ shadow. For simple $\fg$, the
...
a conformal vector at non-critical level together with a BRST
antighost contraction; for affine Kac--Moody this is the Sugawara
primitive. This datum makes $\bC$-translations $Q$-exact and
topologises the holomorphic direction. Without such a primitive the
structure remains $\SCchtop$.
...
For a finite set $S_\cA$, Theorem~H identifies the curve-chart
cohomology with the support model in a family datum
$H_H(\cA;S_\cA)$.  The bounded vertex complex enters through a
bounded-to-chart quasi-isomorphism.  The Gelfand--Fuchs cohomology of
the mode Lie algebra is a separate functor and carries its own
support.


\subsection{From the datum to physics}\label{subsec:physics}

The derived chiral centre is the universal closed-sector algebra on the
boundary-linear comparison surface of a 3d HT gauge theory. Under the
BBL identification it is compared with the physical bulk observables:
the bar construction on $\cA$ produces a coalgebra; the chiral
Hochschild cochain complex of $\cA$ is computed from the bar
resolution; the
$\SCchtop$ structure on the pair
$(\cZ^{\mathrm{der}}_{\mathrm{ch}}(\cA), \cA)$ encodes the
open/closed interaction. The datum $\cH(\cA)$ is the compact package
for this comparison surface.

Three boundaries remain. The global BBL triangle is proved in the
boundary-linear direction, while full reconstruction is open. The
class~$\mathbf{M}$ topologisation is cohomological until a chain-level
model is built. The $\En$ circle closes only where the Drinfeld-centre
identification
\[
Z(\mathbf{U}_{\cA}) \simeq \cZ^{\mathrm{der}}_{\mathrm{ch}}(\cA)
\]
has been proved.
