...
Verdier-dual Koszul algebra, closed-string derived centre, spectral
...
on the compact-generator open--closed surface: the closed-sector
vertex is the chiral derived centre
...
The bulk--boundary--line (BBL) triangle is not an ad hoc diagram.
The ordered bar coalgebra is the level-$2$ twisting input.  The
boundary-to-bulk trace passes through the derived chiral centre, the
boundary-to-lines passage uses Koszul duality, and the bulk-to-lines
edge is the complementarity cofibre. Commutativity of the triangle is
Theorem~A on the boundary-linear modular-Koszul locus.
...
\cH(\cH_k) \;=\;
\bigl(\cH_k,\; H^*(\barB(\cH_k)),\; \Sym^{\mathrm{ch}}(V^*),\;
...
common source: its trace produces the boundary-to-bulk map, its
Verdier dual produces the boundary-to-lines map, and its
complementarity cofibre produces the bulk-to-lines map.
...
\item \emph{Lines}: under compact-generation and open--closed
