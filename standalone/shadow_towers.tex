...
% ================================================================
% Earlier shadow-tower statement; canonical refinements are in v3.
% ================================================================
% This file records a weaker shadow-tower statement.
% The canonical Platonic-ideal standalone is
%   standalone/shadow_towers_v3.tex
% with the following stronger results:
%   (1) closed-form Sigma_Vir^{(k)} = [P_k + Q_k log(1+6z)]/(1+6z)^{2k}
%       (pole-doubling for all k; thm:pole-doubling-all-k);
%   (2) C_A = 6 universality on non-logarithmic class M with Virasoro
%       subalgebra, decomposition 6 = r_0 . S_{r_0} = 3 . 2;
%   (3) large-c-compliant large-c asymptotic S_4 ~ 2/c^2 (not 2/(5c^2));
%   (4) multi-weight cross-channel correction
%       delta F_2(W_3) = (c + 204)/(16c) via the seven stable graphs
%       of M-bar_{2,0}; prop:delta-f-cross-w3-g2;
%   (5) sub-subleading coefficient beta_N = (N+1)(N+2)/2 (Vol II bridge);
%   (6) Riccati seed (r_0, S_{r_0}(Vir)) = (3, 2).
%
% The retraction from class M of the 1/e Stirling-cancellation
% obstruction (B87) and the Kummer-irregular label retractions for
% primes {1423, 3067, 23, 43, 419} (B92) are inscribed in
% chapters/theory/shadow_tower_higher_coefficients.tex and the
% companion standalone standalone/arithmetic_shadows.tex.
%
% New readers should read shadow_towers_v3.tex directly; this file
% is preserved for archival reference only.
% ================================================================
...
A vertex algebra $A$ on a smooth projective curve~$X$ produces,
for each stable pair $(g,n)$, a space of conformal blocks
$\mathbb{V}_{g,n}(A)$ on $\Mbar_{g,n}$. When $A$ is chirally
Koszul (Definition~\ref{def:chirally-koszul}), computing these
spaces reduces to the operator product expansion (OPE).

Sewing and factorization express genus-$g$ amplitudes as sums over
stable graphs $\Gamma$: each vertex contributes a genus-$g_v$
conformal block, each edge a propagator. These graph sums are
the Feynman expansion of a two-dimensional conformal field theory
and produce characteristic classes on $\Mbar_{g,n}$ whose
integrals are the free energies~$F_g(A)$.

We prove that for any chirally Koszul vertex algebra, the full
tower of deformation invariants underlying these free energies
is controlled by a single algebraic relation of degree~$2$.


\subsection{The bar construction and the shadow obstruction tower}

The algebraic framework is the chiral bar construction.
For a vertex algebra~$A$ on a curve~$X$, the bar complex $B(A)$
is a factorization coalgebra on $\Ran(X)$ whose differential
encodes OPE collision residues on the Fulton--MacPherson
compactifications $\FM_n(X)$. When $A$ is \emph{chirally Koszul}
(bar cohomology $H^{\bullet}(B(A))$ concentrated in bar
...
The answer is \emph{three invariants}: the entire infinite tower
$\{S_r\}_{r \geq 2}$ is algebraic of degree~$2$, determined by the
genus-$0$ triple $(\kappa, \alpha, S_4)$.
...
An immediate structural consequence.
The \emph{critical discriminant}
$\Delta := 8\kappa S_4$ gives the Gaussian decomposition
\[
...
$1/16$ \textup{(}tadpole\textup{)}, and
$21/(4c)$ \textup{(}barbell\textup{)}.
The tadpole contribution is independent of~$c$.
\end{theorem}
...
higher-degree components of the bar differential, which have no
...
\[
\lambda_g := c_g(\mathbb{E}_g) \in
H^{2g}(\Mbar_g, \mathbb{Q}),
\qquad
\mathbb{E}_g = \pi_*\omega_{\pi}.
\]
The first Chern class of the Hodge line is written
\(\lambda_1=c_1(\det\mathbb E_g)\). The
...
\label{rem:ordered-bar-primitive}
The bar complex $B(A)$ of Definition~\ref{def:bar-complex} is the
\emph{symmetric} (factorization) bar $B^\Sigma(A)$, the
$\Sigma_n$-coinvariant quotient of the ordered bar
$B^{\mathrm{ord}}(A) = T^c(s^{-1}\overline{A})$. The ordered bar
carries a deconcatenation coproduct (coassociative, not
cocommutative) and supports the ordered collision kernel
$K_A^{\mathrm{coll}}(z)\in\mathfrak{g}^{E_1}_A$; the shadow obstruction tower
$\{S_r\}$ and the algebraicity relation $H(t)^2 = t^4 Q(t)$ are
statements about normalized traces of the homotopy-coinvariant
operator classes.  At degree~$2$,
$\tau_{A,2}^{\mathrm{res}}q_2(K_A^{\mathrm{coll}})=\kappa$ in
abelian and scalar families and equals $\kappa_{\mathrm{dp}}$ on the
non-abelian affine row, where $\dim(\fg)/2$ completes $\kappa$.
At higher degree,
$S_n=\tau_{A,n}^{\mathrm{res}}q_n((\Theta_A^{E_1})_{0,n})$.
The ordered bar is the level-$2$ chain object before averaging; the shadow tower is its
averaged projection.
\end{remark}
...
$\xi$ and $\eta$ at genera $g_1$, $g_2$ and degrees $n_1$, $n_2$,
...
degree filtration: $[\xi, \eta]$ has degree $\leq n_1 + n_2 - 2$
...
The MC element $\Theta_A$ encodes all genera and degrees.
...
the components of degree at most~$r$ in $\Defcyc^{\mathrm{mod}}(A)$.
...
\]
the component of $\Theta_A$ at degree exactly~$r$.
\end{definition}

The MC equation~\eqref{eq:mc-equation}, projected onto
the degree-$r$ component, yields the \emph{all-degree master
equation}:

\begin{proposition}[All-degree master equation]
\label{prop:master-eq}
For each $r \geq 2$, the degree-$r$ projection of the MC
equation gives
...
$\Defcyc^{\mathrm{mod}}(A)$ both respect the degree filtration.
Restricting the MC equation to degree~$r$ gives exactly
...
(the eigenvalue of the curvature operator on the degree-$r$
...
a double zero at $t = 0$ from the degree offset~$t^2$.
\end{remark}
...
\item on every one-dimensional primary line $L$, the shadow
 ...
 so the single-line tower terminates at degree~$3$; but
...
\item $d_{\mathrm{arith}} \geq 0$ is the \emph{arithmetic
 ...
 Shadow coefficients of weight $\leq 2r$ at degree~$r$
...
The genus-$g$ formal scalar coefficient is $F_g(\cH_k) = k \cdot
...
at degree~$2$ (class~$\mathbf{G}$ restricted to this line).
...
\begin{theorem}[Virasoro weighted-Riccati coefficients]
\label{thm:virasoro-coefficients}
Let $R_r$ be defined by
$t^2\sqrt{Q(t)}=\sum_{r\ge2}rR_rt^r$ with
$Q(t)=(c+6t)^2+80t^2/(5c+22)$.  Through degree~$10$,
\begin{align*}
 R_2 &\;=\; \frac{c}{2},
 \\[3pt]
 R_3 &\;=\; 2,
 ...
 \\[3pt]
 R_{10} &\;=\; \frac{256(91125c^3+1050975c^2+3989790c+4969967)}{c^7(5c+22)^4}.
\end{align*}
All coefficients belong to $\mathbb{Q}(c)$ with poles
only at $c = 0$ and $c = -22/5$. The signs alternate
starting from $R_5$:
$R_5 < 0$, $R_6 > 0$, $R_7 < 0$, \ldots\ for $c > 0$.
An ordered residue package $\mathsf H_{\mathrm{res}}(\Vir_c;X)$,
together with a chain comparison carrying its transferred scalar
brackets to this weighted recurrence, gives
...
carries independent class~$\mathbf{M}$ data with even-degree
...
\noindent The tadpole mixed amplitude is
...
tadpole ($1/16$), barbell ($21/(4c)$). The remaining three (smooth, figure-eight,
...
\item \emph{The $c$-independent piece.}\enspace
 The tadpole contribution $1/16$ is the only
 $c$-independent term:
...
\item \emph{Relative magnitude.}\enspace
 ...
 correction becomes subleading at large central charge.
 The crossover $\delta F_2 = F_2^{\mathrm{scal}}$
 occurs at $c \approx 147$.
 At moderate~$c$ the correction dominates:
 at $c = 50$, $\delta F_2/F_2^{\mathrm{scal}} \approx 6.3$.
 At large~$c$ the correction is suppressed:
 at $c = 500$, $\delta F_2/F_2^{\mathrm{scal}} \approx 0.17$.
\end{enumerate}
\end{remark}
...
genus variable~$q$ and the degree variable~$t$. The
...
\bibitem{LorgatVirR}
R.~Lorgat,
\emph{The Virasoro spectral $R$-matrix: closed form and non-formality},
preprint, 2026.
