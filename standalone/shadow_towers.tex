...
%%% Title
\title{Shadow towers and the algebraicity of chiral deformation invariants}
\author{Raeez Lorgat}
\date{}
...
We compute shadow towers explicitly for all standard families
...
correction $\delta F_2(\cW_3) = (c+120)/(16c)$ by a
graph-by-graph stable graph sum over the six genus-$2$ stable
graphs.
\end{abstract}
...
\subsection{The bar construction and the shadow tower}
...
The \emph{shadow tower} of~$A$ is the sequence of
...
The algebraicity theorem has a clean structural consequence.
...
The shadow tower also carries a logarithmic connection.
...
\begin{theorem}[Genus-$2$ universality and the $\cW_3$
  cross-channel correction; Theorem~\ref{thm:genus2}]
...
$T$ (weight~$2$) and $W$ (weight~$3$), the multi-channel
stable graph sum over the six genus-$2$ stable graphs
...
$9/(2c)$ \textup{(}theta\textup{)}, and
$1/16$ \textup{(}lollipop\textup{)}.
...
invariants:\ $\kappa = c$ for Heisenberg, but
...
Section~\ref{sec:landscape} computes explicit shadow towers for
...
\subsection{The shadow tower}

The MC element $\Theta_A$ encodes all genera and arities.
Its finite-order truncations form the shadow tower.

\begin{definition}[Shadow tower]
\label{def:shadow-tower}
The \emph{shadow tower} of~$A$ is the sequence of truncations
...
in which the shadow tower is algebraic of degree~$2$: the
...
that the shadow tower is determined by three invariants
...
$H(t) = t^2(2\kappa + 3\alpha\,t)$.
If additionally $\alpha = 0$, then $H(t) = 2\kappa\,t^2$,
...
\sqrt{-32\kappa^2\Delta}\bigr)\big/
(9\alpha^2 + 2\Delta)$,
...
For algebras of class~$\mathbf{M}$, the shadow tower is
...
\item $d_{\mathrm{alg}} \in \{0, 1, 2, \infty\}$ is the
  ...
  OPE-generated shadow tower on the primary line.  By the
...
are at
$t_{\pm} = (-6\kappa\alpha \pm \sqrt{-32\kappa^2\Delta})/
(9\alpha^2 + 2\Delta)$.
For $\Delta > 0$, the discriminant
$-32\kappa^2\Delta < 0$, so $t_{\pm}$ are complex
...
\section{Explicit shadow towers for the standard landscape}
\label{sec:landscape}
...
\begin{proposition}[Heisenberg shadow tower]
...
\]
The shadow tower terminates immediately:
...
\begin{proposition}[Lattice VOA shadow tower]
...
shadow tower: they live on charged strata.  The
...
\begin{proposition}[Affine Kac--Moody shadow tower]
...
The Koszul dual is $V_k(\fg)^{!} = V_{-k-2h^\vee}(\fg)$,
and $\kappa(k) + \kappa(-k-2h^\vee) = 0$ (exact
anti-symmetry).
...
\begin{proposition}[$\beta\gamma$ shadow tower]
...
\begin{proposition}[Virasoro shadow tower]
...
The Virasoro shadow tower coefficients through arity~$10$
are:
\begin{align*}
  S_2 &\;=\; \frac{c}{2},
  \\[3pt]
...
invariants in a controlled way.  For Virasoro, the Koszul
dual is $\mathrm{Vir}_c^{!} = \mathrm{Vir}_{26-c}$
(the Feigin--Frenkel involution $c \mapsto 26 - c$).
...
$R$-matrix corrections.  Whether $R$-matrix dressing modifies
the result at $g \geq 2$ is the content of the open problem
\textup{op:multi-generator-universality}.  The computation
below is therefore conditional on this open problem.
The per-channel scalar part
...
There are six stable graphs of genus~$2$ with no marked
points:
...
of the five boundary graphs ($\Gamma_1$--$\Gamma_5$;
...
\begin{equation}\label{eq:cross-channel-total}
  ...
  \frac{c + 120}{16c}.
\end{equation}
Conditional on the resolution of
\textup{op:multi-generator-universality}, the full genus-$2$
...
\[
  ...
  \;\;+\;\;
  \underbrace{\frac{c + 120}{16c}}_{\displaystyle
    \delta F_2^{\mathrm{cross}}}.
\]
\end{theorem}

\begin{proof}
The cross-channel contributions from all six graphs
are computed in
\eqref{eq:banana-cross}--\eqref{eq:lollipop-cross}.
Three graphs contribute: banana ($3/c$), theta ($9/(2c)$),
lollipop ($1/16$).  The remaining three (smooth, figure-eight,
...
For the sum:
\[
  \frac{3}{c} + \frac{9}{2c} + \frac{1}{16}
  \;=\;
  \frac{6}{2c} + \frac{9}{2c} + \frac{1}{16}
  \;=\;
  \frac{15}{2c} + \frac{1}{16}
  \;=\;
  \frac{120}{16c} + \frac{c}{16c}
  \;=\;
  \frac{c + 120}{16c}.
  \qedhere
\]
...
\item \emph{The rational piece.}\enspace
  The banana and theta contributions combine to $15/(2c)$.
  Both arise from genus-$0$ vertices (quartic and trivalent
  respectively), mediated by the $\mathbb{Z}_2$-allowed
  mixed assignments.  They dominate at small~$c$.

\item \emph{Vanishing at $c = -120$.}\enspace
  The cross-channel correction vanishes at $c = -120$.
...
\subsection*{Shadow towers for non-standard families}
...
infinite shadow tower $\{S_r\}_{r \geq 2}$ produces an infinite
...
relations constructed by Pixton and proved to be complete
...
shadow tower defines a shadow partition function
...
The shadow tower converges because its coefficients decay
...
The passage from algebraic shadow tower to analytic
...
\emph{Relations on $\overline{\mathcal{M}}_{g,n}$ via
$3$-spin structures},
J.~Amer.\ Math.\ Soc.\ \textbf{28} (2015), 279--309.
