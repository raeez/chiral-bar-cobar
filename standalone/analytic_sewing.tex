...
HS-sewing theorem verifies the criterion for Heisenberg, positive
integer-level affine Kac--Moody, lattice vertex algebras, and for
Virasoro and principal $\cW_N$ after a positive Hilbert model with
polynomial OPE bounds is fixed.
...
estimate delivers the sewing package for the \(q\)-damped kernel:
every closed amplitude is trace class on the weighted completion
...
coderived category (Theorem~\ref{thm:bv-bar-coderived-vol1})
...
continuous map $E \to \cA^{\mathrm{sew}}$ with dense image, provided
...
$\cA^{\mathrm{sew}}$ is complete, this map extends uniquely from
the dense algebraic core to~$E$.
\end{proof}
...
or Moriwaki's ind-Hilbert completion.  They compare to
$\cA^{\mathrm{sew}}$ by continuous maps once their amplitudes are
continuous; embeddings or quotient descriptions require additional
topological hypotheses.  The HS-sewing criterion of the next section
constructs a concrete Hilbert completion~$H_q$ equipped with a dense
comparison map $H_q \to \cA^{\mathrm{sew}}$.
\end{remark}
...
\end{equation}
	...
	The damping is part of the sewing kernel.  The raw multiplication
...
trace class on~$H_q$;
\item the weighted Hilbert completion~$H_q$ admits a continuous
linear map
$H_q \to \cA^{\mathrm{sew}}$ with dense image.
\end{enumerate}
\end{proposition}
...
weight-$(a+b)$ subspace of $H_q^{\widehat{\otimes} 2}$,
	the raw sector component $m_{a,b}^c$ contributes
	$q^{2(c-a-b)}\|m_{a,b}^c\|_{\HS}^2$.  The damping
	$q^{(3a+3b-c)/2}$ multiplies this by $q^{3a+3b-c}$, so the
	sector contribution is $q^{a+b+c}\|m_{a,b}^c\|_{\HS}^2$.
	Summing gives
$\|m_q\|_{\HS}^2 =
...
\item \emph{Closed-amplitude trace class:} one could require
 trace-class convergence only for the closed amplitudes
 entering a given partition function.  This can be weaker than
 controlling the full pair-of-pants operator, but it is less local
 and is harder to verify from OPE data alone.
\end{enumerate}
...
\label{thm:heisenberg-sewing-vol1}
...
\begin{enumerate}[label=\textup{(\roman*)}]
\item \textup{(\textbf{Hardy--Ramanujan sector growth})}\;
...
satisfies HS-sewing after a positive Hilbert model is fixed and the
PBW monomial basis has polynomial OPE matrix coefficients.  Finite
strong generation gives Hardy--Ramanujan sector growth; PBW gives
the finite exponent in hypothesis~(ii).
\end{corollary}
...
$N = 2$ in the PBW monomial norm, and in the unitary range this is
a genuine Hilbert estimate.  At nonunitary or degenerate central
charges the Shapovalov form is not a positive Hilbert form; the
criterion applies only after choosing a positive quotient or
completion for which the displayed polynomial bound is verified.
\end{example}
...
$0 < q < 1$ in any positive Hilbert realization satisfying the
same bound.
\end{example}
...
landscape.  The entries prove the criterion in the indicated
positive Hilbert realizations; for nonunitary degenerate quotients
the polynomial bound is an additional normed-model hypothesis.
\begin{center}
\renewcommand{\arraystretch}{1.15}
...
For the standard landscape, each layer has the following proof state:
...
(Theorem~\ref{thm:bv-bar-coderived-vol1}); Layers~1 and~2 are
...
The weighted spaces~$H_q$ form a natural nuclear Hilbert scale as
$q \to 1^-$. Identifying this scale with the sewing envelope requires
the sewing topology and the weighted Hilbert-scale topology to control
one another; HS-sewing alone gives only one comparison map.

\begin{proposition}[Nuclear Hilbert scale]
\label{prop:nuclear-structure}
Assume the sector dimensions have subexponential growth. Then the
weighted completions form a projective Hilbert scale
\[
\mathcal H^{\mathrm{scale}}
\;:=\;
\varprojlim_{q \to 1^-} H_q
\]
whose transition maps $H_{q'} \to H_q$ (for $q' > q$) are
Hilbert--Schmidt. In particular, $\mathcal H^{\mathrm{scale}}$ is a
nuclear Fr\'echet space. If, in addition, the sewing seminorms and
the Hilbert-scale seminorms define the same completion on
$\cA_{\mathrm{alg}}$, then
$\cA^{\mathrm{sew}}\cong\mathcal H^{\mathrm{scale}}$.
\end{proposition}
...
The nuclear structure implies the Schwartz kernel theorem on
$\mathcal H^{\mathrm{scale}}$. It applies to
$\cA^{\mathrm{sew}}$ only after the additional topology-comparison
hypothesis in Proposition~\ref{prop:nuclear-structure}.
...
Positselski~\cite{Positselski11} introduced the coderived category for
curved dg-algebras and matrix factorisations.  The totalisation of a
short exact sequence of curved complexes need not be acyclic, since
$d^2 \neq 0$, but it is always coacyclic.  Coacyclicity is the correct
curved replacement for ordinary acyclicity.
\end{remark}
...
\item The BV/bar comparison: the harmonic discrepancy at the
 ...
 by Theorem~\ref{thm:bv-bar-coderived-vol1}(iv), so the coderived
 comparison holds.
\end{enumerate}
...
$\kappa(\cA) + \kappa(\cA^!) = \varkappa(\cA)$
(the scalar complementarity invariant): $\varkappa = 0$ for
KM/Heis/lattice/free; $\varkappa = 13$ for Virasoro; $\varkappa = 250/3$
for $\cW_3$. The scalar $\varkappa$ is distinct from the Trinity
ghost-charge conductor $K(\cA):=c(\cA)+c(\cA^!)=-c_{\mathrm{ghost}}(\BRST)$
(taking values $K(\Vir)=26$, $K(\cW_3)=100$, $K(\mathrm{BP})=196$); the two
invariants are related by $\varkappa=\varrho_{\cA}\cdot K$ with $\varrho$
family-specific.  The
...
S_\cA(u) + S_{\cA^!}(u)
\;=\;
\varkappa \cdot \zeta(u)\,\zeta(u+1)
\;+\;
\text{lower-order terms},
\end{equation}
where the leading term involves the scalar complementarity invariant.  For
...
\end{enumerate}
For Hilbert spaces, nuclear and trace class agree.  In general,
trace class $\subset$ Hilbert--Schmidt $\subset$ compact, and the
first inclusion is strict even for positive operators: the diagonal
operator $\operatorname{diag}(1/n)$ on $\ell^2$ is positive and
...
% ================================================================
% Harmonic projector, chain-level nodal sewing, and weight-completed class M
% ================================================================

\section{Harmonic identification and chain-level sewing at nodes}
\label{sec:analytic-mc5-harmonic}

The analytic sewing package of this paper is sharpened against
three precise tests: (i)~the harmonic discrepancy coefficients
$c_r(\cA)$ are not merely named but \emph{identified} with the
shadow invariants $S_r(\cA)$; (ii)~chain-level sewing at nodes
conditionally lifts to $\overline{\cM}_{g,n}$ via Mok25 logarithmic
Fulton--MacPherson and Francis--Gaitsgory factorization-gluing;
(iii)~the class~M
...
\begin{theorem}[Conditional chain-level sewing at nodes via log-FM
 and Francis--Gaitsgory]\label{thm:analytic-chain-level-nodes}
Assume the logarithmic Fulton--MacPherson compactification used
below exists with the stated smooth snc boundary and that the
Francis--Gaitsgory factorization-gluing theorem applies to the
chosen factorization category. Under HS-sewing, the analytic sewing
theorem extends from the open stratum $\cM_{g,n}$ to the boundary
...
\end{enumerate}
The resulting chain-level sewing is proved only under these two
geometric hypotheses; classes~G, L, C then live on the original
complex, while class~M lives in the weight-completed ambient.
\end{theorem}

\begin{remark}[Weight-completed class~M is the correct scope]
\label{rem:analytic-weight-completed-M}
...
valid when the finite-stage maps satisfy the required
Mittag-Leffler/surjectivity hypotheses. Thus class~M lives in the
weight-completed category, not in the raw direct-sum ambient
(compare Vol~III
...
\label{rem:analytic-degree-2-curvature-uniqueness}
...
\begin{remark}[Alternative: Costello--Gwilliam factorization
 homology]\label{rem:analytic-CG-alt}
The alternative proof route via Costello--Gwilliam factorization
homology (H07 in the programme theorem map) is genuinely
...
degeneration using Mok25 relative log-FM geometry as conditional
geometric input. This provides an independent verification route for
the MC5 analytic theorem once the residue-pushforward compatibilities
are supplied.
\end{remark}
