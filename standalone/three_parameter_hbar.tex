...
to the Killing form) in the Kazhdan--Lusztig (Sugawara) normalization.
The numerical prefactor $1/(k + h^\vee)$ is a single scalar in the
KZ convention. The papers compared below extract this scalar from
three different directions,
calling it by three different names. This paper identifies all three.

\begin{remark}[KZ vs.\ trace convention]\label{rem:3hbar-kz-vs-trace}
Equation~\eqref{eq:km-collision-residue} is in the KZ normalization,
where $\Omega = \Omega_{\mathrm{KZ}}$ is the Killing-normalized
Casimir and the factor $(k+h^\vee)^{-1}$ is the Sugawara prefactor
appearing in $T_{\mathrm{Sug}} = \tfrac{1}{2(k+h^\vee)} \sum_a
{:}J^a J_a{:}$. The \emph{trace convention} uses $\Omega_{\mathrm{tr}}
= (2h^\vee)^{-1} \Omega_{\mathrm{KZ}}$ and writes $r_{\mathrm{tr}}(z)
= k\,\Omega_{\mathrm{tr}}/z$, which vanishes at the abelian limit
$k=0$ (satisfied). The two conventions are two presentations
of the same operadic datum related by a level-dependent rescaling
of the Casimir, \emph{not} by a rational-function identity in~$k$:
at $k = 0$ the trace form gives $0$ while the KZ form gives
$\Omega_{\mathrm{KZ}}/(h^\vee z) \neq 0$. The three parameter
identifications below hold in the KZ/RTT convention. If one starts
instead from the trace collision residue
$r_{\mathrm{tr}}=k\Omega_{\mathrm{tr}}/z$, the scalar matching to
$\Omega_{\mathrm{KZ}}/((k+h^\vee)z)$ is convention-dependent; with
$\Omega_{\mathrm{tr}}=(2h^\vee)^{-1}\Omega_{\mathrm{KZ}}$ it is
$2h^\vee/[k(k+h^\vee)]$.
\end{remark}
...
The trace-form collision kernel
$K_{k,\fg}^{\mathrm{coll}}(z)=k\Omega_{\mathrm{tr}}/z$ lives on the
...
quantizes its represented KZ coordinate
$r_{\mathrm{KZ}}(z)=\Omega/((k+h^\vee)z)$ via the Yangian $R$-matrix.
The classical scalar trace is
\[
\kappa_{\mathrm{cl}}
...
constants, and $\lambda$ is the formal variable. The central
$k\lambda$ term gives the trace-form residue
$r^{\mathrm{KM}}_{\mathrm{tr}}(z)=k\,\Omega_{\mathrm{tr}}/z$.
The KZ-normalized connection is obtained only after inserting the
...
\begin{equation}\label{eq:pva-r-matrix}
 ...
 \cdot \frac{\Omega}{z}.
\end{equation}
The factor $1/(k+h^\vee)$ does not follow from the PVA bracket alone;
it is the KZ/Sugawara coupling.
...
order, it reduces to the classical Yang--Baxter equation for
$r(z) = \Omega/z$. The loop parameter is the formal deformation
parameter of the Yangian $Y_\hbar(\fg)$.
...
\end{equation}
The bar propagator absorbs one pole, so
...
\end{equation}
The $d\log$ extraction of the double-pole term gives the trace-form
residue $k\,\Omega_{\mathrm{tr}}/z$; the simple-pole term contributes
the structure-constant piece and is not combined with the central
tensor. The KZ-normalized $r$-matrix
$r_{\mathrm{KZ}}(z)=\Omega_{\mathrm{KZ}}/((k+h^\vee)z)$ is obtained
after inserting the Sugawara/KZ normalization.
...
D - d_0$. The central double-pole and the structure-constant
simple-pole terms are different tensor types in the bar projection.
In trace form the central term gives $k\,\Omega_{\mathrm{tr}}/z$.
Passing to the KZ-normalized connection inserts the Sugawara factor
$(k+h^\vee)^{-1}$ and the KZ Casimir
$\Omega_{\mathrm{KZ}}=2h^\vee\Omega_{\mathrm{tr}}$. Thus the
bar-side prefactor identified with the KZ coupling is
$1/(k+h^\vee)$; the trace-form residue remains a separate
normalization.
\end{proof}
...
% ================================================================
\section{Miura cross-universality: $(\Psi{-}1)/\Psi$ at all spins}
\label{sec:miura-3hbar}
% ================================================================

The three-parameter identification of~$\hbar$ on affine Kac--Moody
has a companion identification on principal $W$-algebras that fixes
the \emph{cross-term coefficient} of the Drinfeld coproduct. This
coefficient is the single datum relating $\hbar_{\mathrm{Miura}}$
(the level-uniformizing variable on $\cW_N$) to the three affine
$\hbar$'s of Theorem~\ref{thm:three-parameter}.

\begin{theorem}[Miura cross-universality]\label{thm:miura-cross-universality-3hbar}
Let $\cW_N$ be the principal $W$-algebra at level parameter
\[
 \Psi \;=\; \frac{k + N}{k + N - 1}, \qquad \Psi \neq 0, 1.
\]
For every spin $s \geq 2$, the Drinfeld coproduct on the primary
generator $W_s$ contains the cross-term
\[
 \Delta_z(W_s) \;\supset\;
 \frac{\Psi - 1}{\Psi}\,\bigl(J \otimes W_{s-1}
 \;+\; W_{s-1}\otimes J\bigr),
\]
with the coefficient $(\Psi-1)/\Psi$ \emph{universal across all spins
$s \geq 2$}. Composite corrections (normally-ordered products of
lower-spin generators) appear at higher orders; the primary
cross-term coefficient is $s$-independent and $N$-independent.
\end{theorem}

\begin{remark}[Coincidence-masking at $\Psi = 2$]
\label{rem:miura-coefficient-coincidence-3hbar}
The coefficient $(1/\Psi)\bigl(J \otimes W_{s-1}
+ W_{s-1}\otimes J\bigr)$ agrees numerically with the correct
coefficient $(\Psi-1)/\Psi$ at $\Psi=2$. The value $\Psi=3$
separates the two formulas: $(\Psi-1)/\Psi=2/3$ and $1/\Psi=1/3$.
A direct Miura computation at $N = 3$, spin $s = 3$,
$\Psi = 3$ (verifiable by elementary symmetric expansion of
$T(u) = \prod_{i=1}^3 (u + \Lambda_i)$) reads off $2/3$.
\end{remark}

\begin{remark}[$\Psi = 0, 1$ degenerations]
\label{rem:psi-degenerations-3hbar}
The two special values of $\Psi$ sit at the endpoints of the Miura
quantization line:
\begin{enumerate}[label=(\roman*)]
 \item $\Psi = 1$ (free boson): $(\Psi-1)/\Psi = 0$, the cross-term
 vanishes, and $\Delta_z$ collapses to the plain coproduct on an
 abelian Heisenberg parent. The OPE obstruction to chiral
 quantization disappears.
 \item $\Psi = 0$ ($k = -N$, free-abelian degeneration): the
 coefficient diverges as a rational function of $\Psi$; the chiral
 algebra degenerates to an abelian Heisenberg factor with identity
 quantum group, and the Miura coefficient is not defined.
\end{enumerate}
\end{remark}

\begin{proposition}[$\hbar_{\mathrm{Miura}}$ as the fourth
parameter]\label{prop:hbar-miura-3hbar}
The \emph{Miura coupling}
\[
 \hbar_{\mathrm{Miura}} \;:=\; \frac{\Psi - 1}{\Psi}
 \;=\; \frac{1}{k + N}
\]
is the principal-$W$ companion of the three affine couplings.
At $\fg = \fsl_N$ and $k$ non-critical, $\hbar_{\mathrm{Miura}} =
\hbar_{\mathrm{KZ}}/(k + N - h^\vee + h^\vee) = 1/(k+N) =
\hbar_{\mathrm{KZ}}$, so the four couplings all reduce to the
shifted-level reciprocal $1/(k+h^\vee)$ on $\fsl_N$ (where $h^\vee =
N$).
\end{proposition}

\begin{proof}
Direct: $\Psi - 1 = 1/(k+N-1)$ inverted gives $\Psi = (k+N)/(k+N-1)$,
hence $(\Psi-1)/\Psi = 1/(k+N)$. For $\fsl_N$, $h^\vee = N$, so
$k+N = k+h^\vee$ and the Miura coupling equals $\hbar_{\mathrm{KZ}}$.
\end{proof}

% ================================================================
\section{Four Yangian types must be kept distinct}
\label{sec:four-yangian-types-3hbar}
% ================================================================

The three parameters of Theorem~\ref{thm:three-parameter} extract
the \emph{same} coupling from three \emph{different} algebraic
structures. A prerequisite for stating this precisely is
distinguishing four Yangian notions that the framework uses
interchangeably in less-disciplined expositions.

\begin{definition}[Four Yangian types]
\label{def:four-yangian-types-3hbar}
We fix the following four types with superscript discipline:
\begin{enumerate}[label=\textup{(\arabic*)}]
 \item $Y_\hbar(\fg)$ \emph{classical}: the $E_1$-topological
 Yangian on $\RR$, Drinfeld's 1985 first presentation.
 \item $Y^{\mathrm{dg}}_\hbar(\fg)$ \emph{dg-shifted}: the derived
 Yangian at a point or on the formal disk, carrying a shifted
 symplectic structure.
 \item $Y_\hbar(\fg)^{\mathrm{ch}}$ \emph{chiral}: $E_1$-chiral on
 a smooth curve $X$, presented as a $D$-module with spectral
 parameter in the ordered collision coordinate. This is the home
 of the Yangian for the Drinfeld--Kohno bridge.
 \item $Y_\hbar(\fg)^{\mathrm{spec}}$ \emph{spectral}: the
 factorization algebra on $\mathbb{A}^1_u$ obtained by transposing
 the spectral coordinate with the worldsheet coordinate.
\end{enumerate}
The three-parameter identification of
Theorem~\ref{thm:three-parameter} is between $\hbar$-parameters of
type~(3), the chiral Yangian $Y_\hbar(\fg)^{\mathrm{ch}}$.
\end{definition}

\begin{remark}[Two Yangian definitions on the chiral type]
\label{rem:two-yangian-defs-3hbar}
Within type~(3), two abstract definitions coexist: (a) an
$E_1$-factorization algebra on $X$ together with an RTT
presentation, and (b) a quadruple $(\cA, S(z), \Delta^{\mathrm{ch}},
\{m_k\})$ consisting of a chiral algebra, an $S$-operator, a
chiral coproduct, and a modular MC tower. Definition~(b) is
strictly stronger because the MC tower is not derivable from the
RTT data alone. The three $\hbar$'s of
Theorem~\ref{thm:three-parameter} agree under both definitions: (a)
sees $\hbar$ as the RTT deformation parameter; (b) sees it as the
leading coefficient of $m_2$. Equality of the two definitions on
the ordered Koszul locus is the content of the chiral QG
equivalence theorem.
\end{remark}
...
The divided-power convention ($\lambda^{(n)} = \lambda^n/n!$
