...
The numerical prefactor $1/(k + h^\vee)$ is a single scalar. Three
recent papers extract this scalar from three different directions,
...
identifications below hold in the KZ convention; the trace-form
analogues are obtained by the Sugawara rescaling
$\hbar \mapsto \hbar \cdot 2h^\vee \cdot k(k+h^\vee)$.
\end{remark}
...
and the $\lambda$-bracket (divided-power convention, $\{-_\lambda -\}_{(n)} = \lambda^n/n!$) yields the $r$-matrix with the
...
$r(z) = \hbar\,\Omega/z$. The loop parameter is the formal deformation
...
\begin{theorem}[Miura cross-universality, AP128/FM31
corrected]\label{thm:miura-cross-universality-3hbar}
...
\begin{remark}[What the AP128/FM31 correction fixes]
\label{rem:ap128-fix-3hbar}
Earlier drafts wrote the cross-term as $(1/\Psi)\bigl(J \otimes W_{s-1}
+ W_{s-1}\otimes J\bigr)$. At $\Psi = 2$ this coincides numerically with
$(\Psi-1)/\Psi = 1/2$, the coincidence that masked the error
(AP186: verify at three parameter values). The Beilinson
discipline substitutes $\Psi = 3$: then $(\Psi-1)/\Psi = 2/3$ but
$1/\Psi = 1/3$. A direct Miura computation at $N = 3$, spin $s = 3$,
$\Psi = 3$ (verifiable by elementary symmetric expansion of
$T(u) = \prod_{i=1}^3 (u + \Lambda_i)$, AP187) reads off $2/3$, not
$1/3$.
\end{remark}
...
distinguishing four Yangian notions that the programme uses
interchangeably in less-disciplined expositions.

\begin{definition}[Four Yangian types, AP159]
...
\begin{remark}[Two Yangian definitions, AP170]
