...
(Theorem~\ref{thm:shadow-eisenstein}) is an instance of
the Rankin--Selberg method, connecting the arithmetic content
of OPE data to classical multiplicative number theory.
The Hecke decomposition of the constrained Epstein zeta function
at higher rank (Section~\ref{sec:epstein}) parallels the
Langlands decomposition of Eisenstein series for $\mathrm{GL}_n$;
the connection to automorphic representations is structural
rather than conjectural at the present stage.
...
% ================================================================
% SECTION 4B: ARITHMETIC DUALITY (Wave-14 reconception anchor, 2026-04-17)
% ================================================================
...
$\{S_r(\mathrm{Vir}_c)\}$. Full proofs are inscribed in
\texttt{chapters/theory/z\_g\_kummer\_bernoulli\_platonic.tex},
\texttt{chapters/theory/higher\_kummer\_arithmetic\_duality\_platonic.tex},
and \texttt{chapters/theory/shadow\_tower\_higher\_coefficients.tex}.

\subsection{The duality}

\begin{theorem}[$Z_g$--$S_r$ arithmetic duality]
\label{thm:z-g-s-r-arithmetic-duality-standalone}
...
absent from $S_r(\mathrm{Vir}_c)$ through $r = 11$. Explicitly:
\begin{enumerate}[(i)]
...
\item\label{duality-iii}
 $\{691, 3617\} \not\subset \mathrm{supp}(S_r(\mathrm{Vir}_c))$
 for $2 \le r \le 11$.
\end{enumerate}
\end{theorem}

The duality is sharp at $\{691, 3617\}$: the next Kummer-irregular
prime $2111$ appears in $N_9$ via the factorisation
$29554 = 2 \cdot 7 \cdot 2111$, but is \emph{not} witnessed on $Z_g$
at $g \le 9$ because $2111 \nmid \mathrm{num}(B_{2m})$ for
$2m \le 16$. Geometric disjointness: $Z_g$ is a Verlinde invariant
localised on $\mathrm{Bun}_{SL_2}(\Sigma_g)$, while $S_r$ is a
shadow coefficient localised on $\mathrm{ChirHoch}(\mathrm{Vir}_c)$;
the two arithmetic spectra are computed on disjoint moduli.

\subsection{Riccati-arithmetic characteristic primes}

The $S_r(\mathrm{Vir}_c)$ numerators through $r = 11$ contain the
Kac-module \emph{characteristic primes}
$\{61, 193, 2111, 16657, 3988097, \ldots\}$, roots of
Shapovalov--Kac factors, arising from the Riccati recursion
$H(t)^2 = t^4\,Q_L(t)$ on the Virasoro primary line.

\begin{remark}[Retractions; B92 reconciliation]
\label{rem:kummer-retractions}
At primary-source Bernoulli-witness verification, the primes
$\{1423, 3067, 23, 43, 419\}$ are \emph{not} Kummer-irregular and
are retracted from the Kummer-irregular label previously used in
draft forms of this arithmetic. They still appear in the
$S_r(\mathrm{Vir}_c)$ numerators as \emph{Riccati-arithmetic}
characteristic primes. The corrected Tier-3 emergence set at
current verification depth is $\{37, 691, 811\}$; $3067$ is
removed. The structural distinction is geometric: Kummer-arithmetic
primes track $B_{2g-2}$ on the Verlinde side, Riccati-arithmetic
primes track the Kac determinant on the shadow side, and the two
spectra are disjoint at the witnessed depth.
\end{remark}
...
emergence is Riccati-seed-driven: they are roots of the Kac
determinant at specific weights that the shadow recursion
propagates, and they constitute a structural signature of the
Virasoro shadow geometry that is invisible to the Hurwitz--Bernoulli
leading term on $Z_g$.
\end{remark}
