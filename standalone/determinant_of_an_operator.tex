...
The terminal object is the \emph{virtual $K_0$-determinant package} on
...
the open problem F11 in the Vol~I frontier.  The scalar shadow is
constructed here; the operator-level Pfaffian is the
distinct second-quantised string construction still to be made.
The separation between scalar shadow and operator algebra
(scalar-operator) is enforced throughout: every invocation of $\Delta_5$ is
...
last item is the modular-shadow realisation constructed below.
...
distinction is enforced by the shadow-object distinction1
...
invariant in five guises (master pattern five-invariant). The Mukai-K3 conductor
$K^{\kappa_{\mathrm{ch}}}_{\mathcal B} = 2 c_+(\mathrm{Mukai}(K3)) = 8$
is yet a sixth scalar, again on a separate lane: it is the Mukai
positive-signature half, not the Borcherds weight. The additive
formula $\kappa_{\BKM} = \kappa_{\mathrm{ch}} +
...
scalar-operator). The Igusa cusp form $\Delta_5$ is a level-$5$ scalar; it is
...
determinant theorem; the reverse direction is the open problem
F11 of the Vol~I frontier.
\end{proof}

\begin{corollary}[Scalar shadow and operator algebra]
\label{cor:scalar-shadow-operator-algebra}
$\Delta_5$ is the Borcherds denominator at level $5$. The level-$4$
operator algebra is reconstructed only from operator data; the
scalar denominator supplies its shadow and not its Hilbert space,
path integral, or Pfaffian algebra.
\end{corollary}
...
\begin{remark}[Compact-realisation reduction]
\label{rem:vd-heal-path}
The compact-realisation problem factors through the local Borcherds
and Hall--Drinfeld target data of~\cite{LorgatIgusa} and the
CY-to-chiral two-stage functor of~\cite{LorgatVol3},
...
\end{array}
\]
The determinant construction closes at level $5$. Level $6$
is the second-quantised string construction, distinct in level and in
...
\cite{LorgatVol3}. The scalar $\Delta_5$ constructed here is the
...
The determinant chain extends into the modular-shadow
regime. The virtual $K_0$-determinant package has
scalar value $\Delta_5$ on $K3 \times E$, a
...
E$, and the Borcherds singular-theta lift of $\phi_{0,1}$. The scalar-operator
