...
\section{Finite chain-matrix calculation}
For each total polynomial weight \(w\leq5\), take the normalized bar basis
formed by words in nonunit monomials and grade it by word length.  The
alternating adjacent-multiplication differential gives an exact rational
matrix.  For the quantum plane set \(q=2\); its ranks are generic away from
the finite determinantal exceptional locus.  The resulting kernel/image
dimensions are:
...
\end{tabular}
\end{center}
All weights four and five have zero homology.  Direct application of the
degree-two differential sends each displayed cycle to zero.
...
\chapter{Enveloping algebras and planar-sequence obstructions}\label{chap:enveloping}
...
\begin{theorem}[Non-identification of Motzkin and Riordan sequences with ordered bar homology]\label{thm:legacy-audit}
...
\begin{statusbox}
Finite calculation.  Exact rational elimination on the weight-graded
Chevalley--Eilenberg complex of \(L_1\) gives the cohomology for
\(n=1,2,3,4\), with \(d^2=0\) checked on every cochain space,
occupying the weights \((1,2)\), \((5,7)\), \((12,15)\), \((22,26)\), one
dimension in each.  The nonzero coefficients of \(\prod_{n\ge1}(1-q^n)\) occupy
\(0,1,2,5,7,12,15,22,26\) with signs \(+,-,-,+,+,-,-,+,+\).  This
finite calculation is evidence for, but is not used in place of, the
all-degree argument from Goncharova's theorem and Euler's identity.
\end{statusbox}
...
At \(q=2\), evaluation of the first relation in the basis
\(([X],[Y])\) gives the zero vector in \(\mathbb Q^2\).
...
Let the representation categories vary in a factorisation family over \(X\).  Extension correspondences at fixed support define the transverse Hall multiplication; disjoint support gives the chiral coefficient.  If pull--push preserves the factorisation system and orientations, the Hall algebra is a transversely ordered chiral algebra.  If the sheaf theory itself carries ordered chiral convolution, the derived extension algebra becomes a doubly associative chiral object.  The finite-field calculation above gives the exact local structure constants.
...
For the finite label set \(0\le a,b\le3\), \((a,b)\ne(0,0)\), all
\(15^3=3375\) ordered Jacobi sums have zero coefficient in the free
\(\mathbb Q\)-module on the resulting labels.
