...
| ChirHoch^1 KM | PROVED degree-1 at generic k for general simple g; total-dim formula sl_2 only | ChirHoch^1(V_k(g))=g at generic level k ≠ -h^v for general simple g (prop:chirhoch1-affine-km, chapters/theory/chiral_center_theorem.tex:2132-2197). Total dim = dim(g)+2 inscribed ONLY for sl_2 (1+3+1=5) via rem:sl2-chirhoch-dim5 (line 2199-2215) routing through prop:derived-center-explicit(ii) (line 1996-2021), which proves ChirHoch^2(V_k(sl_2)) = 1-dim in rank 1 only. General simple g case OPEN pending ChirHoch^2(V_k(g)) computation beyond rank 1. |
...
| E_3 identification | **PROVED simple g, affine KM V_k(g) at generic non-critical level (thm:e3-identification, en_koszul_duality.tex:5329)**; gl_N EXTENSION via rem:e3-non-simple-gl-N (2-parameter, B_tr and B_ab); non-simple reductive/semisimple case addressed only at remark level in rem:e3-non-simple (Sym^2(g^*)^g controls P_3 biderivations; dim = r + C(dim z+1, 2)); nilpotent/solvable/Heisenberg cases NOT INSCRIBED as theorems | Z^der_ch(V_k(g)) ≅ A^λ as E_3-deformation families over λ H^3(g)[[λ]], λ = k+h^v. **E_3-top vs E_3-chiral scope (AP-TOPOLOGIZATION)**: thm:e3-identification(iv) upgrades to E_3-TOPOLOGICAL via Sugawara only at non-critical level; without conformal vector stuck at E_3-chiral (Dunn-colored E_2-chiral ⊗ E_1-top). Critical level k=-h^v: Sugawara undefined, perturbative only. **Strengthening-inscription audit (2026-04-17, post-Wave-5 AP255 check)**: of the nine items previously advertised, ONE is inscribed (part (iv) Sugawara unconditional, inside thm:e3-identification), ONE is partial (Fresse operad→algebra formality cited at :5420 via Fresse Vol II Thm 16.1.1, NOT 16.2.1 — citation drift), ONE exists as remark only (gl_N 2-parameter case at rem:e3-non-simple-gl-N, :5567, NOT promoted to prop:e3-gl-N); SIX ARE PHANTOM (AP255): prop:e3-gl-N (label absent; the content lives in rem:e3-non-simple-gl-N), thm:e3-identification-semisimple (absent; content only as remark), thm:e3-identification-reductive (absent), prop:e3-heisenberg (absent), thm:e3-identification-solvable (absent), and the "explicit E_3-algebraic vs E_3-topological comparison diagram" (absent). Item (9) CY_4 p_1-twisted: this row cross-linked to the CY_4 row which itself is CONSTRUCTED+CONJECTURED (not PROVED) with the K3×K3 N(X)=0 subclaim uninscribed (generically false since p_1(K3)≠0); the E_3 identification claim for CY_4 is inherited-conjectural at best. Honest count: 1 inscribed + 1 remark-level + 1 partial citation ≠ "9 strengthening inscriptions". Healing options: (a) inscribe the five missing theorems/propositions; (b) narrow the row to "PROVED simple g at non-critical level via thm:e3-identification; gl_N via rem:e3-non-simple-gl-N; semisimple/reductive at remark level; nilpotent/solvable/Heisenberg UNINSCRIBED." Until (a) occurs, (b) is the accurate status. |
...
| Chiral QG equiv | **ORDERED: inscribed as `thm:chiral-qg-equiv` in `standalone/ordered_chiral_homology.tex:2266` AND `chapters/theory/ordered_associative_chiral_kd.tex:8404`, but standalone is NOT `\input`-ed into `main.tex` (AP255 phantom-file-in-consumers; ~35 cross-chapter `\ref{thm:chiral-qg-equiv}` consumers render `[?]` at build per agent a10321); Koszul-locus-CONDITIONAL and load-bearing on JKL26 at `N \geq 3` per agent a290d (`prop:ff-screening-coproduct-obstruction` confirms Feigin–Frenkel is NOT a drop-in replacement; the $(\Psi-1)/\Psi$ obstruction is explicit). ELLIPTIC: NOT INSCRIBED — zero hits for `\label{thm:chiral-qg-equiv-elliptic}` in Vol I source per agent af93; the prior "PROVED" claim conflates Belavin with Felder and carries AP275 propagator drift; only `standalone/seven_faces.tex:1006` `thm:chiral-qg-equiv-elliptic-sf` exists and is standalone-only. TOROIDAL formal-disk: PHANTOM at AP255 severity per agent af2f — no `\label{thm:chiral-qg-equiv-toroidal-formal-disk}` inscribed; `standalone/seven_faces.tex:1020` `thm:chiral-qg-equiv-toroidal-sf` is standalone-only and not `\input`-ed. The 8 "strengthening inscriptions (2026-04-16)" are largely advertised-but-not-inscribed (AP241 + AP255 + AP256)** | Of the 8 advertised strengthenings: (1) `def:ordered-koszul-chiral-algebra` NOT FOUND in any chapter; (2) `prop:yangian-ordered-koszul` NOT FOUND; (3) `thm:chiral-qg-equiv-ordered` NOT FOUND (only the unsuffixed `thm:chiral-qg-equiv` exists); (4) `prop:sl2-yangian-triangle-concrete` NOT FOUND; (5) `thm:glN-chiral-qg` inscribed at `ordered_associative_chiral_kd.tex:10324` but the "bypasses JKL26" claim is retracted (AP256 aspirational-heal drift) — JKL26 is RETAINED at $N \geq 3$ with the obstruction made explicit, not bypassed; the BIALGEBRA/Drinfeld-double scope caveat stands; (6) `thm:w-infty-chiral-qg-completed` NOT FOUND; (7) `thm:grt1-rigidity` exists only as `\phantomsection\label{thm:grt1-rigidity}` in `chapters/frame/preface.tex:5079` (phantomsection mask, AP255); (8) `thm:chiral-qg-equiv-elliptic` and `thm:chiral-qg-equiv-toroidal-formal-disk` NOT FOUND in chapters. Three-structure determination (R-matrix / A_inf / ordered coproduct) genuinely inscribed only for sl_2 Yangian + affine KM at the abstract Koszul-locus level per AP174; concrete verification beyond those two families remains OPEN. Healing priorities: (a) `\input` `standalone/ordered_chiral_homology.tex` into `main.tex` or inline-inscribe `thm:chiral-qg-equiv` into a chapter; (b) retract elliptic+toroidal "PROVED" language programme-wide until the theorems are inscribed; (c) per AP263, audit whether "quantum group equivalence" should be "bialgebra equivalence" since the antipode lift is a proved negative. |
...
| 6d hCS defect | PROVED | Codim-2 defect on C⊂C³: boundary algebra = W_{1+inf} with Psi=-sigma_2. c=1 (Sugawara). N_{C/Y}=C² gives spectral params. 49 tests. |
...
| Drinfeld double vs derived center (Heis) | **CONJECTURED** | `conj:v1-drinfeld-center-equals-bulk` (preface.tex:4228): $Z(U_{\cH_k}) \simeq \cZ^{\mathrm{der}}_{\mathrm{ch}}(\cH_k)$ as $E_3^{\mathrm{top}}$-algebras, conditional on topologization hypotheses of preface §10.2. Three obstructions flagged (pointwise reduction at class M; Verdier dual factorisation; bar-cobar/Hochschild compatibility). |
| Toroidal chiral QG (formal disk) | PROVED on formal disk | `thm:chiral-qg-equiv-toroidal-sf` (standalone/seven_faces.tex:1020, ClaimStatusProvedElsewhere): Drinfeld-Miki toroidal algebra $U_{q,t}(\ddot{\fsl}_N)$ with two spectral parameters $(u,v)$ is the chiral quantum group of an ordered-Koszul factorization algebra on the formal disk $(\mathrm{Spec}\,\bC[\![z]\!])^{\times 2}$, witnessed via SV CoHA on instanton moduli. Global extension to $\PP^1 \times \PP^1$ CONDITIONAL on class-M chain-level topologisation frontier. Companion label `thm:chiral-qg-equiv-toroidal-formal-disk` at standalone/N3_e1_primacy.tex:1160. |
| Coderived E_3 | PARTIAL | Steps 1-2 proved (D^co stable ∞-cat; obstruction coacyclic). Step 3 open. |
| KZB flatness | PARTIAL | Prefactor 1/(4πi) diagonal vs 1/(2πi) off-diagonal verified < 10^{-14} at `rem:kzb-heat-prefactor` (ordered_associative_chiral_kd.tex:12046). Full Bernard heat identity ∂_τ ζ = ℘' verification is FRONTIER: `test_kzb_flatness_2pt` carries `@pytest.mark.xfail`; 2-point commutator-vanishing is trivial (Ω self-commutes). Prior CLAUDE.md formula "d_tau(wp_1) = (1/(4πi))d_w(wp+wp²)" was a confabulation (not inscribed anywhere in .tex or engine) — retracted 2026-04-17 per AP283. |
