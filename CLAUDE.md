...
| Koszul | 10 unconditional on generic parameters (non-critical, non-admissible, non-special); critical/admissible/minimal loci require case-by-case verification | 10 unconditional + Lagrangian (conditional) + D-mod purity (one-dir). Equiv (vii): uniform-weight all-genera / multi-weight genus-0 only; fails at critical level k=-h^v where kappa=0 and bar cohomology becomes the opers complex. Equiv (vi) Barr-Beck-Lurie is a consequence of (v), not an independent route. Equiv (viii): concentration proved; freeness conditional on Massey vanishing. "Tropical Koszulness" 13th equivalence cited in preface is heuristic, not inscribed in the meta-theorem. ALT: proof web (H09). |
...
| Topologization | **PROVED chain-level on original complex for G/critical; PROVED on Q_CS-cohomology for L; PROVED weight-completed for M** (thm:topologization-tower; post-Wave-1 Beilinson audit 2026-04-17 identifies three open directions, not one) | **Topologization tower theorem (2026-04-16; status-table corrected 2026-04-17)**: (1) class G (Heisenberg): E_3^top on original complex (commutative ⇒ Dunn on F^{H_k}); (2) class L (V_k(g), k≠-h^v): E_3^top on Q_CS-cohomology via `thm:E3-topological-km` (Costello-Li 3d hCS + Sugawara identity T_Sug=[Q_CS,G_Sug] on cohomology; a chain-level η_1 antighost-contact inscription making [Q,tilde G_1]=T_Sug strict on the original complex is NOT yet present in Vol II e_infinity_topologization.tex or bv_brst.tex, so the prior CLAUDE.md "STRICT chain-level (not up to Q-exact)" assertion is RETRACTED 2026-04-17 pending genuine η_1 computation); (3) class C (βγ, bc): E_3^top via FMS bosonization reducing to Heisenberg; (4) class M (Vir, W_N): weight-completed chain-level E_3^top via half-BRST + MC5 pattern; (5) critical level k=-h^v: E_2^top (not E_3), Sugawara collapse is a dimension drop, not a failure. **Honest frontier inventory (post-Wave-1, three open directions)**: (I) chain-level class M on the original complex in Ch(Vect) (acknowledged; most likely the right response is to declare the pro/J-adic/weight-completed ambient as part of the data of the original complex, in which case the question dissolves); (II) chain-level class L on the original complex via explicit η_1 formulae (to inscribe: compute [Q,tilde G_1] − T_Sug in the bulk BV chain complex and verify the remainder is identically zero, not merely Q-exact); (III) antighost BRST-commutativity [G^{(n)},G^{(m)}]=Q-exact at spin n,m≥3 (postulated as axiom (T5) of def:higher-spin-stress-tower, not yet derived; `thm:iterated-sugawara-construction` for n≤3 survives, the W_∞ convergence theorem inherits the conjectural status). ALT via CG factorization homology H08 no longer conjectural for G/C and M-weight-completed. |
