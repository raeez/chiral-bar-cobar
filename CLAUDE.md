...
AP176: RETRACTED. The arity→degree terminological normalisation is a DELIBERATE authorial decision. "Degree" is used uniformly throughout all three volumes for ALL indices counting inputs, tensor factors, tree valence, operadic components, Stasheff identity level, and SC mixed sector parameters. The standard operadic term "arity" is NOT used; "degree" subsumes it. This is a conscious departure from Loday-Vallette conventions (which use "arity" for operadic inputs and "weight" for bar grading). The ~25 instances of "an degree" (article mismatch from the rename) should be corrected to "a degree" as a grammatical fix.
