...
| MC5 | **PROVED in weight-completed category** for all four shadow classes; direct-sum chain-level class-M FALSE (correct scope) | Analytic: HS-sewing unconditional. Coderived: coacyclic characterization. **Chain-level class M PROVED (2026-04-16) in WEIGHT-COMPLETED category** via `thm:completed-bar-cobar-strong` + `prop:standard-strong-filtration` (already in manuscript): at each weight stage N, harmonic discrepancy is finite sum absorbed by explicit h·m_0^{j-1} chain homotopy; Milnor/Mittag-Leffler passage to inverse limit. **Direct-sum chain-level class M genuinely FALSE** (S_4(Vir_c) = 10/[c(5c+22)] nonzero at generic c forces bar cohomology in weight 4) — correct scope, not a gap. **AP203 HEALED**: uniqueness of central degree-2 m_0 via Hodge + vacuum-proportionality (H^{2,0}(curve)=0 kills competing bar-length-2 insertions). **c_r = S_r proved** (not just named): harmonic projector on H_g coincides with av on degree-r component. `prop:harmonic-factorization` duplicate + truncated proof HEALED at bv_brst.tex. **H07 UPGRADED**: direct SC^{ch,top} homotopy-Koszulity via Ginzburg-Kapranov for Com⊥Lie closed color + Ass self-duality open + Deligne-purity degeneration (Mok25 log-FM smoothness) — replaces Livernet transport. |
