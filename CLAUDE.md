...
AP176: Arity→degree over-application in operadic contexts. The "arity→degree" terminological normalisation (2026-04-12, ~4000 replacements across 3 volumes) was OVER-APPLIED in genuinely operadic contexts. "Degree" is correct for bar complex grading (position in T^c filtration). "Arity" is correct for: (a) number of inputs to an operadic operation ("a multilinear operation of arity n"); (b) endomorphism operad component ("f in End^ch_A(p)" has arity p, NOT degree p); (c) tree vertex valence in operad theory ("internal vertices of arity >= 2"); (d) SC operad mixed sector bi-arity ("at bi-arity (k,m)"); (e) brace operation input count ("f{g_1,...,g_r} has arity p+Q-r"); (f) cup product inputs ("f of arity p and g of arity q"). CONFIRMED instances requiring correction: chiral_center_theorem.tex lines 95, 322, 382-383, 524-525, 541-543, 746, 1175, 1241, 1366; homotopy_transfer.tex line 255; en_koszul_duality.tex line 1432. Decision tree: Is the index counting (a) tensor factors in a COALGEBRA → degree, or (b) inputs to an OPERATION → arity? If (b), use arity.
