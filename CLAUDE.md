...
- B64. "A multilinear operation of degree n." CORRECT: "of arity n" (operadic inputs). AP176.
- B65. "f of degree p (i.e., f in End^ch_A(p))." CORRECT: "of arity p." AP176.
- B66. "internal vertices of degree >= 2" for trees. CORRECT: "of arity >= 2" in operad theory. AP176.
- B67. "at bi-degree (k,m)" for SC mixed sector. CORRECT: "at bi-arity (k,m)." AP176.
...
**FM29. Bulk rename without operadic context awareness.** Opus applied a global "arity→degree" rename across ~4000 instances without distinguishing bar-complex grading (degree) from operadic input count (arity). The rename was correct for ~95% of instances (bar filtration, shadow tower levels, MC truncations) but incorrect for ~5% (endomorphism operad components, brace operation inputs, tree vertex valence, SC mixed sector bi-arity, multilinear operation inputs). The error pattern: Opus treated "arity" as a deprecated synonym for "degree" rather than as a distinct concept with its own domain. Counter: before any bulk rename, enumerate the DISTINCT SEMANTIC DOMAINS of the target word and verify the replacement is correct in each domain. For arity: (1) bar filtration → degree; (2) operadic inputs → arity (KEEP); (3) tree valence → arity (KEEP); (4) SC sector → bi-arity (KEEP).
...
AP176: Arity→degree over-application in operadic contexts (~85+ instances across 3 volumes). The "arity→degree" terminological normalisation (2026-04-12, ~4000 replacements) was OVER-APPLIED in genuinely operadic contexts. "Degree" is correct for bar complex grading (position in T^c filtration). "Arity" is correct for: (a) number of inputs to an operadic operation ("a multilinear operation of arity n"); (b) endomorphism operad component ("f in End^ch_A(p)" has arity p, NOT degree p); (c) tree vertex valence in operad theory ("internal vertices of arity >= 2"); (d) SC operad mixed sector bi-arity ("at bi-arity (k,m)"); (e) brace operation input count; (f) cup product inputs; (g) cooperad/operad component P(n) ("arity-n component"); (h) Stasheff identity at arity n (the A_inf relation involves compositions of m_k with k inputs — that k is operadic arity); (i) "arity-wise linear dual" (standard operadic terminology); (j) even/odd-arity vanishing of m_k. FIVE SYSTEMATIC ERROR CLASSES: (1) Stasheff identities: "degree-n Stasheff" → "arity-n Stasheff" (~30 instances in Vol II 3d_gravity.tex, w-algebras-*.tex); (2) Endomorphism operad: "of degree p" for f in End(p) → "of arity p" (~10 instances in chiral_center_theorem.tex); (3) Cooperad components: "degree-n component of Lie^c" → "arity-n component" (~15 instances in algebraic_foundations.tex, standalones); (4) SC bi-arity → bi-degree (~4 instances); (5) Article mismatch: "an degree" → "a degree" or "an arity" (~25 instances across all 3 volumes, grammatical artifact of the rename). Decision tree: Is the index counting (a) tensor factors in a COALGEBRA → degree, or (b) inputs to an OPERATION → arity? If (b), use arity.
