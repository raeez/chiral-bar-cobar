...
\subsection{Internal screening domination and the reduction of the mass-gap problem}
\label{subsec:internal-screening-domination-reduction}

\begin{summary}[From defect algebra to a spectral criterion]
Theorem~\ref{thm:conditional-mass-gap-transfer} isolates the final analytic step in the programme:
one needs an untwisted Hamiltonian $H_{\mathrm{YM}}$ and a domination estimate by the screening Laplacian.
The point of the present subsection is to show that the screening side of the comparison is already
\emph{internal} to the instanton-completed boundary package.
More precisely, once one equips the defect algebra with a positive vacuum state,
central screening classes produce a canonical positive quadratic form on the GNS space,
and the mass-gap problem reduces to bounding $H_{\mathrm{YM}}$ below by this internal form on a dense
algebraic core.
\end{summary}
...
We say that $(\A_{\mathrm{def}},\tau,\mathbf s)$ is a \emph{screening state datum} if:
\begin{enumerate}[label=(\roman*)]
...
and let
\[
\mathcal D_\tau:=\pi_\tau(\A_{\mathrm{def}})\Omega_\tau\subset \mathcal H_\tau
\]
...
Since $s_i$ is central, $S_i$ commutes with the left action of $\A_{\mathrm{def}}$.
...
Let $(\A_{\mathrm{def}},\tau,\mathbf s)$ be a screening state datum.
Then:
\begin{enumerate}[label=(\alph*)]
...
\]
holds on $\mathcal H_\tau$, where $P_0$ is the orthogonal projection to $\C\Omega_\tau$;
...
\begin{proof}
Parts (a) and (b) are immediate from Construction~\ref{constr:internal-screening-operators}.
The centrality of the $s_i$ gives commutativity of the $S_i$, and positivity of $H_{\mathrm{scr}}^{\mathrm{int}}$
is obvious from the definition.  The condition $\tau(s_i^*s_i)=0$ means precisely that
$\|S_i\Omega_\tau\|^2=0$, hence $S_i\Omega_\tau=0$ and therefore
$H_{\mathrm{scr}}^{\mathrm{int}}\Omega_\tau=0$.
Part~(c) is the defining identity of the GNS inner product.
For part~(d), note that vectors of the form
$[a]-\tau(a)\Omega_\tau$ span a dense subspace of the orthogonal complement of the vacuum.
Thus the operator inequality implies the algebraic inequality by restriction.
Conversely, the algebraic inequality defines a lower bound for the quadratic form of
$H_{\mathrm{scr}}^{\mathrm{int}}$ on a form core, hence extends by continuity to the whole Hilbert space.
\end{proof}
...
Let $(\A_{\mathrm{def}},\tau,\mathbf s)$ be a screening state datum.
\begin{enumerate}[label=(\alph*)]
\item We say that $(\A_{\mathrm{def}},\tau,\mathbf s)$ has \emph{screening gap} at least $\mu>0$
...
\begin{remark}[Why the estimate is internal]
\label{rem:why-screening-estimate-is-internal}
The quantity $\mathfrak s_{\mathbf s}(a)$ depends only on three pieces of data extracted from the
instanton-completed boundary package: the defect algebra $\A_{\mathrm{def}}$, the vacuum state $\tau$,
and the central screening sequence $\mathbf s$.
No external geometric model is needed to define it.
Thus the only genuinely non-formal input in the mass-gap problem is the comparison of the physical
Hamiltonian with a quadratic form already present in the defect algebra itself.
\end{remark}
...
Let $(\A_{\mathrm{def}},\tau,\mathbf s)$ be a screening state datum with screening gap at least $\mu>0$.
...
\]
In particular, the physical Hamiltonian has a positive mass gap at least $\alpha\mu^2-\varepsilon$.
\end{theorem}

\begin{proof}
Let $\psi=[a]-\tau(a)\Omega_\tau\in \mathcal D_\tau\cap (\C\Omega_\tau)^{\perp}$.
Since $H_{\mathrm{YM}}\Omega_\tau=0$, one has
\[
...
\]
Thus the quadratic form of $H_{\mathrm{YM}}$ is bounded below by $\alpha\mu^2-\varepsilon$ on a dense core
for the orthogonal complement of the vacuum.
By closure of the quadratic form and the min--max principle,
...
\begin{proof}
This is exactly Theorem~\ref{thm:mass-gap-reduction-internal-screening} rewritten using the definition of
$\mathfrak s_{\mathbf s}$.
\end{proof}

\begin{remark}[The remaining frontier]
\label{rem:remaining-frontier-after-reduction}
The reduction theorem separates the problem into two clean pieces.
...
The formalism now contains the correct internal target for the analytic estimate.
What it does not yet supply is the untwisting map that would identify the physical Hamiltonian with an operator
