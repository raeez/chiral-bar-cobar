...
| 78 | r(z)=Omega/z bare | Closed 2026-06-15 | Bare \(\Omega/z\) is not an affine trace-form formula. For affine Kac--Moody current residues the trace-form collision kernel is \(r_k(z)=k\Omega/z\), so it vanishes at \(k=0\). KZ uses \(r_{\mathrm{KZ}}(z)=\Omega/((k+h^\vee)z)\); Yangian/RTT/Costello and level-one lattice checks may use the unit-normalized Casimir kernel only when named. | Patched Vol I affine \(\mathfrak{sl}_3\), quantum-group, Bethe, DK/KZ, Costello, defect, Yangian, and AP49 surfaces; patched Vol II affine/Koszul/Laplace/holographic engines and tests; labeled the Vol III KZ test. Targeted affine scans now find only \(k\Omega/z\), KZ, unit-normalized, or audit-note uses. | closed: normalization/level |
...
| 81 | r^Vir=(c/2)/z^4 | Closed 2026-06-15 | The Virasoro OPE/Laplace kernel has the fourth-order term \((c/2)(z-w)^{-4}\), but the collision \(r\)-matrix is \(r^{\Vir}(z)=(c/2)z^{-3}+2T/z\); \(d\log\)-absorption lowers pole order by one and the \(\partial T/z\) OPE term becomes regular. | Patched the Vol III averaging test text so the pre-averaged Virasoro kernel is no longer described as a scalar simple-pole \(r(z)\). Live scans find only correct OPE/Laplace fourth-order statements or explicit negations of the false collision formula. | closed: OPE vs collision |
| 82 | S_4=-(5c+22)/(10c) | Closed 2026-06-15 | On the regular Virasoro locus \(c(5c+22)\ne0\), the quartic shadow is the reciprocal of the Zamolodchikov norm: \(S_4(\Vir_c)=10/[c(5c+22)]\). Hence \(\Delta=8\kappa S_4=40/(5c+22)\); \(c=-22/5\) is singular, not a finite tower-termination point. | Patched Vol I A-infinity/nonformality engines and tests; independent shadow-metric, discriminant, and higher A-infinity tests verify the reciprocal formula. | closed: reciprocal/S4 |
| 83 | kappa+kappa'=0 universal | Closed 2026-06-15 | The scalar Verdier/Koszul conductor \(K_\kappa(A)=\kappa(A)+\kappa(A^!)\) is family-dependent: it vanishes only on KM/Heisenberg/free/lattice anti-symmetric lanes; Virasoro gives \(13\), BP gives \(98/3\), and \(\mathcal W_3\) gives \(250/3\). | Patched Vol I `physics_horizon` universal wording, Niemeier lattice scope guards, and Vol III mirror-Euler scope guards. | closed: family-dependent conductor |
| 84 | Bar-cobar=bulk | Closed 2026-06-15 | Four objects separated: \(B(A)\) is the twisting coalgebra; \(\Omega B(A)\simeq A\) is inversion; \(D_{\Ran}B(A)\)/\(A^!\) is the Verdier/Koszul line object; \(Z^{\der}_{\ch}(A)=\ChirHoch^\bullet(A,A)\) is the bulk model. | Patched `compute/lib/holographic_entanglement_qec_engine.py` and its tests so bar-cobar is only an algebraic code skeleton, not a physical bulk Hilbert space; active theorem surfaces already carried the guard. | closed: four-object |
...
| 86 | Algebraic E_3=topological E_3 | Closed 2026-06-15 | Distinct: \(E_3^{\mathrm{alg}}\) from Higher Deligne/Kontsevich and \(E_3^{\mathrm{top}}\) from locally constant/framed little disks agree only after a named formality/topologisation/realisation package. | Patched `compute/lib/e3_bar_cobar_engine.py`, `compute/lib/theorem_cs_knot_invariant_engine.py`, and `compute/lib/factorization_homology_engine.py`; existing Vol II/III theorem surfaces already carried this guard. | closed: two-structure |
| 87 | Bare "Hochschild" | Closed 2026-06-15: bare Hochschild is forbidden where the ambient carries content. The programme distinguishes ordinary/\(E_1\) or topological Hochschild \(\HH^{E_1}_\bullet,\HH^{\mathrm{top}}_\bullet\), chiral Hochschild \(\HH^{\mathrm{ch}}_\bullet,\ChirHoch^\bullet\) / derived chiral centre on the curve, and categorical Hochschild \(\HH^{\mathrm{cat}}_\bullet,\HH^\bullet_{\mathrm{cat}}\) of dg/CY categories. Bridges require an explicit comparison theorem (annular bar, BZFN/Keller, HKR/Zhu, local-global FM). | Patched Vol I derived-centre/annulus compute and Heisenberg/entanglement/higher-genus annulus surfaces; patched Vol II annulus examples and gravity trace formulas; patched Vol III chiral-formalism and Drinfeld-centre notes so BZFN is categorical, the derived centre is chiral, and torus trace is topological. Focused stale scans on repaired surfaces find no remaining bare `HH_*`/`HH_0` forms. | closed: Hochschild register explicit |
...
| 89 | SC Koszul self-dual | Closed 2026-06-09: \(SC^{\mathrm{ch,top}}\) is Koszul/homotopy-Koszul, but it is not a fixed point of the Koszul-duality involution. The duality functor satisfies \((P^!)^!\simeq P\); it does not imply \(P^!\simeq P\). On the associated-graded Swiss-cheese shadow, \(SC^!=(Lie^c,Ass^c,\text{shuffle-mixed})\neq SC=(Com,Ass,\text{product-mixed})\); at chain level the closed \(E_2\) sector is self-dual only up to shift, while the two-coloured mixed sector still changes. | Patched Vol II active source symbols and prose: the proposition label now reads `prop:sc-koszul-duality-nonselfdual`, the foundations sentence says operadic Koszul duality rather than self-duality, the guard remark names non-self-duality of \(SC^{\mathrm{ch,top}}\), and quarantined notes no longer assert \((SC^{\mathrm{ch,top}})^!\cong SC^{\mathrm{ch,top}}\). Vol I compute guards verify the dual sector dimensions. | closed: Koszul not self-dual |
| 90 | A^! is SC-algebra | Closed 2026-06-09: \(A^!\) is a Koszul-dual/line algebra governed by the dual Swiss-cheese structure \(SC^!\), not an algebra over the original \(SC=(Com,Ass)\). In the classical shadow the closed dual sector is Lie/Sklyanin and the open sector is Ass/Yangian; at cooperad level \(SC^!=(Lie^c,Ass^c,\text{shuffle-mixed})\). | Active Vol II theorem surfaces already state the \((SC^{\mathrm{ch,top}})^!\)-algebra type. Vol I `sc_koszul_dual_cooperad_engine.py` was corrected at the mixed \((1,1)\) sanity check and verifies closed Lie, open Ass, and mixed shuffle dimensions. Stale scans over active Vol I/II/III theorem surfaces find no claim that \(A^!\) is an algebra over the original \(SC\). | closed: A! over SC!, not SC |
...
| 99 | ChirHoch cup-product freeness | Closed 2026-06-09: Theorem H gives cohomological amplitude \(\{0,1,2\}\), Hochschild duality, degree-\(\le 2\) Hilbert growth, and \(E_2\)-formality on the Koszul locus; it does not assert that \(\ChirHoch^\bullet\) is a free polynomial cup algebra. | Active Vol I/II/III scans found only guarded or conditional statements. A local nonzero square \(z^2\) is compatible with \(\ChirHoch^4=0\) and is not a freeness proof; Vol II W-algebra surfaces explicitly say "Hilbert series polynomial" is not "\(HH^\bullet\) polynomial". | closed: Hilbert growth, not free cup algebra |
| 100 | \(E_8\) fund \(=779247\) | Closed 2026-06-09: the \(E_8\) adjoint/smallest nontrivial lane has dimension \(248\). The canonical fundamental-dimension set is \(\{248,3875,30380,147250,2450240,6696000,146325270,6899079264\}\); \(779247\) is not a member. | Active Vol I code/manuscript surfaces already use \(248\); the local E8 census regression rejects injected \(779247\). Vol III's stale symmetric-cube swarm note was demoted to a non-assertion of the full decomposition. | closed: adjoint \(248\) |
| 101 | \(g=2\) stable graphs \(=6\) | Closed 2026-06-09: \(\overline{\mathcal M}_{2,0}\) has seven stable graphs, not six. The omitted graph is the barbell: two genus-\(0\) vertices, each carrying one self-loop, joined by a bridge; \(|\Aut|=8\). | Active Vol I enumeration/tests already use seven; stale Vol II graph-sum and obstruction engines were repaired to include the barbell. The seven-graph vertex-product sum gives \(\chi^{\mathrm{orb}}(\overline{\mathcal M}_{2,0})=-1/1440\), not the old six-graph \(-181/1440\). | closed: seven graphs |
...
| 103 | Vir \(S_2\) lambda normalization | Closed 2026-06-09: \(S_2=\kappa=c/2\) on the Virasoro lane. The polynomial \(\lambda^3\) coefficient \(c/12\) and the product \(T_{(3)}T=c/2\) are convention data, not the shadow invariant. | Duplicate of row 116; active Vol I/II/III surfaces now separate ordinary lambda powers, divided powers, n-th products, and shadow \(S_2\). | closed: shadow \(S_2=c/2\) |
| 104 | \(K_{\mathrm{BP}}=2\) | Closed 2026-06-09: the Bershadsky--Polyakov central-charge conductor is \(K_{\mathrm{BP}}=c(k)+c(-k-6)=196\) in the canonical FKR convention. The value \(2\) is only a local/ghost-normalisation artefact and is not the conductor. | Active Vol I/II/III compute and manuscript surfaces now use \(K_{\mathrm{BP}}=196\); the stale AP49 engine comment that preserved the old rescaled convention was removed. | closed: \(K_{\mathrm{BP}}=196\) |
...
| # | Wrong Claim | Ghost Theorem | Error | Correct | Type |
|---|-------------|---------------|-------|---------|------|
| 106 | Dunn E_1xE_1=E_2 on A | Closed 2026-06-09: Dunn additivity is real, but it applies only after a second compatible operadic direction has been constructed. It does not manufacture an \(E_2\)-structure on the native algebra \(A\) from the \(E_1\) product alone. | Active Vol I/II/III surfaces now put the programme's \(E_2\) structure on \(\Zder(A)\), \(\operatorname{Mod}_A\), \(\Rep^{E_1}(A)\), or \(\mathcal Z(\Rep^{E_1}(A))\), not on bare \(A\). | closed: Dunn needs constructed second direction |
| 107 | R-matrix promotes A E_1->E_2 | Closed 2026-06-09: \(R(z)\) supplies braiding/half-braiding on \(\operatorname{Mod}_A\), \(\Rep^{E_1}(A)\), or \(\mathcal Z(\Rep^{E_1}(A))\); it does not promote the native algebra \(A\) from \(E_1\) to \(E_2\). | Patched the Vol II spectral-braiding remark and Vol III CoHA/Yangian note cluster; Vol I R-matrix comparison is explicitly categorical. | closed: \(E_2\) on centre/category, not on \(A\) |
| 108 | Vertex algebras and \(E_\infty\) locality | Closed 2026-06-09: OPE poles do not exclude vertex/chiral algebras from the programme's \(E_\infty\)/local factorization convention. Standard vertex algebras are local; pole-freeness is only the stricter BD-commutative subclass. | Active Vol I/II/III scan found no theorem/code surface saying vertex algebras fail \(E_\infty\) because of poles. Surviving hits are antipattern notes explicitly forbidding the slogan. | closed: VAs local; poles allowed |
| 109 | \(E_\infty\) locality versus pole-freeness | Closed 2026-06-09: \(E_\infty\) in the chiral/factorization setting means local/symmetric structure; it does not mean the OPE is pole-free. BD commutative/no-pole chiral algebras form a strict subclass. | Active Vol I/II/III scan found no theorem/code surface equating \(E_\infty\) with absence of OPE poles. Surviving hits are antipattern notes explicitly forbidding the slogan. | closed: local/symmetric, not no-poles |
| 110 | Bar versus factorization homology of the line | Closed 2026-06-09: for an \(\Eone\)-algebra \(A\), \(\int_{\mathbb R} A\simeq A\). The reduced algebraic bar \(B(A)\) is the compact interval theory with augmentation boundary conditions, \(B(A)\simeq\int_{[0,1]}^{k,k}A\), not the line. | Vol II `bar-cobar-review.tex` already states the interval-versus-line proposition. Patched Vol I `higher_genus_complementarity.tex` to scope its \(\int_X\)-comparison as a geometric Ran/configuration-chain statement, and patched the Vol II test citation that described the bar as line factorization homology. | closed: line \(=A\), bar \(=\) interval with boundary |
| 111 | Deconcatenation versus chiral coproduct | Closed 2026-06-09: bar deconcatenation is the coassociative coproduct on \(T^c(s^{-1}\bar A)\), splitting words; the chiral/factorization coproduct is a separate algebraic or factorization-structure map on the chiral object. They may be compared by a theorem, not identified. | Patched `coha_bar_bridge_engine.py` where the bar coalgebra coproduct had been called the vertex coproduct. Active Vol I/II/III surfaces otherwise separate deconcatenation, coshuffle/factorization coproducts, and Yangian/Joyce coproducts. | closed: different coproducts on different objects |
| 112 | \(E_\infty\) input versus constructed \(E_3\) structure | Closed 2026-06-09: ordinary operad restriction is not the issue; the forbidden step is treating the \(E_3\) holomorphic-topological / derived-centre structure as automatic. The constructed \(E_3\) lane requires the chiral higher-Deligne or Swiss-cheese datum with its hypotheses. | Vol I/II/III scans show the active theorem/code surfaces already distinguish \(E_2\) output for \(E_1\) input, algebraic \(E_3\) for higher-Deligne input, and topological \(E_3\) only after the required 3d/HT data. Vol III toric CY wording was tightened so only the equivariance check is automatic. | closed: constructed, not automatic |
| 113 | Bar degree versus operadic \(E_1\) direction | Closed 2026-06-09: bar degree is tensor length / homological filtration in a bar complex; it is not an operadic \(E_1\) direction. An \(E_1\)-page indexed by bar degree is spectral-sequence notation, not an \(E_1\)-operadic structure. | Active Vol I/II/III scan found no theorem/code surface identifying bar degree with the operadic direction. Valid surviving language separates bar filtration, spectral-sequence \(E_1\) pages, and Dunn-additivity \(E_1\) factors. | closed: grading separated from operadic direction |
...
| 115 | Vir lambda coefficient convention | Closed 2026-06-09: in ordinary polynomial lambda powers the Virasoro central term is \((c/12)\lambda^3\); equivalently \(T_{(3)}T=c/2\) in n-th-product notation. Writing a \(c/2\) coefficient next to \(\lambda^3\) is allowed only in an explicitly declared OPE-mode convention. | Patched Vol I `w_finite_arity.py` and its test to stop presenting the n-th product as the ordinary lambda coefficient. `deformation_quantization_examples.tex` already declares OPE-mode before its \(T_{(3)}T\) display. | closed: ordinary coefficient \(c/12\), product \(c/2\) |
| 116 | Vir \(S_2\) shadow normalization | Closed 2026-06-09: \(S_2=\kappa=c/2\). The factor \(c/12\) is only the ordinary-polynomial coefficient of the \(\lambda^3\) term, i.e. \(T_{(3)}T/3!\), and never the shadow \(S_2\). | Patched the remaining active Vol I normalization prose in `w_finite_arity.py`, `test_w_finite_arity.py`, `theorem_thm_h_e3_rectification_engine.py`, and its guard test. Vol II/III had no active runtime drift for this row. | closed: \(S_2=\kappa=c/2\) |
| 117 | Vir m_3 formula errors | Closed 2026-06-09: the Virasoro cubic shadow is \(S_3=2\), independent of \(c\). In the active associator convention \(m_3(T,T,T)=-S_3T=-2T\); formulas that multiply the cubic term by \(c\) or suppress it by \(1/c\) are rejected. | The \(c\)-dependence belongs to \(S_2=\kappa=c/2\) and \(S_4=10/[c(5c+22)]\). Patched the Vol I DNP engine/tests and synchronized Vol III A-infinity/E3/CE/M5/W(2) engines so \(S_3(c)+S_3(26-c)=4\) and alpha remains \(2\). | closed: \(S_3=2\), \(m_3=-2T\) |
| 118 | betaGamma/bc swapped | Closed 2026-06-09: bosonic \(\beta\gamma_{\lambda=1}\) has \(c=2\), \(\kappa_{\mathrm{ch}}=1\). The \(\lambda=\frac12\) symplectic-boson convention gives \(\kappa_{\mathrm{ch}}=-\frac12\), while fermionic \(bc_{\lambda=1}\) / symplectic fermion has \(c=-2\), \(\kappa_{\mathrm{ch}}=-1\). | Vol I `koszulness_landscape.py` and `f2_kappa_verification_engine.py` encode \(\kappa_{\beta\gamma}(\lambda)=6\lambda^2-6\lambda+1\); Vol III `cross_volume_shadow_bridge.py` encodes \(\kappa_{bc}=-\kappa_{\beta\gamma}\). Patched the stale Vol III E3 engine/tests so the dual kappa sum is \(0\), and both class-C lanes keep charged \(S_4=-5/12\). | closed: bg \(\lambda=1\) \(\kappa=1\), bc \(\lambda=1\) \(\kappa=-1\) |
| 119 | W_N collapse E_4 | Closed 2026-06-09: the uniform \(\mathcal W_N\) PBW spectral-sequence bound is \(E_{2N}\), not \(E_4\), for \(N\ge3\). The highest generator has vertex-OPE pole order \(2N\); after the \(d\log\)/collision shift the last possible higher differential is \(d_{2N-1}\), so a generic non-Koszul quotient can collapse only on the next page \(E_{2N}\). The universal Koszul \(\mathcal W_N\) lane is separate and still collapses at \(E_2\). | Vol I active guards already encode this in `bar_pbw_spectral_sequence_engine.py` and `test_bar_pbw_spectral_sequence_engine.py`; `spectral_higher_genus.tex` states \(E_{2N}\). Patched the red-team compute wording so \(2N-1\) is only lambda-bracket/collision order, not vertex-OPE pole order. | closed: OPE \(2N\), last \(d_{2N-1}\), generic \(E_{2N}\) |
| 120 | N=4 k'=-k-2 | Closed 2026-06-09: the Feigin--Frenkel level formula is \(k'=-k-2h^\vee\). For the small \(N=4\) SCA the active affine R-symmetry line is \(\mathfrak{su}_2\), so \(h^\vee=2\) and \(k'=-k-4\); with the AP49 convention \(c=6k\), this is \(c'=-c-24\), self-dual \(c=-12\), and \(\kappa+\kappa'=-8\). | Patched the Vol I superconformal shadow engine and AP49 cross-volume verifier to use \(\kappa=c/3=2k\), and qualified the K3/CY Verdier scalar projection \(k\mapsto -k\) as a separate zero-complementarity lane. The \(W_4/\mathfrak{sl}_4\) guard remains distinct: \(h^\vee=4\), hence \(k'=-k-8\), not the small-\(N=4\) su2 line. | closed: general FF \(2h^\vee\), small \(N=4\) su2 \(h^\vee=2\) |
| 121 | FP lambda_2=1/1152 | Closed 2026-06-09: on the FP/A-hat scalar lane, \(\lambda_2^{\mathrm{FP}}=7/5760\). The value \(1/1152\) remains valid only for adjacent Witten--Kontsevich or separating/dumbbell graph contributions such as \(\langle\tau_4\rangle_2\), not for the FP scalar coefficient. | Patched the stale Vol I open-closed MC engine and guard test; Vol III geometric Langlands shadow now gives \(F_2=\kappa\cdot 7/5760\), so \(V_1(\mathfrak{sl}_2)\) has \(F_2=(9/4)(7/5760)=7/2560\), not \(1/512\). Existing Vol I/II/III FP/A-hat guards already verify \(7/5760\) independently. | closed: FP scalar separated from 1/1152 graph/WK lanes |
| 122 | Heis trivial braiding | Closed 2026-06-09: the non-zero-level Heisenberg/current braiding is scalar abelian but not trivial. After \(d\log\)-absorption the ordered-bar collision kernel is \(r_{\mathrm{ord}}(z)=k/z\) and \(R_{\mathrm{ord}}(z)=\exp(k\hbar/z)\), with checked braiding \(\tau\circ R_{\mathrm{ord}}\). | Patched `theorem_A_infinity_2.tex`, the Heisenberg/Yangian bridge tests, `w_infinity_chiral_qg_engine.py`, and the Givental/shadow R-matrix surfaces so \(R=1\) is scoped only to the pole-free, zero-level/zero-charge, Givental/shadow, E2/Gerstenhaber, or Yangian/MO nonabelian lane. Vol III K3/C3 self-dual chart files now say \(g(u)=1\) trivializes the nonabelian Yangian/MO part, not the scalar Heisenberg descent datum. | closed: scalar-not-trivial |
| 123 | J(z)J(w)~1/(z-w) | Closed 2026-06-09: the Heisenberg/current OPE is \(J(z)J(w)\sim k/(z-w)^2\); the simple pole \(k/(z-w)\) appears only after \(d\log\)-absorption in the collision/r-matrix kernel. | Patched active compute surfaces that still called the OPE or Heisenberg lane simple-pole (`test_rosetta_stone.py`, `gaiotto_3d_ht_comparison_engine.py`, `half_space_rwi.py`, `theorem_w3_commuting_hamiltonians_engine.py`, `theorem_ainfty_nonformality_class_m_engine.py`, `theorem_mc5_analytic_rectification_engine.py`). Vol II/III manuscript surfaces already state the double-pole OPE. | closed: OPE double pole, r-kernel simple pole |
| 124 | d_alg(Vir)=1 | Closed 2026-06-09: active convention rejects the row's stale \(d_{\mathrm{gen}}=1\) target. Generating depth counts operation arity: Virasoro has \(d_{\mathrm{gen}}=3\) because \(m_3\) recursively generates the higher \(m_s\); algebraic depth is \(d_{\mathrm{alg}}=\infty\) because all \(m_s\) remain nonzero. | `chapters/theory/introduction.tex`, `chapters/connections/concordance.tex`, `standalone/survey_modular_koszul_duality_v2.tex`, and `standalone/en_chiral_operadic_circle.tex` already state the two-depth separation. Guarded by `test_depth_separation_complete.py`; row 91 remains a separate duplicate ledger item. | closed: d_gen=3, d_alg=infty |
| 125 | self-dual=critical | Closed 2026-06-09: Virasoro chiral Koszul self-duality is the fixed point of \(c \mapsto 26-c\), hence \(c^*=13\); bosonic/string criticality is \(c_{\mathrm{crit}}=26\), where the dual is \(\Vir_0\), not \(\Vir_{26}\). | Patched the remaining active Vol I labels that called \(c=26\) self-dual or self-dual-adjacent (`editorial_constitution.tex`, `independent_conjectures.py`, `arithmetic_resurgence.py`, `feynman_integral_engine.py`, and targeted tests). Vol II already separates \(c^*_N=\alpha_N/2\) from \(c_{\mathrm{crit},N}=\alpha_N\) on the \(W_N\) lane; Vol III keeps the conductor \(K=13\) with self-dual \(c=13\) distinct from matter--ghost \(c=26\). Guarded by AP8/W3 tests in Vol I/II/III. | closed: self-dual/critical separated |
| 126 | Formality failure=defect | Closed 2026-06-09: active Vol I/II surfaces now treat \(d'=1\) formality failure as retained higher-operation structure, not as failure of Koszulness or construction. | `w-algebras-stable.tex` and `w-algebras-w3.tex` say non-formality is not a failure of chiral Koszulness; it is the genuine higher \(\Ainf\)/Swiss-cheese geometry of class \(\mathbf M\). `theorem_gaiotto_higher_ops_bridge_engine.py` now records AP126 explicitly: GKW's binary \(d'=1\) non-formality is refined by G/L/C/M, and the non-formal cases are features, not defects. | Guarded by `test_ap126_nonformality_is_feature_not_defect`. | closed: feature-not-defect |
| 127 | kappa/S_2 interchangeable | Closed 2026-06-09: `landscape_census.tex` now states the lane convention explicitly: unqualified \(S_2=\kappa\) is allowed only on scalar one-line lanes, namely the Virasoro stress-tensor line and the Heisenberg current line. | The AP39/AP127 verifier no longer treats Heisenberg as a negative control by importing the Virasoro \(c/2\) projection; affine \(sl_2\), affine \(sl_3\), and lattice stress-tensor projections remain negative controls. | Guarded by `test_landscape_census_verification.py`. | closed: lane/projection |
| 128 | W(2)=(betaGamma)^{Z/2} | Closed 2026-06-09: \(\cW(2)\) is now consistently routed through the rank-one symplectic-fermion parent \(\mathcal{SF}^{\mathbb Z_2}\), not the bosonic beta-gamma half-point. | `logarithmic_w_algebras.tex` states the open fixed-point-bar problem without "inheritance" as a proof mechanism; `landscape_census.tex` names \(\mathcal{SF}^{\mathbb Z_2}\). Compute surfaces now separate beta-gamma\(_{\lambda=1/2}\) (symplectic boson, \(c=-1,\kappa=-1/2\)) from symplectic fermion / \(bc_1\) (\(c=-2,\kappa=-1\)) and from \(\cW(2)=\mathcal{SF}^{\mathbb Z_2}\). Guarded by `test_koszulness_landscape.py`, `test_logarithmic_pixton.py`, `test_dmod_counterexample_search.py`, `test_landscape_census_verification.py`, and `test_discriminant_atlas.py`. | closed: wrong parent repaired |
| 129 | Agent composite confabulation | Closed 2026-06-09: active \(E_n\)-circle and seven-faces surfaces no longer stitch the real arrows into an unproved closed composite. | `standalone/seven_faces.tex` now records arrow-by-arrow status: codimension-2 restriction is proved on the Costello--Gwilliam/Costello--Gaiotto Kac--Moody locus, ordered bar is algebraic on the Koszul locus, Drinfeld centre constructs half-braidings rather than averaging, and the higher-Deligne/topologization step closes only on named comparison loci; elsewhere it is an open corridor. The same Drinfeld-centre/averaging distinction was propagated through `standalone/holographic_datum.tex`, `standalone/cy_quantum_groups_6d_hcs.tex`, `standalone/cy_to_chiral_functor.tex`, `standalone/ordered_chiral_homology.tex`, `standalone/survey_modular_koszul_duality_v2.tex`, `chapters/frame/part_iv_platonic_introduction.tex`, and `chapters/connections/holographic_datum_master.tex`. Guarded by `test_ap150_operadic_circle_is_not_composite_confabulation`; Vol II row 208 now records that its old `seven_faces.tex` target is superseded by the GRT-torsor live chapter. | closed: arrow-by-arrow status |
| 130 | Engine+test same wrong | Closed 2026-06-09: the live FM225/AP128 instance in Vol II no longer compares `modular_completion_koszul('abelian_cs', max_genus=3, k=1)` to an engine-shaped hardcoded table `[1/24, 7/5760, 31/967680]`. | `test_modular_completion_genus_data` now derives expected values from the independent Arakelov/zeta-regularised determinant expansion through `arakelov_zeta_regularised_fp_coefficients(3)`. Guarded by `test_modular_completion_genus_data_uses_independent_oracle`; Vol II audit row 274 and the FM225 finding now record the closure. | closed: independent oracle |
| 131 | Spectral R(z)=categorical braiding | Closed 2026-06-09: Vol II `spectral-braiding-core.tex` and duplicate `spectral-braiding.tex` now distinguish the meromorphic exchange family `R(z)` from the single centre-side categorical braiding natural transformation. | Vol III `quantum_chiral_algebras.tex`, `drinfeld_center_heisenberg_bulk.py`, and `chiral_homology_ran_k3.py` now state that `R(z)` is an evaluation or matrix-coefficient family of the Drinfeld-centre half-braiding, not the half-braiding itself. Guarded by `test_spectral_rmatrix_family_not_single_categorical_braiding`, `test_e2_r_matrix_is_evaluation_family`, `test_yangian_braiding_is_half_braiding_matrix_coefficient`, and `test_quantum_chiral_rmatrix_is_evaluation_family`. | closed: specific/general |
| 132 | B^FG=B^Sigma=B^ord | Closed 2026-06-09: active `ordered_associative_chiral_kd.tex`, `algebraic_foundations.tex`, and `ordered_bar_descent_engine.py` already separate the three objects: `B^{ord}` retains order/deconcatenation, `B^\Sigma` is the `R`-twisted coinvariant/factorization shadow, and `B^{FG}` is the zeroth-pole associated-graded shadow. | Standalone summaries no longer say the symmetric and FG bars are simply "the other two images" of the ordered bar. Guarded by `test_ordered_bar_descent_engine.py` and `test_factorization_coproduct_engine.py`; targeted collapse scan is clean. | closed: three-object |
| 133 | PVA=P_inf | Closed 2026-06-09: `algebraic_foundations.tex` now defines `\Pinf` as the homotopy chiral Poisson lift, while PVA/coisson is the strict descended shadow with a classical `\lambda`-bracket. | `deformation_quantization_examples.tex`, the standalone introduction surface, and `deformation_bar.py` now record the opposite directions explicitly: descent gives the PVA shadow; ascent supplies the `\Pinf` homotopy data. Guarded by `test_pinf_pva_direction_recorded_in_tex_sources`, `test_pva_and_pinf_have_opposite_directions`, and `test_pva_descends_pinf_ascends`. | closed: construction/narration |
| 134 | SC bar on R x C | Closed 2026-06-09: `en_koszul_duality.tex` now states that `\FM_k(\bC)\times\Conf_k(\bR)` is only an operation-space witness for the product operad, not a chain-level Deligne--Tamarkin output; the `E_2`/`\SCchtop` action requires passage to the derived chiral center through `thm:operadic-center-hochschild`. | `higher_dim_chiral_comparison_engine.py` no longer says the ordered `E_1` bar recovers full `E_2`; it records only the classical residue shadow. Guarded by `test_bar_product_space_requires_deligne_tamarkin_center_passage` and `test_e2_vs_e1_bar_scope`. | closed: construction/narration |
| 135 | Within-surface E_1+transverse independent | Closed 2026-06-09: Vol II now states that the Dunn split is an operadic presentation of the chiral Hochschild Hom-complex `C^\bullet_{\mathrm{ch}}(\cA,\cA)=\RHom^{\mathrm{ch}}_{\cA\text{-}\cA}(\cA,\cA)`, not a decomposition of `\cA` into two independent `E_1`-chiral structures. | The standalone curved-Dunn bridge now labels the transverse factor as the Hom-side Koszul-dual comparison factor, coupled by the twisting cochain. Guarded by `test_dunn_split_lives_on_hom_center_not_independent_e1s`. | closed: construction/narration |
| 136 | RT from `E_1` ordered | Closed 2026-06-09: the Jones/RT comparison in `ordered_associative_chiral_kd.tex` now says the ordered holomorphic-topological bar supplies monodromy input only; the link invariant requires the external Reshetikhin--Turaev ribbon trace, equivalently the `E_\infty` factorization-homology/CFG topological trace package. | Guarded by `test_rt_trace_not_raw_e1_ordered_output`. | closed: specific/general |
| 137 | Two Yangian defs equivalent | Closed 2026-06-09: Vol III `quantum_chiral_algebras.tex` no longer says the K3 RTT and OPE surfaces are equivalent presentations of the full chiral Yangian. It now records only a Heisenberg mode-algebra dictionary and states that RTT is weaker than the full chiral quadruple with modular MC tower. | Guarded by `test_rtt_ope_dictionary_not_full_yangian_equivalence`; Vol II comparison remarks already treat the chain-level equivalence as open/conditional. | closed: weak/strong |
| 138 | Miura coefficient `1/Psi` | Closed 2026-06-09: active Vol I chapter and standalone text now reserve `1/\Psi` for the single-slot Miura factor and state the universal cross-term as `1 - 1/\Psi = (\Psi-1)/\Psi`. | The equality with `1/\Psi` occurs only at `\Psi=2`; it is not the universal Miura cross-term. | closed: accidental agreement |

## XVI. k3e_cy3_programme.tex Deep Enforcement (2026-04-15, second pass, 3391 lines)

| # | Wrong Claim | Ghost Theorem | Precise Error | Correct Relationship | Type | Location |
|---|-------------|---------------|---------------|---------------------|------|----------|
| 139 | `\kappa_{\mathrm{cat}}(K3)=2` used as a K3xE-spectrum value | Closed 2026-06-09: active Vol III text no longer uses `\kappa_{\mathrm{cat}}(K3)=2` in the K3xE programme surface; the K3 value is routed through the Hodge/sigma-model chiral characteristic, while the total-space categorical value is kept at `0`. | Additional stale boundary-rank surfaces were repaired in the same pass: the `24` free-boson count is `\kappa_{\mathrm{fiber}}(K3)=24` or an OPE-level trace, not compact `\kappa_{\mathrm{ch}}`. | Guarded by `test_k3e_programme_separates_boundary_rank_from_chiral_kappa`. | closed: conflation | Vol III `chapters/examples/k3e_cy3_programme.tex`; `compute/tests/test_twisted_holography_k3e.py` |
| 140 | Table column header `\kappa_{\mathrm{ch}}` covers row with `\kappa_{\mathrm{BKM}}=5` | Closed 2026-06-09: the active Vol III table header is `\kappa_\bullet`, and the rows carry explicit invariant subscripts. | The guard checks the `Object / Symbol / \kappa_\bullet / Class` header directly. | Guarded by `test_k3e_programme_separates_boundary_rank_from_chiral_kappa`. | closed: label/content | Vol III `chapters/examples/k3e_cy3_programme.tex`; `compute/tests/test_twisted_holography_k3e.py` |
| 141 | "conditional on CY-A_3 for the existence of A_{K3 x E}" | Closed 2026-06-09: active Vol III text contains no `conditional on CY-A_3` existence claim in `k3e_cy3_programme.tex`; it cites `thm:derived-framing-obstruction` for the resolved obstruction lane. | Remaining conjectural content is not CY-A3 existence but the factorisation/shadow comparison structure. | Guarded by `test_k3e_programme_separates_boundary_rank_from_chiral_kappa`. | closed: temporal | Vol III `chapters/examples/k3e_cy3_programme.tex`; `compute/tests/test_twisted_holography_k3e.py` |
| 142 | Conjecture: `\kappa_{\mathrm{BKM}}=\kappa_{\mathrm{ch}}(S)+\kappa_{\mathrm{ch}}(S\times E)` universally | Closed 2026-06-09: active Vol III text treats the `5=3+2` equality as an `N=1` convention-mixing accident and states the universal Borcherds law `\kBKM(\Phi_N)=c_N(0)/2`. | The additive rule fails on the CHL ladder; the BKM invariant is the Borcherds weight, not a sum of chiral and fibre terms. | Guarded by `test_k3e_programme_separates_boundary_rank_from_chiral_kappa`. | closed: specific/general | Vol III `chapters/examples/k3e_cy3_programme.tex`; `compute/tests/test_twisted_holography_k3e.py` |
| 143 | Programme C asks whether additive BKM formula holds as open | Closed 2026-06-09: active Vol III text states the additive formula fails at every relevant `N` and records the universal law `\kBKM(\Phi_N)=c_N(0)/2`; the old open-question surface is absent. | For non-K3-fibered CY3s, `\kappa_{\mathrm{BKM}}` is not a defined replacement invariant. | Guarded by `test_k3e_programme_separates_boundary_rank_from_chiral_kappa`. | closed: temporal | Vol III `chapters/examples/k3e_cy3_programme.tex`; `compute/tests/test_twisted_holography_k3e.py` |
...
| # | Wrong Claim | Ghost Theorem | Precise Error | Correct Relationship | Type | Location |
|---|-------------|---------------|---------------|---------------------|------|----------|
| 144 | `\kappa_{\mathrm{ch}} = \operatorname{rank}(\Lambda)` for lattice VOA | Closed 2026-06-09: Vol III now writes this as the Heisenberg-route curvature `\kappa_{\mathrm{ch}}^{\mathrm{Heis}}(V_\Lambda)=\operatorname{rank}(\Lambda)`. | The sentence explicitly says this is not the compact CY Route A Hodge supertrace invariant. | Guarded by `test_toroidal_elliptic_lattice_kappa_uses_heisenberg_route`. | closed: Route A vs Route B vs fiber | Vol III `chapters/examples/toroidal_elliptic.tex`; `compute/tests/test_genus2_chiral_partition.py` |
| 145 | `\kappa_{\mathrm{ch}}(A_E)=24` "(rank of free-boson lattice)" | Closed 2026-06-09: active Vol III text states `c(A_E)=24` is the Mukai-lattice rank `\kappa_{\mathrm{fiber}}=24`, not a chiral invariant. | The same bridge paragraph separates `\kappa_{\mathrm{ch}}^{\mathrm{Hodge}}=0`, `\kappa_{\mathrm{ch}}^{\mathrm{Heis}}=3`, `\kappa_{\mathrm{BKM}}(\Delta_5)=5`, and `\kappa_{\mathrm{fiber}}=24`. | Guarded by `test_toroidal_elliptic_k3xe_bridge_separates_five_invariants`. | closed: Route A vs Route B vs fiber | Vol III `chapters/examples/toroidal_elliptic.tex`; `compute/tests/test_genus2_chiral_partition.py` |
| 146 | Two `\hbar` conventions without bridge (`\hbar_1,\hbar_2` vs plain `\hbar`) | Closed 2026-06-09: the DDCA--toroidal bridge now states `q=e^{\hbar_1}`, `t=e^{-\hbar_2}`, and the plain rational parameter is `\hbar=\hbar_1` after the `\hbar_2\to0` degeneration. | This connects the two-parameter toroidal convention to the rational table convention at the comparison point. | Guarded by `test_toroidal_elliptic_records_hbar_bridge`. | closed: convention bridge | Vol III `chapters/examples/toroidal_elliptic.tex`; `compute/tests/test_genus2_chiral_partition.py` |
| 147 | `\chi(K3)=24` bare without `\chi_{\mathrm{top}}` subscript | Closed 2026-06-09: active Vol III text no longer contains bare `\chi(K3)` in the toroidal elliptic chapter; the K3xE bridge uses the five named invariants instead. | The repair avoids confusion with `\chi(\mathcal O_{K3})=2` by not routing the bridge through a bare Euler-characteristic slogan. | Guarded by `test_toroidal_elliptic_k3xe_bridge_separates_five_invariants`. | closed: label/content | Vol III `chapters/examples/toroidal_elliptic.tex`; `compute/tests/test_genus2_chiral_partition.py` |
| 148 | `\vartheta_1` vs `\theta_1` notation inconsistency | Closed 2026-06-09: Vol III `toroidal_elliptic.tex` now writes the elliptic propagator and Fay trisecant identity with `\theta_1`, matching the rest of the chapter. | The notation switch was local to the elliptic shadow-data construction. | Guarded by `test_toroidal_elliptic_uses_theta_one_not_vartheta_one`, which rejects residual `\vartheta_1` in the chapter and checks the displayed elliptic kernel. | closed: convention | Vol III `chapters/examples/toroidal_elliptic.tex`; `compute/tests/test_genus2_chiral_partition.py` |
...
### Verified clean: ChirHoch cup-product freeness not asserted (entry 122)

Updated 2026-06-09. The current theorem surfaces do not route a cup-product freeness claim through the old vanishing slogan. They state amplitude, Hochschild duality, degree-\(\leq 2\) Hilbert growth, and \(E_2\)-formality on the Koszul locus; any cup-product freeness claim requires a separate multiplicative computation. No unqualified ChirHoch freeness claim remains.
...
|---|-------------|---------|----------|
| 149 | "shadow Postnikov tower" terminology | Closed 2026-06-09: the legacy compound phrase was removed from `working_notes.tex` and reader-facing compute/docstring surfaces. The terminology now says "shadow obstruction tower" or "shadow tower" for the arity-filtered MC obstruction package. | Genuine homotopy-theoretic Postnikov discussions and stable labels were left untouched; the repair only removes the misleading compound phrase where it named the shadow tower itself. | working_notes.tex; higher_genus_modular_koszul.tex comment; modular_koszul_engine.py; e1_shadow_tower.py; shadow_tower_recursive.py; lattice_shadow_periods.py; quartic_residue_engine.py; virasoro_quintic_shadow.py; platonic_datum.py; wk_and_bracket_verifications.py |
...
| # | Wrong Claim | Ghost Theorem | Precise Error | Correct Relationship | Type | Location |
|---|-------------|---------------|---------------|---------------------|------|----------|
| 153 | "Arrow 3 is the Drinfeld center, the categorified analogue of the averaging map" | Z and av ARE related via factorization | Closed 2026-06-09: the preface Arrow~3 paragraph now identifies \(Z\colon E_1\text{-}\mathsf{Cat}\to E_2\text{-}\mathsf{Cat}\) as the right adjoint to the forgetful functor and says it constructs half-braidings. | Corrected surface: \(Z\) categorifies the algebraic centre, not the averaging map. The averaging map is the further lossy \(E_2\to E_\infty\) symmetrisation/scalar-shadow step; the centre constructs the braiding data that averaging destroys. | closed: Drinfeld-centre/right-adjoint vs averaging projection guarded | preface.tex Arrow 3 paragraph; en_koszul_duality.tex AP-CY54 item; test_theorem_preface_positioning_engine.py |

115 claims checked, 2 violations found and fixed. The Vol I preface is the canonical reference — 113 clean claims out of 115.

| 154 | "conditional on chain-level S³-framing for d=3" | CY-A_3 WAS conditional | Closed 2026-06-09: the preface now says the Volume III Calabi--Yau quantum-group construction is proved for \(d=2\) and \(d=3\), with the \(\infty\)-categorical resolution of the \(\mathbb S^3\)-framing obstruction constructing \(A_\mathcal C\) on the stated smooth proper CY\(_3\) framing surface. | Corrected surface: CY-A\(_3\) is not conditional on a chain-level \(S^3\)-framing theorem; the remaining open point is the explicit chain-level model for non-formal presentations beyond the stated \(\infty\)-categorical existence surface. | closed: CY-A3 proved / chain-model caveat guarded | preface.tex CY construction paragraph; test_s3_framing_obstruction.py |
| 155 | Σ F_g x^{2g} = k(x/2/sinh(x/2) - 1) | Â-genus DOES use sinh | Closed 2026-06-09: the Heisenberg frame already has the scalar FP series on the Wick-rotated branch \(k((x/2)/\sin(x/2)-1)\), with a local warning that the real \(\hat A\)-genus uses \(\sinh\) and alternates. Two stale compute docstrings/summary strings that put the positive scalar series on the real \(\sinh\) branch were repaired. | Corrected surface: \(\hat A(x)=(x/2)/\sinh(x/2)\) has coefficients \((-1)^g\lambda_g^{\mathrm{FP}}\); the scalar free-energy generating function uses \(\hat A(ix)-1=(x/2)/\sin(x/2)-1\), so every \(\lambda_g^{\mathrm{FP}}\) coefficient is positive and equals the absolute-value Bernoulli expression. | closed: FP sine branch / real A-hat alternating branch guarded | heisenberg_frame.tex `eq:frame-generating-function`; heisenberg_bv_bar_proof.py `ahat_generating_function_check`; bc_genus34_shadow_graph_engine.py `modular_form_identification`, `quasi_modular_structure`; test_bc_genus34_shadow_graph_engine.py |
...
|---|-------------|---------------|---------------|---------------------|------|-----|
| 156 | "Spectral parameters from FM_k(C)" for End^ch_A | End^ch_A relates to FM via local-global identification | Closed 2026-06-09: `rem:endch-spectral-variable-layering` now states that \(\End^{\mathrm{ch}}_A\) is algebraically defined by multilinear maps valued in iterated formal Laurent series, while Fulton--MacPherson geometry enters only through comparison theorems. The Theorem A notion paragraph now routes spectral variables through formal-disk restriction of ordered Ran configurations. | Corrected surface: three layers are explicit: (1) geometric FM/log-form model on \(\overline C_{n+1}(X)\), (2) formal completion along the total diagonal with \(\lambda_i=z_i-z_{n+1}\), and (3) the algebraic formal-Laurent operad. The comparison is `prop:geometric-algebraic-hochschild` plus `prop:bd-algebraic-bridge`, not the definition of \(\End^{\mathrm{ch}}_A\). | closed: End^ch algebraic/formal-disk/FM layering guarded | chiral_center_theorem.tex `rem:endch-spectral-variable-layering`; theorem_A_infinity_2.tex Notion B paragraph; test_chiral_hochschild_engine.py |
| 157 | "ChirHoch* and THH* are different-sized objects (dim 3 vs infinite)" | ChirHoch and THH compute different invariants | Closed 2026-06-09: the three-Hochschild agreement remark now states explicitly that the contrast is structural, not a size theorem about topological Hochschild homology. A stale chapter sentence saying generic \(\ChirHoch^*(\cA)\) and \(\HH^*(A_{\mathrm{mode}})\) have the same cohomological dimension was repaired. The Heisenberg table witnesses the finite low-degree difference \(\ChirHoch^1(\cH_k)=\mathbb C\) versus \(\HH^1(A_1)=0\), while the spectrum-level THH comparison remains behind the named spectrum-lift package. | Corrected surface: the genuine high-degree divergence target in this chapter is scalar Gel'fand--Fuchs cohomology, not THH. Chiral, mode-algebra, categorical, and topological Hochschild theories have different inputs and operadic structures; dimension slogans are rejected. | closed: structural-not-dimensional Hochschild firewall guarded | three_hochschild_unification_platonic.tex `rem:three-hochschild-agreement-precision`, `prop:three-hochschild-high-degree-divergence`; chiral_hochschild_koszul.tex structural-vs-dimensional paragraph; hochschild_conventions.tex `rem:hochschild-crosswalk-spectrum-lift-package`; test_three_hochschild_unification.py |
| 158 | "BZFN gives different answers depending on ambient category S as tunable parameter" | Two derived centers DO exist and produce different braided categories | Closed 2026-06-09: the Hochschild conventions appendix now names the BZFN ambient discipline. In a single BZFN application the symmetric monoidal target \(\mathcal S\) is fixed, and the \(E_1\)-algebra object plus its Hochschild trace are formed inside that same \(\mathcal S\). | Corrected surface: \(Z^{\mathrm{der}}_{\mathrm{ch}}(\cA)\) and \(\HH^\bullet(A_{\mathrm{mode}},A_{\mathrm{mode}})\) differ because the input algebras and native presentations differ: a chiral factorisation algebra in \(\cD\)-modules on \(\Ran(X)\) versus a formal-disk mode algebra in chain complexes. Comparing them requires the named formal-disk mode-extraction and local-global maps; BZFN alone does not identify the centres. | closed: fixed ambient / different input algebra guarded | hochschild_conventions.tex `rem:bzfn-ambient-not-dial`; three_hochschild_unification_platonic.tex; test_three_hochschild_unification.py |
| 159 | "Topological Drinfeld center has no spectral parameters" | Chiral structure creates translation automorphism enabling evaluation modules | Closed 2026-06-09: the active W-infinity convention now separates the associative Yangian evaluation-module parameter from the chiral Ran collision coordinate. The constant Drinfeld--Jimbo braid-form \(R\)-matrix on fixed tensor factors has no additive evaluation parameter, but the Yangian evaluation modules \(V(u),W(v)\) already carry \(R(u-v)\) in representation theory. | Corrected surface: spectral parameters are not created by the center construction. The primitive topological bar deconcatenation coproduct is \(z\)-independent before evaluation shifts; the chiral bar differential is meromorphic in the Ran collision coordinate. | closed: Yangian evaluation parameter/topological bar/chiral differential firewall guarded | ordered_associative_chiral_kd.tex `Spectral-parameter firewall`; quantum_group_bar_engine.py; integrable_lattice_shadow_engine.py; test_quantum_group_bar_engine.py |
| 160 | BD chiral operad = algebraic End^ch_A (used interchangeably) | They are related by formal disk restriction (isomorphism after coordinate choice) | Closed 2026-06-09: the four-step bridge is now the named Proposition `prop:bd-algebraic-bridge`, with formal-disk restriction, D-module trivialisation, spectral-variable identification, and operadic composition. The preface no longer treats the formal-disk algebraic model as the global BD D-module operad by notation. | Corrected surface: after choosing a formal coordinate and horizontal fibre \(A_p\), the BD chiral endomorphism operad restricted to \(\widehat D_p\) is identified with the algebraic formal-Laurent operad \(\End^{ch}_{A_p}\) by a coordinate-dependent non-\(\Sigma\) operad isomorphism; \(\operatorname{Aut}(\mathcal O)\)-equivariance supplies descent data. | closed: BD/algebraic bridge named and guarded | thqg_open_closed_realization.tex `prop:bd-algebraic-bridge`; preface.tex formal-disk paragraph; test_theorem_open_closed_rectification_engine.py |
| 161 | Geometric ChirHoch = algebraic ChirHoch (used interchangeably at chain level) | They ARE quasi-isomorphic for logarithmic chiral algebras | Closed 2026-06-09: the comparison is now a named proposition, `prop:geometric-algebraic-hochschild`, with logarithmic/completed finite-piece, holonomic coefficient, cofibrant finite-window, and Mittag--Leffler hypotheses. The downstream Gerstenhaber proof no longer cites `rem:comparison-geometric-hoch` as load-bearing authority. | Corrected surface: model switches cite the named geometric--algebraic comparison proposition and its hypotheses. The proposition explicitly parallels `thm:geometric-equals-operadic-bar` and records that at genus \(\ge 1\) the geometric model carries curve-dependent data invisible to the algebraic model. | closed: named model-switch proposition guarded | chiral_center_theorem.tex `prop:geometric-algebraic-hochschild`; chiral_hochschild_koszul.tex Gerstenhaber proof; test_chiral_hochschild_engine.py |
| 162 | "Theorem H fails for THH / concentration has no THH analogue" | Theorem H IS specifically about ChirHoch* | Closed 2026-06-09: the active surfaces already separate the three theories. `rem:theorem-h-chiral-only` says Theorem H is exclusively about \(\ChirHoch^\bullet(\mathcal A)\) on a curve and not about \(\THH\) or categorical Hochschild cohomology. The three-Hochschild chapter isolates GF unboundedness as continuous Lie algebra cohomology of the positive-mode algebra. | Corrected surface: Theorem H gives the chiral amplitude bound; THH is the \(S^1\)-equivariant trace of an \(E_1\)-ring spectrum; mode Hochschild and GF are separate comparison targets. The genuine high-degree divergence in the active chapter is scalar GF, not THH. | closed: chiral/THH/GF firewall guarded | hochschild_conventions.tex `rem:theorem-h-chiral-only`; three_hochschild_unification_platonic.tex `rem:GF-unbounded-but-image-bounded`, `rem:why-theorem-H-not-GF`; preface.tex; test_three_hochschild_unification.py |
| 163 | "Physics requires two different bulk theories" | Physics has ONE bulk per boundary (uniqueness of derived center) | Closed 2026-06-09: the canonical open/closed section already separates \(\operatorname{Obs}^{\mathrm{bulk}}(T)\) from the cochain derived centre \(Z^{\mathrm{der}}_{\mathrm{ch}}(\mathcal A)\). The remaining stale introduction/preface wording saying `conj:drinfeld-center-equals-bulk` was "proved for Heisenberg" and other families was repaired. | Corrected surface: finite-window evidence and conditional centre-comparison checks exist for the standard families, but the physical-bulk equivalence remains the OCA/topologization conjecture. Chiral Hochschild and Drinfeld-centre routes are computational centres for one boundary bulk target, not different physical bulk theories. | closed: one bulk target; computational centres guarded | thqg_open_closed_realization.tex `prop:thqg-universal-action-not-reconstruction`; introduction.tex; preface.tex; theorem_open_closed_rectification_engine.py; test_theorem_open_closed_rectification_engine.py |
| 164 | "Restricting chiral algebra to S^1 gives A_inf algebra" | E_2 restricts to E_1 on real submanifolds (Costello-Gwilliam) | Closed 2026-06-09: the Hochschild convention appendix now has `rem:hochschild-circle-restriction-firewall`, separating four operations: no bare D-module restriction to a real circle; formal-disk mode extraction gives the strict associative mode algebra; circle factorization homology of a locally constant \(E_1\)-model gives Hochschild chains; real-ray pullback gives an \(E_1\)-algebra only after analytic/constructible hypotheses. | Corrected surface: \(A_\infty\) language enters only after choosing a chain model and characteristic-zero homotopy transfer/rectification. The annulus and \(E_1\)-modular surfaces now write \(\int_{S^1}\mathcal F_A\) for the locally constant interval model, not \(\int_{S^1}\mathcal A\) for a chiral \(\cD\)-module on a curve. | closed: circle/mode/trace/real-ray firewall guarded | hochschild_conventions.tex `rem:hochschild-circle-restriction-firewall`; hochschild_cohomology.tex; e1_modular_koszul.tex; cy_factorization_homology_k3e_engine.py; test_three_hochschild_unification.py |
| 165 | Tamarkin inconsistency: C*(H_k, H_k) = k[[kappa]] vs Theorem H dim 3 | Both computations correct | Closed 2026-06-09: the active Heisenberg Hochschild surface now has `rem:heisenberg-family-vs-fixed-fiber`, which says the displayed \((1,1,1)\), total \(3\), is the cohomology of the fixed generic fibre \(\mathfrak H_k\). The completed base \(\mathbb C[[\kappa]]\) or \(\mathbb C[[k]]\) records motion of the one-parameter Heisenberg family, not the fixed-fibre cohomology vector space. | Corrected surface: the fixed-level degree-\(1\) class is the shift derivation \(D(\alpha)=1\). The level motion belongs to the family base and, at \(k\neq0\), is an exact/gauge degree-\(2\) product lane rather than the fixed-fibre \(\ChirHoch^1\) witness. The fixed-level Poincare polynomial remains \(1+t+t^2\). Compute data now marks `cohomology_scope = fixed generic fiber` and records that formal parameter rings are deformation bases, not fixed-fibre ChirHoch spaces. | closed: family/fibre distinction guarded; H-6 degree convention repaired | chiral_hochschild_koszul.tex `rem:heisenberg-family-vs-fixed-fiber`; hochschild_cohomology.tex Heisenberg table; chirhoch_dimension_engine.py; test_chirhoch_dimension_engine.py |

## XXX. Vol I Frontier Chapters Adversarial Review (2026-04-16)

| # | Wrong Claim | Ghost Theorem | Precise Error | Correct Relationship | Type | Location |
|---|-------------|---------------|---------------|---------------------|------|----------|
| 166 | "Holographic codes from Koszul duality" (chapter title + Thm G12 "Koszulness <=> exact holographic reconstruction") | Bar-cobar counit quasi-iso on Koszul locus IS a reconstruction theorem | Closed 2026-06-09: the chapter title is now `Algebraic code skeletons from Koszul duality`. The active G12 theorem states three separate surfaces: (i) Koszulness iff the bar-cobar counit is a quasi-isomorphism for the level-1 chart algebra, (ii) derived-centre transport at level 3, and (iii) physical holographic reconstruction only after a chosen BRST/derived-centre comparison \(\beta_T\) is proved a quasi-isomorphism in the analytic completion. | Corrected surface: HaPPY/ADH-style physical QEC, physical tensor networks, and bulk subregion reconstruction are not consequences of bar-cobar inversion alone. Compute layers now call the objects algebraic code skeletons / bar-graph shadow networks, expose `physical_* = False` status fields, and retain HaPPY only as a non-equivalence comparison. | closed: physical QEC externalized; algebraic reconstruction typed | holographic_codes_koszul.tex title and `thm:hc-koszulness-exact-qec`; qec_koszul_code_engine.py; qec_shadow_code_deep_engine.py; focused QEC guards |
| 167 | "Code distance d=2 universal" (Rem hc-universal-parameters (ii)) | Bar degree shift gives natural 2-grading | Closed 2026-06-09: the active manuscript remark already says the shadow tower begins at degree 2 with scalar \(\kappa(\cA)\) and that this is not a Hilbert-space code-distance formula. The compute surfaces now call the universal `2` an arity proxy / first essential shadow arity, add `physical_distance_status`, and keep historical fields such as `d_h`, `D`, and `distance` only with explicit `*_kind` qualifiers. | Corrected surface: operational QEC distance is not determined by bar degree or shadow depth; it requires a physical inner product, an error algebra/error basis, and recovery maps satisfying the Knill--Laflamme framework. The universal internal invariant is only \(d_{\mathrm{arity}}=2\). | closed: arity proxy separated from QEC distance and guarded | holographic_codes_koszul.tex `rem:hc-universal-parameters`; qec_koszul_code_engine.py; qec_shadow_code_deep_engine.py; theorem_entanglement_page_curve_engine.py; focused QEC tests |
| 168 | "Entanglement modular Koszul" title conflates modular operator (Tomita-Takesaki J) with moduli-space modular structure (M_g, SL(2,Z)) | Both notions of "modular" have legitimate roles in this framework | Closed 2026-06-09: the entanglement chapter now opens with a terminology firewall. In the title and chapter, `modular` in modular Koszul duality means the Hodge/moduli and modular-characteristic package: the \(\lambda_g^{\mathrm{FP}}\) lane, \(\mathrm{SL}_2(\mathbb Z)\)/mapping-class constraints, and \(\kappa(\cA)\). It explicitly does not assert a Tomita--Takesaki modular operator. | Corrected surface: Tomita--Takesaki modular flow and Tomita conjugation appear only through `conj:thqg-tomita-from-verdier` and `conj:thqg-modular-flow`, requiring a unitary sewing envelope, faithful normal state, anti-linearity, and physical Hilbert-space inner product. Concordance G13 no longer says Verdier is proved to be Tomita \(J\); it records only algebraic Verdier/Koszul exchange plus conjectural Tomita-type comparison. | closed: modular-polysemy separated and guarded | entanglement_modular_koszul.tex opening terminology paragraph; thqg_entanglement_theory.tex `conj:thqg-tomita-from-verdier`, `conj:thqg-modular-flow`; concordance.tex G13; compute/tests/test_modular_entanglement_flow_engine.py |
| 169 | "QES = shadow connection Ward identity" (conj:ent-qes) | Shadow connection flatness IS proved; stationarity of entropy functional IS a Ward-identity-style condition | Closed 2026-06-09: active `conj:ent-qes` is conjectural and requires a holographic dictionary: AdS3/CFT2 Brown-Henneaux/RT normalization, FLM/replica functional, and variation map compatible with the shadow connection. The compute layer now calls the scalar calculation a complementarity proxy, not physical QES stationarity, and returns `physical_qes_status`. | Corrected surface: the proved algebraic content is shadow-connection flatness plus the shadow tower. The RT/QES area term appears only after a gravitational dual and Brown-Henneaux dictionary identify scalar entanglement with area. | closed: QES/RT bridge externalized and guarded | entanglement_modular_koszul.tex `conj:ent-qes`; modular_entanglement_flow_engine.py; compute/tests/test_modular_entanglement_flow_engine.py |
| 170 | "Platonic" (chapter title and Def:thqg-completed-platonic-datum) | The "platonic" label names a maximally-structured datum subsuming six-fold -> eight-fold Koszul data | Closed 2026-06-09: the canonical definition is `Completed modular Koszul datum` in `thqg_open_closed_realization.tex`, and it now has an explicit terminology remark saying legacy "platonic" labels mean the completed modular Koszul package, not Platonic-solids/ADE/McKay data. Visible frontier/concordance headings were renamed to completed-datum/resonance-completion language; legacy labels remain only for cross-reference stability. | Corrected surface: the completed datum is the canonical open-sector/bar-Koszul/derived-centre/trace/resonance package. ADE/Kummer/McKay material is a separate specialization only when a theorem introduces those data with its own hypotheses. | closed: terminology ambiguity flagged and guarded | chapters/connections/thqg_open_closed_realization.tex L2143; chapters/connections/frontier_modular_holography_platonic.tex; chapters/connections/concordance.tex; compute/tests/test_theorem_open_closed_rectification_engine.py |
| 171 | "Semistrict for W_3" (cor:semistrictity-classical-W3-chapter) | Finite-degree theorem for polynomial PVA is correct: nonlin degree d => ell_n=0 for n>d+1 | Closed 2026-06-09: the active corollary already states semistrictity only for the formal HT envelope \(\HS(W_3^{\mathrm{cl}})\), not for the modular shadow tower or quantum \(\mathcal W_3\). The remaining ambiguous package titles were tightened to "classical" in `semistrict_modular_higher_spin_w3.tex` and `w_algebras.tex`, and the test surface now rejects the old non-classical titles. | Corrected surface: classical \(W_3\) PVA is quadratic-nonlinear, so its BV/HT envelope is semistrict with \(\ell_n=0\) for \(n\ge4\). Quantum \(\mathcal W_3\) and all-loop HT require the Yamada/KZ endpoint hypotheses and are not proved semistrict here. | closed: classical/quantum scope synchronized | chapters/connections/semistrict_modular_higher_spin_w3.tex L178; chapters/examples/w_algebras.tex L6484; compute/tests/test_w_finite_arity.py |
| 172 | "Hook-type corridor" (thm:hook-transport-corridor) | Inverse Hamiltonian reduction for type A at generic level exists (Fehily, CFLN, Butson-Nair) | Closed 2026-06-09: the active theorem remains \ClaimStatusConditional and assumes bar-cobar/Koszul duality intertwines with reduction functors. Visible summaries in `subregular_hook_frontier.tex`, `concordance.tex`, `outlook.tex`, `w_algebras.tex`, `higher_genus_modular_koszul.tex`, and `frontier_modular_holography_platonic.tex` now distinguish proved reduction/transport-closure data from conditional DS--bar/Koszul-duality transport; the stale undefined `thm:hook-type-transport-koszul-duality` reference was routed to `thm:hook-transport-corridor`. | Corrected surface: principal W-algebras completed Koszul (thm:pbw-slodowy-collapse, cor:principal-w-completed-koszul) are proved; Bershadsky-Polyakov strictness, \(W_4^{(2)}\) canonical degree, and unbounded canonical degree are proved local facts; hook-wide non-principal Koszul duality remains conditional on DS--bar compatibility. | closed: conditional-proof scope synchronized | chapters/connections/subregular_hook_frontier.tex L247; duplicated summaries guarded by compute/tests/test_hook_transport_corridor.py |
| 173 | Outlook "Koszul swampland" (conj:koszul-swampland) | Bar-cobar inversion fails off the Koszul locus; conductor K = kappa + kappa' is level-independent | Closed 2026-06-09: the active conjecture is titled "Strict Koszul boundary" and now states only the ordinary-chain Koszul complement detected by persistent off-diagonal bar-cobar spectral-sequence classes. The visible concordance row now calls \(K_\kappa\) a scalar conductor on verified Verdier surfaces and explicitly not a quantum-gravity distance criterion. The old label is retained only as a cross-reference mnemonic. | Corrected surface: Koszul/non-Koszul is a mathematical dichotomy on chiral algebras. Its identification with any string-quantum-gravity swampland remains only an external interpretation and is not asserted in the active theorem/concordance wording. \(K_\kappa\) is an invariant/conductor, not a proved distance function. | closed: branding-as-theorem removed from active surface | outlook.tex L443-530; concordance.tex open-problems table |
...
| # | Wrong Claim | Ghost Theorem | Precise Error | Correct Relationship | Type | Location |
|---|-------------|---------------|---------------|---------------------|------|----------|
| 174 | Thm D Step 1: `[B^(g)_scalar(A)]^vir = kappa(A)·[E]` in K^0(M̄_g)_Q | Fiberwise curvature d_fib^2 = kappa·omega_g^Ar IS proved | RESOLVED: `prop:scalar-obstruction-hodge-euler` now inscribes the missing K-theory globalization. The scalar bar family is filtered so the fiber associated graded is \(\bigoplus_p \Lambda^p H^0(\Sigma_g,\omega_{\Sigma_g})\otimes\varepsilon_p\); globally this yields the virtual class \([\barB^{(g)}_{\mathrm{scalar}}(\cA)]^{\mathrm{vir}}=\kappa(\cA)\lambda_{-1}(\mathbb E)\). | The degree-\(2g\) Chern-character component is computed by the splitting principle: \(\mathrm{ch}_g(\lambda_{-1}(\mathbb E))=(-1)^g\lambda_g\), with the bar-suspension sign absorbed. The concordance now explicitly says top-Chern equality does not determine a K-class and that \(\mathcal D_\cA^{(g)}\) denotes this scalar virtual class/projection, not \(\kappa(\cA)\mathbb E\). | closed 2026-06-09 | `chapters/theory/higher_genus_foundations.tex`; `chapters/connections/concordance.tex` |
| 175 | "Arakelov-Faltings theorem (Fal84)" cited for Θ_E|_fiber = ω_g^Ar | Identification follows from Arakelov metric definition + BGS anomaly formula | RESOLVED: the manuscript no longer cites an “Arakelov--Faltings theorem” for the fiberwise curvature identification. The relevant surfaces now say the Arakelov metric construction plus the standard period-matrix curvature description identify the zero-mode sector, with Faltings retained only as a Hodge-metric extension/source, not as the theorem proving the curvature equality. | The determinant-of-cohomology bridge is now attributed separately to the Quillen metric and Bismut--Gillet--Soulé anomaly formula. The PTVV/clutching remarks were synced to say “Arakelov-metric/BGS” and “period-matrix Hodge-curvature identification,” not “Faltings identification.” | closed 2026-06-09 | `chapters/theory/higher_genus_foundations.tex` |
| 176 | BGS c_1(λ, h_Q) output = c_g(E) Chern-Weil input | Chern-Weil of rank-g bundle gives c_g(E); BGS gives c_1(det E, h_Q) | RESOLVED: the proof now separates the rank-one determinant-line output from the rank-\(g\) Hodge-bundle top class. BGS supplies the determinant-of-cohomology fiber integral and Quillen Chern--Weil representative of \(c_1(\lambda)\) on the base. | The top class \(c_g(\mathbb E)=\lambda_g\) is obtained only after applying Chern--Weil to the full curvature tensor \(\Theta_{\mathbb E}\) and, equivalently, the splitting principle for the Chern roots of \(\mathbb E\). The later GRR bridge remark already records the same rank-one/rank-\(g\) separation. | closed 2026-06-09 | `chapters/theory/higher_genus_foundations.tex` |
| 177 | Thm B (1) at g=0: "If A is Koszul, then psi_0 is qi" | FTM-equivalence package: TFAE (Koszul morphism, counit qi, unit weak eq, twisted tensor acyclic) | RESOLVED: Theorem B clause (1) in `bar_cobar_adjunction_inversion.tex` is now the strict genus-\(0\) FTM surface. For the twisting datum \((\mathcal A,C_{\mathcal A,0},\tau_{\mathcal A,0},F_\bullet)\), it states the equivalence of: \(\tau_{\mathcal A,0}\) chiral Koszul morphism, \(\psi_0\) quasi-isomorphism, \(\eta_0\) weak equivalence, and acyclicity of \(K^L_{\tau_{\mathcal A,0}}\), \(K^R_{\tau_{\mathcal A,0}}\). | The proof now applies `thm:fundamental-twisting-morphisms` to identify these four faces and explicitly says the counit qi is not proved by reusing `def:koszul-chiral-algebra`. Nearby remarks now describe the Koszul locus as one face of the FTM package. | closed 2026-06-09 | `chapters/theory/bar_cobar_adjunction_inversion.tex` |
| 178 | Thm B (2) "independent" coderived proof via Positselski 7.2 | A chiral CDG-coalgebra Positselski 7.2 theorem exists | RESOLVED: `coderived_models.tex` now isolates the local theorem `thm:chiral-bar-cobar-positselski-7-2` with status `\ClaimStatusConditional` and explicit hypothesis package \(\mathsf{Pos}^{\mathrm{ch}}_{7.2}(C_S)\). The theorem states the stratumwise counit \(\Omega^{\mathrm{ch}}(C_S)\to\tau_{\ge0}C_S\) is an isomorphism in the chiral coderived category when the de Rham realization, second-kind acyclicity preservation, and chiral counit-identification hypotheses hold. | `thm:off-koszul-ran-inversion` Step 1 now applies this named theorem to each \(C_n\), then uses factorization compatibility and stratified conservativity. The concordance summary now cites the named chiral theorem and records Positselski 2011 Thm 7.2 only as its ordinary finite-type input, not as a direct proof of the chiral statement. | closed 2026-06-09 | `chapters/theory/coderived_models.tex`; `chapters/connections/concordance.tex` |
| 179 | Thm A alt proof H01 = "Lurie infinity-categorical nerve-realization" | Lurie HA Ch.5 nerve-realization IS a real structural framework | RESOLVED: the live surface was the unlabelled remark now at `chiral_koszul_pairs.tex:5505`, not the stale old line range. Current `CLAUDE.md` no longer contains the advertised H01 independent-alt-proof table entry. | The remark has been retitled as an \(\infty\)-categorical presentation of the same bar--cobar argument. It now explicitly says the route is independent only of the filtered-comparison lemmas, not a hypothesis-independent alternative proof of Theorem A: it still uses Koszul-effectivity for the Cech/co-Cech resolutions and the Verdier-compatible chiral Koszul-pair package. | closed 2026-06-09 | `chapters/theory/chiral_koszul_pairs.tex` |
| 180 | Thm H test `hochschild_poincare('virasoro') == [1,0,1]` verifies Thm H for Virasoro | Genuine verification requires an independently-computed Poincaré polynomial | RESOLVED: `test_theorem_h_hochschild_polynomial.py` now has `test_virasoro_poincare_independent_primary_source_triple` decorated with `@independent_verification` for `thm:hochschild-polynomial-growth`. | The expected Virasoro triple `[1,0,1]` is computed by a local witness labelled Wang 1998 / Feigin--Fuchs generic Virasoro cohomology and central-extension cocycle data, while the derived sources are `FAMILY_DATA['virasoro']` and the Theorem H bounded-amplitude formula. The test compares the engine against that disjoint witness and checks the higher tail vanishes without reading `FAMILY_DATA`. | closed 2026-06-09 | `compute/tests/test_theorem_h_hochschild_polynomial.py` |
| 181 | Thm H Step 3 of prop:fm-tower-collapse: H^{>0}(FM_m(C)) is d_1-exact by a direct Arnold/Shelton--Yuzvinsky contraction, independent of bar concentration | The FM tower collapses on the symmetric Koszul locus by consuming localized bar concentration | REPAIRED 2026-06-16: `prop:fm-tower-collapse` now routes the fibrewise killing of positive Arnold classes through the residue-twisted tensor complex at each collision stratum. Shelton--Yuzvinsky/Priddy supplies Koszul-complex acyclicity for the fibre algebra only; it does not contract `OS(A_{m-1})^{>=1}`. | The logical chain is sequential on the symmetric surface: PBW criterion ⇒ bar concentration (`thm:bar-concentration`) ⇒ localized residue-twisted acyclicity ⇒ FM-tower collapse. Before averaging, the ordered analogue is `conj:ordered-twisted-tensor-acyclicity`, not a proved direct homotopy. | repaired 2026-06-16 | `chapters/theory/chiral_hochschild_koszul.tex` |
| 182 | Thm C lem:center-isomorphism: Koszul equivalence of modules ⇒ Z(A) ≅ Z(A^!) | Z^der_ch(A) ≅ Z^der_ch(A^!) as brace dg algebras (hence E_2 after Deligne-Tamarkin) | RESOLVED: `lem:center-isomorphism` now states that the functorial object is \(Z^{\mathrm{der}}_{\mathrm{ch}}(\cA)=\ChirHoch^\bullet(\cA,\cA)\) and that the comparison is a brace dg-algebra quasi-isomorphism \(\iota_Z^{\mathrm{der}}\), as in `prop:derived-center-morita-criterion`. | The local-system map used in Theorem C is now explicitly the \(H^0\)-shadow \(\iota_Z=H^0(\iota_Z^{\mathrm{der}})\) after the ordinary flat fiber-center identification. The proof rejects module-Koszul endomorphism algebras as a raw naive-center isomorphism; the Kodaira--Spencer and Verdier-pairing references now call \(\iota_Z\) the \(H^0\) center comparison. | closed 2026-06-09 | `chapters/theory/higher_genus_complementarity.tex`; `chapters/theory/theorem_C_refinements_platonic.tex` |
| 183 | Thm C C0 "unconditional in coderived form" | C0 gives existence of a D^co class unconditionally; identifications on flat representative | RESOLVED: C0(i) now states that the curved fiber model determines a class \([\barB^{(g)}_{\mathrm{curv}}(\cA)]\in D^{\mathrm{co}}\), unique up to coacyclic twist, and explicitly says this does not identify ordinary cohomology or the center local system. | The cohomological concentration and \(H^0\cong\mathcal Z_{\cA}\) identification remain only in the flat perfect ordinary-derived clause. The theorem opening, proof, concordance rows, and C0 refinement corollary title now attach "unconditional" only to the \(D^{\mathrm{co}}\)-class existence surface. | closed 2026-06-09 | `chapters/theory/higher_genus_complementarity.tex`; `chapters/connections/concordance.tex`; `chapters/theory/theorem_C_refinements_platonic.tex` |
| 184 | Thm C C1: Verdier duality gives Q_g(A) ≅ Q_g(A^!)^v[-(3g-3)] | On perfect complexes, Verdier is an anti-equivalence and double dual is reflexive | RESOLVED: Theorem C now splits C1 into \(\mathrm{C}_{1a}\), the homotopy eigenspace decomposition from the represented strict-flat Verdier involution and `lem:involution-splitting`, and \(\mathrm{C}_{1b}\), the perfect Verdier-duality/Lagrangian clause for \(g\ge1\). | The \(\mathrm{C}_{1b}\) clause explicitly assumes perfectness/reflexivity of \(\mathbf C_g(\cA)\) and its direct-summand eigenspaces, plus the nondegenerate anti-invariant Verdier pairing. `prop:lagrangian-eigenspaces`, `prop:ptvv-lagrangian`, the Theorem C refinements, and the concordance status rows now inherit the same perfectness hypothesis. | closed 2026-06-09 | `chapters/theory/higher_genus_complementarity.tex`; `chapters/theory/theorem_C_refinements_platonic.tex`; `chapters/connections/concordance.tex` |
| 185 | Thm C at g=0: Q_0(A^!)=0; Verdier pairing of degree -(3g-3) | For g≥1, the -(3g-3) pairing is honest; for g=0 the unmarked ambient convention is trivial | RESOLVED: Theorem C summaries and corollaries now say that the shifted Verdier/Lagrangian pairing clause is not invoked at \(g=0\). The unmarked convention has \(Q_0(\cA)=H^0(\overline{\mathcal M}_0,\mathcal Z(\cA))\), \(Q_0(\cA^!)=0\), with the point class fixed by \(\sigma\). | The nontrivial genus-zero surface is now named separately as the Beilinson--Drinfeld pointed theory on \(\overline{\mathcal M}_{0,n}\), \(n\ge3\), governed by the Arnold-algebra pairing. `prop:lagrangian-eigenspaces` is restricted to \(g\ge1\); fixed-genus PTVV references are aligned with `prop:ptvv-lagrangian` at \(g\ge2\). | closed 2026-06-09 | `chapters/theory/higher_genus_complementarity.tex` |
| 186 | CLAUDE.md "C2 conditional on uniform-weight" | C2 as written in the .tex is conditional on the named C2 package, not a derivative metadata slogan | RESOLVED/STale: current `CLAUDE.md` no longer contains a C2 status row importing a bare uniform-weight hypothesis. The active theorem surface is `def:C2-shifted-symplectic-hypothesis-package` plus `thm:shifted-symplectic-complementarity`: the definition lists ten inputs, including uniform-weight perfectness and the conditional BV package (`thm:config-space-bv`, `thm:bv-functor`), and the theorem assumes that full package. | The scalar \(\kappa+\kappa'=K\) identity remains outside C2: it follows from C1 plus Theorem D, as recorded in `rem:theorem-C-decomposition`. No theorem edit needed in this pass; the cache row was stale against the current tree. | closed 2026-06-09 | `CLAUDE.md`; `chapters/theory/higher_genus_complementarity.tex`; `metadata/claims.jsonl` |
| 187 | Topologization proof: Dunn additivity on E_2-hol × E_1-top of SC^{ch,top} yields E_3-top | CG factorization algebra on R × C gives a single-colored bulk; Dunn applies there | RESOLVED: the topologization theorem now introduces the named package `\mathsf{BulkExtract}_{\mathrm{CG}}(\mathfrak g,k)` (`def:cg-bulk-extraction-package`) for the bridge from the two-coloured Swiss-cheese derived centre to the closed bulk observables of the single-coloured Costello--Gwilliam factorization algebra. | `thm:topologization`, `prop:e3-via-dunn`, and `prop:khan-zeng-topological` are conditional on that package. The proof text says Dunn applies only after this extraction, on \(\mathcal F^{\mathrm{CS}}_k\) over \(\mathbb R\times\mathbb C\), not on \(\mathsf{SC}^{\mathrm{ch,top}}\). The companion chain-level chapter also replaces the old "open E_1" Dunn phrase by the transverse single-coloured \(E_1^{\mathrm{top}}\) from the \(\mathbb R\)-direction. | closed 2026-06-09 | `chapters/theory/en_koszul_duality.tex`; `chapters/theory/topologization_chain_level_platonic.tex` |
| 188 | Topologization: "shadow-tower truncation at r_max=3 for class L implies no quartic/higher brace contributions" | Shadow invariants S_r are Σ-coinvariant scalar data at degree r | RESOLVED: `prop:sugawara-gauge-rectification` and `thm:chain-level-E3-top-class-L` are no longer proved full original-complex `E_3^{top}` statements. They prove the strict unary Sugawara primitive and the cubic Jacobi cancellation, then name the all-arity higher-brace coherence condition `\mathsf{HBC}_L(\mathfrak g,k)` for the original-complex upgrade. | Shadow depth \(r_{\max}=3\) is now used only for scalar shadow invariants \(S_r=0\) for \(r\ge4\), not for brace-arity vanishing. The BV/bar obstruction proposition likewise says class L kills the cubic harmonic term; quartic/higher ordinary chain-level vanishing is a separate harmonic higher-brace coherence input. | closed 2026-06-09 | `chapters/theory/en_koszul_duality.tex`; `chapters/theory/topologization_chain_level_platonic.tex`; `chapters/connections/bv_brst.tex` |
| 189 | Sugawara antighost: cohomological primitive versus chain-level \(\eta_1\) | Chain-level rectification requires explicit primitives \(\eta_1^{(\mathrm i)},\eta_1^{(\mathrm{ii})}\) | RESOLVED: the main chapter no longer derives the chain-level identity from the old "up to equations of motion" phrase. It imports the explicit primitives from `def:eta1`, `prop:eta-i-primitive`, `prop:eta-ii-primitive`, and `cor:eta-primitive`, and the gauge-rectification proof now cites `thm:sugawara-antighost-primitive-chain-level` for the strict identity \([Q,\widetilde G_1]=T_{\mathrm{Sug}}\). | The construction still records the cohomological operator identity for \(G_1\); the original-complex statement is carried by the corrected \(\widetilde G_1=G_1-\eta_1\) and the separate chain-level primitive theorem. | closed 2026-06-09 | `chapters/theory/en_koszul_duality.tex`; `chapters/theory/topologization_chain_level_platonic.tex` |
| 190 | `prop:harmonic-factorization`: sharp scalar factorization versus \(m_0\)-divisibility | \(m_0\)-divisibility is enough for the coderived comparison | RESOLVED: the broad BV proposition no longer asserts that every harmonic endomorphism is a scalar multiple of \(m_0^{\lfloor r/2\rfloor-1}\). It now states \(\delta_r^{\mathrm{harm}}=\epsilon_r(\cA)\circ m_0^{\lfloor r/2\rfloor-1}\) with \(\epsilon_r(\cA)\in\operatorname{End}(H_g)\); \(c_r(\cA)\) is only the scalar/vacuum-channel projection. | The proof explicitly says scalarity requires the separate completed/vacuum-proportionality package (`prop:central-m0-vacuum-proportionality`) and is not assumed in the broad curvature-divisibility proposition. The coderived theorem now uses only \(m_0\)-power torsion. | closed 2026-06-09 | `chapters/connections/bv_brst.tex`; `chapters/theory/mc5_class_m_chain_level_platonic.tex` |
| 191 | `prop:harmonic-factorization` duplicate/truncated proof audit | Current file has a single `prop:harmonic-factorization` surface | RESOLVED: the current manuscript has exactly one `\label{prop:harmonic-factorization}` at `bv_brst.tex`, the proposition is `\ClaimStatusConditional`, and the proof runs through the Step 5 class-by-class vanishing paragraph before `\end{proof}`. The old duplicate-label/truncated-proof state is no longer present. | Entry 190 separately records the weakening from sharp scalar factorization to \(m_0\)-divisibility. | closed 2026-06-09 | `chapters/connections/bv_brst.tex`; `metadata/claims.jsonl` |
| 192 | MC5 operadic route: classical SC Koszulity versus \(\mathsf{SC}^{\mathrm{ch,top}}\) package | Vol II `thm:homotopy-Koszul` proves homotopy-Koszulity of \(\mathsf{SC}^{\mathrm{ch,top}}\) by classical SC Koszulity + closed-colour formality + transfer | RESOLVED: `rem:bv-bar-coderived-operadic` no longer derives the coderived comparison from Livernet/classical SC Koszulity alone. It now states a conditional second proof route whose input is the full imported \(\mathsf{SC}^{\mathrm{ch,top}}\) homotopy-Koszulity package. | `rem:homotopy-koszulity-center` was tightened to name the Vol II theorem and to state explicitly that classical Swiss-cheese Koszulity alone is insufficient; the chiral comparison and transfer are part of the imported package. | closed 2026-06-09 | `chapters/connections/bv_brst.tex`; `chapters/theory/en_koszul_duality.tex`; Vol II `chapters/connections/line-operators.tex` |
| 193 | E_3 identification Step 2: order-by-order uniqueness via formal-disk \(P_3\) normalization | For simple g, \(H^3(g)=\mathbb C\) forces one-dimensional correction space at each order | RESOLVED: `thm:e3-identification` no longer treats the formal-disk binary bracket as an unexplained all-order calculation. The induction now uses three inputs separately: associated graded agreement, the formal-disk binary \(P_3\) normalization from `thm:chiral-e3-cfg(ii)`, and the filtration comparison from `thm:chiral-e3-cfg(iii)`. | At order \(p+1\), formality reduces the obstruction to the Cartan/Killing generator in the one-dimensional simple-\(g\) correction space; the formal-disk comparison computes that scalar, and one-dimensionality rules out any independent higher-order correction. | closed 2026-06-09 | `chapters/theory/en_koszul_duality.tex` |
| 194 | CLAUDE.md stale non-simple \(E_3\) status | `rem:e3-non-simple` plus `prop:e3-glN-two-parameter` separate reductive, gl_N, and solvable scopes | RESOLVED: current CLAUDE table no longer contains the stale blanket status row. The gl_N two-parameter comparison is now a proved proposition with the legacy label preserved; the standalone survey no longer makes a blanket unresolved-status claim for the non-simple identification. | Simple g remains under the fixed formality/formal-disk package of `thm:e3-identification`; gl_N is proved over `(B_tr,B_ab)` in `prop:e3-glN-two-parameter`; arbitrary reductive cases require componentwise matching in `Sym^2(g^*)^g`; elliptic and solvable extensions are still explicitly conditional/conjectural at their own surfaces. | closed 2026-06-09 | `chapters/theory/en_koszul_duality.tex`; `standalone/survey_modular_koszul_duality_v2.tex`; `chapters/examples/yangians_foundations.tex` |
| 195 | Koszul count mismatch between meta-theorem and atlas | Two different count surfaces are active: the twelve-item meta-theorem and the fourteen-chart associator atlas | Resolved 2026-06-09. Current `CLAUDE.md`, preface, concordance, `chiral_koszul_pairs.tex`, and the survey surfaces now use the twelve-item split for `thm:koszul-equivalences-meta`: seven independent bidirectional equivalences, BBL consequence, conditional FH comparison, one-way Hochschild consequence, shifted-symplectic-package conditional Lagrangian criterion, and one-directional D-module purity. | The standalone `koszulness_fourteen_characterizations.tex` and `koszulness_moduli_scheme.tex` retain the separate fourteen-chart atlas language: ten KZ-home charts plus \(U_{11}\)--\(U_{14}\), with logical-strength qualifications. These are chart coordinates on the associator torsor, not a replacement count for the twelve-item theorem. | resolved: twelve theorem vs fourteen atlas separated | chiral_koszul_pairs.tex; koszulness_moduli_scheme.tex; koszulness_fourteen_characterizations.tex; survey_modular_koszul_duality.tex; survey_modular_koszul_duality_v2.tex; survey_track_a_compressed.tex |
| 196 | Koszul Hochschild consequence advertised too strongly | (i) ⇒ Hochschild amplitude/duality/Hilbert growth is proved by Theorem H | Resolved 2026-06-09. The standalone theorem already places the Hochschild item in the one-way block. The active repair removed the remaining main-chapter and standalone-survey language that treated cup-product freeness as a formal-vanishing upgrade. | Current surfaces state only the proved one-way Theorem-H package: amplitude \([0,2]\), Hochschild duality, degree-\(\leq 2\) Hilbert growth, and \(E_2\)-formality on the Koszul locus. No converse to the bar-cobar counit and no blanket free-polynomial cup-product assertion is made. | resolved: Hochschild consequence one-way; no freeness upgrade | chiral_koszul_pairs.tex; koszulness_fourteen_characterizations.tex; survey_modular_koszul_duality_v2.tex; programme_summary.tex; N1_koszul_meta.tex |
| 197 | Koszul (vii) PBW universality advertised too strongly | The equivalence holds on the PBW-free / freely strongly generated chart | Resolved 2026-06-09. The standalone theorem no longer places (vii) in the bidirectional-equivalence block: it is a one-way PBW-free criterion, and the graph/proof now prove only (vii)⇒(iii). The main atlas also marks \(U_7\) as a sufficient home-chart test. | Free strong generation remains a sufficient input, not a consequence of Koszulness. Simple quotients, logarithmic triplet algebras, and symplectic-fermion orbifolds require separate bar, determinant, or orbifold arguments; the parent symplectic fermion algebra is not used as an open counterexample here. | resolved: PBW-free chart demoted to one-way/sufficient | koszulness_fourteen_characterizations.tex; koszulness_moduli_scheme.tex `v1-rem:kms-pbw-universality-scope` |
| 198 | CLAUDE.md / opener: Chiral QG equivalence "verified concretely for sl_2 Yangian + affine KM" | Concrete theorem surfaces are W_{1+∞}[Ψ] and W_N[Ψ], not a separate sl_2 Yangian / classical affine-KM verification | Resolved 2026-06-08. Current `CLAUDE.md` no longer contains the stale `sl_2 Yangian + affine KM` advert. The active stale surface was the opener of `ordered_associative_chiral_kd.tex`, which said the equivalence was witnessed for the \(\mathfrak{sl}_2\) Yangian in Drinfeld second presentation. | The opener now states the actual inscribed concrete surfaces: completed \(\cW_{1+\infty}[\Psi]\) and \(\cW_N[\Psi]\), matching `thm:w-infty-chiral-qg` and `thm:glN-chiral-qg`. The abstract theorem is `Conditional` on the ordered Koszul locus and fixed associator/descent gauges. | resolved: advertised concrete-verification scope now matches inscriptions | ordered_associative_chiral_kd.tex `rem:ord-assoc-notion-fixed`, `thm:w-infty-chiral-qg`, `thm:glN-chiral-qg` |
| 199 | Chiral QG equivalence: "on E_1-chiral algebra on the Koszul locus" | Ordered bar-cobar counit condition is distinct from the symmetric Ran Koszul locus | Resolved 2026-06-08. `ordered_associative_chiral_kd.tex` now defines `def:ordered-koszul-locus`: the full subcategory of finite-type augmented \(\Eone\)-chiral algebras for which \(\Omega(\bar B^{ord}(\cA))\to\cA\) is a quasi-isomorphism in the chosen complete \(\Eone\)-chiral category. The definition explicitly distinguishes this from the symmetric \(\operatorname{Kosz}(X)\) of `def:koszul-locus`. | `thm:chiral-qg-equiv`, the W-infinity theorem, and the gl_N theorem now cite the ordered Koszul locus for the ordered bar-cobar construction. Symmetric/E∞ inputs may enter only after the ordered forgetful presentation preserves the counit; genuinely E1 inputs must satisfy the ordered condition directly. | resolved: type signature repaired | ordered_associative_chiral_kd.tex `def:ordered-koszul-locus`, `thm:chiral-qg-equiv` |
| 200 | CoHA "independent proof of chiral QG": "CoHA = Y (Schiffmann-Vasserot)" | CoHA(C^3) = Y^+ (positive half of affine Yangian) | Resolved 2026-06-08. `rem:independent-proof-coha` now states that the CoHA surface carries the positive-half analogues of the triangle and reaches the target only after Drinfeld double, completion, and vertex-algebra evaluation. Jordan gives \(Y^+(\widehat{\mathfrak{gl}}_1)\); type-\(A\) quiver-origin surfaces give the appropriate positive half; no rank-\(N\) Jordan substitute is used. | Propagated to compute wording in `coha_bar_bridge_engine.py`, `theorem_coha_bar_duality_engine.py`, `w_infinity_chiral_qg_engine.py`, and `gauge_origami_comparison_engine.py`: `CoHA` now denotes \(Y^+\), while `D(CoHA)` / double-completion denotes the full Yangian target. | resolved: positive-half/full-Yangian distinction restored | ordered_associative_chiral_kd.tex:8961-9109; coha_bar_bridge_engine.py; theorem_coha_bar_duality_engine.py; w_infinity_chiral_qg_engine.py |
| 201 | Class-M chain-level claim (thm:w-infty-chiral-qg) vs class-M coderived-only conjecture (conj:coderived-chiral-coproduct) | Completed/Yangian ambient vs raw direct-sum ambient | Resolved 2026-06-08. The old row was partly stale: `thm:w-infty-chiral-qg` is `Conditional`, not `ProvedHere`. The real contradiction was the coderived subsection's exclusivity language. The manuscript now separates three surfaces: raw direct-sum class-M bars have no ordinary chain-level bar-cobar inverse and use the coderived coproduct; completed positive-energy/Yangian packages such as `thm:w-infty-chiral-qg` and `thm:glN-chiral-qg` may carry strict completed coproducts; the coderived coproduct is their image in `D^co` when both are present. | No downgrade of the completed Yangian theorem was made. The false sentence "for class M, Δ^{ch,co} is the only chiral coproduct available" was replaced by a canonical raw-direct-sum statement plus compatibility with strict completed coproducts. The compute W-infinity engine docstring now records the same ambient distinction. | resolved: ambient separation removes contradiction | ordered_associative_chiral_kd.tex `thm:w-infty-chiral-qg`, `conj:coderived-chiral-coproduct`, `prop:bar-is-coderived-chiral-bialgebra`; w_infinity_chiral_qg_engine.py |
| 202 | gl_N Argument B: "via [JKL26] Schiffmann-Vasserot identification H_{Jor,N} ≅ Y^+(gl_N-hat) for all N" | SV13 gives H_{Jor} ≅ Y^+(gl_1-hat) for the Jordan quiver (single vertex); multi-vertex extension is separate | `\cite{JKL26}` now has a bibliography entry, but the rank-N Jordan-quiver identification was still being imported as a proof surface. The "both arguments" framing was too strong: Argument A is the construction; Argument B is only a conditional positive-half CoHA comparison. | Resolved 2026-06-08 in `ordered_associative_chiral_kd.tex` and `glN_affine_yangian_chiral_qg_engine.py`: the gl_N theorem now routes construction through the Koszul-locus bar-cobar Argument A and records the CoHA route as conditional on a type-A positive-half identification, Drinfeld double/completion, and Miura/DS evaluation. | resolved: phantom citation no longer carries load | ordered_associative_chiral_kd.tex:10116-10128; 11135-11660 |
| 203 | DS intertwining "57 tests verify (π_3 × π_3) ∘ Δ_z^{sl_3} = Δ_z^{W_3} ∘ π_3" | Drinfeld universal coproduct formula T(u)T(u-z) expansion is consistent across names | Resolved in pass 370 by fencing the tautological degree-1 comparison and replacing the advertised non-tautological evidence with a finite degree-2 RTT--Miura witness. The active remark now says the full chain-level DS coproduct identity is expected but not proved by the degree-1 Cartan tests; the first proved non-tautological window is `comp:ds-w3-degree2-rtt-miura-witness`. | The compute engine now derives the sl_3 side from projected RTT component bookkeeping: diagonal Gauss/Cartan entries survive as `psi_1, psi_2`, while six off-diagonal root terms are listed and killed by `pi_3`. The W_3 side is computed independently from the primitive two-boson Miura tensor \(T_M={:}\psi_1\psi_2{:}-(\psi_1+\psi_2)^2/(2\Psi)\). The decorated test `test_ds_w3_degree2_rtt_miura_hz_iv` registers `comp:ds-w3-degree2-rtt-miura-witness`, not the broad remark, and does not call the shared `delta_psi_sl3`/`delta_psi_w1inf` formula. The spin-3 DS-HPL coproduct identity remains the named load-bearing obligation for the full compatibility theorem. | tautological degree-1 tests fenced; non-tautological degree-2 finite window installed | ordered_associative_chiral_kd.tex `comp:ds-w3-degree2-rtt-miura-witness`, `rem:ds-intertwining-w3`; ds_coproduct_intertwining_engine.py `ds_w3_degree2_rtt_miura_witness`; test_ds_coproduct_intertwining_engine.py `test_ds_w3_degree2_rtt_miura_hz_iv` |
| 204 | Miura universality Step 3(iii): "Δ_z does not raise W-spin" | Miura transform is lower-triangular in the W-spin filtration | Resolved in pass 369 by promoting the Step 3(iii) assertion to the named lemma `lem:miura-triangularity-under-Delta`. The lemma proves, under the PR triangular Miura basis and Drinfeld coproduct package, that \(\Delta_z\) preserves the largest-primary \(W\)-spin filtration and that all lower Miura sectors have no \(J\otimes W_{s-1}\) or \(W_{s-1}\otimes J\) component for \(s\ge3\). | The proof of `thm:miura-cross-universality-monograph` now treats spin 2 by `lem:coprod-T-miura` and cites `lem:miura-triangularity-under-Delta` for Step 3. The compute engine no longer relies only on the hard-wired support model: `delta_w_spin_filtration_pairs`, `triangular_delta_bound_for_composite`, and `miura_triangularity_under_delta_witness` derive the bound from Drinfeld \(\psi\)-index support and triangular inversion; the decorated test `test_miura_triangularity_under_delta_finite_witness_spins_4_to_6` checks the spin-3 composite coefficient and lower-sector bounds at spins 4-6. | named triangularity lemma and independent finite witness installed | ordered_associative_chiral_kd.tex `lem:miura-triangularity-under-Delta`; miura_universality_proof_engine.py `miura_triangularity_under_delta_witness`; test_miura_universality_proof_engine.py `test_miura_triangularity_under_delta_finite_witness_spins_4_to_6` |
| 205 | KS scattering diagram (H06) "alternative proof of MC2" (CLAUDE.md alt-proof table) | Kontsevich-Soibelman scattering diagrams give modular MC elements from support data | Resolved in pass 368 by making the missing comparison theorem explicit: `conj:ks-support-comparison-mc2` is now a `ClaimStatusOpen` conjecture requiring a KS charge lattice, central charge, support property, filtered dg-Lie morphism into `\widehat{\gAmod}`, primitive-shadow ray identification, and sector logarithm equal to `\Theta_\cA`. | `rem:mc2-scattering-diagram` is now conditional on `conj:ks-support-comparison-mc2` and says that without this package it is only a formal scattering-diagram model, not an independent proof of MC2. `rem:v1-mc2-ks-comparison` and the concordance MC2 row now state the same status. Current `CLAUDE.md` no longer contains an H06/scattering advert, so no CLAUDE edit was needed in this pass. | support-comparison theorem named open; no unconditional H06 proof surface remains | higher_genus_modular_koszul.tex `conj:ks-support-comparison-mc2`, `rem:mc2-scattering-diagram`, `rem:v1-mc2-ks-comparison`; concordance MC2 row |
| 206 | Thm D alt proof H04 (GRR on universal curve) "does not use shadow tower, MC, or thm:genus-universality" | A genuinely independent GRR derivation of obs_g = κ·λ_g would be a disjoint verification path | Resolved in pass 367 by narrowing the H04 GRR remark: it no longer advertises a disjoint proof of the scalar propagator-to-Hodge-bundle bridge. It now says that, once this scalar package is fixed, the GRR route avoids the shadow tower, MC element, and `thm:genus-universality`, but shares the uniform-weight weight-1 propagator input. | Installed the finite-window disjoint check requested here: `comp:heisenberg-g2-fp-grr-check` computes `F_2(H_1)=7/5760` by the explicit Faber--Pandharipande/Mumford `lambda_2^FP` coefficient and by direct GRR/`\hat A` coefficient extraction, while separately recording that `int lambda_2 lambda_1 = 1/5760` is the different genus-2 socle integral. The compute witness is `genus2_heisenberg_fp_grr_disjoint_check()` and the decorated test is `test_heisenberg_g2_fp_grr_disjoint_scalar_window`. | shared-hypothesis overclaim repaired; finite-window IV installed | higher_genus_modular_koszul.tex `rem:theorem-d-alt-grr`, `comp:heisenberg-g2-fp-grr-check`; faber_pandharipande_cross_verification.py; test_higher_genus_modular_koszul_iv.py |
| 207 | SC-formality iff class G: forward "class G ⇒ no higher kernel ⇒ no higher SC tree" | Shadow invariants S_r = 0 for r ≥ 3 transferred to operadic vanishing m_r^SC = 0 requires operadic-transfer injectivity | The old proof conflated "class G" (Gaussian locus, S_r=0 r≥3) with "no higher operadic kernel." The bridge S_r=0 ⇒ m_r^SC=0 requires a two-colour tree-detection statement; bare averaging is not injective because thm:e1-primacy(iv) leaves a Drinfeld-associator kernel. | Resolved in pass 366 by naming the missing package rather than silently proving too much: `lem:sc-tree-detection-standard` defines `SC^{tree-det}_{std}` and states the degreewise equivalence `iota_r^oc(T)=0 ⇔ T=0 ⇔ pi_r^sh(T)=0` on standard primitive connected genus-0 trees. `prop:sc-formal-iff-class-g` now assumes that package explicitly, and concordance rows no longer reduce the proof to "abelian bracket + nondegenerate kappa." `standard_sc_tree_detection_witness()` records the standard-family support table and keeps `bare_averaging_injective=False`. | operadic tree-detection package explicit; no raw averaging injectivity overclaim | chiral_koszul_pairs.tex:3740-3905; theorem_ainfty_nonformality_class_m_engine.py |
| 208 | Depth gap "d_alg ∈ {0,1,2,∞}, gap at 3" universal across the standard landscape | At κ≠0 on a single primary line, Gaussian decomposition gives the dichotomy + MC recursion confirms | The earlier proof had the single-line argument but did not isolate the multi-channel no-go excluding stratum separation that would terminate beyond the contact row. | Resolved in pass 365: `prop:no-finite-depth-beyond-contact` proves the standard-landscape support alternative. Decoupled sectors compose by max, the beta-gamma/bc contact sector has `Def^{(2q)}=0` and stops at `d_alg=2`, and any coupled sector activates a non-nilpotent cubic pump and has infinite depth. The depth-gap proof now cites this proposition and no longer self-cites `prop:depth-gap-trichotomy`. `no_finite_depth_beyond_contact_witness()` records the same finite-support obstruction in compute tests. | multi-channel d_alg=3 ruled out on standard support patterns | higher_genus_modular_koszul.tex:21135-21404 |
| 209 | thm:convolution-d-squared-zero: D^2 = 0 trivially from ∂^2=0 on M̄_{g,n} | Transport through Hom(C_*(M̄), End_A) respects differentials if the Hom-functor is strict | Three-line proof asserted "D = transport of ∂; ∂^2=0" while leaving the strict Hom-complex hypothesis implicit. | Resolved in pass 364: `thm:convolution-d-squared-zero` now states that \(D=D_{\mathrm{st}}\) is the strict stable-curve differential; the proof computes \(D^2\varphi=d_{\operatorname{End}}^2\varphi-\varphi\partial_{\mathcal M}^2=0\), separates coefficient/twisting terms from stable-curve boundary terms, and explicitly excludes ordered \(E_1\) curvature, planted forests, and relative-family curvature from this theorem. `DSquaredVerification.strict_convolution_transport_status` records the same status split. | av-map strictness made explicit | higher_genus_modular_koszul.tex:35666-35728 |
| 210 | thm:ambient-d-squared-zero Step 4: "mixed codimension-2 term cancels by transport identity" | Transport through Hom identifies collision-with-degeneration combinatorics with bar-with-fiber-curvature combinatorics | Mixed-term cancellation was narrated, not computed. At codimension 2, transport identifies ∂_coll ∂_deg + ∂_deg ∂_coll with [d_sew, d_pf] + [d_int, ℏΔ] + [[τ_A, -], d_sew+d_pf]. The sign/combinatorial cancellation is where AP87's factor-2 cross-term could fail. | Resolved in pass 363: `thm:ambient-d-squared-zero` now displays the worked `(k=3,g=1)` SNC corner with normals `(r_{12},q)`, signs `+1` and `-1`, and no factor-2 contribution; `compute/lib/modular_deformation_package.py::DSquaredVerification.worked_codim2_sign_check_k3_g1` records the same check. The theorem remains conditional on the signed log-FM residue-pushforward package. | mixed-term cancellation computed under package | higher_genus_modular_koszul.tex:35820-35867 |
...
- **Phantom citation carrying load** (entry 202): JKL26 now has a bib
  entry, but the rank-N Jordan-quiver proof surface was repaired by making
  Argument B a conditional positive-half CoHA comparison rather than a
...
- Clause (2) coderived off-locus: routed via thm:off-koszul-ran-inversion. As of 2026-06-09, Step 1 applies the named local theorem `thm:chiral-bar-cobar-positselski-7-2`, whose hypothesis package \(\mathsf{Pos}^{\mathrm{ch}}_{7.2}\) isolates the chiral realization/exactness input instead of citing a definition-only "chiral adaptation."
...
The chiral Positselski 5.3 IS isolated: thm:chiral-co-contra-correspondence. The chiral Positselski 7.2 bar-cobar step is now isolated as `thm:chiral-bar-cobar-positselski-7-2` in `coderived_models.tex`; its current status is conditional on the named \(\mathsf{Pos}^{\mathrm{ch}}_{7.2}\) package.

### (b) Structured proof drafts

#### Target 1: thm:chiral-bar-cobar-positselski-7-2 (standalone)
...
Proof draft (pre-inscription). The current theorem does not treat the chiral adaptation as automatic; it packages the chiral realization/exactness requirements as \(\mathsf{Pos}^{\mathrm{ch}}_{7.2}\).
...
Current inscription status: \ClaimStatusConditional. Non-circular: it does not use thm:bar-cobar-inversion-qi or thm:higher-genus-inversion, but the chiral realization/exactness package remains an explicit hypothesis.
...
#### Target 3: Theorem B clause (2) via the named conditional theorem

The stale external Positselski citation plus missing-subsection reference now rewrites as: "By Theorem~\ref{thm:chiral-bar-cobar-positselski-7-2} applied to \(C_n\), \(\varepsilon_n\) is an iso in \(D^{co}(C_n\text{-comod}^{ch})\)." Steps 2 and 3 of thm:off-koszul-ran-inversion are intact (factorisation compatibility from eq:CDG-factorization-compatibility; conservativity from thm:stratified-conservative-restriction). Clause (2) is conditional on finite-dimensional graded pieces plus \(\mathsf{Pos}^{\mathrm{ch}}_{7.2}\), not a direct theorem imported from Positselski without chiral hypotheses.
...
| 219 | Orlik-Solomon Koszulness treated as a contracting homotopy on `H^{>0}(FM_m(C))` | The genuine input is residue-twisted bar concentration at collision strata, with Shelton-Yuzvinsky/Priddy only supplying Koszul-complex acyclicity for the fibre algebra | `OS(A_{m-1})` has zero differential, so no homotopy on `OS^{>=1}` can satisfy `dh + hd = id - pi_0`; replacing the twisted tensor acyclicity by this homotopy confuses a Koszul complex with the algebra itself | Correct route: `thm:pbw-koszulness-criterion` gives bar concentration on the symmetric Koszul surface; localized at each collision stratum it proves positive-fibre-degree acyclicity of the residue-twisted tensor complex. Shelton--Yuzvinsky/Priddy enters only as the fibre Koszul-complex input. On the ordered side before averaging, the same stratum-local acyclicity is `conj:ordered-twisted-tensor-acyclicity`, not a theorem. | Koszul complex vs object |
...
| # | Wrong Claim | Ghost Theorem | Precise Error | Correct Relationship | Type |
|---|-------------|---------------|---------------|---------------------|------|
| 229 | "ChirHoch* carries E_3 action" stated flatly, with concentration in {0,1,2} routed through E_3 rigidity | E_3 structure lives on `Z^{der}_ch(A)`, while Theorem-H concentration is a Vol-I bar-concentration statement on the symmetric Koszul surface | The attempted E_3-rigidity route repeats the native-versus-derived and E_1-versus-E_3 errors: it puts E_3 on `A` and then replaces PBW/bar concentration by an uninscribed E_3-PBW theorem | Correct route: Vol I proves the symmetric Theorem-H surface through PBW, localized bar concentration, chiral quadratic-Koszul comparison, averaging/completion, and strict Mittag-Leffler passage. The ordered first-page statement remains conditional on `conj:ordered-twisted-tensor-acyclicity`. Vol II's Chiral Higher Deligne theorem concerns the derived centre and does not furnish an unconditional replacement proof. | native vs derived E_n |
...
| 249 | "Unit functor k-Mod -> Dmod(X), M -> M tensor 1_V" cited in LV transfer Step 2 | No strict unit object in (Dmod(X), tensor^{ch}) | FM70: the chiral unit is a factorization structure, not a tensor unit; the cited unit functor does not exist as stated | Correct replacement: work in the conditional Ran factorization ambient \(H_{\Fact}(X)\), where the sheaf-level factorization unit is part of the supplied package. Do not cite GR17 IV.5 for this unit or claim that LV transfer applies verbatim to \(\Dmod(X)\); the transfer to \(\Fact(X)\) is conditional on the ambient/properadic package. | wrong unit functor |
...
| 251 | Vallette Theorem 2.1 cited "specialized to chiral operad" for Quillen equivalence at chain level | Val16 is chain complexes over a field with projective model structure; not Dmod(X)-enriched | FM72: the Vallette model structure is for k-linear cooperads; its transfer to Dmod(X)-enriched cooperads needs a different model structure | Do not replace Val16 by a phantom GR17 IV.5 model theorem. Use Val16 only on the \(k\)-linear pole-free operadic model; the Ran/factorization/properadic lift is the conditional \(H_{\Fact}(X)\) package plus Hackney--Robertson/Hinich machinery, and remains conditional unless an explicit factorization model theorem is proved in this volume. | wrong model category |
...
The proof now handles the subtlety HONESTLY via a two-step tower, NOT by silently assuming vanishing:
...
2. **Higher coherence (not shadow depth alone)**: the brace-deformation defect D^(1) := [m, tilde G_1] - ∂_z has components D^(1)_n for n ≥ 2. D^(1)_2 is the cubic harmonic obstruction, which vanishes by Jacobi for finite-dim simple Lie algebras. For n ≥ 3, shadow depth r_max = 3 only says the scalar Σ-coinvariant shadow invariants S_r vanish for r ≥ 4; it does not force pre-shadow brace components to vanish. The original-complex upgrade is therefore conditional on the named higher-brace coherence package HBC_L(g,k).
...
| $V_k(\mathfrak{sl}_2)$ gen | $(1,0,1)$ | $(1,0,0)$ | $(1,0,1)$ |
| $\mathrm{Vir}_c$ gen | $(1,0,1)$ | $(1,0,0)$ | $(1,0,1)$ |

The old affine mismatch was a zero-mode prequotient artefact: after the
inner quotient, $\ChirHoch^1(V_k(\mathfrak{sl}_2))=0$ and the
cohomological dimension vector agrees with the truncated GF column.
The adjoint $\mathfrak g$ survives only as prequotient/bar-dual metadata;
it is not an outer-derivation enhancement in first Hochschild cohomology.
...
**Canonical violation repaired (comparison-gated)**: `prop:perfectness-standard-landscape`
now invokes `prop:conformal-blocks-bar`
(`chapters/theory/chiral_modules.tex:691`) only under the pointed-bar /
conformal-block comparison package: degreewise finite holonomic pointed
bar terms, exactness above degree \(0\), TUY/Hitchin finite-rank
comparison, sewing, and determinant-anomaly matching.  The pointed bar
complex itself computes derived coinvariants; identifying its \(H^0\)
with classical conformal blocks is conditional, not a proved-here
bridge.
...
The five-archetype $\ChirHoch^3$ portrait (entry 310-W23-chirhoch3-five-archetype-portrait-shadow-finite-zero-vs-infinite-depth-W3Zam) names rows G/L/C as ambient-curve $[\chi_3]=0$, row M as the GF/shadow or extension representative $[W_3^{\mathrm{Zam}}]$, and row B as the CY-3 product-ambient class $[W_3^{\mathrm{Zam}}+\hbar\cdot\mathrm{qt}(J^{(3)})]$. Pattern 265 sharpens the vanishing rows with explicit $\partial$-exact primitives:
...
**Row B (BKM crown)**: $[\chi_3^{\mathbf B}] = \mathbb C \cdot [W_3^{\mathrm{Zam}} + \hbar \cdot \mathrm{qt}(J^{(3)})]$ with Yangian quantum tail on the CY-3 product ambient at the recognition-gated Vol I coordinate $r_{\max}=5$; classical limit $\hbar \to 0$ reduces to row M Virasoro skeleton.

**Structural law**: finite rows $\mathsf G,\mathsf L,\mathsf C$ have $[\chi_3]=0$ in the ambient-curve chiral Hochschild complex by archetype-specific coboundary primitives and Theorem H concentration; row $\mathsf M$ has $r_{\max}=\infty$ but supports the Zamolodchikov class only in the GF/shadow or extension ambient; row $\mathbf B$ supports the non-zero class on the CY-3 product ambient at recognition-gated Vol I coordinate $r_{\max}=5$.
...
- $r_{\max} \in \{2, 3, 4, \infty, 5\}$
- $[\chi_3] \in \{0, 0, 0, \mathbb C_{\mathrm{GF/shadow}}, \mathbb C_{\mathrm{CY3}}\}$ (Pattern 265, with ambient qualifiers)

The BKM crown $\mathbf B = \mathbf H_{\Delta_5}$ sits at $\kappa + \kappa^! = 8$ via Mukai factor + Kodaira 24-fibre residue and at recognition-gated Vol I coordinate $r_{\max}=5$; the M-row extensions at $\{250/3, 98/3\}$ are $\mathsf W$-algebra specialisations.
