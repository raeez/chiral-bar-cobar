...
| H | PROVED sharp Hilbert series on the symmetric Koszul locus; ordered E_1 concentration CONDITIONAL | ChirHoch*(A) concentrated in degrees {0,1,2} with exact formula `P(t,q) = HS_{Z(A)}(q) + HS_{ChirHoch^1(A)}(q)·t + HS_{Z(A^!)}(q)·t^2`. Critical level k=-h^v excluded (load-bearing; `rem:critical-level-dimensional-divergence` at `chiral_hochschild_koszul.tex:1850-1897`: Feigin-Frenkel centre infinite-dimensionalises ChirHoch^0, FM-tower collapse fails, Koszul hypothesis fails). **Step 3 circularity RESOLVED on the symmetric Koszul surface (2026-04-16; firewall refined 2026-06-16)**: rerouted through `thm:pbw-koszulness-criterion` (PBW collapse, inscribed at `chiral_koszul_pairs.tex:784`) and bar concentration localized at collision strata. Shelton--Yuzvinsky/Priddy supplies Koszul-complex acyclicity for the pure-braid Orlik--Solomon algebra; it does **not** furnish a contracting homotopy of the Arnold algebra itself. **AP242 spot-check (2026-04-18)**: `lem:chiral-quadratic-koszul` IS inscribed with a proof body in `chiral_hochschild_koszul.tex:656-727` (NOT a forward reference). Proof rewritten 2026-04-18 to make the Fresse-transport non-degeneracy step explicit (Step 1 = fibrewise-flat reduction to $\cF(E_p)(3)$ where the classical OPE-residue pairing is non-degenerate via Fresse Prop. 7.1.3, with propagation via $\cD_{X^3}$-coherence + semicontinuity + Kontsevich formality at collision divisors; Step 2 = Fresse transport along the pairing; Step 3 = quasi-isomorphism on Koszul locus). Three HZ-IV decorators LIVE at `compute/tests/test_theorem_H_hochschild_koszul.py` (Heisenberg → dim triple (1,1,1) total 3; Virasoro → (1,0,1) total 2; affine sl_2 → (1,0,1) total 2 after the zero-mode quotient) wired to `compute/lib/chirhoch_dimension_engine.py`. Honest HZ-IV residual: the engine's dataclass values cite `chiral_center_theorem.tex` as their VERIFIED source, which is the derivation chapter under review; genuine disjoint wiring to Feigin-Fuchs 1984 Fock-BRST engine and Wang 1998 semi-infinite BRST engine is not yet implemented (decorator prose is correct, test body disjointness is partial — AP277 residual, not vacuous). **Ordered E_1-page variant `thm:hochschild-concentration-E1`** is `\ClaimStatusConjectured`/conditional at `chiral_hochschild_koszul.tex`: it requires `conj:ordered-twisted-tensor-acyclicity`, the residue-twisted tensor acyclicity at every ordered collision stratum. The PBW criterion and Orlik--Solomon Koszulness are necessary inputs but do not by themselves kill positive Arnold fibre degree; the symmetric statement then uses the averaging step of `cor:hochschild-averaging-symmetric`. **AP255 phantom-file flag in `standalone/theorem_index.tex` healed 2026-04-18**: rows for `thm:pbw-koszulness-criterion` and `thm:hochschild-concentration-E1` were tagged `chiral_pbw_koszulness.tex [PHANTOM]`; both labels are in fact inscribed in live chapters. Index corrected. ALT H05 retrievable as sharp-Hilbert-series from `fm-tower-collapse` E_2 page on the symmetric Koszul surface. |
...
| ChirHoch^1 KM | CORRECTED at generic k for general simple g: the normalized fixed-fiber quotient has ChirHoch^0 = C, ChirHoch^1 = 0, ChirHoch^2 = C; the adjoint g is only the zero-mode derivation prequotient / bar-dual generator count. | prop:chirhoch1-affine-km and prop:chirhoch2-affine-km-general in `chapters/theory/chiral_center_theorem.tex` now record the zero-mode quotient firewall. The modes $(J^a)_{(0)}$ implement the adjoint derivations internally by $(J^a)_{(0)}J^b=f^{ab}_{c}J^c$, so Whitehead gives no outer quotient for simple g. Feigin-Frenkel dual level -k - 2h^v is non-critical at generic k, so the dual center still gives the degree-2 scalar class. Theorem H polynomial concentration kills ChirHoch^{>=3}. Rank-independent slots: ChirHoch^0 and ChirHoch^2 both have dimension 1; there is no rank-scaling ChirHoch^1 slot after quotienting. Correct affine totals: sl_2, sl_3, sl_N, and E_8 all have total dimension 2 on the fixed generic fiber; their adjoint dimensions survive only as prequotient metadata. |
...
**AP242 (Forward-reference lemma labelled as inscribed).** A load-bearing lemma cited in a proof as `\ref{lem:foo}` where either (a) the label `\label{lem:foo}` exists nowhere across the three volumes, or (b) the lemma body at the label is itself a forward-reference with no proof. Historical example: Vol I `thm:hochschild-concentration-E1` once cited `lem:chiral-quadratic-koszul` as if it transported a Shelton--Yuzvinsky contraction of the Arnold algebra; the current repair separates the proved chiral quadratic-Koszul comparison from the ordered twisted-tensor acyclicity conjecture and records that Shelton--Yuzvinsky/Priddy contracts only the Koszul complex. Counter: before inscribing a theorem, verify every `\ref{lem:...}` in the proof resolves to a `\label{lem:...}` with a proven body and that the lemma proves the exact complex being used. Paired with HZ-11 for cross-volume resolution.
...
- **β_N all-rank harmonic law** — STATUS CORRECTED (Vol II `chapters/theory/beta_N_closed_form_all_platonic.tex`, `conj:beta-N-harmonic-closed-form`): β_N = 12(H_N - 1) = sum_{s=2}^{N} 12/s is the conjectural harmonic identification, not a proved all-rank closure. Conditional on it, both prior candidates are ruled out at N=4: (N+1)(N+2)/2 predicts 15; N²-N+4 predicts 16; harmonic value is 13. Conditional on the same law, β_N is rational and non-integral for every N ≥ 5; β_5 = 77/5, β_6 = 87/5. The proved finite-rank statement is Vol II `thm:wn-tempered-all-N`, which uses finiteness plus Stirling dominance, not the all-rank harmonic identification.
...
- **AP845 -- Circular proof routing 3-hop (Critical).** $L_1 \to L_2
  ...
  identifying non-circular anchor (PBW / localized bar concentration /
  Arnold-Priddy Koszul-complex input).
