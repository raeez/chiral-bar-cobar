...
On the Koszul locus — the full subcategory of chirally Koszul conilpotent-complete factorization algebras — the counit $\Omega^{\mathrm{ch}}\overline{B}^{\,\mathrm{ch}}(A)\to A$ is a weak equivalence for the certified standard-family lanes: non-critical affine Kac--Moody under the PBW/chiral-Koszul/completion package, Virasoro and $W_N$ on their completed class-\(\mathsf M\) surfaces, Heisenberg and lattice vertex algebras, the DS lanes where the comparison theorem is supplied, and Yangian-type ordered examples with their finite-window hypotheses. Critical affine level is the Feigin--Frenkel centre/Sugawara-degeneration boundary, and admissible simple quotients require the quotient-bar spectral-sequence and completion package; neither is supplied by rationality or \(C_2\)-cofiniteness alone.
