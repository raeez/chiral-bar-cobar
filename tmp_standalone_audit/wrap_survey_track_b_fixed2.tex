...
$r$-matrix is $r_{\mathrm{cl}}(z) = k\,\Omega/z$ with $\Omega = \sum_a
...
\[
\kappa(V_\Lambda^{N,q}) \;=\; \operatorname{rank}(\Lambda) \;=\; N,
\]
...
$R_T^{\mathrm{mod}}(z;\hbar) \in \MC(Y_T^{\mathrm{mod}})$ is an MC element.
...
$\mathfrak{R}_{4,g,n}^{\mathrm{mod}}(\cA)$ satisfies a clutching law
...
of $\Sigma_g$. The modular bar complex $\barB_{\mathrm{mod}}(\cA)$ is
...
$\mathrm{FT}_{\mathsf{Com}_{\mathrm{mod}}/\SCchtop}$ mediates between
...
$L_{\mathrm{mod}}(C) := \Coder(\mathbf B_{\mathrm{mod}}(C))[-1]$ is complete
filtered by genus, with convolution
$L_{\mathrm{mod}}(C) \cong \Hom(\mathbf B_{\mathrm{mod}}(C), sC)[-1]$ under
...
$[\mathfrak R^{\mathrm{mod}}_4(\cA)] = [o_4(\cA)]
...
$\mathfrak R^{\mathrm{mod}}_4$ is the first global nonlinear invariant,
...
$(d_{\BV} + \hbar\Delta_{\mathrm{odd}})\exp(S^{\mathrm{mod}}/\hbar) = 0$
...
factorization envelope $U^{\mathrm{mod}}_X(L)$, left adjoint to
...
envelope $U^{\mathrm{mod}}_X(L)$ satisfies the adjunction
\[
\Hom_{\mathsf{Fact}^{\mathrm{mod}}}(U^{\mathrm{mod}}_X(L), \cF)
\;\cong\;
\Hom_{\mathsf{LCA}^{\cyc}}(L, \mathrm{Prim}^{\mathrm{mod}}(\cF)),
\]
and $\Theta_L = \Theta^{\mathrm{env}}_{\le\infty}(L)$ is the
universal MC class. Direct sums: $\kappa$, $T^{\mathrm{br}}$,
$\Delta$, $R_4^{\mathrm{mod}}$ decompose additively. BRST reduction:
...
The modular bar coalgebra $\barB_{\mathrm{mod}}(\cA)$ is an algebra
over the relative Feynman transform
$\mathrm{FT}_{\mathsf{Com}_{\mathrm{mod}}/\SCchtop}$, the algebraic
...
structure on $\SCchtop_{\mathrm{mod}}$ produces it via formal
completion.
...
is the $\mathsf{Com}_{\mathrm{mod}}$-modular structure.
...
$\mathrm{FT}_{\mathsf{Com}_{\mathrm{mod}}/\SCchtop}^{\;2} \simeq \id$.
