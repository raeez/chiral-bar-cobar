...
Beilinson--Drinfeld~\cite{BD} (chiral algebras), and Lurie~\cite{HA}
...
\begin{definition}[Orientation local system]
\label{def:orientation-line}
For a real smooth manifold $M$ of dimension $d$, the orientation local system
is the rank-one $k$-local system associated to its frame bundle by
$g\mapsto\operatorname{sgn}(\det g)$.
Its integral form has fiber $H_d(M,M\setminus\{p\};\mathbb Z)$ at $p$.
An orientation chooses a generator in each integral fiber, locally consistently.
The real line bundle $\bigwedge^d TM$ instead has transition functions $\det g$.
These objects have the same orientability obstruction, but only the former is defined as a local system.
A complex manifold has its canonical real orientation.
\end{definition}

\begin{proposition}[Tangent determinants and adjunction; \ClaimStatusProvedHere]
\label{prop:det-config}
Let $X$ be a smooth complex curve and $U=\operatorname{Conf}_n(X)$.
Then
\[
T_U\cong\bigoplus_{i=1}^n\operatorname{pr}_i^*T_X,
\qquad
\det_{\mathbb C}T_U\cong\bigotimes_{i=1}^n\operatorname{pr}_i^*T_X.
\]
For a smooth compactification $Y$ of $U$, the first determinant is $K_Y^{-1}|_U$.
For a smooth divisor $D\subset Y$, adjunction gives
\[
K_D\cong(K_Y\otimes\mathcal O_Y(D))|_D.
\]
Permuting two complex coordinate directions preserves the real orientation.
\end{proposition}
\begin{proof}
The tangent splitting restricts the tangent bundle of $X^n$ to $U$.
Taking its top complex exterior power proves the determinant identity.
The conormal sequence
$0\to\mathcal O_D(-D)\to\Omega_Y^1|_D\to\Omega_D^1\to0$
gives the adjunction formula after taking determinants.
Each complex coordinate has two real coordinate directions.
Exchanging two such blocks contributes $(-1)^{2\cdot2}=1$ to the real orientation.
The edge determinant sign instead comes from exchanging two odd logarithmic generators.
\end{proof}

\begin{construction}[Determinant line in the ordered bar complex]
\label{constr:det-bar}
For an augmented differential graded associative algebra $A$, the length-$n$ bar term is
$(A[1])^{\otimes n}$, with $A$ replaced by its augmentation ideal.
The map~\eqref{eq:bar-determinant-map} identifies this term with
$\bigwedge^n k^{[n]}[n]\otimes\bar A^{\otimes n}$.
Its sign comes from moving the degree-minus-one symbols to the left.
The determinant has degree $-n$ and comes before the algebra factors.
For a normal-crossing product chart with divisor labels $E$,
equation~\eqref{eq:log-symbol-comparison} identifies the direct sum of determinant terms over all $S\subseteq E$
with the subcomplex of normal logarithmic forms on all product strata.
The source has the global shift $[-2|E|]$.
A residue moves from the $S$-summand to the $S\cup\{e\}$-summand, so the direct sum is required for closure.
It does not identify all logarithmic forms with a line bundle.
The full residue sheaf has strict coordinate pullbacks by Proposition~\ref{prop:strict-residue-descent}.
The chosen normal image can move under a unit change; its fixed-coefficient overlap comparison is
Theorem~\ref{thm:strict-homotopy-cocycle}, and ordered bar coefficients carry the additional sign~\eqref{eq:bar-overlap-homotopy}.
For state-changing adjacent collisions, the coefficient maps and their exact domains are
\eqref{eq:interval-coefficient-system}--\eqref{eq:state-residue-differential}.
Their local logarithmic comparison is~\eqref{eq:state-logarithmic-homotopy}; the $\mathbb C^*$ counterexamples exclude a general unfiltered or filtered transfer without the respective hypotheses.
\end{construction}
...
\begin{theorem}[Determinant conventions for associative bar signs; \ClaimStatusProvedHere]
\label{thm:det-bar-cobar-signs}
For an augmented differential graded associative algebra $A$, the map
\[
[a_1|\cdots|a_n]\longmapsto
(-1)^{\sum_j(n-j)|a_j|}(e_1\wedge\cdots\wedge e_n)
\otimes a_1\otimes\cdots\otimes a_n
\]
identifies the binary bar differential with
$\sum_{i=1}^{n-1}\iota_{e_i}\otimes\mu_i$.
The determinant has degree $-n$.
The resulting sign on the desuspended word is
$(-1)^{\sum_{j\leq i}|a_j|-i+1}$.
\end{theorem}
\begin{proof}
The input and output reordering exponents differ by
$\sum_{j\leq i}|a_j|$ modulo two.
Combining this with the binary bar sign leaves $i-1$ modulo two,
which is the sign of $\iota_{e_i}$.
The unary differential becomes $(-1)^n d_{\bar A^{\otimes n}}$.
Proposition~\ref{prop:bar-determinant-comparison} supplies the full computation.
\end{proof}

A chiral comparison must also intertwine the native state contractions with the coefficient maps.
The determinant calculation gives the sign of that comparison once those maps are supplied.
The coefficient identities are stated in Theorem~\ref{thm:exact-coefficient-criterion}.
