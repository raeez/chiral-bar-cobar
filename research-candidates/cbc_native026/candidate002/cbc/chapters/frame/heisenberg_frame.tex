...
\end{equation}
This requires care.  The element
...
Only two tensor slots carry currents, namely those at
$z_3$ and $z_4$; the remaining slot is the vacuum scalar
$k$ created by the previous contraction.  So this is not a
generic degree-$2$ bar element.  It is a degree-$2$ element with
one vacuum entry.
...
and $z_4$).  The residue must extract the factor $\eta_{14}$, but that
factor is hidden inside $\eta_{13} \wedge \eta_{34}$.  The right move is
to expose it before restricting, using the Arnold relation on the
triple $(1,3,4)$:
\[
\eta_{13} \wedge \eta_{34}
= \eta_{14} \wedge (\eta_{13} - \eta_{34}).
\]
Now the residue along $D_{14}$ is visible, and the bar differential gives
...
\end{aligned}
\end{equation}
After the collapse $z_1 = z_4$, the residual form is the $1$-form from
the collision point to $z_3$:
\[
(\eta_{13} - \eta_{34})\big|_{z_1 = z_4} = 2\eta_{(14),3}.
\]
We will not use that relabelled representative below.  For the
verification of $d^2 = 0$, only the coefficient and sign matter, and
the structural Arnold-pair cancellation is cleaner.
...
\emph{What this means geometrically.}
The Maurer--Cartan equation is not an axiom imposed on~$\tau$
...
\emph{Pole order.}
The simple pole means the bar differential
$d = d_{\mathrm{bracket}}$ has no curvature component ---
it is entirely bracket-type.  At degree~$1$, it extracts the
simple-pole residue
$\operatorname{Res}_{z=w} \psi(z)\psi(w) = 1$, a scalar.

\emph{Statistics.}
The Koszul sign rule acts on the bar complex: when
$\psi$ is fermionic ($|\psi| = 1$), two desuspended
cogenerators in $\bar{B}^2$ satisfy
$\psi^* \otimes \psi^* = -\psi^* \otimes \psi^*$
under transposition, forcing \emph{antisymmetry}.
The coproduct on $\bar{B}(\mathcal{F})$ is therefore
\emph{co-antisymmetric}: the bar coalgebra is a coLie
coalgebra, and the Koszul dual algebra is the
\emph{commutative} algebra $\mathrm{Com}$.  This is
$\mathrm{Lie}^! = \mathrm{Com}$, visible in the OPE:
the self-pairing $\psi(z)\psi(w) \sim 1/(z-w)$ is
antisymmetric under $z \leftrightarrow w$ (fermionic),
encoding Lie structure.

Contrast with Heisenberg: $\alpha$ is bosonic ($|\alpha| = 0$),
so the desuspended cogenerators commute, the bar coalgebra
is cocommutative (coLie $\to$ Sym), and the Koszul dual is
$\mathrm{Sym}^{\mathrm{ch}}(V^*)$.  The
\emph{statistics of the generators}, not the pole order,
determine the operadic type.
...
\begin{remark}[Three dualities]\label{rem:frame-three-dualities-intro}
The free fermion participates in three distinct dualities: (1)~Koszul duality $\mathcal{F}^! = \beta\gamma$; (2)~bosonization $\psi(z) = {:\!}e^{i\phi(z)}{:\!}$ (representation-theoretic, not Koszul); (3)~level-shifting $\mathcal{H}_k \cong \mathcal{H}_{-k}$ via $\alpha \mapsto -\alpha$ (automorphism).  The $bc$/$\beta\gamma$ pair realizes $\chirLie \leftrightarrow \chirCom$ with $\dim V = 2$.
\end{remark}
...
\end{equation}
which appears in the $W$--$W$ OPE at a \emph{quartic} pole:
\[
W(z)\,W(w) \sim \frac{c/3}{(z-w)^6}
  + \frac{2T(w)}{(z-w)^4}
  + \frac{\partial T(w)}{(z-w)^3}
  + \frac{2\Lambda(w) + \tfrac{3}{10}\partial^2 T(w)}{(z-w)^2}
  + \cdots
\]
