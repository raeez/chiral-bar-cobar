...
\emph{(II.3)} $\{d_{\text{fact}},d_{\text{config}}\}=0$.
On a logarithmic residue summand, take $d_{\text{config}}=d_{\mathrm{dR}}$
and use the left-normal convention of~\eqref{eq:left-residue}.
Writing $\alpha=d\log u\wedge\beta+\gamma$ gives
\[
\operatorname{Res}_{u=0}(d\alpha)
=-d\bigl(\operatorname{Res}_{u=0}\alpha\bigr).
\]
The coefficient contraction must be a horizontal map into the coefficient system on the collision stratum.
Under this hypothesis, it commutes with exterior differentiation.
The displayed identity proves the mixed cancellation on that summand.
Summing over collision components proves the same cancellation in every arity.
Proposition~\ref{prop:residue-identities} states the domains and total shifts explicitly.
The Arnold relation on an affine coordinate configuration is
\begin{equation}\label{eq:arnold-three-point}
\eta_{12}\wedge\eta_{23}+\eta_{23}\wedge\eta_{31}
+\eta_{31}\wedge\eta_{12}=0.
\end{equation}
This wedge identity is distinct from the residue--de Rham identity.
In the logarithmic de Rham complex, $d_{\mathrm{dR}}\eta_{12}=0$.
A current supported on the diagonal does not belong to that complex.
