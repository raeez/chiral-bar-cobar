...
For a universal principal W-algebra, DS cohomology identifies the local vertex algebra with a reduction of the full affine--ghost complex.
For $\mathfrak{sl}_2$, the resulting charge is $c=1-6(k+1)^2/(k+2)$, at $k\ne-2$.
A corresponding construction for conformal blocks must specify a BRST complex of global coinvariants and a comparison map to the W-coinvariants.
In particular, the proposed identification
\[
 \mathbb V(\mathcal W^k(\fg);\vec{\mathcal M};X)
 \simeq H^0_{Q_{\mathrm{DS}}}
 \mathbb V(V^k(\fg)\otimes\mathcal F_\chi;\vec{\mathcal N}_{\mathrm{BRST}};X)
\]
requires the indicated ghost modules, compatible local actions, and a proof that taking coinvariants and BRST cohomology commutes in that category.
The local vertex identification alone does not prove it.
A reflected conformal-block comparison further requires a map compatible with the reflected vertex carrier.
...
\begin{theorem}[Module bars after the subregular Poisson quotient]
