...
\subsection{Filtered states and chain maps}
\label{sec:koszulness-verification}
\input{chapters/theory/filtered_pbw_lifts}

\begin{remark}[State symbols and the chiral coefficient complex]
\label{rem:classical-to-chiral-koszul}
The polynomial contraction above proves exactness of a classical polynomial Koszul resolution.
Applying it to a native chiral complex requires the symbol map and global-section hypotheses of Theorem~\ref{thm:pbw-koszulness-criterion}.
The current and Virasoro state bases supply the state filtration.
They do not identify the full collision sheaves or their differential with that polynomial resolution.
\end{remark}
...
Let $\fg$ be simple and let $k$ be a level.
Suppose the full native Koszul complex of $\widehat{\fg}_k$ satisfies the coefficient, symbol-comparison and global-section hypotheses of Theorem~\ref{thm:pbw-koszulness-criterion}.
Then $\widehat{\fg}_k$ is chiral Koszul for that complex.
\end{theorem}
\begin{proof}
Lemma~\ref{lem:fl19-state-pbw} gives the local vacuum-state basis and its PBW filtration.
Its associated graded state space is $\operatorname{Sym}(\fg\otimes t^{-1}\mathbb C[t^{-1}])$.
The polynomial resolution of this symmetric algebra has the explicit Euler contraction given above.
For $b$ full positive-energy state factors, their energy series is
$\bigl(\prod_{n\ge1}(1-q^n)^{-\dim\fg}-1\bigr)^b$.
These are finite coefficients of state tensors; the factors are full states, rather than only single current generators.
Example~\ref{ex:fl19-poles} shows why this calculation does not establish a bound for all collision sections.
The stated additional native hypotheses supply that filtration and its symbol comparison, so Theorem~\ref{thm:pbw-koszulness-criterion} applies.
\end{proof}
...
Let $c\in\mathbb C$.
Suppose the full native Koszul complex of the universal Virasoro vertex algebra $\mathrm{Vir}_c$ satisfies Theorem~\ref{thm:pbw-koszulness-criterion}.
Then $\mathrm{Vir}_c$ is chiral Koszul for that complex.
\end{theorem}
\begin{proof}
The ordered Virasoro vacuum basis consists of words in $L_{-n}$ with $n\ge2$.
Its associated graded state space is the polynomial algebra on these modes.
At energy $h$, the number of words is the coefficient of $q^h$ in $\prod_{n\ge2}(1-q^n)^{-1}$, and their PBW order is at most $\lfloor h/2\rfloor$.
The preceding polynomial contraction proves the classical resolution statement.
The full native coefficient and section comparisons remain the additional hypotheses in the statement.
Under them, Theorem~\ref{thm:pbw-koszulness-criterion} gives the conclusion.
\end{proof}
...
Bershadsky--Polyakov & 4 & Algebraic level family & Four even fields \\
