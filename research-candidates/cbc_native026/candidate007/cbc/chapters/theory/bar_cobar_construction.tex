...
curve~$X$ is an integral transform whose kernel is the logarithmic
1-form $\eta_{ij} = d\log(z_i - z_j)$ of \S\ref{sec:the-seed},
integrated against the OPE data on configuration spaces.  Its output
is a factorization coalgebra on $\operatorname{Ran}(X)$ with nilpotent
differential ($d^2 = 0$) at genus~$0$.  Verdier duality inverts the
transform: $\Omega \circ \barB \xrightarrow{\sim} \mathrm{id}$ on the
Koszul locus (Theorem~\ref{thm:bar-cobar-inversion-qi}), the
categorical logarithm--exponential correspondence of
...
A comparison between a bar complex and a Batalin--Vilkovisky complex requires two defined carriers and an actual chain map compatible with the relevant operations.
Theorem~\ref{thm:brst-bar-genus0} is a filtered criterion for a specified relative Virasoro state complex and a separately constructed target.
It assumes compatible exhaustive split filtrations, an actual graded chain quasi-isomorphism, and compatible corrections.
A sheaf application also requires relative descent and the prescribed section carrier.
For $V\otimes bc$, the augmentation-kernel target in this chapter is excluded by Proposition~\ref{prop:vir-no-augmentation}, and the standard relative transition restriction fails by Proposition~\ref{prop:vir-relative-target-obstruction}.
The following correspondences therefore require those chain and operation comparisons.
...
\end{center}
\noindent
These correspondences are conditional on those chain and operation comparisons.
Chapter~\ref{ch:bv-brst} proves the Virasoro operator identity and states the precise filtered comparison criterion.
\end{remark}
...
A residue lowers differential-form degree by one.  To state its total degree,
let $U$ be a complex coordinate chart with a normal-crossing divisor
$D=\bigcup_{e\in E}D_e$, where $E$ is finite and $D_e=\{x_e=0\}$, and write $U_S=\bigcap_{e\in S}\{x_e=0\}$
for $S\subseteq E$, with $U_\varnothing=U$.
Let $M_S$ be cochain complexes of complex vector spaces, with degree-zero cochain maps
$\mu_{S,e}:M_S\to M_{S\cup\{e\}}$ for $e\notin S$.
Write $i_S:U_S\hookrightarrow U$ for the inclusion and
$D^S=\bigcup_{e\notin S}(U_S\cap D_e)$ for the residual divisor.
The constant coefficient sheaf with fiber $M_S$ is denoted $\underline{M_S}$.
Use the full direct sum
\[
\mathcal R(M)=\bigoplus_{S\subseteq E}
 i_{S*}\bigl(\underline{M_S}\otimes\Omega_{U_S}^{\bullet}(\log D^S)\bigr)[-2|S|].
\]
The shift convention is $C[k]^j=C^{j+k}$.
Thus $v\otimes\alpha$ on $U_S$, with $|v|=q$ and form degree $p$, has total degree
$q+p+2|S|$.  In the left-normal convention,
\[
\alpha=d\log x_e\wedge\beta+\gamma,
\qquad r_{S,e}\alpha=\beta|_{U_{S\cup\{e\}}},
\]
where $\beta$ and $\gamma$ contain no $d\log x_e$ factor.  In particular,
$r_{S,e}(d\log x_e)=1$: the form degree changes from $p$ to $p-1$,
and the stratum shift contributes $2$.  Their sum changes the total degree by $1$.

With coefficients first, the degree-one operator has three components:
\begin{align*}
d_{\mathrm{internal}}(v\otimes\alpha)&=d_{M_S}v\otimes\alpha,\\
d_{\mathrm{form}}(v\otimes\alpha)&=(-1)^qv\otimes d_{\mathrm{dR}}\alpha,\\
d_{\mathrm{residue}}(v\otimes\alpha)&=
 \sum_{e\notin S}(-1)^q\mu_{S,e}(v)\otimes r_{S,e}\alpha.
\end{align*}
Set $d=d_{\mathrm{internal}}+d_{\mathrm{form}}+d_{\mathrm{residue}}$.
For a tensor of unsuspended coefficient states,
\[
d_M(\phi_1\otimes\cdots\otimes\phi_b)
=\sum_{i=1}^b(-1)^{\sum_{j<i}|\phi_j|}
 \phi_1\otimes\cdots\otimes d\phi_i\otimes\cdots\otimes\phi_b.
\]
Desuspending each coefficient is a different grading: its unary and binary signs
are computed in Proposition~\ref{prop:bar-determinant-comparison}.
The present geometric total degree uses the displayed stratum shifts.

For an ordered associative algebra, $M_S$ consists of the tensor factors indexed
by the consecutive blocks obtained after contracting the cuts in $S$;
$\mu_{S,e}$ multiplies the two adjacent blocks separated by $e$.
The construction and its coefficient identities are given in
Lemma~\ref{lem:interval-coefficient-squares}.
For a chiral algebra, a collision divisor $D_{ij}$ corresponds to $z_i=z_j$,
and its coefficient map must extract the appropriate OPE data.
A chiral product has a separated-point domain and a diagonal-supported target;
it is not an ordinary map $\mathcal A\otimes\mathcal A\to\mathcal A$ on a fixed fiber.
Consequently the state sheaves, their collision maps, and their common iterated targets
must be constructed before the displayed local operator can define a chiral bar differential.
The tensor, orientation, and field-degree signs are fixed by the coefficient-first rule above
and the left-normal residue convention; the residue deletion sign is not inserted twice.
...
Nilpotency requires identities for maps with the same source and target.
The local residues satisfy $r_f r_e=-r_e r_f$ and
$r_e d_{\mathrm{dR}}=-d_{\mathrm{dR}}r_e$.
Indeed, deleting the two normal factors in opposite orders reverses their exterior sign,
and differentiating $d\log x_e\wedge\beta$ gives $-d\log x_e\wedge d\beta$.
If the two coefficient composites satisfy
\[
\mu_{S\cup\{e\},f}\mu_{S,e}
=\mu_{S\cup\{f\},e}\mu_{S,f},
\]
then the two terms of $d_{\mathrm{residue}}^2$ cancel.
The cochain property of $\mu$ cancels its mixed terms with $d_{\mathrm{internal}}$:
the coefficient signs are $(-1)^q$ and $(-1)^{q+1}$.
The residue identity with $d_{\mathrm{dR}}$ cancels the remaining mixed terms.
These conditions prove $d^2=0$ on $\mathcal R(M)$.
They hold for the ordered interval states by associativity and the derivation rule.
For general chiral coefficients, the Borcherds identity is an algebraic input
(Remark~\ref{rem:bicomplex-obstruction}); identifying its terms with the required
common geometric targets and compatible overlap maps is a separate construction.
Neither a residue sign nor Stokes' theorem supplies that construction.
\end{remark}
...
An orientation line controls signs after the face maps and their coefficient identities have been supplied.  It does not by itself prove $d^2=0$, equivariance under $\mathfrak{S}_{p+1}$, the genus-graded $A_\infty$ identities, or $\text{Sp}(2g,\mathbb Z)$-covariance; these require the corresponding maps on the stated coefficient systems.
\end{definition}
...
\input{chapters/theory/edge_contraction_signs}
\begingroup
\let\section\subsection
\let\subsection\subsubsection
\input{chapters/theory/residue_chart_gluing}
\input{chapters/theory/residue_bar_coefficients}
\endgroup

\subsection{Sign compatibility for geometric bar faces}
\label{sec:bar-residue-sign-interface}

\begin{convention}[Determinant and residue signs]
\label{conv:orientations-enhanced}
An algebraic contraction uses the determinant of the remaining labeled edges,
placed before the coefficient complex in degree minus the number of edges.
Its deletion sign is~\eqref{eq:ordered-deletion}.
A logarithmic residue uses the left-normal convention~\eqref{eq:left-residue}.
This residue already includes the same deletion sign.
One does not multiply it by a second copy of that sign.
For a tensor $x\otimes\alpha$ with coefficients first,
a homogeneous map $u\otimes v$ acts with sign $(-1)^{|v||x|}$.
This rule translates the determinant-first convention when factors are reordered.
The coefficient maps must satisfy the identities of
Theorem~\ref{thm:exact-coefficient-criterion} on their stated domains.
\end{convention}

\begin{lemma}[Sign compatibility for ordered bar faces; \ClaimStatusProvedHere]
\label{lem:sign-compatibility}
For an augmented differential graded associative algebra,
\eqref{eq:bar-determinant-map} identifies the desuspended bar differential
with the determinant contraction differential.
For logarithmic forms on a product normal-crossing chart,
\eqref{eq:log-symbol-comparison} identifies the direct sum of determinant terms over all contracted sets
with the normal-logarithmic subcomplex on all product strata.
The source has the global shift $[-2|E|]$, and its edge contractions correspond to left-normal residues.
On a supplied projected atlas, fixed ordered-bar coefficients admit the coherent unfiltered comparison of
Proposition~\ref{prop:bar-residue-descent}.
For the interval coefficient system~\eqref{eq:interval-coefficient-system},
Propositions~\ref{prop:state-residue-complex} and~\ref{prop:interval-normal-comparison}
give the full residue differential, its strict coordinate maps, and the normal embedding.
The logarithmic overlap homotopy for that system requires the stated local logarithms.
\end{lemma}

\begin{proof}
Proposition~\ref{prop:bar-determinant-comparison} computes every unary and binary bar sign.
Equation~\eqref{eq:log-symbol-comparison} computes the logarithmic comparison on each wedge generator.
Each residue lands in the summand indexed by $S\cup\{e\}$, which is present in both direct sums.
Both computations give $(-1)^{i-1}$ for deletion of the $i$th edge.
Their coefficient identities are separate from the determinant comparison,
as stated in Theorem~\ref{thm:exact-coefficient-criterion}.
Lemma~\ref{lem:tensor-residue-homotopies} supplies the additional sign from the degree of an ordered bar word.
For changing interval states, Lemma~\ref{lem:interval-coefficient-squares} identifies the common merged target and proves the coefficient square.
The degree and residue calculations of Proposition~\ref{prop:interval-normal-comparison} then prove the stated comparison.
\end{proof}
...
\begin{theorem}[Nilpotency with compatible collision coefficients; \ClaimStatusProvedHere]
\label{thm:bar-nilpotency-complete}
\index{bar differential!nilpotence}
On a normal-crossing chart, use the full complex $\mathcal R(M)$ and the three
operators of Definition~\ref{def:bar-differential-complete}.
Assume that for every $S\subseteq E$ and distinct $e,f\notin S$ the degree-zero
cochain maps satisfy
\[
\mu_{S\cup\{e\},f}\mu_{S,e}
=\mu_{S\cup\{f\},e}\mu_{S,f}
 :M_S\longrightarrow M_{S\cup\{e,f\}}.
\]
Then $d=d_{\mathrm{internal}}+d_{\mathrm{residue}}+d_{\mathrm{form}}$ satisfies $d^2=0$.
The interval coefficient system satisfies these hypotheses.
\end{theorem}

\begin{proof}
Write $d_1=d_{\mathrm{internal}}$, $d_2=d_{\mathrm{residue}}$,
and $d_3=d_{\mathrm{form}}$, and take a homogeneous input $v\otimes\alpha$
on $U_S$ with $|v|=q$.
There are three square terms and three mixed anticommutators.
The identities $d_1^2=0$ and $d_3^2=0$ follow from $d_{M_S}^2=0$
and $d_{\mathrm{dR}}^2=0$.
For distinct $e,f\notin S$, the two terms of $d_2^2$ are
\begin{align*}
&\mu_{S\cup\{e\},f}\mu_{S,e}(v)\otimes
 r_{S\cup\{e\},f}r_{S,e}\alpha,\\
&\mu_{S\cup\{f\},e}\mu_{S,f}(v)\otimes
 r_{S\cup\{f\},e}r_{S,f}\alpha.
\end{align*}
The tensor signs multiply to $(-1)^{2q}=1$.
The coefficient factors agree by hypothesis, and the residue factors are opposite by
Proposition~\ref{prop:residue-identities}; hence the pair cancels.
There is no repeated-edge term, since $e$ is no longer a remaining component after taking $r_{S,e}$.
Thus $d_2^2=0$.

For the $e$-component of $d_1d_2+d_2d_1$, the two terms are
\[
(-1)^q d_{M_{S\cup\{e\}}}\mu_{S,e}(v)\otimes r_{S,e}\alpha,
\qquad
(-1)^{q+1}\mu_{S,e}(d_{M_S}v)\otimes r_{S,e}\alpha.
\]
They cancel because $\mu_{S,e}$ is a cochain map.
The same degree change of $v$ gives
$d_1d_3+d_3d_1=0$.
For $d_2d_3+d_3d_2$, both tensor signs are $(-1)^{2q}=1$,
so its $e$-component is
\[
\mu_{S,e}(v)\otimes
\bigl(r_{S,e}d_{\mathrm{dR}}+d_{\mathrm{dR}}r_{S,e}\bigr)\alpha=0.
\]
This is again Proposition~\ref{prop:residue-identities}.
All nine terms in the square therefore cancel.
For interval states the common coefficient targets and commuting squares follow from
Lemma~\ref{lem:interval-coefficient-squares}, including adjacent cuts by associativity.
\end{proof}

Applying this theorem to a geometric chiral bar object requires an identification with a
coefficient system of the displayed type, compatible with the internal differential,
residues, and coordinate overlaps.
At an overlapping collision, the Borcherds identity must act on the same merged-state target
as the geometric composites; a formal cyclic expression in OPE coefficients does not establish
that identification.  This comparison is an additional hypothesis, not a conclusion of the theorem.
...
\begin{remark}[Geometric coefficient compatibility]\label{rem:d-squared-synthesis}\label{rem:geometric-miracle}
On the local complex of Theorem~\ref{thm:bar-nilpotency-complete},
nilpotency separates into two statements: the coefficient squares commute,
and the oriented residues anticommute.
For a geometric chiral bar construction, a factorization product on
$\operatorname{Ran}(X)$ must induce these particular coefficient maps,
with their common targets and coordinate compatibility.
The Arnold relation in the Heisenberg example
(\S\ref{sec:frame-bar-deg2}) gives a local residue identity; it does not
by itself construct that general comparison.
\end{remark}

\begin{proposition}[Two-component differential identity; \ClaimStatusProvedHere]\label{prop:pole-decomposition}
...
Let $V=\bigoplus_j V^j$ be a graded complex vector space, and let
$P,Q:V^j\to V^{j+1}$ be homogeneous linear operators.
Write $d=P+Q$ and define their graded anticommutator by
$\{P,Q\}=PQ+QP$.
If $d^2=0$, then
\[
P^2=-\{P,Q\}-Q^2.
\]
In particular, if also $Q^2=0$, then $P^2=-\{P,Q\}$.
These hypotheses do not imply $P^2\ne0$.
\end{proposition}

\begin{proof}
Expanding the square gives
$0=(P+Q)^2=P^2+PQ+QP+Q^2$.
For the last assertion, take $V=\Lambda^\bullet(\mathbf C u\oplus\mathbf C v)$,
where $u,v$ have degree one, and set $P(\alpha)=u\wedge\alpha$ and
$Q(\alpha)=v\wedge\alpha$.
Both operators are nonzero, but $P^2=Q^2=0$ and
$PQ+QP=0$ by the exterior relations.
Thus $d^2=0$ and $P^2=0$ simultaneously.
\end{proof}

\begin{remark}[Pole-order extraction]\label{rem:bicomplex-obstruction}
\index{bar complex!bicomplex structure}
For a vector space $W$ and a finite Laurent principal part
$\eta(t)=\sum_{k\ge -N}w_k t^k\,dt$ with $w_k\in W$,
the residue is the coefficient $\operatorname{Res}_{t=0}\eta=w_{-1}$.
Suppose a local operator product has singular coefficients
$a_{(n)}b\in W$ and is paired with a holomorphic factor
$f(t)=\sum_{j\ge0}f_jt^j$.  Coefficient extraction gives
\[
\operatorname{Res}_{t=0}
 \left(\sum_{n\ge0}(a_{(n)}b)t^{-n-1}f(t)\,dt\right)
 =\sum_{n\ge0}f_n(a_{(n)}b).
\]
For a fixed pair $a,b$, only finitely many singular OPE terms are present,
so this sum is defined without a completion.
The simple pole contributes $f_0a_{(0)}b$; the double pole contributes
$f_1a_{(1)}b$.  Thus a double pole contributes through a derivative of
the regular factor, rather than through the residue of $t^{-2}dt$ alone.
Higher poles contribute higher Taylor coefficients.

For affine current generators, one has
$J^a_{(0)}J^b=J^{[a,b]}$ and
$J^a_{(1)}J^b=k\kappa(a,b)\mathbf1$, with no higher singular terms
for that pair.  This permits a local separation into a bracket term and
a scalar term on current-generator inputs.  Extending this separation
to all states and all collision strata requires additional maps and
compatibility identities.  A general chiral OPE can have more than two
pole orders.

If such a geometric construction supplies degree-one operators
$d_{\mathrm{bracket}},d_{\mathrm{curvature}}$ on a common graded carrier,
and their sum exhausts its residue differential, then
\begin{equation}\label{eq:pole-decomposition}
d_{\mathrm{res}}=d_{\mathrm{bracket}}+d_{\mathrm{curvature}}.
\end{equation}
Under the additional hypotheses $d_{\mathrm{res}}^2=0$ and
$d_{\mathrm{curvature}}^2=0$, Proposition~\ref{prop:pole-decomposition} yields
\[
d_{\mathrm{bracket}}^2
=-\{d_{\mathrm{bracket}},d_{\mathrm{curvature}}\}.
\]
Consequently, if the anticommutator vanishes, the two operators are
square-zero and anticommute.  Under the stated hypotheses, this condition
is equivalent to $d_{\mathrm{bracket}}^2=0$.
Nonvanishing requires a separate example or calculation on the specified carrier.

A bicomplex requires a further grading.  A bigrading refining the total
grading is a decomposition
\[
V^n=\bigoplus_{p+q=n}V^{p,q}.
\]
It is compatible with the two operators if
\[
d_{\mathrm{bracket}}:V^{p,q}\longrightarrow V^{p+1,q},
\qquad
d_{\mathrm{curvature}}:V^{p,q}\longrightarrow V^{p,q+1}.
\]
With such a bigrading, the square-zero and anticommutation identities
make $V^{\bullet,\bullet}$ a bicomplex, using the convention that its
two differentials anticommute.  The operator identities alone do not
supply this decomposition.

Indeed, take $V^0=\mathbf C a$, $V^1=\mathbf C b$, and $V^n=0$ otherwise,
and define degree-one operators $P,Q$ by
\[
P(a)=Q(a)=b,\qquad P(b)=Q(b)=0.
\]
Both composites vanish, so
$P^2=Q^2=PQ+QP=(P+Q)^2=0$.
If a compatible bigrading existed, the one-dimensional space $V^0$
would be concentrated in some bidegree $(p,q)$ with $p+q=0$.
Its nonzero image $b=P(a)=Q(a)$ would then belong both to
$V^{p+1,q}$ and to $V^{p,q+1}$.
These are distinct summands of a direct sum, whose intersection is zero.
This contradicts $b\ne0$, proving that even this pair of anticommuting
square-zero operators need not admit a compatible bigrading.

The assertion $d_{\mathrm{res}}^2=0$ follows from
Theorem~\ref{thm:bar-nilpotency-complete} only after its coefficient and
residue hypotheses have been established for this carrier.
Likewise, replacing one input by the vacuum only addresses a subsequent
collision involving that input.  Disjoint collisions still require their
own cancellation; vacuum annihilation alone does not prove
$d_{\mathrm{curvature}}^2=0$.
At a triple collision, the relevant proposed compatibility has the form
\begin{equation}\label{eq:borcherds-pole-mix}
\sum_{\mathrm{cyclic}}\operatorname{Res}_{D_{j\ell}}
 \operatorname{Res}_{D_{ij}}
 \bigl[\mu(\mu(\phi_i,\phi_j),\phi_\ell)\otimes\omega\bigr]=0.
\end{equation}
Here the iterated products must first be defined as maps into a common
merged-state space, with the displayed residues acting on its form factor.
The Borcherds identity for local OPE coefficients does not alone supply
these geometric maps or their coordinate identifications.
\end{remark}
...
\begin{corollary}[Residues anticommute at corners; \ClaimStatusProvedHere]
\label{cor:residues-anticommute}
For transverse labeled divisors on a smooth complex manifold,
left-normal residues satisfy the exact identity
\[
r_{S+e,f}r_{S,e}+r_{S+f,e}r_{S,f}=0.
\]
Both composites map logarithmic $q$-forms on $D_S$ to
logarithmic $(q-2)$-forms on $D_{S+e+f}$.
\end{corollary}
\begin{proof}
The coefficient of $d\log x_e\wedge d\log x_f$ changes sign
when these two normal forms are exchanged.
Restriction to $x_e=x_f=0$ is independent of order.
This is Proposition~\ref{prop:residue-identities}.
\end{proof}
...
\begin{remark}[Orientation and edge signs]
The smooth complex configuration space has a canonical real orientation.
Its edge determinant $L(E)$ is a separate graded line,
whose signs record the order of collision contractions.
A further coefficient line at positive genus requires its own definition and compatible contraction maps.
It is not supplied by the real orientation of the configuration space.
\end{remark}
...
\begin{lemma}[Orientation of a corner; \ClaimStatusProvedHere]
\label{lem:orientation}
At a transverse intersection of two boundary hypersurfaces of an oriented real manifold with corners,
the two induced boundary orientations are opposite.
For a simple normal crossing divisor in a complex manifold,
the analogous sign occurs in the ordered wedge of its logarithmic normal forms.
\end{lemma}
\begin{proof}
Let $n_e,n_f$ be outward normal vectors.
The two boundary conventions write the ambient orientation as
$n_e\wedge n_f\wedge\omega_{ef}$ and
$n_f\wedge n_e\wedge\omega_{fe}$.
Equality implies $\omega_{ef}=-\omega_{fe}$.
For logarithmic forms, interchange $d\log x_e$ and $d\log x_f$ instead.
Their wedge changes sign once.
The real outward-normal convention and the logarithmic residue convention
have distinct domains, and each gives the stated two-order comparison.
\end{proof}

\begin{lemma}[Residue properties; \ClaimStatusProvedHere]
\label{lem:residue-properties}
On the direct sum of logarithmic forms over labeled strata,
the component $r_e$ is zero after $e$ has been removed.
It satisfies $r_e^2=0$ and $r_er_f+r_fr_e=0$ for distinct components.
It also satisfies $r_ed_{\mathrm{dR}}+d_{\mathrm{dR}}r_e=0$.
\end{lemma}
\begin{proof}
The first assertion follows from the source and target of $r_{S,e}$.
The other assertions are~\eqref{eq:residue-relations}.
These identities concern geometric residues.
Coefficient contractions obey the separate identities in~\eqref{eq:coefficient-defects}.
\end{proof}

\begin{lemma}[Well-definedness of residue; \ClaimStatusProvedHere]
\label{lem:residue-well-defined}
The map
$r_{S,e}:\Omega^q_{D_S}(\log D^S)\to
\Omega^{q-1}_{D_{S+e}}(\log D^{S+e})$
is independent of the local equation of the component.
\end{lemma}
\begin{proof}
Replacing $x_e$ by $u x_e$, with $u$ invertible,
adds the form $d\log u$, which is regular along $D_e$.
It therefore leaves the coefficient of the normal logarithmic form unchanged after restriction.
The local construction preceding Proposition~\ref{prop:residue-identities}
also proves independence of the representative.
\end{proof}
...
\begin{example}[A principal-stage factorization comparison]
\label{ex:winfty-completion-frontier}
The inverse-limit carrier $\varprojlim_N W_N$ requires specified transition maps.
To identify it with a factorization target in Definition~\ref{prop:winfty-factorization-package}, one must prove compatibility of the actual collision operations, including identities of the form
\[
 \mathsf{C}^{\mathrm{res}}_{s,t;u;m,n}(N)
 =\mathsf{C}^{\mathrm{DS}}_{s,t;u;m,n}(N).
\]
A finite set of such identities gives a comparison only if its finite-detection theorem applies to those carriers and operations.
The higher-spin deletion rule already fails when it kills a field with nonzero scalar self-product (Proposition~\ref{prop:cbc28-truncation}).
Thus constructing suitable transitions is a prior requirement for the proposed inverse-limit comparison.
\end{example}
...
\begin{definition}[A principal-stage factorization target]
\label{prop:winfty-factorization-package}
A principal-stage factorization target consists of a complete filtered factorization algebra $F$, specified finite-stage quotients, and compatible bar--cobar maps on those quotients.
Its separated inverse limit must come with a quasi-isomorphism to the proposed completed vertex carrier.
These conditions include the transition maps and their compatibility with all vacuum products.
Proposition~\ref{prop:cbc28-truncation} excludes transition maps that simply delete the weight-three generator of $\mathcal W_3$ at nonzero central charge.
Thus a construction of this target requires different transition data and a proof controlling its derived inverse limit.
\end{definition}

\begin{remark}[Transition maps and vacuum terms]
...
Definition~\ref{prop:winfty-factorization-package}, and let
