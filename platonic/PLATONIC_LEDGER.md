...
`make` → `out/platonic.pdf` — **595 pages, 0 LaTeX errors, 0 undefined
references, 0 undefined citations.** Default target: a plain build produces the
current, merged monograph.

## This repository

**Volume I — Ordered Chiral Geometry**, placed first. This repository additionally carries the *detailed* Volume I treatment (the 104-page reconstruction), integrated into the same document as a final part, so a plain `make` builds both the spine's condensed Volume I and its long form in one monograph.
