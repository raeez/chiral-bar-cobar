\chapter{Enveloping algebras and the exact audit of the legacy tables}
...
Verified in \texttt{compute/tests/test\_witt\_pentagonal\_rigidity.py}
and \texttt{compute/lib/witt\_pentagonal\_rigidity.py}.
...
\section{The legacy planar sequences}
...
\begin{remark}[Three chain models had been mixed]
\label{rem:three-models}
The legacy tables conflated the interval bar $\Barperp(A)$, whose
...
$b_A+d_A+d_{\mathrm{scr}}+\delta_{\mathrm{int}}$. Only the third can carry
both bar words and configuration-space cohomology, and only after
