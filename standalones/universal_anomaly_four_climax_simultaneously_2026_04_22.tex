...
canonicality, the Vol.~III principal $K3\times E$ chiral bialgebra
$\mathbf H_{\Delta_5}$, and the derived-centre Mukai conductor trace
...
Vol.~III principal $K3\times E$ chiral bialgebra $\bfH_{\Delta_5}$,
and with the Bruinier--Mukai conductor trace identity
...
The theorem-grade Volume~III input is a principal-locus
...
the Mukai-principal locus and $E$ an elliptic curve. Volume~III
identifies the stage-two chiral specialisation of the CY3 object
$\mathrm{Perf}(Y)$ with the K3 chiral bialgebra $\bfH_{\Delta_5}$ on
the elliptic factor.

\begin{theorem}[$\bfH_{\Delta_5}$ canonical, Volume~III; \ClaimStatusProvedElsewhere]
\label{thm:climax-III}
On the principal Mukai locus there is an equivalence of $E_1$-chiral
bialgebras on $E$
\[
...
The proof (Volume~III, §6--§8) pins stage one by Kontsevich--Tamarkin
$E_3$-formality and Costello--Gwilliam--Li locality under the
principal-locus hypotheses, and stage two by Ayala--Francis
factorisation-homology Fubini (Ayala--Francis 2015, Theorem~3.16).
...
BL-comparison data of Theorem~\ref{thm:climax-III} provides a
quasi-isomorphism of the Atiyah--Connes obstruction complex with the
$\mathbf H_{\Delta_5}$ witness complex on the principal locus. Part
...
\emph{Double vanishing.} The cocycle $[m_3, B^{(2)}]_{\cH_1} = 0$ vanishes from two independent mechanisms. The toy analogues of the four bridge vertices are unobstructed: Theorem~B is Feigin--Frenkel bar-cobar inversion (the $\sfG$-row archetype), the partition function reduces to the free-boson character, $\bfH_{\Delta_5}$ degenerates to the abelian chiral bialgebra at $K = 1$, and the normalized trace identity $\hbar^2 \cdot K = -1$ gives $\hbar^2 = -1$ on the rank-$1$ sector.
