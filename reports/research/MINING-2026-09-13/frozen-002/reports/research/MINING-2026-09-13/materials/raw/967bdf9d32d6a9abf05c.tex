...
An explicit resolution keeps the witnesses for these two equations.
Let
\[
...
\section{Restricting a transverse coordinate retains a witness}
...
The equation $y=0$ can instead retain its witness, by tensoring with
$k[y]\otimes\Lambda(e_y)$, $de_y=y$.
...
This is an explicit shift in degree, obtained by retaining the
equation's witness.
...
\section{The elementary matrix brane retains its derived equation}

The two holomorphic coordinates also enter the algebra of a point
brane. At matrix rank $N\ge1$, introduce two $N\times N$ matrices
...
\]
Retain a degree-minus-one witness $\psi^i_j$ for each entry.
...
This differential algebra defines the derived commuting-pair equation.
The word ``derived'' here specifies this algebra of equations and
witnesses, including its negative-degree cohomology.
...
It links the witnesses of Chapter~\ref{chap:relations-homotopies}
to the actual brane equation in two holomorphic coordinates.
The principal-part pairing supplies its possible cotangent
coefficients. Constructing the corresponding field operations requires
a differential and a pairing on fields with their spatial dependence
retained.
