...
The following questions concern individual cohomology dimensions, new algebraic families, and structural constructions.

\subsection{Individual bar cohomology dimensions}
...
The value $1352$ would confirm the rational GF (Conjecture~\ref{conj:sl3-bar-gf}) and the shared discriminant with $\mathcal{W}_3$; any other value would refute the rational form and require reconsidering the algebraicity class.
...
\emph{Casimir decomposition} (Computation~\ref{comp:sl3-casimir-decomp}):
...
Related dimensions and obstructions:
\begin{itemize}
\item
The Koszul dual dimensions from series inversion are
$(A^!)_n = [1, 8, 28, 140, 392, 2884, \ldots]$
...
\subsection{Bar cohomology for additional families}
...
cohomology data.  The two methods are direct rank
...
detection).  A finite rank computation concerns the displayed finite complex;
it does not by itself identify that complex with a geometric bar construction.
...
\subsection{Further cohomology questions}
\label{subsec:updated-frontier}

The rank identities and counterexamples above give obstructions
for the individual cohomology questions of \S\ref{sec:open-problems}.
...
A full rank computation requires the degree-$5$ differential.
...
(double-pole OPE contributions), whose chain-level construction for the stated $E_1$-chiral carrier is still required.
...
\noindent
The seven identities above are consistent with one another.
