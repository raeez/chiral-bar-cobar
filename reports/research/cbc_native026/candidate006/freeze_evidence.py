...
source=root/'research-candidates/cbc_native026/candidate006'
...
(report/'render-binding.json').write_text(json.dumps({'pdfs':[{k:r[k] for k in ['kind','pdf','pdf_sha256','pages']} for r in closure],
    'rasters':renders,'inspection_scope':'All eight subregular pages and the listed focused/native integration pages. No whole-native render acceptance.'},indent=2)+'\n')
...
(report/'final-manifest.json').write_text(json.dumps({'source_manifest_sha256':sha(report/'source-manifest.json'),
    'mathematical_status':'New construction awaits independent acceptance; whole native union remains unaccepted.',
