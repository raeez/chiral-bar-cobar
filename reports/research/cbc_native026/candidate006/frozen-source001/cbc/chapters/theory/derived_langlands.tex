...
At an admissible level, the current double-pole coefficient is
$k\kappa(a,b)\mathbf1$, where $\kappa$ is the normalized invariant form.
The mode bracket obtained by testing this coefficient is
$km\delta_{m+n,0}\kappa(a,b)K$ in~\eqref{eq:km-mode-bracket}.
Its Chevalley--Eilenberg differential squares to zero at every level
(Theorem~\ref{thm:km-bar-bicomplex}).  Thus this central term alone does not
establish a nonzero square of an unreduced chiral bar differential.
Such a conclusion requires a specified curved carrier and the actual
coefficient comparison of Remark~\ref{rem:bicomplex-interpretation}.
The Sugawara central charge $k\dim(\mathfrak g)/(k+h^\vee)$ and a curvature
operator are separate constructions from the current central extension.
...
\end{enumerate}
Applying a curved analogue of this definition to a chiral bar object
requires the graded coalgebra, its coderivation, and its curvature map.
Proposition~\ref{prop:pole-decomposition} gives only an identity for two
operators on a common graded space; it does not construct these data.
In particular, a central element in an algebra has zero commutator even
when it is nonzero, so the level coefficient alone cannot imply a
nonzero curvature operator.
\end{definition}
...
conjectural Kazhdan--Lusztig recovery on the semisimplified target
...
semisimplified target is an outer MC3 problem.  Without periodicity,
