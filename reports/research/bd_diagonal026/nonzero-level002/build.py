...
command = ['/Library/TeX/texbin/pdflatex', '-no-shell-escape', '-halt-on-error',
           ...
           f'-output-directory={BUILD}', 'nonadjacent-source.tex']
passes = []
previous = None
for number in range(1, 6):
    ...
    state = b''.join((BUILD / f'nonadjacent-source.{suffix}').read_bytes()
                     for suffix in ('aux', 'toc', 'out')
                     if (BUILD / f'nonadjacent-source.{suffix}').exists())
