...
command = ['/Library/TeX/texbin/pdflatex', '-no-shell-escape', '-halt-on-error',
           ...
           f'-output-directory={BUILD}', 'ternary-comparison.tex']
passes = []
previous = None
for number in range(1, 6):
    ...
    state = b''.join((BUILD / f'ternary-comparison.{suffix}').read_bytes()
                     for suffix in ('aux', 'toc', 'out')
                     if (BUILD / f'ternary-comparison.{suffix}').exists())
