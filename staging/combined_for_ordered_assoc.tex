...
\medskip
\emph{RTT at degree $1$.}
$[t_{11}^{(1)}, t_{12}^{(1)}] = \Psi\, t_{12}^{(1)}$,
$[t_{11}^{(1)}, t_{21}^{(1)}] = -\Psi\, t_{21}^{(1)}$,
$[t_{12}^{(1)}, t_{21}^{(1)}]
= \Psi\,(t_{11}^{(1)} - t_{22}^{(1)})$.

\medskip
\emph{Quantum determinant.}
$\mathrm{qdet}\, T(u)
= t_{11}(u)\, t_{22}(u - \Psi)
- t_{12}(u)\, t_{21}(u - \Psi)$ is central.

\medskip
\emph{Drinfeld coproduct.}
$\Delta_z(t_{ij}(u))
= \sum_{k=1}^{2} t_{ik}(u) \otimes t_{kj}(u-z)$.
At $N = 1$ this is scalar, recovering
Theorem~\ref{thm:w-infty-chiral-qg}.

\medskip
\emph{Identification.}
$\cW_2 = \mathrm{Vir}_c$ with
$c = 1 - 6(\Psi - 1)^2/\Psi$,
