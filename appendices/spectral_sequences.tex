...
\]
The FM compactification $\FM_2(\bA^1) \cong \bA^2$ (since for a curve the diagonal is
codimension~1, so the blow-up is trivial), with boundary divisor
...
\emph{(i)} If $c = 0$, the genus-1 correction vanishes: the quantum correction to the Arnold relation at genus 1 is proportional to $c \cdot E_2(\tau)$, so $d_1 = 0$ when $c = 0$. With $d_1 = 0$, we have $E_2 = E_1$ and no further differentials can produce nonzero contributions at genus $\geq 2$ (since all higher corrections are built from iterations of the genus-1 propagator, each carrying a factor of $c$). Thus the spectral sequence degenerates at $E_1$.
