...
This appendix operates at \textbf{M-level}: spectral sequence
machinery for computing the cohomology of the explicit chain models
(bar, Chevalley--Cousin, genus filtration).
\end{remark}

The governing question of this appendix is: which filtrations and
convergence arguments are needed to convert bar-level constructions into
controlled cohomological output.
...
\begin{theorem}[Identification of early pages; \ClaimStatusProvedHere]
...
\]
Moreover, the spectral sequence is \emph{regular}: $E_r = E_\infty$ for 
...
\begin{proposition}[First quadrant spectral sequences; \ClaimStatusProvedHere]
...
\begin{theorem}[Zeeman comparison theorem; \ClaimStatusProvedHere]
\label{thm:zeeman}
...
\begin{proposition}[Central charge and $d_1$; \ClaimStatusProvedHere]
...
\emph{(i)} If $c = 0$, the genus-1 correction vanishes: the quantum correction to the Arnold relation at genus 1 is proportional to $c \cdot E_2(\tau)$, so $d_1 = 0$ when $c = 0$. With $d_1 = 0$, we have $E_2 = E_1$ and no further differentials can produce nonzero contributions at genus $\geq 2$.  Indeed, by Theorem~\ref{thm:genus-universality}, all genus-$g$ corrections are proportional to $\kappa(\cA) = c/2$, so $c = 0$ implies all quantum corrections vanish simultaneously.  Thus the spectral sequence degenerates at $E_1$.
...
\begin{theorem}[Chevalley--Cousin spectral sequence \cite{Har77, KashiwaraSchapira}; \ClaimStatusProvedElsewhere]
...
\begin{theorem}[Cousin resolution for holonomic D-modules \cite{KashiwaraSchapira}; \ClaimStatusProvedElsewhere]
...
\begin{proposition}[Bar spectral sequence is multiplicative \cite{LodayVallette}; \ClaimStatusProvedElsewhere]
