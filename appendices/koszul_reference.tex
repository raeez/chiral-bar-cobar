...
The genus-graded structure of Koszul duality has physical manifestations in several contexts; the most developed is the identification with the genus expansion of string amplitudes (Chapter~\ref{ch:genus-expansions}), where the spectral sequence pages correspond heuristically to loop order.  The other anticipated connections---to electric-magnetic duality, open-closed string duality, and mirror symmetry---are discussed in the connections chapters but remain at the level of structural analogies.
...
The key point is that a curved $A_\infty$-algebra carries \emph{two}
differentials that must be carefully distinguished:
\begin{itemize}
\item The \emph{bar differential} $d_B$ on the tensor coalgebra
$\bar{B}(\mathcal{A}) = (T^c(s^{-1}\bar{\mathcal{A}}),\, d_B)$,
which encodes all the operations $m_0, m_1, m_2, \ldots$
simultaneously.  The condition $d_B^2 = 0$ is \emph{equivalent} to
...
differential.  The bar differential $d_B$ on $\bar{B}(\mathcal{A})$
nonetheless satisfies $d_B^2 = 0$; this is guaranteed by the full
system of curved $A_\infty$ relations (including the arity-$1$
relation above), which are \emph{equivalent} to $d_B^2 = 0$.  So the
...
\to s^{-1}\mathcal{A}$.  The bar condition $d_B^2 = 0$ always holds
