...
quadratic candidate to the full Koszulness theorem omits the endpoint
...
dg Lie algebras; (c) the bar--cobar counit as a quasi-isomorphism on the
...
candidate + MC quasi-isomorphism + bar--cobar counit on the comparison
...
\begin{theorem}[Essential image of the chiral bar-dual coalgebra; \ClaimStatusConditional]\label{thm:essential-image-koszul}
...
\begin{theorem}[Bar-dual coalgebras are geometrically representable; \ClaimStatusConditional]\label{thm:koszul-geom-rep}
If
$\widehat{\mathcal{C}}\simeq
\mathcal{A}^{\mathrm i}=\barB^{\mathrm{ch}}(\mathcal{A})$
...
From the displayed curvature identity, two regimes emerge:
