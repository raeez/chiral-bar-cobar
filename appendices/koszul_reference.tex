...
\S\ref{app:koszul_higher_genus} (\texttt{koszul\_across\_genera.tex}).
...
\begin{proof}
This is Theorem~\ref{thm:genus-graded-mc} in \S\ref{app:koszul_higher_genus}.
...
We classify chiral algebras by their Koszul duality
...
\emph{Key.}
\begin{itemize}
\item \emph{Type.} Quadratic / Curved / Filtered / General
\item \emph{$\dim(V)$.} Dimension of generating space
\item \emph{Completion?.} Whether nilpotent completion is needed
\item \emph{Koszul dual exists?.} Whether $\mathcal{A}^!$ is well-defined
\end{itemize}
...
The essential image question decomposes into four subproblems:
\emph{recognition} (given a coalgebra, determine whether it is a Koszul dual),
\emph{completeness} (whether every coalgebra arises),
\emph{uniqueness} (whether $\mathcal{A}$ is determined by $\mathcal{A}^!$), and
\emph{reconstruction} (recovering $\mathcal{A}$ from $\widehat{\mathcal{C}}$).
\end{remark}
