...
\begin{theorem}[Genus-graded Koszul duality theorem; \ClaimStatusProvedElsewhere]
...
locally finite-dimensional, and with central curvature, then the classical Koszul dual
\[\mathcal{A}^! := \bigoplus_{g \geq 0} \operatorname{Ext}_{\mathcal{A}^{(g)}}^*(\mathbb{k}, \mathbb{k})\]
is also genus-graded Koszul, and $(\mathcal{A}^!)^! \cong \mathcal{A}$.

\smallskip\noindent\textit{Note.}
The formula $\mathcal{A}^! = \bigoplus_g \operatorname{Ext}^*_{\mathcal{A}^{(g)}}(\mathbb{k},\mathbb{k})$
is the \emph{classical} (BGS) Koszul dual, valid when each $\mathcal{A}^{(g)}$ is a
classical quadratic algebra.  For the chiral Koszul dual (Definition~\ref{def:chiral-koszul-pair}),
the bar construction replaces $\operatorname{Ext}$; the two coincide on the classical locus but
diverge for general vertex algebras (see Remark~\ref{rem:classical-vs-chiral-koszulness}).
\end{theorem}
...
\begin{lemma}[Genus-graded Koszul complex resolution; \ClaimStatusProvedElsewhere]
...
The genus-graded Koszul duality corresponds to the genus expansion of string amplitudes (Chapter~\ref{ch:genus-expansions}); connections to electric-magnetic duality, open-closed string duality, and mirror symmetry appear in the connections chapters.
