...
Branch-line reductions are surgical tools.  The full modular
Maurer--Cartan element $\Theta_\cA$ lives in an infinite-dimensional
convolution algebra, but its essential content is often concentrated
on configurations of minimal complexity: two-point branch lines where
a single modular class becomes visible, rank-one spectral sectors
where a primitive shadow can be isolated, genus-$2$ transparency
windows where all higher corrections vanish.  A branch-line reduction
isolates such a configuration and proves structural theorems inside
the resulting finite-dimensional quotient.

The key objects are the \emph{primitive modular characteristics}:
the irreducible pieces of a completed Maurer--Cartan element that
cannot be decomposed further under the clutching maps of
$\overline{\mathcal{M}}_{g,n}$.  They are to the shadow Postnikov
tower what primitive elements are to a Hopf algebra --- the
generators from which everything else is assembled by
multiplication and coproduct.  The intrinsic invariant ladder
$\kappa \rightsquigarrow \Delta \rightsquigarrow \Theta$ organizes
these primitives by complexity: $\kappa$ is the scalar primitive
(arity~$2$), $\Delta$ is the first spectral primitive (the
discriminant), and the full tower of primitives is the
content of~$\Theta_\cA$
(Theorem~\ref{thm:mc2-bar-intrinsic}).

This chapter reorganizes a thread of frontier calculations around the
constitutional invariant ladder
\[
\kappa(\cA)
\rightsquigarrow
\Delta_{\cA}(x)
...
Second, the scalar-saturated class \(\Theta_{\cA}\) for
one-channel families is available by
Theorems~\ref{thm:universal-theta} and~\ref{thm:mc2-full-resolution};
...
\item For the $\beta\gamma$ system, the
  ...
  vanishes.  At this locus the shadow tower truncates from
  arity~$4$ to arity~$2$, and the residual modular
  characteristic is the scalar~$\kappa(\beta\gamma)
  = -2$, matching the Heisenberg value at rank~$1$.
...
\]
In other words, the proved spectral potential is the one-loop
