...
\(\eta\otimes\Gamma_{\cA}\); on the proved uniform-weight lane
(Definition~\ref{def:scalar-lane}) this further
specializes to \(\kappa(\cA)\eta\otimes\Lambda\).
The non-scalar content is organized by the shadow
obstruction tower (Definition~\ref{def:shadow-postnikov-tower}),
with finite-order shadows proved through degree~$4$.
...
\item For the $\beta\gamma$ system, the
 ...
 degree~$4$ to degree~$2$, and the residual modular
...
are not auxiliary notational devices. They are exact quotient faces of
...
\begin{proposition}[The scalar shadow is an abelian MC element; \ClaimStatusConditional]
...
abelian dg Lie algebra \(\mathfrak{s}_{\cA}\). It is exactly
...
\begin{theorem}[Spectral cumulant hierarchy; \ClaimStatusConditional]
...
Maurer--Cartan element of~\(L\). If in addition
\[
...
The full lift is supplied by the preceding construction, so
Corollary~\ref{cor:filtered-lift-vanishing} is not needed to prove
\(\Theta_{\cA}\). The corollary is diagnostic: any finite or spectral
...
\begin{corollary}[The genus-\(3\) primitive coefficient; \ClaimStatusConditional]
...
\begin{corollary}[Universal coefficients on the shared sheet; \ClaimStatusConditional]
...
The Heisenberg frame example in Part~\ref{part:bar-complex} shows the opposite extreme from
...
leaves two possible positions for the first new terms: the same depth
