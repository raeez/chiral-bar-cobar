...
This appendix operates at \textbf{H-level}: coderived and
contraderived categories are the $\infty$-categorical replacements
for derived categories in the curved setting ($d^2 \neq 0$).  The
CDG-algebra definitions are M-level input; the categorical framework
is the H-level target.
\end{remark}

The governing question of this appendix is: once higher-genus bar
models are curved, which derived-like categories still encode the
correct homotopy content without collapsing to zero.
...
$\mathsf{D}^{\mathrm{co}}_{\mathrm{prov}}(X)$ is conjectured
(\ClaimStatusConjectured) to embed fully faithfully into the eventual coderived category of curved
