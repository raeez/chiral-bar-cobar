...
The governing question of this appendix is why curved bar objects force coderived rather than ordinary derived categories, and exactly which replacements the monograph actually uses.
\end{remark}
...
\emph{$\cA^!$-modules} (the Koszul dual algebra).  That
identification requires the sectorwise grading because the
algebra-coalgebra Koszul duality operates sector by sector.
\end{remark}

\begin{conjecture}[Off-Koszul bar-cobar inversion on Ran;
\ClaimStatusConjectured]
\label{conj:off-koszul-ran-inversion}
...
\begin{enumerate}[label=\textup{(\roman*)}]
\item \emph{Abstract precedent.}  For CDG-coalgebras over a
  field, the bar-cobar counit is always a coderived equivalence
  (Positselski~\cite[Theorem~7.2]{Positselski11}), without any
  Koszulness hypothesis.
\item \emph{On-Koszul case.}
  Theorem~\ref{thm:higher-genus-inversion} establishes inversion
  on the Koszul locus, where
...
\item \emph{Spectral comparison.}
  ...
  $E_1$ pages on and off the Koszul locus; the obstruction to
  convergence lives in higher differentials controlled by the
  curvature.
\item \emph{Completed Yangian case.}
  The completed bar-cobar equivalence for Yangian towers
  (Corollary~\ref{cor:completed-bar-cobar-yangian}) provides
  a family of off-Koszul examples where inversion holds in the
  inverse limit.
\end{enumerate}
The conjecture is the precise formulation of structural gap~H1
