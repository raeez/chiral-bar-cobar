...
\item \emph{Chiral Hochschild cohomology.}
 ...
 The definition fixes the derived object.  A family datum
 $H_H(\cA;S)$ supplies the separate Theorem~H support statement
 $\operatorname{Supp}\ChirHoch^\bullet(\cA)\subseteq S$.
 PBW gives its associated-graded input; the finite-window retract,
 OPE-residue lift, completion, and averaging comparison determine~$S$.
...
The chiral entry of Proposition~\ref{prop:hochschild-crosswalk} is
governed by the family datum $H_H(\cA;S)$ of Theorem~H.  This datum
consists of a complete chart model, its quasi-isomorphism to
$RHH_{\mathrm{ch}}(\cA)$, and a strong deformation retract onto
$K_{\cA,S}$, and therefore gives
\[
 \operatorname{Supp}\ChirHoch^\bullet(\cA)\subseteq S.
\]
PBW chiral Koszulness supplies the associated-graded quadratic
contraction; the OPE-residue lift, incidence compatibility,
completion, and averaging comparison determine~$S$.  The
positive-depth complex $\mathrm{KD}_{\mathrm H}^{\bullet}(\cA)$ of
Definition~\ref{def:koszul-defect-complex} records the remaining
collision columns.  Bakalov--De Sole--Kac give the benchmark support
$\{0,2,3\}$ for Virasoro and dimensions $(2,1,0,\ldots)$ for the
rank-one even superboson, transported to the curve chart by a
bounded-to-chart quasi-isomorphism.  A perfect degree-$d$ pairing is
a separate map and supplies the corresponding degree reflection.
\end{remark}
