...
\item[(iii)] \emph{r-matrix bridge.} The trace-form current-algebra residue
$r_{\mathrm{tr}}(z)=k\,\Omega_{\mathrm{tr}}/z$ and the KZ connection
residue $r_{KZ}(z)=\Omega/((k+h^\vee)z)$ are different normalised
representatives. With $\Omega=2h^\vee\Omega_{\mathrm{tr}}$ one has
$r_{KZ}(z)=2h^\vee\Omega_{\mathrm{tr}}/((k+h^\vee)z)$, so equality with
$r_{\mathrm{tr}}(z)$ is not an identity in~$k$; only the trace form
manifests the level-prefix vanishing $r_{\mathrm{tr}}\big|_{k=0}=0$.
...
(iii) The normalised Killing form satisfies
$(\cdot,\cdot)=2h^\vee(\cdot,\cdot)_{\mathrm{tr}}$, so the Casimirs
satisfy $\Omega=2h^\vee\,\Omega_{\mathrm{tr}}$. Substitution into
$r_{KZ}$ gives the displayed expression. The trace-form residue and
the KZ residue therefore compare by a normalization bridge; they are
not equal rational functions of the level.
...
The Vol II companion appendix (Appendix~\ref*{V2-app:q-conventions-v2}, to be added to \texttt{chiral-bar-cobar-vol2}) records the parallel statement at the level of the bar complex $B^{\mathrm{ord}}(\widehat{\mathfrak{g}}_k)$: the half-monodromy $q_{KL}^\Omega$ is the braiding of ordered bar chains under swap of two adjacent chiral inputs, while the full monodromy $q_{DK}^\Omega$ is the class of a pure braid in $\pi_1(\mathrm{Conf}_2(\mathbb{C}))$.
\end{remark}
