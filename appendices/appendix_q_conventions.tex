...
\item[(iii)] \emph{r-matrix bridge.} The trace-form r-matrix $r_{\mathrm{tr}}(z) = k\,\Omega_{\mathrm{tr}}/z$ and the KZ r-matrix $r_{KZ}(z) = \Omega/((k+h^\vee)z)$ are related by the rescaling $\Omega = 2h^\vee\,\Omega_{\mathrm{tr}}$ of Casimirs between the normalised Killing and trace forms, not by equality as rational functions in $z$; only the trace form manifests the level-prefix vanishing $r_{\mathrm{tr}}\big|_{k=0} = 0$ required by the trace-form convention at the abelian locus.
