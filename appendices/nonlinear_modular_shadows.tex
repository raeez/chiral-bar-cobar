...
consumes shadow data at exactly two points:
\begin{enumerate}[label=\textup{(\roman*)}]
\item \emph{The propagator $r(z) = \tau|_{\deg 2}$.}
  The degree-$2$ bar propagator is the scalar shadow
  $\kappa(\cA) \cdot \eta/z$ in the free/$\beta\gamma$ case
  and $\Omega/z$ in the affine case
  (Propositions~\ref{prop:free-propagator-matching}
  and~\ref{prop:affine-propagator-matching}).
  The higher shadows (cubic~$\mathfrak{C}$, quartic~$\mathfrak{Q}$,
  resonance tower~$\mathfrak{R}^{\mathrm{mod}}_\bullet$) do
  \emph{not} enter the genus-$0$ bridge statement.
  They become relevant only at the modular completion level
  (Conjecture~\ref{conj:modular-yang-baxter}).
\item \emph{The curvature $\kappa(\cA)$.}
  The scalar curvature controls the genus-$1$ deformation of the
  dg-shifted Yangian and the obstructedness of the bridge at
  higher genus.  The bridge at genus~$0$ uses $\kappa$ only
  through its role in the bar differential $d^2 = \kappa \cdot
  \omega_1$ (Theorem~\ref{thm:genus1-universal-curvature}).
\end{enumerate}
\emph{Consequence.}
The nonlinear shadow tower ($\kappa \to \Delta \to \mathfrak{C}
\to \mathfrak{Q} \to \cdots$) is a valuable parallel construction,
but it is \textbf{not a prerequisite} for the Bridge Theorem.
The bridge requires only the degree-$2$ bar propagator (a
genus-$0$ datum) and the scalar curvature $\kappa$ (a genus-$1$
datum).  The higher shadows become prerequisites only when the
programme reaches the modular completion of the bridge---the
all-genera extension governed by the stable-graph Yang--Baxter
equation (Conjecture~\ref{conj:modular-yang-baxter}).

This clarifies the logical dependency: the shadow calculus
\emph{serves} the bridge by providing the input data
$r(z)$ and $\kappa$, but the bridge does not
\emph{require} the full tower to close.
\end{remark}
