...
\begin{proof}
By homogeneity, $E(g) = n \cdot g$ for $g \in \operatorname{Sym}^n$.
Since $\nabla_H = 2E$ on the cyclic deformation complex (this is the
content of the Euler identity for the $H$-Poisson bracket, which
follows from the degree-counting argument in the proof of
Theorem~\ref{thm:nms-all-arity-master-equation}), the claim is immediate.
\end{proof}
...
vanishes.  The OPE $W(z)W(w) \sim \ldots + \Lambda(w)/(z-w)^2 +
\ldots$ with $\Lambda = (TT) - \tfrac{3}{10}\partial^2 T$
(a composite of weight~$4$) generates three quartic channels:
$Q_{TTTT}$, $Q_{TTWW}$, and $Q_{WWWW}$.  The mixed channel has
denominator $c(5c+22)^2$, extending the Virasoro quartic singularity
(Proposition~\ref{prop:nms-quartic-contact-virasoro}).

$W$-charge conservation (the $\mathbb{Z}/2$ symmetry
$W \mapsto -W$) forces $\mathrm{Sh}_n|_{x_T = 0} = 0$ for odd~$n$:
only even powers of $x_W$ appear.  In particular $\mathrm{Sh}_3$
lies entirely in the $x_T$-sector.
\end{example}
...
$m \leq n \cdot \max_i(h_i)$, where $h_i$ are the conformal weights
of the generators.  Shadow singularities occur precisely at null
vector decoupling loci.
\end{proposition}

\begin{proof}
The shadow $\mathrm{Sh}_n$ is constructed by iterating the
$H$-Poisson bracket (equation~\eqref{eq:nms-multivariable-master-equation})
and the Euler inversion (Lemma~\ref{lem:nms-euler-inversion}).
At each step, the denominators arise from two sources:
(i)~the propagator $P = \kappa^{-1}$, whose entries are rational in~$c$
with poles at $c = 0$; and (ii)~the OPE structure constants $C_{ijk}$,
which involve inner products of descendant states and hence carry Kac
determinant factors in their denominators.  At arity~$n$, the
iteration involves at most $n-2$ bracket operations, each introducing
OPE data at conformal weight $\leq \max_i(h_i)$.  The composite
weight bound is $n \cdot \max_i(h_i)$.  The singularity locus is
therefore contained in the zero locus of
$\prod_{m=1}^{n \cdot \max(h_i)} \det M_m(c,0)$.

Conversely, at each null vector decoupling point, the quasi-primary
projection used to define $\mathrm{Sh}_n$ develops a pole: the
inner product matrix on the relevant weight space degenerates, and
the inverse (which enters the quasi-primary projector) diverges.
\end{proof}

\begin{table}[ht]
\centering
\caption{Shadow denominators: evidence for the Kac-shadow
  singularity principle.}
\label{tab:nms-kac-shadow-evidence}
\begin{tabular}{lll}
\toprule
Shadow & Denominator & Kac factor \\
\midrule
$\mathrm{Sh}_4^{\mathrm{Vir}}$ ($Q^{\mathrm{contact}}$)
  & $c(5c+22)$ & $\det M_2 \cdot \det M_4$ \\
$Q_{TTWW}^{\mathcal{W}_3}$
  & $c(5c+22)^2$ & $(\det M_2)^2 \cdot \det M_4$ \\
$C_{333}^2$ ($\mathcal{W}_3$ cubic)
  & $5c(c+24)(5c+22)$
  & $\det M_3 \cdot \det M_4 \cdot \det M_6$ \\
\bottomrule
\end{tabular}
\end{table}
...
$\Lambda = (TT) - \tfrac{3}{10}\partial^2 T$ of conformal weight~$4$,
whose OPE with $W$ is non-quadratic: the $W(z)W(w)$ OPE contains
$\Lambda(w)/(z-w)^2$.  This non-quadratic term is a genuine
$A_\infty$ obstruction (it represents a nontrivial $m_3$ in the
transferred structure).  The quintic obstruction
$o^{(5)}_{\mathcal{W}_3}$ is forced to be nonzero by the same
mechanism as for Virasoro
(Theorem~\ref{thm:nms-virasoro-quintic-forced}): the quartic contact
invariant $Q^{\mathrm{contact}}_{\mathcal{W}_3} \neq 0$ feeds into
the master equation at arity~$5$, and the $W$-charge conservation
does not kill the relevant component.  Hence the tower does not
terminate: $r_{\max}(\mathcal{W}_3) = \infty$, class~$M$.
\end{proof}
...
Let $B \subset \cA$ be a conformal sub-VOA such that the kappa matrix
is block-diagonal between the $B$-generators and the complementary
generators: $\kappa_{ij} = 0$ whenever $\phi_i$ is a $B$-generator
and $\phi_j$ is not (or vice versa).  Then
\[
...
\begin{proof}
Block-diagonality of~$\kappa$ implies block-diagonality of the
...
(by block-diagonality of~$\kappa$),
$\mathrm{Sh}_3(\cA)|_{x_{\cA \setminus B} = 0} = \mathrm{Sh}_3(B)$
(the OPE structure constants $C_{ijk}$ with all indices in the
$B$-sector are exactly the OPE constants of~$B$), and similarly for
$\mathrm{Sh}_4$.
...
\begin{proof}
The $\mathbb{Z}/2$ symmetry $W \mapsto -W$ of $\mathcal{W}_3$ acts
on the deformation coordinate as $x_W \mapsto -x_W$, $x_T \mapsto
x_T$.  Since the shadow tower is equivariant under VOA automorphisms
(the master equation and $H$-Poisson bracket are manifestly
equivariant), $\mathrm{Sh}_n(-x_W, 0) = (-1)^n \mathrm{Sh}_n(x_W,
0)$ when restricted to $x_T = 0$.  But also $\mathrm{Sh}_n(x_W, 0)$
is a monomial $\alpha_n x_W^n$ (being a polynomial of degree~$n$ in
the single variable~$x_W$).  For odd~$n$,
$\mathrm{Sh}_n(-x_W, 0) = -\mathrm{Sh}_n(x_W, 0) = (-1)^n
\mathrm{Sh}_n(x_W, 0) = -\mathrm{Sh}_n(x_W, 0)$, which is
consistent but does not force vanishing.  The vanishing instead follows
from Theorem~\ref{thm:shadow-subalgebra-autonomy}: the pure
$W$-sector ($x_T = 0$) is the shadow of the sub-VOA generated
by~$W$ alone, but $W$ does not generate a sub-VOA (the $WW$ OPE
produces~$T$ and~$\Lambda$, which lie outside the $W$-sector).
Equivalently, the $B$-sector with $B = \langle W \rangle$ has
$C_{WWW} = 0$ (by $W$-charge: weight~$3+3+3 = 9$ is odd under
$W \mapsto -W$), so $\mathrm{Sh}_3|_{x_T=0} = 0$, and the
$WW$-propagator channel feeds only into even-arity shadows via the
master equation.  By induction, all odd-arity restrictions vanish.
\end{proof}
