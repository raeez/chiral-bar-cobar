...
This filtered finite-order engine---not the scalar $\kappa$ alone, and not the conjectural all-arity limit~$\Theta_\cA$---is the primary mathematical object of the nonlinear theory.
...
The point is structural.  The proved scalar tower $\kappa(\cA)\lambda_g$ and the proved spectral package $\Delta_{\cA}$ do not exhaust the modular content of the theory.  The finite-order shadow tower is the primary proved content at the nonlinear level: each truncation $\Theta_\cA^{\leq r}$ is constructive and does not require the full all-genera modular envelope.  It is the first place where complementarity stops looking like an eigenspace decomposition and becomes geometry.  The full all-arity convergence $\Theta_\cA = \lim_r \Theta_\cA^{\leq r}$ is the conjectural horizon.
...
\item $t_g^{(\infty)}$: the conjectural all-arity limit is
...
The finite-order shadow calculus of the preceding sections captures the arity-$\le 4$ face of the universal Maurer--Cartan class.  The natural globalization is not to attack $\Theta_{\cA}$ directly but to identify the \emph{graph-completed carrier object} in which it lives.  This section constructs that carrier, states the resulting conjecture, and introduces three new geometric avatars.
...
\begin{remark}[Why the Feynman transform resolves the three open requirements]
\label{rem:nms-feynman-three-requirements}
The three identified gaps in the construction of the universal class are: (1)~a cyclic structure on bar coderivations; (2)~the completed tensor product with tautological coefficients; (3)~a clutching-compatible Maurer--Cartan solution.  In $\mathfrak F_{\mathrm{mod}}(\cA)$:
\begin{itemize}
...
\begin{conjecture}[Graph-completed modular master element; \ClaimStatusConjectured]
\label{conj:nms-graph-completed-master-element}
\index{modular master element!graph-completed}%
There exists a canonical group-like Maurer--Cartan element
\[
\boldsymbol\Theta_{\cA}\in \mathrm{MC}(\mathfrak F_{\mathrm{mod}}(\cA))
\]
whose projections recover the entire modular characteristic hierarchy:
\begin{enumerate}[label=\textup{(\roman*)}]
...
\end{enumerate}
This is the correct replacement for the raw conjecture ``construct~$\Theta_{\cA}$.''  The true object is not a single formal series but a graph-completed Hamiltonian modular master action.
\end{conjecture}
...
$\mathfrak{T}_{3,\xi}(\cA)$;
\item if the full universal class $\Theta_{\cA}$ exists, this
...
The construction above does not pretend to have solved the entire
H-level problem.  The full foundational task remains: construct the
universal class $\Theta_{\cA}$ globally on the correct modular
locus, prove its clutching and Verdier compatibilities in complete
generality, and identify the resulting Hamiltonian modular master
