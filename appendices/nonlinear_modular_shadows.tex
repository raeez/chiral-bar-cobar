...
\begin{remark}[Propagator variance and multi-channel mixing]
\label{rem:propagator-variance-mixing}%
\index{propagator!variance}%
\index{shadow tower!multi-channel mixing}%
On a multi-channel shadow tower with diagonal curvature~$\kappa_i$ and quartic gradients~$f_i$, the departure from single-line autonomy is measured by the \emph{propagator variance}
\[
\delta_{\mathrm{mix}}
\;=\;
...
Non-negativity is Cauchy--Schwarz; $\delta_{\mathrm{mix}} = 0$ if and only if the quartic gradient vector $(f_1,\dots,f_r)$ is proportional to the curvature vector $(\kappa_1,\dots,\kappa_r)$, restoring effective one-channel behavior.
For~$\mathcal{W}_3$, the mixing polynomial $P_{\mathcal{W}_3}(c) = 25c^2 + 100c - 428$ satisfies
\[
\delta_{\mathrm{mix}}
\;=\;
\frac{1280\,P_{\mathcal{W}_3}(c)^2}{c^3\,(5c+22)^6}.
\]
The enhanced symmetry loci $P_{\mathcal{W}_3} = 0$ correspond to central charges where the quartic gradient is proportional to the curvature, so the full two-variable shadow tower reduces to an effectively one-variable problem.
\end{remark}
