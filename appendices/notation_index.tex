...
$\VD$ & Verdier duality functor on sheaves/D-modules; in the bar-cobar package it sends the bar coalgebra to the homotopy dual algebra after the required completion, not to an ordinary uncompleted graded dual \\
$\DR$ & De Rham functor \\
...
$\B(\cA)$ & Full bar construction of $\cA$; a conilpotent dg coalgebra whose cobar counit reconstructs $\cA$ on the augmented complete surface \\
$\Cobar(\cC)$ & Cobar construction of a conilpotent coalgebra $\cC$; a dg algebra.  The counit $\Cobar(\Bbar(\cA)) \xrightarrow{\sim} \cA$ is the bar--cobar reconstruction map on the augmented complete surface \\
$\Bbar(\cA)$ & Reduced full bar coalgebra $B_X(\cA)=(T^c(s^{-1}\bar{\cA}),d)$.  A quadratic presentation $\cA=T_X(V)/(R)$ supplies $\cA^{\mathrm i}=C_X(s^{-1}V,s^{-2}R)$ and $q_\cA\colon\cA^{\mathrm i}\to\Bbar(\cA)$; under $H_{\mathrm{CL}}$, $H^\bullet(q_\cA)$ identifies their cohomologies \\
$\Bbar^{\mathrm{ord}}(\cA)$ & Ordered reduced bar coalgebra $T^c(s^{-1}\bar{\cA})$ on ordered configuration spaces; it retains the $\Sigma_n$-ordering, braid monodromy, and $R$-matrix data before averaging \\
$\Bbar^{\Sigma}(\cA)$ & Symmetric/coinvariant reduced bar coalgebra $\Sym^c(s^{-1}\bar{\cA})$; it is the $\Sigma_n$-coinvariant shadow of the ordered bar after the required equivariant descent, and the passage from ordered to symmetric is lossy \\
%
$\overline{\Cobar}(\cC)$ & Reduced cobar construction \\
$\Bbar^{\mathrm{geom}}$ & Configuration-space model of the reduced chiral bar coalgebra \\
$\Cobar^{\mathrm{geom}}$ & Geometric cobar complex obtained from the bar coalgebra by the Verdier/distributional dual construction \\
...
$\eta_{ij}$ & FM logarithmic normal form along $D_{ij}$; on an affine/formal collision screen it is represented by $\dlog(z_i - z_j)$, while global curve models use the coordinate-change cocycle and KZB/prime-form replacement data \\
...
$\cA, \cB, \ldots$ & Chiral source/boundary algebras; the bar, presentation, Verdier, chosen-pair, and derived-centre constructions occupy their typed companion slots \\
...
$\cA^{\mathrm i}$ & Quadratic presentation coalgebra $C_X(s^{-1}V,s^{-2}R)$ attached to a chosen presentation $\cA=T_X(V)/(R)$, with canonical comparison $q_\cA\colon\cA^{\mathrm i}\to B_X(\cA)$.  On the Koszul locus, $H^\bullet(q_\cA)$ is an isomorphism onto bar cohomology. \\
%
$K_X(\cA)$ & Ran--Verdier algebra $\VD_{\Ran}B_X(\cA)$.  Contravariance sends $q_\cA$ to $\VD(q_\cA)\colon K_X(\cA)\to\VD_{\Ran}(\cA^{\mathrm i})$. \\
$\cA^!$ & Chosen Koszul-pair algebra, reached by $\nu_\cA\colon K_X(\cA)\to\cA^!$.  Under the finite-type or completed continuous pair hypotheses, $\nu_\cA$ is an equivalence and $\cA^!\simeq\VD_{\Ran}(\cA^{\mathrm i})\simeq(\cA^{\mathrm i})^\vee$. \\
$H^{\mathrm{ch}}_*(X, \cA)$ & Chiral homology of $\cA$ over $X$ \\
$H^{*,\mathrm{ch}}(X, \cA)$ & Chiral cohomology \\
$\CC^*_{\mathrm{ch}}(\cA)$ & Chiral Hochschild cochain complex: the curve/Fulton--MacPherson chiral complex built from chiral products of $\cA$ \\
$\CC_*^{\mathrm{ch}}(\cA)$ & Chiral Hochschild chain complex; distinct from topological Hochschild homology of a ring spectrum \\
%
$\ChirHoch^*(\cA)$ & Chiral Hochschild cohomology. The ordered version $\ChirHoch^{\mathrm{ord},*}(\cA)$ is computed before $\Sigma_n$-averaging; the symmetric version is obtained only after the averaging/descent step. \\
$HH_*^{\mathrm{ch}}(\cA)$ & Chiral Hochschild homology of a chiral algebra on a curve \\
$Z^{\mathrm{der}}_{\mathrm{ch}}(\cA)$ & Chiral derived centre $\ChirHoch^\bullet(\cA,\cA)$, the cochain-level universal closed-sector $E_2$-object attached to $\cA$.  A physical bulk interpretation is supplied by an open--closed comparison map. \\
$\mathrm{HH}^\bullet_{\mathrm{alg}}(A)$ & Classical algebraic Hochschild cohomology $\RHom_{A^e}(A,A)$ of an associative or dg algebra~$A$ \\
$\mathrm{HH}^\bullet_{\mathrm{cat}}(\cC)$ & Categorical Hochschild cohomology of a dg/$A_\infty$-category~$\cC$: endomorphisms of the identity functor, equivalently $\RHom_{\cC^e}(\Delta_\cC,\Delta_\cC)$ \\
$\mathrm{THH}(R)$ & Topological Hochschild homology of an $E_1$-ring spectrum~$R$; not chiral Hochschild and not categorical Hochschild cohomology \\
...
$L_n$ & Virasoro modes \\
$c$ & Virasoro/CFT central charge; family-dependent input to $\kappa(\cA)$, with $\kappa(\mathrm{Vir}_c)=c/2$ rather than $c=\kappa$ \\
\end{longtable}
...
$k$ & Ground field when used as coefficient field; in $V_k(\fg)$ or $\mathcal H_k$, the subscript $k$ is a level parameter, not the ground field \\
...
$\kappa$ & Bare modular characteristic only after a family has been fixed; affine and Heisenberg levels are written as level parameters such as $k$ or $\ell$, not identified with the invariant by notation alone \\
$\kappa(\cA)$ & Obstruction coefficient (modular characteristic) of chiral algebra $\cA$; standard normalizations include $\kappa(\mathcal H_\ell^{\oplus d})=d\ell$, $\kappa(V_k(\fg))=\dim(\fg)(k+h^\vee)/(2h^\vee)$, $\kappa(\mathrm{Vir}_c)=c/2$, and $\kappa(\mathcal W_N)=c(H_N-1)$ \\
$\kappa_d(\cA)$ & Shadow depth of $\cA$ (Definition~\ref{def:shadow-depth-classification}) \\
...
$K_{\mathcal H}^{\mathrm{coll}}(z)$ & Trace-form Heisenberg collision kernel $k\Omega_{\mathcal H}/z$; $\tau_{\mathcal H,2}^{\mathrm{res}}q_2(K_{\mathcal H}^{\mathrm{coll}})=\kappa(\mathcal H_k)=k$ \\
$r^{\mathrm{KM}}(z)$ & Trace-form affine Kac--Moody residue $k\Omega_{\mathrm{tr}}/z$ with the level prefix included; distinct from the KZ residue $\Omega_{\mathrm{KZ}}/((k+h^\vee)z)$ \\
$r^{\mathrm{Vir}}_c(z)$ & Virasoro ordered collision residue $(c/2)z^{-3}+2Tz^{-1}$; the scalar cubic-pole coefficient gives $\kappa(\mathrm{Vir}_c)=c/2$ \\
%
$S_\cA(u)$ & Dirichlet--sewing lift of $\cA$ (Definition~\ref{def:dirichlet-sewing-lift}) \\
$K_\cA(s,t)$ & Sewing reproducing kernel $= S_\cA(s{+}t)$ (Definition~\ref{def:sewing-kernel}) \\
$\langle f, g \rangle_\cA$ & Sewing inner product (Definition~\ref{def:sewing-inner-product}) \\
$D_\cA(u)$ & Euler--Koszul defect (Definition~\ref{def:euler-koszul-defect}) \\
$\operatorname{ek}(\cA)$ & Euler--Koszul class $= \max_i(w_i - 1)$ (Definition~\ref{def:euler-koszul-class}) \\
$\mathscr{M}_\cA(\alpha)$ & Surface moment matrix / sewing Gram matrix (Definition~\ref{def:surface-moment-matrix}) \\
$\tilde\lambda_n(\cA)$ & Prime-side Li coefficients (Definition~\ref{def:prime-side-li}) \\
$\Xi_\cA(u)$ & Regularized sewing lift $(u{-}1)S_\cA(u)$ \\
$\mathscr{D}_1(\cA)$ & Genus-$1$ modular datum (Definition~\ref{def:genus1-modular-datum}) \\
$\mathcal{R}$ & Shadow resonance locus (Theorem~\ref{thm:shadow-resonance-locus}) \\
...
\multicolumn{2}{l}{\emph{Arithmetic shadow obstruction tower
 (Chapter~\ref{chap:arithmetic-shadows}):}} \\
$\varepsilon^c_s(\cA)$ & Constrained Epstein zeta:
 $\sum_{\Delta\in\mathcal{S}}(2\Delta)^{-s}$, summed over scalar
 primary dimensions \\
...
$\alpha(\cA)$ & Cusp growth rate of
 ...
 $\cM_{1,1}$ \\
$d_{\mathrm{arith}}(\cA)$ & Arithmetic depth:
 number of holomorphic Hecke eigenforms in the spectral
 decomposition of~$\widehat{Z}^c$; determines critical-line
 count \\
$d_{\mathrm{alg}}(\cA)$ & Algebraic depth:
 non-holomorphic spectral complexity from the OPE structure;
 $d = 1 + d_{\mathrm{arith}} + d_{\mathrm{alg}}$ \\
$\lambda_{\mathrm{eff}}$ & Effective coupling of the shadow
 tower: $\lambda_{\mathrm{eff}} = -6/c$ for Virasoro
...
Normal ordering conventions are those in the explicit OPE package of Definition~\ref{thm:beta-gamma-ope-complete}.
\end{definition}
...
Locality is encoded by the factorization-locality principle~\ref{rem:factorization-locality}.
\end{definition}
