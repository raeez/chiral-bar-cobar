...
The governing question of this appendix is the mathematical role of \emph{Symbols}: which auxiliary input it supplies to the modular bar-cobar theory on curves, and how it should be used without inflating reference material into new theorematic claims.
\end{remark}
...
\begin{remark}[Two-axis $E$-notation; Remark~\ref{rem:En-terminology}]
...
$\omega_{ij}$ & Propagator form $\dlog(z_i - z_j)$ (denoted $\eta_{ij}$ in Chapters~5--15) \\
$\eta_{ij}$ & Alternative notation for $\omega_{ij}$ (= $\omega_{ij}$; see above) \\
...
\begin{definition}[$A_\infty$-algebra]\label{def:ainf-algebra}
We use the curved/filtered \(A_\infty\) framework fixed in Definition~\ref{def:curved-ainfty-complete}.
\end{definition}
...
\begin{definition}[Classical \(\mathcal{D}\)-module input]\label{def:dmod-classical}
The \(\mathcal{D}\)-module realization used throughout is the one in Theorem~\ref{thm:chiral-ran}.
\end{definition}

\begin{definition}[\(\mathbb{E}_1\)-chiral algebra]\label{def:e1-chiral-algebra}
...
\begin{definition}[\(\mathbb{E}_\infty\)-chiral algebra]\label{def:einf-chiral-algebra}
...
Twisting morphisms are used in the Maurer--Cartan package of Theorem~\ref{thm:twisting-mc}.
\end{definition}
