...
$\B(\cA)$ & Bar construction of algebra $\cA$; a dg coalgebra resolving $\cA$, not the Koszul dual algebra and not the derived center \\
$\Cobar(\cC)$ & Cobar construction of coalgebra $\cC$; a dg algebra. The counit $\Cobar(\Bbar(\cA)) \xrightarrow{\sim} \cA$ is bar-cobar inversion on the Koszul/bar-complete locus, not the definition of $\cA^!$ \\
$\Bbar(\cA)$ & Reduced bar construction $= (T^c(s^{-1}\bar{\cA}),\, d)$: a conilpotent dg coalgebra. Its cohomology is the Koszul-dual coalgebra $\cA^i=H^\bullet(\Bbar(\cA))$ on the Koszul locus; the chain coalgebra itself is not $\cA^!$ \\
...
$\cA, \cB, \ldots$ & Chiral algebras; source/boundary objects, distinct from $\Bbar(\cA)$, $\cA^i$, $\cA^!$, and $Z^{\mathrm{der}}_{\mathrm{ch}}(\cA)$ \\
...
$\cA^i$ & Koszul dual \emph{coalgebra}: $\cA^i=H^\bullet(\Bbar(\cA))$ on the Koszul locus, with completed version $H^\bullet\widehat{\Bbar}^{\mathrm{ch}}(\cA)$. The chain object $\Bbar(\cA)$ resolves $\cA^i$; it is not the strict dual algebra. \\
%
$\cA^!$ & Koszul dual \emph{algebra}: $\cA^!=(\cA^i)^\vee$ under finite-type or completed continuous duality. The homotopy dual is $\VD_{\Ran}\Bbar^{\mathrm{ch}}(\cA)$; the strict algebra is obtained only after the finite-type/completed dual passage. \\
...
$Z^{\mathrm{der}}_{\mathrm{ch}}(\cA)$ & Chiral derived center, $Z^{\mathrm{der}}_{\mathrm{ch}}(\cA)=\ChirHoch^\bullet(\cA,\cA)$ as the cochain-level closed-sector/endocentral $E_2$-object attached to $\cA$; not $\cA$, not $\Bbar(\cA)$, and not $\cA^!$. A physical bulk requires an additional open/closed comparison map. \\
...
$r^{\mathrm{Heis}}(z)$ & Trace-form Heisenberg collision residue $k/z$; its scalar average is $\kappa(\mathcal H_k)=k$ \\
$r^{\mathrm{KM}}(z)$ & Trace-form affine Kac--Moody residue $k\Omega/z$ with the level prefix included; distinct from the KZ residue $\Omega/((k+h^\vee)z)$ \\
