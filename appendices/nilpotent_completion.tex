...
\begin{align*}
F_0C &= k \quad (\text{coaugmentation}) \\
F_1C &= k \oplus \ker(\bar{\Delta}) \quad (\text{primitives}) \\
F_nC &= k \oplus \{c \in \ker(\epsilon) : \bar{\Delta}(c) \in \textstyle\sum_{i+j=n-1} F_iC \otimes F_jC\}
\end{align*}
...
\begin{proof}
We verify that the inverse system $\{\bar{B}(\mathcal{A})/F_n\}_{n \geq 0}$ defines a convergent pro-coalgebra whose limit inherits a well-defined dg structure.

\textbf{Step 1: Finite generation in each bidegree.}
...
\]
where $N(k)$ is a bound coming from the conformal weight grading.  The filtration level $F_n \cap \bar{B}_k$ is a sub-$\mathcal{D}_X$-module of a finitely generated module, hence finitely generated (since $\mathcal{D}_X$ is Noetherian on a smooth algebraic curve~$X$).

\textbf{Step 2: Polynomial growth controls the associated graded.}
The polynomial growth hypothesis states that the OPE coefficients $C^c_{ab}(n)$ (defined by $a_{(n)}b = \sum_c C^c_{ab}(n)\, c$) satisfy $|C^c_{ab}(n)| \leq P(n)$ for a polynomial~$P$.  This implies that the bar differential $d_{\bar{B}}$, which involves residues of OPEs, preserves the filtration ($d_{\bar{B}}(F_n) \subset F_n$) with the induced map on graded pieces $\mathrm{gr}_n \bar{B} \to \mathrm{gr}_n \bar{B}$ bounded polynomially in~$n$.  Therefore:
\[
\dim\bigl((F_n/F_{n+1}) \cap \bar{B}_k\bigr) \leq P(n) \cdot Q(k)
\]
for polynomials $P, Q$ depending only on the generating data of~$\mathcal{A}$.

\textbf{Step 3: Finiteness ensures differential compatibility.}
The finiteness condition $\dim H^*(\mathcal{A}, \mathcal{A}) < \infty$ guarantees that the obstruction theory for extending the differential from $\bar{B}/F_n$ to $\bar{B}/F_{n+1}$ is unobstructed.  Specifically, the obstruction to lifting $d_{\bar{B}} \bmod F_n$ to $d_{\bar{B}} \bmod F_{n+1}$ lives in $H^2(\mathcal{A}, F_n/F_{n+1})$, which is finite-dimensional and eventually zero by the polynomial growth bound.

\textbf{Step 4: Mittag-Leffler condition and inverse limit.}
The polynomial growth bound on $\dim(F_n/F_{n+1})_k$ implies that for each fixed bar degree~$k$, the transition maps $\bar{B}_k/F_{n+1} \twoheadrightarrow \bar{B}_k/F_n$ have images that stabilize: $\mathrm{im}(\bar{B}_k/F_m \to \bar{B}_k/F_n)$ is independent of $m$ for $m \geq m_0(n,k)$.  This is the Mittag-Leffler condition, which implies $\varprojlim^1_n (\bar{B}_k/F_n) = 0$.  Therefore the inverse limit
\[
\widehat{\bar{B}}_k(\mathcal{A}) = \varprojlim_n \bar{B}_k(\mathcal{A})/F_n
\]
exists and is exact.  The dg coalgebra structure passes to the limit because $d_{\bar{B}}$ and $\Delta$ are continuous for the $F$-adic topology.
\end{proof}
...
\textbf{Step 1: Filtration compatibility.}
The conilpotent filtration $\{F_n\bar{B}(\mathcal{A})\}$ on the bar complex (Proposition~\ref{prop:geom-conilpotent}) induces a filtration on the completion: $\widehat{\bar{B}}(\mathcal{A}) = \varprojlim_n \bar{B}(\mathcal{A})/F_n$.  The bar differential $d_{\bar{B}}$ is compatible with this filtration (it decreases degree by~1 and collision depth by at most~1), so $\widehat{\bar{B}}(\mathcal{A})$ is a well-defined dg coalgebra.

\textbf{Step 2: Cobar functor on the completion.}
...
\textbf{Step 3: Comparison with the uncompleted version.}
The canonical map $\bar{B}(\mathcal{A}) \to \widehat{\bar{B}}(\mathcal{A})$ induces a map of cobar complexes:
\[
\Omega(\bar{B}(\mathcal{A})) \leftarrow \Omega(\widehat{\bar{B}}(\mathcal{A}))
\]
(contravariant in the coalgebra argument, since $\Omega$ is left adjoint to~$\bar{B}$).  By the uncompleted bar-cobar inversion theorem, $\Omega(\bar{B}(\mathcal{A})) \xrightarrow{\sim} \mathcal{A}$ is a quasi-isomorphism.

\textbf{Step 4: Completion does not change homotopy type.}
The convergence hypotheses (Theorem~\ref{thm:completion-convergence}) ensure that the inverse system $\{\bar{B}(\mathcal{A})/F_n\}_{n \geq 0}$ satisfies the Mittag-Leffler condition.  Indeed, the polynomial growth of structure constants implies that for each bar complex degree~$k$, the quotients $(F_n \bar{B}_k)/(F_{n+1} \bar{B}_k)$ stabilize for $n$ sufficiently large relative to~$k$.  Therefore $\varprojlim^1 = 0$, and the natural map
\[
H^*(\widehat{\bar{B}}(\mathcal{A})) \xrightarrow{\;\cong\;} \varprojlim_n H^*(\bar{B}(\mathcal{A})/F_n)
\]
...
\begin{proof}
\textbf{Forward direction} ($\widehat{\mathcal{C}} = \widehat{\bar{B}}(\mathcal{A}) \Rightarrow$ (1)--(4)):
...
\medskip
\textbf{Backward direction} ((1)--(4) $\Rightarrow$ $\widehat{\mathcal{C}} \simeq \widehat{\bar{B}}(\mathcal{A})$):
...
\emph{Step 3: Recovery.}  By Theorem~\ref{thm:completed-bar-cobar}, $\Omega(\widehat{\bar{B}}(\mathcal{A})) \simeq \mathcal{A}$.  We must show $\widehat{\bar{B}}(\mathcal{A}) \simeq \widehat{\mathcal{C}}$.  The bar-cobar adjunction provides a counit $\epsilon: \widehat{\bar{B}}(\Omega(\widehat{\mathcal{C}})) \to \widehat{\mathcal{C}}$.  Condition (4) ensures this is a quasi-isomorphism: the $E_2$ degeneration means the bar spectral sequence for $\mathcal{A} = \Omega(\widehat{\mathcal{C}})$ collapses, so $\widehat{\bar{B}}(\mathcal{A})$ has the same cohomology as $\widehat{\mathcal{C}}$.  Since both are complete and conilpotent, a cohomology isomorphism between completed conilpotent coalgebras is a quasi-isomorphism of dg coalgebras (by the comparison theorem for complete filtered complexes, cf.\ Positselski \cite{Pos11}).
\end{proof}
...
\begin{theorem}[BD Chiral Homology; \ClaimStatusProvedElsewhere]\label{thm:BD-chiral-homology}
...
\caption{Computing $\widehat{\bar{B}}(\mathcal{A})/I^N$}
\label{alg:compute-completion}
...
\begin{proposition}[Convergence of the Completion; \ClaimStatusProvedHere]\label{prop:practical-convergence}
...
\begin{enumerate}
\item $\mathcal{A}$ is \textbf{finitely generated}: the bar complex at bar degree $n$
...
\item $\mathcal{A}$ has \textbf{countably many generators} of weights
...
\begin{theorem}[Costello-Gwilliam Renormalization; \ClaimStatusProvedElsewhere]\label{thm:CG-renorm}
...
\begin{theorem}[Main Result of Appendix; \ClaimStatusProvedHere]\label{thm:nilpotent-main}
...
\begin{enumerate}
\item \textbf{Convergence}: $\widehat{\bar{B}}(\mathcal{A})$ is a well-defined dg coalgebra \textup{(Theorem~\ref{thm:completion-convergence})}
\item \textbf{Duality}: $\Omega(\widehat{\bar{B}}(\mathcal{A})) \simeq \mathcal{A}$ \textup{(Theorem~\ref{thm:completed-bar-cobar})}
\item \textbf{Essential image}: A completed coalgebra arises as $\widehat{\bar{B}}(\mathcal{A})$ iff it satisfies conilpotency, topological compatibility, cogenerator existence, and $E_2$ degeneration \textup{(Theorem~\ref{thm:koszul-dual-characterization})}
\item \textbf{BD compatibility}: The completion agrees with Beilinson-Drinfeld's filtered chiral algebra theory \textup{(Theorem~\ref{thm:BD-chiral-homology})}
\item \textbf{Geometric origin}: The conilpotent filtration is the collision depth filtration on FM compactifications \textup{(Proposition~\ref{prop:geom-conilpotent})}
\end{enumerate}
...
Together, these results establish a complete framework for Koszul duality of non-quadratic chiral algebras: the completed bar construction defines the dual coalgebra (1), the cobar construction inverts it (2), the essential image is characterized (3), and the construction is compatible with both the BD algebraic framework (4) and the geometric picture via configuration spaces (5).
\end{proof}
