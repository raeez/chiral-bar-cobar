...
The Virasoro algebra $\mathrm{Vir}_c$ should admit a
positive-energy filtration on the state-space bar complex
with a finite-dimensional resonance piece
$R_{\mathrm{Vir}}$ accounting for the central term and
normal-ordering corrections.  The hypotheses that must be
verified are:
\begin{enumerate}
\item finite-dimensionality of the resonance slice;
\item strict positive-weight growth of mixed operations;
\item compatibility with the finite-stage $\mathcal{W}_N$
  approximants (so that
  $\widehat{\bar B}^{\mathrm{res}}(\mathrm{Vir}_c)$
  recovers the $N \to \infty$ limit of
  $\bar B(W_N)$);
\item DS-reduction compatibility with the resonance
  decomposition.
\end{enumerate}
If these hold, then the current same-family shadow
$\mathrm{Vir}_{26-c}$ is reinterpreted as the
\emph{depth-zero resonance shadow} (the image of the
resonance truncation $R_{\mathrm{Vir}}$), while the
genuine $\mathcal{W}_\infty$-type dual is recovered only
from the full completed object
$\widehat{\bar B}^{\mathrm{res}}(\mathrm{Vir}_c)$.
This is a significant conceptual advance: the same-family
shadow is not the final dual but rather the leading term of a
resonance expansion.
\end{remark}

\subsection{The Platonic completion conjecture}

\begin{conjecture}[Platonic completion theorem]
\label{conj:platonic-completion}
...
\item the Platonic completion conjecture
  \textup{(}Conjecture~\ref{conj:platonic-completion}\textup{)},
  which identifies the irreducible core of MC4 as a finite
  resonance problem.
\end{enumerate}
What is \emph{not} yet elevated to monograph-wide theorematic status is
the Virasoro resonance model: the explicit construction of a
positive-energy filtration with finite-dimensional resonance piece
for $\mathrm{Vir}_c$ and the non-quadratic $\mathcal{W}_N$ algebras.
The Platonic completion conjecture
\textup{(}Conjecture~\ref{conj:platonic-completion}\textup{)}
is the sharp form of the remaining MC4 content:
if every positive-energy chiral algebra has finite resonance rank,
then ``MC4'' is a finite-dimensional problem sitting atop an
