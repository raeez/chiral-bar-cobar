...
is a special case of the formality route
(Proposition~\ref{prop:ds-bar-formality}): all hook partitions
$(N{-}r, 1^r)$ have abelian nilradical $\mathfrak{n}_+$ with a
single positive $\operatorname{ad}(h)$-grade.  The PBW--Slodowy
collapse (Theorem~\ref{thm:pbw-slodowy-collapse}) provides the Li
filtration, and the Kazhdan filtration gives BRST formality.  The
non-hook partitions with abelian $\mathfrak{n}_+$ --- the
rectangular partitions $(m^r)$ --- are equally accessible by this
mechanism.  Together, hooks and rectangles exhaust the abelian
$\mathfrak{n}_+$ locus in type~$A$; the first genuinely
non-abelian case is $(3,2)$ in $\mathfrak{sl}_5$
