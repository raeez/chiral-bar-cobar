...
Koszul-dual coalgebra $A^i=H^\bullet\Barchord(A)$, is the line-side
Koszul-dual algebra
$A^!_{\mathrm{line}}=(A^i)^\vee_{\mathrm{cont}}$; Verdier duality
enters only after symmetric/Ran descent.
...
$A^i$, and the line-side Koszul-dual algebra is
...
rank-one coefficient gives $k\,dz/z$. For affine Kac--Moody the collision residue in trace-form
normalization is $r^{\mathrm{coll}}(z)=k\Omega_{\mathrm{tr}}/z$; the
KZ/Yangian form $\Omega_{\mathrm{KZ}}/((k+h^\vee)z)$ is the rescaled
Casimir-and-coupling normalization, not the trace-form collision residue.

\emph{Flatness.}
Let
\[
A_\nabla=\sum_{i<j}r_{ij}(z_{ij})\,dz_{ij}.
\]
...
It is not a consequence of the consecutive Stasheff relation alone.
Flatness of~$\nabla$ is part of the $R$-matrix descent datum.
...
is the binary genus-$0$ collision projection
$\Res^{\mathrm{coll}}_{0,2}(\Theta_A^{E_1})$ of the ordered universal
Maurer--Cartan element. The governing equation is
\[
D\Theta_A^{E_1}
+\tfrac{1}{2}[\Theta_A^{E_1},\Theta_A^{E_1}]=0.
\]
Projecting its degree-$3$ part to triple binary collisions and using
the Arnold relation gives the CYBE~\eqref{eq:cybe-vol1}; the CYBE is
not the linearisation but the quadratic binary shadow of this MC
equation. The YBE on $R(z)$ is its monodromy form. The $R$-matrix is
...
$\nabla = d - r^{\mathrm{coll}}(z)\,dz$, not independent
...
reflects the pole-order classification: double pole with scalar
residue (class~G) versus simple pole whose pole residue drops but whose
regular contact class is nilpotent (class~C).
\end{remark}
...
periods of mixed Tate motives over~$\mathbb{Z}$.
	...
	keeps the $R$-twisted coinvariant combinations. The scalar
	symmetric bar invariants used in the shadow classification
...
ordered to symmetric configurations. The following table
