...
lattice algebras, $W$-algebras, all of which are $E_\infty$-chiral\textup{)},
...
one power: $\Res_{w=0}\frac{k}{w^2}\cdot w\,d\log w = k/z$\textup{)}.
...
$\ker(\mathrm{av})$ is generated by $R$-matrix monodromy data
and vanishes in the pole-free case. Here we give a second,
representation-theoretic proof of the dimension formula
\begin{equation}\label{eq:kernel-dim-averaging}
\dim\ker(\mathrm{av}_n)
\;=\;
d^n \;-\; \binom{n+d-1}{d-1}
\end{equation}
in the pole-free ($R = \tau$) setting, where $d = \dim V$ and the
averaging map at tensor degree~$n$ is the projection
...
\]
At $n = 2$: $d^2 - \binom{d}{2} - d = \binom{d}{2}$,
...
$R = \tau$ (pole-free $E_\infty$ case), the twisting is trivial and
the Schur--Weyl decomposition applies directly, giving
$\ker(\mathrm{av}_n)$ the structure~\eqref{eq:kernel-schur-weyl}.
When $R \neq \tau$ (non-trivial $R$-matrix), the $\Sigma_n$-action
is deformed by the monodromy, the Schur--Weyl decomposition does not
apply in its classical form, and the kernel acquires the richer
structure described in Proposition~\ref{prop:averaging-surplus}.
\end{remark}
...
$R$-matrix-twisted quotient of Proposition~\ref{prop:r-matrix-descent-vol1}.
