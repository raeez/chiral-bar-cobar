...
Associative chiral Koszul duality is not the Francis--Gaitsgory
equivalence between chiral Lie algebras and cocommutative factorization
coalgebras.  Ordered configuration spaces survive, shuffles do not
collapse under symmetric-group quotients, and after linear duality
chiral algebras appear again on the far side.

Let $C = \Barch(A)$ be the ordered bar coalgebra of an associative
chiral algebra~$A$.  The correct coalgebra-side avatar of the diagonal
bimodule~${}_A A_A$ is the \emph{diagonal bicomodule}
\[
C_\Delta,
\]
the coalgebra $C$ equipped with its coproduct on the left and on the
right.  On the algebra side, interval composition is derived tensor
over~$A$ and the annulus is Hochschild homology.  On the coalgebra
side, interval composition is derived cotensor over~$C$, the diagonal
is the composition unit, and the annulus is the self-cotensor:
\[
...
\item The bordered configuration-space construction is now in
\S\ref{sec:bordered-fm} (Construction~\ref{constr:bordered-fm},
Theorem~\ref{thm:bordered-fm-properties}).
Items~(1)--(3) and~(5) of Conjecture~\ref{conj:bordered} are resolved;
see Remark~\ref{rem:bordered-partial-resolution}.
