...
arity-$0$ curvature $m_0=k\lvert 0\rangle$ is non-zero. The
associated R-matrix
$R(z)=e^{k\hbar/z}$
...
contribution is collected separately in the arity-$0$
...
ordered Heisenberg bar coalgebra: it lives in the arity-$0$
...
\item \emph{Affine $\widehat{\mathfrak{g}}_k$
 ...
 $Y(\mathfrak{u}(1))$ becomes $Y_z^\hbar(\mathfrak{g})$. The bar
...
\subsection{The Drinfeld presentation of $Y_z^\hbar(\mathfrak{sl}_2)$}
...
$Y_z^\hbar(\mathfrak{sl}_2)$ is the ordered Koszul dual living on the
open boundary $\bR_t$.
...
\;=\; \frac{k_1(u)}{k_2(u)}.
\end{align}
The modes $\{E_r,\,F_r,\,H_r\}_{r \ge 0}$ generate $Y_z^\hbar(\mathfrak{sl}_2)$
...
\begin{theorem}[Drinfeld presentation of $Y_z^\hbar(\mathfrak{sl}_2)$]
\ClaimStatusProvedHere
\label{thm:drinfeld-yangian-sl2}
The Yangian $Y_z^\hbar(\mathfrak{sl}_2)$, with $\hbar = 1/(k+2)$, is the
...
$Y_z^\hbar(\mathfrak{sl}_2)$, confirming the factorisation is well-defined
over the Yangian.
\end{proof}
...
The twisted coproduct $\Delta_z \colon Y_z^\hbar(\mathfrak{sl}_2)
\to Y_z^\hbar(\mathfrak{sl}_2) \otimes Y_z^\hbar(\mathfrak{sl}_2)$
...
\end{align}
At $z = 0$ this recovers the standard Hopf coproduct of $Y_z^\hbar(\mathfrak{sl}_2)$.
\end{theorem}
...
\begin{theorem}[PBW theorem for $Y_z^\hbar(\mathfrak{sl}_2)$]
\ClaimStatusProvedHere
...
\,\bigr\}
\]
form a basis of $Y_z^\hbar(\mathfrak{sl}_2)$ over~$\bC$. The Hilbert series
with respect to the grading $\deg(X_r) = r$ is
\[
\operatorname{HS}_{Y_z^\hbar(\mathfrak{sl}_2)}(q)
...
$F_{\le n}\,Y_z^\hbar(\mathfrak{sl}_2) = \langle X_{r_1}\cdots X_{r_k}
\mid r_1 + \cdots + r_k \le n \rangle$, and the associated graded
is $\gr\,Y_z^\hbar(\mathfrak{sl}_2) \simeq U(\mathfrak{sl}_2[t])$.
\end{proof}
...
At $\hbar = 0$, the Yangian $Y_z^\hbar(\mathfrak{sl}_2)$ degenerates to the
...
\end{enumerate}
The PBW filtration on $Y_z^\hbar(\mathfrak{sl}_2)$ has associated graded
$\gr\,Y_z^\hbar(\mathfrak{sl}_2) \cong U(\mathfrak{sl}_2[t])$,
...
The affine Yangian $Y_z^\hbar(\mathfrak{sl}_2)$ has three independent
...
$Y_z^\hbar(\mathfrak{g})$. What is the ordered Koszul dual of
$Y_z^\hbar(\mathfrak{g})$?
...
$\Barchord(Y_z^\hbar(\mathfrak{g}))$ would recover
...
$Y_z^\hbar(\mathfrak{g})$ as the scalar $\hbar = 1/(k + h^\vee)$.
...
The ordered bar differential of $Y_z^\hbar(\mathfrak{g})$ uses the
...
dual is the Yangian $Y_z^\hbar(\mathfrak{g})$. This is the
...
$Y_z^\hbar(\mathfrak{g})$ is a \emph{strict} associative
...
RTT presentation of the Yangian $Y_z^\hbar(\mathfrak{g})$:
...
Type & $\mathfrak{g}$ & $\dim\mathfrak{g}$ & $h^\vee$
 & Fund.\ repr.\ dim & $\hbar$ & $\kappa(Y_z^\hbar(\mathfrak{g}))$ \\
\midrule
...
\kappa\bigl(Y_z^\hbar(\mathfrak{g})\bigr)
\;=\;
...
the $R$-matrix of $Y_z^\hbar(\mathfrak{g})$ acting on evaluation
...
The dg-shifted Yangian $Y_z^\hbar(\fg)$ carries a strict filtered
algebra structure.
\end{theorem}
...
Drinfeld generating functions for $Y_z^\hbar(\mathfrak{sl}_3)$
with $\hbar = 1/(k+3)$:
\begin{align*}
...
$U(\mathfrak{sl}_3)$ inside $Y_z^\hbar(\mathfrak{sl}_3)$.
...
The Serre relations of $Y_z^\hbar(\mathfrak{sl}_3)$ arise from degree-$3$
...
\begin{theorem}[RTT presentation for $Y_z^\hbar(\mathfrak{sl}_3)$
...
\in \operatorname{End}(V)\otimes Y_z^\hbar(\mathfrak{sl}_3)[\![u^{-1}]\!],
\]
...
$Y_z^\hbar(\mathfrak{sl}_3)$ is a strict associative algebra.
\end{theorem}
...
group $U_q(\mathfrak{g})$ from the Yangian $Y_z^\hbar(\mathfrak{g})$.
...
\mathrm{Conf}_n(\mathbb{R})$ produces the Yangian $Y_z^\hbar(\mathfrak{g})$,
...
deformation of the Yangian $Y_z^\hbar(\mathfrak{g})$ to the
...
\item At genus~$0$, the ordered bar complex of $V^k(\mathfrak{g})$ on
 ...
 produces the Yangian $Y_z^\hbar(\mathfrak{g})$ as the line-operator
...
\item The degeneration $\tau \to i\infty$ \textup{(}pinching the
 ...
 = Y_z^\hbar(\mathfrak{g})$, in the sense that the KZB connection
...
\hline
Genus $0$, ordered bar & KZ connection & Yangian $Y_z^\hbar(\mathfrak{g})$ \\
...
Q_0^0(\mathfrak{g})\;\simeq\; Y_z^\hbar(\mathfrak{g}).
\end{equation}
...
is isomorphic to the Yangian $Y_z^\hbar(\mathfrak{g})$:
\[
...
The ordered bar complex and the Yangian $Y_z^\hbar(\mathfrak{g})$ classify the
...
\subsection{The Yangian $Y_z^\hbar(\mathfrak{g})$ and the evaluation homomorphism}

\begin{definition}[Drinfeld Yangian]
\label{def:yangian-drinfeld}
The \emph{Yangian} $Y_z^\hbar(\mathfrak{g})$ is the associative
...
\[
\mathrm{ev}_a\colon
Y_z^\hbar(\mathfrak{g})\longrightarrow U(\mathfrak{g}),
\qquad
J^a_0\mapsto J^a,
\quad
J^a_1\mapsto a\,J^a.
...
$Y_z^\hbar(\mathfrak{g})$-module obtained by pulling back
along $\mathrm{ev}_a$:
\[
V_a
:=
...
$x\in Y_z^\hbar(\mathfrak{sl}_2)$, evaluated on
...
finite-dimensional $Y_z^\hbar(\mathfrak{g})$-modules are in
...
\mathcal{C}_{\mathrm{line}}
\;\simeq\;
Y_z^\hbar(\mathfrak{g})\text{-}\mathrm{mod}^{\mathrm{fd}}.
\]
...
is a $Y_z^\hbar(\mathfrak{g})$-module via the coproduct~$\Delta$.
...
is the universal $R$-matrix of $Y_z^\hbar(\mathfrak{g})$
...
\begin{proof}
The coproduct $\Delta$ of $Y_z^\hbar(\mathfrak{g})$ defines
...
$Y_z^\hbar(\mathfrak{g})$-modules is
\begin{equation}\label{eq:grothendieck-yangian}
K_0\!\bigl(Y_z^\hbar(\mathfrak{g})\text{-}\mathrm{mod}^{\mathrm{fd}}\bigr)
\;\simeq\;
...
$Y_z^\hbar(\mathfrak{g})$-module, becomes irreducible in the
Grothendieck group.
\end{remark}
...
$A^{!,\mathrm{ord}}_{\mathrm{line}}
(\widehat{\mathfrak{g}}_k)\simeq Y_z^\hbar(\mathfrak{g})$
...
$\kappa(Y_z^\hbar(\mathfrak{g})) = -\dim(\mathfrak{g})\cdot
(k+h^\vee)/(2h^\vee)$
(Theorem~\ref{thm:km-yangian},
...
so the dg-shifted Yangian $Y_z^\hbar(\fg)$ carries a strict filtered
...
\item Evaluation homomorphism $\mathrm{ev}_a\colon Y_z^\hbar(\fg)\to U(\fg)$
...
Y_z^\hbar(\fg)\text{-}\mathrm{mod}^{\mathrm{fd}}$ on the affine Koszul locus
...
\item Yangian--quantum group genus-$1$ deformation: $Y_z^\hbar(\mathfrak{g})$
...
the Yangian $Y_z^\hbar(\mathfrak{g})$
...
The Yangian $Y_z^\hbar(\mathfrak{g})$ (a classical algebra
