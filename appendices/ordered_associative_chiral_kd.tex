...
comparison below does not use an unordered symmetric structure until
...
The bar--cobar adjunction $\Cobar\dashv\Barch$ on $E_1$-chiral
...
$A\to\Cobar\Barch(A)$, when it exists, is a Koszul or completed
...
$\hbar\Omega\,dz_{ij}/z_{ij}$, the standard
...
The ordered formalism is the input for the geometric constructions below.
\end{remark}
...
\item \emph{Affine $\widehat{\mathfrak{g}}_k$
 ...
 Casimir kernel $k\Omega/z \in \End(\mathfrak{g}\otimes\mathfrak{g})$.
...
nilpotent matrix). The affine is class~L (shadow depth~$3$, formal, matrix).
...
\ClaimStatusProvedHere
\label{def:spectral-drinfeld-class-app}% renamed: avoids multiply-defined with chapters/examples/ version
...
In summary, four of the five inputs are proved;
...
\label{def:evaluation-module-app}% renamed: avoids multiply-defined with chapters/theory/ version
...
of Volume~II (Frontier): genuinely $\mathsf{E}_1$-chiral algebras
