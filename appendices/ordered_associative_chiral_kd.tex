...
\begin{remark}[Master theorem versus quadratic candidate-dual: MA-12]
...
quadratic-only candidate to the master-theorem statement is the
master pattern MA-12. The endpoint hypothesis package required is
...
sequence with $E_1 = H^\bullet(\Barchord(\mathcal{M}_{\mathrm{Wak}}),
d_{\Barch})$ and $d_1 = Q_{\mathrm{DS}}$ converges to
$\Barchord(\widehat{\fg}_k)$ (Volume~I,
...
ordered bar complex $\Barchord(\mathrm{Vir}_c)$ is nonzero for all
...
Drinfeld--Kohno \textup{(}Theorem~\ref{thm:drinfeld-kohno}(ii)\textup{)}
...
Theorem~\texttt{thm:complete-strictification}), yielding a strict
