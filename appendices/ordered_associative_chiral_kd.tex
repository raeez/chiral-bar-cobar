...
For a chosen quadratic presentation $A=T_X(V)/(R)$, set
\[
 A^i:=C_X(s^{-1}V,s^{-2}R),
 \qquad
 q_A\colon A^i\longrightarrow\Barchord(A).
\]
Under the chiral Koszul comparison package $H_{\mathrm{CL}}(A)$, the
map $H^\bullet(q_A)$ is an isomorphism.  The finite-type continuous
dual of the presentation coalgebra is the line-side Koszul-dual algebra
$A^!_{\mathrm{line}}=(A^i)^\vee_{\mathrm{cont}}$; Verdier duality
enters after symmetric/Ran descent.
For affine Kac--Moody inputs this algebra is the dg-shifted Yangian.

\begin{remark}[$\Eone$-chiral notion used in this appendix]
\label{rem:app-ord-assoc-notion-fixed}
\index{$\mathbb{E}_1$-chiral algebra!notion fixed in ordered appendix}
The phrase ``$E_1$-chiral algebra'' in this appendix means a strict
$\Assch$-algebra, equivalently an algebra over the non-$\Sigma$
chiral associative operad. The homotopical refinement is an
$A_\infty$-chiral structure in $\End^{\mathrm{ch}}_A$; passage between
the two is by homotopy transfer under Hypothesis~\ref{hyp:inputs}.
The Etingof--Kazhdan quantum-vertex notion supplies the same
Koszul-dual on the Koszul locus only after the corresponding chiral
quantum-group comparison has been proved. The ordered associative
comparison does not use an unordered symmetric structure until
the strong-unitary descent datum has been constructed.
\end{remark}

\begin{remark}[Direction of bar--cobar comparison]
\label{rem:app-ord-assoc-koszul-reflection-bridge}
\index{bar--cobar adjunction!ordered associative direction}
The ordered bar--cobar adjunction $\Cobar\dashv\Barchord$ on $E_1$-chiral
algebras gives the algebra-side counit
\[
\varepsilon_A\colon \Cobar\Barchord(A)\xrightarrow{\sim} A
\]
in the strongly admissible regime. The coalgebra-side unit has the
opposite direction, $C\to\Barch\Cobar(C)$. A reverse algebra map
$A\to\Cobar\Barchord(A)$, when it exists, is a Koszul or completed
inversion comparison; it is not a unit of $\Cobar\dashv\Barch$.
The $R$-twisted descent relates $\Barchord(A)$ to the symmetric bar of
Theorem~A only after braid monodromy has been promoted to a
strong-unitary $\Sigma_n$-equivariant descent datum. Before that point
one has a braid-monodromic local system, not a symmetric bar complex.
\end{remark}
...
\]
The associative dual carries the open boundary formalism.
The ordered bar complex is the resolving $\Eone$-chiral coalgebra.
The presentation coalgebra $A^i=C_X(s^{-1}V,s^{-2}R)$ maps to it by
$q_A$.  Under $H_{\mathrm{CL}}(A)$, the comparison
$H^\bullet(q_A)$ is an isomorphism, and the line-side Koszul-dual algebra is
$A^!_{\mathrm{line}}=(A^i)^\vee_{\mathrm{cont}}$.
In finite-type ordered statements below, a bare $A^!$ abbreviates this
line-side continuous dual; a closed/Ran Verdier dual is never inferred
from that notation and is named explicitly whenever it is used.
It plays the role that the Steinberg variety plays for the
Hecke algebra: a geometric convolution object whose cohomology
and finite-type dual carry the algebraic duality.
...
In this appendix, unadorned $\Barch(A)$ in the associative lemmas means
...
passage to coderived categories is legitimate.
\end{remark}
...
\rho=
\tau_{C,M}\circ
(\eps\otimes \id_{C^{\cop}}\otimes \id_M)\rho^e
:
M\to M\otimes C.
\]
...
Let $A$ be a strongly admissible associative chiral algebra and $C=\Barchord(A)$.
...
N(B^\Delta_\bullet(A))
\]
are canonically identified with the reduced ordered chiral bar complex $\Barchord(A)$.
\end{definition}
...
\Barchord(A)\widehat\otimes \Barchord(B)\longrightarrow \Barchord(A\chotimes B),
\]
\[
\mathsf{AW}^{\mathrm{ch}}_{A,B}:
\Barchord(A\chotimes B)\longrightarrow \Barchord(A)\widehat\otimes \Barchord(B),
\]
such that
\[
...
\[
\Barchord(A)\widehat\otimes \Barchord(B)\xrightarrow{\sim}\Barchord(A\chotimes B).
\]
\end{theorem}
...
\[
N(B^\Delta_\bullet(A))\cong \Barchord(A),
\qquad
N(B^\Delta_\bullet(B))\cong \Barchord(B),
\qquad
N(B^\Delta_\bullet(A\chotimes B))\cong \Barchord(A\chotimes B),
\]
...
the suspended Koszul sign. The descent recovers the ordinary
...
\begin{construction}[Collision kernel and represented Kohno connection]
...
The bar differential on $\Barchord(A)$ extracts, at each collision
$z_i=z_j$, an ordered operator
\[
 K^{\mathrm{coll}}_{A,ij}(z_i-z_j)
 \in\End_A(2)\otimes\mathcal O(*\Delta).
\]
By the bar-kernel convention, its pole orders are one less than the
corresponding OPE pole orders. A represented Kohno connection requires
the package $H_{\mathrm{CYBE}}^{\mathrm{rep}}(A;W)$: a map
$\rho_A\colon\End_A(2)\to\End(W^{\otimes2})$ such that
$r_A(z):=\rho_A(K_A^{\mathrm{coll}}(z))$ has infinitesimal-braid
insertions. Under this package the connection on the trivial bundle
...
r_{A,ij}(z_i-z_j)\,d(z_i-z_j).
\end{equation}
Here $r_{A,ij}$ acts in the $(i,j)$ tensor slots of~$W^{\otimes n}$
and $r_{A,ij}(z_{ij})\,dz_{ij}$ is the connection $1$-form
on $\mathrm{Conf}_n^{\mathrm{ord}}(\mathbb{C})$.
For instance, when
$K_{\cH}^{\mathrm{coll}}(z)=k\Omega_{\cH}/z$ with
$\Omega_{\cH}=J\otimes J$ on the Heisenberg current line, its
rank-one coefficient gives $k\,dz/z$. For affine Kac--Moody the
collision kernel in trace-form normalization is
$K_{k,\fg}^{\mathrm{coll}}(z)=k\Omega_{\mathrm{tr}}/z$; the
KZ/Yangian form $\Omega_{\mathrm{KZ}}/((k+h^\vee)z)$ is the rescaled
Casimir-and-coupling coordinate on the represented comparison surface.

\emph{Flatness.}
Let
\[
A_\nabla=\sum_{i<j}r_{A,ij}(z_{ij})\,dz_{ij}.
\]
...
\]
equivalently $dA_\nabla-A_\nabla\wedge A_\nabla=0$ for the
degree-$1$ matrix-valued form convention
$[A_\nabla,A_\nabla]=2A_\nabla\wedge A_\nabla$.
For the logarithmic KZ/Kohno kernels
$A_\nabla=\hbar\sum_{i<j}\Omega_{ij}\,\omega_{ij}$ with
$\omega_{ij}=d\log(z_i-z_j)$, this equation is the Arnold--Kohno
calculation: $d\omega_{ij}=0$, the Arnold relation
\[
\omega_{ij}\wedge\omega_{jk}
+\omega_{jk}\wedge\omega_{ki}
+\omega_{ki}\wedge\omega_{ij}=0
\]
combines with the infinitesimal braid relations
\[
[\Omega_{ij},\Omega_{ik}+\Omega_{jk}]=0,\qquad
[\Omega_{ij},\Omega_{kl}]=0
\quad(\{i,j\}\cap\{k,l\}=\varnothing).
\]
Equivalently, in spectral notation this imposes the classical
Yang--Baxter constraint
...
\;=\;0,
\end{equation}
together with the differential identities of the logarithmic forms.
Its proof uses the full Arnold--Kohno relation together with the
represented infinitesimal-braid identities.
Flatness of~$\nabla$ is the represented-CYBE clause of
$H_{\mathrm{CYBE}}^{\mathrm{rep}}(A;W)$ and is part of the
$R$-matrix descent datum.
...
\;=\;
\mathcal{P}\exp\!\Bigl(
\oint_\gamma r_A(z)\,dz
\Bigr)
\;\in\;
\operatorname{End}(W^{\otimes2})[\![z^{-1}]\!],
\end{equation}
...
Yang--Baxter equation~\eqref{eq:cybe-vol1} in the KZ/Kohno cases.
Equivalently, the half-braid transports used for adjacent deck
transformations give a $B_n$-representation precisely when the
corresponding braid relations hold.
\end{construction}
...
of the braid group, provided the braid relation holds.
...
in the standard affine examples. It is not a consequence of the Yang--Baxter equation alone:
...
\begin{remark}[Binary MC kernel and represented $r$-matrix]
\label{rem:r-matrix-mc-vol1}
\index{R-matrix!Maurer--Cartan interpretation}
The binary genus-$0$ collision projection of the ordered universal MC
element is
\[
K_A^{\mathrm{coll}}(z)
:=\Res^{\mathrm{coll}}_{0,2}(\Theta_A^{E_1})
\in\End_A(2)\widehat\otimes\mathcal O(*\Delta).
\]
The governing equation is
\[
D\Theta_A^{E_1}
+\tfrac{1}{2}[\Theta_A^{E_1},\Theta_A^{E_1}]=0.
\]
Projecting its degree-$3$ part to triple binary collisions gives the
operadic binary-boundary relation.  The package
$H_{\mathrm{CYBE}}^{\mathrm{rep}}(A;W)$ carries its three represented
insertions to CYBE~\eqref{eq:cybe-vol1}; the YBE on $R(z)$ is its
monodromy form. The $R$-matrix is
the descent datum because the convolution dg~Lie algebra governing
$\Sigma_n$-equivariant structures on local systems over
$\mathrm{Conf}_n$ is the Gerstenhaber--Schack complex of bilinear
operations, and Maurer--Cartan elements in it parametrise the
nontrivial descents.
\end{remark}

\begin{remark}[Motivic content of ordered-to-symmetric descent]
...
forgets the ordered iterated-integral classes that are not invariant
under the $R$-twisted $\Sigma_n$-action. This is the sense in which it
is a motivic projection.
...
\noindent
The scalar symmetric shadows used in this volume -- curvature
$\kappa$, complementarity class~$C$, and the shadow obstruction
tower~$Q$ -- are rational residues, hence periods of the trivial
motive~$\mathbb{Q}(0)$. They do not see the non-trivial mixed Tate
periods carried by ordered associators. The kernel
$\ker(\mathrm{av})$ contains this ordered surplus; the statement is not
that every object in the symmetric bar has trivial motivic content, but
that these scalar symmetric shadows do.
...
The ordered bar complex retains the transcendental arithmetic of the
configuration-space integrals. The scalar symmetric shadows record the
rational part left after ordered monodromy and associator data have
been passed through the $R$-twisted descent.
\end{remark}
...
\[
\mathsf{R}_A:\Barchord(A^{\op})\xrightarrow{\sim}\Barchord(A)^{\cop}.
\]
If $A$ is finite-type Koszul so that continuous duality of $A^i$ returns an associative
chiral algebra, then
\[
(A^{\op})^!_{\mathrm{line}}\simeq (A^!_{\mathrm{line}})^{\op}.
\]
\end{theorem}
...
The coproduct on $\Barchord(A)$ is deconcatenation. Reversal sends a cut after the $p$-th letter to a
...
opposite coalgebra. The claim about line-side continuous duals follows because finite-type
continuous duality exchanges opposite coproduct with opposite product.
\end{proof}
...
\[
\iota^!:A^!_{\mathrm{line}}\to (A^!_{\mathrm{line}})^{\op}.
\]
...
\label{rem:app-ordered-verdier-nonexistence}
\index{Verdier duality!ordered bar}
The Verdier intertwining
$\mathbb{D}_{\Ran}\,\barBch(\cA) \simeq \barBch(\cA^!)$
is a statement about the symmetric bar on unordered $\Ran(X)$.
The ordered analogue $\mathbb{D}_{\Ran}\,\barB^{\mathrm{ord}}(\cA)$
is undefined: Verdier duality requires the factorization/cosheaf
property on symmetric monoidal disjoint-open covers, which ordered
configurations on $\mathrm{Conf}^<_n(X)$ do not provide.
The correct $\Eone$ analogue is the opposite-coalgebra duality of
Theorem~\ref{thm:opposite}:
$\barB^{\mathrm{ord}}(\cA)^{\mathrm{op}} \simeq
\barB^{\mathrm{ord}}(\cA^{\mathrm{op}})$.
See Remark~\ref{rem:ordered-verdier-nonexistence} for further
discussion.
\end{remark}
...
Let $A$ be strongly admissible, and assume $(A,A^{\op})$ is EZ-admissible. Set $C=\Barchord(A)$.
Then there is a canonical quasi-isomorphism of conilpotent dg coalgebras
\[
\Barchord(A^e)\xrightarrow{\sim} C^e=C\widehat\otimes C^{\cop}.
\]
If $A$ is finite-type Koszul, then
\[
(A^e)^!_{\mathrm{line}}\simeq (A^!_{\mathrm{line}})^e.
\]
\end{corollary}
...
\[
\Barchord(A^e)=\Barchord(A\chotimes A^{\op})\xrightarrow{\sim}
\Barchord(A)\widehat\otimes \Barchord(A^{\op}).
\]
Now apply Theorem~\ref{thm:opposite} to the second factor:
\[
\Barchord(A^{\op})\xrightarrow{\sim}\Barchord(A)^{\cop}.
\]
This proves the coalgebra statement. The algebra statement follows after finite-type continuous
duality of the Koszul-dual coalgebras.
\end{proof}
...
\label{def:Kbi}
Let $A$ be strongly admissible, let $C=\Barchord(A)$, and let $M$ be an $A$-bimodule.
...
\]
Inside the reduced ordered bar complex $\Barchord(A_\eps)$, let $T_A^{\mathrm{bar}}(M)$ be the
...
\[
\Barchord(A_\eps)\cong \Barchord(A)\oplus \eps\, T_A^{\mathrm{bar}}(M).
\]
The bar tangent $T_A^{\mathrm{bar}}(M)$ is naturally a $C$-bicomodule.
\end{proposition}

\begin{proof}
A reduced bar word in $\Barchord(A_\eps)$ contains some number of $\eps M$-slots.
...
so the one-defect sector is naturally a bicomodule over the ordinary bar coalgebra $C=\Barchord(A)$.
\end{proof}
...
\[
\Barchord(A_\eps)\cong C\oplus \eps\,\KK_A^{\mathrm{bi}}(M).
\]
If $A$ is finite-type Koszul, then after finite-type continuous duality
\[
(A_\eps)^!_{\mathrm{line}}\cong A^!_{\mathrm{line}}\oplus \eps\, M^!
...
\[
\coHoch^{\mathrm{ch}}_\bullet(\Barchord(A_\eps))
\cong
...
Let $A$ be strongly admissible and set $C=\Barchord(A)$.
\begin{enumerate}[label=(\alph*)]
...
\[
\Barchord(A^e)\xrightarrow{\sim} C^e.
\]
...
Let $A$ be strongly admissible, let $(A,A^{\op})$ be EZ-admissible, and set $C=\Barchord(A)$.
...
\begin{remark}[Ordered open formalism and symmetric descent]
The theorem gives the algebraic ordered open package. A fully symmetric
one-brane open TCFT requires additional structure that symmetrizes ordered fusion, for example a
boundary symmetry datum or a direct geometric realization by bordered configuration spaces.
The ordered formalism supplies the input for the geometric constructions.
\end{remark}
...
\ClaimStatusProvedHere
\label{thm:master}
\emph{Type signature:} (Open quadrant; ordered associative chiral
duality; Beilinson level $1\!\leftrightarrow\!2$ with bicomodule
extension to level $1\!\leftrightarrow\!3$; HP = strongly admissible
$+$ EZ-admissible $+$ pro-nilpotent PBW-complete $+$ conilpotent
ordered bar coalgebra $+$ continuous finite graded duality).
...
\[
\Barchord(A\chotimes B)\simeq \Barchord(A)\widehat\otimes \Barchord(B)
\]
...
\[
\Barchord(A_\eps)\cong C\oplus \eps\,\KK_A^{\mathrm{bi}}(M)\qquad (\mathrm{mod}\ \eps^2).
\]
...
\begin{remark}[Master theorem versus quadratic candidate-dual]
\label{rem:master-vs-quadratic-glz}
\index{quadratic candidate versus full theorem}
The master theorem is a bar--cobar inversion theorem
on the strongly-admissible locus, with the full hypothesis package
named in the type signature. It is \emph{not} a quadratic-only
identification: the proof passes through the deconcatenation
coproduct on every bar degree, the bicomodule equivalence at every
weight, and the chiral Hochschild/coHochschild duality, none of
which are determined by the quadratic relations alone.

The Gui--Li--Zeng quadratic chiral
duality~\cite{GLZ22} produces a
\emph{candidate} dual $\cA^!_{\mathrm{GLZ}}$ with a Maurer--Cartan
comparison map (cf.\
Theorem~\textup{\ref{thm:glz-quadratic-duality-scope}} in
Appendix~\textup{\ref{app:dual-methodology}}); this is one ingredient
of the full bar--cobar inversion. Bare promotion of the
quadratic-only candidate to the master-theorem statement omits the
endpoint hypothesis package. The required package is
the strongly-admissible regime of \S\ref{sec:setup} together with
$E_2$-degeneration of the PBW spectral sequence; without these,
the master theorem fails and only the quadratic-candidate identity
survives.
\end{remark}
...
configuration spaces whose geometry we make explicit.
...
This construction gives the operation space, not yet a minimal
...
cancellation is the suspended Stasheff identity for the OPE operations
...
\begin{lemma}[Coderivation compatibility]
\ClaimStatusProvedHere
\label{lem:deconcatenation-coderivation}
\index{bar differential!coderivation compatibility}
Let
\[
d_{\Barchord}
=d_{\mathrm{int}}+\sum_{s\ge 1}\widehat m_s^{\mathrm{ch}}
\]
be the differential part of the ordered bar construction, where
each $\widehat m_s^{\mathrm{ch}}$ is the coderivation of
$T^c(s^{-1}\bar A)$ extending the suspended $s$-ary collision
operation. The deconcatenation coproduct satisfies
\[
\Delta\circ d_{\Barchord}
=
(d_{\Barchord}\otimes\id+\id\otimes d_{\Barchord})\circ\Delta .
\]
The curvature term $m_0$, when present, is a coaugmentation term and
is not included in this coderivation identity.
\end{lemma}

\begin{proof}
Coderivations of the cofree conilpotent coalgebra
$T^c(s^{-1}\bar A)$ are determined by their projection to the
cogenerators $s^{-1}\bar A$. The suspended operations
$b_1(s^{-1}a)=-s^{-1}(d_Aa)$ and
$b_s\colon (s^{-1}\bar A)^{\otimes s}\to s^{-1}\bar A$
therefore have unique coderivation extensions
$\widehat m_s^{\mathrm{ch}}$ for the deconcatenation coproduct. Each
extension satisfies
\[
\Delta\circ\widehat m_s^{\mathrm{ch}}
=
(\widehat m_s^{\mathrm{ch}}\otimes\id
+\id\otimes\widehat m_s^{\mathrm{ch}})\circ\Delta ,
\]
with the Koszul sign already encoded by the tensor product
differential on suspended factors. Summing over $s$ gives the stated
identity for $d_{\Barchord}$.

Equivalently, on a bar word, a collision operation wholly to the left
or wholly to the right of a cut appears on the corresponding side of
the tensor product. If the cut separates the colliding block, the term
is absent on both sides; after contraction the two adjacent cuts around
the merged block give the two coaugmentation terms already present in
the coderivation formula. This is the chain-level compatibility between
holomorphic collision and topological interval-cutting.
\end{proof}
...
suspended Koszul sign. The descent then recovers the ordinary
...
The ordered bar complex $\Barchord(\cH_k)$ is the cofree conilpotent
...
The ordered bar complex $\Barchord(\cH_k)$ is a \emph{curved}
...
\item \textbf{Curved Swiss-cheese structure.}\enspace
...
\emph{Type signature.} Open quadrant; ordered bar at level~$2$ and
Fock-module representation at level~$4$; package
$H_{\mathrm{CYBE}}^{\mathrm{rep}}(\cH_k;\mathrm{Fock}_k)$ supplied by
commuting rank-one tensor insertions.
The collision residue of the Heisenberg OPE on
$\Conf_2^{<}(\bC)$ is
\begin{equation}\label{eq:heisenberg-collision-residue}
K_{\cH_k}^{\mathrm{coll}}(z)
\;=\;
\frac{k}{z},
\end{equation}
a simple pole in the collision residue arising from the
double pole in the OPE \textup{(}the $d\log$~kernel absorbs
one pole order\textup{)}.
...
R(z) \;=\; e^{k\hbar/z}.
\end{equation}
The Fock representation sends $K_{\cH_k}^{\mathrm{coll}}$ to the
scalar connection form $k\,dz/z$.  Commutativity of its three tensor
insertions proves CYBE, and its monodromy gives the displayed
$R$-matrix.
\end{theorem}

\begin{proof}
The OPE $J(z)J(w) \sim k/(z-w)^2$ has a double pole.
The bar propagator is $\omega_{12}=d\log(z_1-z_2)$. In the
normal coordinate $z=z_1-z_2$, contraction with this logarithmic
kernel lowers the double OPE pole by one order and extracts the
simple collision kernel
\[
K_{\cH_k}^{\mathrm{coll}}(z)
\;=\;
\frac{k}{z}.
...
\emph{Type signature:} (Open quadrant; ordered associative chiral
duality; Beilinson level $1\!\leftrightarrow\!2$; HP =
class~$\mathbf G$ \textup{(}rank-$1$ abelian, $r_{\max}=2$\textup{)}
+ formal bar complex with $d_{\Barch}=0$ + non-vanishing level
$k\neq 0$).
...
\begin{proof}
Since the bar differential vanishes on $\Barchord(\cH_k)$,
the Koszul dual is the linear dual of the bar coalgebra:
\[
\cH_k^{!,\mathrm{ord}}
\;=\;
\bigl(\Barchord(\cH_k)\bigr)^\vee
\;=\;
...
the $\Ainf$-structure on $H^\bullet(\Barchord(\cH_k))$
...
strong-unitary $R$-matrix-twisted $\Sigma_n$-coinvariants: since
the monodromy part $R(z) = e^{k\hbar/z}$ is not the identity,
the descent $\Barchord(\cH_k)\to\barB^{\Sigma}(\cH_k)$
...
($E_\infty$-chiral with nontrivial monodromy part $R(z)\neq\id$,
derived from the local OPE). Tier~(i) would require
$R(z)=\id$ in the monodromy part, so the full braiding is the
Koszul-signed flip (no OPE poles); tier~(iii) would require $R(z)$ to be
...
\item \emph{$\beta\gamma$ system
 ...
 and the pole-valued collision residue drops: the $d\log$ kernel absorbs
 the simple OPE pole. The regular contact operator $\Theta$ satisfies
 $\Theta^2=0$, and the bar differential remains trivial
 ($d_{\Barch}=0$) because the zeroth mode product
...
\item \emph{Affine $\widehat{\mathfrak{g}}_k$
 ...
 $1+\Omega/((k+h^\vee)z)+O((k+h^\vee)^{-2})$ after KZ rescaling. The
 abelian ordered RTT model is replaced by the RTT/FRT Yangian attached
 to $(\mathfrak g,V)$ only after choosing the Yangian spectral
 parameter and quantized $R$-matrix. The affine bar differential
 acquires nontrivial components from $[X,Y]_{(0)}\ne0$, but the affine
 vertex-algebra shadow remains class~L with $r_{\max}=3$.
...
\item \emph{Virasoro $(\S\ref{subsec:virasoro-ordered-bar})$:}
 ...
 $r_{\max} = \infty$ (class~M). This is a Virasoro/W-algebra statement,
 not a degeneration of an ordinary Drinfeld Yangian to the single
 generator~$T$: the Yangian/RTT structure and the stress-tensor shadow
 live in different ordered ambients. The complexity migrates from the
 affine Lie bracket lane to the Virasoro bar differential.
\end{enumerate}

\noindent
The four witnesses are separated by one structural parameter at a time:
the Heisenberg is class~G (shadow depth~$2$, formal,
scalar). The $\beta\gamma$ system is class~C (shadow depth~$4$, formal,
regular nilpotent contact). The affine is class~L (shadow depth~$3$, formal, matrix).
The Virasoro is class~M (shadow depth~$\infty$, non-formal, collapsed rank).
...
\subsection{The \texorpdfstring{$\beta\gamma$}{beta-gamma} and
...
operator is retained as a constant nilpotent transport, its exponential is
...
transport terminates at linear order because $\Theta^2=0$. Koszul dual:
the antisymmetric
...
the regular contact transport terminates at linear order. This dichotomy
reflects the composite-closed support-packet classification: the
Heisenberg packet is binary and scalar (class~G), whereas the
...
H^\bullet\bigl(
 \Barchord(\mathcal{M}_{\mathrm{Wak}}),\;
 d_{\Barch} + Q_{\mathrm{DS}}\bigr)
\;\simeq\; \Barchord(\widehat{\fg}_k),
\end{equation}
...
\begin{proof}
Under the stated hypotheses, $Q_{\mathrm{DS}}$ lifts to the ordered bar
coalgebra and is compatible with the PBW filtration. The resulting
spectral sequence has
$E_1=H^\bullet(\Barchord(\mathcal{M}_{\mathrm{Wak}}),d_{\Barch})$ and
$d_1=Q_{\mathrm{DS}}$. Its convergence is part of the hypothesis
package, and its abutment is the reduced ordered bar model.
...
$\cH_k$ & $e^{k\hbar/z}$ \textup{(scalar)}
 ...
 & $\checkmark$ \\[2pt]
$\beta\gamma$ & $\id$ \textup{(contact $\Theta$)}
 & $4$ & $\mathbf{C}$
 & $\bigwedge^*(\beta^*,\gamma^*)$
 & $\checkmark$ \\[2pt]
$bc$ & $\id$ \textup{(contact $\Theta_{bc}$)}
 & $4$ & $\mathbf{C}$
 & $\mathrm{Sym}^*(b^*,c^*)$
 & $\checkmark$ \\[2pt]
...
$\Barchord(\Vlat_\Lambda^\varepsilon)$ has:
...
$\Barch \to \barBgeom$
is the strong-unitary R-matrix-twisted $\Sigma_n$-quotient, with twist
...
and the ordered bar complex $\Barchord(\Vlat_\Lambda^\varepsilon)$ has:
...
$\varepsilon(\alpha,\beta)^{-1}$, and finite-type continuous duality
recovers the lattice algebra with inverse cocycle. Self-duality
$\Lambda = \Lambda^*$ ensures the dual lives on the same lattice.
...
three-tier picture:
\begin{enumerate}[label=(\roman*)]
\item \emph{Pole-free commutative} (monodromy part $R=\id$):
...
\item \emph{$\Einf$ with poles} (nontrivial $R$, derived from locality):
...
\item \emph{Genuinely $\Eone$} (nontrivial $R$, independent input):
 ...
 independent braiding datum, and a symmetric quotient exists only
 after the braid action satisfies strong unitarity.
\end{enumerate}
...
The ordered bar complex $\Barchord(V_k(\mathfrak{sl}_2))$ produces the
...
\item\label{app-drinfeld-sl2-D1} $[H_r, H_s] = 0$ for all $r,s \ge 0$.

\item\label{app-drinfeld-sl2-D2}
$[H(u), E(v)] \;=\; \dfrac{2\hbar}{u-v}\bigl(E(u) - E(v)\bigr)$.

\item\label{app-drinfeld-sl2-D3} $[H(u), F(v)] \;=\; -\,\dfrac{2\hbar}{u-v}\bigl(F(u) - F(v)\bigr)$.

\item\label{app-drinfeld-sl2-D4} $[E(u), F(v)] \;=\; \dfrac{1}{u-v}\bigl(H(u) - H(v)\bigr)$.

\item\label{app-drinfeld-sl2-D5} $(u{-}v)\,[E(u), E(v)]
\;=\; \hbar\bigl(E(u)\,E(v) + E(v)\,E(u)\bigr)$
\textup{(}deformed Serre relation\textup{)}.

\item\label{app-drinfeld-sl2-D6} $(u{-}v)\,[F(u), F(v)]
...
\begin{proof}
Relations~\ref{app-drinfeld-sl2-D1}--\ref{app-drinfeld-sl2-D4} are extracted from the Gauss
...
diagonal entries produce~\ref{app-drinfeld-sl2-D1}, the off-diagonal--diagonal
cross-terms produce~\ref{app-drinfeld-sl2-D2}--\ref{app-drinfeld-sl2-D3}, and the purely off-diagonal
cross-terms produce~\ref{app-drinfeld-sl2-D4}.

For \ref{app-drinfeld-sl2-D1}: the RTT relations $[A(u),A(v)] = 0 = [D(u),D(v)]$
imply that $k_1(u),k_2(u)$ (and hence $H(u) = k_1(u)/k_2(u)$)
generate a commutative subalgebra.

For \ref{app-drinfeld-sl2-D2}: the RTT relation
...
residual identity in the off-diagonal is precisely~\ref{app-drinfeld-sl2-D2}.
Relation~\ref{app-drinfeld-sl2-D3} follows by the same argument applied to
$(u{-}v)[A(u),C(v)] = C(v)A(u) - C(u)A(v)$.

For \ref{app-drinfeld-sl2-D4}: the RTT relation
$(u{-}v)[B(u),C(v)] = D(v)A(u) - D(u)A(v)$
extracts, via Gauss decomposition, to
$(u{-}v)[E(u),F(v)] = H(u) - H(v)$.

For \ref{app-drinfeld-sl2-D5} and \ref{app-drinfeld-sl2-D6}: the RTT relation $[B(u),B(v)] = 0$
...
The sign in~\ref{app-drinfeld-sl2-D6} follows from $[C(u),C(v)] = 0$ by the same
...
\begin{proof}
The PBW property follows from the Drinfeld relations~\ref{app-drinfeld-sl2-D1}--\ref{app-drinfeld-sl2-D6}
by a standard diamond lemma argument: the leading terms of
\ref{app-drinfeld-sl2-D2}--\ref{app-drinfeld-sl2-D6} (in the degree-lexicographic order
...
each contribute a factor of $\prod_{d \ge 1}(1-q^d)^{-1}$ to the
...
\begin{enumerate}[label=\textup{(\roman*)}]
\item Relations \ref{app-drinfeld-sl2-D2}--\ref{app-drinfeld-sl2-D3} at $\hbar = 0$ become
...
\item Relation \ref{app-drinfeld-sl2-D4} at $\hbar = 0$ becomes
$[E_r, F_s] = H_{r+s}$, i.e.\
$[e \otimes t^r,\, f \otimes t^s] = h \otimes t^{r+s}$.
\item Relations \ref{app-drinfeld-sl2-D5}--\ref{app-drinfeld-sl2-D6} at $\hbar = 0$ become
...
\begin{proof}
Setting $\hbar = 0$ in relations~\ref{app-drinfeld-sl2-D1}--\ref{app-drinfeld-sl2-D6} is immediate.
For (i): \ref{app-drinfeld-sl2-D2} becomes $[H(u),E(v)] = 0$ at $\hbar = 0$;
expanding $H(u) = 1 + \hbar\sum H_r u^{-r-1}$ and taking
the leading $\hbar$-linear term recovers
$[h_r, e_s] = 2\,e_{r+s}$.
For (ii): \ref{app-drinfeld-sl2-D4} is $\hbar$-independent.
For (iii): the right-hand side of \ref{app-drinfeld-sl2-D5} vanishes at $\hbar = 0$,
giving $[E(u),E(v)] = 0$.
...
The identification $\hbar = 1/(k+2)$ has a precise KZ-normalisation
origin. The trace-form affine collision residue is
$r^{\mathrm{coll}}(z)=k\Omega_{\mathrm{tr}}/z$; after the
Casimir-and-coupling rescaling of the KZ connection it becomes the
spectral Yangian datum $r_{\mathrm{KZ}}(z)=\Omega/((k+h^\vee)z)$,
...
\subsubsection*{The triple-pole residue}
...
$r^{\mathrm{coll}}_{\mathrm{KM}}(z)=k\Omega_{\mathrm{tr}}/z$ is a
simple pole in trace-form normalization
...
The collapse is Lie-algebraic: the gravitational Yangian has no
independent higher-level data, no non-primitive coproduct, and no
Drinfeld partners. As a Lie algebra it is $\mathrm{Vir}_{26-c}$.
The bar differential still carries higher homotopy.

\subsubsection*{The infinite Stasheff tower}
...
Non-formality is a property of the \emph{bar complex}, not of the
algebra's operadic type. The universal $T$-line has nonzero transferred
...
$X$, and write $A^!_{\mathrm{SC}}$ for the two-colour
Swiss-cheese Koszul dual. Then
\[
(A^!_{\mathrm{SC}})^!_{\mathrm{SC}}\simeq A,
\]
recovering all central extensions and higher-pole data.
\end{theorem}

\begin{proof}
The two-colour duality is not the ordered bar--cobar inversion
$\Omega\Barchord(A)\simeq A$ applied twice. It operates on both colours:
the \emph{closed colour}
...
dualised by finite-type continuous linear duality) carries the spectral
data. Verdier/Ran duality is involutive on the closed constructible
perfect side, and finite-type continuous duality is involutive on the
open graded pieces. Hypothesis~\ref{hyp:inputs}(I) supplies the
chain-level bar--cobar reconstruction on each colour; the involution
comes from combining that reconstruction with the two duality
involutions, not from identifying $\Omega\Barchord$ with Koszul duality.
For $\widehat{\mathfrak{g}}_k$: the closed-colour Verdier dual is
$\widehat{\mathfrak{g}}_{-k-2h^\vee}$ (Feigin--Frenkel dual level), and
the second closed-colour dual returns to $\widehat{\mathfrak{g}}_k$ with
the central extension intact, while the open colour keeps the Yangian
spectral datum.
\end{proof}
...
\emph{Type signature:} (Open quadrant; ordered associative
chiral duality; Beilinson level $1\!\leftrightarrow\!2$; HP =
non-critical level $k\neq -h^\vee$ + class~$\mathbf L$
$E_2$-degeneration + root-space one-dimensionality
[\textup{cf.}~Theorem~\textup{\ref{thm:root-space-one-dim-v1}}] +
spectral Drinfeld class $[\delta] = 0$
[\textup{cf.}~Theorem~\textup{\ref{thm:complete-strictification-v1}}]
+ Drinfeld classification endpoint~\cite{Drinfeld88}).
...
\end{enumerate}
The class~$\mathbf L$ endpoint hypothesis is non-trivial: the
quadratic identification of the open-colour dual with the Yangian
is the candidate-dual statement \textup{(}Step~5 below\textup{)};
its lift to the bar--cobar inversion theorem requires
$E_2$-degeneration of the PBW spectral sequence, the spectral
Drinfeld class to vanish, and Drinfeld's classification of Yangians
as the unique non-trivial Hopf-algebra deformation of $U(\mathfrak{g}[t])$
realising the rational $r$-matrix $r(z) = \hbar\Omega/z$.
\end{theorem}
...
\end{equation}
Substituting
$r^{\mathrm{KZ}}_{ij}(z)=\Omega_{ij}/((k+h^\vee)z)$, the three
...
residue with a simple pole $k\Omega_{\mathrm{tr}}/z$ in trace-form
normalization, equivalently the KZ-normalized spectral term
$\Omega/((k+h^\vee)z)$. The simple-pole bracket term supplies
$d_{\Barch}$ rather than the pole-valued $r$-matrix. This
...
genuine frontier is Kac--Moody algebras
...
\ClaimStatusProvedHere
\label{def:spectral-drinfeld-class-app}
...
The bar-cobar counit $\varepsilon\colon\Omega(\Barchord(\cA))\to\cA$
extracts the chiral algebra from its bar complex (a chiral-product
...
$\Conf_2^<(\bC)$, where
$r^{\mathrm{KZ}}(z)=\Omega/((k+h^\vee)z)$ is the KZ-normalized
classical $r$-matrix~\eqref{eq:km-collision-residue}; the underlying
trace-form collision residue is $k\Omega_{\mathrm{tr}}/z$.
...
\label{thm:r-sp4}
\label{thm:r-g2}
For each of the rank-$2$ types, the trace-form collision residue is
$k\Omega_{\mathrm{tr}}/z$ and the KZ-normalized spectral $r$-matrix is
$\Omega/((k{+}h^\vee)z)$,
...
The genus-$0$ Yangian is the additive stage: the $B$-cycle
...
\ClaimStatusProvedHere{} for the affine lineage]
\label{thm:drinfeld-kohno-appendix}
...
\begin{equation}\label{eq:dk-appendix}
\rho_n^{\mathrm{KZ}}
\;\simeq\;
...
\emph{Type signature:} (Open quadrant; bar-monodromy; Beilinson
level $1\!\rightarrow\!4$ at genus~$0,1$; HP = affine lineage
\textup{(}simple~$\mathfrak{g}$, generic $k\neq -h^\vee$\textup{)}
+ class~$\mathbf L$ one-loop collapse + Drinfeld--Kohno
\textup{(}genus~$0$\textup{)} + Etingof--Schiffmann /
Calaque--Enriquez--Etingof \textup{(}genus~$1$\textup{)}).
Let $\mathfrak{g}$ be a simple Lie algebra and $k \in \mathbb{C}
...
\begin{remark}[Degeneration mechanism]
At genus~$0$, the bar differential on $\mathrm{FM}_n(\mathbb{C})$
encodes the chiral OPE and the $E_1$ coproduct on
$\mathrm{Conf}_n(\mathbb{R})$ encodes associative composition; the
line algebra is the Yangian of the holographic dictionary
\textup{(}Volume~II, Corollary~\ref*{cor:holographic-dictionary}\textup{)}.
At genus~$1$, Theorem~\ref{thm:b-cycle-quantum-group} and
Drinfeld--Kohno \textup{(}Theorem~\ref{thm:drinfeld-kohno-appendix}(ii)\textup{)}
identify the full KZB monodromy with $U_q(\mathfrak{g})$.
The degeneration $\wp_1(z,\tau)\to 1/z+O(z)$ as
$\tau\to i\infty$ sends KZB to KZ and turns the full-cycle factor
$e^{2\pi i\hbar}=q^2$ into the exponential of the Yangian parameter.
\end{remark}
...
\item By the Drinfeld--Kohno theorem
 \textup{(}Theorem~\textup{\ref{thm:drinfeld-kohno-appendix}}\textup{)},
...
lineage) with Theorem~\ref{thm:drinfeld-kohno-appendix}(i) (the KZ monodromy
...
\subsection{Affine lineage and higher-genus monodromy}
\label{sec:genus1-frontier}

\begin{remark}[Scope]
\label{rem:genus1-scope}
...
on $\overline{\FM}_k^{\mathrm{ord}}(\mathbb{C})$) are the image of
the Arnold--Orlik--Solomon relations under the projection to
consecutive-block collision faces. Equivalently, they are the
Stasheff associahedron identities. At degree~$k$, the
codimension-one faces are the consecutive-block collision faces
$D_I$ with $I=\{a,\dots,b\}$ and $|I|\ge2$; the binary part consists
of $D_{i,i+1}$ ($1 \leq i \leq k{-}1$).
The condition
...
Proposition~\ref{prop:r-matrix-descent-vol1} has been constructed. Let
...
symmetric strata. Averaging loses ordering and monodromy data; it does
not create boundary codimension. At genus~$0$ and fixed
degree~$n$, the kernel
...
\begin{remark}[Schur--Weyl proof of the kernel dimension]
\label{rem:schur-weyl-kernel}
\index{Schur--Weyl duality!kernel of averaging map}
\index{averaging map!kernel dimension via representation theory}
\index{symmetric group!irreducible decomposition of tensor power}
Proposition~\ref{prop:averaging-surplus} establishes that
the $R$-dependent part of $\ker(\mathrm{av})$ is generated by
$R$-matrix monodromy data and vanishes in the pole-free case. Here we
give a second, representation-theoretic proof of the ordinary
coinvariant kernel dimension
\begin{equation}\label{eq:kernel-dim-averaging}
\dim\ker(\mathrm{av}_n)
\;=\;
d^n \;-\; \binom{n+d-1}{d-1}
\end{equation}
in the pole-free setting, where $d = \dim V$ and the averaging map at
tensor degree~$n$ is the Reynolds projection
$V^{\otimes n} \to \mathrm{Sym}^n(V)$.

The Schur--Weyl decomposition for $\mathfrak{gl}(V) \cong \mathfrak{gl}_d$
decomposes the tensor power as a $(\mathfrak{gl}_d,\,\Sigma_n)$-bimodule:
\begin{equation}\label{eq:schur-weyl-decomp-app}
V^{\otimes n}
\;\cong\;
\bigoplus_{\lambda \,\vdash\, n,\;
\ell(\lambda) \leq d}
\mathbb{S}_\lambda(V) \otimes S^\lambda,
\end{equation}
where the sum runs over partitions $\lambda$ of~$n$ with at most $d$
parts, $\mathbb{S}_\lambda(V)$ is the Schur functor (the irreducible
$\mathfrak{gl}_d$-representation of highest weight~$\lambda$), and
$S^\lambda$ is the Specht module (the irreducible
$\Sigma_n$-representation indexed by~$\lambda$).
The dimensions are $\dim \mathbb{S}_\lambda(V) = s_\lambda(1^d)$
(the Schur polynomial evaluated at $x_1 = \cdots = x_d = 1$)
and $\dim S^\lambda = f^\lambda$ (the number of standard Young
tableaux of shape~$\lambda$), with the consistency check
$\sum_\lambda s_\lambda(1^d)\,f^\lambda = d^n$.

The averaging map $\mathrm{av}_n\colon V^{\otimes n} \to
\mathrm{Sym}^n(V)$ acts as the $\Sigma_n$-symmetriser
$e_+ = \frac{1}{n!}\sum_{\sigma} \sigma$ on the right tensor factor.
Since $e_+$ is the minimal idempotent projecting onto the trivial
representation $S^{(n)}$ (the one-dimensional Specht module for the
partition $\lambda = (n)$), the image of $\mathrm{av}_n$ restricted
to the $\lambda$-isotypic component is:
\[
\mathrm{av}_n\big|_{\mathbb{S}_\lambda(V) \otimes S^\lambda}
\;=\;
\begin{cases}
\mathbb{S}_{(n)}(V) \otimes S^{(n)}
\;\cong\;
\mathrm{Sym}^n(V)
& \text{if } \lambda = (n), \\[4pt]
0 & \text{if } \lambda \neq (n),
\end{cases}
\]
because $e_+ \cdot S^\lambda = 0$ for every non-trivial irreducible
$S^\lambda$ (by Schur's lemma, the trivial representation does not
appear in $S^\lambda$ for $\lambda \neq (n)$). The kernel is therefore
the sum over all non-trivial irreducible representations of~$\Sigma_n$:
\begin{equation}\label{eq:kernel-schur-weyl}
\ker(\mathrm{av}_n)
\;=\;
...
\mathbb{S}_\lambda(V) \otimes S^\lambda.
\end{equation}
This is a direct sum of irreducible $(\mathfrak{gl}_d,\,\Sigma_n)$-bimodules,
one for each non-trivial partition.

The dimension count follows. The image is
$\dim\mathrm{Sym}^n(V) = \binom{n+d-1}{d-1}$
(the single summand $\lambda = (n)$, with
$\mathbb{S}_{(n)}(V) = \mathrm{Sym}^n(V)$ and $f^{(n)} = 1$).
...
\ell(\lambda) \leq d,\;
\lambda \neq (n)}}
s_\lambda(1^d)\,f^\lambda.
\]
At $n = 2$: $d^2 - d(d{+}1)/2 = d(d{-}1)/2$,
which is $\dim\bigwedge^2 V$, the single non-trivial summand
$\lambda = (1,1)$ with $\mathbb{S}_{(1,1)}(V) = \bigwedge^2 V$
and $f^{(1,1)} = 1$.
At $n = 3$, $d = 2$: $8 - 4 = 4$,
contributed by
$\lambda = (2,1)$ with $s_{(2,1)}(1,1) = 2$ and $f^{(2,1)} = 2$,
giving $2 \cdot 2 = 4$.

The key observation is that
$\dim\mathrm{Sym}^n(V) = \binom{n + d - 1}{d - 1}$
depends only on $d = \dim V$ and is \emph{independent of the Lie type}
of any simple subalgebra $\mathfrak{g} \subset \mathfrak{gl}(V)$
acting on~$V$. Under restriction from $\mathfrak{gl}_d$ to a simple
$\mathfrak{g}$ with $V = V_{\omega_1}$ (the fundamental representation
of dimension~$d$), the Schur--Weyl decomposition
\eqref{eq:schur-weyl-decomp-app} refines: each $\mathbb{S}_\lambda(V)$
may decompose into several $\mathfrak{g}$-irreducibles, so the
multiplicity spaces for $\mathfrak{g}$ are larger than the Specht
modules. The commutant algebra
$\mathrm{End}_{\mathfrak{g}}(V^{\otimes n})$ is a quotient of the
group algebra $\mathbb{C}[\Sigma_n]$ that is generally larger than
$\mathrm{End}_{\mathfrak{gl}_d}(V^{\otimes n}) \cong
\bigoplus_\lambda \mathrm{End}(S^\lambda)$
(by Schur--Weyl for $\mathfrak{gl}_d$). The individual summands
in~\eqref{eq:kernel-schur-weyl} may split or merge under restriction
to~$\mathfrak{g}$. The total dimension of $V^{\otimes n}$ is
$d^n$ regardless, and the symmetric projection
$V^{\otimes n} \to \mathrm{Sym}^n(V)$ depends only on the
$\Sigma_n$-action (not on the Lie algebra), so the
dimension~\eqref{eq:kernel-dim-averaging} is type-independent.

In the chiral setting, the bar space
$(s^{-1}\bar{A})^{\otimes n}$ at tensor degree~$n$ plays the role
of~$V^{\otimes n}$, with $d = \dim(s^{-1}\bar{A})$ in the
finite-type case. The averaging map
$\mathrm{av}_n\colon \Barchord(A)_n \to \BarchSig(A)_n$
is the $R$-twisted variant of the $\Sigma_n$-symmetriser. When
$R(z)=\id$ (pole-free symmetric case), the twisting is trivial and the
Schur--Weyl decomposition applies directly, giving
$\ker(\mathrm{av}_n)$ the structure~\eqref{eq:kernel-schur-weyl}.
When $R(z)\neq\id$ (non-trivial $R$-matrix), the $\Sigma_n$-action
is deformed by the monodromy if the strong-unitary descent datum
exists; the Schur--Weyl decomposition does not apply in its classical
form, and the kernel acquires the richer structure described in
Proposition~\ref{prop:averaging-surplus}.
\end{remark}
...
hierarchy of Volume~II (3D Quantum Gravity). The correspondence is:
...
periods of mixed Tate motives over~$\mathbb{Z}$.
	...
	keeps the $R$-twisted coinvariant operator classes.  The chosen
	functionals $\tau_{A,n}^{\mathrm{res}}$ then produce the scalar
	symmetric-bar coordinates used in the shadow classification
	(curvature~$\kappa$, complementarity class~$C$, shadow
	tower~$Q$) are rational residues, hence periods of the trivial
	motive~$\mathbb{Q}(0)$. Ordered iterated-integral classes that are
	not invariant under the twisted descent action are not seen by these
	scalar shadows.
\end{enumerate}

\noindent
	The ordered surplus $\ker(\mathrm{av})$ contains the
	iterated-integral classes discarded by the symmetric projection. The
	tridegree decomposition stratifies this surplus by boundary depth,
	and this depth filtration is compatible with the motivic weight
	filtration in the mixed Tate configuration-space cases. This is the
	precise sense in which the ordered bar complex is arithmetically
	richer than the scalar symmetric shadows: it remembers
	multiple-zeta periods that those shadows forget.
\end{remark}

\section{Open constructions}
...
The associative formalism developed here is not the Francis--Gaitsgory Lie/cocommutative
formalism. The latter appears as the commutative shadow of the former.
...
\item the induced filtration on $\Barchord(A)$ is complete and strongly convergent.
\end{enumerate}
Then there is a convergent spectral sequence
\[
E_1\cong \overline{B}^{\mathrm{ch},FG}\!\bigl(\gr_{\mathrm{com}}A\bigr)
\Longrightarrow
\gr_{\mathrm{com}}\Barchord(A),
\]
and on the Koszul locus
\[
\gr_{\mathrm{com}}(A^!_{\mathrm{line}})
\simeq \bigl(\gr_{\mathrm{com}}A\bigr)^!_{FG}.
\]
\end{theorem}

\begin{remark}[Commutator filtration construction]
The comparison constructs the commutator filtration directly on the ordered bar complex, identifies
the associated graded with the commutative bar complex after the non-$\Sigma$ defect vanishes, and
identifies the induced differential with the Francis--Gaitsgory differential.
\end{remark}
...
\begin{remark}[Bordered FM construction]
\label{rem:bordered-partial-resolution}
Items~(1)--(3) and~(5) of Conjecture~\ref{conj:bordered} are supplied
...
carrier for the ordered open trace formalism. Volume~II,
Corollary~\texttt{cor:bordered-from-CG}, identifies these operations
with the algebraic interval, cotensor, pair-of-pants, and sewing
operations.
...
linearly ordered points on intervals. The remaining proof obligation is
the thickness-zero annular face, controlled by Mok's degeneration theory
for the family $S^1\times[0,\epsilon]\to\{0\}$.
\end{remark}

\subsection{Associative modular Maurer--Cartan theory}

The higher-genus modular formalism has an associative analogue built from ordered cyclic
...
In the ordered cyclic convolution dg Lie algebra it satisfies
\[
...
\begin{remark}[Associative cyclic compactification]
Volume~II constructs the
$E_1$ modular convolution object: the ordered cyclic compactification is realized by the
...
Principal Drinfeld--Sokolov reduction commutes with the ordered
line-side duality. For arbitrary nilpotent reductions the proof
obligation is PBW compatibility plus a BRST lift to the ordered bar
coalgebra.
...
Then
\[
Q_{DS}(A^!_{\mathrm{line}})\simeq Q_{DS}(A)^!_{\mathrm{line}}.
\]
...
\begin{remark}
For principal DS data, Volume~II supplies the PBW-compatible BRST lift
used in Theorem~\ref{thm:ordered-associative-ds-principal}.
Conjecture~\ref{conj:DS-arbitrary-nilpotent} names the non-principal
proof obligation: verify PBW compatibility and lift the BRST
differential to the ordered bar coalgebra beyond the principal and
hook-type regimes.
\end{remark}
...
$\FM_k(\mathbb{C})\times\mathrm{Conf}_k^<(\mathbb{R})$. The ordered
associative chiral Koszul duality of this chapter is the bar-complex
form of the shifted quantum groups of Finkelberg--Tsymbaliuk (and the
Coulomb-branch construction of Braverman--Finkelberg--Nakajima), the
factorization quantum groups of Latyntsev, and the stable-envelope
programme of Maulik--Okounkov as synthesized for 3d
$\mathcal{N}=4$ theories by Costello--Okounkov--Zhang--Zhou.
...
envelopes are ordinary matrices, not $\Ainf$ data.
\end{itemize}
...
\FM_k(\mathbb{C})\times\mathrm{Conf}_k^<(\mathbb{R}).
\]
The holomorphic factor $\FM_k(\mathbb{C})$ carries the chiral-product
differential (OPE collision residues), while the topological factor
$\mathrm{Conf}_k^<(\mathbb{R})$ carries the coassociative deconcatenation
coproduct (cofree tensor coalgebra structure). What differs among the three traditions
...
filtration, yielding a strict factorisation quantum group.
\end{proof}
...
produces the same object once strictification is carried out; for
simple Lie algebras this is a theorem only under the root-sector
strictification package. The
...
\begin{remark}[Affine case and extension]
...
The ordered bar complex supplies the algebraic boundary datum, and the
Yangian $Y_\hbar(\mathfrak{g})$ supplies the RTT line-algebra
candidate. Identifying its module category with the line operators of a
holomorphic-topological theory requires the corresponding
Costello-type gauge-theory comparison theorem. This section develops
the representation-theoretic side: evaluation modules, the Drinfeld
polynomial classification, tensor products and their braiding, and the
conditional comparison with the line category
...
\label{def:evaluation-module-app}
...
Definition~\textup{\ref{def:evaluation-module-app}} are the line
...
the curvature belongs to the chiral-product differential, the $R$-matrix to the coassociative deconcatenation coproduct.
...
$\kappa(\cA)\cdot\omega_1$. The genus-$1$ $E_1$-chiral coassociative
structure splits:
\[
\text{curved}\;E_1\text{-chiral coassociative}^{(1)}
\;\simeq\;
(\text{curved chiral-product differential})
\;\times\;
(\text{flat deconcatenation coproduct with meromorphic braiding}).
\]
...
We give the sharp form of this dichotomy, isolate the mechanism
...
\toprule
\textbf{Algebra} & $c_0$ & \textbf{Genus-$1$ type} & \textbf{Mechanism} \\
\midrule
...
the Casimir tensor. All $R$-matrices in this subsection are derived from the
...
For every simple $\mathfrak{g}$ and every finite-dimensional $V$, the
KZ/Yangian classical normalization is $r_{\mathrm{KZ}}(u)=\Omega/u$ with
$\Omega = \sum_a I^a \otimes I_a
\in \mathfrak{g} \otimes \mathfrak{g}
\hookrightarrow
\operatorname{End}(V)^{\otimes 2}$.
For the affine chiral algebra $V_k(\mathfrak{g})$, the collision
residue before KZ rescaling is instead
$r^{\mathrm{coll}}(z)=k\Omega_{\mathrm{tr}}/z$, with
$\Omega=2h^\vee\Omega_{\mathrm{tr}}$ and
$\hbar=(k+h^\vee)^{-1}$ in the KZ presentation.
...
The formulas are invariant under the usual choice of suspension conventions, provided one keeps
...
\]
With this convention \(d_{\Barchord}^2=0\) is exactly the suspended
Stasheff identity.
...
The ordered bar complex $\Barchord(A)$ carries the collision kernel
$K_A^{\mathrm{coll}}(z)$.  On families equipped with
$H_{\mathrm{CYBE}}^{\mathrm{rep}}$ and a strong-unitary monodromy
package, its represented image gives an $R$-matrix for descent. The following table
...
Proposition~\ref{prop:r-matrix-descent-vol1}.
None of the families in this table is $\mathsf{E}_1$-chiral in the sense
...
\begin{theorem}[$\mathsf{E}_1$ ordered collision-kernel landscape;
\ClaimStatusConditional]%
...
$\Barchord(A)$ has the following invariants, where\/ $K_A^{\mathrm{coll}}(z)$
...
line category, and
and every $R(z)$ entry carries the corresponding
$H_{\mathrm{CYBE}}^{\mathrm{rep}}$ and monodromy comparison package.
Here ``formal'' means the transferred $\Ainf$ operations
$m_k$ vanish on cohomology for all $k$ greater than the shadow depth:
\begin{center}
\small
...
\textbf{Family}
 ...
 & \textbf{Collision kernel}~$K_A^{\mathrm{coll}}(z)$
 & \makecell{\textbf{represented $R$-matrix}\\\textbf{under $H_{\mathrm{CYBE}}^{\mathrm{rep}}$}}
 & $r_{\max}$
 & $A^!_{\mathrm{line}}$
 & \textbf{Formal?} \\
\midrule
...
$\beta\gamma$
 & $\mathbf{C}$ & $1$
 & $0$ \textup{(contact $\Theta$)}
 & $\id$ \textup{(contact $\Theta$)}
 & $4$
 & $bc^!$
 & yes \\[3pt]
$bc$
 & $\mathbf{C}$ & $1$
 & $0$ \textup{(contact $\Theta_{bc}$)}
 & $\id$ \textup{(contact $\Theta_{bc}$)}
 & $4$
 & $\beta\gamma^!$
 & yes \\[3pt]
$\mathrm{Vir}_c$
...
completely, so the pole-valued kernel is zero; the nilpotent operator
$\Theta$ is the regular contact class.
...
\emph{Represented $R$-matrices.}
Under the family package $H_{\mathrm{CYBE}}^{\mathrm{rep}}$, the
$R$-matrix is the monodromy of the represented connection
$\nabla=d-\rho_A(K_A^{\mathrm{coll}}(z))\,dz$ on
$\mathrm{Conf}_2^{\mathrm{ord}}(\mathbb{C})$.
...
non-formality \textup{(}Volume~II\textup{)}.
...
\item \emph{Pole-free commutative}:
 the monodromy part is $R(z)=\id$, so the full braiding is the
 Koszul-signed flip.
...
\item \emph{Vertex algebras with poles}
 \textup{(}all families in Theorem~\ref{thm:e1-ordered-bar-landscape}\textup{)}:
 the monodromy part $R(z) \ne \id$, but $R(z)$ is \emph{derived} from the
