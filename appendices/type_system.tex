...
The Open quadrant of the four-quadrant programme organises chiral
...
collapses the quadrant grid (master pattern MA-1) and turns a
...
3 & \text{bulk} & Z^{\mathrm{der}}_{\mathrm{ch}}(\Ab)\simeq\ChirHoch^\bullet(\Ab) \\
...
$\mathrm{obs}_g=\kappa\cdot\lambda_g$ as an
...
\]
factors the programme: $\Open$ is Vol~I (algebraic-curve
...
$h_{\Ab}=h_{\mathrm{LV}}/(\kappa+\kappa^!)$ is the equivariant
signature.
\end{itemize}
\end{definition}
...
theorem viewed through different lenses (master pattern MA-1
...
\begin{definition}[Package containment]
...
that closes the proof is the discipline (master pattern MA-13).
...
in the inscription instantiates master pattern MA-1.
\end{proposition}
...
\end{tikzpicture}
\caption{Hypothesis-package lattice on the Open Beilinson-tower
reconstruction theorems. Nodes carry the level interface and the
...
join of the edge increments
(Proposition~\ref{prop:type-composition}).}
\label{fig:type-lattice}
\end{figure}

\begin{proposition}[Lattice well-formedness]
\label{prop:type-lattice-wellformed}
The diagram in Figure~\ref{fig:type-lattice} is a Hasse diagram for a
finite bounded lattice. Every directed path from
$\mathrm{Morita}$ to $\mathrm{Modular\ reconstruction}$ realises a
well-defined composite type signature
$\TypeSig=(\Open,\meet_i\Pres_i,1\!\to\!4,\joinp_i\Hyp_i)$.
\end{proposition}

\begin{proof}
Each edge realises a strict refinement of packages, so the diagram is
acyclic; finiteness of the alphabet bounds the lattice. By
Proposition~\ref{prop:type-meetjoin} every pair of nodes admits meet
and join in the package lattice, and the level-interface composition
is associative on the linearly ordered level set
$\{0,1,2,3,4,5\}$. Hence every directed path produces a well-formed
type signature.
\end{proof}
