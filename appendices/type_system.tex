...
\item \emph{Level 1.} Theorem~A is the level-$1\leftrightarrow2$
Ran reconstruction equivalence together with the separate
Verdier--bar algebra $K_X(A)=\mathbb D_{\Ran}B_X(A)$.
...
\item \emph{Level 3.} A family datum $H_H(\Ab;S)$ gives a strong
deformation retract
\[
 K_{\Ab,S}\ \substack{\xrightarrow{\ \iota_\Ab\ }\\[-2pt]
 \xleftarrow[\ p_\Ab\ ]{}}\ Q_\Ab
 \xrightarrow{\ \gamma_\Ab\ } C^\bullet_{\mathrm{ch}}(\Ab,\Ab),
 \qquad
 dh_\Ab+h_\Ab d=\id-\iota_\Ab p_\Ab,
\]
and Theorem~H concludes
$\operatorname{Supp}\ChirHoch^\bullet(\Ab)\subseteq S$.
\item \emph{Level 4.} Theorem~D is the cross-level
$4\to5$ comparison.  It begins with the native modular deformation class
\[
 \operatorname{Obs}^{\mathrm{def}}_g(\Ab)
 \in H^2\!\left(\Def_g(\Ab)\right),
 \qquad g\geq2.
\]
Under~$H_D^1$, the pointed genus-one class has normalized trace
$\operatorname{tr}_1\operatorname{Obs}^{\mathrm{def}}_{1,1}
=+\kappa(\Ab)\lambda_1
\in H^2(\overline{\cM}_{1,1})$.  Under~$H_D^K$, the virtual Hodge object and
its degree-$2g$ Chern component are
\[
 \mathfrak O_g^K(\Ab)
 =\kappa(\Ab)\lambda_{-1}(\mathbb E_g)
 \in K^0(\overline{\cM}_g),
 \qquad
 \operatorname{ch}_g\!\left(\mathfrak O_g^K(\Ab)\right)
 =(-1)^g\kappa(\Ab)\lambda_g
 \in H^{2g}(\overline{\cM}_g).
\]
The package~$H_D^{\mathrm{tr}}$ supplies the derived comparison between
these carriers.  Under~$H_D^{\mathrm{graph}}$, the level-$5$ graph
trace is
$F_g(\Ab)=\kappa(\Ab)\lambda_g^{\mathrm{FP}}
+\delta F_g^{\mathrm{cross}}(\Ab)\in\mathbb C$.
...
The Verdier--Koszul correspondence sends
\(\Ab\) to the algebra
\(K_X(\Ab):=\mathbb D_{\Ran}B_X(\Ab)\).  On the Koszul locus, the
quadratic comparison and local finite duality supply
\(\nu_\Ab\colon K_X(\Ab)\xrightarrow{\sim}\Ab^!\).  A self-dual point
carries a chosen equivalence
\(\sigma_\Ab\colon\Ab\xrightarrow{\sim}K_X(\Ab)\) compatible with
evaluation and coevaluation. The fixed sublocus
\[
 \KSDual
 :=\{\Ab\mid\Ab\simeq\Ab^!\}
\]
is the $\mathbb Z/2$-fixed locus, computed in the finite-type curved
Verdier--Koszul ambient, on which the four-quadrant grid degenerates
into self-dual form. On the admissible Koszul-complete part of
$\KSDual$:
\begin{itemize}
\item Theorem~A restricts from adjunction to its unit / counit
equivalence under the Koszul-complete package; every other projection
retains its own reconstruction morphism and hypothesis package;
\item for every family datum $H_H(\Ab;S)$, Theorem~H identifies the
complete chiral Hochschild complex with a strong deformation retract
supported in~$S$; critical, non-Koszul, and completed families carry
their own support sets, spectral sequences, and convergence data;
\item the five-archetype label is rigid on the admissible
finite-type curved fixed locus; scalar anti-diagonal relations such
as $\kappa+\kappa^!=0$ record duality-orbit sums, while membership in
$\KSDual$ is carried by the coherent homotopy-fixed datum;
\item a supplied family contraction datum
$(i_{\Ab},p_{\Ab},h_{\Ab};\nu_{\Ab})$ satisfies
\[
d h_{\Ab}+h_{\Ab}d
=\nu_{\Ab}\bigl(\mathrm{id}-i_{\Ab}p_{\Ab}\bigr).
\]
For $\nu_{\Ab}\in\Bbbk^\times$, the homotopy
$\nu_{\Ab}^{-1}h_{\Ab}$ contracts the same complex.  Identifying
$\nu_{\Ab}$ with the level-$2$ Verdier--Ran-on-bar scalar
$\mathcal N(\Ab)=\kappa(\Ab)+\kappa^!_{\mathrm{LV}}(\Ab)$ is the
family trace-comparison clause.  The algebra-level scalar
$K^\kappa$ enters through its own comparison map.
\end{itemize}
\end{definition}
