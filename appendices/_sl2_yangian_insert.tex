...
$Y_z^\hbar(\mathfrak{sl}_2)$ as the open-colour Koszul dual. This is
...
$\mathcal A^!_{\mathrm{line}}=Y_z^\hbar(\mathfrak{sl}_2)$. The
...
that the leading-order piece of $Y_z^\hbar(\mathfrak{sl}_2)$ is
$U(\mathfrak{sl}_2)$.
\end{remark}
...
\begin{theorem}[RTT presentation of $Y_z^\hbar(\mathfrak{sl}_2)$;
\ClaimStatusProvedHere]
\label{thm:rtt-sl2}
...
A(u)&B(u)\\C(u)&D(u)
\end{pmatrix}
\;\in\;\operatorname{End}(V)\otimes Y_z^\hbar(\mathfrak{sl}_2)[\![u^{-1}]\!],
\end{equation}
where $A(u),B(u),C(u),D(u)$ are formal power series in $u^{-1}$ with
coefficients in $Y_z^\hbar(\mathfrak{sl}_2)$. The RTT relation is
...
$\operatorname{End}(V)\otimes\operatorname{End}(V)\otimes Y_z^\hbar$.
...
\begin{theorem}[PBW basis of $Y_z^\hbar(\mathfrak{sl}_2)$;
\ClaimStatusProvedHere]
\label{thm:pbw-sl2}
The Yangian $Y_z^\hbar(\mathfrak{sl}_2)$ admits a PBW basis:
...
\begin{proof}
The PBW property for $Y_z^\hbar(\mathfrak g)$ is classical
...
The Hilbert series of $Y_z^\hbar(\mathfrak{sl}_2)$, graded by mode
number, is
\begin{equation}\label{eq:hilbert-sl2}
\sum_{n\ge 0}\dim\bigl(Y_z^\hbar(\mathfrak{sl}_2)_{\le n}\bigr)\,q^n
\;=\;
\prod_{d\ge 1}\frac{1}{(1-q^d)^3}\,,
\end{equation}
where $Y_z^\hbar(\mathfrak{sl}_2)_{\le n}$ denotes the subspace spanned
...
$V_a=\bC^2$ is the $Y_z^\hbar(\mathfrak{sl}_2)$-module obtained
...
irreducible as a $Y_z^\hbar(\mathfrak{sl}_2)$-module. The
...
is $Y_z^\hbar(\mathfrak{sl}_2)$; \ClaimStatusProvedHere]
...
to the Yangian $Y_z^\hbar(\mathfrak{sl}_2)$ at deformation parameter
$\hbar=1/(k+2)$:
...
$Y_z^\hbar(\mathfrak{sl}_2)$ relations.
...
Koszul dual is the \emph{finite-type} Yangian $Y_z^\hbar(\mathfrak{sl}_2)$.
...
The output $Y_z^\hbar(\mathfrak{sl}_2)$ is a genuinely $E_1$-chiral
...
The ordered bar complex is formal: all higher $\Ainf$ operations
$m_k=0$ for $k\ge 3$, and the open-colour Koszul dual $Y_z^\hbar(\mathfrak{sl}_2)$
