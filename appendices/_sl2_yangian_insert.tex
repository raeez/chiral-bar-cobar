...
We unwind the ordered bar complex for the affine Kac--Moody algebra
...
\emph{Type signature:} (Open quadrant; ordered bar monodromy;
Beilinson level $1\!\rightarrow\!4$; HP = non-critical level
$k\neq -2$ + class~$\mathbf L$ $E_2$-degeneration + Casimir tensor
$\Omega \in (V \otimes V)^{\mathfrak{sl}_2}$).
...
of the CYBE of Proposition~\ref{prop:ybe-from-d-squared} after the
RTT/Yangian quantization input has been supplied. The degree-$3$ bar
condition gives the classical obstruction; the full QYBE is a property
of the quantized Yang $R$-matrix, not a consequence of $d^2=0$ alone.
\end{remark}
...
\ClaimStatusProvedHere]
\label{thm:rtt-sl2}
\emph{Type signature:} (Open quadrant; quadratic generators-and-relations
on class~$\mathbf L$ locus; Beilinson level $2 \to 1$ inversion; HP =
RTT consistency from CYBE + class~$\mathbf L$ formality + non-critical
level + Drinfeld endpoint~\cite{Drinfeld88}).
...
\emph{Koszulness} of $\widehat{\mathfrak{sl}}_2$ as a
class~$\mathbf L$ object: on the locus where the ordered bar complex
is formal (all higher $\Ainf$ operations $m_k=0$ for $k\ge 3$), the
open-colour Koszul dual is quadratic and PBW-complete. The
class~$\mathbf L$ endpoint hypothesis package is named explicitly:
finite shadow depth $r_{\max} = 3$, double-pole OPE absorbed into a
single bar-degree-$2$ relation, $E_2$-degeneration of the PBW
spectral sequence. The implication \emph{class~$\mathbf L$
$\Rightarrow$ $m_k = 0$ for $k\ge 3$} is the
Yamada~\cite{Yamada89} / Penkava--Vanhaecke quadratic-to-PBW
endpoint specialised to the Kac--Moody family at non-critical level;
without these endpoint hypotheses, the quadratic-only
identification of the Yangian is a candidate dual with MC comparison
map (Theorem~\ref{thm:glz-quadratic-duality-scope}), not a witnessed
bar--cobar inversion.
\end{proof}
...
is $Y_\hbar(\mathfrak{sl}_2)$; \ClaimStatusConditional]
\label{thm:sl2-open-colour-yangian-dual}
\emph{Type signature:} (Open quadrant; ordered associative chiral
duality; Beilinson level $1\!\leftrightarrow\!2$; HP = augmented +
complete + finite-dim graded + class~$\mathbf L$ formality
($m_k = 0$, $k\ge 3$) + non-critical level $k\neq -h^\vee$).
...
\item \textbf{PBW completeness (class~$\mathbf L$ endpoint).}
The PBW basis (Theorem~\ref{thm:pbw-sl2}) confirms that no further
relations arise: on the class~$\mathbf L$ locus, the ordered bar
complex is formal ($m_k=0$ for $k\ge 3$ by the
Yamada~\cite{Yamada89} / Penkava--Vanhaecke quadratic-to-PBW endpoint
hypothesis specialised to non-critical Kac--Moody), so the
open-colour dual is quadratic and the PBW theorem applies. The
quadratic identification then lifts to the full bar--cobar counit
under the class~$\mathbf L$ factorization and convergence package
(Theorem~\ref{thm:full-koszulness-separate}, ingredients (a)+(b)+(c)
all witnessed for $\widehat{\mathfrak{sl}}_2$ at non-critical level).
...
Like all affine Kac--Moody algebras at non-critical level,
$\widehat{\mathfrak{sl}}_2$ belongs to pole-order class~$\mathbf L$
(double poles, finite shadow depth). The ordered bar complex is
formal — all higher $\Ainf$ operations $m_k=0$ for $k\ge 3$ — under
the class~$\mathbf L$ endpoint package
(Yamada~\cite{Yamada89} quadratic-to-PBW; non-critical level
$k\neq -2$; $E_2$-degeneration of the PBW spectral sequence). On
this locus the open-colour Koszul dual $Y_\hbar(\mathfrak{sl}_2)$ is
a quadratic algebra, its map to the full bar is a quasi-isomorphism,
and the Ran bar--cobar counit is a chain equivalence.  The Virasoro
...
— the quadratic-only identification fails, and the full Koszul-dual
construction requires the weight-completed coderived Positselski
ambient (completed-ambient) and a non-formal $A_\infty$-Yangian.
\end{remark}
