...
This appendix translates between sign conventions used in different sources.
...
\begin{itemize}
\item \emph{Loday--Vallette}: Use homological grading, so $d$ lowers degree ($|d| = -1$)
\item \emph{This manuscript}: Use cohomological grading, so $d$ raises degree ($|d| = +1$)
\end{itemize}
...
\emph{2.~Suspension.}
\begin{itemize}
\item \emph{Loday--Vallette}: Suspension $s$ is an explicit operator
\item \emph{This manuscript}: Suspension is a shift $[-1]$: $sV = V[-1]$, so $|sv| = |v| + 1$
\end{itemize}
...
\begin{itemize}
\item \emph{Loday--Vallette}: Explicit suspension in the formula
\item \emph{This manuscript}: Suspension is implicit in the D-module structure
\end{itemize}
...
\emph{Wrong.} $\widehat{ij}$ could mean:
\begin{itemize}
...
\emph{Wrong.} Writing $\colldiv{3}{1}$ (indices not increasing).

\emph{Right.} Always write $\colldiv{1}{3}$ with $1 < 3$. If you encounter the ``wrong'' ordering in a formula, swap and introduce a minus sign:
...
\emph{Historical reasons.}
\begin{itemize}
\item \emph{BD.} Developed for D-modules, where orientation comes from holomorphic structure.
\item \emph{CG.} Optimized for BV formalism and factorization algebras.
\item \emph{LV.} Pure operadic theory, no geometric residues.
\item \emph{Us.} Emphasizes geometric realization and explicit calculations.
\end{itemize}
