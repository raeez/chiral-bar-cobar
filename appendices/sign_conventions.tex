...
Logarithmic form sign & Not applicable & $\eta_{ji} = \eta_{ij}$ as forms ($d\log(-1)=0$; OS generators antisymmetric) \\
...
\begin{itemize}
\item \textit{Loday-Vallette}: Use homological grading, so $d$ lowers degree ($|d| = -1$)
\item \textit{This manuscript}: Use cohomological grading, so $d$ raises degree ($|d| = +1$)
\end{itemize}
...
\begin{center}
\begin{tabular}{|>{\raggedright\arraybackslash}p{6cm}|>{\raggedright\arraybackslash}p{4cm}|>{\raggedright\arraybackslash}p{4cm}|}
\hline
...
\textbf{Convention:} We write $\colldiv{i}{j}$ with $i < j$.  Geometrically $D_{ij} = D_{ji}$ (same subvariety $z_i = z_j$).  The sign convention $\colldiv{j}{i} = -\colldiv{i}{j}$ applies to the \emph{oriented divisor} (i.e., to the associated residue forms, not the subvariety itself).
...
\textbf{Higher-degree consequences:}
...
(The degree $|a|$ is taken in whichever grading is active.)  The sign difference between conventions manifests in the \emph{bar differential}, where LV uses explicitly suspended elements $sa$: the bar differential sign $(-1)^{|sa|} = (-1)^{|a|-1}$ in LV's homological convention differs from $(-1)^{|a|}$ in ours.
\end{proof}
...
\begin{definition}[Koszul Sign Rule - Four Conventions]
...
\textbf{LV (on suspended elements $sa, sb, sc$):}
$$d(sa \otimes sb \otimes sc) = d(sa) \otimes sb \otimes sc + (-1)^{m+1} sa \otimes d(sb) \otimes sc
+ (-1)^{m+n+2} sa \otimes sb \otimes d(sc)$$

\textbf{Explanation:} The sign differences arise because $|sa| = m+1$, $|sb| = n+1$, $|sc| = p+1$, and the standard Koszul rule is applied to these shifted degrees. The LV Koszul sign rule itself is the same $(-1)^{|x||y|}$.
\end{example}
...
\begin{remark}[Why Different Conventions Exist]
