...
% RECTIFICATION-FLAG RESOLVED (2026-03-31).  The previous formula had $+\sum$ on
% the comultiplication term, copied from [LV12, Prop.~2.2.5].  LV12 uses
% SUSPENSION $s$ for bar ($b_2(sa,sb) = (-1)^{|sa|}s(ab) = (-1)^{|a|+1}s(ab)$)
% while the manuscript uses DESUSPENSION $s^{-1}$ for bar
% ($b_2(s^{-1}a,s^{-1}b) = (-1)^{|a|}s^{-1}(ab)$).
% The relative sign between bar and cobar changes under this convention swap:
% the bar differential picks up a factor of $(-1)$ relative to LV12, and the
% cobar comultiplication term must do likewise to keep the bar-cobar adjunction
% consistent.  Concretely: with $+\sum$ the counit gives $2(-1)^{|a|+1}ab \neq 0$;
% with $-\sum$ it gives $0$.  The main text (eq:cobar-diff-generators) had the
% correct sign $-\sum$ all along.
\end{computation}
...
\end{align*}
Both produce a minus sign.  This is the most common source of confusion
when importing LV formulas: the exponent looks different ($-1$ vs.\ $1$)
but the \emph{sign} is the same because $(-1)^{-1} = (-1)^1 = -1$.
\end{example}
...
\emph{LV convention} (same formula, but with $(-1)^{r(s-1) + t}$ because LV absorbs
the suspension into the structure maps, defining $b_n := s \circ m_n \circ (s^{-1})^{\otimes n}$).
The relation on the desuspended maps $b_n$ reads:
\[
\sum_{r+s+t=n} (-1)^{rs+t}\, b_{r+1+t} \circ (\mathrm{id}^{\otimes r} \otimes b_s \otimes \mathrm{id}^{\otimes t}) = 0,
\]
which is \emph{identical} to~\eqref{eq:ainf-us}; the $b_n$ are exactly
the desuspended structure maps $b_n = s \circ m_n \circ (s^{-1})^{\otimes n}$.
