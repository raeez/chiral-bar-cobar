...
% RECTIFICATION-FLAG: The sign $+\sum(-1)^{|c'|}$ on the comultiplication term
% matches Loday--Vallette [LV12, Prop.~2.2.5].  The main text
% (bar_cobar_adjunction_curved.tex, eq:cobar-diff-generators) uses
% $-\sum(-1)^{|c'|}$ (opposite overall sign on the comultiplication term).
% Both conventions give $d_\Omega^2=0$ by coassociativity.  The discrepancy
% must be resolved: either the main text or this appendix has the wrong
% relative sign between the internal-differential and comultiplication terms.
% The appendix formula matches LV12 and is declared authoritative (CLAUDE.md).
% Reconcile the main text sign in a dedicated session.
\end{computation}
