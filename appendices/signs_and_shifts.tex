...
$\Omega(C) = T(s^{-1}\overline{C})$ (Loday--Vallette notation; our convention uses $T(s\bar{C})$, see \S\ref{subsec:cobar-diff-ainfty}) has differential on generators:
\[
...
Our cobar differential on $\Omega(C) = T(s\bar{C})$ for a dg coalgebra $(C, d, \Delta)$ is:
\[
d_\Omega(sc) = -s(dc)
  + \sum_{(c)} (-1)^{|c'|}\, (sc') \otimes (sc'').
\]

LV's cobar differential on $\Omega(C) = T(s^{-1}C)$ has the same formula
but with homological signs:
\[
d_\Omega^{\mathrm{LV}}(s^{-1}c) = -s^{-1}(dc)
  + \sum_{(c)} (-1)^{|c'|_{\mathrm{hom}}}\, (s^{-1}c') \otimes (s^{-1}c'').
\]

The formulas are identical once one accounts for $|c'|_{\mathrm{hom}} = -|c'|_{\mathrm{coh}}$
and $(-1)^{-n} = (-1)^n$.  The \emph{minus sign} on $s^{-1}(dc)$ is the same
in both conventions and arises from the suspended differential
