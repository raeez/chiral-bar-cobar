...
\begin{lemma}[Sign Rule for Compositions; \ClaimStatusProvedElsewhere]
...
\begin{enumerate}[label=(\roman*)]
\item The \textbf{suspension} $sV$ (also denoted $V[-1]$ in the cohomological convention or $V[1]$ in the homological convention; see Remark~\ref{rem:shift-notation}) is
defined by:
\[
(sV)^n := V^{n-1}.
\]
...
\item The \textbf{desuspension} $s^{-1}V$ (also denoted $V[1]$ in the cohomological convention or $V[-1]$ in the homological convention) is defined by:
\[
(s^{-1}V)^n := V^{n+1}.
\]
...
\textbf{Warning:} Many references in algebraic topology use the opposite convention where $sV = V[1]$; this corresponds to $(V[n])^k = V^{k-n}$ (the homological convention). When importing results from such references, all shift indices must be negated.
\end{remark}
...
\begin{corollary}[Iterated Suspension; \ClaimStatusProvedElsewhere]
...
\begin{proposition}[Suspension and Koszul Duality; \ClaimStatusProvedElsewhere]
...
\begin{proposition}[Properties of Determinant Lines; \ClaimStatusProvedElsewhere]
...
\begin{lemma}[Determinant and Ordering; \ClaimStatusProvedElsewhere]
...
\begin{proposition}[Determinant Lines on Configuration Spaces; \ClaimStatusProvedElsewhere]
...
\begin{proposition}[Determinant and Residues; \ClaimStatusProvedElsewhere]
...
\begin{theorem}[Determinant Conventions and Bar-Cobar Signs; \ClaimStatusProvedHere]
...
\begin{proposition}[Master Sign Formula; \ClaimStatusProvedElsewhere]
...
use the desuspended convention, matching
$d[a|b] = (-1)^{|a|}[ab]$ and $d[a|b|c] = (-1)^{|a|}[ab|c] - (-1)^{|a|+|b|}[a|bc]$.
\end{remark}
\end{proposition}
...
\begin{proposition}[Orientation System on FM Compactification; \ClaimStatusProvedElsewhere]
...
\begin{lemma}[Residue and Orientation; \ClaimStatusProvedElsewhere]
