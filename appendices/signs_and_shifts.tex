...
\]
This is the same formula as in Loday--Vallette~\cite[\S2.2]{LV12},
using \emph{desuspension} $s^{-1}$ (cohomological convention, $|s^{-1}c| = |c|-1$)
consistently with the bar construction.
The \emph{minus sign} on $s^{-1}(dc)$ arises from the suspended differential
(Proposition~\ref{prop:susp-diff}).
