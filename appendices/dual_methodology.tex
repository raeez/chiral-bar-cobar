...
\begin{theorem}[Concrete bar-cobar equivalence \cite{LV12}; \ClaimStatusConditional]
\label{thm:concrete-bar-cobar}
Let $\cA$ be a complete augmented $\Eone$-chiral algebra in the
holonomic/perfect Koszul lane of
Definition~\ref{def:chiral-koszul-pair}. Then the counit
\[
\varepsilon: \Cobar^{\mathrm{geom}}(\Bbar^{\mathrm{geom}}(\cA)) \xrightarrow{\simeq} \cA
\]
is a quasi-isomorphism. Outside this lane the same formula is a
completed pro-counit, not an equivalence until the Koszul hypotheses
are supplied.
\end{theorem}
...
\noindent\emph{Three constructions, one input}: $B(\cA)$ is a
factorization coalgebra; $\Omega(B(\cA)) \simeq \cA$ recovers the
original algebra; $K_X(\cA):=\mathbb{D}_{\mathrm{Ran}}(B(\cA))$ is the
Verdier--Koszul algebra.  Under the quadratic comparison and local
finite duality, a pair map
\(\nu_\cA\colon K_X(\cA)\xrightarrow{\sim}\cA^!\) identifies its
strict quadratic realization.  These are distinct typed operations.
\begin{proof}
...
distinction is load-bearing and is the quadratic-candidate scope discipline applied
...
\item \textbf{Bar--cobar counit on the comparison
...
\begin{remark}[Quadratic duals and full chiral Koszulness]
\label{rem:glz-three-ingredients}
\index{quadratic candidate versus full theorem}
Asserting GLZ quadratic duality ``establishes Koszulness'' without
ingredients (b) and (c) omits the comparison hypotheses: a
...
quasi-isomorphism and (c) the bar--cobar counit being a
quasi-isomorphism on the comparison locus, which specialise the
...
The false implication is detected by the slogan \emph{``GLZ produces
the Koszul dual''}: GLZ produces a \emph{candidate}
