...
**OF3. Koszulness (viii) ChirHoch freeness beyond concentration.** Concentration proved (Theorem H). Freeness conditional on Massey vanishing. Logarithmic W(p) has explicit Massey ⟨Ω,Ω,Ω⟩ obstruction.
...
**OF6. Super-complementarity canonical pairing.** Two pairings (super-trace vs Berezinian) coexist without programme-level canonicalization. Verdier pairing inscription pending. Scope: sub-Sugawara line only.

**OF7. Admissible-level Koszulness at rank ≥ 2 — REFRAMED AS FALSIFICATION (2026-04-17 Wave-2 finding).** Prior framing was wrong: Koszulness is NOT provable because it is FALSE at rk ≥ 2, q ≥ 3 admissible. The adversarial inscription attempt (draft at `notes/admissible_sl3_koszul_inscription_draft_2026_04_17.md`) found: rk(sl_3) = 2 Cartan classes survive in E_1^{2, h_α} of the Li-bar SS — d_1 cannot kill them because [h, h'] = 0 on the abelian Cartan, and higher d_r (r ≥ 2) have targets vanishing for conformal-weight reasons at q ≥ 3. Hence dim H²(bar^ch(L_k(sl_3))) ≥ 2, falsifying equivalences (i)–(vi) of `thm:koszul-equivalences-meta`. **Low-hanging fruit**: promote `conj:admissible-koszul-rank-obstruction` (`chapters/theory/chiral_koszul_pairs.tex:1648`) to `thm:admissible-sl3-non-koszul-qge3` using periodic-CDG finite-length (2026-04-16) + Arakawa 2015 C_2-cofiniteness + Finkelberg 1996 semisimplification as Step 1. Three HZ-IV decorators drafted (Li-bar Künneth engine / Ext²_{u_q(sl_3)}(k,k) = 2 via Ginzburg-Kumar + Lusztig + Finkelberg / DS cross-check against W_3). Residual: q ≤ 2 compatibility at `:1637-1639` where Cartan classes fall below bar-relevant range remains separate direction. L_k(sl_2) remains Koszul at all admissible (rank-1 escape: no Cartan obstruction). Sharp transition at q = 2 only proved for the universal algebra. **Programme-wide consequence**: admissible simple quotients at rk ≥ 2 populate the NON-KOSZUL locus, not the "open Koszulness" frontier. Propagate to Vol II V2-F4, Vol III CY-D interaction, scalar saturation OF15.
...
**OF17. Drinfeld double at the E_1-chiral level.** Assembling A ⋈ A^! from ordered-bar ingredients. Would reduce holographic modular Koszul datum H(T) from 6-tuple to (U, Θ_A).
...
**OF22. Jones polynomial from ordered chiral homology.** KZ trace ≠ Jones directly; Markov trace + writhe normalization + quantum dimension needed. Framework exists; identification conditional on normalization lemma.
...
**OF25. Stokes regularity at generic non-integral level, genus ≥ 1.** Modular operad composition PROVED genus-0 all levels + all-genera integrable. Stokes regularity at generic non-integral level g ≥ 1 is a residual Vol II gap inherited via cross-volume dependency on Vol I KZB.
