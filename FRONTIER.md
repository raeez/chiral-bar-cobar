...
**CL7 (Theorem C, scoped status — post-2026-04-17 audit + F07 attack heal).** (T1) `conj:derived-center-koszul-equivalence` (renamed from `lem:…`): brace dg level Z^{der}_ch(A) ≅ Z^{der}_ch(A^!) as E_2-algebras is CONJECTURAL (dependent on Deligne-Tamarkin); only H^0 naive-centre identification is unconditional via `lem:naive-center-koszul-identification`. Hinich 2001 rectification and Francis-Gaitsgory/GR17 bypass attempts each inscribed as failing for the comparison zig-zag (see `rem:derived-center-sharpened-scope`). (T2) `prop:perfectness-standard-landscape` UNCONDITIONAL only on the refined lane: Heisenberg (trivial), affine KM at positive-integer level via TUY finite-rank conformal blocks, and affine KM at boundary admissible level via Arakawa C_2-cofiniteness; generic non-critical affine KM = `conj:perfectness-boundary-km-generic`; class-M boundary stratum = `conj:perfectness-boundary-class-M`. Virasoro, W_N, lattice, βγ perfectness at generic parameters: Conjectural pending explicit nodal/family theorem. F07-routing: the proof still needs to bridge weightwise-finiteness → total-finiteness via `prop:conformal-blocks-bar` + degree-0 concentration from `lem:perfectness-criterion` (in-progress heal). (T3) C1 reflexivity unconditional on Koszul locus. (T4) `rem:plus-three-shift-nonissue` retracted the `+3` shift concern (no Verdier pairing at g=0; `thm:theorem-C-g0` folded). (T5) C2 hypotheses pinned: BV + Verdier + bar-chart lift + BV-bracket + nondegeneracy + Kontsevich-Pridham transport. (T6) C0 unconditional on Koszul locus. (T7) class-M C2(iii) weight-completed via MC5 pro-ambient. (T8) δF_2^cross(W_3) = (c+204)/(16c) explicit. (T9) `thm:C-PTVV-alternative`: clauses (i)-(ii) shifted-symplectic form genuinely independent via PTVV + HAG-II; Lagrangian clause (iii) downgraded to CONDITIONAL — forwards to `thm:quantum-complementarity-main`, which is not a back-reference but is inscribed conditional on Mok25 log-FM chain-level sewing at g≥3.
