...
\subsection{Primitive master kernel, branch BV action, and the metaplectic square root}
\label{subsec:vol2-primitive-master-kernel}

The full stable-graph field admits a direct compression to primitive boundary data.
In boundary coordinates this is the first place where the three references become one
construction rather than three separate sources of motivation.
...
\mathfrak K_C
:=
K_{0,2}+K_{0,3}+K_{0,4}+K_{1,1}+R_{\mathrm{pf},2}+R_{\mathrm{pf},3}+\cdots
\]
denote the primitive PVA-coordinate kernel, where absent summands are interpreted as
zero in a given family.  Here $K_{0,3}$ is the first cubic tree corolla, $K_{0,4}$
the quartic/contact corolla, $K_{1,1}$ the one-loop corolla, and
$R_{\mathrm{pf},d}$ the codimension-$d$ planted-forest residue.
\end{definition}

\begin{proposition}[Primitive reconstruction in PVA coordinates; \ClaimStatusProvedHere]
\label{prop:vol2-primitive-reconstruction}
The strict modular field is reconstructed from the primitive boundary kernel by
\[
Q^{\mathrm{mod}}_C
=
\bigl(d_{\mathrm{int}}+[\tau,-]\bigr)
+
\operatorname{ad}_{\mathsf{FT}^{\log}_{\mathrm{mod}}(\mathfrak K_C)},
\qquad
\Theta_C=\mathsf{FT}^{\log}_{\mathrm{mod}}(\mathfrak K_C),
\]
and one has
\[
(Q^{\mathrm{mod}}_C)^2=0
\iff
d\mathfrak K_C+\mathfrak K_C\star\mathfrak K_C=0.
\]
Thus the PVA-coordinate modular theory is generated by one primitive kernel, not by an a priori
expanded infinite stable-graph series.
\end{proposition}

\begin{computation}[Primitive shell equations; \ClaimStatusProvedHere]
\label{comp:vol2-primitive-shell-equations}
The first forcing identities in boundary coordinates are
\begin{align}
\label{eq:vol2-primitive-shell-g2}
R_2^{\mathrm{rel}}
&=
\Delta_{\mathrm{cyc}}(K_{1,1})
+\frac12[K_{1,1},K_{1,1}]
+\sum_{\rho\in\mathsf{Rig}_2}\mathfrak r_\rho(K_{0,2}),
\\
...
+[K_{1,1},K_{2,\bullet}]
+\frac1{3!}[K_{1,1},K_{1,1},K_{1,1}]
+\sum_{\rho\in\mathsf{Rig}_3}\mathfrak r_\rho(K_{0,2}).
\end{align}
Equations~\eqref{eq:vol2-primitive-shell-g2} and~\eqref{eq:vol2-primitive-shell-g3}
are the coordinate-level form of the genus-two and genus-three shell recursions.
Heisenberg keeps only the $\Delta_{\mathrm{cyc}}$ term, affine currents add the separating
bracket term, and Virasoro/$W_3$ activate the rigid-cut residues.
\end{computation}

\begin{definition}[Branch BV action in PVA coordinates]
\label{def:vol2-branch-bv-action}
Let $V^{\mathrm{br}}_C=V_C^+\oplus V_C^-$ be the reduced polarized branch packet.
Set
\[
\mathsf{BV}^{\mathrm{br}}_C
:=
...
\]
The primitive boundary kernel defines multilinear branch coefficients $\mu_{m,n}^C$ and
hence the branch master action
\begin{equation}
\label{eq:vol2-branch-master-action}
S^{\mathrm{br}}_C(x,\xi)
:=
\frac12 x^\top K_C^{\mathrm{br}}x
+
\sum_{m+n\ge 3}\frac{1}{m!\,n!}\mu_{m,n}^C(x^{\otimes m};\xi^{\otimes n}).
\end{equation}
The branch differential is
\[
Q^{\mathrm{br}}_C
:=
\{S^{\mathrm{br}}_C,-\}_{\mathrm{BV}}+\hbar\Delta^{\mathrm{br}}_C.
\]
\end{definition}

\begin{proposition}[PVA-coordinate quantum master equation; \ClaimStatusProvedHere]
\label{prop:vol2-branch-qme}
The branch differential squares to zero if and only if
\[
\hbar\Delta^{\mathrm{br}}_C S^{\mathrm{br}}_C
+
\frac12\{S^{\mathrm{br}}_C,S^{\mathrm{br}}_C\}_{\mathrm{BV}}=0.
\]
This is the finite-rank branch projection of the primitive Maurer--Cartan equation for
$\mathfrak K_C$.
\end{proposition}

\begin{computation}[Archetype master actions; \ClaimStatusProvedHere]
\label{comp:vol2-archetype-master-actions}
The reduced branch actions of the standard families take the following direct forms.
For Heisenberg,
\[
\mathfrak K_{\mathrm{Hei}}=K_{0,2}+K_{1,1},
\qquad
S^{\mathrm{br}}_{\mathrm{Hei}}(x,\xi)=\frac12 K_{ij}x^ix^j.
\]
For affine currents,
\[
\mathfrak K_{\widehat{\mathfrak{sl}}_2}=K_{0,2}+K_{0,3}+K_{1,1},
\qquad
S^{\mathrm{br}}_{\mathrm{aff}}(x,\xi)=\frac12 k\kappa_{ab}x^ax^b+\frac1{3!}f_{abc}x^ax^bx^c.
\]
For $W_3$,
\[
\mathfrak K_{W_3}=K_{0,2}+K_{0,3}+K_{0,4}+K_{1,1}+R_{\mathrm{pf},2}+R_{\mathrm{pf},3}+\cdots,
\]
with quartic coefficient governed by
\[
\Lambda={:}TT{:}-\frac{3}{10}\partial^2T,
\qquad
[S^{\mathrm{br}}_{W_3}]_{\mathrm{quartic}}
\propto
\frac{16}{22+5c}\,\Lambda.
\]
Thus the Heisenberg sector is purely quadratic, the affine sector is cubic-tree,
and the $W_3$ sector is the first genuinely quartic/rigid primitive regime.
\end{computation}

\begin{corollary}[Metaplectic square root of the spectral determinant; \ClaimStatusProvedHere]
\label{cor:vol2-metaplectic-square-root}
Let $T_C^{\mathrm{br,red}}$ be the reduced branch transport on $V_C^{\mathrm{br}}$.
Then
\begin{equation}
\label{eq:vol2-metaplectic-square-root}
\delta_C(x)
:=
\exp\!\Bigl(\frac12\operatorname{Tr}\log\bigl(1-xT_C^{\mathrm{br,red}}\bigr)\Bigr)
\end{equation}
is a canonical square root of the branch determinant:
\[
\delta_C(x)^2=
\det\bigl(1-xT_C^{\mathrm{br,red}}\bigr)=\Delta_C(x).
\]
This is the honest square-root layer of the reduced branch packet: the determinant shadow
comes from a first-order metaplectic half-density on the finite-dimensional polarized sector.
\end{corollary}
