...
functor rather than as an extra external package.
\end{construction}
...
\subsection{Primitive-kernel compression, modular master action, and the metaplectic branch shadow}
\label{subsec:primitive-kernel-compression}

The stable-graph object of Construction~\ref{constr:log-modular-bar-functor}
is cofree.  Therefore its positive-genus part is determined by its projection to
primitive corollas and rigid cutting strata.  This is the direct native form of
the theory: the expanded Maurer--Cartan field is not fundamental; the primitive
kernel is.

\begin{definition}[Primitive logarithmic modular kernel]
\label{def:primitive-log-modular-kernel}
Define the primitive logarithmic input module by
\[
\operatorname{Prim}^{\log}_{\mathrm{mod}}(\cA)
:=
\prod_{2g-2+n>0}
\cC^{\log FM}_{\mathrm{cor}}(g,n)
\,\widehat\otimes\,
\operatorname{End}^{\mathrm{cyc}}_{\mathrm{Ch}_\infty}(A^{\mathrm{ch}}_\infty)(n)
\widehat\oplus
\prod_{\rho\in \mathsf{Rig}}
\cC^{\log FM}_{\rho}
\,\widehat\otimes\,
\operatorname{End}^{\mathrm{cyc}}_{\mathrm{Ch}_\infty}(A^{\mathrm{ch}}_\infty).
\]
A \emph{primitive logarithmic modular kernel} is an element
\[
\mathfrak K_\cA
=
\sum_{2g-2+n>0}K^{\cA}_{g,n}
+
\sum_{\rho\in\mathsf{Rig}}R^{\cA}_{\rho}
\in
\operatorname{Prim}^{\log}_{\mathrm{mod}}(\cA),
\]
where $K^{\cA}_{g,n}$ is the $(g,n)$-corolla coefficient and
$R^{\cA}_{\rho}$ is the rigid-cut coefficient attached to the planted-forest
boundary type~$\rho$.
\end{definition}

\begin{construction}[Logarithmic Feynman transform of the primitive kernel; \ClaimStatusProvedHere]
\label{constr:primitive-kernel-feynman-transform}
For a primitive kernel $\mathfrak K_\cA$ define its logarithmic modular
Feynman transform by the convergent stable-graph sum
\begin{equation}
\label{eq:primitive-kernel-feynman-transform}
\mathsf{FT}^{\log}_{\mathrm{mod}}(\mathfrak K_\cA)
:=
\sum_{\Gamma\in\mathsf{Gr}^{\mathrm{st}}}
\frac{1}{|\Aut(\Gamma)|}
\,W^{\log}_{\Gamma}(\mathfrak K_\cA)\,\Phi^{\cA}_{\Gamma}
\in
\mathfrak g^{\mathrm{mod,log}}_{\cA}(\mathfrak U),
\end{equation}
where every vertex of type $(g(v),n(v))$ is decorated by the corolla
$K^{\cA}_{g(v),n(v)}$, every rigid planted-forest edge by the corresponding
$R^{\cA}_{\rho}$, and $W^{\log}_{\Gamma}$ is the chain weight of the associated
log-FM stratum.
\end{construction}

\begin{theorem}[Primitive-to-global reconstruction; \ClaimStatusProvedHere]
\label{thm:primitive-to-global-reconstruction}
Let $\mathfrak K_\cA$ be a primitive logarithmic modular kernel.
Then:
\begin{enumerate}[label=\textup{(\roman*)}]
\item the modular homological vector field is recovered by
\[
Q^{\mathrm{mod}}_{\cA}
=
\bigl(d_{\mathrm{int}}+[\tau,-]\bigr)
+
\operatorname{ad}_{\mathsf{FT}^{\log}_{\mathrm{mod}}(\mathfrak K_\cA)};
\]
\item the positive-genus Maurer--Cartan field is
\[
\Theta_\cA=\mathsf{FT}^{\log}_{\mathrm{mod}}(\mathfrak K_\cA);
\]
\item one has the exact equivalence
\begin{equation}
...
\end{equation}
where $\star$ is the primitive pre-Lie gluing operation induced by
one-edge sewing and Mok's rigid cutting correspondences.
\end{enumerate}
\end{theorem}

\begin{proof}
By the cofree-coderivation principle, a derivation of the completed stable-graph
bar object is determined by its projection to primitive corollas.  The logarithmic
Feynman transform is precisely the extension from primitive data to all stable
graphs.  Equation~\eqref{eq:primitive-master-equation} is the quadratic identity
obtained by projecting $(Q^{\mathrm{mod}}_{\cA})^2$ back to primitive corollas; the
terms coming from two different one-edge factorizations cancel by the codimension-two
identities of the log-FM boundary stratification.
\end{proof}

\begin{proposition}[First shell equations in primitive form; \ClaimStatusProvedHere]
\label{prop:primitive-shell-equations}
Write $K_1:=K^{\cA}_{1,1}$ and $K_2:=\sum_n K^{\cA}_{2,n}$.
Then the genus-two and genus-three forcing terms are
\begin{align}
...
+\frac12[K_1,K_1]
+\sum_{\rho\in\mathsf{Rig}_2}R^{\cA}_{\rho}(K^{\cA}_{0,2}),
\\
...
+[K_1,K_2]
+\frac1{3!}[K_1,K_1,K_1]
+\sum_{\rho\in\mathsf{Rig}_3}R^{\cA}_{\rho}(K^{\cA}_{0,2}).
\end{align}
Thus Heisenberg is the pure $\Delta_{\mathrm{ns}}$ regime, affine current
algebras add the separating bracket term, and Virasoro/$\mathcal W_N$ activate
all displayed channels including the rigid-cut contributions.
\end{proposition}

\begin{definition}[Reduced branch BV packet and modular master action]
\label{def:reduced-branch-master-action}
Let $V^{\mathrm{br}}_\cA=V^+_\cA\oplus V^-_\cA$ be the canonically extracted
finite-rank branch-active quotient with its complementarity polarization.
Define the completed branch BV algebra
\[
\mathsf{BV}^{\mathrm{br}}_\cA
:=
\widehat{\operatorname{Sym}}\bigl((V^+_\cA)^\vee\bigr)
\,\widehat\otimes\,
\Lambda\bigl((V^-_\cA)^\vee\bigr),
\qquad
\Delta^{\mathrm{br}}_\cA
:=
\sum_i \frac{\partial^2}{\partial x_i\,\partial \xi_i}.
\]
Projecting the primitive kernel to the branch packet produces multilinear
coefficients $\mu^{\cA}_{m,n}$ and a quadratic branch generator
$K^{\mathrm{br}}_\cA$.  The resulting \emph{modular master action} is
\begin{equation}
\label{eq:branch-master-action}
S^{\mathrm{br}}_\cA(x,\xi)
:=
\frac12\,\Omega_\cA\bigl(K^{\mathrm{br}}_\cA x,x\bigr)
+
\sum_{m+n\ge 3}
\frac{1}{m!\,n!}
\mu^{\cA}_{m,n}(x^{\otimes m};\xi^{\otimes n}).
\end{equation}
The induced branch differential is
\[
Q^{\mathrm{br}}_\cA
:=
\{S^{\mathrm{br}}_\cA,-\}_{\mathrm{BV}}
+\hbar\Delta^{\mathrm{br}}_\cA.
\]
\end{definition}

\begin{proposition}[Branch master equation; \ClaimStatusProvedHere]
\label{prop:branch-master-equation}
The branch differential squares to zero if and only if the projected action
satisfies the quantum master equation
\begin{equation}
\label{eq:branch-qme}
\hbar\Delta^{\mathrm{br}}_\cA S^{\mathrm{br}}_\cA
+\frac12\bigl\{S^{\mathrm{br}}_\cA,S^{\mathrm{br}}_\cA\bigr\}_{\mathrm{BV}}
=0.
\end{equation}
Equivalently, the branch packet is the finite-rank BV shadow of the primitive
Maurer--Cartan equation~\eqref{eq:primitive-master-equation}.
\end{proposition}

\begin{proof}
This is the standard BV identity $(\{S,-\}+\hbar\Delta)^2=0$
if and only if~\eqref{eq:branch-qme} holds.  The coefficients of
$S^{\mathrm{br}}_\cA$ are the branch projections of the primitive corolla and
rigid-cut amplitudes, so the branch quantum master equation is precisely the
finite-rank reduction of the primitive master equation.
\end{proof}

\begin{corollary}[Metaplectic square root of the branch determinant; \ClaimStatusProvedHere]
\label{cor:metaplectic-square-root}
Let $T^{\mathrm{br,red}}_\cA\in\Aut(V^{\mathrm{br}}_\cA)$ be the reduced branch
transport operator.  The formal series
\begin{equation}
\label{eq:metaplectic-square-root}
\delta_\cA(x)
:=
\exp\!\Bigl(\frac12\operatorname{Tr}\log\bigl(1-xT^{\mathrm{br,red}}_\cA\bigr)\Bigr)
\in 1+x\Bbbk[\![x]\!]
\end{equation}
is a canonical square root of the spectral shadow:
\[
\delta_\cA(x)^2
=
\det\bigl(1-xT^{\mathrm{br,red}}_\cA\bigr)
=
\Delta_\cA(x).
\]
In particular, the first-order ``square root of the modular Laplacian'' lives
not on the full infinite deformation complex but on the finite-dimensional
branch BV packet extracted from it.
\end{corollary}
