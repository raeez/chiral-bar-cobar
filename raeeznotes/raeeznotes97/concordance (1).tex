...
terms of~\eqref{eq:D-five-components}.  Thus the three pillars already
determine the concrete chain-level programme: modular bar functor,
convolution $L_\infty$-object, graph expansion, and clutching calculus.
The detailed formulas are collected in
...
\paragraph{Native end-state.}
The sharpest compression now available is not another shadow invariant but the direct chain
\[
\mathfrak K_\cA
\rightsquigarrow
Q^{\mathrm{mod}}_\cA
...
\delta_\cA.
\]
Here $\mathfrak K_\cA$ is the primitive logarithmic modular kernel,
$Q^{\mathrm{mod}}_\cA$ the reconstructed modular homological vector field,
$S^{\mathrm{br}}_\cA$ the reduced branch BV master action, and
$\delta_\cA$ the metaplectic square root of the branch determinant.
The scalar class $\kappa(\cA)$, the spectral determinant $\Delta_\cA$, the branch-active quotient,
and the nonlinear resonance classes are therefore successive shadows of one primitive master object,
