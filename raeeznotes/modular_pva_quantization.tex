...
\begin{construction}[Homotopy-chiral/log-FM coordinates for the modular deformation object; \ClaimStatusProvedHere]
\label{const:vol2-three-preprints-chain-level}
Choose an affine cover $\mathfrak U$ of the boundary curve and set
\[
A^{\mathrm{ch}}_\infty := \check C^\bullet(\mathfrak U;\A).
\]
By Malikov--Schechtman~\cite{MS24} this is a $\mathrm{Ch}_\infty$-algebra.
For a logarithmic boundary pair or semistable family write
\[
...
\]
Mok's rigid planted-forest decomposition~\cite{Mok25}, together with the
one-slot homotopy functoriality of Robert-Nicoud--Wierstra~\cite{RNW19} and
Vallette's rectification theorem~\cite{Val16}, presents the modular
deformation object by
\[
\Definfmod_{\log}(C)
:=
\Convinf\!\Bigl(
...
\Bigr).
\]
The existing coderivation algebra $L_{\mathrm{mod}}(C)$ is the bar-side
realization of this strict chart.
\end{construction}

\begin{remark}[Stable-graph modular bar functor in coordinates]
\label{rem:vol2-log-modular-bar-functor}
In these coordinates the modular bar construction is the stable-graph sum
\[
\Bmod^{\log}(C)(g,n)
=
\bigoplus_{\Gamma\in \mathsf{Gr}^{\mathrm{st}}_{g,n}}
\left(
\bigotimes_{v\in V(\Gamma)} \bar B^{\mathrm{ch}}\bigl(A^{\mathrm{ch}}_\infty;\mathrm{In}(v)\bigr)
\otimes
...
\right)_{\Aut(\Gamma)}.
\]
Its total differential is again the sum of the transferred chiral
differential, separating sewing, planted-forest corrections, and the
non-separating loop operator.  This identifies the one-edge principle of
\S3 as the genus-$1$ Taylor coefficient of the same bar functor that
governs all higher collision terms.
\end{remark}
...
\]
In the PVA coordinate models this is the reduced odd Laplacian already
isolated in this chapter; for classical $W_3$ the relevant cohomology
calculation shows that the first modular obstruction vanishes, while the
higher tree-level brackets remain visible in the stable-graph expansion.
\end{computation}

\begin{construction}[PVA-coordinate form of the graph coefficients; \ClaimStatusProvedHere]
\label{const:vol2-pva-coordinate-graph-coefficients}
If $u_1,\dots,u_n$ are local boundary fields, then the first tree
coefficients admit residue expressions over logarithmic FM boundary
strata:
\begin{align}
\ell_3^{(0)}(u_1,u_2,u_3)
&=
...
\end{align}
while the first positive-genus coefficient is the reduced odd Laplacian
\begin{equation}
\label{eq:vol2-loop-laplacian}
\ell_1^{(1)}=\hbar\Delta_{\mathrm{cyc}}.
\end{equation}
The three summands of~\eqref{eq:vol2-ell4-residue} are precisely the
contact, additive, and tree rigid types of Mok's degeneration formula;
this is the PVA-coordinate meaning of the planted-forest correction.
\end{construction}

\begin{computation}[Archetype formulas in boundary coordinates; \ClaimStatusProvedHere]
\label{comp:vol2-archetype-formulas}
For the Heisenberg PVA with
$\{a^i{}_{\lambda}a^j\}=K^{ij}\lambda$,
\[
\ell_3^{(0)}=0,
\qquad
\ell_4^{(0)}=0,
...
\ell_1^{(1)}
=
\hbar\sum_{i,j}K^{ij}\frac{\partial^2}{\partial a^i\partial a^j}.
\]
For the affine current PVA with
$\{J^a{}_{\lambda}J^b\}=f^{ab}{}_cJ^c+k\kappa^{ab}\lambda$,
\[
[\ell_3^{(0)}]
=
[f_{abc}\,J^a\otimes J^b\otimes J^c],
\qquad
\ell_4^{(0)}=0.
\]
For the $W_3$ PVA the quartic tree coefficient is controlled by the
quasi-primary
\[
\Lambda={:}TT{:}-\frac{3}{10}\partial^2T,
\]
and the first nonlinear tree class is proportional to
$\frac{16}{22+5c}\,\Lambda$; equivalently, the first planted-forest
correction is the bar-side avatar of the nonlinear $WW$-bracket.  Thus
Heisenberg is Gaussian, affine currents are Lie/tree, and $W_3$ is the
first genuinely planted-forest sensitive example.
\end{computation}

\begin{remark}[Genus-two entry point in PVA coordinates]
\label{rem:vol2-genus-two-entry}
The genus-two correction in boundary coordinates decomposes as
\[
\Theta^{(2)}
=
...
+\Theta^{(2)}_{\mathrm{pf}}.
\]
Heisenberg only sees the first term, affine currents see the first two,
and $W_3$/Virasoro exhibit all three.  This is the first point where the
one-edge operator, the clutching bracket, and the planted-forest
correction become inseparable.
\end{remark}

\begin{remark}[Rigid-type boundary factorization]
\label{rem:vol2-rigid-type-boundary-factorization}
Mok's degeneration formula rewrites the separating boundary of
$\operatorname{FM}_n(W/B)$ as a sum over rigid planted-forest types.
Accordingly every clutching identity in the strict modular complex is a
push-pull identity between products of lower log-FM factors and the actual
boundary component.  This is the geometric reason that the obstruction
recursion of Theorems~\ref{thm:Ob1} and~\ref{thm:Obg} factorizes by stable
graphs.
\end{remark}
...
The three references make the first nontrivial coefficients of the physical modular package fully explicit.
On the one hand, Robert-Nicoud--Wierstra identify the Taylor coefficients of the convolution
$L_\infty$-structure by the coproduct formula
\[
\ell_k(f_1,\dots,f_k)(x)
=
\gamma_A(\alpha\circ 1_D)(f_1\otimes\cdots\otimes f_k)\Delta_D^k(x).
\]
On the other hand, Malikov--Schechtman's secondary Borcherds operations give an explicit mode-level
realization of the first higher coefficient.  In the PVA language this yields the cubic formula
\begin{equation}
...
\right)
 j'_{(p,q,r)}(a,b,c),
\end{equation}
where the coefficients $j'_{(p,q,r)}$ satisfy the secondary Borcherds identity
\begin{align}
...
\Bigl(
(-1)^j a_{(p+q-j)}\bigl(b_{(r+j)}c\bigr)
-(-1)^{j+p+|a||b|}b_{(r+p-j)}\bigl(a_{(q+j)}c\bigr)
\Bigr)
-\sum_{j\ge 0}\binom{q}{j}\bigl(a_{(p+q-j)}b\bigr)_{(r+j)}c.
\end{align}
Thus the first higher obstruction in the PVA story is simultaneously a planted-forest residue, a modewise
Jacobi homotopy, and the ternary coefficient of the convolution $L_\infty$-object.
...
In the strict chart the first four Taylor coefficients of the modular deformation field are organized as
\begin{align}
...
\end{align}
Equivalently, the quartic sector has exactly four primitive graph channels: one zero-edge contact graph and
three one-edge separating graphs.  The genus-$1$ contribution is the first non-separating loop correction.
\end{proposition}

\begin{remark}[Family-by-family reading of the low-order atlas]
\label{rem:vol2-family-by-family-low-order-atlas}
The low-order atlas already separates the standard physical families.
\begin{enumerate}[label=\textup{(\roman*)}]
\item Free and abelian theories are \emph{Gaussian}: $\ell_3^{(0)}=0$ and the local quartic contact channel vanishes.
\item Affine current algebras are \emph{Lie/tree}: $\ell_3^{(0)}\neq 0$, while quartic behaviour first appears through the three separating tree channels.
\item The weight-changing $\beta\gamma$ slice is \emph{contact/quartic}: $\ell_3^{(0)}=0$ but $\Theta_4^{\mathrm{contact}}\neq 0$.
\item Virasoro and principal $\mathcal W_N$ are \emph{mixed}: both the cubic coefficient and the local quartic contact channel are nonzero.
\end{enumerate}
This is the physical incarnation of the same archetype classification proved abstractly in Volume~I.
\end{remark}
...
\begin{proposition}[Quadratic genus-two forcing along a one-parameter Poisson line; \ClaimStatusProvedHere]
...
\]
If in addition $\Delta_{\mathrm{cyc}}P_c=0$, then the reduced genus-two
forcing term
\[
\mathfrak F_2(\Xi_c)
:=
\Delta_{\mathrm{cyc}}\Xi_c+\frac12[\Xi_c,\Xi_c]
\]
is exact:
\[
\mathfrak F_2(\Xi_c)
=
-\frac12[P_c,\partial_c^2P_c].
\]
Hence the quadratic self-coupling of the genus-one direction is
cohomologically trivial; any genuinely new genus-two obstruction must come
from the rigid planted-forest/log-FM sector or from deformation directions
outside the chosen one-parameter line.
\end{proposition}

\begin{proof}
Differentiate the identity $[P_c,P_c]=0$ twice with respect to $c$:
\[
0=\partial_c^2[P_c,P_c]=2[\partial_cP_c,\partial_cP_c]+2[P_c,\partial_c^2P_c].
\]
This gives the first displayed identity.  If moreover
$\Delta_{\mathrm{cyc}}P_c=0$, then $\Delta_{\mathrm{cyc}}\Xi_c=
\partial_c(\Delta_{\mathrm{cyc}}P_c)=0$ because
$\Delta_{\mathrm{cyc}}$ is independent of the parameter $c$.  Substituting
into the definition of $\mathfrak F_2(\Xi_c)$ gives the exactness formula.
\end{proof}
...
\begin{corollary}[Classical $W_3$: no quadratic self-obstruction on the central line; \ClaimStatusProvedHere]
\label{cor:w3-no-quadratic-self-obstruction}
For the classical $W_3$ family, the reduced genus-two forcing of the unique
relevant genus-one direction $\Xi_c=\partial_cP_c$ is exact:
\[
\mathfrak F_2(\Xi_c)
=
\Delta_{\mathrm{cyc}}\Xi_c+\frac12[\Xi_c,\Xi_c]
\in B^2_{\delta_{P_c}}.
\]
Thus within the relevant one-dimensional sector, the first possible
nontrivial genus-two obstruction cannot come from $\frac12[\Xi_c,\Xi_c]$
or from $\Delta_{\mathrm{cyc}}\Xi_c$; it must enter through the rigid
planted-forest part of the full log-FM modular bar theory or by leaving the
relevant sector.
\end{corollary}

\begin{proof}
Theorem~\ref{thm:W3mod0} gives $\Delta_{\mathrm{cyc}}P_c=0$, and
Proposition~\ref{prop:quadratic-genus-two-line} then identifies the reduced
...
and generated by $\partial_cP_c$.  The new genus-two calculation of
Corollary~\ref{cor:w3-no-quadratic-self-obstruction} sharpens this: the
quadratic self-coupling of that unique one-loop direction is exact, so the
first potentially nontrivial genus-two phenomenon is forced into the rigid
planted-forest/log-FM sector.
