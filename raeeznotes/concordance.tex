...
On each primary line~$L$ the shadow tower is controlled by the \emph{shadow metric} $Q_L(t) = (2\kappa + \alpha t)^2 + 2\Delta\, t^2$, a quadratic form on~$L$ (Definition~\ref{def:shadow-metric}), where $\Delta = 8\kappa S_4$ is the critical discriminant (eq.~\eqref{eq:critical-discriminant}).
...
The Gaussian decomposition (Corollary~\ref{cor:gaussian-decomposition}) separates $Q = (\text{free})^2 + 2\Delta\,t^2$ into Gaussian and interacting contributions.
