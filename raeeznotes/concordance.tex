...
Section~\ref{subsec:three-preprints-chain-level}: in fixed
...
\item \emph{MC3}:  MC3 is proved in type~$A$ (Theorem~\ref{thm:mc3-type-a-resolution}); beyond type~$A$, the categorical lift requires two-sided functoriality
  ...
  The character-level CG closure extends to all Dynkin types by the distributive law: verified computationally for types~$B_2$ and~$G_2$ (\texttt{prefundamental\_cg\_typeB.py}, 46~cases), confirming the algebraic proof of the universal CG theorem.
...
The Lee--Yang singularities $c = -22/5$ and $c' = 152/5$ of $\cA$ and $\cA^!$ are Koszul dual ($c + c' = 26$); the discriminant complementarity $\Delta + \Delta' = 6960/[(5c{+}22)(152{-}5c)]$ (Theorem~\ref{thm:shadow-connection}(vi)) has constant numerator.
...
\item\label{item:kp-ainfty}
  ...
  converse: Theorem~\ref{thm:ainfty-koszul-characterization}.
  \hfill ?
...
\smallskip\noindent\textbf{The shadow metric (unified single-line theory).}
On each primary line~$L$ the shadow tower is controlled by the \emph{shadow metric} $Q_L(t) = (2\kappa + 3\alpha t)^2 + 2\Delta\, t^2$, where $\kappa = S_2$, $\alpha = S_3$, and $\Delta = 8\kappa S_4$ is the critical discriminant (Definition~\ref{def:shadow-metric}).
The MC equation on~$L$ is equivalent to the algebraic relation $H^2 = t^4 Q$ (Theorem~\ref{thm:riccati-algebraicity}), making the shadow tower algebraic of degree~$2$.
The Gaussian decomposition (Corollary~\ref{cor:gaussian-decomposition}) separates $Q = (2\kappa{+}3\alpha t)^2 + 2\Delta\,t^2$ into Gaussian and interacting contributions.
The single-line dichotomy (Theorem~\ref{thm:single-line-dichotomy}) follows: $\Delta = 0$ iff $Q$ is a perfect square iff the tower terminates; $r_{\max} \in \{2,3,\infty\}$.

\smallskip\noindent\textbf{Propagator variance (multi-channel extension).}
The multi-channel extension is the \emph{propagator variance}
$\delta_{\mathrm{mix}} = \operatorname{Var}_\kappa(f) \ge 0$,
computable from arity-$2$ and arity-$4$ data alone
(Proposition~\ref{thm:propagator-variance});
vanishing holds iff the quartic gradient is curvature-proportional.
For~$\mathcal{W}_3$: the mixing polynomial is $P_{\mathcal{W}_3}(c) = 25c^2 + 100c - 428$.
...
\item \emph{Subregular/minimal in $\mathfrak{sl}_3$}:
  Bershadsky--Polyakov duality (Conjecture~\ref{conj:bp-duality}).
...
\item \emph{Per-family primitive kernels.}
  ...
  \end{tabular}
  \end{center}
  The table exhibits the progressive activation of channels across
  the archetype families: the Heisenberg uses only BV
  self-contraction, the affine adds the separating bracket, the
  $\beta\gamma$ adds rigid cutting, and the Virasoro activates all
...
\item \emph{Shell equations as genus-forcing mechanism.}
  ...
  The per-family shell equations are:
...
\item \emph{Branch BV finite-rank reduction.}
  ...
  Gaussian on weight-changing line, $\dim V^{\mathrm{br}} = 1$),
  Virasoro (Computation~\ref{comp:virasoro-branch-bv}: infinite
  series, $\dim V^{\mathrm{br}} = 1$ on primary slice).

\item \emph{Metaplectic half-density.}
  The metaplectic square root
  $\delta_\cA^2 = \Delta_\cA$
  (Corollary~\ref{cor:metaplectic-square-root}) specialises to:
  $(1 - \kappa x)^{1/2}$ for Heisenberg,
  the Weyl denominator for affine, and a genuine power series for
  Virasoro.
...
\item \emph{Flat connection and conformal blocks.}
  ...
  \]
  recovers the KZ connection at genus $0$ (affine), the Hitchin
  connection at higher genus (Virasoro), and the trivial connection
  (Heisenberg) as special cases.
\end{enumerate}
...
\noindent\emph{DS complementarity tower and the $\sigma$-invariant
shadow ring.}
The Feigin--Frenkel involution acts as
$\lambda \mapsto -\lambda$ on the 't~Hooft coupling
(Computation~\ref{comp:ff-thooft-involution}),
giving $K(\widehat{\fg}_k) = 0$ for all affine KM\@.
For principal $\mathcal{W}$-algebras,
$K = K_\fg \cdot \varrho(\fg)$ where
$K_\fg = 2r + 4dh^\vee$ is the Koszul conductor and
$\varrho = \sum 1/(m_i{+}1)$ the exponent-sum invariant
(Theorem~\ref{thm:ds-complementarity-tower-main}).
The $\sigma$-invariant shadow ring $(\cA^{\mathrm{sh}})^\sigma$
(Definition~\ref{def:sigma-invariant-shadow-ring-main})
has a level-independence filtration with threshold at arity~$3$,
and is recursively determined by three initial data
($\Delta^{(2)}$, $\Delta^{(3)}$, $\Delta^{(4)}$)
for Virasoro (verified through arity~$7$).
The Gaberdiel--Gopakumar involution $\lambda \mapsto 1 - \lambda$
is the level-rank involution, distinct from
$\lambda \mapsto -\lambda$.
The $\mathcal{W}_3$ $W$-line shadow tower has the closed-form
generating function
$A(x) = x/3 + [(1{-}12x)^{3/2} - 1]/54$
(Computation~\ref{comp:w-infty-shadow-tower}).
The Bershadsky--Polyakov algebra ($\fsl_3$ subregular DS)
has shadow class~$M$ (infinite depth), confirming that
DS reduction increases shadow depth for both principal and
non-principal reductions ($442$~tests across $9$~modules).
