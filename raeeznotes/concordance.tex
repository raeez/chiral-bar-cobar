...
\smallskip\noindent\textbf{Closed-form generating function (class~$\mathbf{M}$).}
The Virasoro shadow generating function has closed form
$H_{\mathrm{Vir}}(t,c) = t^2\sqrt{c^2 + 12ct + \alpha(c)\,t^2}$
with $\alpha(c) = 36 + 80/(5c{+}22)$
(Theorem~\ref{thm:virasoro-shadow-generating-function}).
The infinite tower ($r_{\max} = \infty$) is algebraic of degree~$2$:
one rational parameter $\alpha(c)$ controls all shadow coefficients.
The deviation from the Gaussian class~$\mathbf{G}$ ($\alpha = 36$)
is the quartic contact invariant $80/(5c{+}22)$.

\smallskip\noindent\textbf{Propagator variance (multi-channel mixing).}
On a multi-channel shadow tower with diagonal curvatures~$\kappa_i$ and quartic gradients~$f_i$, the departure from single-line autonomy is measured by the \emph{propagator variance}
$\delta_{\mathrm{mix}} = \sum_i f_i^2/\kappa_i - (\sum_i f_i)^2/(\sum_i \kappa_i) \geq 0$,
computable from arity-$2$ and arity-$4$ data alone
(Theorem~\ref{thm:propagator-variance}).
Non-negativity is Cauchy--Schwarz; vanishing holds iff the quartic gradient is curvature-proportional (effective one-channel behavior).
For~$\mathcal{W}_3$: the mixing polynomial is $P_{\mathcal{W}_3}(c)=25c^2+100c-428$; the enhanced symmetry loci $P=0$ are the central charges where the quartic gradient aligns with the curvature.
