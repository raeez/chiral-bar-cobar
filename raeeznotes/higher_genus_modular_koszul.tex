...
\item \emph{Virasoro $\mathrm{Vir}_c$}: $\ell_3^{(0)} \neq 0$,
  ...
  computation
  (Theorem~\ref{thm:virasoro-quartic-contact}).
...
Section~\ref{sec:chain-level-modular-convolution-refoundation}.  There the
...
\textup{(}Proposition~\ref{prop:projected-genus-recursion-log-strict-chart}\textup{)}, and the
...
\textup{(}Conjecture~\ref{conj:modular-chern-weil-shadow-principle}\textup{)}.
\end{remark}
...
(cf.~\S\ref{sec:concordance-frontier}): $L_1$ is the scalar
