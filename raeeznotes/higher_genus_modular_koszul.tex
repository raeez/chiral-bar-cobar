...
$\Theta_\cA^{\leq r}$ that is constructive and proved at each finite
order: the modular characteristic $\kappa(\cA)$ at arity~$2$, the
cubic shadow $\mathfrak{C}$ at arity~$3$, the quartic resonance class
$\mathfrak{Q}$ at arity~$4$, with obstruction classes
$o_{r+1}(\cA) \in H^2(\cA^{\mathrm{sh}}_{r+1,0})$ measuring the
failure to extend from arity~$r$ to $r{+}1$.  The full unrestricted
element $\Theta_\cA \in \MC(\gAmod)$ exists by the bar-intrinsic
construction (Theorem~\ref{thm:mc2-bar-intrinsic}); the finite-order
shadows are its projections.
...
The section delivers four constructions and one conjecture:
...
\mathbf{B}_X(\cA^!)$
as underlying $\cD$-module complexes, functorially over
$\overline{\mathcal{M}}_{g,n}$.
The Verdier dual $\mathbb{D}_{\Ran}\mathbf{B}_X(\cA)$ naturally
carries a factorization \emph{algebra} structure
$\cA^!_\infty$; the right-hand side carries a factorization
\emph{coalgebra} structure
(Remark~\ref{rem:verdier-type-convention}).
...
Theorem~\ref{thm:algebraic-family-rigidity} below resolves this
affirmatively for all algebras in algebraic families with
rational OPE coefficients; the remaining open case is restricted to
isolated non-algebraic-family constructions at specific levels.
\end{remark}
...
\item[\textup{(c$'$)}] the OPE structure constants
  ...
  of\/~$k$ on~$U$.
\end{enumerate}
Then $\dim H^2_{\mathrm{cyc,prim}}({\cA}_k) = 0$ for all
$k \in U$ outside a finite exceptional set~$E$.
In particular, $\dim H^2_{\mathrm{cyc}}({\cA}_k, {\cA}_k) = 1$
and\/~${\cA}_k$ is scalar-saturated for all $k \in U \setminus E$.
\end{theorem}

\begin{proof}
\emph{Stage~1: Whitehead reduction.}
The Whitehead decomposition
(Theorem~\ref{thm:cyclic-rigidity-generic}, Stage~1) uses only
hypotheses~(a)--(b) and applies at all~$k \in U$:
\begin{equation}\label{eq:whitehead-family}
H^2_{\mathrm{cyc}}({\cA}_k, {\cA}_k)
\;=\;
\mathbb{C}\!\cdot\!\eta
\;\oplus\;
H^2_{\mathrm{cyc,prim}}({\cA}_k).
\end{equation}
The primitive part consists of $\fg$-equivariant cyclic
$2$-cocycles vanishing on all current-containing pairs.

\medskip
\emph{Stage~2: Finite-dimensional reduction.}
After the Whitehead reduction, a primitive cocycle $c'$ is
determined by its values on primary--primary pairs.  Let
...
\medskip
\emph{Stage~3: Semicontinuity.}
By Theorem~\ref{thm:cyclic-rigidity-generic},
...
\]
is contained in the zero locus of the maximal minors
of~$M(k)$, hence is a \emph{finite} subset of~$U$.
For all $k \in U \setminus E$:
$\ker M(k) = 0$, so
$H^2_{\mathrm{cyc,prim}}({\cA}_k) = 0$,
and $\dim H^2_{\mathrm{cyc}}({\cA}_k) = 1$.
Proposition~\ref{prop:saturation-equivalence}(ii)$\Rightarrow$(i)
gives scalar saturation.
\end{proof}
...
\item \emph{Differential.}
  ...
  \textup{(}matching Construction~\textup{\ref{constr:five-pieces-differential})}:
...
structures on both sides of the adjunction.  This is the deep reason
that homotopy transfer produces the correct genus-refined operations
...
\begin{construction}[Chain-level presentation of the modular homotopy object; \ClaimStatusProvedHere]
\label{constr:three-preprints-chain-level}
Choose a finite affine cover $\mathfrak U$ of the curve supporting~$\cA$ and set
\[
A^{\mathrm{ch}}_\infty := \check C^\bullet(\mathfrak U;\cA).
\]
Malikov--Schechtman~\cite{MS24} identify this \v{C}ech complex with a
homotopy chiral algebra.  For a logarithmic curve or semistable family write
\[
\cC^{\log FM}_{X\mathbin{|}D}(n):=C_\bullet\!\bigl(\operatorname{FM}_n(X\mathbin{|}D)\bigr),
\qquad
\cC^{\log FM}_{W/B}(n):=C_\bullet\!\bigl(\operatorname{FM}_n(W/B)\bigr).
\]
Mok's rigid planted-forest stratification~\cite{Mok25} turns these chains into
an explicit modular coefficient system.  Holding the geometric slot strict and
varying the chiral slot up to $\infty$-morphism, the one-slot functoriality of
Robert-Nicoud--Wierstra~\cite{RNW19} yields a concrete representative
\[
...
\Bigr)
\]
of the invariant object $\Definfmod(\cA)$, together with a strict chart
\[
...
\Bigr).
\]
Theorem~\ref{thm:modular-homotopy-convolution} identifies the filtered
$L_\infty$-quasi-isomorphism class of
$\Definfmod_{\log}(\cA;\mathfrak U)$ with $\Definfmod(\cA)$, while
Corollary~\ref{cor:strictification-comparison} identifies
$\mathfrak g^{\mathrm{mod,log}}_{\cA}(\mathfrak U)$ as the corresponding
strict model.
\end{construction}

\begin{definition}[Rigid/planted-forest depth filtration; \ClaimStatusProvedHere]
\label{def:rigid-planted-forest-depth-filtration}
Each logarithmic boundary type of
$\operatorname{FM}_n(X\mathbin{|}D)$ carries a depth
\[
d(\rho)
:=
\#E_{\mathrm{sep}}(\rho)
...
\]
where the first two terms count separating and non-separating internal
edges and $\operatorname{depth}_{\mathrm{pf}}(\rho)$ is the nesting depth of
its planted-forest component.  Equivalently, $d(\rho)$ is the codimension
of the corresponding rigid/planted-forest logarithmic boundary stratum.
Write
\[
F^d_{\log}\,\mathfrak g^{\mathrm{mod,log}}_{\cA}(\mathfrak U)
:=
\{\Phi\mid \Phi\text{ vanishes on chains supported on strata of depth }<d\}.
\]
Then $\mathfrak g^{\mathrm{mod,log}}_{\cA}(\mathfrak U)$ is complete for the
resulting depth filtration, and every coefficient of $\Theta_\cA$ acquires a
tridegree
\[
(g,n,d)= (\text{loop genus},\text{arity},\text{logarithmic boundary depth}).
\]
This is the concrete filtration induced by Mok's planted-forest and rigid-type
stratification.
\end{definition}

\begin{construction}[Logarithmic modular bar functor; \ClaimStatusProvedHere]
\label{constr:log-modular-bar-functor}
In the coordinates of Construction~\ref{constr:three-preprints-chain-level},
the completed modular bar object is represented by the stable-graph sum
\begin{equation}
\label{eq:log-modular-bar-functor}
\mathrm{B}^{\log}_{\mathrm{mod}}(A^{\mathrm{ch}}_\infty)(g,n)
=
\bigoplus_{\Gamma\in \mathsf{Gr}^{\mathrm{st}}_{g,n}}
\left(
\bigotimes_{v\in V(\Gamma)}
\bar B^{\mathrm{ch}}\bigl(A^{\mathrm{ch}}_\infty;\mathrm{In}(v)\bigr)
\otimes
...
\right)_{\Aut(\Gamma)}.
\end{equation}
The total differential is the five-piece operator
\[
D
=
d_{\mathrm{int}}+[\tau,-]+d_{\mathrm{sew}}+d_{\mathrm{pf}}+\hbar\Delta
\]
of~\eqref{eq:D-five-components}.  In the present chart:
$d_{\mathrm{int}}+[\tau,-]$ is the transferred
$\mathrm{Ch}_\infty$ differential on $A^{\mathrm{ch}}_\infty$;
$d_{\mathrm{sew}}$ is separating clutching;
$d_{\mathrm{pf}}$ is the planted-forest correction from codimension-$\geq2$
log-FM boundary strata; and $\hbar\Delta$ is non-separating self-gluing.
The bar-intrinsic Maurer--Cartan class is therefore
\[
\Theta_\cA
=
D-\bigl(d_{\mathrm{int}}+[\tau,-]\bigr),
\]
now viewed as the positive-genus part of the logarithmic modular bar
functor rather than as an extra external package.  Equivalently it admits the
explicit logarithmic graph expansion
\begin{equation}
\label{eq:theta-logarithmic-graph-expansion}
\Theta_\cA
=
\sum_{\Gamma}
\frac{1}{|\Aut(\Gamma)|}\,
W^{\log}_{\Gamma}\otimes \Phi^{\cA}_{\Gamma},
\end{equation}
where the sum runs over connected stable graphs carrying at least one
separating edge, planted-forest corner, or loop;
$W^{\log}_{\Gamma}\in C_\bullet\!\bigl(\operatorname{FM}^{\log}_{\Gamma}\bigr)$ is
the chain class of the corresponding rigid logarithmic boundary stratum; and
$\Phi^{\cA}_{\Gamma}$ is obtained by decorating each vertex with the
transferred operation $\ell^{\mathrm{tr}}_{|v|}$, each internal edge with the
propagator $P_\cA$, and each loop with the trace contraction.
\end{construction}

\begin{construction}[Graphwise log-FM cocomposition and the explicit modular $L_\infty$-brackets; \ClaimStatusProvedHere]
\label{constr:graphwise-log-fm-cocomposition}
For every rigid stable graph $\Gamma\in \mathsf{Gr}^{\mathrm{st}}_{g,n}$, let
$D^{\log}_{\Gamma}\subset \operatorname{FM}^{\log}_n(X\mathbin{|}D)$ be the
corresponding boundary stratum, $\widetilde D^{\log}_{\Gamma}\to D^{\log}_{\Gamma}$
...
\]
the proper birational correspondence of Mok.  The graphwise cocomposition map is
\begin{equation}
...
\colon
\cC^{\log FM}_{\mathrm{mod}}(g,n)
\longrightarrow
\bigotimes_{v\in V(\Gamma)}
\cC^{\log FM}_{\mathrm{mod}}\!\bigl(g(v),I_v\sqcup H(v)\bigr)[-|E_{\mathrm{int}}(\Gamma)|].
\end{equation}
If $f_1,\dots,f_k\in \mathfrak g^{\mathrm{mod,log}}_{\cA}(\mathfrak U)$ and
$|V(\Gamma)|=k$, let $\mu_\Gamma$ be the graph composition in
$\operatorname{End}_{\mathrm{Ch}_\infty}(A^{\mathrm{ch}}_\infty)$ obtained by
decorating each vertex with the transferred chiral operation and
contracting each internal edge with the cyclic kernel $P_\cA$.
Writing $\alpha^{\mathrm{mod}}_\Gamma$ for the component of the universal
twisting morphism on the boundary type~$\Gamma$, the $\Gamma$-amplitude is
\begin{equation}
...
\Delta^{\log}_{\Gamma}.
\end{equation}
Summing over stable graphs gives the explicit Taylor coefficients
\begin{equation}
...
\ell_\Gamma.
\end{equation}
Equations~\eqref{eq:graphwise-log-cocomposition}--\eqref{eq:graphwise-taylor-coefficients}
are the concrete chain-level incarnation of the Robert-Nicoud--Wierstra
convolution formula in the log-FM chart.
\end{construction}

\begin{construction}[Boundary operators as residue correspondences; \ClaimStatusProvedHere]
\label{constr:boundary-operators-residue-correspondence}
On a pure tensor $a\otimes\eta$ in the summand indexed by a stable graph
$\Gamma$, the geometric pieces of the modular bar differential are given by
residue/pushforward formulas
\begin{align}
...
\end{align}
Here $E_{\mathrm{sep}}(\Gamma)$ and $E_{\mathrm{ns}}(\Gamma)$ denote the
separating and non-separating internal edges, $\mu_e$ and $\mu_F$ are the
corresponding vertexwise chiral compositions, and $\operatorname{Tr}_e$
contracts the half-edges with the cyclic pairing.  Thus codimension-one
separating strata produce $d_{\mathrm{sew}}$, codimension-one
non-separating strata produce $\Delta$, and rigid codimension-two
planted-forest strata produce $d_{\mathrm{pf}}$.  The identity $D^2=0$ is
therefore the chain-level cancellation of codimension-two faces in Mok's
normal-crossings compactification.
\end{construction}

\begin{computation}[Low-genus amplitudes in the log-FM chart; \ClaimStatusProvedHere]
\label{comp:low-genus-log-fm-chart}
The first Taylor coefficients of
$\Definfmod_{\log}(\cA;\mathfrak U)$ are precisely the graph amplitudes of
...
\bigr).
\]
Consequently the three fundamental low-genus archetypes are recovered in one
uniform chart: Heisenberg is Gaussian
$\bigl(\ell_3^{(0)}=0\bigr)$, affine
$\widehat{\mathfrak{sl}}_2$ is Lie/tree
$\bigl(\ell_3^{(0)}\neq0,\ \ell_4^{(0)}=0\bigr)$, and Virasoro together
with principal $\mathcal W_N$ is genuinely nonlinear
$\bigl(\ell_3^{(0)}\neq0,\ \ell_4^{(0)}\neq0\bigr)$.
\end{computation}

\begin{computation}[First stable-graph shells and the genus-two entry point; \ClaimStatusProvedHere]
\label{comp:first-stable-graph-shells}
Write
\[
\Theta_\cA
=
...
\frac{W^{\log FM}_\Gamma}{|\Aut(\Gamma)|}\,\Phi^\cA_\Gamma.
\]
Then
\begin{align*}
\Theta_\cA^{(1)}
&=
\Theta_{\cA,\mathrm{loop}}^{(1)},\\
\Theta_\cA^{(2)}
&=
...
\end{align*}
The three genus-two shells correspond respectively to iterated
non-separating self-gluing, mixed separating/non-separating gluing,
and the first rigid planted-forest correction.  For Heisenberg only the
pure loop shell survives; for affine $\widehat{\mathfrak{sl}}_2$ the
mixed separating/loop shell also survives; for Virasoro and principal
$\mathcal W_N$ all three shells occur.  Thus genus~$2$ is the first
level at which loops, clutching, and planted forests interact genuinely
rather than sequentially.
\end{computation}

\begin{remark}[Rigid-type boundary factorization]
\label{rem:rigid-type-boundary-factorization}
For a separating degeneration with rigid type~$\rho$, Mok's degeneration
correspondence
\[
\operatorname{FM}_n(W/B)(\rho)\longrightarrow
\prod_{v\in V(S_\rho)}
\operatorname{FM}_{I_v}(Y_v\mathbin{|}D_v)
\]
turns the abstract clutching identity for $\Theta_\cA$ into the chain-level
factorization law
\[
\Theta_\cA\big|_{\rho}
=
...
\right).
\]
The contact term, the planted-forest correction, and the non-separating loop
term are thus not parallel structures: they are the three boundary faces of the
same log-FM bar differential.  This is the concrete meaning of the slogan that
the three foundational papers force a rewrite of MC2 into a modular-convolution
programme.
\end{remark}


\begin{construction}[Rigid planted-forest correction as a push--pull sum; \ClaimStatusProvedHere]
\label{constr:rigid-planted-forest-push-pull}
Let $\mathsf{PF}^{\mathrm{rig}}_{g,n}$ denote Mok's rigid planted-forest
combinatorial types for the special fibre of $\operatorname{FM}_n(W/B)$.  For
each $\rho\in \mathsf{PF}^{\mathrm{rig}}_{g,n}$ there is a proper birational
map
\[
...
\operatorname{FM}_{I_v}(Y_v\mathbin{|}D_v)
\]
and the induced clutching pushforward
\[
(\kappa_\rho)_*\colon
C_\bullet\!\Bigl(
\prod_{v\in V(S_\rho)}
...
\]
The planted-forest component of the logarithmic bar differential is the
completed push--pull sum
\begin{equation}
\label{eq:dpf-push-pull}
d_{\mathrm{pf}}
=
...
$\mathrm{Ch}_\infty$-operations and contracts them according to the planted
forest $S_\rho$.  Thus $d_{\mathrm{pf}}$ is literally Mok's degeneration
formula transported into the modular convolution algebra.
\end{construction}

\begin{corollary}[The log-FM chart identifies $\Theta_\cA$ with modular twisting data; \ClaimStatusProvedHere]
\label{cor:theta-log-fm-twisting-data}
The Deligne--Getzler--Hinich deformation space of the log-FM chart is
canonically equivalent to the simplicial set of modular twisting data:
\[
...
\Bigr).
\]
Under this identification the bar-intrinsic class $\Theta_\cA$ is the
universal modular twisting morphism determined by the total differential
$D$ of~\eqref{eq:log-modular-bar-functor}.
\end{corollary}

\begin{proof}
Corollary~\ref{cor:theta-twisting-morphism} identifies Maurer--Cartan points
of a convolution $L_\infty$-algebra with twisting morphisms.  Apply it to the
strict chart of Construction~\ref{constr:three-preprints-chain-level}, using
Robert-Nicoud--Wierstra's operadic identification
\cite[Theorem~4.1]{RNW19} and Vallette's rectification theorem
\cite[Theorem~1.2]{Val16}.
\end{proof}

\begin{proposition}[First two weights of the universal graph expansion; \ClaimStatusProvedHere]
\label{prop:first-two-weights-log-fm}
Let $D_{\mathrm{loc}}:=d_{\mathrm{int}}+[\tau,-]$ and decompose the positive
part of the total differential by internal-edge weight:
\[
\Theta_\cA=\sum_{r\ge 1}\Theta_\cA^{[r]}.
\]
Then
\begin{equation}
\label{eq:first-two-weight-relations}
[D_{\mathrm{loc}},\Theta_\cA^{[1]}]=0,
\qquad
[D_{\mathrm{loc}},\Theta_\cA^{[2]}]
+\frac12[\Theta_\cA^{[1]},\Theta_\cA^{[1]}]=0.
\end{equation}
Moreover the first weight is completely explicit:
\begin{equation}
...
\end{equation}
Under the \v{C}ech comparison, the combination
$\Phi_s+\Phi_t+\Phi_u$ is the Malikov--Schechtman secondary Borcherds
Jacobiator homotopy $F_3$.  Every genuinely logarithmic contribution first
appears in weight~$2$, where
\begin{equation}
...
\begin{proof}
Write $D=D_{\mathrm{loc}}+\Theta_\cA$ and expand the identity $D^2=0$ by
internal-edge weight.  The weight-$1$ and weight-$2$ components give
\eqref{eq:first-two-weight-relations}.  Formula
\eqref{eq:weight-one-boundary-terms} is the explicit description of the three
boundary points of $\overline{\cM}_{0,4}$ and the irreducible boundary of
$\overline{\cM}_{1,1}$ under
Construction~\ref{constr:explicit-convolution-linfty}.  The last statement is
exactly the push--pull formula~\eqref{eq:dpf-push-pull} for the rigid
planted-forest sector.
\end{proof}

\begin{remark}[What first becomes genuinely modular]
\label{rem:first-genuinely-modular}
Weight~$1$ already sees the Jacobiator homotopy and the one-loop BV operator,
but it does not yet force the full logarithmic geometry.  Weight~$2$ is the
first place where Mok's rigid planted-forest types enter, so it is the first
layer in which the modular theory stops being a tree-plus-loop correction and
becomes genuinely logarithmic.  In Volume~II this principle becomes
computational: for the classical $W_3$ central-parameter line the quadratic
self-coupling of the unique genus-one tangent direction is exact, so any
nontrivial genus-two obstruction must come from the rigid planted-forest
sector.
\end{remark}

\begin{definition}[Modular tangent complex and characteristic projections; \ClaimStatusProvedHere]
\label{def:modular-tangent-complex}
Let $\Theta_\cA\in \operatorname{MC}(\Definfmod_{\log}(\cA;\mathfrak U))$.
The twisted differential on the underlying complete complex is
\begin{equation}
\label{eq:twisted-differential-theta}
d_{\Theta_\cA}(x)
:=
...
\end{equation}
The resulting complete complex
\[
T^{\mathrm{mod}}_{\Theta_\cA}
:=
\bigl(\Definfmod_{\log}(\cA;\mathfrak U),\,d_{\Theta_\cA}\bigr)
\]
is the \emph{modular tangent complex}.  Its first characteristic
projections are
\begin{equation}
\label{eq:characteristic-projections-theta}
...
\end{equation}
Thus the scalar characteristic, the spectral discriminant, and the
quartic resonance class are all concrete shadows of one twisted complex.
\end{definition}

\begin{remark}[Modular Chern--Weil interpretation]
\label{rem:modular-chern-weil-endstate}
Definition~\ref{def:modular-tangent-complex} is the end-state
interpretation of the theory: the three foundational papers produce not
merely a deformation complex but a modular Chern--Weil machine.  The
homotopy chiral input of Malikov--Schechtman gives the connection data,
the log-FM chains of Mok give the curvature coefficients, and the
convolution formalism of Robert-Nicoud--Wierstra together with Vallette
turns the graph expansion into a homotopy-invariant characteristic
package.  In this sense
\[
\Theta_\cA
=
\sum_{\Gamma}
\frac{W^{\log FM}_\Gamma}{|\Aut(\Gamma)|}\,\Phi^\cA_\Gamma
\]
is the universal modular curvature field of the theory.
\end{remark}

\begin{remark}[Genus recursion and characteristic shadows]
\label{rem:genus-recursion-and-characteristic-shadows-pointer}
The concrete chain chart of the present subsection is developed further in
Section~\ref{subsec:three-preprints-chain-level}.  There the
fixed-$(g,n)$ projection of the Maurer--Cartan equation is rewritten as a
three-source stable-graph recursion with separating, non-separating, and
planted-forest terms
\textup{(}Equation~\ref{eq:all-genus-recursion-logfm}\textup{)}, and the
scalar, spectral, cubic, quartic, and higher packages are organized as
characteristic shadows of the twisted tangent complex at $\Theta_\cA$
\textup{(}cf.~Definition~\ref{def:modular-chern-weil-transform}\textup{)}.
\end{remark}
...
The primary mathematical object of this monograph is the
...
\begin{example}[The standard archetypes in miniature]
\label{ex:tower-four-archetypes}
\index{shadow Postnikov tower!archetype table}
\begin{center}
\small
\renewcommand{\arraystretch}{1.15}
\begin{tabular}{lcccc}
\toprule
& \emph{Step~1} & \emph{Step~2} & \emph{Step~3}
  & \emph{Termination} \\
\midrule
Heisenberg &
  $\kappa$ & $o_3 = 0$ & $o_4 = 0$  & arity~$2$ \\
Affine &
  $\kappa$ & $\mathfrak{C} = \kappa(x,[y,z])$ &
  $o_4 = 0$ (Jacobi) & arity~$3$ \\
$\beta\gamma$ &
  $\kappa{=}0$ & $o_3 = 0$ &
  $\mathfrak{Q} = \operatorname{cyc}(m_3)$ & arity~$4$ \\
Virasoro &
  $\kappa{=}c/2$ & $\mathfrak{C} = 2x^3$ &
...
Conjecture~\textup{\ref{conj:operadic-complexity}}.\textup{)}
...
\begin{example}[Verification for the standard archetypes]
...
does not terminate).
The \emph{shadow depth class} of~$\cA$ is:
\begin{itemize}[nosep]
\item $\mathbf{F}_r$ (finite, depth~$r$) if
  $r_{\max}(\cA) = r < \infty$, i.e.\
  $o_{r}(\cA) \neq 0$ but $o_{s}(\cA) = 0$ for all $s > r$;
\item $\mathbf{M}$ (mixed) if
  $r_{\max}(\cA) = \infty$, i.e.\
  $o_r(\cA) \neq 0$ for infinitely many~$r$.
\end{itemize}
Equivalently, $\mathbf{F}_r$ means $\gAmod$ is formal above
arity~$r$ (the $L_\infty$ structure truncates), while
$\mathbf{M}$ means $\gAmod$ is intrinsically non-formal.
...
\end{tabular}
\end{center}
The names $\mathbf{G}$, $\mathbf{L}$, $\mathbf{C}$ are
shorthand for the standard-landscape representatives.
There is no structural obstruction to realising
$\mathbf{F}_r$ for $r \geq 5$; the gap between
$\mathbf{F}_4$ and $\mathbf{M}$ is an empirical feature
of the computed families, not a theorem.
\end{definition}
...
\begin{remark}[Envelope-shadow functor]
\label{rem:envelope-shadow-functor-xref}
\index{envelope-shadow functor!from operadic complexity}
When the chiral algebra arises as an enveloping vertex algebra
$\cA = \Uvert(R)$ of a Lie conformal algebra~$R$
(Nishinaka~\cite{Nish26}), the shadow depth becomes a functor
on Lie conformal input:
$\chienv(R) := r_{\max}(\Uvert(R))$
(Definition~\ref{def:envelope-shadow-complexity}).
The arity-$r$ shadow depends only on the $(r{+}1)$-jet
$\Uvert(R)/F_{r+1}$
(Proposition~\ref{prop:finite-jet-rigidity}), and is
polynomial of degree~$\leq r{-}1$ in a central-extension
parameter
(Proposition~\ref{prop:polynomial-level-dependence}).
The Gaussian collapse
(Proposition~\ref{prop:gaussian-collapse-abelian})
identifies Heisenberg as the universal fixed point.
See \S\ref{sec:concordance-nishinaka-vicedo} for the full
development and conjectural frontier (DS descent, plumbing-Taylor,
Lie-conformal complementarity, critical-level concentration,
super shadow machine).
\end{remark}
...
\begin{conjecture}[Shadow depth is a homotopy invariant]
\label{conj:shadow-depth-invariant}
\ClaimStatusConjectured
\index{shadow depth!invariance conjecture}
\begin{enumerate}[label=\textup{(\roman*)}]
...
\end{enumerate}
Part~(i) follows from
Conjecture~\textup{\ref{conj:operadic-complexity-detailed}},
since $d_\infty$ is manifestly a quasi-isomorphism
invariant of the minimal model.
Part~(ii) follows from homotopy invariance of the shadow
algebra
(Theorem~\ref{thm:shadow-homotopy-invariance}): generic
deformations preserve homotopy type.
\end{conjecture}

\begin{conjecture}[Tropical Koszulness]
...
\begin{remark}[Tropical descent]
\label{rem:tropical-descent-gap}
\index{tropical Koszulness!descent obstruction}
\index{tropical descent}
...
The implication ``tropical acyclicity $\Rightarrow$
algebraic acyclicity'' requires a \emph{tropical descent
theorem}: acyclicity of the tropicalization of a
filtered complex implies acyclicity of the original.
For smooth pairs $(X,D)$, the relevant weight spectral
sequence converges at~$E_1$ by formality of the real
blowup; for nodal degenerations, convergence is not
established.  This tropical descent is the single
analytic input needed to promote
Conjecture~\ref{conj:tropical-koszulness} to a theorem.
It is closely related to the planted-forest
tropicalization
...
\begin{conjecture}[Stratified curve-independence at
  higher genus]
\label{conj:stratified-curve-independence}
\ClaimStatusConjectured
\index{curve-independence!stratified conjecture|textbf}
\index{modular Koszulness!curve-independence}
Let $\cA$ be a modular Koszul chiral algebra.
\begin{enumerate}[label=\textup{(\roman*)}]
\item \emph{Open stratum}:
  On $\mathcal{M}_g$ (smooth curves of genus~$g$),
  modular Koszulness is constant.
  The shadow algebra $\cA^{\mathrm{sh}}$ is locally
  constant on $\mathcal{M}_g$ by homotopy invariance
...
\begin{remark}[Evidence for higher-genus curve-independence]
\label{rem:curve-independence-evidence}
\index{curve-independence!evidence}
Three observations support
Conjecture~\textup{\ref{conj:stratified-curve-independence}}:
\textup{(1)}~homotopy invariance of
$\cA^{\mathrm{sh}}$
(Theorem~\ref{thm:shadow-homotopy-invariance})
implies local constancy on connected families;
\textup{(2)}~the curvature $\kappa(\cA) \cdot \omega_g$
...
connected strata;
\textup{(3)}~the clutching law for $\mathfrak{Q}(\cA)$
...
\begin{proposition}[Chriss--Ginzburg structure principle; \ClaimStatusProvedHere]
...
\begin{remark}[The Chriss--Ginzburg philosophy for factorization algebras]
\label{rem:chriss-ginzburg-philosophy}
\index{Chriss--Ginzburg structure principle!factorization algebra interpretation}
This is the Chriss--Ginzburg philosophy applied to factorization
algebras on curves: master objects are defined directly as
...
\begin{proof}
The key observation is that the ambient differential $D_{\cA}$ is
...
(cf.~\S\ref{sec:concordance-koszulness-programme}): $L_1$ is the scalar
