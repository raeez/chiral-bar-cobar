...
partitions the standard Lie-theoretic landscape into four shadow-depth
classes: Gaussian ($r_{\max} = 2$), Lie ($r_{\max} = 3$), contact
($r_{\max} = 4$), and mixed ($r_{\max} = \infty$), with $\Delta = 0$
on the primary line forcing finite shadow depth and $\Delta \ne 0$
forcing infinite shadow depth.
...
On the formal disk, the genus-$0$ chiral bar construction
recovers the classical bar complex; already on~$\mathbb{P}^1$,
Fulton--MacPherson compactifications and Arnold relations
enrich the picture beyond the classical setting.  At
genus~$g \ge 1$, the Hodge bundle forces genuinely new
...
Five movements.  First: the universal MC element $\Theta_\cA$
and its shadow obstruction tower.  Second: algebraicity, the tower
as a square root $H = t^2\sqrt{Q_L}$ controlled by a quadratic form.
Third: the four-class partition G/L/C/M from the critical
discriminant~$\Delta$.  Fourth: explicit computations (genus-$2$
graph sums, the $E_8$ test, gravitational coproduct primitivity).
Fifth: the arithmetic frontier (Epstein zeta of the shadow metric,
the shadow field, the descent fan from $\Theta_\cA$ to three
mathematical worlds).
...
genus-$0$, arity-$2$, $\Delta = 0$ stratum via formal-disk
restriction, but the embedding is not an equivalence:
the deformation retract from the curve to a point is
additional data, and the configuration-space geometry on
$\mathbb{P}^1$ has no classical counterpart.
Everything beyond class~$\mathbf{G}$ is new; already within
class~$\mathbf{G}$, the chiral theory is richer than the
classical one.
...
$\delta F_2 = (c+204)/(16c)$.  The multi-generator universality
problem is resolved negatively: this correction is $R$-matrix
independent and is not cancelled by CohFT $R$-matrix terms.
...
compose into a single functor: a \emph{fan}, not a chain.
...
\emph{Proved unconditionally}: algebraicity
(Theorem~\ref{thm:main}(i)--(ii)); the four-class
classification; $F_g = \kappa \cdot \lambda_g^{\mathrm{FP}}$
for all single-generator chirally Koszul algebras at all genera;
genus-$1$ universality $F_1 = \kappa/24$ for all
families including multi-generator; gravitational
...
The genus-$2$ $\cW_3$ computation (\S\ref{sec:genus2-w3})
exhibits the cross-channel correction that any proof must cancel.
Whether the Koszul--Epstein zeta has zeros on a single critical
line remains open: it requires understanding the arithmetic of
the shadow field beyond present methods.

\subsection{Relation to existing work}

The chiral bar construction originates with
Beilinson--Drinfeld~\cite{BD04} and was developed in the
factorization framework by Francis--Gaitsgory~\cite{FG12}
and Costello--Gwilliam~\cite{CG17}.
The modular characteristic~$\kappa$ appears implicitly in
Feigin--Fuchs's work on Hodge bundles of conformal blocks;
$\kappa(\Vir_c) = c/2$ can be extracted from
Beilinson--Schechtman~\cite{BS88}.
Moduli-space invariants from conformal blocks are treated by
Faber--Pandharipande~\cite{FP00}, Mumford~\cite{Mumford83}, and
the CohFT literature following
Kontsevich--Manin~\cite{KM94}.  The Epstein zeta function
of a binary quadratic form is due to Epstein~\cite{Epstein1903},
with functional equation and Bessel expansion due
to Terras~\cite{Terras73}.  The conformal/modular bootstrap
connection is studied by Benjamin--Chang~\cite{BenjaminChang22}.
...
Koszul--Epstein construction appear to be new.  The link
between shadow depth and $L_\infty$ non-formality, while
consonant with Kontsevich's philosophy~\cite{Kontsevich03},
has not been established in the chiral setting.

Drinfeld--Sokolov reduction and its homological algebra are
classical~\cite{KRW03, FF90, DS85};
gravitational coproduct primitivity
...
The identity $F_g = \kappa \cdot \lambda_g^{\mathrm{FP}}$ fails at
genus~$g \ge 2$ for multi-weight algebras.
The genus-$2$ $\cW_3$ computation
(\S\ref{sec:genus2-w3}) exhibits a cross-channel
correction $\delta F_2 = (c+204)/(16c)$ that is $R$-matrix
independent: the CohFT $R$-matrix does not produce compensating
terms.  The correct decomposition is
$F_g = \kappa \cdot \lambda_g^{\mathrm{FP}} +
\delta F_g^{\mathrm{cross}}$.
The structure of $\delta F_g^{\mathrm{cross}}$ at
genus~$g \ge 3$ remains to be computed.
