...
# ============================================================================
# TIER 8: exact hook arithmetic and typed DS--bar obligations
# ============================================================================

from sympy import Symbol


class TestHookDSBarTypedSurface:
    """Exact hook data and the named comparison packages."""

    @pytest.mark.parametrize(
        ("partition", "expected"),
        [
            ((3, 1), (-24 * Symbol("k")**2 - 115 * Symbol("k") - 136) / (Symbol("k") + 4)),
            ((2, 1, 1), (-6 * Symbol("k")**2 - 9 * Symbol("k")) / (Symbol("k") + 4)),
            ((4, 1), (-60 * Symbol("k")**2 - 429 * Symbol("k") - 765) / (Symbol("k") + 5)),
            ((3, 1, 1), (-24 * Symbol("k")**2 - 134 * Symbol("k") - 190) / (Symbol("k") + 5)),
            ((2, 1, 1, 1), (-6 * Symbol("k")**2 - 5 * Symbol("k") + 5) / (Symbol("k") + 5)),
        ],
    )
    def test_krw_hook_formulas(self, partition, expected):
        from sympy import Symbol, simplify
        from compute.lib.hook_type_w_duality import krw_central_charge

        level = Symbol("k")
        assert simplify(krw_central_charge(partition, level) - expected) == 0
    ...
    def test_hook_generator_ledgers_are_even(self, partition, count):
        ...
        assert data.n_even == count
        assert data.n_odd == 0
        assert {parity for _, _, parity in data.strong_generators} == {"even"}

    @pytest.mark.parametrize(
        "partition",
        [(3, 1), (2, 1, 1), (4, 1), (3, 1, 1), (2, 1, 1, 1)],
    )
    def test_rho_and_kappa_require_modular_input(self, partition):
        ...
        rho = anomaly_ratio_from_partition(partition)
        kappa = ds_kappa_from_affine(partition)
        assert rho.status is ClaimStatus.OPEN
        assert rho.value is None
        assert kappa.status is ClaimStatus.CONDITIONAL
        assert kappa.value is None
        assert any("H_hook^{DS/bar}" in item for item in kappa.hypotheses)

    def test_sl4_transpose_pair_is_partition_combinatorics(self):
        from sympy import Symbol, simplify
        from compute.lib.hook_type_w_duality import (
            ClaimStatus,
        ...
        assert simplify(hook_dual_level_sl_n(4, hook_dual_level_sl_n(4, level)) - level) == 0

        data = sl4_hook_duality_data(level)
        assert data.transpose_relation
        assert data.ds_bar_commutation.status is ClaimStatus.CONDITIONAL
        assert data.koszul_duality.status is ClaimStatus.CONDITIONAL
        assert data.ksdual_membership.status is ClaimStatus.OPEN
        assert data.kappa_sum.status is ClaimStatus.OPEN

    @pytest.mark.parametrize("partition", [(3, 1), (2, 1, 1), (4, 1)])
    def test_bar_h1_is_a_conditional_comparison(self, partition):
        from compute.lib.hook_type_w_duality import (
            ClaimStatus,
        ...
        assert bar_degree_one_generator_count(partition) == (
            w_algebra_generator_data(partition).f_centralizer_dimension
        )
        packet = bar_cohomology_h1_generators(partition)
