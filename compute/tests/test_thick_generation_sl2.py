...
# =========================================================================
# 10. Tensor ideal structure (from CG ramification)
# =========================================================================

class TestTensorIdealStructure:
    """The tensor ideal generated by L- in O_Y."""

    def test_l_minus_generates_verma_characters(self):
        """All Verma character containments hold via CG: ch(M(lam)) <= ch(V_lam tensor L-)."""
        ...
        for lam in range(0, 15):
            ...
            assert r["containment"], f"Containment fails at lam={lam}"

    def test_tensor_ideal_closed_under_eval_tensor(self):
        """thick<L-, V_1> is closed under V_n-tensoring via CG."""
        from compute.lib.prefundamental_clebsch_gordan import (
            prefundamental_clebsch_gordan as pcg,
        )
        for n in range(1, 10):
            r = pcg(n, depth=40)
            assert r["match"], f"CG fails at n={n}, tensor ideal not closed"

    def test_rep_fd_in_thick_closure(self):
        """All V_n are in thick<V_1> by iterated evaluation CG."""
        from compute.lib.sl2_baxter import eval_module_Vn, tensor_product_characters
        # V_1 tensor V_n = V_{n+1} + V_{n-1} => V_{n+1} in thick<V_1,...,V_n>
        # By induction, all V_n in thick<V_1>.
        V1 = eval_module_Vn(1)
        for n in range(1, 8):
            Vn = eval_module_Vn(n)
            tensor = tensor_product_characters(V1, Vn)
            # Check tensor has weights n+1, n-1, ..., -(n+1) as expected
            max_w = max(tensor.keys())
            assert max_w == n + 1, f"V_1 tensor V_{n} max weight = {max_w}, expected {n+1}"


# =========================================================================
# 11. Ext^1 nearest-neighbor pattern
# =========================================================================

class TestExtNearestNeighbor:
    """Ext^1(L-(a), L-(b)) is nonzero only for |a-b| small (nearest-neighbor).

    At the character level, this is detected by checking that
    Hom(L-, V_n tensor L-) = 0 for odd n (parity obstruction)
    and nonzero for even n, with the first nonzero contribution
    concentrated at the boundary of the CG summand decomposition.
    """

    def test_hom_parity_obstruction(self):
        """Hom(L-, V_n tensor L-) = 0 for odd n (parity mismatch)."""
        ...
        for n in [1, 3, 5, 7]:
            ...
            # tensor has only weights with parity n%2, L has even weights
            # so no common weights => Hom = 0
            common = set(tensor.keys()) & set(L.keys())
            assert len(common) == 0, f"Common weights at odd n={n}: {common}"

    @pytest.mark.parametrize("n", [0, 2, 4, 6])
    def test_hom_nonzero_even_n(self, n):
        """Hom(L-, V_n tensor L-) > 0 for even n."""
        r = hom_bound_L_minus_Vn(n)
        assert r["hom_weight_bound"] > 0

    def test_hom_dimension_grows_with_n(self):
        """dim Hom(L-, V_n tensor L-) = floor(n/2)+1 for even n."""
