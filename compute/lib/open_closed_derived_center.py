...
class ChiralHochschildComplex:
    ...
    def euler_characteristic(self) -> int:
        ...
        This is an Euler characteristic of the truncated cochain model,
        not a Hilbert series for B(A), A^i, or A^!.
        """
        chi = 0
...
def modular_characteristic(algebra: ChiralAlgebraData) -> Fraction:
    ...
    For each family:
      ...
      Virasoro: kappa = c/2
      W_3: kappa = 5c/6 (= c/2 from T + c/3 from W)

    These formulas are family-specific.
    """
    name = algebra.name
    ...
    if name == "Heisenberg":
        ...
        # kappa(W_3) = c(H_3 - 1), H_3 = 1 + 1/2 + 1/3.
...
def derived_center_dimensions(
    ...
    open/closed resonance computations:
      Z^0: vacuum centre line,
      Z^1: level or central-charge deformation line,
      Z^2: obstruction/trace line paired with Z^0 on the Koszul locus,
      Z^n: zero for n >= 3 in this scalar model.

    This function does not compute the bar coalgebra B(A), the Koszul
    cohomology coalgebra A^i, or the Koszul dual algebra A^!.
    """
    # Z^0: center of A
    ...
    # Z^2: scalar obstruction/trace line paired with Z^0.
    z2 = z0
...
# ======================================================================
#  Koszul-dual labels and derived-center degree pairing
# ======================================================================

def koszul_dual_family(name: str) -> str:
    """
    Return the Koszul dual algebra-family label.
...
def koszul_duality_check(algebra: ChiralAlgebraData) -> bool:
    """
    Check the degree-pairing shadow on derived-center dimensions.

    This compatibility check is deliberately weaker than constructing
    A^!. The construction of A^! runs through A^i = H^*(B(A)) and
    Verdier/linear duality; bar-cobar inversion Omega(B(A)) = A is a
    different operation. The predicate below only verifies the scalar
    pairing dim Z^0 = dim Z^2 and concentration in degrees {0, 1, 2}.
    """
    dims = derived_center_dimensions(algebra)
    return set(dims) <= {0, 1, 2} and dims[0] == dims[2] and dims[1] >= 0
