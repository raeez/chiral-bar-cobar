...
U^mod_X(L) records higher-genus data in the modular Koszul projection
package Pi_X(L).
...
The modular Koszul projection package:
  Pi_E(L_bdy) = (F_E(L_bdy), B-bar_E(L_bdy), Theta_L, L_L, (V^br, T^br), R_4^mod)

This six-projection package is not the seven-entry holographic package
(A, A^i, A^!, C, r(z), Theta_A, nabla^hol).  The bar entry is a
coalgebraic projection; it is not the Verdier/Koszul branch A^!.
...
# =========================================================================
# Section 11: Modular Koszul projection package assembly
# =========================================================================

@dataclass
class PlatonicDatum:
    r"""The modular Koszul projection package Pi_E(L_bdy).
    ...
    This is a six-primary-projection compute object.  It is distinct
    from the seven-entry holographic package, and its B-bar_E(L) entry
    is the bar coalgebra.  The Verdier/Koszul branch A^! is obtained
    only after the finite-type or completed continuous-dual step.
...
def boundary_platonic_datum(gmax: int = 5) -> PlatonicDatum:
    """Construct the modular Koszul projection package for the boundary algebra."""
    fg = {}
    for g in range(1, gmax + 1):
...
def k3_sigma_platonic_datum(gmax: int = 5) -> PlatonicDatum:
    """Construct the modular Koszul projection package for the K3 sigma model."""
    fg = {}
    for g in range(1, gmax + 1):
...
# =========================================================================
# Section 12: Verdier/Koszul branch and complementarity
# =========================================================================

def boundary_koszul_dual_data() -> Dict[str, Any]:
    r"""Verdier/Koszul branch data for the boundary algebra A_E.

    A_E = Heisenberg rank 24, kappa = 24.
    A_E^! denotes the Verdier/Koszul branch, with kappa(A_E^!) = -24
    in the finite-type free-field model.

    For free fields (AP24): kappa + kappa' = 0.
    For the boundary algebra: kappa = 24, kappa' = -24, sum = 0.

    The branch A_E^! is the curved second-kind dual-Heisenberg
    Sym^{ch}(h_{24}^*[1]) branch.  It is obtained from the bar-dual
    coalgebra A_E^i by Verdier duality, not by the bar-cobar inversion
    Omega B(A_E) ~= A_E.

    AP33 WARNING: at k != 0, H_k^! is not H_{-k} and not the uncurved
    ...
    return {
        ...
        'dual_type': 'curved second-kind Sym^{ch}(h_{24}*[1]) with dual Mukai pairing',
        'ap33_check': True,  # curved dual branch is not H_{-k}
    }


def k3_sigma_koszul_dual_data() -> Dict[str, Any]:
    r"""Verdier/Koszul branch data for the K3 sigma model VOA.

    V_{K3} has c = 6, kappa = 2.
    The Verdier/Koszul branch V_{K3}^! has kappa(V_{K3}^!) = -2 on
    the finite-type free-field comparison surface.
    ...
    This routine records the scalar branch package only.  Identifying
    V_{K3}^! with a sigma-model VOA at a specific dual moduli point
    requires a separate geometric comparison.
    """
    return {
        'kappa_A': F(2),
        'kappa_A_dual': F(-2),
