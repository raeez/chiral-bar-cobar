...
def non_koszul_ope(epsilon=None) -> TropicalOPEData:
    ...
    Construction: take the truncated polynomial algebra k[x]/(x^3), written
    with generators x and y=x^2.  The only nonzero propagating channel is

      x(z)x(w) ~ epsilon * y(w)/(z-w).

    The products x*y, y*x, and y*y vanish.  Thus the cubic word x*x*x
    produces an intermediate y and then stops.  The cubic relation x^3=0
    is not generated by quadratic relations in the tropical propagation
    graph, and H^{-1}(B^trop_3) is nonzero.

    At epsilon=0 the algebra is abelian and the obstruction disappears.
    """
    if epsilon is None:
        epsilon = Rational(1)
...
def shadow_tower_agrees_with_algebraic(family: str, max_arity: int = 4) -> Dict[int, bool]:
    ...
    We compare against known values:
      ...
      beta-gamma: direct OPE lane kappa_trop = 0, C_3 = 0; the closed
                  chiral shadow has class C and is read from contact data
    ...
    if family == "heisenberg":
        ...
        results = {}
        kappa_trop = shadow.kappa_tropical()
        # beta-gamma: the simple poles beta_(0)gamma=1 and
        # gamma_(0)beta=-1 are bar/contact data. They are not closed
        # collision curvature. The direct tropical OPE lane is uncurved:
        # the two vacuum contact coefficients cancel, so kappa_trop=0.
        # The class-C chiral shadow is recovered from the contact/quartic
        # surface, not from a nonzero closed collision residue.
        results[2] = True
        if max_arity >= 3:
            results[3] = shadow.cubic_shadow_tropical() == 0
        return results
