...
PROOF SKETCH:
  ...
  suffice at any given precision → the shadow obstruction tower terminates at finite depth.
...
THE βγ RESOLUTION:
  ...
  in the shadow obstruction tower (terminates at arity 4 because μ_{βγ} = 0).
...
def betagamma_virasoro_rate_coincidence():
    ...
    - βγ: the growth is EXACTLY 1/η² → the shadow obstruction tower terminates at depth 4
    - Virasoro: the growth involves the Virasoro vacuum character → infinite depth

    The TERMINATION of βγ's shadow obstruction tower at depth 4 corresponds to the
    SIMPLICITY of the 1/η² function (it's a product, not a representation-
    theoretic object). The NON-TERMINATION of Virasoro's shadow obstruction tower
...
def depth_4_barrier_explanation():
    ...
    and equal to the Virasoro rate at c=2. The shadow obstruction tower terminates at
