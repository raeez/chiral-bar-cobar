...
ALGEBRAS COVERED:
    - Heisenberg H_k:  A! = curved Sym^ch(V*[1]) with curvature -k*omega
...
CRITICAL DISTINCTIONS (AP19, AP25, AP33):
    ...
    - H_k^! is the curved second-kind Sym branch, not H_{-k} (AP33):
    ...
    - Z_ch^der(A) is the derived-centre closed-sector, not A^i or A!.
...
def bar_koszul_object_firewall(algebra_name: str = "A",
    ...
    return {
        ...
        "derived_center": f"Z_ch^der({algebra_name}): closed-sector actor",
    }
...
def heisenberg_dual_dim(n: int) -> int:
    ...
    The Koszul dual of H_k at k != 0 is the curved second-kind
    Sym^ch(V*[1]) branch with curvature -k*omega.  Its vector-space
    shadow has dim = p(n-2) for n >= 2, 1 for n = 1.
...
def kappa_heisenberg_dual(k: Rational) -> Rational:
    """kappa(H_k^!) = -k for the curved second-kind Sym branch."""
    return -Rational(k)
...
def identify_heisenberg_generators(k: Rational,
    ...
    """Compute H^1(B(H_k))^v: generators of the curved Sym branch.

    H_k has one generator J of weight 1.  At k != 0 the Koszul dual is
    the curved second-kind Sym^ch(V*[1]) branch; its associated
    chiral-symmetric vector-space shadow is freely generated by J*.
    ...
    Verification: the associated chiral-symmetric vector-space shadow
    has full weight-space dimensions p(n), while dim H^1_n(B)=p(n-2)
    counts the BAR COHOMOLOGY, which for a free algebra equals the
    weight-space dimension of the generating space Lie^v (dual of the
    cobar).
    """
    pres = KoszulDualPresentation()
    pres.algebra_name = f"H_{k}"
    pres.dual_name = "curved Sym^ch(V*[1])"
...
def heisenberg_dual_ope(k: Rational) -> Dict[str, Any]:
    """OPE shadow of the curved Koszul dual H_k^!.

    At k != 0, H_k^! is the curved second-kind Sym^ch(V*[1])
    branch with curvature -k.  Its associated OPE shadow has a
    generator J* of weight 1 with:
        J*(z) J*(w) ~ (-k)/(z-w)^2

    This is not the uncurved polynomial centre of Sym^ch(V*) and not
    the algebra H_{-k}; it is the curved dual branch with the same
    scalar modular characteristic (AP33).
