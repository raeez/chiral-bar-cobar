r"""Spectral curve engine: extracting algebraic/spectral curves from shadow obstruction tower data.

The shadow obstruction tower generates a sequence of rational functions S_r(c) (shadow
...
The SPECTRAL CURVE of the shadow obstruction tower is the Riemann surface defined by
    y^2 = Q_L(t) = q_0 + q_1 t + q_2 t^2
where Q_L is the shadow metric.  The weighted generating function satisfies
    H(t) = t^2 sqrt(Q_L(t)),
so the spectral curve carries the full shadow obstruction tower as a function on it.
...
5. **Jacobian and theta functions** (Section 5): The Jacobian torus
   ...
   shadow obstruction tower coefficients as theta-function values.
...
7. **Connection to integrable systems** (Section 7): Spectral curve of
   the shadow obstruction tower = spectral curve of the dg-shifted Yangian r(z).
...
def affine_shadow_metric(level_sym=None):
    ...
    For the shadow obstruction tower: S_2 = kappa, S_3 = C_3, S_4 = 0 (class L).
...
def shadow_theta_identification(c_val: float, r_max: int = 10) -> dict:
    """Attempt to identify shadow obstruction tower coefficients with theta-function values.
...
def w3_spectral_surface(c_val: float) -> dict:
    """Spectral surface for the W_3 shadow obstruction tower.
...
def toda_spectral_curve(k_val: float = 1.0) -> dict:
    ...
    The shadow obstruction tower for affine sl_2 should recover this curve
...
def kdv_spectral_curve(c_val: float = 26.0) -> dict:
    ...
    For the shadow obstruction tower connection: the shadow metric Q_Vir(t) defines
...
def yangian_spectral_identification(family: str, c_val: float = None,
    ...
    """Verify the identification: spectral curve of shadow obstruction tower = spectral
...
def shadow_coefficients_from_curve(c_val: float, r_max: int = 20) -> dict:
    """Reconstruct shadow obstruction tower coefficients from the spectral curve data.
    ...
    y^2 = Q(t), we recover the shadow obstruction tower.
    """
    alpha_c = (180.0 * c_val + 872.0) / (5.0 * c_val + 22.0)
    q0 = c_val**2
    q1 = 12.0 * c_val
