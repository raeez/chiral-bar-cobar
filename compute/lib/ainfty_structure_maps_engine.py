...
def virasoro_weight_truncated_bar(c: Fraction, max_weight: int = 6,
    ...
    Weight-graded structure:
      Weight 2: B_1 = span{s^{-1}T} (1-dim)
      Weight 4: B_2 = span{s^{-1}T|s^{-1}T} + descendants at B_1 (weight 4)
      Weight 6: B_3 = span{s^{-1}T|s^{-1}T|s^{-1}T} + mixed + descendants

    For the PRIMARY SECTOR (all inputs = s^{-1}T), the bar differential
    ...
    # At weight w, the relevant space is generated by words in {s^{-1}T} of
    # total weight w (each s^{-1}T contributes weight 2).
