r"""Finite-window W(p) PBW test: PBW character vs vacuum character.
...
If PBW_char[h] == actual_char[h] for all h through weight 20:
    => no relation is detected through that finite window.
    => this is evidence only; it is not a global free-generation theorem.
...
RESULT (computed below):
    W(2): no relation detected through weight 20 in this finite window.
    W(3): no relation detected through weight 20 in this finite window.
    W(4): PBW > actual starting at weight 14 = 2*(2*4-1) in this model.
    W(5): PBW > actual starting at weight 18 = 2*(2*5-1) in this model.

GLOBAL STATUS:
    W(p) is C_2-cofinite and therefore not freely strongly generated by a
    finite positive-weight generator set. Chiral Koszulness remains
    conjectural/open; finite-window PBW agreement does not prove it.
...
def _fgst_vacuum_dims(p: int, max_weight: int) -> List[int]:
    ...
    with equality giving finite-window PBW agreement only).
...
def _compute_wp_dims_from_embedding(p: int, max_weight: int) -> List[int]:
    ...
    # - For p = 2: NONE detected through weight 20 in this finite window.
    # - For p = 3: NONE detected through weight 20 in this finite window.
...
def character_comparison(p: int, max_weight: int = 20) -> Dict:
    ...
    RESULT:
      - If PBW[h] == actual[h] for all h: no relation is detected through
        weight max_weight. This is finite-window evidence only.
    ...
    return {
        ...
        'global_freely_strongly_generated': False,
        'global_non_free_generation': True,
        'koszulness_conclusion': (
            'FINITE-WINDOW PBW AGREEMENT; GLOBAL KOSZULNESS CONJECTURAL/OPEN'
            if is_freely_generated
            else f'FINITE-WINDOW RELATION at weight {first_discrepancy}; '
                 'GLOBAL KOSZULNESS CONJECTURAL/OPEN'
        ),
    }
...
def koszulness_verdict(p: int, max_weight: int = 20) -> Dict:
    """Finite-window Koszulness status for W(p).

    Based on the character comparison through max_weight. The global
    Koszulness status remains conjectural/open.
    """
    comp = character_comparison(p, max_weight)

    if comp['is_freely_generated_through_max_weight']:
        # No relations found through this finite window.
        h_null_predicted = first_null_weight_predicted(p)
        verdict = 'FINITE_WINDOW_PBW_AGREEMENT'
        if h_null_predicted is None:
            evidence = f'PBW = actual through weight {max_weight}; no global relation bound supplied'
        else:
            evidence = (
                f'PBW = actual through weight {max_weight}; '
                f'predicted first finite-window relation weight is {h_null_predicted}'
            )
    else:
        h_disc = comp['first_discrepancy_weight']
        verdict = 'FINITE_WINDOW_RELATION_DETECTED'
    ...
    return {
        ...
        'koszulness': 'CONJECTURAL/OPEN; finite-window PBW data is not a proof',
    }
