...
\noindent\emph{Governing question.} Which representation-theoretic
families in Part~II are genuinely different, and which are different
faces of one lattice input mechanism?  This chapter fixes that common
input so later portraits can focus on what each family newly reveals.
...
This chapter operates at \textbf{S-level}: lattice VOA bar cohomology
dimensions, root system data, and character formulas.  These are
extracted from \textbf{M-level} models (lattice vertex algebra bar
complexes, cocycle constructions).  H-level structures (factorization
algebras on moduli of $G$-bundles) are referenced but not constructed.
\end{remark}

\begin{remark}[Status legend]\label{rem:lattice-status}
The lattice VOA $V_L$ is \emph{unconditional}
(Tier~T1, alongside free fields and Kac--Moody):
$V_L$ is quadratic for even unimodular $L$, so all results
of Theorems~A--D apply without the PBW degeneration
hypothesis.  Every
theorem in this chapter carries \ClaimStatusProvedHere{} or
\ClaimStatusProvedElsewhere; there are no conjectural claims.
\end{remark}
