The governing question of this section is: which deep $\mathcal{W}$-algebra
structures can be made completely explicit at chain level so higher-spin claims
in the portrait layer are anchored in verifiable computations.

\begin{remark}[Semantic levels; Convention~\ref{conv:hms-levels}]
\label{rem:w-deep-hms}
This section operates at \textbf{S-level}: explicit degree-$3$ bar
data for $\mathcal{W}_3$, Bershadsky--Polyakov bar structure, and
DS hierarchy tables.  These are extracted from \textbf{M-level}
models (bar complex differentials, jet bundle resolutions).
\end{remark}
...
\subsection{Explicit bar complex coalgebra for W-algebras}
...
\subsection{W-algebras at general levels and screening operators}
...
non-principal theorematic seeds; the global statement that Koszul
duality follows Barbasch--Vogan orbit duality for arbitrary nilpotent
data is still Conjecture~\ref{conj:w-orbit-duality}.  The purpose of the
section is to isolate the mechanism that a future theorem must
globalize, not to blur that frontier.
\end{remark}
...
\subsection{W-algebra bar complex dimension tables}
...
\subsection{$\mathcal{W}_{1+\infty}$ and the large-$N$ limit}
\label{sec:w-infty}
...
\subsection{$\mathcal{W}_3$ bar complex: degree-3 computation}
...
\subsection{$\mathcal{W}_3$ degree-3 cohomology}
...
\subsection{$\mathcal{W}_3$ bar complex at the dual level}
...
\subsection{W-algebra Koszul duality across the DS hierarchy}
