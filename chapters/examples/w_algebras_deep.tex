...
Conjecture~\ref{conj:winfty-factorization-package}: a separated
...
Conjecture~\textup{\ref{conj:winfty-factorization-package}}
...
\item \emph{Finite-stage factorization Koszul duality.}
  ...
  weight-$w$ sector finite-dimensional (finitely many compositions
  of~$w$ into parts from $\{2, \ldots, N\}$).  The factorization
  bar-cobar counit
  \[
...
\item \emph{DS--KD identification at each stage.}
  ...
  (Proposition~\ref{prop:ds-koszul-hierarchy}): in type~$A$ the
  BV dual of the principal nilpotent $(N)$ is the zero partition
  $(1^N)$, and DS at the trivial nilpotent is the identity.
  The factorization Verdier intertwining
  $\mathbb{D}_{\operatorname{Ran}}\,
  \barB^{\mathrm{fact}}(W_N)
  \simeq
  \barB^{\mathrm{fact}}(\widehat{\mathfrak{sl}}_{N,\,-k-2N})$
  holds at each finite stage.
...
\item \emph{Completed factorization target.}
  ...
  Mittag--Leffler, and the completed factorization bar-cobar
  counit is a quasi-isomorphism:
  \[
...
\begin{proof}
\emph{Part~(i): sectorwise convergence.}
The principal $\mathcal{W}$-algebra $W_N$ has strong generators
$W^{(s)}$ of conformal weight~$s$ for $s = 2, \ldots, N$.  The
conformal weight defines a non-negative grading on the bar complex:
a monomial $W^{(s_1)*} \otimes \cdots \otimes W^{(s_n)*}$ in bar
degree~$n$ has weight $s_1 + \cdots + s_n$.
At fixed weight~$w$, the bar complex
$\barB_w(W_N)$ is a \emph{finite} chain complex: the number of
compositions of~$w$ into~$n$ parts from $\{2, \ldots, N\}$ is
bounded by the partition function~$p(w)$, independent of~$n$
for $n > w/2$.

The hypotheses of
Theorem~\ref{thm:bar-cobar-isomorphism-main} (Theorem~A) are
satisfied for each~$W_N$ at generic level~$k$:
$\Einf$-Koszulness holds by
Theorem~\ref{thm:master-pbw}
(MC1 for principal $\mathcal{W}$-algebras).  The factorization
extension from $\operatorname{Ran}_1(X) \cong X$ to
$\operatorname{Ran}(X)$ follows from the same
K\"unneth--descent argument as
Theorem~\ref{thm:h-level-factorization-kd}(i)--(ii): on
disjoint opens
$U_1 \sqcup \cdots \sqcup U_k$, both source and target
decompose as tensor products by factorization; each factor is a
quasi-isomorphism by the diagonal case (Theorem~B); K\"unneth
preserves quasi-isomorphisms over a field; cosheaf descent
extends to overlapping configurations.  Sectorwise finiteness
ensures $\barB^{\mathrm{fact}}(W_N)$ is a genuine factorization
coalgebra on~$\operatorname{Ran}(X)$, not a formal pro-object.
...
spaces $C_n(X)$ in place of ordered configuration spaces
$C_n^{\mathrm{ord}}(X)$.  Parts~(i) and~(ii) of that
theorem use only the general Theorems~A/B and factorization
cosheaf structure, not the $\Eone$-specific braiding machinery.)

\emph{Part~(ii): BV orbit computation.}
By Proposition~\ref{prop:ds-koszul-hierarchy}, the Koszul dual
of $W_N^k = \mathrm{DS}_{f_{\mathrm{prin}}}(\widehat{\mathfrak{sl}}_{N,k})$
is
$\mathrm{DS}_{f_{\mathrm{prin}}^{\mathrm{BV}}}(
(\widehat{\mathfrak{sl}}_{N,k})^!)$.
The Koszul dual of the ambient algebra is
$(\widehat{\mathfrak{sl}}_{N,k})^!
...
$(N)^t = (1^N)$, which is the zero nilpotent.  Since DS at the
zero nilpotent is the identity functor,
$(W_N^k)^! \simeq \widehat{\mathfrak{sl}}_{N,-k-2N}$.
The factorization Verdier intertwining follows from part~(i)
composed with
...
generator).  Mittag--Leffler for the M-level tower is
...
The completed bar-cobar quasi-isomorphism follows from the same
Milnor exact sequence argument as
Corollary~\ref{cor:completed-bar-cobar-yangian}:
each finite-stage factorization QI (part~(i)) assembles into the
completed QI via the vanishing of $\varprojlim^1$.
Continuity of the completed differential is
Proposition~\ref{prop:inverse-limit-differential-continuity}.

\emph{Part~(iv): residue identities.}
The chain-level DS--KD intertwining
(Theorem~\ref{thm:ds-koszul-intertwine},
equation~\eqref{eq:ds-bar-commute}) gives, for each~$N$:
\[
\barB_W(W_N) \;\simeq\; H^0_{\mathrm{DS}}(\barB(
\widehat{\mathfrak{sl}}_{N,k})).
\]
The ``residue-side'' coefficients $\mathsf{C}^{\mathrm{res}}$
are entries of $\barB_W(W_N)$ (extracted by residues on
$\overline{C}_n(X)$ from the $\mathcal{W}$-algebra OPE),
and the ``DS-side'' coefficients $\mathsf{C}^{\mathrm{DS}}$
are entries of $H^0_{\mathrm{DS}}(\barB(
\widehat{\mathfrak{sl}}_{N,k}))$ (the DS projection of
the KM bar complex).
The chain-level isomorphism equates them.
The factorization descent of part~(i) extends this
identification from $\operatorname{Ran}_1$ to
$\operatorname{Ran}(X)$.
...
\Omega^{\mathrm{fact}}(
\widehat{\barB}^{\mathrm{fact}}(\mathcal{W}_\infty))$.
Item~(1) of the conjecture: the conformal weight filtration on
...
Theorem~\ref{thm:winfty-factorization-kd} is the exact $\Einf$
counterpart of the $\Eone$ Yangian construction
(Corollary~\ref{cor:dk23-all-types},
Corollary~\ref{cor:completed-bar-cobar-yangian}), with
\emph{DS reduction playing the role of the RTT relation}:
\begin{center}
\renewcommand{\arraystretch}{1.2}
\begin{tabular}{lll}
& \textbf{Yangian} ($\Eone$) &
...
\emph{Sectorwise grading} &
  root lattice $Q(\fg)$ &
  conformal weight $\mathbb{Z}_{\ge 0}$ \\
...
\smallskip\noindent
The structural parallel extends to a single organizing
principle: \emph{the construction procedure that builds the
algebra is the same structure that computes its Koszul dual.}
For the Yangian, the RTT relation simultaneously defines the
algebra and supplies the quadratic duality data.  For the
$\mathcal{W}$-algebra, the BRST complex of DS reduction
simultaneously constructs $W_N$ from~$\widehat{\fg}_k$
and computes $\barB(W_N)$ via
Theorem~\ref{thm:ds-koszul-intertwine}.

The deepest consequence: the residue identities
$\mathsf{C}^{\mathrm{res}} = \mathsf{C}^{\mathrm{DS}}$ that
were treated as the live MC4 frontier are not independent
conjectures but instances of the proved DS--KD intertwining
theorem.  The ``extraction problem'' on the
$\mathcal{W}_\infty$ side of MC4 is already solved by the
chain-level content of Theorem~\ref{thm:ds-koszul-intertwine}.
\end{remark}
