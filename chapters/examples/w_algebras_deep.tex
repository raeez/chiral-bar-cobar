...
The governing question of this chapter is which geometric and representation-theoretic mechanisms produce principal $\mathcal{W}$-algebras beyond the summary portrait, and how those mechanisms interface with the modular bar-cobar story.
\end{remark}
...
\begin{theorem}[$\mathcal{W}$-algebras as chiral de Rham \cite{Arakawa}; \ClaimStatusProvedElsewhere]\label{thm:w-cdr}
...
\begin{theorem}[$\mathcal{W}$-algebra bar coalgebra; \ClaimStatusProvedHere]\label{thm:w-bar-coalg}
...
$\mathcal{W}^k(\mathfrak{sl}_3, f_{\min})$.  Its generators are an $\widehat{\mathfrak{sl}}_2$ subalgebra at level $k' = k + 1/2$ (from the $\mathfrak{sl}_2 \subset \mathfrak{sl}_3$ commuting with $f_{\min}$) and two fermionic generators $G^+, G^-$ of conformal weight $3/2$ (from the remaining root spaces).
...
\begin{theorem}[$\mathcal{W}$-algebras and Toda systems \cite{FF}; \ClaimStatusProvedElsewhere]\label{thm:w-toda}
...
$\mathcal{W}_\infty$ via DS--sectorwise convergence;
...
\begin{proposition}[$\mathcal{W}_3$ degree-3 vacuum cancellation; \ClaimStatusProvedHere]
