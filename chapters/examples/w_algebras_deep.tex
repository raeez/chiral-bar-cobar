...
At weight $h = 7$, the degree-3 space is $\bC^6$ (three generator
orderings $\times$ two form directions).  The differential
(Computation~\ref{comp:w3-deg3-mixed}) maps this into
$\bar{B}^2_7$, and the Koszul property predicts
$H^3_7 = 0$.

\emph{Dimension table through weight $h = 10$.}
\begin{center}
\renewcommand{\arraystretch}{1.2}
\begin{tabular}{c|cccccc}
$h$ & 6 & 7 & 8 & 9 & 10 \\
\hline
$\dim \bar{B}^3_h$ & 2 & 6 & 16 & 36 & 78 \\
$\dim H^3_h$ & 0 & 0 & 0 & 0 & 0
\end{tabular}
\end{center}
The vanishing $H^3_h = 0$ for $h \leq 10$ is consistent with the
...
\[
c = 0 \iff k = -2 + \sqrt{4/24 \cdot 3} = -2 \pm \frac{1}{\sqrt{6}}
\quad (\text{irrational levels})
\]
The zero-curvature locus consists of two irrational levels, in contrast
...
Zero-curvature level & $c = 0$ ($k$ irrational) & $k = -1/2$
\end{tabular}
\end{center}
...
\begin{proof}
The DS--KD intertwining theorem
(Theorem~\ref{thm:ds-koszul-intertwine}) establishes that Koszul
duality commutes with DS reduction.  For the ambient algebra,
$(\widehat{\fg}_k)^! \simeq \widehat{\fg}_{-k-2h^\vee}$
(Corollary~\ref{cor:level-shifting-part1}).  Applying DS reduction
with nilpotent $f_i$ on both sides:
\[
\mathrm{DS}_{f_i}\bigl((\widehat{\fg}_k)^!\bigr)
\simeq \mathrm{DS}_{f_i}\bigl(\widehat{\fg}_{-k-2h^\vee}\bigr)
= \mathcal{W}^{-k-2h^\vee}(\fg, f_i)
\]
The left-hand side equals
$\bigl(\mathrm{DS}_{f_i^{\mathrm{BV}}}
(\widehat{\fg}_k)\bigr)^!
= \bigl(\mathcal{W}^k(\fg, f_i^{\mathrm{BV}})\bigr)^!$
by the compatibility of DS reduction with Koszul duality,
where the nilpotent orbit transforms under the Barbasch--Vogan
duality $f_i \mapsto f_i^{\mathrm{BV}}$ (which exchanges the orbit
structure of $\fg$ and $\fg^\vee$).

For simply-laced $\fg$ (types $A$, $D$, $E$), $\fg = \fg^\vee$
and $f_i^{\mathrm{BV}} = f_i$ (the BV duality is the identity on
orbits).  For non-simply-laced $\fg$, the BV duality exchanges
...
\emph{Koszul duality.}
Since $\mathfrak{sl}_4$ is simply-laced, the BV duality is the
identity: each orbit is self-BV-dual.  The level-shifting duality
$k \mapsto -k-2h^\vee = -k-8$ maps each W-algebra to itself at
the dual level:
\[
\mathcal{W}^k(\mathfrak{sl}_4, f_\lambda)^!
\simeq \mathcal{W}^{-k-8}(\mathfrak{sl}_4, f_\lambda)
\]
