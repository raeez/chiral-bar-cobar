...
\textbf{Tag: LARGELY SPECULATIVE.}  The bar complex
...
The shadow tower gives $F_g = 3 \cdot \lambda_g^{\mathrm{FP}}$ (UNIFORM-WEIGHT)
at each genus.
\end{remark}
...
effective-level prefix vanishes at $k_{\mathrm{eff}}=0$ per AP126),
...
$F_g = \kappa(\cA) \cdot \lambda_g^{\mathrm{FP}}$ (UNIFORM-WEIGHT) with
...
class $\mathbf{G}$, and
\emph{identical shadow data through arity~$4$}.  The shadow
...
$\kappa/c = k$ independent of $N$.  This is consistent with
AP24: Heisenberg is class $\mathbf{G}$ with
...
symmetrization cannot generate higher-arity obstructions.
At $k = 0$ the seed degenerates and $\kappa(\cH_0) = 0$ on
every symmetric power, consistent with AP126.
\end{remark}
...
coalgebra (augmentation ideal throughout, per AP132), and
...
is the classical $r$-matrix of $A$ at arity~$2$, then
\[
  r_{\operatorname{Sym}^N(A)}(z)
    \;=\; N \cdot r_A(z)
\]
(at arity~$2$, on the symmetric diagonal), and consequently
\[
...
distribute the $r$ bar-arity inputs across $N$ copies of
...
$r_{\operatorname{Sym}^2(\cH_0)}$, consistent with AP126.
The bar-complex arity-$2$ class
...
identification is compatible with AP33: bar and Koszul
...
\item \emph{Tautological insulation.}  The shadow amplitude
  $F_g = \kappa(\cA)\,\lambda_g^{\mathrm{FP}}$ (UNIFORM-WEIGHT) lives in the
...
$F_g = \kappa(\cA) \cdot \lambda_g^{\mathrm{FP}}$ (UNIFORM-WEIGHT) receives no
...
amplitude is $F_g = \kappa(\cA) \cdot \lambda_g^{\mathrm{FP}}$ (UNIFORM-WEIGHT)
...
F_g(K3 \times E) \;=\; \kappa(K3 \times E) \cdot \lambda_g^{\mathrm{FP}} \quad\text{(UNIFORM-WEIGHT)}
...
$F_g = \kappa(\cA) \cdot \lambda_g^{\mathrm{FP}}$, UNIFORM-WEIGHT) and what it
