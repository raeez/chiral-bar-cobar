...
\begin{remark}[AP48 in action: $\kappa(K3) = 2 \neq c/2 = 3$]
\label{rem:ap48-k3}
\index{AP48!K3 example}
The K3 sigma model provides a sharp illustration of AP48: the
...
\item The bar differential $d\colon B^2 \to B^1$ is a
  ...
  the logarithmic kernel $d\log(z-w)$ \textup{(}AP19:
  $r$-matrix pole orders are one less than OPE pole orders;
  AP41: the bar differential extracts ALL modes, not just
  simple-pole\textup{)}.
...
$\phi_{0,1}(\tau,0) = 12$ \textup{(}AP38\textup{)}.  At $z = 0$:
...
(AP48: $\kappa = \mathrm{rank}$ for lattice VOAs).  The
...
\item $\kappa(\cA_{K3}^!) = -2$ \textup{(}by complementarity:
  $\kappa + \kappa' = 0$ for CY sigma models, AP24\textup{)}.
\item $c(\cA_{K3}^!) = -6$
...
$h$ & $0$ & $1$ & $2$ & $3$ & $4$ & $5$ & $6$ \\
\hline
$n^0_h$ & $1$ & $24$ & $324$ & $3200$ & $25650$ & $176256$ & $1073720$ \\
$n^1_h$ & $0$ & $-2$ & $-54$ & $-800$ & $-8550$ & $-73440$ & $-536860$ \\
$n^2_h$ & $0$ & $0$ & $3$ & $104$ & $1701$ & $19656$ & $176358$
\end{tabular}
\end{center}
Signed convention: $n^g_h = (-1)^g |n^g_h|$.  The genus-$0$
values are the Yau--Zaslow numbers; $n^0_1 = 24 = \chi(K3)$.
\end{proposition}
...
$\kappa(A_E) = 24$ \textup{(}AP48: rank of the free-boson lattice\textup{)},
$\kappa(A_E^!) = -24$ \textup{(}AP24: $\kappa + \kappa' = 0$ for
free fields\textup{)}.
\end{construction}
...
$A = A_E$ ($c = 24$, $\kappa = 24$),
$A^! = A_E^!$ ($\kappa = -24$),
$C = Z^{\mathrm{der}}_{\mathrm{ch}}(A)$ (universal bulk, AP25),
...
\textup{(}Construction~\textup{\ref{constr:twisted-holo-k3xe})}
with Mukai-pairing level matrix, and
...
\begin{proof}
$\kappa(A_E) = 24$ (rank of the free-boson lattice, AP48).
...
directions in the superalgebra.
\end{proposition}
...
\textup{(}Conjecture~\textup{\ref{conj:platonic-adjunction})},
...
\item \emph{Two MC elements.}  The shadow obstruction tower
  ...
  $\phi_{0,1}$ multiplicities (AP42: quantitative
...
contributions must be included \textup{(}AP42\textup{)}.
\end{conjecture}
...
structure.  The topological string partition function
...
(AP38).  In the DVV convention, $\Phi_{10}$ may differ
...
\textup{(}AP48: this is NOT $c/2$; the central charge is
...
literature \textup{(}AP20, AP48\textup{)}:
\begin{center}
\small
\renewcommand{\arraystretch}{1.2}
\begin{tabular}{lll}
