...
The exact rows follow from the displayed rational identities
\(c(k)+c(-k-6)=100\), the coefficients \(32/D(c)\) and
\(16/D(c)\), the normalization \(N_\Lambda=cD(c)/10\), the action
\(\Lambda_0\lvert h,w\rangle=(h^2+h/5)\lvert h,w\rangle\), and the
formula for \(\det M_1\) in
Equation~\eqref{eq:w3hol-level-one-null-curve}.  Every other row is
conditional or open under the named hypothesis packages displayed in
the table.
