...
\begin{remark}[Shadow Postnikov tower for free fields]\label{rem:free-fields-master-mc}
Free fields ($bc$/$\beta\gamma$) are the simplest non-Heisenberg atoms.
Their shadow Postnikov towers in $\mathfrak{g}^{\mathrm{mod}}_\cA$
(Definition~\ref{def:modular-convolution-dg-lie}) test the nonlinear
shadow calculus at weight~$4$: the quartic contact invariant first
appears here, while the scalar level~$\kappa$ vanishes for
$\beta\gamma$ and is nonzero for~$bc$
...
The four free-field atoms --- the free fermion, the $\beta\gamma$ system, the $bc$ ghost system, and the single-generator fermion--boson pair --- are defined below with their OPE structure and relations.
