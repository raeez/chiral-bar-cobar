...
$B(\cA)=T^c(s^{-1}\bar\cA)$, the quadratic coalgebra
$\cA^{\mathrm{i}}=C_X(s^{-1}V,s^{-2}R)$ of a chosen presentation, the
Verdier algebra $K_X(\cA)=\mathbb D_{\Ran}B_X(\cA)$, the chosen
partner~$\cA^!$, the genus tower, and
the shadow invariants are distinct outputs of this datum. The
cobar equivalence $\Omega B(\cA)\simeq \cA$ expresses reconstruction;
$\mathbb D(q_\cA)\colon K_X(\cA)\to\mathbb D_{\Ran}(\cA^{\mathrm i})$
and $\nu_\cA\colon K_X(\cA)\to\cA^!$ give the quadratic and pair
comparisons. For free-field archetypes, the
...
The numerical invariants used below have distinct type signatures.
...
$\operatorname{Supp}\ChirHoch^\bullet\subseteq S_{\mathcal F}$ under
$H_H(\mathcal F;S_{\mathcal F})$ \\
Heisenberg $\cH_k$ & $2$ & $1$ & $2$ &
$k$ & $1$ &
$\mathrm{Sym}^{\mathrm{ch}}(V^*)$, $m_0=-k\omega_{\mathrm{cen}}$ &
$2+t$ when $\chi^{\mathrm{bd}}_{B_{\mathfrak h}}$ is a quasi-isomorphism \\
$\beta\gamma_\lambda$ & $1$ & $0$ & $4$ &
$6\lambda^2-6\lambda+1$ & $2(6\lambda^2-6\lambda+1)$ &
$bc_\lambda$ &
$\operatorname{Supp}\ChirHoch^\bullet\subseteq S_{\beta\gamma}$ under
$H_H(\beta\gamma_\lambda;S_{\beta\gamma})$ \\
$bc_\lambda$ & $1$ & $0$ & $4$ &
$-(6\lambda^2-6\lambda+1)$ & $-2(6\lambda^2-6\lambda+1)$ &
$\beta\gamma_\lambda$ &
$\operatorname{Supp}\ChirHoch^\bullet\subseteq S_{bc}$ under
$H_H(bc_\lambda;S_{bc})$ \\
Symplectic boson $\beta\gamma_{1/2}$ & $1$ & $0$ & $4$ &
$-1/2$ & $-1$ &
$bc_{1/2}$ &
$\operatorname{Supp}\ChirHoch^\bullet\subseteq S_{\mathrm{sb}}$ under
$H_H(\beta\gamma_{1/2};S_{\mathrm{sb}})$ \\
Symplectic fermion $\mathcal{SF}=bc_1$ & $1$ & $0$ & $4$ &
$-1$ & $-2$ &
$\beta\gamma_1$ &
$\operatorname{Supp}\ChirHoch^\bullet\subseteq S_{\mathrm{sf}}$ under
$H_H(\mathcal{SF};S_{\mathrm{sf}})$ \\
Lattice $V_\Lambda$ & $2$ & $1$ & $2$ &
$\operatorname{rank}\Lambda$ & $\operatorname{rank}\Lambda$ &
$V_{\Lambda^\vee}$ with opposite scalar curvature &
$\operatorname{Supp}\ChirHoch^\bullet\subseteq S_\Lambda$ under
$H_H(V_\Lambda;S_\Lambda)$ \\
\bottomrule
\end{tabular}
\end{center}
...
uniform conformal weight $h = 1/2$, and a single generator. Under
$H_D^K$, Theorem~\ref{thm:modular-characteristic} gives
\[
 \mathfrak O_g^K(\mathcal F)
 =\kappa(\mathcal F)\lambda_{-1}(\mathbb E_g),
 \qquad
 \operatorname{obs}^{\mathrm{Hdg}}_g(\mathcal F)
 =(-1)^g\kappa(\mathcal F)\lambda_g.
\]
The genus-$g$ graph free energy is
...
\item\label{item:fermion-genus-complementarity}
 Under~$H_D^K$, the genus-$g$ Hodge characters cancel:
 $\operatorname{ch}_g(\mathfrak O_g^K(\mathcal F))
 +\operatorname{ch}_g(\mathfrak O_g^K(\mathcal F^!))=0$,
 equivalently $(-1)^g(1/4-1/4)\lambda_g=0$.
\end{enumerate}
\end{proposition}
...
\beta(z)\gamma(w)\sim (z-w)^{-1}.
\]
In the manuscript's pole-valued collision convention the \(d\log\)-bar
kernel absorbs this simple pole, so
...
\item Koszul-dual curvature:
 ...
 $\mathcal H_k^!=(\mathrm{Sym}^{\mathrm{ch}}(V^*[1]),m_0^!)$.
...
then the Verdier algebra
\[
K_X(\mathcal B^\bullet):=
\mathbb D_{\mathrm{Ran}}\bar B_X(\mathcal B^\bullet)
\]
admits a pair equivalence
$\nu_{\mathcal B}\colon K_X(\mathcal B^\bullet)\to
(H^\bullet B(\mathcal B^\bullet))^\vee$ and carries a derived
free-field model whose degree-shifted symplectic
...
the Koszul shift.
\end{enumerate}
The $A_\infty$ structure on the dual side belongs to the algebra
$K_X(\mathcal B^\bullet)=
\mathbb D_{\mathrm{Ran}}(\bar B(\mathcal B^\bullet))$. The separate
equivalence
$\Omega(\bar B(\mathcal B^\bullet))\simeq\mathcal B^\bullet$
is bar--cobar reconstruction of~$\mathcal B^\bullet$.
\end{remark}
...
$\mathcal{H}_k^!=(\mathrm{Sym}(V^*[1]),m_0=-k\omega)$ in the
curved second-kind ambient.
...
Verdier/continuous linear dual
$\mathcal{H}_k^! \simeq
...
D(\mathrm{Mod}^{\Eone,\,\mathrm{compl}}_{\mathcal{H}_k})
\;\simeq\;
D(\mathrm{Mod}^{\Eone,\,\mathrm{compl}}_{\mathcal H_k^!}).
\end{equation}
Under this equivalence, Ext groups are exchanged:
$\mathrm{Ext}^i_{\mathcal{H}_k}(L, M) \cong
\mathrm{Ext}^i_{\mathcal H_k^!}(\mathrm{Ind}(M), \mathrm{Ind}(L))$.

Since the associated graded of $\mathcal H_k^!$ is commutative, its
complete module theory is considerably more tractable than Heisenberg
Fock module theory. This equivalence ``linearizes'' Heisenberg
representation theory via curved Koszul duality.
\end{corollary}
...
curved second-kind dual algebra $\mathcal H_k^!$.
\end{remark}
...
$\mathcal{H}_k^! \simeq
(\mathrm{Sym}^{\mathrm{ch}}(V^*[1]),m_0=-k\omega)$ by
...
\begin{computation}[Bar complex of the associated-graded symmetric shadow
\texorpdfstring{$\mathrm{Sym}^{\mathrm{ch}}(V^*)$}{Symch(V^*)}]
\label{comp:bar-sym-ch}
\index{commutative chiral algebra!bar complex}
We compute $\bar{B}^{\mathrm{ch}}(\mathrm{Sym}^{\mathrm{ch}}(V^*))$
explicitly for the uncurved associated-graded shadow of the
Heisenberg dual. This verifies the symmetric shadow of the Koszul
duality inverse; for $k\ne0$ the object-level dual is the curved
second-kind branch
$(\mathrm{Sym}^{\mathrm{ch}}(V^*[1]),m_0=-k\omega)$:
...
structure over the curved second-kind dual $\mathcal{H}_k^!$.
...
$\kappa(\mathcal{H}_k^{\oplus d})=dk$. Under~$H_D^K$ the genus-$g$
Hodge character is
$\operatorname{ch}_g(\mathfrak O_g^K)=
(-1)^g\kappa(\mathcal{H}_k^{\oplus d})\lambda_g$, with rank-one
genus-$1$ graph value
...
\hline
\textbf{Algebra $\mathcal{A}$} & \textbf{Verdier/Koszul branch} & \textbf{Type} & \textbf{Context} & \textbf{Status} \\
\hline
...
$\widehat{\mathfrak{g}}_k$ & $\mathrm{CE}^{\mathrm{ch}}(\widehat{\mathfrak{g}}_{-k-2h^\vee})$ on the affine non-critical lane & Filt./Curved & Feigin--Frenkel & \ClaimStatusProvedHere\textsuperscript{a} \\
Virasoro$_c$ & $K_X(A)$ with conditional pair map to $\mathrm{Vir}_{26-c}$ & Curved/completed & $c \mapsto 26-c$ & \ClaimStatusConditional\textsuperscript{c} \\
$\mathcal{W}_N$ & $K_X(A)$ with conditional DS pair map & Filt./Curved & Higher spin & \ClaimStatusConditional\textsuperscript{d} \\
...
\noindent\textsuperscript{d} The strict same-family
$\mathcal W_N$ entry is available only after the DS--bar and
DS--Verdier hypotheses of
Proposition~\ref{prop:ds-transferred-koszul-dual}; the characteristic
statement is Theorem~\ref{thm:w-algebra-koszul-main}. The naive dual
on generators $L^*, W^*$
...
\item Form the quadratic coalgebra
$\mathcal{A}^{\mathrm{i}}=C_X(s^{-1}V,s^{-2}R)$ and the canonical map
$q_{\mathcal A}\colon\mathcal A^{\mathrm i}\to B_X(\mathcal A)$.
On the quadratic Koszul locus, $q_{\mathcal A}$ is a quasi-isomorphism,
so $H^\bullet(q_{\mathcal A})$ computes bar cohomology from this model.
...
\item Check Koszulity: the condition
$\mathrm{Tor}_{\mathcal{A}}^{i,j}(\mathbb{C}, \mathbb{C})=0$ for
$i\neq j$ makes~$q_{\mathcal A}$ a quasi-isomorphism and yields strict
quadratic duality. A complete curved duality instead carries a
complete filtered category, a central curvature element~$m_0$, and
the Positselski finiteness hypotheses.
\end{enumerate}
The cobar counit
...
The Koszul dual of $\mathcal{H}_k$ was computed in
Theorem~\ref{thm:heisenberg-koszul-dual-early}; see
Chapter~\ref{ch:heisenberg-frame}\index{Heisenberg algebra|textbf}\index{Fock space}
for conventions. For $k\neq0$ it is the curved second-kind branch
\[
\mathcal{H}_k^! \simeq
(\mathrm{Sym}^{\mathrm{ch}}(V^*[1]),m_0=-k\omega),
\]
whose uncurved $\mathrm{Sym}^{\mathrm{ch}}(V^*)$ is only the
associated-graded or $k=0$ shadow. The bar complex
$\bar{B}^{\mathrm{ch}}(\mathcal{H}_k) \simeq
\mathrm{coLie}^{\mathrm{ch}}(V^*)$ is cocommutative because the
double-pole OPE produces symmetric coproducts.
...
\mathcal H_k^!
\simeq(\mathrm{Sym}^{\mathrm{ch}}(V^*[1]),m_0=-k\omega)
\not\simeq \mathcal H_k .
\]

The algebras have different structure:
\begin{center}
\begin{tabular}{c|c}
\textbf{Heisenberg } $\mathcal{H}_k$ & \textbf{Curved second-kind symmetric branch} \\ \hline
...
\begin{proof}
By Theorem~\ref{thm:heisenberg-bar}, the bar complex
$\bar{B}^{\mathrm{ch}}(\mathcal{H}_k)$ has cocommutative coproduct:
the Heisenberg OPE $\alpha(z)\alpha(w) \sim k(z-w)^{-2}$ produces
only the double-pole term, whose residue gives a symmetric
(primitive) coproduct
$\Delta(\alpha) = \alpha \otimes 1 + 1 \otimes \alpha$.
Cocommutative coalgebras are Koszul dual to commutative algebras
($\mathrm{Com}^! = \mathrm{Lie}$, see \cite{LV12},
Theorem~7.6.5), but the nonzero double-pole coefficient makes the
dual curved: for $k\neq0$,
\[
\mathcal H_k^!
\simeq(\mathrm{Sym}^{\mathrm{ch}}(V^*[1]),m_0=-k\omega).
\]
This curved second-kind branch has commutative associated graded,
while $\mathcal{H}_k$ has a nonzero singular OPE, so
...
\begin{enumerate}
\item \emph{Koszul duality} (algebra $\to$ dual coalgebra $\to$ curved dual algebra).
 \[
 \mathcal{H}_k \xrightarrow{\;\bar{B}\;}
 \bar{B}^{\mathrm{ch}}(\mathcal{H}_k)
 \xrightarrow{\;\mathbb D_{\Ran}\;}
 K_X(\mathcal H_k)
 \xrightarrow{\;\nu_k\;}
 (\mathrm{Sym}^{\mathrm{ch}}(V^*[1]),m_0=-k\omega)
 \]
 ...
 second arrow is Verdier/continuous linear duality on that coalgebra,
 producing the curved second-kind Koszul dual \emph{algebra}
 $(\mathrm{Sym}^{\mathrm{ch}}(V^*[1]),m_0=-k\omega)$.
 Here
 $K_X(\mathcal H_k)=
 \mathbb{D}_{\operatorname{Ran}}\bar{B}^{\mathrm{ch}}(\mathcal{H}_k)$
 is the homotopy Koszul-dual factorization algebra, and~$\nu_k$ is the
 Heisenberg pair equivalence.  The separate cobar construction
 $\Omega(\bar{B}(\mathcal{H}_k))\simeq\mathcal{H}_k$ reconstructs the
 original algebra (Theorem~\ref{thm:bar-cobar-inversion-qi}).
...
\simeq(\mathrm{Sym}^{\mathrm{ch}}(V^*[1]),m_0=-k\omega),
\]
with uncurved symmetric algebra only after passing to the
associated-graded or \(k=0\) shadow.
\end{theorem}
...
$\mathcal{H}_k^!=(\mathrm{Sym}^{\mathrm{ch}}(V^*[1]),m_0=-k\omega)$
...
$\cH_k^!=(\mathrm{Sym}^{\mathrm{ch}}(V^*[1]),m_0=-k\omega)$, not to the
...
coalgebra of $\cA$. Under the Verdier package of
Theorem~\ref{thm:bar-cobar-isomorphism-main}, its dual is already the
homotopy Koszul-dual factorization algebra
\[
K_X(\cA):=\mathbb{D}_{\operatorname{Ran}}\bar{B}(\cA).
\]
A chosen Koszul partner carries a further algebra map
$\nu_{\cA}\colon K_X(\cA)\to\cA^!$, which is an equivalence on the
finite-type Koszul-pair locus.  The cobar counit
$\Omega\bar{B}(\cA)\simeq\cA$ gives bar--cobar reconstruction.
...
$\cH_k^!=(\mathrm{Sym}^{\mathrm{ch}}(V^*[1]),m_0=-k\omega)$; the
...
Dual group $\hat{G}$
 & Koszul dual $\cA^!$
 & $\cH_k^!=(\mathrm{Sym}^{\mathrm{ch}}(V^*[1]),m_0=-k\omega)$ \\[2pt]
...
Fourier inversion
 & $K_X(\cA):=\mathbb{D}_{\mathrm{Ran}}\bar{B}(\cA)$;
...
\item \textup{(This monograph, $n = 2$)}\quad
 ...
 specializes to the Verdier algebra
 $K_X(\cA)=\mathbb{D}_{\mathrm{Ran}}\bar{B}(\cA)$
...
This is \emph{not} the same as the curved second-kind Koszul duality
$\mathcal{H}_k^! =
(\mathrm{Sym}^{\mathrm{ch}}(V^*[1]),m_0=-k\omega)$ established above;
it is an additional duality structure that exists in the
curved/filtered category.
\end{theorem}
...
For the full Koszul duality table, see \S\ref{sec:kd-computations-free-fields}.
...
\textbf{$r$-matrix} & pole-valued \(r_{\mathrm{coll}}(z)=0\)
after \(d\log\)-absorption; raw OPE contraction nonzero \\
...
\begin{theorem}[Free fermion genus-1 formal coefficient;
\ClaimStatusConditional]
\label{thm:fermion-F1-shadow}
The genus-$1$ formal scalar coefficient of the free fermion is:
...
\end{equation}
Under~$H_D^K$ the Hodge character is
$\operatorname{ch}_g(\mathfrak O_g^K(\cF))
=(-1)^g\tfrac14\lambda_g$, independent
...
Therefore $\operatorname{ch}_g(\mathfrak O_g^K(\cF))
=(-1)^g\kappa(\cF)\lambda_g=(-1)^g\frac14\lambda_g$, exactly as for the Heisenberg
...
\(\operatorname{ch}_g(\mathfrak O_g^K(\cF))
=(-1)^g\kappa(\cF)\lambda_g=(-1)^g\frac14\lambda_g\) are
...
(Definition~\ref{def:modular-characteristic-package}) that enters
the perfect object $\mathfrak O_g^K(\cF)=
\kappa(\cF)\lambda_{-1}(\mathbb E_g)$ under~$H_D^K$.
\end{remark}
...
Verdier-dualizes the resulting coalgebra, producing the algebra
\[
K_X(\cA):=\mathbb{D}_{\mathrm{Ran}}B(\cA).
\]
On a chosen finite-type Koszul pair, the additional comparison
$\nu_{\cA}\colon K_X(\cA)\xrightarrow{\sim}\cA^!$ realizes the strict
dual \textup{(}Theorem~\textup{\ref{thm:bar-cobar-isomorphism-main})}.
\end{remark}
...
\begin{proposition}[Virasoro standard-family computation ledger;
\ClaimStatusConditional]
\label{prop:virasoro-standard-family-ledger}
\index{Virasoro algebra!standard-family ledger}
\index{central charge!Virasoro exceptional values}
Let \(T(z)=\sum_n L_n z^{-n-2}\) be the stress tensor of
\(\mathrm{Vir}_c\), normalized by
\begin{equation}\label{eq:virasoro-ledger-ope}
T(z)T(w)\sim
\frac{c/2}{(z-w)^4}
+\frac{2T(w)}{(z-w)^2}
...
Equivalently,
\[
T_{(3)}T=\frac c2\,\mathbf1,\qquad
T_{(1)}T=2T,\qquad
T_{(0)}T=\partial T .
\]
The modular characteristic is
\begin{equation}\label{eq:virasoro-ledger-kappa}
\kappa(\mathrm{Vir}_c)=\frac c2 .
\end{equation}

In the ordered bar presentation, the degree-two simple-pole
differential is
\begin{equation}\label{eq:virasoro-ledger-bar-d2}
d_{\Barch}[\susp^{-1}T\,|\,\susp^{-1}T]
=\susp^{-1}(\partial T),
\end{equation}
while the quartic and double poles enter the collision-residue
normalization after one \(d\log\) pole shift:
\begin{equation}\label{eq:virasoro-ledger-r}
r_{\mathrm{Vir}_c}^{\mathrm{coll}}(z)
=\frac{c/2}{z^3}+\frac{2T}{z}
\end{equation}
\textup{(}Proposition~\ref{prop:vir-collision-residue}\textup{)}.
The \(2T\)-coefficient is the universal gravitational cubic shadow:
\[
S_3(\mathrm{Vir}_c)=2.
\]
On the non-singular surface \(c(5c+22)\ne0\), the first quartic
contact coefficient is
\begin{equation}\label{eq:virasoro-ledger-S4}
S_4(\mathrm{Vir}_c)
=\frac{10}{c(5c+22)}
\neq0 ,
\end{equation}
computed from the quasi-primary
\(\Lambda_{\mathrm{Vir}}={:}TT{:}-\frac{3}{10}\partial^2T\) and
\(\langle\Lambda_{\mathrm{Vir}}|\Lambda_{\mathrm{Vir}}\rangle
=c(5c+22)/10\)
\textup{(}Theorem~\ref{thm:w-virasoro-quartic-explicit} and
Proposition~\ref{prop:virasoro-shadow-canonical}\textup{)}.  Thus the
generic Virasoro tower is class~\(\mathsf M\), not Gaussian.

The exceptional central charges have different statuses:
\begin{enumerate}[label=\textup{(\roman*)}]
\item \(c=0\): \(\kappa=0\), the uncurved quadratic OPE surface of
Theorem~\ref{thm:virasoro-self-duality}; the generic
quartic-normalization formula \eqref{eq:virasoro-ledger-S4} is
singular and must not be specialized blindly.
\item \(c=-22/5\): the Lee--Yang Kac determinant at level~\(4\)
vanishes; \(\Lambda_{\mathrm{Vir}}\) becomes null and the
generic quartic coefficient has a pole
\textup{(}Theorem~\ref{thm:w-virasoro-quartic-explicit}\textup{)}.
\item \(c=26\): the bosonic-string matter value admits the proved
BRST/equivariant descent of
Theorem~\ref{thm:virasoro-moduli}; it is not a same-family
Verdier self-dual point, since its partner is \(c=0\).
\item \(c=13\): this is the fixed point of the same-family
Verdier/Koszul involution \(c\mapsto26-c\).  In this ledger,
``Virasoro self-dual'' means this same-family Verdier/Koszul
self-duality, and therefore occurs only at \(c=13\).
\end{enumerate}

Under the hypotheses of the generic Virasoro Koszul-dual theorem
\textup{(}Proposition~\ref{prop:virasoro-generic-koszul-dual}\textup{)},
the algebra-level Verdier partner is
\[
(\mathrm{Vir}_c)^!\simeq \mathrm{Vir}_{26-c}.
\]
Only on that theorem surface does the scalar complementarity
calculation apply:
\begin{equation}\label{eq:virasoro-ledger-K}
K^\kappa(\mathrm{Vir}_c,\mathrm{Vir}_{26-c})
=\frac c2+\frac{26-c}{2}=13
\end{equation}
\textup{(}Corollary~\ref{cor:uc-K-virasoro}\textup{)}.
\end{proposition}
...
H^\bullet B(\cA),\qquad
K_X(\cA)=\mathbb D_{\Ran}B(\cA),\qquad
\cA^!,\qquad
\cZ^{\mathrm{der}}_{\mathrm{ch}}(\cA).
\]
Here $q_\cA\colon\cA^{\mathrm i}\to B(\cA)$ is the quadratic
comparison, $\mathbb D(q_\cA)$ is its Verdier transpose, and
$\nu_\cA\colon K_X(\cA)\to\cA^!$ is the chosen pair map.  The algebra
$\cA^!$ is the line/defect algebra, and
...
is flat. Its bar cohomology is
$H^\bullet(\barB(\cA)[[\hbar]],\D_\hbar)$.  Verdier duality forms
$K_{X,\hbar}(\cA):=\mathbb D_{\Ran}
(\barB(\cA)[[\hbar]],\D_\hbar)$; a completed curved Koszul-pair map
$\nu_{\cA,\hbar}\colon K_{X,\hbar}(\cA)\to\cA^!_\hbar$ produces the
curved $A_\infty$ algebra
$(\cA^!_\hbar,m_0^\hbar,m_1^\hbar,\ldots)$ whose curvature term is
dual to~$h_\hbar$.
\end{theorem}
...
$\D_\hbar^2=0$ and the displayed bar cohomology is defined.  The
finite-type Verdier hypothesis forms~$K_{X,\hbar}(\cA)$, and the
completed pair map~$\nu_{\cA,\hbar}$ identifies it with the curved
$A_\infty$ algebra~$\cA^!_\hbar$. This is the curved
...
The five tables below record the theorem interfaces carried by the
universal MC element
$\Theta_\cA \in \MC(\gAmod)$
(Theorem~\ref{thm:mc2-bar-intrinsic}). Each row states its own
hypothesis and epistemic status; the shadow archetype column
...
H (chiral Hochschild) &
 $\chi^{\mathrm{bd}}_{B_{\mathfrak h}}$ is a quasi-isomorphism &
 $\dim\ChirHoch^n(\cH_k)=(2,1,0,0,\ldots)$ \\
\bottomrule
\end{tabular}
\end{center}
Write $B_{\mathfrak h}$ for the rank-one even superboson presentation
of $\cH_k$ at nonzero level.  Bakalov--De Sole--Kac compute
\[
 ...
 \qquad\textup{\cite[Theorem~7.4]{BDSK21}}.
\]
Since $\Pi\mathfrak h$ is one-dimensional and odd, the bounded
cohomology dimensions are $(2,1,0,0,\ldots)$.  A bounded-to-chart
quasi-isomorphism $\chi^{\mathrm{bd}}_{B_{\mathfrak h}}$ transports
this calculation to the curve chart and gives
\[
 P_{\cH_k}(t)=2+t,
 \qquad
 \operatorname{Supp}\ChirHoch^\bullet(\cH_k)=\{0,1\}.
\]
The diagonal two-sided bar resolution supplies the chart complex;
its bar-length filtration and the cohomological support are distinct
gradings.
The tower terminates at $r = 2$: $\Theta_{\cH_k}^{\leq r}
= \Theta_{\cH_k}^{\leq 2} = \kappa \cdot \eta \otimes \Lambda$
for all $r \geq 2$.  Its scalar shadow is quadratic.
...
D (modular char.) &
 ...
 $\kappa(\mathcal{F}) = c/2 = 1/4$ \\
H (chiral Hochschild) &
 Family datum $H_H(\mathcal F;S_{\mathcal F})$ &
 $\operatorname{Supp}\ChirHoch^\bullet(\mathcal F)\subseteq
 S_{\mathcal F}$ \\
\bottomrule
\end{tabular}
\end{center}
...
H (chiral Hochschild) &
 Family datum $H_H(\beta\gamma_\lambda;S_{\beta\gamma})$ &
 $\operatorname{Supp}\ChirHoch^\bullet(\beta\gamma_\lambda)
 \subseteq S_{\beta\gamma}$ \\
\bottomrule
\end{tabular}
\end{center}
...
H (chiral Hochschild) &
 Family datum $H_H(bc_\lambda;S_{bc})$ &
 $\operatorname{Supp}\ChirHoch^\bullet(bc_\lambda)\subseteq S_{bc}$ \\
\bottomrule
\end{tabular}
\end{center}
...
D (modular char.) &
 ...
 $\kappa(V_\Lambda) = \operatorname{rank}(\Lambda)$ \\
H (chiral Hochschild) &
 Family datum $H_H(V_\Lambda;S_\Lambda)$ on finite-type sectors &
 $\operatorname{Supp}\ChirHoch^\bullet(V_\Lambda)\subseteq S_\Lambda$ \\
\bottomrule
\end{tabular}
\end{center}
