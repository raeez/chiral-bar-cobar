...
system is the first atom whose tower extends beyond the scalar
...
free fermion is formal
($\Definfmod$ equals its strict model), while $\beta\gamma$ and $bc$
carry higher $L_\infty$ brackets that the strict model
truncates.
...
below; these are the base cases that Part~I reproduces.
...
Each free-field atom specializes the universal MC element $\Theta_\cA \in \MC(\gAmod)$
(Theorem~\ref{thm:mc2-bar-intrinsic}).  The Gaussian archetype is
characterized by maximal truncation: the shadow
obstruction tower terminates at arity~$2$, so that
...
The bar cohomology of the free-field atoms fixes the polynomial
growth baseline for all subsequent families.
...
(Theorem~\ref{thm:ds-bar-gf-discriminant}).  Among free-field atoms, only those whose OPE has a simple pole
produce infinite bar cohomology.  The polynomial
growth ($\sim 3^{n/2}$) calibrates the
exponential growth of interacting theories
(Kac--Moody, $\mathcal{W}$-algebras).
\end{remark}

\section{Free-field atoms: setup and OPE structure}\label{sec:free-field-atoms-setup}

The four free-field atoms (the free fermion, the $\beta\gamma$ system, the $bc$ ghost system, and the single-generator fermion--boson pair) are defined below with their OPE structure and relations.  Each definition specifies input data for the universal construction
