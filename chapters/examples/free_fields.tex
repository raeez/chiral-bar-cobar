...
binary pole data are not uniform: Heisenberg carries the scalar
residue $k/z$, while $\beta\gamma$ and $bc$ carry the nilpotent
ordered contact residue
$\Theta=a\otimes a'-a'\otimes a$ with $\Theta^2=0$
...
stratum together with this nilpotent ordered residue.
...
bc_\lambda,\;\;
\Theta_{\beta\gamma}^{\mathrm{ord}}\,\bigr),
\qquad
\mathfrak{T}_{bc}
\;=\;
\bigl(\,bc_\lambda,\;\;
\beta\gamma_\lambda,\;\;
\Theta_{bc}^{\mathrm{ord}}\,\bigr),
\end{equation}
...
The $\beta\gamma$--$bc$ pair is different: its simple pole is
regular as a scalar rational residue, but it survives on the
ordered bar side as the nilpotent kernel
\[
...
(Theorem~\ref{thm:ds-bar-gf-discriminant}). Among free-field
archetypes, the two-generator simple-pole systems
($\beta\gamma$ and $bc$) are the ones with infinite genus-$0$
bar cohomology. Their exponential rate ($\sim 3^{n/2}$)
calibrates the growth of interacting theories
...
the curvature $m_0 = -(1/4)\,\omega$; together they span
...
$h_\gamma=\lambda$ and $h_\beta=1-\lambda$. The physical
weight-$(1,0)$ system is the special point $\lambda=0$. Its bar
...
\begin{itemize}
\item $\beta(z)$ of conformal weight $h_\beta = 1-\lambda$,
\item $\gamma(z)$ of conformal weight $h_\gamma = \lambda$.
\end{itemize}
with OPEs:
\[
...
curvature $m_0=k\,\omega_{\mathrm{cen}}$ lives on the curved
cobar side.
...
Heisenberg central extension. In the rank-one convention used here it
is
\[
m_0=k\,\omega_{\mathrm{cen}},
\]
where $\omega_{\mathrm{cen}}$ is the central two-cochain dual to the
pairing $\langle J,J\rangle=k$. This is a level statement, not the
central charge $c=1$ of the Heisenberg vertex algebra.
\end{remark}
...
has ordinary square-zero differential. Its Koszul-dual curved cobar
algebra carries the level curvature:
\begin{enumerate}
\item Curvature: $m_0 = k\,\omega_{\mathrm{cen}}$, where
...
\item Curved relation: $m_1(m_0) = 0$ (the central class is closed)
...
trace-form collision convention of
Remark~\ref{rem:kappa-vir-is-c-over-2}, it produces the ordered
residue $r_{\cH_k}^{\mathrm{coll}}(z)=k/z$ and the central
two-cochain $\omega_{\mathrm{cen}}$.
...
=k\,\omega_{\mathrm{cen}}.
\]
This is the curvature $m_0$ on the curved cobar/Koszul-dual side.
...
Since $m_0=k\,\omega_{\mathrm{cen}}$ is central, $[m_0,a]=0$ and
so $m_1^2=0$, consistent with $\dzero^2=0$ on the genus-$0$ bar
complex.
\end{proof}
...
\emph{Step~5.}
The residue pairing matrix of Step~3 is rank~$3$: the generators
$\psi^{(-1)},\psi^{(0)},\psi^{(1)}$ pair with the four generators
$\beta,\gamma,b,c$ of the derived $\beta\gamma$-$bc$ system. The
pairing $\langle \psi^{(i)},-\rangle\colon \mathcal{B}^{\bullet}
\to \mathbb{C}$ identifies $\mathcal{F}^{\bullet}$ with the linear
dual of the cogenerators of $\bar{B}(\mathcal{B}^{\bullet})$.
Those cogenerators sit in bar degree~$1$ and are dual to
$\psi^{(0)},\psi^{(\pm 1)}$; the bar differential
$d_{\bar{B}}$ encodes the BRST differential $Q$ of Step~4.

The $A_\infty$ structure on $\mathcal{F}^{\bullet}$ is recovered from
the Verdier dual
$\mathbb{D}_{\mathrm{Ran}}(\bar{B}(\mathcal{B}^{\bullet}))$. This is
the homotopy Koszul-dual factorization algebra computed from the bar
resolution (Theorem~\ref{thm:bar-cobar-isomorphism-main}), and it
identifies $\mathcal{F}^{\bullet}$ as the homotopy Koszul dual
$(\mathcal{B}^{\bullet})^!_\infty$.

The functor
$\Omega(\bar{B}(\mathcal{B}^{\bullet})) \simeq \mathcal{B}^{\bullet}$
is bar-cobar inversion: it recovers $\mathcal{B}^{\bullet}$ itself,
not the dual algebra.
\end{evidence}
...
one of the $W_\infty$/Yangian finite-window comparisons; no analogue of
...
\item \emph{Koszul duality} (algebra $\to$ dual coalgebra $\to$ dual algebra).
 ...
 Level behavior: $k$ appears as the curvature of the curved
 cobar/Koszul-dual algebra; it does not change the duality type.
...
curvature appears after passing to the curved cobar/Koszul-dual
algebra: for $\cH_k$ it is the level class
$m_0=k\,\omega_{\mathrm{cen}}$
...
The modular $S$-transformation thus sends $k \to 1/k$, while level \emph{negation} $k \to -k$ arises instead from Koszul duality: the dual algebra~$\mathcal{H}_k^!$ has curvature $-m_0=-k\,\omega_{\mathrm{cen}}$.
...
the dual algebra~$\mathcal{H}_k^!$ has
$m_0=-k\,\omega_{\mathrm{cen}}$
...
(Table~\ref{tab:master-invariants}) at the chain-counting level.
...
\item \emph{Curvature = conformal anomaly.}
 ...
 anomaly; free-field/Kac--Moody anomaly duality
...
between $\cA$ and a candidate holographic package with three separate
objects: $B(\cA)$ is the boundary bar coalgebra,
$\cA^!=(H^\bullet B(\cA))^\vee$ supplies the line/defect sector after
Verdier-linear duality, and
$\cZ^{\mathrm{der}}_{\mathrm{ch}}(\cA)$ is the chiral Hochschild
cochain algebra supplying bulk observables. The cobar equivalence
recovers $\cA$ itself; it does not identify $\cA^!$ with the derived
center. The bar filtration degree maps to the radial coordinate.
(Contributing to Conjecture~\ref{conj:master-bv-brst}.)
\end{conjecture}
...
non-vanishing $m_0 = \kappa\,\omega_1$ makes
$\cA^!_\hbar$ a curved cobar algebra with curvature
...
the curved-Koszul model of
Conjecture~\ref{conj:loop-string-coupling}, with
$m_0 = c/24N$.
\end{example}
...
= \kappa(\mathrm{matter}) + \kappa(\mathrm{ghost}) = 0$ \textup{(}free-field string sector\textup{)}
...
This final section compares the holographic Witten-diagram shadow,
the interaction between filtrations and bar complexes, and the main
theorems verified by free-field examples.
...
B (inversion) &
 ...
 cobar counit; genus-$0$ chain rank $2 \cdot 3^{n-1}$ (Thm~\ref{thm:betagamma-bar-dim}) \\
C (complementarity) &
...
B (inversion) &
 ...
 Genus-$0$ qi \\
C (complementarity) &
 $\kappa(V_\Lambda) + \kappa(V_{\Lambda^*}) = 0$ \textup{(}lattice pair\textup{)} &
...
At $\Phi_2(D^b\mathrm{Coh}(K3))$ the free-field output is the
Mukai-Heisenberg algebra $\mathcal{H}_{\mathrm{Muk}}$ at $c = 24$ on
$\Lambda_{\mathrm{Muk}} = \mathrm{II}_{4,20}$. Its bar Euler product is
$\eta(q)^{24}$. The Hodge-supertrace invariant is
...
\]
The $\eta^{24}$ free-field denominator reappears on the elliptic
$\tau$-direction, times the Jacobi form $\phi_{10,1}$ on the
$\rho$-direction.
\end{remark}
