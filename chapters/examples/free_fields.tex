...
terminates at arity~$2$, its $r$-matrix vanishes after $d\log$
...
arity~$2$. A second boson coupled through a symplectic pairing
...
tower extends to arity~$4$ (class~$\mathsf{C}$). A fermion-boson
pair with a simple-pole OPE gives the $bc$ ghost system, Koszul dual
to $\beta\gamma$ with matching shadow invariants in the opposite
statistical sector. The dichotomy is sharp: one generator forces
class~$\mathsf{G}$; two generators forces class~$\mathsf{G}$ or
class~$\mathsf{C}$ according to the statistics of the pairing, and
no finer gradation exists within the free-field regime. The
$r$-matrix profile is uniform across all three cases after $d\log$
absorption: $r(z) = 0$. What distinguishes the classes is the
arity~$4$ contact stratum, not the binary shadow data.
...
The bar cohomology of the free-field archetypes fixes the polynomial
...
(Theorem~\ref{thm:ds-bar-gf-discriminant}).  Among free-field archetypes, only those whose OPE has a simple pole
...
The four free-field archetypes (the free fermion, the $\beta\gamma$ system, the $bc$ ghost system, and the single-generator fermion--boson pair) are defined below with their OPE structure and relations.  Each definition specifies input data for the universal construction
$\Theta_\cA := D_\cA - d_0 \in \MC(\gAmod)$
(Theorem~\ref{thm:mc2-bar-intrinsic}).  The bar complex, Koszul dual, genus tower, and shadow invariants computed below are all \emph{outputs} of that single MC element, projected to the relevant arity and genus.  For free-field archetypes, the projection is maximally simple: the Gaussian archetype (free fermion) has $\Theta_\cA = \Theta_\cA^{\leq 2}$, and the contact/quartic archetype ($\beta\gamma$, $bc$) terminates at $\Theta_\cA^{\leq 4}$.
...
\begin{proof}[Sketch]
The PBW computation at arities $n \leq 4$ shows the same contact/quartic
...
the single simple-pole OPE is fixed.  Stratum separation at arity~$5$
...
at $\lambda = 1/2$ (self-dual weight), $c_{bc} = 1$ and
$\kappa(bc) = 1/2$.
...
The bar complex of each free-field archetype is determined by OPE residues along collision divisors in the Fulton--MacPherson compactification; the results range from collapse (free fermion) through finite rank growth ($\beta\gamma$, Heisenberg) to lattice-graded structure (lattice VOAs).
...
Curved ($\kappa = k$) & Curved ($m_0 = -k\omega$) \\
