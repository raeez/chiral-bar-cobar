...
collapse pattern $r_{\max}=2$). MA-6 discipline: the
...
which lowers the pole order by one in the collision-residue
normalization of Table~\ref{tab:rmatrix-census}. Thus an OPE term
$a_n(z-w)^{-n}$ contributes $a_n(z-w)^{-(n-1)}$ to the polar
trace-form residue. For the free fermion the only singular
coefficient is the scalar identity $a_1=\mathbf 1$, so the extracted
term is regular and scalar. The scalar contraction survives in the
ordered bar differential before reduction, but its projection to the
field-valued rational $r$-matrix has no polar part. Hence
$r(z)=0$.
...
data of Theorem~\ref{thm:modular-classification}.
\end{theorem}
...
Conjecture~\ref{conj:w-algebra-quantum-cohomology} identifies it with
$QH^*(G/B)$ under the stated matching of Schubert cell combinatorics
with collision strata.
\end{remark}
