...
are the atoms from which all conformal field theories assemble
by OPE extension.  In each, higher bar operations vanish
for structural reasons: statistics, absence of higher-order
poles, or the weight filtration on the bar complex.

In the shadow obstruction tower of $\gAmod$
(Definition~\ref{def:modular-convolution-dg-lie}), the free-field
atoms exhibit two of the four archetypes.  The free fermion
and Heisenberg algebra lie in the Gaussian family (class~G):
towers terminate at weight~$2$.  The $\beta\gamma$
...
weight~$4$ (class~C; explored in full
in Chapter~\ref{chap:beta-gamma}).  The Koszul dual $bc$ system
shares class~C ($r_{\max} = 4$): its two-generator
structure admits a nontrivial quartic contact shadow by the same
stratum separation mechanism, terminating at arity~$4$.  (The
single-generator free fermion, by contrast, has $S_3 = 0$ from fermionic antisymmetry and terminates at arity~$2$.)
The remaining two
archetypes belong to interacting families: Lie/tree (class~L,
...
Each free-field pair exhibits Koszul duality in its simplest form:
statistics exchange
(bosonic $\leftrightarrow$ fermionic) and propagator inversion.
The bar complex, Koszul dual, and genus tower of each pair are computed
below; they form the base cases that Part~I must
reproduce.
