...
\emph{structural}, not miraculous: it is forced by statistics, by the
...
In the Chriss--Ginzburg framework, each free-field atom is a
...
(Theorem~\ref{thm:virasoro-koszul-dual}).  The full \emph{kinematic}
...
\begin{proof}
By Theorem~\ref{thm:heisenberg-bar}, the bar complex $\bar{B}^{\mathrm{ch}}(\mathcal{H}_k)$ has cocommutative coproduct: the Heisenberg OPE $\alpha(z)\alpha(w) \sim k(z-w)^{-2}$ produces only the double-pole term, whose residue gives a symmetric (primitive) coproduct $\Delta(\alpha) = \alpha \otimes 1 + 1 \otimes \alpha$.  Cocommutative coalgebras are Koszul dual to commutative algebras ($\mathrm{coLie}^! = \mathrm{Com}$, see \cite{LV12}, Theorem~7.6.5), so $\mathcal{H}_k^! = \mathrm{Sym}^{\mathrm{ch}}(V^*)$.  This is commutative with regular OPE, while $\mathcal{H}_k$ is non-commutative with singular OPE, so $\mathcal{H}_k \not\cong \mathcal{H}_k^!$.
\end{proof}
...
\item \textup{(This monograph, $n = 2$)}\quad
  ...
  specializes to the Verdier intertwining
  $\mathbb{D}_{\mathrm{Ran}}\, \bar{B}(\cA) \simeq \bar{B}(\cA^!)$
...
For the comprehensive Koszul duality table (including all entries formerly listed here), see \S\ref{sec:kd-computations-free-fields}.
...
\begin{proof}
The essential point is that at $c = 26$, the conformal anomaly of the
...
Chapter~\ref{chap:kac-moody-koszul}.  Here we prove the key identification with string BRST
cohomology.
...
Chapter~\ref{chap:w-algebras}.  Here we record the key results that
connect the Wakimoto free-field realization to the bar complex.
...
if and only if the theory is gapped.  In the Chriss--Ginzburg
framework, the mass deformation adds a quadratic perturbation to
