...
zero: the archetype is Gaussian (class~$\mathsf{G}$).
...
degree~$4$ contact stratum, not the binary shadow data.
...
Gaussian class because the standard conformal-weight families
$\beta\gamma_\lambda$ and $bc_\lambda$ satisfy
$S_2 = 6\lambda^2 - 6\lambda + 1$, $S_3 = 0$, $S_4 = -5/12$,
and $S_r = 0$ for $r \ge 5$, even though the internal
...
(Theorem~\ref{thm:cech-hca}) terminates at degree~$3$;
...
(Theorem~\ref{thm:mc2-bar-intrinsic}). The bar complex, Koszul dual, genus tower, and shadow invariants computed below are all \emph{outputs} of that single MC element, projected to the relevant degree and genus. For free-field archetypes, the projection is maximally simple: the Gaussian archetype (free fermion) has $\Theta_\cA = \Theta_\cA^{\leq 2}$, and the contact/quartic archetype ($\beta\gamma$, $bc$) terminates at $\Theta_\cA^{\leq 4}$.
...
of the form $F_g = \kappa \cdot \lambda_g^{\mathrm{FP}}$
\textup{(}UNIFORM-WEIGHT\textup{)}. The ratio at
each genus is:
\[
...
\begin{theorem}[$\beta\gamma$ global shadow depth; \ClaimStatusProvedElsewhere]
...
Conjecture~\ref{conj:v1-master-infinite-generator}, not to the standard
...
obstruction class $\mathrm{obs}_g = \frac{1}{4}\lambda_g$
...
At $\Phi_2(D^b\mathrm{Coh}(K3))$ the free-field output is the Mukai-Heisenberg $\mathcal{H}_{\mathrm{Muk}}$ at $c = 24$ on $\Lambda_{\mathrm{Muk}} = \mathrm{II}_{4,20}$. Bar Euler product $\eta(q)^{24}$. The Hodge-supertrace invariant is $\kappa_{\mathrm{CY}}=\chi(\mathcal{O}_{K3})=2$, while the Theorem~C $\mathcal{B}$-row uses the Mukai positive-signature scalar $\kappa_{\mathrm{Muk}}=c_+(\Lambda_{\mathrm{Muk}})=4$. Koszul conductor on the Mukai-doubling side: $K^{\kappa_{\mathrm{Muk}}}=2\,c_+(\mathrm{Mukai}(K3)) = 8$. The Theorem~C bucket enlarges on the Lorentzian-lattice-parametric $\mathcal{B}$-family to $\{0, 8, 13, 250/3, 98/3\}$.
\end{remark}

\begin{remark}[Genus-2 Siegel character factorisation]
\label{rem:ff-genus2-factorisation}
At the separating-node degeneration of the genus-$2$ Siegel character of the K3 chiral bialgebra $\mathbf H_{\Delta_5}$ (Vol~III Chapter~\ref{ch:k3-chiral-bialgebra-platonic}), $1/\Phi_{10}(\mathrm{diag}(\tau, \rho)) = 1/\eta(\tau)^{24} \cdot 1/\phi_{10,1}(\rho)$: the $\eta^{24}$ free-field denominator reappears on the elliptic $\tau$-direction, times the Jacobi form $\phi_{10,1}$ on the $\rho$-direction.
\end{remark}
