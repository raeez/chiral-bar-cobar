...
\label{sec:ff-wave13-DNA}

\begin{remark}[Mukai-Heisenberg as free-field output of $\Phi_2$ on K3]
\label{rem:ff-DNA-mukai-heisenberg}
At $\Phi_2(D^b\mathrm{Coh}(K3))$ the free-field output is the Mukai-Heisenberg $\mathcal{H}_{\mathrm{Muk}}$ at $c = 24$ on $\Lambda_{\mathrm{Muk}} = \mathrm{II}_{4,20}$. Bar Euler product $\eta(q)^{24}$; $\kappa_{\mathrm{ch}} = 2 = \chi(\mathcal{O}_{K3})$. Koszul conductor on the Mukai-doubling side: $K^{\kappa_{\mathrm{ch}}} = 2\,c_+(\mathrm{Mukai}(K3)) = 8$. Wave~13 Theorem~C bucket enlarges on the Lorentzian-lattice-parametric $\mathcal{B}$-family to $\{0, 8, 13, 250/3, 98/3\}$.
\end{remark}

\begin{remark}[Genus-2 Siegel character factorisation]
\label{rem:ff-DNA-genus2-factorisation}
At the separating-node degeneration of the genus-$2$ Siegel character of the K3 chiral bialgebra (Vol~III Wave~13), $1/\Phi_{10}(\mathrm{diag}(\tau, \rho)) = 1/\eta(\tau)^{24} \cdot 1/\phi_{10,1}(\rho)$: the $\eta^{24}$ free-field denominator reappears on the elliptic $\tau$-direction, times the Jacobi form $\phi_{10,1}$ on the $\rho$-direction.
\end{remark}
