...
This chapter is the periodic table.  The free fermion, the $bc$ ghost
system, and the $\beta\gamma$ system are the atoms from which all
conformal field theories are assembled by OPE extension.  In each free
system a great many higher operations vanish, but the vanishing is
\emph{structural}: it is forced by statistics, by the
absence of higher-order poles, or by the weight filtration on the bar
complex.  Understanding precisely \emph{why} each bar component
vanishes is the prerequisite for recognizing what goes wrong --- and
what becomes interesting --- when the same analysis is attempted for
the nonabelian Kac--Moody and $\mathcal{W}$-algebra families of later
chapters.

From the standpoint of the shadow Postnikov tower in $\gAmod$
(Definition~\ref{def:modular-convolution-dg-lie}), the free-field
atoms span the first three shadow archetypes.  The free fermion,
having a simple-pole OPE, lies in the same Gaussian family as the
Heisenberg algebra: its shadow tower terminates at weight~$2$.
The $bc$ ghost system, though fermionic, carries a nonzero curvature
$\kappa_{bc} \neq 0$ and terminates at the same level.  The
$\beta\gamma$ system is the first atom where the tower extends beyond
the scalar level: $m_0 = 0$ (no double pole) but the quartic contact shadow is
nontrivial, so $\Theta^{\leq 4} \neq 0$ and the tower terminates at
weight~$4$ (the contact/quartic archetype, explored in full in
Chapter~\ref{chap:beta-gamma}).  Thus the periodic table already
exhibits termination at weight~$2$ and weight~$4$; the intermediate
case (weight~$3$, the Lie/tree archetype) belongs to the affine
Kac--Moody family of Chapter~\ref{chap:kac-moody-koszul}, and the
infinite tower appears only with $\mathcal{W}$-algebras
(Chapter~\ref{chap:w-algebra-koszul}).  At the homotopy level, each
free-field atom occupies a different position in the formality
landscape: the fermion and $bc$ systems are trivially formal
($\Definfmod$ equals its strict model), while $\beta\gamma$ carries
genuine higher $L_\infty$ brackets that the strict model truncates.
...
\begin{remark}[Shadow Postnikov tower for free fields: the Gaussian base case]
...
The free fermion and $bc$ ghost system are Gaussian with
$\kappa \neq 0$; the $\beta\gamma$ system breaks out of the Gaussian
class because $\Theta^{\leq 4} \neq \Theta^{\leq 2}$ (quartic contact
shadow nontrivial), even though $\kappa_{\beta\gamma} = 0$ on the
weight-changing line.  Thus the free-field periodic table already
exhibits the transition from the Gaussian to the contact/quartic
archetype --- the first step up the shadow tower classification
...
(Theorem~\ref{thm:mc2-bar-intrinsic}).  The bar complex, Koszul dual, genus tower, and shadow invariants computed below are all \emph{outputs} of that single MC element, projected to the relevant arity and genus.  For free-field atoms, the projection is maximally simple: the Gaussian archetype (free fermion, $bc$) has $\Theta_\cA = \Theta_\cA^{\leq 2}$, and even the contact/quartic archetype ($\beta\gamma$) terminates at $\Theta_\cA^{\leq 4}$.
...
Having computed the bar complexes, we now extract the Koszul duals.  The dualities exchange statistics (bosonic $\leftrightarrow$ fermionic) and invert the propagator; they extend to module categories and twisted sectors.
...
Virasoro$_{26}$ & String vertex algebra & Classical & Moduli $\leftrightarrow$ BRST & \ClaimStatusProvedHere \\
...
\emph{H-level (MC4 closed on $\mathcal{W}_\infty$)}: the infinite-generator identifications Virasoro $\rightsquigarrow W_\infty$, $\mathcal{W}_N \rightsquigarrow Y(\mathfrak{gl}_N)$, Super-Virasoro $\rightsquigarrow$ Super-$W_\infty$.  The M-level theory (bar complex, inversion, Koszulness, complementarity) is proved for all principal $\mathcal{W}_N$ at generic level.  On the $\mathcal{W}_\infty$ side, the finite-packet coefficient identities $\mathsf{C}^{\mathrm{res}} = \mathsf{C}^{\mathrm{DS}}$ on $\mathcal{I}_N$ are unconditionally closed at all stages via $\mathcal{W}_N$ rigidity (coefficient identification, Theorem~\ref{thm:winfty-all-stages-rigidity-closure}) within the strong completion-tower framework (structural proof, Theorem~\ref{thm:completed-bar-cobar-strong}).  On the Yangian side, the evaluation-core comparison $\mathsf{K}^{\mathrm{line}} = \mathsf{K}^{\mathrm{RTT}}$ on $\Delta_{a,0}(N)$ is verified ($249$ tests); the remaining DK-5 categorical packet is downstream of MC3 (now accessible in type~A).
\end{remark}

\begin{remark}[Scope: conjectural table entries]
The three infinite-generator identifications each involve curved Koszul duality ($m_0 \neq 0$) with infinitely many generators.  Two structural features: (1)~non-quadratic OPE prevents the quadratic framework of Chapter~\ref{chap:koszul-pairs} from applying directly; (2)~the infinite-generator flank requires the coefficient-identification package, which is now unconditionally resolved on the $\mathcal{W}_\infty$ side via $\mathcal{W}_N$ rigidity (MC4 closed).  The bar--semi-infinite identification (Theorem~\ref{thm:bar-semi-infinite-w}) constrains the remaining problem: the same-family involution $\mathrm{Vir}_c \leftrightarrow \mathrm{Vir}_{26-c}$ gives the proved M/S-level shadow, while the H-level realization asks for an infinite-generator dual whose bar cohomology matches the semi-infinite growth (Motzkin numbers, Theorem~\ref{thm:virasoro-chiral-koszul}).  The super row remains a separate extension flank adjoining Conjecture~\ref{conj:master-infinite-generator}.
\end{remark}
...
\item \emph{W-algebras.} At critical level: $\mathcal{W}^{-h^\vee}(\mathfrak{g}, f) \leftrightarrow \mathcal{W}^{-h^{\vee,\vee}}(\mathfrak{g}^\vee, f^\vee)$ (quantum Langlands); at general central charge: $\mathcal{W}_N^c \leftrightarrow \mathcal{W}_N^{c'}$ with $c + c' = 2(N-1)(2N^2+2N+1)$.
...
The bar curvature $m_0$ is the one-loop determinant: $m_0 = k/2$ for $\cH_k$, $m_0 = -13$ for $bc$.  Anomaly cancellation (Theorem~\ref{thm:anomaly-koszul}): $\barB^{\mathrm{ch}}(\cH^{26} \otimes bc)$ is uncurved ($26 \cdot \tfrac{1}{2} - 13 = 0$), yielding the BV quantization of the bosonic string.  The formula $m_0 = \kappa/2$ (Proposition~\ref{prop:pole-decomposition}) matches the QME coefficient.
\end{remark}
...
At genus~$1$, Koszul duality is electromagnetic duality on $T^2$: A-cycle winding (position) exchanges with B-cycle winding (momentum), and the S-transformation $\Omega \to -1/\Omega$ acts as Fourier transform on $\mathrm{Jac}(T^2)$.  The level shift $k \to -k$ is the quantum manifestation of this classical symplectic duality.
\end{remark}
...
The Weyl--Kac character formula (Theorem~\ref{thm:kw-bar-spectral}) replaces the dual-lattice sum in classical Poisson summation with an alternating sum over $W_{\mathrm{aff}} = W \ltimes Q^\vee$.  The bar spectral sequence realizes this: $E_1$ is indexed by Verma modules $\mathcal{M}(w \cdot \lambda)$, collapsing at generic level and reducing to a finite Weyl orbit at admissible level $k = -h^\vee + p/q$.
\end{remark}
...
$\dim(L^-_a)_{-2k} = p(k)$ of the Yangian negative prefundamental
modules (Theorem~\ref{thm:shifted-prefundamental-generation},
Step~2), establishing a partition-theoretic bridge between the
free-field bar complex and the MC3 thick generation package
...
The Virasoro bar complex is computed at general central charge and specialized to $c = 26$, where it identifies with moduli space cohomology.  The string BRST correspondence, amplitude computations, and modular invariance follow.
...
\subsubsection{Bar complex and moduli space}\label{sec:virasoro-bar-moduli-main}
...
ghost contribution and the bar complex becomes an honest cochain
complex.  The resulting cohomology is the cohomology of moduli space
itself --- the bar construction reconstructs $\overline{\mathcal{M}}_{0,n}$
...
language: the curvature $\kappa(\mathcal{A})$ vanishes, and the
uncurved bar complex computes the cohomology of moduli space directly.
...
diffeomorphism group is, in bar-cobar language, the Koszul dual
sector.  The BRST operator $Q_{\mathrm{BRST}}$ is the bar
differential~$d_{\barB}$.  Physical states
$\ker Q_{\mathrm{BRST}} / \operatorname{im} Q_{\mathrm{BRST}} =
H^*(\barB(\cA))$ are bar cohomology classes.  The ghost number
grading is the cohomological degree of the bar complex.  This
identification transforms the gauge-fixing auxiliary device into a
structural component of Koszul duality: the ghost sector is not
introduced by hand but \emph{required} by the algebraic structure of
the bar construction on~$\mathrm{Vir}_{26}$.
\end{remark}
...
\begin{remark}[Virasoro bar cohomology as moduli space cohomology]
\label{rem:virasoro-bar-moduli-polyakov}
\index{Virasoro algebra!moduli correspondence!Polyakov significance}
The Virasoro-moduli correspondence (Theorem~\ref{thm:virasoro-moduli}) states $H^n(\bar{B}_{\mathrm{geom}}(\mathrm{Vir}_{26})) \cong H^n(\overline{\mathcal{M}}_{0,n+3})$.  In Polyakov's language: at the critical dimension $c = 26$, the bar complex of the worldsheet CFT computes the cohomology of moduli space.  This is the algebraic content of the statement that the bosonic string path integral localises on the moduli space of Riemann surfaces.  Away from $c = 26$, the curvature $d^2 = (c/2 - 13)\,\kappa_1$ obstructs localisation; the shadow tower measures the obstruction arity by arity.
\end{remark}
...
\begin{corollary}[Genus-\texorpdfstring{$0$}{0} string amplitudes from bar complex;
...
At $g = 0$ both are established (Corollary~\ref{cor:string-amplitude-genus0}).
...
\begin{theorem}[Bar complex computes genus-\texorpdfstring{$g$}{g} string
...
\begin{theorem}[Modular anomaly for KM and \texorpdfstring{$\mathcal{W}$}{W}-algebras;
\ClaimStatusProvedHere]
...
The physically relevant values are $c = 26$ (bosonic string; cf.\ Theorem~\ref{thm:virasoro-moduli}) and $c = 15$ (superstring).
\end{example}
...
\begin{theorem}[Quantum integrability via \texorpdfstring{$A_\infty$}{A-infinity}; \ClaimStatusProvedHere]\label{thm:w-integrability}
The quantum W-algebra $A_\infty$ structure extends classical integrability: the higher operations $m_k$ ($k \geq 3$) encode quantum corrections $m_k = O(\hbar^{k-2})$, and the $A_\infty$ master equation ensures quantum integrability.  The proof proceeds by induction on filtration degree using the PBW (Li) filtration and the curved $A_\infty$ formalism (Theorem~\ref{thm:curved-koszul-pairs}); the obstruction at each stage lies in $H^1(\mathcal{W}^{\mathrm{cl}}(\mathfrak{g}), \mathrm{ad})$, which vanishes by the rigidity of the Drinfeld--Sokolov Poisson structure (Kostant slice theorem).  See Chapter~\ref{chap:w-algebras} for the complete argument.
\end{theorem}
...
Free fields verify all three main theorems in the abelian case.
\emph{Theorem~A}: fermion$^! = \operatorname{Sym}^{\mathrm{ch}}(\gamma)$, $(bc)^! = \beta\gamma$, $\mathcal{H}_k^! = \operatorname{Sym}^{\mathrm{ch}}(V^*)$ with $m_0 = -k\,\omega$; dualities are exact ($c + c' = 0$).
\emph{Theorem~B}: genus-$0$ inversion via Theorem~\ref{thm:bar-cobar-inversion-qi}; genus-$1$ inputs from Theorems~\ref{thm:elliptic-fermion-bar} and~\ref{thm:heisenberg-genus-g}.
\emph{Theorem~C}: $\operatorname{obs}_g(\mathcal{H}_k) = k \cdot \lambda_g$ with $\kappa(\mathcal{H}_k) + \kappa(\mathcal{H}_k^!) = 0$.
\end{remark}
...
The bar complex computes resolutions on the Ran space; the
\v{C}ech complex (Theorem~\ref{thm:cech-hca}) computes resolutions
on the Zariski site.  On $\mathbb{P}^1$ with the standard two-element
...
Proposition~\ref{prop:cech-bar-intertwining} sends the strict
