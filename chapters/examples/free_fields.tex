...
their shadow towers terminate at weight~$2$.  The $\beta\gamma$
...
The shadow tower is purely scalar:
...
\subsubsection{Shadow Postnikov tower of the free fermion}
...
The shadow Postnikov tower of the free fermion
...
class~G termination: when the shadow tower terminates at arity~$2$,
...
for all free-field Koszul pairs \textup{(}AP24: this holds for
...
\subsection{\texorpdfstring{Shadow Postnikov tower of the $\beta\gamma$ system}{Shadow Postnikov tower of the beta-gamma system}}
...
Gaussian class.  The full shadow Postnikov tower is controlled by the
...
\item \emph{The weight-changing line}: the deformation that shifts the
  ...
  so the shadow tower is purely quadratic by rank-one abelian rigidity
...
$c = 2(6\lambda^2 - 6\lambda + 1)$.  The shadow tower restricted to
this one-dimensional slice is the Virasoro shadow tower at this value
...
\begin{proposition}[$\beta\gamma$ shadow tower: T-line data;
\ClaimStatusProvedHere]
\label{prop:betagamma-T-line-shadows}
\index{beta-gamma system@$\beta\gamma$ system!T-line shadow data}
On the stress tensor line, the shadow tower of $\beta\gamma$ at
weight~$\lambda$ is
\begin{alignat}{2}
...
OPE pole by AP19), and $S_4 = Q^{\mathrm{contact}}_{\mathrm{Vir}}(c) =
...
\begin{proposition}[$\beta\gamma$ shadow tower: weight-changing line;
\ClaimStatusProvedHere]
\label{prop:betagamma-weight-line-shadows}
\index{beta-gamma system@$\beta\gamma$ system!weight-changing line}
On the weight-changing line, the shadow tower vanishes at all arities:
\[
...
$n \geq 2$.  The shadow tower is therefore purely quadratic, and since
...
shadow tower on the unique primary line \emph{is} the global tower.
\end{remark}
...
\begin{computation}[$\beta\gamma$ shadow tower: special weight table;
\ClaimStatusProvedHere]
...
Central charge $c = 1$.
Shadow tower terminates at the scalar level:
...
with theta constants.  The shadow tower prediction
...
\textbf{$r$-matrix} & $r(z) = 0$ (simple pole absorbed: AP19) \\
...
\subsection{Free energy extraction and the shadow tower
prediction}
...
shadow tower, $\kappa$ is a genus-$0$ invariant
...
\end{enumerate}
In particular, for affine Kac--Moody, anomaly cancellation
...
\end{tabular}
\end{center}
Shadow tower: $\Theta^{\leq 2} = \kappa \cdot \eta \otimes \Lambda$
...
The full shadow tower data (multi-channel structure,
...
$\beta\gamma$ data: the genus tower, shadow tower, and
...
and the shadow tower is purely scalar.  For root lattices
