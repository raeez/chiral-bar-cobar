...
\item \emph{$\widehat{\mathfrak{sl}}_2$}:
  $\dim (\widehat{\mathfrak{sl}}_2^!)_n = R(n+3)$ where $R$ denotes the Riordan numbers
  (OEIS~A005043).  This follows from the chiral Koszulness of
  $\widehat{\mathfrak{sl}}_2$
  (Theorem~\ref{thm:km-chiral-koszul}), the universal
...
\item \emph{$\mathcal{W}_3$}: computed through degree~$3$
  ...
  (Conjecture~\ref{conj:w3-algebraicity}) and should be regarded
  as conjectural until independently verified.\textsuperscript{$\dagger$}
...
\item \emph{Exponential growth.}
  ...
  \item $\widehat{\mathfrak{sl}}_2$: $\dim (\widehat{\mathfrak{sl}}_2^!)_n = R(n+3)$,
...
The Koszul dual dimension $\dim (\widehat{\mathfrak{sl}}_{2,k}^!)_n
...
\item The formula predicts:
  ...
  $\dim H^7 = 598592$
  (verified by computer through degree~$7$).
...
\emph{Scope.}  Verification requires computing $\dim H^4(\barBgeom(\widehat{\mathfrak{sl}}_{3,k}))$: the value $1352$ would confirm the recurrence~\eqref{eq:sl3-bar-recurrence} and the rationality of the GF, while any other value would rule out this rational form.  The computation requires the rank of a $24576 \times 786432$ matrix, which is beyond hand computation but potentially accessible to numerical linear algebra.
\end{conjecture}
...
\begin{remark}[Degree-$2$ bar cohomology as symmetric square; \ClaimStatusProvedHere]\label{rem:bar-deg2-symmetric-square}
\index{bar complex!degree-2 cohomology}
For any affine Kac--Moody algebra $\widehat{\mathfrak{g}}_k$ with weight-$1$ generators,
\[
\dim H^2(\barBgeom(\widehat{\mathfrak{g}}_k))
= \binom{\dim\mathfrak{g} + 1}{2}
= \dim S^2(\mathfrak{g}).
\]
The chain group $\bar{B}^2 \cong \mathfrak{g}^{\otimes 2} \otimes \mathrm{OS}^1(C_2) \cong \mathfrak{g}^{\otimes 2}$ has dimension $(\dim\mathfrak{g})^2$.  The bar differential $d_2 \colon \bar{B}^2 \to \bar{B}^1$ encodes the Lie bracket (from the simple-pole OPE) and has $\mathrm{rank}(d_2) = \dim[\mathfrak{g},\mathfrak{g}] = \dim\mathfrak{g}$ (for semisimple~$\mathfrak{g}$).  Meanwhile, the differential $d_3 \colon \bar{B}^3 \to \bar{B}^2$ has $\mathrm{rank}(d_3) = \binom{\dim\mathfrak{g}}{2} - \dim\mathfrak{g}$ (from the Jacobi/Arnold relations).  Thus $H^2 = \ker(d_2)/\mathrm{im}(d_3)$ has dimension $\binom{\dim\mathfrak{g}+1}{2}$.  Verified: $\mathfrak{sl}_2$ ($H^2 = 6 = \binom{4}{2}$), $\mathfrak{sl}_3$ ($H^2 = 36 = \binom{9}{2}$).
\end{remark}
