...
$\widehat{\mathfrak{sl}}_3$ (level~$k$)
  & 8 & 36 & 204 & 1352$^\dagger$ & 9892$^\dagger$ & 76084$^\dagger$ & 598592$^\dagger$ & 4755444$^\dagger$ & 37933596$^\dagger$ & 303105608$^\dagger$ & $8^n$ \\
\hline
$\mathrm{Vir}_c$
  & 1 & 2 & 5 & 12 & 30 & 76 & 196 & 512 & 1353 & 3610 & $3^n$ \\
\hline
$\mathcal{W}_3$
  & 2 & 5 & 16 & 52 & 171$^\dagger$ & 564$^\dagger$ & 1862$^\dagger$ & 6149$^\dagger$ & 20308$^\dagger$ & 67072$^\dagger$ & $3.3^n$ \\
\hline
Yangian $Y(\mathfrak{sl}_2)$
  & 4 & 10 & 28 & 82$^*$ & 244$^*$ & 730$^*$ & 2188$^*$ & 6562$^*$ & 19684$^*$ & 59050$^*$ & $3^n$ \\
\hline
\end{tabular}
}
...
\begin{conjecture}[Spectral interpretation of the discriminant; \ClaimStatusConjectured]
\label{conj:discriminant-spectral}
\index{discriminant!spectral interpretation}
\index{Kodaira--Spencer operator!discriminant}
For a simple Lie algebra $\mathfrak{g}$, there exists a
finite-rank operator
$\mathrm{KS}_{\mathfrak{g}}$ on the reduced first bar cohomology
$H^{\mathrm{red}}_1(\cA) \coloneqq
H^1(\bar{B}(\cA)) / H^1_{\mathrm{free}}(\cA)$
such that:
\begin{equation}\label{eq:discriminant-spectral}
\Delta_{\mathfrak{g}}(x)
...
and~$-1$.  Note $3 = \dim \mathfrak{sl}_2$; the
eigenvalue~$-1$ corresponds to the simple pole of
...
The pattern $\dim \mathfrak{g}$ appearing as the largest eigenvalue
is suggestive of a connection to the adjoint representation.
\end{conjecture}
...
Conjecture~\ref{conj:discriminant-spectral} together suggest that the
...
Each Koszul dual component $(\cA^!)_n$ carries a natural filtration by conformal weight: $F^{\leq h} (\cA^!)_n = \{[\alpha] \in (\cA^!)_n : \text{all generators in $\alpha$ have weight $\leq h$}\}$.  The associated graded $\operatorname{gr}^h (\cA^!)_n$ refines the single number $\dim (\cA^!)_n$ into a bigraded Hilbert series $\sum_{n,h} \dim \operatorname{gr}^h (\cA^!)_n \cdot x^n q^h$.  For the Virasoro algebra, the conformal weight grading on $(\mathrm{Vir}_c^!)_n$ comes from the weight of the stress tensor $T$ (weight $2$) and its descendants, giving a $q$-refinement of the Motzkin difference sequence.
\end{remark}

\begin{conjecture}[Motzkin path model for Virasoro bar cohomology;
...
\noindent\emph{Evidence.}
\begin{enumerate}[label=\textup{(\roman*)}]
\item \emph{Dimensions.}
  The number of Motzkin paths of length $n{+}1$ starting with $U$
  equals $M(n{+}1) - M(n)$: the Motzkin recurrence
  $M(n{+}1) = M(n) + \sum_{k=0}^{n-1} M(k) M(n{-}1{-}k)$
  decomposes paths by first step ($F$ contributes $M(n)$;
  $U$ contributes the convolution sum), and the difference
  $M(n{+}1) - M(n)$ counts exactly the $U$-initial paths.
  This matches $\dim H^n(\bar{B}(\mathrm{Vir}_c))$
  by Theorem~\ref{thm:spectral-sequence-collapse}.
\item \emph{DS compatibility.}
  The $\widehat{\mathfrak{sl}}_2$ bar cohomology has Riordan
  dimensions $R(n{+}3)$, which count Motzkin paths avoiding flat
  steps at ground level.  The DS reduction
  $\widehat{\mathfrak{sl}}_2 \to \mathrm{Vir}$
  should correspond to a combinatorial projection on paths:
  BRST reduction removes certain ground-level configurations,
  mapping ground-level-$F$-avoiding paths to $U$-initial paths.
\item \emph{Associahedron structure.}
  The faces of the associahedron $K_{n+2}$
  are counted by $\sum_{k} \binom{n{+}2}{k{+}1}\binom{n{-}k}{k}$,
  which refines the Motzkin differences.  The CW decomposition
  of $\overline{\mathcal{M}}_{0,n+3}$ by boundary strata
  provides a natural cell structure whose cells should biject
  with the Motzkin paths via the bijection
  between boundary strata and planar tree topologies.
\end{enumerate}
\end{conjecture}
