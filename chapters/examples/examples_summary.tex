...
\item[\textbf{T2.}] \emph{PBW-conditional} (Kac--Moody, $\mathcal{W}$-algebras):
  Koszulness proved via the PBW spectral sequence
  (Theorems~\ref{thm:km-chiral-koszul}, \ref{thm:virasoro-chiral-koszul}).
  Duality and invariants follow from the main theorems once
  Koszulness is established.
...
\item \emph{Virasoro}:
  $\dim (\mathrm{Vir}_c^!)_n = M(n{+}1) - M(n)$ where $M(n)$ is the $n$-th
  Motzkin number (OEIS~A001006).  This reflects the
  associahedron structure of $\overline{M}_{0,n+2}$,
  proved via the chiral Koszulness of $\mathrm{Vir}_c$
  (Theorem~\ref{thm:virasoro-chiral-koszul}) and the
  $E_2$ collapse
  (Corollary~\ref{cor:bar-cohomology-koszul-dual}).
...
Conjecture~\ref{conj:sl3-discriminant}):
...
\begin{conjecture}[$\widehat{\mathfrak{sl}}_3$ bar generating function; \ClaimStatusConjectured]\label{conj:sl3-discriminant}
\label{conj:sl3-bar-gf}
