...
\item the first genuinely open standard-tower packets are:
  ...
  corridor carries no new independent coefficient, while
  ...
  closes DK-4/DK-5 on the canonical target, while
...
\begin{corollary}[Genus-$1$ free energy and anomaly ratio; \ClaimStatusProvedHere]
...
\begin{remark}[Koszul conductor for $\mathcal{W}_N$]\label{rem:koszul-conductor-explicit}
\index{Koszul conductor!explicit formula}
The \emph{Koszul conductor} $K = c(\cA) + c(\cA^!)$ (Theorem~\ref{thm:central-charge-complementarity}) admits a closed-form expression for the $\mathcal{W}_N$ series.  Using the Drinfeld--Sokolov central charge $c(k) = (N-1)(1 - N(N+1)(k+N)^{-1}(k'+N)^{-1})$ and the Freudenthal--de Vries formula $c + c' = 2r + 4h^\vee d/(h^\vee)$ with $r = N-1$, $h^\vee = N$, $d = N^2-1$:
\[
...
\begin{remark}[$E_8$ complementarity]\label{rem:e8-complementarity}
...
\begin{remark}[Status of $H^4(\mathcal{W}_3) = 52$]
...
\begin{remark}[Table values vs.\ genus-$0$ vanishing]\label{rem:table-vs-vanishing}
...
\begin{proposition}[Construction of $H^{\mathrm{red}}_1$ for
...
\begin{conjecture}[$\widehat{\mathfrak{sl}}_3$ bar generating function; \ClaimStatusConjectured]\label{conj:sl3-bar-gf}
...
\begin{remark}[Degree-$2$ bar cohomology at lowest weight; \ClaimStatusProvedHere]\label{rem:bar-deg2-symmetric-square}
...
\begin{corollary}[{$\beta\gamma$ generating function via discriminant}; \ClaimStatusProvedHere]
