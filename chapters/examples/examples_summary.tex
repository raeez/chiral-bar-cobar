...
For the Yangian row, ``PH'' records only the proved DK-0/DK-1 package:
the chain-level bar-cobar / $R \mapsto R^{-1}$ duality
\textup{(}Theorem~\ref{thm:yangian-koszul-dual}\textup{)} together with
the evaluation-locus factorization comparison
\textup{(}Theorem~\ref{thm:factorization-dk-eval}\textup{)}.  The full
factorization categories are not being claimed in this table.

The remaining Yangian frontier is the staged DK-2--DK-5 ladder.
DK-2 is the preservation/thick-generation problem for
$D^b(\mathcal{O}_{Y_\hbar})$
\textup{(}Proposition~\ref{prop:yangian-dk2-thick-generation}\textup{)}.
DK-3 is the evaluation-generated factorization-core reconstruction
problem
\textup{(}Proposition~\ref{prop:yangian-dk3-generated-core}\textup{)}.

DK-4/DK-5 are the RTT-complete H-level comparison and completed
...
\item[\textbf{DK-0/1.}] \emph{$\Eone$-Yangian regime}:
  ...
  The later DK stages remain conjectural, but they are now exact:
  DK-2 is preservation/thick generation in category~$\mathcal{O}$,
  DK-3 is the evaluation-generated factorization-core reconstruction
  problem, and DK-4/DK-5 are the dg-shifted RTT-complete comparison.
...
\item Yangians: the proved content is DK-0/DK-1.  At chain level
  ...
  later stages to full factorization categories and the dg-shifted
  Yangian comparison remain conjectural
...
The Riordan generating function~\eqref{eq:gf-sl2-riordan}
was derived from the prediction
...
$\widehat{\mathfrak{sl}}_2$ requires revision;
...
the matrices are similar unconditionally.  The first non-trivial
test case is $\mathfrak{g} = \mathfrak{sl}_3$, where the spectral
branch object has rank $\geq 3$ and similarity is not automatic
...
A natural candidate:
the $\operatorname{rank}(\mathfrak{g})$ Casimir operators
$C_1, \ldots, C_r$ act on each bar cohomology group
...
at degree~$n$ is controlled by the recurrence.  The
``Casimir companion'' matrix, recording how the isotypical
contributions to $H^n$ determine those to $H^{n+1}$, should
recover~$T_{\mathrm{rec}}$.
\end{remark}
...
the conjectural full characteristic hierarchy, whose H-level apex is
...
\begin{remark}[Degree-$2$ bar cohomology at lowest weight; \ClaimStatusOpen]\label{rem:bar-deg2-symmetric-square}
\index{bar complex!degree-2 cohomology}
At the \emph{lowest conformal weight} $h = 2$ (where all
generators are at mode~$1$), the bar complex of any affine
Kac--Moody algebra $\widehat{\mathfrak{g}}_k$ gives:
\[
\dim H^2(\barBgeom(\widehat{\mathfrak{g}}_k))_{h=2}
= \binom{\dim\mathfrak{g} + 1}{2}
= \dim S^2(\mathfrak{g}).
\]
The chain group $\bar{B}^2_{h=2} \cong \mathfrak{g}^{\otimes 2} \otimes \mathrm{OS}^1(C_2) \cong \mathfrak{g}^{\otimes 2}$ has dimension $(\dim\mathfrak{g})^2$.  The bar differential $d_2 \colon \bar{B}^2 \to \bar{B}^1$ encodes the Lie bracket (from the simple-pole OPE) and has $\mathrm{rank}(d_2) = \dim[\mathfrak{g},\mathfrak{g}] = \dim\mathfrak{g}$ (for semisimple~$\mathfrak{g}$).  Meanwhile, $\bar{B}^3_{h=2} = 0$ (three mode-$1$ generators have weight~$3$), so $\mathrm{im}(d_3) = 0$.  Thus $H^2_{h=2} = \ker(d_2) = (\dim\mathfrak{g})^2 - \dim\mathfrak{g} = \binom{\dim\mathfrak{g}+1}{2}$ (using $(a-1) + 1 = a$ for semisimple: $\ker = d^2 - d$, equal to $\binom{d+1}{2}$).  For $\mathfrak{sl}_2$: $H^2_{h=2} = 6 = \binom{4}{2}$.

\emph{Caution.}
This is the bar cohomology at a \emph{single} conformal weight.
The \emph{total} bar cohomology $\dim H^2 = \sum_h \dim H^2_h$
differs from $\binom{\dim\mathfrak{g}+1}{2}$ in general: the
PBW spectral sequence
(Corollary~\ref{cor:bar-cohomology-koszul-dual},
Remark~\ref{rem:ce-vs-exterior}) shows that higher-weight
contributions from the CE differential modify the total.
For $\widehat{\mathfrak{sl}}_2$, the CE computation gives
$\dim H^2 = 5$ (Computation~\ref{comp:sl2-ce-verification}),
with the weight-$2$ contribution of~$6$ reduced by~$1$
through the interaction with the weight-$3$ CE differential.
The precise mechanism by which the PBW spectral sequence
reconciles $H^2_{h=2} = 6$ (direct bar complex) with
$H^2_{\mathrm{CE}} = 5$ (PBW $E_2$ page) remains under
investigation.
\end{remark}
...
(Theorem~\ref{conj:betagamma-bar-dim}) gives the \emph{chain space}
