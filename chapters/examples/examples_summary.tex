...
\begin{conjecture}[$\widehat{\mathfrak{sl}}_3$ discriminant; \ClaimStatusConjectured]\label{conj:sl3-discriminant}
\index{sl3@$\mathfrak{sl}_3$!bar generating function}
The bar cohomology generating function $P_{\widehat{\mathfrak{sl}}_3}(x) = \sum_{n \geq 1} \dim H^n(\barBgeom(\widehat{\mathfrak{sl}}_{3,k}))\, x^n$ is an algebraic function of degree at most~$4$ over $\mathbb{Q}(x)$, with discriminant $\Delta_3(x)$ satisfying $\Delta_3(0) = 1$, leading coefficient $8^n$, and roots at $x = 1/8$ and (at least) one negative real root.  If the DS discriminant principle (Remark~\ref{rem:ds-discriminant-invariant}) holds, then $P_{\mathcal{W}_3}(x)$ shares the same branch locus.

\emph{Scope.} This requires computing $\dim H^4(\barBgeom(\widehat{\mathfrak{sl}}_3))$ (rank of a $24576 \times 786432$ matrix), which is beyond hand computation.  The first three values $8, 36, 204$ are consistent with an algebraic generating function but do not yet determine it uniquely.
\end{conjecture}
