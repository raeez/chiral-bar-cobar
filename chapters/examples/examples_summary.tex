...
\frac{F_1(\mathcal{W}^k(\mathfrak{g}))}{c} = \frac{\sigma(\mathfrak{g})}{24}
\end{equation}
where $\sigma(\mathfrak{g}) = \sum_{i=1}^r \frac{1}{m_i+1}$ is the anomaly ratio (the sum over exponents $m_1, \ldots, m_r$ of~$\mathfrak{g}$).  This is $1/2$ for $\mathfrak{sl}_2$ (Virasoro), $5/6$ for $\mathfrak{sl}_3$ ($\mathcal{W}_3$), and $H_N - 1$ for $\mathfrak{sl}_N$ ($\mathcal{W}_N$).  The identity follows from $F_1 = \kappa/24$ (Remark~\ref{rem:genus-1-verification}) and $\kappa = c \cdot \sigma(\mathfrak{g})$ (Theorem~\ref{thm:genus-universality}\textup{(ii)}).
\end{corollary}
...
For the exceptional Lie algebra $E_8$: $\dim = 248$, $h^\vee = 30$, $r = 8$.  The Master Table gives $c + c' = 2 \cdot 248 = 496$, which equals $2d$.  The Koszul conductor $K = 496$ is the same as the dimension of the $E_8 \times E_8$ gauge group in heterotic string theory.  From the genus universality theorem: $\kappa(\widehat{E}_{8,k}) = 62(k+30)/15$, and $\kappa + \kappa' = 0$ (since affine Kac--Moody).  The anomaly ratio is $\sigma(E_8) = \sum_{i=1}^8 1/(m_i+1)$ where the exponents are $(1,7,11,13,17,19,23,29)$:
\[
\sigma(E_8) = \frac{1}{2} + \frac{1}{8} + \frac{1}{12} + \frac{1}{14} + \frac{1}{18} + \frac{1}{20} + \frac{1}{24} + \frac{1}{30} = \frac{121}{126}.
\]
Note: for Kac--Moody algebras the anomaly ratio $\sigma(\mathfrak{g})$ is defined for the associated $\mathcal{W}$-algebra $\mathcal{W}(\mathfrak{g}, f_{\mathrm{prin}})$, not for $\widehat{\mathfrak{g}}_k$ itself; the Kac--Moody obstruction coefficient is $\kappa = (k+h^\vee)\dim\mathfrak{g}/(2h^\vee)$ directly.
\end{remark}
...
$\dim H^n = 3^n + 1$ (Conjecture~\ref{conj:yangian-bar-gf})
...
\item \emph{$\mathcal{W}_3$}: computed through degree~$3$
  ...
  $\dim H^4 = 52$ is obtained from the
  generators-and-relations analysis of the bar complex
  at conformal weights $h \leq 12$.
\item \emph{Yangian $Y(\mathfrak{sl}_2)$}: computed through
  degree~$3$ from the RTT presentation
  (Theorem~\ref{thm:yangian-bar-rtt}).
  The conjectured formula $\dim H^n = 3^n + 1$
...
\medskip
\noindent
{\footnotesize $^*$\,Conjectured: $\dim H^n = 3^n + 1$
for all $n \geq 1$
(Conjecture~\ref{conj:yangian-bar-gf}).\\
...
Among the Koszul chiral algebras in Table~\textup{\ref{tab:bar-dimensions}}, the bar cohomology $\dim H^n(\barBgeom(\cA))$ grows sub-exponentially if and only if $\cA$ is a free field (Heisenberg or free fermion).  All interacting algebras (Kac--Moody, Virasoro, $\mathcal{W}_3$, $bc$, $\beta\gamma$, Yangian) have exponential growth.
\end{corollary}
...
\begin{corollary}[Algebraicity of bar generating functions; \ClaimStatusProvedHere]
\label{cor:algebraicity-koszul}
\index{bar complex!generating function!algebraicity}
For every algebra in Table~\textup{\ref{tab:bar-dimensions}} with a known closed-form generating function, that function is algebraic over $\mathbb{Q}(x)$.  This algebraicity is a consequence of the Koszul property: the bar spectral sequence collapse at $E_2$ (Theorem~\ref{thm:spectral-sequence-collapse}) constrains the generating function to satisfy a polynomial equation.
\end{corollary}
...
\begin{remark}[Monodromy interpretation of the discriminant]\label{rem:discriminant-monodromy}
\index{discriminant!monodromy interpretation}
The shared discriminant $\Delta(x) = (1-3x)(1+x)$ admits a natural interpretation as a \emph{monodromy characteristic polynomial}.  The generating function $P(x)$ of bar cohomology is algebraic, so it satisfies a polynomial equation $Q(x, P) = 0$; the discriminant $\Delta(x) = \operatorname{disc}_P Q$ determines the branch locus.  The roots $x = 1/3$ and $x = -1$ of $\Delta$ are the singularities of a rank-two local system on the genus-one modular deformation complex.  In this framing, the DS-invariance of $\Delta$ (Theorem~\ref{thm:ds-bar-gf-discriminant}) reflects the fact that DS reduction preserves the reduced center and its Kodaira--Spencer transport, hence preserves the monodromy.  The eigenvalues $3$ and $-1$ (reciprocals of the roots) are the monodromy eigenvalues of a canonical genus-one transport operator acting on a rank-two piece of the modular deformation complex.  This perspective suggests that $\Delta_{\mathfrak{g}}(x)$ is an intrinsic invariant of the Koszul pair: $\Delta_{\mathfrak{g}}(x) = \det(1 - x \cdot \operatorname{KS}_{\mathfrak{g}} \mid H^{\mathrm{red}}_1(\mathcal{A}))$.
\end{remark}
...
\item The formula predicts $\dim H^4 = 1352$ and $\dim H^5 = 9892$.
...
the bar cohomology discriminant $\Delta_{\cA}(x)$ is an invariant of
the modular characteristic package.  Specifically:
\begin{enumerate}[label=\textup{(\roman*)}]
\item If the bar cohomology generating function
  $P_{\cA}(x) = \sum_{n \geq 0} \dim H^n(\barB(\cA))\, x^n$
...
$\dim H^n(\barB(\widehat{\fg}_k)) \leq (\dim \fg)^n \cdot (n-1)!$
...
Remark~\ref{rem:discriminant-monodromy} together suggest that the
...
\item If $d_{\mathrm{bracket}} \neq 0$ (bracket channel active),
  ...
  $\dim H^n \sim C \cdot 3^n \cdot n^{-3/2}$.
...
\item If $d_{\mathrm{bracket}} = 0$ (curvature-only), the generating function
  ...
  $\dim H^n \sim C' \cdot 3^n \cdot n^{-1/2}$.
...
For a Koszul algebra $\cA$, the bar cohomology dimensions $\dim H^n(\barBgeom(\cA))$ equal the dimensions of the weight-$n$ component of the Koszul dual $\cA^!$ (Theorem~\ref{thm:bar-cobar-isomorphism-main}).  The generating function $P_{\cA}(x) = \sum_n \dim H^n(\barBgeom(\cA))\, x^n$ is therefore the Hilbert series of $\cA^!$.  In the Kac--Moody case, $P_{\widehat{\mathfrak{g}}_k}(x)$ is the Hilbert series of $\widehat{\mathfrak{g}}_{-k-2h^\vee}$ as a graded algebra --- this is independent of~$k$ because the Koszul dual has the same graded structure at all generic levels.
\end{remark}
...
= \dim S^2(\mathfrak{g}).
\]
This holds because the bar differential $d_2 \colon \bar{B}^2 \to \bar{B}^1$ vanishes identically (the OPE singularity combined with the OS form $\eta_{12}$ produces at least a double pole, whose residue vanishes), and $\bar{B}^2 \cong \mathfrak{g}^{\otimes 2} \otimes \mathrm{OS}^1(C_2) \cong \mathfrak{g}^{\otimes 2}$ maps to $H^2 = \bar{B}^2/\mathrm{im}(d_3)$.  Explicit computation gives $\mathrm{rank}(d_3) = \dim\mathfrak{g}^{\otimes 2} - \binom{\dim\mathfrak{g}+1}{2} = \binom{\dim\mathfrak{g}}{2}$, so $H^2 = \binom{\dim\mathfrak{g}+1}{2}$.  Verified: $\mathfrak{sl}_2$ ($H^2 = 6 = \binom{4}{2}$), $\mathfrak{sl}_3$ ($H^2 = 36 = \binom{9}{2}$).
\end{remark}
...
For affine Kac--Moody algebras $\widehat{\mathfrak{g}}_k$, the exponential growth rate of bar cohomology dimensions satisfies
\[
\lim_{n \to \infty} \frac{\log \dim H^n(\barBgeom(\widehat{\mathfrak{g}}_k))}{\log n} = \dim \mathfrak{g}.
\]
More precisely, $\dim H^n \sim C \cdot (\dim \mathfrak{g})^n \cdot n^{-\alpha}$ for some $C > 0$ and $\alpha > 0$.  The verified cases are: $\dim \mathfrak{sl}_2 = 3$ (growth $3^n$), $\dim \mathfrak{sl}_3 = 8$ (growth $8^n$).
\end{corollary}

\begin{proof}
The bar chain groups have $\dim \bar{B}^n = (\dim \mathfrak{g})^n \cdot (n{-}1)!$ (Remark~\ref{rem:bar-dims-level-independent}).  The Koszul property constrains the bar \emph{cohomology} to grow at most as fast as the chain groups modulo the differential.  The generating function, being algebraic with radius of convergence $1/\dim\mathfrak{g}$ (by the DS discriminant, Theorem~\ref{thm:ds-bar-gf-discriminant} for $\mathfrak{sl}_2$), has exponential growth rate $(\dim \mathfrak{g})^n$.
\end{proof}
...
For any simple $\mathfrak{g}$, DS reduction cannot increase bar cohomology dimensions:
\[
\dim H^n(\barBgeom(\mathcal{W}^k(\mathfrak{g}))) \leq \dim H^n(\barBgeom(\widehat{\mathfrak{g}}_k))
\]
for all $n \geq 1$.  This follows from the BRST reduction: $H^n(\barBgeom(\mathcal{W})) = H^0_{\mathrm{DS}}(H^n(\barBgeom(\widehat{\mathfrak{g}})))$ is a subquotient.  The inequality is strict for $\mathfrak{sl}_2$: for example at $n = 3$, $\dim H^3(\barBgeom(\mathrm{Vir})) = 5 < 15 = \dim H^3(\barBgeom(\widehat{\mathfrak{sl}}_2))$.
\end{remark}
...
Each bar cohomology group $H^n(\barBgeom(\cA))$ carries a natural filtration by conformal weight: $F^{\leq h} H^n = \{[\alpha] \in H^n : \text{all generators in $\alpha$ have weight $\leq h$}\}$.  The associated graded $\operatorname{gr}^h H^n$ refines the single number $\dim H^n$ into a bigraded Hilbert series $\sum_{n,h} \dim \operatorname{gr}^h H^n \cdot x^n q^h$.  For the Virasoro algebra, the conformal weight grading on $H^n$ comes from the weight of the stress tensor $T$ (weight $2$) and its descendants, giving a $q$-refinement of the Motzkin difference sequence.
\end{remark}
