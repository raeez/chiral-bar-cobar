...
The bar differential $d \colon \bar{B}^2_{h=2} \to \bar{B}^1_{h=2}$
is the $(-1)$-st product (normally ordered product), \emph{not} the
$(0)$-th product (Lie bracket): it sends $J^a \otimes J^b$ to
$J^a_{-1}J^b_{-1}|0\rangle$.  Since
$J^a_{-1}J^b_{-1} - J^b_{-1}J^a_{-1} = f^{ab}{}_c\, J^c_{-2}$
(from the mode commutation relation with no central term at
$m + n = -2$), the nine products $J^a_{-1}J^b_{-1}|0\rangle$
($a,b \in \{e,h,f\}$) span all of~$\bar{A}_2$: the symmetric
combinations give $S^2(\mathfrak{g}_1)$ and the antisymmetric
ones give~$\mathfrak{g}_2$.  Thus $d$ is an isomorphism
($\mathrm{rank} = (\dim\mathfrak{g})^2$), and since
$\bar{B}^3_{h=2} = 0$, one obtains $H^2_{h=2} = 0$.

There is therefore no discrepancy with the PBW spectral sequence:
$H^2(\barBgeom(\widehat{\mathfrak{sl}}_2)) = 5$
(Computation~\ref{comp:sl2-ce-verification}), with all five classes
at weight~$\geq 3$.
\end{remark}
