...
Table~\ref{tab:master-invariants} records, for each Koszul chiral algebra~$\cA$ computed in this monograph, the Koszul dual~$\cA^!$, the central charge~$c(\cA)$, the level-independent sum $c(\cA) + c(\cA^!)$ under Koszul duality (Theorem~\ref{thm:central-charge-complementarity}), and the obstruction coefficient $\kappa(\cA)$ from the genus universality theorem $\operatorname{obs}_g(\cA) = \kappa(\cA) \cdot \lambda_g$ (Theorem~\ref{thm:genus-universality}).
...
All dualities in this table have complete proofs; see
Theorem~\ref{thm:yangian-koszul-dual} for the Yangian case.
...
Koszul dual column for $E_\infty$-chiral algebras (for the Yangian, an $\Eone$-chiral algebra, the analogous role is played by the $\Eone$-chiral Koszul duality theorem, Theorem~\ref{thm:e1-chiral-koszul-duality});
...
Conjecture~\ref{conj:discriminant-ks-operator} together suggest that the
