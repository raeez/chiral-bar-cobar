...
The governing question of this chapter is which invariants are actually computed across the example families, and how those shadows separate the proved modular core from still-open portrait data.
\end{remark}
...
recorded here; in the Virasoro row, the displayed partner
$\mathrm{Vir}_{26-c}$ is the proved M/S-level same-family shadow used in
the complementarity and semi-infinite calculations, while the stronger
H-level infinite-generator realization is delegated to the exact
...
On the current theorem surface, the first live higher-spin packets are
already explicit: stage-$3$ is a packet of fifteen primary
coefficients, reduced by the explicit $W_3$ OPE to three nonzero
entries and twelve forced zeros.

Stage-$4$ is cut by skew-symmetry to six live coefficients with
twenty-three forced zeros, organized as three local OPE blocks and
reduced further on the mixed higher-spin side to the single mixed
$(3,4)$ triple, whose $W^{(3)}$ target channel is swap-even and whose
$W^{(2)}$ and $W^{(4)}$ target channels are swap-odd under reversing
the mixed OPE.

On the principal Drinfeld--Sokolov side, the mixed $W^{(2)}$ target
vanishes, while the principal $W^{(4)}$-$W^{(4)}\to T$ coefficient is
fixed to `2'.  The live stage-$4$ comparison is therefore the exact
six-entry identity packet on~$\mathcal{I}_4$:
\[
...
\end{gathered}
\]
The first four principal targets are explicit up to the chosen
strong-generator signs, while the last two are fixed exactly by
\[
...
two-point/Ward package, equivalently if the H-level target is
stage-$4$ Ward-normalized in the sense of
Definition~\ref{def:winfty-stage4-ward-normalized}, then
...
boundary exactly: once a principal-stage compatible H-level target is
constructed, the comparison at each finite stage reduces to the seed
packet $\mathcal{I}_N$, and the first genuinely open higher-spin packet
is the displayed stage-$4$ six-entry identity packet on
$\mathcal{I}_4$.  Proposition~\ref{prop:w4-ds-ope-explicit} makes the
principal Drinfeld--Sokolov side explicit, so the live stage-$4$
question is no longer DS-side extraction but equality of the bar-side
residue channels with those explicit targets.
...
duality
\textup{(}Theorem~\ref{thm:yangian-koszul-dual}\textup{)} together with
the evaluation-locus factorization comparison
\textup{(}Theorem~\ref{thm:factorization-dk-eval}\textup{)} and the
evaluation-generated factorization core at all simple types
\textup{(}Corollary~\ref{cor:dk23-all-types}; for finite-dimensional
type~$A$, also Theorem~\ref{thm:dk-fd-typeA}\textup{)}.  The full
category-$\mathcal{O}$/KL completion and the RTT-complete H-level
comparison are not being claimed in this table.

The remaining Yangian frontier begins beyond that proved core: the
extension from the evaluation-generated core to the
ordinary-derived/completed/coderived enlargement and then DK-4/DK-5, the
RTT-complete H-level comparison and completed
module-category enhancement, detected by the identities
\[
\mathsf{K}^{\mathrm{line}}_{a,b}(N)
=\mathsf{K}^{\mathrm{RTT}}_{a,b}(N)
\]
on the boundary strip $\Delta_{a,0}(N)$.  For the standard type-A RTT
tower, Proposition~\ref{prop:yangian-dk4-typea-frontier} reduces that
analytic packet further to the auxiliary-kernel identity
$L_a(u)=R_{0a}(u-a)$ and hence, after the standard normalization, to
...
line.  Under one-loop exactness,
Corollary~\ref{cor:yangian-typea-degree2-plus-generators} sharpens this
local packet further to the single degree-$2$ mixed-tensor coefficient
of $r(z)=\tau|_{\deg 2}$ on the ordered fundamental line, while
Proposition~\ref{prop:yangian-dk5-compact-generators} reduces the
remaining completed module-category enhancement to the compact
generator packet.

The first faithful-evaluation boundary-strip
packets are already closed computationally:
$\Delta_{a,0}(4)$ and $\Delta_{a,0}(6)$ vanish on generic tensor
lengths $2$ and $3$ in ranks $2,3,4$, so the live Yangian frontier is
the uniform all-rank transport of this packet, not the first visible
low-stage defects.
Central charges, $c{+}c'$, and $\kappa$ are not defined for
$\Eone$-chiral algebras (Yangians) in the same sense as for
vertex algebras.

\begin{proposition}[Paired standard-tower MC4 frontier packets;
...
\item the first genuinely open standard-tower packets are:
  on the \texorpdfstring{$W_\infty$}{W_infty} side, the unconditional
  stage-$4$ identity packet is the six-entry block on~$\mathcal{I}_4$
  of Proposition~\ref{prop:winfty-mc4-frontier-package}, and under the
  stage-$4$ Ward-normalized H-level input of
  Definition~\ref{def:winfty-stage4-ward-normalized} it contracts to
  the four higher-spin channels of
  Corollary~\ref{cor:winfty-stage4-residue-four-channel}; the exact
  missing lemma for making that contraction unconditional is
  Conjecture~\ref{conj:winfty-stage4-ward-inheritance}; once the
  visible Virasoro Ward action is fixed,
  Proposition~\ref{prop:winfty-stage4-visible-pairing-gap} reduces its
  open content to the single visible weight-$4$ normalization,
  equivalently the scalar identity
  \(\mathsf{C}^{\mathrm{res}}_{4,4;2;0,6}(4)=2\), and
  Corollary~\ref{cor:winfty-stage4-single-scalar-equivalent}
  packages this as the exact theorematic form of the four-channel
  refinement; on the visible pairing locus,
  Corollary~\ref{cor:winfty-stage4-primitive-transport-square-triple}
  reduces the signless higher-spin packet to the primitive-plus-transport
  square triple, and
  ...
  square-class packet agrees with the principal side;
  Proposition~\ref{prop:winfty-stage-growth-virasoro-target-contraction}
  then gives the uniform reduced-packet contraction under the
  normalized residue package; the first next reduced stage is the
  eleven-entry packet $\mathcal{J}_5^{\mathrm{red}}$ of
  Corollary~\ref{cor:winfty-ds-stage5-reduced-packet}, and
  Corollary~\ref{cor:winfty-stage5-residue-eight-channel} and
  Corollary~\ref{cor:winfty-stage5-higher-spin-packet} identify the
  first next higher-spin packet $\mathcal{J}_5^{\mathrm{hs}}$ with
  eight channels, and
  Proposition~\ref{prop:winfty-stage5-higher-spin-subblocks} splits it
  as the four source-pair blocks $1+3+3+1$, while
  Corollary~\ref{cor:winfty-stage5-entry-transport} isolates the first
  ...
  Proposition~\ref{prop:winfty-stage5-reduced-tail-singleton}
  identifies $(3,4;5;0,2)$ as the entire reduced tail input at stage~$5$,
  while Proposition~\ref{prop:winfty-stage5-tail-mechanism} identifies
  its exact missing comparison mechanism as the $W^{(5)}$-projection in
  the top pole of $W^{(3)}(z)W^{(4)}(w)$,
  while
  Proposition~\ref{prop:winfty-stage5-higher-spin-target-blocks}
  regroups the same packet by target spin, and
  Corollary~\ref{cor:winfty-stage5-target5-corridor} identifies the
  first local three-channel strip as the target-$5$ corridor with first
  step $(3,4;5;0,2)$, while
  ...
  identifies that residual continuation as the comparison of the
  $W^{(5)}$-projection in $W^{(3)}(z)W^{(5)}(w)$ and
  $W^{(4)}(z)W^{(5)}(w)$, while
  Proposition~\ref{prop:winfty-stage5-target5-transport-singletons}
  splits that residual target-$5$ strip into the pole-$3$ singleton
  $(3,5;5;0,3)$ and the pole-$4$ singleton $(4,5;5;0,4)$, while
  Proposition~\ref{prop:winfty-stage5-visible-w5-normalization} makes
  the visible $W^{(5)}$ normalization theorematic under the stage-$5$
  Virasoro package, and on the visible pairing loci of
  ...
  vanishes, the pole-$4$ singleton is tied to the self-return
  singleton, and the tail singleton is tied to neighboring target-$4$/
  target-$3$ channels; Corollary~\ref{cor:winfty-stage5-target5-corridor-to-tail}
  kills the transport part on the visible $W^{(4)}$/$W^{(5)}$ pairing
  locus, and
  ...
  corridor carries no new independent coefficient; Propositions~\ref{prop:winfty-stage5-target4-pole5-w4-vanishing},
  \ref{prop:winfty-stage5-target3-pole5-w3-vanishing}, and
  \ref{prop:winfty-stage5-transport-cross-target-reduction} then
  collapse the remaining target-$4$/target-$3$ transport front to one
  ...
  `conj:winfty-stage5-principal-target5-transport-vanishing` and
  `conj:winfty-stage5-principal-tail-cross-target-reduction` form the
  target-`5` staircase, while
  `conj:winfty-stage5-principal-residual-front-vanishings` and
  `conj:winfty-stage5-principal-residual-cross-target-reduction` form
  the residual-front staircase, while
  Proposition~\ref{prop:winfty-stage5-principal-one-coefficient-factorization}
  packages their conjunction as
  Conjecture~\ref{conj:winfty-stage5-principal-one-coefficient-normal-form}, and
  ...
  \)
  of Conjecture~\ref{conj:winfty-stage5-one-coefficient-comparison}, and
  Corollary~\ref{cor:winfty-stage5-exact-remaining-input} packages the
  exact remaining local input as those two principal structural packets
  together with the target-$4$ singleton match,
  while
  Proposition~\ref{prop:winfty-stage5-transport-target-ladders}
  splits that mixed packet into the three fixed-target ladders, and
  Proposition~\ref{prop:winfty-stage5-transport-pole-profiles}
  identifies the target-$5$ ladder as the lowest-pole transport lane,
  and
  Conjecture~\ref{conj:winfty-stage5-higher-spin-identities} states the
  next finite bar-vs-DS identity list; on the
  Yangian side, the boundary-strip identity family
  \[
  \mathsf{K}^{\mathrm{line}}_{a,b}(N)
  =
  \mathsf{K}^{\mathrm{RTT}}_{a,b}(N),
  \]
  sharpened in the standard type-$A$ RTT tower by
  ...
  shows that this local coefficient is already closed, so the live
  local move is dg-shifted transport to that factorization coefficient.
  The H-level target itself already exists canonically as the
  formal-moduli tangent Lie algebra of
  ...
  \(U^{\mathrm{comp}}(\mathfrak{g}_{\cA})\); once that canonical dg
  model carries the RTT-adapted finite-quotient/shared-seed package,
  Proposition~\ref{prop:yangian-typea-realization-criterion}
  upgrades it to the standard RTT realization, and
  Corollary~\ref{cor:yangian-canonical-realization-to-spectral-seed}
  reduces the remaining canonical-target input to the standard
  spectral vector seed-and-shift datum; on the spectral vector-line
  locus,
  Corollary~\ref{cor:yangian-canonical-realization-plus-vector-line}
  closes DK-4/DK-5 on the canonical target, while on the sharper
...
\end{enumerate}
Thus the standard-tower MC4 frontier is already reduced, on both sides,
...
corresponding H-level infinite-generator dual object remains part of
MC4.
...
Throughout Part~XI, the dual abstract-concrete methodology was at work: on the abstract side, the $\infty$-categorical bar-cobar adjunction and operadic Koszul duality; on the concrete side, explicit generators and relations, computed differentials, and verified acyclicity.
...
\item In general, for a Koszul algebra, the exponential
  ...
  calculations; the corresponding H-level infinite-generator dual
  remains an MC4 completion problem.
\end{enumerate}
\end{remark}
...
(Computation~\ref{comp:sl2-ce-verification},
Remark~\ref{rem:bar-deg2-symmetric-square}).
...
\begin{conjecture}[DS preservation of the spectral branch object;
\ClaimStatusConjectured]\label{conj:ds-spectral-branch-preservation}
\index{Drinfeld--Sokolov reduction!spectral branch preservation}
\index{spectral branch object!DS invariance}
...
Then the branch transport operators are similar:
\[
T_{\mathrm{br},\mathcal{W}^k(\mathfrak{g})}
\;\sim\;
T_{\mathrm{br},\widehat{\mathfrak{g}}_k}.
\]
In particular, DS reduction preserves the full spectral branch
object $(V_{\cA}^{\mathrm{br}}, T_{\mathrm{br},\cA})$
\textup{(}Definition~\textup{\ref{def:spectral-branch-object})},
not merely its determinant~$\Delta_{\cA}(x)$
\textup{(}Theorem~\textup{\ref{thm:ds-bar-gf-discriminant})}.

\emph{Evidence.}
For $\mathfrak{g} = \mathfrak{sl}_2$, both
$T_{\mathrm{br},\widehat{\mathfrak{sl}}_2}$ and
$T_{\mathrm{br},\mathrm{Vir}}$ act on a $2$-dimensional
space with characteristic polynomial $1 - 2x - 3x^2
= (1-3x)(1+x)$; since the eigenvalues $3$ and $-1$ are distinct,
the matrices are similar unconditionally.

For $\mathfrak{g} = \mathfrak{sl}_3$ ($154$~computational tests),
the conjectured discriminants are
$\Delta_{\widehat{\mathfrak{sl}}_3}(x) = (1-8x)(1-3x-x^2)$
and $\Delta_{\mathcal{W}_3}(x) = (1-x)(1-3x-x^2)$.
These share the sub-discriminant $(1-3x-x^2)$ but differ in
the dominant eigenvalue ($8$ vs~$1$), so full
$T_{\mathrm{br}}$ similarity \emph{fails}.  The conjecture
should therefore be read as asserting preservation of the
\emph{DS-invariant sub-discriminant}, not full operator
similarity.  This refined formulation is consistent with
Theorem~\ref{thm:ds-bar-gf-discriminant}: DS preserves the
algebraicity class and the shared branch locus, while the
leading eigenvalue (equal to $\dim\mathfrak{g}$ for
$\widehat{\mathfrak{g}}$ and~$1$ for the vacuum
contribution in~$\mathcal{W}$) transforms under the
reduction.
\end{conjecture}
...
\begin{remark}[Degree-\texorpdfstring{$2$}{2} bar cohomology at lowest weight; \ClaimStatusProvedHere]\label{rem:bar-deg2-symmetric-square}
...
\noindent
The computations of Part~II are complete.  Part~III connects the bar-cobar framework to adjacent programs --- Feynman diagrams, BV-BRST, holomorphic-topological field theories, the 4d/2d correspondence --- and concludes with the concordance and the programme toward modular Koszul duality (\S\ref{sec:modular-koszul-programme}).
