...
\noindent\emph{Role in the route.} This summary is the portrait-layer
control table: it consolidates theorematic outputs of Part~II and
passes only stabilized invariants forward to the synthesis chapters.

\noindent\emph{The governing question is which invariants are genuinely
stabilized outputs of the portrait layer and therefore admissible as inputs to
the synthesis/control chapters.}

\begin{remark}[Semantic levels; Convention~\ref{conv:hms-levels}]
\label{rem:summary-hms}
This summary chapter is purely \textbf{S-level}: a tabulation of
numerical invariants (central charges, curvatures, discriminants,
bar dimensions) extracted from the M-level computations of the
preceding chapters.
\end{remark}
...
H-level infinite-generator realization remains on the MC4 frontier.
See Theorem~\ref{thm:yangian-koszul-dual} for the Yangian case.
...
\item[\textbf{T3.}] \emph{$\Eone$-regime} (Yangians):
  The $\Eone$-chiral Koszul duality theorem
  (Theorem~\ref{thm:e1-chiral-koszul-duality}) applies directly; the
  Virasoro-based invariants ($c$, $\kappa$) are not defined.
\end{enumerate}
...
the conjectural full modular characteristic package~$\mathcal{C}_{\cA}$
