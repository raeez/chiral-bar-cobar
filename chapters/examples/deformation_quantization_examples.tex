...
(Definition~\ref{def:pinf-chiral-detailed}): a homotopy chiral
Poisson lift carrying an $\Einf$-chiral product together with
$L_\infty$ chiral Lie operations linked by coherent Leibniz
homotopies. It is not a Poisson vertex algebra. The PVA/coisson
object is the strict descended shadow, while the $\Pinf$ object is
the upward homotopy lift used for quantization.
...
A \emph{$\Pinf$-chiral algebra} (Definition~\ref{def:pinf-chiral}) is
a homotopy chiral Poisson lift. In a strict model it carries the
following compatible chiral-operad data:
\begin{enumerate}[label=(\roman*)]
\item A $\chirCom$-algebra structure: commutative chiral product (i.e.\
 an $\Einf$-chiral algebra / vertex algebra).
\item An $L_\infty$ refinement of a $\chirLie$-algebra structure:
 chiral Lie operations whose binary bracket is written $\{-, -\}$.
\item Leibniz compatibility up to coherent homotopy; in the strict
 model this reads
...
Equivalently, $\mathcal{A}$ is an algebra over a cofibrant
$\Pinf$ replacement of the chiral Poisson operad; only in the
strict model does this reduce to
...
as $T_{(3)}T\,\lambda^3$ here, and as $(c/12)\lambda^3$ in
ordinary polynomial powers under the divided-power convention.
\end{remark}
...
\begin{theorem}[Deformation--duality compatibility; \ClaimStatusConditional]
...
bar coalgebra $\bar{B}(\mathcal{A})$ and its cohomology coalgebra,
and hence the Koszul dual algebra obtained from that cohomology
coalgebra, therefore
...
(iii) Under $H_D^K$, the signed Hodge outputs satisfy
\[
 \operatorname{obs}^{\mathrm{Hdg}}_g(\mathcal A)
 +\operatorname{obs}^{\mathrm{Hdg}}_g(\mathcal A^!)
 =(-1)^g\bigl(\kappa(\mathcal A)+\kappa(\mathcal A^!)\bigr)\lambda_g.
\]
Theorem~\ref{thm:quantum-complementarity-main} supplies the separate
Verdier-eigenspace decomposition of
$H^*(\mathcal M_g,\mathcal Z(\mathcal A))$.
\end{proof}
...
$\operatorname{hom}_\alpha(\cC^{\textup{!\textasciigrave}}, \dCrit(W))$ has
