...
The examples in this chapter are not decorative.  They are the proving
ground where formality, bar-cobar transport, higher operations, and
quantization encounter explicit coefficients and either survive or
fail.  Each example tests the deformation-quantization machinery of
the preceding chapter against a concrete chiral algebra: the classical
affine Poisson structure on $L\fg^*$ recovers
$\widehat{\fg}_k$ by quantization
(Example~\ref{ex:classical-affine}), the $\beta\gamma$-system exhibits
the first non-formal quantization with a quartic contact obstruction
(Corollary~\ref{cor:nms-betagamma-mu-vanishing}), and the Virasoro
coisson algebra produces the full tower of Kontsevich-type weights with
...
Since $\chirCom^! \cong \chirLie$, the two structures sit across
...
$\Pinf$-deformation complex, and its obstructions are computed by the
chiral Hochschild cohomology $\operatorname{ChirHoch}^*(\cA)$ of
Theorem~\ref{thm:chiral-hochschild}.  The examples below trace this
