...
The examples range from Coisson algebras (Poisson vertex algebras, the commutative starting point) through star products on lattice vertex algebras to symplectic fermion deformations, illustrating the full hierarchy $\mathrm{Coisson} \to \Einf \to \Eone$ and verifying the bar-cobar predictions in each case.

The governing question is: which concrete quantization examples already
realize the hierarchy
$\mathrm{Coisson} \to \Einf \to \Eone$ at theorem level, and which steps still
serve as programme-level templates for broader families.

\begin{remark}[Semantic levels; Convention~\ref{conv:hms-levels}]
\label{rem:deformation-examples-hms}
This chapter operates at \textbf{S-level}: explicit star products,
Coisson bracket formulas, and lattice/symplectic fermion deformation
data.  The quantization hierarchy
$\mathrm{Coisson} \to \Einf \to \Eone$ is an M-level construction
verified through these S-level shadows.
\end{remark}
...
\begin{theorem}[Coisson = $(\chirPois)^c$-coalgebra; \ClaimStatusProvedElsewhere{} \cite{BD04}]\label{thm:coisson-coalgebra}
...
\begin{theorem}[Formality for $\Pinf$-chiral; \ClaimStatusProvedElsewhere{} \cite{Kon03,FG12}]\label{thm:pinf-formality}
...
\emph{Bar complex interpretation.}
The bar complex $\bar{B}(V_L)$ has degree~1 generated by $\mathfrak{h} \oplus L$ (Heisenberg modes and lattice translations); in degree~2, the bar differential encodes both the Heisenberg OPE $a_i(z)a_j(w) \sim \langle e_i, e_j \rangle/(z-w)^2$ and the lattice vertex operator OPE; and the cocycle $\varepsilon$ appears in the signs of the bar differential: $d(s^{-1}e^\alpha \otimes s^{-1}e^\beta) = \varepsilon(\alpha,\beta) s^{-1}e^{\alpha+\beta}$.
...
Its Koszul dual is the $bc$ system at $\lambda = 1$.
For $\lambda = 1$: $c(bc) = 1 - 3(2\lambda - 1)^2 = -2$
and $c(\beta\gamma) = -1 + 3(2\lambda - 1)^2 = 2$, giving
$c + c' = 0$ $\checkmark$, consistent with the general
complementarity formula for the $bc$-$\beta\gamma$ duality.
