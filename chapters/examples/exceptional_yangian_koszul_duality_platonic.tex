...
Fix the numbering in which the Cartan matrices are
\[
...
\end{pmatrix}.
\]
Iterated simple reflections give
\[
\begin{array}{c|ccccc}
...
\begin{proof}
Starting from the simple roots, repeatedly apply the simple reflections
$s_i(\alpha_j)=\alpha_j-A_{ij}\alpha_i$ and retain the positive roots.
This gives the displayed finite sets $\Phi_+$.  Then
$\dim\fg=\operatorname{rk}\fg+2|\Phi_+|$.  Solving
$d_iA_{ij}=d_jA_{ji}$ gives the displayed minimal positive
symmetrizers, while direct permutation of the finite node sets gives
the diagram-automorphism orders.  Substitution of the standard dual
Coxeter numbers in the formula for $\kappa$ gives the final row.
\end{proof}

\begin{proposition}[Finite exceptional representation calculations;
\ClaimStatusComputed]
...
For the following decompositions, the dimension sums and Casimir
eigenvalues are:
\[
\begin{array}{c|c|c}
...
\end{array}
\]
No $F_4$ tensor-square decomposition is supplied here.  Thus the first
$F_4$ construction is a decomposition of
$V_{F_4}(\omega_4)^{\otimes2}$ together with explicit primitive
idempotents in $\End(V_{F_4}(\omega_4)^{\otimes2})$.
\end{proposition}

\begin{proof}
For each displayed highest weight, evaluate the finite Weyl product
$\prod_{\alpha\in\Phi_+}(\lambda+\rho,\alpha)/(\rho,\alpha)$.
On every listed summand, evaluate
$C_2(\lambda)=(\lambda,\lambda+2\rho)$ using the symmetrized Cartan
form.  The resulting dimensions sum to $(\dim V_0)^2$ in the four
listed tensor squares.  This calculation determines dimensions and
scalar eigenvalues only; the matrix projectors $P_\lambda$, transfer
matrices $T(u)$, and exceptional kernels $K_{\fg,\hbar}(u)$ remain
...
\section{Finite arithmetic and its boundary}
\label{sec:exceptional-yangian-verification}

The preceding tables use the following finite computations.
\begin{enumerate}[label=\textup{(\alph*)}]
\item Simple reflections enumerate the positive roots of the five
displayed Cartan matrices, and the equations
$d_iA_{ij}=d_jA_{ji}$ determine their symmetrizers.
\item The Weyl product evaluates the displayed $E_6$, $E_7$, $E_8$,
and $G_2$ dimensions.
\item The stated $E_6$, $E_7$, $E_8$, and $G_2$ decompositions give
their component dimensions and Casimir scalars; no $F_4$
decomposition is asserted.
\item For the permutation operator $P$ on $V_0^{\otimes2}$,
$(1-\hbar P/u)(1+\hbar P/u)=(1-\hbar^2/u^2)I$.
\item The finite node permutations give the displayed diagram
automorphism orders, and $(-I)^2=I$.
\item The classical dimension comparison is
Corollary~\ref{cor:exceptional-yangian-graded}.
\item The equality
$\sum_\lambda\dim V_\lambda=(\dim V_0)^2$ checks only the dimension
of a proposed tensor-square decomposition.  It is not the
Yang--Baxter equation, which appears separately as construction~\textup{(2)} in
Theorem~\ref{thm:exceptional-yangian-koszul-duality-all-five-types}.
\end{enumerate}

\begin{remark}[Scope of the finite calculations]
\label{rem:exceptional-yangian-hziv}
The arithmetic above is finite and exact.  The exceptional theorem has
