...
The governing question is: what appears in the first genuinely
non-abelian family that the Heisenberg frame cannot detect?  The
answer is the Feigin--Frenkel involution
$k \mapsto -k - 2h^\vee$ as Koszul duality together with Killing-form
transport of curvature and spectral data.  The Koszul dual of
$\widehat{\mathfrak{g}}_k$ is
$\widehat{\mathfrak{g}}_{-k-2h^\vee}$, realized by the curved chiral
Chevalley--Eilenberg coalgebra; the curvature
$\kappa(\widehat{\mathfrak{g}}_k)=\dim(\mathfrak{g})\cdot k/(k+h^\vee)$
controls the scalar genus tower (Theorem~D$_{\mathrm{scal}}$), and
the complementarity $\kappa + \kappa' = 0$ is the antisymmetry of the
Killing form.
...
Koszul structure (unconditional at all genera by
Theorem~\ref{thm:pbw-allgenera-km}), preserved by Drinfeld--Sokolov reduction.
...
\label{rem:km-status}
The results in this chapter fall into three regimes:
\begin{enumerate}
\item[\textbf{T2a.}] \emph{Generic level} ($k \notin \bQ_{\le 0}$):
  Koszulness proved via PBW spectral sequence
  (Theorem~\ref{thm:km-chiral-koszul}).  Duality, $\kappa$, and
  discriminant $\Delta$ follow unconditionally.
  Modular Koszulity at all genera is unconditional
  (Theorem~\ref{thm:pbw-allgenera-km}): the three-step
  mechanism---PBW decomposition, Whitehead acyclicity, Killing-form
  contraction---eliminates the $H^1(\Sigma_g)$ enrichment at every
  genus.
\item[\textbf{T2b.}] \emph{Admissible/root-of-unity level}:
  Periodic CDG/$N$-complex structure and identification with coderived
  categories are conjectural
  (contributing to Conjecture~\ref{conj:master-dk-kl}).
\item[\textbf{T2c.}] \emph{Representation category target}:
  The Kazhdan--Lusztig target is the semisimplified tilting
  category~$\cC(U_q(\fg))$, \emph{not} the full $\mathrm{Rep}(U_q(\fg))$.
  Full factorization-categorical equivalence is conjectural
...
\label{rem:km-hms}
This chapter operates at \textbf{S-level}: central charges, Riordan
numbers, level-rank data, and discriminant polynomials.  These are
extracted from \textbf{M-level} models (bar complexes, PBW spectral
sequences, screening operators).  H-level structures (factorization
categories, derived Kazhdan--Lusztig) are referenced as targets but
not constructed here.
\end{remark}

\begin{remark}[Portrait route]
\label{rem:km-route}
This chapter is the first non-abelian portrait and is organized to show
three features absent from the Heisenberg frame:
\begin{enumerate}[label=\textup{(\roman*)}]
\item non-abelian simple-pole transport through the Killing form;
\item Feigin--Frenkel level involution as concrete Koszul transport;
\item functorial passage to $\mathcal{W}$-algebras via DS reduction.
\end{enumerate}
\end{remark}
