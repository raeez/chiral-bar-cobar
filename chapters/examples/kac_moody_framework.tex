...
collapses at $E_1$, not $E_2$, reflecting the fact that the
...
In WZW models and Chern--Simons theory:
\begin{itemize}
\item \emph{Bulk-boundary correspondence}: open string modes on D-branes (level $k$) are dual to closed string modes in the bulk (level $-k-2h^\vee$).
\item \emph{Modular invariance}: characters transform under $k \to -k-2h^\vee$ via modular $S$-transformation.
\item \emph{Quantum groups}: the quantized enveloping algebra $U_q(\mathfrak{g})$ at $q = e^{\pi i/(k+h^\vee)}$ connects both sides.
\item \emph{Gauge theory}: level shifting appears in S-duality of 4d gauge theories compactified on circles.
\end{itemize}
\end{remark}
...
\item \emph{Geometric realization} \cite{Feigin-Frenkel90}. The bar complex computes de Rham cohomology of the affine flag variety:
\begin{equation}
...
\begin{enumerate}
\item \emph{Generator level}: identify the generating fields and their conformal weights.
\item \emph{OPE level}: compute operator product expansions as multi-residues on configuration spaces.
\item \emph{Relation level}: extract the quadratic (and higher) relations from OPE associativity.
\item \emph{Cohomology level}: verify the bar-cobar quasi-isomorphisms compute correct cohomology.
\end{enumerate}
\end{remark}
...
\begin{itemize}
\item \emph{Classical limit}: the theory admits a commutative chiral algebra structure.
\item \emph{Infinite-dimensional symmetry}: the center $Z(\widehat{\mathfrak{g}}_{-h^\vee})$ becomes infinite-dimensional.
\item \emph{Gauge theory connection}: corresponds to self-dual Yang--Mills theory.
\end{itemize}
\end{remark}
...
\begin{theorem}[Wakimoto for $\mathfrak{sl}_3$; \ClaimStatusProvedElsewhere]\label{thm:w3-wakimoto-sl3}
...
The principal case of this theorem is a manifestation of geometric Langlands duality:
\begin{itemize}
\item $\mathfrak{g} \leftrightarrow \mathfrak{g}^\vee$: Langlands dual Lie algebras
\item Nilpotent orbits: $f_{\mathrm{prin}} \leftrightarrow f^\vee_{\mathrm{prin}}$ under duality
\item W-algebras: quantum deformations of Slodowy slices in Langlands duality
\end{itemize}
\end{remark}
...
\begin{theorem}[Representation theory at admissible level; \ClaimStatusProvedElsewhere]
...
\begin{proposition}[DS reduction at admissible level; \ClaimStatusProvedElsewhere]
...
\begin{proposition}[Whittaker modules and DS reduction;
...
\item Periodicity of chiral Hochschild cohomology with period $2h$
...
terms of the derived oper resolution.  The full derived
identification~\eqref{eq:bar-derived-oper} would connect the
bar-cobar adjunction to the Frenkel--Gaitsgory programme for
geometric Langlands~\cite{FG06}.  The remaining gap is identifying
$H^n$ for $n \geq 2$ with higher cotangent cohomology of the oper
space, which requires methods from derived algebraic geometry
(Lurie~\cite{HA}).
\end{remark}
