...
\[
J^a(z) = \partial\phi^a(z) + \sum_{\alpha \in \Delta_+} :\beta_\alpha(z)\,[\rho^\alpha(e_a)\,\gamma^\alpha](z): + \cdots
\]
where $\rho^\alpha$ denotes the action of $e_a$ in the adjoint representation.  The single-pole residue $\mathrm{Res}_{z=w}[J^a(z)J^b(w)/(z-w)]$ comes entirely from the $\beta\gamma$-contractions; the Cartan boson $\partial\phi^a(z)\partial\phi^b(w)$ contributes only a double pole.  Each such contraction produces a factor $\beta_\alpha(w)$ times an operator in $\gamma^\alpha$: this is precisely the screening charge $S_\alpha(w) = e^{\alpha(\phi(w))}\prod_\beta \gamma^\beta(w)^{n_{\alpha,\beta}}$ (cf.\ the definition above).
...
\item Tensor structure: the KL equivalence is monoidal
  ($\boxtimes \leftrightarrow \otimes_q$); proving this via bar-cobar
  requires Conjecture~\ref{conj:fusion-bar-cobar} below.
...
The general fusion preservation conjecture
(Conjecture~\ref{conj:fusion-bar-cobar}) specializes to
...
\begin{conjecture}[Full derived oper identification;
\ClaimStatusConjectured]\label{conj:oper-bar}
...
Conjecture~\ref{conj:oper-bar}: our $H^0$ and $H^1$
...
the oper side of Conjecture~\ref{conj:oper-bar} is now
\emph{theorem}---the remaining gap is purely on the
bar-complex side: identifying $H^n(\barB(\widehat{\fg}_{-h^\vee}))$
with $\Omega^n(\mathrm{Op}_{\fg^\vee}(D))$ for all $n \geq 2$.
\end{remark}
...
building on the FLE of Raskin~\cite{Raskin24}, which makes the
connection from bar-complex cohomology to Langlands all the more
urgent: the target (the oper side) is established, and
Conjecture~\ref{conj:oper-bar} asks whether the bar complex
provides a \emph{vertex-algebraic} construction of this
derived geometry.  The remaining gap is identifying
$H^n$ for $n \geq 2$ with $\Omega^n(\mathrm{Op}_{\fg^\vee}(D))$,
which requires methods from derived algebraic geometry
(Lurie~\cite{HA}).
\end{remark}
...
\end{itemize}
These are the first two terms of the conjectural identification
$\barB(\widehat{\fg}_{-h^\vee})
\simeq \mathcal{O}^{\mathrm{der}}(\mathrm{Op}_{\fg^\vee}(D))$
(Conjecture~\ref{conj:oper-bar}), which asserts that the full
...
(Theorem~\ref{thm:e1-module-koszul-duality}) is expected to
preserve fusion products
(Conjecture~\ref{conj:fusion-bar-cobar}).  For Kac--Moody
...
appear in Conjecture~\ref{conj:fusion-bar-cobar}.
...
(Conjecture~\ref{conj:oper-bar}), and the admissible-level bar
