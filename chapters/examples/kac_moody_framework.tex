...
\item Finite-dimensionality: the bar cohomology at admissible
  level must be shown to be finite-dimensional (a consequence of the
  Arakawa--Frenkel vanishing theorem for admissible representations).
\end{itemize}
...
\begin{conjecture}[Oper space from bar complex;
...
Corollary~\ref{cor:critical-level-universality}) and provides a
chain-level resolution of the Feigin--Frenkel center:
...
Moreover, the full bar complex computes the derived space of opers:
...
The $H^0$ identification~\eqref{eq:bar-oper} is established:
Proposition~\ref{prop:km-generic-acyclicity} shows that at
$k = -h^\vee$ the uncurved bar complex has $H^0 = \mathfrak{z}$,
and the Feigin--Frenkel isomorphism
$\mathfrak{z} \cong \mathrm{Fun}(\mathrm{Op})$ is
\ClaimStatusProvedElsewhere{} \cite{Feigin-Frenkel, FG06}.

The full derived identification~\eqref{eq:bar-derived-oper} would
connect the bar-cobar adjunction to the Frenkel--Gaitsgory programme
for geometric Langlands~\cite{FG06}: the bar complex at critical
level provides an explicit chain-level model for the local geometric
Langlands correspondence.  The gap is identifying the higher bar
cohomology $H^n(\barB(\widehat{\fg}_{-h^\vee}))$ for $n \geq 1$
with the cotangent complex of the oper space, which requires methods
from derived algebraic geometry (Lurie \cite{HA}).
\end{remark}
