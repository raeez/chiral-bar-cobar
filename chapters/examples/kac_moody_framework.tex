...
for levels~$k$ and~$k'$, yields the parameter correspondence
...
\item \emph{Complementarity.}  The Koszul dual at level
  ...
  \textup{(}Theorem~\textup{\ref{thm:central-charge-complementarity})}.
  Since $k' + h^\vee = -3/2 < 0$, the dual level does not
  admit an admissible parametrization $k' = -h^\vee + p'/q'$
  with $p', q' > 0$.  The associated variety
