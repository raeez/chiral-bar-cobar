...
\]
for arbitrary $f$ (and the Barbasch--Vogan dual $f^\vee$) is conjectural
(\ClaimStatusConjectured): geometric Langlands considerations indicate this
extension, but proving it requires extending the Wakimoto-based proof to a
Beilinson--Bernstein localization framework; see
Theorem~\ref{thm:w-algebra-koszul-main} in Chapter~\ref{chap:w-algebra-koszul}
for the treatment of the principal case there.
\end{remark}
...
Accordingly, we record the following conjectural reformulation
(\ClaimStatusConjectured): the CDG periodicity
$\barB^{n+2q} \cong \barB^n$ of
Conjectures~\ref{conj:kl-periodic-cdg}--\ref{conj:kl-braided} can be reformulated as: at
