...
The affine Kac--Moody algebras $\widehat{\mathfrak{g}}_k$ are the first non-abelian examples: unlike the Heisenberg algebra of Chapter~\ref{ch:heisenberg-frame}, the simple poles in the OPE produce a nontrivial bracket contribution $d_{\mathrm{bracket}}$ to the bar differential, requiring the full PBW spectral sequence for computation.

The Feigin--Frenkel involution $k \mapsto -k - 2h^\vee$ has
appeared in many guises: as the duality between Wakimoto modules
and contragredient duals, as the relation between screening
operators at $k$ and at $k'$, as the symmetry underlying the
Kazhdan--Lusztig equivalence between affine representations and
quantum group modules.  This chapter identifies its source:
\emph{the Feigin--Frenkel involution is Koszul duality}.

The Koszul dual of $\widehat{\mathfrak{g}}_k$ is
$\widehat{\mathfrak{g}}_{-k-2h^\vee}$, and the bar construction
realizes the curved chiral Chevalley--Eilenberg coalgebra
$\mathrm{CE}^{\mathrm{ch},c}(\mathfrak{g}_{k'})$.  The
curvature $\kappa(\widehat{\mathfrak{g}}_k) =
\dim(\mathfrak{g}) \cdot k / (k + h^\vee)$ controls all
higher-genus corrections through the universal
Maurer--Cartan class~$\Theta_{\cA}$, and the complementarity
$\kappa + \kappa' = 0$ is the antisymmetry of the Killing form
under $k \leftrightarrow -k - 2h^\vee$.  Unlike free fields,
the Kac--Moody bar complex has chain groups of dimension
$(\dim\mathfrak{g})^n$ and cohomology governed by the
Chevalley--Eilenberg complex --- so the bar spectral sequence
collapses at $E_2$, reflecting the fact that the
OPE poles have order at most~$2$ (the ``almost quadratic''
property): $E_1$ carries nontrivial differentials from the bracket,
but $E_2$ stabilizes by the Koszul property.
...
The representation theory of the affine Kac--Moody algebra $\widehat{\mathfrak{g}}_k$ at level $k$ is related by Feigin--Frenkel duality to the theory at level $-k - 2h^\vee$, where $h^\vee$ is the dual Coxeter number.
...
The computation of the Koszul dual proceeds through four stages.
First, one identifies the generating fields and their conformal
weights.  Second, the operator product expansions are computed as
multi-residues on configuration spaces --- this is the geometric
content.  Third, the quadratic (and higher) relations are extracted
from OPE associativity, determining the bar differential.
Fourth, the bar-cobar quasi-isomorphisms are verified to compute the
correct cohomology.  We carry out this programme in complete detail
for $\widehat{\mathfrak{sl}}_2$ (the simplest nontrivial case,
already exhibiting the Virasoro connection via Drinfeld--Sokolov
reduction), for $\widehat{\mathfrak{sl}}_3$ (the first case in
which Serre relations and the two-dimensional weight lattice
intervene), and for general $\widehat{\mathfrak{g}}$ (a functorial
construction valid for any simple Lie algebra).
\end{remark}

\section{Affine Kac--Moody algebras: precise setup}

The affine Kac--Moody algebra $\widehat{\mathfrak{g}}_k$ is the
simplest chiral algebra built from a finite-dimensional Lie algebra:
one takes the loop algebra $\mathfrak{g} \otimes \mathbb{C}((t))$,
adjoins a central element whose eigenvalue is the level~$k$, and
discovers that the resulting structure encodes both representation
theory (via category~$\mathcal{O}$) and geometry (via the Sugawara
construction, which manufactures conformal symmetry from the Killing
form).  The reader familiar with the OPE formalism may skip to
\S\ref{sec:km-koszul-abstract}; the present section fixes conventions
and establishes the translation between vertex algebra and chiral
algebra languages that the bar complex computation requires.
...
Why does level-shifting preserve every structural feature of
$\widehat{\mathfrak{g}}_k$ --- its representation theory, its
conformal blocks, its quantum group shadow --- while reversing the
sign of the central extension?  The answer is that $k \mapsto -k -
2h^\vee$ is not a coincidence of formulas but a consequence of
Verdier duality on configuration spaces: the Serre dual of the
level-$k$ invariant form on $\overline{C}_{n+1}(X)$ is the
level-$(-k-2h^\vee)$ invariant form, with the shift $2h^\vee$
arising from the Casimir eigenvalue of the adjoint representation.
This section establishes the general pattern; the explicit
computations for $\widehat{\mathfrak{sl}}_2$ and
$\widehat{\mathfrak{sl}}_3$ in the preceding sections provide the
...
For the Heisenberg algebra, $\dim(\bar{B}^n) = 1$ at each degree (one generator, one form factor per degree; \S\ref{sec:frame-bar-all}).  For $\widehat{\mathfrak{sl}}_3$ the growth is polynomial in $\dim\mathfrak{g} = 8$:
...
The level-shifting duality has far-reaching consequences beyond its immediate scope: at the level of quantum groups it produces the $q \leftrightarrow q^{-1}$ symmetry (\S\ref{sec:km-quantum-groups} below), at critical level it recovers the Feigin--Frenkel center and touches geometric Langlands, and for the bar complex it explains why bar cohomology dimensions are generically level-independent (Theorem~\ref{thm:bar-cohomology-level-independence}).  The physical content of the level-shifting duality is anomaly cancellation: by Corollary~\ref{cor:anomaly-duality-km}, the semi-infinite anomalies of $\widehat{\fg}_k$ and $\widehat{\fg}_{-k-2h^\vee}$ cancel ($\kappa + \kappa' = 0$), and the complementarity theorem gives the full anomaly duality at all genera.
...
For the principal nilpotent $f = f_{\mathrm{prin}} = \sum_i e_{-\alpha_i}$ (sum of simple root lowering operators):
...
$\mathrm{Rep}(U_q(\mathfrak{g})) \to
\mathrm{Rep}(U_{q^{-1}}(\mathfrak{g}))$
on quantum group representations,
...
\mathrm{Rep}(U_q(\mathfrak{g}))$ preserves this tensor structure.
...
$|\mathrm{Irr}(\mathrm{Rep}(U_q(\mathfrak{sl}_2)))| = k+1$
...
(d^{(1)})^2 = (k+3) \cdot \omega_1 \cdot \operatorname{id}_{\mathfrak{sl}_3}
\end{equation}
...
All $8$ generators have conformal weight~$1$, giving a bounded-below exhaustive weight filtration.  The $E_1$ differential agrees with the genus-0 bar differential at leading order (since $K^{(1)}(z,w|\tau) = 1/(z-w) + O(1)$), computing $H^*(\mathfrak{sl}_3, V_k)$.
...
\begin{conjecture}[KL equivalence from bar-cobar;
\ClaimStatusConjectured]\label{conj:kl-from-bar-cobar}
...
periodic CDG structure with period dividing $2q$, and there is an
...
The equivalence is realized at chain level by the bar-cobar adjunction:
\begin{enumerate}[label=\textup{(\roman*)}]
...
\emph{What remains}:
\begin{itemize}
\item Root-of-unity bar analysis: at admissible $k$, the parameter
...
the curved dg structure ($d^2 = \kappa \cdot \omega_1$) with a truncated
...
Conjecture~\ref{conj:kl-from-bar-cobar} should be reformulated as: at
...
bar analysis of Conjecture~\ref{conj:kl-from-bar-cobar} and
...
\begin{proof}
At $k = -h^\vee$, $\kappa = 0$
(Corollary~\ref{cor:critical-level-universality}), so $d^2 = 0$
and the bar complex is a genuine cochain complex.  The PBW spectral
...
(Conjecture~\ref{conj:kl-from-bar-cobar}).  Does the full genus-graded
