...
The shadow class undergoes a transition
$\mathbf{L} \to \mathbf{G}$ at $k = 1$ in the simply-laced setting:
the lattice realisation makes the cubic shadow exact, collapsing
$r_{\max} = 3$ to $r_{\max} = 2$ \textup{(}see
Proposition~\ref{prop:level1-cubic-vanishing} and the class
transition table of
Remark~\ref{rem:level1-class-transition}\textup{)}. This is the
unique non-trivial cross-class transition in the standard
landscape that respects the quadrichotomy
$\{\mathbf{G},\mathbf{L},\mathbf{C},\mathbf{M}\}$ analysed in
Chapter~\ref{chap:shadow-quadrichotomy-platonic}; the
complementarity sum $\kappa^{\mathrm{KM}} + (\kappa^{\mathrm{KM}})' = 0$
inherited from the parent affine Kac--Moody family
\textup{(}canonical for affine KM, distinct from
$\kappa + \kappa' = 13$ for Virasoro and from
the conditional $\kappa + \kappa' = 25/3$ for
Bershadsky--Polyakov\textup{)} is the
canonical conductor value $K_{\mathrm{KM}} = 0$ of
...
H (chiral Hochschild) &
 Under a family datum $H_H(\widehat{\fg}_1;S_{\fg,1})$,
 $\operatorname{Supp}\ChirHoch^*(\widehat{\fg}_1)
 \subseteq S_{\fg,1}$; finite graded pieces give
 $P_{\widehat{\fg}_1}(t)=
 \sum_{n\in S_{\fg,1}}\dim H^n(K_{\widehat{\fg}_1,S_{\fg,1}})t^n$
 & Conditional
...
formula $\kappa=\dim(\fg)(k+h^\vee)/(2h^\vee)$ applies to the
...
\emph{independent} generators. This derivation
