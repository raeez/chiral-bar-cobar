...
Koszul algebra: the shadow Postnikov tower terminates at arity~$2$,
...
\textbf{Complementarity} & $\kappa + \kappa' = \kappa + (-\kappa) = 0$ \\
\textbf{$r$-matrix} & $r(z) = \kappa/z$ (single pole, no Lie bracket) \\
\textbf{Genus expansion} & $F_g = \kappa \cdot \lambda_g^{\mathrm{FP}}$,
GF $= (\kappa/\hbar^2)(\hat{A}(i\hbar) - 1)$
\end{tabular}
\end{center}
...
differential as genus corrections, and the Heisenberg is the simplest
non-trivial instance of Theorem~D: the general theory predicts that
$\kappa$ determines a universal generating function with
$\hat{A}$-genus shape, and this prediction can be verified term by
term against the Eisenstein expansion of the period matrix.

\section{Compute \texorpdfstring{$\Theta^{\mathrm{oc}}$}{Theta oc}:
bar complex, shadow tower, and modular characteristic}
...
\label{rem:heisenberg-eisenstein-master-mc}
For the Heisenberg algebra~$\cH_\kappa$, the shadow
Postnikov tower terminates at weight~$2$:
...
at arity~$r$ is a sum of graph amplitudes with $r$
...
trivially) to all higher arities:
$\Theta_{\cH_\kappa} = \Theta^{\leq 2} = \kappa$.
For the Heisenberg family, this one-channel Gaussian situation
lies on the proved scalar lane, so
$\Theta_{\cH_\kappa} = \kappa \cdot \eta \otimes \Lambda$
...
We now extract the five standard projections.
...
\kappa(\cH_\kappa) + \kappa(\cH_\kappa^!)
\;=\; \kappa + (-\kappa) \;=\; 0.
\end{equation}
...
(Theorem~\ref{thm:heisenberg-bar-complex-genus0}, \S\ref{sec:heisenberg-koszul-dual});
it has modular characteristic $\kappa(\cH_\kappa^!) = -\kappa$, giving
$\kappa + \kappa' = 0$.  This is the abelian case of
...
= \mathcal{H}_{-\kappa}$
(level negation by Verdier duality), giving
...
is the \emph{only} contribution to the shadow tower.
...
\end{equation}
where $K_{0,2}^{\mathcal{H}} = \kappa\,\delta^{ij}$ is the arity-$2$
...
The open-sector category $\Cop := \Perf(\cH_\kappa)$ of the
Heisenberg algebra carries a modular cooperad structure: a system of
cocomposition maps indexed by stable bordered graphs, satisfying
coassociativity.  This is the first example in which the full modular
cooperad completion programme
(concordance~\S\ref{sec:concordance-modular-cooperad-completion})
admits a complete proof.  The proof strategy: the one-particle
Bergman reduction (Theorem~\ref{thm:heisenberg-one-particle-sewing})
reduces the full Fock-space cooperad to an operator-algebraic
statement on the one-particle Bergman space~$A^2(D)$, where
coassociativity is the multiplicativity of Fredholm determinants and
the composition law of Bergman restriction operators.
...
\begin{theorem}[Modular cooperad structure on $\Cop(\cH_\kappa)$;
\ClaimStatusProvedHere]
...
carries a modular cooperad structure: the cocomposition maps
$\{\delta_\Gamma\}_\Gamma$ of
Construction~\textup{\ref{constr:heisenberg-cocomposition}},
indexed by stable bordered graphs, satisfy the following
properties.
\begin{enumerate}[label=\textup{(\Roman*)}]
\item \textbf{Well-definedness.}
  Each $\delta_\Gamma$ is a well-defined bounded linear map between
  the completed endomorphism spaces.
\item \textbf{Equivariance.}
  The maps $\delta_\Gamma$ are equivariant under the
  automorphism group $\operatorname{Aut}(\Gamma)$ of the
  bordered graph.
...
\item \textbf{Self-sewing.}
  ...
  \operatorname{End}(\cF_\lambda),
  \end{equation}
  where $\operatorname{Tr}_{\Cop}$ is the categorical trace on the
  dualizable category $\Cop(\cH_\kappa)$.
\item \textbf{Compatibility with the closed sector.}
  The closed projection
  $\pi_{\mathrm{cl}}\colon \delta_\Gamma(Z_g)
  \mapsto F_g = \kappa \cdot \lambda_g^{\mathrm{FP}}$
  recovers the genus tower of Theorem~D.
\item \textbf{Genus-$g$ partition function.}
  The partition function is the total trace of the cooperad:
  ...
  \end{equation}
  where $\mathcal{G}_{g,0}$ is the set of stable graphs of
  genus $g$ with $0$ legs.
\end{enumerate}
\end{theorem}
...
\medskip\noindent
\textit{Proof of~\textup{(I)}: well-definedness.}
...
composed operator at each vertex is trace class, and the cocomposition
map is well-defined as a bounded operator.

\medskip\noindent
\textit{Proof of~\textup{(II)}: equivariance.}
The sewing formula is manifestly symmetric in isomorphic graph
decorations: an automorphism $\sigma \in \operatorname{Aut}(\Gamma)$
permutes the internal edges and vertices, and the trace over
Fock states is invariant under permutation of the sewing channels
(since the trace is cyclically invariant and the Bergman operators
commute; see Step~2 below).  Hence $\delta_\Gamma$ descends to
the quotient by $\operatorname{Aut}(\Gamma)$.

\medskip\noindent
\textit{Proof of~\textup{(III)}: coassociativity.}
...
\medskip\noindent
\textit{Proof of~\textup{(IV)}: self-sewing = trace.}
...
\end{equation}
where the Hochschild homology of the semisimple category
...
\medskip\noindent
\textit{Proof of~\textup{(V)}: compatibility with the closed sector.}
...
= F_g(\cH_\kappa)$,
which is the genus tower of Theorem~D.
...
\medskip\noindent
\textit{Proof of~\textup{(VI)}: genus-$g$ partition function.}
...
\item \emph{Semisimplicity.}  The open-sector category
  ...
  degree~$0$.
\item \emph{Gaussian termination.}  The shadow tower terminates
  at arity~$2$ (Proposition~\ref{prop:heisenberg-gaussian-termination}),
...
$\Theta^{\mathrm{oc}}_\cA$ and the higher-arity shadow data
...
(Theorem~\ref{thm:heisenberg-modular-cooperad}(III))
