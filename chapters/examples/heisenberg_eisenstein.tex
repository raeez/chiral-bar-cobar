...
\textbf{Complementarity} \textup{(}Heisenberg is free-field, AP24 type~(i); distinct from Virasoro $13$\textup{)} & $\kappa_{\mathrm{Heis}}(\cH_\kappa) + \kappa_{\mathrm{Heis}}(\cH_\kappa^!) = 0$ \textup{(}free-field type~I\textup{)} \\
\textbf{$r$-matrix} & $r(z) = \kappa/z$ (single pole, no Lie bracket) \\
\textbf{Genus expansion} & $F_g = \kappa \cdot \lambda_g^{\mathrm{FP}}$ \textup{(UNIFORM-WEIGHT)},
...
\text{(free-field, AP24 type~I):}\qquad \kappa_{\mathrm{Heis}}(\cH_\kappa) + \kappa_{\mathrm{Heis}}(\cH_\kappa^!) \;=\; 0 \qquad \text{(not the Virasoro $13$).}
\end{equation}
...
the free-field pairing $\kappa_{\mathrm{Heis}} + \kappa_{\mathrm{Heis}}' = 0$ \textup{(}AP24 type~I, Heisenberg; not the Virasoro~$13$\textup{)}.
This is the abelian case of Theorem~\ref{thm:modular-characteristic}(iv).
...
maximally symmetric: both sides contribute equally
...
\end{equation}
which is independent of $\kappa$, a consequence of the universality factorization $F_g = \kappa \cdot \lambda_g^{\mathrm{FP}}$ \textup{(UNIFORM-WEIGHT)}.
\end{computation}
...
Specifically, for the Heisenberg algebra (free boson) at level $\kappa = 1$, the free energy at genus $g$ is \textup{(UNIFORM-WEIGHT)}:
\begin{equation}
...
\begin{table}[ht]
\centering
\caption{Free energy values $F_g = \lambda_g^{\mathrm{FP}}$ \textup{(UNIFORM-WEIGHT)} at $\kappa = 1$ for genera $1 \le g \le 10$}
...
so for the Heisenberg free-field family the pairing is $\kappa_{\mathrm{Heis}} + \kappa_{\mathrm{Heis}}' = 0$ \textup{(}AP24 type~I; not Virasoro~$13$\textup{)}.
...
The two are related by \textup{(UNIFORM-WEIGHT)}:
\[
...
\textup{(UNIFORM-WEIGHT; Heisenberg is single-generator)}:
...
= F_g(\cH_\kappa)$ \textup{(UNIFORM-WEIGHT)},
