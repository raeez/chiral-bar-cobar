...
The Heisenberg algebra~$\cH_\kappa$ is the Gaussian archetype: the
unique entry of the standard landscape that satisfies
$\kappa^{\mathrm{Heis}}(\cH_k) = k$ as a free parameter (not as a
function of a level shift), the unique entry whose modular Koszul
triple has zero quadratic complementarity sum
$\kappa^{\mathrm{Heis}}(\cH_k) + \kappa^{\mathrm{Heis}}(\cH_k^!) = 0$,
and the unique entry that lies in quadrichotomy class~G with shadow
depth $r_{\max} = 2$ (Theorem~\ref{thm:quadrichotomy},
Chapter~\ref{chap:shadow-quadrichotomy-platonic}). It is the simplest
modular Koszul algebra: the shadow obstruction tower terminates at
degree~$2$, the complementarity potential is exactly quadratic, and
the full open/closed MC element reduces to the scalar modular
characteristic~$\kappa^{\mathrm{Heis}}$. All five main theorems
(A through~D and~H) are immediate. The genus expansion is controlled
entirely by~$\kappa^{\mathrm{Heis}}$ and assembles into the
$\hat{A}$-genus.

The κ-value $\kappa^{\mathrm{Heis}}(\cH_k) = k$ is the
...
$-2k$ resolves $\cH_k$, so
$K(\cH_k) = -(-2k)/2 = k = \kappa^{\mathrm{Heis}}(\cH_k)$
(Corollary~\ref{cor:uc-K-heisenberg},
Chapter~\ref{chap:universal-conductor}). The seed
$\kappa^{\mathrm{Heis}}$ is verified against the master table of
Chapter~\ref{ch:landscape-census}, row Heisenberg
(\texttt{landscape\_census.tex}, equation~\eqref{eq:census-triples},
triple $\mathfrak{T}_{\cH} = (\cH_\kappa, \mathrm{Sym}^{\mathrm{ch}}(V^*), \kappa/z)$).
...
\section{Synthesis}
\label{sec:heisens-wave13-DNA}

\begin{remark}[Heisenberg-Eisenstein at the Borel cusp of $\bar{\mathcal A}_2^{\mathrm{BB}}$]
\label{rem:heisens-DNA-1}
The Heisenberg-Eisenstein construction, applied at the Wave~13 K3 base
point, produces the Gritsenko 1995 additive lift
\[
\Delta_5 \;=\; \mathrm{Grit}\bigl(\phi^{J,\mathrm{Eis}}_{5,1}\bigr) \;\in\; M_5^{\mathrm{cusp}}(\mathrm{Sp}_4(\mathbb Z),\mathrm{ch}),
\]
where $\phi^{J,\mathrm{Eis}}_{5,1}$ is the Jacobi-Eisenstein series of
weight $5$ and index $1$, and $\mathrm{Grit}$ is the Gritsenko additive
lift raising a Jacobi form to a Siegel modular form via Fourier-Jacobi
expansion. At the Borel cusp of $\bar{\mathcal A}_2^{\mathrm{BB}}$
(the class-G Heisenberg chamber in Theorem~C at Wave~13), the Wave~13
chiral bialgebra $\mathbf H_{\Delta_5}$ degenerates to the Heisenberg
algebra $\mathcal H(\Lambda_{\mathrm{Muk}})$ on the Mukai lattice, with
$\kappa_{\mathrm{ch}} = \mathrm{rk}(V_+) = 4 = c_+(\mathrm{Mukai}(K3))$.
This Heisenberg degeneration is the Eisenstein part of the Arthur-Hecke
decomposition $L(s,\Phi_{10}) = L(s,\Delta\otimes E_6)\cdot\zeta(s-9)\cdot\zeta(s-8)$,
with the two $\zeta$-shifts corresponding to the two Heisenberg-dual
directions in the Eisenstein-modular spectrum. Primary-lit: Gritsenko
1995, Eichler-Zagier 1985 (Jacobi forms), Langlands-Shahidi theory.
\end{remark}
