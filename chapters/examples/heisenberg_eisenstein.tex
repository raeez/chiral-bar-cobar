...
$F_g(\cH_\kappa)=F_g^{\mathrm{sc}}(\cH_\kappa)
= \kappa(\cH_\kappa)\cdot\lambda_g^{\mathrm{FP}}$,
$\delta F_g^{\mathrm{cross}}(\cH_\kappa)=0$; GF
$= (\kappa(\cH_\kappa)/\hbar^2)(\hat{A}(i\hbar) - 1)$
\end{tabular}
\end{center}
...
Theorem~D: the modular characteristic $\kappa(\cH_\kappa)$ determines
the scalar genus tower, and Gaussian exactness makes it the full
genus tower:
$F_g(\cH_\kappa)=F_g^{\mathrm{sc}}(\cH_\kappa)
=\kappa(\cH_\kappa)\lambda_g^{\mathrm{FP}}$ and
$\delta F_g^{\mathrm{cross}}(\cH_\kappa)=0$.
...
dependence. This is exact Gaussianity in the scalar shadow.
\end{proof}
...
$F_g(\cH_\kappa)=F_g^{\mathrm{sc}}(\cH_\kappa)
= \kappa(\cH_\kappa) \cdot \lambda_g^{\mathrm{FP}}$ and
$F_g(\cH_\kappa^!)=F_g^{\mathrm{sc}}(\cH_\kappa^!)
= \kappa(\cH_\kappa^!) \cdot \lambda_g^{\mathrm{FP}}$,
...
\begin{equation}
F_2(\mathcal{H}_\kappa)=F_2^{\mathrm{sc}}(\mathcal{H}_\kappa)
= \kappa(\mathcal{H}_\kappa) \cdot \frac{2^3-1}{2^3} \cdot \frac{|B_4|}{4!}
= \kappa(\mathcal{H}_\kappa) \cdot \frac{7}{8} \cdot \frac{1/30}{24}
= \frac{7\kappa(\mathcal{H}_\kappa)}{5760}
= \frac{7\kappa}{5760}
\end{equation}
...
= \frac{7}{240}
\end{equation}
which is independent of $\kappa$, a consequence of the universality factorization
$F_g(\mathcal{H}_\kappa)=F_g^{\mathrm{sc}}(\mathcal{H}_\kappa)
...
$\kappa E_2(\tau)$, hence carries the standard quasimodular
$E_2$ anomaly. For $g=2$, scalar
...
\frac{|B_{2g}|}{(2g)!}.
\end{equation}
For the Heisenberg Gaussian lane,
$\delta F_g^{\mathrm{cross}}(\mathcal H_\kappa)=0$, hence
$F_g(\mathcal H_\kappa)=F_g^{\mathrm{sc}}(\mathcal H_\kappa)$.
...
\begin{table}[ht]
\centering
\caption{Heisenberg scalar/full free energy values
$F_g=F_g^{\mathrm{sc}}=\lambda_g^{\mathrm{FP}}$ at $\kappa = 1$
for genera $1 \le g \le 10$}
...
The scalar/full free energy at genus~2 is:
\[
F_2(\mathcal{H}_\kappa)=F_2^{\mathrm{sc}}(\mathcal{H}_\kappa)
 = \kappa(\mathcal H_\kappa)
   \cdot \int_{\overline{\mathcal{M}}_{2,1}}\psi_1^2 \lambda_2
 = \kappa(\mathcal H_\kappa)\cdot \frac{7}{5760}
 = \kappa\cdot \frac{7}{5760}
\]
(Theorem~\ref{thm:heisenberg-all-genus}).
...
F_g^{\mathrm{sc}}(\mathcal H_\kappa)
 = \kappa(\mathcal H_\kappa)
   \cdot \int_{\overline{\mathcal{M}}_{g,1}}\psi_1^{2g-2} \lambda_g
\]
...
\begin{definition}[Shadow jets on a cyclic slice; \ClaimStatusDefinitional]
...
(The Heisenberg spectral $R$-matrix $R(z)=z^{2h}$ is the Gaussian
rank-$1$ analogue of the primary-state Virasoro Ward factor
$R^{\mathrm{prim}}_{h,c}(z)=z^{2h}\exp(-(c/4)/z^2)$ of
Computation~\ref{comp:virasoro-spectral-r-matrix}.)
\end{corollary}
...
number; the equality \(F_g=F_g^{\mathrm{sc}}\) is the vanishing of the
Heisenberg cross-channel summand. The first values are:
\begin{align}
 \label{eq:heisenberg-F1-shadow}
 F_1=F_1^{\mathrm{sc}}
 &=
 \kappa(\mathcal H_\kappa)\cdot \frac{1}{2} \cdot \frac{|B_2|}{2!}
 \;=\; \kappa(\mathcal H_\kappa)\cdot \frac{1}{2} \cdot \frac{1/6}{2}
 \;=\; \frac{\kappa}{24},\\[4pt]
 \label{eq:heisenberg-F2-shadow}
 F_2=F_2^{\mathrm{sc}}
 &=
 \kappa(\mathcal H_\kappa)\cdot \frac{7}{8} \cdot \frac{|B_4|}{4!}
 \;=\; \kappa(\mathcal H_\kappa)\cdot \frac{7}{8} \cdot \frac{1/30}{24}
 \;=\; \frac{7\kappa}{5760},\\[4pt]
 \label{eq:heisenberg-F3-shadow}
 F_3=F_3^{\mathrm{sc}}
 &=
 \kappa(\mathcal H_\kappa)\cdot \frac{31}{32} \cdot \frac{|B_6|}{6!}
 \;=\; \kappa(\mathcal H_\kappa)\cdot \frac{31}{32} \cdot \frac{1/42}{720}
...
\begin{equation}\label{eq:heisenberg-ahat-from-shadows}
 ...
 \;=\;
 \sum_{g \ge 1} F_g^{\mathrm{sc}}(\mathcal{H}_\kappa)\,\hbar^{2g-2}
 \;=\;
 \frac{\kappa(\mathcal H_\kappa)}{\hbar^2}
...
(Theorem~\ref{conj:nms-nonlinear-phase-standard}), the Heisenberg
...
\pi_{\mathrm{cl}}(\delta_\Gamma(Z_g))
\;=\;
\xi^*_\Gamma(F_g^{\mathrm{sc}}(\cH_\kappa)),
\qquad
F_g^{\mathrm{sc}}(\cH_\kappa)
=\kappa(\cH_\kappa) \cdot \lambda_g^{\mathrm{FP}},
\]
