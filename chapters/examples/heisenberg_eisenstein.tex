...
This is absolutely convergent and has quasiperiodicity $\zeta(z+1) = \zeta(z) + 2\eta_1$, $\zeta(z+\tau) = \zeta(z) + 2\eta_2$, where $\eta_1 = \pi^2 E_2(\tau)/3$ is the quasi-period.
...
For the free boson:
$$\omega_g = \lambda_g$$
where $\lambda_g = c_g(\mathbb{E})$ is the top Chern class of the Hodge bundle (not to be confused with $c_1(\mathbb{E})^g$, which is a different class for $g \geq 2$).
