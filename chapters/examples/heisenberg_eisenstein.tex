...
The governing question of this chapter segment is: how the theorem-level
Heisenberg genus tower becomes an explicit modular-form computation that can be
reused as a calibration model for non-abelian families.

\begin{remark}[Semantic levels; Convention~\ref{conv:hms-levels}]
\label{rem:heisenberg-eisenstein-hms}
This chapter operates at \textbf{S-level}: Eisenstein series expansions,
genus-$g$ free energies, and the $\hat{A}$-genus generating function.
These are extracted from \textbf{M-level} models (the Heisenberg bar
complex and its genus tower).
\end{remark}
