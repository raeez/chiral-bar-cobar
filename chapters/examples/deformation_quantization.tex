...
\]
Then the
scalar genus expansion of the bulk partition function maps to the
...
higher-genus extension. The proof obligation in
...
The remaining proof obligation is the chain-level bridge from those
...
$H^2(\mathfrak g_{\Delta_5})^{\Z/2,\,K(1)}\cong\bC\cdot\Delta_5$
(Remark~\ref{rem:defq-1}) names the first-order line. The added
rigidity hypothesis kills the higher torsor ambiguity, so equal
first-order classes determine the same formal gauge class. No analytic
specialization at \(\hbar^2=-1/8\) is used in this formal limit.
\end{proof}
...
\texorpdfstring{$\mathbf H_{\Delta_5}$}{H Delta 5}; \ClaimStatusProvedHere]
