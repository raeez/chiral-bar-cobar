...
Weight: $w_{\Gamma_3} = -\frac{1}{12}$ (by the orientation reversal under $a \leftrightarrow b$)
...
$$a \star b = \sum_{g=0}^\infty \hbar^{2g} \star^{(g)}(a,b)$$
where $\star^{(g)}$ involves integration over $\overline{\mathcal{M}}_{g,2}$ (genus-$g$ surfaces with two marked points for the insertions $a$ and $b$).
\end{theorem}

\begin{proof}
By the modular operad decomposition (Theorem~\ref{thm:prism-higher-genus}), the bar complex of the quantized chiral algebra decomposes by genus: $\B(\cA_\hbar) = \bigoplus_{g \geq 0} \B^{(g)}(\cA_\hbar)$.  The star product $a \star b$ is computed by the bar-cobar formalism applied to two insertions, and the genus-$g$ contribution is $\star^{(g)}(a,b) = \int_{\overline{\mathcal{M}}_{g,2}} \mathrm{ev}^*(\B^{(g)}_2(a \otimes b))$.  The power $\hbar^{2g}$ arises because each handle of the genus-$g$ surface contributes $\hbar^2$ from the propagator (the Planck constant enters the chiral bracket at order~$\hbar$, and a genus-$g$ surface has $g$ handles, each contributing two propagator insertions).
\end{proof}
