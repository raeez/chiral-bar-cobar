...
Deformation quantization reverses the question posed by the preceding chapters: rather than computing the bar complex of a \emph{given} chiral algebra, one starts from a \emph{classical} (commutative) structure and constructs a chiral algebra by deformation.  Kontsevich's formality theorem accomplishes this for Poisson manifolds via configuration space integrals --- precisely the same configuration spaces that define our bar complex.  The bar-cobar machinery provides the homotopical framework in which Kontsevich's star product formula is the genus-$0$ shadow of a chiral quantization defined at all genera.

\noindent\emph{Governing question.} Can the same configuration-space
machinery serve both analysis directions: extracting invariants from a
given chiral algebra and building a chiral algebra from classical data?
In this chapter, genus-$0$ quantization statements are theorematic;
higher-genus extension claims are explicitly tagged as open.

\begin{remark}[Semantic levels; Convention~\ref{conv:hms-levels}]
\label{rem:dq-hms}
This chapter operates at \textbf{M-level}: the chiral Kontsevich
construction, formality quasi-isomorphisms, and configuration space
integral models are explicit dg/bar-complex constructions.
\textbf{S-level} shadows include star product coefficients and
graph weight computations.  H-level structures ($\Einf$-formality,
derived deformation theory) provide the conceptual framework.
\end{remark}
...
The deformation quantization problem asks for a noncommutative deformation of $C^\infty(M)$ governed by a Poisson structure.
...
\begin{theorem}[Coisson quantization at genus $0$; \ClaimStatusProvedHere]\label{thm:chiral-quantization}
...
\begin{theorem}[Bar complex computes deformation \cite{LV}; \ClaimStatusProvedElsewhere]
...
\begin{proposition}[$A_\infty$ operations \cite{Kon03}; \ClaimStatusProvedElsewhere]
...
\[
HH^*_{\mathrm{ch}}(\mathcal{A}_{\mathrm{cl}})[\hbar]
\;\leftrightarrow\;
...
The conjecture identifies the bar complex genus expansion with the bulk gravity path integral in $\mathrm{AdS}_3$, translating configuration space integrals into holographic free energies.
\end{remark}
...
\begin{computation}[Complete $\hbar^2$ weight verification]
...
\]
Kontsevich's computation~\cite{Kontsevich03} evaluates this via
...
\begin{computation}[Graph enumeration at $\hbar^3$]
...
\begin{computation}[Heisenberg chiral star product through $\hbar^3$]
...
The genus-1 curvature $m_0(\widehat{\fg}_k) = (k+h^\vee)\kappa/(2h^\vee)$
(Proposition~\ref{prop:km-bar-curvature}) has a deformation-theoretic
interpretation: it measures the failure of the classical star product
to extend from the formal disk to the elliptic curve $E_\tau$.

In the deformation quantization language:
\begin{itemize}
\item $m_0 = 0$ (critical level $k = -h^\vee$):
the star product extends to all genera without obstruction.
The algebra is ``classically integrable'' in the sense that
the Poisson structure on $\fg[z]$ lifts to a flat connection
on the moduli of bundles.
\item $m_0 \neq 0$ (generic level):
the genus-1 extension requires a curved $A_\infty$ correction,
and the star product on $E_\tau$ involves the Eisenstein series
$E_2(\tau)$ (Theorem~\ref{thm:elliptic-vs-rational}).
The higher-genus corrections involve modular forms of higher weight.
\end{itemize}
\end{remark}
...
\begin{enumerate}
\item \emph{Higher genus formality.} Does Kontsevich formality extend to $\overline{\mathcal{M}}_{g,n}$ for $g \geq 2$?

\item \emph{Infinite-dimensional algebras.} Can we quantize Virasoro using these methods?

\item \emph{Quantum groups.} How does this relate to Drinfeld's quantum group quantization?

\item \emph{Topological recursion.} Connection to Eynard--Orantin recursion?

\item \emph{3d Chern--Simons.} Can we realize 3d TQFTs via 2d chiral algebra quantization?
\end{enumerate}
