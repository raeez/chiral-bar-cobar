...
\item The KZ coefficient is the separate rational factor
 ...
 \]
 It is not the affine level, and it is not the trace-form collision
 residue \(k\Omega_{\mathrm{tr}}/z\).  The conversion between
 \(r_{\mathrm{coll}}\) and \(r_{\mathrm{KZ}}\) uses the scalar
 normalisations of Appendix~\ref{sec:scalar-normalization-appendix}.
...
Fix a quadratic presentation $A=T_X(V)/(R)$.  The deformation complex
above separates the following typed objects:
\[
A,\qquad \barB(A),\qquad
A^{\mathrm i}=C_X(s^{-1}V,s^{-2}R),\qquad
H^\bullet\barB(A),\qquad
K_X(A)=\mathbb D_{\Ran}\barB(A),\qquad A^!,\qquad
...
shifted deformation dg Lie algebra, and
\[
q_A\colon A^{\mathrm i}\to\barB(A),\qquad
\mathbb D(q_A)\colon K_X(A)\to\mathbb D_{\Ran}(A^{\mathrm i}),
\qquad
\nu_A\colon K_X(A)\to A^!
\]
are respectively the quadratic, dual quadratic, and Koszul-pair
comparisons.  On the Koszul locus $H^\bullet(q_A)$ identifies
presentation-coalgebra cohomology with bar cohomology. The derived chiral centre
\[
Z^{\mathrm{der}}_{\mathrm{ch}}(A)
\simeq C^\bullet_{\mathrm{ch}}(A,A)
\]
is the Hochschild closed-sector object. Maurer--Cartan quantization
lives in coderivations of~\(\barB(A)\).  The counit
\(\Omega(\barB(A))\to A\) records bar--cobar inversion, while
\(K_X(A)\) records Verdier duality.
\end{remark}
...
Let \(\alpha\) be an augmentation-preserving descended
Maurer--Cartan lift and let
\(\mathcal A_\hbar\) be the resulting complete augmented filtered
chiral algebra.  Its full completion-aware coalgebra is
\(B_X(\mathcal A_\hbar)\).  In the pro-nilpotent chiral Ran ambient,
under the hypothesis package
\(H_1=H_{\mathrm{fact}}\cup H_{\mathrm{conv}}\),
Theorem~A \textup{(}Theorem~\ref{thm:mr-A}\textup{)} supplies the
universal reconstruction counit
\[
 \Omegach_X B_X(\mathcal A_\hbar)
 \xrightarrow{\;\sim\;}\mathcal A_\hbar.
\]

A chosen complete quadratic presentation
\(\mathcal A_\hbar=T_X(V)/(R)\) supplies the quadratic coalgebra and
comparison
\[
 \mathcal A_\hbar^{\mathrm i}=C_X(s^{-1}V,s^{-2}R),
 \qquad
 q_{\mathcal A_\hbar}\colon
 \mathcal A_\hbar^{\mathrm i}\longrightarrow B_X(\mathcal A_\hbar).
\]
Theorem~B \textup{(}Theorem~\ref{thm:mr-B}\textup{)} recognizes the
quadratic Koszul locus by the quasi-isomorphism property of this map,
equivalently by the adjoint map
\(\Omega_X(\mathcal A_\hbar^{\mathrm i})\to\mathcal A_\hbar\) and
the two acyclic twisted tensor products, under
\(H_{\mathrm{CL}}(\mathcal A_\hbar,
\mathcal A_\hbar^{\mathrm i},\tau_{\mathrm i})\).  This is the
quadratic operadic lane of~\cite{GLZ22}.  On the dualizable Ran locus,
Verdier duality forms the algebra
\(\mathcal A_\hbar^!=\mathbb D_{\Ran}
(\mathcal A_\hbar^{\mathrm i})\).

For a fixed complete chiral CDG-coalgebra~\(C\), the Positselski
fixed-coalgebra lane~\cite{Positselski11} and the package
\(\mathsf{Pos}^{\mathrm{ch}}_{\mathrm{co-ctr}}(C)\) supply the
second-kind co--contra equivalence of
Proposition~\ref{prop:mr-fixed-C-second-kind}.  A comparison from~\(C\)
to an algebra~\(A\) through a twisting morphism~\(\tau\colon C\to A\)
uses the additional package
...
\begin{remark}[Typed theorem interfaces for deformation quantization;
\ClaimStatusConditional]
\label{rem:deformation-three-theorems}
\index{main theorems!deformation quantization}
Choose an augmentation-preserving descended Maurer--Cartan
lift~\(\alpha\) and write
\(\mathcal A_\hbar\) for its complete augmented filtered chiral
algebra.  The local Kontsevich--chiral comparison
\textup{(}Theorem~\ref{thm:chiral-kontsevich}\textup{)} identifies the
disk boundary-factorisation identities defining~\(\alpha\) with the
corresponding components of the completed chiral bar coderivation.
The geometric--operadic comparison
\textup{(}Theorem~\ref{thm:geometric-equals-operadic-bar}\textup{)}
transports this disk model to the operadic bar construction.  In the
pro-nilpotent chiral Ran ambient, under
\(H_1=H_{\mathrm{fact}}\cup H_{\mathrm{conv}}\), Theorem~A
\textup{(}Theorem~\ref{thm:mr-A}\textup{)} then
gives the completion-aware universal reconstruction
\[
 \Omegach_X B_X(\mathcal A_\hbar)
 \xrightarrow{\;\sim\;}\mathcal A_\hbar.
\]

For a chosen complete quadratic presentation, Theorem~B
\textup{(}Theorem~\ref{thm:mr-B}\textup{)} recognizes its Koszul locus
through
\[
 q_{\mathcal A_\hbar}\colon
 \mathcal A_\hbar^{\mathrm i}\longrightarrow B_X(\mathcal A_\hbar)
...
and the package
\(H_{\mathrm{CL}}(\mathcal A_\hbar,
\mathcal A_\hbar^{\mathrm i},\tau_{\mathrm i})\).
The higher-genus inversion theorem
\textup{(}Theorem~\ref{thm:higher-genus-inversion}\textup{)} supplies
the genus-wise modular realization of the reconstruction counit of
Theorem~A under the modular pre-Koszul package~\(\mathrm{MK1--MK3}\).
Theorem~C
\textup{(}Theorem~\ref{thm:quantum-complementarity-main}\textup{)}
supplies the Verdier eigenspace decomposition under its represented
involution package~\(\mathrm C_1\).  For a fixed chiral
CDG-coalgebra~\(C\), Proposition~\ref{prop:mr-fixed-C-second-kind}
supplies the independent second-kind co--contra equivalence under
\(\mathsf{Pos}^{\mathrm{ch}}_{\mathrm{co-ctr}}(C)\); an algebra
comparison through~\(\tau\colon C\to A\) uses
\(\mathsf{Tw}^{\mathrm{ch}}_{\mathrm{acyc}}(C,A,\tau)\).

The local formality theorem \textup{(}Theorem~\ref{thm:chiral-formality}\textup{)}
identifies the genus-$0$ disk deformation complex with its polyvector
model.  The open higher-genus obligation in
\S\ref{sec:open-questions} is a log-FM modular \(L_\infty\)
comparison compatible with these four typed interfaces on every
finite window.
\end{remark}
...
algebraic closed-sector algebra of the Landau--Ginzburg model is
identified with functions on the \emph{derived critical locus}. Its
physical-bulk reading requires the Landau--Ginzburg open/closed
comparison. The simplest case is the \emph{boundary-linear}
superpotential $W(x,y) = \langle y, F(x) \rangle$, for which the
closed-sector identification is a direct consequence of the
...
spaces over \(\overline{\mathcal M}_{g,n}\) for \(g\geq2\).  Its
compatibility package has five independent clauses:
\begin{enumerate}[label=\textup{(\roman*)}]
\item the completion-aware universal unit and counit of Theorem~A
 \textup{(}Theorem~\ref{thm:mr-A}\textup{)} in the pro-nilpotent
 chiral Ran ambient and every modular finite window under
 \(H_1=H_{\mathrm{fact}}\cup H_{\mathrm{conv}}\);
\item for each chosen quadratic presentation, the comparison
 \[
 q_A\colon A^{\mathrm i}\longrightarrow B_X(A)
 \]
 and the quadratic recognition package~\(H_{\mathrm{CL}}\) of
 Theorem~B \textup{(}Theorem~\ref{thm:mr-B}\textup{)};
\item the Verdier/Koszul algebra
 \(A^!=\mathbb D_{\Ran}(A^{\mathrm i})\) on the dualizable modular
 Koszul locus;
\item for each fixed complete chiral CDG-coalgebra~\(C\), the
 second-kind co--contra equivalence under
 \(\mathsf{Pos}^{\mathrm{ch}}_{\mathrm{co-ctr}}(C)\), together with
 \(\mathsf{Tw}^{\mathrm{ch}}_{\mathrm{acyc}}(C,A,\tau)\) whenever a
 twisting morphism compares~\(C\) with~\(A\);
\item the complementarity eigenspace of Theorem~C
 \textup{(}Theorem~\ref{thm:quantum-complementarity-main}\textup{)}
 under its represented-involution package~\(\mathrm C_1\).
\end{enumerate}

The open proof obligation is the chain-level bridge from these
higher-genus structures to the explicit genus-$0$ weights: a system
of higher-genus Stokes identities and factorization relations for
Kontsevich weights compatible with the completed modular bar
construction and with all five clauses above.
...
\(\hbar\)-adic quantization theorem only after specifying the source
Lie bialgebra \((\mathfrak g,\delta)\), its completed/pro-nilpotent
topology, a Drinfeld associator \(\Phi\), and the target
QUE/quasi-Hopf category;
...
the corresponding formal EK quantization for the specified completed
Lie bialgebra, associator, topology, and quasi-Hopf target. Identifying the full formal
...
\(\hbar=0\), after their source, completion topology, associator, and
target categories have been fixed. Connecting the formal neighbourhood to the Lusztig point
