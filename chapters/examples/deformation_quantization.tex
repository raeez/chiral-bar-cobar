...
\begin{enumerate}[label=\textup{(\roman*)}]
\item The classical limit $V^{\mathrm{cl}}$ carries a Poisson $\lambda$-bracket $\{a_\lambda b\}$ which is the arity-$2$ shadow $\pi_{2,0}(\Theta_{V^{\mathrm{cl}}})$.
\item The quantization obstruction $o_3(V^{\mathrm{cl}}) \in H^2(\Defcyc(\cA), d_2)$ is the cubic shadow projection.
\item Koszulness of $V^{\mathrm{cl}}$ (the PVA satisfying Jacobi) is equivalent to vanishing of $o_3$ and all higher obstructions: the shadow tower terminates.
\end{enumerate}
\end{construction}
...
$m_3 \neq 0$ (arity-$3$ product in the $A_\infty$ model), while
the node has $m_2 \neq 0$.  This arity difference is reflected
