...
The examples of the preceding chapters --- free fields, Kac--Moody, W-algebras, minimal models --- are all \emph{given} chiral algebras whose bar complexes we compute.  Deformation quantization reverses the question: starting from a \emph{classical} (commutative) structure, construct a chiral algebra by deformation.  Kontsevich's formality theorem accomplishes this for Poisson manifolds via configuration space integrals --- precisely the same configuration spaces that define our bar complex.  This is not a coincidence: the bar-cobar machinery provides the homotopical framework in which Kontsevich's star product formula is the genus-$0$ shadow of a chiral quantization defined at all genera.
...
The upper half-plane $\mathbb{H}$ is the simplest example of a worldsheet.
...
\begin{definition}[Coisson algebra on a curve]
\label{def:coisson-curve}
...
($\Einf$-chiral algebra), not an $\Eone$-chiral algebra; see
Definition~\ref{def:coisson} for
the precise relationship to the chiral hierarchy.  For the gauge-theoretic origin of PVA structures via 3d holomorphic-topological theories, see Khan--Zeng~\cite{KhanZeng25}.
\end{definition}
...
This has the same structure as Kontsevich's formula.
\begin{itemize}
...
The paper on quadratic duality for chiral algebras \cite{QuadDual} focuses on Koszul duality for quadratic operads. The deformation quantization framework of this monograph:

\begin{itemize}
\item \emph{Generalizes.} From quadratic to arbitrary (non-quadratic via curvature)
\item \emph{Geometrizes.} Koszul duality = Bar-cobar via configuration spaces
\item \emph{Computes.} Explicit formulas for dualizing
\end{itemize}
...
\begin{proposition}[Configuration space weights and MZVs;
...
\emph{Dictionary.}
For a Poisson chiral algebra $\cA_{\mathrm{cl}}$ (a Coisson algebra
in the sense of Beilinson--Drinfeld):
\begin{align}
