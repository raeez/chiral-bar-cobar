...
\begin{theorem}[Chiral Kontsevich Formula; \ClaimStatusConjectured]
...
\begin{remark}[Scope]
The \emph{existence} of a deformation quantization for chiral Poisson
algebras is not in question: Theorem~\ref{thm:chiral-formality}
(\ClaimStatusProvedElsewhere, Francis--Gaitsgory) establishes formality
of chiral polyvector fields, guaranteeing a star product by
Maurer--Cartan theory.  The present theorem gives the \emph{explicit
integral formula} in terms of chiral configuration space integrals and
admissible graphs.  It is labeled \ClaimStatusConjectured{} because the
angle-form weights
$w_\Gamma = \int_{\overline{C}_n^{\mathrm{ch}}(X)} \omega_\Gamma$
require verification in the chiral setting (on curves rather than
$\mathbb{R}^d$), where holomorphic vs.\ antiholomorphic contributions
and the global geometry of $X$ modify the weight computation.  This is
provable in principle --- the low-order terms ($\hbar^0$ through
$\hbar^2$) are verified explicitly below
(Computation~\ref{comp:star-order-2}), and the general case reduces to
Stokes' theorem boundary cancellations on the chiral FM compactification
--- but a complete all-orders verification has not appeared in the
literature.
\end{remark}
