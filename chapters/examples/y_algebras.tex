\chapter{\texorpdfstring{$Y$-algebras (corner VOAs)}{Y-algebras (corner VOAs)}}\label{chap:y-algebras}\label{ch:y-algebras}
...
Throughout this chapter \(Y_{N_1,N_2,N_3}[\Psi]\) means the
Gaiotto--Rap\v{c}\'ak junction datum with the three
\(\Omega\)-background parameters of
\eqref{eq:y-omega-params} and boundary labels
\(\mathrm{GL}(N_1),\mathrm{GL}(N_2),\mathrm{GL}(N_3)\).  Algebraic
BRST/truncation statements, chiral Koszul statements, and physical
bulk or boundary-condition statements are separate lanes; a comparison
between them is used only when a named comparison package is supplied.
...
(the scalar-lane conflation; $\kappa$ is determined by the
trace-form normalisation, not by the central charge).

\begin{table}[ht]
\centering
\small
\caption{Five-theorem status table for $Y_{N_1,N_2,N_3}[\Psi]$
...
B (inversion) &
 ...
 Y[\Psi]$
 & Conditional & Thm~\ref{thm:bar-cobar-inversion-qi},
 Cor~\ref{cor:universal-koszul} \\
C (complementarity) &
 For \(Y_{1,1,1}\),
 \(\kappa(\Psi)+\kappa(-\Psi)=0\); general \(Y\)-channel
 complementarity requires \(H_Y^\vee\)
 & Conditional & Prop~\ref{prop:y-complementarity} \\
D (modular char.) &
 $\kappa = \sum_i \kappa_i$ (channel-by-channel) under the
 channel-decomposition package
 & Conditional & Thm~\ref{thm:y-kappa-general} \\
H (chiral Hochschild) &
 Given a family support datum \(H_H(Y[\Psi];S_Y)\) with
 finite-dimensional \(H^n(K_{Y[\Psi],S_Y})\),
 $\operatorname{Supp}\mathrm{ChirHoch}^*(Y[\Psi])\subseteq S_Y$
 and
 $P_{Y[\Psi]}(t)=\sum_{n\in S_Y}\dim H^n(K_{Y[\Psi],S_Y})t^n$
 & Conditional & Thm~\ref{thm:hochschild-polynomial-growth} \\
\bottomrule
\end{tabular}
\end{table}
...
$S_3$ triality & Permutes $N$-indices, acts on
 $\Psi$ by M\"obius \\
Koszul dual & Conditional Verdier/BRST companion
...
\begin{remark}[$S_3$ triality; \ClaimStatusProvedElsewhere]%
\label{rem:y-triality}
\index{Y-algebra@$Y$-algebra!triality}
By Gaiotto--Rap\v{c}\'ak~\cite[eq.~3.38]{gaiotto-rapchak},
...
shadow depth class are transported across these isomorphisms, hence are
triality invariants of the isomorphism class.
\end{remark}
...
\begin{theorem}[{Truncation-lane central-charge scalar for
$Y_{N_1,N_2,N_3}[\Psi]$; \ClaimStatusConditional}]%
\label{thm:y-central-charge}
\index{Y-algebra@$Y$-algebra!central charge}
With $h_1 = \Psi$, $h_2 = -1$, $h_3 = 1 - \Psi$, the
scalar attached here to the \(\cW_{1+\infty}\)-truncation lane is
...
\{i,j,k\} = \{1,2,3\}.
\end{equation}
This is not asserted to be the physical VOA central charge after BRST
reduction, decoupled \(U(1)\)-normalisation, or gauge-theory boundary
realisation unless the corresponding normalisation-comparison package
is supplied.
\end{theorem}
...
$\cW_{1+\infty}$ truncation scalar specializes
to~\eqref{eq:y-central-charge}.  The special cases below show why this
statement is typed as a truncation-lane scalar rather than an
unqualified physical central charge: for \(Y_{0,0,N}\) the expression
gives the \(\cW_{1+\infty}\)-normalised value, while the physical
\(\cW_N\times\mathfrak{gl}(1)\) presentation has its own decoupled
\(U(1)\) contribution.
\end{proof}
...
\item \emph{$Y_{1,1,1}[\Psi]$:}
 ...
 for $Y_{1,1,1}$, independent of~$\Psi$. This confirms
 Theorem~\ref{thm:y111-central-charge}.
\end{enumerate}
\end{computation}
...
; for $Y$-algebras with $c = 0$ but nontrivial
Heisenberg currents, $\kappa \neq c/2$ generically.

\begin{theorem}[{Channel-by-channel $\kappa$ for
$Y_{1,1,1}[\Psi]$; \ClaimStatusConditional}]%
...
\item \emph{Heisenberg channel} ($J$-channel):
 ...
 \eqref{eq:heisenberg-kappa-formula}.
...
\begin{proof}
At genus~$1$, the pointed trace is
\[
\operatorname{tr}_1\operatorname{Obs}^{\mathrm{def}}_{1,1}(Y_{1,1,1}[\Psi])
...
of conformal weight). The $T$-channel contributes
...
gives the ordered collision kernel
$K_J^{\mathrm{coll}}(z)=\Psi/z$.  Its homotopy coinvariant remains
the operator class $q_2(K_J^{\mathrm{coll}})$, and the normalized
central-channel functional satisfies
$\tau_{Y,2}^{\mathrm{res}}q_2(K_J^{\mathrm{coll}})=\kappa_J=\Psi$.
On Fock-charge modules this rank-one kernel has commuting tensor
insertions, so the resulting represented operator satisfies CYBE and
is the Heisenberg classical $r$-matrix.
\end{proof}
...
. For $Y_{1,1,1}$, the Heisenberg channel dominates
...
cross-channel mixed terms vanish at degree~$2$: the mixed OPE
...
\begin{proposition}[{Verdier/BRST comparison for the
$Y_{N_1,N_2,N_3}[\Psi]$ dual companion; \ClaimStatusConditional}]%
\label{prop:y-koszul-dual}
\index{Y-algebra@$Y$-algebra!Koszul dual}
\index{Koszul dual!Y-algebra@$Y$-algebra}
At generic coupling, assume the package \(H_Y^\vee\): the
Gaiotto--Rap\v{c}\'ak junction datum with fixed \(\Omega\)-background
parameters and \(\mathrm{GL}(N_i)\) boundary labels, PBW chiral
Koszulness for \(Y_{N_1,N_2,N_3}[\Psi]\), convergence of the
Verdier-dual completed bar construction, and a BRST/DS comparison
identifying the Verdier-dual output with the parameter-reflected corner
VOA.  Then the homotopy Koszul-dual companion is
...
Y_{N_1,N_2,N_3}[-\Psi].
\end{equation}
If a strict algebra \(Y_{N_1,N_2,N_3}[\Psi]^!\) is named instead of
the homotopy dual \(K_X(Y[\Psi])\), a separate formality/minimal-model
comparison is also required.  The parameter reflection
\(\Psi\mapsto-\Psi\) is not, by itself, the Feigin--Frenkel centre
theorem and is not bar--cobar inversion.
\end{proposition}

\begin{proof}
The BRST definition~\eqref{eq:y-brst-definition} supplies the
algebraic corner-VOA model, and the \(\Omega\)-background parameters
fix the reflected candidate \(Y_{N_1,N_2,N_3}[-\Psi]\).  The
bar--cobar counit itself reconstructs \(Y_{N_1,N_2,N_3}[\Psi]\);
it does not construct the dual.  The comparison in
\eqref{eq:y-koszul-dual} therefore consumes exactly the Verdier branch
and BRST/DS comparison included in \(H_Y^\vee\).  Passing from this
homotopy dual to a strict \(Y[\Psi]^!\) is a further formality
assertion, not a consequence of the parameter reflection alone.
\end{proof}
...
\begin{remark}[Complementarity sum and the antisymmetry $\kappa+\kappa'=0$]%
\label{rem:y-complementarity-level}
...
Theorem~\ref{thm:complementarity} for $\cW$-algebras.
\end{remark}
...
terminates at degree~$2$ (class~$\mathsf{G}$,
$r_{\max} = 2$).
...
either abelian (spin~$1$) or $\cW$-type (higher spin).
  ...
  \begin{remark}[Depth classification mechanism for $Y$-algebras]%
  \label{rem:y-algebra-depth-classification}%
  \ClaimStatusProvedHere%
  At generic coupling, the depth dichotomy is controlled by the
  largest box-length \(N=\max(N_1,N_2,N_3)\) in the plane-partition
  presentation. If \(N\leq1\), the only nontrivial local fields are a
  Heisenberg current and the stress tensor; their self-OPEs contribute
  only quadratic scalar data to the ordered bar complex, so the shadow
  tower stops at degree \(2\). If \(N\geq2\), one channel contains a
  principal \(\cW_N\)-type truncation with a higher-spin generator
  \(W_s\), \(s\geq3\). The self-OPE \(W_s(z)W_s(w)\) contains
  normally ordered composite fields, and the ordered collision
  coproduct recursively feeds these composites into higher bar length.
  Thus the shadow obstruction tower does not terminate, giving
  \(r_{\max}=\infty\). The Lie/tree and contact classes are absent at
  generic coupling because the corner presentation has neither an
  isolated nonabelian current algebra without higher-spin composites
  nor a \(\beta\gamma\)-type quartic contact generator.
  \end{remark}
...
modular characteristic $\kappa = \Psi$ at degree~$2$ and
...
for all $g \geq 1$ \textup{(}ALL-WEIGHT, with
$\delta F_g^{\mathrm{cross}} = 0$ because $\kappa_T = 0$\textup{)},
driven entirely by the Heisenberg channel.
\end{remark}


\section{The collision kernel of $Y_{1,1,1}[\Psi]$}%
\label{sec:y111-r-matrix}

\begin{computation}[{Collision residue for $Y_{1,1,1}[\Psi]$;
\ClaimStatusProvedHere}]%
\label{comp:y111-collision-residue}
\index{Y-algebra@$Y$-algebra!Y111@$Y_{1,1,1}$!collision kernel}
\index{collision kernel!Y111@$Y_{1,1,1}$}
The ordered bar complex $B^{\mathrm{ord}}(Y_{1,1,1}[\Psi])$
has deconcatenation coproduct on $T^c(s^{-1}\bar{V})$ where
$V = \bC J \oplus \bC T$. The degree-$2$ collision residue
$K_Y^{\mathrm{coll}}(z) = \mathrm{Res}^{\mathrm{coll}}_{0,2}(\Theta_Y)$
decomposes into channels:

\emph{$J$-channel.}
The OPE $J(z)\,J(w) \sim \Psi/(z-w)^2$ has a double pole.
After $d\log$ absorption (reducing pole order by one):
\begin{equation}\label{eq:y111-r-j-channel}
K_J^{\mathrm{coll}}(z) \;=\; \frac{\Psi}{z}.
\end{equation}
This is a single simple pole.  Its Fock-charge representation is the
standard Heisenberg classical $r$-matrix at level~$\Psi$.
...
K_{JT}^{\mathrm{coll}}(z) \;=\; \frac{J}{z}.
\end{equation}

The binary ordered collision kernel of $Y_{1,1,1}[\Psi]$ is the
OPE-operation-valued meromorphic matrix
...
$V = \bC J \oplus \bC T$.  The homotopy coinvariant
$q_2(K_Y^{\mathrm{coll}})$ retains all four operator-valued entries.
Define
\[
 \tau_{Y,2}^{\mathrm{res}}\colon
 \End_Y(2)_{h\Sigma_2}\longrightarrow\bC
\]
to extract the normalized central coefficient in the $J$--$J$
channel.  Then
\[
 \tau_{Y,2}^{\mathrm{res}}q_2(K_Y^{\mathrm{coll}})=\Psi
 =\kappa(Y_{1,1,1}[\Psi]).
\]
A classical $r$-matrix associated with the complete kernel is a
represented realization: it consists of a map
$\rho_Y\colon\End_Y(2)\to\End(W^{\otimes2})$ together with
$H_{\mathrm{CYBE}}^{\mathrm{rep}}(Y;W)$, which asserts CYBE for the
three insertions of $\rho_Y(K_Y^{\mathrm{coll}})$.
\end{computation}

\begin{remark}[Collision-kernel and represented-$r$ tiers]%
\label{rem:y111-r-tier}
\index{r-matrix@$r$-matrix!tier classification!Y111@$Y_{1,1,1}$}
The OPE determines $K_Y^{\mathrm{coll}}$ in the collision-kernel
tier.  A chosen package $H_{\mathrm{CYBE}}^{\mathrm{rep}}(Y;W)$
places its image in tier~(b) of
\S\ref{sec:concordance-three-tier-r-matrix}.  The kernel has a single
simple pole at $z=0$; the central specialization $c=0$ leaves the
$J$--$J$ central channel and the field-valued simple-pole channels.
\end{remark}
...
\begin{proposition}[{Genus-$1$ formal coefficient of $Y_{1,1,1}[\Psi]$;
\ClaimStatusConditional}]%
\label{prop:y111-genus1}
\index{Y-algebra@$Y$-algebra!Y111@$Y_{1,1,1}$!genus tower}
The genus-$1$ formal scalar coefficient is
...
$F_1(Y_{1,1,1}[\Psi])
 = \kappa(Y_{1,1,1}[\Psi])/24$ unconditionally. With
$\kappa(Y_{1,1,1}[\Psi]) = \Psi$
(Theorem~\ref{thm:y111-kappa-channels}), this gives
$F_1(Y_{1,1,1}[\Psi]) = \Psi/24$.
\end{proof}
...
\;=\; \kappa(Y_{1,1,1}[\Psi]) \cdot \lambda_g^{\mathrm{FP}}
\;=\; \Psi \cdot \lambda_g^{\mathrm{FP}}
\qquad\textup{(}\text{ALL-WEIGHT},\;
\delta F_g^{\mathrm{cross}} = 0\textup{)},
\end{equation}
where
\(\lambda_g^{\mathrm{FP}}
=\int_{\overline{\cM}_{g,1}}\psi_1^{2g-2}\lambda_g\)
is the marked Faber--Pandharipande scalar coefficient. The first values:
\begin{center}
\begin{tabular}{ll}
$F_1 = \Psi/24$, &
$F_2 = 7\Psi/5760$, \\
...
sense of Theorem~\ref{thm:multi-weight-genus-expansion}.
The multi-weight genus expansion
...
The conditional parameter-reflected dual-companion lane
\(\Psi \mapsto -\Psi\) has no real self-dual fixed point
($\Psi = -\Psi$ gives $\Psi = 0$, which is
...
\item \emph{Pillar~A (homotopy chiral, MS24):}
 ...
 no secondary Borcherds operations at degree~$\geq 3$). This
...
\item \emph{Pillar~C (log FM, Mok25):}
 ...
 at degree~$2$ and all higher-degree codimension-${\geq}2$
...
(\S\ref{sec:concordance-holographic-correspondence}), the junction
origin provides:
\begin{enumerate}[label=\textup{(\roman*)},nosep]
\item $A = Y_{N_1,N_2,N_3}[\Psi]$,
 and, under \(H_Y^\vee\),
 \(K_X(A)\simeq Y_{N_1,N_2,N_3}[-\Psi]\): the boundary algebra and
 its conditional homotopy Koszul-dual companion;
\item $K_A^{\mathrm{coll}}(z)$: the collision kernel
...
\item $\Theta_A$: the universal MC element, with
 $\kappa(Y_{1,1,1}[\Psi])=\Psi$ controlling the genus tower in the
 trivalent case;
\item $\nabla^{\mathrm{hol}}$: the holomorphic connection on the
 bundle over $\overline{\cM}_{g,n}$, with curvature
 $\kappa(A) \cdot \omega_g$ at genus~$g \geq 1$.
\end{enumerate}
