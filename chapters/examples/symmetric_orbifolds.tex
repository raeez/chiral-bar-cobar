...
B (inversion) &
 ...
 & Reynolds descent hypothesis; Prop.~\ref{prop:symn-inversion} \\
C (complementarity) &
...
H (chiral Hochschild) &
 $H_H(X_N^{\mathrm{id}};S_N)$ gives
 $\operatorname{Supp}\operatorname{ChirHoch}^\bullet
 (X_N^{\mathrm{id}})\subseteq S_N$
 & family-retract lane; Prop.~\ref{prop:symn-hochschild} \\
\bottomrule
\end{tabular}
\end{table}
...
filtration, and with finite pointed genus-$1$ trace
$\operatorname{tr}_1\operatorname{Obs}^{\mathrm{def}}_{1,1}(X)
=\kappa(X)\lambda_1$. For the fixed-point chiral algebra
$X_N^{\mathrm{id}}=(X^{\otimes N})^{S_N}$,
\[
 \kappa^{\mathrm{id}}_1(X_N^{\mathrm{id}})
 \;=\; N \cdot \kappa(X).
...
\[
 \operatorname{tr}_1\operatorname{Obs}^{\mathrm{def}}_{1,1}(X^{\otimes N})
 = \sum_{i=1}^N
   \operatorname{tr}_1\operatorname{Obs}^{\mathrm{def}}_{1,1}(X^{(i)})
 = N\,\kappa(X)\lambda_1 .
\]
...
\ClaimStatusConditional]\label{prop:symn-inversion}
...
same Reynolds descent as Proposition~\ref{prop:symn-inversion}. A
...
\ClaimStatusConditional]\label{prop:symn-hochschild}
Let $S_N\subset\mathbb Z$ be finite and assume the fixed-point chiral
algebra carries the family datum $H_H(X_N^{\mathrm{id}};S_N)$.  Then
\[
 \operatorname{ChirHoch}^n(X_N^{\mathrm{id}})
 \cong H^n(K_{X_N^{\mathrm{id}},S_N}),
 \qquad
 \operatorname{Supp}\operatorname{ChirHoch}^\bullet
 (X_N^{\mathrm{id}})\subseteq S_N.
\]
This is an ambient-curve statement.  The categorical Hochschild
cohomology of the full twisted orbifold module category and singular
quotient charts carry their own family models and comparison maps.
A companion family datum and a perfect degree-$d$ pairing supply the
separate degree reflection.
\end{proposition}

\begin{proof}
Theorem~\ref{thm:main-koszul-hoch} applied to
$H_H(X_N^{\mathrm{id}};S_N)$ gives the displayed identification.
Twisted sectors are modules for $X_N^{\mathrm{id}}$ in the present
chart; a constructed $G$-crossed algebra supplies a distinct chiral
Hochschild input.  Theorem~\ref{thm:hochschild-polynomial-growth}
gives a polynomial Hilbert series when the groups
$H^n(K_{X_N^{\mathrm{id}},S_N})$ are finite-dimensional.
\end{proof}
...
\begin{theorem}[DMVV product formula;
\ClaimStatusProvedElsewhere{} \cite{DMVV97}]
\label{thm:symn-dmvv-product}
For a seed theory~$X$ with elliptic genus
$Z_X(\tau, z) = \sum_{n,\ell} c_X(n,\ell)\, q^n y^\ell$,
the DMVV product computes the generating series over all~$N$
simultaneously:
...
\begin{proposition}[DMVV Hecke-operator form;
\ClaimStatusProvedElsewhere{} \cite{DMVV97}]
\label{prop:symn-hecke-form}
...
target genus-$2$ character of the K3 chiral-bialgebra recognition
object $\mathbf{H}_{\Delta_5}^{\mathrm{tgt}}$, conditional on the
Hall/CoHA source, compact pairing, PBW/no-extra-relations theorem,
and comparison maps. The first limit isolates the Jacobi coefficient,
while the second is the genuine separating degeneration.
\end{remark}
