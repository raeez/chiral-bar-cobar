...
\index{Master Pattern MA-6!moonshine}
\index{Master Pattern MA-7!moonshine K3 row}
Type signature: (Open quadrant for the $\Vnat,\Vsnat,W^{\Th}$
chart algebras; CY-coupled row for $\VKthree$ via the Vol~III
two-stage factorisation
$\Phi_2^{(\Sigma_1,C)}=\SpCh_{\Sigma_1,C}\circ\PhiFA_2$;
chart-level presentation; Beilinson level $1$ chart entries with
level $5$ scalar shadow projections; hypothesis package: working
loci of Definition~\ref{def:moonshine-four-families} together with
the channel-supremum classification rule).
...
(Master Pattern MA-6: five $\kappa$-measurements per row, never
collapsed to one; Master Pattern MA-7: the $\mathbf B$-row entry
specifies its $(\Sigma_{d-1},C)$ pair, never a one-stage
$\Phi_d$).
\begin{center}
...
witness whose canonical row is recorded by
Frontier~F$\mathbf B$ in FRONTIER.md;
...
Type signature: (Open quadrant; chain-level presentation; Beilinson
level $2 \to 5$; hypothesis package: augmented weight-graded chiral
algebra with finite weight pieces, primitive-bar finite-type model,
external denominator-recognition datum of
...
Type signature: (Open quadrant; chain-level presentation; Beilinson
level $0 \to 1$ via finite-orbifold descent; hypothesis package: the
three data of Definition~\ref{def:e3-topological}, namely the abelian
Leech HT model, anomaly-free $\bZ/2$ lift, and FLM boundary
identification; class transition recorded separately by
...
Type signature: (Open quadrant; chain-level presentation;
Beilinson levels $1$ chart and $2$ bar; hypothesis package:
Duncan's $N{=}1$ super-VOA structure on $\Vsnat$, $\Co_0$ acting
by super-VOA automorphisms, no Conway BKM denominator asserted).
...
Type signature: (Open quadrant; chain-level presentation;
Beilinson levels $1$ chart and $2$ bar; hypothesis package:
external $\Th$-equivariant chiral superalgebra $\VTh$ supplied
together with an isomorphism of graded $\Th$-modules
$H^\bullet(\operatorname{Prim}\Barch(\VTh))_n\cong W^{\Th}_n$;
Thompson BKM denominator if its specialised root multiplicities
match the named graded supertraces).
...
Type signature: (Open quadrant for the $\VKthree$ chart algebra;
CY quadrant after the Vol~III two-stage factorisation
$\Phi_2^{(\Sigma_1,C)}=\SpCh_{\Sigma_1,C}\circ\PhiFA_2$ with
$\Sigma_1$ a transverse one-real-dimensional manifold and $C$ a
fixed reference curve; chain-level presentation; Beilinson levels
$1$ chart, $5$ scalar shadow; hypothesis package: Eguchi--Ooguri
--Tachikawa decomposition into $c=6$ $\cN{=}4$ characters, Gannon
$\Mtw$-module theorem, DMVV multiplicative lift, Gritsenko
--Nikulin Borcherds lift; for the bar comparison, the finite
denominator-recognition datum of
Definition~\ref{def:moonshine-finite-denominator-recognition};
Master Pattern MA-7: the two-stage decomposition is named, never
a one-stage $\Phi_d$).
...
Borcherds/DMVV lane.  The celestial K3 construction uses the
$\cN{=}4$ matter lane.  The scalar conductor $K^\kappa=8$ is the
Mukai-doubling complementarity value of
Proposition~\ref{prop:archetype-complementarity-bridge}, not the BKM
weight~$5$.  A bar-Euler identity between these lanes requires an
...
\begin{theorem}[Twined shadow comparison: K3 scope and general
...
Type signature: (Open quadrant on the algebra side, scalar shadow
target; chain-level presentation; Beilinson levels $2$ bar and
$5$ scalar shadow; hypothesis package: K3 scope unconditional via
Vol~II celestial--Mathieu theorem and Mellin--shadow dictionary;
general $(A,G_A)$ scope conditional on a chain map compatible
with twining and depth filtrations from the primitive bar complex
to the Rademacher polar-data complex, plus the sign, Weyl-vector,
