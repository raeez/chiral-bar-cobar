...
\begin{definition}[Finite denominator-recognition datum; \ClaimStatusDefinitional]
...
bar--cobar theorem.  When a quadratic presentation
$A=T_X(V)/(R)$ is chosen, the objects
\[
 A,\qquad
 \Barch(A),\qquad
 A^{\mathrm i}=C_X(s^{-1}V,s^{-2}R),\qquad
 H^\bullet\Barch(A),\qquad
 K_X(A)=\mathbb D_{\Ran}\Barch(A),\qquad
 A^!,\qquad
 \ZcSig(A)
\]
are connected by
\[
q_A\colon A^{\mathrm i}\to\Barch(A),\qquad
\mathbb D(q_A)\colon K_X(A)\to\mathbb D_{\Ran}(A^{\mathrm i}),
\qquad
\nu_A\colon K_X(A)\to A^!.
\]
The counit $\Omega\Barch(A)\to A$ is bar--cobar inversion, while
$\ZcSig(A)=C^\bullet_{\mathrm{ch}}(A,A)$ is the Hochschild/closed-sector
object.
\end{remark}
...
\end{enumerate}
This is a statement about the descended HT model.  The bar coalgebra
$\Barch(A)$, its cohomology $H^\bullet\Barch(A)$, any quadratic
presentation coalgebra $A^{\mathrm i}=C_X(s^{-1}V,s^{-2}R)$,
the Verdier algebra $K_X(A)$, the chosen partner~$A^!$, and the
derived chiral centre~$\ZcSig(A)$ retain their respective types.
The counit $\Omega\Barch(A)\to A$ records bar--cobar inversion.
\end{definition}
