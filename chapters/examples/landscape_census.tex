...
$\mathfrak{sl}_3$; in general it follows from invariance of the
Casimir tensor);
...
The four unconditional witness formulas and the principal
$\mathcal W_3$ formula are evaluated by the three routes just listed.
The latter formula is read on
$H_{\mathrm{diag}}^{g=1}+H_{W_3}^{\mathrm{DS/bar}}$
and uses the displayed $(\varrho,K,K^\kappa,\kappa^*)$ tuple.
...
class-L Koszul row. The same Jacobi argument applies to the exceptional
and non-simply-laced non-critical cases.
...
\textup{(}Computation~\textup{\ref{comp:fe-minimal-models}}\textup{)}.
}
\end{table}
...
The linear growth $\dim H^n = 2n+1$ differs from the Riordan formula
...
\begin{itemize}
 ...
 eq.~$\textup{(}2.20\textup{)}$.
\end{itemize}
The values in this comparison are the ones established in
Propositions~\textup{\ref{prop:canonical-prs-coefficient-cm214}--\ref{prop:canonical-bcov-quintic}}.
\end{remark}
