...
HF = honest frontier (scope stated, invariant not yet closed).
\begin{itemize}[leftmargin=*]
  ...
  \item HF (cosets): $c(\cA/\cB) = c(\cA) - c(\cB)$ is
    ...
    not yet computed family-by-family. See
...
distinct downstream invariant, not to be confused with $S_4$ itself).
...
The Virasoro shadow obstruction tower coefficients $S_r = S_r(\mathrm{Vir}_c)$ are pinned across all three volumes as follows:
\begin{align*}
...
\Delta &= 8\kappa S_4 = \frac{40}{5c+22} \qquad \text{(critical discriminant, downstream invariant)}.
\end{align*}
...
$\Delta$ is \emph{not} $S_4$; confusion between the two has been
the source of the misprint $S_4 = 40/[c(5c+22)]$ appearing in
earlier drafts. Any occurrence of $S_4 = 40/[c(5c+22)]$ should be read
as the critical discriminant $\Delta = 40/(5c+22)$ with the factor
$c$ in the denominator removed.
\end{proposition}
...
\begin{table}[ht]
\centering
\caption{Scalar-lane free energies $F_1 = \kappa/24$ and
$F_2^{\mathrm{scalar}} = 7\kappa/5760$ for $21$ standard families;
...
Beilinson--Drinfeld Koszul-conductor identity. The scope is explicitly the
...
Remark~\ref{rem:mukai-doubling-K-kappa}, where $\hbar^2\cdot K^{\kappa_{\mathrm{ch}}}
...
within that scope, the Beilinson--Drinfeld Koszul-conductor identity
...
(Vol~III Chapter~\ref{ch:k3-chiral-bialgebra-platonic}
Remark~\ref{rem:mukai-doubling-K-kappa}).
\end{proof}

\begin{proposition}[Anomaly-ratio bifurcation between $\mathsf{G}$- and
$\mathsf{B}$-Heisenbergs; \ClaimStatusProvedHere]
...
\textup{[Regime: curved-central, Koszul locus,
Convention~\textup{\ref{conv:regime-tags}}; scope qualifier
...
\begin{remark}[Degree-three Hochschild class across the archetype
  landscape]
\label{rem:chirhoch3-archetype-column}
\index{Hochschild cohomology!degree-three archetype table}
\index{Zamolodchikov W3 generator!as $\ChirHoch^3$ representative}
The archetype table of
Proposition~\textup{\ref{prop:archetype-complementarity-bridge}}
extends by one column, recording the degree-$3$ chiral
Hochschild class $[\chi_3^{\cA}]$ of each witness family
(Theorem~\ref{thm:chirhoch3-five-archetype-portrait} of
\texttt{hochschild\_cohomology.tex}, which collapses through
Corollary~\ref{cor:chirhoch3-unified-formula} to
\[
 [\chi_3^{\cA}]\;=\;
 \begin{cases}
  0 & \cA\in\{\mathsf G,\mathsf L,\mathsf C\},\ r_{\max}<\infty, \\
  \mathbb C\cdot\bigl[{:}T\partial T{:}-\tfrac14\partial^3 T\bigr] &
    \cA=\mathrm{Vir}_c,\ c\text{ generic, ambient-curve scope}, \\
  \mathbb C\cdot\bigl[{:}T\partial T{:}-\tfrac14\partial^3 T+
    \hbar\cdot\mathrm{qt}(J^{(3)})\bigr] &
    \cA=\mathbf H_{\Delta_5}=\Phi_3(\mathrm{K3}\times E),
 \end{cases}
\]
the scope-enlarged ambient being $X\times(\mathrm{K3}\times E)$
for the crown row.) The weight-$5$ quasi-primary
${:}T\partial T{:}-(1/4)\partial^3 T$ is the Zamolodchikov
$W_3$-extension generator
(Zamolodchikov~\textup{\cite{Zam1985}}~eq.~$(2.6)$), and
the quantum tail $\hbar\cdot\mathrm{qt}(J^{(3)})$ is the
Schiffmann--Vasserot degree-$3$ CoHA current. The unified
Virasoro skeleton is common to the $\mathsf M$ and $\mathbf B$
rows; the shadow-finite rows $\mathsf G,\mathsf L,\mathsf C$
admit only $\partial$-exact triples and carry
$[\chi_3^{\cA}]=0$.
\end{remark}
...
singleton $\{c = 13\}$ on the scalar lane (the
...
\begin{proposition}[Fateev--Lukyanov central-charge formula; canonical form]
\label{prop:fateev-lukyanov-canonical}
The principal $\cW_N$ central charge~\eqref{eq:fateev-lukyanov-wn} is the source of truth across all three volumes. The alternative parameterisation $c(t) = (N-1)(1 - N(N+1)(t-1)^2/t)$ with $t = k+N$ (Fateev--Lukyanov~\cite{FateevLukyanov88}, equation~(7)) is equivalent by $(N-1)N(N+1) = N(N^2-1)$. Any displayed formula using $(k+N)^{-1}(k'+N)^{-1}$ in place of $(k+N-1)^2/(k+N)$ is a mixed-parameter shorthand valid only at the Koszul self-dual locus and must be recognised as such.
\end{proposition}
...
\item Complementarity at $N=2$: the partner $k' = -k - 4$ sends $k=1 \mapsto k' = -5$, giving $c' = 1 - 6 \cdot 16/(-3) = 1 + 32 = 33$, and $c + c' = -7 + 33 = 26 = K_2$. Verified.
\end{itemize}
\end{remark}
...
(Table~\ref{tab:master-invariants}). They are flagged here as
...
\begin{enumerate}[leftmargin=*]
  ...
  admissible levels are not yet treated as a separate family row
...
re-entered in this master table. The Schellekens $71$~holomorphic
...
\begin{remark}[Landscape census row at K3$\times$E: the canonical K3 base point]
\label{rem:landscape-1}
The landscape census's K3$\times$E row is the canonical K3 base
point, with the four $\kappa_\bullet$-invariants:
\[
 \begin{aligned}
	 \kappa_{\mathrm{cat}}(K3\times E) &= \chi(\cO_{K3})\cdot\chi(\cO_E) = 2\cdot 0 = 0,\\
	  \kappa_{\mathrm{ch}}^{\mathrm{Heis}}(K3\times E) &= 3,\\
	  \kappa_{\mathrm{BKM}}(\Delta_5) &= c_{1}(0)/2 = 5,\\
	  \kappa_{\mathrm{fiber}}(K3) &= 24,
	 \end{aligned}
\]
and the separate K3 fibre witness $\kappa_{\mathrm{cat}}(K3)=2$.
The values $\{0,3,5,24\}$ and the witness~$2$ come from distinct
constructions (K\"unneth Hodge supertrace, Heisenberg shadow, Gritsenko
$\Delta_5$ Borcherds weight, and K3 fibre rank). The Mukai-doubling identity
...
\begin{remark}[Landscape census rows beyond K3$\times$E]
\label{rem:landscape-2}
Extending the K3 base point through the landscape: (i) CY-$1$
(elliptic curve) $\kappa_{\mathrm{ch}} = 0$, no $\Delta_5$-analogue, no
BKM weight; (ii) CY-$2$ (K3) $\kappa_{\mathrm{ch}}$ is the Hodge
supertrace $\sum_q (-1)^q h^{0,q}(K3) = 2$ at the $\chi(\cO_{K3}) = 2$ face,
with $\Delta_5$ BKM weight $5$ and Mukai-doubling $8$; (iii) CY-$3$
(K3$\times$E or quintic) $\kappa_{\mathrm{ch}} = 0$ with $\kappa_{\mathrm{BKM}}(\Delta_5) = 5$
still via the product structure; (iv) CY-$4$ (local $\mathbb P^2$)
$\kappa_{\mathrm{ch}} = 3/2$ via McKay/direct shadow. Each row at
$d\geq 2$ has an (in principle) BKM lift with the universal identity
$\kappa_{\mathrm{BKM}}(\Phi_N) = c_N(0)/2$ for $N\in\{1,2,3,4,6\}$
\cite{Borcherds1998,Gritsenko1999}. Cross-ref
...
\index{canonical values!coefficient wave-2|textbf}

The five propositions below pin the coefficient values that appear
across all three volumes at the $c=-214$ K3 base point and at
adjacent CY rows. Each value is a single number provable from a
single primary source; each proposition should be consulted before
any new chapter inscribes these coefficients, and before any new
compute engine hardcodes them. Discrepancies with these values
indicate either a convention shift (which must be declared
explicitly) or an error.
...
plus the Jacobi identity on the four-generator family, verified
...
$c(V^\natural)|_{24A}$-value that frequently appears in the programme
is the $n=2$-graded piece
\[
 H_2(24A) \;=\; 4,
\]
...
Cross-verification: each $H_n(g)$ satisfies the $T_g$-recursion
...
rank-$5$ and rank-$2$ paramodular cusps]
...
\]
The admissible Heegner discriminants entering the programme
genus-$2$ shadow analysis are $D\in\{1, 4, 5, 8, 9, 12, 13, 16, \ldots\}$
...
\begin{proposition}[Oberdieck--Pandharipande reduced DT Fourier
coefficients of $\phi_{0,1}^{K3}$]
...
$\phi_{0,1}^{K3}(\tau, 0) = \chi(K3) = 24$, admits the Fourier--Jacobi
expansion
\[
 \phi_{0,1}^{K3}(\tau, z)
 \;=\; \sum_{n\geq 0,\ r\in\Z} c(4n - r^2)\,q^n\,y^r,
\]
with Oberdieck $2018$ \emph{Geom.~Topol.~}$\mathbf{22}$ pages
$323$--$437$ normalisation
\[
 c(-1)=2,\quad c(0) = 20,\quad c(3) = -128,\quad c(4) = 216,\quad
 c(7) = -1026,\quad c(8) = 1616.
\]
Cross-check against Eguchi--Ooguri--Taormina--Yang $1989$:
their $\phi_{0,1}$ equals $2\cdot\phi^{K3}_{\mathrm{Obd}}$ in the
present normalisation, so the ATLAS values \textup{(}Mathieu
moonshine decomposition Cheng--Duncan--Harvey $2014$\textup{)}
$c_{\mathrm{EOT}}(3) = -64,\ c_{\mathrm{EOT}}(4) = 108$ scale to
$c_{\mathrm{Obd}}(3) = -128,\ c_{\mathrm{Obd}}(4) = 216$ by the
factor $2$. Both conventions appear in the literature: the
Oberdieck--Pandharipande reduced DT generating function uses
$c_{\mathrm{Obd}}$; the K3 elliptic genus uses $c_{\mathrm{EOT}}$.
Converting between them requires the factor-of-$2$ bridge.
...
\begin{proof}
The Eguchi--Ooguri--Taormina--Yang $1989$ explicit formula gives
$\phi_{0,1}^{K3} = 2\,A_{1,1}(\tau, z)$ with $A_{1,1}$ the
Appell--Lerch $\mu$-function combination. Direct
...
in the EOT convention. The Oberdieck $2018$ normalisation is
twice the EOT one \textup{(}Oberdieck \S$0.1$ chooses
$\phi_{0,1}^{K3}(0)=24$; EOT chooses $\phi_{0,1}^{K3}(0)=12$\textup{)}.
Cross-verification: Mathieu moonshine \textup{(}Cheng--Duncan--Harvey
$2014$\textup{)} decomposes $\phi_{0,1}^{K3}$ into $M_{24}$-irreducible
characters; the character sums match the Fourier coefficients
in EOT normalisation.
\end{proof}

\begin{proposition}[BCOV genus-$1$ and genus-$2$ constant-map
contributions on the quintic mirror]
\label{prop:canonical-bcov-quintic}
\index{BCOV!quintic mirror!$F_g^{\mathrm{const}}$}
\index{constant-map contribution!genus $1$ and $2$}
\ClaimStatusProvedElsewhere{}
For $X = Q_5\subset\bP^4$ the smooth quintic threefold with
Euler characteristic $\chi(X) = -200$ and
$\kappa_{\mathrm{ch}}(Q_5) = \chi(X)/24 = -25/3$, the BCOV
...
\]
derived from $\chi(Q_5)/24 = -200/24 = -25/3$ multiplied by the
...
sign and by a combinatorial factor of $5/2$. The
programme-canonical convention is BCOV $1994$: positive
...
at $g=2$, giving the $5/144$ value in magnitude with the programme
...
normalisation bridge \textup{(}Klemm--Zaslow Appendix~A\textup{)}, and the
...
\begin{remark}[Three-path verification discipline for each pinned
value]
...
5/144$ BCOV-canonical\textup{)} has three independent verification
paths as required by the programme's three-path discipline:
\begin{itemize}
...
proposition label and the three verification paths above.
\end{remark}
