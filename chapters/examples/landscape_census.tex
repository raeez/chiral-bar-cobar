...
The extended examples of Part~II are not merely illustrations of the
general theory; they are the empirical foundation from which the
theory was extracted.  This summary collects the invariants computed
across all families into a single table, making visible the patterns
that organize the landscape: the complementarity sum $c + c' = 2d$
for Kac--Moody algebras, the universal generating function
$x/\sin x$ for the $\hat{A}$-genus, the coincidence of spectral
discriminants across apparently unrelated families.

The table should be read both horizontally --- each row records the
complete set of invariants for one algebra --- and vertically: the
columns reveal structural regularities that no single example can
exhibit.  That $\kappa(\widehat{\fg}_k) = td/2h^\vee$ and
$\kappa(\mathrm{Vir}_c) = (26 - c)/12$ both descend from the same
universality theorem is a fact visible only in the census.
