...
(class~M). The MC element $\Theta_\cA$ forces the classification:
once $\kappa$ and $S_4$ are computed, the archetype is determined.
Frontier rows whose
Koszul-dual or shadow surfaces are not yet closed are recorded below
as explicit non-coverage rather than folded into this statement.
...
\noindent\textbf{Scope column}: PH = proved in this monograph
(\ClaimStatusProvedHere); CJ = conjectured (\ClaimStatusConjectured);
HF = open frontier (scope stated, invariant not yet closed).
...
$S_r = S_r(\mathrm{Vir}_c)$ are pinned across all three volumes as
follows:
\begin{align*}
...
Checked by \texttt{test\_koszulness\_landscape.py}
...
The six rows of the table are computed individually.
...
\[
 ...
 \cA=\mathrm{Vir}_c,\ c\text{ generic, GF/shadow or extension lane}, \\
...
All conjectural values ($n \geq 4$ for
$\widehat{\mathfrak{sl}}_3$ and $\mathcal{W}_3$,
$n \geq 4$ for Yangian) require independent computation.}
\end{table}

\begin{remark}[Scope of \texorpdfstring{$H^4(\mathcal{W}_3) = 52$}{H4(W3) = 52}]
...
An independent computation, either by direct bar differential computation
...
\begin{proof}
The recurrence~\eqref{eq:riordan-recurrence-explicit} follows by
direct computation from the Riordan generating
...
\emph{Scope.} An independent computation of $\dim H^4(\barBgeom(\widehat{\mathfrak{sl}}_{3,k}))$ directly from the chiral bar differential is required. The value $1352$ would confirm the recurrence~\eqref{eq:sl3-bar-recurrence} and the rationality of the GF, while any other value would rule out this rational form.

The degree-$4$ chain group has dimension $\dim \bar{B}^4 = (\dim\mathfrak{sl}_3)^4 \cdot 3! = 24576$, and the chiral bracket differential $d\colon \bar{B}^4 \to \bar{B}^3$ maps into the $1024$-dimensional space $\bar{B}^3$. The bracket differential at degree~$4 \to 3$ is computationally surjective (rank~$1024$; Computation~\ref{comp:sl3-modular-rank}). The independent computation of $\dim H^4 = 1352$ therefore reduces to computing $\dim\ker(d^{4 \to 3}) - \dim\operatorname{im}(d^{5 \to 4})$, where the first term equals $24576 - 1024 = 23552$ (by surjectivity) and the second requires the rank of the degree-$5$ differential on the $786432$-dimensional chain group $\bar{B}^5 = 8^5 \cdot 4! = 786432$. This rank computation is feasible via modular arithmetic over $\mathbb{F}_p$ for suitable primes~$p$.
\end{conjecture}
...
$\mathfrak{sl}_2$ family, this is computed explicitly:
...
\]
Asymptotically, $\dim (\widehat{\mathfrak{g}}_k^!)_n \sim C \cdot (\dim \mathfrak{g})^n \cdot n^{-\alpha}$ for some $C > 0$ and $\alpha > 0$. The computed cases are: $\dim \mathfrak{sl}_2 = 3$ (growth $3^n$), $\dim \mathfrak{sl}_3 = 8$ (growth $8^n$).
\end{corollary}
...
Independent check: Mathieu moonshine \textup{(}Cheng--Duncan--Harvey
...
whereas the BCOV one-loop lane carries
