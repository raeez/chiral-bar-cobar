...
$\beta\gamma$ (diagonal)
  ...
  & --- \\
\hline
\multicolumn{6}{|c|}{\textit{Class L (Lie/tree): collision residue $r^{\mathrm{coll}}(z) = k\Omega/z$}} \\
\hline
...
\texorpdfstring{$\mathfrak{sl}_2$}{sl_2} via Riordan GF; \ClaimStatusProvedHere]
\label{prop:hred-sl2}
\index{bar complex!reduced bar cohomology!$\mathfrak{sl}_2$}
\index{Picard--Fuchs equation!bar cohomology}
For $\mathfrak{g} = \mathfrak{sl}_2$, the \emph{Riordan} dimension
sequence $a_n = R(n+3)$ satisfies
the $P$-recursive recurrence of order~$2$:
\begin{equation}\label{eq:riordan-recurrence-explicit}
(n+1)\, a_n \;=\; (2n-2)\, a_{n-1} + (3n-6)\, a_{n-2},
\qquad a_1 = 3,\; a_2 = 6.
\end{equation}
\textup{(}The proved bar cohomology value is $\dim H^2 = 5 \neq 6$;
the Riordan GF requires revision, but the discriminant and
$H^{\mathrm{red}}_1$ construction below are independent
of the closed form.\textup{)}
