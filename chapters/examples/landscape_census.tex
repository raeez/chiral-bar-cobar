...
\hline
$\widehat{\fg}_k$ (non-simply-laced: $B_2, G_2, F_4$) & L & 3 & $C$: Lie bracket & \S\ref{sec:B2-details}, \S\ref{sec:G2-details} \\
\hline
...
cyclic line at the inverse limit.  The archetype classifies the shadow depth: at which
...
$\beta\gamma$ (diagonal)
  ...
  & --- \\
\hline
\multicolumn{6}{|c|}{\textit{Class L (Lie/tree): Casimir $r$-matrix $r(z) = k\Omega/z$}} \\
\hline
...
\begin{remark}[$r$-matrix vs the arity-$3$ MC equation]
...
element~$\Theta_\cA$.  It carries only part of the arity-$3$
content of the MC equation: the CYBE on~$r(z)$ follows
from the arity-$3$ MC equation after collision extraction and the
Arnold relation on $\mathrm{Conf}_3(\mathbb{C})$, but the two live
in different spaces.

For class-L algebras (affine Kac--Moody, $r_{\max} = 3$), the
CYBE exhausts the arity-$3$ content.  For class-M algebras
(Virasoro, $\cW_N$, $r_{\max} = \infty$), the arity-$3$ MC
equation carries the homotopy Jacobiator, strictly more than
the CYBE alone.
See Remark~\ref{rem:arity3-vs-cybe} for the full discussion.
\end{remark}
...
\begin{table}[ht]
\centering
\caption{Free energies $F_1 = \kappa/24$ and $F_2 = 7\kappa/5760$ for $15$ standard families}
...
\hline
Leech $V_\Lambda$ & $24$ & $1$ & $\tfrac{7}{240}$ & scalar \\
\hline
\end{tabular}
}% end small
\end{table}
...
all generators of the same conformal weight), for which $F_g = \kappa \cdot
\lambda_g^{\mathrm{FP}}$ is proved at all genera (Theorem~\ref{thm:genus-universality});
``multi'' = multi-weight algebra, for which $F_g = \kappa \cdot \lambda_g^{\mathrm{FP}}$
is proved at $g = 1$ and conditional at $g \geq 2$
(Remark~\ref{rem:propagator-weight-universality}).  The universal ratio
$F_2/F_1 = 7/240$ is independent of~$\cA$.

\begin{table}[ht]
\centering
\caption{Shadow tower invariants: discriminant, shadow coefficients, growth rate}
...
type~$A$, Theorem~\ref{thm:dk-fd-typeA} gives only the corresponding
...
\item[\textbf{DK-core.}] \emph{$\Eone$-Yangian regime}:
  ...
  type~$A$, Theorem~\ref{thm:dk-fd-typeA} only records the matching
...
The following table records the complete data for exceptional and non-simply-laced affine Kac--Moody algebras, all proved instances of the Master Table.  Every entry satisfies $\kappa + \kappa' = 0$ and $c + c' = 2d$ (AP24: the clean anti-symmetric case for Kac--Moody), and every family is class L with shadow depth $r_{\max} = 3$.
\begin{center}
\renewcommand{\arraystretch}{1.3}
...
$B_2 \cong C_2$ & $10$ & $4$ & $3$ & $5(k+3)/3$ & $20$ & $20 = 2\cdot 10$ \\
...
The anomaly ratio $\varrho(\mathfrak{g}) = \sum_{i=1}^r 1/(m_i+1)$ for the associated principal $\mathcal{W}$-algebra (see Remark~\ref{rem:general-w-kappa-values}) satisfies $\varrho(E_8) = 121/126 < 1$: the $E_8$ shadow tower converges at all non-critical levels.  More precisely:
\[
...
(AP23: $H$ has a double zero at $t=0$).
...
\begin{proposition}[Koszul dual Hilbert functions; \ClaimStatusConditional]
