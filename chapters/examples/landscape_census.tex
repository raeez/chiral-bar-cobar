...
\hline
$Y_{1,1,1}[\Psi]$ (corner VOA, $c = 0$) & G & 2 & ---; cubic vanishes ($J$ abelian) & Rem~\ref{rem:y111-class-g} \\
\hline
...
\hline
$\mathcal{N}{=}2$ superconformal $\mathrm{SCA}_c$ & M & $\infty$ & $T$-channel infinite; $J$-channel terminates at arity $2$; $G$-channel at arity $3$ & Rem~\ref{rem:n2-shadow-tower} \\
\hline
...
\hline
Quantum lattice $\Vlat_\Lambda^{N,q}$ (strict $\Eone$, $N \geq 3$) & ?\ & --- & deformed braiding; same character as $V_\Lambda$ but strict $\Eone$-chiral & Thm~\ref{thm:quantum-lattice-structure} \\
...
\hline
Kronecker $K_m$ ($m \geq 3$) & W & --- & bar SS does not collapse; DT invariants replace shadows & Rem~\ref{rem:wild-quiver-class} \\
\hline
\end{tabular}%
}% end resizebox
}% end small
...
(Theorem~\ref{thm:genus-universality}; UNIFORM-WEIGHT);
...
$F_g = \kappa \cdot \lambda_g^{\mathrm{FP}}$ is proved at $g = 1$ and
...
(AP48; Remark~\ref{rem:lattice:monster-shadow}).
...
tower terminates at arity~$2$, $24$ weight-$1$ generators).

The root cause is AP48: the modular characteristic depends on
...
$g = 1, \ldots, 5$ (UNIFORM-WEIGHT; multi-weight requires
...
(UNIFORM-WEIGHT entries; multi-weight entries require the cross-channel
correction $\delta F_g^{\mathrm{cross}}$ of
...
(Table~\ref{tab:genus-obstructions}; UNIFORM-WEIGHT; multi-weight
