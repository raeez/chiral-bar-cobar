...
\hline
Heisenberg $\mathcal{H}_\kappa$ & G & 2 & --- & \S\ref{ch:heisenberg-frame} \\
\hline
...
The anomaly ratio $\varrho(\cA) := \kappa(\cA)/c(\cA)$ measures how much of the conformal anomaly survives from the Lie bracket's cyclic deformation.  For the Heisenberg algebra, $\varrho(\mathcal{H}_\kappa) = 1$: the Polyakov formula $F_1 = c/24$ and the shadow formula $F_1 = \kappa/24$ coincide.  For all other standard families, $\varrho \neq 1$:
\begin{center}
\begin{tabular}{lccc}
Family & $c$ & $\kappa$ & $\varrho = \kappa/c$ \\ \hline
$\mathcal{H}_\kappa$ & $\kappa$ & $\kappa$ & $1$ \\
...
\end{tabular}
\end{center}
The ratio $\varrho(\mathrm{Vir}_c) = 1/2$ means that half the conformal anomaly is absorbed by the nonlinearity of the Virasoro OPE.  For $\widehat{\mathfrak{sl}}_{2}$ at level~$k$, $\varrho = (k{+}2)^2/(4k) \to 1/4$ as $k \to \infty$ and diverges at $k \to 0$.  The genus-$g$ free energy $F_g = \kappa \cdot \lambda_g^{\mathrm{FP}}$ carries the modular characteristic~$\kappa$, not the central charge~$c$.
\end{remark}
...
\Delta_{\mathcal{W}_3}(x) = \bigl(\tfrac{3 + \sqrt{13}}{2}\bigr)(1 - 3x - x^2).
\]
The shared factor $(1 - 3x - x^2)$ is DS-invariant: it survives Drinfeld--Sokolov reduction from $\hat{\mathfrak{sl}}_3$ to $\mathcal{W}_3$, while the dominant eigenvalue changes from $8 = \dim(\mathfrak{sl}_3)$ to $(3 + \sqrt{13})/2$.
\end{proposition}
...
Chapter~\ref{chap:casimir-divisor}) resolves this for
...
order by~$1$.
\end{enumerate}
Together: $r+1$ constraints, giving a Picard--Fuchs D-module
of rank $r{+}1$ whose singular locus is
$\Delta_{\mathfrak{g}}(x)$.
The growth-mode factorization
(Theorem~\ref{thm:growth-mode-factorization}) then decomposes
the Casimir recurrence module into the dominant chain-group
growth mode (eigenvalue $\dim\mathfrak{g}$) and the
DS-invariant spectral core (dimension~$r$).
\end{remark}
