...
\noindent The Schellekens $71$~holomorphic VOAs at $c = 24$
are represented in this master table only by the
Monster~$V^{\natural}$ and Leech~$V_{\Lambda_{24}}$ rows; the
$69$ non-Monster Schellekens algebras carry $c = 24$ with
$\kappa = 24$ uniformly \textup{(}lattice-type bound inherited
from rank\textup{)} but differ in strong-generator content
and in the genus field $\operatorname{ek}(V) \in \{0,1,2,\dots\}$;
a family-by-family row inscription for the full $71$-list is
a frontier task
(Part~\ref{part:v1-frontier};
Remark~\ref{rem:census-silent-non-coverage}).
...
\end{enumerate}
The $\mathcal{N}{=}1,2,4$ superconformal rows, the super-Yangian
row, and the Schellekens $71$~holomorphic VOAs at $c=24$ are
covered in parts and appear in dedicated chapters but are not
re-inscribed in this master table. The sixth Niemeier--Monster
coincidence $c = 24$ and the $71$ Schellekens algebras are
listed only implicitly through the Monster / Leech rows.
\end{remark}
