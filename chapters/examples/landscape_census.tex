...
Kac--Moody algebra on the Feigin--Frenkel level-shift lane. The
universal generating function $x/\sin x$ governs the scalar
Faber--Pandharipande genus series on the proved uniform-weight lane
...
Koszul-closed rows of the standard-landscape census carries a typed
shadow datum: the scalar $\kappa$, the lane on which it is measured,
...
Kac--Moody tree lane (class~L); $S_3=0$, $S_4\ne0$, and
$S_r=0$ for $r\ge5$ is the contact/quartic lane
(class~C); the stress-tensor or higher-spin tower that does not
truncate is class~M. The critical discriminant
$\Delta=8\kappa S_4$ is a downstream scalar on a chosen lane, not by
itself an archetype classifier. Frontier rows whose Koszul-dual or
shadow surfaces are not yet closed are recorded below as explicit
non-coverage rather than folded into this statement.
...
The table below records the completed invariant rows and the fenced
frontier rows treated in Part~\ref{part:physics-bridges}. Read
horizontally, each PH row is a complete portrait of one algebra on its
stated lane; CJ and HF rows display only the typed datum written in
...
\noindent\emph{The Master Table below collects the completed invariants
computed throughout Part~\ref{part:physics-bridges} and fences the
frontier rows whose partner or scalar data are not yet closed. For the
...
\noindent\textbf{Type signature.} Every row carries the four-tuple
\((\text{quadrant},\text{presentation},\text{level},\text{hypothesis package})\):
quadrant Open ($\mathbf G,\mathbf L,\mathbf C,\mathbf M$) or CY
($\mathbf B$); presentation chart-level (level~1) or specialisation-cycle
(level~5); level on the Beilinson tower $\{0,\ldots,5\}$; hypothesis
package the family-specific finiteness, completion, and ambient
choices. CY-derived rows carry the explicit
$(\Sigma_{d-1},C)$-indexed two-stage decomposition
\(\Phi_d^{(\Sigma_{d-1},C)}=\SpCh_{\Sigma_{d-1},C}\circ\PhiFA_d\)
(Master Pattern MA-7); a bare $\Phi_d$ is shadow-level and forbidden.

\begin{table}[ht]
\centering
\caption{The $5\times 5$ $\kappa$-stratification matrix. Each row pins
...
\mathbf H_{\Delta_5}$. Bare $\kappa$ without subscript is forbidden
(Master Pattern MA-6).}
...
``open'' tag records this absence; the heal path is the embedding
$\cA\hookrightarrow\Phi_3^{(\Sigma_2,C)}(\text{CY}_3)$ on a
witnessing CY-$3$ host (when one exists), at which point the row
...
chiral and fibre scalars is licensed (Master Pattern MA-6).
...
\[
 ...
 \EnHolFA(C)
\]
(Master Pattern MA-7; cf.\ Vol.~III
...
and the BCOV-curving lane replaces the Heisenberg-Mukai specialisation.
A bare $\Phi_d$ without $(\Sigma_{d-1},C)$ data is shadow-level and
suppresses the choice of transverse manifold and curve, the
canonical Master-Pattern-MA-7 violation.
...
Koszul dual or explicitly typed same-family partner datum, the central
...
CJ or HF are typed frontier data; their pending columns are not
...
The scalar conductor packages used in this table are the numbers
\[
K^c(\cA,\cA^!)=c(\cA)+c(\cA^!),\qquad
K^{\kappaChHodge}(\cA,\cA^!)=\kappaChHodge(\cA)+\kappaChHodge(\cA^!).
\]
They are defined only on the displayed Verdier scalar lanes. They are
not the averaging morphism $K_\cA=\Av_\cA$, not the chiral Hochschild
derived centre $\DerZ(\cA)$, and not a physical bulk algebra.
The Hodge superscript is essential: the Vol~III BKM conductor
$K^{\kappaBKM}_{\mathbf B} = 2c_+(\mathrm{Mukai}(K3)) = 8$ on the
Borcherds row $\mathbf B$ is a separate invariant on a different lane
\textup{(}Vol~III, Theorem~\ref{V3-thm:humbert-order-K-kappa}\textup{)};
it is not a value of $K^{\kappaChHodge}$.
...
\begin{table}[ht]
\centering
\caption{Computed invariants and fenced frontier rows for chiral algebras}
...
\multicolumn{6}{|c|}{\textit{Affine Kac--Moody
 \textup{(}non-critical universal lane; Feigin--Frenkel level shift $k \mapsto -k - 2h^\vee$\textup{)}}} \\
\hline
...
Non-rational / indefinite lattice $V_L$
 ...
 & signature-typed lattice partner pending
 & $\operatorname{sig}(L)=(p,q)$; lane must be specified
 & pending
 & pending & HF \\[4pt]
\hline
Universal $V_k(\fg)$ restricted to $k + h^\vee = p/q$
 \textup{(}$\gcd(p,qh^\vee) = 1$, $p,q \ge 1$\textup{)}
 & $V_{-k-2h^\vee}(\fg)$ on the reflected universal lane
 & $\dfrac{kd}{k + h^\vee}$ & $2d$
...
\begin{itemize}[leftmargin=*]
 ...
 self-duality convention; the Mukai row below is the pinned example
...
level recorded here. CJ and HF rows record the typed scalar or
...
The $\Eone$ and frontier rows are typed separately and are not read as
...
refined spectral lane admits depths
...
positive-definite lattice VOA the scalar oscillator lane has
...
\ClaimStatusProvedHere
The completed vertex-algebra rows of
Table~\textup{\ref{tab:shadow-tower-census}} that lie in the
averaged-shadow conductor domain are partitioned as
\[
...
\{0,\,13,\,250/3,\,98/3\}
\]
on the classical $G/L/C/M$ witness lanes with the two
...
\item the completed M-level comparison packages are proved,
 given by
...
comparison package; (viii) records chiral Hochschild concentration
...
$F_1 = \kappa/24$ is the genus-$1$ evaluation
...
On the scalar lane, the anomaly ratio
...
formula $F_1 = \kappa/24$ coincide because $\kappa = c = d$. For all
...
universal lane, all proved instances of the Master Table. Every entry
...
instantiates the four main theorems on its stated lane:
...
\begin{proposition}[Koszul dual Hilbert functions under finite-character data]
...
\begin{proof}
For unmarked rows, $\dim(\cA^!)_n$ is computed from the
...
\begin{corollary}[Sub-exponential growth in the computed rows]
\label{cor:subexp-free-field}
\index{bar complex!sub-exponential growth}
Among the rows of Table~\textup{\ref{tab:bar-dimensions}} whose Hilbert
functions are computed from bar complexes, the Koszul dual Hilbert
function $\dim (\cA^!)_n$ grows sub-exponentially exactly for the
free fields (Heisenberg or free fermion) and affine
...
\begin{corollary}[Closed forms for computed interacting rows]
\label{cor:algebraicity-koszul}
\index{bar complex!generating function!algebraicity}
Every interacting row of Table~\textup{\ref{tab:bar-dimensions}} with a
closed form proved from the bar complex has a rational or quadratic
algebraic generating function over~$\mathbb{Q}(x)$.  The marked
...
The interacting rows proved directly here
($bc$, $\beta\gamma$, $\widehat{\mathfrak{sl}}_2$, Virasoro)
therefore have rational or quadratic algebraic generating functions.
...
linear growth (Remark~\ref{rem:garland-lepowsky-sl2}).
\end{proof}

\begin{theorem}[Rank-one discriminant chart and finite-character denominator data]
...
\begin{theorem}[Divisor-core form of DS sub-discriminance]
...
\begin{proposition}[A2 divisor-core calculation]
...
\texorpdfstring{$\mathfrak{sl}_2$}{sl_2}]
\label{prop:hred-sl2}
...
\begin{theorem}[Bar cohomology generating-function chart]
...
\begin{proposition}[Discriminant as a finite characteristic invariant]
...
\begin{theorem}[Linear dependence in the rank-one branch family]
...
\begin{proof}
The three branch functions~\eqref{eq:gf-sl2-riordan}--\eqref{eq:gf-betagamma}
...
\begin{proposition}[Bridge obstruction for a coefficient-level relation]
...
The two structural maps therefore impose the following bridge data:
\begin{enumerate}[label=\textup{(\roman*)}]
...
\begin{proposition}[Pole decomposition and singularity type]
...
\begin{lemma}[Degree-\texorpdfstring{$2$}{2} bar cohomology at lowest weight]\label{lem:bar-deg2-symmetric-square}
...
\begin{corollary}[Exponential growth rate in a finite-character Kac--Moody chart]
...
\begin{theorem}[Dominant branch point in a finite-character Kac--Moody chart]
...
On the scalar Hodge lane of Theorem~\ref{thm:quantum-complementarity-main},
...
\begin{enumerate}[leftmargin=*]
 ...
 compatibility with chiral Koszul duality on the universal lane. The
...
only the scalar Ito--Tanaka lane recorded in its scope note. The
...
\begin{proposition}[K3/BKM scalar package and Hall--Drinfeld taxonomy]
\label{prop:landscape-k3-bkm-taxonomy}
\ClaimStatusProvedElsewhere
The K3$\times E$ Borcherds row contributes the scalar package
\[
...
Hall--Drinfeld double. The taxonomy and scalar packages are therefore
disjoint.
\end{proof}
