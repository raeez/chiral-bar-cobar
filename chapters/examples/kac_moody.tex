...
$\Theta = \kappa \cdot \eta \otimes \Lambda$.  But the strict model
$\Convstr$ is not trivially formal --- there is a cubic shadow at
weight~$3$, the shadow of the Lie bracket itself, which dies at the
next stage because the quartic obstruction $o_4$ vanishes by the
Jacobi identity.  The tower terminates at $r = 3$.  At critical level
...
(iv)~genus tower with $\kappa = k + h^\vee$, Lie/tree shadow
