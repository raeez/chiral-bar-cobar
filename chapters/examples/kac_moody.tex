...
algebra.  The derived chiral centre is the Hochschild cochain bulk
object, not the Koszul dual.  The five theorem-surface statements below
...
Beilinson level~$5$; HP = five distinct measurements named, with
...
data; collapse pattern $r_{\max}=3$).
The five entries are five distinct invariants per family
(MA-6 discipline); their collapse pattern under averaging is the
...
(diverging at $k=-h^\vee$ -- the universal chain-homotopy
denominator coincides with the Sugawara denominator).
...
Hochschild cochain bulk object; at critical level its degree-zero
...
(\S\ref{sec:frame-koszul-dual}): the dual algebra changes type.
...
enlarged Feigin--Frenkel center in the center/Hochschild lane. They
...
class~\(\mathsf G\), and it does not upgrade admissible or critical
...
The comparison cohomology dimensions
(Table~\ref{tab:bar-dimensions}) are significantly smaller
than the chain-group dimensions and exhibit polynomial growth for
Koszul algebras.
\end{lemma}
...
\begin{remark}[Low-degree computation and proof obligation]
...
$\{-3, \ldots, 3\}$, hence \(V_3\).
The all-degree proof obligation is an identification of \(H^n\)
...
$\mathrm{obs}_g = \kappa \cdot \lambda_g$ for every $g \geq 1$
...
\(\zeta_k \mapsto \zeta_k^{-1}\). This is~(iii). See Remark~\ref{rem:km-categorical-refinement}
...
adjunction's Verdier intertwining; and a coderived comparison with
quantum groups would have to explain how the curvature variable becomes
the root-of-unity parameter. We state these as precise conjectures with
explicit hypotheses, separated from the proved theorems above.
...
\begin{remark}[Proof obligations for the KL lift]\label{rem:kl-evidence}
...
Three proof obligations remain.  First, the root-of-unity parameter
...
FLE (Raskin~\cite{Raskin24}, \ClaimStatusProvedElsewhere)
...
proof~\cite{GLC24}. Combined with the bar-Ext identification
(Corollary~\ref{cor:bar-computes-ext}), one obtains
$H^n_{\mathrm{bExt}}(\widehat{\fg}_{-h^\vee})
...
(\S\ref{sec:sl2-genus-one-pipeline}) shows that modular forms enter
...
identification is the open proof obligation; it is not a formal
...
is expected to intertwine the level-rank substitution
