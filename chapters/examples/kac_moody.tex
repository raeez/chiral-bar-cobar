...
\emph{minimal scalar package}
$\Theta^{\min} = \kappa \cdot \eta \otimes \Lambda$.  But the strict model $\Convstr$ is not trivially formal in the
...
on the proved scalar lane, with
...
\textup{(}minimal scalar package on the proved scalar lane\textup{)},
