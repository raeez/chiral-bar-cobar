...
the full bar cohomology.  At critical level the scalar class vanishes,
but this degeneration does not identify the full higher-arity shadow tower; for
$\mathcal{W}$-algebras
...
\item \emph{Level fixed point.} The critical level $k = -h^\vee$ is a fixed point of the level-shifting map $k \mapsto -k-2h^\vee$; the Koszul dual $\bar{B}(\widehat{\mathfrak{g}}_{-h^\vee})^\vee$ is uncurved ($m_0 = 0$ since $\kappa(-h^\vee) = 0$), and bar-cobar inversion $\Omega(B(\widehat{\mathfrak{g}}_{-h^\vee})) \xrightarrow{\sim} \widehat{\mathfrak{g}}_{-h^\vee}$ recovers the algebra (Theorem~B).
\end{enumerate}
\end{theorem}
