...
algebras occupy the Lie/tree archetype: intermediate
between the Gaussian families (Heisenberg, lattice, free fermion;
$r_{\max} = 2$) and the contact/quartic archetype of
$\beta\gamma$ ($r_{\max} = 4$).  At generic level
...
$\Theta^{\min} = \kappa \cdot \eta \otimes \Lambda$.  The strict model $\Convstr$ is not trivially formal: a cubic shadow at arity~$3$, the
shadow of the Lie bracket, survives, but dies at the next stage
because the Jacobi identity kills $o_4$.
The tower terminates at $r = 3$.  At the homotopy level,
...
all are projections of the single geometric duality on the Fulton--MacPherson
compactification.
...
For the Heisenberg algebra, Koszul duality sends the Lie-type $\mathcal{H}_k$ to the Com-type $\mathrm{Sym}^{\mathrm{ch}}(V^*)$ (\S\ref{sec:frame-koszul-dual}): the dual algebra changes type.  For non-abelian Kac--Moody algebras, Koszul duality shifts the level instead: the dual remains $\widehat{\mathfrak{g}}$ but at level $k' = -k - 2h^\vee$.
