...
scalar-normalization conventions of
Appendix~\ref{sec:scalar-normalization-appendix}.  The
...
Three structures meet at the shifted parameter $k+h^\vee$.  Verdier
duality on the bar coalgebra gives the algebra
$K_X(V_k(\fg))=\mathbb D_{\Ran}B_X(V_k(\fg))$.  The affine-family
comparison is the additional map
\[
 \nu_k^{\mathrm{aff}}\colon
 K_X(V_k(\fg))\xrightarrow{\sim}V_{-k-2h^\vee}(\fg).
\]
Under its finite-window, pairing, completion, and factorization
hypotheses, this map realizes the Feigin--Frenkel level involution.
...
H (chiral Hochschild) &
 A family datum $H_H(\widehat{\fg}_k;S)$ transports the complete
 chart complex to an $S$-supported model; the bounded affine
 calculation supplies conjectural BDSK upper bounds
 & Conditional family support; bounded affine bounds conjectural &
...
Sugawara singularity, and the critical-level reflection has its fixed
point at the same denominator that drives the KZ connection
(Theorem~\textup{\ref{thm:universal-kac-moody-koszul}}).
The critical-level reflection \(k \mapsto -k - 2h^\vee\) records the
level parameter of the affine scalar companion. Verdier duality acts
on the bar coalgebra and then passes through the chiral CE dual; it is
not the bare level relabelling. The
...
$5\times 5$ stratification matrix are read at level
$k\neq -h^\vee$ as follows.  The trace-datum scalar and the
Heisenberg coordinate are
\[
...
\]
the second by the Cartan-oscillator specialisation in the
level-weighted convention of the census matrix head
(a rank-$r$ Heisenberg at level $k$ carries
$\kappa^{\mathrm{Heis}}_{\mathrm{ch}}=rk$).  The coordinates
$\kappa_{\mathrm{cat}}$ and $\kappa^{\mathrm{Hodge}}_{\mathrm{ch}}$
are functions of a compact CY host
($\chi(\cO_X)$ and $\Xi(X)$ respectively); their evaluation on
row~$\mathsf{L}$ is the open realization problem
$\diamond_{V_k(\fg)}$ of the census matrix.  The BKM and fibre
measurements $\kappa_{\mathrm{BKM}}, \kappa_{\mathrm{fiber}}$
require a Borcherds input and a fibre datum; on
class~$\mathsf{L}$ outside the lattice locus where
$\widehat{\fg}_k$ embeds in a Borcherds--Kac--Moody algebra no
such input is specified, and these entries are open.
This is an Open-quadrant chart-level scalar tuple at Beilinson
level~$5$. It uses five distinct measurements:
$\kappa_{\mathrm{cat}}$ the categorical Euler scalar of a host,
...
of equation~\eqref{eq:kac-moody-koszul-basic}.  Suppose the
family construction supplies maps $(i_L,p_L,h_L)$ on this complex
with
\[
d h_L+h_Ld
=2(k+h^\vee)\bigl(\mathrm{id}-i_Lp_L\bigr).
\]
For $2(k+h^\vee)\in\Bbbk^\times$, the resulting contraction uses
$[2(k+h^\vee)]^{-1}h_L$.  The critical fibre has scalar
$2(k+h^\vee)=0$ and belongs to the centre-rich completed lane with
its own contraction datum.
...
Hackney--Robertson--Yau~\cite[Theorem~5.3]{HRY17} construct the
model category of simplicial properads.  The factorization-enriched
extension belongs to the transfer package $H_{\mathrm{prop}}$ of
Theorem~\ref{thm:factorization-properad-transfer}.  Under that package,
connected-graph extension carries the associative bar--cobar pair on
$V_k(\fg)$ to its properadic envelope.
\end{itemize}
The four presentations coincide on the generic Koszul locus (where
...
The affine comparison $\nu_k^{\mathrm{aff}}$ realizes the
Feigin--Frenkel shift $k\mapsto-k-2h^\vee$.  Substitution in
\(q=e^{\pi i/(k+h^\vee)}\) gives (q\mapsto q^{-1}).  A WZW
orientation-reversal statement additionally requires the full-CFT
sewing comparison; a modular-$S$ statement requires the character
representation of $\mathrm{SL}_2(\mathbb Z)$; a four-dimensional
duality statement requires the compactification functor.  These are
three comparison theorems attached to the same shifted parameter.
...
\textup{(}; all currents $J^a$ have conformal weight~$1$, and
$H_D^K$ gives
$\operatorname{ch}_g(\mathfrak O_g^K)=(-1)^g\kappa\lambda_g$
on this surface\textup{)};
...
\begin{proposition}[Affine standard-family computation ledger;
\ClaimStatusProvedHere]
\label{prop:affine-standard-family-ledger}
\index{affine Kac--Moody algebra!standard-family ledger}
\index{bar complex!affine degree two}
Let \(\fg\) be simple, \(d=\dim\fg\), and let
\(\kappa_{\fg}\) be the normalized invariant form, with long roots of
square~\(2\).  Choose a \(\kappa_{\fg}\)-dual current basis and write
\(\kappa_{\fg}^{ab}\) for the inverse matrix.  For the non-critical
generic affine chiral algebra \(V_k(\fg)\), \(k\neq -h^\vee\), the
current OPE is
\begin{equation}\label{eq:affine-ledger-ope}
J^a(z)J^b(w)
\sim
...
\end{equation}
In the ordered bracket projection of the chiral bar complex, the
degree-two differential is
\begin{equation}\label{eq:affine-ledger-bar-d2}
d_{\Barch}[\susp^{-1}J^a\,|\,\susp^{-1}J^b]
=
f^{ab}{}_{c}[\susp^{-1}J^c],
\end{equation}
as in Equation~\ref{eq:km-bar-d2}.  In the curved symmetric/CE
presentation the same OPE also carries the double-pole scalar through
\[
d_k=d_{\mathrm{sp}}+k\,d_{\mathrm{dp}}
=d_{\mathrm{crit}}+(k+h^\vee)\delta,\qquad
d_k^{\,2}=
\left[\frac{k+h^\vee}{2h^\vee}\mathsf C_{\fg},-\right]
\]
\textup{(}Theorem~\ref{thm:km-bar-bicomplex}\textup{)}.  Thus the
ordered formula records the simple-pole Lie bracket, while the
curved presentation records the shifted scalar curvature; these are
two presentations of the same current OPE data, not two definitions
of the affine object.

The cubic shadow is the cyclic Lie tensor
\[
\mathfrak C_{\mathrm{aff}}(x,y,z)
=\kappa_{\fg}(x,[y,z]),
\]
after the \(1/3!\)-normalisation of
Proposition~\ref{prop:affine-cubic-ad-invariance}.  The quartic
obstruction is
\[
\mathfrak o^{(4)}_{\mathrm{aff}}
=\tfrac12\{\mathfrak C_{\mathrm{aff}},
\mathfrak C_{\mathrm{aff}}\}_{H_{\mathrm{aff}}}=0
\]
by the Jacobi mechanism of
Proposition~\ref{prop:affine-jacobi-quartic-vanishing}.  Hence the
affine shadow tower terminates at
\[
r_{\max}\bigl(V_k(\fg)\bigr)=3
\]
on the strict current-level sector
\textup{(}Corollary~\ref{cor:affine-postnikov-termination}\textup{)}.

The collision and monodromy normalizations are distinct:
\begin{equation}\label{eq:affine-ledger-r-data}
...
\end{equation}
The algebra-level Verdier/Feigin--Frenkel partner is obtained by the
level reflection
\[
k^{\mathrm{FF}}=-k-2h^\vee ,
\]
and the scalar conductor is
\[
\kappa(V_k(\fg))=\frac{d(k+h^\vee)}{2h^\vee}.
\]
Consequently
\begin{equation}\label{eq:affine-ledger-kappa-sum}
K^\kappa(V_k(\fg))
=\kappa(V_k(\fg))+\kappa(V_{-k-2h^\vee}(\fg))=0,
\end{equation}
which is Proposition~\ref{prop:kappa-anti-symmetry-ff} in this
normalization.

At \(k=-h^\vee\) the generic ledger stops: the Sugawara and KZ
denominators in \eqref{eq:affine-ledger-r-data} are singular, the
generic finite-window derived-centre computations used off the
critical hyperplane do not apply, and the
degree-zero centre is the infinite-dimensional Feigin--Frenkel centre
\[
Z(V_{-h^\vee}(\fg))
\cong \mathcal O(\mathrm{Op}_{\fg^\vee}(D))
\]
of Theorem~\ref{thm:critical-level-structure}.  This critical centre
is a separate completed critical-level object; it is not obtained by
specializing the off-critical scalar centre \(\mathbb C\).
\end{proposition}
...
\item The critical-level reflection fixes the level:
...
A^i&=&C_X(s^{-1}V,s^{-2}R),\\[2pt]
H^\bullet\barB(A)&=&
H^\bullet\!\bigl(\barB(V_{-h^\vee}(\fg))\bigr),\\[2pt]
K_X(A)&=&\mathbb D_{\Ran}\barB(A),\\[2pt]
...
\]
The comparison
$q_A\colon A^i\to\barB(A)$ carries quadratic recognition;
$\mathbb D(q_A)$ and the pair map
$\nu_A\colon K_X(A)\to A^!$ carry the dual comparison.
Bar-cobar inversion recovers $A$ from $\barB(A)$.  The derived centre is the
...
Remark~\ref{rem:admissible-koszul-status}: the
$L_k(\mathfrak{sl}_2)$ admissible simple-quotient lane has strong
finite Shapovalov/character evidence but remains conditional until the
quotient-bar spectral-sequence package is supplied, while
...
\begin{corollary}[Rank-one admissible BRST resolution under the
quotient-bar package]
\label{cor:wakimoto-brst-admissible}
\ClaimStatusConditional
\index{admissible level!Wakimoto resolution}
Assume the rank-one admissible quotient-bar package
$H_{\mathrm{adm},\mathfrak{sl}_2}^{\mathrm{qbar}}$: the quotient bar
spectral sequence for $L_k(\mathfrak{sl}_2)$ is constructed in a
finite-window complete ambient, the PBW/Shapovalov filtration detects
all higher differentials, and the finite windows converge strongly.
Then, for every admissible level $k=-2+p/q$ with $p\geq2$,
$q\geq1$, and $\gcd(p,q)=1$, the admissible Wakimoto screening complex
resolves the simple quotient $L_k(\mathfrak{sl}_2)$:
\[
...
\begin{proof}
Under $H_{\mathrm{adm},\mathfrak{sl}_2}^{\mathrm{qbar}}$, the
rank-one quotient spectral sequence is the admissible
$\mathfrak{sl}_2$ comparison of
Remark~\ref{rem:admissible-koszul-status}. The explicit $k=-1/2$
character calculation supplied by Theorem~\ref{thm:kw-bar-spectral}
checks the finite Euler-characteristic surface. The screening relation
$(Q_{\mathrm{BRST}}^{(k)})^2=0$ follows from the rank-one quantum
Serre relation; positive-degree exactness of the quotient complex is
the additional hypothesis package, not a consequence of the
Kac--Wakimoto character formula alone.
\end{proof}
...
\textup{(}Remark~\textup{\ref{rem:garland-lepowsky-sl2}})
holds in the finite window \(1\leq n\leq10\) by exact rank calculation
on the weight-graded Chevalley--Eilenberg differential.  The
...
$\{-3, \ldots, 3\}$, hence \(V_3\).
The finite rank calculation does not determine the all-degree module
structure.  The required mathematical condition is an identification of \(H^n\)
...
surface of affine currents, the higher-genus signed Hodge component is
...
\(\operatorname{obs}^{\mathrm{Hdg}}_g(\widehat{\fg}_k)
=(-1)^g\kappa(\widehat{\fg}_k)\lambda_g\) for every $g \geq 1$
...
where $K^{(1)}_{\mathrm{hol}}(z|\tau) = \theta_1'(z|\tau)/\theta_1(z|\tau)$
is the holomorphic coefficient of the genus-$1$ propagator with
$A$-periodicity~\eqref{eq:A-cycle-periodicity} and
$B$-quasi-periodicity~\eqref{eq:B-cycle-quasi-periodicity}:
$K^{(1)}_{\mathrm{hol}}(z+\tau|\tau) = K^{(1)}_{\mathrm{hol}}(z|\tau) - 2\pi i$.
The descended one-form adds the Arakelov coefficient cancelling this
shift, as in~\eqref{eq:genus1-propagator}.
...
critical-level reflection, and bar-cobar inversion recovers the same
algebra:
\begin{equation}
...
critical-level reflection fixes the critical level. \qedhere
\end{proof}
...
\subsection{Conformal-block comparison for Kac--Moody examples}
...
By Proposition~\ref{prop:conformal-blocks-bar}, after the pointed-bar
exactness and TUY comparison package is supplied, the space of
...
$\overline{C}_{n+r}(\mathbb{P}^1)$, and on the comparison lane its
degree-$0$ target has the Verlinde dimension. Since
$\overline{C}_n(\mathbb{P}^1)$ is a smooth projective variety
of dimension $n - 3$ (for $n \geq 3$) with known Betti numbers,
the bar complex gives a finite geometric model for the comparison; it
does not replace the TUY/Verlinde theorem without the exactness and
...
Remark~\ref{rem:conformal-blocks-koszul} gives a finite-type
Verdier-dual bar-comodule statement. It does not say that the
positive-level Verlinde category is transported unchanged under
$k \mapsto -k - 4$: for $k\ge0$ the reflected level is negative, so
there is no second non-negative integrable-level Verlinde category on
that reflection. Classical level-rank duality is a different
categorical comparison~\cite{NakanishiTsuchiya}.
\end{example}

\begin{theorem}[Verlinde recovery through the Kac--Moody pointed-bar
...
Type signature: \textup{(}Open quadrant, Kac--Moody TUY target with
pointed-bar comparison presentation, Beilinson levels~$2$ and~$5$,
hypothesis package: positive integral affine level, integrable-module
truncation, TUY factorization, modular \(S\)-matrix diagonalization,
determinant-anomaly matching, and the pointed-bar comparison package of
Proposition~\textup{\ref{prop:conformal-blocks-bar}}\textup{)}.
For $V_k(\mathfrak{g})$ at integrable level $k \in \mathbb{Z}_{>0}$,
the TUY conformal-block rank, and hence the genus-$g$ pointed-bar
\(H^0\) dimension on the comparison lane, equals the Verlinde
dimension:
...
(pointed-bar \(H^0\) compared with conformal blocks under its stated
package), Zhu's theorem
...
Verlinde dimension.
For the no-insertion theory, $Z_0(\mathfrak g,k)=1$ by unitarity of
the first row of the modular \(S\)-matrix; at genus~$1$,
...
For $\fg = \mathfrak{sl}_2$ the finite Shapovalov/character evidence
is strong, but promotion to simple-quotient Koszulness remains
conditional on the rank-one quotient-bar package
...
For $\fg = \mathfrak{sl}_2$, the finite tests are compatible with
Koszulness, but the gap is closed only after the rank-one
quotient-bar package named in
...
counterexample to the higher-rank obstruction statement. At rank~$1$
there is only one Cartan direction, and the $k = -1/2$ analysis
\textup{(}Example~\ref{ex:sl2-admissible},
Theorem~\ref{thm:kw-bar-spectral}\textup{)} supplies finite
character-level evidence for absorption of the rank-one class. A
uniform simple-quotient Koszul theorem for all admissible
$\mathfrak{sl}_2$ levels still requires the quotient-bar package of
Remark~\ref{rem:admissible-koszul-status}.
...
the inequality to $\mathrm{rk}(\fg) \geq 3$\textup{)}.  Equality for
the admissible family $k=-3+p/q$, $q\geq3$, remains the conjectural
strengthening stated above.
\end{remark}
...
The holomorphic theta quotient entering the genus-$1$ propagator is
(Remark~\ref{rem:wp-config-space}):
\begin{equation}\label{eq:genus1-propagator}
K_{\mathrm{hol}}^{(1)}(z,w|\tau)
=
\frac{\theta_1'(z-w|\tau)}{\theta_1(z-w|\tau)} .
\end{equation}
where $\theta_1$ is the Jacobi theta function. This coefficient has a simple pole at $z=w$ with residue~$1$, and satisfies:
\begin{align}
K_{\mathrm{hol}}^{(1)}(z+1,w|\tau) &= K_{\mathrm{hol}}^{(1)}(z,w|\tau) \label{eq:A-cycle-periodicity} \\
K_{\mathrm{hol}}^{(1)}(z+\tau,w|\tau) &= K_{\mathrm{hol}}^{(1)}(z,w|\tau) - 2\pi i \label{eq:B-cycle-quasi-periodicity}
\end{align}
The $A$-cycle periodicity~\eqref{eq:A-cycle-periodicity} is exact, but the $B$-cycle quasi-periodicity~\eqref{eq:B-cycle-quasi-periodicity} introduces the \emph{constant} shift $-2\pi i$. The normalized one-form is
\[
\eta^{(1)}(z,w|\tau)
=
\left[
K_{\mathrm{hol}}^{(1)}(z,w|\tau)
+2\pi i\,\frac{\operatorname{Im}(z-w)}{\operatorname{Im}\tau}
\right](dz-dw).
\]
The correction cancels the $B$-cycle shift and its $\bar\partial$
supplies the Arakelov curvature term computed below.
...
criterion detects the first null vector.  Koszulness of the simple
quotient is the additional bar-complex problem, in which the null
vectors contribute extension data.
...
$E_\tau$ with holomorphic propagator coefficient
$K^{(1)}_{\mathrm{hol}} = \theta_1'/\theta_1$
...
\]
for all $n \geq 0$. See
Theorem~\ref{thm:oper-bar-dl} for the typed formal-disc proof.
\end{proof}
...
\item \emph{Critical-level reflection:}
 $k \mapsto k' = -k-2h^\vee$.
...
obtained by the critical-level reflection
...
The critical-level reflection $k \mapsto -k - 2h^\vee$ is comparable
...
\begin{proposition}[Kappa anti-symmetry under the critical-level reflection]
\label{prop:kappa-anti-symmetry-ff}
\ClaimStatusProvedHere
\index{critical-level reflection!kappa anti-symmetry}
\index{kappa@$\kappa$!critical-level reflection anti-symmetry}
...
It is not the critical-level reflection: level-rank changes the
finite Lie algebra, while the reflection keeps \(\fg\) fixed and
...
Under the critical-level reflection
$k \mapsto k' := -k - 2h^\vee$:
\[
...
critical-level reflection for a fixed simple Lie algebra, and the case
