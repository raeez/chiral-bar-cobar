...
\section{Synthesis}
\label{sec:km-wave13-DNA}

\begin{remark}[Kac-Moody and BKM: Hall-Drinfeld double at K3 x E]
\label{rem:km-DNA-K3xE}
Beyond Kac-Moody $\widehat{\mathfrak{g}}_k$, the Vol~III Wave~13 K3 chiral bialgebra arises from the generalised Borcherds-Kac-Moody superalgebra $\mathfrak{g}_{\Delta_5}$ on $\Lambda^{2,1}_{II} \subset \Lambda^{3,2}$. Since BKM lacks a Kac-Moody Cartan, Weyl action on imaginary simple roots, and J-presentation, the quantum-group deformation is \emph{not} a Drinfeld Yangian: it is the K3 chiral Hall--Drinfeld double $\mathcal{D}_\hbar(\mathcal{Y}^{\mathrm{Hall}}_\hbar(\mathrm{CoHA}_{K3 \times E}))$ with Schiffmann--Vasserot $2012$ shuffle (Davison $2017$ CY-$3$ extension). Manin pair not triple on signature-$(2,1)$ Cartan.
\end{remark}

\begin{remark}[Frenkel-Kac closure at K3 Fock]
\label{rem:km-DNA-frenkel-kac-K3}
