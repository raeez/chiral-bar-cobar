...
nonabelian world arrives.  This is not merely the next example in a
list; it is a whole geometric universe of currents, levels, screening
operators, and dual centers, unified by the modular Koszul engine of
Part~I.  The Feigin--Frenkel level involution
...
Frenkel--Gaitsgory --- all of these are successive faces of a single
Maurer--Cartan package.  The bar-cobar adjunction
(Theorem~\ref{thm:bar-cobar-verdier}) does not merely
\emph{accommodate} the Feigin--Frenkel shift: it \emph{explains} it as
Verdier duality on the Fulton--MacPherson compactification.  The
resulting picture is richer than any single aspect suggests, and the
density of interconnections is the payoff for the abstract machinery
developed in Part~I.
...
the full bar cohomology.  This critical-level degeneration is the
prototype of the phenomenon that, in the $\mathcal{W}$-algebra
setting (Chapter~\ref{chap:w-algebra-koszul}), gives rise to infinite
shadow towers.

The motivating
observation is that the Feigin--Frenkel level shift
$\kappa \mapsto -\kappa - 2h^\vee$ --- discovered through
representation-theoretic methods --- is precisely Verdier duality on
...
The bar-cobar framework unifies all of these manifestations: they are
shadows of a single geometric duality acting on the Fulton--MacPherson
compactification.  The identification is proved below for
$\mathfrak{sl}_2$ and $\mathfrak{sl}_3$, with explicit bar complexes,
Koszul duals, and genus-$1$ curvature classes.
...
At level~$k$ with $\kappa=3k/(k+2)$, the genus-$1$ deformation
...
\begin{remark}[What \texorpdfstring{$\widehat{\mathfrak{sl}}_2$}{sl-hat_2} reveals]\label{rem:sl2-portrait}
The $\mathfrak{sl}_2$ computation exhibits all three structural features of Kac--Moody Koszul duality in their simplest form:
\begin{enumerate}
\item \emph{Level shift as curvature.}  The double pole $k/(z-w)^2$ in $e(z)f(w)$ creates curvature $m_0 \propto (k+h^\vee)\kappa$ in the bar complex.  The cobar construction inverts this curvature, producing the dual level $-k-2h^\vee = -k-4$.  This is the chain-level mechanism behind the Feigin--Frenkel involution.
...
\item \emph{Central charge non-cancellation.}  Unlike the $\beta\gamma$/$bc$ pair (Proposition~\ref{prop:betagamma-bc-koszul-detailed}), the central charges satisfy $c(k) + c(-k-4) = c_{\mathrm{crit}}$ rather than zero: curvature prevents cancellation.
\end{enumerate}
\end{remark}
...
We now unfold the proof into explicit computations: the OPE data
determining the cyclic slice, the cubic shadow via ad-invariance,
the Jacobi-identity mechanism killing the quartic obstruction,
explicit channel-by-channel sewing for $\mathfrak{sl}_2$, and the
genus loop vanishing by antisymmetry.
...
This is the algebraic reflection of the fact that the Lie
...
adjunction, \S\ref{sec:bar-cobar-adjunction}) seen through physics
...
The most striking structural consequence of the gauge-theoretic
...
\begin{corollary}[Bar-complex intertwining]
...
\textup{(Theorem~A, \S\ref{sec:bar-cobar-adjunction})} intertwining
Verdier duality.
\end{corollary}
