...
$Q_c$ of corank one: the cubic shadow $\mathfrak{C}(x,y,z) = \kappa^{\mathrm{KM}}(x,[y,z])$
...
collision $r$-matrix $r^{\mathrm{KM}}(z)$ are cross-checked against
...
trace-form normalisation $r(z) = k\Omega/z$ (vanishing at $k = 0$)
and the KZ normalisation $r(z) = \Omega/((k+h^\vee)z)$ (whose
...
Fulton--MacPherson compactification. All five main
...
\end{equation}
where $\Omega = \sum_a J^a \otimes J_a$ is the Casimir element, and the
$r$-matrix is recorded in the KZ normalisation
...
\index{Kac--Moody algebra!as modular Koszul family (unconditional)}
...
level (the bar curvature $m_0 = (k+h^\vee)\dim\fg/(2h^\vee)\cdot\mathsf C$
vanishes while the Sugawara central charge diverges; the limits
commute only after separate regularisation). The modular
characteristic $\kappa = (k+h^\vee)\dim\fg/(2h^\vee)$
...
\subsection{Wakimoto BRST exactness on the full non-degenerate locus}
\label{subsec:wakimoto-brst-nondegenerate}

The Koszul-dual triangle at $k = -h^\vee$ is a shadow of a stronger
statement valid for every non-critical level. Let
\[
...
level $k$) on the \emph{entire} non-degenerate locus
$k\ne -h^\vee$, or only in the asymptotic regimes $|k|\to\infty$
and $k\to\mathrm{admissible}$?

\begin{definition}[Non-degenerate locus]
\label{def:wakimoto-nondegenerate-locus}
\index{non-degenerate locus!Wakimoto}
The \emph{non-degenerate locus} of Kac--Moody levels is
\(
\mathcal{L}^{\mathrm{nd}}_{\mathfrak{g}}
\;\coloneqq\;\{k\in\mathbb{C}\;:\;k\ne -h^\vee\},
\)
the complement of the critical level in the complex $k$-line. It
contains the admissible levels $k + h^\vee = p/q$ with
$\gcd(p,q)=1$ and $p\geq h^\vee$ as a countable dense subset, and the
generic level $k\in\mathbb{C}\setminus\mathbb{Q}$ as its complement.
\end{definition}

\begin{theorem}[Wakimoto BRST exactness on the full non-degenerate
locus; \ClaimStatusProvedHere]
\label{thm:wakimoto-brst-full-nondegenerate}
\index{Wakimoto BRST resolution!full non-degenerate locus}
\index{Feigin--Frenkel!BRST resolution}
For every $k\in\mathcal{L}^{\mathrm{nd}}_{\mathfrak{g}}$, the
...
bosonisation of $\widehat{\mathfrak{g}}_k$ chain-level at every
non-critical level.
\end{theorem}
...
spectral-sequence argument propagating exactness from the asymptotic
regime to the full non-degenerate locus.
...
(Feigin--Frunkel \cite{FeiginFrenkel90bosonization},
...
Away from the critical level, every
...
\]
vanishes precisely at the admissible levels
$k + h^\vee = p/q$. On the complement of this countable union of
hyperplanes the Verma modules are irreducible, and the Wakimoto
module $W_k(\mathfrak{g})$ is isomorphic to a Verma module of
highest weight~$0$. For every $k$ with
$k + h^\vee\notin\mathbb{Q}$, the map
...
\emph{Step 4: Spectral-sequence propagation to all of
$\mathcal{L}^{\mathrm{nd}}_{\mathfrak{g}}$.}
...
identically. Therefore $H^n(F^{\leq N}) = 0$ for $n>0$ and every
$k\in\mathcal{L}^{\mathrm{nd}}_{\mathfrak{g}}$.
...
\begin{remark}[Admissible levels are not an obstruction]
\label{rem:wakimoto-admissible-robust}
The spectral-sequence argument passes through the admissible
hyperplanes $k + h^\vee\in\mathbb{Q}_{>0}$ without difficulty: the
Kac--Kazhdan determinant vanishes on these hyperplanes, but the
BRST cohomology does not. The reason is that on admissible levels
the Wakimoto module $W_k(\mathfrak{g})$ is no longer irreducible;
it decomposes as a sum of admissible highest-weight modules
\cite{KacWakimoto88}. The screening kernels
$\bigcap_i\ker S_{\alpha_i}$ extract the simple admissible module
$L_k(\mathfrak{g})\hookrightarrow V_k(\mathfrak{g})$ from
$W_k(\mathfrak{g})$, and the higher cohomology vanishes because the
BRST differential intertwines the reducibility structure: the
Kac--Kazhdan singular vectors are exactly the BRST-exact part of
the Wakimoto module.
\end{remark}

\begin{corollary}[Admissible-level BRST resolution]
\label{cor:wakimoto-brst-admissible}
\index{admissible level!Wakimoto resolution}
For every admissible level $k + h^\vee = p/q$ with $\gcd(p,q)=1$,
$p\geq h^\vee$, and every simply-laced simple $\mathfrak{g}$, the
Feigin--Frenkel BRST complex
$(W_k(\mathfrak{g})\otimes\Lambda^\bullet[c],
Q_{\mathrm{BRST}}^{(k)})$ resolves the simple admissible module
$L_k(\mathfrak{g})$:
\[
H^n\bigl(W_k(\mathfrak{g})\otimes\Lambda^\bullet[c],
Q_{\mathrm{BRST}}^{(k)}\bigr)
\;=\;
\delta_{n,0}\;L_k(\mathfrak{g}).
\]
...
For $\widehat{\mathfrak{sl}}_3$ at $k = -1$, one has
$h^\vee = 3$, so $k + h^\vee = 2 = 2/1$ is admissible of
principal type:
\[
L_{-1}(\mathfrak{sl}_3)
\;=\;
\bigcap_{i=1,2}\ker S_{\alpha_i}
\;\subset\;
\mathcal{F}_{\beta_1\gamma_1}\otimes\mathcal{F}_{\beta_2\gamma_2}
\otimes\mathcal{F}_{\beta_3\gamma_3}\otimes\Pi^{\sqrt{2}}_{\mathfrak{h}},
\]
and the two screenings $S_{\alpha_1}$, $S_{\alpha_2}$
satisfy the $\mathfrak{sl}_3$ Serre relations
$[S_{\alpha_i},[S_{\alpha_i},S_{\alpha_j}]]=0$ ($i\ne j$), so
$(Q_{\mathrm{BRST}}^{(-1)})^2 = 0$ and the resolution is chain-level
explicit.
\end{corollary}

\begin{proof}
Specialisation of Theorem~\ref{thm:wakimoto-brst-full-nondegenerate}
to $k + h^\vee \in\mathbb{Q}_{>0}$. The identification of $H^0$
with $L_k(\mathfrak{g})$ (not the larger $V_k(\mathfrak{g})$) uses
Remark~\ref{rem:wakimoto-admissible-robust}: on admissible levels
the intersection $\bigcap_i\ker S_{\alpha_i}$ is the simple
admissible quotient. The $\mathfrak{sl}_2$ and $\mathfrak{sl}_3$
cases are verified by direct computation: the Serre relations for
$S_{\alpha_i}$ follow from the quantum Gaussian decomposition
\cite[Proposition~6.3.4]{FBZ04} of the positive
nilpotent algebra.
\end{proof}

\begin{corollary}[Class $\mathsf{L}$ reduces to class $\mathsf{G}$
at the chain level]
\label{cor:class-L-reduces-to-G-chain-level}
\index{class reduction!L to G via Wakimoto}
On the full non-degenerate locus
$\mathcal{L}^{\mathrm{nd}}_{\mathfrak{g}}$, every affine Kac--Moody
vertex algebra $V_k(\mathfrak{g})$ (class $\mathsf{L}$,
$r_{\max} = 3$) is the BRST cohomology of a pure
Heisenberg--$\beta\gamma$ algebra (class $\mathsf{G}$,
$r_{\max} = 2$):
\[
V_k(\mathfrak{g})
\;=\;
...
\]
Consequently the shadow tower of class $\mathsf{L}$ is computed by
BRST reduction from the shadow tower of class $\mathsf{G}$: the
cubic shadow $\mathfrak{C}(x,y,z) = \kappa^{\mathrm{KM}}(x,[y,z])$
emerges from the screening-charge OPE as the quantum Serre
deformation of the Heisenberg cubic (which is identically zero).
\end{corollary}

\begin{proof}
The statement $V_k(\mathfrak{g}) = H^0$ is
Theorem~\ref{thm:wakimoto-brst-full-nondegenerate}. The shadow
reduction is the statement that $\kappa^{\mathrm{KM}}$ and
$\mathfrak{C}$ are computed by the Feigin--Frenkel screening
residues, and these residues reduce class-$\mathsf{L}$ OPE data to
composite $\beta\gamma\oplus\mathcal{H}$ data. Explicitly, the
quadratic invariant pairs via
$\kappa^{\mathrm{KM}}(J^a,J^b)
=\kappa^{\mathcal{H}}(\partial\phi^a,\partial\phi^b)
+\sum_\alpha \rho^\alpha(e_a,e_b)\,\kappa^{\beta\gamma}_\alpha$,
and the cubic shadow vanishes identically in the
$\beta\gamma\oplus\mathcal{H}$ sector (abelian OPE) — the
nontrivial class-$\mathsf{L}$ cubic is generated entirely by the
BRST reduction $Q_{\mathrm{BRST}}^{(k)}$.
\end{proof}
...
structure. The critical-level BRST cohomology computed via
Theorem~\ref{thm:wakimoto-koszul} is the Feigin--Frenkel center
$\mathfrak{z}(\widehat{\mathfrak{g}})$, an enlargement of the
universal Kac--Moody chiral algebra by classical opers. The
non-degenerate locus theorem recovers the universal chiral algebra
$V_k(\mathfrak{g})$; the critical-level theorem recovers the
enlarged Feigin--Frenkel center. The two statements are limiting
cases of a single uniform BRST construction, labelled by whether
the Wakimoto pairing is quantum (non-critical) or classical
(critical).
\end{remark}

\begin{remark}[Lane declaration and the $(\infty,1)$-upgrade]
...
upgrade is the assertion that the BRST resolution is a
...
\begin{remark}[Master Gravity upgrade and class-$\mathsf{L}$
unconditionality]
\label{rem:wakimoto-brst-gravity-upgrade}
Corollary~\ref{cor:class-L-reduces-to-G-chain-level} upgrades
every class-$\mathsf{L}$ statement in the Master Gravity Theorem
(Vol~II \texttt{chapters/connections/3d\_gravity.tex}) from
conditional (contingent on BRST exactness) to unconditional
(BRST exactness proved for every $k\ne -h^\vee$). In particular,
the class-$\mathsf{L}$ shadow-tower depth $r_{\max} = 3$ is now
computed from chain-level Heisenberg--$\beta\gamma$ BRST data on
the full non-critical locus, not merely in the asymptotic
$|k+h^\vee|\to\infty$ regime.
\end{remark}
...
\emph{trace-form} basis in which the level dependence is absorbed
into the Casimir normalization; the two forms agree after the
identification $k\Omega_{\mathrm{tr}} = \Omega/(k+h^\vee)$.
...
The bar cohomology at conformal weight $n(n{+}1)/2$
...
The dimension identity $\dim H^n = 2n + 1$
\textup{(}Remark~\textup{\ref{rem:garland-lepowsky-sl2}})
is verified through $n = 10$ by the compute engine
\texttt{bar\_cohomology\_sl2\_explicit\_engine}
(124~tests). The representation-theoretic refinement
is verified at $n = 1, 2, 3$ by explicit weight
decomposition of the CE cohomology
...
(Theorem~\ref{thm:modular-characteristic}(i)) controls every genus
$g \geq 1$ via $\mathrm{obs}_g = \kappa \cdot \lambda_g$
...
$r(z) = \Omega/\bigl((k{+}h^\vee)\,z\bigr)$ is well-defined and the
generic Chevalley--Eilenberg count applies\textup{)} down to exactly
...
the inequality to $\mathrm{rk}(\fg) \geq 3$\textup{)}. The compute engine
\verb|compute/tests/test_admissible_koszul_rank2_engine.py|
\textup{(}$51$ tests\textup{)} verifies the $\mathfrak{sl}_3$
prediction explicitly at a sweep of admissible levels
...
\paragraph{Step~2: Degree~$3$, nontrivial cubic shadow.}
...
\paragraph{Step~3: Degree~$4$, Jacobi identity kills the
obstruction.}
...
\paragraph{Step~4: Termination at degree~$3$.}
...
\item $K_{0,2}^{\mathrm{aff}}(x,y) = k_{\mathrm{eff}}\,\kappa(x,y)$
 is the degree-$2$ corolla \textup{(}the effective-level Killing form
...
\begin{remark}[Evidence and scope for Conjecture~\ref{conj:sc-deformation-critical}]
Parts~(i) and~(iii) are proved:
(i)~is the Feigin--Frenkel theorem
(Theorem~\ref{thm:critical-level-structure}),
...
Part~(iv) is the genuinely conjectural content: the claim that the
spectral sequence of the bicomplex (with deformation parameter
$\epsilon = k + h^\vee$) provides a \emph{wall-crossing formula}
governing the dimension jump. At $\epsilon = 0$ the spectral
sequence degenerates at~$E_1$; at $\epsilon \neq 0$ the
differentials act and collapse the infinite-dimensional
$E_1$~page to the finite-dimensional Koszul answer.
The wall-crossing is therefore the bicomplex spectral sequence itself.

The critical-level SC moduli should be compared with the moduli of
$\fg^\vee$-opers on the formal disc
...
(70~tests, all simple types at multiple levels).
...
\item \emph{Critical discriminant:}
 ...
 tower terminates.
\item \emph{$r$-matrix:}
 $r(z) = \Omega/\bigl((k{+}h^\vee)\,z\bigr)$ with a simple pole at
 $z = 0$ \textup{(}one order below the OPE double pole, by
 $d\log$ extraction followed by dualisation through the level-shifted
 Killing form; KZ normalisation, cf.\
 Remark~\textup{\ref{rem:km-collision-residue-rmatrix}}\textup{)}.
 At the critical level $k = -h^\vee$ the denominator vanishes, tracking
 the Sugawara singularity.
\end{enumerate}
\end{proposition}
...
$r$-matrix $r(z) = \Omega/\bigl((k{+}h^\vee)\,z\bigr)$
\textup{(}simple pole in the KZ normalisation, cf.\
Remark~\textup{\ref{rem:km-collision-residue-rmatrix}}\textup{)}.
\end{proposition}
...
The gauge-theoretic interpretation of the
...
\begin{remark}[Kac-Moody and BKM: Hall-Drinfeld double at K3 x E]
\label{rem:km-K3xE}
Beyond Kac-Moody $\widehat{\mathfrak{g}}_k$, the K3 chiral bialgebra of Vol~III Chapter~\ref{ch:k3-chiral-bialgebra-platonic} arises from the generalised Borcherds-Kac-Moody superalgebra $\mathfrak{g}_{\Delta_5}$ on $\Lambda^{2,1}_{II} \subset \Lambda^{3,2}$. Since BKM lacks a Kac-Moody Cartan, Weyl action on imaginary simple roots, and J-presentation, the quantum-group deformation is \emph{not} a Drinfeld Yangian: it is the K3 chiral Hall--Drinfeld double $\mathcal{D}_\hbar(\mathcal{Y}^{\mathrm{Hall}}_\hbar(\mathrm{CoHA}_{K3 \times E}))$ with Schiffmann--Vasserot $2012$ shuffle (Davison $2017$ CY-$3$ extension). Manin pair not triple on signature-$(2,1)$ Cartan.
\end{remark}
