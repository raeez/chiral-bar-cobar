...
$\widehat{\fg}_k^! = \widehat{\fg}_{-k-2h^\vee}$ for all simple
$\fg$ (Theorem~\ref{thm:universal-kac-moody-koszul}).  The genus-$1$
complementarity for $\mathfrak{sl}_2$ and $\mathfrak{sl}_3$.  The
shadow Postnikov tower terminates
(Proposition~\ref{prop:borcherds-shadow-identification}) at $r=3$ (Lie/tree archetype),
with cubic shadow $= $ Lie bracket and quartic obstruction $= 0$
by the Jacobi identity
(Corollary~\ref{cor:affine-postnikov-termination}).
Key quantitative results:
$\kappa(\widehat{\fg}_k) = \dim(\fg)(k{+}h^\vee)/(2h^\vee)$
(for $\mathfrak{sl}_2$: $\kappa = 3(k{+}2)/4$);
Feigin--Frenkel antisymmetry
$\kappa(\widehat{\fg}_k) + \kappa(\widehat{\fg}_{-k-2h^\vee}) = 0$;
critical level $k = -h^\vee$: $\kappa = 0$, Sugawara undefined,
Feigin--Frenkel center emerges.
\end{remark}

With the affine Kac--Moody algebra $\widehat{\mathfrak{g}}_k$, the
nonabelian world arrives.  The Feigin--Frenkel level involution
$k \mapsto -k - 2h^\vee$, the critical-level center and its opers,
the PBW filtrations on enveloping algebras, the derived center of
Frenkel--Gaitsgory --- all are faces of a single
Maurer--Cartan package.  The bar-cobar adjunction
(Theorem~\ref{thm:bar-cobar-verdier}) \emph{explains} the Feigin--Frenkel shift as
Verdier duality on the Fulton--MacPherson compactification.

In the shadow Postnikov tower of $\gAmod$
(Definition~\ref{def:modular-convolution-dg-lie}), affine Kac--Moody
algebras exhibit the Lie/tree archetype --- the intermediate case
between the Gaussian families (Heisenberg, lattice, free fermion,
terminating at weight~$2$) and the contact/quartic archetype of
$\beta\gamma$ (terminating at weight~$4$).  At generic level
$k \neq -h^\vee$, the scalar curvature
$\kappa = \dim(\mathfrak{g}) \cdot (k + h^\vee)/(2h^\vee)$ controls
the genus tower, and one-channel scalar saturation
(Theorem~\ref{thm:theta-direct-derivation}) guarantees
$\Theta = \kappa \cdot \eta \otimes \Lambda$.  But the strict model $\Convstr$ is not trivially formal in the
Heisenberg sense --- there is a cubic shadow at weight~$3$, the
shadow of the Lie bracket itself, which dies at the next stage
because the quartic obstruction $o_4$ vanishes by the Jacobi
identity.  The tower terminates at $r = 3$.  At the homotopy level,
however, the $L_\infty$-algebra $\Definfmod(\widehat{\fg}_k)$ is
\emph{strictly formal} by
Theorem~\ref{thm:km-strictification}: no higher homotopy
corrections at any genus, making Kac--Moody the archetype of
strictification
...
cohomology, and the derived center of $\widehat{\mathfrak{g}}_k$ is
the full bar cohomology.  At critical level the shadow tower collapses; for
$\mathcal{W}$-algebras
(Chapter~\ref{chap:w-algebra-koszul}), it becomes infinite.

The Feigin--Frenkel level shift
$k \mapsto -k - 2h^\vee$ is Verdier duality on
configuration spaces
(Theorem~\ref{thm:bar-cobar-verdier}).
In the WZW model at level~$k$, this shift exchanges the holomorphic
and anti-holomorphic sectors; geometrically, it reverses the
orientation of the worldsheet.  The same involution appears in modular
...
all projections of the single geometric duality acting on the Fulton--MacPherson
compactification.

\begin{remark}[Shadow Postnikov tower for Kac--Moody]\label{rem:km-master-mc}
For $\widehat{\mathfrak{g}}_k$ at generic level, the shadow
Postnikov tower in the modular convolution algebra
$\mathfrak{g}^{\mathrm{mod}}_\cA$
(Definition~\ref{def:modular-convolution-dg-lie}) saturates at the
scalar level: $\Theta^{\leq 2} = \kappa$ with
$\kappa = \dim(\mathfrak{g})\cdot(k+h^\vee)/(2h^\vee)$, and
$\Theta = \kappa \cdot \eta \otimes \Lambda$ by one-channel
scalar saturation
(Theorem~\ref{thm:theta-direct-derivation}).
The bar complex, Koszul duals, genus tower, and complementarity
splitting computed in this chapter are projections of this scalar
level (Remark~\ref{rem:unifying-principle}).
...
reduction applies and the shadow tower simplifies.
\end{remark}
...
The affine algebra illustrates the four levels of the modular engine:
...
\begin{enumerate}
\item \emph{Level shift as curvature.}  The double pole $k/(z-w)^2$ in $e(z)f(w)$ creates curvature $m_0 \propto (k+h^\vee)\kappa$ in the bar complex.  The cobar construction inverts this curvature, producing the dual level $-k-2h^\vee = -k-4$.
\item \emph{Critical level as curvature-free locus.}  At $k=-2$, the curvature vanishes ($m_0 = 0$), the bar complex becomes an honest dg coalgebra, and the Koszul dual is $\widehat{\mathfrak{sl}}_{2,-2}$ itself --- self-duality.  Simultaneously, the Sugawara construction degenerates and the center enlarges to the oper algebra.
...
\end{equation}
This determines $(\widehat{\mathfrak{g}}_k)^!$ uniquely up to quasi-isomorphism.  Since we have identified the dual bar OPE as that of $\widehat{\mathfrak{g}}_{-k-2h^\vee}$, and the structure constants $f^{ab}{}_c$ are preserved (the Lie bracket is level-independent), the Koszul dual algebra is $\bar{B}(\widehat{\mathfrak{g}}_k)^\vee \simeq \widehat{\mathfrak{g}}_{-k-2h^\vee}$.  (Bar-cobar inversion $\Omega(\bar{B}(\widehat{\mathfrak{g}}_k)) \simeq \widehat{\mathfrak{g}}_k$ recovers the original algebra at level~$k$.)
...
\begin{theorem}[\texorpdfstring{$\mathcal{W}$}{W}-algebra Koszul duality at critical level; \ClaimStatusProvedHere]\label{thm:w-algebra-koszul}
...
\]
The Wakimoto realization gives cohomology concentrated in degree $0$ for the principal nilpotent \cite[Thm.~3.1]{FF}, so the spectral sequence collapses.  Applying the cobar functor:
\[
...
The proof is stated for $f_{\mathrm{prin}}$, where DS cohomology concentrates in degree~$0$ \cite{FF}.  For general $f$, DS cohomology may be non-trivial in other degrees, and the general case
\[
\mathcal{W}^{-h^\vee}(\mathfrak{g}, f)^! \simeq \mathcal{W}^{-h^\vee}(\mathfrak{g}^\vee, f^\vee)
\]
for arbitrary $f$ (Barbasch--Vogan dual $f^\vee$) is conjectural (\ClaimStatusConjectured): it separates into three non-principal packets (dual-orbit input, orbit-indexed level shift, paired DS seed transport).  See Theorem~\ref{thm:w-algebra-koszul-main} for the principal case.
\end{remark}

\begin{remark}[Langlands duality]
The principal case of Theorem~\ref{thm:w-algebra-koszul} is a manifestation of geometric Langlands duality: the exchange $\mathfrak{g} \leftrightarrow \mathfrak{g}^\vee$ of Langlands dual Lie algebras carries the principal nilpotent $f_{\mathrm{prin}}$ to its dual $f^\vee_{\mathrm{prin}}$, and the corresponding W-algebras --- quantum deformations of the Slodowy slices $\mathcal{S}_f$ and $\mathcal{S}_{f^\vee}$ --- are interchanged by Koszul duality.  That the bar-cobar adjunction produces the Langlands dual at critical level motivates a project-level conjectural direction (\ClaimStatusConjectured): the chiral bar complex could provide a constructive approach to the geometric Langlands correspondence, building the dual D-modules on $\mathrm{Bun}_G$ from configuration space data on the curve (see \S\ref{sec:km-conformal-blocks} for the conformal block interpretation).
\end{remark}
...
$r(z) = \Omega/z$ (the Casimir $r$-matrix, governing the KZ equation),
$\kappa(\widehat{\mathfrak{g}}_k) = (k+h^\vee)\dim\mathfrak{g}/(2h^\vee)$,
$\Theta_{\widehat{\mathfrak{g}}_k} = \kappa \cdot \eta \otimes \Lambda$
(scalar saturation; Lie/tree archetype with
$\mathfrak{C} \ne 0$, $o_4 = 0$).
\end{remark}
...
r_{ij}(z_{ij})\, dz_{ij}.
\end{equation}
This is precisely the Knizhnik--Zamolodchikov connection.
...
relation.  The shadow tower of $\widehat{\mathfrak{sl}}_{2,k}$
...
$X_{L_{-1/2}} = \{0\}$, so the reduced Poisson bar complex is
...
resolution is $T^*\mathbb{P}^1 \to \mathcal{N}$.
By Theorem~\ref{thm:associated-variety-koszulness}(iii),
applied to the reduced quotient:
\[
HP^n(\mathcal{N}_{\mathfrak{sl}_2})
\cong H^n(T^*\mathbb{P}^1,\, \mathcal{O}).
\]
Since $T^*\mathbb{P}^1$ is the total space of the line
bundle $\mathcal{O}(-2)$ on $\mathbb{P}^1$:
$H^0(T^*\mathbb{P}^1, \mathcal{O})
= \mathcal{O}(\mathcal{N})$, and
$H^n(T^*\mathbb{P}^1, \mathcal{O}) = 0$ for $n > 0$
(Serre vanishing on the affine total space).  Hence the
Poisson cohomology is concentrated in degree~$0$, and the
Li--bar $E_2$ page is diagonally concentrated.

The reduced Poisson cohomology is therefore concentrated
in degree~$0$.  By Theorem~\ref{thm:associated-variety-koszulness},
this means the Li--bar spectral sequence has concentrated
$E_2$~page \emph{on the reduced level}.  The remaining
obstruction to Koszulness lies in the nilradical of the
...
This pattern holds for all types: the \emph{reduced}
Poisson cohomology of nilpotent orbit closures is always
concentrated (Broer $+$ affineness), so the geometric
obstruction to Koszulness at degenerate admissible levels
is concentrated in the nilradical fiber.
\end{example}
...
\end{tabular}
\end{center}
At admissible level, $d_1$ cancels all but finitely many terms, producing the KW formula as a finite alternating sum over $\mathcal{W}_{\mathrm{adm}}$ (of size~$q$).
\end{remark}

\begin{theorem}[KW formula via bar complex: general simple \texorpdfstring{$\mathfrak{g}$}{g};
\ClaimStatusProvedHere]
...
(Arakawa~\cite{Arakawa17}) ensures finite-page
degeneration; the precise identification
$E_2 = E_\infty$ follows a~posteriori from
Arakawa's rationality theorem, which gives
semisimplicity of the module category and hence
Ext vanishing
(Remark~\ref{rem:admissible-rationality-finiteness}).
\end{proof}

\begin{remark}[Admissible module counts]
\label{rem:admissible-module-count}
\index{admissible level!module count}
At non-degenerate admissible $k = -h^\vee + p/q$ ($\gcd(p,q) = 1$, $p \geq h^\vee$), the admissible weight lattice~\cite{KW88} gives, for $\mathfrak{sl}_2$:
\[
...
Kac--Wakimoto character formulas are recovered by finite-page
degeneration
\textup{(}Proposition~\ref{prop:bar-admissible},
Theorem~\ref{thm:kw-bar-general-rank}\textup{)}.

What is \emph{not} proved here is the stronger statement that
$L_k(\fg)$ is chirally Koszul.  The ordinary semisimplicity of the
admissible module category controls Zhu theory and conformal
blocks, but it does not identify the Ext groups computed by the
bar complex with Ext groups in the ordinary module category.
Hence Theorems~A--D$+$H are not promoted to simple admissible
quotients on the basis of rationality alone.
\end{remark}
...
\item \emph{Spectral-sequence control:}
  ...
  $d_1$ differentials from singular vector embeddings cancel all but
  $|\mathcal{W}_{\mathrm{adm}}|$ Verma modules; $C_2$-cofiniteness
  gives $E_2 = E_\infty$.  Provides the explicit Kac--Wakimoto
  character formula as a bonus.
\end{enumerate}
...
In the bar complex this shows up as extra extension data attached
to the null vectors.
...
\end{equation}
with braiding phases $e^{2\pi i \lambda\mu/(k+h^\vee)} \mapsto e^{-2\pi i \lambda\mu/(k+h^\vee)}$ under $k \mapsto -k - 2h^\vee$.  The admissible-level case requires the root-of-unity bar analysis of Conjectures~\ref{conj:kl-periodic-cdg}--\ref{conj:kl-braided}.
\end{remark}
...
identifies $\widehat{\fg}_{-h^\vee}$-modules with the Whittaker category, a cornerstone of the geometric Langlands proof~\cite{GLC24}.  Combined with the bar-Ext identification (Corollary~\ref{cor:bar-computes-ext}), one obtains $H^n(\barB(\widehat{\fg}_{-h^\vee})) \cong \Omega^n(\mathrm{Op}_{\fg^\vee}(D))$ for all~$n$.
\end{remark}

\begin{remark}[Scope]\label{rem:oper-bar-scope}
\index{geometric Langlands!from bar complex}
Theorem~\ref{thm:oper-bar} provides a vertex-algebraic construction of the derived oper geometry: bar-Ext (Corollary~\ref{cor:bar-computes-ext}) combined with Frenkel--Teleman (Remark~\ref{rem:frenkel-teleman-oper}) gives $H^n \cong \Omega^n(\mathrm{Op})$ for all~$n$.  Independent PBW confirmation exists for $n \leq 3$ (Chapter~\ref{ch:derived-langlands}); the Whitehead spectral decomposition (Proposition~\ref{prop:whitehead-spectral-decomposition}) extends to all~$n$.
\end{remark}
...
\index{shadow Postnikov tower!affine construction}
...
\begin{corollary}[Affine shadow Postnikov tower:
finite termination at arity~$3$; \ClaimStatusProvedHere]
\label{cor:affine-postnikov-termination}
\index{shadow Postnikov tower!affine termination}
...
the bar complex produces no higher operations because the Heisenberg
...
\begin{theorem}[Critical-level emergence;
...
\begin{corollary}[Critical self-intersection as Maurer--Cartan;
...
representation theory (Feigin--Frenkel center):
the locus $\{a : [a,a] = 0\}$ parametrises flat
$\fg$-connections on the formal disc, whose invariant
functions recover the oper algebra
...
Maurer--Cartan locus.  The identification with flat connections
and the Feigin--Frenkel center is
...
terms, recovering the oper space
(Theorem~\ref{thm:oper-bar}) from purely shadow-theoretic data.
\end{remark}
