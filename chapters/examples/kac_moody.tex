...
arity~$2$, and every obstruction is a scalar multiple of~$\kappa$.
...
arity~$3$; the Jacobi identity forces the quartic shadow to vanish;
...
% AP142: r(z) lives on C_2(X), not over a point.  The formal disk
...
the unique point where the arity-$2$ projection vanishes
because $\kappa(-h^\vee) = 0$.  This kills the scalar
curvature and gives $\Delta = 8\kappa S_4 = 0$, but it does
not force the higher-arity part of $\Theta_{\cA}$ to vanish or
