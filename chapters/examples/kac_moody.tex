...
This chapter computes $\Theta_{\widehat{\fg}_k}$ and its projections.
The bar-cobar adjunction
(Theorem~\ref{thm:bar-cobar-verdier}) \emph{explains} the
Feigin--Frenkel shift as Verdier duality on the
Fulton--MacPherson compactification.  The shadow Postnikov tower
terminates at arity~$3$ (Lie/tree archetype, class~$L$,
$r_{\max} = 3$): a nontrivial cubic shadow from the Lie bracket,
killed at the next stage by the Jacobi identity.  All five main
...
Koszul dual & $\widehat{\fg}_{-k-2h^\vee}$
  (Feigin--Frenkel) \\
...
In the shadow Postnikov tower, affine Kac--Moody
...
terminating at arity~$2$) and the contact/quartic archetype of
$\beta\gamma$ (terminating at arity~$4$).  At generic level
...
Heisenberg sense: there is a cubic shadow at arity~$3$, the
...
the $L_\infty$-algebra $\Definfmod(\widehat{\fg}_k)$ is
...
for exceptional types.  We now record the full shadow tower data.
...
(the affine KM shadow tower terminates at arity~3 regardless
of $\varrho$), the anomaly ratio controls the
$\mathcal{W}$-algebra shadow tower, not the affine one.
...
shadow tower converges absolutely at all non-critical
...
\index{shadow Postnikov tower!non-simply-laced types}
...
involving the level shift must use $h^\vee$.  The shadow tower
...
\begin{proposition}[Non-simply-laced shadow tower;
\ClaimStatusProvedHere]%
...
central charge, and shadow tower.  The lacing number is $r^\vee = 2$
...
\index{shadow Postnikov tower!exceptional types unified}
...
level $k \neq -h^\vee$, the shadow tower data is as follows.
\begin{center}
...
the shadow tower structure (class~L, depth~$3$ uniformly) is
...
the shadow tower has no Langlands asymmetry: the
...
shadow towers of $B_n$ and $C_n$ have different $h^\vee$
...
unique modular form of weight~$4$.  The shadow tower of the
