...
Fulton--MacPherson compactification. The five theorem-surface
statements below are used only in the ambient stated in their cited
proofs: the affine universal algebra lies on the PBW-Koszul locus,
while simple quotients at admissible level require the separate
admissible analysis of
...
shell $\Theta^{(2)}_{\mathrm{pf}}$. This is because the conditional
...
At $k = -h^\vee$ (the \emph{critical level}), the Sugawara construction is undefined: the denominator $k + h^\vee$ vanishes, so no Virasoro subalgebra is produced. The critical level is distinguished on three counts: the Feigin--Frenkel center is a large commutative chiral algebra, $Z(\widehat{\mathfrak{g}}_{-h^\vee})$ identifies with functions on opers, and the critical affine algebra is the local algebraic input for self-dual Yang--Mills. For Koszul duality, the critical level is the fixed point of the Feigin--Frenkel involution $k \mapsto -k - 2h^\vee$: the curvature $m_0 \propto (k + h^\vee)$ vanishes, making the bar complex a genuine (uncurved) dg coalgebra and the Koszul dual algebra uncurved.
\end{remark}
...
\begin{equation}
\Omega\bigl(\bar{B}^{\mathrm{geom}}(\widehat{\mathfrak{g}}_k)^\vee\bigr)
\simeq
\mathrm{CE}^{\mathrm{ch}}(\widehat{\mathfrak{g}}_{-k-2h^\vee})
\end{equation}
...
$q\geq 3$ non-degenerate admissible lane.
\end{remark}
...
non-degenerate admissible denominator-$q\geq3$ lane,
...
on the generic nonresonant chain-level locus]
...
enlarged Feigin--Frenkel center. They are two related BRST
...
\begin{remark}[Master Gravity comparison and class-$\mathsf{L}$
scope]
\label{rem:wakimoto-brst-gravity-upgrade}
Corollary~\ref{cor:class-L-reduces-to-G-chain-level} supplies the
chain-level Heisenberg--$\beta\gamma$ model needed by the Master
Gravity comparison on the Wakimoto-nonresonant locus. It does not
upgrade admissible or critical cases to unconditional statements:
...
$r$-matrix (Computation~\ref{comp:sl2-collision-residue-kz},
Step~2). The collision-residue normalization and the KZ connection
normalization are distinct:
...
$m_0 = \frac{k+3}{6}\cdot\kappa_{\mathfrak{sl}_3}$, vanishing precisely
at $k = -3$.
\end{proof}
...
$\bar{B}^{\bullet}(\widehat{\mathfrak{g}})$ recording the coefficient
of the double-pole residue. The critical differential still contains
the level-$(-h^\vee)$ double-pole term; what vanishes at critical level
is the shifted curvature class, not the OPE double pole. The triple
...
\end{itemize}
Write \(d_k=d_{\mathrm{sp}}+k\,d_{\mathrm{dp}}\), where
\(d_{\mathrm{sp}}\) is the simple-pole contribution and
\(d_{\mathrm{dp}}\) is the coefficient of the double-pole contribution.
Then
\[
...
\begin{theorem}[Screening charges implement the current-valued critical
...
vanishes, the current-valued part of the bar complex differential is
...
\emph{Step~4: Cobar.}
Dualize $\bar{B}^n \mapsto (\bar{B}^n)^\vee$, reverse the grading,
twist the differential by curvature, and form
$(\widehat{\mathfrak{g}}_k)^!
= \Omega(\bar{B}(\widehat{\mathfrak{g}}_k)^\vee)$.
...
\((\widehat{\mathfrak{g}}_k)^! \simeq
\mathrm{CE}^{\mathrm{ch}}(\widehat{\mathfrak{g}}_{-k-2h^\vee})\).
The conjectural step is the downstream MC5 dictionary comparing
bar-cobar data with holographic boundary/bulk physics.
...
The collision extraction records the trace-form residue
$r^{\mathrm{coll}}(z)=k\Omega_{\mathrm{tr}}/z$
(Remark~\ref{rem:km-collision-residue-rmatrix}). After dualising the
simple-pole Lie bracket through the level-shifted form, the KZ
coefficient is the rational Casimir tensor. The Casimir element of
...
higher KZ corrections is the \emph{complete} genus-$0$ shadow datum:
...
(Theorem~\ref{thm:sl2-koszul-dual}) is the starting point. We carry
out the genus-1 pipeline for $\widehat{\mathfrak{sl}}_2$ at generic
level $k \neq -2$: the first non-abelian genus-$1$ check of the
adjunction, inversion, and complementarity surfaces in this chapter.
...
The quasi-isomorphism $\Omega(\barB^{(1)}) \xrightarrow{\sim} \widehat{\mathfrak{sl}}_{2,k}$ then follows from the general inversion theorem (Theorem~\ref{thm:higher-genus-inversion}), whose hypotheses are the Koszul property (Ext-vanishing) and the completeness of the weight filtration.
\end{proof}
...
\section{\texorpdfstring{Genus-1 pipeline for $\widehat{\mathfrak{sl}}_3$}{Genus-1 pipeline for sl-3}}
...
\mathrm{CE}^{\mathrm{ch}}(\widehat{\mathfrak{sl}}_{3,-k-6})\)
(Theorem~\ref{thm:sl3-koszul-dual}) uses $h^\vee = 3$. We carry out
the genus-1 pipeline for $\widehat{\mathfrak{sl}}_3$ on the stated
generic surface. The key parameters are:
...
\emph{Remaining} (Conjecture~\ref{conj:kl-periodic-cdg}): the CDG periodicity $\barB^{n+2q} \cong \barB^n$ at root-of-unity $q = e^{\pi i/(k+h^\vee)}$. Computational evidence: $N$-complex structure of $\mathfrak{u}_q(\mathfrak{sl}_2)$ is checked by $74$ tests ($d_q^N = 0$, $d_q^2 \neq 0$ for $N \geq 3$).
...
\item \emph{$r$-matrix data:}
 ...
 \textup{(}Remark~\textup{\ref{rem:km-collision-residue-rmatrix}}\textup{)}.
...
$\kappa(G_2, k{=}1) = 35/4$,
$\kappa(F_4, k{=}1) = 260/9$.
All entries are checked by 168 compute tests
...
$r^{\mathrm{KZ}}(z)=
\Omega_{\mathrm{KZ}}/\bigl((k{+}h^\vee)\,z\bigr)$
\textup{(}trace-form collision and KZ normalisations, cf.\
...
= -d(k+h^\vee)/(2h^\vee) = -\kappa$.
All entries are checked by 253 compute tests
...
The Feigin--Frenkel involution $k \mapsto -k - 2h^\vee$ should be
compared with level-rank duality, but it is not literally the
