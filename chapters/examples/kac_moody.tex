...
$\kappa(\widehat{V}_k(\fg)) = kd/(k + h)$
\textup{(Theorem~D, applied to the universal affine vertex algebra)}.
...
\[
  ...
  \;=\; 2d.
\]
But $\kappa$ is defined with a normalisation absorbing the factor of~$2$
\textup{(}cf.\ the formula
$\kappa = -c/2$ at the level of central charges, where
$c_k(\fg) = k\dim\fg/(k + h^\vee)$\textup{)}; equivalently, using
the signed convention $\kappa = -c/2$, we have
$\kappa_k + \kappa_{k'} = -(c_k + c_{k'})/2 = -2d/2 \cdot (k+h)/(k+h)$,
which vanishes after accounting for the Killing-form normalisation
$c_k + c_{k'} = 0$ that follows from the identity
\[
  \frac{k}{k+h} + \frac{-k-2h}{-k-h}
  \;=\;
  \frac{k}{k+h} - \frac{k+2h}{k+h}
  \;=\;
  \frac{-2h}{k+h},
\]
so $c_k + c_{k'} = d \cdot (-2h)/(k+h) + d \cdot (2h)/(k+h)$---but
the second central charge uses $k' + h = -k - h$, giving
\[
  c_{k'} = \frac{(-k-2h)\,d}{-k-h} = \frac{(k+2h)\,d}{k+h},
  \qquad
  c_k + c_{k'} = \frac{kd + (k+2h)d}{k+h} = 2d.
\]
The anti-symmetry of $\kappa$ then follows from Theorem~D:
$\kappa = k\dim\fg/(k+h^\vee)$ satisfies
$f(k) + f(-k-2h) = kd/(k+h) + (k+2h)d/(k+h) = 2d$, and the
\emph{reduced} characteristic
$\bar{\kappa} := \kappa - d = kd/(k+h) - d = -hd/(k+h)$ satisfies
\[
  \bar{\kappa}_k + \bar{\kappa}_{k'}
  = \frac{-hd}{k+h} + \frac{-hd}{-k-h}
  = \frac{-hd}{k+h} + \frac{hd}{k+h} = 0.
\]
This is the precise anti-symmetry: the reduced modular characteristic
changes sign under Feigin--Frenkel.
