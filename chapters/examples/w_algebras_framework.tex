...
\end{equation}
Since $m_1^2 \neq 0$, the $A_\infty$ structure is curved.  The curvature element $m_0$ (determined implicitly by the curved $A_\infty$ relation $m_1^2(a) = [m_0, a]_{m_2}$) is a degree-2 element in the bar complex, \emph{not} a scalar multiple of the vacuum.  The curvature is proportional to $(k + h^\vee)$ and vanishes at critical level.

For $\mathcal{W}_3$ (Drinfeld--Sokolov reduction of $\widehat{\mathfrak{sl}}_3$ at level $k$), we have $c = 2 - \frac{24(k+2)^2}{k+3}$; the relation $m_1^2(T) = \frac{c}{2} \cdot \mathbf{1}$ shows that the curvature vanishes only at $c = 0$.  (The $\mathcal{W}_3$ minimal models $\mathcal{W}_3(p,q)$ with $p > q \geq 3$ coprime have $c = 2(1 - 12(p-q)^2/(pq))$; for consecutive parameters $(p, p-1)$ this gives $c = 2 - 24/(p(p-1))$.  These arise from the DS formula at specific rational values of $k$, not by the substitution $p = k+3$.  The two parametrizations are related but distinct.)
