\chapter{The Bershadsky--Polyakov algebra}\label{chap:bershadsky-polyakov}

The Bershadsky--Polyakov algebra is the first non-principal
Drinfeld--Sokolov reduction in which the bar-cobar, Verdier, and
shadow packages separate visibly. On the finite-type or completed
Verdier lane, and only after the subregular DS/bar transport
statement is supplied, the self-transpose partition $(2,1)$ produces
a same-family companion at level $-k-6$. On the nondegenerate shadow
locus of Convention~\ref{conv:bp-generic-loci}, the shadow tower has
class~$\mathbf{M}$ on the Virasoro $T$-line and class~$\mathbf{G}$
on the residual-current $J$-line; the scalar class of the whole
algebra is the channel supremum, not a claim that every generator
line carries a total class-$\mathbf{M}$ tower. The complementarity
sum $\kappa + \kappa' = \varrho K^c = 98/3 \neq 0$ records the
non-principal failure of affine anti-symmetry.

The conductor identity $K^c_{\mathcal{B}} \equiv 196$
(Theorem~\ref{thm:bp-koszul-conductor-polynomial}) holds as a
polynomial identity in $\mathbb{Q}(k)$: the level-dependent summands
$c(\mathcal{B}^k) + c(\mathcal{B}^{-k-6})$ cancel to the constant
rational function $196$. This central-charge conductor is not the
Drinfeld--Sokolov ghost scalar. Chapter~\ref{chap:universal-conductor}
separates the scalar packages
\[
K^c(\mathrm{BP}_k,\mathrm{BP}_{-k-6})=196,\qquad
K^\kappa(\mathrm{BP}_k,\mathrm{BP}_{-k-6})=\frac{98}{3},\qquad
K_{\mathrm{ghost}}^{\mathrm{leg}}(\mathrm{BP}_k;\mathrm{DS})
=-\frac{6(4k^2+9k+3)}{k+3}.
\]
The mixed-class profile
$(T\colon\mathbf{M},\;J\colon\mathbf{G},\;G^{\pm}\colon\text{fermionic})$
is a non-principal-DS instance of the channel-maximum rule of
Chapter~\ref{chap:shadow-quadrichotomy-platonic}: the global
shadow class is $\mathbf{M}$ \textup{(}the maximum over channels\textup{)}
even though three of the four generators carry strictly shallower
shadows.

\begin{table}[ht]
\centering
\small
\caption{Five-theorem data for the Bershadsky--Polyakov
algebra $\mathcal{B}^k = \mathcal{W}^k(\mathfrak{sl}_3, f_{(2,1)})$
on the BP working loci of Convention~\ref{conv:bp-generic-loci}.}
\label{tab:bp-five-theorems}
\begin{tabular}{lll}
\toprule
\textbf{Theorem} & \textbf{Statement for $\mathcal{B}^k$}
 & \textbf{Hypothesis / reference} \\
\midrule
A (adjunction) &
 $\Omega_X^{\mathrm{cont}}\dashv \bar B_X$ applies to
 $\mathcal{B}^k$; the Verdier companion is a separate output
 & Thm~\ref{thm:bar-cobar-inversion-qi};
 Prop~\ref{prop:bar-cobar-object-firewall-bci} \\
B (inversion) &
 $\Omega_X^{\mathrm{cont}}(C_{\mathcal{B}^k})
 \xrightarrow{\sim} \mathcal{B}^k$
 on the strict BP Koszul lane
 & raw finite-window or completed pro-conilpotent ambient;
 Thm~\ref{thm:bar-cobar-inversion-qi} \\
C (complementarity) &
 $\kappa(k) + \kappa(-k{-}6)
 = \varrho\, K^c_{\mathcal{B}} = 98/3$, \;
 $K^c_{\mathcal{B}} = 196$, $\varrho = 1/6$
 & Prop~\ref{prop:bp-complementarity} \\
D (modular char.) &
 $\kappa(\mathcal{B}^k)
 = \dfrac{c}{6}
 = \dfrac{1}{6}\Bigl(2 - \dfrac{24(k{+}1)^2}{k{+}3}\Bigr)$
 & Prop~\ref{prop:bp-kappa} \\
H (chiral Hochschild) &
 curve-level $\ChirHoch_X^n(\mathcal{B}^k)=0$ for
 $n\notin\{0,1,2\}$; $P(t)=1+2t+t^2$ after the BP derivation
 calculation
 & Thm~\ref{thm:hochschild-polynomial-growth};
 Prop~\ref{prop:bp-curve-level-chirhoch} \\
\bottomrule
\end{tabular}
\end{table}

\begin{table}[ht]
\centering
\small
\caption{Shadow archetype data for the Bershadsky--Polyakov
algebra.}\label{tab:bp-shadow-archetype}
\begin{tabular}{ll}
\toprule
\textbf{Invariant} & \textbf{Value} \\
\midrule
Archetype tag & $\mathbf{M}$ (mixed-channel: $T$-channel $\mathbf{M}$,
 $J$-channel $\mathbf{G}$, $G^\pm$ fermionic);
 global class is the channel supremum \\
Shadow depth $r_{\max}$ & $\infty$ (on $T$-line);
 $r_{\max}^J = 2$ (on $J$-line) \\
$\kappa(\mathcal{B}^k)$ &
 $c/6$; \; anomaly ratio $\varrho = 1/6$ \\
Central charge & $c = 2 - 24(k{+}1)^2/(k{+}3)$
 (Fehily--Kawasetsu--Ridout) \\
Central-charge conductor & $K^c_{\mathcal{B}} = c(k) + c(-k{-}6) = 196$ \\
Complementarity & $\kappa + \kappa' = 98/3$ (W-type) \\
Self-dual point & $c = 98$ (no real level; $k = -3 \pm 2i$) \\
Critical level & $k = -3$: Sugawara undefined \\
$T$-line poles & $c=0$ or $c=-22/5$ \\
$J/G$ degeneracies & $k=-3/2$ for $J_{(1)}J$;
 $k=-1,-3/2$ for $G^+_{(2)}G^-$ \\
$J$-line level & $(2k+3)/3$ in Feigin--Semikhatov normal form \\
$r$-matrix $r(z)$ & Multi-channel:
 $T$-channel has poles at $z^{-3}, z^{-1}$;
 $J$-channel has pole at $z^{-1}$ \\
Partition & $(2,1)$ (self-transpose, hook-type) \\
DS parent & $\widehat{\mathfrak{sl}}_3$ at the minimal nilpotent \\
\bottomrule
\end{tabular}
\end{table}

\begin{remark}[$5\times5$ $\kappa$-stratification row for $\mathcal{B}^k$]
\label{rem:bp-5x5-kappa-row}
\index{$\kappa$-stratification matrix!Bershadsky--Polyakov row}
\index{Master Pattern MA-6!Bershadsky--Polyakov}
\index{Master Pattern MA-7!Vol III specialisation absent for BP}
Type signature: (Open quadrant; chart-level presentation; Beilinson
level $1$ chart $A_b=\mathcal{B}^k$ with the standard generator
quadruple $(T,J,G^+,G^-)$ as the implicit chosen vacuum; hypothesis
package: working loci $U_{\mathrm{OPE}}$, $U_{\mathrm{nd}}$, $U_T$
of Convention~\ref{conv:bp-generic-loci}).

On the standard Open-quadrant Beilinson tower, the five
$\kappa$-measurements
$(\kappa_{\mathrm{cat}},\kappa^{\mathrm{Hodge}}_{\mathrm{ch}},
\kappa^{\mathrm{Heis}}_{\mathrm{ch}},\kappa_{\mathrm{BKM}},
\kappa_{\mathrm{fiber}})$ specialise on $\mathcal{B}^k$ to
\[
\bigl(\kappa_{\mathrm{cat}}(\mathcal{B}^k),\;
\kappa^{\mathrm{Hodge}}_{\mathrm{ch}}(\mathcal{B}^k),\;
\kappa^{\mathrm{Heis}}_{\mathrm{ch}}(\mathcal{B}^k),\;
\kappa_{\mathrm{BKM}}(\mathcal{B}^k),\;
\kappa_{\mathrm{fiber}}(\mathcal{B}^k)\bigr)
=\bigl(0,\;c/6,\;(2k{+}3)/3,\;-,\;-\bigr).
\]
The Heisenberg slot $(2k+3)/3$ is the residual-current level
$k_J=J_{(1)}J$ of the Feigin--Semikhatov normal form,
i.e.\ the $J$-line scalar $S_2^J=\kappa_J$ of
Proposition~\ref{prop:bp-jline-depth}; the Hodge slot is the
genus-1 anomaly-ratio scalar $\varrho c=c/6$ of
Proposition~\ref{prop:bp-kappa}; the categorical slot is set to
$0$ by convention in the absence of a CY host
(Table~\ref{tab:five-by-five-kappa-stratification},
Footnote *).
The two right-hand entries are recorded as ``$-$'' because no
Borcherds denominator and no compact CY fibre is attached to
$\mathcal{B}^k$ as a chart algebra. The five entries are five
distinct invariants on five distinct lanes (Master Pattern MA-6:
five $\kappa$ are not one invariant; the collapse pattern across
columns under the averaging morphism
$\Av:\fg^{E_1}\to\fg^{\mathrm{mod}}$ is itself the classification
axis). An injection of $\mathcal{B}^k$ into a row of the
$5\times5$ $\kappa$-stratification matrix endowed with all five
columns would require a companion CY${}_d$ datum supplied by
Vol~III through the two-stage factorisation
$\Phi_d^{(\Sigma_{d-1},C)}=\mathrm{Sp}^{\mathrm{ch}}_{\Sigma_{d-1},C}
\circ\Phi_d^{\mathrm{FA}}$ at a named pair $(\Sigma_{d-1},C)$
(Master Pattern MA-7: a bare one-stage $\Phi_d$ is shadow-level
and forbidden); no such pair $(\Sigma_{d-1},C)$ is asserted to
target $\mathcal{B}^k$ in the present chapter, hence the dashes.
\end{remark}

\begin{remark}[Archetype tag: BP is mixed-channel class~$\mathbf{M}$,
not a hybrid]
\label{rem:bp-archetype-attack-heal}
\index{Bershadsky--Polyakov algebra!archetype tag}
\index{five-archetype dichotomy!channel-supremum rule}
Type signature: (Open quadrant; chart-level presentation; Beilinson
level $1$; hypothesis package: nondegenerate generator-pairing
locus $U_{\mathrm{nd}}\cap U_T$; channel-supremum rule of
Chapter~\ref{chap:shadow-quadrichotomy-platonic}).

The global archetype tag of $\mathcal{B}^k$ is class~$\mathbf{M}$
(``mixed channel''), not a fifth archetype distinct from G/L/C/M/B.
The channel-supremum rule
(Chapter~\ref{chap:shadow-quadrichotomy-platonic}) reads the global
$r_{\max}$ as the maximum of per-line $r_{\max}$, here
$\max(\infty,2)=\infty$, hence class $\mathbf{M}$. The
class-$\mathbf{B}$ row is reserved for Mukai-enhanced K3-type rows
with a Borcherds denominator (Vol~III's
$\mathcal H_{\mathrm{Muk}}(K3)$ row at
$(\kappa_{\mathrm{cat}},\kappa^{\mathrm{Hodge}}_{\mathrm{ch}},
\kappa^{\mathrm{Heis}}_{\mathrm{ch}},\kappa_{\mathrm{BKM}},
\kappa_{\mathrm{fiber}})=(0,0,3,5,24)$); BP carries no
$\Delta_5$-style denominator and no Mukai lattice. The mixed
$T$-line/$J$-line profile is the diagnostic that the algebra is
non-principal DS (partition $(2,1)$); the global tag remains $\mathbf{M}$.
Witnesses: Fehily--Kawasetsu--Ridout~\cite{FKR20} for the OPE and
$c(k)$; Creutzig--Linshaw--Nakatsuka--Sato~\cite{CLNS24} for the
hook-type DS/bar transport sending $\mathcal{B}^k\mapsto\mathcal{B}^{-k-6}$.
\end{remark}


\section{Definition and OPE data}

\begin{definition}[Bershadsky--Polyakov algebra]
\label{def:bp-algebra}
\index{Bershadsky--Polyakov algebra}
\index{W-algebra!non-principal}
Let $k \in \mathbb{C} \setminus \{-3\}$. The
\emph{Bershadsky--Polyakov algebra}
$\mathcal{B}^k = \mathcal{W}^k(\mathfrak{sl}_3, f_{(2,1)})$ is the
BRST cohomology of the quantum Drinfeld--Sokolov reduction of
$V_k(\mathfrak{sl}_3)$ at the minimal nilpotent element
$f \in \mathfrak{sl}_3$ corresponding to the partition $(2,1)$
of~$3$. The Jacobson--Morozov $\mathfrak{sl}_2$-triple $(e,h,f)$
determines an $\mathrm{ad}(h)$-grading on $\mathfrak{sl}_3$; the
centralizer $\mathfrak{g}^f$ has dimension~$4$ (the number of strong
generators of $\mathcal{W}^k(\mathfrak{g}, f)$ equals $\dim \mathfrak{g}^f$
by Kac--Roan--Wakimoto), and the DS reduction produces four strong generators:
\begin{center}
\renewcommand{\arraystretch}{1.3}
\begin{tabular}{cccc}
\toprule
Generator & Conformal weight & Parity & $J$-charge \\
\midrule
$T$ & $2$ & bosonic & $0$ \\
$J$ & $1$ & bosonic & $0$ \\
$G^+$ & $3/2$ & fermionic & $+1$ \\
$G^-$ & $3/2$ & fermionic & $-1$ \\
\bottomrule
\end{tabular}
\end{center}
Here $J$ is the $\mathrm{U}(1)$ current from the residual Cartan line.
In the Feigin--Semikhatov normal form
\cite[(A.3)]{FeiginSemikhatov} its current level is
\[
k_J=\frac{2k+3}{3}.
\]
\end{definition}

The bosonic OPE data ($T$, $J$ bosonic):
\begin{align}
T_{(3)}T &= \tfrac{c}{2}, &
T_{(1)}T &= 2T, &
T_{(0)}T &= \partial T,
\label{eq:bp-ope-TT} \\
J_{(1)}J &= k_J=\frac{2k+3}{3}, &
J_{(0)}J &= 0, &
T_{(1)}J &= J.
\label{eq:bp-ope-bosonic}
\end{align}
The conformal action on the odd generators is
\begin{align}
T_{(1)}G^\pm &= \tfrac{3}{2}G^\pm, &
T_{(0)}G^\pm &= \partial G^\pm, &
J_{(0)}G^\pm &= \pm G^\pm .
\label{eq:bp-ope-primary}
\end{align}
The fermionic OPE data:
\begin{align}
G^+_{(2)}G^- &= (k+1)(2k+3), &
G^+_{(1)}G^- &= 3(k+1)J, &
G^+_{(0)}G^- &= 3{:}JJ{:}+\tfrac{3}{2}(k+1)\partial J-(k+3)T,
\label{eq:bp-ope-GpGm} \\
G^-_{(2)}G^+ &= (k+1)(2k+3), &
G^-_{(1)}G^+ &= -3(k+1)J, &
G^-_{(0)}G^+ &= 3{:}JJ{:}-\tfrac{3}{2}(k+1)\partial J-(k+3)T.
\label{eq:bp-ope-GmGp}
\end{align}
The self-OPEs $G^\pm_{(n)} G^\pm = 0$ vanish identically. The
maximal OPE pole order among generators is $p_{\max}(\mathcal{B}^k) = 4$
(from $T_{(3)}T$), and the collision depth is $k_{\max} = 3$.

\begin{convention}[BP working loci]
\label{conv:bp-generic-loci}
The universal Feigin--Semikhatov OPE presentation is used on
\[
U_{\mathrm{OPE}}=\mathbb{C}\setminus\{-3\}.
\]
The nondegenerate generator-pairing locus removes the residual
current and odd-pairing zeros:
\[
U_{\mathrm{nd}}
=U_{\mathrm{OPE}}\setminus\{-1,-3/2\},
\qquad
k_J=\frac{2k+3}{3}\ne0,\quad
(k+1)(2k+3)\ne0.
\]
The Virasoro $T$-line shadow coefficients
$S_4^T$ and $S_5^T$ require
\[
U_T
=U_{\mathrm{OPE}}\setminus
\left\{
\frac{-23\pm\sqrt{97}}{24},
\frac{-13\pm2\sqrt{31}}{15}
\right\},
\]
equivalently $c(k)\ne0$ and $5c(k)+22\ne0$. The sigma-invariant uses
the paired locus $U_T^\vee=\{k\in U_T\mid -k-6\in U_T\}$.
The curve-level chiral Hochschild statements use
$U_{\mathrm{Hoch}}$, the intersection of these loci with the PBW
Koszul, $E_\infty$-completed, finite-type/perfectness hypotheses of
Theorem~\ref{thm:hochschild-polynomial-growth}; admissible or
minimal-model simple quotient levels are excluded unless the missing
simple-quotient comparison is proved separately.
\end{convention}


\section{Central charge}\label{sec:bp-central-charge}

\begin{proposition}[BP central charge]
\label{prop:bp-central-charge}
\index{Bershadsky--Polyakov algebra!central charge}
The central charge of $\mathcal{B}^k$ is
\begin{equation}\label{eq:bp-central-charge}
c(k) \;=\; 2 - \frac{24(k+1)^2}{k+3}\,.
\end{equation}
This is the Feigin--Semikhatov / Kac--Roan--Wakimoto normalization
used by Fehily--Kawasetsu--Ridout~\cite{FKR20}.
\end{proposition}

\begin{proof}
The quantum Drinfeld--Sokolov reduction of
$V_k(\mathfrak{sl}_3)$ at the minimal nilpotent has the
Feigin--Semikhatov strong-generator packet
\eqref{eq:bp-ope-bosonic}--\eqref{eq:bp-ope-GmGp}. The
Kac--Roan--Wakimoto central-charge formula in this normalization is
the rational function~\eqref{eq:bp-central-charge}. Equivalently,
\[
c(k)=2-\frac{24(k+1)^2}{k+3}
=-\frac{2(12k^2+23k+9)}{k+3}.
\]
Two checks fix the normalization. At the admissible level
$k=-3/2$ one gets $c=-2$. Under the Feigin--Frenkel involution
$k\mapsto -k-6$, direct addition gives
$c(k)+c(-k-6)=196$, the conductor used in
Theorem~\ref{thm:bp-koszul-conductor-polynomial}.
\end{proof}

\begin{remark}[Normalization]
\label{rem:bp-convention}
The level $k$ is the affine $\mathfrak{sl}_3$ level before
minimal Drinfeld--Sokolov reduction. The formula
\eqref{eq:bp-central-charge} is the one compatible with the
Feigin--Semikhatov OPE constants
\[
J_{(1)}J=\frac{2k+3}{3},\qquad
G^+_{(2)}G^-=(k+1)(2k+3),
\]
and with the conductor identity $c(k)+c(-k-6)=196$.
\end{remark}


\section{Verdier companion and conductor}\label{sec:bp-koszul}

The partition $(2,1)$ is self-transpose: $(2,1)^t = (2,1)$. The
objects remain distinct:
\[
A=\mathcal{B}^k,\qquad B(A),\qquad
A^i=H^*(B^{\mathrm{ch}}(A)),\qquad
A^!=(A^i)^\vee,\qquad Z^{\mathrm{der}}_{\mathrm{ch}}(A).
\]
Bar--cobar inversion identifies $\Omega(B(A))$ with $A$; Verdier
duality on the finite-type or completed coalgebra lane produces
$A^!$. Under the hook-type transport theorem
(Fehily~\cite{FehilyHook}, Creutzig--Linshaw--Nakatsuka--Sato
\cite{CLNS24}), this Verdier companion preserves the nilpotent orbit
and acts on the level via the Feigin--Frenkel involution of the parent
algebra $\widehat{\mathfrak{sl}}_3$.

\begin{theorem}[Bershadsky--Polyakov central-charge conductor identity]
\label{thm:bp-koszul-conductor-polynomial}
\index{Bershadsky--Polyakov algebra!central-charge conductor!polynomial identity}
\index{central-charge conductor!polynomial identity}
Type signature: (Open quadrant; chain-level presentation in the
Feigin--Semikhatov normal form; Beilinson level $5$ scalar shadow;
hypothesis package: rational-function field $\mathbb{Q}(k)$, no
admissible-quotient or simple-quotient comparison invoked).

In the rational-function field $\mathbb{Q}(k)$, the central-charge conductor
function
\[
K^c_{\mathcal{B}}(k)
\;:=\;
c(\mathcal{B}^k) + c(\mathcal{B}^{-k-6})
\;=\;
\Bigl(2 - \frac{24(k+1)^2}{k+3}\Bigr)
 + \Bigl(2 - \frac{24(-k-5)^2}{-k-3}\Bigr)
\]
is identically equal to $196$:
\[
K^c_{\mathcal{B}}(k) \;\equiv\; 196 \qquad \text{as an element of }
\mathbb{Q}(k).
\]
Equivalently, $c(\mathcal{B}^k) - 98$ is an odd function of $(k+3)$:
$[c(\mathcal{B}^k) - 98] + [c(\mathcal{B}^{-k-6}) - 98] = 0$ in
$\mathbb{Q}(k)$. The fixed point $k = -3$ of the level involution
$k \mapsto -k-6$ coincides with the critical level
$-h^\vee(\mathfrak{sl}_3) = -3$, where both summands are singular;
the polynomial identity holds globally even though no single real
level attains $c(\mathcal{B}^k) = 98$ as a regular function value.
\end{theorem}

\begin{proof}
Direct polynomial-identity computation in $\mathbb{Q}(k)$. Using
$-k-3 = -(k+3)$ and $(-k-5)^2 = (k+5)^2$,
\begin{align*}
K^c_{\mathcal{B}}(k)
&= \Bigl(2 - \frac{24(k+1)^2}{k+3}\Bigr)
 + \Bigl(2 - \frac{24(k+5)^2}{-(k+3)}\Bigr) \\
&= 4 - \frac{24(k+1)^2}{k+3} + \frac{24(k+5)^2}{k+3}
 = 4 + \frac{24\bigl[(k+5)^2 - (k+1)^2\bigr]}{k+3} \\
&= 4 + \frac{24 \cdot (2k+6)(4)}{k+3}
 = 4 + \frac{192(k+3)}{k+3}
 = 4 + 192 = 196\,.
\end{align*}
The numerator $(k+5)^2 - (k+1)^2 = 8(k+3)$ carries the
pole-killing factor exactly once, so the cancellation is an identity
of rational functions (not a limit). Hence $K^c_{\mathcal{B}}(k) - 196
\equiv 0$ in $\mathbb{Q}(k)$.
\end{proof}

\begin{proposition}[BP Verdier companion involution and conductor]
\label{prop:bp-self-duality}
\index{Bershadsky--Polyakov algebra!Verdier-Koszul companion}
\index{Koszul duality!non-principal Verdier companion}
Type signature: (Open quadrant; chain-level Verdier-Koszul lane
$1\leftrightarrow 2$ with reconstruction theorem Theorem~A
\textup{(}chiral Koszul reflection $K^2\simeq\mathrm{id}$\textup{)};
Beilinson levels $1$ and $5$ scalar shadow; hypothesis package:
finite-type-or-completed bar-dual coalgebra,
non-critical level $k\neq-3$, subregular hook-type DS/bar
transport statement \cite{CLNS24,FehilyHook}).

For $k \neq -3$ \textup{(}away from the critical level $k = -h^\vee$\textup{)},
assume that the bar-dual coalgebra
$A^i=H^*(B^{\mathrm{ch}}(A))$, with $A=\mathcal{B}^k$, is finite type in each
weight, or is completed with a continuous nondegenerate Verdier
pairing, and assume the subregular DS/bar transport statement.
Then the same-family Verdier-Koszul companion of $\mathcal{B}^k$ is
$(\mathcal{B}^k)^! \simeq \mathcal{B}^{k'}$ with
$k' = -k - 6$. The dual central charge is $c' = c(-k-6) = 196 - c$.
The central-charge conductor satisfies
\begin{equation}\label{eq:bp-conductor}
K^c_{\mathcal{B}} \;=\; c(k) + c(-k-6) \;=\; 196
\qquad \text{as a polynomial identity in } \mathbb{Q}(k)
\end{equation}
per Theorem~\ref{thm:bp-koszul-conductor-polynomial}.
\end{proposition}

\begin{proof}
The polynomial identity~\eqref{eq:bp-conductor} is
Theorem~\ref{thm:bp-koszul-conductor-polynomial}. The
self-transposition of $(2,1)$ and the Feigin--Frenkel involution
identify the companion level
$k^\vee = -k - 2h^\vee = -k-6$
\textup{(}where $h^\vee(\mathfrak{sl}_3) = 3$\textup{)}.
Under the assumed subregular DS/bar transport statement on the DS
quotient, the hook-transport theorem~\cite{CLNS24,FehilyHook} gives
$(\mathcal{B}^k)^! \simeq \mathcal{B}^{-k-6}$.
\end{proof}

\begin{remark}[Comparison of central-charge conductors]
\label{rem:bp-conductor-comparison}
\index{central-charge conductor!comparison}
The central-charge conductor grows rapidly with the complexity of the
DS reduction:
\begin{center}
\renewcommand{\arraystretch}{1.2}
\begin{tabular}{lcc}
\toprule
Algebra & Partition & $K$ \\
\midrule
$\mathrm{Vir}_c$ & (principal, $\mathfrak{sl}_2$) & $26$ \\
$\mathcal{W}_3$ (principal) & $(3)$ & $100$ \\
$\mathcal{B}^k$ (minimal) & $(2,1)$ & $196$ \\
\bottomrule
\end{tabular}
\end{center}
The principal $\mathcal{W}_3$ conductor $K = 100$ arises from a
different central charge parametrization: $c_{\mathcal{W}_3}(k) =
2 - 24(k{+}2)^2/(k{+}3)$. The self-dual point
$c_{\mathrm{sd}} = K/2$ is $c = 13$ for Virasoro, $c = 50$ for
$\mathcal{W}_3$, and $c = 98$ for the BP algebra. Unlike the
Virasoro and $\mathcal{W}_3$ cases, the BP self-dual point $c = 98$
is not achieved at any real level: the central charge
$c(k)$ satisfies $c \leq 2$ for $k > -3$ and $c \geq 194$ for
$k < -3$, leaving the interval $(2, 194) \ni 98$ unattained.
The formal self-dual point requires complex level
$k = -3 \pm 2i$.
\end{remark}

\begin{proposition}[Unified shift formula for $\mathfrak{sl}_3$
central-charge conductors]
\label{prop:sl3-conductor-shift-formula}
\index{central-charge conductor!sl3@$\mathfrak{sl}_3$!unified shift formula}
\index{shift parameter!nilpotent orbit}
For each non-zero nilpotent orbit
$\mathcal{O}_\lambda \subset \mathfrak{sl}_3$, let
$\mathcal{W}^k_\lambda := \mathcal{W}^k(\mathfrak{sl}_3, f_\lambda)$
denote the Drinfeld--Sokolov reduction at $f_\lambda \in \mathcal{O}_\lambda$.
Both non-trivial central charges admit the common parametrization
\begin{equation}\label{eq:sl3-conductor-c-shifted}
c\bigl(\mathcal{W}^k_\lambda\bigr)
\;=\;
2 \;-\; \frac{24\bigl(k + h^\vee - s_\lambda\bigr)^2}{k + h^\vee},
\qquad h^\vee(\mathfrak{sl}_3) = 3,
\end{equation}
where the \emph{conductor shift} $s_\lambda \in \mathbb{Z}_{>0}$ is the
orbit-specific invariant
\begin{equation}\label{eq:sl3-shift-table}
s_{(3)} \;=\; 1\quad(\text{principal}),
\qquad
s_{(2,1)} \;=\; 2\quad(\text{minimal}).
\end{equation}
Under the Feigin--Frenkel involution $k \mapsto k' = -k - 2h^\vee$,
the central-charge conductor evaluates to the closed-form Lie-theoretic
expression
\begin{equation}\label{eq:sl3-conductor-formula}
K_\lambda \;:=\; c\bigl(\mathcal{W}^k_\lambda\bigr)
+ c\bigl(\mathcal{W}^{k'}_\lambda\bigr)
\;=\; 4 + 96\, s_\lambda
\quad \text{in } \mathbb{Q}(k).
\end{equation}
Specialising~\eqref{eq:sl3-conductor-formula} to the two non-trivial
$\mathfrak{sl}_3$ orbits recovers
$K_{(3)} = 4 + 96 = 100$ and $K_{(2,1)} = 4 + 192 = 196$.
\end{proposition}

\begin{proof}
The presentation~\eqref{eq:sl3-conductor-c-shifted} is read off from
the Kac--Roan--Wakimoto central-charge formula for principal
\textup{(}Fateev--Lukyanov, $s = 1$\textup{)} and minimal
\textup{(}Bershadsky--Polyakov,
Proposition~\textup{\ref{prop:bp-central-charge}}, $s = 2$\textup{)}
DS reductions of $\mathfrak{sl}_3$ at $h^\vee = 3$.
The Feigin--Frenkel involution $k' = -k - 6$ sends
$k + h^\vee \mapsto -(k + h^\vee)$, hence
$k + h^\vee - s_\lambda \mapsto -(k + h^\vee + s_\lambda)$, so
$(k + h^\vee - s_\lambda)^2 \mapsto (k + h^\vee + s_\lambda)^2$,
while the denominator $k + h^\vee$ flips sign once.
Adding the two summands,
\begin{align*}
K_\lambda
&= 4 \;-\; \frac{24(k + h^\vee - s_\lambda)^2}{k + h^\vee}
 \;+\; \frac{24(k + h^\vee + s_\lambda)^2}{k + h^\vee} \\
&= 4 \;+\; \frac{24\bigl[(k + h^\vee + s_\lambda)^2
 - (k + h^\vee - s_\lambda)^2\bigr]}{k + h^\vee}.
\end{align*}
Using the difference-of-squares identity
$(A + B)^2 - (A - B)^2 = 4AB$ with $A = k + h^\vee$ and $B = s_\lambda$,
the numerator becomes $4 s_\lambda (k + h^\vee)$, and the
$(k + h^\vee)$ factor cancels the denominator exactly once
\textup{(}as a rational-function identity, not a limit\textup{)}.
This yields $K_\lambda = 4 + 96 s_\lambda$ in $\mathbb{Q}(k)$, with
the level-independence reduced to the algebraic cancellation.
\end{proof}

\begin{remark}[Three conductor computations]
\label{rem:sl3-conductor-verification}%
\index{central-charge conductor!multi-path computation}
Equation~\eqref{eq:sl3-conductor-formula} has three independent
numerical faces.
First, direct rational-function expansion at $\lambda = (2,1)$,
$s_\lambda = 2$, gives
$K = 4 + 96 \cdot 2 = 196$, matching
Theorem~\textup{\ref{thm:bp-koszul-conductor-polynomial}} and
Proposition~\textup{\ref{prop:bp-self-duality}}.
At $\lambda = (3)$, $s_\lambda = 1$, it gives $K = 4 + 96 = 100$,
matching the Freudenthal--de Vries-style formula
$K_N = 2(N{-}1)(2N^2 + 2N + 1) = 100$ at $N = 3$
\textup{(}Remark~\textup{\ref{rem:koszul-conductor-explicit}})\textup{.}
Second, the self-dual central charge is $c_* = K_\lambda / 2$, so
$c_*((2,1)) = 98$ and $c_*((3)) = 50$, matching the values quoted in
Remark~\textup{\ref{rem:bp-conductor-comparison}} and
\textup{\ref{rem:self-dual-complementarity}}.
Third, with anomaly ratios $\varrho_{(3)} = 5/6$ and
$\varrho_{(2,1)} = 1/6$,
the $\kappa$-complementarity sums are
$\varrho_\lambda K_\lambda = (5/6)(100) = 250/3$ and
$(1/6)(196) = 98/3$, matching
Proposition~\textup{\ref{prop:bp-complementarity}} and the
$\mathcal{W}_3$ entry of
Table~\textup{\ref{tab:complementarity-landscape}}.
\end{remark}

\begin{remark}[Why the shift parameter $s_\lambda$ is the genuine
Lie-theoretic invariant]
\label{rem:shift-parameter-lie-theoretic}%
\index{shift parameter!Lie-theoretic interpretation}
The shift $s_\lambda$ in
Proposition~\textup{\ref{prop:sl3-conductor-shift-formula}} encodes
the orbit-dependent characteristic-grading offset that distinguishes
one $\mathfrak{sl}_3$ DS reduction from another at fixed $h^\vee = 3$
and fixed quartic-coefficient $24 = N(N^2{-}1)\big|_{N=3}$. Both
non-zero $\mathfrak{sl}_3$ orbits share the common KRW shape
$c = 2 - 24(k + h^\vee - s_\lambda)^2/(k + h^\vee)$; only the offset
$s_\lambda \in \{1, 2\}$ distinguishes principal from minimal. The
constant $96 = 4 \cdot 24$ in
$K_\lambda = 4 + 96 s_\lambda$ factors as
\textup{(}difference-of-squares factor $4$\textup{)} times
\textup{(}KRW quartic-coefficient $24$\textup{)}, so any analogous
identity at higher rank would require both factors to remain tied to
$h^\vee$ and the orbit data of $\mathfrak{g}$. Whether
$K_\lambda = 4 + (4 \cdot N(N^2{-}1)) \cdot s_\lambda$ holds for
$\mathcal{W}^k(\mathfrak{sl}_N, f_\lambda)$ at \emph{all} non-principal
orbits is conjectural; here we record only the $\mathfrak{sl}_3$
specialisation as unconditional. The principal-orbit specialisation
$K_N = 4 + 4 N(N^2{-}1) \cdot 1 + \mathrm{lower\ rank\ terms}$ matches
$K_3 = 100$ but for $N \geq 4$ the closed form
$K_N = 2(N{-}1)(2N^2 + 2N + 1)$
\textup{(}Remark~\textup{\ref{rem:koszul-conductor-explicit}}\textup{)}
is no longer of this two-parameter linear-in-$s$ shape, because the
principal-orbit Fateev--Lukyanov coefficient $N(N^2{-}1)$ scales with
$N$. The unified $\mathfrak{sl}_3$ form is therefore an
\emph{$\mathfrak{sl}_3$-rank-specific} bridge identity, not a
universal Lie-theoretic identity.
\end{remark}


\section{Modular characteristic}\label{sec:bp-kappa}

\begin{proposition}[Modular characteristic of $\mathcal{B}^k$]
\label{prop:bp-kappa}
\index{Bershadsky--Polyakov algebra!modular characteristic}
\index{kappa@$\kappa$!Bershadsky--Polyakov}
For $k \in U_{\mathrm{OPE}}$, the modular characteristic of the
Bershadsky--Polyakov algebra is
\begin{equation}\label{eq:bp-kappa}
\kappa(\mathcal{B}^k) \;=\; \frac{c}{6}
\;=\; \frac{1}{6}\Bigl(2 - \frac{24(k+1)^2}{k+3}\Bigr).
\end{equation}
When $c(k)\ne0$, the anomaly ratio is
$\varrho(\mathcal{B}^k) = \kappa/c = 1/6$.
\end{proposition}

\begin{proof}
The full-algebra modular characteristic is the anomaly-ratio scalar,
not the sum of the pure line curvatures. In the
Feigin--Semikhatov normal form the strong generators have weights and
parities
\[
J_{h=1}^{\mathrm{bos}},\qquad
G^+_{h=3/2}^{\mathrm{ferm}},\qquad
G^-_{h=3/2}^{\mathrm{ferm}},\qquad
T_{h=2}^{\mathrm{bos}}.
\]
The genus-$1$ anomaly ratio is therefore
\[
\varrho(\mathcal{B})
=1-\frac{1}{3/2}-\frac{1}{3/2}+\frac{1}{2}
=1-\frac{2}{3}-\frac{2}{3}+\frac{1}{2}
=\frac{1}{6}.
\]
Multiplying the Kac--Roan--Wakimoto central charge
\eqref{eq:bp-central-charge} gives
\[
\kappa(\mathcal{B}^k)
=\varrho(\mathcal{B})\,c_{\mathrm{BP}}(k)
=\frac{c_{\mathrm{BP}}(k)}{6}.
\]
The line-level curvatures remain visible but separate:
$S_2^T=c/2$, $S_2^J=(2k+3)/3$, and the mixed odd pairing
$G^+_{(2)}G^-=(k+1)(2k+3)$. These line constants are projections of
the BP obstruction tower; they do not replace the full scalar
$\kappa=c/6$. The Drinfeld--Sokolov BRST ghost scalar is also a
different package:
\[
K_{\mathrm{ghost}}^{\mathrm{leg}}(\mathrm{BP}_k;\mathrm{DS})
=-\frac{6(4k^2+9k+3)}{k+3}
\]
by Corollary~\ref{cor:uc-K-BP}, hence it is neither the conductor
$196$ nor the modular characteristic $c/6$.
\end{proof}

\begin{proposition}[Complementarity]
\label{prop:bp-complementarity}
\index{Bershadsky--Polyakov algebra!complementarity}
\index{complementarity!Bershadsky--Polyakov}
For $k \neq -3$, on the same finite-type or completed Verdier lane as
Proposition~\ref{prop:bp-self-duality}, the complementarity sum for the
Bershadsky--Polyakov Verdier-Koszul pair is
\begin{equation}\label{eq:bp-complementarity}
\kappa(k) + \kappa(-k-6)
\;=\;
\frac{c(k)}{6} + \frac{196 - c(k)}{6}
\;=\;
\frac{196}{6}
\;=\;
\frac{98}{3}\,.
\end{equation}
This is an instance of the general W-type complementarity
$\kappa + \kappa' = \varrho \cdot K$ here $\varrho = 1/6$ and $K = 196$.
\end{proposition}

\begin{proposition}[Curve-level chiral Hochschild package for BP]
\label{prop:bp-curve-level-chirhoch}
\index{Bershadsky--Polyakov algebra!chiral Hochschild}
\index{chiral Hochschild cohomology!Bershadsky--Polyakov}
Type signature: (Open quadrant; chain-level chiral-Hochschild
lane $1\to 3$ with reconstruction theorem Theorem~H
\textup{(}$\ChirHoch^\bullet\subset\{0,1,2\}$\textup{)};
Beilinson level $3$ derived chiral centre; hypothesis package:
working locus $U_{\mathrm{Hoch}}$ of
Convention~\ref{conv:bp-generic-loci} \textup{(}PBW Koszul,
$E_\infty$-completed, finite-type/perfectness; admissible and
minimal-model simple quotients excluded\textup{)}, BP centre
condition $Z_{\mathrm{ch}}(\mathcal{B}^k)=\mathbb{C}\mathbf{1}$,
BP outer-derivation calculation
$\operatorname{OutDer}_{\mathrm{ch}}=\mathbb{C}\langle\delta_J,
\delta_k\rangle$).

Let $k\in U_{\mathrm{Hoch}}$. Assume the BP center condition
\[
Z_{\mathrm{ch}}(\mathcal{B}^k)=\mathbb{C}\mathbf{1},
\qquad
Z_{\mathrm{ch}}(\mathcal{B}^{-k-6})=\mathbb{C}\mathbf{1},
\]
and the BP derivation calculation
\[
\operatorname{OutDer}_{\mathrm{ch}}(\mathcal{B}^k)
=\mathbb{C}\langle \delta_J,\delta_k\rangle ,
\]
where $\delta_J$ is the residual-current deformation and $\delta_k$
is the level deformation of the Feigin--Semikhatov family. Then the
curve-level chiral Hochschild cohomology satisfies
\[
\ChirHoch_X^n(\mathcal{B}^k)=0
\qquad (n\notin\{0,1,2\}),
\]
and its Hilbert polynomial is
\[
P_{\mathcal{B}^k}(t)
=\sum_{n\ge0}\dim\ChirHoch_X^n(\mathcal{B}^k)t^n
=1+2t+t^2.
\]
Without the displayed derivation calculation, Theorem~\ref{thm:hochschild-polynomial-growth}
gives only the curve-level form
$P_{\mathcal{B}^k}(t)=1+d_1t+t^2$ with
$d_1=\dim\ChirHoch_X^1(\mathcal{B}^k)$.
No topological Hochschild, categorical Hochschild, product-ambient
bulk, or physical bulk identification is asserted.
\end{proposition}

\begin{proof}
Theorem~\ref{thm:hochschild-polynomial-growth} applies on
$U_{\mathrm{Hoch}}$ and gives the amplitude bound
$\ChirHoch_X^n(\mathcal{B}^k)=0$ for $n<0$ and $n>2$. Its polynomial
formula is
\[
P_{\mathcal{B}^k}(t)
=\dim Z_{\mathrm{ch}}(\mathcal{B}^k)
 +\dim\ChirHoch_X^1(\mathcal{B}^k)t
 +\dim Z_{\mathrm{ch}}\bigl((\mathcal{B}^k)^!\bigr)t^2.
\]
On the Verdier lane of Proposition~\ref{prop:bp-self-duality},
$(\mathcal{B}^k)^!\simeq\mathcal{B}^{-k-6}$, so the two center
hypotheses give the constant and quadratic coefficients equal to~$1$.
The displayed outer-derivation calculation gives the linear
coefficient~$2$, generated by the residual current and level
directions.
\end{proof}


\section{Shadow obstruction tower}\label{sec:bp-shadow}

The Bershadsky--Polyakov algebra has two bosonic generator lines:
the $T$-line (Virasoro restriction) and the $J$-line ($\mathrm{U}(1)$
restriction). The shadow obstruction tower is computed independently
on each line.

\subsection{The T-line: class M}

On the $T$-line, the shadow projections $S_r$ are determined
entirely by the Virasoro self-OPE $T_{(3)}T = c/2$, $T_{(1)}T = 2T$,
$T_{(0)}T = \partial T$. The shadow data coincides with that of
$\mathrm{Vir}_{c(k)}$:

\begin{proposition}[T-line shadow depth]
\label{prop:bp-tline-depth}
\index{Bershadsky--Polyakov algebra!shadow tower!T-line}
For $k\in U_T$, the shadow obstruction tower of $\mathcal{B}^k$
on the $T$-line has infinite depth:
$r_{\max}^T = \infty$. The tower is class~$M$. The shadow data is
\begin{equation}\label{eq:bp-tline-shadows}
S_2^T = \frac{c}{2}, \qquad
S_3^T = 2, \qquad
S_4^T = \frac{10}{c(5c+22)}, \qquad
S_5^T = \frac{-48}{c^2(5c+22)}, \qquad
\Delta_T = \frac{40}{5c+22}\,.
\end{equation}
Equivalently, after substituting
$c=c_{\mathrm{BP}}(k)=-2(12k^2+23k+9)/(k+3)$,
\[
S_4^T
=\frac{5(k+3)^2}
{8(12k^2+23k+9)(15k^2+26k+3)},\qquad
S_5^T
=\frac{3(k+3)^3}
{2(12k^2+23k+9)^2(15k^2+26k+3)}.
\]
The per-line modular characteristic is $\kappa_T = c/2$
\textup{(}the Virasoro value, distinct from the full-algebra
$\kappa = c/6$\textup{)}.
\end{proposition}

\begin{proof}
On the $T$-line, the MC equation projects to the single-line
recursion with $\kappa_T = c/2$, $\alpha = S_3 = 2$, and
$S_4 = Q^{\mathrm{contact}}_{\mathrm{Vir}} = 10/(c(5c+22))$.
The next Virasoro coefficient is $S_5=-48/[c^2(5c+22)]$, and direct
substitution of $c_{\mathrm{BP}}(k)$ gives the displayed rational form.
The critical discriminant is $\Delta_T = 8 \kappa_T S_4 =
8 \cdot (c/2) \cdot 10/(c(5c+22)) = 40/(5c+22)$. Since
$k\in U_T$ gives $c\ne0$ and $5c + 22 \neq 0$,
$\Delta_T \neq 0$ and the shadow metric is irreducible.
The tower does not terminate.
\end{proof}

\begin{remark}[Quartic line separation]
\label{rem:bp-quartic-line-separation}
The coefficient $S_4^T$ in
Proposition~\ref{prop:bp-tline-depth} is only the Virasoro
$T$-line quartic contact. A scalar quartic coefficient on the
mixed odd bilinear line generated by $G^+G^-$ is a different
invariant: the Feigin--Semikhatov simple pole contains
$3{:}JJ{:}+\tfrac32(k+1)\partial J-(k+3)T$, so the mixed line
does not inherit $S_4^T$ by restriction.
\end{remark}


\subsection{The J-line: class G}

The $J$-line is spanned by the $\mathrm{U}(1)$ current. The
$J$-self-OPE has a single double pole $J_{(1)}J = (2k+3)/3$
and $J_{(0)}J = 0$: a purely quadratic
(Heisenberg-type) structure at the residual level.

\begin{proposition}[J-line shadow depth]
\label{prop:bp-jline-depth}
\index{Bershadsky--Polyakov algebra!shadow tower!J-line}
For $k\in U_{\mathrm{OPE}}$, the $J$-line shadow coefficients are
\[
S_2^J=\kappa_J=\frac{2k+3}{3},\qquad
S_r^J=0\quad(r\ge3).
\]
If $k\ne -3/2$, this is a nondegenerate class~$G$ tower with
$r_{\max}^J=2$. At $k=-3/2$ the residual current level vanishes and
the $J$-line is a degenerate Gaussian line, not a nondegenerate
depth-$2$ witness.
\end{proposition}

\begin{proof}
The $J$-self-OPE is purely quadratic: $J_{(1)}J = (2k+3)/3$
and $J_{(0)}J = 0$. The cubic shadow requires a composite-field
contribution from $J_{(0)}J$, which vanishes. Higher degrees
vanish by induction: the shadow obstruction tower terminates at
degree~$2$.
\end{proof}

\begin{remark}[Mixed depth: the first non-principal witness]
\label{rem:bp-mixed-depth}
\index{shadow depth!mixed}
For $k\in U_{\mathrm{nd}}\cap U_T$, the Bershadsky--Polyakov algebra
has shadow depth $\infty$ on the $T$-line and shadow depth~$2$ on the
$J$-line. In the principal
landscape, each algebra has a single shadow class
(Heisenberg is~$G$ on every line, $\widehat{\mathfrak{sl}}_N$
is~$L$, Virasoro is~$M$). The BP algebra is the first
non-principal example of mixed shadow depth within a single algebra.
The global shadow class is $M$ (the maximum over all lines).
The per-line classification is
$(T\colon M,\; J\colon G,\; G^\pm\colon \text{fermionic})$.
This multi-channel structure is shared with the $\mathcal{N}=2$
SCA (Chapter~\ref{chap:n2-sca}), which has the same generator
content and the same per-line classification.
\end{remark}


\subsection{Sigma-invariant}

The sigma-invariant measures the complementarity defect at each
degree:
\begin{equation}\label{eq:bp-sigma}
\sigma^{(r)}(k) \;:=\; S_r^T(c(k)) + S_r^T(c(-k-6)),
\qquad c' = 196 - c\,.
\end{equation}

\begin{proposition}[Sigma non-vanishing]
\label{prop:bp-sigma}
\index{Bershadsky--Polyakov algebra!sigma-invariant}
For $k\in U_T^\vee$, the $T$-line sigma functions satisfy
$\sigma^{(2)} = 98$,
$\sigma^{(3)} = 4$,
\[
\sigma^{(4)}
=
\frac{10}{c(5c+22)}
+
\frac{10}{(196-c)(1002-5c)},
\]
and, for every $r\ge2$, $\sigma^{(r)}$ is a nonzero rational
function of~$k$. At a fixed level it is nonzero after removing its
finite pole and zero divisors.
\end{proposition}

\begin{proof}
At degree~$2$: $\sigma^{(2)} = c/2 + (196-c)/2 = 98$.
At degree~$3$: $\sigma^{(3)} = 2 + 2 = 4$.
At degree~$4$, substitute $c'=196-c$ into the Virasoro quartic
contact coefficient $S_4=10/[c(5c+22)]$:
\[
\sigma^{(4)}
=\frac{10}{c(5c+22)}
 +\frac{10}{c'(5c'+22)}
=\frac{10}{c(5c+22)}
 +\frac{10}{(196-c)(1002-5c)}.
\]
This rational function is not identically zero. Its polar divisors in
the $c$-line are $c=0$, $c=196$, $c=-22/5$, and $c=1002/5$. After
$c=c_{\mathrm{BP}}(k)$ the pole pairs are
$(c,c')=(0,196)$ and $(-22/5,1002/5)$, matching
Computation~\ref{comp:bp-shadow-tower}.
The Riccati algebraicity theorem~\ref{thm:riccati-algebraicity}
propagates the nonzero rational-function statement to all higher
degrees on the Virasoro $T$-line. A level lying on a higher zero
divisor is excluded from the fixed-level non-vanishing statement.
\end{proof}


\section{Comparison with $\mathcal{W}_3$ and the
$\mathcal{N}=2$ SCA}\label{sec:bp-comparison}

The three DS reductions of $\widehat{\mathfrak{sl}}_3$ are the
full affine algebra (trivial orbit), the Bershadsky--Polyakov
algebra (minimal orbit), and $\mathcal{W}_3$ (principal orbit).
The comparison:

\begin{center}
\renewcommand{\arraystretch}{1.3}
\begin{tabular}{l|ccc}
\toprule
& $\widehat{\mathfrak{sl}}_{3,k}$ &
$\mathcal{B}^k$ (minimal) &
$\mathcal{W}_3$ (principal) \\
\midrule
Partition & $(1,1,1)$ & $(2,1)$ & $(3)$ \\
Generators & $J^a$ ($a = 1,\ldots,8$) & $T, J, G^\pm$ & $T, W$ \\
$\#$ generators & $8$ & $4$ & $2$ \\
Generator weights & $1$ & $2, 1, 3/2, 3/2$ & $2, 3$ \\
$\kappa$ & $\dfrac{4(k+3)}{3}$ & $\dfrac{c}{6}$ & $\dfrac{5c}{6}$ \\
$\varrho = \kappa/c$ & $\dfrac{(k+3)^2}{6k}$ & $\dfrac{1}{6}$ & $\dfrac{5}{6}$ \\
$K$ & $16$ & $196$ & $100$ \\
Shadow class & L & M (mixed) & M \\
\bottomrule
\end{tabular}
\end{center}

The row $\varrho = \kappa/c$ records the instantaneous ratio; it coincides with the complementarity slope $\varrho_A$ appearing in $\kappa(A) + \kappa(A^!) = \varrho_A \cdot K(A)$ only when $\kappa$ is linear in $c$. For the $\mathcal{W}$-family ($\mathcal{B}^k$, $\mathcal{W}_3$, Virasoro) the two senses agree. For $\widehat{\mathfrak{sl}}_{3,k}$ the instantaneous ratio $(k+3)^2/(6k)$ is level-dependent, while the complementarity slope is $\varrho_{\mathrm{KM}} = 0$ (reflecting $\kappa(V_k(\mathfrak{g})) + \kappa(V_{-k-2h^\vee}(\mathfrak{g})) = 0$ for affine Kac--Moody). The pattern $\varrho(\mathcal{B}^k) + \varrho(\mathcal{W}_3) = 1/6 + 5/6 = 1$ reflects the complementary roles of the minimal and principal reductions: together they account for the full anomaly of the parent algebra.

\begin{remark}[Relation to the $\mathcal{N}=2$ SCA]
\label{rem:bp-n2-comparison}
\index{Bershadsky--Polyakov algebra!relation to N=2 SCA}
The BP algebra and the $\mathcal{N}=2$ SCA share the same
generator content ($T, J, G^+, G^-$ with the same conformal
weights and parities) and the same per-line shadow classification
($T\colon M$, $J\colon G$). The OPE structures differ:
the $\mathcal{N}=2$ SCA has
$J_{(1)}J = c/3$ and $G^+_{(2)}G^- = c/3$
(Chapter~\ref{chap:n2-sca}, eq.~\eqref{eq:n2-ope-JJ}
and~\eqref{eq:n2-ope-GG}),
while the BP algebra has
\[
J_{(1)}J=\frac{2k+3}{3},\qquad
G^+_{(2)}G^-=(k+1)(2k+3),
\]
and the simple pole of $G^+G^-$ contains
$3{:}JJ{:}+\tfrac32(k+1)\partial J-(k+3)T$. The shared generator
profile therefore does not identify the two OPE algebras. The
modular characteristics also differ:
$\kappa(\mathrm{SCA}_c) = (6{-}c)/(2(3{-}c))$ (a rational
function) vs $\kappa(\mathcal{B}^k) = c/6$ (a linear function of
$c$).
\end{remark}


\section{The hook-type corridor}\label{sec:bp-hook}

The Bershadsky--Polyakov algebra is the seed of the hook-type
transport corridor. A hook partition
$\lambda = (N-j, 1^j)$ of $N$ is self-transpose if and only if
$N$ is odd and $j = (N-1)/2$
(Proposition~\ref{prop:bp-hook-series} below).

\begin{proposition}[Self-transpose hook companion involution]
\label{prop:bp-hook-series}
\index{hook-type corridor}
For every odd $N = 2m+1$, at non-critical level
($k \neq -h^\vee = -N$), and $\lambda = (m+1, 1^m)$, assume the
hook-type DS/bar transport statement and a finite-type or completed
continuous Verdier pairing on the bar-dual coalgebra. Then the
$\mathcal{W}$-algebra
$\mathcal{W}_k(\mathfrak{sl}_N, f_\lambda)$ has same-family
Verdier-Koszul companion
$(\mathcal{W}_k(\mathfrak{sl}_N, f_\lambda))^!
\simeq
\mathcal{W}_{k'}(\mathfrak{sl}_N, f_\lambda)$
with $k' = -k - 2N$ \textup{(}where $h^\vee(\mathfrak{sl}_N) = N$,
so $k' = -k - 2h^\vee$\textup{)}.
The first three cases are:
\begin{center}
\renewcommand{\arraystretch}{1.2}
\begin{tabular}{cccc}
\toprule
$N$ & $\lambda$ & Algebra & Dual level shift \\
\midrule
$3$ & $(2,1)$ & $\mathcal{B}^k$ & $k' = -k - 6$ \\
$5$ & $(3,1,1)$ & $\mathcal{W}^k(\mathfrak{sl}_5, f_{(3,1,1)})$
 & $k' = -k - 10$ \\
$7$ & $(4,1,1,1)$ & $\mathcal{W}^k(\mathfrak{sl}_7, f_{(4,1,1,1)})$
 & $k' = -k - 14$ \\
\bottomrule
\end{tabular}
\end{center}
\end{proposition}

\begin{corollary}[Hook-series seed: BP conductor]
\label{cor:bp-koszul-conductor-seed}
The Bershadsky--Polyakov algebra
$\mathcal{B}^k = \mathcal{W}_k(\mathfrak{sl}_3, f_{(2,1)})$
is the $N=3$ case of Proposition~\ref{prop:bp-hook-series}: at
non-critical level $k \neq -3$, the central-charge conductor identity
\[
 K^c_{\mathcal{B}}(k) \;:=\;
 c(\mathcal{B}^k) + c(\mathcal{B}^{-k-6}) \;\equiv\; 196
 \qquad \text{in } \mathbb{Q}(k)
\]
of Theorem~\ref{thm:bp-koszul-conductor-polynomial} seeds the
hook-series: every self-transpose hook of odd $N$ produces a
same-family companion fixed locus, with algebraic Verdier-Koszul
companionhood conditional on hook-type DS/bar transport and the
finite-type or completed Verdier pairing; BP ($N=3$) is the first
entry.
\end{corollary}

\begin{remark}[Bala--Carter dependence of the conductor]
\label{rem:kw-bp-vs-principal}
The Bershadsky--Polyakov conductor $K^c_{\mathcal{B}} = 196$
differs from the $\mathcal{W}_3$ conductor $K^c_{\mathcal{W}_3} = 100$
\textup{(}principal $\mathfrak{sl}_3$, partition $(3)$\textup{)},
reflecting the Bala--Carter dependence
$(\mathfrak{g}, f) \mapsto K^c$. The principal-case formula
$K = 2r + 12\sum_i m_i(m_i+1)$ of the W-algebra chapter does not
extend to non-principal nilpotents. BP and $\mathcal{W}_3$ have
distinct same-family companion involutions ($k \mapsto -k-6$ for BP
versus the principal involution for $\mathcal{W}_3$); interpreting
these fixed loci as algebraic Verdier-Koszul companion loci is
conditional on the corresponding DS/bar transport statement and
Verdier finiteness/completion.
\end{remark}
