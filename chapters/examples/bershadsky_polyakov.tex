...
$J$-self-OPE has a single double pole $J_{(1)}J = c/3$ and
$J_{(0)}J = 0$: a purely quadratic (Heisenberg-type) structure.
...
$\kappa_J = c/3$, with cubic and all higher shadows vanishing:
$S_r^J = 0$ for $r \geq 3$.
\end{proposition}

\begin{proof}
The $J$-self-OPE is purely quadratic: $J_{(1)}J = c/3$ and
$J_{(0)}J = 0$.  The cubic shadow requires a composite-field
...
while the BP algebra has $J_{(1)}J = c/3$ and
