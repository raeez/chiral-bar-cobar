...
centralizer $\mathfrak{g}^f$ has dimension~$2$, and the DS reduction
produces four strong generators:
\begin{center}
...
\begin{theorem}[Bershadsky--Polyakov Koszul conductor]
\label{thm:bp-koszul-conductor-explicit}
For the Bershadsky--Polyakov algebra $\mathcal{B}^k = \mathcal{W}_k(\mathfrak{sl}_3, f_{\mathrm{min}})$
...
\begin{proof}
Fateev--Kac--Roan central charge
$c(\mathcal{B}^k) = 2 - 24(k+1)^2/(k+3)$. Under the Koszul involution
$k \mapsto k' = -k - 6$,
\[
  c(\mathcal{B}^{-k-6})
  \;=\;
...
\begin{remark}[$\kappa_W$ depends on Bala--Carter pair]
\label{rem:kw-bp-vs-principal}
The Bershadsky--Polyakov conductor $\kappa_W(\mathcal{B}^k) = 196$
differs from the principal $\mathcal{W}(\mathfrak{sl}_3, f_{\mathrm{prin}})
= \mathcal{W}_3$ conductor $\kappa_W(\mathcal{W}_3) = 100$, reflecting
the Bala--Carter dependence $(\mathfrak{g}, f) \mapsto \kappa_W$. The
principal-case formula $\kappa_W = 2r + 12\sum_i m_i(m_i+1)$ of
the W-algebra chapter does not extend to non-principal nilpotents.
Both BP and $\mathcal{W}_3$ are Koszul self-dual on the non-critical
admissible lane; the self-duality levels are distinct.
\end{remark}
