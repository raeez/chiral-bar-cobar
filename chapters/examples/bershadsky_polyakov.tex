...
Wave-14 anchors. The conductor identity
$K_{\mathcal{B}} \equiv 196$
\textup{(}Theorem~\ref{thm:bp-koszul-conductor-polynomial};
polynomial identity in $\mathbb{Q}(k)$, NOT $K_{\mathcal{B}} = 2$,
which is /B25 forbidden\textup{)} is the largest known
W-algebra conductor in the standard landscape and is computed
in Chapter~\ref{chap:universal-conductor} via
$K = -c_{\mathrm{ghost}}(\BRST(\mathrm{BP}))$ in the universal
ghost-charge formula. The mixed-class profile
