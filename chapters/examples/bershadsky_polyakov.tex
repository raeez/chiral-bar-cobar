...
failure of anti-symmetry (AP24) in the non-principal landscape.
...
C (complementarity) &
  $\kappa(k) + \kappa(-k{-}6)
  = K_{\mathcal{B}}/2$, \; $K_{\mathcal{B}} = 196$
...
Complementarity & $\kappa + \kappa' = 98/3$ (W-type, AP24) \\
...
The bosonic OPE data (AP44: OPE-mode convention $a_{(n)}b$):
\begin{align}
T_{(3)}T &= \tfrac{c}{2}, &
T_{(1)}T &= 2T, &
T_{(0)}T &= \partial T,
...
(AP19: $d\log$ absorption reduces pole order by one).
...
should be consulted with care (AP38, AP49).
\end{remark}
...
The Koszul dual of $\mathcal{B}^k$ is
...
&= 4 + \frac{24\bigl((k+5)^2 - (k+1)^2\bigr)}{k+3}
 ...
 = 4 + 192 = 196\,.
\end{align*}
The self-transposition of $(2,1)$ and the
Feigin--Frenkel involution $k \mapsto -k - 2h^\vee = -k-6$
(where $h^\vee(\mathfrak{sl}_3) = 3$) on the DS quotient
give $(\mathcal{B}^k)^! \simeq \mathcal{B}^{-k-6}$.
\end{proof}
...
The modular characteristic of the Bershadsky--Polyakov algebra is
...
The complementarity sum for the Bershadsky--Polyakov Koszul pair is
...
$\kappa + \kappa' = \varrho \cdot K$ \textup{(}Theorem~D,
AP24\textup{)}: here $\varrho = 1/6$ and $K = 196$.
\end{proposition}
...
contribution from $J_{(0)}J$, which vanishes.  Higher arities
...
\begin{proof}
At arity~$2$: $\sigma^{(2)} = c/2 + (196-c)/2 = 98$.
At arity~$3$: $\sigma^{(3)} = 2 + 2 = 4$.
At arity~$4$: the quartic sigma-invariant is a nontrivial
...
For every odd $N = 2m+1$ and $\lambda = (m+1, 1^m)$, the
$\mathcal{W}$-algebra
