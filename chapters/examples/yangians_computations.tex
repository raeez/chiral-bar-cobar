...
\item \emph{Bar coassociativity implies QYBE.}
 ...
 $D\,\Theta + \frac{1}{2}[\Theta, \Theta] = 0$ at arity~$3$
 gives the full QYBE.
\end{enumerate}
\end{proposition}
...
absorption and drops. The arity-$2$ projection of the MC element
...
holds at all arities. The arity-$3$ projection of this equation is
exactly the Yang--Baxter equation for the arity-$2$ collision residue
...
convolution algebra, restricted to genus~$0$ and arity~$n$, gives
...
\item \emph{XXX chain \textup{(}rational, from $\hat{\mathfrak{sl}}_2$\textup{)}.}
 ...
 restricted to genus~$0$, arity~$L$, produces the KZ connection
...
the genus-$0$ arity-$n$ shadow amplitude
...
$\mathcal{O}_{Y_z^\hbar(\mathfrak{sl}_N)}$ is the full subcategory of
$Y_z^\hbar(\mathfrak{sl}_N)$-modules~$M$ satisfying:
\begin{enumerate}[label=\textup{(\roman*)}]
\item $M$ is finitely generated over~$Y_z^\hbar(\mathfrak{sl}_N)$;
...
$\mathcal{O}_{\mathrm{poly}} \subset \mathcal{O}_{Y_z^\hbar}$ is the
...
$\mathcal{O}_{\mathrm{poly}} = \mathcal{O}_{Y_z^\hbar}$. For
$N \ge 3$, it is an open question whether
$\mathcal{O}_{\mathrm{poly}}$ exhausts $\mathcal{O}_{Y_z^\hbar}$
...
of $Y(\mathfrak{sl}_N)$,
\[
\operatorname{Ext}^n_{Y_z^\hbar}(L(\lambda)(a), L(\mu)(b))
\;\cong\;
...
\subset D^b(\mathcal{O}_{Y_z^\hbar}),
\qquad
\mathcal{D}_{-\hbar}^{\mathrm{eval}}
...
\[
F\colon D^b(\mathcal{O}_{Y_z^\hbar}) \to D^b(\mathcal{O}_{Y_{-\hbar}}),
\qquad
G\colon D^b(\mathcal{O}_{Y_{-\hbar}}) \to D^b(\mathcal{O}_{Y_z^\hbar})
\]
...
\mathcal{D}_{\hbar}^{\mathrm{eval}}
\;\subset\;
D^b\bigl(\mathrm{Rep}_{\mathrm{fd}}(Y_z^\hbar(\mathfrak{sl}_N))\bigr)
\]
generated by evaluation modules equals
$D^b(\mathrm{Rep}_{\mathrm{fd}}(Y_z^\hbar(\mathfrak{sl}_N)))$.
\end{proposition}
...
\item \emph{Infinite-dimensional extension}: thick generation of
 $D^b(\mathcal{O}_{Y_z^\hbar})$ by evaluation modules, extending
...
finite-dimensional irreducible $Y_z^\hbar(\mathfrak{g})$-module is
...
$\mathcal{O}_{Y_z^\hbar}$ be a full subcategory of
$\mathcal{O}_{Y_z^\hbar(\mathfrak{sl}_N)}$ satisfying:
\begin{enumerate}[label=\textup{(\alph*)}]
\item $\mathcal{O}_{Y_z^\hbar}$ is Artinian
 \textup{(}every object has finite composition length\textup{)};
\item every simple object of $\mathcal{O}_{Y_z^\hbar}$ is
 finite-dimensional.
\end{enumerate}
...
\begin{enumerate}[label=\textup{(\roman*)}]
\item \emph{$N = 2$.}
 Category~$\mathcal{O}$ for $Y_z^\hbar(\mathfrak{sl}_2)$
...
\item \emph{Polynomial category~$\mathcal{O}$, any~$N$.}
 ...
 \mathcal{O}_{Y_z^\hbar(\mathfrak{sl}_N)}$ (modules
...
\item there is an exact ambient extension
 \[
 \widetilde{\Phi}\colon
 D^b(\operatorname{Rep}_{\mathrm{fd}}(Y_z^\hbar(\mathfrak{sl}_N)))
 \to
...
$D^b(\mathcal{O}_{Y_z^\hbar(\mathfrak{sl}_N)})$, the only additional
...
evaluation modules thickly generate
$D^b(\mathcal{O}_{Y_z^\hbar(\mathfrak{sl}_N)})$ and that the required
...
D^b(\mathcal{O}_{Y_z^\hbar(\mathfrak{sl}_N)})$
...
$\mathcal{O}_{Y_z^\hbar(\mathfrak{sl}_N)}$ is thickly generated
...
$\barB^{\mathrm{fact}}(Y_z^\hbar(\fg))$ admits:
\begin{enumerate}[label=\textup{(\alph*)}]
...
$\operatorname{gr}^F Y_z^\hbar(\fg) \cong
U(\fg[t])$, the universal enveloping algebra of the
current algebra.
\end{proposition}

\begin{proof}
(a)~The root lattice grading of $Y_z^\hbar(\fg)$ (the generators $J^{(r)}_a$ carry weight
...
theorem for $Y_z^\hbar(\fg)$ \cite{Drinfeld85,molev-yangians} gives
...
Thick generation of
$D^b(\mathcal{O}_{Y_z^\hbar(\mathfrak{sl}_N)})$ by evaluation
modules holds if every Verma module
$M(\lambda, a) \in \mathcal{O}_{Y_z^\hbar}$ admits a finite
resolution
...
\Phi\colon
D^b\bigl(\operatorname{Rep}_{\mathrm{fd}}(Y_z^\hbar(\mathfrak{sl}_N))\bigr)
\;\xrightarrow{\;\sim\;}\;
D^b\bigl(\operatorname{Rep}_{\mathrm{fd}}
...
\begin{proof}
\emph{$(\supseteq)$:}
Every finite-dimensional simple $Y_z^\hbar(\mathfrak{sl}_N)$-module
...
and is a Serre subcategory of $\mathcal{O}_{Y_z^\hbar}$ (closed under
...
$D^b(\mathcal{O}_{Y_z^\hbar})$. Since it contains all evaluation
...
\mathcal{D}_\hbar^{\mathrm{eval}}
\;=\; D^b(\operatorname{Rep}_{\mathrm{fd}})
\;\subsetneq\; D^b(\mathcal{O}_{Y_z^\hbar}).
\end{equation}
The left equality is the theorem; the strict inclusion follows from
Lemma~\ref{lem:fd-thick-closure} whenever $\mathcal{O}_{Y_z^\hbar}$
...
For the Yangian $Y_z^\hbar(\mathfrak{g})$, the analogous
...
of $U_q(\hat{\mathfrak{g}})$ and $Y_z^\hbar(\mathfrak{g})$.
...
The representation theory of $Y_z^\hbar(\fsl_N)$ admits a natural
...
The Yangian $Y_z^\hbar(\fsl_N)$ decomposes naturally into three
...
\begin{equation}
 \widehat{\bar{B}}_{\mathrm{wt}}\bigl(Y_z^\hbar(\fsl_N)\bigr)
 :=
...
\begin{enumerate}
 ...
 \begin{equation}
 \widehat\Omega\bigl(
 \widehat{\bar{B}}_{\mathrm{wt}}(Y_z^\hbar(\fsl_N))
 \bigr)
 \xrightarrow{\;\sim\;}
...
succeeds for $Y_z^\hbar(\fsl_N)$ despite the failure of unweighted
