...
\begin{proposition}[Automatic generalization of MC3 proof steps]
\label{prop:mc3-automatic-generalization}
\label{rem:mc3-proof-step-analysis}% backward compatibility
\ClaimStatusProvedHere
The proof of Theorem~\ref{thm:mc3-type-a-resolution} consists of four components:
\begin{enumerate}[label=\textup{(\roman*)}]
\item Prefundamental Clebsch--Gordan closure (type-specific).
\item Chromatic filtration (categorical, type-independent).
\item Pro-Weyl / Mittag-Leffler stabilization (weight-stratum, type-independent).
\item Efimov--Francis--Gaitsgory compact generation (categorical, type-independent).
\end{enumerate}
Components~\textup{(ii)}--\textup{(iv)} use only chromatic filtration,
weightwise stabilization, and compact generation, hence extend to
every simple Lie algebra once the relevant prefundamental subcategory
is available.  Component~\textup{(i)} is the sole type-specific input:
it is proved in type~$A$ by
Theorem~\ref{thm:mc3-type-a-resolution}, while beyond type~$A$ the
character-level shadow is known
\textup{(}Proposition~\ref{prop:character-cg-all-types}\textup{)}.
\end{proposition}

\begin{remark}[Remaining type-specific MC3 input]
Beyond type~$A$, the categorical prefundamental Clebsch--Gordan
decomposition remains open.  Proposition~\ref{prop:character-cg-all-types}
records the character-level shadow currently available in all types.
\end{remark}

\begin{proposition}[Type~$B$ from type~$A$ by folding]
\label{prop:mc3-type-b-folding}
\ClaimStatusConjectured
MC3 for type $B_n$ is expected to follow from MC3 for type $A_{2n-1}$ via the folding functor $\mathrm{Rep}(\hat{\mathfrak{sl}}_{2n}) \to \mathrm{Rep}(\hat{\mathfrak{so}}_{2n+1})$.  At the character level ($K_0$), the folding functor preserves CG closure (Proposition~\ref{prop:character-cg-all-types}).  The categorical lift requires verifying that the folding functor, which is exact and monoidal on finite-dimensional representations, extends to the shifted category $\mathcal{O}^{\mathrm{sh}}$ and preserves prefundamental CG decompositions on infinite-dimensional modules.  Langlands duality gives MC3 for type $C_n$ conditional on $B_n$.
\end{proposition}

\begin{remark}[Difficulty gradient across Dynkin types]
\label{rem:mc3-difficulty-gradient}
The following table records the difficulty of the categorical CG lift for each simple type, based on structural analysis of the prefundamental representation theory:
\begin{center}
\begin{tabular}{ccccc}
\toprule
Type & Minuscule count & Simply-laced & Status & Grade \\
\midrule
...
\end{tabular}
\end{center}
Types with zero minuscule representations ($G_2$, $F_4$, $E_8$) lack the evaluation-core bootstrap available in types~$A$--$D$--$E_6$.  The Coulomb branch approach (Braverman--Finkelberg--Nakajima) or the Hern\'andez--Leclerc cluster-category machinery may provide alternative routes for these types.
\end{remark}
