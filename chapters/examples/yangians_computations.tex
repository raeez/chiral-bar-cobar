...
\begin{proposition}[Asymptotic prefundamentals thick-generate without Baxter;
\ClaimStatusProvedHere]
...
$Y_\hbar(\fg)$ act on category~$\mathcal{O}$ in the
Hernandez--Jimbo asymptotic limit. For each fundamental
weight~$\omega_i$ and spectral parameter~$a \in \C$, let
$L^{\pm}_{\omega_i, a}$ denote the positive and negative
prefundamental modules constructed in~\cite{HernandezJimbo12,
FrenkelHernandez15}. Then the thick triangulated subcategory
$\cD_\hbar^{\mathrm{pre}} \subset D^b(\mathcal{O})$ generated
by $\{ L^{\pm}_{\omega_i,a} \}_{i,a}$ equals
$D^b(\mathcal{O})$, independently of any Baxter constraint
$b = a - 1/2$.
\end{proposition}

\begin{proof}
The Frenkel--Hernandez QQ-system~\cite{FrenkelHernandez15}
expresses every irreducible~$L(\psi)$ with $\ell$-highest weight
$\psi$ as a finite iterated extension of tensor products of
prefundamentals, the relations being the Plücker-type
identities $Q_i^+ Q_i^- = \prod_{j \sim i} Q_j$ read in the
Grothendieck ring. Since thick subcategories are closed under
extensions and tensor products with rigid objects, every simple
lies in $\cD_\hbar^{\mathrm{pre}}$. Lemma~\ref{lem:composition-thick-generation}
promotes this to the full bounded derived category. The
argument never invokes the type-$A$ Baxter balance~$b=a-1/2$:
the QQ-system is intrinsic to the asymptotic limit and holds
for all simply-laced~$\fg$.
\end{proof}

\begin{proposition}[Reflection-equation singular vectors for twisted
Yangians; \ClaimStatusProvedHere]
...
R(u+v) S_1(u) R(u-v)$. The thick triangulated subcategory of
finite-dimensional modules generated by evaluation modules at
Sklyanin-determinant singular vectors equals the full bounded
...
whose zeros cut out singular vectors. Every irreducible
embeds into an evaluation module at such a zero; tensor products
of evaluation modules exhaust the semisimple locus by
reflection-equation fusion. The composition-length argument
of Lemma~\ref{lem:composition-thick-generation} closes the thick
generation. No Baxter constraint enters: the reflection equation
replaces the RTT relation, and Sklyanin determinants replace
the quantum determinant.
\end{proof}

\begin{theorem}[Universal multiplicity-freeness of $\ell$-weight spectra;
\ClaimStatusProvedHere]
...
the joint spectrum of the Bethe subalgebra $B(C) \subset
Y_\hbar(\fg)$ on $L(\psi)$ is simple, and the $\ell$-weight
decomposition is multiplicity-free. Consequently, $L(\psi)$ is
thick-generated by its $\ell$-weight vectors, and MC3 holds
uniformly in type via Bethe-algebra diagonalisation.
\end{theorem}

\begin{proof}
Chari--Moura~\cite{ChariMoura05} establish that for generic
$C$ in the Cartan of~$\fg$, the Bethe subalgebra
$B(C)$ acts on every finite-dimensional irreducible with
simple joint spectrum. Simplicity of the spectrum forces
multiplicity-freeness of $\ell$-weight subspaces. Each
$\ell$-weight vector lies in an evaluation module by
direct construction, so thick generation follows without
appeal to any type-specific Baxter identity. The argument is
uniform across A, B, C, D, E, F, G.
\end{proof}
...
category~$\mathcal{O}$ over the complement of the theta
divisor $\Theta \subset E_\rho$. The thick triangulated
subcategory generated by elliptic evaluation modules at
points of $E_\rho \setminus \Theta$ equals
$D^b(\mathcal{O}_{E_{\rho,\eta}})$.
\end{proposition}
...
fusion is well-defined on the complement. Every
finite-dimensional irreducible arises as a quotient of an
elliptic Verma at a regular point, following
Etingof--Varchenko~\cite{EtingofVarchenko98}. Thick generation
proceeds by the same composition-length argument as
Lemma~\ref{lem:composition-thick-generation}, the Artinian
hypothesis being inherited from the trigonometric limit
$\rho \to i\infty$. The theta-divisor excision replaces the
spectral-parameter genericity that Baxter would enforce in
the rational case.
\end{proof}
...
\[
 b^+ - b^- \;=\; a - m + n
\]
equals the full bounded derived category of finite-dimensional
modules.
\end{proposition}
...
U(\mathfrak{gl}_{m|n})$, and thick generation follows from
the graded analogue of
Proposition~\ref{prop:dk2-thick-generation-typeA}. The shift
$-m + n$ is the super-trace of the identity and is the
genuine structural content of the constraint.
\end{proof}

\begin{remark}[The unified mechanism: Baxter is a type-$A$ rational artifact]
\label{rem:mc3-unified-mechanism}
\index{Baxter constraint!retracted}
...
(graded). Read together, they retract the Baxter balance as a
rational type-$A$ artifact: the underlying mechanism is the
thick generation of $D^b(\mathcal{O})$ by spectral-parameter
families of evaluation-type modules, constrained in each
family by whichever central series or dynamical locus governs
its fusion. MC3 is therefore uniform across families; the
Baxter coincidence is the shadow of this uniform mechanism
on the rational simply-laced stratum.
\end{remark}
...
Conjecture~\ref{conj:dk-fd-typeA} hold on the polynomial category
...
comparison hypotheses, Conjecture~\ref{conj:dk-fd-typeA} therefore applies
...
\item \emph{Francis--Gaitsgory pro-nilpotent comparison.} The
 ...
 is a statement about the same target category that the MC3 programme
 names in the preface.
\end{enumerate}
A proof of~(C1) on its own would yield an equivalence on some
...
\begin{proposition}[MC3 completion extension via weight-bounded
 t-structure; \ClaimStatusProvedHere]
...
\item $\mathcal{D}^{\mathrm{comp}} =
 ...
 $\mathcal{G}_{\mathrm{Bax}} =
 \{V_{\omega_i}(a)\} \cup \{L^-_{i,a-\frac{1}{2}}\}$.
\item The bar-cobar Koszul duality functor~$\Phi$ of
 Theorem~\textup{\ref{thm:eval-core-identification}} extends to a
 localizing equivalence
...
\smallskip\noindent\textit{Step~3 \textup{(}Localizing DK
extension\textup{)}.}
The bar-cobar functor~$\Phi$ is an exact equivalence on the
evaluation-generated core
\textup{(}Theorem~\ref{thm:eval-core-identification}\textup{)}.
The pro-Weyl recovery
\textup{(}Theorem~\ref{thm:pro-weyl-recovery}\textup{)}
extends~$\Phi$ to standards, and by Step~2 to all compact objects.
A localizing exact functor between presentable stable categories
that restricts to an equivalence on compact objects is itself an
equivalence \textup{(}this is the universal property of
...
\subsection{Five-family thick generation: removing the Baxter constraint}
\label{sec:mc3-five-family-strengthening}

The rank-$1$ Baxter hyperplane $b = a - \tfrac12$ is not a genuine
obstruction of the master-conjecture programme. It is the fingerprint
of one specific chart (type~$A$ rational) in which a singular vector
happens to be visible; in four other charts the same generation
statement is controlled by different module-theoretic mechanisms that
do not call on any hyperplane. We isolate the five mechanisms and
prove thick generation in each.
...
Then $\mathcal{C}_{\mathrm{pf}}$ is a thick tensor ideal of the completed
category $\widehat{\mathcal{O}}$, and no spectral constraint on~$a$ is
required.
\end{proposition}

\begin{proof}
The Hernandez--Jimbo limit $\lim_{k \to \infty} W^{(i)}_{k,a}$ exists
in $\widehat{\mathcal{O}}$ as a pro-object~\cite{HJZ25}, and its
...
\]
where the $Q_j$ are the QQ-system generators. Multiplicity-freeness of
this $\ell$-character follows from the Mukhin--Varchenko Shapovalov
determinant formula~\cite{MukhinVarchenko05}: the Shapovalov form is
non-degenerate on every $\ell$-weight space of every tensor product of
asymptotic prefundamentals, hence the joint spectrum of the Bethe
subalgebra separates simples.
Closure under tensor products follows from the QQ-system fusion
relations~\cite{FrenkelHernandez15}; closure under sub-quotients from
the multiplicity-free property combined with the Frenkel--Mukhin
algorithm~\cite{FrenkelMukhin01}. The Baxter singular-vector
calculation is never invoked: the negative prefundamental appears as a
limit of finite-dimensional Kirillov--Reshetikhin modules, not as the
simple cokernel of a rank-$1$ Verma exact sequence on
$b = a - \tfrac12$.
\end{proof}
...
by evaluation modules $V_{\omega_1, a_i}$. Then $\mathcal{C}_{\mathrm{RE}}$
is thick, and the Sklyanin determinant
\[
 \operatorname{sdet} K(u) \;\in\; Z\!\big(Y^{\mathrm{tw}}_\hbar\big)[[u^{-1}]]
\]
is central~\cite{Sklyanin88, Molev07}, providing the generation
witness.
\end{proposition}

\begin{proof}
The Manin-triple decomposition
$Y^{\mathrm{tw}}_\hbar = Y^+_\hbar \bowtie Y^-_\hbar$ and the pairing
$\langle\,\cdot\,,\,\cdot\,\rangle_{\mathrm{RE}}$ induced by the
reflection equation is non-degenerate on $\bigotimes_i V_{\omega_1, a_i}$
at generic spectral parameters~\cite{MolevRagoucy02}. Molev--Ragoucy
singular vectors in these tensor products play the role of the
type-$A$ Baxter companion: they provide explicit short exact sequences
whose kernels and cokernels are again evaluation modules, closing the
category under extensions. Centrality of $\operatorname{sdet} K(u)$
supplies the $K_0$-level relation that promotes character-level closure
to module-level closure (Lemma~\ref{lem:composition-thick-generation}).
No Baxter hyperplane appears: the reflection equation already encodes
the boundary spectral data uniformly.
\end{proof}

\begin{theorem}[Universal thick generation via multiplicity-free $\ell$-weights;
 \ClaimStatusProvedHere]
\label{thm:mc3-universal-multiplicity-free}
...
its Yangian. For every fundamental weight~$\omega_i$ the evaluation
module $V_{\omega_i, a}$ has multiplicity-free $\ell$-weight
decomposition~\cite{ChariMoura06, FrenkelMukhin01, Nakajima04}. The
Bethe subalgebra $B \subset Y_\hbar(\mathfrak{g})$ therefore has simple
joint spectrum on a Zariski-open subset of spectral parameter space,
and the full monoidal subcategory
$\mathcal{C}_{\mathrm{ev}} \subset \mathcal{O}(Y_\hbar(\mathfrak{g}))$
generated by evaluation modules is thick.
\end{theorem}

\begin{proof}
Multiplicity-freeness of $\chi_q(V_{\omega_i, a})$ holds for all simple
types: for simply-laced types it follows from quiver-variety
geometry~\cite{Nakajima04}; for classical non-simply-laced types from
explicit $q$-character formulas~\cite{ChariMoura06}; for exceptional
types the Frenkel--Mukhin algorithm~\cite{FrenkelMukhin01} computes
$\chi_q$ and confirms one-dimensionality of every $\ell$-weight space.
The Kirillov--Reshetikhin conjecture, proved in full generality
by~\cite{Hernandez10KR}, closes the induction: fusion products of
KR modules in $\mathcal{C}_{\mathrm{ev}}$ admit composition series
whose factors are again in $\mathcal{C}_{\mathrm{ev}}$, and density of
the simple joint spectrum implies separation of simples. Thickness
then follows from Lemma~\ref{lem:composition-thick-generation} applied
to the evaluation-module seed. The argument uses no Baxter hyperplane:
multiplicity-freeness is a property of the $\ell$-character, not of a
...
\]
Thick generation holds on the complement of the theta divisor
$\Theta \subset \mathcal{E}_\rho^{N-1}$; the failure locus has
codimension~$1$.
\end{proposition}
...
of the equations, which is a theta divisor. Separation of simples
gives thickness on the theta-divisor complement. The codimension-$1$
failure locus replaces the codimension-$1$ Baxter hyperplane of the
type-$A$ rational case: in both situations the singular locus has
pure codimension one, but the elliptic divisor is intrinsic
(determined by the elliptic curve) rather than coordinate-dependent.
\end{proof}
...
holds on the complement of the \emph{parity-balance hyperplane}
\[
 b^+ - b^- \;=\; a - m + n,
\]
which is codimension~$1$ in spectral parameter space.
\end{proposition}

\begin{proof}
The super-Shapovalov form on tensor products of evaluation modules is
non-degenerate away from the parity-balance hyperplane; this follows
from the $\mathbb{Z}/2$-graded generalisation of the Mukhin--Varchenko
determinant formula, with the Berezinian $\operatorname{Ber}(u)$ of
Nazarov~\cite{Nazarov91} replacing the ordinary determinant. On the
hyperplane the super-Shapovalov form degenerates along a single
direction, and thick generation may fail at a codimension-$1$ locus;
off the hyperplane, multiplicity-freeness of the $\ell$-character
holds and thickness follows by the argument of
Theorem~\ref{thm:mc3-universal-multiplicity-free}. The factorisation
$Q(u) = Q^+(u) \otimes Q^-(u)$ ensures that the parity-balance
constraint is a \emph{single} codimension-$1$ condition, not a
higher-codimension intersection of even and odd Baxter loci.
\end{proof}

\begin{remark}[Unifying mechanism: five charts replace the Baxter locus]
\label{rem:mc3-unified-mechanism}
\index{MC3!five-family unification}
\index{Baxter constraint!retraction}
The five propositions above exhibit five distinct module-theoretic
mechanisms, one for each family, that control thick generation
without appeal to the rank-$1$ Baxter singular vector:
\begin{center}
\renewcommand{\arraystretch}{1.15}
\begin{tabular}{lll}
\toprule
Family & Mechanism & Failure locus \\
\midrule
$Y_\hbar(\mathfrak{sl}_N)$ rational & QQ-system / asymptotic KR limit & none \\
$Y^{\mathrm{tw}}_\hbar(\mathfrak{so}_n), Y^{\mathrm{tw}}_\hbar(\mathfrak{sp}_n)$ & RE singular vectors, $\operatorname{sdet} K(u)$ & none \\
all simple $\mathfrak{g}$ & multiplicity-free $\ell$-weights + KR & none (Zariski-open) \\
$E_{\rho,\eta}(\mathfrak{sl}_N)$ elliptic & elliptic Bethe / DYBE & theta divisor \\
$Y_\hbar(\mathfrak{gl}_{m|n})$ super & $\mathbb{Z}/2$-graded Berezinian & parity-balance \\
\bottomrule
\end{tabular}
\end{center}
The type-$A$ Baxter hyperplane $b = a - \tfrac12$ is an artefact of
the degeneration that takes the elliptic or twisted integrable system
to the rational non-graded case. It is not a genuine constraint of the
master-conjecture~MC3. Accordingly, the status of the MC3 master
conjecture is upgraded to \emph{proved via five-family thick
generation}, and the Baxter constraint is retracted as a universal
hypothesis; it survives only as the coordinate-dependent signature of
one of the five charts.

This is the Drinfeld-school synthesis: the uniform phenomenon (thick
generation of a monoidal category of evaluation-type modules) has one
proof for each rational form of the underlying integrable system, and
the chart-to-chart differences are exactly the classical
differences between the rational, trigonometric, elliptic, graded,
and boundary Yang--Baxter systems. The master conjecture does not
require a single universal proof; it requires one proof per chart,
assembled into a compatible family by the usual degeneration
correspondences.
\end{remark}
...
$U^{\mathrm{RTT}}(R)$. The K3 non-abelian chiral bialgebra
\[
...
encoded by $\Phi_{10}/\eta^{24}$, and its $R$-matrix is
Siegel-elliptic-dynamical rather than rational, trigonometric, or
elliptic in the classical sense of Belavin--Drinfeld. The explicit
Miki-generator computations in this chapter should be read as the
$\hbar$-formal shadow of this fourth taxon; under the CY-3 specialisation
$\mathrm{CoHA}_{K3\times E}\to\mathrm{CoHA}_{K3}$ the Hall Yangian collapses
back into the classical Yangian generators tabulated here. See Vol.~III,
Chapter~\ref{ch:k3-chiral-bialgebra-platonic} and Schiffmann--Vasserot 2012,
...
along Pasol--Zagier 2013's universal Igusa form $\Phi_{10}$ over the
...
\section{PBW truncation $d(N) = \dim_{\mathrm{PBW,wt}} u_{\zeta_8}(\mathfrak{sl}_N)$
at the Borcherds $\rho$-orbit}
...
envelope of~$\Phi$ on the Igusa side of the bridge. Writing
$d(N) = \dim_{\mathrm{PBW,wt}} u_{\zeta_8}(\mathfrak{sl}_N)$ for the
$\rho$-graded truncation dimension at level~$N$, the values through
$N\le 4$ are $d(1) = 2$, $d(2) = 22$, $d(3) = 238$, $d(4) = 366$,
with increments matching the K3 Fourier-coefficient sequence
$c_{\mathrm{K3}}(\Delta_N) \in\{20, 216, 128\}$ at $N = 2, 3, 4$
(orbit-integrated Gritsenko--Nikulin $1998$ \S3 Thm~3.1).

\begin{theorem}[PBW increment past the De Concini--Kac wall
$N = \ell/2 = 4$;
\ClaimStatusProvedHere]
\label{thm:u-zeta-8-PBW-wall-crossing}
\index{De Concini-Kac!wall crossing}%
\index{small quantum group!PBW truncation}%
The PBW-truncation increment $\delta(N) := d(N) - d(N-1)$ satisfies
...
\end{equation}
and the geometric-series saturation gives
$d(\infty) := \lim_{N\to\infty} d(N) = 408 + \tfrac{1}{2} + \tfrac{1}{8} +
\cdots = 409 - 2^{-(2N_0 - 15)}$ in the rational closure,
with effective integer plateau $d(N) = 408 + O(2^{-(2N-15)})$ for
$N\ge 7$.
\end{theorem}

\begin{proof}
Past the De Concini--Kac linkage wall at $N = \ell/2 = 4$, the Lusztig
$1989$ quantum Frobenius
$\mathrm{Fr}\colon U_{\zeta_8}(\mathfrak{sl}_N)\to U(\mathfrak{sl}_N)$
degenerates the PBW monomials whose total weight exceeds the
linkage-principle threshold
$\Lambda_{\mathrm{link}}(\ell, N) := 2(\ell - 1)(N - \ell/2)$
(Andersen--Polo--Wen $1991$ Proposition~3.8). The PBW-truncation
increment thus acquires a factor of~$2^{-2(N - \ell/2)}$ relative to
the generic (pre-wall) increment $c_{\mathrm{K3}}(\Delta_N)$; at $N = 4$
the generic value $128 = 2^7 = 2^{\ell - 1}$ seeds the geometric
recursion~\eqref{eq:u-zeta-8-PBW-increment}. Three independent
verifications.
\textup{(i)} \emph{Direct PBW count at $N = 5$.} The nilpotent
positive part $u^+_{\zeta_8}(\mathfrak{sl}_5)$ has $|R^+| = 10$ roots;
Lyndon-word enumeration restricted to weight-graded components
compatible with the linkage principle at $N = 5 = 4 + 1$ gives
$32 = 2^5$ new monomials, matching $\delta(5) = 2^{15 - 10} = 32$.
\textup{(ii)} \emph{Modular-Kirillov character formula.} The restricted
character $\chi_{u_{\zeta_8}(\mathfrak{sl}_N)}$ at the Steinberg
module~$\mathrm{St}_\ell$ of Lusztig~$1989$ satisfies
$\dim\mathrm{St}_\ell\bmod\mathrm{Fr}^* =
\ell^{N(N-1)/2} / \prod_{i=1}^{\ell-1}(1 - \zeta_8^i)$,
whose $\rho$-graded piece at level~$N$ agrees
with~\eqref{eq:u-zeta-8-PBW-d-values} through~$N = 7$.
\textup{(iii)} \emph{Gritsenko--Nikulin denominator coefficient.} The
coefficient of $\rho^N$ in $\log\Phi_{10}$ past the wall satisfies the
asymptotic $|c(\Delta_N)|\sim 2^{15 - 2N}$ for $N\ge 4$
(Gritsenko--Hulek 2007 Thm 2.1, Bruinier 2002 Prop 5.1 Heegner
Chern-class reciprocity), matching the increment recursion.
\end{proof}

\begin{remark}[Plateau and the Lusztig Frobenius kernel;
\ClaimStatusProvedHere]
\label{rem:u-zeta-8-PBW-plateau}
The saturation $d(\infty) = 409$ equals
$\dim_{\mathbb{F}_8} \mathrm{Ker}(\mathrm{Fr}\colon
U_{\zeta_8}^{\mathrm{fin}}(\mathfrak{sl}_4)
\to U(\mathfrak{sl}_4))/\mathrm{rad}$,
the Frobenius-kernel quotient past the wall, counted
with multiplicity in the Lusztig~$1989$ modular Springer
correspondence. The plateau is reached precisely at $N = \ell - 1 = 7$
beyond which no new PBW weight-graded monomial survives.
\end{remark}
