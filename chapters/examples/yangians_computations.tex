...
space gives
\[
r(z) \;=\; \frac{\Omega}{(k+3)\,z},
\]
which reproduces the Yang $R$-matrix in the rank-$2$ case.  The
...
The classical $r$-matrix $r(z) = \Omega/z$ is the leading-order
...
\item \emph{Classical $r$-matrix.}
  ...
  \end{equation}
  where $\Omega_\fg = \sum_a I^a \otimes I_a$ is the quadratic
  Casimir tensor \textup{(}$I^a$ and $I_a$ are dual bases
  under the trace form\textup{)}.
...
affine OPE (AP19).  The double pole $k\,g^{ab}/(z{-}w)^2$
contributes to arity~$1$ (curvature); the simple pole
...
\begin{theorem}[Finite-dimensional extension criterion from evaluation generators,
type~\texorpdfstring{$A$}{A}; \ClaimStatusConditional]
...
Theorem~\ref{thm:dk-fd-typeA}, applied on the chosen
...
Theorem~\ref{thm:dk-fd-typeA} hold on the polynomial category
...
comparison hypotheses, Theorem~\ref{thm:dk-fd-typeA} therefore applies
...
Theorem~\ref{thm:dk-fd-typeA}; a separate H-level sectorwise route
...
\item \emph{Type-$A$ resolution.}
  ...
  Proposition~\ref{prop:mc3-automatic-generalization} are
...
\begin{corollary}[All-types MC3 reduction after categorical CG closure;
  \ClaimStatusProvedHere]
\label{cor:mc3-all-types}%
\index{MC3!all simple types|textbf}%
For every simple Lie algebra~$\mathfrak{g}$, the categorical
Clebsch--Gordan part of MC3 is proved
...
shifted-prefundamental generation, pro-Weyl recovery, and the compact
/ completed DK extension.
\end{corollary}

\begin{proof}
Theorem~\ref{thm:categorical-cg-all-types} gives package~(i), and
Corollary~\ref{cor:dk23-all-types} gives the all-types DK comparison
on the evaluation-generated core.  What remains after these two
results is exactly the post-CG extension/completion packet described
in the statement.
\end{proof}
...
\begin{proposition}[Type-independence of the remaining MC3 completion mechanisms;
