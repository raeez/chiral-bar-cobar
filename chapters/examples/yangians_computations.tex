...
DK-0/1: proved for all simple~$\fg$.  DK-2/3: proved on the evaluation-generated core at all types, via thick generation (Corollary~\ref{cor:dk-poly-catO}, Corollary~\ref{cor:mc3-all-types}) and sectorwise convergence (Corollary~\ref{cor:dk23-all-types}); extension beyond $\mathcal{O}_{\mathrm{poly}}$ requires completed/coderived techniques.  DK-4: structural framework proved (MC4 closed via $\mathcal{W}_N$ rigidity, Theorem~\ref{thm:completed-bar-cobar-strong}); algebraic identification open (Conjecture~\ref{conj:dk4-formal-moduli}).  DK-5: now accessible for all simple types (downstream of MC3).  The extension from the evaluation locus to all finite-dimensional representations follows by thick-closure arguments (Proposition~\ref{prop:dk2-thick-generation-typeA}, Lemma~\ref{lem:monoidal-thick-extension}).
(Contributing to Conjecture~\ref{conj:master-dk-kl}.)
\end{remark}
...
(Now proved; see Theorem~\ref{thm:mc3-type-a-resolution} and
...
at a cofinal family.
This extension to arbitrary simple $\mathfrak{g}$ is now complete:
the multiplicity-free $\ell$-weight property (Chari--Moura) replaces
the minuscule hypothesis, yielding the categorical CG closure for
all simple types
...
generation).
This extension to arbitrary type is now complete: the four-package
resolution is proved for all simple types
(Theorem~\ref{thm:categorical-cg-all-types},
Corollary~\ref{cor:mc3-all-types}).
\end{remark}
...
The four-package MC3 extension programme
\textup{(}prefundamental CG closure, shifted-prefundamental
generation, pro-Weyl recovery, DK on compacts\textup{)} is
resolved for all simple Lie algebras~$\mathfrak{g}$.
\end{corollary}

\begin{proof}
Package~(i) is
Theorem~\ref{thm:categorical-cg-all-types}.
Packages~(ii)--(iv) follow by the same argument as in
type~$A$ (Theorem~\ref{thm:mc3-type-a-resolution}):
the DK equivalence on the evaluation-generated core is
available for all types
(Corollary~\ref{cor:dk23-all-types}), thick generation
of $\mathrm{Rep}_{\mathrm{fd}}$ by evaluation modules
holds for all types
(Corollary~\ref{cor:dk2-thick-generation-all-types}),
and the extension mechanisms
\textup{(}chromatic filtration, Mittag-Leffler,
Francis--Gaitsgory completion\textup{)} are categorical
...
The extension from the evaluation-generated subcategory to the full
category in step~(iv) uses Francis--Gaitsgory pro-nilpotent
completion: evaluation modules are compact objects
(finite composition series in $\mathcal{O}^{\mathrm{sh}}$), so an
equivalence on the compact-generated thick subcategory extends to the
ind-completion.  This is a purely categorical step and does not
depend on Lie type.
\end{remark}
...
\begin{proposition}[All MC3 components are type-independent;
  \ClaimStatusProvedHere]
