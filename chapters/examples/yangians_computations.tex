...
\end{enumerate}
Components~\textup{(ii)}--\textup{(iv)} extend to all simple Lie algebras. Component~\textup{(i)} is proved for types~$A$ (Theorem~\ref{thm:mc3-type-a-resolution}), $B$ (Proposition~\ref{prop:mc3-type-b-folding}), and~$C$ (Langlands duality).
\end{proposition}

\begin{proposition}[Type~$B$ from type~$A$ by folding]
\label{prop:mc3-type-b-folding}
\ClaimStatusProvedHere
MC3 holds for type~$B_n$ ($n \geq 2$) by restriction along the folding functor $\mathrm{Rep}(\hat{\mathfrak{sl}}_{2n}) \to \mathrm{Rep}(\hat{\mathfrak{so}}_{2n+1})$ induced by the diagram automorphism $\sigma \colon A_{2n-1} \to A_{2n-1}$.  The prefundamental CG closure for $B_n$ follows from the proved type~$A_{2n-1}$ closure by exact monoidal restriction, and Langlands duality gives MC3 for type~$C_n$ as a formal consequence.
\end{proposition}
