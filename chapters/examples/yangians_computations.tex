...
\begin{remark}[Proof-step analysis for arbitrary type]
\label{rem:mc3-proof-step-analysis}
The type~$A$ proof (Theorem~\ref{thm:mc3-type-a-resolution}) decomposes into four steps:
\begin{enumerate}[label=\textup{(\roman*)}]
\item \emph{Prefundamental Clebsch--Gordan closure}: character-level decomposition $V_n \otimes L^- = \bigoplus_{j=0}^n L^-(n-2j)$, upgraded to categorical splitting in $\mathcal{O}^{\mathrm{sh}}$.
\item \emph{Chromatic filtration}: purely categorical, rank-independent.
\item \emph{Pro-Weyl / Mittag-Leffler}: weight-stratum argument, rank-independent.
\item \emph{Efimov + Francis--Gaitsgory}: compact generation in ind-coherent sheaves, purely categorical.
\end{enumerate}
Steps~(ii)--(iv) generalize automatically to all simple Lie algebras.  Step~(i) is the sole remaining obstruction: the categorical CG lift from $K_0$-level generation (proved for all types) to module-level thick generation.
\end{remark}
