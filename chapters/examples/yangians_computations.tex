...
(Proposition~\ref{prop:prefundamental-clebsch-gordan}).
Extension to arbitrary simple~$\mathfrak{g}$ requires establishing
the prefundamental Clebsch--Gordan decomposition for the
Hern\'andez--Jimbo modules $L^-_{i,a}(\mathfrak{g})$.
\end{remark}
...
\begin{proposition}[Prefundamental Clebsch--Gordan; \ClaimStatusProvedHere]
\label{prop:prefundamental-clebsch-gordan}%
\index{prefundamental!Clebsch--Gordan}%
\index{Clebsch--Gordan!prefundamental}%
For $Y(\mathfrak{sl}_2)$, the negative prefundamental module
satisfies the character-level identity
\begin{equation}\label{eq:prefundamental-cg}
  \operatorname{ch}(V_n \otimes L^-)
  \;=\;
  \sum_{j=0}^{n} \operatorname{ch}\bigl(L^-(\mathrm{hw} = n - 2j)\bigr),
  \qquad \forall\, n \geq 0.
\end{equation}
\end{proposition}

\begin{proof}
Distributivity of formal power series convolution:
$\operatorname{ch}(V_n) = \sum_{j=0}^{n} q^{n-2j}$ and
$\operatorname{ch}(L^-) = \sum_{k \geq 0} p(k)\, q^{-2k}$, so
$\operatorname{ch}(V_n) \cdot \operatorname{ch}(L^-)
= \sum_{j} q^{n-2j} \cdot \sum_{k} p(k)\, q^{-2k}
= \sum_{j} \operatorname{ch}(L^-(\mathrm{hw} = n{-}2j))$.
Verified: $n = 1, \ldots, 20$ at depth~$60$
...
This upgrades the previously verified range $\lambda \leq 6$ to
all~$\lambda$: the $j = 0$ summand of
equation~\eqref{eq:prefundamental-cg} has multiplicity
$p(k) \geq 1$ at weight $\lambda - 2k$.
\end{corollary}

\begin{corollary}[$K_0$ generation of $\mathcal{O}_Y$; \ClaimStatusProvedHere]
...
Every Verma class $[M(\lambda)]$ in $K_0(\mathcal{O}_Y)$ lies in
the $\mathbb{Z}$-span of
$\{[V_n] \cdot [L^-] \mid n \geq 0\}$.
\end{corollary}

\begin{proof}
By Proposition~\ref{prop:prefundamental-clebsch-gordan},
$[V_n] \cdot [L^-] = \sum_{j} [L^-(\text{shifted}_j)]$.
The Fock--Verma factorization~\eqref{eq:fock-verma-factorization}
gives $[L^-] = [M(0)] + [K_1]$.  Baxter TQ
($[V_1] \cdot [M(\lambda)] = [M(\lambda{+}1)] + [M(\lambda{-}1)]$)
then generates all $[M(\lambda)]$ by induction.
Verified: $\lambda = 0, \ldots, 30$ ($31$~tests).
\end{proof}
...
simple Lie algebras~$\mathfrak{g}$ once the following structural input
is established: for each node $i \in I$ of the Dynkin diagram
of~$\mathfrak{g}$, the Hern\'andez--Jimbo prefundamental modules
$L^-_{i,a}(\mathfrak{g})$ satisfy a Clebsch--Gordan closure
\begin{equation}\label{eq:prefundamental-cg-general-type}
  V_{\omega_i}(a) \otimes L^-_{i}(b)
  \;\cong\;
  \bigoplus_{j} L^-_{i}(b_j)
\end{equation}
at the level of $Y(\mathfrak{g})$-modules
\textup{(}generalizing
Proposition~\textup{\ref{prop:prefundamental-clebsch-gordan}}
from type~$A$\textup{)}.
The remaining steps---Mittag--Leffler pro-Weyl recovery, spectral
...
(Theorem~\ref{thm:lines_as_modules} of Vol~II): the category of
line operators in the 3d gauge theory is equivalent to the module
category of the shifted cotangent loop algebra.
In the Chriss--Ginzburg framework, $T^*[-1]\fg[\lambda]^*$ is the
