...
\begin{remark}[$\mathcal{O}_{\mathrm{poly}}$ is not dense in
$\mathcal{O}^{\mathrm{sh}}$]
\label{rem:opoly-not-dense}
\index{prefundamental module!density obstruction}
The negative prefundamental $L^-(b)$ has weight multiplicities
$\dim(L^-)_{-2k} = p(k)$, unbounded as $k \to \infty$.
Every module in $\mathcal{O}_{\mathrm{poly}}$ has composition
factors with bounded weight multiplicities.  Therefore
...
\emph{not} extend to $L^-$ by density.  A separate argument
is needed: either the $\lambda = 0$ unconditional trick
(which avoids the spectral constraint entirely) or an
explicit pro-Weyl approximation of~$L^-$ by objects in
$\mathcal{O}_{\mathrm{poly}}$.  The resolution obstruction
$\delta(k) = p(k) - 1$ is superpolynomial but sub-exponential
(Hardy--Ramanujan), so any finite-length resolution of~$L^-$
by evaluation modules has unbounded length.
\end{remark}
