...
\begin{theorem}[$E_1$-chiral thick generation for $Y(\mathfrak{sl}_2)$;
\ClaimStatusProvedHere]
...
In the separated pro-completed shifted category
$\widehat{D}(\mathcal{O}^{\mathrm{sh}}_{\leq 0}(Y(\mathfrak{sl}_2)))$,
the compact objects are the thick idempotent-complete closure of
\[
\mathcal{G}_{\mathrm{shift}}
\;:=\;
\{V_n(a) \mid n \in \mathbb{Z}_{\geq 0},\; a \in \mathbb{C}\}
\;\cup\;
\{L^-_a \mid a \in \mathbb{C}\},
\]
where $L^-_a$ are the negative prefundamental modules of
Hernandez--Jimbo--Zhang~\cite{HJZ25}.
\end{theorem}
