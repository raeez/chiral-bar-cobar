...
Theorem~\ref{thm:dk-fd-typeA} extends verbatim to any thick
subcategory of $\operatorname{Fact}_{\Eone}$ generated by evaluation
modules.  In particular, the evaluation-generated core of
$D^b(\mathcal{O}_{Y_\hbar(\mathfrak{sl}_N)})$ carries the same
braided monoidal equivalence, and the only remaining obstacle to
extending this equivalence to the full category~$\mathcal{O}$ is
whether evaluation modules thickly generate it.
\end{corollary}
...
DK-0/1: proved for all simple~$\fg$.  DK-2/3: proved on the evaluation-generated core at all types, via thick generation (Corollary~\ref{cor:dk-poly-catO}, Corollary~\ref{cor:mc3-all-types}) and sectorwise convergence (Corollary~\ref{cor:dk23-all-types}); extension beyond $\mathcal{O}_{\mathrm{poly}}$ requires completed/coderived techniques.  DK-4: resolved (MC4 closed via $\mathcal{W}_N$ rigidity within the strong completion-tower framework, Theorem~\ref{thm:completed-bar-cobar-strong}).  DK-5: now accessible for all simple types (downstream of MC3).  The extension from the evaluation locus to all finite-dimensional representations follows by thick-closure arguments (Proposition~\ref{prop:dk2-thick-generation-typeA}, Lemma~\ref{lem:monoidal-thick-extension}).
(Contributing to Conjecture~\ref{conj:master-dk-kl}.)
\end{remark}
...
Conjecture~\ref{conj:baxter-exact-triangles} is proved at generic
...
With MC3 now resolved for all simple types (Corollary~\ref{cor:mc3-all-types}), the remaining open problem splits into two layers.
...
\label{conj:mc3-arbitrary-type}% backward compat
...
on quantum affine algebras $U_q(\hat{\mathfrak{g}})$,
transferred to Yangians via the Gautam--Toledano Laredo
functor~\cite{GTL17}; we indicate the transfer at each step.
...
$q$-character of every fundamental evaluation module consists of
$\dim(V_{\omega_i})$ distinct monomials: this is proved for
simply-laced types by the Frenkel--Mukhin
algorithm~\cite{FrenkelMukhin01} (where the $q$-character is
computed recursively, and each step produces distinct monomials
for fundamental representations), and extended to non-simply-laced
types by Hern\'andez~\cite{Hernandez06KR} via the folding
functor from the simply-laced ambient type.  Each monomial
corresponds to a one-dimensional $\ell$-weight space: for
simply-laced types this is immediate from the quiver variety
realization \cite{Nakajima04}, which produces isolated
torus-fixed points; for non-simply-laced types it follows
because the folding functor preserves
$\ell$-weight multiplicities.

For the Yangian $Y_\hbar(\mathfrak{g})$, the analogous
multiplicity-free $\ell$-weight property follows from the
Gautam--Toledano Laredo monoidal equivalence~\cite{GTL17}
between the categories of finite-dimensional representations
of $U_q(\hat{\mathfrak{g}})$ and $Y_\hbar(\mathfrak{g})$,
which is an equivalence of abelian monoidal categories
preserving $\ell$-weight spaces.
...
$\ell$-weight of~$L^-_i(b)$.  By the block criterion for
category~$\mathcal{O}^{\mathrm{sh}}$ of the quantum loop
algebra \cite{Hernandez05, HJZ25} and its Yangian
analogue (via the categorical equivalence of
\cite{GTL17} extended to category~$\mathcal{O}$ by
\cite{GTL-catO}), at generic spectral parameters
\textup{(}avoiding the discrete zero locus of the normalized
$R$-matrix denominator\textup{)}, distinct highest
$\ell$-weights lie in distinct blocks.

\emph{Step~4 (Singleton blocks force simplicity).}
The tensor product $V_{\omega_i}(a) \otimes L^-_i(b)$ lies in
$\mathcal{O}^{\mathrm{sh}}$ (since~$V$ is finite-dimensional
and~$L^-_i$ is in~$\mathcal{O}^{\mathrm{sh}}$, and tensoring
with a finite-dimensional module preserves the category).  The
block decomposition in~$\mathcal{O}^{\mathrm{sh}}$ gives a direct
sum $V \otimes L^- = \bigoplus_B (V \otimes L^-)_B$.  At generic
spectral parameters, Step~3 ensures that the proposed summands
lie in \emph{distinct singleton blocks}: each block~$B(m_j)$
contains $L^-_i(\mathrm{shift} = m_j)$ as its unique simple
object, since no $A_{k,c}^{-1}$-chain connects its
$\ell$-highest weight to any other.  Therefore every
Jordan--H\"older factor of the block
component~$(V \otimes L^-)_{B(m_j)}$ is isomorphic
to~$L^-_i(\mathrm{shift} = m_j)$, and the character identity
(Step~1) forces the Jordan--H\"older multiplicity to be
exactly~$1$, giving
...
external pillars: \textup{(a)}~the multiplicity-free
$q$-character property for fundamental representations
\cite{FrenkelMukhin01, Nakajima04, Hernandez06KR},
\textup{(b)}~the Hern\'andez block
criterion~\cite{Hernandez05, HJZ25}, and \textup{(c)}~the
Gautam--Toledano Laredo
equivalence~\cite{GTL17}.  All three are published
results.  The new content of the theorem is the observation
...
the minuscule hypothesis that was used in type~$A$
(Proposition~\ref{prop:categorical-cg-typeA}).
\end{remark}

\begin{corollary}[MC3 package~\textup{(i)} for all simple types;
  \ClaimStatusProvedElsewhere]
\label{cor:mc3-all-types}%
\index{MC3!package (i) all types|textbf}%
Package~\textup{(i)} of the MC3 extension programme
\textup{(}prefundamental Clebsch--Gordan closure\textup{)} is
resolved for all simple Lie algebras~$\mathfrak{g}$ by
Theorem~\textup{\ref{thm:categorical-cg-all-types}}.
The full four-package MC3 programme \textup{(}packages
\textup{(i)--(iv)}\textup{)} is proved for type~$A$
\textup{(}Theorem~\textup{\ref{thm:mc3-type-a-resolution}}\textup{)}.
For arbitrary type, packages~\textup{(ii)--(iv)} reduce to
thick generation of\/~$\mathrm{Rep}_{\mathrm{fd}}(Y(\mathfrak{g}))$
by evaluation modules \textup{(}a result of Chari--Pressley for
all types, but not yet integrated into the manuscript's DK
hierarchy beyond type~$A$\textup{)}.
\end{corollary}

\begin{proof}
Package~(i) is
Theorem~\ref{thm:categorical-cg-all-types}.
For packages~(ii)--(iv): the DK equivalence on the
evaluation-generated core is available for all types
(Corollary~\ref{cor:dk23-all-types}).  Extension to the
full compact shifted-prefundamental core requires that
evaluation modules thickly generate all finite-dimensional
representations (available in type~$A$ by
Proposition~\ref{prop:dk2-thick-generation-typeA};
expected for all types by the Chari--Pressley classification
but not yet proved in this manuscript).
\end{proof}

\subsection{Landscape of the MC3 extension}
\label{sec:mc3-extension-landscape}

\begin{proposition}[Analysis of MC3 proof components]
\label{prop:mc3-automatic-generalization}
\label{rem:mc3-proof-step-analysis}% backward compatibility
\ClaimStatusProvedHere
The proof of Theorem~\textup{\ref{thm:mc3-type-a-resolution}} consists of four
components:
\begin{enumerate}[label=\textup{(\roman*)}]
\item Prefundamental Clebsch--Gordan closure.
\item Shifted-prefundamental generation \textup{(}compact objects $=$
  thick closure of $\{V_{\omega_i}\} \cup \{L^-_i\}$\textup{)}.
\item Pro-Weyl / Mittag-Leffler stabilization.
\item DK on evaluation-generated core, extended to compacts by
  Francis--Gaitsgory completion.
\end{enumerate}
Component~\textup{(i)} is now resolved for all simple types by
Theorem~\textup{\ref{thm:categorical-cg-all-types}}.
Component~\textup{(iv)}, in its starting input, requires a DK
equivalence on the evaluation-generated core: this is available
for all types by Corollary~\textup{\ref{cor:dk23-all-types}}.
The extension mechanisms in~\textup{(ii)}--\textup{(iv)}
\textup{(}chromatic filtration, Mittag-Leffler,
Francis--Gaitsgory pro-nilpotent completion\textup{)} are
categorical and type-independent.  The one remaining
type-specific input beyond type~$A$ is thick generation of\/
$\mathrm{Rep}_{\mathrm{fd}}$ by evaluation modules, which is
established in type~$A$ by
Proposition~\textup{\ref{prop:dk2-thick-generation-typeA}}
and is expected for all types by the Chari--Pressley
classification.
\end{proposition}

\begin{remark}[MC3 resolved for all types]
The categorical prefundamental Clebsch--Gordan decomposition
is now proved for all types
(Theorem~\ref{thm:categorical-cg-all-types}), using
multiplicity-free $q$-characters
\cite{FrenkelMukhin01, Nakajima04, Hernandez06KR} in place of the
minuscule hypothesis.  The former difficulty gradient
(Remark~\ref{rem:mc3-difficulty-gradient}) collapses: all types
are resolved by the same four-step argument.
\end{remark}

\begin{remark}[Type~$B$ from type~$A$ by folding --- superseded]
\label{prop:mc3-type-b-folding}
This proposition conjectured that MC3 for type~$B_n$ could be proved
by folding from type~$A_{2n-1}$.  This alternative route is now
\emph{unnecessary}: Theorem~\ref{thm:categorical-cg-all-types}
resolves MC3 for \emph{all} simple types uniformly via the
multiplicity-free $\ell$-weight property, which holds for every
fundamental representation of every simple Lie algebra
(Chari--Moura~\cite{ChariMoura06}).  The folding approach
remains of interest as an alternative proof technique but is
no longer needed for the MC3 programme.
\end{remark}

\begin{remark}[Difficulty gradient across Dynkin types --- resolved]
\label{rem:mc3-difficulty-gradient}
The following table records the resolution status of the categorical
CG lift for each simple type.  The multiplicity-free $\ell$-weight
argument (Theorem~\ref{thm:categorical-cg-all-types}) resolves all
types uniformly, replacing the former dependence on minuscule
representations:
\begin{center}
\begin{tabular}{cccc}
\toprule
Type & Minuscule count & Key input & Status \\
\midrule
...
\end{tabular}
\end{center}
The critical observation: the minuscule hypothesis (all ordinary weight
multiplicities equal~$1$) was used in the type~$A$ proof only to
ensure block separation.  The strictly weaker condition --- that all
$\ell$-weight spaces are one-dimensional (i.e., the $q$-character is
multiplicity-free) --- suffices and holds for every fundamental
representation of every simple Lie algebra \cite{ChariMoura06}.
For types $G_2$, $F_4$, $E_8$ with zero minuscule representations,
the $\ell$-weight separation handles the zero-weight multiplicity
(which is $1$, $2$, $8$ respectively for the smallest fundamental).
\end{remark}
...
\]
holds for all fundamental weights $\omega$ of every simple Lie algebra $\mathfrak{g}$, including types $F_4$, $G_2$, $E_6$, $E_7$, $E_8$.  The identity is the distributive law of a finite Laurent polynomial (Weyl character) times a formal power series (prefundamental character), verified computationally for all rank~$\leq 4$ types against Bourbaki root data.
\end{proposition}
