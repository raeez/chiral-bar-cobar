...
\begin{conjecture}[CoHA chiralisation through a comparison functor; \ClaimStatusConjectured]\label{conj:coha-e1}
The CoHA $\mathcal{H}_Q$ is an associative Hall algebra. A chiral
algebra appears only after a comparison functor from the Hall product
to vertex operators on a target factorisation space is specified.
The expected comparison has three necessary features:
...
parameterising deformation directions.
\end{enumerate}
A rigorous construction requires specifying the vertex operators, the
target factorisation algebra, and the Borcherds identity on that target.
\end{conjecture}
...
\subsection{\texorpdfstring{Cohomological Hall algebras as ordered Hall inputs}{Cohomological Hall algebras as ordered Hall inputs}}
\label{sec:coha-structure}
\index{CoHA!ordered Hall structure}
...
\ClaimStatusProvedHere]\label{prop:yangian-scalar-gauge-selfdual}
...
The following computation records one finite comparison with exact
diagonalization at $L=6$.
\end{proof}
...
No finite-dimensional diagonalization comparison for this higher-rank
...
\begin{corollary}[Conditional thick generation criterion for all simple types;
\ClaimStatusConditional]
\label{cor:dk2-thick-generation-all-types}%
\index{thick generation!all simple types|textbf}%
Let $\mathfrak g$ be simple. Assume that every finite-dimensional
irreducible $Y_\hbar(\mathfrak g)$-module is a subquotient of an
ordered tensor product of the chosen fundamental spectral modules
$V_{\omega_i}(a)$ inside the completed finite-dimensional
representation category. Then these modules thickly generate\/
$D^b(\mathrm{Rep}_{\mathrm{fd}}(Y_\hbar(\mathfrak{g})))$.
\end{corollary}

\begin{proof}
The hypothesis supplies Step~1 of
Proposition~\ref{prop:dk2-thick-generation-typeA}. The remaining
steps are purely categorical
...
\begin{theorem}[Conditional finite-dimensional spectral core from Drinfeld
polynomials;
\ClaimStatusConditional]
\label{thm:mc3-universal-multiplicity-free}
\index{Bethe subalgebra!joint spectrum}
\index{multiplicity-free!Chari--Moura}
For $\fg$ simple of any type, assume that the Drinfeld-polynomial
classification is realized by the chosen fundamental spectral modules:
every finite-dimensional irreducible
$Y_\hbar(\fg)$-module~$L(\psi)$ is a subquotient of an ordered tensor
product of those modules with parameters determined by its Drinfeld
polynomials. Then the fundamental spectral modules thickly generate
...
\begin{proof}
The hypothesis places every simple finite-dimensional module in the
thick closure of the fundamental spectral seed.
...
\subsubsection*{Four constructions for the category~$\mathcal{O}$
generation problem}
\label{sec:cat-O-generation-constructions}
\index{Category O@Category $\mathcal{O}$!generation constructions}
...
\begin{remark}[Interaction of the four constructions]
\label{rem:category-O-generation-obstruction}
\index{thick generation!construction comparison}
\begin{center}
\renewcommand{\arraystretch}{1.2}
\begin{tabular}{lll}
\textbf{Construction} & \textbf{Reduces to} & \textbf{Scope} \\
\hline
...
Conjecture~\ref{conj:mc3-automatic-generalization} from type~$A$
...
\begin{computation}[Thick generation obstruction analysis for
\texorpdfstring{$Y(\mathfrak{sl}_2)$}{Y(sl_2)}; \ClaimStatusProvedHere]
...
\item \emph{Type-$A$ resolution.}
 ...
 Conjecture~\ref{conj:mc3-automatic-generalization} are
...
of the Hern\'andez ordinary/shifted bridge
...
\begin{theorem}[Conditional finite-dimensional spectral core from Drinfeld polynomials;
 \ClaimStatusConditional]
...
its Yangian. Assume that every finite-dimensional irreducible is a
subquotient of an ordered tensor product of the chosen fundamental
spectral modules with parameters prescribed by its Drinfeld
polynomials. Then the thick subcategory of
\[
D^b(\operatorname{Rep}_{\mathrm{fd}}(Y_\hbar(\mathfrak{g})))
\]
generated by those modules is the whole bounded
...
\begin{proof}
The hypothesis puts every simple object of
\(\operatorname{Rep}_{\mathrm{fd}}\) in the thick closure of the
fundamental spectral seed. Lemma~\ref{lem:composition-thick-generation}
...
H^\bullet\barB(\cA),\qquad
K_X(\cA)=\mathbb D_{\Ran}\barB(\cA),\qquad
\cA^!,\qquad
Z^{\mathrm{der}}_{\mathrm{ch}}(\cA).
\]
Here
$q_\cA\colon\cA^{\mathrm i}\to\barB(\cA)$ carries quadratic
recognition, $\mathbb D(q_\cA)$ is its Verdier transpose, and
...
into a single algebraic family.

\begin{definition}[Baxter--Rees chart; \ClaimStatusDefinitional]
\label{def:baxter-rees-chart}
Consider the chain of Kirillov--Reshetikhin modules
...
\end{equation}
valid for all sufficiently large $k > l$. This datum is the
\emph{Baxter--Rees family}. The algebraicity and flatness of this family
are proved in Theorem~\ref{thm:yangian-baxter-rees-algebraicity}; its
generic and boundary fibers are identified in
Theorem~\ref{thm:yangian-generic-boundary-fibers}.
\end{definition}
...
$U^{\mathrm{RTT}}(R)$. The K3 chiral Hall--Drinfeld target
\[
\mathbf{H}_{\Delta_5}^{\mathrm{tgt}} \;:=\; \mathcal{D}_\hbar\!\left(\mathcal{Y}^{\mathrm{Hall}}_\hbar(\mathrm{CoHA}_{K3\times E}),\,
\widetilde{\Phi}^{\mathrm{Sieg\text{-}Bor}}_{\mathrm{Sp}_4}[\Phi_{10}^{\mathrm{un}}/\eta^{24}],\,
R_{\mathrm{Sieg,dyn}}\right)
\]
occupies a fourth taxonomic slot only conditionally, after the finite
Hall/CoHA source, compact Hopf pairing, PBW/no-extra-relations
theorem, comparison maps, and Mittag--Leffler exact inverse limit are
supplied. Its underlying Miki generators are Hall-theoretic rather
than Cartan-theoretic, its target coproduct is twisted by the
Siegel--Borcherds comparison datum
...
Miki-generator computations in this chapter are finite-window
\(\hbar\)-formal shadows of this fourth taxon. Under the CY-3 specialisation
...
\S\ref{sec:yangian-bar-explicit} admit a curved lift only at $\hbar^2 = -1/8$,
