...
\item \emph{Type-$A$ resolution.}
  ...
  category.
  %
  The extension to arbitrary simple~$\mathfrak{g}$ uses the
  generalized prefundamental Clebsch--Gordan closure via
...
For simply-laced types, this follows from Nakajima's
theory of quiver varieties~\cite{Nakajima04}: the torus-fixed
points on the quiver variety $\mathfrak{M}(\mathbf{v},
\mathbf{w}_{\omega_i})$ are isolated, so each
$q$-character monomial arises with multiplicity~$1$.
For all types including non-simply-laced, the multiplicity-free
property is established by the explicit $q$-character formulas of
Chari--Moura~\cite{ChariMoura06} for classical types and
by the Frenkel--Mukhin algorithm~\cite{FrenkelMukhin01} whose
termination for fundamental modules
\textup{(}special in the sense of~\cite{FrenkelMukhin01},
hence correctly computed by the algorithm\textup{)}
produces distinct monomials.
...
this is an exact faithful functor sending simples to simples,
and since it exponentiates the Yangian commuting fields to
quantum loop algebra $\ell$-weights, it preserves
$\ell$-weight space dimensions.
...
category~$\mathcal{O}$ of the quantum loop
algebra~\cite{Hernandez05, HJZ25}
\textup{(}transferred to the Yangian setting
via~\cite{GTL17, GTL-catO}\textup{)}, two simple
highest-$\ell$-weight modules lie in the same block only if their
$\ell$-weight ratio is a product of $A_{k,c}^{-1}$-factors.
At generic spectral parameters
\textup{(}avoiding a finite set of values of~$a - b$ where such
factorizations occur\textup{)}, the $\dim(V_{\omega_i})$
...
Gautam--Toledano Laredo equivalence between quantum
loop and Yangian representations
\cite{GTL17, GTL-catO}.
The new content is the observation
that these inputs, combined with the character identity
...
reference: for simply-laced types it is geometric
\cite{Nakajima04}; for classical types it is algebraic
\cite{ChariMoura06}; for all types the Frenkel--Mukhin
algorithm~\cite{FrenkelMukhin01} computes the $q$-character
and confirms multiplicity-freeness.
\end{remark}
