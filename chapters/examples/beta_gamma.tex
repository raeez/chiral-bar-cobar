...
terminating at degree~$4$. Within the quadrichotomy
$\{\mathbf{G},\mathbf{L},\mathbf{C},\mathbf{M}\}$ of
Chapter~\ref{chap:shadow-quadrichotomy-platonic}, $\beta\gamma$
is the unique witness of class~$\mathbf{C}$ in the standard
landscape; the Koszul-conductor identity
$c_{\beta\gamma} + c_{bc} = 0$ realises
$K_{\beta\gamma} = 0$ in the universal-conductor formula
$K = -c_{\mathrm{ghost}}(\BRST(\cdot))$ of
Chapter~\ref{chap:universal-conductor}, with the
$\beta\gamma$--$bc$ Koszul pair playing the role of matter and
ghost in opposite statistical sectors. Three invariants pin down the archetype
...
degree~$4$ detected by the transferred $A_\infty$ operation $m_4$.
...
the secondary Borcherds operation at degree~$3$, and the quartic
...
degree~$4$ ($r_{\max} = 4$), so genus-$2$ corrections are purely
...
\textup{(}UNIFORM-WEIGHT scope; for multi-weight the identity
acquires a cross-channel correction $\delta F_g^{\mathrm{cross}}$
at $g \geq 2$, see Theorem~\ref{thm:multi-weight-genus-expansion}\textup{)}
$m_0^{(g)}(\cA) = \kappa(\cA) \cdot \lambda_g$
where $\lambda_g \in H^{2g}(\overline{\mathcal{M}}_g)$ is the
$\lambda$-class. The $\beta\gamma$ system is uniform-weight
\textup{(}single conformal-weight pair $(\lambda, 1{-}\lambda)$\textup{)},
so the formula applies unconditionally for all~$g$. At $g = 1$,
...
weight-changing deformations at degrees~$3$ and~$4$ give
$d_{\mathrm{alg}} = 2$
...
\paragraph{Step~3: Degree~$4$, quartic contact appears.}
...
\paragraph{Step~4: Termination at degree~$4$.}
...
degree~$4$ on weight-changing line; \ClaimStatusProvedHere]
...
of $\beta\gamma$ vanishes through degree~$3$:
$\Theta_{\beta\gamma}^{\leq 3}\big|_{\mathrm{w.c.}} = 0$
\textup{(}both the degree-$2$ and cubic shadows vanish on
this slice\textup{)}.
The first nontrivial level is degree~$4$, where
...
$\mu_{\beta\gamma} = 0$, so the tower terminates at degree~$4$
...
quartic contact corolla $K_{0,4}$ composes with the degree-$2$
...
$d\log$ absorption gives collision depth $k_{\max} = 0$ and the degree-$2$ transferred bracket vanishes on
...
of the tower termination at degree~$4$.

Kinematically, $\beta\gamma$ is \emph{simpler} than Virasoro
($h_K = 0.567$) despite having richer shadow depth (degree~$4$ vs
