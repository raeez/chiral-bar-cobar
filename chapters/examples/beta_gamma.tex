...
which is absorbed by $d\log$ extraction (AP19).  The leading
nontrivial interaction datum is the quartic contact shadow,
not the $r$-matrix.

The genus-$0$ curvature vanishes ($m_0 = 0$: the $\beta\gamma$ OPE
has no double pole), so the bar complex is uncurved and the genus
tower is trivial at the scalar level.  The tower does not terminate
there.  The quartic contact shadow $\Theta^{\leq 4}$ is nontrivial:
the modular $L_\infty$ algebra $\Definfmod(\cA)$ carries higher
brackets that the strict model~$\Convstr$ truncates.  The tower
terminates at weight~$4$ because the quintic obstruction $o_5$
vanishes by rank-one abelian rigidity.  This places $\beta\gamma$ in the contact/quartic archetype
(class~C, $r_{\max} = 4$): beyond the Gaussian families
(Heisenberg, lattice; $r_{\max} = 2$) and the Lie/tree archetype
(Kac--Moody; $r_{\max} = 3$), but still finite, distinct from
