...
tower to terminate at arity~$2$ or~$3$.

The depth classification assigns $\beta\gamma$ to a class of its
own. Heisenberg and lattice algebras (class~$\mathsf{G}$) have
shadow towers terminating at arity~$2$; affine Kac--Moody algebras
(class~$\mathsf{L}$) terminate at arity~$3$; Virasoro and
$\mathcal{W}_N$ (class~$\mathsf{M}$) run to infinity. The
$\beta\gamma$ system is the sole representative of class~$\mathsf{C}$,
the contact/quartic class: nontrivial beyond the scalar level, yet
terminating at arity~$4$. Three invariants pin down the archetype
(AP59): the maximal OPE pole order $p_{\max} = 1$, the collision
depth $k_{\max} = p_{\max} - 1 = 0$, and the shadow depth
$r_{\max} = 4$. The shadow depth is not inherited from the pole
order; it arises from an independent contact interaction at
arity~$4$ detected by the transferred $A_\infty$ operation $m_4$.
The Koszul dual is the $bc$ ghost system
(Theorem~\ref{thm:betagamma-fermion-koszul}), exchanging bosonic
and fermionic statistics: the chiral lift of
$\mathrm{Sym}(V)^! = \Lambda(V^*)$, with matching shadow invariants
in opposite statistical sector. Two generators of complementary
conformal weight $\lambda$ and $1-\lambda$, coupled by the simple-pole
OPE $\beta(z)\gamma(w) \sim 1/(z{-}w)$, are the minimal data forcing
this archetype.
...
A (adjunction) &
  $(\beta\gamma)^! \simeq bc$
  & Proved & Thm~\ref{thm:betagamma-bc-koszul} \\
B (inversion) &
  $\Omega(\barB(\beta\gamma)) \xrightarrow{\sim}
  \beta\gamma$
  & Proved & Thm~\ref{thm:betagamma-complete-bar} \\
C (complementarity) &
  $\kappa(\beta\gamma) + \kappa(bc) = 0$
  & Proved & \S\ref{sec:koszul-duality-free-fields} \\
D (modular char.) &
  $\kappa(\beta\gamma)
...
which is absorbed by $d\log$ extraction.  The leading
nontrivial interaction datum is the quartic contact shadow,
not the $r$-matrix.

The genus-$0$ curvature vanishes ($m_0 = 0$: the $\beta\gamma$ OPE
has no double pole), so the genus-$0$ bar complex is uncurved.
On the weight-changing line, the arity-$2$ shadow vanishes
...
The quartic contact shadow $\Theta^{\leq 4}$ is nontrivial:
$\Definfmod(\cA)$ carries higher
brackets that $\Convstr$ truncates.  The tower
terminates at weight~$4$ because the quintic obstruction $o_5$
vanishes by rank-one abelian rigidity: class~C
($r_{\max} = 4$), beyond the Gaussian families
($r_{\max} = 2$) and Lie/tree
($r_{\max} = 3$), but still finite, distinct from
the infinite tower of Virasoro and $\mathcal{W}_N$
(Chapter~\ref{chap:w-algebra-koszul}).
...
$\Theta_{\beta\gamma}^{\leq 2}\big|_{\mathrm{w.c.}} = 0$
while $\Theta_{\beta\gamma}^{\leq 4}\big|_{\mathrm{w.c.}} \neq 0$
(the quartic contact shadow exists, even
...
the arity-$2$ vanishing is specific to the weight-changing slice,
...
\item $k_{\max}(\beta\gamma) = 0$ (the collision depth, by AP19
  $d\log$ absorption);
\item $r_{\max}(\beta\gamma) = 4$ (the shadow depth).
\end{itemize}
The discrepancy between $k_{\max} = 0$ and $r_{\max} = 4$ arises
because the shadow obstruction tower at higher arities is \emph{not}
...
\begin{warning}[AP59: never conflate $p_{\max}$, $k_{\max}$, $r_{\max}$]
\label{warn:bg-ap59}
\index{AP59!beta-gamma@$\beta\gamma$ three-invariant conflation}
...
\textup{(}c\textup{)}~The actual shadow depth is~$4$, controlled by
the quartic contact class descending from composite currents.
\emph{Never conflate $p_{\max}$, $k_{\max}$, $r_{\max}$.}
\end{warning}
...
\emph{trivial}: they lie in the $k_{\max} = 0$ trichotomy class.  This
places $\beta\gamma$ in a distinguished position: a chiral algebra
with a nontrivial higher shadow obstruction tower
$(r_{\max} = 4)$ but trivial commuting Hamiltonians.  Any treatment of
the $\beta\gamma$ system in the Russian-school tradition must make
this distinction visible, since the commuting-Hamiltonian
triviality cannot be read off from shadow depth or from the central
charge.  The
three-invariant disagreement is verified against compute by the
\texttt{theorem\_three\_paper\_intersection\_engine.py} module, which
hardcodes $(p_{\max}, k_{\max}, r_{\max}) = (1,0,4)$ for
$\beta\gamma$.
\end{remark}
...
the arity-$2$ shadow restricted to the weight-changing line vanishes
($\Theta_{\beta\gamma}^{\leq 2}\big|_{\mathrm{w.c.}} = 0$:
...
correspond to non-semisimple phenomena in equivariant homological
algebra: the parent $\beta\gamma$ system is chirally Koszul, but
its triplet quotient may fail bar concentration.
\end{remark}
...
$\beta\gamma$ system as a boundary algebra.  The resulting
...
($\cH_k^! = \mathrm{Sym}^{\mathrm{ch}}(V^*)$, not~$\cH_k$; AP33),
...
at the level of the vertex algebra OPE.  The nonlinear
...
weight-changing deformations at arities~$3$ and~$4$ give
$d_{\mathrm{alg}} = 2$
...
$W(\beta\gamma) = \{1,1\}$ (both generators have conformal
weight~$1$), so $\operatorname{ek}(\beta\gamma) = 0$ and
$S_{\beta\gamma}(u) = 2\,\zeta(u)\,\zeta(u{+}1)$.
The $\beta\gamma$ system is \emph{exact Euler--Koszul}
...
\begin{corollary}[$\beta\gamma$ shadow obstruction tower:
...
of $\beta\gamma$ vanishes through arity~$3$:
$\Theta_{\beta\gamma}^{\leq 3}\big|_{\mathrm{w.c.}} = 0$
\textup{(}both the arity-$2$ and cubic shadows vanish on
this slice\textup{)}.
The first nontrivial level is arity~$4$, where
$\Theta_{\beta\gamma}^{\leq 4}\big|_{\mathrm{w.c.}}$ encodes
the quartic contact invariant~$\mu_{\beta\gamma}$.
...
\textup{(}Off the weight-changing line, the global modular
characteristic $\kappa(\beta\gamma) = c/2 \neq 0$ is the
arity-$2$ shadow;
see Proposition~\textup{\ref{prop:betagamma-obstruction-coefficient}}.\textup{)}
\end{corollary}
...
On the weight-changing line, the primitive logarithmic modular kernel
\textup{(}Definition~\textup{\ref{def:primitive-log-modular-kernel})}
of the $\beta\gamma$ system is
...
\item $K_{0,2}^{\beta\gamma}$
  is the arity-$2$ corolla from the bilinear $\beta\gamma$ pairing;
...
line, so $K_{0,3} = 0$.  The first nontrivial transferred operation
is $m_3$ (from the composite $:$$\beta\partial\gamma$$:$ interaction),
giving $K_{0,4} = \operatorname{cyc}(m_3)$.  Higher operations
...
for $n \geq 5$.  The rigid-cutting residue $R_{\mathrm{pf}}$ is
nonzero: the quartic contact stratum of the planted-forest boundary
produces a nontrivial residue via $K_{0,4}$.
\end{proof}

\begin{proposition}[\texorpdfstring{$\beta\gamma$}{beta-gamma}
primitive shell equations;
\ClaimStatusProvedHere]
\label{prop:betagamma-primitive-shell}
\index{shell equation!beta-gamma@$\beta\gamma$}
The genus-$2$ shell equation for $\beta\gamma$ exhibits all three
channels of the primitive master equation:
...
\end{equation}
The separating bracket vanishes by antisymmetry
\textup{(}the branch space on the weight-changing line is rank~$1$\textup{)},
but the BV and rigid-cutting channels are both nonzero.
\end{proposition}
...
weight-changing line.  The rigid-cutting channel is nonzero: the
quartic contact corolla $K_{0,4}$ composes with the arity-$2$
corolla $K_{0,2}$ through the planted-forest boundary of $\overline{M}_{2,n}$,
producing a nonzero rigid residue.  This is the channel absent
from both Heisenberg
(Proposition~\ref{prop:heisenberg-primitive-shell})
and affine
(Proposition~\ref{prop:affine-primitive-shell}), and its
presence is the signature of the contact class.
\end{proof}
...
Despite the presence of a quartic
corolla $K_{0,4} \neq 0$ in the primitive kernel, the contact
invariant on the weight-changing line projects to zero in the branch
BV packet.  The spectral discriminant and metaplectic half-density
...
The detailed computations underlying
...
interaction of the $\beta\gamma$ OPE, the homotopy transfer
...
\begin{proposition}[Why $\beta\gamma$ is class~$\mathsf{C}$: Sugawara composite and stratum separation]
\label{prop:betagamma-sugawara-class-c}
\ClaimStatusProvedHere
\index{beta-gamma system@$\beta\gamma$ system!class C mechanism}
\index{stratum separation!class C mechanism}
\index{Sugawara construction!class C depth}
The $\beta\gamma$ system is class~$\mathsf{C}$
$($shadow depth $r_{\max} = 4)$ despite being a free field.
Three independent mechanisms interact to produce this classification:
\begin{enumerate}
\item[\textup{(i)}]
\textbf{Abelianness kills the cubic pump.}
The generator OPE
$\beta(z)\gamma(w) \sim 1/(z{-}w)$ has a simple pole:
$d\log$ absorption gives collision depth $k_{\max} = 0$
$($AP19$)$, and the arity-$2$ transferred bracket vanishes on
the weight-changing line
$($Lemma~\textup{\ref{lem:betagamma-ell2-vanishing}}$)$.
Every ternary bracket factors through the vanishing binary bracket:
$\ell_3^{\mathrm{tr}}(\eta,\eta,\eta) = 0$
$($Proposition~\textup{\ref{prop:betagamma-ell3-vanishing}}$)$.
The cubic pump $\mathrm{ad}_{S_3}$ that drives the tower to
...
but creates genuine shadow depth beyond arity~$3$.

\item[\textup{(iii)}]
\textbf{Stratum separation kills the quintic pump.}
The cyclic deformation complex decomposes by charge:
$\Defcyc^{\mathrm{mod}} = \bigoplus_q \Defcyc^{(q)}$.
The quartic contact invariant
$\mathfrak{Q}_{\beta\gamma}$ lives on the charged stratum
$($charge $q \neq 0$$)$.  The self-bracket
$\Defcyc^{(q)} \otimes \Defcyc^{(q)} \to \Defcyc^{(2q)}$
has $\dim \Defcyc^{(2q)} = 0$ for rank-one systems: the
quartic-to-quintic pump does not activate.
On the primary weight-changing line,
$\Delta = 8\kappa S_4 = 0$ and $\rho = 0$
$($landscape census, Table~\textup{\ref{tab:shadow-tower-census}}$)$.
This is rank-one abelian rigidity
$($Theorem~\textup{\ref{thm:betagamma-rank-one-rigidity}}$)$
recast as stratum separation
$($Remark~\textup{\ref{rem:contact-stratum-separation}}$)$.
\end{enumerate}
The Heisenberg system, by contrast, has $T = (1/2)\normord{JJ}$
with the same composite Sugawara structure, but no charged stratum:
$\Defcyc^{(q)} = 0$ for $q \neq 0$ $($single abelian current$)$, so
the quartic contact has no charged sector to inhabit.
This places Heisenberg in class~$\mathsf{G}$ $($depth $2$$)$, not
class~$\mathsf{C}$.
\end{proposition}

\begin{proof}
Part~(i) is Lemma~\ref{lem:betagamma-ell2-vanishing} and
Proposition~\ref{prop:betagamma-ell3-vanishing}.
Part~(ii) is Theorem~\ref{thm:betagamma-quartic-birth}: the
transferred operation $m_3$ produces a nonzero quartic corolla
because the composite $T_\lambda$ carries a quartic OPE pole.
Part~(iii) is Theorem~\ref{thm:betagamma-rank-one-rigidity},
which proves that on a one-dimensional cyclic subspace
with vanishing transferred brackets, the complementarity
potential is exactly quadratic; equivalently, the quintic
obstruction $o_5 = \{\mathfrak{C},\mathfrak{Q}\}_H = 0$
$($since $\mathfrak{C} = 0$$)$ and all higher obstructions
vanish by charge-graded dimension counting
$($Remark~\ref{rem:contact-stratum-separation}$)$.
The tower terminates at $r_{\max} = 4$: class~$\mathsf{C}$.
\end{proof}
...
The line operators of the $\beta\gamma$ system
