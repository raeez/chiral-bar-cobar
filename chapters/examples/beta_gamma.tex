...
The $\beta\gamma$ system has bar complex growth rate $3^n$, governed by the discriminant $\Delta(x) = (1-3x)(1+x)$.  Its Koszul dual is the $bc$ ghost system.

\noindent\emph{Governing question.} How does the first genuinely
non-abelian free-field example alter the bar differential while
remaining fully computable?  This chapter is a complete theorematic
portrait: every claim is \ClaimStatusProvedHere{} or follows from
proved core theorems.

\section{Setup and conventions}
\index{beta-gamma system@$\beta\gamma$ system|textbf}

\begin{remark}[Semantic levels; Convention~\ref{conv:hms-levels}]
\label{rem:betagamma-hms}
This chapter operates at \textbf{M-level}: the bar complex spectral
sequence for the $\beta\gamma$ system is constructed explicitly, with
degree-by-degree differential matrices.  \textbf{S-level} shadows
include the growth rate $3^n$, cohomology dimensions, and the
...
For any $\lambda \in \mathbb{C}$, the $\beta\gamma$ system has commuting fields of weights $(\lambda, 1-\lambda)$.  The $bc$ ghost system is the fermionic counterpart; at $\lambda = 2$ it provides the BRST ghosts of the bosonic string.
\end{remark}
...
The bar complex $\bar{B}(\beta\gamma)$ in degrees~4 and~5 extends the computation of \S\ref{sec:beta-gamma-complete-analysis}.
