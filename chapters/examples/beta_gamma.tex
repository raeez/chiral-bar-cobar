...
\begin{corollary}[$\beta\gamma$ weight-changing line is shadow-trivial; \ClaimStatusProvedHere]
...
The standard conformal-weight family formulas are exactly the
class-$\mathsf{C}$ clause of
...
case of Proposition~\ref{prop:depth-gap-trichotomy}. The
weight-changing slice statement is Theorem~\ref{thm:betagamma-rank-one-rigidity},
which gives vanishing of the shadow tower on the one-dimensional
...
enters the Wave~13 construction as a building block. The class-$\mathcal S$
...
(Vol.~II, \S\ref{sec:lineop-ext-wave13-DNA}) is generated by a
