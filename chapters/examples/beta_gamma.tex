...
the Heisenberg algebra of Chapter~\ref{ch:heisenberg-frame} (where the bar complex is governed by partition
...
growth rate $3^n$, governed by the discriminant
$\Delta(x) = (1-3x)(1+x)$ invisible in the abelian case.  This chapter develops the complete bar complex
...
Unlike the Heisenberg bar complex (\S\ref{sec:frame-bar-all}), where $d_{\mathrm{bracket}} = 0$ and the bar differential is determined entirely by the abelian OPE, the $\beta\gamma$ system has a nontrivial bar differential arising from the mixed $\beta$--$\gamma$ OPE poles.
...
The rank (number of independent generator types) satisfies $\operatorname{rank}(\bar{B}^n) = 2 \cdot 3^{n-1}$ for all $n \geq 1$ (Theorem~\ref{conj:betagamma-bar-dim}).
\end{theorem}
...
\end{tabular}
\end{center}
A type with $k$ copies of $\beta$ and $4-k$ copies of $\gamma$ has exactly $k(4-k)$ nonzero residues (one for each mixed pair).%
\footnote{In this table, $D_{ij}$ denotes the bar differential extracting the OPE between positions $i$ and $j$.  For the $\beta\gamma$ system, $D_{ij}$ is nonzero whenever positions $i$ and $j$ carry complementary fields ($\beta$ and $\gamma$, in either order), since both $\beta(z)\gamma(w) \sim 1/(z-w)$ and $\gamma(z)\beta(w) \sim -1/(z-w)$ have simple poles.}
