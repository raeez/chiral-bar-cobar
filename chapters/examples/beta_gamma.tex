...
lane. Two generators of complementary conformal weight $\lambda$ and
...
class-$\mathsf C$ lane the planted-forest contact channel is present:
...
\begin{remark}[Theorem A in four lanes: $\beta\gamma$]
\label{rem:betagamma-theorem-A-four-lanes}
\index{Theorem A!four lanes!beta-gamma@$\beta\gamma$}
...
homotopy is the contact one $h_C=h_{\mathrm{LV}}/(\kappa+\kappa^!)$
which simplifies because $\kappa+\kappa^!=0$ for the $\beta\gamma$/$bc$
free-field pair after taking the formal residue along
...
\end{itemize}
The four lanes coincide on the entire $\lambda$-family because
...
$b(z)b(w) \sim 0$, $c(z)c(w) \sim 0$
(Definition in \S\ref{sec:free-fermion}), with graded Clifford
...
so $\kappa(\beta\gamma)+\kappa(bc)=0$ on the scalar lane. This is
...
the bulk is the separate chiral Hochschild object
...
it is present on the standard class-$\mathsf C$ lane with
...
\begin{definition}[The $\beta\gamma$ line $r$-matrix]
...
C (complementarity) &
 Cross-polarization &
 scalar $\kappa$-lane complementarity \\
D (modular char.) &
