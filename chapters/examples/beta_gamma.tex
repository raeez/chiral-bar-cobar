...
Complete bar-cobar and Koszul computation for $\beta\gamma$.
Shadow archetype: \emph{contact/quartic}, terminating at weight~$4$.
Key quantitative results: $\kappa_{\beta\gamma} = 0$ on the weight-changing line (uncurved there; globally $\kappa = 6\lambda^2 - 6\lambda + 1$), yet
...
The $\beta\gamma$ system is the first nontrivial complete computation
in this monograph that remains fully manageable from start to finish.
Every structure theorem of Part~I --- the bar-cobar adjunction, the
Koszul dual, the complementarity splitting, the genus tower, and the
shadow Postnikov tower --- can be exhibited here in closed form,
without truncation or appeal to spectral sequences.  The chapter
therefore presents the $\beta\gamma$ system in three synchronized
lights: as a concrete chiral algebra with explicit OPE, mode algebra,
and stress tensor; as a bar-cobar and Koszul object whose bar complex
shares the discriminant $\Delta(x) = (1-3x)(1+x)$ with
$\widehat{\mathfrak{sl}}_2$ and the Virasoro algebra; and as the first
example where the Maurer--Cartan dictionary of Part~I truly earns its
keep, detecting structure invisible to classical methods.
...
\noindent\textbf{Chriss--Ginzburg frame.}\;
...
(Construction~\ref{constr:tower-template}) for the $\beta\gamma$
system on the weight-changing line.

\paragraph{Step~1: Arity~$2$.}
$\kappa(\beta\gamma) = 0$: the bar complex is uncurved.
$\Theta_{\beta\gamma}^{\leq 2} = 0$.
...
$\infty$).  This is the cleanest demonstration that shadow depth
(dynamic complexity) and completion entropy (kinematic complexity) are
genuinely independent invariants.
\end{remark}
...
(Theorem~\ref{thm:Koszul_dual_Yangian}).  Because the $\beta\gamma$
...
combinatorics.  This is the simplest non-trivial evaluation of the
