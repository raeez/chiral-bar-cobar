...
From the explicit bar complex computation
(Theorem~\ref{thm:betagamma-fermion-koszul} together with
\S\ref{subsec:bar-cobar-verification}), one obtains:
\begin{itemize}
...
\end{itemize}
The central charges are complementary:
\[
c_{\beta\gamma} = +2(6\lambda^2 - 6\lambda + 1),
\qquad
c_{bc} = -2(6\lambda^2 - 6\lambda + 1),
\]
so $c_{\beta\gamma} + c_{bc} = 0$.

This cancellation is specific to the Sym/Ext exchange.  Replacing
bosonic statistics by fermionic statistics negates the central charge.
It does not hold for general Koszul dual pairs: for affine
Kac--Moody algebras
$\hat{\fg}_k^! \simeq \mathrm{CE}(\fg_{-k-2h^\vee})$,
