...
A (adjunction) &
 ...
 $q_A\colon A^{\mathrm i}\xrightarrow{\sim}\bar B(A)$, and
 $\nu_A\colon K_X(A)\xrightarrow{\sim}bc_\lambda$
 for $A=\beta\gamma_\lambda$
...
H (chiral Hochschild) &
 $H_H(\beta\gamma_\lambda;S_{\beta\gamma})$ gives
...
$A^{\mathrm{i}}=C_X(s^{-1}V,s^{-2}R)$ is the quadratic coalgebra,
$K_X(A)=\mathbb D_{\Ran}B_X^\Sigma(A)$ is the symmetric Verdier
algebra, and
$Z^{\mathrm{der}}_{\mathrm{ch}}(A)=\ChirHoch^\bullet(A,A)$ is the
cochain-level chiral closed sector.  The maps
$q_A\colon A^{\mathrm i}\to B_X^\Sigma(A)$ and
$\nu_A\colon K_X(A)\xrightarrow{\sim}bc_\lambda$ give the quadratic
and chosen-pair comparisons on finite-type truncations.  The identity
$\Omega(B(A))\simeq A$ is bar--cobar inversion.  The family datum
$H_H(A;S_{\beta\gamma})$ belongs to the derived-centre object and gives
\[
 \operatorname{Supp}\ChirHoch^\bullet(A,A)
 \subseteq S_{\beta\gamma}.
\]
The bar coalgebra and the Koszul-dual algebra retain their respective
types.  A perfect degree-$d$ chain pairing is the additional map that
produces a degree reflection.
...
Theorem~\ref{thm:betagamma-bar-complex}.  The L1 proof obligation is
the construction of maps $(i_C,p_C,h_C)$ satisfying
$d h_C+h_Cd=\nu_C(\mathrm{id}-i_Cp_C)$.  A solution in the
free-field OPE normalisation has $\nu_C=1$ and hence contraction
homotopy $\nu_C^{-1}h_C=h_C$.  A selected Loday--Vallette comparison
may then identify this constructed homotopy with a representative
$h_{\mathrm{LV}}$.  The cited bar-complex theorem supplies the
complex and differential; the contraction datum is the additional
L1 construction.  The level-$2$ scalar $\nu_C=1$ and the
algebra-level complementarity ceiling
$K^\kappa=\kappa+\kappa^!=0$ are carried by their respective
comparison maps.
...
standard-family ledger; \ClaimStatusConditional]
\label{prop:betagamma-standard-family-ledger}
\index{beta-gamma system@$\beta\gamma$ system!standard-family ledger}
\index{bc system@$bc$ system!statistics convention}
\index{symplectic boson!standard specialization}
Let \(\beta\gamma_\lambda\) be the bosonic two-generator system with
\[
h_\beta=\lambda,\qquad h_\gamma=1-\lambda,\qquad
\beta(z)\gamma(w)\sim\frac1{z-w},
\]
and no singular \(\beta\beta\) or \(\gamma\gamma\) OPE.  In the
ordered bar presentation the first nonzero differential is the
degree-two mixed contraction
\begin{equation}\label{eq:betagamma-ledger-bar-d2}
d[\beta|\beta]=0,\qquad d[\gamma|\gamma]=0,\qquad
d[\beta|\gamma]=\mathbf 1,\qquad d[\gamma|\beta]=-\mathbf 1 .
\end{equation}
In degree three, the same simple-pole extraction gives, for example,
\begin{equation}\label{eq:betagamma-ledger-bar-d3}
d[\beta_1|\beta_2|\gamma_3]
=[\beta_1|\mathbf 1]-[\mathbf 1|\beta_2],
\end{equation}
with the full lowest-weight skeleton recorded in
Theorem~\ref{thm:betagamma-complete-bar}.  The simple pole therefore
feeds the ordered bar differential, while the closed pole-valued
collision \(r\)-matrix is regular after the \(d\log\)-absorption
convention.

The class-\(\mathsf C\) standard conformal-weight family has
\begin{equation}\label{eq:betagamma-ledger-shadow}
S_2=6\lambda^2-6\lambda+1,\qquad
S_3=0,\qquad
S_4=-\frac{5}{12},\qquad
S_r=0\quad(r\ge5).
\end{equation}
The quartic term is the cyclic contact
\(\mathfrak Q_{\beta\gamma}=\operatorname{cyc}(m_3)\) on the
standard contact direction
\textup{(}Theorem~\ref{thm:betagamma-quartic-birth}\textup{)}.  On the
rank-one weight-changing line
\[
L_{\mathrm{w.c.}}=\mathbb C\eta,\qquad
\eta=\partial_\lambda T_\lambda
=\normord{\beta\,\partial\gamma}
-\normord{(\partial\beta)\,\gamma},
\]
the transferred brackets vanish:
\[
\ell_2^{\mathrm{tr}}(\eta,\eta)=0,\qquad
\ell_3^{\mathrm{tr}}(\eta,\eta,\eta)=0,\qquad
\mu_{\beta\gamma}:=\langle\eta,m_3(\eta,\eta,\eta)\rangle=0
\]
\textup{(}Lemma~\ref{lem:betagamma-ell2-vanishing},
Proposition~\ref{prop:betagamma-ell3-vanishing}, and
Corollary~\ref{cor:betagamma-mu-vanishing}\textup{)}.  Thus the
standard contact surface has a nonzero quartic coefficient, while the
internal weight-changing line has no quartic scalar projection.

The modular characteristics are
\begin{equation}\label{eq:betagamma-ledger-kappa}
\kappa(\beta\gamma_\lambda)
=6\lambda^2-6\lambda+1,\qquad
\kappa(bc_\lambda)=-(6\lambda^2-6\lambda+1),
\end{equation}
so \(K^\kappa(\beta\gamma_\lambda,bc_\lambda)=0\).  The Koszul dual
is parity-changing:
\[
(\beta\gamma_\lambda)^!\simeq bc_\lambda,\qquad
(bc_\lambda)^!\simeq\beta\gamma_\lambda .
\]
Here \(bc_\lambda\) is the two-generator fermionic ghost system with
odd fields \(b,c\) of weights \(\lambda,1-\lambda\) and Clifford
contact tensor \(b\otimes c+c\otimes b\).  It is not the
single-generator free fermion \(\mathcal F\), whose Koszul partner is
a one-generator bosonic symmetric chiral algebra.

At \(\lambda=\tfrac12\), the bosonic system
\(\beta\gamma_{1/2}\) is the rank-one symplectic boson:
\[
c_{\beta\gamma}=-1,\qquad \kappa(\beta\gamma_{1/2})=-\tfrac12.
\]
Its Koszul partner \(bc_{1/2}\) is instead the complex-fermion pair,
with \(c_{bc}=1\) and \(\kappa(bc_{1/2})=\tfrac12\).  The
\(\mathbb Z_2\)-equivariant bar statement at this point is
\[
H^\bullet(\bar B(\beta\gamma_{1/2}))^{\mathbb Z_2}
\cong
\bigl((\beta\gamma_{1/2})^{\mathrm i}\bigr)^{\mathbb Z_2},
\]
and the corresponding equivariant twisted Koszul resolution has only
vacuum cohomology, over characteristic zero, as in
Proposition~\ref{prop:symplectic-equivariant-cohomology}.
\end{proposition}
...
For generic conformal weight $\lambda$, choose the quadratic
presentation coalgebra
$(\beta\gamma_\lambda)^{\mathrm i}
=C_X(s^{-1}V,s^{-2}R)$ and its canonical comparison~$q_\lambda$.
Under $H_{\mathrm{CL}}$,
\[
H^\bullet(q_\lambda)\colon
H^\bullet\!\bigl((\beta\gamma_\lambda)^{\mathrm i}\bigr)
\xrightarrow{\;\sim\;}
H^\bullet\!\bigl(\bar{B}_{\mathrm{geom}}
(\beta\gamma_\lambda)\bigr).
\]
Ran--Verdier duality forms
$K_X(\beta\gamma_\lambda)$, and the chosen-pair map
$\nu_\lambda\colon K_X(\beta\gamma_\lambda)
\xrightarrow{\sim}(\beta\gamma_\lambda)^!$
identifies the latter with $bc_\lambda$ on the finite-type completed
pair locus.  The bar cohomology has positive-degree classes.  The
twisted Koszul resolution
$\beta\gamma_\lambda\otimes_{\tau}\bar{B}(\beta\gamma_\lambda)$,
has augmentation cohomology $\mathbb{C}$.
\end{theorem}
...
Clifford relation for $bc_\lambda$.  The map
$q_A\colon A^{\mathrm i}\to\bar B(A)$ compares the presentation
coalgebra with the full bar; $H_{\mathrm{CL}}$ makes
$H^\bullet(q_A)$ an isomorphism.  Finite-window Verdier duality and
$\nu_A$ then carry the presentation data to the $bc$ partner.  Thus
the positive bar cohomology classes correspond to the coalgebra
...
\cdot \mathbb{1}
\end{align}
where the holomorphic coefficient of $\eta_{12}^{(1)}$ is
$\partial_u\log\theta_1(u|\tau)$ and the descended one-form also
includes the Arakelov coefficient
$2\pi i\,\operatorname{Im}(u)d u/\operatorname{Im}\tau$; here
$u=z_1-z_2$. After the rational double-pole term is subtracted,
the normalized elliptic kernel records the coefficient of $u^{-1}$ in
$P^{(1)}/u$. From
