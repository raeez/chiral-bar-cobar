...
The $\beta\gamma$ system is the chiral lift of $\mathrm{Com}^! = \mathrm{Lie}$: the symplectic pairing $\langle\beta,\gamma\rangle = 1$ produces a nontrivial bar differential that kills mixed tensor types while leaving pure types as cocycles, giving $3^n$ growth and discriminant $\Delta(x) = (1-3x)(1+x)$.  The Koszul dual is the $bc$ ghost system --- statistics exchange (bosonic $\leftrightarrow$ fermionic) is the two-generator shadow of operadic duality.
\end{remark}
...
\section{Setup and conventions}\label{rem:betagamma-hms}
...
\begin{theorem}[Cobar gives \texorpdfstring{$\beta\gamma$}{beta-gamma}; \ClaimStatusProvedHere]
...
\begin{proposition}[Central charge complementarity for \texorpdfstring{$\beta\gamma$}{beta-gamma}/\texorpdfstring{$bc$}{bc}; \ClaimStatusProvedHere]\label{thm:betagamma-bc-koszul-detailed}
