...
This construction, due to Beem et al., produces vertex algebras from 4d SCFTs.
It is distinct from the $\Omega$-background/Nekrasov partition-function
approach used in the AGT correspondence below.
\end{construction}

\begin{theorem}[4d/2d is $\Einf$-Chiral; \ClaimStatusProvedElsewhere]\label{thm:4d2d-einf}
...
The OPE of the resulting chiral algebra is generically \emph{non-commutative}: for instance, the 4d/2d image of $SU(2)$ $\mathcal{N}=4$ SYM is the small $\mathcal{N}=4$ superconformal vertex algebra at $c = -9$, which contains $\widehat{\mathfrak{sl}}_2$ at the critical level $k = -2 = -h^\vee$ as its R-symmetry current subalgebra.  (Note: the stress tensor is an independent generator, not the Sugawara tensor, since the Sugawara construction is undefined at the critical level.)  The $\Einf$ structure arises from the full vertex algebra axioms (locality, associativity of OPE), not from commutativity.
\end{theorem}
...
\]
the W-algebra associated to $\mathfrak{sl}_n$ at the critical level $k = -n = -h^\vee$. At this special level the Sugawara construction degenerates and $\mathcal{W}^{-h^\vee}(\mathfrak{sl}_n)$ coincides with the Feigin--Frenkel center $\mathfrak{z}(\widehat{\mathfrak{g}})$, a commutative vertex algebra (see Arakawa \cite{Ara15} for the precise relationship). The dependence on the Riemann surface $C$ and its pants decomposition appears at the level of conformal blocks and modules, not the algebra itself.
\end{example}
...
\begin{theorem}[Non-Commutative CS; \ClaimStatusConjectured]\label{thm:nc-cs}
...
\begin{remark}[Scope]
Theorem~\ref{thm:nc-cs} requires input from non-commutative gauge theory:
...
consistent with our framework but its verification requires non-commutative
...
\begin{theorem}[D-Brane Algebras are $\Eone$; \ClaimStatusConjectured]\label{thm:dbrane-e1}
...
\begin{remark}[Scope]
Theorem~\ref{thm:dbrane-e1} requires string-theoretic input: the construction
...
\begin{theorem}[AGT Correspondence; \ClaimStatusProvedElsewhere]\label{thm:agt}
...
\begin{corollary}[Virasoro from Gauge Theory; \ClaimStatusProvedElsewhere]\label{cor:virasoro-gauge}
...
\begin{theorem}[$q$-AGT; \ClaimStatusConjectured]\label{thm:q-agt}
...
\begin{remark}[Scope]
Theorem~\ref{thm:q-agt} requires the 5d lift of the AGT correspondence,
...
as an $\Eone$-chiral algebra is mathematically well-defined (Feigin--Frenkel--Reshetikhin),
...
algebraic and geometric methods of this monograph.  Our framework provides
the correct target category ($\Eone$-chiral algebras) but not the
gauge-theoretic derivation.
\end{remark}
