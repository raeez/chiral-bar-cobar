...
We begin with the genus expansion of the MC equation and
proceed through the contracting homotopy, the genus spectral
sequence, comparison with the conformal bootstrap, explicit
computations through genus~$4$, and consequences for the genus
expansion.
...
$g_2 = 0$ (which each contribute $[\Theta^{(0)}, \Theta^{(g)}]$,
since by graded symmetry
$[\Theta^{(g)}, \Theta^{(0)}] = -(-1)^{1 \cdot 1}[\Theta^{(0)},
\Theta^{(g)}] = +[\Theta^{(0)}, \Theta^{(g)}]$
for odd-degree elements; the two orderings
give $2[\Theta^{(0)}, \Theta^{(g)}]$).
...
\end{align}
where we used $D_0^2 = 0$ in the first term, the derivation
property $D_0[a,b] = [D_0 a, b] + (-1)^{|a|}[a, D_0 b]$ in
the cross terms, the graded Jacobi identity
...
The explicit form of the MC
equation~\eqref{eq:thqg-VII-genus-g-mc-eq} at each genus
from $g = 0$ through $g = 4$ is as follows.
...
- \dpf^{(2)}.
\end{equation}
The right side decomposes into three shells:
\begin{enumerate}[label=\textup{(\roman*)}]
\item the \emph{loop-squared shell}
 $\Theta^{(2)}_{\mathrm{loop}^2} :=
 -\tfrac{1}{2}[\Theta^{(1)},\Theta^{(1)}]$,
 corresponding to iterated application of the BV operator
...
\item the \emph{separating-loop shell}
 ...
 corresponding to the boundary stratum where a genus-$2$
 surface degenerates through a separating node connecting
 a genus-$1$ and a genus-$1$ component, and where BV is
 applied to the genus-$1$ data;
...
= \tfrac{1}{2} \cdot 2\,[\Theta^{(1)},\Theta^{(2)}]
= [\Theta^{(1)},\Theta^{(2)}]$,
since $[\Theta^{(2)},\Theta^{(1)}] = +[\Theta^{(1)},
\Theta^{(2)}]$ (graded symmetry: both elements have odd
degree~$1$ in $\gAmod$, so the Koszul sign is
$-(-1)^{1 \cdot 1} = +1$).
\end{proof}
...
+ \tfrac{1}{2}[\Theta^{(2)},\Theta^{(2)}],
\end{align*}
using graded symmetry
$[\Theta^{(3)},\Theta^{(1)}]
= +[\Theta^{(1)},\Theta^{(3)}]$
(both elements have odd degree in $\gAmod$, so
$[a,b] = -(-1)^{|a||b|}[b,a] = +[b,a]$).
Note $[\Theta^{(2)},\Theta^{(2)}] \neq 0$ in general:
for odd-degree elements, the bracket is \emph{symmetric},
not antisymmetric (it vanishes only for Gaussian algebras).
\end{proof}
...
\end{equation}
This is the genus analogue of the obstruction tower in
...
Lemma~\ref{lem:degree-cutoff} ensures that the series terminates
...
(Theorem~\ref{thm:genus2-shell-activation}), the vanishing pattern
...
distinguishes between shadow-depth classes. At genus~$1$,
the MC element $\Theta^{(1)}$ is controlled entirely by the scalar
$\kappa$; all classes look identical at genus~$1$. At genus~$2$,
...
(Theorem~\ref{thm:genus2-shell-activation}). The $d_1$
...
The comparison is made precise below.
...
\label{thm:thqg-VII-crossing-as-mc}
...
is a consequence of the MC equation, not an additional axiom.
\end{corollary}
...
\label{rem:thqg-VII-higher-degree-crossing}
...
$d^2 = 0$ by the Arnold relation. The Koszul dual is
$\cH_k^! = \mathrm{Sym}^{\mathrm{ch}}(V^*)$ with curvature
$m_0 = -k\omega$, and $\kappa(\cH_k^!) = -k$.
...
\subsection{Implications for the genus expansion}
\label{subsec:thqg-VII-gravity-loops}
\index{quantum gravity!loop corrections from MC recursion|textbf}

The MC recursion has direct implications for the genus expansion and,
in HT language, admits a gravitational reading. The proved content of
this subsection is algebraic: genus-$g$ classes, integrated scalar
functionals, and their recursion/complexity properties. The literal
bulk-gravity interpretation remains a heuristic dictionary.

\subsubsection{Heuristic gravitational interpretation}
\label{subsubsec:thqg-VII-grav-interpretation}

In the HT setting, the genus expansion
$\Theta_\cA = \sum_g \hbar^g \Theta^{(g)}$ is often read as a
\emph{gravitational loop expansion}: heuristically one identifies the
parameter $\hbar$ with the gravitational coupling $G_N$
\textup{(}Newton's constant in the bulk\textup{)}, and interprets the
genus-$g$ term $\Theta^{(g)}$ as the algebraic shadow of a $g$-loop
bulk correction.
...
\end{tabular}
\end{center}
This is a heuristic dictionary, not a proved bulk reconstruction
theorem.
\end{principle}

\subsubsection{The genus-\texorpdfstring{$1$}{1} scalar term}
\label{subsubsec:thqg-VII-one-loop}

\begin{theorem}[Genus-\texorpdfstring{$1$}{1} scalar term from
 \texorpdfstring{$\Theta^{(1)}$}{Theta(1)};
\ClaimStatusProvedHere]
\label{thm:thqg-VII-one-loop-gravity}
\index{genus-1 scalar term}
The genus-$1$ MC element $\Theta^{(1)}$ determines the algebraic
genus-$1$ scalar contribution to the formal partition series:
...
moduli space $\overline{\cM}_{1,1}$). Equivalently,
\[
Z_1^{\mathrm{alg}}(\cA):=\exp\!\bigl(F_1^{\mathrm{alg}}(\cA)\bigr)
...
\begin{proof}
The genus-$1$ scalar contribution is
$F_1^{\mathrm{alg}}(\cA)=\kappa(\cA)\cdot
...
gives $F_1 = \kappa/24 = k/24$.
\end{proof}

\subsubsection{The integrated genus-\texorpdfstring{$g$}{g} functional}
\label{subsubsec:thqg-VII-g-loop}

\begin{theorem}[Integrated genus-\texorpdfstring{$g$}{g}
 MC functional;
\ClaimStatusProvedHere]
\label{thm:thqg-VII-g-loop-amplitude}
\index{genus-g integrated functional}
For a modular Koszul chiral algebra $\cA$, the genus-$g$
integrand class and its integrated scalar functional are
...
\mathcal{I}_g(\cA),
\end{equation}
where $\langle - \rangle$ denotes the evaluation pairing producing the
moduli-space class before integration. The integrated scalar
functional $\mathcal{A}_g(\cA)$ is:
\begin{enumerate}[label=\textup{(\roman*)}]
...
\begin{theorem}[Scalar genus expansion for Gaussian algebras;
\ClaimStatusProvedHere]
\label{thm:thqg-VII-non-renormalization}
\index{Gaussian algebra!scalar genus expansion}
For a Gaussian algebra $\cA$ \textup{(}$r_{\max} = 2$\textup{)},
the integrated genus expansion has the following scalar
form: the genus-$g$ functional
...
\subsubsection{Beyond the Gaussian: higher-degree complexity}
\label{subsubsec:thqg-VII-beyond-gaussian}

\begin{remark}[Higher-degree corrections from the shadow obstruction tower]
\label{rem:thqg-VII-grav-corrections}
\index{higher-degree corrections!shadow tower}
For non-Gaussian algebras ($r_{\max} \geq 3$), the MC recursion
produces genuine higher-degree genus corrections that are
...
\item For the mixed archetype ($r_{\max} = \infty$,
 ...
 genus-$g$ integrand class receives contributions
 from all degrees up to $g + 2$ (by the weight bound
 $w = 2g - 2 + r + d$).
\end{enumerate}
These corrections measure the algebraic complexity of the boundary
algebra: a larger shadow depth forces higher-degree contributions in
the genus expansion.
\end{remark}

\begin{proposition}[Complexity bounds on genus-\texorpdfstring{$g$}{g}
 integrand classes;
\ClaimStatusProvedHere]
\label{prop:thqg-VII-complexity-bounds}
\index{genus-g integrand class!complexity bounds}
For a modular Koszul chiral algebra $\cA$ with shadow depth
$r_{\max}(\cA) < \infty$, the genus-$g$ integrand class
$\mathcal{I}_g(\cA)$ is a polynomial of degree
...
factors. The moduli-space dependence contributes the
$H^*(\overline{\cM}_{g,n})$ factor before any final integration to a
scalar functional.
\end{proof}
...
heuristic determination of bulk amplitudes from the boundary OPE data.
The proved algebraic ingredients are:
...
\item $r_{\max} = 2$ \textup{(}Gaussian\textup{)}: the bulk
 gravity is \emph{free at the degree level}: the higher-degree
 shadow obstruction tower vanishes, and the genus
 spectral sequence collapses at $E_1$. Example: Heisenberg
 $\cH_k$ (free boson in the bulk). On the proved uniform-weight lane
 (Definition~\ref{def:scalar-lane}) this further gives a scalar
 Fredholm determinant controlled by~$\kappa$.
...
(Theorem~\ref{thm:genus2-shell-activation})
...
(Lemma~\ref{lem:degree-cutoff}) bounds the number of
