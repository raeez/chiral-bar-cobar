...
\label{v1-chap:dg-shifted-factorization}
...
A dg-shifted Yangian is not a strict factorization quantum group.
It canonically produces a \emph{spectral quasi-factorization quantum
group}, and the passage from quasi to strict is governed by a concrete
obstruction complex whose first low-filtration sectors can be computed
exactly.

The bridge between these three languages cannot be built by imposing a
strict $R$-matrix directly on the $A_\infty$ object: the higher
operations obstruct any naive strictification. The correct route
passes first to ordered configuration spaces, constructs a flat
connection from the bar-horizontal data, reads off regularized
holonomy, and only then formulates the spectral factorization problem.
The passage from dg-shifted Yangian to spectral quasi-factorization
quantum group is canonical; the subsequent strictification is governed
by a concrete filtered cohomology theory whose low-order sectors
(Cartan, commuting, adjacent-root triangle, root quadrilateral) can be
computed exactly. The naive ``shift-invariance'' slogan requires
correction at mixed-root level: the shift changes local transport
coefficients by explicit ratios of shift characters, but this
sensitivity is entirely forced and does not produce new independent
obstruction classes in filtration~$2$ or~$3$.

\begin{remark}[Yangian as the ordered boundary face]
\label{rem:yangian-ordered-boundary-face}
\index{dg-shifted Yangian!as ordered face of universal twisting}
\index{jet principle|textbf}
The dg-shifted Yangian MC kernel $r_\cA(z) \in Y \otimes Y((z^{-1}))$
(Definition~\ref{def:dg-shifted-yangian} below) is not a
separate object. It is the restriction to degree~$2$ of the
universal twisting morphism
$\tau_\cA\colon \hat{B}(\cA) \to \cA^!$, the ordered $E_1$
boundary face of the modular cumulant transform
$M(\cA) = (\hat{T}^c(sQ(\cA)^\vee), D_\cA, \tau_\cA,
r_\cA(z), \Theta_\cA)$
(Equation~\eqref{eq:modular-cumulant-transform} in the
concordance). Expanding $r_\cA(z) = \sum_{m \geq 0}
r_m\, z^{-m-1}$ yields a jet hierarchy:
\begin{align*}
m_1(r_0) &= 0, \\
m_1(r_1) + m_2(r_0, r_0) &= 0, \\
m_1(r_2) + m_2(r_0, r_1) + m_2(r_1, r_0) + m_3(r_0, r_0, r_0) &= 0,
\end{align*}
and so on. The \emph{jet principle}
(Conjecture~\ref{conj:jet-principle}) predicts that
reduced-weight-$q$ bar windows $K_q(\cA)$
(Definition~\ref{def:finite-window}) determine Yangian jets
through order $z^{-q}$. This turns the Yangian frontier from an
all-at-once completed monster into a finite jet tower, checkable
window by window against the proved finite-stage bar-cobar data.
For $\mathfrak{g}$-coloured loop towers with $d = \dim\mathfrak{g}$,
the window sizes grow as $h_q = d(d+1)^{q-1}$
...
\begin{corollary}[Derived Drinfeld--Kohno shadow]
\ClaimStatusProvedHere
...
\begin{theorem}[Boundary residue theorem]
\ClaimStatusProvedHere
...
\begin{proposition}[Quadratic hexagon obstruction]
\ClaimStatusProvedHere
...
\begin{proposition}[Three-particle obstruction identity]
\ClaimStatusProvedHere
...
\begin{theorem}[Abelian strictification theorem]
\ClaimStatusProvedHere
...
\begin{theorem}[Residue-bounded completion theorem]
\ClaimStatusProvedHere
...
\begin{proposition}[Cartan shift cocycle identities]
\ClaimStatusProvedHere
...
\begin{theorem}[Cartan gauge-twist theorem]
\ClaimStatusProvedHere
...
\begin{theorem}[Cartan-diagonal shift defect is exact]
\ClaimStatusProvedHere
...
\begin{theorem}[Triangle localization formula]
\ClaimStatusProvedHere
...
\begin{corollary}[Normalized rational triangle coefficient]
\ClaimStatusProvedHere
...
\begin{proposition}[Triangle shift transport law]
\ClaimStatusProvedHere
...
\begin{lemma}[Closed generating kernel]
\ClaimStatusProvedHere
...
\begin{theorem}[Exact free two-body transport]
\ClaimStatusProvedHere
...
The first genuinely class-$3$ computation follows.
...
\begin{theorem}[Quadrilateral localization formula]
\ClaimStatusProvedHere
...
\begin{theorem}[Quadrilateral rigidity]
\ClaimStatusProvedHere
...
\begin{corollary}[Normalized rational quadrilateral coefficient]
\ClaimStatusProvedHere
...
\begin{proposition}[Quadrilateral shift transport law]
\ClaimStatusProvedHere
...
\begin{corollary}[No new independent shift obstruction in filtration $3$]
\ClaimStatusProvedHere
...
\begin{remark}[First place the frontier can move]
...
\begin{enumerate}
\item The first mixed-root triangle coefficient is $\frac12$ in the
 normalized rational ansatz (all root coefficients $c_\alpha = 1$).
\item The first root-quadrilateral coefficient is $\frac13$ in the
 normalized rational ansatz.
...
which a new obstruction can appear.
