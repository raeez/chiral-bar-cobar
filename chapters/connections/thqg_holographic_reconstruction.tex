...
eventually vanishes. The proof is a formal consequence of the
...
obstruction classes, obstruction towers),
...
\begin{proof}
\emph{Part~(i): finite shadow depth implies stabilization.}
...
\medskip
\emph{Part~(ii): infinite shadow depth implies non-stabilization.}
...
provides a canonical section. Surjectivity of
transition maps is the Mittag--Leffler condition
for countable inverse systems.
...
need to use only bivalent vertices (from $m_2$), but a
connected graph on bivalent vertices alone is either a
...
The transferred ternary operation $m_3^{\mathrm{tr}}$
for the Kac--Moody algebra vanishes.
The reason: the Kac--Moody OPE has poles of order at
most~$2$, and the $(1)$-product
$J^a_{(1)}J^b = k\kappa^{ab}\mathbf{1}$ is a scalar.
In the homological perturbation lemma, the transferred
$m_3^{\mathrm{tr}}$ involves the composition
$m_2 \circ (1 \otimes h) \circ m_2$ where $h$ is the
homotopy. The inner $m_2$ produces either a Lie bracket
(from the $(0)$-product) or a scalar (from the
$(1)$-product). When it produces a scalar, the outer
$m_2$ acts on $(\text{element}) \otimes \mathbf{1}$, which
gives zero in the reduced bar complex. When it produces
a Lie bracket $[J^a, J^b] = f^{ab}{}_c J^c$, the homotopy~$h$
maps this back to a bar element, and the outer $m_2$
acts. The resulting expression is the triple bracket
$[J^a, [J^b, J^c]] - [J^b, [J^a, J^c]]$, which by
the Jacobi identity equals $[[J^a, J^b], J^c]$. But
this is already the tree-level contribution from the
boundary bracket, so the local quartic vertex is absorbed
by the tree graph.

More precisely: the transferred $m_3^{\mathrm{tr}}$ is
related to the $L_\infty$ bracket $\ell_3$ by the Koszul
sign rule. For Lie algebras, the identity
\begin{equation}\label{eq:ell3-jacobi-vanishing}
\ell_3(a, b, c) = [a, [b, c]] - [[a,b], c] - (-1)^{|a||b|}[b,[a,c]]
= 0
\end{equation}
(which is the Jacobi identity) shows $\ell_3 = 0$, and
consequently the transferred cubic-cubic Poisson bracket
is exact.
...
The vanishing $o_4 = 0$ is forced by Koszulness of $\Lie$:
the bar resolution of any Lie algebra is formal, so
there are no higher obstructions. The shadow obstruction tower
terminates at arity~$3$ because $\Lie$ is a Koszul operad
and all compositions of Lie brackets are algebraically
determined by the bracket itself.
\end{remark}
...
This vanishing is the \emph{rank-one abelian rigidity}:
on a one-dimensional primary space, the cyclic
inner product $\langle \eta, m_3(\eta,\eta,\eta)\rangle$
vanishes because the cubic operation~$m_3$ on a single
generator is forced to be a scalar multiple of the
generator, and the cyclic symmetry of the inner product
...
where the Virasoro algebra degenerates). Therefore
...
Theorem~\ref{thm:operadic-complexity} states that
...
current THQG dictionary, this means that the same depth
invariant measures the highest transferred arity visible
from the boundary deformation package.
\end{remark}
...
\subsection{Synthesis: reconstruction of the modular
Maurer--Cartan class from obstruction data}
\label{subsec:holographic-synthesis}
\index{Maurer--Cartan class!reconstruction from obstruction data}

\begin{theorem}[Finite-order shadow reconstruction theorem;
\ClaimStatusProvedHere]
\label{thm:holographic-reconstruction}
\index{shadow reconstruction theorem|textbf}
Let $\cA$ be a modular Koszul chiral algebra. The full modular
Maurer--Cartan class $\Theta_\cA$ is determined by the
...
Input: $\kappa(\widehat{\fg}_k) = (k+h^\vee)\dim(\fg)/(2h^\vee)$,
$\mathfrak{C}_{\widehat{\fg}} = f^{ab}{}_c\,x_a x_b x^c$.
Output:
$\Theta_{\widehat{\fg}_k} = \bigl((k+h^\vee)\dim\fg/(2h^\vee)\bigr)\,\kappa_{ab}x^a x^b
+ f^{ab}{}_c x_a x_b x^c$.
...
The holographic reconstruction problem, that of determining the
...
$S_{r+1} = -6r/(c(r+1))\,S_r$
\textup{(}the cubic-source approximation, exact for
$r \leq 5$\textup{)} makes the infinite tower
tractable. The convergence for $|c| > 6$ and
...
these results a THQG dictionary: the shadow depth of the
boundary algebra determines the maximal transferred genus-$0$
arity in the modular deformation package. Gaussian
boundaries give only the quadratic shadow, Lie/tree
boundaries add a cubic term and stop, contact boundaries
have vanishing cubic but nonzero quartic term, and mixed
boundaries carry an infinite shadow obstruction tower.
