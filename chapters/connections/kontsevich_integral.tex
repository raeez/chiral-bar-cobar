...
What happens when the bar complex of an affine Kac--Moody algebra
is evaluated not on a complex algebraic curve but on a real circle?
The propagator $\omega_{ij} = d\log(z_i - z_j)$, which drives
the entire chiral bar-cobar machinery, restricts on the unit circle
$S^1 \subset \bC$ to $(1/2\pi)\, d\arg(t_i - t_j)$ plus exact
corrections.  This is precisely the Kontsevich propagator, and its
integrals over configuration spaces of circles produce universal
Vassiliev invariants.  The bar complex weight systems of
Proposition~\ref{prop:vassiliev-genus0} are the genus-$0$ seed
of this connection; the full genus tower should produce the loop
expansion of the Kontsevich integral.  This chapter develops the
mathematical content of this observation.

\section{Vassiliev invariants: mathematical setup}
\label{sec:vassiliev-setup}
\index{Vassiliev invariants!mathematical setup}

We recall the essential definitions from the theory of finite-type
knot invariants, following Bar-Natan~\cite{BarNatan95}.
...
The connection between bar-cobar duality and knot invariants rests
on the following observation: the bar differential extracts exactly
the data that defines a weight system.
...
The holomorphic propagator on a complex curve restricts to the
Kontsevich propagator on the real locus.
...
The deepest connection between the bar complex and knot theory runs
through the Knizhnik--Zamolodchikov equation.  The bar complex
on~$C_3(\bC)$ carries a flat connection whose monodromy is the
Drinfeld associator.
...
The genus-$0$ results extend to a conjectural identification
of the full genus tower with the loop expansion of the
Kontsevich integral.
...
The results above establish that the bar complex
produces weight systems and KZ monodromy at genus~$0$,
and that the genus tower identifies with the Feynman transform.
The remaining gap between the bar complex and the full Kontsevich
integral is the passage from holomorphic to topological:
\begin{enumerate}
...
The correct mathematical framework connecting holomorphic bar
complexes on curves to topological invariants of knots is
