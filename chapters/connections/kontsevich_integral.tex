...
The propagator $\omega_{ij} = d\log(z_i - z_j)$ restricts on $S^1 \subset \bC$ to $(1/2\pi)\, d\arg(t_i - t_j)$ plus exact corrections --- the Kontsevich propagator.  Its integrals over configuration spaces of circles produce universal Vassiliev invariants.  The genus-$0$ bar complex weight systems (Proposition~\ref{prop:vassiliev-genus0}) seed this connection; the full genus tower should produce the loop expansion of the Kontsevich integral.
...
Chern--Simons gauge theory provides the bridge from holomorphic bar complexes to topological knot invariants.
