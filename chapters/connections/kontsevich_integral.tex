...
\begin{remark}[Programme status]\label{rem:programme-kontsevich}
This chapter develops the connection between bar complex propagators
and Vassiliev invariants of knots.  The genus-$0$ weight system
identification (Proposition~\ref{prop:vassiliev-genus0}) is fully proved.
The conjecture that the full genus-$g$ tower produces the loop expansion
of the Kontsevich integral (\ClaimStatusConjectured) requires verification
that higher-genus propagators on configuration spaces match
the Kontsevich propagator asymptotics; this remains open and is
downstream of Conjecture~\ref{conj:master-bv-brst} (MC5).
Each claim carries its status tag.
\end{remark}

The chapter is organized as a strict theorem/conjecture split.
At genus~$0$, the restriction
$\omega_{ij}=d\log(z_i-z_j)\mapsto \omega^K_{ij}$ and the associated
weight-system map are proved
\textup{(}Proposition~\ref{prop:propagator-restriction},
Theorem~\ref{thm:bar-weight-systems}\textup{)}.
The extension from this genus-$0$ shadow to the full loop expansion of
the Kontsevich integral is conjectural and tracked as an MC5
downstream item.

The governing question is exactly where the genus-$0$ proved shadow ends and
the full loop-expansion bridge to Kontsevich theory begins.

\begin{remark}[Semantic levels; Convention~\ref{conv:hms-levels}]
The weight system identification and Kontsevich propagator
restriction are \textbf{M-level} (explicit configuration space
integral constructions).  Vassiliev invariant values and chord
diagram evaluations are \textbf{S-level} numerical consequences.
\end{remark}
...
\item For each chord diagram $\Gamma$ of degree~$n$, the bar
  ...
  \]
  where $\omega_{ij} = d\log(z_i - z_j)$ is the FM propagator on
...
(Theorem~\ref{thm:bar-differential-structure}): at genus~$0$, the
bar differential is $d = \sum_{i<j} \Res_{D_{ij}}(\omega_{ij}
...
+ \omega_{ki}\omega_{ij} = 0$ implies the same linear combination
...
by $z = e^{i\theta}$.  The FM propagator $\omega_{ij} = d\log(z_i-z_j)$
...
\omega^K_{ij}
\]
where $\omega^K_{ij} = \frac{1}{2\pi}\, d\arg(t_i - t_j)$ is
...
\]
The real part of $\omega_{ij}|_{S^1} = d\log(z_i-z_j)|_{S^1}$
splits as
$\Re(\omega_{ij}) = d\log|z_i-z_j| = d\log|e^{i\theta_i}-e^{i\theta_j}|
...
$\Im(\omega_{ij})|_{S^1} = d\arg(z_i-z_j)$, which is the Kontsevich
...
$\omega^K_{ij} = (1/(2\pi i))\, d\log(z_i - z_j)$
...
(Theorem~\ref{thm:bar-differential-structure}) is
$d = \sum_{i<j} \Res_{D_{ij}} (\omega_{ij} \wedge -)$,
where $\omega_{ij} = d\log(z_i-z_j)$ and $\Res_{D_{ij}}$
...
$d = t_{12}\, \omega_{12} + t_{13}\, \omega_{13} + t_{23}\, \omega_{23}$
where $\omega_{ij} = d\log(z_i - z_j)$ and
...
\begin{proposition}[Feynman transform produces graph complex; \ClaimStatusOpen]
...
\item The passage from holomorphic propagators on complex curves
  ...
  Remark~\ref{rem:holomorphic-to-topological}.
\end{enumerate}
\end{proposition}
...
\noindent\emph{Status split.} This section is entirely
\ClaimStatusConjectured and records the precise interface required to
upgrade the proved genus-$0$ shadow to the full loop expansion.
...
The conjecture identifies the bar complex of $\widehat{\fg}_k$ with the perturbative Chern--Simons factorization algebra, computing knot invariants via bar cohomology with module insertions.
