...
(proved on the uniform-weight lane at all genera; at genus~$1$ unconditionally)
is the trace of this transform, the degree-zero projection
...
$\Theta_\cA$
(Theorem~\ref{thm:mc2-bar-intrinsic}): on the uniform-weight lane, $\Theta_\cA^{\min} = \kappa(\cA) \cdot \eta \otimes \Lambda$ (Theorem~\ref{thm:explicit-theta}), giving
$m_0^{(g)} = \kappa(\cA) \cdot \lambda_g$, where $\lambda_g$ is
the Hodge class on $\overline{\mathcal{M}}_g$.  For multi-weight algebras at $g \geq 2$, the identification $\Gamma_\cA = \kappa(\cA)\Lambda$ remains open (Problem~\ref{op:multi-generator-universality}).  The full MC equation
