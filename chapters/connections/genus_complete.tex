...
The identity $\Dg{g}^{\,2} = 0$ for the total differential at the convolution level is equivalent to the Maurer--Cartan equation for the curved $A_\infty$ structure, which is proved by genus induction in Theorem~\ref{thm:genus-induction-strict}.  (At the ambient level with planted-forest corrections, $D^2 = 0$ is Theorem~\ref{conj:differential-square-zero}.)
