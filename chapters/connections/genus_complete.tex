...
The governing question of this chapter is how far the complete-genus bridge extends the printed higher-genus core, and exactly where it crosses from theorematic comparison into programme.
\end{remark}
...
\end{remark}
\label{rem:programme-genus-complete}
...
corresponding filtered H-level bulk target and matching its stagewise
local coefficients with the theorematic $\mathcal{W}_\infty$/Yangian
packets belongs to MC4.  The amplitude/bar identification built on that
target is the downstream MC5 step.

\emph{Homotopy template} (Convention~\ref{conv:hms-levels}): Type~VII (physics-dictionary).
The conjecture treats bar complex cohomology as conjectural algebraic
shadow data for string scattering amplitudes at all genera, extending
the proved genus-$0$ case to the full string perturbation series only
after additional physics input.
(Contributing to Conjecture~\ref{conj:master-bv-brst}.)
\end{remark}

\begin{remark}[Physical interpretation]
Each term in the recursion matches the shape of a Feynman diagram, with
vertices given by chiral algebra elements (local operators), edges by
propagators (Green's functions on $\Sigma_g$), and loops by genus
contributions (quantum corrections).  In this physics template, the bar
differential plays the algebraic role expected of the BRST operator of
string theory.
\end{remark}

\begin{conjecture}[Holographic duality via bar-cobar; \ClaimStatusConjectured]
Conjecturally, the bar-cobar duality supplies the algebraic template for AdS/CFT:
\begin{center}
\begin{tabular}{|c|c|}
\hline
...
The conjectural comparison takes the form:
$$Z_{\text{CFT}}[\mathcal{J}] = Z_{\text{gravity}}[\phi_0 = \mathcal{J}]$$
where the bar complex is conjectured to model the gravity path integral.
...
\begin{remark}[Scope]
The holographic duality corollary treats bar-cobar duality as an
algebraic template for AdS$_3$/CFT$_2$.  The mathematical content --- that the bar
...
holographic target, when required, belongs upstream to the exact MC4
package of filtered targets, coefficient identities, and finite
detection, and the
AdS/CFT dictionary itself is the downstream MC5 comparison problem.

\emph{Homotopy template} (Convention~\ref{conv:hms-levels}): Type~VII (physics-dictionary).
The conjecture treats bar-cobar duality as a conjectural algebraic
shadow of the AdS$_3$/CFT$_2$ correspondence, with the genus expansion
providing candidate $1/N$ data for the holographic dual only after
