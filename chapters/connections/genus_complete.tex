...
The four projections of the Chern--Weil transform (eq.~\eqref{eq:cw-scalar}--\eqref{eq:cw-line}) correspond to successive K-theoretic invariants of the modular tangent complex $T^{\mathrm{mod}}_{\Theta_\cA}$ (Construction~\ref{const:vol1-modular-tangent-complex}):
\begin{center}
\renewcommand{\arraystretch}{1.2}
\begin{tabular}{lll}
\textbf{K-theoretic invariant} & \textbf{CW projection} & \textbf{Shadow} \\
\hline
First Chern class $c_1$ & Scalar trace & $\kappa(\cA)$ \\
Chern character $\operatorname{ch}$ & Spectral determinant & $\Delta_\cA(x)$ \\
Adams operations $\psi^k$ & Resonance classes & $\mathfrak{R}^{\mathrm{mod}}_{r,\bullet}$ \\
Holonomy representation & Line kernel & $r_\cA(z)$
\end{tabular}
\end{center}
The scalar projection $\kappa = c_1(\det T^{\mathrm{mod}})$ is the first Chern class of the determinant line bundle of the modular functor.  The spectral projection $\Delta_\cA = \operatorname{sdet}(1 - t \cdot H(d_{\Theta_\cA}|_{\mathrm{red}}))$ is the Chern character (eq.~\eqref{eq:delta-chern-character}).  The resonance classes at arity $r \geq 4$ are the nonlinear characteristic classes: the first genuinely new invariant beyond the classical Chern--Weil homomorphism.
\end{remark}
...
\begin{remark}[Stokes data and the Givental $R$-matrix]
\label{rem:stokes-r-matrix}
\index{Stokes groupoid!Givental R-matrix}
\index{Givental--Teleman reconstruction!Stokes data}
The complementarity Stokes groupoid $\operatorname{Sto}_{\mathrm{comp}}(\cA)$ and the Givental $R$-matrix $R_\cA(z)$ (Theorem~\ref{thm:cohft-reconstruction}) encode the same data in different coordinates.  The $R$-matrix coefficients $P_\cA^{(k)}$ are the Taylor coefficients of the complementarity propagator, while the Stokes multipliers of the shadow ODE are the exponentials of the monodromy of the flat connection $\nabla_H = d - \kappa(\cA) \cdot \omega$ on the Legendrian $\Lambda_\cA$.  For class-$\mathbf{G}$ algebras, both are trivial ($R_\cA = 1$, $\operatorname{Sto} = \{1\}$).  For class-$\mathbf{M}$ algebras, the Stokes phenomenon of the shadow ODE (the irregular singularity of the shadow tower at $c = 0$ and $c = -22/5$ for Virasoro) corresponds to the wall-crossing of the Givental action at the non-semisimple locus of the genus-$0$ Frobenius manifold.
\end{remark}
...
\[
\begin{array}{c|cc|c}
\cA & \dim H_n & |C| & \sum_n q^n\dim H_n
\\[2pt]\hline
\rule{0pt}{12pt}
\mathcal{H}_k & p(n) & \leq k
  & q^{-1/24}/\eta(q) \\
V_k(\fg) & \leq Ce^{\alpha\sqrt{n}}
  & \leq K(n{+}1)^{h^\vee}
  & \chi_{V_k}(q) \\
\mathrm{Vir}_c & p(n) & \leq K(n{+}1)^4
  & q^{(1-c)/24}/\eta(q) \\
\mathcal{W}_N & \leq Ce^{\alpha\sqrt{n}}
  & \leq K(n{+}1)^{2N} & \chi_{\mathcal{W}_N}(q) \\
...
The sewing operator $T_q$ on $A^2(\partial D)$ has
eigenvalues~$q^n$ and is trace class:
$\|T_q\|_1 = (1-q)^{-1}$.
...
The poles of $L_\cA$ from $\zeta(v{+}1)$ in $S_\cA$
occur at $v = \rho/2$ for each nontrivial zero~$\rho$
of~$\zeta$, tracing the line $s + u = 1 + \rho/2$
in the $(s,u)$-plane.
\item
...
Higgs field $\theta_\cA$ on the nonabelian Hodge triple, and the
