...
Let $W(\cA) = \{w_1, \ldots, w_r\}$ be the conformal-weight
multiset of the strong generators of~$\cA$.  Then
...
\qquad i, j \geq 0,
\end{equation}
is an infinite Hankel matrix.  Write
$\mu_k(\cA;\alpha) :=
...
$\mu_k(\cH; 2) > 0$ and $\mu_k(\mathrm{Vir}; 2) > 0$
for $k = 1, 2, 3, 4$.  For all~$k$:
\begin{equation}\label{eq:surface-minor-ratio}
\frac{\mu_k(\mathrm{Vir};\alpha)}{\mu_k(\cH;\alpha)}
\;=\;
\det\Bigl(
\bigl(1 - \zeta(\alpha{+}i{+}j)^{-1}\bigr)
\cdot \mathscr{M}_\cH(\alpha)_{ij}
\,/\,
\mathscr{M}_\cH(\alpha)_{ij}
\Bigr)_{0 \leq i,j < k},
\end{equation}
...
\begin{proof}
Positivity: numerical verification.
The ratio uses $S_{\mathrm{Vir}}(u) =
(1 - 1/\zeta(u))\cdot S_\cH(u)$, giving the entry-wise
factorization.  Since $1 - 1/\zeta(n) < 1$ for $n \geq 2$,
the ratio of determinants decreases geometrically.
\end{proof}
...
\varepsilon^1_s(R{=}1)
= 4\,\zeta(2s)
= \frac{4}{\zeta(2s{+}1)}\cdot S_\cH(2s).
\end{equation}
The functional-equation factor
$\zeta(2s)/\zeta(2s{-}1)$ has poles at $s = (1{+}\rho)/2$
for each nontrivial zero~$\rho$;
these are the spectral-side counterparts of the prime-side
poles of $D'_{\mathrm{Vir}}/D_{\mathrm{Vir}}$
\textup{(}Proposition~\textup{\ref{prop:zeta-zeros-defect-derivative}}\textup{)}.
\end{proposition}
...
$\operatorname{ek}(\cA)$ depends only on the character
of~$\cA$; $\kappa_d(\cA)$ depends on the OPE.
\end{corollary}

\begin{proof}
$\operatorname{ek} = \max_i(w_i - 1)$ is determined by the
weight multiset, hence by the graded character.
$\kappa_d$ depends on higher $A_\infty$-brackets
(Theorem~\ref{thm:shadow-archetype-classification}), which
require OPE data: $\beta\gamma$ and $\cH \oplus \cH$ share
$\operatorname{ek} = 0$ and the same character grading but
differ in $\kappa_d$ ($4$ vs.\ $2$).
\end{proof}
