...
onto the determinant line bundle of the bar family.  The full transform carries
...
the determinant line bundle of the bar family, computed via
...
Passing to $H^*(V_{g,n})$ gives the classical cohomological modular-functor package~\cite{BK01}; for $\widehat{\mathfrak{g}}_k$ at integrable level, $H^0(V_{g,n})$ is the Tsuchiya--Ueno--Yamada space of conformal blocks.  The chain-level refinement is necessary at generic~$k$ (multi-degree bar complex) and when $\kappa \neq 0$ (curvature $m_0^{(g)} = \kappa \cdot \lambda_g$; Theorem~\ref{thm:genus-graded-bar}).
\end{remark}
...
defect algebra, algebraic line-side model, genus-zero $r$-matrix, modular
...
analytic sewing data.  The analytic completion is the
first place where formal shadow invariants can interact with
unitarity and partition-function data.
\end{remark}
