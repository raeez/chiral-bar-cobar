...
\begin{proof}
(i)~$H_{N-1} = \log(N{-}1) + \gamma + O(1/N)$, so
$\frac{N}{N{-}1}H_{N-1} = \log N + \gamma + O(1/N)$.
...
\end{equation}
where $\operatorname{Sh}_r(\Theta_\cA^{\leq r};\,s,t)$
is the Mellin transform in the $q$-variable of the arity-$r$
shadow projection of $\Theta_\cA^{\leq r}$
(Definition~\ref{def:shadow-algebra}),
evaluated at spectral parameters $(s,t)$.
At $R = 2$: $K_\cA^{\leq 2} = K_\cA$ (the free-field kernel
shifted by~$\kappa$).
At $R = 4$: the quartic resonance class
$\mathfrak{Q}(\cA)$ enters for the first time.
\end{construction}
...
$\Theta_\cA^{(1),\mathrm{conn}}(\tau)$
the connected genus-$1$ shadow of the MC element
(the $\log$-level projection of the genus-$1$ partition
function, carrying graph-connected contributions only).
...
y^{u-2}\,dx\,dy,
\end{equation}
where $\cF = \mathrm{SL}(2,\bZ)\backslash\cH$ is the
modular fundamental domain and the regularization is
Zagier truncation
$\cF^T := \{\tau \in \cF : \mathrm{Im}(\tau) \leq T\}$,
$T \to \infty$.
\end{definition}

\begin{remark}[Scalar slices of $L_\cA$]%
\label{rem:L-object-slices}%
\index{L-object@$L$-object!scalar slices}%
The two variables separate the zero-side and prime-side data:
\begin{enumerate}[label=\textup{(\roman*)}]
\item
\emph{Scalar $s$-slice}: the zero-side
Rankin--Selberg/Eisenstein interface.
At this level, the zeta-zero poles appear at
$s = (1{+}\rho)/2$ for each nontrivial zero~$\rho$
of~$\zeta$, and the oscillatory coefficients
$\varepsilon_c(\mu)$, $\delta_{k,c}(\mu)$ are modular
overlaps
(Benjamin--Chang~\cite{BenjaminChang22}).
\item
\emph{Scalar $u$-slice}: the prime-side
connected sewing lifts $S_\cA(u)$
\textup{(}Definition~\textup{\ref{def:dirichlet-sewing-lift}}\textup{)}.
\item
\emph{First non-scalar correction}: the spectral
discriminant $\Delta_\cA$.
\item
\emph{First nonlinear correction}: the quartic
resonance class $\mathfrak{Q}(\cA)$.
\end{enumerate}
The full characteristic package is therefore
$(\Theta_\cA,\;\kappa(\cA),\;\Delta_\cA,\;S_\cA,\;
\Pi_\cA,\;H_\cA)$, with $S_\cA$ the arithmetic face
and $L_\cA(s,u)$ the unified receptacle.
The six components have sharp
roles: $\kappa(\cA)$ controls the scalar modular anomaly;
$\Delta_\cA$ controls the spectral sheet;
$S_\cA$ controls connected arithmetic sewing;
$\Theta_\cA$ unifies them as a connected modular homotopy
object; modular curves expose the zeta-zero poles;
...
Distinct families can share the same spectral sheet
while having different arithmetic sewing shadows.
For example, $\widehat{\mathfrak{sl}}_2$, $\mathrm{Vir}_c$,
and $\beta\gamma$ all sit on the discriminant
$\Delta_\cA(x) = (1{-}3x)(1{+}x)$, yet their
Dirichlet--sewing lifts $S_\cA(u)$ are immediately
distinguishable:
$S_{\beta\gamma} = 2\zeta(u)\zeta(u{+}1)$
(exact Euler--Koszul),
$S_{\mathrm{Vir}} = \zeta(u{+}1)(\zeta(u){-}1)$
(finitely defective),
$S_{\widehat{\mathfrak{sl}}_2}
= 3\zeta(u)\zeta(u{+}1)$
(exact Euler--Koszul, dimension~$3$).
\end{remark}
