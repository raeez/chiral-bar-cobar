...
\begin{remark}[Programme status]\label{rem:programme-genus-complete}
This chapter synthesizes the complete genus-$g$ theory for chiral algebras,
from flat structures (genus~$0$) through curved $A_\infty$ (all genera).
Results range from fully proved theorems (\ClaimStatusProvedHere,
\ClaimStatusProvedElsewhere) to conjectures requiring higher-genus
geometric input (\ClaimStatusConjectured).  Each statement carries its
status tag.  Conjectures in this chapter are
downstream of Conjecture~\ref{conj:master-theta} (MC2).
\end{remark}

\emph{Differential conventions.}
Throughout this chapter, we use the differential notation
of Convention~\ref{conv:higher-genus-differentials}:
$\dfib$ for the fiberwise curved differential ($\dfib^{\,2} = \kappa \cdot \omega_g$),
$\Dg{g}$ for the total strict differential ($\Dg{g}^{\,2} = 0$),
and $\dzero$ for the genus-$0$ collision differential.

\begin{remark}[Semantic levels; Convention~\ref{conv:hms-levels}]
The genus-$g$ bar complex, elliptic propagators, and curved
$A_\infty$ relations are \textbf{M-level} (explicit chain models on
configuration spaces).  Eynard--Orantin recursion data and
genus-expansion generating functions are \textbf{S-level} shadows.
The conjectured universal $\Theta_{\cA}$ (MC2) is an H-level target.
\end{remark}

\noindent\emph{Status split for this chapter.} The elliptic and
higher-genus chain constructions are theorematic as tagged below
\textup{(}\ClaimStatusProvedHere/\ClaimStatusProvedElsewhere\textup{)};
string and holographic identifications in
\S\ref{subsec:physics} remain \ClaimStatusConjectured.

The governing question is how far explicit genus-$g$ chain models carry the
theory before one reaches the physics-horizon conjectural layer.

\subsection{Genus one extensions}\label{subsec:elliptic}

\begin{remark}[Elliptic geometry at genus one]
At genus one, the torus $\mathbb{T} = \mathbb{C}/(\mathbb{Z} + \tau\mathbb{Z})$ introduces double periodicity, $SL_2(\mathbb{Z})$-invariance, and theta functions as the natural basis for sections.
\end{remark}
...
The genus-one theory is controlled by a single invariant: the central charge $c$.  It determines the modular properties at genus one (any $c$ can extend; $c = 0, 15, 26$ are distinguished by anomaly cancellation in string theory), the modular anomaly ($c/24$ in transformation laws), and the vacuum energy shift ($-c/24$ in the partition function).
\end{remark}
...
The chain-level refinement is necessary for two reasons.  At generic~$k$, the bar complex does not reduce to a single cohomological degree, so the modular functor exists only at chain level.  When $\kappa \neq 0$, the curvature $m_0^{(g)} = \kappa \cdot \lambda_g$ prevents $d^2 = 0$ in the uncompleted complex; one must work with $\bar{B}^{\mathrm{total}}(\mathcal{A})[[\hbar]]$ (Theorem~\ref{thm:genus-graded-bar}).
\end{remark}
...
\begin{theorem}[Eynard--Orantin recursion for bar complex; \ClaimStatusOpen]\label{thm:EO-recursion}
...
\noindent\emph{Status split.} This subsection is a conjectural horizon:
it records precise physics-facing targets compatible with the proved
genus tower but does not supply new proofs.
...
(Conjecture~\ref{conj:anomaly-physical}).  The current theorematic
input therefore stops at the bar/semi-infinite description and the
same-family shadow data on the boundary side; identifying the
corresponding H-level bulk or infinite-generator dual object remains on
the MC4/MC5 frontier.

\emph{Homotopy template} (Convention~\ref{conv:hms-levels}): Type~VII (physics-dictionary).
The conjecture identifies bar complex cohomology with string scattering amplitudes at all genera, extending the proved genus-$0$ case to the full string perturbation series.
(Contributing to Conjecture~\ref{conj:master-bv-brst}.)
\end{remark}

\begin{remark}[Physical interpretation]
Each term in the recursion corresponds to a Feynman diagram, with vertices given by chiral algebra elements (local operators), edges by propagators (Green's functions on $\Sigma_g$), and loops by genus contributions (quantum corrections).  Under this correspondence, the bar differential is identified with the BRST operator of string theory.
\end{remark}
...
The genus expansion provides the $1/N$ expansion: genus $0$ is the large-$N$ limit (classical gravity), genus $1$ gives $1/N$ corrections (one-loop quantum gravity), and genus $g$ gives $1/N^{2g}$ corrections.
\end{conjecture}
...
scope of this monograph.  Accordingly, the current theorematic boundary
input should be read only as bar/semi-infinite data together with the
same-family shadow partner information; the H-level infinite-generator
holographic dual remains part of the MC4/MC5 frontier.

\emph{Homotopy template} (Convention~\ref{conv:hms-levels}): Type~VII (physics-dictionary).
The conjecture identifies bar-cobar duality with the AdS$_3$/CFT$_2$ correspondence, with the genus expansion providing the $1/N$ expansion of the holographic dual.
(Contributing to Conjecture~\ref{conj:master-bv-brst}.)
\end{remark}
...
The genus expansion stratifies algebraic structure by topological complexity: genus $0$ gives tree-level operations, genus $1$ introduces modular forms, genus $g$ brings Siegel modular forms of degree $g$.  The invariants $b_1 = 2g$, $\dim_\mathbb{C} \mathcal{M}_g = 3g - 3$, and $\Omega \in \mathfrak{H}_g$ control the quantum corrections at each level.
\end{remark}
