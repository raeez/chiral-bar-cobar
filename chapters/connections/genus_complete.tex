...
\begin{remark}[Scope]
The Eynard--Orantin topological recursion is a theorem for matrix model correlators (Eynard--Orantin 2007) and has been proved to apply to Hurwitz numbers, Gromov--Witten invariants of $\mathbb{P}^1$, and related enumerative problems.  The claim that bar complex correlation functions satisfy this recursion requires establishing that the spectral curve of the chiral algebra (extracted from the genus-0 data $\omega_{0,1}$ and $\omega_{0,2}$) governs the higher-genus corrections in the same way.  For the free boson/Heisenberg algebra, this follows from the equivalence with the Gaussian matrix model.  For general chiral algebras, the claim remains conjectural pending verification that the bar complex satisfies the axioms of the abstract topological recursion framework (Kontsevich--Soibelman, Andersen--Borot--Orantin).
\end{remark}
