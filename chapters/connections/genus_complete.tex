...
\begin{definition}[Formal genus class and Hodge normalisation;
\ClaimStatusDefinitional]
\label{def:formal-genus-class-hodge-normalization}
\index{genus class!formal}
\index{Hodge class!normalisation}
\index{Faber--Pandharipande coefficient!normalisation}
Let $\pi\colon \mathcal C_{g,n}\to\overline{\cM}_{g,n}$ be the
universal stable curve and let
$\mathbb E_g:=\pi_*\omega_\pi$ be the rank-$g$ Hodge bundle.  Write
\[
\lambda_i:=c_i(\mathbb E_g),
\qquad
\lambda_g:=c_g(\mathbb E_g).
\]
The symbol $\omega_g$ denotes the Chern--Weil Hodge-curvature package of
$\mathbb E_g$, normalised so that its scalar top component is
$\lambda_g$; at genus~$1$ this is the usual curvature representative of
$c_1(\det\mathbb E_1)$.

The \emph{formal genus class} of a modular Koszul chiral algebra~$\cA$
is
\[
\mathcal F_g(\cA)
:=\operatorname{CW}_{\cA,g}(\Theta_\cA)
\in R^\bullet(\overline{\cM}_{g,0})\widehat\otimes
\operatorname{End}_{\mathrm{ch}}(\cA),
\]
the genus-$g$ component of the modular Chern--Weil transform of the
ambient Maurer--Cartan element.  The Faber--Pandharipande scalar trace is
the numerical functional
\[
\operatorname{tr}^{\mathrm{FP}}_g(\alpha)
:=\int_{\overline{\cM}_{g,1}}\psi_1^{2g-2}\,\alpha,
\qquad
\lambda_g^{\mathrm{FP}}
:=\operatorname{tr}^{\mathrm{FP}}_g(\lambda_g)
=
\frac{2^{2g-1}-1}{2^{2g-1}}\frac{|B_{2g}|}{(2g)!}.
\]
Thus $F_g(\cA)$ in the algebraic part of this chapter denotes a formal
class or its specified numerical trace.  It is called an analytic free
energy only after an HS-sewing or trace-class hypothesis supplies a
convergent partition function
\textup{(}Definition~\ref{def:hs-sewing} and
Definition~\ref{def:shadow-partition-function}\textup{)}.
\end{definition}

\begin{definition}[Diagonal trace and ordered curvature;
\ClaimStatusDefinitional]
\label{def:diagonal-trace-ordered-curvature}
\index{diagonal trace}
\index{ordered curvature}
\index{scalar projection}
The map
\[
\operatorname{tr}_{\mathrm{diag}}\colon
\operatorname{End}_{\mathrm{ch}}(\cA)\widehat\otimes
R^\bullet(\overline{\cM}_{g,0})
\longrightarrow
R^\bullet(\overline{\cM}_{g,0})
\]
is the cyclic trace after projection to the diagonal OPE channel: every
internal edge of the stable graph carries the same channel label.  It is
normalised by
\[
\operatorname{tr}_{\mathrm{diag}}\bigl(r_\cA(z)\bigr)=\kappa(\cA),
\]
where $r_\cA(z)$ is the ordered two-point residue kernel of
Construction~\ref{constr:r-matrix-monodromy-vol1} and
equation~\ref{eq:ordered-classical-r-kernel-A}.  The operator
$d_{\mathrm{fib}}^{\mathrm{ord}}$ is the ordered genus-$g$ fibrewise bar
differential before passing to symmetric coinvariants.
\end{definition}

\begin{proposition}[Ordered curvature and scalar Hodge projection;
\ClaimStatusConditional]
\label{prop:ordered-curvature-scalar-hodge-projection}
\index{ordered curvature!scalar projection}
\index{Hodge class!from ordered curvature}
\textbf{Type signature.} (Open quadrant; ordered $E_1$ chiral bar
presentation; level~$3$ for the endomorphism-valued curvature and
level~$5$ after trace + clutching; hypothesis package: finite-window
ordered bar coalgebra, the trace + clutching package, and the signed
log-FM residue-pushforward package.)
In each finite genus--bar window,
\begin{equation}\label{eq:ordered-curvature-r-omega}
\bigl(d_{\mathrm{fib}}^{\mathrm{ord}}\bigr)^2
=
r_\cA(z)\cdot\omega_g
\end{equation}
as an equality of endomorphism-valued curvature classes on
$\overline{\cM}_{g,\bullet}$.  It is neither a scalar identity nor an
unrenormalised equality of raw chain-level operators.  Applying the
diagonal trace and then the top Hodge
Chern--Weil projection gives
\begin{equation}\label{eq:ordered-curvature-diag-hodge}
\operatorname{tr}_{\mathrm{diag}}
\bigl((d_{\mathrm{fib}}^{\mathrm{ord}})^2\bigr)
=
\kappa(\cA)\lambda_g
\quad\text{in }R^\bullet(\overline{\cM}_{g,\bullet}).
\end{equation}
After the Faber--Pandharipande trace of
Definition~\ref{def:formal-genus-class-hodge-normalization}, this becomes
$F_g^{\mathrm{diag}}(\cA)=\kappa(\cA)\lambda_g^{\mathrm{FP}}$.
\end{proposition}

\begin{proof}
The local collision residue of the ordered bar differential is the
kernel $r_\cA(z)$ by
equation~\ref{eq:ordered-classical-r-kernel-A}.  The only genus-$g$
curvature source not present on a fixed smooth configuration fibre is
the Hodge curvature of the universal stable curve; with the
normalisation of
Definition~\ref{def:formal-genus-class-hodge-normalization} this is
$\omega_g$.  Their product is therefore the endomorphism-valued
curvature class~\eqref{eq:ordered-curvature-r-omega}.  The diagonal
trace kills non-diagonal OPE channels and sends the ordered kernel to
$\kappa(\cA)$ by
Definition~\ref{def:diagonal-trace-ordered-curvature}, giving
\eqref{eq:ordered-curvature-diag-hodge}; applying
$\operatorname{tr}^{\mathrm{FP}}_g$ gives the displayed numerical
coefficient.
\end{proof}

\begin{remark}[Genus-one anomaly and KZB comparison]
\label{rem:genus-one-anomaly-kzb-comparison}
\index{KZB connection!genus-one anomaly comparison}
\index{Eisenstein series!$E_2$ anomaly}
\index{holomorphic anomaly!genus one}
At genus~$1$ the formal scalar class has a holomorphic
quasi-modular representative: the logarithmic derivative of the
Dedekind $\eta$-factor is proportional to $E_2(\tau)$.  Modular descent
replaces it by the almost-holomorphic completion
\[
\widehat E_2(\tau)=E_2(\tau)-\frac{3}{\pi\,\operatorname{Im}\tau}.
\]
The anomaly term is the difference
$\widehat E_2-E_2$, and it has no holomorphic preimage on the ordered
$E_1$ side; this is precisely the no-section statement of
Proposition~\ref{prop:e1-nonsplitting-genus1}.  The genus-one KZB
connection supplies the analytic local-system comparison: its
$d\tau$ component has Hodge shadow $\omega_1$ as in
equation~\ref{eq:v1-kzb-conn}.  Thus KZB records the same genus-one
curvature channel, but the cohomological class, the quasi-modular
representative, and the analytic connection are different objects.
\end{remark}
...
\]
This is the whole formal genus class on the uniform-weight scalar lane
and at genus~$1$ for every modular Koszul algebra. Off that lane, the
all-weight formal genus class is
\[
...
The formal stable-curve skeleton is the modular-operad structure of
Getzler--Kapranov.  Its lift to a chain-level boundary-residue modular
functor is conditional on the signed log-FM residue-pushforward package
(Theorem~\ref{thm:chain-modular-functor}): to each surface
$\Sigma_{g,n}$ a cochain complex, to each degeneration a chain map, and
to each consistency relation a chain homotopy. When this package is
present, passing to cohomology gives the classical cohomological
modular-functor package; for $\widehat{\fg}_k$ at integrable level,
...
the pointed genus-$1$ trace class
\[
\operatorname{tr}_1\operatorname{Obs}^{\mathrm{def}}_{1,1}(\mathcal{A}_0)
...
=(c/2)\lambda_1$; its zero locus is $c=0$. The integrated Virasoro
trace is
...
\begin{definition}[Higher-genus configuration spaces; \ClaimStatusDefinitional]\label{def:higher-genus-config}
...
\begin{theorem}[Master tower of boundary residues; \ClaimStatusConditional]\label{thm:master-tower}
...
$\{\overline{\cM}_{g,n}\}_{g,n}$, plus the signed log-FM
residue-pushforward package of
Definition~\ref{def:signed-logfm-residue-pushforward-package} whenever
the displayed residues are read as chain-level maps on the bar
coefficients.)
...
\begin{proof}
There are two layers.  The stable-curve modular-operad skeleton is
proved by Getzler--Kapranov: clutching and self-gluing maps on
$\{\overline{\mathcal M}_{g,n}\}$ have boundary-of-boundary
codimension-two cancellations.  The stronger assertion that the
residue symbols above are determinant-line-correct chain maps on the
bar coefficient system is conditional on the signed log-FM
...
genus-$g$ bar complexes into a formal modular coalgebra. The maps
$\rho_{\mathrm{irr}}$ and $\rho_{g_1,I}$ are the residue maps to
the irreducible and separating boundary strata.  On the formal
modular-operad skeleton they are compatible with the Feynman-transform
differential because that differential is the signed boundary operator
of the modular operad; the two codimension-$2$ presentations of a
boundary stratum cancel with opposite signs.  On the log-FM bar
coefficient system this compatibility additionally requires the global
Gysin residues, proper pushforwards, stabiliser normalisations, and
homotopy-coherent comparison maps of
...
\begin{proposition}[Stable graph weights and finite-window passage;
\ClaimStatusConditional]
\label{prop:stable-graph-finite-window-passage}
\index{stable graph weights}
\index{cross-channel correction!stable graph sum}
\index{BV operator!nonseparating self-gluing}
\textbf{Type signature.} (Open quadrant; modular-operad/Feynman-transform
presentation; level~$3$ for graph weights and level~$5$ after scalar
trace; hypothesis package: finite channel window, signed log-FM
residue-pushforward package, and the Mittag--Leffler condition of
Lemma~\ref{lem:genus-window-ml-passage}.)
Fix a finite genus--bar--channel window.  For a stable graph
$\Gamma\in\mathcal G_{g,0}$ and an OPE-channel labelling
$\sigma\colon E(\Gamma)\to\{1,\ldots,r\}$, the graph weight is
\begin{equation}\label{eq:stable-graph-channel-weight}
W_{\Gamma,\sigma}(\cA)
:=
...
\end{equation}
with inverse Zamolodchikov edge metric and vertex OPE amplitude as in
Construction~\ref{constr:cross-channel-graph-sum}.  The mixed-edge
correction is the finite stable-graph sum
\begin{equation}\label{eq:cross-channel-mixed-edge-sum}
\delta F_g^{\mathrm{cross}}(\cA)
=
\sum_{\Gamma\in\mathcal G_{g,0}}
\sum_{\substack{\sigma:E(\Gamma)\to\{1,\ldots,r\}\\
\sigma\ \mathrm{mixed}}}
W_{\Gamma,\sigma}(\cA),
\end{equation}
where ``mixed'' means that at least two internal edges carry different
OPE-channel labels.  Consequently
\[
\delta F_1^{\mathrm{cross}}=0,
\]
because the genus-$1$ boundary graph has at most one internal edge,
whereas mixed-edge graphs exist at every genus $g\ge2$ for
multi-channel algebras: already the genus-$2$ sunset and theta graphs
carry two or three internal edges.

The finite-window graph sum is finite.  Passing from finite windows to
the completed all-weight tower is the strict
Mittag--Leffler passage of Lemma~\ref{lem:genus-window-ml-passage}; no
analytic convergence of a partition function is used in this algebraic
step.

The bar object is an algebra over the Feynman transform of the
commutative modular operad.  Its genus-$0$ restriction is cyclic; the
nonseparating self-gluing makes the full object wheeled/modular.  The
genus-raising operator
\[
\hbar\Delta:=\hbar\sum_{\mathrm{nonsep}}\Delta_{\mathrm{nonsep}}
\]
is the BV self-gluing component of the modular differential.  The
identities $\Delta^2=0$ and $[d_{\mathrm{sew}},\Delta]=0$ are the two
nonseparating boundary-of-boundary cancellations in the
Getzler--Kapranov modular operad, transported through the signed log-FM
package.  A loop edge is oriented by the determinant line of its two
half-edges; reversing the two half-edges changes both the
edge-orientation line and the odd cooperadic suspension, so the scalar
nonseparating loop has sign $+1$ after diagonal trace.  Before scalar
trace the remaining sign is the ordinary Koszul sign of the cyclic
pairing.
\end{proposition}
...
For multi-weight algebras the full formal genus class also contains
...
\ClaimStatusConditional]\label{thm:chain-modular-functor}
...
package on $\{\overline{\cM}_{g,n}\}$ and the signed log-FM
residue-pushforward package for chain-level residue maps. The functor's mapping-class
...
\end{itemize}
At the formal stable-curve skeleton, compatibility with the
Feynman-transform differential follows from the modular operad
structure \cite[Theorem~5.4]{GeK98}.  At the level of actual chiral
bar coefficient chains, the same sentence is true only after the
signed log-FM residue-pushforward package supplies the Gysin residue
maps, determinant-line signs, proper pushforwards, and
automorphism-group normalisations.
...
\mathrm{id}^{\otimes t}) = 0$.  The comparison between the two
global residue-pushforward targets is part of the same signed log-FM
package; without it this theorem is only the formal modular-operad
skeleton, not a chain-level modular functor.
\end{proof}
...
\ClaimStatusConditional]\label{cor:dual-modular-functor}
...
$\overline{\cM}_{g,n}$; signed log-FM residue-pushforward packages for
the $\cA$ and $\cA^!$ chain-level boundary-residue functors. On the
\emph{KSDual} sublocus
...
Theorem~\ref{thm:poincare-extended} is perfect and that the conditional
chain-level functors of Theorem~\ref{thm:chain-modular-functor} are
available for both sides. The chain-level modular functor attached to
$\mathcal{A}^!$ is the Verdier dual of the bar functor attached to
$\mathcal{A}$:
...
identification of the underlying genuswise complexes.  The
factorization and self-sewing maps are the conditional residue maps of
Theorem~\ref{thm:chain-modular-functor}; once the signed log-FM
pushforward package supplies them on both sides, compatibility follows
from naturality of Verdier duality for the boundary embeddings
...
Under the hypotheses of Theorem~\ref{thm:chain-modular-functor}, the
chain-level modular functor produces, at each genus~$g$, a sheaf of
cochain complexes
...
\begin{theorem}[Bar complex defines tautological moduli integrals; \ClaimStatusConditional]\label{thm:bar-moduli-integrals}
...
signed log-FM residue-pushforward package for the module-insertion
coefficient system; properness of the Fulton--MacPherson configuration
map; finite-window compatibility with chiral operations.)
For a Koszul chiral algebra $\mathcal{A}$ and modules
$\mathcal{V}_1,\ldots,\mathcal{V}_n$, assume the signed log-FM
residue-pushforward package of
Definition~\ref{def:signed-logfm-residue-pushforward-package} is
available for these module insertions. Then the bar complex with module
...
\]
by the resulting proper pushforward from the logarithmic
Fulton--MacPherson configuration space.  Pairing this class with the
virtual fundamental class gives
\[
...
is the formal power series in $g_s$ assembled from the conditional
chain-level genus-graded bar complex of Theorem~\ref{thm:master-tower}.
Without the signed residue-pushforward package one retains only the
formal stable-curve pairing after a class has been supplied; the
displayed pushforward construction itself is not proved.
\end{theorem}

\begin{proof}
Under the signed log-FM residue-pushforward package, the bar complex at
genus~$g$ with module insertions defines a logarithmic coefficient
object on the compactified configuration space over
...
(Theorem~\ref{thm:prism-higher-genus}).  The package supplies the
determinant-line-correct residue maps and the global proper pushforward
on this coefficient object. The forgetful map
...
\begin{remark}[Scope]
The conditional moduli integrals $\mathcal{I}_{g,n}$ are the
construction of Theorem~\ref{thm:bar-moduli-integrals}; the formal bar
and BRST comparison inputs are established in Part~\ref{part:bar-complex}.
At genus~$0$, the algebraic BRST/bar comparison is conditional at the
theorem surface, and the moduli-integration/amplitude identification is
also conditional
...
inductively under the signed log-FM package
...
\begin{theorem}[Poincaré--Verdier duality for genus pieces; \ClaimStatusConditional]\label{thm:poincare-extended}
...
hypothesis package: a presentation
$\cA=T_X(V)/(R)$ with
$\cA^{\mathrm i}=C_X(s^{-1}V,s^{-2}R)$; a quasi-isomorphism
$q_\cA\colon\cA^{\mathrm i}\to B^{\mathrm{ch}}(\cA)$; finite
duality; perfect genus-$g$ Poincar\'e--Koszul pairing.)
Let $B^{(g)}(\mathcal{A})$ be the genus-$g$ bar object. If
$\mathcal{A}^{\mathrm i}$ is finite-type and the
genus-$g$ Poincar\'e--Koszul pairing between the bar coefficients of
$\mathcal A$ and the cobar coefficients of $\mathcal A^{\mathrm i}$ is perfect,
then Verdier duality on $\overline{\mathcal{M}}_{g,n}$ identifies the
algebra
$K_X^{(g)}(\mathcal A):=
\mathbb D_{\overline{\mathcal M}_{g,n}}B^{(g)}(\mathcal A)$ with the
cobar object built from $\mathcal{A}^{\mathrm i}$:
\[
\gamma_g\colon K_X^{(g)}(\mathcal A)
\xrightarrow{\sim}\Omega^{(g)}(\mathcal{A}^{\mathrm i}).
\]
The map~$\gamma_g$ is the genus-$g$ Verdier--cobar comparison.  The
counit $\Omega B(\mathcal A)\to\mathcal A$ gives reconstruction, while
$Z^{\mathrm{der}}_{\mathrm{ch}}(\mathcal A)$ remains the Hochschild
centre.
\end{theorem}
...
with the cobar coefficient system generated by
$\mathcal A^{\mathrm i}$; finite type gives
$\mathcal A^!=(\mathcal A^{\mathrm i})^\vee$ on the algebra side.
The reversed residue differential is exactly the cobar differential.
Thus~$\gamma_g$ identifies the Verdier algebra of the genus-$g$ bar
object with the genus-$g$ cobar object on $\mathcal{A}^{\mathrm i}$.
The dualizing-complex shift is
...
The complete packaging of these ingredients (algebraic closed-sector
object, Koszul-dual defect algebra, algebraic line-side model,
genus-zero $r$-matrix, genus-refined MC class, and shadow connection)
is the \emph{holographic modular Koszul datum} $\mathcal{H}(T)$
...
\index{closed sector!from boundary}
...
\begin{definition}[Genus-graded holographic formal partition series]
\label{def:holographic-partition-function}
\index{partition function!genus-graded holographic}
The all-genera holographic formal partition series is
...
formal genus class at all genera. For arbitrary modular Koszul algebras the
...
All scalar normalisations in this paragraph use
...
The scalar row uses the normalisation conventions of
Appendix~\ref{sec:scalar-normalization-appendix}.
\begin{center}
...
\(\kappa(\mathcal{H}_\kappa)\), which evaluates to the level
\(\kappa\) for the rank-one Heisenberg algebra.
...
Chern--Weil projection is a function of the single level parameter
\(\kappa\) alone, while the scalar projection itself factors through
the typed characteristic:
...
\begin{theorem}[Conditional nonlinear modular characteristic hierarchy]
\label{thm:conditional-modular-characteristic-hierarchy}
\ClaimStatusConditional
...
\begin{equation}
\label{eq:conditional-modular-characteristic-hierarchy}
...
\begin{definition}[HS-sewing condition; \ClaimStatusDefinitional]%
\label{def:hs-sewing}%
...
\begin{definition}[Analytic bar coalgebra; \ClaimStatusDefinitional]%
...
\begin{definition}[Connected genus-$1$ sewing scalar]%
\label{def:connected-genus1-free-energy}%
\index{sewing scalar!connected genus-1}%
\ClaimStatusDefinitional
Let $\cA$ satisfy HS-sewing
(Definition~\ref{def:hs-sewing}) with genus-$1$ sewing
operator~$K_q(\cA)$.
The \emph{connected genus-$1$ sewing scalar}
...
$\log Z_\cA$: the connected vacuum amplitude on the torus, hence the
analytic connected free energy only after HS-sewing has supplied the
...
\begin{definition}[Weight multiset; \ClaimStatusDefinitional]%
...
\begin{definition}[Euler--Koszul defect and class; \ClaimStatusDefinitional]%
...
\begin{definition}[Prime-side Li coefficients; \ClaimStatusDefinitional]%
...
\begin{definition}[Sewing moment pairing; \ClaimStatusDefinitional]%
