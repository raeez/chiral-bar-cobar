...
$\overline{\mathcal{M}}_{g,n}$.  The scalar genus tower on the
...
By the genus universality theorem (Theorem~\ref{thm:genus-universality}), the \emph{scalar} genus expansion on the proved scalar lane is determined by a single numerical invariant: the obstruction coefficient $\kappa(\mathcal{A})$, which depends on the genus-$1$ curvature alone.  In practice, $\kappa$ is computed from the central charge and the structure of the OPE: $\kappa(\widehat{\mathfrak{g}}_k) = (k+h^\vee)\dim\mathfrak{g}/(2h^\vee)$, $\kappa(\mathrm{Vir}_c) = c/2$, $\kappa(\mathcal{H}_\kappa) = \kappa$.  On that lane, the scalar genus expansion $F_g(\mathcal{A}) = \kappa(\mathcal{A}) \cdot \lambda_g^{\mathrm{FP}}$ is given by Bernoulli numbers.  For arbitrary modular Koszul algebras, only the genus-$1$ scalar term is unconditional.  The separately proved spectral invariants, and the full Maurer--Cartan completion on the simple-Lie-symmetry locus of Theorems~\ref{thm:universal-theta} and~\ref{thm:mc2-full-resolution}, are \emph{not} determined by~$\kappa$ alone; see Theorem~\ref{thm:spectral-characteristic} and Definition~\ref{def:full-modular-package}.
