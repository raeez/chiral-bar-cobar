...
cubic, quartic, and all-degree shadows as
tautological classes (the all-degree existence is
...
$F_g = \kappa(\cA) \cdot \lambda_g^{\mathrm{FP}}$ \textup{(}Theorem~D\textup{)}
...
Here $\lambda_1 = c_1(\mathbb{E})$ is the first Chern class of the Hodge bundle, consistent with $\operatorname{obs}_g(\mathcal{A}) = \kappa(\mathcal{A})\cdot\lambda_g^{\mathrm{FP}}$ \textup{(}UNIFORM-WEIGHT\textup{}) by Theorem~\ref{thm:genus-universality} at $\kappa(\mathrm{Vir}_c) = c/2$. Characters transform as vector-valued modular forms; the phase $e^{-2\pi i c/24}$ is trivial only for $c \in 24\mathbb{Z}$.
\end{theorem}
...
$\kappa(\cA) \cdot \lambda_g$ while the
...
The scalar genus expansion $F_g(\cA) = \kappa(\cA)\lambda_g^{\mathrm{FP}}$ and the
...
The scalar $\kappa(\cA)$, the spectral $\Delta_\cA$, the nonlinear shadow
...
$F_g = \kappa\lambda_g^{\mathrm{FP}}$ $\Delta(x) = 1 - \kappa x$,
...
$\Theta_\cA^{\leq 2} = \kappa(\cA)$ (proved); the spectral
...
For every degree cutoff~$R$, the interacting sewing kernel
...
The nontrivial content is that higher-degree shadow contributions
$b_r(N)$ depend on the single variable~$N$, not separately on the
degree-$r$ and genus-$g$ indices that might introduce independent
...
\section{genus-2 Siegel character 1/Phi_10 at K3 x E}
\label{sec:genus-complete-wave13-DNA}

\begin{remark}[Genus-$2$ Siegel character $1/\Phi_{10}$]
\label{rem:genus-complete-DNA-character}
The $K3 \times E$ chiral bialgebra of Vol~III Wave~13 has Siegel character
$\mathrm{Ch}(\mathbf{H}_{\Delta_5})(Z) = 1/\Phi_{10}(Z)$ on the Siegel upper half-space
$\mathbb{H}_2$; at a separating-node degeneration $Z \to \mathrm{diag}(\tau, \rho)$ the
character factorises as
\[
  1/\Phi_{10}(Z) \;\longrightarrow\; \frac{1}{\eta(\tau)^{24}} \cdot \frac{1}{\phi_{10,1}(\rho)},
\]
recovering the $\eta^{24}$ genus-$1$ denominator of the K3 chiral algebra times the
Jacobi form of the elliptic direction. Since $\Delta_5^2 = C \cdot \Phi_{10}$ (Gritsenko--Nikulin
$1998$), the BKM weight is $\kappa_{\mathrm{BKM}}(\mathfrak{g}_{\Delta_5}) = c_\Lambda(0)/2 =
\mathrm{wt}(\Delta_5) = 5$.
\end{remark}

\begin{remark}[Bi-based factorisation and higher-genus averaging]
\label{rem:genus-complete-DNA-bi-based}
...
\label{sec:genus-complete-wave15-DNA}

\begin{remark}[Tannakian fibre functor at genus $2$]
\label{rem:genus-complete-wave15-tannakian}
The Siegel-genus-$2$ character $1/\Phi_{10}(Z)$ of
Remark~\ref{rem:genus-complete-DNA-character} is the
\emph{Tannakian graded trace} of the fibre functor
...
Vol~III Remark~\ref{rem:cydkappa-wave15-gerbe}), while at the Humbert
...
$F_{ij}(Z)=(\Phi_{10}/\eta^{24}|_{U_i}\,/\,\Phi_{10}/\eta^{24}|_{U_j})^{1/8}_{\mathrm{princ}}$
on the Igusa fundamental-domain cover of $U=\overline{\mathcal A_2}\setminus(H_1\cup H_4)$
trivialises the gerbe on $U$ (Vol~III
Theorem~\ref{thm:modtr-wave18-banding-cocycle}), and the
...
The Feynman-transform genus-tower formula
$F_g = \kappa(\mathcal{A}) \cdot \lambda_g^{\mathrm{FP}}$ of
Theorem~\ref{thm:chain-modular-functor} is the genus-$g$ avatar of
this Tannakian trace on the K3 chiral bialgebra: at $g=2$,
$\kappa \cdot \lambda_2^{\mathrm{FP}}$ is the first Chern class of the
determinant bundle pulled back along $\mathrm{av}$. Cross-ref
Vol~III Remark~\ref{rem:cydkappa-wave15-fibre-functor}.
\end{remark}

\begin{remark}[Mukai-grading compatibility with modular operad]
\label{rem:genus-complete-wave15-mukai-modular}
...
lattice rank. This confirms the Wave~15 direction rule
(cache entry W15-Mukai-grading-direction): grading is on the
...
Remark~\ref{rem:genus-complete-DNA-bi-based} precisely encodes this
...
grading (Mukai-lattice variation).  Cross-ref Vol~II
Remark~\ref{rem:htpo-wave15-mukai-direction} and Vol~III
Remark~\ref{rem:cydkappa-wave15-mukai-grading-direction}.  Primary:
Getzler--Kapranov 1998 Comp.~Math.~110 (modular operad);
Mukai 1987 Nagoya Math.~J.~81 (Mukai pairing); Gritsenko--Nikulin
1998 Amer.~J.~Math.~120 (Siegel $\Phi_{10}$ factorisation at
separating node).
\end{remark}
