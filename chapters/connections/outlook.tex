...
The shadow metric $Q_L$ (Definition~\ref{def:shadow-metric}) reduces the infinite tower to three finite parameters $(\kappa, \alpha, S_4)$ on each primary line.  The algebraic relation $H^2 = t^4 Q$ (Theorem~\ref{thm:riccati-algebraicity}) makes the shadow tower algebraic of degree~$2$: infinite depth with finite complexity.  The logarithmic shadow connection (Theorem~\ref{thm:shadow-connection}) identifies the Koszul sign with the monodromy at the spectral branch points, and the propagator variance (Theorem~\ref{thm:propagator-variance}) governs multi-channel mixing.
...
\item \emph{Analytic sewing.}  The sewing envelope $\cA^{\mathrm{sew}}$,
  ...
  (Theorem~\ref{thm:general-hs-sewing}).  Lattice sewing envelope now
...
\begin{proposition}[Celestial cross-conjecture relation]
\label{prop:celestial-cross-conjecture-vol1}
\ClaimStatusProvedHere
\index{celestial OPE!cross-conjecture relation|textbf}
\index{quartic shadow!celestial cross-conjecture}
The three provisional celestial OPE conjectures
\textup{(}Vol~II: the symmetric quartic jet $Q^{\mathrm{sym}}_{110}$,
the ordered quartic jet $Q^{\mathrm{ord}}_{110}$, and the first
mixed bubble coefficient $B_{\mathrm{mix}}$\textup{)} are not
independent.  The identity
...
\frac{11}{90}
\end{equation}
holds as a consequence of crossing symmetry applied to the
quartic shadow $R^{\mathrm{mod}}_{4,0,4}(\cA)$
\textup{(}Definition~\textup{\ref{def:quartic-resonance-class}}\textup{)}:
the difference between the symmetric and ordered celestial jets
is fixed by the single-trace/multi-trace decomposition of the
genus-$0$ four-point amplitude, and the mixed bubble coefficient
$B_{\mathrm{mix}}$ is the residue of the same amplitude at the
exchange channel.
\end{proposition}
