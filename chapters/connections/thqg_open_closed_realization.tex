...
Gerstenhaber algebra. The universal bulk in the Swiss-cheese
theorem is the dg object
$\mathbf{Z}^{\mathrm{der}}_{\mathrm{ch}}(\cA)$; its on-shell
bulk fields are the classes in
$\mathcal{Z}^{\mathrm{der}}_{\mathrm{ch}}(\cA)$.
...
is a triple $(\cB, \cA, \{\mu_{p;q}\})$ where:
\begin{enumerate}[label=\textup{(\alph*)}]
\item $\cB$ is a continuous brace dg algebra, or equivalently
 the closed colour algebra in the local chiral Swiss-cheese model;
\item $\cA$ is an $A_\infty$-chiral algebra, the open colour;
\item the mixed operations
 $\mu_{p;q}\colon \cB^{\otimes p}\otimes A^{\otimes q}
 \to A((\lambda_1))\cdots((\lambda_{p+q-1}))$
 satisfy the local chiral Swiss-cheese identities.
\end{enumerate}
Equivalently, the one-closed-input operations define a continuous
...
identifies the universal bulk with the cochain-level object
$\mathbf{Z}^{\mathrm{der}}_{\mathrm{ch}}(\cA)$.
...
\]
\[
\mathbb{D}_{\Ran}\barB_X(\cA_i)\simeq \cA^!
\quad\text{is Verdier duality,}
\]
whereas
$\mathbf{Z}^{\mathrm{der}}_{\mathrm{ch}}(\cA)$ is governed by
chiral Hochschild cochains. The Koszul dual algebra~$\cA^!$ is not
the bulk unless an additional compact-generator Hochschild
comparison identifies the relevant category of boundary conditions
with a Hochschild center.
\end{remark}
...
\textup{(}closed-sector endomorphism decorations\textup{)}, not
mixed-\emph{sector} bulk-to-boundary $\mathsf{SC}$ operations.
\end{remark}
...
\item \emph{Closed formal chart.}
 ...
 \mathcal{C}^{\bullet}_{\mathrm{ch}}(\cA, \cA).
 \end{equation}
 This chart sees the closed algebra~$\cA$; it does not use the
 compact generator~$b$.
...
\item \emph{Open categorical center.}
 ...
 equivalence class of~$\mathcal M$, not on the choice of~$b$.
...
When $A_b$ is the formal chiral boundary algebra induced from~$\cA$,
the geometric--algebraic comparison upgrades this to the chiral
cochain model
$\mathcal{C}^{\bullet}_{\mathrm{ch}}(A_b, A_b)$. On the Koszul/PBW
locus covered by Theorem~H, its cohomology is concentrated in
degrees~$\{0,1,2\}$ as in~\eqref{eq:thqg-derived-center-three-term}.
\end{corollary}
...
The open/closed MC element sees the projection of the holographic
modular Koszul datum
$\mathcal{H}(\cA) = (\cA, \cA^!, \mathcal{C}, r(z),
\Theta_\cA, \nabla^{\mathrm{hol}})$
(Definition~\ref{def:thqg-holographic-datum})
to its closed, open, mixed, and shadow components:
\begin{enumerate}[label=\textup{(\roman*)}]
...
\item The \emph{shadow obstruction tower projections}
 ...
 in the open sector.
\end{enumerate}
The projection does not reconstruct $\cA^!$, $r(z)$, or
$\nabla^{\mathrm{hol}}$ from the MC element alone. The algebra
$\cA^!$ is obtained through Verdier duality on the bar coalgebra,
$r(z)$ from the collision residue of the OPE, and
$\nabla^{\mathrm{hol}}$ from the modular connection data.
\end{theorem}
...
\end{enumerate}
Since all three sectors are projections of the same
...
\item \emph{Open-closed consistency at every genus.}
 ...
 constrains their joint values in the open/closed convolution
 algebra.
\end{enumerate}
\end{theorem}
...
Part~(d) is the structural consequence. The genus-$g$
...
open sector. The compatibilities are projections of the Stokes theorem
...
\item \emph{The independent open-sector derivation.}
 ...
 consistency states the target identity for such a construction;
 it does not produce the open-sector calculation.
\end{enumerate}
\end{remark}
...
trace recovers the open-closed identification proved in
Theorem~\ref{thm:thqg-mc-forced-consistency}. The remaining
structural question is whether these data assemble into a modular
cooperad in the sense of Getzler--Kapranov
\cite{GetzlerKapranov98}, and whether that assembled structure is
...
Conjecture~\ref{conj:ct2} has three logically separate obligations.
...
is homotopy Koszul; the needed input is the Vol.~II PVA descent
for the open-sector category.
...
\begin{remark}[Proved loci and conjectural loci]
...
\item The genus-$1$ trace of $\Theta^{\mathrm{oc}}_\cA$
 recovers the annulus identification by a Zhu-type argument,
 producing the open-closed pairing on $\mathrm{HH}_\ast(\cA)$
 at $g = 1$ (Theorem~\ref{thm:thqg-annulus-trace}). This proves
 the genus-$1$ trace clause of Conjecture~\ref{conj:ct2}(b).
\end{enumerate}
The unproved obligations are:
\begin{enumerate}[label=\textup{(\roman*)}]
\item Lift the modular cooperad structure along
...
\item Integrate the homotopy-coherent clutching
...
\end{enumerate}
These two obligations are exactly the existence and uniqueness
clauses of Conjecture~\ref{conj:ct2}.
\end{remark}

\begin{remark}[Categorical localization obstruction]
\label{rem:ct2-brochier-woike}
\index{Brochier--Woike localization|textbf}
The remaining obstruction is categorical, not geometric. The
bordered FM geometry, the chain-level differential, and the
genus-$1$ annulus trace are all constructed at the level of dg
categories and their homologies; clutching compatibility is a
concrete chain-level identity. What is missing is the passage from
these strict chain-level data to a homotopy-coherent
$(\infty,2)$-category of open-sector factorization algebras in
which cooperad structures are classified up to equivalence.
...
$\Theta^{\mathrm{oc}}_\cA$ as the classifying datum. The remaining
obstruction is therefore isolated to a single categorical step;
no geometric or chain-level obstruction is visible in the present
model.
\end{remark}
...
$\mathbf{Z}^{\mathrm{der}}_{\mathrm{ch}}(\cA)$
 & cochain-level derived center (universal bulk)
...
by adding the cochain-level derived center (universal bulk), the annulus trace
...
\item \emph{Bulk.}
 ...
 universal bulk, reconstructed from the boundary. Its cohomology
 $\mathcal{Z}^{\mathrm{der}}_{\mathrm{ch}}(\cA)$ is the on-shell
 Gerstenhaber algebra of bulk fields. Holography
 is the assertion that the boundary data $\cA$ determines the
 bulk $\mathbf{Z}^{\mathrm{der}}_{\mathrm{ch}}(\cA)$ and
...
\section{K3 class-\texorpdfstring{$\mathcal S$}{S} open-closed realization}
...
$\mathfrak{su}(2)^{24}$, providing a $4d$/$2d$ open-closed
realisation in which the Schur sector is the K3 chiral algebra
...
records the K3 chiral-bialgebra chart.
\end{remark}
...
\cite{ConwaySloane1988,KugaSatake1967}; see
...
pair $(\mathbf{Z}^{\mathrm{der}}_{\mathrm{ch}}(\mathbf{H}_{\Delta_5}),
...
is the matching normalization for the Maulik--Okounkov
braid-group representation at the self-dual fixed point.
...
first-principles $a_{4d} = 403/24$ by $18/24 = 3/4$; this is an
open normalization mismatch for the $a$-anomaly, not an input to
the $c_{4d}$ calculation. The six-fold pentagon-umbral
...
higher rank does not follow from the rank formula
$\tfrac12(21(N-1)+2)$, which would already contradict the $N=2$
...
candidate source, but the BKM-root structure at $N \ge 3$ does not
reduce to Mathieu moonshine and may lack a closed automorphic form.
...
this is a functorial identification, not an equality. The rank-$N$
extension preserves the distinction between a cohomological Hall
algebra and an $E_1$-chiral bialgebra.
\end{remark}
