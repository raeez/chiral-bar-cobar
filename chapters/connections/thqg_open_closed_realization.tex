...
\partial \;=\; \cA, \qquad
\mathrm{closed}_{\mathrm{alg}} \;=\;
Z^{\mathrm{der}}_{\mathrm{ch}}(\cA), \qquad
\mathrm{interaction} \;=\; \mathsf{SC}^{\mathrm{ch,top}}\text{-brace}.
\end{equation}
Here $\mathrm{closed}_{\mathrm{alg}}$ is not the physical
factorization algebra
$\operatorname{Obs}^{\mathrm{bulk}}(T)$ of an
HT theory.  The boundary chart is the chiral algebra, the closed
algebraic slot is the
...
$\mathsf{SC}^{\mathrm{ch,top}}$-brace closed-sector theorem at level~$3$ of the
...
\begin{definition}[Open--closed comparison datum \textup{(}OCA\textup{)};
\ClaimStatusDefinitional]
\label{def:thqg-oca-datum}
\index{open--closed comparison datum|textbf}
\index{OCA datum|see{open--closed comparison datum}}
Let \(T\) be a holomorphic--topological field theory with boundary
condition whose boundary chart is a complete curved
\(A_\infty\)-chiral algebra~\(\cA\). An
\emph{open--closed comparison datum} for~\(T\) over~\(\cA\) consists
of:
\begin{enumerate}[label=\textup{(\roman*)}]
\item a boundary identification
 \[
 \alpha_\partial\colon
 \operatorname{Obs}^{\partial}(T)\xrightarrow{\;\simeq\;}\cA
 \]
 in the completed chiral \(A_\infty\)-ambient;
\item a completed boundary local-operator shadow
 \(\operatorname{Loc}_{\partial}
 \operatorname{Obs}^{\mathrm{bulk}}(T)\), equipped with the
 continuous brace dg algebra structure induced by the closed colour;
\item a continuous brace dg algebra map
 \begin{equation}\label{eq:thqg-oca-map}
 \beta_T\colon
 ...
 \end{equation}
\item compatibility of \(\beta_T\) with the
 \(\mathsf{SC}^{\mathrm{ch,top}}\)-mixed action on the pair
 \((\mathbf{Z}^{\mathrm{der}}_{\mathrm{ch}}(\cA),\cA)\);
\item compatibility with the conformal-weight, pole-order, and
 analytic completions in which the observables of~\(T\) and the
 chiral Hochschild cochains are defined.
\end{enumerate}
The OCA datum is an \emph{identification datum} only when
\(\beta_T\) is a quasi-isomorphism in the chosen completion. Without
that quasi-isomorphism, it is merely a comparison map from the physical
bulk shadow to the algebraic derived centre. In particular, neither
\(\bar B(\cA)\) nor \(Z^{\mathrm{der}}_{\mathrm{ch}}(\cA)\) is, by
itself, the physical bulk factorisation algebra of~\(T\).
\end{definition}
...
(Lemma~\ref{lem:partial-comp-assoc}(b)).
...
setting is given by Theorem~\ref{thm:brace-dg-algebra} and
Definition~\ref{def:chiral-hochschild-cochain-brace}.
\end{proof}

\begin{definition}[Chiral derived center; \ClaimStatusDefinitional]
\label{def:thqg-chiral-derived-center}
\index{derived center!chiral|textbf}
The \emph{cochain-level chiral derived center} of~$\cA$ is the
active bimodule derived endomorphism object
\begin{equation}\label{eq:thqg-derived-center-rhom}
\mathbf{Z}^{\mathrm{der}}_{\mathrm{ch}}(\cA)
\;:=\;
R\!\operatorname{Hom}^{\mathrm{act}}_{\mathsf{Fact}_{\cA-\cA}}
(\cA,\cA),
\end{equation}
computed in the completed factorisation \(\cA\)-\(\cA\)-bimodule
ambient. In the chiral Hochschild model this is the brace dg algebra
\[
\mathbf{Z}^{\mathrm{der}}_{\mathrm{ch}}(\cA)
:=
...
$\mathcal{Z}^{\mathrm{der}}_{\mathrm{ch}}(\cA)$.
The \(E_2\)-language is used only after applying the chiral
Deligne--Tamarkin rectification to this brace dg algebra; before that
rectification the structure being asserted is the explicit continuous
brace dg algebra above.
...
and supplying the OCA datum of
Definition~\ref{def:thqg-oca-datum}, with \(\beta_T\) a
brace-compatible quasi-isomorphism.
Let $S\subset\mathbb Z$ be finite.  A family datum $H_H(\cA;S)$
gives
\begin{equation}\label{eq:thqg-derived-center-family-support}
\mathcal{Z}^{\mathrm{der}}_{\mathrm{ch}}(\cA)
\;=\;
\bigoplus_{n\in S}\mathcal Z^n,
\qquad
\mathcal Z^n\cong H^n(K_{\cA,S}).
\end{equation}
The Gerstenhaber bracket has degree~$-1$ and therefore maps
$\mathcal Z^p\otimes\mathcal Z^q$ to
$\mathcal Z^{p+q-1}$.  Bakalov--De Sole--Kac give the benchmark
support $\{0,2,3\}$ for Virasoro and dimensions $(2,1,0,\ldots)$ for
the rank-one even superboson after the bounded-to-chart comparison.
Critical, minimal/admissible, and $E_1$-chiral families carry their
own support data and averaging comparisons.  A perfect degree-$d$
pairing supplies the separate Koszul-dual degree reflection.
\end{definition}

\begin{definition}[Genus-$g$ chiral derived center; \ClaimStatusDefinitional]
...
\Dg{g} \;=\; \dfib + \nabla^{\mathrm{GM}}
\end{equation}
has two components: the curved fiberwise differential $\dfib$, whose
curvature element satisfies $m_1^2=[m_0^{(g)},-]$ and whose scalar
diagonal projection is
...
connection absorbs the projected fiberwise curvature
$\kappa(\cA) \cdot \omega_g$.
...
\begin{definition}[Local chiral open/closed pair; \ClaimStatusDefinitional]
...
cochains on algebra elements. Thus the
\(\mathsf{SC}^{\mathrm{ch,top}}\)-output is the typed pair
\[
(\mathbf{Z}^{\mathrm{der}}_{\mathrm{ch}}(\cA),\,\cA),
\]
with closed colour the cochain derived centre and open colour the
boundary algebra. Passing to cohomology gives the on-shell
Gerstenhaber algebra \(\mathcal{Z}^{\mathrm{der}}_{\mathrm{ch}}(\cA)\),
but terminality is a cochain-level statement. The bar coalgebra
\(\bar B(\cA)\) is the level-\(2\) twisting coalgebra used to compute
or resolve this object; it is not itself the
\(\mathsf{SC}^{\mathrm{ch,top}}\) closed colour and is not a physical
bulk without the OCA datum of Definition~\ref{def:thqg-oca-datum}.
\end{theorem}
...
conventions is given in Theorem~\ref{thm:chiral-deligne-tamarkin}.
\end{proof}
...
the cochain derived center is the OCA datum of
Definition~\ref{def:thqg-oca-datum}; in particular it contains:
\begin{enumerate}[label=\textup{(\roman*)}]
\item a boundary identification
...
\item compatibility with the
 $\mathsf{SC}^{\mathrm{ch,top}}$ mixed action on
 $(\mathbf{Z}^{\mathrm{der}}_{\mathrm{ch}}(\cA),\cA)$;
\item compatibility with the conformal-weight, pole-order, and
 analytic completions in which both sides are defined.
\end{enumerate}
...
\]
\[
K_X(\cA):=\mathbb{D}_{\Ran}\barB_X(\cA),\qquad
\nu_{\cA}\colon K_X(\cA)\longrightarrow\cA^!
\quad\text{is Verdier duality followed by the chosen-pair map,}
\]
whereas
...
identifying the represented collision kernel
$\rho(K_\cA^{\mathrm{coll}})$ with the relevant Yangian $R$-matrix.
\end{proposition}
...
scalar diagonal curvature $\kappa(\cA) \cdot \omega_g$ is closed-sector data, and
...
This is the center of the boundary category.  The completed formal
chiral boundary comparison has target
$\mathbf{Z}^{\mathrm{der}}_{\mathrm{ch}}(\cA)$: here $A_b$ is the
completed formal chiral boundary algebra induced from~$\cA$, and the
logarithmic finite-piece hypotheses of
Proposition~\ref{prop:geometric-algebraic-hochschild} hold.  Under
these hypotheses the geometric--algebraic comparison identifies
$\mathrm{HH}^{\bullet}_{\mathrm{alg}}(A_b)$ with the corresponding
chiral cochain model
$\mathcal{C}^{\bullet}_{\mathrm{ch}}(A_b, A_b)$.  A family datum
$H_H(A_b;S_b)$ then gives
\[
 \operatorname{Supp}\mathrm{HH}^{\bullet}_{\mathrm{alg}}(A_b)
 \subseteq S_b
\]
by transport along this comparison.  A perfect degree-$d$ pairing is
the additional datum governing any Koszul-dual degree reflection.
\end{corollary}

\begin{proof}
The first formula is the definition of the derived centre of the
boundary dg category. Morita invariance follows from the preceding
theorem, which identifies the centre in a compact-generator chart with
Hochschild cochains of $A_b$.  Proposition~\ref{prop:geometric-algebraic-hochschild}
supplies the final comparison, and the retract in $H_H(A_b;S_b)$
supplies the support set transported through it.
\end{proof}
...
Verdier duality sends the bar coalgebra directly to the homotopy
Koszul-dual algebra
\[
K_X(\cA):=\mathbb{D}_{\Ran}\barB_X(\cA).
\]
A chosen finite-type Koszul pair supplies an algebra map
$\nu_{\cA}\colon K_X(\cA)\to\cA^!$; its equivalence identifies
$\cA^!\simeq\mathbb D_{\Ran}(\cA^{\mathrm i})
\simeq(\cA^{\mathrm i})^\vee$ as the strict line-operator algebra
when the quadratic comparison
$q_\cA\colon\cA^{\mathrm i}\to\barB_X(\cA)$ is an equivalence.
The derived-center and categorical-trace
slots are respectively
$\mathbf{Z}^{\mathrm{der}}_{\mathrm{ch}}(\cA)$ and
$\operatorname{Tr}(\mathcal M)$.
\end{enumerate}
A derivation of the closed-sector scalar formula
$F_g=\kappa(\cA)\lambda_g$ on the uniform-weight surface from the
...
where
\[
B^{\mathrm{cyc}}_n(A_b)=A_b\otimes (sA_b)^{\otimes n}
\]
is the Hochschild cyclic-object model and \(d_{\mathrm{cyc}}\) is the
ordinary Hochschild boundary, including the wrap-around face.  Connes'
operator belongs to the cyclic/negative-cyclic refinement, not to this
ordinary Hochschild homology formula.
\end{theorem}
...
algebras in
Theorem~\ref{thm:circle-fh-hochschild}; the present proof
...
In a chart $\mathcal{M} \simeq \operatorname{Perf}(A_b)$
with compact generator~$b$, the interval evaluation
...
\end{align}
For the intersection term: the intersection
...
on the right interval term has the left action
...
Substituting into the excision equivalence:
...
\medskip
\noindent\textit{Step~4: The Hochschild chain model.}
The derived tensor product above is computed by resolving the diagonal
\(A_b\)-bimodule by the standard \(A_b^e\)-free bar resolution
\(B_\bullet(A_b,A_b,A_b)\):
\begin{equation}\label{eq:thqg-two-sided-bar}
B_n(A_b,A_b,A_b)
\;=\;
A_b \otimes (sA_b)^{\otimes n} \otimes A_b, \qquad n \ge 0.
\end{equation}
Here the two outside factors carry the \(A_b^e\)-module structure and
the middle tensor powers record the internal \(A_\infty\)-operations.
The differential $d_{\mathrm{bar}}$ on this resolution is
...
Tensoring this \(A_b^e\)-free resolution with the diagonal bimodule
\(A_b\) over \(A_b^e\) identifies the two external \(A_b\)-actions.
The resulting complex is the ordinary Hochschild chain complex
\[
C_n(A_b)
  = A_b\otimes (sA_b)^{\otimes n},
\qquad
b_{\mathrm{Hoch}}=b_{\mathrm{int}}+b_{\mathrm{face}}+
b_{\mathrm{wrap}},
\]
where \(b_{\mathrm{wrap}}\) is the last face term composing the final
input with the first one, with the usual Koszul sign; for an
\(A_\infty\)-algebra the same formula includes the higher \(m_k\) around
the cyclic order.  The notation \(B^{\mathrm{cyc}}(A_b)\) below denotes
this Hochschild cyclic-object model.  It is not the coinvariant quotient
of an ordinary ordered bar complex by a naive cyclic permutation, and the
Connes operator \(B\) belongs to the cyclic/negative-cyclic enhancement,
not to the ordinary Hochschild differential used in
\(\mathrm{HH}_{\bullet,\mathrm{alg}}(A_b)\).
...
\;=\; \mathrm{HH}_{\bullet,\mathrm{alg}}(A_b).
\end{equation}
The first equivalence is that the bar resolution computes the derived
tensor product (it is a free $A_b^e$-resolution of $A_b$; see
Loday~\cite[\S5.3]{Loday98} and
Loday--Vallette~\cite[\S2.2]{LV12}). The second is the definition of
algebraic Hochschild homology in chain complexes.
...
\begin{theorem}[Open/closed MC equation; \ClaimStatusConditional]
...
\nabla^{\mathrm{hol}}).
\]
Here a chosen presentation supplies
$\cA^{\mathrm i}=C_X(s^{-1}V,s^{-2}R)$ and
$q_\cA\colon\cA^{\mathrm i}\to B^{\mathrm{ch}}(\cA)$.
The Verdier algebra is
$K_X(\cA)=\mathbb D_{\Ran}B^{\mathrm{ch}}(\cA)$, its quadratic
comparison is
$\mathbb D(q_\cA)\colon K_X(\cA)\to
\mathbb D_{\Ran}(\cA^{\mathrm i})$, and the chosen line algebra enters
through $\nu_\cA\colon K_X(\cA)\to\cA^!$.  The object
...
completed datum. The
$q_\cA$-comparison identifies
$H^\bullet(\cA^{\mathrm i})$ with chiral bar cohomology under
$H_{\mathrm{CL}}$; the chosen-pair map
$\nu_\cA\colon K_X(\cA)\to\cA^!$ reaches the line algebra from the
Ran--Verdier algebra under the finite-type or completed continuous
pair hypotheses; $r(z)$ comes from the
...
On the uniform-weight surface, this is the genus tower
...
$\kappa(\cA)\lambda_g$ on the uniform-weight surface;
...
is proved on the uniform-weight surface and at genus~$1$ for all
...
$\cA^{\mathrm i}$ & quadratic presentation coalgebra
 & $C_X(s^{-1}V,s^{-2}R)$ with $q_\cA\colon\cA^{\mathrm i}\to B^{\mathrm{ch}}(\cA)$ \\
$\cA^!$ & chosen Koszul-pair line algebra
 & $\nu_\cA\colon\mathbb D_{\Ran}B^{\mathrm{ch}}(\cA)\to\cA^!$ \\
$\mathbf{Z}^{\mathrm{der}}_{\mathrm{ch}}(\cA)$
...
$K_\cA^{\mathrm{coll}}(z)$ & binary collision kernel; represented
classical $r$-matrix under $H_{\mathrm{CYBE}}^{\mathrm{rep}}$
...
$\cA^{\mathrm i}=C_X(s^{-1}V,s^{-2}R)$ is the quadratic presentation
coalgebra, $q_\cA$ compares it with the full bar,
$K_X(\cA)=\mathbb D_{\Ran}B^{\mathrm{ch}}(\cA)$ is the Ran--Verdier
algebra, $\nu_\cA$ reaches the chosen line algebra~$\cA^!$, and
...
\begin{remark}[Terminology: completed datum, not ADE/McKay]
\label{rem:thqg-platonic-terminology}
\index{completed modular Koszul datum!terminology}
The legacy word ``platonic'' in some labels and indices names this
completed modular Koszul datum: a canonical package of open-sector,
bar--Koszul, derived-centre, trace, and resonance data. It is not the
Platonic-solids/ADE/McKay usage of the word. No ADE root system,
finite subgroup of $\mathrm{SU}_2$, McKay quiver, or ADE
classification is part of Definition~\ref{def:thqg-completed-platonic-datum}
unless such data are introduced by a separate theorem with its own
hypotheses. When ADE/Kummer/McKay material appears in the K3 or
Calabi--Yau comparisons, it is a distinct specialization, not the
meaning of the completed modular Koszul datum.
\end{remark}
...
$\mathsf{SC}^{\mathrm{ch,top}}$-brace closed-sector theorem
\textup{(}identifying $\partial = \cA$, algebraic closed-sector actor
$= Z^{\mathrm{der}}_{\mathrm{ch}}(\cA)$, interaction
$= \mathsf{SC}^{\mathrm{ch,top}}$-brace; comparison with a physical
bulk is the additional OCA datum below, and the gravity-line completion
