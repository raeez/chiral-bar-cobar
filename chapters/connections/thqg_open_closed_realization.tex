...
\xrightarrow{\sim} \operatorname{Spec} \mathbb{C}[[z]]$.  The following four-step
chain produces a canonical isomorphism of non-$\Sigma$ operads:
...
\item \textbf{D-module trivialisation.}
  On the formal disk $\widehat{D}_p$, every coherent D-module
  trivialises: the global-sections functor
  $\Gamma(\widehat{D}_p, -) \colon D\text{-}\mathrm{mod}(\widehat{D}_p)
  \to \mathrm{Vect}$ is an equivalence of symmetric monoidal
  categories.  The chiral algebra $\cA|_{\widehat{D}_p}$ becomes a
  graded vector space $V = A_p$ equipped with a translation
  operator $\partial = \nabla_{d/dz}$.
...
\begin{proof}
Step~2 follows from the contractibility of the formal disk:
$\pi_1(\widehat{D}_p) = 0$, so every regular flat connection on
$\widehat{D}_p$ trivialises (regularity is inherited from the
standing conventions of
\S\ref{sec:standing-assumptions}), and $\Gamma(\widehat{D}_p, -)$ is
an exact tensor equivalence.  Step~3 is the definition of the
$j_* j^*$-functor applied to the trivialised sheaf: sections
with poles along the diagonal $z_i = z_j$ become formal Laurent
series in the difference variables $\lambda_i = z_i - z_n$.
...
(cf.\ Fulton--MacPherson~\cite{}; the formal-disk
...
$\operatorname{Aut}(\mathcal{O})$-equivariance is immediate from the
...
curvature $\kappa(\cA) \cdot \omega_g$ is purely closed-sector. Genus-raising
operations act only on closed inputs; open inputs are passive. Beware
: the ``mixed channels'' of the multi-weight correction
...
trace is $\mathrm{THH}$, but the present theorem remains in chain
complexes.
...
upgrades
$\mathrm{HH}_{\bullet,\mathrm{alg}}(A_b)
\simeq \mathrm{HH}^{2 - *}_{\mathrm{alg}}(A_b)$ by Poincar\'e
...
\end{equation}
where
$\Delta_{\mathrm{ns}} \colon \mathrm{HH}_{\bullet,\mathrm{cat}}(\mathcal{M})
...
$\operatorname{Tr}_\cA \in \mathrm{HH}_{0,\mathrm{cat}}(\mathcal{M})$ by contracting
...
chiral Hochschild cochain complex by
Theorem~\ref{thm:thqg-local-global-bridge}(ii).
The shadow obstruction tower projections (iv) follow from the degree filtration
...
conjectural content.
\begin{enumerate}[label=\textup{(\roman*)}]
\item \emph{Phase 1: Bordered FM construction.}
...
\item \emph{Phase 2: Chain-level $D^2_{\mathrm{oc}} = 0$.}
...
\item \emph{Phase 3: Annulus trace at genus~$1$.}
...
\item \emph{Phase 4: Categorical lift existence.}
...
\item \emph{Phase 5: $(\infty,2)$-uniqueness.}
...
\section{Synthesis}
\label{sec:thqg-occ-wave13-DNA}

\begin{remark}[Class-$\mathcal S$ on $\Sigma_{0,24}$ as the open-closed realisation]
\label{rem:thqg-occ-DNA-1}
The class-$\mathcal S$ theory $\mathcal T[A_1,\Sigma_{0,24}]$ has
$c_{4d} = 107/6$, $c_{2d} = -214$, Coulomb rank $21$, flavour
$\mathfrak{su}(2)^{24}$ (Wave~15 canonical values; see
Remark~\ref{rem:thqg-occ-wave15-wave14-retraction} for the
double-retraction of the intermediate Wave~14 evaluation
$(26, -312)$), providing a $4d$/$2d$ open-closed realisation
in which the Schur sector is the K3 chiral algebra
$\mathbf H_{\Delta_5}$-module. The $24$ punctures on $\Sigma_{0,24}$
host the $24$ flavour $\mathfrak{su}(2)$'s and correspond to the $24$
Kodaira $I_1$ fibres $E^{\mathrm{nod,sm}}_{24}$ in the Wave~13 bi-based
Ran space. This is the class-$\mathcal S$ face of the six coordinate
charts on $\mathbf H_{\Delta_5}$ \cite{Gaiotto2009,BeemRastelli2015}.
Cross-ref Vol.~III Chapter~\ref{ch:k3-chiral-bialgebra-platonic}.
\end{remark}

\begin{remark}[Bi-based Ran structure in the open-closed TFT]
\label{rem:thqg-occ-DNA-2}
The Wave~13 bi-based Ran space $(\mathrm{Ran}(E^{\mathrm{nod,sm}}_{24}),\,\bar{\mathcal A}_2)$
provides the open-closed TFT's two-base structure: the open (boundary)
sector lives on $\mathrm{Ran}(E^{\mathrm{nod,sm}}_{24})$ (i.e., $24$
boundary defects at the Kodaira $I_1$ fibres) and the closed (bulk)
sector lives on $\bar{\mathcal A}_2$ (genus-$2$ Siegel upper
half-space, Kuga--Satake target). The Steiner $S(5,8,24)$ rigidity
(Conway--Sloane 1988) cuts out the unique $M_{24}$-symmetric $24$-point
boundary configuration compatible with the open-closed Cardy condition
\cite{ConwaySloane1988,KugaSatake1967}. Cross-ref
...
admits an $\Omega$-deformation refinement compatible with the
open/closed structure. On the open sector
$\mathrm{Ran}(E^{\mathrm{nod,sm}}_{24})$ the two equivariant
parameters $(\epsilon_1,\epsilon_2)$ act by rotation of the two
transverse planes at each Kodaira $I_1$ node; the derived-centre
pair $(\mathcal{Z}^{\mathrm{der}}_{\mathrm{ch}}(\mathbf{H}_{\Delta_5}),
\mathbf{H}_{\Delta_5})$ carries an induced
$T^2 = U(1)_{\epsilon_1}\times U(1)_{\epsilon_2}$ action whose
equivariant trace is the $\Omega$-deformed K3 partition function
$Z^{K3}_{\mathrm{inst}}(\epsilon_1,\epsilon_2;\vec a)$ computed by
Nekrasov--Okounkov equivariant localisation
\cite{Nekrasov2003,NekrasovOkounkov2006}. The universal-bulk
identification
$\mathrm{Obs}^{\mathrm{bulk}}(T_{\mathbf{H}_{\Delta_5}})\simeq
\mathcal{Z}^{\mathrm{der}}_{\mathrm{ch}}(\mathbf{H}_{\Delta_5})$
(Theorem~\ref{thm:thqg-chiral-derived-center-universal}, via the
Vol~II master theorem) sends the $\Omega$-deformed bulk trace to
the self-dual-phase Siegel character
\[
...
\]
The programme's $\hbar^2 = -1/8$ specialisation thus fixes
$\epsilon^2 = 1/8$, i.e.\ $\epsilon = \pm 1/(2\sqrt{2})$; this is
the Nekrasov--Shatashvili self-dual point
\cite{NekrasovShatashvili2009} at which the $\Omega$-deformed
theory reduces to a quantum-integrable system with Bethe Ansatz
equations, and at which the Maulik--Okounkov stable-envelope
$R$-matrix is precisely the $R_{\mathrm{Sieg,dyn}}$ of the K3
Hall--Drinfeld double
\cite{MaulikOkounkov12,SchiffmannVasserot13}. The Humbert
monodromy identity $\mathrm{ord}(H_1) = K^\kappa = 2c_+ = 8$
(Wave~13 Beilinson B-1) coincides with the order of the
Maulik--Okounkov braid-group representation at the self-dual
fixed-point. Primary-literature frame: Nekrasov~2003
...
\begin{remark}[Character matching: Nekrasov = chiral character]
\label{rem:thqg-occ-character-match}
A Nekrasov-voice falsification rule (``if the character does not
match $Z_{\mathrm{inst}}$, the construction is wrong'') applies
directly to $\mathbf{H}_{\Delta_5}$: the Schur-limit of the
$\Omega$-deformed K3 partition function computed by
plethystic-exponential expansion of the class-$\mathcal S$
$\mathcal{T}[A_1,\Sigma_{0,24}]$ $4d$/$2d$ shadow (compute module
\texttt{k3\_yangian\_wave14\_schur\_index\_classS\_A1\_24.py}) reads
\[
...
\[
  ...
  \Delta_5
\]
is Wave~13 Gaiotto~W13-Gai-1 and is falsifiable at
Proposition~(platonic, \S~k3-schur-q-expansion): any Fourier
coefficient mismatch through $q^{10}$ falsifies the
class-$\mathcal S$ origin.
\end{remark}

\section{Synthesis}
\label{sec:thqg-occ-wave15-DNA}

\begin{remark}[The $N$-parametric family
  $\mathbf H^{(N)}_{\Delta_5}$ at $N \ge 2$]
\label{rem:thqg-occ-wave15-N-parametric}
...
Wave~13 base point $\mathcal T[A_1, \Sigma_{0,24}]$ extends to a
family
\[
...
\]
indexed by $N$; the Wave~14 object is the $N = 2$ entry.
\end{remark}
...
Shapere--Tachikawa formulas gives the table. Compute module:
\texttt{k3\_yangian\_wave15\_schur\_index\_classS\_ANm1\_24.py}
(\texttt{calabi-yau-quantum-groups/compute/lib/}).
\end{proof}

\begin{remark}[Retraction of Wave~14's Chacaltana--Distler formula]
\label{rem:thqg-occ-wave15-wave14-retraction}
Wave~14 asserted
$c_{4d}(A_1, \Sigma_{g,n}) = (12(g-1) + 7n)/6$ based on an ansatz
that fails the $n = 4$ cross-check: at $(g, n) = (0, 4)$ it returns
...
and $c_{2d} = -214$, coinciding with the boxed Wave~13 value
(\texttt{k3\_chiral\_bialgebra\_platonic.tex} line~1095). The
Wave~14 retraction $c_{4d} = 26$ is itself retracted here. The
earlier Wave~13 $a_{4d} = 421/24$ differs from the first-principles
$a_{4d} = 403/24$ by $18/24 = 3/4$; the source of this discrepancy
is flagged as open. The six-fold pentagon-umbral
$\mathbb{Z}/6$ anomaly identified in Wave~14 is independent of this
central-charge correction (it lies in $H^2(M_{24}, U(1))$, not in the
$c_{4d}$ data) and survives unchanged.
\end{remark}

\begin{remark}[$\mathfrak{su}(N)^{24}$ flavour subalgebra and
  $M_{24}$ permutation]
\label{rem:thqg-occ-wave15-flavour}
...
$M_{24}$-equivariant with the same projective Schur cocycle of
order $6$ as in Wave~14 (Theorem~k3-projective-M24-equivariance,
Vol.~III).
\end{remark}

\begin{remark}[Siegel lift at higher $N$:
  weight$-k_N$ cusp form on $\mathrm{Sp}_{2g+2}(\mathbb{Z})$;
  conjectural]
\label{rem:thqg-occ-wave15-siegel}
...
\texttt{arXiv:alg-geom/9609022} Theorem~13.3). For $\mathcal T[A_{N-1},
\Sigma_{0,24}]$ at $N \ge 2$, the natural target of the analogous
lift is a Siegel modular form on the paramodular group
$\Gamma_0^{\mathrm{para}}(k_{\mathrm{lvl}})\subset
\mathrm{Sp}_{2\,r(N)}(\mathbb{Z})$ of signature determined by the
Lorentzian lattice $\Lambda^{N+1, 2}$ (rank $= n_{\mathrm{Cou}} + 2
= 21(N-1) + 2$ in the Maulik--Okounkov normalisation), with
\emph{weight} fixed by the Borcherds rank formula
$\mathrm{wt} = \tfrac{1}{2}\mathrm{rank}\,\Lambda$. At $N = 2$ this
returns $\mathrm{wt} = 5$, matching $\Delta_5$; at $N \ge 3$ the
formula predicts $\mathrm{wt} = \tfrac{1}{2}(21(N-1) + 2)$, which
at $N = 3$ gives $\mathrm{wt} = 22$, a Siegel-modular weight consistent
with paramodular-form existence results of Gritsenko--Hulek 1998
\texttt{arXiv:alg-geom/9609022}. The existence and automorphic
identification of $\Delta_{k_N}^{(N)}$ for $N \ge 3$ is a
CONJECTURE; the $N$-independent CY-to-chiral functor $\Phi$
(Vol.~III Chapter~\ref{ch:cy-to-chiral-functor}) predicts
existence, but the BKM-root structure at $N \ge 3$ does not reduce
to Mathieu moonshine and may lack a closed automorphic form.
...
\label{rem:thqg-occ-wave15-k3-specificity}
The K3-specificity of Wave~13's choice
...
$\mathcal T[A_{N-1}, \Sigma_{g,n}]$ for $g \ge 1$) not connected to
K3 by the $\Phi$-arrow, so the name $\mathbf H^{(N)}_{\Delta_5}$ is
reserved for the $(g, n) = (0, 24)$ branch.
\end{remark}

\begin{remark}[CoHA vs chiral: $\mathbf H^{(N)}_{\Delta_5}$ is a chiral
  bialgebra, not a CoHA]
\label{rem:thqg-occ-wave15-coha-vs-chiral}
...
object: it is an $E_1$-chiral bialgebra on the curve
$E^{\mathrm{nod,sm}}_{24}$ in the Ran topology. The CY-to-chiral
functor $\Phi$ (Vol.~III) relates them by $\Phi(\mathrm{CoHA}_{\mathrm{K3},
N}) \simeq \mathbf H^{(N)}_{\Delta_5}$ on the Koszul locus, but
this is a functorial identification, not an equality. Conflation
of CoHA with chiral algebra is the cache-pattern error
\texttt{coha\_ne\_chiral} (AP-CY\,61 in Vol.~III); the rank-$N$
extension preserves the same care of distinction.
\end{remark}
