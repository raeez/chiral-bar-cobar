...
$V = (\mathfrak{g}^e)^*$, generators from the Slodowy slice &
 $A^! = H^*(\widehat{\overline{B}}(A))^\vee = \widehat{\Lambda}_\partial(sV)$ \\
normal-ordering corrections in $F^{\leq p} \setminus F^{<p}$ &
 canonical $A_\infty$ Taylor coefficient $m_p$ \\
canonical homotopy degree &
 maximal $r$ with $m_r \neq 0$
\end{tabular}
\end{center}
\noindent
For the Bershadsky--Polyakov algebra ($\mathcal{W}_3^{(2)}$), the
correction is purely quadratic and the dual is strict (degree $2$).
For $\mathcal{W}_4^{(2)}$ a genuine cubic term survives (degree $\geq 3$;
Theorem~\ref{thm:w4-cubic}).
In higher rank the canonical degree grows without bound.
...
\mathrm{Slodowy\text{-}slice\ free\ differential\ polynomial\ }\operatorname{gr}_F A)$\textup{]}
...
k\ne -h^\vee;
\mathrm{Arakawa\ Kazhdan\text{-}filtration\ theorem\ for\ principal\ DS})$\textup{]}
...
\]
The first axis is now proved on a substantial type~$A$ corridor via
...
\mathrm{KSDual\ sublocus\ when\ }\eta=\eta^t;
\mathrm{Vol\ III\ }(\Sigma_{d-1},C)\text{-specialisation\ for\ stage\text{-}2\ shadow\ at\ }d=3,\ C\ \mathrm{the\ Koszul\ comparison\ curve})$\textup{]}
...
\]
The self-transpose hook partitions ($\eta=\eta^t$, e.g.\ $(2,1)$ in
$\mathfrak{sl}_3$) lie in the KSDual sublocus
\textup{(}Convention~\textup{\ref{conv:master-type-signature}};
$\mathbb{Z}/2$-fixed locus of $\cA\mapsto\cA^!$\textup{)}, where
the displayed equivalence is a strict involution rather than an
adjunction with comparison map.
\end{theorem}
...
\begin{proposition}[Hook-type compatibility criteria; \ClaimStatusConditional]
...
\begin{theorem}[Generator-degree detection of canonical degree; \ClaimStatusProvedHere]
...
of generator degree $>d$, then no Taylor coefficient of degree $>d$ can appear.
Conversely, a nonzero degree-$d$ term in the fixed normal form contributes a
nonzero degree-$d$ Taylor coefficient under dualization. Since we are speaking
...
Theorem~\ref{thm:canonical-degree-detection} concerns the canonical completed
dual attached to a chosen strong generating set. Whether the same degree
...
For higher $n$ we obtain an exact canonical-degree theorem and formulate a
minimal-degree conjecture.
\end{remark}
...
products created by the screening differential; the free-field factor
contributes no additional higher operations.
\end{remark}
...
\mathrm{canonical\ normal\ form\ at\ generators\ }J,G^\pm,T)$\textup{]}
For every $k\neq -3$, the canonical normalized completed dual of the
Bershadsky--Polyakov algebra $\mathcal W_3^{(2)}(k)$ has
\[
d_r=0 \qquad (r\geq 3).
\]
Equivalently, its canonical homotopy degree is $2$. The canonical
$d=2$ statement is at the chosen normal form; the minimal-model
transferred dual is conjectured to inherit the same maximal nonzero
degree under
Conjecture~\textup{\ref{conj:hook-canonical-minimal}}, but
``quadratic generator OPE'' alone does not entail equivalence with
any all-loop quantum HT theory.
\end{theorem}
...
$E(z)F(w)$. Apply Theorem~\ref{thm:canonical-degree-detection} with $d=2$.
\end{proof}

\begin{computation}[Modular characteristic of $\mathrm{BP}_k$; \ClaimStatusConditional]
...
\begin{proposition}[Complementarity constant for $\mathrm{BP}_k$; \ClaimStatusConditional]
...
\mathrm{level}=1\leftrightarrow 2,
\mathrm{hypothesis\ package}=k\ne -3;
\mathrm{subregular\ DS/bar\ transport};
\mathrm{KSDual\ sublocus\ since\ }(2,1)=(2,1)^t;
\mathrm{Prochazka\text{--}closed\ OPE\ for\ }\mathrm{BP};
\mathrm{Yamada\ analytic\ continuation\ at\ generic\ }k;
\mathrm{modularity\ here\ refers\ to\ the\ open\text{-}category\ trace\ +\ clutching\ on\ }(X,D,\tau),\ \mathrm{not\ a\ closed\text{-}algebra\ property\ of\ }\mathrm{BP};
\mathrm{Vol\ III\ }(\Sigma_2,C)\text{-specialisation\ at\ }d=3)$\textup{]}
For $k \neq -3$ \textup{(}away from the critical level\textup{)}, assume the subregular DS/bar transport statement. Then the Bershadsky--Polyakov algebra
$\mathcal W_k(\mathfrak{sl}_3, f_{\mathrm{sub}})$, the subregular
non-principal $\mathcal W$-algebra of $\mathfrak{sl}_3$, has
same-family Koszul companion under the hook-type transport duality.
The hook partition
$(2,1)$ is self-transpose, $(2,1)^t = (2,1)$, so $\mathrm{BP}$ lies in
the KSDual sublocus and its Koszul dual is again Bershadsky--Polyakov
at the Feigin--Frenkel dual level $k^\vee = -k - 6$. In the
...
current\textup{)}. This is the first non-principal same-family
companion fixed locus in the standard Lie-theoretic landscape; the
algebraic Koszul self-duality interpretation is conditional on the
...
\begin{theorem}[\texorpdfstring{$\mathcal W_4^{(2)}$}{W4(2)} has canonical degree $3$; \ClaimStatusProvedHere]
\label{thm:w4-cubic}
\textup{[}Type signature:
$(\mathrm{Open}\cap\mathrm{Chain},\mathrm{chain\text{-}level},
\mathrm{level}=1\leftrightarrow 2,
\mathrm{hypothesis\ package}=(k+2)(11k+32)\ne 0;
\mathrm{Feigin\text{--}Semikhatov\ canonical\ normal\ form};
\mathrm{Yamada\ generic\text{-}level\ finite\ presentation};
\mathrm{Penkava\text{--}Vanhaecke\ PVA\text{-}to\text{-}quantum\ bridge\ for\ minimal\text{-}model\ passage};
\mathrm{statement\ records\ a\ canonical\text{-}normal\text{-}form\ degree\text{-}3\ obstruction,\ not\ the\ full\ }A_\infty\text{-}\mathrm{minimal\ degree})$\textup{]}
For every $k$ with $(k+2)(11k+32)\neq 0$, the canonical normalized completed
dual of $\mathcal W_4^{(2)}(k)$ has
\[
d_3\neq 0.
\]
In particular, its canonical homotopy degree is at least $3$.
The canonical-degree statement is at the chosen Feigin--Semikhatov
normal form; promotion to a minimal-model statement
\textup{(}Conjecture~\textup{\ref{conj:hook-canonical-minimal}}\textup{)}
requires the named PBW endpoint package
\textup{(}Yamada finite presentation;
Penkava--Vanhaecke PVA-to-quantum bridge\textup{)}.
\end{theorem}
...
Theorem~\ref{thm:canonical-degree-detection} implies $d_3\neq 0$.
\end{proof}
...
\begin{theorem}[Unbounded canonical degree in the subregular line; \ClaimStatusProvedHere]
\label{thm:unbounded-canonical-degree}
\textup{[}Type signature:
$(\mathrm{Open}\cap\mathrm{Chain},\mathrm{chain\text{-}level},
\mathrm{level}=1\leftrightarrow 2,
\mathrm{hypothesis\ package}=\mathrm{generic\ level\ }k;
\mathrm{Feigin\text{--}Semikhatov\ centered\ Appell\ normal\ form};
\mathrm{Bell\text{-}recursion\ for\ raw\ singular\ coefficients};
\mathrm{Prochazka\text{--}Rapcak\ }W_n^{(2)}\mathrm{\ closure\ for\ minimal\text{-}model\ passage};
\mathrm{KZ\ analytic\ SDR\ for\ classical\text{-}to\text{-}quantum\ continuation})$\textup{]}
For generic level $k$ and every $n\geq 3$, the canonical normalized completed
dual of $\mathcal W_n^{(2)}(k)$ has nonzero Taylor coefficients in all degrees
\[
2\leq r\leq n-1.
\]
Equivalently, the canonical homotopy degree of the subregular family is
unbounded as $n\to\infty$. The unboundedness statement is at the
canonical-normal-form level; passage to the minimal-model transferred
dual is conjectural \textup{(}Conjecture~\textup{\ref{conj:minimal-degree-subregular}}\textup{)}.
\end{theorem}

\begin{proof}
For $n=3$ strictness gives canonical degree $2$ by
Theorem~\ref{thm:bp-strict}. For $n=4$, degree $3$ is nonzero by
Theorem~\ref{thm:w4-cubic}.
...
Theorem~\ref{thm:canonical-degree-detection}. Therefore the canonical completed
dual has nonzero Taylor coefficients in each degree $r\leq n-1$.
\end{proof}

\begin{conjecture}[Minimal degree survives generically; \ClaimStatusConjectured]
\label{conj:minimal-degree-subregular}
For generic $k$, the minimal transferred completed dual of
$\mathcal W_n^{(2)}(k)$ has maximal nonzero degree exactly $n-1$. Equivalently,
the canonical degrees detected in
Theorem~\ref{thm:unbounded-canonical-degree} survive minimal-model transfer.
\end{conjecture}
...
\item \emph{Strict versus higher duality.} The canonical homotopy degree is
 ...
 \mathcal W_n^{(2)} \text{ has unbounded canonical degree as } n\to\infty.
 \]
\end{enumerate}
...
second is cubic, and thereafter the canonical degree grows linearly with the
rank.

\begin{conjecture}[Hook-type canonical-versus-minimal coincidence; \ClaimStatusConjectured]
\label{conj:hook-canonical-minimal}
For generic level, the maximal nonzero degree of the canonical normalized
completed dual agrees with the maximal nonzero degree of the minimal transferred
...
functions of $k$.
\item Upgrade canonical degree to minimal degree by proving
...
centered-Appell normal form reveals the growth of homotopy degree. And the
...
For $\fg = \mathfrak{sl}_N$ with $N \geq 3$, the subregular
partition $(N-1, 1)$ yields a $\mathcal{W}$-algebra whose
structure at generic level is anticipated to admit a conformal
embedding of $\cW^{k'}(\mathfrak{sl}_{N-2})$ tensored with an
auxiliary $\mathfrak{sl}_2$-current sector and a $\beta\gamma$
pair. This picture is classical for the first nontrivial case
$N = 3$, where it degenerates to Bershadsky--Polyakov
$\mathrm{BP} = \cW^k(\mathfrak{sl}_3, f_{(2,1)})$
\textup{(}Computation~\textup{\ref{comp:bp-bar}})}, and the
relevant $\beta\gamma$-type reduction is inscribed as
Proposition~\ref{prop:ds-bar-hook-commutation}(iii). The general
$N \geq 4$ decomposition is a structural expectation consistent
with the Kac--Roan--Wakimoto \cite{KRW03} BRST construction and
with Arakawa's generic-level Kazhdan filtration
\cite{Arakawa07}, but the verbatim identification
$\cW^k(\mathfrak{sl}_N, f_{\mathrm{subreg}}) \simeq
\cW^{k'}(\mathfrak{sl}_{N-2}) \otimes \mathrm{sl}_2\text{-current}
\otimes \beta\gamma$
is inscribed here \emph{only} at $N = 3$. The propagation of
Koszul-duality commutation along this decomposition is covered by
Conjecture~\ref{conj:ds-kd-arbitrary-nilpotent}; the semisimple-Levi
window $r \leq N - 3$ of
Proposition~\ref{prop:hook-pbw} excludes the subregular partition
$(N-1, 1)$ for every $N \geq 3$, so the subregular route is not a
corollary of the hook-corridor theorem.
\item \emph{Minimal via coset.}
The minimal nilpotent orbit yields a $\mathcal{W}$-algebra
$\cW^k(\fg, f_{\mathrm{min}})$ whose generator spectrum has a
distinguished Heisenberg-plus-triplet form and admits a
conjectural coset description generalising the
Bershadsky--Polyakov case. In type $A$ with $\fg = \mathfrak{sl}_N$
and $N \geq 4$, the minimal and subregular orbits differ
(they coincide only at $N = 3$, where both are
$(2,1)$). A verbatim theorem of the form ``minimal
$\mathcal{W}$-algebra equals a specific coset at every $N$'' is
not inscribed in this volume; the minimal route is documented at
$N = 3$ (via BP) and sketched via the Kac--Roan--Wakimoto BRST
complex \cite{KRW03} at generic level. Koszulness at generic
level holds by Arakawa's Kazhdan filtration \cite{Arakawa07};
Koszul-duality commutation is conditional under
Conjecture~\ref{conj:ds-kd-arbitrary-nilpotent}.
\item \emph{Screening-kernel Koszul-locus preservation.}
The statement ``the screening-kernel preserves the Koszul locus''
is used implicitly in the hook-type corridor
\textup{(}Theorem~\textup{\ref{thm:hook-transport-corridor})} and
in the parabolic extension of the
$\mathrm{MC4}^0$ SDR for non-principal hook-type
(standalone \texttt{N4\_mc4\_completion.tex}). The precise
content is: for a screening set
$\{Q_\alpha\}_{\alpha \in \Delta_P^+}$ defining
$\cW^k(\fg, f) = \bigcap_\alpha \ker Q_\alpha$ inside a parent
lattice-or-Heisenberg--Fock VOA~$B$ at generic~$k$, the induced
subalgebra inherits PBW-free strong generation from~$B$ whenever
the screening kernel is compatible with the conformal weight
grading and the Fock parent is itself Koszul. At generic~$k$
the compatibility holds for hook partitions by direct
conformal-weight matching; beyond the hook case the
compatibility statement is the structural content of
Conjecture~\ref{conj:ds-kd-arbitrary-nilpotent}.
\end{enumerate}
\end{remark}

\section{subregular / hook frontier and K3 anomalous classes}
\label{sec:subreg-supplement}

\begin{remark}[Anomalous $M_{24}$ classes at the subregular hook frontier]
\label{rem:subreg-anomalous-classes}
At the subregular / hook frontier, the Mathieu-moonshine anomalous classes of
$M_{24}$, namely $\{7A, 7B, 11A, 23A, 23B\}$, are non-Mukai (not realised by any K3
complex-structure polarisation; Gaberdiel--Hohenegger--Volpato $2012$) and carry
non-trivial generalised-moonshine anomaly phases in $H^3(M_{24}, \mathrm{U}(1)) \supseteq
\mathbb{Z}/12$ (generalised-Mathieu-moonshine 't~Hooft anomaly,
Gaberdiel--Persson--Ronellenfitsch--Volpato 2013). The Serre functor on
$D^b\mathrm{Coh}(K3)$ under the projective $\widetilde{M}_{24}$ cover induced by the
$H^3$ anomaly acquires an order-$6$ cocycle (Bridgeland--Maciocia $2001$); the
subregular hook frontier receives additional twist from the anomalous-class
phases. Andrianov spinor $L$-function $L(s, \Delta_{10}) = L(s, \Delta_{E_6})\zeta(s-9)\zeta(s-8)$
gives the Hecke eigenvalue structure at the anomalous primes.
\end{remark}
