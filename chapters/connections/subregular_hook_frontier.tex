...
first explicit non-abelian comparison surface
...
hook family and is included as the first rectangular comparison surface.
\end{computation}
...
\begin{remark}[Transport-to-transpose scalar calculation]
...
\begin{computation}[Bershadsky--Polyakov scalar calculation;
\ClaimStatusProvedHere]
...
Direct substitution in the displayed rational central-charge functions gives
the standard FKR identity $K^c_{\mathrm{BP}}=50$ and the shifted secondary
identity $K^{c,\mathrm{shift}}_{\mathrm{BP}}=196$.  The four generators
\(J,T,G^+,G^-\) are even.  The genus-$1$ invariants remain open because no
curvature comparison is supplied.
...
$\alpha_b(n,k)$ and $\beta_b(n,k)$ are lengthy.  The exact Bell recursion
together with the triangular projection mechanism determines the full
coefficient packets without listing each coefficient separately.
\end{remark}
...
\begin{proposition}[Finite-rank transport calculation for type $A$;
\ClaimStatusConditional]
...
\begin{remark}[Even and self-transpose orbit diagnostics]
...
For the self-transpose BP partition $(2,1)$, the scalar calculation gives
$K^c_{\mathrm{BP}}=50$.
\end{remark}
...
The following data organise the first comparison beyond the hook spine for
...
\noindent\textit{Comparison classes beyond hooks.}
...
$\mathsf{B}$ (self-tr.\ rect.) & $\lambda = \lambda^t$, rectangular &
 KRW calculation & open comparison \\
...
\noindent
The $(2,2)$ partition is the first rectangular comparison surface;
