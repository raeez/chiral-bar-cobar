...
Three independent paths give $\kappa(\mathrm{BP}_k) = (k-15)/(6(k+3))$:
\begin{enumerate}
\item \emph{Anomaly ratio}:
  ...
  = \frac{1}{6}.
  \]
  Since $c(\mathrm{BP}_k) = (k-15)/(k+3)$
  \textup{(}the KRW central charge for the
  $\mathfrak{sl}_3$ subregular reduction\textup{)}, the modular
  characteristic is $\kappa = \varrho \cdot c = (k-15)/(6(k+3))$.
\item \emph{Deficit from affine $\mathfrak{sl}_3$}:
  ...
  is verified by direct subtraction:
  \[
  \frac{4(k{+}3)}{3} - \frac{k{-}15}{6(k{+}3)}
  = \frac{8(k{+}3)^2 - (k{-}15)}{6(k{+}3)}
  = \frac{8k^2 + 47k + 87}{6(k{+}3)}.
  \]
\item \emph{Complementarity}:
  the Feigin--Frenkel dual level is $k^\vee = -k-6$, so
  $c(k^\vee) = (k+21)/(k+3)$ and
  $\kappa(k^\vee) = (k+21)/(6(k+3))$.  The sum
  \[
  \kappa(k) + \kappa(k^\vee)
  = \frac{k-15}{6(k+3)} + \frac{k+21}{6(k+3)}
  = \frac{2k+6}{6(k+3)}
  = \frac{1}{3}
  = \varrho \cdot K_{\mathrm{BP}},
  \]
  confirming the $k$-independent complementarity with
  Koszul conductor $K_{\mathrm{BP}} = 2$.
\end{enumerate}
Special values: $\kappa(15) = 0$ (uncurved bar complex),
$\kappa(0) = -5/6$, $\kappa(1) = -7/12$.
...
$K_{\mathrm{BP}} = c(\mathrm{BP}_k) + c(\mathrm{BP}_{-k-6})$
equals~$2$, independent of the level~$k$.
\end{proposition}

\begin{proof}
Direct calculation:
$c(k) + c(-k-6) = (k-15)/(k+3) + (-k-21)/(-k-3)
= (k-15)/(k+3) + (k+21)/(k+3) = (2k+6)/(k+3) = 2$.
\end{proof}
