...
DS/bar compatibility across the hook network. The type-$A$
abelian-$\mathfrak{n}_+$ locus is not used here as a global theorem:
``all hook partitions are abelian'' is not a proved input. The safe
...
degree under the hook-minimal transfer conjecture, but
...
\textup{(}the hook-minimal transfer conjecture\textup{)}
...
dual is conjectural \textup{(}the subregular minimal-degree conjecture\textup{)}.
\end{theorem}
