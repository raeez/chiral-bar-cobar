...
\]
which is finite at every genus: the integration domain $\overline{\mathcal{M}}_g$ is compact, the integrand is a smooth cohomology class, and the Maurer--Cartan equation $D_\cA\Theta_\cA + \tfrac{1}{2}[\Theta_\cA,\Theta_\cA] = 0$ provides an exact genus-by-genus Ward identity.  No UV counterterms enter.  The resulting genus expansion $\sum_g \hbar^g \mathcal{A}_g^{\mathrm{tw}}$ converges absolutely for $|\hbar| < 4\pi^2$ and extends meromorphically to~$\mathbb{C}$; any literal bulk-gravity amplitude interpretation is additional HT dictionary structure.
...
uniform-weight lane ($\mathrm{obs}_g = \kappa \cdot \lambda_g$, cf.\
...
On the uniform-weight lane, the genus-$g$ free energy depends only on
...
The bosonic string partition function has $F_g^{\mathrm{string}} \sim (2g-2)!\,\alpha'^{-g}$ (factorial growth from the noncompactness of the $d$-dimensional target space $\mathbb{R}^d$).  The shadow tower genus expansion of a chiral algebra on a compact curve has $F_g(\cA) = \kappa(\cA) \cdot \lambda_g^{\mathrm{FP}}$ with $|\lambda_g^{\mathrm{FP}}| = \frac{|B_{2g}|}{2g(2g{-}2)} \sim \frac{2}{(2\pi)^{2g}}$, which decays exponentially.  The genus expansion converges precisely because the bar complex is defined on compact Fulton--MacPherson spaces $\overline{C}_n(X)$, requiring no UV regularization: the compactness of the underlying curve~$X$ suffices (Remark~\ref{rem:thqg-I-spectral-finiteness}).
\end{remark}
...
\]
is finite at every genus, converges absolutely for $|\hbar| < 4\pi^2$, and admits a meromorphic continuation to $\mathbb{C}$ with explicit pole structure.  No UV regularization or renormalization is required.
\end{remark}
