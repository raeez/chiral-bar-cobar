...
The boundary chiral algebra~$\cA$ controls the bulk through the holographic modular Koszul datum $\mathcal{H}(T) = (\cA, \cA^!, \mathcal{C}, r(z), \Theta_\cA, \nabla^{\mathrm{hol}})$.  The genus-$g$ integrated shadow functional is
\[
...
The HS-sewing condition guarantees that sewing amplitudes on pairs of pants compose to give trace-class operators on the sewing Hilbert space, bridging the finite-dimensional OPE data of~$\cA$ to infinite-genus partition functions.
...
Since each $H_n$ is finite-dimensional, different choices of Hermitian norm on $H_n$ give equivalent topologies on $\mathcal{H}_q(\cA)$.  The $q$-weighting ensures that high-weight states are exponentially suppressed.  For $q \to 1^-$, the Hilbert space degenerates; for $q \to 0^+$, only the vacuum sector $H_0$ survives.
\end{remark}
