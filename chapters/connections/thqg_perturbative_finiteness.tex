...
The holographic modular Koszul datum $\mathcal{H}(T) = (\cA, \cA^!, \mathcal{C}, r(z), \Theta_\cA, \nabla^{\mathrm{hol}})$ encodes the bulk through the boundary chiral algebra~$\cA$. The genus-$g$ integrated shadow functional is
\[
\mathcal{A}_g^{\mathrm{tw}}
\;=\; \int_{\overline{\mathcal{M}}_g}
 \operatorname{Sh}_{g,0}(\Theta_\cA),
\]
which is finite at every genus: the integration domain
$\overline{\mathcal{M}}_g$ is compact, the integrand is a smooth
cohomology class, and the Maurer--Cartan equation
$D_\cA\Theta_\cA + \tfrac{1}{2}[\Theta_\cA,\Theta_\cA] = 0$ provides an
exact genus-by-genus Ward identity. No UV counterterms enter. On the
uniform-weight lane (Definition~\ref{def:scalar-lane}) one further has
$\mathcal{A}_g^{\mathrm{tw}}=\kappa(\cA)\lambda_g^{\mathrm{FP}}$, so the
scalar genus expansion $\sum_g \hbar^g \mathcal{A}_g^{\mathrm{tw}}$
converges absolutely for $|\hbar| < 4\pi^2$ and extends meromorphically
to~$\mathbb{C}$; for arbitrary modular Koszul algebras the manuscript
only proves genuswise finiteness and the universal genus-$1$ scalar
identity. Any literal bulk-gravity amplitude interpretation is
additional HT dictionary structure.
...
The HS-sewing condition forces pair-of-pants amplitudes to compose as trace-class operators on the sewing Hilbert space, bridging the finite-dimensional OPE data of~$\cA$ to infinite-genus partition functions.
...
Since each $H_n$ is finite-dimensional, all Hermitian norms on $H_n$ yield equivalent topologies on $\mathcal{H}_q(\cA)$. The $q$-weighting exponentially suppresses high-weight states: for $q \to 1^-$ the Hilbert space degenerates; for $q \to 0^+$ only the vacuum sector $H_0$ survives.
\end{remark}
...
A genus-$g$ surface with $n$ punctures decomposes into $2g - 2 + n$ pairs of pants sewn along $3g - 3 + n$ circles, each contributing one composition of pair-of-pants operators. The HS condition forces each composition to be Hilbert--Schmidt; the composition of two Hilbert--Schmidt operators is trace class, the requirement for the partition-function trace. The sewing modulus is $q = e^{-t}$ with $t > 0$ the collar length.
\end{remark}
...
A pants graph $\Gamma_P$ determines the decomposition of $\Sigma_{g,n}$ into $2g - 2 + n$ trinions and $3g - 3 + n$ sewing circles. Different choices of $\Gamma_P$ give different factorizations of the sewing operator, but the trace is independent of $\Gamma_P$ by the factorization property. The flip graph connects pants decompositions by elementary moves (Dehn twists and handle slides); invariance under flips follows from the pentagon and hexagon identities of the chiral algebra.
\end{remark}
...
The polynomial-OPE criterion is not optimal: examples with super-polynomial but sub-exponential OPE growth (such as certain completed W-algebras) may still satisfy HS-sewing. The criterion covers the entire standard landscape of finitely strongly generated universal chiral algebras (for simple quotients at admissible levels, see Remark~\ref{rem:thqg-I-admissible}). The bound~\eqref{eq:thqg-I-hs-criterion-bound} is loose by factors of order $n^3$ due to the crude estimate $\sum_{a+b+c=n} \dim H_a \dim H_b \dim H_c \leq (\sum_{j \leq n} \dim H_j)^3$; tighter bounds are available for specific families.
\end{remark}

\begin{remark}[The gap between HS-sewing and convergence]
\label{rem:thqg-I-gap-hs}
\index{HS-sewing!necessary vs sufficient}
The HS-sewing condition is sufficient for absolute convergence of the genus-$g$ partition function but not necessary. Weaker conditions (e.g., trace-class sewing without the Hilbert--Schmidt factorization) may suffice at specific genera. The advantage of the HS condition is uniformity: a single verification at the OPE level yields finiteness at every genus.
\end{remark}
...
\item The HS norm:
 ...
 \|m_q\|_{\HS}^2 \;\leq\; C^2\,\prod_{n=1}^\infty(1-q^n)^{-4} \;<\; \infty \quad\text{for all } q \in (0,1).
 \end{equation}
\end{itemize}
The exponent $-4$ in $\prod(1-q^n)^{-4}$ reflects the two bosonic degrees of freedom ($\beta$ and $\gamma$), each contributing $\prod(1-q^n)^{-2}$ from the respective partition function. (The Dedekind eta $\eta(q) = q^{1/24}\prod(1-q^n)$ includes a $q^{1/24}$ prefactor that is absent from the HS bound, which counts modes without vacuum-energy normalization.)
\end{computation}
...
\item The HS norm factors as
 ...
 \|m_q\|_{\HS}^2 \;\leq\; C_\Lambda^2\,\prod_{n=1}^\infty(1-q^n)^{-2r} \,\Theta_\Lambda(q)^2 \;<\; \infty \quad\text{for all } q \in (0,1),
 \end{equation}
 where $\Theta_\Lambda(q) = \sum_{\alpha \in \Lambda} q^{\langle\alpha,\alpha\rangle/2}$ is the lattice theta function.
\end{itemize}
The factorization reflects the tensor-product structure $V_\Lambda = \mathcal{H}^{\otimes r} \otimes \mathbb{C}[\Lambda]$: the Heisenberg part contributes $\prod_{n=1}^\infty(1-q^n)^{-2r}$ (the $r$-colored partition function squared, without the $q^{r/24}$ vacuum prefactor of the Dedekind eta) and the lattice group ring contributes $\Theta_\Lambda(q)^2$.
\end{computation}
...
\begin{corollary}[Genus-$g$ partition function; \ClaimStatusProvedHere]
...
The scalar projection of the Maurer--Cartan element $\Theta_\cA$ onto genus~$g$ is the shadow free energy $F_g(\cA)$. This is the degree-$0$, genus-$g$ component of the shadow algebra $\cA^{\mathrm{sh}} = H_\bullet(\Defcyc^{\mathrm{mod}}(\cA))$ (Definition~\ref{def:shadow-algebra}).
...
The shadow free energy $F_g(\cA)$ occupies the $(r,g) = (0,g)$ slot of the shadow algebra $\cA^{\mathrm{sh}} = H_\bullet(\Defcyc^{\mathrm{mod}}(\cA))$, bigraded by degree~$r$ and genus~$g$. Higher-degree shadows at genus $g$ (the cubic shadow $\mathfrak{C}_g$ at $r = 3$, the quartic resonance class $\mathfrak{Q}_g$ at $r = 4$) encode gravitational interactions beyond the free-energy level. The shadow free energy is the simplest invariant, controlled by the modular characteristic $\kappa(\cA)$ alone.
\end{remark}
...
lane, the scalar formula fails for multi-weight algebras at
...
For any modular Koszul chiral algebra $\cA$ whose strong generators
have the same conformal weight:
\begin{equation}\label{eq:thqg-I-Fg-formula}
\boxed{ F_g(\cA) \;=\; \kappa(\cA) \cdot \frac{2^{2g-1}-1}{2^{2g-1}} \cdot \frac{|B_{2g}|}{(2g)!}\,. }
\end{equation}
In particular, $F_g$ depends only on $g$
and the single scalar $\kappa(\cA)$.
For multi-weight algebras, the unconditional scalar identity is
$F_1(\cA) = \kappa(\cA)/24$.
\end{proposition}
...
(Proposition~\ref{prop:thqg-I-faber-pandharipande}).
\end{proof}
...
If $\cA$ is a modular Koszul algebra on the uniform-weight lane (all strong generators have the same conformal weight), then the
...
On the uniform-weight lane, the scalar shadow free energies are given by the
...
At genus $g = 1$: $(2^{2 \cdot 1 - 1} - 1)/2^{2 \cdot 1 - 1} = 1/2$, $|B_2| = 1/6$, $2! = 2$, so $F_1 = \kappa \cdot \frac{1}{2} \cdot \frac{1/6}{2} = \kappa/24$.

At genus $g = 2$: $(2^{2 \cdot 2 - 1} - 1)/2^{2 \cdot 2 - 1} = (2^3 - 1)/2^3 = 7/8$, $|B_4| = 1/30$, $4! = 24$. $F_2 = \kappa \cdot \frac{7}{8} \cdot \frac{1/30}{24} = \frac{7\kappa}{5760}$.
...
\item \emph{Degree cutoff.} At each genus~$g$, only finitely many graph topologies contribute (Lemma~\ref{lem:degree-cutoff}). The number of contributing graphs at genus $g$ and degree $r$ is bounded by a combinatorial function of $(g,r)$, and each graph amplitude is a finite integral.
\end{enumerate}
\end{proof}
...
\begin{proof}
Part~(i) is the degree-cutoff lemma (Lemma~\ref{lem:degree-cutoff}). The set $\Gmod(g,r)$ of connected modular graphs with loop genus $g$ and total degree $\leq r$ is finite: the number of vertices $v$, edges $e$, and external legs $n$ satisfy $v - e + n = 1 - g$ (the Euler characteristic), $2e + n \leq rv$ (each vertex has at most $r$ half-edges), and $g = e - v + 1 + \sum_{v'} g_{v'}$ where $g_{v'}$ is the genus decoration at each vertex. These constraints bound $v, e, n$ in terms of $g$ and $r$.
...
$F_g^{(r)}$ is the degree-$r$ correction. On the scalar lane,
$F_g^{\mathrm{scal}} = \kappa \cdot \lambda_g^{\mathrm{FP}}$ for
all $g \geq 1$ on the proved scalar lane. For arbitrary modular
Koszul algebras, the unconditional scalar statement is
$F_1^{\mathrm{scal}}=\kappa/24$. For
...
\end{equation}
where $\det{}'_\zeta$ denotes the zeta-regularized determinant of the scalar Laplacian $\Delta_{\Sigma_g}$ on $\Sigma_g$ \textup{(}excluding the zero mode\textup{)}. On the Gaussian scalar lane one also has
...
\begin{proof}
The analytic partition-function formula is
Theorem~\ref{thm:heisenberg-sewing}. The scalar free-energy formula is
the Gaussian specialization of
Proposition~\ref{prop:thqg-I-Fg-closed-form} at
$\kappa(\mathcal{H}_k)=k$. No direct coefficient-extraction theorem
from the analytic determinant to the algebraic scalar free energy is
proved here.
\end{proof}
...
The Fredholm determinant at genus $2$ involves the sewing of three
pairs of pants along three circles. In the Schottky uniformization,
$\Sigma_2 = (\mathbb{P}^1 \setminus \{6 \text{ discs}\}) /
\langle \gamma_1, \gamma_2, \gamma_3 \rangle$ with three sewing
parameters $q_1, q_2, q_3$. The one-particle sewing operator is
$K_2 = R_1 \circ R_2 \circ R_3$, and
$\det(1 - K_2) = \prod_{n_1,n_2,n_3 \geq 1}
(1 - q_1^{n_1} q_2^{n_2} q_3^{n_3})$. This gives the analytic
genus-$2$ partition function; the scalar free energy
$F_2(\mathcal{H}_k)=7k/5760$ is a separate algebraic output.
\end{example}
...
\]
This exponential formula gives a convergent analytic expansion of
$\log Z_g$ in the sewing moduli. On the Gaussian lane, the scalar
free energy is known independently from
$\kappa(\mathcal{H}_k)=k$ and the universal Bernoulli formula; a
direct coefficient-extraction theorem from $\log Z_g$ is not proved
here. The one-particle reduction is specific to the Heisenberg
(Gaussian shadow depth $r_{\max} = 2$): for non-Gaussian algebras, the
multi-particle sector contributes genuinely new data that cannot be
recovered from the one-particle space.
\end{remark}
...
\end{equation}
which converges absolutely for all $q_i \in (0,1)$. This yields a
well-defined analytic expansion of $\log Z_g$ in the sewing
parameters; no extraction of the algebraic shadow free energy from
this expansion is proved here.
\end{proposition}
...
\end{equation}
No comparison theorem identifying~\eqref{eq:thqg-I-btz-full} with the
full analytic BTZ partition function is proved here.
\end{theorem}
...
shadow method provides the scalar series~\eqref{eq:thqg-I-btz-full} on
the Virasoro scalar lane, but its identification with the full analytic
BTZ partition function is additional dictionary data not proved here.
\end{remark}
...
The shadow depth classification ($G/L/C/M$ for Gaussian/Lie/Contact/Mixed) stratifies the current THQG dictionary by nonlinear complexity:
\begin{enumerate}[label=\textup{(\alph*)}]
...
\item \emph{Lie-type ($r_{\max} = 3$):} Affine Kac--Moody algebras. The first nonlinear shadow term is cubic, and no higher transferred genus-$0$ degree survives.
\item \emph{Contact-type ($r_{\max} = 4$):} $\beta\gamma$ system. The cubic term vanishes, the quartic term survives, and the shadow obstruction tower terminates after two nonlinear steps.
\item \emph{Mixed ($r_{\max} = \infty$):} Virasoro, $\mathcal{W}_N$. Infinitely many degrees survive, so the full shadow obstruction tower is needed. The quintic obstruction $o^{(5)}_{\mathrm{Vir}} \neq 0$ forces the tower to be genuinely infinite.
\end{enumerate}
Within the current HT dictionary, the Brown--Henneaux boundary algebra $\mathrm{Vir}_c$ lies in the mixed class.
\end{remark}
...
$r_{\max}(\cA)$ (shadow depth) & gravitational complexity \\
	$4\pi^2$ (scalar-lane radius) & Hagedorn temperature \\
$Q^{\mathrm{contact}}$ (quartic invariant) & two-loop graviton self-energy \\
$\cA^!$ (Koszul dual) & dual boundary conditions \\
\end{tabular}
\end{center}
The passage from boundary data to bulk amplitudes is algorithmic: given
the OPE data of $\cA$, the modular characteristic $\kappa$ is
computable, the genuswise shadow integrals are finite, and on the
proved scalar lane the closed form
$Z_{\mathrm{grav}}^{\mathrm{scal}} = \kappa \cdot x/(2\sin(x/2))$
follows. No UV regularization, no renormalization group flow, and no
anomaly matching are required.
\end{remark}
...
The perturbative finiteness of twisted quantum gravity rests on a chain
of three implications:
...
$\Theta_\cA := D_\cA - d_0$ is automatically Maurer--Cartan because the
bar differential squares to zero. The second implication is the
projection onto genus~$g$: the shadow free energy is the integral of a
cohomology class over a compact moduli space, hence finite. On the
proved scalar lane this genuswise finiteness is supplemented by the
closed formula $F_g=\kappa\lambda_g^{\mathrm{FP}}$; in general the
manuscript only proves the universal genus-$1$ scalar identity
$F_1=\kappa/24$. The entire mechanism is algebraic (no analysis of
divergences, no regularization, no renormalization) and exact (no
perturbative truncation, no asymptotic expansion).
\end{remark}
...
Under HS-sewing, the graph sum~\eqref{eq:thqg-I-graph-sewing} converges absolutely at each genus $g$. The number of contributing graphs is finite at each degree level, and the degree sum converges:
\begin{equation}\label{eq:thqg-I-graph-sum-degree}
...
\item If $r_{\max} = \infty$ (mixed class), then the degree sum converges at each genus by the shadow obstruction tower convergence theorem \textup{(}Theorem~\ref{thm:recursive-existence}\textup{)}, so the full genus-$g$ amplitude is well defined for every fixed $g$.
...
\begin{table}[ht]
\centering
\caption{Modular characteristics, shadow depths, and shadow depth classes for the standard landscape. Higher-genus scalar entries are shown only where the scalar factorization is proved.}
...
\begin{proposition}[Effective bound on the genus-$g$ scalar free energy on the scalar lane; \ClaimStatusProvedHere]
\label{prop:thqg-I-effective-bound}
\index{shadow free energy!effective bound}
For any modular Koszul chiral algebra $\cA$ with modular
characteristic $\kappa$ and HS-sewing
parameter $q_0 \in (0,1)$, the genus-$g$ scalar shadow free energy
satisfies the effective bound
...
\begin{corollary}[Tail bound for the scalar genus expansion on the scalar lane; \ClaimStatusProvedHere]
...
\item \emph{Exponential decay on the proved scalar lane.} On the
scalar lane, the free energies
$F_g \sim \kappa/(2\pi)^{2g}$ decay exponentially, not factorially. The
scalar genus series converges, so there is no Borel resummation
ambiguity or renormalon divergence there.
\end{enumerate}
\end{remark}
...
with $\lambda = N/(k+N)$ fixed, the central charge is
$c_N = (N-1) - N(N^2-1)(k+N-1)^2/(k+N) \sim -N^2\lambda$ for large $N$. The
modular characteristic scales as $\kappa \sim N^2$, so the universal
genus-$1$ scalar term scales as $F_1=\kappa/24 \sim N^2/24$. If the
conjectural higher-genus scalar factorization extended to
$\mathcal{W}_N$, one would expect
$F_g \sim N^2/(2\pi)^{2g}$ and the closed form
\[
Z_{\mathrm{grav}}^{\mathcal{W}_N}(\hbar) \;\approx\; N^2 \cdot \frac{\sqrt{\hbar}/2}{\sin(\sqrt{\hbar}/2)},
\]
but for $\mathcal{W}_N$ with $N \geq 3$ this remains heuristic because
the higher-genus scalar factorization is not proved. In particular, no
all-genus convergence-radius statement is established here for the
multi-weight large-$N$ family.
\end{remark}
...
The Fredholm determinant formula for the Heisenberg partition function admits a spectral-theoretic reformulation via the Selberg zeta function, giving an independent computation of the shadow free energy.
...
The finiteness of the Heisenberg partition function follows from the discreteness and positivity of the Laplacian spectrum on a compact hyperbolic surface. The eigenvalues $\lambda_n > 0$ for $n \geq 1$ (by compactness), the spectral zeta function converges for $\Re(s) > 1$ (by the Weyl law $\lambda_n \sim Cn$), and the zeta regularization provides a canonical way to define $\det{}'_\zeta\Delta$. The analytic continuation to $s = 0$ is smooth because $\Sigma_g$ is compact. No UV regularization enters: the compactness of $\Sigma_g$ is the only input.
\end{remark}
...
For the Heisenberg at level $k$, the integrand is
$|\mu_g|^{-2k/13} \cdot (\det\operatorname{Im}\Omega)^{-k}$. This
analytic picture is compatible with the algebraic scalar formula
$F_g(\mathcal{H}_k)=k\lambda_g^{\mathrm{FP}}$, but no direct
coefficient-extraction theorem from the integrand to $F_g$ is proved
here.
\end{remark}
...
\begin{remark}[Shadow corrections beyond the Gaussian sector]\label{rem:shadow-corrections-beyond-gaussian}
\index{shadow tower!corrections beyond Gaussian}
\index{effective action!shadow decomposition}
The Polyakov formula computes the Gaussian (free-field) sector of the worldsheet effective action exactly. For interacting chiral algebras, the shadow obstruction tower provides systematic corrections at each degree $r \geq 3$: the cubic shadow~$\mathfrak{C}$ at $r = 3$ (tree-level nonlinearity from the Lie bracket), the quartic contact invariant $\mathfrak{Q}^{\mathrm{contact}}$ at $r = 4$ (the first stratum-separation correction from $\overline{C}_4(X)$), and higher obstruction classes $o^{(r+1)}$ at each subsequent degree. These corrections vanish for the Gaussian class~(G), terminate at~$r = 3$ for the Lie class~(L), at~$r = 4$ for the contact class~(C), and persist to all orders for the mixed class~(M). The full effective action decomposes as
\[
S_{\mathrm{eff}}(\cA) = \sum_{r=2}^{r_{\max}} S_{\mathrm{eff}}^{(r)}(\cA)
\]
with $S_{\mathrm{eff}}^{(2)} = \kappa \cdot S_{\mathrm{Polyakov}}$.
\end{remark}
...
\item Residues $\Res_{\hbar = \hbar_n} Z^{\mathrm{scal}} = 8(-1)^n \kappa n^2\pi^2$.
\item No zeros: $Z^{\mathrm{scal}}(\hbar) \neq 0$ for all $\hbar \in \mathbb{C} \setminus \{4\pi^2 n^2\}_{n \geq 1}$.
...
\begin{proof}
(i)--(ii): The zeros of $\sin(\sqrt{\hbar}/2)$ occur at $\sqrt{\hbar}/2 = n\pi$, i.e., $\hbar = 4\pi^2 n^2$. Writing $s(\hbar) = \sin(\sqrt{\hbar}/2)$, the derivative is $s'(\hbar) = \cos(\sqrt{\hbar}/2)/(4\sqrt{\hbar})$, so $s'(\hbar_n) = (-1)^n/(8n\pi)$. The numerator at the pole is $g(\hbar_n) = \kappa \cdot n\pi$. Hence
\[
\Res_{\hbar = \hbar_n} \frac{\kappa\sqrt{\hbar}/2}{\sin(\sqrt{\hbar}/2)}
= \frac{g(\hbar_n)}{s'(\hbar_n)}
= \frac{\kappa n\pi}{(-1)^n/(8n\pi)}
= 8(-1)^n \kappa n^2\pi^2.
\]
...
\end{equation}
which converges for $\hbar \notin \{4\pi^2 n^2 : n \geq 1\}$. The poles at $\hbar = 4\pi^2 n^2$ are immediate; the expansion is the Mittag-Leffler decomposition of $\sqrt{\hbar}/(2\sin(\sqrt{\hbar}/2))$.
\end{remark}
...
\begin{enumerate}[label=\textup{(\arabic*)}]
\item \emph{Non-Koszul algebras.} The theorem applies to modular Koszul chiral algebras. All standard landscape algebras (Heisenberg, affine Kac--Moody, Virasoro, $\beta\gamma$, $\mathcal{W}_N$, lattice VOAs) are chirally Koszul by Theorem~MC1; ``non-Koszul'' is not a property any of these families acquires. For hypothetical non-Koszul algebras outside the standard landscape, the MC recursion may fail to terminate at each degree and finiteness would require separate analysis.
