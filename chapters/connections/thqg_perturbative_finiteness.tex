...
The shadow free energy $F_g(\cA)$ occupies the $(r,g) = (0,g)$ slot of the shadow algebra $\cA^{\mathrm{sh}} = H_\bullet(\Defcyc^{\mathrm{mod}}(\cA))$, bigraded by arity~$r$ and genus~$g$.  Higher-arity shadows at genus $g$ (the cubic shadow $\mathfrak{C}_g$ at $r = 3$, the quartic resonance class $\mathfrak{Q}_g$ at $r = 4$) encode gravitational interactions beyond the free-energy level.  The shadow free energy is the simplest invariant, controlled by the modular characteristic $\kappa(\cA)$ alone.
\end{remark}
...
\begin{table}[ht]
\centering
\caption{Shadow free energies $F_g(\cA) = \kappa \cdot \lambda_g^{\mathrm{FP}}$ \textup{(UNIFORM-WEIGHT)} for three standard families, $g = 1, \ldots, 5$.}
...
$F_g^{(r)}$ is the arity-$r$ correction.  On the scalar lane,
...
\item \emph{Lie-type ($r_{\max} = 3$):} Affine Kac--Moody algebras.  The first nonlinear shadow term is cubic, and no higher transferred genus-$0$ arity survives.
...
\item \emph{Mixed ($r_{\max} = \infty$):} Virasoro, $\mathcal{W}_N$.  Infinitely many arities survive, so the full shadow obstruction tower is needed.  The quintic obstruction $o^{(5)}_{\mathrm{Vir}} \neq 0$ forces the tower to be genuinely infinite.
\end{enumerate}
...
\begin{proposition}[Polyakov formula as Chern--Weil image of the arity-$2$ shadow; \ClaimStatusProvedHere]
\label{prop:polyakov-chern-weil}
\index{Polyakov formula!Chern--Weil image}
\index{shadow tower!arity-2 evaluation}
The Polyakov formula (Proposition~\ref{prop:thqg-I-polyakov}) is the Chern--Weil image of the arity-$2$ shadow.  For a chirally Koszul algebra~$\cA$ with modular characteristic $\kappa = \kappa(\cA)$, the evaluation of the arity-$2$ shadow $\Theta_\cA^{\leq 2} = \kappa \cdot \eta \otimes \Lambda$ against a conformal deformation $g_1 = e^{2\sigma}g_0$ on~$\Sigma_g$ yields
...
\]
so the conformal variation scales linearly in~$\kappa$.  Since the arity-$2$ shadow $\Theta_\cA^{\leq 2} = \kappa \cdot \eta \otimes \Lambda$ is concentrated in arity~$2$, its Chern--Weil evaluation against~$\sigma$ equals $\kappa$ times the single-boson evaluation, giving~\eqref{eq:polyakov-chern-weil}.  For a general chirally Koszul algebra~$\cA$, the same formula holds by universality of the modular characteristic (Theorem~\ref{thm:modular-characteristic}): $\kappa(\cA)$ is the unique invariant controlling the arity-$2$ shadow, and the conformal variation is linear in~$\kappa$.
\end{proof}
...
The Polyakov formula computes the Gaussian (free-field) sector of the worldsheet effective action exactly.  For interacting chiral algebras, the shadow obstruction tower provides systematic corrections at each arity $r \geq 3$: the cubic shadow~$\mathfrak{C}$ at $r = 3$ (tree-level nonlinearity from the Lie bracket), the quartic contact invariant $\mathfrak{Q}^{\mathrm{contact}}$ at $r = 4$ (the first stratum-separation correction from $\overline{C}_4(X)$), and higher obstruction classes $o^{(r+1)}$ at each subsequent arity.  These corrections vanish for the Gaussian class~(G), terminate at~$r = 3$ for the Lie class~(L), at~$r = 4$ for the contact class~(C), and persist to all orders for the mixed class~(M).  The full effective action decomposes as
\[
