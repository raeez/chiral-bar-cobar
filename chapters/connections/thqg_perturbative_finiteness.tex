...
The Polyakov formula computes the Gaussian (free-field) sector of the worldsheet effective action exactly.  For interacting chiral algebras, the shadow tower provides systematic corrections at each arity $r \geq 3$: the cubic shadow $C$ at $r = 3$ (tree-level nonlinearity), the quartic contact invariant $\mathfrak{Q}^{\mathrm{contact}}$ at $r = 4$ (one-loop worldsheet correction), and higher obstruction classes $o^{(r+1)}$ at each subsequent arity.  These corrections vanish for the Gaussian class~(G), terminate at~$r = 3$ for the Lie class~(L), at~$r = 4$ for the contact class~(C), and persist to all orders for the mixed class~(M).  The full effective action decomposes as
\[
...
The bosonic string partition function has $F_g^{\mathrm{string}} \sim (2g-2)!\,\alpha'^{-g}$ (factorial growth from the noncompactness of the $d$-dimensional target space).  The shadow tower genus expansion of a chiral algebra on a compact curve has $F_g(\cA) = \kappa(\cA) \cdot \lambda_g^{\mathrm{FP}}$ with $|\lambda_g^{\mathrm{FP}}| = \frac{|B_{2g}|}{2g(2g-2)} \sim \frac{2}{(2\pi)^{2g}}$, which decays exponentially.  The genus expansion converges precisely because the bar complex is defined on compact configuration spaces, requiring no UV regularization.
\end{remark}
