...
If $\cA$ has generators of arbitrary conformal weights, then the
...
If $\cA$ has generators of arbitrary conformal weights, then using the
...
\item \emph{Scalar partition function.}  For uniform-weight modular Koszul algebras, the series $\sum_{g=1}^{\infty} F_g(\cA)\,\hbar^g$ converges absolutely for $|\hbar| < 4\pi^2$, with the closed-form expression $Z_{\mathrm{grav}}^{\mathrm{scal}}(\cA;\,\hbar) = \kappa(\cA) \cdot \frac{\sqrt{\hbar}/2}{\sin(\sqrt{\hbar}/2)}$.  For arbitrary modular Koszul algebras, the unconditional scalar statement is only $F_1(\cA)=\kappa(\cA)/24$; the all-genera closed form requires the strong scalar ansatz of Theorem~\ref{thm:multi-generator-universality}, proved on the uniform-weight lane and open in general for multi-weight families.
...
As the level $k$ varies, the modular characteristic $\kappa(k) = td/(2h^\vee)$ with $t = k + h^\vee$ traces a linear function of $k$.  The critical level $k = -h^\vee$ is the unique singular fiber where $\kappa$ vanishes.  Near the singular fiber, the scalar shadow free energies $F_g(k)$ tend to $0$ uniformly: $|F_g(k)| \leq C |k + h^\vee| / (2\pi)^{2g}$ for all $g$.  At $k = -h^\vee$, the scalar class vanishes and the bar complex is uncurved, but this does not imply $\Theta_\cA = 0$ or collapse of the full higher-arity shadow tower.
\end{remark}
...
\item \emph{Contact-type ($r_{\max} = 4$):} $\beta\gamma$ system.  The cubic term vanishes, the quartic term survives, and the shadow tower terminates after two nonlinear steps.
\item \emph{Mixed ($r_{\max} = \infty$):} Virasoro, $\mathcal{W}_N$.  Infinitely many arities survive, so the full shadow tower is needed.  The quintic obstruction $o^{(5)}_{\mathrm{Vir}} \neq 0$ forces the tower to be genuinely infinite.
\end{enumerate}
...
\]
For $N \geq 3$, the higher-genus scalar factorization remains open on
the current record, so no all-genus closed scalar partition function
is asserted here.  The difference from the Virasoro case lies in the
non-scalar corrections: $\mathcal{W}_N$ has shadow depth
$r_{\max} = \infty$ and the shadow tower includes contributions from
...
$\mathbb{R}^d$).  On the proved scalar lane, the shadow tower genus
expansion satisfies
