...
Killing form, satisfies the classical Yang--Baxter equation
...
$\Omega/((k+h^\vee)z)$.  This is the explicit formula~\eqref{eq:hdm-face-5}.
...
$r^{\mathrm{cl}} = r^{\mathrm{Dr}} = \Omega/z$ is the classical
...
$r^{\mathrm{cl}} = r^{\mathrm{Dr}} = \Omega/z$ the antisymmetric
...
residue $r_{\widehat{\fg}_k}(z) = \Omega/((k+h^\vee)z)$ becomes the
...
(Theorem~\ref{thm:hdm-face-5}).  Substituting into the
Gaiotto--Zeng formula~\eqref{eq:hdm-face-4} of Face~4 and rescaling
by $(k+h^\vee)$ produces~\eqref{eq:hdm-face-7}, which is the
...
\item \emph{Inverse spectral parameter:} $\hbar = 1/z$ in the formal
  expansion $r^{\mathrm{Dr}}(z) = \Omega/z = \Omega \cdot \hbar$,
