...
a tangential log curve $(X,D,\tau)$, the stable presentable
factorisation category $\mathsf C$ with compact opposite
$\cC^\op=(\mathsf C^\omega)^\op$ at level~$0$, a factorisation
compact generator $b$, the chart algebra
$A_b=\RHom_{\mathsf C}(b,b)$ at level~$1$, the derived chiral centre
...
\item the stable presentable factorisation category $\mathsf C$ on
$(X,D,\tau)$ and its compact opposite
$\cC^\op=(\mathsf C^\omega)^\op$, level~$0$;
\item the factorisation compact generator $b=(b_U)$;
\item the chart algebra
$A_b(U)=\RHom_{\mathsf C(U)}(b_U,b_U)$, level~$1$;
...
is not the level-$3$ closed sector;
$Z^{\mathrm{der}}_{\mathrm{ch}}(A_b)$ is the universal closed sector,
and a physical bulk interpretation requires the
...
$b\in\mathsf C^\omega$ on $(X,D,\tau)$; the symbol is shorthand for
...
chart algebra~$\cA = A_b$, its presentation coalgebra~$\cA^{\mathrm i}$
and full bar~$\barBch_X(\cA)$, the
...
\begin{enumerate}[label=\textup{(\arabic*)}]
\item \emph{Boundary slot.} $\cA=A_b=\RHom_{\mathsf C}(b,b)$ is the
 chart algebra of the factorisation compact generator~$b$, sitting as the boundary factor of
...
\item \emph{Quadratic coalgebra slot.}
 A chosen presentation supplies
 $\cA^{\mathrm i}:=C_X(s^{-1}V,s^{-2}R)$ and
 $q_\cA\colon\cA^{\mathrm i}\to\barBch_X(\cA)$.
 The full bar remains the chain coalgebra~$\barBch_X(\cA)$;
 $H^\bullet(q_\cA)$ compares their cohomology on the Koszul locus.
...
~$\cA^{!}_\infty$ when the rectification map
 ...
 $(\cA^{\mathrm i})^\vee$ is the finite-type quadratic target of the Verdier
 construction.
...
\item \emph{Collision-kernel slot.} We pin the residue entry to the
 ...
 \[
 K^{\mathrm{coll}}_{\mathrm{Heis},k}(z)=\frac{k}{z}\,J\otimes J,
 \qquad
 K^{\mathrm{coll}}_{KM,\fg,k}(z)=\frac{k\,\Omega_{\mathrm{tr}}}{z},
 \]
 and
 \[
 K^{\mathrm{coll}}_{\mathrm{Vir},c}(z)=\frac{c/2}{z^3}+\frac{2T}{z}.
 \]
 A classical $r$-matrix is a represented image
 $r_\cA=\rho_\cA(K_\cA^{\mathrm{coll}})$ under
 $H_{\mathrm{CYBE}}^{\mathrm{rep}}(\cA;W)$.  On the affine KZ lane,
 $r^{KM,\fg}_{\mathrm{KZ},k}(z)=
 \Omega_{\mathrm{KZ}}/((k+h^\vee)z)$ after the stated comparison map.
...
characteristic is the traced binary coordinate
\[
 \kappa(\cA)=\tau_{\cA,2}^{\mathrm{res}}
 q_2(K_\cA^{\mathrm{coll}}),
\]
and, on the Brown--Henneaux comparison lane of
Theorem~\ref{thm:genus-universality}, the coefficient in the
$3$d partition-function comparison. The shadow obstruction tower
$\{S_r(\cA)\}_{r \geq 2}$ is the scalar sequence
\[
 S_r(\cA)=\tau_{\cA,r}^{\mathrm{res}}
 q_r\bigl((\Theta_\cA^{E_1})_{0,r}\bigr),
\]
defined when the family supplies the higher residue/trace
functionals. Cross-channel towers for multi-generator
...
$\mathrm{Ass}^{\mathrm{ch}}\hookrightarrow\mathrm{SC}^{\mathrm{ch,top}}$,
witnessed by $(K_\cA^{\mathrm{coll}}(z),\Theta_\cA,\nabla^{\mathrm{hol}})$ on the
named modular-Koszul / boundary-linear / principal DS surfaces
(introduction, Remark~\ref{rem:five-facets-boundary-bulk}).

The collision kernel~$K_\cA^{\mathrm{coll}}(z)$ is the residue entry
of this datum.  On a strict matched-pair surface carrying
$H_{\mathrm{CYBE}}^{\mathrm{rep}}$, its represented image is the
classical term of the spectral $R$-matrix of~$U_\cA$; intrinsically it
is the binary residue of the universal ordered convolution MC element.
The seven-face master theorem
is the comparison theorem for this residue entry: the available independent
computations of~$K_\cA^{\mathrm{coll}}(z)$ across the seven frameworks must agree on
...
=\mathcal T_{\cA,\Xi},\qquad
\eta^\partial_\cA\colon
\Obs^\partial(\mathcal T_{\cA,\Xi})\xrightarrow{\simeq}\cA.
\]
...
\emph{collision kernel} is the projection of $\Theta_\cA^{E_1}$ onto the
...
\operatorname{Res}^{\mathrm{coll}}_{0,2}(\Theta_\cA^{E_1})
\;=\;
\Theta_\cA^{E_1}\bigm|_{\partial\overline{\mathcal{M}}_{0,2}^{\mathrm{coll}}}.
\end{equation}
The notation $K_\cA^{\mathrm{coll}}(z)$ records the residual spectral parameter that
...
family $K_\cA^{\mathrm{coll}}(z)
\in\mathrm{End}_\cA(2)[\![z^{-1}]\!]$ indexed by
the relative coordinate $z = z_1 - z_2$.

The existence of $K_\cA^{\mathrm{coll}}(z)$ follows from the bar-intrinsic
...
presentation-independent, and the collision residue is an invariant
of~$\cA$ on the cyclic-rigid generic locus. Its scalar projections
depend additionally on the designated primary line and trace datum:
the degree-two coefficient is an invariant of the triple
...
$(\cC^{\mathrm{op}}, b, A_b)$ with
$A_b=\RHom_{\mathsf C}(b,b)$ modular Koszul
...
Let $\cA=A_b$ in this hypothesis package. The collision kernel
$K_\cA^{\mathrm{coll}}(z)
=\mathrm{Res}^{\mathrm{coll}}_{0,2}(\Theta_\cA^{E_1})$ admits
the following realizations on their stated comparison surfaces:
\begin{enumerate}[label=\textup{(F\arabic*)}]
\item the binary collision component $(\pi_\cA)_2^{\mathrm{coll}}$
...
faces, their typed extraction maps have equal images as elements of
$\mathrm{End}_\cA(2)[\![z^{-1}]\!]$, after the explicit
normalization conventions of \S\ref{sec:hdm-master}.  Every face
called a classical $r$-matrix includes
$H_{\mathrm{CYBE}}^{\mathrm{rep}}$ and identifies its represented
operator with $\rho_\cA(K_\cA^{\mathrm{coll}})$. On the affine
...
identifies its extracted kernel with $K_\cA^{\mathrm{coll}}$ there,
citing the formal
...
twisting morphism out of the bar coalgebra, while
$K_\cA^{\mathrm{coll}}(z)$ is its
arity-two collision kernel.
After applying
$q_\cA\colon\cA^{\mathrm{i}}=C_X(s^{-1}V,s^{-2}R)
\to\barBch_X(\cA)$, and then passing to strict Verdier-dual algebra
coordinates when Upgrade~\textup{(U2)} holds,
...
$H_{\mathrm{CYBE}}^{\mathrm{rep}}$ supplied by the KZ representation
$\rho_{\mathrm{KZ}}$.

For $\cA = A_b = \widehat{\fg}_k$ the affine Kac--Moody chart algebra
at level $k \neq -h^\vee$, the KZ representation of the collision
kernel is
\begin{equation}\label{eq:hdm-face-5}
r^{KM,\fg}_{\mathrm{KZ},k}(z)
\;:=\;\rho_{\mathrm{KZ}}
\bigl(K_{k,\fg}^{\mathrm{coll}}(z)\bigr)
\;=\;
...
\textup{(}Theorem~\textup{\ref{thm:collision-depth-2-ybe}})}, maps
under $H_{\mathrm{CYBE}}^{\mathrm{rep}}$ to the classical
Yang--Baxter equation for $r^{\mathrm{Dr}}(z)$.
\end{theorem}

\begin{proof}
The Kac--Moody OPE
$J^a(z)\, J^b(w) \sim k\,\delta^{ab}/(z-w)^2 + f^{ab}_{\;\;c}\,
J^c(w)/(z-w)$ has poles of order~$2$ and~$1$. The trace-form
collision kernel in the central Casimir channel is
$K_{k,\fg}^{\mathrm{coll}}(z)=k\Omega_{\mathrm{tr}}/z$; the zeroth
product $a_{(0)}b=f^{ab}_{\;\;c}J^c$ supplies its Lie-bracket
compatibility. The KZ representation applies the level-shifted
invariant form and the explicit comparison
$k\Omega_{\mathrm{tr}}\mapsto\Omega/(k+h^\vee)$, giving
\eqref{eq:hdm-face-5}.
At the critical level $k = -h^\vee$ the denominator vanishes,
tracking the Sugawara singularity.
At $k=0$ the trace-form kernel specializes to zero, while the
KZ-normalized represented kernel specializes to
$\Omega/(h^\vee z)$; these are two comparison coordinates joined by
the stated normalization map. The collision-depth-$2$ theorem gives
the operadic three-insertion relation. The represented-CYBE clause of
$H_{\mathrm{CYBE}}^{\mathrm{rep}}$ evaluates it against the Casimir
and gives the Yang--Baxter equation. The full
...
\begin{theorem}[Semenov-Tian-Shansky Sklyanin bracket;
\ClaimStatusProvedElsewhere]
\label{thm:hdm-sts-sklyanin-bracket}
Semenov-Tian-Shansky~\cite{STS83} formulates the notion of a
classical $r$-matrix from the perspective of Poisson geometry and
shows that any classical $r$-matrix on a Lie algebra $\fg$ defines a
quadratic Poisson bracket on the dual $\fg^*$, the
...
to the Jacobi identity for $\{\cdot,\cdot\}_{\mathrm{STS}}$.
\end{theorem}

This is the Poisson-geometric face of the spectral $r$-matrix: where Faces~1
...
classical Poisson structure on the dual. This is the
Drinfeld--Semenov-Tian-Shansky classical-limit input; the later
three-parameter affine comparison separates this proved classical
limit from the conditional higher-depth parameter identifications.
\end{proof}
...
available face-specific expression of the universal collision kernel
$K_\cA^{\mathrm{coll}}(z)$.
\end{remark}
...
hub~$F_1$, where the universal ordered collision kernel
$K_\cA^{\mathrm{coll}}(z)=
\Res^{\mathrm{coll}}_{0,2}(\Theta_\cA^{E_1})$ lives in the
$E_1$ convolution dGLA.  Its homotopy coinvariant
$q_2(K_\cA^{\mathrm{coll}})$ lives in the modular dGLA
$\mathfrak{g}_\cA^{\mathrm{mod}}$. The
...
seen in the following exact degree-$2$ calculation.  On
$\mathfrak{sl}_2^{\otimes2}$ with ordered basis
$\{x\otimes y:x,y\in\{e,f,h\}\}$, the symmetrizer
$P_+=(1+\tau)/2$ is a $9\times9$ idempotent of rank~$6$, with
kernel
$\langle e\wedge f,e\wedge h,f\wedge h\rangle$.  The Casimir
$\Omega=\tfrac12h\otimes h+e\otimes f+f\otimes e$ lies in
$\operatorname{im}P_+$.  Hence averaging is injective on the
one-dimensional collision line $k\zeta(z)\Omega$ for $k\ne0$,
although it has a three-dimensional kernel on the full ordered tensor
square.  Adding the affine shift $3/2$ gives
$3k/4+3/2=3(k+2)/4$.
\end{remark}
...
\item \emph{Quadratic coalgebra and full bar.} The chosen boundary
 presentation gives
 $\cA^{\mathrm i}=C_X(s^{-1}V,s^{-2}R)$ and
 $q_\cA\colon\cA^{\mathrm i}\to\barBch_X(\cA)$.
...
Theorem~C forms the strict-flat centre-local-system eigensummands.
Their normalized Theorem~D trace gives
$\kappa(V_k)+\kappa(V_{-k-2h^\vee})=0$ on the trace-form
normalization.

\emph{Virasoro $\mathrm{Vir}_c$.} The coderived dual is
$\mathrm{Vir}_c^{!}_\infty \simeq \mathrm{Vir}_{c'}^{\text{copy}}_\infty$
with $c + c' = 26$.  The C1 centre-local-system eigensummands acquire
the normalized Theorem~D scalar trace $c/2+c'/2=13$. The representing
...
\item Collision kernel and represented $R$-matrix: the single-generator OPE has
 double pole $k$, and the $d\log$ absorption strips one
 pole, producing
 \[
 K^{\mathrm{coll}}_{\mathrm{Heis},k}(z)
 \;=\; \frac{k}{z}\, J \otimes J,
 \]
 whose Fock representation has commuting tensor insertions and hence
 supplies $H_{\mathrm{CYBE}}^{\mathrm{rep}}$ and the level-$k$
 Drinfeld $r$-matrix at $\fg=\C$.
\item Universal MC element: $\Theta_{\cH_k}$ has a single non-zero
 degree~$2$ component (class~$\mathbf{G}$, $r = 2$), and
 $\kappa(\cH_k)=\tau_{\cH_k,2}^{\mathrm{res}}
 q_2(K^{\mathrm{coll}}_{\mathrm{Heis},k})=k$ by the normalized
 degree-$2$ residue/trace
 (Heisenberg entry).
...
$(\cH_k,\cH_k^i,\cH_k^!,\cC,K^{\mathrm{coll}}_{\mathrm{Heis},k}(z),
...
witness is the curved second-kind branch
\(\cH_k^!=(\mathrm{Sym}^{\mathrm{ch}}(V^*[1]),m_0=-k\omega)\).
The level-flipped algebra \(\cH_{-k}\) is only the scalar-shadow and
open-colour comparison object with the same \(\kappa=-k\). Apply
...
$\cH_k\otimes \cH_{-k}$ realises these as the doubled open-colour lattice
...
of the braided coproduct. This is the universal property of the free
Weyl/open-colour doubled algebra; it is not an object-level Koszul
self-duality statement.
...
object $U_\cA=\cA\bowtie\cA^!_\infty$. The chart slot and five
functorial entries are images of $U_\cA$ under the specified
$(\infty,1)$-functors.  The boundary projection determines the full
bar cohomology $H^\bullet\barBch(P_\cA(U_\cA))$; the quadratic
coalgebra~$\cA^{\mathrm i}$ additionally uses the chosen presentation
of the projected boundary algebra:
\begin{enumerate}[label=\textup{(P\arabic*)}]
...
$\operatorname{Res}^{\mathrm{coll}}_{0,2}$ used to define
$K_\cA^{\mathrm{coll}}(z)$
in Section~\ref{sec:hdm-collision-residue}. $\delta$ is the universal
bar construction: $\Theta_\cA$ defines a codifferential on
$B^{\mathrm{ord}}(\cA)$.  The quadratic presentation supplies
$\cA^i=C_X(s^{-1}V,s^{-2}R)$ and
$q_\cA\colon\cA^i\to B^{\mathrm{ord}}(\cA)$; under
$H_{\mathrm{CL}}(\cA)$, the induced map $H^\bullet(q_\cA)$ is an
isomorphism.  After
...
coefficient $K_\cA^{\mathrm{coll}}(z)$; in the affine KZ lane its
represented image is
...
\end{enumerate}
The Koszul-locus conservativity is sharp on the stated comparison
...
$\mathsf C$ ceiling~$250/3$, $\mathsf M$ ceiling~$25/3$). All cross-composite
...
\item \emph{Climax identity.} The full datum $\mathcal{H}(T)$
 condenses into the typed KZ--Arnold bar-superconnection theorem of
 Chapter~\ref{ch:climax-theorem}: the modular shadow connection
 $\nabla^{\mathrm{hol}}$ is compared with the local Arnold/KZB
 representative, and the bar-complex operator appears after
 Fulton--MacPherson boundary-residue realization.  The seven-face
 master theorem records the seven names under which the binary
 collision residue~$r_\cA(z)$ enters that comparison. Outside the
 stated comparison surfaces this remains
...
\begin{remark}[Drinfeld-centre right-adjoint discipline]
\label{rem:hdm-ap-cy54-discipline}

\index{Drinfeld centre!right-adjoint discipline}
...
\begin{remark}[Theorem~C scalar trace bucket and the Mukai arithmetic candidate]
\label{rem:holo-master-theoremC-bucket}
\index{Theorem~C!scalar trace bucket $\{0,13,250/3\}$}
\index{B-family!$K^{\kappa_{\mathrm{ch}}} = 8$ Bruinier--Heegner reciprocity}
The scalar Verdier sum
$K^\kappa(\cA):=\kappa(\cA)+\kappa(\cA^!)$ is the normalized
Theorem~D trace shadow of the two C1 eigensummands of
$\mathbf C_g(\cA)=R\Gamma(\overline{\mathcal M}_g,\mathcal Z(\cA))$.
The represented Vol~I witnesses give the computed bucket
\[
 K^\kappa(\cA)
 \;\in\;
 \{0,\;13,\;250/3\}.
\]
The zero lane is represented by the Heisenberg, affine, and free-field
pairs; Virasoro gives~$13$; the principal~$\mathcal W_3$ witness gives
$250/3$.  For Bershadsky--Polyakov the central-charge identity
$K^c_{\mathrm{BP}}=50$ is exact, while the proposed modular value
$25/3$ awaits the genus-one curvature comparison.

The Mukai lattice gives the exact arithmetic value
\[
 2c_+(\mathrm{II}_{4,20})=8.
\]
Its interpretation as
$K^{\kappa_{\mathrm{ch}}}(\mathrm{Mukai}(K3))=8$ is conjectural under
$H_{\mathsf B}$ and is transported through Bruinier--Heegner
reciprocity~\cite{Bruinier02}.  The Mukai arithmetic, the Borcherds
weight
$\kappa_{\mathrm{BKM}}(\Delta_5)=c_1(0)/2=\mathrm{wt}(\Delta_5)=5$,
and the Oberdieck--Pixton normalization
\[
 D_5=64^{-1}\Delta_5,\qquad
 \Phi_{10}^{\mathrm{OP}}=D_5^2=4096^{-1}\Delta_5^2,\qquad
 Z_{\mathrm{OP/DT}}=-D_5^{-2}=-4096\,\Delta_5^{-2}
\]
occupy distinct scalar targets.  The singular-theta/Heegner
correspondence supplies the cross-volume bridge among them.
\end{remark}
...
\[
\eta^\partial_\cA\colon
\Obs^\partial(\Phi_{\mathrm{hol}}(\cA,\omega,T_\omega,\Xi_\cA))
\xrightarrow{\simeq}\cA
\]
is an $E_1$-chiral equivalence on $X$ for every
\((\cA,\omega,T_\omega,\Xi_\cA)\in
\ChirAlg^{\omega,\mathrm{BL,adm},\Xi}_X\).
Hypothesis~\textup{\ref{H4-triangle}} places
\(\Phi_{d,\sigma}(\cC)\) in the algebraic source, and
Hypothesis~\textup{\ref{H6-triangle}} supplies the anomaly-free
BV/BRST comparison surface, hence the required
\(\Xi_{\Phi_{d,\sigma}(\cC)}\). The identification applies with
\(\cA = \Phi_{d,\sigma}(\cC)\). Vol~II's Theorem
``Bulk-to-boundary OPE consistency'' sharpens the comparison from a
quasi-isomorphism of underlying objects to a chain-level
identification of $E_1$-chiral algebra structures \textup{(}part~(c)
of that theorem: the algebra structure on the realized boundary
\(\Obs^\partial(\Phi_{\mathrm{hol}}(\cA,\omega,T_\omega,\Xi_\cA))\)
agrees strictly at chain level with the chiral product on
\(\cA\)\textup{).} This gives
