...
$Z^{\mathrm{der}}_{\mathrm{ch}}(A_b) \simeq
\mathcal{C}^\bullet_{\mathrm{ch}}(A_b,A_b)$, level~$3$, the bulk;
...
is not the bulk; $Z^{\mathrm{der}}_{\mathrm{ch}}(A_b)$ is. Throughout
...
every~$\cA$, but the line-side presentation and the closed-sector/line
 ...
 tagged \ClaimStatusConjectured: the slots are pinned and partially
...
$P_\cC(U_\cA) \to
\mathcal Z_{(\infty,1)}\bigl(\mathrm{Rep}^{E_1}(\cA)\bigr)$ fails to
be a strict $E_2$-equivalence before $\mathrm{Pro}$-completion, with
obstruction measured by
...
\item\label{H6-triangle} \emph{BV/BRST anomaly-vanishing hypothesis.}
 ...
 item is a proof obligation, not a consequence of
