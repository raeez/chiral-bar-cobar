...
gives $c_{4d} = 107/6$ (Shapere--Tachikawa 2008 \S 3; the
intermediate value $26$ is retracted per Vol~I
Chapter~\ref{ch:thqg-open-closed-realization} line~1932) and
...
% KZ convention, consistent with eq:hdm-face-5; at k=0 gives Omega/(h^v z) != 0.
...
(Theorem~\ref{thm:hdm-grt-cocycle-ledger}(iv)) corresponds at
$K3 \times E$ to the BKM denominator cocycle of Vol~III
(the $\Phi_{10}$ Igusa cusp form discussed immediately above), and the
...
corepresented by $\cA^! = H^\star\bigl(\barBch(\cA)\bigr)^\vee$ on
chiral coalgebras, and its enrichment to Koszul pairs is corepresented
by the tensor product of the boundary factor~$\cA$ with the dual
factor~$\cA^!$ \textup{(}Theorem~\textup{\ref{thm:bar-cobar-adjunction}}
in the bar--cobar lane and
...
$r^{KM,\fg}_k(z) = k\, \Omega_{\mathrm{tr}}/z$, recovering slot~\textup{(4)}.
...
$R$-matrix of~$U_{V_k(\fg)}$ is the collision residue
$r^{KM,\fg}_k(z) = k\,\Omega_{\mathrm{tr}}/z$ and matches the
quasitriangular structure on $D(U_q(\fg))$ under this equivalence.
...
level of $R$-matrices: the spectral $R$-matrix
$r^{KM,\fg}_k(z) = k\,\Omega_{\mathrm{tr}}/z$ expands as a rational
function of~$z$ whose monodromy at~$z=\infty$ reproduces Drinfeld's
universal $R$-matrix on~$D(U_q(\fg))$, by the Drinfeld--Kohno
...
two readings coincide on the nose; off it they agree after
...
as the double-pole residue $k$, agreeing on the nose. No gauge
...
universal $R$-matrix equal to $k\,\Omega_{\mathrm{tr}}/z$.
...
\begin{theorem}[Five recovery functors out of $U_\cA$; \ClaimStatusProvedHere]
...
$\otimes/\mathrm{Hom}$ adjunction; the monadic-localization status is
...
\begin{remark}[Status map of the six projections]
...
\begin{theorem}[Class-$\mathsf M_\infty$ fibre; \ClaimStatusProvedHere]
...
$\mathsf M_\infty$ tower; \ClaimStatusProvedHere]
...
\Phi^{\mathrm{FA}}_3$ (stage~$1$ canonical $E_3$-holomorphic
factorisation algebra on the CY$_3$ variety, pinned by
Kontsevich--Tamarkin; stage~$2$ factorisation homology over a
closed $2$-cycle $\Sigma_2$ followed by restriction to a reference
curve $C$) realises the seven faces of $r_{\Phi_3(X)}(z)$ for
K3-fibered Class~A threefolds on the $K3$-fibre shadow
$\mathrm{Sp}^{\mathrm{ch}}_{\Sigma_2 = p_{K3}^{-1}(\mathrm{pt}), C = E}$,
with the universal trace identity
...
established outside those five families. This supplies the conductor side
of slot~(5) in the
BPS-counting category.
\end{remark}

\begin{remark}[Universal $\Phi$-trace identity at $K3 \times E$: the conductor lands on $\mathrm{wt}(\Delta_5) = 5$]
...
The Vol~I holographic datum assigns to each modular Koszul chiral
algebra $\cA$ the scalar
$K(\cA) = -c_{\mathrm{ghost}}(\BRST^{\mathrm{can}}(\cA))$
of Remark~\ref{rem:hdm-anchors}(W2). The Vol~III universal
$\Phi$-trace identity specialises this scalar at $\cA = \Phi_3(X)$
for a Class~A K3-fibered threefold $X$ via the Borcherds
...
correspondence). At the $K3 \times E$ specialisation, $\Phi_3(K3
\times E)$ is the Gritsenko--Nikulin~\cite{GritsenkoNikulin98a} BKM
algebra $\mathfrak{g}_{\Delta_5}$, whose Borcherds weight is read off
its denominator $\Delta_5$:
\[
 ...
 5.
\]
The chain-level universal $\Phi$-trace identity of Vol~III
conjecturally closes the triangle
$K(\Phi_3(K3\times E))/2 = c_\Lambda(0)/2 = \mathrm{wt}(\Delta_5)$,
matching the Vol~I BRST ghost conductor on the left with the Vol~III
...
(as $\Phi_{10} = \Delta_5$ in the Vol~III labelling) is one;
the $\mathrm{wt}(\Delta_5) = 5$ answer is therefore an established
cross-volume datum, not a conjectural extension, on exactly this
family. This is the single numerical incarnation of the Vol~I
holographic datum at the BKM edge of Vol~III.
\end{remark}
...
The scalar conductor $K(\cA) = c(\cA) + c(\cA^!)$ attached to the
...
Part~\ref{part:standard-landscape}) a family-dependent discrete
bucket: on the four standard-landscape lanes $\mathsf G / \mathsf L /
\mathsf C / \mathsf M$ the value $\kappa(\cA) + \kappa(\cA^!)$ is
forced into
\[
 \kappa(\cA) + \kappa(\cA^!)
 \;\in\;
 \{0,\; 8,\; 13,\; 250/3,\; 98/3\}
\]
by the class-by-class OPE normalisations recorded in
Remark~\ref{rem:hdm-heisenberg-verification} and its W/Virasoro
analogues. The value~$0$ is the free-field abelian locus
...
entry~$8$ is the B-family rigidity value: on the Borcherds class of
Chapter~\ref{ch:climax-theorem}, the three cross-volume faces
(chiral BRST conductor, singular-theta integral of
Remark~\ref{rem:holo-master-phi-trace}, and BKM denominator
weight on five verified families $N \in \{1,2,3,4,6\}$) are
unified by the Bruinier Heegner-reciprocity theorem~\cite{Bruinier02}
at $K^{\kappa_{\mathrm{ch}}} = 8$, matching twice the
$\mathrm{wt}(\Delta_5) = 5$ modular weight up to the
$\mathbb{Z}/2$-average coming from the Fourier--Mukai involution of
Remark~\ref{rem:holo-master-igusa}. The five-value
bucket is therefore not a coincidence but the Theorem~C finite
complementarity dichotomy, projected through the averaging map $\av$
onto the dual scalar pair $(\kappa, \kappa^!)$. No sixth value is
admitted on the chirally Koszul boundary-linear locus where slot~(2)
of Proposition~\ref{prop:hdm-u-a-recoverability} is strict.
\end{remark}
...
$\cA = \Phi_d(\cC)$. Vol~II's Theorem ``Bulk-to-boundary OPE
...
lanes simultaneously, and the averaging composite is the unique
...
the universal-property gap at $\Phi_d$. It \emph{names} where the
...
Vol~II Theorem ``Bulk-to-boundary OPE consistency''; the
...
bialgebra, produces the canonical non-abelian-chiral holographic datum
\[
\mathcal H(\mathbf H_{\Delta_5}) \;=\; \bigl(\mathbf H_{\Delta_5},\; \mathbf H_{\Delta_5}^{\!(!)},\; B_{\mathrm{ch}}(\mathbf H_{\Delta_5}),\; R_{\mathrm{Sieg,dyn}},\; \Theta_{\mathbf H_{\Delta_5}},\; \nabla^{\mathrm{KZB}}_{\bar{\mathcal A}_2}\bigr).
\]
...
\]
with $D$ the bi-based Ran-space de~Rham differential. The holographic
reconstruction theorem -- that $\mathcal H(T)$ determines $T$ up to
quasi-equivalence -- therefore applies to $\mathbf H_{\Delta_5}$ via
the bi-based Ran-space datum. Primary-lit cited in Vol.~III,
Chapter~\ref{ch:k3-chiral-bialgebra-platonic}, Theorems~A--H.
\end{remark}
