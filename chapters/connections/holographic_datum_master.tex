...
\item $\Theta_\cA$ is the universal Maurer--Cartan element, the
  all-arity boundary-bulk coupling produced by the bar-intrinsic
...
arity-$2$ scalar component of $\Theta_\cA$ and the Brown--Henneaux
...
that lifts the chiral bracket to all arities and all genera.  The
...
complex has one-dimensional second cohomology in arity~$2$ on the
...
chiral algebra (with the divided-power conversion of AP44).  The
...
operators is governed by the collision depth, which by AP19 equals
...
algebra of a simple Lie algebra~$\fg$.  The classical $r$-matrix at level $k$
...
Killing form (the level prefix~$k$ is mandatory; the abelian limit
$k = 0$ gives $r^{\mathrm{Dr}} = 0$, AP126), satisfies the classical
...
$d\log(z-w)$ absorbs one power (AP19), so the collision residue
extracts the simple-pole coefficient with the structure-constant
contribution $f^{ab}_{\;\;c} J^c$ together with the level term;
after Killing-form normalization and the standard rescaling by
$1/(k+h^\vee)$ that produces the dual Coxeter shift, the result is
$k\,\Omega/((k+h^\vee)z)$.  This is the explicit
formula~\eqref{eq:hdm-face-5}; the level prefix~$k$ is preserved by
the rescaling and gives the abelian-limit vanishing $r_{\widehat{\fg}_0} = 0$
required by AP141.
...
$r^{\mathrm{cl}} = r^{\mathrm{Dr}} = k\Omega/z$ (vanishing at $k=0$
per AP126) is the classical
...
at $k=0$ per AP126) the antisymmetric
...
\sum_{j \neq i} \frac{\Omega_{ij}}{z_i - z_j}.
\end{equation}
The level dependence on the right-hand side cancels exactly: the
removable singularity at $k=0$ in the prefactor $(k+h^\vee)/k$
is balanced by the explicit $k$ in the numerator of
$r_{\widehat{\fg}_k}$ from~\eqref{eq:hdm-face-5}, leaving a
level-independent expression.
Equivalently, $H_i^{\mathrm{Gaudin}} = \frac{k+h^\vee}{k}\, H_i^{\mathrm{GZ}}$,
...
(Theorem~\ref{thm:hdm-face-5}; the level prefix~$k$ is required
by AP126).  Substituting into the Gaiotto--Zeng
formula~\eqref{eq:hdm-face-4} of Face~4 and rescaling by
$(k+h^\vee)/k$ produces~\eqref{eq:hdm-face-7}, which is the
...
present framework, the critical level is the locus where the level
normalization $1/(k+h^\vee)$ diverges and the Gaudin Hamiltonians
...
& \frac{k\,\Omega}{z}\;\textrm{(Drinfeld85)}
  \ar@{<->}[u]^{\textrm{(F5--F7)}}
}
\end{equation}
...
\item Quasitriangular $R$-matrix: the single-generator OPE has
  double pole $k$, and the $d\log$ absorption of AP19 strips one
  pole, producing
  \[
  r_{\cH_k}(z) \;=\; \frac{k}{z}\, J \otimes J,
  \]
  which is the level-$k$ Drinfeld $r$-matrix at $\fg = \C$; vanishing
  at $k = 0$ is the AP126 level sanity check.
\item Universal MC element: $\Theta_{\cH_k}$ has a single non-zero
  arity~$2$ component (class~$\mathbf{G}$, $r = 2$), and
  $\kappa(\cH_k) = k$ by the arity-$2$ projection
  (Heisenberg entry, AP39).
...
\item \emph{Inverse spectral parameter:} $\hbar = 1/z$ in the formal
  ...
  (level-$k$ form, vanishing at $k=0$ per AP126),
...
from higher-arity shadow contributions.
\end{remark}
...
This is the multi-path verification mandate (CLAUDE.md, three-paths
...
(documented as AP1 in CLAUDE.md).  The DNP non-renormalization
...
formality with Swiss-cheese formality (AP14).  In each case, the
