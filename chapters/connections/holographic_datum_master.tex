...
$\cZ^{\mathrm{der}}_{\mathrm{ch}}(\cA)$ produced by the bar-cobar
pair is PROVED at the chain level on the Koszul locus
(Theorem~A and Theorem~B of Vol~I; Vol~II's bulk-from-boundary
recovery package). Off the Koszul locus, the global triangle
\[
 \cA \;\longleftrightarrow\; \cA^!
 \;\longleftrightarrow\; \cZ^{\mathrm{der}}_{\mathrm{ch}}(\cA)
 \;\longleftrightarrow\; \cA
\]
is conjectural beyond classes~$\mathsf{G}$ and~$\mathsf{L}$:
class~$\mathsf{C}$ is conditional on harmonic decoupling and
class~$\mathsf{M}$ chain-level closure is replaced by the
weight-completed statement of Theorem~MC5. The seven-face
master theorem of this chapter operates entirely in the
boundary-linear bulk regime, where slot~(4) is unconditional;
the four consistency conditions
(Remark~\ref{rem:hdm-four-consistency-conditions}) record
exactly the boundary-bulk coherences that the global triangle
must satisfy beyond the Koszul locus.
\end{remark}
