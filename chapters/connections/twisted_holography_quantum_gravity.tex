...
\begin{enumerate}[label=\textup{(\roman*)}]
\item The Koszul dual $\mathrm{Vir}_{26}^! \simeq \mathrm{Vir}_0$
  degenerates: $\kappa(\mathrm{Vir}_0) = 0$, so the holographic
  datum $\mathcal{H}(\mathrm{Vir}_{26})$ has a trivial dual sector
  under gravitational $S$-duality (G4);
  \textup{(}self-conjugacy $\cA \simeq \cA^!$ occurs at
  $c = 13$, not $c = 26$\textup{)}
...
For the entire standard landscape, the HS-sewing criterion
...
Let $(\cA, X, \langle\cdot,\cdot\rangle, k)$ be a gravitational input.
All sixteen theorems \textup{(G1)--(G16)} follow from:
\begin{enumerate}[label=\textup{(\roman*)}]
...
The dependency partial order among the sixteen results is:
\begin{center}
\begin{tikzpicture}[
