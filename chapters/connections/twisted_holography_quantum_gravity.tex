...
\begin{theorem}[\textbf{G13}: Modular flow from shadow connection]\ClaimStatusConjectured
...
The Verdier involution $\sigma$ is the Tomita conjugation $J$
...
\begin{theorem}[\textbf{G14}: Holographic error correction]\ClaimStatusProvedHere
\label{thm:thqg-g14-error-correction}\label{item:g14}
The bar-cobar adjunction provides an error-correcting code structure
(Proposition~\ref{prop:thqg-barcobar-error-correction}):
encoding via $B$, decoding via $\Omega$, code perfection from
Koszulness (Theorem~B).  The entanglement wedge is the open/closed
realization (Theorem~\ref{thm:thqg-entanglement-wedge}), with the
annulus trace as partial trace
...
The complementarity theorem (Theorem~C) imposes a Page bound on the
entanglement entropy at all genera
(Theorem~\ref{thm:thqg-page-constraint}):
$S_{\mathrm{EE}}^{(g)}(\cA) \le \min(C_g(\cA), C_g(\cA^!))$,
...
on the replica manifold $\Sigma_{g,n}$ of genus $n(g-1)+1$
...
$\varprojlim_r \Theta_\cA^{\le r}$ & Full MC element
  & G8 \\
$\cH_g, \sigma, \Theta_\cA^{(g)}$ & entanglement rank $E_g$
  & G11, G15 \\
