...
The genus-zero binary collision of $\Theta_\cA \in \MC(\gAmod)$ extracts a spectral-parameter-dependent $r$-matrix controlling gravitational scattering of line operators.
This section promotes the extraction to a full dg-shifted Yangian
$\Ydg(\cA^!)$ inside the pronilpotent modular
dg Lie algebra $\Ymod_\cA$. The genus-refined binary collision
projections define a formal higher-genus series
$R^{\mathrm{bin}}_\cA(z;\hbar)$; comparison
with a genuine modular kernel
$R^{\mathrm{mod}}_\cA(z;\hbar)$ satisfying the stable-graph
Yang--Baxter equation remains frontier.
Six stages:
...
The organizing identity is the MC equation
...
Everything up to the genus-zero/evaluation-core
level follows from the proved algebraic core; higher-genus modular-kernel statements are explicitly fenced.
...
\end{equation}
of codimension $|E_{\mathrm{int}}(T)| = |V_{\mathrm{int}}(T)| - 1$,
where $|v|$ is the valence of~$v$. The open stratum
$\mathcal{M}_{0,n+1}$ corresponds to the unique tree with a single
internal vertex.
\end{definition}

\begin{remark}[Relation to the degree filtration]
\label{rem:thqg-V-collision-vs-degree}
...
\begin{proof}
The Lie bracket on the convolution algebra is defined by composition of
operations via the modular operad structure. Composing an operation
$\phi$ supported on strata of codimension $\geq d_1$ with an operation
$\psi$ supported on strata of codimension $\geq d_2$ produces an
operation supported on strata of codimension $\geq d_1 + d_2$: the
composed operation evaluates on a chain in
$\overline{\mathcal{M}}_{0,n+m-1}$ whose boundary depth is the sum of
the constituent boundary depths.

More precisely: the operadic composition map
\[
\circ_i\colon
C_*(\overline{\mathcal{M}}_{0,n+1}) \otimes
C_*(\overline{\mathcal{M}}_{0,m+1})
\to
C_*(\overline{\mathcal{M}}_{0,n+m})
\]
sends the stratum $\sigma_{T_1} \times \sigma_{T_2}$ to the stratum
$\sigma_{T_1 \circ_i T_2}$ obtained by grafting $T_2$ into the
$i$-th leaf of~$T_1$. The internal-edge count satisfies
$|E_{\mathrm{int}}(T_1 \circ_i T_2)| =
|E_{\mathrm{int}}(T_1)| + |E_{\mathrm{int}}(T_2)| + 1
\geq d_1 + d_2 + 1 \geq d_1 + d_2$,
...
restricted to binary collisions: it extracts all OPE modes via
the logarithmic kernel from the collision of two insertions.
...
The Arnold relation is the vanishing of the $2$-form
$\eta_{ij} \wedge \eta_{jk} + \eta_{jk} \wedge \eta_{ki}
+ \eta_{ki} \wedge \eta_{ij}$, which is exact on $\Conf_3(\C)$:
it equals $d\bigl(\operatorname{Li}_1(z_{ij}/z_{ik})\bigr)$
...
This is the condition that the bar differential
...
The $r$-matrix $r_\cA(z)$ is computed below for the three
...
($J^i_{(0)}J^j = 0$): the collision residue extracts the
zeroth Borcherds product, which vanishes for Heisenberg.
On the Koszul dual side $(\mathcal{H}^N_\kappa)^!$,
the twisting morphism encodes the pairing as the Casimir structure,
giving the $r$-matrix
...
Here $J^i$ denotes the Koszul-dual generator corresponding to
$J^i \in \mathcal{H}$, living in
$\mathcal{H}^! = \operatorname{Sym}^{\mathrm{ch}}(V^*)$
(Remark~\ref{rem:fourier-heisenberg-not-selfdual};
note $\mathcal{H}_\kappa^! \neq \mathcal{H}_{-\kappa}$).
...
degree~$2$ (Corollary~\ref{cor:heisenberg-postnikov-termination}),
...
L_m \otimes L_n,
\end{equation}
where the $z$-dependent coefficients are
determined by the Virasoro collision residue
(the $d\log$ measure absorbs one pole order
from the OPE):
...
shadow obstruction tower: the higher-order poles generate an infinite sequence
of non-trivial higher-degree corrections
...
\begin{remark}[Notational disambiguation: $r(z)$]
\label{rem:thqg-V-r-matrix-disambiguation}
\index{r-matrix@$r$-matrix!disambiguation!gravitational chapter}
The $r$-matrix $r_\cA(z)$ constructed in this chapter is the
\emph{collision residue}
$r_\cA^{\mathrm{coll}}(z) \in \cA^!\,\widehat{\otimes}\,\cA^!$ of
Remark~\textup{\ref{rem:three-r-matrices}}\textup{(b)},
valued in the tensor square of the Koszul dual
and carrying collision-residue pole orders (one less than
the OPE; see \textup{loc.\ cit.}).
In particular, for Virasoro,
$r^{\mathrm{coll}}(z)$ has a cubic pole $z^{-3}$ from
the central extension (the OPE's quartic pole $z^{-4}$
drops by one under $d\log$ extraction)
\textup{(}Computation~\textup{\ref{comp:thqg-V-virasoro-r})}.

This should be distinguished from the \emph{$E_1$ scalar shadow}
$r^{\mathrm{sc}}(z) = c/(2z)$ appearing in the $E_1$ archetype table
\textup{(}Example~\textup{\ref{ex:e1-shadow-archetypes})}, which is
the vacuum projection of $r^{\mathrm{coll}}(z)$ to a single
scalar channel. The relation is
$r^{\mathrm{sc}}(z) = \langle\mathrm{vac}|\,
r^{\mathrm{coll}}(z)\,|\mathrm{vac}\rangle$,
i.e.\ the leading-pole coefficient on the primary line,
with $\operatorname{av}(r^{\mathrm{sc}}(z)) = \kappa(\cA)$.
\end{remark}
...
\item Higher collision residues
 ...
 exchange amplitudes, governed by the shadow obstruction tower.
\end{enumerate}
\end{proposition}
...
The MC equation
\begin{equation}\label{eq:thqg-V-mc-degree3}
D_\cA\Theta_{0,3}
\;+\;
...
a Ward identity (cf.~Theorem~\ref{thm:thqg-g6-soft-graviton}).
\end{remark}
...
Definition~\ref{def:modular-yangian-pro} in the gravitational
...
\label{rem:thqg-V-genus-one-correction}
...
The quartic graviton vertex is the degree-$4$ component of
...
on the affine comparison / evaluation-core surfaces either:
\begin{enumerate}[label=\textup{(\roman*)}]
...
\end{equation}
where $V_\lambda(z)$ denotes the evaluation object associated to the
dominant weight~$\lambda$ and spectral parameter~$z$. The derived
Drinfeld--Kohno comparison is proved on this core
\textup{(}Corollary~\textup{\ref{cor:dk23-all-types})}. Beyond the
evaluation-generated core, Theorem~\ref{thm:mc3-type-a-resolution}
...
Conjecture~\ref{conj:dk-compacts-completion}, which asks to extend the
...
tautological. Corollary~\ref{cor:dk23-all-types} proves the DK
comparison on the evaluation-generated core. Theorem~\ref{thm:mc3-type-a-resolution}
...
Conjecture~\ref{conj:dk-compacts-completion}. Transporting those
...
\index{Drinfeld--Kohno!DK-5!accessibility}
...
Conjecture~\ref{conj:dk-compacts-completion}. No further independent
type-$A$ obstruction remains beyond that packet.
\end{corollary}

\begin{proof}
Immediate from Theorem~\ref{thm:thqg-V-mc3-thick-generation} and
Theorem~\ref{thm:mc3-type-a-resolution}.
\end{proof}

\begin{remark}[MC3 for arbitrary type: evaluation core proved, post-core conditional]
\label{rem:thqg-V-mc3-arbitrary-type}
\index{MC3!arbitrary type resolution}
The prefundamental Clebsch--Gordan closure
\textup{(}package~\textup{(i)} of MC3\textup{)} is proved for all simple
types by Theorem~\ref{thm:categorical-cg-all-types}, and the DK
comparison on the evaluation-generated core is proved for all simple
types by Corollary~\ref{cor:dk23-all-types}. In type~$A$,
Theorem~\ref{thm:mc3-type-a-resolution} proves the
...
Proposition~\ref{conj:mc3-automatic-generalization} are separately
...
sgYBE & All-loop graviton consistency
 ...
 (Def~\ref{def:thqg-V-stable-graph-ybe}) \\
...
Shadow connection $\nabla^{\mathrm{hol}}$
 & KZ on affine KM comparison surface; BPZ on Virasoro
 degenerate-representation surface
 & Proved (Thm~\ref{thm:shadow-connection-kz},
 Prop~\ref{prop:shadow-connection-bpz}) \\
...
gravitational Yangian $\Ydg_\cA$ is a refinement: the ordered
structure retains the spectral-parameter dependence that the
unordered structure averages over. For
...
boundary operators. Its infinite shadow obstruction tower mirrors the expected
...
structure (Lemma~\ref{lem:degree-cutoff}).
