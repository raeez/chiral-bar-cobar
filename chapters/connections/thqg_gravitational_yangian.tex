...
\textup{(}Corollary~\textup{\ref{cor:mc3-all-types})}:
\begin{enumerate}[label=\textup{(\roman*)}]
...
\item Francis--Gaitsgory pro-nilpotent completion extends the thick subcategory
...
action.  Francis--Gaitsgory pro-nilpotent completion
extends the thick subcategory from the
evaluation-generated core to the full category.
\end{proof}

\begin{corollary}[DK-5 accessibility;
\ClaimStatusProvedHere]
\label{cor:thqg-V-dk5-type-a}
\index{Drinfeld--Kohno!DK-5!accessibility}
The MC3 thick generation theorem
...
follows from Francis--Gaitsgory pro-nilpotent completion.
\end{proof}

\begin{remark}[MC3 for arbitrary type: resolved]
\label{rem:thqg-V-mc3-arbitrary-type}
\index{MC3!arbitrary type resolution}
The prefundamental Clebsch--Gordan closure
(package~(i) of MC3) is now proved for all simple
types by Theorem~\ref{thm:categorical-cg-all-types},
using multiplicity-free $\ell$-weights in place of the
minuscule hypothesis.
The full four-package MC3 is proved for type~$A$
(Theorem~\ref{thm:mc3-type-a-resolution});
for other types, packages~(ii)--(iv) reduce to thick
generation of $\mathrm{Rep}_{\mathrm{fd}}$ by evaluation
modules (Corollary~\ref{cor:mc3-all-types}).
In the gravitational Yangian context: the line-operator
category is compactly generated in type~$A$; for other types
the gravitational $r$-matrix $\rgrav(z)$ exists
(Theorem~\ref{thm:thqg-V-collision-twisting}) and the
categorical structure awaits integration of the Chari--Pressley
thick-generation input into the DK hierarchy.
\end{remark}
