...
The governing question of this chapter is the mathematical role of \emph{Holomorphic-topological boundary conditions and 4d origins}: how this bridge interfaces with the proved modular Koszul core without blurring theorematic content and programme.
\end{remark}

\begin{remark}[H/M/S route; Convention~\ref{conv:hms-levels}]
This chapter is mixed-level. Proved comparison statements are H-level only where theorem labels say so, explicit kernels or complexes live at M-level, and physical or numerical bridges remain S-level or programme material unless printed otherwise.
\end{remark}
\index{holomorphic-topological theory|textbf}
\label{rem:programme-ht}

\section{Mathematical relationships between frameworks}
\label{sec:precise-relationships-frameworks}
\noindent\emph{Status split.} This section separates proved
twist-level constructions \textup{(}\ClaimStatusProvedElsewhere\textup{)}
from the conjectural criterion that promotes general CL output to
genuine chirality \textup{(}\ClaimStatusConjectured\textup{)}.
...
\begin{remark}[Scope]
Conjecture~\ref{conj:CL-produces-chiral} is labeled
\ClaimStatusConjectured\ because the three conditions
(holomorphicity, conformal symmetry, $\mathcal{D}$-module structure)
involve physics input: condition~(1) requires the existence of a
holomorphic complex structure on the Coulomb branch that survives the
twist, condition~(2) requires vanishing of specific anomalies in the
twisted gauge theory, and condition~(3) requires extending the
factorization algebra to the Ran space in a manner compatible with
the $\mathcal{D}$-module structure.
...
for boundary chiral algebras via holomorphic twists, Gaiotto--Kulp--Wu
\cite{GKW24} for perturbative BRST-anomaly brackets and their quadratic
identities in flat holomorphic-topological theories, and Zeng \cite{Zeng23}
for the twisted holography perspective.  Computing the bar complex
$\bar{B}^{\mathrm{ch}}(\mathcal{A}_{\mathrm{bdy}})$ for specific CDG20
boundary chiral algebras remains an open problem.  GKW24 should be read
here as local perturbative input rather than as a proof of the
curvewise bar dictionary: the paper isolates regularized higher
operations and the factorization-algebra side of the story, but it does
not construct a chiral envelope on a curve or an all-genus bar
comparison.  On the MC4 side, the
remaining comparison is now exact: one must construct the filtered
factorization-theoretic target whose finite quotients recover the
theorematic $W_N$ or RTT stages and then prove the coefficient
identities
...
on the finite packets $\Delta_{a,0}(N)$ and $\mathcal{I}_N$.
...
established mathematically.  In our framework, condition~(ii)
($c_1(R) = 0$) is the statement that the universal MC class
$\Theta_{\cA_{\mathrm{CL}}}$ (Theorem~\ref{thm:explicit-theta})
has vanishing genus-$0$ component --- the bar complex is
uncurved at tree level.  The identification of the Koszul dual with the
...
but requires independent verification at higher genus.  The current
theorematic input therefore stops at boundary/bar data and same-family
shadow information; the filtered H-level bulk target together with its
exact stagewise coefficient comparison belongs to MC4.  The physical
bulk/bar identification built on top of that comparison is the
downstream MC5 step.
...
\begin{remark}[Scope]
This conjecture requires constructing the chiral operad $\mathcal{P}_{\mathrm{HCS}}$
from the holomorphic Chern--Simons path integral.  The operad structure
(composition of observables with marked boundary points) is conjecturally predicted from
...
\noindent\emph{Status split.} The open-string bar identification is
\ClaimStatusProvedHere; the closed-string cobar identification and the
full open-closed equivalence remain \ClaimStatusConjectured.
...
is conjecturally modeled by bar-cobar duality: opening up the disk to a sphere with punctures sends boundary operators to bulk insertions via Stokes' theorem, with residues at the boundary corresponding to distributions in the bulk.
\end{remark}
...
complex coincides with the cobar complex.  In particular, the proved
2D/bar-semi-infinite input stops at boundary data and same-family
shadow partners; any corresponding H-level bulk or infinite-generator
target belongs upstream to MC4, and the open-closed field-theory
comparison itself is the downstream MC5 step.

\emph{Homotopy template} (Convention~\ref{conv:hms-levels}): Type~II (equivalence of homotopy categories).
The conjecture asserts an equivalence between the cobar complex $\Omega^{\mathrm{ch}}(\mathcal{C}_{\mathrm{bulk}})$ and the closed string field theory state space, with the open-closed correspondence interpreted by a bar-cobar adjunction at the level of $\infty$-categories.
(Contributing to Conjecture~\ref{conj:master-bv-brst}.)
\end{remark}
...
\begin{remark}[Scope]
This conjecture asserts that each step in the dimensional reduction tower
...
\emph{Homotopy template} (Convention~\ref{conv:hms-levels}): Type~VII (physics-dictionary).
The conjecture treats each step of the dimensional reduction tower
$4\mathrm{d} \to 3\mathrm{d} \to 2\mathrm{d} \to 1\mathrm{d}$ as
modeled by a bar or cobar functor, with conjectural quasi-isomorphisms
between the $L_\infty$ algebras controlling deformations at adjacent
dimensions supplied only after additional physics input.
(Contributing to Conjecture~\ref{conj:master-bv-brst}.)
\end{remark}
...
Each reduction step should be modeled by applying bar or cobar
construction to the previous level.
\end{example}

\section{$\mathcal{W}$-algebras from Hitchin moduli}
\noindent\emph{Status split.} Generic-level bar and Hochschild
statements in this section are \ClaimStatusProvedHere; admissible-level
Koszul refinements and gauge-theoretic identifications are
...
\emph{Homotopy template} (Convention~\ref{conv:hms-levels}): Type~VII (physics-dictionary).
The conjecture treats the chiral algebra of local operators on the
Hitchin moduli space $\mathcal{M}_{\mathrm{Hit}}(\Sigma_g, G)$ as
conjecturally modeled by the W-algebra $\mathcal{W}(G)$, relating the
deformation problem of Higgs bundles to that of the W-algebra through a
conjectural quasi-isomorphism of controlling $L_\infty$ algebras.
(Contributing to Conjecture~\ref{conj:master-bv-brst}.)
\end{remark}
...
\begin{theorem}[$\mathcal{W}$-algebra bar complex; \ClaimStatusProvedHere]
...
this monograph.
The genus-graded nilpotency $(d_\hbar)^2 = 0$ is the direct
consequence of the Maurer--Cartan equation satisfied by
$\Theta_\cA$ (Theorem~\ref{thm:explicit-theta}):
$\ell_1(\Theta) + \tfrac{1}{2}\ell_2(\Theta, \Theta) = 0$
encodes the identity $\sum_{g_1 + g_2 = g} d_{g_1} \circ d_{g_2} = 0$
at each genus, with $\hbar$ serving as the genus bookkeeping
parameter from the modular operad decomposition.
\end{remark}
...
Both limits are visible in the universal MC class
(Theorem~\ref{thm:explicit-theta}): the Lie-algebraic limit
$c = 0$ corresponds to $\Theta_\cA = 0$ (uncurved bar
complex), while the semiclassical limit $c \to \infty$
sends $\kappa \to \infty$, producing a strongly curved
$A_\infty$ structure whose curvature $m_0 = \kappa \cdot
\mathbf{1}$ dominates all higher operations.
\end{example}

\section{Summary and outlook}

\begin{remark}[Summary]
Chiral algebras appear as boundary operator algebras for HT field
theories and arise from twisted 4d gauge theories; this chapter proves
the chain-level boundary/bar data and isolates the open conjectural
layer.  The open-closed, AGT, and loop-correction interpretations are
programme-level comparison statements built on that theorematic
boundary package rather than additional proved identifications.
\end{remark}
...
\begin{definition}[$\mathcal{W}$-algebra (CFT definition)]\label{def:w-algebra-cft}
...
\begin{conjecture}[$\mathcal{W}$-algebra bar-cobar duality]\label{conj:w-algebra-bar-cobar}
...
between the classical BV complex (Costello--Gwilliam~\cite{CG17}) and the geometric bar complex.  For non-free algebras, the precise equivalence remains conjectural (\ClaimStatusConjectured) and has not been fully established.
\end{theorem}
...
\item The quantum bar differential: the total differential $\Dg{g} = d_0 + \sum_{k=1}^g t_k d_k$
...
gauge-theoretic input beyond the methods of this monograph.  The
current theorematic 2D input identifies only the bar/semi-infinite
complex and the same-family partner data on the boundary side.

It does not yet construct the H-level infinite-generator targets on
which the physics comparison would rest.  The dependency order is now
exact.  First comes the MC4 layer: a filtered factorization model on the
$W_\infty$ side whose finite quotients match the proved principal
stages, and in parallel an RTT-adapted dg-shifted filtration on the
Yangian side whose finite quotients recover the finite RTT tower.
Only after those targets exist does the MC5 layer begin: the bulk/bar or
AGT identification with the physical factorization algebra.

In the present state of the manuscript, these are no longer vague
existence problems: the Yangian side now reduces to RTT-triangularity
and pole-order locality for the line-operator formulas, while the
$W_\infty$ side reduces to stable higher-spin factorization ideals
whose quotients recover the principal stages.  At the theorematic
bookkeeping level, the Yangian preservation statement is now extracted
from the rational line-operator formulas themselves, and the
$W_\infty$ statement is extracted from spin-triangular OPE formulas.

The remaining MC4 step is coefficient-level identification:
the Yangian quotients must satisfy the truncated RTT relations with the
standard evaluation modules, while the $W_\infty$ quotients must match
the principal Drinfeld--Sokolov OPE coefficients and finite-stage bar
operations.

At this point even that statement can be read literally:
one must extract the local kernel coefficients from the rational
line-operator formulas and the local OPE/residue coefficients from the
higher-spin boundary algebra, then check equality with the principal
finite-stage RTT and Drinfeld--Sokolov coefficients.  The MC5 physics
comparison is downstream of that finite-stage equality statement, not a
substitute for it.

Equivalently, the live comparison now asks for the mode-by-mode
identities
...
The next reduction is finite:
the Yangian identities should be checked on a faithful evaluation
family, and the $W_\infty$ identities on the finitely many
generator-level residue coefficients from which translation generates
all descendants.

At fixed stage $N$, this is now an actual finite checklist: the
Yangian side reduces to the boundary strip $\Delta_{a,0}(N)$, and the
$W_\infty$ side to the finite primary index set $\mathcal{I}_N$.

The Yangian boundary strip is itself detected on generic tensor powers
of the fundamental evaluation module of lengths at most $N+1$, and it
collapses further once the fundamental $L$-operator and twisted
coproduct satisfy the standard RTT propagation law.  For the standard
type-A RTT tower, the remaining pairwise step on the fundamental line
is already reduced to three local checks: $\mathfrak{sl}_M$-equivariance,
unit asymptotic, and residue normalization~$-\hbar P$.  Once the
line-operator construction fixes the symmetry and asymptotic pieces,
the only genuinely analytic Yangian check is the residue formula, now
reduced further to the two residue eigenvalues on $\operatorname{Sym}^2(V)$ and
$\Lambda^2(V)$, and in fact to the single ordered tensor-line residue
$e_1\otimes e_2\mapsto -\hbar\,e_2\otimes e_1$.

On the $W_\infty$ side, the first nontrivial stage beyond Virasoro is
already an explicit fifteen-coefficient packet at stage~$3$; the
explicit $W_3$ OPE reduces that packet to three nonzero primary
coefficients and twelve forced vanishings.

At stage~$4$, the remaining six top-pole coefficients are organized as
three local OPE blocks, and the only genuinely mixed higher-spin data
are the three coefficients in the $W^{(3)}$-$W^{(4)}$ block.  Among
those three, the $W^{(3)}$ target channel is swap-even under reversing
the order of the mixed OPE, while the $W^{(2)}$ and $W^{(4)}$ target
channels are swap-odd.  The principal Drinfeld--Sokolov $W^{(2)}$
target then vanishes by the mixed-weight Ward-identity argument, while
the principal $W^{(4)}$-$W^{(4)}\to T$ coefficient is fixed to `2' by
the universal self-coupling Ward-identity computation.  Thus the live
stage-$4$ comparison is the exact six-entry identity packet on
~$\mathcal{I}_4$:
\[
...
\end{gathered}
\]
This is the same first open higher-spin packet isolated in
Corollary~\ref{cor:winfty-stage4-closure-criterion}.

Stage~$4$ is already compressed further: after removing the exact
stress-tensor sector and the theorematic $(3,3)$ $W_3$ sector, the
residual higher-spin packet has exactly twenty-nine primary
coefficients.  Primaryity then reduces those to seven top-pole
coefficients and twenty-two forced zeros.  Skew-symmetry removes the
odd self-OPE entry $(4,4,3,5)$, leaving six live coefficients and
twenty-three forced zeros, organized as three explicit local OPE
blocks.  Among those six, only the triple in the mixed
$W^{(3)}$-$W^{(4)}$ block is genuinely mixed higher-spin data.
...
The AGT correspondence admits a reformulation in bar-cobar terms: on
the 4D side, the BV complex of gauge theory is the bar complex of the
factorization algebra; on the 2D side, configuration space integrals
give the bar complex of the chiral algebra, which equals the
semi-infinite complex of $\mathcal{W}_k(\fg)$
(Theorem~\ref{thm:bar-semi-infinite-w}).  The 2D identification is a
theorem; the 4D identification is gauge-theoretic input.  The present
bridge therefore stops at the boundary/bar-semi-infinite comparison and
its same-family shadow data, not at an already-realized H-level
infinite-generator holographic dual.  The remaining frontier is
dependency-ordered: MC4 first constructs the filtered targets with the
correct finite quotients, and only then does MC5 compare those targets
with the physical bulk theory.  Concretely, the remaining MC4 inputs are
the exact identities
$\mathsf{C}^{\mathrm{res}}_{s,t;u;m,n}(N)
=\mathsf{C}^{\mathrm{DS}}_{s,t;u;m,n}(N)$ on the finite
$W_\infty$ seed packets $\mathcal{I}_N$ and
$\mathsf{K}^{\mathrm{line}}_{a,b}(N)
=\mathsf{K}^{\mathrm{RTT}}_{a,b}(N)$ on the Yangian boundary strips
$\Delta_{a,0}(N)$.  One must first build the physical factorization
targets whose local formulas realize those packets before asking for
the final BV/bar or holographic identification.
\end{remark}
...
\begin{theorem}[$E_n$ Koszul duality on $n$-manifolds;
...
theory of Feynman transforms \cite{GeK98} motivate the following
conjectural bridge (\ClaimStatusConjectured): the bar complex
...
This should be read as the first theorematic non-principal family, not
as a resolution of Conjecture~\ref{conj:w-orbit-duality}.  The general
orbit-duality theorem remains a distinct non-principal frontier, while
any bulk interpretation requiring filtered H-level infinite-tower
comparisons beyond the theorematic principal-stage packages belongs to
the separate standard MC4 $\mathcal{W}_\infty$/Yangian lane.
\end{remark}
