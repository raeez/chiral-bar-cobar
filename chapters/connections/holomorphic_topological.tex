...
\begin{remark}[Scope]
For $\mathcal{N}=4$ SYM, chirality is proved (Theorem~\ref{thm:cl-n4-chirality}, Costello--Li \cite{CL16}).  The general characterization remains conjectural: the three conditions involve physics input (Coulomb branch holomorphicity, anomaly vanishing, $\mathcal{D}$-module extension to Ran space).  See also \cite{CDG20} for boundary chiral algebras, \cite{GKW24} for perturbative BRST-anomaly brackets (local input, not curvewise bar comparison), and \cite{Zeng23} for twisted holography.  Computing $\barB^{\mathrm{ch}}(\mathcal{A}_{\mathrm{bdy}})$ for CDG20 boundary algebras remains open.
...
\begin{remark}[Scope]
Conditions~(i)--(iii) are motivated by anomaly cancellation but not mathematically established beyond $\mathcal{N}=4$.  In our framework, $c_1(R) = 0$ means $\Theta_{\cA_{\mathrm{CL}}}$ (Theorem~\ref{thm:explicit-theta}) has vanishing genus-$0$ component.  The Koszul dual/Langlands dual identification is a physics prediction from S-duality.  Filtered targets and coefficient identities belong to MC4; the bulk/bar identification is the downstream MC5 step.
(Contributing to Conjecture~\ref{conj:master-bv-brst}.)
\end{remark}
...
\begin{remark}[Scope]
The open-string identification is proved for Kac--Moody (Theorem~\ref{thm:bar-semi-infinite-km}) and W-algebra (Theorem~\ref{thm:bar-semi-infinite-w}) boundary conditions.  The closed-string half requires constructing the closed string field theory action \cite{Zwi93} and verifying its BRST complex coincides with the cobar complex; the bulk target belongs to MC4 and the field-theory comparison to MC5.
(Contributing to Conjecture~\ref{conj:master-bv-brst}.)
\end{remark}
...
\begin{remark}[Scope]
Each individual level is established: factorization algebras produce chiral algebras \cite{BD04}, and boundary conditions yield modules \cite{CG17}.  The iterated bar-cobar claim requires the holomorphic twist and successive dimensional reductions; the $1$d identification uses Costello--Witten--Yamazaki \cite{CWY18}.
(Contributing to Conjecture~\ref{conj:master-bv-brst}.)
\end{remark}
...
\begin{remark}[Scope]
The 2D side is proved (Theorem~\ref{thm:agt-2d-bar}); the 4D--2D AGT bridge \cite{AGT09} is proved in specific cases \cite{SV13,MO19} but not in full generality.  The remaining stagewise coefficient identities ($\mathsf{K}^{\mathrm{line}} = \mathsf{K}^{\mathrm{RTT}}$ on $\Delta_{a,0}(N)$, $\mathsf{C}^{\mathrm{res}} = \mathsf{C}^{\mathrm{DS}}$ on $\mathcal{I}_N$; six coefficients at stage~$4$ by Corollary~\ref{cor:winfty-stage4-closure-criterion}) belong to MC4; the bulk identification is the downstream MC5 step (Chapter~\ref{chap:concordance}).  Contributing to Conjecture~\ref{conj:master-bv-brst}.
\end{remark}
