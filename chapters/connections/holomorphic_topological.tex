...
\textbf{Key fact:} Both constructions produce \emph{the same} W-algebras. 
...
implies that Verma modules are irreducible (the Shapovalov form is
non-degenerate at each weight), so
$\mathrm{Ext}^q_{\mathcal{O}_k}(V(h_1), V(h_2)) = 0$ for $q > 0$ and all pairs of irreducible Verma modules (no non-trivial extensions between non-isomorphic irreducibles).
...
The AGT correspondence, originally a subtle duality between 4D gauge theory
