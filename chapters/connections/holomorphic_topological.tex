...
Chiral algebras arise as boundary operator algebras of 4d holomorphic-topological field theories: the bar construction on the 2d boundary is the descent of the 4d BV complex, and Koszul duality becomes the 4d S-duality.
\end{remark}

\begin{remark}[H/M/S route; Convention~\ref{conv:hms-levels}]
Mixed-level; physical identifications are programme material unless theorem-labeled.
\end{remark}
...
comparison.  The remaining filtered targets and coefficient
identities belong to MC4; see Chapter~\ref{chap:concordance} for the
exact frontier.
...
\begin{conjecture}[\texorpdfstring{$\mathcal{W}$}{W}-algebra bar-cobar duality]\label{conj:w-algebra-bar-cobar}
...
which the physics comparison would rest.  The filtered targets and
coefficient identities belong to MC4, with the AGT/bulk identification
downstream at MC5; see Chapter~\ref{chap:concordance} for the exact
dependency order and remaining packets.
...
theorem; the 4D identification is gauge-theoretic input.  The remaining
frontier (MC4 filtered targets, then MC5 bulk comparison) is recorded
in Chapter~\ref{chap:concordance}.
\end{remark}
