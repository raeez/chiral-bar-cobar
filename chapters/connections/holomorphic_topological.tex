...
For $\mathcal{N}=4$ SYM, chirality is proved
(Theorem~\ref{thm:cl-n4-chirality},
established by Costello--Li~\cite{CL16}).
The general ``if and only if'' characterization for arbitrary
$\mathcal{N}=2$ matter content remains a physics conjecture.  See also Costello--Dimofte--Gaiotto \cite{CDG20}
...
but requires independent verification at higher genus.  The proved theorematic core covers boundary/bar data
(Theorems~A--D) and the universal Maurer--Cartan class
$\Theta_{\cA}$ (MC2); the filtered H-level bulk target
together with its exact stagewise coefficient comparison
belongs to the MC4 frontier.  The physical bulk/bar
identification built on top of that comparison is the
downstream MC5 step.
