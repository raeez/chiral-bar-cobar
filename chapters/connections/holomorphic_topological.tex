...
\noindent\emph{Differential conventions.}  Throughout this chapter, we use the differential notation of Convention~\ref{conv:higher-genus-differentials}: $\dfib$ for the fiberwise curved differential ($\dfib^{\,2} = \kappa \cdot \omega_g$), $\Dg{g}$ for the total strict differential ($\Dg{g}^{\,2} = 0$), and $\dzero$ for the genus-$0$ collision differential.

\medskip

\noindent\emph{Governing question: how far does the Costello--Li
holomorphic-topological output already align with the modular Koszul
programme?  We separate three regimes explicitly:
\textup{(i)} proved constructions from the twist
\textup{(}\ClaimStatusProvedElsewhere\textup{)},
\textup{(ii)} chain-level bar/cobar compatibility in this monograph
\textup{(}\ClaimStatusProvedHere\textup{)}, and
\textup{(iii)} functorial boundary/open-closed identifications
\textup{(}\ClaimStatusConjectured\textup{)}.}

\begin{remark}[Semantic levels; Convention~\ref{conv:hms-levels}]
Proved results (Costello--Li twist, boundary algebra identification)
operate at \textbf{M-level} (explicit chain-level constructions).
The conjectured extensions---AGT as factorization Koszul duality,
4d/2d as functorial boundary---are \textbf{H-level} programme items
requiring $\infty$-categorical input.  Numerical checks (central
charges, partition functions) are \textbf{S-level} shadows.
\end{remark}
...
remaining comparison is now explicit: one must construct a separated
complete factorization-theoretic target whose finite quotients recover
the theorematic $W_N$ or RTT stages.
...
shadow information; an H-level infinite-generator bulk dual remains on
the MC4/MC5 frontier.
...
shadow partners; identifying the corresponding H-level bulk or
infinite-generator dual object remains on the MC4/MC5 frontier.
...
It does not yet construct an H-level infinite-generator bulk dual,
which remains part of the MC4/MC5 frontier.  More precisely, the
missing bulk step is a filtered factorization model whose finite
quotients match the proved principal stages.  On the Yangian flank, the
parallel missing input is an RTT-adapted dg-shifted filtration whose
finite quotients recover the finite RTT tower.
...
infinite-generator holographic dual; that stronger object remains on
the MC4/MC5 frontier and now reduces to the construction of a filtered
target with the correct finite quotients.  Concretely, the remaining
inputs are the higher-spin factorization-ideal theorem on the
$W_\infty$ side and the RTT-triangularity theorem on the Yangian side:
one must build the actual physical factorization targets whose local
formulas satisfy those theorematic criteria.
\end{remark}
...
orbit-duality theorem and any bulk interpretation requiring completed
infinite-generator objects remain on the non-principal/MC4 frontier.
\end{remark}
