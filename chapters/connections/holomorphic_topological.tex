...
\begin{theorem}[W-Algebra Bar Complex; \ClaimStatusConjectured]
...
\begin{remark}[Scope]
The claim that the bar complex of $\mathcal{W}_N$ computes conformal blocks
and encodes fusion rules is a combination of two ingredients:
(1)~the general bar complex machinery from Part~I, which applies to any
chiral algebra; and (2)~the identification of bar complex cohomology with
conformal blocks, which for $\mathcal{W}_N$ requires representation-theoretic
input (Feigin--Frenkel \cite{FF92}, Arakawa \cite{Ara07}).  The modular
functor structure (item~3) further requires compatibility with
$\overline{\mathcal{M}}_{g,n}$ compactification, which is established for
the Virasoro case ($\mathcal{W}_2$) but remains conjectural for general
$\mathcal{W}_N$.
\end{remark}
...
\begin{theorem}[W-Algebra Bar-Cobar Duality; \ClaimStatusConjectured]\label{thm:w-algebra-bar-cobar}
...
\begin{remark}[Scope]
Items~(1) and~(4) follow from the general bar-cobar framework of Part~I:
the geometric bar construction applies to any chiral algebra, and OPE data
is encoded by residues on FM compactifications.  Item~(3) (bar-cobar
quasi-isomorphism) follows from the general theory
(Theorem~\ref{thm:bar-cobar-isomorphism-main}) once the Koszul property
is established.  The key gap is item~(2): the Koszul property for
$\mathcal{W}_k(\mathfrak{g})$ is known only for specific cases
($\mathcal{W}_3$ at $c = -2$ by Arakawa \cite{Ara07}) and remains
conjectural for general $\mathcal{W}_k(\mathfrak{g})$.  Establishing the
Koszul property in general is one of the central open problems of this
monograph (Move~1 of the Execution Plan).
\end{remark}

\begin{proof}[Proof Strategy]
The proof follows the general bar-cobar framework established in Part~I,
...
\textbf{Step 3: Koszul property.} This is the deep step.  For $\mathcal{W}_3$ at $c = -2$ (admissible level), Arakawa~\cite{Ara07} proved rationality and $C_2$-cofiniteness of the representation category. We conjecture that this implies the chiral Koszul property, and extend it to the chiral algebra setting using:
\begin{itemize}
...
\item Explicit verification in low degrees (Example \ref{ex:w3-c-minus-2-explicit})
\end{itemize}
