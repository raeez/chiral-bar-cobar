...
Many of the chiral algebras in this monograph --- affine Kac--Moody, W-algebras, certain vertex superalgebras --- arise as boundary operator algebras of four-dimensional gauge theories.  The mechanism is the \emph{holomorphic-topological twist}: starting from 4d $\mathcal{N}=2$ Yang--Mills on $\mathbb{C} \times \Sigma$ (where $\Sigma$ is holomorphic and $\mathbb{C}$ is topological), one obtains a factorization algebra on $\Sigma$ whose global sections form a chiral algebra.  In this setting, bar-cobar duality realizes the open-closed correspondence: the bar complex computes bulk observables (closed strings), while the cobar complex recovers the boundary algebra (open strings).  The genus expansion $F_g(\mathcal{A})$ computes the genus-$g$ partition function of the twisted theory.
...
The preceding conjecture identifies three conditions for the CL construction
to produce a genuine chiral algebra.  The following theorem and conjecture
make this precise in terms of the gauge-theoretic data $(G, R)$.
...
\emph{Open string sector} (proved above).
\begin{itemize}
...
\emph{Closed string sector} (conjectural).
\begin{itemize}
...
\begin{conjecture}[Factorization along dimension tower; \ClaimStatusConjectured]
...
\end{itemize}
The 2D identification is now a theorem; the 4D identification is
