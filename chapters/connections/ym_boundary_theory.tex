...
\item Verdier duality converts the open sector into a factorization algebra:
\[
\mathbb{D}_{\Ran}\, \mathsf{Open}_{\cB} \simeq \mathsf{Bulk}^{\vee}_{\cB} = \barBch(\cA_{\cB}^!).
\]
On cohomology, $H^\bullet(\mathbb{D}_{\Ran}\, \mathsf{Open}_{\cB}) \simeq \cA_{\cB}^!$ (Theorem~A, Verdier intertwining).
\end{enumerate}
\end{theorem}
...
Theorem~\ref{thm:verdier-bar-cobar} (Verdier intertwining $\mathbb{D}_{\Ran}\, \barBch(\cA) \simeq \barBch(\cA^!)$) specialized to $\cA_{\cB}$.
\end{proof}
