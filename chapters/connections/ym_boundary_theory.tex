...
Yang--Mills theory is the hardest test this programme can face.  The
boundary data of a four-dimensional gauge theory are nonlinear,
gauge-theoretic, and physically meaningful: they encode the chiral
algebra of a twisted holomorphic-topological theory whose defect
categories are controlled by bar-cobar duality.  If the abstract
machinery of Parts~I and~II is to have genuine physical content, it must
survive contact with the nonperturbative, instanton-rich, confining
world of Yang--Mills.

The strategy is a chain of controlled reductions.  From the boundary
chiral algebra one passes, by Koszul duality, to its derived center;
from the derived center one reads off mixed and higher-body couplings
via relative Hochschild cohomology and \v{C}ech descent; instanton
completion enlarges the framework to include nonperturbative sectors;
central screening extracts the genuinely visible infrared content; and a
conditional spectral-gap argument translates visible-center collapse
into a mass-gap criterion.  Each arrow in this chain is internal to the
mathematics developed in the present chapter and the two that follow
it---only the final passage from the twisted to the untwisted theory
remains conditional.

The emphasis throughout is on isolating what the algebraic engine proves
unconditionally from what requires additional physical input.  The
boundary BRST package, the tangent-to-center correspondence, central
formality, relative derived centers, and the screening Hodge theorem are
all proved here.  The realization of the dual algebra as the full
physical defect algebra, and the existence of a natural Hilbert-screening
bridge for untwisted Yang--Mills, remain conditional---but they are no
longer outside the formalism.
...
\]
The first four arrows are internal to the mathematics developed in the
present chapter and the two that follow it.  The passage from the
visible defect category to actual untwisted Yang--Mills dynamics remains
conditional, but it is no longer outside the formalism: it has been
reduced to the construction of a natural untwisting bridge datum and
the verification of a screening domination estimate
(Chapter~\ref{ch:ym-instanton-screening}).

\begin{theorem}[Bridge theorem: boundary chiral algebra $\to$ mass-gap criterion via bar-cobar; \ClaimStatusProvedHere]
...
\;\xrightarrow{\;\text{bar}\;}
\cA_{\cB}^!
\;\xrightarrow{\;\text{Koszul}\;}
Z(\cA_{\cB}^!)
\;\xrightarrow{\;\text{screening}\;}
Z^{\mathrm{vis}}
\;\xrightarrow{\;\text{gap}\;}
\text{mass-gap criterion}
\]
...
\item \textup{(Bar $\to$ Koszul dual)}\quad
  Verdier duality on the bar complex yields the cobar complex of
  the Koszul dual:
...
\item \textup{(Mass-gap criterion)}\quad
  Visible-center collapse plus the screening Hodge theorem give a
  conditional spectral-gap criterion: if the screening Laplacian
  dominates the untwisted Hamiltonian, the mass gap is positive
  \textup{(}Theorem~\textup{\ref{thm:conditional-mass-gap-transfer}}\textup{)}.
\end{enumerate}
Steps~\textup{(i)--(iv)} are proved unconditionally.
Step~\textup{(v)} is conditional on the existence of an untwisting
bridge datum and a domination estimate.
\end{theorem}
...
\begin{theorem}[Grand synthesis principle; \ClaimStatusProvedHere]
...
The role of the present section is to isolate the mathematically controlled input from
...
\]
where $RHH_{\mathrm{ch}}(\cA_{\cB})$ carries the standard shifted dg~Lie structure of the chiral Hochschild complex --- a strict model of the underlying homotopy-invariant deformation $L_\infty$-algebra (Remark~\ref{rem:conv-strict-vs-homotopy}).  Its Maurer--Cartan functor governs formal boundary deformations of~$\cA_{\cB}$.
\end{definition}
...
\end{enumerate}
These statements push the framework from a static duality theorem to an actual local geometry of boundary moduli.
\end{remark}
