...
\index{MA-5!corrected explanation|textbf}
...
through the bar of the open algebra alone (the master critique
classifies this slogan as a forbidden form; cross-volume Vol~II
...
$\cA$, the bar-cobar construction should encode the following
...
their pairing is expected to model the local S-matrix pairing:
\[
...
For an uncurved $A_\infty$ algebra in the Keller convention used in
Appendix~\ref{app:homotopy-transfer}, the operations satisfy the
constraint
\[
m_1 \circ m_n =
...
\item \textbf{Vol.~III:} the refined CY-$3$ output of the two-stage
 ...
 is expected to compare with a refined BKM algebra whose denominator
...
the $24$-term KK-mode sum at $\hbar^2 = -1/8$ is the same scalar
evidence entering the conditional $\Delta_5$ target comparison.
\end{proof}
