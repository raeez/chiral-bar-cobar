...
\item \emph{Cross-volume bridges.} Vol~II's universal celestial
 holography (Vol~II Theorem~\ref{thm:universal-holography-master})
 ...
 K3-fibered Class~A threefolds, Vol~III's CY-to-chiral
 output realises these amplitudes as BPS-state count generating
...
\begin{remark}[Explicit diagrammatic computation; \ClaimStatusProvedElsewhere]
...
Vol~I~\cite[Theorem~\ref*{V1-thm:bvbrst-4loop-feynman-sum}]{Lorgat1}
inscribed at \texttt{chapters/connections/bv\_brst.tex}
\S\ref*{V1-subsec:bvbrst-4loop-enumeration}.}
...
\begin{remark}[Wave 22: diagrammatic moves and 5-loop extension]
\label{rem:feyn-wave22-5loop-companion}
\index{Wave 22!4-loop diagrammatic moves}
\index{Wave 22!5-loop cancellation}
...
Theorem~\ref*{V1-thm:bvbrst-wave22-5loop-sum}.
\end{remark}
...
\item \textbf{Vol.~III:} the refined CY-3 $\Phi_3$ output
...
\hbar_2 = \epsilon_2$ distinguished (AP151). Cross-ref Vol.~II
core/frontier companion \texttt{ordered\_associative\_chiral\_kd.tex}
Remarks~\ref{rem:v2-oackd-W15-two-param},
\ref{rem:v2-oackd-W15-e1-e2};
...
\begin{remark}[Self-dual slice recovers chiral Koszul discipline; \ClaimStatusProvedElsewhere]
...
\begin{remark}[AP-CY20 spectral-parameter mechanism via $\Omega$-intermediary; \ClaimStatusHeuristic]
\label{rem:feyn-W16-apcy20-omega-intermediary}
Cache-guard AP-CY20: the spectral parameters of the MO-Yangian on
...
\item \textbf{Spectral output.} The arguments of the R-matrix
  ...
  (see Vol~II \S\ref{sec:fsc-K3-vs-A2}).
\end{enumerate}
The AP-CY20 discipline: the chain $N_{C/Y} \to \Omega \to (u, v)$ is
...
residues matching the Wave-$18$ Jacobi coefficients $c_n =
c_{\phi_{-2,1}}(n)$ are recorded in this section.

\begin{proposition}[Self-dual Nekrasov
$= M_{24}$-Liouville on $\Sigma_{0,24}$; \ClaimStatusProvedElsewhere]
...
\]
matching Theorem~\ref{thm:K3xE-coha-character-igusa} of Vol~III
...
\begin{remark}[Humbert-pole residues $c_n\cdot [H_n]$:
...
$\log Z^{\mathrm{Nek}}$ at $H_n$ equals the Wave-$18$
...
Wave-$18$ check: direct $q, y$-expansion of
...
$2d$ chiral algebra output of Vol~III's CY-$3$ functor
$\Phi_3$ applied to $D^b(K3\times E)$; the $24$-term
