...
\begin{theorem}[General $m_k$ Structure; \ClaimStatusConjectured]
\label{thm:mk-general-structure}
For general $k \geq 2$, including all-genus quantum corrections, the operation $m_k$ has the conjectural Feynman expansion:
...
For the $A_\infty$ operations $m_k$ arising from homotopy transfer, only \emph{tree-level} graphs contribute ($\ell = 0$): the homotopy transfer formula sums over planar binary trees with $k$ leaves and $k-2$ internal edges.  (Note: the Feynman graph loop number $\ell$ is distinct from the genus~$g$ of the underlying curve; genus-$g$ bar complex contributions come from $\bar{B}^{(g)}$, not from~$m_k$ alone.)
\end{theorem}

\begin{proof}[Sketch]
By the loop number formula $\ell = E_{\mathrm{int}} - V_{\mathrm{int}} + 1$
for connected graphs, a planar binary tree with $k$ leaves has $V_{\mathrm{int}} = k-1$ internal vertices (each trivalent) and $E_{\mathrm{int}} = k-2$ internal edges, giving $\ell = (k-2) - (k-1) + 1 = 0$.  The restriction to tree-level is not a combinatorial bound on loop numbers but a structural property of the homotopy transfer formula: each propagator insertion corresponds to an internal edge of the tree, and the total number of insertions is fixed at $k-2$ by the form-degree constraint on the configuration space $\overline{C}_{k}(X)$.
\qed
\end{proof}
