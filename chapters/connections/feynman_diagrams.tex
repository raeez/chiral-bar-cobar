...
\begin{proposition}[Two-channel reduction for a compactified logarithmic
 ternary packet; \ClaimStatusProvedHere]
\label{prop:compactified-ternary-two-channel}
\textup{(}\emph{quadrant} geometric/logarithmic comparison lemma for
the Open disk-local lane, \emph{presentation} logarithmic $1$-forms on
$\overline{\mathcal{M}}_{0,4} \cong \bP^1$,
\emph{level} local ternary boundary stratum,
\emph{hypothesis package} two already constructed logarithmic
$1$-forms on $\overline{\mathcal{M}}_{0,4} \cong \bP^1$, poles only at
the three boundary points, the same boundary-channel labelling and
orientation convention, the residue theorem on~$\bP^1$, and
...
This proposition proves only the purely logarithmic $\bP^1$ residue
reduction: it does not construct the perturbative ternary form, does
not compare it with the bar form, and does not prove
...
\begin{remark}[Conditional application to the disk-local ternary packet]
...
Conjecture~\ref{conj:v1-disk-local-perturbative-fm} only after the
compactified perturbative and bar logarithmic forms have been
constructed on $\overline{M}_{0,4}$ with the stated pole and
orientation conventions. Under that conditional application, the
proposition reduces the ternary comparison from three residue
equalities to two. It does not supply the compactification/Stokes
upgrade or the perturbative/FM identification.
\end{remark}
...
\begin{proof}
Corollary~\ref{cor:genus0-standard-chart-two-residues} reduces the
claim to equality of the residues at $0$ and $1$ in the
...
\item \emph{Universal conductor.} The genus-$1$ free energy
 $F_1=\kappa(\cA)/24$ is the graph evaluation of the pointed trace
 $\operatorname{tr}_1\operatorname{Obs}^{\mathrm{def}}_{1,1}(\cA)
 =\kappa(\cA)\lambda_1$.  In a BV model it is
...
\item \emph{Climax identity.} The bar differential is the signed sum
 of collision residues of the chiral product, with the affine/formal
 representative $\eta_{ij}=d\log(z_i-z_j)$ supplying the Arnold
 logarithmic form.  Chapter~\ref{ch:climax-theorem} states this as a
 typed KZ--Arnold bar-superconnection theorem whose bar operator is
 obtained only after Fulton--MacPherson boundary-residue realization.
 This is an algebraic residue theorem.  The off-shell/on-shell reading
 is the separate conjectural pairing of
 Conjecture~\ref{conj:physical-pairing}.
...
\begin{remark}[Four-loop comparison problem on $K3\times\bC^2$;
\ClaimStatusOpen]
\label{rem:feyn-4loop-explicit-expansion}
\index{4-loop Feynman!explicit expansion}
\index{K3 4-loop enumeration}
The number-theoretic target is the Humbert-divisor coefficient of the
Borcherds product of $\phi_{0,1}$.  A four-loop physics comparison
requires the twisted-supergravity BV complex, its connected vacuum
graphs, propagators, symmetry factors, regularized amplitudes, and a
chain map from the BV obstruction complex to the Heegner divisor
complex.  The target coefficient is known, but the comparison graph
integral remains to be constructed.
Gritsenko--Nikulin determine the divisor multiplicities from the
Fourier coefficients of $\phi_{0,1}$.  This arithmetic calculation is
the target of the proposed BV-to-Heegner map and supplies no graph
amplitude before that map and its regularization have been built.
\end{remark}

\begin{remark}[Five-loop comparison problem; \ClaimStatusOpen]
\label{rem:feyn-5loop-companion}
\index{4-loop!diagrammatic moves}
\index{5-loop!cancellation}
The five-loop comparison has the same type signature as the
four-loop problem: a connected BV graph complex and its regularized
amplitude map on the physics side, a Humbert-divisor coefficient on
the automorphic side, and a chain map between them.  The arithmetic
coefficient is determined from the Fourier expansion of
$\phi_{0,1}$.  Its identification with a five-loop obstruction is the
equation defining the conjectural comparison map~$\Psi_5$ in
