...
\item \textbf{3-loop ($\hbar^3$).} $F^{(3)} \neq 0$, carrying the
 ...
 (\Phi_{10}^{\mathrm{un}}/\eta^{24}) c_{\Phi_{10}^{\mathrm{un}}}$ is the explicit pentagon
 coboundary witness at order $\hbar^3$ in
 $\mathsf{SC}^{\mathrm{mod}}$. The pentagon obstruction is detected
 by the $\Phi_{10}^{\mathrm{un}}/\eta^{24}$-twisted associator of the Hall--Drinfeld
...
order, and the 3-loop Pentagon obstruction reduces to the classical
 ...
 $\Phi_{10}^{\mathrm{un}}$ cocycles are $\mathbb{Z}/2$-odd and vanish under
 $\epsilon_1 \mapsto -\epsilon_2$ exchange). The full
 $\Phi_{10}^{\mathrm{un}}/\eta^{24}$-twisted Drinfeld associator is therefore
 ...
 \times E}, \widetilde{\Phi}^{\mathrm{Sieg\text{-}Bor}}[\Phi_{10}^{\mathrm{un}}/
...
$\mathrm{ch}(\mathbf H_{\Delta_5})=(\Phi_{10}^{\mathrm{un}})^{-1}$,
and the Humbert-pole
...
$= M_{24}$-Liouville on $\Sigma_{0,24}$ under the
$M_{24}$-symmetric class-$\mathcal S$ realisation]
...
correlator graded character equals $(\Phi_{10}^{\mathrm{un}})^{-1}$;
(Borcherds
lift of Gritsenko--Nikulin~1998 Thm.~$2.1$) the Siegel slice
of $\log(\Phi_{10}^{\mathrm{un}}/\eta^{24})$ expands with
