...
\begin{theorem}[Heuristic Feynman--bar mnemonic; \ClaimStatusHeuristic]
...
Feynman-style reading is meant to probe:
\begin{center}
...
\begin{proof}
The safe bar-side content is as follows.  Theorem~\ref{thm:bar-cobar-isomorphism-main}
identifies the bar differential with residues of the logarithmic forms
$\eta_{ij} = d\log(z_i-z_j)$ along collision divisors, so the bar
complex really does carry propagator-like singular kernels and OPE
vertex residues.  The Fulton--MacPherson compactification replaces raw
short-distance collisions by boundary strata, which is the geometric
regularization built into the bar formalism.  What is \emph{not}
proved here is a chain-level perturbative field-theory complex
identification, nor the stronger claim that graph loop number equals
the genus grading of the bar package: later in this chapter
Theorem~\ref{thm:loop-genus-correspondence} recalls that ribbon-graph
...
component has scalar trace $c/48$, matching $F_1(\mathrm{Vir}_c) = \kappa/24 = c/48$.  Higher
