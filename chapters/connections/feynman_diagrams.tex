...
The Poincar\'e residue along $D_{12}$ extracts the constant term ($u^0$ coefficient) of the OPE---since $\operatorname{Res}_{u=0}[f(u)\,d\log u] = f(0)$---giving:
$$\operatorname{Res}_{D_{12}} = {:}TT{:}(z_2) \cdot T(z_3) \cdot \eta_{23}$$
(Note: the $c/2 \cdot u^{-4}$ and $2T \cdot u^{-2}$ terms contribute higher-order poles in $du/u$, not log poles, and do not survive the Poincar\'e residue.)
