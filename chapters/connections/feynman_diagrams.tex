...
The bar complex is a Feynman diagram complex: the propagator is the
logarithmic form $\eta_{ij}$, each collision residue is a vertex, and
Verdier duality is the passage from off-shell to on-shell.  This
chapter makes the identification precise.
\end{remark}

\begin{remark}[H/M/S route; Convention~\ref{conv:hms-levels}]
This chapter is mixed-level.  Proved statements carry theorem labels;
the physical identifications (bar~$=$~off-shell amplitudes,
cobar~$=$~on-shell propagators, pairing~$=$~S-matrix) are heuristic
physics-dictionary entries (Type~VII) unless explicitly promoted.
\end{remark}
