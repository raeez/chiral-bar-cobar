...
this resolves MC1 for the standard finite-type interacting families and
removes the last PBW-conditional modular-Koszul statements from
Part~II.}
\end{theorem}
...
\ClaimStatusConjectured{}
The bar-cobar framework extends to chiral algebras whose Koszul duals
have infinitely many generators
($\mathcal{W}_\infty$, Yangian towers, and similar).
...
and~\ref{thm:pbw-allgenera-virasoro}, it upgrades the standard
finite-type interacting families to unconditional modular Koszulity.
...
\item \textbf{MC1} (Theorem~\ref{conj:master-pbw}):
  ...
  \ref{thm:pbw-allgenera-virasoro},
  and~\ref{thm:pbw-allgenera-principal-w}).
  Remaining $\mathcal{W}$-direction scope begins with
  $\mathcal{W}_\infty$ completion and non-principal orbit problems,
  which now fall under
  Conjectures~\ref{conj:master-infinite-generator}
  and~\ref{conj:w-orbit-duality}.
...
\item \textbf{Independent}: ${\sim}8$ genuinely open problems
  (Virasoro/$\mathcal{W}_\infty$ Koszul dual identification,
  reflected modular periodicity, derived bc-$\beta\gamma$)
  not subsumed by any master conjecture.
\end{itemize}
\end{remark}
...
What is proved: bar-cobar adjunction for finite-type chiral algebras
(Theorem~A), truncation at each conformal weight level.
What remains: pro-nilpotent completion $\varprojlim_N \bar{B}(\cA/F_N)$
with topology, convergence of finite-stage approximations, comparison
with $\mathcal{W}_\infty = \varprojlim_N \mathcal{W}_N$.
...
coalgebra theory~\cite{Positselski}.
