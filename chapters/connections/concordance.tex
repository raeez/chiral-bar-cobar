...
full shadow tower, not a projection of it.
...
the pipeline extends: the shadow Postnikov tower is a functor
...
the shadow tower $\Thetaenv(R)$ from this input.  The super
...
are class~$\mathbf{L}$: finite shadow tower
...
infinite shadow tower, $\kappa + \kappa' \neq 0$,
...
For uniform-weight algebras, the shadow tower free energy
...
\item \emph{Honest scope.}
  ...
  (Vol~II, Theorem~thm:Koszul\_dual\_Yangian).
...
\item \emph{Non-perturbative completion.}
  ...
  algebraic shadow tower $\sum_g F_g \hbar^{2g}$ converges
...
\item \textbf{Shadow radius $\rho < 1$ analytic test.}
  The shadow tower converges absolutely when the shadow
  ...
  the comparison between the algebraic shadow tower and the
...
The shadow tower is trivial on this axis: since $\kappa = 0$,
...
\medskip\noindent
\textbf{Axis~2: Off-critical shadow tower ($\kappa \neq 0$).}\enspace
...
axis~1; the shadow tower, the genus expansion, and
...
deform continuously into the shadow tower curvature data
...
In particular, the shadow tower records the Taylor
...
explores arithmetic consequences of the shadow tower.
...
The shadow tower, the MC equation, and all five main
...
The shadow tower is perturbative: it is a formal power
...
Virasoro, and the convergence of the shadow tower.
...
at the chain level
(Vol~II, Theorem~thm:ds-hpl-transfer).
...
platonic packages, and the shadow cascade
...
\medskip
\noindent
\textbf{(c) Shadow tower convergence for Virasoro.}
...
$c^* \approx 6.125$.
If $\rho \geq 1$ for all~$c$, the shadow tower
...
The shadow Postnikov tower would be a formal
...
$\Theta_\cA^{\le 2}$),
but the full shadow tower and all
...
\item \emph{Spectral projection.}
  ...
  Target: the shadow tower, the algebraic function
...
the shadow tower coefficients with theta-function
...
\midrule
$0$ & Pure algebra & Bar complex, MC equation, shadow tower,
...
\item \textbf{DS--KD intertwining for arbitrary nilpotent}
  ...
  \end{quote}
  In words: DS reduction is a morphism of platonic packages.
...
\item \textbf{Platonic adjunction}
  ...
  completed platonic datum $\Pi^{\mathrm{oc}}_X(\cA)$
...
\item \emph{Functoriality problems}: is $\Theta^{\mathrm{oc}}_\cA$
  ...
  (DS--KD intertwining, platonic adjunction, BV/BRST bridge,
  DK bridge.)
\end{enumerate}
...
\item \textbf{``Gravity Yangian at strict level.''}
  ...
  non-formality content of the shadow tower.
...
\item \textbf{``The global open sector lives on an
  ...
  at each puncture
  (Volume~II, Definition~def:bordified-curve),
...
\item \textbf{``The dg-shifted Yangian is a universal
  ...
  recognition theorem
  (Vol~II, Theorem~thm:yangian-recognition) identifies
