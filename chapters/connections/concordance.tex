...
monograph.  It defines the five master conjectures
...
\item Full universal Maurer--Cartan class $\Theta_{\cA}$
  ...
  of Theorem~D.
\item PBW spectral sequence degeneration at higher genus
  (Conjecture~\ref{conj:pbw-higher-genus-degeneration}): the
  single obstruction to unconditional modular Koszulity for
...
\subsection{Five master conjectures}
\label{subsec:five-master-conjectures}
\index{master conjectures|textbf}

The ${\sim}99$ individual conjectures collapse to five master
conjectures.  Resolving any one would discharge a substantial
cluster of the individual claims.

\begin{conjecture}[Higher-genus PBW degeneration]\label{conj:master-pbw}
\ClaimStatusConjectured{}
For generic Virasoro
$\mathrm{Vir}_c$ and generic $\mathcal{W}_N$-algebras, the genus-$g$ PBW
spectral sequence degenerates at~$E_2$ for all $g \geq 1$.

\emph{The Kac--Moody case is fully resolved by
Theorem~\textup{\ref{thm:pbw-allgenera-km}}.
This conjecture remains the single missing hypothesis for unconditional
modular Koszulity of the Virasoro and $\mathcal{W}_N$ families
\textup{(}Proposition~\textup{\ref{prop:conditional-modular-koszul})}.}
\end{conjecture}
...
downstream of Conjectures~\textup{\ref{conj:master-pbw}}--\textup{%
...
Conjecture~\ref{conj:master-pbw} is the \emph{entry point}: its
resolution for Kac--Moody (Theorem~\ref{thm:pbw-allgenera-km})
has already upgraded that entire family to unconditional modular
Koszulity; the remaining Virasoro and $\mathcal{W}_N$ cases
would complete the upgrade.
...
The five master conjectures map to the nine futures
(\S\ref{subsec:nine-futures}) as follows:
Conjecture~\ref{conj:master-pbw} $\to$ Future~2;
...
chapters reduce to the five master conjectures as follows.
Each local conjecture carries a parenthetical annotation identifying
its master parent.
\begin{itemize}
\item \textbf{MC1} (Conjecture~\ref{conj:master-pbw}):
  \emph{Resolved for Kac--Moody}
  (Theorem~\ref{thm:pbw-allgenera-km}).
  Remaining scope: all ``conditional on PBW degeneration'' statements
  for Virasoro and $\mathcal{W}_N$ (${\sim}10$ conjectures across
  Chapters~\ref{chap:higher-genus},
  \ref{chap:w-algebras}, \ref{ch:genus-expansions}).
...
Detailed proof strategies for each master conjecture:

\emph{MC1} (PBW degeneration):
See Remark~\ref{rem:mk4-status}.  What is proved: genus-$0$
PBW degeneration at~$E_1$, genus-$g$ associated-graded isomorphism,
convergence in each bidegree, locality of leading differentials.
What remains: showing that the enriched $E_2$ page at genus~$g$
concentrates as it does at genus~$0$.
Proof route: d$_1$-acyclicity of the genus enrichment, computational
verification at genus~$1$ for $\widehat{\mathfrak{sl}}_2$, induction
via clutching.
...
the conjecture makes.  The five master conjectures above
