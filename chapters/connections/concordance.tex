...
\hline
Bar complex $\bar{B}^{\mathrm{ch}}(\cA)$
  & \v{C}ech complex $M\check{C}^\bullet(\mathcal{U};\cA)$ \\
...
$m_3$ from homotopy transfer
  & $F_3 = $ Jacobiator homotopy \\
...
same object---chiral homology---enhanced with compatible algebraic
structures.  Our bar $A_\infty$ operations (constructed analytically
by integration over FM compactifications) and the MS HCA operations
(constructed algebraically by the Moore functor) are intertwined by
a quasi-isomorphism of $L_\infty$ algebras
(Proposition~\ref{prop:cech-bar-intertwining}).
The MS framework has the advantage of being purely algebraic (no
configuration space integrals needed); our framework has the advantage
of working at all genera via the modular operad and the
shadow Postnikov tower
(Definition~\ref{def:shadow-postnikov-tower}); the
curvature $d^2 = \kappa \cdot \omega_g$ and finite-order
shadows have no \v{C}ech counterpart in \cite{MS24}.
\end{remark}

\begin{remark}[Genus-$0$ agreement]
\label{rem:ms-genus0-agreement}
At genus~$0$, the two frameworks produce the same answer:
the \v{C}ech HCA is strict for abelian chiral algebras (Heisenberg,
free fermion) and has nontrivial $F_3$ for non-abelian algebras
($\widehat{\mathfrak{sl}}_2$, $\beta\gamma$), matching the
bar-side $A_\infty$ structure.  The comparison is fully explicit
for the standard cover of $\mathbb{P}^1$
(Examples~\ref{ex:cech-hca-heisenberg}
and~\ref{ex:cech-hca-sl2}).
\end{remark}
...
The integration yields seven identification theorems, of which five are
proved and two are conjectural:
