...
For generic affine Kac--Moody $\widehat{\fg}_k$, generic Virasoro
$\mathrm{Vir}_c$, and generic $\mathcal{W}_N$-algebras, the genus-$g$ PBW
spectral sequence degenerates at~$E_2$ for all $g \geq 1$.

\emph{This is the single missing hypothesis for unconditional
modular Koszulity of the standard interacting families
...
resolution would immediately upgrade all interacting-family
examples from conditional to unconditional modular Koszulity.
...
the scalar system: $\kappa(\cA)$ determines the entire genus
...
\begin{enumerate}[label=\textup{(\alph*)}]
\item \emph{Modular periodicity} (Type~I).  Proved for minimal models
  (Theorem~\ref{thm:modular-periodicity-minimal}) and WZW models at
  all positive integer levels
  (Theorem~\ref{thm:modular-periodicity-wzw}); conjectured for
...
\midrule
1 & $H^1(\mathcal{M}_g)$-parameterized theory & \textbf{Reformulated \& proved} \\
2 & Modular Koszul programme & \textbf{Largely realized} \\
3 & Modular homotopy theory & Alive (Definition~\ref{def:modular-homotopy-theory-working}) \\
4 & Universal characteristic package & Full 5-tuple formalized (Def.~\ref{def:full-modular-package}); $\Theta_{\cA}$ open \\
5 & Index-theoretic genus expansions & \textbf{Realized} (Theorem~\ref{thm:family-index}) \\
6 & Derived Drinfeld--Kohno & Triple bridge formalized (Conj.~\ref{conj:full-dk-bridge}) \\
7 & Unified periodicity doctrine & Stratified: proved for minimal models/WZW; conjectural in general (Def.~\ref{def:derived-scalar-period}) \\
8 & Spectral discriminant geometry & \textbf{Proved} + branch object (Def.~\ref{def:spectral-branch-object}) \\
9 & BV/BRST/physics completion & Genus $0$ proved; $g \geq 1$ open \\
\bottomrule
\end{tabular}
\end{center}
\noindent
...
\item Conjecture~\ref{conj:cobar-physical}:
  ...
  (\S\ref{sec:first-complete-proof}).
\item Theorems~\ref{thm:modular-periodicity-minimal},
  \ref{thm:modular-periodicity-wzw} /
  Conjecture~\ref{conj:modular-periodicity}:
  Modular periodicity of bar cohomology (proved for minimal models
  and WZW; conjectured for general rational chiral algebras)
