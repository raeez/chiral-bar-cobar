...
is genus~$0$, arity~$2$, $\Delta = 0$: the class~G
...
Part~IV (Heisenberg, affine Kac--Moody,
...
\item \emph{Chiral Hochschild cohomology
  ...
  $\ChirHoch^n(\Walg^k(\fg)) = 0$ for $n > 2$, with Hilbert series
...
\item The \emph{shadow archetypes in Part~IV}: each standard
...
The modular homotopy type of a chiral algebra~$\cA$---the formal
moduli problem
$\cM^{\mathrm{mod}}_\cA(R)
= \mathrm{MC}(\mathrm{Def}^{\mathrm{mod}}_\infty(\cA) \otimes R)/\mathrm{gauge}$---determines, through its finite-order projections, every
...
\noindent
The $\mathcal{N}{=}2$ SCA is the definitive AP14/AP63 witness:
...
(AP24; Proposition~\ref{prop:n2-complementarity}), and
...
\item \emph{Subregular/minimal in $\mathfrak{sl}_3$}:
  ...
  \cW_k(\mathfrak{sl}_3, f_{\mathrm{min}})$ is chirally Koszul,
...
Parts~I--VI describe the modular/symmetric framework that is the
...
\label{conj:pixton-from-shadows}% NOTE: retains conj: prefix for backward compatibility; claim is ProvedHere
...
\begin{remark}[Proof of Theorem~\ref{conj:pixton-from-shadows}]
...
ideal.  This proves
Theorem~\ref{conj:pixton-from-shadows} for all principal
...
This proves Theorem~\ref{conj:pixton-from-shadows} for
...
Theorem~\ref{conj:pixton-from-shadows} is automatically satisfied
...
$\mathcal{W}$-algebra chapters of Part~IV at genus~$0$.
...
DK & DK-0/1/$1\frac{1}{2}$: chain-level, eval-locus, lattice; & DK-0/1/$1\frac{1}{2}$: proved (all types); & Thms~\ref{thm:derived-dk-affine}, \\
  ...
  & DK-4/5: dg-shifted/triple bridge & uses Molev PBW \cite{molev-yangians}); DK-4: ML proved, alg.\ id.\ open; DK-5: \textbf{proved for $\mathfrak{sl}_2$} (FRT, Prop.~\ref{prop:dk5-sl2-frt}); general: conj. & \ref{thm:dk-fd-typeA}, \ref{cor:dk23-all-types}, \ref{thm:rtt-mittag-leffler}; Conj.~\ref{conj:full-dk-bridge} \\[2pt]
...
\item \textup{Conjecture~\ref{conj:d-module-purity-koszulness} ($\cD$-module purity $\Rightarrow$ Koszulness).}
  Forward direction proved.  Converse reduces to PBW~$=$~Saito weight filtration; proved for KM, open for rank~$\geq 2$.
\end{enumerate}
\end{remark}
...
& Pixton ideal generation from class-M shadows
  \textup{(}Theorem~\ref{conj:pixton-from-shadows}\textup{)} \\
...
\item \textbf{Pixton ideal generation from class-M shadows}
  (Theorem~\ref{conj:pixton-from-shadows}).
