...
Theorem~\ref{thm:baxter-exact-triangles-opoly}: the TQ relations
of Zhang~\cite{Zhang24} lift to $Y(\mathfrak{sl}_2)$-equivariant
short exact sequences on~$\mathcal{O}_{\mathrm{poly}}$, functorial
via the spectral constraint $b = a - (\lambda+1)/2$
(Proposition~\ref{prop:baxter-yangian-equivariance}, $85$~tests)
and naturality (Corollary~\ref{cor:baxter-naturality-opoly}).
The $D^b$-level octahedral axiom for iterated TQ is verified
($724$~total tests).
\emph{Remaining frontier}: extension from
$\mathcal{O}_{\mathrm{poly}}$ to the shifted-prefundamental locus
