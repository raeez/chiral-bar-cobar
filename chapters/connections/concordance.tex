...
The promotions above clear the landscape of conjectures that
were artefacts of incomplete tools.  Three genuinely open
mathematical problems remain at the interface between the
algebraic engine and the physics/geometry it models.

\begin{enumerate}
\item \textbf{Chiral Koszulness from physics.}
  The standing physical axioms (H1)--(H4) should imply that the
  boundary chiral algebra of a holomorphic-topological field theory
  is chirally Koszul.  The mechanism: (H3) gives configuration-space
  integrals defining the $A_\infty$ operations~$m_k$; (H3d)--(H3e)
  force Arnold relations at codimension-$2$ corners, which is the
  geometric shadow of the Koszul quadraticity condition.
  The gap is that (H3) controls the weight forms
  $\omega_k \in \Omega^*_{\log}(\FM_k(\C))$ but does \emph{not}
  directly constrain the \emph{homological} condition
  $H^n(\barB(\cA)) = 0$ for $n \geq 3$.  In the three verified
  examples (free multiplet, Landau--Ginzburg cubic, abelian CS),
  Koszulness follows by direct computation; the general case
  requires a theorem linking logarithmic form regularity on
  $\FM_k(\C)$ to bar cohomology concentration.

\item \textbf{Axioms (H1)--(H4) in general.}
  The axioms are verified in three examples.  For a general
  holomorphic-topological theory, (H1c) (one-loop finiteness)
  follows from the Gwilliam--Rabinovich--Williams theorem when
  $d' = 1$; (H2) (propagator regularity) is standard for
  theories with polynomial interactions; (H4) (factorization
  compatibility) follows from general results of
  Costello--Gwilliam.  The genuinely open content is (H3):
  that Feynman diagram contributions define \emph{logarithmic}
  forms on $\FM_k(\C)$ satisfying the Arnold relations at all
  codimension-$2$ corners.  This is the geometric input that
  distinguishes the holomorphic-topological setting from general
  QFT.

\item \textbf{Filtration $\geq 4$ spectral Drinfeld classes.}
  The dg-shifted factorization bridge
  proves that the spectral Drinfeld class
  $\mathfrak{D}_{\mathrm{spec}}(\cA, \Omega) \in
  H^3_{\mathrm{spec}}(\operatorname{gr}^n \cA)$
  controls strictification of the quasi-factorization
  $R$-matrix datum.  Through filtration~$3$, the spectral
  cohomology vanishes (no obstruction).  At filtration~$4$,
  new Hall words and higher Magnus corrections appear,
  and $H^3_{\mathrm{spec}}(\operatorname{gr}^4 \cA)$ may be
  nonzero.  Computing this cohomology group for concrete
  algebras (affine, $\mathcal{W}$, Yangian) is the first
  genuinely new obstruction beyond the proved regime.
\end{enumerate}
