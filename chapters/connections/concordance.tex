...
$B_n$ & $1$ & no & \textbf{Proved} (folding) & A \\
$C_n$ & $1$ & no & \textbf{Proved} (Langlands) & A \\
...
\end{tabular}
\end{center}
Types $B_n$ and $C_n$ are proved by folding from type~$A$ (Proposition~\ref{prop:mc3-type-b-folding}) and Langlands duality.  Types with zero minuscule representations ($G_2$, $F_4$, $E_8$) lack the evaluation-core bootstrap (Proposition~\ref{prop:e8-root-uniformity}).
\end{construction}
...
For Virasoro at $c > 6$: positive for all
$\alpha \geq 2$.
