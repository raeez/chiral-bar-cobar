...
\textbf{D} & Modular characteristic
  & scalar identity on the proved scalar lane
    $\mathrm{obs}_g(\cA)=\kappa(\cA)\lambda_g$;
    genus~$1$ universal for arbitrary modular Koszul algebras;
...
\item \emph{Chain-level modular functor
  ...
  The scalar genus tower on the scalar lane
...
essentially scalar on the proved scalar lane: the single
number~$\kappa(\cA)$ and its $\lambda_g$ tower, plus the spectral
...
\item \emph{Scalar-orbit convolution formality}: on the proved scalar lane, the distinguished scalar Maurer--Cartan orbit in $\mathfrak{g}_\cA^{\mathrm{mod}}$ is formal; converse and one-channel generalizations remain open.
...
The shadow tower free energy on the scalar lane
...
$\mathrm{obs}_g = \kappa \cdot \lambda_g$ is, on the proved
scalar lane, a pure
...
\item On the proved scalar lane, the scalar shadow
...
On this scalar lane, the genus-$g$ free energy is the
...
On the same scalar lane, the total generating function is
...
In physical terms, on the scalar lane the genus expansion of a
chiral algebra is
...
algebra.  On the scalar lane the genus-$2$ ratio
