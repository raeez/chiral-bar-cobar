...
model.  The shadow Postnikov tower
...
(through arity~$4$), and the full unrestricted $\Theta_\cA$ is the
conjectural limit.  Theorems~A--D+H are proved projections of the
scalar level~$\kappa$.
...
\item The ambient $D^2 = 0$
  (Conjecture~\ref{conj:differential-square-zero}) reduces to
  tropical faithfulness: if the tropicalisation of
  $\operatorname{FM}_n^{\mathrm{log}}$ induces a quasi-isomorphism
  on chains (Hypothesis~\ref{hyp:tropical-faithfulness}), then
  $D_{\cA}^2 = 0$ follows by one-slot functorial transfer
  (Proposition~\ref{prop:tropical-reduction-d-squared}).
\end{enumerate}
...
is MC2 (conjectural), while finite-order projections
$\Theta_\cA^{\leq r}$ are proved.  Item~(7) is an external structural
...
\item \emph{MC3}:  The categorical lift requires two-sided functoriality
...
\item \emph{MC4}:  Unconditional closure on infinite towers
  ($\mathcal{W}_\infty$, Yangians) follows from the modular
  characteristic's stability under Pillar~B rigidity theorems,
  independent of MC3 extension.
