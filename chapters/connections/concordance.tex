...
\medskip\noindent
\textbf{Tier~3: structural equivalences (conditional and conjectural).}
...
\item\label{item:kp-dmod}
  ...
  \hfill (\emph{conjectured}; $(\ref{item:kp-fmbdy})\Rightarrow(\ref{item:kp-dmod})$ proved, converse open)
...
\noindent
\emph{Status}: ten equivalences proved unconditionally
\textup{(}conditions~(\ref{item:kp-def})--(\ref{item:kp-nullvec})
in the numbering above\textup{)};
the Lagrangian criterion~(\ref{item:kp-lagrangian}) is
conditional on perfectness of the ambient tangent complex;
$\mathcal{D}$-module purity~(\ref{item:kp-dmod}) is
partially proved (forward direction established, converse open).
...
are proved separately in their respective chapters but are not
among the twelve numbered conditions of the meta-theorem
...
extends to all unconditional conditions: $A_\infty$ formality
...
Shapovalov non-degeneracy forcing PBW injectivity, and the
...
(Theorem~\ref{conj:non-lattice-ramanujan}):
...
(Theorem~\ref{conj:irrational-ramanujan}):
