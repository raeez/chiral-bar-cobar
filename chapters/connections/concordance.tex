...
It embeds into the genus-$0$, arity-$2$, $\Delta = 0$ stratum
...
\item \texttt{thm:FG-shadow}: resolved at arities $2, 3, 4$
...
\begin{itemize}
\item \texttt{thm:FG-shadow} at all arities: the full
...
\begin{itemize}
\item At arity~$2$: $\kappa(\cA) = \operatorname{av}(r(z))$.
 The modular characteristic is the scalar projection of the
 classical $r$-matrix.
\item At arity~$3$: the cubic shadow $\mathfrak{C}(\cA) =
 \operatorname{av}(r_3)$, where $r_3(z_1, z_2)$ is the
 genus-$0$ KZ associator.
\item At arity~$r$: the shadow invariant $S_r(\cA) =
...
three equivalent readings of the arity-$r$ shadow
...
core, all simple types, unconditional), MC3b (categorical
thick generation, all types, unconditional), MC3c (full
category~$\cO$, type~A unconditional, other types conditional
on Conjecture~\ref{conj:rank-independence-step2}).
The new Conjecture~\ref{conj:rank-independence-step2}
(Lemma~L, the lift-and-lower hypothesis) is the sole
...
\item \emph{Full tower self-duality}
 ...
 Self-duality holds at all arities:
 ...
 functions. The higher-arity corrections are odd in the
...
\ref{thm:nms-all-arity-resonance-boundary}
...
since $\kappa = 0$, the leading (arity-$2$) shadow vanishes
and Theorem~D gives $\mathrm{obs}_g = 0$ for all $g \geq 1$;
the discriminant $\Delta = 8\kappa S_4 = 0$.
However, higher-arity components of $\Theta_\cA$ are
...
the bar-intrinsic MC element retains higher-arity structure.
...
$\Theta_\cA$ may retain nonzero higher-arity components. No theorem on axis~1 implies a theorem on
...
\index{crossing symmetry!arity-$4$ MC projection}
\index{unitarity bound!shadow metric positivity}
The genus-$0$ arity-$4$ projection of the Maurer--Cartan
...
& Virasoro shadow growth rate from Riccati
 \textup{(}Proposition~\ref{prop:virasoro-shadow-ratio-riccati}\textup{)} \\[4pt]
...
\item \textbf{BV/BRST $=$ bar at higher genus}
 ...
 all genera. The key mechanism for class~M: the arity-$r$
...
\item \emph{BV/BRST $=$ bar at higher genus}
 ...
 and all shadow classes: the arity-$r$ chain-level obstruction
