...
\item \emph{Genus universality
  ...
  $H^{2g}(\overline{\mathcal{M}}_g)$.  For multi-generator
  algebras ($\mathcal{W}_N$, $N \geq 3$) at $g \geq 2$,
  both the cohomological factorization and the free-energy
  factorization depend on whether the individual Hodge-type
  integrals $\int \lambda_g^{(j)}$ coincide
  (Open Problem~\ref{op:multi-generator-universality});
  at $g = 1$, universality holds unconditionally.
...
peer-reviewed journal as of March~2027), the specific result
...
\item \emph{No proved-theorem risk}: \cite{GZ26},
  ...
  --- cited as motivation, consistency checks, or inputs to
...
\emph{Caveat.}
At genus~$3$, the vertex weights at genus-$1$ vertices
(type $(1,2)$ and $(1,3)$) are approximated by~$\kappa$
rather than the exact MC-recursion-determined
$\ell_k^{(1)}$; the genus-$0$ vertex weights~$S_k$
are exact.  The genus-$2$ formula involves only genus-$0$
vertex weights and is therefore exact.
\end{remark}
...
entanglement.  The key structural insight: the shadow tower
...
\item Theorem~\ref{thm:hc-koszulness-exact-qec} (G12):
  Koszulness $\Leftrightarrow$ exact quantum error correction.
\item Proposition~\ref{prop:hc-knill-laflamme}:
  Knill--Laflamme condition from Lagrangian isotropy.
