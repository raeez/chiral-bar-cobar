...
\textbf{B} & Bar-cobar inversion
 ...
 \textup{(}Thm~\ref{thm:tbsp-admissible-strict-scope}\textup{)},
 ...
 Rem.~\ref{rem:tbsp-5-tor-class} the load-bearing case;
 ...
 \textup{(}Rem.~\ref{rem:tbsp-6-ml-per-weight}\textup{)};
 an $A_\infty$-correction on a formal neighbourhood of $H_3$
 installs the homotopy $h^{(3)}$ witnessing the Costello
 coefficient $c_3 = -8$
...
\section{Cross-volume type-error registry}
\label{sec:concordance-supplement}

An adversarial audit of the Volume~III K3 non-abelian chiral bialgebra
programme produced five cross-convergent type-error warnings that are
load-bearing for multiple chapters in all three volumes. They are
reproduced inline below. The full registry of fifty-nine detailed
warnings lives in the Volume~III source tree under
\verb|notes/| with per-voice companion notes.

\subsection*{Top-five type errors}
...
Full per-entry content -- wrong claim $\to$ ghost theorem $\to$
precise error $\to$ correct relationship $\to$ type -- for all
fifty-nine warnings is recorded in Volume~I
\verb|appendices/first_principles_cache.md|.  The K3 chiral
bialgebra construction itself lives in Volume~III at
\verb|chapters/examples/k3_chiral_bialgebra_platonic.tex|, with
surgical interweavings in the K3 Yangian chapter, K3$\times E$ BKM
chapter, \verb|cy_to_chiral.tex|, and the two connection bridges
(bar-cobar, modular-Koszul).
...
Remark~\ref{rem:concord-retraction} below; intermediate
$(26, -312)$ retracted); Vol~III supplies the
...
\begin{remark}[Central-charge double-retraction;
...
\textbf{Canonical values} (this constitutional record):
\[
 \boxed{\;
...
Vol.~I Remark~\ref{rem:thqg-occ-retraction}
...
\begin{remark}[Constitutional: ratio of Lusztig levels equals ratio of Mukai-positive-signature]
\label{rem:concord-ratio-of-lusztig-levels}
\index{constitution!ratio-of-Lusztig-levels law}
...
Thm~\ref{thm:bkm-enriques-psi-landscape}; the Conway $V^{s\natural}$
row (Leech, $c_+(\Lambda_{24}) = 24$, positive-definite, no Fricke
involution) lies outside the scope by Vol~III
Thm~\ref{thm:bkm-conway-universal-identity-fails}. The ratio law
...
Primary: Vol~III
Thm~\ref{thm:bkm-ratio-of-levels} (inscribed);
Vol~III Thm~\ref{thm:bkm-universal-identity} (universal three-faces).
\end{remark}
