...
\item \emph{MC3}:  MC3 is only partially resolved.  For all simple
types, package~(i) categorical CG decomposition is proved via
...
evaluation-generated core is proved separately
...
\ref{thm:pro-weyl-recovery}\textup{)} and reduces the old four-package
MC3 problem to the single remaining compact/completed extension packet
...
\textbf{MC3 is not globally closed.}
...
\textup{(}proved\textup{)},
MC3 \textup{(}partially resolved: all-types package~(i) and
evaluation-generated-core DK-2/3; remaining completion packets
open\textup{)},
...
\item \textbf{Filtration $\geq 4$ spectral Drinfeld classes.}
  ...
  the Drinfeld-class obstruction is unobstructed through
...
& MC3 partially resolved: categorical CG
  for all simple types; post-CG completion
  packets open beyond type~$A$ \\
...
\item \textbf{``All master conjectures resolved.''}
  \emph{False.}
  MC1, MC2, MC4, MC5 are proved; MC3 is partially
  resolved (CG all types; post-CG packets open
  beyond type~$A$).
  The broader programme contains open conjectures:
  multi-generator universality at $g \geq 2$
  (Open Problem~\ref{op:multi-generator-universality}),
  modular cooperad completion,
  ...
  Four MC statements are proved; MC3 is partially
  resolved.  None is the end of the programme.
  \emph{Violated anti-pattern}: AP7
  (scope inflation: universal claim, partial proof).
