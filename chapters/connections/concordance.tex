...
disagree, this chapter governs.  It states the five main theorems
with their precise status, locates every result against the prior
literature, and serves as the authoritative reference for what
...
$\mathfrak{g}^{\mathrm{mod}}_\cA$ carries a modular $L_\infty$
...
Theorems~A--C are consequences of the MC
...
Theorems~D and~H are \emph{projectional outputs}: $\kappa$ is the scalar trace
...
Theorems~A--C have no classical analogue:
algebraicity, the formality identification,
and Lagrangian complementarity are genuinely new.
