...
\item\label{item:kp-ainfty}
  ...
  (Theorem~\ref{conj:ainfty-koszul-characterization}).
...
\index{structural obstruction!resolution}

The shadow--spectral correspondence
(Chapter~\ref{chap:arithmetic-shadows},
Theorem~\ref{thm:shadow-spectral-correspondence}) identifies the
shadow depth with the Hecke eigenform count of the partition function.
The \emph{structural obstruction}
(Remark~\ref{rem:structural-obstruction}) shows that the
shadow tower constrains spectral coefficients~$c(t)$ on the real
$t$-axis, while the zeros of~$\zeta$ and $L(s,f_j)$ live at complex
spectral parameters.

Four approaches to resolving this obstruction are developed
in~\S\S\ref{sec:spectral-continuation}--\ref{sec:reassessment}:

\begin{enumerate}[label=\textup{(\Alph*)}]
\item \textbf{Mellin continuation functor}
  (\S\ref{sec:spectral-continuation}).
  For lattice vertex algebras, the shadow tower data determines the
  Hecke decomposition of~$\Theta_\Lambda$; the Mellin transform of each
  eigenform provides analytic continuation to the complex plane.
  \textbf{Status: PROVED}
  (Theorem~\ref{thm:spectral-continuation-bridge}).
  The structural obstruction is \emph{resolved} for this class.

\item \textbf{Vector-valued Rankin--Selberg theory}
  (\S\ref{sec:vvmf-rankin-selberg}).
  For rational VOAs (minimal models), the character vector~$F(\tau)$
  is a vector-valued modular form; the Rankin--Selberg
  integral $\int \langle F, \bar{F}\rangle E_s\,d\mu$ unfolds to a
  Dirichlet series.  The Hecke decomposition of this integral would
  extract individual $L$-functions.
  \textbf{Status: Conjectural}
  (Proposition~\ref{prop:vvmf-hecke-decomposition}).

\item \textbf{Scattering resonance extraction.}
  The resolvent $R(s) = (\Delta - s(1{-}s))^{-1}$ on
  $\mathrm{SL}(2,\bZ)\backslash\mathfrak{H}$ is meromorphic with poles at
  scattering resonances.  If the shadow tower constrains the
  resolvent trace on the spectral line, analytic continuation
  probes the resonance positions.
  \textbf{Status: Framework only.}

\item \textbf{Complex analysis of the shadow generating function}
  (\S\ref{sec:shadow-complex-structure}).
  The Virasoro shadow generating function
  $G(t) = -\log(1{+}6t/c)$ analytically continues to
  $\bC\setminus\{-c/6\}$.  The shadow coefficients~$S_r$ are moments
  of a spectral measure~$\rho$; Carleman's condition guarantees
  uniqueness of~$\rho$ from the moment data.
  \textbf{Status: PROVED}
  (Propositions~\ref{prop:shadow-gf-borel},
  \ref{prop:shadow-spectral-measure},
  \ref{prop:carleman-virasoro}).
  The shadow data \emph{does} determine the analytic continuation of
  $G(t)$; but the bridge from~$G(t)$ to zero locations requires
  the scattering matrix, where the obstruction persists.
\end{enumerate}

\noindent\textbf{Summary.}
The structural obstruction is fully resolved for lattice VOAs~(A),
partially resolved for rational VOAs~(B), and persists for
Virasoro at generic~$c$: the shadow generating function continues
analytically~(D), but the bridge to zero locations re-enters
through the scattering matrix of the moduli-space Laplacian~(C).
The deepest open question: does the shadow tower's moment data,
composed with the Rankin--Selberg integral, sufficiently constrain
the scattering matrix to determine zero locations?  This connects
the shadow Postnikov tower to the Langlands programme.
