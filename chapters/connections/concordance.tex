...
\begin{remark}[Top-down architecture]
\label{rem:top-down-architecture}
\index{top-down architecture|textbf}
\index{five main theorems!as MC projections}
The five main theorems (A--D+H) are not independent achievements
requiring separate proofs; they are five projections of the single
Maurer--Cartan equation
$D \cdot \Theta_\cA + \tfrac{1}{2}[\Theta_\cA, \Theta_\cA] = 0$
in the modular convolution algebra~$\gAmod$.
The scalar projection~$\pi_{\mathrm{sc}}$ yields
Theorems~C and~D; the twisting-morphism structure yields
Theorems~A and~B; the Hochschild projection yields Theorem~H.
