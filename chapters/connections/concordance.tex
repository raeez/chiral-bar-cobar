...
monograph.  It defines the resolved PBW entry theorem and the four
...
\item \emph{Four remaining master conjectures.}
  The foundational universal class, the full DK/KL categorical lift,
  completed infinite-generator bar theory, and the BV/BRST/bar
...
\subsection{Resolved Entry Theorem and Four Remaining Master Conjectures}
...
resolved entry theorem and four remaining master conjectures.  Taken
together, these statements still organize the dependency structure of
the manuscript.
...
it would complete the modular characteristic package.
...
\item \textbf{MC2} (Conjecture~\ref{conj:master-theta}):
  ...
  the full modular characteristic package, periodicity synchronization,
  and index-theoretic genus-expansion predictions (${\sim}20$
...
What remains: cyclic $L_\infty$ structure on bar coderivations,
completed tensor product with $R\Gamma(\overline{\mathcal{M}}_{g,\bullet})$,
MC solution with clutching compatibility.
...
truncation at each conformal weight level.
...
\widehat{\bar B}(\cA)=\varprojlim_N \bar B(\cA_{\le N})
\]
as a separated complete pro-nilpotent curved coalgebra, continuity of
the completed differential, convergence of finite-stage
approximations, and comparison with inverse-limit targets such as
$\mathcal{W}_\infty = \varprojlim_N \mathcal{W}_N$ and Yangian towers.

Proof route: (1)~equip the bar construction with the inverse-limit
topology from the conformal weight filtration; (2)~show the
completed bar differential is continuous and $d^2 = 0$
(or curved) in the completed setting; (3)~prove that the finite-type
...
($\Theta_{\cA}$ conjectural; Conjecture~\ref{conj:universal-MC});
...
\noindent\textbf{Future~4: Universal modular characteristic package.}

\emph{Original vision.}
Not only $\kappa(\cA)$ but a richer characteristic object
$(\Theta_{\cA}, H_{\cA}, \Delta_{\cA})$ whose shadows are genus
free energies, obstruction classes, and spectral invariants.
...
(page~\pageref{thm:complete-periodicity-classification}), proved for
minimal models and WZW but conjectural for general rational CFT,
assembles these into an lcm upper bound.  The \emph{structural
content}---that three independent periodicity sources combine
via lcm---is unconditional.  The specific formulas for
$N_{\mathrm{mod}}$ and $N_{\mathrm{geom}}$ are conjectural.
...
\emph{stratified periodicity theorem}: unconditional for minimal
models (where all three sources are controlled via theta
functions), conditional for general rational algebras (where
the modular and geometric inputs remain conjectural), and
\emph{spectral} in all cases (the period divides the lcm,
with the individual factors determined by the modular
...
7 & Periodicity doctrine & Proved for min.\ models/WZW; general open (Def.~\ref{def:derived-scalar-period}) \\
...
\item Conjectures~\ref{conj:kl-periodic-cdg}--\ref{conj:kl-braided}:
  Kazhdan--Lusztig equivalence from bar-cobar (three stages:
  periodic CDG, coderived identification, braided monoidality;
