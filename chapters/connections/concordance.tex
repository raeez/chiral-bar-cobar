...
An earlier reading placed $\Delta_5$ as an input on the right-hand side of
the K3 chiral bialgebra synthesis. The correct reading takes $\Delta_5$ as the
...
ingredients: the pentagon coboundary at $\hbar^3$, the $\widetilde M_{24}$ Schur
...
The first-principles pants-decomposition gives
$(c_{4d}, c_{2d}) = (107/6, -214)$. An intermediate retraction in
favour of the Chacaltana--Distler ansatz
$c_{4d}(A_1, \Sigma_{g,n}) = (12(g-1) + 7n)/6 = 26$ at
$(g,n) = (0,24)$, yielding $c_{2d} = -312$, was itself falsified
by the $\mathrm{SU}(2)$ $N_f = 4$ cross-check (the ansatz returns
$c_{4d} = 8/3$ at $(g,n) = (0,4)$, whereas Shapere--Tachikawa
$2008$ Table~4 and Tachikawa $2013$ eq.~(13.6) give $c_{4d} = 7/6$);
the first-principles values are restored via the correct formula
\[
...
manuscript passages in Vols.~I, II, III displaying those
numerical values are scope-qualified to the retracted
evaluation; their surrounding mathematical content (shadow
...
dictum: intermediate numerical claims are false until
independently audited against primary literature. The
intermediate ansatz passed no primary-source audit; the
first-principles formula passed three cross-checks ($n = 3, 4, 24$).
Future revisions of central-charge values for class-$\mathcal S$ theories must
...
Primary: Vol~III
Thm~\ref{thm:bkm-ratio-of-levels} (established);
