...
\item \emph{Genus universality
  ...
  remain conditional for multi-weight families
...
\item The \emph{multi-variable shadow obstruction tower}
  ...
  correction $\delta F_2 = (c{+}120)/(16c)$
...
$\mathrm{obs}_1 = \kappa\lambda_1$; higher-genus multi-weight
purity remains conditional on the strong scalar ansatz.
...
\item \emph{Modularity $=$ trace $+$ clutching.}
  ...
  Two distinct questions remain open:
  (a)~the specific identification $F_g = \kappa \cdot \lambda_g$
  for multi-weight algebras at $g \geq 2$
  (\textbf{op:multi-generator-universality}), which concerns
  which tautological class appears, not open-closed compatibility;
...
\item \emph{Honest scope.}
  ...
  The BV/BRST~$=$~bar identification at higher genus is conjectural
...
algebra at $g \geq 2$ passes Tests~1, 3, 4 while Test~2 remains
open on the multi-weight lane
(Open Problem~\ref{op:multi-generator-universality}).
...
\item \emph{Multi-weight algebras at genus $g \geq 2$.}
  ...
  is open.  Test~2 holds at genus~$1$; the higher-genus
  extension is
  Open Problem~\ref{op:multi-generator-universality}.
\item \emph{BV/BRST $=$ bar at higher genus.}
  The identification of the BV/BRST differential with the bar
  differential at genus $g \geq 1$ is conjectural
...
& Multi-generator universality at $g \geq 2$:
  $\mathrm{obs}_g = \kappa \cdot \lambda_g$ for
...
\item \textbf{Multi-generator universality at $g \geq 2$}
  ...
  recovers the genus-$g$ component of $\Theta_\cA$.
  Algebraic-family rigidity
  (Theorem~\ref{thm:algebraic-family-rigidity}) proves that the
  cyclic deformation complex $\Defcyc(\cA)$ has a
  one-dimensional $H^2$ in the $\eta$-direction, so that
  $\Theta_\cA^{\min} = \eta \otimes \Gamma_\cA$ for a
  tautological coefficient
  $\Gamma_\cA \in H^*(\overline{\mathcal{M}}_g, \mathbb{Q})$.
  The question is:
  \begin{quote}
  \emph{Does the tautological coefficient
  $\Gamma_\cA$ equal $\kappa(\cA) \cdot \Lambda$
  $($where $\Lambda = \sum_g \lambda_g)$ for
  multi-weight algebras at $g \geq 2$?}
  \end{quote}
  Equivalently: the genus-$g$ scalar trace
  $\operatorname{tr}\bigl(\pi_{\mathrm{cl}}(\Theta^{\mathrm{oc}}_\cA)\bigr)\big|_g$
  is $\kappa(\cA) \cdot \lambda_g$ at $g = 1$
  for all families (proved).  At $g \geq 2$, the same
  identity holds for uniform-weight algebras (proved).
  The gap is the tautological purity statement
  $\Gamma_\cA = \kappa(\cA) \cdot \Lambda$ when
  generators have distinct weights.  The Kuranishi map
  of the deformation complex vanishes by parity, so the
  MC equation does not determine which class in
  $H^*(\overline{\mathcal{M}}_g)$ appears.

  \smallskip\noindent
  \emph{Strategy from $\Theta^{\mathrm{oc}}$.}
  The clutching sector of the MC equation
  (Theorem~\ref{thm:thqg-oc-mc-equation}(d))
  relates the genus-$g$ component to lower-genus data via
  separating and non-separating clutching.  A direct genus-$2$
  computation of the multi-channel graph sum with
  $R$-dressed propagators, using the proved clutching law
  (Theorem~\ref{thm:nms-clutching-law-modular-resonance}),
  would settle the $g = 2$ case and provide evidence for
  all genera.
...
\item \textbf{Shadow--Siegel gap for $K3 \times E$}
  ...
  developed in \S\ref{subsec:cy-research-programmes}
  of Chapter~\ref{chap:toroidal-elliptic}, each with
  formal conjectures:
...
\item \emph{Multi-generator universality}
  ...
  For multi-weight algebras at $g \geq 2$, algebraic-family
  rigidity gives $\Theta_\cA^{\min} = \eta \otimes \Gamma_\cA$
  with $\Gamma_\cA \in H^*(\overline{\mathcal{M}}_g)$
  undetermined.  The question:
  does the clutching sector of the MC equation
  force $\Gamma_\cA = \kappa(\cA) \cdot \Lambda$?
