...
\textbf{A} & Bar-cobar adjunction
 & Chain-level bar-cobar adjunction with universal twisting morphism
 and Verdier-intertwined counit witness, together with the
 factorization-$\infty$ adjunction on $\Ran(X)$; the algebra-level
 Verdier half appears only after $\mathbb{D}_{\Ran}$:
 $\mathbb{D}_{\Ran}\barB_X(\cA_i)\simeq \cA^!_{\infty,i}$
...
\textbf{B} & Bar-cobar inversion
 ...
 for the K3 chiral bialgebra $\mathbf{H}_{\Delta_5}$ over the
 Siegel base $\overline{\mathcal{A}_2}$ the strict Koszul
 locus is the admissible-Humbert complement
 $U^{\mathrm{adm}}=\overline{\mathcal{A}_2}
 \setminus\bigcup_{n\ \text{admissible}} H_n$
 \textup{(}Thm~\ref{thm:tbsp-wave18-admissible-strict-scope}\textup{)},
 with the three-wall $\{H_1, H_3, H_4\}$ specialisation and its
 $\mathrm{Tor}_1$ obstruction classes
 $c^{\mathrm{Br}}_1/\phi^{(3)}/b^{\mathrm{KS}}_{H_4}$ of
 Rem.~\ref{rem:tbsp-wave18-DNA-5-tor-class} the load-bearing case;
 off the strict lane, the explicit weight-completed model
 $\widehat{C}=\widehat{\barB}^{\mathrm{ch}}(\cA)$ in holonomic
 factorization $\mathcal{D}$-modules carries the unconditional
 coderived/contraderived Positselski equivalence
 \textup{(}Thm~\ref{thm:chiral-positselski-weight-completed}\textup{)},
 per-weight Mittag--Leffler stable
 \textup{(}Rem.~\ref{rem:tbsp-wave18-DNA-6-ml-per-weight}\textup{)};
 an $A_\infty$-correction on a formal neighbourhood of $H_3$
 installs the homotopy $h^{(3)}$ witnessing the Wave-17 Costello
 coefficient $c_3 = -8$
 \textup{(}Thm~\ref{thm:climax-W18-global-inversion}(c)\textup{)},
 while raw direct-sum class-$M$ chain inversion fails
 \textup{(}Prop.~\ref{prop:chiral-positselski-raw-direct-sum-class-M-false}\textup{)}
 & \ClaimStatusProvedHere \\
...
\textbf{H} & Chiral Hochschild
 & This is the chiral Hochschild theory only
 \textup{(}not topological or categorical Hochschild\textup{)}.
 On the Koszul locus at generic level with
 $E_{\infty}$-chiral PBW completion:
 $\ChirHoch^*(\cA)$ has cohomological amplitude $[0,2]$,
 satisfies the duality shift~$[2]$, has degree-$\leq 2$
 Hilbert polynomial, and obeys
 $\ChirHoch^1(V_k(\fg))\cong\fg$ for affine KM;
 the collision-depth spectral sequence collapses by local
 ...
 Proved unconditionally for classes G, L, C, M at generic
 level via
 Cor.~\textup{\ref{cor:hochschild-averaging-symmetric}}
 \textup{(}ordered bound of
 Thm.~\textup{\ref{thm:hochschild-concentration-E1}} plus
 $E_{\infty}$-averaging\textup{)}. Conjectural for
 $E_{1}$-chiral inputs without PBW
 \textup{(}Conj.~\textup{\ref{conj:hochschild-concentration-E1-only}}\textup{)}
 and for class M at minimal-model / admissible levels
 \textup{(}Conj.~\textup{\ref{conj:hochschild-concentration-class-M-non-generic}}\textup{)};
 unbounded at critical level $k=-h^{\vee}$
 \textup{(}Rem.~\textup{\ref{rem:critical-level-lie-vs-chirhoch}}\textup{)}.
 The Swiss-cheese structure belongs to
 $(C^\bullet_{\mathrm{ch}}(\cA,\cA),\cA)$; $\barB(\cA)$ remains an
 $E_1$-coalgebra
 & \ClaimStatusProvedHere
\end{tabular}
\end{center}
...
\medskip\noindent
All five descend from a single object. The modular $L_\infty$
convolution algebra
$\mathfrak{g}^{\mathrm{mod}}_\cA$ carries its $L_\infty$
structure from the Feynman transform of the modular operad;
the dg~Lie algebra of Definition~\ref{def:modular-convolution-dg-lie}
is its strict model $\Convstr$, and the homotopy-invariant object
is $\Definfmod(\cA)$
(Theorem~\ref{thm:modular-homotopy-convolution}). The universal
MC element $\Theta_\cA \in \MC(\gAmod)$ exists by the bar-intrinsic
construction (Theorem~\ref{thm:mc2-bar-intrinsic}:
$\Theta_\cA := D_\cA - d_0$ is MC because $D_\cA^2 = 0$).
This is the coinvariant carrier for the main theorem.
The ordered lift
$\Theta_\cA^{E_1} \in \MC((\gAmod)^{E_1})$
is constructed separately in the $\Eone$-modular chapter
\textup{(}Theorem~\ref{thm:e1-mc-element}\textup{)},
and averaging recovers the concordance-level class
$\Theta_\cA$.
...
Three distinct Hochschild theories appear in this programme.
They differ in input, output, and geometric origin;
...
\item \emph{Topological Hochschild homology}
 ...
 = \RHom_{A^e}(A,A)$, but that cochain complex is not the
 definition of~$\mathrm{THH}$.
...
\item \emph{Chiral Hochschild cohomology}
 ...
 Output: a graded vertex algebra, concentrated in
 cohomological degrees $\{0, 1, 2\}$ for
 modular Koszul algebras (Theorem~H).
...
\emph{topological}, \emph{chiral}, or
\emph{categorical} is required.
...
\item \emph{Configuration space bar-cobar adjunction
 ...
 differential by inclusions of strata. The adjunction holds
 for all Koszul chiral algebras at all genera in the
 ...
 by PBW propagation, and otherwise conditional on
...
\item \emph{Genus universality
 ...
 For a Koszul chiral algebra~$\cA$ with uniform-weight
 generators, the genus-$g$
 obstruction factors as
 $\mathrm{obs}_g(\cA) = \kappa(\cA) \cdot \lambda_g$ in
 $H^{2g}(\overline{\mathcal{M}}_g)$.
 For multi-weight families, only the genus-$1$
 identity is unconditional. Rigidity
 proves level-direction concentration
...
\item \emph{Deformation-obstruction complementarity
 (Theorem~\ref{thm:quantum-complementarity-main}).}
 The package splits into three layers. C0 identifies the fiber object
 with the derived center in the coderived category unconditionally,
 and in the ordinary derived category on the flat perfect locus
...
\item \emph{Explicit computations.}
 ...
 $\kappa = (k{+}4)/4$, complementarity $\kappa + \kappa' = 1$,
 Koszul despite nonvanishing $H^2_{\mathrm{CE}}$\textup{)},
...
\item \emph{Chiral Hochschild cohomology
 ...
 On the Koszul locus at generic level, the collision-depth spectral
 sequence for the Fulton--MacPherson stratification collapses at
 $E_2=E_\infty$ because each local
 $\FM_m(\mathbb{C})$ is formal. Hence $\ChirHoch^*(\cA)$ has
 cohomological amplitude $[0,2]$, carries the
 derived duality shift~$[2]$, and has Hilbert polynomial of degree at
 most~$2$.
 For the principal $\Walg$ and Virasoro lanes this polynomial is
 $1+t^2$ at generic level.
...
\end{enumerate}
The slogan: \emph{envelope $+$ bar coalgebra $+$
canonical MC class $=$ modular form datum}.
Every invariant (scalar anomaly, spectral
sheet, nonlinear resonance) is a shadow of~$\Theta_L$.
\end{construction}
...
envelope~\cite{Nish26,Vic25}. The stronger adjunction
...
architecture} is a constitutional amendment.
...
\item \emph{MC5} (\textbf{analytic, coderived, and canonical-ambient chain-level proved; tree-level pairing conditional}):
 Inductive genus
 ...
 The genuswise chain-level BV/BRST/bar identification is proved on the
 equivalent pro-object / $J$-adic topological / filtered
 weight-completed presentations of the raw bar complex
 ...
 direct-sum ambient $\mathrm{Ch}(\mathrm{Vect})$ fails already in
 class~M through $S_4(\mathrm{Vir}_c)=10/[c(5c+22)]\neq 0$, so the
 slogan ``class~M chain-level false'' is true only for that naive
 ambient.
...
\item The \emph{shadow archetypes in Part~\ref{part:physics-bridges}}: each standard
 ...
 are the first explicit degree-$5$ and degree-$6$ computations
 ...
 The DS depth-increase mechanism at degree~$5$ is verified:
...
the shadow archetypes follow from
this framework. Each ring
(Ring~1: proved core; Ring~2: finite-order shadow
engine; Ring~3: physics-facing frontier) requires
...
\label{prop:top-down-architecture}% backward compatibility
...
\item\label{item:kp-hochschild}
 $\ChirHoch^*(\cA)$ has cohomological amplitude
 $[0,2]$, satisfies Koszul-dual duality, has polynomial Hilbert
...
$k + h^\vee = p/q$, the Koszulness status is \emph{rank-dependent}.
At rank~$1$ ($\fg = \mathfrak{sl}_2$), $L_k(\mathfrak{sl}_2)$ is
chirally Koszul at every admissible level
...
amplitude; the genus tower $F_g = \kappa \cdot \lambda_g$
\textup{(}UNIFORM-WEIGHT\textup{}) is the
...
genus-$g$ vacuum amplitude
 & free energy $F_g(\cA) = \kappa \cdot \lambda_g^{\mathrm{FP}}$
 & scalar trace of $\Theta^{(g)}$ \\[2pt]
loop expansion
 & genus tower $\{F_g\}_{g \geq 1}$
 & $\hat{A}$-genus generating function \\[2pt]
$S$-matrix
 & collision $R$-matrix $r(z)$
 & $\Res^{\mathrm{coll}}_{0,2}(\Theta_\cA)$ \\[2pt]
$S$-matrix symmetry
 & Yangian $Y_{\hbar}(\fg)$
 & RTT presentation from $r(z)$ \\[2pt]
$S$-duality
...
\smallskip\noindent\textbf{Superconformal hierarchy}
(Remark~\ref{rem:n2-susy-hierarchy}).
The four fundamental superconformal families form a hierarchy
under extended supersymmetry, with complementarity sum
$\kappa + \kappa'$ strictly decreasing:
\begin{center}
\small
\renewcommand{\arraystretch}{1.15}
...
\noindent
The $\mathcal{N}{=}2$ SCA is the definitive witness:
$H^2_{\mathrm{CE}}(\mathfrak{g}_-) \neq 0$ at conformal weight~$3$,
yet $H^2(\barB(\mathrm{SCA}_c)) = 0$ (chirally Koszul by PBW
universality). Sub-leading OPE modes invisible to the CE complex
provide coboundaries that kill the CE cocycles
...
(iv)~admissible Koszulness for simple quotients is
\emph{rank-dependent}: $L_k(\mathfrak{sl}_2)$ is Koszul at all
admissible levels, but $L_k(\mathfrak{sl}_3)$ at $q \geq 3$ is not,
with the obstruction coming from the abelian Cartan subalgebra
(Remark~\ref{rem:admissible-koszul-status},
...
MC5 & Genus tower & Five components:
 ...
 (4)~genuswise chain-level BV/BRST/bar identification \textbf{proved}
 on the pro-object / $J$-adic / filtered weight-completed
 presentations of the raw bar complex
 \textup{(}Theorems~\ref{thm:mc5-class-m-chain-level-pro-ambient},
 \ref{thm:mc5-class-m-topological-chain-level-j-adic};
...
\item \emph{Subregular/minimal in $\mathfrak{sl}_3$}:
 ...
 \cW_k(\mathfrak{sl}_3, f_{\mathrm{min}})$ is chirally Koszul at generic level ($k \neq -3$),
 with dual $\mathrm{BP}_k^! = \mathrm{BP}_{-k-6}$
 (the level-shifted Feigin--Frenkel involution for
 $\mathfrak{sl}_3$ applied to the minimal nilpotent).
...
\item \texttt{thm:FG-shadow}: resolved at degrees $2, 3, 4$
...
\begin{itemize}
\item \texttt{thm:FG-shadow} at all degrees: the full
...
\begin{itemize}
\item At degree~$2$: $\kappa(\cA) = \operatorname{av}(r(z))$
 for abelian algebras; for non-abelian Kac--Moody,
 $\kappa = \operatorname{av}(r(z)) + \dim(\fg)/2$.
 The modular characteristic is the scalar projection of the
 classical $r$-matrix.
\item At degree~$3$: the cubic shadow $\mathfrak{C}(\cA) =
...
\begin{theorem}[Pixton ideal from shadow obstruction tower; \ClaimStatusProvedElsewhere]
\label{thm:pixton-from-shadows}% NOTE: retains conj: prefix for backward compatibility
...
$H$ & Generic-level Theorem~H package: cohomological amplitude & \textbf{Proved} on the Koszul locus & Thms~\ref{thm:main-koszul-hoch}, \\
 & $[0,2]$, duality, polynomial Hilbert growth, & at generic level; critical level & \ref{thm:hochschild-polynomial-growth}; \\
...
C1 \ClaimStatusProvedElsewhere, C2 \ClaimStatusConditional]\label{thm:lagrangian-complementarity}
...
$\mathrm{obs}_g = \kappa(\cA)\lambda_g$ is proved for
uniform-weight algebras at all genera and unconditionally at
genus~$1$ (Theorem~\ref{thm:genus-universality}).
...
\item \emph{Full tower self-duality}
 \textup{(}Proposition~\textup{\ref{prop:c13-full-self-duality}}\textup{)}.
 Self-duality holds at all degrees:
 $S_r(\operatorname{Vir}_{13}) = S_r(\operatorname{Vir}_{13}^!)$
 for every $r \geq 2$, and the shadow trace formula
 $\operatorname{RTF}(f) = 0$ vanishes for all test
 functions. The higher-degree corrections are odd in the
 complementarity asymmetry $\delta_\kappa = c - 13$ and hence
 vanish identically.
...
$\mathcal{H}(\operatorname{Vir}_{13})$ is self-dual in all six
components simultaneously; for no other Virasoro central charge
does the datum enjoy this symmetry.

\noindent\emph{Cross-reference.}\enspace
...
\ref{prop:ker-av-schur-weyl}
 & Kernel of averaging: $\dim\ker(\mathrm{av}_n) = d^n - \binom{n+d-1}{d-1}$
...
\hline
\multicolumn{3}{l}{\textit{%
 April 2026 session: bar-complex operadic correction}} \\
\hline
\multicolumn{3}{p{13cm}}{%
 \textbf{ correction (constitutional).}
...
\item \textup{Conjecture~\ref{conj:ds-kd-arbitrary-nilpotent} (DS--KD for arbitrary nilpotent).}
 Hook-type corridor proved; self-transpose rectangular (e.g.\ $(2,2)$)
 proved by self-duality. First open non-hook test case: $(3,2)$ in
...
\item \textup{Conjecture~\ref{conj:type-a-transport-to-transpose} (type~A transport-to-transpose).}
 Hook-type seeds proved; transport-closure covers $\operatorname{Par}(N)$
 for $N \leq 8$. Beyond hooks: DS--KD proved for self-transpose
 rectangular partitions (e.g.\ $(2,2)$ in $\mathfrak{sl}_4$) by
 self-duality; conjectural for remaining classes
...
$\ChirHoch^*$ has cohomological amplitude $[0,2]$, satisfies
...
is proved strictly primitive at all degrees
...
the full numerical bootstrap programme \textup{(}which requires
...
session's structural corrections (operadic scoping,
...
& SC-formality $\iff$ class~$\mathbf{G}$
 \textup{(}Proposition~\ref{prop:sc-formal-iff-class-g}\textup{)} \\
& Algebraic depth gap:
...
& Koszul boundary as swampland boundary:
...
\item \textbf{BV/BRST $=$ bar at higher genus}
 ...
 The coderived resolution absorbs this obstruction.
 ...
 The coderived resolution is complete. The remaining question
 is whether a chain-level quasi-isomorphism exists for class~M,
 which would require cancelling the non-holomorphic harmonic
 obstruction $\delta_4^{\mathrm{harm}}$ by a different mechanism
 (e.g.\ a cochain homotopy involving the Beltrami differential).
...
\item \textbf{Shadow--Siegel gap for $K3 \times E$}
 ...
 ($\kappa^{\mathrm{Heis}}(\cA) = 3$, $\kappa_{\mathrm{BCOV}} = 0$,
 ...
 \item $\kappa^{\mathrm{Heis}}(K3 \times E) = 3 = \dim_\bC$
 (Heisenberg-level Route~B; the Hodge-supertrace Route~A
 gives $\Xi(K3 \times E) = 0$ by K\"unneth multiplicativity),
 verified by $6$ independent paths in Volume~III, including
 Proposition~\ref{prop:shadow-k3e}; see Volume~III
 Remark~\ref{V3-rem:beauville-kappa-formula-subscript-split}
 for the adjudication.
 \item The factor~$2$ in $Z_{K3} = 2\,\phi_{0,1}$ is
 $\kappa^{\mathrm{Heis}}(\cA_{K3})$; the bar complex provides the
 universal moonshine multiplier $A_n = \kappa^{\mathrm{Heis}} \cdot
 \dim(\rho_n)$.
 \item The three scalars on $K3 \times E$ stay separated:
 $\kappa^{\mathrm{Heis}}(K3 \times E)=3$,
 $\Xi(K3 \times E)=0$ by K\"unneth multiplicativity on the
 Hodge column, and $\kappa_{\mathrm{BKM}}(K3 \times E)=5$.
...
\item \textbf{``All master conjectures resolved.''}
 \emph{Misleading.}
 MC1 through MC4 are proved. MC5 is resolved on the canonical
 pro-object / $J$-adic / filtered weight-completed presentations of
 the raw bar complex: the analytic HS-sewing package is proved
 ...
 genuswise chain-level BV/BRST/bar identification is proved on those
 equivalent ambients \textup{(}Theorems~\ref{thm:mc5-class-m-chain-level-pro-ambient},
 \ref{thm:mc5-class-m-topological-chain-level-j-adic};
...
\textup{(}Conjecture~\ref{conj:dk-compacts-completion}\textup{)}.
 ...
 The programme continues beyond the five MC statements.
 \emph{Violated anti-pattern}:
...
\section{Wave 13 anti-pattern registry (cross-volume)}
\label{sec:concordance-wave13}

The Wave 13 elite-voice adversarial campaign on the Volume~III K3 non-abelian
chiral bialgebra programme (ten voices: Gelfand, Kazhdan, Etingof, Polyakov,
Nekrasov, Beilinson, Drinfeld, Witten, Costello, Gaiotto; $84$ total
ATTACK--HEAL cycles) registered $59$ new anti-patterns with the prefix
\texttt{AP-CY-W13-*}, itemised below by voice. The full statements live in
\verb|notes/k3_nonabelian_yangian_swarm_wave13_20260419/SYNTHESIS_WAVE13.md|
(\S G) in the Volume~III source tree, with per-voice inscriptions in the
companion agent files. The five cross-voice-convergent top-level patterns are
reproduced inline below because they are load-bearing for multiple chapters in
all three volumes.

\subsection*{Top-5 Wave-13 cross-voice anti-patterns}

\begin{description}[itemsep=4pt, leftmargin=2.0em, style=nextline]
\item[W13-TOP-1 (Ikeda $\leftrightarrow$ Gritsenko conflation).]
...
multiplicative lift from the weak Jacobi form $\phi_{0,1}$). Ikeda's $2001$
Saito--Kurokawa lift and Ikeda's $2006$ general lift produce
$\Delta_{10}=\Delta_5^2$ from weight-$16$ elliptic sources on $\mathrm{Sp}_4$, a
distinct correspondence. Conflating the two is a type error.

\item[W13-TOP-2 (BKM-as-Yangian conflation).]
...
\item[W13-TOP-3 ($\Delta_5$ as input vs output).]
Earlier inscriptions placed $\Delta_5$ as an input on the right-hand side of
the K3 chiral bialgebra synthesis. Wave~13 reframes $\Delta_5$ as the
...
Type IIB 1/4-BPS dyon denominator $1/\Phi_{10}$ on $K3\times S^1$
...
\item[W13-TOP-4 (abelian at Lie / non-abelian at vertex).]
...
\item[W13-TOP-5 (CY-$2$ $[2]$-shift vs CY-$3$ $[3]$-shift).]
...
$6$ in $H^2(M_{24},\mathrm{U}(1))\cong\mathbb{Z}/12$.
\end{description}

\subsection*{Per-voice count (full registry in Vol~III Wave-13 synthesis)}

\begin{itemize}[itemsep=2pt]
\item Gelfand: $5$ entries, prefix \texttt{AP-CY-W13-Gel-1..5}.
\item Kazhdan: $5$ entries, \texttt{AP-CY-W13-Kaz-1..5}.
\item Etingof: $9$ entries, \texttt{AP-CY-W13-E-1..9}.
\item Polyakov: $9$ entries, \texttt{W13-POL-AP-1..9}.
\item Nekrasov: $5$ entries, \texttt{W13-Nek-AP-1..5}.
\item Beilinson: $6$ entries, \texttt{AP-CY-W13-B-1..6}.
\item Drinfeld: $8$ entries, \texttt{W13-D-AP-1..8}.
\item Witten: $3$ entries, \texttt{W13-Wit-AP-1..3}.
\item Costello: $6$ entries, \texttt{AP-CY-W13-Cos-1..6}.
\item Gaiotto: $3$ entries, \texttt{W13-Gai-AP-1..3}.
\end{itemize}

Full per-entry content (wrong claim $\to$ ghost theorem $\to$ precise error $\to$ correct relationship $\to$ type) for all $59$ anti-patterns is inscribed as a markdown-table registry at Volume~I \verb|appendices/first_principles_cache.md| lines $69$--$127$ (appended in Wave $14$ mechanical propagation); each entry carries its own unique identifier \verb|W13-<Voice>-<N>| keyed to the per-voice count above. The registry is ordered by voice (Gelfand, Kazhdan, Etingof, Polyakov, Nekrasov, Beilinson, Drinfeld, Witten, Costello, Gaiotto) and within each voice by cycle number. Additional per-voice agent
files \verb|agent_NN_VOICE_wave13.md|. The Wave~13 platonic-ideal inscription
of the K3 chiral bialgebra lives in Volume~III at
\verb|chapters/examples/k3_chiral_bialgebra_platonic.tex|, with surgical
interweavings in the K3 Yangian chapter, K3$\times E$ BKM chapter,
\verb|cy_to_chiral.tex|, and the two connection bridges (bar-cobar,
modular-Koszul). Every Wave-13 anti-pattern is paired with a substantive
heal in the corresponding inscription.

\section{Synthesis}
\label{sec:concord-wave13-DNA}

\begin{remark}[The Vol I/Vol II/Vol III concordance at the Wave~13 base point]
\label{rem:concord-DNA-1}
The three-volume concordance crystallises at the Wave~13 K3 base point
...
(canonical Wave-15 values per
Remark~\ref{rem:concord-wave15-retraction} below; intermediate
Wave-14 $(26, -312)$ retracted); Vol~III supplies the
...
Wave~13 base point to a sharpened K3-specific statement
...
\label{rem:concord-DNA-2}
The universal identity
$\hbar^2 \cdot K^{\kappa_{\mathrm{ch}}} = -1$ at $\hbar^2 = -1/8, K^{\kappa_{\mathrm{ch}}} = 8$
is the concordance hinge: it fixes the Lusztig-specialisation point
where Vol~I's chiral Positselski, Vol~II's class-$\mathcal S$ topological
holography, and Vol~III's CY/BKM functor $\Phi$ all coincide on
$\mathbf H_{\Delta_5}$. The three faces of $8$ (Mukai-doubling, Humbert-monodromy,
Lusztig-root-of-unity) are the three volumes' entry points to the same
$\mathbb Z/8$-class \cite{Mukai1987,Bruinier2002,Lusztig1990}. Cross-ref
Chapter~\ref{ch:k3-chiral-bialgebra-platonic}.
\end{remark}

\begin{remark}[Six-lens concordance and bi-based Ran factorisation]
\label{rem:concord-DNA-3}
...
Pasol--Zagier 2013; (iv) bi-based Ran factorisation $(\mathrm{Ran}(E^{\mathrm{nod,sm}}_{24}),\,\bar{\mathcal A}_2)$
...
The transition functions between charts are exactly the Wave~13 DNA
strands (pentagon coboundary at $\hbar^3$, $\widetilde M_{24}$ Schur
cocycle, Steiner $S(5,8,24)$ rigidity) \cite{PasolZagier2013,CostelloHolomorphic,ConwaySloane1988}.
Cross-ref Chapter~\ref{ch:k3-chiral-bialgebra-platonic}.
\end{remark}

\section{Wave-15 central-charge retraction: constitutional record}
\label{sec:concord-wave15-central-charge-retraction}

\begin{remark}[Wave-15 central-charge double-retraction;
canonical values for $\mathcal T[A_1, \Sigma_{0,24}]$]
\label{rem:concord-wave15-retraction}
\ClaimStatusProvedElsewhere
The concordance records three successive evaluations of the
class-$\mathcal S$ central charges at $\mathcal T[A_1, \Sigma_{0,24}]$:
Wave~13 first-principles pants-decomposition gave
$(c_{4d}, c_{2d}) = (107/6, -214)$;
Wave~14 retracted in favour of the Chacaltana--Distler ansatz
$c_{4d}(A_1, \Sigma_{g,n}) = (12(g-1) + 7n)/6 = 26$ at
$(g,n) = (0,24)$, yielding $c_{2d} = -312$;
Wave~15 Gaiotto falsified the Wave~14 ansatz against the
$\mathrm{SU}(2)$ $N_f = 4$ cross-check (the ansatz returns
$c_{4d} = 8/3$ at $(g,n) = (0,4)$, whereas Shapere--Tachikawa
$2008$ Table~4 and Tachikawa $2013$ eq.~(13.6) give $c_{4d} = 7/6$)
and restored the Wave~13 values via the correct formula
\[
...
Shapere--Tachikawa $2008$ eq.~(2.20)), cross-verified at $n = 3$
($c_{4d} = 1/3$) and $n = 4$ ($c_{4d} = 7/6$).
\textbf{Canonical Wave-15 values} (this constitutional record):
\[
 \boxed{\;
...
eq.~(3.14). Coulomb rank $21$, flavour $\mathfrak{su}(2)^{24}$,
Steiner $S(5,8,24)$ rigidity unchanged.

The intermediate Wave~14 value
$(c_{4d}, c_{2d}) = (26, -312)$ is retracted constitutionally. All
manuscript inscriptions in Vols.~I, II, III displaying the
Wave~14 numerical values are scope-qualified to the retracted
Wave~14 evaluation; their surrounding mathematical content (shadow
towers, Cardy counting, Zamolodchikov norm, mock-modular weight,
four-factorisation numerology of $-312$) depends on the global
$c_{2d}$ value through combinations $c/2$, $c(5c+22)/10$, $-c/24$,
$-48/[c^2(5c+22)]$ which must be recomputed at $c_{2d} = -214$;
see the master caveat at
Vol.~III Remark~\ref{rem:k3-classS-numerical-caveat}
(\texttt{k3\_chiral\_bialgebra\_platonic.tex}), the master
proposition at
Vol.~III Proposition~\ref{prop:k3-chacaltana-distler-24}, and
Vol.~I Remark~\ref{rem:thqg-occ-wave15-wave14-retraction}
(\texttt{thqg\_open\_closed\_realization.tex}). The
structural content --- factorisation algebra, bi-based Ran
datum, Steiner rigidity, $M_{24}$ equivariance, $\Phi_{10}$
denominator, averaging morphism --- is independent of the
numerical $c_{2d}$ and survives all three waves unchanged.

Primary-literature audit: Chacaltana--Distler $2010$
...
Epistemic remark (Beilinson's dictum in action): the
double-retraction is not a bug but the working-correctness of the
dictum: intermediate-wave numerical claims are false until
independently audited against primary literature. The Wave~14
ansatz passed no primary-source audit; the Wave~13 formula passed
three cross-checks ($n = 3, 4, 24$). Future waves inheriting
central-charge values for class-$\mathcal S$ theories must
cross-verify at $n = 3, 4$ before using any new $\Sigma_{g,n}$
specialisation.
\end{remark}
...
\begin{remark}[Shimura--Waldspurger bridge: constitutional conversion
...
$L(1/2, \mathrm{Sh}(f)) = L(1/2, f_{16})$ with
$f_{16} = E_4 \cdot \Delta$ the unique Hecke eigenform
in $S_{16}(\mathrm{SL}_2(\mathbb Z))$, and the ratio evaluates via
Gritsenko--Nikulin 1998 Thm 4.1 to
\begin{equation}
...
\begin{proof}[Three verification paths for $C_5 = 1/64$]
...
see Remark~\ref{rem:walgdeep-DNA-W16-borch-vs-grit} for the
exponent table). Squaring the Borcherds product
$\Delta_5^{\mathrm{Borch}}$ gives a weight-$10$ Siegel form
$(\Delta_5^{\mathrm{Borch}})^2 = \Phi_{10}/2^{12}$ by Borcherds
...
Square-rooting: $\Delta_5^{\mathrm{Borch}} = \Phi_{10}^{1/2}/2^6 =
\Phi_{10}^{1/2}/64$. On the Gritsenko side
$\Delta_5^{\mathrm{Grit}} = \Phi_{10}^{1/2}$ by Gritsenko--Nikulin
1998 Thm 4.1. Quotient: $C_5 = 1/64$.

\textup{(Path 2 --- Waldspurger period at the
central point.)}
Waldspurger's formula \cite{Waldspurger81} for
$f \in S_{k+1/2}^+(\Gamma_0(4))$ gives
$|a_f(D)|^2 = L(1/2, \mathrm{Sh}(f) \times \chi_D) / \langle f, f\rangle$
up to explicit Euler factors. At $k = 4$ and $D = -4$ (the K3
Heegner divisor $H_1$), the central $L$-value
$L(1/2, f_{16} \times \chi_{-4})$ equals
$L(1/2, f_{16}) \cdot L(1/2, \chi_{-4})$ by the
$\chi_{-4}$-quadratic-twist factorisation; substituting Kohnen's
$\langle f, f\rangle = \pi^{-(k+1/2)} (k+1/2)!
\,L(1, \mathrm{Sym}^2 f) / 2^{2k-2}$ (Kohnen 1985
\emph{Math.\ Ann.}~271 Thm~3) yields
$|a_f(-4)|^2 = 2^{-12}$ times the Gritsenko-normalised coefficient.
The square root gives $C_5 = 2^{-6} = 1/64$.

\textup{(Path 3 --- cross-check via the universal identity
$\hbar^2 \cdot K = -1$.)}
The K3 three-faces identity $\hbar^2_{K3} = -1/8$,
$K_{K3} = 8 = 2 c_+(\mathrm{II}_{2,2})$
(\S\ref{rem:concord-DNA-2}) fixes the
$\mu_8$-gerbe banding. The $\mu_8$-banding shifts the
singular-theta integrand by a factor of
$e^{2\pi i / 8} = \zeta_8$ per sheet; summing over
the $\mu_8$-covering sheets multiplies the Borcherds product by
$|\mu_8|^{-2} = 1/64$ relative to the Gritsenko additive sum
(Bruinier 2002 LNM~1780 Prop~5.1). Quotient: $C_5 = 1/64$.

Three structurally independent routes --- direct Jacobi input
normalisation, Waldspurger central-$L$-value evaluation, and
$\mu_8$-banding sheet count --- converge on $C_5 = 1/64$.
\end{proof}
...
require the weight-specific $C_k$; the programme uses only $k = 4$
