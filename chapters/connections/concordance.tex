...
\textbf{Package~(i)} (categorical prefundamental CG decomposition) is
\textbf{proved for all simple types} by
Theorem~\ref{thm:categorical-cg-all-types}, using multiplicity-free
$q$-characters \cite{FrenkelMukhin01, Nakajima04, Hernandez06KR}
transferred to Yangians via
\cite{GTL17, GTL-catO}.
The \textbf{full four-package MC3} is proved for
\textbf{type~$A$} (Theorem~\ref{thm:mc3-type-a-resolution}).
For other types, packages~(ii)--(iv) reduce to thick generation
of $\mathrm{Rep}_{\mathrm{fd}}(Y(\mathfrak{g}))$ by evaluation
modules (expected for all types by the Chari--Pressley
classification; proved in type~$A$ by
...
\medskip\noindent
\textbf{Conditional and partial} (xi)--(xii):
...
Principle~\ref{princ:modular-trace} (Volume~II) formalizes the
organizing thesis that modularity is not an adjective on the closed
algebra but a trace-plus-clutching structure on the open factorization
category $\cC$: cyclic structure gives the invariant pairing, Hochschild
chains give the annulus/open-to-closed map, framing-compatibility gives
the ribbon twist, and survival under all clutching degenerations gives
genus-$g$ amplitudes.  The genus-$1$ curvature $\kappa(\cA)$ is the
image of the annular trace under the clutching map that degenerates an
annulus to a node; the full shadow Postnikov tower is the
genus-and-arity-graded expansion of the universal twisting
element~$\Theta_\cC$.
...
(\texttt{pixton\_shadow\_bridge.py}, $54$ tests).
The quartic shadow~$S_4$
enters only through codim-$2$ graph~$C$ (sunset),
whose Hodge integral vanishes identically at genus~$2$;
the first $S_4$-dependent contribution appears at genus~$3$
where the sunset Hodge integral is generically nonzero.
\end{remark}
...
\begin{enumerate}[label=\textup{(\alph*)}]
\item Theorem~\ref{thm:ent-scalar-entropy}:
  $S_{\mathrm{EE}}^{\mathrm{sc}} = (c/3)\log(L/\varepsilon)$
  from $\kappa(\cA) = c/2$ and the twist operator dimension
  (Calabrese--Cardy~\cite{Calabrese-Cardy04}).
\item Theorem~\ref{thm:entanglement-complementarity}:
  $S_{\mathrm{EE}}(\cA) + S_{\mathrm{EE}}(\cA^!) = (26/3)\log(L/\varepsilon)$
  for the Virasoro family (from $\kappa + \kappa' = 13$).
\item Proposition~\ref{prop:ent-complexity-classification}:
  four-class entanglement complexity (G/L/C/M), with shadow radius
  $\rho(\cA)$ controlling convergence of quantum corrections.
\item Proposition~\ref{prop:ent-qes}: the quantum extremal surface
  condition as a shadow connection Ward identity.
\end{enumerate}
Status: \textbf{all proved}.
External input: twist operator dimension (standard CFT).
