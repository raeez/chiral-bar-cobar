...
\emph{Frontier} (conjectural): the DK/KL categorical lift beyond the
...
\begin{remark}[Sharpened MC3 diagnosis: five serious gaps]
\label{rem:mc3-sharpened-diagnosis}
Adversarial analysis (red-team/blue-team/green-team computational verification,
$438$ additional tests) has sharpened the MC3 frontier diagnosis as follows.

\emph{Character-level evidence.} The prefundamental Clebsch--Gordan decomposition
$V_n \otimes L^- = \bigoplus_{j=0}^{n} L^-(n - 2j)$ is verified computationally
for $n \le 25$, and $K_0$-generation $[M(\lambda)] \in K_0\text{-span}(\{L^-(\text{shifted})\})$
is verified for $\lambda = 0, \ldots, 100$.  No failure has been observed.

\emph{Five serious gaps in the categorical lift.}
\begin{enumerate}
\item \emph{$K_0 \neq$ thick.} $K_0$-generation does not imply thick generation
  in a triangulated category.  The Thomason--Trobaugh theorem requires compact objects,
  but $L^-$ is infinite-dimensional with $\dim(L^-)_{-2k} = p(k)$.
\item \emph{Pro-completion changes the category.} Francis--Gaitsgory pro-nilpotent
  completion gains modules (Verma via pro-Weyl) and may lose asymptotic modules
  (Hernandez--Zhang).  The thick subcategory statement is for the \emph{completed}
  category, not the original $\mathcal{O}^{\mathrm{sh}}$.
\item \emph{$\mathcal{O}_{\mathrm{poly}}$ is not dense.}  $L^-$ has weight
  multiplicities $p(k) \to \infty$, so $L^- \notin \mathcal{O}_{\mathrm{poly}}$.
  Baxter equivariance on $\mathcal{O}_{\mathrm{poly}}$ does not automatically extend.
\item \emph{Resolution obstruction.}  The sequence $\delta(k) = p(k) - 1$ is unbounded
  and superpolynomial: thick generation (finite iterated cones) fails, and
  the correct statement requires \emph{localizing} generation with infinite coproducts.
\item \emph{Braided monoidal transfer.} The braided structure (R-matrix, hexagon axioms)
  is additional data beyond the categorical equivalence; transfer through bar--cobar
  requires the derived Drinfeld--Kohno theorem.
\end{enumerate}

\emph{Recommended approach: chromatic $+$ Efimov hybrid.} The conformal weight filtration
decomposes MC3 into countably many finite-dimensional problems, each solvable by the
proved sectorwise spectral-sequence convergence.  Efimov's categorical formal completion
then assembles the answer via the pro-Weyl--Mittag-Leffler tower.  A critical
negative finding: naive weight truncation fails (capture ratios decrease to $0$),
but bidegree-wise spectral-sequence convergence succeeds.  This hybrid path breaks
the circular dependency on $L^-$ compactness by working stratum-by-stratum where
...
\begin{remark}[Arakelov--bar transfer: the single missing MC5 ingredient]
\label{rem:arakelov-bar-transfer}
The entire MC5 programme at genus $g \ge 2$ reduces to one theorem.

\emph{Statement (conjectured).}  For any chiral algebra $\cA$ on a smooth
curve $\Sigma_g$ of genus $g \ge 2$, the fiberwise bar differential satisfies
$d_{\mathrm{fib}}^2 = \kappa(\cA) \cdot \omega_g$, where the identity follows
from applying the Arakelov--Green identity $dd^c G(z,w) = \delta(z,w) - \omega_g$
to each term in the OPE expansion of the bar construction.

\emph{Why the factorization $d^2 = (\text{algebra}) \times (\text{geometry})$ is forced.}
Four constraints:
\begin{enumerate}
\item $d^2$ is additive in $\cA$ (tensor product gives additive curvature);
\item $d^2$ is antisymmetric under Koszul duality (complementarity, Theorem~C);
\item $d^2$ is a scalar invariant of $\cA$ (Schur's lemma for the conformal symmetry action);
\item $H^{1,1}(\Sigma_g) = 1$ for all $g \ge 1$ (Hodge theory on curves), so the geometric
  factor is forced into a one-dimensional space.
\end{enumerate}
By Theorem~D, $\kappa(\cA)$ is the \emph{unique} additive, antisymmetric scalar
with $\hat{A}$-genus generating function.  Hence IF $d^2$ factors, the algebra
factor is $\kappa(\cA)$ and the geometric factor is $\omega_g$.

\emph{Four required ingredients.}
(i)~Fay trisecant identity (kills the holomorphic Arnold combination; algebraic geometry);
(ii)~Arakelov--Green identity $dd^c G = \delta - \omega_g$ (produces $\omega_g$; arithmetic geometry);
(iii)~OPE convergence in the Arakelov metric (vertex algebra theory);
(iv)~cyclic-sum cancellation in the bar formalism (homotopical algebra).

\emph{Alternative: clutching induction.}  Since genus~$0$ and genus~$1$ are proved,
non-separating degeneration $\Sigma_g \to \Sigma_{g-1} \cup_{\text{node}}$ reduces
genus~$g$ to genus~$g-1$ plus a local nodal correction.  The nodal correction
is controlled by the genus-$0$ identification (locality), potentially closing
the induction without Costello renormalization.  The sewing correction
$\lambda_g^{\mathrm{FP}} - \lambda_{g-1}^{\mathrm{FP}}$ is strictly negative
for all $g \ge 2$ (verified to $g = 15$), consistent with factorial decay.

\emph{Computational evidence.}
$\lambda_g^{\mathrm{FP}}$ matches the Bernoulli formula to $g = 15$ ($231$ tests).
Cross-family universality $F_g(\cA)/F_g(\cB) = \kappa(\cA)/\kappa(\cB)$ verified
at $g = 1, 2, 3, 5, 10$ for five families.  Complementarity
$\kappa(\cA) + \kappa(\cA^!) = \mathrm{const}$ verified at $g = 1, \ldots, 7$.
\end{remark}
...
\item The \emph{branch-line reductions} appendix
  ...
  cumulant hierarchy.
\item The \emph{ambient complementarity} section
  (\S\ref{sec:ambient-complementarity}) upgrades complementarity
  from an additive splitting to shifted-symplectic Lagrangian
  geometry: one formal moduli problem with two Lagrangian maps,
  a complementarity potential~$S_\cA$ whose Taylor jets are the
  nonlinear modular invariants, and a derived critical locus
  governing simultaneous self-dual deformations.
\item The \emph{graph-completed Feynman transform programme}
  ...
  full universal class should live, introduces the
  \emph{complementarity Legendrian}~$\Lambda_\cA = j^1 S_\cA$
  (contactifying the complementarity potential), the full
  \emph{resonance tower}~$\mathfrak{R}^{\mathrm{mod}}_{r}$ for all
  arities~$r \ge 4$, the \emph{nonlinear phase
  index}~$\nu_{\mathrm{nl}}(\cA,u)$, and the \emph{nonlinear
...
\emph{filtration controls everything.}
