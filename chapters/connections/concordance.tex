...
the factorization itself}: the Arnold defect on~$\Sigma_g^3$, which
a~priori lives in a $g(g{+}1)/2$-dimensional space of quasi-modular
$(1,1)$-forms, must be shown to collapse to a scalar multiple
...
genus-$0$ identification.  The sewing correction
$\lambda_g - \lambda_{g-1}$ is strictly negative for $g \ge 2$ and
decays factorially.
\end{remark}
