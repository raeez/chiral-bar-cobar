...
a \emph{consequence} of chiral Koszul duality via Verdier duality on
Fulton--MacPherson compactifications
(Theorem~\ref{thm:universal-kac-moody-koszul};
see Chapter~\ref{chap:kac-moody} for the full treatment,
including the critical-level recovery of the FF center
...
MC4 & M-level proved (Conj.~\ref{conj:master-infinite-generator})
  & $\mathcal{W}_\infty$/Yangian H-level comparison;
    exact packet identification is the remaining frontier. \\
...
for both towers; the $\mathcal{W}_\infty$ reduction terminates at
an exact stage-$4$ packet
(Remark~\ref{rem:mc4-winfty-computation-target}), and the Yangian
reduction terminates at a single degree-$2$ coefficient
