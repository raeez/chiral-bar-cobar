...
\item \emph{Monoidal bar-cobar.}
  ...
  monoidal setting.  This is relevant to the fusion product conjecture
  (Conjecture~\ref{conj:fusion-bar-cobar}): the bar construction
  should be a monoidal functor from the tensor category of chiral
...
Recent work of Idrissi~\cite{Idrissi22} establishes that Poincar\'e duality on configuration spaces ensures $d^2 = 0$ for the propagator-based differential on $\mathsf{E}_n$-algebras, providing part~(i) for general~$n$.  Knudsen's higher enveloping algebras~\cite{Knudsen18} supply the $\mathsf{E}_n$ analogue of the universal enveloping algebra ($U_n(\mathfrak{g})$), with PBW and Koszul duality theorems in the $\mathsf{E}_n$ setting; these are the natural target for part~(iii).
\end{remark}
...
\end{enumerate}
The synthesis of these components --- assembling the genus-$0$ chain
map into a full bar-BRST identification for chiral algebras --- is a
concrete next step, distinct from (and more tractable than) the
higher-genus generalization.
\end{remark}
