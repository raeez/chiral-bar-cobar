...
conjectural frontier: MC3/MC4 as the next structural layer, and MC5 as
...
the remaining DK-$5$ categorical packet is downstream of MC3.  MC5 at
...
renormalization input.  The remaining algebraic frontier of the full
programme is therefore exactly MC3 (categorical extension beyond the
evaluation-generated core).
...
\item Shifted-prefundamental generation: Hernandez--Jimbo--Zhang
  ...
  \item \emph{Prefundamental CG}:
    ...
    at character level (verified $n = 1,\ldots,8$, depth~$40$).
...
proved for $Y(\mathfrak{sl}_2)$
(Theorem~\ref{thm:shifted-prefundamental-generation}):
the compact objects of
$\widehat{D}(\mathcal{O}^{\mathrm{sh}}_{\leq 0})$ are the
thick closure of $\{V_n(a)\} \cup \{L^-_a\}$.
The proof uses the \emph{prefundamental Clebsch--Gordan}
($V_n \otimes L^- \cong \bigoplus L^-(\text{shifted})$, $15$~tests,
$n = 1, \ldots, 8$, depth~$40$)
and the \emph{Baxter SES for $L^-$}
(equation~\eqref{eq:baxter-ses-prefundamental}, unconditional at
$\lambda = 0$, $13$~tests), together with the pro-Weyl convergence
(Mittag--Leffler, $344$~tests) and $E_1$~braiding compatibility.
Remaining: type~$A$ at $N \geq 3$ (extension from $\mathfrak{sl}_2$).
...
(verified $n = 1, \ldots, 8$, depth~$40$;
$15$~tests in \texttt{test\_prefundamental\_clebsch\_gordan})
proves that $L^-$ is \emph{evaluation-stable}: tensoring with~$V_n$
preserves the prefundamental class.  This immediately upgrades
character containment from $\lambda \leq 6$ to \emph{all}~$\lambda$
and determines the $K_0$ ring structure:
$[V_n] \cdot [L^-] = \sum_{j=0}^{n} [L^-(n-2j)]$.
Combined with the unconditional Baxter SES,
$L^-(b') \in \operatorname{thick}\langle L^-(b), V_1\rangle$ for
any~$b, b'$.  The $K_0$-generation of all Verma modules by
$\{L^-(\text{shifted})\}$ is verified for $\lambda = 0, \ldots, 10$.
Total: $1{,}690$+~tests supporting the four-step proof.
...
Extension to $g \geq 1$ remains conjectural (MC5).
\end{proposition}
