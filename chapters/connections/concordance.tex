...
conjectural frontier: MC3 is \textbf{proved for type~$A$} at
all~$N$ (Theorem~\ref{thm:shifted-prefundamental-generation}),
with four categorical inputs remaining for the full lift;
...
the remaining DK-$5$ categorical packet is formal once the four
MC3 categorical inputs (Thomason--Trobaugh, Francis--Gaitsgory,
$L^-$~compactness, braided transfer) are established.  MC5 at
...
renormalization input.  With MC3 proved for type~$A$, the remaining
algebraic frontier consists of (i)~the four categorical inputs for the
full MC3 lift, and (ii)~the genus~$\geq 2$ analytic completion~(MC5).
...
MC3 & Core proved (Conj.~\ref{conj:master-dk-kl})
  & DK/KL equivalences, Yangian Koszulness; core proved at all types
    (Cor.~\ref{cor:dk23-all-types}); Baxter equivariance proved on
    $\mathcal{O}_{\mathrm{poly}}$
    (Prop.~\ref{prop:baxter-yangian-equivariance});
    extension beyond eval-gen.\ core conjectural
    (Rem.~\ref{rem:corrected-mc3-frontier}(b)). \\
...
\emph{MC3} (factorization-categorical DK/KL): \textbf{core proved}
on the evaluation-generated core at all simple types, by thick
generation for type~$A$ (Theorem~\ref{thm:dk-fd-typeA}) and
sectorwise convergence for all types
(Corollary~\ref{cor:dk23-all-types}); extension beyond the
generated core requires four conjectural packages
...
\item Shifted-prefundamental generation: Hernandez--Jimbo--Zhang
  ...
  Thick generation obstruction analysis ($206$~tests,
  ...
  V_n) \leq \lfloor n/2\rfloor + 1$ (exact for even~$n$);
  Euler characteristic $\chi(L^-, V_n) = \sum_{k=0}^{n/2} p(k)$;
  character containment
  $\operatorname{ch}(M(\lambda)) \leq \operatorname{ch}(V_\lambda
  \otimes L^-)$ for \emph{all}~$\lambda$ via the prefundamental
  Clebsch--Gordan (R1).

  Three structural results (R1--R3):
  ...
  \item \emph{Prefundamental CG}
    ...
    at character level (\textbf{proved} for all~$n$; verified
  ...
  (Corollary~\ref{cor:k0-generation-OY}; $31 + 109 = 140$~new tests
  in \texttt{test\_prefundamental\_cg\_closure}).
  \emph{Gap}: categorical thick generation (lift CG projectors
  from characters to $Y$-equivariant splittings).
...
\]
\begin{itemize}
\item The \emph{nonlinear modular shadows} appendix
  (Appendix~\ref{app:nonlinear-modular-shadows}) makes the
