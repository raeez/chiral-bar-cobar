...
(11 unconditional, 1 conditional; D-module purity partially proved).  This is the spine.  Nothing in the extension
...
\noindent
\emph{Status}: eleven equivalences proved unconditionally
(including tropical Koszulness and bifunctor decomposition);
the Lagrangian criterion is conditional on perfectness of the
ambient tangent complex.
$\mathcal{D}$-module purity is a thirteenth characterization,
partially proved (one direction established, converse open)
...
where $\mathfrak{n}_+$ is non-abelian and the filtration formality
...
\item \textbf{Chiral Koszulness from (H1)--(H4).}
  ...
  filtration may degenerate, and Koszulness is
  genuinely open.
...
\item \textbf{Filtration $\geq 4$ spectral Drinfeld classes.}
  ...
  Consequently, strictification is proved through filtration~$4$.
  Beyond filtration~$4$, $\operatorname{grt}_1$ is nonzero
  at all weights $\geq 5$ except~$6$, so the spectral
  Drinfeld class must be analysed case by case.
\end{enumerate}
...
$\operatorname{grt}_1$ is nonzero at all odd weights and at
all even weights $\geq 8$
\textup{(}$\dim\operatorname{grt}_1(\text{weight }8) = 1$ from
$[\sigma_3,\sigma_5]$\textup{)},
so the spectral Drinfeld class must be analysed case by case.
\end{remark}
