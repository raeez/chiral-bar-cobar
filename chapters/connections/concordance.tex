...
At rank $\geq 2$ with denominator $q \geq 3$, Koszulness \emph{fails}:
the abelian Cartan subalgebra $\fh \subset \fg$ produces
$\mathrm{rk}(\fg)$ classes in the bar spectral sequence $E_1^{2,q}$
that survive to $E_\infty$ because $[\fh, \fh] = 0$ kills the
Lie-bracket differential $d_1$
(Conjecture~\ref{conj:admissible-koszul-rank-obstruction}).
The obstruction is $\dim H^2(\barB(L_k(\fg))) \geq \mathrm{rk}(\fg)$.
