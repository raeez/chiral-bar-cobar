...
\textbf{B} & Bar-cobar inversion
  & $\Omegach(\barBch(\cA)) \xrightarrow{\sim} \cA$
    ...
    genus~$0$ unconditional; $g \geq 1$ requires
    axiom~\ref{MK:modular}, verified for Heisenberg, KM,
    Virasoro, principal $\cW$-algebras
    \textup{(}Remark~\ref{rem:higher-genus-mk3-conditionality}\textup{)}
  & \ClaimStatusProvedHere \\
...
\textbf{D} & Modular characteristic
  & $\kappa(\cA)$ universal, additive, duality-constrained
...
The five theorems above decompose into two tiers.
Theorems~A, B, and~C are \emph{structure theorems}:
they establish that the shadow tower is algebraic,
that it coincides with the $L_\infty$ formality obstruction tower,
and that Koszul duality acts on it by Lagrangian complementarity.
...
\item \textbf{Formality identification}
  ...
  the genus-$0$ projection of the transferred $r$-ary
  $L_\infty$ bracket on~$\gAmod$ evaluated on the truncated
  MC element.  The shadow tower \emph{is} the $L_\infty$
  formality obstruction tower.
...
\item \textbf{Complementarity}
  ...
  and the two summands are Lagrangian.
\end{itemize}
...
All standard families (Heisenberg, affine Kac--Moody,
$bc$-$\beta\gamma$, Virasoro, $\Walg_N$) are chirally
Koszul (Theorem~\ref{thm:koszul-equivalences-meta}).
...
The shadow tower free energy $F_g = \kappa \cdot \lambda_g^{\mathrm{FP}}$
...
This covers the entire standard landscape.  The proof
...
carries a proved Verdier shifted symplectic structure, and the
ambient complementarity deformation problem carries a proved
$(-1)$-shifted symplectic structure.  In both settings,
$Q_g(\cA)$ and $Q_g(\cA^!)$ appear as complementary Lagrangians in the
...
\item a $(-1)$-shifted symplectic structure on the ambient
  complementarity formal moduli problem
