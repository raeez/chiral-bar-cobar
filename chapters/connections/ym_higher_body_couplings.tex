...
The volume's true object is not a single strict algebra but a
hierarchy of coherent multiplications, restrictions, and
transgressions.  Once one asks how several boundary or defect sectors
interact, ordinary Hochschild theory must be relativized, and binary
couplings are no longer enough: the multi-body problem forces a
simplicial package whose nerve organises all pairwise and higher
mixed deformations into a single coherent datum.  The relative
Hochschild cochain complex
$CC^\bullet_{\mathrm{rel}}(M_{12};M_1,M_2)$, defined as the
homotopy cofibre of the restriction maps, captures the genuine mixed
couplings that cannot be seen from either boundary alone; and the
higher-body \v{C}ech complex extends this picture to an arbitrary
finite collection of boundary conditions, assembled via a simplicial
boundary nerve whose cohomology controls the simultaneous
deformability of the entire configuration.

The four editorial laws organise the material.
\emph{Strictness as shadow}: single-boundary Hochschild cohomology
$HH^\bullet(M_i)$ and its Serre-dual deformation algebra are the
strict starting data; mixed and higher-body structures are coherent
corrections to the fiction that each boundary deforms independently.
\emph{Modularity at chain level}: the relative duality theorem
(Theorem~\ref{thm:relative-duality-theorem}) is a projection of
Verdier intertwining in the convolution algebra
$\mathfrak{g}^{\mathrm{mod}}_\cA$; the long exact sequence it
produces is the Mayer--Vietoris sequence of the modular deformation
complex restricted to the boundary configuration.
\emph{Examples as proving grounds}: the principal
$\mathcal{W}$-boundary of twisted Yang--Mills provides the test
case, where the mixed Kodaira--Spencer class and its higher-body
\v{C}ech generalisation are computed explicitly and matched against
the predictions of complementarity.
\emph{Close the loop}: the simplicial boundary package defined here
is the input to the instanton screening formalism of
Chapter~\ref{ch:ym-instanton-screening}; the Novikov completion of
the higher-body complex controls how nonperturbative sectors mix
