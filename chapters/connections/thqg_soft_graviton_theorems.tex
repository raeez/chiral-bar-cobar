...
\section{Soft graviton theorems from the shadow obstruction tower}
\label{sec:thqg-soft-graviton-theorems}
\index{soft graviton theorem|textbf}
\index{Ward identity!gravitational soft theorem}
\index{shadow obstruction tower!soft graviton theorems}
...
(Definition~\ref{def:holographic-modular-koszul-datum}) centers on the modular
shadow connection
...
connection, decomposed by the degree filtration of the shadow obstruction
tower (Definition~\ref{def:shadow-postnikov-tower}), are the soft
graviton theorems of the title.

Four results emerge:
\begin{enumerate}[label=\textup{(\arabic*)}]
\item to prove that the degree-$2$ Ward identity of
...
\item to prove that the degree-$3$ Ward identity gives the
...
\item to prove that the degree-$4$ Ward identity produces the
...
\label{rem:thqg-VI-degree-filtration}
...
\begin{equation}\label{eq:thqg-VI-degree-decomposition}
\Sh_{g,n}(\Theta_\cA)
\;=\;
\sum_{r=2}^{\infty}
...
\begin{equation}\label{eq:thqg-VI-flatness-degree-r}
F^{(r)}_{g,n}
\;:=\;
\Bigl(
...
\item \emph{Degree~$2$}:
\begin{equation}\label{eq:thqg-VI-flat-degree-2}
\nabla^{(2)}_{g,n} \circ \nabla^{(2)}_{g,n} = 0
...
\item \emph{Degree~$3$}:
\begin{equation}\label{eq:thqg-VI-flat-degree-3}
...
\item \emph{Degree~$4$}:
\begin{equation}\label{eq:thqg-VI-flat-degree-4}
...
\begin{equation}\label{eq:thqg-VI-flat-degree-r-general}
...
degree decomposition~\eqref{eq:thqg-VI-nabla-degree}. Write
...
the endomorphism equation~\eqref{eq:thqg-VI-flat-degree-r-general}
...
\begin{remark}[Degree truncation and the depth of soft gravity]
\label{rem:thqg-VI-degree-truncation}
\index{shadow depth!soft graviton hierarchy}
For an algebra of shadow depth $r_{\max}(\cA)$, the system
\eqref{eq:thqg-VI-flat-degree-r-general} closes at $r = r_{\max}$:
...
\begin{equation}\label{eq:thqg-VI-ward-degree-r}
\boxed{%
...
For~(iii): the degree-$2$ shadow $\Theta^{(2)}_{\widehat{\fg}_k}$
acts through the Casimir pairing $k\,\delta_{ab}/2$ (the Killing form
at level~$k$); its trace is the scalar modular characteristic
$\kappa(\widehat{\fg}_k) = (k+h^\vee)\dim(\fg)/(2h^\vee)$.
The shadow representation at genus~$0$ acts by
...
framework, this is the degree-$2$ Ward identity of the modular
...
(equation~\eqref{eq:thqg-VI-flat-degree-2}) states that
...
\;=\; 0,
\end{equation}
for every holomorphic function~$f$. This is the degree-$2$ shadow
...
soft theorem (it is the degree-$2$ flatness condition), and
...
arises from the degree-$3$ Ward identity of the shadow connection.
...
insertion in the degree-$3$ sector. The subleading soft limit is
...
(equation~\eqref{eq:thqg-VI-flat-degree-3}) states
\[
...
subleading (degree-$3$) Ward identities generate superrotations;
...
theorems is the degree-$4$ shadow
...
The Virasoro algebra has the following structure at weight~$4$:
the space of singular vectors in the Verma module $M(0)$ at
weight~$4$ is controlled by the Gram matrix determinant
\[
\det G_4 \;=\; \frac{c^2(5c + 22)}{100} \cdot (\text{factors}).
\]
The factor $c(5c + 22)$ in the denominator
of~\eqref{eq:thqg-VI-virasoro-quartic} comes from the inverse of
this Gram determinant: the quartic contact invariant is the
...
denominator is the Shapovalov norm. The explicit computation uses
the Virasoro commutation relations and gives the factor $10$ in the
numerator.
...
(equation~\eqref{eq:clutching-factorization-frontier}), projecting
...
degree-$4$ flatness equation~\eqref{eq:thqg-VI-flat-degree-4}.
\end{theorem}
...
\subsubsection{The general degree-$r$ soft theorem}

\begin{theorem}[General degree-$r$ soft graviton theorem;
\ClaimStatusProvedHere]
\label{thm:thqg-VI-general-soft}
\index{soft graviton theorem!general degree|textbf}
...
\subsubsection{The degree-$5$ soft factor for Virasoro}
...
For $\cA = \mathrm{Vir}_c$, the degree-$5$ soft factor at genus~$0$
is:
...
the degree-$r$ shadow $\Sh_r(\cA)$ coincides (at degrees $2,3,4$)
...
that this identification extends to all degrees.
\end{remark}
...
f_\infty(\cA)$ (Theorem~\ref{thm:operadic-complexity-detailed},
the operadic complexity theorem, proved at all degrees) states
that the shadow depth equals the $A_\infty$-depth equals the
$L_\infty$-formality level. In the
...
for the standard-landscape archetypes.
\end{remark}

\subsubsection{The higher-degree soft factors: structure theorems}
...
or level, the degree-$r$ soft factor $S^{(r-2)}$ at genus~$0$ has
...
direct reinterpretation in celestial holography:
...
graviton theorems. The connection between
the shadow obstruction tower and celestial holography is direct.

\subsubsection{The celestial OPE algebra from the shadow obstruction tower}
...
\item $\mathfrak{S}(\cA)$ is generated by the degree-$2$ and
 degree-$3$ components (the Weinberg and Cachazo--Strominger
 sectors), subject to the flatness relations of
 Theorem~\ref{thm:thqg-VI-flatness-by-degree}.
\item The degree-$2$ sector generates a subalgebra isomorphic to
 the supertranslation algebra
 (Corollary~\ref{cor:thqg-VI-bms-supertranslation}).
\item The degree-$3$ sector, combined with degree-$2$, generates a
...
class-$\mathbf{L}$, degrees~$2$ and~$3$ are present, giving
$\mathrm{BMS}_4$. For classes~$\mathbf{C}$ and~$\mathbf{M}$,
the higher degrees provide extensions.
\end{proof}
...
degree are the Ward identities of the $\mathcal{W}_{1+\infty}$
currents: the degree-$r$ soft theorem corresponds to the
...
$\beta\gamma_\lambda$ &
 $\mathbf{C}$ & $4$ & $3$ &
 $\kappa = 6\lambda^2{-}6\lambda{+}1$ \textup{(}$= 0$ on wt-changing line\textup{)}, $\mathfrak{Q} = \mathrm{cyc}(m_3)$ \\
$\mathrm{Vir}_c$ &
...
\item \textbf{(G1)} Perturbative finiteness: The soft factors
 ...
 (degree cutoff, Lemma~\ref{lem:degree-cutoff}).
...
\item \textbf{(G3)} Symplectic polarization: The soft Ward
 ...
 the soft factor at each degree respects the bulk/defect
 decomposition.
