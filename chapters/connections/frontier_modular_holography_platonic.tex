...
rational $r$-matrix $r^{\mathrm{cl}}(z) = \Omega/z$ is the classical
...
\label{thm:frontier-sft-vertices}
...
\item \emph{D3 brane \textup{(}twisted $\mathcal{N} = 4$ SYM\textup{)}.}
  ...
  $r(z) = \Omega/z$ (classical Yang--Baxter,
  $\Omega$ the Casimir element), and the genus expansion
...
The R-matrix $r(z) = \Omega/z$ is the collision residue
$\mathrm{Res}^{\mathrm{coll}}_{0,2}(\Theta_\cA)$:
the current algebra OPE
$J_a(z)J_b(w) \sim k\delta_{ab}/(z{-}w)^2 +
f^c_{ab}J_c/(z{-}w)$
has at most a double pole; the $d\log$ extraction
(AP19) gives a simple-pole r-matrix $r_{ab}(z) = \Omega_{ab}/z$.
