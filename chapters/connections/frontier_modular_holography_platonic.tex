...
$\Delta_{\mathrm{cyc}}$
(Computation~\ref{comp:vol2-low-genus-control}).
...
(Theorem~\ref{thm:W3mod0}), and the unique genus-$1$ lift
...
partition function $F_g(\cA)=\kappa\cdot B_{2g}/(2g(2g-2))$
...
Genus-$1$ Hessian: $H^{(1)}=\kappa=3k/(k+2)$ (scalar,
...
edge---binary trees with two vertices.  For $n$ insertions at
...
\end{enumerate}
At $c=26$: $\kappa=0$ and the shadow connection is flat at
leading order.
...
$\kappa=(c-26)/2$ enters through the Hodge bundle class
...
second-order genus-$1$ variation.  At $c=26$: the first term
vanishes, and the shadow connection reduces to its nonlinear
corrections.
...
$\Delta_{\mathrm{Vir}}(x)=1-\frac{c-26}{2}\,x$;
single branch point at $x=2/(c-26)$, diverging at $c=26$.
\end{computation}
...
$F_g(\beta\gamma) = (-2)\cdot B_{2g}/(2g(2g-2))$, and the
...
\item $\kappa = 3(2k+1)/(k+3) - 26/2 + \ldots$
  (the precise formula depends on the normalisation of the
  Sugawara construction for the non-principal embedding).
...
(Conjecture~\ref{conj:non-principal-koszul-ds-commutation}).
\end{computation}
...
\noindent
(ii)~From~(i) by $\kappa=(c-c_{\mathrm{crit}})/2$ applied
to each factor.

\noindent
(iii)~The genus-$g$ free energy is
$F_g=\kappa\cdot B_{2g}/(2g(2g-2))$ at the scalar level
