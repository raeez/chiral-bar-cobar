...
This section assembles the holographic content of
the monograph into a single theorematic package and extends it past
the linear boundary.
The proved modular Koszul core supplies:
...
\end{enumerate}
These ingredients yield a mathematically complete
\emph{protected holographic calculus}, extracted in the first part of the section.  The second part adds a genuswise
...
The proved claims comprise the full protected holographic calculus:
reconstruction, transposition, cotangent realization, fixed-point
...
YBE, shadow = KZ/BPZ, quartic = $L_\infty$ obstruction), the
...
reduction.  In every case the algebraic content is entirely proved;
...
$\cA^!_{\mathrm{univ}} \simeq \Omega(\barBch(\cA^!))$ is the
definition of Koszul duality:
Theorem~\ref{thm:bar-cobar-isomorphism-main} already identifies the
boundary algebra with the cobar of the Verdier-dual bar coalgebra.
The protected bulk reconstruction theorem
...
\begin{theorem}[Sphere reconstruction; \ClaimStatusProvedHere]
...
$\nabla^{\mathrm{hol}}_{0,n}$ recovers the BPZ conformal Ward
identities
...
of conformal blocks $\mathcal{V}_{0,n}(\cA;V_1,\dotsc,V_n)$.
...
For a general modular Koszul algebra~$\cA$, part~(ii) follows from
parts~(i) and~(iii): flatness of the shadow connection gives a local
system on~$\mathcal{M}_{0,n}$, and the shadow connection form
$\operatorname{Sh}_{0,n}(\Theta_\cA)$ generates the differential
equations whose horizontal sections are the genus-$0$ correlators
by the reconstruction principle of part~(iii).  The general Ward identity
is thus a \emph{consequence} of the modular shadow construction, not an
independent input.
...
match above is the first instance of a general reconstruction theorem
from modular shadow data.
\end{remark}
...
Appendix~\textup{\ref{app:nonlinear-modular-shadows}}.
Moreover, $R^{\mathrm{mod}}_{4,g,n}(\cA)$ is canonically identified
...
vanishes, and the dual shadow tower collapses.
...
The Hessian eigenvalue $(c-26)/2$ (the deviation of the curvature
$c/2$ from the complementarity midpoint~$13$) gives
...
it is established for the entire standard landscape by the
general HS-sewing theorem
