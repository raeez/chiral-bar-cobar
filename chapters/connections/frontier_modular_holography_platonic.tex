...
(vanishing at $k=0$ per AP126) is the classical
...
\item \textup{($k_{\max}=0$: trivial Hamiltonians.)}
  ...
  trivial: there is no nonzero contribution at arity~$2$.
...
$r_{\max}=4$, and the higher-arity shadow data arises from composite-field
...
the shadow obstruction tower.  Arity controls the order of the shadow
...
collapse the dual higher-arity shadow obstruction tower.
...
\end{equation}
the smallest arity at which the shadow obstruction tower terminates.
\end{definition}
...
\begin{proof}
The shadow obstruction tower at arity~$r$ involves the transferred
...
shadow obstruction tower of an abelian envelope terminates at arity~$2$:
\[
...
\item \emph{Closed string.}
  The genus-$g$, arity-$n$ shadow amplitudes
  ...
  is the genus-$1$ shadow at arity~$1$.
...
shadow amplitude at genus~$g$, arity~$n$.
...
\item \emph{Gauge theory.}
  ...
  $r_{ab} = f^c_{ab}\,J_c$ \textup{(}a constant:
  by AP19, $d\log$ extraction of the simple-pole OPE
  $f^c_{ab}J_c/(z{-}w)$ yields a bar element of pole
  order zero\textup{)}.
  The celestial R-matrix is the exponential of this
...
which is the arity-$0$ projection of the shadow complex
...
\begin{enumerate}[label=\textup{(\roman*)}]
\item \emph{Instanton number $=$ bar arity.}
The $k$-instanton sector of the Nekrasov partition function
$Z = \sum_{k \geq 0} q^k Z_k$ corresponds to the arity-$k$
...
\item \emph{D3 brane \textup{(}twisted $\mathcal{N} = 4$ SYM\textup{)}.}
  ...
  $k=1$; the level-prefixed form $k\Omega/z$ vanishes at $k=0$
  per AP126; $\Omega$ the Casimir element), and the genus expansion
...
$\mathrm{Res}^{\mathrm{coll}}_{0,2}(\Theta_\cA)$ (vanishing at $k=0$
per AP126; at the D3 level $k=1$ this reduces to $\Omega/z$):
the current algebra OPE
$J_a(z)J_b(w) \sim k\delta_{ab}/(z{-}w)^2 +
f^c_{ab}J_c/(z{-}w)$
has at most a double pole; the $d\log$ extraction
(AP19) gives a simple-pole r-matrix $r_{ab}(z) = k\Omega_{ab}/z$.
...
\item \emph{Complementarity sum.}
  ...
  \end{equation}
  which is nonzero for all $N \geq 1$, confirming AP24.
...
\item \emph{Irreducible anomaly.}
  ...
  \textup{(}AP24: contrast with the anti-symmetric case
  $\kappa + \kappa^! = 0$ for affine Kac--Moody\textup{)}.

\item \emph{$N = 2$: unique physical point.}
...
$\mathrm{obs}_g = \kappa(\cA) \cdot \lambda_g$ on
$\overline{\cM}_g$ (UNIFORM-WEIGHT) and determines the
...
\item Class~$C$ (shadow depth $r = 4$; $\beta\gamma$ systems):
  the tower terminates at arity~$4$.
...
the conductor is preserved:
$K(\cA^!) = \kappa(\cA^!) + \kappa((\cA^!)^!) = \kappa(\cA^!) + \kappa(\cA) = K(\cA)$.
The value of~$K$ is family-specific:
$K = 0$ for affine Kac--Moody, Heisenberg, and free-field algebras;
...
$W_3$ at $K = 250/3$, Bershadsky--Polyakov at $K = 196$) measures the
