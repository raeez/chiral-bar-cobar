...
the eight-fold \emph{completed modular Koszul datum}
$\Pi^{\mathrm{oc}}_X(\cA)$
...
sewing. The metaplectic correction encodes the interaction of determinant-line geometry, Weyl
quantization, and gluing topology.
\end{remark}
...
projections are the shadow obstruction tower. The holographic programme
amounts to applying a single functor (the \emph{holographic shadow
functor}) to this MC element: the full bulk/boundary
package is derived output, not assembled input.
...
\item \emph{Verdier/Koszul-dual branch.}
 $\cA^!$ is the strict Verdier/Koszul dual of~$\cA$ on the modular
 Koszul locus. Off that locus the canonical object supplied by the
 bar/Verdier formalism is the factorization companion
 $\cA^{!}_\infty$, and the passage
 $\cA^{!}_\infty \to \cA^!$ is exactly the strict bar-cobar inversion
 of Theorem~B on the Koszul locus and is conjectural in general.
\item \emph{Bulk/line comparison slot.}
 ...
 global-triangle problem. The base curve~$X$ is ambient data and is not
 counted as a slot of~$\mathcal{H}(T)$.
...
strict assembly of the six slots above into a single HT holographic
package. That assembly is proved on the modular Koszul / boundary-linear
/ DS comparison lanes recorded in Volume~I and Volume~II, and is
conjectural as a uniform statement for arbitrary HT inputs.
...
holographic modular Koszul datum $\mathcal{H}(T)$ is the physics-side name for $M(\cA)$; the completion kinematics is its algebraic backbone.
\end{remark}

\begin{remark}[Frontier DNA: bi-based Ran-space factorisation datum for $K3 \times E$ and the Kodaira-nearby-cycles averaging]
\label{rem:frontier-DNA-bi-based-ran-K3E}
\ClaimStatusConjectured
...
The holographic datum $\mathcal{H}(T)$ extends to a
...
\item \emph{Parameter base.}
 ...
 in the Kodaira list).
\end{enumerate}
The averaging morphism on the bi-based datum factors as
\begin{equation}\label{eq:frontier-DNA-bi-based-averaging}
\operatorname{av}
\;=\;
...
group~$\Gamma_{K3} \subset O(2,19)$. The composite~\eqref{eq:frontier-DNA-bi-based-averaging}
...
$\Theta_\cA$~lives. The bi-based assembly~$(\operatorname{Ran}, \overline{A_2})$
makes the K3-coupled chiral algebra into a genuine bulk/boundary
factorisation $\infty$-system, with the holographic
datum~$\mathcal{H}(T)$ of Definition~\ref{def:holographic-modular-koszul-datum}
arising as the fibre at a generic basepoint of $\overline{A_2}$ over
the smooth Kodaira locus. The construction is conjectural as a
uniform statement and is proved on the modular Koszul locus on each
Kodaira chamber of $\overline{A_2}$ via the Francis--Gaitsgory
factorisation $\infty$-category~\cite{FG12}; the global gluing along
the regular-singular hyperplane arrangement $H_1 \cup H_4$ is the
content of the bi-based Ran datum.
\end{remark}
...
\emph{the dg-shifted Yangian is the
...
or degree truncation of the convolution algebra~$\gAmod$.
...
\begin{remark}[Frontier DNA: the Hall--Drinfeld double is not a Yangian; Manin-pair signature obstruction and Schiffmann--Vasserot shuffle]
\label{rem:frontier-DNA-hall-drinfeld-vs-yangian}
\ClaimStatusProvedHere
...
\item \emph{No Lagrangian decomposition.}
 ...
 signature $(n,n)$. On our side the obstruction class
 $\varkappa(\cA) = \kappa(\cA) + \kappa(\cA^!)$
 \textup{(}see Remark~\ref{rem:shadow-tower-tqft-taylor}\textup{)} is
 precisely the obstruction to such a Lagrangian splitting:
...
The point is fully proved at the level of the Manin pair and at the
level of the obstruction class; the explicit dictionary between the
...
\item \textup{(Collision-depth expansion.)}
 ...
 \end{equation}
 where $k_{\max} = p_{\max}-1$ \textup{(}AP\textup{19} d\,log absorption,
 see Chapter~\textup{\ref{ch:three-invariants})}.
...
\item \textup{($\cW_N$ prediction.)}
 For $\cA=\cW_N$, $k_{\max}=2N-1$, and the $H_i$ are differential
 operators of order $2N-2$. The existence and commutativity follow
 from the MC equation; the explicit term-by-term comparison with
 GZ\textup{26} is the content of
 Remark~\textup{\ref{rem:gz26-wn-comparison-conjectural}}.
\end{enumerate}
\end{theorem}
...
\begin{theorem}[Gaudin--Yangian identification; \ClaimStatusProvedHere]
...
\ClaimStatusProvedHere{} on the new three-parameter identification;
classical Sklyanin/Drinfeld content is \ClaimStatusProvedElsewhere]
...
\textup{(}AP\textup{60}: status restricted to the new content\textup{)}.
...
AP\textup{59} \textup{(}three distinct invariants\textup{)}.
\end{theorem}
...
\begin{remark}[Relation to GZ26 commuting differentials: historical note]
...
\hline
Boundary--defect & genus~0, full degree & Koszul duality $\cA \leftrightarrow \cA^!$\\
...
%: Fateev-Lukyanov formula. OLD WRONG: 2-24/(k+3) (simplified, linear).
% Verified: k=0 -> -30; k=1 -> -52.
...
%: c(aff) - c(W) is k-DEPENDENT with the correct FL formula.
% OLD WRONG: c_ghost = 2|Delta_+| (k-independent).
...
%: formula from wn_central_charge_canonical.py. Verified:
% k=0 -> c(W_2)=-2, c(W_3)=-30; k=1 -> c(W_2)=-7, c(W_3)=-52.
\[
...
\begin{remark}[SFT vertex identification; \ClaimStatusHeuristic]
\textup{(}LOCAL: scope fixed in this environment\textup{})
...
\item \emph{Closed string.}
 ...
 are the closed string field theory vertices
 $V_{g,n}$ of Zwiebach~\cite{Zwiebach93}:
...
The identification $V_{g,n} = \mathrm{Sh}_{g,n}(\Theta_\cA)$
follows from the modular operad structure: the CSFT vertices
are the string amplitudes on $\overline{\cM}_{g,n}$, and the
MC element encodes exactly these amplitudes through the
...
(Shenker, 1990).
The bar complex thus provides a convergent resummation
of the closed string field theory: the shadow partition
function converges where the string perturbation
series diverges.
\end{remark}
...
the off-shell algebraic framework in which Sen's
...
\item \emph{Conformally soft theorems.}
 The soft graviton theorem at sub${}^n$-leading order is
 a bar cohomology class of the
...
\begin{proof}
Part~(i): the current algebra $J_a(z)J_b(w) \sim
...
Part~(ii): the $\mathfrak{w}_{1+\infty}$ algebra has
...
Part~(iii): the soft theorem at order~$n$ is the statement
that bar cohomology $H^n(B(\mathfrak{w}_{1+\infty}))$
is nonzero at conformal dimension $\Delta = 1 - n$.
This follows from the structure of the
$\mathfrak{w}_{1+\infty}$ OPE and the
resulting spectral sequence on the bar complex.
...
\begin{proposition}[Chern--Simons from the shadow obstruction tower;
\ClaimStatusProvedHere]
...
\item \emph{Verlinde formula.}
 ...
 This is the dimension of $H^0$ of the factorization
 homology on~$\Sigma_g$, which is the object
 $\mathbf{Q}_g(\widehat{\mathfrak{g}}_k)$ in the shadow obstruction tower.

\item \emph{Genus-$1$ matching.}
 The genus-$1$ shadow amplitude
 $F_1 = \kappa/24$
 matches the perturbative Chern--Simons partition function
 $Z_1^{\mathrm{pert}} = \exp(-(3/2)\log(k{+}h^\vee))$
 up to the identification
 $\kappa/(24) = \dim(\mathfrak{g})(k{+}h^\vee)/(48h^\vee)$.
\end{enumerate}
\end{proposition}
...
determines the genus-$1$ contribution. The Verlinde
formula computes $\dim H^0$ of the factorization homology,
which is the degree-$0$ projection of the shadow complex
$\mathbf{Q}_g$. The $S^3$ partition function is the
genus-$0$ datum of the TQFT, controlled by the quantum
dimensions.
Computational verification:
...
In the factorization framework, both sides are shadows of the
...
\begin{theorem}[Twisted holography datum; \ClaimStatusProvedHere]
...
duality between a boundary chiral algebra~$\cA$ and
$\cA^!$. The holographic modular Koszul datum
\[
\mathcal{H}(\mathcal{T})
\;=\;
...
\begin{theorem}[ABJM holographic datum; \ClaimStatusProvedHere]
\label{thm:frontier-abjm}
\index{ABJM theory|textbf}
For ABJM theory with gauge group
...
\item \emph{Genus-$1$ free energy.}
 $F_1 = \kappa/24 = -(N^2+1)/24$.
 At large~$N$, $F_1 \sim -N^2/24$,
 matching the localization result for ABJM.
...
\begin{theorem}[M5 brane holographic datum; \ClaimStatusProvedHere]
...
\item \emph{Modular characteristic.}
 Using the Virasoro-line extraction $\kappa = c_{2\mathrm{d}}/2$:
 ...
 At $N = 1$: $\kappa = 0$ (free tensor multiplet).

\item \emph{Koszul dual.}
 The $W_\infty$ involution $\lambda \mapsto 2 - \lambda$ gives
 $\cA_{\mathrm{M5}}(N)^! = W_{1+\infty}[\lambda = 2 - N]$
 with Koszul dual modular characteristic
...
\item \emph{Complementarity sum.}
 ...
 \kappa(\mathrm{M5}_N) + \kappa(\mathrm{M5}_N^!)
 \;=\;
 \frac{4N^3 - 3N - 1}{2} + (2 - N)^3
 \;=\;
 \frac{2N^3 + 12N^2 - 27N + 15}{2},
 \end{equation}
 which is nonzero for all $N \geq 1$, confirming.
 At the leading-order level (matching the geometric $N^3$
 anomaly $\kappa_{\mathrm{lead}} = N^3$,
...
\cite{BRvR14-6d}. Verification: at $N = 1$, the formula gives
$c = 0$ (free tensor multiplet); at $N = 2$, $c = 25$.
...
$W_{1+\infty}$ is the Linshaw--Rapcak duality; the Koszul dual
characteristic follows from Theorem~C.
...
\begin{proposition}[Phantom M5 Koszul dual; \ClaimStatusProvedHere]
...
The Koszul dual $\mathrm{M5}_N^! = W_{1+\infty}[\lambda = 2 - N]$ is
a \emph{phantom} for $N \geq 3$: it satisfies
$\kappa(\mathrm{M5}_N^!) = (2-N)^3 < 0$, so the genus expansion
$F_g(\mathrm{M5}_N^!) = \kappa^! \cdot \lambda_g^{\mathrm{FP}} < 0$
\textup{(}UNIFORM-WEIGHT\textup{})
has the wrong sign at every genus, precluding a perturbative
interpretation as a unitary quantum field theory.
\begin{enumerate}[label=\textup{(\roman*)}]
\item \emph{Irreducible anomaly.}
 ...
 This is the W-algebra-type complementarity of
 Theorem~\textup{\ref{thm:complementarity}} \item \emph{$N = 2$: unique physical point.}
 At $N = 2$: $\lambda^! = 0$ and $\kappa(\mathrm{M5}_2^!) = 0$,
 so the Koszul dual is \emph{uncurved} (vanishing bar curvature).
 The dual algebra is
 $W_{1+\infty}[\lambda = 0] \cong \mathfrak{u}(\infty)_1$, the
 free boson tower (vacuum module of the affine Yangian). This is
 the unique value of~$N$ at which the phantom has a concrete
 physical realization as a vertex algebra with non-negative genus
 expansion.
...
\begin{proof}
The formula $\kappa(\mathrm{M5}_N^!) = (2 - N)^3$ follows from
part~(iv) of Theorem~\ref{thm:frontier-m5} via the Linshaw
involution $\lambda \mapsto 2 - \lambda$. For~(i): the identity
$N^3 + (2-N)^3 = 6N^2 - 12N + 8$ is verified by direct expansion,
and the quadratic $6x^2 - 12x + 8$ has negative discriminant
$\Delta = (-12)^2 - 4 \cdot 6 \cdot 8 = -48$, so it has no real
roots. For~(ii): $\kappa(\mathrm{M5}_2^!) = (2-2)^3 = 0$ and the
...
\begin{remark}[Frontier DNA: 4d class-$\mathcal S$ $A_1$ on $\Sigma_{0,24}$ as the
$\mathcal N{=}2$ parent of the K3-coupled M5 datum]
\label{rem:frontier-DNA-class-S-K3-parent}
\ClaimStatusProvedHere
...
Remark~\ref{rem:frontier-DNA-bi-based-ran-K3E}). The Chacaltana--Distler
...
\item \emph{Conformal anomalies.}
 $c_{4\mathrm d}(A_1, \Sigma_{0,24}) = 26$ in the standard
 Chacaltana--Distler normalisation
 \textup{(}sum of $a$-anomaly contributions per regular puncture
 plus the bulk genus-zero contribution\textup{)}; the protected
 chiral subalgebra of Beem et al.~\cite{Beem-etal-2014}
 has $c_{2\mathrm d}(A_1, \Sigma_{0,24}) = -312$
 \textup{(}the standard $c_{2\mathrm d} = -12 \cdot c_{4\mathrm d}$
 ratio gives $-312$, confirmed by the puncture sum:
 $24$ punctures times the per-puncture
 $-13$ contribution recovers $-312$\textup{)}.
...
\item \emph{Steiner-system rigidity.}
 ...
 underlying the Mathieu group $M_{24}$\textup{)}: the $759$ blocks
 of $S(5,8,24)$ are exactly the $759$ Coulomb-branch flat
 directions on which the chiral subalgebra has a free-field
 reduction, in agreement with the $K3$-Mukai-lattice computation
 of $\#\{\text{rank-}8\text{ even unimodular sublattices}\} = 759$
 sub-lattices that survive the Borcherds
 lift~\cite{Borcherds1995}.
\end{enumerate}
The M5 datum $\mathcal{A}_{\mathrm{M5}}(2) = W_{1+\infty}[\lambda{=}2]$
of Theorem~\ref{thm:frontier-m5} is the
$2\mathrm d$ chiral image of the
$4\mathrm d$ class-$\mathcal S$ $A_1$ theory on $\Sigma_{0,24}$
under the Beem--Lemos--Liendo--Peelaers--Rastelli--van~Rees
protected-subalgebra
functor~\cite{Beem-etal-2014}: the $4\mathrm d$ Coulomb dimension $21$
and the $2\mathrm d$ central charge $c_{2\mathrm d} = -312$ are
related by the protected-subalgebra
formula $c_{2\mathrm d} = -12\,c_{4\mathrm d} \cdot
(\dim\operatorname{Coul}/\dim\operatorname{Coul}_{\mathrm{trivial}})$
which reproduces the Chacaltana--Distler datum on the nose.
The Steiner-system rigidity fixes the configuration as
\emph{combinatorially unique}: among all genus-zero $A_1$ theories
with $24$ punctures, only the $S(5,8,24)$-symmetric arrangement
admits a coupling to the K3 boundary M5 datum compatible with the
$M_{24}$-equivariant Borcherds lift.
This is the four-dimensional $\mathcal N{=}2$ origin of the
M5 boundary datum at $N=2$ on the K3-coupled
branch.
\end{remark}

\begin{remark}[Frontier DNA: $\Delta_5 = 1$-loop output of twisted 11D~SUGRA on $K3 \times T^2$ with 24 M5 on $I_1$ Kodaira fibres]
\label{rem:frontier-DNA-Delta5-onel-loop-11d-K3T2}
\ClaimStatusConjectured
...
The fifth-shadow obstruction $\Delta_5(\cA)$ on the M5 boundary
datum on the K3-coupled branch is the
$1$-loop-forced output of twisted eleven-dimensional supergravity
on the eleven-manifold
\begin{equation}\label{eq:frontier-DNA-Delta5-eleven-manifold}
\mathcal Y_{11}
\;=\;
...
\begin{equation}\label{eq:frontier-DNA-Delta5-weight-decomposition}
\boxed{\;
\Delta_5 \;=\; 2 \;+\; 3,\;}
\end{equation}
...
The total weight $5 = 2 + 3$ matches the universal
shadow-tower depth of the K3-coupled M5 datum, with the $2 + 3$
decomposition mirroring the bulk/boundary split of the twisted
...
shadow at depth~$3$. The identification is forced by the
$1$-loop twist-cancellation theorem of Costello--Paquette--Sharma
~\cite{CPS2306} once the K3 boundary geometry is restored, and is
proved at one loop in the simplified single-fibre setting of
Bittleston--Costello~\cite{BittlestonCostello25}; the
$24$-fold version of~\eqref{eq:frontier-DNA-Delta5-weight-decomposition}
is conjectural in general and is the $11$D-SUGRA evidence for the
K3-enlarged Theorem~C bucket of
Remark~\ref{rem:frontier-DNA-K3-bucket-enlargement}.
\end{remark}
...
The seven standard holographic data of the proved core
are:
\begin{center}
\renewcommand{\arraystretch}{1.25}
\small
...
\end{tabular}
\end{center}
Each datum is fully determined by the boundary VOA~$\cA$
and the universal MC element~$\Theta_\cA$.
\end{remark}
...
no computation beyond height~$1$ has been verified.
\end{remark}
...
\item Class~$G$ (shadow depth $r = 2$; Heisenberg):
 ...
 The associated field theory is a finite cobordism theory;
...
\item Class~$L$ (shadow depth $r = 3$; affine Kac--Moody):
 ...
 The field theory extends through affine cobordisms,
...
$\kappa(\cA) = \kappa(\cA^!)$
that forces all odd-parity obstruction classes to vanish.
The resulting field theory is Euler-type:
the partition function $Z_\cA(\Sigma_g)$ is multiplicative
under cutting and gluing along separating curves,
because the self-duality eliminates the cross-channel
corrections $\delta F_g^{\mathrm{cross}}$
that obstruct multiplicativity in the general case.
...
The cobordism hypothesis~\cite{Lurie09TFT} sharpens this picture
from heuristic to forced.
...
From this vantage the scalar complementarity $\varkappa(\cA)$ is the obstruction
to the point-value being self-conjugate under the Verdier involution
$\cA \leftrightarrow \cA^!$; its vanishing ($\varkappa = 0$ for affine
Kac--Moody and free-field families) means the TQFT is
\emph{orientably self-dual}, while $\varkappa \neq 0$ (Virasoro at $\varkappa = 13$,
$W_3$ at $\varkappa = 250/3$, Bershadsky--Polyakov at $\varkappa = 98/3$) measures the
failure of orientation-reversal symmetry in the cobordism functor.
The cobordism hypothesis leaves no room for an alternative reading:
any symmetric monoidal functor from the cobordism category to the
target $(\infty,n)$-category is determined by the algebraic
structure of the point-value, and the shadow tower is precisely the
data that specifies that structure.
\end{remark}

\begin{remark}[Frontier DNA: K3 enlarges the Theorem C bucket to $\{0,\;8,\;13,\;250/3,\;98/3\}$]
\label{rem:frontier-DNA-K3-bucket-enlargement}
\ClaimStatusConjectured
...
$4$-dimensional; its two positive generators supply
...
\item \emph{Mukai-lattice face.}
 $c_+(\widetilde{H}(K3)) = 2$ and the Koszul conductor reads
 $K^{\kappa_{\mathrm{ch}}} = 2 c_+ = 8$ on the bar-cobar side,
 matching the $24$ Euler number of~$K3$ via the factorization
 $24 = 8 + 16$ where the $16 = 2\cdot 8$ contribution is the
 symmetric square of the positive-definite direction.
...
\end{enumerate}
The three faces unify under Bruinier's Heegner Chern-class
reciprocity \textup{(}regularized theta-integrals $=$ divisor-class
pullbacks on modular towers\textup{)}: in every normalization the
chiral Koszul characteristic reads $\varkappa = 8$ on the $K3$-coupled
family. The Theorem~C bucket is therefore enlarged to
\begin{equation}\label{eq:frontier-DNA-K3-bucket}
\varkappa(\cA)
\;\in\;
...
($\varkappa = 13$). The enlargement is conjectural as a
bar-cobar theorem on the full Vol~I core, but proved on the
Koszul-exact automorphic locus at the level of Borcherds products
\textup{(}\cite{Borcherds1995}, see~\cite[Ch.~4]{Borcherds92}\textup{)}.
\end{remark}

\begin{remark}[Frontier DNA: universal $\Phi$-trace identity bridging Vol~I conductor and Vol~III BKM~$\kappa$]
\label{rem:frontier-DNA-universal-phi-trace}
\ClaimStatusConjectured
...
supertrace of the bar--cobar ghost system; on the Vol~III side the
Borcherds--Kac--Moody characteristic of a Lorentzian lattice BKM
algebra~$\mathfrak g_\Lambda$ is the zero-Fourier coefficient of the
automorphic product attached to~$\Lambda$:
$\kappa_{\mathrm{BKM}}(\mathfrak g_\Lambda) = c_\Lambda(0)/2$ where
...
nearby-cycles to the Kodaira-degenerate locus. The
\emph{universal $\Phi$-trace identity} predicts
\begin{equation}\label{eq:frontier-DNA-phi-trace}
\boxed{\;
K(\Phi(Y))
\;=\;
...
\operatorname{tr}_{\cZ^{\mathrm{der}}_{\mathrm{ch}}(\Phi(Y))}
 \!\bigl(K_{\Phi(Y)}\bigr)
\;\xlongequal{\;\Phi\;}\;
2\,\kappa_{\mathrm{BKM}}(\mathfrak g_\Lambda)
\;=\;
c_\Lambda(0),
\;}
\end{equation}
where the left-hand equality is the definition of the chiral
conductor as a derived-centre supertrace
...
middle equality is the Vol~I ghost-cancellation theorem
\textup{(}subsumed in Remark~\ref{rem:shadow-tower-tqft-taylor}\textup{)},
and the right-hand equality is the Borcherds--pushforward~$\Phi$
that sends the bar-cobar supertrace to the zero-Fourier coefficient
of the automorphic lift. Concretely, for $\Lambda = K3$-Mukai the
identity reads $K(\Phi(K3)) = 8 = c_\Lambda(0)$ in agreement with
Remark~\ref{rem:frontier-DNA-K3-bucket-enlargement}; for
$\Lambda = \text{II}_{1,1}$ the identity reduces to the Heisenberg
...
\end{tabular}
\end{center}
The identity~\eqref{eq:frontier-DNA-phi-trace} sharpens the eight-fold
holographic datum~$\Pi^{\mathrm{oc}}_X(\cA)$ on the K3-coupled branch:
the sixth entry $\kappa(\cA) + \kappa(\cA^!)$ of the complementarity
bucket equals the zero-Fourier coefficient of the automorphic lift,
and the entire Vol~I bar-cobar supertrace calculus pushes forward
across~$\Phi$ to the Vol~III BKM side. The equality is proved on the
Borcherds-lift-exact locus (Lorentzian Heegner lattices of signature
$(2,n)$) and conjectural in general; no counterexample is known.
\end{remark}

\section{Wave 13 DNA perspective}
\label{sec:fmhp-wave13-DNA}

\begin{remark}[Wave 13 holographic datum]
\label{rem:fmhp-DNA-1}
The platonic holographic programme
$\mathcal H(T) = (A, A^!, C, r(z), \Theta_A, \nabla^{\mathrm{hol}})$
achieves its K3 realisation at the Wave~13 bialgebra $\mathbf H_{\Delta_5}$
via the data
\[
\begin{array}{rcl}
A & = & \mathbf H_{\Delta_5}, \\
A^! & = & \Omega_X\bigl(B_{\mathrm{ch}}(\mathbf H_{\Delta_5})\bigr) \simeq \mathbf H_{\Delta_5}\ \text{at}\ \hbar^2=-1/8, \\
C & = & B_{\mathrm{ch}}(\mathbf H_{\Delta_5}), \\
...
\end{array}
\]
The four recovery theorems of the holographic programme (from $\mathcal H(T)$
one reconstructs $T$'s ordered bar complex, its Koszul dual, its
$r$-matrix, and its universal MC element) all hold at the Wave~13 base
point on the Lusztig locus $\hbar^2 = -1/8$. Cross-ref Vol.~III,
Chapter~\ref{ch:k3-chiral-bialgebra-platonic}.
\end{remark}

\begin{remark}[Global holographic triangle on the B-family]
\label{rem:fmhp-DNA-2}
...
B-family at Wave~13 through the Bruinier 2002 Prop.~5.1 Heegner
Chern-class reciprocity: the $\mathbb Z/8$-class $[L^{\Delta_5}]\in\mathrm{CH}^1(H_1)$
is simultaneously the boundary modular holonomy (Mukai doubling), the
bulk monodromy (Humbert $H_1$), and the line fusion determinant
(Lusztig $\ell = 8$). At the Lusztig specialisation $\hbar^2 = -1/8$
the boundary-linear bulk is \emph{proved} (Theorem~H, concentration
in degrees $\{0,1,2\}$), while the global triangle remains conjectural
beyond the Koszul locus. See Vol.~III,
Chapter~\ref{ch:k3-chiral-bialgebra-platonic},
\S\ref{sec:four-recovery-theorems-at-wave13}.
\end{remark}
