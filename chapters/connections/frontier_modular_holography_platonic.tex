...
geometry of the SFT overlap conditions\textup{)}:
\begin{enumerate}[label=\textup{(\roman*)}]
...
\texttt{compute/lib/string\_field\_vertices.py}.
\end{proof}
...
\textbf{Shadow tower} & \textbf{Seiberg--Witten} \\
\midrule
...
\begin{proposition}[Chern--Simons from the shadow tower;
\ClaimStatusProvedHere]
...
\item \emph{Verlinde formula.}
  ...
  $\mathbf{Q}_g(\widehat{\mathfrak{g}}_k)$ in the shadow tower.
...
% RECTIFICATION-FLAG: The BRST reduction may shift the total
% by ghost/matter cancellation.  The naive additive formula above
% does not account for BRST ghosts, which may contribute finite
% corrections at small N.  At large N the CS contribution N^2-1
% is subleading relative to the matter contribution -2N^2.
...
(Katz~\cite{Katz90}).  The shadow tower captures
...
The six standard holographic data of the proved core
are:
\begin{center}
\renewcommand{\arraystretch}{1.25}
\small
