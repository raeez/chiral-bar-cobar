...
$c_{\mathrm{aff}} = c_{W_N} + c_{\mathrm{ghost}}$
with $c_{\mathrm{ghost}} = 2|\Delta^+(\mathfrak{sl}_N)|
= N(N-1)$.
For $N=3$: $c_{\mathrm{ghost}}=6$, verified in the preface.
The genus-$g$ partition function decomposes
$F_g(\widehat{\mathfrak{sl}}_N{}_k)
= F_g(\mathcal W_N{}_{c(k)}) + F_g(\mathrm{ghost})$
at every genus by functoriality of the graph sum.
\end{construction}
...
For principal DS reduction of $\widehat{\mathfrak{sl}}_N$ at
level~$k$:
...
c_{\mathrm{ghost}},
\qquad
c_{\mathrm{ghost}}\;=\;N(N-1).
\end{equation}
The modular characteristics satisfy
\begin{equation}\label{eq:ds-kappa-decomposition}
\kappa(\widehat{\mathfrak{sl}}_N{}_k)
\;=\;
\kappa(\mathcal W_N{}_{c(k)})
\;+\;
\kappa(\mathrm{ghost}),
\end{equation}
and the genus-$g$ free energies decompose:
...
\begin{proof}
The ghost system for principal DS of $\mathfrak{sl}_N$ is a
product of $bc$-systems, one for each positive root
$\alpha\in\Delta^+(\mathfrak{sl}_N)$.  The ghost central charge
is $c_{\mathrm{ghost}}=\sum_{\alpha>0}(-2(6j_\alpha^2-6j_\alpha+1))$
where $j_\alpha$ is the conformal weight of the ghost at
root~$\alpha$.  For principal embedding, the positive roots of
$\mathfrak{sl}_N$ have heights $1,1,\dots,2,2,\dots,N-1$ and
the sum evaluates to $2|\Delta^+|=N(N-1)$.  Direct verification:
$c_{\mathrm{aff}}=(N^2-1)k/(k+N)$,\;
$c_{W_N}=(N-1)(1-N(N+1)/(k+N))$,\;
$c_{\mathrm{aff}}-c_{W_N}
=((N^2-1)k-(N-1)(k+N)+N(N-1)(N+1))/(k+N)
=N(N-1)$.
The genus-$g$ decomposition follows from the additivity of the
graph sum~\eqref{eq:preface-theta} under tensor products
...
\end{tabular}
\end{center}
For $\mathrm{Vir}_{26}$: the bosonic string is anomaly-free.
For $\mathcal W_3{}_{50}$: the $\mathcal W_3$ string is
anomaly-free.  For $\widehat{\mathfrak g}_0$: the WZW model
at level~$0$ has trivial genus tower.
\end{corollary}

\begin{proof}
$\nabla^{\mathrm{hol}}=d-\kappa\omega_g-\cdots$; flatness at
leading order requires $\kappa=0$.  Substitution of the
computed $\kappa$-values from
Computations~\ref{comp:virasoro-holographic-datum},
