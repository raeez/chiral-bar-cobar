...
\emph{protected transform calculus} on the proved core. The
...
characteristics, anomaly towers, and reciprocity laws. The
...
\cite{BRvR14-6d}. Check: at $N = 1$, the formula gives
...
\item \emph{Conformal anomalies.}
 ...
 the intermediate value $(c_{4d}, c_{2d}) = (26, -312)$ is false
 by Vol~I Section~\ref{sec:thqg-open-closed-realization}\textup{)}.
...
no computation beyond height~ available.
\end{remark}
...
of the automorphic lift. Concretely, on the paramodular
...
on the Borcherds-lift-exact surface, the sixth entry
$\kappa(\cA)+\kappa(\cA^!)$ of the complementarity bucket is compared
with the zero-Fourier coefficient of the automorphic lift, and the
Vol~I bar-cobar supertrace calculus pushes forward across~$\Phi$ to
the Vol~III BKM side. The equality is proved on the
Borcherds-lift-exact locus (Lorentzian Heegner lattices of signature
$(2,n)$) and conjectural in general; outside that locus the comparison
is an explicit proof obligation, not a scalar identity.
\end{remark}
...
\begin{array}{rcl}
A & = & \mathbf H_{\Delta_5}, \\
A^! & = & \mathbb D_{\Ran}B_{\mathrm{ch}}(\mathbf H_{\Delta_5})
 \simeq \mathbf H_{\Delta_5}\ \text{on the self-dual locus }
 \hbar^2=-1/8, \\
C & = & B_{\mathrm{ch}}(\mathbf H_{\Delta_5}), \\
\Omega_X(C) & \simeq & \mathbf H_{\Delta_5}
