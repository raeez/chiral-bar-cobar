...
genuine $3$d HT bulk (T18(iii)) factorization algebra, \textup{(b)}~the
...
$\cZ^{\mathrm{der}}_{\mathrm{ch}}(\cA)$ (T18(i): universal
chain-level bulk), the
...
\item \emph{Conformal anomalies.}
 ...
 the intermediate value $(c_{4d}, c_{2d}) = (26, -312)$ is
 retracted per Vol~I
...
$24$-fold version of~\eqref{eq:frontier-Delta5-weight-decomposition}
...
Remark~\ref{rem:frontier-K3-bucket-enlargement}; for
...
\end{tabular}
\end{center}
The identity~\eqref{eq:frontier-phi-trace} sharpens the eight-fold
...
achieves its K3 realisation at the bialgebra $\mathbf H_{\Delta_5}$
via the data
\[
\begin{array}{rcl}
A & = & \mathbf H_{\Delta_5}, \\
...
$r$-matrix, and its universal MC element) all hold at the K3 base
...
B-family through the Bruinier 2002 Prop.~5.1 Heegner
...
beyond the Koszul locus. The $\kappa$~indexing on the two sides of
the triangle is reconciled by
...
evaluating to $12 = 2\cdot 5 + 0 + 2$ on $K3 \times E$; the
$+2$ is the bar--cobar double-parity correction and is the
quantitative signature that, at the Lusztig specialisation, the two
indexings are bulk-boundary projections of the same Drinfeld double
$U_\cA = \cA \bowtie \cA^{!}_\infty$. The class~$\mathsf{M}$ residue
left open by the boundary-linear argument closes at chain level via
the DS--Hochschild bridge of Vol.~II
(Corollary~\ref{thm:class-M-chain-bulk}); the residue beyond the
