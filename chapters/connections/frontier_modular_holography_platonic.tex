...
\item \textbf{H/M-level.}
 ...
 $\mathbf C_g(\cA)=R\Gamma(\overline{\mathcal M}_g,\mathcal Z(\cA))$
 \textup{(}the $\mathrm C_0$ strict-flat fibre-centre comparison
 identifies the degree-zero strict-flat fibre with the ordinary
 centre local system~$\mathcal Z(\cA)$; the brace comparison
 $\iota_Z^{\mathrm{der}}$ relates the Theorem~H derived centre to
 this object only after $H^0$ and that flat fibre-centre
 comparison\textup{)}, define
...
+\mathrm{separatedness};
\mathrm{C_0\ strict\text{-}flat\ centre\ models}
+\mathrm{C_{1a}\ genuine\text{-}partner\ datum\ for\ parts\ (iii)-(iv)};
\mathrm{all\ genera})$
...
\item \emph{H/M-level cofiber identification.}
 Supply the $\mathrm C_0$ strict-flat centre models and the
 $\mathrm C_{1a}$ genuine-partner datum
 \textup{(}Convention~\textup{\ref{conv:hgmk-theorem-c-stable-consumer}}\textup{)}.
 Then for every $g\ge 0$,
 \begin{equation}\label{eq:frontier-holographic-cofiber-identification}
 \mathbf H^{\mathrm{hol}}_g(\cA)
 \simeq
 (\chi^-_{\cA;g})^{-1}\mathbf Q_g(\cA^!),
 \end{equation}
 where $\mathbf Q_g(\cA^!)$ is the positive eigensummand in
 $\mathbf C_g(\cA^!)$ and $(\chi^-_{\cA;g})^{-1}$ is its
 genuine-partner realization in the $\cA$-ambient complex.
\item \emph{S-level shadow equality.}
 Under the same $\mathrm C_0+\mathrm C_{1a}$ package, for every
 $g\ge 0$,
...
For the cofiber statement, the $\mathrm C_0$ strict-flat centre
models and the $\mathrm C_{1a}$ genuine-partner datum supply the
involution splitting in the $\cA$-ambient complex,
\[
\mathbf C_g(\cA)
\simeq
\mathbf Q_g(\cA)\oplus (\chi^-_{\cA;g})^{-1}\mathbf Q_g(\cA^!),
\]
with $\mathbf Q_g(\cA^!)$ the positive eigensummand in
$\mathbf C_g(\cA^!)$.
...
Theorem~\ref{thm:frontier-protected-bulk-antiinvolution} uses the
bar--cobar/Verdier core together with the $\mathrm C_0$ strict-flat
centre models and the $\mathrm C_{1a}$ genuine-partner datum; parts
(iii)--(iv) are conditional on that package. It does \emph{not}
...
\end{equation}
By Theorem~\ref{thm:quantum-complementarity-main}, under the
$\mathrm C_{1b}$ perfectness and nondegenerate anti-invariant pairing
package, there is a perfect duality
...
\mathrm{C_0}+\mathrm{C_{1a}\ genuine\text{-}partner\ datum};
\mathrm{C_{1b}\ perfectness/nondegeneracy};
...
\begin{proof}
Under the $\mathrm C_0+\mathrm C_{1a}$ package the decomposition
theorem gives, in the $\cA$-ambient complex,
\[
\mathbf C_g(\cA)
\simeq
\mathbf Q_g(\cA)\oplus (\chi^-_{\cA;g})^{-1}\mathbf Q_g(\cA^!),
\]
with $\mathbf Q_g(\cA^!)$ the positive eigensummand in
$\mathbf C_g(\cA^!)$; we suppress the genuine-partner realization
$(\chi^-_{\cA;g})^{-1}$ from the notation in the remaining displays.
By definition,
$W_g(\cA)=\mathbf Q_g(\cA^!)[d_g]$, hence, under the $\mathrm C_{1b}$
duality~\eqref{eq:frontier-vg-wg-duality},
...
\item If $(\alpha,\beta)=(0,1)$, then every self-holographic fixed point is
 ...
 \quad \forall g\ge 1.
 \end{equation}
\item More generally, the genus-$1$ affine law propagates through $H_D^K$:
 \begin{equation}\label{eq:frontier-all-genus-scalar-propagation}
 \operatorname{ch}_g(\mathfrak O_g^K(\cA^!))=
 (-1)^g\bigl(\alpha-\beta\kappa(\cA)\bigr)\lambda_g.
 \end{equation}
\end{enumerate}
\end{theorem}
...
exactly the assumed $H_D^K$ package:
\[
\operatorname{ch}_g(\mathfrak O_g^K(\cA^!))
=(-1)^g\kappa(\cA^!)\lambda_g
=(-1)^g(\alpha-\beta\kappa(\cA))\lambda_g.
\]
...
\begin{convention}[Quadratic Gaussian sector; \ClaimStatusDefinitional]
...
\item \emph{H-level nonlinear genus control.}
 ...
 Its scalar boundary shadow is the signed Hodge tower
 $\sum_{g\ge 1}(-1)^g\kappa(\cA)\lambda_g$ on the scalar lane
 \textup{(}Hodge shadow, sign $(-1)^g$, under $H_D^K$\textup{)}, and its
...
\begin{remark}[Completed-datum summary]
...
\item \emph{Quadratic coalgebra.}
 ...
 $q_\cA\colon\cA^{\mathrm i}\to B^{\mathrm{ch}}(\cA)$.
 On the Koszul locus, $H^\bullet(q_\cA)$ computes bar cohomology from
 this presentation model.
\item \emph{Verdier/Koszul-dual branch.}
 $K_X(\cA)=\mathbb D_{\Ran}B^{\mathrm{ch}}(\cA)$ is the Verdier
 algebra, and $\cA^!$ is the chosen strict target of
 $\nu_\cA\colon K_X(\cA)\to\cA^!$ on the finite-type modular Koszul
 locus, or from the corresponding
...
\item \emph{Modular Maurer--Cartan class.}
 ...
 Theorem~\ref{thm:mc2-bar-intrinsic}). Its scalar shadow is the
 Hodge character
 $(-1)^g\kappa(\cA)\lambda_g$ \textup{(}sign $(-1)^g$, under
 $H_D^K$; Theorem~\ref{thm:genus-universality}\textup{)},
...
\begin{proposition}[Seven-slot holographic/celestial scope gate;
\ClaimStatusConditional]
\label{prop:frontier-seven-slot-scope-gate}
\index{holographic datum!seven-slot scope gate}
\index{celestial comparison!scope gate}
\index{AdS/CFT!unsupported identification excluded}
\textup{[}Type signature:
$(\mathrm{Open/HT},\mathrm{seven\text{-}slot\ tuple\ with\ maps},
\mathrm{levels}=1\to5,
\mathrm{hypothesis\ package}=\mathrm{boundary\ algebra}
\ +\ \mathrm{bar/Verdier\ dual}
\ +\ \mathrm{line\ category}
\ +\ \mathrm{OCA}
\ +\ \mathrm{modular\ MC\ connection})$\textup{].}
Every theorem in this chapter is first an algebraic theorem about the
seven-slot tuple
\[
\mathcal H(T)=
\bigl(\cA,\cA^i,\cA^!,\mathcal C,r(z),\Theta_\cA,
\nabla^{\mathrm{hol}}_{\bullet,\bullet}\bigr)
\]
together with the displayed comparison maps.  Physical language
\textup{(}holographic, celestial, AdS/CFT, bulk reconstruction\textup{)}
is a corollary only after the relevant map has been constructed:
\[
\mathfrak{Bulk}(T)|_X\to Z^{\mathrm{der}}_{\mathrm{ch}}(\cA),
\qquad
\mathcal C_{\mathrm{line}}\to Z^{\mathrm{der}}_{\mathrm{ch}}(\cA),
\qquad
\mathrm{Cel}(T)\to B^{\mathrm{ch}}(\cA_{\mathrm{cel}}).
\]
The celestial algebra input is a chiral algebra
$\cA_{\mathrm{cel}}$ on the celestial sphere together with a
collinear comparison map from four-dimensional amplitudes to its OPE.
The celestial theorem is therefore scoped to factorization of
collinear OPEs through the bar complex; it is not an unsupported
AdS/CFT identification.

Reconstruction from the tuple means reconstructing, in order, the
boundary chart $\cA$, the quadratic coalgebra $\cA^{\mathrm i}$ and
full bar~$B_X(\cA)$, the strict or
completed Verdier/Koszul branch $\cA^!$, the line/closed comparison
slot $\mathcal C$, the binary line kernel $r(z)$, the modular MC class
$\Theta_\cA$, and the flat shadow connection
$\nabla^{\mathrm{hol}}_{\bullet,\bullet}$.  The first failure modes
are: no strict Verdier dual, no compact line-category comparison, no
OCA quasi-isomorphism, no collinear celestial comparison, no modular
flatness for the protected-state package, or no analytic completion
for the physical bulk.  When any of these maps is absent, the
corresponding physics statement is not asserted.
\end{proposition}
...
of the chiral theory: the Kodaira-nearby-cycles step
...
\;=\;K_\cA^{\mathrm{coll}}(z)
\in\End_\cA(2)\widehat\otimes\cO(*\Delta).
\end{equation}
The binary genus-zero residue is the ordered collision kernel.  A
represented classical $r$-matrix is
$r_\cA=\rho_\cA(K_\cA^{\mathrm{coll}})$ under
$H_{\mathrm{CYBE}}^{\mathrm{rep}}$.  A dg-shifted Yangian requires
the additional quantization/line-operator package $H_Y(\cA;W)$.
The trivalent collision residue
$\operatorname{Res}^{\mathrm{coll}}_{0,3}(\Theta_\cA)$
encodes the operadic three-insertion relation; under $H_Y$ its image
is the $A_\infty$ Yang--Baxter relation:
...
\end{enumerate}
The collision claim used below is that
$K_\cA^{\mathrm{coll}}$ is the binary genus-zero projection of the
ordered MC class.  Its represented Yangian image belongs to the
package $H_Y$. The 2026 exact sphere-correlator
...
typed projections of the bar-intrinsic class~$\Theta_\cA$.
Theorem~D carries the native deformation class, its K-theoretic
image, the signed Hodge component, and the graph-evaluated scalar;
the last projection passes through~$\kappa(\cA)$. The bar projection
gives A/B, the Verdier-Lagrangian projection gives C, and the chiral
Hochschild projection gives H. The five targets below are
...
\begin{theorem}[Collision-to-Yangian comparison; \ClaimStatusConditional]
...
+H_{\mathrm{CYBE}}^{\mathrm{rep}}(\cA;W)+H_Y(\cA;W);
\mathrm{KSDual\ sublocus\ for\ canonicity\ of\ }r(z);
\mathrm{Verdier\ window\ for\ part\ (ii)})$\textup{]}
Let $\cA$ be a modular Koszul chiral algebra equipped with the stated
represented-CYBE and Yangian quantization packages. Then the binary
collision kernel and its ternary operadic relation map to the
dg-shifted Yangian structure on the chosen representation surface:
\begin{equation}\label{eq:yangian-shadow}
K_\cA^{\mathrm{coll}}(z)
=\operatorname{Res}^{\mathrm{coll}}_{0,2}(\Theta_\cA^{E_1}),
\qquad
\rho_\cA(K_\cA^{\mathrm{coll}})=r_\cA(z),
\qquad
\operatorname{Res}^{\mathrm{coll}}_{0,3}(\Theta_\cA^{E_1})
\;\Longrightarrow\;
...
\begin{proof}
The intrinsic kernel statement is the binary recovery theorem in
\S\ref{subsec:collision-filtration-recovery}; the represented
statements use the two named hypothesis packages.

\smallskip\noindent
\emph{Binary collision and represented $r$-matrix.}\;
Theorem~\ref{thm:collision-residue-twisting} identifies
$K_\cA^{\mathrm{coll}}$ with the binary component
$(\pi_\cA)_2^{\mathrm{coll}}$ of the universal twisting morphism
$\pi_\cA\in\mathrm{Tw}(\barBch(\cA),\cA)$.
Choose $\cA^{\mathrm i}=C_X(s^{-1}V,s^{-2}R)$ and
$q_\cA\colon\cA^{\mathrm i}\to\barBch(\cA)$.
On the Koszul locus, $H^\bullet(q_\cA)$ identifies the quadratic model
with bar cohomology; Verdier transposition and~$\nu_\cA$ then produce
coordinates in the chosen partner.  The package
$H_{\mathrm{CYBE}}^{\mathrm{rep}}$ supplies the representation map
$\rho_\cA$ and identifies its image with the spectral
$r$-matrix $r_\cA(z)$ of
Definition~\ref{def:dg-shifted-yangian}; this is
Theorem~\ref{thm:collision-residue-twisting}(ii).

\smallskip\noindent
\emph{Ternary collision and $A_\infty$-YBE.}\;
The MC equation restricted to the genus-zero, four-point sector and
projected to collision depth~$2$ gives the operadic three-insertion
relation \textup{(}Theorem~\ref{thm:collision-depth-2-ybe}\textup{)}.
The packages $H_{\mathrm{CYBE}}^{\mathrm{rep}}$ and $H_Y$ carry this
relation to the classical Yang--Baxter equation and its chosen
$A_\infty$ quantization.
...
The intrinsic conclusion for every modular Koszul~$\cA$ is the
collision kernel and its operadic boundary relation; the Yangian
conclusion is the represented comparison supplied by $H_Y$.
\end{proof}
...
identification of $K_\cA^{\mathrm{coll}}$ with the binary component of
...
The dg-shifted Yangian supplied by $H_Y(\cA;W)$ from the represented
collision kernel
$r_\cA(z)=\rho_\cA(K_\cA^{\mathrm{coll}}(z))$
\textup{(}Theorem~\ref{thm:yangian-shadow-theorem}\textup{)} is
formally distinct from the Drinfeld double of a Hall
...
\item \emph{Shuffle presentation.}
 ...
 different presentation: as the deformation quantization, under
 $H_Y$, of the represented collision kernel~$r_\cA(z)$ at the
 prefactor $\hbar=1/(k+h^\vee)$
...
level-$k$ trace-form rational $r$-matrix
$r^{\mathrm{cl}}(z) = k\Omega_{\mathrm{tr}}/z$
...
The normalized comparison parameter is
...
\;=\;
\underbrace{\frac{1}{k+h^\vee}}_{\substack{\text{KZ-normalized}\\\text{collision prefactor}}}.
\end{equation}
In the trace-form collision convention the residue is
$r^{\mathrm{tr}}(z)=k\,\Omega_{\mathrm{tr}}/z$. It becomes the third
term of~\eqref{eq:three-parameter-hbar} only after the
Sugawara--Casimir rescaling to the KZ convention
$r^{\mathrm{KZ}}(z)=\Omega/((k+h^\vee)z)$. Thus
\eqref{eq:three-parameter-hbar} is an equality of normalized
deformation parameters, not a literal equality of the unnormalised
rational kernels.
\end{theorem}
...
(Remark~\ref{rem:four-levels}). The correct completed-datum
...
Theorem (Remark~\ref{rem:bridge-theorem-comparison}).
...
The ordered binary projection is the collision kernel
\[
K_\cA^{\mathrm{coll}}(z)
=\operatorname{Res}^{\mathrm{coll}}_{0,2}(\Theta_\cA^{E_1}).
\]
Under the quadratic comparison of Gui--Li--Zeng~\cite{GLZ22} and a
package $H_{\mathrm{CYBE}}^{\mathrm{rep}}$, its represented image is
the classical $r$-matrix.  A quantum deformation
$r(z)=r^{\mathrm{cl}}(z)+\hbar r^{(1)}(z)+\cdots$ belongs to the
additional quantization package $H_Y$.
...
\subsubsection{The mature completed-datum theorem}

\begin{remark}[The mature statement]
\label{rem:mature-platonic-statement}
\index{holographic modular Koszul datum!completed theorem}
...
The open mathematical conditions are:
...
\begin{theorem}[Collision kernel as binary component of the universal twisting morphism;
\ClaimStatusConditional]
...
\mathrm{level}=2\to 4,
\mathrm{hypothesis\ package}=\mathrm{modular\ Koszul};
\mathrm{Verdier\ window}+H_{\mathrm{CYBE}}^{\mathrm{rep}}
\mathrm{\ for\ part\ (ii)};\mathrm{KSDual\ sublocus\ if\ }
...
\begin{enumerate}[label=\textup{(\roman*)}]
\item The restriction of\/ $\Theta_\cA^{E_1}$ to the genus-zero,
 two-point collision stratum recovers the binary component of the
 universal twisting morphism:
 \begin{equation}\label{eq:collision-residue-equals-twisting}
 K_\cA^{\mathrm{coll}}(z)
 :=\operatorname{Res}^{\mathrm{coll}}_{0,2}(\Theta_\cA^{E_1})
 \;=\;
 (\pi_\cA)_2^{\mathrm{coll}}
 \;\in\;\End_\cA(2)\widehat\otimes\cO(*\Delta),
 \end{equation}
...
\item After applying the Koszul comparison
 $q_\cA\colon\cA^{\mathrm i}=C_X(s^{-1}V,s^{-2}R)
 \to\barBch(\cA)$ and, in a finite Verdier window, the pair
 map~$\nu_\cA$, a specified representation map $\rho_\cA$, and
 $H_{\mathrm{CYBE}}^{\mathrm{rep}}(\cA;W)$, the binary component
...
\textup{(}Corollary~\ref{cor:three-bijections}\textup{)} identify the
binary twisting component with the intrinsic kernel; the stated
representation package supplies its $r$-matrix image on the evaluation
locus.
\end{proof}

\begin{theorem}[Collision-depth-$2$ operadic relation and represented CYBE;
\ClaimStatusConditional]
...
H_{\mathrm{CYBE}}^{\mathrm{rep}}(\cA;W)\mathrm{\ for\ the\ CYBE\ conclusion};
...
gives the operadic three-insertion relation for
$K_\cA^{\mathrm{coll}}$. On an affine comparison surface equipped
with $H_{\mathrm{CYBE}}^{\mathrm{rep}}$, its represented image is the
...
\end{equation}
This represented relation implies the classical Yang--Baxter equation for the represented
spectral kernel $r_\cA=\rho_\cA(K_\cA^{\mathrm{coll}})$ via partial fractions on
...
give the intrinsic operadic relation. The mechanism is the Arnold relation
(Theorem~\ref{thm:arnold-relations})
$\eta_{12} \wedge \eta_{23} + \eta_{23} \wedge \eta_{31} +
\eta_{31} \wedge \eta_{12} = 0$
tensored with the structure-constant data of~$\cA$.  Applying
$H_{\mathrm{CYBE}}^{\mathrm{rep}}$ gives the displayed IBR and CYBE.
The equivalence
...
Scalar $\kappa$ & $\operatorname{tr}(\Theta_\cA)$
 & Signed Hodge tower $(-1)^g\kappa\lambda_g$
 & \textbf{Conditional} ($H_D$; Thm~D) \\
...
The MC entry controls the shadow tower, while the quadratic coalgebra,
full bar,
...
\item $\cA^{\mathrm i}=C_X(s^{-1}V,s^{-2}R)$ is the quadratic
 Heisenberg coalgebra, and
 $q_{\mathcal H_\kappa}\colon\cA^{\mathrm i}\xrightarrow{\sim}
 B^{\mathrm{ch}}(\mathcal H_\kappa)$ is the Gaussian comparison.
...
\item $\mathcal{C} = \mathrm{Vect}^{\mathbb{Z}}$; no non-trivial lines
 beyond Fock modules.
\item $K_{\mathcal H}^{\mathrm{coll}}(z)=
 \kappa\Omega_{\mathcal H}/z$; its Fock-charge representation has
 commuting insertions, supplies $H_{\mathrm{CYBE}}^{\mathrm{rep}}$,
 and gives $r_{\mathcal H}(z)=\kappa/z$.
...
\end{enumerate}
Complementarity is the scalar statement
$\kappa_k + \kappa_{-k-2h^\vee} = 0$ for the reflected current
presentation.
...
finite genus-$0$ tree sum for the bar-intrinsic MC element as follows.
...
\noindent
(iii)~The scalar genus-$g$ free energy is
$F_g^{\mathrm{sc}}=\kappa\cdot\lambda_g^{\mathrm{FP}}$
\textup{(}Theorem~\ref{thm:genus-universality}\textup{)};
additivity of~$\kappa$ gives additivity of~$F_g^{\mathrm{sc}}$.
At the non-scalar level, the graph-sum formula~\eqref{eq:preface-theta}
is additive under
tensor products by
...
The preceding subsections organized the holographic comparison around
...
technology (the chiral envelope of Beilinson--Drinfeld~\cite[\S3.7]{BD04},
the factorization envelope of Costello--Gwilliam~\cite{CG17}, and Vicedo's
...
\begin{definition}[Cyclically admissible Lie conformal algebra; \ClaimStatusDefinitional]
...
$(\mathrm{Open}\cap\mathrm{Cat},\mathrm{factorisation},
\mathrm{level}=1\leftrightarrow 5,
\mathrm{hypothesis\ package}=\mathrm{factorization\ envelope\ at\ genus\ }0;
...
\bigr).
\end{equation}
At genus~$0$, this recovers the factorization envelope. The output
...
\begin{evidence}
At genus~$0$, the factorization envelope is already left adjoint to the
...
\]
The reduction-network axis is proved on a substantial type~$A$
transport graph; the Koszul-duality transport along that graph
remains conditional on DS--bar compatibility. The second axis
remains the true frontier.
\end{remark}

After the principal Feigin--Frenkel case, the first genuinely
non-principal reduction-theoretic corridor is the hook-type family in
type~$A$. Inverse reductions connect the hook-type nodes, but full
hook-type Koszul duality remains conditional on the DS--bar
compatibility package; the 2023--2025 work of Fehily,
...
\begin{evidence}
The hook-type corridor supplies the reduction-theoretic seed family.
The 2023--2025 inverse Hamiltonian reduction results in type~$A$
...
genus creation in the cochain-level closed sector is the open Connes
trace mechanism viewed through the derived-center map.
\end{conjecture}
...
Modular envelope
 ...
 & Genus-$0$ envelope adjunction; shadow functor
...
\item \emph{Color decoration via a represented $R$-matrix.}
 Passing from the free theory to an interacting
 chiral algebra~$\cA$ at level~$k$ dresses the
 bar-complex combinatorics with the collision kernel
 $K_\cA^{\mathrm{coll}}(z)=k\Omega_{\mathrm{tr}}/z$.
 Under $H_{\mathrm{CYBE}}^{\mathrm{rep}}$ its represented image is
 ...
 is the $\Sigma_n$-homotopy-coinvariant shadow, which retains the
 equivariant operator class.  The chosen functional
 $\tau_{\cA,2}^{\mathrm{res}}$ produces
 $\kappa_{\mathrm{dp}}=	au_{\cA,2}^{\mathrm{res}}
 q_2(K_\cA^{\mathrm{coll}})$; on the affine row the Sugawara term
...
\item \emph{Gauge theory.}
 ...
 $K^{\mathrm{coll}}_{ab}=f^c_{ab}J_c$.  A celestial $R$-operator
 arises after a representation/monodromy package
 $H_{\mathrm{CYBE}}^{\mathrm{rep}}$ is supplied on tensor products of
 collinear modules; its classical term is the represented image of
 this kernel.
...
shadow connection's flat sections.  The statement remains conditional
on the comparison that identifies those flat sections with the
...
The scalar shadow genus expansion
$F_g^{\mathrm{sc}} = \kappa \cdot \lambda_g^{\mathrm{FP}}$
(Theorem~\ref{thm:modular-koszul-duality-main})
reproduces the scalar vacuum sector of the Nekrasov genus expansion
...
The Costello--Li twisted holography correspondence identifies
...
\item \emph{D3 brane \textup{(}twisted $\mathcal{N} = 4$ SYM\textup{)}.}
 ...
 $r(z) = k\Omega_{\mathrm{tr}}/z$ (classical Yang--Baxter; at the D3 locus
 $k=1$ this specializes to the level-$1$ kernel, while the
 level-prefixed form $k\Omega_{\mathrm{tr}}/z$ vanishes at $k=0$;
 $\Omega_{\mathrm{tr}}$ the trace-form Casimir element), and the genus expansion
 $F_g^{\mathrm{sc}} = \kappa\cdot\lambda_g^{\mathrm{FP}}$
 on the uniform-weight scalar lane computes the protected scalar
 twisted $\mathcal{N} = 4$ amplitude at genus~$g$.
...
The trace-form collision kernel is
$K_\cA^{\mathrm{coll}}(z)
=\mathrm{Res}^{\mathrm{coll}}_{0,2}(\Theta_\cA^{E_1})
=k\Omega_{\mathrm{tr}}/z$.  Under the affine KZ package
$H_{\mathrm{CYBE}}^{\mathrm{rep}}$, its represented image is the
classical $r$-matrix. The KZ-normalized representative
...
has at most a double pole; the $d\log$ extraction
 gives the simple-pole collision kernel
 $K_{ab}^{\mathrm{coll}}(z)=k\Omega_{ab}/z$ and its represented
 $r$-matrix on the KZ comparison surface.
...
$\kappa_{\mathrm{red}}=-N^2$ is a proposed post-reduction value. The
precise finite-$N$ comparison between the two requires a
ghost-cancellation computation inside the same BRST complex; it is not
deduced from the displayed pre-BRST sum.
...
\begin{theorem}[Protected and $\Omega$-deformed M5 chiral algebras;
\ClaimStatusConditional]
\label{thm:frontier-m5}
\index{M5 brane|textbf}
\emph{Type signature.}
(Open quadrant; chiral presentation; levels $1$ and~$5$; hypothesis
package: protected-sector functor, $\Omega$-background reduction,
finite-$N$ quotient, and stress-trace map.)

For $N\geq2$, let $\mathcal A_N^{\mathrm{prot}}$ denote the protected
chiral algebra of the interacting six-dimensional $(2,0)$ theory of
type~$A_{N-1}$, and let $\mathcal A_N^\Omega$ denote the chiral algebra
of the corresponding M5 system in Costello's $\Omega$-background.
The source results and the comparison problem are as follows.
\begin{enumerate}[label=\textup{(\roman*)}]
\item Beem--Rastelli--van Rees construct the protected chiral sector
and conjecturally identify
\[
 \mathcal A_N^{\mathrm{prot}}\simeq
 \mathcal W(\mathfrak{sl}_N)
\]
at the principal Toda central charge
\begin{equation}\label{eq:frontier-m5-central-charge}
 c_{2\mathrm d}(A_{N-1})=4N^3-3N-1.
\end{equation}

\item Costello constructs a homomorphism from the central extension of
the differential-operator algebra,
\[
 \pi_N:\ \mathcal W_{1+\infty}^{(N)}
 \longrightarrow \mathcal A_N^\Omega .
\]
For the $A_0$ transverse singularity, the finite-$N$ target is expected
to be the quotient $\mathcal W(\mathfrak{sl}_N)$; the large-$N$ limit
recovers $\mathcal W_{1+\infty}$.  A comparison
\[
 \Xi_N:\mathcal A_N^\Omega\longrightarrow
       \mathcal A_N^{\mathrm{prot}}
\]
between the two twists is additional structure.

\item Once $\Xi_N$ identifies the two stress tensors, the Vol.~I
stress-trace convention gives
\begin{equation}\label{eq:frontier-m5-kappa}
 \kappa_{\mathrm{stress}}(A_{N-1})
 =\frac{c_{2\mathrm d}(A_{N-1})}{2}
 =\frac{4N^3-3N-1}{2}.
\end{equation}
Thus $\kappa_{\mathrm{stress}}\sim2N^3$ on the interacting
$A_{N-1}$ family.

\item The abelian tensor multiplet has protected chiral algebra the
rank-one Heisenberg algebra and central charge~$1$
\cite[\S3]{BRvR14-6d}.  It is the primitive $N=1$ case.  Formula
\eqref{eq:frontier-m5-central-charge} describes the interacting
$A_{N-1}$ sector for $N\geq2$.

\item Choose an augmented Ran model of
$\mathcal W(\mathfrak{sl}_N)$ satisfying the Verdier dualizability
package $H_{\mathrm{VD}}$.  Its Koszul-dual object is formed in the
completed Ran category:
\begin{equation}\label{eq:frontier-m5-kappa-dual}
 \mathcal W(\mathfrak{sl}_N)^!
 :=\mathbb D_{\operatorname{Ran}}
       B_{\operatorname{Ran}}\bigl(\mathcal W(\mathfrak{sl}_N)\bigr).
\end{equation}
Its stress trace requires a realization morphism and a compatible
conformal vector.  With those data, the complementarity scalar is
\begin{equation}\label{eq:frontier-m5-complementarity}
 K_N^{\mathrm{stress}}
 :=\kappa_{\mathrm{stress}}\bigl(\mathcal W(\mathfrak{sl}_N)\bigr)
  +\kappa_{\mathrm{stress}}
       \bigl(\mathcal W(\mathfrak{sl}_N)^!\bigr).
\end{equation}
The source literature supplies the first summand.  The second summand
is the Vol.~I Koszul-dual trace problem.

\item The source-backed large-$N$ statement is
\begin{equation}\label{eq:frontier-m5-complementarity-leading}
 \kappa_{\mathrm{stress}}\bigl(\mathcal W(\mathfrak{sl}_N)\bigr)
 =2N^3-\frac32N-\frac12
 \sim2N^3.
\end{equation}
A shadow class for the full M5 datum is assigned after computing the
ordered bar differential, its completion, and the higher-spin trace.

\item The M2 and M5 families carry different source constructions.
The M5 stress scalar has cubic growth by
\eqref{eq:frontier-m5-kappa}; the ABJM scalar and its Airy limit arise
from the localization comparison of
Theorem~\ref{thm:frontier-abjm}.  A common normalization requires the
two specified trace maps.
\end{enumerate}
\end{theorem}

\begin{proof}
Beem--Rastelli--van Rees construct the cohomological chiral sector and
argue for the principal $\mathcal W(\mathfrak g)$ identification
\cite[\S\S2--4]{BRvR14-6d}.  Their equation~(4.4) is
\eqref{eq:frontier-m5-central-charge}; their Section~3 gives the
abelian tensor algebra and $c_{2\mathrm d}=1$.

Costello's Sections~1.7--1.9 construct the $\Omega$-background
description.  For transverse type~$A_k$, the central extension
$\mathcal W_{k+1+\infty}^{(N)}$ maps to the M5 chiral algebra; at
$k=0$ he records the finite-$N$ expectation
$\mathcal W(\mathfrak{sl}_N)$ and the large-$N$ limit
$\mathcal W_{1+\infty}$ \cite{Costello2111}.  These statements give
parts~(i)--(iv).  Part~(iii) applies the Vol.~I stress-trace definition.
Part~(v) is the Ran Verdier--bar definition of the dual object and
states its remaining realization map.  Parts~(vi)--(vii) follow from
\eqref{eq:frontier-m5-kappa} and the named comparison maps.
\end{proof}

\begin{problem}[Koszul-dual trace of the protected M5 algebra;
\ClaimStatusOpen]
\label{prop:phantom-m5-koszul-dual}
\index{M5 brane!Koszul-dual trace|textbf}
For an augmented Ran model of the protected algebra
$\mathcal W_N:=\mathcal W(\mathfrak{sl}_N)$ satisfying
$H_{\mathrm{VD}}$, form the completed
Verdier--bar dual
\[
 \mathcal W_N^!
 =\mathbb D_{\operatorname{Ran}}B_{\operatorname{Ran}}(\mathcal W_N).
\]
Construct a chiral realization of $\mathcal W_N^!$, a compatible
conformal vector $T_N^!$, and the corresponding stress trace
$\kappa_{\mathrm{stress}}(\mathcal W_N^!,T_N^!)$.  The resulting
complementarity scalar is
\begin{equation}\label{eq:m5-complementarity-polynomial}
 K_N^{\mathrm{M5}}
 =\frac{4N^3-3N-1}{2}
 +\kappa_{\mathrm{stress}}(\mathcal W_N^!,T_N^!).
\end{equation}
A physical duality statement additionally requires a functor from the
realized dual algebra to observables of a specified six-dimensional
background.  These constructions determine both the algebraic and
physical meanings of $K_N^{\mathrm{M5}}$.
\end{problem}
...
The protected $A_1$ M5 algebra is conjecturally
$\mathcal A_2^{\mathrm{prot}}\simeq
\mathcal W(\mathfrak{sl}_2)$ with central charge~$25$.
A proposed four-dimensional comparison uses a class-$\mathcal S$
theory of type~$A_1$
...
\item \emph{Conformal anomalies.}
 ...
 These are the class-$\mathcal S$ parent values.  The M5 protected
 algebra in Theorem~\ref{thm:frontier-m5} carries its own stress
 central charge.
...
\item \emph{Steiner-system rigidity.}
 ...
 governed by a separate geometric comparison map.
\end{enumerate}
The proposed comparison therefore asks for an interface, BRST
reduction, or protected-index morphism from the class-$\mathcal S$
datum of central charge~$-214$ to the protected
$\mathcal W(\mathfrak{sl}_2)$ datum of central charge~$25$ in
Theorem~\ref{thm:frontier-m5}.  Such a morphism necessarily carries a
specified change of stress normalization.  The $4\mathrm d$ Coulomb dimension
...
\begin{remark}[Proposed one-loop $\Delta _5$ comparison for a K3 background]
\label{rem:frontier-Delta5-onel-loop-11d-K3T2}
\ClaimStatusConjectured
\index{eleven-dimensional supergravity!K3 background}
\index{Igusa cusp form!one-loop comparison problem}
Raghavendran--Saberi--Williams construct their interacting twisted
eleven-dimensional theory on $\mathbb R\times Z_5$, with $Z_5$ a
Calabi--Yau fivefold \cite{RSW23}.  The product
\begin{equation}\label{eq:frontier-Delta5-eleven-manifold}
 \mathcal Y_{11}
 =\mathbb R_t\times K3\times E\times\mathbb C^2
\end{equation}
has this form, with $Z_5=K3\times E\times\mathbb C^2$.  Fix an
elliptic K3 with $24$ nodal $I_1$ fibres and choose M5 source
worldvolumes $W_a\subset\mathcal Y_{11}$ indexed by its discriminant
points.  Let
\[
 [\mathfrak o_1]\in
 H^1\operatorname{Obs}_{\mathrm{loc}}
 \bigl(\mathcal E_{11}^{\mathrm{tw}}
       (\mathcal Y_{11};W_1,\ldots,W_{24})\bigr)
\]
be the one-loop BV obstruction class.

The proposed comparison is a morphism
\[
 \Theta_1:
 H^1\operatorname{Obs}_{\mathrm{loc}}
 \bigl(\mathcal E_{11}^{\mathrm{tw}}
       (\mathcal Y_{11};W_1,\ldots,W_{24})\bigr)
 \longrightarrow
 H^0\bigl(\mathcal A_2,\lambda^5\otimes\chi\bigr)
\]
satisfying $\Theta_1([\mathfrak o_1])=\Delta _5$.  Its arithmetic
normalization is
\begin{equation}\label{eq:frontier-Delta5-weight-decomposition}
 \operatorname{wt}(\Delta _5)=\frac{f(0)}2
 =\frac{10}{2}=5,
\end{equation}
where $f(0)=10$ is the constant coefficient of the weight-$0$,
index-$1$ Jacobi form $\phi_{0,1}$ in the Gritsenko--Nikulin product
\cite[eqs.~(1.8)--(1.10)]{GN96Igusa}.

The construction of $\Theta_1$ consists of four geometric steps:
globalize the M5 source backgrounds on
\eqref{eq:frontier-Delta5-eleven-manifold}, quantize the sourced theory,
push the local obstruction class to the elliptic-K3 moduli problem,
and identify the resulting automorphic section with $\Delta _5$ by
its divisor and leading Fourier coefficient.  These steps give the
precise content of the conjecture.
\end{remark}
...
ABJM (M2)
 ...
 & Airy, $N^{3/2}$ scaling \\
M5
 & $\mathcal W(\mathfrak{sl}_N)$ \textup{(protected, conjectural)}
 & $(4N^3{-}3N{-}1)/2$
 & --
 & $N^3$ stress trace; class requires bar computation \\
Celestial gauge
 & $J_{\mathrm{col}}$
 & $0$ (self-dual)
 & G
...
\item Shadow class: $\mathsf{C}$ per $\beta\gamma$ pair
 ...
 shadow depth\textup{)}.
\item Scalar genus tower \textup{(}Theorem~D,
 uniform-weight scalar lane: all generators at
 $\lambda = 1$\textup{)}:
 \begin{align}
 F_1^{\mathrm{sc}}(\cA_{\mathrm{Burns}})
 ...
 = \tfrac{7}{1440},
 \label{eq:burns-F2} \\
 F_3^{\mathrm{sc}}(\cA_{\mathrm{Burns}})
 &= \kappa \cdot \lambda_3^{\mathrm{FP}}
 = 4 \cdot \tfrac{31}{967680}
 = \tfrac{31}{241920}.
 \label{eq:burns-F3}
 \end{align}
 The values $F_2^{\mathrm{sc}}$ and~$F_3^{\mathrm{sc}}$ are scalar
 predictions beyond
...
\item $T$-line planted-forest correction at genus~$2$:
 ...
 $F_g^{\mathrm{sc}} = \kappa \cdot \lambda_g^{\mathrm{FP}}$ on the
 uniform-weight scalar lane
 \textup{(}Theorem~D\textup{)}.
\end{enumerate}
The three scalar values are the exact substitutions
$4\lambda_g^{\mathrm{FP}}$ for $g=1,2,3$; the planted-forest value is
the displayed rational substitution in $S_3(10S_3-\kappa)/48$.
\end{computation}
...
The characteristic $\kappa(\cA)$ of Theorem~D enters the
$H_D^K$ Hodge character
$\operatorname{ch}_g(\mathfrak O_g^K)=
(-1)^g\kappa(\cA)\lambda_g$ on $\overline{\cM}_g$ and determines the
...
correspondence is the conjectural identification
(\ClaimStatusConjectured) of the genus-$n$ shell of the genus
spectral sequence (Construction~\ref{const:vol1-genus-spectral-sequence})
with the $E(n)$-localization in stable homotopy: at chromatic
height~$n$, the $E(n)$-localization of the spectral Steinberg object
(Vol~II, the spectral-braiding chapter) recovers the genus-$n$ shell
of the shadow obstruction tower. Under this identification:
\begin{enumerate}[label=\textup{(\roman*)}]
\item Height $0$ (rational):
 $\kappa(\cA)$, the degree-$2$ scalar trace: in abelian and scalar
 families $\kappa(\cA)=\tau_{\cA,2}^{\mathrm{res}}
 q_2(K_\cA^{\mathrm{coll}})$, while for non-abelian affine
 Kac--Moody this trace is $\kappa_{\mathrm{dp}}$ and
 $\kappa(V_k(\fg))=\kappa_{\mathrm{dp}}+\dim(\fg)/2$; it controls
 $F_g^{\mathrm{sc}}$ on the uniform-weight scalar lane.
...
$\varkappa = 25/3$, conditionally, for Bershadsky--Polyakov
(the central-charge conductor $K^c_{\mathrm{BP}} = 50$ is
Proposition~\ref{prop:bp-complementarity-constant}; the genus-one
curvature $\kappa_{\mathrm{BP}}$ remains open).
...
Bershadsky--Polyakov conditionally at $\varkappa = 25/3$) measures scalar failure of
...
\begin{remark}[K3 boundary data and the Theorem C bucket $\{0,\;8,\;13,\;250/3,\;25/3\}$]
...
The scalar complementarity invariant
$\varkappa(\cA) = \kappa(\cA) + \kappa(\cA^!)$
inventoried above (with values $\{0,\;13,\;250/3,\;25/3\}$ on the
...
$\varkappa = 25/3$ the conditional Bershadsky--Polyakov value. The fact that
...
\begin{remark}[Conditional $\Phi$-trace comparison bridging Vol~I conductor and Vol~III BKM~$\kappa$]
\label{rem:frontier-universal-phi-trace}
\ClaimStatusConjectured
\index{Phi-trace comparison@$\Phi$-trace comparison}
...
product~\cite{Borcherds1995,Borcherds92}. Fix an admissible slicing
datum $(\Sigma_{d-1},C)$ and write the Vol~III correspondence as
\[
\Phi_d^{(\Sigma_{d-1},C)}
=
\mathrm{Sp}^{\mathrm{ch}}_{\Sigma_{d-1},C}\circ\Phi_d^{\mathrm{FA}}.
\]
When this composite is defined on~$Y$, set
$A_Y^{(\Sigma,C)}:=\Phi_d^{(\Sigma_{d-1},C)}(Y)$. On a
...
the \emph{conditional $\Phi$-trace comparison} predicts
\begin{equation}\label{eq:frontier-phi-trace}
\boxed{\;
K(A_Y^{(\Sigma,C)})
\;=\;
-c_{\mathrm{ghost}}(\BRST(A_Y^{(\Sigma,C)}))
\;=\;
\operatorname{tr}_{\cZ^{\mathrm{der}}_{\mathrm{ch}}(A_Y^{(\Sigma,C)})}
 \!\bigl(K_{A_Y^{(\Sigma,C)}}\bigr)
\;\xrightarrow{\;\Phi_{\mathrm{tr}}^{(\Sigma,C)}\;}\;
...
middle arrow is the desired trace pushforward
$\Phi_{\mathrm{tr}}^{(\Sigma,C)}$, and the
...
\small
\renewcommand{\arraystretch}{1.2}
\begin{tabular}{c@{\;\;\xrightarrow{\;\Phi_{\mathrm{tr}}^{(\Sigma,C)}\;}\;\;}c}
...
supertrace calculus pushes forward across the trace comparison to the
Vol~III BKM side when the map~$\Phi_{\mathrm{tr}}^{(\Sigma,C)}$
exists. The automorphic equality is proved
...
obligation unless the Vol~I to Vol~III comparison correspondence is
constructed.
\end{remark}
...
\begin{array}{rcl}
A & = & \mathbf H_{\Delta_5}, \\
A^i & = & C_X(s^{-1}V_{\Delta_5},s^{-2}R_{\Delta_5}), \\
q_A & : & A^i\longrightarrow B_{\mathrm{ch}}(\mathbf H_{\Delta_5}), \\
K_X(A) & = & \mathbb D_{\Ran}B_{\mathrm{ch}}(\mathbf H_{\Delta_5}), \\
A^! & \xleftarrow{\ \nu_A\ } & K_X(A)
 \simeq \mathbf H_{\Delta_5}\ \text{on the self-dual
 strictification locus }\hbar^2=-1/8, \\
...
\Omega_X(B_{\mathrm{ch}}(\mathbf H_{\Delta_5})) & \simeq
 & \mathbf H_{\Delta_5}
\ \text{by the bar--cobar inversion counit}, \\
...
the boundary chiral Hochschild sector is governed by a family datum
$H_H(\cA_{\mathrm B};S_{\mathrm B})$, and Theorem~H gives
$\operatorname{Supp}\ChirHoch^\bullet(\cA_{\mathrm B})
\subseteq S_{\mathrm B}$.  Its passage to a physical bulk is the
separate open/closed comparison problem. The global
boundary/bulk/line triangle remains conjectural beyond the typed
comparison maps. The $\kappa$~indexing on
the two sides of
