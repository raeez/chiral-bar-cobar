...
\subsubsection{Motivation: the universal problem of killing curvature}
\label{subsubsec:transgression-motivation}

The genus-$g$ bar complex of a cyclic chiral algebra~$\cA$ is a family
of curved cochain complexes
(Proposition~\ref{prop:genus-g-curvature-package}):
\begin{equation}\label{eq:g9-curved-bar}
\bigl(\barB^{(g)}(\cA),\; \dfib\bigr),
\qquad
\dfib^{\,2} = \kappa(\cA) \cdot \omega_g \cdot \id,
\end{equation}
where $\kappa(\cA) = \mathrm{Tr}_\cA$
(Definition~\ref{def:modular-curvature-direct}) and $\omega_g$ is the
Arakelov $(1,1)$-form on the genus-$g$ fiber. The curvature
$\kappa \cdot \omega_g$ is a scalar multiple of the identity:
it lives in $H^0(\cA) \otimes H^{1,1}(\Sigma_g)$.

The fundamental algebraic question is: \emph{given a dg algebra $(B, d)$
satisfying $d^2 = \lambda \cdot \id$ for some central element
$\lambda \ne 0$, what is the universal extension of~$B$ in which
the curvature is the square of a degree-one element?}

The answer is the transgression construction, which adjoins a formal
square root of the curvature. The name is chosen by analogy with the
transgression in the Leray--Serre spectral sequence: if a class $\alpha$
survives to a later page, the differential $d_r(\alpha)$ is the
transgression, and $\alpha$ itself is the ``square root'' of the
$d_{r+1}$-boundary.
...
\mathrm{Cl}_{2g}(u)$ admits two structurally distinct
...
In the BRST interpretation of this Clifford sector, the generators
...
\label{subsubsec:complementarity-asymmetry}
...
\label{def:complementarity-asymmetry}
\label{def:delta-kappa-vs-kappa-eff}
...
\begin{equation}\label{eq:g9-delta-kappa}
...
\begin{equation}\label{eq:g9-two-invariants-vir}
\delta_\kappa = 0
\;\;\Longleftrightarrow\;\;
c = 13
...
\begin{remark}[Koszul self-duality vs.\ uncurved locus]
\label{rem:g9-two-self-dualities}
\index{Virasoro!self-duality and uncurved locus}
The Virasoro family has two distinguished points:
\begin{enumerate}[label=\textup{(\alph*)}]
\item \emph{Uncurved locus at $c = 0$}
(Theorem~\ref{thm:virasoro-self-duality}). Here $\kappa = 0$: the bar
complex is a genuine cochain complex and the \emph{quadratic} OPE
is self-dual in the classical sense. However, the chiral
Koszul dual is $\mathrm{Vir}_0^! = \mathrm{Vir}_{26} \neq
\mathrm{Vir}_0$
(Example~\ref{ex:virasoro-koszul-dual}): uncurved does not
imply chiral Koszul self-dual. The Clifford completion is in the
gravitational regime (exterior degeneration).

\item \emph{Chiral Koszul self-duality at $c = 13$}
(Theorem~\ref{thm:vir-self-duality-c13}). Here $\kappa = 13/2 \neq 0$:
the bar complex is curved, $\mathrm{Vir}_{13}^! \simeq \mathrm{Vir}_{13}$,
and the Clifford completion is in the topological
regime (Clifford algebra with $u = (13/2)\omega_g \neq 0$).
\end{enumerate}
...
The Arakelov form $\omega_g = \frac{i}{2}\sum_{\alpha,\beta=1}^g
...
extending the genus-$0$ BV/bar dictionary of
Remark~\ref{rem:bv-bar-bridge}? The quantum master
