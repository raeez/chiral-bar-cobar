...
% genus-0 arity in the modular deformation package.  The four depth
...
\begin{theorem}[Shadow-depth stabilization and transferred arity cutoff]
\label{thm:thqg-complexity-controls-bulk}
\ClaimStatusProvedHere
\index{gravitational complexity!arity cutoff theorem}
...
minimal-model deformation package vanish above arity
$r_{\max}(\cA)$:
\begin{equation}\label{eq:thqg-transferred-arity-cutoff}
\ell_s^{(0),\mathrm{tr}} = 0
...
the transferred genus-$0$ brackets vanish above arity
$r_{\max}(\cA)$.  By the shadow--formality identification,
those same arity components are exactly the higher shadow
coefficients on the minimal model, so no new arity appears above
$r_{\max}(\cA)$ and the tower stabilises:
$\Theta_\cA^{\le r} = \Theta_\cA^{\le r_{\max}}$ for
$r \ge r_{\max}(\cA)$, hence $o_{r+1}(\cA)=0$ there.  The
arity cutoff for the transferred brackets is exactly the equality
...
bracket arity.  The primary-line potential
...
receives an arity-$r$ shadow coefficient as the $z^{-r}$ term.
...
\item \textbf{Lie} ($r_{\max} = 3$):
  The cubic shadow is nonzero and all higher arities vanish.
