...
% boundary chiral algebra — determines the maximal transferred
% genus-0 degree in the modular deformation package. The four depth
% classes G/L/C/M emerge as the only possibilities from the structure
...
Equivalently (Theorem~\ref{thm:operadic-complexity-detailed}):
...
\begin{theorem}[Shadow-depth stabilization and transferred degree cutoff]
\label{thm:thqg-complexity-controls-bulk}
\ClaimStatusProvedHere
\index{gravitational complexity!degree cutoff theorem}
Let $\cA$ be a chirally Koszul chiral algebra satisfying the
standing hypotheses~\textup{\ref{def:thqg-standing-hypotheses}}.
...
o_{r+1}(\cA) = 0.
\end{equation}
The transferred genus-$0$ $L_\infty$-brackets on the
minimal-model deformation package vanish above degree
$r_{\max}(\cA)$:
\begin{equation}\label{eq:thqg-transferred-degree-cutoff}
\ell_s^{(0),\mathrm{tr}} = 0
\qquad \text{for all } s > r_{\max}(\cA).
\end{equation}
Conversely, if $\ell_r^{(0),\mathrm{tr}} \neq 0$, then
$r_{\max}(\cA) \geq r$.
\end{theorem}

\begin{proof}
The equality $r_{\max}(\cA) = f_\infty(\cA)$ from
Theorem~\ref{thm:operadic-complexity-detailed} implies that
the transferred genus-$0$ brackets vanish above degree
$r_{\max}(\cA)$. By the shadow--formality identification,
those same degree components are exactly the higher shadow
coefficients on the minimal model, so no new degree appears above
$r_{\max}(\cA)$ and the tower stabilises:
$\Theta_\cA^{\le r} = \Theta_\cA^{\le r_{\max}}$ for
$r \ge r_{\max}(\cA)$, hence $o_{r+1}(\cA)=0$ there. The
degree cutoff for the transferred brackets is exactly the equality
$f_\infty(\cA) = r_{\max}(\cA)$ from
Theorem~\ref{thm:operadic-complexity-detailed}. The converse
implication is the same equality read in the opposite direction.
\end{proof}
...
\begin{proof}
$r_{\max} = d_\infty$
(Theorem~\ref{thm:operadic-complexity-detailed}), and $d_\infty$
...
rigidity theorem (Theorem~\ref{thm:nms-rank-one-rigidity}).
Class~$\mathbf{C}$.
...
\subsection{Transferred \texorpdfstring{$L_\infty$}{L-infty} profiles of the four classes}
\label{subsec:thqg-grav-interpretation}

\begin{theorem}[Transferred \texorpdfstring{$L_\infty$}{L-infty} profiles of the four classes]
\label{thm:thqg-grav-content}
\ClaimStatusProvedHere
\index{gravitational complexity!genus-0 bracket profile}
The four shadow-depth classes are distinguished by the transferred
genus-$0$ $L_\infty$-profile on the modular deformation complex
and by the first nonzero term of the primary-line potential:
\begin{center}
\small
\renewcommand{\arraystretch}{1.25}
\begin{tabular}{c@{\quad}l@{\quad}l}
\toprule
\emph{Class} & \emph{Transferred genus-$0$ bracket pattern} &
\emph{Primary-line potential profile} \\
\midrule
$\mathbf{G}$ &
 $\ell_r^{(0),\mathrm{tr}} = 0$ for $r \geq 3$. &
 Quadratic only. \\
$\mathbf{L}$ &
 $\ell_3^{(0),\mathrm{tr}} \neq 0$,
 $\ell_r^{(0),\mathrm{tr}} = 0$ for $r \geq 4$. &
 Cubic first nonlinear term. \\
$\mathbf{C}$ &
 $\ell_3^{(0),\mathrm{tr}} = 0$,
 $\ell_4^{(0),\mathrm{tr}} \neq 0$,
 $\ell_r^{(0),\mathrm{tr}} = 0$ for $r \geq 5$. &
 Quartic first nonlinear term. \\
$\mathbf{M}$ &
 $\ell_r^{(0),\mathrm{tr}} \neq 0$ for infinitely many $r$. &
 Full infinite series. \\
\bottomrule
\end{tabular}
\end{center}
\end{theorem}
...
\begin{proof}
The equality $r_{\max} = f_\infty$ from
Theorem~\ref{thm:operadic-complexity-detailed} identifies the
depth class with the highest nonzero transferred genus-$0$
bracket degree. The primary-line potential
$S_\cA(x) = \sum_{r \geq 2} S_r x^r/r$ records the same
vanishing pattern in its coefficients. Class~$\mathbf{G}$ has
only the quadratic term, class~$\mathbf{L}$ has first nonlinear
term cubic, class~$\mathbf{C}$ has cubic term zero and quartic
term nonzero, and class~$\mathbf{M}$ has infinitely many
...
For Gaussian algebras on the proved uniform-weight lane
(Definition~\ref{def:scalar-lane};
e.g.\ Heisenberg and the uniform-weight Gaussian examples), the scalar genus-$g$ partition function is
$Z_g^{\mathrm{scal}}(\cA) = (\det(1 - K_g))^{-\kappa/2}$
\textup{(}Theorem~\ref{thm:heisenberg-sewing}\textup{)}.
For general multi-weight Gaussian families, only the genus-$1$
scalar term is unconditional.
\end{corollary}
...
\begin{example}[Complementarity potentials of the standard archetypes]
\label{ex:thqg-four-potentials}
\index{complementarity potential!standard archetypes}
\begin{enumerate}[label=\textup{(\alph*)}]
\item $\cH_k$ ($\mathbf{G}$):
...
\begin{remark}[Normalisation convention for $\rho^{(1)}$]
\label{rem:thqg-rho1-normalisation}
The loop ratio $\rho^{(1)}_{\mathrm{Vir}} := \delta H^{(1)}/\kappa^2 = 480/[c^4(5c+22)]$
used here differs from the convention in Appendix~\ref{app:nonlinear-modular-shadows},
where $\rho^{(1)}_{\mathrm{Vir}} := \delta H^{(1)}/H^{(0)} = 240/[c^3(5c+22)]$ with $H^{(0)} = \kappa = c/2$.
The two conventions are related by a factor of $1/\kappa = 2/c$; the present convention
normalises by $\kappa^2$ rather than $\kappa$ to make $\rho^{(1)}$ dimensionless in the gravitational context.
\end{remark}
...
$\mathfrak{Q}(\cA) \neq 0$
(Proposition~\ref{prop:shadow-formality-low-degree}(iii)):
...
Both finite and infinite shadow-depth algebras can be Koszul:
\begin{center}
\small
\renewcommand{\arraystretch}{1.15}
\begin{tabular}{lcc}
...
\begin{proposition}[Complexity is a Koszul duality invariant; \ClaimStatusConditional]
\label{prop:thqg-duality-preserves-complexity}
% Conditional on Conjecture~\ref{conj:holographic-koszul-deformation}
...
$\Defcyc(\cA)$
(Conjecture~\ref{conj:holographic-koszul-deformation}).
\end{proof}
...
\begin{proof}
The Laplace kernel $r^L(z) = \sum_{r \geq 2} S_r z^{-r}$
receives an degree-$r$ shadow coefficient as the $z^{-r}$ term.
Stabilization at $r_{\max}$ truncates the Laurent expansion.
The connection component listed in the table is the same shadow
data repackaged as a one-form, so it follows the same
degree-vanishing pattern.
\end{proof}
...
\subsection{Summary: the shadow-depth classification theorem}
\label{subsec:thqg-complexity-summary}

\begin{maintheorem}[Shadow-depth classification; result (G2)]
...
\item \textbf{Gaussian} ($r_{\max} = 2$):
 Only the quadratic shadow survives.
 All higher genus-$0$ transferred brackets vanish.
 Archetype: Heisenberg.

\item \textbf{Lie} ($r_{\max} = 3$):
 The cubic shadow is nonzero and all higher degrees vanish.
 Jacobi kills the first possible higher obstruction.
 Archetype: affine $V_k(\fg)$.

\item \textbf{Contact} ($r_{\max} = 4$):
 The cubic term vanishes, the quartic term is nonzero,
 and no higher degree survives.
 Cubic gauge triviality plus rank-one rigidity terminate the tower.
 Archetype: $\beta\gamma$.

\item \textbf{Mixed} ($r_{\max} = \infty$):
 Infinitely many degrees survive in the transferred
 genus-$0$ package.
...
\subsection{The MC equation on the primary line}
\label{subsec:thqg-mc-action}
\index{Maurer--Cartan equation!primary-line potential}

\begin{theorem}[Primary-line MC critical-point equation]
\label{thm:thqg-mc-euler-lagrange}
\ClaimStatusProvedHere
\index{complementarity potential!critical-point equation}
The MC equation $D_\cA\,\Theta_\cA +
\frac{1}{2}[\Theta_\cA, \Theta_\cA] = 0$,
evaluated on the one-generator primary line, is the
formal critical-point equation for the complementarity potential
$S_\cA(x)$:
...
Taylor coefficients of $S_\cA$. Writing
$S_\cA(x) = \sum_{r \geq 2} S_r x^r/r$ and differentiating term
by term gives the displayed critical-point equation. The
quadratic coefficient is $\kappa(\cA)$, so
$S''_\cA(0) = \kappa(\cA) = H_\cA$.
\end{proof}

\begin{remark}[Heuristic gravity analogues of the four potentials]
...
\subsection{The collision-filtration spectral sequence}
...
\begin{theorem}[Collision-filtration spectral sequence]
\label{thm:thqg-holographic-ss}
\ClaimStatusProvedHere
\index{spectral sequence!collision filtration}
...
\index{collision-filtration spectral sequence!$E_1$ for Virasoro}
...
\ClaimStatusProvedHere
\index{collision-filtration spectral sequence!collapse for affine}
...
\ClaimStatusProvedHere
\index{collision-filtration spectral sequence!collapse for $\beta\gamma$}
...
\end{equation}
The coefficient matches the entry in the table of
Theorem~\ref{thm:thqg-virasoro-tower-explicit}.
...
in type~A is proved
(\S\ref{subsec:non-principal-w-algebra-duality}); the general case
