...
$K_X(\cA)$ are level-$2$ statements (bar coalgebra and Verdier
...
\textup{(}Vol~III\textup{)} enter through separately named maps from
$K_X(\cA)$ to a CY-side object. Modular invariance of any
...
and its Verdier dual is already an algebra:
\[
K_X(\cA):=\mathbb{D}_{\Ran}\barB_X^{\mathrm{ch}}(\cA).
\]
For a quadratic presentation, the coalgebra comparison
$q_\cA\colon\cA^{\mathrm i}\to\barB_X^{\mathrm{ch}}(\cA)$ dualizes to
\[
\mathbb D(q_\cA)\colon K_X(\cA)\longrightarrow
\mathbb D_{\Ran}(\cA^{\mathrm i}).
\]
A chosen Koszul partner carries a map
$\nu_\cA\colon K_X(\cA)\to\cA^!$; formality and dualizability make it
an equivalence with $\cA^!=(\cA^{\mathrm i})^\vee$.  The cobar counit
$\Omegach_X\barB_X^{\mathrm{ch}}(\cA)\simeq\cA$ gives bar--cobar
reconstruction.
...
$K_X(\cA)$, with the usual dimension shift and orientation line.
...
is \emph{not} level-rank duality $SU(N)_k \leftrightarrow SU(k)_N$,
which exchanges different algebras. It is operadic:
$\mathrm{chirLie}^! = \mathrm{chirCom}$ in the curved second-kind
ambient, realized at the factorization algebra level by Verdier
duality on configuration spaces.
\end{remark}
...
$r^{\mathrm{KM}}(z)=k\Omega_{\mathrm{tr}}/z$, while Verdier duality negates the
shifted curvature
\[
...
K_X(\widehat{\mathfrak g}_k)\xrightarrow{\sim}
...
the signed Hodge component on the uniform-weight scalar-diagonal lane of
...
\]
and its pointed genus-$1$ trace is $(c/2)\lambda_1$. Since Virasoro has one
...
bar chain model to the corresponding component of the algebra
$K_X(\mathcal A)=\mathbb D_{\Ran}\bar B_X(\mathcal A)$.
\end{definition}
