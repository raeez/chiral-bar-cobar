...
The direct bar-side $(-1)$-shifted symplectic realization via the BV
formalism is conditional
\textup{(}Theorem~\ref{thm:shifted-symplectic-complementarity}\textup{)},
while the ambient Verdier and cyclic-deformation shifted-symplectic
structures are proved unconditionally.
The full holographic content of the upgrade is as follows.
...
Lemma~\ref{lem:center-isomorphism}).
...
(Remark~\ref{rem:theorem-C-decomposition}; note: the
na\"ive sum $\Theta_\cA + \Theta_{\cA^!} = 0$ is
\emph{false} in general).
At the scalar level,
$\kappa(\cA) + \kappa(\cA^!) = 0$ for Kac--Moody and free-field algebras;
$\kappa + \kappa' = \rho \cdot K$ for
$\mathcal{W}$-algebras (e.g.\ $\kappa(\mathrm{Vir}_c) + \kappa(\mathrm{Vir}_{26-c}) = 13$)
...
When the strict flat relative bar family is perfect
...
\begin{proof}
Perfectness of $R\pi_{g*}\barB^{(g)}_{\mathrm{flat}}(\cA)$ ensures that
...
\begin{proposition}[{Kontsevich--Pridham correspondence; \ClaimStatusProvedElsewhere;
\cite{Pridham17}}]
...
\item \emph{Perfectness:} $R\pi_{g*}\barB^{(g)}_{\mathrm{flat}}(\cA)$ is
...
\begin{proposition}[Compatibility of the BV antibracket with the
Verdier pairing; \ClaimStatusProvedHere]
...
complex, under which the BV antibracket descends to the
holographic Verdier pairing on $\mathbf{C}_g$. Concretely:
\begin{equation}\label{eq:thqg-III-compatibility-diagram}
\begin{tikzcd}
\MC(L_g) \arrow[r, "{\text{BV}}"] \arrow[d] &
...
$E_\infty$ page of the strict flat representative is
concentrated in degree~$0$ and coincides with
$\mathcal{H}^0(R\pi_{g*}\barB^{(g)}_{\mathrm{flat}}(\cA))
\cong \mathcal{Z}(\cA)$.
...
Definition~\textup{\ref{def:nms-degree-r-shadow-jet}}.
...
Definition~\ref{def:nms-degree-r-shadow-jet}. The explicit
formula \eqref{eq:thqg-III-shadow-projection} is
equation~\eqref{eq:nms-degree-r-shadow-jet}.
\end{proof}
...
\textup{(}Theorem~\textup{\ref{thm:nms-all-degree-master-equation})}:
\begin{equation}\label{eq:thqg-III-all-degree-master}
...
obstruction
\textup{(}Definition~\textup{\ref{def:nms-degree-r-obstruction})}.
...
This is Theorem~\ref{thm:nms-all-degree-master-equation}.
...
For the standard-landscape shadow depth classes:
\begin{center}
...
\textup{(}Theorem~\textup{\ref{thm:nms-all-degree-separating-boundary})}:
...
(Theorem~\ref{thm:nms-all-degree-separating-boundary}).
\end{proof}
...
The BTZ entropy $S_{\BTZ} = 2\pi\sqrt{c \cdot h_{\mathrm{eff}} / 6}$
\textup{(}$h_{\mathrm{eff}} = h - c/24$\textup{)}
...
We verify the family-by-family hypotheses entering the
complementarity package, together with the unconditional
eigenspace census, shadow depth class, and first nontrivial
shadow jet.
...
\textup{(H1)} perfectness and \textup{(H2)} nondegeneracy for the
Verdier pairing are satisfied, and the unconditional eigenspace
decomposition \textup{(C1)} holds. The shadow depth classes are:
...
& $\dim\mathfrak{g}\cdot(k+h^{\vee})/(2h^{\vee})$ & L & $3$ & cubic
& $\mathfrak{C}_{\mathrm{aff}}$
\\
$\beta\gamma$ (weight $\lambda$)
& $6\lambda^2{-}6\lambda{+}1$ & C & $4$ & quartic & $\mathfrak{Q}_{\beta\gamma}$
\\
Lattice $V_\Lambda$
...
The strict flat bar complex has finite-dimensional fiber cohomology
...
The Koszul dual $\widehat{\mathfrak{g}}_k^! = H^*(\bar{B}(\widehat{\mathfrak{g}}_k))^\vee$ has modular characteristic
...
\begin{remark}[Conditional C2 upgrade on the standard landscape]
\label{rem:thqg-III-landscape-c2}
Theorem~\ref{thm:thqg-III-landscape-census} verifies the
perfectness and nondegeneracy inputs family by family. Therefore the
conditional Verdier shifted-symplectic/Lagrangian package
\textup{(}C2\textup{)} of
Theorem~\ref{thm:thqg-III-lagrangian-polarization} applies to each
standard family. We keep that upgrade separate from the proved-here
census to avoid conflating verification of hypotheses with the
general conditional theorem itself.
\end{remark}
...
The complementarity package
is the third component of the holographic modular
...
(Proposition~\ref{thm:collision-residue-twisting}).
\end{remark}
