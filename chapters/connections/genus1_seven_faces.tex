...
At genus~$0$, the ordered collision kernel $K_\cA^{\mathrm{coll}}(z)$
has one polar profile:
...
The propagator determines the collision kernel. At genus~$0$, the
rational kernel $1/(z_1 - z_2)$ extracts Laurent coefficients; the
collision kernel $K_\cA^{\mathrm{coll}}(z)$ is the resulting element of
...
kernel $K_\cA^{\mathrm{coll}}(z)=\sum_{n\geq0}c_n/z^{n+1}$, where
...
Laurent expansion of $K_\cA^{(1),\mathrm{coll}}$ near $z = 0$ agrees with
...
The quasi-periodicity of $K_\cA^{(1),\mathrm{coll}}(z,\tau)$ is therefore inherited
entirely from the $\xi_\tau(z)$ term:
\begin{equation}\label{eq:g1sf-B-cycle-monodromy}
K_\cA^{(1),\mathrm{coll}}(z+\tau,\tau)
-K_\cA^{(1),\mathrm{coll}}(z,\tau)
\;=\;
-2\pi i \cdot c_0.
\end{equation}
...
The genus-$1$ collision kernel is the binary collision component of
the genus-$1$ twisting morphism:
\begin{equation}\label{eq:g1sf-face-1}
K_\cA^{(1),\mathrm{coll}}(z, \tau)
\;=\;
\bigl(\pi_\cA^{(1)}\bigr)_{2}^{\mathrm{coll}}(z,\tau)
\;:=\;
...
\end{equation}
Here $\pi_\cA^{(1)}\in
\mathrm{Tw}(\barBch(\cA)|_{E_\tau},\cA|_{E_\tau})$ is the complete
twisting morphism; the displayed expression records its binary
component.
...
and $H_{\mathrm{CYBE}}^{\mathrm{rep}}$ supplied by the KZB
representation $\rho_{\mathrm{KZ}}$.)
For $\cA = \widehat{\fg}_k$ with $\fg$ simple and
$k \neq -h^\vee$, the represented genus-$1$ collision kernel is the
classical elliptic $r$-matrix with the standard level normalization:
\begin{equation}\label{eq:g1sf-elliptic-rmatrix}
\rho_{\mathrm{KZ}}\!\left(
K_{\widehat{\fg}_k}^{(1),\mathrm{coll}}(z,\tau)\right)
\;=\;
\frac{k}{k + h^\vee}\,
...
The collision-depth-$2$ projection of the modular MC equation at
genus~$1$ gives the operadic three-insertion relation
\textup{(}Theorem~\ref{thm:collision-depth-2-ybe}\textup{)}.  The
KZB representation in $H_{\mathrm{CYBE}}^{\mathrm{rep}}$ carries it
to the classical Yang--Baxter equation
\eqref{eq:g1sf-elliptic-cybe}:
...
Casimir. The KZB representation sends the elliptically regularized
Cartan and root channels of the collision kernel to this $r$-matrix,
with theta-function ratios $\phi_\alpha(z, \tau)$ indexed by the
roots $\alpha$ of $\mathfrak{sl}_N$. The
identification~\eqref{eq:g1sf-elliptic-rmatrix} holds under
$H_{\mathrm{CYBE}}^{\mathrm{rep}}$ for all simple $\fg$.
\end{remark}
...
must be distinguished when the level prefix is included:
\begin{enumerate}[label=\textup{(\roman*)}]
...
$K_\cA^{(1),\mathrm{coll}}(z,\tau)$ as defined by
...
genus-$0$ \emph{pole-valued} collision \(r\)-matrix vanishes, and the
genus-$1$ pole-valued collision \(r\)-matrix is identically zero:
$r_{\beta\gamma}^{(1)}(z, \tau) = 0$. This does not say that the
ordered binary OPE contraction vanishes:
\[
\beta(z)\gamma(w)\sim (z-w)^{-1}
\]
is nonzero. The simple pole is absorbed by the \(d\log\)-bar kernel in
the pole-valued \(r_{\mathrm{coll}}\) convention, while the regular
contact transport
\(\Theta_{\beta\gamma}^{\mathrm{ord}}
=\beta\otimes\gamma-\gamma\otimes\beta\)
remains the nonzero class-\(C\) contact datum. The
...
$K_\cA^{(1),\mathrm{coll}}=(\pi_\cA^{(1)})_2^{\mathrm{coll}}$, the binary component of the genus-$1$ bar-cobar twisting
morphism \textup{(}Theorem~\textup{\ref{thm:g1sf-face-1})}.

\item \textbf{Elliptic line-operator $r$-matrix.}
$\mathsf E_2^{(1)}(r^{\mathrm{DNP},(1)})=K_\cA^{(1),\mathrm{coll}}$ for the
...
classical PVA on $E_\tau$ to the kernel $K_\cA^{(1),\mathrm{coll}}$
...
$\rho_{\mathrm{KZ}}(K_\cA^{(1),\mathrm{coll}})
=k\,r^{\mathrm{ell}}_\fg/(k+h^\vee)$
...
[\rho_{\mathrm{KZ}}(K_\cA^{(1),\mathrm{coll}}(z_i-z_j))]_{ij}$
...
\section{Finite genus-$1$ calculations}
\label{sec:g1sf-compute}

Fix $z=0.2+0.15\mathrm i$, $\tau=0.3+\mathrm i$, the standard basis
of $V=\CC^2$, and a $60$-term Jacobi-theta expansion in complex double
precision.  For $\fsl_2$ at levels $k\in\{1,2,5,10\}$, both the bar
collision scalar and the scalar elliptic kernel are, by their displayed
definitions,
\[
 \frac{\zeta(z,\tau)}{k+2}.
\]
Their four numerical differences are therefore zero; this is an
identity of formulas, not an independent proof of the seven-face
comparison.

On $V^{\otimes2}$ with ordered basis
$e_i\otimes e_j$, the two-point elliptic Gaudin Hamiltonians at the
same $(z,\tau)$ have commutator residual
$8.671\times10^{-16}$ in the entrywise maximum norm.  This
two-point calculation does not imply three-point commutativity or the
elliptic classical Yang--Baxter equation.  No three-point finite
calculation is asserted here.

For the degeneration, set $k=1$ and compare
$\theta_1'(z\mid\tau)/\theta_1(z\mid\tau)$ with
$\pi\cot(\pi z)$.  At
$\tau\in\{0.5\mathrm i,\mathrm i,2\mathrm i,5\mathrm i,10\mathrm i\}$
the relative residuals are, respectively,
\[
 2.182\times10^{-1},\quad 8.763\times10^{-3},\quad
 1.631\times10^{-5},\quad1.062\times10^{-13},\quad
 1.143\times10^{-16}.
\]
This is a finite illustration of the theta-series degeneration, whose
limit is established by the analytic argument above.  Finally,
substitution in~\eqref{eq:g1sf-vir-genus1} shows algebraically that the
$\wp'$ term vanishes at $c=0$ and is nonzero at
$c\in\{1/2,1,25,26\}$ for this $(z,\tau)$.  These calculations make no
claim about the corresponding $\cW_3$ coefficients.
...
\begin{proof}
The $B$-cycle monodromy of $K_\cA^{(1),\mathrm{coll}}$ is computed
in~\eqref{eq:g1sf-b-cycle}: it equals
...
\item \emph{Climax identity, elliptic.} The KZB connection of
 Face~F4 is the elliptic instance of the typed KZ--Arnold
 bar-superconnection theorem of Chapter~\ref{ch:climax-theorem}:
after Fulton--MacPherson boundary-residue realization, the elliptic
Arnold/KZB connection
$d-\xi_\tau(z_i-z_j)\Omega_{ij}\,d(z_i-z_j)$ is pulled through the
KZB map to produce the genus-$1$ bar differential. The Arakelov correction is
...
\item \emph{Koszul reflection at genus~$1$.} The genus-$1$
 ...
 (Theorem~\ref{thm:koszul-reflection}, Vol~I bar--cobar Theorem~A)
...
\begin{remark}[K3 two-loop comparison]
\label{rem:g1sf-cross-ref}
\ClaimStatusHeuristic{}
Conjecture~\ref{prop:bvbrst-2loop-obstruction} asks for a graded map
\[
 \Psi _2:H^1\operatorname{Obs}_{\mathrm{loc}}
   (\mathcal E_{11}^{\mathrm{tw}}(Y))\longrightarrow \bQ(y)[[q]]
\]
carrying the renormalized two-loop class $[\mathfrak o_2]$ to the
Fourier--Jacobi coefficient $[p^2]\log\Phi_{10}^{\circ}$.  The K3
topological factor in the theta integral is
$\int_{K3}c_2(TK3)=24$; the elliptic degeneration presents the same
integer as the number of Kodaira $I_1$ fibres.  The vertices,
propagator, counterterm, and regularization determine the remaining
field-theoretic coefficient.  Thus the precise bridge to
Proposition~\ref{prop:g1sf-seven-K3-faces} is the equality, under one
regularization convention, between the renormalized theta amplitude
and the sum of the corresponding local degeneration kernels.  This
equality is the first value required of~$\Psi _2$.
\end{remark}
