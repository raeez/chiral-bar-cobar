...
genus~$1$ is
$r_{\widehat{\fg}_k}^{(1)}(z, \tau) = k\,\Omega_{\mathrm{KZ}}\, \zeta_\tau(z)/(k + h^\vee)$,
by Theorem~\ref{thm:g1sf-elliptic-regularization} applied to the
Killing-form-Casimir presentation of the genus-$0$ collision residue,
consistent with the KZB connection~\cite{Bernard88} and the
Casimir-convention bridge of Lemma~\ref{lem:g1sf-casimir-bridge}.
% AP288: level prefix k/(k+h^v) mandatory in Killing-form convention;
...
\item $\Omega_{\mathrm{KZ}} := \sum_a I^a \otimes I_a$, with
$\{I^a\},\{I_a\}$ dual bases for the Killing form of~$\fg$. This
Casimir is $k$-independent and related to $\Omega_{\mathrm{tr}}$ by
$\Omega_{\mathrm{KZ}} = \Omega_{\mathrm{tr}}/(2h^\vee)$ on
simple~$\fg$, since the Killing form equals $2h^\vee$ times the
normalized trace form and the Casimir built from dual bases scales
inversely with the form
\textup{(}normalization of \cite[\S 1]{EK96}\textup{)}. It appears in
the Etingof--Kazhdan KZ/Kazhdan-normalized $r$-matrix as
...
combined with the $k$-independent identity
$\Omega_{\mathrm{KZ}} = \Omega_{\mathrm{tr}}/(2h^\vee)$
\textup{(}intrinsic to the Lie algebra\textup{)}, this reduces to
$k(k+h^\vee) = 1/(2h^\vee)$, which is a quadratic condition on
$k$ with two specific solutions, not an identity in $k$.
\emph{The two $r$-matrices are therefore genuinely different families,}
distinguished by the level prefix they carry; they are not the same
$r$-matrix in two notations. The landscape census
\textup{(}Chapter~\ref{chap:landscape-census}, class-$L$ row\textup{)}
writes ``$k\,\Omega_{\mathrm{tr}}/z = \Omega/((k+h^\vee)z)$'' as an
\emph{abuse of notation}, with the understanding that a single
uniformly-scaled Casimir symbol $\Omega$ is used that makes the
equation hold tautologically; the unrescaled intrinsic Casimirs
$\Omega_{\mathrm{tr}}$ and $\Omega_{\mathrm{KZ}}$ do \emph{not} satisfy
this equation at all $k$. This is the AP288 level-prefix discipline
...
and~\eqref{eq:g1sf-master-equation} use the \emph{Killing-form
Casimir} convention with explicit $k/(k+h^\vee)$ prefix
...
= k\,\Omega_{\mathrm{KZ}}\,\zeta_\tau(z)/(k+h^\vee)$ vanishes.
...
$k\to 0$ specialization of the Killing-form-Casimir face identified
...
$2h^\vee\,k(k+h^\vee) = 1$ and differ at $k=0$, where the first
...
\begin{proof}
(i)--(ii): The intrinsic relation
$\Omega_{\mathrm{KZ}} = \Omega_{\mathrm{tr}}/(2h^\vee)$ on
simple~$\fg$ follows from the standard identity
...
pairing gives $\Omega_{\mathrm{KZ}} = \Omega_{\mathrm{tr}}/(2h^\vee)$.
This is a $k$-independent relation between the two intrinsic Casimir
tensors.

The intertwining constraint $r^{\mathrm{Dr}}_k = r^{\mathrm{KZ}}_k$
reads $k\,\Omega_{\mathrm{tr}}/z = \Omega_{\mathrm{KZ}}/((k+h^\vee)z)$.
Substituting the intrinsic relation:
$k\,\Omega_{\mathrm{tr}}/z = \Omega_{\mathrm{tr}}/(2h^\vee(k+h^\vee)z)$,
which reduces to $2h^\vee\,k(k+h^\vee) = 1$, a quadratic equation in
...
evaluated on the Killing-form Casimir~\cite{Bernard88}. This is not
...
convention $k \neq -h^\vee$ is therefore a \emph{genuine hypothesis}
on the Faces~F5$^{(1)}$--F7$^{(1)}$ identifications, not a
typographical convenience.
\end{proof}

\begin{remark}[Cross-volume cross-check]
\label{rem:g1sf-cross-volume}
The Casimir-convention bridge of Lemma~\ref{lem:g1sf-casimir-bridge}
reveals an abuse-of-notation in the cross-volume table in
Chapter~\ref{chap:landscape-census} of this volume
\textup{(}landscape census, class~$L$ row\textup{)}: there, the
collision residue is written
``$r^{\mathrm{coll}}(z) = k\Omega_{\mathrm{tr}}/z = \Omega/((k+h^\vee)z)$''
with a single Casimir symbol ``$\Omega$'' whose implicit definition is
$\Omega := k(k+h^\vee)\,\Omega_{\mathrm{tr}}$ \textup{(}a
level-dependent rescaling that forces the equality at all $k$ by
definition\textup{)}. This differs from the intrinsic
Killing-form Casimir $\Omega_{\mathrm{KZ}} = \Omega_{\mathrm{tr}}/(2h^\vee)$
of Lemma~\ref{lem:g1sf-casimir-bridge} by a factor of
$2h^\vee\,k(k+h^\vee)$, which equals~$1$ on the quadratic locus and
differs generically. Readers applying the landscape-census formula
numerically must fix the level $k$ and the Casimir convention
simultaneously.

Volume~II \textup{(}$A_\infty$ Chiral Algebras and 3D~HT~QFT,
$\mathsf{SC}^{\mathrm{ch,top}}$ bar differential\textup{)} and
Volume~III \textup{(}CY Categories, K3 Yangian\textup{)} use the
...
AP5 propagation requires that any statement involving a numerical
value of $r(z)$ at fixed $z = z_0$, $k = k_0$ be tagged with which
Casimir convention is in force at that statement; the present chapter
tags all Face~F5$^{(1)}$--F7$^{(1)}$ statements with the Killing-form
convention $r^{(1)} = k\,\Omega_{\mathrm{KZ}}\zeta_\tau/(k+h^\vee)$
...
is immediate from Theorem~\ref{thm:g1sf-face-1} in the Killing-form
convention (Lemma~\ref{lem:g1sf-casimir-bridge}):
$r_{\widehat{\fg}_k}^{(1)}(z, \tau) = k\,\Omega_{\mathrm{KZ}}\, \zeta_\tau(z)/(k+h^\vee)$,
...
The detailed cross-volume verifications are recorded in Vol.~II; in
particular, the F3$^{(1)}$ Khan--Zeng PVA identification is a
genus-$1$ specialisation of the freely-generated-PVA-with-conformal-vector
construction of~\cite{KhanZeng25}, which is a cited physical input to
the programme and whose full topological-upgrade theorem is attributed
in Vol.~II rather than inscribed in Vol.~I.
...
the Killing-form-Casimir level normalization
\textup{(}Lemma~\ref{lem:g1sf-casimir-bridge}\textup{)}:
$r^{(1)}_{\widehat{\fg}_k}(z, \tau) \to k\,\Omega_{\mathrm{KZ}}/((k+h^\vee)z)$.
% AP288: level prefix k/(k+h^v) in Killing-form convention; at k=0 the
% Killing-form-convention r-matrix vanishes (distinct from KZ-convention
...
enlarges\textup{)}; at $k = 0$ the Killing-form-Casimir collision
...
in the Killing-form-Casimir normalization used throughout this chapter
% AP288: level prefix k/(k+h^v); equivalent to Drinfeld k*Omega_tr/z on
% k-generic locus via Lemma g1sf-casimir-bridge, but distinct at k=0.
\textup{(}cf.\ Eq.~\eqref{eq:g1sf-elliptic-rmatrix} and the
Killing-form-Casimir presentation of the genus-$0$ collision residue
$r_{\widehat{\fg}_k}(z) = k\,\Omega_{\mathrm{KZ}}/((k+h^\vee)z)$,
which equals the KZ-convention residue
$\Omega_{\mathrm{KZ}}/((k+h^\vee)z)$ of Theorem~\ref{thm:hdm-face-5}
$k$-generically under Lemma~\ref{lem:g1sf-casimir-bridge} but differs
at $k = 0$\textup{)}.
...
\label{sec:g1sf-wave14-anchors}

\begin{remark}[Wave-$14$ anchor map for the genus-$1$ seven faces]
\label{rem:g1sf-wave14-anchors}
\index{wave-14 anchors!genus-1 seven-faces entry point}
\index{cross-volume bridge!genus-1 seven faces}
The genus-$1$ seven-face theorem
(Theorem~\ref{thm:g1sf-master}) sits at the elliptic specialisation
of three wave-$14$ structural anchors:
...
\item \emph{Climax identity, elliptic.} The KZB connection of
 ...
 Arnold connection $d - \zeta_\tau(z_i - z_j) \cdot \Omega_{ij}\,
 d(z_i - z_j) - 2\eta_1(\tau)\,\Omega_{ij}\,dz_i$ through the KZB
 map produces the genus-$1$ bar differential, with the
 quasi-periodicity correction encoded by the
 $-3/(\pi\,\mathrm{Im}\,\tau)$ Eisenstein anomaly.
...
celestial holography (Vol~II Theorem~\ref{thm:universal-holography-master})
...
are the holographic line defects in this 3d theory. Vol~III's
CY-to-chiral functor~$\Phi$ realises the elliptic seven faces for
$\Phi(\mathrm{K3} \times E)$ in the BPS-counting category, with
the Borcherds product $\Phi_N$ supplying the BKM character whose
elliptic specialisation matches the $E_\tau$-collision residue.
\end{remark}

\section{seven faces of r(z) meet the Wave-13 crown}
\label{sec:g1sf-wave13-DNA}

\begin{remark}[Seven faces of $r(z)$ and the Wave 13 K3 crown]
\label{rem:g1sf-DNA-seven-faces}
Vol~I seven-faces analysis of the genus-$1$ $r$-matrix $r(z)$ specialises at K3 to the
Mukai-enhanced context. The Koszul conductor on the B-family is
$K^{\kappa_{\mathrm{ch}}}(\mathcal{H}_{\mathrm{Muk}}(K3)) = 2\,c_+(\mathrm{Mukai}(K3)) = 8$.
Wave~13 identifies three independent faces of this $8$ via Bruinier $2002$ Proposition~$5.1$
...
All three coincide as the same $\mathbb{Z}/8$-class in $\mathrm{CH}^1(H_1)$. Vol~I
Theorem~C bucket enlarges to $\{0, 8, 13, 250/3, 98/3\}$.
\end{remark}

\begin{remark}[Siegel-elliptic dynamical $R$-matrix as eighth face]
\label{rem:g1sf-DNA-R-sieg}
Beyond the seven faces of genus-$1$ $r(z)$ catalogued in Vol~I, Wave~13 adds an eighth
at $K3 \times E$: the Siegel-elliptic dynamical $R$-matrix $R_{\mathrm{Sieg,dyn}}(u - v;
\tau, \rho, z)$ built from the genus-$2$ Riemann theta $\theta[\alpha/\beta](w; \tau, z, \rho)$
on the abelian surface $A_\tau$ with period matrix $\bigl(\begin{smallmatrix}\tau & z\\ z & \rho\end{smallmatrix}\bigr) \in \mathbb{H}_2$, via Pasol--Zagier $2013$ extension of
Kronecker--Eisenstein. Fourth $R$-matrix class beyond Drinfeld--Belavin's rational /
trigonometric / elliptic triple. Dynamical YBE on paramodular $K(1) \subset \mathbb{H}_2$;
off $K(1)$ the YBE has an elliptic anomaly proportional to the Jacobi index $1/2$ of
$\Delta_5$.
\end{remark}

\section{seven faces of $r(z)$ at K3-genus-$1$}
\label{sec:g1sf-wave15-DNA}

The Vol~I genus-$1$ catalogue of seven faces $F1$--$F7$ of the
collision residue $r(z)$ specialises at K3 to seven \emph{concrete}
face-instances on the elliptic fibre $E_\tau$ of the elliptic K3 with
24 Kodaira $I_1$ fibres. Wave~$15$ tabulates these and identifies each
with a specific face of the Wave~$13$ Siegel-elliptic dynamical
$R$-matrix $R_{\mathrm{Sieg,dyn}}$.

\begin{proposition}[Seven K3-genus-$1$ faces of $r(z)$; Wave~$15$]
\label{prop:g1sf-wave15-seven-K3-faces}
\index{seven faces!K3-genus-1|textbf}
\index{R-matrix@$R$-matrix!K3 catalogue|textbf}
\ClaimStatusProvedElsewhere{} \textup{(}individual face equivalences:
...
structure}, \widetilde{M}_{24})$, the seven Vol~I faces of the genus-$1$
$r$-matrix admit seven distinct K3-modular face-instances:
\begin{enumerate}
\item \textbf{(Rational face.)} On the smooth cover
$E_\tau^{\mathrm{sm}}\setminus\{24\}$, the Belavin--Drinfeld rational
$r$-matrix $r^{\mathrm{rat}}(z) = \Omega/z$ recovers the base-affine
chiral OPE of the $24$ Heisenberg stalks.
\item \textbf{(Trigonometric face.)} Near each Kodaira node
$p_i \in E^{\mathrm{nod}}_{24}$, the nearby-cycles $\mathcal{D}$-module
yields the trigonometric Belavin--Drinfeld $r^{\mathrm{trig}}(z)$
with spectral parameter $q_i = e^{2\pi i\tau_i}$ capturing the
$I_1$-monodromy.
\item \textbf{(Elliptic Belavin face.)} Away from nodes, the
Belavin elliptic $r^{\mathrm{ell}}(z; \tau) = \theta_1'(0)/\theta_1(z)\cdot\Omega$
(Belavin 1981; Belavin--Drinfeld 1982) on $E_\tau\setminus\{0\}$ is
Face~$5$ of the Vol~I catalogue.
\item \textbf{(Mukai-twisted face.)} The Mukai enhancement
$\mathcal{H}_{\mathrm{Muk}}(K3) = H^{\mathrm{ev}}(K3, \bZ)$ twists
$r^{\mathrm{ell}}$ by the Serre-functor cocycle of order $\widetilde{M}_{24}$
(Wave~$13$ §E.$5$), producing
\[
 r^{\mathrm{Muk}}(z; \tau) \;=\; r^{\mathrm{ell}}(z;\tau) + \mathrm{Serre}^{\widetilde{M}_{24}}(z),
\]
valued in $\mathrm{End}(\mathcal{H}_{\mathrm{Muk}})$ with the
$K^{\kappa_{\mathrm{ch}}} = 8$ Koszul conductor encoding Mukai
doubling. Face~$5$-variant.
\item \textbf{(Harvey--Moore face.)} The $1$-loop heterotic
threshold integrand on $K3\times T^2$
\cite{HarveyMoore1996} produces
\[
 r^{\mathrm{HM}}(z;\tau)
 \;=\;
...
data; this is the CFT incarnation of the Bruinier $2002$ Heegner
Chern-class Lie bialgebra.
\item \textbf{(Borcherds-product face.)} The singular-theta lift of
the K3 elliptic genus $\phi_{0,1}$ by Borcherds 1998 produces the
...
\]
whose Hecke-like symmetry encodes the $24$-node combinatorics as
multiplicities $m(\alpha)$.
\item \textbf{(Siegel-dynamical face.)} At the crown: the
Siegel-elliptic dynamical $R$-matrix
$R_{\mathrm{Sieg,dyn}}(u-v; \tau,\rho,z)$ of
Remark~\ref{rem:g1sf-DNA-R-sieg} lifts $r^{\mathrm{ell}}$ to the
paramodular $K(1)\subset\mathbb{H}_2$ locus; this is the Wave~$13$
eighth face, but under the genus-$1$ specialisation
$\rho\to i\infty$ collapses back to Face~$3$ (elliptic Belavin)
plus a dynamical correction of Pasol--Zagier type.
\end{enumerate}
Faces $1$--$3$ are Vol~I canonical; faces $4$--$7$ are Wave~$13$--$15$
K3-specific refinements. At the Humbert divisor $H_1\cup H_4
\subset\overline{\mathcal{A}_2}$, faces $4$--$7$ collapse onto the
$\ell=8$ monodromy class of Remark~\ref{rem:g1sf-DNA-seven-faces}
(Wave~$13$ proposition; Mukai doubling, Humbert monodromy, Lusztig
specialisation).
\end{proposition}

\begin{proof}[Sketch; pointers to primary literature]
Face $1$ (rational): Drinfeld $1983$; Vol~I Face~$5$ rational
specialisation (Chapter~\ref{ch:seven-faces}). Face $2$
(trigonometric): Belavin--Drinfeld $1982$ Theorem~$3.1$; Reshetikhin
$1992$. Face $3$ (elliptic Belavin): Belavin $1981$ §$3$;
Belavin--Drinfeld $1982$ Theorem~$1.2$. Face $4$ (Mukai-twisted):
Mukai $1981$ $K3$ Torelli; Caldararu $2002$ derived categories of K3;
Wave~$13$ Costello-Cycle-$1$ CY-$2$ $[2]$-shift argument (K3 is
CY-$2$, so $\mathrm{Serre}^{\widetilde{M}_{24}}$ shifts by $[2]$ not
$[3]$). Face $5$ (Harvey--Moore): Harvey--Moore $1996$ arXiv:hep-th/$9510182$
equation ($5.22$) for the threshold integrand; Bruinier $2002$
Chapter~$5$ for the Heegner divisor / Chern-class reciprocity. Face
$6$ (Borcherds-product): Borcherds $1998$ Invent.~Math.~$132$ pages
$491$--$562$; Gritsenko--Nikulin $1997$ additive lift yields the same
$\Delta_5$. Face $7$ (Siegel-dynamical): Pasol--Zagier $2013$
Kronecker--Eisenstein extension; Wave~$13$ Etingof-Cycle-$1$
dynamical-YBE verification on paramodular $K(1)$.

Collapse at Humbert divisor: Bruinier $2002$ Proposition~$5.1$
Heegner-Chern-class identity; the $\ell = 8$ Lusztig specialisation
$\zeta^8 = 1$ with $\hbar^2 = -1/8$ (Wave~$13$ D.$6$) is the common
image of all four K3-specific faces under the Humbert-stratum
restriction.
\end{proof}

\begin{remark}[Faces vs.\ specialisations; naming discipline]
\label{rem:g1sf-wave15-faces-vs-specializations}
The Wave~$15$ seven K3-genus-$1$ faces are not seven independent
$r$-matrices but seven \emph{face-instances} of a single object:
the K3-modular collision residue. The base genus-$1$ $r$-matrix is
$r^{\mathrm{ell}}$; the four K3-specific refinements
(Mukai-twist / Harvey--Moore / Borcherds-product / Siegel-dynamical)
are functorial enhancements capturing, respectively, the CY-$2$
Mukai doubling, the Narain-lattice CFT content, the singular-theta
lift, and the genus-$2$ paramodular dynamical structure. All agree
on the smooth Ran-space generic stalk $E_\tau^{\mathrm{sm}}
\setminus\{24\}$ (Face $1$ rational + Face $3$ elliptic); they
diverge on the $24$-node stratum (Face $2$ trigonometric) and the
Humbert walls (Faces $4$--$7$). This is the Beilinson bi-based
F-datum (Wave~$13$ E.$3$) realised at the level of $r$-matrices.
\end{remark}

\begin{remark}[Cross-volume: Wave~$15$ ties seven faces to BV
obstruction vanishing]
\label{rem:g1sf-wave15-cross-ref}
The vanishing of the $2$-loop BV obstruction class
(Proposition~\ref{prop:bvbrst-wave15-2loop-obstruction}) and the
seven K3-genus-$1$ face catalogue of
Proposition~\ref{prop:g1sf-wave15-seven-K3-faces} are linked by the
Costello twisted-$11$D-SUGRA holography: the theta-graph $\chi(K3) = 24$
contribution at $2$-loop is precisely the sum over the $24$ Kodaira
nodes of the Face~$2$ (trigonometric) spectral-parameter contribution,
and the cancellation against the $24\cdot\chi(\cO_{K3}) = 24$
dumbbell-plus-figure-eight sum is the Face~$1$+Face~$5$ (rational
plus Harvey--Moore) counter-contribution.
\end{remark}
