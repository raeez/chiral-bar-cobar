...
\S\ref{sec:g1sf-prime-form-conventions}), $\theta_1$ is the Jacobi
...
\label{sec:g1sf-prime-form-conventions}
...
Drinfeld $r$-matrix $\Omega/((k+h^\vee)z)$
...
$\zeta_\tau(z) \cdot H \otimes H/2$ in the Cartan channel.
...
Hamiltonians $H_i^{\mathrm{Gaudin}} = \sum_{j \neq i}\Omega_{ij}/(z_i - z_j)$
(Theorem~\ref{thm:hdm-face-7}).  At genus~$1$, the Gaudin model
...
terminates at $c_0 = \Omega/(k+h^\vee)$: the collision depth is
...
$z = 0$ gives
$2T/z - (c/4)\cdot(-3!/z^4 + O(1)) = 2T/z + (3c/2)/z^4 + \cdots$;
the leading pole $2T/z$ matches the genus-$0$ collision residue at
order $z^{-1}$, and the $z^{-4}$ pole matches $3c/z^4 \cdot (-1/2)$,
consistent with $c_2 = c/2$ and the Laurent coefficient
$\wp'(z) = -2/z^3 + O(z)$.
...
$r_\cA^{(1)} = r^{\mathrm{ell}}_\fg/(k + h^\vee)$
\textup{(}Theorem~\textup{\ref{thm:g1sf-face-5})}.
...
$H_i^{\mathrm{ell}} = (k + h^\vee)\, [r_\cA^{(1)}]_{ij}$
...
H_z^{\mathrm{KZB}}
\;=\;
\frac{r^{\mathrm{ell}}_\fg}{k + h^\vee}
\;=\;
\frac{H_i^{\mathrm{ell}}}{k + h^\vee}
\;}
\end{equation}
...
$r^{\mathrm{ell}}_\fg(z, \tau) \to \Omega/z$ through the
...
$\sum_{j \neq i}\Omega_{ij}/((k+h^\vee)(z_i - z_j))\, dz_{ij}$,
...
rational $r$-matrix $\Omega/z$.  Both limits are continuous in the
topology of formal Laurent series.
\end{proof}
...
2\eta_\tau \cdot c_0.
\end{equation}
For $\cA = \widehat{\fg}_k$, $c_0 = \Omega/(k + h^\vee)$, and the
monodromy is $2\eta_\tau\, \Omega/(k + h^\vee)$.  The
