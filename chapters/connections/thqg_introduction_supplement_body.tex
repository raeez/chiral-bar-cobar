...
presented as a diagram of three vertices and three edges.  We now
construct it from the bar complex alone, by progressively extracting
...
\index{bar complex!as Swiss-cheese algebra}
...
\item The Koszul dual~$\cA^!$ is the Verdier dual of the bar
  cohomology: $\cA^! = (H^*(\barB_X(\cA)))^\vee$.  By Theorem~A,
...
\item The spectral $r$-matrix is the binary genus-$0$ collision residue
  ...
  (\S\ref{sec:thqg-intro-collision-filtration}).
...
\begin{theorem}[Shadow $=$ KZ connection;
\ClaimStatusProvedElsewhere]
\label{thm:thqg-intro-shadow-kz}
The shadow connection $\nabla^{\mathrm{hol}}$ restricted to genus~$0$
with~$n$ marked points recovers the Knizhnik--Zamolodchikov connection
on the bundle of conformal blocks:
\[
\nabla^{\mathrm{KZ}}
\;=\;
...
$L_\infty$-obstruction to lifting the KZ connection from genus~$0$ to
genus~$1$:
\[
\mathfrak{Q}(\cA)
\;=\;
...
\section{The shadow connection and conformal blocks}
...
the moduli of marked curves.  This connection governs the transport
of derived conformal blocks across the moduli
space~$\overline{\mathcal{M}}_{g,n}$.  Its genus-$0$ specialization
is the Knizhnik--Zamolodchikov connection; its higher-genus extensions
produce the BPZ differential equations; its flatness is guaranteed by
the MC equation.
...
\to \overline{\mathcal{C}}_{g,n}$.  The bundle of conformal blocks
is the sheaf on~$\overline{\mathcal{M}}_{g,n}$ defined by
...
points.  For conformal vertex algebras, the fiber over a smooth
curve~$\Sigma_g$ with marked points $z_1, \ldots, z_n$ is the
...
The \emph{modular shadow connection} on the bundle of conformal blocks
is the connection
...
component acting on the fiber of conformal blocks at genus~$g$
...
The flatness of the shadow connection is the statement that conformal
blocks form a local system on $\overline{\mathcal{M}}_{g,n}$: parallel
transport between any two points is path-independent (up to homotopy).
This is a known result for conformal vertex algebras at integrable level
(Tsuchiya--Ueno--Yamada~\cite{TUY89}); the shadow-connection
construction extends it to all Koszul chiral algebras via the bar-intrinsic
MC element.
...
normalizing the $r$-matrix by the modular characteristic:
$r(z) = \kappa^{-1}(\cA) \cdot \Omega / z$ for
...
The KZ equations on conformal blocks are the parallel-transport
equations for this flat connection.

\subsection{The BPZ equations from the shadow tower}
\label{subsec:thqg-intro-bpz}
\index{BPZ equations!from shadow tower}

For the Virasoro algebra $\cA = \mathrm{Vir}_c$, the shadow connection
at genus~$0$ with $n + 1$ marked points---one carrying a degenerate
module $M_{r,s}$ and the rest carrying generic modules---produces the
...
consistent, and the transported cycle produces the BPZ equation at each
...
of conformal blocks extends across the boundary of
$\overline{\mathcal{M}}_{g,n}$.  The monodromy around a boundary
...
\index{conformal blocks!higher genus}
...
the shadow Postnikov tower at arity~$r$ and genus~$1$.

The term $\kappa(\cA) \cdot \omega_1$ is the curvature of the Hodge
bundle, coupling the conformal blocks to the complex structure of the
elliptic curve.  The higher-arity corrections encode the nonlinear
dependence of conformal blocks on the modular parameter~$\tau$: the
...
tower terminates at arity~$2$), and the genus-$1$ conformal blocks
transform by multiplication by $\eta(\tau)^{-k}$, where $\eta$ is the
Dedekind eta function.  For the Virasoro algebra, the infinite shadow
tower produces the full modular differential equation satisfied by
Virasoro conformal blocks on the torus.
...
\subsection{The four shadow-depth classes}
\label{subsec:thqg-intro-four-classes}
\index{shadow depth!four classes}
...
The standard landscape of chiral algebras falls into four classes
according to shadow depth
...
obstruction.  In class~\textbf{M}, all obstructions are nonzero.)
...
an effective interaction vertex.  The shadow Postnikov tower is an
...
\item \emph{Arity $2$: the free-field kernel.}
  ...
  $\kappa(\psi_k) = k/2$ for fermions.  In the EFT language,
...
Postnikov tower is the Taylor expansion of the full MC element
...
The shadow depth is expected to equal the $A_\infty$-depth of the
bar cohomology:

\begin{conjecture}[Operadic complexity;
Conjecture~\ref{conj:operadic-complexity}]
\label{conj:thqg-intro-operadic-complexity}
...
$r$-ary bracket.  The genus-$0$ case of this conjecture is proved
(Proposition~\ref{prop:shadow-massey-identification}); the all-genera
case is the shadow-formality identification at low arities
...
(Theorem~\ref{thm:koszul-equivalences-meta}, twelve equivalent
conditions) is independent of the shadow depth classification.
...
acts on the bar coalgebra by
$\mathbb{D}_{\operatorname{Ran}}\,\barB_X(\cA) \simeq \barB_X(\cA^!)$
(Theorem~A).  The induced involution on the ambient complex is
...
(Theorem~\ref{thm:shifted-symplectic-complementarity}).  The shift
...
$\kappa(\cA) + \kappa(\cA^!) = 0$ (for algebras with
$\dim H^2_{\mathrm{cyc}} = 1$).  The \emph{critical locus} is the
...
$\kappa(\mathrm{Vir}_c) + \kappa(\mathrm{Vir}_{26-c}) = 0$ by the
complementarity $c + c' = 26$, and the critical locus is $c = 13$
(the Virasoro self-dual point).  For the $\beta\gamma$ system,
$\kappa(\beta\gamma) = -1$ and $\kappa((\beta\gamma)^!) = +1$; the
system is never critical but the tensor product
$\beta\gamma \otimes (\beta\gamma)^!$ has $\kappa = 0$.
...
the anti-symmetry of~$\kappa$ together with the Sugawara
formula/Drinfeld--Sokolov formula for the central charge.  The
explicit values are:
\begin{center}
\small
\begin{tabular}{lll}
...
$\beta\gamma$ & $-2$ & \\[2pt]
Virasoro $\mathrm{Vir}_c$ & $26$ & $c = 13$ self-dual
\end{tabular}
\end{center}
...
\end{align}
The anti-symmetry at each level is a formal consequence of the
Verdier anti-involution on the modular convolution algebra: $\sigma$
acts as $-1$ on the odd-arity components of $\Defcyc^{\mathrm{mod}}(\cA)$.
The duality of the shadow tower means that the gravitational complexity
...
completion of the dg-shifted Yangian with genus corrections.  Its
$R$-matrix
...
\end{equation}
is a Maurer--Cartan element in the completed convolution algebra,
and the equation $R^{\mathrm{mod}} \star R^{\mathrm{mod}} = 1$ (the
quantum Yang--Baxter equation for the modular $R$-matrix) is a
consequence of the MC equation $D\Theta + \frac{1}{2}[\Theta,\Theta] = 0$
at the level of the modular convolution algebra.

At genus~$0$, the modular $R$-matrix reduces to the classical
$r$-matrix: $R^{(0)}_T(z) = r(z)$.  At genus~$1$, the correction
$R^{(1)}_T(z)$ encodes the elliptic deformation: the $r$-matrix
acquires a dependence on the modular parameter $\tau$ of the torus,
controlled by the modular characteristic~$\kappa(\cA)$.  The
genus expansion of the modular $R$-matrix is the perturbative
expansion of the holographic $S$-matrix in powers of the string
coupling constant~$\hbar = g_s$.
...
$U_q(\mathfrak{g})$-modules.  The bar complex provides both arrows:
the KZ connection is the genus-$0$ shadow connection, and the
quantum group is the formal completion of the dg-shifted Yangian.
...
The shadow Postnikov tower is an algebraic construction: it lives in
...
\sum_{g \ge 1}
\mathrm{obs}_g(\cA) \cdot \hbar^{2g-2}
\;=\;
...
$\mathrm{obs}_g = \kappa \cdot \lambda_g$ are the Chern characters
...
perspective, the ghost system is the cobar of the matter Virasoro
bar complex: $\Omega(\barB(\mathrm{Vir}_c))$ has central charge
$26 - c$, and the BRST complex
$\barB(\mathrm{Vir}_c) \otimes \Omega(\barB(\mathrm{Vir}_c))$
has total central charge $c + (26 - c) = 26$
...
\[
\kappa(\mathrm{Vir}_c \otimes \mathrm{Vir}_{26-c})
= \kappa(\mathrm{Vir}_c) + \kappa(\mathrm{Vir}_{26-c})
= \frac{c}{2} + \frac{26-c}{2} - 13 = 0,
\]
using the additivity of $\kappa$ under tensor product and the
anti-symmetry $\kappa(\cA) + \kappa(\cA^!) = 0$ at the self-dual
point.  The critical dimension is not an input but a consequence
of Koszul duality.
...
spaces (in the sense of Moriwaki~\cite{Moriwaki26}), and
...
each building on the bar-complex foundation of Part~I and extending
outward through the standard landscape (Part~II), the bridges to
mathematical physics (Part~III), and the frontier (Part~IV).  The
...
The nonlinear characteristic layer (Part~I,
...
The shadow connection
(\S\ref{sec:thqg-intro-shadow-connection}) extends the KZ
connection to all genera.  Its flatness is the MC equation; its
monodromy is the genus tower; its specialization to degenerate
modules produces the BPZ equations.  The KZ equations are the
genus-$0$ case; the genus-$1$ extension involves the Hodge bundle
and the modular characteristic~$\kappa$.
...
the passage from the KZ connection to the quantum group
...
between proved and open territory.  MC1--MC5 are all proved.  The
remaining frontier is MC3 in arbitrary simple type
(Conjecture~\ref{conj:mc3-arbitrary-type}).  The completed bar-cobar
...
Part~III connects the engine to Feynman diagrams, BV-BRST quantization
(Chapter~\ref{ch:bv-brst}), Yang--Mills boundary theory, $\En$ Koszul
duality, and geometric Langlands.  The bar complex is the BRST
...
factorization; together they form the Swiss-cheese algebra on
...
(Theorem~\ref{thm:koszul-equivalences-meta}) provides twelve equivalent
characterizations of the Koszul property for chiral algebras.  All
twelve are proved.  The conditions range across several mathematical
disciplines:
\begin{enumerate}[label=\textup{(\roman*)}]
\item $A_\infty$-formality of the bar cohomology.
\item Shadow-formality: the shadow tower is equivalent to the
...
\end{enumerate}
The equivalences are proved by chain-level arguments connecting each
condition to the degeneration of the bar spectral sequence at~$E_2$.
...
Shadow tower & projections of $\Theta_\cA$ \\[2pt]
...
BPZ equations & null vectors + shadow connection \\[2pt]
HS-sewing & OPE growth bounds \\[2pt]
$c = 26$ & Virasoro Koszul duality + anti-symmetry of $\kappa$
\end{tabular}
\end{center}
...
Free fermion $\psi_k$ & $k/2$ & $1$ & $2$ & G \\[2pt]
...
full proofs and computations appear in Parts~I--IV of Volume~I and in
Volume~II.
