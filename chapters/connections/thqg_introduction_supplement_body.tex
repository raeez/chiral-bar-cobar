...
bulk-boundary-line triangle. The holomorphic direction supplies the
...
\cA^i:=H^*(\barB_X(\cA)),
\]
the bar-dual coalgebra. The line algebra $\cA^!$ is obtained from
$\cA^i$ only after finite-type or completed continuous Verdier
duality.
...
canonical object is the Verdier-dual factorization companion
...
embedding of the bar shadow. At genus $g \ge 1$, on the uniform-weight
lane the trace image carries the scalar obstruction
$\mathrm{obs}_g(\cA)=\kappa(\cA)\lambda_g$ in the modular obstruction
tower.
...
\item The bar cohomology
 $\cA^i=H^*(\barB_X(\cA))$ is the Koszul-dual coalgebra.
\item The strict Verdier/Koszul dual algebra
 $\cA^!=(\cA^i)^\vee$ is obtained from $\cA^i$ under the finite-type
 or completed continuous duality convention. By Theorem~A,
 $\mathbb{D}_{\operatorname{Ran}}\,\barB_X(\cA) \simeq \cA^!_\infty$
 (the homotopy Koszul dual factorization algebra).
...
genus-$1$ obstruction
$\mathrm{obs}_1(\cA) = \kappa(\cA) \cdot \lambda_1$ vanishes. This
does not by itself kill the higher shadow classes. On a physical
comparison surface it is the scalar anomaly-cancellation condition for
...
rational kernel $r^{KM}(z) = k\,\Omega/z$ is the semiclassical part
...
collision kernel~$r(z)$. At genus~$1$, when the MC lift exists,
...
\(\mathrm{obs}_g(\cA)=\kappa(\cA)\lambda_g\) (proved for uniform-weight algebras at all genera, and unconditionally at $g=1$) are the Chern characters
...
bulk/boundary/line triangle in~$3$d holomorphic-topological QFT\@. The
...
\item Theorem~H: chiral Hochschild amplitude $[0,2]$ and
 Koszul-dual Hochschild duality on the Koszul locus.
...
Bar-dual coalgebra $\cA^i$ & chiral bar cohomology \\[2pt]
Verdier dual $\cA^!$ & Theorem~A plus finite-type/completion hypotheses \\[2pt]
