...
The holographic triangle of \S\ref{sec:holographic-triangle} has three
vertices and three edges. Its algebraic spine is the ordered bar
complex. Its line vertex uses Verdier strictification. Its closed
cochain slot is the chiral derived centre. A physical
three-dimensional holomorphic-topological bulk appears only after
closed-open comparison data have been supplied. The typed layers are
the Swiss-cheese structure on the derived-centre pair, the algebraic
closed-boundary-line triangle, the seven-entry holographic datum
constrained by the Maurer--Cartan equation; and recovery on the stated
modular homotopy surface.
...
\index{bar complex!$\Eone$ coassociative coalgebra}
The \emph{$\Eone$-chiral coassociative structure} on $\barB_X(\cA)$ is the
pair $(d_C, d_R)$ satisfying~\eqref{eq:thqg-intro-swiss-cheese-squares}.
The differential $d_C$ is $\Einf$-chiral (factorization on~$X$);
the coproduct $d_R$ is $\Eone$-topological (deconcatenation on
ordered configurations of~$\mathbb{R}$)
(Theorem~\ref{thm:bar-swiss-cheese}).
The $\mathrm{SC}^{\mathrm{ch,top}}$ two-colour datum emerges not on
$\barB_X(\cA)$ itself but on the derived center pair
$(C^\bullet_{\mathrm{ch}}(\cA,\cA),\,\cA)$; see Volume~II\@.
\end{definition}

The two-component decomposition is the geometric input for the
closed-sector--boundary--line triangle. The holomorphic direction supplies the
bar complex and, after the chiral Hochschild comparison, the derived
centre $\mathbf{C}_{\mathrm{ch}}(\cA)$. The topological direction
records ordered module transport; after Verdier strictification it
acts on modules over~$\cA^!$. The boundary is the chiral algebra~$\cA$
at the $\mathbb{R}=0$ slice of~$\Sigma$.

\subsection{The closed-sector--boundary--line triangle}
\label{subsec:thqg-intro-bbl-triangle}
\index{closed-sector--boundary--line triangle!construction}

The three vertices of the holographic triangle
(\S\ref{subsec:triangle-diagram}) have the following typed
construction.
...
differential $d_C$ acts on $\barB_X(\cA)$ and produces its bar
cohomology. A chosen quadratic presentation supplies
\[
\cA^{\mathrm i}=C_X(s^{-1}V,s^{-2}R),\qquad
q_\cA\colon\cA^{\mathrm i}\longrightarrow\barB_X(\cA).
\]
On the Koszul locus, $H^\bullet(q_\cA)$ identifies the cohomology of
this quadratic model with bar cohomology. The strict line algebra is
$(\cA^{\mathrm i})^\vee$, while the chain-level line algebra is
$K_X(\cA)=\mathbb D_{\Ran}\barB_X(\cA)$.

\smallskip\noindent\textbf{Derived-centre closed-sector slot.}
The cochain-level closed-sector slot is the chiral derived centre
\[
...
\]
Its genus-$g$ ambient complex is
$\mathbf{C}_g(\cA) = R\Gamma(\overline{\mathcal{M}}_g,\,
\mathcal{Z}(\cA))$, where
$\mathcal{Z}(\cA)$ is the corresponding centre local system. The
ambient complex receives a map from the bar complex via the trace:
...
\smallskip\noindent\textbf{Lines.}
On the finite-type Verdier/Koszul comparison locus, the line-operator
category is modelled by
$\mathsf{Line}(\cA)=\cA^!\text{-}\mathsf{mod}$
(\S\ref{subsec:vertex-lines}). Before that strictification, the
canonical homotopy line algebra is the Verdier algebra
$K_X(\cA):=\mathbb{D}_{\operatorname{Ran}}\barB_X(\cA)$.
The chosen-pair map
$\nu_{\cA}\colon K_X(\cA)\to\cA^!$ is an equivalence on the completed
finite-type comparison locus.
The open-sector differential $d_R$ records the ordered-configuration
module structure; it becomes an action of the strict algebra~$\cA^!$
only after the Verdier comparison and compact-generator hypotheses are
met.
...
three independent objects connected by ad hoc functors: the boundary
bar construction, the bar-dual coalgebra, the Verdier line algebra,
and the chiral derived centre are linked by the same ordered
bar-complex and its comparison functors.
...
\item \emph{Boundary $\to$ derived-centre slot} (bar trace).
 ...
 coalgebra into the ambient derived-centre complex. On the genus-$0$
 faithful-trace comparison surface this map is represented by an
embedding of the bar shadow.  At genus $g\ge1$, $H_D^K$ gives the
perfect object $\mathfrak O_g^K(\cA)=
\kappa(\cA)\lambda_{-1}(\mathbb E_g)$ and its signed Hodge character.

\item \emph{Derived-centre slot $\to$ Lines} (complementary shadow).
 The complementarity decomposition
 $\mathbf{C}_g(\cA) \simeq \mathbf{Q}_g(\cA) \oplus \mathbf{Q}_g(\cA^!)$
 identifies the cofiber of the trace-shadow inclusion, when the
 inclusion is represented by a cofibration:
 $\operatorname{cofib}(\mathbf{Q}_g(\cA) \hookrightarrow \mathbf{C}_g(\cA))
 \simeq \mathbf{Q}_g(\cA^!)$.
 The cofiber is the dual shadow inside the ambient centre complex.
 On the Verdier-strict comparison locus it supplies the $\cA^!$ branch
 whose module category is $\mathsf{Line}(\cA)$.
...
\item \emph{Boundary $\to$ Lines} (Koszul duality).
 ...
 \cA^i$.  On the finite-type Koszul locus, Verdier duality turns
 $\cA^i$ into the line algebra $\cA^!$; in the completed case one uses
 the continuous dual with the Mittag--Leffler and separatedness
 hypotheses. The module category $\cA^!\text{-}\mathsf{mod}$ is the
 categorical output only on this strict comparison locus.
\end{enumerate}
...
independent objects. Its entries are constructed from the boundary
algebra by the bar construction, Verdier duality, chiral Hochschild
cochains, collision residue, the MC element, and the shadow connection.

\begin{enumerate}[label=\textup{(\roman*)}]
\item The boundary algebra~$\cA$ is the input.
\item A chosen presentation supplies
 $\cA^{\mathrm i}=C_X(s^{-1}V,s^{-2}R)$ and
 $q_\cA\colon\cA^{\mathrm i}\to\barB_X(\cA)$.
\item The strict Verdier/Koszul dual algebra
 $\cA^!=(\cA^{\mathrm i})^\vee$ is obtained from
 $\cA^{\mathrm i}$ under the finite-type
 or completed continuous duality convention. By Theorem~A, the
 homotopy dual algebra is
 $K_X(\cA):=\mathbb{D}_{\operatorname{Ran}}\barB_X(\cA)$, and the
 quadratic comparison is
 $\mathbb D(q_\cA)\colon K_X(\cA)\to
 \mathbb D_{\operatorname{Ran}}(\cA^{\mathrm i})$. The
 pair datum supplies
 $\nu_{\cA}\colon K_X(\cA)\xrightarrow{\sim}\cA^!$ on the completed
 finite-type surface.
\item The cochain-level closed-sector slot is
 $\mathbf{C}_{\mathrm{ch}}(\cA)
 =Z^{\mathrm{der}}_{\mathrm{ch}}(\cA)$, the chiral Hochschild
 endomorphism algebra of~$\cA$ as a chiral bimodule over itself.
\item The spectral $r$-matrix is the binary genus-$0$ collision residue
 ...
 \end{equation}
 After Verdier strictification and the line-kernel comparison, the
 same kernel may be written in $\cA^!\widehat{\otimes}\cA^!$
 coordinates.
...
The line category
$\mathsf{Line}(\cA)=\cA^!\text{-}\mathsf{mod}$
is attached to the third entry. It is not the closed-sector entry
$\mathbf{C}_{\mathrm{ch}}(\cA)$.

The datum is subordinated to the single identity $D_\cA^2 = 0$. The
MC entry satisfies
$D\Theta + \frac{1}{2}[\Theta,\Theta] = 0$; the remaining entries are
the bar, Verdier, Hochschild, residue, and connection constructions
attached to that MC surface. The holographic construction extracts its
physical comparisons from these typed algebraic objects.

\subsection{Recovery on the modular surface}
\label{subsec:thqg-intro-reconstruction}
\index{holographic datum!recovery theorems}

On a fixed modular homotopy classification surface, the seven-entry
datum and the typed algebraic triangle encode equivalent algebraic
comparison data. They do not by themselves reconstruct a
physical holomorphic-topological quantum field theory. The recovery
statements are:

\begin{enumerate}[label=\textup{(\roman*)}]
\item \emph{Boundary from the bar coalgebra.}
 On the Koszul locus, Theorem~\ref{thm:higher-genus-inversion} gives
 the reconstruction equivalence
 \[
 \varepsilon_\cA\colon\Omega_X\barB_X(\cA)
 \xrightarrow{\sim}\cA.
 \]
 The dual shadow branch $\mathbf Q_g(\cA^!)$ is an eigenspace in the
 ambient centre complex.  A reconstruction from this branch requires
 a separately supplied coalgebra~$C_g^!$, a realization
 $C_g^!\xrightarrow{\sim}\mathbf Q_g(\cA^!)$, and a comparison
 $C_g^!\xrightarrow{\sim}\barB_X(\cA)$ compatible with the counit.

\item \emph{Cochain closed-sector slot from boundary.}
 The cochain-level closed-sector slot attached to the boundary is
 $\mathbf{C}_{\mathrm{ch}}(\cA)
 =R\mathcal{H}om_{\cA\text{-bimod}}(\cA,\cA)$.
 Its ambient complex is
 $\mathbf{C}_g(\cA) \simeq R\Gamma(\overline{\mathcal{M}}_g,
 \mathcal{Z}(\cA))$, where the centre local system
 $\mathcal{Z}(\cA)$ is a computable bimodule functor of~$\cA$.
 Identifying this object with a physical bulk factorization algebra
 requires the additional open/closed comparison data.

\item \emph{Lines from line-kernel data.}
 On the Verdier-strict line comparison locus, a line-kernel lift of
 $r(z)$ equips $\cA^!\text{-}\mathsf{mod}$ with a meromorphic tensor
 product. Associativity of the tensor product is the quantum
 Yang--Baxter condition for a lift
 $R(z)=1+\hbar r(z)+O(\hbar^2)$; the CYBE for~$r(z)$ is only its
 semiclassical collision-depth-$2$ shadow.

\item \emph{MC element from the datum.}
 Conversely, after the boundary algebra and bar presentation are fixed,
 the datum records the MC correction:
 the boundary~$\cA$ determines the bar complex, the bar complex
 determines~$D_\cA$, and $\Theta_\cA = D_\cA - d_\cA^{(0)}$ is
 the positive-genus correction. The entries $\cA^i$, $\cA^!$,
 $\mathbf{C}_{\mathrm{ch}}(\cA)$, $r(z)$, and
 $\nabla^{\mathrm{hol}}$ are then obtained by the bar, Verdier,
 Hochschild, collision, and connection constructions.
\end{enumerate}

The reconstruction claim is therefore not a bulk-reconstruction
theorem. It says that the algebraic datum classifies the modular bar
construction on the modular homotopy classification surface of
Proposition~\ref{prop:modular-homotopy-classification}: equivalent
data are exactly quasi-isomorphic modular bar constructions.

\begin{proposition}[Four-layer separation; \ClaimStatusProvedHere]
\label{prop:thqg-intro-four-layer-separation}
For a chiral algebra~$\cA$ on~$X$, the following constructions have
different source and target categories:
\[
\begin{array}{rcl}
\Omega_X\barB_X(\cA) &\longrightarrow& \cA,\\[2pt]
K_X(\cA):=\mathbb{D}_{\operatorname{Ran}}\barB_X(\cA)
&\xrightarrow{\ \nu_{\cA}\ }&\cA^!,\\[2pt]
\mathbf{C}_{\mathrm{ch}}(\cA) &=&
R\mathcal{H}om_{\cA\text{-bimod}}(\cA,\cA),\\[2pt]
...
\end{array}
\]
The first line is bar-cobar inversion on the Koszul/Positselski
surface. The second is Verdier line strictification. The third is the
cochain-level derived centre. The fourth is closed-open comparison
data for a physical holomorphic-topological bulk. The first three
lines determine the algebraic triangle. They do not determine a
physical bulk. They become physical-bulk data only after the fourth
line is included and is an equivalence in the chosen analytic or
coderived completion.
\end{proposition}

\begin{proof}
The bar-cobar counit has target~$\cA$ and classifies twisting
morphisms. Verdier duality sends the bar coalgebra to the homotopy
Koszul dual factorization algebra; finite-type or completed
continuous hypotheses are needed before this becomes a strict algebra
$\cA^!$. The chiral derived centre is instead the Hochschild
endomorphism algebra of~$\cA$ as a bimodule over itself; by the
open/closed Swiss-cheese theorem
(Theorem~\ref{thm:thqg-swiss-cheese}) it is the cochain-level
universal algebra of closed operators acting on the boundary, not the
bar coalgebra and not the Verdier dual algebra. A physical HT bulk is
a factorization algebra with a closed-open map into this acting
algebra.
Appending a decoupled closed factor with zero boundary action changes
the physical bulk without changing the algebraic datum. Hence the
algebraic datum cannot reconstruct the physical bulk unless the
closed-open comparison map is part of the data and satisfies the
stated equivalence condition.
\end{proof}
...
The following four typed comparison theorems are the recovery results
used here. Each operates on its stated comparison surface.
...
binary propagator extracted from the universal twisting morphism
...
\;=\;
\operatorname{Prop}^{\mathrm{coll}}_{0,2}(\tau)
\;\in\;
\mathrm{End}_{\cA}(2)[\![z^{-1}]\!].
\end{equation}
The twisting morphism and the Laurent collision kernel have different
types; equation~\eqref{eq:thqg-intro-collision-equals-twisting} is
typed by the propagator-extraction map.
...
applied to the degree-$3$ MC equation.
This is the degree-$2$ collision theorem under the hypothesis package of
...
Together, these four theorems identify the displayed algebraic entries
as successive projections of the identity $D_\cA^2 = 0$; they do not
reconstruct an external physical theory.
...
the moduli of marked curves. On the general proved surface,
...
algebra~$\mathfrak{g}$ at level~$k$ (that is, $r(z) = k\,\Omega_{\mathrm{tr}}/z$ where
$\Omega_{\mathrm{tr}} = \sum_a t^a \otimes t_a$ is the trace-form
Casimir element), the
...
The degenerate module $M_{r,s}$ has a null vector at level $rs$; this
null vector produces a polynomial differential operator
$\mathcal{D}_{r,s}$ of order~$rs$ in the insertion variable~$z_0$.
The flatness of the shadow connection implies that the conformal block
...
From the bar-complex perspective, the null vector in~$M_{r,s}$ is a
cycle in the bar complex
...
The BPZ package of null vectors, differential equations, fusion
rules, and minimal model classification is thus a shadow of the MC
equation on the Virasoro degenerate-module comparison surface.
...
The regular-singular property gives the extension of parallel
transport for the derived coinvariant / protected-state package across
...
$\kappa(\cA)$ is the modular characteristic, and the higher-degree
shadows $\operatorname{Sh}_{1,n}^{(r)}$ are the contributions from
the shadow obstruction tower at degree~$r$ and genus~$1$.
...
The higher-degree corrections encode the nonlinear
...
tower terminates at degree~$2$), and on the standard free-boson
...
convergence of the sum is guaranteed by the all-degree master equation
\eqref{eq:all-degree-master-eq-intro}. The genus-$2$ shadow connection
...
\emph{shadow depth} $r_{\max}(\cA)$, the smallest degree at which the
tower terminates (or $\infty$ if it does not). This invariant measures
the algebraic complexity of the modular MC element. On a physical
HT comparison surface it is read as gravitational complexity: the
number of independent multi-particle vertices needed by the modular
sector.
...
$\mathbf{F}_r$ has $r_{\max} = r$ (tower terminates at degree~$r$),
...
degree~$3$. In class~\textbf{C}, the quartic is the last nonzero
obstruction. In general class~$\mathbf{F}_r$, the tower terminates
at degree~$r$. In class~\textbf{M}, all obstructions are nonzero.)
...
Each obstruction class $o_{r+1}(\cA)$ is an algebraic obstruction in
the cyclic deformation complex. On a physical comparison surface the
same class is represented by an effective interaction vertex. This
gives the following EFT dictionary:

\begin{enumerate}[label=\textup{(\roman*)}]
\item \emph{Degree $2$: the free-field kernel.}
...
\item \emph{Degree $3$: the cubic interaction.}
...
\item \emph{Degree $5$ and beyond: the quintic forced.}
 For the Virasoro and $\mathcal{W}_N$ algebras, $o_5(\cA) \ne 0$:
 the quintic interaction is forced, and the tower never terminates.
 Each higher degree~$r$ introduces a new effective vertex, coupled to
 the lower-order vertices by the all-degree master equation
 $\nabla_H(\mathrm{Sh}_r) + o^{(r)} = 0$.
\end{enumerate}

The hierarchy is organized by a single filtration principle: the
shadow obstruction tower is the degree filtration of the full MC
element~$\Theta_\cA$. The shadow depth~$r_{\max}$ is a truncation
index, not an analytic radius of convergence. For Gaussian algebras
only the quadratic layer survives; for Lie/tree algebras the last
nonzero layer is cubic; for contact algebras the last nonzero layer is
quartic; for mixed algebras no finite truncation captures the MC
element.
...
The descent from degree~$r$ to degree~$r-2$ is governed by the
\emph{genus loop}
...
The genus loop reduces the degree by~$2$: from the quartic shadow,
...
from higher degree. The primitive-shadow decomposition
...
\;+\; \cdots
\end{equation}
expresses the full shadow at degree~$r$ as a sum of primitive
contributions from higher degrees, contracted downward by the
...
\label{thm:thqg-intro-operadic-complexity}
Under the finiteness and transfer hypotheses of
Theorem~\textup{\ref{thm:operadic-complexity}}, for a chiral
algebra~$\cA$ on~$X$,
\[
...
Proposition~\ref{prop:shadow-formality-low-degree}, proved at
degrees~$2$, $3$, and~$4$, extended by the HPL tree formula to all
finite degrees); $f_\infty = d_\infty$ follows because antisymmetrization
...
Koszulness itself. The Koszulness characterization theorem
(Theorem~\ref{thm:koszul-equivalences-meta}: seven intrinsic
bidirectional equivalences, one monadic consequence, one conditional
factorization-homology comparison, one Theorem~H-conditional chiral
Hochschild consequence, one perfectness-conditional Lagrangian criterion, and one
one-directional $\mathcal{D}$-module purity implication) is independent
...
Lagrangian polarization records the two bar-shadow branches of Koszul
duality. Its physical phase-space interpretation requires the
open/closed comparison data of
Proposition~\ref{prop:thqg-intro-four-layer-separation}.
...
sends the bar coalgebra to the algebra
$K_X(\cA):=\mathbb{D}_{\operatorname{Ran}}\barB_X(\cA)$.
On the completed finite-type pair locus, the map
$\nu_{\cA}\colon K_X(\cA)\xrightarrow{\sim}\cA^!$ realizes the chosen
Koszul partner.
...
The Lagrangian pair $(\mathbf{Q}_g(\cA), \mathbf{Q}_g(\cA^!))$ is the
algebraic phase space of the genus tower: its two Lagrangians are the
$\cA$ and $\cA^!$ shadow branches, paired by the shifted symplectic
form. After a physical open/closed realization, this pairing becomes
the boundary/closed-sector coupling. Koszul duality exchanges the two
...
the scalar Hodge-bundle obstruction at genus~$1$ vanishes.

On the critical locus, the Lagrangian decomposition persists and the
pointed trace
$\operatorname{tr}_1\operatorname{Obs}^{\mathrm{def}}_{1,1}(\cA)
=\kappa(\cA)\lambda_1$ vanishes.  The higher shadow classes retain
their independent deformation data. On a physical comparison surface
this is the scalar anomaly-cancellation condition for
the Hodge-bundle contribution; full higher-genus propagation also
requires the remaining shadow and sewing conditions.
...
$\beta\gamma$ & $0$ & $\beta\gamma/bc$ Verdier pair \\[2pt]
...
For a strict Verdier involution over~$\mathbb{C}$, the ordinary
intersection of the two eigenspaces is zero:
...
0.
\end{equation}
The centre in the holographic triangle is therefore not this
set-theoretic intersection. It is the derived endomorphism algebra
\[
\mathbf{C}_{\mathrm{ch}}(\cA)
=
R\mathcal{H}om_{\cA\text{-bimod}}(\cA,\cA).
\]
When $\sigma$ is only a homotopy involution, the relevant geometric
object is the derived fibre product of the two Lagrangian branes, not
the ordinary intersection of their underlying eigenspaces. On the
perfect clean-intersection comparison locus its endomorphism complex
identifies with $\mathbf{C}_{\mathrm{ch}}(\cA)$; its $H^0$ is the
ordinary chiral centre. The Virasoro vector belongs to the boundary
OPE data and becomes central only on representations where its
non-negative modes vanish.
...
codimension~$2$. A Yangian appears only after the affine
Verdier-strict comparison is supplied with an RTT-complete quantum
line-kernel lift and a generation theorem for the relevant line
category.
...
after the Verdier strictification and line-kernel comparison have been
made. Before that comparison, the collision residue is the Laurent
kernel
\[
r(z)\in \mathrm{End}_{\cA}(2)[\![z^{-1}]\!].
\]
On the line side it becomes the \emph{spectral $r$-matrix}: a
meromorphic endomorphism of the tensor square of the Verdier-dual line
algebra, with a pole at $z = 0$ (the collision locus) and regular as
$z \to \infty$ (the separation limit).

The residue of $r(z)$ at $z = 0$ is the leading OPE mode; after
strictification it is written
...
encode the subleading OPE modes; they become higher-spin Yangian data
only on a comparison surface where the dg-shifted Yangian recognition
theorem applies.
...
restricted to genus~$0$ and degree~$3$ gives
\begin{equation}\label{eq:thqg-intro-mc-degree3}
d_{\barB}(\Theta^{(3)})
+ \tfrac{1}{2}[\Theta^{(2)}, \Theta^{(2)}]^{(3)}
\;=\; 0,
\end{equation}
where $\Theta^{(r)}$ denotes the degree-$r$ component. The second
...
point is that no quantum-group input is needed: the CYBE is the
classical collision identity inherited from the Arnold relation on
configuration spaces. The QYBE and any Yangian presentation require
the additional quantum line-kernel lift.
...
On the affine Verdier-strict line comparison surface, $r(z)$ by
itself is only the semiclassical collision kernel. If it admits an
RTT-complete quantum lift
$R(z)=1+\hbar r(z)+O(\hbar^2)$ satisfying the quantum Yang--Baxter
equation, and if the evaluation objects generate the completed line
category, then the lifted kernel determines a braided tensor structure
on $\mathsf{Line}(\cA)=\cA^!\text{-}\mathsf{mod}$. The corresponding
endomorphism algebra is the dg-shifted Yangian on that affine
comparison surface.

\begin{definition}[The dg-shifted Yangian]
\label{def:thqg-intro-dg-yangian}
\index{dg-shifted Yangian!definition}
For an affine RTT-complete line-kernel datum, the
\emph{dg-shifted Yangian} $\mathcal{Y}^{\mathrm{dg}}_T(\cA)$
is the dg algebra generated by the modes of the quantum line kernel,
with RTT/QYBE relations whose semiclassical part is the CYBE and with
the higher collision conditions:
\begin{equation}\label{eq:thqg-intro-yangian-generators}
\mathcal{Y}^{\mathrm{dg}}_T(\cA)
\;=\;
\operatorname{End}_{\otimes_{R(z)}}\!
\bigl(\mathsf{Line}(\cA)\bigr),
\end{equation}
the endomorphism algebra of the tensor functor $\otimes_{R(z)}$ on
$\cA^!\text{-}\mathsf{mod}$ on the strict line locus.
\end{definition}

For affine Kac--Moody $\cA = \widehat{\mathfrak{g}}_k$, the
evaluation-generated RTT-complete comparison identifies the
dg-shifted Yangian with the Drinfeld Yangian $Y(\mathfrak{g})$: the
rational kernel $r^{KM}(z) = k\,\Omega_{\mathrm{tr}}/z$ is the semiclassical part
of the RTT presentation of $Y(\mathfrak{g})$
(Theorem~\ref{thm:yangian-bar-rtt}). The CYBE is the first-order
shadow of the QYBE, not by itself the defining relation. The
evaluation homomorphism $\operatorname{ev}_a \colon Y(\mathfrak{g}) \to
U(\mathfrak{g})$ corresponds to the restriction of the $r$-matrix
to the depth-$1$ collision stratum.

For the Virasoro algebra, the same collision filtration gives a
higher-pole Virasoro collision algebra. The kernel is
\[
r^{\mathrm{Vir}}(z)
=
\frac{c/2}{z^3}+\frac{2T}{z},
\]
with the cubic central pole and the simple stress-tensor pole feeding
the BPZ and shadow-obstruction constructions. An identification with
a Yangian requires a separate comparison theorem; the affine Yangian
extraction above does not supply it.

\subsection{The RTT presentation}
\label{subsec:thqg-intro-rtt}
\index{RTT presentation}

On the affine RTT-complete surface, the Drinfeld Yangian admits a
presentation through the RTT equation. Define the \emph{monodromy
matrix}
...
\end{equation}
where $R(z) = 1 + \hbar r(z) + O(\hbar^2)$ is a quantum lift of the
classical collision kernel, $T_i(z) = T(z)$ acts in the $i$-th tensor
factor, and the equation holds in
$\operatorname{End}(V^{\otimes 2}) \otimes \mathcal{Y}[\![z_1^{-1}, z_2^{-1}]\!]$.

The QYBE for~$R$ is the quantum associativity condition; the classical
limit $\hbar \to 0$ of $R = 1 + \hbar r + O(\hbar^2)$ gives the CYBE
for~$r(z)$. From the bar-complex perspective, the depth-$3$ strata of
$\overline{C}_4(X)$ supply the classical compatibility constraints
among depth-$2$ collision generators. A quantized presentation requires
the chosen quantum lift and RTT-complete comparison; on the affine
comparison surface, Theorem~\ref{thm:yangian-bar-rtt} identifies the
corresponding quantized relations with the RTT presentation.
...
completion of the dg-shifted Yangian that records genus corrections
when the modular line-kernel comparison is available. A modular
$R$-matrix is additional MC data
\begin{equation}\label{eq:thqg-intro-modular-r}
R^{\mathrm{mod}}_T(z; \hbar)
\;=\;
1 + \sum_{g \ge 0} \hbar^{2g} R^{(g)}_T(z)
\end{equation}
in the completed convolution algebra. Where this modular quantum lift
exists, its higher-genus Yang--Baxter condition is the modular-kernel
shadow of the MC equation
$D\Theta + \frac{1}{2}[\Theta,\Theta] = 0$ at the level of the modular
convolution algebra.

At genus~$0$, the semiclassical part of $R^{(0)}_T(z)$ is the
represented collision kernel~$\rho(K_T^{\mathrm{coll}}(z))$. At genus~$1$, when the MC lift exists,
$R^{(1)}_T(z)$ is the elliptic correction depending on the modular
parameter $\tau$ and controlled at scalar level by
$\kappa(\cA)$. Thus the genus expansion is conditional higher-genus
line-kernel data, not an automatic consequence of the bar complex.
...
category is braided monoidal). From the bar-complex perspective, this
is the affine comparison leg from the shadow connection
(\S\ref{sec:thqg-intro-shadow-connection}) to the quantum-group side,
adjacent to but not identical with the Yangian extraction of
...
This is the genus-$0$ affine evaluation-locus comparison. A full
factorization Drinfeld--Kohno equivalence, and the dg-shifted
Yangian/line-operator comparison at the intended target, require the
ordered factorization category, generation beyond evaluation objects,
monadic reconstruction, and RTT-complete control.
...
an analytic input: convergence of the genus tower. The analytic
completion supplies the sewing envelope behind the critical-string
comparison.
...
HS-sewing condition genus by genus. In particular, the HS-sewing
condition holds on the established finite-energy sewing loci for the
standard families: Heisenberg, free fermion, lattice, $\beta\gamma$,
affine Kac--Moody, Virasoro, and principal $\mathcal{W}$-algebras.
The proof is
...
For a rank-$r$ Heisenberg core $\cH_{V,\kappa}$, the holomorphic
oscillator part of the genus-$g$ partition function is
\begin{equation}\label{eq:thqg-intro-heisenberg-fredholm}
Z^{\mathrm{ch}}_{g,\mathrm{osc}}(\cH_{V,\kappa})
\;=\;
\det\!\left(1 - S^{(1)}_g\right)^{-r},
\end{equation}
where $S^{(1)}_g$ is the one-particle Bergman sewing operator,
a trace-class operator on the one-particle Hilbert space
$\bigoplus_{n \ge 1} V\otimes\mathbb{C}\cdot\alpha_{-n}|0\rangle$.
...
The Fredholm determinant formula is the Gaussian case of sewing:
because the Heisenberg has only a double-pole OPE, the
...
The generating function of the scalar-diagonal graph functionals is the
$\hat{A}$-genus
(Theorem~\ref{thm:modular-characteristic}):
\begin{equation}\label{eq:thqg-intro-ahat-generating}
\sum_{g \ge 1}
F_g^{\mathrm{sc}}(\cA) \hbar^{2g}
\;=\;
...
\end{equation}
The $\hat{A}$-genus appears because $H_D^K$ gives
$\operatorname{ch}_g(\mathfrak O_g^K(\cA))=
(-1)^g\kappa(\cA)\lambda_g$, while $H_D^{\mathrm{graph}}$ applies the
Faber--Pandharipande functional.  Grothendieck--Riemann--Roch on the
universal curve gives
$\operatorname{td}(\mathbb{E}^\vee)=\hat{A}(i\hbar)$.
...
discriminant $\Delta(\tau)$ on the moduli of elliptic curves. For a
rank-$r$ Heisenberg core, the holomorphic oscillator factor is
$Z^{\mathrm{ch}}_{1,\mathrm{osc}} = \eta(\tau)^{-r}$, where
$\eta(\tau) = q^{1/24}
...
The Virasoro conductor $26$ is an algebraic Koszul-duality invariant.
The Virasoro algebra $\mathrm{Vir}_c$ is Koszul dual to
$\mathrm{Vir}_{26-c}$
...
c + c' = c + (26 - c) = 26.
\]
The physical bosonic string adds the BRST ghost system. Its
...
The ghost system ($bc$ at $c = -26$) realizes BRST anomaly
cancellation; it is not the cobar
$\Omega(\barB(\mathrm{Vir}_c))$, and it is not identified with
$\mathrm{Vir}_{26-c}$ without an additional comparison theorem. The
cobar recovers $\mathrm{Vir}_c$ itself by bar-cobar inversion
(Theorem~B), not the dual (Remark~\ref{rem:bosonic-string-26}).

At $c = 26$, the matter-ghost system has vanishing scalar modular
characteristic:
\[
...
at $c = -26$ has $\kappa = -13$. This cancels the scalar genus
obstruction. Perturbative finiteness additionally requires BRST
nilpotence, modular sewing, and analytic convergence of the completed
amplitudes.
...
The analytic comparison chain is
\begin{equation}\label{eq:thqg-intro-analytic-chain}
...
the analytic sewing obligations.
...
Ten principal lines organize the monograph. They build on the
categorical logarithm of Part~\ref{part:bar-complex}, the nonlinear
engine of Part~\ref{part:characteristic-datum}, and the $E_1$ wing of
Part~\ref{part:standard-landscape}, then extend through the standard
landscape (Part~\ref{part:physics-bridges}), the bridges to
mathematical physics (Part~\ref{part:seven-faces}), and the frontier
(Part~\ref{part:v1-frontier}). The
holographic datum $\mathcal{H}(\cA)$ of
\S\ref{subsec:holographic-datum-seven-entry} provides the unifying thread:
each result is a bar, Verdier, Hochschild, collision, or connection
construction constrained by the MC element~$\Theta_\cA$.
...
\begin{enumerate}[label=\textup{(\roman*)}]
\item The bar-cobar adjunction $\Omega_X \dashv \barB_X$ with
...
\item The bar-intrinsic MC element
 ...
 (Theorem~\ref{thm:mc2-bar-intrinsic}).
\item Finite-order projections: $\kappa$ (degree~$2$),
 $\mathfrak{C}$ (degree~$3$), $\mathfrak{Q}$ (degree~$4$).
...
\item Shadow algebra $\cA^{\mathrm{sh}} = H_\bullet(\Defcyc^{\mathrm{mod}}(\cA))$
 as the chain-level home for the modular shadow invariants.
\end{enumerate}
...
into the datum
$\mathcal{H}(\cA)=(\cA,\cA^i,\cA^!,
\mathbf{C}_{\mathrm{ch}}(\cA),r(z),\Theta_\cA,
\nabla^{\mathrm{hol}})$. The Swiss-cheese
factorization (Chapter~\ref{sec:swiss-cheese}), the
closed-sector--boundary--line triangle, the collision filtration
(\S\ref{sec:thqg-intro-collision-yang-baxter}), and the four recovery
theorems (\S\ref{subsec:thqg-intro-four-recovery}) produce a typed
algebraic package whose internal consistency is the MC equation.
...
The collision-to-Yang--Baxter construction
(\S\ref{sec:thqg-intro-collision-yang-baxter}) constructs the
dg-shifted Yangian on the affine comparison surface only after the
collision kernel has an RTT-complete quantum lift and the line category
is generated by the comparison objects. The CYBE is the Arnold relation
at codimension~$2$; the QYBE is extra quantum line-kernel data. The
modular Yangian carries the genus corrections as a pro-MC element where
the modular line-kernel comparison is available. The Drinfeld--Kohno
theorem is the genus-$0$ affine comparison from KZ monodromy to the
...
is proved analytically/coderived and on the strict Mittag--Leffler
pro-object / $J$-adic / filtered weight-completed chain surface
\textup{(}analytic HS-sewing package at all genera; genuswise
BV/BRST/bar identification on completed/pro ambients; bounded
direct-sum failure in class~M already at weight~$4$\textup{)}.
...
Part~\ref{part:seven-faces} connects the engine to Feynman diagrams,
BV-BRST quantization (Chapter~\ref{ch:v1-bv-brst}), Yang--Mills
boundary theory, and geometric Langlands. On the BV-BRST comparison
surface, the bar complex maps to the BRST complex, the MC equation maps
to the quantum master equation
$\hbar\Delta S + \frac{1}{2}\{S,S\} = 0$, and the shadow connection
maps to the gauge connection of the BV formalism.

\subsection{Volume~II and beyond}
\label{subsec:thqg-intro-guide-vol2}

Volume~II supplies the physical comparison data for the
closed-sector/boundary/line algebraic triangle in~$3$d
holomorphic-topological QFT\@; the physical BBL reading requires the
open--closed comparison datum. The
bar complex's differential is $\mathbb{C}$-direction factorization;
its deconcatenation coproduct is $\mathbb{R}$-direction
factorization; together they make $\barB_X(\cA)$ an $\Eone$-chiral
coassociative coalgebra on
$\mathrm{FM}_k(\mathbb{C}) \times \operatorname{Conf}_k(\mathbb{R})$.
The $\mathrm{SC}^{\mathrm{ch,top}}$ structure emerges on the derived
center pair $(C^\bullet_{\mathrm{ch}}(\cA,\cA),\,\cA)$. Volume~II
supplies the recognition theorem, homotopy-Koszulity, PVA descent, and
the derived conformal-block line-operator identification as comparison
theorems for that product-formal local-shadow ambient. The PVA
quantization bridge and holographic applications are the physical
realization layer attached to the algebraic datum.

\subsection{The Koszulness characterization theorem}
...
tests for the Koszul property. The intrinsic bidirectional core is
formed by seven independent equivalences:
\textup{(i)--(v)} and \textup{(ix)--(x)}. The
Barr--Beck--Lurie comparison \textup{(vi)} is a proved consequence of
the bar-cobar counit equivalence \textup{(v)}, not an independent
route. Factorization homology \textup{(vii)} is conditional on the
comparison package $\mathsf{FH}_{\mathbb{P}^1}(\cA)$ and becomes a
criterion only under the detecting strengthening
$\mathsf{FH}^{\mathrm{det}}_{\mathbb{P}^1}(\cA)$. Theorem~H gives
\textup{(viii)} as a one-way Hochschild consequence on the Koszul
locus. The Lagrangian criterion \textup{(xi)} is conditional on
perfectness and nondegeneracy; $\mathcal D$-module purity
\textup{(xii)} implies the FM-boundary criterion \textup{(x)}, with
converse still open. The twelve tests are:
\begin{enumerate}[label=\textup{(\roman*)}]
\item Chiral Koszulness.
\item $E_2$-collapse of the PBW spectral sequence on
 $\barBgeom(\cA)$.
\item Formality of the minimal $A_\infty$ model of
 $\barBgeom(\cA)$.
...
\item Barr--Beck--Lurie: the monadic characterization via the
 bar-cobar monad.
\item Conditional factorization homology concentration at genus~$0$,
 with all-genera uniform-weight strengthening under the stated
 $\mathsf{FH}_{\Sigma_g}$ hypotheses.
\item Theorem~H: a family datum $H_H(\cA;S)$ gives
 $\operatorname{Supp}\ChirHoch^\bullet(\cA)\subseteq S$.
 A companion datum $H_H(\cA^!;S^!)$ together with a perfect
 degree-$d$ chain pairing gives the separate Koszul-dual reflection
 $\ChirHoch^n(\cA)\cong
 \ChirHoch^{d-n}(\cA^!)^\vee\otimes\omega_X$.
\item Kac--Shapovalov nondegeneracy in the bar-relevant range.
...
\item $\mathcal D$-module purity of the geometric bar terms.
\end{enumerate}
The seven independent equivalences are proved by chain-level
arguments connecting each core condition to degeneration of the bar
spectral sequence at~$E_2$.
The characterization surface is the seven-fold TFAE of
Chapter~\ref{chap:ftm-seven-fold-tfae-platonic}.
...
Presentation coalgebra $\cA^{\mathrm i}$
 & $C_X(s^{-1}V,s^{-2}R)$ and
 $q_\cA\colon\cA^{\mathrm i}\to\barB_X(\cA)$ \\[2pt]
Ran--Verdier algebra $K_X(\cA)$
 & $\mathbb D_{\Ran}\barB_X(\cA)$ and $\mathbb D(q_\cA)$ \\[2pt]
Chosen partner $\cA^!$
 & $\nu_\cA\colon K_X(\cA)\to\cA^!$ plus pair hypotheses \\[2pt]
Chiral derived centre $\mathbf{C}_{\mathrm{ch}}(\cA)$ &
$R\mathcal{H}om_{\cA\text{-bimod}}(\cA,\cA)$ \\[2pt]
Ambient complex $\mathbf{C}_g(\cA)$ &
$R\Gamma(\overline{\mathcal{M}}_g,\mathcal{Z}(\cA))$ \\[2pt]
...
Collision kernel $K_\cA^{\mathrm{coll}}(z)$ & binary component of
$\Theta_\cA^{E_1}$ on $\barB_X(\cA)$ \\[2pt]
Represented CYBE & Arnold relation plus $H_{\mathrm{CYBE}}^{\mathrm{rep}}$
on $\overline{C}_3(X)$ \\[2pt]
Yangian $\mathcal{Y}^{\mathrm{dg}}_T$ &
RTT-complete quantum line-kernel lift + generation hypotheses \\[2pt]
...
\end{tabular}
\end{center}
The algebraic shadow entries in the right column are either geometric
inputs (FM compactification, boundary stratification of moduli) or
projections of the identity $D_\cA^2 = 0$. The analytic, physical, and
quantum-group entries require the comparison hypotheses displayed in
the table.

\subsection{The master invariant table}
\label{subsec:thqg-intro-guide-master-table}

The principal numerical invariants of the standard families are
projections of~$\Theta_\cA$.
...
\noindent
On comparison surfaces carrying a physical realization, the modular
characteristic $\kappa$ determines the one-loop term, the conductor
$c + c'$ determines the scalar anomaly-cancellation sum, and the
shadow depth $r_{\max}$ determines the gravitational complexity.
Algebraically, all three invariants are computable from the bar
complex of~$\cA$, and all three are projections of the holographic
datum~$\mathcal{H}(\cA)$.

The constructions above run from Swiss-cheese factorization to the
master invariant table. The full proofs and computations appear in
Parts~\ref{part:bar-complex}--\ref{part:physics-bridges} of Volume~I
and in Volume~II.

\section{Synthesis}
\label{sec:thqg-intro-suppl-supplement}

\begin{remark}[Algebraic sector versus dynamical-metric path integral;
\ClaimStatusProvedHere]
\label{rem:thqg-intro-algebraic-sector-vs-dynamical-metric}
\textit{Type signature.}
Quadrant: Open. Presentation: chain-level. Level: $1 \to 3$.
Hypothesis package: augmented chiral $\cA$ on a smooth projective
curve; bar-cobar adjunction; chiral Hochschild cochains.
\medskip

The algebraic-sector identification
$(\partial,\,\mathrm{closed},\,\mathrm{interaction})
= (\cA,\,Z^{\mathrm{der}}_{\mathrm{ch}}(\cA),\,
\mathsf{SC}^{\mathrm{ch,top}}\text{-brace})$
of~\eqref{eq:supp-algebraic-sector} is a typed functorial statement:
the boundary chart is the chiral algebra, the closed-sector slot is the chiral
Hochschild endomorphism algebra of~$\cA$ as a chiral bimodule over
itself, and the interaction is the two-colour
$\mathsf{SC}^{\mathrm{ch,top}}$-brace action on the derived-centre
pair (Volume~II, the
$\mathsf{SC}^{\mathrm{ch,top}}$-brace bulk theorem).

The seven-entry datum~$\mathcal{H}(\cA)$ does not construct a compact
dynamical-metric path integral. The level-5 scalar shadow
(modular form, $\kappa$-tuple, partition function) factors through
the level-4 brane/operator algebra by modular trace and clutching,
and the inverse direction is the operator-level Pfaffian /
square-root reconstruction problem. A gravity-line completion of the
algebraic sector to a compact 3d quantum-gravity path integral
requires four additional inputs:
\begin{enumerate}[label=\textup{(\roman*)}]
\item modular invariance of the boundary partition function under
 the action of $\mathrm{SL}(2,\mathbb{Z})$ on the moduli of marked
 curves;
\item vacuum (identity-block) dominance in the high-temperature
 (Cardy~\cite{Cardy86}) channel;
\item saddle stability of the BTZ instanton in the gravitational path
 integral~\cite{BTZ92};
\item Maloney--Witten / Manschot--Moore Farey-tail completion of
 the $\mathrm{SL}(2,\mathbb{Z})$-orbit sum~\cite{Maloney-Witten07}.
\end{enumerate}
With these four inputs supplied, the Vol.~II climax produces the
gravity-line completion target
\[
Z_{\mathrm{3d\,QG,\,Vir}}(S^2 \times S^1_\tau)
\;=\;
q^{(1-c)/24}\,(1-q)/\eta(\tau)
\]
as the Virasoro-vacuum scalar shadow attached to
$Z^{\mathrm{der}}_{\mathrm{ch}}(\mathrm{Vir}_c)$. Without
\textup{(i)--(iv)}, the algebraic sector by itself does not produce a
compact dynamical metric. The propagation Vol.~III $\Sigma_{d-1}, C$
target is the two-stage functor
$\Phi_d^{(\Sigma_{d-1}, C)} = \mathrm{Sp}^{\mathrm{ch}}_{\Sigma_{d-1},
C} \circ \PhiFA_d$ on the CY-quadrant side.
\end{remark}

\begin{remark}[K3 comparison locus]
\label{rem:thqg-intro-suppl-1}
On the K3 base, the comparison target is the K3 chiral
...
\right),
\]
not a Drinfeld Yangian. At the Lie/Hopf level the local charts are
Hall/CoHA and quantum-toroidal charts; the Borcherds--Kac--Moody
non-abelianity belongs to vertex-operator closure on the K3 Fock
module. This target is the common comparison point for Vol.~I's modular
Koszul duality on $\bar{\mathcal A}_2$, Vol.~II's twisted holographic
3d gravity-line completion with $24$-fold Kodaira-I$_1$ graviton
emission (level-5 scalar shadow under the BTZ / Cardy /
Maloney--Witten / Farey-tail hypothesis package of
Remark~\ref{rem:thqg-intro-algebraic-sector-vs-dynamical-metric}), and
Vol.~III's two-stage CY-to-chiral factorisation
$\Phi_2 = \SpCh_{\Sigma_1, C} \circ \PhiFA_2$ from
$D^b\mathrm{Coh}(K3)$ at the Lusztig locus
$\hbar^2 = -1/8$. The numerical hinge is
\[
K^{\kappa_{\mathrm{ch}}}=8,\qquad
\hbar^2\cdot K^{\kappa_{\mathrm{ch}}}=-1.
\]
The compact Hall double and the seven Vol.~III coordinate charts for
$\mathbf H_{\Delta_5}$ remain comparison data with open transition
proofs; the $21$ pairwise equivalences are proof obligations, not
inputs to the algebraic recovery statements above. See Vol.~III,
