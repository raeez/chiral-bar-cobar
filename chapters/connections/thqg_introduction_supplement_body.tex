...
The two-component decomposition is the geometric origin of the
bulk-boundary-line triangle. The holomorphic direction produces the bulk:
factorization homology $\int_X \cA$ in the $X$-direction. The topological
direction produces the lines: modules over the Koszul dual. The
...
object (the bar complex with its $\Eone$-chiral coassociative structure) viewed
from three angles.
...
$\Theta_\cA$ in powers of the degree variable. The radius of
...
independent bidirectional equivalences, one one-way chiral
Hochschild consequence, one perfectness-conditional Lagrangian
criterion, one one-directional $\mathcal{D}$-module purity
implication) is independent of the shadow depth classification.
...
rational $r$-matrix $r(z) = k\,\Omega/z$ generates the RTT presentation
...
(Chapter~\ref{ch:v1-bv-brst}), Yang--Mills boundary theory,
...
is $\mathbb{C}$-direction factorization; its deconcatenation coproduct is $\mathbb{R}$-direction
...
The $\mathrm{SC}^{\mathrm{ch,top}}$ structure emerges on the derived center pair
$(C^\bullet_{\mathrm{ch}}(\cA,\cA),\,\cA)$; the recognition theorem,
homotopy-Koszulity, PVA descent, and the derived
...
\item Shadow-formality: the shadow obstruction tower is equivalent to the
 $L_\infty$-formality obstruction tower at degrees $2$, $3$, $4$.
...
\end{enumerate}
The eight genuinely independent unconditional equivalences are
proved by chain-level arguments connecting each condition to the
degeneration of the bar spectral sequence at~$E_2$; Barr--Beck--Lurie
monadicity is a proved consequence of bar-cobar quasi-isomorphism
via the standard comonadic reduction rather than an independent
route, and chiral Hochschild concentration is a one-way consequence
on the Koszul locus. The Lagrangian criterion requires
...
\begin{remark}[Introductory supplement: the K3 climax]
\label{rem:thqg-intro-suppl-1}
The twisted HT quantum-gravity introduction is completed, on the
K3 base, by the single non-abelian chiral bialgebra
$\mathbf H_{\Delta_5}$ that interweaves all three volumes. Vol.~I's
modular Koszul duality on $\bar{\mathcal A}_2$, Vol.~II's twisted
holographic 3d quantum gravity with $24$-fold Kodaira-I$_1$ graviton
emission, and Vol.~III's two-stage CY-to-chiral factorisation
$\Phi_2 = \SpCh_{\Sigma_1, C} \circ \PhiFA_2$ from
$D^b\mathrm{Coh}(K3)$
(Stage~1 $\PhiFA_2$ producing the canonical
$E_2$-holomorphic factorisation algebra on K3, Stage~2
$\SpCh_{\Sigma_1, C}$ specialising to the reference curve via
factorisation homology over a $1$-cycle)
-- all three commute at the Lusztig locus
$\hbar^2 = -1/8$, with the universal identity
$\hbar^2\cdot K^{\kappa_{\mathrm{ch}}} = -1$ as the common numerical
signature. The Vol.~III seven incarnations of $\mathbf H_{\Delta_5}$ are a
structural parallel to the Vol.~I seven faces of $r(z)$; none of the $21$
pairwise equivalences has been proved. The introduction here should be
read as the gateway to the K3 saga, with the main body chapters supplying
the detailed developments across all three volumes. Cross-ref Vol.~III,
