...
\label{sec:thqg-intro-suppl-wave13-DNA}

\begin{remark}[Introductory supplement: the Wave 13 K3 climax]
\label{rem:thqg-intro-suppl-DNA-1}
The twisted HT quantum-gravity introduction is completed, in the
Wave~13 K3 perspective, by the single non-abelian chiral bialgebra
...
emission, and Vol.~III's CY-to-chiral functor $\Phi$ from
$D^b\mathrm{Coh}(K3)$ -- all three commute at the Lusztig locus
$\hbar^2 = -1/8$, with the universal identity
$\hbar^2\cdot K^{\kappa_{\mathrm{ch}}} = -1$ as the common numerical
signature. The introduction here should be read as the gateway to the
Wave~13 saga, with the main body chapters supplying the detailed
developments across all three volumes. Cross-ref Vol.~III,
