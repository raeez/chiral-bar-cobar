...
\label{conj:thqg-intro-operadic-complexity}% NOTE: retains conj: prefix for backward compatibility; claim is ProvedHere
...
each building on the categorical logarithm of Part~I,
the nonlinear engine of Part~II, and the $E_1$ wing of Part~III,
then extending outward through the standard landscape (Part~IV),
the bridges to mathematical physics (Part~V), and the frontier
(Part~VII).  The
...
Part~IV computes the algebraic engine on every major family.  Each
...
Part~V connects the engine to Feynman diagrams, BV-BRST quantization
