...
\end{equation}
The level-independent part is the \emph{perturbative anomaly ring}
$R^{\mathrm{pert}}$, generated by $K_\fg \cdot \varrho(\fg)$
at arity~$2$ and $-K_\fg \cdot \varrho(\fg)$ at arity~$3$.
The level-dependent part is the \emph{dynamical anomaly ideal}
$I^{\mathrm{dyn}}$, with first nonzero entry at arity~$4$.
...
\begin{theorem}[Finite generation of $(\cA^{\mathrm{sh}})^\sigma$;
...
For $\cA = \mathrm{Vir}_c$, the $\sigma$-invariant shadow ring on the
primary line is generated as a graded ring by three elements:
