...
\item \emph{Arity $3$, genus $0$.}
  On the Virasoro subalgebra line:
  \begin{equation}\label{eq:ds-tower-cubic}
  \Delta_\phi^{(3,0)}\big|_{T}
  \;=\;
  -K_\fg \cdot \varrho(\fg),
  \end{equation}
...
\emph{(i)}
The affine OPE
$J^a(z)J^b(w) \sim k\kappa^{ab}/(z{-}w)^2 + f^{abc}J_c/(z{-}w)$
is linear in~$k$: the double-pole coefficient is proportional
to~$k$ and the simple-pole coefficient is $k$-independent.
In the shifted variable $t = k + h^\vee$, the modular characteristic
$\kappa = td/(2h^\vee)$ and every higher shadow are odd functions
of~$t$ (the structure constants~$f^{abc}$ are $t$-independent;
the Killing form scales as~$t$; every graph amplitude is a
monomial in these).  The Feigin--Frenkel involution $t \mapsto -t$
therefore sends $\Sh_s \mapsto (-1)^s \Sh_s$ at each arity, giving
$\Delta^{(s,g)} = \Sh_s + (-1)^s \Sh_s = 0$ for odd~$s$
and $= 2\Sh_s$ for even~$s$---but $\Sh_s$ for even $s \geq 4$
vanishes because the affine shadow tower terminates at arity~$3$.
Hence $\Delta = 0$ at all arities.
