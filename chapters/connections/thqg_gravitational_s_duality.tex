...
\item The shadow duality theorem: $\sigma(\Sh_r(\cA)) = \Sh_r(\cA^!)$
 at all degrees, with explicit verification at degrees $2$, $3$, $4$.
...
The Verdier involution descends to a graded Lie algebra isomorphism
...
The Lie bracket on $A^{\mathrm{sh}}$ is induced by the bracket on $\gAmod$;
since $\sigma$ preserves the bracket, $\sigma_{\mathrm{sh}}$ is a Lie algebra
...
\end{equation}
Under the Liouville parametrization
$c = 1 + 6(b + b^{-1})^2$, the duality $c \mapsto 26 - c$
induces $b \mapsto b'$ where $26 - c = 1 + 6(b' + (b')^{-1})^2$.
\end{proposition}
...
according to the Koszul dual module structure, and $b \mapsto b'$
where $b'$ is determined by $26 - c = 1 + 6(b' + (b')^{-1})^2$.
...
The Liouville parametrization $c = 1 + 6(b + b^{-1})^2$ is a
coordinate on the moduli of Virasoro algebras. Under Koszul
duality $c \mapsto 26 - c$, it transforms to
$26 - c = 1 + 6(b' + (b')^{-1})^2$. The self-dual point
$c = 13$ (where $\mathrm{Vir}_c \cong \mathrm{Vir}_{26-c}$)
corresponds to $b = b'$, i.e., $13 = 1 + 6(b + b^{-1})^2$,
giving $(b + b^{-1})^2 = 2$.
\end{remark}
...
degree, with the constant $13$ at degree~$2$ reflecting the critical
...
$b \mapsto b'$ (Liouville parametrization), and the eigenvalues
transform by a nontrivial algebraic map. At the self-dual point
$c = 13$: $b = b'$ and the braiding is self-conjugate.
\end{remark}
...
Define the \emph{DS complementarity tower} entry at degree~$s$
and genus~$g$:
...
\item \emph{Degree $2$, all genera.}
 \begin{equation}\label{eq:ds-tower-degree2}
 \Delta_\phi^{(2,g)}
 \;=\;
...
\item \emph{Degree $\geq 4$: level dependence.}
 For $\fg = \fsl_2$:
 \[
 \Delta_\phi^{(4,0)}
 \;=\;
...
at degree~$2$ and $-K_\fg \cdot \varrho(\fg)$ at degree~$3$.
The level-dependent part $I^{\mathrm{dyn}}$
has first nonzero entry at degree~$4$.
The threshold is degree~$3$ for all simple~$\fg$.
\end{corollary}

\begin{proof}
Level independence at degrees $2$--$3$: parts~(ii)--(iii) of
Theorem~\ref{thm:ds-complementarity-tower}; the argument uses only
that the affine OPE is linear in $k + h^\vee$, which holds for
every simple~$\fg$.
Level dependence at degree~$4$: part~(iv); the DS central charge
...
\end{tabular}
\end{center}
The degree-$2$ universal entry at genus~$1$ is
...
$\kappa = 13/2$), where the shadow obstruction tower is nontrivially self-dual
with nonzero data at every degree.
At $c = 0$, the bar complex is uncurved
($\kappa = 0$, so $\Theta_{\mathrm{Vir}_0}^{\min} = 0$), but this
does not identify the full higher-degree shadow obstruction tower. The
...
(Construction~\ref{constr:v1-platonic-package}) transforms under $\sigma$:
