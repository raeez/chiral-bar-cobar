...
(Theorem~\ref{thm:verdier-bar-cobar}; underlying complex
$\simeq \barB_X(\cA^!)$) is the chain-level mechanism
...
$\lambda \mapsto 1 - \lambda$ is the DS image of
...
Let $\phi\colon \widehat{\fsl}_{N,k} \to \mathcal{W}_N^k$ be the
principal DS reduction.  For each arity $r \geq 2$ and genus $g \geq 0$,
define the \emph{DS complementarity tower} entry
\begin{equation}\label{eq:ds-tower-def}
\Delta_\phi^{(r,g)}
\;:=\;
\Sh_{r,g}\bigl(\mathcal{W}_N^k\bigr)
\;+\;
\sigma\!\bigl(\Sh_{r,g}(\mathcal{W}_N^k)\bigr).
\end{equation}
\begin{enumerate}[label=\textup{(\roman*)}]
\item \emph{Affine vanishing.}
  The corresponding tower for $\widehat{\fsl}_{N,k}$
  \textup{(}without DS\textup{)} vanishes:
  $\Delta_{\mathrm{aff}}^{(r,g)} = 0$ at all $(r,g)$.

\item \emph{Arity $2$, all genera.}
  \begin{equation}\label{eq:ds-tower-arity2}
  \Delta_\phi^{(2,g)}
  \;=\;
  K_N \cdot (H_N - 1) \cdot \lambda_g^{\mathrm{FP}},
  \end{equation}
  independent of the level~$k$.

\item \emph{Arity $3$, genus $0$ \textup{(}Virasoro\textup{)}.}
  \begin{equation}\label{eq:ds-tower-virasoro-cubic}
  \Delta_\phi^{(3,0)}\big|_{N=2}
  \;=\;
  \mathfrak{C}_c + \mathfrak{C}_{26-c}
  \;=\; -26,
  \end{equation}
  independent of the central charge~$c$.

\item \emph{Factorization.}
  $\Delta_\phi^{(2,g)} = K_N \cdot \varrho(\fsl_N) \cdot
  \lambda_g^{\mathrm{FP}}$ where $\varrho = H_N - 1$ is the
  exponent-sum invariant and $K_N = 2(N{-}1)(2N^2{+}2N{+}1)$ is the
  Koszul conductor.

\item \emph{Asymptotics.}
  $\delta_K(\phi) \sim 4N^3 \log N$ for $N \to \infty$.

\item \emph{Arity $\geq 4$: level dependence.}
  The quartic entry $\Delta_\phi^{(4,0)}$ for $\mathrm{Vir}_c$
  depends on~$c$:
  \[
  \Delta_\phi^{(4,0)}\big|_{N=2}
  \;=\;
  \frac{10(10c^2 - 260c + 3952)}{c(5c+22)(26-c)(152-5c)}.
  \]
  Level independence holds at arities $2$ and~$3$ but fails at
  arity~$4$.
\end{enumerate}
\end{theorem}

\begin{proof}
\emph{(i)} For $\widehat{\fg}_k$, $\kappa = (k+h^\vee)d/(2h^\vee)$
is \emph{linear} in the shifted level $t = k + h^\vee$.
The Feigin--Frenkel involution sends $t \mapsto -t$,
hence $\kappa \mapsto -\kappa$, $\Sh_r \mapsto -\Sh_r$
at all arities (the OPE is linear in~$t$:
structure constants are $t$-independent, Killing form proportional
to~$t$).  Therefore $\Sh_r + \sigma(\Sh_r) = 0$.

\emph{(ii)} $K(\mathcal{W}_N^k) = K_N \cdot (H_N - 1)$
(Theorem~\ref{thm:thqg-IV-w-K}).  The genus-$g$ projection:
$\Delta^{(2,g)} = K \cdot \lambda_g^{\mathrm{FP}}$
by the scalar universality of the genus expansion.

\emph{(iii)} Computation~\ref{comp:thqg-IV-cubic-virasoro}:
$\mathfrak{C}_c = -c$ and
$\mathfrak{C}_{26-c} = c - 26$; sum $= -26$.

\emph{(iv)} Immediate from~(ii) and the definition
$\varrho(\fsl_N) = H_N - 1$.

\emph{(v)} $K_N = 4N^3 + O(N^2)$, $H_N - 1 = \log N + O(1)$.

\emph{(vi)} $Q^{\mathrm{contact}}_c = 10/(c(5c+22))$;
$Q^{\mathrm{contact}}_{26-c} = 10/((26-c)(152-5c))$.
Summing: numerator $= 10c^2 - 260c + 3952
= 10(c - 13)^2 + 2262$, which depends on~$c$.
\end{proof}

\begin{remark}[Interpretation]
\label{rem:ds-defect-interpretation}
\index{DS complementarity defect!interpretation}
The tower $\Delta_\phi^{(r,g)}$ measures the failure of
the DS functor to preserve the balanced Lagrangian splitting
$Q_g(\cA) + Q_g(\cA^!) = 0$.
For affine algebras, the splitting is balanced at every arity
and genus ($K = 0$); DS reduction breaks this balance, and the
defect $\delta_K = K_N \cdot \varrho$ is controlled by two
root-datum invariants: the Koszul conductor~$K_N$ (measuring
DS nonlinearity) and the exponent-sum~$\varrho$ (measuring
Lie-theoretic complexity).
The level-independence threshold at arity~$3$
(cubic conductor $= -26$ for Virasoro) and its failure at
arity~$4$ reflects the transition from the Lie bracket
(DS-invariant) to the $A_\infty$ structure (DS-variant).
\end{remark}
...
\index{sigma-invariant shadow ring@$\sigma$-invariant shadow ring!definition}
For a chiral Koszul pair $(\cA, \cA^!)$ with Verdier involution
$\sigma\colon \cA^{\mathrm{sh}} \to \cA^{\mathrm{sh}}$,
the \emph{$\sigma$-invariant shadow ring} is
\begin{equation}\label{eq:sigma-ring-def}
(\cA^{\mathrm{sh}})^\sigma
\;:=\;
\bigl\{x \in \cA^{\mathrm{sh}} : \sigma(x) = x\bigr\}
\;=\;
\bigoplus_{r \geq 2,\, g \geq 0}
\Delta^{(r,g)},
\end{equation}
where $\Delta^{(r,g)} := \Sh_{r,g}(\cA) + \sigma(\Sh_{r,g}(\cA))$.
It inherits a bigraded commutative ring structure from the
$L_\infty$ brackets on~$\cA^{\mathrm{sh}}$.
\end{definition}

\begin{theorem}[Arity-$3$ universality; \ClaimStatusProvedHere]
\label{thm:arity3-universality}
\index{sigma-invariant shadow ring@$\sigma$-invariant shadow ring!universality}
\index{arity-3 universality}
For every principal DS reduction $\phi\colon \widehat{\fg}_k \to
\mathcal{W}(\fg)$ of a simple Lie algebra~$\fg$ with
exponents $m_1, \ldots, m_r$, the $\sigma$-invariant shadow ring
decomposes into two layers:
\begin{enumerate}[label=\textup{(\roman*)}]
\item \emph{Perturbative anomaly ring $R^{\mathrm{pert}}$
  \textup{(}arities $2$--$3$\textup{)}.}
  The entries $\Delta^{(2,g)}$ and $\Delta^{(3,0)}$ are
  level-independent.  On the $T$-line:
  \begin{equation}\label{eq:perturbative-anomaly}
  \Delta^{(2,g)} = K_\fg \cdot \varrho(\fg)
  \cdot \lambda_g^{\mathrm{FP}},
  \qquad
  \Delta^{(3,0)}\big|_{T} = -K_\fg \cdot \varrho(\fg),
  \end{equation}
  where $K_\fg = 2r + 4d \cdot h^\vee$ is the Koszul conductor
  \textup{(}$d = \dim\fg$, $h^\vee$ the dual Coxeter number\textup{)}
  and $\varrho(\fg) = \sum_{i=1}^r 1/(m_i + 1)$ is the exponent-sum
  invariant.

\item \emph{Dynamical anomaly ideal $I^{\mathrm{dyn}}$
  \textup{(}arity $\geq 4$\textup{)}.}
  The entry $\Delta^{(4,0)}$ depends on the central charge~$c$:
  for $\fg = \fsl_2$ \textup{(}Virasoro\textup{)},
  \[
  \Delta^{(4,0)}
  \;=\;
  ...
  {c(5c+22)(26-c)(152-5c)},
  \]
  which is not constant in~$c$.  Higher arities are likewise
  level-dependent.

\item \emph{Threshold.}
  The level-independence threshold is exactly arity~$3$
...
The affine OPE
$J^a(z) J^b(w) \sim k\kappa^{ab}/(z{-}w)^2 + f^{abc}J_c(w)/(z{-}w)$
has double-pole coefficient $k\kappa^{ab}$ \emph{linear} in~$k$ and
simple-pole coefficient $f^{abc}$ \emph{independent} of~$k$.
The Feigin--Frenkel involution $k \mapsto -k - 2h^\vee$ (equivalently
$t = k + h^\vee \mapsto -t$) therefore acts by:
\[
\text{double pole: } k \mapsto -k - 2h^\vee, \qquad
\text{simple pole: } f^{abc} \mapsto f^{abc}.
\]
At arity~$2$: $\kappa = (k + h^\vee)d/(2h^\vee)$ is linear in~$t$;
$\kappa + \kappa' = 0$.  After DS:
$\kappa(\mathcal{W}) = c(k) \cdot \varrho$, and
$\kappa + \kappa' = (c + c') \cdot \varrho = K_\fg \cdot \varrho$,
which is level-independent because
$c + c' = K_\fg = 2r + 4dh^\vee$ is a root-datum constant.

At arity~$3$: the cubic shadow on the $T$-line is
$\mathfrak{C}(\cA) = -c \cdot \varrho$ (Sugawara normal-ordering
at the $T$-subalgebra).  Therefore
$\mathfrak{C} + \mathfrak{C}' = -(c + c') \cdot \varrho
= -K_\fg \cdot \varrho$.  Level-independent.

\emph{(ii)}
The DS central charge $c(k) = r - dh^\vee(t{-}1)^2/t$ is
\emph{quadratic} in~$t$.  The quartic shadow $Q(c)$ is rational
in~$c$; the sum $Q(c) + Q(K_\fg - c)$ is not constant.
For $\fsl_2$: the explicit formula follows from
$Q^{\mathrm{contact}}_c = 10/(c(5c+22))$ and clearing
denominators.

\emph{(iii)}
Combines (i) and (ii): arities $2$--$3$ are level-independent
by the linearity of the affine OPE in~$k$; arity~$4$
is level-dependent by the quadratic nonlinearity of the DS
parametrization.
\end{proof}
...
$F_4$ & $52$ & $9$ & $4$ & $(1,5,7,11)$ & $1880$ & $143/168$ \\
$E_6$ & $78$ & $12$ & $6$ & $(1,4,5,7,8,11)$ & $3756$ & $3127/2772$ \\
$E_8$ & $248$ & $30$ & $8$ & $(1,7,\ldots,29)$ & $29776$
& $\tfrac{1}{2}{+}\tfrac{1}{8}{+}\tfrac{1}{12}{+}\tfrac{1}{14}
{+}\tfrac{1}{18}{+}\tfrac{1}{20}{+}\tfrac{1}{24}{+}\tfrac{1}{30}$ \\
\bottomrule
\end{tabular}
\end{center}
The arity-$2$ perturbative anomaly at genus~$1$ is
