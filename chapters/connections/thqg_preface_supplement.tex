...
boundary $\{0\}\times\C$, a Koszul dual~$\cA^!$ to the bulk, and
a line-operator category~$\cC$ to the interval
...
of the MC element~$\Theta_\cA$.  The gravitational programme does
not add new axioms to the algebraic engine; it extracts new
consequences from the proved core.  The genus expansion of
$\Theta_\cA$ \emph{is} the perturbative expansion of 3d quantum
gravity, the shadow Postnikov tower \emph{is} the complexity
hierarchy of gravitational interactions, and the Koszul involution
$\cA\leftrightarrow\cA^!$ \emph{is} $S$-duality.
...
\item $\cA^!$ is the chiral Koszul dual, obtained by Verdier
  duality on $\Ran(X)$:
  $\mathbb D_{\Ran}\,\barB_X(\cA)\simeq\barB_X(\cA^!)$.  It is
  the bulk chiral algebra.
...
scalar-level entry~$\kappa(\cA)$.  The shadow Postnikov tower
...
residue of the graph-sum formula~\eqref{eq:preface-modular-bar}
...
\smallskip\noindent
\emph{(R3) Shadow as KZ.}\enspace
The modular shadow connection at genus~$0$ and arity~$n$ is the
...
Ward identities, are here a structural consequence of $D^2=0$
on $\overline{\FM}_n(\C)$.
...
is the first $L_\infty$-obstruction class of the shadow tower.
...
$q=e^{i\pi/(k+N)}$, $\kappa_k=kN\dim\mathfrak{sl}_N
/[2(k+N)]$.  The collision $r$-matrix $r(z)=\Omega_N/z$ satisfies
...
connection at genus~$0$ and arity~$n$ is the KZ connection:
...
\kappa_c\;=\;\frac{c}{2}\,.
\end{align*}
The $r$-matrix has a third-order pole (from the double-pole OPE
$T(z)T(w)\sim c/2(z-w)^{-4}+\cdots$): it is a
...
derivatives.  The shadow tower is infinite:
...
irreducible bulk vertices---the hallmark of a gravitational
theory, which is never polynomial.
...
$\kappa_c+\kappa_{c'}=0$ (a polynomial equation in $c,c'$
depending on $N$).  The $r$-matrix is an
...
The Koszul dual $(\beta\gamma)^!$ is a curved chiral algebra
with curvature $m_0=\omega$; the negative curvature $\kappa=-1$
reflects the fermionic statistics (spin-statistics: bosonic
$\beta\gamma$ contributes $-1$ to the central charge per pair).
The holographic datum encodes the minimal example with a
nontrivial quartic interaction: the simplest genuinely
nonlinear gravitational theory.
...
\end{align*}
with $\kappa(\cA)+\kappa(\cA^!)=0$ at the scalar level.  The
...
\bigl(1-k\cdot K_g^{(1)}\bigr)^{-1/2},
\]
where $K_g^{(1)}$ is the genus-$g$ Bergman kernel acting on the
one-particle Hilbert space $\cH_k^{(1)}\cong L^2(\Sigma_g)$.
The determinant is a Fredholm determinant: it converges because
$K_g^{(1)}$ is trace-class on~$L^2(\Sigma_g)$ (the Bergman kernel
is smooth away from the diagonal and square-integrable).
The formula $Z_g=\det(1-kK)^{-1/2}$ is the exact partition
function at every genus, not a perturbative approximation.  Its
logarithm is
\[
\log Z_g(\cH_k)
\;=\;
-\frac{1}{2}\operatorname{tr}\log(1-kK_g^{(1)})
\;=\;
\frac{1}{2}\sum_{n\ge 1}\frac{k^n}{n}\,
\operatorname{tr}(K_g^{(1)})^n.
\]
...
The scalar-level genus expansion of the MC element
gives the free energies $F_g(\cA)=\kappa(\cA)\cdot\lambda_g$,
where $\lambda_g$ is the universal coefficient at genus~$g$.
The generating function assembles into the $\hat A$-genus:
\begin{equation}\label{eq:Ahat-preface-supp}
\sum_{g\ge 1}F_g(\cA)\,\hbar^{2g-2}
\;=\;
\kappa(\cA)\cdot
...
is the Hirzebruch $\hat A$-genus, evaluated at $x=i\hbar$.
The $\hat A$-function appears because the universal coefficients
$\lambda_g=B_{2g}/(2g(2g-2))$ are ratios of Bernoulli numbers,
and $\hat A$ is the exponential generating function for these
ratios.

The gravitational interpretation:
equation~\eqref{eq:Ahat-preface-supp} states that the
perturbative partition function of the HT theory is an
$\hat A$-genus, weighted by the gravitational central charge
$\kappa(\cA)$.  The $\hat A$-genus is the index density of the
Dirac operator; the genus expansion of $\Theta_\cA$ produces it
as a formal consequence of $D^2=0$ on the moduli space of
...
inclusion for the entire standard landscape.
...
the smallest integer~$r$ such that the shadow Postnikov tower
...
The MC element $\Theta_\cA=\kappa\cdot\eta\otimes\Lambda$ is
purely scalar.  All higher shadows vanish: $\mathfrak C=0$,
...
$\psi\bar\psi$, $bc$-systems of any weight.  Every abelian
chiral algebra belongs to class~G.

The gravitational content of a class~G theory is entirely
captured by the single scalar~$\kappa$.  The genus-$g$ partition
function is the $\hat A$-genus weighted by~$\kappa$, the shadow
connection is flat with constant curvature, and the modular
homotopy type is formal: no Massey products, no higher
obstructions, no contact interactions.
...
\emph{Representatives}: $\beta\gamma$-system at any value of the
conformal dimension.  The quartic contact invariant of $\beta\gamma$
is
\[
...
The shadow tower does not terminate: there exist irreducible
...
$\mathcal W$-algebras.  The quintessential class~M algebra is
...
shadow Postnikov tower---an infinite sequence of irreducible
bulk vertices that cannot be truncated.  This is the hallmark
of gravity: Einstein gravity is not renormalisable in the
power-counting sense, and the infinite tower of higher-derivative
corrections reflects this non-polynomiality in the modular
Koszul framework.

\subsubsection*{The operadic complexity conjecture}
...
the shadow tower terminates at arity~$r$ iff the minimal
...
locus at all.  The twelve equivalent conditions for Koszulness
(Theorem~\ref{thm:koszul-equivalences-meta}) are orthogonal to
shadow depth.
...
Q_g(\cA)\;\oplus\;Q_g(\cA^!),
\]
with $\kappa(\cA)+\kappa(\cA^!)=0$.  The splitting is the
...
at the scalar level; at higher arities it imposes constraints
on the shadow tower of the pair $(\cA,\cA^!)$.
...
shadow connection has $\kappa(\mathrm{Vir}_{13})=13/2$ and
$\kappa(\mathrm{Vir}_{13}^!)=-13/2$: the curvatures are
...
involution is the Feigin--Frenkel duality
$k\mapsto-k-2h^\vee$:
\[
...
\]
The fixed point $k=-h^\vee$ is the critical level, where the
Sugawara construction is undefined ($\kappa\to\infty$) and the
center becomes the algebra of $\mathfrak g$-opers
(Feigin--Frenkel).  Self-duality for the affine family means
$k=-k-2h^\vee$, i.e.\ $k=-h^\vee$, which is \emph{not} the
self-dual point but the singular point.  The affine Koszul
involution has no smooth fixed point: the duality interchanges
$k$ and $-k-2h^\vee$ with no fixed generic level.
...
\kappa_k=\frac{3k}{k+2},
\qquad
\kappa_{-k-4}=\frac{3(-k-4)}{(-k-4)+2}
=\frac{-3(k+4)}{-(k+2)}=\frac{3(k+4)}{k+2}\,,
\]
and $\kappa_k+\kappa_{-k-4}=3k/(k+2)+3(k+4)/(k+2)
=3(2k+4)/(k+2)=6$.  The curvatures sum to a nonzero constant
$6=2\dim\mathfrak{sl}_2$---the complementarity ``constant'' for
the affine family, analogous to $13$ for Virasoro.
...
$R_{\mathrm{Vir}}$ at depth zero in the shadow tower.  It is
...
(Conjecture~\ref{conj:platonic-completion}).  The distinction is
...
The condition $\kappa(\cA)+\kappa(\cA^!)=0$ ensures that the
...
double-pole OPE of the stress tensor.
...
the dg-shifted stress-tensor Yangian, a new algebraic object
whose representation theory governs the line-operator category
of 3d gravity.
...
analogous statement is that $\cC_c\simeq
\Ydg(\mathrm{Vir}_c)\text{-mod}^{\mathrm{fd}}$, which is the
content of the Yangian-shadow target theorem.

The modular extension: define $\mathcal{Y}^{\mathrm{dg,mod}}(\cA)$
as the pronilpotent completion of the convolution dg Lie algebra
at genus~$0$.  The modular $R$-matrix
$R^{\mathrm{mod}}_\cA(z;\hbar)$ is an MC element
...
$R_{12}R_{13}R_{23}=R_{23}R_{13}R_{12}$ is the MC equation for
$R^{\mathrm{mod}}$.  The genus expansion of the $r$-matrix
parallels the genus expansion of the MC element: each genus-$g$
correction is a projection of $\Theta_\cA^{(g,2)}$.
...
The shadow connection at genus~$g$ and arity~$n$ is a KZ-type
connection with $r^{(g)}$ as its structure matrices:
\[
\nabla^{\mathrm{hol}}_{g,n}
\;=\;
...
% ====================================================================
\subsection*{Soft theorems from the shadow tower}
% ====================================================================
...
The hierarchy of soft theorems mirrors the shadow Postnikov
tower:
\begin{center}
\renewcommand{\arraystretch}{1.15}
\begin{tabular}{lll}
...
\end{tabular}
\end{center}
Each level of the shadow tower produces one order of the
...
The contrast with Virasoro is sharp.  For gravity (class~M), the
...
The generating function for the scalar soft factors at all genera
is controlled by the $\hat A$-genus.  The genus-$g$ leading
soft factor is $\kappa(\cA)\cdot\lambda_g$, and the generating
function
$\sum_{g\ge 1}\lambda_g\,\hbar^{2g-2}=\hat A(i\hbar)-1$
assembles the soft factors into the $\hat A$-class.  The
$\hat A$-genus is therefore the universal soft factor for the
scalar-level shadow: it is the generating function for the
leading soft graviton theorem at every genus.
...
increasingly overdetermined, and the consistency conditions become
...
the Koszul dual $\mathrm{Vir}_0$ is uncurved, so the bar
complex $\barB(\mathrm{Vir}_0)$ is a strict dg coalgebra (no
curvature term, $d^2=0$ on the nose).  The cobar construction
...
is an honest quasi-isomorphism of dgas (not merely of curved
$A_\infty$-algebras).

Define the \emph{transgression algebra} of $\mathrm{Vir}_c$ as
the cobar of the dual bar:
\[
\mathrm{Tr}_c
\;:=\;
\Omega\bigl(\barB(\mathrm{Vir}_{26-c})\bigr).
\]
At $c=26$: $\mathrm{Tr}_{26}
=\Omega(\barB(\mathrm{Vir}_0))
\simeq\mathrm{Vir}_0$, a strict quasi-isomorphism.  The
transgression algebra is the free resolution of the dual.

At $c\neq 26$: $\mathrm{Vir}_{26-c}$ is curved
($\kappa_{26-c}=(26-c)/2\neq 0$), the bar complex is curved,
and the cobar $\Omega(\barB(\mathrm{Vir}_{26-c}))$ is a curved
$A_\infty$-algebra with $m_0=(26-c)/2\cdot\omega$.  The
transgression algebra exists but is not strict: the curvature
of the dual prevents the cobar from being a genuine dga.

The critical-string dichotomy:
\begin{itemize}
\item At $c=26$: $\mathrm{Tr}_{26}$ is strict.  The BRST
  complex of the bosonic string is a strict dg algebra, and the
  physical state space is $H^*(\mathrm{Tr}_{26})$---ordinary
  cohomology of a cochain complex.  No curved $A_\infty$
  machinery is needed.
\item At $c\neq 26$: $\mathrm{Tr}_c$ is curved.  The ``BRST
  complex'' has $d^2\neq 0$, and the physical state space must
  be defined in the curved or coderived sense.  The theory
  requires the full machinery of curved Koszul duality.
\end{itemize}
The critical dimension $c=26$ is the unique value where the
transgression algebra becomes strict: where the gravitational
path integral reduces to ordinary cohomology.

\subsubsection*{The coderived passage at $c\neq 26$}

At generic $c$, the curvature $\kappa_{26-c}\neq 0$ forces
...
\subsubsection*{The BRST complex as bar construction}

The BRST complex of the bosonic string is the bar construction
of the combined matter-ghost system.  The matter central charge
$c_{\mathrm{matter}}$ and the ghost central charge
$c_{\mathrm{ghost}}=-26$ sum to
$c_{\mathrm{total}}=c_{\mathrm{matter}}-26$.  The BRST
differential $Q_{\mathrm{BRST}}$ satisfies
\[
Q_{\mathrm{BRST}}^2
\;=\;
\kappa(\mathrm{Vir}_{c_{\mathrm{total}}})\cdot\omega
\;=\;
\frac{c_{\mathrm{matter}}-26}{2}\cdot\omega.
\]
At $c_{\mathrm{matter}}=26$: $Q_{\mathrm{BRST}}^2=0$, and the
BRST cohomology is well-defined.  At
$c_{\mathrm{matter}}\neq 26$: the BRST differential is curved,
and physical states must be defined coderived.

The bar construction encodes this precisely.  The bar complex
$\barB(\mathrm{Vir}_{c_{\mathrm{total}}})$ has curvature
$\kappa_{c_{\mathrm{total}}}=(c_{\mathrm{matter}}-26)/2$.  The
vanishing of this curvature \emph{is} the no-anomaly condition
of the bosonic string.  The modular characteristic $\kappa$ is
the anomaly.
...
dual Swiss-cheese algebra is uncurved, and the open-closed BRST
complex is strict.
...
$\kappa(\mathrm{SVir}_c)+\kappa(\mathrm{SVir}_{15-c})=0$.  The
critical dimension is $c=15$ (ten spacetime dimensions):
$\kappa(\mathrm{SVir}_0)=0$, and the dual bar complex is
uncurved.
...
\kappa_0=0.
\end{align*}
The pattern is identical to the bosonic case: the critical
dimension is where the dual is uncurved, the self-dual point is
the midpoint of the Koszul involution, and the shadow tower
classifies the gravitational complexity.  The only difference is
the numerical value of the critical charge ($15$ instead of $26$)
and the presence of the fermionic generator~$G$.
...
This reflects the physical expectation that higher-spin
theories in three dimensions are never ``critical'' in the
string-theoretic sense: they are always curved, always requiring
the full coderived machinery.  The bosonic string's critical
dimension $c=26$ is a finite-type phenomenon specific to the
Virasoro algebra; it does not survive the higher-spin limit.
...
chapters of Part~III of this monograph:
...
\medskip\noindent
The constructions of Parts~I and~II provide the MC element, the
shadow tower, the Koszul dual, and the genus expansion.  Part~III
interprets these in the gravitational context.  Part~IV pushes
