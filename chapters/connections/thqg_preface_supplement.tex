...
is the residue of the genus-$0$, degree-$2$ component of
...
$q=e^{i\pi/(k+N)}$, $\kappa_k=(N^2-1)(k+N)/(2N)$. The collision $r$-matrix $r(z)=k\,\Omega_N/z$ satisfies
...
connection at genus~$0$ and degree~$n$ identifies with the KZ
...
at principal nilpotent, the shadow depth jumps from $3$ (affine) to
...
\]
where $\mathrm{Sh}_{g,r}$ is the genus-$g$, degree-$r$ shadow
...
$(g,n)$-stratum. At genus~$0$ and degree~$n$ it reduces to the
KZ equation. At genus~$1$ and degree~$0$ it gives
$\kappa(\cA)\cdot\omega_1=0$ on the total space, the curvature
equation. At genus~$1$ and degree~$1$ it is the heat equation for
...
The MC element is purely scalar in degree:
...
higher-degree interactions appear. The shadow connection is flat
...
nonzero: the degree-$4$ log-FM residue produces a nontrivial
...
degrees. The bulk theory has infinitely many independent
...
beyond degree~$4$, and the infinite shadow depth is forced by the
conformal anomaly.
...
the shadow obstruction tower terminates at degree~$r$ iff the minimal
...
are proved at degrees $2$, $3$, $4$
(Proposition~\ref{prop:shadow-formality-low-degree}): the shadow
tower agrees with the $L_\infty$-formality obstruction tower at
low degrees. The conjecture extends this to all degrees.
...
\item \emph{DS reduction increases depth}: for
 $\widehat{\mathfrak{sl}}_N\to\mathcal W_N$, the shadow depth jumps
...
for Virasoro, $\kappa+\kappa^!=13$); at higher degrees it
...
The genus-$0$, degree-$2$ component of $\Theta_\cA$, evaluated
...
$r(z)=k\,\Omega/z$, the full Casimir element at level~$k$. For Virasoro:
...
lowest-degree, lowest-genus projection of the MC element, and
...
degree-$3$ component of the MC element. The MC equation at
genus~$0$ and degree~$3$ is
...
is the $d^2=0$ identity at genus~$0$ and degree~$3$, viewed
...
The shadow connection at genus~$g$ and degree~$n$ has a KZ-like
binary leading term with $r^{(g)}$ as its structure matrices,
together with separate higher-degree corrections:
\[
\nabla^{\mathrm{hol}}_{g,n}
\;=\;
...
\]
The higher-degree shadows $\mathrm{Sh}_{g,r}$ for $r\ge 3$ are the
...
The shadow connection at genus~$0$ and degree~$4$, in the limit
$z_4\to z_3$, gives:
\[
...
\hline
$\kappa$ (degree $2$) & leading soft
 & BMS supertranslation \\[2pt]
$\mathfrak C$ (degree $3$) & subsubleading soft
 & stress-tensor three-point coupling \\[2pt]
$\mathfrak Q$ (degree $4$) & contact soft
 & four-graviton contact interaction \\[2pt]
$\mathrm{Sh}_r$ (degree $r$) & order-$(r-2)$ soft
 & $r$-graviton vertex
\end{tabular}
\end{center}
Each level of the shadow obstruction tower produces one order of the
soft expansion. The infinite shadow depth of the Virasoro tower
...
soft expansion never terminates: every degree contributes an
...
$[r,r]=[k\,\Omega/z_1,k\,\Omega/z_2]$, which reduces to
$k^2[\Omega,\Omega]/(z_1z_2)=0$
...
\end{tabular}
\end{center}
The number of bulk vertices is $r_{\max}-2$: the first two degrees
are the propagator and the two-point function (kinematic data),
and each higher degree contributes one dynamical interaction vertex.
...
the MC recursion converges in both the genus and degree
