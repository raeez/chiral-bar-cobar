...
\begin{itemize}
\item At $c=26$: $\mathrm{Tr}_{26}$ is strict.  The algebraic
  transgression model is an ordinary cochain complex, so no curved
  $A_\infty$ machinery is needed for this algebraic object.
  Identifying $H^*(\mathrm{Tr}_{26})$ with the physical state space of
  the full bosonic string would require additional BRST/bar comparison
  input and is not proved here.
\item At $c\neq 26$: $\mathrm{Tr}_c$ is curved.  The ``BRST
  complex'' has $d^2\neq 0$, so the algebraic transgression object
  naturally lives in the curved or coderived sense.  Any physical
  state interpretation requires extra input beyond this algebraic
  dichotomy.  The theory requires the full machinery of curved Koszul
  duality.
\end{itemize}
The critical dimension $c=26$ is the unique value where the
transgression algebra becomes strict at the algebraic level; beyond
that, one must use curved or coderived replacements instead of
ordinary cohomology.
