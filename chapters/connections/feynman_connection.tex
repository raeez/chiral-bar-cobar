...
\begin{remark}[Shadow-Feynman dictionary: $S_r = (r{-}1)$-loop graph sum]
...
(Remark~\ref{rem:feynman-bar-bridge}), the shadow tower coefficients
$S_r(\cA)$ for $r \geq 2$ correspond to the connected $(r-1)$-loop
graph sums of the chiral perturbative expansion: $S_2 = \kappa(\cA)$
is the one-loop tadpole anomaly, $S_3$ is the two-loop sunset,
$S_4$ is the three-loop watermelon, and the
quadrichotomy of shadow class
$\mathsf{G}/\mathsf{L}/\mathsf{C}/\mathsf{M}$ controls
diagrammatic complexity:
\begin{itemize}
\item Class~$\mathsf{G}$ (Heisenberg): tower terminates at
 $r = 2$; only the one-loop tadpole survives, all higher
 connected loop sums vanish.
\item Class~$\mathsf{L}$ (affine Kac--Moody): tower terminates at
 $r = 3$; connected loop sums vanish from two loops upward.
\item Class~$\mathsf{C}$ (free $\beta\gamma$): tower terminates at
 $r = 4$; the three-loop watermelon is the highest non-vanishing
 graph sum.
\item Class~$\mathsf{M}$ (Virasoro, $\Walg_N$): tower extends to
 $r = \infty$; every connected loop graph sum is non-vanishing,
 and the asymptotic ratio $\limsup |S_{r+1}/S_r| = C_\cA$
 records the exponential base of the loop-amplitude growth (e.g.\
 $C_{\mathrm{Vir}} = 6$, $C_{W_3}^{W\text{-line}} = 12$).
\end{itemize}
This shadow-Feynman dictionary is the loop-counting interpretation
of the depth quadrichotomy
(Theorem~\ref{thm:depth-decomposition}): the diagrammatic
complexity of the perturbative expansion is bounded by the algebraic
depth of the shadow tower.
\end{remark}

\begin{remark}[Wave-$14$ structural cross-references]
\label{rem:feynman-wave14-anchors}
\index{wave-14 anchors!Feynman entry point}
The Feynman picture intersects the wave-$14$ anchor map at three
points:
\begin{enumerate}[label=\textup{(W\arabic*)}]
\item \emph{Universal conductor.} The free-boson partition function
 $Z_g^{\mathcal{H}_\kappa} = \kappa\,\lambda_g/24$
 (genus~$g$ Heisenberg) is the per-family instance of the
...
\item \emph{Climax identity.} The bar differential
 $d^{(0)}[a \otimes a \otimes \eta_{12}] = \kappa^{\mathrm{Heis}} \cdot \mathbf{1}$
 displayed in equation~(2) of this chapter is the Heisenberg
 Feynman-rule realisation of the climax identity
 $d_{\mathrm{bar}} = \mathrm{KZ}^*(\nabla_{\mathrm{Arnold}})$
 of Chapter~\ref{ch:climax-theorem}: the Arnold form
 $\eta_{12}$ pulled back through the KZ map produces the
 free-boson propagator residue $\kappa^{\mathrm{Heis}}$.
\item \emph{Cross-volume bridge.} The all-genera Heisenberg
 ...
 lifts to Vol~II's universal celestial holography on the closed
 colour, where the path-integral identification becomes the
 universal master theorem
 (Vol~II Theorem~\ref{thm:universal-holography-master}); for
 ...
 with the conjectural conductor comparison to the proved Borcherds
