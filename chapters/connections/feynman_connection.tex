...
(\mathcal{H}_{\kappa^{\mathrm{Heis}}})
=(-1)^g\kappa^{\mathrm{Heis}}\lambda_g,\qquad
...
\item \emph{Universal conductor.} The free-boson partition function
 ...
 Under~$H_D^K$ its genus-$g$ Hodge character is
 $\operatorname{ch}_g(\mathfrak O_g^K(\mathcal H_\kappa))
 =(-1)^g\kappa\lambda_g$, and under~$H_D^{\mathrm{graph}}$ the
 genus-$1$ scalar free energy is
...
Feynman-rule realisation of the typed KZ--Arnold
bar-superconnection theorem of Chapter~\ref{ch:climax-theorem}:
after Fulton--MacPherson boundary-residue realization, the local
Arnold representative supplies the logarithmic collision form, and the
double-pole OPE supplies the scalar residue
$\kappa^{\mathrm{Heis}}$.
...
the Voronov Swiss-cheese Deligne conjecture~\cite{Voronov99} and the
Kontsevich--Soibelman Deligne conjecture~\cite{KS00} \textup{(}the
Swiss-cheese operad itself is not formal, by Livernet's obstruction;
the argument routes around formality\textup{)}. The proof
...
bar--cobar reconstruction,
$K_X(\cA):=\mathbb{D}_{\Ran}\bar{B}_X(\cA)$ is the Verdier algebra,
$\nu_\cA\colon K_X(\cA)\to\cA^!$ is the chosen-pair comparison, and
