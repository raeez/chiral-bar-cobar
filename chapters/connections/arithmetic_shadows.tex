...
The shadow obstruction tower $\Theta_\cA^{\leq r}$
...
(the shadow obstruction tower detects the \emph{algebra
...
and the shadow obstruction tower organizes the algebraic data that,
for lattice vertex algebras, connects to both the Langlands programme
(arithmetic) and the Sullivan programme (homotopy theory) through the
single Maurer--Cartan element $\Theta_\cA$.

\smallskip
\noindent\emph{Epistemic scope.}\;
The Langlands programme and the shadow obstruction tower occupy
\emph{orthogonal} axes: at the critical level $k = -h^\vee$
the modular characteristic $\kappa$ vanishes, the bar complex becomes
uncurved, and the bar cohomology $H^*(\barB(\fg_{-h^\vee}))$
recovers the formal-disc Feigin--Frenkel center and its higher
...
The scalar shadow obstruction tower assigns $\kappa = 24$ and
...
classical modular form of any finite weight.  The shadow obstruction tower
...
The arithmetic content of the shadow obstruction tower is extracted by an
...
hence at \emph{complex} spectral parameters.  The shadow obstruction tower and
...
The five verifications above exhibit a deficiency that is
intrinsic to the scalar shadow obstruction tower.
...
the scalar shadow obstruction tower is completely blind to the root
system~$R(\Lambda)$, the theta function~$\Theta_\Lambda$, and
the lattice geometry.
This subsection resolves the Niemeier discrimination problem
at three levels of the MC~hierarchy, each strictly refining
the previous.
...
$24$ lattices.
The full scalar shadow obstruction tower $(S_r)_{r \geq 2} = (24, 0, 0, \ldots)$
is universal.
...
Adjoining $h$ costs nothing computationally (the
PBW filtration is already present in the bar complex)
but adds complete resolution.
...
algebra admits a canonical decomposition into
oscillator and lattice contributions.
...
\item $Z_g^{\mathrm{osc}}(\Omega)
  ...
  the scalar shadow obstruction tower
...
\item $\Theta_\Lambda^{(g)}(\Omega)
  ...
  arithmetic.
\end{enumerate}
In particular, the scalar shadow obstruction tower controls
...
The per-factor bar data is a \emph{lossy} projection
...
Each arity of the shadow obstruction tower detects exactly one
...
the shadow obstruction tower $\Theta_{V_\Lambda}^{\le r}$ decomposes
...
$\mathrm{Re}(s)=\frac14$.  The shadow obstruction tower terminates at
...
from the Epstein zeta function.  This is the shadow obstruction tower's
...
The non-lattice regime is where the shadow obstruction tower
...
The Virasoro shadow obstruction tower provides an
...
$d_{\mathrm{arith}} = 0$: the infinite shadow obstruction tower at a
...
scalar shadow obstruction tower can detect the orbifold transition.
\end{remark}
...
the spectral measure collapses and the shadow obstruction tower
...
The measure~$\rho$ is the spectral measure of the shadow obstruction tower:
...
The infinite Virasoro shadow obstruction tower
...
mixed-Tate world.  The shadow obstruction tower is the
...
The shadow obstruction tower admits two non-perturbative extensions:
an archimedean Borel analysis of the formal power series,
and a $p$-adic valuation of its coefficients.
Both reveal structural arithmetic content invisible to
...
The shadow obstruction tower has Borel singularities in two
...
This is the resurgent incarnation of the critical-dimension
...
The shadow obstruction tower inherits this structure: the arity-$r$
shadow coefficients $S_r(\cA)$, when reduced modulo~$p$,
...
the shadow obstruction tower to the Iwasawa-theoretic framework.
\end{remark}
...
\end{equation}
is verified for all standard families.  This is the
...
\item The Nahm eigenvalue of the shadow metric satisfies
  $\lambda_{\mathrm{Nahm}} = \rho^2$, where
  $\rho = \kappa/c$ is the anomaly ratio.
...
appear as periods of the moduli spaces~$\cM_{0,n}$.
...
The shadow obstruction tower invariants of a chirally Koszul algebra~$\cA$
...
The identification $r(z) = \Omega/z$ with the Casimir
...
are activated, and the shadow obstruction tower coefficients
...
each weight is an open problem connected to the
...
properties encode the shadow data.  The following
classification is exhaustive for the standard landscape.
...
genus-$0$ property.
$\kappa(V^\natural) = 12$.
The vanishing constant term $c(0) = 0$ is the
...
The shadow obstruction tower at genus~$1$ sees the Eisenstein part
...
These are \emph{not} determined by the shadow obstruction tower:
...
coefficient $c_\Delta$, which the shadow obstruction tower cannot
detect.
\end{enumerate}
\end{proposition}
...
\]
The cubic shadow $S_3^2 = 4$ always dominates the product $\kappa \cdot S_4 = 5/(5c{+}22)$: the shadow obstruction tower is intrinsically \emph{non-Gaussian} at the moment level.
\end{proposition}
...
($q = e^{2\pi i \tau}$).  The former is a Dirichlet series in the shadow coefficients; the latter is a $q$-deformation of the shadow obstruction tower incorporating genus-$1$ modular structure.  At the self-dual point $c = 13$, the Koszul self-duality $S_r(13) = S_r(13)$ gives $Z_{\mathrm{sh}}(s, 13)$ a trivial functional equation under $c \to 26{-}c$.
\end{construction}

\begin{theorem}[The shadow $L$-function is Eisenstein]
...
\index{Rankin--Selberg unfolding!shadow coefficients}
Define the \emph{shadow $L$-function}
\begin{equation}\label{eq:shadow-l-function-def}
  L_\cA^{\mathrm{sh}}(s)
  \;:=\;
  \sum_{r \ge 2} S_r(\cA)\,r^{-s},
\end{equation}
where $S_r(\cA)$ are the shadow coefficients of a chirally
Koszul algebra~$\cA$.  Then
\begin{equation}\label{eq:shadow-eisenstein-identity}
  L_\cA^{\mathrm{sh}}(s)
  \;=\;
  -\kappa(\cA)\,\zeta(s)\,\zeta(s-1).
\end{equation}
In particular, $L_\cA^{\mathrm{sh}}$ is an Eisenstein series
in the Selberg spectral decomposition\textup{:} it is
\emph{not} cuspidal.

The intertwining kernel connecting the graph-sum amplitudes
$G_r$ to the shadow $L$-function is
...
\begin{proof}
The shadow coefficients of an algebraic family satisfy
$S_r = \kappa \cdot B_{2r}/(2r)!$ at leading order, where
$B_{2r}$ are the Bernoulli numbers
(Theorem~\ref{thm:shadow-tower-asymptotics}, the
$\hat{A}$-genus generating function).  The Dirichlet
series of the Bernoulli ratios is classical:
\[
  \sum_{r \ge 1} \frac{B_{2r}}{(2r)!}\,r^{-s}
  \;=\;
  -\zeta(s)\,\zeta(s-1)
\]
(Ramanujan's identity, a consequence of the Mellin transform
of $x/\sinh x$; the right side is the Dirichlet series of
$\sigma_{-1}(n) = \sum_{d \mid n} d^{-1}$, and the
Bernoulli ratios arise from the Laurent expansion of the
cotangent).  Substituting $S_r = \kappa \cdot B_{2r}/(2r)!$
yields~\eqref{eq:shadow-eisenstein-identity}.

The Rankin--Selberg unfolding of the shadow-spectral
correspondence
(Theorem~\ref{thm:shadow-spectral-correspondence})
decomposes the constrained Epstein zeta into Eisenstein and
cuspidal contributions.  The shadow $L$-function
$L_\cA^{\mathrm{sh}}$ contributes entirely to the Eisenstein
part: its Mellin transform against $E_s(\tau)$ is a product
of zeta functions with no cuspidal projection.  This is
consistent with the structural diagnosis
(\S\ref{sec:modular-rigidity}): the shadow coefficients
$S_r$ are determined by the algebraic data of~$\cA$ alone,
carrying no spectral-parameter dependence.

For~\eqref{eq:intertwining-kernel}: the Mellin
transform of the arity-$r$ graph amplitude
...
$L$-functions.  The connection to the shadow--DK bridge
...
frame bundle.  These are \emph{identically} the Bernoulli
numbers controlling the shadow $\hat{A}$-genus
generating function of
Theorem~\ref{thm:shadow-tower-asymptotics}: the
shadow coefficient $S_r = \kappa \cdot B_{2r}/(2r)!$ and
the CS perturbative coefficient
$a_n^{\mathrm{CS}} = \mathrm{vol}(M) \cdot B_{2n}/(2n)!$
share the same Bernoulli factor.  The coincidence is
structural: both arise from the $\hat{A}$-genus of the
tangent bundle to the moduli space of flat connections
(for CS) and the moduli space of curves (for the shadow
tower), and the Bernoulli numbers enter via the Todd
...
The shadow obstruction tower of the Virasoro algebra carries a
...
generators, the shadow obstruction tower carries a triple of compatible
...
\textup{(}order~$3$\textup{)} places the shadow obstruction tower in the
...
The shadow obstruction tower defines a spectral triple
...
invariant bilinear form.  The shadow obstruction tower defines a BPS spectrum
...
the DT/PT wall-crossing formula exactly:
...
For the Heisenberg algebra at central charge~$c$, the shadow obstruction tower
...
The structural reason to expect the principal class: the
...
$2\Delta\,t^2$ preserves this is unclear.
...
The entire shadow obstruction tower $\{S_r\}_{r \geq 2}$ is determined by
...
has been verified computationally for the Virasoro shadow metric
...
The shadow obstruction tower and the zeta-zero interface live on different
layers: the shadow obstruction tower organizes the MC deformation data,
...
permanent: the shadow obstruction tower modifies the Dirichlet
...
The root cause is spectral, not arithmetic\textup{:}
\begin{enumerate}[label=\textup{(\roman*)}]
\item
...
questions through the shadow obstruction tower must bypass the Li
...
The structural consequence: the Taylor expansion of
...
This is the natural generating function for the Li coefficients;
...
input beyond the shadow obstruction tower.
\end{remark}
...
The proof's own caveat acknowledges unverified
Phragm\'en--Lindel\"of estimates for $\mathrm{GL}(j)$ twists
at $j \geq 3$.  The polynomial growth hypothesis~(c) requires
...
not been verified in the present context.  The theorem is
logically valid conditional on these analytic estimates;
the algebraic-combinatorial content (meromorphic continuation,
...
(the MC element $\Theta_\cA \in \MC(\gAmod)$, the shadow obstruction tower
...
The structural obstruction admits a representation-theoretic
reformulation that clarifies its depth.
The bar complex~$\barBch(\cA)$ is constructed from the OPE
data of~$\cA$: the structure constants, the central charge, the
conformal weights of generators.  This is data of the
\emph{algebra} itself.  The primary spectrum
$\{\Delta_i\}_{i \in I}$ (the set of conformal dimensions
of irreducible modules) is data of the
\emph{representation theory} of~$\cA$.  The shadow obstruction tower, the
MC equation, and the Koszul--Epstein function all live in the
algebraic world: they are determined by $\cA$ as an algebra, not
by $\cA\text{-}\mathrm{mod}$ as a category.  The zeros
of~$\zeta(s)$ enter through the spectral decomposition of the
partition function, which requires summing over the full primary
...
The preceding sections establish two different
kinds of arithmetic output from the MC element $\Theta_\cA$.
The \emph{algebraic output} (the shadow obstruction tower, the Hecke
...
\begin{proposition}[Hecke defect for standard families;
\textup{(i)} \ClaimStatusProvedHere;
\textup{(ii)} \ClaimStatusProvedHere;
\textup{(iii)} \ClaimStatusConjectured]%
...
\item \emph{Virasoro:}
  ...
  $\tau$-independence of the shadow obstruction tower on the scalar lane.
...
The bar complex at critical level computes the formal-disc oper
...
curvature obstruction; the resulting uncurved deformation complex has
cohomology governed by the oper differential-form package.  The
manuscript does not identify its Maurer--Cartan moduli with the oper
space.
...
shadow obstruction tower operates at generic level.  These are
complementary, not competing: the Langlands side governs the
\emph{categorical} structure of $\widehat{\fg}$-modules, while
the shadow obstruction tower governs the \emph{scalar} arithmetic invariants
...
This section gives a complete accounting of the prime-locality
programme, distinguishing what is proved from what is
structurally obstructed.  The Beilinson principle applies: every
claim is false until independently verified, and false ideas
must be dismissed before they propagate.
...
Route~C (Theorem~\ref{thm:route-c-propagation}) claims to
reduce prime-locality to two hypotheses: character-level Hecke
equivariance~(a) and low-arity determination~(b).  The argument
is that the MC recursion determines $S_r$ from
$\{S_2, S_3, S_4\}$ at high arity, and if the low-arity data
is Hecke-equivariant, then so is the high-arity data.
...
The shadow obstruction tower satisfies a strong algebraic constraint that
has arithmetic content but is \emph{not} prime-locality in the
...
fails axioms S3 \textup{(}Euler product\textup{)} and S4
\textup{(}Ramanujan hypothesis on coefficients\textup{)} of the
...
\item \emph{No Euler product.}
  $L_\cA^{\mathrm{sh}}$ is a product of two zeta functions
  \textup{(}an Eisenstein series\textup{)}, not a single
  Euler product.  The Dirichlet coefficients
  $a_n = -\kappa\,\sigma_{-1}(n)$ are multiplicative, but
  $L_\cA^{\mathrm{sh}}$ is already the Rankin--Selberg
  convolution $\zeta \otimes \zeta$, not a primitive
  $L$-function.  The shadow $L$-function of a tensor product
  satisfies $L_{A \otimes B}^{\mathrm{sh}} = L_A^{\mathrm{sh}}
  + L_B^{\mathrm{sh}}$ \textup{(}additive in~$\kappa$, not
  multiplicative in the Euler product sense\textup{)}: there is
  no factorisation
  $L_\cA^{\mathrm{sh}}(s) = \prod_p F_p(p^{-s})^{-1}$
  over primes~$p$ with polynomial local factors~$F_p$,
  because $\zeta(s)\,\zeta(s{-}1)$ is the Rankin--Selberg
  convolution of two primitive $L$-functions.
\item \emph{Coefficient growth.}
  $|a_n| = |\kappa|\,\sigma_{-1}(n) = O(n^\varepsilon)$ for
  all $\varepsilon > 0$, so S4 actually holds individually.
  The failure is S3 alone.
\item \emph{Subconvexity.}
  The convexity bound
  $\mu(1/2) = 0$ for class~$\mathbf{M}$ algebras
  \textup{(}bounded on the critical line\textup{)}: the
...
$L_\cA^{\mathrm{sh}} = -\kappa\,\zeta(s)\,\zeta(s{-}1)$
...
Additivity $L_{A \otimes B}^{\mathrm{sh}} = L_A^{\mathrm{sh}}
+ L_B^{\mathrm{sh}}$ follows from the additivity of~$\kappa$
(Theorem~D, the independent-sum factorization):
$\kappa(A \otimes B) = \kappa(A) + \kappa(B)$.  The
subconvexity bound follows from the exponential decay of
$S_r$ (Theorem~\ref{thm:shadow-tower-asymptotics}): the
Dirichlet series converges absolutely in a half-plane
strictly to the left of $\operatorname{Re}(s) = 1$.
\end{proof}
...
The shadow and Riemann zeta functions are generically
independent: the shadow $L$-function
$L_\cA^{\mathrm{sh}}(s) = -\kappa\,\zeta(s)\,\zeta(s{-}1)$
...
\textbf{Assessment.}
Prime-locality is \emph{not} dead for non-lattice algebras,
but the live version is narrower than the original aspiration.
For rational VOAs, it is proved: the Franc--Mason Hecke theory
provides the eigendecomposition, and the theta-bridge resolves
the non-multiplicativity of the scalar Dirichlet series.
For irrational VOAs, the question reduces to a concrete
problem in the Hecke theory of quasi-modular forms: does the
graph-sum structure of the shadow amplitudes, with vertex
weights from the tau-independent MC recursion and edge weights
from $E_2^*(\tau)$, respect the Hecke decomposition of the
ambient space of quasi-modular forms?  This is a question about
the interaction between operadic graph sums and Hecke operators,
not about the MC equation per se.

Route~C is logically sound at the shadow coefficient level but
has a genuine gap at the amplitude level.  The gap is
\emph{not} a counting argument failure \textup{(}the
overdetermination count is correct\textup{)} but a
\emph{compatibility} question between two algebraic structures
\textup{(}the graph-sum operad and the Hecke algebra\textup{)}
acting on the same space of quasi-modular forms.  Closing this
gap requires either: \textup{(A)}~a vanishing theorem for
