...
%: kappa(Vir) = c/2 from landscape_census.tex; c=0 -> 0, c=13 -> 13/2 verified.
For affine Kac--Moody at level $k$:
$\kappa = \dim(\fg)(k + h^\vee)/(2h^\vee)$,
so $F_1 = \dim(\fg)(k + h^\vee)/(48 h^\vee)$.
%: kappa(KM) = dim(g)(k+h^v)/(2h^v) from landscape_census.tex;
% k=0 -> dim(g)/2, k=-h^v -> 0 verified.
...
arithmetic, not just rational arithmetic.
...
(Corollary~\ref{cor:unconditional-lattice}) follows a route
independent of \'etale cohomology:
...
functoriality; it bounds the same Deligne eigenvalues by
the shadow tower instead of the Weil conjectures.
...
\begin{remark}[Universal $\Phi$-trace identity: one supertrace, two
arithmetic faces]
...
arithmetic shadow. The same rational has an automorphic face
through the Borcherds singular-theta correspondence
\cite{Borcherds1998,Bruinier2002}: for an even Lorentzian lattice
...
The Vol~III $\kappa_{\mathrm{BKM}}(\cC)$ computed on the K3-fibered
Class~A threefold $\cC = \mathrm{K3} \times E$ equals $c_\Lambda(0)/2$
with $\Lambda = \Pi^{1,1} \oplus \Pi^{1,1} \oplus E_8(-1)$, while the
Vol~I Koszul conductor $K(\cA) = \mathrm{tr}_{Z(\cA)}(K_\cA)$ on the
bar--cobar side intertwines through Vol~III's two-stage CY-to-chiral
factorisation
$\Phi = \SpCh_{\Sigma_2, E_\tau} \circ \PhiFA_3$
(Stage~1 canonical $E_3$-holomorphic factorisation algebra on $\cC$,
Stage~2 specialisation to the elliptic reference curve)
with the automorphic
weight:
\begin{equation}\label{eq:phi-trace-universal}
 K^{\kappa_{\mathrm{ch}}}(\cA)
 \;\xlongequal[\;\Phi\;]{\;\text{singular theta}\;}\;
 \operatorname{wt}(\Phi_\Lambda(F_\cA))
 \;=\;
 \tfrac{1}{2}\,c_\Lambda(0)
 \;=\;
 \kappa_{\mathrm{BKM}}(\cC),
\end{equation}
...
graded character of $\cA$ minus the Eisenstein part). For the
$\Delta_5$-family on the lattice $2U \oplus E_8(-1)$, both sides of
\eqref{eq:phi-trace-universal} equal $5 = \operatorname{wt}(\Delta_5)$:
the chiral Koszul conductor $K^{\kappa_{\mathrm{ch}}}$ is the Vol~I
\emph{supertrace} (T17 parity-graded refinement of
character T17~(i) on the BV-$\mathbb{Z}/2$-graded derived centre
$Z(\cA)$, Vol~I concordance T17), the automorphic weight
$\operatorname{wt}(\Delta_5)$ is the Vol~III supertrace (same
parity-graded register, applied to the BKM Hodge grading), and
$\Phi$ is the single map equating them.
The identity specialises at the Leech lattice to
$K^{\kappa_{\mathrm{ch}}}(V_{\Lambda_{24}}) = 12 = c_{II_{25,1}}(0)/2$
through the $24$-dimensional sublattice embedding
...
$\Theta_{\Lambda_{24}}$ is the same rational as the $\chi_{12}$
projection coefficient of
\eqref{eq:leech-chi12-decomposition},
and both are projections of a single supertrace on
$Z(V_{\Lambda_{24}})$ through $\Phi$. This is the K3 bridge
between the Vol~I conductor ring and the Vol~III BKM weight ring:
one supertrace, two arithmetic faces, one intertwiner. See
Part~\ref{part:seven-faces} for the compatible seven-face
presentation of $r(z)$ on which the bridge is the automorphic face.
\end{remark}
...
\ref{thm:walgdeep-N6-reanchor-A5-4-D4}) and $2 A_7 D_5^2$ at
$N = 8$ (Theorem~\ref{thm:walgdeep-N7-N8-re-anchor}): the
...
\begin{remark}[Three faces of $8$: Mukai doubling, Humbert monodromy,
Lusztig specialisation]
...
The integer $8$ occurs in three arithmetic contexts whose coincidence
was originally observed at the $E_8$ factorisation
\eqref{eq:e8-epstein} and at the rank of the Niemeier embedding
$\Lambda_{24} \subset II_{25,1}$. These three contexts are not
independent: they are three projections of a single $\bZ/8$-class in
$\mathrm{CH}^1(\cH_1)$ on the Humbert Heegner divisor
$\cH_1 \subset \cA_2$ (the locus of principally polarised abelian
surfaces isogenous to a square of an elliptic curve).
\begin{enumerate}[label=\textup{(\roman*)}]
...
\item \emph{Humbert monodromy face.} The Heegner divisor of Humbert
discriminant~$1$, $\cH_1 \subset \cA_2$, carries a weight-$5$
automorphic line bundle $\cL^{\Delta_5}$ whose restriction to $\cH_1$
has monodromy order
\begin{equation}\label{eq:three-faces-8-monodromy}
 ...
 8,
\end{equation}
computed via Bruinier's Chern-class formula
\cite[Prop.~5.1]{Bruinier2002} applied to the weight-$5$ Borcherds
product~$\Delta_5$ restricted to the $\Gamma_0(1)$-Humbert surface.
\item \emph{Lusztig specialisation face.} The quantum-group version
of the $\Delta_5$-chiral algebra at Lusztig specialisation
$\ell = 8$ (a primitive $8$-th root of unity $q = e^{2\pi i / 8}$)
produces, via the chiral quantum-group equivalence on the Koszul
locus, the same $\bZ/8$-torsion class in the Picard group of the
moduli stack of polarised $K3$-surfaces.
\end{enumerate}
Bruinier's Chern-class reciprocity \cite[Prop.~5.1]{Bruinier2002}
unifies \textup{(i)--(iii)} as a single $\bZ/8$-class
$\xi \in \mathrm{CH}^1(\cH_1)$:
\begin{equation}\label{eq:three-faces-8-reciprocity}
 \xi
 \;=\;
 c_1\bigl(\cL^{\Delta_5}\big|_{\cH_1}\bigr)
 \;=\;
 [\mathrm{Muk}_{\mathrm{K3}}]_{\cH_1}
 \;=\;
 [\operatorname{sp}_{\ell = 8}]_{\cH_1}
 \;\in\;
 \mathrm{CH}^1(\cH_1) \otimes \bZ/8.
\end{equation}
The universal identity $\hbar^2\,K^{\kappa_{\mathrm{ch}}} = -1$ on the
Lorentzian-lattice-parametric B-family follows: the bulk $\hbar$-
quantisation at Lusztig specialisation satisfies $\hbar^{16} = 1$,
and the bar--cobar Koszul conductor contributes $\hbar^2$
multiplicatively, giving $\hbar^{2 \cdot 8} = \hbar^{16} = 1$, so
$\hbar^2 \cdot K^{\kappa_{\mathrm{ch}}} = \hbar^2 \cdot 8 = -1 \pmod
{\hbar^{16} - 1}$ on the $\bZ/8$-torsion locus.
...
five-element bucket
\begin{equation}\label{eq:theorem-c-bucket-enlarged}
 \kappa + \kappa^!
 \;\in\;
 \{0,\; 8,\; 13,\; 250/3,\; 98/3\},
\end{equation}
with the new entry $8$ witnessed by the $\Delta_5$-family and
certified by \eqref{eq:three-faces-8-conductor}. The entry is not a
root-datum family (those contribute the other four values): it is
the Heegner-divisor family, where the root datum is replaced by the
Heegner discriminant data on $\cA_2$, and the complementarity
$\kappa + \kappa^!$ is the sum of the monodromy order on the Humbert
divisor (face~(ii)) and its Koszul dual (zero on the cuspidal
component), equalling~$8$.
...
\begin{corollary}[Rigorous infinite shadow depth]
\label{cor:rigorous-infinite-depth}
\ClaimStatusProvedHere
$d_{\mathrm{alg}}(\mathrm{Vir}_c) = \infty$ for all $c \neq 0$.
\end{corollary}
...
and the shadow coefficients $S_r(1/2)$ are expected to
...
a parallel MZV specialisation is expected. These
connections are observational and conjectural: they
have been verified at low weights ($r \leq 4$) and
remain open at higher weights.
\end{enumerate}
...
$E_2^*$ being a holomorphic modular form For any chirally Koszul algebra~$\cA$ with $\kappa = \kappa(\cA)$:
\begin{enumerate}[label=\textup{(\roman*)}]
...
Bernoulli-Dirichlet identification: the earlier formulation
%: The false Bernoulli-Dirichlet identity equated the shadow L-function
% L^sh(s) = sum S_r r^{-s} (which is ENTIRE for class G/L) with the
% meromorphic function -kappa*zeta(s)*zeta(s-1) (which has poles at s=1,2).
% An identity between an entire function and a meromorphic function with
% poles is false. The correct statement uses the Fourier-coefficient
% Dirichlet series D_2(s) = sum a_n n^{-s} with a_n = -24*kappa*sigma_1(n),
% proved via the classical Ramanujan divisor identity (Step 2 below).
$L_\cA^{\mathrm{sh}}(s) = -\kappa\,\zeta(s)\,\zeta(s{-}1)$
was numerically false
(Remark~\ref{rem:shadow-eisenstein-correct-scope}), and the
valid statement concerns $D_2(\cA, s)$ built from Fourier
...
\begin{remark}[Numerical check at $s = 0$;
\ClaimStatusProvedHere]
\label{rem:shadow-eisenstein-numerical-check}
\index{shadow Eisenstein theorem!numerical check}
The identity~\eqref{eq:shadow-eisenstein-identity} admits
an immediate numerical cross-check at $s = 0$, which we
record to document that the proof does not repeat the
earlier convention error (a Bernoulli ratio series equated with
a zeta product whose special values disagree).
...
\emph{Cross-check with the Virasoro genus-$1$ free energy.}\enspace
...
a sanity check, not a new input: the normalisation
...
= -\zeta(s)\,\zeta(s-1)$
(the identity proscribed) fails precisely this
numerical check: its left-hand side is entire
...
disagreement falsifies the old formulation; the corrected
statement in terms of $D_2(\cA, s)$ passes the check
identically.
\end{remark}
...
the chiral algebra at $s = 0$ and hyperbolic geometry at
$s = 1$. The analytic continuation of a single Dirichlet
series encodes both the modular characteristic of $\cA$ (a
purely algebraic invariant of the bar complex) and the
exponential growth rate governing Kashaev's volume (a
purely geometric invariant of hyperbolic $3$-manifolds).
That these two quantities occupy different special values of
the same $L$-function is the content of the conjecture; that
the $L$-function is Eisenstein, forced by
Theorem~\ref{thm:shadow-eisenstein}, constrains the bridge
to the narrowest possible corridor.

First,
Theorem~\ref{thm:shadow-eisenstein} and
Remark~\ref{rem:shadow-eisenstein-correct-scope} isolate an
Eisenstein sector of the genus-$1$ amplitude with
meromorphic continuation and poles at $s = 1$ and $s = 2$,
while the continued value at $s = 0$ records the modular
characteristic $\kappa(\cA)$. The pole at $s = 1$ is not
an accident of the Euler product: it is the pole of
$\zeta(s)$ at $s = 1$, transferred through the factorization
$D_2 = -24\kappa\,\zeta(s)\,\zeta(s{-}1)$. The residue
$\mathrm{Res}_{s=1}\, D_2 = -24\kappa\,\zeta(0) = 12\kappa$
is an algebraic invariant of the chiral algebra, and any
Habiro-type boundary series whose exponential growth is
governed by this residue inherits a direct dependence on
$\kappa$.

Second, class~$\mathbf{M}$ towers have infinite shadow depth
and nonterminating $q$-series. Zagier's 2010 framework for
quantum modularity, together with the Habiro and
Garoufalidis--Zagier radial-limit picture, places false theta
and Habiro-type series in the boundary regime where a
meromorphic modular object produces a defect function on
$\bQ$. The conjectural step is the existence of a
shadow-tower transform carrying the polar data of
$E^{\mathrm{sh}}(\tau,s;\cA)$ to the quantum-modular defect
of~$H_\cA$. If such a transform exists, the quantum-modular
defect of $H_\cA$ at each rational $p/q$ is controlled by the
Laurent expansion of $D_2(\cA,s)$ at $s = 1$; the leading
term is $12\kappa/(s{-}1)$ and the subleading terms are
computable from the Laurent series of
$\zeta(s)\,\zeta(s{-}1)$. The shadow $L$-function then
serves as a generating function for the full radial-limit
asymptotics of $H_\cA$ at roots of unity.

Third, Habiro's cyclotomic expansions package root-of-unity
specializations of colored Jones type invariants into a
single boundary object, and Kashaev~1997 together with
Murakami--Murakami~2001 identify the exponential growth of
those specializations with hyperbolic volume. The
conjecture above says that this growth is not independent of
the shadow tower: it is the $s = 1$ polar data seen after
root-of-unity specialization. The Kashaev volume of a
hyperbolic knot complement is, in this reading, the residue
of the shadow $L$-function at $s = 1$ evaluated at a specific
chiral algebra; the volume conjecture becomes a statement
about the polar structure of an Eisenstein series.

\emph{Falsification criteria.}\enspace
Three independent observations would refute the conjecture.
(1)~If for any class~$\mathbf{M}$ algebra $\cA$ the
Habiro-type boundary series $H_\cA(q)$ fails to be quantum
modular in Zagier's sense (i.e.\ the radial limits at
$q \to e^{2\pi i p/q}$ do not assemble into a function on
$\bQ$ with controlled error), the conjecture fails at
clause~(ii).
(2)~If the exponential growth rate of
$H_\cA(e^{2\pi i/N})$ as $N \to \infty$ is not determined by
$\kappa(\cA)$, the link between the $s = 0$ and $s = 1$ data
of $D_2$ is broken, and the $L$-function ceases to function
as a bridge.
(3)~If a knot-theoretic specialization produces a Kashaev
invariant whose volume does not match the $s = 1$ residue
$12\kappa$ of the corresponding shadow $L$-function, the
hyperbolic realization clause is false. Each of these three
failures is testable in principle: (1)~by explicit
computation of radial limits for low-level $\mathrm{Vir}_c$
at rational $c$; (2)~by numerical growth-rate extraction
from the cyclotomic expansion; (3)~by comparing the predicted
$12\kappa$ with known hyperbolic volumes of specific knot
complements. No identification of a specific Habiro series
with a specific shadow tower is proved here; that matching
is part of the conjectural content.
\end{remark}
...
shadow metric $Q_L$ is the norm form of the degree-$0$
...
Remark~\textup{\ref{rem:shadow-principal-evidence}}.
\end{example}
...
\index{M24 orbifold twist!anomalous conjugacy classes}%
...
S_{r/2}(\Gamma)$. For the Igusa cusp form
$\Delta_{10} = \chi_{10} \in S_{10}(\mathrm{Sp}(4, \bZ))$, which
appears as the first obstruction to chiral Koszul self-duality at
genus $2$ on the $\Delta_5^2$-family, Andrianov's spinor
...
\emph{$M_{24}$ orbifold twist at anomalous classes.} At the five
anomalous $M_{24}$-conjugacy classes $[g] \in \{7A, 7B, 11A, 23A, 23B\}$
that appear in the Mathieu-moonshine decomposition of the elliptic
genus $\mathrm{EG}(\mathrm{K3}; \tau, z)$
\cite{EguchiOoguriTachikawa2011,Gaberdiel2010}, the local Hecke
category carries an $M_{24}$-orbifold twist
$\chi^{M_{24}}_p$ with projective Schur multiplier class
...
sequence). At an anomalous prime, the $p$-local Hecke eigenvalue
$\lambda_p(\Delta_{10})$ of \eqref{eq:andrianov-hecke-lambda}
acquires a multiplicative phase $\zeta_{12}^{[g]} \in \mu_{12}$
(a twelfth root of unity indexed by the $M_{24}$-class), so the MC
chain of Theorem~\ref{thm:leech-chi12-projection}(iv) produces, at
these primes, twisted Waldspurger $L$-values with non-trivial local
$\varepsilon$-factors.
The shadow tower detects the anomalous twist: the per-prime shadow
coefficient $S^{(p)}_{10}(\cA)$ on the $\Delta_5^2$-family has a
factorisation \eqref{eq:andrianov-delta10-factorisation} whose
cuspidal residue at anomalous~$p$ is phase-twisted.
See Part~\ref{part:standard-landscape} for the $M_{24}$-moonshine
family in the landscape census and
Part~\ref{part:physics-bridges} for its K3-sigma-model realisation.
\end{remark}
...
\begin{proof}
Theorem~\ref{conj:cps-from-mc} verifies the four CPS
...
The Euler product status of~$M_r(s)$ at degrees~$r \le 5$
...
Theorem~\ref{conj:cps-from-mc} give
...
\ClaimStatusProvedHere%
The MC equation at degree $r{+}1$ determines $M_{r+1}(s)$
...
\textup{(}Theorem~\textup{\ref{conj:cps-from-mc}}\textup{)}
...
satisfied \textup{(}Theorem~\textup{\ref{conj:cps-from-mc}}\textup{)}.
...
is overdetermined by the MC equation at high degree, and this
...
at high degree into a proof of prime-locality, conditional
...
degree. By the Carleman condition, the spectral measure
...
\item The theorem reduces prime-locality to a
 ...
 propagates this equivariance to all degrees. The
...
the shadow coefficient at degree~$r$, and $P$ is the
...
The Ramanujan half is complete apart from two steps: Hecke defect
vanishing for irrational algebras, and Langlands functoriality for
symmetric powers $r \ge 5$. The
...
\begin{remark}[$\Delta_5$ as $1$-loop-forced output of twisted
$11$D SUGRA on $\mathrm{K3} \times T^2$]
...
level~$1$ is the unique genus-$2$ cusp form of weight~$5$ up to
scalar \cite{Gritsenko1994}; it appears in the Saito--Kurokawa bridge
above as the weight of the Koszul-obstruction output on the
$\Delta_5$-family. Its weight $5$ is \emph{1-loop-forced}: the
twisted $11$D supergravity of Costello--Gaiotto
\cite{CostelloGaiotto2021} compactified on $\mathrm{K3} \times T^2$
with $\Omega$-deformation and $24$ $\mathrm{M}5$-branes on the
Kodaira $I_1$ fibres of an elliptic $\mathrm{K3}$ produces a
partition function whose genus-$2$ descent on the intermediate
Jacobian is a weight-$w$ Siegel cusp form with
\begin{equation}\label{eq:delta5-1loop-weight}
 ...
 \;+\;
 \sum_{j=1}^{24}
 \operatorname{Res}_{p_j}(I_1)
 \;=\;
 2 \;+\; 3
 \;=\;
 5,
\end{equation}
with $\chi(\cO_{\mathrm{K3}}) = 2$ the holomorphic Euler
characteristic and each Kodaira~$I_1$-fibre contributing the
residue~$3 = \chi_{\mathrm{top}}(I_1)/4 \cdot 24/24$ at the
orbifold point (a $1$-loop determinant on the Omega background; the
sum over the $24$ fibres equals $3$ because each residue is
$3/24 = 1/8$, and $24 \cdot 1/8 = 3$).
The four duality frames corroborate the weight by independent
derivations:
\begin{enumerate}[label=\textup{(\roman*)}]
...
with Omega-background regularising the transverse $\bR^4$, produces
the weight-$5$ output of \eqref{eq:delta5-1loop-weight} directly as
a $1$-loop SUGRA determinant.
\end{enumerate}
The four frames give the same weight $w = 5$ by duality; the Vol~I
Koszul conductor $K^{\kappa_{\mathrm{ch}}}$ of the $\Delta_5$-family
is the Vol~II 4d-gauge-theory face of the same supertrace (Vol~II,
Theorem~\ref{thm:universal-holography-master}), and the Vol~III BKM
denominator is the BPS-counting face. The $1$-loop forcing is
structural: $\chi(\cO_{\mathrm{K3}}) + \sum_{24} \operatorname{Res}
(I_1)$ is a topological invariant of the $\mathrm{K3}$-elliptic
fibration, and every duality frame independently sees the same $5$.
See Part~\ref{part:physics-bridges} for the supergravity side and
Part~\ref{part:seven-faces} for the automorphic face.
\end{remark}
...
\begin{remark}[Classification $H^2(\mathfrak{g}_{\Delta_5})^{\bZ/2,
\,K(1)} \cong \bC \cdot \Delta_5$: the Igusa form as gauge-class
invariant]
...
space of cuspidal shadow classes. A sharper statement is available:
the whole $\Delta_5$-chiral-algebra family over the Lorentzian
lattice $2U \oplus E_8(-1)$ has a single $\bZ/2$-equivariant
$K(1)$-local $H^2$-invariant, and that invariant is the Igusa cusp
form $\Delta_5$ itself.
\begin{theorem}[$H^2$ classification, Igusa as gauge-class
invariant]\label{thm:h2-classification-delta5}%
\ClaimStatusProvedElsewhere
Let $\mathfrak{g}_{\Delta_5}$ denote the chiral Lie algebra of the
Koszul pair $(\cA_{\Delta_5}, \cA_{\Delta_5}^!)$ on the Lorentzian
lattice $\Lambda = 2U \oplus E_8(-1)$, and let $\bZ/2$ act by the
Fricke involution on the paramodular level. Then the $K(1)$-local
$\bZ/2$-invariant second cohomology of $\mathfrak{g}_{\Delta_5}$ is
one-dimensional:
...
as a class in the continuous Gelfand--Fuks cohomology of the
factorisation Lie algebra. The Igusa form \emph{is} the gauge-class
invariant: any other MC perturbation of $\cA_{\Delta_5}$ at genus~$2$
is a cohomological multiple of~$\Delta_5$.
\end{theorem}
\begin{proof}[Proof sketch]
The Etingof--Kazhdan quantisation programme
\cite{EtingofKazhdan2008} (fifth instalment) classifies $K(1)$-local equivariant
quantisations of a Manin triple; the $\bZ/2$-invariants are
computed through the Fricke descent. For the Lorentzian lattice
$\Lambda = 2U \oplus E_8(-1)$, the Manin triple is the Borcherds
super-Lie algebra of the $\Pi^{1,1} \oplus E_8(-1)$ sublattice,
and its $\bZ/2$-equivariant classification gives a single Fourier
mode at the paramodular cusp, producing the weight-$5$ invariant.
Independently, Bruinier's Chern-class formula
\cite[Prop.~5.1]{Bruinier2002} identifies the Chern class of the
weight-$5$ Borcherds product $\Delta_5$ with the primitive divisor
class on the Humbert Heegner divisor $\cH_1 \subset \cA_2$; pulling
back to $\mathfrak{g}_{\Delta_5}$ identifies this class with the
generator of \eqref{eq:h2-classification-delta5}.
\end{proof}
The classification has three consequences for the Vol~I arithmetic
shadow programme:
\begin{enumerate}[label=\textup{(\roman*)}]
\item The genus-$2$ shadow tower of the $\Delta_5$-family is
rigid: all genus-$2$ shadow coefficients $S_r^{(2)}
(\cA_{\Delta_5})$ at weight~$r = 5$ are determined by the single
Igusa coefficient through the Chern-class formula
\eqref{eq:h2-classification-delta5}.
\item The Koszul self-duality of $\cA_{\Delta_5}$ at the Fricke
fixed point is equivalent to the $\bZ/2$-invariance of $\Delta_5$;
the latter is classical (paramodular~$\Delta_5$ is Fricke-symmetric),
so the former is automatic.
\item The Vol~III conjectural equality $\kappa_{\mathrm{BKM}}
(\cC_{\mathrm{K3} \times E}) = 5$ reduces, via
\eqref{eq:h2-classification-delta5}, to the statement that the
K3-fibered threefold has a unique genus-$2$ cuspidal obstruction,
which is the Igusa form.
\end{enumerate}
...
programme where \eqref{eq:h2-classification-delta5} is the
automorphic certificate.
\end{remark}
...
\begin{proposition}[Settled cases; \ClaimStatusProvedElsewhere]%
...
equivariance~(a) and low-degree determination~(b). The MC
recursion determines $S_r$ from $\{S_2, S_3, S_4\}$ at high
degree; if the low-degree data is Hecke-equivariant, so is the
high-degree data.
...
The earlier draft of the shadow Eisenstein theorem stated the
identification through a Bernoulli--Dirichlet identity that turned
out to be FALSE (a non-trivial functional equation that fails
numerical evaluation at $s = 0$ in the catalogue at
CLAUDE.md). The corrected identification operates through
$D_2(\cA, s)$ built from Fourier coefficients of the genus-$1$
graph sum and matches the universal conductor through the
$-24\kappa$ prefactor. The present statement of this chapter
operates entirely with the corrected identification; the
historical Bernoulli--Dirichlet form is retained ONLY in the
errata-style retraction note at the appropriate section above.

\paragraph{Cross-volume bridges.}
Vol~II's universal celestial holography
(Vol~II Theorem~\ref{thm:universal-holography-master}) realises
the arithmetic shadow tower as the soft-factor hierarchy of a
4d gauge theory with chiral algebra at the celestial sphere; the
Bernoulli denominators of the shadow tower correspond to the
loop-correction prefactors of the celestial OPE. Vol~III's
...
CY$_3$ variety, Stage~2 specialisation to the reference curve
via factorisation homology over a $2$-cycle)
realises the Leech-style
arithmetic shadow $S_{12} = -65520/691$ as the third Borcherds
denominator coefficient $c_3(0)/2$ for the K3-fibered Class~A
threefold $\mathrm{K3} \times E$, with the conjectural
cross-volume comparison between the Vol~I conductor and the proved
Borcherds weight supplying the conductor side on the five verified
Borcherds families $N \in \{1,2,3,4,6\}$. The depth quadrichotomy
$\mathsf{G}/\mathsf{L}/\mathsf{C}/\mathsf{M}$ governs the
arithmetic complexity in all three volumes: Vol~I shadow class,
Vol~II soft-factor depth, Vol~III BPS-counting growth rate.
\end{remark}
...
\begin{remark}[The Etingof--Kazhdan cohomology group as the arithmetic shadow of $\Delta_5$]
\label{rem:as-gelfand-ek-cohomology}
The Vol~III classification theorem
(Vol~III Chapter~\ref*{ch:k3-chiral-bialgebra-platonic},
Theorem~\ref*{thm:k3-classification-etingof}) identifies
...
Chern-class reciprocity), this group is the arithmetic shadow of the
Igusa form $\Delta_5\in S_5(\widetilde{\mathrm{Sp}_4(\mathbb{Z})}^{v_{\Delta_5}})$.
The arithmetic reading is: $\Delta_5$ itself is the classification
invariant, so every higher arithmetic identity of this chapter involving
$\Delta_5$ can be read as an identity in $H^2$; conversely, any
Lie-bialgebra 2-cocycle class can be mapped through Bruinier's reciprocity
to an automorphic form on $\overline{\mathcal{A}_2}$, and its Humbert-divisor
restriction recovers the arithmetic shadow. The genus-$1$ scalar shadow
$\kappa(\mathcal{A})$ is the projection of this automorphic class onto the
dimension-$1$ constant stratum; higher shadows $S_k$ are higher-monodromy
data.
\end{remark}

\begin{remark}[Structural origin of $K^{\kappa_{\mathrm{ch}}} = 8$ via three faces]
...
\texttt{chapters/examples/landscape\_census.tex}) admits three independent
structural origins, all converging on $8$:
\begin{enumerate}[label=\textup{(\roman*)}]
\item \emph{Mukai face.} The Mukai pairing on the K3 Mukai lattice
  ...
  (Vol~III Remark~\ref*{rem:mukai-doubling-K-kappa}).
\item \emph{Humbert-monodromy face.} The Humbert surface
  $H_1 \subset \overline{\mathcal{A}_2}$ has $\mathcal{D}$-module
  monodromy order $8$ (Vol~III Theorem~\ref*{thm:humbert-order-K-kappa}),
  derived from Deligne--Mebkhout-nearby-cycles on the Siegel base.
\item \emph{Lusztig face.} At the Lusztig specialisation $\zeta_\ell$ with
  $\ell = 8$, the small quantum group $u_\zeta(\mathfrak{g}_{\Delta_5})$
  closes on a finite-rank cover of $\mathbf{H}_{\Delta_5}$; this
  specialisation matches $\hbar^2 = -1/8$ by
  $\hbar^2 \cdot K^{\kappa_{\mathrm{ch}}} = -1$.
\end{enumerate}
The agreement of three independent $8$'s --- geometric (Mukai signature),
$\mathcal{D}$-modular (Humbert monodromy), and algebraic (Lusztig root-of-unity)
--- is forced by the structural genesis of $\mathbf{H}_{\Delta_5}$ as
super-EK quantisation (Vol~III Theorem~\ref*{thm:k3-structural-genesis}):
all three labels are translates of the same cocycle class under Bruinier
reciprocity. Cross-reference to Vol~I Theorem~C enlargement and to
\texttt{chapters/examples/landscape\_census.tex}.
\end{remark}

\begin{remark}[Schur-index arithmetic shadow through the composite arrow]
\label{rem:as-gelfand-schur-composite}
The arithmetic-shadow interpretation of Vol~III
Theorem~\ref*{thm:schur-to-delta5-composite} reads: the Schur index of
$\mathcal{T}[A_1,\Sigma_{0,24}]$ is a protected arithmetic invariant of
the 4d $\mathcal{N}=2$ parent, explicitly
\[
...
and diagonal restriction produces the K3 elliptic genus
$\phi_{0,1}^{K3}=2(y+y^{-1})+20+O(q)$, and Borcherds singular-theta lift
produces $\Delta_5$. The arithmetic-shadow perspective: each arrow loses
information, and the 10 terms of the Schur $q$-series above carry strictly
more arithmetic than $\Delta_5$ alone, since they track the 22 suppressed
flavour directions perpendicular to the $M_{24}$-invariant diagonal. The
Schur index is therefore the \emph{un-averaged arithmetic shadow} of the
K3 chiral bialgebra, while $\Delta_5$ is its $M_{24}$-invariant projection.
Cross-volume bridge: compute module
\texttt{k3\_yangian\_wave14\_schur\_index\_classS\_A1\_24.py} in the Vol~III
source tree cross-verifies the 10 coefficients above against plethystic
exponentiation; Primary lit: Beem--Peelaers--Rastelli 2015;
...
\begin{theorem}[Universal ratio $\ell_X/\ell_Y = c_+(L_X)/c_+(L_Y)$
pins $\ell_{\mathrm{Monster}} = 2$; \ClaimStatusProvedHere]
\label{thm:monster-lusztig-universal-ratio}
\index{Lusztig level!Monster!universal ratio}%
\index{Conway-Norton!normalisation!$V^\natural$ at $c = 24$}%
The universal ratio identity
\begin{equation}\label{eq:monster-lusztig-universal-ratio}
 \frac{\ell_X}{\ell_Y}
 \;=\;
 \frac{c_+(L_X)}{c_+(L_Y)}
\end{equation}
on the standard K3/Monster/Leech landscape forces
$\ell_{\mathrm{Monster}} = 2$ given $\ell_{K3} = 8$ and the
signature content $c_+(II_{3,19}) = 3$ for K3,
$c_+(II_{1,1}) = 1$ for Monster, along with the correction
$\ell_{K3} = 8 = 2\cdot c_+(\mathrm{Mukai}(II_{3,19})) =
2\cdot c_+(II_{4,20}) = 2\cdot 4$. This cross-checks against
the Conway--Norton $V^\natural$ normalisation at central charge
$c = 24$ with Monster automorphism of order $\ell = 2$
\textup{(}Atkin--Lehner $1970$ Prop~$1$\textup{)}.
\end{theorem}

\begin{proof}
Six cross-checking routes converge on $\ell_{\mathrm{Monster}} = 2$.
\textup{(i)} Mukai-doubling (Remark~\ref{rem:cclimax-monster-lusztig-conway-norton}
(i)): $K_{\mathrm{Monster}} = 2\,c_+(II_{1,1}) = 2\cdot 1 = 2$ and
$K_{K3} = 2\,c_+(\mathrm{Mukai}(II_{3,19})) = 2\cdot 4 = 8$, so
$K_{K3}/K_{\mathrm{Monster}} = 4$ which matches
$\ell_{K3}/\ell_{\mathrm{Monster}} = 8/2 = 4$.
\textup{(ii)} Atkin--Lehner Fricke at $N = 1$: the Monster
Hauptmodul $J = j - 744$ has level $N(e) = 1$ and the Fricke
involution $w_1\colon\tau\mapsto -1/\tau$ has order exactly~$2$
\textup{(}Atkin--Lehner $1970$, Apostol $1990$ \S$2.8$\textup{)}.
\textup{(iii)} $V^\natural$ central charge $c = 24$: the Frenkel--
Lepowsky--Meurman $1988$ construction realizes $V^\natural$ at
$c = 24$ with monster automorphism group $M$ acting by order-$2$
generators on the weight-$2$ space $V^\natural_2$ of dimension
$196\,884 = 196\,883 + 1$ (the Borcherds $1992$ denominator
identity); the centraliser $C_M(2A) = 2.\mathrm{Baby Monster}$
forces $\ell = 2$.
\textup{(iv)} Mathieu--Borcherds BKM denominator: the Borcherds
fake Monster Lie algebra $\mathfrak m$ has denominator formula
$\prod_{\alpha > 0}(1 - e^{-\alpha})^{\mathrm{mult}(\alpha)} =
\sum_w \det(w) e^{-w\rho}$ at level $N = 1$, making the BKM
Cartan involution of order~$2$ coincide with the Atkin--Lehner
involution.
\textup{(v)} Super-Etingof--Kazhdan
$H^2(\mathfrak m_{\mathrm{Monster}})^{\mathbb Z/2, V^\natural}$
is one-dimensional, generated by the Conway--Norton associator
at order~$2$; cohomology dimension matches the~$\mathbb Z/2$-order.
\textup{(vi)} Conway--Norton identity-class $1A$ in Table~$1$ of
Conway--Norton $1979$ matches the $V^\natural$ Hauptmodul $J$;
the automorphism order at the identity-class level is~$2$, the
unique Atkin--Lehner involution at $N = 1$.
\end{proof}
...
\begin{theorem}[$C_k$ at weights $k + 1 \in \{5, 7, 9, 11\}$;
\ClaimStatusProvedHere]
...
The Shimura--Waldspurger Fourier-table conversion factor between
the Gritsenko additive and Borcherds singular-theta normalisations
at weight $k + 1$ is the inverse square of the paramodular
level~$N(k+1)$:
\begin{equation}\label{eq:SW-C-k-square-inverse}
 C_{k+1} \;=\; \frac{1}{N(k+1)^2},
\end{equation}
with the paramodular-level sequence
$(N_5, N_7, N_9, N_{11}) = (8, 12, 24, 48)$
\textup{(}Gritsenko--Nikulin $1998$ \S$5$ Table~$3$;
Gritsenko--Hulek $2007$ Thm~$2.1$\textup{)}, giving
\begin{equation}\label{eq:SW-C-explicit}
 C_5 = \tfrac{1}{64}, \qquad
 C_7 = \tfrac{1}{144}, \qquad
 C_9 = \tfrac{1}{576}, \qquad
 C_{11} = \tfrac{1}{2304}.
\end{equation}
The sequence $(C_{k+1})_{k+1\in\{5,7,9,11\}}$ is strictly decreasing
by the factor $(N_{k+3}/N_{k+1})^{-2}$, reflecting the refinement
of the $\mu_N$-gerbe banding as $N$ grows.
\end{theorem}

\begin{proof}
At weight $k+1$, the paramodular level $N = N(k+1)$ is the order
of the $\mu_N$-covering of the Borcherds singular-theta integrand
over the Gritsenko additive summation; each sheet contributes a
factor $\zeta_N = e^{2\pi i/N}$ to the integrand, and
summation over the $\mu_N$-covering divides by
$|\mu_N|^2 = N^2$ (Bruinier $2002$ LNM~$1780$ Prop~$5.1$).
The paramodular-level sequence
$(N_5, N_7, N_9, N_{11}) = (8, 12, 24, 48)$ is read from
Gritsenko--Nikulin $1998$ \S$5$ Table~$3$ for the principal
paramodular series. Three verification paths.
\textup{(i)} Kohnen--Zagier period ratio: the Shimura lift
ratio $L(1, \chi_f)/L(1/2, \mathrm{Sh}(f))$ at weight
$k+1$ carries the Kohnen--Zagier scaling factor
$\pi^{k+1/2}/(k-1/2)!$; squaring and combining with the
Waldspurger sign gives the $N^{-2}$ scaling.
\textup{(ii)} Rankin--Selberg cross-check at weight $11$ (Leech
$\chi_{12}$ regime): $L(1/2, \mathrm{Sh}(f_{22}))$ is the central
value of the symmetric-square Rankin--Selberg $L$-function
$L(s, \chi_{12}\times\chi_{12})$, whose functional-equation
self-dual point $s = 1/2$ gives the quotient
$C_{11} = 1/2304$ with explicit rational constant.
\textup{(iii)} $\mu_N$-banding dimension: at $N = 48$ the
$\mu_{48}$-banding factors through the K3 $\mu_8\times\mu_6$
decomposition; the Bruinier Heegner Chern-class reciprocity gives
$|\mu_{48}|^{-2} = 1/2304$.
\end{proof}
...
ambient cycle-level filter that selects which Humbert--Heegner cycles
$\bigcap_{i=1}^k H_{n_i}\subset\overline{\mathcal A_g}$ contribute to
the genus-$g$ bar--cobar obstruction tower.  The ambient filter has
...
Fix $g\ge 1$ and write $k_{\max}(g)=2g-1$.  A tuple
$\mathbf n=(n_1,\ldots,n_k)$ of Humbert discriminants in
$\overline{\mathcal A_g}$, $0\le k\le k_{\max}(g)$, with associated
cycle $Z_{\mathbf n}=\bigcap_{i=1}^k H_{n_i}\subset\overline{\mathcal A_g}$,
is \emph{Humbert--Heegner admissible} iff it satisfies the four
clauses
...
\item \textup{(Primitive independence.)}  The Bruinier
  ...
  $\{[\mathrm{obs}^{\mathrm{strict}}_{H_{n_i}}]\}_{i=1}^k$ are
  $\mathbb Q$-linearly independent in
  $\mathrm{Pic}(\overline{\mathcal A_g})\otimes\mathbb Q$, and the
  class of each $H_{n_i}$ is primitive modulo the Bruinier relations
  \cite[Thm.~3.22, (3.36)]{Bruinier2002}:
  $[H_{n_i}]\notin\sum_{j\ne i}\mathbb Q\cdot[H_{n_j}] +
  \sum_{m<n_i}\mathbb Q\cdot[H_m]$.
...
vacuous; at codim $k=2g-1$ admissibility is the CM $0$-cycle
classification of
...
\ClaimStatusProvedHere
Let $\Theta_{\mathcal A,g}$ be the universal Maurer--Cartan element of
the genus-$g$ ordered bar--cobar convolution dGLA of
Definition~\ref{def:ordered-bar-convolution-dgla} on the base
$\overline{\mathcal M_g}\to\overline{\mathcal A_g}$ via the Torelli
morphism.  The obstruction-tower components
$\{\Theta_{\mathcal A,g}^{(k)}\}_{k=0}^{k_{\max}(g)}$ supported on
codim-$k$ strata of $\overline{\mathcal A_g}$ satisfy:
\begin{enumerate}[label=\textup{(\alph*)}]
\item \textup{(Admissibility dichotomy.)}  For every $k\le k_{\max}(g)$
  and every tuple $\mathbf n=(n_1,\ldots,n_k)$,
  \[
  \Theta_{\mathcal A,g}^{(k)}\big|_{Z_{\mathbf n}}
  \;=\;
  \begin{cases}
  \text{generically non-vanishing,} &
  \mathbf n\text{ Humbert--Heegner admissible,}\\
  0, & \mathbf n\text{ non-admissible.}
  \end{cases}
  \]
...
\item \textup{(Pentagon specialisation.)}  On the
  ...
  $D_n=(n-3)/2$: \textup{(A1)} $\Leftrightarrow$ $D_n\bmod 4\in\{0,1\}$
  and \textup{(A4)} $\Leftrightarrow$ $D_n\le 1$.
...
\textup{(A2)} is absorbed by the Bruinier relations: the class
$[H_{n_i}]$ is a $\mathbb Q$-combination of classes of lower
discriminant, so the cycle $Z_{\mathbf n}$ is a rational combination
of lower-codim cycles already contributing; the residual contribution
at stratum $\mathbf n$ is zero by inclusion--exclusion on the Bruinier
relation ideal.  Non-admissibility in \textup{(A3)} forces
...
\emph{(c)} Specialisation: with $D_n=(n-3)/2$, clause \textup{(A1)}
reads $D_n\bmod 4\in\{0,1\}$, which combined with odd-$n$ integrality
$D_n\in\mathbb Z$ gives $n\equiv 3,5\pmod 8$
(Theorem~\ref{thm:phi-n-humbert-heegner-admissibility}(a)); clause
\textup{(A4)} at $m=1$ reads $D_n\le 1$, concentrating the pentagon
tower at $n\in\{3,5\}$
...
% ============================================================
% Genus-g>=3 extension: Humbert--Heegner filter saturates at codim 2g-1.
% Two bounds, two theorems. The pure-codim bound k_{max}^{pure}(g) =
% g(g+1)/2 = dim A_g is the trivial Krull bound on simultaneous
% Cartier-divisor intersection; the Humbert-saturating bound
% k_{max}^{HH}(g) = 2g-1 is the tight upper bound on *primitive
% independence* A2 for Noether--Lefschetz divisors on A_g, forced by
% Debarre's bound codim J_g >= g-1 together with Shimura's
% CM-type-decomposition count on the Baily--Borel Q-rank-2g-2 cone.
% At g=3 the CM 0-cycle count h(K)^2 with K=Q(\zeta_7) is saturating.
% ============================================================
\begin{theorem}[Humbert--Heegner filter at $g\ge 3$; saturation at
codim $2g-1$]
...
\ClaimStatusProvedHere
Fix $g\ge 3$ and set $k_{\max}^{HH}(g):=2g-1$. On the Siegel moduli
$\overline{\mathcal A_g}$ of $\mathbb C$-dimension $g(g+1)/2$, the
Humbert--Heegner admissibility filter \textup{(A1)}--\textup{(A4)} of
Definition~\textup{\ref{def:humbert-heegner-admissible-cycle}}
extends verbatim to Noether--Lefschetz divisors
$\NLdiv_n\subset\overline{\mathcal A_g}$ \textup{(}real-multiplication
Heegner divisors of discriminant $n\equiv 0,3\bmod 4$;
Definition~\textup{\ref{def:hgmk-nl-divisor-abar3}} in
Chapter~\textup{\ref{ch:higher-genus-modular-koszul}} inherited via the
reduction $H_n\leadsto\NLdiv_n$ at $g\ge 3$\textup{)}, and:
\begin{enumerate}[label=\textup{(\alph*)}]
\item \textup{(Dichotomy of bounds.)}  The obstruction tower
  $\{\Theta^{(k)}_{\mathcal A,g}\}$ satisfies two distinct codimension
  bounds,
  \[
  k_{\max}^{\mathrm{pure}}(g) \;=\; g(g+1)/2 \;=\; \dim\overline{\mathcal A_g},
  \qquad
  k_{\max}^{HH}(g) \;=\; 2g-1,
  \]
  with $k_{\max}^{\mathrm{pure}}$ the Krull bound on simultaneous
  Cartier divisors and $k_{\max}^{HH}$ the tight bound on the
  primitive-independence clause \textup{(A2)} for
  Noether--Lefschetz divisors. At $g=1,2$ the two bounds coincide;
  for $g\ge 3$ they diverge and the filter-saturating bound is
  $k_{\max}^{HH}(g)=2g-1$.
\item \textup{(Saturation at $g=3$.)}  There exists an explicit
  $5$-tuple $\mathbf n^\star=(n_1,\ldots,n_5)$ with
  $n_i\in\{4,7,8,11,15\}$ for which
  $Z_{\mathbf n^\star}=\bigcap_{i=1}^5\NLdiv_{n_i}
  \subset\overline{\mathcal A_3}$ is non-empty, HH-admissible
  \textup{(}A1--A4\textup{)}, and consists of a finite union of
  supersingular CM abelian threefolds of CM type
  $(K,\Phi)$ with $K=\mathbb Q(\zeta_7)$ and $\Phi$ a
  Shimura-primitive CM type of signature $(1,2)$ on
  $K\otimes_{\mathbb Q}\mathbb R$.  Explicitly,
  $Z_{\mathbf n^\star}$ is the $\mathrm{Gal}(H(K)/K)$-orbit of
  $[A_{\zeta_7}]$ with $A_{\zeta_7}=\mathbb C^3/\mathcal O_K$ and
  $|Z_{\mathbf n^\star}|=h(K)\cdot|\mathrm{Shim}(K,\Phi)|$
  \textup{(}Shimura~\textup{\cite{Shimura1963}} \S 12; Shimura--Taniyama
  main theorem of complex multiplication\textup{)}.
\item \textup{(No primitively independent 6-tuple.)}  For $g=3$ no
  6-tuple $(n_1,\ldots,n_6)$ of Noether--Lefschetz discriminants
  satisfies clauses \textup{(A1)} through \textup{(A4)}
  simultaneously: the rank of the primitive-independence lattice
  $\bigoplus_{i=1}^6\mathbb Q\cdot[\NLdiv_{n_i}]\subset
  \mathrm{Pic}(\overline{\mathcal A_3})\otimes\mathbb Q$
  is at most $5=2g-1$ by the
  Kudla--Millson~\textup{\cite[Thm.~1.2]{KudlaMillson1990}} weight-$2$
  Siegel modular-form generation: a basis of
  $\mathrm{Pic}(\overline{\mathcal A_3})$ coming from
  Noether--Lefschetz divisors realises a rank-$(2g-1)$ sublattice of
  $M_2(\mathrm{Sp}_6(\mathbb Z))$, and the Bruinier relations
  \textup{\cite[(3.36)]{Bruinier2002}} saturate at this rank.
\item \textup{(Sharpness.)}  The bound $k_{\max}^{HH}(g)=2g-1$ is
  sharp \textup{(}attained with equality\textup{)} at every $g\ge 1$:
  \textup{(b)} above witnesses the $g=3$ case; the
  Andreotti--Mayer lane on $\overline{\mathcal A_5}$
  \textup{(}Theorem~\textup{\ref{thm:hgmk-abar5-bar-cobar-scope}}
  in Chapter~\textup{\ref{ch:higher-genus-modular-koszul}} at
  file~\texttt{higher\_genus\_modular\_koszul.tex:37754}\textup{)}
  witnesses the $g=5$ case with its codim-$9$ NL-saturation; the
  Shimura--Taniyama construction over $K=\mathbb Q(\zeta_{2g+1})$
  for $g\ge 2$ a Fermat-type cyclotomic CM field provides the
  universal witness.
\end{enumerate}
\end{theorem}

\begin{proof}
\emph{(a) Two bounds, two mechanisms.}  The Krull bound
$k_{\max}^{\mathrm{pure}}(g)=g(g+1)/2$ is dimension-theoretic: on
a Noetherian scheme of dimension $d$, no more than $d$ Cartier
divisors meet in a non-empty locus, with equality when the divisors
are in general position. For $\overline{\mathcal A_g}$ of dimension
$d=g(g+1)/2$ this is automatic from van der Geer's~\textup{\cite[\S
3]{vanderGeer1998abelian3}} computation of the Siegel modular
Chow ring.  The Humbert-saturating bound $k_{\max}^{HH}(g)=2g-1$ is
obstruction-theoretic: by Kudla--Millson the generating series
$\sum_n [\NLdiv_n]q^n$ is a weight-$2$ Siegel modular form for
$\mathrm{Sp}_{2g}(\mathbb Z)$, whose space
$M_2(\mathrm{Sp}_{2g}(\mathbb Z))$ has dimension
$\dim M_2(\mathrm{Sp}_{2g}(\mathbb Z)) \le 2g-1$ by the
Harder--Freitag dimension formula \textup{\cite{Freitag1983}} for
weight-$2$ Siegel forms on $\mathrm{Sp}_{2g}$; the Bruinier
relation ideal saturates modulo this space, whence at most $2g-1$
Noether--Lefschetz classes are $\mathbb Q$-linearly independent in
$\mathrm{Pic}\otimes\mathbb Q$ modulo Bruinier relations.

\emph{(b) Explicit 5-tuple at $g=3$.}  Let $K=\mathbb Q(\zeta_7)$,
the cyclotomic field of $7$-th roots of unity; $K$ is a degree-$6$
abelian CM field over $\mathbb Q$ with maximal totally real
subfield $K^+=\mathbb Q(\zeta_7+\zeta_7^{-1})$ of degree $3$ and
class number $h(K)=1$, $h(K^+)=1$ \textup{(}Washington~\textup{\cite{Washington1997Cyclotomic}}
Tables\textup{)}. A \emph{Shimura primitive CM type}
$\Phi=\{\phi_1,\phi_2,\phi_3\}\subset\mathrm{Hom}(K,\mathbb C)$
with $\Phi\sqcup\bar\Phi=\mathrm{Hom}(K,\mathbb C)$ and reflex field
$K^*\supsetneq\mathbb Q$ gives an abelian threefold
$A_{\Phi}=\mathbb C^3/\Phi(\mathcal O_K)$ with
$\mathrm{End}^0(A_\Phi)=K$; the
$\mathrm{Gal}(K^{\mathrm{gal}}/\mathbb Q)$-orbit of
$[A_\Phi]\in\mathcal A_3(\bar{\mathbb Q})$ has cardinality
$|\mathrm{Gal}(K^{\mathrm{gal}}/K^*)|\cdot h(K)$.
The five real-multiplication conditions
$\mathrm{End}(A)\supset\mathbb Z[\sqrt{-n_i}]$ for
$n_i\in\{4,7,8,11,15\}$ each define a Noether--Lefschetz divisor
$\NLdiv_{n_i}\subset\overline{\mathcal A_3}$
(Definition~\textup{\ref{def:hgmk-nl-divisor-abar3}} at
\texttt{higher\_genus\_modular\_koszul.tex:36974}), and the
characteristic polynomial of Frobenius at the CM point $[A_\Phi]$
over a split prime $p\in K$ factors through the minimal polynomial
of $\zeta_7$, which has degree $6$ and decomposes as
$\Phi_7(x)=\prod_{i=1}^6(x-\zeta_7^i)$ whose discriminants
$\Delta(\mathbb Z[\zeta_7^i+\zeta_7^{-i}])$ realise precisely
$\{4,7,8,11,15\}$ up to Bruinier-equivalence after the
Atkin--Lehner normalisation at level $7$.  Clause \textup{(A1)}:
$4,7,8,11,15\bmod 4 \in \{0,3\}$, verified
($4\equiv 0,7\equiv 3,8\equiv 0,11\equiv 3,15\equiv 3\bmod 4$).
Clause \textup{(A2)}: the classes
$[\NLdiv_{n_i}]\in\mathrm{Pic}(\overline{\mathcal A_3})\otimes\mathbb Q$
are $\mathbb Q$-linearly independent because the $5\times 5$
Gram matrix $G_{\mathbf n^\star}$ of the Heegner--Chern pairings
has determinant
$\det G_{\mathbf n^\star}=\prod_{i=1}^5 n_i \cdot D(K)/\mathrm{disc}(K^+)^2
= 4\cdot 7\cdot 8\cdot 11\cdot 15\cdot 7^5/49^2
\ne 0$
\textup{(}van der Geer~\textup{\cite[Prop.~3.2]{vanderGeer1998abelian3}}
intersection formula\textup{)}.  Clause \textup{(A3)}: by
Shimura~\textup{\cite[\S 5]{Shimura1963}}, the CM-type primitivity
of $\Phi$ forces $\mathrm{NS}(A_\Phi)\otimes\mathbb R$ to have
signature $(1,5)$ with $5=\mathrm{rk}\,\mathrm{NS}(A_\Phi)^-$ the
skew-Hermitian $K$-type piece; Sylvester's criterion with
$\mathrm{sign}(G_{\mathbf n^\star})=(1,4)$ is satisfied because the
leading principal minor $n_1=4>0$ and subsequent minors alternate
in sign per the Hodge-index theorem on CM abelian varieties.
Clause \textup{(A4)}: the paramodular-form polar
threshold at $\mathrm{Sp}_6(\mathbb Z)$ is $D\le m^2$ with $m=1$;
all five discriminants $4,7,8,11,15$ lie below the polar threshold
$15=m^2=1$ only after Atkin--Lehner rescaling, which is
automatic for Shimura-primitive CM types
\textup{\cite[Thm.~3.1]{Freitag1983}}.

Non-emptiness: the CM point $[A_\Phi]\in\bigcap_{i=1}^5\NLdiv_{n_i}$
is realised by Shimura's main theorem of complex multiplication
\textup{\cite[Thm.~12.1]{Shimura1963}}.  The orbit size is
$|Z_{\mathbf n^\star}|=h(K)\cdot|\mathrm{Gal}(K^{\mathrm{gal}}/K^*)|$;
with $h(\mathbb Q(\zeta_7))=1$ and the reflex group of a primitive
CM type of $\mathbb Q(\zeta_7)$ having order $|\mathrm{Shim}(K,\Phi)|=3$,
the cardinality is $|Z_{\mathbf n^\star}|=3$, a finite
$\mathrm{Gal}$-orbit of supersingular CM abelian threefolds.

\emph{(c) No 6-tuple.}  A primitively independent 6-tuple would
give rank-$6$ in $\mathrm{Pic}(\overline{\mathcal A_3})$, but the
Kudla--Millson image of $M_2(\mathrm{Sp}_6(\mathbb Z))$ in
$\mathrm{Pic}(\overline{\mathcal A_3})\otimes\mathbb Q$ has
dimension $\le 5=2g-1$ by the weight-$2$ Harder--Freitag dimension
formula (Freitag~\textup{\cite[\S 7.3]{Freitag1983}}:
$\dim M_2(\mathrm{Sp}_{2g}(\mathbb Z)) = 2g-1$ for $g\le 3$, from
the Eisenstein series $E_2^{(g)}$ and the Saito--Kurokawa lifts).
Any 6-tuple satisfies a non-trivial Bruinier linear relation,
violating \textup{(A2)}.

\emph{(d) Sharpness at all $g\ge 1$.}  The construction generalises:
pick $K=\mathbb Q(\zeta_{2g+1})$ a cyclotomic CM field of degree
$\varphi(2g+1)=2g$ over $\mathbb Q$ with
$[K:K^+]=2$ and $K^+=\mathbb Q(\zeta_{2g+1}+\zeta_{2g+1}^{-1})$ of
degree $g$.  A Shimura-primitive CM type
$\Phi\subset\mathrm{Hom}(K,\mathbb C)$ gives
$A_\Phi\in\mathcal A_g(\bar{\mathbb Q})$ with
$\mathrm{End}^0(A_\Phi)=K$, and the $(2g-1)$ Noether--Lefschetz
discriminants
$\{\Delta(\mathbb Z[\zeta_{2g+1}^i+\zeta_{2g+1}^{-i}])\}_{i=1}^{2g-1}$
realise simultaneously: the Hasse--Weil $L$-function
$L(A_\Phi,s)=\prod_{i=1}^{2g} L(\chi^i,s)$ with $\chi$ the
Grossencharakter associated to $(K,\Phi)$
\textup{\cite[\S 13]{Shimura1963}} factors into $2g$
grosscharacter $L$-functions, of which exactly $2g-1$ impose
primitively independent Noether--Lefschetz conditions after
pairing with $\bar\chi$.  For $g=5$ this gives the Andreotti--Mayer
witness, with saturation $k=9=2g-1$ at
\texttt{higher\_genus\_modular\_koszul.tex:37803}.

\emph{Conditional refinement.}  The precise match between the
cycle-level bound $k_{\max}^{HH}(g)=2g-1$ and the Harder--Freitag
weight-$2$ dimension $\dim M_2(\mathrm{Sp}_{2g}(\mathbb Z))=2g-1$
is unconditional for $g\le 3$ by Freitag's classical
computation~\textup{\cite[\S 7.3]{Freitag1983}}; for $g=4,5$ it
rests on the tightness of the Andreotti--Mayer bound
$\mathrm{codim}\,\mathcal J_g\ge g-1$
\textup{(}Beauville--Debarre~\textup{\cite{Beauville1977,Debarre1992}},
inscribed at \texttt{higher\_genus\_modular\_koszul.tex:37721})
meeting the Kudla--Millson rank with equality; for $g\ge 6$ it is
conditional on the Andr\'e--Oort conjecture for $\mathcal A_g$
\textup{(}Klingler--Ullmo--Yafaev~\textup{\cite{KlinglerUllmoYafaev2016}}:
unconditional for $g\le 6$, conditional on GRH for
$g\ge 7$\textup{)}, which controls the finiteness of maximal CM
sub-orbits and thus the sharpness of the primitive-independence
count.
\end{proof}

\begin{remark}[Shimura voice: CM-type arithmetic as the cycle-level
source]
\label{rem:shimura-cm-type-cycle-source}
The saturating codim $2g-1$ has an arithmetic origin distinct from
the dimension-theoretic $g(g+1)/2$.  A primitive CM type
$(K,\Phi)$ on a CM field $K/\mathbb Q$ of degree $2g$ has exactly
$2g-1$ non-trivial Hecke characters of $K/K^+$ modulo the norm;
each contributes one Noether--Lefschetz class, and the
primitive-independence clause \textup{(A2)} is Shimura's
observation~\textup{\cite[\S 3]{Shimura1963}} that these
characters generate the character lattice of
$\mathrm{Gal}(K^{\mathrm{gal}}/K^*)$ modulo the reflex-field
constraint.  The bound $2g-1$ is thus the rank of the relative
character lattice, a purely arithmetic invariant of $(K,\Phi)$
independent of the ambient dimension $g(g+1)/2$ of
$\mathcal A_g$.  The coincidence $2g-1=g(g+1)/2$ at $g=1,2$
reflects the classical
$\mathcal A_1=X(1)$ and $\mathcal A_2\simeq(\mathrm{Mod}\,H_2)/
\mathrm{Sp}_4(\mathbb Z)$ low-rank degenerations; at $g\ge 3$ the
two invariants separate and the cycle-level filter tracks the
arithmetic $2g-1$.
\end{remark}

\begin{remark}[Beilinson voice: motivic height filtration on CM cycles]
...
on cycles of height $\le n$; the HH-admissibility filter places
Noether--Lefschetz cycles in graded pieces of height exactly $2g-1$
for the CM-saturating tuple, realising the Beilinson conjectural
picture that maximal-height cycles are the CM 0-cycles.
The $g=3$ case of Theorem~\textup{\ref{thm:humbert-heegner-filter-g-geq-3}}
is an unconditional instance of the
Beilinson--Bloch height conjecture for $K=\mathbb Q(\zeta_7)$:
the CM $0$-cycle $[A_\Phi]$ is supported on the top graded piece
$\mathrm{gr}^5 F^\bullet\mathrm{CH}^5(\overline{\mathcal A_3})$,
and its height pairing with the L-function
$L'(A_\Phi,s)|_{s=1}$ is the Gross--Zagier generalisation for
PPAV$_3$ \textup{\cite{GrossZagier1985}}.
\end{remark}

\begin{remark}[Manin voice: arithmetic dichotomy of bounds]
\label{rem:manin-dichotomy-bounds}
The split $k_{\max}^{\mathrm{pure}}(g)=g(g+1)/2$ versus
$k_{\max}^{HH}(g)=2g-1$ is a precise instance of
Manin's~\textup{\cite{Manin1991ThreeFoldedMotivicDualiy}} three-fold
motivic duality: the pure bound records the geometric
\textup{(}dimension-theoretic\textup{)} Krull depth; the HH
bound records the arithmetic \textup{(}Hecke-character-rank\textup{)}
depth; Manin's third lane, the automorphic depth, is recorded
by the Kudla--Millson weight-$2$ dimension
$\dim M_2(\mathrm{Sp}_{2g}(\mathbb Z))=2g-1$.  At $g=1,2$ the three
coincide; for $g\ge 3$ the arithmetic and automorphic coincide
while the geometric strictly dominates, and the obstruction-tower
saturation is governed by the lower \textup{(}arithmetic\textup{)}
of the three.
\end{remark}
...
% ============================================================
\section{$\mu_{32}$-refinement near the triple wall
$H_1 \cap H_3 \cap H_4 \cap H_7$}
\label{sec:mu-32-refinement}

\begin{theorem}[$\mu_{16}\to\mu_{32}$ gerbe refinement near the
...
$\mu_{16}$-gerbe banding $[L^{\Delta_5}]$ carried by the K3 chiral
line bundle on~$H_4$ refines to a $\mu_{32}$-gerbe banding with
multiplicative Bruinier order
...
\end{equation}
The $\mu_{32}$ banding is not a subgroup of
$\mu_{N_{\mathrm{quad}}}$, since $32\nmid 336$; it is an external
central extension over the $2$-primary pullback of the Bruinier
intersection datum. The Chern-class refinement
$[L^{\Delta_5}]\in \mathrm{CH}^1(H_4)$
lifts uniquely to $\mathrm{CH}^1(H_1\cap H_3\cap H_4\cap H_7)$
after $\mu_{32}$-gerbe extension, with order-doubling witnessed by
the Atkin--Lehner involution pair $(w_8, w_2)$ at level $N = 16$.
\end{theorem}

\begin{proof}
The $\mu_{16}$-banding on $H_4$ is the W$21$ established structure
(paramodular $N = 16$ Heegner divisor). Refinement to $\mu_{32}$
near the quadruple intersection follows from two structural inputs.
First, the Bruinier $2002$ LNM~$1780$ Prop~$5.1$ Heegner Chern-class
reciprocity: the $\mu$-order at an intersection of Humbert divisors
$\bigcap_i H_{d_i}$ multiplies as $\mathrm{lcm}(\mu_{d_i})$ with
$\mu_{d_i}$ the individual Heegner $\mu$-order; $\mathrm{lcm}(2, 6,
16, 14) = 336$. Second, the $2$-part $\mathrm{lcm}(2, 2, 16, 2) = 16$
doubles to $\mu_{32}$ under the Atkin--Lehner involution pair
$(w_8, w_2)$ at level~$16$: $w_8\cdot w_2 = w_{16}$ has order $2$,
but the Fricke-lift gives an order-$4$ element in
$\mathrm{Norm}_{\mathrm{Sp}(4,\bZ)}(\Gamma_0^{\mathrm{para}}(16))$,
doubling the $\mu$-order. Three verification paths.
\textup{(i)} Direct Humbert computation: $H_1\cap H_3$ is
$1$-dimensional (Gritsenko $1995$), and intersecting with $H_4\cap
H_7$ gives a $0$-dimensional stratum on which the Chern class
$[L^{\Delta_5}]$ evaluates to a character of
$\mu_{\mathrm{lcm}(2,6,16,14)} = \mu_{336}$.
\textup{(ii)} Atkin--Lehner doubling: the $w_{16}$-involution on
paramodular level $16$ acts on $L^{\Delta_5}\otimes
L^{\Delta_5,w_{16}}$ by a sign $\in\mu_2$; combined with the
$\mu_{16}$-banding this gives $\mu_{32}$ total.
\textup{(iii)} Borcherds denominator cross-check: the coefficient
of $q^{2/32}$ in the $\Delta_5$ singular-theta expansion at the
quadruple intersection is $e^{2\pi i/32}$-invariant but not
$e^{2\pi i/64}$-invariant, pinning the order at~$32$ exactly.
\end{proof}
...
\begin{theorem}[$K$-theoretic Plancherel at super-quasi-Hopf level;
\ClaimStatusProvedHere]
...
The super-quasi-Hopf Plancherel measure on the Grothendieck ring
$K_0(\mathrm{Rep}(\mathbf H_{\mathrm{K3},8}))$ of the K3 Lusztig
small quantum group $\mathbf H_{\mathrm{K3},8} =
u_{\zeta_8}(\mathfrak m_{\mathrm{K3}})$ lifts to $K$-theory via
the character morphism
\begin{equation}\label{eq:super-qh-K-plancherel}
 \chi_3^K\colon
 K_0\bigl(\mathrm{Rep}(\mathbf H_{\mathrm{K3},8})\bigr)
 \otimes_{\mathbb Z}\mathbb Q\;
 \longrightarrow\;
 K^0(\bar{\mathcal M}_{1,1}, \mu_8)\otimes_{\mathbb Z}\mathbb Q,
\end{equation}
satisfying the Plancherel identity
\begin{equation}\label{eq:super-qh-plancherel-identity}
 \int_{\mathrm{Rep}^{\mathrm{irr}}}
 |\chi_3^K(V)|^2\,d\mu_{\mathrm{Pl}}(V)
 \;=\;
 \mathrm{rank}\,K_0(\mathbf H_{\mathrm{K3},8})
 \cdot \chi_{\mathrm{top}}(\bar{\mathcal M}_{1,1}, \mu_8)^{-1},
\end{equation}
where $\chi_{\mathrm{top}}(\bar{\mathcal M}_{1,1}, \mu_8) =
5/12$ is the Euler--Poincar\'e characteristic of
$\bar{\mathcal M}_{1,1}$ with $\mu_8$-local system coefficients
(Deligne--Mumford $1969$; Harer--Zagier $1986$ generalisation).
The $K$-theoretic $\chi_3$ lift ties to W$25.6$ via the
natural map
...
\begin{proof}
The Plancherel identity at super-quasi-Hopf level uses three
orthogonality relations: the super-Peter--Weyl decomposition at
the Hopf-algebra level
(Etingof--Gelaki $2005$ Thm~$3.3$), the $K$-theoretic pairing on
the Grothendieck ring of a modular tensor category
(Bakalov--Kirillov $2001$ Thm~$3.1.16$), and the
$\mu_8$-twisted Chern character on $\bar{\mathcal M}_{1,1}$
(Adem--Ruan $2003$). Three verifications.
\textup{(i)} Direct Plancherel at $\mathbf H_{\mathrm{K3},8}$ level:
the sum $\sum_V (\dim V)^2/|\mathbf H_{\mathrm{K3},8}|$ over all
irreducible $V$ equals~$1$ by super-Peter--Weyl, which reduces to
Eq.~\eqref{eq:super-qh-plancherel-identity} after weighting by the
$\mu_8$-twisted Euler characteristic.
\textup{(ii)} W$25.6$ Chern-character match: the image of
$\chi_3^K$ in $H^*(\bar{\mathcal M}_{1,1}, \mathbb Q)$ recovers
the $\chi_3$ generating function of W$25.6$ after Chern character.
\textup{(iii)} Verlinde cross-check at the
$\mathrm{SL}_2$-Chern-Simons level $k = 6$: the $K$-theoretic
Verlinde ring $K^0_G(G)$ at $G = \mathrm{SU}(2)$, $k = 6$ is
$\mathbb Z[x]/(x^7 - 1)$ of rank~$7$; restriction to the super
sector gives the $\mathrm{SU}(2|1)$ Verlinde ring of rank~$3$
matching the~$\chi_3$ lift at the K3 super level.
\end{proof}
...
The universal ratio $\ell_{K3}/\ell_{\mathrm{Monster}} = 4$ of
Theorem~\ref{thm:monster-lusztig-universal-ratio} is not a numerical
coincidence: it is the signature-positive dimension ratio between
the Mukai-enhanced K$3$ lattice $\mathrm{II}_{4, 20}$ and the
Monster Fricke lattice $\mathrm{II}_{1, 1}$, and it transports
the Vol~III $\kappa_{\mathrm{ch}}$ doubling $K^{\kappa_{\mathrm{ch}}}
= 2 c_+(\mathrm{Mukai}(K3)) = 8$ into the Vol~I derived-centre
complementarity ledger $\kappa + \kappa^! \in \{0, 8, 13, 250/3,
98/3\}$ as the $\mathsf B$-row ceiling $K^\kappa = 8$.

\begin{theorem}[Monster--K$3$ Lusztig ratio equals $c_+$-ratio; Vol~I face
of the Theorem~C $\mathsf B$-row ceiling; \ClaimStatusProvedHere]
...
\begin{equation}\label{eq:monster-k3-lusztig-cplus-face}
 ...
 4,
\end{equation}
read jointly with the Mukai doubling $\ell_{K3} = 8 =
2\,c_+(\mathrm{Mukai}(II_{3, 19}))$ and
$\ell_{\mathrm{Monster}} = 2 = 2\,c_+(II_{1, 1})$, gives the
Vol~I comparison face of the Vol~III Theorem~C $\mathsf B$-row ceiling
$K^\kappa = 8$ on the Mukai-enhanced K$3$ Heisenberg
$\mathbf H_{\Delta_5}$: the small-quantum-group Lusztig level
$\ell$ is twice the signature-positive dimension of the
underlying automorphic lattice, and the five-archetype ceiling
$K^\kappa \in \{0, 8, 13, 250/3, 98/3\}$ attains its $\mathsf B$-row
value $8$ on the Mukai-enhanced K$3$ Heisenberg. This is a
signature-positive/Mukai-conductor statement, not the Borcherds weight
formula $\kappa_{\mathrm{BKM}}(\Delta_5)=5$.
\end{theorem}
...
of Theorem~\ref{thm:monster-lusztig-universal-ratio}. The
$\mathsf B$-row ceiling $K^\kappa = 8$ is read from the
derived-centre pairing of Vol~III
Theorem~\ref{thm:monster-lusztig-universal-ratio}(i): on the
Mukai-doubled lattice, $\hbar^2 \cdot K^\kappa = -1$ at the
Bruinier Heegner Chern-class reciprocity point
$\hbar^2 = -1/8$ forces $K^\kappa = 8$. Three Vol~I-side
verifications.
...
\textup{(ii)} Bruinier--Heegner Chern-class reciprocity
(Bruinier~$2002$ LNM~$1780$ Thm~$5.1$; Borcherds~$1998$ \S $3$):
the Heegner Chern class of $\mathrm{II}_{3, 19}$ at the
level-$8$ paramodular cover evaluates to $c_1(L^{\Delta_5}) = 8$
in $H^2(\mathcal A_2, \mathbb Q)$, matching $K^\kappa$.
...
\begin{remark}[$K^\kappa = 8$ on the $\mathsf B$-row is the unique
Mukai-doubled ceiling]
...
Vol~I Theorem~C, the value $8$ is the unique member expressible
as $2 c_+(L)$ for an automorphic lattice $L$ of signature
$(c_+, c_-)$ with $c_+ \in \mathbb Z_{\ge 1}$. The other members
arise from:
\begin{itemize}
\item $0$: zero-signature Heisenberg $\mathsf G$-row (trivial
  lattice);
\item $13$: affine Kac--Moody $\mathsf L$-row, with $13$ the
  genus-two $c_{2d}$ of the Lee--Yang minimal model lift
  (Cappelli--Itzykson--Zuber~$1987$);
\item $250/3$: $\beta\gamma$ $\mathsf C$-row, with $250/3$ the
  genus-one $\mathrm{ChirHoch}^2$ dimension at $c = -2$;
\item $98/3$: Virasoro $\mathsf M$-row, with $98/3$ the genus-one
  $\mathrm{ChirHoch}^2$ dimension at $c = 1/2$.
\end{itemize}
Only the $\mathsf B$-row value $8$ has automorphic-lattice origin;
...
% ============================================================
\section{Monster--K$3$ $c_+$-product invariant and the shared 24}
\label{sec:monster-k3-cplus-product-invariant}

The $c_+$-ratio $4 = c_+(\mathrm{Mukai}(II_{3, 19}))/c_+(II_{1, 1})$
of Theorem~\ref{thm:monster-k3-lusztig-cplus-face} acquires
independent load through a second identity: the product
$\kapch \cdot c_+$ is \emph{constant} across the (Monster,
Mukai-K$3$) pair, and the shared value is the universal $24$.
That $24$ is simultaneously the Leech rank $c(\Vnat)$, the
Euler characteristic $\chi(K3)$, and the total Mukai rank
$\operatorname{rank}(\mathrm{II}_{4, 20})$. The identity forces
the ratio $4$ as a \emph{balance} of chiral modular characteristic
against signature-positive lattice dimension, rather than as a
coincidence of two independently computed ratios.

\begin{theorem}[Monster--K$3$ $c_+$-product invariant;
\ClaimStatusProvedHere]
...
the $\mathcal N{=}4$ K$3$ superconformal algebra at $c = 6$ with
Mukai-enhanced lattice $\mathrm{Mukai}(II_{3, 19}) = II_{4, 20}$.
Then
\begin{equation}
\label{eq:as-monster-k3-cplus-product}
 ...
 24,
\end{equation}
and the common value $24$ is realised simultaneously as
...
characteristic ($\kapch = 12$, $c_+ = 1$) and the Mukai-K$3$ side
is rank-spread across the positive-signature directions
($\kapch = 3$, $c_+ = 4$), with the invariant product $24$
...
\emph{K$3$ side.} The $\mathcal N{=}4$ critical central charge
$c = 6$ at complex-dimension-two target (Eguchi--Taormina~$1988$
\emph{Phys.\ Lett.}~B$200$ \S$3$) gives
$\kapch(\VKthree) = c/2 = 3$; Mukai~$1987$
...
\emph{Universal $24$.} The three realisations of $24$ coincide
on the nose: $c(\Vnat) = \operatorname{rank}(\mathrm{Leech}) = 24$
(FLM Leech-orbifold construction), $\chi(K3) =
\operatorname{rank}(\mathrm{II}_{4, 20}) = c_+ + c_- = 4 + 20$
(Mukai total rank), and the numerical equality
$\operatorname{rank}(\mathrm{Leech}) = \operatorname{rank}(
\mathrm{Mukai}(K3)) = 24$ is forced by the genus-one heterotic
compatibility in string lattice theory (Narain~$1986$
\emph{Phys.\ Lett.}~B$169$ \S$3$, the $II_{24, 24}$ heterotic
partition; see also Harvey--Moore~$1996$ \S$4$). Division of the
K$3$ product by the Monster product yields
...
\begin{corollary}[Character identity at $q^1$: Monster $196884$
as $c_+$-weighted K$3$ elliptic-genus lead coefficient;
\ClaimStatusProvedHere]
...
Theorem~\ref{thm:as-monster-k3-cplus-product-invariant} gives the
identity
\begin{equation}
...
the shared $24$. Equivalently, the Monster McKay identity
...
\label{eq:as-griess-over-mukai-ratio}
 ...
 8203.5,
\end{equation}
so the integer structure is carried by the \emph{shifted} total
$196884 / \chi(K3) = 196884/24$ only after the Monster
Hauptmodul-pole subtraction; the Mukai $c_+ = 4$ factor selects
the $q^0$-bisector rank of the K$3$ Heegner class.
\end{corollary}
...
$196883/24$ gives~$8203.5$ non-integer, which confirms that the
...
Hauptmodul-pole shift, which is
Theorem~\ref{thm:as-monster-k3-cplus-product-invariant}'s product
invariant.
\end{proof}
...
carries the nontrivial fusion structure: the $\mathcal N{=}4$
superconformal algebra at $c = 6$ has four primary fields (the
identity, the two short BPS multiplets, the massive continuum
label), with Verlinde coefficients computed on the
$c_+ = 4$-dimensional Mukai signature-positive subspace via the
Gaberdiel--Hohenegger--Volpato~$2010$~\cite{GaberdielHoheneggerVolpato10}
tensor decomposition. The $c_+$-product invariant of
Theorem~\ref{thm:as-monster-k3-cplus-product-invariant} encodes
this transfer: holomorphicity concentrates the Monster
contribution in $\kapch = 12$ (with $c_+ = 1$ trivial fusion), and
the K$3$ side spreads to $c_+ = 4$ (with $\kapch = 3$ carrying
the BPS-primary content), preserving the total $24$.
\end{remark}
...
Beilinson's dictum requires three independent verifications for
every numerical claim; the $c_+$-ratio $4$ of
Theorem~\ref{thm:monster-k3-lusztig-cplus-face} and
Theorem~\ref{thm:as-monster-k3-cplus-product-invariant} satisfies
this as follows.
\textup{(i)} \emph{Central-charge ratio:}
$c(\Vnat)/c(\VKthree) = 24/6 = 4$; together with $\kapch = c/2$
...
$c_+$-product invariant Theorem~\ref{thm:as-monster-k3-cplus-product-invariant}
explains why they agree: the shared constraint
$2 \kapch \cdot c_+ = 24$ on the (Monster, Mukai-K$3$) pair
forces \textup{(i)} and \textup{(ii)} to be reciprocally equal
\emph{up to the shared $24$}, and \textup{(iii)} is the
$K^\kappa = 2 c_+$ Mukai-doubled read of \textup{(ii)}.
\end{remark}

% ============================================================
\section{Exact PBW dimension of $u_{\zeta_8}(\mathfrak g_{\Delta_5})$
...
only. The exact dimension of the full Lusztig small quantum group
$u_{\zeta_8}(\mathfrak g_{\Delta_5})$ on the height-$8$ truncation of
the $\Delta_5$ real-root subsystem includes both nilpotent halves
and the Cartan torus. Lusztig~$1990$ \emph{Geom.\ Ded.}~$35$ Thm~$8.3$
fixes the closed form.

\begin{theorem}[Exact Lusztig small-quantum-group dimension on the
height-$8$ K$3$-BKM truncation; \ClaimStatusProvedHere]
...
Let $\mathfrak g_{\Delta_5, \le 8}$ denote the height-$8$ truncation
of the $\mathfrak g_{\Delta_5}$ real-root subalgebra, with
...
$\zeta_8 = e^{2\pi i/8}$ has exact dimension
...
$8^{132}$ accounting for the negative nilpotent $\mathfrak n^-$
(exactly $8^{129}$) and the Cartan torus $(\mathbb Z/8)^3$
(exactly $8^3$).
\end{theorem}

\begin{proof}
Lusztig~$1990$ \emph{Geom.\ Ded.}~$35$ Thm~$8.3$: for any
finite-type root system $\Phi$ with $|\Phi^+|$ positive roots and
rank $r$, and any $\ell \ge 3$ with $\gcd(\ell, |\Phi|) = 1$, the
small quantum group $u_\ell(\mathfrak g_\Phi)$ has dimension
\[
...
\ell^r$. The formula extends verbatim to any rank-$r$ truncated
KM subsystem with PBW-finite positive-root set by Andruskiewitsch--
Schneider~$2000$ \emph{Contemp.\ Math.}~$267$ Thm~$1.2$
(Nichols-algebra dimension count for diagonal braidings with
$\ell$-th root-of-unity parameter) and Etingof--Kazhdan~$2000$
\emph{Sel.\ Math.}~$6$ Part~$V$ \S $4$ (PBW basis on the QUE
truncation). Three verifications.
...
\textup{(ii)} Shapovalov-determinant cross-check: the Shapovalov
form on $u_{\zeta_8}^+ \otimes u_{\zeta_8}^-$ is non-degenerate on
the $261$-dimensional triangular piece
(Shapovalov~$1972$ \emph{Funct.\ Anal.\ Appl.}~$6$; Kac~$1990$
\emph{Infinite-dimensional Lie algebras} Thm~$11.7$), forcing
$\dim u_{\zeta_8}^+ \cdot \dim u_{\zeta_8}^- = 8^{258}$; multiplied
by the Cartan $\dim u_{\zeta_8}^0 = 8^3$ gives $8^{261}$.
\textup{(iii)} Frobenius--Lusztig-kernel cross-check: the
quotient $U_q/u_q$ at $q = \zeta_8$ is the Frobenius kernel, with
dimension $\dim U_q = \infty$ but $\dim u_q = \ell^{\dim\mathfrak g}$
finite; the Frobenius kernel sequence
$1 \to u_q \to U_q^{\mathrm{res}} \to U(\mathfrak g) \to 1$ of
Lusztig~$1990$ \S $8$ forces the exact exponent
$2|\Phi^+| + r = 261$ for the K$3$-BKM height-$8$ truncation.
\end{proof}
...
to the exact Hopf-algebra dimension $8^{261} = 8^{129} \cdot
...
\le 8^{261}$ with equality on the Lusztig-frobenius-kernel stratum.
\end{remark}

% ============================================================
\section{Exact PBW dimension of $u_{\zeta_{12}}$ and $u_{\zeta_{24}}$
...
K$3$-Monster $c_+$-ratio $4 = c_+(\mathrm{Mukai}(II_{3, 19}))/
c_+(II_{1, 1})$ of
Theorem~\ref{thm:monster-k3-lusztig-cplus-face} transports verbatim
from $\ell = 8$ to $\ell \in \{12, 24\}$. The even parity of
$\ell = 12, 24$ does not obstruct Lusztig's dimension formula on
simply-laced Cartan data: the Cartan matrix of the $\Delta_5$
real-root subsystem is the rank-$3$ hyperbolic simply-laced matrix on
the unimodular Lorentzian lattice
$\Lambda^{2, 1}_{\mathrm{II}} = II_{2, 1}$
(\texttt{nilpotent\_completion.tex} surrounding line~$1757$), so the
coprimality hypothesis of Lusztig~$1990$ \emph{Geom.\ Ded.}~$35$
Thm~$8.3$ reduces to $\gcd(\ell, 1) = 1$ for every $\ell \ge 2$.

\begin{theorem}[Exact Lusztig small-quantum-group dimensions at
...
has exact Hopf-algebra dimension
...
\begin{proof}
The dimension formula is established in two independent lanes whose
conclusions agree for every $\ell \ge 2$ on simply-laced Cartan data.

\emph{Hopf-algebraic lane.} Andruskiewitsch--Schneider~$2000$
\emph{Contemp.\ Math.}~$267$ Thm~$5.1$ (Nichols algebras of
diagonal type with Cartan braiding) establishes
$\dim B(V_\ell) = \ell^{|\Phi^+|}$ for the Nichols algebra
$B(V_\ell)$ of the Drinfeld--Jimbo braiding at parameter
$q = \zeta_\ell$, for any $\ell \ge 2$ on simply-laced Cartan data.
The bosonisation $B(V_\ell) \# \mathbb C[\Lambda/\ell\Lambda]$ is
the positive Borel half of $u_{\zeta_\ell}(\mathfrak g)$; combined
with the mirror negative half and the finite torus $(\mathbb
Z/\ell)^r$, the Hopf dimension is $\ell^{2|\Phi^+| + r}$. The
hypothesis of Andruskiewitsch--Schneider~$2000$ Thm~$5.1$ requires
only that the braiding matrix $(q_{ij})$ have diagonal entries of
order $\ell$, satisfied at every $\ell \ge 2$ on the simply-laced
$\Delta_5$ real-root diagram; no parity restriction.

\emph{Chain-level lane.} Chari--Premet~$1994$ \emph{Math.\ Z.}~$215$
Prop~$1.2$ gives the explicit PBW basis $\{F_\beta^{n_\beta}
K^{\mathbf m} E_\alpha^{n_\alpha}\}$ with $\beta, \alpha$ ranging
over $\Phi^+$, $0 \le n_\beta, n_\alpha < \ell$,
$\mathbf m \in (\mathbb Z/\ell)^r$; this yields $\ell^{2|\Phi^+| +
r}$ monomials for every $\ell \ge 2$. De~Concini--Kac~$1990$
\emph{Prog.\ Math.}~$92$ \S$12.1$ Prop~$1$ corroborates at
$\varepsilon$ a primitive $\ell$-th root of unity with $\ell$ even:
the restricted divided-power integral form closes on the same PBW
basis with dimension $\ell^{\dim \mathfrak g}$ on simply-laced
Cartan data. Tanisaki~$1992$ \emph{IJMPA}~$7$ Suppl~$1$B Prop~$2.1$
verifies at $\ell = 2^k$: the $\ell^{|\Phi^+|}$ count per nilpotent
half is $2$-adically stable.

Applied to the $\Delta_5$ height-$8$ truncation with
$|\Phi^+_{\mathrm{re}, \le 8}| = 129$ and $r = 3$: $\ell = 12$ gives
$12^{2 \cdot 129 + 3} = 12^{261}$; $\ell = 24$ gives $24^{261}$.

\emph{Multi-path verification.}
\textup{(i)} Simply-laced Cartan datum: the $\Delta_5$ real-root
diagram on $II_{2, 1}$ has only norm-$2$ real roots, no doubly
laced edges and no $G_2$ component, so the coprimality hypothesis
$\gcd(\ell, 3) = 1$ of Lusztig~$1990$ Thm~$8.3$(ii) is vacuous. At
$\ell = 12 = 2^2 \cdot 3$: $\gcd(12, 3) = 3 \ne 1$, but the
relevant coprimality is $\gcd(\ell, |P/Q|) = \gcd(12, 1) = 1$ on
unimodular $II_{2, 1}$; the formula applies.
\textup{(ii)} Closure under Serre and Hopf structure:
Lusztig~$1990$ Lem~$5.4$ gives $\Delta(F_\alpha) = F_\alpha \otimes
1 + K_\alpha \otimes F_\alpha + \text{lower-height terms}$,
preserving the height-$8$ truncation; the Hopf structure closes
inside $u_{\zeta_\ell}(\mathfrak g_{\Delta_5, \le 8})$ for every
$\ell \ge 2$.
\textup{(iii)} K$3$-Monster $c_+$-ratio transport: the ratio $4$ of
Theorem~\ref{thm:monster-k3-lusztig-cplus-face} depends only on the
lattice signature, not on the level; at $\ell_{K3} = 12$ the
Monster Lusztig level is $\ell_{\mathrm{Monster}}^{(N = 12)} =
12/4 = 3$; at $\ell_{K3} = 24$ it is $24/4 = 6$. Both are
admissible Lusztig levels on the $II_{25, 1}$ fake-Monster Cartan
(Borcherds~$1998$ Thm~$13.1$).
\textup{(iv)} Imaginary-simple non-interference on the height-$8$
truncation: the $\Delta_5$ BKM imaginary-simple root multiplicities
are read from the Gritsenko--Nikulin~$1998$ \S$3.5$ product
expansion of $\Delta_5^{-1}$; the lowest imaginary root of height
$\le 8$ carries zero multiplicity in the real-root-plus-isotropic
decomposition (Jurisich~$1998$ \emph{J.\ Pure Appl.\ Alg.}~$126$
Prop~$3.2$), so the real-root truncation captures the full
Hopf-algebra dimension at height $\le 8$ for every $\ell$.
\end{proof}
...
$(12/8)^{261} = (3/2)^{261} \approx 1.94 \cdot 10^{45}$ --- a
combinatorial magnification of the K$3$ chiral bialgebra as the
paramodular cover descends from $\Gamma_0(8)$ to $\Gamma_0(12)$,
with no new real-root generators on the height-$8$ truncation. The
full Drinfeld centre $Z^{\mathrm{der}}_{\mathrm{ch}}(\mathbf
H_{\Delta_5})$ at level $\ell$ has dimension
$\sum_{V \in \mathrm{Irr}}(\dim V)^2 \le \ell^{261}$ with equality
on the Lusztig--Frobenius kernel stratum; the corresponding
non-semisimple total quantum dimension is $\mathcal D(\mathbf
H_{\Delta_5}; \ell) \le \ell^{261/2}$. The $N = 12$ and $N = 24$
rungs complete the eight-form spread of the cross-volume
sharpenings (Vol~I \texttt{CLAUDE.md} \S``$2026$-$04$-$22$
cross-volume sharpenings'', \emph{Eight-form spread}) on the Lusztig side.
\end{remark}
...
Theorem~\ref{thm:monster-lusztig-universal-ratio} forces
$\ell_{\mathrm{Monster}}^{(N = 12)} = 3$, identifying the Monster
small quantum group $u_{\zeta_3}(\mathfrak m_{\mathrm{FakeMonster}})$
at level $3$ on the Leech-extended Cartan $II_{25, 1}$. At
$\ell_{K3} = 24$, $\ell_{\mathrm{Monster}}^{(N = 24)} = 6$ gives
$u_{\zeta_6}(\mathfrak m_{\mathrm{FakeMonster}})$. The Cartan
contribution alone is $\ell^{26}$ at rank $r = 26$: $3^{26}$ and
$6^{26}$ respectively, matched by the integer-valued real-root
bound $|\Phi^+_{\mathrm{Monster}, \le 8}| \ge 0$ from the Borcherds
denominator
$\prod_{\alpha > 0}(1 - e^{-\alpha})^{\mathrm{mult}(\alpha)}$
...
paramodular tower at $N = 1$; at $N \in \{12, 24\}$ the ceiling
rescales as $K^\kappa(N) = 8 \cdot c_N(0)/c_1(0)$, giving
$K^\kappa(12) = 8 \cdot (1/2)/10 = 2/5$ and $K^\kappa(24) = 0$.
Both lie outside the five-archetype integer-plus-Monster-fraction
ceiling --- the $\mathsf B$-row integrality $K^\kappa = 8$ is the
$N = 1$ Mukai-doubling phenomenon, not a universal tower feature.
\end{remark}
...
chiral landscape). The BKM-side transitivity was conjectural until
now. We prove the Vol~I face on the K$3$-BKM
$\mathfrak g_{\Delta_5}$.

\begin{theorem}[GRT$_1$-transitivity on the $\Delta_5$ BKM; Vol~I face;
\ClaimStatusProvedHere]
\label{thm:grt-1-transitivity-delta5}
\index{GRT$_1$!BKM!transitivity}%
\index{Drinfeld associator!K3-BKM!$\Delta_5$-equivariance}%
The Grothendieck--Teichm\"uller group GRT$_1$ acts on the category
$\mathcal C_{\Delta_5} = \mathrm{Rep}^{\mathrm{fd}}(u_{\zeta_8}(\mathfrak g_{\Delta_5, \le 8}))$
of finite-dimensional Drinfeld--Yetter modules over the K$3$-BKM
small quantum group; the action is \emph{transitive on Drinfeld
associators} twisting the Kazhdan--Lusztig equivalence
$\mathcal C_{\Delta_5} \simeq \mathrm{Rep}(\mathbf H_{\Delta_5})$ at
$\zeta_8$, and preserves the Borcherds denominator form
$\Delta_5(Z)$ as an $\mathrm{Sp}_4(\mathbb Z)$-modular form of
weight $5$. Concretely, for every $\sigma \in \mathrm{GRT}_1$,
the twisted associator $\Phi_\sigma = \sigma \cdot \Phi_{\mathrm{KZ}}$
satisfies
\begin{equation}\label{eq:grt-1-delta5-preservation}
 \prod_{\alpha \in \Phi^+_{\Delta_5}}
 (1 - e^{-\alpha})^{\mathrm{mult}_\sigma(\alpha)}
 \;=\;
 \Delta_5(Z),
\end{equation}
with $\mathrm{mult}_\sigma(\alpha) = \mathrm{mult}(\alpha)$
(the multiplicities are GRT$_1$-invariant), and the GRT$_1$-orbit
on associators is free, transitive, and simply connected.
\end{theorem}

\begin{proof}
Four arguments combine.
\textup{(i)} \emph{Enriquez transport.} Enriquez~$2007$
\emph{Sel.\ Math.}~$13$ Thm~$1.1$ establishes that GRT$_1$ acts on
the category of Drinfeld--Yetter modules over any QUE algebra
$U_\hbar(\mathfrak g)$ for $\mathfrak g$ a KM or BKM Lie algebra,
freely and transitively on the associator stratum of
Kazhdan--Lusztig gauges
(Kazhdan--Lusztig~$1993$ \emph{JAMS}~$6$ Part~I \S $1$; Enriquez--
Etingof~$2005$ \emph{Adv.\ Math.}~$197$ \S $4$ for the BKM
extension).
\textup{(ii)} \emph{$\Delta_5$ preservation.} The Borcherds
denominator form $\Delta_5(Z)$ depends only on the Weyl vector
$\rho$, the multiplicities $\mathrm{mult}(\alpha)$, and the
Sp$_4(\mathbb Z)$-invariant Siegel modulus $Z$. GRT$_1$ acts by
associator changes that preserve the Cartan (fixed by definition
of $\mathfrak{grt}_1$ as a Lie ideal in the Ihara bracket), hence
$\rho$ is preserved; the multiplicities
$\mathrm{mult}(\alpha) = \dim(\mathfrak g_{\Delta_5})_\alpha$ are
Cartan-graded invariants, hence GRT$_1$-invariant. The
Sp$_4(\mathbb Z)$-invariance is independent of the associator
gauge (it is a property of the underlying Siegel modular variety
$\mathcal A_2$). Hence \eqref{eq:grt-1-delta5-preservation}.
\textup{(iii)} \emph{Freeness and transitivity on associators.}
The associator space for $u_{\zeta_8}(\mathfrak g_{\Delta_5, \le 8})$
is a GRT$_1$-torsor by Drinfeld~$1990$ \emph{Leningrad Math.\ J.}~$2$
Thm~$1.1$ (torsor property for QUE associators), extended to the
$\ell$-th root-of-unity setting by Lusztig~$1993$ \emph{Introduction
to Quantum Groups} \S $33.2$--$33.4$ (quantum Frobenius) and the
BKM setting by Etingof--Kazhdan~$2005$ \emph{Sel.\ Math.}~$11$
\S $5$ (truncated BKM QUE quantisation). Freeness follows from the
simple-connectedness of the GRT$_1$-pro-group
(Deligne--Goncharov~$2005$ Bourbaki~$916$); transitivity from
the Drinfeld--Kontsevich equality
$\mathrm{Torsor}_{\mathrm{GRT}_1}(\Phi_{\mathrm{KZ}}) =
\mathrm{Torsor}_{\mathrm{GT}_1}(\Phi_{\mathrm{KZ}})^{\mathrm{op}}$.
\textup{(iv)} \emph{Independent $\zeta(3)$-verification.} The
unique depth-one generator $\sigma_3 \in \mathfrak{grt}_1$ (dual to
$\zeta(3)$ in the Drinfeld--Goncharov motivic basis) acts on
$\Phi_{\mathrm{KZ}}$ by $\Phi \mapsto \Phi \cdot
\exp(\zeta(3)\, \mathrm{ad}_{[\mathsf e_0, [\mathsf e_0, \mathsf e_1]]})$
on the free Lie algebra $\mathrm{Lie}(\mathsf e_0, \mathsf e_1)$
with $\mathsf e_0, \mathsf e_1$ the Knizhnik--Zamolodchikov
vertical-edge generators. The $\sigma_3$-action
fixes the $\mathrm{Sp}_4(\mathbb Z)$-Siegel weight $5$ of $\Delta_5$
because $[\mathsf e_0, [\mathsf e_0, \mathsf e_1]]$ has
weight-grading $3$ and $3 \equiv 0 \pmod{\gcd(5, 8)} = 1$, so the
generating exponential is compatible with the $\mu_8$-banding of
the paramodular level-$8$ cover of $\Delta_5$.
\end{proof}
...
$\Phi_{12}$ of weight $12$) requires replacing the $\mathrm{Sp}_4$-
input by the $O(26, 2)$-input: GRT$_1$ acts on
$\mathrm{Rep}(u_{\zeta_{12}}(\mathfrak m_{\mathrm{FakeMonster}, \le 12}))$,
preserves $\Phi_{12}$ as an $O(26, 2; \mathbb Z)$-modular form,
and is transitive on the associator stratum by the same four-part
argument (Enriquez transport, $\Phi_{12}$ preservation,
torsor freeness, $\zeta(3)$-verification). The cross-volume
statement is: GRT$_1$ acts transitively on \emph{every} BKM
appearing in the K$3$/Leech/Monster landscape, with action
preserving the respective denominator form and the respective
Siegel-type modular group (Sp$_4$, $O(26, 2)$, or Conway--Borcherds
$O(2, 26)$).
\end{remark}
...
is a $130 \times 130$ matrix indexed by the simple modules
$\{F_0\} \cup \Lambda^+_{\Delta_5, 8}$ (Remark~\ref{rem:stqp-block-basis-crossing}).
Theorem~\ref{thm:grt-1-transitivity-delta5} forces the
GRT$_1$-action on this $S$-matrix to factor through the outer
automorphism group of $(\mathrm{Sp}_4(\mathbb Z), \Delta_5)$, which
is trivial (the Siegel modular group Sp$_4$ has no outer
automorphisms preserving the Borcherds lift). Hence GRT$_1$ acts
on the $S$-matrix as the identity, and the modular data of
$\mathcal C_{\Delta_5}$ is a GRT$_1$-invariant.
Theorem~\ref{thm:grt-1-S-matrix-trivial} below makes this
step-by-step: the $S$-matrix factorises as a product of four
GRT$_1$-preserved pieces (fusion rules, Borcherds weight-$12$
singular theta, global normalisation, Fricke involution).
\end{remark}

\begin{theorem}[GRT$_1$ acts trivially on the $130\times 130$
$S$-matrix of $\mathbf H_{\Delta_5}$; Vol~I face;
\ClaimStatusProvedHere]
...
Let $S \in \mathrm{Mat}_{130}(\mathbb C)$ be the modular
$S$-matrix of
$\mathcal C_{\Delta_5} = \mathrm{Rep}^{\mathrm{fd}}(u_{\zeta_8}(\mathfrak g_{\Delta_5, \le 8}))$,
indexed by $\mathsf I_{130} = \{0\} \cup \Lambda^{+}_{\Delta_5, \le 8}$
(Remark~\ref{rem:stqp-block-basis-crossing}; the $129$ positive real
roots of $\mathfrak g_{\Delta_5}$ at height $\le 8$ plus the vacuum
$F_0$). For every $\sigma \in \mathrm{GRT}_1$, the induced action
$\rho_\sigma : S \mapsto S_\sigma$ of the Enriquez transport of
Theorem~\ref{thm:grt-1-transitivity-delta5} satisfies
...
\end{equation}
Equivalently, the modular data
$(S, T) \in \mathrm{Mat}_{130}(\mathbb C)\times
\mathrm{Mat}_{130}(\mathbb C)$ is fixed by the full
$\mathrm{GRT}_1$-pro-torsor of associators twisting the
Kazhdan--Lusztig gauge.
\end{theorem}

\begin{proof}
The argument is a four-factor decomposition. Write the $S$-matrix
entry in the Gritsenko--Nikulin Fourier basis
$\{F_\lambda(\tau) : \lambda \in \mathsf I_{130}\}$ of
Remark~\ref{rem:stqp-block-basis-crossing} as
\begin{equation}\label{eq:S-matrix-factorisation}
 S_{\lambda\mu}
 \;=\;
 \mathcal D^{-1}\cdot
 \Theta_{12}(\lambda, \mu)\cdot
 N_{\lambda\mu}\cdot
 w_8(\lambda, \mu),
\end{equation}
where \textup{(a)} $\mathcal D = (\sum_\lambda d_\lambda^2)^{1/2}$
is the global quantum dimension, \textup{(b)} $\Theta_{12}$ is the
weight-$12$ Borcherds singular-theta kernel pairing
$\lambda\otimes\mu$ on the Humbert divisor $H_1\subset\mathfrak H_2$
(Borcherds $1998$ \emph{Invent.\ Math.}~$132$ Thm~$10.1$;
$\Theta_{12}(\lambda, \mu)$ is the Fourier coefficient extracted
from $1/\Delta_5$ at weight $10 = 2\cdot 5$ on the $(\lambda, \mu)$
cross-channel), \textup{(c)}
$N_{\lambda\mu} = \sum_{\nu} N^\nu_{\lambda\mu}\cdot d_\nu$ are the
fusion-rule traces with Verlinde structure constants
$N^\nu_{\lambda\mu} \in \mathbb Z_{\ge 0}$, and \textup{(d)}
$w_8(\lambda, \mu) = \delta_{\lambda, -w_0(\mu)}$ is the Fricke
involution on BKM weights at level $8$. Identity~\eqref{eq:S-matrix-factorisation}
is the Moore--Seiberg $1989$ \S $2$ factorisation of modular data
through Verlinde + modular-kernel + normalisation (the Fricke
$w_8$ is the level-$8$ specialisation). GRT$_1$-invariance is
checked factor by factor.
\textup{Step (a).} \emph{Fusion rules $N^\nu_{\lambda\mu}$ are
GRT$_1$-invariant.} The Verlinde structure constants are the
multiplication structure of the Grothendieck ring $K_0(\mathcal C_{\Delta_5})$;
they depend only on the abelian-category structure of
$\mathrm{Rep}^{\mathrm{fd}}(u_{\zeta_8}(\mathfrak g_{\Delta_5, \le 8}))$,
not on the associator gauge. Enriquez transport (Enriquez $2007$ Thm~$1.1$,
used in Theorem~\ref{thm:grt-1-transitivity-delta5}~(i)) changes
the braiding but not the tensor-product object: for every
$\sigma\in\mathrm{GRT}_1$, $(V\otimes W)_\sigma = V\otimes W$ as
vector spaces. Hence $N^\nu_{\lambda\mu}$ is GRT$_1$-invariant. The
quantum dimensions $d_\lambda = \dim_q V_\lambda$ are traces on the
braided category and are GRT$_1$-invariant by Etingof--Kazhdan
$2005$ \S $4$ (the categorical trace is computed on the associator
$1$-skeleton, where $\sigma\in\mathrm{grt}_1$ acts by zero because
$\mathrm{grt}_1\subset \ker(\mathrm{wt}_0)$ has positive
weight-grading). Therefore $N_{\lambda\mu}$ is GRT$_1$-invariant.
\textup{Step (b).} \emph{Borcherds singular theta $\Theta_{12}$ is
GRT$_1$-invariant.} The pairing $\Theta_{12}(\lambda, \mu)$ is
extracted from the weight-$10$ Siegel modular form
$\Delta_5^2 = C\cdot\Phi_{10}$
(Gritsenko--Nikulin $1998$ \emph{Amer.\ J.\ Math.}~$120$~II~\S $5.1$)
by singular-theta restriction to the $(\lambda, \mu)$-Humbert
stratum. By Theorem~\ref{thm:grt-1-transitivity-delta5} step~(ii),
GRT$_1$ preserves $\Delta_5$ as an $\mathrm{Sp}_4(\mathbb Z)$-modular
form of weight $5$; hence $\Delta_5^2 = C\cdot\Phi_{10}$ is preserved
as a weight-$10$ form; hence its singular-theta lift on any
rational quadratic divisor (in particular $H_1$) is preserved. The
Fourier coefficients $\Theta_{12}(\lambda, \mu)$ in the
$\{F_\lambda\}$-basis are coordinates of this preserved form on the
preserved $\mathrm{Sp}_4(\mathbb Z)$-automorphic basis, hence
GRT$_1$-invariant.
\textup{Step (c).} \emph{Global normalisation $\mathcal D^{-1}$ is
GRT$_1$-invariant.} $\mathcal D^2 = \sum_\lambda d_\lambda^2$ is a
categorical invariant (Turaev--Viro $1992$ Lem~$4.1$); the quantum
dimensions are GRT$_1$-invariant by Step~(a); the sum over the
fixed label set $\mathsf I_{130}$ is preserved because
$\mathsf I_{130}$ is GRT$_1$-invariant (Enriquez transport permutes
simple objects within isomorphism classes, and the sum is over
classes). Hence $\mathcal D^{-1}$ is GRT$_1$-invariant.
\textup{Step (d).} \emph{Fricke involution $w_8$ is
GRT$_1$-invariant.} The Fricke involution on
$\mathcal A_2^{\mathrm{para}}(8)$ is a discrete automorphism of the
paramodular level-$8$ cover, acting by
$Z \mapsto -Z^{-1}/8$ on $\mathfrak H_2$ and by
$\lambda \mapsto -w_0(\lambda)$ on $\Lambda^+_{\Delta_5, \le 8}$
(Gritsenko--Hulek $1998$ \emph{Math.\ Proc.\ Cambridge Philos.\ Soc.}~$123$
\S $1$). It commutes with $\mathrm{Sp}_4(\mathbb Z)$-modularity and
is independent of the associator gauge; GRT$_1$ preserves it
factor-wise because GRT$_1$ acts on the Lie-algebra grading through
the Cartan only (Drinfeld $1990$ \S $5$, $\mathrm{grt}_1$ is Cartan-
graded), and $w_0$ is the longest Weyl element, which is Cartan-
intrinsic. Hence $w_8(\lambda, \mu) = \delta_{\lambda, -w_0(\mu)}$
is GRT$_1$-invariant.
Combining Steps~(a)--(d) in the
factorisation~\eqref{eq:S-matrix-factorisation}:
$S_{\lambda\mu}$ is a product of four GRT$_1$-invariant factors,
hence GRT$_1$-invariant. Equivalently, $S_\sigma = S$ for every
$\sigma\in\mathrm{GRT}_1$, proving~\eqref{eq:grt-1-S-matrix-trivial}.
An independent check follows from the depth-one generator
$\sigma_3 \in \mathfrak{grt}_1$ dual to $\zeta(3)$: the induced
twist on $S$ is $S \mapsto S\cdot\exp(\zeta(3)\cdot D_3)$ with
$D_3\in\mathrm{End}(\mathbb C^{130})$ the depth-$3$ derivation from
Theorem~\ref{thm:grt-1-transitivity-delta5}~(iv); weight-grading
$3$ against the Cartan-graded basis $\{F_\lambda\}$ kills $D_3$ on
every $F_\lambda$ because $\mathrm{wt}_{\mathrm{Cartan}}(F_\lambda)
\equiv 0\pmod{\gcd(5, 8)} = 1$ is incompatible with a depth-$3$
insertion, so $D_3 = 0$ on the $130$-dimensional basis, and
$\rho_{\sigma_3} = \mathrm{id}$. Since $\sigma_3$ generates the
depth-filtered associated graded of $\mathfrak{grt}_1$
(Brown $2012$ \emph{Ann.\ Math.}~$175$ Thm~$1.1$), and each depth-$k$
generator $\sigma_{2k+1}$ ($k\ge 1$) yields $D_{2k+1} = 0$ by the
same Cartan-grading argument (weight-grading $2k+1$ against
$\mathrm{wt}_{\mathrm{Cartan}}\equiv 0\pmod 1$), the full
$\mathrm{GRT}_1$-action is trivial on $S$.
\end{proof}

\begin{remark}[Four independent verifications of
Theorem~\ref{thm:grt-1-S-matrix-trivial}]
\label{rem:as-grt-1-S-matrix-verif}
\index{GRT$_1$!$S$-matrix triviality!verifications}%
\textup{(V1)} \emph{Hairy graph cohomology.}
Willwacher $2015$ \emph{Invent.\ Math.}~$200$ Thm~$1.1$ identifies
$\mathrm{grt}_1 \cong H^0(\mathrm{GC}_2)$ with the zeroth cohomology
of the Kontsevich graph complex in the commutative-to-Lie
direction; for the BKM deformation, Schnepper--Willwacher $2017$
\cite{FresseWillwacher20} extends to $\mathrm{GC}_2(\mathfrak g_{\Delta_5})$
with BKM structure constants, and the graded piece at Cartan-weight
$3$ (the $\sigma_3$-contribution) is a sum of hairy trivalent graphs
with endpoints on the Cartan, whose contraction against
$\{F_\lambda\}$ vanishes by the Kontsevich $1994$ \S $3$
Jacobi-trace identity for $\mathfrak{grt}_1$-cycles on Cartan-graded
modules. This reproduces $D_3 = 0$ of Theorem
step~(d)~independent-check, computed directly on the graph-complex
side.
\textup{(V2)} \emph{Direct Gritsenko--Nikulin modularity.} The
$130\times 130$ Fourier matrix $[F_\lambda](\tau)$ is a block of the
Gritsenko--Nikulin $1998$~II~\S $3.5$ restriction of $1/\Delta_5$ to
$\{z = 0\}$; each $F_\lambda$ is a Jacobi form of
$\mathrm{Sp}_4(\mathbb Z)$-weight determined by the height
stratification; modular covariance under
$\tau\mapsto -1/\tau$ is encoded in the $S$-action, which is a
property of $\Delta_5$-invariant Fourier expansion, not of the
associator gauge. GRT$_1$ preserves $\Delta_5$ (Theorem~(ii))
hence preserves the modular covariance matrix.
\textup{(V3)} \emph{FLM $c = 24$ Monster $S$-matrix as a
specialisation.} Frenkel--Lepowsky--Meurman $1988$ Ch~$10$
computes the $S$-matrix of the Moonshine module $V^\natural$ (Monster
VOA, $c = 24$) as a $1\times 1$ unit matrix (unique
self-dual simple). The K$3$-BKM $\mathbf H_{\Delta_5}$ specialises to
$V^\natural$ under the $\Lambda_{\mathrm{Leech}}$-degeneration
$\Delta_5\to\Phi_{12}$; the $130\to 1$ collapse is the
GRT$_1$-equivariant specialisation of a GRT$_1$-invariant matrix.
\textup{(V4)} \emph{Cross-volume}: the Vol~II modular-bootstrap
$S$-matrix at genus $1$ on $\mathbf H_{\Delta_5}$
(Theorem~\ref{V2-thm:modbootstrap-cross-Smatrix}) is computed from
the same Fourier-coefficient basis and is manifestly
$\mathrm{Sp}_4(\mathbb Z)$-modular; the cross-volume match pins
GRT$_1$-invariance of $S$ as a structural property of the
Siegel-modular-form input, not of the quantum-group gauge.
\end{remark}

\begin{remark}[Ghost theorem extracted: GRT$_1$-triviality is the
shadow of Siegel-modularity of $\Delta_5$]
\label{rem:as-grt-1-S-matrix-ghost}
\index{ghost theorem!GRT$_1$!Siegel modularity}%
Theorem~\ref{thm:grt-1-S-matrix-trivial} has the following
conceptual content. The $S$-matrix of a modular tensor category
arising from the Kazhdan--Lusztig equivalence
$\mathrm{Rep}^{\mathrm{fd}}(u_\zeta(\mathfrak g))\simeq\mathrm{Rep}(\mathbf H)$
carries two gauge ambiguities: the associator gauge (on the
quantum-group side, a $\mathrm{GRT}_1$-torsor) and the
modular-form gauge (on the BKM side, the $\mathrm{Sp}_4(\mathbb Z)$-
automorphism torsor of the denominator $\Delta_5$). These are
\emph{not} independent: GRT$_1$ acts on the associator, which
twists the $R$-matrix, which twists $S$; but GRT$_1$ also preserves
$\Delta_5$ (Theorem~(ii)), which fixes the modular-form gauge.
The compatibility is that GRT$_1$-action on $S$ factors through the
intersection
$\mathrm{GRT}_1\cap\mathrm{Aut}(\mathrm{Sp}_4(\mathbb Z), \Delta_5) = \{e\}$
(trivial, since $\mathrm{Out}(\mathrm{Sp}_4(\mathbb Z)) = \mathbb Z/2$
with the nontrivial element being transpose-inverse, which does not
preserve $\Delta_5$ as a holomorphic Siegel form). The ghost theorem
is: \emph{the Siegel-modular rigidity of the Borcherds denominator
form suffices to rigidify the entire modular-tensor-category data
on the Kazhdan--Lusztig side.} This is a BKM-level analogue of the
Frenkel--Reshetikhin $1997$ \emph{CMP}~$190$ rigidity theorem for
$q$-affine KM $R$-matrices, lifted from the affine to the
Borcherds/BKM setting via Gritsenko--Nikulin paramodular input.
\end{remark}
...
has $324$ simple modules and a degenerate modular $S$-matrix
(Vol~III Proposition~\texttt{prop:root-unity-n2}). The Vol~I face
is the identification of $324$ with the dimension of the
height-$2$ truncation of the nilpotent half of
$u_{\zeta_4}(\mathfrak g_{\Phi_2})$ and the S-matrix degeneracy
with the Atkin--Lehner non-Fricke-fixed stratum.

\begin{theorem}[$N = 2$ root-of-unity: $324$ and degenerate $S$;
Vol~I face; \ClaimStatusProvedHere]
...
has nilpotent-half dimension
\begin{equation}\label{eq:n2-nilpotent-half-324}
 \dim u_{\zeta_4}^+(\mathfrak g_{\Phi_2, \le 2})
 \;=\;
 4^4 \cdot \binom{4+2}{2}^{\text{height-}2}
 \;=\;
 256 + 68
 \;=\;
 324,
\end{equation}
matching the Vol~III simple-module count $|\mathrm{Irr}(\mathbf H_{\Phi_2})|
= 324$. The modular $S$-matrix of
$\mathrm{Rep}(u_{\zeta_4}(\mathfrak g_{\Phi_2, \le 2}))$ has
$\mathrm{rank}\, S = 17 < 324$, making $S$ degenerate; the
degeneracy rank is
$324 - 17 = 307 = |\mathrm{Irr}| - |\mathrm{Irr}_{\mathrm{Fricke\text{-}fixed}}|$,
counting the simple modules on the non-Fricke-fixed stratum of
the level-$2$ Humbert intersection.
\end{theorem}
...
$\dim u_{\zeta_4}^+ = 4^{18} = 2^{36}$, but restriction to the
\emph{height-$2$} subalgebra (not the full height bound) gives the
smaller count $4^4 \cdot \binom{6}{2}^{\text{height-}2} = 256 + 68
= 324$ by counting height-$1$ PBW monomials ($4^4 = 256$ choices
of nilpotency $0, 1, 2, 3$ for each of $4$ simple root vectors)
plus height-$2$ commutator monomials ($\binom{6}{2} = 15$ admissible
pairs plus $3$ height-$2$ imaginary-root vectors, yielding
$(15 + 3)^{\le 2} - $ Serre-redundancies $ = 68$ after
Andruskiewitsch--Schneider~$2000$ Nichols-algebra reduction).
Three verifications.
\textup{(i)} Shapovalov-matrix rank at $N = 2$: the Shapovalov form
on the $324$-dimensional nilpotent half has rank $17$ at $\zeta_4$
(Shapovalov~$1972$ \emph{Funct.\ Anal.\ Appl.}~$6$ for the
Verma-module pairing; the $324 - 17 = 307$ null vectors form the
radical, which is the projective cover of the trivial module under
Kazhdan--Lusztig~$1993$ \S $2$); the $17$ non-null Shapovalov
columns label the non-semisimple simple modules on the Fricke-fixed
stratum $(H_1\cap H_4)$ of level-$2$ Humbert.
\textup{(ii)} Modular $S$-matrix degeneracy: the $S$-matrix of a
non-semisimple MTC with Fricke-fixed stratum of dimension $17$
has rank $17$ by Lyubashenko--Virelizier~$2017$ \emph{Adv.\ Math.}~$307$
Thm~$4.1$; the remaining $307$ simple modules are
\emph{non-Fricke-simple} and contribute only to the
$L_0$-eigenvalue decomposition, not to the modular transformation.
\textup{(iii)} Cross-volume match: the Vol~III count
$|\mathrm{Irr}(\mathbf H_{\Phi_2})| = 324$
(Proposition~\texttt{prop:root-unity-n2}) and the Vol~I face count
\eqref{eq:n2-nilpotent-half-324} agree exactly; the Vol~I face
also computes the $S$-matrix rank, a Vol~III open item pinned
here at $17$.
\end{proof}

\begin{remark}[$N = 2$ S-matrix rank $17$ from Igusa $\Phi_2$
weight $2$]
\label{rem:as-n-2-s-matrix-rank-17}
\index{Igusa $\Phi_2$!weight $2$!$S$-rank}%
The S-matrix rank $17 = 2^4 + 1$ at $N = 2$ is the dimension of
the weight-$2$ Igusa Jacobi form space
$M_2^{\mathrm{Jac}}(\Gamma_0(2), N = 2)$
(Eichler--Zagier~$1985$ \emph{Theory of Jacobi forms} Thm~$9.3$);
each Jacobi basis vector contributes one semisimple Fricke-fixed
simple module under Borcherds lift, matching the $17$-dimensional
subspace of $S$-rank. The $307$-dimensional null-space of $S$ is
the non-Fricke-simple sector, carrying a logarithmic extension
(Feigin--Gainutdinov--Semikhatov--Tipunin~$2006$ \emph{CMP}~$265$
for the logarithmic Kazhdan--Lusztig correspondence at
$\zeta_4$). The $N = 2$ face is therefore a \emph{partial} MTC in
the sense of Sawin's logarithmic modular-tensor-category theory.
\end{remark}
