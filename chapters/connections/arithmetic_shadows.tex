...
This is the scalar lattice face of the general phenomenon. The shadow
...
eigenforms through its graph-sum amplitudes when the theta component
is scalar and Hecke-semisimple. In that regime each degree resolves
one constituent of the constrained Epstein zeta
...
algebraic-depth component $d_{\mathrm{alg}}$, measuring whether the
transferred operations $m_r$ on bar cohomology truncate.
For the universal Virasoro family, $d_{\mathrm{arith}}$ is finite on
each rational model while $d_{\mathrm{alg}}=\infty$ over $\bQ(c)$: the
tower detects the algebra itself via the self-referential OPE
$T \in T_{(1)} T$, not only its representation theory. The depth
decomposition
...
\smallskip
\noindent\emph{Critical and non-critical levels.}\;
...
counts the functional-equation centres of the Hecke constituents in
the constrained Epstein decomposition; it does not locate the zeros
of the resulting linear combination. The algebraic component
$d_{\mathrm{alg}} \ge 0$
is the $A_\infty$ tower-depth component
(Theorem~\ref{thm:ainfty-formality-depth}):
$d_{\mathrm{alg}} = \max\{r : m_r \ne 0\} - 2$ for finite
shadow depth, $d_{\mathrm{alg}} = \infty$ when all
transferred operations are nonzero. For even unimodular scalar
lattice vertex algebras,
$d_{\mathrm{arith}} = 2 + \dim S_{r/2}(\mathrm{SL}_2(\bZ))$ is finite
because the Hecke decomposition terminates. The shadow--spectral
correspondence
(Theorem~\ref{thm:shadow-spectral-correspondence})
identifies each Hecke eigenform $f_j$ with a depth stratum in this
scalar lane.

The lattice-VOA Ramanujan comparison criterion
(Corollary~\ref{cor:conditional-lattice-operadic-rs}) follows the conditional
route
$\text{finite-Hecke-span}+\text{prime-locality}+\mathrm{MC\ moments}
...
\smallskip
\noindent\emph{Scalar and charged sectors.}\;
...
the charged-sector bar complex is a genus-$1$ refinement whose
sector labels retain the root-system data erased by the scalar trace.
...
constrains the spectral coefficients. These are different objects.
On the scalar full-level lattice lane the shadow depth counts
the Hecke constituent slots of the constrained Epstein decomposition.

\begin{theorem}[Shadow--spectral correspondence]
\label{thm:shadow-spectral-correspondence}
\ClaimStatusProvedHere
Let $V_\Lambda$ be an even unimodular lattice vertex algebra of
rank~$r$, and put $k=r/2$. Assume the scalar Hecke-span stability of
the graph amplitudes. Thus
$\Theta_\Lambda\in M_k(\mathrm{SL}_2(\bZ))$ is a scalar modular form.
Write
$\Theta_\Lambda = c_E\, E_{r/2} + \sum_j c_j f_j$ for the
decomposition into the Eisenstein series~$E_{r/2}$ and Hecke
eigenforms $f_j\in S_{r/2}(\mathrm{SL}_2(\bZ))$. Then:
\begin{enumerate}[label=\textup{(\roman*)}]
\item The constrained Epstein zeta decomposes as
...
of~$\pi$.
\item The shadow depth records the multiset of
functional-equation centres of the Hecke constituents appearing in
the decomposition:
\[
 d(V_\Lambda)
 =
 1+\#\{\zeta(s),\zeta(s-k+1)\}
   +\#\{f_j:c_j\ne0\}.
\]
If RH for $\zeta$, the shifted RH for
$\zeta(s-r/2+1)$, GRH for every $L(s,f_j)$ with $c_j\ne0$, and the
needed non-cancellation hypotheses are imposed, the zeros of each
constituent lie on its own functional-equation centre.
\end{enumerate}
\end{theorem}

For non-unimodular or vector-valued lattice theta series
one must replace the displayed Eisenstein term by the corresponding
level and representation packet. For non-lattice algebras the shadow
depth may exceed the number of analytic constituent centres
(\S\ref{sec:non-lattice-theories}).

\begin{remark}[Lattice specificity of the Hecke decomposition]
\label{rem:lattice-specificity}
\index{lattice VOA!Hecke decomposition specificity}
\index{Hecke decomposition!lattice-specific}
For scalar lattice lanes $V_\Lambda$, the partition function
...
\begin{enumerate}[label=\textup{(\roman*)}]
\item in the even-unimodular full-level case the theta function
 $\Theta_\Lambda \in M_{r/2}(\mathrm{SL}_2(\bZ))$ is a scalar
 classical modular form of known weight;
...
\item \emph{Functional equation}:
 ...
 $\zeta(2s)/\zeta(2s{-}1)$).
\item \emph{$L$-function decomposition}:
 Hecke decomposition of~$\Theta$ gives
 $\varepsilon^c_s = \sum c_j\,L(s,f_j)$
 (each eigenform contributes one functional-equation centre).
\item \emph{Shadow--spectral correspondence} (lattice case):
 $d(\cA) = 1 + {}$constituent-centre count
 (the shadow depth counts the Hecke constituents; the scattering
 matrix carries the zero-location problem).
\end{enumerate}
...
\begin{theorem}[Regularized sewing--Selberg formula]
\label{thm:sewing-selberg-formula}
\ClaimStatusProvedHere
After subtraction of the linear $\eta$-anomaly at the cusp and
Rankin--Selberg regularization, the sewing Fredholm determinant
satisfies
\begin{equation}\label{eq:sewing-selberg}
 \operatorname*{Reg}_{\mathrm{RS}}\int_{\cM_{1,1}}
 \log\det(1-K(\tau))\cdot E_s(\tau)\,d\mu(\tau)
 = -2(2\pi)^{-(s-1)}\,\Gamma(s-1)\,\zeta(s-1)\,\zeta(s).
\end{equation}
Without this regularization the formula has no invariant meaning at
the cusp.
\end{theorem}

\begin{proof}
With the stated regularization, Rankin--Selberg unfolding replaces
the integral over the fundamental domain by
$\int_0^\infty a_0(y)\,y^{s-2}\,dy$, where
...
\subsection{The pure-sector Narain identity}

\begin{theorem}[Pure-sector rank-one Narain identity]
\label{thm:narain-universality}
\ClaimStatusProvedHere
For the pure momentum-plus-pure winding diagonal sector of the
rank-$1$ Narain lattice vertex algebra at compactification radius~$R$:
\begin{equation}\label{eq:narain-universality}
 \varepsilon^1_s(R)
 = 2\bigl(R^{2s}+R^{-2s}\bigr)\,\zeta(2s).
\end{equation}
The full Narain lattice contains mixed momentum--winding states; they
are not removed by T-duality and are not counted in
\eqref{eq:narain-universality}.
The character factor $2\cosh(2s\log R)$ is positive for
real~$s$ and $R>0$. Consequently, for this diagonal sector:
\begin{enumerate}[label=\textup{(\roman*)}]
...
\begin{proof}
On the two coordinate axes of the Narain lattice the scalar primaries
have dimensions
...
\end{equation}
The two factors have functional-equation centers
...
\begin{remark}[Zero-location input for the $E_8$ factors]
\ClaimStatusProvedHere
Under RH for $\zeta$, the nontrivial zeros of the two zeta factors in
\eqref{eq:e8-epstein} lie on
$\mathrm{Re}(s)=\tfrac12$ and $\mathrm{Re}(s)=\tfrac72$. The
factorization itself is the proved assertion.
\end{remark}
...
integers. Under RH for $\zeta$ and GRH for
$L(s,\chi_{-4})$, both factors have their nontrivial zeros on
$\mathrm{Re}(s)=\tfrac12$.
\end{proposition}
...
\begin{proposition}[Leech Epstein constituent factorization]
...
of~$\pi$. This is a decomposition into Eisenstein and cuspidal
constituents. It is not a zero theorem for the linear combination
$E_{\mathrm{Leech}}(s)$.
\end{proposition}
...
factorization~\eqref{eq:leech-epstein}. The centers of the functional
equations of the constituents are
$\mathrm{Re}(s)=\tfrac12$ for $\zeta(s)$,
$\mathrm{Re}(s)=\tfrac{23}{2}$ for $\zeta(s{-}11)$, and
$\mathrm{Re}(s)=6$ for $L(s,\Delta_{12})$. A zero of the sum need not
be a zero of any summand.
\end{proof}

\begin{remark}[Functional-equation centres of constituents]
\ClaimStatusProvedHere
Under RH for the two zeta factors and GRH for
$L(s,\Delta_{12})$, the nontrivial zeros of the individual completed
constituents lie on the three lines displayed in
Proposition~\ref{prop:leech-epstein}. No corresponding assertion is
made for the zeros of their Leech linear combination.
\end{remark}

\begin{remark}[Borcherds weight and chiral conductor are distinct
arithmetic faces]
\label{rem:arith-shad-11}%
\index{Phi comparison@$\Phi$ comparison!trace}%
\index{Borcherds singular theta correspondence!Koszul conductor bridge}%
\index{Phi-trace comparison!K3 BKM}%
...
The open mathematical condition for a stronger statement is precise:
...
In every row, the depth equals $1$ plus the constituent-centre count.
...
and scalar genus expansion
$F_g^{\mathrm{sc}}(V_\Lambda) = r \cdot \lambda_g^{\mathrm{FP}}$
at every genus.
...
$\kappa + \kappa^! \in \{0, 13, 250/3, 25/3\}$ of
...
\eqref{eq:three-faces-8-conductor}.  This equation establishes the
scalar conductor entry.  Its Humbert, Lusztig, and Heegner-divisor
realizations begin with the comparison class~\eqref{eq:three-faces-8-reciprocity}
...
\emph{Step~3 ($L$-function decomposition).}
By the Hecke correspondence:
$\sum\sigma_{k-1}(n)n^{-s}=\zeta(s)\zeta(s-k+1)$, and
$\sum a_n(f_j)n^{-s}=L(s,f_j)$.
So $E_\Lambda(s)$ decomposes as in~\eqref{eq:epstein-factorization}.

\emph{Step~4 (Constituent centres).}
The functional equations of the constituents have central lines
$\mathrm{Re}(s)=\tfrac12$ for $\zeta(s)$,
$\mathrm{Re}(s)=k-\tfrac12$ for $\zeta(s-k+1)$, and
$\mathrm{Re}(s)=\tfrac{k}{2}$ for $L(s,f_j)$. RH for $\zeta$ and GRH
for the cusp-form $L$-functions are separate analytic inputs; Deligne's
bound $|a_p(f_j)| \leq 2p^{(k-1)/2}$ does not imply those hypotheses.
The centre attached to a cusp constituent is counted with its
Hecke-eigenform multiplicity; distinct cusp eigenforms of weight~$k$
may have the same vertical centre $\mathrm{Re}(s)=k/2$.
...
$3$ & Eisenstein shift (root structure)
 & $\zeta(s - k + 1)$ \\
$3+j$ ($j\ge1$) & $j$-th nonzero cusp constituent $c_jf_j$
 & $L(s, f_j)$ \\
\bottomrule
\end{tabular}
\end{center}
The obstruction $o_{3+j+1}$ vanishes iff the Hecke projection onto the
$(j{+}1)$-st nonzero cusp constituent is zero. The tower terminates at
degree $d = 3 + \#\{j:c_j\ne0\}$ for $k \ge 4$. If every cusp
eigencomponent occurs, then for even unimodular lattices of rank
$r \ge 8$:
\begin{equation}\label{eq:depth-cusp-formula}
 d(V_\Lambda) = 3 + \dim S_{r/2}.
\end{equation}
The multiset of constituent functional-equation centres has cardinality
$d - 1 = 2 + \#\{j:c_j\ne0\}$:
two from the Eisenstein $\zeta(s)\,\zeta(s-k+1)$, and one
for each nonzero cusp constituent.
\end{proof}
...
\begin{proposition}[Hecke-span stability criterion]
\label{prop:hecke-all-orders}
\label{prop:loop-corrections}
\ClaimStatusProvedHere
\index{Hecke decomposition!loop corrections}
Assume that every loop correction to the lattice shadow amplitude
remains in the finite Hecke span generated by the eigencomponents of
$\Theta_\Lambda$. Then the constituent-centre count
$d_{\mathrm{arith}}$ is stable under loop corrections:
\begin{enumerate}[label=\textup{(\roman*)}]
\item On the Virasoro primary line
 \textup{(}$\dim H^*(\barB) = 1$\textup{)}: every connected graph
 contracts to a scalar multiple of the same one-dimensional trace.
 The chain graph therefore spans the contribution, and the scalar
 formula is exact to the verified order $r = 12$.
\item For lattice VOAs with multi-dimensional bar cohomology: loop
 ...
 $\Theta_\Lambda = \sum c_j f_j$. If these multilinear expressions
 project back to the same finite Hecke span, loops modify the
 coefficients~$c_j$ but not the set of Hecke constituents. Under this
 hypothesis $d_{\mathrm{arith}}$ is unchanged by loop corrections.
\end{enumerate}
\end{proposition}

\begin{proof}
For~(i): on a $1$-dimensional generator space $\bC\cdot\bar{T}$,
all contractions are scalar contractions of the same tensor power.
For~(ii): under the stated hypothesis, the lattice sum
...
The obstruction is precisely that products of eigenforms need not be
eigenforms. The criterion asserts that after contraction and
projection the loop contribution lies again in the original Hecke
span; diagonalization in that finite span then gives the stated
stability.
\end{proof}
...
\begin{proposition}[Period--shadow dictionary under Hecke-span stability]
\label{prop:period-shadow-dictionary}
\ClaimStatusProvedHere
Assume the Hecke-span stability criterion of
Proposition~\textup{\ref{prop:loop-corrections}}. For an even
unimodular lattice VOA $V_\Lambda$ of rank~$r\ge8$,
...
\end{tabular}
\end{center}
Under the same hypothesis the tower terminates at degree $d = 3 + g_k$,
and
...
By Proposition~\ref{prop:loop-corrections}, under Hecke-span
stability loop corrections do not introduce new eigenforms.
\end{proof}
...
\[
 ...
 = 2\,\zeta(2s).
\]
Depth~$2$ (class~$\mathbf{G}$). One constituent centre at
$\mathrm{Re}(s)=\frac14$ for the functional-equation center of
$\zeta(2s)$. The tower terminates at
...
\[
 ...
 = 240\cdot 2^{-s}\,\zeta(s)\,\zeta(s-3).
\]
Depth~$3$ (class~$\mathbf{L}$). The two constituent centers are
$\mathrm{Re}(s)=\frac12$ and $\mathrm{Re}(s)=\frac72$.
...
\begin{equation}\label{eq:leech-three-L}
 ...
 \frac{65520}{691}\,C_\Delta(s)\,L(s,\Delta_{12}).
\end{equation}
Depth~$4$ (class~$\mathbf{C}$). Three constituent centers:
\begin{center}
\renewcommand{\arraystretch}{1.2}
\begin{tabular}{clcl}
\toprule
\textbf{Degree} & \textbf{Source} & \textbf{Center} & \textbf{Period} \\
\midrule
...
$\lvert\tau(p)\rvert \le 2p^{11/2}$ (Deligne's theorem). This
controls Satake parameters; it does not place zeros of
$L(s,\Delta_{12})$ on $\mathrm{Re}(s)=6$; that assertion is GRH for
the modular $L$-function. The tower terminates at degree~$4$ because
...
a Dedekind zeta function. Under RH for $\zeta$ and GRH for
$L(s,\chi_{-4})$, the nontrivial zeros of both factors lie on
$\mathrm{Re}(s) = \frac12$: a single constituent line.
...
$\omega = e^{2\pi i/3}$.
Under the corresponding GRH for the Eisenstein factors, again
depth~$2$ and one constituent centre. These rank-$2$ examples
illustrate the boundary between Gaussian (no root structure)
and Lie (root structure visible at degree~$3$): the weight is too
low for the Eisenstein product to produce a second constituent centre.
\end{computation}

\begin{theorem}[Spectral decomposition under Hecke-span stability]
\label{thm:spectral-decomposition-principle}
\ClaimStatusProvedHere
Assume the Hecke-span stability criterion of
Proposition~\textup{\ref{prop:loop-corrections}}. For a lattice
vertex algebra $V_\Lambda$ with Hecke decomposition
...
The OPE nonlinearity type controls the shadow depth $d(\cA)$ only after
the relevant obstruction classes are known to survive:
\begin{enumerate}[label=\textup{(\roman*)}]
...
\item \textup{(Controlled, Tier~$2$, computed families):}
 ...
 then $d(\cA) < \infty$ on the verified finite packet. For lattice VOAs
 this is conditional on the arithmetic sieve
 \textup{(}$d = 3 + \dim S_{r/2}$,
 ...
 Across these families the depth is \emph{unbounded}: it grows
...
\item \textup{(Self-referential, Tier~$3$):}
 ...
 and the iterated self-loop classes are nonzero in the transferred
 obstruction complex for all orders, then $d(\cA) = \infty$.
\end{enumerate}
\end{proposition}
...
For~(ii), the finiteness of depth is established or conditionally
reduced in the named families. For lattice VOAs, the asserted depth
equals $3 + \dim S_{r/2}$ under the shadow--spectral correspondence
(Theorem~\ref{thm:shadow-spectral-correspondence}), and this number is
...
For~(iii), the direct self-loop $a \in a_{(k)}a$ produces a candidate
term in each iterated obstruction
$o_{r+1} = \{o_r, \Theta^{\leq r}\}_H$. The stated non-cancellation
hypothesis is exactly the assertion that these candidate terms survive
in cohomology for all~$r$.
\end{proof}
...
The conjecture is reduced for lattice VOAs to the Hecke
decomposition, conditionally giving $d = 3 + \dim S_{r/2}$, and it is
proved for the rank-one $\beta\gamma$ system by abelian rigidity. The
general standard-family claim remains a case-by-case assertion. The
...
\begin{corollary}[Primitive conformal vector and infinite shadow depth]
\label{cor:conformal-vector-infinite-depth}
\ClaimStatusProvedHere
If the stress tensor~$T$ of~$\cA$ is a strong generator
\textup{(}not a composite like the Sugawara construction\textup{)}, and
if the iterated Virasoro self-loop classes survive in the transferred
obstruction complex,
then $d(\cA) = \infty$.
\end{corollary}
...
after the stated non-cancellation hypothesis.
\end{proof}

This gives the obstruction test for $\mathcal{W}$-algebras,
$\mathcal{N} = 1, 2, 3, 4$ superconformal algebras, and every VOA whose
conformal vector is a primitive generator. It is a theorem only on the
sub-loci where the Virasoro self-loop obstruction classes have been
computed nonzero. It excludes Heisenberg
...
\ClaimStatusProvedHere
For the universal Virasoro algebra over the rational function field
$\bQ(c)$, the formal shadow depth is infinite:
$d(\mathrm{Vir}_{\bQ(c)})=\infty$.
For a fixed central charge, the same conclusion requires the
specialized obstruction classes to remain nonzero at infinitely many
orders.
\end{corollary}

\begin{proof}
$T$ is the strong generator of the universal Virasoro algebra and
$T_{(1)}T=2T$. The Laurent calculation~\eqref{eq:shadow-leading}
shows that, for every $r\geq4$, the coefficient $S_r$ has a nonzero
leading term in $\bQ((c^{-1}))$. Hence no finite truncation kills the
universal tower over $\bQ(c)$. On the nonsingular
...
\begin{remark}[Depth decomposition]
\ClaimStatusProvedHere
Under the hypotheses of the depth-decomposition package, the Virasoro
self-loop lies in the homotopy-defect summand $d_{\mathrm{alg}}$.
\end{remark}
...
\begin{equation}\label{eq:refined-inequality}
 d(\cA) \ge 1 + (\text{number of constituent centres of
...
$1$~constituent centre, giving a gap of~$2$. The gap originates from
...
(\mathrm{Ising}) = 0$.
The universal Virasoro $T$-line has infinite shadow depth over
$\bQ(c)$; the fixed Ising quotient is a specialization and must be
checked after quotienting null vectors. Thus the arithmetic depth
$d_{\mathrm{arith}} = 0$ is not read from the universal algebraic
tower: the latter at a
...
independent cusp forms, each contributing a constituent centre at
the functional-equation centre $\mathrm{Re}(s)=k/2$.
...
\begin{equation}\label{eq:depth-from-rank}
 d(V_\Lambda)
 = 1 + \#\{\text{distinct constituent centres in the Hecke
 decomposition of }E_\Lambda(s)\}
 \le 1 + 2 + \dim S_{r/2}.
\end{equation}
...
\label{sec:genus-two-arithmetic-frontier}
\label{sec:spectral-continuation}

\begin{theorem}[Depth decomposition]
\label{thm:depth-decomposition}
\ClaimStatusConditional
Assume the refined shadow--spectral comparison, the single-line
dichotomy, and the archetype classification for~$\cA$. Then the
shadow depth of~$\cA$ decomposes as
...
\[
 ...
 \;\ge\; 0
\]
is the \emph{homotopy defect}. The constituent-centre count
equals~$d_{\mathrm{arith}}$ alone: each holomorphic eigenform of
weight~$k$ contributes a centre at $\mathrm{Re}(s)=k/2$,
while the Maass cusp-form projections have functional-equation center
$\mathrm{Re}(s)=\tfrac12$. Zero-location on these centers is a
separate GRH input and is not part of the decomposition.
...
$\mathrm{Vir}_c$ (minimal)
 & finite & conditional $\infty$ & self-loop non-cancellation \\
$\mathrm{Vir}_c$ ($c>1$)
 & $\le\infty$ & $\le\infty$ & \\
\bottomrule
\end{tabular}
...
distinct functional-equation centers. The Maass projections contribute
$L$-functions with center $\mathrm{Re}(s)=\tfrac12$. Hence
...
giving one constituent centre at $\mathrm{Re}(s)=1/2$),
...
\textup{(}finitely many primaries\textup{)}. The universal Virasoro
family has $d_{\mathrm{alg}}=\infty$ over $\bQ(c)$ by
Theorem~\ref{thm:shadow-tower-asymptotics}; a fixed minimal
specialization inherits this only after the specialized obstruction
classes are checked nonzero at infinitely many orders.
...
The decomposition~\eqref{eq:depth-decomposition} is a conditional
extension beyond lattice VOAs. It rests on the following hypotheses:
\begin{enumerate}[label=\textup{(\roman*)}]
...
\item the constituent-centre counting is unambiguous: holomorphic
 eigenforms have functional-equation centre $\mathrm{Re}(s)=k/2$,
 while the completed Maass $L$-functions have centre
 $\mathrm{Re}(s)=1/2$; no zero-location assertion is used here;
...
\begin{remark}[The moonshine module: same $\kappa$, self-loop test]%
...
$\kappa(V_\Lambda) = \operatorname{rank}(\Lambda) = 24$. The assertion
that $V^\natural$ is class~M is the non-cancellation statement for its
weight-$2$ Griess/Virasoro self-loop. The self-referential OPE supplies
candidate obstruction classes at all degrees; Theorem~\ref{thm:shadow-tower-asymptotics}
proves the corresponding all-order statement for the universal
Virasoro one-generator family, not for the full moonshine module.
...
$V^\natural$ is the conditional class~M candidate
...
is replaced by the conditional class~M candidate\textup{)}, and shows
that the scalar shadow tower detects the orbifold transition.
\end{remark}
...
\begin{theorem}[Interacting Gram positivity criterion;
\ClaimStatusProvedHere]%
...
\end{equation}
where the product is finite, or converges absolutely after choosing the
real logarithm on the positive locus. Assume that
$a_\cA(N)\geq 0$ has infinite support and that
$w_\cA(N;\alpha)>0$ on an infinite subset of that support. Then the
finite moment Gram matrices built from the power functions $N^{-s}$ are
positive definite on that subset. The following sufficient conditions
give positivity of the displayed weights:
\begin{enumerate}[label=\textup{(\roman*)}]
...
\item \emph{Positive-atom case}
 ...
 $\alpha > 2\log\lambda_{\max}^+/\log 2$. Positivity on the full
 support also requires the $N=1$ factors
 $1-\lambda_j$ to be positive whenever $a_\cA(1)>0$.
\end{enumerate}
The criterion is a positivity statement for the weighted moment
pairing; it is not a theorem that an arbitrary interacting correction is
reflection-positive.
\end{theorem}
...
for all $\alpha \geq 2$ whenever the Virasoro sewing coefficients
satisfy the diagonal positivity and infinite-support hypotheses.
\end{corollary}
...
\begin{theorem}[Sign defect and resonance are distinct;
\ClaimStatusProvedHere]%
\label{thm:shadow-resonance-locus}%
\index{shadow resonance locus|textbf}%
For the one-atom Virasoro model with
$\lambda_{\mathrm{eff}}=-6/c$, the unitary half-line $c>0$ has no
positive atom and hence no sign defect in the criterion of
Theorem~\ref{thm:interacting-gram-positivity}. For $c<0$ write
$\lambda=|6/c|$. If $a_{\mathrm{Vir}}(1)>0$ and $\lambda>1$, then the
first corrected weight $w(1;\alpha)=a_{\mathrm{Vir}}(1)(1-\lambda)$ is
negative for every~$\alpha$, so the full weighted moment form is not
positive semidefinite.

This sign failure is not the same as determinant resonance. A zero of a
diagonal weight occurs only when
\[
  1-\lambda N^{-\alpha/2}=0
  \qquad\text{for some } N \text{ in the sewing support}.
\]
Thus the determinant-zero locus is contained in the countable union of
curves $\lambda=N^{\alpha/2}$; negative weights away from these curves
give indefiniteness, not a zero determinant. The Lee--Yang value
$c=-22/5$ lies in the sign-defect region, since $|6/c|=15/11>1$; it is
not a determinant-zero point at $N=1$.
\end{theorem}

\begin{proof}
For $c>0$, $\lambda_{\mathrm{eff}}<0$, so all factors
$1-\lambda_{\mathrm{eff}}N^{-\alpha/2}$ are positive. For $c<0$,
$\lambda=|6/c|$. The first weight is
$a_{\mathrm{Vir}}(1)(1-\lambda)$ and is negative when $\lambda>1$.
A determinant zero requires a zero weight or a linear dependence in the
Vandermonde system. On distinct powers $N^{-s_i}$ the Vandermonde
minor is nonzero, so the displayed equation is the diagonal source of
resonance.
\end{proof}
...
positive for non-unitary $c < 0$. In the one-atom Virasoro model,
unitary $c>0$ satisfies the negative-atom sufficient condition once the
underlying sewing coefficients are positive. In the non-unitary regime
there are two separate phenomena: a sign defect when a corrected weight
is negative, and resonance when a corrected weight is zero.
\end{remark}
...
\begin{proposition}[Weighted Riccati degree growth; \ClaimStatusProvedHere]%
\label{prop:gevrey-zero-degree}%
\index{Gevrey class!shadow tower|textbf}%
\index{convergence radius!shadow tower|textbf}%
Assume the weighted Riccati algebraicity theorem for the Virasoro
line. The weighted degree expansion
$\sum_{r \geq 2} \widehat S^{\rm wt}_r(c) t^r$ has sub-factorial
growth:
$|\widehat S^{\rm wt}_r(c)| \sim C(c) \cdot \rho(c)^r \cdot r^{-5/2}$
as $r \to \infty$, away from collision and vanishing of the nearest
branch point. Its convergence radius is
$1/\rho(c) = |t_*|$, where $t_*$ is the nearest zero
of the weighted quadratic~$Q_L(t)$ to the origin. The Darboux exponent
$-5/2 = -3/2$ \textup{(}from the algebraic branch point
of $\sqrt{Q_L}$\textup{)} minus~$1$
\textup{(}from the $1/r$ in
$\widehat S^{\rm wt}_r = a_{r-2}/r$\textup{)}.
At the evaluation point $t=1$ the weighted series converges absolutely for
$c > c^* \approx 6.125$
\textup{(}unique positive root of
$5c^3 + 22c^2 - 180c - 872 = 0$\textup{)} and is outside its
disc of convergence for $c < c^*$. In both regimes, the coefficient
growth is Gevrey-$0$:
$|\widehat S^{\rm wt}_r|/r! \to 0$.
\end{proposition}
...
$\rho = 1/|t_*|$. Division by~$r$ in
$\widehat S^{\rm wt}_r = a_{r-2}/r$
...
\begin{theorem}[Shadow--MZV period dictionary;
\ClaimStatusProvedHere]%
\label{thm:shadow-mzv-dictionary}%
\index{shadow tower!MZV dictionary|textbf}%
\index{multiple zeta values!shadow identification}%
The shadow invariants of a chirally Koszul algebra~$\cA$ determine the
weight filtration in which the genus-$0$ bar-amplitude periods live.
This is a period-weight dictionary, not an injectivity theorem from
shadow coefficients to motivic periods.
\begin{enumerate}[label=\textup{(\roman*)}]
\item \textup{(Weight $2$.)}
...
The $c$-independent identity
$S_3(\mathrm{Vir}_c)=2$
\textup{(}Theorem~\ref{thm:s3-virasoro-c-independent}\textup{)}
is the Virasoro weight-three scalar shadow.  Class~$\mathsf{M}$
membership follows from the quartic/contact obstruction together with
the completed tower data.
...
$\Omega_{\mathrm{tr}} = \sum t^a \otimes t_a$ the trace-form Casimir; the level
...
The identification $r(z) = k\,\Omega_{\mathrm{tr}}/z$ with Casimir
...
properties encode the shadow data. The table below records benchmark
families; it is not a classification of all chirally Koszul algebras.

\begin{theorem}[Benchmark partition-function modular classes;
\ClaimStatusProvedHere]%
...
For the following benchmark families of chirally Koszul algebras,
the genus-$1$ partition function $Z(\cA, \tau)$ belongs
to the following modular class:
\begin{enumerate}[label=\textup{(\roman*)}]
\item \textup{(Heisenberg.)}
For the rank-$1$ Heisenberg oscillator,
$Z(\cH_{\kappa}, \tau) = 1/\eta(\tau)$: weight $-1/2$ with
multiplier system. Not a modular form, but
$\eta(\tau)^{-1}$ has a Hecke-equivariant Fourier
expansion via the partition function $p(n)$.
The oscillator partition function is independent of the Heisenberg
level; the scalar shadow coefficient is $\kappa(\cH_\kappa)=\kappa$.
The displayed row uses the normalization $\kappa=1$. The shadow class
is~$\mathbf{G}$.

\item \textup{(Affine $\widehat{\fsl}_2$ level $1$.)}
In the standard theta-sum normalization,
...
The $\kappa$ values are computed in the scalar convention of
Definition~\ref{def:kappa-scalar-vs-mumford}: the displayed
Heisenberg row is normalized by $\kappa(\cH_1)=1$; the universal
affine $\widehat{\fsl}_2$ level-$1$ vacuum has
\[
\kappa(V_1(\fsl_2))
=\frac{\dim(\fsl_2)(1+h^\vee)}{2h^\vee}
=\frac{3(1+2)}{4}
=\frac94,
\]
whereas the simple Frenkel--Kac--Segal lattice quotient is a separate
lattice row; the lattice VOAs have scalar $\kappa$ equal to the rank
of their oscillator sector, hence $8$ for $E_8$ and $16$ for
$D_{16}^+$; and $V^\natural$ has central charge~$24$ with no
weight-one Heisenberg channel, so the Virasoro scalar lane gives
$\kappa(V^\natural)=24/2=12$.
\end{proof}
...
of $\Delta$ in $\bZ[q]/(q^{114})$.  Three independent checks are:
\begin{enumerate}[label=\textup{(\roman*)}]
...
in $\bZ[q]/(q^{230})$; three independent checks confirm each entry.
...
The weighted Riccati-metric shadow generating function
$H^{\rm wt}(t,c)=\sum_{r\ge2}r\,\widehat S^{\rm wt}_r(c)t^r$
satisfies, conditional on Riccati algebraicity, an algebraic equation
over~$\bQ(c)[t]$. It is therefore a section of a line bundle on a
spectral curve in the weighted normalization.

\begin{theorem}[Algebraic weighted shadow generating function]
\label{thm:spectral-curve}
\ClaimStatusConditional
\index{spectral curve!shadow tower}
\index{shadow tower!spectral curve}
Assume the weighted Riccati algebraicity theorem. For the Virasoro
algebra, the weighted Riccati-metric shadow
...
\begin{proof}
From the conditional weighted theorem
Theorem~\ref{thm:virasoro-shadow-generating-function},
$H^{\rm wt}(t,c)$ is algebraic of degree~$2$ over $\bQ(c)[t]$;
squaring~$H^{\rm wt}$ yields~\eqref{eq:spectral-curve}. The genus
...
\index{Langlands correspondence!shadow Eisenstein|textbf}
...
Thus the open mathematical condition is exact. One must construct a
...
scalar-lane genus-$g$ coefficient
\[
F_g^{\mathrm{sc}}(\cA)
...
\emph{degree}-$r$ shadow coefficient~$S_r$. The two
...
Under weighted Riccati algebraicity, the weighted Virasoro shadow tower carries a
...
\item \emph{Flat $\to$ Higgs:}
 $\Theta_\cA \mapsto \{\widehat S^{\rm wt}_r\}
 \mapsto H^{\rm wt}(t) \mapsto M(t)$.
...
The weighted Riccati theorem identifies the Taylor coefficients of
$H^{\rm wt}(t)=t^2\sqrt{c^2+12ct+\alpha t^2}$ with the spectral data
of~$M(t)$. The separate leading-Laurent recurrence
$S_{r+1}=-3(r{-}1)PS_r/r$ controls the large-$c$ leading term of the
manuscript shadow coefficients; it is not the full square-root Taylor
series. The harmonic metric~$h = P^{-1} = P_0^{-1} = c/2$ converts each
recursion step from the flat ($\Theta$) to the Higgs ($M$) frame. The Koszulness condition
...
For a chirally Koszul algebra~$\cA$ with $N$ strong
...
variety is the weighted shadow generating function's algebraic locus
...
(B)~Under the weighted Riccati algebraicity hypothesis, the recursion operator
...
algebraic by the weighted shadow generating-function theorem
(Theorem~\ref{thm:virasoro-shadow-generating-function} for
Virasoro; the analogous finiteness is a separate hypothesis for other
standard families).
...
irreducible; the self-referential OPE supplies the non-terminating
candidate classes, and termination is excluded only where these classes
survive.
...
\item \emph{Shadow vs Hitchin discriminant.}
 The Virasoro scalar discriminant
 $\Delta_{\mathrm{sc}} = 8\kappa S_4 = 40/(5c{+}22)$
 (Definition~\ref{def:shadow-metric}) differs from the
 Hitchin discriminant
 $\Delta_H = \operatorname{disc}(\det M)
 = -320c^2/(5c{+}22)$
 by a factor $-8c^2$: the Hitchin discriminant is
 $\Delta_H = -8c^2\Delta_{\mathrm{sc}}$. The scalar
 discriminant is a lane witness for the mixed Virasoro wheel after
 the support packet has been fixed; it is not, by itself, the tower
 termination classifier. The Hitchin discriminant controls the
...
tower.  Identifying these formal potentials with spectral ODEs
requires a comparison from the shadow connection to the corresponding
Schr\"odinger operator; that comparison is not constructed here.
\end{remark}
...
\index{Langlands correspondence!shadow deformation hierarchy}
...
\begin{theorem}[Leading plethystic shadow of the Virasoro obstruction tower]
...
\begin{equation}\label{eq:shadow-bps}
 ...
 \frac{(-6/c)^r}{r}\,t^r.
\end{equation}
The plethystic logarithm of $Z_{\mathrm{pleth}}$ is \emph{linear}:
$\operatorname{PLog}(Z_{\mathrm{pleth}}) =
...
The corresponding formal product is
...
The associated spectral-curve period square is
...
\textup{(}where the period expression has a pole\textup{)}.
...
For the formal product:
$\exp(-\log(1+6t/c)) = 1/(1+6t/c)$.
The period square $Z^2 = 80c^2/(45c+218)$
...
\begin{remark}[The primitive plethystic generator is the self-referential OPE]
\label{rem:bps-self-referential}
\index{BPS state!self-referential OPE}
The single primitive generator is the algebraic incarnation
...
a \emph{multi-particle} plethystic spectrum. The shadow depth
$d = 3 + \dim S_k$ counts the number of independent primitive
charges.
\end{remark}

\begin{theorem}[Formal BPS dictionary for the shadow obstruction tower]
\label{thm:general-bps}
\ClaimStatusConditional
\index{BPS spectrum!general construction}
Let $\cA$ be a finitely strongly generated vertex algebra with
invariant bilinear form. The following is a formal dictionary:
when a stability condition and an enumerative moduli problem are
supplied, the shadow tower gives candidate BPS indices; by itself it
defines only the plethystic spectrum
$\Omega_r(\cA) := S_r(\cA)$ whose plethystic logarithm
$\operatorname{PLog}(\sum \Omega_r t^r) = \sum p_r(\cA)\,t^r$
identifies the primitive plethystic classes: $p_r \neq 0$ iff the shadow
...
$H^*(\barB(\cA))$.
These are algebraic inputs, not stability data.
Simple quotients $L_k(\fg)$ at admissible~$k$, logarithmic VOAs,
and coset/orbifold theories therefore carry formal plethystic
shadows only after these inputs are constructed.

Koszulness can upgrade the formal shadow to the nonabelian Hodge
triple only after the comparison hypotheses of
Theorem~\textup{\ref{thm:general-nahc}} are imposed.
Without Koszulness: the flat and Higgs sides persist; the harmonic
metric degenerates on the null-vector kernel.
\end{theorem}

\begin{proof}
Under conditions \textup{(a)}--\textup{(c)}, the plethystic logarithm
is the formal operation
...
\begin{remark}[Formal Donaldson--Thomas/Pandharipande--Thomas dictionary;
\ClaimStatusConditional]
...
The formal plethystic spectrum of Theorem~\ref{thm:shadow-bps} has the
shape of the DT/PT wall-crossing formula only after a Donaldson--Thomas
moduli problem, a Pandharipande--Thomas chamber, and a stability wall
are supplied:
...
In this chapter the formula is a comparison template for the four
shadow depth classes $\mathbf{G}/\mathbf{L}/\mathbf{C}/\mathbf{M}$,
not an enumerative wall-crossing theorem. For
class~$\mathbf{G}$ (Heisenberg), the formal model reduces to the
MacMahon function itself; for class~$\mathbf{M}$
(Virasoro), the nontrivial formal comparison involves the
...
\item \emph{Universal motivic pole.}
 ...
 candidate pole at $s = -1/2$, independent of the
 algebra~$\cA$ in the formal model. The corresponding
 Denef--Loeser assertion would identify this pole with an
 eigenvalue of local monodromy; no such monodromy theorem is proved
 here. The formal log-canonical threshold is
 $\mathrm{lct}=1/2$.
\item \emph{Crystalline discriminant.}
 ...
 \frac{-320\,c^2}{5c + 22},
 \end{equation}
 On the weighted Riccati locus of
 Theorem~\textup{\ref{thm:spectral-curve}}, this is the
 spectral-curve discriminant. The
...
\item \emph{Integrality.}
 ...
 $n_g^d := \operatorname{PLog}_g(Z^{\mathrm{pleth}})$
 extracted from the formal shadow partition function are candidate
 integers. Their integrality is an enumerative assertion, not a
 consequence of the shadow tower alone.
\end{enumerate}
\end{remark}
...
gives a finite-atomic test for a proposed spectral model. The
self-interaction encoded by
$D\Theta + \tfrac{1}{2}[\Theta,\Theta] = 0$
constrains the moments; it determines atoms only after a
finite-atomic ansatz and enough initial moments have been fixed.

\begin{proposition}[MC bracket in a two-atom spectral ansatz]%
\label{prop:mc-bracket-determines-atoms}%
\ClaimStatusProvedHere%
\index{spectral atoms!from MC bracket}
Assume the Stieltjes spectral representation and restrict to a
two-atom Satake ansatz
$\rho = c_1\,\delta(\alpha) + c_2\,\delta(\beta)$
\textup{(}with nonzero total mass and nondegenerate first moments\textup{)}.
Then the MC recursion at each degree, rewritten in spectral
coordinates, is equivalent to the Newton
relation
...
Thus the MC equation tests the two-atom model by the recurrence
\eqref{eq:mc-atom-constraint}; it is not an atom-recovery theorem for
an arbitrary signed measure. More precisely:
\begin{enumerate}[label=\textup{(\roman*)}]
\item
...
consistent with the leading-order one-atom Virasoro asymptotic.
\end{enumerate}
\end{proposition}
...
Under prime-locality, the MC recursion, rewritten in spectral
coordinates via the Newton relations, encodes the power sums $p_r$,
which are the Dirichlet coefficients of
$L(s,\operatorname{Sym}^{r-1}f)$
(Proposition~\ref{prop:shadow-symmetric-power}).
With the finite-atomic and eigencomponent hypotheses inserted, the
MC equation gives the symmetric-power Dirichlet coefficients at all
degrees. Analytic continuation of
$L(s,\operatorname{Sym}^r f)$ for the required symmetric powers is a
separate automorphic input. Once that input is supplied, Serre's
reduction gives the Ramanujan bound
...
class-number growth gives $h > 1$ for all sufficiently large
admissible discriminants.
...
At each central charge, the seven evaluation points
...
its nontrivial zeros lie on the induced line
$\operatorname{Re}(s)=1/4$ under RH for~$\zeta$.
\item
...
GRH puts the zeros of each constituent on its functional-equation
line, while zeros of the resulting linear combination require the
separate single-constituent or non-cancellation hypothesis.
\item
...
poles. The remaining route is simultaneous in the algebra~$\cA$:
one must compute the MC constraint on $\varepsilon^c_s$ beyond the
Narain locus, prove that off-line residues violate the MC constraints,
and close the resulting central-charge bootstrap.
\end{remark}

% ============================================================
\section{The Hecke module structure}
\label{sec:recomputed-arithmetic-shadow}

\begin{theorem}[Hecke-equivariant MC element under finite-span stability]
...
Assume that the stable-graph operations in the bar MC element
preserve the finite Hecke span generated by $\Theta_\Lambda$.
\begin{enumerate}[label=\textup{(\roman*)}]
\item The Hecke operators $T_n \colon M_{r/2} \to M_{r/2}$ control
 the graph amplitudes: for every stable graph~$\Gamma$ of degree~$r$,
 \begin{equation}\label{eq:hecke-equivariance}
 \ell_\Gamma^{(g)}(V_\Lambda)
 \;\in\;
 \mathrm{Span}_{\bC}\{T_n\Theta_\Lambda \mid n \geq 1\}
 \subset M_{r/2}(\mathrm{SL}(2,\bZ)),
 \end{equation}
 after projection to weight~$r/2$.
...
\item The shadow at degree~$r$ decomposes into Hecke eigenspaces:
 ...
 Hecke coordinate of $\Theta_\Lambda$ in the $f_j$-line and
 $\alpha_j$ is a combinatorial weight.
...
By hypothesis these operations preserve the finite Hecke module
generated by $\Theta_\Lambda$. Hence $\ell_\Gamma$ lies in the Hecke
span of the theta component.
...
\begin{equation}\label{eq:andrianov-delta10-factorisation}
 ...
 \;=\;
 L(s, f_{18})\,\zeta(s-9)\,\zeta(s-8),
\end{equation}
where $f_{18}=E_6\Delta\in S_{18}(\mathrm{SL}_2(\bZ))$ is the
normalized elliptic source of the scalar-weight-ten
Saito--Kurokawa lift. At each
...
\begin{equation}\label{eq:arthur-packet-delta10}
 \psi_{\Delta_{10}}
 \;=\;
 \phi_{f_{18}} \boxtimes \operatorname{Sym}^1,
\end{equation}
...
not an automatic twist of a Vol~I Hecke category. The anomaly
class lies in
\begin{equation}\label{eq:m24-schur-multiplier}
 [\chi^{M_{24}}_g]
 \;\in\;
 H^3(M_{24}, \mathrm{U}(1))
 \;\supseteq\;
 \bZ / 12,
\end{equation}
the generalised-Mathieu-moonshine 't~Hooft anomaly per
Gaberdiel--Persson--Ronellenfitsch--Volpato 2013
(\texttt{arXiv:1211.7074}); compare
Gaberdiel--Hohenegger--Volpato \cite{GaberdielHoheneggerVolpato2012}
for the twined-genus realisation. The Schur multiplier
$M(M_{24}) = H^2(M_{24}, \bZ)$ is trivial (ATLAS); the $\bZ/12$
appears one degree up, as the anomaly cocycle that obstructs lifting
the projective $M_{24}$-action on the moonshine module to a linear
action. A phase-twisted local Hecke statement is therefore a
...
% ============================================================
\section{Quartic residue geometry}%
\label{sec:quartic-residue-theory}%
\index{quartic residue geometry|textbf}
...
\subsection{Compatibility ratios and closure}%
\label{sec:clutching-closure-theory}%
\index{closure|textbf}
...
\label{sec:four-gaps}%
\index{arithmetic shadow!obstructions}
...
\emph{Gap~B.} The pure-sector Narain identity
($\varepsilon^1_s(R) = 2(R^{2s} + R^{-2s})\zeta(2s)$)
shows that the diagonal momentum-plus-winding character factor
carries no zeros; it does not decide the full Narain spectral
measure. The genuine bottleneck is passing to the Virasoro
...
\index{arithmetic shadow!integrability vs positivity}%
...
breaking the circularity for lattice VOAs
...
are not Li coefficients of a completed $L$-function with one
functional-equation line. Consequently their sign is not equivalent to
RH or GRH for the constituent $L$-functions. The obstruction is
spectral, not arithmetic\textup{:}
\begin{enumerate}[label=\textup{(\roman*)}]
\item
The regularised sewing lift
$\Xi_\cA(u) = (u{-}1)\,S_\cA(u)$
need not have a single critical line.
For the Heisenberg,
...
on a \emph{single} line $\operatorname{Re}(u) = 1/2$. The shifted
Heisenberg factor already violates this hypothesis, even if RH holds
for $\zeta$.
\item
For Virasoro and~$\cW_N$, the harmonic defect terms
$H_{w-1}(u)$ are finite Dirichlet polynomials. They introduce zeros
which are not governed by an automorphic functional equation unless an
additional spectral interpretation is supplied.
\end{enumerate}
...
framework; the genus-$2$ B\"ocherer bridge theorem
...
Part~(ii): Li's theorem applies to a completed function whose zeros
are symmetric about one line and lie on that line precisely when the
Li coefficients are nonnegative. The Heisenberg sewing lift has the
additional shifted factor $\zeta(u+1)$, so it is outside the theorem's
domain.
...
these zeros are not zeros of a completed automorphic $L$-function
with the required functional equation.
\end{proof}
...
For product cases its zero set contains the shifted zero loci of its
factors; for genuine linear combinations, cancellation may create
additional zeros. For the Heisenberg, the factors give the zeros of
...
universal Euler--Koszul data (Euler product, $\sigma_{-1}$,
...
\index{quartic residue geometry!clutching defect}
...
ratios of~\S\ref{sec:clutching-closure-theory}: the clutching defect
...
Quartic residues require genuine
...
\index{quartic residue geometry!decisive computation}%
...
\subsection{Two faces of the genus-one arithmetic shadow}%
\label{sec:arithmetic-synthesis}%
\index{arithmetic shadow!synthesis|textbf}
...
\begin{remark}[Arithmetic closure]%
\label{rem:arithmetic-end-state}%
\index{arithmetic shadow!closure}%
The closed arithmetic picture is:
\begin{enumerate}[label=\textup{(\roman*)}]
\item
...
For lattice VOAs, Deligne's theorem supplies the Ramanujan bound.
The Hecke--Newton closure
(Theorem~\ref{thm:hecke-newton-lattice}) is the conditional
bar--cobar mechanism that would recover the same prime-local data
from the MC tower.
\end{remark}
...
\begin{proposition}[Prime-local shadow--symmetric power criterion]%
...
Assume prime-locality identifies the degree-$r$ shadow coefficient
with the $p$-local Satake trace. Then the local shadow contribution
satisfies
...
\end{equation}
and these local traces are the Dirichlet coefficients
...
Hecke decomposition, prime-locality is precisely the assertion that
the local component of $\lambda_j$ is proportional to $a_{f_j}(p)$.
Under this assertion
$a_{f_j}(p)^r = (\alpha_p+\beta_p)^r
= \sum_{m=0}^r \binom{r}{m}\alpha_p^m\beta_p^{r-m}$.
The complete symmetric polynomial
$h_{r-1}(\alpha_p,\beta_p)=\sum_{j=0}^{r-1}
\alpha_p^j\beta_p^{r-1-j}$ is
$\operatorname{tr}(\operatorname{Sym}^{r-1}
\operatorname{diag}(\alpha_p,\beta_p))$. It is obtained from the
elementary symmetric functions $\alpha_p+\beta_p$ and
$\alpha_p\beta_p$ by Newton identities, equivalently from the power
sums $p_m(\alpha_p,\beta_p)=\alpha_p^m+\beta_p^m$ for
$m\le r-1$.
...
\begin{remark}[The Serre--Langlands reduction;
\ClaimStatusProvedElsewhere]%
\label{rem:serre-reduction}%
\index{Serre's observation}%
\index{Langlands functoriality!and symmetric powers}%
For a holomorphic cuspidal Hecke eigenform~$f$ of weight~$k$,
write the normalized Satake parameters at an unramified prime~$p$ as
\[
\widetilde\alpha_p=\alpha_p\,p^{-(k-1)/2},\qquad
\widetilde\beta_p=\beta_p\,p^{-(k-1)/2},
\qquad
\widetilde\alpha_p\widetilde\beta_p=1 .
\]
The Serre--Langlands reduction
\cite{Serre1968Abelian,Langlands1970Problems,LuoRudnickSarnak1999}
is the following implication.  If, for every $r\geq 1$, the
symmetric-power lift $\operatorname{Sym}^r f$ is automorphic on
$\mathrm{GL}_{r+1}$ and lies in the automorphic class to which the
Luo--Rudnick--Sarnak local bound applies, then
$|\widetilde\alpha_p|=|\widetilde\beta_p|=1$ for every unramified
prime~$p$, equivalently
$|\alpha_p|=|\beta_p|=p^{(k-1)/2}$.
Indeed, the largest normalized Satake parameter of
$\operatorname{Sym}^r f$ at~$p$ has size
$|\widetilde\alpha_p|^r$ if $|\widetilde\alpha_p|>1$; the
Luo--Rudnick--Sarnak bound for automorphic representations of
$\mathrm{GL}_{r+1}$ forces this size to be at most
$p^{1/2-1/((r+1)^2+1)}$, and letting $r\to\infty$ gives
$|\widetilde\alpha_p|\leq 1$. Applying the same argument after
interchanging $\alpha_p$ and~$\beta_p$ gives
$|\widetilde\beta_p|\leq1$, and the product-one relation gives
equality.

If prime-locality holds at every degree, the shadow obstruction tower
encodes the local coefficients of the symmetric power $L$-functions.
The MC constraints at all degrees give polynomial relations between
these local traces. The conditional chain is:
\[
\text{MC (all degrees)}
\;\xrightarrow{\;\text{Prop.~\ref{prop:shadow-symmetric-power}}\;}
\operatorname{Sym}^r \text{ data}
\;\xrightarrow{\;\text{[automorphy/analytic input]}\;}
L(s,\operatorname{Sym}^r f)
\;\xrightarrow{\;\text{Serre--Langlands reduction}\;}
\text{Ramanujan.}
\]
...
$\operatorname{Sym}^r(\rho_f)$ for all~$r$.  For holomorphic
eigenforms of regular algebraic weight this functoriality is now
available by Newton--Thorne~\cite{NewtonThorne2021}; Deligne's
Weil-theoretic proof of Ramanujan for holomorphic modular forms
\cite{Deligne1974} is the stronger classical input already used on
the lattice-VOA lane.
\end{remark}
...
\begin{theorem}[Petersson identification under finite Hecke span]%
\label{thm:petersson-identification}%
\ClaimStatusProvedHere%
\index{Petersson inner product!shadow bracket identification}%
Assume the finite-Hecke-span stability criterion and the
chain-level shadow bracket projection to Hecke eigenspaces. For a
lattice VOA $V_\Lambda$ with Hecke decomposition
...
% ============================================================
\section{Modular rigidity}%
\label{sec:modular-rigidity-theory}%
\index{modular rigidity|textbf}
...
\begin{definition}[Rigidity defect; \ClaimStatusDefinitional]%
...
\begin{equation}\label{eq:rigidity-defect}
 \delta(m, r) \;:=\; (r - 1) - 2m\,.
\end{equation}
When $\delta(m,r)>0$, we say that the truncated MC system is
\emph{overdetermined}; in this terminology the corresponding
spectral measure is \emph{rigid at degree~$r$}.
\end{definition}
...
For $\delta(m,r) > 0$, the equations
$S_j = -(1/j)\sum_i c_i\,\lambda_i^j$,
$j = 2,\ldots,r$, form an overdetermined system. They may determine
the $m$~atoms $(\lambda_1,\ldots,\lambda_m)$ and $m$~weights
$(c_1,\ldots,c_m)$ uniquely up to permutation, but this uniqueness is
an additional rigidity assertion, not a consequence of counting alone.
Even when it holds, algebraic uniqueness does \emph{not} yield
...
\S\ref{sec:operadic-transfer-theory}):
...
\end{enumerate}
The third input is the operadic form of the missing analytic
continuation.
\end{remark}

\begin{proposition}[Lattice Ramanujan from rigidity hypotheses]%
\label{prop:lattice-ramanujan-rigidity}%
\ClaimStatusProvedHere%
\index{Ramanujan bound!lattice rigidity route}%
Assume prime-locality and uniqueness of the finite-atom rigidity
system. For lattice VOAs, the cusp-form atoms satisfy the Ramanujan
...
\begin{proof}
Under prime-locality, the atoms are
...
\emph{exact} identity, together with the assumed uniqueness of the
finite-atom rigidity system.
\end{proof}
...
$5$~equations in $4$~unknowns ($\delta = 1$). Under the
prime-locality and uniqueness hypotheses of
Proposition~\ref{prop:lattice-ramanujan-rigidity}, the solution is
...
arithmetic interface up to the stated prime-locality hypotheses:
shadow-symmetric power data
(Proposition~\ref{prop:shadow-symmetric-power}),
overdetermination
...
independent comparison only after finite-Hecke-span and
prime-locality hypotheses are supplied.
\item
\emph{Rational VOAs}:
...
The operadic functorial transfer
(\S\ref{sec:operadic-transfer-theory}) targets
...
\label{sec:operadic-transfer-theory}%
\index{operadic functorial transfer|textbf}
...
\begin{remark}[Analytic open mathematical conditions]
...
\begin{proposition}[Weighted Virasoro signed Stieltjes obstruction]%
...
Assume the weighted Riccati-metric Stieltjes model for the Virasoro
line. For $c > -22/5$, $c\neq 0$, its reduced quadratic discriminant
satisfies
...
\emph{complex} roots. For $c>0$ the spectral measure~$\rho$ in the
weighted Stieltjes representation
\textup{(}Theorem~\textup{\ref{thm:shadow-spectral-measure}}\textup{)}
is necessarily signed: $c_2 = \int\lambda^2\,d\rho = -c < 0$
for $c > 0$. Thus the unitary Virasoro class-$M$ line has no positive
Stieltjes measure in this weighted model. For
$-22/5<c<0$ the complex-root computation remains true, but the
second-moment sign test gives no positivity obstruction. Non-Virasoro
class-$M$ theories require their own discriminant and moment tests.
\end{proposition}
...
In the weighted Stieltjes normalization the second moment is
$c_2 = -2\widehat S^{\rm wt}_2 = -c$, so for $c>0$
...
\textbf{Consequence for prime-locality.}
...
By Proposition~\ref{prop:quasi-modular-propagator}, the genus-$1$
propagator is the quasi-modular Eisenstein series~$E_2^*(\tau)$,
whose non-holomorphic completion is
...
This is the Kaneko--Zagier ring of level-one quasi-modular forms
\cite{KanekoZagier1995}.
...
\begin{remark}[Euler-product consequence of the quasi-modular
obstruction; \ClaimStatusOpen]%
\label{rem:quasimodular-rankin-selberg-open}%
\index{shadow-level multiplicativity!Euler product question}%
The moment $L$-function
$M_r(s) = \int \operatorname{Sh}_r^{(1)} \cdot E(s,\tau)\,d\mu$
involves Rankin--Selberg integrals of \emph{quasi-modular}
forms against~$E(s,\tau)$, in the regularized regime of
Zagier~\cite{Zagier1981RankinSelberg}.
The unfolding theory for such integrals is more subtle than for
holomorphic eigenforms, since it involves~$\widehat{E}_2$.
The Euler product question for~$M_r(s)$ at degrees~$r \le 5$
is open: it requires an explicit quasi-modular Rankin--Selberg
analysis that has not been carried out.
\end{remark}
...
For lattice VOAs, the Hecke--Newton closure supplies this arrow only
under the finite-Hecke-span and prime-locality hypotheses stated in
Theorem~\ref{thm:hecke-newton-lattice}. The resulting chain is a
...
this is conditional on the Hecke module structure
of~$\gAmod[V_\Lambda]$
(Theorem~\ref{thm:spectral-continuation-bridge}).
This section resolves
prime-locality for lattice VOAs under that hypothesis via the
...
the Hecke algebra only after finite-Hecke-span stability and
prime-locality are supplied. This is the comparison mechanism.

\begin{theorem}[Hecke--Newton closure for lattice VOAs under finite Hecke span]%
\label{thm:hecke-newton-lattice}%
\ClaimStatusProvedHere%
\index{Hecke--Newton closure!lattice VOAs}%
\index{prime-locality!conditional for lattice VOAs}%
...
in $M_{r/2}(\mathrm{SL}(2,\bZ))$. Assume the finite-Hecke-span
stability criterion, the prime-locality identification of
Proposition~\textup{\ref{prop:shadow-symmetric-power}}, and
Rankin--Selberg admissibility of the moment functions. Then:
\begin{enumerate}[label=\textup{(\roman*)}]
\item
...
\item
At each prime~$p$ and for each Hecke eigenform~$f_j$, the
prime-local MC recursion gives Newton's identity:
...
\item
Prime-locality is then expressed by
$S_r^{(p)} \propto
...
\begin{proof}
\emph{(i)} Under the finite-Hecke-span hypothesis, the Hecke module
structure
...
\emph{(ii)} The prime-local MC equation at degree $r{+}1$ gives
Newton's identity $p_r = e_1\,p_{r-1} - e_2\,p_{r-2}$
(Proposition~\ref{prop:mc-bracket-determines-atoms}). For
each Hecke eigenform~$f_j$, the assumed prime-local component of~$\mu_r$
...
\emph{(iii)} This is the prime-locality hypothesis rewritten in
Newton coordinates.
\end{proof}

\begin{corollary}[Conditional operadic RS for lattice VOAs]%
\label{cor:conditional-lattice-operadic-rs}%
\label{cor:ramanujan-conditional-comparison}%
\ClaimStatusConditional%
...
For lattice VOAs, finite-Hecke-span stability, prime-locality,
Rankin--Selberg admissibility, and the MC moment recursion give the
comparison chain
\[
\text{MC moments on the prime-local lattice surface}
\Rightarrow \text{CPS automorphy}
\]
...
\begin{theorem}[Non-lattice Ramanujan implication]%
...
\textup{(}Langlands implication\textup{)}.}
With prime-locality imposed at the character level by the theorem,
the implication is:
\begin{itemize}[nosep]
\item Poisson summation is replaced
 with the modular transformation law of~$F$ and the
 vector-valued Hecke spectral theorem.
\item The MC equation, intertwining, and
 HS-sewing are properties of the algebra, not of
 the lattice.
\item Rankin--Selberg, CPS, prime-locality, strong
 multiplicity one, and Serre are applied to the
...
\begin{remark}[Spectral mechanism]
...
\smallskip
With these inputs and symmetric-power functoriality, the implication
acts component-by-component:
\begin{itemize}[nosep]
\item holomorphy of~$\chi_0$ and the
 Roelcke--Selberg spectral decomposition $+$
 multiplicativity of $\phi_j$ replace the scalar theta
 computation;
\item the MC equation, intertwining, HS-sewing, and the
 Rankin--Selberg method remain algebraic and analytic inputs;
\item CPS, strong multiplicity one, and Serre are applied to each
 spectral component $L(s, \phi_j \times \bar\phi_j)$. The CPS
 hypotheses are supplied by Conjecture~\ref{conj:cps-from-mc};
...
\emph{Automorphic input.} The Serre reduction
...
\noindent
For lattice VOAs, the Hecke--Newton closure under finite Hecke span
...
\item Hypothesis~\textup{(c)} is the amplitude-level content.
 ...
 It remains to prove that both the extraction of
...
\textup{(}Koszul self-dual\textup{)}, and if the finite
Hecke decomposition of $Z_\cA$ is identified with the shadow
constituents of $\cA$, then the constrained
Epstein zeta $\varepsilon^c_s(\cA)$ decomposes as a finite Hecke
packet of standard automorphic $L$-functions:
\begin{equation}\label{eq:self-dual-factorization}
 \varepsilon^c_s(\cA)
 \;=\;
 \sum_{j\in J} c_j\,\Gamma_j(s)\,L(s, f_j),
\end{equation}
where $J$ is finite, each $f_j$ is an Eisenstein or cuspidal Hecke
eigenform for $\Gamma_0(N)$, and $\Gamma_j(s)$ collects archimedean
factors. The equality $|J|=d(\cA)-1$ is not part of the theorem; it is
the separate non-cancellation assertion in the shadow--spectral
correspondence.
\end{theorem}
...
The Hecke algebra is semisimple on the finite-dimensional modular-form
space after separating Eisenstein, new, and old packets; multiplicity
one identifies each new cuspidal constituent up to scalar. Hence
$Z_\cA$ decomposes as a finite sum of simultaneous Hecke eigenpackets
$f_j$.

\smallskip
\emph{Step~4} \textup{(standard $L$-functions)}.
Each Hecke eigenpacket~$f_j$ gives a standard automorphic
$L$-function $L(s,f_j)$ with Euler product and meromorphic
continuation (Hecke theory). The shadow--spectral correspondence
(Theorem~\ref{thm:shadow-spectral-correspondence}) records the
functional-equation centres of these constituents; it does not by
itself prove that no constituents cancel or merge.
Under the additional Hecke--Newton hypotheses, the shadow
algebra~$\cA^{\mathrm{sh}}$ determines the Hecke coordinates via
the Newton relations in spectral coordinates
...
\begin{remark}[Resolution of the non-lattice gap for minimal models]%
\label{rem:theta-bridge-resolution}%
\index{non-lattice gap!resolved for minimal models}%
\index{minimal model!theta bridge scope}%
Proposition~\ref{prop:theta-bridge-rational} resolves the
non-lattice obstruction
(Remark~\ref{rem:non-lattice-gap}) for Virasoro minimal models with
...
For a general rational VOA, the analogous statement requires the
vector-valued modular representation and its Hecke or theta
decomposition; it is not a consequence of the Rocha--Caridi formula.
The irrational central-charge regime is still further away:
$|\eta(\tau)|^{2(c-1)}$ is not a modular form for any congruence
subgroup, and the spectral decomposition necessarily involves
continuous spectrum (Conjecture~\ref{conj:irrational-ramanujan}).
\end{remark}
...
On the finite-Hecke-span and prime-local comparison surface, the
shadow algebra
$\cA^{\mathrm{sh}} =
H_\bullet(\operatorname{Def}^{\mathrm{mod}}_{\mathrm{cyc}}(\cA))$
maps to the moment algebra of the Hecke spectral decomposition of
the partition function~$Z_\cA$. Specifically, after a chosen
Hecke eigencomponent has been fixed, the shadow power sums
$p_r = r\cdot S_r$ determine the elementary symmetric
polynomials~$e_k$ of the corresponding Satake parameters by Newton's
recursion:
...
No algebra isomorphism
$\cA^{\mathrm{sh}}\cong\mathbb T_{\mathrm{Hecke}}$ is asserted
outside this comparison surface.
\end{proposition}
...
On the lattice finite-Hecke-span and prime-local surface, the Hecke
module structure
...
The MC equation at degree~$4$ gives the Newton recurrence
$p_4 = e_1\,p_3 - e_2\,p_2$
(Proposition~\ref{prop:mc-bracket-determines-atoms}).
The Hecke relation $T_p^2=T_{p^2}+p^{k-1}T_1$ lives in the Hecke
operator algebra; under the same Satake coordinates it uses the same
two elementary parameters \(e_1=a_f(p)\) and \(e_2=p^{k-1}\).
This is the promised structural parallel, not an identification of
the MC bracket with Hecke multiplication.
\end{proof}

\subsection{The Davenport--Heilbronn principle}

\begin{remark}[Class-group obstruction and on-line zeros]%
\label{rem:davenport-heilbronn-koszul}%
\ClaimStatusProvedElsewhere%
\index{Davenport--Heilbronn!class-group obstruction}%
\index{class number one!partial zeta factorisation}%
The Davenport--Heilbronn phenomenon is controlled by the class group,
not by Koszul self-duality alone. Let $Q$ be a positive definite
primitive binary quadratic form of discriminant~$D<0$, and let
$[Q]\in\mathrm{Cl}(D)$ be its class. Its Epstein zeta is the partial
zeta function of the class~$[Q]$. If $h(D)=1$, this partial zeta is
the Dedekind zeta of $\bQ(\sqrt D)$ and therefore factors as
$\zeta(s)L(s,\chi_D)$; under GRH its nontrivial zeros lie on
$\operatorname{Re}(s)=1/2$. If $h(D)>1$, a partial zeta is a linear
combination of class-group Hecke $L$-functions. Such a linear
combination has a functional equation, but no Euler product for the
individual class, and Davenport--Heilbronn gives examples with
off-line zeros. The Verdier involution records class inversion
$[Q]\mapsto[Q]^{-1}$; it does not turn a partial zeta into the principal
Dedekind factorisation.

Thus the algebraic shadow is class-group symmetry. The analytic input
for on-line zeros is factorisation into automorphic $L$-functions
together with GRH for the constituents.
\end{remark}
...
expansion, and the conditional Hecke--Newton closure all flow from the
holomorphic sewing construction. It also limits
the arithmetic mechanism: the K\"ahler
...
\begin{proof}
Part~(i): with the Fock-space sewing normalization of
Proposition~\ref{prop:divisor-sum-decomposition}, the operator
$K(\tau)$ has eigenvalues~$q^n$ for $n\ge 1$, so
...
connection to all~$s$: the regularized Rankin--Selberg integral
$\operatorname*{Reg}_{\mathrm{RS}}\int\log\det(1-K)\cdot
E_s\,d\mu$ equals a product of gamma and zeta functions, providing
explicit meromorphic continuation. The complexified integral below
asks what structure this hierarchy reveals at the scattering poles
$s = \rho/2$.
\end{remark}
...
conformal factors, chosen to make the integral convergent. The
normalization is fixed by the real section
$\mathcal{C}_{\bR} = \{\sigma\colon E_\tau \to\bR\}$: since
$S_L$ is quadratic for the flat torus, this reference contour is
Gaussian and is normalized to recover the standard spectral
determinant.
\end{construction}
...
% ============================================================
\subsection{Obstruction diagram}%
\label{subsec:polyakov-conformal-gauge-obstruction}%
\index{Polyakov complexification!obstruction diagram}
...
\begin{tabular}{clll}
\toprule
& \textbf{Component} & \textbf{Mathematical input} & \textbf{Reference} \\
\midrule
...
Complexification introduces three structural obstructions beyond the
four of~\S\ref{sec:four-gaps}:
\begin{itemize}[nosep]
\item \emph{Gap P1: Genus-one triviality.}
 ...
 The genus-$1$ arithmetic shadow lives where this mechanism is absent.
...
The Polyakov integral complements the spectral approach but
...
\item Gap~P3 shows that the MC constraint, while governing
 ...
 metric.
\end{itemize}
The precise target is a representation of the partition function not
obtained by RS unfolding and whose analytic structure in~$s$ retains
the scattering poles. Higher-genus Polyakov integrals, complexified
Teichm\"uller space, and non-unfolded spectral representations remain
the possible sources.
\end{remark}

\begin{corollary}[Regularized nonvanishing at the first scattering pole]%
...
\label{sec:analytic-continuation-theory}%
\index{analytic continuation|textbf}
...
output
\textup{(}\S\S\ref{sec:sewing-RS-bridge}--\ref{sec:operadic-transfer-theory}\textup{)}.
...
\begin{definition}[Scattering coupling; \ClaimStatusDefinitional]%
...
Assume the spectral measure satisfies the Carleman condition and that
descendant corrections are absorbed by the chosen Mellin completion.
Then the algebra-dependent factor is controlled by the shadow
obstruction tower:
\begin{enumerate}[label=\textup{(\roman*)}]
...
\end{enumerate}
Consequently, under these hypotheses, the MC element~$\Theta_\cA$ determines
...
that, under Carleman and Mellin-completion hypotheses, the MC
equation determines the \emph{coupling strength} $\cE_\rho(\cA)$
of each chiral algebra to each scattering pole. It does \emph{not}
determine the \emph{location} of the poles: the zero~$\rho$ enters
only through the evaluation
...
simultaneously:
\begin{enumerate}[label=\textup{(\roman*)}]
\item \textup{(Algebraic accounting, conditional on the CPS
vertical-strip estimates in Corollary~\textup{\ref{cor:moment-automorphy}}.)}\quad
 Under the hypotheses of Theorem~\textup{\ref{thm:scattering-coupling-factorization}},
...
\end{enumerate}
Together, these show that the listed genus-$1$ algebraic outputs do
not contain zero-location information, and that the remaining
zero-location question requires
...
Hecke decomposition) are the algebraic data extracted here from the
...
for the genus-$2$ arithmetic opened by this non-diagonality.
\end{remark}
...
\begin{proposition}[Vanishing of the Hecke defect under finite Hecke span;
\ClaimStatusProvedHere]%
\label{prop:hecke-defect-lattice}%
\index{Hecke defect!vanishing for lattices}%
For a lattice VOA $V_\Lambda$, assume the finite-Hecke-span stability
criterion and a chain-level Hecke action commuting with the OPE
contractions entering $d_{\mathrm{sew}}$. Then
$[\delta_p(V_\Lambda)] = 0$ for all primes~$p$.
\end{proposition}
...
every graph amplitude $\ell_\Gamma$ lies in the finite Hecke span.
By the chain-level commutation hypothesis, $T_p$ commutes with the
OPE contractions and propagators defining $d_{\mathrm{sew}}$ up to
$D_\cA$-exact correction. Hence the commutator represents the zero
class in $H^1(\gAmod,\mathrm{ad}_{T_p})$.
\end{proof}
...
\item The \emph{graph-sum Hecke compatibility:}
 for every genus-$1$ shadow amplitude,
 \[
  T_p\bigl(\operatorname{Sh}_r(\Theta_\cA)\bigr)
  =
  \operatorname{Sh}_r\bigl(T_p(\Theta_\cA)\bigr)
 \]
 after projection to the finite Hecke span generated by the
 character or theta components.
\end{enumerate}
Then prime-locality holds under these four hypotheses. On the rational
central-charge locus the Ramanujan bound is the Langlands implication
\textup{(}Theorem~\textup{\ref{thm:non-lattice-ramanujan}}\textup{)};
on the irrational locus it is the assertion of
...
The Hecke operator $T_p$ acts on the finite Hecke span generated by
the Fourier coefficients of the MC element $\Theta_\cA$. Hypothesis
\textup{(d)} is precisely the compatibility of this Hecke action with
the operadic graph polynomial that defines
...
at each prime, giving prime-locality. The Langlands implication gives
the Ramanujan bound on the rational central-charge locus; the
irrational extension has the hypotheses of
Conjecture~\ref{conj:irrational-ramanujan}.
\end{proof}

\begin{conjecture}[Ramanujan reduction for chirally Koszul
...
The Ramanujan implication is reduced to the vanishing of
$[\delta_p(\cA)] \in H^1(\gAmod, \mathrm{ad}_{T_p})$ for all
primes~$p$, together with graph-sum Hecke compatibility.
\end{conjecture}

\subsection{Obstruction diagram}%
\label{subsec:analytic-continuation-obstruction}%
\index{analytic continuation!obstruction diagram}
...
\begin{tabular}{clll}
\toprule
& \textbf{Component} & \textbf{Mathematical input} & \textbf{Obstruction} \\
\midrule
\multicolumn{4}{l}{\emph{Ramanujan bound}} \\
\cmidrule{1-4}
...
7 & Ramanujan (irrational)
 ...
 & Langlands $r \ge 5$ \\
\cmidrule{1-4}
\multicolumn{4}{l}{\emph{Zero-location}} \\
\cmidrule{1-4}
...
\noindent
Two mechanisms remain. The Ramanujan mechanism is an implication
diagram: the lattice input is Hecke--Newton prime-locality, and the
additional inputs are CPS vertical-strip and twist estimates,
local-factor comparison, rational graph-sum Hecke compatibility, and
Langlands functoriality for higher symmetric powers. The
zero-location mechanism is structurally blocked at genus~$1$ by
...
\begin{problem}[Three analytic-continuation obstructions]%
\label{prob:ac-open-problems}%
\index{analytic continuation!obstructions}
\begin{enumerate}[label=\textup{(\Alph*)}]
\item
...
revision of the Ramanujan implication; a systematic vanishing
...
generic to critical level) is the admissible-level problem
...
and the conditional Hecke-equivariant MC element
(Theorem~\ref{thm:spectral-continuation-bridge}) determine a single
geometric object after the finite-Hecke-span and $L$-packet
hypotheses are imposed. Under those hypotheses the arithmetic content
and the homotopy defect are complementary components of a flat
...
Under the spectral-continuation bridge hypothesis, for a lattice
VOA~$V_\Lambda$ all nilpotent parts vanish and
\[
...
\begin{definition}[Frontier defect form; \ClaimStatusDefinitional]%
...
the three analytic-continuation obstructions of~\textup{\ref{prob:ac-open-problems}}
reformulate as:
...
Under the spectral-continuation bridge hypothesis, for principal~$\cW_N$
...
the three obstructions reduce to the defect sector.
...
Assume the genus-one sewing--Hecke collapse package. It does not
extend to genus~$2$. Specifically, for the
...
open mathematical condition: the twisted $11$D supergravity of
...
\begin{remark}[Genus-$2$ frontier]%
\label{rem:genus2-arithmetic-frontier}%
\index{genus-2 arithmetic frontier!obstruction boundary}%
...
\item The comparison between the spin $L$-function
 ...
 (Andrianov--Zagier), so the automorphic comparison exists.
 The remaining problem is the strength of the MC input.
\end{enumerate}
...
components. The separate identification of the cusp projection is the
conditional comparison problem treated below.
\end{proposition}
...
\begin{theorem}[B\"ocherer bridge under three-shell reconstruction;
\ClaimStatusProvedHere]%
...
of~$\Theta_\Lambda^{(2)}$. Assume that the genus-$2$ three-shell
reconstruction map identifies the cusp projection of
$\Theta_\Lambda^{(2)}$ from the MC data and boundary restrictions.
Then:
\begin{enumerate}[label=\textup{(\roman*)}]
\item \textup{(Conditional MC determination)}
The genus-$2$ MC equation
\textup{(}$D^2 = 0$ restricted to genus~$2$, giving the three-shell
loop-square, separating-loop, and planted-forest decomposition\textup{)}
determines the cusp projection of $\Theta_\Lambda^{(2)}$ from
genus-$1$ shadow data and boundary contributions.
...
\begin{proof}
Part~(i) is the stated reconstruction hypothesis applied to the MC
equation at genus~$2$:
...
Under the reconstruction hypothesis,
the B\"ocherer factorization~\eqref{eq:bocherer-factorization}
converts the recovered Fourier coefficients to
...
Access $\neq$ control.
Under the three-shell reconstruction hypothesis, the MC equation
recovers central $L$-values, but it does not by itself \emph{force}
positivity or non-vanishing.
...
Under the three-shell reconstruction hypothesis, the MC equation at
genus~$2$ determines the partition
...
\begin{remark}[Leech weight-$12$ cusp line is Saito--Kurokawa;
\ClaimStatusProvedHere]%
\label{rem:leech-weight12-sk}%
\index{Leech lattice!weight-12 cusp line}%
\index{Saito--Kurokawa lift!Leech lattice}%
For the Leech lattice the relevant full-level genus-$2$ weight is
$k = r/2 = 12$. In that single weight,
$\dim S_{12}(\mathrm{Sp}(4,\bZ)) = 1$, and the unique
cusp form $\chi_{12} = \mathrm{SK}(f_{22})$ is a
Saito--Kurokawa lift
\cite{DPSS20,PiatetskiShapiro1983,Schmidt2007}.
For weights beyond the Leech weight, the Siegel cusp spectrum is
larger.
...
\begin{theorem}[Leech $\chi_{12}$-projection and Waldspurger consequence
under three-shell reconstruction; \ClaimStatusProvedHere]%
...
so the functional equation does not force
$L(11,\, f_{22} \times \chi_D)$ to vanish. Non-vanishing is a
separate arithmetic input.

\item \textup{(Conditional MC chain)}
Assume the genus-$2$ three-shell reconstruction hypothesis of
Theorem~\textup{\ref{thm:bocherer-bridge}}. Then the composition
\[
D_\cA^2 = 0
...
L(11,\, f_{22} \times \chi_D)
\]
recovers the corresponding twisted central values from MC data.
This is a conditional instance of homotopy-algebraic data reaching a
critical-line value.
\end{enumerate}
\end{theorem}
...
Part~(iv) is conditional on the reconstruction hypothesis in
Theorem~\ref{thm:bocherer-bridge}.
\end{proof}
...
locus where \eqref{eq:h2-classification-delta5} is the comparison
target.
\end{remark}
...
% ============================================================
\section{Prime-locality boundary}%
\label{sec:prime-locality-obstruction}%
...
structurally obstructed in prime-locality.
...
\label{prop:prime-locality-settled-reduced-cases}%
\index{prime-locality!settled and reduced cases|textbf}
...
\item \emph{Lattice VOAs} \textup{(}all ranks, all levels\textup{):}
 ...
 into Hecke eigenforms. The vanishing of
 $[\delta_p(V_\Lambda)]$ is the Hecke-stability of this finite
 eigenspace together with the theta-factorization of the shadow
 graph sum; it is not a consequence of Hecke operators being ring
 homomorphisms
...
Five obstructions govern the extension of prime-locality beyond the
proved cases:
\begin{enumerate}[label=\textup{(\alph*)}]
...
\item \emph{Quasi-modularity of shadow amplitudes:}
 ...
 \textup{(}hence fixed by the scalar action on constants\textup{)}, the
...
\label{subsec:route-c-overdetermination}%
\index{prime-locality!MC overdetermination|textbf}
...
\begin{theorem}[Weighted Riccati determinacy; \ClaimStatusProvedHere]%
\label{thm:riccati-determinacy}%
\index{Riccati algebraicity!as modified prime-locality}
Assume the weighted Riccati algebraicity theorem for the family under
consideration. Then the weighted Riccati-metric coefficients
$\widehat S^{\rm wt}_r$ at all degrees are determined by
$S_2 = \kappa$, $S_3 = \alpha$, and $S_4 = Q^{\mathrm{contact}}$
through a polynomial recursion:
\[
 \widehat S^{\rm wt}_r
 \;=\;
 \text{polynomial in $\kappa, \alpha, S_4$}.
\]
...
weighted coefficient data at all degrees is determined by three
algebraic invariants of the OPE. These invariants are
tau-independent for algebraic families; this means only that the
coefficient tower is fixed by the scalar action of Hecke operators on
constants. It is not Hecke equivariance of genus-$1$ amplitudes.
...
The following negative results give the boundary of the arithmetic
shadow. Each is as important as a positive theorem: it identifies a
structural wall rather than a defect of technique.
...
\textup{(}degree-indexed by shadow coefficients, not
Fourier-indexed by the genus-$1$ amplitude coefficients\textup{)}
...
introduces shifted zero loci and removes the single-line hypothesis
in Li's theorem.
...
\begin{remark}[Prime-locality boundary]%
...
the Hecke algebra acting on quasi-modular forms. The compatibility
condition is one of: \textup{(A)}~a vanishing theorem for
...
prime-locality to the Faber--Pandharipande tautological relations.
\end{remark}

\begin{proposition}[Arithmetic shadow tower and modular-form scope
ledger; \ClaimStatusConditional]
\label{prop:as-arithmetic-shadow-scope-ledger}
\index{arithmetic shadow tower!scope ledger}
\index{moment L-function@moment $L$-function!definition}
\index{Hecke--Newton closure!scope ledger}
\textup{[}Type signature:
$(\mathrm{Open},\mathrm{genus\text{-}one\ arithmetic\ shadow},
\mathrm{level}=5,
\mathrm{hypothesis\ package}=\mathrm{HS\text{-}sewing}
\ +\ \mathrm{Rankin\text{--}Selberg\ moderate\ growth}
\ +\ \mathrm{Hecke\ lift}
\ +\ \mathrm{CPS/prime\text{-}locality\ when\ automorphy\ is\ claimed})$
\textup{].}
The arithmetic shadow tower is the functor
\[
\mathsf{ASh}\colon
\langle\Theta_\cA\rangle_{\mathrm{genus}\,1}
\longrightarrow
\mathrm{DirSeries}_{\mathrm{mer}},
\qquad
\pi_{1,r}(\Theta_\cA)\longmapsto M_r(s),
\]
where the moment \(L\)-function is the regularized
Rankin--Selberg pairing
\begin{equation}
\label{eq:as-moment-l-function-ledger}
M_r(s):=
\operatorname*{Reg}_{\mathrm{RS}}
\int_{\SL_2(\mathbb Z)\backslash\mathfrak H}
\operatorname{Sh}^{(1)}_r(\cA;\tau)\,
E^*(\tau,s)\,d\mu(\tau).
\end{equation}
The integral converges absolutely in the initial half-plane
\(\mathrm{Re}(s)>1+N_r\), where \(N_r\) is the polynomial cusp-growth
degree of the shadow amplitude.  Meromorphic continuation is asserted
only after the Rankin--Selberg/Zagier regularization hypotheses for
that amplitude have been verified.

The Hecke action on \(M_k(\SL_2(\mathbb Z))\) is
\begin{equation}
\label{eq:as-hecke-action-ledger}
(T_n f)(\tau)=
n^{k-1}
\sum_{\substack{ad=n\\0\le b<d}}
d^{-k}\,
f\!\left(\frac{a\tau+b}{d}\right).
\end{equation}
A lift of \(T_n\) to the modular convolution algebra
\(\gAmod\) is a separate chain map
\[
T_n^{\gAmod}\colon \gAmod(\cA)\to\gAmod(\cA)
\]
intertwining the genus-one projection of \(\Theta_\cA\).  This lift is
proved on the lattice finite-Hecke-span surface of
Theorem~\ref{thm:hecke-newton-lattice}; outside that surface it is a
hypothesis.  Prime-locality is the assertion that the local \(p\)-part
of the MC recursion matches the \(p\)-th Hecke operator before
forming an Euler product.  It is proved only in the stated lattice
finite-Hecke-span lane, is a separate hypothesis for non-lattice
rational VOAs, and is conditional for irrational theories.

The CPS package means: analytic continuation, functional equations,
and vertical-strip boundedness for the relevant twists, sufficient to
apply the Cogdell--Piatetski-Shapiro converse theorem.  When this
package holds, write \(\pi_r\) for the resulting automorphic
representation attached to \(M_r(s)\).  The functorial
identification
\begin{equation}
\label{eq:as-symmetric-power-functoriality-ledger}
\pi_r\simeq\operatorname{Sym}^{r-1}(\pi_f)
\end{equation}
is a hypothesis for \(r\ge5\), not a consequence of the MC equation.
Accordingly no Ramanujan bound is claimed beyond known functoriality:
Deligne applies to holomorphic cuspidal \(\GL_2\) forms in its
scope, while the unconditional Kim--Sarnak bound gives
\(|\alpha_p|,|\beta_p|\le p^{7/64}\) for the relevant Maass
\(\GL_2\) normalisation.

The Hecke--Newton closure is the finite-Hecke-span theorem for lattice
VOAs: Newton identities applied primewise to the finite Hecke span of
theta eigenforms convert the MC moments into local symmetric-power
polynomials.  Non-lattice and irrational extensions require separate
Hecke modules and are not included in that theorem.

Dirichlet \(L\)-functions arising from a shadow metric are the
characters \(L(s,\chi_\rho)=\sum_{n\ge1}\chi_\rho(n)n^{-s}\) obtained
from a one-dimensional eigenspace \(\rho\) of the shadow metric under
the Hecke action.  The Roelcke--Selberg spectral decomposition is the
expansion of \(L^2(\SL_2(\mathbb Z)\backslash\mathfrak H)\) into
constant, Eisenstein, and cuspidal components; compatibility with the
chiral partition function means that the genus-one character lies in
the finite Hecke span used above.  The sewing operator is the
Fredholm operator \(K_q\) of Proposition~\ref{prop:sewing-trace-formula};
the regularized sewing--Selberg formula is
Theorem~\ref{thm:sewing-selberg-formula}.

For genus two, the Saito--Kurokawa lift is used only where the
input elliptic eigenform and its CAP lift are known.  The B\"ocherer
bridge means the square relation between Fourier coefficients of the
genus-two Siegel form and central \(L\)-values, again only under the
known hypotheses of that theorem.  Critical-line access is conditional
unless the relevant automorphic continuation, functional equation,
positivity, and non-vanishing inputs have been supplied.

The Borcherds product used on the K3 branch is
\begin{equation}
\label{eq:as-delta5-borcherds-product-ledger}
\Delta_5(Z)
=
e^{2\pi i\langle\rho_{\Delta_5},Z\rangle}
\prod_{\alpha\in\Phi^+_{\Delta_5}}
\left(1-e^{2\pi i\langle\alpha,Z\rangle}\right)^{
\operatorname{mult}(\alpha)}.
\end{equation}
Here the multiplicities are the Gritsenko--Nikulin/Borcherds
Fourier-coefficient multiplicities in the \(\Delta_5\) convention.
\end{proposition}

\begin{proposition}[BKM, Igusa, associator, and bracket scope gate;
\ClaimStatusConditional]
\label{prop:as-bkm-igusa-associator-bracket-gate}
\index{BKM invariant!Delta5 coefficient gate@$\Delta_5$ coefficient gate}
\index{Igusa cusp form!scope gate}
\index{Yetter-Drinfeld!Schauenburg bracket!bar differential separation}
\index{GRT1@$\mathrm{GRT}_1$!K3 BKM scope}
\textup{[}Type signature:
$(\mathrm{Open}/\mathrm{CY},
\mathrm{Borcherds\ shadow\ comparison},
\mathrm{level}=5,
\mathrm{hypothesis\ package}=\mathrm{Borcherds\ lift}
\ +\ \mathrm{CHL\ normalization}
\ +\ \mathrm{EK/GRT\ associator\ map\ when\ used}
\ +\ \mathrm{YD\ comparison\ map\ when\ used})$\textup{].}
In the K3 Borcherds convention the input weak Jacobi form satisfies
\[
 \phi_{0,1}^{K3}(\tau,z)
 = y + 10 + y^{-1} + O(q).
\]
Thus the constant Fourier coefficient in Borcherds' weight formula is
\begin{equation}
\label{eq:as-delta5-c0-bkm-gate}
 c_{\Delta}(0,0)=10,\qquad
 \kappa_{\mathrm{BKM}}(\Delta_5)
 =\operatorname{wt}(\Delta_5)
 =\frac{c_{\Delta}(0,0)}{2}
 =5.
\end{equation}
This is the BKM automorphic lane.  It is not the K3 Mukai
Koszul-conductor lane \(K^{\kappa_{\mathrm{ch}}}\), and no CHL
constant may be transported to the Mukai value without a named
comparison morphism.  The CHL Borcherds-products considered here are
the standard \(N\in\{1,2,3,4,6\}\) cases; outside that set the CHL
normalization is not being invoked.

The scalar Igusa form used in this chapter is
\begin{equation}
\label{eq:as-igusa-phi10-gate}
 \Phi_{10}^{\mathrm{un}}(Z)=\Delta_5(Z)^2,
\end{equation}
the unnormalised weight-\(10\) Siegel cusp form on the
\(\mathrm{Sp}_4\)-side.  The monic Oberdieck--Pandharipande
normalization \(D_5=64^{-1}\Delta_5\) changes scalar products but not
\(\partial_Z\log\Phi_{10}^{\mathrm{un}}\).  If the quotient
\(\Theta_{\Lambda_{24}}/\eta^{24}\) is used, it is the weight-zero
modular function \(j-720\): \(\Theta_{\Lambda_{24}}\) and
\(\eta^{24}\) both have weight \(12\), and the multiplier cancellation
is part of the convention, not an implicit equality with the K3 BKM
line.

An occurrence of \(\Phi_{10}^{\mathrm{un}}\) or \(\Delta_5\) in an
associator cocycle is an algebraic theorem only after a chain map from
the chosen EK/KZ associator complex to
\[
 H^2(\mathfrak g_{\Delta_5})^{\mathbb Z/2,K(1)}
 \quad\text{or}\quad
 \mathrm{YD}(\mathbf H_{\Delta_5})
\]
has been constructed and the relevant cocycle has been pushed through
that map.  Without this map it is an automorphic shadow, not a bar
differential or a physical operator.  The Fricke large-deviation
sub-leading correction is exactly the nodewise statement of
Theorem~\ref{thm:fricke-ldp-sub-leading}.  The
Shimura--Waldspurger conversion used here is the period-ratio theorem
at weights \(k+1\in\{7,9,11\}\), recorded in
Theorem~\ref{thm:shimura-waldspurger-higher-weights}.

The Yetter--Drinfeld Schauenburg bracket is the braided-coboundary
operation on \(\mathrm{YD}(\mathbf H_{\Delta_5})\).  The displayed
\(\delta^{(n)}\) values are precisely the Catalan--Brown dimensions in
Theorem~\ref{thm:YD-delta-7-8-9}.  They are not terms of the chiral bar
differential unless a comparison map from the YD braided-coboundary
complex to the bar complex has been specified.  Likewise
\(\mathrm{GRT}_1\) acts on formality and associator choices
\textup{(}Theorem~\ref{thm:defq-grt1-equivariance}\textup{)}; it does
not act transitively on K3--BKM source recognitions unless the finite
Hall--Borcherds recognition datum and formal-rigidity hypothesis are
added.  All arithmetic material in this proposition therefore sits
beyond the theorem spine unless it is used to prove a named algebraic
claim through one of the comparison maps above.
\end{proposition}
...
\item \emph{Universal conductor.} The genus-$1$ free energy
 ...
 the arithmetic shadow: the universal
...
\item \emph{Climax identity.} The shadow Eisenstein identity
 ...
 of the typed KZ--Arnold bar-superconnection theorem of
 Chapter~\ref{ch:climax-theorem}: after Fulton--MacPherson
 boundary-residue realization, the local affine/formal Arnold
 representative $\eta_{ij}$ is replaced in genus~$1$ by the KZB
 representative, whose scalar projection produces the quasi-modular
 Eisenstein series $E_2^*$.  The Mellin transform of the resulting
 graph sum is the displayed product of two Riemann zeta functions.
 The conductor
...
\item \emph{Koszul reflection in the arithmetic locus.} The
 ...
 (Theorem~D inscription, Vol~I main theorem line) is the
...
inside the derived-centre closed-sector data on that comparison
surface; it is not an equality with the scalar shadow invariants.
...
\end{enumerate}
The phrase ``three faces'' is therefore only a comparison locus until
the Humbert and Lusztig targets are promoted by these data. The
...
diagonal restriction, and primitive normalisation produce the half K3 weak
Jacobi form
$\phi_{0,1}^{K3}=y+y^{-1}+10+O(q)$, while
$Z_{K3}=2\phi_{0,1}^{K3}$ is the doubled elliptic-genus trace.  The
Borcherds singular-theta lift of $\phi_{0,1}^{K3}$ has target output
$\Delta_5$. Each arrow loses information, and the 10
...
multiplier cocycle, the diagonal restriction map to the half K3 Jacobi form,
...
% ============================================================
\section{Formal coordinates for a Yetter--Drinfeld correction}
\label{sec:YD-delta-n-7-8-9}

Let \(\mathcal T_n\) be the set of planar binary trees with \(n\)
leaves, and let \(\mathcal B_n^{\mathrm{mot}}\) be a basis of the weight-\(n\)
component of Brown's motivic multiple-zeta algebra.  Their product is
a formal coordinate set.  A Schauenburg correction cochain is obtained
after a word-to-Yetter--Drinfeld map and its differential relations
have been specified.

\begin{theorem}[Catalan--motivic coordinate counts;
\ClaimStatusProvedHere]
\label{thm:YD-delta-7-8-9}
\index{Yetter-Drinfeld!Schauenburg bracket!formal coordinates}%
The coordinate set
\(\mathcal I_n=\mathcal T_n\times\mathcal B_n^{\mathrm{mot}}\) has cardinality
\[
 |\mathcal I_n|=C_{n-1}d_n,
 \qquad
 \sum_{n\ge0}d_nt^n=\frac{1}{1-t^2-t^3}.
\]
For \(7\le n\le12\), the exact values are
\begin{equation}
\begin{aligned}
n=7 &: \quad 132\cdot3=396,\\
n=8 &: \quad 429\cdot4=1716,\\
n=9 &: \quad 1430\cdot5=7150,\\
n=10&: \quad 4862\cdot7=34034,\\
n=11&: \quad 16796\cdot9=151164,\\
n=12&: \quad 58786\cdot12=705432.
\end{aligned}
\label{eq:YD-delta-789-dim}
\end{equation}
These numbers count formal tree--period coordinates.  The rank of a
represented Yetter--Drinfeld cochain is computed after the comparison
map, differential, and relations have been applied.
\end{theorem}

\begin{proof}
The planar binary trees with \(n\) leaves are counted by
\[
 C_{n-1}=\frac1n\binom{2n-2}{n-1}.
\]
Brown's motivic multiple-zeta algebra has Hilbert series
\((1-t^2-t^3)^{-1}\), giving
\[
 (d_7,d_8,d_9,d_{10},d_{11},d_{12})=(3,4,5,7,9,12).
\]
Multiplication by the six Catalan numbers gives the displayed values.
\end{proof}

\begin{remark}[Yetter--Drinfeld and pentagon complexes]
\label{rem:YD-delta-filter-scope}
\index{Humbert-Heegner!admissibility!scope}%
The Schauenburg bracket \(\delta^{(n)}\) is a cochain operation in a
Yetter--Drinfeld category.  The pentagon class \(\phi^{(n)}\) belongs
to an associator deformation complex.  A Humbert divisor belongs to
the cycle theory of \(\overline{\mathcal A}_2\).  A filter shared by
these objects is therefore a diagram of comparison maps.  Its data are
recorded below as
\[
 \mathsf H_{\mathrm{res}}+\mathsf H_{\mathrm{mot}}
 +\mathsf H_{\mathrm{Kum}}.
\]
Chapter~\ref{ch:shadow-tower-higher-coefficients} records the
labels~\ref{thm:phi-n-humbert-heegner-admissibility} and
Remark~\ref{rem:three-filter-composite-scope} as open comparison problems on
this diagram.
\end{remark}

% ============================================================
\section{Humbert cycles and the pentagon comparison}
\label{sec:humbert-heegner-admissibility-filter}

For a positive discriminant \(D\equiv0,1\pmod4\), the Humbert surface
\(H_D\subset\mathcal A_2\) is the locus of principally polarized
abelian surfaces admitting a symmetric endomorphism of discriminant
\(D\).  Its closure in a fixed toroidal compactification is an
algebraic divisor.  This classical geometry supplies the target cycle
classes.  The pentagon tower supplies a source only after its
chain-level construction has been fixed.

\begin{definition}[Humbert-cycle comparison package]
\label{def:humbert-heegner-admissible-cycle}
\index{Humbert--Heegner!admissible cycle|textbf}%
\ClaimStatusDefinitional
A Humbert-cycle comparison package for an ordered pentagon model
consists of:
\begin{enumerate}[label=\textup{(H\arabic*)}]
\item a toroidal compactification
 \(\overline{\mathcal A}_2^{\,\Sigma}\) and the closures
 \(\overline H_D\);
\item represented intersections
 \(Z_{\mathbf D}=\overline H_{D_1}\cap\cdots\cap\overline H_{D_k}\)
 in the Chow or cycle complex, with excess-intersection data;
\item an ordered residue package \(\mathsf H_{\mathrm{res}}\) for the
 pentagon deformation complex;
\item a filtered motivic comparison
 \(\mathsf H_{\mathrm{mot}}\) from represented pentagon classes to the
 cycle complex;
\item an integral comparison \(\mathsf H_{\mathrm{Kum}}\) compatible
 with discriminants, Fourier coefficients, and reduction modulo
 primes.
\end{enumerate}
A tuple \(\mathbf D\) is admissible for this package when each divisor
exists on the chosen compactification and the represented intersection
lies in the domain of all three comparison maps.
\end{definition}

\begin{proposition}[Typed Humbert support theorem]
\label{prop:humbert-heegner-admissibility-filter}
\index{Humbert--Heegner admissibility filter|textbf}%
\ClaimStatusConditional
\emph{Type signature:}
\textup{(Open/automorphic comparison, pentagon and cycle
presentations, Beilinson levels \(2\leftrightarrow5\),
\(\mathsf H_{\mathrm{res}}+\mathsf H_{\mathrm{mot}}
+\mathsf H_{\mathrm{Kum}}\)).}
Let \(\chi_{\mathrm{HH}}\) be the chain map supplied by the comparison
package.  For every represented pentagon class \(x\),
\[
 \operatorname{supp}\chi_{\mathrm{HH}}(x)
 \subseteq
 \bigcup_{\mathbf D\in\mathcal I(x)} Z_{\mathbf D},
 \qquad
 \mathcal I(x):=\{\mathbf D:[Z_{\mathbf D}]
 \text{ occurs in }\chi_{\mathrm{HH}}(x)\}.
\]
A congruence condition on the arity \(n\) follows when the comparison
map identifies its weight filtration with an explicitly computed
discriminant filtration.  A prime congruence follows when
\(\mathsf H_{\mathrm{Kum}}\) identifies the corresponding primitive
integral classes.
\end{proposition}

\begin{proof}
The support statement is the definition of the cycle-valued chain
map.  Compatibility with the weight and discriminant filtrations
transports an explicitly computed target condition to the source.
Compatibility of primitive integral lattices transports reduction
modulo primes.
\end{proof}

\begin{remark}[Cycle filtration]
\label{rem:humbert-heegner-manin-beilinson}
The codimension filtration on the cycle complex gives a natural target
for the arity filtration of the pentagon complex.  A Beilinson-style
interpretation further requires a regulator or height realization and
its compatibility with \(\chi_{\mathrm{HH}}\).
\end{remark}

% ============================================================
\begin{proposition}[Special-cycle boundary in genus \(g\ge3\)]
\label{thm:humbert-heegner-filter-g-geq-3}
\index{Humbert--Heegner admissibility filter!$g\ge 3$}%
\ClaimStatusProvedHere
Fix a compactification of \(\mathcal A_g\) and a specified family of
Cartier special divisors \(Z(T_i)\).  Then
\[
 \operatorname{codim}
 \bigl(Z(T_1)\cap\cdots\cap Z(T_k)\bigr)
 \le
 \min\!\left\{k,\frac{g(g+1)}2\right\}
\]
for every component of a proper intersection.  Its cycle class is
computed by the intersection product, including the excess bundle
when the intersection is non-transverse.

A transfer of the genus-two pentagon comparison to \(g\ge3\) is
defined after the ambient lattice, the special-divisor family, the
compactification, and the chain map from the source deformation
complex have been specified.
\end{proposition}

\begin{proof}
Each \(Z(T_i)\) is Cartier and therefore contributes at most one to
codimension.  The ambient dimension is
\(\dim\mathcal A_g=g(g+1)/2\).  Fulton's excess-intersection formula
gives the cycle class for a represented non-transverse intersection.
\end{proof}

\begin{conjecture}[Primitive special-cycle saturation]
\label{conj:primitive-humbert-heegner-saturation}
\ClaimStatusConjectured
For a specified lattice and compactification, there may exist families
of primitive special divisors whose independent intersection classes
saturate a linear window in \(g\).  A proof consists of explicit
divisor classes, their intersection matrix, and primitive
nonvanishing in the chosen Chow or cohomology theory.
\end{conjecture}

\begin{remark}[Shimura data as a cycle source]
\label{rem:shimura-cm-type-cycle-source}
CM types and special endomorphisms provide canonical sources of
special cycles.  Their discriminants, reflex fields, and Galois
orbits furnish the arithmetic data that a comparison package can
transport to a pentagon filtration.
\end{remark}

\begin{remark}[Motivic heights]
\label{rem:beilinson-motivic-height-cm}
A height pairing on homologically trivial special cycles gives a
motivic target beyond the Chow class.  Its use in the obstruction
tower requires a regulator-compatible lift of
\(\chi_{\mathrm{HH}}\).
\end{remark}

\begin{remark}[Dimension and independence]
\label{rem:manin-dichotomy-bounds}
Krull dimension bounds codimension.  Linear independence is computed
in the chosen Picard, Chow, or cohomology group.  These two statements
form complementary parts of a saturation proof.
\end{remark}

\begin{remark}[Catalan and motivic source dimensions]
\label{rem:YD-delta-general-rule}
The product \(C_{n-1}d_n\) is the cardinality of a formal indexing set
formed from planar trees and a chosen \(d_n\)-element motivic basis.
The rank of a Schauenburg correction is the rank of the represented
cochain after its differential, relations, and comparison map have
been applied.  Computing that rank is the exact all-arity
Yetter--Drinfeld problem.
\end{remark}
...
set $\{0, 8, 13, 250/3, 25/3\}$ has the Mukai-doubling-compatible
element $8$ exactly on the $\mathsf B$-row; the classical
$\mathsf G/\mathsf L/\mathsf C/\mathsf M$ subset
$\{0, 13, 250/3, 25/3\}$ excludes $8$, pinning the Vol~III
...
Among the five ceiling values $\{0, 8, 13, 250/3, 25/3\}$ of
...
\item $25/3$ (conditional): the Bershadsky--Polyakov row.
\end{itemize}
...
corresponding $c_+$-ratio satisfies
\begin{equation}
...
\begin{remark}[Relation with the Lusztig face; \ClaimStatusConditional]
\label{rem:monster-k3-cplus-lusztig-face-relation}
The identification of the ratio in
\eqref{eq:as-cplus-ratio-from-product} with the Lusztig face is
conditional on Theorem~\ref{thm:monster-k3-lusztig-cplus-face}.  The
product identity \eqref{eq:as-monster-k3-cplus-product} itself uses
only the stated Virasoro characteristic and lattice signatures.
\end{remark}
...
integer-valued representative in $\{0, 8, 13, 250/3, 25/3\}$ on the
...
% ============================================================
\section{Higher formal Yetter--Drinfeld coordinates}
\label{sec:YD-delta-n-13-16}

\begin{theorem}[Catalan--motivic counts in weights \(13\)--\(16\);
\ClaimStatusProvedHere]
\label{thm:YD-delta-13-16}
\index{Yetter-Drinfeld!Schauenburg bracket!formal coordinates}%
For the formal coordinate sets
\(\mathcal I_n=\mathcal T_n\times\mathcal B_n^{\mathrm{mot}}\),
\begin{equation}
\begin{aligned}
 n=13&:\quad 208012\cdot16=3\,328\,192,\\
 n=14&:\quad 742900\cdot21=15\,600\,900,\\
 n=15&:\quad 2\,674\,440\cdot28=74\,884\,320,\\
 n=16&:\quad 9\,694\,845\cdot37=358\,709\,265.
\end{aligned}
\label{eq:YD-delta-13-16-dim}
\end{equation}
These are cardinalities before the Yetter--Drinfeld differential and
comparison relations are imposed.
\end{theorem}

\begin{proof}
The Catalan formula gives
\[
 (C_{12},C_{13},C_{14},C_{15})
 =(208012,742900,2\,674\,440,9\,694\,845).
\]
The Hilbert series \((1-t^2-t^3)^{-1}\) gives
\[
 (d_{13},d_{14},d_{15},d_{16})=(16,21,28,37).
\]
Their products are the displayed integers.
\end{proof}
...
the GRT$_1$ action on categorical modular data is trivial, and no
proof even for this finite scalar $S$-matrix without the displayed
factorisation and factorwise invariance package.
\end{theorem}
...
$17$. The open open mathematical condition is to construct the level-$2$
