...
\end{itemize}
% RECTIFICATION-FLAG: The Borel--Laplace claim below applies to the
% leading-order ASYMPTOTIC coefficients S_r^{(0)} = (2/r)(-3)^{r-4}(2/c)^{r-2}
% for r >= 4, NOT to the exact shadow coefficients (S_2 = c/2, S_3 = 2
% are exact and do not match the formal series (-6/c)^r/r).
...
Borel--Laplace sum recovers~$-\log(1 + 6t/c)$.
\end{theorem}
...
\begin{equation}\label{eq:shadow-bps}
  ...
  = \sum_{r\ge1}
    \frac{(-6/c)^r}{r}\,t^r.
\end{equation}
% RECTIFICATION-FLAG: Z_BPS is the Borel resummation of the asymptotic
% shadow series for r >= 4.  The exact shadow generating function
% G(t) = sum S_r t^r = (c/2)t^2 + 2t^3 + S_4 t^4 + ... differs at low
% arities (S_2 = c/2, S_3 = 2 are not (−6/c)^r/r).
...
$S_r^{(0)} = \lambda_{\mathrm{eff}}^r / r$ for $r \ge 4$.
