...
% AP1: kappa(Heis) = k, kappa(Vir) = c/2, kappa(KM) = dim(g)(k+h^v)/(2h^v)
...
% AP1: kappa(Vir) = c/2 from landscape_census.tex; c=0 -> 0, c=13 -> 13/2 verified.
For affine Kac--Moody at level $k$:
$\kappa = \dim(\fg)(k + h^\vee)/(2h^\vee)$,
so $F_1 = \dim(\fg)(k + h^\vee)/(48 h^\vee)$.
% AP1: kappa(KM) = dim(g)(k+h^v)/(2h^v) from landscape_census.tex;
...
depth $1$, and \emph{not} a holomorphic modular form (AP15).
The arity-$2$ shadow amplitude at genus~$1$ is
...
anomaly in the shadow tower at higher arity.
Graph sums at arity $r$ produce degree-$r$ polynomials
in $E_2^*$, pushing the arithmetic into the full
quasi-modular ring.

The Leech lattice vertex algebra makes the arithmetic
structure explicit at a single arity.  Its shadow
