...
$\kappa$ is a rational number. For Heisenberg at level~$k$ it is
$k$; for Virasoro at central charge~$c$ it is $c/2$; for affine
Kac--Moody it is $\dim(\fg)(k + h^\vee)/(2 h^\vee)$. In each case
the value is rational, and in each case it appears as the
leading coefficient of a shadow tower whose higher coefficients
$S_r(\cA)$ are also rational. Does a rational number this central
to chiral duality carry arithmetic content?

The Leech lattice vertex algebra settles the question at a single
arity. Its shadow coefficient is $S_{12} = -65520/691$, where
$691$ is the numerator of the Bernoulli number $B_{12}/12$ and the
denominator of Ramanujan's $\tau$-function normalized at level~$1$.
The appearance of $691$ is not accidental: the Eisenstein
congruence $\tau(n) \equiv \sigma_{11}(n) \pmod{691}$ is the
arithmetic shadow of the same rational number seen on the chiral
side. This is the starting data of a broader phenomenon. The
shadow obstruction tower, classified by shadow depth into the hierarchy
...
eigenforms through its graph-sum amplitudes: each arity resolves
one eigenspace of the constrained Epstein zeta
$\varepsilon^c_s(\cA) = \sum_{\Delta \in \mathcal{S}}(2\Delta)^{-s}$,
which factors into $L$-functions. This is the arithmetic component
$d_{\mathrm{arith}}$ of shadow depth. A second component is purely
homotopy-theoretic: the $A_\infty$ non-formality depth
...
programme and the Sullivan programme. The discriminant, the
quadratic extension cut out by $\kappa$, and the $p$-adic locality
of the shadow ring of integers organize the arithmetic side below.
...
prefix $k$ enforces $r = 0$ at $k = 0$, AP126/AP141\textup{)}.
...
satisfies the $k = 0$ vanishing check of AP126/AP141.
...
for the Virasoro shadow tower at arity $r \geq 4$
...
enter the denominator pattern at any arity.  The shadow tower's
...
support.  The statement is uniform in arity: no arity threshold
...
corresponds to $S_3$, the cubic shadow \textup{(}arity-$3$
...
Remark~\textup{\ref{rem:calogero-moser-quartic}}: the arity-$4$
...
At arity~$2$, the shadow coefficient $S_2 = \kappa$ is the
only quadratic invariant of the bar complex, identifying it
with the kinetic term.  At arity~$3$, $S_3$ is the unique
...
arity-$2$ amplitude.  It does \emph{not} pass through any
Bernoulli-Dirichlet identification: the earlier formulation
% AP74: The false Bernoulli-Dirichlet identity equated the shadow L-function
...
$P(\tau) = -E_2^*(\tau)/12$, and the arity-$2$ shadow
...
For the arity-$r$ graph amplitude, let
...
arity-$r$ formula is stated as an asymptotic
...
earlier AP74 error (a Bernoulli ratio series equated with
...
$\kappa\,E_2^*(\tau)$ of the arity-$2$ amplitude is
...
= -\zeta(s)\,\zeta(s-1)$
(the identity proscribed by AP74) fails precisely this
