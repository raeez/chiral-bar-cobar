...
membership is certified only after the quartic/contact obstruction
and completed tower data are included.
...
(Zagier--Brown Padovan recurrence $d_n=d_{n-2}+d_{n-3}$ with
$(d_0,d_1,d_2)=(1,0,1)$, equivalently
$(d_3,d_4,d_5)=(1,1,2)$, on the full motivic MZV dimension sequence).
The Borcherds weight satisfies
...
Brown's motivic MZV dimension theorem gives $d_{10}=7$, equivalently
the coefficient of $x^{10}$ in $(1-x^2-x^3)^{-1}$ is~$7$; product
...
\ClaimStatusConditional
Assume the genus-$2$ Heegner transversality, admissible-emptiness,
pentagon-admissibility, and bar--cobar depth-reduction theorems.
...
For $g\ge 3$, the Humbert--Heegner result proved in
...
Part~\textup{(c)} records the missing open mathematical conditions. The previous
...
open mathematical condition, not as an unconditional consequence.
\end{remark}
