...
component} $d_{\mathrm{alg}}$ measures the $A_\infty$ non-formality of
bar cohomology: for Virasoro minimal
models, $d_{\mathrm{arith}}$ is finite but $d_{\mathrm{alg}} = \infty$
(the tower detects the \emph{algebra
itself} via the self-referential OPE $T \in T_{(1)}T$, not its
...
For lattice vertex algebras, the single Maurer--Cartan element $\Theta_\cA$
connects to both the Langlands programme
(arithmetic) and the Sullivan programme (homotopy theory).
...
This chapter treats the complementary regime where the tower does
...
because the Hecke decomposition terminates; the substantive content
is the shadow--spectral correspondence
...
(Corollary~\ref{cor:unconditional-lattice}) follows a new
...
The five verifications expose an intrinsic deficiency of the scalar shadow obstruction tower.
...
admits resolution at three levels of the MC~hierarchy, each strictly refining
the previous.
