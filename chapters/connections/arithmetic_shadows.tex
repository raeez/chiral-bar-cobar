...
\;\geq\; 0,
\end{equation}
a sum of \emph{central $L$-values at $s = 1/2$}
--- on the critical line, not at
$\operatorname{Re}(s) > 1$.
\item
...
\item Prime-locality holds: the shadow coefficients
  ...
  each prime~$p$.
\end{enumerate}
Moreover, \textup{(i)--(iii)} are conditionally equivalent to\textup{:}
...
\item \textup{(Decisive test)}
  The Leech lattice B\"ocherer coefficient
  $B_f(D)$ for the weight-$22$ eigenform is computable from
  the known inner-product distribution of minimal vectors
  ($93{,}150$ orthogonal pairs per root).
  A nonzero result for $B_f(-3)$ or $B_f(-4)$ would be the
  first instance of MC data determining a central $L$-value.
\end{enumerate}
\end{remark}
