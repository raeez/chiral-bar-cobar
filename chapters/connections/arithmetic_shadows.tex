...
and the shadow Postnikov tower organizes the algebraic data that,
...
The scalar shadow tower assigns $\kappa = 24$ and
...
intrinsic to the scalar shadow tower.
...
the scalar shadow tower is completely blind to the root
...
$24$ lattices.
The full scalar shadow tower $(S_r)_{r \geq 2} = (24, 0, 0, \ldots)$
is universal.
...
\item $\Theta_\Lambda^{(g)}(\Omega)
  ...
  arithmetic.
\end{enumerate}
In particular, the scalar shadow tower controls
...
hence $\kappa = c/2 = 12$, the same value as \emph{all}
$24$~Niemeier lattice VOAs.  But $V^\natural$ is class~M
...
This is the first known pair of holomorphic $c = 24$ vertex
algebras with identical $\kappa$ but different shadow classes:
all Niemeier lattice VOAs are class~G ($d_{\mathrm{alg}} = 0$),
while $V^\natural$ is class~M ($d_{\mathrm{alg}} = \infty$).
The $\bZ/2\bZ$-orbifold $V_{\mathrm{Leech}} \to V^\natural$
kills the weight-$1$ currents and promotes the shadow class
from G to~M, while preserving $\kappa = 12$.  The scalar
shadow tower is completely blind to this transition.
\end{remark}
...
% ============================================================
\section{Non-perturbative and $p$-adic arithmetic of the shadow tower}%
...
\subsection{The $p$-adic shadow tower}%
...
the shadow tower to the Iwasawa-theoretic framework.
\end{remark}
...
The shadow tower invariants of a chirally Koszul algebra~$\cA$
...
are activated, and the shadow tower coefficients
...
The entire shadow tower $\{S_r\}_{r \geq 2}$ is determined by
...
\item \emph{Hypothesis~\textup{(a)}} is satisfied in all
  ...
  is compatible with the shadow tower, which is automatic
...
\emph{representation theory} of~$\cA$.  The shadow tower, the
...
recursion-constrained shadow tower.
\end{definition}
...
\item \emph{Heisenberg:}
  ...
  arities~$r \ge 5$.  The Heisenberg shadow tower terminates at
...
\item \emph{Virasoro:}
  ...
  $\tau$-independence of the shadow tower on the scalar lane.
...
\begin{proof}
Part~(i): the Heisenberg shadow tower is
...
a stronger invariant than the scalar shadow tower and
...
which in turn depend on the full shadow tower
...
\item The MC recursion does determine $S_r$ from lower-arity
  ...
  theorem: the shadow tower is algebraic of degree~$2$,
...
The shadow tower satisfies a strong algebraic constraint that
...
\item Riccati algebraicity: the shadow tower is finitely
