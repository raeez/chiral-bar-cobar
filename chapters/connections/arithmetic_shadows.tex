...
$\Delta_{10}:=\Phi_{10}^{\mathrm{un}}=\Delta_5^2=\chi_{10} \in S_{10}(\mathrm{Sp}(4, \bZ))$, which
...
coefficient $S^{(p)}_{10}(\cA)$ on the unnormalised
$\Phi_{10}^{\mathrm{un}}=\Delta_5^2$ family has a
...
\subsection{The complete proof table}
...
\begin{remark}[Hypotheses~\textup{(a)} and~\textup{(b)}]%
...
complementarity record $\kappa + \kappa^! \in \{0, 8, 13, 250/3,
...
form of weight $5$; hence $\Phi_{10}^{\mathrm{un}}=\Delta_5^2$ is preserved
