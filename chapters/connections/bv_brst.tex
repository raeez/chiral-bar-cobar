...
No regularization is needed: both terms are residues of logarithmic
forms along normal-crossing divisors of the smooth
variety~$\overline{C}_{n+1}(X)$, and the Poincar\'e residue map
$\Res_{D_{ij}}\colon \Omega^k_{\log}(\overline{C}_{n+1})
\to \Omega^{k-1}_{D_{ij}}$ is a well-defined morphism of sheaves
(no distributional extension required).  The apparent
singularity $1/(z_i - z_k)$ at $z_i = z_k$ is a pole along
the \emph{different} divisor $D_{ik}$, which is transverse
to~$D_{ij}$ by the normal-crossing property of the FM
compactification (Theorem~\ref{thm:FM}(3)).
