...
The bar complex $\barB^{\mathrm{ch}}(\cA)$ is algebraic:
its differential collects OPE residues along collision divisors in
the Fulton--MacPherson compactification of the configuration space.
The BV--BRST complex of $\cA$ is physical: it encodes the
fields, antifields, and ghosts governing the quantization of
a two-dimensional conformal field theory.
This chapter asks whether these two objects coincide.

At genus~$0$, the answer is yes: the geometric bar complex and the BV complex are
isomorphic (Theorem~\ref{thm:bv-bar-geometric}).
...
The identification holds for conformal vertex algebras on
...
At genus~$g \geq 1$, the general identification remains conjectural
(Conjecture~\ref{conj:master-bv-brst}), but the Heisenberg case is
resolved at all genera at the scalar level: the BV free energy and
the bar free energy coincide,
...
What is proved at all genera for general algebras
...
\item the scalar genus-$1$ obstruction is the modular characteristic
  ...
  coefficient.
\end{enumerate}
The chain-level identifications involving antifields versus
...
line a one-loop anomaly equation; higher lines encode
...
collects connected $g$-loop diagrams.  The modular MC hierarchy reads heuristically as an all-loop Ward identity.  The connected
...
the additional content isolated as conditional later in this
chapter.
\end{remark}
...
\begin{proof}
The proof has two sides and a bridge.

\medskip
\noindent\emph{Bar side.}
...
\medskip
\noindent\emph{Bridge: Grothendieck--Riemann--Roch.}
...
\medskip
Combining: the BV free energy at genus~$g$ is
$F_g^{\mathrm{BV}}(\cH_\kappa)
= \kappa \cdot \lambda_g^{\mathrm{FP}}
= F_g^{\mathrm{bar}}(\cH_\kappa)$.
\end{proof}

\begin{remark}[Four independent verification paths]
\label{rem:heisenberg-bv-bar-four-paths}
\index{multi-path verification!Heisenberg BV/bar}
The identity~\eqref{eq:bv-bar-heisenberg} is confirmed by
four independent computations:
\begin{enumerate}[label=\textup{(\alph*)},nosep]
\item \emph{Zeta-regularization + Quillen anomaly}
  \textup{(}Steps~1--4 above\textup{)}.
\item \emph{Selberg zeta function.}
  The D'Hoker--Phong determinant formula
  $\det'_\zeta\Delta_{\Sigma_g}
  = Z_{\mathrm{Sel}}(1;\Sigma_g) \cdot e^{c_g}$
  \textup{(}Theorem~\textup{\ref{thm:thqg-I-determinant-formula}}\textup{)}
  reduces the partition function to the Selberg zeta factor
  $Z_g(\cH_\kappa) = (\det\operatorname{Im}\Omega)^{-\kappa/2}
  \cdot Z_{\mathrm{Sel}}(1)^{-\kappa/2} \cdot e^{-\kappa c_g/2}$
  \textup{(}Corollary~\textup{\ref{cor:thqg-I-heisenberg-selberg}}\textup{)},
  whose moduli integration produces
  $\kappa\cdot\lambda_g^{\mathrm{FP}}$.
\item \emph{Direct GRR.}
  The family index of $\bar\partial^{\oplus\kappa}$ on the universal
  curve produces the $\hat{A}$-genus generating function, whose
  extraction at degree~$2g$ gives
  $\lambda_g^{\mathrm{FP}}$
  \textup{(}\S\textup{\ref{subsec:why-ahat}}\textup{)}.
\item \emph{Numerical.}
  At each genus $g = 1, \ldots, 15$, the Bernoulli formula,
  the $\hat{A}$-series extraction, and an independent
  implementation all produce the same rational number.
\end{enumerate}
\end{remark}

\begin{remark}[Scope: scalar level vs.\ chain level]
...
Conjecture~\ref{conj:master-bv-brst} at the \emph{scalar level}
\textup{(}partition function\slash free energy\textup{)} for the
Heisenberg family at all genera.  What remains open, even for
free fields, is:
