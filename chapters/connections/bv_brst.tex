...
The governing question of this chapter is the mathematical role of \emph{BV-BRST formalism and Gaiotto's perspective}: how this bridge interfaces with the proved modular Koszul core without blurring theorematic content and programme.
\end{remark}
...
In the language of the universal Maurer--Cartan class
(Theorem~\ref{thm:explicit-theta}), this $F_1$ is the scalar
trace of the genus-$1$ component:
$F_1 = \operatorname{tr}(\Theta_{\mathrm{Vir}_c}^{(1)})
= \kappa(\mathrm{Vir}_c) \cdot \lambda_1 = (c/2) \cdot (1/12)
= c/24$.  The full $\Theta_{\mathrm{Vir}_c}$ assembles these
partition functions across all genera into a single
Maurer--Cartan element in the cyclic deformation complex.
\end{remark}
...
(Theorem~\ref{thm:explicit-theta}) organizes both the
genus-$0$ curvature $m_0$ and the higher-genus
obstructions into a single object: at anomaly-free
levels ($c = 26$ for the bosonic string, $c = 0$ for
the free theory), $\Theta_\cA$ vanishes; away from
these, $\Theta_\cA = \kappa \cdot \eta \otimes \Lambda$
curves the bar complex at every genus simultaneously.
\end{remark}
...
\begin{remark}[BRST nilpotence as the logarithmic condition]
\label{rem:brst-nilpotence-periodicity}
\index{nilpotence-periodicity correspondence!BRST instance}
The nilpotence condition $Q_{\mathrm{BRST}}^2 = 0$ is the physical
manifestation of the nilpotence-periodicity correspondence
(Remark~\ref{rem:nilpotence-periodicity}).  The BRST charge is the
\emph{logarithmic} (nilpotent, infinitesimal) datum of gauge symmetry:
it encodes the infinitesimal gauge transformations.  The physical state
space $H^{\infty/2+\bullet}(\cA)$ carries the \emph{periodic} (global)
structure: modular invariance, factorization over moduli, string
amplitudes.  The condition $c = 26$ (or more generally
$\kappa_{\mathrm{tot}} = 0$) is the condition that the logarithm is
\emph{single-valued}: the categorical logarithm (bar construction) has
trivial monodromy, and the periodic structure of physical states is
faithfully encoded by the nilpotent BRST data.  When
$\kappa_{\mathrm{tot}} \neq 0$, the BRST charge fails nilpotence
by $Q^2 = \frac{c-26}{12}\,c_0$---the curvature of the
logarithm---and the anomaly $(c-26)/12$ is the infinitesimal
generator of the monodromy obstruction.
\end{remark}
...
\begin{theorem}[Bar complex = semi-infinite complex for $\mathcal{W}$-algebras;
\ClaimStatusProvedHere]
...
\begin{corollary}[Anomaly complementarity for $\mathcal{W}$-algebra pairs;
\ClaimStatusProvedHere]
...
genus-$g$ quantum corrections sum to
  ...
  further to the four free channels
  $c_{334}$, $c_{444}$,
...
\item The obstruction coefficient $\kappa$ IS the semi-infinite
  anomaly;
\item The complementarity relation in $\kappa$ IS the semi-infinite
  anomaly relation: exact cancellation in affine/free-field cases,
  and a level-independent same-family budget for Virasoro and
  principal finite-type $\mathcal{W}$-families;
...
beyond the scope of this monograph.  Within the repo's frontier order,
the principal finite-type $W_N$ algebraic package is already theorematic;
the live task here is the gauge-theoretic comparison itself, and any
filtered $W_\infty$ / Yangian target-building remains upstream to MC4.
