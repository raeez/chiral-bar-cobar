...
\textup{(}AP137 bosonic branch\textup{)}:
\[
...
pattern $F_g = \kappa \cdot \lambda_g^{\mathrm{FP}}$ of Theorem~D.
...
% AP1: from landscape_census.tex; k=0 -> r=0 verified (AP126/AP141)
...
$|s^{-1}v| = |v| - 1$.  Then for every arity $n \geq 3$
...
propagates from the binary comparison to every finite arity $n \geq 3$.
\end{proof}
...
genus $g \geq 0$ and every arity $n \geq 1$, the renormalized
BV-BRST complex of the genus-$g$ WZW model with $n$ marked points is
filtered quasi-isomorphic to the genus-$g$, arity-$n$ bar complex of
