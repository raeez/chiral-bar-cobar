...
The BV-BRST formalism is the physical refoundation of deformation
theory.  Its central message is that deformations, anomalies, line
defects, and higher operations naturally package into homotopical
algebra---and that strict dg structures cannot, in general, suffice.
The reason is structural: once BRST anomaly brackets are known to
satisfy quadratic identities (the master equation $\{S,S\} = 0$ and its
quantum lift $\hbar\,\Delta\,S + \tfrac{1}{2}\{S,S\} = 0$), the
cancellation pattern among ghosts, antighosts, and auxiliary fields
forces an infinite hierarchy of coherence data that is exactly an
$A_\infty$- or $L_\infty$-structure.  Truncating to strict
differentials loses the anomaly information.

For chiral algebras on curves, the BV complex is the bar complex in
disguise.  The fields of the theory are the generators, the antifields
are the desuspended cogenerators, the BV bracket is the Poincar\'e
residue on the FM compactification, and the classical master equation
$\{S,S\} = 0$ is the statement $d^2_{\mathrm{bar}} = 0$ at genus~$0$.
At higher genus, the quantum master equation acquires a BV Laplacian
term $\hbar\,\Delta\,S$ that is precisely the curvature
$\kappa(\cA)$---the modular characteristic of Part~I.
Gaiotto's programme~\cite{CDG23} makes this identification the
starting point of a physical construction: begin with a twisted
holomorphic-topological field theory, extract its boundary chiral
algebra, and read the BV complex as the bar complex whose duality
theory governs the defect categories.

This chapter develops the BV--bar identification in detail, shows that
the BV bracket is a residue operation on the log-de~Rham complex of the
configuration space, and uses the resulting dictionary to translate
between physical anomaly cancellation and the algebraic obstruction
theory of Part~I.
...
factor $\tfrac{1}{2}$ in QME && curvature $m_0 = \kappa \cdot \mathbf{1}$; $\kappa = c/2$ absorbs the $\tfrac{1}{2}$
\end{tabular}
\end{center}
...
The holomorphic (semi-infinite) BRST identification is proved: bar $=$ holomorphic BRST for KM and W-algebras (Theorems~\ref{thm:bar-semi-infinite-km}, \ref{thm:bar-semi-infinite-w}), and bar $\simeq$ string BRST at $c = 26$ (Theorem~\ref{thm:brst-bar-genus0}).  The topological BRST conjecture is stronger: it claims the bar complex also incorporates diffeomorphism ghosts from $\mathrm{Diff}(X)$.  The key difficulty is that $\mathrm{Diff}(X)$ is infinite-dimensional, so neither PBW filtration nor the semi-infinite approach directly applies.
...
The theorem synthesizes: Arkhipov~\cite{Arkhipov97} (classical bar--CE comparison $\Phi_0$), the Eilenberg--Moore comparison theorem (Weibel~\cite[Theorem~5.2.12]{Weibel94}), the conformal weight filtration (Frenkel--Ben-Zvi~\cite{FBZ04}), and our geometric realization of $\barB^{\mathrm{ch}}_0(\cA_{\mathrm{tot}})$ on $\overline{C}_n(\mathbb{P}^1)$ (Theorem~\ref{thm:bar-cobar-isomorphism-main}).  The contribution is the PBW filtration lift of $\Phi_0$ to the chiral setting; $c = 26$ ensures $d_{\mathrm{bar}}^2 = 0$.
\end{remark}
...
The theorem synthesizes Arkhipov~\cite{Arkhipov97} (classical bar--CE and semi-infinite duality for $\mathbb{Z}$-graded algebras), Frenkel--Garland--Zuckerman~\cite{FGZ86} (semi-infinite cohomology), and our PBW spectral sequence (Theorems~\ref{thm:pbw-koszulness-criterion}, \ref{thm:km-chiral-koszul}).  The contribution is the lift from $\mathbb{Z}$-graded associative to chiral algebras via PBW, using the geometric bar complex realization (Theorem~\ref{thm:bar-cobar-isomorphism-main}).
\end{remark}
...
The physical content of this section---Gaiotto's framework,
boundary chiral algebras from holomorphic Chern--Simons, the HT
boundary condition, W-algebras from Higgs branches, and the
BV-physics dictionary---has been moved to Volume~II, where it is
developed alongside the Swiss-cheese operad and the
holomorphic--topological factorization on
$\mathbb{C} \times \mathbb{R}$.

The algebraic core---the BV algebra structure on the bar complex
from configuration-space geometry---remains in this volume
