...
The BV--BRST formalism and the bar-cobar adjunction suggest a common
convolution-algebra template, but the exact theorematic scope must be
kept narrow.  At genus~$0$, the external geometric comparison of
Theorem~\ref{thm:bv-bar-geometric} identifies the BV complex with the
geometric bar complex.  Within the present manuscript, the higher-genus
content that is actually proved is the scalar curvature/anomaly package
encoded by the universal MC class~$\Theta_\cA$ and the coefficient
$\kappa(\cA)$.  A full chain-level identification of BV Laplacian,
quantum master equation, and modular bar curvature remains heuristic in
this chapter.

\begin{theorem}[BV/bar dictionary; \ClaimStatusHeuristic]
...
\item the geometric BV complex and the geometric bar complex are
  ...
  elsewhere\textup{)};
\item the classical master-equation slogan should be read as the
...
\item the scalar genus-$1$ obstruction is the modular characteristic
  $\kappa(\cA)$, which plays the role of a one-loop anomaly
  coefficient.
\end{enumerate}
...
BRST versus bar cohomology are heuristic guideposts rather than proved
...
\end{alignat*}
This should be read as a heuristic BV translation of the proved
modular MC hierarchy rather than as a proved BV identity.  The
genus-$0$ line suggests a classical master equation; the genus-$1$
line suggests a one-loop anomaly equation; higher lines suggest
nonlinear modular interference terms.  At the scalar level, the trace
of~\eqref{eq:modular-qme-bv} recovers the genus tower
$F_g = \kappa(\cA)\lambda_g$ \textup{(}Theorem~D\textup{)}.  At the
...
\begin{theorem}[Heuristic QME/MC comparison; \ClaimStatusHeuristic]
...
\begin{theorem}[Heuristic genus-\texorpdfstring{$0$}{0} amplitude/bar comparison;
\ClaimStatusHeuristic]\label{thm:genus0-amplitude-bar}
...
\]
is often read as parallel to the bar-complex Euler characteristic
...
At genus~$1$, the $\hat{A}$-genus gives $F_1 = \kappa/24$.  For $\cA = \mathrm{Vir}_c$ with $\kappa = c/2$: $F_1 = c/48$.
\end{remark}
...
Theorem~\ref{thm:central-charge-complementarity}: the principal finite-type
