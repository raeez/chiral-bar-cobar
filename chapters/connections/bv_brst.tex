...
\label{rem:bv-brst-wave14-anchors}
...
\label{rem:bvbrst-DNA-koszul-conductor}
...
\label{prop:bvbrst-wave15-2loop-obstruction}
...
$5 = \chi(\cO_{K3}) + 3 = 2 + 3$ of Remark~\ref{rem:bvbrst-DNA-paramodular-anomaly}:
...
\label{rem:bvbrst-wave15-theta-graph}
...
\]
which is the vanishing of Proposition~\ref{prop:bvbrst-wave15-2loop-obstruction}
...
\label{rem:bvbrst-wave15-scope}
Proposition~\ref{prop:bvbrst-wave15-2loop-obstruction} is a statement
...
\label{rem:bvbrst-wave15-two-hbar}
...
\label{rem:bvbrst-wave15-partition-match}
The $2$-loop contribution to the BV partition function on
$T^2\times K3$ computed by Proposition~\ref{prop:bvbrst-wave15-2loop-obstruction}
...
Remark~\ref{rem:bvbrst-wave15-theta-graph} supplies exactly this
...
Wave~18]
\label{prop:bvbrst-wave18-cech-cocycle}
...
\label{prop:bvbrst-wave18-banding-counterterm}
...
Proposition~\textup{\ref{prop:bvbrst-wave18-cech-cocycle}} corresponds
to a \emph{zero-loop} BV counterterm in the twisted $11$D SUGRA quantum
action $S_q$ of Proposition~\textup{\ref{prop:bvbrst-wave15-2loop-obstruction},}
of the form
\[
...
Proposition~\ref{prop:bvbrst-wave15-2loop-obstruction}: the
...
\label{rem:bvbrst-wave18-linking-number}
...
Proposition~\ref{prop:bvbrst-wave18-cech-cocycle}: on the nerve of the
...
\label{rem:bvbrst-wave18-two-hbar-banding}
\index{two-hbar bridge!Wave 18}
The pre-existing two-$\hbar$ bridge
\textup{(}Remark~\ref{rem:bvbrst-wave15-two-hbar}\textup{)} pins
...
\textup{(}Proposition~\ref{prop:bvbrst-wave15-2loop-obstruction};
...
\textup{(}Proposition~\ref{prop:bvbrst-wave15-2loop-obstruction}\textup{),}
...
\textup{(}Remark~\ref{rem:bvbrst-wave15-two-hbar};
Remark~\ref{rem:bvbrst-wave18-two-hbar-banding}\textup{),} where the
root-of-unity phase truncates the tower to its $\mu_8$-banded part.
\end{proof}

\begin{remark}[Four-loop theta-product graph and $c_4 = +12$]
\label{rem:bvbrst-heegner-c4-feynman}
\index{4-loop BV!K3 theta-product graph}
\index{Feynman graph!c_4 = 12}
The leading four-loop contribution to $c_4$ is carried by the
theta-product graph $\Gamma_{\theta^{\otimes 2}}$ (two disjoint theta
subgraphs joined by two $K3$-propagators along the $\bC^2$
transverse direction) in the graph enumeration of Costello--Gwilliam
\cite[Ch.~10, Vol.~1]{CostelloGwilliam2017}. The combinatorial
contribution reads
\[
 \mathrm{Feyn}^{(4)}(\Gamma_{\theta^{\otimes 2}})
 \;=\; \tfrac{1}{2!\,2!}\cdot\chi(K3)^{\otimes 2}\cdot
 \int_{K3\times\bC^2}\!\eta^{-48}\,\phi_{10,4}
 \;=\; c_{\phi_{-2,1}}(4)\cdot [H_4]
 \;=\; 12\cdot [H_4],
\]
where the combinatorial prefactor $\chi(K3)^2/(2!\,2!) = 576/4 = 144$
factors against the $\phi_{10,4}/\eta^{48}$ integrand's $-12/144$
coefficient as read off from the Gritsenko--Nikulin expansion of
$\Phi_{10}$. The eleven other four-loop topologies
(wheel~$W_4$, double-theta $\Theta^2$, ladder $L_4$, box~$B_4$,
two-triangle~$\Delta^2$, dumbbell-tadpole, pretzel, Z-graph,
sunrise-square, figure-eight-loop, tadpole-loop) sum pairwise
via the Gritsenko--Nikulin denominator
identity~\cite[Thm.~2.1]{GritsenkoNikulin1998} to zero at $p^4$. Cross
verification: Oberdieck--Pixton $2016$ Donaldson--Thomas invariants on
$K3\times E$~\cite{OberdieckPixton2017} match $c_{\phi_{-2,1}}(4) = 12$
via the DT/GW correspondence and the reduced Gromov--Witten potential
of~K3.
\end{remark}
