...
\noindent\emph{Governing question: which parts of the BV-BRST dictionary
are theorematic consequences of the bar-cobar machine, and which remain
programme-level physics input?  In this chapter, chain-level BV/bar
compatibilities are proved where tagged \ClaimStatusProvedHere{} (or
\ClaimStatusProvedElsewhere{} with citation), while global
open-closed/anomaly-flatness identifications are retained as
\ClaimStatusConjectured{} and feed MC5.}

\begin{remark}[Semantic levels; Convention~\ref{conv:hms-levels}]
The BV-bar chain map and the quantum master equation are
\textbf{M-level} identifications (explicit dg isomorphisms between
bar and BRST complexes).  The conjectured extensions---anomaly
cancellation as flatness of $\Theta_{\cA}$, gauge fixing as Verdier
duality on the Ran space---are \textbf{H-level} programme items
(MC5).
\end{remark}
...
\noindent\emph{Scope for this chapter.} This section records only the
BV/BRST interfaces used later; the full 4d-to-2d bridge is developed in
Chapter~\ref{ch:ht-boundary}.  Statements below are tagged to separate
proved structural input from physics-level conjectural extensions.
...
\noindent\emph{Status split.} The 4d gauge-theory origin of the
W-algebra package is \ClaimStatusConjectured, while central-charge
identities used downstream are \ClaimStatusProvedElsewhere.
...
\begin{remark}[Scope]
This conjecture is labeled \ClaimStatusConjectured{} because the derivation
...
\noindent\emph{Status split.} This section separates proved BRST
cohomology statements \textup{(}\ClaimStatusProvedElsewhere\textup{)}
from the still-heuristic BV integration dictionary
\textup{(}\ClaimStatusHeuristic\textup{)}.
