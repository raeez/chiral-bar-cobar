...
agrees with the programme characteristic only in lineages where the
...
$A$, distinguishing it from the cubic-Casimir anomaly of $5$d hCS and
forbidding the slogan ``$6$d hCS $=$ $3$d CS in disguise'' (MA-9).
...
Theorem~\ref{thm:log-form-ghost-law} should extend to a filtered
...
$z_i-z_j$ to the logarithmic form~$\eta_{ij}$. It is expected to be a
quasi-isomorphism only on the nilpotent genus-$0$ relative locus and
only after the PBW lifting hypotheses of
...
Theorem~\ref{thm:bv-bar-coderived-vol1} is the Swiss-cheese instance of the
...
raises the anomaly degree by one and forbids the slogan ``$6$d hCS $=$
$3$d CS in disguise'' (MA-9): $3$d Chern--Simons carries the cubic
...
vertex algebra~\cite{Costello2021hCS}, and the master critique
discipline that identifies this slogan as a forbidden form.
...
\]
in the Gritsenko--Nikulin normalisation; it is not obtained in this
volume by an additive identity
...
the Borcherds weight $5$; it is the graph-side proof obligation for the
...
proof obligation of Remark~\ref{rem:bvbrst-theta-graph} is to produce
...
is expected to define a $\mu_8$-gerbe $\mathcal{G}$ on
...
part is expected to be the gerbe banding obstruction
...
chain-level proof obligation: on the nerve of the
...
complete unprojected BV graph proof is a separate proof obligation.
\end{remark}
...
renormalisation formalism is expected to compute the same
Heegner-projected coefficient as a sum of graph integrals over
$K3\times\bC^2$; this is precisely the comparison-map input.
...
five-loop vacuum topologies, the expected topology count is
...
the local graph-encoding proof obligation.
\end{proof}
...
symmetry, and pairing data are proof obligations for a graph-encoding
...
are expected to agree on the bulk--boundary
