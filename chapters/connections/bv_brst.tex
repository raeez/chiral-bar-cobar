...
\textup{(}Theorem~\ref{thm:bv-bar-coderived-vol1}\textup{)} is our extension,
...
\begin{proposition}[Koszul duality preserves BRST anomaly cancellation; \ClaimStatusConditional]
...
follows from Corollary~\ref{cor:anomaly-physical-genus0-vol1} applied to
$\cA_{\mathrm{tot}}^{\vee}$.
\end{proof}
...
The analysis pushes one order further: the $\hbar^2$ piece of the quantum
master equation. The central output is a vanishing
obstruction-cohomology class, which we prove in two independent ways
and then match against the Fourier expansion of
$(\Phi_{10}^{\mathrm{un}})^{-1}$.

\begin{proposition}[Two-loop BV obstruction is exact]
...
Etingof classification (Theorem~$\mathrm{E}1$; Gerstenhaber
...
classical BRST algebra. By the Etingof classification
theorem, the only
...
(the one-loop $\chi(\cO_{K3}) = 2$ and $24$-node residue sum
...
identification of the Igusa form as the classification
invariant in $H^2(\mathfrak{m}_{K3})$ (Etingof) allows a
...
$\Phi_{10}/\eta^{24}$ associator twist). The two-loop analysis
...
order-$8$ torsion. The chain-level witness
...
banding $F_U \in H^2(U,\mu_8)$ admits the explicit \v Cech
representative $F_{ijk}\colon U_{ijk}\to\mu_8$ given by
\[
...
Faltings--Chai; hence $[F_U] = 0$. Alternatively: the
...
\begin{proposition}[Banding as BV zero-loop counterterm]
...
The \v Cech $2$-cocycle $F_{ijk}$ of
...
anomaly or the two-loop obstruction-vanishing.
\end{proposition}
...
of $F_U$ is the linking-number formula
\[
 F_U(x_0, x_1, x_2) \;=\;
...
chain-level witness upgrades this: the denominator $8$ is literally the
...
The preceding analysis established an order-by-order
...
via the one-loop paramodular anomaly, $c_2 = 0$ proved by
Feynman $24-24$ cancellation, and $c_3 = 176256
\cdot [H_3]$ computed via the $p^3$-order of $\log(\Phi_{10}/\eta^{24})$
...
\begin{equation}\label{eq:bvbrst-5loop-sum}
 c_5 \;=\; \sum_{\Gamma\in\text{Table~\ref{tab:bvbrst-5loop-enum}}}
...
Propositions~\ref{prop:bvbrst-4loop-pair-a}--\ref{prop:bvbrst-4loop-pair-c}
extended to 5-loop:
...
The vanishing $c_5 = 0$ of Theorem~\ref{thm:bvbrst-5loop-sum}
...
$[\log(\Phi_{10}^{\mathrm{un}}/\eta^{24})]_{p^5} = 0$ by direct Fourier
...
The Vol~II companion of Theorem~\ref{thm:bvbrst-5loop-sum} is
recorded at \texttt{vol2/chapters/connections/ht\_bulk\_boundary\_line.tex}
...
the $\hbar$-series is the asymptotic expansion. The identity is isolated
and stated as a theorem at four sharpness
...
$Z_{\mathrm{bulk}}\cdot Z_{\mathrm{boundary}} =
(\Phi_{10}^{\mathrm{un}})^{1/\hbar - 1}$ that collapses at
$\hbar = 1$ to an exact holographic duality.
...
regime the BV Borel resummation addresses is regime~(b),
...
\begin{equation}\label{eq:bvbrst-allloop-effective}
 ...
 \hbar\log\!\bigl(\Phi_{10}^{\mathrm{un}}(Z)/\eta(q)^{24}\bigr).
\end{equation}
...
(\Phi_{10}^{\mathrm{un}})^{1/\hbar}|_{\hbar = 1}
= \Phi_{10}^{\mathrm{un}}$ at the Lusztig locus.
