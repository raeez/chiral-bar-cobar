...
\noindent\emph{The bar complex of the Heisenberg algebra (Chapter~\ref{ch:heisenberg-frame}) is the BV complex of the free boson.  The curvature $d^2 = k \cdot \omega_1$ is the conformal anomaly.  This chapter establishes the BV-bar identification for all chiral algebras.}
...
The preceding sections established a structural dictionary between the BV-BRST formalism
and the geometric bar complex.  We now construct an explicit chain map at genus~$0$,
proving that the BRST complex of a chiral algebra is quasi-isomorphic to its
geometric bar complex on~$\mathbb{P}^1$.
...
$d^2 = \kappa \cdot \omega_1$ & Conformal anomaly \\
...
and the bar side has $d^2 = \kappa_{\mathrm{tot}} \cdot \omega_1
...
The genus-$0$ BRST-bar quasi-isomorphism
(Theorem~\ref{thm:brst-bar-genus0}) requires the anomaly cancellation
condition $c = 26$: the total system (matter $\otimes$ ghosts) must be
anomaly-free for both complexes to have square-zero differentials.  We
now prove a stronger result for affine Kac--Moody algebras at
\emph{arbitrary} level, identifying the chiral bar complex with
semi-infinite cohomology.  This is intrinsic to the algebra ---
no coupling to a ghost system is needed --- and holds at every
non-critical level.
...
\begin{itemize}
\item $\mathrm{gr}\,\barB^{\mathrm{ch}}(\widehat{\fg}_k)$ reduces
  to the classical bar complex $\barB(\fg)$ of the
  finite-dimensional Lie algebra $\fg$
  (Theorem~\ref{thm:brst-bar-genus0}, Step~2: only the
  leading OPE pole survives, giving the Lie bracket of~$\fg$).
\item $\mathrm{gr}\,C^{\infty/2+\bullet}(\widehat{\fg}_k, V_k)$
  reduces to the Chevalley--Eilenberg complex
  $C^*_{\mathrm{CE}}(\fg, \mathbb{C})$
  (the semi-infinite vacuum collapses to the ordinary exterior
  algebra $\bigwedge^*\!\fg^*$ when all mode corrections are dropped;
...
The bar--semi-infinite identification extends from Kac--Moody algebras
to all W-algebras obtained by Drinfeld--Sokolov reduction.  The key
input is that DS reduction, being itself a BRST construction, commutes
with the bar functor up to quasi-isomorphism.
