...
The question is whether they coincide.
...
At genus~$g \geq 1$, the identification is resolved in the
coderived category $D^{\mathrm{co}}$ for all four shadow classes
(Theorem~\ref{thm:bv-bar-coderived}): the BV and bar complexes
are quasi-isomorphic in $D^{\mathrm{co}}(\cA)$ for every chirally
Koszul algebra.  At the chain level, the identification holds for
classes~$\mathsf{G}$, $\mathsf{L}$, and~$\mathsf{C}$ but fails
for class~$\mathsf{M}$
(Conjecture~\ref{conj:master-bv-brst}; the classical-quantum bridge of Theorem~\ref{thm:kz-classical-quantum-bridge} provides the genus-$0$ comparison for classes $\mathsf{G}$ and~$\mathsf{L}$).
...
\item \textbf{Moduli dependence.}
  ...
  determinant.
\item \textbf{Higher-arity coupling through the harmonic propagator.}
  ...
  \item \emph{Class~L} \textup{(}affine Kac--Moody\textup{)}:
    ...
    At genus~$1$, obstruction~\textup{(3)} is killed.
  \item \emph{Class~C}
    \textup{(}$\beta\gamma$\textup{)}: \textbf{resolved}.
    Although the quartic interaction vertex is nonzero
    \textup{(}shadow depth $r_{\max} = 4$\textup{)},
    the harmonic-propagator correction vanishes by
    a three-mechanism argument
    \textup{(}Remark~\textup{\ref{rem:bv-bar-class-c-proof}}\textup{)}.
  \item \emph{Class~M}
    \textup{(}Virasoro, $\cW_N$\textup{)}:
    genuinely open.  The quartic and higher
    interaction vertices produce nontrivial harmonic-propagator
    corrections that do not cancel by any known identity.
  \end{itemize}
\end{enumerate}
\end{proposition}
...
Obstruction~(3): for class~G, the claim is vacuous (shadow depth
$r_{\max} = 2$, no interaction vertices).  For class~L
(affine Kac--Moody at non-critical level $k \neq -h^\vee$),
the interaction vertex is the structure-constant tensor
$f^{abc}$.  The cubic harmonic correction has the form
$\sum_c f^{abc}\cdot I_{\mathrm{harm}}(z_1,z_2)\cdot f^{cde}$,
where $I_{\mathrm{harm}}$ is the integral of the harmonic
propagator against the cubic vertex measure.
Antisymmetry of $f^{abc}$ and the Jacobi identity
$f^{abc}f^{cde} + \text{cyclic} = 0$ ensure that this trace
vanishes.  Since class~L has $r_{\max} = 3$, no quartic or
higher vertices appear, so obstruction~(3) is resolved at all
arities for affine Kac--Moody at genus~$1$.
...
The same mechanism kills all higher-arity obstructions:
$\delta_r \propto m_0^{\lfloor r/2 \rfloor - 1}$, which lies in
$\operatorname{Im}(d)$ in $D^{\mathrm{co}}$ at every arity.
...
and the Quillen anomaly formula.  Obstruction~(3) (higher-arity
coupling through the harmonic propagator) produces, at arity~$r$,
a discrepancy
\[
  \delta_r^{\mathrm{harm}}
...
element~$x$.  Each arity-$r$ obstruction is a multiple of a positive
power of~$m_0$:
\[
  \delta_r^{\mathrm{harm}}
  \;=\; c_r(\cA) \cdot m_0^{\lfloor r/2 \rfloor - 1}
...
arity-$r$ graph produces a factor of $P_{\mathrm{harm}}$.
The curvature $d^2 = m_0 = \kappa(\cA) \cdot \omega_g$ arises from
exactly this harmonic piece via the Quillen anomaly, so every
arity-$r$ discrepancy is a polynomial in~$m_0$ with the stated
...
$P_{\mathrm{bar}} = d\log E(z,w)$.  The discrepancy at arity~$r$
