...
The governing question of this chapter is why the Heisenberg algebra is the minimal commutative/modular atom of the theory, and how much of the full modular bar-cobar mechanism it already displays in computable form.
\end{remark}

\begin{remark}[H/M/S route; Convention~\ref{conv:hms-levels}]
This frame chapter is mixed H/M/S. Heisenberg is computed explicitly at M-level, its scalar and spectral outputs are read at S-level, and only the printed comparison theorems are used as H-level input.
\end{remark}
\begin{remark}[The first example]\label{rem:frame-entry-role}
One should begin with the smallest object in which the geometry is
already complete.  For modular Koszul duality on curves, that object is
the Heisenberg algebra.  The bar differential can be written down
explicitly, Verdier duality is visible without auxiliary generators,
the genus-$1$ defect is scalar, and the genus tower closes to the
$\hat{A}$-series.  What Heisenberg cannot see is ordered
factorization: there is no nontrivial braid reversal and no $R$-matrix
to invert.  That omission is structural.  The Yangian supplies the
first genuinely braided example on the $\Eone$ side.
\end{remark}
...
precisely because its bar cohomology is concentrated in
degrees $0$ and $2$, so the bar complex is formal.

This is Theorem~A (Theorem~\ref{thm:bar-cobar-isomorphism-main})
for the simplest non-trivial case.  The general theorem
(which decomposes into three layers
$\mathrm{A}_0/\mathrm{A}_1/\mathrm{A}_2$; see
Remark~\ref{rem:theorem-A-decomposition}) establishes
