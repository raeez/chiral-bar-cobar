...
\noindent\emph{Governing question.} Can one algebra already expose the
entire route of the monograph --- strict bar-cobar duality, modular
curvature, and complementarity --- while keeping every computation
explicit?  This frame chapter answers yes and fixes the reference model
to which all later chapters are compared.

\begin{remark}[Semantic levels; Convention~\ref{conv:hms-levels}]
\label{rem:frame-hms}
This chapter operates at M-level (explicit bar complex computation)
with S-level shadows (generating functions, obstruction scalars).
The proved content is the scalar modular package
(items~(1)--(3) of Definition~\ref{def:frame-modular-package});
the conjectural H-level completion
(item~(4), universal Maurer--Cartan class $\Theta_{\mathcal{H}_k}$)
is previewed but not constructed.
\end{remark}
...
tower, the complementarity --- assembles into a modular package
with a proved scalar/spectral layer and a conjectural full
completion.
...
the following proved and conjectural data:
\begin{enumerate}
...
\item The \emph{conjectural universal Maurer--Cartan class}
...
For the Heisenberg algebra, the \emph{scalar} modular characteristic
package is fully determined by the single number $\kappa = k$.  This
is an artifact of the abelian (rank-$1$) structure.  In
Chapters~\ref{chap:kac-moody-koszul}
and~\ref{chap:w-algebras}, the
package acquires non-trivial spectral discriminant and richer ambient
deformation complex.

\begin{remark}[Proved versus conjectural]%
\label{rem:frame-kappa-not-fundamental}
Items~(1)--(3) of Definition~\ref{def:frame-modular-package} are
proved in this monograph: the scalar package
...
\item[\textbf{DK}.]
  \emph{Chain-level / evaluation-locus derived Drinfeld--Kohno.}
