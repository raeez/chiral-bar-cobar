...
duality.  At genus~$0$, it specializes to classical operadic Koszul
duality.  At genus $g \geq 1$, the propagator acquires period
corrections from the Hodge bundle over $\overline{\mathcal{M}}_g$,
the bar differential develops curvature, and the genus tower organizes
into a modular deformation whose generating function is the
$\hat{A}$-genus.
...
hierarchy with three distinct statuses: a proved scalar package, a
proved ambient/spectral shadow, and a conjectural full homotopy
completion.

\begin{definition}[Characteristic hierarchy for Heisenberg]
\label{def:frame-modular-package}
The Heisenberg frame exhibits the following proved and conjectural
characteristic data:
\begin{enumerate}
...
\item The \emph{conjectural full homotopy completion}
  ...
  \right)
  \end{equation}
  whose scalar trace would recover items~(1)--(3).
