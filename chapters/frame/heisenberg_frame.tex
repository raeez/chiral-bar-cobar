...
The computations in this chapter instantiate all four main theorems
for~$\mathcal{H}_k$ before the general theory of Part~I:
\begin{enumerate}
\item[\textbf{A.}] \emph{Duality}
  (\S\ref{sec:frame-koszul-dual}):
  $\barB(\mathcal{H}_k) \simeq
  \mathrm{coLie}^{\mathrm{ch}}(V^*)$;\quad
  $\mathcal{H}_k^! =
  \bigl(\mathrm{Sym}^{\mathrm{ch}}(V^*),\; m_0 = -k\omega\bigr)$.
\item[\textbf{B.}] \emph{Inversion}
  (\S\ref{sec:frame-inversion}):
  $\Omega\barB(\mathcal{H}_k) \xrightarrow{\sim}
  \mathcal{H}_k$ (formality: $H^*$ in degrees $0, 2$).
\item[\textbf{C.}] \emph{Complementarity}
  (\S\ref{sec:frame-complementarity}):
  $Q_g(\mathcal{H}_k) \oplus Q_g(\mathcal{H}_k^!) =
  H^*(\overline{\mathcal{M}}_g, \mathcal{Z}_{\mathcal{H}_k})$.
\item[\textbf{D.}] \emph{Characteristic}
  (\S\ref{sec:frame-genus-tower}):
  $\kappa(\mathcal{H}_k) = k$;\quad
...
In conformal field theory, the curvature $\dfib^{\,2} = k \cdot \omega_1$
...
Definition~\ref{def:frame-modular-package} is a hierarchy, not a flat
theorematic package.  Items~(1)--(3) are proved in this monograph: the
scalar package
(Theorem~\ref{thm:modular-characteristic}), the complementarity
complex (Theorem~\ref{thm:quantum-complementarity-main}), and the
spectral discriminant (Theorem~\ref{thm:spectral-characteristic}).
Item~(4), the universal Maurer--Cartan class
$\Theta_{\mathcal{A}}$, is now constructed
(Theorem~\ref{thm:universal-theta},
...
charge $c/2$ for Virasoro, and
$(k+h^\vee)\dim(\mathfrak{g})/(2h^\vee)$ for affine Kac--Moody ---
is the first characteristic number of the anticipated
$\Theta_{\mathcal{A}}$, not the fundamental object itself.
\end{remark}
...
The free fermion participates in three distinct dualities, which must
not be conflated:
\begin{enumerate}
\item \emph{Koszul duality}: $\mathcal{F}^! = \beta\gamma$.  This is
  the chiral bar-cobar duality, proved via acyclicity of
  $\barB(\mathcal{F})$.
\item \emph{Bosonization}: the fermion $\psi(z)$ can be written as
  $:\!e^{i\phi(z)}\!:$ for a free boson $\phi$.  This is a
  representation-theoretic equivalence, not a Koszul duality.
\item \emph{Level-shifting}: the Heisenberg algebra at level~$k$
  and level~$-k$ are related by $\alpha \mapsto -\alpha$.  This is
  an automorphism, not a duality.
\end{enumerate}
The bc system (conformal weights $(\lambda, 1{-}\lambda)$) and the
$\beta\gamma$ system (weights $(1,0)$) form a two-generator Koszul
pair with $\dim V = 2$, realizing $\chirLie \leftrightarrow \chirCom$
