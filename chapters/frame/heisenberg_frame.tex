...
Dualize.  The graded linear dual of a cofree cocommutative
coalgebra on~$V^*$ is the free commutative algebra
on~$V^*$ (this is the algebra-coalgebra duality
underlying $\mathrm{Com}^! = \mathrm{Lie}$):
...
\end{equation}
This is the Koszul dual \emph{algebra}, obtained by dualizing
the bar coalgebra --- not by applying the cobar functor, which
would instead recover~$\mathcal{H}_k$ itself
...
is an instance of a general pattern: the bar complex
$\bar{B}(\cA)$ of any augmented chiral algebra~$\cA$ is a
coalgebra --- the Koszul dual \emph{coalgebra}.  The Koszul dual
\emph{algebra}~$\cA^!$ is obtained by dualizing this coalgebra
(Verdier duality on $\operatorname{Ran}(X)$; see
Theorem~\ref{thm:bar-cobar-verdier}).  The cobar of the bar
recovers~$\cA$ itself: $\Omega(\bar{B}(\cA)) \simeq \cA$
on the Koszul locus (Theorem~\ref{thm:bar-cobar-inversion-qi}).
The general bar-cobar adjunction is developed in
Chapter~\ref{chap:bar-cobar}.

\begin{remark}[Three different ``dualities'']
\label{rem:frame-three-dualities}
Three distinct operations must not be confused: (1)~Koszul duality $\mathcal{H}_k \leftrightarrow \mathrm{Sym}^{\mathrm{ch}}(V^*)$, where the bar coalgebra $\bar{B}(\mathcal{H}_k)$ is dualized to produce the Koszul dual algebra (this chapter); (2)~level-shifting $\mathrm{Rep}(\mathcal{H}_k) \simeq \mathrm{Rep}(\mathcal{H}_{-k})$; (3)~boson-fermion correspondence $\mathrm{Rep}(\mathcal{H}_k) \simeq \mathrm{Rep}(\mathcal{F}^{\otimes 2})$.  Only~(1) is the subject of this monograph.
\end{remark}
...
The Heisenberg algebra has a single current with a \emph{double} pole.
The free fermion has a single current with a \emph{simple} pole:
\begin{equation}\label{eq:frame-fermion-ope}
\psi(z)\,\psi(w) = \frac{1}{z-w} + \text{regular}.
\end{equation}
No double pole, no curvature component: the bar differential
$d = d_{\mathrm{bracket}}$ is entirely bracket-type.
At degree~$1$, it extracts the simple-pole residue
$\operatorname{Res}_{z=w} \psi(z)\psi(w) = 1$,
which is a scalar.  At degree~$2$, the Arnold relation gives
$d^2 = 0$ exactly as for Heisenberg, but now the mechanism is the
antisymmetry of the wedge product $\psi \wedge \psi$
rather than the vanishing of a bracket.
...
Acyclicity of the bar complex is the strongest possible form of
the Koszul property.  Compare with Heisenberg, where
$H^2(\barB(\mathcal{H}_k)) = \mathbb{C} \cdot c_k \neq 0$
(Theorem~\ref{thm:frame-heisenberg-bar}): the double-pole curvature
obstructs acyclicity.  The free fermion, having no double pole,
has no such obstruction.

\medskip
\noindent\textbf{The Koszul dual: the $\beta\gamma$ system.}
The Koszul dual of the free fermion is the $\beta\gamma$ system,
a chiral algebra with two generators $\beta(z)$ of weight~$1$ and
$\gamma(z)$ of weight~$0$, with OPE
\begin{equation}\label{eq:frame-betagamma-ope}
\beta(z)\,\gamma(w) = \frac{1}{(z-w)^2} + \text{regular},
\qquad
\gamma(z)\,\beta(w) = \frac{-1}{(z-w)^2} + \text{regular}.
\end{equation}

The $\beta\gamma$ system is a commutative chiral algebra (its OPE has
double poles only, no simple pole); the free fermion is a Lie-type
chiral algebra (simple pole only).  The Koszul duality
$\mathcal{F}^! \cong \beta\gamma$
(Theorem~\ref{thm:betagamma-fermion-koszul})
is the chiral incarnation of the classical operadic duality
$\mathrm{Lie}^! = \mathrm{Com}$:
\begin{center}
\small
\begin{tabular}{c|c|c|c}
& \textbf{Heisenberg} $\mathcal{H}_k$ &
...
The free field example reveals a clean structural dichotomy.  The
bar differential of any chiral algebra decomposes as
$d = d_{\mathrm{bracket}} + d_{\mathrm{residue}}$: the bracket
component from simple poles and the curvature component from double
poles.  Heisenberg has $d_{\mathrm{bracket}} = 0$; the free fermion
has $d_{\mathrm{residue}} = 0$.  The general case --- affine
Kac--Moody algebras --- has both, and neither is individually a
differential.
