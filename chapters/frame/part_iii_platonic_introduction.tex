...
binary collision kernel, the scalar conductor, and the shadow
...
The resulting partition is the finite-atlas quotient that the chiral
bar demands. Part~\ref{part:standard-landscape} is the catalogue of
charts on the moduli stack
...
\begin{description}
 \item[\S III.0 Structural introduction.]
 \label{sec:part-iii-0}
 Central-charge deficiency. The eight-section atlas and the five
 carrier theorems replace scalar classification by the full
fingerprint.
 ...
 as subregular DS reduction (hook-type, conditional Verdier companion
 $\mathcal B^{-k-6}$, central-charge conductor
 $K^c = 50$); Gaiotto--Rapcak $Y_{N_1, N_2, N_3}[\psi]$
...
\subsection{Theorem C: collision kernel from PBW and OPE}
\label{sec:part-iii-theorem-C}

\begin{theorem}[The collision kernel is determined by PBW and OPE
data; represented $r$-matrix under $H_{\mathrm{CYBE}}^{\mathrm{rep}}$;
\ClaimStatusConditional]
...
basis on the augmentation ideal. The binary ordered collision kernel
\begin{equation}
 K_\cA^{\mathrm{coll}}(z)
 \;:=\; \Res^{\mathrm{coll}}_{0,2}(\Theta_\cA^{E_1})
 \;\in\; \End_\cA(2)\,\widehat\otimes\,\cO(*\Delta)
\end{equation}
is determined by the chosen PBW presentation and the OPE operation.
Its standard-row formulas are:
\begin{enumerate}[label=\textup{(\roman*)}, leftmargin=*]
 \item Heisenberg (class~$G$):
 $K_\cA^{\mathrm{coll}}(z)=\kappa\Omega_{\mathcal H}/z$.
 \item Affine $V_k(\fg)$ (class~$L$, trace-form convention):
 $K_\cA^{\mathrm{coll}}(z)=k\Omega_{\mathrm{tr}}/z$, where
 $\Omega_{\mathrm{tr}}\in\fg\otimes\fg$ is the trace-form Casimir.
 \item Virasoro (class~$M$):
 $K_\cA^{\mathrm{coll}}(z)=(c/2)/z^3+2T/z$, cubic and
 simple pole (the descendant coefficient $T(w)/(z-w)^2$ is absorbed
 by the $d\log$ kernel into the simple-pole term).
 \item $\cW_N$ (class~$M$):
 $K_\cA^{\mathrm{coll}}(z)=\sum_{n=2}^{N}K^{(n)}(z)$
 with $K^{(n)}(z)=\kappa^{(n)}/z^{2n-1}+\text{(descendant
 corrections)}$, one component per stress tensor.
 \item $\beta\gamma,bc$ (class~$C$): logarithmic absorption produces
 a regular binary operation, and the first scalar contact coordinate
 is $S_4$, obtained by arity-four residue/trace extraction.
\end{enumerate}
The homotopy coinvariant $q_2(K_\cA^{\mathrm{coll}})$ remains an
operator class in $\End_\cA(2)_{h\Sigma_2}$. A chosen normalized
functional
$\tau_{\cA,2}^{\mathrm{res}}\colon
\End_\cA(2)_{h\Sigma_2}\to\bC$
produces its scalar coordinate. A specified representation
$\rho_\cA\colon\End_\cA(2)\to\End(W^{\otimes2})$ together with
$H_{\mathrm{CYBE}}^{\mathrm{rep}}(\cA;W)$ produces the classical
$r$-matrix $r_\cA(z)=\rho_\cA(K_\cA^{\mathrm{coll}}(z))$.
\end{theorem}
\begin{proof}
The kernel $K_\cA^{\mathrm{coll}}
=\Res^{\mathrm{coll}}_{0,2}(\Theta_\cA^{E_1})$
is computed from the arity-two truncation of the ordered MC element
\textup{(}Theorem~\ref{thm:mc2-bar-intrinsic}\textup{)}. The
...
\begin{itemize}[leftmargin=*]
 \item Class~$G$: the double OPE pole yields a simple collision pole
 with level coefficient.
 \item Class~$L$: the Casimir channel of the double pole yields the
 trace-form simple-pole kernel. Its normalized scalar coordinate is
 $\tau_{\cA,2}^{\mathrm{res}}q_2(K_\cA^{\mathrm{coll}})$; adjoining
 the Sugawara contribution $\dim\fg/2$ gives
 $\kappa_{\mathrm{ch}}$ in the affine normalization of this volume.
 ...
 \item Class~$C$: logarithmic absorption sends the simple generator
 pole to a regular binary operation.  For $\beta\gamma$, the quartic
 contact class gives $S_4=-5/12$ and $r_{\max}=4$.
\end{itemize}
Applying $q_2$ gives the equivariant operator class; applying
$\tau_{\cA,2}^{\mathrm{res}}$ gives its scalar coordinate; applying
$\rho_\cA$ under $H_{\mathrm{CYBE}}^{\mathrm{rep}}$ gives the
represented classical $r$-matrix. $\quad\square$
\end{proof}
...
$\Lambda_{\mathrm{Muk},II}^{2,1}$.  Its compact Hall--Drinfeld
recognition target \(\mathbf H_{\Delta_5}^{\mathrm{tgt}}\) has Weyl
denominator \(\Delta_5 \in S_5(K(1))\) (Gritsenko paramodular
weight-$5$ cusp form).  The notation
\(\mathbf H_{\Delta_5}\) is reserved for the recognized compact object
carrying a finite Hall source, Hopf pairing, source-to-target comparison
maps, a PBW presentation theorem, and a Mittag--Leffler inverse-limit
passage.
Four sibling functors index the chart:
\begin{description}
 \item[$\Psi$] reflective interior, producing the generic target
 comparison points;
...
\begin{theorem}[K3 determinant comparison problem on the atlas;
 \ClaimStatusOpen]
...
$H_{\Delta_5}^{\mathrm{det}}$ consist of the genus-$1$ determinant
...
divisor class.  These maps form the construction package for an
actual deformation of the fingerprint coset slot.
\end{remark}
...
$\Psi_{\mathrm{coset}}$ specializes, the collision kernel is read from
the PBW/OPE carrier, its classical $r$-matrix is supplied on the
$H_{\mathrm{CYBE}}^{\mathrm{rep}}$ comparison surface, and the
associator comparison action relates the
...
The five carrier theorem surfaces
...
\hline
\ref{thm:part-iii-rmatrix-emergent-from-pbw}
 (collision kernel from PBW and OPE)
...
collision kernel, and Koszul conductor are explicit functionals of
...
charge, collision kernel, and scalar conductor are read by the typed
functionals above.
\end{proposition}
\begin{proof}
Composition of
...
(collision-kernel functional and represented-CYBE comparison),
Theorem~\ref{thm:part-iii-grt-orbit-structure}
...
coset-dependent functional of the fingerprint, the collision kernel is
read from the PBW and OPE data, and the classical $r$-matrix is its
represented CYBE realization.
