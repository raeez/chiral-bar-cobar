...
The atlas of $\cM_{\ChirAlg}$ carries a distinguished chart
...
$\Lambda_{\mathrm{Muk}}^{2,1}_{II}$; its chiral bialgebra realisation
...
Chapter~\ref{ch:k3-chiral-bialgebra-platonic}: the Humbert
...
rule out $L'(0,\Delta_{10},\mathrm{ad}^0)$. Cross-reference
...
The paramodular chart of $\cM_{\ChirAlg}$ at
$\mathbf H_{\Delta_5}$ has a one-dimensional arithmetic tangent:
$\dim H^1_f(\mathbb Q,\mathrm{std}\,\rho_{\Delta_5}) = 1$, the
cyclotomic-paramodular Hida family. Under the
$\GRT(\Q)$-action on the atlas, this Selmer line is the
$\Q$-rational tangent along which the fingerprint coset slot
varies. The one-dimensional Bloch--Kato input is the arithmetic
shadow of Theorem~\ref{thm:part-iii-atlas-completeness}'s
non-degeneracy hypothesis.
\end{remark}

\begin{remark}[Humbert--Heegner admissibility on the atlas]
\label{rem:part-iii-humbert-heegner-atlas}
The pentagon tower $\{\phi^{(n)}\}_{n\ge 3}$ on the K3 chart of
$\cM_{\ChirAlg}$ satisfies
$\phi^{(n)} = 0$ unless $n\equiv 3,5\pmod 8$, equivalently
$D_n = (n-3)/2\in\{0,1\}\pmod 4$, with first admissible nonvanishing
rung $\phi^{(5)} = -2\,[\mathrm{gen}]^{\otimes 5}$. The admissibility
filtration is the Heegner-divisor reading of the shadow-tower
vanishing pattern on the paramodular chart, bypassing
