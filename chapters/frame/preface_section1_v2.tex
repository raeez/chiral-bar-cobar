...
\item \emph{Same pair.}\;
$d_{\mathrm{res}}^{(ij)}\,d_{\mathrm{res}}^{(ij)} = 0$ because the
first Poincar\'e residue removes the unique logarithmic normal factor
along \(D_{ij}\).  After restriction to that divisor there is no second
normal direction for the same diagonal; the desuspension sign only
records the ordered-bar orientation.
\end{enumerate}
...
The ordered bar is the level-$2$ chain object before averaging. The symmetric bar is a
...
genus-zero representative, not a global coordinate difference. The
Arnold relation is the local cocycle condition for that representative.

The cobar functor $\Omega$ inverts the bar on the Koszul locus:
$\Omega(\barB(\cA))\xrightarrow{\sim}\cA$ (Theorem~B).
Thus bar--cobar reconstruction recovers the original algebra.

A chosen quadratic presentation $\cA=T_X(V)/(R)$ produces
$\cA^{\mathrm i}=C_X(s^{-1}V,s^{-2}R)$ and
$q_\cA\colon\cA^{\mathrm i}\to B_X(\cA)$.
Graded linear duality gives the strict algebra
$\cA^!:=(\cA^{\mathrm i})^\vee$. Verdier duality on
$\mathrm{Ran}(X)$ produces its chain-level homotopy model directly:
\begin{equation}\label{eq:pf1-verdier}
K_X(\cA):=\mathbb D_{\mathrm{Ran}}\barB_X(\cA),
\qquad
...
Here $K_X(\cA)$ is a factorization algebra retaining the full
chain-level data.  Theorem~A makes~$\nu_\cA$ an equivalence on the
chosen finite-type Koszul-pair locus.
...
\mathbb D_{\mathrm{Ran}}(B(\cA))
 &=& K_X(\cA)
 & \text{(Verdier duality: the homotopy dual algebra)},\\[3pt]
\mathbb D_{\mathrm{Ran}}(\cA^{\mathrm i}) &=& \cA^!
 & \text{(quadratic finite duality: the strict dual)}.
\end{array}
\]
Their source and target categories distinguish the three operations.
...
$\Sigma_2$-invariant. Averaging loses no noncommutative Casimir data,
and its degree-two trace is the modular characteristic
$\kappa(\mathcal H_k)=k$.
...
kernel $k\Omega_{\mathcal H}/z$. The averaging map collapses the Casimir to a
scalar:
\begin{equation}\label{eq:pf1-km-kappa}
\mathrm{av}\bigl(k\Omega_{\mathrm{tr}}/z\bigr)
\;=\;
...
$r(z) = k\Omega_{\mathrm{tr}}/z$ to $\kappa(\widehat{\mathfrak g}_k)$ discards
...
The ordered story is the primitive; the symmetric / modular-Koszul story (T15 sense (iii)) is its shadow.
...
Everything above takes place on the genus-$0$ affine/formal screen:
there the logarithmic normal form is represented by
\(\eta_{12}=d\log(z_1-z_2)\), and the Arnold relation holds in that
local coordinate. On \(\mathbb P^1\) the representative is glued with
the projective linear relations; on a curve of genus \(g \geq 1\), the
function \(z_1-z_2\) has no global meaning: the curve is compact, and
any meromorphic function with a single simple zero must also have a
simple pole.
