...
cochain complexes over $\overline{\cM}_g$.  Its native deformation
class lies in $H^2(\Def_g)$.  Under $H_D^K$, the K-theoretic and
signed Hodge outputs are
\[
...
=\kappa\lambda_1$ holds for every family.  For multi-weight
...
\subsection*{Feigin--Frenkel centre and the critical-level reflection}
...
\]
is the scalar level transform visible on the affine Koszul lane; it
is not the Feigin--Frenkel centre theorem and not, by itself, an
identification of the chiral Koszul dual object. At the level of
modular characteristics,
$\kappa(\widehat{\fg}_k)+
\kappa(\widehat{\fg}_{-k-2h^\vee})=0$:
complementarity is a direct consequence of this reflection. The
actual Feigin--Frenkel theorem enters at the fixed critical level,
where the centre is the oper algebra for the Langlands dual group.
The chiral Koszul dual still passes through the Verdier-dual bar and
the chiral CE construction, not through a bare relabelling of~$k$.
...
the modular MC element is the $\Sigma_n$-homotopy-coinvariant shadow
of the ordered one. At degree~$2$ it retains
$q_2(K_\cA^{\mathrm{coll}})$ as an operator class. The normalized
functional gives
$\tau_{\cA,2}^{\mathrm{res}}q_2(K_\cA^{\mathrm{coll}})=\kappa(\cA)$
in abelian and scalar families; on the non-abelian affine row it gives
$\kappa_{\mathrm{dp}}$, and the Sugawara contribution
$\dim(\fg)/2$ completes $\kappa(V_k(\fg))$.
...
compactification, the signed residue-pushforward image of
~$\Theta_\cA$ factors as a tensor product over vertices of the dual
graph:
\[
\Delta^{\log}_{\mathrm{pf},\rho}(\Theta_\cA)
\;=\;
\bigotimes_{v\in V(S_\rho)}\Theta_{\cA,v}
\]
Here
$\Delta^{\log}_{\mathrm{pf},\rho}
=(\kappa_\rho)_*(\operatorname{pr}_\rho)_*i_\rho^!$ includes the
Gysin residue, proper pushforward, determinant-line sign, and
finite-automorphism normalisation of the signed log-FM package.
...
\subsection*{4.7.\enspace Arthur parametrisation of the K3-BKM
chiral bialgebra}

The modular characteristic $\kappa(\cA) \cdot \lambda_g$ of the
preceding sections indexes the obstruction cohomology of the generic
bar–cobar framework. On the K3 sub-landscape the obstruction carries
additional arithmetic structure: the automorphic representation of the
Siegel cusp form $\Delta_{10} \in S_{10}(\mathrm{Sp}_4(\mathbb{Z}))$
attaches an Arthur $A$-parameter to the BKM chiral bialgebra
$\mathbf{H}_{\Delta_5}$ (Vol~III
Chapter~\ref{V3-ch:k3-chiral-bialgebra-platonic}), and the preceding
Hodge-bundle geometry lifts to a global local--global compatibility
statement.

\begin{description}
\item[Arthur packet.] The Siegel form $\Delta_{10}$ is a genuine
cuspidal Hecke eigenform; its Arthur global $A$-parameter, in the
sense of Arthur~(2013) classified via endoscopic transfer from
$\mathrm{Sp}_4$ to $\mathrm{GL}_5$, is
\[
 \psi_{\Delta_{10}}
 \;=\;
 ...
 {}^L\mathrm{Sp}_4,
\]
where $\phi_{\Delta E_6}\colon L_F \to \mathrm{GL}_2(\mathbb{C})$ is
the cuspidal Langlands parameter of the unique weight-$18$ Hecke cusp
form $\Delta E_6 = \Delta\cdot E_6 \in S_{18}(\mathrm{SL}_2
(\mathbb{Z}))$, and $\mathrm{Sym}^1$ is the $2$-dimensional standard
representation of the Arthur $\mathrm{SL}_2$. This Arthur parameter is
non-tempered: the $\mathrm{SL}_2$-factor is non-trivial, indicating the
Saito--Kurokawa CAP origin of $\Delta_{10}$ via Piatetski-Shapiro
1983 (Klingen-parabolic residual construction).

\item[Spinor $L$-function factorisation (Andrianov 1987).]
\[
 L(s,\Delta_{10},\mathrm{Spin})
 \;=\;
 L(s,\Delta E_6)
 \cdot \zeta(s-9) \cdot \zeta(s-8).
\]
The degree-$4$ spinor $L$ of $\Delta_{10}$ factors as degree-$2$
(from the cuspidal $\Delta E_6$) times two degree-$1$ shifted Riemann
zetas, matching the non-tempered SK shape of $\psi_{\Delta_{10}}$.

\item[Spin refinement to $\Delta_5$.] $\Delta_5^2 = \Delta_{10}$
(Gritsenko--Nikulin 1998) on $\mathrm{Sp}_4(\mathbb{Z})$; the
half-integral weight $5/2$ of $\Delta_5$ forces the Maass
$\mathbb{Z}/2$-spin cover
\[
 \Gamma^{\mathrm{spin}}_{\Delta_5}
 \;=\;
 \widehat{\mathrm{Sp}_4(\mathbb{Z})}^{v_{\Delta_5}},
\]
with cocycle $v_{\Delta_5}$ given by Maass 1964. On this cover,
$\Delta_5$ is a genuine cuspidal Hecke eigenform (not paramodular at
any integer level $N$; the earlier "paramodular $K(1)$" claim has been
retracted). The Langlands dual of
$\Gamma^{\mathrm{spin}}_{\Delta_5}$ is a double cover of
${}^L\mathrm{Sp}_4 = \mathrm{SO}_5$, namely $\mathrm{Spin}_5 \cong
\mathrm{Sp}_4$: the K3-BKM is Langlands self-dual in the sense that its
$L$-group equals the group from which the Arthur parameter lifts.

\item[Local--global compatibility.] On the $\mathcal{B}$-family the
obstruction class $\Theta_{\mathbf{H}_{\Delta_5}}$ globalises as a
restricted tensor product
\[
 \Theta_{\mathbf{H}_{\Delta_5}}
 \;=\;
 \bigotimes_{p}{}^{\mathrm{res}}\,
 \Theta_{\mathbf{H}_{\Delta_5,\,p}}
\]
of local factors, with each local $p$-adic factor $\mathbf{H}_{\Delta_5,
p}$ determined by (i) the Satake parameters $\alpha_p + \alpha_p^{-1}
= a_p(\Delta E_6)/p^{17/2}$ of $\Delta E_6$ at $p$, (ii) the spin sign
$\epsilon_p = v_{\Delta_5}(\mathrm{Frob}_p) \in \{\pm 1\}$, and (iii) a
$M_{24}$-orbifold character $\chi^{M_{24}}_p$ supported on the
anomalous primes $p \in \{7, 11, 23\}$ (CDH umbral moonshine).

\item[Three-path verification at $p = 2, 3, 7$.]
Hecke eigenvalues of $\Delta E_6$ at small primes
$(a_2, a_3, a_7) = (-528, -4284, +4057560)$ (LMFDB label
\texttt{18.1.a.a}) specialise to Satake parameter magnitudes
$(|\alpha_2 + \alpha_2^{-1}|, |\alpha_3 + \alpha_3^{-1}|, |\alpha_7 +
\alpha_7^{-1}|) \approx (1.031, 0.377, 0.266)$, all within the
Ramanujan bound $\leq 2$. The product $L_p(s, \Delta E_6)
\cdot \zeta_p(s-9) \cdot \zeta_p(s-8)$ matches the Andrianov spinor
factorisation for each prime $p$ unramified for $\Delta_{10}$.
\end{description}

\begin{theorem}[Arthur parametrisation of the K3 chiral bialgebra;
\ClaimStatusConditional]
\label{thm:pref-k3-arthur-parametrisation}
\index{Arthur parameter!K3 BKM}
\index{Langlands reciprocity!K3 BKM}
Under the Humbert--Lusztig--Mukai hypothesis package, the K3 chiral
bialgebra $\mathbf{H}_{\Delta_5}$ of Vol~III
Chapter~\ref{V3-ch:k3-chiral-bialgebra-platonic} globalises as a
restricted tensor product of local chiral quantum groups indexed by the
Arthur packet of $\Delta_{10}$:
\[
 \mathbf{H}_{\Delta_5}
 \;=\;
 \bigotimes_{p\leq\infty}{}^{\mathrm{res}}\,
 \mathbf{H}_{\Delta_5,\,p}\bigl(\rho_{\Delta E_6}|_{G_{\mathbb{Q}_p}},\;
 \epsilon_p,\; \chi^{M_{24}}_p\bigr),
\]
with $\rho_{\Delta E_6}$ the Deligne $\ell$-adic Galois representation
of $\Delta E_6$, $\epsilon_p = v_{\Delta_5}(\mathrm{Frob}_p)$ the
Maass-spin-cover sign, and $\chi^{M_{24}}_p$ the umbral orbifold
character (trivial outside $\{7, 11, 23\}$). At the archimedean place
$p = \infty$, $\mathbf{H}_{\Delta_5, \infty}$ is the Harish-Chandra
module of the Saito--Kurokawa archimedean packet refined by the
$\mathbb{Z}/2$-spin class, with Schmidt archimedean parameters
$(7/2, 5/2)$ (tempered; correcting an earlier $(17/2, 1/2)$ which
belongs to $\Delta_{10}$ not $\Delta_5$).
\end{theorem}

\begin{proof}[Proof (reference)]
The Arthur packet is Vol~III
Theorem~\ref{V3-thm:arthur-packet-H-delta5}. Its structural input is the
super-Etingof--Kazhdan classification
(Theorem~\ref{V3-thm:k3-structural-genesis}); its Humbert conductor is
Vol~III Theorem~\ref{V3-thm:humbert-order-K-kappa}. The three-path
verification (Hecke, Heegner Chern-class, Lusztig specialisation) is the
content of Vol~I
Theorem~\ref{thm:humbert-lusztig-mukai-8} (chain-level $\Ethree$
identification chapter,
\S\ref{sec:kl-humbert-eighth-root}).
\end{proof}

\begin{remark}[Non-paramodular-$K(N)$ retraction]
\label{rem:pref-non-paramodular}
The earlier "paramodular $K(1)$ level" attribution for $\Delta_5$
is retracted: $K(1) = \mathrm{Sp}_4(\mathbb{Z})$ itself, so
"paramodular level $N = 1$" is not a genuine level refinement. The
correct home of $\Delta_5$ is the Maass spin cover
$\widehat{\mathrm{Sp}_4(\mathbb{Z})}^{v_{\Delta_5}}$, a
$\mathbb{Z}/2$-central extension.
\end{remark}

\section{Synthesis}
\label{sec:pref24-supplement}

\begin{remark}[Preface \S2-\S4 draft as K3 warm-up]
\label{rem:pref24-1}
The preface supplementary sections \S2-\S4 warm the reader to the
K3 base point through the K3 $\{2,3,5,24\}$ spectrum and the
Mukai-pairing signature $(4, 20)$. The unique object
$\mathbf H_{\Delta_5}$ sits at the intersection of these data; the
universal identity $\hbar^2 \cdot K^{\kappa_{\mathrm{ch}}} = -1$ at
$\hbar^2 = -1/8$ is the single algebraic equation selecting
$\mathbf H_{\Delta_5}$. This supplementary draft provides a gentler
onramp than the main preface, emphasising the Mukai doubling
$K^{\kappa_{\mathrm{ch}}} = 8$ and the bi-based Ran pair
\cite{Mukai1987,GritsenkoNikulin1998}. Cross-ref Vol.~III
