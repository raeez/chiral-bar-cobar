...
The level-$2$ object before averaging is the \emph{ordered bar complex}
\[
...
arbitrary curve. The Arnold relation is the local cocycle condition for
that representative.
...
% ====================================================================
\subsection*{Two Koszul dualities: the ordered bar before averaging}
% ====================================================================
...
\]
a tensor-valued rational function of the spectral parameter~$z$
encoding the full binary OPE data. Applying the finite-window
degree-two trace after averaging collapses the $z$-dependent profile
to a single scalar:
\[
\tau_{\cA,2}^{\mathrm{res}}q_2(K_\cA^{\mathrm{coll}})
\;=\;\kappa(\cA)
\quad \text{on the normalized finite trace window,}
\]
...
ordered story is the primitive; the symmetric / modular-Koszul story (T15 sense (iii)) is its shadow.
...
absorbs one power of $(z-w)$), producing the first-order tensor pole
$k\Omega_{\mathcal H}/z$ with rank-one coefficient $k/z$. Applying the
averaged degree-two trace:
\[
\mathrm{av}\bigl(r_{\mathcal H}\bigr) \;=\; k \;=\; \kappa(\mathcal H_k).
\]
...
and its degree-two trace is the modular characteristic~$k$.
...
the passage from $r(z)=k\Omega_{\mathrm{tr}}/z$ to $\kappa(\widehat{\mathfrak g}_k)$
