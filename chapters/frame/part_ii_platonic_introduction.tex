...
%
% The Platonic formulation says the
...
%
% This chapter is the Platonic introduction to Part~II: a replacement of
...
The Virasoro climax of classical CFT, in this formulation, is
...
denominator bound $c^{r-3}(5c + 22)^{\lfloor (r-2)/2 \rfloor}$ for
$S_r(\Vir_c)$ in the range $4\le r\le 8$; the weight-$1/2$ McKay--Thompson
...
\end{align}
The pattern $\mathrm{den}(S_r)\mid c^{r-3}(5c + 22)^{\lfloor (r-2)/2
\rfloor}$ for $4\le r\le 8$ is recovered motivically from the
...
S_r \in \mathbb{Q}(c)$.  The first two rungs are
$S_2=c/2$ and $S_3=2$. For $r\in\{4,5,6,7,8\}$, the denominator of
$S_r(\Vir_c)$ as a rational function of $c$ divides
\begin{equation}
  D_r(c)
  \;=\; c^{r-3}\,(5c + 22)^{\lfloor (r-2)/2 \rfloor},
  \qquad r \in \{4, 5, 6, 7, 8\},
...
\end{equation}
	acts trivially on the Virasoro shadow coefficients: by
	Theorem~\ref{thm:virasoro-motivic-rationality-all-r} every
	$S_r(\Vir_c)$ is a rational function of~$c$, so its motivic lift is
	Tate-rational. In particular, the weight-$2$ rung
	$S_2^{\mot} = \kappa^{\mot} = (c/2)^{\mot}$ is primitive, and the
	higher Virasoro rungs still carry no non-trivial motivic coaction.
	Non-trivial motivic content belongs to higher-primary families such
	as $\mathcal W_3$, not to the Virasoro rational tower.
\end{theorem}
...
\ref{thm:s8-virasoro-closed-form}.  The factor $c^{r-3}$ comes
from the Virasoro recurrence after the $S_3=2$ rung removes one central
factor from the naive $c^{r-2}$ count over the first nontrivial
...
\textup{(}independent verification check
...
\begin{equation}
  \varphi(V^\natural) \;=\;
  \bigl(\,4,\; \infty,\; \chi_{\VOA}(V^\natural),\; 1,\;
  \mathrm{coset}_{V^\natural}\,\bigr),
\end{equation}
recording $p_{\max}(V^\natural) = 4$ from the stress-tensor OPE,
$r_{\max}(V^\natural) = \infty$ from the Virasoro sub-VOA at
$c = 24$ \textup{(}placing $V^\natural$ in class $\mathrm{M}$, with
$\kappa(V^\natural) = 12$\textup{)}, and
$n_{\mathrm{strong}}(V^\natural) = 1$ from the single strong
generator $T$ of weight $2$ \textup{(}$\dim V^\natural_1 = 0$, so
the Virasoro sub-VOA determines $\kappa$; see the landscape
census\textup{)},
with coset slot
\begin{equation}
  \mathrm{coset}_{V^\natural} \;=\;
...
$r_{\max}(V^\natural) = \infty$ follows because the inherited
Virasoro sub-VOA carries the full Virasoro shadow tower with
non-vanishing $S_r$ at every $r \ge 2$ \textup{(}class $\mathrm{M}$,
$\kappa(V^\natural) = c/2 = 12$\textup{)};
...
$n_{\mathrm{strong}}(V^\natural) = 1$ records the single strong
generator $T$ of weight $2$ \textup{(}$V^\natural_1 = 0$ by
Frenkel--Lepowsky--Meurman, so the strong-generating set reduces to
the stress tensor inherited from the Virasoro sub-VOA\textup{)}; and
...
$\chi_{\VOA}$, not through $n_{\mathrm{strong}}$.
...
\begin{enumerate}[(i)]
  ...
  $r \le 8$; the denominator pattern for $r\ge4$,
  $c^{r-3}(5c + 22)^{\lfloor (r-2)/2 \rfloor}$, is a
...
hyperbolic lattice $\Lambda_{\mathrm{Muk}}^{2,1}_{II}$: a rank-$3$
hyperbolic core orthogonal to the rank-$24$ Mukai lattice. Its
chiral bialgebra realisation $\mathbf H_{\Delta_5}$ carries the
Weyl denominator $\Delta_5 \in S_5(K(1))$, the Gritsenko paramodular
weight-$5$ cusp form.

\begin{theorem}[K3 master L-value identity for Part~II;
  \ClaimStatusProvedElsewhere]
\label{thm:part-ii-k3-master-L-value}
The genus-$1$ free energy of $\mathbf H_{\Delta_5}$ is the
Quillen--Arakelov norm of $\Delta_5$:
\[
\log Z^{(1)}_{\mathbf H_{\Delta_5}}
\;=\;-\log\bigl\|\Delta_5\bigr\|^2_{\mathrm{Quillen}}
\;=\;-\log\Delta_5 \;-\; 24\,L'(0,\Delta_5,\mathrm{std}),
\]
with $24 = \chi_{\mathrm{top}}(K3)$ the Bismut--Gillet--Soul\'e
coefficient (Bruinier--K\"uhn $2003$ + Bismut--Gillet--Soul\'e $1988$).
The $L$-function is standard, not adjoint-spinor: Pitale--Saha--Schmidt
$2014$ CAP factorisation plus the Waldspurger squaring rule rule out
$L'(0,\Delta_{10},\mathrm{ad}^0)$.
Cross-reference Chapter~\ref{ch:k3-chiral-bialgebra-platonic}.
\end{theorem}
...
(Theorem~\ref{thm:part-ii-fingerprint-complete-invariant}) reads
...
The pentagon tower $\{\phi^{(n)}\}_{n\ge 3}$ of $\mathbf H_{\Delta_5}$
satisfies
\[
\phi^{(n)} \;=\; 0
...
$\phi^{(5)} = -2\,[\mathrm{gen}]^{\otimes 5}$. This bypasses the
Zagier--Hoffman regularisation of the motivic shadow tower
(Theorem~\ref{thm:part-ii-motivic-purity-virasoro-r-le-8}): the
admissibility condition on $(D_n\pmod 4)$ is a Heegner-divisor
reading of the shadow denominator pattern.
\end{remark}

\begin{remark}[Bloch--Kato Selmer deformation of the fingerprint]
\label{rem:part-ii-selmer-deformation}
The fingerprint $\varphi(\mathbf H_{\Delta_5})$ admits a
...
The motivic Galois action on the shadow tower
(Theorem~\ref{thm:part-ii-motivic-purity-virasoro-r-le-8}) lifts
at the K3 base point to a non-split extension
$\mathrm{grt}_1^{(1/2)}\twoheadrightarrow\mathrm{grt}_1$ by even-weight
generators carrying Saito--Kurokawa motivic content, with
fundamental bracket
\[
[\widetilde\sigma_2,\widetilde\sigma_2] \;=\; 288\,\widetilde\sigma_4,
\qquad 288 \;=\; \tau(2)^2/2.
\]
The Ramanujan tau value $\tau(2)^2/2 = 288$ is the first
metaplectic structure constant; it refines the Ihara bracket on
$\mathrm{grt}_2$.
\end{remark}
...
five-slot invariant; the four-class quadrichotomy is its coarse
projection; the Monster's $194$ conjugacy classes are the moonshine
orbit; the motivic denominator bound
$c^{r-3}(5c + 22)^{\lfloor (r-2)/2\rfloor}$ for $r\ge4$ is the
Feigin--Fuchs Shapovalov trace through $r = 8$.
...
\hline
\ref{thm:part-ii-quadrichotomy-coarse-projection}
(coarse projection)
  & Chapter~\ref{ch:infinite-fingerprint}
    \S\ref{sec:quadrichotomy-coarse}
...
Each independent verification path carries a one-sentence disjoint
rationale recorded in the compute-library registry
\texttt{compute/lib/independent\_verification.py}.  The audit target
\texttt{make verify-independence} records these five claims as
