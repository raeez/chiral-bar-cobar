...
\begin{equation}
 ...
 \otimes \mathbb{Q}(c),
\end{equation}
assuming the motivic shadow-lift construction of
Theorem~\textup{\ref{thm:shadow-tower-motivic-lift}}, with Brown
period map specialization $\mathrm{per}(S_r^{\mot}) =
...
$V^\natural_1=0$ by Frenkel--Lepowsky--Meurman, so no affine
weight-one current is present; the Virasoro vector nevertheless
generates only the Virasoro sub-VOA. The integer in the fourth slot
is therefore a Griess-algebra invariant, not the number~$1$; and
