...
(motivic residue problem $r \le 8$)
 ...
 (\texttt{virasoro\_motivic\_purity});
 Chapter~\ref{ch:shadow-tower-higher-coefficients}
...
\section{Status records within Part~II}
\label{sec:part-ii-hz-iv-closures}

The five structural surfaces of Part~II carry one status record each.
Proved rows list independent checks; open rows list the construction
package that closes the claim.

\begin{center}
\begin{tabular}{p{4cm}|p{5cm}|p{6cm}}
\hline
\textbf{Surface} & \textbf{Input} & \textbf{Verification or remaining
obligation} \\
\hline
Theorem~\ref{thm:part-ii-fingerprint-reconstruction-surface}
(conditional fingerprint reconstruction)
...
(completeness)
 ...
 & Residual-free reconstruction
 $\Phi_{\mathrm{recon}}$ tested on $\{H_k, V_k(\fsl_2),
