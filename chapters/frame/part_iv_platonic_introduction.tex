...
Part~\ref{part:modular-tower} develops the four Theorem~D objects:
$\operatorname{Obs}^{\mathrm{def}}_g(A_b)\in H^2(\Def_g(A_b))$ for
$g\geq2$;
the trace
$\operatorname{tr}_1\operatorname{Obs}^{\mathrm{def}}_{1,1}(A_b)
=\kappa(A_b)\lambda_1$ under~$H_D^1$;
$\mathfrak O_g^K(A_b)=\kappa(A_b)\lambda_{-1}(\mathbb E_g)$ and its
signed Hodge character under~$H_D^K$; and
$F_g=\kappa\lambda_g^{\mathrm{FP}}+\delta F_g^{\mathrm{cross}}$ under
$H_D^{\mathrm{graph}}$.  The package~$H_D^{\mathrm{tr}}$ compares the
deformation class with the perfect object.  The part also develops
the shadow-tower quadrichotomy via
the algebraic branch structure of the weighted Riccati series, and the
modular trace + clutching package on the open category. The present
chapter inscribes the five
...
typed Theorem~D outputs in concrete physics
...
K_\cA^{\mathrm{coll}}(z), \Zder(\cA), \nabla^{\mathrm{hol}}_\cA)$, where
$\Theta_\cA = D_\cA - d_0$ is the universal Maurer--Cartan
element \textup{(}Theorem~\ref{thm:mc2-bar-intrinsic}\textup{)},
$K_\cA^{\mathrm{coll}}(z)$ is its binary collision kernel; its
represented image is a classical $r$-matrix on the locus
...
Drinfeld-centre passage to categorical half-braidings; higher Deligne.
The passage is proved single-direction, arrow by arrow, and should not
be read as a round-trip isomorphism. Closure to a full operadic circle
requires the Drinfeld-centre/derived-centre comparison together with a
conformal vector making translations $Q$-exact; it is proved
chain-level on the original complex only for the affine Kac--Moody
case at non-critical level
...
statement uses the conditional coderived comparison
...
\begin{theorem}[Class-$M$ ladder through $\SCchtop$ and, under
 ...
 \texttt{V2-thm:iterated-sugawara-construction}.  The statement below
 uses the weight-completed carrier specified by Volume~II
...
\begin{enumerate}[label=\textup{(\roman*)}, leftmargin=*]
 ...
 \textup{(}Volume~II
 Theorem~\ref{V2-thm:e-infinity-topologization-ladder}\textup{)}
 ...
 $\mathrm{ChirHoch}^\bullet(\cA_\infty)^\wedge$; the
 bounded-direct-sum calculation identifies the weight completion as
 part of the class-\(M\) carrier;
...
\textup{(}Volume~II
Theorem~\ref{V2-thm:e-infinity-topologization-ladder}, class-$M$
...
\cite{Gaiotto-Rapcak-Y}.  This action is defined on the
$\SCchtop$ carrier.  Descent to $E_3^{\mathrm{top}}$ is the
equivariance of the topologisation map in the coloured operad.
\(\quad\square\)
\end{proof}

\begin{remark}[Class-$M$ ambient]
\label{rem:part-iv-thm-4-hziv}
For class \(M\), the chain-level topologisation is carried by the
pro-object / \(J\)-adic / filtered weight-completed ambient.  The
Linshaw--Gaiotto--Rapcak construction of
\(\cW_{1+\infty}[\mu]\) supplies the infinite-rank algebraic carrier.
On the declared comparison surface, the Costello--Gwilliam
holomorphic-topological factorization algebra supplies an independent
\(E_3^{\mathrm{top}}\) model.
\end{remark}
...
\begin{remark}[Six coordinate charts for the apex carrier]
\label{rem:pivpi-2}
The coordinate-chart idealisation realises the $\mathcal M$-class apex
