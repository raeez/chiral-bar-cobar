...
\;\in\;\Bbbk.
\]
Under~$H_D^1$ this scalar gives the pointed trace
$\kappa(\cA)\lambda_1$.  Under~$H_D^K$ it gives
$\mathfrak O_g^K(\cA)=\kappa(\cA)\lambda_{-1}(\mathbb E_g)$ and its
signed Hodge character. On
the $E_1$ side, $q_2(K_\cA^{\mathrm{coll}})$ is the
$\Sigma_2$-homotopy-coinvariant operator class; the normalized
functional $\tau_{\cA,2}^{\mathrm{res}}$ produces $\kappa$. Explicit values:
\begin{center}
...
First Chern class $c_1$
 & Scalar curvature $\kappa(\cA)$ (abelian/scalar:
 $\tau_{\cA,2}^{\mathrm{res}}q_2(K_\cA^{\mathrm{coll}})$; affine KM:
 $\kappa_{\mathrm{dp}}+\dim(\fg)/2$)\\[2pt]
Chern character
...
algebraic of degree~$2$: $H(t)=t^2\sqrt{Q_L(t)}$. The scalar
discriminant $\Delta = 8\kappa S_4$ governs the branch structure of
the primary-line square root after the support stratum has been fixed;
the depth classification itself is carried by the support packet.
...
dichotomy (Theorem~\ref{thm:single-line-dichotomy}) is a statement on
a fixed primary line: the perfect-square scalar trace is finite, and a
nonzero scalar discriminant witnesses the mixed Virasoro wheel only
after the mixed support packet has been fixed.
...
$R$-kernel: $r_{\mathcal H}(z)=k\Omega_{\mathcal H}/z$ (rank-one,
$\Sigma_2$-invariant; averaging loses no noncommutative Casimir
channel). Koszul dual:
...
poles. $R$-matrix: $r(z)=k\Omega_{\mathrm{tr}}/z$, the Casimir tensor of
...
the $R$-matrix $r(z)=k\Omega_{\mathrm{tr}}/z$ is the genus-$0$ binary shadow
...
$c_{\mathrm{ghost}}=-26$. In this manuscript this is an
anomaly-accounting dictionary. A string-critical claim requires a determinant line, the
\(bc\)-ghost complex, a BRST current, and \(Q^2=0\). With those data
fixed, the bar-cobar counterparts are:
\begin{itemize}[nosep]
...
\item The string-critical dimension \(c_{\mathrm{matter}}=26\) is the
 BRST anomaly-cancellation equation
 \(c_{\mathrm{matter}}+c_{\mathrm{ghost}}=0\) for the chosen ghost
 complex. The KSDual fixed-point condition belongs to the separate
 Verdier--Koszul lane.
\item The ghost system $bc$ with $c_{\mathrm{ghost}}=-26$ is the
 external BRST ghost resolution. A named BRST/Koszul comparison map
 relates it to the Koszul partner $\cA^!$.
...
\item The genus expansion
 ...
 is the scalar diagonal bar-complex lane. It matches the Polyakov
 anomaly coefficient under the determinant-line and BRST comparison
 package; Weil--Petersson integration remains a separate geometric
 input.
\end{itemize}
...
\textup{(viii)} is the family-indexed chiral Hochschild support
statement under $H_H(\cA;S)$, with duality supplied by a separate
perfect degree-$d$ pairing. Item \textup{(xi)} is conditional on the
...
\item A family datum $H_H(\cA;S)$ gives
 $\operatorname{Supp}\mathrm{ChirHoch}^\bullet(\cA)\subseteq S$;
 a perfect degree-$d$ pairing gives the separate reflection
...
chirally Koszul on the universal non-critical PBW/Koszul lane;
critical level is a separate Feigin--Frenkel centre/Sugawara
degeneration boundary, and admissibility is a simple-quotient
condition rather than a universal-PBW enhancement); and bifunctor
decomposition (one-directional, converse open). A further
heuristic characterization; \emph{tropical Koszulness}, phrased
via a weight spectral sequence on the real blowup; is suggestive
of a thirteenth equivalence but is not inscribed as a proved
equivalence here; see the scope remark attached to
Theorem~\ref{thm:koszul-equivalences-meta} and the ``see also''
discussion in the body of the text.

For simple quotients $L_k(\fg)$ at admissible levels, rationality is
not a Koszulness criterion. The $L_k(\mathfrak{sl}_2)$ lane carries
strong finite Shapovalov/character evidence, but it is conditional
until the quotient-bar spectral sequence and convergence package is
installed. At $\mathrm{rk}(\fg)\ge 2$ with denominator $q\ge 3$,
Koszulness may fail via a bar obstruction from the abelian Cartan
subalgebra.
...
algebra of the identity functor on $\cA$-modules.  For a principal
$\mathcal W$-algebra, PBW supplies the associated-graded quadratic
input.  A family datum
$H_H(\mathcal W^k(\fg);S_{k,\fg})$ supplies the complete retract and
gives
\[
\operatorname{Supp}\mathrm{ChirHoch}^\bullet
(\mathcal W^k(\fg))\subseteq S_{k,\fg},
\qquad
P(t)=\sum_{n\in S_{k,\fg}}
\dim H^n(K_{\mathcal W^k(\fg),S_{k,\fg}})t^n.
\]
The bounded Virasoro calculation has support $\{0,2,3\}$ with
one-dimensional groups.  Identifying a class in
$\mathrm{ChirHoch}^2$ with the level deformation
$k\mapsto k+\epsilon$ requires the family-specific tangent map.
Continuous Gel'fand--Fuks cohomology is a distinct functor connected
to the curve chart by a filtered comparison.  A brace-compatible
formality contraction supplies a formal $E_2$-algebra presentation.
