...
\textup{(G1)} and \textup{(G2)} the seven theorems of this volume
descend as corollaries: the Koszul reflection (Theorem~A),
bar--cobar inversion (Theorem~B), the Lagrangian decomposition
into complementary halves $\mathbf Q_g(\cA)\oplus\mathbf Q_g(\cA^!)$
with scalar shadow $\kappa+\kappa^!=\varrho_\cA K$
(Theorem~C, $\varrho_\cA$ family-specific, $K$ the Koszul conductor
of \textup{(G1)}), the genus tensor curvature
$\mathrm{obs}_g=\kappa\lambda_g$ \textup{(UNIFORM-WEIGHT)} (Theorem~D), the chiral
Hochschild concentration theorem in $\{0,1,2\}$ (Theorem~H), the universal
...
Theorem~A constructs the bar-cobar $(\infty,2)$-properad adjunction
with $R$-twisted $\Sigma_n$-descent and its Verdier intertwining,
producing the pair $(\cA,\cA^{!}_\infty)$ from
$\barB^{\mathrm{ord}}(\cA)$.
Theorem~B proves bar-cobar inversion
$\Omega(\barB(\cA))\xrightarrow{\sim}\cA$ on the Koszul locus at all
genera: genus~$0$ unconditional; the standard CFT-type surface at all
genera except integer-spin $\beta\gamma$ unconditional; the
admissible-level fragment formerly conditional on
axiom~\ref{MK:modular} now a theorem via Positselski's
nonhomogeneous curved differential graded coalgebra theorem and the
periodic CDG filtration.
...
into complementary halves controlled by~$\cA$ and~$\cA^!$ and lifts
the ambient to a canonical $-(3g{-}3)$-shifted symplectic structure on
$\overline{\cM}_{g,n}$ with Lagrangian section from
$\cA\otimes\cA^{!}_\infty$; the Verdier-side Lagrangian splitting is
proved, and perfectness of the pair on the standard landscape is a
theorem rather than an imposed hypothesis.
...
\textup{(UNIFORM-WEIGHT)}, with the all-genera universality closed
through the Graber--Vakil tautological-ring socle theorem applied to
the genus-$1$ base with separating and non-separating clutching
additivity; outside that lane at $g \ge 2$ the free energy acquires
the explicit cross-channel correction $\delta F_g^{\mathrm{cross}}$
...
family-dependent Hilbert polynomial of degree at most~$2$ and carries
a universal $\Ethree$-algebra structure via the heptagon coherence
diagram; at critical affine level $k=-h^\vee$ the theorem does not
apply, and the Feigin--Frenkel centre makes the cohomology unbounded.
...
\noindent\textbf{Theorem~A} (bar-cobar $(\infty,2)$-properad
adjunction and Verdier intertwining). The bar functor~$B$ and the
cobar functor~$\Omega$ form an $(\infty,2)$-adjunction of properads
with $R$-twisted $\Sigma_n$-descent on the category of chiral
algebras over~$\operatorname{Ran}(X)$. Verdier duality
on~$\operatorname{Ran}(X)$ intertwines the bar of~$\cA$ with the bar
of the Koszul dual: $\mathbb D_{\operatorname{Ran}}(B(\cA))\simeq\cA^!_\infty$.
The statement is proved unconditionally on the fixed smooth curve;
extension to the full $\overline{\cM}_{g,n}$ including boundary is
CONDITIONAL on two uninscribed ingredients: Francis--Gaitsgory
six-functor base-change on the relative Ran prestack
(cited to~\cite[Ch.~III, \S10]{GR17}), and Mok25 logarithmic
factorization-gluing at the nodal boundary. See
\ref{rem:A-infinity-2-modular-family-scope} for the inscribed
scope remark.
\medskip
...
$\Omega(\barB(\cA))\xrightarrow{\sim}\cA$. The inversion holds at
three distinct ambient levels with distinct scopes. At the coderived
level $\mathsf{D}^{\mathrm{co}}_{\chi}(X)$ the counit is an
isomorphism unconditionally on the full Koszul locus, via Positselski's
nonhomogeneous curved differential graded coalgebra theorem adapted to
the chiral setting (cited from \cite{Positselski2011}, Theorems~4.6
and~7.2; see
...
\noindent\textbf{Theorem~C} ($-(3g{-}3)$-shifted symplectic
structure with Lagrangian section). The derived chiral centre
$\cZ^{\mathrm{der}}_{\mathrm{ch}}(\cA)=
R\!\operatorname{Hom}_{\cA\otimes\cA^{\mathrm{op}}}(\cA,\cA)$
carries a canonical $-(3g{-}3)$-shifted symplectic structure on the
moduli stack $\overline{\cM}_{g,n}$, with Lagrangian section given by
the tautological map from $\cA\otimes\cA^{!}_\infty$. Its genus-$g$
...
rather than as an imposed hypothesis. The shift is read off the
dimension count $\dim\overline{\cM}_{g,n}=3g-3+n$ via the
Pantev--To\"en--Vaqui\'e--Vezzosi mapping-stack symplectic form; the
Lagrangian property combines the Verdier anti-symplectic involution
with the bar-chart lift. The scalar consequence is family-dependent
complementarity $\kappa(\cA)+\kappa(\cA^{!})=\varrho_\cA K(\cA)$, where
$K(\cA)=c(\cA)+c(\cA^{!})$ is the Koszul conductor of \textup{(G1)}
and $\varrho_\cA$ is a family-specific anomaly ratio
(principal $\cW_N$: $\varrho_N = H_N-1$; non-principal: algebra-specific,
read from $\kappa(\cA) = \varrho_\cA c(\cA)$):
$\kappa+\kappa^!=0$
for affine Kac--Moody and free-field families (with $\varrho_\cA=0$
or $K(\cA)=0$ by antisymmetric duality; $K(V_k(\fg))=2\dim\fg$),
$\kappa+\kappa^!=13$ for Virasoro at self-dual $c=13$
(with $\varrho=\tfrac12$, $K=26$), $\kappa+\kappa^!=250/3$ for
$\cW_3$ (with $\varrho=\tfrac56=H_3-1$, $K=100$), and
$\kappa+\kappa^!=98/3$ for Bershadsky--Polyakov (with $\varrho=\tfrac16$,
$K=196$).
\medskip
...
unconditional for every family. On the uniform-weight lane, the
extension
$\operatorname{obs}_g(\cA)=\kappa(\cA)\cdot\lambda_g$
\textup{(UNIFORM-WEIGHT)} is unconditional on the nose at
$g\in\{1,2\}$, where the nilpotent ideal $\cN^g$ of the tautological
ring vanishes; at $g\ge 3$, clutching-uniqueness
\textup{(\refname*{prop:clutching-uniqueness})} fixes the scalar
coefficient on the socle $R^g/\cN^g$ of the tautological ring, and
the on-the-nose lift from the socle to $R^g$ is conditional on
Faber's $\lambda_g$-conjecture. For multi-generator families such as
$V_k(\fg)$ with $\fg$ non-abelian, coincidence with the physical
bar-curvature obstruction requires the scalar-diagonal hypothesis
\textup{(\refname*{def:scalar-diagonal-hypothesis})}, automatic for
rank-$1$ strong-generator families. The proof is the
Bismut--Gillet--Soul\'e fiberwise Chern-curvature formula in
Arakelov metric, lifted from~$c_1$ to the Hodge-bundle scalar
channel by the $K$-theoretic class
$\alpha\cdot\lambda_{-1}(\mathbb{E})$; an independent
Grothendieck--Riemann--Roch derivation on the universal curve
provides the complementary verification path on the socle.
The theorem includes additivity, Koszul-duality constraints, and the
Faber--Pandharipande $\hat A$-series
$\sum_{g\ge 1}F_g(\cA)\,x^{2g}=\kappa(\cA)(\hat A(ix)-1)
=\kappa(\cA)\bigl(\tfrac{x/2}{\sin(x/2)}-1\bigr)$ for the scalar
free energy. For multi-weight algebras at $g \ge 2$, the scalar
formula is replaced by
$F_g=\kappa(\cA)\lambda_g+\delta F_g^{\mathrm{cross}}$
\textup{(ALL-WEIGHT $+\delta F_g^{\mathrm{cross}}$)}; for
$\cW_3$ at $g=2$, the correction reads
$\delta F_2^{\mathrm{cross}}(\cW_3)=(c+204)/(16c)$.
\medskip

\noindent\textbf{Theorem~H} (chiral Higher Deligne; Koszul locus,
generic affine level in the non-abelian case). On the Koszul locus
the chiral Hochschild complex
$\operatorname{ChirHoch}^*(\cA)$ has cohomological
amplitude $[0,2]$ and carries a universal $\Ethree$-algebra
structure via the heptagon coherence diagram, with an explicit
...
open-to-closed is forbidden. An $\SCchtop$-operation of degree~$(k,m)$
lives on
...
Verdier/Koszul dual on the Koszul locus, $\cC$ the bulk/line comparison
slot \textup{(}realized as the derived chiral centre on the open/closed
side and, when the line comparison is proved, as the dg-category of
line operators\textup{)}, $r(z)=\operatorname{Res}^{\mathrm{coll}}_{0,2}(\Theta_\cA)$
...
and degree~$n$). The datum is the natural unit of holographic data: the
...
outside those lanes, the six-slot packaging remains conjectural.
...
stratum at degree~$2$ extracts the universal twisting morphism:
...
genus-$0$ shadow one-form at degree~$n$ is the KZ connection
$\operatorname{Sh}_{0,n}(\Theta_{\widehat{\fg}_k})
=\sum_{i<j}\Omega_{ij}/(h^\vee{+}k)\cdot d\log(z_i{-}z_j)$;
flatness is the MC equation at genus~$0$ and degree~$n$. For
...
obstruction to extending the Yangian seed from degree~$2$ to degree~$4$;
...
$\kBKM(X)=c_N(0)/2$, defined for the K3-fibered Class~A landscape
($N\in\{1,\dots,6\}$ via diagonal orbifolds and the STU model;
\texttt{prop:bkm-weight-universal}), and Volume~I's universal
conductor $K(\cA)=-c_{\mathrm{ghost}}(\mathrm{BRST}(\cA))$ are
evaluations of one trace principle through the Calabi--Yau-to-chiral
functor $\Phi_{\mathrm{CY}}$:
\[
\kBKM(X)\;\equiv\;\tfrac12\,K\bigl(\Phi_{\mathrm{CY}}(X)\bigr)
\qquad\text{(K3-fibered Class~A only; sign and factor conventions absorbed).}
\]
The identity does not extend to Class~B Calabi--Yau threefolds
(quintic, $\bC^3$, conifold, local~$\bP^2$): for those, $\kBKM$ is
undefined, replaced by $\kappa_{\mathrm{BCOV}}=\chi(X)/24$ and the
...
at programme level, verified per-family for K3-fibered Class~A; it
...
tautological-ring socle, upgrading Theorem~D to all-genera
UNIFORM-WEIGHT unconditional. \emph{Drinfeld--Sokolov--Hochschild
