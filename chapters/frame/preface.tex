...
\noindent
This monograph studies holomorphic chiral factorisation
(co)homology via bar and cobar chain constructions at various
geometric locations on algebraic curves. The geometry determines
...
factorisation (co)homology. The seven theorems of this volume
are structural properties of the result at each level:
the five invariant-skeleton theorems A--D and~H extract the
$\Sigma_n$-invariant content, and the two architectural theorems F
and~G (universal holography and infinite-fingerprint completeness)
close the structure on the non-degenerate locus.
...
logarithmic kernel $\eta = d\log(z_1 - z_2)$ on
...
axiom~\ref{MK:modular} (FM251) now a theorem via Positselski's
...
$\cA_1^{!}\cong\cA_2^{!}$; the quaternitomy $G/L/C/M$ is the coarse
...
axiom~\ref{MK:modular} (FM251) is closed via Positselski's
...
complementarity $\kappa(\cA)+\kappa(\cA^{!})=K(\cA)$: $K=0$ for
affine Kac--Moody and free-field families, $K=13$ for Virasoro
(self-dual at $c=13$), $K=250/3$ for $\cW_3$, $K=98/3$ for
Bershadsky--Polyakov.
\medskip
...
formerly conditional (AP225) is now a theorem. The proof is the
...
proof chain, formerly circular (AP222), is rerouted through the
...
the quaternitomy $G/L/C/M$; the critical-level companion
...
brings the HZ-IV independent-verification decorators to complete
...
questions. \emph{Periodic CDG admissible-level} (FM251): closed via
