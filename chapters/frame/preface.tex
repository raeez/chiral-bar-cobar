...
A boundary chiral algebra $\cA$ determines its bulk through a single
functor, the chiral derived center
...
bar complex is the holographic central charge of that bulk: the scalar
that governs, at genus one without restriction and at higher genera on
the uniform-weight lane, the Brown--Henneaux-type asymptotic behaviour
of the genus-$g$ free energies through
$\operatorname{obs}_g(\cA) = \kappa(\cA)\cdot\lambda_g$.  The shadow
obstruction tower $\{S_r(\cA)\}_{r\ge 2}$ extracted from the universal
Maurer--Cartan element $\Theta_\cA := D_\cA - d_0$ is the holographic
correction hierarchy: each finite-order projection measures a distinct
bulk operator, and the tower terminates when the bulk is finite-range.
The Drinfeld double
...
per the four-part programme developed in Volume~II, Part~IV: its derived
trace is conjectured to reconstruct the bulk partition function from
purely boundary data.
What the present monograph actually proves is the skeleton on which
that programme stands.  Theorems~A--D and~H supply the five proved
ingredients: Theorem~A constructs the pair $(\cA,\cA^{!}_\infty)$ on
which any such double would be defined; Theorem~B proves the
...
information there; Theorem~C realises the symmetry
$\kappa(\cA)+\kappa(\cA^!)$ as a sum rule on the Lagrangian halves of
the centre; Theorem~D computes $\kappa$ as the Brown--Henneaux-type
coefficient of the genus tower at genus~$1$ unconditionally, and at
higher genera on the uniform-weight lane; Theorem~H, at generic level,
bounds the chiral Hochschild complex in degrees $\{0,1,2\}$ with
family-specific total dimension.  At critical level $k=-h^\vee$ the
concentration fails
...
picture, and it is in that sense that they should be read together.
...
The name has been read, historically, as an analogy with classical
quadratic Koszul duality enriched by genus.  The actual content is
sharper.  Modular Koszul duality, at the level this monograph proves,
is the statement that the bar functor $\cA\mapsto\barB(\cA)$ and the
cobar functor $\Omega$ are mutually inverse on the Koszul locus: the
boundary algebra can be recovered from its bar complex.  The bar
functor is the boundary-to-bulk map in the following sense: it takes
the chiral algebra living on the boundary of a three-dimensional
holomorphic-topological geometry and produces the coalgebraic shadow
from which, conjecturally, the bulk operator algebra can be
reassembled through the derived centre
$\cA\mapsto\cZ^{\mathrm{der}}_{\mathrm{ch}}(\cA)$.  The
proved backbone is bar-cobar inversion on the Koszul locus: Theorem~B
gives $\Omega(\barB(\cA))\xrightarrow{\sim}\cA$ there, and Theorem~A
upgrades the inversion to a Verdier intertwining that pairs
$\barB(\cA)$ with $\cA^{!}_\infty$.  The identification of the derived
centre with the chiral bulk, and of $\barB$ with a quasi-inverse to
that bulk-from-boundary functor, is the conjectural content of the
four-part programme in Volume~II, Part~IV; at the level of Volume~I
what is proved is bar-cobar inversion, not bulk reconstruction.  The
shadow tower $\{S_r(\cA)\}_{r\ge 2}$ measures the precise sense in
which the bar-cobar inversion fails off the Koszul locus: at arity
$r$, the shadow $S_r$ is the obstruction to inversion holding
strictly, and the tower terminates exactly when the bulk admits a
finite presentation.  Modular Koszul duality is therefore neither an
operadic analogy nor a higher-categorical decoration: it is the
precise statement that bar and cobar are each other's quasi-inverse
on the Koszul locus, up to the obstructions catalogued by the shadow
tower.  Everything else in this monograph is either a proved
projection of that fact, or a conjectural extension of it into the
bulk reconstruction picture.
