...
\textup{(G1)} and \textup{(G2)} the seven theorem packages of this
volume are read back in their native ambients: the Koszul reflection
in chain and factorization-$\infty$ lanes (Theorem~A), bar--cobar
...
scalar Verdier sums
$0,13,\frac{250}{3},\frac{98}{3}$ on the enumerated families
...
topological and categorical Hochschild (Theorem~H), the universal
holography functor $\Phi_{\mathrm{hol}}$ (Theorem~F), and the
infinite-fingerprint completeness $\phi(\cA)$ (Theorem~G). Five
extract the $\Sigma_n$-invariant content of the ordered bar; two
close the architectural structure on the non-degenerate locus.
...
\noindent\textbf{The seven theorems.}\enspace
...
The seven theorems are the structural skeleton: five (A--D, H) extract
the $\Sigma_n$-invariant content of the ordered bar, and two (F, G)
complete the architectural closure. The full $E_1$ structure (the
...
The independent-verification registry and the HEAL/UPGRADE theorem
upgrades live in the compute layer of each of the three volumes under
a single namespace. The
...
\subsection*{The seven theorems}

Seven theorems extract the structural content of the bar complex:
five invariant-skeleton theorems A--D and~H, together with two
architectural theorems F and~G (universal holography and infinite
fingerprint completeness). Four former frontiers
(periodic CDG admissible-level, curved-Dunn
$H^2=0$ at $g\ge 2$, Drinfeld--Sokolov--Hochschild in class~$M$,
chiral Deligne--Tamarkin) now close inside the theorem statements,
upgrading each theorem where the closure applies.
...
concentration is Koszul-functorial. On the CY-pulled-back crown
$\mathbf H_{\Delta_5}=\Phi_3(\Phi_2(K3))$ the amplitude sharpens to
$\operatorname{ChirHoch}^k(\mathbf H_{\Delta_5})=0$ for $k\ge d+1=4$
with degree-$3$ cocycle
$\chi_3={:}T\partial T{:}-(1/4)\partial^3 T+\hbar\cdot\mathrm{qt}(J^{(3)})$
paired against $e_3^{\mathrm{K3}\times E}$ to the Siegel--Eisenstein
period
$\langle[\chi_3],[e_3^{\mathrm{K3}\times E}]\rangle_{\Phi_3}
=2\,\mathrm{Vol}(E)\cdot(2\pi i)^3
=\chi(\mathcal O_{\mathrm{K3}})\cdot\mathrm{Res}_{s=1/2}
[E_5^{(2)}(Z,s)]|_{K(1)}$.
Six disjoint verification paths pin this value:
CoHA Casimir (Schiffmann--Vasserot), Igusa $\Phi_{10}^{-1}$
elliptic rigidity (Oberdieck reduced DT),
relative Kuznetsov HPD over~$E$ with K3 as Kuznetsov component of a
cubic fourfold, cyclic Feigin--Tsygan, NC Hodge Mukai--Hochschild, and
derived-deformation Gritsenko--Nikulin classifying morphism. The
...
shadows indexed by $(\Sigma_{d-1},C)$. The HEAL and UPGRADE theorems
and the independent-verification decorators are installed into the
compute layer of each volume; the computational infrastructure of the
three volumes aligns under a single registry.
...
Four formerly-deepest frontier questions now close inside the
theorem statements. \emph{Periodic CDG admissible-level}: closed via
the screening-adjoint periodic filtration and Arakawa's
$C_2$-cofiniteness bridge, upgrading Theorem~B to unconditional on
the admissible locus. \emph{Curved-Dunn $H^2=0$ at $g\ge 2$}: closed
via the Bismut--Gillet--Soul\'e computation and the
tautological-ring socle, upgrading the scalar route to Theorem~D and
...
natural isomorphism on homotopy-fixed points.
