...
\noindent
A point of a curve has no product with itself. Two nearby points do:
their difference has the logarithmic derivative
\[
\eta_{12}=d\log(z_1-z_2).
\]
Three nearby points force the relation
\[
\eta_{12}\wedge\eta_{23}
+\eta_{23}\wedge\eta_{31}
+\eta_{31}\wedge\eta_{12}=0.
\]
This identity is the first non-trivial piece of the whole
volume. It is the residue calculus that makes the ordered bar
differential square to zero; it is the flatness of Arnold's
connection; after averaging it becomes the scalar $\kappa$; at higher
genus it reappears as the Hodge-class obstruction
$\kappa\lambda_g$; after Calabi--Yau specialisation it is read as
the trace identity $\hbar^2K=-1$ on the K3 Borcherds witness.

\medskip\noindent
The development therefore runs from the particular to the universal:
\[
d\log(z_1-z_2)
\;\longrightarrow\;
...
Calabi--Yau arithmetic are consequences of this order. A claim that
does not survive this chain is not a theorem of the volume.

\medskip\noindent
This monograph studies one phenomenon: the chiral Koszul reflection
$K\colon\mathrm{Kosz}(X)\to\mathrm{Kosz}(X)^{\mathrm{op}}$ on
chirally Koszul algebras over a smooth algebraic curve~$X$, together
with its conductor and its modular shadow. The geometry determines
the operad: a curve~$X$ gives $\Einf$ (symmetric factorisation on
the Ran space), ordered configurations on~$X$ give~$\Eone$
(the associative chiral operad $\operatorname{Ass}^{\mathrm{ch}}$),
and the product $\bC \times \bR$ gives the holomorphic-topological
Swiss-cheese operad $\mathrm{SC}^{\mathrm{ch,top}}$. Each operad
determines a bar complex; the bar complex computes the
holomorphic chiral factorisation (co)homology of~$\cA$. The
computational engine is the ordered bar coalgebra
\[
\barB^{\mathrm{ord}}_X(\cA)\;=\;T^{c}\bigl(s^{-1}\bar\cA\bigr),
\]
a cofree $E_1$-coalgebra with deconcatenation coproduct whose
differential encodes the complete OPE data of~$\cA$ through the
logarithmic kernel $\eta=d\log(z_1-z_2)$ on Fulton--MacPherson
configuration spaces.

\medskip\noindent
Two equations carry the entire content. The first identifies the
bar differential as the pullback of Arnold's universal flat
connection on configuration spaces by the Knizhnik--Zamolodchikov
functor:
\[
...
\tag{G1}
\]
Read together, the Koszul reflection is involutive:
$K^{2}\simeq\id$ on $\mathrm{Kosz}(X)$, with
$\Omega_X B^{\mathrm{ord}}_X$ a symmetric-monoidal adjoint
equivalence on the Koszul locus
(Chapter~\ref{chap:climax-platonic}, the Climax Theorem). From
\textup{(G1)} and \textup{(G2)} the five theorem packages of this
volume are read back in their native ambients: the bar-cobar arena in
chain and factorization-$\infty$ lanes (Theorem~A), bar--cobar
inversion as strict Koszul-locus recovery together with the
weight-completed coderived Positselski theorem (Theorem~B), the
Lagrangian decomposition into complementary halves
$\mathbf Q_g(\cA)\oplus\mathbf Q_g(\cA^!)$ together with the verified
scalar Verdier sums
$0,8,13,\frac{250}{3},\frac{98}{3}$ on the enumerated families
(Theorem~C), the genus tensor curvature package
...
(Theorem~D), the \emph{chiral}
Hochschild concentration theorem in $\{0,1,2\}$, distinct from
topological and categorical Hochschild (Theorem~H). Universal
holography and infinite-fingerprint completeness are downstream
reconstruction/classification surfaces; they do not replace the five
main theorem packages.

\medskip\noindent
On the Koszul-self-dual subcategory of chiral algebras admitting
both a BRST resolution and a Borcherds-lift input on a
Calabi--Yau target, the conductor $K$ carries three equal
readings: $K_{E}=K_{c}=K_{g}$, the Euler characteristic of the
bar tower equals the sum of central charges $c+c^{!}$ equals the
ghost charge of the BRST resolution (Chapter~\ref{chap:universal-conductor}).
...
to one identity functorial in~$\cA$.
...
\noindent\textbf{The five theorem packages.}\enspace
Five theorem packages extract the structural content of the ordered bar.
...
$\Omega(\barB(\cA))\xrightarrow{\sim}\cA$ on the Koszul locus, and off
...
sums are $0$ for $\mathsf G/\mathsf L/\mathsf C$, $13$ for Virasoro,
$250/3$ for principal $\cW_3$ on the $\mathsf M$-row, and $98/3$
for subregular Bershadsky--Polyakov on the $\mathsf M^{\mathrm{ext}}$-row.
The value $8$ at the $\mathsf B$-row is a distinct lattice invariant
$K=2c_+(L)$ on the $\mathcal H_{\mathrm{Muk}}(K3)$ witness: the
Mukai lattice
$\widetilde H(K3,\mathbb{Z}) = H^0 \oplus H^2 \oplus H^4$ has rank
$1+22+1=24$ and positive signature $c_+=4$, and the
CY$_2$ Serre pairing reads this positive Mukai contribution on
both Verdier halves, producing $K=2c_+(\mathrm{II}_{4,20})=8$.
The universal trace identity
$\hbar^2\cdot K=-1$ at $(K,\hbar^2)=(8,-1/8)$ admits three
convergent routes --- Mukai $2c_+$ unconditional, Lusztig
reflection length $\ell_{\zeta_8}=8$ unconditional, and Humbert
$H_1$-monodromy order programme-conjectured via Bruinier's
Heegner--Chern reciprocity (Bruinier 2002 \emph{LNM} 1780 Prop.~5.1).
...
Theorem~F defines the universal holography functor
...
The five theorem packages are the structural skeleton: A--D and~H
extract the $\Sigma_n$-invariant content of the ordered bar. The
F/G surfaces are downstream reconstruction and classification
theorems built on that spine. The full $E_1$ structure (the
$r$-matrix, the Yangian, the spectral braiding) is developed in this
volume; Volume~II adds the open/closed holomorphic-topological lift
through $\SCchtop$ and the topologised $3$d HT bulk (T18(iii));
Volume~III constructs the geometric source via the two-stage
Calabi--Yau-to-chiral functor
$\Phi_d=\mathrm{Sp}^{\mathrm{ch}}_{\Sigma_{d-1},C}\circ\Phi^{\mathrm{FA}}_d$.
The compute metadata of the three volumes is subordinate to these
theorem statements. The
Drinfeld double $U_\cA := \cA \bowtie \cA^{!}_\infty$, assembled from
the ordered bar's two factors, is the universal boundary-bulk
reconstructor of Theorem~F. On the non-degenerate locus
$\{\kappa\ne 0,\ \text{non-critical level},\ \text{Koszul locus}\}$
each theorem package carries, for the specific family instances
established in the body, at least two genuinely independent structural
verification lanes, and the scalar numerical clauses of Theorems~C
and~D are accompanied where established by three or more family-level
checks. Theorem~H is checked via Shelton--Yuzvinsky, Feigin--Fuchs,
and Whitehead; Theorem~D via Chern--Weil, PTVV, and family
specialization; Theorems~A and~B via twisting-morphism and Lurie
$\infty$-categorical routes. The programme-wide registry of these
decorators remains under active installation. Off the locus (the
Feigin--Frenkel stratum $FF$, the Creutzig--Kanade--Linshaw
logarithmic admissible lane, the super-Yangian cluster, the
Calabi--Yau functor at complex dimension $d\ge 3$) the residual
frontier is explicit and scoped.
...
hyperbolic lattice $\Lambda_{\mathrm{Muk}}^{2,1}_{II}$ (rank-$3$
...
canonical pro-object / $J$-adic / filtered weight-completed ambient,
where the inversion is restored
...
gravity statement is available on the canonical pro-object /
$J$-adic / filtered weight-completed ambient only, in
...
(g,n,d)
\;=\;
(\text{loop genus},\;\text{external degree},\;
...
proved; MC5 is proved on the canonical pro-object / $J$-adic /
...
identification on those equivalent ambients\textup{)}; the bounded
...
ambient-qualified: it is proved on the equivalent pro-object /
$J$-adic topological / filtered weight-completed presentations of the
raw bar complex \textup{(}Theorems~\ref{thm:mc5-class-m-chain-level-pro-ambient},
\ref{thm:mc5-class-m-topological-chain-level-j-adic};
...
\item \emph{MC5 at chain level, with class~M obstruction explicit.}
 ...
 are proved, as is the genuswise chain-level BV/BRST/bar
 identification on the canonical pro-object / $J$-adic / filtered
 weight-completed presentations of the raw bar complex; what remains
 conjectural is a lift to the bounded direct-sum ordinary complex,
 where class~M already fails.
...
\phantomsection\label{thm:completed-bar-cobar-strong}%
...
\phantomsection\label{thm:delta-f-cross-w3-g2}%
...
\section{The K3 chiral bialgebra $\mathbf H_{\Delta_5}$ as fourth Etingof--Kazhdan taxon}
