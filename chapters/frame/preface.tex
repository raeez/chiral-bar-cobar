...
families below exhaust the standard Lie-theoretic landscape:
each realises a specific depth class, curvature profile, and
genus-two shell activation pattern.
...
\medskip
\noindent\textbf{Bershadsky--Polyakov} (class M).
The Drinfeld--Sokolov reduction of
$\widehat{\mathfrak{sl}}_3{}_k$ by the subregular nilpotent
$f_{(2,1)}$.  Generators: a current $J$ (weight~$1$), a pair of
fields $G^\pm$ (weight~$3/2$), and the stress
tensor $T$ (weight~$2$).  The central charge
$c(k) = 2 - 24(k{+}1)^2/(k{+}3)$; the modular characteristic
$\kappa(\mathcal{B}^k) = c(k)/6$, with anomaly ratio
$\varrho_{\mathrm{BP}} = 1/6$.  The Koszul dual is the
Bershadsky--Polyakov algebra at dual level $k^\vee = -k - 6$,
with Koszul conductor
$K_{\mathrm{BP}} = c(k) + c(-k{-}6) = 196$
and complementarity sum
$\kappa(\mathcal{B}^k) + \kappa(\mathcal{B}^{-k-6}) = 98/3$.
The self-dual central charge is $c = 98$ (attained at non-real
levels $k = -3 \pm 2i$); $k = -3$ is the critical level.
The multi-channel $r$-matrix has poles at $z^{-3}$ and $z^{-1}$
in the $T$-channel, driving the infinite shadow tower.
...
\noindent
Koszulness is a concentration property of the ordered bar: the
cohomology $H^*(\barB^{\mathrm{ord}}(\cA))$ sits in bar degree~$1$.
When this holds, the bar-cobar spectral sequence collapses, the
$E_1$~$R$-matrix is determined by its leading term, and every
$\Sigma_n$-coinvariant projection (the five theorems) achieves
maximal rigidity.  The twelve equivalent formulations below
detect this concentration from different vantage points.
