...
each of the seven theorems admits at least two genuinely independent
verification paths registered in the programme's computational
infrastructure: the \emph{bell theorem}. Off this locus (the
Feigin--Frenkel stratum $FF$, the Creutzig--Kanade--Linshaw
logarithmic admissible lane, the super-Yangian cluster, the
Calabi--Yau functor at complex dimension $d\ge 3$) the bell rings
and the residual frontier is explicit and scoped.
...
The statement is proved unconditionally on the fixed curve and on the
relative smooth locus; extension to the full $\overline{\cM}_{g,n}$
including boundary is inscribed via Beilinson--Drinfeld holonomic
descent, Mok logarithmic-FM sewing, and Francis--Gaitsgory
factorization-gluing.
\medskip

\noindent\textbf{Theorem~B} (bar-cobar inversion on the Koszul
locus at all genera). On the Koszul locus the cobar of the bar
recovers the original algebra:
$\Omega(\barB(\cA))\xrightarrow{\sim}\cA$. Genus~$0$ is
unconditional; on the standard CFT-type surface the inversion is
unconditional at all genera except the integer-spin $\beta\gamma$
lane. The admissible-level fragment formerly conditional on
axiom~\ref{MK:modular} is closed via Positselski's
nonhomogeneous curved differential graded coalgebra theorem adapted
to the chiral setting and the periodic CDG filtration of the
screening-adjoint construction: for a conilpotent chiral
CDG-coalgebra~$C$ on~$X$ with finite-dimensional graded pieces, the
counit is an isomorphism in the coderived category
$\mathsf{D}^{\mathrm{co}}_{\chi}(X)$. The bar complex is therefore a
complete invariant on the full Koszul locus.
\medskip
...
all-genera extension
$\operatorname{obs}_g(\cA)=\kappa(\cA)\cdot\lambda_g$
\textup{(UNIFORM-WEIGHT)} is closed through the Graber--Vakil
tautological-ring socle theorem applied to the genus-$1$ base with
separating and non-separating clutching additivity on
$\partial\overline{\mathcal{M}}_g$: the clutching-uniqueness step
is now a theorem. The proof is the
Bismut--Gillet--Soul\'e fiberwise Chern-curvature formula in
Arakelov metric, lifted from~$c_1$ to the full Hodge-bundle Chern
class by the splitting principle and scalar-channel linearity of the
bar curvature; an independent Grothendieck--Riemann--Roch derivation
on the universal curve provides the alternative verification path.
...
\noindent\textbf{Theorem~F} (universal holography). The assignment
\[
...
(\cA,\cA^{!}_\infty)\bigr),
\]
is a functor from the category of conilpotent boundary-Lagrangian
chiral algebras on~$X$ to the category of holomorphic-topological
quantum field theories on $X\times\bR$ with anchored boundary
condition. It sends the bar-cobar pair of~$\cA$ to the bulk-boundary
pair of the reconstructed HT QFT; the Drinfeld double
$U_\cA := \cA\bowtie\cA^{!}_\infty$, assembled from the ordered
bar's two factors, is the universal boundary-bulk reconstructor (a
theorem, formerly conjectural). The alternative verification path is
Costello--Gwilliam factorization homology on $X\times\bR$. The
functor is constructed in
Chapter~\ref{chap:universal-holography-functor} of Volume~II.
\medskip
...
$\{\kappa\ne 0,\ \text{non-critical level},\ \text{Koszul locus}\}$
each of the seven theorems admits at least two genuinely independent
verification paths: the \emph{bell theorem}. Off the locus (the
...
at $d\ge 3$) the bell rings and the residual frontier is explicit
and scoped. The proofs of A, B, C, D, H, F, G occupy
...
brings the independent-verification decorators to complete
coverage, and aligns the computational infrastructure of the three
...
now given a verified home, and certifies the bell theorem on the
