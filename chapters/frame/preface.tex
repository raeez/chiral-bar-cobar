...
Hochschild concentration in $\{0,1,2\}$ (Theorem~H), the universal
...
topological Hochschild $\RHom_{\cA^e}(\cA,\cA)$
(not THH in the stable homotopy sense; this is classical
$\HH^*(\cA_{\mathrm{mode}})$): the chiral and
topological constructions use different operadic inputs and produce
different outputs.
...
(ii)~topological Hochschild (= classical $\HH^*$, not THH in the
stable-homotopy sense; $E_1$-bar composition on $\R$,
producing $\HH^*(\cA, \cA)$);
(iii)~categorical Drinfeld centre ($E_1$-monoidal, producing the
$E_2$-braided module category). The geometry determines which:
curve $\to$ chiral; circle $\to$ topological; category $\to$
Drinfeld.
...
\item \emph{Topological Hochschild homology}
 ...
 using the $E_1$-structure. The topological avatar of
 the genus-$1$ trace.
