...
$\operatorname{obs}_g(\cA)=\kappa(\cA)\cdot\lambda_g$ holds
via clutching morphisms on $\partial\overline{\mathcal{M}}_g$,
where $\kappa(\cA)$ is a single scalar determined by the OPE
\textup{(UNIFORM-WEIGHT)}; the clutching-uniqueness step is
the principal conditional element, and an independent GRR
derivation provides an alternative path.
The theorem includes additivity,
...
the $\SCchtop$ operad governs the derived center pair, and the target
$E_3^{\mathrm{top}}$ is the full topological field theory.
Volume~III constructs the geometric source: a functor from
Calabi--Yau categories to chiral algebras that closes the circle
by supplying the input that Volumes~I and~II assumed.
