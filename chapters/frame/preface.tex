...
with spectral parameters from $\FM_k(\C)$. This is \emph{not}
topological Hochschild $\RHom_{\cA^e}(\cA,\cA)$: the chiral and
...
proved). Theorem~H: the chiral Hochschild cohomology
...
(ii)~topological Hochschild ($E_1$-bar composition on $\R$,
producing the line category);
...
\cong A^\lambda$ as $E_3$-families over
