...
\noindent
The operator product expansion of a chiral algebra~$\cA$ on a curve~$X$
is local data: it governs the singularity of $a(z)\,b(w)$ as $z\to w$
on one fibre at a time.  The moduli space $\overline{\mathscr{M}}_g$
is global: it parametrises all curves of genus~$g$ simultaneously.
No direct map sends local OPE coefficients to classes in
$H^*(\overline{\mathscr{M}}_g)$.  The bar complex provides that map.
At genus one it produces a single number
$\kappa(\cA)$ multiplied by the universal topological invariant
$\lambda_1$; for uniform-weight algebras the
identity $\mathrm{obs}_g(\cA) = \kappa(\cA) \cdot \lambda_g$
(UNIFORM-WEIGHT)
persists at all genera.

\medskip

\noindent\textbf{The thesis of this monograph.}\enspace
A three-dimensional holomorphic-topological field theory has a bulk
and a boundary; given only the boundary chiral algebra~$\cA$, no
general method recovers the bulk.  The chiral derived centre
$\cZ^{\mathrm{der}}_{\mathrm{ch}}(\cA) :=
R\!\operatorname{Hom}_{\cA\otimes\cA^{\mathrm{op}}}(\cA,\cA)$
is the candidate: a single functor from boundary to bulk.
The modular characteristic
$\kappa(\cA) = \operatorname{av}(r(z))$, extracted at arity~$2$ from the
bar complex, is the holographic central charge of that bulk.  At
genus~$1$ without restriction, and at all genera on the uniform-weight
locus (UNIFORM-WEIGHT), it governs the genus-$g$ free energies through
$\operatorname{obs}_g(\cA) = \kappa(\cA)\cdot\lambda_g$.
The arity-$2$ scalar $\kappa(\cA)$ is only the first term.
The universal Maurer--Cartan element
$\Theta_\cA := D_\cA - d_0$ projects to a shadow invariant
$S_r(\cA)$ at each arity $r\ge 2$; the resulting tower
$\{S_r(\cA)\}_{r\ge 2}$ is the correction hierarchy, and it
terminates precisely when the bulk is finite-range.
The Drinfeld double
$U_\cA := \cA \bowtie \cA^{!}_\infty$,
assembled from the boundary algebra and its Verdier-dual factorization
partner, is conjectured to be the universal boundary-bulk reconstructor,
per the programme developed in Volume~II: its derived
trace is conjectured to reconstruct the bulk partition function from
purely boundary data.

What the present monograph proves is the skeleton on which
that programme stands.  Five theorems, and five only:
Theorem~A constructs the pair $(\cA,\cA^{!}_\infty)$ on
which any such double would be defined.
Theorem~B proves
bar-cobar inversion $\Omega(\barB(\cA))\xrightarrow{\sim}\cA$ on the
Koszul locus, so that the boundary-to-bulk functor $\barB$ loses no
information there.
Theorem~C decomposes the centre into Lagrangian halves controlled
by~$\cA$ and~$\cA^!$ respectively, with the sum rule
$\kappa(\cA)+\kappa(\cA^!)$ as the scalar shadow of the splitting.
Theorem~D computes $\kappa(\cA)$ as the genus-$1$ coefficient
unconditionally, and at higher genera on the uniform-weight locus
(UNIFORM-WEIGHT).
Theorem~H bounds the chiral Hochschild complex in cohomological
degrees $\{0,1,2\}$ with family-specific total dimension, at generic
level.  At critical level $k=-h^\vee$ the
concentration fails
(Feigin--Frenkel centre), and the reconstruction picture is
correspondingly restricted.  The five theorems are what is proved; the
reconstruction thesis organises them into a single conjectural
picture.
...
input satisfies the axioms of a chiral algebra.  The bar construction
is that functor.  Its integral kernel is the logarithmic $1$-form
$d\log(z_1-z_2)$; its compactification is Fulton--MacPherson; its
coalgebra is cofree on desuspended generators; its differential
squares to zero if and only if the Borcherds identity holds.
...
The ordered bar is the primitive object.  The symmetric bar is a
$\Sigma_n$-coinvariant quotient; it discards the $R$-matrix, the KZ
associator, and the full quantum group deformation.  The five main
theorems of this monograph concern the symmetric bar: they are
precisely the invariants that survive that quotient.  What the
quotient destroys is the subject of Volume~II.
...
\noindent\textbf{What modular Koszul duality really means.}\enspace
The name suggests an analogy: classical quadratic Koszul duality
enriched by genus.  The content is sharper than the analogy.
At the level this monograph proves, modular Koszul duality is the
statement that the bar functor $\cA\mapsto\barB(\cA)$ and the
cobar functor $\Omega$ are mutually inverse on the Koszul locus.
The boundary algebra $\cA$ can be recovered from its bar complex;
no information is lost.  Theorem~B gives
$\Omega(\barB(\cA))\xrightarrow{\sim}\cA$ on that locus, and
Theorem~A upgrades the inversion to a Verdier intertwining that pairs
$\barB(\cA)$ with $\cA^{!}_\infty$.

The conjectural extension is that $\barB$ is the boundary-to-bulk
map: it takes the chiral algebra on the boundary of a
three-dimensional holomorphic-topological geometry and produces
the coalgebraic shadow from which the bulk operator algebra can be
reassembled through the derived centre
$\cA\mapsto\cZ^{\mathrm{der}}_{\mathrm{ch}}(\cA)$.
The identification of the derived centre with the chiral bulk is
the content of the programme developed in Volume~II;
at the level of Volume~I what is proved is bar-cobar inversion,
not bulk reconstruction.

Off the Koszul locus, the inversion fails.  The shadow tower
$\{S_r(\cA)\}_{r\ge 2}$ measures the failure: at arity~$r$,
$S_r$ is the obstruction to strict inversion, and the tower
terminates exactly when the bulk admits a finite presentation.
Modular Koszul duality is the precise statement that bar and
cobar are each other's quasi-inverse on the Koszul locus, with
obstructions catalogued by the shadow tower.  Every result in this
monograph is either a proved projection of that statement or a
conjectural extension into the bulk reconstruction picture.
...
\medskip
\noindent\textbf{The thesis at the atom.}\enspace
The Heisenberg algebra is small enough to verify the reconstruction
thesis of this monograph as a special case, and it is instructive
to do so before the general picture becomes conjectural.  The
boundary chiral algebra is $\cA = \mathcal H_k$.  The derived centre
$\cZ^{\mathrm{der}}_{\mathrm{ch}}(\mathcal H_k)$, at generic level
$k\ne 0$, is the chiral Hochschild cohomology of $\mathcal H_k$,
concentrated in degrees $\{0,1,2\}$ with total dimension~$2$: the
unit in degree~$0$, the level deformation $k\mapsto k+\epsilon$ in
degree~$2$, and nothing else.  This matches the algebra of
observables of abelian Chern--Simons theory at level~$k$: the
three-dimensional theory whose boundary, under holomorphic-topological
quantisation on $\mathbb C_z\times\mathbb R_t$, produces the
Heisenberg vertex algebra on the $\mathbb C_z$ face.  In this
Heisenberg case, the derived-centre calculation recovers the abelian
Chern--Simons bulk exactly; the general statement that
$\cZ^{\mathrm{der}}_{\mathrm{ch}}(\cA)$ is the chiral bulk of~$\cA$
remains conjectural and is developed in Volume~II, Part~IV.  The
modular characteristic is $\kappa(\mathcal H_k) = k$, and this agrees
with the Brown--Henneaux-type central charge of the abelian
Chern--Simons bulk: at genus~$1$ the free energy is $F_1 = k/24$
(uniform-weight: $\mathcal H_k$ has a single generator at weight
one), matching the torus partition function of abelian Chern--Simons
to leading order in the genus expansion.  The shadow obstruction
tower terminates at $r = 2$: the Heisenberg algebra is class~G
(Gaussian), and every invariant at arity $r\ge 3$ is forced to
vanish by the absence of a first-order OPE pole.  No holographic
corrections beyond the leading Brown--Henneaux coefficient arise
because the bulk is Gaussian.  The candidate Drinfeld double is
$U_{\mathcal H_k} = \mathcal H_k \bowtie \mathcal H_k^!$, where
$\mathcal H_k^! = (\mathrm{Sym}^{\mathrm{ch}}(V^*),\,m_0 = -k\omega)$
is the curved dual assembled from the same generating space with
curvature $-k\omega$; at the level of cohomology this reduces to a
Heisenberg pair $\mathcal H_k\otimes\mathcal H_{-k}$ modulo the
diagonal unit, which assembles into the full bulk operator algebra
on this verification case.  Every clause of the reconstruction thesis is
exhibited, as a verified special case, on this one algebra:
boundary determines bulk (via the derived centre, computed here
directly), $\kappa$ is the holographic central charge (via $F_1 =
k/24$), the shadow tower is the correction hierarchy (trivial here,
and this is precisely why Heisenberg serves as the CG opening rather
than as an atom of $\Eone$: it is the commutative base case where
every clause of the reconstruction thesis becomes computable, while
the genuine $\Eone$ atom is the Yangian, AP108), and the candidate
Drinfeld double reproduces the abelian Chern--Simons bulk from
boundary data.  This verification case therefore provides the only
algebra in which the full boundary-bulk picture is unconditionally
available in Volume~I; the extension to nonabelian and higher-weight
algebras is the subject of the conjectural programme.
...
The bar complex produces five structural theorems.  Presented in
isolation they appear heterogeneous: an adjunction, an inversion, a
decomposition, a characteristic class, a cohomological bound.  Read
under the reconstruction thesis of this monograph they organise
into a single picture in five tempi: the five proved ingredients on
which a conjectural boundary-bulk reconstruction of a chiral algebra
through the chiral derived centre would rest.  Each theorem supplies
one structural fact about the pair $(\cA,\cA^{!}_\infty)$ and about
the modular characteristic.  The proved content of each theorem is
stated first; its conjectural role in the reconstruction thesis is
marked as such.
...
\emph{Role in the thesis (conjectural extension).}  Theorem~A
constructs the pair $(\cA,\cA^{!}_\infty)$ on which any candidate
Drinfeld double $U_\cA = \cA \bowtie \cA^{!}_\infty$ would be
defined: starting from a single boundary algebra it produces the
Verdier-dual factorization partner.  Whether that double functions
as a universal boundary-bulk reconstructor is the conjectural
content of Volume~II, Part~IV; Theorem~A provides only the pair on
which such a double can be built, and ensures that pair is
canonical rather than choice-dependent.
...
there.
\emph{Role in the thesis (conjectural extension).}  Theorem~B shows
that on the Koszul locus the boundary-to-bulk functor $\barB$ is
reversible by the cobar, so the boundary algebra can be recovered
from its bar complex.  Whether this lifts to faithfulness of a
bulk-reconstruction through the Drinfeld double is conjectural; at
the level of Volume~I, what Theorem~B proves is bar-cobar inversion
on the Koszul locus, not bulk faithfulness.  Off the Koszul locus,
inversion fails by a controlled amount, measured by the shadow
tower.
...
\emph{Role in the thesis.}  Theorem~C realises the sum rule
$\kappa(\cA)+\kappa(\cA^!)$ as the scalar shadow of a Lagrangian
splitting of the centre: what the boundary sees as a deformation,
the dual boundary sees as an obstruction, and their sum is the
anomaly of the bulk theory.  The splitting is the boundary-bulk
dictionary at the cohomological level.
...
\emph{Role in the thesis.}  Theorem~D identifies the modular
characteristic with the Brown--Henneaux-type central charge of the
bulk: $\kappa(\cA)$ is the single scalar that governs the leading
genus-tower behaviour, and the $\hat A$-genus generating function
is the precise analogue of the semiclassical expansion around the
bulk saddle.  Theorem~D is the holographic central charge theorem.
...
critical locus.
\emph{Role in the thesis (conjectural extension).}  Theorem~H bounds
the chiral Hochschild cohomology, and under the conjectural
identification of $\cZ^{\mathrm{der}}_{\mathrm{ch}}$ with the chiral
bulk this bound would give a finite-dimensional bulk with at most
four generators in three cohomological degrees at generic level.
This is what would make the holographic reconstruction \emph{tight}
on the conjectural programme: the bulk would be small enough that
the boundary algebra determines it unambiguously, and the
correction tower $\{S_r(\cA)\}_{r\ge 2}$ would live in
finite-codimension subspaces of that small bulk.  Without the
Theorem~H bound, the chiral Hochschild cohomology could be too
large for any such pinning to be possible.

\medskip

\noindent
Read together, the five theorems say: the pair $(\cA,\cA^{!}_\infty)$
...
modular characteristic $\kappa$ that governs the genus tower in the
...
single conjectural boundary-bulk picture, developed in the
four-part programme of Volume~II, Part~IV.  The proofs of A, B, C,
...
$\fg$ scaled by the level, matrix-valued and vanishing in the
abelian limit $k=0$.  Averaging collapses $k\,\Omega$, and the
Sugawara shift by $h^\vee$ yields
...
for all~$g\ge 1$ and all~$\lambda$
(Remark~\ref{rem:betagamma-cross-channel-vanishing}).
Two independent mechanisms force this: the single shadow channel
(one-dimensional cyclic cohomology) leaves no room for
mixed-channel assignments, and the off-diagonal Zamolodchikov
metric kills the propagator cross-term.
The scalar formula
$F_g(\beta\gamma)=\kappa(\beta\gamma)\cdot
\lambda_g^{\mathrm{FP}}$ is exact at all genera, not merely
uniform-weight.  This is a special case of a general principle:
free-field algebras (Heisenberg, $\beta\gamma$, $bc$, lattice)
have vanishing cross-channel correction at all genera, so
the scalar formula is exact without the uniform-weight hypothesis
...
MC1 through MC4 are proved; MC5 is partially proved.  The analytic HS-sewing package is proved at all genera; the genus-$0$ algebraic BRST/bar comparison is proved; BV=bar in the coderived category is proved for all four shadow classes (including class~M).  The genuswise chain-level BV/BRST/bar identification remains conjectural (class~M chain-level false), and the tree-level amplitude pairing is conditional.
