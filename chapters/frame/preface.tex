...
\medskip\noindent
The Trinity Theorem (Chapter~\ref{chap:universal-conductor})
records that $K$ is one functor with three equal incarnations:
$K_{E}=K_{c}=K_{g}$, that is, Euler characteristic of the bar
tower equals sum of central charges $c+c^{!}$ equals ghost charge
of the BRST resolution. Per-family kappa formulas
($\kappa(\cH_k)=k$, $\kappa(V_k(\fg))=\dim(\fg)(k+h^{\vee})/(2h^{\vee})$,
$\kappa(\mathrm{Vir}_c)=c/2$, $\kappa(\cW_N)=c(H_N-1)$) collapse to one
identity functorial in~$\cA$.

\medskip\noindent
The shadow obstruction tower of $\Theta_\cA$ obeys a master
Riccati identity $H(t)^{2}=t^{4}Q_c(t)$ on each primary
deformation line. The Quadrichotomy Theorem
(Chapter~\ref{chap:shadow-quadrichotomy-platonic}) forces
$r_{\max}\in\{2,3,4,\infty\}$ and partitions the standard
landscape into classes $\mathbf{G}/\mathbf{L}/\mathbf{C}/\mathbf{M}$
with critical companion $\mathbf{FF}$, with five Picard--Fuchs
corollaries (master Maurer--Cartan recursion, spectral curve,
Borel summability, shadow--Feynman dictionary, mock modularity).
A fifth archetype $\mathbf{B}$ -- the BKM crown $\mathbf H_{\Delta_5}$
of Lorentzian-lattice-parametric origin -- shares $r_{\max}=\infty$
with class~$\mathbf{M}$ but is separated by the Mukai-doubling
conductor $\kappa+\kappa^!=8=2c_+(\mathrm{Mukai}(K3))$; the enlarged
Theorem-C spectrum is
...
sums are exactly $0$, $8$, $13$, $250/3$, and $98/3$. The new value
$8$ is the Lorentzian-lattice-parametric $\mathcal{B}$-family witness
$\mathcal{H}_{\mathrm{Muk}}(K3)$: the Mukai lattice
$\widetilde H(K3,\mathbb{Z}) = H^0 \oplus H^2 \oplus H^4$ has rank
$1+22+1=24$, hence Heisenberg central charge $c=24$, while its
positive index is $c_+=4$; the CY$_2$ Serre pairing reads this
positive Mukai contribution on both Verdier halves, producing
$K^{\kappa_{\mathrm{ch}}}=2c_+=8$. Bruinier 2002, Proposition~5.1 is
the arithmetic reason the same integer controls the Humbert-$H_1$
monodromy and the family identity
...
through $\SCchtop$ and the topologized bulk; Volume~III constructs the
geometric source via the Calabi--Yau-to-chiral functor $\Phicy$;
Volume~IV establishes the HEAL and UPGRADE theorems and aligns the
independent-verification registry across all four volumes. The
...
deformation. The derived centre recovers the bulk.
...
Chapter~\ref{ch:k3-chiral-bialgebra-platonic} Theorem
...
Calabi--Yau-to-chiral source through $\Phicy$; Volume~IV establishes
the complete independent-verification registry across all four
volumes. On the non-degenerate locus
$\{\kappa\ne 0,\ \text{non-critical level},\ \text{Koszul locus}\}$
...
in Chapter~\ref{ch:k3-chiral-bialgebra-platonic}: at
...
\noindent\textbf{Pseudo-character $S^{\mathrm{ps}}$ and the Galois
bridge.}\enspace
...
pseudo-character $S^{\mathrm{ps}}\colon
\mathbb T^{\mathrm{par}}_1\to\mathbb C$ with
$S^{\mathrm{ps}}_1(T_p)=\lambda_p=a_p(f_{16})+p^8+p^9$ and
$S^{\mathrm{ps}}_4(T_p,T_p,T_p,T_p)=p^{32}$
(verified on the twenty-two primes $p\le 79$). Under the
Saito--Kurokawa lift
$f_{16}\mapsto\mathrm{SK}(f_{16})=\Delta_5$,
$S^{\mathrm{ps}}$ is the chiral-bialgebra trace on
$\mathrm{ChirHoch}^2(\mathbf H_{\Delta_5})$; on the Galois side
it is the trace of the $4$-dimensional
$\mathrm{GSp}_4$-representation attached to~$\Delta_5$. The
bridge $S^{\mathrm{ps}}\leftrightarrow\rho_{\Delta_5}$ links the
Theorem-C crown value $\kappa+\kappa^!=8$ to the Euler system of
Hecke eigenvalues on the paramodular locus, and
$\chi_3$ of Theorem~H is the degree-$3$ pseudo-character trace
...
inscribed through $n=32$. Its dimension sequence
...
The Trinity Theorem $K_E=K_c=K_g$ records that Euler characteristic,
sum of central charges, and ghost charge are three names for the
same number. The standard landscape census (Heisenberg, free fermion,
...
claim is:
\[
\text{if }A_X:=\Phi_3(D^b(\mathrm{Coh}(X)))\text{ admits a quasi-free BRST resolution, then }
\kBKM(X)\;=\;\tfrac12\,K(A_X).
\]
...
Lorentzian-lattice-parametric $\mathcal{B}$-family term
$K^{\kappa_{\mathrm{ch}}}=8$, enlarging the Theorem-C bucket set by a
...
Volume~III constructs the geometric source: the
Calabi--Yau-to-chiral functor $\Phicy$, scope-dependent on complex
dimension~$d$ (an $E_2$-chiral output for $d\le 2$, an $E_1$-chiral
output for $d\ge 3$), produces the K3~Yangian, the
Borcherds--Kac--Moody conductor $\kBKM=c_N(0)/2$, and the class-$M$
$E_3$ bar of dimension~$6^g$ (at $g\in\{1,2,3\}$; $g\ge 4$ conditional
on $d_5$ degeneration per Volume~III \ref{V3-cor:class-m-higher-genus}),
closing the circle by supplying the
input that Volumes~I and~II assumed.
Volume~IV establishes the HEAL and UPGRADE theorems in explicit form,
extends the independent-verification decorators to further families,
and aligns the computational infrastructure of the three
preceding volumes into a single registry; it contains no new
...
% preface compilation. The wave-14 chapter anchors
...
% references introduced in the wave-14 climax framing.
% =====================================================================
...
% prop:pbw-koszulness-criterion stub retired 2026-04-18 (Wave-15): the
% real label is thm:pbw-koszulness-criterion at chapters/theory/chiral_koszul_pairs.tex:788;
% two consumers (preface.tex, programme_overview_platonic.tex) retargeted
% from "Proposition~\ref{prop:...}" to "Theorem~\ref{thm:...}" (AP125
% environment-prefix match).
% chap:infinite-fingerprint-classification stub retired 2026-04-18 (Wave-15):
% second \label{chap:infinite-fingerprint-classification} established at the
...
\phantomsection\label{chap:universal-holography-functor}% alias: Vol II chapter universal_celestial_holography.tex (cross-volume, AP286-legitimate)%
\phantomsection\label{chap:periodic-cdg-admissible}%
\phantomsection\label{chap:theorem-A-modular-family}% alias: this chapter is the CONDITIONAL modular-family extension of thm:A-infinity-2 (scope remark rem:A-infinity-2-modular-family-scope at theorem_A_infinity_2.tex:890); not separately established pending GR17 + Mok25 inscription, see notes/true_frontier_2026_04_17.md%
\phantomsection\label{chap:chiral-higher-deligne}% alias: Vol II chapter chiral_higher_deligne.tex (cross-volume, AP286-legitimate)%
\phantomsection\label{thm:completed-bar-cobar-strong}%
% thm:grt1-rigidity phantomsection stub retired 2026-04-18: the GRT_1
...
% retired 2026-04-18 attack-and-heal (AP255/AP286 resolution).
...
Volume~III asks where it comes from (the CY-to-chiral correspondence
programme $\{\Phi_d\}$ composed with the Borcherds singular-theta
...
\section{The preface through the K3 chiral bialgebra}
\label{sec:v1-preface-supplement}

\begin{remark}[Preface through the K3 base]
\label{rem:v1-preface-1}
Read through the K3 base, Vol~I's preface enters the
bar-cobar-Koszul programme at $\mathbf H_{\Delta_5}$,
the fourth taxon in the Etingof--Kazhdan classical/elliptic/rational
trichotomy, constructed as the chiral Hall--Drinfeld double
...
The preface's five-theorem summary (A bar-cobar, B chiral Positselski,
C derived-centre complementarity, D obstruction-tower universality,
H Hochschild concentration) at the K3 base point becomes five
K3-specific sharpened statements, each pinning a face of
$\mathbf H_{\Delta_5}$ \cite{EtingofKazhdan2007PartV,GritsenkoNikulin1998}.
Cross-ref Vol.~III Chapter~\ref{ch:k3-chiral-bialgebra-platonic}.
\end{remark}

\begin{remark}[Mukai doubling $K^{\kappa_{\mathrm{ch}}} = 8$ in the preface]
\label{rem:v1-preface-2}
The Mukai doubling $K^{\kappa_{\mathrm{ch}}} = 8$ (universal identity
at Lusztig specialisation $\hbar^2 = -1/8$) reads the preface's CY-chiral
correspondence through three faces: Mukai-pairing $(4,20)$ signature
($c_+ = 4$, $c_- = 20$, doubling $2c_+ = 8$); Humbert-divisor monodromy
order on $\bar{\mathcal A}_2$; Lusztig root-of-unity order $\ell = 8$
at the quantum-group specialisation. The $\mathbb Z/8$-class is the
preface's hidden axis, making Vol~I's Theorem~B (chiral Positselski)
a K3-specific Koszul equivalence at $\hbar^2 = -1/8$
\cite{Mukai1987,Lusztig1990,Bruinier2002}. Cross-ref
Chapter~\ref{ch:k3-chiral-bialgebra-platonic}.
\end{remark}

\begin{remark}[Bi-based Ran factorisation as preface thesis]
\label{rem:v1-preface-3}
The preface's factorisation thesis -- ``chiral algebras live on
configuration spaces'' -- refines at the K3 base point to the
bi-based Ran pair $(\mathrm{Ran}(E^{\mathrm{nod,sm}}_{24}),\bar{\mathcal A}_2)$
...
$E^{\mathrm{nod,sm}}_{24}$; the Kuga--Satake map forces the $\bar{\mathcal A}_2$
base. The preface's framing "bar-cobar on chiral side" becomes "bar-cobar
for the $\mathbf H_{\Delta_5}$-module category on the bi-based Ran side"
...
The preface's quadrichotomy $G/L/C/M$ has a paramodular
pre-image. Over the Baily--Borel--Freitag stratification of
...
deformation is the arithmetic shadow of the preface's scope
qualifier ``non-degenerate locus''; the Bloch--Kato vanishing off
the generic locus is the arithmetic version of the Koszul
obstruction. Cross-ref
Chapter~\ref{ch:k3-chiral-bialgebra-platonic}.
\end{remark}

\begin{remark}[Metaplectic Grothendieck--Teichm\"uller $\mathrm{grt}_1^{(1/2)}$]
\label{rem:v1-preface-8}
The preface's $\GRTone(\bQ)$-torsor action on the face lattice
