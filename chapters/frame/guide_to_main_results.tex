% ======================================================================
% GUIDE TO MAIN RESULTS
% A compact reference for the five main theorems and their dependencies.
% ======================================================================

\chapter*{Guide to main results}
\addcontentsline{toc}{chapter}{Guide to main results}
\label{ch:guide}

\noindent
This guide lists the principal results proved in the monograph,
with labels, locations, and logical dependencies.  All five
theorems hold for augmented chiral algebras~$\cA$ on a smooth
projective curve~$X$ satisfying the standing finiteness
conditions (\S\ref{sec:standing-assumptions}).

\medskip

\renewcommand{\arraystretch}{1.3}
\begin{longtable}{@{}lp{8.5cm}l@{}}
\textsc{Label} & \textsc{Statement} & \textsc{Location} \\
\hline
\endfirsthead
\textsc{Label} & \textsc{Statement} & \textsc{Location} \\
\hline
\endhead
\textbf{Theorem A} &
  Bar--cobar adjunction: $\barB_X \dashv \Omega_X$ on
  $\operatorname{Alg}_{\mathrm{aug}}(\operatorname{Fact}(X))
  \rightleftarrows
  \operatorname{CoAlg}_{\mathrm{conil}}(\operatorname{Fact}(X))$,
  intertwined with Verdier duality:
  $\mathbb{D}_{\operatorname{Ran}} \bar{B}_X(\cA)
  \simeq \cA^!_\infty$ (underlying complex
  $\simeq \bar{B}_X(\cA^!)$). &
  Thm~\ref{thm:bar-cobar-isomorphism-main} \\[4pt]

\textbf{Theorem B} &
  Bar--cobar inversion: on the Koszul locus, the counit
  $\Omega_X \bar{B}_X(\cA) \xrightarrow{\sim} \cA$ is a
  quasi-isomorphism at all genera. &
  Thm~\ref{thm:higher-genus-inversion} \\[4pt]

\textbf{Theorem C} &
  Complementarity: $Q_g(\cA) \oplus Q_g(\cA^!)
  \cong H^*(\overline{\cM}_g, \cZ_\cA)$ for all $g \ge 1$.
  Upgraded to shifted-symplectic Lagrangian geometry. &
  Thm~\ref{thm:quantum-complementarity-main} \\[4pt]

\textbf{Theorem D} &
  Modular characteristic: $\kappa(\cA)$ is universal,
  additive under direct sums, constrained by Koszul
  duality ($\kappa + \kappa' = 0$ for KM/free fields),
  and generates the genus tower via the $\hat{A}$-genus. &
  Thm~\ref{thm:modular-characteristic} \\[4pt]

\textbf{Theorem H} &
  Chiral Hochschild: $\ChirHoch^*(\cA)$ has polynomial
  growth, is Koszul-functorial, and admits an explicit
  Koszul resolution of length bounded by the number
  of generators. &
  Thm~\ref{thm:hochschild-polynomial-growth} \\[4pt]

\hline\\[-8pt]
\multicolumn{3}{@{}l}{\textsc{Additional principal results}} \\[2pt]
\hline

\textbf{MC element} &
  The universal Maurer--Cartan element
  $\Theta_\cA := D_\cA - \dzero \in
  \MC(\mathfrak{g}_\cA^{\mathrm{mod}})$ is proved by the
  bar-intrinsic construction: $D_\cA^2 = 0$ implies the
  MC equation. &
  Thm~\ref{thm:mc2-bar-intrinsic} \\[4pt]

\textbf{Koszulness} &
  Twelve equivalent characterizations of chiral Koszulness:
  ten unconditionally proved equivalent, one conditional
  (Lagrangian criterion, pending perfectness/nondegeneracy),
  one partially proved (D-module purity: forward direction
  established, converse open). &
  Thm~\ref{thm:koszul-equivalences-meta} \\[4pt]

\textbf{MC3 (all types)} &
  Thick generation of Yangian category~$\mathcal{O}$ for all simple~$\mathfrak{g}$ via
  categorical prefundamental Clebsch--Gordan closure (multiplicity-free $\ell$-weights), Francis--Gaitsgory completion,
  and DK on compacts.  &
  Cor~\ref{cor:mc3-all-types} \\[4pt]

\textbf{MC4} &
  Strong completion-tower theorem:
  completed bar--cobar homotopy equivalence on
  $\CompCl(\mathcal{F}_{\mathrm{ft}})$. &
  Thm~\ref{thm:completed-bar-cobar-strong} \\[4pt]

\textbf{MC1 (PBW)} &
  PBW spectral sequence degenerates at $E_2$ for all
  standard families (Kac--Moody, Virasoro, $\cW_N$,
  free fields, lattice). &
  Prop~\ref{prop:pbw-universality} \\[4pt]

\textbf{MC2 ($\Theta_\cA$)} &
  Bar-intrinsic construction of the universal MC element:
  $\Theta_\cA := D_\cA - \dzero$ is MC because $D_\cA^2 = 0$.
  Scalar saturation proved for all algebraic families. &
  Thm~\ref{thm:mc2-bar-intrinsic} \\[4pt]

\textbf{MC5 (sewing)} &
  General HS-sewing criterion: polynomial OPE growth $+$
  subexponential sector growth $\Longrightarrow$ convergence at
  all genera.  Heisenberg sewing via one-particle Bergman
  reduction and Fredholm determinant. &
  Thm~\ref{thm:general-hs-sewing} \\[4pt]

\textbf{Ambient $D^2 = 0$} &
  At the ambient level (with planted-forest corrections),
  $D^2 = 0$ is proved via Mok's log-FM normal-crossings
  result. &
  Thm~\ref{thm:ambient-d-squared-zero} \\[4pt]

\textbf{All-arity convergence} &
  The full shadow tower converges:
  $\Theta_\cA = \varprojlim \Theta_\cA^{\le r}$ exists. &
  Thm~\ref{thm:recursive-existence} \\[4pt]

\textbf{$\Eone$ A--H} &
  All five main theorems proved in the ordered ($\Eone$)
  setting at all genera: ribbon modular operad, planar
  planted forests, spectral $R$-matrix. &
  Thms~\ref{thm:e1-theorem-A-modular}--\ref{thm:e1-theorem-H-modular} \\[4pt]
\end{longtable}

\subsection*{The MC frontier}

The five MC conjectures, which together constitute the
programme of extending bar--cobar duality to the full modular
envelope, are resolved at the mathematical level (the full
BV/BRST/bar identification at higher genus remains conjectural;
see Chapter~\ref{chap:editorial-constitution}):
\begin{enumerate}[label=\textbf{MC\arabic*},leftmargin=3em]
\item \textbf{PBW concentration}: proved for all standard families
  (Proposition~\ref{prop:pbw-universality}).
\item \textbf{Bar-intrinsic MC element}: $\Theta_\cA := D_\cA - \dzero$
  is automatically MC because $D_\cA^2 = 0$
  (Theorem~\ref{thm:mc2-bar-intrinsic}).
\item \textbf{Categorical thick generation}: proved for all simple
  Lie types via multiplicity-free $\ell$-weights
  (Corollary~\ref{cor:mc3-all-types}).
\item \textbf{Completion towers}: strong completion-tower theorem
  (Theorem~\ref{thm:completed-bar-cobar-strong}).
  MC4 splits into MC4$^+$ (positive towers, solved) and
  MC4$^0$ (resonant towers, reduced to finite resonance).
\item \textbf{Sewing at all genera}: general HS-sewing criterion
  (Theorem~\ref{thm:general-hs-sewing}).
\end{enumerate}

\subsection*{The shadow Postnikov tower}

The universal MC element $\Theta_\cA$ has finite-order
projections $\Theta_\cA^{\le r}$, each a graph sum through
arity~$r$ with obstruction class
$\mathfrak{o}_{r+1}(\cA) \in \mathrm{Def}_{\mathrm{cyc}}^{\mathrm{mod}}(\cA)$.
The named shadows are:
\[
\underbrace{\kappa}_{\text{arity 2}}
\;\to\;
\underbrace{C}_{\text{arity 3}}
\;\to\;
\underbrace{Q}_{\text{arity 4}}
\;\to\; \cdots
\]
Shadow depth classifies complexity within the Koszul world:
class~G ($r_{\max} = 2$, Heisenberg), class~L ($r_{\max} = 3$,
affine), class~C ($r_{\max} = 4$, $\beta\gamma$), class~M
($r_{\max} = \infty$, Virasoro/$\cW_N$).  The full tower
$\Theta_\cA = \varprojlim \Theta_\cA^{\le r}$ converges
(Theorem~\ref{thm:recursive-existence}).

\subsection*{Dependency structure}

The logical dependencies among the main theorems are:
\begin{center}
\begin{tikzcd}[column sep=small, row sep=small]
  & \text{A} \ar[r] \ar[d] & \text{B} \\
  & \text{D} \ar[r] & \text{C} \\
  \text{H} & & \Theta_\cA \ar[u] \ar[ul]
\end{tikzcd}
\end{center}
Theorem~A (the adjunction) is logically first.  Theorem~B
(inversion) requires~A.  Theorem~D (modular characteristic)
requires~A at genus~$0$ and extends inductively.  Theorem~C
(complementarity) requires~D.  Theorem~H (Hochschild) is
independent.  The MC element $\Theta_\cA$ depends on~A and
implies~D as a projection.

\subsection*{Notation}

$\barB$: bar functor.  $\Omega$: cobar functor.
$\cA^!$: Koszul dual.
$\kappa(\cA)$: modular characteristic.
$Q_g(\cA)$: genus-$g$ partition complex.
$\ChirHoch^*$: chiral Hochschild cohomology.
$\gAmod$: modular convolution dg~Lie algebra.
$\Theta_\cA$: universal MC element.
$\Eone$, $\Einf$: associative and commutative operadic levels.
$\FM_n(X)$: Fulton--MacPherson compactification.
