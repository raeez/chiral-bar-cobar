...
\item \textbf{Face-model DDYBE, scope-restricted}
  ...
  $<10^{-4}$ ($4$ generic-$\Omega$ compute tests, plus a
  tier-(T3) diagonal-$\Omega$ consistency check). The
  separating-degeneration regime carries no genuinely $g=2$
  content.
...
\item \textbf{Generic $\Omega$ regime (numerical, residual
  ...
  \texttt{relative < 1e-4}, across four distinct generic-$\Omega$
  parameter choices: two distinct $\boldsymbol\lambda$, two
  distinct $(z,w)$ spectral pairs, and $\eta=0.25, 0.2, 0.4$
  (levels $k=2,3$ and strong-coupling $\eta=0.4$). The sweep
  covers a codimension-$0$ subset of the three-complex-dimensional
  moduli $\HHH_2$ and provides pointwise, not uniform, evidence.
  A fifth test at $\Omega=\mathrm{diag}(1\mathrm{i},1.3\mathrm{i})$,
...
\texttt{test\_ddybe\_strong\_coupling}), and the diagonal-$\Omega$
sanity check
\texttt{test\_ddybe\_diagonal\_omega} (tier T3, consistency
with regime~(i)), the residual passes the stated threshold.
The $10^{-4}$ ceiling reflects the $N=8$ truncation of the
genus-$2$ theta series (an engineering budget, not a theoretical
error term of the form $O(\hbar^2)$); at $N=12$ the residual
empirically drops to $10^{-6}$, and the $10^{-12}$ figure
advertised in the prior CLAUDE.md row is not attained without
a higher-precision arithmetic backend. This is \emph{numerical
evidence} for \texttt{conj:g2-ddybe} at finitely many parameter
points, not a proof and not a uniform bound across $\HHH_2$.

\emph{Proof of~(iii) (factorised-limit).}
...
are preparatory or degeneration checks. Total face-model
test count: $30$ (across \texttt{TestThetaFunctions},
...
weight unitarity. It is distinct from the three-term Szeg\H{o}
trisecant identity~\eqref{eq:fay-three-term}
$S_{12}S_{23}+S_{23}S_{31}+S_{31}S_{12}=0$
(Fay 1973 Cor.~2.5) which governs the bar-complex Stokes
analysis in Remark~\ref{rem:why-a-not-b}. Both are theta
identities due to Fay; only~\eqref{eq:fay-three-term} is
the trisecant form. The programme uses the trisecant form in
the bar-differential $d_{\mathrm{bar}}^2 = 0$ proof and the
Boltzmann-unitarity form in the face-model numerical checks.
\end{remark}
...
\item ``Fay trisecant extends to genus~$2$'' $\rightarrow$
  ...
  holds uniformly at all $g\ge 0$ by direct application, no
  extension argument needed; distinct from both the
  four-theta-nulls quartic identity on the theta divisor and
  the Boltzmann-weight unitarity identity
  $\alpha\beta-\gamma\delta = \Theta(z-\eta)/\Theta(z+\eta)$
  tested numerically''.
\item ``$29$ tests'' $\rightarrow$ ``$30$ face-model tests, of
  which $4$ are genuinely generic-$\Omega$ DDYBE at
  \texttt{relative < 1e-4}; a $5$th DDYBE test uses
  diagonal-$\Omega$ as a tier-(T3) consistency check with the
  genus-$1$ Felder theorem, and the remaining $25$ tests
  verify genus-$1$ DYBE, $\theta$ antisymmetry, unitarity,
