...
it is NOT a theoretical error term of the form $O(\hbar^2)$.
...
The sampling is POINTWISE at a pair of $\Omega$ values;
it does NOT witness the full three-complex-dimensional
$\Omega$-dependence of the claim, nor does it probe the
non-separating boundary stratum where $\Omega_{22}\to
i\infty$ with $\Omega_{12}$ fixed nonzero
(Remark~\ref{rem:g2-nonseparating-untested}). It does NOT
establish: an exact proof of \eqref{eq:ddybe} at
...
test count: $32$ after the 2026-04-18 attack-heal additions
(across \texttt{TestThetaFunctions},
...
\begin{remark}[Falsification-ladder discipline]
\label{rem:tolerance-ladder-falsification}
The $N=8$ truncation of the $32$ face-model tests above is a
single snapshot on a falsification ladder. The regression test
...
that the current engineering-budget bound $10^{-4}$ is not
sensitive enough to rule out, and would force a downgrade of
regime~(ii) from ``numerical evidence'' to ``numerical
evidence with explicit residual $r_{\infty}>0$.'' As of the
present session the ladder itself has not been numerically
executed in the sandbox (numpy unavailable); the deferred
execution is logged in
\texttt{adversarial\_swarm\_20260418/attack\_heal\_g2\_ddybe\_20260418.md}
\S6.
\end{remark}
