...
logarithmic normal forms \(\eta_{ij}\), represented by
\(d\log(z_i-z_j)\) only on affine/formal collision screens.  At
positive genus the total modular equation remains a flat MC equation;
the new term is the
...
configuration spaces of~\(X\), governed locally by the Arnold relation
for the affine/formal representatives \(d\log(z_i-z_j)\) of the
logarithmic normal forms along collision diagonals.  The terms \(D^{(g)}\) for
...
\]
The diagonal graph functional is
$F_g^{\mathrm{diag}}(\cA)=\kappa(\cA)\lambda_g^{\mathrm{FP}}$.
Under~$H_D^K$, the corresponding Hodge character is
...
their diagonal scalar term is $\kappa(\cA)\lambda_g^{\mathrm{FP}}$, and their full
...
\begin{definition}[Modular homotopy package; \ClaimStatusDefinitional]
...
\item $W_\Gamma$ is the geometric Feynman weight, and
 ...
 half-edges through the cyclic/BV pairing.
	 ...
	 hypotheses of Definition~\ref{def:hs-sewing}, each fixed
...
the standard quadratic bracket normalization.  The second term is the
...
normalized Moore-degree calculation for the Eilenberg--Zilber action
is the two-element cover argument of
Proposition~\ref{prop:cech-two-element-strict}: every higher
component $F_n$ with $n\geq 3$ lands beyond the degree-$1$ window of
the Moore complex. Hence the target \v{C}ech group vanishes.
...
\begin{definition}[Secondary Borcherds operations; \ClaimStatusDefinitional]
