...
% Regimes II--III: curved and filtered (Convention~\ref{conv:regime-tags}).

The MC element $\Theta_\cA$ satisfies
$D\Theta_\cA + \tfrac{1}{2}[\Theta_\cA, \Theta_\cA] = 0$; the
genus-$g$ components of this equation are the quantum corrections to
the genus-$0$ bar differential. At genus~$0$, the Arnold relation
ensures $d^2 = 0$; at genus~$g \geq 1$, it deforms by coherent
higher homotopies whose leading term is the modular curvature
$\kappa(\cA)$.

\begin{definition}[Quantum corrections]\label{def:quantum-corrections}
\index{quantum corrections|textbf}
In this monograph, \emph{quantum corrections} are the
genus-$g \geq 1$ components of the modular bar differential
that vanish on genus-$0$ configuration spaces. Precisely:
the total differential $D = \sum_{g \geq 0} \hbar^g D^{(g)}$
splits by loop genus; the genus-$0$ part $D^{(0)}$
is the chiral bar differential on configuration spaces of~$X$,
governed by the Arnold relation for the logarithmic forms
$\eta_{ij} = d\log(z_i - z_j)$ on $\FM_n(X)$
(this is \emph{not} the classical bar differential over a point,
which retains no configuration-space geometry), and the
higher-genus parts $D^{(g)}$ for $g \geq 1$ are the quantum
corrections.
The parameter~$\hbar$ is a formal loop-counting variable
with $|\hbar| = 0$; it tracks the genus grading and has
no physical content beyond bookkeeping.
\end{definition}

When strict Arnold relations fail at genus $g \geq 1$,
they fail by coherent higher homotopies: quantum
corrections are new higher operations in the homotopy
algebra, not errors.

Four principles govern the structure.
...
identity in $\Convinf(\barBch(\cA), \cA)$. When the curve acquires
non-trivial periods, that identity deforms by an Eisenstein
correction whose leading term is $\kappa(\cA)$.
\emph{Modularity at chain level.} The correction is a
Maurer--Cartan deformation of the differential in the modular
convolution algebra $\mathfrak{g}^{\mathrm{mod}}_{\cA}$, dictated by
the Feynman transform of the modular operad and recorded in the
shadow obstruction tower of Chapter~\ref{sec:koszul-across-genera}.
\emph{Examples.} Each standard family behaves differently:
Heisenberg terminates at degree~$2$ (pure curvature), affine algebras
at degree~$3$ (cubic shadow), $\beta\gamma$ at degree~$4$ (quartic
contact), while Virasoro and $\mathcal{W}_N$ produce infinite towers
forced by non-vanishing quintic obstructions.
\emph{Closing the loop.} Every quantum correction computed here
reappears as input to Theorem~C (complementarity,
Chapter~\ref{chap:higher-genus}) and Theorem~D (modular
characteristic, Chapter~\ref{ch:genus-expansions}), confirming that
the corrections assemble into a universal, additive,
duality-constrained invariant.

Strict genus-$0$ nilpotence breaks once the propagator remembers the
periods of the curve. Curvature terms, Eisenstein regularizations,
and their assembly into a curved $A_\infty$ package constitute the
explicit correction mechanism behind the higher-genus theory. The
Heisenberg computation of Chapter~\ref{ch:heisenberg-frame} is the
frame case; the genus-$1$ identity already exhibits the universal
pattern.
...
These relations are the integrability conditions for the geometric bar complex: $d^2 = 0$ is flatness of the Knizhnik--Zamolodchikov connection on the local system of conformal blocks \cite[Section~3.4]{BD04}.
...
We derive the defect from first principles via the Eisenstein-regularised
Laurent expansion of~$\zeta$.
...
\mathcal{A}_3^{(1)}
= \bigl[\zeta(u)\zeta(v)
 + \zeta(v)\zeta(w) + \zeta(w)\zeta(u)\bigr]
 \cdot \alpha \wedge \beta.
\end{equation}

\emph{Step~2: Pole cancellation.}
Denote the coefficient function $S(u,v) = \sum_{\mathrm{cyc}}\zeta(u)\zeta(v)$.
...
Therefore $S$ is an \emph{entire} function of~$(u,v)$.

\emph{Step~3: Laurent expansion via Eisenstein summation.}
The Weierstrass $\zeta$-function admits the
Eisenstein-regularised expansion
(cf.~\S\ref{sec:km-bar-complex}):
\begin{equation}\label{eq:zeta-eisenstein-expansion}
\zeta(z|\tau)
= \frac{1}{z} - G_2(\tau)\,z - G_4(\tau)\,z^3 - G_6(\tau)\,z^5 - \cdots
\end{equation}
where $G_{2k}(\tau) = \sum'_{(m,n)} (m + n\tau)^{-2k}$
and $G_2(\tau) = \tfrac{\pi^2}{3}\,E_2(\tau)$ requires
Eisenstein summation (inner sum over~$m$ first).
The quasi-period is $2\eta_1 = G_2(\tau)$.

\emph{Step~4: Constant term of $S$.}
Substituting~\eqref{eq:zeta-eisenstein-expansion} into $S$
...
\emph{Step~5: Higher-order terms are cohomologically trivial.}
The position-dependent corrections~\eqref{eq:arnold-higher}
are polynomial in $(u,v)$ with $\tau$-dependent coefficients.
Since $\alpha \wedge \beta = du \wedge dv$ and any polynomial
$p(u,v)\,du \wedge dv$ on~$\mathbb{C}^2$ is exact
(primitives are $\partial\bar\partial$-exact on the
universal cover and descend to exact forms on~$C_3(E_\tau)$
because they have at most polynomial growth), they do not
contribute to the cohomology class
$[\mathcal{A}_3^{(1)}] \in H^{2,0}(C_3(E_\tau))$.

\emph{Step~6: The Arnold defect.}
...
($q = e^{2\pi i \tau}$).
The right-hand side is a holomorphic $(2,0)$-form on
$C_3(\Sigma_\tau)$ whose cohomology class is
$\pi^2 E_2(\tau) \cdot [\alpha \wedge \beta]$;
the factor~$\pi^2$ is the residue of the Eisenstein
regularisation and is absorbed into $\kappa(\mathcal{A})$
when the Arnold defect is contracted against the OPE data
to produce the bar-complex curvature
...
As at genus~$1$, the singular terms cancel and the defect reduces to
a $(2,0)$-form on $C_3(\Sigma_2)$ whose cohomology class depends on
the period matrix~$\Omega$. The genus-$2$ analogue of the
Eisenstein correction $3G_2(\tau)$ of~\eqref{eq:arnold-defect-genus1}
involves the Siegel Eisenstein series $E_2^{(2)}(\Omega)$ and
determines the genus-$2$ bar curvature
$m_0^{(2)} = \kappa(\cA) \cdot \lambda_2$
(Theorem~\ref{thm:genus-universality}). The explicit graph-sum
computation of the planted-forest correction $\delta_{\mathrm{pf}}^{(2,0)}$
at genus~$2$ appears in~\S\ref{sec:koszul-across-genera}.

\section{\texorpdfstring{The $A_\infty$ structure}{The A-infinity structure}}

The $A_\infty$ structure (Stasheff~\cite{Sta63}) encodes the higher coherences for consistency across genera.
...
cohomology of $\overline{\mathcal{M}}_g$, not through the degree of the
$A_\infty$ operations.
...
chiral Koszul dual is $(\mathcal{W}^k(\mathfrak{g}))^! =
\mathcal{W}^{k'}(\mathfrak{g})$ where
$k' = -k - 2h^\vee$ is the Feigin--Frenkel involution
(Chapter~\ref{chap:w-algebra-koszul}). This is chiral Koszul
duality within the $\mathcal{W}$-algebra family, not the
Feigin--Frenkel--Langlands correspondence (which relates the
critical-level center to opers on $\mathfrak{g}^\vee$).
Koszul complementarity (Theorem~C) then gives
\[\dim \mathcal{Q}_g(\mathcal{W}^k(\mathfrak{g})) + \dim \mathcal{Q}_g(\mathcal{W}^{k'}(\mathfrak{g})) = \dim H^*(\overline{\mathcal{M}}_{g}, Z(\mathcal{W}^k(\mathfrak{g})))\]
where $Z(\mathcal{W}^k(\mathfrak{g}))$ is the center, viewed as a local system on $\overline{\mathcal{M}}_g$.
...
(Definition~\ref{def:spectral-quantum-group}). The \emph{modular homotopy
package} is
\[
...
full $L_\infty$ structures on both sides, so homotopy transfer (Appendix~\ref{app:homotopy-transfer})
...
not an ad hoc perturbative approximation.
\end{remark}
...
\item $W_\Gamma$ is the \emph{Feynman weight} of the graph,
 defined as follows. Assign to each internal edge~$e$
 the propagator $h_e = K^{(g_e)}(z_{s(e)}, z_{t(e)})$
 (the genus-$g_e$ Cauchy kernel), and to each vertex~$v$
 the classical operation
 $m_{|v|}^{\mathrm{cl}}$ evaluated at the adjacent fields.
 Then
 \[
 W_\Gamma
 \;:=\;
 ...
 \cdot
 \prod_{v \in V(\Gamma)} m_{|v|}^{\mathrm{cl}},
 \]
 composed via the contraction map $\mathrm{Cont}_\Gamma$.
 When $\Gamma$ has internal loops ($g(\Gamma) > 0$),
 the loop contractions $\sum_a e_a \otimes e^a$ produce
 the BV operator~$\Delta$. The product is well-defined
 (finite, no regularization needed) when the OPE has
 finitely many singular terms, which holds for all
 chiral algebras with polynomial OPE growth
 (\S\ref{subsec:hs-sewing}). The term
 ``regularized'' in earlier versions of this chapter
 referred to this finiteness condition; no analytic
 regularization (dimensional or otherwise) is used.
\end{itemize}
\end{definition}
...
\begin{proof}
The first sum is the genus-splitting term; the second is
the loop-contraction/BV term. Together they are the
...
\S\ref{sec:derived-dk}). Its consistency
...
and the genus-raising operations $\ell_{g,n}$). A theorem
...
For quasi-linear theories, the transferred $A_\infty$ operations computed by
homotopy transfer
(Definition~\ref{def:tree-level-ainfty})
are exact, not leading-order approximations. The only quantum corrections enter
...
computed in Part~\ref{part:characteristic-datum} via the Laplace transform of the
...
Physically, loop corrections in the
...
\item \emph{The bar differential is exact.} For quasi-linear theories,
 ...
 enter through the curvature $m_0^{(g)} = \kappa(\cA) \cdot \omega_g$
 (i.e.\ $m_1^2(a) = [m_0^{(g)}, a]$), mediated by the BV
 operator~$\Delta$.
\item \emph{The spectral $r$-matrix is exact.} The line-operator OPE
 kernel $r(z)$ computed in Part~\ref{part:characteristic-datum} for each standard family is the
...
Affine $\widehat{\mathfrak{sl}}_2$ & $j' \neq 0$ at degree $3$ & Lie/tree \\
$\beta\gamma$ & $j' \neq 0$ at degree $3$; $j'_4 = 0$ & Contact/quartic \\
Virasoro/$W_N$ & $j' \neq 0$ at all degrees & Mixed (infinite tower) \\
\bottomrule
\end{tabular}
\end{center}
This classification coincides with the shadow-tower termination
pattern: the degree at which secondary Borcherds operations first
vanish equals the degree at which the shadow obstruction tower terminates
(Heisenberg at~$2$, affine at~$3$, $\beta\gamma$ at~$4$,
Virasoro never).
\end{remark}

\begin{proposition}[Secondary Borcherds operations as shadow obstruction tower
...
operations $F_n$ of \cite{MS24} at degree $n \geq 3$ are
identified with the shadow obstruction tower obstruction classes $o_n(\cA)$
...
the $L_\infty$ relation at degree~$4$: the left-hand side is the
...
\item $F_4$ \textup{(}secondary Jacobi homotopy\textup{)}
 ...
 $\mathrm{Vir}_c$.
\item At degree $n \geq 5$, the higher operations $F_n$ encode
...
\begin{proof}
The proof uses only the genus-$0$ operadic equivalence, not any
full \v{C}ech--bar $L_\infty$ intertwining statement.
...
\section{Synthesis}
\label{sec:qc-wave13-DNA}

\begin{remark}[Pentagon coboundary at $\hbar^3$ as the universal quantum correction]
\label{rem:qc-DNA-1}
The Wave~13 quantum correction at the pentagon level has explicit form
\[
 \phi^{(3)}
 \;=\;
...
\]
with three linearly independent third-order closure pieces:
$\zeta(3)$-piece (symmetric part, Drinfeld--Kohno--associator $KZ$-contribution),
$(25/3)$-piece (timelike-anomaly, c.f. class-$\mathcal S$
$c_{2d} = -214 = -12\cdot 107/6$ at the $\Sigma_{0,24}$ punctures;
the pentagon-constant $25/3 = \mathrm{rk}(II_{25,1})/3$ is
independent of $c_{2d}$, so the Wave-15 retraction
$-312 \to -214$ leaves this coefficient unchanged), and $(\Phi_{10}/\eta^{24})$-piece (BKM
imaginary-root contribution, the Igusa cusp-form weight-$5$ denominator)
\cite{Drinfeld1990,Borcherds1992,GritsenkoNikulin1998}. Cross-ref
Chapter~\ref{ch:k3-chiral-bialgebra-platonic} \S on pentagon closures.
\end{remark}

\begin{remark}[$\widetilde M_{24}$ quantum correction as order-$6$ Schur cocycle]
\label{rem:qc-DNA-2}
The Wave~13 quantum correction attaches the $\widetilde M_{24}$ Schur
cocycle of order $6$ in $H^2(M_{24}, U(1)) = \mathbb Z/12$ to the
$M_{24}$-equivariant quantisation of the K3 chiral structure. At the
bar/cobar level this is the central extension
\[
 1\to \mathbb Z/6\to \widetilde M_{24}\to M_{24}\to 1
\]
whose extension class restricts to the $\mathrm{Mathieu}\ M_{12}$-Schur
cocycle of order $12$ (Cheng--Duncan--Harvey 2014 umbral shift
$m_\sigma$ at conjugacy class $\sigma$), detecting the obstruction to
lifting the $M_{24}$-action on the K3 elliptic genus to a strict
action on the chiral side. See Vol.~III Chapter~\ref{ch:k3-chiral-bialgebra-platonic}.
\end{remark}

\begin{remark}[Drinfeld audit: the classical $r$-matrix of the K3 chiral bialgebra]
\label{rem:drinfeld-audit-k3-rmatrix}%
...
Every Yangian-like chiral quantum group is, in the Drinfeld sense, a
deformation quantisation of a universal enveloping algebra by a classical
$r$-matrix satisfying CYBE; without such an $r$-matrix the label
``Yangian'' is a type error. We record the classical $r$-matrix
$r^{K3}(z)$ underlying the K3 chiral bialgebra $\mathbf{H}_{\Delta_5}$
(Vol.~III Definition~\ref{def:k3-chiral-bialgebra}) and its status. The
home Lie algebra is the Manin pair
...
The classical $r$-matrix is the Pasol--Zagier--Siegel Kronecker--Eisenstein
series
\[
  r^{K3}(Z)
  \;=\;
...
\]
with $F^{\mathrm{Sieg}}_z$ the Pasol--Zagier--Siegel extension of the
Kronecker series on $\mathbb{H}_2$ (Pasol--Zagier 2013, \S $3$) and
$\Omega$ the BKM Cartan form on $\Lambda^{3,2}$. This is \emph{Siegel-elliptic
dynamical} in the sense of Felder 1994 extended to $\mathrm{Sp}_4$; it is
neither rational ($r^{KM}(z) = k\Omega/z$), nor trigonometric
($r^{U_q}(z) = k\Omega \cdot (z+1)/(z-1)$), nor elliptic
(single-parameter $r^{\mathrm{Felder}}(u,\tau)$), but a fourth class.
CYBE holds on the paramodular locus $K(1)\subset \mathbb{H}_2/\mathrm{Sp}_4(\mathbb{Z})$
via the Pasol--Zagier cyclic identity
\[
  F^{\mathrm{Sieg}}_z(u_{12}+\lambda,Z) + F^{\mathrm{Sieg}}_z(u_{23}+\lambda,Z)
  + F^{\mathrm{Sieg}}_z(u_{31}+\lambda,Z) \;=\; 0
\]
(Pasol--Zagier 2013 Theorem $3.2$, cyclic Siegel-Kronecker identity). Off
...
The residue at the diagonal $Z\to 0$ recovers the Heisenberg $r$-matrix
$r^{\mathrm{Heis}}(z) = k/z$ on the abelian-at-Lie stratum with level
$k = \mathrm{rk}(\Lambda_{K3}) = 24$, matching the fact (Nekrasov 2026)
that $\mathbf{H}_{\Delta_5}$ is abelian at the Lie level
($24$ Miki/Heisenberg copies) and that non-abelian BKM arises only under
Frenkel--Kac vertex closure on the K3 Fock module. The deformation
parameter satisfies $\hbar^2 \cdot K^{\kappa_{\mathrm{ch}}} = -1$ on the
$\mathcal{B}$-family with $K^{\kappa_{\mathrm{ch}}} = 2c_+(\mathrm{Mukai}(K3)) = 8$,
whence $\hbar^2 = -1/8$, in line with the Lusztig small-quantum-group order
$\ell = 8$ and the Bruinier 2002 Heegner Chern-class reciprocity
(Proposition~5.1). The nomenclature ``K3 Yangian'' is therefore a category
error: no Drinfeld Yangian exists ($\mathfrak{g}_{\Delta_5}$ has no
Kac--Moody Cartan, no Weyl group on imaginary simple roots, no
J-presentation), the correct label is the \emph{K3 chiral Hall--Drinfeld
double}, and the underlying $r$-matrix is Siegel-elliptic dynamical.
Further detail is in Vol.~III
Section~\ref{sec:R-Sieg-dynamical}.
\end{remark}
