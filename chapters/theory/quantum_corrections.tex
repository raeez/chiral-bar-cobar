...
\label{rem:quantum-corrections-hms}
This chapter operates at M-level (explicit curvature formulas,
Eisenstein regularizations) with S-level shadows
(numerical obstruction scalars $\kappa$).
\end{remark}

Strict genus-$0$ nilpotence breaks as soon as the propagator remembers the
periods of the curve.  This chapter isolates that failure at chain level.
...
\noindent\emph{Governing question.} Which deviations from strict
genus-$0$ nilpotence are universal geometric corrections, and which are
algebra-specific coefficients?  The mechanism is universal, while the
scalar shadow is carried by $\kappa$; the chapter makes that split
explicit at chain level.
...
These relations are the integrability conditions for the geometric bar complex: $d^2 = 0$ is equivalent to flatness of a connection on a graded vector bundle.
...
The $A_\infty$ structure simplifies dramatically: $m_1 = 0$ (the bar complex is already a complex), $m_2$ is the standard product, and $m_n = 0$ for $n \geq 3$.  The transferred $A_\infty$ structure is therefore strictly associative.  The genus-$g$ corrections for $g \geq 1$ enter not through higher $m_n$ but through the propagator and the cohomology of $\mathcal{M}_g$ (cf.\ Part~III).
