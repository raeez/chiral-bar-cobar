...
The $A_\infty$ structure is strict: $m_0 = 0$ (uncurved, so $m_1^2 = 0$), $m_2$ is the standard product, and $m_n = 0$ for $n \geq 3$.  The genus-$g$ corrections for $g \geq 1$ enter not through higher $m_n$ but through the curvature $m_0^{(g)} = \kappa \cdot \lambda_g$ and the cohomology of $\overline{\mathcal{M}}_g$ (cf.\ Part~II).
...
\item \emph{The bar differential is exact.}  For quasi-linear theories,
  ...
  enter through the curvature $m_0^{(g)} = \kappa \cdot \omega_g$
