...
This chapter should therefore be read as the first place
where the reader encounters why exponentiating a categorical
logarithm can require filtered or completed models---a theme that
dominates the higher-genus and curved Koszul duality chapters
that follow.
...
\begin{definition}[Categorical logarithm]
\label{def:categorical-logarithm}
\index{categorical logarithm}
The bar functor~$\barBch$ is a \emph{categorical logarithm}:
$\barBch(\cA \otimes \cA') \simeq \barBch(\cA) \oplus \barBch(\cA')$,
the logarithmic form $\eta_{ij} = d\log(z_i - z_j)$ is its
integral kernel, and the Arnold relation
$\eta_{ij}\wedge\eta_{jk} + \eta_{jk}\wedge\eta_{ki}
+ \eta_{ki}\wedge\eta_{ij} = 0$ is the cocycle condition
($d_{\barB}^2 = 0$).  The cobar functor~$\Omegach$ is the
\emph{categorical exponential}: $\Omegach(\barBch(\cA)) \simeq \cA$
on the Koszul locus
