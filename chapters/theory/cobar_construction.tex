...
The cobar complex $\Omega(\barB(\cA))$, viewed as an $\mathrm{FCom}$-algebra (Theorem~\ref{thm:bar-modular-operad}), carries the full shadow Postnikov obstruction tower: for each $r \geq 2$, the arity-$r$ operadic structure map of $\Omega(\barB(\cA))$ encodes $\Theta_\cA^{\leq r}$.
\end{proposition}
...
\index{level!shifted}
% RECTIFICATION-FLAG: AP25/AP33 — This result involves Verdier duality on the bar
% coalgebra (Theorem A), NOT bar-cobar inversion (Theorem B).  Bar-cobar inversion
% recovers the original algebra: Omega(B(g_k)) ~ g_k.  The level-shifted algebra
% arises from the Verdier intertwining D_Ran(B(g_k)) ~ B(g_{k'}) with k' = -k-2h^v.
% The curvature-negation argument below identifies the Verdier dual's curvature;
% it does NOT show that Omega(B(g_k)) produces g_{k'}.
% See AP25 and AP33 in CLAUDE.md for the full analysis.
...
\item Higher genus: the cubic and quartic shadows
  ...
  $g \geq 2$ bar differential via the shadow Postnikov obstruction tower
  (Definition~\ref{def:shadow-postnikov-tower}).
\end{itemize}
\end{remark}
