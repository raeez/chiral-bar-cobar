...
$d_{\mathrm{fib}}^2 = \kappa \cdot \omega_g$, reflecting the curved
$A_\infty$ structure $\mu_1^2 = [\mu_0, -]$
(Chapter~\ref{chap:bar-cobar-adjunction}). The total bar differential
$d_{\mathrm{bar}}^2 = 0$ remains nilpotent. The cobar functor
...
of the bar complex absorbs one power of $\epsilon$ (pole-order absorption,), so the Verdier dual of a double pole is a
...
Both quasi-isomorphisms are induced by the Verdier pairing
(Theorem~\ref{thm:bar-cobar-verdier}). When $(\mathcal{A}, \mathcal{C})$
form a chiral Koszul pair (Definition~\ref{def:chiral-koszul-pair})
with $\mathcal{C} \simeq \bar{B}^{\mathrm{ch}}(\mathcal{A})$, the two
sides combine to exhibit the bar and cobar functors as mutually
quasi-inverse.
\end{corollary}
...
Under hypothesis~(A1), the Koszulity condition ensures that $E_2$ collapses:
the only nonzero column is $p = 0$, where it gives $\mathcal{A}$.

Convergence of the spectral sequence is guaranteed by hypothesis~(A2):
...
$\cA_{\le N}$, the degree cutoff
(Lemma~\ref{lem:degree-cutoff}) ensures only $\mu_r$ for $r \le N$
...
\emph{Coalgebra side.}
Dually, filter $\Omega^{\mathrm{ch}}(\mathcal{C})$ by word length in the
free chiral algebra. Conilpotence (C2) makes the filtration exhaustive;
coKoszulity (C1) collapses the spectral sequence to a single column.
The unit
$\mathcal{C} \to \bar{B}^{\mathrm{ch}}(\Omega^{\mathrm{ch}}(\mathcal{C}))$
is a quasi-isomorphism by the same spectral sequence argument, applied
with the roles of algebra and coalgebra interchanged.
\end{proof}
...
Theorem~\ref{thm:bar-cobar-inversion-qi} (Theorem~B), not of this
...
the programme\textup{)}.
The counit $\psi_{\cA}$ is a quasi-isomorphism if and only if $\cA$ lies on
the Koszul locus
\textup{(}Theorem~\ref{thm:bar-cobar-inversion-qi}\textup{)}; off the
Koszul locus the conclusion of (1)--(4) still holds, but $\psi_{\cA}$ is
only an isomorphism in the coderived category
...
bar coalgebra. The desuspension identification
$\bar{\cA} \xrightarrow{\sim} s^{-1}s\bar{\cA} = s^{-1}\bar{B}^{\mathrm{ch}}_1(\cA)$
...
a unique chiral-algebra morphism extending the generator-level projection
$s^{-1}\overline{B} \xrightarrow{s^{-1}\pi_1} s^{-1}\bar{B}^{\mathrm{ch}}_1(\cA)
\xrightarrow{\sim} \bar{\cA} \hookrightarrow \cA$. This is exactly the
...
Proposition~\ref{prop:cobar-bar-augmentation}: bar degree $n=1$ produces
$d_\Omega(s^{-1}[s\bar{a}]) = 0$; bar degree $n=2$ uses the Maurer--Cartan
equation for the chiral universal twisting morphism
$\pi = \psi_{\cA}|_{s^{-1}\overline{B}}$
...
$\psi_{\cA}$ off the Koszul locus, correcting a scope conflation in the
historical statement of this result; see
...
Let $\cC$ be a coaugmented chiral coalgebra on~$X$ with reduced
comultiplication
...
chiral algebra on~$V$.
\begin{enumerate}[label=\textup{(\roman*)}]
\item \emph{Universal extension.} For every chiral algebra~$\cA$ and every
...
$\mathrm{Free}_{\mathrm{ch}}(V)$, and yields $d_\Omega^2 = 0$ on generators
from coassociativity of $\bar{\Delta}$.
\end{enumerate}
\end{lemma}
...
$(p+1, q-1)$ on $V$ — degree $+1$ as required for a cobar differential.
...
convention $d_\Omega := -s^{-1} \circ \bar{\Delta} \circ s$ that ensures
$d_\Omega$ has the same orientation as the cobar differential of
\cite[\S2.2]{LV12} and extends to a derivation rather than a coderivation.
Coassociativity of $\bar{\Delta}$ then gives $d_\Omega^2 = 0$ on
generators by the standard \cite[Proposition~2.2.6]{LV12} computation:
...
derivation property extends $d_\Omega^2 = 0$ to all of
...
collapse input off it. The historical conflation of these two statements
into a single ``geometric unit theorem'' \textup{(}with backwards arrow
direction\textup{)} obscured the dichotomy of
Convention~\ref{conv:regime-tags}; we keep them separate here.

In particular, the prior wording ``unit
$\eta\colon\cA\to\Omega^{\mathrm{ch}}(\bar{B}^{\mathrm{ch}}(\cA))$'' was
doubly incorrect: the canonical natural transformation in this geometric
direction is the \emph{counit} $\psi$, not a unit, and it points
$\Omega^{\mathrm{ch}}(\bar{B}^{\mathrm{ch}}(\cA)) \to \cA$, not the reverse.
The reverse arrow $\cA \to \Omega^{\mathrm{ch}}(\bar{B}^{\mathrm{ch}}(\cA))$
exists (and is canonical) only when $\psi_{\cA}$ is a qi, in which case it
is the homotopy inverse of $\psi_{\cA}$ -- and that is exactly the Koszul-
locus content of Theorem~\ref{thm:bar-cobar-inversion-qi}.
\end{remark}

\begin{corollary}[Chiral bar-cobar Hom-bijection backbone; \ClaimStatusProvedHere]
...
(chiral Hochschild cochains, Theorem~H, the bulk).
\end{itemize}
...
The resulting central extension is universal because any 2-cocycle $\omega: \mathcal{A} \otimes \mathcal{A} \to \mathbb{C}$ that defines a central extension of~$\mathcal{A}$ factors through the genus-1 trace pairing. The chiral Hochschild 2-cocycle condition requires $\omega$ to be $\mathcal{A}$-bilinear and invariant, which forces $\omega = c \cdot \mathrm{Tr}$ for some constant~$c$. The genus-1 construction produces exactly this with $c = \kappa$.
\end{proof}
...
The isomorphism is constructed by reading each axiom as a construction
step: logarithmic structure determines integration kernels, the support
condition ensures locality, the residue formula recovers the OPE, and
the relevant configuration-space relations ensure associativity
\textup{(}Arnold on an affine screen, projective linear relations on
$\mathbb{P}^1$, and period relations for $g\geq1$; see
Proposition~\ref{prop:arnold-higher-genus}\textup{)}.
...
\begin{lemma}[Obstruction operator {\cite{Positselski11}}; \ClaimStatusProvedElsewhere]\label{lem:obstruction-class}
...
\label{rem:cobar-1}
The cobar construction $\Omega_X$, applied to the bar coalgebra of the
Wave~13 K3 chiral bialgebra, produces
\[
\Omega_X\bigl(B_{\mathrm{ch}}(\mathbf H_{\Delta_5})\bigr)
\;=\;
T_{\mathrm{ch}}\!\bigl(s\,\overline{B_{\mathrm{ch}}(\mathbf H_{\Delta_5})}\bigr)
\;\simeq\;
\mathbf H_{\Delta_5},
\]
...
the bar differential $d_B$ decomposed in \S\ref{sec:bar-cobar-review-supplement}.
The quasi-isomorphism $\Omega_X\bigl(B_{\mathrm{ch}}(\mathbf H_{\Delta_5})\bigr)\simeq \mathbf H_{\Delta_5}$
is the Wave~13 incarnation of bar--cobar inversion (Theorem~A).
At $\hbar^2 = -1/8$ this is a genuine equivalence in the Hall
subcategory; away from this Lusztig specialisation it is only a
quasi-isomorphism of filtered complexes, with the filtration controlled
by the Siegel-modular weight. Cross-ref Vol.~III,
Chapter~\ref{ch:k3-chiral-bialgebra-platonic}, Theorem~A.
\end{remark}

\begin{remark}[Cobar differential four-piece decomposition]
\label{rem:cobar-2}
The cobar differential $d_\Omega$ on $\Omega_X(B_{\mathrm{ch}}(\mathbf H_{\Delta_5}))$
decomposes, by duality to the bar differential, as
\[
...
The closedness $d_\Omega^2 = 0$ is the Wave~13 Maurer--Cartan equation
$D\cdot \Theta_{\mathbf H_{\Delta_5}} + \tfrac{1}{2}[\Theta_{\mathbf H_{\Delta_5}},\Theta_{\mathbf H_{\Delta_5}}] = 0$
on the bi-based Ran-space. Primary-lit: Schiffmann--Vasserot 2012,
...
\[
  ...
  \mathbf{H}_{\Delta_5}\bigr),
\]
where the second isomorphism is the bar--cobar adjunction
(Theorem~\ref{thm:bar-cobar-inversion-qi}) applied to
$\mathbf{H}_{\Delta_5}$. Explicitly: an element
...
chapter.
\begin{enumerate}[label=\textup{(\alph*)}]
\item \emph{Generators.} A basis for $B_{\mathrm{ch}}(\mathbf{H}_{\Delta_5})$
...
element of $\mathfrak{C}^{\Omega}_{\mathbf{H}_{\Delta_5}}$ is
specified by a pair $(w, y)$ with $w\in B_{\mathrm{ch}}$ and
$y\in\mathbf{H}_{\Delta_5}$: it maps $w\mapsto y$ and kills all
other basis words.
...
\textup{(}Theorem~\ref{thm:bar-cobar-inversion-qi}\textup{)}, the
...
Theorem~\ref{thm:bar-cobar-inversion-qi}; the universal twisting morphism
...
$\hbar^3, \hbar^4$; \ClaimStatusProvedElsewhere]
\label{rem:cobar-MC-verification-k3}
The cobar-side closedness $d_\Omega^2 = 0$ on
$\Omega_X(B_{\mathrm{ch}}(\mathbf{H}_{\Delta_5}))$ translates,
via Proposition~\ref{prop:bar-cobar-MC-naturality-k3}, to the
bar-side MC equation verified in
...
(Vol.~III, Theorem~\ref{thm:pentagon-sieg-bor});
