...
If the bar construction is logarithmic descent, the cobar construction
is re-expansion: the reverse engineering of algebra from coalgebra,
performed geometrically rather than formally.  Verdier duality,
distributions, and coevaluation maps are the concrete instruments of
the reversal.  Where the bar complex harvests residues at collision
divisors on the compactified configuration space
$\overline{C}_n(X)$, the cobar complex integrates delta-function
distributions over the open configuration space $C_n(X)$.  Their
pairing is the residue-integration duality on logarithmic forms.

The construction is intrinsic: given a conilpotent chiral
coalgebra~$\cC$ on~$X$ valued in holonomic $\cD$-modules, the
\emph{geometric cobar complex} is defined by
...
Where the bar complex uses $j_*$ (maximal extension) and
logarithmic forms to extract residues, the cobar complex uses
$j_!$ (extension by zero) and distributional sections to
reinsert data---the exchange $\mathbb{D} \circ j_* \cong
j_! \circ \mathbb{D}$
(Lemma~\ref{lem:verdier-extension-exchange}) is the precise
mechanism.  The cobar kernel is therefore a distributional
delta-function: the Schwartz kernel
$K \in \cD'(C_n(X) \times C_m(X))$ of
Theorem~\ref{thm:schwartz-kernel-cobar} is the distributional
partner of the logarithmic propagator
$\eta_{ij} = d\log(z_i - z_j)$ that drives the bar complex.
This bar/cobar duality---residues versus delta
functions, logarithmic versus distributional, $j_*$ versus
$j_!$---is the geometric content of the adjunction
