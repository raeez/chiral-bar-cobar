...
valued in holonomic $\mathcal{D}$-modules, and let $\mathcal{C}$ be a
...
\item \emph{Conilpotence or completion}: either
 ...
 (Definition~\ref{def:i-adic-topology}),
\end{enumerate}
...
\begin{theorem}[Bar-cobar Verdier pairing; \ClaimStatusConditional]\label{thm:poincare-verdier}
...
$\cA = \widehat{\mathfrak{g}}_k$, the Koszul dual is the chiral
Chevalley--Eilenberg object
\[
\cA^! \simeq \mathrm{CE}^{\mathrm{ch}}(\widehat{\mathfrak{g}}_{k'}),
\qquad k' = -k - 2h^\vee.
\]
The affine level reflection records the current presentation inside
this CE object; it also sends the scalar modular characteristic to its
negative, since
\[
\kappa^{\mathrm{KM}}(V_{k'}(\mathfrak g))
=\frac{\dim(\mathfrak g)(k'+h^\vee)}{2h^\vee}
=-\frac{\dim(\mathfrak g)(k+h^\vee)}{2h^\vee}.
\]
The reflected current presentation has OPE
\[
