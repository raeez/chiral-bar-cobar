...
The bar-cobar adjunction is mediated by a single object:
the universal twisting morphism
$\tau \in \MC\bigl(\operatorname{Conv}(\cC, \cA)\bigr)$,
a Maurer--Cartan element in the convolution algebra
(Definition~\ref{def:twisting-morphism}).
The bar complex and the cobar complex are its two projections.
The bar complex $\barB(\cA) = (\cA \otimes_\tau \cC, d_\tau)$
is the coalgebra-side twisted tensor product: it harvests residues
at collision divisors on~$\overline{C}_n(X)$ via the logarithmic
propagator $\eta_{ij} = d\log(z_i - z_j)$.
The cobar complex $\Omega(\cC) = (\mathrm{Free}(s^{-1}\bar{\cC}), d_\tau)$
is the algebra-side twisted tensor product: it integrates
delta-function distributions over the open configuration space
$C_n(X)$.
The duality between them (residues versus delta functions,
logarithmic versus distributional, $j_*$ versus $j_!$) is not a
separate phenomenon but the duality of the two factors
of $\operatorname{Conv}(\cC, \cA)$ under Verdier exchange.
...
The cobar complex already carries the seeds of curved and modular
phenomena.  Once genus enters, strict inversion can fail on the nose
and survive only after completion: the cobar differential may acquire
curvature, and the quasi-isomorphism
$\Omega^{\mathrm{ch}}(\barB(\cA)) \xrightarrow{\sim} \cA$
(Theorem~B) may require coderived or completed contexts.
At genus~$g \geq 1$ the cobar differential picks up a curvature
correction $\kappa(\cA) \cdot \omega_g$ from the Hodge bundle,
so that $d_{\mathrm{cobar}}^2 = \kappa \cdot \omega_g$ rather
than zero; this is the cobar-side avatar of the curved
$A_\infty$ structure $\mu_1^2 = [\mu_0, -]$ that appears in the
bar-cobar adjunction chapter
(Chapter~\ref{chap:bar-cobar-adjunction}).  The modular seed is
deeper: the cobar functor, applied to the bar coalgebra viewed
as an FCom-algebra
(Theorem~\ref{thm:bar-modular-operad}), already contains the
data of the shadow Postnikov tower, the finite-order
projections $\Theta_\cA^{\leq r}$ of the universal
Maurer--Cartan element that organize Part~I.
Exponentiating a categorical logarithm can require filtered
or completed models, a theme that dominates the higher-genus
and curved Koszul duality chapters that follow.

In the three-ring architecture, the cobar construction at genus~$0$
is proved Ring~1 content; the curved cobar at genus~$g \geq 1$
connects to the shadow tower of Ring~2; and the distributional
formalism anticipates the BV/BRST interpretation
(Chapter~\ref{ch:quantum-corrections}) in Ring~3.
Chapter~\ref{ch:algebraic-foundations} defined the convolution
algebra and twisting morphisms abstractly;
Chapter~\ref{chap:bar-cobar} built the bar complex as the
logarithmic side of the transform; here the cobar complex
provides the distributional inverse, and the next chapter
(Chapter~\ref{chap:bar-cobar-adjunction}) joins them as an
adjoint pair.

\subsection{Motivation: reversing the prism}

\begin{remark}[Inverse prism principle]
The bar construction extracts residues at collision divisors; the cobar construction integrates over configuration spaces.  Their duality is the residue-integration pairing on logarithmic forms.
...
The cobar complex $\Omega(\barB(\cA))$, viewed as an $\mathrm{FCom}$-algebra (Theorem~\ref{thm:bar-modular-operad}), carries the full shadow Postnikov tower: for each $r \geq 2$, the arity-$r$ operadic structure map of $\Omega(\barB(\cA))$ encodes $\Theta_\cA^{\leq r}$.
\end{proposition}
...
\index{shadow Postnikov tower!genus hierarchy}
...
\item Higher genus: the cubic and quartic shadows
  ...
  $g \geq 2$ bar differential via the shadow Postnikov tower
  (Definition~\ref{def:shadow-postnikov-tower}).
\end{itemize}
\end{remark}
