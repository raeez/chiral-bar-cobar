...
Let $\cC$ be a conilpotent chiral coalgebra.  The geometric cobar
complex
$\Omega^{\mathrm{ch}}(\cC)=\bigoplus_{n\geq0}
\Omega^{\mathrm{ch}}_n(\cC)$ is the free chiral algebra on the
desuspended coaugmentation coideal, with differential induced by the
reduced coproduct.  For an augmented chiral algebra~$\cA$, the
twisting adjunction supplies the counit
\[
\varepsilon_\cA\colon
\Omega_X^{\mathrm{ch}}B_X^{\mathrm{ch}}(\cA)\longrightarrow\cA.
\]
The enhanced associative Ran theorem makes this map an equivalence;
its Fulton--MacPherson and completed realizations use the stated
factorization and convergence packages.  The quadratic comparison
$q_\cA\colon\cA^{\mathrm i}\to B_X(\cA)$ is governed by Theorem~B.
Definitions~\ref{def:factorization-coalgebra},
...
For a conilpotent chiral coalgebra~$\cC$ on~$X$ valued in holonomic
$\cD$-modules, the
...
is the open inclusion. For an uncurved conilpotent coalgebra the cobar
...
CDG cobar object whose square is the inner curvature operator.
...
(Chapter~\ref{chap:bar-cobar-adjunction}). The uncurved genus-$0$
complex, a flat total comparison differential, a curvature-cancelling
Maurer--Cartan twist, and the coderived totalization give the four
square-zero realizations used below. The cobar functor
...
Bar and cobar form an adjunction between augmented algebras and
conilpotent coalgebras. Its triangular identities compare
$\Omega_XB_X(A)$ with~$A$ and $C$ with $B_X\Omega_X(C)$. Verdier
duality defines the contravariant algebra-valued functor
\[
K_X(A):=\mathbb D_{\operatorname{Ran}}B_X(A)
\]
on its holonomic dualizable domain. Theorem~\ref{thm:bar-cobar-verdier}
constructs the natural comparison between $K_X(A)$ and the geometric
cobar of the Verdier-dual coalgebra $\mathbb D_XA$.

\subsection{Motivation: reversing the prism}

\begin{remark}[Reconstruction, Verdier duality, and the derived centre]
\label{rem:cobar-three-functors}
Three constructions associated with an augmented chiral algebra~$\cA$
have distinct sources and targets.
\begin{enumerate}[label=\textup{(\roman*)}]
\item The twisting adjunction gives the reconstruction morphism
\[
R_X(\cA):=\Omegach_X\barB_X(\cA)
\xrightarrow{\varepsilon_\cA}\cA .
\]
\item Verdier duality gives the post-Verdier algebra
\[
K_X(\cA):=\mathbb D_{\operatorname{Ran}}^{\mathrm{cont}}
\barB_X(\cA),
\]
whose multiplication is the transpose of the bar coproduct.
\item Chiral Hochschild cochains give the derived centre
\[
Z^{\mathrm{der}}_{\mathrm{ch}}(\cA)
=C^\bullet_{\mathrm{ch}}(\cA,\cA).
\]
\end{enumerate}
Thus $R_X$ is covariant and algebra-valued, while $K_X$ is
contravariant and algebra-valued. A quadratic presentation supplies
the coalgebra map $q_\cA\colon\cA^{\mathrm i}\to B_X(\cA)$ and hence
the algebra map
\[
\mathbb D_{\operatorname{Ran}}(q_\cA)\colon
K_X(\cA)\longrightarrow\mathbb D_{\operatorname{Ran}}\cA^{\mathrm i}.
\]
Theorem~\ref{thm:bar-cobar-verdier} identifies $K_X(\cA)$ with
$\Omega_X^{\mathrm{geom,cont}}(\mathbb D_X\cA)$ under its duality and
...
\begin{definition}[Conilpotent and completed coalgebras;
\ClaimStatusDefinitional]\label{def:conilpotent-cobar}
...
\begin{definition}[Intrinsic geometric cobar complex;
\ClaimStatusDefinitional]\label{def:geom-cobar-intrinsic}
...
(Theorem~\ref{thm:poincare-verdier}), the dual operation is
...
This requires verifying the diagonal and off-diagonal cancellation
  families mirroring the bar square-zero theorem
...
\emph{Degree 4 (4h-1):}
\[d(s^{-1}v^4) = -4(s^{-1}v) \cdot (s^{-1}v^3) - 6(s^{-1}v^2) \cdot (s^{-1}v^2) - 4(s^{-1}v^3) \cdot (s^{-1}v)\]
In a commutative setting:
\[d(s^{-1}v^4) = -8(s^{-1}v) \cdot (s^{-1}v^3) - 6(s^{-1}v^2)^2\]

\emph{Degree 5 (5h-1):}
\[d(s^{-1}v^5) = -5(s^{-1}v) \cdot (s^{-1}v^4) - 10(s^{-1}v^2) \cdot (s^{-1}v^3) - 10(s^{-1}v^3) \cdot (s^{-1}v^2) - 5(s^{-1}v^4) \cdot (s^{-1}v)\]
In a commutative setting:
\[d(s^{-1}v^5) = -10(s^{-1}v) \cdot (s^{-1}v^4) - 20(s^{-1}v^2) \cdot (s^{-1}v^3)\]
...
\subsection{The Verdier dual of the bar coalgebra}

\begin{theorem}[Verdier--bar comparison; \ClaimStatusConditional]\label{thm:bar-cobar-verdier}
\label{thm:verdier-bar-cobar}
\index{Verdier duality!bar comparison|textbf}
\textup{[Type signature: Open quadrant; holonomic factorization
presentation; Beilinson level~$2$; hypothesis package
$H_{\mathbb D}^{\mathrm{bar}}(\mathcal A)$.]}

Let $X$ be a smooth projective curve and let $\mathcal A$ be an
augmented chiral algebra.  The package
$H_{\mathbb D}^{\mathrm{bar}}(\mathcal A)$ consists of the following
data:
\begin{enumerate}[label=\textup{(V\arabic*)}]
\item finite bar windows $B_{\leq N}(\mathcal A)$ represented by
 bounded regular holonomic complexes on the Fulton--MacPherson stages,
 with strict surjections
 $B_{\leq N+1}(\mathcal A)\twoheadrightarrow B_{\leq N}(\mathcal A)$
 and continuous bar object
 $B_X^{\mathrm{cont}}(\mathcal A)=\varprojlim_NB_{\leq N}(\mathcal A)$;
\item Verdier dualizability and biduality in every window;
\item the tensor exchange equivalences
 \[
 \chi_{M,N}\colon
 \mathbb D(M\otimes^!N)\xrightarrow{\sim}
 \mathbb DM\otimes^*\mathbb DN
 \]
 for the terms occurring in the bar coproduct;
\item compatibility of Verdier duality, the bar differential, the
 coproduct, and the transition maps, together with the continuous
 identification
 \[
 \mathbb D_{\operatorname{Ran}}^{\mathrm{cont}}
 B_X^{\mathrm{cont}}(\mathcal A)
 =\varinjlim_N\mathbb D B_{\leq N}(\mathcal A).
 \]
\item the transposed coalgebra
 $\mathcal A_{\mathbb D}^{\vee}:=\mathbb D_X\mathcal A$ is
 coaugmented and complete conilpotent, its geometric cobar is defined,
 and its continuous geometric cobar is the compatible filtered colimit
 \[
 \Omega_X^{\mathrm{geom,cont}}(\mathcal A_{\mathbb D}^{\vee})
 :=\varinjlim_N
 \Omega_X^{\mathrm{geom}}(\mathcal A_{\mathbb D}^{\vee})_{\leq N}.
 \]
\end{enumerate}
This is the finite-window Fulton--MacPherson realization of the
Verdier package $H_{\mathrm{VD}}$ used in Theorem~A.
Then
\[
K_X(\mathcal A):=
\mathbb D_{\operatorname{Ran}}^{\mathrm{cont}}
B_X^{\mathrm{cont}}(\mathcal A)
\]
is an augmented algebra in the post-Verdier $*$-tensor
factorization category.  Its product is
\[
\mu_K=
\mathbb D(\Delta_B)\circ\chi_{B,B}^{-1}\colon
K_X(\mathcal A)\otimes^*K_X(\mathcal A)\longrightarrow K_X(\mathcal A).
\]
For a morphism $f\colon\mathcal A\to\mathcal A'$ preserving the
package, the map
\[
K_X(f):=\mathbb D_{\operatorname{Ran}}^{\mathrm{cont}}B_X(f)\colon
K_X(\mathcal A')\longrightarrow K_X(\mathcal A)
\]
makes $K_X$ a contravariant functor.

Write $\mathcal A_{\mathbb D}^{\vee}:=\mathbb D_X\mathcal A$, equipped
with the coalgebra structure transposed from the multiplication of
$\mathcal A$.  Verdier biduality induces a natural comparison of
post-Verdier algebras
\begin{equation}\label{eq:verdier-bar-cobar-typed-comparison}
\gamma_{\mathcal A}\colon
\Omega_X^{\mathrm{geom,cont}}(\mathcal A_{\mathbb D}^{\vee})
\longrightarrow K_X(\mathcal A).
\end{equation}
It is an equivalence under \textup{(V1)--(V5)}.  This is the typed
intertwining
\[
\Omega_X^{\mathrm{geom,cont}}\circ\mathbb D_X
\xRightarrow{\ \gamma\ }
\mathbb D_{\operatorname{Ran}}^{\mathrm{cont}}\circ B_X^{\mathrm{cont}}
\]
between contravariant algebra-valued functors.

If $\mathcal A=T_X(V)/(R)$ has quadratic coalgebra
$\mathcal A^{\mathrm i}=C_X(s^{-1}V,s^{-2}R)$ and comparison
...
\]
Under $H_{\mathrm{CL}}(\mathcal A,\mathcal A^{\mathrm i},
\tau_{\mathrm i})$ and exact continuous Verdier duality, this map is a
quasi-isomorphism precisely when $q_{\mathcal A}$ is a
quasi-isomorphism.
\end{theorem}

\begin{proof}
For every finite window, Verdier duality is an exact contravariant
equivalence on the bounded regular holonomic category.  Applying it to
the bar coproduct gives
\[
\mathbb D(B\otimes^!B)\xrightarrow{\mathbb D(\Delta_B)}\mathbb DB.
\]
The inverse tensor exchange $\chi_{B,B}^{-1}$ identifies the source
with $\mathbb DB\otimes^*\mathbb DB$.  Coassociativity of
$\Delta_B$ and coherence of~$\chi$ give associativity of~$\mu_K$;
the bar counit gives the unit and the bar coaugmentation gives the
augmentation. Compatibility with the strict
transition maps assembles these products on
$\varinjlim_N\mathbb DB_{\leq N}$.  Functoriality follows from the
covariance of~$B_X$ and the contravariance of~$\mathbb D$.

Let
$\delta_{\mathcal A}\colon\mathcal A\xrightarrow{\sim}
\mathbb D_X\mathbb D_X\mathcal A$ be Verdier biduality.  By
Definition~\ref{def:geom-cobar-intrinsic},
\[
\Omega_X^{\mathrm{geom,cont}}(\mathbb D_X\mathcal A)
=
\mathbb D_{\operatorname{Ran}}^{\mathrm{cont}}
B_X^{\mathrm{cont}}(\mathbb D_X\mathbb D_X\mathcal A).
\]
The morphism $B_X(\delta_{\mathcal A})$ and contravariance of Verdier
duality give
\[
\gamma_{\mathcal A}
=\mathbb D_{\operatorname{Ran}}B_X(\delta_{\mathcal A})\colon
\mathbb D_{\operatorname{Ran}}B_X(\mathbb D_X\mathbb D_X\mathcal A)
\longrightarrow
\mathbb D_{\operatorname{Ran}}B_X(\mathcal A).
\]
Biduality makes this an equivalence in every finite window, and
\textup{(V4)--(V5)} carry it to the continuous objects.  Naturality is the
naturality of~$\delta$.

Finally, $q_{\mathcal A}$ is a coalgebra morphism, so its Verdier
transpose is an algebra morphism in the displayed direction.  Exactness
in each finite window and the strict continuous passage identify its
cone with the Verdier dual of $\operatorname{Cone}(q_{\mathcal A})$.
This proves the last assertion.
\end{proof}

\begin{lemma}[Square-zero fibre verification: the dual of the bar
coalgebra is the completed quadratic dual;
\ClaimStatusProvedHere]\label{lem:verdier-bar-square-zero-toy}
\index{Verdier duality!square-zero fibre verification}
\index{dual numbers!bar dual}
Let $A=k[\varepsilon]/(\varepsilon^{2})=k\oplus k\varepsilon$ with
$|\varepsilon|=0$, augmented by $\varepsilon\mapsto0$, regarded as the
fibre model of an augmented chiral algebra at a point (the
Fulton--MacPherson stages degenerate to points and Verdier duality
restricts to graded linear duality), with quadratic presentation
$V=k\varepsilon$, $R=k\,(\varepsilon\otimes\varepsilon)$.
\begin{enumerate}[label=\textup{(\roman*)}]
\item The bar coalgebra $\barB(A)=T^{c}(s^{-1}\bar A)$ carries zero
differential; its word-length-$n$ component is the line
$k\,(s^{-1}\varepsilon)^{\otimes n}$ in cohomological degree~$-n$.
\item Set $y:=(s^{-1}\varepsilon)^{\vee}$, of degree $+1$. The
transposed product $\mu_{K}=\mathbb D(\bar\Delta)$ obeys
$y^{m}\cdot y^{n}=(-1)^{mn}\,y^{m+n}$ in the raw dual basis; the
rescaled basis $t_{n}:=(-1)^{n(n-1)/2}y^{n}$ has
$t_{m}\cdot t_{n}=t_{m+n}$, and
\[
\mathbb D\,\barB(A)\;=\;\prod_{n\geq0}k\,t_{n}\;=\;k[[y]],
\qquad
\varinjlim_{N}\mathbb D B_{\leq N}(A)\;=\;k[y],
\]
the completed and finite-window quadratic dual algebras: here
$A^{!}=T(V^{\vee})/(R^{\perp})=k[y]$ since $R=V\otimes V$ forces
$R^{\perp}=0$.
\item No cobar functor intervenes: the algebra structure on
$\mathbb D\,\barB(A)$ is $\mu_{K}$ itself, and
$H^{\bullet}\bigl(\mathbb D\,\barB(A)\bigr)=k[[y]]$ is the completion
of the Yoneda algebra $\operatorname{Ext}_{A}(k,k)=k[y]$.
\end{enumerate}
\end{lemma}

\begin{proof}
(i) The reduced bar differential multiplies adjacent letters, and
every adjacent product is $\varepsilon\cdot\varepsilon=0$ in $\bar A$,
so $d_{\barB}=0$. Each letter carries $|s^{-1}\varepsilon|=-1$.

(ii) Deconcatenation of the unique length-$(m{+}n)$ word contains the
$(m,n)$ split exactly once with coefficient $1$; the Koszul sign in
$(f\otimes g)(w'\otimes w'')=(-1)^{|g||w'|}f(w')g(w'')$ evaluates to
$(-1)^{n\cdot(-m)}=(-1)^{mn}$, giving
$y^{m}\cdot y^{n}=(-1)^{mn}y^{m+n}$. The $2$-cocycle
$\sigma(m,n)=(-1)^{mn}$ on $\mathbb N$ is the coboundary of
$b(n)=(-1)^{n(n-1)/2}$, since
$b(m)b(n)b(m+n)^{-1}=(-1)^{-mn}=(-1)^{mn}$; the basis $t_{n}=b(n)y^{n}$
therefore has structure constants~$1$. Associativity is transposed
coassociativity of $\bar\Delta$. Every window $B_{\leq N}$ is
finite-dimensional, so the package \textup{(V1)--(V5)} holds trivially
over the point; the full linear dual of $\bigoplus_{n}$ is
$\prod_{n}$, giving $k[[y]]$, while the window colimit of the
truncation-dual inclusions gives $k[y]$. The annihilator of
$R=V\otimes V$ inside $V^{\vee}\otimes V^{\vee}$ is zero, so
$A^{!}=T(V^{\vee})=k[y]$.

(iii) The differential vanishes by (i), so cohomology is the
underlying graded object. $\operatorname{Ext}_{A}(k,k)=k[y]$ is the
Koszulity of the dual numbers; each graded piece is one-dimensional,
so the full dual is the augmentation-ideal completion of the graded
dual.
\end{proof}

\begin{remark}[The toy calibrates the typing]
\label{rem:square-zero-toy-typing}
Lemma~\ref{lem:verdier-bar-square-zero-toy} is the smallest instance
of Theorem~\ref{thm:bar-cobar-verdier}: the Verdier dual of the bar
coalgebra is already the (completed) Koszul-dual \emph{algebra}, with
product the transpose of deconcatenation. Applying a cobar functor to
$\mathbb D\,\barB(A)$ is type-incorrect --- $k[[y]]$ carries no
conilpotent coproduct --- and any composite that did so would compute
the identity on finite windows rather than the dual. On a curve the
same typing holds with graded linear duality replaced by
$\mathbb D_{\operatorname{Ran}}^{\mathrm{cont}}$ under
$H_{\mathbb D}^{\mathrm{bar}}$.
\end{remark}

\begin{corollary}[Fulton--MacPherson realization of bar--cobar
reconstruction; \ClaimStatusConditional]\label{cor:bar-cobar-inverse}
\index{bar-cobar inversion|textbf}
\index{quasi-isomorphism!bar-cobar}
Let $\mathcal A$ be an augmented chiral algebra and let $\mathcal C$
be a coaugmented conilpotent chiral coalgebra. Assume the factorization
comparison package identifies the geometric Fulton--MacPherson models
with the enhanced associative Ran bar and cobar, and assume the
finite-window or strict Mittag--Leffler completion package. Then the
adjunction maps are quasi-isomorphisms:
\[
\varepsilon_{\mathcal A}\colon
\Omega_X^{\mathrm{ch,cont}}B_X^{\mathrm{ch,cont}}(\mathcal A)
\xrightarrow{\sim}\mathcal A,
\qquad
\eta_{\mathcal C}\colon
\mathcal C\xrightarrow{\sim}
B_X^{\mathrm{ch,cont}}\Omega_X^{\mathrm{ch,cont}}(\mathcal C).
\]
For a quadratic presentation, the further map
$q_{\mathcal A}\colon\mathcal A^{\mathrm i}\to B_X(\mathcal A)$ is
governed by the quadratic recognition package of Theorem~B.
\end{corollary}

\begin{proof}
In every finite window these maps are the unit and counit of the
ordinary associative bar--cobar resolution
\cite[Corollary~2.3.4]{LV12}.  The factorization comparison transports
the finite-window contractions to the Fulton--MacPherson strata and
their collision maps.  The strict Mittag--Leffler hypothesis identifies
the continuous mapping cones with the inverse limits of the
finite-window cones.  Their cohomology vanishes, which proves both
continuous quasi-isomorphisms.  Quadratic recognition concerns the
additional map $q_{\mathcal A}$ and follows from
\cite[Theorem~3.4.6]{LV12} after its own comparison package.
\end{proof}

\begin{proposition}[Explicit cobar-bar augmentation; \ClaimStatusConditional]
...
This is the piece of Gaitsgory--Rozenblyum geometry closest to
...
\begin{definition}[Algebraic cobar underlying graded chiral algebra;
\ClaimStatusDefinitional]
\label{def:algebraic-cobar-free}
Let $\cC$ be a coaugmented chiral coalgebra with coaugmentation coideal
$\bar{\cC}=\ker(\varepsilon\colon\cC\to\omega_X)$.  The algebraic
graded chiral algebra underlying its cobar is
\[
\Omega^{\mathrm{ch}}_{\mathrm{alg}}(\cC)
:=\mathrm{Free}_{\mathrm{ch}}(s^{-1}\bar{\cC}).
\]
The geometric and distributional cobar models in this chapter are
Verdier/Fulton--MacPherson realizations of this graded object.  The
uncurved cobar differential is the unique degree-$+1$ derivation whose
restriction to generators is the desuspended reduced comultiplication.
\end{definition}

\begin{theorem}[Complete conilpotent chiral cobar functor;
\ClaimStatusProvedHere]
\label{thm:complete-conilpotent-cobar-functor}
\index{cobar construction!complete conilpotent functor}
Type signature: \textup{(}Open quadrant, complete conilpotent
chiral-coalgebra presentation, Beilinson level~$2\to1$, hypothesis
package: coaugmented complete conilpotent chiral coalgebra with
separated finite-type length filtration, continuous reduced
comultiplication, and the canonical desuspended sign convention of
Theorem~\ref{thm:bar-sign-coherence}\textup{)}.
Let
\[
\mathrm{CoAlg}^{\mathrm{ch,conil,comp}}_X
\]
denote the category of coaugmented complete conilpotent chiral
coalgebras on~$X$ in the sense of
Definition~\ref{def:conilpotent-cobar}. There is a functor
\[
\Omega^{\mathrm{ch,cont}}_X\colon
\mathrm{CoAlg}^{\mathrm{ch,conil,comp}}_X
\longrightarrow
\mathrm{dgAlg}^{\mathrm{ch,aug,comp}}_X
\]
whose value on $\cC$ is the completed free chiral algebra
\[
\Omega^{\mathrm{ch,cont}}_X(\cC)
 :=
 \widehat{\mathrm{Free}}_{\mathrm{ch}}(s^{-1}\bar{\cC})
 =
 \varprojlim_N
 \mathrm{Free}_{\mathrm{ch}}(s^{-1}\bar{\cC})/
 F^{N+1}_{\mathrm{len}} .
...
Its differential is
\[
d_\Omega=d_{\cC}+d_{\bar\Delta},
\]
where $d_{\cC}$ is extended as a continuous derivation and
$d_{\bar\Delta}$ is the unique continuous degree-$1$ derivation whose
restriction to a generator $s^{-1}c$, for
$\bar\Delta(c)=\sum c'\otimes c''$, is
\[
d_{\bar\Delta}(s^{-1}c)
 =
 -\sum (-1)^{|c'|}
 (s^{-1}c')\cdot_{\mathrm{ch}}(s^{-1}c'').
\]
Equivalently,
\[
s^{-1}\bar{\cC}
 \xrightarrow{\;s\;} \bar{\cC}
 \xrightarrow{\;-\bar\Delta\;} \bar{\cC}\chirtensor\bar{\cC}
 \xrightarrow{\;(s^{-1}\chirtensor s^{-1})\,\sigma\;}
 (s^{-1}\bar{\cC})\chirtensor(s^{-1}\bar{\cC})
 \hookrightarrow
 \widehat{\mathrm{Free}}_{\mathrm{ch}}(s^{-1}\bar{\cC}),
\]
with $\sigma$ the ordered desuspension sign of
Theorem~\ref{thm:bar-sign-coherence} and
Lemma~\ref{lem:cobar-derivation-extension}. In the uncurved case this
defines a dg chiral algebra. In the curved CDG case it defines the
continuous curved cobar object and
$d_\Omega^{\,2}=[h_\Omega,-]$, as in
Corollary~\ref{cor:cobar-nilpotence-verdier}.
On a coalgebra morphism $f\colon\cC\to\cC'$ the functor sends
$f$ to the unique continuous chiral-algebra morphism generated by
$s^{-1}\bar f\colon s^{-1}\bar\cC\to s^{-1}\bar\cC'$.
\end{theorem}

\begin{proof}
For a complete conilpotent coalgebra the length filtration on the
free chiral algebra is complete and separated, so the displayed
inverse limit is well-defined. A continuous derivation of a completed
free chiral algebra is determined by its restriction to the completed
generators. The internal differential and the desuspended reduced
coproduct therefore determine the two summands of $d_\Omega$
uniquely. The sign is the same desuspension sign used in
Lemma~\ref{lem:cobar-derivation-extension}; with that convention,
coassociativity of $\bar\Delta$ gives the cancellation
$d_{\bar\Delta}^2=0$ on generators, while the coderivation identity
for $d_\cC$ gives $d_\cC d_{\bar\Delta}+d_{\bar\Delta}d_\cC=0$.
The derivation property extends the identity to the completed free
algebra. In the curved case the same calculation is the
Verdier-transposed CDG calculation of
Corollary~\ref{cor:cobar-nilpotence-verdier}. A coalgebra morphism
$f\colon\cC\to\cC'$ induces
$s^{-1}\bar f\colon s^{-1}\bar\cC\to s^{-1}\bar\cC'$ and hence, by the
completed free universal property, a continuous chiral-algebra map
$\Omega^{\mathrm{ch,cont}}_X(f)$ commuting with the displayed
differentials. This proves functoriality.
\end{proof}
...
\begin{proof}
By construction (Definition~\ref{def:algebraic-cobar-free}), the
underlying graded chiral algebra of $\Omega^{\mathrm{ch}}(\mathcal{C})$
is $\mathrm{Free}_{\mathrm{ch}}(s^{-1}\bar{\mathcal{C}})$, the free
chiral algebra generated by $s^{-1}\bar{\mathcal{C}}$ as a
$\mathcal{D}_X$-module. The universal property of free algebras then
gives: for any chiral algebra~$\mathcal{A}$ and graded
$\mathcal{D}_X$-module morphism
$f\colon s^{-1}\bar{\mathcal{C}} \to \mathcal{A}$, there exists a
unique chiral algebra morphism
$\tilde{f}\colon \mathrm{Free}_{\mathrm{ch}}(s^{-1}\bar{\mathcal{C}})
\to \mathcal{A}$ extending~$f$.
...
$\Omega^{\mathrm{ch}} \dashv \bar{B}^{\mathrm{ch}}$ \textup{(}Theorem~A\textup{)}.
...
\textup{(}Theorem~\ref{thm:poincare-verdier},
...
\textup{(}Theorem~\ref{thm:poincare-verdier}\textup{)}, applied
...
Theorem~A: the backbone adjunction
...
$\Omega^{\mathrm{ch}} \dashv \bar{B}^{\mathrm{ch}}$ \textup{(}Theorem~A\textup{)}, verified independently of any Koszulness or
...
\begin{proposition}[Adjunction maps and transferred weak equivalences;
\ClaimStatusConditional]
\label{prop:bar-cobar-adjunction-quillen-scope}
\index{bar-cobar adjunction!unit and counit}
\index{weak equivalence!bar-cobar transferred}
Type signature: \textup{(}Open quadrant, complete conilpotent
bar--cobar presentation, Beilinson levels~$2\leftrightarrow1$,
hypothesis package: the complete cobar functor of
Theorem~\ref{thm:complete-conilpotent-cobar-functor}, the ordered
bar functor, the chiral twisting-morphism bijection
\eqref{eq:psi-eta-hom-bijection}, and, for the final Quillen clause,
the transferred model/coderived package \textup{(Q1)--(Q4)}
below\textup{)}.
On the complete conilpotent surface of
Theorem~\ref{thm:complete-conilpotent-cobar-functor}, the adjunction
\[
\Omega^{\mathrm{ch,cont}}_X \dashv \bar B^{\mathrm{ch}}_X
\]
has the following explicit maps.
\begin{enumerate}[label=\textup{(\roman*)}]
\item For an augmented chiral algebra~$\cA$, the counit
\[
\psi_\cA\colon
\Omega^{\mathrm{ch,cont}}_X\bar B^{\mathrm{ch}}_X(\cA)
\longrightarrow \cA
\]
is the unique chiral-algebra map extending the universal twisting
morphism
$\pi_\cA\colon\bar B^{\mathrm{ch}}_X(\cA)\to\cA$.
Equivalently, if
$b\in\bar B^{\mathrm{ch}}_X(\cA)$ is a bar generator, then
\[
\psi_\cA(s^{-1}b)=\pi_\cA(b),
\]
and the value on a product in the completed free chiral algebra is
obtained by the chiral multiplication of~$\cA$. For a morphism
$u\colon\cA\to\cA'$ of augmented chiral algebras, the naturality square
\[
\begin{tikzcd}[ampersand replacement=\&]
\Omega^{\mathrm{ch,cont}}_X\bar B^{\mathrm{ch}}_X(\cA)
 \arrow[r,"\psi_\cA"]
 \arrow[d,"\Omega_X^{\mathrm{ch,cont}}\bar B_X^{\mathrm{ch}}(u)"'] \&
\cA \arrow[d,"u"]\\
\Omega^{\mathrm{ch,cont}}_X\bar B^{\mathrm{ch}}_X(\cA')
 \arrow[r,"\psi_{\cA'}"'] \&
\cA'
...
\]
commutes.
\item For a complete conilpotent chiral coalgebra~$\cC$, the unit
\[
\eta_\cC\colon
\cC\longrightarrow
\bar B^{\mathrm{ch}}_X\Omega^{\mathrm{ch,cont}}_X(\cC)
\]
is the unique chiral-coalgebra map corresponding to the identity
twisting morphism
$\Omega^{\mathrm{ch,cont}}_X(\cC)\to
\Omega^{\mathrm{ch,cont}}_X(\cC)$ under
\eqref{eq:psi-eta-hom-bijection}. Its projection to the cogenerators
of the cofree coalgebra
$\bar B^{\mathrm{ch}}_X\Omega^{\mathrm{ch,cont}}_X(\cC)$ is the
universal twisting cochain
\[
\tau_\cC\colon \cC\longrightarrow
\Omega^{\mathrm{ch,cont}}_X(\cC),\qquad
c\longmapsto s^{-1}\bar c
\quad\textup{for }c\in\bar\cC .
\]
Equivalently, on the conilpotent finite stages,
\[
\eta_\cC(c)
=
\sum_{r\geq1}
\sum_{\bar\Delta^{(r-1)}(c)=\sum c_1\otimes\cdots\otimes c_r}
[s^{-1}c_1|\cdots|s^{-1}c_r]_{\Omega\cC},
\]
where $[-|\cdots|-]_{\Omega\cC}$ denotes a bar word in
$\bar B_X^{\mathrm{ch}}\Omega_X^{\mathrm{ch,cont}}(\cC)$. The sum is
finite on conilpotent elements and converges in the complete
filtration. For a coalgebra morphism
$v\colon\cC\to\cC'$, the naturality square
\[
\begin{tikzcd}[ampersand replacement=\&]
\cC \arrow[r,"\eta_\cC"] \arrow[d,"v"'] \&
\bar B^{\mathrm{ch}}_X\Omega^{\mathrm{ch,cont}}_X(\cC)
 \arrow[d,"\bar B_X^{\mathrm{ch}}\Omega_X^{\mathrm{ch,cont}}(v)"]\\
\cC' \arrow[r,"\eta_{\cC'}"'] \&
\bar B^{\mathrm{ch}}_X\Omega^{\mathrm{ch,cont}}_X(\cC')
\end{tikzcd}
\]
commutes.
\item The transferred Quillen statement is not part of the bare
adjunction. It is available only after the following package is
imposed:
\begin{enumerate}[label=\textup{(Q\arabic*)}]
\item the complete conilpotent coalgebra side carries the transferred
model or coderived structure whose weak equivalences are exactly the
maps $f$ for which
$\Omega_X^{\mathrm{ch,cont}}(f)$ is a quasi-isomorphism in the chosen
square-zero or completed ambient;
\item the augmented chiral-algebra side carries the corresponding
model or coderived structure for which $\bar B_X^{\mathrm{ch}}$
preserves fibrations and weak equivalences between fibrant objects;
\item the smallness and completeness hypotheses needed for transfer
hold on the finite-type conilpotent stages and pass to the strict
Mittag--Leffler inverse limit;
\item the unit and counit become weak equivalences exactly on the
Koszul/coderived loci of Theorem~\ref{thm:bar-cobar-inversion-qi}.
\end{enumerate}
Under \textup{(Q1)--(Q4)} the pair
$\Omega_X^{\mathrm{ch,cont}}\dashv\bar B_X^{\mathrm{ch}}$ is a
Quillen adjunction; it is a Quillen equivalence precisely on those
Koszul/coderived loci. No unrestricted curved Quillen equivalence is
asserted.
\end{enumerate}
\end{proposition}

\begin{proof}
The Hom-bijection of
Corollary~\ref{cor:chiral-adjunction-hom-bijection} identifies a
chiral-algebra map
$\Omega_X^{\mathrm{ch,cont}}(\cC)\to\cA$ with a twisting morphism
$\cC\to\cA$, and identifies the same twisting morphism with a
coalgebra map $\cC\to\bar B_X^{\mathrm{ch}}(\cA)$. Taking
$\cC=\bar B_X^{\mathrm{ch}}(\cA)$ and the identity coalgebra map gives
the counit. Its restriction to generators is the universal twisting
morphism $\pi_\cA$, hence the displayed formula for $\psi_\cA$ follows
from the completed free-algebra universal property in
Theorem~\ref{thm:complete-conilpotent-cobar-functor}.

Taking $\cA=\Omega_X^{\mathrm{ch,cont}}(\cC)$ and the identity
algebra map gives the unit. The cofree coalgebra universal property
determines a coalgebra map by its projection to cogenerators; that
projection is the universal twisting cochain
$\tau_\cC(c)=s^{-1}\bar c$. Iterating the reduced coproduct gives the
displayed finite-stage expansion, and conilpotence or completeness is
exactly the hypothesis that makes the expansion finite or convergent.
The two naturality squares say that the identity twisting morphism and
the universal twisting morphism are functorial in their variables; the
same assertion gives the triangular identities of the adjunction.

For the Quillen clause, \textup{(Q1)--(Q3)} are precisely the
model-transfer hypotheses for the adjunction. Under those hypotheses
$\Omega_X^{\mathrm{ch,cont}}$ preserves cofibrations and acyclic
cofibrations by definition of the transferred coalgebra-side weak
equivalences, while $\bar B_X^{\mathrm{ch}}$ preserves fibrations and
weak equivalences on the corresponding fibrant algebra objects. Clause
\textup{(Q4)} is the additional unit--counit test for a Quillen
equivalence. Theorem~\ref{thm:bar-cobar-inversion-qi} supplies exactly
that test on the Koszul/coderived loci and denies it outside the
stated convergence, completion, and coderived hypotheses.
\end{proof}

\begin{remark}[The twisting adjunction and the Verdier algebra]
\label{rem:cobar-as-koszul-reflection-inverse}
\index{cobar construction!twisting adjunction}%
The bar functor and the cobar functor have opposite categorical
directions,
\[
...
\]
with counit
$\varepsilon_\cA\colon\Omega_XB_X(\cA)\to\cA$ and unit
$\eta_\cC\colon\cC\to B_X\Omega_X(\cC)$.  Their triangular
identities express bar--cobar reconstruction in the enhanced Ran
ambient and in the Fulton--MacPherson model under its comparison
package.

Verdier duality gives the contravariant functor
\[
K_X=\mathbb D_{\operatorname{Ran}}^{\mathrm{cont}}\circ B_X^{\mathrm{cont}}
\colon
(\operatorname{Alg}^{\mathrm{aug}}_X)^{\mathrm{op}}\longrightarrow
\operatorname{Alg}^{*,\mathrm{aug}}_X.
\]
The natural map~\eqref{eq:verdier-bar-cobar-typed-comparison} compares
$K_X(\cA)$ with $\Omega_X^{\mathrm{geom,cont}}(\mathbb D_X\cA)$.
The derived centre is the third algebra
$C^\bullet_{\mathrm{ch}}(\cA,\cA)$.  These formulas specify every
source, target, and variance directly.
\end{remark}
...
\item \emph{Additivity} ($\log$ sends products to sums):
 ...
 \]
 This holds because the logarithmic normal forms
 \(\eta_{ij}\), represented by \(d\log(z_i-z_j)\) only on
 affine/formal genus-zero screens, are additive under the product of
 chiral brackets:
 $\eta_{ij}^{\cA\otimes\cA'}
 =\eta_{ij}^\cA+\eta_{ij}^{\cA'}$.
\item \emph{Integral kernel}:
 the bar differential is the integral operator with kernel the
 logarithmic normal form \(\eta_{ij}\) along the collision diagonal;
 \(d\log(z_i-z_j)\) is its affine/formal representative, while
...
\begin{theorem}[Poincar\'e--Verdier pairing for the bar object;
\ClaimStatusConditional]\label{thm:poincare-verdier}
Assume $H_{\mathbb D}^{\mathrm{bar}}(\cA)$.  Put
\[
K_X(\cA)=\mathbb D_{\operatorname{Ran}}^{\mathrm{cont}}B_X(\cA).
\]
For every finite Fulton--MacPherson window~$N$, Verdier evaluation gives
a perfect pairing
\[
B_{\leq N}(\cA)\otimes^!
\mathbb D B_{\leq N}(\cA)\longrightarrow
\omega_{\mathrm{FM}_{\leq N}(X)}.
\]
...
\]
the second factor has the FM-current realization dual to the
logarithmic-form realization of the first.  On compactly supported
global sections the pairing is represented by
\[
\langle\omega_{\mathrm{bar}},K_{\mathrm{Verdier}}\rangle
=\int_{\overline C_n(X)}
\omega_{\mathrm{bar}}\wedge\iota^*K_{\mathrm{Verdier}}.
\]
The reconstruction counit
$\Omega_XB_X(\cA)\to\cA$ and the Verdier algebra~$K_X(\cA)$ are the
two algebra-valued constructions attached to the bar coalgebra.
\end{theorem}

\begin{proof}
Perfectness is Verdier biduality for the bounded regular holonomic
complex $B_{\leq N}(\cA)$.  The exchange
$\mathbb D j_*\simeq j_!\mathbb D$ sends logarithmic extensions to
boundary-current extensions and sends each residue morphism to its
transpose.  Compactly supported pushforward gives the displayed scalar
integral.  Compatibility in~\textup{(V4)} assembles the finite-window
pairings, while Theorem~\ref{thm:bar-cobar-verdier} supplies
$\gamma_\cA$.
\end{proof}
...
curvature under the pairing of Theorem~\ref{thm:poincare-verdier}.
...
the same anomaly measured by the curved bar square. For affine
  ...
  Proposition~\ref{prop:km-bar-curvature}. At genus~$1$, the finite
  Heisenberg trace quotient realizes this mechanism by producing the
  standard central extension by $\mathbb{C}$
  (Theorem~\ref{thm:genus1-cobar-bar}); with nonzero curvature one remains
  in the CDG/coderived category.
\end{proof}
...
\index{level!shifted}
Let $\mathfrak g$ be simple, let $k\neq-h^\vee$, and put
$k'=-k-2h^\vee$. Assume $H_{\mathbb D}^{\mathrm{bar}}
(\widehat{\mathfrak g}_k)$ and a constructed quasi-isomorphism of
post-Verdier factorization algebras
\begin{equation}\label{eq:level-shifting-part1}
\nu_k\colon
K_X(\widehat{\mathfrak g}_k)
:=
\mathbb D_{\operatorname{Ran}}^{\mathrm{cont}}
B_X^{\mathrm{ch,cont}}(\widehat{\mathfrak g}_k)
\xrightarrow{\sim}
\widehat{\mathfrak g}_{k'}.
\end{equation}
Then $K_X(\widehat{\mathfrak g}_k)$ has the reflected affine
presentation.  The bar--cobar counit gives the independent
reconstruction map
\[
\Omega_X^{\mathrm{ch,cont}}
B_X^{\mathrm{ch,cont}}(\widehat{\mathfrak g}_k)
\xrightarrow{\sim}\widehat{\mathfrak g}_k.
\]
...
\begin{proof}
The map~$\nu_k$ is the family-specific comparison assumed in the
statement.  Its construction requires the reflected-current map,
compatibility with the bar differential and factorization products,
and strict passage through the completed windows.  The scalar
identities follow from $k'=-k-2h^\vee$; in particular,
\[
\kappa^{\mathrm{KM}}(V_{k'}(\mathfrak g))
=\frac{\dim(\mathfrak g)(k'+h^\vee)}{2h^\vee}
=-\frac{\dim(\mathfrak g)(k+h^\vee)}{2h^\vee}.
...
\begin{theorem}[Essential image recognition with unit witness; \ClaimStatusConditional]
...
Bar-cobar inversion gives $\Omega(\bar{B}(\mathcal{H}_k)) \simeq
\mathcal{H}_k$ automatically, while Koszul duality yields
$\mathcal{H}_k^!\simeq
(\mathrm{Sym}^{\mathrm{ch}}(V^*[1]),m_0=-k\,\omega)$
(cf.\ \S\ref{sec:heisenberg-koszul}). These are distinct statements:
bar-cobar inverts $\mathcal{H}_k$, but the Koszul dual is a different
type of algebra (curved commutative vs.\ Lie). See
Part~\ref{part:characteristic-datum}.
\end{example}
...
At the cochain level, assume the center-transport package of
Theorem~\ref{thm:quantum-complementarity-main} supplies an involution
$\sigma$ on the center local system $\mathcal{Z}(\mathcal{A})$.
...
finite-window projection of the Hall--Borcherds gravity-line datum of
Convention~\ref{conv:master-hall-borcherds-datum}. It consists of the
...
multiplicity.
\item derived-centre trace and $\mathsf{SC}^{\mathrm{ch,top}}$
compatibility on the window: the trace of the current-envelope
comparison is the finite truncation of
$\Delta_5^{-2}$, and the mixed closed-to-open operations commute with
that trace after passing to the radical quotient.
\item strict inverse-system data: for every smaller downward-saturated
window $W'\subset W$, the transition map
$\mathcal H^+_{K3\times E,\le W}\to
\mathcal H^+_{K3\times E,\le W'}$ is compatible with the Hall product,
coproduct, parity fixture, comparison map~$\gamma_W$, trace, and
PBW associated gradeds.
\end{enumerate}
A recognition system is a compatible family of such gates for all
cutoffs $W$, with transition maps commuting with the $\gamma_W$ and
strict enough for the Mittag--Leffler passage in
...
Fourier modes of the half K3 weak Jacobi form $\phi_{0,1}^{K3}$ with
