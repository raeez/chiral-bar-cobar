...
The unit $\eta\colon \cA \to \Omegach(\barBch(\cA))$ of
Theorem~\ref{thm:bar-cobar-adjunction} is the unit of the unified
\emph{Koszul reflection} of
Theorem~\ref{thm:koszul-reflection}\,\ref{KR-i}: the bar and
cobar functors $K = \barBch$ and $K^{-1} = \Omegach$ assemble
into an involutive symmetric-monoidal adjoint equivalence
$K^{2}\simeq\mathrm{id}$ on the Koszul locus $\Kosz(X)$
\textup{(}clause~\ref{KR-ii}\textup{)}, and a coderived
