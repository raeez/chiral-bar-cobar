...
(Theorem~\ref{thm:arnold-three}). The uncurved condition $d^2 = 0$ is equivalent to this cancellation.
...
\emph{Rigorous alternative.} A clean proof avoiding distributional issues entirely is given in Corollary~\ref{cor:cobar-nilpotence-verdier} above, which deduces the uncurved identity $d^2 = 0$ from Verdier duality on the Fulton--MacPherson compactification without reference to distributional products.
...
This is the distributional analog of the Arnold relation.
\emph{Caveat:} products of delta functions supported on intersecting
submanifolds require wavefront set conditions for rigorous definition;
the argument here is heuristic. The rigorous proof of the uncurved
$d^2 = 0$ identity for the cobar complex is provided by Verdier duality
(Corollary~\ref{cor:cobar-nilpotence-verdier}).

In all cases, the signs are chosen so that the Arnold relations
hold, ensuring $d_{\text{extend}}^2 = 0$ in the uncurved Verdier model.
\end{proof}
...
Hence $d_{\text{extend}}^2(K_3) = 0$ in this uncurved three-point model.
\end{example}
...
The geometric cobar is Verdier-dual to the bar
(Section~\ref{sec:bar-nilpotency}): in the uncurved lane the
three-component differential, the $d^2 = 0$ verification, and the
Arnold relations mirror their bar counterparts; in the curved lane the
same mirror gives the inner-curvature square
$d_{\Omega,h}^{\,2}=[h_\Omega,-]$.
\end{remark}
...
For a curved chiral coalgebra $(\mathcal{C}, \kappa_C)$ with
curvature element
$\kappa_C \in \mathcal{C}^{\otimes 2}[2]$, the cobar construction is
the CDG chiral algebra
\[
\Omega^{\mathrm{ch}}_{\mathrm{curv}}(\mathcal{C})
=
\bigl(\Omega^{\mathrm{ch}}(\mathcal{C}), d_{\Omega,h}, h_\Omega\bigr),
\]
not an ordinary cochain complex in general. The curvature term is
geometrically realized by
\[
h_\Omega = \int_{S^1 \times X}
\kappa_C(z, w) \wedge K_{\text{prop}}(z, w)
\]
with $K_{\text{prop}}$ the propagator kernel encoding quantum
corrections. The defining CDG identities are
\[
d_{\Omega,h}^{\,2}=[h_\Omega,-],
\qquad
d_{\Omega,h}(h_\Omega)=0.
\]
The uncurved cobar complex is the special case $h_\Omega=0$.
\end{definition}
...
\item away from the critical level, the bar object is CDG:
 $d_{\bar{B},k}^{\,2}=[m_0,-]$. Strict square-zero holds only at
 critical level, after a curvature-killing Maurer--Cartan twist, or
 after passage to the flat comparison/coderived totalization.
\end{enumerate}
\end{proposition}
...
This is the affine instance of the CDG identity
$d_{\bar{B},k}^{\,2}=[m_0,-]$ with
$m_0=((k+h^\vee)/(2h^\vee))\Omega$.
\end{proof}
...
\emph{Step 1: Individual Nilpotency.} On an uncurved strict chiral
