...
\begin{theorem}[Priddy's Fundamental Discovery; \ClaimStatusProvedElsewhere]
...
\begin{itemize}
\item Degree shift: elements in weight $n$ of $\mathcal{A}_1$ correspond to elements in weight $n$ of $(\mathcal{A}_2^!)^\vee$ (the linear dual), with a cohomological shift by the suspension degree
...
\textbf{Generators to Relations (Ext--Sym duality):}
\begin{itemize}
...
provide crucial examples where Koszul duality manifests as
...
\end{equation}
See Theorem~\ref{thm:yangian-koszul-dual} for the explicit $\mathfrak{sl}_2$ computation.
\end{theorem}

\begin{proof}
\textbf{Step 1: RTT quadraticity.}
In the RTT presentation (Definition~\ref{def:yangian-rtt}), the defining relation $R_{12}(u-v) T_1(u) T_2(v) = T_2(v) T_1(u) R_{12}(u-v)$ is quadratic in $T$.  By Theorem~\ref{thm:yangian-bar-rtt}, the bar complex is concentrated in degrees $\leq 2$.

\textbf{Step 2: Koszul dual via $R$-matrix inversion.}
By Theorem~\ref{thm:yangian-koszul-dual}, the quadratic dual has relation space $R^\perp$, which is the RTT relation for $R^{-1}$.  For the Yang $R$-matrix $R(u) = 1 - \hbar P/u$, the inverse is $R^{-1}(u) = (1 - \hbar P/u)^{-1}$; since $P^2 = \mathrm{id}$, the geometric series gives $R^{-1}(u) = \frac{u^2}{u^2 - \hbar^2}\,I + \frac{\hbar u}{u^2 - \hbar^2}\,P$.  At the \emph{quadratic} level (which is what determines the Koszul dual), the leading approximation $R^{-1}(u) \approx 1 + \hbar P/u = R(u; -\hbar)$ suffices: the higher-order corrections affect only the higher filtration of the Yangian, not the quadratic relations defining the Koszul dual.

\textbf{Step 3: Self-duality for simply-laced $\mathfrak{g}$.}
...
The \textbf{coproduct} $\Delta: \mathcal{A}_2^! \to \mathcal{A}_2^! \boxtimes \mathcal{A}_2^!$ is the universal coproduct from the cofree construction. For generators $\phi_i^* \in \mathcal{V}^{\vee}$, the full coproduct is:
...
But this is \textbf{precisely the associativity constraint} for the chiral product in $\mathcal{A}_2$: the second sum encodes $(\phi_i \cdot \phi_\alpha) \cdot \phi_\beta$ evaluated at $\phi_\gamma$, while the first sum encodes the rearrangement through $\phi_i \cdot (\phi_\alpha \cdot \phi_\beta)$.
...
\begin{remark}[The Fundamental Duality]
...
\medskip
\noindent\textbf{Stage 1: Intrinsic construction of $\mathcal{A}^!$.}
...
\medskip
\noindent\textbf{Stage 2: Verification of coalgebra axioms.}
...
\emph{Nilpotence $d_!^2 = 0$.}  This is the heart of the matter.  By the coderivation property, $d_!^2$ is itself a coderivation, so it suffices to check $d_!^2 = 0$ on cogenerators $\phi_i^* \in \mathcal{V}^{\vee}$.  On cogenerators, the computation in the proof of Theorem~\ref{thm:coalgebra-axioms-verified}, part (4), yields the identity
...
\medskip
\noindent\textbf{Stage 3: The comparison map is a quasi-isomorphism.}
...
\medskip
\noindent\textbf{Logical independence.}  We record that the argument is circularity-free:
\begin{itemize}
...
\subsection{\texorpdfstring{$W_3$ Algebra: Complete Calculation}{W-3 Algebra: Complete Calculation}}
...
\textbf{Key feature:} The structure constants involve \textbf{irrational functions} of the central charge (e.g., $\sqrt{c}$ or $\sqrt{5c+22}$ appear in OPE coefficients), although the OPE poles themselves are always integer-order as required by vertex algebra locality.  This requires:
\begin{itemize}
...
\subsubsection{\texorpdfstring{Feynman Rules $\leftrightarrow$ Bar-Cobar Operations}{Feynman Rules Bar-Cobar Operations}}
...
The \textbf{loop number} (first Betti number) is:
$$\ell(\Gamma) = E - V + 1$$
for a connected graph.  For a ribbon (fat) graph with $F$ boundary components,
the \textbf{genus} is determined by Euler's formula $V - E + F = 2 - 2g$:
...
\subsubsection{\texorpdfstring{Example: Scalar $\phi^4$ Theory}{Example: Scalar 4 Theory}}
...
\begin{theorem}[Kontsevich; \ClaimStatusProvedElsewhere]
...
\textbf{In bar-cobar:} This corresponds to the genus 1 correction to the OPE computed above.
...
\textbf{pro-nilpotent} if the inverse system
\[
...
\begin{remark}[Convergence role of pro-nilpotence]
...
\textbf{Step 1: Reduction to the classical operadic statement.}
...
\textbf{Step 2: Koszulity of $\operatorname{Ass}$ in the chiral
tensor category.}
...
\textbf{Step 3: Geometric realization via configuration spaces.}
...
\textbf{2. Ext-Tor duality:} For any $\mathcal{A}_1$-module $M$, setting $M' = \mathbb{R}\mathrm{Hom}_{\mathcal{A}_1}(\mathcal{A}_2, M)$:
$$\text{Ext}^i_{\mathcal{A}_1}(\mathcal{A}_2, M) \cong \text{Tor}_i^{\mathcal{A}_2}(\mathcal{A}_1, M')^*$$
...
\begin{align*}
\mathrm{Ind}\colon \mathrm{Mod}^{\Eone}_{\mathcal{A}}
&\;\longrightarrow\;
\mathrm{Mod}^{\Eone}_{\mathcal{A}^!}, &
\mathcal{M} &\;\longmapsto\;
\bar{B}^{\mathrm{ch}}(\mathcal{A}) \otimes_{\mathcal{A}} \mathcal{M},
\\
\mathrm{Res}\colon \mathrm{Mod}^{\Eone}_{\mathcal{A}^!}
&\;\longrightarrow\;
\mathrm{Mod}^{\Eone}_{\mathcal{A}}, &
...
\begin{enumerate}[label=\textup{(\roman*)}]
\item Simple $\mathcal{A}$-modules correspond to projective
   $\mathcal{A}^!$-modules.
\item The Ext groups are exchanged:
   $\mathrm{Ext}^i_{\mathcal{A}}(L, M) \cong
   \mathrm{Ext}^i_{\mathcal{A}^!}(\mathrm{Ind}(M), \mathrm{Ind}(L))$.
...
\textbf{(1) Generators $\leftrightarrow$ Relations.}
...
\textbf{(2) Products $\leftrightarrow$ Coproducts.}
...
\subsection{\texorpdfstring{$A_\infty$ Operations Exchange}{A- Operations Exchange}}
...
The Virasoro algebra with central charge $c$ does \textbf{not} admit a Koszul dual in the standard (quadratic, uncurved) sense. However, as shown in Example~\ref{ex:virasoro-koszul-dual}, a \emph{completed curved} Koszul dual can be constructed using the techniques of \S\ref{sec:i-adic-completion}.
...
\textbf{Note}: The Koszul duals of Heisenberg and Kac-Moody are both \textbf{Chevalley-Eilenberg DG chiral algebras} at \emph{negated level}: $\mathcal{H}_k^! = \mathrm{CE}(\mathfrak{h}_{-k})$, $\hat{\mathfrak{g}}_k^! = \mathrm{CE}(\hat{\mathfrak{g}}^{\vee}_{-k-2h^\vee})$. For Heisenberg (abelian, $h^\vee = 0$), the differential $d_{\text{CE}} = 0$, but the curvature $m_0 \neq 0$ (proportional to the level~$k$, encoding the central extension class) is essential.
\end{remark}
...
\textbf{Obstruction}: The bar complex always has $d^2 = 0$ (it is a differential complex by construction), but the Koszul complex has nontrivial cohomology at degree 4:
\begin{equation}
