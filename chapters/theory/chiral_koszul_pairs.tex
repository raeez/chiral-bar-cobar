...
produces the coalgebra $\mathrm{coLie}^{\mathrm{ch}}(V^*)$,
and the cobar of that coalgebra recovers
$\mathrm{Sym}^{\mathrm{ch}}(V^*)$.  The composite
$\Omega_X(\bar{B}_X(\mathcal{H}_k)) \xrightarrow{\;\sim\;} \mathcal{H}_k$
is a quasi-isomorphism.  Two structural features make this work:
\begin{enumerate}
...
recognition criteria for chiral Koszulness.  They apply equally to
...
The corrected Step~4 shows that the bar cohomology of a
non-abelian chiral Koszul algebra is the CE cohomology of
$\fg \otimes t^{-1}\bC[t^{-1}]$, \emph{not} the exterior
algebra $\Lambda^n(V^*)$ directly.  This distinction is
numerically significant: for $\widehat{\mathfrak{sl}}_2$,
the $E_1$ (exterior algebra) totals by conformal weight are
$1, 3, 6, 13, 24, 42, \ldots$
$\bigl(= \prod_{m \geq 1}(1+q^m)^3\bigr)$,
while the $E_2$ (CE cohomology) totals are $1, 3, 5, \ldots$
by bar degree.  The CE value $\dim H^2 = 5$ arises because
$d_1$ kills some of $\Lambda^2(V^*)$:
$\ker(d_1^{2,3}) = 8$ and $\mathrm{im}(d_1^{1,3}) = 3$
at weight~$3$ (the only contributing weight), giving
$8 - 3 = 5$.

Consequently, any formula for
$\dim H^n(\barBgeom(\widehat{\fg}_k))$ must account for the
$d_1$ differential.  Earlier versions of this proof
contained an error in Step~4, claiming $d_1 = 0$ and hence
$E_\infty = E_1 = \Lambda(V^*)$; the correct analysis
gives $E_\infty = E_2 \neq E_1$ for non-abelian algebras.
In particular, the Riordan number prediction
$\dim(\widehat{\mathfrak{sl}}_2^!)_n = R(n{+}3)$
(Appendix~\ref{app:combinatorial-frontier}) requires
revision at degree~$n \geq 2$: the CE computation gives
$\dim H^2 = 5$, not $R(5) = 6$.
\end{remark}
...
\begin{remark}[Bigrading convention]\label{rem:bigrading-convention}
\index{bigrading!bar complex}
Theorem~\ref{thm:bar-concentration} uses the bigrading $(p,q)$
where $p$ is the bar degree (number of tensor factors minus one)
and $q$ is the bar-differential cohomological degree within each
bar component.  The total cohomological degree is $n = p + q$.
Concentration at $q = 0$ means
$H^n_{\mathrm{tot}}(\bar{B}^{\mathrm{ch}}(\cA_1))
= H^{n,0}(\bar{B}^{\mathrm{ch}}(\cA_1)) \cong (\cA_2^!)_n$,
recovering the dimension identity of
Corollary~\ref{cor:bar-cohomology-koszul-dual}.
The $E_2$ collapse of the PBW spectral sequence
(Corollary~\ref{cor:bar-cohomology-koszul-dual}, Step~4)
is the concentration statement in the $(p,q)$ bigrading:
$E_1^{p,q} = 0$ for $q \neq 0$ already at the first page.
\end{remark}
...
\begin{proposition}[Yangian Koszulness for all simple $\mathfrak{g}$; \ClaimStatusProvedHere]\label{rem:yangian-koszul}
...
\cite[Ch.~4]{PP05} applied to $Y(\mathfrak{g})$, and is proved
in full generality in Proposition~\ref{prop:yangian-koszul}.
