...
The cubic shadow $S_3$ is computed from the trilinear form
\[
  C(x,y,z) \;=\; \kappa\bigl(x, [y,z]\bigr),
\]
where $\kappa$ is the invariant bilinear form and $[-,-]$ the
current algebra bracket.  SC-formality forces
$m_3^{\mathrm{SC}} = 0$, hence $S_3 = 0$, hence $C = 0$.
Since $\kappa$ is nondegenerate on the standard landscape,
$C = 0$ implies $[y,z] = 0$ for all $y,z$; the current algebra
is abelian.  An abelian current algebra at level~$k$ is
Heisenberg, which is class~$G$.
\end{proof}
...
Abouzaid~\cite{AbouzaidGeneration} proves that a split-generating object in the
wrapped Fukaya category with wrapped endomorphisms computed from a disk model.
In the ordered setting, \(B^{\mathrm{ord}}(\mathcal A)\) on the disk is the
chain model whose endomorphism algebra is the wrapped endomorphism algebra.
Ganatra--Pardon--Shende~\cite{GPSSectorialDescent} show that sectorial
descent for wrapped Fukaya categories identifies this Koszul model with the
wrapped Fukaya category, and in this identification \(\mathcal A^!\) is the
corresponding wrapped \(A_\infty\)-algebra.
The objects are distinct: \(B^{\mathrm{ord}}(\mathcal A)\) is the ordered
bar object, while \(\mathcal A^!\) is its Koszul dual algebra.
\end{remark}
