...
tests and consequences around chiral Koszulness. Eight are
genuinely independent bidirectional equivalences: $E_2$-collapse
of the bar spectral sequence, vanishing of transferred
$A_\infty$ operations $m_k$ for $k \geq 3$, acyclicity of the
twisted tensor product $K_\tau^L(\cA, \cC)$, truncation of the
shadow tower $\Theta_\cA^{\leq r}$, and four others. The
...
useful for applications. One item (Hochschild duality and
concentration in degrees $\{0,1,2\}$) is a proved one-way
...
identification. On the grading $n = 0$ summand the map is the
obvious inclusion $a \otimes 1 \mapsto a \in \cA[-1]$. The
...
stack because the residue bookkeeping on $\overline{C}_n(X)$ is
...
moduli, not in the standard-chiral-algebra landscape, all of
which are Koszul\textup{)}, no such closed-form contraction
exists in finite bar-degree, and one must pass to the coderived
...
The unconditional content of
Theorem~\ref{thm:koszul-equivalences-meta} separates exactly as
$8 + 1 + 1 + 1 + 1 = 12$ numbered items. Items
\textup{(i)--(v)}, \textup{(ix)--(x)}, and the genus-$0$ clause
of \textup{(vii)} are eight \emph{genuinely independent}
bidirectional equivalences on the standard landscape at
...
non-degeneracy of the ambient tangent complex; unconditional for
the standard landscape by
Proposition~\ref{prop:lagrangian-perfectness}. Item~\textup{(xii)}
...
the opers complex. A separately inscribed characterization,
tropical Koszulness
...
\begin{remark}[Open two-colour transfer frontier]
...
A named inscription of this two-colour transfer lemma (the
analogue of Theorem~\ref{thm:e1-primacy} in the
ordered-to-mixed direction rather than the ordered-to-symmetric
direction) is a frontier item: Theorem~\ref{thm:e1-primacy}(iv)
establishes that $\ker(\operatorname{av})$ is non-trivial
(contains the Drinfeld associator at degree~$3$), so the transfer
from the mixed-sector vanishing to the ordered tree vanishing
requires an injectivity statement genuinely different from the
averaging map. Pending inscription, the converse direction of
Proposition~\ref{prop:sc-formal-iff-class-g} is conditional on
this lemma; the forward direction is unconditional.
\end{remark}
...
The unconditional section (items \textup{(i)--(vii)},
\textup{(ix)}, \textup{(x)}: eight genuinely independent
bidirectional equivalences plus item~\textup{(vi)} as a listed
consequence of~\textup{(v)}) together with the Hochschild
...
unconditional equivalence core lose force simultaneously --- eight
genuinely independent bidirectional equivalences (items
\textup{(i)--(v)}, \textup{(ix)--(x)}, and the genus-$0$ clause of
\textup{(vii)}) and the listed consequence~\textup{(vi)} ---
...
Each of the eight genuinely independent bidirectional
equivalences \textup{(i)--(v)}, \textup{(ix)--(x)}, and the
genus-$0$ clause of \textup{(vii)} in
...
decategorification recovers the scalar identity
...
\section{Wave 13 DNA perspective: K3 chiral Hall--Drinfeld double as Koszul pair}
\label{sec:ckp-wave13-DNA}

\begin{remark}[K3 chiral bialgebra as Koszul-pair datum]
\label{rem:ckp-DNA-hall-drinfeld-pair}
At $\cC = D^b\mathrm{Coh}(K3)$ and CY-dimension $d = 2$, the K3 chiral bialgebra of Vol~III Wave~13 presents as a chiral Koszul pair with abelian core + BKM vertex extension: Lie-level $\mathbf{H}_{\Delta_5}^{\mathrm{Lie}} = 24$ copies of Miki $U_{q_1, q_2}(\widehat{\widehat{\mathfrak{gl}}}_1)$ with $\widetilde{M}_{24}$-twisted permutation via the umbral cocycle (Cheng--Duncan--Harvey $2014$); vertex-level closure via Frenkel--Kac $1980$ on $\mathcal{F}_{K3} = \mathrm{Sym}(\bigoplus_{n \ge 1} H^*(K3) \otimes t^{-n})$ produces the BKM superalgebra $\mathfrak{g}_{\Delta_5}$ as non-abelian extension. The Koszul dual coalgebra of this pair satisfies
\[
  (\mathbf{H}_{\Delta_5})^! \;\simeq\; V(\mathfrak{g}_{\Delta_5})^{\mathrm{coalg}}[2] \otimes \eta_{\widetilde{M}_{24}},
\]
with $[2]$-shift (CY-$2$) and projective Schur cocycle $\eta_{\widetilde{M}_{24}}$ of order $6$ in $H^2(M_{24},\mathrm{U}(1))\cong\mathbb{Z}/12$.
\end{remark}

\begin{remark}[Koszul conductor $K^{\kappa_{\mathrm{ch}}}=8$ on Mukai-enhanced K3]
\label{rem:ckp-DNA-mukai-doubling}
The Koszul-pair conductor of the Mukai-enhanced K3 Koszul pair evaluates to
\[
...
\]
with $c_+ = 4$ from the positive signature of the rank-$24$ Mukai pairing on $\Lambda_{\mathrm{Muk}} = \mathrm{II}_{4,20}$. This is the Wave~13 $\mathcal{B}$-family entry to the Theorem~C bucket set $\kappa + \kappa^! \in \{0, 8, 13, 250/3, 98/3\}$. The universal identity $\hbar^2 \cdot K^{\kappa_{\mathrm{ch}}} = -1$ pins the Lusztig $u_\zeta$-specialisation at $\ell = 8$, matching the order of the monodromy of $\mathcal{L}^{\Delta_5}$ around the Humbert divisor $H_1 \subset \overline{\mathcal{A}_2}$ (Bruinier $2002$ Proposition~$5.1$ Heegner Chern-class reciprocity).
\end{remark}

\section{Wave 13 DNA perspective (deep saturation)}
\label{sec:ckp-wave13-DNA-deep}

\begin{remark}[The Wave 13 Koszul triple $(\mathbf H_{\Delta_5}, \mathbf H^!_{\Delta_5}, B_{\mathrm{ch}}(\mathbf H_{\Delta_5}))$]
\label{rem:ckp-DNA-deep-1}
The Wave~13 Koszul triple on the bi-based Ran-space
$(\mathrm{Ran}(E^{\mathrm{nod,sm}}_{24}),\bar{\mathcal A}_2)$ consists
of: the chiral bialgebra $\mathbf H_{\Delta_5}$; its Verdier-Koszul
dual $\mathbf H^!_{\Delta_5}$; and the bar coalgebra
$B_{\mathrm{ch}}(\mathbf H_{\Delta_5}) = T^c(s^{-1}\overline{\mathbf H_{\Delta_5}})$
in the Hall subcategory of $\mathrm{CoHA}_{K3\times E}$. The triple
satisfies the Koszul sign rule and cyclicity from the Mukai pairing
on $\Lambda_{\mathrm{Muk}} = II_{4,20}$. Cross-ref Vol.~III,
Chapter~\ref{ch:k3-chiral-bialgebra-platonic}.
\end{remark}

\begin{remark}[Self-Koszulness at $\hbar^2 = -1/8$]
\label{rem:ckp-DNA-deep-2}
At the Lusztig specialisation $\hbar^2 = -1/\ell = -1/8$ the Wave~13
Koszul pair is \emph{self-dual}: there is a canonical quasi-isomorphism
$\mathbf H^!_{\Delta_5}\simeq \mathbf H_{\Delta_5}$ in the derived Hall
category of $K3\times E$. The obstruction to self-Koszulness away from
$\hbar^2 = -1/8$ is controlled by the Bruinier 2002 Prop.~5.1 Heegner
Chern-class reciprocity: on the B-family, the Chern class
$c_1(L^{\Delta_5})\mid_{H_1}\in\mathrm{CH}^1(H_1)$ is $\mathbb Z/8$-torsion,
and the self-Koszulness obstruction is a representative of this class.
Primary-lit: Bruinier 2002 (arXiv:math/0108079), Mukai 1987.
\end{remark}

\begin{remark}[Koszul complex on the bi-based Ran-space]
\label{rem:ckp-DNA-deep-3}
The Wave~13 Koszul complex on $(\mathrm{Ran}(E^{\mathrm{nod,sm}}_{24}),\bar{\mathcal A}_2)$
is
\[
\mathrm{Kos}_\bullet(\mathbf H_{\Delta_5})\ :\ \dotsb\to \mathrm{Sym}^k(\mathbf H_{\Delta_5}[-1])\otimes\Lambda^k(\mathbf H_{\Delta_5}^\vee)\to\dotsb
\]
with differential supplied by the Gritsenko-Nikulin denominator factor.
The Koszul complex computes the chiral Hochschild cohomology
$\mathrm{ChirHoch}^\bullet(\mathbf H_{\Delta_5})$ (Vol.~I,
\S\ref{sec:hochcoh-wave13-DNA}), concentrated in degrees $\{0,1,2\}$
by Theorem~H, with the degree-$2$ component the $\mathbb C\cdot \Delta_5$
line from Etingof-Kazhdan 2007 Part~V super-category extension.
\end{remark}
