...
The passage from classical Koszul duality
(Chapter~\ref{ch:algebraic-foundations}) to its chiral counterpart is
not a routine generalization: it is a shift from algebraic composition
to meromorphic locality.  On a point, the bar complex
$\barB(A) = T^c(s^{-1}\bar{A})$ is built from tensor powers of the
augmentation ideal and the product map; on a curve~$X$, the chiral bar
complex $\barB_X(\cA)$ is built from configuration spaces of colliding
points and integration kernels with prescribed pole orders along
diagonals.  The tensor product $V^{\otimes n}$ is replaced by the
factorization product $\cA^{!n}$ over the Fulton--MacPherson
compactification $\FM_n(X)$; the differential is no longer a
combinatorial sum of face maps but a sum of residues at boundary
strata, each governed by the OPE singularities of~$\cA$.  This is
where the operadic story meets Beilinson--Drinfeld chiral algebra
theory, and where every subsequent construction in the monograph finds
its genus-zero foundation.

The central conceptual point is that the chiral extension preserves the
Maurer--Cartan architecture of the classical theory while enriching its
geometry.  A classical twisting morphism
$\tau \colon \barB(A) \to A^!$ is a degree-$+1$ element in the
convolution dg Lie algebra $\Hom(\barB(A), A)$ satisfying $d\tau +
\tau \star \tau = 0$; its chiral analogue
...
integration kernel on $X^2 \setminus \Delta$.  The acyclicity of the
twisted tensor product $K_\tau^L(\cA, \cC)$ (the defining property of
a Koszul pair) is checked by spectral sequence methods adapted to the
PBW filtration by bar degree
(Theorem~\ref{thm:pbw-koszulness-criterion}).
Francis--Gaitsgory~\cite{FG12} extend chiral and factorization theory
to the homotopy world: their $\chirLie \dashv \chirCom$ adjunction on
$\Ran(X)$ is the operadic backbone of our Theorem~A
(Theorem~\ref{thm:bar-cobar-isomorphism-main}).  The present chapter
extends their framework beyond the Lie--Com pair to arbitrary chiral
Koszul pairs, including the non-quadratic examples (Virasoro,
$\mathcal{W}$-algebras, Yangians) that the standard operadic
adjunction does not directly cover.

The constructions assembled here constitute the genus-zero gate to
three structure theorems that emerge at higher genus.  Classical Koszul
pairs (Priddy~\cite{Priddy70}; Beilinson--Ginzburg--Soergel~\cite{BGS96})
govern decomposition over a point; the chiral extension enriches
duality with the configuration-space geometry of colliding points on
curves.  Three phenomena distinguish the chiral setting from its
...
Lagrangian complementarity $Q_g(\cA) \oplus Q_g(\cA^!) \simeq
H^*(\overline{\mathcal{M}}_g, Z(\cA))$
(Theorem~\ref{thm:quantum-complementarity-main});
and the critical discriminant $\Delta = 8\kappa S_4$, whose vanishing
controls termination of the shadow Postnikov tower
(Theorem~\ref{thm:riccati-algebraicity}).
At genus zero all three collapse: $\kappa$ is not yet defined,
complementarity reduces to the classical bar-cobar adjunction, and
$\Delta$ plays no role.  The present chapter therefore recovers
classical Koszul duality as a tautological special case and
simultaneously provides the genus-zero input from which the full
modular theory is built.
The standard families exemplify the four shadow-depth classes
(Theorem~\ref{thm:shadow-archetype-classification}):
Heisenberg (class~$G$, depth~$2$, formal),
affine Kac--Moody (class~$L$, depth~$3$, single Massey product),
$\beta\gamma$ (class~$C$, depth~$4$, contact then rigid),
Virasoro and $\mathcal{W}_N$ (class~$M$, depth~$\infty$,
intrinsically non-formal).
The genus-zero arity-two projection of the universal MC element
$\Theta_\cA$ (Theorem~\ref{thm:mc2-bar-intrinsic}) recovers the
classical $R$-matrix, linking the Koszul pairs of this chapter to
the Drinfeld--Kohno bridge of
Chapter~\ref{chap:koszul-pair-structure}.

The chapter proceeds in three stages.
First (\S\ref{sec:chiral-twisting-data}ff.), we extract from the
Heisenberg example the general notion of chiral twisting data and
formulate the PBW recognition criterion.  Second, we define chiral
Koszul pairs (Definition~\ref{def:chiral-koszul-pair}) and prove the
full bar-cobar adjunction and inversion on the Koszul locus
(Theorem~A).  Third, we establish the Verdier intertwining that
connects $\barB_X(\cA)$ with $\barB_X(\cA^!)$ and thereby completes
the genus-zero layer of the duality.  The passage to higher genus
(Chapter~\ref{chap:higher-genus}), where curvature enters and the
differential ceases to square to zero, builds on every structure
assembled here.
...
lives in a modular convolution algebra bigraded by arity and genus.
...
The shadow tower probes all genera; Koszulness constrains only
genus~$0$.  Twelve equivalent characterizations of chiral
Koszulness (ten proved unconditionally, one conditional on
perfectness/nondegeneracy (K11, the Lagrangian criterion), and one
partially proved (D-module purity, forward direction established)), each
...
\begin{theorem}[Converse: formality implies chiral Koszulness;
\ClaimStatusProvedHere]
...
(Proposition~\ref{prop:shadow-formality-low-arity}).  On each fiber,
...
shadow Postnikov tower $\Theta_\cA^{\leq 3}, \Theta_\cA^{\leq 4},
...
measuring non-formality.  The shadow Postnikov tower
...
\label{conj:ext-diagonal-vanishing}%
...
The reason is categorical: the ordinary positive-energy module
category $V$-$\mathrm{gmod}$ controls Zhu theory and conformal
blocks, whereas the bar complex computes Ext groups in the
chiral/factorization module setting
...
\item finite-page bar and Li--bar spectral sequences in the
  ordinary-character calculations
...
simple quotient.  Thus admissible simple quotients remain a live
audit surface rather than a closed theorem.
\end{remark}
...
\item When $R_V \cong \operatorname{Sym}(V_0)$ is a polynomial
  algebra with trivial Poisson bracket, $d_1 = 0$ and the
  spectral sequence degenerates at~$E_1$.
\item When $R_V = \mathcal{O}(Y)$ for an affine Poisson
  variety~$(Y, \pi)$, the $E_1$ page computes
  $H^*_{\mathrm{bar}}(\mathcal{O}(Y), \{-,-\}_\pi)$:
  the bar cohomology of the commutative algebra
  $\mathcal{O}(Y)$ with differential induced by~$\pi$.
\end{enumerate}
\end{proposition}
...
For~(ii): when $R_V = \mathcal{O}(Y)$ with $Y$ Poisson,
$d_1$ adds the Lichnerowicz--Poisson differential to the
...
\begin{enumerate}[label=\textup{(\roman*)}]
\item \emph{Sufficiency from concentrated Poisson bar
  cohomology.}
  If the Poisson-deformed bar cohomology of~$R_V$
...
\item \emph{Orbit criterion.}
  ...
  the Kirillov--Kostant Poisson structure.  Then
  $V$~is chirally Koszul if and only if the Poisson
  cohomology
  \[
  HP^n(\overline{\mathbb{O}}) :=
  H^n\!\bigl(\mathcal{O}(\overline{\mathbb{O}}),
    \delta_\pi\bigr)
  \]
  \textup{(}with $\delta_\pi = [\pi, -]_{\mathrm{SN}}$ the
  Lichnerowicz--Poisson differential\textup{)} is concentrated
  on the diagonal of the Li--bar bigrading.
\item \emph{Symplectic resolution criterion.}
  If $\overline{\mathbb{O}}$ admits a symplectic
  resolution $\widetilde{\mathbb{O}} \to
  \overline{\mathbb{O}}$ \textup{(}e.g., Springer resolution
  for the nilpotent cone, Slodowy slices for subregular
  orbits\textup{)}, then the Poisson cohomology is computed by
  \[
  HP^n(\overline{\mathbb{O}})
  \cong H^n(\widetilde{\mathbb{O}},\,
    \mathcal{O}_{\widetilde{\mathbb{O}}}).
  \]
  Diagonal concentration of the right-hand side is a
  cohomological condition on the resolution.
\end{enumerate}
\end{theorem}
...
For~(ii): the orbit case is the first instance where the
geometric criterion has genuine content beyond the
Kac--Shapovalov test.  The Poisson variety
$(\overline{\mathbb{O}}, \pi)$ inherits a Poisson
bivector from the Kirillov--Kostant structure
$\{f, g\}(x) = x([df_x, dg_x])$ on~$\fg^*$.  The
Lichnerowicz--Poisson differential $\delta_\pi$ acts
on the Chevalley--Eilenberg-type complex of polyvector
fields restricted to~$\overline{\mathbb{O}}$.

Forward: if $V$ is Koszul, the PBW spectral sequence
collapses at~$E_2$
(Theorem~\ref{thm:pbw-koszulness-criterion}).  Since the
Li filtration refines the PBW filtration, the Li--bar $E_2$
page must also be diagonally concentrated, which forces the
Poisson cohomology to be diagonal.

Converse: diagonal concentration of $HP^*$ gives diagonal
$E_2$ by~(i), hence $E_2 = E_\infty$, hence Koszulness.

For~(iii): when $\widetilde{\mathbb{O}} \to
\overline{\mathbb{O}}$ is a symplectic resolution, the
Brylinski spectral sequence computes the
Poisson cohomology of the singular space via the
cohomology of the resolution.  The isomorphism
$HP^n(\overline{\mathbb{O}}) \cong
H^n(\widetilde{\mathbb{O}}, \mathcal{O})$ holds because
the resolution is crepant (symplectic) and the Poisson
deformation complex on the smooth locus extends.
The diagonal concentration question then becomes:
does $H^n(\widetilde{\mathbb{O}}, \mathcal{O}) = 0$
for $n \neq 0$?  For the Springer resolution
$T^*(G/B) \to \mathcal{N}$, this is
$H^n(T^*(G/B), \mathcal{O})$, which is known from the
Grauert--Riemenschneider vanishing theorem.
\end{proof}

\begin{corollary}[Minimal-orbit levels under a reducedness
assumption; \ClaimStatusConditional]
...
\begin{proof}
The minimal nilpotent orbit closure
$\overline{\mathbb{O}}_{\min} = \mathbb{O}_{\min}
\cup \{0\}$ admits a symplectic resolution by the
cotangent bundle of the minimal flag variety:
$T^*(\mathbb{P}(V_{\omega_1})) \to
\overline{\mathbb{O}}_{\min}$ (for type~$A$, this is
$T^*\mathbb{P}^{N-1} \to
\overline{\mathbb{O}}_{\min} \subset \mathfrak{sl}_N^*$).
By Theorem~\ref{thm:associated-variety-koszulness}(iii),
the Poisson cohomology is
\[
HP^n(\overline{\mathbb{O}}_{\min})
\cong H^n(T^*\mathbb{P}^{N-1},\,
  \mathcal{O}_{T^*\mathbb{P}^{N-1}}).
\]
The minimal nilpotent orbit closure $\overline{\mathbb{O}}_{\min}$ is an affine variety, so $H^n(\overline{\mathbb{O}}_{\min}, \mathcal{O}) = 0$ for $n > 0$.  By the Grauert--Riemenschneider theorem applied to the resolution $T^*\mathbb{P}^{N-1} \to \overline{\mathbb{O}}_{\min}$, the Poisson cohomology is concentrated in degree~$0$, giving diagonal concentration of the Li--bar $E_2$ page.

For general type: $\overline{\mathbb{O}}_{\min}$ always
admits a symplectic resolution by the cotangent bundle of
the projective variety $\mathbb{P}(V_\theta) \cong G/P_\theta$
where $\theta$ is the highest root and $P_\theta$ the
corresponding parabolic.  The coherent cohomology
$H^n(T^*(G/P_\theta), \mathcal{O})$ vanishes for $n > 0$
...
\item \emph{Reduced obstruction.}
  The Poisson cohomology of the reduced variety
  $\overline{\mathbb{O}}$ equipped with the
  Kirillov--Kostant bracket.  By Broer's theorem and
  affineness of~$\overline{\mathbb{O}}$, the Springer
  resolution gives $HP^n(\overline{\mathbb{O}}) = 0$
  for $n > 0$ whenever $\overline{\mathbb{O}}$ admits
  a symplectic resolution with
  $R^i\mu_*\mathcal{O} = 0$ for $i > 0$.  This holds
  for the full nilcone and for minimal orbit closures
  in type~$A$, so the \emph{reduced} Poisson cohomology
  is concentrated.
\item \emph{Nilradical obstruction.}
  ...
  $\mathcal{I} \neq 0$, the bar complex of the
  non-reduced algebra $R_{L_k}$ can have
  off-diagonal cohomology even when the reduced quotient
  does not.  Concretely: the null vectors of $L_k(\fg)$
  that are not visible on $\overline{\mathbb{O}}$
  contribute nilpotent elements to $R_{L_k}$, and the
  Poisson bar cohomology of
  $R_{L_k} \twoheadrightarrow
  \mathcal{O}(\overline{\mathbb{O}})$ detects them
  via the long exact sequence in bar cohomology.
\end{enumerate}
\end{proposition}

\begin{proof}
For~(i): the nilpotent cone $\mathcal{N} \subset \fg^*$
is an affine variety.  When $\overline{\mathbb{O}}$
admits a symplectic resolution
$\mu \colon \widetilde{\mathbb{O}} \to
\overline{\mathbb{O}}$ with
$R^i\mu_*\mathcal{O} = 0$ for $i > 0$
(Broer's theorem holds for the nilcone in type~$A$ and
for minimal orbit closures), the Leray spectral
sequence gives
$H^n(\widetilde{\mathbb{O}}, \mathcal{O})
= H^n(\overline{\mathbb{O}},
\mu_*\mathcal{O})
= H^n(\overline{\mathbb{O}}, \mathcal{O}) = 0$
for $n > 0$ by affineness.

For~(ii): the short exact sequence
$0 \to \mathcal{I} \to R_{L_k} \to
\mathcal{O}(\overline{\mathbb{O}}) \to 0$
induces a long exact sequence in bar cohomology.
The connecting homomorphism
$\delta \colon H^n_{\mathrm{bar}}(
\mathcal{O}(\overline{\mathbb{O}}))
\to H^{n+1}_{\mathrm{bar}}(\mathcal{I})$
can produce non-zero classes: the ideal $\mathcal{I}$
is generated by the images of singular vectors under
the Li filtration, and the Poisson bracket may pair
a reduced class with a nilpotent one to produce
off-diagonal bar cohomology.

The obstruction to Koszulness at degenerate levels is
thus controlled by the interplay between the Poisson
geometry of $\overline{\mathbb{O}}$ (which is well-behaved)
and the nilpotent fiber $\mathcal{I}$ (which encodes
the singular-vector relations of~$L_k$).
\end{proof}
...
\textbf{Orbit} & \textbf{Partition} &
  \textbf{Reduced Poisson} & \textbf{Koszulness} \\
\midrule
$\{0\}$ & $(1^N)$ &
  finite-dim.\ & nilradical-dependent \\
$\mathbb{O}_{\min}$ & $(2, 1^{N-2})$ &
  $H^{>0} = 0$ & conditional on reducedness
  \textup{(}Cor~\ref{cor:minimal-orbit-koszul}\textup{)} \\
$\mathbb{O}_{\mathrm{subreg}}$ & $(N{-}1, 1)$ &
  Slodowy slice, $H^{>0} = 0$ & nilradical-dependent \\
$\mathcal{N}$ & $(N)$ &
  Springer, $H^{>0} = 0$ & nilradical-dependent \\
\bottomrule
\end{tabular}
\end{center}

\smallskip\noindent
The reduced Poisson cohomology vanishes in positive degree
for many orbit closures admitting symplectic resolutions
\textup{(}by Broer's theorem $+$ affineness\textup{)}.
This controls the \emph{reduced} part of the Li--bar
spectral sequence, but it does not remove nilradical
contributions.  Even when $X_{L_k}$ is a point or a minimal
orbit closure, the full simple-quotient problem still requires a
check that $R_{L_k}$ carries no extra nilpotent bar
cohomology.
...
The gain is maximal when $X_V \neq \{0\}$ but the Poisson
structure is sufficiently controlled (e.g., when
$\overline{\mathbb{O}}$ has a symplectic resolution with
vanishing higher coherent cohomology) to reduce the problem to
the nilradical of~$R_V$.  Minimal-orbit levels are the first
test case where the reduced geometry suggests concentration, but
an unconditional Koszul theorem still requires the extra
reducedness hypothesis recorded in
Corollary~\ref{cor:minimal-orbit-koszul}.
\end{remark}

\begin{theorem}[Equivalences of chiral Koszulness; \ClaimStatusProvedHere]
...
them.  Condition~\textup{(x)} implies condition~\textup{(xii)}.
...
\item[\textup{(vii)}] Factorization homology
  ...
  for all~$g$.
\item[\textup{(viii)}] $\mathrm{ChirHoch}^*(\cA)$ is a polynomial algebra
  with generators concentrated in degrees $\{0, 1, 2\}$
  \textup{(}Theorem~H,
...
\textup{(iii)}$\Leftrightarrow$%
\textup{(v)}$\Leftrightarrow$\textup{(viii)}:
...
$\Omega(\barBgeom(\cA)) \xrightarrow{\sim} \cA$ is a free
resolution.  The bigraded Hochschild complex inherits the bar-cobar
filtration, forcing polynomial vanishing
(Theorem~\ref{thm:main-koszul-hoch}).

\smallskip\noindent
\textup{(viii)}$\Rightarrow$\textup{(i)}:
Polynomial growth of $\mathrm{ChirHoch}^*$
(Theorem~\ref{thm:hochschild-polynomial-growth}) constrains the
bar complex growth rate, forcing Koszul-complex acyclicity.
...
each FM stratum (Proposition~\ref{prop:shadow-formality-low-arity});
...
filtration of $\int_{\Sigma_g} \cA$ has associated graded
given by $\barB_n$ contributions, each concentrated in degree~$0$
by PBW collapse.

\smallskip\noindent
\textup{(vii)}$\Rightarrow$\textup{(i)}:
Suppose $H^k(\int_{\Sigma_g} \cA) = 0$ for $k \neq 0$ at all~$g$.
At $g = 0$ with~$\Sigma_0 = \mathbb{P}^1$, the factorization
...
differential restricts to the tensor product of lower-arity bar
complexes.  $E_2$-collapse at each vertex arity
...
non-degeneracy of the ambient tangent complex.  Condition~\textup{(x)}
implies condition~\textup{(xii)}, but the converse direction is still
open.
\end{remark}
...
\begin{proof}
Condition~(i) is equivalent to the vanishing of all genus-$0$
obstruction classes $o_{r+1}^{(0)}(\cA) = 0$ for $r \geq 2$, which by
Proposition~\ref{prop:shadow-formality-low-arity} is the
$A_\infty$-formality condition~(iii) of
Theorem~\ref{thm:koszul-equivalences-meta}.
Condition~(ii) is equivalent to~(i) by the arity filtration on $\gAmod$:
$E_2$-degeneration of the bar spectral sequence is PBW collapse,
condition~(ii) of Theorem~\ref{thm:koszul-equivalences-meta}.
Condition~(iii) is equivalent to~(i) because a dg Lie algebra is formal
if and only if all higher Massey brackets vanish, and the genus-$0$
Massey brackets of $\mathfrak{g}^{(0)}_\cA$ are precisely the
obstruction classes $\{o_{r+1}^{(0)}\}_{r \geq 2}$.
\end{proof}
...
\index{shadow Postnikov tower!depth vs.\ Koszulness}
...
The shadow Postnikov tower probes the \emph{dynamic} axis of the
...
(\cA_2)_\infty$ (underlying complex $\simeq \bar{B}_X(\cA_2)$).
...
\cA^!_\infty$ (underlying complex $\simeq \bar{B}_X(\cA^!)$)
...
\item \emph{Homological algebra:} Derived equivalences, Ext-Tor duality, spectral sequences
