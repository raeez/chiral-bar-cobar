...
Chapter~\ref{ch:heisenberg-frame} identified the Koszul dual of the Heisenberg algebra as $\mathrm{Sym}^{\mathrm{ch}}(V^*)$.  This chapter develops the general theory: when two chiral algebras are related by bar-cobar duality, and how the Koszul property controls spectral sequence collapse.
...
Chapters~\ref{chap:config-spaces}--\ref{chap:bar-cobar}
constructed bar and cobar functors intertwined by Verdier
duality on $\operatorname{Ran}(X)$.  The Koszul property (diagonal
Ext vanishing) identifies when the adjunction is an equivalence;
it is the genus-$0$ prerequisite for modular Koszulity
(Definition~\ref{def:modular-koszul-chiral}) and structures the entire
landscape of examples in Part~II.
...
theorem of twisting morphisms \cite[Theorem~2.3.1]{LV}.
\end{remark}
...
For a chiral twisting datum $(\cA, \cC, \tau, F_\bullet)$
...
The acyclicity of $K_\tau^L(\cA,\cC)$ means the twisted
differential $d_\cA + d_\cC + d_\tau^L$ on $\cA \otimes \cC$
is exact.  This implies the counit
$\varepsilon_\tau\colon \Omega_X(\cC) \to \cA$ induces an
isomorphism on cohomology: the twisted tensor product
$K_\tau^L$ is precisely the mapping cone of~$\varepsilon_\tau$,
and its acyclicity is equivalent to $\varepsilon_\tau$ being a
quasi-isomorphism.

\emph{\ref{ftm:acyclic}$\Rightarrow$\ref{ftm:unit}:}
Dually, $K_\tau^R(\cC,\cA)$ acyclic implies
$\eta_\tau\colon \cC \to \bar{B}_X(\cA)$ is a weak equivalence
of coalgebras, since $K_\tau^R$ is the mapping cone of~$\eta_\tau$.

\emph{\ref{ftm:counit}$\Rightarrow$\ref{ftm:koszul}:}
If $\varepsilon_\tau$ is a quasi-isomorphism, filter
$\Omega_X(\cC)$ by cobar degree and use the convergence
hypothesis (Definition~\ref{def:chiral-koszul-morphism},
condition~3) to deduce $E_2$ collapse of the associated
spectral sequence.  The associated graded level gives
classical Koszulity (condition~2), and the acyclicity of the
twisted tensor products follows from the quasi-isomorphism via
the mapping cone identification.

\emph{\ref{ftm:unit}$\Rightarrow$\ref{ftm:koszul}:}
Symmetric argument applied to the unit.

The key geometric input throughout is that the filtration
$F_\bullet$ on $\bar{B}_X(\cA)$ and $\Omega_X(\cC)$ is
compatible with the FM compactification stratification, so
the spectral sequence convergence is ensured by the strong
...
To handle non-quadratic examples, we extend the notion of Koszul pairs beyond the quadratic setting. The defining property is a set of \emph{recognition criteria}---checkable conditions on the pair---from which the full bar-cobar identification follows as a theorem.
...
condition (Definition~\ref{def:chiral-koszul-morphism}), and
Verdier compatibility can all be verified without invoking
bar-cobar duality itself.
Theorem~\ref{thm:bar-cobar-isomorphism-main} then \emph{proves}
the full bar-cobar identification as a consequence.
...
\begin{remark}[From recognition to identification]\label{rem:koszul-relationship}
The recognition criteria of Definition~\ref{def:chiral-koszul-pair}
are inputs; Theorem~\ref{thm:bar-cobar-isomorphism-main} produces
the following identification as output:
...
These diagrams are \emph{consequences} of the recognition criteria,
...
\begin{remark}[Theorem~A as a theorem about Koszul pairs]
\label{def:chiral-dual-pair}% backward-compatible label
\label{rem:theorem-a2-dual-pairs}
Theorem~\ref{thm:bar-cobar-isomorphism-main}
is a theorem about chiral Koszul pairs
(Definition~\ref{def:chiral-koszul-pair}): the fundamental theorem
(Theorem~\ref{thm:fundamental-twisting-morphisms}) produces
$\cC_i \simeq \bar{B}_X(\cA_i)$, and Verdier compatibility
gives $\mathbb{D}_{\operatorname{Ran}} \bar{B}_X(\cA_1) \simeq
\bar{B}_X(\cA_2)$.
\end{remark}
...
The PBW criterion reduces chiral Koszulness to the
classical statement that the associated graded --- a commutative
chiral algebra, hence a vertex Poisson algebra --- is Koszul.
For vertex enveloping algebras of Lie$^*$-algebras
(such as affine Kac--Moody algebras), the associated graded is
the symmetric algebra $\operatorname{Sym}(\fg[t^{-1}]t^{-1})$,
which is Koszul by Priddy's theorem (a polynomial algebra is
Koszul with dual the exterior algebra).  For the Virasoro algebra,
the associated graded is $\operatorname{Sym}(L_{-2}, L_{-3}, \ldots)$,
which is also a polynomial algebra and hence Koszul.
The key point is that Koszulness of the chiral algebra does
\emph{not} require the underlying Lie algebra to be semisimple ---
it follows from the PBW structure alone.
\end{remark}
...
\emph{Step~4: Collapse.}
Since $E_1^{p,q} = 0$ for $q \neq 0$, all differentials
$d_r\colon E_r^{p,q} \to E_r^{p+r, q-r+1}$ vanish for
$r \geq 1$ (they map into or out of the zero row).
Therefore $E_2 = E_1 = E_\infty$ and:
\[
...
$J^a_{-n}$ for $n \geq 1$).
At conformal weight $h$ in bar degree $n$:
\[
\dim \Lambda^n(V^*)_h
= \dim \Lambda^n(\bC^3 \otimes t\bC[t])_h.
\]
For $n=1$, $h=1$: $\Lambda^1 = V^*$, so $\dim = 3$
(the three generators $J^{a*}_{+1}$).
For $n=2$, $h=2$: $\dim \Lambda^2(\bC^3)_{(1,1)} = \binom{3}{2} = 3$
(wedge products $J^{a*}_{+1} \wedge J^{b*}_{+1}$, $a < b$).
These match the known bar cohomology dimensions at the
associated-graded level, confirming the computation.
\end{proof}
...
Then the augmented bar complex $\bar{B}^{\mathrm{ch}}(\cA_1)$ has
cohomology concentrated in degree~$0$:
\[
H^i(\bar{B}^{\mathrm{ch}}(\cA_1)) = 0
\quad\text{for } i \neq 0,
\qquad
H^0(\bar{B}^{\mathrm{ch}}(\cA_1)) \cong \cA_2^!.
\]
In particular, the natural map
$\bar{B}^{\mathrm{ch}}(\cA_1) \to \cA_2^!$ is a
quasi-isomorphism of chiral coalgebras.
\end{theorem}
...
$\dim H^n = \dim (\cA_2^!)_n$.
The coalgebra structure on $H^0$ is identified with
$\cA_2^!$ by the acyclicity of the twisted tensor products
...
$H^0(\bar{B}^{\mathrm{ch}}(\cA_1)) \to \cA_2^!$
that is an isomorphism because $K_\tau$ is acyclic
(cf.\ \cite[Theorem~2.3.1]{LV12}).
\end{proof}
...
dual~$\cA_1^!$.
The relative/family statement follows from the compatibility of the
...
$\boldsymbol{\Omega}_X$ form an adjoint pair on the homotopy
category of factorization (co)algebras on $\operatorname{Ran}(X)$;
Verdier duality intertwines bar constructions via
...
\emph{Step~C} (S-level):
The cohomological shadow is the identification
$H^*(\bar{B}_X(\cA)) \cong \cA^!$ as graded chiral algebras,
with bar cohomology dimensions controlled by the spectral
...
Product in $\cA_1$ & Collisions in $\overline{C}_n(X)$ with residue extraction \\
Coproduct in $\cA_2^!$ & Boundary divisors $\partial \overline{C}_n(X)$ \\
...
\end{tabular}
\end{center}
Every abstract algebraic assertion becomes a computable geometric fact
...
\begin{theorem}[Yangian as quantization {\cite{Drinfeld85}}; \ClaimStatusProvedElsewhere]
...
\begin{theorem}[Affine Yangian from W-algebras {\cite{SV13}}; \ClaimStatusProvedElsewhere]\label{thm:affine-yangian-w}
...
The quadratic dual has relation space $R^\perp$ (the orthogonal complement of the RTT relations under the trace pairing on $\mathrm{End}(V)^{\otimes 2}$).  Writing the RTT relation as $\Phi_R(X) = R_{12} X - \sigma(X) R_{12}$ and its dual as $\Phi_{R^{-1}}$, direct computation shows $R^\perp = \mathrm{Im}(\Phi_{R^{-1}})$: the Koszul dual is the RTT algebra for $R^{-1}$.  For the Yang $R$-matrix, $R^{-1}(u) = (1 - \hbar P/u)^{-1}$; since $P^2 = \mathrm{id}$, the geometric series gives $R^{-1}(u) = \frac{u^2}{u^2 - \hbar^2}\,I + \frac{\hbar u}{u^2 - \hbar^2}\,P$.  At the \emph{quadratic} level (which determines the Koszul dual), the leading approximation $R^{-1}(u) \approx 1 + \hbar P/u = R(u; -\hbar)$ suffices: the higher-order corrections affect only the higher filtration, not the quadratic relations.  More precisely, the RTT relation $R_{12}T_1T_2 = T_2T_1R_{12}$ is quadratic in the generators $T_{ij}(u)$, and the Koszul dual is determined by the quadratic part of the relations (the orthogonal complement $R^\perp \subset V^{\otimes 2}$).  The matrix $R^{-1}(u)$ appears in computing $R^\perp$ via the dual RTT relation, but only the leading terms $(1 - \hbar P/u + O(\hbar^2))$ contribute to the quadratic orthogonal complement; higher-order terms in $R^{-1}$ produce relations of degree $\geq 3$ in the generators.
...
\begin{remark}[Yangian Koszulness]\label{rem:yangian-koszul}
The Yangian $Y(\mathfrak{sl}_2)$ is Koszul: the bar spectral sequence degenerates at~$E_2$ and the quadratic dual computed above is the true Koszul dual.  This is proved in Proposition~\ref{prop:yangian-koszul} via the Polishchuk--Positselski PBW criterion: the Molev PBW basis gives a flat deformation from $Y(\mathfrak{sl}_2)$ to $\mathrm{Sym}(V)$, and Koszulness transfers along flat deformations.  For general~$\mathfrak{g}$, Koszulness is expected by the same PBW argument (Molev's PBW basis exists for all $\mathfrak{g}$) but the local finiteness conditions require verification case by case.
\end{remark}

\subsection{Hopf algebra structure and bar-cobar}

\begin{theorem}[Yangian as Hopf algebra {\cite{Drinfeld85}}; \ClaimStatusProvedElsewhere]
...
\begin{theorem}[Bar construction for Hopf algebras {\cite{LV}}; \ClaimStatusProvedElsewhere]
...
$$H^*(\bar{B}(Y(\mathfrak{g}))) \cong \operatorname{Ext}_{Y(\mathfrak{g})}(\mathbb{C}, \mathbb{C})$$

For the Yangian $Y(\mathfrak{g})$, this Ext algebra is generated in degrees~$1$ and~$2$ by the classes dual to the generators $J^a_{(0)}, J^a_{(1)}$, with relations determined by the RTT presentation~\cite{MNO96}.
\end{theorem}
...
\begin{theorem}[Yangian vs.\ quantum group {\cite{Drinfeld85}}; \ClaimStatusProvedElsewhere]
...
\begin{theorem}[Kontsevich {\cite{Kon94}}; \ClaimStatusProvedElsewhere]\label{thm:kontsevich-graph-formality}
...
The bar-cobar adjunction (cf.\ \cite[§5.2.2]{HA} for the $E_1$-algebra bar construction
in the $\infty$-categorical setting) restricts to an equivalence of
$\infty$-categories:
\[
\bar{B}^{\mathrm{ch}}:
...
\begin{conjecture}[Derived bc-$\beta\gamma$ Koszul duality; \ClaimStatusConjectured]\label{conj:derived-bc-betagamma}
...
The Virasoro algebra with central charge $c$ does \emph{not} admit a Koszul dual in the standard \emph{quadratic} (uncurved) sense, because its OPE involves a quartic pole. However, it \emph{is} chiral Koszul in the PBW sense of Theorem~\ref{thm:virasoro-chiral-koszul}: the PBW filtration yields a classical Koszul associated graded, and the spectral sequence degenerates at~$E_1$. A \emph{completed curved} Koszul dual can be constructed using the techniques of \S\ref{sec:i-adic-completion}.
