...
Chiral Koszul pairs exist because configuration space residues respect
operadic quadratic structure: the generators and relations of~$\cA$
determine~$\cA^!$ via the quadratic dual, and this pairing controls
every subsequent theorem.
\end{remark}
...
\begin{theorem}[Affine Yangian from \texorpdfstring{$\mathcal{W}$}{W}-algebras {\cite{SV13}}; \ClaimStatusProvedElsewhere]\label{thm:affine-yangian-w}
...
\begin{theorem}[Bar construction for Hopf algebras {\cite{LV12}}; \ClaimStatusProvedElsewhere]\label{thm:bar-hopf-algebras}
...
\begin{example}[Bar complex for \texorpdfstring{$Y(\mathfrak{sl}_2)$}{Y(sl2)}]
