...
The operadic foundations go back to Priddy and Quillen.
...
\begin{proof}
The Koszul complex acyclicity
(Theorems~\ref{thm:km-chiral-koszul},
\ref{thm:virasoro-chiral-koszul}) implies the
quasi-isomorphism $\barBgeom(\cA) \simeq \cA^!$
of graded coalgebras
(Definition~\ref{def:chiral-koszul-pair}, Version~I).
The PBW spectral sequence on $\barBgeom(\cA)$ has
$E_1 \cong H^*(\barBgeom(\operatorname{gr}_F \cA))
\cong \Lambda^*(V^*)$ (exterior coalgebra on dual generators),
and collapses at $E_2$ because the $d_1$ differential
corresponds to the (vanishing) next-order OPE correction
in the associated graded, which is zero by the Koszul
complex acyclicity.  Thus
$H^n(\barBgeom(\cA)) \cong E_2^n = (\cA^!)_n$.
\end{proof}
