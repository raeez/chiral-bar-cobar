...
\begin{lemma}[Left twisted tensor product as mapping cone; \ClaimStatusConditional]
...
\begin{lemma}[Right twisted tensor product as mapping cone; \ClaimStatusConditional]
...
universal pair; \ClaimStatusConditional]
...
\begin{lemma}[Bar-side filtered comparison; \ClaimStatusConditional]
...
\begin{theorem}[Fundamental theorem of chiral twisting morphisms; \ClaimStatusConditional]
...
\begin{remark}[Koszul Reflection orientation: the twelve tests as properties of $K$]
...
Hilbert series. No free-polynomial description of the cup-product
algebra is asserted by this theorem; the Hilbert polynomial is a
dimension statement, not a freeness statement.
...
\item[\textup{(viii)}] $\mathrm{ChirHoch}^*(\cA)$ has cohomological
 ...
 + \dim Z(\cA^!)\cdot t^2,
 \]
 and no free-polynomial description of the graded-commutative
 cup-product algebra is asserted by this clause
 \textup{(}Theorem~H,
...
Massey products. This does not upgrade the graded-commutative
cup-product algebra to a free polynomial algebra; no blanket
freeness assertion is part of the equivalence core.
No converse from
...
polynomial algebra is not asserted here. The theorem records
amplitude, duality, and Hilbert growth, not freeness of the
cup-product algebra.
\end{remark}
...
polynomial Hilbert growth. It does not give a free-polynomial
cup-product algebra, and no such upgrade is routed through
...
sharper than the table records. Proposition~\ref{prop:swiss-cheese-nonformality-by-class}
...
\item
 ...
 Theorem~\ref{v1-thm:kms-fourteen-unconditional} form a finite affine
...
This presentation is independent only of the filtered-comparison
lemmas. It is not a hypothesis-independent alternative proof of
Theorem~\ref{thm:bar-cobar-isomorphism-main}: it still uses the same
Koszul-effectivity hypothesis for the \v{C}ech/co-\v{C}ech
resolutions and the same Verdier-compatible chiral Koszul-pair
package. The remaining family-specific input is that the Verdier-dual
datum must be represented by the opposite member of the chosen Koszul
pair. That verification is exactly the role isolated in
...
Remark~\ref{rem:bar-cobar-relative-theory}. The strongest
...
\begin{definition}[Intrinsic bar-dual chiral coalgebra; \ClaimStatusDefinitional]\label{def:koszul-dual-coalgebra}
...
identities and the Arnold--Orlik--Solomon relations with the ordered
bar-cobar signs of Theorem~\ref{thm:det-bar-cobar-signs}, and assume
the corelations
...
relation and the ordered bar-cobar signs of
Theorem~\ref{thm:det-bar-cobar-signs}, gives
\[
...
\item\label{hyp:poly} the transferred OPE residues preserve the
 ...
 Arnold identities with the ordered bar-cobar signs of
 Theorem~\ref{thm:det-bar-cobar-signs},
...
counts the strong generators of the bar-dual coalgebra and is
...
\]
with the sign fixed by the ordered desuspension convention of
Definition~\ref{def:suspension}. Therefore multiplication in
