...
\smallskip
\noindent\textbf{Conjectural equivalences:}
\begin{hypothesis}[Accessible equivalences]
\label{hyp:koszul-accessible-equivalences}
Each of the following is equivalent to chiral Koszulness.  The
precise gap preventing proof is stated for each.
\begin{enumerate}
\item[\textup{(iv)}] Ext diagonal vanishing:
  $\operatorname{Ext}^{p,q}_\cA(\omega_X, \omega_X) = 0$ for
  $p \neq q$
  \textup{(}Conjecture~\textup{\ref{conj:ext-diagonal-vanishing})}.
  \emph{Gap}: the bigraded $t$-structure on the derived category
  of chiral modules requires the Gaitsgory--Raskin factorization
  module formalism.
\item[\textup{(vi)}] The Barr--Beck--Lurie comparison for the
  bar-cobar adjunction is an equivalence on the fiber over
  $\barBgeom(\cA)$.
  \emph{Gap}: monadicity verification for the bar-cobar monad
  on factorization coalgebras requires conservativity, which is
  open beyond genus~$0$.
\item[\textup{(vii)}] Factorization homology
  $\int_X \cA$ is concentrated in degree~$0$.
  \emph{Gap}: the comparison between factorization homology and
  bar cohomology requires an Eilenberg--Moore spectral sequence
  for factorization algebras.
\item[\textup{(ix)}] The Kac--Shapovalov determinant
  $\det G_h \neq 0$ in the bar-relevant range
  \textup{(}Conjecture~\textup{\ref{conj:kac-shapovalov-koszulness})}.
  \emph{Gap}: identification of the Shapovalov degeneracy locus
  with the $E_2$-collapse failure locus requires a bar-level
  Jantzen filtration.
\end{enumerate}
\end{hypothesis}
\end{theorem}
...
This is the converse direction of Theorem~H.
\end{proof}
