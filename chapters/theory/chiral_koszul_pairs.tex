...
\label{chap:koszul-pairs}\label{ch:chiral-koszul-pairs}
...
The bar--cobar adjunction of Theorem~A has unit and counit
\[
\eta_\cC\colon\cC\longrightarrow B_X\Omega_X(\cC),
\qquad
\varepsilon_\cA\colon\Omega_XB_X(\cA)\longrightarrow\cA.
\]
Its enhanced associative Ran form is an equivalence; the geometric
factorization realization uses the comparison and completion packages
of this chapter. Verdier duality defines the contravariant algebra
\[
K_X(\cA):=\mathbb D_{\Ran}^{\mathrm{cont}}B_X^{\mathrm{cont}}(\cA).
\]
The natural comparison of Theorem~\ref{thm:bar-cobar-verdier} is
$\Omega_X^{\mathrm{geom,cont}}(\mathbb D_X\cA)\xrightarrow{\sim}
K_X(\cA)$; the cobar functor acts here on the coalgebra
$\mathbb D_X\cA$. Lagrangian
complementarity
...
Thus $\Omega_XB_X(\cA)\to\cA$ is reconstruction, while
$K_X(\cA)=\mathbb D_{\Ran}B_X(\cA)$ is the post-Verdier algebra.
...
tests and consequences around the quadratic twisting datum
$(\cA,\cA^{\mathrm i},\tau_{\mathrm i})$. Seven form the intrinsic
bidirectional core under the stated PBW detection hypotheses:
...
Barr--Beck--Lurie comparison is the monadic consequence of the
quadratic comparison maps under its conservativity and totalization
package. The factorization-homology item is the external integration
avatar supplied by the conditional comparison model of
Proposition~\ref{prop:bar-fh}; its converse uses the stated detection
package. One item is the family-indexed
Hochschild implication
$H_H(\cA;S)\Rightarrow
\operatorname{Supp}\ChirHoch^\bullet(\cA)\subseteq S$; a perfect
degree-$d$ pairing supplies its separate duality statement, and,
because De Sole--Kac variational Poisson cohomology is nonvanishing
in unboundedly many degrees for Virasoro-type Poisson vertex algebras
\cite{BDSK21}, the implication lives strictly on the stated
Koszul/bounded locus, with the bounded-to-chart comparison part of
$H_H(\cA;S)$. One (the Lagrangian eigenspace
criterion) is conditional on the shifted-symplectic hypothesis
package, including perfectness of the bar-cobar normal complex.
One ($\mathcal{D}$-module purity) is a
...
For a chosen quadratic presentation $A=T(V)/(R)$, the presentation
coalgebra is $A^{\mathrm i}=C(s^{-1}V,s^{-2}R)$ and the canonical map
is $q_A\colon A^{\mathrm i}\to\barB(A)$.  Under $H_{\mathrm{CL}}$,
$H^\bullet(q_A)$ identifies its cohomology with bar cohomology.  The
full bar object has the dual algebra
$K(A)=\mathbb D\barB(A)$, and a chosen comparison
$\nu_A\colon K(A)\to A^!$ selects the Koszul partner.
...
and the Heisenberg quadratic presentation supplies
$q_{\mathcal H_k}\colon\mathcal H_k^{\mathrm i}\to
\barB_X(\mathcal H_k)$.  The Heisenberg instance of
$H_{\mathrm{CL}}$ makes $H^\bullet(q_{\mathcal H_k})$ an
isomorphism. On the finite-type Verdier lane the algebra
$K_X(\mathcal H_k)=\mathbb D_{\operatorname{Ran}}
\barB_X(\mathcal H_k)$ carries the chosen pair equivalence to
$\mathcal{H}_k^!$, where
$\mathcal{H}_k^!
\simeq(\mathrm{Sym}^{\mathrm{ch}}(V^*[1]),m_0=-k\omega)$
for \(k\ne0\); the uncurved symmetric algebra is the associated-graded
or \(k=0\) shadow.
The bar-cobar counit
$\Omega_X(\bar{B}_X(\mathcal{H}_k)) \xrightarrow{\;\sim\;} \mathcal{H}_k$
is the universal reconstruction equivalence. Two structural features make this work:
...
\begin{definition}[Chiral twisting datum;
\ClaimStatusDefinitional]\label{def:chiral-twisting-datum}
...
\ClaimStatusDefinitional]\label{def:chiral-koszul-morphism}
\index{Koszul morphism!chiral|textbf}
\textup{[Type signature: Open quadrant; general chiral twisting
presentation; Beilinson levels~$1\leftrightarrow2$; fixed completed
twisted products.]}
A chiral twisting datum $(\cA,\cC,\tau,F_\bullet)$ is
\emph{Koszul} when the two augmented twisted products
\[
 K_\tau^L(\cA,\cC),\qquad K_\tau^R(\cC,\cA)
\]
are acyclic.  The morphism~$\tau$ is then called a
\emph{chiral Koszul morphism}.  This is the chiral form of the
ordinary definition in Loday--Vallette~\cite[\S2.3]{LV12}; their
fundamental theorem identifies the two acyclicity conditions with the
bar and cobar comparison maps~\cite[Theorem~2.3.2]{LV12}.

Quadratic compression is an additional specialization.  If~$\cA$ is
equipped with quadratic data and
\[
 \cA^{\mathrm i}=C(s^{-1}V,s^{-2}R),\qquad
 \tau_{\mathrm i}\colon\cA^{\mathrm i}\longrightarrow\cA,
\]
then the induced coalgebra map
\[
 q_\cA\colon\cA^{\mathrm i}\longrightarrow\bar B_X(\cA)
\]
is the quadratic comparison.  A filtration whose associated graded is
this quadratic datum, together with strong convergence, is a
\emph{quadratic recognition package} for~$\tau_{\mathrm i}$; it is governed
over a point by Loday--Vallette~\cite[Theorem~3.4.6]{LV12}.
\end{definition}

\begin{remark}[Resolution and quadratic recognition]
\label{rem:construction-vs-resolution}
For an augmented dg algebra over a field, the canonical counit
\[
 \Omega B(A)\longrightarrow A
\]
is a quasi-isomorphism for every \(A\); dually, the unit
\(C\to B\Omega C\) is a quasi-isomorphism for every conilpotent dg
coalgebra \(C\)
\cite[Corollary~2.3.4]{LodayVallette12}.  Quadratic Koszulness governs
a second comparison: the compression of the full bar coalgebra to the
quadratic coalgebra \(A^{\mathrm i}\), equivalently the map
\(\Omega A^{\mathrm i}\to A\)
\cite[Theorem~3.4.6]{LodayVallette12}.  The Ran-space construction
inherits this distinction.  Francis--Gaitsgory supply the ambient
pro-nilpotent bar--cobar equivalence; the ordered
Fulton--MacPherson realization and its factorization descent require
the comparison package below.
\end{remark}

\begin{definition}[Chiral Comparison-Lemma package;
\ClaimStatusDefinitional]
\label{def:chiral-comparison-lemma-package}
For a twisting datum \((\cA,\cC,\tau,F_\bullet)\), write
\(H_{\mathrm{CL}}(\cA,\cC,\tau)\) for the following data.
\begin{enumerate}[label=\textup{(\roman*)},leftmargin=*]
\item a stable presentable Ran category carrying continuous bar,
 cobar, and two-sided twisted-product functors, together with complete
 augmented-algebra and conilpotent-coalgebra subcategories preserved by
 those functors;
\item exhaustive, separated, complete filtrations on
 \(\cA,\cC,\Omega_X\cC,\bar B_X\cA,K_\tau^L,K_\tau^R\), split and
 finite in every conformal-weight window, with strict filtered unit,
 counit, and twisting maps;
\item stratumwise identifications of the associated-graded objects
 with connected weight-graded ordinary dg algebra, dg coalgebra, bar,
 cobar, and twisted-product models;
\item compatibility of those identifications with signed collision
 residues, nested-set boundary maps, factorization gluing, and the
 chosen ordered-to-symmetric descent;
\item a Mittag--Leffler inverse system in every bidegree and strong
 convergence of the six associated spectral sequences.
\end{enumerate}
The geometric content lies in items~\textup{(iii)}--\textup{(iv)}:
they constitute the chiral Comparison Lemma.  Items~\textup{(ii)}
and~\textup{(v)} transport its associated-graded conclusion to the
completed Ran objects.
\end{definition}

\begin{theorem}[Fundamental theorem of ordinary twisting morphisms;
\ClaimStatusProvedElsewhere]
\label{thm:ordinary-fundamental-twisting-morphisms}
Let \(A\) be a connected weight-graded augmented dg algebra, let \(C\)
be a connected weight-graded conilpotent dg coalgebra, and let
\(\tau\colon C\to A\) be a twisting morphism.  The following
conditions are equivalent:
\begin{enumerate}[label=\textup{(\roman*)}]
\item the left twisted product \(A\otimes_\tau C\) is acyclic;
\item the right twisted product \(C\otimes_\tau A\) is acyclic;
\item the induced algebra map \(\Omega C\to A\) is a
 quasi-isomorphism;
\item the induced coalgebra map \(C\to BA\) is a
 quasi-isomorphism.
\end{enumerate}
This is Loday--Vallette's fundamental theorem
\cite[Theorem~2.3.2]{LodayVallette12}.
\end{theorem}

\begin{lemma}[Counit comparison; \ClaimStatusConditional]
\label{lem:twisted-product-cone-counit}
Assume \(H_{\mathrm{CL}}(\cA,\cC,\tau)\).  Then
\[
 K_\tau^L(\cA,\cC)\text{ is acyclic}
 \quad\Longleftrightarrow\quad
 \varepsilon_\tau\colon\Omega_X(\cC)\xrightarrow{\sim}\cA
 \text{ is a quasi-isomorphism}.
\]
\end{lemma}

\begin{proof}
On every finite conformal-weight window, the associated-graded
comparison identifies the two conditions with clauses
\textup{(i)} and~\textup{(iii)} of
Theorem~\ref{thm:ordinary-fundamental-twisting-morphisms}.  Strictness
and the splitting reflect quasi-isomorphisms on the associated graded.
Strong convergence lifts the equivalence to each finite window, and
the Mittag--Leffler passage lifts it to the completion.
\end{proof}

\begin{lemma}[Unit comparison; \ClaimStatusConditional]
\label{lem:twisted-product-cone-unit}
Assume \(H_{\mathrm{CL}}(\cA,\cC,\tau)\).  Then
\[
 K_\tau^R(\cC,\cA)\text{ is acyclic}
 \quad\Longleftrightarrow\quad
 \eta_\tau\colon\cC\xrightarrow{\sim}\bar B_X(\cA)
 \text{ is a weak equivalence}.
\]
\end{lemma}

\begin{proof}
The associated-graded statement is the equivalence of clauses
\textup{(ii)} and~\textup{(iv)} in
Theorem~\ref{thm:ordinary-fundamental-twisting-morphisms}.  The split
finite-window filtration, strictness, strong convergence, and the
Mittag--Leffler inverse limit give the Ran-space assertion.
\end{proof}

\begin{proposition}[Canonical one-sided bar resolution;
\ClaimStatusConditional]
\label{prop:canonical-contracting-homotopy}
For an augmented dg algebra \(A\) over a field, the canonical twisting
morphism \(\pi\colon B(A)\to A\) makes
\(A\otimes_\pi B(A)\) the normalized one-sided bar resolution of the
unit.  Its augmentation is a quasi-isomorphism and its positive
homology vanishes.  For a chiral algebra \(\cA\) satisfying
\(H_{\mathrm{CL}}(\cA,\bar B_X\cA,\pi)\), the corresponding completed
one-sided Ran object has the same cohomology.
\end{proposition}

\begin{proof}
The point-model assertion is the canonical case of
Theorem~\ref{thm:ordinary-fundamental-twisting-morphisms}; the
normalized simplicial bar object carries the standard extra
degeneracy.  The chiral assertion follows from
Lemma~\ref{lem:twisted-product-cone-counit} and the convergent
finite-window comparison in
\(H_{\mathrm{CL}}(\cA,\bar B_X\cA,\pi)\).
\end{proof}

\begin{remark}[Scope of the canonical resolution]
\label{rem:canonical-homotopy-scope}
The extra degeneracy belongs to the canonical one-sided bar
resolution over a point.  Its Ran-space lift consists of
factorization-compatible degeneracy maps commuting with the signed
residue differential and with completion.  These maps are part of
\(H_{\mathrm{CL}}\).  A general twisting morphism is governed by the
Comparison Lemma through its induced unit and counit.
\end{remark}

\begin{lemma}[Filtered counit comparison; \ClaimStatusConditional]
\label{lem:filtered-comparison}
Assume \(H_{\mathrm{CL}}(\cA,\cC,\tau)\).  If
\(\varepsilon_\tau\colon\Omega_X\cC\to\cA\) is a
quasi-isomorphism, then the associated-graded twisting morphism is
Koszul and both completed twisted products are acyclic.
\end{lemma}

\begin{proof}
Strictness and the split finite-window filtration carry the counit
quasi-isomorphism to the associated graded.  Loday--Vallette
Theorem~\ref{thm:ordinary-fundamental-twisting-morphisms} makes the
associated-graded twisting morphism Koszul and makes both
associated-graded twisted products acyclic.  Strong convergence and
Mittag--Leffler completion give the asserted acyclicity.
\end{proof}

\begin{lemma}[Filtered unit comparison; \ClaimStatusConditional]
\label{lem:filtered-comparison-unit}
Assume \(H_{\mathrm{CL}}(\cA,\cC,\tau)\).  If
\(\eta_\tau\colon\cC\to\bar B_X\cA\) is a weak equivalence, then the
associated-graded twisting morphism is Koszul and both completed
twisted products are acyclic.
\end{lemma}

\begin{proof}
The split strict filtration carries the unit equivalence to the
associated graded.  Clauses~\textup{(ii)} and~\textup{(iv)} of
Theorem~\ref{thm:ordinary-fundamental-twisting-morphisms} yield the
ordinary Koszul datum and acyclicity of both twisted products.
Convergence and the Mittag--Leffler passage lift the conclusion.
\end{proof}

\begin{theorem}[Fundamental theorem of chiral twisting morphisms;
\ClaimStatusConditional]
\label{thm:fundamental-twisting-morphisms}
\index{twisting morphism!fundamental theorem|textbf}
\textup{[Type signature: Open quadrant; completed Ran/factorization
presentation; Beilinson levels \(1\leftrightarrow2\); hypothesis
package \(H_{\mathrm{CL}}(\cA,\cC,\tau)\).]}

Let \((\cA,\cC,\tau,F_\bullet)\) be a chiral twisting datum satisfying
\(H_{\mathrm{CL}}(\cA,\cC,\tau)\).  The following are equivalent:
\begin{enumerate}[label=\textup{(\roman*)}]
\item\label{ftm:koszul}
 \(\tau\) is a chiral Koszul morphism;
\item\label{ftm:counit}
 \(p_\tau:=\varepsilon_\tau\colon\Omega_X(\cC)\to\cA\) is a
 quasi-isomorphism;
\item\label{ftm:unit}
 \(q_\tau:=\eta_\tau\colon\cC\to\bar B_X(\cA)\) is a weak
 equivalence;
\item\label{ftm:acyclic}
 \(K_\tau^L(\cA,\cC)\) and \(K_\tau^R(\cC,\cA)\) are acyclic.
\end{enumerate}
This is the completed chiral transport of the ordinary four-way
equivalence of Loday--Vallette~\cite[Theorem~2.3.2]{LV12}.

For \(\cC=\cA^{\mathrm i}\) and \(\tau=\tau_{\mathrm i}\), the maps are
the quadratic compression
\[
 q_\cA\colon\cA^{\mathrm i}\longrightarrow\bar B_X(\cA),
 \qquad
 p_\cA\colon\Omega_X(\cA^{\mathrm i})\longrightarrow\cA,
\]
and the theorem specializes to the Koszul criterion of
Loday--Vallette~\cite[Theorem~3.4.6]{LV12}.  For
\(\cC=\bar B_X(\cA)\) and the universal twisting morphism
\(\pi_\cA\), the induced coalgebra map is the identity of
\(\bar B_X(\cA)\); the corresponding map
\(\Omega_X\bar B_X(\cA)\to\cA\) is the universal reconstruction
counit, modeled over a point by
Loday--Vallette~\cite[Corollary~2.3.4]{LV12}.
\end{theorem}

\begin{proof}
Theorem~\ref{thm:ordinary-fundamental-twisting-morphisms} gives the
four-way equivalence on every associated-graded finite window.
The package~$H_{\mathrm{CL}}$ supplies the strict split filtrations,
finite-window comparison, strong convergence, and completion.  Lemmas
\ref{lem:twisted-product-cone-counit},
\ref{lem:twisted-product-cone-unit},
\ref{lem:filtered-comparison}, and
\ref{lem:filtered-comparison-unit} transport the four conditions
between the associated graded, the finite windows, and their completed
inverse limit.
\end{proof}
...
Let $\cC$ be a conilpotent complete factorization coalgebra and let
$\cA$ be an augmented chiral algebra on a smooth curve~$X$.  Assume
that the chosen Ran category represents continuous free and cofree
constructions and that their universal properties commute with
factorization gluing and completion.  Then
...
\begin{proof}
Loday--Vallette's ordinary adjunction gives the three bijections over
a point \cite[Theorem~2.2.9]{LodayVallette12}.  The assumed continuous
free and cofree universal properties apply the same construction on
each Ran stratum.  Compatibility with factorization gluing assembles
the stratumwise maps, and compatibility with completion gives the
displayed continuous bijections.
\end{proof}

\begin{remark}[Algebraic Maurer--Cartan equation and geometric realization]
\label{rem:MC-is-Stokes}
\index{Maurer--Cartan equation!geometric realization}
The equation
\[
 d\tau+\tau\star\tau=0
\]
is the algebraic condition that makes the twisted differential square
to zero.  Under the local collision package
\(H_{\mathrm{loc}}\), signed residue maps realize the two summands as
the internal differential and the codimension-one collision
differential.  In arity three, the nested-set
boundary-of-boundary identity, the Arnold relation, and the
Jacobi--Borcherds identity give the corresponding square-zero
calculation.  A real oriented-blow-up model expresses the same
calculation by Stokes' formula; an algebraic logarithmic model
expresses it by iterated residues.

At positive genus, a cyclic trace and stable-graph sewing define the
curved modular equation
\[
 d\tau_{\mathrm{mod}}
 +\tau_{\mathrm{mod}}\star\tau_{\mathrm{mod}}
 =m_0 .
\]
The curvature \(m_0\) is part of the named modular package and its
scalar trace is a subsequent construction.
\end{remark}
\begin{remark}[Theorem~A reconstruction and Koszul compression]
\label{rem:theorem-A-decomposition}
\index{Theorem A!decomposition}
Three choices of coalgebra organize the theorem surface:
\begin{itemize}
\item \emph{General twisting comparison.}
 For a specified~$\cC$ and~$\tau\colon\cC\to\cA$,
 Theorem~\ref{thm:fundamental-twisting-morphisms} identifies
 twisted-product acyclicity with the two maps
 $q_\tau\colon\cC\to\bar B_X(\cA)$ and
 $p_\tau\colon\Omega_X\cC\to\cA$ under~$H_{\mathrm{CL}}$.
 This is the chiral form of Loday--Vallette
 \cite[Theorem~2.3.2]{LV12}.
\item \emph{Quadratic compression.}
 For~$\cC=\cA^{\mathrm i}=C(s^{-1}V,s^{-2}R)$ and
 $\tau=\tau_{\mathrm i}$, the load-bearing map is
 $q_\cA\colon\cA^{\mathrm i}\to\bar B_X(\cA)$.
 Its equivalence is the quadratic Koszul criterion
 \cite[Theorem~3.4.6]{LV12}; bar concentration identifies the image
 with the Koszul diagonal.
\item \emph{Universal reconstruction and Verdier duality.}
 For~$\cC=\bar B_X(\cA)$, the comparison~$q_{\pi_\cA}$ is the
 identity and the counit
 $\Omega_X\bar B_X(\cA)\to\cA$ is the full-bar reconstruction
 equivalence of Theorem~A, modeled by
 \cite[Corollary~2.3.4]{LV12}.  Verdier duality then carries the bar
 coalgebra to the post-Verdier algebra on its separately stated
 dualizable domain.
\end{itemize}
The general comparison contains the quadratic criterion as the choice
$\cC=\cA^{\mathrm i}$ and contains universal reconstruction as the
choice~$\cC=\bar B_X(\cA)$.  These choices have distinct mathematical
content because the first compares a compressed coalgebra with the full
bar object, while the second begins with the full bar object itself.
\end{remark}

\begin{proposition}[Universal property of the completed Ran bar;
\ClaimStatusConditional]
\label{prop:bar-universal-property}
\index{bar construction!universal property}
Assume the continuous free/cofree and factorization-gluing clauses of
$H_{\mathrm{CL}}$.  For any augmented chiral algebra~$\cA$, the bar construction
...
\begin{proof}
The ordinary universal property is
\cite[Theorem~2.2.9]{LodayVallette12}.  In the chosen Ran category,
...
\begin{theorem}[Comparison lemma for chiral twisted products;
\ClaimStatusConditional]
...
and let $f \colon C \to C$ be a strict filtered coalgebra
quasi-isomorphism such that $\alpha = \beta \circ f$.  Assume the two
twisting data satisfy $H_{\mathrm{CL}}$ and that
$\operatorname{gr}f$ is a coalgebra quasi-isomorphism on every finite
weight window. Then the induced map on twisted tensor
products
\[
...
On associated-graded finite windows this is the ordinary Comparison
Lemma \cite[Lemma~2.1.9]{LodayVallette12}.  The strict split
filtrations and strong convergence in $H_{\mathrm{CL}}$ lift that
quasi-isomorphism window by window.  The Mittag--Leffler passage gives
the completed chiral comparison.
\end{theorem}

\subsection{Chiral Koszul pairs}

The Koszul-pair notion extends beyond the quadratic setting through a
set of \emph{recognition criteria} on the pair.  The full bar--cobar
identification follows from those criteria through
Theorem~\ref{thm:fundamental-twisting-morphisms}.

\begin{definition}[Chiral Koszul pair; \ClaimStatusDefinitional]\label{def:chiral-koszul-pair}
...
\]
in one fixed completed factorization category: the coalgebras~$\cC_i$ are complete
...
equivalent continuous finite-window algebraic model. These
topological and finite-type hypotheses promote the twisting data to a
Verdier--Koszul pair. The pair is equipped with equivalences of
post-Verdier factorization algebras
\[
\theta_{12}\colon
\mathbb{D}_{\operatorname{Ran}}^{\mathrm{cont}}(\cC_1)
\xrightarrow{\sim}\cA_2,
\qquad
\theta_{21}\colon
\mathbb{D}_{\operatorname{Ran}}^{\mathrm{cont}}(\cC_2)
\xrightarrow{\sim}\cA_1,
\]
compatible with the twisting morphisms and filtrations.
In this situation we write $\cA_2 \simeq \cA_1^!$ and
$\cA_1 \simeq \cA_2^!$.
The coalgebra~$\cC_i$ maps to the full bar coalgebra by the unit
$\eta_i\colon\cC_i\to B_X(\cA_i)$. Cobar reconstruction gives
$\Omega_X(\cC_i)\to\cA_i$, while Verdier duality gives the algebra
$\mathbb D_{\operatorname{Ran}}\cC_i$. The maps~$\theta_{12}$ and
$\theta_{21}$ identify this algebra with the opposite family member.
...
Theorem~\ref{thm:bar-cobar-isomorphism-main} composes them with the
unit and counit maps.

\smallskip\noindent
\emph{Standard construction.}
For a concrete family one constructs $\cC_i$, the twisting morphisms,
the convergent PBW filtrations, and the two maps~$\theta_{ij}$.
Theorem~\ref{thm:bar-cobar-verdier} gives the natural algebra
$K_X(\cA_i)$ and its comparison with
$\Omega_X^{\mathrm{geom,cont}}(\mathbb D_X\cA_i)$. Identifying this
algebra with the opposite family member is the explicit content of
$\theta_{ij}$.
The Heisenberg pair
\[
\bigl(\mathcal{H}_k,
(\mathrm{Sym}^{\mathrm{ch}}(V^*[1]),m_0=-k\omega)\bigr)
\]
is the prototype in the curved second-kind category
(\S\ref{sec:frame-koszul-dual}); its uncurved symmetric algebra is
only the $k=0$ or associated-graded shadow.
...
\begin{remark}[Five typed objects]\label{rem:five-object-firewall-kp}
This is the canonical Chiral Koszul Pairs specialization of
Convention~\ref{conv:master-five-object-firewall} and
Theorem~\ref{thm:typed-verdier-koszul-firewall}.
An augmented chiral algebra~$\cA$ determines five typed objects:
\[
\cA,\qquad
\barB_X(\cA),\qquad
\cA^{\mathrm i},\qquad
K_X(\cA)=\mathbb D_{\operatorname{Ran}}\barB_X(\cA),\qquad
...
Yangian, affine Yangian, and $r$-matrix constructions. A locality and
descent package supplies its map to the symmetric bar.

For a quadratic presentation $\cA=T_X(V)/(R)$, the symbol
$\cA^{\mathrm i}=C_X(s^{-1}V,s^{-2}R)$ denotes its quadratic coalgebra. Its
finite Verdier dual is the strict quadratic algebra
$\cA^!_{\mathrm{quad}}:=\mathbb D_{\operatorname{Ran}}\cA^{\mathrm i}$.
The algebra $K_X(\cA)$ is the Verdier dual of the full bar coalgebra.
The derived centre is the Hochschild cochain algebra
...
\]
\[
\mathbb D(q_\cA)\colon K_X(\cA)\longrightarrow\cA^!_{\mathrm{quad}},
\qquad
\varepsilon_\cA\colon\Omega_X\barB_X(\cA)\longrightarrow\cA.
\]
The first map is quadratic recognition, the second is its
contravariant Verdier transpose, and the third is universal
reconstruction.
\end{remark}
...
and the dual has commutative associated graded
$\mathrm{Sym}^{\mathrm{ch}}(V^*)$ but is object-level curved for
$k\ne0$: the chiral incarnation of classical
$\mathrm{Lie}$--$\mathrm{Com}$ duality occurs inside the
second-kind curved ambient.
...
coalgebra forms
$K_X(\mathcal H_k)=\mathbb D_{\operatorname{Ran}}
\barB_X(\mathcal H_k)$; the chosen pair map identifies it with
$\mathcal{H}_k^!
\simeq(\mathrm{Sym}^{\mathrm{ch}}(V^*[1]),m_0=-k\omega)$:
the bar complex $\bar{B}(\mathcal{H}_k)$ was computed degree by
degree in \S\ref{sec:frame-bar-deg1}--\S\ref{sec:frame-bar-all},
producing the coalgebra whose finite dual is the curved
commutative Koszul dual
...
\mathcal H_k^!)$ is manifestly acyclic under the stated completed
second-kind hypotheses.
\end{example}
...
The comparison begins with the quadratic coalgebra
$\mathcal{BC}^{\mathrm i}=C_X(s^{-1}V,s^{-2}R)$ and its canonical
recognition map
\[
q_{\mathcal{BC}}\colon
\mathcal{BC}^{\mathrm i}
...
\]
under the split quadratic instance of $H_{\mathrm{CL}}$.  Its
continuous Verdier transpose has source
$K_X(\mathcal{BC})$ and target
$\mathcal{BC}^!_{\mathrm{quad}}
=\mathbb D_{\operatorname{Ran}}\mathcal{BC}^{\mathrm i}$.
The chosen pair map identifies this algebra with
$\mathcal{BG}$. Thus the classical
$\Lambda^* \leftrightarrow \mathrm{Sym}^*$ exchange occurs in the
...
The residues at collision divisors define the bar differential and
its compatibility with deconcatenation. Cobar reconstructs
$\mathcal{BC}$ from the bar coalgebra on the Koszul locus.  Continuous
Verdier duality followed by the chosen pair map identifies
$K_X(\mathcal{BC})$ with~$\mathcal{BG}$.
\end{example}
...
\begin{corollary}[PBW collapse and quadratic recognition;
\ClaimStatusConditional]
...
at $E_2$ and computes the bar-cohomology coalgebra
$H^\bullet\barBgeom(\cA)$.  Suppose in addition that $\cA$ carries a
chosen quadratic presentation with
$\cA^{\mathrm i}=C_X(s^{-1}V,s^{-2}R)$ and that
$H_{\mathrm{CL}}(\cA,\cA^{\mathrm i},\tau_{\mathrm i})$ holds.  Then
\[
 H^\bullet(q_\cA)\colon
 H^\bullet(\cA^{\mathrm i})
 \xrightarrow{\sim}H^\bullet\barBgeom(\cA)
\]
identifies the two cohomology coalgebras.  On a finite-type Verdier
lane the full bar object determines
$K_X(\cA)=\mathbb D_{\operatorname{Ran}}\barB_X(\cA)$, and a chosen
pair map $\nu_\cA\colon K_X(\cA)\to\cA^!$ supplies the algebra-level
identification.  For a PBW non-quadratic family such as the universal
Virasoro family, the canonical output of the collapse is
$H^\bullet\barBgeom(\cA)$ together with $K_X(\cA)$; a presentation
coalgebra $\cA^{\mathrm i}$ enters after an additional quadratic or
curved-PBW comparison has been chosen.
\end{corollary}
...
By the associated-graded identification above, the $E_1$ page
...
commutative algebra, it is Koszul with Koszul dual
 ...
 (Priddy's theorem applied to the chiral PBW associated graded).
Therefore:
...
=: \mathsf H^{\mathrm{bar}}_n(\cA).
\]
For a chosen quadratic presentation satisfying $H_{\mathrm{CL}}$,
$H^\bullet(q_\cA)$ identifies this coalgebraic piece with the
corresponding cohomology of~$\cA^{\mathrm i}$.  Under
$H_{\mathbb D}^{\mathrm{bar}}$, its continuous dual is the
corresponding cohomology piece of $K_X(\cA)$; the pair map~$\nu_\cA$
then compares it with the selected partner~$\cA^!$.
...
\begin{theorem}[Bar concentration for Koszul pairs; \ClaimStatusConditional]
...
$\cA_1=T_X(V_1)/(R_1)$ and set
\[
\cA_1^{\mathrm i}=C_X(s^{-1}V_1,s^{-2}R_1),\qquad
q_{\cA_1}\colon\cA_1^{\mathrm i}\longrightarrow
\bar B_X^{\mathrm{ch}}(\cA_1).
\]
...
\quad\text{for } q \neq 0,
\qquad
H^{p,0}(\bar{B}^{\mathrm{ch}}(\cA_1))
\cong H^{p,0}(\cA_1^{\mathrm i}).
\]
Equivalently,
$q_{\cA_1}$ is a quasi-isomorphism of graded chiral coalgebras.
Verdier duality gives
\[
K_X(\cA_1):=\mathbb D_{\Ran}\bar B_X^{\mathrm{ch}}(\cA_1),
\qquad
\mathbb D(q_{\cA_1})\colon
K_X(\cA_1)\longrightarrow\mathbb D_{\Ran}(\cA_1^{\mathrm i}),
\]
and the chosen pair map
$\nu_{12}\colon K_X(\cA_1)\xrightarrow{\sim}\cA_2$
identifies the Verdier algebra with the partner.
\end{theorem}
...
The chiral Comparison Lemma applied to the canonical twisting
morphism of the presentation identifies the PBW comparison with the
coalgebra map~$q_{\cA_1}$.  Acyclicity of the left and right Koszul
complexes makes $q_{\cA_1}$ a quasi-isomorphism
\cite[Theorem~2.3.2]{LodayVallette12}.  Taking cohomology yields the
displayed diagonal identification.  Verdier duality gives
$\mathbb D(q_{\cA_1})$, and the pair datum supplies~$\nu_{12}$.
\end{proof}
...
the Barr--Beck--Lurie consequence of the quadratic resolution, the
...
\begin{theorem}[Koszulness and strict diagonal formality;
\ClaimStatusConditional]
\label{thm:ainfty-koszul-characterization}%

Chiral Koszulness is equivalent to the vanishing of the off-diagonal
transferred operations in the bar minimal model. Strict
$A_\infty$-formality, $m_n=0$ for all $n\geq3$, is the additional
condition that every diagonal shadow class vanishes. Thus strict
formality implies chiral Koszulness, and the converse holds precisely
on the class-$G$ surface where the diagonal shadow tower is zero.
\end{theorem}

\begin{proof}
Strict formality implies Koszulness by
Proposition~\ref{prop:ainfty-formality-implies-koszul}. Conversely,
the PBW filtration is defined fiberwise and is compatible with the FM
stratification. The HPL transfer on each stratum identifies the
off-diagonal transferred operations with the higher PBW differentials
$d_r$, $r\geq2$. Chiral Koszulness is exactly $E_2=E_\infty$, hence
these off-diagonal operations vanish. Operations preserving the Koszul
diagonal are invisible to concentration; their classes are the shadow
invariants. Requiring those classes to vanish gives strict
$A_\infty$-formality, and this is precisely class~$G$ in the shadow
classification.
\end{proof}
...
means: PBW degeneration holds at all~$k$;
the full Koszulness
\textup{(}diagonal Ext concentration\textup{)}
holds at generic~$k$
but fails at $k = -h^\vee$.
At critical level Theorem~H is governed by its own family datum
$H_H(V_{-h^\vee}(\fg);S_{\mathrm{crit}})$.  The diagonal criterion
supplies associated-graded input, while the critical chart comparison
and finite-window retract determine $S_{\mathrm{crit}}$.
Remark~\textup{\ref{rem:critical-level-lie-vs-chirhoch}} gives the
corresponding Beilinson--Drinfeld comparison.
\end{remark}
...
For $\fg = \mathfrak{sl}_2$, the finite Shapovalov and character
computations are especially sharp, but they do not by themselves close
the theorem.  In every checked finite window the first vacuum null is
single-weight, the quotient agrees with the universal algebra below
the null weight, and the finite diagnostic reports the same model
cohomology as $V_k(\mathfrak{sl}_2)$. To promote this to a theorem for
every admissible level $k = -2 + p/q$, one still has to supply the
quotient bar spectral sequence together with PBW/Shapovalov detection
of all higher differentials, finite-window exactness, and strong
convergence. Until that package is installed, the
$\mathfrak{sl}_2$ statement is finite evidence / conditional, not a
settled input to Theorem~H or bar--cobar inversion.

For $\fg=\mathfrak{sl}_3$ at denominator~$3$, the uniform truncated
Li--bar model has
$E_1^{2,3}\cong\mathfrak{sl}_3$.  The exact incoming Poisson matrix
$d_1\colon E_1^{3,4}\to E_1^{2,3}$ has rank~$6$ and root-space
image; its cokernel is the two-dimensional Cartan space.  Theorem
\ref{thm:admissible-sl3-non-koszul-qge3} promotes this finite-model
calculation to a conditional statement about the cone of
$q_A\colon A^{\mathrm i}\to\bar B(A)$ under the quotient comparison,
higher-page persistence, and strong-convergence package.

For denominators~$q>3$, the current finite engine exhibits
off-diagonal $E_1$ classes and leaves the incoming $d_1$ rank as the
next computation.  For $q\leq2$, its associated-graded diagnostic is
diagonally concentrated.  These three denominator lanes record the
present evidence with their respective statuses.
\end{remark}

\begin{theorem}[Cartan cone for quadratic compression in the
denominator-three Li--bar model of $L_k(\mathfrak{sl}_3)$]%
...
$\widehat{\mathfrak{sl}}_3$, with $q=3$, $p\geq3$, and
$(p,3)=1$,
and put $A=L_k(\mathfrak{sl}_3)$.  Let
$A^{\mathrm i}$ be the quadratic coalgebra determined by the chosen
Li/PBW quadratic presentation, let
$\tau_{\mathrm i}\colon A^{\mathrm i}\to A$ be its twisting morphism, and let
\[
q_A\colon A^{\mathrm i}\longrightarrow\bar B^{\mathrm{ch}}(A)
\]
be the induced quadratic comparison.  Assume
$H_{\mathrm{CL}}(A,A^{\mathrm i},\tau_{\mathrm i})$ and the following finite
admissible Li--bar package.
...
\item In the bar-relevant window the reduced associated graded is the
 uniform eight-generator model
 \[
 R^{\mathrm{red}} \;\cong\;
 \bigotimes_{a=1}^{8}\mathbb C[x_a]/(x_a^3),
 \]
 where two generators form a Cartan basis and six are root
 generators.
\item The $d_1$ differential on the resulting Li--bar page is the
 Kirillov--Kostant Poisson differential of
 Proposition~\ref{prop:li-bar-poisson-differential}; its exact
 finite matrix
 \[
 d_1\colon E_1^{3,4}\longrightarrow E_1^{2,3}
 \]
 has rank~$6$, its image is the six-dimensional root subspace, and
 its cokernel is the Cartan span
 $\mathbb C[H_1]\oplus\mathbb C[H_2]$.
\item The map~$q_A$ is filtered; its quadratic source has
 $H^{2,3}(A^{\mathrm i})=0$; the quotient-to-model comparison
 carries the computed Cartan cokernel to the actual Li--bar page; and
 the non-reduced extension data and higher differentials preserve
 those two classes to the abutment.
\end{enumerate}
Then the quadratic comparison has a rank-two Cartan cone.  More
precisely:
\begin{enumerate}[label=\textup{(\roman*)}]
\item $\dim H^2(\barB^{\mathrm{ch}}(L_k(\mathfrak{sl}_3)))
 \geq \mathrm{rk}(\mathfrak{sl}_3) = 2$, and
 \[
 \dim H^{2,3}\!\left(\operatorname{Cone}(q_A)\right)\geq2.
 \]
\item The nonzero cohomology of $\operatorname{Cone}(q_A)$ is a
 quadratic Koszul defect for~$\tau_{\mathrm i}$.  Equivalently, the
 mapping cone of the quadratic cobar map
 $p_A\colon\Omega_X(A^{\mathrm i})\to A$ and each of the two
 quadratic twisted products has nonzero cohomology.  This is the
 specialization of
 Loday--Vallette~\cite[Theorem~2.3.2 and Theorem~3.4.6]{LV12}.
\item In the twelve-equivalence meta-theorem
 \textup{(}Theorem~\ref{thm:koszul-equivalences-meta}\textup{)},
 the quadratic-compression conditions occupy the obstructed branch.
 The universal full-bar counit
 $\Omega_X\bar B_X(A)\xrightarrow{\sim}A$ retains its independent
 reconstruction status from
 Loday--Vallette~\cite[Corollary~2.3.4]{LV12} and Theorem~A.
\end{enumerate}
For denominators~$q>3$, the finite engine presently computes
off-diagonal classes on the $E_1$ page.  The corresponding incoming
$d_1$ rank and higher-page persistence form the explicit extension
problem beyond this denominator-three theorem.
\end{theorem}
...
admissible control; the theorem uses them through the explicit finite
package stated above.

\emph{Step~2 \textup{(}$E_1$ page via K\"unneth on the truncated
associated graded\textup{)}.}
For one factor $\mathbb C[x]/(x^3)$, the degree-two periodic
resolution contributes a class of internal weight~$3$.
K\"unneth therefore gives
\[
 E_1^{2,3}\cong\mathfrak{sl}_3,
 \qquad \dim E_1^{2,3}=8,
\]
with the decomposition into six root coordinates and two Cartan
coordinates.

\emph{Step~3 (Exact $d_1$ matrix).}
By Hypothesis~\textup{(A3)}, the incoming Poisson differential
\[
 d_1\colon E_1^{3,4}\longrightarrow E_1^{2,3}
\]
has rank~$6$ and image equal to the root subspace.  The outgoing
target $E_1^{1,2}$ is zero.  Consequently
\[
 H(E_1,d_1)^{2,3}
 \cong \operatorname{coker}
 \bigl(d_1\colon E_1^{3,4}\to E_1^{2,3}\bigr)
 \cong \mathfrak h,
\]
and this group has dimension~$2$.  The exact rational matrix, its
rank, its root-space image, and the two Cartan cokernel classes are
precisely the data required by Hypothesis~\textup{(A3)}; this step
makes no unconditional assertion that the rank condition holds.

\emph{Step~4 (Passage from the finite model to the quotient bar).}
Hypothesis~\textup{(A4)} is the quotient comparison and persistence
statement.  It carries the computed Cartan cokernel to the actual
Li--bar spectral sequence and preserves it through the higher pages.
Strong convergence from~\textup{(A1)} therefore gives a
two-dimensional subspace of
$H^{2,3}(\bar B^{\mathrm{ch}}(A))$.

\emph{Step~5 (The quadratic comparison cone).}
The source clause of~\textup{(A4)} gives
$H^{2,3}(A^{\mathrm i})=0$.  The long exact cohomology sequence of
$\operatorname{Cone}(q_A)$ places the two Cartan classes in
$H^{2,3}(\operatorname{Cone}(q_A))$.  This proves~(i) and exhibits
the nonzero quadratic Koszul defect of~$q_A$.  Applying the general
twisting theorem to $(A,A^{\mathrm i},\tau_{\mathrm i})$ identifies the same
defect in~$p_A$ and in the two quadratic twisted products,
proving~(ii).

For the universal twisting morphism
$\pi_A\colon\bar B(A)\to A$, the induced coalgebra comparison is
$\mathrm{id}_{\bar B(A)}$; Loday--Vallette
\cite[Corollary~2.3.4]{LV12} supplies the corresponding
reconstruction counit.  Thus the calculation locates the Cartan
class in the compression~$q_A$, which gives~(iii).
\end{proof}

\begin{conjecture}[Higher-rank admissible Cartan cone, $\mathrm{rk}(\fg) \geq 3$]%
...
denominator $q \geq 3$ has a Cartan subspace of dimension
$\mathrm{rk}(\fg)$ in the cohomology of its quadratic-comparison
cone, and hence
$\dim H^2(\barB(L_k(\fg))) \geq \mathrm{rk}(\fg)$.  At rank~$2$
and denominator~$3$, Theorem~\ref{thm:admissible-sl3-non-koszul-qge3}
establishes the corresponding conditional finite-model statement.
The extension to $q>3$ begins with the incoming $d_1$ matrix, while
the higher-rank extension also includes the $d_2$ contributions from
triple-root brackets.
\end{conjecture}
...
\begin{construction}[Li--bar spectral sequence; \ClaimStatusDefinitional]
...
\begin{remark}[Finite-window KZ--Arnold residue realization;
\ClaimStatusConditional]
\label{rem:ckp-kz-arnold-residue-realization}
\index{KZ--Arnold superconnection!finite-window residue realization}
On a genus-zero affine or tangent window with infinitesimal-braid
representation, Theorem~\ref{thm:climax-genus-zero} constructs the
flat connection
\(
 \nabla^{B,0}_{\cA,V,n}
 =\mathrm{KZ}_{\cA,V,n}^{*}(\nabla_{\mathrm{Arnold}})
\)
on the coefficient bundle over the ordered configuration space.  The
finite-window KZ--Arnold bar-superconnection identity is the comparison
obtained after Fulton--MacPherson residue realisation: signed residues
along the boundary divisors assemble into the corresponding component
of~\(d_{\bar B}\).  Thus the connection and the chain differential
retain their distinct types, while the residue-realisation functor
provides the map between them.  Extension to every weight and every
collision stratum belongs to the completion and incidence-compatibility
package of the cited theorem.
\end{remark}
...
obstructions for~$L_k$ and~$L_{k'}$ are conjecturally
Barbasch--Vogan dual.
\end{remark}
...
test case where the reduced geometry supports a concentration test, but
...
\begin{remark}[Quadratic recognition, reconstruction, and Verdier duality]
\label{rem:koszul-reflection-orientation}
\index{Koszul reflection!the twelve tests as properties of K}
\index{Theorem A!unified form pulls back the twelve tests}
The twelve conditions of
Theorem~\ref{thm:koszul-equivalences-meta} organize three constructions.
The quadratic construction consists of
\[
\tau_{\mathrm i}\colon\cA^{\mathrm i}\to\cA,
\qquad
q_\cA\colon\cA^{\mathrm i}\to B_X(\cA),
\qquad
p_\cA\colon\Omega_X(\cA^{\mathrm i})\to\cA.
\]
Items~\textup{(i)--(v)} and~\textup{(ix)--(x)} concern these maps and
their PBW, Ext, Shapovalov, and Fulton--MacPherson realizations under
$H_{\mathrm{CL}}$.

The universal construction is the adjunction
\[
\Omega_X\colon\operatorname{CoAlg}^{\mathrm{conil}}_X
\rightleftarrows
\operatorname{Alg}^{\mathrm{aug}}_X:B_X,
\]
with unit $\eta_C\colon C\to B_X\Omega_XC$ and counit
$\varepsilon_A\colon\Omega_XB_XA\to A$.  The two triangular
identities express the equivalence between these two categories in
the enhanced pro-nilpotent Ran ambient.  Its factorization realization
is governed by $H_{\mathrm{fact}}$ and $H_{\mathrm{conv}}$.

The Verdier construction is the contravariant functor
\[
K_X(A)=\mathbb D_{\operatorname{Ran}}^{\mathrm{cont}}B_X^{\mathrm{cont}}(A).
\]
For a quadratic presentation, $\mathbb D(q_A)$ compares $K_X(A)$ with
$\mathbb D A^{\mathrm i}$.  Item~\textup{(vi)} is the monadic
consequence of the quadratic resolution under its conservativity and
totalization package; item~\textup{(vii)} is the conditional
factorization-homology comparison; item~\textup{(viii)} is governed by
the family-indexed Hochschild theorem and its own retract package;
item~\textup{(xi)} uses the shifted-symplectic hypotheses; and
item~\textup{(xii)} records the one-directional purity implication.
Each clause therefore retains the map and hypothesis package from
which it is derived.
\end{remark}
...
monadicity) is the monadic consequence of item~\textup{(v)} under
the stated conservativity and totalization package for the quadratic
resolution; it is listed as the $\infty$-categorical formulation most
...
extra detection hypothesis stated in the theorem.  It therefore
occupies the conditional-comparison lane.
Item~\textup{(viii)} is the family-indexed implication supplied by
$H_H(\cA;S)$; its Koszul-dual reflection uses the additional perfect
degree-$d$ pairing. Item~\textup{(xi)} (the Lagrangian criterion)
is conditional on the shifted-symplectic hypothesis package, including
perfectness of the bar-cobar normal complex, non-degeneracy of the
ambient tangent complex, cyclic/BV compatibility, and bar-chart
acyclicity. On the standard landscape
Proposition~\ref{prop:lagrangian-perfectness} discharges the
perfectness input; the remaining package inputs stay part of the
statement. Item~\textup{(xii)}
...
with PBW filtration $F_\bullet$.  Fix a quadratic coalgebra and its
twisting morphism
\[
 \cA^{\mathrm i}=C(s^{-1}V,s^{-2}R),\qquad
 \tau_{\mathrm i}\colon\cA^{\mathrm i}\longrightarrow\cA,
\]
and assume the comparison package
$H_{\mathrm{CL}}(\cA,\cA^{\mathrm i},\tau_{\mathrm i})$ together with the
PBW detection hypotheses stated below.  Write
\[
 q_\cA\colon\cA^{\mathrm i}\longrightarrow\bar B_X(\cA),
 \qquad
 p_\cA\colon\Omega_X(\cA^{\mathrm i})\longrightarrow\cA
\]
for the two maps induced by~$\tau_{\mathrm i}$.  The intrinsic equivalence
core is read under the split finite-window PBW hypotheses: conformal-weight/bar-length
...
$E_\infty$ with its associated graded.  Each subsequent implication
is read with the convergence or comparison package stated beside it.
...
strengthening in which the off-diagonal PBW subquotient injects into
the corresponding Goodwillie subquotient; equivalently, degree-zero concentration of
$\int_\Sigma\cA$ reflects degree-zero concentration of
$\barB^{\mathrm{geom}}_\Sigma(\cA)$.
These comparison and detection packages are explicit hypotheses.
Conditions \textup{(i)--(v)} and \textup{(ix)--(x)} below are seven
coordinated diagnostics for the quadratic comparison~$q_\cA$ under the
stated PBW hypotheses.  Their equivalence uses the general twisting theorem
\cite[Theorem~2.3.2]{LV12} and its quadratic specialization
\cite[Theorem~3.4.6]{LV12}.  Condition~\textup{(vi)}
(Barr--Beck--Lurie comparison) is the corresponding monadic
consequence under its conservativity and totalization package.
Condition~\textup{(vii)} is a conditional factorization-homology
consequence under $\mathsf{FH}_{\mathbb{P}^1}(\cA)$; it becomes a
genus-$0$ criterion precisely under the stronger
$\mathsf{FH}^{\mathrm{det}}_{\mathbb{P}^1}(\cA)$.
Under the uniform-weight hypothesis, condition~\textup{(vii)} strengthens to
the all-genera concentration statement on the curves carrying
$\mathsf{FH}_{\Sigma_g}(\cA)$ for the relevant curves.
Condition~\textup{(viii)} is the family-indexed Hochschild implication
under $H_H(\cA;S)$.  It yields support in~$S$ and the corresponding
polynomial Hilbert series.  A perfect degree-$d$ pairing with a
companion family datum supplies duality.  The multiplicative structure
of the cup-product algebra belongs to its separate comparison theorem;
the present Hilbert polynomial records graded dimensions.
...
\begin{enumerate}
\item[\textup{(i)}] $\tau_{\mathrm i}\colon\cA^{\mathrm i}\to\cA$ is a
...
\item[\textup{(iii)}] The off-diagonal part of the minimal
 $A_\infty$-model of $\barBgeom(\cA)$ is formal: the transferred
 operations whose outputs lie off the Koszul diagonal vanish
...
\item[\textup{(iv)}] Ext diagonal vanishing:
 ...
 $p \neq q$.
\item[\textup{(v)}] The quadratic comparison maps are equivalences:
 \[
 q_\cA\colon\cA^{\mathrm i}\xrightarrow{\sim}\bar B_X(\cA),
 \qquad
 p_\cA\colon\Omega_X(\cA^{\mathrm i})
 \xrightarrow{\sim}\cA.
 \]
 This is the chiral quadratic criterion modeled by
 Loday--Vallette~\cite[Theorem~3.4.6]{LV12}.
\item[\textup{(vi)}] Under the Barr--Beck--Lurie package for the
 quadratic resolution~$p_\cA$, the associated comparison functor is
 an equivalence on the complete component generated by~$\cA$.
...
The canonical full-bar twisting morphism
$\pi_\cA\colon\bar B_X(\cA)\to\cA$ belongs to universal
reconstruction.  Its induced coalgebra comparison is
$\mathrm{id}_{\bar B_X(\cA)}$, and its counit
\[
 \Omega_X\bar B_X(\cA)\xrightarrow{\sim}\cA
\]
is the full bar--cobar resolution of Theorem~A, modeled by
Loday--Vallette~\cite[Corollary~2.3.4]{LV12}.  Condition~\textup{(v)}
measures the additional compression from~$\cA^{\mathrm i}$ to this
full bar coalgebra.
...
\item[\textup{(vii)}] If
 ...
 \qquad (k \neq 0)
 \]
 for every $g \geq 0$.  With the additional package~$H_D^K$, the
 surviving Hodge character for $g\ge1$ is
 \[
 \operatorname{ch}_g(\mathfrak O_g^K(\cA))
 =(-1)^g\kappa(\cA)\lambda_g
 \]
 \textup{(UNIFORM-WEIGHT; Theorem~\textup{\ref{thm:genus-universality})}.}
 Under $\mathsf{FH}^{\mathrm{det}}_{\mathbb{P}^1}(\cA)$, the same
 comparison reflects concentration and gives the converse.
\end{enumerate}

\smallskip
\noindent\textbf{Family-indexed chiral Hochschild support:}
\begin{enumerate}
\item[\textup{(viii)}] Let $S\subset\mathbb Z_{\geq0}$ be finite and
 assume $H_H(\cA;S)$.  Then
 \[
  \operatorname{Supp}\ChirHoch^\bullet(\cA)\subseteq S,
  \qquad
  P_\cA(t)=\sum_{n\in S}\dim H^n(K_{\cA,S})t^n.
 \]
 If $\cA^!$ carries $H_H(\cA^!;S^!)$ and the pair carries a perfect
 degree-$d$ chain pairing, then
 \[
  \ChirHoch^n(\cA)
  \cong \ChirHoch^{d-n}(\cA^!)^\vee \otimes \omega_X.
 \]
 Its graded-commutative cup-product algebra is governed by the separate
 multiplicative comparison
 \textup{(}Theorem~\textup{\ref{thm:main-koszul-hoch}},
 ...
 \cite{BDSK21}, so the support inclusion lives strictly on the stated
 Koszul/bounded locus, with the bounded-to-chart comparison part of
 $H_H(\cA;S)$.
\end{enumerate}
...
Koszulness of~$\tau_{\mathrm i}$ is acyclicity of
$K^L_{\tau_{\mathrm i}}(\cA,\cA^{\mathrm i})$ and
$K^R_{\tau_{\mathrm i}}(\cA^{\mathrm i},\cA)$.  On each associated-graded
finite window, Loday--Vallette's quadratic criterion
\cite[Theorem~3.4.6]{LV12} identifies this with diagonal
concentration of the quadratic bar comparison.  The split PBW
detection and strong convergence hypotheses transport that statement
to $E_2$-collapse of the chiral bar spectral sequence, in both
directions.

\smallskip\noindent
\textup{(ii)}$\Rightarrow$\textup{(v)}:
$E_2$-collapse gives bar concentration
(Theorem~\ref{thm:bar-concentration}).  The associated-graded map of
$q_\cA$ is the ordinary quadratic inclusion
$\cA^{\mathrm i}\to B(\cA)$, hence is a quasi-isomorphism by
Loday--Vallette~\cite[Theorem~3.4.6]{LV12}.  The strict filtration,
strong convergence, and Mittag--Leffler clauses of~$H_{\mathrm{CL}}$
lift it to the displayed equivalence~$q_\cA$; the same fundamental
twisting theorem gives the equivalence~$p_\cA$.

\smallskip\noindent
\textup{(v)}$\Rightarrow$\textup{(i)}:
Condition~\textup{(v)} is precisely the pair of maps induced by the
quadratic twisting morphism~$\tau_{\mathrm i}$.  The fundamental theorem of
chiral twisting morphisms
\textup{(}Theorem~\ref{thm:fundamental-twisting-morphisms}\textup{)},
transporting Loday--Vallette~\cite[Theorem~2.3.2]{LV12}, identifies
either comparison equivalence with acyclicity of both quadratic
twisted products.  This is condition~\textup{(i)}.

\medskip
\textsc{family-indexed chiral Hochschild clause}
\textup{(viii)}:

\smallskip\noindent
The strong deformation retract in $H_H(\cA;S)$ and its chart
quasi-isomorphism identify
$C^\bullet_{\mathrm{ch}}(\cA,\cA)$ with $K_{\cA,S}$.
Theorem~\ref{thm:main-koszul-hoch} gives the support inclusion, and
Theorem~\ref{thm:hochschild-polynomial-growth} gives the displayed
Hilbert polynomial.  The companion family datum and the perfect
degree-$d$ pairing give the degree reflection.  Proposition~\ref{prop:e2-formality-hochschild}
governs the additional brace and Massey-product structure.
...
$\mathsf{FH}_{\mathbb{P}^1}$, with the converse under
...
cohomologically concentrated in degree~$0$.  With~$H_D^K$,
Theorem~\ref{thm:genus-universality} identifies the Hodge character by
\[
\operatorname{ch}_g(\mathfrak O_g^K(\cA))
=(-1)^g\kappa(\cA)\lambda_g
\qquad (g \geq 1)
\]
The uniform-weight hypothesis leaves a single channel, so
$\delta F_g^{\mathrm{cross}}=0$ and degree-zero concentration is
preserved. Hence
...
The detecting package
$\mathsf{FH}^{\mathrm{det}}_{\mathbb{P}^1}(\cA)$ is precisely the
hypothesis that reflects Goodwillie concentration to the PBW page;
the converse is read on this detecting domain.
...
Under the uniform-weight hypothesis the higher-genus refinement is proved.
...
Definition~\ref{def:scalar-lane} selects one channel, so every
boundary graph is monochromatic.  Under~$H_D^K$, the surviving Hodge
character is identified by Theorem~\ref{thm:genus-universality}:
\[
\operatorname{ch}_g(\mathfrak O_g^K(\cA))
=(-1)^g\kappa(\cA)\lambda_g.
\]
...
$\kappa(\cA)\lambda_g$ by Grothendieck--Riemann--Roch, independently
of the shadow-tower route through the Maurer--Cartan element and
Theorem~\ref{thm:genus-universality}.  It gives a second proof of the
scalar part of the all-genera uniform-weight statement alongside the
conditional bar/factorization-homology concentration argument.

For multi-weight algebras, the higher-genus form of
condition~\textup{(vii)} passes to the scalar projection of the
genus-$g$ obstruction
...
In this form, condition~\textup{(vii)} survives at all genera at the
scalar-trace level.  The uniform-weight hypothesis is essential for
full cohomological concentration: the interacting algebra~$\cW_3$
leaves the scalar locus
...
hence every assignment is single-channel and
$\delta F_1^{\mathrm{cross}}=0$. At genus~$g \geq 2$,
...
Theorem~\ref{thm:hochschild-polynomial-growth} applied to
$H_H(\cA;S)$ prove family support and polynomial Hilbert growth.  A
companion datum $H_H(\cA^!;S^!)$ and a perfect degree-$d$ pairing
prove the separate duality
$n\leftrightarrow d-n$.  The one-dimensional Fulton--MacPherson
boundary calculus enters the construction of the family retract; its
OPE-residue lift determines~$S$.  Product ambients, categorical
Hochschild cochains of Calabi--Yau categories, and topological
Hochschild theory carry their respective K\"unneth and comparison
packages.  A multiplicative presentation of $\ChirHoch^*(\cA)$ is
governed by its brace-compatible formality theorem.
\end{remark}
...
Condition~\textup{(vi)} assumes precisely these two properties for
the functor defined by the quadratic resolution~$p_\cA$.
Condition~\textup{(v)} identifies that resolution, through~$q_\cA$,
with the full bar resolution on the complete component generated
by~$\cA$.  The Barr--Beck--Lurie theorem therefore gives the
comparison equivalence on that component.

\smallskip\noindent
\textup{(vi)}$\Rightarrow$\textup{(i)}:
If the BBL comparison functor $\Phi$ is an equivalence, then
$\barB_\kappa$ is conservative and its comparison unit on the
generator~$\cA$ is an equivalence.  Under the quadratic-resolution
package in~\textup{(vi)}, this unit is represented by~$q_\cA$ and
$p_\cA$; hence~\textup{(v)} holds, and the general twisting theorem
gives~\textup{(i)}.
...
split: $E_\infty = E_2$ as bigraded objects. Hence
$E_2$-collapse, hence~(i).

\medskip
\textsc{Lagrangian criterion}
\textup{(i)}$\Leftrightarrow$\textup{(xi)}
\emph{(under perfectness, non-degeneracy, and tangent-complex
...
On the Koszul locus, the quadratic cobar map~$p_\cA$ provides a free
resolution and~$q_\cA$ compares it with the full bar resolution.
Theorem~C supplies the Verdier-side homotopy eigenspace
decomposition
$\mathbf{Q}_g(\cA) \oplus \mathbf{Q}_g(\cA^!)
\simeq H^*(\overline{\cM}_g, \cZ_\cA)$
as a cochain-level eigenspace splitting whose scalar trace is a
subsequent projection. Under perfectness and non-degeneracy this
...
\quad\text{(quadratic twisting morphism and comparison package),}\qquad
...
For the quadratic twisting morphism
$\tau_{\mathrm i}\colon\cA^{\mathrm i}\to\cA$,
condition~\textup{(v)} is the equivalence of
$q_\cA\colon\cA^{\mathrm i}\to\bar B_X(\cA)$ and
$p_\cA\colon\Omega_X(\cA^{\mathrm i})\to\cA$, and
Theorem~\ref{thm:fundamental-twisting-morphisms}
identifies that directly with
$\tau_{\mathrm i}$ being a chiral Koszul morphism.  This is the direct
Loday--Vallette~\cite[Theorem~2.3.2 and Theorem~3.4.6]{LV12}
comparison route.

\smallskip\noindent
The link from the intrinsic core to~\textup{(viii)} carries the
additional family datum $H_H(\cA;S)$.  Condition~\textup{(i)} supplies
the associated-graded quadratic contraction; the OPE-residue lift,
incidence compatibility, completion, and averaging comparison in
$H_H(\cA;S)$ supply the complete retract.  Theorem~\ref{thm:main-koszul-hoch}
then gives family support, and
Theorem~\ref{thm:hochschild-polynomial-growth} gives polynomial
Hilbert growth.  A perfect degree-$d$ pairing supplies duality, while
Proposition~\ref{prop:e2-formality-hochschild} governs the
brace-compatible cup-product presentation.
...
Condition~\textup{(xi)} remains conditional on the
shifted-symplectic hypothesis package: perfectness, non-degeneracy of
the ambient tangent complex, cyclic/BV compatibility, and bar-chart
acyclicity. Condition~\textup{(xii)}
...
\item \emph{Chiral tensor product.}
 ...
 $\cA \boxtimes \cB$ is chirally Koszul, its tensor presentation
 coalgebra is $\cA^{\mathrm i}\boxtimes\cB^{\mathrm i}$, and the
 colored K"unneth map supplies the corresponding $q$-comparison to
 the full colored bar object.
\item \emph{Reflexive quadratic dualization.}
 Let $\cA=T_X(V)/(R)$ with $V$ perfect and
 $R\subset V\otimes V$ a locally split perfect subobject.  Assume
 $R^{\perp\perp}=R$, and assume the comparison maps for both
 quadratic presentations satisfy $H_{\mathrm{CL}}$. Then the canonical
 bidual map of quadratic presentations
 \[
 \beta_\cA\colon\cA\xrightarrow{\sim}
 (\cA^!_{\mathrm{quad}})^!_{\mathrm{quad}}
 \]
 is an isomorphism, and quadratic Koszulness of~$\cA$ is equivalent to
 quadratic Koszulness of~$\cA^!_{\mathrm{quad}}$.
\item \emph{Smooth base change.}
 ...
 duality, and PBW convergence preserved by $f^*$, and with the chosen
 pair map commuting with base change, then $f^*\cA$ is
...
\cA^{\mathrm i}\boxtimes\cB^{\mathrm i}$ at the level of the chosen
quadratic presentations. Finite duality gives the quadratic dual
algebra statement; the chosen pair maps transport it to the partner
algebras.

\textup{(b)}: Perfectness gives $V\simeq V^{\vee\vee}$ and the assumed
orthogonality gives $R\simeq R^{\perp\perp}$.  These maps identify the
generators and relations of the two quadratic presentations and define
$\beta_\cA$.  The quadratic twisting complexes for the two
presentations are finite duals of one another.  Exact finite duality,
followed by the two $H_{\mathrm{CL}}$ comparisons, identifies their
acyclicity.  This is the quadratic theorem of
Loday--Vallette~\cite[Theorem~3.4.6]{LV12} in the stated perfect
...
\begin{remark}[Off-diagonal $A_\infty$ formality and Gaiotto--Kulp--Wu]
\label{rem:ainfty-formality-gkw}
\index{Gaiotto--Kulp--Wu!formality characterization}
The off-diagonal $A_\infty$ formality characterization
(item~(iii) of Theorem~\ref{thm:koszul-equivalences-meta})
is the bar-complex statement of the formality theorem of
Gaiotto--Kulp--Wu~\cite{GKW2025}: the transferred operations with
off-diagonal output vanish if and only if $\cA$ is chirally Koszul.
The shadow depth classification
$\mathbf{G}/\mathbf{L}/\mathbf{C}/\mathbf{M}$
(Remark~\ref{rem:shadow-depth-gkw-refinement}) records the diagonal
operations left invisible by concentration, giving a four-class
stratification governed by the discriminant $\Delta = 8\kappa S_4$.
\end{remark}
...
The Swiss-cheese operations $m_k^{\mathrm{SC}}$ on~$\cA$ itself are
not the off-diagonal transferred operations killed by Koszulness.
They are diagonal shadow operations and are classified by shadow depth
as follows.
\begin{center}
...
The finite class-$M$ calculation therefore consists of the three
displayed coefficients
\[
 S_3=2,\qquad S_4=\frac{10}{c(5c+22)},\qquad
 S_5=-\frac{48}{c^2(5c+22)};
\]
the classification of all four landscape classes uses the named
structural results in the preceding paragraphs, not this finite list
alone.
\end{proof}

The four classes exhaust the finite standard-landscape atlas, not a
classification beyond it, but the relationship between SC-formality
and this atlas partition is sharper than the table records.
...
\begin{lemma}[Two-colour tree detection on the standard support;
\ClaimStatusConditional]
\label{lem:sc-tree-detection-standard}
\index{Swiss-cheese operad!two-colour tree detection}
\index{averaging map!not injective on all ordered trees}
\textup{(}Open quadrant; ordered-to-mixed genus-$0$
presentation; Beilinson levels~$3$--$4$; hypothesis package
$\mathsf{SC}^{\mathrm{tree}\text{-}\mathrm{det}}_{\mathrm{std}}$\textup{).}
Let~$\cA$ lie in the standard landscape and fix $r \geq 3$.
Let
\[
\mathsf{Tree}^{\mathrm{ord}}_{r,\mathrm{std}}(\cA)
\]
be the span of connected ordered genus-$0$ HPL trees of arity~$r$,
decorated by the standard-landscape OPE channels and quotiented by
the HPL boundary relations.  The same ordered tree sum has two
realisations:
\[
\iota^{\mathrm{oc}}_r\colon
\mathsf{Tree}^{\mathrm{ord}}_{r,\mathrm{std}}(\cA)
\longrightarrow
\mathrm{Hom}^{\mathrm{SC}}_r(\cA^{\otimes r},\cA),
\qquad
\pi^{\mathrm{sh}}_r\colon
\mathsf{Tree}^{\mathrm{ord}}_{r,\mathrm{std}}(\cA)
\longrightarrow
\operatorname{Sh}_r(\cA).
\]
Here $\iota^{\mathrm{oc}}_r$ is the open-colour
bulk-to-boundary realisation defining $m_r^{\mathrm{SC}}$, and
$\pi^{\mathrm{sh}}_r$ is the closed-colour shadow projection obtained
from the averaging identity
$\operatorname{av}(\Theta_\cA^{\Eone})=\Theta_\cA$.
The package
$\mathsf{SC}^{\mathrm{tree}\text{-}\mathrm{det}}_{\mathrm{std}}$
asserts that, on this standard primitive support,
\[
\iota^{\mathrm{oc}}_r(T)=0
\quad\Longleftrightarrow\quad
T=0
\quad\Longleftrightarrow\quad
\pi^{\mathrm{sh}}_r(T)=0 .
\]
Consequently, under this package,
\[
m_r^{\mathrm{SC}}(\cA)=0
\quad\Longleftrightarrow\quad
\operatorname{Sh}_r(\cA)=0
\qquad (r\geq3)
\]
degree by degree on the standard landscape.
\end{lemma}

\begin{proof}
The two maps are the colour-separated readings of the genus-$0$ tree
formula in Theorem~\ref{thm:shadow-formality-identification}, together
with the ordered-to-symmetric identity
$\operatorname{av}(\Theta_\cA^{\Eone})=\Theta_\cA$
of Theorem~\ref{thm:e1-primacy}.  What is not supplied by bare
averaging is detection of the ordered tree from either coloured
output: Theorem~\ref{thm:e1-primacy}(iv) explicitly leaves a
non-trivial kernel for $\operatorname{av}$, already containing the
degree-$3$ Drinfeld associator class.  The named package says exactly
that the standard primitive support used in the
$\mathbf G/\mathbf L/\mathbf C/\mathbf M$ classification does not meet
that kernel, and that the open-colour bulk-to-boundary realisation has
no additional kernel on connected positive-degree trees.  With these
two injectivity statements, vanishing of either coloured realisation is
equivalent to vanishing of the common ordered tree class.
\end{proof}
...
Let~$\cA$ be a chiral algebra in the standard landscape satisfying
the two-colour tree-detection package
$\mathsf{SC}^{\mathrm{tree}\text{-}\mathrm{det}}_{\mathrm{std}}$
of Lemma~\ref{lem:sc-tree-detection-standard}.
...
$\operatorname{av}(\Theta_\cA^{\Eone}) = \Theta_\cA$
\textup{(}Theorem~\ref{thm:e1-primacy}\textup{)}, with the
averaging kernel controlled on the standard primitive support by
Lemma~\ref{lem:sc-tree-detection-standard}.
...
(no connected tree survives); equivalently,
Lemma~\ref{lem:sc-tree-detection-standard} sends the vanishing
shadow tree to the vanishing open-colour operation.  Hence
...
Let~$T_r(\cA)$ be the ordered genus-$0$ tree class whose
open-colour realisation is $m_r^{\mathrm{SC}}$ and whose
closed-colour realisation is $\operatorname{Sh}_r(\cA)$.
By Lemma~\ref{lem:sc-tree-detection-standard},
\begin{equation}
...
\operatorname{Sh}_r(\cA)=0 .
\end{equation}
Thus SC-formality gives $S_r(\cA)=0$ for all $r\geq3$.
...
\textup{(}Theorem~\ref{thm:e1-primacy}\textup{)}, the two-colour
tree-detection package
\textup{(}Lemma~\ref{lem:sc-tree-detection-standard}\textup{)}, and
...
They are distinct from operadic Koszul duality of the
...
PBW-local; the fourth is the family-specific open mathematical condition.
...
\noindent
Thus the exact open mathematical condition is the equality
...
12 proved Koszul via equivalences~\textup{(ii)} and~\textup{(ix)},
...
\textup{(}$L_k(\mathfrak{sl}_3)$ at admissible $q = 3$:
$\mathrm{rk}(\fg) = 2$ Cartan classes survive in $H^2$ under
Theorem~\textup{\ref{thm:admissible-sl3-non-koszul-qge3}}\textup{)},
3 open ($V_{-2}(\mathfrak{sl}_2)$ at critical level,
$L_1(\mathfrak{sl}_2)$, and triplet $\cW(2)$).
...
\begin{remark}[Lattice examples and the five typed objects]
...
$L_k(\mathfrak{sl}_3)$, adm.\ $q = 3$${}^{\ddagger}$
 & simple quot.\ & $8$ & Cond.\ cone & Cartan $H^2$ & $L$, $r_{\max}=3$ \\
\midrule
...
Not K.\ = proved non-Koszul; Cond.\ cone = a nonzero quadratic-comparison
cone under the finite admissible Li--bar package of
...
\item \emph{Admissible-level simple quotients at higher rank.}
 ...
 At $\mathrm{rk}(\fg)=2$ and $q=3$, their survival follows
 conditionally from the finite admissible Li--bar package of
 Theorem~\ref{thm:admissible-sl3-non-koszul-qge3}.  The rank-two
 case with $q>3$ awaits the incoming $d_1$ calculation; the
 higher-rank case is conjectural
...
$r^{\mathrm{cl}}(z) = k\Omega_{\mathrm{tr}}/z$ of an affine algebra
...
$r$-matrix $r^{\mathrm{cl}}(z) = k\Omega_{\mathrm{tr}}/z$ has vanishing second
Poisson cohomology:
\[
...
The associated graded of the ordered bar deformation complex carries
the Chevalley--Eilenberg coalgebra on $\fg^*$ as its semiclassical
shadow; rigidity of the
...
\item
 the fourteen admissible atlas charts of
 Theorem~\ref{v1-thm:kms-fourteen-home-chart} form a finite affine
...
\begin{theorem}[Yangian chart inclusion; \ClaimStatusConditional]
...
evaluation-module construction defines a finite-PBW $\Eone$-chiral
avatar and that the chart-comparison functor is representable and
reflects the Shapovalov open locus. Then the associative Koszulness
moduli embeds as an open subscheme of the chiral Koszulness moduli:
\[
...
The theorem is a conditional comparison of moduli functors, not a
proof that every Yangian chart has already been constructed in the
chiral atlas.
\end{theorem}
...
By the assumed finite-PBW $\Eone$-chiral avatar, the Yangian carries
the required chiral structure on the formal disk via its evaluation
modules $V_u$, and the PBW filtration is compatible with the chiral
bracket. The associative chart coordinate
...
By the second hypothesis, the chart $U_1^{\mathrm{assoc}}$ is the
complement of the Shapovalov zero locus and its preimage in the
chiral moduli is the complement of the same zero locus. Hence the
comparison morphism is an open immersion on the comparison component.
\end{proof}
...
the precise content of the chart-surjectivity problem: do there exist
chiral charts
...
\begin{proof}
(i) By Koszul concentration, the off-diagonal deformation complex of
$H^*(\barB(\cA))$ is acyclic. The remaining diagonal operations are the
shadow classes of $\cA$; they are fixed by the genus-$0$ OPE data and
are not moduli. The MC moduli is therefore zero-dimensional, and
uniqueness of $\Theta_\cA$ follows from the bar-intrinsic construction
...
\begin{definition}[Primitive cumulant quotient; \ClaimStatusDefinitional]
...
Under $H_{\mathbb D}^{\mathrm{bar}}(\cA)$, the formula
\[
K_X(\cA)=\mathbb D_{\Ran}^{\mathrm{cont}}B_X^{\mathrm{cont}}(\cA)
\]
defines a post-Verdier factorization algebra.  A chiral Koszul pair
supplies the further comparison between this algebra and the opposite
member of the pair.

\begin{theorem}[Geometric bar--cobar duality; \ClaimStatusConditional]
\label{thm:bar-cobar-isomorphism-main}
\index{bar-cobar duality!main theorem|textbf}
\index{Theorem A|textbf}
\textup{[Type signature: Open quadrant; complete factorization
presentation; Beilinson levels~$1\leftrightarrow2$; hypothesis packages
$H_{\mathrm{CL}}$ and $H_{\mathbb D}^{\mathrm{bar}}$, together with the
pair maps~$\theta_{12},\theta_{21}$.]}
...
$\operatorname{Ran}(X)$. The counit maps in part~\textup{(1)} are
equivalences of complete augmented factorization algebras for the
chiral tensor product.  The maps in parts~\textup{(2)}--\textup{(3)}
are equivalences of augmented algebras in the post-Verdier $*$-tensor
factorization category.
...
\item contravariance of Verdier duality and the pair maps define
 equivalences of post-Verdier algebras
\[
\nu_{12}:=
\theta_{12}\circ\mathbb D_{\operatorname{Ran}}(\eta_1)\colon
K_X(\cA_1)\xrightarrow{\sim}\cA_2,
\qquad
\nu_{21}:=
\theta_{21}\circ\mathbb D_{\operatorname{Ran}}(\eta_2)\colon
K_X(\cA_2)\xrightarrow{\sim}\cA_1;
\]

\item if $\cA_2$ is denoted by $\cA_1^!$, then
\[
K_X(\cA_1)\xrightarrow[\nu_{12}]{\sim}\cA_1^!,
\qquad
K_X(\cA_1^!)\xrightarrow[\nu_{21}]{\sim}\cA_1.
\]
 For a quadratic presentation, the further map
 $\mathbb D(q_{\cA_1})\colon K_X(\cA_1)\to
 \mathbb D\cA_1^{\mathrm i}$ is the Verdier transpose of quadratic
 recognition.
\end{enumerate}
\end{theorem}
...
\]
be the unit equivalence from part~\textup{(1)}. Applying
$\mathbb{D}_{\operatorname{Ran}}$ to $\eta_1$ gives
\[
K_X(\cA_1)=\mathbb{D}_{\operatorname{Ran}}\barB_X(\cA_1)
\xrightarrow{\sim}
\mathbb{D}_{\operatorname{Ran}}(\cC_1).
\]
Composing with $\theta_{12}$ gives~$\nu_{12}$. The same argument with
the indices exchanged gives~$\nu_{21}$.
Part~(3) is the same statement after naming $\cA_2$ as the Koszul
dual~$\cA_1^!$.
\end{proof}

\begin{remark}[$\infty$-categorical presentation of the bar--cobar argument]
Theorem~\ref{thm:bar-cobar-isomorphism-main} admits the following
$\infty$-categorical presentation of the same Koszul-effectivity and
Verdier-compatibility argument from the formulation of
...
recovers part~\textup{(1)} without passing through the filtered
comparison from the simplicial bar object to the explicit dg model.

For part~\textup{(2)}, continuous Verdier duality reverses the unit
$\eta_1\colon\cC_1\to B_X(\cA_1)$ and gives
\[
K_X(\cA_1)=\mathbb D_{\Ran}B_X(\cA_1)
\xrightarrow{\mathbb D(\eta_1)}
\mathbb D_{\Ran}\cC_1
\xrightarrow{\theta_{12}}\cA_2.
\]
Theorem~\ref{thm:bar-cobar-verdier} gives the separate natural
equivalence
$\Omega_X^{\mathrm{geom,cont}}(\mathbb D_X\cA_1)\simeq K_X(\cA_1)$.
The cobar in this formula has the coalgebra $\mathbb D_X\cA_1$ as its
argument.

Thus the $\infty$-categorical presentation uses the same
Koszul-effectivity and continuous duality packages as the dg
presentation.  The maps~$\theta_{ij}$ contain the family-specific
identification with the opposite algebra.
\end{remark}
...
\[
\theta_{12,S}\colon
\mathbb{D}_{\operatorname{Ran}(\mathcal{X}/S)}^{\mathrm{cont}}(\cC_{1,S})
\xrightarrow{\sim}\cA_{2,S},
\qquad
\theta_{21,S}\colon
\mathbb{D}_{\operatorname{Ran}(\mathcal{X}/S)}^{\mathrm{cont}}(\cC_{2,S})
\xrightarrow{\sim}\cA_{1,S},
\]
...
\begin{remark}[Open mathematical conditions for the modular-family extension]
\label{rem:bar-cobar-relative-theory}
\index{bar-cobar duality!modular-family open mathematical conditions}
...
\item prove the relative Verdier/base-change isomorphism on the
 ...
 gluing maps.
\end{enumerate}
The fixed-curve conclusion carries the packages
$H_{\mathrm{CL}}$, $H_{\mathbb D}^{\mathrm{bar}}$, and
$\theta_{12},\theta_{21}$ stated in
...
Theorem~\textup{A} holds over~$S$. Over
$\overline{\mathcal{M}}_{g,n}$, the extension carries an additional
boundary condition: the bar complex over a nodal fiber involves
...
open mathematical condition recorded in
Remark~\ref{rem:bar-cobar-relative-theory}. Thus a fixed smooth curve
uses the theorem packages above; a smooth relative family adds
relative holonomicity, base change, and pair lifts; and the stable
compactification adds nodal sewing.
\end{remark}
...
$\operatorname{Ran}(X)$ under the factorization-closure package.  The
enhanced associative Ran theorem makes the unit and counit
equivalences. Verdier duality gives the algebra
$K_X(\cA)=\mathbb D_{\operatorname{Ran}}\mathbf B_X(\cA)$, and the pair
map $\nu\colon K_X(\cA)\to\cA^!$ gives the family-specific dual
identification.
...
Choose equivalences
$u_i\colon B_i\xrightarrow{\sim}B_X(\cA)$ from two admissible dg
presentations to the fixed intrinsic bar object.  The space of
comparison equivalences $f\colon B_1\to B_2$ satisfying
$u_2f\simeq u_1$ is contractible
(Proposition~\ref{prop:model-independence}).  After the maps~$u_i$ are
forgotten, the comparison space carries the natural
$\operatorname{Aut}(B_X(\cA))$-torsor ambiguity.

\emph{Cohomology.}
The bar coalgebra $\bar{B}_X(\cA)$ has cohomology
$H^\bullet\bar B_X(\cA)$ concentrated on the Koszul diagonal.  For a
chosen quadratic presentation satisfying $H_{\mathrm{CL}}$, the map
$H^\bullet(q_\cA)$ identifies this coalgebra with
$H^\bullet(\cA^{\mathrm i})$. When the finite-type Verdier dual exists,
the post-Verdier algebra~$\cA^!$ is characterized by the pair equivalence
$\nu\colon K_X(\cA)=\mathbb{D}_{\operatorname{Ran}}
\bar{B}_X(\cA)\xrightarrow{\sim}\cA^!$ under the pair hypotheses, with
dimensions controlled by the spectral
discriminant~$\Delta_{\cA}(x)$.

\emph{Cohomology extraction.}
Taking cohomology of the dg model gives
$H^\bullet\bar B_X(\cA)$.
...
and the resulting Hilbert series is
$\sum_n \dim H^n(\bar{B}_X(\cA))\, x^n$.  Under $H_{\mathrm{CL}}$
this equals the cohomological Hilbert series of~$\cA^{\mathrm i}$.
Under $H_{\mathbb D}^{\mathrm{bar}}$ and the pair equivalence~$\nu$,
finite Verdier duality gives the corresponding series for~$\cA^!$,
with radius of convergence determined by~$\Delta_{\cA}(x)$.
\end{remark}
...
defines the same quadratic algebra as $R(u;-\hbar)$. The presentation
object computed here is the quadratic coalgebra $Q_R^{\mathrm i}$; the notation
...
\cong \mathsf H^{\mathrm{bar,ord}}_{\hbar,\le m}(\mathfrak g)
\]
as a conilpotent ordered chiral coalgebra. Passing to the inverse limit
over~$m$ gives the completed bar-cohomology coalgebra
$\mathsf H^{\mathrm{bar,ord}}_\hbar(\mathfrak g)$ whenever the finite windows form the
Mittag--Leffler system of Lemma~\ref{lem:completion-convergence}.
If a finite window carries a chosen quadratic presentation coalgebra
$Y_{\hbar,\le m}(\mathfrak g)^{\mathrm i}$ satisfying
$H_{\mathrm{CL}}$, its map $H^\bullet(q_Y)$ identifies that
presentation cohomology with the displayed bar cohomology.  Continuous
duality of the full bar windows gives $K_X(Y_\hbar)$, and a chosen
pair map compares this algebra with $Y_\hbar(\mathfrak g)^!$.
\end{proposition}
...
The construction therefore produces
$\mathsf H^{\mathrm{bar,ord}}_\hbar(\mathfrak g)$ and the full ordered
bar coalgebra.  A quadratic presentation supplies a separate
$Y_\hbar(\mathfrak g)^{\mathrm i}$ and comparison~$q_Y$; continuous
Verdier duality supplies $K_X(Y_\hbar)$; the selected partner is the
target of its pair map.
\end{proof}
...
\section{Quadratic recognition and residue-dual comparison}
\label{sec:three-stage-construction}

\subsection{The type-correct comparison problem}

\begin{problem}[Typed quadratic and Verdier comparisons]
\label{prob:circularity}
Let
$\mathcal A=T_{\mathrm{ch}}(\mathcal V)/(R)$ be an augmented
quadratic chiral algebra.  Its chosen presentation determines
\[
 \mathcal A^{\mathrm i}
 =C_X(s^{-1}\mathcal V,s^{-2}R),
 \qquad
 q_{\mathcal A}\colon
 \mathcal A^{\mathrm i}\longrightarrow B_X(\mathcal A).
\]
The package
$H_{\mathrm{CL}}(\mathcal A,\mathcal A^{\mathrm i},
\tau_{\mathrm i})$ makes $q_{\mathcal A}$ a quasi-isomorphism and
hence gives the cohomological recognition map
\[
 H^\bullet(q_{\mathcal A})\colon
 H^\bullet(\mathcal A^{\mathrm i})
 \xrightarrow{\sim}H^\bullet B_X(\mathcal A).
\]
The full bar coalgebra has the Verdier-dual algebra
\[
 K_X(\mathcal A):=\mathbb D_{\operatorname{Ran}}B_X(\mathcal A),
\]
and a chiral Koszul pair supplies a chosen algebra map
$\nu_{\mathcal A}\colon K_X(\mathcal A)\to\mathcal A^!$.
On a finite quadratic lane, Verdier duality also gives
\[
 \mathbb D(q_{\mathcal A})\colon
 K_X(\mathcal A)\longrightarrow
 \mathcal A^!_{\mathrm{quad}},
 \qquad
 \mathcal A^!_{\mathrm{quad}}
 :=\mathbb D_{\operatorname{Ran}}\mathcal A^{\mathrm i}.
\]
A comparison
$c_{\mathcal A}\colon\mathcal A^!_{\mathrm{quad}}\to\mathcal A^!$
recovers the chosen pair map precisely when
\[
 \nu_{\mathcal A}
 =c_{\mathcal A}\circ\mathbb D(q_{\mathcal A}).
\]
These arrows separate quadratic recognition, universal
reconstruction, Verdier duality, and the family-specific choice of
dual partner.
\end{problem}
...
residue calculus after the ordered desuspension convention of
Definition~\ref{def:suspension}.
\end{remark}

%================================================================
% RESIDUE-DUAL COMPARISON COALGEBRA
%================================================================

\subsection{The residue-dual comparison coalgebra}

\begin{definition}[Residue-dual comparison coalgebra;
\ClaimStatusDefinitional]\label{def:koszul-dual-coalgebra}
\label{def:intrinsic-koszul-dual}
\label{def:koszul-dual-chiral}
\index{Koszul dual!residue-dual comparison|textbf}
...
$R_{\geq 2}$, and transferred OPE residues
\[
...
Put
\[
\mathcal{V}^{\vee}:=\mathbb D_X\mathcal V
...
$\Eone$ lane.  Let $R_{\geq2}^{\perp}$ denote the aritywise
annihilator of the leading multilinear relation system under the
Serre--residue pairing.  The \emph{residue-dual comparison
coalgebra} $\mathcal C_{\mathcal A}^{\mathrm{res}}$ is the largest
closed conilpotent subcoalgebra of
$\widehat{T}^{c}_{\mathrm{ch}}(s^{-1}\mathcal{V}^{\vee})$ whose
corelation system lies in $R_{\geq2}^{\perp}$.
...
coderivation
\[
q_{\mathrm{res}}=\sum_{r\geq 1}q_{\mathrm{res},r}
\]
is the unique continuous coderivation whose projection to cogenerators
is dual, under Serre--residue pairing and the ordered desuspension
convention of Definition~\ref{def:suspension}, to the transferred OPE
residues
...
higher residues. If $q_{\mathrm{res},r}=0$ for $r\geq3$, this is a strict dg chiral
coalgebra. In general it is a completed conilpotent
$A_\infty$ chiral coalgebra, and the completed cobar functor is taken in
that $A_\infty$ sense.

For a quadratic presentation
$\mathcal A=T_{\mathrm{ch}}(\mathcal V)/(R)$, the underlying quadratic
coalgebra is
\[
 \mathcal C_{\mathcal A}^{\mathrm{res}}
 =C_X(s^{-1}\mathcal V^\vee,s^{-2}R^\perp)
 =\bigl(\mathcal A^!_{\mathrm{quad}}\bigr)^{\mathrm i}.
\]
Thus $\mathcal C_{\mathcal A}^{\mathrm{res}}$ is attached to the
residue-dual presentation.  A comparison with
$\mathcal A^{\mathrm i}=C_X(s^{-1}\mathcal V,s^{-2}R)$ requires a
chosen self-duality of the presentation.
\end{definition}

\begin{definition}[Residue self-duality comparison package;
\ClaimStatusDefinitional]
\label{def:residue-self-duality-package}
For a finite-type quadratic presentation
$\mathcal A=T_{\mathrm{ch}}(\mathcal V)/(R)$, the package
$H_{\mathrm{res}}(\mathcal A;\beta,\chi)$ consists of:
\begin{enumerate}[label=\textup{(\roman*)}]
\item a perfect presentation polarization
$\beta\colon\mathcal V\xrightarrow{\sim}\mathbb D_X\mathcal V$;
\item the relation compatibility
$\beta^{\boxtimes2}(R)=R^\perp$, including the suspension and
collision-residue conventions;
\item a continuous filtered $A_\infty$ coalgebra morphism
\[
 \chi_{\mathcal A,\beta}\colon
 \mathcal A^{\mathrm i}\longrightarrow
 \mathcal C_{\mathcal A}^{\mathrm{res}}
\]
whose cogenerator component is $s^{-1}\beta$ and which intertwines
the quadratic twisting coderivation with $q_{\mathrm{res}}$;
\item finite-dimensional filtration windows, strong convergence, and
Mittag--Leffler transition maps, with
$\operatorname{gr}(\chi_{\mathcal A,\beta})$ an isomorphism.
\end{enumerate}
In the strict finite quadratic case, items~\textup{(i)}--\textup{(ii)}
induce $\chi_{\mathcal A,\beta}$ by the universal property of the
quadratic coalgebra, and this map is an isomorphism.
\end{definition}

\begin{remark}[Presentation dependence]
\label{rem:independence-construction}
The datum $\mathcal C_{\mathcal A}^{\mathrm{res}}$ depends on the
chosen generators, leading relation system, transferred residues, and
residue pairing.  The quadratic coalgebra $\mathcal A^{\mathrm i}$
depends on the chosen quadratic presentation.  The full bar coalgebra
$B_X(\mathcal A)$ is functorial in the augmented algebra, while the
chosen partner $\mathcal A^!$ enters through
$\nu_{\mathcal A}\colon K_X(\mathcal A)\to\mathcal A^!$.
\end{remark}
...
\begin{theorem}[Coalgebra structure on
\texorpdfstring{$\mathcal C_{\mathcal A}^{\mathrm{res}}$}{C-A-res};
\ClaimStatusConditional]
\label{thm:coalgebra-axioms-verified}
\textup{[Type signature: Open quadrant; residue-dual PBW
presentation; Beilinson level~$2$; hypothesis package = transferred
chiral $A_\infty$ identities + corelation stability + completion.]}
Assume the transferred residues $\{\mu_r\}$ satisfy the
Borcherds--Jacobi and Arnold--Orlik--Solomon boundary identities with
the ordered bar--cobar signs of
Theorem~\ref{thm:det-bar-cobar-signs}.  Assume further that
$q_{\mathrm{res}}$ preserves the closed subcoalgebra of
Definition~\ref{def:intrinsic-koszul-dual}. Then
\[
(\mathcal C_{\mathcal A}^{\mathrm{res}},
 \Delta,\epsilon,q_{\mathrm{res}})
\]
is a completed conilpotent chiral coalgebra. In the strict case
$q_{\mathrm{res},r}=0$ for $r\geq3$, it satisfies
\begin{enumerate}
...
\item $\Delta q_{\mathrm{res}}
=(q_{\mathrm{res}}\boxtimes\mathrm{id}
+\mathrm{id}\boxtimes q_{\mathrm{res}})\Delta$;
\item $q_{\mathrm{res}}^2=0$.
\end{enumerate}
\end{theorem}
...
counit. Since $q_{\mathrm{res}}$ is defined as the continuous coderivation extending
its projection to cogenerators, the coderivation identity follows from
the cofree universal property.  This universal extension includes the
middle deconcatenation terms on every cogenerator word.

For the square-zero identity, recall that on a cofree conilpotent
coalgebra a coderivation is zero if and only if its projection to
cogenerators is zero. Hence $q_{\mathrm{res}}^2=0$ is checked after projection to
...
\]
With the ordered bar-cobar signs fixed in
Theorem~\ref{thm:det-bar-cobar-signs},
...
subcoalgebra. Therefore $q_{\mathrm{res}}^2=0$ in the strict case, and the general
case satisfies the completed $A_\infty$ coalgebra identities.
\end{proof}

\begin{remark}
The theorem constructs the residue-dual coalgebra.  The comparison
with $\mathcal A^{\mathrm i}$ is governed by
$H_{\mathrm{res}}(\mathcal A;\beta,\chi)$; the comparison with the
full bar coalgebra is governed by $q_{\mathcal A}$ and
$H_{\mathrm{CL}}$; the algebra partner is governed by
$\nu_{\mathcal A}$.
\end{remark}
...
\begin{theorem}[Quadratic recognition with residue self-duality;
\ClaimStatusConditional]
\label{thm:bar-computes-koszul-dual-complete}
\label{thm:chiral-koszul-duality}
\textup{[Type signature: Open quadrant; finite-type quadratic
presentation; Beilinson level~$2$ and post-Verdier level~$2$;
hypothesis packages
$H_{\mathrm{CL}}$, $H_{\mathrm{res}}$, and
$H_{\mathbb D}^{\mathrm{bar}}$.]}
Let $\mathcal A=T_{\mathrm{ch}}(\mathcal V)/(R)$ carry its quadratic
coalgebra $\mathcal A^{\mathrm i}$, its canonical comparison
$q_{\mathcal A}\colon\mathcal A^{\mathrm i}\to B_X(\mathcal A)$,
and a residue self-duality package
$H_{\mathrm{res}}(\mathcal A;\beta,\chi)$.  Under
$H_{\mathrm{CL}}(\mathcal A,\mathcal A^{\mathrm i},
\tau_{\mathrm i})$, the diagram
\begin{equation}
\label{eq:bar-quadratic-residue-roof}
 B_X(\mathcal A)
 \xleftarrow[\sim]{\ q_{\mathcal A}\ }
 \mathcal A^{\mathrm i}
 \xrightarrow[\sim]{\ \chi_{\mathcal A,\beta}\ }
 \mathcal C_{\mathcal A}^{\mathrm{res}}
\end{equation}
is a roof of filtered quasi-isomorphisms of completed conilpotent
chiral coalgebras.  In particular,
\[
 H^\bullet(q_{\mathcal A})\colon
 H^\bullet(\mathcal A^{\mathrm i})
 \xrightarrow{\sim}H^\bullet B_X(\mathcal A).
...
Under $H_{\mathbb D}^{\mathrm{bar}}(\mathcal A)$, continuous Verdier
duality gives the algebra diagram
\begin{equation}
\label{eq:verdier-quadratic-residue-cospan}
 K_X(\mathcal A)
 \xrightarrow{\ \mathbb D(q_{\mathcal A})\ }
 \mathcal A^!_{\mathrm{quad}}
 \xleftarrow{\ \mathbb D(\chi_{\mathcal A,\beta})\ }
...
$c_{\mathcal A}\colon\mathcal A^!_{\mathrm{quad}}\to\mathcal A^!$
with
$\nu_{\mathcal A}=c_{\mathcal A}\circ\mathbb D(q_{\mathcal A})$
identifies the cospan with the selected chiral Koszul partner.
\end{theorem}

\begin{proof}
The morphism $q_{\mathcal A}$ is the canonical map associated with the
quadratic twisting morphism
$\tau_{\mathrm i}\colon\mathcal A^{\mathrm i}\to\mathcal A$.
The package $H_{\mathrm{CL}}$ asserts its acyclicity, so
$q_{\mathcal A}$ is a quasi-isomorphism.  The package
$H_{\mathrm{res}}$ makes $\chi_{\mathcal A,\beta}$ a filtered
$A_\infty$ coalgebra morphism whose associated graded is an
isomorphism.  Lemma~\ref{lem:completion-convergence} promotes that
associated-graded isomorphism to the second quasi-isomorphism in
\eqref{eq:bar-quadratic-residue-roof}.

Continuous Verdier duality reverses both arrows in the roof.  The
first transpose has source
$K_X(\mathcal A)=\mathbb D_{\operatorname{Ran}}B_X(\mathcal A)$ and
target
$\mathcal A^!_{\mathrm{quad}}
=\mathbb D_{\operatorname{Ran}}\mathcal A^{\mathrm i}$, which gives
\eqref{eq:verdier-quadratic-residue-cospan}.  The displayed
factorization of $\nu_{\mathcal A}$ is exactly the additional datum
which selects the partner algebra~$\mathcal A^!$.
\end{proof}
...
\begin{corollary}[Typed residue-self-dual quadratic comparison;
\ClaimStatusConditional]
\label{cor:circularity-free-koszul}
Let $\mathcal A=T_{\mathrm{ch}}(\mathcal V)/(R)$ be a finite-type
quadratic chiral algebra on a smooth curve~$X$.  Assume:
\begin{enumerate}[label=\textup{(\roman*)}]
\item\label{hyp:fg}
$H_{\mathrm{CL}}(\mathcal A,\mathcal A^{\mathrm i},
\tau_{\mathrm i})$ holds for
$\mathcal A^{\mathrm i}=C_X(s^{-1}\mathcal V,s^{-2}R)$;
\item\label{hyp:poly}
$H_{\mathrm{res}}(\mathcal A;\beta,\chi)$ holds for the
residue-dual comparison coalgebra
$\mathcal C_{\mathcal A}^{\mathrm{res}}$;
\item\label{hyp:pbw-koszul}
$H_{\mathbb D}^{\mathrm{bar}}(\mathcal A)$ supplies continuous
Verdier duality on the full bar object;
...
$\nu_{\mathcal A}=c_{\mathcal A}\circ\mathbb D(q_{\mathcal A})$.
\end{enumerate}
Then the three coalgebras are identified in the derived category by
the canonical roof
\[
 B_X(\mathcal A)
 \xleftarrow[\sim]{q_{\mathcal A}}
 ...
 \mathcal C_{\mathcal A}^{\mathrm{res}},
\]
and the selected dual algebra is reached by
\[
 K_X(\mathcal A)
 \xrightarrow{\mathbb D(q_{\mathcal A})}
 \mathcal A^!_{\mathrm{quad}}
 \xrightarrow{c_{\mathcal A}}\mathcal A^!.
\]
Universal reconstruction is the counit
$\Omega_XB_X(\mathcal A)\to\mathcal A$.
\end{corollary}

\begin{proof}
Theorem~\ref{thm:bar-computes-koszul-dual-complete} gives the roof and
its Verdier transpose.  Hypothesis~\textup{(v)} composes that
transpose with the chosen partner comparison.  The bar--cobar counit
has source $\Omega_XB_X(\mathcal A)$ and target $\mathcal A$ by the
enhanced associative Ran reconstruction theorem.
\end{proof}
...
$R \subset j_*j^*(\mathcal{V}^{\boxtimes 2})$, its presentation
coalgebra and finite quadratic dual algebra are
\[
 \mathcal A^{\mathrm i}
 =C_X(s^{-1}\mathcal V,s^{-2}R),
 \qquad
 \mathcal A^!_{\mathrm{quad}}
 =\mathbb D_{\operatorname{Ran}}\mathcal A^{\mathrm i}
 \simeq T_{\mathrm{ch}}(s\mathcal V^\vee)/(R^\perp).
\]
The residue-dual candidate is the quadratic coalgebra of this dual
presentation:
\[
 \mathcal C_{\mathcal A}^{\mathrm{res}}
 =C_X(s^{-1}\mathcal V^\vee,s^{-2}R^\perp)
 =\bigl(\mathcal A^!_{\mathrm{quad}}\bigr)^{\mathrm i}.
\]
A polarization $\beta$ carrying $R$ to $R^\perp$ identifies
$\mathcal A^{\mathrm i}$ with
$\mathcal C_{\mathcal A}^{\mathrm{res}}$.  This is the precise
self-dual presentation locus.  The map
$q_{\mathcal A}\colon\mathcal A^{\mathrm i}\to B_X(\mathcal A)$
remains the Ginzburg--Kapranov/Loday--Vallette quadratic recognition
map.
\end{remark}
...
T_{(0)}T=\partial T
\]
with the suspension signs fixed in Theorem~\ref{thm:det-bar-cobar-signs}.

The completed bar-cohomology coalgebra is
\[
\mathsf H_c^{\mathrm{bar}}(\mathrm{Vir})
:=H^*\!\left(
\widehat{\barB_X^\Sigma(\mathrm{Vir}_c)},
d_{\mathrm{bar}}
\right),
\]
or the corresponding ordered cohomology on the $\Eone$ lane.  The
full completed bar complex is carried by the completed cofree
coalgebra on the descendants
\[
T,\partial T,\partial^2T,\ldots
\]
and has a continuous coderivation whose arity-two components are the
three displayed OPE modes and their $\partial$-translates. Its
cohomology is $\mathsf H_c^{\mathrm{bar}}(\mathrm{Vir})$. In a fixed conformal
...
The full bar object gives the post-Verdier algebra
\[
K_X(\mathrm{Vir}_c)
=\mathbb D_{\operatorname{Ran}}
\widehat{\barB_X^\Sigma(\mathrm{Vir}_c)},
\]
or the continuous finite-window dual in the affine model.  A chosen
pair map $\nu_{\mathrm{Vir}_c}\colon K_X(\mathrm{Vir}_c)
\to\mathrm{Vir}_c^!$ supplies the family-specific partner. The scalar
central channel is normalized by
\[
\kappa(\mathrm{Vir}_c)=\frac{c}{2};
\]
it is a curvature component after the curved genus package is
turned on.  The completed tensor coalgebra $\widehat{T^c(T^*)}$
belongs to the coalgebra presentation of the bar complex, while
$K_X(\mathrm{Vir}_c)$ and $\mathrm{Vir}_c^!$ belong to the
post-Verdier algebra lane.
\end{example}
...
\[
\frac{16}{22+5c}\frac{\Lambda(w)}{(z-w)^2}
+\frac{8}{22+5c}\frac{\partial\Lambda(w)}{z-w},
\]
...
components are the OPE modes of $T,T$, $T,W$, and $W,W$.  The
Fulton--MacPherson collision map retains every singular OPE channel;
the sextic central pole is one of the scalar
...
Its bar-cohomology coalgebra is
\[
\mathsf H^{\mathrm{bar}}(W_3)
:=H^*\!\left(\widehat{\barB_X^\Sigma(W_3)},d_{\mathrm{bar}}\right),
\]
or the ordered version in the $\Eone$ refinement. The full bar complex
is represented by a closed subcoalgebra of the completed cofree
conilpotent coalgebra on the duals of
\[
...
beginning with $\Lambda^\vee$. Its post-Verdier algebra is
\[
K_X(W_3)=\mathbb D_{\operatorname{Ran}}
\widehat{\barB_X^\Sigma(W_3)},
\]
when the Verdier finite-type hypotheses are satisfied; a pair map
$\nu_{W_3}\colon K_X(W_3)\to W_3^!$ selects the partner algebra.
Completion converges under the bidegreewise finite-dimensional and
...
\begin{construction}[$W_N$ completed bar and its cohomology]
...
The bar-cohomology coalgebra is
\[
\mathsf H^{\mathrm{bar}}(W_N)
:=H^*\!\left(\widehat{\barB_X^\Sigma(W_N)},d_{\mathrm{bar}}\right).
\]
The full bar complex sits inside the completed conilpotent coalgebra
cogenerated by the duals of the strong generators, their
$\partial$-descendants, and the normal-ordered composite coordinates
forced by the OPE closure relations. Its structure
...
\[
K_X(W_N)=\mathbb D_{\operatorname{Ran}}
\widehat{\barB_X^\Sigma(W_N)},
\]
under the same finite-type and completion hypotheses as in
Corollary~\ref{cor:circularity-free-koszul}.  A chosen pair map
$\nu_{W_N}\colon K_X(W_N)\to W_N^!$ supplies the family-specific
dual partner.
\end{construction}
...
$\mathcal{V}$ equal to the chosen factorization/Ran enhancement
satisfying $H_{\Fact}(X)$. The coefficient category
$\mathcal{D}\text{-mod}(X)$ supplies the stable presentable
linear ambient and the Beilinson--Drinfeld pseudo-tensor operations,
but not by itself an ordinary symmetric monoidal chiral tensor
bifunctor. We verify the required hypotheses on~$\mathcal{V}$:
\begin{enumerate}
...
\item \emph{Symmetric monoidal/factorization ambient.}
The Beilinson--Drinfeld chiral operations form a pseudo-tensor
structure by \cite[\S3.4]{BD04}; they are not used here as an
ordinary symmetric monoidal bifunctor on $\mathcal{D}\text{-mod}(X)$.
The symmetric monoidal operation required by
Lemma~\ref{lem:operadic-koszul-transfer} is the factorization/Ran
enhancement and Francis--Gaitsgory star-product isolated as the conditional
package $H_{\Fact}(X)$ in
...
is compared with the quadratic presentation coalgebra
$\mathcal H_k^{\mathrm i}$ by
$q_{\mathcal H_k}\colon\mathcal H_k^{\mathrm i}	o
\barB_X(\mathcal H_k)$.  Finite Verdier duality gives
$K_X(\mathcal H_k)$, the chosen pair map identifies it with the curved
commutative partner, and cobar reconstructs $\mathcal H_k$.
...
\begin{corollary}[Ordered \texorpdfstring{$\Eone$}{E1} bar and
continuous dual; \ClaimStatusConditional]
...
$\Eone$\nobreakdash-chiral algebra~$\mathcal{A}$ is again of
associative coalgebra type.  Set
\[
\mathcal C_{\mathcal A}^{\mathrm{bar,ord}}
\coloneqq \bar{B}^{\mathrm{ch}}(\mathcal{A}).
\]
The bar--cobar counit is
\[
 \Omega^{\mathrm{ch}}
 \mathcal C_{\mathcal A}^{\mathrm{bar,ord}}
 \xrightarrow{\sim}\mathcal A.
\]
When the continuous Verdier dual exists, it forms the associative
algebra
\[
 K_X^{\mathrm{ord}}(\mathcal A)
 :=\mathbb D_{\operatorname{Ran}}
 \mathcal C_{\mathcal A}^{\mathrm{bar,ord}}.
\]
A chosen pair equivalence
$\nu_{\mathcal A}\colon K_X^{\mathrm{ord}}(\mathcal A)
\xrightarrow{\sim}\mathcal A^!$ supplies the partner algebra.
For a quadratic presentation, the separate map
$q_{\mathcal A}\colon\mathcal A^{\mathrm i}	o
\mathcal C_{\mathcal A}^{\mathrm{bar,ord}}$ is the quadratic
recognition map.
\end{corollary}

\begin{proof}
Theorem~\ref{thm:e1-chiral-koszul-duality} gives the counit
\eqref{eq:e1-counit}.  Operadic self-duality places the bar output in
the associative coalgebra category.  Continuous Verdier duality forms
$K_X^{\mathrm{ord}}(\mathcal A)$, while the pair equivalence~$\nu$
is the additional family-specific datum.  The quadratic map~$q$
comes from the chosen presentation and its twisting morphism.
\end{proof}
...
\cite{KontsevichHMS,SeidelBook}; these geometric inputs complement the
...
\(\mathcal A^{\mathrm i}=C(s^{-1}V,s^{-2}R)\) is the coalgebra of a
chosen quadratic presentation,
\(K_X(\mathcal A)=\mathbb D B^{\mathrm{ord}}(\mathcal A)\) is the
finite-type Verdier algebra, and
\(\nu_{\mathcal A}\colon K_X(\mathcal A)\to\mathcal A^!\) selects a
Koszul partner.  The Hochschild cochain object
$Z^{\mathrm{der}}_{\mathrm{ch}}(\mathcal{A})=\ChirHoch^*(\mathcal A)$
is the cochain-level closed-sector actor; a separate open--closed
comparison identifies physical local operators with this cochain
object when the corresponding geometric package holds.
\end{remark}
...
bar coalgebra
$C_{\mathcal{A}}^{\mathrm{bar}} := \bar{B}^{\mathrm{ch}}(\mathcal{A})$
\textup{(}Theorem~\ref{thm:e1-chiral-koszul-duality},
Corollary~\ref{cor:e1-self-duality}\textup{)}. When continuous
Verdier duality exists, write
\[
K_X^{\mathrm{ord}}(\mathcal A)
:=\mathbb D_{\operatorname{Ran}}C_{\mathcal A}^{\mathrm{bar}}.
\]
Then the bar-cobar adjunction
restricts to an equivalence of derived categories between
\emph{complete} (pro-nilpotent) $\mathcal{A}$-modules and
\emph{conilpotent} $C_{\mathcal{A}}^{\mathrm{bar}}$-comodules:
\[
D(\mathrm{Mod}^{\Eone,\,\mathrm{compl}}_{\mathcal{A}})
\;\simeq\;
D(\mathrm{CoMod}^{\Eone,\,\mathrm{conil}}_{C_{\mathcal{A}}^{\mathrm{bar}}})
\]
given by the bar-cobar induction-restriction adjunction:
\begin{align*}
\mathrm{Ind}\colon \mathrm{Mod}^{\Eone,\,\mathrm{compl}}_{\mathcal{A}}
&\;\longrightarrow\;
\mathrm{CoMod}^{\Eone,\,\mathrm{conil}}_{C_{\mathcal{A}}^{\mathrm{bar}}}, &
\mathcal{M} &\;\longmapsto\;
\bar{B}^{\mathrm{ch}}(\mathcal{A}) \otimes_{\mathcal{A}} \mathcal{M},
\\
\mathrm{Res}\colon \mathrm{CoMod}^{\Eone,\,\mathrm{conil}}_{C_{\mathcal{A}}^{\mathrm{bar}}}
&\;\longrightarrow\;
\mathrm{Mod}^{\Eone,\,\mathrm{compl}}_{\mathcal{A}}, &
\mathcal{N} &\;\longmapsto\;
\Omega^{\mathrm{ch}}(C_{\mathcal{A}}^{\mathrm{bar}})
\otimes_{C_{\mathcal{A}}^{\mathrm{bar}}}
\mathcal{N}.
\end{align*}
Under this equivalence:
\begin{enumerate}[label=\textup{(\roman*)}]
\item Suppose a module~$L$ carries a twisting-compatible cofree
 comparison
 $\mathrm{Ind}(L)\xrightarrow{\sim}
 C_{\mathcal A}^{\mathrm{bar}}\otimes L_0$ for a finite lowest-weight
 object~$L_0$.  Then the bar image
 $\mathrm{Ind}(L)$ is a cofree, hence injective,
 $C_{\mathcal{A}}^{\mathrm{bar}}$-comodule. On finite-type dualized
 lanes, its graded dual is projective over
 $K_X^{\mathrm{ord}}(\mathcal A)$.
\item Derived extension groups are exchanged with derived morphisms on
 ...
 \operatorname{Hom}_{D(\mathrm{CoMod}^{\Eone,\,\mathrm{conil}}_{C_{\mathcal{A}}^{\mathrm{bar}}})}
 (\mathrm{Ind}(L), \mathrm{Ind}(M)[i]).
 \]
\item On a split finite quadratic lane where
$\mathcal A^{\mathrm i}$ satisfies $H_{\mathrm{CL}}$, character
formulas for $\mathcal A$-modules are computed by the resolution with
$\mathcal A^!_{\mathrm{quad}}=\mathbb D\mathcal A^{\mathrm i}$.
A grading-preserving comparison
$c_\mathcal A\colon\mathcal A^!_{\mathrm{quad}}\to\mathcal A^!$
transports the formula to the chosen partner.
\end{enumerate}
\end{theorem}
...
using \emph{ordered} configurations, retaining the full ordered
$n$-tuple at each arity, so
...
\emph{Property (i): cofree transport under the stated comparison.}
The additional equivalence
$\mathrm{Ind}(L)\simeq
C_{\mathcal A}^{\mathrm{bar}}\otimes L_0$ identifies the image with a
cofree comodule.  Cofree comodules are injective, and finite continuous
duality sends this object to a projective
$K_X^{\mathrm{ord}}(\mathcal A)$-module.
...
On the split finite quadratic lane, the usual dual-algebra Koszul
resolution may be used.  The Koszul resolution of any
$\mathcal{A}$-module $\mathcal{M}$ by free
$\mathcal{A}$-modules has terms
$\mathcal{P}_n = \mathcal{A} \otimes
(\mathcal{A}^!_{\mathrm{quad}})_n
\otimes \mathcal{M}$
...
h_{\mathcal{A}^!_{\mathrm{quad}}}(-t) = 1$.
The comparison~$c_{\mathcal A}$ transports the formula to the chosen
partner whenever it preserves the grading.
\end{proof}
...
$\cA^{\mathrm i}=C_X(s^{-1}V,s^{-2}R)$ satisfies
$H_{\mathrm{CL}}$ and admits the finite Verdier dual algebra
$\cA^!_{\mathrm{quad}}=\mathbb D_{\operatorname{Ran}}
\cA^{\mathrm i}$. Then the Hilbert series of the algebra and of the
quadratic dual satisfy the classical Koszul
identity
\begin{equation}\label{eq:koszul-character-identity}
 h_\cA(t)\,h_{\cA^!_{\mathrm{quad}}}(-t)=1.
\end{equation}
A chosen partner map
$c_\cA\colon\cA^!_{\mathrm{quad}}\to\cA^!$ transports the identity
to $h_{\cA^!}$ when it is a grading-preserving quasi-isomorphism.
The series in~\eqref{eq:koszul-character-identity} is the graded
Hilbert series of the quadratic dual; quasi-primary characters form a
separate invariant of the original algebra.
\end{proposition}
...
series is $h_{\cA^!_{\mathrm{quad}}}(-t)$. This proves
\eqref{eq:koszul-character-identity}. For PBW non-quadratic or
completed families, the same expression becomes a finite-window Euler
identity under the Mittag--Leffler hypotheses.
\end{proof}

\begin{proposition}[Bar cohomology and quasi-primary count;
\ClaimStatusProvedHere]
...
The bar function
$b_\cA(h):=\dim H^1(\barBgeom(\cA))_h$ is a derived invariant of the
full bar coalgebra.  For a chosen quadratic presentation satisfying
$H_{\mathrm{CL}}$, it equals
$\dim H^1(\cA^{\mathrm i})_h$ through $H^\bullet(q_\cA)$.
The quasi-primary function
$d_{\mathrm{qp}}(h) = p_{\geq 2}(h) - p_{\geq 2}(h-1)$
counts quasi-primary fields of the \emph{original}
algebra~$\cA$.  For the universal Virasoro algebra,
\[
 b_{\mathrm{Vir}}(2)=1,
 \qquad b_{\mathrm{Vir}}(4)=0,
 \qquad d_{\mathrm{qp}}(2)=d_{\mathrm{qp}}(4)=1.
\]
Hence the two functions already separate at weight~$4$.
\end{proposition}

\begin{proof}
For the Virasoro algebra, $\dim H^1(\barBgeom(\mathrm{Vir}))_h$
is computed by the degree-one part of the PBW bar complex and is
concentrated at weight~$2$:
$\dim H^1(\barBgeom(\mathrm{Vir}))_2 = 1$ (the single
generator~$T$) and $\dim H^1(\barBgeom(\mathrm{Vir}))_h = 0$ for
$h \neq 2$, independent of~$c$
  (Theorem~\ref{thm:virasoro-chiral-koszul}).
The quasi-primary count
$d_{\mathrm{qp}}(h) = p_{\geq 2}(h) - p_{\geq 2}(h{-}1)$
grows as a partition function, with
$d_{\mathrm{qp}}(2) = 1$, $d_{\mathrm{qp}}(4) = 1$,
$d_{\mathrm{qp}}(6) = 2$, $d_{\mathrm{qp}}(8) = 3$. The two
sequences agree at weight~$2$ but diverge at weight~$4$:
$\dim H^1(\barBgeom(\mathrm{Vir}))_4 = 0$ while
$d_{\mathrm{qp}}(4) = 1$ (the composite quasi-primary $\Lambda$).
The sequences therefore differ at the first nontrivial weight
above the generator.
\end{proof}
...
after forming the finite post-Verdier algebra
$K_X(\cA)=\mathbb D_{\operatorname{Ran}}B_X(\cA)$, choosing the pair
map $\nu_\cA\colon K_X(\cA)\to\cA^!$, and inserting the family-specific conductor
normalization
...
\item \emph{Bezrukavnikov--Finkelberg--Mirkovi\'{c}.}
 The BFM theorem~\cite{BFM04} constructs a derived
...
\begin{theorem}[Structure exchange on the finite quadratic lane;
\ClaimStatusConditional]\label{thm:structure-exchange}
\textup{[Type signature: Open quadrant; split finite quadratic
presentation; Beilinson levels~$1\leftrightarrow2$; hypothesis package
$H_{\mathrm{CL}}+H_{\mathbb D}^{\mathrm{bar}}+c_{\mathcal A_1}$.]}
Let
$\mathcal A_1=T_{\mathrm{ch}}(V)/(R)$ and
$\mathcal A_1^{\mathrm i}=C_X(s^{-1}V,s^{-2}R)$.  Assume
$H_{\mathrm{CL}}$ for
$q_{\mathcal A_1}\colon\mathcal A_1^{\mathrm i}	o
B_X(\mathcal A_1)$ and a grading-preserving pair equivalence
\[
 c_{\mathcal A_1}\colon
 \mathcal A^!_{1,\mathrm{quad}}
 :=\mathbb D_{\operatorname{Ran}}\mathcal A_1^{\mathrm i}
 \xrightarrow{\sim}\mathcal A_2
\]
satisfying
$\nu_{\mathcal A_1}=c_{\mathcal A_1}\circ
\mathbb D(q_{\mathcal A_1})$. Then:
\begin{enumerate}
\item \emph{Generators and relations.}
With the suspension conventions of
Definition~\ref{def:suspension},
\[
 \mathcal A_2\simeq
 T_{\mathrm{ch}}(s\mathbb D_XV)/(R^\perp).
\]
Thus the dual generator object is $s\mathbb D_XV$ and the dual
relation object is the residue annihilator~$R^\perp$.

\item \emph{Twisting and coproduct.}
The multiplication of $\mathcal A_1$ enters the quadratic twisting
morphism
$\kappa_{\mathcal A_1}\colon\mathcal A_1^{\mathrm i}	o
\mathcal A_1$ and the cobar differential on
$\Omega_X(\mathcal A_1^{\mathrm i})$.  The deconcatenation coproduct
of $\mathcal A_1^{\mathrm i}$ dualizes to the multiplication of
$\mathcal A^!_{1,\mathrm{quad}}$.

\item \emph{Koszul resolution.}
The unit has the resolution
\[
 \cdots\longrightarrow
 \mathcal A_1\otimes(\mathcal A_1^{\mathrm i})_2
 \longrightarrow
 \mathcal A_1\otimes(\mathcal A_1^{\mathrm i})_1
 \longrightarrow\mathcal A_1\longrightarrow\mathbf1,
\]
whose differential is induced by~$\kappa_{\mathcal A_1}$.
\end{enumerate}
\end{theorem}

\begin{proof}
The quadratic presentation coalgebra is the subcoalgebra
\[
 \mathcal A_1^{\mathrm i}
 =C_X(s^{-1}V,s^{-2}R)
 \subset T^c_{\mathrm{ch}}(s^{-1}V).
\]
Finite Verdier duality turns this subcoalgebra into the quadratic
quotient
\[
 \mathcal A^!_{1,\mathrm{quad}}
 \simeq T_{\mathrm{ch}}(s\mathbb D_XV)/(R^\perp).
\]
The map $c_{\mathcal A_1}$ transports this presentation to
$\mathcal A_2$.

The universal property of the twisting morphism gives the cobar
differential on $\Omega_X(\mathcal A_1^{\mathrm i})$ from
$\kappa_{\mathcal A_1}$ and the coproduct.  Its counit has target
$\mathcal A_1$.  The package $H_{\mathrm{CL}}$ makes the twisted
tensor product
$\mathcal A_1\otimes_{\kappa_{\mathcal A_1}}
\mathcal A_1^{\mathrm i}$ acyclic, which is precisely the displayed
Koszul resolution.  Verdier duality converts the coproduct on
$\mathcal A_1^{\mathrm i}$ into the product on
$\mathcal A^!_{1,\mathrm{quad}}$.
\end{proof}

\subsection{\texorpdfstring{$A_\infty$ operations exchange}{A-infinity operations exchange}}

\begin{theorem}[\texorpdfstring{$A_\infty$}{A-infinity} operations under Verdier pairing;
\ClaimStatusConditional]
\label{thm:ainfty-duality-exchange}
Let $\mathcal A_1=T_{\mathrm{ch}}(V)/(R)$ satisfy the hypotheses of
Theorem~\ref{thm:structure-exchange}.  Assume a filtered deformation
retraction of $B_X(\mathcal A_1)$ onto
$\mathcal A_1^{\mathrm i}$, a cyclic homotopy transfer datum, and a
perfect chain-level Verdier pairing compatible with the pair
equivalence
$c_{\mathcal A_1}\colon\mathcal A^!_{1,\mathrm{quad}}	o
\mathcal A_2$. Then:
\begin{itemize}
...
\item after finite duality, these cooperations become higher
 operations on $\mathcal{A}^!_{1,\mathrm{quad}}$ and hence on
 $\mathcal A_2$ through~$c_{\mathcal A_1}$;
\item Massey products on $\mathcal{A}_1$ pair with the corresponding
 comassey classes on $\mathcal{A}_1^{\mathrm i}$ whenever the
 transferred classes are defined.
\end{itemize}
\end{theorem}
...
the presentation coalgebra. Perfectness identifies each operation
with its transpose cooperation. Applying finite continuous duality turns the
cooperation on $\mathcal{A}_1^{\mathrm i}$ into an operation on the dual
algebra~$\mathcal{A}^!_{1,\mathrm{quad}}$, and
$c_{\mathcal A_1}$ transports it to~$\mathcal A_2$.  Cyclic homotopy
transfer descends the perfect chain pairing to the defined Massey and
comassey classes, which gives the final assertion.
\end{proof}
...
\mathsf H_i^{\mathrm{bar}}:=H^\bullet(\cC_i),\qquad
K_X^{\mathrm{curv}}(\cA_i):=
\mathbb D_{\operatorname{Ran}}\cC_i,
\]
so $\cC_i$ is the completed curved bar coalgebra,
$\mathsf H_i^{\mathrm{bar}}$ is its cohomology, and
$K_X^{\mathrm{curv}}(\cA_i)$ is its Verdier-dual algebra. The Hochschild
...
\item the curved bar coalgebras $\cC_i$ are degreewise finite-type
 Verdier dualizable on the chosen lane, and the candidate pair maps
 \[
K_X^{\mathrm{curv}}(\cA_1)\longrightarrow \cA_2,
 \qquad
K_X^{\mathrm{curv}}(\cA_2)\longrightarrow \cA_1
\]
...
\textup{(3)} are equivalences
$K_X^{\mathrm{curv}}(\cA_1)\simeq\cA_2$ and
$K_X^{\mathrm{curv}}(\cA_2)\simeq\cA_1$ on this lane. The scalar
...
\]
\[
K^\kappa_{\mathrm{fam}}=25/3\quad\text{for Bershadsky--Polyakov
(conditional on the ratio lane)},
\qquad
...
let this comparison pass to the completed filtered object with
vanishing $\varprojlim^1$ correction. The result at this point is a
statement about the coalgebras~$\cC_i$ and their bar cohomology
$\mathsf H_i^{\mathrm{bar}}$.  Condition~(3) forms the Verdier
algebras $K_X^{\mathrm{curv}}(\cA_i)$ and identifies them with the
partner algebras.  The Hochschild objects
$Z^{\mathrm{der}}_{\mathrm{ch}}(\cA_i)$ remain a separate construction.
...
The precise open mathematical conditions are:
\begin{enumerate}[label=\textup{(\alph*)}]
...
\emph{Note}: The bar-cohomology coalgebra is
$\mathsf H^{\mathrm{bar}}(\widehat{\mathfrak g}_k)
=H^*(\barBgeom(\widehat{\mathfrak{g}}_k))$.  A chosen quadratic
presentation supplies
$\widehat{\mathfrak g}_k^{\mathrm i}$ and
$H^\bullet(q_{\widehat{\mathfrak g}_k})$ identifies its cohomology
with $\mathsf H^{\mathrm{bar}}$ under $H_{\mathrm{CL}}$.  The
finite-type Verdier algebra is
$K_X(\widehat{\mathfrak g}_k)=\mathbb D_{\Ran}
\barB_X(\widehat{\mathfrak g}_k)$, and the chosen pair map has target
$\widehat{\mathfrak g}_k^!$.  Its modular characteristic satisfies
...
$\mathcal{H}_k^!
\simeq(\mathrm{Sym}^{\mathrm{ch}}(V^*[1]),m_0=-k\omega)$
(cf.\ \S\ref{sec:heisenberg-koszul}); the curvature encoding the
central extension is essential, and the uncurved symmetric algebra is
only the associated-graded or \(k=0\) shadow.
\end{remark}
...
analysis.
\item PBW non-quadratic algebras use the completed bar coalgebra and
its cohomology; a residue-dual candidate enters through
$H_{\mathrm{res}}$ as in
Section~\ref{sec:three-stage-construction}.
\end{itemize}
\end{remark}
...
Let $\cA_i=T_X(V_i)/(R_i)$ be finite-type quadratic chiral algebras on
a smooth curve~$X$, with presentation coalgebras
$\cA_i^{\mathrm i}=C_X(s^{-1}V_i,s^{-2}R_i)$ satisfying
$H_{\mathrm{CL}}$.  Assume chosen partner comparisons
$c_i\colon\cA^!_{i,\mathrm{quad}}\xrightarrow{\sim}\cA_i^!$.
...
quadratic presentation coalgebra satisfies
...
\[
(\cA_1 \otimes^{\mathrm{ch}} \cA_2)^!_{\mathrm{quad}}
\simeq
\cA^!_{1,\mathrm{quad}}\otimes^{\mathrm{ch}}
\cA^!_{2,\mathrm{quad}}.
\]
The tensor product $c_1\otimes c_2$ identifies this algebra with the
chosen partner $\cA_1^!\otimes^{\mathrm{ch}}\cA_2^!$. In particular, for
...
statement, and $c_1\otimes c_2$ gives the partner comparison. The
Kac--Moody specialization uses the modular characteristic equality
from Theorem~\ref{thm:universal-kac-moody-koszul}; the affine algebra
at shifted level supplies the matching scalar datum.
\end{proof}
...
$K^\kappa_{\mathrm{fam}}\in \{0, 8, 13, 250/3, 25/3\}$.  The comparison
...
coproduct, and the completed pairing.  The Gritsenko--Nikulin
denominator factor supplies a target Euler-character and coefficient
check.  Suppose the finite-window Koszul comparison and exact cofinal
system assemble a family datum
$H_H(\mathbf H_{\Delta_5};S_{\Delta_5})$.  Theorem~H then gives
\[
 \operatorname{Supp}\mathrm{ChirHoch}^\bullet
 ( \mathbf H_{\Delta_5})\subseteq S_{\Delta_5}.
\]
Under the separate degree-two pairing and the
Etingof--Kazhdan/Gritsenko--Nikulin comparison, the line
$\mathbb C\cdot\Delta_5$ is the recognized degree-$2$ target class
