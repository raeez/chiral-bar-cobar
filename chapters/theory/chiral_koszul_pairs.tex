...
In Chapter~\ref{ch:heisenberg-frame}, we identified the Koszul dual of the Heisenberg algebra as $\mathrm{Sym}^{\mathrm{ch}}(V^*)$ by reading off the bar cohomology.  This chapter develops the general theory of Koszul pairs: when two chiral algebras are related by bar-cobar duality, and how the Koszul property controls spectral sequence collapse.
...
constructed the bar and cobar functors and proved that Verdier
duality on $\operatorname{Ran}(X)$ intertwines them.  That
construction takes as input a single chiral algebra $\cA$; the
present chapter identifies when a \emph{pair} $(\cA, \cA^!)$
forms a Koszul pair --- the condition under which the bar-cobar
adjunction becomes an equivalence.  The Koszul property (diagonal
Ext vanishing) is the genus-$0$ prerequisite for modular
Koszulity
(Definition~\ref{def:modular-koszul-chiral}), and the
classification of Koszul pairs structures the entire
landscape of examples in Part~II.

The operadic foundations go back to
Priddy~\cite{Priddy70} ($H_*(B(\mathrm{Com})) \cong
\mathrm{Lie}^*[-1]$), Quillen's equivalence between
dg~Lie algebras and cocommutative coalgebras, and
Ginzburg--Kapranov's framework of Koszul operads:
a quadratic operad $\mathcal{P}$ is Koszul when
$\mathcal{P}^! \hookrightarrow B(\mathcal{P})$ is a
quasi-isomorphism, yielding
$\mathcal{P} \xrightarrow{\sim} \Omega B(\mathcal{P})$.
The BRST ghost-antighost system (Becchi--Rouet--Stora--Tyutin)
is a physical manifestation of this duality.
The present chapter extends this circle of ideas to the chiral
setting.
...
The pair $(\mathcal{H}_k, \mathrm{Sym}^{\mathrm{ch}}(V^*))$ is the prototype (\S\ref{sec:frame-koszul-dual}): the bar of one is the dual coalgebra of the other, and the cobar reconstructs the partner.

Two chiral algebras $(\mathcal{A}_1, \mathcal{A}_2)$ on a curve $X$ form a \emph{chiral Koszul pair} if they satisfy the following conditions (the equivalence of Versions I and II is immediate: set $\mathcal{C}_i = \mathcal{A}_i^!$ in Version~II to recover Version~I; conversely, Version~I implies acyclicity of the Koszul complexes in Version~II by Theorem~\ref{thm:bar-cobar-isomorphism-main}):
...
Despite the difficulties listed above, a systematic method is available
for verifying that a chiral algebra is Koszul.  The strategy reduces
...
We now apply this criterion to the main examples.
...
\begin{theorem}[Virasoro algebra is chiral Koszul; \ClaimStatusProvedHere]
