...
\begin{remark}[$W(2)$ at $c = -2$: Koszulness via Kazhdan filtration]
...
The $W(2)$ algebra at $c = -2$ is chirally Koszul.
The proof uses the Kazhdan filtration: $W(2)$ embeds into
the lattice VOA $V_{2\mathbb{Z}}$ as the momentum-zero sector,
and the associated graded under the Kazhdan filtration is
polynomial, so the PBW criterion
\textup{(}Proposition~\ref{prop:pbw-universality}\textup{)} applies.
Note that $W(2)$ is the $\mathbb{Z}/2$-orbifold of the
symplectic fermion algebra~$\mathrm{SF}$, \emph{not}
$\mathrm{SF}$~itself; the ``null vector at weight~$15$'' reported
in earlier analyses is a Virasoro Kac determinant zero, not a
$W(2)$ null vector.
\end{remark}
...
reduced to a single gap, as follows.
Beyond these twelve conditions, a 13th characterization
(D-module purity, Proposition~\ref{prop:d-module-purity-km}) is
partially established, and a 14th---Sklyanin--Poisson rigidity
