...
The MC element $\Theta_\cA \in \MC(\gAmod)$ exists unconditionally
for every chiral algebra on a curve, but its shadow Postnikov tower
$\Theta_\cA^{\leq r}$ has polynomial solutions (finite shadow depth)
only when $\cA$ satisfies a Koszul condition.  This chapter
establishes the chiral Koszul framework: the conditions under which
the bar complex $\barB(\cA)$ has cohomology concentrated in bar
degree~$1$, the twisting morphism is acyclic, and the MC element
admits finite-order truncations.  The twelve equivalent
characterizations of chiral Koszulness
(Theorem~\ref{thm:koszul-equivalences-meta}) are consequences of
this framework.
...
\item \emph{Nilradical obstruction.}
  ...
  data alone do not determine chirally Koszulness of~$L_k(\fg)$.
\end{enumerate}
\end{proposition}
...
If $\ChirHoch^*(\cA)$ is polynomial with generators concentrated
in degrees $\{0,1,2\}$, then the bar resolution has
cohomological dimension~$\leq 2$: the generators of the
polynomial algebra correspond to bar cogenerators, and
$\mathrm{Ext}^n_{\mathrm{ChirAlg}}(\cA,\cA) = 0$ for $n > 2$
by the polynomial generation bound.  The counit
$\Omega(\barBgeom(\cA))\to\cA$ is then a quasi-isomorphism
by the spectral sequence comparison
(Theorem~\ref{thm:pbw-koszulness-criterion}).
...
computationally for a landscape of 15 algebras
...
Cross-family consistency checks (AP10): $\kappa$ additivity,
complementarity consistency, and shadow depth independence
from Koszulness (AP14: shadow classes G, L, C, M each contain
Koszul algebras).
\end{remark}

\begin{table}[ht]
\centering
\caption{Koszulness landscape: 15 algebras}
...
all four classes contain Koszul algebras (AP14:
shadow depth $\neq$ Koszulness).
...
\item Curvature $m_0 = \kappa(\Walg_N)\cdot\omega_g$ where
  $\kappa = c(H_N - 1)$; vanishes at $c = 0$, not at
  $c_{\mathrm{minimal}}$.
\end{enumerate}
\end{construction}
...
\item \emph{Homological algebra:} spectral sequences and the proved $\Eone$ module-duality lane; broader ordinary-derived Ext--Tor packages remain frontier
