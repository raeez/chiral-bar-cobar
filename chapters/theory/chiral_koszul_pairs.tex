...
Koszulness is the condition under which perturbation theory
terminates at each arity: the bar spectral sequence collapses at
$E_2$, the transferred $A_\infty$ operations $m_k$ vanish for
$k \geq 3$, and every graph sum truncates to a finite computation.
The shadow depth $r_{\max}$ counts the independent counterterms.
The four complexity classes G/L/C/M correspond to free theories (no
counterterms), gauge theories (one cubic counterterm), contact
theories (one quartic), and gravitational theories (infinitely many).

The MC element $\Theta_\cA \in \MC(\gAmod)$ exists unconditionally
for every chiral algebra on a curve, but its shadow obstruction tower
...
proved, all consequences of this framework.
...
(Theorem~\ref{thm:bar-cobar-isomorphism-main}); see
Costello--Francis--Gwilliam~\cite{CFG25} for the general chiral
Koszul duality framework.  The present chapter
...
\label{conj:ext-diagonal-vanishing}% NOTE: retains conj: prefix for backward compatibility; claim is ProvedHere
...
$S_4 = -(5c+22)/(10c)$ and $\Delta = -2(5c+22)/5 \neq 0$
...
$r^{\mathrm{cl}}(z) = \Omega/z$ of an affine algebra
...
$r$-matrix $r^{\mathrm{cl}}(z) = \Omega/z$ has vanishing second
Poisson cohomology:
\[
