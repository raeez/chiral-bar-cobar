...
\item[\textup{(vii)}] Factorization homology
 ...
 but is conditional on cross-channel cancellation in the
...
The Yangian $Y(\mathfrak{g})$ and affine Yangian $Y_z^{\hbar}(\widehat{\mathfrak{g}})$
...
The affine Yangian $Y_z^\hbar(\widehat{\mathfrak{gl}}_n)$ has generators $\{T^a_{(r)}\}$ for $a = 1, \ldots, n^2$ and $r \geq 0$.
...
\[\widehat{\widehat{\bar{B}}}(Y_z^\hbar) = \varprojlim_n \varprojlim_m \bar{B}(Y_z^\hbar)/I^n \cdot \hbar^m\]
\end{example}
