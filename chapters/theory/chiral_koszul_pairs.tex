...
\begin{remark}[Open-frontier closures]
...
therefore heals the gap whenever the missing step is the
...
coalgebra identities. Theorem~\ref{thm:bar-computes-koszul-dual-complete}
...
is the Hochschild closed-string sector.
\end{remark}
...
Filtered algebras $(\cA_1, \cA_2)$ with modular curvatures
$\kappa_1,\kappa_2$ form a curved Koszul pair if:
\begin{enumerate}
\item Associated graded $(\mathrm{gr}\,\cA_1, \mathrm{gr}\,\cA_2)$ are classical Koszul dual
\item Curvatures satisfy the family conductor identity
\[
\kappa_1+\kappa_2=K^\kappa_{\mathrm{fam}}
\]
under the Koszul pairing
\item The spectral sequence associated to the filtration degenerates at $E_2$
\end{enumerate}
For the canonical families,
\[
K^\kappa_{\mathrm{fam}}=
...
\qquad
K^\kappa_{\mathrm{fam}}=8\quad\text{for the Mukai-enhanced K3 row.}
\]
\end{theorem}

\begin{proof}
The filtered bar complex has associated graded
$\mathrm{gr}\,\B(\cA_1)=\B(\mathrm{gr}\,\cA_1)$. Condition~(1) gives the
classical Koszul comparison on the $E_1$ page. In a curved
$A_\infty$ bar-cobar complex, the curvature term is not universally
anti-invariant; it is shifted by the conductor of the family. The
identity $\kappa_1+\kappa_2=K^\kappa_{\mathrm{fam}}$ is precisely the
compatibility condition for the curved twisting morphism in the
Positselski sense~\cite{Positselski11}. The displayed constants are the
canonical values from the landscape census:
$\kappa(V_k(\fg))=\dim(\fg)(k+h^\vee)/(2h^\vee)$,
$\kappa(\mathrm{Vir}_c)=c/2$, and
$\kappa(\mathcal W_N)=c(H_N-1)$, together with the recorded
Bershadsky--Polyakov and Mukai rows. Condition~(3) lifts the $E_1$
comparison to the filtered curved bar-cobar equivalence by
...
\begin{remark}[Theorems used by later chapters]\label{rem:ckp-dependency-digest}
...
not part of the theorem surface proved here.
...
\]
with $c_+ = 4$ from the positive signature of the rank-$24$ Mukai pairing on $\Lambda_{\mathrm{Muk}} = \mathrm{II}_{4,20}$. This is the $\mathcal{B}$-family entry to the Theorem~C conductor bucket set $K^\kappa_{\mathrm{fam}}\in \{0, 8, 13, 250/3, 98/3\}$. On this row the identity $\hbar^2 \cdot K^{\kappa_{\mathrm{ch}}} = -1$ gives the Lusztig $u_\zeta$-specialisation at $\ell = 8$, matching the order of the monodromy of $\mathcal{L}^{\Delta_5}$ around the Humbert divisor $H_1 \subset \overline{\mathcal{A}_2}$ by Bruinier's Heegner Chern-class reciprocity~\cite[Proposition~5.1]{Bruinier2002}.
\end{remark}
...
on $\Lambda_{\mathrm{Muk}} = \mathrm{II}_{4,20}$; this is the
Vol.~III K3 chiral-bialgebra construction of
...
Bruinier~\cite{Bruinier2002} and Mukai~\cite{Mukai1987} supply the
Heegner Chern-class and Mukai-lattice inputs.
\end{remark}
...
line from the Etingof--Kazhdan quantum vertex-operator construction
\cite{EtingofKazhdan2007PartV}.
\end{remark}
