...
Which chiral algebras admit Koszul duality?  The question is not
rhetorical.  Every chiral algebra $\cA$ on a smooth curve $X$
produces a bar complex $\barB(\cA)$ and an MC element
$\Theta_\cA \in \MC(\gAmod)$, but the round trip
$\Omega(\barB(\cA)) \to \cA$ is an equivalence only for
algebras satisfying a homological acyclicity condition on the
bar complex, and a direct diagnostic that can be checked against
the standard families without running the cobar functor is
missing from the existing literature.  The Francis--Gaitsgory
adjunction $\chirLie \dashv \chirCom$ on $\operatorname{Ran}(X)$
settles the Koszul duality of the Lie--Com pair at the operadic
level, but it does not by itself tell anyone whether the
Virasoro algebra, a $\cW$-algebra, or a Yangian is chirally
Koszul: those algebras are not quadratic, their bar complexes
carry genus corrections that the operadic adjunction does not
see, and their shadow obstruction towers have depths ranging
from two to infinity.  A diagnostic is needed that is invariant
under the choice of presentation, stable under base change along
the moduli of curves, and that recognises the three phenomena
absent at genus zero: the modular characteristic $\kappa(\cA)$,
Lagrangian complementarity between $\cA$ and $\cA^!$, and the
critical discriminant $\Delta = 8\kappa S_4$ whose vanishing
governs the shadow tower.

Twelve equivalent conditions make up such a diagnostic, and
Theorem~\ref{thm:koszul-equivalences-meta} assembles them.  Ten
are unconditional; one is conditional on perfectness; one is a
one-directional implication coming from D-module purity.  Each
characterisation isolates a different facet of the same
phenomenon: $E_2$-collapse of the bar spectral sequence,
...
$\Theta_\cA^{\leq r}$, and so on.  The four complexity classes
G/L/C/M correspond to shadow depths $\{2, 3, 4, \infty\}$, not
to failure of Koszulness: all standard families are chirally
Koszul, and the classes record instead the arity at which the
shadow obstruction tower first becomes nontrivial.

The passage from classical Koszul duality
(Chapter~\ref{ch:algebraic-foundations}) to its chiral counterpart is
not a routine generalization but a shift from algebraic composition
...
factorization product $\cA^{\boxtimes n}$ over the Fulton--MacPherson
compactification $\FM_n(X)$; the differential is no longer a
combinatorial sum of face maps but a sum of residues at boundary
strata, each governed by the OPE singularities of~$\cA$.

The chiral extension preserves the
...
$\Ran(X)$ provides the general chiral Koszul duality framework and
the operadic backbone of our Theorem~A
...
controls termination of the shadow obstruction tower
...
differential ceases to square to zero, builds on these structures.
...
\item[\textup{(vii)}] Factorization homology
  ...
  for $g = 0$ \textup{(UNIFORM-WEIGHT, AP32)}; for $g \geq 1$
  the analogous concentration holds on the uniform-weight lane
  but is conditional on AP32 cross-channel cancellation in the
  multi-weight case (see
  Remark~\ref{rem:fh-vii-uniform-weight-scope} below).
...
\smallskip\noindent
\textup{(viii)}$\Rightarrow$\textup{(v)}:
If $\ChirHoch^*(\cA)$ is a polynomial algebra with generators
concentrated in degrees~$\{0, 1, 2\}$, then the bar resolution
computes $\operatorname{Ext}^{p,q}_\cA(\omega_X, \omega_X)$ by the
Beilinson--Drinfeld comparison, and the polynomial structure forces
diagonal concentration: the generators in degree~$0$ contribute
to~$\operatorname{Ext}^{0,0}$, those in degree~$1$
to~$\operatorname{Ext}^{1,1}$, and those in degree~$2$
to~$\operatorname{Ext}^{2,2}$; all products of these generators remain
on the diagonal $p = q$.  This is condition~(iv).  By the already
established (iv)$\Rightarrow$(i)$\Rightarrow$(v), the counit is a
quasi-isomorphism.
...
\textup{(}UNIFORM-WEIGHT, scoped at $g = 0$; cf.\
...
holds on the uniform-weight lane (AP32); in the multi-weight case
...
\begin{remark}[Scope of factorization-homology concentration; AP32]
\label{rem:fh-vii-uniform-weight-scope}
Condition~\textup{(vii)} is asserted at $g = 0$ unconditionally and
at $g \geq 1$ only on the uniform-weight lane (AP32).  The
...
recovered as a Koszulness $+$ uniform-weight conjunction (AP32).
\end{remark}
...
\end{itemize}
This ordering refines the Koszulness programme: all four classes are chirally Koszul (bar $E_2$-collapse), but they differ in Swiss-cheese formality depth (AP14).
\end{remark}
...
\medskip\noindent
\emph{Mechanism.}
For class~$G$\textup{:} the bracket $[a_m, a_n] = km\,\delta_{m+n,0}$
is central, so nested brackets
$[[a_m, a_n], a_p] = 0$ and all higher tree sums vanish.
For class~$L$\textup{:} the OPE has at most a double pole; the
...
\begin{proof}
Class~$G$: centrality of the Heisenberg bracket kills all nested
compositions; $m_k^{\mathrm{SC}} = 0$ for $k \geq 3$.
...
the Heisenberg bracket $[a_m, a_n] = km\,\delta_{m+n,0}$ is
central, so all nested brackets vanish and
$m_k^{\mathrm{SC}} = 0$ for $k \geq 3$.

For the converse, suppose~$\cA$ is SC-formal.
The cubic shadow $S_3$ is the normalized projection of the
trilinear form
\[
...
\[
  ...
  C = 0.
\]
The last step is the one that kills.  Every chiral algebra in the
standard landscape carries a nondegenerate invariant bilinear
form~$\kappa$ (this is a consequence of the self-duality or
Lagrangian complementarity of the landscape; see
Theorem~\ref{thm:modular-characteristic}).  Nondegeneracy means
$\kappa$ is a perfect pairing: if $C(x,y,z) = \kappa(x,[y,z]) = 0$
for all~$x$, then $[y,z] = 0$ for all $y,z$.  The bracket is
identically zero; the current algebra is abelian.  An abelian
current algebra at level~$k$ is Heisenberg, which is class~$G$.

The converse direction is a unique-survivor argument.  Four
classes enter; one constraint eliminates three.  The
nondegeneracy of~$\kappa$ converts a vanishing condition on
shadows ($S_3 = 0$) into a structural condition on the bracket
($[-,-] = 0$).  Classes~$L$, $C$, and~$M$ all have nonzero
brackets; SC-formality is incompatible with each.  The only
algebra whose bracket is already zero is Heisenberg.
\end{proof}
...
(AP62): it depends on more than $\dim(\fg)$ or Euler
...
The ten unconditional equivalences have been verified
...
Proposition~\ref{prop:shadow-formality-low-arity} identifies
the genus-$0$ obstruction classes with the transferred $L_\infty$
brackets $\ell_{r+1}^{(0)}$ at arities $r = 2,3,4$; at each
higher arity the same inductive mechanism applies (the
obstruction at arity $r{+}1$ is the next transferred bracket
evaluated on the truncated MC element), establishing the
identification at all finite arities.
\end{proof}
...
ten unconditional Koszul equivalences lose force simultaneously.
...
\emph{Why the ten equivalences break.}
Each of the ten unconditional equivalences~(i)--(x) in
...
it the equivalence chain.  The 11th equivalence (Lagrangian,
conditional on perfectness) and the 12th (D-module purity,
one-directional) fail \emph{a fortiori}, since both rely on
...
\item The shadow depth $r_{\max}(\cA)$ equals the bootstrap
  ...
  crossing equation
  $D\Theta + \tfrac{1}{2}[\Theta,\Theta] = 0$ at arity
  $n = 4$ is satisfied.
...
special-case coincidence (AP3): it holds when the algebra is
...
\emph{Note} (AP33 caution): The Koszul dual
