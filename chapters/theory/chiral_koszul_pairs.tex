...
Verdier duality on $\Ran(X)$, connecting $\barB_X(\cA)$ with
$\barB_X(\cA^!)$; (c)~produces Lagrangian complementarity
...
arity at which the shadow obstruction tower first becomes
...
complementarity reduces to the classical bar-cobar adjunction,
$\Delta$ is vacuous.  The present chapter therefore recovers
classical Koszul duality as a special case and simultaneously
provides the genus-zero input from which the full modular
theory is built.  The standard families exemplify the four
...
intrinsically non-formal).  The genus-zero arity-two projection
...
connects $\barB_X(\cA)$ with $\barB_X(\cA^!)$ completes the
