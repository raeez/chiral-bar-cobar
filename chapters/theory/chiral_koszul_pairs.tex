...
We verify the \emph{Mittag-Leffler condition} for the inverse system $\{\bar{B}/I^n\}$: for each fixed bar degree~$k$, the images $\mathrm{im}(\bar{B}_k/I^{n+m} \to \bar{B}_k/I^n)$ stabilize for $m \gg 0$.  This holds because the polynomial growth bound gives $\dim(\bar{B}_k \cap I^n / \bar{B}_k \cap I^{n+1}) \leq P(n,k)$ for a polynomial $P$, and the finite-dimensionality of $\bar{B}_k/I^n$ (from finite generation over~$\cD_X$) implies the descending chain of images stabilizes in finitely many steps.  The Mittag-Leffler condition then ensures $\varprojlim^1 = 0$ and the inverse limit is exact.
...
\cite[Proposition~5.5.3.6]{Lur17} and \cite[§1.1]{HA}.
...
\begin{conjecture}[Extended bc-$\beta\gamma$ vs two fermions; \ClaimStatusConjectured]
