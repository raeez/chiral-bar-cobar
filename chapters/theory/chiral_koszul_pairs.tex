...
The Koszul property is proved at \textbf{M-level} (explicit bar-cobar models, PBW spectral sequences, Hilbert series).  The main equivalence theorem (Theorem~\ref{thm:bar-cobar-isomorphism-main}) is stated at \textbf{H-level} (adjunction in factorization $\infty$-categories) and proved at M-level (Step~B).  Spectral data, Hilbert series, and the Koszul dual identification tables are \textbf{S-level} shadows.
\end{remark}
...
Symmetric argument applied to the unit, using
Lemma~\ref{lem:filtered-comparison} with the bar-degree
filtration.
...
(Contributing to Conjecture~\ref{conj:master-infinite-generator}.)
\end{remark}
