...
genus~$0$.  This section develops nine equivalent ways to
detect this formality---four proved, five conjectural---each
...
\begin{conjecture}[Converse: chiral Koszulness implies formality;
\ClaimStatusConjectured]
\label{conj:ainfty-koszul-characterization}

The converse of Proposition~\textup{\ref{prop:ainfty-formality-implies-koszul}}
holds: if $\cA$ is chirally Koszul, then $m_n = 0$ for all $n \geq 3$.

\smallskip\noindent\textbf{Gap.}
The converse requires constructing a contracting homotopy~$h$ for
$\barBgeom_X(\cA)$ as a factorization coalgebra.  On the formal disk
$\hat{D}$ this exists by standard homological algebra and the statement
reduces to the classical one \cite[Theorem~3.4.1]{LPWZ09}.  On a
compact curve of genus $g \geq 1$, the construction of such a
propagator for the factorization bar complex remains open.
\end{conjecture}
...
\begin{conjecture}[Ext diagonal vanishing criterion; \ClaimStatusConjectured]
...
$H^1(\bP^1, \cO) = 0$.
For genus $g \geq 1$, the Hodge-theoretic contributions from
$H^0(X, \omega_X)$ may cause genus-dependent failure; the expected
failure locus is the exceptional set $\Sigma(\fg)$ of
Theorem~\ref{thm:bar-cohomology-level-independence}.

\smallskip\noindent\textbf{Gap.}
The derived category of chiral modules on a compact curve lacks a
complete $t$-structure theory; the bigrading on Ext requires the
factorization module formalism of Gaitsgory--Raskin.
\end{conjecture}
...
Conjecture~\textup{\ref{conj:kac-shapovalov-koszulness}}.
\end{corollary}

\begin{conjecture}[Kac--Shapovalov criterion for simple quotients;
\ClaimStatusConjectured]
\label{conj:kac-shapovalov-koszulness}
...
\smallskip\noindent\textbf{Gap.}
The Shapovalov form controls injectivity of the PBW-to-bar
comparison.  The missing input is the identification of the
degeneracy locus $\{\det G_h = 0\}$ with the failure locus of
$E_2$-collapse, which requires a bar-level Jantzen filtration.
\end{conjecture}
...
with PBW filtration $F_\bullet$.  The following are equivalent:

\smallskip
\noindent\textbf{Proved circuit} \textup{(Theorems A, B, H):}
\begin{enumerate}
...
\item[\textup{(iii)}] The minimal $A_\infty$-model of
  ...
  \textup{(}implication
  \textup{(ii)}$\Rightarrow$\textup{(iii)} is
  Proposition~\textup{\ref{prop:ainfty-formality-implies-koszul}};
  the converse is
  Conjecture~\textup{\ref{conj:ainfty-koszul-characterization})}.
...
$\textup{(i)} \Leftrightarrow \textup{(v)}
\Leftrightarrow \textup{(viii)}$
is proved.

\smallskip
\noindent\textbf{Extended equivalences:}
\begin{enumerate}
...
\item[\textup{(xi)}] The Lagrangian criterion: $\cM_\cA$ and
  ...
  \textup{Conditional on perfectness and non-degeneracy of the
  ambient tangent complex.}
\end{enumerate}
\end{theorem}

\begin{proof}
\textsc{The proved circuit}
...
(Keller): a formal $A_\infty$-algebra is quasi-isomorphic to
its cohomology with $m_2$ alone, so the PBW filtration on the
bar complex of~$H^*$ has trivial differentials $d_r = 0$ for
$r \geq 2$.  The HPL transfer preserves the filtration, so the
spectral sequence on~$\barBgeom(\cA)$ also collapses at~$E_2$.
...
Totalization preservation: the log FM compactification is a smooth
manifold with corners whose strata are contractible
(Mok, \cite{Mok25}); totalizations are computed fiber-by-fiber
on the FM strata, each of which is acyclic.
...
$\operatorname{Sym}^{\mathrm{ch}}(V)_h \to \barB_h(\cA)$
is injective at every bar-relevant weight~$h$.  Injectivity
forces the PBW spectral sequence to degenerate at~$E_2$: any
$d_r$-differential with $r \geq 2$ would kill an element in
$\operatorname{Sym}^{\mathrm{ch}}(V)$ that is non-zero in the
Shapovalov pairing, contradicting $\det G_h \neq 0$.
Hence~(ii), hence~(i).
...
classes contradicting~(iv)).  Hence $E_2$-collapse, hence~(i).
...
anomaly.  This subsection develops the complementary
\emph{kinematic} axis, which depends only on the vacuum character
...
Classical Koszul duality for quadratic algebras over a point is an
algebraic assertion about generating functions.  The following theorem
lifts it to a geometric fact: Verdier duality on the Ran space of a
curve literally exchanges the bar construction of one chiral algebra
with that of its dual.  The bar-cobar adjunction is not a formal
analogy but an instance of Poincar\'e--Lefschetz duality in the
