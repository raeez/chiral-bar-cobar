...
We open by recalling the shadow obstruction tower
$\{S_r(\cA)\}_{r\ge 2}$ as the sequence of Maurer--Cartan
coefficients projected from the bar-intrinsic element $\Theta_\cA$
to a primary line $L$.
...
We extract five structural corollaries from the Riccati identity, each
...
the $g \ge 2$ multi-line correction is non-zero in general (witness
$\cW_3$, which is single-line at $T$ but acquires a cross-channel
correction at $W$ at $g = 2$).
\end{proof}
...
mandatory in every appearance of the formula by \end{remark}
