...
The propagators $\eta_{ij} = d\log(z_i - z_j)$ from the Heisenberg bar complex (Chapter~\ref{ch:heisenberg-frame}) live on Fulton--MacPherson compactifications of configuration spaces.  This chapter constructs these compactifications, develops the Orlik--Solomon algebra, and proves the Arnold relations forcing $d^2 = 0$.

\section{Fulton--MacPherson compactification}
\label{sec:FM-compactification}

\index{Verdier duality!on configuration spaces}
The Fulton--MacPherson compactification $\overline{C}_n(X)$ adds
boundary divisors encoding all collision patterns.  Verdier duality exchanges logarithmic forms
...
The Arnold relations admit three independent proofs---topological (partial fractions), geometric (boundary calculus), and algebraic (Orlik--Solomon)---each revealing different structure.
...
For $n=3$, the three codimension-$1$ boundaries are $D_{12}$, $D_{23}$, $D_{13}$ (pairwise collisions).  The three codimension-$2$ corners $D_{ij} \cap D_{ijk}$ correspond to the three binary trees on $\{1,2,3\}$: first $z_i \to z_j$, then the cluster meets $z_k$.  Non-nested pairs satisfy $D_{ij} \cap D_{jk} = \emptyset$ after blowup.
...
Arnold~\cite{Arnold69} proved $H^*(C_n(\mathbb{C})) \cong \bigwedge^* \langle e_{ij} \rangle / (e_{ij} + e_{ji},\; e_{ij} \wedge e_{jk} + e_{jk} \wedge e_{ki} + e_{ki} \wedge e_{ij})$.  As differential forms $\omega_{ij} = \omega_{ji}$; the antisymmetry is a convention on OS generators.
\end{remark}

\begin{remark}[Geometric interpretation]
This geometric proof makes the Arnold relations transparent: they express
$\partial^2 = 0$.  Kontsevich's formulation of configuration space
integrals \cite{Kontsevich03} relies on this perspective: all consistency
conditions come from the combinatorics of boundary strata intersections.
\end{remark}
...
Since there is only one generator, the Arnold relation is vacuous.
...
The topological proof connects Arnold relations to braid groups, the geometric proof deduces $d^2 = 0$ from $\partial^2 = 0$ via Stokes, and the algebraic proof gives computability via the Orlik--Solomon algebra.
\end{remark}
...
The logarithmic forms at higher genus are built from prime forms and period integrals.  The next section addresses the normal crossing structure needed for residue maps.
