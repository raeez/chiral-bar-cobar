...
The logarithmic forms $\eta_{ij} = d\log(z_i - z_j)$ that served as propagators in the Heisenberg bar complex (Chapter~\ref{ch:heisenberg-frame}) live naturally on Fulton--MacPherson compactifications of configuration spaces. This chapter develops the geometry of these compactifications: their boundary stratification, the Orlik--Solomon algebra of logarithmic forms, and the Arnold relations that ensured $d^2 = 0$ in the Heisenberg case.

\section{Fulton--MacPherson compactification}
\label{sec:FM-compactification}

The preceding chapter developed Koszul duality as a formal
adjunction on dg-categories.  This chapter gives it a geometric
body.  The key identification is:
\begin{quote}
\emph{The bar differential on a chiral algebra is the residue map
on Fulton--MacPherson compactifications, and Verdier duality on
configuration spaces exchanges bar and cobar.}
\end{quote}
Configuration spaces $C_n(X)$ parametrize $n$ distinct insertion
points on the curve; the Fulton--MacPherson compactification
$\overline{C}_n(X)$ adds boundary divisors encoding all collision
patterns; logarithmic forms $\eta_{ij} = d\log(z_i - z_j)$ carry
the correct singularity data.  The Arnold relations among these
forms translate into $d^2 = 0$ for the bar differential.  Verdier
duality on these spaces
...
\end{align*}
realizes the Ran-space duality $\mathbb{D}_{\operatorname{Ran}}
\bar{B}_X(\cA) \simeq \bar{B}_X(\cA^!)$ at the chain level.
Everything in this monograph --- the genus tower, the
complementarity of quantum corrections, the discriminant invariant
--- rests on the geometry developed here.
...
\begin{remark}[Necessity of compactification]\label{rem:why-compactify}
...
\begin{itemize}
\item \emph{Simultaneous collision:} Multiple points approach the same location
\item \emph{Sequential collision:} Points collide in stages with different rates
\item \emph{Angular information:} The relative angles of approach matter
\item \emph{Topological degenerations (genus $g \geq 1$):} Cycles can pinch, creating nodal curves
\end{itemize}
...
We construct the Fulton--MacPherson compactification following \cite{FM94, BD04}. Blow-ups encode not just \emph{which} points collide, but \emph{how} they collide---their relative rates and angles of approach.  For a logarithmic variant applicable to punctured curves and stable curves with nodes, see Mok~\cite{Mok25}.

\subsubsection{Iterated blow-up construction}

\begin{theorem}[Fulton--MacPherson compactification at genus $g$ \cite{FM94}; \ClaimStatusProvedHere]\label{thm:FM}
...
The exceptional divisor $E_n$ is a $\mathbb{P}^{n-2}$-bundle over $\Sigma_g$ (the projectivization of the normal bundle $N_{\Delta_n/\Sigma_g^n} \cong \bigoplus^{n-1} T\Sigma_g$), parametrizing:
\begin{itemize}
...
\begin{itemize}
\item \emph{Rates:} If $z_i \to z_j$ as $t \to 0$, at what rate? The blow-up records $|z_i - z_j| \sim \epsilon(t)$.
\item \emph{Angles:} From which direction? The blow-up records $\arg(z_i - z_j) = \theta$.
\item \emph{Hierarchies:} If points collide in stages ($z_1, z_2$ collide first, then their center collides with $z_3$), the nested blow-ups record this hierarchy.
\end{itemize}

This is precisely what is needed for OPE:
...
\begin{theorem}[Boundary strata of $\overline{\mathcal{M}}_{g,n}$ {\cite{DM69,Knu83}}; \ClaimStatusProvedElsewhere]\label{thm:boundary-higher-genus}
...
\emph{Exhaustiveness.}
Every point of $\partial\overline{C}_n(\Sigma_g) = \overline{C}_n(\Sigma_g) \setminus C_n(\Sigma_g)$ belongs to at least one stratum: either some subset of marked points has collided (type 1), or the underlying curve has degenerated (type 2), or both.  This follows from the properness of $\overline{\mathcal{M}}_{g,n}$ (Deligne--Mumford--Knudsen \cite{DM69,Knu83}): every sequence in $C_n(\Sigma_g)$ has a subsequence converging to a stable $n$-pointed curve, and the limit is classified by its dual graph and the collision data of the marked points.
\end{proof}
...
For a fixed smooth curve $\Sigma_g$, there is a natural \emph{stabilization} map:
$$\pi: \overline{C}_n(\Sigma_g) \to \overline{\mathcal{M}}_{g,n}, \qquad (p_1, \ldots, p_n) \mapsto [\Sigma_g; p_1, \ldots, p_n]^{\mathrm{st}}$$
that records the stable $n$-pointed curve obtained by stabilizing $(\Sigma_g, p_1, \ldots, p_n)$ (contracting any unstable rational components in the bubble tree).

\begin{itemize}
\item The image lies in the closure of the locus of curves isomorphic to $\Sigma_g$.
\item Over boundary strata of $\overline{\mathcal{M}}_{g,n}$, the bubble tree structure of $\overline{C}_n(\Sigma_g)$ maps to the nodal curve structure of $\overline{\mathcal{M}}_{g,n}$.
\end{itemize}

This connection underlies:
\begin{enumerate}
\item \emph{Modular properties:} The chiral algebra correlators are sections of line bundles over $\overline{\mathcal{M}}_{g,n}$
\item \emph{Factorization:} Degenerations correspond to factorization of correlation functions
\item \emph{Anomalies:} Failure of sections to extend over boundary = conformal anomalies
\end{enumerate}
\end{remark}
...
The normal crossing property of the boundary divisor ensures that residues are well-defined.
...
For the Heisenberg algebra, the bar differential $d_{\mathrm{res}}$ extracted the double-pole coefficient $k$ at each collision divisor (\S\ref{sec:frame-bar-deg1}).  The following theorem gives the general residue machinery.
...
\begin{theorem}[Functoriality of FM compactification {\cite{FM94}}; \ClaimStatusProvedElsewhere]\label{thm:FM-functorial}
...
\begin{theorem}[Universal property: operadic structure {\cite{FM94,LV}}; \ClaimStatusProvedElsewhere]\label{thm:FM-operad}
...
is standard in operad references such as \cite{LV}.
\end{remark}
...
\begin{example}[Kac--Moody modules on $\mathbb{P}^1$ with
$3$ punctures]\label{ex:log-fm-km}
...
with differential respecting the genus stratification:
...
\item \emph{Genus 1:} Elliptic Arnold relations with modular corrections
...
\begin{remark}[Logarithmic forms]
Logarithmic forms are the unique meromorphic 1-forms with prescribed residues at collision divisors. When operators collide in conformal field theory, the singularity structure is captured by forms $d\log(z_i - z_j)$. Making these forms single-valued requires choosing branch cuts, and these choices encode the monodromy data that will later appear in our $A_\infty$ relations.
\end{remark}


\begin{definition}[Branch cut convention]
...
\begin{proposition}[Logarithmic form properties \cite{Deligne71}; \ClaimStatusProvedElsewhere]
The forms $\eta_{ij} = d\log(z_i - z_j)$ satisfy:
\begin{enumerate}
\item $\eta_{ji} = \eta_{ij}$ as differential forms: $d\log(z_j - z_i) = d\log(z_i - z_j)$ since $\log(z_j-z_i)$ and $\log(z_i-z_j)$ differ by the constant $i\pi$, which is killed by $d$ (see Proposition~\ref{prop:eta})
...
\begin{proposition}[Explicit local charts for $\overline{C}_n(X)$ \cite{FM94}; \ClaimStatusProvedElsewhere]\label{prop:local-charts}
...
\begin{itemize}
\item \emph{Canonical}: Independent of choices (uses only the complex structure)
\item \emph{Functorial}: Natural with respect to curve morphisms
\item \emph{Minimal}: The unique smooth compactification with normal crossing divisors
\end{itemize}
\end{proposition}
...
\begin{proposition}[Properties of $\eta_{ij}$; \ClaimStatusProvedHere]\label{prop:eta}
...
\begin{theorem}[Arnold relations; \ClaimStatusProvedHere]
...
\begin{theorem}[Elliptic compactification; \ClaimStatusProvedHere]
...
\begin{theorem}[Period integrals and bar differential \cite{BD04}; \ClaimStatusProvedElsewhere]
...
\]
given by forgetting the $n$-th point (with fiber $\mathbb{C} \setminus \{z_1, \ldots, z_{n-1}\}$). The NBC basis respects this fibration:
\begin{itemize}
...
differential satisfies $d^2 = 0$. These relations connect to:
\begin{itemize}
\item \emph{Topology}: Cohomology of braid groups and hyperplane arrangements
\item \emph{Geometry}: Boundary structure of configuration space compactifications
\item \emph{Algebra}: Quadratic-cubic relations in Orlik--Solomon algebras
\end{itemize}

We present three complete, independent proofs, each emphasizing a different aspect of the structure.
...
\emph{Step 1: Setup and notation.}
...
\emph{Step 2: Independence of generators.}

The forms $\omega_{ij} = d\log(z_i - z_j)$ are symmetric: $\omega_{ij} = \omega_{ji}$
(since $d\log(z_j - z_i) = d\log(-(z_i - z_j)) = d\log(z_i - z_j)$ on $C_n(\mathbb{C})$, as $d\log(-1) = 0$).

The cohomology classes $[\omega_{ij}]$ for $1 \leq i < j \leq n$
are \emph{all independent} in $H^1(C_n(\mathbb{C}))$, giving $\dim H^1 = \binom{n}{2}$.
There are \emph{no linear relations} among the $\omega_{ij}$ in cohomology; the relations
are all quadratic (Arnold relations in $H^2$).

\begin{remark}
The partial fraction identity
$\frac{z_i - z_j}{(z_i - z_k)(z_k - z_j)} = \frac{1}{z_k - z_j} + \frac{1}{z_i - z_k}$
relates rational functions, not their logarithmic derivatives.  Taking $d\log$ of both
sides does \emph{not} give $d\log(z_i - z_j) = d\log(z_i - z_k) + d\log(z_k - z_j)$
(the $d\log$ of a sum is not the sum of $d\log$s).
\end{remark}

\emph{Step 3: Wedge products and quadratic relations.}
...
\emph{Step 4: Cyclic sum and partial fractions.}
...
\emph{Step 5: Verification of partial fraction identity.}
...
studying the cohomology of braid groups. Configuration spaces 
of points are complements of hyperplane arrangements, and their cohomology rings have 
quadratic presentations (the Arnold relations are degree 2 in the exterior algebra).
\end{remark}
...
\begin{theorem}[Arnold relations: geometric form; \ClaimStatusProvedHere]
...
\emph{Step 1: Boundary structure of $\overline{C}_n(X)$.}
...
There are three codimension-2 corners, corresponding to the three binary trees on $\{1,2,3\}$:
\begin{itemize}
\item $D_{12} \cap D_{123}$: first $z_1 \to z_2$, then the cluster collides with $z_3$
\item $D_{13} \cap D_{123}$: first $z_1 \to z_3$, then the cluster collides with $z_2$
\item $D_{23} \cap D_{123}$: first $z_2 \to z_3$, then the cluster collides with $z_1$
\end{itemize}
Note that $D_{ij} \cap D_{jk} = \emptyset$ for distinct pairs $\{i,j\}$ and $\{j,k\}$: the blowup of the total diagonal separates these boundary divisors.

\emph{Step 2: Stokes theorem on $\overline{C}_n(X)$.}
...
\emph{Step 3: Iterated residues and corners.}

Consider a corner $D_{ij} \cap D_{ijk}$ where first $z_i \to z_j$ (within divisor $D_{ij}$), then the cluster $\{z_i, z_j\}$ collides with $z_k$ (within divisor $D_{ijk}$). Note: non-nested pairs $D_{ij} \cap D_{jk} = \emptyset$ after blowup; only nested pairs $D_{ij} \cap D_{ijk}$ give corners. There are two iterated residues at this corner:
\begin{enumerate}
\item First take residue at $D_{ij}$, then at $D_{ijk}$: $\text{Res}_{D_{ijk}} \circ
\text{Res}_{D_{ij}}$
\item First take residue at $D_{ijk}$, then at $D_{ij}$: $\text{Res}_{D_{ij}} \circ
\text{Res}_{D_{ijk}}$
\end{enumerate}

These give the same answer if we can continuously deform one path to the other.

\emph{Step 4: Three corners and the Arnold relation.}

For three points $i, j, k$, there are three codimension-2 corners, each nested inside $D_{ijk}$:
\begin{itemize}
\item $D_{ij} \cap D_{ijk}$: first $z_i \to z_j$, then cluster collides with $z_k$
\item $D_{jk} \cap D_{ijk}$: first $z_j \to z_k$, then cluster collides with $z_i$
\item $D_{ki} \cap D_{ijk}$: first $z_k \to z_i$, then cluster collides with $z_j$
\end{itemize}

These three corners are \emph{distinct} codimension-2 strata in $\overline{C}_3(X)$, corresponding to the three binary trees on $3$ leaves (i.e., three different collision orderings).

In $\overline{C}_3(X)$, the three codimension-2 strata $D_{ij} \cap D_{ijk}$, $D_{jk} \cap D_{ijk}$, $D_{ki} \cap D_{ijk}$ meet at a common codimension-3 stratum (where all relative directions have been resolved).

\emph{Step 5: Orientation and signs.}
...
\emph{Step 6: Conclusion from $\partial^2 = 0$.}
...
\begin{enumerate}
\item $e_{ij} + e_{ji} = 0$ (antisymmetry of generators; note that as \emph{differential forms}, $\omega_{ij} = \omega_{ji}$ since $d\log(z_i-z_j) = d\log(z_j-z_i)$; the antisymmetry is a convention on the OS algebra generators $e_{ij}$)
\item $e_{ij} \wedge e_{jk} + e_{jk} \wedge e_{ki} + e_{ki}
\wedge e_{ij} = 0$ (Arnold relations)
\end{enumerate}
\end{remark}

\begin{remark}[Geometric interpretation]
This geometric proof makes the Arnold relations transparent: they express
...
\begin{corollary}[Stokes theorem and differential; \ClaimStatusProvedHere]
...
\begin{theorem}[Arnold relations: algebraic form; \ClaimStatusProvedHere]
\label{thm:arnold-algebraic}
The cohomology ring $H^*(C_n(\mathbb{C}), \mathbb{Z})$ is isomorphic to the 
\emph{Orlik--Solomon algebra} $OS(\mathcal{A}_n)$ associated to the braid arrangement 
...
\emph{Step 1: Definition of Orlik--Solomon algebra.}
...
Orlik and Solomon \cite{OS80} proved that $H^*(M(\mathcal{A}), \mathbb{Z})$ 
has a purely combinatorial description in terms of the intersection lattice of $\mathcal{A}$.

\emph{Definition}: The Orlik--Solomon algebra is:
\begin{equation}
...
\emph{Step 2: Apply to braid arrangement.}
...
\emph{Step 3: Identify with the Arnold relation.}

The OS generators $e_{ij}$ are indexed by \emph{hyperplanes} $H_{ij} = \{z_i = z_j\}$, and $H_{ij} = H_{ji}$, so $e_{ij} = e_{ji}$ (symmetric).  The OS relation
\begin{equation}
e_{ij} \wedge e_{jk} - e_{ij} \wedge e_{ik} + e_{jk} \wedge e_{ik} = 0
\end{equation}
is identified with the Arnold relation via the isomorphism $e_{ij} \mapsto \omega_{ij} = d\log(z_i - z_j)$.  Since $\omega_{ij} = \omega_{ji}$ (as $d\log(z_i - z_j) = d\log(z_j - z_i)$, the branch cut contributing only a constant), the Arnold relation in forms is:
\begin{equation}
\omega_{ij} \wedge \omega_{jk} + \omega_{jk} \wedge \omega_{ki} + \omega_{ki} \wedge \omega_{ij} = 0
\end{equation}
which follows from the partial fraction identity $\frac{1}{(z_i{-}z_j)(z_j{-}z_k)} + \frac{1}{(z_j{-}z_k)(z_k{-}z_i)} + \frac{1}{(z_k{-}z_i)(z_i{-}z_j)} = 0$.

\emph{Step 4: Isomorphism with cohomology.}
...
\emph{Step 5: Quadratic presentation.}

The algebra $OS(\mathcal{A}_n)$ is Koszul: it has a
...
The Koszul property means that $OS(\mathcal{A}_n)$ admits a linear resolution, which controls the bar-cobar duality.

\emph{Step 6: Connection to chiral algebras.}
...
\index{symmetric group!action on configuration space}
The symmetric group $S_n$ acts on $C_n(\mathbb{C})$ by permuting points, and the
quotient $C_n(\mathbb{C})/S_n$ is a $K(B_n, 1)$---this is the classical definition
of the braid group $B_n = \pi_1(C_n(\mathbb{C})/S_n)$. The OS algebra computes
$H^*(C_n(\mathbb{C}); \mathbb{Q})$, the cohomology of the ordered configuration space.
\end{remark}

\subsection{Equivalence of the three proofs}
\label{sec:arnold-equivalence}

\begin{theorem}[Equivalence of Arnold formulations; \ClaimStatusProvedHere]
\label{thm:arnold-equivalence-complete}
The three formulations of Arnold relations are equivalent:
\begin{enumerate}
\item \emph{Topological}: $\omega_{ij} \wedge \omega_{jk} + \omega_{jk} \wedge 
\omega_{ki} + \omega_{ki} \wedge \omega_{ij} = 0$ in $H^*(C_n(X))$
\item \emph{Geometric}: $\text{Res}_{D_{ij}} \circ \text{Res}_{D_{jk}} + 
\text{Res}_{D_{jk}} \circ \text{Res}_{D_{ki}} + \text{Res}_{D_{ki}} \circ 
\text{Res}_{D_{ij}} = 0$
\item \emph{Algebraic}: $e_{ij} \wedge e_{jk} + e_{jk} \wedge e_{ki} + e_{ki} 
\wedge e_{ij} = 0$ in $OS(\mathcal{A}_n)$
\end{enumerate}
\end{theorem}

\begin{proof}
\emph{(1) $\Leftrightarrow$ (3)}: This follows from the Orlik--Solomon isomorphism:
\begin{equation}
...
\emph{(1) $\Leftrightarrow$ (2)}: This uses the relationship between forms and residues.
...
\emph{(2) $\Leftrightarrow$ (3)}: Both are manifestations of $\partial^2 = 0$.
...
\begin{remark}[Dictionary between perspectives]
...
\subsection{\texorpdfstring{Explicit computations for $n = 2, 3, 4, 5$}{Explicit computations for n = 2, 3, 4, 5}}
...
Since there is only one generator, there are no non-trivial relations. The Arnold relation 
is vacuous.
...
\emph{Verification}: $d^2 = \text{Res}_{D_{12}}^2 = 0$ trivially since there is only 
one divisor.
\end{example}
...
These are all independent in $H^1(C_3(\mathbb{C}))$: $\dim H^1 = \binom{3}{2} = 3$
(there are \emph{no} linear relations among the $\omega_{ij}$ in cohomology; the
Arnold relation is quadratic).

The cohomology is:
\begin{equation}
H^*(C_3(\mathbb{C})) = \bigwedge^*(\omega_{12}, \omega_{13}, \omega_{23}) / (\text{Arnold relation})
\end{equation}

The fundamental Arnold relation:
\begin{equation}
\omega_{12} \wedge \omega_{23} + \omega_{23} \wedge \omega_{31} + \omega_{31} 
\wedge \omega_{12} = 0
\end{equation}
...
\dim H^1(C_3(\mathbb{C})) &= 3 \quad \text{(}\omega_{12}, \omega_{13}, \omega_{23}\text{)}\\
...
These relations are \emph{independent}: no one follows from the others.
...
Note that $C_4(\mathbb{C})$ is non-compact, so Poincaré duality does not apply; the polynomial is \emph{not} palindromic.
\end{example}
...
\emph{crossing symmetry} in scattering amplitudes. Different orders of taking limits 
...
\begin{corollary}[Operadic associativity \cite{LV}; \ClaimStatusProvedElsewhere]
...
Each proof of the Arnold relations provides different information:
\begin{itemize}
\item \emph{Topological}: Reveals connection to braid groups and hyperplane arrangements
\item \emph{Geometric}: Deduces nilpotency $d^2 = 0$ from $\partial^2 = 0$ via Stokes' theorem
\item \emph{Algebraic}: Provides computational tools via Orlik--Solomon algebra
\end{itemize}

Together, these proofs show that Arnold relations are intrinsic to the geometry of configuration spaces and essential for consistency of chiral algebras.
\end{remark}
...
\emph{Exceptional divisor.} $E = D_{12} \cong X$
\begin{itemize}
\item The single $X$ factor parametrizes the collision location $u = z_1 = z_2$
\item For a curve~$X$, the normal bundle to the diagonal $\Delta \hookrightarrow X \times X$ has rank~1, so the projectivized fiber is $\mathbb{P}^0 = \mathrm{pt}$; the angular direction $\theta$ contributes no additional moduli
\end{itemize}
\end{example}
...
\emph{negative normal bundles}, making the compactification stable.
...
\subsection{The role of coordinates}

The explicit coordinate systems on $\overline{C}_n(X)$ enable evaluation of
integrals and residues, reveal boundary strata and normal bundles, and connect
configuration space geometry to chiral algebra operations.
Every calculation in the bar complex relies on these coordinate systems.
...
\bigskip
\noindent
The geometry of configuration spaces and Ran spaces is now in
place.  The Fulton--MacPherson compactification provides the
smooth manifold with normal-crossing boundary on which the bar
differential lives; the Orlik--Solomon algebra and Arnold
relations ensure $d^2 = 0$; the logarithmic forms provide
conformally invariant propagators with canonical residues;
and the Ran space furnishes the $\infty$-categorical home for
factorization algebras.  In Chapter~\ref{chap:bar-cobar},
these ingredients assemble: the bar differential will be the
residue map on $\overline{C}_n(X)$, the Verdier pairing between
logarithmic forms and distributions will produce the bar-cobar
adjunction, and the Koszul property will be detected by the
collapse of a spectral sequence on configuration space
cohomology.
