...
Every construction in Parts~I--III takes place on the
Fulton--MacPherson compactification $\overline{C}_n(X)$, whose boundary
strata encode the full hierarchy of collision data.  On
...
the algebraic invariants computed in Part~II\@.  The abstract
...
\emph{Pole-order counting (AP19).}  The propagator $\eta_{12} = d\log(z_1-z_2)$
...
not the $(h_i+h_j-h_k+1)$-th.  In the language of AP19, the $d\log$
...
$\Omega/z$ with a single pole.  For the Virasoro OPE with $z^{-4},
