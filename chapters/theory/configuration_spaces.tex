...
Configuration spaces appear in our construction for three interrelated reasons.
First, $C_n(X)$ are the natural domains for $n$-ary operations in chiral algebras,
parametrizing insertion points $(z_1, \ldots, z_n)$.
Second, the Fulton--MacPherson compactification $\overline{C}_n(X)$ provides a smooth
manifold with corners whose boundary divisors encode collision patterns and whose
logarithmic forms give well-defined residues.
Third, configuration spaces support factorization homology
$\int_X \cA = \operatorname{colim}_n \int_{C_n(X)} \cA^{\otimes n}$,
and Verdier duality on these spaces
...
\end{align*}
is the geometric mechanism underlying chiral Koszul duality.
