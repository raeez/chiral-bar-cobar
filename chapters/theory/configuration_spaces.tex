...
\item \textbf{Degeneration strata:} $D_{\Gamma}$ where the curve degenerates to a stable nodal curve of genus $g$ with dual graph $\Gamma$
\end{enumerate}
...
$D_S \subset \overline{C}_n(\Sigma_g)$ has \textbf{codimension~1}. The divisor $D_S$
...
\begin{enumerate}
\item \textbf{Symmetry:} $\eta_{ji} = \eta_{ij}$ (as differential forms, since $d\log(z_j - z_i) = d\log(-(z_i-z_j)) = d\log(z_i-z_j)$, using $d\log(-1)=0$). Antisymmetry $e_{ji} = -e_{ij}$ is imposed at the level of the Orlik-Solomon algebra generators, not the forms themselves.

\item \textbf{Arnold relation:} For distinct $i,j,k$:
$$\eta_{ij} \wedge \eta_{jk} + \eta_{jk} \wedge \eta_{ki} + \eta_{ki} \wedge \eta_{ij} = 0$$

\item \textbf{Completeness (genus $0$):} For $\Sigma_0 = \mathbb{C}$, the classes $\eta_{ij}$ generate
...
\begin{proof}
\textbf{Part 1: Symmetry of forms.}
...
\medskip
\noindent\textbf{Step 1: Local expression.}
...
\medskip
\noindent\textbf{Step 2: Residue computation.}
...
\medskip
\noindent\textbf{Step 3: Coordinate independence.}
...
\item \textbf{Residue theorem:} If $d\omega=0$, then the residue family $\{\text{Res}_{D_\alpha}(\omega)\}_\alpha$ satisfies the cocycle condition in the residue sequence of Theorem~\ref{thm:residue-sequence} (equivalently, its total boundary residue vanishes).
\end{enumerate}
\end{theorem}
...
\item \textbf{For open embeddings:} If $f: \Sigma_g \hookrightarrow \Sigma_{g'}$ is an open embedding, there is an induced map:
$$\overline{f^{(n)}}: \overline{C}_n(\Sigma_g) \to \overline{C}_n(\Sigma_{g'})$$
...
\begin{itemize}
\item \textbf{Collision} (genus-preserving): merging two insertions at a smooth point
\item \textbf{Non-separating node}: $\Sigma_g \to \Sigma_{g-1}$ with two new marked points (pinching a non-separating cycle)
\item \textbf{Separating node}: $\Sigma_g \to \Sigma_{g_1} \cup \Sigma_{g_2}$, $g_1 + g_2 = g$ (pinching a separating cycle)
\end{itemize}
...
\begin{itemize}
\item \textbf{Genus 0:} pure braid group $P_n$ on $n$ strands.
\item \textbf{Genus 1:} an extension of $P_n$ by an elliptic braid group with modular data.
\item \textbf{Genus $g \geq 2$:} an extension by a surface braid group with mapping class group action.
\end{itemize}
...
\textbf{Genus 0 ($\mathbb{P}^1$):} We compute $\overline{M}_{0,4}$ explicitly (the Deligne--Mumford compactification of the moduli space of $4$ distinct points on~$\mathbb{P}^1$, which governs the genus-$0$ bar complex at degree~$1$):
\begin{enumerate}
...
\begin{itemize}
\item \textbf{Genus 0 (Arnold relation):}
$\eta_{ij} \wedge \eta_{jk} + \eta_{jk} \wedge \eta_{ki} + \eta_{ki} \wedge \eta_{ij} = 0$
...
\item \textbf{Genus 1:} Elliptic Arnold relations with modular corrections
\item \textbf{Genus $g \geq 2$:} Siegel modular corrections from period integrals
\end{itemize}

The Arnold wedge product relation is crucial for the bar differential to satisfy
...
\begin{theorem}[Arnold Relations - Rigorous; \ClaimStatusProvedHere]
...
\textbf{Note:} This $L^2$ convergence condition is very restrictive.  In practice, the bar complex uses distributional pairings (residues at collision divisors), which are well-defined whenever the OPE poles have finite order.  The condition $h_i + h_j < h_{\min}$ governs $L^2$ integrability of the logarithmic forms, not the existence of the bar differential.
...
\textbf{Codimension 2}: At $D_{ijk}$ (the stratum where all three points $z_i, z_j, z_k$ collide simultaneously):
...
\begin{theorem}[Presentation Independence - REFINED; \ClaimStatusProvedHere]\label{thm:presentation-independence}
...
\textbf{NBC count:} For the braid arrangement, the NBC basis satisfies $f(n) = n \cdot f(n-1)$
...
\begin{proposition}[Algorithm Correctness; \ClaimStatusProvedHere]
...
\begin{proof}
Correctness follows from the residue formula in Theorem~\ref{thm:residue-operations}. We only get nonzero contributions when the weight condition is satisfied, corresponding to simple poles. The algorithm checks all $\binom{n}{2}$ pairs, each in time $T_{\text{OPE}}$.
\end{proof}
...
\textbf{Step 2: Independence of generators.}
...
\textbf{Key fact:} The cohomology classes $[\omega_{ij}]$ for $1 \leq i < j \leq n$
...
These three corners are \textbf{distinct} codimension-2 strata in $\overline{C}_3(X)$, corresponding to the three binary trees on $3$ leaves (i.e., three different collision orderings).

\textbf{Key geometric fact}: In $\overline{C}_3(X)$, the three codimension-2 strata $D_{ij} \cap D_{ijk}$, $D_{jk} \cap D_{ijk}$, $D_{ki} \cap D_{ijk}$ meet at a common codimension-3 stratum (where all relative directions have been resolved).
...
\textbf{Step 3: Identify with the Arnold relation.}
...
\textbf{Poincaré polynomial}: $P_t(H^*(C_3(\mathbb{C}))) = (1+t)(1+2t) = 1 + 3t + 2t^2$.
\end{example}
...
\textbf{Poincaré polynomial}: $P_t(H^*(C_4(\mathbb{C}))) = (1+t)(1+2t)(1+3t) = 1 + 6t + 11t^2 + 6t^3$.
...
\textbf{Exceptional divisor:} $E = D_{12} \cong X$
\begin{itemize}
...
\textbf{Divisor:} $D_{12} = \{\epsilon_{12} = 0\} \cong X \times X$
\begin{itemize}
...
\subsection{\texorpdfstring{Complete Example: $n=4$ with All Coordinates}{Complete Example: n=4 with All Coordinates}}
...
\begin{computation}[Serre: Explicit Examples; \ClaimStatusProvedHere]
...
\textbf{Claim:} $S_1 = \{0\}$ lies in the closure of $\mathrm{Ran}^{\geq 2}(\mathbb{R})$, so cardinality strata are not closed and $\mathrm{Ran}(\mathbb{R})$ is not Hausdorff.

\textbf{Proof:} In the Ran topology, the sequence $\{0, 1/k\} \in \mathrm{Ran}^2(\mathbb{R})$ converges to $\{0\} \in \mathrm{Ran}^1(\mathbb{R})$ as $k \to \infty$ (collision of the two points).  Hence $S_1 = \{0\}$ is a limit point of $\mathrm{Ran}^2(\mathbb{R})$ but does not lie in it.  Since $\mathrm{Ran}^2$ is not closed, $\mathrm{Ran}(\mathbb{R})$ is not $T_1$ (hence not Hausdorff).

\textbf{Interpretation:} The non-Hausdorff property encodes the physics of particle collisions: one cannot separate configurations with different particle numbers.
\end{example}
