...
The governing question of this chapter is the mathematical role of \emph{Configuration spaces}: which part of the modular bar-cobar construction or its inversion it isolates, and which boundary between construction, resolution, and shadow it clarifies.
\end{remark}
...
\end{remark}
The propagators $\eta_{ij} = d\log(z_i - z_j)$ from the Heisenberg bar complex (Chapter~\ref{ch:heisenberg-frame}) live on Fulton--MacPherson compactifications of configuration spaces.  The compactification encodes all collision patterns in boundary divisors; the Orlik--Solomon algebra organizes the logarithmic forms on these divisors; and the Arnold relations among triple collisions force $d^2 = 0$.

\label{rem:config-spaces-hms}
...
\begin{theorem}[Fulton--MacPherson compactification at genus $g$ \cite{FM94}; \ClaimStatusProvedElsewhere]\label{thm:FM}
...
For the braid arrangement $\mathcal{A}_n = \{H_{ij} : z_i = z_j\}$, the hyperplanes are $H_{ij}$ for $1 \leq i < j \leq n$, and a triple $\{H_{ij}, H_{jk}, H_{ik}\}$ is dependent because
...
\end{equation}
where $T(\mathbb{C}^{\binom{n}{2}})$ is the tensor algebra on $\binom{n}{2}$ generators (one per pair $i < j$) and $R \subset T^2$ is the space of quadratic relations (Arnold relations).
