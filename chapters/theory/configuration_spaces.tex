...
The bar differential involves residues of meromorphic forms on products of curves.  To make this rigorous---to give meaning to the limit as marked points collide---requires a compactification of the configuration space $C_n(X) = X^n \setminus \Delta$ that remembers the \emph{direction} and \emph{rate} of approach along each diagonal.  The Fulton--MacPherson compactification provides exactly this: it replaces each collision locus with a screen recording the relative positions and rates, producing a smooth variety with normal crossing boundary on which the propagators $\eta_{ij} = d\log(z_i - z_j)$ extend to logarithmic forms and the Arnold relations among triple collisions force $d^2 = 0$.
...
\begin{proof}
On $C_n(\mathbb{C}) = \mathbb{C}^n \setminus \bigcup_{i<j}\{z_i = z_j\}$, define closed $1$-forms $\omega_{ij} = d\log(z_i - z_j)$.  The classes $[\omega_{ij}]$ for $i < j$ are independent in $H^1$, giving $\dim H^1 = \binom{n}{2}$.  For the cyclic sum, expanding each term via
$\omega_{ij} \wedge \omega_{jk} = \frac{(dz_i - dz_j)\wedge(dz_j - dz_k)}{(z_i - z_j)(z_j - z_k)}$
and summing cyclically, the coefficient of each $2$-form $dz_a \wedge dz_b$ reduces to the partial fraction identity
\[
\frac{1}{(z_i - z_j)(z_j - z_k)} + \frac{1}{(z_j - z_k)(z_k - z_i)} + \frac{1}{(z_k - z_i)(z_i - z_j)} = 0,
\]
which holds because the numerator of the common-denominator expression is $(z_k - z_i) + (z_i - z_j) + (z_j - z_k) = 0$.  Therefore $\omega_{ij} \wedge \omega_{jk} + \omega_{jk} \wedge \omega_{ki} + \omega_{ki} \wedge \omega_{ij} = 0$. \qedhere
\end{proof}
