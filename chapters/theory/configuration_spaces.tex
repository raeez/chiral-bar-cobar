...
coordinate expression \(\eta_{ij}=d\log(z_i-z_j)\) is only an affine or
formal-screen representative of the logarithmic normal form along the
diagonal; it does not extend as a smooth global form on an arbitrary
curve. The bar construction needs a weaker and sharper object: a
...
The logarithmic \(1\)-forms \(\eta_{ij}\), represented on an affine
genus-$0$ screen by \(d\log(z_i-z_j)\), are the scalar coefficients of
the universal KZ connection one-form on that screen,
...
differential is the FM-residue realization of the image of this flat
connection under the KZ specialisation:
\[
...
\end{equation}
Third, this relation supplies the form half of the affine simple-pole
component of $d_{\mathrm{residue}}^2$
...
the genus-$0$ Arnold relation is the precise local cancellation identity
for the scalar logarithmic-form coefficients. For a chiral algebra
coefficient system the same codimension-two corner also consumes the
Borcherds coefficient identity on OPE coefficients. The equivalence between
the local Arnold relation and scalar-form triple-residue nilpotency is
Theorem~\ref{thm:arnold-iff-nilpotent}; on the affine genus-$0$ screen
it is the only relation on the scalar propagator classes, not the whole
operator-valued bar-square proof. On
...
(Proposition~\ref{prop:elliptic-arnold-relations}) feed the curved
positive-genus fibrewise square; only after scalar projection does this
produce the Hodge obstruction
...
$\operatorname{Ran}(X)$ exchanges coalgebras and algebras and forms
the Verdier--Koszul algebra
\[
K_X(\cA)
:=\mathbb{D}_{\operatorname{Ran}}\, \bar{B}_X(\cA)
\]
Convention~\textup{\ref{conv:bar-coalgebra-identity}}).  A
finite-type quadratic comparison supplies the pair map
\(\nu_\cA\colon K_X(\cA)\to\cA^!\).  The bar--cobar counit
$\Omega(\bar{B}_X(\cA)) \simeq \cA$ is the separate reconstruction
functor on the same bar input.
...
\begin{definition}[Stable graph; \ClaimStatusDefinitional]\label{def:stable-graph}
...
On an affine genus-zero chart, the forms
\(\eta_{ij}=d\log(z_i-z_j)\) that served as propagators in the
Heisenberg bar complex (\S\ref{sec:frame-bar-deg1}) are the simplest
instances of the following class. On a general curve the notation
\(\eta_{ij}\) means the logarithmic
normal form along \(D_{ij}\) together with its coordinate-change
cocycle.
...
In the degree-$2$ Heisenberg frame computation
\textup{(}Proposition~\textup{\ref{prop:frame-arnold}}\textup{)}, the
identity $d^2 = 0$ reduces to the three-term relation
$\eta_{12} \wedge \eta_{23} + \eta_{23} \wedge \eta_{31}
+ \eta_{31} \wedge \eta_{12} = 0$. This is the Arnold relation.
Geometrically the relation is the scalar differential-form half of the
flatness of the universal Arnold connection
...
\end{align*}
  ...
  \emph{Step 1: the three numerator $2$-forms coincide.} Using
  $dz_{ki} = -(dz_{ij} + dz_{jk})$, the differential of the displayed
  cyclic identity:
\begin{align*}
...
\end{align*}
  by the displayed cyclic identity. This holds for any triple $i,j,k$
  and any $n$.
...
\begin{proposition}[Higher-genus correction to the affine Arnold scalar presentation; \ClaimStatusProvedHere]
...
additional affine Arnold scalar relation among closed pairwise classes computes
...
\item There is an open embedding
 ...
 with dense image.
\item The logarithmic structure on
 $\mathrm{FM}_m(X \mathbin{|} D)$ is the fine saturated divisorial
 log structure of the toroidal boundary
 $\partial\mathrm{FM}_m(X\mathbin{|}D)$; equivalently
 \[
 M_{\mathrm{FM}}=
 j_*\mathcal O^\times_{C_m(X\setminus D)}
 \cap \mathcal O_{\mathrm{FM}_m(X\mathbin{|}D)}
 \]
 inside the total quotient sheaf on each toroidal chart.
\item The boundary
...
\begin{corollary}[Punctured bar coinvariants and conformal-block comparison;
\ClaimStatusConditional]
\label{cor:conformal-blocks-punctured-bar}
Type signature: \textup{(}Open quadrant, punctured pointed-bar
coinvariant comparison presentation, Beilinson levels~$2$ and~$5$,
hypothesis package: punctured FM bar complex, module-action
nilpotence, Čech/bar resolution of the coinvariant functor,
degree-$0$ exactness, and a TUY/Hitchin or equivalent finite-rank
conformal-block comparison for the inserted modules\textup{)}.
The punctured bar complex computes the derived coinvariant complex.
Under the stated exactness and conformal-block comparison package, its
zeroth cohomology identifies with the space of conformal blocks:
\[
...
On the comparison lane, this is
$\mathbb{V}(\cA;\, \vec{\mathcal{M}};\, X)$
\textup{(}Definition~\ref{def:conformal-blocks}\textup{)}.
...
\medskip
For a fixed tangential log curve, the bordered FM compactification is:
...
\Delta_{S,B,j}^{\mathrm{mix}}.
\end{equation}
This is performed in the order prescribed above. In the relative
modular family over
$\overline{\mathcal M}_{g,r}$ one further blows up the pullbacks of
the nodal boundary strata $D_\Gamma^{\mathrm{nod}}$ from Step~5; those
relative strata are not boundary faces of the fixed-fiber space in
\eqref{eq:bordered-fm-result}.
\end{construction}
...
\begin{proposition}[Fixed and relative boundary decomposition;
\ClaimStatusProvedHere]
\label{prop:four-type-boundary}
\index{boundary strata!four types}
The codimension-one boundary of the fixed-fiber bordered FM
compactification
$\overline{C}{}^{\,\mathrm{oc}}_{k,\vec{m}}(X, D, \tau)$
decomposes into three collision types:
\begin{equation}\label{eq:boundary-three-types-fixed}
\partial\,\overline{C}{}^{\,\mathrm{oc}}_{k,\vec{m}}(X,D,\tau)
\;=\;
\partial_{\mathrm{I}}\,
\overline{C}{}^{\,\mathrm{oc}}_{k,\vec{m}}(X,D,\tau)
\;\cup\;
...
\overline{C}{}^{\,\mathrm{oc}}_{k,\vec{m}}(X,D,\tau).
\end{equation}
For the relative bordered FM compactification over the
Deligne--Mumford boundary of $\overline{\mathcal M}_{g,r}$, the
pullbacks of the nodal boundary divisors add a fourth type:
\begin{equation}\label{eq:boundary-four-types}
\partial\,\overline{C}{}^{\,\mathrm{oc,rel}}_{g;k,\vec{m}}
\;=\;
\partial_{\mathrm{I}}\,
\overline{C}{}^{\,\mathrm{oc,rel}}_{g;k,\vec{m}}
\;\cup\;
\partial_{\mathrm{II}}\,
\overline{C}{}^{\,\mathrm{oc,rel}}_{g;k,\vec{m}}
\;\cup\;
...
listed types. A codimension-one face is defined by one
...
\item in the relative modular family only, $t_e = 0$ for some edge~$e$
of a stable graph: nodal clutching (Type~IV).
\end{enumerate}
...
independent local coordinates in the relative family; on a fixed curve
there is no $t_e$ coordinate, hence no Type~IV face. Exhaustiveness
follows from the compactness of
$\overline{C}{}^{\,\mathrm{oc}}_{k,\vec{m}}$ in the fixed case and of
the relative compactification in the modular case: every sequence
...
= \overline{C}_k(X)$.
For a fixed curve only Type~I faces are present in the closed sector;
Type~II and Type~III are \emph{new} in the bordered setting and
encode the open-string sector. Type~IV faces occur only in the
relative modular family where the underlying curve varies. Thus the
fixed bordered boundary has Types~I--III, matching the Swiss-cheese
operad structure of Voronov~\cite{Voronov99}, while the relative
family adds nodal clutching faces.
\end{remark}
...
The fixed-fiber bordered FM compactification of
\S\ref{sec:bordered-fm} supplies the collision part of the
open-closed differential by Stokes' theorem on its boundary. The
full \emph{modular Maurer--Cartan equation with clutching}, the master
equation
$d\Theta + \tfrac{1}{2}[\Theta, \Theta]
+ \hbar\Delta_{\mathrm{clutch}}(\Theta) = 0$
governing the genus expansion of chiral correlators on
punctured stable curves, lives in the modular open-closed convolution
algebra. Its Type~IV terms are relative-moduli terms: they are supplied
by the separating and non-separating parts of the modular operad
square-zero identity, not by Stokes' theorem on a fixed curve.
...
\begin{proof}
The proof has two inputs, and the conditional status of the theorem is
attached to their compatibility in the open-closed relative family.

\emph{Source 1: fixed-fiber Stokes.}
On the fixed tangential log curve $(X,D,\tau)$, Stokes' theorem on
$\overline{C}{}^{\,\mathrm{oc}}_{k,\vec{m}}(X,D,\tau)$ sees only the
collision faces of
Proposition~\ref{prop:four-type-boundary}, namely Types~I--III. This
geometric calculation gives the open-closed differential
$d=d_{\mathrm{int}}+d_{\mathrm{bdy}}+d_{\mathrm{mix}}$ applied to
$\Theta_\cA$.

\emph{Source 2: modular-operad square-zero.}
The genus-completed open-closed bar differential
$D_\cA^{\mathrm{oc}}=\dzero+\Theta_\cA$ is part of the
$\mathrm{FCom}^{\mathrm{oc}}$-algebra structure invoked in
Definition~\ref{def:modular-twisting-morphism-oc}. The assertion that
this relative open-closed structure squares to zero is the
open-closed extension of Theorem~\ref{thm:bar-modular-operad}. Its
Getzler--Kapranov edge decomposition supplies the separating bracket
and non-separating clutching terms. Thus Type~IV is a relative-moduli
or algebraic modular-operad contribution, not a boundary face of the
fixed-fiber Stokes chain.

\medskip
\noindent\textsc{Fixed-fiber Stokes setup.}
Consider a smooth chain
$\sigma \in C_{2k + |\vec{m}|}
(\overline{C}{}^{\,\mathrm{oc}}_{k,\vec{m}}(X,D,\tau))$. The
...
codimension-one faces of~$\partial\sigma$. By the fixed-fiber clause
of Proposition~\ref{prop:four-type-boundary}, these faces decompose
into Types~I--III. They produce exactly the three components of
$d\,\Theta_\cA$.
...
\medskip
\noindent\textsc{Relative modular contribution:
bracket $\tfrac{1}{2}[\Theta_\cA, \Theta_\cA]^{\mathrm{oc}}$
and clutching $\hbar\,\Delta_{\mathrm{clutch}}(\Theta_\cA)$.}

The nodal terms are not produced by the preceding fixed-fiber Stokes
calculation. They enter through the relative modular family and the
genus-completed modular operad algebra structure of the bar complex
(Theorem~\textup{\ref{thm:bar-modular-operad}}). In the relative
bordered FM family, the Type~IV divisors split into two geometrically
distinct classes. The \emph{separating} nodal divisors Type~IV(a) are
the source of the bracket
$\frac{1}{2}[\Theta_\cA, \Theta_\cA]^{\mathrm{oc}}$: each separating
node joins two distinct components, composing two operations via the
interior pairing. The \emph{non-separating} clutching divisors
Type~IV(b) contribute
$\hbar\,\Delta_{\mathrm{clutch}}(\Theta_\cA)$ by tracing two legs of
one operation. Equivalently, these terms are the separating and
non-separating edge parts of
$(D_\cA^{\mathrm{oc}})^2=0$.
...
Combining the fixed-fiber Stokes calculation for collision faces with
the relative modular-operad square-zero identity gives, componentwise:
\begin{align}
0
&=
\underbrace{\text{Type~I} + \text{Type~II}
 ...
 \;+\;
 \underbrace{\text{separating modular edges}}_{\displaystyle
 \frac{1}{2}[\Theta_\cA, \Theta_\cA]^{\mathrm{oc}}}
 \;+\;
 \underbrace{\text{non-separating modular edges}}_{\displaystyle
 \hbar\,\Delta_{\mathrm{clutch}}(\Theta_\cA)}.
 \label{eq:modular-two-source-assembly}
\end{align}
The first brace is the value of
$\int_{\partial\sigma}\omega^{(g)}$ on the fixed-fiber boundary
Types~I--III. The second and third braces are the separating and
non-separating parts of the Getzler--Kapranov decomposition
\eqref{eq:gk-decomposition} for the relative modular
$\mathrm{FCom}^{\mathrm{oc}}$ structure. Thus the displayed identity
is not a single fixed-curve Stokes formula; it is the compatibility of
fixed-fiber collision Stokes with the relative modular square-zero
operator. This compatibility is the named open-closed relative-family
hypothesis in the theorem. We obtain
\eqref{eq:modular-mc-clutching}:
\[
d\,\Theta_\cA
+ \tfrac{1}{2}[\Theta_\cA, \Theta_\cA]^{\mathrm{oc}}
+ \hbar\,\Delta_{\mathrm{clutch}}(\Theta_\cA)
...
\begin{itemize}
\item \emph{Genus 0:} affine/formal representatives
\(\eta_{ij}=d\log(z_i-z_j)\), glued on \(\mathbb P^1\) with the
projective linear relations
\item \emph{Genus 1:} Elliptic forms
\[
\eta_{ij}^{(1)}
=
\left[
...
\right]_{u=z_i-z_j}d(z_i-z_j)
\]
with modular parameter~$\tau$
...
d(z_i-z_j)
\]
where $\theta_1(z|\tau) = -i\sum_{n \in \mathbb{Z}}(-1)^n q^{(n+1/2)^2/2}e^{i\pi(2n+1)z}$ with $q = e^{2\pi i\tau}$ and period lattice $\Lambda_\tau=\mathbb Z+\tau\mathbb Z$.
\end{definition}
...
holomorphic as a coefficient: the regularization is a $(1,0)$-form
with real-analytic coefficient. Its $\bar\partial$ contributes the
smooth term in
$d\eta_{ij}^{(1)}=2\pi i(\delta_{\Delta_{ij}}-q_{ij}^{*}\omega_\tau)$,
and this Arakelov term is the source of the $(1,1)$ period class.
\end{proposition}
...
\begin{definition}[Finite-window residue datum; \ClaimStatusDefinitional]
...
\begin{definition}[Chiral endomorphisms; \ClaimStatusDefinitional]\label{def:chiral-endo}
...
\begin{corollary}[Affine scalar residue differential squares to zero; \ClaimStatusProvedHere]
\label{cor:bar-d-squared-zero}
On an affine genus-$0$ screen, the scalar Orlik--Solomon residue
differential is square-zero by the Arnold relations. For the
operator-valued chiral bar differential one must also impose the
Borcherds coefficient identity; for $X=\mathbb{P}^1$ or
$g(X)\geq 1$, one must add the projective or period relations of
Proposition~\ref{prop:arnold-higher-genus}.
\end{corollary}
\begin{proof}
The scalar statement is Theorem~\ref{thm:arnold-iff-nilpotent}. The
operator-valued bar statement is the Arnold--Borcherds cancellation of
...
d(z_i-z_j),
\]
where $\vartheta_1(z|\tau)$ is the odd Jacobi theta function for the
period lattice $\mathbb Z+\tau\mathbb Z$.
\end{theorem}
...
cocycle witness of
Theorem~\ref{thm:confspaces-ran-space-cocycle-witness} proves that
