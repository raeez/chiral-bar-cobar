...
The wave-14 reading of this chapter is that $\eta = d\log(z_i - z_j)$
is the connection one-form of the universal flat connection
$\nabla_{\mathrm{Arnold}}$ on $\operatorname{Conf}_n(\mathbb{C})$,
and that the bar differential is its pullback by the KZ functor:
\[
   d_{\mathrm{bar}} \;=\; \mathrm{KZ}^*\bigl(\nabla_{\mathrm{Arnold}}\bigr).
\]
The chapter delivers the geometric ingredients on which this
identity rests: $\overline{C}_n(X)$ as the host
(\S\ref{sec:FM-compactification}), the logarithmic propagator
(Definition~\ref{def:log-forms}), the Arnold relation
(Theorem~\ref{thm:arnold-relations}) as the flatness of
$\nabla_{\mathrm{Arnold}}$, the residue maps
(Theorem~\ref{thm:residue-operations}) as the ambient calculus
that turns ``propagator wedged with OPE'' into ``bar
differential.'' The KZ pullback identity itself is the
Climax Theorem of Chapter~\ref{chap:climax-platonic}; the
ingredients constructed here are the inputs to that theorem.
...
The configuration space $C_n(\Sigma_g)$ is non-compact. The Fulton--MacPherson compactification~\cite{} resolves this by iterated blow-ups along diagonals, recording collision rates and angles in exceptional divisors. For a logarithmic variant on punctured and nodal curves, see Mok~\cite{Mok25}.

\subsubsection{Iterated blow-up construction}

\begin{theorem}[Fulton--MacPherson compactification at genus \texorpdfstring{$g$}{g} \cite{}; \ClaimStatusProvedElsewhere]\label{thm:FM}
...
\begin{corollary}[Boundary divisors in the FM compactification \cite{}; \ClaimStatusProvedElsewhere]\label{cor:dimension-strata}
...
\begin{theorem}[Boundary stratification \cite{}; \ClaimStatusProvedElsewhere]
...
not the $(h_i+h_j-h_k+1)$-th. In the language , the $d\log$
...
boundary-strata compatibility are established in \cite{}; the statements
...
Fulton--MacPherson framework \cite{}, and the operadic packaging used here
...
is independent of the ordering~\cite{}.
\end{lemma}
...
manifolds with corners (see~\cite{}, Appendix~B for
...
\begin{theorem}[Stokes on configuration spaces \cite{}; \ClaimStatusProvedElsewhere]
...
\cite{}). When we form the fiber product
...
blowups (Fulton--MacPherson~\cite{}):
...
\begin{proposition}[Transition functions \cite{}; \ClaimStatusProvedElsewhere]\label{prop:transition-functions}
