...
and, on the Arnold-local ordered genus-$0$ subcategory, the bar
differential is its pullback along the KZ functor:
\[
 d_{\mathrm{bar}} \;=\; \mathrm{KZ}^*\bigl(\nabla_{\mathrm{Arnold}}\bigr).
\]
Six geometric inputs carry this identity: $\overline{C}_n(X)$
...
it is the \emph{only} relation on the scalar propagator classes. On
...
$S_1, \ldots, S_m$ are \emph{nested} subsets
$S_1 \subset S_2 \subset \cdots \subset S_m$, there are local analytic
coordinates:
...
of Chapter~\ref{chap:climax-platonic} is then a genus-$0$
Arnold-local corollary, and
...
degree~$2$ on that affine screen is the Arnold relation
...
on $\mathbb{P}^1$ and on positive-genus curves it is supplemented by
the projective and Totaro--period terms of
Proposition~\ref{prop:arnold-higher-genus}.
\end{proposition}
...
When convergence fails, the available regularisations are:
\begin{itemize}
...
\begin{proof}
Proceed by induction on $n$. For $n = 2$, the single NBC element is
$\eta_{12}$ with $d\eta_{12} = 0$.
...
\end{theorem}
\begin{proof}
The cell decomposition is explicit. Points in $C_\pi$ have configuration type
\[
x_{B_1} < x_{B_2} < \cdots < x_{B_k}
\]
where $x_{B_i}$ denotes the common position of points in block $B_i$.
The geometric dimension of $C_\pi$ is $k$ (the number of free position
parameters). The formula $\dim C_\pi = n - k$ refers to the cochain
degree: partitioning $[n]$ into $k$ blocks imposes $n-k$ collision
conditions, so $C_\pi$ has codimension $n-k$ in $C_n(\mathbb{R})$; this
codimension is the grading for the cellular cochain complex.
...
A \emph{nested} blow-up is needed:
\begin{enumerate}
...
\emph{Case \textup{(d)}.} Seven convergent constructions place the
chiral algebra on $E^{\mathrm{nod}}_{24}$.
...
\emph{Bar-Euler computation.} The bar-Euler characteristic
...
\begin{proof}
The map is exhibited at chain level, where the quasi-isomorphism is
checked.
...
each proved at chain level with explicit witnesses.
\end{corollary}
...
The five statements of this subsection are \emph{chain-level}
...
$\infty$-lift and is compatible with the chain-level construction in
