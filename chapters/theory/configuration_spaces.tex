...
\textbf{Claim:} $S_1$ and $S_2$ cannot be separated by open sets.

\textbf{Proof:} In the Ran topology, an open set $U \ni S_2$ contains all finite subsets sufficiently close to $S_2$ in the Hausdorff metric, \emph{including subsets of different cardinality obtained by colliding points}.  In particular, $U$ contains $\{0, 1/k\}$ for all $k$ sufficiently large (since $\{0, 1/k\}$ is close to $\{0, 1\}$ in the sense that $0$ is near $0$ and $1/k$ can be added near $0$; more precisely, $U$ contains all 2-element subsets $\{x, y\}$ with $x$ near $0$ and $y$ near $1$).

However, the key point is that as $k \to \infty$, $\{0, 1/k\} \to \{0\} = S_1$ in the Ran topology (the second point collides with the first).  Every neighborhood $V \ni S_1$ contains singletons $\{x\}$ for $x$ near $0$, and since the Ran topology identifies the limit of colliding points, $V$ eventually contains $\{0, 1/k\}$ as well.

Therefore any neighborhood of $S_2$ contains elements arbitrarily close to $S_1$, so $\mathrm{Ran}(\mathbb{R})$ is not Hausdorff.
