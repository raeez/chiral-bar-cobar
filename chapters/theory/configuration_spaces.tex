...
$\sum_g \hbar^g$ and the modular operad structure of the bar
...
This is the crucial structural point.  In
...
\begin{proof}
This is Theorem~\ref{thm:arnold-relations}, Part~2.  The proof there is valid for any triple of distinct indices $i,j,k \in \{1,\ldots,n\}$ and any $n$: the cyclic identity $z_{ij} + z_{jk} + z_{ki} = 0$ forces all three numerator $2$-forms to coincide (as $dz_{ij} \wedge dz_{jk}$), after which the coefficient sum vanishes by the same cyclic identity.  The relation therefore holds in $\Omega^2(C_n(X))$ for every $n \geq 3$.
\end{proof}
...
By the same calculation (replacing $i,j,k$ by $2,3,1$ and $3,1,2$ respectively), the other two terms are:
\begin{align}
\eta_{23} \wedge \eta_{31} &= \frac{dz_2 \wedge dz_3 - dz_2 \wedge dz_1 + dz_3 \wedge dz_1}{(z_2-z_3)(z_3-z_1)} = \frac{dz_1 \wedge dz_2 - dz_1 \wedge dz_3 + dz_2 \wedge dz_3}{(z_2-z_3)(z_3-z_1)}\\
\eta_{31} \wedge \eta_{12} &= \frac{dz_3 \wedge dz_1 - dz_3 \wedge dz_2 + dz_1 \wedge dz_2}{(z_3-z_1)(z_1-z_2)} = \frac{dz_1 \wedge dz_2 - dz_1 \wedge dz_3 + dz_2 \wedge dz_3}{(z_3-z_1)(z_1-z_2)}
\end{align}
All three numerator $2$-forms are identical: $\Omega := dz_1 \wedge dz_2 - dz_1 \wedge dz_3 + dz_2 \wedge dz_3$.  Factoring:
\begin{equation}
\eta_{12} \wedge \eta_{23} + \eta_{23} \wedge \eta_{31} + \eta_{31} \wedge \eta_{12} = \frac{\Omega}{(z_1{-}z_2)(z_2{-}z_3)(z_3{-}z_1)} \bigl[(z_3{-}z_1) + (z_1{-}z_2) + (z_2{-}z_3)\bigr] = 0
\end{equation}
by the telescoping identity $(z_3-z_1)+(z_1-z_2)+(z_2-z_3) = 0$.
