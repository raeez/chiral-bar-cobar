...
\begin{theorem}[Boundary strata of \texorpdfstring{$\overline{\mathcal{M}}_{g,n}$}{M-bar(g,n)} {\cite{DeligneM69,Knu83}}; \ClaimStatusProvedElsewhere]\label{thm:boundary-higher-genus}
...
\emph{Exhaustiveness.}
Every point of $\partial\overline{C}_n(\Sigma_g) = \overline{C}_n(\Sigma_g) \setminus C_n(\Sigma_g)$ belongs to at least one stratum: either some subset of marked points has collided (type 1), or the underlying curve has degenerated (type 2), or both.  This follows from the properness of $\overline{\mathcal{M}}_{g,n}$ (Deligne--Mumford--Knudsen \cite{DeligneM69,Knu83}): every sequence in $C_n(\Sigma_g)$ has a subsequence converging to a stable $n$-pointed curve, and the limit is classified by its dual graph and the collision data of the marked points.
\end{proof}
