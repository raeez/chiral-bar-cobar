...
\noindent\emph{Governing question.} Which geometric identities on
configuration spaces force the algebraic properties used later
(nilpotence, residue compatibility, Verdier pairing), and which are
imported background results?  The chapter isolates that geometry as the
load-bearing substrate for Theorems~A--C.

\begin{remark}[Semantic levels; Convention~\ref{conv:hms-levels}]
\label{rem:config-spaces-hms}
This chapter is \textbf{M-level} throughout: it builds the explicit geometric models (Fulton--MacPherson compactifications, log forms, Orlik--Solomon algebra, boundary stratifications) that underlie all M-level proofs in later chapters.  The H-level abstraction (factorization homology over $\operatorname{Ran}(X)$, Ayala--Francis framework) uses these spaces as its geometric input.
\end{remark}
