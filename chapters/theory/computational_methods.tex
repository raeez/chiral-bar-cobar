...
\begin{lemma}[Taylor recursion for the formal square root;
\ClaimStatusProvedHere]
\label{lem:comp-sqrt-recursion}
Write $\sqrt{\ShadowMetric(t)} = \sum_{n \geq 0} a_n\, t^n$.
Then
...
\begin{proof}
Let $A(t)=\sum_{n\ge0}a_nt^n$.  The defining identity
$A(t)^2=\ShadowMetric(t)$ gives, by comparing coefficients of
$t^0,t^1,t^2$, the three initial values in
\eqref{eq:comp-sqrt-recursion}.  For $n\ge3$ the coefficient of
$t^n$ in $\ShadowMetric(t)$ vanishes, hence
$2a_0a_n+\sum_{j=1}^{n-1}a_ja_{n-j}=0$.  Solving for~$a_n$ gives
\eqref{eq:comp-sqrt-higher}.
\end{proof}
...
The convolution recursion gives the weighted Riccati-metric
coefficients
\begin{align*}
R_2 &= \frac{c}{2}, \\
R_3 &= 2, \\
R_4 &= \frac{10}{c(5c+22)}, \\
R_5 &= \frac{-48}{c^2(5c+22)}, \\
R_6 &= \frac{80(45c+193)}{3\,c^3(5c+22)^2}, \\
R_7 &= \frac{-2880(15c+61)}{7\,c^4(5c+22)^2}, \\
R_8 &= \frac{80(2025c^2 + 16470c + 33314)}{c^5(5c+22)^3}, \\
R_9 &= \frac{-1280(2025c^2 + 15570c + 29554)}{3\,c^6(5c+22)^3}, \\
R_{10} &= \frac{256(91125c^3 + 1050975c^2 + 3989790c + 4969967)}
 {c^7(5c+22)^4}.
\end{align*}
...
branch point.  An ordered residue package
$\mathsf H_{\mathrm{res}}(\Vir_c;X)$ and its chain comparison
determine the geometric equality
$S_r(\Vir_c;\mathsf H_{\mathrm{res}})=R_r(\Vir_c)$.
\end{example}
...
\begin{proof}
The three methods are the direct Drinfeld--Sokolov transfer, the MC
recursion on the transferred $T$-line data, and the shadow-metric
reconstruction from the same transferred invariants.  For each of the
$4\times5$ pairs $(N,k)$, each method produces an exact rational value
for every $S_r$, $2\le r\le20$.  The finite comparison is implemented by
$N\in\{2,3,4,5\}$, $k\in\{1,2,3,5,10\}$, and $2\le r\le20$.
Form the residual vector in $\mathbb Q^{760}$ whose two entries for
each $(N,k,r)$ are
$S_r^{\mathrm{DS}}-S_r^{\mathrm{MC}}$ and
$S_r^{\mathrm{DS}}-S_r^{\mathrm{metric}}$.  Exact rational
evaluation gives the zero vector.  Hence the three outputs agree
entrywise on precisely this finite parameter set.
\end{proof}
...
\begin{example}[DS for $\fsl_2 \to \mathrm{Vir}$ at the Ising level
$k = -2/3$]
\label{ex:comp-ds-sl2-k10}
The DS formula $c = 1 - 6(k+1)^{2}/(k+2)$ gives $c = 1/2$ at the
admissible level $k = -2/3$.
Affine data: $\kappa(\AffKM{sl}_2, -2/3)
= \dim(\fsl_2)(k + h^\vee)/(2h^\vee) = 3 \cdot (4/3)/4 = 1$,
...
an integer sequence encoding the intrinsic bar-dual coalgebra. In a
finite Verdier window this gives coordinates for the Koszul-dual
algebra. For chirally
...
\begin{proposition}[CE page and the bar comparison hypothesis]
\label{prop:comp-ce-bar}
\ClaimStatusConditional
For $\cA = V_k(\fg)$ at non-critical level, in the Open quadrant,
standard bar presentation, and Beilinson level~$2$, the leading-pole
PBW filtration on the chiral bar complex has Chevalley--Eilenberg page
\begin{equation}\label{eq:comp-ce-bar}
 E_2^{p,H}
 \;\cong\; H^p_{\CE}(\fg_-, \C)_H,
\end{equation}
where $\fg_- = \fg \otimes t^{-1}\C[t^{-1}]$ is the
negative-mode Lie algebra.  Equality with
$H^\bullet(\barBch(\cA))$ is asserted only under the
OS-corrected PBW-collapse hypothesis package: the Orlik--Solomon
configuration-space factor has been included and no higher
subleading-pole differential survives in the finite window under
comparison.
\end{proposition}
...
numbers sum to zero. This proves the displayed CE page. Passing from
that page to the actual chiral bar cohomology is a separate collapse
assertion: for multi-generator algebras the Orlik--Solomon factor
changes the bar chain groups, and higher OPE poles may contribute
later PBW differentials. Those effects are part of the stated
hypothesis package, not consequences of the leading-pole CE page alone.
\end{proof}
...
\begin{theorem}[Finite-window comparison for bar cohomology]
\label{thm:comp-three-way-bar}
\ClaimStatusConditional
In the Open quadrant, standard bar presentation, Beilinson level~$2$,
under the OS-corrected PBW-collapse hypothesis package, the direct bar
matrices, closed-form sequence formulae, and CE/Zhu comparisons agree
in the degree-$1$ through degree-$6$ windows displayed above whenever they compute the same
typed object.  The unconditional rows are the single-generator
free-field and Virasoro windows and the affine low-degree windows
where the OS factor is trivial or explicitly included.  The
multi-generator $\cW_N$ rows are conditional on the OS-corrected
PBW-collapse package of Proposition~\ref{prop:comp-ce-bar}.
\end{theorem}

\begin{proof}
For each displayed algebra and degree, write the bar differential on
the ordered word basis, compute $\dim\ker d_n-dim\operatorname{im}
d_{n-1}$, and compare it with the listed closed-form sequence.  The
difference vector is zero in the displayed finite windows after the
typed-object distinction is imposed: CE
computes the leading-pole page, while the chiral bar computation also
contains the Orlik--Solomon and subleading-pole data.  The theorem is
therefore a finite-window conditional comparison, not an
all-landscape collapse theorem.
\end{proof}
...
$F_g^{\mathrm{osc}} = r \cdot \lambda_g^{\mathrm{FP}}$ \textup{(UNIFORM-WEIGHT)}.
...
The shadow obstruction tower extends to superconformal algebras.
...
\hline
 ...
 \Gamma_5 & (0,3){+}(1,1) & 2 & 1 & 2 & \text{tadpole} \\
...
\begin{remark}[Algebraic consequence]
\label{rem:s3-vir-compute}
The exact cancellation in the preceding proof shows that
$S_3(\mathrm{Vir}_c)=2$ wherever $\kappa(\mathrm{Vir}_c)\ne0$.
The identity furnishes a finite
...
% =========================================================
\subsection{Finite ranges}
\label{subsec:comp-summary}
% =========================================================

\begin{table}[htbp]
\centering
\caption{Finite ranges of the exact calculations in this chapter.}
\label{tab:comp-methods-summary}
\small
\[
\begin{array}{@{}l|l|l@{}}
 \text{Invariant}&\text{parameter set}&\text{finite conclusion} \\
\hline
 S_r \; (r = 2 \ldots 30)
 & \text{15 displayed algebras}&\text{exact rational values} \\
 \dim H^n(\barBch) \; (n \leq 12)
 & \text{8 displayed algebras}&\text{finite matrix ranks} \\
 F_2
 & \text{10 displayed algebras}&\text{exact rational values} \\
 S_r(\cW_N) \; (\text{DS})
 & N=2,3,4,5;\ k=1,2,3,5,10;\ 2\le r\le20
 & \text{zero residual in }\mathbb Q^{760}
\end{array}
\]
\end{table}

\section{Synthesis}
\label{sec:cm-supplement}

\begin{remark}[Four finite arithmetic carriers]
\label{rem:cm-1}
Four distinct finite calculations enter the comparison with Volume~III:
(i) an exact persymmetric ZTE matrix with zero anti-diagonal and ten
distinct rational entries realizes the $\widetilde M_{24}$-twined
trace at order~$24$;
(ii) finite Feynman sums give $m_5=-80/9$, $m_7=-24320/81$, and
$m_8=33157760/19683$ from $G_5^{\mathrm{conn}}=775/5184$;
(iii) the coefficient identity
$\kappa_{\mathrm{BKM}}(\Phi_N)=c_N(0)/2$ is evaluated for
$N\in\{1,2,3,4,6\}$ against the Gritsenko series;
(iv) the Pasol--Zagier construction realizes
$R_{\mathrm{Sieg,dyn}}$ on the $\mathrm{Sp}_4(\mathbb Z)$ modular locus
\cite{PasolZagier2013,Gritsenko1999,Borcherds1992}.
Cross-ref Vol.~III Chapter~\ref{V3-ch:k3-chiral-bialgebra-platonic}.
\end{remark}

\begin{remark}[Typed scope of the three scalar calculations]
\label{rem:cm-2}
At the specified parameters, Lusztig specialization has $\ell=8$,
Mukai doubling has $c_+=4$ on the lattice $II_{4,20}$, and the three
linearly independent order-$\hbar^3$ pieces of the pentagon
coboundary sum to zero modulo $\Phi_{10}/\eta^{24}$.  The numerical
substitution $\hbar^2=-1/8$ and $K^{\kappa_{\mathrm{ch}}}=8$ gives
$\hbar^2K^{\kappa_{\mathrm{ch}}}=-1$.  These are differently typed
calculations; identifying their carriers requires additional comparison
maps.  See \cite{Mukai1987,Lusztig1990,EtingofKazhdan2007PartV} and
cross-ref
Chapter~\ref{V3-ch:k3-chiral-bialgebra-platonic}.
\end{remark}

\begin{remark}[$\Delta_5$ level, character, and $p = 2$ ramification]
\label{rem:cm-delta5-ramification}
The Arthur parametrisation of $\Delta_{10}$ (the Saito--Kurokawa
lift $\Delta_{10} = I_2(\Delta_{E_6})$ in the Arthur packet of
Vol~III Theorem~\ref{V3-thm:arthur-packet-H-delta5}) is unramified
everywhere because $\Delta_{10} \in S_{10}(\mathrm{Sp}_4(\Z))$ carries
the \emph{trivial} multiplier system. The Saito--Kurokawa partner
$\Delta_5$ at weight $5$ is strictly a cusp form
\emph{twisted by the sign character}
$v_{\Delta_5} \colon \mathrm{Sp}_4(\Z) \to \{\pm 1\}$ of
Gritsenko--Nikulin 1998: $\Delta_5 \in S_5(\mathrm{Sp}_4(\Z), v_{\Delta_5})$.
The character $v_{\Delta_5}$ factors through
$\mathrm{Sp}_4(\Z/2) \cong S_6$ with kernel of index $2$; this factorisation
introduces a \emph{ramification locus at $p = 2$}, where the local Langlands
parameter requires the quadratic-twist refinement rather than the
unramified Satake recipe used at odd primes. At $p = 2$ the local
parameter for $\Delta_5$ is the non-tempered CAP representation
$\pi_2(\Delta_5) = \pi(\Delta_{E_6})_2 \otimes (\mathrm{sgn} \boxtimes
\mathrm{Sym}^1)$ with $\mathrm{sgn}$ the non-trivial character of
$\mathbb{Q}_2^\times / (\mathbb{Q}_2^\times)^2$ pulled back along
$v_{\Delta_5}$, while at odd primes the parameter reduces to the
Arthur formula $\psi_{\Delta_{10}} = \phi_{\Delta_{E_6}} \boxtimes
\mathrm{Sym}^1$. Primary:
Ibukiyama 1984 ``Paramodular forms and the Saito--Kurokawa lifting''
(Osaka J.\ Math.); Gritsenko--Nikulin 1998 ``Automorphic forms and
Lorentzian Kac--Moody algebras II'' (Int.\ J.\ Math.); Maass 1979
``Siegel modular forms and Dirichlet series'' (LNM 216).
Cross-ref Vol.~III
Chapter~\ref{V3-ch:k3-chiral-bialgebra-platonic}.
\end{remark}

\begin{remark}[Maass spin cover, \emph{not} paramodular $K(1)$]
\label{rem:cm-maass-spin}
The arithmetic group carrying $\Delta_5$ is the \emph{Maass spin cover}
$\widetilde{\mathrm{Sp}_4(\Z)}$, the unique $\Z/2$-cover of
$\mathrm{Sp}_4(\Z)$ fixing $v_{\Delta_5}$: writing
$1 \to \Z/2 \to \widetilde{\mathrm{Sp}_4(\Z)} \to \mathrm{Sp}_4(\Z) \to 1$
with the $\Z/2$ exactly $\ker(v_{\Delta_5})^c$.  Thus the level-one
paramodular group \(K(1)=\mathrm{Sp}_4(\Z)\) alone does not encode the
multiplier, and one has
$\Delta_5 \in S_5(\widetilde{\mathrm{Sp}_4(\Z)})_{\mathrm{triv}}$
on which the spin cover acts with trivial multiplier
(Maass 1979, \S 4, identification of spin weights).
The Maass spin cover is distinct from a metaplectic cover
of $\mathrm{Sp}_4$: the metaplectic $\mathrm{Mp}_4$ is a $\Z/2$-cover
controlled by the \emph{Kubota cocycle}, while
$\widetilde{\mathrm{Sp}_4(\Z)}$ is controlled by the
\emph{sign cocycle} $v_{\Delta_5}$. At $p = 2$, the two covers
differ by a quadratic-residue phase; at odd $p$ they coincide.
Primary: Maass 1979 LNM 216 \S 4; Gritsenko--Nikulin 1998 \S 2.4;
Ibukiyama 1984. Cross-ref Vol.~III
Chapter~\ref{V3-ch:k3-chiral-bialgebra-platonic} and Vol.~I
Chapter~\ref{chap:computational-methods}.
\end{remark}

\begin{remark}[Order-$6$ cocycle $6A$ vs $6B$]
\label{rem:cm-M24-order6}
The refinement of Remark~\ref{rem:cm-2} to the order-$6$ sector of
$M_{24}$ distinguishes two classes: $6A$ (cycle shape
$1^2 2^2 3^2 6^2$, umbral shift $m_{6A} = 2$) and $6B$ (cycle shape
$6^4$, umbral shift $m_{6B} = 6$), with $\chi_{23}(6A) = -1$ and
$\chi_{23}(6B) = +1$ \cite{ChengDuncanHarvey2014,ConwayCurtisNortonParkerWilson1985}.
Their pentagon-$\hbar^3$ cocycles lie in $H^3(\Z/6, U(1)) = \Z/6$ at
distinct classes $2$ and $3$ respectively. The sum
$2 + 3 = 5 \equiv -1 \pmod 6$ is the restriction of the universal
identity $\hbar^2 \cdot K^{\kappa_{\mathrm{ch}}} = -1$ to the order-$6$
sector. This is the order-$6$ cyclic restriction in Vol~III's
pentagon--umbral agreement theorem. Cross-ref Vol.~II
