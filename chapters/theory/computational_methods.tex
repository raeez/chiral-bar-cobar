...
%
%   The shadow tower is a square root.
%
% ============================================================
...
\begin{quote}\itshape
The shadow tower of a chirally Koszul algebra is a square root.
\end{quote}
...
This chapter develops the consequences.  The structure is not
three independent methods that happen to agree, but four languages
for a single object:
\begin{center}
\small
...
rational numbers defined by the arity-$r$ projection of
$\Theta_\cA$~along~$L$.
...
The shadow tower sees only the first Gram determinant.  The
quartic contact invariant $Q^{\mathrm{ct}}_{\mathrm{Vir}}
= 10/[c(5c+22)]$ has poles at $c = 0$ and $c = -22/5$: the
zeros of $\det G_4 = c^2(5c+22)/2$.  The shadow tower's poles
...
section: the $t^2$ prefactor is the arity offset, reflecting
that the shadow tower begins at arity~$2$.

The Gaussian decomposition
$\ShadowMetric = (2\kappa + 3\alpha t)^2 + 2\CritDisc\, t^2$
separates the free part (the perfect square, contributing a
polynomial shadow tower) from the interacting part (the
$\CritDisc$~correction, creating infinite depth).  This
...
\end{array}
\]
The self-dual point $c = 13$ is the unique minimum of
$\ShadowGrowth(c)$ on the positive real axis, with
$\ShadowGrowth(13) \approx 0.467 < 1$.
\end{example}
...
The \emph{Borel transform} of the shadow tower is
...
quadratic shadow metric, and therefore that the shadow tower
...
The DS ghost system has
$\kappa_{\mathrm{ghost}} = N(N-1)/2$, independent of~$k$.
For $N = 2$: the $W$-algebra anomaly ratio is $\kappa/c = 1/2$,
matching the free boson, and the kappa deficit
$C(2,k) = \kappa(\AffKM{sl}_2, k) - \kappa(\mathrm{Vir}, k) = 1$
is $k$-independent.  For $N \geq 3$: the anomaly ratio is
$\kappa/c = H_N - 1 \neq 1/2$ (where
$H_N = 1 + 1/2 + \cdots + 1/N$), and the deficit depends
on~$k$ through the $W$-algebra central charge.
\end{remark}
...
produce identical rationals through arity~$20$.
\end{example}
...
\end{equation}
where $r(v_i, v_{i+1})$ is the simple-pole residue.  The
$d\log$ kernel absorbs one pole order: the $r$-matrix
has poles one degree lower than the OPE.
...
cohomology dimensions through weight~$12$ agree across the
direct matrix, CE, and Zhu methods.
\end{theorem}
...
\item \textup{Arity~$3$}:
  ...
  The MC equation at arity~$3$ is the CE cocycle condition
  $d_{\CE}(f) = 0$.
\item \textup{Arity~$4$}:
...
this sense, the arity-$3$ MC equation.
\end{remark}
...
of $\det G$.  The shadow tower's poles are the
...
The shadow tower programme extends to superconformal algebras.
...
\begin{proof}
% RECTIFICATION-FLAG: the individual coset pieces
% kappa(fermions)=1/2 and kappa(U(1))=k/2+1 should be
% verified against the precise Kazama-Suzuki embedding.
% The final answer (k+4)/4 is verified independently by
% c + c' = 6 complementarity (see below).
...
\]
Self-dual at $c = 3$ (the free-field limit $k \to \infty$,
unreachable at finite level).
...
\ClaimStatusProvedHere
The spectral flow automorphism preserves the shadow tower:
...
universal constant.  Self-dual at $c = 13$: both denominator
...
Two structural results through arity~$7$:
...
% -----------------------------------------------------------------
\subsection{Shadow tower coefficients}
\label{subsec:comp-shadow-table}
% -----------------------------------------------------------------

\begin{table}[htbp]
\centering
\caption{Shadow tower coefficients on the $T$-line.
...
shadow data is treated in Chapter~\ref{chap:betagamma}.
\end{remark}
...
The $\beta\gamma$ dimensions are the Fibonacci numbers.  The
Virasoro dimensions begin at weight~$2$ and grow as Motzkin
differences.  The $\AffKM{sl}_2$ dimensions grow as (modified)
Riordan numbers.
\end{remark}
...
DS-transferred tower has infinite depth.  The affine input
has depth~$3$.  The quartic contact invariant
