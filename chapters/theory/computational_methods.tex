...
\section{Synthesis}
\label{sec:cm-wave13-DNA}

\begin{remark}[Wave 14 compute modules as algorithmic Wave~13 DNA carriers]
\label{rem:cm-DNA-1}
The Wave~14 compute modules scaffolded for Volume~III each carry a
Wave~13 DNA imprint at the algorithmic level:
...
$\hbar^2 = -1/8$ is computationally verified across the Wave~14 modules:
...
\begin{remark}[Wave 15: $\Delta_5$ level, character, and $p = 2$ ramification]
\label{rem:cm-wave15-delta5-ramification}
The Wave~14 Arthur parametrisation of $\Delta_{10}$ (the Saito--Kurokawa
lift $\Delta_{10} = I_2(\Delta_{E_6})$, cf.\ the Wave~14 compute module
\texttt{k3\_yangian\_wave14\_arthur\_hecke\_delta10.py}) is unramified
...
$v_{\Delta_5}$, while at odd primes the parameter reduces to the Wave~14
...
\begin{remark}[Wave 15: Maass spin cover, \emph{not} paramodular $K(1)$]
\label{rem:cm-wave15-maass-spin}
The arithmetic group carrying $\Delta_5$ is \emph{not} the paramodular
group $K(1) = \mathrm{Sp}_4(\Z)$ at level one (Wave~14 retracted the
earlier ``paramodular $K(N)$'' claim for $N = 1$ explicitly). The
...
Chapter~\ref{ch:k3-chiral-bialgebra-platonic} and Vol.~I
Chapter~\ref{chap:computational-methods}.
\end{remark}

\begin{remark}[Wave 15: order-$6$ cocycle $6A$ vs $6B$]
\label{rem:cm-wave15-M24-order6}
The refinement of Remark~\ref{rem:cm-DNA-2} to the order-$6$ sector of
...
sector. Computationally verified by
\texttt{k3\_yangian\_wave14\_M24\_umbral\_cocycle\_order6.py} in the
Wave~15 extension. Cross-ref Vol.~II
