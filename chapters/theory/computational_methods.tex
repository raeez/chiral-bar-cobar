...
\texttt{systematic\_comparison} in
\texttt{compute/lib/ds\_transferred\_shadows.py}: with
$N\in\{2,3,4,5\}$, $k\in\{1,2,3,5,10\}$, and
\texttt{max\_arity=20}, it compares all entries by exact rational
equality and returns \texttt{all\_agree=True}.  Hence the three exact
outputs agree entrywise.
\end{proof}
...
under the OS-corrected PBW-collapse hypothesis package, the finite
windows implemented by the local bar-dimension
engines, the direct bar matrices, closed-form sequence formulae,
and CE/Zhu comparison routines agree whenever they compute the same
...
\begin{proof}
The direct matrices are built in
\texttt{compute/lib/bar\_cohomology\_dimensions.py}.  The independent
closed-form witnesses live in \texttt{compute/lib/bar\_complex.py} and
are tested in \texttt{compute/tests/test\_bar\_complex.py} and
\texttt{compute/tests/test\_bar\_cohomology\_dimensions.py}.  The
single-generator CE comparison is isolated in
\texttt{compute/lib/ce\_vs\_chiral\_bar\_reconciliation\_engine.py}
and its test file.  These computations agree in the implemented
finite windows after the typed-object distinction is imposed: CE
...
\begin{remark}[Compute modules as algorithmic carriers]
\label{rem:cm-1}
The compute modules used for Volume~III each carry a
K3-chiral imprint at the algorithmic level:
(i)~\texttt{zte\_t\_matrix.sage} computes the exact persymmetric ZTE $T$
matrix with zero anti-diagonal and $10$ distinct rational entries,
realising the $\widetilde M_{24}$-twined trace at order $24$;
(ii)~\texttt{shadow\_tower\_m5\_m8.sage} computes $m_5 = -80/9$,
$m_7 = -24320/81$, $m_8 = 33157760/19683$ from $G_5^{\mathrm{conn}} = 775/5184$
via Feynman sums, aligning to the Borcherds-product expansion of
$\Delta_5$; (iii)~\texttt{kappa\_BKM\_universal.sage} verifies
$\kappa_{\mathrm{BKM}}(\Phi_N) = c_N(0)/2$ for $N\in\{1,2,3,4,6\}$
against Gritsenko 1999 series;
(iv)~\texttt{pasol\_zagier\_universal\_igusa.sage} realises
$R_{\mathrm{Sieg,dyn}}$ on $\mathrm{Sp}_4(\mathbb Z)$ modular locus
\cite{PasolZagier2013,Gritsenko1999,Borcherds1992}.
Cross-ref Vol.~III Chapter~\ref{V3-ch:k3-chiral-bialgebra-platonic}.
\end{remark}

\begin{remark}[Algorithmic verification of the universal identity]
\label{rem:cm-2}
The universal identity $\hbar^2\cdot K^{\kappa_{\mathrm{ch}}} = -1$ at
$\hbar^2 = -1/8$ is computationally verified across the compute modules:
Lusztig specialisation at $\ell = 8$ (root-of-unity module tally),
Mukai-doubling at $c_+ = 4$ (signature computation on $II_{4,20}$),
pentagon coboundary $\phi^{(3)}$ at $\hbar^3$ (associator closure at
third order, with three linearly independent pieces summing to zero
modulo $\Phi_{10}/\eta^{24}$). The test suite checks each module's
output against the identity as a cross-cutting invariant
\cite{Mukai1987,Lusztig1990,EtingofKazhdan2007PartV}. Cross-ref
...
The arithmetic group carrying $\Delta_5$ is \emph{not} the paramodular
group $K(1) = \mathrm{Sp}_4(\Z)$ at level one (the earlier
``paramodular $K(N)$'' claim for $N = 1$ was retracted explicitly). The
correct home is the \emph{Maass spin cover}
...
with the $\Z/2$ exactly $\ker(v_{\Delta_5})^c$, one has
