...
chain-level presentations: after the source Lie bialgebra,
\(\hbar\)-adic topology, associator, and chiral QUE/quasi-Hopf target
are fixed, the Etingof--Kazhdan quantisation of the chiral coproduct
uses $\Phi_{\KZ}^{\mot}$ to intertwine the two natural
$E_2$-structures on $(\cA, \cA)$. Accordingly the chiral
...
$\kappa_j(\cW_N) = c/j$ in the normalization used here. Rationality on a line follows only
...
governed by the shadow data $(\kappa_j, \alpha_j, S_4^{W_j})$. These data are $\mathbb{Q}(c)$-rational
...
\begin{proof}
Expanding the binomial coefficient in the displayed closed form gives
...
and its standard central charge is
$c_{\mathrm{BP}}(k) = -(2k+3)(3k+1)/(k+3)$
(Fehily--Kawasetsu--Ridout 2020), with level-independent Koszul
conductor
$K_{\mathrm{BP}}(k) = c_{\mathrm{BP}}(k) + c_{\mathrm{BP}}(-k-6) = 50$
(\texttt{bp\_self\_duality.tex} Thm.\
\texttt{thm:bp-koszul-conductor-polynomial}). The computations below
use the explicitly shifted conformal-vector scalar
$c^{\mathrm{shift}}_{\mathrm{BP}}(k) = 2 - 24(k+1)^2/(k+3)$, whose
reflection sum is $196$; the shifted lane is a separate
conformal-vector convention, not the FKR central charge.
...
S_r^{\Vir}(c_{\mathrm{BP}}(k))$ by restriction to the Virasoro
subalgebra. Apply
...
\begin{proposition}[\label{prop:bp-fl-convention-caveat}Standard
Koszul conductor: polynomial constant]
\ClaimStatusProvedHere
Under the standard Fehily--Kawasetsu--Ridout convention
$c_{\mathrm{BP}}(k) = -(2k+3)(3k+1)/(k+3)$, the
...
\]
The shifted conformal-vector scalar is a change of stress tensor on
the \emph{same} algebra; Theorem~\ref{thm:bp-motivic-rationality-arakawa}
therefore applies to either scalar, and the reflection-sum identity
remains polynomial in both. The \emph{value} depends on the conformal
vector: $50$ for the standard FKR vector, $196$ for the explicitly
shifted vector; the shift is a redefinition of the stress tensor and
central charge, not a change in the algebra.
\end{proposition}

\begin{proof}
Direct substitution: writing
$c_{\mathrm{BP}}(k) = (-6k^2 - 11k - 3)/(k + 3)$ and
$c_{\mathrm{BP}}(-k-6) = (6k^2 + 61k + 153)/(k + 3)$,
...
and therefore holds wherever the two rational functions are defined.
Rational substitution preserves
$\mathbb{Q}(k)$-membership, so the motivic-rationality conclusion of
Theorem~\ref{thm:bp-motivic-rationality-arakawa} holds for either
conformal-vector scalar.
\end{proof}

\begin{remark}[\label{rem:bp-conductor-polynomial-constant}Polynomial-constant
identity in both conformal-vector conventions]
Direct substitution of $c_{\mathrm{BP}}(k)$ and
$c_{\mathrm{BP}}(-k-6)$ yields the polynomial-constant
value $K_{\mathrm{BP}} = 50$: any putative meromorphic
...
identity $K_{\mathrm{BP}} = 50$. The explicitly shifted conformal
vector gives the auxiliary reflection sum $196$; it corresponds to a
change of stress tensor and must not be cited as the FKR
...
Theorem~\ref{thm:miura-cross-universality} adds the rational prefactor
