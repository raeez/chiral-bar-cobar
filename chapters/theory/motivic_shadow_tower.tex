...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
\section{Virasoro rationality: weighted coefficients and formal relations}
\label{sec:virasoro-motivic-rationality-all-r}
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%

The weighted Riccati coefficients $R_r$ satisfy their one-line
transport recurrence.  A separate quadratic relation uses the same
seeds through arity five and determines an order-six coefficient:
\[
 R^{\mathrm{Ricc}}_6(\Vir_c)
 =\frac{80(45c+193)}{3c^3(5c+22)^2},
 \qquad
 C^{\mathrm{rel}}_6(\Vir_c)=\frac{4(240c+1031)}{c^3(5c+22)^2}.
\]
The rationality theorem below concerns the weighted recurrence.  A
comparison with an ordered residue-bar coordinate requires
$\mathsf H_{\mathrm{res}}(\Vir_c;X)$.
...
\]
The formal order-six relation coefficient
$C_6^{\mathrm{rel}}(\Vir_c)$ also belongs to $\mathbb Q(c)$.
A level-six radical/decoupling map supplies its singular-vector
interpretation, while $\mathsf H_{\mathrm{res}}(\Vir_c;X)$ supplies the
geometric coordinate $S_6(\Vir_c;\mathsf H_{\mathrm{res}})$.
\end{theorem}
...
on these coefficients.  The quadrichotomy chapter solves the separate
formal relation
$2R_2C_6^{\mathrm{rel}}+2R_3R_5+R_4^2=0$, giving the displayed rational
value and separating it from the weighted coefficient.
\end{proof}
...
gives the all-$r$ induction for that weighted recurrence.  The
quadrichotomy chapter supplies the separate order-six formal relation.
\end{remark}
...
The pentagon-equation coboundary at order $\hbar^3$ of the candidate
K3 Siegel--Borcherds target cocycle decomposes as
\[
 \phi^{(3)}
 \;=\;
...
Parts~I--V.  This is an order-$\hbar^3$ target-cocycle statement.  An
all-order Hall-realised associator additionally requires the compact
Hall source, comparison maps, and completed pentagon identity.
\end{remark}
...
the single-valued image of $\MZVmot$.  A curve-level family datum
$H_H(\cA;S)$ gives
$\operatorname{Supp}\ChirHoch^\bullet(\cA)\subseteq S$; the
single-valued restriction refines its period realization.  A perfect
degree-$d$ pairing remains the separate input for a Hochschild degree
reflection.
\end{remark}
