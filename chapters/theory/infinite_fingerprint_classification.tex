...
conditional reconstruction datum: it separates the standard
witness rows, it records the finite-window data carried by the
ordered bar complex, it survives
...
Every fingerprint slot is extracted from the ordered bar complex
$\barB^{\mathrm{ord}}(\cA) = T^c(s^{-1}\bar\cA)$: slots 1--4 from
$\barB^{\mathrm{ord}}$ and its Maurer--Cartan element $\Theta_\cA$
directly, slot 5 from the commutant pairing on $\barB^{\mathrm{ord}}$,
and the supplementary slot $\kappa_{\mathrm{ch}}$ from the
genus-$g \geq 1$ bar curvature $d_{\mathrm{fib}}^2 = \kappa \cdot \omega_g$.
The symmetric bar $\barB^{\Sigma}(\cA) = \mathrm{av}_*\barB^{\mathrm{ord}}(\cA)$
loses $n_{\mathrm{strong}}$, $\mathrm{coset}$, and $\kappa_{\mathrm{ch}}$
under $\Sigma_n$-coinvariants, collapsing the fingerprint to a partial
invariant. The ordered bar is the unique carrier of $\varphi$. This chapter inscribes the four
strengthening theorems that complete the fingerprint theorem: the
...
\section{The conditional fingerprint reconstruction surface}
...
\begin{theorem}[Conditional fingerprint reconstruction equivalence;
strengthening of Theorem~\ref{thm:fingerprint-completeness}]
\label{thm:fingerprint-reconstruction-equivalence}
\ClaimStatusConditional
...
+\mathrm{slot\ presentation\ independence}
+\mathrm{bar\text{-}intrinsic\ MC}
+\mathrm{Koszul\ locus}
+\mathrm{residual\text{-}free\ recovery})$.\textup{]}
Let~$\cA_1,\cA_2$ be chiral algebras in the standard landscape, and
assume the fingerprint-reconstruction package for the pair: the
typed projection of Theorem~\ref{thm:fingerprint-completeness}, the
presentation-independence of
Lemma~\ref{lem:fingerprint-slot-presentation-independence}, the
bar-intrinsic Maurer--Cartan tower of
Theorem~\ref{thm:mc2-bar-intrinsic}, the Koszul-locus bar--cobar
comparison of Chapter~\ref{ch:cobar}, and residual-free recovery of
the maps, higher braces, ordered coinvariants, and OPE coefficients
forgotten by~$\Pi_{\mathrm{fp}}$ in the chosen completed landscape
row. Under this package, the following are equivalent:
...
\end{enumerate}
Without the residual-free recovery clause, the theorem asserts only
that the bar/tower data determine the fingerprint; it does not assert
that the five slots reconstruct the forgotten structure.
\end{theorem}
...
the residual-free recovery clause in the stated
fingerprint-reconstruction package, there is an
...
for, this invariant direction. Together with the conditional
Step~1, this supplies both directions of the biconditional
\[
\varphi(\cA_1)=\varphi(\cA_2)
\Longleftrightarrow
\bigl(
...
\textup{(c)} under the stated package. Without the residual-free
clause, Steps~2 and~3 remain the asserted bar-to-fingerprint
direction, and Step~1 is an open reconstruction obligation rather
than a theorem.
\end{proof}
...
\item \textup{(}Position in the classification\textup{)}
 ...
 the quartic shadow vanishes by the Jacobi identity. Thus FF is the
 critical fixed-point boundary stratum of class~L. The refined
 fingerprint records this stratum separately, while the generic
 shadow-depth classification retains its four classes.
\item \textup{(}Scalar invariants\textup{)}
 ...
 \qquad
 \operatorname{obs}^{\mathrm{Hdg}}_g(\cA_{\mathrm{FF}})
 =(-1)^g\kappa(\cA_{\mathrm{FF}})\lambda_g = 0
 \quad (g \ge 1).
 \]
\item \textup{(}Sugawara degeneration and bar-cobar scope\textup{)}
 ...
 \]
 has a pole at $k=-h^\vee$. The critical formulation is the
 weight-completed/coderived bar--cobar package; the direct-sum
 Sugawara topologisation belongs to the punctured level line.
\item \textup{(}Center, derived center, and opers\textup{)}
 ...
 \qquad (n \ge 0),
 \]
 while the derived centre remains
 $C^\bullet_{\mathrm{ch}}(\cA_{\mathrm{FF}},\cA_{\mathrm{FF}})$.
 The critical bar complex supplies resolution input for the vacuum
 Ext calculation
 \textup{(}Theorem~\textup{\ref{thm:langlands-bar-bridge}(iii)}\textup{)}.
\item \textup{(}Theorem~H boundary\textup{)}
 the critical bar--oper calculation has nonzero terms
 $\Omega^n(\mathrm{Op}_{\fg^\vee}(D))$ in arbitrarily large degree.
 A conclusion about chiral Hochschild support requires a chain map
 \[
  \chi_{\mathrm{FF}}^{\mathrm{bar}}
  \colon C^\bullet_{\mathrm{bar\text{-}Ext}}(\cA_{\mathrm{FF}})
  \longrightarrow
  C^\bullet_{\mathrm{ch}}(\cA_{\mathrm{FF}},\cA_{\mathrm{FF}})
 \]
 from the completed bar--Ext model, together with a proof that it is a
 quasi-isomorphism and that its source cohomology is the displayed
 oper-form algebra. Under this comparison, the FF chart has unbounded
 support, and every finite-support datum $H_H(\cA_{\mathrm{FF}};S)$ is
 excluded. The bar--oper calculation and the chart support remain
 separately typed until this comparison is constructed.
...
The critical-level reflection \(k \mapsto -k-2h^\vee\) fixes
$k=-h^\vee$, hence $\kappa(\cA_{\mathrm{FF}}^{!})=0$ as well and the
complementarity sum is~$0$.  Under~$H_D^K$, the perfect object and
its Hodge character therefore vanish for~$\cA_{\mathrm{FF}}$.
...
placing the fixed point in the completed critical construction.
...
its role as resolution input and keeps the derived centre in the
chiral Hochschild category. Theorem~H computes chart support from a
family datum $H_H(\cA;S)$. The comparison map
$\chi_{\mathrm{FF}}^{\mathrm{bar}}$ named in clause~(v) is therefore
the exact bridge. If it is a quasi-isomorphism with the displayed
bar--oper source cohomology, the nonzero oper forms in arbitrarily
large degree transport to the chart, giving unbounded support.
...
\begin{remark}[Three consistency checks on the FF stratum]
\label{rem:ff-consistency-attacks}
The FF characterization satisfies three independent checks.
\begin{enumerate}[label=\textup{(\arabic*)},leftmargin=*]
\item \emph{Shadow-depth check.}
 The affine cubic shadow survives and the Jacobi identity kills the
 quartic term, so the coarse depth value remains~$3$ and the critical
 refinement lies inside class~L.
\item \emph{Theorems~C/D/H check.}
 The self-dual critical level gives $\kappa^!=0$ and therefore
 $\mathfrak O_g^K=0$ under~$H_D^K$. Independently, the completed bar--oper
 calculation gives unbounded bar cohomology. The map
 $\chi_{\mathrm{FF}}^{\mathrm{bar}}$ would transport it to chiral
 Hochschild cohomology. Theorem~D and the conditional chart-support
 calculation therefore occupy distinct levels of the Beilinson tower.
\item \emph{Theorems~A/B critical scope.}
 The Sugawara denominator places the critical point in the
 weight-completed/coderived regime already constructed elsewhere in
 Volume~I.
\end{enumerate}
The remaining comparison obligation is the construction and
quasi-isomorphism proof for $\chi_{\mathrm{FF}}^{\mathrm{bar}}$.
Every chart-support assertion above is conditional on this map.
\end{remark}

\begin{remark}[AP\textup{77} consequence: W(p) has $4$ strong
generators]
\label{rem:w-p-strong-generation}
...
Each row is a citation to the landscape census
	\textup{(}Chapter~\ref{ch:landscape-census}\textup{)}, with the
	$\beta\gamma$ row carried by
	Chapter~\ref{ch:three-invariants}: Heisenberg
...
\textup{(}Remark~\ref{rem:w-p-strong-generation}\textup{)}. The
...
contains at least two fingerprints distinguished by at least one
non-depth slot.
...
up to sign; they agree in $p_{\max}=2$ but differ in the coset slot
\textup{(}$\mathrm{Sp}(2,\mathbb{R})$-centraliser versus abelian\textup{)}.
\item \emph{Class~$L$ fibre:} generic non-critical affine algebras
$\widehat{\mathfrak{sl}_2}_k$ and
$\widehat{\mathfrak{sl}_3}_{k'}$ share shadow depth~$r_{\max}=3$;
they differ in the strong-generator and coset slots.
\item \emph{Class~$C$ fibre via parity:} symplectic boson
$\beta\gamma_{1/2}$ and symplectic fermion~$\mathcal{SF}$ share
the same coarse class under the full shadow tower, with
$(p_{\max},r_{\max})=(1,4)$, yet their fingerprints differ in
sign of~$\chi_{\mathrm{VOA}}$ and in the coset pair
$\mathrm{Sp}(2)$ versus~$\mathrm{OSp}(1|2)$.
\item \emph{Class~$M$ fibre:} $\Walg_3$ and super-$\Walg_3$ share
$p_{\max}=6$, $r_{\max}=\infty$, and two strong generators; the
super variant has an odd weight-$3/2$ generator, so the
$\chi_{\mathrm{VOA}}$ slot and the coset slot differ.
\item \emph{Critical-surface fibre:} critical affine
$V_{-h^\vee}(\fg)$ and critical $\Walg_N$ both lie on the
$\mathrm{FF}$ surface; they differ in pole order, strong-generator
count, and oper-coset slot.
\end{enumerate}
...
\begin{proof}
Surjectivity is the landscape census, and the five displayed witness
pairs hit the five target classes $G,L,C,M,\mathrm{FF}$. Non-triviality
is verified by reading the five fingerprint slots: the coarse-class
slot agrees, while at least one of the remaining slots disagrees.
...
\emph{Witness (ii): two affine Lie fibres.}
At generic non-critical levels,
\[
\varphi(\widehat{\mathfrak{sl}_2}_k)
  = (2,3,\operatorname{ch}_q\widehat{\mathfrak{sl}_2},3,
     \mathfrak{sl}_2/\cH),
\qquad
\varphi(\widehat{\mathfrak{sl}_3}_{k'})
  = (2,3,\operatorname{ch}_q\widehat{\mathfrak{sl}_3},8,
     \mathfrak{sl}_3/\cH).
\]
Both are class~$L$, but slots~(3), (4), and~(5) distinguish them.

\emph{Witness (iii): symplectic boson versus symplectic fermion.}
Both have $(p_{\max}, r_{\max}) = (1, 4)$ and live in class~$C$
under the full shadow tower.
...
\emph{Witness (iv): $\Walg_3$ versus super-$\Walg_3$.}
Both have $p_{\max}=6$, $r_{\max}=\infty$, and two strong generators,
yet the super variant includes a weight-$3/2$ fermionic primary
which contributes
$\operatorname{str}(q^{L_0}) \ni -q^{3/2}\mathbb{Z}[\![q]\!]_+$.
This forces $\chi_{\mathrm{VOA}}$ to differ, and the super coset is
not the ordinary $\Walg_3^!$ coset.

\emph{Witness (v): critical affine versus critical $W$.}
On the $\mathrm{FF}$ surface one has
\[
\varphi(V_{-h^\vee}(\fg))
  = (2,3,\operatorname{ch}_q V_{-h^\vee}(\fg),\dim\fg,
     \Fun\mathrm{Op}_{\fg^\vee}(D)),
\qquad
\varphi(\textup{critical }\Walg_N)
  = (2N,\infty,\operatorname{ch}_q\Walg_N,N-1,
     \textup{critical oper-coset}).
\]
The pole-order, strong-generator, and oper-coset slots distinguish
the two fingerprints while the coarse projection records only the
critical surface.
\end{proof}

\begin{remark}[Witness (iii) closes FM\textup{107}]
\label{rem:fm107-closure}
FM\textup{107} warned that ordinary $W$-algebras could be conflated
with super-$\Walg_3$ under the quadrichotomy. The class~$M$ witness
of Theorem~\ref{thm:quadrichotomy-is-coarse-projection}
...
on the critical-level reflected comparison side this is the
Feigin--Frenkel \(W\)-algebra duality
$\Walg^\Gamma_k(\fg) \leftrightarrow \Walg^{\Gamma^\vee}_{k'}(\fg^\vee)$
with~$(k + h^\vee)(k' + h^{\vee,\vee}) = 1$, not an identification
...
\textup{(}Chapter~\ref{chap:w-algebras}\textup{)}. The escalation from
...
\begin{calculation}[Fingerprint witness-row separation;
\ClaimStatusProvedHere]
\label{calc:fingerprint-stratum-separation}
Read the displayed rows as generic landscape families, with rows such
as~$\widehat{\fg}_k$, $\Walg_N$, and critical $\Walg_N$ excluding the
specialisations already listed separately. The five-slot fingerprint
values separate the witness rows. Across different coarse classes, the
shadow-depth slot or the critical-surface flag separates. Within a
single coarse class, the separating slots are:
\[
\begin{array}{c|c}
\text{coarse class} & \text{separating non-depth slots}\\
\hline
G & \mathrm{coset}\text{, and for lattices also }p_{\max}
\text{ or }n_{\mathrm{strong}}\\
L & n_{\mathrm{strong}}\text{ and the affine coset}\\
C & \chi_{\mathrm{VOA}}\text{ and } \mathrm{coset}\\
M & p_{\max},\ \chi_{\mathrm{VOA}},\ n_{\mathrm{strong}},
\text{ or } \mathrm{coset}\\
\mathrm{FF} & p_{\max},\ n_{\mathrm{strong}},\text{ and oper-coset}
\end{array}
\]
The parity column in the displayed table is only the leading sign of
the full Hodge-filtered supertrace slot; the calculation uses the full
formal series~$\chi_{\mathrm{VOA}}$, so ghost and symplectic-fermion
rows with the same leading parity are still separated by their
standard supertrace series.
\end{calculation}

\begin{corollary}[Witness-row separation on the standard landscape]
\label{cor:fingerprint-separates-landscape}
\ClaimStatusProvedHere
Calculation~\ref{calc:fingerprint-stratum-separation} exhibits
pairwise distinct fingerprints for the standard-landscape witness
rows, including pairs within a single coarse class:
\begin{itemize}[nosep]
...
establishes that~$\varphi$ is the canonical separating row datum for
the listed standard-landscape witnesses.
\end{corollary}

\begin{proof}
This is the slot-by-slot inspection carried out in
Calculation~\ref{calc:fingerprint-stratum-separation}. The stratum
theorem supplies the coarse-class partition, and the fingerprint
slots refine that partition by the displayed non-depth separators.
\end{proof}
...
$A$ is the VOA, $B(A)$ its bar coalgebra,
$A^{\mathrm i}=C_X(s^{-1}V,s^{-2}R)$ its presentation coalgebra,
$K_X(A)=\mathbb D_{\Ran}B_X(A)$ its Verdier algebra, and
$Z^{\mathrm{der}}_{\mathrm{ch}}(A)=\ChirHoch^\bullet(A,A)$ its derived
center. The maps $q_A$, $\mathbb D(q_A)$, and~$\nu_A$ supply the
quadratic and chosen-pair comparisons used by the theorem.
\end{proof}

\begin{proposition}[Weight-one threshold under the filtered
Hochschild comparison]
\label{prop:schellekens-weight-one-threshold}
...
w_\ast = 1.
\]
Assume for every chart~$A$ a family datum $H_H(A;S_A)$ and a
filtration-preserving bar-to-chart quasi-isomorphism whose restriction
to conformal weights at most one identifies the $V_1$ bracket. Then
the same threshold is seen in each
of the three constructions:
\begin{enumerate}[label=\textup{(\roman*)},leftmargin=*]
\item \textup{(}bar--cobar lane\textup{)}
 the weight-$\le1$ truncation of the ordered bar differential records
 the weight-one OPE residues, hence reconstructs~$V_1$ and separates
 all~$71$ rows, including the row~$V_1=0$;
\item \textup{(}chiral Hochschild lane\textup{)}
 Theorem~H identifies
 $\ChirHoch^n(A)$ with $H^n(K_{A,S_A})$ and places its
 cohomological support in~$S_A$.  The assumed filtered comparison
 transports the weight-$\le1$ row classifier to this complex;
\item \textup{(}derived-center lane\textup{)}
 the evaluation pairing on
 $Z^{\mathrm{der}}_{\mathrm{ch}}(A)=\ChirHoch^\bullet(A,A)$ restricted
 to conformal weights~$\le1$ is read on the same filtered cochain
 complex as the Hochschild lane.
\end{enumerate}
Thus the conformal-weight threshold is uniform across the three
constructions under the stated comparison.  The support set~$S_A$
records cohomological degrees and remains independent of conformal
weight.
\end{proposition}

\begin{proof}
Schellekens's rows are indexed by~$V_1$, with the row~$V_1=0$ included
as one value of that invariant. Thus conformal weight~$1$ determines
all~$71$ rows. Conformal weight~$0$ contains the common vacuum line,
so weight~$1$ is the minimal uniform threshold.
...
conformal weight~$1$ reconstructs~$V_1$, including its zero value. The
family datum computes the cohomological support
of the chart complex.  The additional filtered quasi-isomorphism
identifies its conformal-weight-$\le1$ truncation with the bar
truncation, and the derived centre retains this complex together with
its $E_2$-structure.  Hence all three filtered objects have the common
threshold $w_\ast=1$.
\end{proof}

\begin{remark}[Independent verification record for the numerical claims]
\label{rem:schellekens-numerical-record}
...
\begin{enumerate}[label=\textup{(\arabic*)},leftmargin=*]
\item \textup{(}$71 = 24_{\mathrm{Niemeier}} + 1_{V_1=0} +
46_{\mathrm{nonlat}}$\textup{)}
 \begin{enumerate}[label=\textup{(\alph*)},leftmargin=*]
...
\item \textup{(}level~$1$ on the Niemeier rows\textup{)}
 ...
 \end{enumerate}
\item \textup{(}$w_\ast = 1$\textup{)}
 \begin{enumerate}[label=\textup{(\alph*)},leftmargin=*]
 \item Schellekens 1993, pp.~4, 23--24, indexes every row by its
 weight-one Lie algebra, including the value~$V_1=0$.
 \item The ordered bar differential on weight-one generators
 reconstructs~$V_1$, while conformal weight~$0$ is the common vacuum
 line.
 \item A datum $H_H(A;S_A)$ computes cohomological support in~$S_A$;
 the assumed filtered comparison transports the conformal-weight-one
 row classifier to the same cochain complex. These are independent
 gradings.
 \end{enumerate}
\end{enumerate}
\end{remark}
...
\item \textup{(}Vol~III categorical DT\textup{)}
 ...
 namely the conformal-weight filtration through~$1$. Its
 cohomological support is recorded separately by the family set~$S_A$,
 as in Proposition~\ref{prop:schellekens-weight-one-threshold}.
\end{enumerate}
\end{remark}
...
\section{Synthesis}
\label{sec:ifc-supplement}

\begin{remark}[Class-$\mathcal S$ fingerprint on $\Sigma_{0,24}$]
\label{rem:ifc-1}
The canonical class-$\mathcal S$ fingerprint for
$\mathcal T[A_1,\Sigma_{0,24}, \mathrm{all\ max}]$ is the triple
\[
 (c_{4d}, c_{2d}, \mathrm{rank}_{\mathrm{Coul}})
 \;=\;
 \bigl(107/6,\, -214,\, 21\bigr),
 \qquad
 \mathrm{flavour}\,=\,\mathfrak{su}(2)^{24},
\]
an infinite-fingerprint invariant pinning down the K3 chiral algebra
image as $\mathbf H_{\Delta_5}$-module. The $c_{2d} = -214 = -2\cdot 107$
factorisation (with $107$ prime) reads the Virasoro shadow-tower face
against the $24$ Schur/flavour $\mathfrak{su}(2)$ currents at the
Kodaira $I_1$ punctures. The $c_{4d} = 107/6 = (5\cdot 24 - 13)/6$
evaluation follows Shapere--Tachikawa 2008 \S 3
(\texttt{arXiv:0804.1957}) applied to the Chacaltana--Distler
class-$\mathcal S$ count $(n_v, n_h) = (63, 88)$ on
$(A_1, \Sigma_{0,24}, \mathrm{all\ max})$ (eq.~(5.14) therein);
$c_{2d} = -12 c_{4d}$ by Beem--Rastelli (BLLPRvR 2013 eq.~(3.14)).
The earlier intermediate value $(c_{4d}, c_{2d}) = (26, -312)$ is
false (Vol~I Section~\ref{sec:thqg-open-closed-realization},
with \cite{Gaiotto2009,BeemRastelli2015}). Cross-ref
Vol.~III Chapter~\ref{V3-ch:k3-chiral-bialgebra-platonic}.
\end{remark}

\begin{remark}[Coulomb-rank $21$ vs Mukai-signature $20$]
\label{rem:ifc-2}
The fingerprint distinguishes Coulomb rank $21$ from the Mukai
negative-signature part $c_- = 20$ by a ``$+1$'' shift matching the
$U(1)$ topological charge on $\Sigma_{0,24}$; equivalently, the shift
from $\Lambda_{\mathrm{Muk}} = II_{4,20}$ to the class-$\mathcal S$
lattice $II_{4,20}\oplus\langle 1\rangle$. At the correct
first-principles central charge $c_{4d} = 107/6$, $c_{2d} = -214$
of Remark~\ref{rem:ifc-1}, the Virasoro-face depth reads
$c_{2d} + 2 c_{4d} = -214 + 107/3 = -535/3$, which is no longer a
naive integer-multiple of the Mukai signature; rather, the
denominator $3$ encodes the $6d$ trinion-gluing datum on
$\Sigma_{0,24}$ via the Shapere--Tachikawa normalisation.
The Mukai-doubling identity $K^{\kappa_{\mathrm{ch}}} = 8 = 2 c_+$
survives as a purely lattice-theoretic statement on $II_{4,20}$
(Mukai 1987, Borcherds 1998), independent of the class-$\mathcal S$
central-charge correction.
\cite{Mukai1987,Borcherds1998}. Cross-ref
Chapter~\ref{V3-ch:k3-chiral-bialgebra-platonic}.
\end{remark}

\begin{remark}[Pentagon target cocycle and umbral cohomology class as fingerprint refinement]
\label{rem:ifc-3-pentagon-umbral}
A cohomological fingerprint refinement extends the numerical triple
$(c_{4d}, c_{2d}, \mathrm{rank}_{\mathrm{Coul}}) = (107/6, -214, 21)$
of Remark~\ref{rem:ifc-1}. The pentagon
$\hbar^3$-obstruction of the candidate Siegel--Borcherds target cocycle
$\widetilde{\Phi}^{\mathrm{Sieg\text{-}Bor}}_{\mathrm{Sp}_4}[\Phi_{10}/\eta^{24}]$
and the projective $\widetilde{M}_{24}$-umbral cocycle of order $6$ on the
K3 Serre functor live in \emph{the same transgressed cohomology class}
in $H^3_{\mathrm{transgr}}(\langle g\rangle, U(1)) \cong H^2(\langle
g\rangle, U(1)) \cong \mathbb{Z}/6$ for any order-$6$ element
$g\in M_{24}$. The shared class is the $M_{24}$ global-symmetry 't~Hooft
anomaly of the class-$\mathcal S$ parent $\mathcal T[A_1,\Sigma_{0,24}]$
in its Beem--Rastelli protected chiral sector, detected by the
Atiyah--Segal transgression $H^3_{\mathrm{Lie}}\to H^2_{\mathrm{grp}}$
applied to the $(\Phi_{10}/\eta^{24})\cdot c_{\Phi_{10}}$ summand of
$\phi^{(3)}$. The fingerprint is therefore a quadruple
\[
 \bigl(c_{4d}, c_{2d}, \mathrm{rank}_{\mathrm{Coul}},\,
 [\phi^{(3)}|_{\langle g\rangle}]\bigr)
 \;=\;
 \bigl(107/6,\, -214,\, 21,\, \mathrm{umbral}_{m_g}\bigr),
\]
with the last entry a class in $\mathbb{Z}/6$ indexed by the umbral shift
$m_g\in\{0,2,6\}$ for order-$6$ conjugacy classes $6A, 6B$ of $M_{24}$
(Cheng--Duncan--Harvey~$2014$, Table~$1$). The cohomological refinement
survives the $(c_{4d}, c_{2d}): (26, -312) \to (107/6, -214)$
retraction unchanged: the $\mathbb{Z}/6$ obstruction class lies in
$H^3(M_{24}, U(1))$ (generalised-Mathieu-moonshine 't~Hooft
anomaly, Gaberdiel--Persson--Ronellenfitsch--Volpato 2013), not
in the central-charge data. Two theories with
identical numerical fingerprints but different umbral-cocycle classes
are distinct at the level of the cohomological refinement; cross-ref
Vol.~III Theorem~\ref{V3-ch:k3-chiral-bialgebra-platonic}'s
Theorem on pentagon--umbral agreement and its Corollary on the $M_{24}$
't~Hooft anomaly. Primary-lit: Cheng--Duncan--Harvey 2014
(\texttt{arXiv:1307.5793}); Atiyah--Segal 1989; Costello--Gaiotto 2018
(\texttt{arXiv:1812.00516}).
\end{remark}
