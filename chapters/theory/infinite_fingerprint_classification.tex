...
\begin{remark}[Why the quaternitomy is insufficient]
\label{rem:why-quaternitomy-insufficient}
The quaternitomy $\{G,L,C,M\}$ is the image of the projection
...
fermion, $\Walg_3$ versus super-$\Walg_3$) share a quaternitomy
class but not a bar complex. (iii) The DS reduction
$\cA\mapsto H^\bullet_{\mathrm{DS}}(\cA)$ scrambles the quaternitomy
...
quaternitomy~$\{G,L,C,M\}$ shifted by~$-2$.
\end{theorem}
...
\begin{corollary}[Quaternitomy as depth shift]
\label{cor:quaternitomy-depth-shift}
...
\begin{theorem}[Quaternitomy is a coarse projection; strengthening
of Proposition~\ref{prop:coarse-projection-functor}]
\label{thm:quaternitomy-is-coarse-projection}
\ClaimStatusProvedHere
...
\end{enumerate}
Consequently, the quaternitomy $\Pi_{\mathrm{coarse}}\circ\varphi$
is strictly lossy.
\end{theorem}
...
conflated with super-$\Walg_3$ under the quaternitomy. Witness
(iii) of Theorem~\ref{thm:quaternitomy-is-coarse-projection}
produces the separation at the fingerprint level: slot~(3)
($\chi_{\mathrm{VOA}}$) and slot~(5) (coset) carry the
$\Zmod 2$-grading information that the quaternitomy discards.
\end{remark}
