...
The genus-$0$ bar complex $\barB(\cA)$ is built from trees: a
tree with $k$ internal vertices records $k$ successive binary
collisions on~$\mathbb{P}^1$, the associahedron organises
their boundary incidences, and the operadic bar construction
sums over them.  Trees are connected graphs of genus~$0$; the
bar differential squares to zero because every
codimension-two boundary stratum of $\overline{\cM}_{0,n}$ is
reached by exactly two one-edge degenerations that cancel.
But a smooth curve of genus $g \geq 1$ can degenerate by
pinching handles as well as by collision of points, and the
tree combinatorics cannot see these degenerations.  The MC
element $\Theta_\cA \in \MC(\gAmod)$ acquires genus-$g$
corrections: the fiberwise bar differential picks up curvature
$\dfib^{\,2} = \kappa(\cA) \cdot \omega_g$ from the Hodge class, and
the shadow obstruction tower $\Theta_\cA^{\leq r}$ records
the higher-arity modular data that trees miss entirely.
