...
% RECTIFICATION-FLAG: the proof of thm:multi-generator-universality
% writes Lambda = sum lambda_g as if it follows from dim H^2 = 1,
% but the Kuranishi vanishing is unconditional (any Omega works),
% so the identification Omega_g = lambda_g requires the bar
% construction, not scalar saturation.
\end{proof}
...
$\dfib^{\,2} = \kappa(\cA) \cdot \omega_g$
(Theorem~\ref{thm:quantum-arnold-relations}).
As $[\Sigma_g]$ varies over~$\overline{\cM}_g$, the class of
$\omega_g$ is $\lambda_g = c_g(\mathbb{E})$, giving
...
\begin{proposition}[Grothendieck--Riemann--Roch bridge on the scalar lane; \ClaimStatusProvedHere]
\label{prop:grr-bridge}
\index{Grothendieck--Riemann--Roch!genus expansion}
\index{Hodge bundle!genus expansion}
On the scalar lane for a modular Koszul chiral algebra
...
genus universality on that scalar lane.  Constructing
