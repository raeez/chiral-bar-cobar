...
the birth of the programme's modular-Koszul theory (T15 sense (iii)).
...
The content of Theorem~\ref{thm:genus-universality} is the precise
inertness of the \emph{uniform-weight scalar factor}. A genus-$47$
surface and a torus see the same genus-$1$ coefficient
$\kappa(\cA)$ in that scalar lane; the Hodge-theoretic factor changes
from $\lambda_1$ to $\lambda_g$, a topological datum of
$\overline{\mathcal M}_g$ rather than of the algebra. Thus
\[
  \mathrm{obs}_g(\cA)=\kappa(\cA)\lambda_g
\]
on the uniform-weight scalar lane. For all-weight algebras the same
scalar spine remains, but it is no longer the whole graph sum:
\[
  F_g(\cA)=\kappa(\cA)\lambda_g^{\mathrm{FP}}
  +\delta F_g^{\mathrm{cross}}(\cA).
\]
The cross-channel correction is the new dynamical information carried
by mixed conformal weights; it vanishes exactly on the uniform-weight
lane.
...
$\Phi_d = \SpCh_{\Sigma_{d-1}, C} \circ \PhiFA_d$ of Vol~III, with
...
a reference curve $C$. On the higher-genus Vol~I side, the
...
\]
On the Vol~III side, for the $K3$-fibre specialisation of the
canonical $E_3$-holomorphic factorisation algebra
$\PhiFA_3(\cC_{K3 \times E})$, the Borcherds pushforward of the
BKM $\kappa$-invariant over $K3 \times E$ gives
\[
\mathrm{push}_{K3 \times E}^{\mathrm{Borcherds}}\!\bigl(\kappa_{\mathrm{BKM}}\bigr)
\;=\;
\mathrm{STr}_{\mathrm{bar}}^{\mathrm{mod-Koszul}}\!\bigl(\Phi_3^{-1}(\mathcal{A})\bigr)
\qquad
\bigl(\mathcal{A} = \SpCh_{\Sigma_2, C}(\PhiFA_3(\cC_{K3 \times E}))\bigr),
\]
a single numerical-class identity that binds the higher-genus
modular-Koszul trace (this chapter) to the BKM side of Vol~III via
the stage-$2$ shadow of $\PhiFA_3$.
