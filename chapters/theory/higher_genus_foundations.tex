...
Level~(v): $C_2$-cofiniteness implies, by Zhu's
theorem~\cite{Zhu96}, that the space of genus-$g$ conformal
blocks is finite-dimensional and carries a projectively flat
connection. This implies convergent partition functions
...
\caption{Genus-$g$ obstruction classes. For scalar-diagonal families
 ...
 at $g \geq 3$ the cross-channel correction is named via the graph-sum construction but its closed form
 is conjectural.}
...
\item Every scalar propagator is the standard weight-$1$
 form $d\!\log E(z,w)$
 \textup{(}Remark~\textup{\ref{rem:propagator-weight-universality}}\textup{)},
 so the geometric zero-mode sector as the curve varies is the
 Hodge bundle $\mathbb{E} = R^0\pi_*\omega_\pi$; no higher Hodge
 bundle $R^0\pi_*\omega_\pi^{\otimes h}$ with $h \neq 1$ enters
 the scalar channel.
\item The Arakelov--Faltings theorem~\cite{Fal84} identifies
 the Arakelov form~$\omega_g^{\mathrm{Ar}}$ on each fiber with the
 fiberwise restriction of the curvature of the Hodge
 bundle in the $L^2$ metric
 \textup{(}\eqref{eq:arakelov-curvature-identification}\textup{)}.
\item Chern--Weil applied to this curvature gives
 $c_g(\mathbb{E}) = \lambda_g$; alternatively,
 GRR on the universal curve
 $\pi\colon\overline{\cC}_g \to \overline{\cM}_g$ computes
 the same Chern classes~\cite{Mumford83}
 \textup{(}\eqref{eq:grr-hodge}\textup{)}.
\end{enumerate}
This completes the identification
\eqref{eq:genus-universality} at all genera.
