...
The reader should be prepared to see the modular characteristic
$\kappa(\cA)$ as the linear shadow (Ring~1 of the architecture),
the spectral deformation~$\Delta_{\cA}$ as the first nonlinear shadow
(Ring~2), and the entire genus tower as one completed homotopy object
...
a pathology: it is a \emph{projection} of a deeper structure.  The
...
the period-corrected level.  This section develops the strict dg model
(the three differentials and their interaction); the full homotopy
...
\item \emph{Hodge bundle structure.}\;
  The Hodge bundle $\mathbb{E}^\vee = \pi_*\omega_\pi$
...
\item \emph{Affine Kac--Moody} $\widehat{\fg}_k$:
  $\mathrm{Tr}_{\widehat{\fg}_k} = k \cdot \dim(\fg) / (2h^\vee)$.
...
(Theorems~\ref{thm:heisenberg-genus1},
\ref{thm:km-curvature}, \ref{thm:virasoro-genus1-curvature}).
\end{proof}
...
$\mathbb{D}(\barB(\cA)) \simeq \barB(\cA^!)$ is a geometric
...
(Theorem~\ref{thm:shifted-symplectic-bar-complementarity}),
...
\label{conj:log-clutching-degeneration}
