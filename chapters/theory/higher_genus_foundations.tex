...
The logarithm leaves the affine line and enters the modular world.
...
organizes all of these corrections; the present chapter constructs the
genus-level foundations on which that tower rests.

The bar-cobar story therefore acquires a tower indexed by genus, with
each level carrying its own curvature, its own obstruction class, and
its own interplay between the algebra~$\cA$ and its Koszul
dual~$\cA^!$.  The central actors are three: the curved fiberwise
...
The fiberwise curvature $\dfib^{\,2} = \kappa(\cA)\cdot\omega_g$ is not
a pathology but a projection of a deeper structure
(cf.\ Remark~\ref{rem:mc-uncurving} for the general
...
The derived--coderived equivalence, established via bar-cobar
adjunction for homotopy-Koszul operads, ensures that the flat
and curved models compute the same derived invariants.
...
Arnold defect computation: the wedge product
$\eta_{12}^{(g)} \wedge \eta_{23}^{(g)} + \text{cyclic}$
equals $\kappa(\cA)\cdot\omega_g$ because the
$\bar\partial$-exact correction terms contribute a
$(1,1)$-form proportional to
$\sum_k\omega_k\wedge\bar\omega_k$, and the coefficient
is the self-contraction trace
(Theorem~\ref{thm:quantum-arnold-relations}).
...
\midrule
$H_\kappa$ & $\kappa$ & $-\kappa$ & $0$ & $\kappa/24$ \\
...
The Heisenberg factorization $\mathrm{obs}_g = k \cdot \lambda_g$
\textup{(}\S\ref{sec:frame-genus-tower}\textup{)} is universal on the
uniform-weight lane.

For every Koszul chiral algebra $\mathcal{A}$ whose strong generators all have the same conformal weight, there is a genus-independent scalar $\kappa(\mathcal{A})$, determined by the genus-$1$ curvature, such that
...
universal scalar statement is then only the genus-$1$
free-energy identity together with
...
For uniform-weight algebras, the genus-$g$ free energy is
$F_g = \kappa \cdot \frac{2^{2g-1}-1}{2^{2g-1}} \cdot \frac{|B_{2g}|}{(2g)!}$,
where $B_{2g}$ are Bernoulli numbers.
For arbitrary modular Koszul algebras, the same formula holds
...
only the genus-$1$ free-energy coefficient is unconditional in general.
For uniform-weight algebras, the same scalar factorization yields
$F_g = \kappa \cdot \lambda_g^{\mathrm{FP}}$ at all genera.
For multi-weight algebras, the all-genus free-energy factorization
...
Part~(iii) is the Faber--Pandharipande evaluation on the
uniform-weight lane, together with the genus-$1$ reduction for all
families:
$\int_{\overline{\mathcal{M}}_{g,1}} \psi_1^{2g-2}\,\lambda_g = \frac{2^{2g-1}-1}{2^{2g-1}} \cdot \frac{|B_{2g}|}{(2g)!}$
...
Nevertheless, the Faber--Pandharipande formula evaluates
$\lambda_g = \lambda_g^{(1)}$ against $\psi$-classes to the standard
Bernoulli expression (Theorem~\ref{thm:mumford-formula}).
Whether $\int_{\overline{\mathcal{M}}_{g,1}}
\psi_1^{2g-2}\,\lambda_g^{(j)}$ is independent of~$j$ for all $g
\geq 2$ is not established; if it is, the free-energy factorization
$F_g = \kappa \cdot [\text{Bernoulli}]$ holds unconditionally for
multi-generator algebras as well.
See Open Problem~\ref{op:multi-generator-universality}.
\end{remark}

\begin{openproblem}[Multi-generator cohomological universality]%
\label{op:multi-generator-universality}%
\index{multi-generator obstruction!open problem}%
Let $\mathcal{W}_N^k$ be the principal $\mathcal{W}$-algebra of type~$A$
with $N \geq 3$ generators of weights $2, 3, \ldots, N$.
Does the \emph{cohomological identity}
$\mathrm{obs}_g(\mathcal{W}_N^k) = \kappa \cdot \lambda_g$
hold in $H^{2g}(\overline{\mathcal{M}}_g)$ for $g \geq 2$,
or does the obstruction decompose as
$\sum_{s=2}^{N} \kappa_s \cdot \lambda_g^{(s)}$ with genuinely
independent Hodge-type contributions?

\textup{\textbf{Structural diagnosis}}
\textup{(}Proposition~\textup{\ref{prop:f2-quartic-dependence}}\textup{)}.
The genus-$2$ free energy depends on the quartic shadow coefficient
$S_4$ through the banana graph:
$\partial F_2 / \partial S_4 = 1/(8\kappa^2) \neq 0$.
For single-generator algebras, $S_4(\kappa)$ is determined by~$\kappa$
via the MC recursion, so $F_2 = \kappa \cdot \lambda_2^{\mathrm{FP}}$
is automatic.
For multi-generator algebras,
$S_4^{\mathrm{total}}(\mathcal{W}_N) \neq
S_4^{\mathrm{Vir}}(\kappa(\mathcal{W}_N))$:
the total quartic differs from the Virasoro quartic at the same~$\kappa$.
The \emph{scalar} graph sum with total shadow data gives
$F_2 \neq \kappa \cdot \lambda_2^{\mathrm{FP}}$.
A multi-channel CohFT computation with per-channel propagators
\textup{(}Remark~\textup{\ref{rem:multigen-structural-diagnosis}}\textup{)}
might restore factorisation if the per-channel $R$-matrix is
$h$-independent; this remains \textbf{open}.

The sharpest test case is $\mathcal{W}_3$ at genus~$2$, where
$H^4(\overline{\mathcal{M}}_2) \cong \mathbb{C}^2$
(generated by $\lambda_2$ and $\lambda_1^2$)
and the cross-term between the $T$--$W$ curvature channels
(computed in Theorem~\ref{thm:w3-obstruction}) must be checked
against the cohomological decomposition.
If the obstruction splits, the multi-generator genus tower
carries more than one parameter's worth of
algebra-dependent modular data, a second independent
invariant beyond~$\kappa$.
See Remark~\ref{rem:four-levels} and
Conjecture~\ref{conj:categorical-modular-kd}.
\end{openproblem}

% RECTIFICATION-FLAG: The multi-generator universality question is
% subtle.  The free-energy identity F_g = kappa * lambda_g^FP is
% proved for single-generator algebras (Theorem D).  For multi-generator
% algebras, the integrated identity requires showing the per-channel
% contributions use the SAME lambda_g^FP.  The cohomological class
% identity lambda_g^{(h_i)} = lambda_g is a STRONGER statement that
% involves the Hodge bundles E_j = R^0 pi_* omega^{otimes j} of
% different ranks; at g >= 2 these are genuinely distinct geometric
% objects.  Open Problem op:multi-generator-universality remains OPEN
% at the class level.  See prop:f2-quartic-dependence for the genus-2
% diagnostic and rem:multichannel-resolution for the structural picture.
...
\textup{(c)} \textbf{Open.}
The assumption in~(b) that the $R$-matrix is $h_i$-independent
requires justification: the propagator for a weight-$h$ field
involves the $h$-th power of the canonical bundle, so the
resulting Hodge integrals involve~$\mathcal{E}_h = R^0\pi_*
\omega^{\otimes h}$, whose Chern classes depend on~$h$
(by Mumford--Chiodo).
At genus~$1$, all $\lambda_1^{(h)}$ are proportional
(since $H^2(\overline{\cM}_{1,1}) \cong \mathbb{C}$);
at genus~$g \ge 2$, the classes $c_g(\mathcal{E}_h)$ live in
$H^{2(2h{-}1)(g{-}1)}$, which depends on~$h$.
Open Problem~\textup{\ref{op:multi-generator-universality}}
remains \textbf{open} at the cohomological level for $g \ge 2$.
\end{remark}
...
the $T$-multiplication) ensures factorization via the
Givental--Teleman reconstruction into independent channels,
yielding $F_g = \kappa \cdot \lambda_g^{\mathrm{FP}}$.
...
\end{enumerate}
When these hold, $H^0(\bar{B}^{\mathrm{ch}}(\mathcal{A}))$ is a genuine associative chiral algebra \textup{(}not merely $A_\infty$\textup{)}.
\end{corollary}
...
uniform conformal weight, the genus generating function is a Hodge
integral:
...
cotangent line class.  For arbitrary modular Koszul algebras, this
same identity is unconditional at genus~$1$.  On the same
uniform-weight lane, the total
generating function
...
Theorem~\ref{thm:genus-universality} on the uniform-weight lane
...
$\sum F_g x^{2g}=\kappa(\cA)(\hat{A}(ix)-1)$ on the
uniform-weight lane as the GRR
...
$\mathbb{D}(\barB(\cA)) \simeq \cA^!_\infty$ (underlying complex $\simeq \barB(\cA^!)$) is a geometric
...
deep structures in number theory.  The Lie algebra
...
The first-principles development in this section
reveals five structural pillars:
\begin{enumerate}[label=\textup{(\Roman*)}]
