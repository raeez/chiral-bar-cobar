...
minimal-model MC element to $\Theta^{\min} = \kappa\cdot\eta
...
\index{multi-generator obstruction!open}%
$\mathrm{obs}_g(\mathcal{W}_N^k) = \kappa \cdot \lambda_g$
holds in $H^{2g}(\overline{\mathcal{M}}_g)$ for all $g \geq 2$
and all~$N$?

What is now proved is weaker:
\textup{(a)} the genus-$1$ identity
$\mathrm{obs}_1 = \kappa \cdot \lambda_1$ is unconditional;
\textup{(b)} the weight-$1$ propagator
(Remark~\ref{rem:propagator-weight-universality}) shows that all
edge contributions use the standard Hodge bundle~$\mathbb{E}$;
\textup{(c)} the new rigidity argument
(Proposition~\ref{prop:saturation-equivalence},
Theorem~\ref{thm:algebraic-family-rigidity})
forces the minimal-model MC element into the cyclic line
$\Theta_\cA^{\min} = \eta \otimes \Gamma_{\cA}$.
The missing step is to identify
$\Gamma_{\cA} = \kappa(\cA)\Lambda$.

Three graph-sum strategies were investigated before the
minimal-model proof was found:
\textup{(a)}~weight-$1$ propagator alone (controls edges, not
vertices);
\textup{(b)}~Teleman reconstruction (requires $R = \mathrm{Id}$
and flat unit, neither of which holds);
\textup{(c)}~direct CohFT decomposition (blocked by~(b)).
None of these routes has yet been upgraded to a complete proof.
\end{openproblem}
