...
That amount is the fiberwise curvature class
$\kappa(\cA)[\omega_g^{\mathrm{Ar}}]$, and its appearance is
forced. Here $\omega_g^{\mathrm{Ar}}$ denotes the Arakelov
$(1,1)$-form on the fixed smooth fiber~$\Sigma_g$. It is not itself
a class on $\overline{\cM}_g$; the Chern--Weil image of the Hodge
metric on $\mathbb{E}=\pi_*\omega_\pi$ gives
$\lambda_1=c_1(\mathbb{E})$, and the scalar obstruction lane uses
the top Hodge class $\lambda_g=c_g(\mathbb{E})$. $\kappa(\cA)$ is
the scalar extracted from the $\Sigma_2$-coinvariant of the
degree-two bar coefficient $r(z)$; their product is the leading genus
invariant that survives averaging. The source of the curvature is
...
$\kappa$ through the self-contraction trace, matched with the class
$\lambda_1 = \tfrac{1}{12}\delta_{\mathrm{irr}}$ on the moduli side.
At higher genus the same mechanism repeats through the Feynman
transform, summing over stable graphs weighted by handles. The
passage from trees to stable graphs is the passage from operads to
modular operads, and the obstruction tower $\Theta_\cA^{\leq r}$
records the higher-degree modular data that no tree could ever see.
...
differential $D_\cA^2 = 0$ in each genus. It is not a proof that the
fixed-fiber differential $\dfib$ is square-zero; the fixed-fiber
square is the curvature term above.
...
introduces the curvature class
$[\dfib^{\,2}] = \kappa(\cA)[\omega_g^{\mathrm{Ar}}]$, where
...
the holomorphic tangent-screen cancellation; it does not turn the
positive-genus global configuration-space algebra into the affine
Arnold algebra.
...
\]
with $\delta F_1^{\mathrm{cross}}=0$ in genus~$1$. On the
...
positive-genus Arnold-defect computation in the normalized
Arakelov--Green model: the affine Arnold relation remains exact on
each genus-$0$ tangent screen, while the global prime-form propagator
has a period defect whose de~Rham class is
$2\pi i[\omega_g]$
(Theorem~\ref{thm:quantum-arnold-relations}). Contracting that class
with the degree-two OPE data of~$\cA$ extracts the
self-contraction trace~$\kappa(\cA)$, giving the curvature class
$[\dfib^{\,2}] = \kappa(\cA)[\omega_g]$; with the Arakelov
representative fixed, this is the displayed chain-level curvature
operator.
Part~(iii): the Fay trisecant identity gives the holomorphic
tangent-screen cancellation. The square-zero identity for $\Dg{g}$
uses the period and Gauss--Manin correction terms as well; it is not
an Arnold-only identity in the positive-genus cohomology algebra
(Theorem~\ref{thm:quantum-diff-squares-zero}).
...
curved fiber component incorporating the genus-$g$ curvature. At
$g = 0$, this is the tree-level MC equation. At $g = 1$, the class
$\kappa\,\omega_1$ is the elliptic Arakelov defect; the strict
square-zero differential is the total period-corrected
...
With the Arakelov representative fixed, the fiberwise bar differential
squares to the Arnold defect contracted with the OPE data of~$\cA$
...
The genus-$2$ cocycle $c_2$ above is integrated against the Weil--Petersson measure on $\mathcal{M}_2$, and its modular content is controlled by the unscaled Igusa cusp form $\Phi_{10}^{\mathrm{un}}(Z)$ of weight~$10$ on $\mathbb{H}_2$. At a \emph{separating} node, where $\Sigma_2$ degenerates to $\Sigma_1 \cup \Sigma_1$ and the period matrix degenerates as $Z \to \mathrm{diag}(\tau, \rho)$, the cusp form factorises explicitly:
\[
...
The formal degree count is carried out in
...
\begin{equation}
\eta_{ij}^{(1)}
= d\log \theta_1(z_i-z_j\mid\tau)
+2\pi i\,\frac{\operatorname{Im}(z_i-z_j)}
{\operatorname{Im}\tau}\,d\bar z_i .
\end{equation}
The theta quotient alone is quasi-periodic; the displayed
non-holomorphic term is the Arakelov correction that descends to the
elliptic curve.
...
\]
Thus the affine Arnold relation survives on each genus-$0$ tangent
screen, but it is not a complete positive-genus presentation.
...
$\cA$ gives the fiberwise curvature class
\[
[\dfib^{\,2}]
=
\kappa(\cA)[\omega_g^{\mathrm{Ar}}].
\]
The scalar obstruction on the uniform-weight lane is its
Chern--Weil Hodge-bundle projection
...
$B$-period monodromy. The added term
$2\pi i\,\operatorname{Im}(z_i-z_j)d\bar z_i/\operatorname{Im}\tau$
is the unique translation-invariant Arakelov correction with residue
one on the diagonal. Its $\bar\partial$ is the normalized volume
form, so the cyclic residue computation gives
...
$[\mathcal A_3^{(g)}]=2\pi i[\omega_g^{\mathrm{Ar}}]$ and hence
$[\dfib^{\,2}]=\kappa(\cA)[\omega_g^{\mathrm{Ar}}]$. Choosing the
...
weight-$10$ Siegel scalar attached to this class is the unnormalised
Igusa square $\Phi_{10}^{\mathrm{un}}=\Delta_5^2$: under the Torelli
embedding $\mathcal{M}_2 \hookrightarrow \mathcal{A}_2$, the pullback
$\Delta_5$ lifts to a section of the metaplectic line with character,
and its square has the $\Phi_{10}^{\mathrm{un}}$ transformation law under
...
\begin{theorem}[Nilpotence of obstruction (every genus); \ClaimStatusConditional]
\label{thm:obstruction-nilpotent-all-genera}
\label{thm:obstruction-nilpotent-higher}% backward-compatible label
\index{obstruction class!nilpotence!every genus}
...
$\lambda_g^2 = 0$ for \emph{every} genus, including $g = 3$
...
For uniform-weight algebras, the same nilpotence holds in every genus;
...
\begin{remark}[Scalar-diagonal scope]
\label{rem:scalar-diagonal-honest}
\index{scalar-diagonal hypothesis!scope}
The $K$-theoretic construction in
...
\emph{Relative Euler characteristic in $K$-theory.} The filtration varies holomorphically
...
\emph{Scalar-lane upgrade via the top Chern character.} Apply the Chern character and read off
...
\textbf{GRR on the universal curve.}
...
\textbf{Determinant of cohomology and the
...
\textbf{The Hodge bundle and its Arakelov metric.}
The Hodge bundle
...
\textbf{The determinant line bundle and the
Quillen metric.}
...
\textbf{The Bismut--Gillet--Soul\'e anomaly formula.}
...
\textbf{From the Arakelov curvature to
$c_g(\mathbb{E})$.}
...
\emph{Chern--Weil is corroborative, not a second derivation.}
The linearity of $\mathrm{obs}_g(\cA)$
in~$\kappa(\cA)$ is established by the scalar-lane
$K$-theoretic calculation: the virtual class
$[\barB^{(g)}_{\mathrm{scalar}}(\cA)]^{\mathrm{vir}}
= \kappa(\cA)\cdot \lambda_{-1}(\mathbb{E})$
...
The differential-geometric machinery (Arakelov--Faltings
...
Lemma~\ref{lem:k-theoretic-globalization-bar}. Concretely, the splitting principle applied to the
...
established by the scalar-lane $K$-class, not an independent
derivation.
...
Lemma~\ref{lem:k-theoretic-globalization-bar} and Mumford's
$c_g(\mathbb{E}) = \lambda_g$ suffice.
...
\textbf{The bridge from fiber curvature to
base class.}
...
\textbf{Combining the three inputs:} the scalar obstruction class
...
factor~$\mathbb{E}$. Grothendieck--Riemann--Roch computes
$c_g(\mathbb{E}) = \lambda_g$ via GRR.
The determinant-of-cohomology bridge identifies the chain-level
Arakelov curvature
from the quantum Arnold relations
\textup{(}Theorem~\textup{\ref{thm:quantum-arnold-relations})}
with the curvature of the Hodge bundle via
...
\begin{remark}[The PTVV route relative to
Proposition~\textup{\ref{prop:scalar-obstruction-hodge-euler}}]
...
\emph{Comparison of the two presentations.}
...
\caption{Genus-$g$ obstruction classes. For scalar-diagonal families
 ...
 \textup{(}UNIFORM-WEIGHT, SCALAR-DIAGONAL\textup{)} holds unconditionally in every genus for
...
\textup{[Regime: curved-central on the Koszul locus; each genus
\textup{(}Convention~\textup{\ref{conv:regime-tags})}.]}
...
$\delta F_1^{\mathrm{cross}}(\cA)=0$ in the stated all-weight genus-$1$ setting.
If $h_1=\cdots=h_r$ \textup{(}the uniform-weight scalar lane of
Definition~\textup{\ref{def:scalar-lane}}\textup{)}, then
$\delta F_g^{\mathrm{cross}}(\cA)=0$ for every $g \geq 1$, and the
...
In particular, affine Kac--Moody duality gives opposite curvatures
...
algebras directly),
the Virasoro curvature sum is $13$
($\varrho = 1/2$, $K = 26$),
and the $\mathcal{W}_3$ curvature sum is $250/3$
($\varrho = 5/6$, $K = 100$).
...
The Heisenberg dual has opposite curvature:
$\kappa(\mathcal{H}_\kappa^!) = -\kappa$,
...
\textup{(}\text{ALL-WEIGHT} + \delta F_g^{\mathrm{cross}}\textup{}),
\]
with $\delta F_1^{\mathrm{cross}}(\cA)=0$ in the stated all-weight genus-$1$ setting and
...
\item[\textup{Level~2.}] \emph{Koszul} (curve-independent
 ...
 uniform-weight algebras in every genus, and
 $\mathrm{obs}_1 = \kappa \cdot \lambda_1$
 \textup{(g=1 only; all-weight at g=1 is unconditional)}
 unconditionally
...
\item[\textup{Level~3.}] \emph{Analytic} (sewing and
 ...
 provides convergent partition functions in every genus.
...
In each genus the \emph{edges} of the genus-$g$ graph sum carry
...
formula $F_g = \kappa \cdot \lambda_g^{\mathrm{FP}}$ \textup{(}UNIFORM-WEIGHT\textup{)} hold unconditionally in every genus. For multi-generator
...
the Borcherds pushforward on the CY side, with the unnormalised Igusa
square $\Phi_{10}^{\mathrm{un}}=\Delta_5^2$ and the weight-$5$ class $\Delta_5$ of
...
weight-$10$ cusp form. The higher-genus modular Koszul
