...
\begin{theorem}[Leray decomposition of the corrected differential; \ClaimStatusProvedHere]
...
At genus~$g \geq 2$: $\lambda_g = c_g(\mathbb{E})$ has degree~$g < 3g{-}3 = \dim_{\mathbb{C}}\overline{\mathcal{M}}_g$, so the obstruction class $\mathrm{obs}_g = \kappa \cdot \lambda_g$ \textup{(}UNIFORM-WEIGHT, SCALAR-DIAGONAL\textup{)} is a non-trivial class in $H^{2g}(\overline{\mathcal{M}}_g)$ but is \emph{not} a top-degree class. Its numerical evaluation requires pairing with a complementary tautological class of degree~$2g{-}3$; the relevant intersection numbers are computable from the Faber--Pandharipande formula involving Bernoulli numbers.
...
holds in every genus because $\mathrm{obs}_g=\kappa\lambda_g$.
...
The all-weight numerical identity
...
$F_g = \kappa \cdot \lambda_g^{\mathrm{FP}}$ is not the full
...
$H^*(\overline{\mathcal{M}}_g)$ appears at genus~$g$.
 The all-weight numerical identity
 $F_g = \kappa \cdot \lambda_g^{\mathrm{FP}}$ \emph{fails}
...
$\mathrm{obs}_g=\kappa\lambda_g$, so
\[
\mathrm{obs}_g(\mathcal{A}\otimes\mathcal{B})
...
$\bar{B}^{\mathrm{ch}}(\mathcal{A})$ carries tautological data beyond
...
$H^*(\widehat{\barB}^{(g)}(\cA))$ for every fixed genus~$g$.
