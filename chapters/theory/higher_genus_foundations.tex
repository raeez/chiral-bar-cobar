...
boundary is not a single divisor with a single scalar residue:
separating clutching pulls the Hodge bundle back as a direct sum, while
...
first calculation, not the boundary law in higher genus. The passage
...
curvature is the Arakelov form
$\omega_g^{\mathrm{Ar}}$ on the fiber; the degree-$2$ Hodge class on
the base is $\lambda_1=c_1(\det\mathbb{E})$. The scalar
genus-$g$ obstruction uses the Euler polynomial of the full
rank-$g$ Hodge bundle, hence the top class
$\lambda_g=c_g(\mathbb{E})$, not the fiber form itself.
Theorem~\ref{thm:genus-universality} upgrades this fiberwise
statement to the genus-$g$ free-energy package
\[
F_g(\cA)
\;=\;
F_g^{\mathrm{sc}}(\cA)
...
\item[\textup{(i)}] \textbf{Fiberwise curved differential} $\dfib$.
 ...
 (a specific smooth chain on~$\Sigma_g$ carrying chain-level data).
 On the uniform-weight scalar lane, the cohomological curvature
 element
 $\mcurv{g} = \kappa(\cA)\cdot\lambda_g
 \in H^{2g}(\overline{\cM}_g)$ is obtained by taking the
 zero-mode determinant of the rank-$g$ Hodge bundle and applying the
 Chern--Weil Euler polynomial. It is not the class of
 $\omega_g^{\mathrm{Ar}}$ itself and not a single fiber integral;
 the fiberwise form supplies the curvature representative whose
 Hodge determinant is $c_g(\mathbb{E})$
...
\item \emph{Chain-level Gauss--Manin connection.}\;
 ...
 \otimes \Omega^*_{\log})$
 over the smooth locus of $\overline{\cM}_g$ carries the
 ...
 + \Theta_{\mathrm{KS}},
 \end{equation}
 where $\Theta_{\mathrm{KS}}$ is the connection one-form obtained by
 composing the Kodaira--Spencer map
 $T_{\overline{\cM}_g}\to R^1\pi_*T_{\Sigma_g}$ with cup product on
 the variation of Hodge structure. In a local Torelli chart this
 connection is written in the coordinates
 $\Omega_{\alpha\beta}$ of the period matrix. The normalized
 holomorphic differentials $\omega_k$ are a moving frame of the
 Hodge bundle, not one-forms on the base, and the normalized
 $A$-periods are fixed by
 $\oint_{A_l}\omega_k=\delta_{kl}$.
...
\item \emph{Chain-level uncurving identity.}\;
 The total corrected differential on the relative de~Rham bar complex
 \[
 \Dg{g}=\dfib+d_{\mathrm{per}}^{(g)}
 +\nabla_{\mathrm{KS}}^{\mathrm{GM}}
 \]
 ...
 \textup{(}the Riemann bilinear relation and the Lagrangian
 normalization of the $A$-cycle subspace cancel the Arakelov form
 against the period correction\textup{)}, and
 $(d_{\mathrm{per}}^{(g)}+\nabla_{\mathrm{KS}}^{\mathrm{GM}})^2=0$
 \textup{(}flatness of the corrected relative de~Rham connection\textup{)}.
...
\item \emph{The uncurving is not a gauge equivalence.}\;
 ...
 Rather, $d_{\mathrm{per}}^{(g)}
 +\nabla_{\mathrm{KS}}^{\mathrm{GM}}$ is independent geometric datum
 (the period correction on the fiber together with the connection on
 the base $\overline{\cM}_g$), and the
...
The connection is computed in a local Torelli chart by
differentiating the period matrix and projecting the variation of the
normalized basis $\omega_k$ back to the Hodge filtration. The
normalization $\oint_{A_l}\omega_k=\delta_{kl}$ fixes the symplectic
gauge; it is not a system of flat deformation coordinates. Flatness
$(\nabla^{\mathrm{GM}})^2 = 0$ is the Gauss--Manin theorem
(Griffiths~\cite{Griffiths68}).

(ii)~The anticommutator
$\{\dfib, d_{\mathrm{per}}^{(g)}
 +\nabla_{\mathrm{KS}}^{\mathrm{GM}}\}$
computes the obstruction to commutativity of the fiberwise bar
differential with the period-corrected base connection. The computation
...
period correction $d_{\mathrm{per}}^{(g)}$ compensates exactly the
non-holomorphic part of the propagator that produces the Arnold
defect, while $\nabla_{\mathrm{KS}}^{\mathrm{GM}}$ transports the
zero modes along the base. The chain-level cancellation
$\dfib^{\,2} + \{\dfib, d_{\mathrm{per}}^{(g)}
 +\nabla_{\mathrm{KS}}^{\mathrm{GM}}\} = 0$
...
\item \emph{Corrected differential.}\;
 ...
 satisfies $\Dg{g}^{\,2} = 0$
 \textup{(}Theorem~\textup{\ref{thm:quantum-diff-squares-zero})}.
 The period operator $d_{\mathrm{per}}^{(g)}$ is built from the
 Abel--Jacobi and period-matrix terms in the Arakelov-normalized
 propagator, and the Gauss--Manin term transports the Hodge zero
 modes. Together they cancel the curvature via the Riemann bilinear
 relation and the Lagrangian normalization of the
...
\item \emph{Hodge bundle structure.}\;
 ...
 $F_g^{\mathrm{sc}}(\cA)$, whose generating function on the uniform-weight lane is
...
\item \emph{Chain representative determines cohomology.}\;
 ...
 The Arakelov metric and the standard period-matrix curvature
 description of the Hodge metric identify the geometric zero-mode
 sector of the bar curvature with this Hodge curvature matrix, and
 Chern--Weil gives
...
curvature matrix. The Arakelov metric and the standard
period-matrix curvature description identify the zero-mode sector of
the bar curvature with $\Theta_{\mathbb{E}}$, and Chern--Weil gives
\[
\det\!\Bigl(\frac{i}{2\pi}\Theta_{\mathbb{E}}\Bigr)
\;=\;
c_g(\mathbb{E})
...
Arakelov metric, determinant-of-cohomology, and GRR on the universal curve;
...
the Verdier-dual currents encode the coalgebra structure of
$\cA^!_\infty$.
...
\item Curvature element
 $m_0^{(g)} = \kappa(\cA) \cdot
 \omega_g^{\mathrm{Ar}}\cdot\mathbf{1}$,
...
\begin{proof}
The curvature
$\dfib^{\,2} = \kappa(\cA)\cdot\omega_g^{\mathrm{Ar}}$ is proved
...
\begin{theorem}[Bar-cobar isomorphism, retained for equation labels; \ClaimStatusConditional]
...
(d_{\mathrm{per}}^{(g)}
+\nabla_{\mathrm{KS}}^{\mathrm{GM}})^2
+\{\dfib,\sum_{r\geq1}d_{\partial,r}^{(g)}\}
+\cdots
&=0 ,
\label{eq:leray-boundary-cancels}\\
&\vdots
\end{align}
The equation $(d^{(0)})^2=0$ is reserved for the genus-$0$
tangent-screen collision differential $\dzero$; it is not the
positive-genus fiberwise statement, where the square is the
curvature in~\eqref{eq:leray-fiber-curvature}.
\end{theorem}
...
$d_{\mathrm{per}}^{(g)}$, while variation of the Hodge bundle over
the base gives $\nabla_{\mathrm{KS}}^{\mathrm{GM}}$. The Riemann
bilinear relation and Gauss--Manin flatness give
...
\;=\;
2\pi i\bigl(\delta_{z=w}-\omega_1\bigr),
\qquad
...
$-\omega_1$ is the positive-genus Arnold defect. After contraction
with the Heisenberg OPE coefficient it gives
\[
\dfib^{\,2}=k\,\omega_1\cdot\mathrm{id}.
\]
...
+\nabla_{\mathrm{KS}}^{\mathrm{GM}}.
\]
The heat equation for $\vartheta_1$ and the anomalous transformation
law of $E_2$ say precisely that the Kodaira--Spencer variation of the
prime-form propagator has central curvature $-k\,\omega_1$ in the
Heisenberg channel. Equivalently,
\[
\{\dfib,d_{\mathrm{per}}^{(1)}
+\nabla_{\mathrm{KS}}^{\mathrm{GM}}\}
+
...
=
-\,k\,\omega_1\cdot\mathrm{id}.
\]
Adding this identity to $\dfib^{\,2}=k\,\omega_1\cdot\mathrm{id}$
gives $\Dg{1}^{\,2}=0$. Thus nilpotence is a cancellation in the
period-corrected total differential, not a termwise vanishing of the
residue, elliptic, and modular pieces.
\end{proof}
...
\textup{(g=1 only; all-weight at g=1 is unconditional)}:
\begin{equation}
\mathrm{obs}_1(\mathcal H_\kappa)
...
\textup{(g=1 only; all-weight at g=1 is unconditional)}:
\begin{equation}
\int_{\overline{\mathcal{M}}_{1,1}} \mathrm{obs}_1(\mathcal H_\kappa)
...
\textup{(}UNIFORM-WEIGHT, SCALAR-DIAGONAL\textup{)}; at genus~$1$ the same scalar formula is
\textup{(g=1 only; all-weight at g=1 is unconditional)}.
...
The full bar differential decomposes as:
 ...
 (We use the closed-string convention $\hbar^{2g-2}$; the alternative
 Feynman-diagram convention uses $\hbar^{g-1}$, corresponding to
 $\hbar_{\text{there}} = \hbar_{\text{here}}^2$.)
...
\qquad \textup{(}UNIFORM-WEIGHT, SCALAR-DIAGONAL\textup{)} \end{equation}
...
fiberwise Arakelov curvature form $\omega_g^{\mathrm{Ar}}$.
The base class is obtained from the zero-mode determinant of the
Hodge bundle: at $g=1$ this is the Quillen determinant anomaly
$c_1(\mathbb{E})$~\cite{Quillen85}, while for general~$g$ the
splitting-principle/Chern--Weil Euler polynomial gives the top class
$c_g(\mathbb{E})$. The fiberwise result is:
\begin{equation}
...
At genus~$g \geq 2$: $\lambda_g = c_g(\mathbb{E})$ has degree~$g < 3g{-}3 = \dim_{\mathbb{C}}\overline{\mathcal{M}}_g$, so the obstruction class
\(\mathrm{obs}_g(\cA)=\kappa(\cA)\lambda_g\)
\textup{(}UNIFORM-WEIGHT, SCALAR-DIAGONAL\textup{)} is a non-trivial class in $H^{2g}(\overline{\mathcal{M}}_g)$ but is \emph{not} a top-degree class. Its numerical evaluation requires pairing with a complementary tautological class of degree~$2g{-}3$; the relevant intersection numbers are computable from the Faber--Pandharipande formula involving Bernoulli numbers.

\emph{Step 6: Obstruction class.}

Therefore:
\[
\mathrm{obs}_g(\mathcal H_\kappa)
=\kappa(\mathcal H_\kappa)\lambda_g
=\kappa\lambda_g
\qquad \textup{(}UNIFORM-WEIGHT, SCALAR-DIAGONAL\textup{)}\]
...
For the affine Kac--Moody vertex algebra $\widehat{\mathfrak{g}}_k$
at level $k \neq -h^\vee$, the scalar-diagonal uniform-weight
genus-$g$ obstruction is:
\begin{equation}
...
At genus~$1$ the scalar formula is unconditional; for non-abelian
generic level at $g\ge2$ the scalar-diagonal hypothesis is the stated
scope condition.

For specific Lie algebras :
\begin{align}
\mathfrak{g} = \mathfrak{sl}_2: \quad \text{obs}_g &= \frac{3(k+2)}{4} \lambda_g \textup{(}UNIFORM-WEIGHT, SCALAR-DIAGONAL\textup{)} \\
\mathfrak{g} = \mathfrak{sl}_3: \quad \text{obs}_g &= \frac{4(k+3)}{3} \lambda_g \textup{(}UNIFORM-WEIGHT, SCALAR-DIAGONAL\textup{)} \\
\mathfrak{g} = E_8: \quad \text{obs}_g &= \frac{62(k+30)}{15} \lambda_g \textup{(}UNIFORM-WEIGHT, SCALAR-DIAGONAL\textup{})
\qquad \end{align}

The collision residue convention is the trace-form convention
\[
r^{\mathrm{coll}}_{V_k(\fg)}(z)=\frac{k\,\Omega_{\mathrm{tr}}}{z}.
\]
The shifted scalar
$\kappa(V_k(\fg))=\dim(\fg)(k+h^\vee)/(2h^\vee)$ comes from the
level double-pole channel plus the simple-pole Lie bracket channel.
It is not obtained by replacing the collision residue by the KZ
normalization $\Omega/((k+h^\vee)z)$.
...
m_0 = \frac{k + h^\vee}{2h^\vee} \cdot \Omega_{\mathrm{tr}}
\end{equation}
where $\Omega_{\mathrm{tr}} = \sum_a J^a \otimes J_a$ is the trace-form Casimir element. This curvature arises
...
The passage from this fiberwise form to the base obstruction is the
same zero-mode determinant calculation as in the Heisenberg case
(Theorem~\ref{thm:heisenberg-obs}, Step~5): the algebraic trace gives
the scalar coefficient, and the Chern--Weil Euler polynomial of the
Hodge bundle gives $\lambda_g=c_g(\mathbb{E})$.

\emph{Step 3: Assembling the formula.}

Each generator $J^a$ contributes a copy of the curvature $m_0$. Summing over
$a = 1, \ldots, \dim(\mathfrak{g})$ and noting that
$\Omega_{\mathrm{tr}}$ evaluated on the trace gives
...
The scalar obstruction on the scalar-diagonal lane is:
...
as a full all-weight coefficient fails for multi-weight
...
so \(\mathrm{obs}_1(\cA)^2=\kappa(\cA)^2\lambda_1^2=0\)
by Mumford's relation $\lambda_g^2 = 0$.
For scalar-diagonal uniform-weight algebras, the same nilpotence
holds in every genus because
\(\mathrm{obs}_g(\cA)=\kappa(\cA)\lambda_g\).
For multi-weight algebras at $g\geq2$, the full obstruction class
contains cross-channel terms; nilpotence must be checked for that
class and is not a formal consequence of the scalar formula.
\end{remark}
...
On the scalar-diagonal lane write, on the fixed space
$\overline{\mathcal M}_g$,
\[
\Lambda_i(\cA):=\kappa(\cA)\lambda_i
\qquad (0\leq i\leq g),\qquad \Lambda_0(\cA):=\kappa(\cA).
\]
The top member $\Lambda_g(\cA)$ is the genus-$g$ scalar obstruction.
The normalized classes $\lambda_i=\kappa^{-1}\Lambda_i$ for
$\kappa\neq0$ satisfy Mumford's relation; equivalently the
unnormalized $\Lambda_i$ satisfy the following relations.
\begin{enumerate}[label=\textup{(\alph*)}]
\item \textbf{Nilpotence} (Theorem~\ref{thm:obstruction-nilpotent-all-genera}):
$(\Lambda_g)^2 = 0$ for all $g \geq 1$.
...
(-1)^j \Lambda_i \cdot \Lambda_j = 0
\qquad \text{in }
\mathrm{CH}^k(\overline{\mathcal{M}}_g)
\otimes Z(\mathcal{A})^{\otimes 2}.
\end{equation}
\item \textbf{Recursive structure}: Setting $k = 2$ in~\eqref{eq:mumford-degree-k}
gives $\Lambda_1^2 = 2\Lambda_0\Lambda_2$, i.e.
$(\kappa\lambda_1)^2=2\kappa(\kappa\lambda_2)$.
More generally, for $g \geq 3$, setting $k = 2g - 2$ gives the
Hodge recurrence
...
Equivalently,
\[
\Lambda_{g-1}^2=2\Lambda_g\Lambda_{g-2}.
\]
\end{enumerate}
\end{corollary}

\begin{proof}
Part~(b) follows from extracting the degree-$k$ component of
Mumford's relation
$c(\mathbb{E}) \cdot c(\mathbb{E}^\vee) = 1$, then multiplying
each term by the common factor $\kappa^2$. Part~(a) is the case
$k = 2g$.
...
\textup{(}UNIFORM-WEIGHT, SCALAR-DIAGONAL\textup{)}, Theorem~\ref{thm:obstruction-nilpotent-all-genera} gives
...
% Two comparison routes, not disjoint at the scalar bridge:
% (A) GRR on the universal curve,
...
$\Theta_{\mathbb{E}} \in
\Omega^{1,1}(\overline{\cM}_g, \mathrm{End}(\mathbb{E}))$.
The Arakelov metric construction, together with the standard
period-matrix curvature computation for the Hodge metric
\cite{Ara74,Fal84,Soule92}, identifies the fiberwise zero-mode
form entering this curvature with the Arakelov form:
...
\textup{)}.
This is a fiberwise zero-mode identification, not a theorem saying
that a base Chern class is already present on a single fiber. The
Chern--Weil homomorphism applied to the full
...
The differential-geometric machinery (Arakelov metric and
period-matrix curvature identification, Quillen metric, BGS anomaly formula)
...
\emph{Scope of Faber's $\lambda_g$ conjecture.}
No Faber conjecture is used in the direct scalar-diagonal argument.
Lemma~\ref{lem:k-theoretic-globalization-bar} identifies the
scalar virtual class in $K^0(\overline{\cM}_g)\otimes\Q$, and the
Chern character gives the degree-$2g$ class
$\kappa(\cA)\lambda_g$ in rational cohomology for every
$g\geq1$. Faber's $\lambda_g$ conjecture enters only the
\emph{independent clutching-only route}: boundary restrictions
determine the class on the socle quotient
$R^g/\cN^g$, and the conjectural vanishing of the numerical kernel
$\cN^g$ would upgrade that boundary-only proof to equality in the
full tautological ring for $g\geq3$.
...
$\Theta_{\mathbb{E}}|_{\mathrm{fiber}}
 = \omega_g^{\mathrm{Ar}}$
\textup{(}the Arakelov metric curvature formula,
...
\item The BGS anomaly formula~\eqref{eq:bgs-anomaly}
 supplies the determinant-of-cohomology fiber integral and
 the Quillen-metric representative of the determinant-line
 curvature on the base~$\overline{\cM}_g$. The full rank-$g$
 Chern classes are then obtained by applying Chern--Weil to
 $\Theta_{\mathbb{E}}$ and, equivalently, by the splitting
 principle for the Chern roots of~$\mathbb E$.
...
\textup{)}.
The passage \textup{(ii)} $\to$ \textup{(iii)} uses the
determinant-of-cohomology/BGS bridge: it converts fiberwise
curvature data into the Quillen Chern--Weil representative of the
determinant line on the base via fiber integration. The passage from
that rank-$1$ determinant representative to the top Hodge class uses
the full Chern polynomial of~$\mathbb E$ and the splitting principle.
...
the Arakelov metric curvature formula, and the
Bismut--Gillet--Soul\'e anomaly formula converts this
fiberwise curvature into the determinant-line Chern--Weil
representative on the base; the splitting principle for
\(\mathbb E\) then extracts \(c_g(\mathbb E)=\lambda_g\).
Therefore
\[
\mathrm{obs}_g(\cA)
\;=\;
...
\begin{enumerate}[label=\textup{(\alph*)}]
\item The Arakelov metric construction and the period-matrix
 curvature formula identify the fiberwise bar curvature with the
 fiberwise zero-mode part of the Hodge-bundle curvature
 \textup{(}a fiberwise statement, both sides living on~$\Sigma_g$\textup{)}.
...
\item The Bismut--Gillet--Soul\'e anomaly
 ...
 $c_1(\lambda)$ on the base~$\overline{\cM}_g$. The
 top class $c_g(\mathbb{E})$ is then read from the full
 Chern polynomial of the rank-$g$ Hodge bundle via the
 splitting principle.
\item Mumford's GRR computation~\cite{Mumford83} confirms
 $c_g(\mathbb{E}) = \lambda_g$.
\end{enumerate}
The external inputs are: Arakelov's metric construction and
Faltings' Hodge-metric extension \textup{(}classical, 1974/1984\textup{)},
the BGS anomaly formula
...
ring. The direct Arakelov-metric/BGS/GRR route already proves
\[
\mathrm{obs}_g(\cA)
\;=\;
\kappa(\cA)\cdot\lambda_g
...
$\mathrm{obs}_g/\kappa = \lambda_g$ uniquely" means:
...
Arakelov-metric/BGS chain), its non-separating restriction
...
or the period-matrix Hodge-curvature identification.
...
\caption{Genus-$g$ obstruction classes. For scalar-diagonal families
 ...
 \(\mathrm{obs}_g(\cA)=\kappa(\cA)\lambda_g\)
 \textup{(}UNIFORM-WEIGHT, SCALAR-DIAGONAL\textup{)} holds on the
 nose in every genus by the Arakelov--BGS/Chern--Weil route of
 Proposition~\textup{\ref{prop:scalar-obstruction-hodge-euler}}.
 The independent clutching/PTVV route proves the same class on the
 socle quotient $R^g/\cN^g$ for $g \geq 3$ and on the nose in the
 low genera where $\cN^g=0$.
...
$\delta F_2^{\mathrm{cross}}(W_3) = (c+204)/(16c)$
 \textup{(}explicit full-coefficient correction proved at $g=2$%
 \textup{)} is present
...
\begin{tabular}{|l|c|c|}
\hline
\textbf{Chiral Algebra} & \textbf{Scalar lane / all-weight qualification} & \textbf{Physical Meaning} \\
\hline
...
\hline
$W_3(c)$ & $\frac{5c}{6}\lambda_g$ scalar projection\rlap{${}^*$} & Conformal anomaly plus mixed channels \\
\hline
Virasoro $(c)$ & $\frac{c}{2} \lambda_g$ & Conformal anomaly \\
\hline
\multicolumn{3}{|l|}{\small ${}^*$Unconditional at $g=1$; for $g \geq 2$ the all-weight free energy includes $\delta F_g^{\mathrm{cross}}$.}\\
\hline
\end{tabular}
\end{table}
...
If $h_1=\cdots=h_r$ and $\cA$ satisfies the scalar-diagonal
hypothesis \textup{(}Definition~\textup{\ref{def:scalar-diagonal-hypothesis}}\textup{)}
\textup{(}the proved uniform-weight scalar lane\textup{)}, then
...
$\kappa = (k + h^\vee)\dim\mathfrak{g}/(2h^\vee)$ is a semisimple
affine formula and has no meaning at $h^\vee = 0$; the abelian case
is defined directly by
$\kappa(\mathcal{H}_\kappa) = \kappa$ from the genus-$1$ curvature
...
\item[\textup{Level~2.}] \emph{Koszul} (curve-independent
 ...
 \(\kappa(\cA)\), $S_r$, and the modular characteristic
 \(\mathrm{obs}_g(\cA)=\kappa(\cA)\lambda_g\) for
 scalar-diagonal uniform-weight algebras in every genus, and
 \(\mathrm{obs}_1(\cA)=\kappa(\cA)\lambda_1\)
 \textup{(g=1 only; all-weight at g=1 is unconditional)}
 unconditionally
...
at genus~$1$, \(\mathrm{obs}_1(\cA)=\kappa(\cA)\lambda_1\)
\textup{(g=1 only; all-weight at g=1 is unconditional)} with
\(\kappa(\cA)=\sum_i \kappa_{h_i}\), unconditionally
...
The assertion that the full all-weight coefficient equals the scalar
trace \(\kappa(\cA)\lambda_g^{\mathrm{FP}}\)
\emph{fails} at $g \geq 2$ for multi-weight algebras
($\mathcal{W}_N$, $N \geq 3$): instead the full coefficient decomposes as
\(F_g=F_g^{\mathrm{sc}}+\delta F_g^{\mathrm{cross}}\) with
...
\begin{remark}[Higher-Hodge genus-$2$ diagnostic]
\label{rem:higher-hodge-genus2-diagnostic}
The exclusion of the higher Hodge bundles is visible already at
genus~$2$. If one inserts a weight-$j$ Hodge bundle
\(\mathbb E_j=R^0\pi_*\omega^{\otimes j}\) into the compactified
calculation, the resulting diagnostic has the form
\[
 I_2(j;P)
 =
 \frac{e(j)^2}{480}
 ...
 \qquad
 e(j)=6j^2-6j+1,\quad
 B_3(j)=j^3-\frac32j^2+\frac12j .
\]
Here \(P=\int_{\overline{\mathcal M}_2}\kappa_1 c_3\) denotes the
compactified boundary pairing supplied by the full
Mumford--Chiodo GRR calculation. The polynomial has \(j^4\)
coefficient \(3/40\) for every value of~\(P\), so it is a genuine
higher-Hodge diagnostic. It is not the bar-complex scalar lane:
the bar propagator uses the standard weight-$1$ Hodge bundle. The
collision value
\[
 P=\frac{1003}{8640}
\]
gives \(I_2(1;P)=I_2(2;P)=7/5760\), the
Faber--Pandharipande scalar coefficient. Thus genus-$2$ numerical
coincidence between weights cannot identify a higher-Hodge formula
with the compactified bar obstruction. The theorem uses only
\(\lambda_2^{\mathrm{FP}}=7/5760\) on the scalar lane and the
mixed-channel graph correction \(\delta F_2^{\mathrm{cross}}\) on
the multi-weight lane.
\end{remark}

\begin{proposition}[Multi-weight failure of scalar genus universality;
\ClaimStatusConditional]%
...
$N \geq 3$, the scalar free-energy formula
\(F_g^{\mathrm{sc}}(\cA)=\kappa(\cA)\lambda_g^{\mathrm{FP}}\) is not the full
all-weight answer at $g \geq 2$.
...
$H^*(\overline{\mathcal{M}}_g)$ appears at genus~$g$.
 The assertion that the full all-weight coefficient equals the
 scalar trace \(\kappa(\cA)\lambda_g^{\mathrm{FP}}\) \emph{fails}
 at $g \geq 2$ for multi-weight algebras: the full free energy
...
For algebras with generators of the same conformal weight, the
factorization
\(\mathrm{obs}_g(\cA)=\kappa(\cA)\lambda_g\)
\textup{(}UNIFORM-WEIGHT, SCALAR-DIAGONAL\textup{)} and the
free-energy formula
\(F_g(\cA)=F_g^{\mathrm{sc}}(\cA)
=\kappa(\cA)\lambda_g^{\mathrm{FP}}\)
\textup{(}UNIFORM-WEIGHT, SCALAR-DIAGONAL\textup{)} hold on the proved
scalar-diagonal lane. This is unconditional for Heisenberg, Virasoro,
and rank-$1$ free fields; for non-abelian affine Kac--Moody it is
proved on the positive-integral lattice/coset realizations and is
conditional on scalar-diagonality at generic level. For multi-generator
multi-weight algebras ($\mathcal{W}_N$, $N \geq 3$),
...
the full higher-genus factorization \emph{fails}: the full coefficient
has a nonzero cross-channel summand
$\delta F_g^{\mathrm{cross}} \neq 0$
(Theorem~\ref{thm:multi-weight-genus-expansion}).
\end{remark}
...
scalar-diagonal lane, then
\[
	\mathrm{obs}^{\mathrm{sc}}_g(\mathcal{A} \otimes \mathcal{B})
	= \mathrm{obs}^{\mathrm{sc}}_g(\mathcal{A}) + \mathrm{obs}^{\mathrm{sc}}_g(\mathcal{B})
\qquad (g\ge1).
\]
...
additive. On the scalar-diagonal lane,
	\(\mathrm{obs}^{\mathrm{sc}}_g(\cA)=\kappa(\cA)\lambda_g\), so
\[
	\mathrm{obs}^{\mathrm{sc}}_g(\mathcal{A}\otimes\mathcal{B})
	= (\kappa(\mathcal{A})+\kappa(\mathcal{B}))\lambda_g
	= \mathrm{obs}^{\mathrm{sc}}_g(\mathcal{A})+\mathrm{obs}^{\mathrm{sc}}_g(\mathcal{B}).
\]
For all-weight tensor products, the same argument proves only the
additivity of the scalar coefficient; it does not identify or cancel
the cross-channel summands.
\end{proof}
...
\item the scalar fiberwise bar model is uncurved:
$m_0=0$, equivalently $d_{\mathrm{fib}}^2=0$ on the genus-$1$
curvature lane.
\end{enumerate}
On the proved scalar lane, these equivalent conditions further imply
	$\mathrm{obs}^{\mathrm{sc}}_g(\mathcal{A}) = 0$ and $F_g^{\mathrm{sc}}(\mathcal{A}) = 0$ for all
...
When $m_0=0$, $H^0(\bar{B}^{\mathrm{ch}}(\mathcal{A}))$ is a genuine
associative chiral algebra. The internal commutator identity
$m_1^2(a)=[m_0,a]$ is not itself a curvature-vanishing criterion:
central nonzero curvature can make the commutator vanish.
\end{corollary}
...
(i)$\Leftrightarrow$(iv): The scalar curvature $m_0$ is proportional to
\(\kappa(\cA)\) (Proposition~\ref{prop:km-bar-curvature} for
Kac--Moody; the general case follows from the genus-$1$ computation).
Thus \(\kappa(\cA)=0\) iff $m_0 = 0$ iff
$d_{\mathrm{fib}}^2=0$ on the scalar fiberwise model. The curved
$A_\infty$ identity remains
$m_1^2(a)=m_2(m_0,a)-m_2(a,m_0)=[m_0,a]$; since the standard scalar
curvatures are central, the implication
$m_1^2=0\Rightarrow m_0=0$ is not valid without a noncentrality
...
$m_0=0$, the bar complex is a strict dg coalgebra, so its cobar is
...
\[
\mathrm{obs}^{\mathrm{sc}}_g(\mathcal{A}) = \kappa(\mathcal{A}) \cdot \lambda_g
\qquad \textup{(}UNIFORM-WEIGHT, SCALAR-DIAGONAL\textup{)}
...
scalar projection of this spectral sequence is already the Hodge
Euler class $\kappa\cdot\lambda_g$; higher differentials may produce
...
\(\mathrm{obs}_g(\cA)=\kappa(\cA)\lambda_g\)
\textup{(}UNIFORM-WEIGHT, SCALAR-DIAGONAL\textup{)}
by Theorem~\ref{thm:genus-universality} is
...
Equivalently:
\begin{enumerate}[label=\textup{(\roman*)}]
\item If \(\kappa(\cA)\neq0\), the scalar obstruction classes
 $\mathrm{obs}_g(\cA)$ are nonzero for every genus $g \geq 1$;
\item If \(\kappa(\cA)=0\), the scalar obstruction classes vanish for all
...
\(\mathrm{obs}_g(\cA)=\kappa(\cA)\lambda_g\)
\textup{(}UNIFORM-WEIGHT, SCALAR-DIAGONAL; proved scalar lane, see
Theorem~\ref{thm:genus-universality}\textup{)}
with $\lambda_g \neq 0$ for all $g \geq 1$.
Thus $\mathrm{obs}_g(\cA)=0$ for some $g$ iff \(\kappa(\cA)=0\) iff
\(\mathrm{obs}_g(\cA)=0\) for all $g$ on that scalar lane. For arbitrary
modular Koszul algebras, the universal genus-$1$ identity
\(\mathrm{obs}_1(\cA)=\kappa(\cA)\lambda_1\)
\textup{(g=1 only; all-weight at g=1 is unconditional)} gives
\(\mathrm{obs}_1(\cA)=0 \iff \kappa(\cA)=0\).
\end{proof}
...
$F_g^{\mathrm{sc}}(\cA)=\kappa(\cA)\lambda_g^{\mathrm{FP}}$ with
$\lambda_g^{\mathrm{FP}}
= \int_{\overline{\mathcal{M}}_{g,1}}\psi_1^{2g-2}\lambda_g$.
...
$F_g=F_g^{\mathrm{sc}}(\cA)+\delta F_g^{\mathrm{cross}}(\cA)$,
...
\item \emph{Maximal loop order} \textup{(}$\ell = g$\textup{):}
 ...
 data. For Koszul $\cA$, the intrinsic genus-zero bar dual
 $\cA^i_{(0)} := H^*(\barB^{(0)}(\cA))$ is concentrated in
 degree~$0$, so the maximal-loop-order page reduces to the graph
 complex with coefficients in~$\cA^i_{(0)}$. In a finite Verdier
 window $\cA^i_{(0)} \cong (\cA^!)^\vee$.
\end{enumerate}
\end{proposition}
...
The $d_0$-cohomology replaces these with the intrinsic genus-zero
bar dual $H^*(\barB^{(0)}(\cA)) = \cA^i_{(0)}$ at each vertex. For
Koszul~$\cA$ this is concentrated in degree~$0$; after finite Verdier
duality one may write $\cA^i_{(0)} \cong (\cA^!)^\vee$. This is
genus-zero bar cohomology, not bar-cobar inversion.
\end{proof}
...
\]
a numerical-class identity binding the higher-genus modular-Koszul
trace to the BKM side of Vol~III through the stage-$2$ shadow of
$\PhiFA_3$. Without the finite source matrices, parity fixture,
comparison maps, no-extra-relations theorem, and PBW compatibility,
the Borcherds pushforward phrase denotes only this scalar target
coordinate; it does not construct a Hall source, a compact CoHA, or a
chain-level recognition map. See Part~\ref{part:modular-tower-physics} for
