...
\textup{(}the uniform-weight hypothesis\textup{)}.
...
\;=\;
(-1)^g\, c_g(\mathbb{E}) + \bigl(\text{lower degree}\bigr).
\]
In degree~$2g$ this is $(-1)^g\,c_g(\mathbb{E})
= (-1)^g\,\lambda_g$, and the sign is absorbed by the bar
...
The passage from $c_1$ \textup{(}which BGS computes
directly for the determinant line
$\lambda = \det(\mathbb{E})$\textup{)} to the higher
Chern classes $c_j(\mathbb{E})$ and in particular to
$c_g(\mathbb{E})$ is the splitting principle. Let
$f\colon F(\mathbb{E}) \to \overline{\cM}_g$ be the
full flag bundle of~$\mathbb{E}$; on~$F(\mathbb{E})$ the
pullback splits as a sum of line bundles
$f^*\mathbb{E} = L_1 \oplus \cdots \oplus L_g$, and
$f^*$ is injective on rational cohomology. Writing
$x_i = c_1(L_i)$, the total Chern class satisfies
$f^* c(\mathbb{E}) = \prod_{i=1}^{g}(1+x_i)$ and the top
Chern class obeys
\begin{equation}\label{eq:splitting-principle-top-chern}
f^* c_g(\mathbb{E}) \;=\; \prod_{i=1}^{g} c_1(L_i)
\;=\; x_1 \cdots x_g.
\end{equation}
The scalar-channel linearity of the bar curvature is
precisely the statement that fiberwise contraction
against the scalar generator produces the trace
$\operatorname{tr}(\kappa(\cA)\cdot\mathrm{id}_{\mathbb{E}})
= g\cdot\kappa(\cA)$ in the rank-$g$ Hodge bundle; on the
splitting $\mathbb{E} = L_1 \oplus\cdots\oplus L_g$ this
distributes as $\kappa(\cA)$ on each line summand $L_i$.
Dividing by~$g$ to isolate the scalar channel (equivalently,
selecting a single line summand from the trace) preserves
the coefficient~$\kappa(\cA)$ on each~$L_i$, so
applying BGS line-bundle-by-line-bundle gives
$c_1(L_i, h_{Q,i}) = \kappa(\cA)\cdot x_i^{(\mathrm{Ar})}$
for Chern--Weil representatives $x_i^{(\mathrm{Ar})}$ of
the split Arakelov curvature. Multiplying across $i=1,\ldots,g$
via~\eqref{eq:splitting-principle-top-chern} yields
\[
f^*\bigl(\kappa(\cA)\cdot c_g(\mathbb{E})\bigr)
\;=\;
\prod_{i=1}^{g}\bigl(\kappa(\cA)\cdot x_i^{(\mathrm{Ar})}\bigr)
\;=\;
\kappa(\cA)^g \prod_{i=1}^{g} x_i^{(\mathrm{Ar})},
\]
and the scalar-channel projection extracts the
linear-in-$\kappa(\cA)$ component, which is the
elementary symmetric function $e_g = x_1\cdots x_g$ scaled
by a single copy of~$\kappa(\cA)$. This reproduces
$\mathrm{obs}_g(\cA) = \kappa(\cA)\cdot\lambda_g$ on
the base after descending along~$f^*$. The higher
elementary symmetric functions $e_j$ for $j<g$ produce
sub-top Chern classes $\lambda_j$, which are absorbed into
the total $\lambda$-class appearing in the virtual bar
identification of Step~1 (they do not affect the top
degree~$2g$).
...
\begin{proposition}[Clutching uniqueness of the Hodge Euler class;
\ClaimStatusProvedHere]
...
\end{enumerate}
Then $\alpha_g = \lambda_g$ in
$H^{2g}(\overline{\cM}_g,\Q)$.
\end{proposition}

\begin{proof}
The tautological ring $R^*(\overline{\cM}_g)$ is a finitely
generated graded subring of $H^*(\overline{\cM}_g,\Q)$; by
Faber--Pandharipande~\cite{FP00} it is spanned in degree
$2g$ by the kappa classes, the psi classes, and the lambda
classes, organised along the boundary stratification. The
Graber--Vakil relative virtual localisation and tautological
vanishing theorem~\cite{GraVak05} states that the kernel of
the simultaneous pullback
\[
R^*(\overline{\cM}_g) \;\longrightarrow\;
\bigoplus_{\xi} R^*(\partial^\xi \overline{\cM}_g),
\]
ranging over all boundary strata $\xi$ (one non-separating
stratum and $\lfloor g/2\rfloor$ separating strata), is zero in
degree $2g$: an element of the tautological ring in degree $2g$
is determined by its restrictions to the boundary.
Mumford~\cite{Mumford83} computed the restrictions of
$\lambda_g$ to each boundary stratum:
$\xi_{\mathrm{irr}}^*\lambda_g = \lambda_g$ on
$\overline{\cM}_{g-1,2}$ (the node contributes trivially at
degree $2g$) and
$\xi_h^*\lambda_g = \lambda_h \boxtimes \lambda_{g-h}$
(the Hodge bundle splits as a direct sum under separating
degeneration, and the top Chern class of a direct sum of
bundles of ranks $h$ and $g-h$ is the box product of their top
Chern classes). Conditions (b) and (c) match $\alpha_g$'s
boundary restrictions with those of $\lambda_g$. Combining
(a) with the Graber--Vakil boundary-injectivity statement gives
$\alpha_g - \lambda_g = 0$ in $R^{2g}(\overline{\cM}_g)$,
hence in $H^{2g}(\overline{\cM}_g,\Q)$ by tautological
inclusion.
\end{proof}
...
$H^{2g}(\overline{\cM}_g,\Q)$. Part~(iii) follows from
Proposition~\ref{prop:scalar-obstruction-hodge-euler}
combined with the uniqueness statement
Proposition~\ref{prop:clutching-uniqueness}: the
factorization-homology output is tautological by
construction, its non-separating and separating restrictions
match those of $\lambda_g$ by the factorization axioms
\textup{(}the PTVV pairing on a nodal curve factorises as the
product of pairings on the normalised components\textup{)},
and the scalar coefficient $\kappa(\cA)$ is detected on the
genus-$1$ component. Hence
$\int_{\overline{\cM}_g}\cA = \kappa(\cA)\cdot\lambda_g$.
\end{proof}

\begin{remark}[Independence of Proposition~%
\textup{\ref{prop:scalar-obstruction-hodge-euler}}]
\label{rem:ptvv-independence}
Proposition~\ref{prop:theorem-D-factorization-homology-alt}
does not use the Arakelov metric, the BGS anomaly formula, or
the Faltings identification of the Hodge-bundle curvature.
Its inputs are the factorization structure of $\cA$, the PTVV
transgression theorem~\cite{PTVV13}, and the AKSZ
construction~\cite{Costello17,CG-vol2}. The two propositions
agree on their common output, providing a second independent
proof of the scalar obstruction identity on the uniform-weight
lane. This is the $H$-series alternative proof \textup{(}GRR
on the universal curve via factorization homology\textup{)}
referenced in the Theorem~D entry of the theorem-status table.
\end{remark}
