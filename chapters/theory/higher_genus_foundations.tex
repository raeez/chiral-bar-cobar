...
\begin{proposition}[The genus-$g$ curvature package; \ClaimStatusConditional]
...
\begin{proposition}[Chain-level curvature operator; \ClaimStatusConditional]
...
\item \emph{Chain-level structure not recoverable from cohomology.}\;
 ...
 \in H^{1,1}(\Sigma_g)$ produce coderived-equivalent but
 not isomorphic curved bar complexes; ordinary quasi-isomorphism is
 not the invariant of a genuinely curved CDG object.
\end{enumerate}
\end{proposition}
...
$\exp(\kappa(\cA) \cdot \alpha)$ provides a filtered curved gauge
equivalence whose cone is coacyclic in the coderived CDG category,
but not an isomorphism of curved coalgebras. After passing through
the period-corrected square-zero model, the cobar functors applied to
the two representatives give quasi-isomorphic but potentially
non-isomorphic $A_\infty$ algebras: the difference is an exact
deformation, invisible in cohomology but present at chain level.
\end{proof}
...
\item \emph{Homotopical \textup{(}bar-cobar comparison in
 ...
 and Appendix~\textup{\ref{app:coderived-models})}.
...
\begin{theorem}[Convergence for filtered algebras; \ClaimStatusConditional]
...
\begin{theorem}[Obstruction theory for quantum corrections; \ClaimStatusConditional]
...
\begin{theorem}[Kac--Moody obstruction at genus \texorpdfstring{$g$}{g}; \ClaimStatusConditional]\label{thm:kac-moody-obs}
...
\begin{theorem}[\texorpdfstring{$W_3$}{W3} fiberwise obstruction; \ClaimStatusConditional]\label{thm:w3-obstruction}
...
\begin{computation}[Explicit genus-$1$ value of the $W_3$ obstruction; \ClaimStatusConditional]\label{comp:w3-obs-explicit}
...
\begin{theorem}[Nilpotence of obstruction (\texorpdfstring{$g \leq 2$}{g <= 2}); \ClaimStatusConditional]\label{thm:obstruction-nilpotent}
...
\begin{theorem}[Nilpotence of obstruction (all genera); \ClaimStatusConditional]
...
\begin{corollary}[Mumford multiplicative relations for obstruction classes; \ClaimStatusConditional]
...
\begin{theorem}[Genus universality; \ClaimStatusConditional]\label{thm:genus-universality}
...
\begin{corollary}[Additivity of the obstruction coefficient; \ClaimStatusConditional]\label{cor:kappa-additivity}
...
\begin{theorem}[Obstruction-deformation pairing; \ClaimStatusConditional]\label{thm:obs-def-pairing-explicit}
\textup{[Regime: curved-central on the Koszul locus; all genera
\textup{(}Convention~\textup{\ref{conv:regime-tags})}.]}
...
\begin{proposition}[Scalar obstruction lifting criterion; \ClaimStatusConditional]
...
\begin{theorem}[Wick-rotated \texorpdfstring{$\widehat{A}$}{A-hat} generating function on the proved scalar lane; \ClaimStatusConditional]
...
in Vol.~III, Chapter~\ref{V3-ch:k3-chiral-bialgebra-platonic}.
...
coefficients of $K3$. See Vol.~III, Chapter~\ref{V3-ch:k3-chiral-bialgebra-platonic},
