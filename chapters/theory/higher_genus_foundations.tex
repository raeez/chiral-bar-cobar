...
algebra~$\gAmod$ whose Maurer--Cartan elements encode the full
genus expansion.
...
$\Theta_\cA^{(g)} \in \mathrm{MC}(\gAmod)$ projects to: the
curvature~$\kappa(\cA)$ at the scalar level (Ring~1); the curved
differential~$\dfib$ at the differential level (Ring~2); and the
...
For $X = \PP^1$ with $D = \{0, 1, \infty\}$, the identification
$\operatorname{FM}_{n-3}(\PP^1 \mathbin{|} 0{+}1{+}\infty)
\cong \overline{\cM}_{0,n}$
of \cite[\S3.4.1]{Mok25} gives an explicit model: the log~FM space
