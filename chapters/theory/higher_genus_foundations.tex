...
% Macro safety net for cross-chapter wave-14 references (BRST functor of
...
% standalone extraction (AP-FM6/AP-FM36).
...
What changes at genus~$g \geq 1$ is that the bar differential
...
global. The fiberwise bar differential $\dfib$ inherits a
curvature term from these degenerations, and the genus-zero
identity $\dfib^{\,2} = 0$ fails by a measurable amount.

That amount is $\kappa(\cA) \cdot \omega_g$, and its appearance
is forced. Here $\omega_g$ denotes the Arakelov $(1,1)$-form on
the fiber~$\Sigma_g$; its pushforward along the universal curve
produces the Hodge class $c_1(\mathbb{E}) \in
H^2(\overline{\cM}_{g,n})$ on moduli. $\kappa(\cA)$ is the scalar extracted from the
$\Sigma_2$-coinvariant of the degree-two bar coefficient $r(z)$;
their product is the leading genus invariant that survives
averaging. The source of the curvature is geometric: the
irreducible stable graph
...
$\kappa$ through the trace of the identity on $\cA$, matched
with the class $\lambda_1 = \tfrac{1}{12}\delta_{\mathrm{irr}}$
on the moduli side. At higher genus the same mechanism repeats
through the Feynman transform, summing over stable graphs
weighted by handles. The passage from trees to stable graphs
is the passage from operads to modular operads, and the
obstruction tower $\Theta_\cA^{\leq r}$ records the higher-degree
modular data that no tree could ever see.
...
cancellation is the geometric origin of $D_\cA^2 = 0$ at all genera.
...
\S\ref{sec:koszul-across-genera}, organizes these corrections;
the present chapter establishes the foundations on which that tower
rests.
...
curvature is governed in degree~$2$ by the Hodge class
$\omega_g = c_1(\det\mathbb{E})$, where $\mathbb{E}$ is the Hodge
bundle. Theorem~\ref{thm:genus-universality} upgrades this
fiberwise statement to the genus-$g$ free-energy package
\[
F_g(\cA)
\;=\;
...
\]
with $\delta F_1^{\mathrm{cross}}=0$ universally. On the
uniform-weight lane the correction vanishes, and the cohomological
obstruction class becomes
$\mathrm{obs}_g(\cA)=\kappa(\cA)\cdot\lambda_g$ with
$\lambda_g = c_g(\mathbb{E})$ the \emph{top} Chern class of the Hodge
bundle. The scalar-lane generating function is then
...
Under the wave-14 universal conductor functor
(Chapter~\ref{chap:universal-conductor}) the scalar prefactor admits
the BRST-ghost reading
$\kappa(\cA) = K(\cA) = -c_{\mathrm{ghost}}(\BRST(\cA))$ on the
Koszul-self-dual locus, so the uniform-weight specialization
$\mathrm{obs}_g(\cA) = K(\cA) \cdot \lambda_g$ records the genus-$g$
ghost-charge cost of gauging directly on
$H^{2g}(\overline{\mathcal{M}}_g)$. The chiral climax of
Chapter~\ref{chap:climax-platonic} encodes the algebra side of this
identification ($d_{\mathrm{bar}} = \mathrm{KZ}^*(\nabla_{\mathrm{Arnold}})$,
$\kappa = -c_{\mathrm{ghost}}(\BRST)$), and the Koszul reflection
Theorem~\ref{thm:koszul-reflection} promotes the scalar-lane identity
from a per-family check to a Verdier-symmetric statement about the
bar/cobar pair $(\cA, \cA^!)$. The clutching-only residue at
$g \geq 3$ is isolated later as
Conjecture~\ref{conj:F6-lambda-g-clutching-uniqueness}, rather than as
a hypothesis of the all-weight theorem.
...
\item[\textup{(i)}] \textbf{Fiberwise curved differential} $\dfib$.
 ...
 On the uniform-weight scalar lane, the cohomological curvature
 element
 $\mcurv{g} = \kappa(\cA)\cdot\lambda_g \in H^{2g}(\overline{\cM}_g)$ is the Chern--Weil image of this chain-level operator
...
On the Koszul locus, the comparison is made by replacing the
curved object with this strict flat representative:
Theorem~\ref{thm:cobar-resolution-scoped} gives the ordinary
derived resolution on the flat side, while the curved side
remains coderived.
\end{proof}
...
\item \emph{Hodge bundle structure.}\;
 ...
 Pairing~\eqref{eq:obstruction-hodge} with $\psi_1^{2g-2}$ on
 $\overline{\mathcal{M}}_{g,1}$ gives the scalar free energy
 $F_g(\cA)$, whose generating function on the uniform-weight lane is
 \begin{equation}\label{eq:family-index-package}
 \sum_{g \geq 1} F_g(\cA)\,\hbar^{2g}
 \;=\;
 \kappa(\cA)\cdot(\hat{A}(i\hbar) - 1).
 \end{equation}
\end{enumerate}
...
$(\operatorname{Im}\Omega)^{-1}_{\alpha\beta}$). Part~(ii) is the
...
Part~(iv): on the uniform-weight lane the algebraic part of the
curvature is one-dimensional, so the only $\cA$-dependent scalar is
$\kappa(\cA)$ from part~(ii). The geometric part is the Hodge
bundle~$\mathbb{E}$: because every scalar propagator is the
weight-$1$ form $d\!\log E(z,w)$, no higher Hodge bundle
$R^0\pi_*\omega_\pi^{\otimes h}$ with $h \neq 1$ enters the scalar
sector. Let $\Theta_{\mathbb{E}}$ be the Chern curvature matrix of
$\mathbb{E}$ in the Arakelov metric. Chern--Weil gives
\[
...
\lambda_g .
\]
Applying this Euler-form polynomial to the geometric factor and the
linear scalar extraction to the algebraic factor yields
\eqref{eq:obstruction-hodge}. Pairing with $\psi_1^{2g-2}$ and using
Theorem~\ref{thm:mumford-formula} gives the generating series
...
\item \emph{Chain representative determines cohomology.}\;
 The cohomological obstruction class is obtained from the
 chain-level identity~\eqref{eq:chain-curvature-operator}
 by Chern--Weil on the Hodge bundle. Let
 $\Theta_{\mathbb{E}} \in
 \Omega^{1,1}(\overline{\cM}_g,\mathrm{End}(\mathbb{E}))$
 be the curvature matrix of the Hodge bundle
 $\mathbb{E} = \pi_*\omega_\pi$ in the Arakelov metric.
 The Arakelov--Faltings description of the Hodge metric identifies
 the geometric zero-mode sector of the bar curvature with this
 Hodge curvature matrix, and Chern--Weil gives
 ...
 \lambda_g .
 \end{equation}
 On the uniform-weight scalar lane, the algebraic factor is the
 one-dimensional scalar channel, so the only $\cA$-dependent
 coefficient is the linear scalar~$\kappa(\cA)$. Therefore
 \[
 \mathrm{obs}_g(\cA)
 \;=\;
 \kappa(\cA)\,
 ...
 \]
 and at genus~$1$ the same formula is unconditional for all families.
 The chain-level datum $\omega_g^{\mathrm{Ar}}$ determines
 $\lambda_g$ through the Hodge curvature matrix, but is not equal to
 it: the passage from the fiberwise $(1,1)$-form to the base class
 uses the Euler-form polynomial of the rank-$g$ bundle
 $\mathbb{E}$.
...
curve~$\Sigma_g$ is constructed as a coderivation of
$T^c(s^{-1}\bar\cA)$
...
hence acts as scalar multiplication by $\kappa(\cA)\cdot\omega_g^{\mathrm{Ar}}$
on the underlying graded vector space.
(When $\kappa \neq 0$, $\dfib$ is \emph{not} a coderivation
of the deconcatenation coproduct $\Delta$ on $T^c$: applying
$\Delta \circ \dfib^{\,2}$ and $(1 \otimes \dfib + \dfib \otimes 1)^2 \circ \Delta$
to a length-$n$ element, the cross terms
$(d \otimes d) \circ \Delta$ give a factor-$2$ discrepancy
at interior splittings. The period-corrected
differential $\Dg{g}$ with $\Dg{g}^{\,2} = 0$ is a
coderivation. The operator $\dfib^{\,2}$ is nevertheless a
well-defined degree-$2$ endomorphism of the underlying graded
vector space, and its action is scalar.)

(ii)~The geometric factor of~\eqref{eq:chain-curvature-operator} is
the Hodge bundle~$\mathbb{E} = \pi_*\omega_\pi$ equipped with its
Arakelov metric. Let $\Theta_{\mathbb{E}}$ be the corresponding
curvature matrix. The Arakelov--Faltings description of the Hodge
metric identifies the zero-mode sector of the bar curvature with
$\Theta_{\mathbb{E}}$, and Chern--Weil gives
\[
\det\!\Bigl(\frac{i}{2\pi}\Theta_{\mathbb{E}}\Bigr)
\;=\;
c_g(\mathbb{E})
...
\]
On the uniform-weight scalar lane, the algebraic factor is the
one-dimensional scalar channel, so the obstruction class is obtained
by applying the linear scalar extraction to the algebraic factor and
the Euler-form polynomial to the geometric factor:
\[
\mathrm{obs}_g(\cA)
\;=\;
\kappa(\cA)\,\lambda_g
\qquad \textup{(}UNIFORM-WEIGHT\textup{}).
\]
At genus~$1$ the same formula is unconditional for all families.
...
\begin{theorem}[Genus extension hierarchy; \ClaimStatusProvedHere]
...
Each level is strictly contained in the next, and the standard
landscape satisfies all five.
\begin{enumerate}[label=\textup{(\roman*)}]
\item \emph{Algebraic \textup{(}bar $D^2=0$ at all genera\textup{)}.}\;
 For \emph{any} chiral algebra $\cA$, the bar differential satisfies
 $D_\cA^2 = 0$ at all genera, and the universal MC element
...
\item \emph{Koszul \textup{(}chiral bar-cobar
 ...
 resolution of~$\cA$, not merely a coderived model.
...
The weight-by-weight SDR
(Cliff~\cite[Proposition~8.1]{Cliff19})
applies at each genus to produce the transferred structure
in $D^{\mathrm{co}}$; the key input is that the homotopy
transfer theorem requires only finite-dimensional weight
spaces, not Koszulness.
...
$\Sigma_g$ induced by the Hodge inner product. The algebraic part of
the scalar bar curvature contributes the coefficient $\kappa(\cA)$,
while the geometric zero-mode sector is the Hodge bundle
$\mathbb{E} = \pi_*\omega_\pi$. Let $\Theta_{\mathbb{E}}$ denote the
Chern curvature matrix of $\mathbb{E}$ in the Arakelov metric. The
genus-$g$ obstruction is the Chern--Weil Euler form of the Hodge
bundle multiplied by the scalar coefficient:
\[
\mathrm{obs}_g(\cA)
\;=\;
\kappa(\cA)\,
...
\qquad \textup{(}UNIFORM-WEIGHT\textup{}).
\]
No clutching-uniqueness step is needed: the geometric factor is
the Euler form of $\mathbb{E}$ itself
(Proposition~\ref{prop:scalar-obstruction-hodge-euler}).
At genus~$1$ the same formula is unconditional
for all families
...
$\kappa(\cA)\lambda_1$
\textup{(}g=1 only; all-weight at g=1 is unconditional\textup{)}.
The higher-genus identification does not use a uniqueness-from-clutching
argument: Proposition~\ref{prop:scalar-obstruction-hodge-euler}
identifies the scalar geometric factor directly with the
Chern--Weil Euler form $c_g(\mathbb{E}) = \lambda_g$ via the
Arakelov--Faltings theorem and GRR on the universal curve;
Theorem~\ref{thm:genus-universality} then assembles the
result. Pairing with the
...
character, while the higher obstruction classes are the Hodge
Euler-form classes selected by the same weight-$1$ zero-mode bundle.
...
\item The Eilenberg--Zilber homotopies $F_n^{(g)}$ acquire curvature
 ...
 + \kappa(\cA) \cdot \omega_g \cdot (\text{lower-degree terms})$.
...
\begin{remark}[Separating-node factorisation of the Igusa cusp form]
\label{rem:hgf-DNA-siegel-sepnode}
The genus-$2$ cocycle $c_2$ above is integrated against the Weil--Petersson measure on $\mathcal{M}_2$, and its modular content is controlled by the Igusa cusp form $\Phi_{10}(Z)$ of weight~$10$ on $\mathbb{H}_2$. At a \emph{separating} node, where $\Sigma_2$ degenerates to $\Sigma_1 \cup \Sigma_1$ and the period matrix degenerates as $Z \to \mathrm{diag}(\tau, \rho)$, the cusp form factorises explicitly:
\[
\frac{1}{\Phi_{10}(\mathrm{diag}(\tau, \rho))}
\;=\;
\frac{1}{\eta^{24}(\tau)} \cdot \frac{1}{\phi_{10,1}(\rho, 0)}
\;+\;
\text{Jacobi-form corrections},
\]
with $\eta^{24}$ the genus-$1$ modular discriminant and $\phi_{10,1}$ the Jacobi cusp form of weight $10$ and index $1$. This is the bi-based Ran-space datum $\mathrm{Ran}(E^{\mathrm{nod,sm}}_{24}) \rightrightarrows A_2$-bar at genus~$2$: the separating-degeneration term in the cocycle condition $d^{(2)} c_2 = 0$ (third bullet above) inherits a concrete modular factorisation from the Igusa form's behaviour at the boundary $\mathcal{M}_{1,1} \times \mathcal{M}_{1,1} \subset \partial \overline{\mathcal{M}}_2$. The averaging map $\mathrm{av} = \mathrm{Torelli}_{K3} \circ \mathrm{KugaSatake} \circ j_{\mathrm{Kodaira}}$ lifts this boundary factorisation to the K3 side, where it becomes the product decomposition of $1/\Phi_{10}$ over $K3$-elliptic fibrations with $24$ nodal Kodaira fibres. See Part~\ref{part:physics-bridges} for the physics/K3 side.
\end{remark}
...
The genus $g$ organizes corrections: at genus~$0$ the bar differential satisfies~$d^{(0)2} = 0$ exactly (the Arnold relation ensures nilpotency on the curve; on the formal disk this recovers the classical statement), genus~$1$ produces central extensions, and genus $g \geq 2$ introduces modular structures.
...
\begin{remark}[Siegel classification of the weight-five obstruction]
\label{rem:hgf-DNA-H2-classification}
The cohomological locus where the genus-$2$ bar differential's $\Delta_5$
obstruction lives admits a sharp classification: on the Koszul locus of the
chiral family, the second graph cohomology of $\overline{\mathcal{M}}_{2}$
relevant to the bar-curvature class decomposes as
\[
H^{2}\bigl(\overline{\mathcal{M}}_2; \mathcal{O}(\kappa)\bigr)_{\mathrm{bar}}
\;\cong\;
\mathbb{C} \cdot \Delta_5,
\]
a one-dimensional space spanned by the weight-$5$ class $\Delta_5$. The
Igusa cusp form $\Phi_{10}$ is a scalar invariant for this class in the
sense that under the Torelli embedding $\mathcal{M}_2 \hookrightarrow
\mathcal{A}_2$, the pullback $\Delta_5$ lifts to a section of a line
bundle on $\mathcal{A}_2 = \mathbb{H}_2 / Sp_4(\mathbb{Z})$ with
transformation law $\Phi_{10}$-twisted under the Siegel modular group.
This is the genus-$2$ analogue of the genus-$1$ statement that the
elliptic bar curvature is controlled by $\eta^{24}$; at genus~$2$ the
weight doubles from $12$ to a $2+3$-type decomposition (weight
$5 = 2 + 3$), matching the Jacobi-form structure of
Remark~\ref{rem:hgf-DNA-siegel-sepnode}. See Part~\ref{part:seven-faces}
...
\begin{remark}[Weight-five obstruction as one-loop SUGRA output]
\label{rem:hgf-DNA-sugra-delta5}
The weight-$5$ class $\Delta_5$ controlling the genus-$2$ bar curvature
(Remark~\ref{rem:hgf-DNA-H2-classification}) arises as a one-loop-forced
output on the physics side: twisted $11$D supergravity compactified on
$K3 \times T^2$ with $24$ M$5$-branes wrapped on $I_1$ Kodaira fibres
produces, at one loop, a modular integrand whose coefficient is exactly
$\Delta_5$. The weight decomposes as $5 = 2 + 3$: weight $2$ from the
$T^2$ factor (elliptic $\tau$-direction) and weight $3$ from the $K3$
factor (the anti-symmetric direction transverse to the nodal fibre).
Four duality frames assemble this output:
\begin{enumerate}[label=\textup{(\roman*)}]
...
\end{enumerate}
Each frame exhibits $\Delta_5$ as the obstruction to flat holomorphic
factorisation across the separating node. This is the physics manifestation
of the bar-curvature weight $5$ at genus~$2$; see
Part~\ref{part:physics-bridges} for the detailed duality-frame analysis.
\end{remark}
...
\label{rem:hgf-DNA-humbert-mukai-lusztig}
...
chiral deformation. The negative sign is what forces the
weight-$5$ curvature class $\Delta_5$ of
Remark~\ref{rem:hgf-DNA-H2-classification} to appear rather
than cancel. See Part~\ref{part:standard-landscape} for the
B-family definition and verification.
\end{remark}
...
algebra~$\cA$ is the genus-$g$, degree-$0$ projection of the
scalar level of the shadow obstruction tower:
...
\textup{(}UNIFORM-WEIGHT\textup{)}; at genus~$1$ the same scalar formula is
\textup{(g=1 only; all-weight at g=1 is unconditional)}.
...
(We use the closed-string convention $\hbar^{2g-2}$; the Feynman-diagram convention in Remark~\ref{sec:feynman_genus_g} uses $\hbar^{g-1}$, corresponding to $\hbar_{\text{there}} = \hbar_{\text{here}}^2$.)
...
The genus-$g$ correction form on the fiber can be written schematically as
\[\omega_g^{\mathrm{Ar}} \sim \int_{\gamma \in H_1(\Sigma_g)} \eta \wedge \bar{\eta}\]
...
\qquad \textup{(}UNIFORM-WEIGHT\textup{)} \end{equation}
where $\lambda_g = c_g(\mathbb{E})$ is the top Chern class of the Hodge bundle.

Explicitly:
\begin{itemize}
\item $g=1$: $\text{obs}_1 = \kappa \cdot \lambda_1$ \textup{(g=1 only; all-weight at g=1 is unconditional)} where $\lambda_1 = c_1(\mathbb{E}) \in H^2(\overline{\mathcal{M}}_{1,1})$
\item $g=2$: $\text{obs}_2 = \kappa \cdot \lambda_2 = \kappa \cdot c_2(\mathbb{E})$ \item $g \geq 3$: $\text{obs}_g = \kappa \cdot \lambda_g$ \end{itemize}
\end{theorem}
...
\begin{align}
\dfib^{\,2}(a_m) &= \dfib\!\left(\sum_k a_k \otimes a_{m-k} \otimes \omega_g^{\mathrm{Ar}}\right) \\
&= \sum_{k_1, k_2} [a_{k_1}, a_{k_2}] \otimes a_{m-k_1-k_2}
\otimes \omega_g^{\mathrm{Ar}}
\end{align}
...
fiberwise Arakelov curvature form $\omega_g^{\mathrm{Ar}}$; its
pushforward along the universal curve produces the Hodge class
$c_1(\mathbb{E})$ via the Quillen determinant anomaly
formula~\cite{Quillen85}. The fiberwise result is:
\begin{equation}
\dfib^{\,2}(a_m) = \kappa \cdot a_m \otimes \omega_g^{\mathrm{Ar}}
\end{equation}
...
At genus~$g \geq 2$: $\lambda_g = c_g(\mathbb{E})$ has degree~$g < 3g{-}3 = \dim_{\mathbb{C}}\overline{\mathcal{M}}_g$, so the obstruction class $\mathrm{obs}_g = \kappa \cdot \lambda_g$ \textup{(}UNIFORM-WEIGHT\textup{)} is a non-trivial class in $H^{2g}(\overline{\mathcal{M}}_g)$ but is \emph{not} a top-degree class. Its numerical evaluation requires pairing with a complementary tautological class of degree~$2g{-}3$; the relevant intersection numbers are computable from the Faber conjectures (proved by Faber--Pandharipande) involving Bernoulli numbers.
...
\text{dim}(\mathfrak{g})}{2h^\vee} \cdot \lambda_g \qquad \textup{(}UNIFORM-WEIGHT\textup{})
\end{equation}
...
\begin{align}
\mathfrak{g} = \mathfrak{sl}_2: \quad \text{obs}_g &= \frac{3(k+2)}{4} \lambda_g \textup{(}UNIFORM-WEIGHT\textup{)} \\
\mathfrak{g} = \mathfrak{sl}_3: \quad \text{obs}_g &= \frac{4(k+3)}{3} \lambda_g \textup{(}UNIFORM-WEIGHT\textup{)} \\
\mathfrak{g} = E_8: \quad \text{obs}_g &= \frac{62(k+30)}{15} \lambda_g \textup{(}UNIFORM-WEIGHT\textup{})
\qquad \end{align}
...
By the general obstruction formula (Theorem~\ref{thm:obstruction-general}), the fiberwise genus-$g$
obstruction is:
\[\text{obs}_g^{\mathrm{fib}} = \omega_g^{\mathrm{Ar}} \otimes m_0\]
For $\widehat{\mathfrak{g}}_k$, the curvature element $m_0$ from Step~1 acts on each of the $\dim(\mathfrak{g})$ current generators.
Pushing this fiberwise form forward along the universal curve produces the Hodge class $\lambda_g$
...
$\sum_a (J^a, J_a) = \dim(\mathfrak{g})$, one obtains :
\begin{align}
...
the Heisenberg structure constants vanish ($f^{abc} = 0$),
and the Casimir correction is absent. The obstruction reduces to
...
= \frac{-(k+h^\vee)\dim(\mathfrak{g})}{2h^\vee} \cdot \lambda_g
\qquad = -\text{obs}_g^{\widehat{\mathfrak{g}}_k}\]
...
The obstruction is \textup{(}UNIFORM-WEIGHT\textup{)}:
\[\text{obs}_g = \frac{(k+2) \cdot 3}{2 \cdot 2} \lambda_g = \frac{3(k+2)}{4} \lambda_g\]

At genus 1 with $k=1$ \textup{(g=1 only; all-weight at g=1 is unconditional)}:
...
\begin{remark}[Scope of the Kac--Moody obstruction at higher genus]%
\label{rem:kac-moody-obs-scope}%
\index{Kac--Moody obstruction!scope at higher genus}%
\index{scalar-diagonal hypothesis!Kac--Moody instance}%
Step~2 of the proof writes the fiberwise genus-$g$ obstruction as
$\mathrm{obs}_g^{\mathrm{fib}} = \omega_g^{\mathrm{Ar}}\otimes m_0$ and
Step~3 sums over the $\dim(\fg)$ adjoint currents assuming each
contributes a copy of the scalar curvature $m_0$. This summation is
the scalar-diagonal hypothesis
\textup{(}Definition~\textup{\ref{def:scalar-diagonal-hypothesis}}%
\textup{)} applied to $\widehat{\fg}_k$: it requires the fiberwise
curvature to decompose as $\kappa(\cA)$ acting uniformly on every
weight-$p$ channel of the associated graded
$\bigoplus_p \Lambda^p H^0(\Sigma_g,\omega)\otimes\varepsilon_p$,
with $\kappa(\cA)$ independent of~$g$.

At genus~$1$ the identification is unconditional because
$H^2(\overline{\cM}_{1,1},\Q) = \Q\cdot\lambda_1$ is one-dimensional;
no scalar-diagonal hypothesis is needed.

At $g\geq 2$ with $\fg$ non-abelian, scalar-diagonality of
$V_k(\fg)$ is discharged for positive-integer levels via the
Frenkel--Kac lattice realisation: at level~$1$ the
affine vertex algebra is isomorphic to the lattice VOA $V_{Q}$ of the
root lattice~$Q\subset\fh^*$, and on that realisation the currents
reduce to rank-$r$ Heisenberg plus lattice vertex operators
$e^{\pm\alpha}$, each of which satisfies scalar-diagonality by the
rank-$1$ automaticity of Heisenberg together with the scalar
lattice pairing $\langle\alpha,\alpha\rangle$. At higher positive-integer
level the same conclusion follows from the parafermionic coset
$\widehat{\fg}_k / (\widehat{\mathfrak h})^{\oplus r}$.

At generic (non-integer) level~$k\neq -h^\vee$, scalar-diagonality
is open: the Wakimoto realisation provides a rank-$r$ Heisenberg
plus $\dim(\fg/\mathfrak h)$ copies of the $\beta\gamma$ ghost
system, and scalar-diagonality on the ghost sector is neither proved
nor disproved in the present inscription. The genus-$g$ formula
$\mathrm{obs}_g = ((k+h^\vee)\dim(\fg)/(2h^\vee))\cdot\lambda_g$ at
$g\geq 2$ is therefore conditional on scalar-diagonality of
$V_k(\fg)$ at the level in question; the unconditional genus-$1$
case and the genus-$g$ cases at positive-integer level are proved.
\end{remark}
...
The bar construction does \emph{not} produce $\lambda_g$ at $g \geq 2$
for multi-weight algebras: the free energy receives a nonvanishing
cross-channel correction
(Theorem~\textup{\ref{thm:multi-weight-genus-expansion}},
Open Problem~\textup{\ref{op:multi-generator-universality}} resolved
negatively).
\end{enumerate}
\end{theorem}
...
The $B$-cycle quasi-periodicity produces the same fiberwise
Arakelov form $\omega_g$ in both channels; its pushforward along
the universal curve gives $c_1(\mathbb{E})$ on moduli. Thus
...
identification $\Omega = \sum \lambda_g$
\textup{(all-weight, with cross-channel correction $\delta F_g^{\mathrm{cross}}$)}
fails for multi-weight
algebras: the bar construction at genus~$g \geq 2$ produces a
class that deviates from~$\lambda_g$ by a cross-channel correction
...
\begin{proof}
Theorem~\ref{thm:w3-genus1-curvature} gives
$\kappa(\mathcal{W}_3)=5c/6$, so at the minimal-model value
$c=4/5$ one gets $\kappa=2/3$.
The genus-$1$ free energy is always
$F_1=\kappa/24$ on the genus-$1$ lane, hence
$F_1=(2/3)/24=1/36$.
The preceding curvature analysis and
Remark~\ref{rem:propagator-weight-universality} show that both the
$T$-channel and the $W$-channel couple to the same weight-$1$
propagator class, namely $\lambda_1=c_1(\mathbb E)$.
Because the bar complex never introduces the higher Hodge bundles
$\mathbb E_j$ for $j\ge 2$, no additional genus-$1$ class can enter.
\end{proof}
...
$\operatorname{obs}_g = \kappa(\mathcal{A}) \cdot \lambda_g$
\textup{(}UNIFORM-WEIGHT\textup{}) satisfy the
...
$\operatorname{obs}_g = \kappa \cdot \lambda_g$ \textup{(}UNIFORM-WEIGHT\textup{)}, Theorem~\ref{thm:obstruction-nilpotent-all-genera} gives
$(\operatorname{obs}_g)^2 = \kappa^2 \cdot \lambda_g^2 = 0$
for \emph{all} $g \geq 1$, by Mumford's relation.
\end{example}

% ================================================================
% PROPOSITION: Scalar obstruction as Hodge Euler class (AP225 resolution)
% Closes the genus-universality gap by proving the passage from fiberwise
% curvature (chain-level, on each Sigma_g) to a cohomology class on M-bar_g.
% Two independent proof paths: (A) GRR on the universal curve,
% (B) Chern--Weil on the Hodge bundle in the Arakelov metric.
% ================================================================

\begin{definition}[Scalar-diagonal hypothesis;
\ClaimStatusProvedElsewhere]
\label{def:scalar-diagonal-hypothesis}
\index{scalar-diagonal hypothesis|textbf}
A modular Koszul chiral algebra $\cA$ satisfies the
\emph{scalar-diagonal hypothesis} if, on every fiber
$\Sigma_g$, the fiberwise bar curvature
$\dfib^{\,2}$ acts on the fiberwise associated graded
$\mathrm{gr}^{\bullet}\barB^{(g)}_{\mathrm{geom}}(\cA)|_{\Sigma_g}
= \bigoplus_{p=0}^{g}\Lambda^p H^0(\Sigma_g,\omega)
\otimes \varepsilon_p$ as multiplication by the same
scalar $\kappa(\cA)$ on every weight-$p$ channel
$\varepsilon_p$, with $\kappa(\cA)$ independent of~$g$.
Equivalently: the algebraic factor of the bar tower is
a single one-dimensional representation on which the
curvature endomorphism is central at every weight, and
the weight-$p$ channels $\varepsilon_p$ are mutually
orthogonal as $\kappa(\cA)$-eigenspaces.
Automatic for rank-$1$ strong-generator families
\textup{(}Heisenberg, Virasoro, every rank-$1$ free-field
family\textup{)}; a genuine hypothesis for multi-generator
uniform-weight families such as affine Kac--Moody
$V_k(\fg)$ with $\fg$ non-abelian, where the Sugawara
identification $\mathrm{av}(r(z)) + \dim(\fg)/2 = \kappa$ is
established at $g=1$ but its $g$-independence at higher
genus is the content of the hypothesis.
\end{definition}

\begin{proposition}[Scalar obstruction equals Hodge Euler class;
\ClaimStatusProvedHere]
\label{prop:scalar-obstruction-hodge-euler}
\index{obstruction class!Hodge Euler class identification|textbf}
\index{Hodge bundle!Euler class from bar curvature}
\index{Chern--Weil!bar curvature to Hodge class}
\index{Grothendieck--Riemann--Roch!scalar obstruction}
Let $\cA$ be a modular Koszul chiral algebra on the Koszul locus
whose strong generators all have the same conformal weight
\textup{(}the uniform-weight hypothesis\textup{)} and which
satisfies the scalar-diagonal hypothesis
\textup{(}Definition~\textup{\ref{def:scalar-diagonal-hypothesis}}%
\textup{)}.
Consider the family of genus-$g$ bar complexes
$\barB^{(g)}(\cA)|_{[\Sigma_g]}$ parametrized by
$[\Sigma_g] \in \overline{\cM}_g$, and let
$\mathrm{obs}_g(\cA) \in H^{2g}(\overline{\cM}_g, \mathbb{Q})$
be the scalar obstruction class extracted from the fiberwise
curvature via the modular bar construction.
Then for all $g \geq 1$:
\begin{equation}\label{eq:scalar-obs-hodge-euler}
\mathrm{obs}_g(\cA)
\;=\;
...
\end{equation}
where $\mathbb{E} = R^0\pi_*\omega_\pi$ is the Hodge bundle over
$\overline{\cM}_g$ and $\lambda_g = c_g(\mathbb{E})$ its top
Chern class.
\end{proposition}

\begin{remark}[Honest reading: what Step~1 does and does not prove]
\label{rem:scalar-diagonal-honest}
\index{scalar-diagonal hypothesis!honest scope}
The $K$-theoretic construction of Step~1 below
\textup{(}Lemma~\textup{\ref{lem:k-theoretic-globalization-bar}}%
\textup{)} \emph{defines} the scalar obstruction class
by projection of the virtual $K$-class to the one-dimensional
weight-$p$ channel $\varepsilon_p \subset \mathrm{gr}^p
\barB^{(g)}$, and then reads off the degree-$2g$ component
of its Chern character. Under this definition, the
identification
$\mathrm{obs}_g(\cA)=\kappa(\cA)\cdot\lambda_g$ reduces to the
\emph{cohomological} identity
$\mathrm{ch}_g(\lambda_{-1}(\mathbb{E}))=(-1)^g\,c_g(\mathbb{E})$,
which is unconditional. The \emph{substantive} content of
the proposition is that this \emph{projected} class
coincides with the physical bar-curvature obstruction
extracted from the fiberwise chain-level curvature
$\dfib^{\,2}$ on the nose; equivalently, that the
non-scalar weight channels contribute zero to the base
cohomology class at degree~$2g$. The scalar-diagonal
hypothesis is precisely the statement that this non-scalar
contribution vanishes weight-by-weight on the fiberwise
associated graded; it is the hypothesis required for the
splitting-principle / BGS chain of Step~3 to produce
$\kappa(\cA)\cdot\lambda_g$ rather than a higher polynomial
in $\kappa(\cA)$.
\end{remark}

\begin{proof}
The proof proceeds in three steps: identify the relevant
$K$-theory class, apply Grothendieck--Riemann--Roch, and extract
the degree-$g$ component.

\textbf{Step 1: the virtual bundle on moduli.}
On the uniform-weight scalar lane, the bar complex over a fixed
curve~$\Sigma_g$ has a one-dimensional algebraic factor
(the scalar channel) tensored with a geometric factor given by the
de~Rham complex of~$\Sigma_g$ with coefficients in
$\omega_{\Sigma_g}$ (the canonical bundle), because every
propagator is the standard weight-$1$ form $d\!\log E(z,w)$
\textup{(}Remark~\textup{\ref{rem:propagator-weight-universality}}\textup{)}.
As the curve~$\Sigma_g$ varies over $\overline{\cM}_g$, the
zero-mode sector of this geometric factor is the direct image
$R^0\pi_*\omega_\pi = \mathbb{E}$, the Hodge bundle of rank~$g$
over~$\overline{\cM}_g$. The scalar virtual class of the bar
family is therefore
$[\barB^{(g)}_{\mathrm{scalar}}(\cA)]^{\mathrm{vir}}
= \kappa(\cA) \cdot [\lambda_{-1}(\mathbb{E})]$
in $K^0(\overline{\cM}_g) \otimes \mathbb{Q}$, where the scalar
coefficient $\kappa(\cA)$ is the self-contraction trace from
Proposition~\ref{prop:genus-g-curvature-package}\textup{(ii)} and
$\lambda_{-1}(\mathbb{E}) = \sum_{p=0}^{g} (-1)^p [\Lambda^p \mathbb{E}]$
is the total lambda class.

We give the $K$-theoretic globalization explicitly; it is the
cohomology-to-$K$-theory step that is silently invoked
elsewhere in the operadic literature and that deserves
inscription here.

\emph{Step~1a \textup{(}fiberwise filtration\textup{)}.} Fix
$[\Sigma_g] \in \overline{\cM}_g$. The restriction
$\barB^{(g)}_{\mathrm{geom}}(\cA)|_{\Sigma_g}$ carries the
de~Rham filtration by Hodge weight, and the associated graded
is the fiberwise Koszul complex
\[
\mathrm{gr}^{\bullet}\,
\barB^{(g)}_{\mathrm{geom}}(\cA)\big|_{\Sigma_g}
\;\cong\;
\bigoplus_{p=0}^{g}
\Lambda^p H^0(\Sigma_g,\omega_{\Sigma_g})
\otimes \varepsilon_p,
\]
where $\varepsilon_p$ is the one-dimensional scalar channel at
weight~$p$ \textup{(}uniform-weight hypothesis: the algebraic
factor contributes only the scalar $\kappa(\cA)$ and a sign
$(-1)^p$ from the bar suspension\textup{)}. This is the
fiberwise instance of the Koszul resolution for
$\omega_{\Sigma_g}$-valued forms; the $\Lambda^p$ comes from the
propagator being a top form in each factor.

\emph{Step~1b \textup{(}relative Euler characteristic in
$K$-theory\textup{)}.} The filtration varies holomorphically
with~$[\Sigma_g]$: the sheaf
$R^0\pi_*\omega_\pi = \mathbb{E}$ is the rank-$g$ Hodge bundle,
and the fiberwise $\Lambda^p H^0(\Sigma_g,\omega_{\Sigma_g})$
globalizes to the exterior power
$\Lambda^p \mathbb{E}$ on~$\overline{\cM}_g$. Taking the
alternating sum of graded pieces in $K^0(\overline{\cM}_g)$
\textup{(}which is well-defined because the filtration is
bounded by the rank~$g$ and the extensions split rationally in
$K^0\otimes\Q$\textup{)} gives
\[
[\barB^{(g)}_{\mathrm{geom}}(\cA)]
\;=\;
\sum_{p=0}^{g} (-1)^p\, [\Lambda^p \mathbb{E}]
\;=\;
\lambda_{-1}(\mathbb{E})
\;\in\; K^0(\overline{\cM}_g)\otimes\Q.
\]
Multiplication by the scalar $\kappa(\cA)$ from the algebraic
channel yields the virtual class
$[\barB^{(g)}_{\mathrm{scalar}}(\cA)]^{\mathrm{vir}}
= \kappa(\cA)\cdot \lambda_{-1}(\mathbb{E})$.

\emph{Step~1c \textup{(}scalar-lane upgrade via top Chern
character\textup{)}.} Apply the Chern character and read off
the degree-$g$ component. By the splitting principle,
if $\mathbb{E}$ formally splits as
$\mathbb{E} = L_1 \oplus \cdots \oplus L_g$ with
$x_i = c_1(L_i)$, then
\[
\mathrm{ch}\bigl(\lambda_{-1}(\mathbb{E})\bigr)
\;=\;
\prod_{i=1}^{g}\bigl(1 - e^{x_i}\bigr)
\;=\;
\prod_{i=1}^{g}\bigl(-x_i + O(x_i^2)\bigr)
\;=\;
(-1)^g\, c_g(\mathbb{E}) + \bigl(\text{higher-degree corrections
to the top $(-1)^g c_g(\mathbb{E})$ contribution that vanish
after projection to degree $2g$}\bigr).
\]
The expansion $\prod_{i=1}^{g}(1 - e^{x_i})$ begins in degree
$2g$ (each factor starts at degree~$2$, and their product has
minimal total degree~$2g$); corrections arise from the
$O(x_i^2)$ remainders inside each factor and contribute in
degree strictly greater than~$2g$. Projection of
$\mathrm{ch}(\lambda_{-1}(\mathbb{E}))$ to degree~$2g$
extracts exactly $(-1)^g\,c_g(\mathbb{E})
= (-1)^g\,\lambda_g$; the sign is absorbed by the bar
suspension convention $|s^{-1}v|=|v|-1$. Thus the virtual class
contributes $\kappa(\cA)\cdot\lambda_g$ at the top Chern
degree, as required. We record this as a named identity for
reuse.

\begin{lemma}[$K$-theoretic globalization of the scalar bar class;
\ClaimStatusProvedHere]
\label{lem:k-theoretic-globalization-bar}
\index{K-theory!globalization of scalar bar class}
\index{lambda class!of Hodge bundle}
The scalar virtual class of the bar family satisfies
\[
\bigl[\barB^{(g)}_{\mathrm{scalar}}(\cA)\bigr]^{\mathrm{vir}}
\;=\;
\kappa(\cA)\cdot \lambda_{-1}(\mathbb{E})
\;\in\; K^0(\overline{\cM}_g)\otimes\Q,
\qquad
\mathrm{ch}_g\bigl(\lambda_{-1}(\mathbb{E})\bigr)
\;=\;
(-1)^g\,\lambda_g.
\]
In particular, passing to rational cohomology and projecting
to degree~$2g$ gives
$\mathrm{obs}_g(\cA) = \kappa(\cA)\cdot \lambda_g$ at the
$K$-theoretic level, independently of any fiber-metric data.
\qed
\end{lemma}

\textbf{Step 2: GRR on the universal curve.}
Let $\pi\colon \overline{\cC}_g \to \overline{\cM}_g$ be the
universal curve. Grothendieck--Riemann--Roch gives
\begin{equation}\label{eq:grr-hodge}
\mathrm{ch}(R\pi_*\omega_\pi)
\;=\;
\pi_*\!\bigl(\mathrm{ch}(\omega_\pi) \cdot
\mathrm{Td}(T_\pi)\bigr).
\end{equation}
The degree-$g$ component of $\mathrm{ch}(\mathbb{E})$
is $\mathrm{ch}_g(\mathbb{E})$, and the relation between the
Chern character and the top Chern class of a rank-$g$ bundle gives
\[
c_g(\mathbb{E})
\;=\;
\lambda_g
\;\in\;
\mathrm{CH}^g(\overline{\cM}_g).
\]
This is Mumford's computation~\cite{Mumford83}: the degree-$1$
component gives $\mathrm{ch}_1(\mathbb{E}) = c_1(\mathbb{E})
= \lambda_1$, and the higher-degree Chern classes $\lambda_j$
for $2 \leq j \leq g$ are determined by the full pushforward via the
Newton identities relating Chern characters to Chern classes.

\textbf{Step 3: Determinant of cohomology and the
fiber-to-base bridge.}
%
% AP130 fiber-base discipline: the fiberwise curvature
% d_fib^2 = kappa * omega_Ar lives on FIBERS Sigma_g.
% The obstruction class obs_g lives on the BASE M-bar_g.
% The passage from fiber to base requires the determinant-of-
% cohomology construction + BGS anomaly formula.
%
The fiberwise curvature $\dfib^{\,2} = \kappa(\cA) \cdot
\omega_g^{\mathrm{Ar}} \cdot \mathrm{id}$
\textup{(}Proposition~\textup{\ref{prop:genus-g-curvature-package}(ii))}
is a form on each fiber~$\Sigma_g$. The obstruction class
$\mathrm{obs}_g$ lives on the base~$\overline{\cM}_g$.
Bridging these two spaces requires the
\emph{determinant of cohomology} construction, which we
now give in five sub-steps.

\textbf{Step 3a: The Hodge bundle and its Arakelov metric.}
The Hodge bundle
$\mathbb{E} = R^0\pi_*\omega_\pi$
is a rank-$g$ holomorphic vector bundle on
$\overline{\cM}_g$. Its fiber over a point
$[\Sigma_g] \in \overline{\cM}_g$ is the $g$-dimensional
space $H^0(\Sigma_g, \Omega^1_{\Sigma_g})$ of holomorphic
differentials on~$\Sigma_g$.
This bundle carries a canonical Hermitian metric, the
\emph{Arakelov metric}~\cite{Ara74,Fal84,Soule92}:
Arakelov~\cite{Ara74} constructed the admissible
metric on the canonical sheaf of an arithmetic surface;
Faltings~\cite[\S 2]{Fal84} extended this to the
$L^2$-inner product on holomorphic differentials
\eqref{eq:arakelov-l2-inner-product} and proved it
descends to a smooth Hermitian metric on the Hodge
bundle over $\overline{\cM}_g$; Soul\'e~\cite[Ch.~III]{Soule92}
assembled these ingredients with the Knudsen--Mumford
determinant of cohomology and Quillen's analytic
torsion into the arithmetic Riemann--Roch theorem for
families of curves.
On each fiber $H^0(\Sigma_g, \Omega^1)$ the inner product is
\begin{equation}\label{eq:arakelov-l2-inner-product}
\langle \omega, \eta \rangle_{\mathrm{Ar}}
\;=\;
\frac{i}{2}\int_{\Sigma_g}
\omega \wedge \overline{\eta}.
\end{equation}
In terms of a symplectic basis
$\omega_1, \ldots, \omega_g$ of holomorphic differentials
normalized by $\oint_{A_\alpha}\omega_\beta = \delta_{\alpha\beta}$,
the Gram matrix of~\eqref{eq:arakelov-l2-inner-product} is
$(\operatorname{Im}\Omega)_{\alpha\beta}$, the imaginary
part of the period matrix.

\textbf{Step 3b: The determinant line bundle and the
Quillen metric.}
The \emph{determinant line bundle} is the top exterior power
\[
\lambda
\;=\;
\det(\mathbb{E})
...
\Lambda^g(\mathbb{E}),
\]
a holomorphic line bundle on $\overline{\cM}_g$ whose
fiber at $[\Sigma_g]$ is
$\Lambda^g H^0(\Sigma_g, \Omega^1)$.
More generally, for the family of Dolbeault operators
$\bar\partial_{\Sigma_g}\colon
\Omega^{0,0}(\Sigma_g, \omega) \to
\Omega^{0,1}(\Sigma_g, \omega)$
parametrized by
$[\Sigma_g] \in \overline{\cM}_g$,
the \emph{determinant of cohomology}
in the sense of Knudsen--Mumford is
\[
\det R\Gamma(\Sigma_g, \omega)
\;=\;
\Lambda^{\mathrm{max}}H^0(\Sigma_g,\omega)
\;\otimes\;
(\Lambda^{\mathrm{max}}H^1(\Sigma_g,\omega))^{-1}.
\]
By Serre duality
$H^1(\Sigma_g,\omega) \cong H^0(\Sigma_g, \cO)^* \cong \mathbb{C}$,
so $\det R\Gamma(\Sigma_g,\omega) \cong
\Lambda^g H^0(\Sigma_g,\omega) = \lambda|_{[\Sigma_g]}$.
The Arakelov metric on $\mathbb{E}$ induces a metric on
$\lambda = \det(\mathbb{E})$. After correcting by the
analytic torsion (the regularized determinant of the
Laplacian $\bar\partial^*\bar\partial$ acting on
$\omega$-valued functions), one obtains the
\emph{Quillen metric} $h_Q$ on
$\lambda$~\cite{Quillen85}:
\[
h_Q
\;=\;
h_{\det(\mathrm{Ar})}
\cdot
(\det\nolimits' \Delta_{\bar\partial})^{-1},
\]
where $\det'$ denotes the zeta-regularized determinant
and $\Delta_{\bar\partial}$ the Dolbeault Laplacian.

\textbf{Step 3c: The Bismut--Gillet--Soul\'e anomaly formula.}
The curvature of the Chern connection of
$(\lambda, h_Q)$ is computed by the
\emph{Bismut--Gillet--Soul\'e anomaly
formula}~\cite{BGS88a}
\textup{(}see also Bismut--Freed~\cite{BismutFreed86}
for the determinant-line-bundle
construction\textup{)}:
\begin{equation}\label{eq:bgs-anomaly}
c_1(\lambda, h_Q)
\;=\;
\int_{\Sigma_g/\overline{\cM}_g}
\mathrm{Td}(T_\pi, h_\pi) \cdot
\mathrm{ch}(\omega_\pi, h_{\mathrm{Ar}})
\;\in\;
H^2(\overline{\cM}_g, \mathbb{R}).
\end{equation}
The right-hand side is a fiber integral: $T_\pi$ is the
relative tangent bundle of the universal curve
$\pi\colon \overline{\cC}_g \to \overline{\cM}_g$
with its induced metric~$h_\pi$, and $\mathrm{Td}$ and
$\mathrm{ch}$ denote the Chern--Weil representatives
of the Todd class and Chern character with respect to the
indicated Hermitian metrics.
This is the \emph{differential-geometric refinement} of the
Grothendieck--Riemann--Roch formula~\eqref{eq:grr-hodge}:
where GRR computes rational Chern classes in
$\mathrm{CH}^*(\overline{\cM}_g)_{\mathbb{Q}}$, the BGS
formula computes their de~Rham representatives in terms of
explicit curvature data on the fibers.

\textbf{Step 3d: From the Arakelov curvature to
$c_g(\mathbb{E})$.}
The curvature of the Chern connection of the full rank-$g$
Hodge bundle $(\mathbb{E}, h_{\mathrm{Ar}})$ is the
$\mathrm{End}(\mathbb{E})$-valued $(1,1)$-form
$\Theta_{\mathbb{E}} \in
\Omega^{1,1}(\overline{\cM}_g, \mathrm{End}(\mathbb{E}))$.
The Arakelov--Faltings theorem~\cite{Fal84} identifies
this curvature in terms of the fiberwise Arakelov form:
\begin{equation}\label{eq:arakelov-curvature-identification}
\Theta_{\mathbb{E}}|_{\mathrm{fiber}}
\;=\;
\omega_g^{\mathrm{Ar}}
\;=\;
\frac{i}{2}\sum_{\alpha,\beta=1}^{g}
...
\omega_\alpha \wedge \overline{\omega}_\beta.
\end{equation}
Here $(\operatorname{Im}\Omega)^{-1}_{\alpha\beta}$ is the
$(\alpha,\beta)$-entry of the inverse of the $g \times g$
matrix $\operatorname{Im}\Omega$; at $g \geq 2$ this is
a genuine matrix inverse, not a scalar reciprocal
\textup{(}cf.\ the $g = 1$ collapse
$(\operatorname{Im}\Omega)^{-1} = 1/\operatorname{Im}\tau$%
\textup{)}.
The Chern--Weil homomorphism applied to the full
curvature tensor $\Theta_{\mathbb{E}}$ \textup{(}as a
form on the base~$\overline{\cM}_g$, assembled from
the fiber integrals via the BGS
formula~\eqref{eq:bgs-anomaly}\textup{)} gives
the Chern classes of~$\mathbb{E}$. In particular, the
top Chern class is
\begin{equation}\label{eq:chern-weil-lambda-g}
c_g(\mathbb{E})
\;=\;
\det\!\Bigl(\frac{i}{2\pi}\Theta_{\mathbb{E}}\Bigr)
\;=\;
\lambda_g
\;\in\;
H^{2g}(\overline{\cM}_g, \mathbb{Q}).
\end{equation}

% HEAL 2026-04-17: Step 3d's prior splitting-principle argument
% conflated kappa^g * prod(x_i) (a single monomial of degree g
% in kappa) with a polynomial in kappa possessing a linear-in-kappa
% component to project onto. The correct route is Step 1c's
% K-theoretic class kappa * lambda_{-1}(E), which is linear in
% kappa by construction; Chern-character top-degree extraction
% then yields kappa * c_g(E) = kappa * lambda_g. Step 3's
% BGS/Arakelov argument provides a differential-geometric
% representative for the class identified in Step 1; it
% CORROBORATES rather than independently derives the linear
% scaling. This reorganization does not change the Theorem D
% conclusion at g in {1, 2} but makes the g >= 3 dependence on
% Faber's lambda_g-conjecture (numerical-equivalence kernel
% N^g = 0 on M-bar_g) explicit.
%
\emph{Step~3 is Chern--Weil corroboration, not an independent
proof path.} The linearity of $\mathrm{obs}_g(\cA)$
in~$\kappa(\cA)$ is established in Step~1c: the virtual class
$[\barB^{(g)}_{\mathrm{scalar}}(\cA)]^{\mathrm{vir}}
= \kappa(\cA)\cdot \lambda_{-1}(\mathbb{E})$
\textup{(}Lemma~\ref{lem:k-theoretic-globalization-bar}\textup{)}
is \emph{linear} in~$\kappa(\cA)$ by construction, as the
prefactor of a fixed $K$-theory class. Applying the Chern
character and projecting to degree~$2g$ gives
\[
\mathrm{obs}_g(\cA)
\;=\;
\mathrm{ch}_g\bigl(\kappa(\cA)\cdot\lambda_{-1}(\mathbb{E})\bigr)
\;=\;
\kappa(\cA)\cdot\mathrm{ch}_g\bigl(\lambda_{-1}(\mathbb{E})\bigr)
\;=\;
\kappa(\cA)\cdot\lambda_g,
\]
where the last equality uses
$\mathrm{ch}_g(\lambda_{-1}(\mathbb{E})) = (-1)^g\,\lambda_g$
\textup{(}with the sign absorbed by the bar suspension
convention\textup{)}. This is the load-bearing derivation:
the $K$-theoretic class carries the linear-in-$\kappa$
scaling, and the Chern character is a ring map that commutes
with scalar multiplication, so no "projection onto a
linear-in-$\kappa$ component" of any higher polynomial is
needed.

Step~3's differential-geometric machinery (Arakelov--Faltings
curvature identification, Quillen metric, BGS anomaly formula)
provides an explicit \emph{Chern--Weil representative} for the
class $\kappa(\cA)\cdot c_g(\mathbb{E})$ already identified in
Step~1c. Concretely, the splitting principle applied to the
Hodge bundle $\mathbb{E}$ on the flag bundle
$f\colon F(\mathbb{E}) \to \overline{\cM}_g$ gives
$f^*\mathbb{E} = L_1 \oplus \cdots \oplus L_g$ with
$f^* c_g(\mathbb{E}) = x_1\cdots x_g$ where $x_i = c_1(L_i)$
\textup{(}$f^*$ injective on rational cohomology\textup{)}.
The BGS anomaly formula~\eqref{eq:bgs-anomaly}, applied
line-bundle-by-line-bundle on $F(\mathbb{E})$, yields
Chern--Weil representatives $x_i^{(\mathrm{Ar})}$ for each
$x_i$ in terms of the fiberwise Arakelov curvature. Assembling
these representatives via the splitting principle gives a de
Rham representative for $c_g(\mathbb{E}) = \lambda_g$, and
scalar multiplication by $\kappa(\cA)$ on the ambient
$K$-theory class (independently of the splitting) yields a de
Rham representative for $\kappa(\cA)\cdot\lambda_g$. This is a
differential-geometric \emph{presentation} of the class
established in Step~1c, not an independent derivation.

\emph{Scope qualifier (Faber's $\lambda_g$-conjecture).}
At $g \in \{1, 2\}$, the identification
$\mathrm{obs}_g(\cA) = \kappa(\cA)\cdot\lambda_g$ is
unconditional on the nose: at $g=1$, $H^2(\overline{\cM}_{1,1})
= \Q\cdot\lambda_1$ is one-dimensional and the scalar channel
forces the coefficient to be~$\kappa(\cA)$; at $g=2$, the
tautological ring $R^*(\overline{\cM}_2)$ is known completely
(Mumford~\cite{Mumford83}) and the degree-$4$ piece is spanned
by~$\lambda_2$, so the $K$-theoretic identification of
Step~1c and Mumford's $c_g(\mathbb{E}) = \lambda_g$ suffice.
At $g \geq 3$, the identification continues to hold in the
tautological ring $R^{2g}(\overline{\cM}_g)$ \emph{modulo}
classes numerically equivalent to zero. Faber's
$\lambda_g$-conjecture~\cite{FP00} (proved for
$\overline{\cM}_g$ in the socle degree~$g-2$ but still open
in higher Chow degree for $g \geq 3$) predicts that the
kernel~$\mathcal{N}^{g}$ of the numerical-equivalence map on
$R^{2g}(\overline{\cM}_g)$ vanishes, which would upgrade the
identification to an equality on the nose. Until that
conjecture is settled, the $g \geq 3$ statement is
\emph{unconditional at the level of numerical classes} and
conditional on Faber's conjecture as a statement of
cohomological equality.

\textbf{Step 3e: The bridge from fiber curvature to
base class.}
The chain of identifications is now complete. The key point
is that each link operates at a definite geometric level:
\begin{enumerate}[label=\textup{(\roman*)}]
\item The bar complex on a fixed fiber~$\Sigma_g$ computes a
 fiberwise curvature
 $\dfib^{\,2} = \kappa(\cA) \cdot \omega_g^{\mathrm{Ar}}
 \cdot \mathrm{id}$
 \textup{(}a form on~$\Sigma_g$, from the quantum Arnold
 relations\textup{)}.
\item The Arakelov metric on $\mathbb{E}$, restricted to
 each fiber, has curvature
 $\Theta_{\mathbb{E}}|_{\mathrm{fiber}}
 = \omega_g^{\mathrm{Ar}}$
 \textup{(}the Arakelov--Faltings theorem,
 \eqref{eq:arakelov-curvature-identification}\textup{)}.
 This identifies the bar curvature with
 $\kappa(\cA)$ copies of the Hodge-bundle curvature
 restricted to the fiber.
\item The BGS anomaly formula~\eqref{eq:bgs-anomaly}
 assembles the fiberwise curvature data into
 Chern classes on the base~$\overline{\cM}_g$ via
 fiber integration. Since the bar curvature is
 $\kappa(\cA)$ times the Hodge-bundle curvature,
 this produces $\kappa(\cA) \cdot c_j(\mathbb{E})$
 at each Chern degree~$j$.
\item At degree~$g$ \textup{(}the top Chern class\textup{)},
 $c_g(\mathbb{E}) = \lambda_g$ by
 \eqref{eq:chern-weil-lambda-g} and Mumford's GRR
 computation~\cite{Mumford83} \textup{(}Step~2\textup{)}.
\end{enumerate}
%
The passage \textup{(i)} $\to$ \textup{(ii)} is a fiberwise
identification \textup{(}both sides are forms on~$\Sigma_g$%
\textup{)}.
The passage \textup{(ii)} $\to$ \textup{(iii)} is the
content of the BGS theorem: it converts fiberwise
curvature data into Chern classes on the total base space
$\overline{\cM}_g$ via fiber integration in the
determinant-of-cohomology framework.
The passage \textup{(iii)} $\to$ \textup{(iv)} is
Mumford's classical computation.

\textbf{Combining Steps 1--3:} the scalar obstruction class
$\mathrm{obs}_g(\cA)$ is obtained from the bar family by
extracting the scalar algebraic coefficient $\kappa(\cA)$
\textup{(}Step~1\textup{)} and applying the
degree-$g$ Chern polynomial to the geometric
factor~$\mathbb{E}$. Step~2 computes
$c_g(\mathbb{E}) = \lambda_g$ via GRR.
Step~3 provides the determinant-of-cohomology bridge:
the chain-level Arakelov curvature
from the quantum Arnold relations
\textup{(}Theorem~\textup{\ref{thm:quantum-arnold-relations})}
is identified with the curvature of the Hodge bundle via
the Arakelov--Faltings theorem, and the
Bismut--Gillet--Soul\'e anomaly formula converts this
fiberwise curvature into the Chern--Weil representative
of $\lambda_g$ on the base~$\overline{\cM}_g$. Therefore
\[
\mathrm{obs}_g(\cA)
\;=\;
\kappa(\cA) \cdot \lambda_g
...
\text{for all } g \geq 1.
\]
The genus-$1$ case is unconditional for all families because
$H^2(\overline{\cM}_{1,1}) \cong \mathbb{Q}\cdot\lambda_1$
and the scalar channel is one-dimensional regardless of the
number of generators.
\end{proof}

\begin{remark}[Independence from clutching uniqueness]
\label{rem:no-clutching-needed}
\index{clutching!not needed for genus universality}
\index{determinant of cohomology!role in genus universality}
\index{Bismut--Gillet--Soul\'e!anomaly formula}
Proposition~\textup{\ref{prop:scalar-obstruction-hodge-euler}}
does \emph{not} invoke a clutching-uniqueness argument
(showing that $\lambda_g$ is the unique tautological class with
certain boundary restrictions). Instead, the identification is
direct, via the determinant-of-cohomology framework:
\begin{enumerate}[label=\textup{(\alph*)}]
\item The Arakelov--Faltings theorem~\cite{Fal84} identifies the
 fiberwise bar curvature with the curvature of the Hodge bundle
 in the Arakelov metric \textup{(}a fiberwise statement,
 both sides living on~$\Sigma_g$\textup{)}.
\item The Quillen metric on the determinant line bundle
 $\lambda = \det(\mathbb{E})$, constructed from the Arakelov
 metric and the analytic torsion~\cite{Quillen85}, provides
 the metrized line bundle on~$\overline{\cM}_g$.
\item The Bismut--Gillet--Soul\'e anomaly
 formula~\cite{BGS88a} converts the fiberwise curvature
 data into the Chern--Weil representative of
 $c_1(\lambda)$ on the base~$\overline{\cM}_g$, and the
 same mechanism extends to all Chern classes
 $c_j(\mathbb{E})$ via the splitting principle.
\item Mumford's GRR computation~\cite{Mumford83} confirms
 $c_g(\mathbb{E}) = \lambda_g$.
\end{enumerate}
The external inputs are: the Arakelov--Faltings theorem
\textup{(}classical, 1984\textup{)}, the BGS anomaly formula
\textup{(}classical, 1988\textup{)}, and Mumford's GRR
computation \textup{(}classical, 1983\textup{)}. No
clutching-uniqueness or boundary-restriction argument is
needed.
\end{remark}

% ================================================================
% PROPOSITION: Explicit clutching formulas for lambda_g.
% ================================================================

\begin{proposition}[Clutching formulas for the Hodge Euler class;
\ClaimStatusProvedHere]
\label{prop:lambda-g-clutching}
\index{clutching!Hodge Euler class formulas|textbf}
\index{Hodge bundle!clutching formulas}
Let
\[
\xi_h\colon \overline{\cM}_{h,1}\times\overline{\cM}_{g-h,1}
\longrightarrow \overline{\cM}_g
\qquad (1 \leq h \leq g-1)
\]
be the separating clutching morphism and let
\[
\xi_{\mathrm{irr}}\colon \overline{\cM}_{g-1,2}
\longrightarrow \overline{\cM}_g
\]
be the irreducible boundary clutching morphism. Then
\[
\xi_h^*\mathbb{E}
\;\cong\;
\mathrm{pr}_1^*\mathbb{E}_h \oplus \mathrm{pr}_2^*\mathbb{E}_{g-h},
\qquad
...
0.
\end{equation}
In particular, the irreducible-boundary restriction is zero:
there is no extra line-class factor on $\Delta_0$.
\end{proposition}

\begin{proof}
For a stable curve of compact type
$C=C_1\cup_q C_2$ with genera $h$ and $g-h$, a section of the
dualizing sheaf $\omega_C$ is exactly a pair of holomorphic
differentials on $C_1$ and $C_2$; there is no residue-matching
condition because the node is separating. Hence
\[
H^0(C,\omega_C)
\;\cong\;
...
\]
which globalizes to
$\xi_h^*\mathbb{E}\cong \mathrm{pr}_1^*\mathbb{E}_h
\oplus \mathrm{pr}_2^*\mathbb{E}_{g-h}$.
Taking top Chern classes and using Whitney gives
\eqref{eq:lambda-g-separating}.

For an irreducible nodal curve $C$ of genus~$g$ with normalization
$\nu\colon \widetilde C \to C$ and preimages $p,q$ of the node,
sections of $\omega_C$ identify with meromorphic differentials on
$\widetilde C$ having at most simple poles at $p$ and $q$ whose
residues sum to zero. This gives an exact sequence
\[
0 \longrightarrow H^0(\widetilde C,\omega_{\widetilde C})
\longrightarrow H^0(C,\omega_C)
\xrightarrow{\ \mathrm{res}_p\ }
\mathbb{C}
\longrightarrow 0.
\]
The residue parameter trivializes the quotient line, so over
$\overline{\cM}_{g-1,2}$ one gets
$\xi_{\mathrm{irr}}^*\mathbb{E}\cong \mathbb{E}_{g-1}\oplus\cO$.
Since $\mathbb{E}_{g-1}$ has rank $g-1$, the Whitney formula yields
\[
\xi_{\mathrm{irr}}^*\lambda_g
\;=\;
...
0,
\]
which is \eqref{eq:lambda-g-nonseparating}.
\end{proof}

% ================================================================
% PROPOSITION: Clutching uniqueness of the Hodge Euler class.
% Independent corroboration of prop:scalar-obstruction-hodge-euler
% via the boundary stratification of M-bar_g.
% ================================================================

\begin{proposition}[Clutching uniqueness of the Hodge Euler
class, socle scope; \ClaimStatusProvedHere]
\label{prop:clutching-uniqueness}
\index{clutching!uniqueness of Hodge Euler class|textbf}
\index{Hodge bundle!clutching uniqueness}
\index{Faber--Pandharipande!tautological ring}
Let $\alpha_g \in H^{2g}(\overline{\cM}_g,\Q)$ be a cohomology
class with the following three properties:
\begin{enumerate}[label=\textup{(\alph*)}]
\item $\alpha_g$ lies in the tautological subring
 $R^*(\overline{\cM}_g)$ \textup{(}Mumford~\cite{Mumford83}%
 \textup{)}.
\item The pullback of $\alpha_g$ along the clutching morphism
 $\xi_{\mathrm{irr}}\colon \overline{\cM}_{g-1,2} \to
 \overline{\cM}_g$ \textup{(}non-separating node\textup{)}
 equals the pullback of $\lambda_g$.
\item The pullback of $\alpha_g$ along each separating clutching
 $\xi_{h}\colon \overline{\cM}_{h,1}\times\overline{\cM}_{g-h,1}
 \to \overline{\cM}_g$ for $1\leq h\leq g-1$ satisfies the
 Mumford multiplicativity
 $\xi_h^*\alpha_g = \lambda_{h}\boxtimes\lambda_{g-h}$.
\end{enumerate}
Then $\alpha_g$ and $\lambda_g$ coincide in the socle quotient
$R^g(\overline{\cM}_g)/\cN^g(\overline{\cM}_g)$, where
$\cN^g(\overline{\cM}_g)$ denotes the numerical-equivalence
kernel at degree~$g$. At $g \in \{1,2\}$, $\cN^g = 0$ and
the identification lifts to on-the-nose equality
$\alpha_g = \lambda_g$ in $H^{2g}(\overline{\cM}_g,\Q)$; at
$g \geq 3$, the lift requires the $\lambda_g$-conjecture
\textup{(}Faber--Pandharipande~\cite{FP00}\textup{)}, which
is open in general.
\end{proposition}

\begin{proof}
The socle-projection argument is carried out in detail in
Theorem~\ref{thm:clutching-uniqueness-socle-projection} of
\texttt{clutching\_uniqueness\_platonic.tex}. The tautological
ring $R^*(\overline{\cM}_g)$ is a finitely generated graded
subring of $H^*(\overline{\cM}_g,\Q)$
\textup{(}Faber--Pandharipande~\cite{FP00}\textup{)}. The
boundary-restriction induction of
Theorem~\ref{thm:clutching-uniqueness-socle-projection}
shows that conditions~(a)--(c) force
$\alpha_g - \lambda_g \in \cN^g(\overline{\cM}_g)$, which is
precisely the socle-projection identity
$\pi_{\mathrm{soc}}(\alpha_g) = \pi_{\mathrm{soc}}(\lambda_g)$.
At $g = 1$, $R^1(\overline{\cM}_{1,1}) = \Q\cdot\lambda_1$ is
one-dimensional so $\cN^1 = 0$; at $g = 2$, Mumford's
computation of $R^*(\overline{\cM}_2)$ gives $\cN^2 = 0$ by
explicit check of the socle pairing~\cite{Mumford83}. At
$g \geq 3$, the kernel $\cN^g$ is the content of Faber's
$\lambda_g$-conjecture, known only in specific low-genus
cases.
\end{proof}

\begin{conjecture}[Conjecture F6: clutching-uniqueness of the Hodge
Euler class; \ClaimStatusConjectured]
\label{conj:F6-lambda-g-clutching-uniqueness}
\index{F6 conjecture!clutching-uniqueness of $\lambda_g$|textbf}
\index{lambda-g conjecture@$\lambda_g$ conjecture!clutching-uniqueness form}
For every $g \geq 3$, if
$\alpha_g \in R^g(\overline{\cM}_g)$ is a tautological class whose
separating and non-separating clutching pullbacks agree with those of
$\lambda_g = c_g(\mathbb{E})$, then
\[
\alpha_g \;=\; \lambda_g
\qquad \text{in } R^g(\overline{\cM}_g).
\]
Equivalently, the numerical-equivalence kernel
$\cN^g(\overline{\cM}_g)$ vanishes, so the socle identity of
Proposition~\ref{prop:clutching-uniqueness} lifts to on-the-nose
equality. This is the residual open statement of the
clutching-only proof route; it is \emph{not} an input to the
all-weight free-energy formula of
Theorem~\ref{thm:genus-universality}.
\end{conjecture}

\begin{remark}[F6 status]
\label{rem:F6-status}
What remains open at $g \geq 3$ is only the \emph{clutching-only}
uniqueness lift from the socle quotient to the full tautological
ring. The direct Arakelov--Faltings--BGS--GRR route already proves
\[
\mathrm{obs}_g(\cA)
\;=\;
\kappa(\cA)\cdot\lambda_g
\qquad \textup{(}UNIFORM-WEIGHT\textup{})
\]
in cohomology, and the scalar Faber--Pandharipande pairing gives
\[
\sum_{g \geq 1}F_g(\cA)\,x^{2g}
\;=\;
\kappa(\cA)\cdot\bigl(\widehat{A}(ix)-1\bigr).
\]
Conjecture~\ref{conj:F6-lambda-g-clutching-uniqueness} asks only
whether the same conclusion follows from the clutching identities of
Proposition~\ref{prop:lambda-g-clutching} alone, equivalently whether
$\cN^g(\overline{\cM}_g)=0$.
\end{remark}

\begin{remark}[AP225 scope correction]
\label{rem:ap225-scope-correction}
\index{AP225!scope correction socle vs on-the-nose}
The status-table entry "AP225 resolved via
clutching-uniqueness pinning $\mathrm{obs}_g/\kappa = \lambda_g$
uniquely" should be read as: uniquely pinned on the socle
quotient $R^g/\cN^g$, equivalently under the Faber socle
pairing $\int_{\overline{\cM}_g}(\cdot)\cdot\omega_{\mathrm{soc}}$
for every admissible~$\omega_{\mathrm{soc}}$. On-the-nose
equality in $R^g(\overline{\cM}_g)$ is unconditional at
$g \in \{1,2\}$ and conditional on the
$\lambda_g$-conjecture at $g \geq 3$. The direct
Arakelov--Faltings--BGS path
\textup{(}Proposition~\ref{prop:scalar-obstruction-hodge-euler}%
\textup{)} produces on-the-nose equality in
$H^{2g}(\overline{\cM}_g,\Q)$ under the scalar-diagonal
hypothesis
\textup{(}Definition~\ref{def:scalar-diagonal-hypothesis}%
\textup{)}; the clutching-uniqueness path above operates at
the \emph{socle} level for $g \geq 3$ but requires neither
scalar-diagonality nor Arakelov input.
\end{remark}

\begin{remark}[Role in the genus-universality argument]
\label{rem:clutching-uniqueness-role}
Proposition~\ref{prop:clutching-uniqueness} provides an
\emph{independent corroboration} of the identification
$\mathrm{obs}_g(\cA) = \kappa(\cA)\cdot\lambda_g$ obtained by
Proposition~\ref{prop:scalar-obstruction-hodge-euler}.
Concretely: the scalar obstruction
$\mathrm{obs}_g(\cA)/\kappa(\cA)$ is tautological (it is
constructed from Chern classes of the Hodge bundle, via the
Arakelov--Faltings--BGS chain), its non-separating restriction
matches $\lambda_g$ by the degeneration of the bar family at
a node, and its separating restriction factorises as
$\lambda_h\boxtimes\lambda_{g-h}$ by the factorisation
property of the bar complex at separating nodes. Hence by
Proposition~\ref{prop:clutching-uniqueness},
$\mathrm{obs}_g(\cA)/\kappa(\cA) = \lambda_g$. The
determinant-of-cohomology path
\textup{(}Steps~1--3 of
Proposition~\ref{prop:scalar-obstruction-hodge-euler}%
\textup{)} and the clutching-uniqueness path of this
proposition are logically independent and reach the same
conclusion.
\end{remark}

% ================================================================
% PROPOSITION: Factorization-homology alternative proof
% via the PTVV mapping-stack construction.
% ================================================================

\begin{proposition}[Factorization-homology derivation via PTVV;
\ClaimStatusConditional]
\label{prop:theorem-D-factorization-homology-alt}
\index{PTVV!mapping stack derivation of Theorem D}
\index{factorization homology!Theorem D alternative route}
\index{1-shifted symplectic!mapping stack on M-bar_g}
Let $\cA$ be a modular Koszul chiral algebra on the uniform-weight
scalar lane, and let $BG_{\cA}$ denote the classifying stack of
the factorization group scheme associated with~$\cA$
\textup{(}in the sense of
Costello--Gwilliam~\cite{CG17,CG-vol2}\textup{)}.
Consider the derived mapping stack
$\mathbf{RMap}(\overline{\cM}_g, BG_{\cA})$. Then:
\begin{enumerate}[label=\textup{(\roman*)}]
\item \emph{Shifted symplectic structure.}
$\RMap(\overline{\cM}_g, BG_{\cA})$ carries a canonical
$1$-shifted symplectic structure, constructed by the
Pantev--To\"en--Vaqui\'e--Vezzosi transgression
procedure~\cite{PTVV13} applied to the
$(1-\dim\overline{\cM}_g)$-shifted symplectic form on
$BG_{\cA}$.
\item \emph{AKSZ partition function.}
The corresponding AKSZ-type factorization-homology partition
function
$\int_{\overline{\cM}_g}\cA$
computes a class in $H^{2g}(\overline{\cM}_g,\Q)$ whose scalar
component equals $\mathrm{obs}_g(\cA)$.
\item \emph{Identification with the Hodge Euler class.}
On the scalar lane, $\int_{\overline{\cM}_g}\cA
= \kappa(\cA)\cdot\lambda_g$, matching the conclusion of
Proposition~\ref{prop:scalar-obstruction-hodge-euler}.
\end{enumerate}
\end{proposition}

\begin{proof}[Proof sketch]
Part~(i) is the direct application of
\cite[Theorem~2.5]{PTVV13}: the stack $BG_{\cA}$ is
$(1-\dim\overline{\cM}_g) = (1-(3g-3))$-shifted symplectic on
the nose in the factorization framework, and transgression
along the mapping stack adds $\dim\overline{\cM}_g$ shift,
producing the $1$-shifted symplectic form on
$\mathbf{RMap}(\overline{\cM}_g, BG_{\cA})$. Part~(ii) is the AKSZ
construction~\cite{Costello17,CG-vol2,Pridham17}: the
$1$-shifted symplectic form integrates against the
fundamental class $[\overline{\cM}_g]$ to produce a Chern--Weil
representative of the anomaly class in
$H^{2g}(\overline{\cM}_g,\Q)$.

Part~(iii) is derived \emph{internally to the PTVV framework},
without citation back to
Proposition~\ref{prop:scalar-obstruction-hodge-euler}. The
AKSZ partition function $\int_{\overline{\cM}_g}\cA$ is
tautological by construction: it is assembled from the PTVV
pairing on a nodal curve, which factorises as the product of
pairings on the normalised components. On the genus-$1$
component $\overline{\cM}_{1,1}$, the AKSZ integral reduces
to the one-dimensional cohomology class
$\kappa(\cA)\cdot\lambda_1$ determined by the PTVV pairing
on the universal elliptic curve; this is where the scalar
coefficient $\kappa(\cA)$ is \emph{detected}, as the
unique scalar in $H^2(\overline{\cM}_{1,1},\Q) =
\Q\cdot\lambda_1$ making the AKSZ output match the genus-$1$
bar curvature. The boundary restrictions of the AKSZ output
satisfy the Mumford multiplicativity
$\xi_h^*(\int_{\overline{\cM}_g}\cA) = (\int_{\overline{\cM}_{h,1}}\cA)
\boxtimes (\int_{\overline{\cM}_{g-h,1}}\cA)$ by the
factorisation axioms, and the non-separating restriction
$\xi_{\mathrm{irr}}^*(\int_{\overline{\cM}_g}\cA)$ degenerates
to the genus-$(g-1)$ AKSZ integral with two marked points.
Conditions (a)--(c) of
Proposition~\ref{prop:clutching-uniqueness} are therefore
satisfied, and clutching-uniqueness on the socle yields
$\int_{\overline{\cM}_g}\cA = \kappa(\cA)\cdot\lambda_g$ in
the socle quotient $R^g/\cN^g$. At $g \in \{1,2\}$ the
identification lifts to on-the-nose equality in
$H^{2g}(\overline{\cM}_g,\Q)$; at $g \geq 3$ it is
conditional on the $\lambda_g$-conjecture
\textup{(}inherited from
Proposition~\ref{prop:clutching-uniqueness}\textup{)}.
\end{proof}

\begin{remark}[Scope, genus-stratified]
\label{rem:ptvv-aksz-chain-level-scope}
Part~(iii) invokes AKSZ boundary factorisation at separating and
non-separating nodes of $\overline{\cM}_g$ via the factorisation
axioms of~$\cA$. The chain-level inscription of this
factorisation-at-nodes input is genus-stratified. At $g = 1$ no
nodal sewing is required: the AKSZ pairing on
$\overline{\cM}_{1,1}$ is evaluated directly on the universal
elliptic curve and the scalar coefficient $\kappa(\cA)$ is read
off from $H^2(\overline{\cM}_{1,1},\Q)=\Q\cdot\lambda_1$. At
$g = 2$, the boundary of $\overline{\cM}_2$ consists of the
separating divisor $\delta_1$ and the non-separating divisor
$\delta_{\mathrm{irr}}$; the chain-level sewing at both is
handled by the classical Knudsen--Mumford stable reduction for
nodal curves of compact type together with Deligne--Mumford
properness, neither of which requires logarithmic
Fulton--MacPherson machinery. At $g \geq 3$, the chain-level
sewing across the full boundary stratification of
$\overline{\cM}_g$ invokes Mok~\cite{Mok25} logarithmic
Fulton--MacPherson compactification and the Francis--Gaitsgory
six-functor base-change for the relative Ran prestack. Per
Theorem~\ref{thm:A-infinity-2} modular-family scope, these
inputs are conditional at $g \geq 3$ only; see
Remark~\ref{rem:A-infinity-2-modular-family-scope}.
\end{remark}

\begin{remark}[Status of the PTVV route relative to
Proposition~\textup{\ref{prop:scalar-obstruction-hodge-euler}}]
\label{rem:ptvv-independence}
Proposition~\ref{prop:theorem-D-factorization-homology-alt}
does not use the Arakelov metric, the BGS anomaly formula,
or the Faltings identification of the Hodge-bundle curvature.
Its inputs are the factorization structure of $\cA$, the PTVV
transgression theorem~\cite{PTVV13}, the AKSZ
construction~\cite{Costello17,CG-vol2}, and the socle
clutching-uniqueness
\textup{(}Proposition~\ref{prop:clutching-uniqueness}%
\textup{)}. It does \emph{not} invoke
Proposition~\ref{prop:scalar-obstruction-hodge-euler}, the
scalar-diagonal hypothesis, or any Chern--Weil/BGS machinery:
the scalar coefficient $\kappa(\cA)$ is detected on the
genus-$1$ component directly from the PTVV pairing, and the
all-$g$ identification is achieved by clutching.
\medskip

\emph{Honest status assessment.}
The two propositions furnish \emph{distinct presentations}
of the same underlying obstruction class
$\int_{\overline{\cM}_g}\cA = \mathrm{obs}_g(\cA)$:
Proposition~\ref{prop:scalar-obstruction-hodge-euler} via
differential-geometric Chern--Weil representatives
\textup{(}needing scalar-diagonality\textup{)}, and the
present proposition via derived-algebraic AKSZ + socle
clutching \textup{(}needing the $\lambda_g$-conjecture at
$g \geq 3$ for on-the-nose equality, but \emph{not} needing
scalar-diagonality\textup{)}. The two sides of their proof
chain are algebraically independent in the sense that no
intermediate lemma or definition is shared, but they each
access only a \emph{partial} scope of the full all-genus
all-family identification: the PTVV path gives the result
for \emph{all} uniform-weight families on the socle
\textup{(}without scalar-diagonality\textup{)}, while the
Arakelov--BGS path gives it on the nose for scalar-diagonal
families \textup{(}without requiring the
$\lambda_g$-conjecture\textup{)}. The intersection of their
scopes, i.e.\ scalar-diagonal uniform-weight families at
$g \in \{1,2\}$, is where both yield on-the-nose equality.
This should be recorded as \emph{complementary} rather than
\emph{genuinely-disjoint} verification: an HZ-IV decorator
built on the present proposition must declare
\texttt{verified\_against}\,=\,
\texttt{clutching\_socle\_PTVV}
and \texttt{disjoint\_rationale}\,=\,
\texttt{no Chern--Weil/Arakelov input; scope
restricted to socle at $g\ge3$}, recognising that the
coincidence-of-output with
Proposition~\ref{prop:scalar-obstruction-hodge-euler}
is not by construction but by the abstract uniqueness
statement of Proposition~\ref{prop:clutching-uniqueness}.
\end{remark}
...
\begin{table}[ht]
\centering
\caption{Genus-$g$ obstruction classes. For scalar-diagonal families
 \textup{(}Heisenberg, Virasoro, rank-$1$ free fields unconditionally;
 $V_k(\fg)$ at positive-integer level via the Frenkel--Kac lattice
 realisation and the parafermionic coset; generic-level $V_k(\fg)$ with
 $\fg$ non-abelian conditional on
 Definition~\textup{\ref{def:scalar-diagonal-hypothesis}}\textup{)},
 $\mathrm{obs}_g = \kappa \cdot \lambda_g$
 \textup{(}UNIFORM-WEIGHT, SCALAR-DIAGONAL\textup{)} holds unconditionally at all genera for
 $g \in \{1, 2\}$ and on the socle quotient $R^g/\cN^g$ for $g \geq 3$
 \textup{(}on the nose conditional on the $\lambda_g$-conjecture\textup{)}.
 For multi-weight families \textup{(}$W_3$\textup{)}, the scalar formula is unconditional at $g = 1$;
 at $g = 2$ a cross-channel correction
 $\delta F_2^{\mathrm{cross}}(W_3) = (c+204)/(16c)$
 \textup{(}ALL-WEIGHT + $\delta F_g^{\mathrm{cross}}$; explicit closed form proved at $g=2$%
 \textup{)} is present
 \textup{(}Theorem~\textup{\ref{thm:multi-weight-genus-expansion}}\textup{)};
 for $\cW_3$ at $g = 3$ one has the closed form
 $\delta F_3^{\mathrm{cross}}(\cW_3)
 = (5c^3 + 3792c^2 + 1149120c + 217071360)/(138240\,c^2)$
 \textup{(}Proposition~\textup{\ref{prop:w3-genus3-cross-channel}}\textup{)};
 beyond these first examples the graph-sum construction remains the
 general source of explicit higher-genus corrections.}
...
Let $\cA$ be a modular Koszul chiral algebra with strong generators of
conformal weights $h_1,\ldots,h_r$. There exists a genus-independent
scalar $\kappa(\cA)$, determined by the genus-$1$ curvature, and an
explicit mixed-channel correction $\delta F_g^{\mathrm{cross}}(\cA)$
for each $g \geq 1$ such that
...
\textup{(}\text{ALL-WEIGHT} + \delta F_g^{\mathrm{cross}}\textup{})
\end{equation}
for all $g \geq 1$, where
$\lambda_g = c_g(\mathbb{E})$ is the top Chern class of the Hodge
bundle $\mathbb{E} = R^0\pi_*\omega$ and
$\lambda_g^{\mathrm{FP}}
= \int_{\overline{\mathcal{M}}_{g,1}}\psi_1^{2g-2}\lambda_g$.
The correction satisfies
$\delta F_1^{\mathrm{cross}}(\cA)=0$ universally.
If $h_1=\cdots=h_r$ \textup{(}the uniform-weight scalar lane of
Definition~\textup{\ref{def:scalar-lane}}\textup{)}, then
$\delta F_g^{\mathrm{cross}}(\cA)=0$ for all $g \geq 1$, and the
cohomological obstruction class satisfies
\[
\mathrm{obs}_g(\cA)
\;=\;
\kappa(\cA)\cdot\lambda_g
...
\]
In particular,
$\delta F_g^{\mathrm{cross}}(\Vir_c)=0$ for every $g \geq 1$, while
for the first multi-weight family one has
\[
...
($\varrho = 5/6$, $K = 100$).
\emph{Vanishing locus of $\kappa$ (lane-internal; not universal).}
On the affine Kac--Moody lane, $\kappa(V_k(\fg)) = 0$ if and only if
$k = -h^\vee$, the critical level; in that case the Feigin--Frenkel
involution sends the dual to its own critical level, so
$\kappa(\mathcal{A}^!) = 0$ as well. On the Virasoro lane,
$\kappa(\mathrm{Vir}_c) = c/2 = 0$ if and only if $c = 0$, with the
Verdier dual at $c = 26$ (where $\kappa(\mathrm{Vir}_{26}) = 13$).
On the $\beta\gamma_\lambda$ lane,
$\kappa(\beta\gamma_\lambda) = 6\lambda^2 - 6\lambda + 1 = 0$ if and
only if $\lambda = (3 \pm \sqrt{3})/6$, irrational values where there
is no notion of ``critical level'' in any affine $h^\vee$-shifted
sense. On the $\mathcal{W}_N$ lane, $\kappa(\mathcal{W}^k(\fg)) = 0$
if and only if the underlying central charge $c = 0$, in which case
$\kappa(\mathcal{A}^!) = \varrho(\fg) K \neq 0$ (the dual is not at
its own ``critical'' point).
The classical heuristic ``$\kappa = 0$ iff critical level'' is
\emph{lane-internal} to affine Kac--Moody and is \emph{not} a
universal statement across the standard landscape; a uniform
statement is given by the lane-stable equivalent
$\kappa(\cA) = 0 \iff
[\Theta_{\cA}^{(1)}]_{\mathrm{scalar}} = 0$
(genus-$1$ scalar curvature vanishes), which is a definition
unfolded, and is the only statement with universal scope.
...
On the scalar lane (Definition~\ref{def:scalar-lane}), the genus-$g$
free energy is
$F_g = \kappa \cdot \frac{2^{2g-1}-1}{2^{2g-1}}
\cdot \frac{|B_{2g}|}{(2g)!}$,
where $B_{2g}$ are Bernoulli numbers.
For arbitrary modular Koszul algebras, the unconditional scalar
statement is the genus-$1$ identity
$F_1 = \kappa/24$
\textup{(g=1 only; all-weight at g=1 is unconditional)},
and the full generating series decomposes as
\[
\sum_{g \geq 1} F_g(\cA)\,x^{2g}
\;=\;
\kappa(\cA)\cdot\left(\frac{x/2}{\sin(x/2)} - 1\right)
\;+\;
\sum_{g \geq 2}\delta F_g^{\mathrm{cross}}(\cA)\,x^{2g}.
\]
\end{enumerate}
\end{theorem}
...
For the uniform-weight scalar lane,
...
\qquad \textup{(}UNIFORM-WEIGHT\textup{)}
\]
for all $g \geq 1$ in the scalar-diagonal regime. For a general
modular Koszul algebra, Theorem~\ref{thm:multi-weight-genus-expansion}
gives the all-weight decomposition
\[
F_g(\cA)
\;=\;
\kappa(\cA)\cdot\lambda_g^{\mathrm{FP}}
...
\textup{(}\text{ALL-WEIGHT} + \delta F_g^{\mathrm{cross}}\textup{}),
\]
with $\delta F_1^{\mathrm{cross}}(\cA)=0$ universally and
$\delta F_g^{\mathrm{cross}}(\cA)=0$ on the uniform-weight lane.
Since $\Vir_c$ has a single strong generator, it lies on that lane and
hence $\delta F_g^{\mathrm{cross}}(\Vir_c)=0$ for every $g \geq 1$.
The explicit $\cW_3$ formulas are
Theorem~\ref{thm:multi-weight-genus-expansion}\textup{(vi)} at genus~$2$
and Proposition~\ref{prop:w3-genus3-cross-channel} at genus~$3$.
...
(Theorem~\ref{thm:mumford-formula}) together with the
all-weight decomposition above, summed over $g$.
\end{proof}

The content of Theorem~\ref{thm:genus-universality} is that genus
is inert. A genus-$47$ surface and a torus produce the same
obstruction coefficient $\kappa$; the only quantity that changes is
the intersection number $\lambda_g$, which is a topological datum
of the moduli space, not of the algebra. The entire
higher-genus tower is therefore determined at genus~$1$: the
single number $\kappa(\cA)$, extracted from the curvature of the
Hodge bundle over the modular parameter $\tau$, propagates without
modification to every genus. No new dynamical information enters.
The partition function, at every loop order, knows only one number.

\begin{remark}[Universality hierarchy for genus extension]
\label{rem:universality-hierarchy}
\index{universality!hierarchy for genus extension}
\index{genus extension!five-level hierarchy}
\index{quasi-conformal vertex algebra!genus extension}
\index{Koszulness!universality hierarchy}
\index{modular functor!universality hierarchy}
The extension of a chiral algebra from genus~$0$ to higher
genera is controlled by a five-level hierarchy of
increasingly stringent conditions. The bottom level is
Cliff universality
(Remark~\ref{rem:universality-precondition}); each
subsequent level imposes additional structure that unlocks
stronger genus-extension theorems.

\begin{enumerate}[label=\textup{Level~\arabic*.},
 leftmargin=3.5em, labelsep=0.7em]
\item[\textup{Level~0.}] \emph{Algebraic} (universal
 chiral algebra). A quasi-conformal vertex algebra
 $V$ determines a chiral algebra $\cA_X$ on every
 smooth curve~$X$
 (Cliff~\cite[Proposition~8.1]{Cliff19};
 Frenkel--Ben-Zvi~\cite[Ch.~6]{FBZ04}).
 The bar complex
 $\barB_X(\cA_X)$ is defined at every genus; the OPE
...
\item[\textup{Level~2.}] \emph{Koszul} (curve-independent
 ...
 \textup{(g=1 only; all-weight at g=1 is unconditional)}
 unconditionally
 (Theorem~\ref{thm:genus-universality}), are consequences
 of Koszulness combined with universality:
 the \'etale descent datum from Level~0 ensures
 that the shadow obstruction tower is extracted from
 curve-independent data
 (Remark~\ref{rem:cliff-universality}).
\item[\textup{Level~3.}] \emph{Analytic} (sewing and
 convergence). The sewing envelope $\cA^{\mathrm{sew}}$
 (Hausdorff completion of the algebraic chiral algebra)
 provides convergent partition functions at all genera.
 The HS-sewing criterion
 (Theorem~\ref{thm:general-hs-sewing}: polynomial OPE
 growth $+$ subexponential sector growth $\Rightarrow$
 convergence) is proved for the entire standard landscape.
 Universality at this level means the sewing amplitudes
 are consistently defined across all curves.
\item[\textup{Level~4.}] \emph{Modular functor}.
 The strongest level: the conformal blocks
 $\{V_{g,n}(\cA)\}$ assemble into a modular functor
 (Theorem~\ref{thm:chain-modular-functor}). This
 requires $C_2$-cofiniteness \emph{in addition to}
 universality. Note that $C_2$-cofiniteness is
 orthogonal to Koszulness
 (\S\ref{conj:c2-cofiniteness-koszul-interaction}):
 a universal $V_k(\fg)$ is always Koszul but generically
 not $C_2$-cofinite; the simple quotient $L_k(\fg)$ at
 admissible level is $C_2$-cofinite but Koszulness may
 fail. Koszul duality exchanges the two conditions.
\end{enumerate}

The bar-cobar framework of this monograph operates at
Levels~0--3. Level~0 (universality) is the precondition
that makes all other levels meaningful. The standard
families (Heisenberg, affine Kac--Moody, Virasoro,
$\mathcal{W}_N$, $\beta\gamma$, lattice VOAs) satisfy
Levels~0--3; the simple quotients at admissible level
reach Level~4. A non-universal chiral algebra (one
lacking the $\operatorname{Aut}(\mathcal{O})$-action)
can still have a bar complex on a fixed curve, but its
invariants may depend on the choice of curve, and the
shadow obstruction tower loses its curve-independent
interpretation.
\end{remark}
...
tadpole) first appear, and mixed-channel
assignments $(T, W, W)$, $(T, W)$, etc.\ contribute nonzero
amplitudes. The $c$-independent tadpole piece $1/16$ arises
...
$\mathrm{obs}_1(\cA) = \kappa \cdot \lambda_1$
\textup{(g=1 only; all-weight at g=1 is unconditional)} unconditionally.
...
This has been resolved negatively:
...
For affine Kac--Moody $\widehat{\mathfrak{g}}_k$, the obstruction coefficient is periodic under level shift: $\kappa(\widehat{\mathfrak{g}}_{k+n}) = \kappa(\widehat{\mathfrak{g}}_k) + n \cdot d/(2h^\vee)$ for $n \in \mathbb{Z}$. In particular, the free energy $F_g$ is a linear function of $k$ with slope $\lambda_g^{FP} \cdot d/(2h^\vee)$ \textup{(}UNIFORM-WEIGHT\textup{}). For $\mathcal{W}$-algebras, $\kappa$ is a rational function of $k$ (through its dependence on the DS central charge), and the periodicity statement becomes: $\kappa(\mathcal{W}^{k+n}(\mathfrak{g})) - \kappa(\mathcal{W}^k(\mathfrak{g}))$ is a rational function of $k$ with poles only at $k = -h^\vee$.
\end{corollary}
...
$\mathrm{obs}_g = \kappa \cdot \lambda_g$ \textup{(}UNIFORM-WEIGHT\textup{)} and the free-energy
formula $F_g = \kappa \cdot \lambda_g^{\mathrm{FP}}$ \textup{(}UNIFORM-WEIGHT\textup{)} hold unconditionally at all genera. For multi-generator
...
\qquad \textup{(g=1 only; all-weight at g=1 is unconditional)}
\;\in\; R^1(\overline{\mathcal{M}}_1).
\]
On the proved scalar lane, one also has
\[
...
$\kappa \cdot \lambda_g \in R^g(\overline{\mathcal{M}}_g)$ These scalars are the scalar level
...
in the example chapters of Part~\ref{part:characteristic-datum}, extend this table into a
...
\prod_i\psi_i^{h_i}$ \textup{(}UNIFORM-WEIGHT\textup{)} gives products in
...
$\mathrm{obs}_g = \kappa \cdot \lambda_g$ \textup{(}UNIFORM-WEIGHT\textup{)} by Theorem~\ref{thm:genus-universality} and is
...
$\mathrm{obs}_g = \kappa \cdot \lambda_g$
\textup{(}UNIFORM-WEIGHT; proved scalar lane, see
...
$\mathrm{obs}_1 = \kappa \cdot \lambda_1$
\textup{(g=1 only; all-weight at g=1 is unconditional)} gives
...
Maurer--Cartan element $\Theta_{\cA}$ or the higher-degree shadow obstruction tower
to vanish.
\end{remark}
...
$F_g = \kappa \cdot \lambda_g^{FP}$ \textup{(}UNIFORM-WEIGHT\textup{)} is therefore a tautological
...
\begin{theorem}[Wick-rotated \texorpdfstring{$\widehat{A}$}{A-hat} generating function on the proved scalar lane; \ClaimStatusProvedHere]
...
On the proved uniform-weight scalar lane for a modular Koszul chiral
algebra $\cA$, with
$\widehat{A}(x)=\frac{x/2}{\sinh(x/2)}$, the scalar genus
coefficients are the Faber--Pandharipande Hodge integrals
...
cotangent line class. Equivalently,
$F_g(\cA)=\kappa(\cA)\lambda_g^{\mathrm{FP}}$ with
$\lambda_g^{\mathrm{FP}}
= \int_{\overline{\mathcal{M}}_{g,1}}\psi_1^{2g-2}\lambda_g$.
...
\psi_1^{2g-2}\, c_g(\mathbb{E})\right) x^{2g}
\;=\;
\kappa(\cA) \cdot \bigl(\widehat{A}(ix) - 1\bigr)
\;=\;
\kappa(\cA) \cdot \Bigl(\frac{x/2}{\sin(x/2)} - 1\Bigr)
\end{equation}
is an explicit Hodge integral series. This covers the scalar-lane
rows of the standard landscape, including Heisenberg, affine
Kac--Moody, Virasoro, lattice, and moonshine examples. For
multi-weight families such as $\beta\gamma$, $bc$, and
$\mathcal{W}_N$ \textup{(}$N \geq 3$\textup{})}, the full free
energy at $g \geq 2$ is
$F_g=\kappa(\cA)\lambda_g^{\mathrm{FP}}+\delta F_g^{\mathrm{cross}}$;
in particular
$\delta F_2^{\mathrm{cross}}(\cW_3)=(c+204)/(16c)\neq 0$, so the
cross-channel term is not absorbed by the same
$\widehat{A}(ix)$-series. For arbitrary modular Koszul algebras, the
unconditional scalar specialization is
...
\begin{proof}
By Theorem~\ref{thm:genus-universality}, on the proved
uniform-weight scalar lane one has
\[
\mathrm{obs}_g(\cA)
\;=\;
\kappa(\cA)\lambda_g
...
this class with $\psi_1^{2g-2}$ on $\overline{\mathcal{M}}_{g,1}$,
which gives \eqref{eq:grr-bridge}. The Grothendieck--Riemann--Roch
interpretation of the same Hodge integral is
\[
\operatorname{ch}(R\pi_*\omega_\pi)
\;=\;
\pi_*\!\bigl(\operatorname{ch}(\omega_\pi)\operatorname{Td}(T_\pi)\bigr),
\]
so the Hodge integral side of \eqref{eq:grr-bridge} is exactly the
degree-$g$ GRR pushforward for the universal curve.
...
\]
and summing these coefficients gives the middle equality in
\eqref{eq:grr-bridge-total}. The identity
$\widehat{A}(ix)=\frac{x/2}{\sin(x/2)}$ gives the last equality in
\eqref{eq:grr-bridge-total}. At genus~$1$ the same computation yields
$F_1(\cA) = \kappa(\cA)/24$ for every modular Koszul algebra.

The Bismut--Gillet--Soul\'e input here is only the rank-$1$ statement
$c_1(\det\mathbb{E})=\lambda_1$ in the Quillen metric. The top class
$\lambda_g=c_g(\mathbb{E})$ enters through the full Chern polynomial
and the splitting-principle/Chern--Weil argument already inscribed in
Proposition~\ref{prop:scalar-obstruction-hodge-euler}; no false
identity $\lambda_g=c_1(\mathbb{E})$ is used. The multi-weight clause
is exactly the $g \geq 2$ correction term of
Theorem~\ref{thm:genus-universality}.
\end{proof}
...
\frac{c+204}{16c},
\]
so the cross-channel correction is genuinely additional data.
\end{example}
...
Proposition~\ref{prop:scalar-obstruction-hodge-euler}
(via the Arakelov--Faltings theorem and Chern--Weil): the scalar
...
(ii)~In the QME at genus~$1$ and degree~$0$, the non-separating
...
propagator analysis in Part~\ref{part:characteristic-datum}.
\end{corollary}
...
\begin{proof}
The theorem immediately above shows that the loop-order spectral
sequence collapses at $E_1$, so
$E_1^{\ell,q}=E_\infty^{\ell,q}$ for every bidegree.
Because the loop filtration is exhaustive and complete, the bar
cohomology is the direct sum of its associated graded pieces
$E_\infty^{\ell,*}$.
By definition of the $E_1$ page, those pieces are exactly the
cohomologies of the loop-$\ell$ stable-graph complexes
$H^*(\mathsf{SG}_g^{(\ell)}(\cA),d_{\mathrm{tree}})$.
Putting these identifications together gives the displayed
decomposition of $H^*(\barB^{(g)}(\cA))$ by loop order.
\end{proof}
...
\begin{remark}[Universal $\Phi$-trace identity binding Vol~I higher-genus to Vol~III]
\label{rem:hgf-DNA-phi-trace-identity}
The five pillars above admit a single cross-volume synthesis via the
CY-to-chiral functor $\Phi$. On the higher-genus Vol~I side, the
modular-Koszul supertrace of the bar-curvature class aggregates the
all-genus data $\{\lambda_g\}_{g \geq 1}$ against the family $\kappa$-class:
\[
\mathrm{STr}_{\mathrm{bar}}^{\mathrm{mod-Koszul}}\!\bigl(\mathcal{A}\bigr)
\;=\;
\sum_{g \geq 1} \kappa(\mathcal{A}) \cdot \lambda_g
\in
H^{\bullet}(\overline{\mathcal{M}}_{g,n}).
\]
On the Vol~III side, the Borcherds pushforward of the
BKM $\kappa$-invariant over $K3 \times E$ gives
\[
\mathrm{push}_{K3 \times E}^{\mathrm{Borcherds}}\!\bigl(\kappa_{\mathrm{BKM}}\bigr)
\;=\;
\mathrm{STr}_{\mathrm{bar}}^{\mathrm{mod-Koszul}}\!\bigl(\Phi^{-1}(\mathcal{A})\bigr),
\]
a single numerical-class identity that binds the higher-genus modular-Koszul
trace (this chapter) to the BKM side of Vol~III via the functor $\Phi$.
This is the universal $\Phi$-trace identity: it states that the
all-genus obstruction-tower aggregate on the chiral side is computed by
the Borcherds pushforward on the CY side, with the Igusa cusp form
$\Phi_{10}$ and the weight-$5$ class $\Delta_5$ of
Remark~\ref{rem:hgf-DNA-H2-classification} both appearing as concrete
low-genus witnesses of the identity. See Part~\ref{part:physics-bridges}
for the $\Phi$-functor construction and the Vol~III $K3 \times E$ side.
\end{remark}
...
\label{sec:hgfnd-wave13-DNA}

\begin{remark}[Gritsenko additive lift of $\Delta_5$ and higher-genus modular Koszul duality]
\label{rem:hgfnd-DNA-1}
The higher-genus foundations assembled in this chapter meet the Wave~13
structure at the Gritsenko 1995 additive lift. Starting from the Jacobi
cusp form of weight $5$, index $1$ on $\Gamma^J_0(1)$, the Gritsenko
additive lift produces
\[
...
specific character $\mathrm{ch}$, such that $\Delta_5^2 = \Phi_{10}$
is the Igusa weight-$10$ cusp form. The higher-genus modular Koszul
duality for the K3 chiral bialgebra $\mathbf H_{\Delta_5}$ emerges as
the pullback of Bruinier 2002 Prop.~5.1 Heegner Chern-class reciprocity
along the Kuga--Satake morphism $K3\hookrightarrow\bar{\mathcal A}_2$.
In particular, the $g=2$ modular Koszul pair refines the $g=1$ pair
$(B_{\mathrm{ch}}(A),\Omega_X)$ through an explicit Humbert-divisor
resolution that carries the $\mathbb Z/8$-monodromy class established
in Vol.~III, Chapter~\ref{ch:k3-chiral-bialgebra-platonic}.
...
point is the Borel cusp. The higher-genus foundations of
$\mathbf H_{\Delta_5}$ descend through this stratification:
...
Heisenberg-type limit on $\mathcal F_{K3}$. The Gritsenko--Nikulin
1997--98 denominator identity
\[
\Phi_{10}(Z) \;=\; \prod_{(n,l,m)>0} (1 - q^n \zeta^l p^m)^{c(nm,l)}
\]
is the higher-genus trace of the bar--cobar adjunction on
$\mathbf H_{\Delta_5}$: each factor on the right corresponds to a
physical state labelled by a positive root of the associated BKM
algebra, and the exponents $c(nm,l)$ are the elliptic-genus Fourier
coefficients of $K3$. See Vol.~III, Chapter~\ref{ch:k3-chiral-bialgebra-platonic},
