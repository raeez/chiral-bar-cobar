...
is forced.  The Hodge class $\omega_g = c_1(\mathbb{E}) \in
H^2(\overline{\cM}_{g,n})$ lives on the moduli space, not on
the curve; $\kappa(\cA)$ is the scalar extracted from the
...
\item \emph{Hodge bundle structure.}\;
  ...
  \textup{(}or unconditionally at $g = 1$\textup{)}:
  ...
  class.  For multi-weight algebras at $g \geq 2$,
...
Explicitly:
\begin{itemize}
\item $g=1$: $\text{obs}_1 = \kappa \cdot \lambda_1$ where $\lambda_1 = c_1(\mathbb{E}) \in H^2(\overline{\mathcal{M}}_{1,1})$
...
identification $\Omega = \sum \lambda_g$ fails for multi-weight
...
$\mathrm{obs}_g = \kappa \cdot \lambda_g$ receives a nonvanishing
...
$\mathrm{obs}_1 = \kappa \cdot \lambda_1$ remains
...
The identity $\mathrm{obs}_g = \kappa \cdot \lambda_g$ \emph{fails}
...
$F_g \neq \kappa \cdot \lambda_g^{\mathrm{FP}}$ at $g \ge 2$
