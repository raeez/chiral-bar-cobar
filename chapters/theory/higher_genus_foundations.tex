...
Everything up to this point takes place at genus zero, and at
genus $g \geq 1$ the bar differential no longer squares to zero.
The genus-$0$ proof is combinatorial: every codimension-two
...
$\Sigma_2$-coinvariant of the arity-two bar coefficient $r(z)$;
...
obstruction tower $\Theta_\cA^{\leq r}$ records the higher-arity
...
function of these classes on the uniform-weight scalar lane is
...
\item \emph{Hodge bundle structure.}\;
  ...
  For uniform-weight algebras \textup{(UNIFORM-WEIGHT)}, and at
  ...
  The generating function on the uniform-weight lane is
  \textup{(UNIFORM-WEIGHT)}
...
\item \emph{Homotopical \textup{(}bar-cobar inversion in
  ...
  SDR exists and the homotopy transfer theorem produces
  the transferred $A_\infty$ structure on $H^*(\barBch(\cA))$
  at each genus.

\item \emph{Koszul \textup{(}classical bar-cobar
...
the fiberwise curvature endomorphism over $\bigwedge^g \mathbb{E}$
gives the scalar obstruction class \textup{(UNIFORM-WEIGHT)}; at
genus~$1$ the same formula is
\textup{(g=1 only; ALL-WEIGHT at g=1 is unconditional)}:
\[
\mathrm{obs}_g(\cA)
= \kappa(\cA) \cdot c_g(\mathbb{E})
...
The top $\lambda$-class, paired with the appropriate $\psi$-class, evaluates to \textup{(LOCAL)}:
...
\end{equation}
where $B_{2g}$ are Bernoulli numbers~\cite{FP03}.  The integrand has degree $(2g-2)+g = 3g-2 = \dim\overline{\mathcal{M}}_{g,1}$.  At $g = 1$: $\frac{1}{2} \cdot \frac{1/6}{2} = \frac{1}{24}$; at $g = 2$: $\frac{7}{8} \cdot \frac{1/30}{24} = \frac{7}{5760}$ \textup{(LOCAL)}.
\end{theorem}
...
$\mathrm{obs}_g(\cA) = \kappa(\cA) \cdot \lambda_g$
\textup{(UNIFORM-WEIGHT)}; at genus~$1$ the same scalar formula is
\textup{(g=1 only; ALL-WEIGHT at g=1 is unconditional)}.
...
The genus-$g$ obstruction is computed by \textup{(LOCAL)}:
\begin{equation}
...
(We use the closed-string convention $\hbar^{2g-2}$; the Feynman-diagram convention in \S\ref{rem:feynman_genus_g} uses $\hbar^{g-1}$, corresponding to $\hbar_{\text{there}} = \hbar_{\text{here}}^2$.)
...
genus-$g$ obstruction is \textup{(UNIFORM-WEIGHT)}:
\begin{equation}
\text{obs}_g^{\mathcal{H}} = \kappa \cdot \lambda_g \in H^{2g}(\overline{\mathcal{M}}_g, \mathbb{C})
\qquad \textup{(UNIFORM-WEIGHT)}
\end{equation}
where $\lambda_g = c_g(\mathbb{E})$ is the top Chern class of the Hodge bundle \textup{(LOCAL)}.
...
\item $g=2$: $\text{obs}_2 = \kappa \cdot \lambda_2 = \kappa \cdot c_2(\mathbb{E})$ \textup{(UNIFORM-WEIGHT)}
\item $g \geq 3$: $\text{obs}_g = \kappa \cdot \lambda_g$ \textup{(UNIFORM-WEIGHT)}
\end{itemize}
\end{theorem}
...
formula~\cite{Quillen85}.  The fiberwise result is \textup{(LOCAL)}:
\begin{equation}
...
At genus~$g \geq 2$: $\lambda_g = c_g(\mathbb{E})$ has degree~$g < 3g{-}3 = \dim_{\mathbb{C}}\overline{\mathcal{M}}_g$, so the obstruction class $\mathrm{obs}_g = \kappa \cdot \lambda_g$ \textup{(UNIFORM-WEIGHT)} is a non-trivial class in $H^{2g}(\overline{\mathcal{M}}_g)$ but is \emph{not} a top-degree class.  Its numerical evaluation requires pairing with a complementary tautological class of degree~$2g{-}3$; the relevant intersection numbers are computable from the Faber conjectures (proved by Faber--Pandharipande) involving Bernoulli numbers.

\emph{Step 6: Obstruction class.}

Therefore \textup{(UNIFORM-WEIGHT)}:
...
the genus-$g$ obstruction is \textup{(UNIFORM-WEIGHT)}:
\begin{equation}
...
For specific Lie algebras \textup{(UNIFORM-WEIGHT)}:
\begin{align}
...
$\sum_a (J^a, J_a) = \dim(\mathfrak{g})$, one obtains \textup{(UNIFORM-WEIGHT)}:
\begin{align}
...
\emph{Critical level}: At $k = -h^\vee$, $\text{obs}_g = 0$ for all $g$ \textup{(UNIFORM-WEIGHT)}. This is consistent
...
$\text{obs}_g = k \cdot \lambda_g$ \textup{(UNIFORM-WEIGHT)}, in agreement with Theorem~\ref{thm:heisenberg-obs}.
...
The obstruction is \textup{(LOCAL)}:
...
The obstruction formula exhibits level-rank duality explicitly \textup{(UNIFORM-WEIGHT)}. For $\mathfrak{sl}_N$
...
the genus-$g$ obstruction satisfies \textup{(UNIFORM-WEIGHT)}:
...
$\operatorname{obs}_g = \kappa \cdot \lambda_g$ \textup{(UNIFORM-WEIGHT)} where
...
and it suffices to show $\lambda_g^2 = 0$ \textup{(LOCAL)} in
...
\qquad \textup{(LOCAL)},
\]
whence $\lambda_g^2 = 0$ \textup{(LOCAL)} in
...
established $\lambda_g^2 = 0$ \textup{(LOCAL)} by the weaker observation that
$\mathrm{CH}^{2g}(\overline{\mathcal{M}}_g) = 0$ when
$2g > 3g - 3$, i.e., $g < 3$.
Mumford's relation is strictly stronger: it proves
$\lambda_g^2 = 0$ \textup{(LOCAL)} for \emph{all} genera, including $g = 3$
...
by Mumford's relation $\lambda_g^2 = 0$ \textup{(LOCAL)}.
...
\textup{(UNIFORM-WEIGHT)} satisfy the
...
\begin{proof}
Since $\operatorname{obs}_g = \kappa \cdot \lambda_g$
\textup{(UNIFORM-WEIGHT)} by
...
$\lambda_2 - \lambda_1^2 + \lambda_2 = 0$ \textup{(LOCAL)}, i.e.,
$\lambda_1^2 = 2\lambda_2$ \textup{(LOCAL)}.  For the degree-$(2g-2)$ relation with
...
\begin{table}[ht]
\centering
\caption{Genus-$g$ obstruction classes \textup{(UNIFORM-WEIGHT)}.
  For single-generator families, $\mathrm{obs}_g = \kappa \cdot \lambda_g$
...
curvature, such that \textup{(UNIFORM-WEIGHT)}
...
lollipop) first appear, and mixed-channel
...
For affine Kac--Moody $\widehat{\mathfrak{g}}_k$, the obstruction coefficient is periodic under level shift: $\kappa(\widehat{\mathfrak{g}}_{k+n}) = \kappa(\widehat{\mathfrak{g}}_k) + n \cdot d/(2h^\vee)$ for $n \in \mathbb{Z}$.  In particular, the free energy $F_g$ is a linear function of $k$ with slope $\lambda_g^{FP} \cdot d/(2h^\vee)$ \textup{(UNIFORM-WEIGHT)}.  For $\mathcal{W}$-algebras, $\kappa$ is a rational function of $k$ (through its dependence on the DS central charge), and the periodicity statement becomes: $\kappa(\mathcal{W}^{k+n}(\mathfrak{g})) - \kappa(\mathcal{W}^k(\mathfrak{g}))$ is a rational function of $k$ with poles only at $k = -h^\vee$.
\end{corollary}
...
$\mathrm{obs}_g = \kappa \cdot \lambda_g$ \textup{(UNIFORM-WEIGHT)} and the free-energy
...
$\mathrm{obs}_g(\mathcal{A}) = 0$ and $F_g(\mathcal{A}) = 0$
\textup{(UNIFORM-WEIGHT)} for all
$g \geq 1$.  This scalar vanishing does not by itself determine the
higher-arity shadow obstruction tower or the full Maurer--Cartan element
...
$\kappa(\mathcal{A}) \cdot \lambda_g$ \textup{(UNIFORM-WEIGHT)}.  The image of the group
...
scalar contribution to the single class $\kappa \cdot \lambda_g$
\textup{(UNIFORM-WEIGHT)}, so
...
\prod_i\psi_i^{h_i}$ \textup{(LOCAL)} gives products in
...
\textup{(}i.e., $\mathrm{obs}_{g+1}(\cA) = 0$
\textup{(UNIFORM-WEIGHT)} in the scalar
obstruction group\textup{)} if and only if $\kappa(\cA) = 0$.
Equivalently:
\begin{enumerate}[label=\textup{(\roman*)}]
\item If $\kappa \neq 0$, the scalar obstruction classes
  $\mathrm{obs}_g(\cA)$ \textup{(UNIFORM-WEIGHT)} are nonzero for every genus $g \geq 1$;
...
Maurer--Cartan element $\Theta_{\cA}$ or the higher-arity shadow obstruction tower
to vanish.
\end{remark}
...
\textup{(UNIFORM-WEIGHT)} is therefore a tautological
...
function is a Hodge integral \textup{(UNIFORM-WEIGHT)}:
...
\textup{(UNIFORM-WEIGHT)} as the GRR
