...
uniform-weight algebras (UNIFORM-WEIGHT scope),
where $\lambda_g = c_g(\mathbb{E})$ is
...
(Theorem~\ref{thm:family-index}). Under the wave-14 universal
conductor functor (Chapter~\ref{chap:universal-conductor}) the
scalar prefactor admits the BRST-ghost reading
$\kappa(\cA) = K(\cA) = -c_{\mathrm{ghost}}(\BRST(\cA))$ on the
Koszul-self-dual locus, so the uniform-weight identity
...
Theorem~\ref{thm:koszul-reflection} promotes the uniform-weight
identity from a per-family check to a Verdier-symmetric statement
about the bar/cobar pair $(\cA, \cA^!)$. (genus universality
beyond $g=1$) is healed in the present chapter via the
clutching-uniqueness statement of Chapter~\ref{chap:clutching-uniqueness-socle}
combined with the scalar-channel linearity of Chern--Weil
