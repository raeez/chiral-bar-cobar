...
H^*(\overline{\mathcal{M}}_g)$ \textup{(UNIFORM-WEIGHT)} for
...
\item $g=2$: $\text{obs}_2 = \kappa \cdot \lambda_2 = \kappa \cdot c_2(\mathbb{E})$ \textup{(UNIFORM-WEIGHT)} \item $g \geq 3$: $\text{obs}_g = \kappa \cdot \lambda_g$ \textup{(UNIFORM-WEIGHT)} \end{itemize}
\end{theorem}
...
\textbf{Step 3: Chern--Weil identification.}
The Hodge bundle $\mathbb{E}$ carries the Arakelov
metric~\cite{Faltings84,Arakelov74}, a canonical Hermitian metric
whose restriction to each fiber $H^0(\Sigma_g, \Omega^1)$ is the
$L^2$-inner product
$\langle \omega, \eta \rangle
= \frac{i}{2}\int_{\Sigma_g}
\omega \wedge \overline{\eta}$.
The curvature of the Chern connection of
$(\mathbb{E}, h_{\mathrm{Ar}})$ is a $(1,1)$-form
$\Theta_{\mathbb{E}} \in
\Omega^{1,1}(\overline{\cM}_g, \mathrm{End}(\mathbb{E}))$.
Its restriction to the fiber direction is exactly the
Arakelov $(1,1)$-form $\omega_g^{\mathrm{Ar}}$ of
Proposition~\ref{prop:genus-g-curvature-package}\textup{(ii)}:
...
\omega_\alpha \wedge \overline{\omega}_\beta.
\end{equation}
This is the Arakelov--Faltings theorem
\cite{Fal84}: the curvature of the Hodge bundle
in the $L^2$ metric equals the fiber integral of the
Arakelov form.
The Chern--Weil homomorphism then gives
\[
c_g(\mathbb{E})
\;=\;
\det\!\Bigl(\frac{i}{2\pi}\Theta_{\mathbb{E}}\Bigr)
...
and applying the degree-$g$ Chern polynomial to the geometric
factor~$\mathbb{E}$. Step~1 identifies the geometric factor.
Step~2 computes $c_g(\mathbb{E}) = \lambda_g$ via GRR.
Step~3 identifies the chain-level Arakelov curvature
(from the quantum Arnold relations,
Theorem~\ref{thm:quantum-arnold-relations}) with the curvature
of the Hodge bundle in the Arakelov metric, so the Chern--Weil
representative of $\lambda_g$ is exactly the form computed by the
bar complex. Therefore
\[
\mathrm{obs}_g(\cA)
\;=\;
\kappa(\cA) \cdot \lambda_g
...
direct: the bar curvature on the fiber IS the Arakelov curvature
of the Hodge bundle
\textup{(}the Arakelov--Faltings
theorem~\cite{Fal84}\textup{)}, and Chern--Weil applied
to this curvature produces $c_g(\mathbb{E}) = \lambda_g$.
The only external input is Mumford's computation of the Chern
classes of the Hodge bundle via GRR~\cite{Mumford83}, which is
classical.
\end{remark}
