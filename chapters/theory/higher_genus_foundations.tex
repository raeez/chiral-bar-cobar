...
genus~$g \geq 1$ the bar differential acquires curvature
$d^2 = \kappa(\cA) \cdot \omega_g$, the coalgebraic shadow
of the open-sector cyclic trace composed with clutching, and
the MC element acquires modular corrections recorded by the
shadow obstruction tower $\Theta_\cA^{\leq r}$.  This chapter
constructs the genus-$g$ bar complex by passing from trees to
...
the present chapter constructs the foundations on which that tower
rests.
...
$\textup{(v)} \subsetneq \textup{(iv)} \subsetneq
...
systems\textup{)} satisfies all five levels.
\end{theorem}
...
(hence level~(iv)), Koszulness (the Zhu algebra is
finite-dimensional, ensuring PBW collapse), finite-dimensional
weight spaces (hence level~(ii)), and $D^2=0$ (which is
universal).
...
\item[\textup{Level~4.}] \emph{Modular functor}.
  ...
  (\S\ref{rem:c2-cofiniteness-koszul-interaction}):
