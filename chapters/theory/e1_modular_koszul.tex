...
kernel is a $\mathrm{GRT}_1$-torsor at each arity $n \ge 3$
(at arity $2$ the symmetric group is $\Sigma_2$ and the kernel
...
ordered chiral homology on the formal disk~$D$: the arity-$n$
...
\otimes H^{2}(\overline{\cM}_g)$ at arity~$2$), which upon
...
\;=\; 1 + t + t^2.
\end{equation}
Arity by arity: $\mathrm{HH}^0 = \bC$ (the centre consists
...
The KZ connection at arity~$2$ is
...
\begin{example}[Ordered chiral homology of $Y_z^\hbar(\mathfrak{sl}_2)$
...
\smallskip
\noindent
\textbf{KZB connection.} At arity $n$, with
...
\smallskip
\noindent
\textbf{Arity $0$: the center.} At arity~$0$ there is no
configuration space: the ordered chiral homology reduces to
the center $Z(Y_z^\hbar(\mathfrak{sl}_2))
...
Koszul), the bar cohomology at arity~$1$ is
$s^{-1}\mathfrak{sl}_2$ in degree~$1$. With
$\dim H^*(E_\tau) = 4$:
\begin{equation}\label{eq:v1-ell-arity1}
  H^*\bigl(\cC_1^{\mathrm{ord}}(E_\tau,
  Y_z^\hbar(\mathfrak{sl}_2))\bigr)
...
The KZB monodromy at arity~$2$ has three generators:
\begin{enumerate}[label=\textup{(\roman*)}]
...
ordered complex at arity~$n$ has
