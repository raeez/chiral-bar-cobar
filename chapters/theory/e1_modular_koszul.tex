...
Write the tensor (ordered) cooperad underlying the ribbon modular
...
verification of Theorem~\ref{conj:FG-shadow}: at finite arity, the
...
differ by the scalar factor $k/(k{+}h^\vee)$, the usual Sugawara
rescaling that converts the level-$k$ current bilinear form to the
Virasoro stress-tensor normalisation on~$\cA^!$.
...
The five main theorems of Volume~I (existence~(A), inversion~(B),
complementarity~(C), leading coefficient~(D), and Hochschild
ring~(H)) admit an ordered parallel that is not a
specialization but a \emph{refinement}: each symmetric theorem is
recovered from its ordered counterpart by taking
$\Sigma_n$-coinvariants.  The ordered theorems carry strictly more
information.  At arity~$2$, the scalar curvature $\kappa(\cA)$ is
promoted to the spectral $r$-matrix $r(z)$; at arity~$3$, the
cubic shadow $\mathfrak{C}(\cA)$ is promoted to the
Drinfeld associator $\Phi_{\mathrm{KZ}}$; at all genera, the
genus tower acquires an ordered refinement governed by the ribbon
modular operad $\cM_\Ass$ in place of the commutative modular
operad $\cM_\Com$.

The five theorems below are therefore not appendices to the
symmetric theory.  They are the \emph{native language} of the
ordered bar complex, and the symmetric theorems are their shadows.
...
\textup{(}$\kappa + \kappa^! = \rho \cdot K$ for $W$-algebras,
...
ribbon modular operad.  A genuine 't~Hooft $1/N$ expansion requires
one further datum: a cyclic trace on the open-color algebra that
endows each Feynman graph with a canonical ribbon thickening.  This
section identifies the precise algebraic condition and its
consequences for the large-$N$ expansion.
