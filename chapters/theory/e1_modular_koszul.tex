...
The scalar curvature $\kappa(\cA)$ is the leading invariant of the
bar complex, but it is a single number: the $\Sigma_2$-coinvariant of
a meromorphic function $r(z)$ that encodes the full spectral
scattering of the algebra.  At arity~$3$, the cubic shadow
$\mathfrak{C}(\cA)$ is the $\Sigma_3$-coinvariant of the Drinfeld
associator.  The information lost in taking coinvariants is
precisely the ordered structure of the bar complex: the associative
factorization pattern governed by Stasheff associahedra rather than
symmetric groups.

Replacing the commutative modular operad $\cM_{\Com}$ by the ribbon
modular operad $\cM_{\Ass}$ and its Feynman transform $F\!\Ass$
recovers this ordered structure.  The resulting $E_1$ modular Koszul
theory is not a variant of the commutative theory but its
refinement: every commutative invariant is the
$\Sigma_n$-coinvariant of an ordered invariant that carries strictly
more information.  The five main theorems A--H admit ordered
counterparts $\mathrm{A}^{E_1}$--$\mathrm{H}^{E_1}$ in which the
spectral $R$-matrix, the KZ associator, and the ordered genus tower
are the native objects, and the $E_\infty$ theory is their shadow.
...
The commutative modular operad $\cM_{\Com}$ forgets the ordering of
collision points.  To retain this ordering, replace $\cM_{\Com}$ by
the ribbon modular operad $\cM_{\Ass}$, whose combinatorics is
governed by cyclic orderings at vertices rather than symmetric
groups.
...
Write the tensor (ordered) coalgebra underlying the ribbon modular
operad as $T^c$: at arity~$n$ its component is the ordered tensor
coalgebra on the $n$ external legs, with coproduct given by
deconcatenation along ribbon-graph edges.
\begin{equation}
...
The genus-$0$ theory of the preceding sections promoted each
$E_\infty$ shadow invariant to an $E_1$ shadow at the same arity:
the scalar $\kappa$ to the spectral $r$-matrix, the cubic shadow
$\mathfrak{C}$ to the Drinfeld associator.  The genus tower,
however, remained symmetric: the genus-$g$ deformation of the bar
complex used the commutative modular operad $\cM_\Com$ and its
Feynman transform.  The genus-$g$ ordered bar complex on
$\cM_\Ass$ carries strictly more structure: the $R$-matrix acts on
internal edges, the associator controls vertex reordering, and
the genus-$g$ partition function acquires a ribbon surface
decomposition invisible to $\Sigma_n$-coinvariants.
