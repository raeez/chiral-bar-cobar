...
bar-intrinsic MC element $\Theta_\cA$, the shadow obstruction tower,
...
Virasoro
& $(c/2)/z^3 + 2T/z$% RECTIFICATION-FLAG: AP19 correction from c/(2z); the full
% collision residue r^{coll}(z) = (c/2)/z^3 + 2T/z per rem:three-r-matrices
% line 524. If this table is intended to show only the scalar shadow
% r^{sc}(z), revert to c/(2z) and document the projection convention.
& non-trivial
& non-trivial \\
$Y_\hbar(\fg)$
& $r_{\mathrm{YB}}(z)$
...
which variant is meant.  In the $E_1$ shadow obstruction tower
...
\medskip
\noindent\textbf{Comparison with Theorem~A\@.}
Theorem~A says that the symmetric bar complex $\Barch(\cA)$ is an
$F\!\Com$-coalgebra --- a factorization coalgebra on the unordered
configuration space.  Theorem~$\mathrm{A}^{E_1}$ says that
${\Barch}^{\mathrm{ord}}(\cA)$ is an $F\!\Ass$-coalgebra --- an
$E_1$-factorization coalgebra on the \emph{ordered} configuration
space, with the Stasheff associahedra $K_n$ governing the
combinatorics in place of the permutohedra.  Physically, Theorem~A
encodes the holomorphic factorization of bulk observables;
Theorem~$\mathrm{A}^{E_1}$ encodes the \emph{associative}
factorization of line operators, retaining the spectral parameter.
...
\medskip
\noindent\textbf{Comparison with Theorem~B\@.}
Theorem~B says $\Omega(\Barch(\cA)) \simeq \cA$: the symmetric
bar-cobar round-trip is invertible on the Koszul locus.
Theorem~$\mathrm{B}^{E_1}$ says
$\Omega^{\mathrm{ord}}({\Barch}^{\mathrm{ord}}(\cA)) \simeq \cA$:
the ordered round-trip is invertible on the $E_1$ Koszul locus.
The ordered Koszul dual $\cA^{!,E_1} = \cA^!_{\mathrm{line}}$
lives on the \emph{line} $\mathbb{R}_t$, and its structure
as an $E_1$-chiral algebra encodes the associative monodromy
of the Drinfeld--Kohno connection.  Physically, Theorem~B
reconstructs the chiral algebra from its symmetric bar data;
Theorem~$\mathrm{B}^{E_1}$ reconstructs it from the
\emph{ordered} bar data, which retains the spectral $R$-matrix
and Drinfeld associator as additional invariants.
\end{theorem}
...
\medskip
\noindent\textbf{Comparison with Theorem~C\@.}
Theorem~C says that the genus-$g$ partition functions of $\cA$ and
$\cA^!$ are complementary Lagrangians:
$Q_g(\cA) + Q_g(\cA^!) = H^*(\overline{\cM}_{g,n},
Z(\cA))$, where $Z(\cA)$ is the ordinary center.
Theorem~$\mathrm{C}^{E_1}$ replaces the center by the $E_1$ center
$Z^{E_1}(\cA)$ --- the space of $R$-matrix-equivariant elements,
which is the Drinfeld center of the monoidal category of
$\cA$-bimodules --- and the moduli space
$\overline{\cM}_{g,n}$ by its ribbon refinement
$\overline{\cM}_{g,n}^{\,\mathrm{rib}}$.
The curvature complementarity identity
\textup{(}$\kappa + \kappa^! = 0$ for KM/free fields;
$\kappa + \kappa^! = \rho \cdot K$ for $W$-algebras,
Theorem~\textup{\ref{thm:modular-characteristic})} is
\emph{the same} in both theorems; the ordered refinement
does not alter the scalar complementarity, but enriches
it with an $R$-matrix-equivariance condition on the ambient
cohomology.
Physically, the symmetric complementarity governs the genus
expansion of the bulk partition function; the ordered
complementarity governs the genus expansion of the
\emph{line-operator partition function}.
\end{theorem}
...
\medskip
\noindent\textbf{Comparison with Theorem~D\@.}
Theorem~D says that the leading invariant of the bar complex is
the scalar curvature $\kappa(\cA) = c_2(\cA)$, a single number
extracted as the arity-$2$ shadow.
Theorem~$\mathrm{D}^{E_1}$ promotes this to the
\emph{collision residue} $r(z)$: a meromorphic function of the
spectral parameter whose $\Sigma_2$-coinvariant recovers
$\kappa(\cA)$, together with the full genus-refined series
$R^{E_1,\mathrm{bin}}(z;\hbar)$.  The passage from a scalar to a
spectral function is the passage from the bulk curvature to the
line-operator $R$-matrix: Theorem~D records the total charge,
while Theorem~$\mathrm{D}^{E_1}$ records the
charge-resolved scattering amplitude.
Physically, $\kappa(\cA)$ governs the genus tower of the
closed-string partition function; $r(z)$ governs the
spectral-parameter-dependent scattering of line operators.
\end{theorem}
...
\medskip
\noindent\textbf{Comparison with Theorem~H\@.}
Theorem~H says that the chiral Hochschild cohomology
$\HH^{\mathrm{ch}}_\bullet(\cA, \cA)$ computes the derived center
$Z^{\mathrm{der}}_{\mathrm{ch}}(\cA)$: the deformation ring of
the symmetric bar complex.  Theorem~$\mathrm{H}^{E_1}$ says that
the \emph{ordered} Hochschild homology
$\HH^{\mathrm{ch},(g)}_\bullet(\cA, M)$ is computed by the
ordered bar complex, and the resulting invariant is the
\emph{ordered center} --- the $R$-matrix-equivariant Hochschild
complex, whose $\Sigma_n$-coinvariant recovers the symmetric
Hochschild complex of Theorem~H\@.
Physically, Theorem~H identifies the bulk algebra (the observables
of the 3d HT theory); Theorem~$\mathrm{H}^{E_1}$ identifies the
\emph{line-operator algebra}, computed as the ordered Hochschild
homology with ordered bar differential.
\end{theorem}
