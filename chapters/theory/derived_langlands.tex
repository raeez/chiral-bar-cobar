...
to the full finite-dimensional quantum-group category is a separate
non-semisimple problem.

The governing question is: how far does the proved critical-level
bar/oper identification extend toward admissible-level
Kazhdan--Lusztig equivalence, and exactly where the periodic-CDG
infrastructure remains conjectural.

\begin{remark}[Semantic levels; Convention~\ref{conv:hms-levels}]
\label{rem:derived-langlands-hms}
This chapter operates primarily at \textbf{H-level}: derived stacks, oper spaces, and the Kazhdan--Lusztig equivalence are $\infty$-categorical programme statements.  The bar complex at critical level provides \textbf{M-level} partial constructions (explicit chain complexes computing oper functions).  The Feigin--Frenkel isomorphism and KL equivalence are targets, not yet fully realized within bar-cobar duality.
\end{remark}
...
\begin{multline}\label{eq:whitehead-decomposition}
E_1^{p,q}
= H^q\bigl(\fg;\, S^p(\fg[t^{-1}]t^{-1})\bigr)
\cong
S^p(\fg[t^{-1}]t^{-1})^{\fg}
...
Conjecture~\ref{thm:modular-periodicity-wzw}.
