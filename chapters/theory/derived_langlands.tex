...
The bar complex of an affine Kac--Moody algebra depends on a
continuous parameter: the level~$k$.  At generic level, the curvature
$m_0 = \kappa(\widehat{\fg}_k)$ is nonzero and the bar complex is a
curved dg-coalgebra.  At the critical level $k = -h^\vee$, the
curvature vanishes: the bar complex becomes an honest chain complex,
and its cohomology acquires geometric meaning.
Theorem~\ref{thm:oper-bar-dl} proves
\[
...
\]
identifying the bar cohomology with the algebra of differential forms
on the space of $\fg^\vee$-opers.  This is an algebraic bridge to the
oper side of the Frenkel--Gaitsgory geometric Langlands programme; it
does not by itself prove the full categorical localization.
...
% AP1: formula from CLAUDE.md C3 / landscape_census.tex:1102; k=0 -> dim(g)/2; k=-h^v -> 0 verified
(Corollary~\ref{cor:critical-level-universality}).
This vanishing has two consequences for the bar complex that must be
carefully distinguished.

\emph{First consequence: uncurving.}  Since $\kappa = 0$, the genus-$0$
...
obstruction $d_{\mathrm{fib}}^2 = \kappa \cdot \omega_g$ likewise
vanishes.

\emph{Second consequence: the $r$-matrix survives.}  The classical
$r$-matrix is $r^{\mathrm{KM}}(z) = k\,\Omega/z$
% AP126: level prefix mandatory; k=0 -> r=0 verified
(Theorem~\ref{thm:modular-characteristic}), which at critical level
specializes to $r(z) = -h^\vee \cdot \Omega/z$; this is
\emph{nonzero}.  The vanishing $\kappa = 0$ means only that the
$\Sigma_n$-coinvariant projection $\mathrm{av}(r(z)) = 0$ vanishes
(the averaging map kills $r$ without $r$ itself being zero).  In
particular, the arity-$2$ component
$\Theta_{\cA}^{(0,2)} = r(z) = -h^\vee \cdot \Omega/z \neq 0$
persists.

This is the key structural distinction between the critical level
$k = -h^\vee$ and the abelian limit $k = 0$
(Remark~\ref{rem:two-critical-points}).  At $k = 0$: both
$\kappa = 0$ and $r(z) = 0 \cdot \Omega/z = 0$ (the $r$-matrix
vanishes identically), so $\Theta_{\cA}$ reduces to the purely
higher-arity tail.  At $k = -h^\vee$: $\kappa = 0$ but
$r(z) \neq 0$, and $\Theta_{\cA}$ retains its full
structure at all arities $\geq 2$.

For $\fg = \mathfrak{sl}_2$ ($h^\vee = 2$,
$\dim(\fg) = 3$,
$\kappa(V_k(\mathfrak{sl}_2)) = 3(k+2)/4$),
% AP1: census check; k=0 -> 3/2; k=-2 -> 0 verified
...
The $r$-matrix $r(z) = -2\,\Omega/z$ is the arity-$2$ shadow of
...
relation at each conformal weight) constrains the higher-arity
...
% AP1: from CLAUDE.md C3; k=0 -> dim(g)/2; k=-h^v -> 0 verified
...
$\kappa$ uncurves the bar complex (making the dg coalgebra
...
\item The shadow-tower projections
  ...
  \operatorname{Vect},
  \]
  with the arity-$2$ shadow satisfying
  $\operatorname{av}\bigl(r(z)\bigr) = \kappa^{\mathrm{KM}}$ and
  recovering the rank of the fiber functor at arity~$2$.
\end{enumerate}
\end{conjecture}
...
$\kappa = 0$, so the bar complex degenerates to the uncurved case; this
...
arity-$2$ shadow and hence with the trivial fiber functor in this
...
% AP132: augmentation ideal \bar A = ker(epsilon)
...
$\operatorname{av}^{(\leq r)}$ truncated at arity~$r$.
...
extracted at each arity specialize to the categorical dimension,
Casimir eigenvalue, and quadratic form of the corresponding
Satake representation.  At arity~$2$, the averaging identity
...
$s_+ + s_- = 1$ and $s_+ \cdot s_- = \kappa$.  For affine
...
of degree~$2$ in the arity variable~$t$, while the Hitchin
