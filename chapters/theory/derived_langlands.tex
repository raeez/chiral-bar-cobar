...
The governing question of this chapter is the mathematical role of \emph{Derived structures and the geometric Langlands correspondence}: which part of the modular bar-cobar construction or its inversion it isolates, and which boundary between construction, resolution, and shadow it clarifies.
\end{remark}
...
\label{rem:derived-langlands-hms}

\begin{remark}[Periodicity frontier]
The theorematic core of this chapter is asymmetric.  The critical-level
bar/oper identification is proved, and the manuscript isolates a
rank-$1$ periodic shadow in
...
frontier.  Even on the KL side, the ordinary semisimplified target
...
See Theorem~\ref{thm:oper-bar} for the version stated in
...
Conjecture~\ref{conj:modular-periodicity-wzw}.
...
(Theorem~\ref{thm:affine-periodicity-critical}): the periodicity follows from
...
Only rank-$1$ strict periodicity is proved in this chapter
\textup{(}Proposition~\ref{prop:sl2-periodicity-dl}\textup{)}.
For higher-rank affine algebras, strict periodicity is not established and
can fail; the active statement remains the periodic CDG conjecture
\textup{(}Conjecture~\ref{conj:periodic-cdg}\textup{)} together with
growth/profile control.
...
\section{Kazhdan--Lusztig from bar-cobar: chain-level adjunction and periodic frontier}
...
programme-level Kazhdan--Lusztig recovery on the semisimplified target
...
$\mathrm{Rep}^{\mathrm{fd}}(U_q(\fg))$ is an additional frontier,
not part of the theorematic endpoint used elsewhere in the manuscript;
the completed/coderived MC3 enlargement lies further out still.
...
Accordingly, the ordinary target of this chapter is
$\mathcal{C}(U_q(\fg))$; the coderived category enters only as the
ambient bar-side formalism for the conjectural lift, not as the
printed KL endpoint.
...
modules.  Two further passages must be kept separate.
\begin{enumerate}[label=\textup{(\alph*)}]
\item The semisimplified KL step: the passage from the derived
  category of $\widehat{\fg}_k$-modules to
  $\mathcal{C}(U_q(\fg))$ requires identifying the \emph{target} of
  the bar functor with quantum-group comodules, which in turn
  requires the periodic CDG structure of
  Conjecture~\ref{conj:periodic-cdg}.
\item The outer MC3 enlargement: any lift beyond the semisimplified
  target to the full non-semisimple quantum-group category or a
  larger completed/coderived envelope requires additional input and
  is not implied by the semisimplified KL equivalence.
\end{enumerate}
...
\item The periodic CDG structure arises from the interaction of
  ...
  $q = e^{\pi i/(k+h^\vee)}$.
\item The passage from the periodic coderived bar side to the
  semisimplified target uses Positselski's abstract Koszul duality,
  applied to the specific CDG-coalgebra produced by our bar complex,
  together with Lusztig's stable-to-tilting comparison.  Any
  non-semisimple lift is additional structure.
\end{enumerate}
...
\item At $k = -h^\vee + p/q$ (rational): the CDG structure becomes
  conjecturally periodic, and the bar complex should recover the
  semisimplified KL target.  Any non-semisimple or
  completed/coderived enlargement lies beyond this chapter's
  theorematic endpoint.
\end{enumerate}
...
\item The full derived category equivalence follows from
  Theorem~\ref{thm:oper-bar-dl}: the bar complex computes the
...
\item Any non-semisimple or completed/coderived KL lift beyond
  $\mathcal{C}(U_q(\fg))$: this is a separate outer MC3 extension and
  is not implied by the semisimplified KL step.
