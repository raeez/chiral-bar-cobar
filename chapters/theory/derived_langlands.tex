...
the dichotomy.  At generic level, the curvature $\kappa \neq 0$
obstructs the formation of cohomology: the bar complex is a curved
dg-coalgebra with $d^2 = \kappa \cdot \omega_1 \neq 0$, and no
functor from curved coalgebras to graded algebras recovers opers.
At critical level, $\kappa = 0$ is the \emph{unique} value at which
...
r-matrix $r(z) = k\Omega/z$ at $k = -2$ gives
$r(z) = -2\Omega/z \neq 0$;
% AP126: k=0 -> r=0 verified; k=-h^v -> r = -h^v * Omega/z != 0
...
$\kappa = 0$ uncurves the bar complex while the r-matrix continues to
