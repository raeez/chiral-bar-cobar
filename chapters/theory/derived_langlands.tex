...
\item The induced map on Ext groups recovers BGG multiplicities:
  ...
  \cong
  \mathrm{Coext}^n_{\widehat{\fg}_{-k-2h^\vee}}(\Phi_k(M(\lambda)),
  \Phi_k(\omega_X))$.
...
\mathrm{Rep}^{\mathrm{fd}}(U_q(\fg))$
as braided monoidal categories.  The remaining ingredient beyond
...
quantum group $u_q(\fg)$, which is equivalent to
$\mathrm{Rep}^{\mathrm{fd}}(U_q(\fg))$ by Lusztig's theorem.
\end{conjecture}
...
$\widehat{\fg}_k$-modules to the representation category of the
quantum group $U_q(\fg)$ requires identifying the \emph{target} of
the bar functor with quantum group comodules, which in turn requires
the periodic CDG structure of Conjecture~\ref{conj:periodic-cdg}.
