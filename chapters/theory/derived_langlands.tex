...
shadow are proved.  The full admissible-level periodic CDG structure
in higher rank, and its relationship to the Kazhdan--Lusztig
equivalence, remain open---they are part of the MC3
extension problem that is proved in type~A and conjectural beyond.
...
a separate outer MC3 problem (proved in type~A; conjectural beyond type~A).
...
extension problem (MC3 itself is proved in type~A; the periodicity aspect is open).  Even on the KL side, the ordinary semisimplified target
...
the completed/coderived MC3 enlargement (proved in type~A; conjectural beyond type~A) lies further out still.
...
semisimplified target is an outer MC3 problem (resolved in type~A; open beyond type~A).  Without periodicity,
...
\item Any non-semisimple or completed/coderived KL lift beyond
  $\mathcal{C}(U_q(\fg))$: this is a separate outer MC3 extension
  (proved in type~A; conjectural beyond type~A) and
