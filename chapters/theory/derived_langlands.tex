...
This is where the categorical logarithm touches geometric Langlands.
The bar complex of an affine Kac--Moody algebra, viewed as a function
of the level $k$, interpolates between two radically different
geometric worlds: at generic level, the bar complex is curved and its
deformation theory is controlled by the central extension; at the
critical level $k = -h^\vee$, the curvature vanishes, the bar complex
becomes an honest chain complex, and its cohomology acquires geometric
meaning as functions on the space of $\fg^\vee$-opers.  The
bar-cobar adjunction at critical level is therefore a shadow of the
geometric Langlands correspondence: the bar side sees the
automorphic category (modules for the chiral algebra), and the
cobar side sees the spectral category (quasi-coherent sheaves on
the oper space).  At the homotopy level, the critical-level
...
complex whose MC moduli is the oper space.
...
level computes the derived oper space, the Koszul dual coalgebra
carries the Hecke action, and admissible-level deformations recover
quantum-group categories via a periodic CDG structure on the bar
complex.
...
shadow are proved.  The MC3 categorical core (thick generation of the factorization
category) is now proved for all simple types
...
a separate outer MC3 problem (proved for all simple types; Corollary~\ref{cor:mc3-all-types}).

\begin{theorem}[Bridge theorem: bar complex at critical level $\longrightarrow$ opers $\longrightarrow$ geometric Langlands; \ClaimStatusProvedHere]
...
\item \textup{(Derived oper identification)}\quad
  The bar cohomology computes the de~Rham complex of the oper space:
  \[
  ...
  at $n \ge 2$ this gives the full derived enhancement.
\item \textup{(Derived center)}\quad
  ...
  \;\cong\;
  \mathcal{O}^{\mathrm{der}}\bigl(\mathrm{Op}_{\fg^\vee}(D)\bigr),
  \]
  and the bar complex $\barB(\widehat{\fg}_{-h^\vee})$ is a
  concrete chain-level model for this derived oper algebra.
  This is the entry point of bar-cobar duality into the geometric
  Langlands programme: the bar side computes the automorphic
  category \textup{(}$\widehat{\fg}$-modules\textup{)}, and the
  cobar side computes the spectral category \textup{(}quasi-coherent
  sheaves on the oper space\textup{)}.
\end{enumerate}
\end{theorem}
...
(Frenkel--Gaitsgory~\cite{FG06}).  Part~(iii) identifies the
derived center as the derived global sections of the structure
sheaf on the oper space; the bar complex provides the explicit
chain model by combining parts~(i) and~(ii) with the bar-Ext
identification and the Frenkel--Teleman theorem~\cite{FT06}.
\end{proof}
...
extension problem (MC3 itself is proved for all simple types; the periodicity aspect is open).  Even on the KL side, the ordinary semisimplified target
...
The Kazhdan--Lusztig equivalence $\mathrm{KL}_k \colon \mathcal{O}^{\mathrm{int}}_k(\hat{\mathfrak{g}}) \xrightarrow{\sim} \mathrm{Rep}^{\mathrm{fd}}(U_q(\mathfrak{g}))$ at $q = e^{\pi i/(k+h^\vee)}$ is the categorical shadow of MC3: thick generation of $\mathcal{O}^{\mathrm{sh}}$ by prefundamental representations underlies both the KL equivalence and the chiral bar-cobar categorical lift (Theorem~\ref{thm:mc3-type-a-resolution}).

The shared mechanism is thick generation by prefundamental representations.  In the KL setting, the category $\mathcal{O}^{\mathrm{int}}_k(\hat{\mathfrak{g}})$ is generated by Weyl modules $V_\lambda$; the quantum group $U_q(\mathfrak{g})$ acts on the same generating set via the $R$-matrix.  In the MC3 setting, the shifted category $\mathcal{O}^{\mathrm{sh}}$ is generated by prefundamental modules $L^-(\mu_j)$; the dg-shifted Yangian $Y^{\mathrm{dg}}_\cA$ acts via the spectral $R$-matrix $r(z) = \mathrm{Res}^{\mathrm{coll}}_{0,2}(\Theta^{\Eone}_\cA)$.  Both equivalences arise from the bar-cobar Quillen adjunction applied to thick subcategories generated by these representations.
\end{construction}
...
The result refines the Frenkel--Gaitsgory localization functor
$\Delta\colon D(\widehat{\fg}_{-h^\vee}\text{-}\mathrm{mod})
\to D(\mathrm{QCoh}(\mathrm{Op}))$
at the chain level; see Theorem~\ref{thm:oper-bar}.
\end{remark}
...
Proposition~\ref{prop:bar-as-localization} is not an isolated
...
Frenkel--Gaitsgory localization functor~$\Delta$ is thus the
...
but the \emph{modular} bar complex acquires genus-$1$ curvature
$\kappa(\widehat{\fg}_k) = \dim(\fg)\,(k+h^\vee)/(2h^\vee)
\neq 0$
(Theorem~\ref{thm:genus1-curvature-formula}).
The nonvanishing modular curvature has two consequences for
...
home: the critical-level bar complex \emph{is} the jet scheme of the
Kostant section, and DS reduction at any level restricts from the
...
the completed/coderived MC3 enlargement (proved for all simple types; Corollary~\ref{cor:mc3-all-types}) lies further out still.
...
semisimplified target is an outer MC3 problem (resolved for all simple types; Corollary~\ref{cor:mc3-all-types}).  Without periodicity,
...
\end{tikzcd}
\]
Our bar complex computes the top arrow $\Delta$ at the chain level
...
\item Any non-semisimple or completed/coderived KL lift beyond
  $\mathcal{C}(U_q(\fg))$: while the underlying MC3 thick generation
  is now proved for all simple types
  (Corollary~\ref{cor:mc3-all-types}), this lift requires additional
