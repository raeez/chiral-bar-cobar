...
(where $\mathfrak{z}$ is the centre), the situation is more
delicate: $H^3(\fg) \cong
H^3(\fg_1) \oplus \cdots \oplus H^3(\fg_r) \oplus
\Lambda^3 \mathfrak{z}^*$ has dimension $r + \binom{\dim \mathfrak{z}}{3}$.
At each order in $\lambda$, the obstruction space is
$r$-dimensional for semisimple~$\fg$ and larger
when $\mathfrak{z} \neq 0$.
The uniqueness argument requires matching
$r + \binom{\dim \mathfrak{z}}{3}$ independent scalars, which demands
a more refined comparison: for $\fg = \mathfrak{gl}_N$
($r = 1$, $\dim \mathfrak{z} = 1$, so $\dim H^3 = 2$), one must
separately match the $\mathfrak{sl}_N$ and abelian
deformation parameters. This is expected to hold by
compatibility with the direct-sum decomposition of
Chern--Simons theory, but has not been verified at
chain level.
\end{remark}
