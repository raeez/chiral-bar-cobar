...
$\kappa(\cA) \cdot \lambda_g$ \textup{(UNIFORM-WEIGHT)} (Theorem~\ref{thm:genus-universality})
...
\emph{deconcatenation} coproduct~$\Delta$, is a dg coalgebra over
the holomorphic--topological Swiss-cheese operad
$\mathsf{SC}^{\mathrm{ch,top}}$:
\begin{center}
\small
\renewcommand{\arraystretch}{1.15}
...
coshuffle coproduct; the Swiss-cheese coalgebra structure in this
...
$t_i$.  This is the $E_1$ coalgebra structure on the open color of
$\mathsf{SC}^{\mathrm{ch,top}}$, acting on the ordered bar
$\barB^{\mathrm{ord}}(\cA)$.
...
than $n+1$); the Swiss-cheese coalgebra structure of this theorem
...
naturally carries the full two-coloured coalgebra structure.
\end{construction}
...
\item \emph{The corrected differential
  ...
  Swiss-cheese coalgebras is therefore controlled at the chain
...
Swiss-cheese coalgebra'' picture is rescued only by passing to the
