...
$\barB^{\Sigma}(\cA)$ is recovered as the
$R$-twisted $\Sigma_n$-coinvariants of $\barB^{\mathrm{ord}}(\cA)$
...
a distinct object from $B(\cA)$ itself.
...
orientation\textup{)}.  Bar-cobar on the universal family
...
\item \textup{(PARTIAL.)}  The bar cohomology
  $H^*(\barB(\cA_E))$ of an elliptic chiral algebra~$\cA_E$
