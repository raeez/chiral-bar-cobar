...
\item \emph{$\mathsf{SC}^{\mathrm{ch,top}}\ne\Ethree$.}
 ...
 (cohomological in general; chain-level on the original complex
 via Theorem~\ref{thm:chain-level-E3-top-class-L}); for class
 $\mathbf{M}$ the chain-level statement is open on the original
 complex and \textup{Proved} only in the weight-completed
 category (Vol~II
...
\item \emph{topologization scope.}
 ...
 on the original complex). Every reference to
 $\Ethree^{\mathrm{top}}$ \emph{on the original complex} is
 scope-restricted to class~$\mathbf{L}$ at non-critical level
 unless explicitly noted; class~$\mathbf{G}$ is immediate
 (commutative); class~$\mathbf{C}$ via FMS bosonization;
 class~$\mathbf{M}$ in the weight-completed category only.
...
Kac--Moody at non-critical level, the chain-level $\Ethree$ on the
original BRST complex is proved by gauge rectification of the
Sugawara contraction together with finite shadow depth
...
\ClaimStatusProvedElsewhere{} \cite{}]
...
\begin{enumerate}[label=\textup{(\roman*)}]
\item \emph{Closed color $=$ $3$d HT bulk observables (T18(iii)).}
 The closed-sector algebra
...
\item \emph{Centre $=$ interior chiral derived centre (T18(i)).} The
...
\begin{theorem}[$E_\infty$-Topologization via iterated Sugawara]\label{thm:e-infinity-topologization}
\ClaimStatusProvedHere
\index{topologization theorem!E_infty ladder}
Let $\cA$ be a chiral algebra equipped with a \emph{higher-spin stress tower} of depth $N \in \{2, 3, \ldots, \infty\}$: a graded family $\mathcal{T}_\bullet(\cA) = \{T^{(n)}(z) \in \cA \mid 2 \le n \le N+1\}$ with $L_0 T^{(n)} = n\,T^{(n)}$, such that
\begin{enumerate}[label=\textup{(\roman*)}]
\item each $T^{(n)}$ is \emph{inner}: expressible as a normally ordered Sugawara-type polynomial in the generators of $\cA$ at a non-critical level;
\item the modes satisfy the $\mathcal{W}_{1+\infty}$-type bracket relations truncated at depth~$N$;
\item each $T^{(n)}$ admits a BRST primitive $G^{(n)}$ in the CFG 3d holomorphic-topological BV complex, giving $T^{(n)} = [Q_{\mathrm{tot}}, G^{(n)}]$ on cohomology.
\end{enumerate}
Then the chiral derived center $Z^{\mathrm{der}}_{\mathrm{ch}}(\cA)$ carries, for each $k \le N$, an $\Ethree^{\mathrm{top}}$-\,\dots-$E_{k+2}^{\mathrm{top}}$-algebra structure, compatible with the forgetful tower $E_{k+2}^{\mathrm{top}} \twoheadrightarrow E_{k+1}^{\mathrm{top}}$ induced by truncating $\mathcal{T}_\bullet$ at depth $k$. In the $N = \infty$ limit
\[
 Z^{\mathrm{der}}_{\mathrm{ch}}(\cA) \;\in\; E_\infty^{\mathrm{top}}\text{-}\mathrm{Alg} \;=\; \varprojlim_k E_{k+2}^{\mathrm{top}}\text{-}\mathrm{Alg}.
\]
The climb $\Etwo^{\mathrm{ch}} \to \Ethree^{\mathrm{top}} \to E_4^{\mathrm{top}} \to \cdots \to E_\infty^{\mathrm{top}}$ is driven by the iterated Sugawara filtration $F_n = \mathrm{span}\{T^{(2)}, \ldots, T^{(n)}\}$; each inclusion $F_n \hookrightarrow F_{n+1}$ purchases exactly one $E_1^{\mathrm{top}}$ factor.
\end{theorem}

\begin{proof}
The $n$-th Sugawara identity $T^{(n)}(z) = [Q_{\mathrm{tot}}^{(n)}, G^{(n)}(z)]$ in $H^\bullet(\mathcal{A}^{\mathrm{BV}}_{3d}, Q_{\mathrm{tot}})$ trivializes the $(n{-}1)$-st jet of holomorphic translation on the formal disk. Spin $n=2$ is Theorem~\ref{thm:topologization} with $G^{(2)} = b(z)$ the Faddeev--Popov antighost and $T^{(2)} = T_{\mathrm{Sug}}$. Spin $n \ge 3$: $G^{(n)}(z)$ is the Drinfeld--Sokolov higher-spin improvement antighost obtained by integrating the order-$(n{-}1)$ descent current of $Q_{\mathrm{DS}}$ against the $n$-th Miura row; $T^{(n)} = T^{(n)}_{\mathrm{imp}} + [Q_{\mathrm{tot}}, G^{(n)}]$ with $T^{(n)}_{\mathrm{imp}}$ the Kac--Roan--Wakimoto improvement at spin~$n$. For non-principal nilpotent $f$, the antighost $G^{(n)}_f$ carries fractional conformal weight determined by the $\mathfrak{sl}_2$-triple grading of $f$, and the iterated Sugawara is recovered by fractional-weight ghost integration on the branched cover $\widetilde{X} \to X$ where Dynkin weights integralize; descent to $X$ produces the non-Cartan improvement term while preserving $Q_{\mathrm{tot}}$-exactness. The antighosts $G^{(n_i)}$ BRST-commute: their classical Poisson shadows vanish by higher-spin $\mathcal{W}$-commutativity on the classical limit. Dunn additivity on $Z^{\mathrm{der}}_{\mathrm{ch}}(\cA)$ assembles the $k$ independent $E_1^{\mathrm{top}}$ factors:
\[
 E_{k+2}^{\mathrm{top}} \;=\; \underbrace{\Etwo^{\mathrm{chiral}}}_{\text{Deligne on }Z^{\mathrm{der}}_{\mathrm{ch}}} \;\otimes_{\mathrm{Dunn}}\; \underbrace{E_1^{\mathrm{top}}(T^{(n_1)})}_{\text{holomorphic }\partial_z} \;\otimes_{\mathrm{Dunn}}\; \cdots \;\otimes_{\mathrm{Dunn}}\; \underbrace{E_1^{\mathrm{top}}(T^{(n_k)})}_{\text{transverse }\partial_z^{n_k-1}}.
\]
The $N=\infty$ limit is the inverse limit over stress-tower depth; each depth truncation is a strict Dunn factor, so the inverse limit assembles into $E_\infty^{\mathrm{top}}$.
\end{proof}

\begin{corollary}[$E_n$ ladder specializations]\label{cor:e-ladder-vir-wn-winfty}
\ClaimStatusProvedHere
\index{topologization theorem!ladder specializations}
Under the hypotheses of Theorem~\ref{thm:e-infinity-topologization}:
\begin{enumerate}[label=\textup{(\roman*)}]
\item \textup{Virasoro / affine Kac--Moody} ($N = 2$, only $T_{\mathrm{Sug}}$ present): $Z^{\mathrm{der}}_{\mathrm{ch}}(V_k(\fg)) \in \Ethree^{\mathrm{top}}\text{-}\mathrm{Alg}$, recovering Theorem~\ref{thm:topologization} as the first rung.
\item \textup{$\mathcal{W}_N$-algebra} (stress tower $T, W^{(3)}, \ldots, W^{(N)}$): $Z^{\mathrm{der}}_{\mathrm{ch}}(\mathcal{W}_N(\fg, f_{\mathrm{prin}})) \in E_{N+1}^{\mathrm{top}}\text{-}\mathrm{Alg}$, with each higher-spin generator $W^{(n)}$ supplying its Kac--Roan--Wakimoto antighost.
\item \textup{$\mathcal{W}_\infty$} (full quasi-finite higher-spin tower): $Z^{\mathrm{der}}_{\mathrm{ch}}(\mathcal{W}_\infty) \in E_\infty^{\mathrm{top}}\text{-}\mathrm{Alg}$; the Platonic endpoint, commutative up to all higher coherences, Koszul-self-dual to the $E_\infty$-cooperad.
\end{enumerate}
\end{corollary}

\begin{remark}[Class $\mathrm{M}$ free-PVA closure via tower depth]\label{rem:class-M-free-pva-tower}
Virasoro's class-$\mathrm{M}$ quartic-pole OPE appears because the stress tower truncated at depth $N=2$ leaves the $\lambda^3$ coefficient uncancelled. In the iterated-Sugawara tower, the spin-3 entry $W^{(3)}$ contributes an improvement $[Q_{\mathrm{tot}}, G^{(3)}]$ whose classical shadow is precisely the quartic-pole residue; adding it cancels the class-$\mathrm{M}$ obstruction, promoting class $\mathrm{M} \to \mathrm{L}$ at depth $N=3$. Climbing the tower progressively restores freeness: $\mathcal{W}_\infty$ is free-PVA at all depths simultaneously. The Khan--Zeng freely-generated PVA hypothesis is therefore not an obstruction to class $\mathrm{M}$ but a feature of the first-rung truncation.
\end{remark}
...
cohomology
\textup{(}Theorem~\ref{thm:topologization}\textup{(i)}\textup{)},
the unconditional chain-level $\Ethree^{\mathrm{top}}$ on a
quasi-isomorphic model
\textup{(}Theorem~\ref{thm:topologization}\textup{(ii)}\textup{)},
and the chain-level $\Ethree^{\mathrm{top}}$ on the
\emph{original} BRST complex
\textup{(}Proposition~\ref{prop:sugawara-gauge-rectification}\textup{)}.
The last step uses gauge rectification of the Sugawara
contraction together with the finite shadow depth of
class~$L$
\textup{(}$r_{\max} = 3$\textup{)} to show that the
...
Proposition~\ref{prop:sugawara-gauge-rectification}; for class~$M$,
the original-complex $\Ethree$ remains open on the ordinary complex
and requires the coderived approach
(Conjecture~\ref{conj:coderived-e3}). The status sharpens as follows.
\begin{enumerate}[label=\textup{(\roman*)}]
...
\item \emph{Affine Kac--Moody: an explicit gauge-rectification
 ...
 For affine Kac--Moody, this gauge procedure terminates:
 class~$L$ has finite shadow depth $r_{\max} = 3$,
...
%====================================================================
% MASTER THEOREM: E_infinity-TOPOLOGIZATION VIA HIGHER-SPIN STRESS TOWER
%====================================================================
...
A chiral algebra $\cA$ carries a \emph{higher-spin stress
tower of depth} $N \in \{2,3,\ldots,\infty\}$ if there exist
holomorphic fields
\[
 T_\bullet(\cA) \;=\; \bigl\{\,T^{(n)}(z)
 \;\big|\; 2 \le n \le N+1\,\bigr\}
\]
with $T^{(n)}(z)$ of conformal weight~$n$, such that
\begin{enumerate}[label=\textup{(T\arabic*)}]
\item each $T^{(n)}$ is an inner Sugawara-type primary at
 non-critical parameters, i.e.\ a normally ordered
 polynomial of degree $n$ in the strong generators of $\cA$
 whose OPE with itself produces the rank-$n$ central
 extension $c_n$ at its top pole;
\item the collection $\{T^{(n)}\}_{n=2}^{N+1}$ satisfies the
 truncated $\cW_{1+\infty}$ brackets: writing
 $\{T^{(m)}_\lambda T^{(n)}\}$ for the $\lambda$-bracket,
 \[
 \{T^{(m)}_\lambda T^{(n)}\}
 \;=\; \sum_{p=2}^{m+n-1}
 c_{m,n,p}\!\cdot\!\lambda^{\,m+n-p}\,T^{(p)}
 \;+\; (\text{divided-power central terms}),
 \]
 with structure constants $c_{m,n,p}$ matching those of
 $\cW_{1+\infty}$ at central charge $c_{m,n,p}(c)$ truncated
 at spin $N+1$;
\item each $T^{(n)}$ admits a BRST primitive $G^{(n)}(z)$ of
 ghost number $-1$ and conformal weight $n-1$ in the
 Costello--Francis--Gwilliam $3$d HT BV complex for $\cA$,
 satisfying
 \begin{equation}\label{eq:sugawara-identity-n}
 T^{(n)}(z) \;=\; [\,Q_{\mathrm{tot}},\, G^{(n)}(z)\,]
 \qquad\text{on $Q_{\mathrm{tot}}$-cohomology.}
 \end{equation}
\end{enumerate}
The field $G^{(2)}(z) =: b(z)$ is the Faddeev--Popov
antighost; $G^{(n)}$ for $n \ge 3$ is the DS improvement
antighost at spin~$n$.
\end{definition}

\begin{theorem}[$\Einf$-topologization master theorem]
...
Let $\cA$ be a chiral algebra with higher-spin stress tower
$T_\bullet(\cA) = \{T^{(n)}\}_{n=2}^{N+1}$ of depth
$N \in \{2,\ldots,\infty\}$ in the sense of
Definition~\textup{\ref{def:higher-spin-stress-tower}}.
Then, for every $1 \le k \le N$, the derived chiral center
$Z^{\mathrm{der}}_{\mathrm{ch}}(\cA)$ carries a
\emph{depth-$k$ topological structure}
\[
 \Zder^{\mathrm{ch}}(\cA)
 \;\in\;
 ...
 \text{-mod}\bigr)
\]
obtained by iterated Dunn additivity
\begin{equation}\label{eq:ladder-dunn}
 E_{k+2}^{\mathrm{top}}
 \;\simeq\;
 E_2^{\mathrm{ch}}
 \,\otimes_{\mathrm{Dunn}}\,
 E_1^{\mathrm{top}}\!\bigl(T^{(n_1)}\bigr)
 \,\otimes_{\mathrm{Dunn}}\,
 E_1^{\mathrm{top}}\!\bigl(T^{(n_2)}\bigr)
 \,\otimes_{\mathrm{Dunn}}\,
 \cdots
 \,\otimes_{\mathrm{Dunn}}\,
 E_1^{\mathrm{top}}\!\bigl(T^{(n_k)}\bigr)
\end{equation}
for any choice of $k$ distinct spins
$2 \le n_1 < n_2 < \cdots < n_k \le N+1$, where
$E_1^{\mathrm{top}}(T^{(n_i)})$ denotes the topological
$E_1$-factor generated by the translation generated by
$T^{(n_i)}$, rendered $Q_{\mathrm{tot}}$-exact by the
antighost $G^{(n_i)}$. The resulting structure is
compatible with bar truncation in the sense that the natural
map $B\colon E_{k+2}^{\mathrm{top}}\text{-}\mathsf{Alg}
\to E_{k+1}^{\mathrm{top}}\text{-}\mathsf{CoAlg}$ takes the
depth-$k$ rung to the depth-$(k-1)$ rung. In the
$N = \infty$ limit,
\begin{equation}\label{eq:e-infty-limit}
 \Zder^{\mathrm{ch}}(\cA)
 \;\in\;
 \Einf^{\mathrm{top}}\text{-}\mathsf{Alg}
 \;=\;
 \varprojlim_{k \ge 1}\,
 E_{k+2}^{\mathrm{top}}\text{-}\mathsf{Alg}.
\end{equation}
\end{theorem}

\begin{proof}
We verify that each $T^{(n_i)}$ renders the translation it
generates $Q_{\mathrm{tot}}$-exact, extract an independent
topological $E_1$-factor from each, and then assemble the
factors by Dunn additivity.

\medskip\noindent
\textbf{Step 1: Cohomological $Q$-exactness at each spin.}
Fix a spin $n_i \in \{n_1,\ldots,n_k\}$ and let
$\partial_{z,n_i}$ denote the derivation on
$\Zder^{\mathrm{ch}}(\cA)$ generated by $T^{(n_i)}$:
\[
 \partial_{z,n_i}\cO
 \;=\;
 \oint_{|w-z|=\epsilon}
...
$T^{(n_i)} = [Q_{\mathrm{tot}}, G^{(n_i)}]$. The graded
Jacobi identity and $[Q_{\mathrm{tot}}, \cO] = 0$
(bulk observables are $Q_{\mathrm{tot}}$-closed) give
\begin{equation}\label{eq:translation-Q-exact-n}
 \partial_{z,n_i}\cO
 \;=\;
 \bigl[[Q_{\mathrm{tot}},G^{(n_i)}],\,\cO\bigr]
 \;=\;
 \bigl[Q_{\mathrm{tot}},\,[G^{(n_i)},\cO]\bigr].
\end{equation}
Hence $\partial_{z,n_i}$ is a $Q_{\mathrm{tot}}$-coboundary
on the space of $Q_{\mathrm{tot}}$-closed bulk observables.

\medskip\noindent
\textbf{Step 2: One $E_1^{\mathrm{top}}$-factor per spin.}
The derivation $\partial_{z,n_i}$ generates a one-parameter
family of automorphisms of the factorisation algebra on the
$\bC$-direction. On $Q_{\mathrm{tot}}$-cohomology this
family is trivial by~\eqref{eq:translation-Q-exact-n};
equivalently, the associated $E_1$-action is null-homotopic,
so the action factors through the \emph{topological}
$E_1$-operad rather than the holomorphic one. Write
$E_1^{\mathrm{top}}(T^{(n_i)})$ for this topological
$E_1$-factor.

The antighosts
$\{G^{(n_i)}\}_{i=1}^{k}$ BRST-commute. Indeed,
\[
 \bigl[Q_{\mathrm{tot}}, [G^{(n_i)}, G^{(n_j)}]\bigr]
 \;=\;
 [T^{(n_i)}, G^{(n_j)}] - [G^{(n_i)}, T^{(n_j)}]
 \;=\;
 \partial_{z,n_i}G^{(n_j)} - \partial_{z,n_j}G^{(n_i)},
\]
and the $\cW_{1+\infty}$ bracket (T2) is symmetric in the
two spins up to a total derivative in the cross-spin
components, so the right-hand side is a total derivative
and hence $Q_{\mathrm{tot}}$-exact. Passing to cohomology,
$[G^{(n_i)}, G^{(n_j)}] = 0$. The factors
$E_1^{\mathrm{top}}(T^{(n_i)})$ therefore \emph{commute} in
the bracket sense required by Dunn.

\medskip\noindent
\textbf{Step 3: The closed $E_2^{\mathrm{ch}}$ base rung.}
The closed colour of $\SCchtop$ on the pair
$(\Zder^{\mathrm{ch}}(\cA), \cA)$ carries an
$E_2^{\mathrm{ch}}$-structure
(Theorem~\ref{thm:operadic-center-hochschild}). The spin-2
antighost $G^{(2)} = b(z)$ is the Faddeev--Popov antighost
of Construction~\ref{constr:sugawara-antighost}; writing the
spin-2 identity explicitly,
\begin{equation}\label{eq:spin-2-explicit}
 T^{(2)}(z)
 \;=\;
 \bigl[\,Q_{\mathrm{tot}},\, b(z)\,\bigr].
\end{equation}
The local constancy argument of
Theorem~\ref{thm:topologization} applies verbatim, using
$b(z)$ in place of $G_{\mathrm{Sug}}$, to upgrade
$E_2^{\mathrm{ch}}$ to $E_2^{\mathrm{top}}$ on
$Q_{\mathrm{tot}}$-cohomology. This is the $k = 1$ case of
the ladder.

\medskip\noindent
\textbf{Step 4: Spin-$n$ rung ($3 \le n \le N+1$) via DS
improvement antighost.}
For each $n \ge 3$, write the identity explicitly as
\begin{equation}\label{eq:spin-n-explicit}
 T^{(n)}(z)
 \;=\;
 \bigl[\,Q_{\mathrm{tot}},\, G^{(n)}(z)\,\bigr],
\end{equation}
with $G^{(n)}(z)$ the DS improvement antighost at spin $n$:
\begin{equation}\label{eq:DS-improvement-antighost}
 G^{(n)}(z)
 \;=\;
 \frac{1}{n!\,(k+h^\vee)^{\,n-1}}
 \sum_{a_1,\ldots,a_n}
 d^{a_1\cdots a_n}
 {:}J^{a_1}(z)\cdots J^{a_{n-1}}(z)\,
 \bar c_{a_n}(z){:}\;,
\end{equation}
where $d^{a_1\cdots a_n}$ is the totally symmetric rank-$n$
invariant tensor on $\fg$ (the primitive Casimir of degree
$n$; for $\cW_N$ the tensor is the $n$th symmetric invariant
on $\fsl_N$; for $\cW_\infty$ the tower uses all symmetric
primitives). At spin $n=3$:
\begin{equation}\label{eq:spin-3-explicit}
 T^{(3)}(z)
 \;=\;
 \bigl[\,Q_{\mathrm{tot}},\, G^{(3)}(z)\,\bigr],
 \qquad
 G^{(3)}(z) \;=\;
 \frac{1}{6(k+h^\vee)^2}
 \sum_{a,b,c} d^{abc}
 {:}J^a(z)\,J^b(z)\,\bar c_c(z){:}\,.
\end{equation}
At spin $n=N$ (for finite depth $N$):
\begin{equation}\label{eq:spin-N-explicit}
 T^{(N)}(z)
 \;=\;
 \bigl[\,Q_{\mathrm{tot}},\, G^{(N)}(z)\,\bigr],
 ...
 \sum_{\vec a}
 d^{a_1\cdots a_N}
 {:}J^{a_1}\cdots J^{a_{N-1}}\,\bar c_{a_N}{:}(z).
\end{equation}
Substituting \eqref{eq:DS-improvement-antighost} into the
$3$d BV action, the $Q_{\mathrm{tot}}$-action on
$G^{(n)}$ evaluates to $T^{(n)}$ modulo $Q$-exact terms by
the same mechanism as Theorem~\ref{V2-thm:E3-topological-DS}:
each current $J^a = [Q_{\mathrm{tot}}, \bar c^a]$ on
cohomology (BRST-triviality of currents in the $3$d bulk),
and the combinatorial coefficient
$1/(n!\,(k+h^\vee)^{n-1})$ is precisely the one fixed by the
$\cW_{1+\infty}$ bracket (T2).

\medskip\noindent
\textbf{Step 5: Iterated Dunn additivity.}
Steps 1--4 produce $k$ mutually commuting topological
$E_1$-factors $E_1^{\mathrm{top}}(T^{(n_i)})$, each
rendering its spin-$n_i$ translation $Q$-exact, together
with the closed $E_2^{\mathrm{top}}$-factor of Step~3. The
$\infty$-categorical Dunn additivity theorem
(Lurie~\cite{HA}, Theorem~5.1.2.2) asserts
\[
 E_p \otimes_{\mathrm{Dunn}} E_q \;\simeq\; E_{p+q}
\]
for any non-negative integers $p,q$, and iterates to
\[
 E_2^{\mathrm{top}}
 \otimes_{\mathrm{Dunn}}
 \underbrace{E_1^{\mathrm{top}}\otimes\cdots\otimes E_1^{\mathrm{top}}}_{k\text{ factors}}
 \;\simeq\;
 E_{k+2}^{\mathrm{top}}.
\]
Applied to the $k+1$ mutually commuting
locally-constant-factorisation structures on
$\Zder^{\mathrm{ch}}(\cA)$ produced by Steps 3--4, this
gives~\eqref{eq:ladder-dunn}. The BRST-commutation of the
antighosts (Step 2) is the hypothesis that makes Dunn
additivity applicable: without it, the factors would not be
mutually commuting in the required homotopy sense.

\medskip\noindent
\textbf{Step 6: Compatibility with bar truncation.}
The bar functor $B\colon
E_{k+2}^{\mathrm{top}}\text{-}\mathsf{Alg}
\to E_{k+1}^{\mathrm{top}}\text{-}\mathsf{CoAlg}$ is defined
by $B = k \otimes^L_{(-)} k$ relative to any chosen
$E_1$-factor. Applied to the depth-$k$ rung and using the
topmost factor $E_1^{\mathrm{top}}(T^{(n_k)})$, $B$ removes
that factor and yields the depth-$(k-1)$ rung. This is
functorial by construction of the Dunn product and shows
that the ladder is an infinite-length compatible tower.

\medskip\noindent
\textbf{Step 7: The $N = \infty$ limit.}
For $N = \infty$, the tower
$\{E_1^{\mathrm{top}}(T^{(n)})\}_{n=2}^{\infty}$ is
countable and all factors mutually BRST-commute (Step 2).
The inverse limit in the category of $E_n$-operads,
$\varprojlim_n E_n = E_\infty$, yields
$\Einf^{\mathrm{top}}$-structure
on~$\Zder^{\mathrm{ch}}(\cA)$, which is the
assertion~\eqref{eq:e-infty-limit}. The limit is
well-defined because the bar-truncation maps of Step~6
provide the compatible transition maps.
\end{proof}
...
stress tower has depth $N = 2$ with $T^{(2)} = T(z)$ the
Virasoro stress tensor and no higher entries. The
spin-$2$ antighost is the Faddeev--Popov $b(z)$ of $3$d
gravity:
\[
 T(z) \;=\; \bigl[\,Q_{\mathrm{tot}},\, b(z)\,\bigr].
\]
Theorem~\textup{\ref{thm:e-infinity-topologization-master}} at
$k = 1$ gives $\Zder^{\mathrm{ch}}(\mathrm{Vir}_c)
\in E_3^{\mathrm{top}}\text{-}\mathsf{Alg}$, recovering the
climax of Volume~II
\textup{(}the climax appears as the $k=1$ rung, $N=2$
truncation of the master ladder\textup{)}.
\end{corollary}
...
For $\cA = \cW_N$ at generic non-critical level, the
higher-spin stress tower has depth $N$ with entries
$\{T^{(2)}, T^{(3)},\ldots,T^{(N+1)}\}$: the primary spin
fields of~$\cW_N$. Theorem~\textup{\ref{thm:e-infinity-topologization-master}}
at $k = N$ gives
\[
 \Zder^{\mathrm{ch}}(\cW_N)
 \;\in\;
 E_{N+2}^{\mathrm{top}}\text{-}\mathsf{Alg}.
\]
The rung indexed by spins $\{2, 3, \ldots, n+1\}$ for
$n \le N$ yields an $E_{n+2}^{\mathrm{top}}$-structure;
taking $n = N$ saturates the tower.
\end{corollary}
...
For $\cA = \cW_{1+\infty,c}$ at generic central charge, the
stress tower has depth $N = \infty$. The inverse limit of
Theorem~\textup{\ref{thm:e-infinity-topologization-master}} gives
\[
 \Zder^{\mathrm{ch}}(\cW_{1+\infty,c})
 \;\in\;
 \Einf^{\mathrm{top}}\text{-}\mathsf{Alg}
 \;=\;
 \varprojlim_{k} E_{k+2}^{\mathrm{top}}\text{-}\mathsf{Alg}.
\]
\end{corollary}

\begin{remark}[Fractional-weight ghost integration for
non-principal DS; closes ]
...
spin-$n$ improvement antighost acquires fractional
conformal weights. Explicitly, for a field
$\beta^{(n)}(z)$ carrying a grading $d_f$ from the
$f$-grading, define
\begin{equation}\label{eq:fractional-antighost}
 G^{(n)}_f(z)
 \;=\;
 \oint_\gamma
 \omega_n(w,z)^{1/d_f}\,\beta^{(n)}(w)\,dw
 \qquad
 \text{on the branched cover }
 \widetilde X \to X,
\end{equation}
where $\omega_n(w,z)$ is the spin-$n$ propagator on~$X$ and
the $d_f$-fold branched cover $\widetilde X \to X$ is
ramified along the DS-reduction divisor. The branch cut
absorbs the fractional weight, producing a well-defined
single-valued section of an appropriate line bundle on
$\widetilde X$. The BRST identity
$T^{(n)}_f = [Q_{\mathrm{tot}}, G^{(n)}_f]$ holds on
$\widetilde X$ by the same Cartan-current exactness
argument as Theorem~\ref{V2-thm:E3-topological-DS-general} of
Volume~II, with the fractional-weight contour integral
replacing the integer-weight contour of principal DS.
\end{remark}

\begin{remark}[Class~M $\to$ class~L promotion via spin-3
entry; closes ]
...
The shadow obstruction tower of a class~M algebra
(Virasoro, generic~$\cW_2$) has leading quartic-pole
coefficient at depth~$4$. When the tower admits a spin-3
entry $W^{(3)}(z)$ satisfying
$T^{(3)} = [Q_{\mathrm{tot}}, G^{(3)}]$, the residue
computation of the bar differential receives a cross-spin
contribution
\[
 \Res_{z=0}\,W^{(3)}(z)\cdot T^{(2)}(0)
\]
whose coefficient exactly cancels the class~M quartic-pole
residue by the $\cW_{1+\infty}$ structure constant
$c_{2,3,4}$ in Definition~\ref{def:higher-spin-stress-tower}(T2).
The resulting freely-generated PVA has polynomial shadow
depth: class~M with $W^{(3)}$ present descends to class~L at
the next filtration step. This is the mechanism by which
$\cW_3$ (which has $T^{(2)}$ and $T^{(3)}$) yields
$E_4^{\mathrm{top}}$ rather than the purely class~M
$E_3$ that Virasoro alone provides.
\end{remark}
...
Theorem~\ref{thm:e-infinity-topologization-master} embeds each
chiral algebra with stress tower of depth~$N$ into an
infinite bidirectional ladder
\[
 ...
 \longleftrightarrow \cdots,
\]
where the two types of arrows are:
\begin{itemize}
...
\end{itemize}
The composite $Z \circ B$ is the identity on the stable
part of the ladder; the composite $B \circ Z$ is the
shadow projection to the next lower rung. At each rung,
the presence of a higher-spin stress tensor
$T^{(n)}$ with BRST primitive $G^{(n)}$ adjoins one
topological $E_1$-factor and advances the rung by one.
\end{remark}
...
\item A theory of \emph{factorisation algebras valued in
 ...
 status depends on the coalgebra: for the conilpotent case with
...
\ClaimStatusProvedHere
Let $n \geq 2$ and let $V$ be a commutative algebra in
chain complexes \textup{(}equivalently an
$\Einf$-algebra\textup{)}. Write
$\mathsf{P}_n = H_*(\En)$ for the $n$-Poisson operad
\textup{(}commutative product $+$ degree-$(1{-}n)$ Lie
bracket satisfying Leibniz\textup{)}.
Suppose $\En$ is formal over~$\bR$, i.e.,
there exists a quasi-isomorphism of operads
$\En \xrightarrow{\sim} \mathsf{P}_n$ in chain complexes.
Then the space of $\En$-algebra structures on $V$ deforming
the given commutative structure, modulo
$\En$-quasi-isomorphisms, is equivalent to the space of
$\mathsf{P}_n$-algebra structures on $H^*(V)$ deforming the
commutative structure:
\[
 \pi_0\bigl(\En\text{-}\mathrm{Alg}^{\mathrm{def}}(V)\bigr)
 \;\simeq\;
 \pi_0\bigl(\mathsf{P}_n\text{-}\mathrm{Alg}^{\mathrm{def}}
 (H^*(V))\bigr).
\]
In particular, for $V = \Sym(W)$ free commutative, the
$\mathsf{P}_n$ deformations are classified by
degree-$(1{-}n)$ Lie brackets on $W$ satisfying the Leibniz
rule with respect to the symmetric product
\textup{(}$\mathsf{P}_n$ biderivations\textup{)}.
\end{lemma}
...
we obtain an equivalence between the $\infty$-categories
of $\En$-algebras and $\mathsf{P}_n$-algebras in chain
complexes.

\emph{Step 2: deformation equivalence.}
Fix a commutative algebra $V$, viewed as a
$\mathsf{P}_n$-algebra with zero Lie bracket. Under the
equivalence of Step~1, $V$ qua $\mathsf{P}_n$-algebra
corresponds to $\phi^*(V)$ qua $\En$-algebra, which is $V$
with its commutative (hence $\En$) structure.
The equivalence restricts to the formal neighbourhoods of
these base points: filtered deformations of $V$ as an
$\En$-algebra correspond bijectively to filtered
deformations of $V$ as a $\mathsf{P}_n$-algebra (on the
level of equivalence classes).

\emph{Step 3: biderivation reduction.}
For $V = \Sym(W)$ free commutative, a
...
on $\Sym(W)$ is equivalent to the Jacobi identity on
generators (extend by Leibniz), so
$\mathsf{P}_n$-deformations are classified by
degree-$(1{-}n)$ Lie brackets on $W$ extended as
biderivations.
\end{proof}
...
Two constructions produce $\Ethree$-deformation families
...
(Theorem~\ref{thm:cfg}). Both are parametrised by
$(k+h^\vee)\,H^3(\fg)[[k+h^\vee]]$.
For simple~$\fg$, $\dim H^3(\fg) = 1$, so the
deformation base is one-dimensional at each order:
the two families must coincide.
...
\begin{proof}
We construct an isomorphism of $\Ethree$-algebras order by
order in $\lambda = k + h^\vee$. The key
input is $\Ethree$ formality, which reduces the
chain-level matching to a cohomological computation
in $\Pthree$ deformations.
...
deduces that the operad-level formality
quasi-isomorphism $\En \xrightarrow{\sim} \mathsf{P}_n$ induces an
equivalence of deformation theories for algebras.
...
H_*(\Ethree;\, \bR)$
identifies chain-level $\Ethree$ structures on $C^*(\fg)$,
up to quasi-isomorphism, with $\Pthree$-algebra structures
on $H^*(C^*(\fg)) = C^*(\fg)$.
...
isomorphism modulo $\lambda^{p+2}$ lies in
...
functor $\Gamma(D, -)$ is symmetric monoidal and
preserves $\Ethree$-structures
...
$(a,b) \mapsto k\,(a,b)$), the two constructions agree
at order~$p+1$.
...
compatible with~$\varphi$ order by order
...
the obstruction-theoretic argument of Steps~1--3 produces
the correct isomorphism, with the KZ coupling
...
$\dim H^3(\fg) = 1$ to force uniqueness at each order.
...
\]
of filtered $\Ethree$-algebras holds over the
two-dimensional base
$(\lambda_1, \lambda_2) \in \CC^2$, order by order in
the joint $(\lambda_1, \lambda_2)$-adic filtration.
...
$\Ethree$ identification holds unconditionally on the
$\binom{r+1}{2}$-dimensional deformation base. A full
complete proof is postponed; the deformation
...
the chain-level $\Ethree$ on the original BRST complex is proved by
gauge rectification of the Sugawara contraction
(Proposition~\ref{prop:sugawara-gauge-rectification}). For class~M
algebras, where the chain-level $\Ethree$ encounters obstructions from
...
The K3 chiral bialgebra sits in a four-rung $E_n$-ladder
\[
...
\]
with the first rung the ordered associative (Hall-theoretic) structure,
the second rung the factorised pair-of-points structure on
$\mathrm{Ran}(E^{\mathrm{nod,sm}}_{24})$, the third the K3
$\mathsf{SC}$-type coherence, and the fourth the $E_3$-topological
bulk of twisted holographic 3d quantum gravity. Primary-lit for each
rung: Beilinson-Drinfeld 2004 ($E_1^{\mathrm{ch}}$), Francis-Gaitsgory
2012 ($E_2^{\mathrm{ch}}$), Vol.~II $\mathsf{SC}$-programme, Lurie HA
($E_3^{\mathrm{top}}$).
\end{remark}

\begin{remark}[$E_2$-chiral factorisation over pairs on $E^{\mathrm{nod,sm}}_{24}$]
\label{rem:enkd-deep-2}
The $E_2$-chiral factorisation of $\mathbf H_{\Delta_5}$ over pairs of
points on the Kodaira curve $E^{\mathrm{nod,sm}}_{24}$ produces a
factorisation algebra on the second configuration space. At pairs
$(z_1,z_2)$ where both points lie on the same Kodaira I$_1$ fibre, the
pair-factorisation reduces to the single-point Hall structure modulo
the $\Phi_{10}/\eta^{24}$ twist. At pairs on distinct Kodaira fibres
the pair-factorisation is governed by the Siegel-elliptic dynamical
$R$-matrix $R_{\mathrm{Sieg,dyn}}(z_1,z_2;\tau,w)$. This two-case structure
is the K3-base realisation of the factorisation-algebra locality axiom.
\end{remark}

\begin{remark}[$E_3^{\mathrm{top}}$ = holographic bulk]
\label{rem:enkd-deep-3}
After topologisation (Vol.~II, \S\ref{V2-sec:schtop-heptagon-supplement}),
the K3 $\mathsf{SC}^{\mathrm{ch,top}}$-shadow becomes a fully
topological $E_3$-algebra, which is exactly the $3$d bulk of the
twisted holographic quantum gravity on $\mathrm{AdS}_3\times S^2$
(Vol.~II, \S\ref{V2-sec:3dgravvix-supplement}). The universal identity
...
the Lusztig locus. Primary-lit: Costello-Gwilliam 2017, Francis-Gaitsgory
2012, Lurie HA Chapter 5.
\end{remark}
