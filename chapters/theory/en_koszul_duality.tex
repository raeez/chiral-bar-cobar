...
\end{equation}
This is the precise mechanism by which a $2$-dimensional chiral
algebra on a curve~$X$ produces a $3$-dimensional
holomorphic-topological bulk/boundary system on $X \times \bR$.
The extra dimension is \emph{not} an ad hoc addition: it is the
universal normal-collar direction of a codimension-one boundary
condition, forced by the center construction.

The physical realization of this algebraic principle has been
established independently by Khan--Zeng~\cite{KhanZeng25}, who
construct a mixed holomorphic-topological gauge theory on
$\bR \times \bC$ from a freely generated Poisson vertex algebra
and prove that gauge invariance holds if and only if the
$\lambda$-Jacobi identity is satisfied.  Moreover, they show that
the existence of an inner Virasoro element upgrades the
holomorphic-topological symmetry to fully topological symmetry,
matching the chain-level statement that the cocycle $[T]$ representing
holomorphic translation is exact in $H^\bullet(C^\bullet_{\mathrm{ch}}(\cA,\cA), \delta)$
when $T \in \cA$ satisfies $T(0) = \partial$.

In the framework of this monograph, the
Khan--Zeng construction realizes the right-hand arrow
of~\eqref{eq:deligne-tamarkin-chain} via a BV path integral:
the universal algebraic thickening
$\bigl(C^\bullet_{\mathrm{ch}}(\cA,\cA),\cA\bigr)$ is the
perturbative local observable algebra of the
holomorphic-topological gauge theory.  The degree constraint
forces the holomorphic direction to be one-dimensional (a
curve), and the topological direction to be one-dimensional
(an interval or line), yielding the $2{+}1$-dimensional
spacetime $\bC \times \bR$.
\end{remark}
