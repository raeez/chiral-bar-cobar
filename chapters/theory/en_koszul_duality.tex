...
\item \emph{$\mathsf{SC}^{\mathrm{ch,top}}\ne\Ethree$.}
 ...
 and the original-complex lift is the separate class-$L$
 theorem~\ref{thm:chain-level-E3-top-class-L}. For
...
\item \emph{topologization scope.}
 ...
 (cohomological in general; chain-level for class~$\mathbf{L}$
 only through Theorem~\ref{thm:chain-level-E3-top-class-L}).
 Every reference to $\Ethree^{\mathrm{top}}$ \emph{on the
 original complex} is scope-restricted to the cases where an
 explicit original-complex theorem is invoked: class~$\mathbf{L}$
 at non-critical level, class~$\mathbf{G}$ by commutativity, and
...
general chiral algebras with conformal vector, this remains a
separate conjectural topologization problem.
\end{theorem}
...
\item Affine Kac--Moody at non-critical level gives the first
 ...
 primitive. On the original complex this is the class-$L$
...
Sugawara contraction. Its proof uses the finite shadow depth
of class~$L$ \textup{(}$r_{\max}=3$\textup{)} and the Jacobi
vanishing of the cubic harmonic obstruction to terminate the
$A_\infty$-coherence tower of
...
For affine Kac--Moody this missing datum is supplied by
Theorem~\ref{thm:chain-level-E3-top-class-L} and by the
explicit criterion in
...
with the class-$L$ termination theorem. For general chiral
...
is supplied by Theorem~\ref{thm:chain-level-E3-top-class-L}.
...
\item \emph{Affine Kac--Moody: the class-$L$ gauge criterion.}
 ...
 For affine Kac--Moody, this gauge procedure terminates by
 Theorem~\ref{thm:chain-level-E3-top-class-L}: class~$L$ has
 finite shadow depth $r_{\max} = 3$,
 the cubic harmonic obstruction vanishes by Jacobi
 \textup{(}Proposition~\textup{\ref{prop:chain-level-three-obstructions}}\textup{)},
 and no quartic or higher shadow contributions exist.
 The corrected contraction $\widetilde G_1 = G_1 - \eta_1$
 solves $[m, \widetilde G_1] = \partial_z$ in the brace
 deformation complex; see
...
\begin{proposition}[Chain-level $\Ethree^{\mathrm{top}}$ for
affine Kac--Moody via gauge rectification]
...
center $Z^{\mathrm{der}}_{\mathrm{ch}}(\cA)$ admits a
chain-level $\Ethree^{\mathrm{top}}$-algebra structure on
the original BRST complex
\textup{(}not merely on a quasi-isomorphic
model\textup{)}.
\begin{enumerate}[label=\textup{(\roman*)}]
\item \textup{(Gauge-rectified Sugawara contraction.)}
 ...
 explicit primitives
 \textup{(}equation~\eqref{eq:TQG-result}\textup{)}.
 ...
 $Q$-cohomology.
\item \textup{(Termination of the coherence tower.)}
 ...
 Each component $D^{(1)}_n$ is a sum of brace
 compositions involving $m_r$ with $r \geq 2$.
 For $V_k(\fg)$, class~$L$ has shadow depth
 $r_{\max} = 3$. The only contributing higher
 operation is $m_2$ \textup{(}the chiral product\textup{)}.
 ...
 class~$L$\textup{)}.
 Since $r_{\max} = 3$, there are no quartic or higher
 shadow contributions: the defect $D^{(1)}$ vanishes
 identically.
 The $A_\infty$-coherence tower of
 Remark~\textup{\ref{rem:topologization-chain-vs-cohomology}}
 therefore terminates, with $G = (\widetilde{G}_1)$
 already solving
 \[
 [m, \widetilde{G}_1] \;=\; \partial_z
 \]
 in the brace deformation complex.
\item \textup{(Chain-level $\Ethree^{\mathrm{top}}$.)}
 By part~\textup{(ii)}, holomorphic translation
 $\partial_z$ is null-homotopic in the brace deformation
 complex of the original cochain complex
 $Z^{\mathrm{der}}_{\mathrm{ch}}(\cA)$.
 The factorization algebra on~$\bC$ is therefore locally
 constant on cochains
 \textup{(}Lurie~\cite{HA},
 Theorem~5.4.5.9\textup{)}.
 Combined with $\Eone^{\mathrm{top}}$ from the
 $\bR$-direction, Dunn additivity gives a chain-level
 $\Ethree^{\mathrm{top}}$-structure on the original
 BRST complex
 $Z^{\mathrm{der}}_{\mathrm{ch}}(\cA)$.
 On BRST cohomology, this recovers the cohomological
 $\Ethree^{\mathrm{top}}$ of
 Theorem~\textup{\ref{thm:topologization}}\textup{(i)}.
\end{enumerate}
\end{proposition}
...
of equation~\eqref{eq:QG-expansion}. The construction
proves that both $R_{\mathrm{ghost}}$ and $R_{\mathrm{self}}$
are $Q$-exact on the bulk BV complex: writing
$R_1 = [Q, \eta_1]$ with $\eta_1$ the explicit primitive
given in the proof of
Construction~\ref{constr:sugawara-antighost}, the corrected
contraction $\widetilde G_1 := G_1 - \eta_1$ satisfies
\begin{equation}\label{eq:corrected-contraction}
[Q, \widetilde G_1] \;=\; T_{\mathrm{Sug}}
\end{equation}
on the original BRST complex. The operator
...
\emph{Part~\textup{(ii)}: finiteness of the coherence tower
from shadow depth.}
...
For $n \geq 3$, the defect involves brace compositions with
$m_r$ for $r \geq 3$. Affine Kac--Moody is class~$L$ with
shadow depth $r_{\max} = 3$: the only nonzero
$A_\infty$-operations beyond $m_1$ and $m_2$ are in the
shadow sector at degree~$3$, and these are determined by the
classical $r$-matrix
$r(z) = k\Omega/z$
% trace-form convention; k=0 -> r=0 verified
through the Maurer--Cartan equation. The degree-$3$
shadow operation $m_3$ acts on the bar complex through
the Lie bracket and level; it does not produce a
translation-non-equivariant correction because the
structure constants are translation-invariant and
$[\partial_z, \widetilde G_1] = 0$.
The resulting brace compositions all vanish: the shadow
tower truncates at depth~$3$, and the truncated tower
produces no defect beyond degree~$2$.
Hence $D^{(1)} = 0$ and
$[m, \widetilde G_1] = \partial_z$ in the brace
deformation complex.

\emph{Part~\textup{(iii)}: recognition and Dunn additivity.}
By part~(ii), the holomorphic translation derivation
$\partial_z$ is null-homotopic in the brace deformation
complex of
$(Z^{\mathrm{der}}_{\mathrm{ch}}(\cA), m)$.
This is the $A_\infty$-coherent
family of homotopies required by
Theorem~\ref{thm:topologization}(iii):
the family $G = (\widetilde G_1)$ with $G_k = 0$ for
$k \geq 2$ satisfies $[m, G] = \partial_z$.
...
\emph{Compatibility.}
On BRST cohomology, the chain-level structure
induces the cohomological $\Ethree^{\mathrm{top}}$ of
Theorem~\ref{thm:topologization}(i), because the passage
to cohomology kills $D^{(1)}$ regardless; for affine
Kac--Moody, $D^{(1)} = 0$ already on cochains, so
the two structures are compatible by construction.
...
finite shadow depth $r_{\max} = 3$ (requires class~$L$,
which is affine Kac--Moody). For class~$M$
...
Proposition~\ref{prop:sugawara-gauge-rectification}
resolves the chain-level topologization for affine Kac--Moody
by combining three independent constructions.
\begin{enumerate}[label=\textup{(\alph*)}]
...
\item \emph{Shadow-depth truncation
 \textup{(}Approach~C\textup{)}.}
 For class~$L$, the coherence tower has only three
 nontrivial degrees. The cubic obstruction vanishes by
 Jacobi; higher obstructions are absent because
 $r_{\max} = 3$. This is the ingredient that makes the
 gauge procedure terminate.
...
\item \textup{(Costello--Gwilliam factorisation algebra.)}
 ...
 On the chiral centre, the $\Etwo^{\mathrm{hol}}$ factor
 encodes the chiral Gerstenhaber bracket from the
 $\mathsf{SC}^{\mathrm{ch,top}}$ structure
 \textup{(}Theorem~\textup{\ref{thm:operadic-center-hochschild}}\textup{)}.
...
$2$-manifold). The bulk observables of the $3$d theory on the
$\bC$-slice reproduce the derived chiral centre
$Z^{\mathrm{der}}_{\mathrm{ch}}(\cA)$ by the
holomorphic descent identification of~\cite{CG-vol2},
Chapter~5.
...
trivially on $H_Q^\bullet$. The factorisation algebra on $\bC$
therefore becomes locally constant on BRST cohomology.
...
\emph{same} scalar at each order.
By $\Ethree$ formality (Step~1), this scalar is
determined by the $\Pthree$ bracket on generators:
...
By Theorem~\ref{thm:chiral-e3-cfg}(ii), the
chiral $\Pthree$ bracket restricted to~$D$ recovers
the CFG $\Pthree$ bracket at every order: both give
$\{\phi_a, \phi_b\} = h_{\mathrm{KZ}} \cdot (\phi_a,
\phi_b)$ on generators (with $h_{\mathrm{KZ}} =
1/(k + h^\vee)$, the KZ coupling constant).
Since the $\Pthree$ bracket on generators determines
the full $\Ethree$ deformation class
(biderivation property,
Lemma~\ref{lem:bv-p3-commutativity}, Step~2), and
the space of $\fg$-equivariant corrections at each order
is one-dimensional (spanned by the Killing-form pairing
$(a,b) \mapsto k\,(a,b)$), the two MC classes agree at
order~$p+1$.
...
\begin{remark}[Obstruction analysis for $\mathfrak{gl}_N$]
\label{rem:e3-non-simple-gl-N}
We carry out the uniqueness argument of
Theorem~\ref{thm:e3-identification} for
$\fg = \mathfrak{gl}_N = \mathfrak{sl}_N \oplus
\mathfrak{gl}_1$, where the deformation space is
two-dimensional at each order.
...
of Remark~\ref{rem:e3-non-simple-gl-N} applied
...
the equivalence~\eqref{eq:disk-equivalence} endows
...
the chain-level $\Ethree$ on the original BRST complex is the
class-$L$ theorem
(Theorem~\ref{thm:chain-level-E3-top-class-L}); the explicit
Sugawara gauge criterion is
