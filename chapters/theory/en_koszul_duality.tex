...
The $\Etwo$ case identifies shadow-tower termination at arity~$2$ with the $\Etwo$-formality of $\ChirHoch^*(\cA)$, and the $\Eone$ case is the associative analogue via the ordered bar complex $\barB^{\mathrm{ord}}(\cA)$ (Chapter~\ref{chap:e1-modular-koszul}).
\end{proof}
...
\begin{theorem}[Topologization; {\cite{KhanZeng25}}]
\label{thm:topologization}
\ClaimStatusProvedHere
\index{topologization theorem!statement}
Let $\cA$ be a chiral algebra with inner conformal
vector \textup{(}Definition~\textup{\ref{def:inner-conformal-vector}}\textup{)}. The chiral
...
is an $\Etwo^{\mathrm{top}}$-algebra. Combined with the
...
Affine Kac--Moody $V_k(\fg)$ at generic non-critical
level: the Sugawara element provides the inner conformal
vector. Topologization produces the perturbative
...
is 3d gravity. Topologization requires $T$ to be $Q$-exact
in the 3d gravity bulk; this is expected at generic
$c \neq 0$ but requires specifying the BRST complex of
3d HT gravity and verifying $T = [Q, G]$ explicitly.
At present this is verified for affine Kac--Moody
(where the Sugawara mechanism is explicit) but remains
conditional for the gravitational case.

$\cW$-algebras $\cW_N$: the Virasoro subalgebra provides
the conformal vector. Topologization succeeds at generic
level.
...
The chapter establishes six results. The Totaro presentation
...
correct statement is that the \emph{derived center}
$Z^{\mathrm{der}}_{\mathrm{ch}}(\cA)
