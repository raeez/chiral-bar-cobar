...
mixed operations). On the commutative/local lane, the
...
on a lane where the required open action has been supplied, the bar
...
center-generated. The open proof obligation is to verify the
...
Off the Koszul locus, center generation is expected to fail. A
...
ordinary complex into class~$\mathrm{L}$. The true proof obligation
...
homotopy-invariant lane and an original-complex lane. For affine
Kac--Moody, the original-complex lane is resolved by
...
\emph{Proof sketch.} The commutativity
...
\begin{remark}[Three approaches to chain-level topologization]
\label{rem:three-approaches-chain-level}%
\index{topologization theorem!three approaches to chain-level}%
Proposition~\ref{prop:sugawara-gauge-rectification}
resolves the chain-level topologization for affine Kac--Moody
by combining three independent strategies.
\begin{enumerate}[label=\textup{(\alph*)}]
...
\item \textup{(Topologisation of the holomorphic factor.)}
 By Theorem~\textup{\ref{thm:topologization}}, the Sugawara element
 $T_{\mathrm{Sug}} = \frac{1}{2(k+h^\vee)}\sum_a {:}J^a J_a{:}$
...
On the symmetric finite-type or completed lane, the chiral bar complex
...
finite-type or completed continuous lane.
\begin{enumerate}[label=\textup{(\roman*)}]
...
The Vol~I Koszul conductor $K(\cA) = -c_{\mathrm{ghost}}(\mathrm{BRST}(\cA)) = \mathrm{tr}_{\mathfrak{Z}(\cA)}(\mathfrak{K}_{\cA})$ on the chiral side intertwines via Borcherds singular-theta pushforward with Vol~III $\kappa_{\mathrm{BKM}}(\mathfrak{g}_{\Delta_5}) = c_\Lambda(0)/2 = \mathrm{wt}(\Delta_5) = 5$. The Mukai-doubling value $K^{\kappa_{\mathrm{ch}}}(\mathcal{H}_{\mathrm{Muk}}(K3)) = 2 c_+(\mathrm{Mukai}(K3)) = 8$ enlarges Theorem~C on the Lorentzian-lattice-parametric $\mathcal{B}$-family to $\kappa + \kappa^! \in \{0, 8, 13, 250/3, 98/3\}$. Universal identity $\hbar^2 \cdot K^{\kappa_{\mathrm{ch}}} = -1$ pins the Lusztig $u_\zeta$-specialisation at $\ell = 8$, matching the order of the monodromy of $\mathcal{L}^{\Delta_5}$ around Humbert $H_1 \subset \overline{\mathcal{A}_2}$ (Bruinier $2002$ Proposition~$5.1$).
\end{remark}
...
topological $E_3$-algebra structure. This $E_3$ object is the bulk
observable algebra of the twisted holographic $3$d theory on
...
$\hbar^2\cdot K^{\kappa_{\mathrm{ch}}} = -1$ is the consistency
condition for this $E_3$-topologisation: it forces the chiral-direction
central charge to cancel against the bulk cosmological constant at
the Lusztig locus.
\end{remark}
