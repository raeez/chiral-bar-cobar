...
The ordered bar complex $\barB^{\Eone}(\cA)$ on ordered configuration spaces $\mathrm{FM}^{\mathrm{ord}}_n(\mathbb{C})$ carries the associative shadow tower $(\Theta^{\Eone}_\cA)^{\leq r}$; formality of the ordered convolution algebra is the vanishing of the transferred $A_\infty$-operations $m_n^{\mathrm{tr}}$ for $n \geq 3$ (Chapter~\ref{chap:e1-modular-koszul}).
\end{enumerate}
\end{proposition}
...
\subsection{Configuration space Betti numbers and the $\En$ shadow tower}
...
\section{The Kontsevich graph complex and the shadow tower}
...
\begin{proposition}[Shadow tower to $\mathrm{GC}_2$ bridge;
\ClaimStatusProvedHere]
...
\hline
G \textup{(}depth $2$\textup{)} & $\sigma_3$ only
  & Heisenberg, lattice \\
...
The depth truncation follows from the shadow tower termination:
...
encode the ``shape'' of the shadow tower, independent of the overall
scale set by~$\kappa$.
\end{definition}
...
the output fiber at the basepoint $z_0 \in X$.  This is precisely
...
\item \emph{Step 4} (comparison) requires that the $\Etwo$-structure
  ...
  conjecture proof.  In the chiral case, the comparison additionally
  requires that the configuration-space forms are compatible with the
...
\section{\texorpdfstring{The $\En$ shadow tower}{The En shadow tower}}
\label{sec:en-shadow-tower}
\index{shadow tower!En@$\En$|textbf}
\index{En algebra@$\En$ algebra!shadow tower|textbf}

The shadow tower of a chiral algebra~$\cA$ is built from the
...
\begin{definition}[$\En$ shadow tower]
...
\begin{theorem}[$\En$ shadow tower: universality of $\kappa$ and formality collapse;
\ClaimStatusProvedHere]
...
\item \textup{(Formality collapse.)}
  ...
  then the full shadow tower collapses:
  \[
  S_r^{\En}(A) \;=\; S_r^{\Einf}(A)
...
\item \textup{(Shadow depth classification.)}
  The four-class partition \textup{(}Definition~\textup{\ref{def:shadow-depth-class})}
  \[
  \mathrm{G}\ (r_{\max} = 2),
  \quad
  \mathrm{L}\ (r_{\max} = 3),
...
$\{S_r\}_{r \geq 2}$ (Definition~\ref{def:shadow-depth-class}).
...
at arity~$r$ is bounded by $r - 1$ (the maximum number of
propagator insertions at arity~$r$).  Once $n - 1 > r - 1$, i.e.,
$n > r$, all topological correction terms lie in degrees exceeding
the bar-complex range, hence contribute zero.  Therefore
$S_r^{\En}(A) = S_r^{\operatorname{Com}}(A)$ for $n > r$,
giving $N(r) \leq r + 1$.  For the binary invariant ($r = 2$),
this gives $N(2) \leq 3$; in fact, $N(2) = 1$ by~(i).
\end{proof}
...
\item \textup{($\mathsf{E}_3$ on the derived center.)}
  ...
  \textup{(}Lurie~\cite{Lurie_HA}, Francis~\cite{Francis13}\textup{)},
...
This is proved by Lurie (\cite{Lurie_HA}, Theorem~5.3.1.30)
and Francis (\cite{Francis13}).
...
The $\En$ shadow tower of the factorization envelope
