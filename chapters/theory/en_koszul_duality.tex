...
$C^*(\fg)$ are classified by $\Pthree$-deformations: the
...
$\fg^*[-1] \subset C^*(\fg)$ therefore determines the
full $\Ethree$ structure, since a $\Pthree$ bracket
on the free commutative algebra $\Sym(\fg^\vee[-1])$ is
equivalent to a degree~$-2$ Lie bracket on $\fg^*[-1]$
(the biderivation property).
The space of $\fg$-equivariant such brackets is
$\Hom_\fg(\Lambda^2\fg^*, \CC)[-2] \cong H^3(\fg)[-2]$,
one-dimensional for simple~$\fg$
(Lurie~\cite{HA},
Costello--Francis--Gwilliam~\cite{CFG26}, Theorem~1.4).
