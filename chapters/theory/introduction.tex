...
\end{enumerate}
In this monograph, only the S-level (scalar) and M-level (curved
coderivation) shadows are fully constructed; the H-level universal
non-scalar Maurer--Cartan object~$\Theta_{\cA}$ remains a
programme-level conjecture
(Conjecture~\ref{conj:universal-theta}).
The
...
The affine Kac--Moody family is unconditionally modular Koszul
at all genera (Theorem~\ref{thm:pbw-allgenera-km},
Corollary~\ref{cor:unconditional-allgenera-km}).
The Virasoro family is now unconditionally modular Koszul
at all genera
(Theorem~\ref{thm:pbw-allgenera-virasoro},
Corollary~\ref{cor:unconditional-allgenera-virasoro}).
The principal finite-type $\mathcal{W}$ families, including higher
$\mathcal{W}_N$, are now unconditionally modular Koszul at all genera
...
modular homotopy theory for factorization algebras on
curves---factorization algebras on $\operatorname{Ran}(X)$, coderived
categories, shifted-symplectic structures---is articulated in
\S\ref{sec:modular-koszul-programme}.
The historical five-master-conjecture package now has one resolved
entry theorem and four remaining live conjectures
...
while the cyclic $L_\infty$ deformation algebra
\textup{(}Conjecture~\textup{\ref{conj:master-theta}}\textup{)}
is the foundational target.
