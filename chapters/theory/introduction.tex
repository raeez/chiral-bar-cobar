...
$(\infty,2)$-adjunction $B^{\mathrm{ch}} \dashv \Omega^{\mathrm{ch}}$
is the natural home for chiral bar--cobar.
...
and partitions the standard landscape into five archetypes
$\mathbf{G}/\mathbf{L}/\mathbf{C}/\mathbf{M}/\mathbf{B}$ with scalar
complementarity
\[
\kappa(\cA) + \kappa(\cA^!) \in \{0,\; 8,\; 13,\; \tfrac{250}{3},\;
\tfrac{98}{3}\}
\]
realised respectively by free-field/Kac--Moody ($\mathbf{G}$ and
$\mathbf{L}$ at $0$), the BKM crown $\mathbf{H}_{\Delta_5}$
($\mathbf{B}$ at $8$) with $\hbar^2 K^{\kappa_{\mathrm{ch}}} = -1$ at
$(K, \hbar^2) = (8, -1/8)$, Virasoro ($\mathbf{M}$ at $13$), and
principal $\mathcal W$-algebra extensions ($\mathbf{M}$-row at
$250/3$ and $98/3$). The Lagrangian splitting is proved; the ambient
...
shadow. The Mukai pairing contributes the conductor value
$K^{\kappa_{\mathrm{ch}}}=8$, enlarging Theorem~C by a
Lorentzian-lattice-parametric $\mathcal{B}$-family term, distinct from
...
% ================================================================
% Four phantomsection stubs retired 2026-04-18 (Wave-15) via consumer
% retarget to canonical inscribed labels:
...
$E_2$-chiral output of the Volume~III CY-to-chiral correspondence
$\Phi_2$ applied to $D^b\mathrm{Coh}(K3)$; the Mukai pairing has signature
$(4, 20)$ with positive central charge $c_+(\mathrm{Mukai}(K3)) = 4$.
...
\[
  ...
  8,
\]
adding the fifth archetype $\mathbf{B}$ to the Theorem-C landscape
$\kappa + \kappa^! \in \{0,\, 8,\, 13,\, 250/3,\, 98/3\}$ on the
Lorentzian-lattice-parametric family.
...
\]
and the pseudo-character $S^{\mathrm{ps}}$ of dimension $4$ on the
paramodular Hecke algebra $\mathbb T^{\mathrm{par}}_1$ satisfies
\[
S^{\mathrm{ps}}_1(T_p)
\;=\;
\lambda_p
...
a_p(f_{16}) + p^8 + p^9,
\qquad
S^{\mathrm{ps}}_4(T_p, T_p, T_p, T_p)
\;=\;
p^{32},
\]
...
classification invariant $\Delta_5$; the $\mathbf{B}$-family entry
$K^\kappa = 8$ completes the shadow-tower landscape to five archetypes.
The universal identity $\hbar^2 \cdot K^\kappa = -1$ on this family pins
...
not be conflated (anti-pattern W13-TOP-$1$ in the concordance).
...
\section{Vol~I introduction through the K3 base}
\label{sec:v1-intro-supplement}

\begin{remark}[Vol~I introduction through bar-cobar at $\mathbf H_{\Delta_5}$]
\label{rem:v1-intro-1}
Vol~I's bar-cobar programme instantiates at the
K3 base as a sharp statement about $\mathbf H_{\Delta_5}$: the
chiral Koszul bar-cobar adjunction $B^{\mathrm{ch}}\dashv \Omega^{\mathrm{ch}}$
at $\hbar^2 = -1/8$ restricts to an equivalence between $\mathbf H_{\Delta_5}$-modules
and coalgebra-over-$\Delta_5$ modules, via Booth--Lazarev 2023
(arXiv:2304.08409) coderived model structure and curved Quillen
equivalence. The introduction's ``bar-cobar triangle'' becomes, at the
K3 base, the averaging triangle
...
\begin{remark}[Introduction's five theorems at the K3 base]
\label{rem:v1-intro-2}
Each of the introduction's five theorems admits a K3-specific
sharpening: (A) bar-cobar becomes K3 bar-cobar with conductor
$K^{\kappa_{\mathrm{ch}}} = 8$; (B) chiral Positselski holds at
...
concentration becomes the Andrianov $L(s,\Phi_{10}) = L(s,\Delta\otimes E_6)\zeta(s-9)\zeta(s-8)$
factorisation \cite{Andrianov1979,ChengDuncanHarvey2014,Mukai1987}.
Cross-ref Chapter~\ref{ch:k3-chiral-bialgebra-platonic}.
\end{remark}

\begin{remark}[Six lenses on the introduction's chiral object]
\label{rem:v1-intro-3}
The introduction's ``chiral algebra $A$ on a smooth curve $C$'' refines
at the K3 base to $\mathbf H_{\Delta_5}$ on the bi-based
Ran $(\mathrm{Ran}(E^{\mathrm{nod,sm}}_{24}),\,\bar{\mathcal A}_2)$,
viewed through six coordinate charts: Hall/Schiffmann-Vasserot,
Frenkel-Kac/Borcherds, Siegel-paramodular $R_{\mathrm{Sieg,dyn}}$
via Pasol-Zagier 2013, bi-based Ran factorisation, Costello $1$-loop
twist, class-$\mathcal S$ AGT with $c_{2d} = -214$
(first-principles Shapere--Tachikawa $c_{4d} = 107/6$; intermediate
intermediate value $-312$ retracted per Vol~I
Chapter~\ref{ch:thqg-open-closed-realization} line~1932). The transition
functions are the underlying bar--cobar, factorisation, and class-$\mathcal S$ bridges
...
octachotomy (B), five-archetype centre complementarity (C), genus-ladder
