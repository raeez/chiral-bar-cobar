...
\begin{remark}[The governing question and the route through the book]
\label{rem:governing-question-route}
The governing question is not whether classical Koszul duality survives
unchanged on curves.  The question is which single structure
simultaneously explains residues on configuration spaces, Verdier
duality on $\operatorname{Ran}(X)$, genus-dependent curvature, and the
behavior of the standard representation-theoretic families.  The answer
developed in this monograph is modular Koszul duality; the larger
subject toward which it points is modular homotopy theory for
factorization algebras on curves.

The exposition is organized as a controlled four-part route so that
these faces appear as one subject rather than as parallel analogies.
\emph{Frame}: Chapter~\ref{ch:heisenberg-frame} exhibits the complete
mechanism in one algebra.  \emph{Core}: Part~I proves the modular
Koszul theorems in general form.  \emph{Portraits}: Part~II tests the
core across the main representation-theoretic families.
\emph{Synthesis}: Part~III records the bridges to adjacent subjects and
tags each bridge as theorem-level or conjectural with explicit missing
input.
\end{remark}

\begin{remark}[Synthesis protocol (Chriss--Ginzburg style)]
\label{rem:chriss-ginzburg-synthesis}
To synthesize disparate deep subjects without reducing them to analogy,
we use one load-bearing object and many controlled shadows.

The load-bearing object is modular Koszul duality on curves: a chiral
algebra together with its bar-cobar, Verdier, and modular-curvature
data.  Every bridge in the monograph is then required to arise by
transport of this object through a specific functorial interface
(configuration-space geometry, representation categories, or
field-theoretic deformation complexes), not by rhetorical comparison.

Operationally, each synthesis claim is accepted only when three items
are explicit:
\begin{enumerate}[label=\textup{(\roman*)}]
\item the source theorematic package in Parts~I--II,
\item the transport mechanism to the target subject,
\item the status tag of the transported statement
(\ClaimStatusProvedHere, \ClaimStatusProvedElsewhere,
\ClaimStatusConjectured, \ClaimStatusOpen, or \ClaimStatusHeuristic).
\end{enumerate}
This is the protocol used throughout Part~III and the concordance
chapter.
\end{remark}
...
\item \emph{Theorem~D$_{\mathrm{scal}}$} (Scalar modular characteristic,
  ...
  The non-scalar completion via a universal
  Maurer--Cartan class $\Theta_{\cA}$ is conjectural
...
(on the Koszul locus, with the scalar package and spectral
invariants proved, while the full Maurer--Cartan package
remains conjectural).
...
\noindent
Quantum corrections are not a perturbative afterthought to Koszul
duality; they are the modular completion of the duality itself.  The
genus tower $\{\bar{B}^{(g)}(\mathcal{A})\}_{g \geq 0}$ is one
...
That single deformation appears at the three semantic levels of
Convention~\ref{conv:hms-levels}:
\begin{enumerate}[label=(\roman*)]
\item \emph{H-level:} a conjectural Maurer--Cartan element
  $\Theta_{\cA}$ in the cyclic deformation complex
...
In this monograph the M-level and S-level shadows are constructed,
while the universal H-level class $\Theta_{\cA}$ remains conjectural
(Conjecture~\ref{conj:universal-theta}).  The proved core already
contains the free-field families
...
\]
with $\Theta_{\cA}$ conjectural
\textup{(}Definition~\ref{def:full-modular-package}\textup{)}.  This is a hierarchy of
...
The programme of completing the proved modular Koszul core into
modular homotopy theory for factorization algebras on curves is
articulated in \S\ref{sec:modular-koszul-programme}.

The old
five-master-conjecture picture is now a dependency-ordered frontier
(\S\ref{subsec:five-master-conjectures}):
higher-genus PBW concentration is now a theorem
\textup{(}Theorem~\textup{\ref{conj:master-pbw}}\textup{)},

the cyclic $L_\infty$ deformation algebra
\textup{(}Conjecture~\textup{\ref{conj:master-theta}}\textup{)}
is the foundational next target,
the factorization-categorical comparison problems
\textup{(}Conjectures~\textup{\ref{conj:master-dk-kl}}
and~\textup{\ref{conj:master-infinite-generator}}\textup{)}
form the next structural layer,
and the BV/BRST--bulk identifications
\textup{(}Conjecture~\textup{\ref{conj:master-bv-brst}}\textup{)}
are downstream consequences.

On the current theorem surface, the
foundational MC2 layer is already reduced to three tasks: construct
the intrinsic cyclic model $\Defcyc(\cA)$, realize the geometric
completed tensor / clutching package, and solve the resulting
one-channel genus-by-genus normalization problem in the simple-Lie
case \textup{(}Proposition~\textup{\ref{prop:mc2-reduction-principle}},
Corollary~\textup{\ref{cor:one-dim-obstruction}}\textup{)}.
Propositions~\textup{\ref{prop:tautological-line-support-criterion}}
and~\textup{\ref{prop:one-channel-normalization-criterion}} further
reduce that last package to a joint clutching/trace isolation
statement for the tautological line, followed by one normalized scalar
comparison with the already-proved shadow coefficient~$\kappa(\cA)$.
Periodicity is tracked separately as an orthogonal weak flank rather
than as a prerequisite in this chain.
...
package together with the separately proved spectral invariants
\textup{(}Definition~\textup{\ref{def:scalar-modular-package}},
Theorem~\textup{\ref{thm:spectral-characteristic}}\textup{)}; unconditional
modular Koszulity for free fields
(Proposition~\ref{prop:standard-examples-modular-koszul});
unconditional modular Koszulity for Kac--Moody
(Theorem~\ref{thm:pbw-allgenera-km});
unconditional modular Koszulity for Virasoro
(Theorem~\ref{thm:pbw-allgenera-virasoro});
unconditional modular Koszulity for principal finite-type
$\mathcal{W}$-algebras
\textup{(}Theorem~\textup{\ref{thm:pbw-allgenera-principal-w}},
Corollary~\textup{\ref{cor:unconditional-allgenera-principal-w}}\textup{)};
evaluation-locus
...
construction) together with the full coderived
factorization-algebra formalism; the full factorization-categorical
derived Drinfeld--Kohno; the H-level infinite-generator comparison
problems (Virasoro~$\leftrightarrow W_\infty$,
$\mathcal{W}_N \leftrightarrow Y(\mathfrak{gl}_N)$) beyond the
theorematic completed M-level principal-stage packages;
BV/BRST/path-integral identifications beyond genus~$0$; and the
little-disks $E_n$-Koszul
duality extension to higher-dimensional manifolds.
