...
The key observation is elementary but fundamental: the logarithmic differential form $d\log(z_1 - z_2) = \frac{dz_1 - dz_2}{z_1 - z_2}$ has a simple pole precisely when $z_1 = z_2$. When we compute the residue
...
\item This manuscript provides a homotopy-geometric construction of the Bar complex that is left implicit in their work, and further extends the construction to all genera
\end{itemize}
...
\textbf{strictly}, not just up to homotopy, at genus~$0$. This requires:
\begin{itemize}
...
\begin{enumerate}
\item[\textup{(a)}] \textbf{Affine Kac--Moody:}
  $c(\widehat{\mathfrak{g}}_k) + c(\widehat{\mathfrak{g}}_{k'}) = 2 \dim \mathfrak{g}$.
\item[\textup{(b)}] \textbf{Principal $\mathcal{W}$-algebras:}
...
\begin{proof}
\textbf{Part (a).}  The Sugawara central charge is
...
\textbf{Part (b).}  The Drinfeld--Sokolov central charge for the principal
