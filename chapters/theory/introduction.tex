...
\item \emph{Theorem~D$_{\mathrm{scal}}$} (Scalar modular characteristic,
  ...
  $\mathcal{W}$-algebras).  Its vanishing is anomaly
  ...
  The universal Maurer--Cartan class $\Theta_{\cA}$ exists
  unconditionally
  ...
  $\dim H^2_{\mathrm{cyc}}(\cA,\cA) = 1$, by a
  graded-antisymmetry argument that is independent of the
  specific bracket structure.
  This condition is verified at all non-admissible, non-critical
  levels by cyclic rigidity
  (Theorem~\ref{thm:cyclic-rigidity-generic}),
  and on the full standard Lie-theoretic landscape by
  the saturation equivalence criterion
  (Proposition~\ref{prop:saturation-equivalence}),
  functorial stability
  (Proposition~\ref{prop:saturation-functorial}), and
  the effective quadruple reduction
  (Corollary~\ref{cor:effective-quadruple}).
  Conjecture~\ref{conj:scalar-saturation-universality}
  extends scalar saturation to admissible levels,
  where the primitive deformation sector
  $H^2_{\mathrm{cyc,prim}}(\cA)$ is not yet known to vanish.
\end{itemize}
...
Theorem~\ref{conj:master-theta} (originally MC2) through three inputs:
...
\item \emph{Clutching of stable curves.}
  ...
  proved by Theorem~\ref{thm:mc2-full-resolution})
  is compatible with clutching, trace, and
  Verdier duality.
...
scalar and spectral characteristic packages, unconditional modular
Koszulity for free fields, Kac--Moody, Virasoro, and principal
$\mathcal{W}$-algebras, and the universal Maurer--Cartan
class $\Theta_{\cA}$ (Theorem~\ref{thm:mc2-full-resolution},
resolving Theorem~\ref{conj:universal-MC}) are proved.
The full coderived factorization formalism and the categorical
Drinfeld--Kohno extension beyond the evaluation locus remain open.
The precise frontier is recorded in Chapter~\ref{chap:concordance}
and the programme of~\S\ref{sec:modular-koszul-programme}.
\end{remark}
...
%  see Chapter~\ref{chap:hochschild-cyclic} for the full treatment,
%  including periodicity (Theorem~\ref{thm:periodicity-cohomology}).]
