...
separately proved spectral layer, and a conjectural full homotopy
...
\right)
\end{equation}
(Conjectures~\ref{conj:universal-MC} and~\ref{conj:universal-theta}; constructing $\operatorname{Def}_{\mathrm{cyc}}(\cA)$ as a cyclic $L_\infty$-algebra and solving the MC equation is the principal open problem of the modular programme).  The conditional completion theorem (Theorem~\ref{thm:mc2-conditional-completion}) shows that three exact packages --- the intrinsic cyclic model, the geometric completed tensor product with clutching, and tautological-line support of the genus-$g$ obstruction --- suffice for the full conjecture.
...
(inversion) holds on the Koszul locus.  At higher genus, the curved
fiberwise differential $\dfib^{\,2} = \kappa \cdot \omega_g$
defines a family over~$\overline{\mathcal{M}}_g$ whose
characteristic invariants---scalar ($\kappa$), spectral
($\Delta_\cA$), and the conjectural Maurer--Cartan
class~$\Theta_\cA$---form a hierarchy of increasing refinement
...
the spectral discriminant $\Delta_{\cA}(x)$, and the conjectural
universal class $\Theta_{\cA}$
\textup{(}Definition~\ref{def:full-modular-package}\textup{)}.

The principal open problem is the construction of the cyclic
$L_\infty$ deformation algebra $\Defcyc(\cA)$ and the
Maurer--Cartan element $\Theta_\cA$ within it
(Conjecture~\ref{conj:master-theta}).  This reduces to three
constructions: the intrinsic cyclic model, the geometric completed
...
class $\Theta_{\cA}$ (Conjecture~\ref{conj:universal-MC}), the full
