...
%%% BKM enlargement of the shadow menu %%%

\section*{The K3 chiral bialgebra and the B-family enlargement}

The K3 Mukai-Heisenberg $\mathcal{H}_{\mathrm{Muk}}$ at $c = 24$ on the
rank-$24$ lattice $\Lambda_{\mathrm{Muk}} = \mathrm{II}_{4,20}$ arises as
the $E_2$-chiral output of the Volume~III CY-to-chiral correspondence
...
\[
  ...
  8,
\]
yielding the Theorem-C enlargement
$\kappa + \kappa^! \in \{0,\,8,\,13,\,250/3,\,98/3\}$ on the
Lorentzian-lattice-parametric $\mathcal{B}$-family.

At $d = 3$ the $K3 \times E$ geometry produces a BKM superalgebra
$\mathfrak{g}_{\Delta_5}$ whose denominator is the Gritsenko--Nikulin
Igusa cusp form $\Delta_5$ of weight $5$ on $\mathrm{O}^+(3,2)$; the
quantum-group deformation is \emph{not} a Drinfeld Yangian but a
K3 chiral Hall--Drinfeld double
$\mathcal{D}_\hbar(\mathcal{Y}^{\mathrm{Hall}}_\hbar(\mathrm{CoHA}_{K3\times E}))$
with Schiffmann--Vasserot shuffle presentation. The Igusa form $\Delta_5$
is its classification invariant:
$H^2(\mathfrak{g}_{\Delta_5})^{\mathbb{Z}/2, K(1)} \cong \mathbb{C} \cdot \Delta_5$
(Etingof--Kazhdan super-category extension; Bruinier $2002$
Proposition~$5.1$ Heegner Chern-class reciprocity).
...
output; the BKM extension at $d = 3$ is a conjectural
Hall--Drinfeld object whose classification invariant is $\Delta_5$;
the $\mathcal{B}$-family $K^\kappa = 8$ entry enlarges the shadow-tower
menu by a fifth item. The universal identity $\hbar^2 \cdot K^\kappa = -1$
on this family pins the Lusztig $u_\zeta$ specialisation at $\ell = 8$,
matching the monodromy order of the Humbert divisor $H_1 \subset
\overline{\mathcal{A}_2}$ (Volume~III Theorem~\texttt{thm:humbert-order-K-kappa}).
