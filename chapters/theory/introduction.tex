...
and the four shadow classes are characterized by a formality
criterion: Swiss-cheese formality ($m_k^{\mathrm{SC}} = 0$ for
...
$F_g = \kappa \cdot \lambda_g$ is exact at all genera without
the uniform-weight hypothesis: the cross-channel correction
$\delta F_g^{\mathrm{cross}}$ vanishes identically
(Proposition~\ref{prop:free-field-scalar-exact};
ALL-WEIGHT free-field exact case).


The shadow depth $r_{\max}$ should not be confused with the \emph{generating depth} $d_{\mathrm{gen}}$, which is the smallest arity at which all higher operations are determined recursively, nor with the \emph{algebraic depth} $d_{\mathrm{alg}}$, which records whether the tower terminates (Definition~\ref{def:generating-depth}).  Virasoro has $d_{\mathrm{gen}} = 3$ but $d_{\mathrm{alg}} = \infty$: $m_3$ generates all higher operations, yet none vanish.
...
The single claim that organizes the five theorems is this.
...
Theorem~A supplies the bar-cobar adjunction; its part~(1) gives the
unit and counit quasi-isomorphisms, while the Verdier intertwining
of part~(2) unpacks the Verdier clause built into
Definition~\ref{def:chiral-koszul-pair}.
Theorem~B inverts the transform at genus $g \ge 1$ via the inductive
genus step; at genus~$0$ the counit quasi-isomorphism follows
directly from any of the equivalent characterisations of chiral
Koszulity assembled in
Theorem~\ref{thm:koszul-equivalences-meta}.
Theorem~C gives the family-specific holographic sum rule
$\kappa(\cA) + \kappa(\cA^!) = K_\cA$ (AP24: zero for affine
Kac--Moody and free fields, fixed nonzero for Virasoro and the
principal $\mathcal{W}$-algebras). Theorem~D identifies $\kappa$
as the Brown--Henneaux one-loop coefficient of the bulk partition
function on the uniform-weight lane (AP32); the multi-weight
correction $\delta F_g^{\mathrm{cross}}$ at $g \ge 2$ is a
separate higher-loop term. Theorem~H, at generic level on the
Koszul locus, bounds the total state space of the bulk algebra to
four states, forcing holographic tightness.
