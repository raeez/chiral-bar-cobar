...
(Theorem~\ref{thm:uc-trinity}) records that
\[
...
(Theorem~\ref{thm:uc-trinity}, ghost-identity clause) is the closed form
\[
...
Theorem~C ($\kappa+\kappa^{!}=0$ for KM/free, and on the
$\mathcal{W}$-side
$\kappa(\cA)+\kappa(\cA^{!})=\varrho_\cA K(\cA)$; witness values are
$13$ for Virasoro, $250/3$ for $\cW_3$, $98/3$ for
Bershadsky--Polyakov) are evaluations of the same functor through the family-specific
...
%     (universal_conductor_K_platonic.tex:376)
%   thm:ghost-identity-platonic -> thm:uc-trinity (same theorem; the
%     ghost identity K = -c_ghost(BRST(A)) is the uc-trinity's
%     ghost-identity clause, inscribed alongside K_E=K_c=K_g).
