...
\section{K3 and arithmetic frontier: comparison obligations}
...
\ClaimStatusProvedHere
Write (e_6(0)=1), (e_6(j)=-504\sigma_5(j)) for (j>0), and

\[
 a_n=\sum_{m=1}^{n}\tau(m)e_6(n-m),
 \qquad \lambda_p=a_p+p^8+p^9.
\]
Exact integer evaluation on the (46) primes (p\leq199) gives the
zero residual vector

\[
 \bigl(\lambda_p-a_p-p^8-p^9\bigr)_{p\leq199}=0\in\mathbb Z^{46}.
\]
This is a finite evaluation of the local-factor formula above, not an
independent proof of that formula.
\end{remark}
...
\end{enumerate}
Completion of these four constructions produces a global coderived inversion
