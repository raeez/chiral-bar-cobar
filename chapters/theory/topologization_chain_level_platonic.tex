...
\label{rem:topol-platonic-brst-conformance}
\index{topologization scope!chain level vs cohomological}
\index{topologization scope!chain level vs cohomological}
...
\item \textbf{Class-restriction.}
 The full original-complex chain-level theorem is conditional
 for class~$\mathbf{L}$ (affine Kac--Moody at non-critical level);
 the strict unary primitive and cubic cancellation are proved.
 Class~$\mathbf{G}$
...
promoted to $\Etwo^{\mathrm{top}}$. After the
Costello--Gwilliam bulk-extraction bridge, Dunn additivity with the
transverse single-coloured $\Eone^{\mathrm{top}}$ from the
$\bR$-direction yields $\Ethree^{\mathrm{top}}$ on cohomology.
...
\begin{enumerate}
 ...
 \item cubic obstruction cancellation for class~$L$ by
 Jacobi, with all-arity original-complex topologization
 conditional on the separate higher-brace coherence
 condition;
...
\begin{definition}[Normal ordering in the CFG model; \ClaimStatusDefinitional]
\label{def:no-cfg}
...
\begin{convention}[Conventions; \ClaimStatusDefinitional]
...
\begin{remark}[Normal-ordering associativity; \ClaimStatusProvedElsewhere]
\label{rem:NO-assoc}
...
\begin{theorem}[Cubic rectification and conditional
$\Ethree^{\mathrm{top}}$ for class $L$]
...
$Z^{\mathrm{der}}_{\mathrm{ch}}(V_k(\fg))$ has the strict
unary identity
$[Q_{\mathrm{tot}},\widetilde G_1]=T_{\mathrm{Sug}}$, and its
cubic brace defect vanishes by the Jacobi identity. If the
higher-brace coherence condition $\mathsf{HBC}_L(\fg,k)$ holds,
namely if there exist corrections $G_{\geq2}$ with
$[m,\widetilde G_1+G_{\geq2}]=\partial_z$ in the brace
deformation complex, then the derived chiral centre carries a
chain-level $\Ethree^{\mathrm{top}}$-algebra structure on the
original BRST complex.
\end{theorem}
...
Degrees $\ge 3$: shadow depth $r_{\max}=3$ means that the
scalar $\Sigma$-coinvariant shadow invariants vanish in degrees
$\ge4$. It does not force the higher brace components
$D^{(1)}_n$ to vanish, because those components live before the
shadow projection and may contain iterated $m_2$-compositions.
Thus the all-arity original-complex statement is conditional on
$\mathsf{HBC}_L(\fg,k)$.

Assuming $\mathsf{HBC}_L(\fg,k)$, choose $G_{\geq2}$ with
$[m,\widetilde G_1+G_{\geq2}]=\partial_z$ on the brace
deformation complex. By Lurie's recognition theorem
...
Theorem~\ref{thm:chain-level-E3-top-class-L} separates the proved
chain-level primitive from the conditional all-arity upgrade. The
...
model on a quasi-isomorphic complex. Theorem~\ref{thm:chain-level-E3-top-class-L}
proves the strict original-complex unary identity and the cubic
Jacobi cancellation. It gives the full structure on the
\emph{original} complex
$Z^{\mathrm{der}}_{\mathrm{ch}}(V_k(\fg))$ without a model change
only under $\mathsf{HBC}_L(\fg,k)$.
\end{remark}
...
Construction~\ref{constr:G1-recall} gives the displayed formulas.
On the basis $(e,f,h)$, lowering the first index with
$\kappa(e,f)=1$ and $\kappa(h,h)=2$ makes $f_{abc}$ totally
antisymmetric; the $3^4=81$ basis instances of Jacobi then vanish.
The same substitution gives the prefactor $1/6$, central charge
...
\begin{enumerate}
 ...
 \item Dunn additivity with the transverse single-coloured
 $\Eone^{\mathrm{top}}$ from $\bR$ produces
...
Theorem~\ref{thm:chain-level-E3-top-class-L} is the class $L$
(affine Kac--Moody at non-critical level) cubic rectification and
conditional original-complex theorem. The other
shadow classes:
\begin{itemize}
...
available conditional statement on the original complex, confined
to class $L$ at non-critical level.
\end{remark}
...
Theorem~\ref{thm:chain-level-E3-top-class-L} supplies the
strict primitive $\widetilde G_1$ and the cubic Jacobi
cancellation. Its lift of CFG's cohomological result to the
original complex is conditional on $\mathsf{HBC}_L(\fg,k)$.
\end{remark}
...
Theorem~\ref{thm:chain-level-E3-top-class-L} establishes the strict
unary Sugawara primitive and the cubic Jacobi cancellation for
affine Kac--Moody at non-critical level, and gives strict
chain-level $\Ethree^{\mathrm{top}}$-topologization on the original
complex under $\mathsf{HBC}_L(\fg,k)$. The proof is concrete and
...
and the cubic harmonic defect vanishes by Jacobi
...
complex under $\mathsf{HBC}_L$) with class $G, C$ (original
complex, simpler arguments),
...
The cohomological comparison is supplied by the BV factorization trace
of CFG~2602.12412.  On the chain level, the preceding $\mathfrak{sl}_2$
calculation establishes antisymmetry and Jacobi on the basis
$(e,f,h)$, while the Sugawara--current commutation is the
Kac--Wakimoto identity.  These are differently typed statements: the
cohomological trace does not replace the chain-level calculation.
