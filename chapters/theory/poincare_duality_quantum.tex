...
The universal MC class $\Theta_\cA$ (Theorem~\ref{thm:explicit-theta}) mediates genus-graded Koszul duality: the Verdier involution exchanges the eigenspace contributions of $\Theta_\cA$ and $\Theta_{\cA^!}$, producing the Lagrangian polarization of the quantum corrections across the genus tower.  At the scalar level, $\kappa(\cA) + \kappa(\cA^!) = 0$ for Kac--Moody and free-field pairs; for $\mathcal{W}$-algebras, the sum is a nonzero constant (Theorem~C).
\end{remark}
...
On the Koszul locus, $\mathcal{A}^!$ is computed by the cohomology of the bar complex:
\[\mathcal{A}^! \;\simeq\; H^*(\bar{B}(\mathcal{A}))^{\vee}\]
with the Verdier dual providing the algebra structure.
In general (off the Koszul locus), $\mathcal{A}^!_\infty = \mathbb{D}_{\mathrm{Ran}}\bar{B}(\mathcal{A})$ carries a homotopy algebra structure.
...
\begin{proposition}[\texorpdfstring{$\beta\gamma$}{beta-gamma} bar complex coalgebra; \ClaimStatusProvedHere]\label{conj:bg-bar-coalg}\label{prop:bg-bar-coalg}
...
\begin{proof}
Part~(3) is a special case of Theorem~\ref{thm:bar-cobar-isomorphism-main}: the bar complex of \emph{any} augmented chiral algebra carries a factorization coalgebra structure, and coassociativity is the operad axiom for the boundary strata of~$\overline{C}_n(X)$.  Part~(1) is the explicit formula for the deconcatenation coproduct on $T^c(s^{-1}\overline{\beta\gamma})$ restricted to the generators $\beta_{-m-\lambda}$, $\gamma_{-m-1+\lambda}$; the normal-ordering statement follows from the standard basis of the tensor coalgebra.  Part~(2) is a character computation: the augmentation ideal $\overline{\beta\gamma}$ has generators of conformal weights $m + \lambda$ ($\beta$ modes) and $m + 1 - \lambda$ ($\gamma$ modes) for $m \geq 0$, so the generating function of the cofree coalgebra $T^c(s^{-1}\overline{\beta\gamma})$ at bar degree~$n$ and weight~$h$ is the stated plethystic product.  The infinite-dimensionality of~$\bar{B}^n$ at unfixed weight is standard for any chiral algebra with infinitely many modes; the positive-energy axiom ensures finite-dimensionality at each weight.
\end{proof}
...
The Feynman transform $\mathrm{FT}_{\cM\!od}(\cA)$ is a
coalgebra.  Its coderivation complex is the deformation-theoretic
dual: the dg~Lie algebra whose MC elements are the algebraic
structures that the Feynman transform encodes.  We define this
algebra and show that its genus-$0$ shadow is the tree-level chiral
convolution.
...
arity~$r$ gives the arity-$r$ shadow.
\end{proof}
