...
\subsection{Holographic Paradigm: Genus-Graded Koszul Duality}
...
\textbf{Koszul dual is $\beta\gamma$:}
...
where $h^\vee$ is the dual Coxeter number. For the Heisenberg subalgebra alone ($h^\vee = 0$), the Koszul dual is $\mathcal{H}_\kappa^! \simeq \mathrm{Sym}^{ch}(V^*)$ (Theorem~\ref{thm:heisenberg-koszul-dual-early}).  The bar complex $\bar{B}(\mathcal{H}_\kappa)$ is the Chevalley--Eilenberg complex $\mathrm{CE}(\hat{\mathfrak{h}}_\kappa)$, but this is the \emph{bar construction}, not the Koszul dual itself.

The curvature element $m_0 = \kappa \cdot \omega_X$ (where $\omega_X$ is the canonical class) satisfies $m_1^2 = [m_0, -]$ on the $A_\infty$ structure; the bar differential itself always satisfies $d_B^2 = 0$ (cf.\ Theorem~\ref{thm:heisenberg-bar}).
\end{example}
...
In the Drinfeld ``new'' presentation, the Yangian $Y_\hbar(\mathfrak{gl}_N)$ has generators $\{T_{ij}^{(r)}\}_{1 \leq i,j \leq N,\, r \geq 0}$ with $T_{ij}^{(0)} = \delta_{ij}$ and the \textbf{RTT relation}:
...
\subsubsection{\texorpdfstring{Chiral Coalgebra Structure for $\beta\gamma$}{Chiral Coalgebra Structure for}}
...
\item \textbf{Growth:} The bar complex is bigraded by bar degree $n$ and conformal weight $h$.  At each conformal weight, $\bar{B}^n_h$ is finite-dimensional with generating function
...
\item \textbf{Coassociativity:} Follows from the factorization property of the FM operad.  The boundary divisors $D_S \subset \overline{C}_{n}(X)$ parametrizing collision of a subset $S$ of points induce maps
\[
...
\item \textbf{Genus~$0$:} The chiral operad
...
\item \textbf{Bar as operadic bar:} For a chiral algebra $\mathcal{A}$
...
\item \textbf{Cobar as operadic cobar:} Dually, the geometric cobar
...
\item \textbf{Bar-cobar resolution:} For a Koszul chiral algebra
...
\begin{proof}
\textbf{Part (i): HyCom--Grav Koszul duality.}
...
\textbf{Part (ii): Identification of the bar complex.}
...
\textbf{Part (iii): Cobar via Verdier duality.}
...
\textbf{Part (iv): Bar-cobar resolution.}
...
A \textbf{modular operad} (Getzler--Kapranov \cite[\S5]{GeK98})
...
\item \textbf{Separating gluing maps}: for each partition
...
\item \textbf{Non-separating gluing maps}: for each pair of
...
These maps satisfy:
\begin{itemize}
\item \textbf{Equivariance}: compatibility with the $\Sigma_n$-actions;
\item \textbf{Associativity}: iterated gluings are independent of
   the order in which they are performed;
\item \textbf{Commutativity}: separating gluing is symmetric in
   the two factors.
\end{itemize}
...
over $\mathcal{M}od$.  The \textbf{Feynman transform}
...
\begin{proof}
\textbf{Part (i)} is the foundational result of
...
\begin{enumerate}[label=\textup{(\roman*)}]
\item \textbf{Genus-$0$ spectrum:} The bar complex extracts the
...
\item \textbf{Higher-genus harmonics:} At genus $g \geq 1$, the
  ...
  so forth.
\item \textbf{Reconstruction:} The cobar construction
...
\begin{enumerate}
\item \textbf{Translation and scale invariance:} Under $z \mapsto az + b$, $\eta_{ij} \mapsto \eta_{ij}$
