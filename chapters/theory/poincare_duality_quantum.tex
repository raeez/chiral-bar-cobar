...
\emph{Modularity at chain level}: at the scalar projection,
$\kappa(\cA) + \kappa(\cA^!) = 0$ for Kac--Moody and free-field
pairs, while $\kappa(\cA) + \kappa(\cA^!) = \rho \cdot K$ for
$\mathcal{W}$-algebras (Theorem~C).  The full chain-level
complementarity lives in the modular convolution algebra.
...
The universal MC class $\Theta_\cA$ (Theorem~\ref{thm:explicit-theta}) mediates genus-graded Koszul duality: the Verdier involution exchanges the eigenspace contributions of $\Theta_\cA$ and $\Theta_{\cA^!}$, producing the Lagrangian polarization of the quantum corrections across the genus tower.  At the scalar level, $\kappa(\cA) + \kappa(\cA^!) = 0$ for Kac--Moody and free-field pairs; for $\mathcal{W}$-algebras, $\kappa(\cA) + \kappa(\cA^!) = \rho \cdot K$ where $\rho$ is the anomaly ratio and $K$ is the Killing form normalization (Theorem~C; e.g.\ $\kappa(\mathrm{Vir}_c) + \kappa(\mathrm{Vir}_{26-c}) = 13$).
\end{remark}
...
\emph{Caution \textup{(}AP33\textup{)}}: the Feigin--Frenkel involution
...
itself always satisfies $d_{\bar{B}}^2 = 0$ (cf.\ Part~II).
\end{example}
