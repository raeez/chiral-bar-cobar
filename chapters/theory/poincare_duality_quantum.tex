...
$\mathcal{D}(\cA) = \mathbb{D}_{\Ran}\barBch(\cA)$ is the Verdier
dual of the bar coalgebra, carrying the homotopy Koszul dual
$\cA^!_\infty$ as its algebra structure. This
...
conjecturally related by genus-graded Koszul duality. The pairing
is the codomain of the Koszul Reflection
$K = \Bbarch_X$ of Theorem~\ref{thm:koszul-reflection} composed with
Verdier duality $\mathbb{D}_{\Ran}$; the involutivity
$K^{2}\simeq\mathrm{id}$ on the Koszul locus
(Theorem~\ref{thm:koszul-reflection})
delivers the genus-zero strict pairing
$\mathcal{D}(\cA) \simeq \cA^!$, while the higher-genus tower lifts
this involutivity to a genus-graded statement whose scalar shadow
is the Trinity conductor identity
$K(\cA) = c(\cA) + c(\cA^!) = -\,c_{\mathrm{ghost}}(\BRST(\cA))$
(Chapter~\ref{chap:universal-conductor}); the family-dependent
companion $\kappa(\cA) + \kappa(\cA^!) = \varrho_{\cA} \cdot K(\cA)$
relates the $\kappa$-sum to the Trinity $K$ via the per-family
anomaly ratio $\varrho_{\cA}$.
...
\emph{Modularity at chain level}: at the scalar projection, the
complementarity sum is family-dependent (Theorem~C).
For Kac--Moody and free-field pairs, $\kappa(\cA) + \kappa(\cA^!) = 0$.
For Virasoro (self-dual at $c = 13$), the Koszul partner is
$\mathrm{Vir}^!_c = \mathrm{Vir}_{26-c}$ and the sum
$\kappa(\mathrm{Vir}_c) + \kappa(\mathrm{Vir}_{26 - c}) = 13$.
For $\mathcal{W}$-algebras, $\kappa(\cA) + \kappa(\cA^!) = \rho \cdot K$,
where $\rho$ is the anomaly ratio and $K$ the Killing-form
normalization. The full chain-level complementarity lives in the
...
\emph{Close the loop}: the Verdier dual
$\mathcal{D}(\cA) = \mathbb{D}_{\Ran}\barBch(\cA) \simeq \cA^!$ on
the Koszul locus provides the dual algebra, while bar-cobar inversion
(Theorem~B) gives $\Omega(\barBch(\cA)) \simeq \cA$, recovering the
original algebra. These are distinct functors applied to the same
input.

\subsection{Holographic principle: genus-graded Koszul duality}

\begin{remark}[Costello--Li holographic conjecture across genera]\label{rem:costello-li}
For $N$ D-branes, the genus-graded boundary operator algebra (at $N \to \infty$) and the twisted supergravity bulk algebra are conjecturally related by genus-graded Koszul duality~\cite{CostelloGaiotto2020,FernandezCostelloP24}.
\end{remark}

\begin{remark}[Genus-graded Koszul duality]
The universal MC class $\Theta_\cA$ (Theorem~\ref{thm:explicit-theta}) mediates genus-graded Koszul duality: the Verdier involution exchanges the eigenspace contributions of $\Theta_\cA$ and $\Theta_{\cA^!}$, producing the Lagrangian polarization of the quantum corrections across the genus tower. At the scalar level, the complementarity sum is family-dependent (Theorem~C): $\kappa(\cA) + \kappa(\cA^!) = 0$ for Kac--Moody and free-field pairs; for Virasoro (self-dual at $c = 13$), $\kappa(\mathrm{Vir}_c) + \kappa(\mathrm{Vir}_{26-c}) = 13$; and for $\mathcal{W}$-algebras, $\kappa(\cA) + \kappa(\cA^!) = \rho \cdot K$ where $\rho$ is the anomaly ratio and $K$ is the Killing form normalization.
\end{remark}
...
\item \emph{Geometric realization:} $\mathcal{D}(\mathcal{A}) \cong \mathbb{D}_{\mathrm{Ran}}\bar{B}(\mathcal{A})$ (Verdier dual of bar, \emph{not} cobar of bar; see Theorem~\ref{thm:defect-koszul})
\end{enumerate}
\end{definition}
...
On the Koszul locus, the strict Koszul dual $\mathcal{A}^!$ is
computed by the linear dual of bar cohomology:
\[\mathcal{A}^! \;\simeq\; H^*(\bar{B}(\mathcal{A}))^{\vee},\]
with algebra structure from the cup product on bar cohomology.
In general (off the Koszul locus), the homotopy Koszul dual
$\mathcal{A}^!_\infty = \mathbb{D}_{\mathrm{Ran}}\bar{B}(\mathcal{A})$
is obtained via Verdier duality and carries a homotopy algebra
structure. The compatibility
$H^*(\mathcal{A}^!_\infty) \simeq \mathcal{A}^!$ on the Koszul
locus is the content of Theorem~A
(Convention~\ref{conv:bar-coalgebra-identity}), not a tautology.

\emph{Caution.} The cobar construction $\Omega(\bar{B}(\mathcal{A}))$
recovers $\mathcal{A}$ itself \textup{(}bar-cobar inversion, Theorem~B\textup{)}, \emph{not} $\mathcal{A}^!$. The Koszul dual is obtained via Verdier duality, not cobar.
\end{theorem}
...
\]
\item the completed bar construction produces a curved coalgebra
$\hat{\bar{B}}(\mathcal{A})$, and Verdier duality yields the curved
homotopy Koszul dual
$\mathcal{A}_{\mathrm{curv}}^{!\,\infty}
= \mathbb{D}_{\mathrm{Ran}}(\hat{\bar{B}}(\mathcal{A}))$;
\item the completed bar-cobar adjunction gives the corresponding derived
equivalence in the curved setting (the cobar recovers $\mathcal{A}$
itself, not the dual).
\end{enumerate}
\end{theorem}
...
\emph{Caution }: the Feigin--Frenkel involution
...
Koszul duality produces $(\widehat{\mathfrak{g}}_k)^! = H^*(\bar{B}(\widehat{\mathfrak{g}}_k))^\vee$, a different algebra whose modular
characteristic satisfies $\kappa((\widehat{\mathfrak{g}}_k)^!) =
-\kappa(\widehat{\mathfrak{g}}_k)$ (Theorem~C). The two operations
produce algebras with the \emph{same} $\kappa$-value but different OPE
structures.
...
$m_1^2 = [m_0, -]$ on the $A_\infty$ structure; the bar differential
itself always satisfies $d_{\bar{B}}^2 = 0$ (cf.\ Part~\ref{part:characteristic-datum}).
\end{example}
...
$D_0$ preserves genus and $D_1 = \hbar\Delta$ raises it by one.
...
\leavevmode\\
Throughout this conjecture \emph{bulk} is the $3$d HT bulk of T18(iii):
an AdS$_3$-side gravitational / higher-spin gauge theory whose
asymptotic boundary carries the chiral algebra $\cA_{\mathrm{bdy}}$;
it is related to but not equal to the chiral derived centre
$Z^{\mathrm{der}}_{\mathrm{ch}}(\cA_{\mathrm{bdy}}) = T18(\mathrm{i})$,
with equality demanded only by the conjecture below.
For appropriate chiral algebra pairs
$(\mathcal{A}_{\mathrm{bdy}}, \mathcal{A}_{\mathrm{bulk}})$,
the following dictionary is expected:
boundary observables and $3$d HT bulk amplitudes are related by the square below.
...
\item The bar-cobar adjunction is conjecturally interpreted as the
 algebraic shadow of holographic duality: bar classifies couplings,
 center acts as bulk
\end{enumerate}

\emph{Example.} For $\mathcal{A}_{\text{bdy}} = \mathcal{W}_{\infty}[\lambda]$ at $c = N$:
\begin{itemize}
...
\item Derived center: should organize the same higher-spin interaction
 ...
 (the bar complex provides the coalgebraic input, but the bulk
 observable algebra is the center, not the bar complex itself)
...
\label{rem:pdq-1}
The Mukai pairing on $\Lambda_{\mathrm{Muk}} = H^\star(K3,\mathbb Z) \cong II_{4,20}$
is the quantum-Poincar\'e-duality pairing on $\mathbf H_{\Delta_5}$.
...
Cross-ref Chapter~\ref{ch:k3-chiral-bialgebra-platonic}. The
chain-level realisation of the Mukai form as a non-degenerate
...
shadow tower is $c_{2d} = -2\cdot 107$ with $107$ prime, reflecting
the first-principles structure $5n - 13$ at $n = 24$ rather than a
Mukai-signature decomposition. The quantum-Poincar\'e-duality reads
...
first-principles $-214$, not the withdrawn intermediate value $-312$.
Cross-ref Vol.~III Chapter~\ref{ch:k3-chiral-bialgebra-platonic}
\S on class-$\mathcal S$ AGT chiral algebra, and
Vol~I Chapter~\ref{ch:thqg-open-closed-realization} line~1890.
\end{remark}

\begin{remark}[Witten DNA: DMVV / Nakajima-BPS character of
$\mathbf{H}_{\Delta_{5}}$]
...
multiplicative lift $1/\Phi_{10}(Z)$ in variables
...
Theorem~\ref{thm:k3-siegel-character} shows that the genus-$2$ Siegel
character of the K3 chiral bialgebra
$\mathbf{H}_{\Delta_{5}}$
(Vol~III Chapter~\ref{ch:k3-chiral-bialgebra-platonic}
Definition~\ref{def:k3-chiral-bialgebra}) is $1/\Phi_{10} =
...
of symmetric-product-K3 Hilbert spaces. The pairing
$\mathcal{D}(\mathbf{H}_{\Delta_{5}}) \simeq
\mathbf{H}_{\Delta_{5}}^{\,!}$ bosonises this BPS generating function:
...
quantum-Poincar\'e-duality pairing class of
Theorem~\ref{thm:defect-koszul} specialised to the K3-input; its induced
pairing on $Z_{\mathrm{der}}^{\mathrm{ch}}(\mathbf{H}_{\Delta_{5}})$ is
the bulk side of the holographic pairing, matching the BPS microstate
count of Maldacena--Moore--Strominger 1999 \texttt{hep-th/9903163} for
the D1-D5 system on $K3 \times S^{1}$. The ``non-abelian'' attribute of
$\mathbf{H}_{\Delta_{5}}$ (Vol~III Theorem~\ref{thm:k3-abelian-at-lie})
...
Theorem~\ref{thm:bkm-simple-roots-walls}; Vol~II
\texttt{chapters/connections/modular\_pva\_quantization\_frontier.tex}
Remark~\ref{rem:witten-het-iia-mukai-narain} (Het/IIA duality frame).
\end{remark}
