...
In the Drinfeld ``new'' presentation, the Yangian $Y_z^\hbar(\mathfrak{gl}_N)$ has generators $\{T_{ij}^{(r)}\}_{1 \leq i,j \leq N,\, r \geq 0}$ with $T_{ij}^{(0)} = \delta_{ij}$ and the \emph{RTT relation}:
