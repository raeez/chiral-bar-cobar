...
Universal chiral defects arise when the NAP duality of the previous chapter is deformed by curvature: the Verdier pairing acquires quantum corrections, and the resulting defect class is the universal Maurer--Cartan element~$\Theta_\cA$.
\end{remark}
...
Loday--Vallette \cite[Chs.~10--11]{LV12} and
Positselski \cite[Chs.~6--7]{Positselski11}.
\end{enumerate}
\emph{Status note.}
...
\end{enumerate}
\emph{Status note.}
This theorem is tagged \ClaimStatusProvedElsewhere (algebraic statement; see Positselski~\cite{Positselski11} and Gui--Li--Zeng~\cite{GLZ22}). The
...
\begin{theorem}[\texorpdfstring{$\beta\gamma$}{beta-gamma} bar complex coalgebra; \ClaimStatusProvedHere]\label{thm:bg-bar-coalg}
