...
Ran/Fulton--MacPherson compactification.  The theorem has the following
typed layers.  For a quadratic presentation
$\cA=T_X(V)/(R)$ they are:
\[
\cA,\qquad
\barBch_X(\cA),\qquad
\cA^{\mathrm i}:=C_X(s^{-1}V,s^{-2}R),\qquad
H^\bullet\barBch_X(\cA),\qquad
K_X(\cA):=\mathbb D_{\Ran}\barBch_X(\cA),\qquad
\cA^!,\qquad
...
\[
q_\cA\colon\cA^{\mathrm i}\to\barBch_X(\cA),\qquad
...
Theorem~C applies Verdier duality to the strict-flat
centre-local-system complex
\[
 \mathbf C_g(\cA)
 =R\Gamma(\overline{\mathcal M}_g,\mathcal Z(\cA)).
\]
Its normalized scalar trace is supplied by Theorem~D.  For the standard
witnesses,
\[
...
$N\to\infty$ and the twisted supergravity BRST algebra are conjectured
...
\begin{theorem}[Verdier defect and Koszul dual {\cite{LV12, Positselski11}}; \ClaimStatusConditional]\label{thm:defect-koszul}
...
$\mathbb{D}_{\Ran}\barBch_X(\mathcal A)$ produces the
...
\begin{theorem}[Curved Koszul duality (algebraic form) {\cite{Positselski11, GLZ22}}; \ClaimStatusConditional]\label{thm:curved-koszul}
...
\item Verdier duality yields the curved Verdier--Koszul algebra
\[
K_X^{\mathrm{II}}(\mathcal A)
=
\mathbb{D}_{\mathrm{Ran}}\widehat{\bar{B}}(\mathcal{A}),
\]
whose multiplication and curvature are the continuous Verdier
transposes of the bar coproduct and curvature functional;
...
Verdier part produces $K_X^{\mathrm{II}}(\mathcal A)$ directly.
\end{enumerate}
\end{theorem}
...
\begin{example}[Affine Kac--Moody: Koszul duality vs critical-level reflection]
...
\emph{distinct} operations. The reflection replaces the level within
the \emph{same} family of affine algebras; chiral
...
\]
on the finite or completed Koszul surface. Both the reflected
companion $\widehat{\mathfrak g}_{-k-2h^\vee}$ and the Koszul dual have
...
$\cH_k^!\simeq
(\mathrm{Sym}^{\mathrm{ch}}(V^*[1]),m_0=-k\omega_X)$
(Part~\ref{part:characteristic-datum}). This is \emph{not} the same as
...
\begin{proposition}[\texorpdfstring{$\beta\gamma$}{beta-gamma} bar complex coalgebra; \ClaimStatusConditional]\label{prop:bg-bar-coalg}
...
\ClaimStatusConditional]\label{thm:prism-operadic}
...
\begin{definition}[Modular operad; \ClaimStatusDefinitional]\label{def:modular-operad}
...
\begin{definition}[Feynman transform; \ClaimStatusProvedElsewhere]\label{def:feynman-transform}
...
propagator~$k$.  Under~$H_D^K$ its Hodge character is
$\operatorname{ch}_g(\mathfrak O_g^K(\mathcal H_k))=
(-1)^gk\lambda_g$, and under~$H_D^{\mathrm{graph}}$ integration
against the Faber--Pandharipande class gives the
coefficient computed in~\S\ref{sec:frame-genus-tower}.
\end{example}

\begin{corollary}[The prism principle; \ClaimStatusConditional]\label{cor:prism-principle}
...
\arrow[d, "\text{closed-sector action}"] &
\Obs_{\mathrm{BRST}}(\mathcal A_{\mathrm{bulk}}^{3d})
\arrow[d, "\text{amplitudes}"] \\
\text{Boundary observables}
\arrow[r, "\text{open--closed}"] &
\text{Intrinsic closed-sector operators}
...
Specifically:
\begin{enumerate}[label=\textup{(\roman*)}]
\item The boundary-to-intrinsic-closed-sector map is the centre map
...
couplings.  Ran--Verdier duality forms
$K_X(\cA_{\mathrm{bdy}})=\mathbb D_{\Ran}
\barBch_X(\cA_{\mathrm{bdy}})$, and a chosen-pair map
$\nu_{\cA_{\mathrm{bdy}}}\colon K_X(\cA_{\mathrm{bdy}})
\to\cA_{\mathrm{bdy}}^!$ reaches the defect/Koszul-dual lane.
...
\end{enumerate}
The K3 chiral-bialgebra comparison places
$\kappa_{\mathrm{ch}},\kappa_{\mathrm{cat}},\kappa_{\mathrm{BKM}}$,
and~$\kappa_{\mathrm{fiber}}$ in four distinct measurement targets
\cite{Mukai1987,Bruinier2002,Borcherds1998}.  Theorem~C remains the
Verdier decomposition of
$R\Gamma(\overline{\mathcal M}_g,\mathcal Z(\mathbf H_{\Delta_5}))$;
Theorem~H remains the support theorem for
$\ChirHoch^\bullet(\mathbf H_{\Delta_5},\mathbf H_{\Delta_5})$.
A supplied $\iota_Z^{\mathrm{der}}$, followed by~$H^0$ and C0, joins
these two surfaces.
