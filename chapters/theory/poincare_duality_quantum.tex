...
identities. The holographic assertion adds the identification of bar
generators with bulk higher-spin fields and of cobar reconstruction
with boundary correlators. Those two identifications are the
AdS$_3$/CFT$_2$ input.
\end{remark}

\begin{remark}[Algebraic core and external comparison]
The algebraic core of Conjecture~\ref{conj:holographic-koszul} is the
completed M-level bar-cobar package. Its finite stages give the
coefficient identities used by the boundary $\mathcal W$-algebra.
A filtered H-level higher-spin bulk target belongs to the MC4
construction problem. The comparison with Witten diagrams and bulk
fields is an MC5 physics-dictionary problem. Thus the conjecture is
not used as input to the bar-cobar theorem; it is an application lane
for the theorem after the Gaberdiel--Gopakumar and
AdS$_3$/CFT$_2$ identifications are supplied.
\end{remark}
