...
The governing question of this chapter is the mathematical role of \emph{Koszul duality and universal chiral defects}: which part of the modular bar-cobar construction or its inversion it isolates, and which boundary between construction, resolution, and shadow it clarifies.
\end{remark}
...
The genus expansion and Feynman transform structure computed for the Heisenberg algebra in Chapter~\ref{ch:heisenberg-frame} extend to arbitrary Koszul chiral algebras via the modular operad of stable curves $\overline{\mathcal{M}}_{g,n}$: curvature classes enter through the Hodge bundle, and Koszul duality is ``quantized'' to accommodate the genus tower.

\label{rem:quantum-poincare-hms}
...
\emph{The AdS/CFT correspondence, when appropriately twisted, should be modeled by genus-graded Koszul duality.}
...
\end{enumerate}
are conjecturally related by (a deformation of) genus-graded Koszul
duality, with each genus contributing specific modular forms and period
integrals.
\end{remark}

\begin{remark}[Genus-graded Koszul duality]
Genus-graded Koszul duality exchanges generators with relations, commutative with Lie structures (plus modular corrections), classical limits with quantum deformations, and tree-level with loop corrections --- all simultaneously across the genus tower.
The universal MC class $\Theta_{\cA}$
(Theorem~\ref{thm:explicit-theta}) mediates this
simultaneous exchange: it is the single homotopy-algebraic
object whose Verdier dual $\Theta_{\cA^!}$ encodes the dual
genus tower, and the complementarity
$\Theta_{\cA} + \Theta_{\cA^!} = K \cdot \eta \otimes \Lambda$
gives the Lagrangian polarization of the quantum corrections.
\end{remark}
...
\begin{theorem}[Universal defect = Koszul dual {\cite{LV, Positselski}}; \ClaimStatusProvedElsewhere]\label{thm:defect-koszul}
...
Following Costello \cite{Costello-1705.02500v1}, consider $K$ M2 branes at an $A_{N-1}$ singularity in M-theory.
...
\emph{Koszul template.}
$$\text{Yangian}(\mathfrak{gl}_N) \quad \text{should be modeled as the Koszul-dual boundary package to} \quad U_{\hbar,c}(\text{Diff}(\mathbb{C}) \otimes \mathfrak{gl}_N)$$

This is a \emph{curved} Koszul duality with deformation parameter $c$ encoding backreaction.
\end{example}

\begin{theorem}[Curved Koszul duality (algebraic form) {\cite{Positselski, GLZ22}}; \ClaimStatusProvedElsewhere]\label{thm:curved-koszul}
...
\end{enumerate}
\emph{Status note.}
This theorem is tagged \ClaimStatusProvedElsewhere (algebraic statement; see Positselski~\cite{Positselski} and Gui--Li--Zeng~\cite{GLZ22}). The
additional identification of $m_0$ with gravitational backreaction remains
conjectural and is isolated in Conjecture~\ref{conj:backreaction}.
\end{remark}
...
After twisting by a nilpotent supercharge $Q$ with $Q^2 = 0$, one expects the following physics dictionary:

\begin{center}
\begin{tabular}{|c|c|c|}
\hline
\textbf{Boundary} & $\leftrightarrow$ & \textbf{Bulk candidate} \\
\hline
...
The conjectural Koszul template becomes:
...
\begin{remark}[Scope]
Conjecture~\ref{conj:backreaction} is tagged \ClaimStatusConjectured{} because
...
The conjecture asserts that $1/N$ gravitational corrections to the Koszul pairing are captured by curving the $A_\infty$ structure on the bar complex, translating AdS$_3$ backreaction into algebraic deformation data.
...
The bar-cobar adjunction is conjecturally interpreted as an open-closed
string duality template:
...
\item Closed string field theory in bulk $\to$ conjectural Koszul-dual package $\mathcal{A}^!$
\item Disk amplitudes with boundary data in $\mathcal{A}$ should be modeled by sphere amplitudes in the Koszul-dual package
\end{itemize}

Within the frontier order of this monograph, the theorematic content is
the universal defect and bar-cobar machinery itself; any string-theoretic
identification of that machinery with a bulk theory is a downstream MC5
comparison problem.
\end{remark}

\begin{theorem}[Universal defect construction {\cite{LV}}; \ClaimStatusProvedElsewhere]\label{thm:universal-defect-construction}
...
$$\langle \mathcal{O}_1 \cdots \mathcal{O}_n \rangle_{\text{CFT}} \stackrel{\text{conj.}}{=} \int_{\text{AdS}} \mathcal{O}_1^! \cdots \mathcal{O}_n^! \cdot e^{-S_{\text{gravity}}}$$

\emph{Quantum groups from gravity.}
A suitable AdS gravity theory should determine a quantum-group-type
boundary package through the same Koszul template.

\emph{Categorification.}
$$\text{D}^b(\mathcal{A}\text{-mod}) \simeq \text{D}^b(\mathcal{A}^!\text{-mod})^{\text{op}}$$

\emph{Higher spin gravity.}
Vasiliev theory should fit the same conjectural higher-spin Koszul
template.
\end{applications}
...
\begin{definition}[Modular graph coefficient algebra; \ClaimStatusConjectured]
...
\begin{theorem}[Bar-cobar adjunction {\cite{LV}}; \ClaimStatusProvedElsewhere]\label{thm:bar-cobar-adjunction-operadic}
...
The poset $\barPi_n$ has the homology of a wedge of $(n-1)!$ spheres of dimension $n-2$, with the $S_n$-action on the top homology given by the Lie representation tensored with the sign. This follows from the classical results of Björner-Wachs \cite{BW83} and Stanley \cite{Sta97}, who computed:
\[
...
physical input that we do not attempt to supply.  In the repo's frontier
