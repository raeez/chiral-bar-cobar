...
Assume the tangent complex $T_{\mathrm{comp}}(\cA)$ integrates to a
filtered complete cyclic $L_\infty$-algebra presenting a formal moduli
problem $\mathcal{M}_{\mathrm{comp}}(\cA)$.  Then
$\mathcal{M}_{\mathrm{comp}}(\cA)$ carries a canonical $(-1)$-shifted
symplectic structure, and the one-sided deformation problems define
Lagrangian maps
\[
\mathcal{M}_{\cA}
\longrightarrow
...
\begin{theorem}[Critical dimension from shadow connection flatness;
\ClaimStatusProvedHere]
...
be the modular shadow connection.  Then $\nabla^{\mathrm{hol}}$
is flat if and only if the MC equation
$D\Theta_\cA + \tfrac{1}{2}[\Theta_\cA, \Theta_\cA] = 0$ holds.
For a one-parameter family $\cA_c$ parametrised by central
charge~$c$, the scalar shadow connection
$\nabla^{\mathrm{hol}} = d - \kappa \cdot \eta \otimes \Lambda$
degenerates $(\kappa = 0)$ at the following critical values:
\begin{enumerate}[label=\textup{(\roman*)}]
\item $\kappa(\mathrm{Vir}_c) = c/2 = 0$ at $c = 0$.
  The complementarity dual $\mathrm{Vir}_{26-c}$ has
  $\kappa(\mathrm{Vir}_{26-c}) = (26-c)/2 = 0$ at $c = 26$.
\item $\kappa(\mathcal{W}_3^c) = 2c/(c+3)$ vanishes at $c = 0$;
  the dual $(\mathcal{W}_3^c)^!$ has $\kappa^! = 0$ at the
  corresponding critical value.
\end{enumerate}
For the Virasoro algebra: anomaly cancellation
$\kappa(\mathrm{Vir}_c \otimes \mathrm{Vir}_{c'}) = 0$
gives $c + c' = 0$; complementarity
$\kappa + \kappa^! = 0$ gives $c + (26-c) = 26$; the
physical critical dimension is $c = 26$, where the Koszul
dual curvature vanishes.
\end{theorem}

\begin{proof}
Flatness: $(\nabla^{\mathrm{hol}})^2 = 0$ iff
$d\operatorname{Sh}(\Theta_\cA) +
\tfrac{1}{2}[\operatorname{Sh}(\Theta_\cA),
\operatorname{Sh}(\Theta_\cA)] = 0$, which is the MC
equation projected through the shadow functor.

At the scalar level: $\kappa(\mathrm{Vir}_c) = c/2$
(Theorem~D).  $\kappa(\mathrm{Vir}_c^!) =
\kappa(\mathrm{Vir}_{26-c}) = (26-c)/2$.  The scalar
shadow connection is flat iff
$\kappa \cdot (\kappa + \kappa^!) = 0$.  On the Koszul
locus $\kappa + \kappa^! = 0$
(Theorem~\ref{thm:modular-characteristic-main}), so flatness
is automatic.  The curvature of the dual vanishes at $c = 26$:
$\kappa(\mathrm{Vir}_{26-c})|_{c=26} = 0$.  At this value
the dual bar complex $\barB(\mathrm{Vir}_0)$ is uncurved
($d_{\mathrm{fib}}^2 = 0$), and the complementarity
splitting becomes a direct sum without period corrections.
\end{proof}
...
envelope programme
(Conjecture~\ref{conj:platonic-adjunction}), this upgrades
