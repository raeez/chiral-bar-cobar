...
setting, this explains the expected exchange: smooth
$\mathrm{CY}_1$ data on~$\cA$, given by a nondegenerate invariant
bilinear form, should correspond after the bar construction is placed
in a finite or completed Koszul surface to proper $\mathrm{CY}_1$ data
on $\barB(\cA)$. This supports finite cohomology at each fixed finite
...
is therefore a strict cochain complex at every genus, and the
...
\begin{remark}[Separating-node factorisation $1/\Phi_{10}^{\mathrm{un}} \to 1/\eta^{24} \cdot 1/\phi_{10,1}$]
\label{rem:hgc-sepnode}
At $K3 \times E$ the genus-$2$ Siegel character $\mathrm{Ch}(\mathbf{H}_{\Delta_5})(Z) = 1/\Phi_{10}^{\mathrm{un}}(Z)$ on $\mathbb{H}_2$ factorises at a separating-node degeneration $Z \to \mathrm{diag}(\tau, \rho)$ into $1/\eta(\tau)^{24} \cdot 1/\phi_{10,1}(\rho)$, recovering the genus-$1$ $\eta^{24}$ denominator of the K3 chiral algebra times the Jacobi form of the elliptic direction. The convention-qualified square identity $\Phi_{10}^{\mathrm{un}}=\Delta_5^2$ (Gritsenko--Nikulin $1998$, with the leading Fourier--Jacobi coefficient fixing the scalar) gives $\kappa_{\mathrm{BKM}}(\mathfrak{g}_{\Delta_5}) = c_\Lambda(0)/2 = \mathrm{wt}(\Delta_5) = 5$.
\end{remark}
