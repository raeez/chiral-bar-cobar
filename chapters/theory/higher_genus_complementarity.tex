...
Complementarity is the branch structure of the categorical logarithm.
The genus-$g$ quantum corrections to~$\cA$ decompose into a part
visible to the algebra and a complementary part visible to its Koszul
dual: the deformation-obstruction complexes $Q_g(\cA)$ and
$Q_g(\cA^!)$ sit as complementary summands inside the ambient
cohomology $R\Gamma(\overline{\mathcal{M}}_g, \mathcal{Z}(\cA))$.
At the level of Theorem~C this is a direct-sum decomposition; the
ambient complementarity upgrade---developed in the appendix on
nonlinear modular shadows---promotes it to a pair of complementary
Lagrangian subspaces inside a shifted-symplectic ambient complex,
conditional on perfectness and nondegeneracy hypotheses for the
Verdier pairing.  It is in this upgraded geometry that the finite-order
shadow tower~$\Theta_{\cA}^{\leq r}$ acquires its geometric home.
At the homotopy level, the Lagrangian splitting is a decomposition of
the modular $L_\infty$-deformation object $\Definfmod(\cA)$ into
complementary half-dimensional sub-objects; the dg~Lie algebra $L_g$
appearing below is the genus-$g$ stratum of its strict model.

Within the three-ring architecture, complementarity occupies a pivotal
position.  At Ring~1 it is a proved theorem: the decomposition is
direct, exhaustive, and natural in Koszul pairs.  At Ring~2 it becomes
the organizing principle of the shadow Postnikov tower, where the
obstruction class~$o_{r+1}(\cA)$ at each arity measures the failure
of the tower to close at the previous stage.  The interplay between
$Q_g(\cA)$ and~$Q_g(\cA^!)$ is the algebraic engine behind the
matter--ghost pairing of conformal field theory: what~$\cA$ sees as
anomaly, $\cA^!$ sees as deformation, and vice versa.
...
\item \emph{H-level.}  The shadow Postnikov tower
  ...
  its conjectural all-arity limit
  $\Theta_\cA \in
  \mathrm{MC}(\mathfrak{g}_\cA^{\mathrm{mod}})$
  would give a single modular twisting cochain
...
and the proof that the metaplectic anomaly series converges to the
all-arity Maurer--Cartan element $\Theta_{\cA}$.  The shadow tower
provides the finite-order approximations; the bulk enhancement
provides the conjectural limit.
\end{remark}
