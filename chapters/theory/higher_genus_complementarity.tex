...
An additive splitting can always be faked by eigenspace
decompositions, arbitrary direct sums, or linear bookkeeping.
The ambient theorem is harder to fake: it requires a single
deformation problem rather than two unrelated ones, a shifted
symplectic pairing on that problem, isotropicity of the one-sided
packages, maximality of the isotropicity, and compatibility with
clutching, factorization, and duality.  The last item is decisive:
the theorem is not about tangent complexes at one point but about
a modular geometry stable under the basic operations of the theory.
\end{remark}
