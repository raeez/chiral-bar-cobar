...
Lagrangian and Verdier-dual to one another; without those hypotheses
they are only complementary eigenspaces.
...
\item \emph{Fiber--center identification
 ...
 Only this ordinary flat-perfect realization produces the ambient
 complex on which~\textup{(C1)} and~\textup{(C2)} operate.
...
$Q_g(\cA^!) = \ker(\sigma + \mathrm{id})$ are the eigenspaces of
 the Verdier involution~$\sigma$. This is the $\mathrm{C}_{1a}$
 homotopy-eigenspace statement and uses only the represented
 involution plus Lemma~\ref{lem:involution-splitting}. For
 $g \geq 1$, if the strict flat ambient complex is perfect
 ...
 $Q_g(\cA) \cong Q_g(\cA^!)^\vee$. At $g=0$, this
 perfect-duality clause is not invoked: in the unmarked ambient
 convention the point class is $\sigma$-fixed, so
 $Q_0(\cA)=H^0(\overline{\mathcal{M}}_0,\mathcal{Z}(\cA))$ and
 $Q_0(\cA^!)=0$
 \textup{(}Theorem~\textup{\ref{thm:quantum-complementarity-main})}.
 The nontrivial genus-$0$ Beilinson--Drinfeld content belongs to
 the pointed spaces $\overline{\mathcal M}_{0,n}$, $n\geq3$, and
 the Arnold-algebra pairing there, not to the
 $-(3g{-}3)$ shifted Verdier formula.

\item \emph{Shifted-symplectic Lagrangian upgrade
 \textup{(}conditional on the $\mathrm{C}_2$ package\textup{)}.}\;
 For $g\geq1$, when the $\mathrm{C}_2$ shifted-symplectic hypothesis package of
 ...
 \mathcal{Z}(\cA))$ carries a $({-}(3g{-}3))$-shifted symplectic
...
$\kappa(\cA)+\kappa(\cA^!)=K^\kappa(\cA)$ does not supply them.
...
\item the $\mathrm{C}_1$ represented Verdier involution and homotopy
...
$\mathbf{C}_g(\cA)$, anti-invariant under the represented involution,
...
\item closedness of the cyclic two-form on each finite window, so that
the PTVV constant-form construction applies without a residual de Rham
boundary term;
...
Theorems~\ref{thm:config-space-bv} and~\ref{thm:bv-functor}, with the
anomaly terms killed on the chosen finite window.
\end{enumerate}
The package is a hypothesis list, not a theorem.  Supplying only the
scalar Verdier sum or only the uniform-weight diagonal condition does
not certify~$\mathrm{C}_2$.
\end{definition}
...
\item \emph{\(\mathrm{C}_{1b}\): perfect duality for $g \geq 1$.}
 Assume $g \geq 1$, assume that $\mathbf{C}_g(\cA)$ is a perfect
 ...
 Verdier involution~$\sigma$. Under these hypotheses, each summand in
...
$\mathrm{C}_{1b}$ clause hold, then for
$g \geq 1$ they are Lagrangians for the induced pairing
on~$\mathcal{H}_g$, and
\begin{equation}\label{eq:quantum-duality}
Q_g(\cA) \cong Q_g(\cA^!)^\vee.
\end{equation}
At genus $0$, the shifted Verdier-pairing clause above is not
used. In the unmarked ambient convention, the unique point class is
$\sigma$-fixed, so
\[
...
Q_0(\cA^!)=0.
\]
The nontrivial genus-$0$ chiral operations are instead the
Beilinson--Drinfeld pointed theory on $\overline{\mathcal M}_{0,n}$,
$n\geq3$, governed by the Arnold algebra.
\end{theorem}
...
For $g\geq1$, assume perfectness/reflexivity of
$\mathbf{C}_g(\cA)$ and its direct-summand eigenspaces, and assume
that the Verdier pairing is non-degenerate and anti-invariant.
...
derived duals of degree $-(3g{-}3)$, hence
$\mathbf{Q}_g(\cA)\simeq
\mathbf{Q}_g(\cA^!)^\vee[-(3g{-}3)]$ and, on cohomology,
$Q_g(\cA)\cong Q_g(\cA^!)^\vee$. At $g=0$ the moduli space is a point
in this unmarked ambient convention and the Verdier orientation class is
fixed; no $-(3g{-}3)$ shifted Lagrangian pairing is asserted there.
...
$D^2 = 0$ (unconditional on the fixed smooth-curve carrier, not the
relative log-FM enhancement).
\end{remark}
...
Third, the perfect-duality content of~$\mathrm{C}_{1b}$ is consistent
with PTVV, but the precise comparison is polarization, not derived
...
base of dimension $3g{-}3$ for $g\geq1$, gives the
...
\]
At genus $0$, in the unmarked ambient convention and not as a
$-(3g{-}3)$ shifted Verdier pairing, the cohomological-shadow
eigenspaces are instead
$Q_0(\mathcal{A}) = H^0(\overline{\mathcal{M}}_0, Z(\mathcal{A}))$
and $Q_0(\mathcal{A}^!) = 0$
\textup{(}Theorem~\textup{\ref{thm:quantum-complementarity-main}}\textup{)}.
The pointed genus-$0$ theory on $\overline{\mathcal M}_{0,n}$,
$n\geq3$, remains the Beilinson--Drinfeld/Arnold-algebra surface.
\end{corollary}
...
Z(\mathcal{A}))$, provided the following $H^0$ center-comparison datum is supplied.
...
Deligne--Tamarkin rectification. It is not, without taking \(H^0\)
and imposing the ordinary flat fiber--center comparison, an
isomorphism of the naive center local systems used in Theorem~C.
The forgetful passage from the brace dg algebra to the associative
regular-module endomorphism algebra therefore gives no center
local-system isomorphism by itself. Once the \(H^0\)-shadow
...
Let $(\cA, \cA^!)$ be a chiral Koszul pair, let $g\geq1$, and let
...
$\overline{\mathcal{M}}_g$ is a constructible sheaf. Under the
...
\mathcal{Z}(\cA^!) \cong \mathcal{Z}(\cA)$
(the last isomorphism by the \(H^0\) center comparison of
...
At genus~$0$ in the unmarked ambient convention:
$Q_0(\cA)=\mathbb{C}$, $Q_0(\cA^!)=0$ (tree-level,
no obstruction). This is separate from the pointed
$\overline{\mathcal M}_{0,n}$, $n\geq3$, Beilinson--Drinfeld
surface.
...
\[
\mathrm{obs}_g(V_k(\mathfrak g))
=\kappa(V_k(\mathfrak g))\lambda_g
...
\textup{(}UNIFORM-WEIGHT\textup{)} is proved, is controlled by the same data.
