...
\begin{convention}[Stable modular base and dimension;
\ClaimStatusDefinitional]
\label{conv:theorem-c-stable-base}
\index{Theorem C!stable modular base}
For a stable pair \((g,n)\), so \(2g-2+n>0\), set
\[
 d_{g,n}:=3g-3+n,
 \qquad
 \mathbf C_{g,n}(\cA)
 :=R\Gamma\!\left(\overline{\mathcal M}_{g,n},\mathcal Z(\cA)\right).
\]
The genus shorthand uses
\[
 n_g:=
 \begin{cases}
  3,&g=0,\\
 ...
 \end{cases}
 \qquad
 \overline{\mathcal M}_g:=\overline{\mathcal M}_{g,n_g},
 \qquad
 d_g:=d_{g,n_g},
 \qquad
 \mathbf C_g:=\mathbf C_{g,n_g}.
\]
Thus every genus-indexed modular object in this section lives on a
stable Deligne--Mumford stack.  Theorem~C summands retain both indices:
the stable low-genus bases are written \((0,3)\) and \((1,1)\), while
the unpointed higher-genus base is written \((g,0)\) for \(g\geq2\).
The active Theorem~C notation always displays the marking index.
\end{convention}
...
$\cA^i=C_X(s^{-1}V,s^{-2}R)$ with
$q_\cA\colon\cA^i\to B(\cA)$, the bar cohomology
$H^\bullet B(\cA)$, and the dual algebras
$K_X(\cA)=\mathbb D_{\Ran}B(\cA)$ and~$\cA^!$ connected by the
chosen Koszul-pair map~$\nu_\cA$.
...
terminates. At stable genus~$g$, the Hodge bundle
$\mathbb{E}_g=\pi_*\omega_\pi$ on $\overline{\mathcal{M}}_{g,n_g}$
...
\]
This scalar curvature identity belongs to the genus-$g$ curvature
surface.  For $g\geq2$, Theorem~D first produces
$\operatorname{Obs}^{\mathrm{def}}_g\in H^2(\Def_g)$; the package
$H_D^{\mathrm{tr}}$ carries it to
$\mathfrak O_g^K=\kappa\lambda_{-1}(\mathbb E_g)$, whose degree-$g$
Chern character is $(-1)^g\kappa\lambda_g$.  The graph functional
then gives $F_g^{\mathrm{sc}}=\kappa\lambda_g^{\mathrm{FP}}$ on the
scalar diagonal. Theorem~C acts on a different object: the
strict flat ambient complex
$\mathbf{C}_{g,n}(\cA)=
R\Gamma(\overline{\mathcal{M}}_{g,n},\mathcal{Z}(\cA))$.
When the Verdier--Koszul involutions and the anti-equivariant
genuine-partner comparison of
Definition~\ref{def:complementarity-complexes} are represented, their
eigenspaces give
\begin{equation}\label{eq:complementarity-chapter-opening}
 H^*\bigl(\overline{\mathcal{M}}_{g,n},\,\mathcal{Z}(\cA)\bigr)
 \;=\;
 Q_{g,n}(\cA) \;\oplus\;
 (\chi^-_{\cA;g,n})^{-1}Q_{g,n}(\cA^!),
\end{equation}
where \(Q_{g,n}(\cA)\) and \(Q_{g,n}(\cA^!)\) are the genuine
positive eigensummands of their respective centre complexes and
\(\chi^-_{\cA;g,n}\) identifies the negative \(\cA\)-summand with
the positive \(\cA^!\)-summand.
If the strict flat ambient complex is perfect and the Verdier pairing
is non-degenerate and anti-invariant, these eigenspaces are
Lagrangian and Verdier-dual to one another.  Before the pairing package
is supplied, the complementary eigenspace decomposition acquires its
...
\[
\begin{array}{c|c|c}
\text{family} & \kappa(\cA)+\kappa(\cA^!) & \text{status} \\ \hline
\text{free fields and affine Kac--Moody} & 0 & \text{computed} \\
\mathrm{Vir}_c=\cW_2 & 13 & \text{computed} \\
\cW_3 & 250/3 & \text{computed} \\
\text{Bershadsky--Polyakov} & \text{open} & \text{genus-one DS curvature} \\
\text{Vol III Mukai-enhanced K3 Heisenberg witness} & 8 &
\text{conjectural under }H_{\mathsf B}
\end{array}
\]
For principal $\cW_N$ the general formula is
$\kappa+\kappa^!=\varrho_NK_N$, with
$\varrho_N=H_N-1$ and
$K_N=2(N-1)(2N^2+2N+1)$. These are scalar identities.  The bar
coalgebra comparison, the derived-centre comparison, and the bar-side
BV pairing are separate objects carried by their named reconstruction
maps and hypothesis packages.
...
\item \emph{Fiber--center identification
 \textup{(}coderived class, flat-perfect realization\textup{)}.}\;
 For every chiral algebra~$\cA$ carrying a modular pre-Koszul datum,
 the curved fiber bar family
 $(\barB^{(g)}(\cA),\dfib)$ determines a class in
 $D^{\mathrm{co}}$, unique up to coacyclic twist, and is read through
 its strict flat comparison model
 $(\barB^{(g)}(\cA),\Dg{g})$
 ...
 This ordinary flat-perfect realization produces the ambient complex
 on which~\textup{(C1)} and~\textup{(C2)} operate.

\item \emph{Verdier eigenspace decomposition and perfect duality
\textup{(}strict flat representative\textup{)}.}\;
 For every chiral Koszul pair $(\cA, \cA^!)$ on a smooth projective
 curve~$X$ and every stable pair \((g,n)\), represent strict-flat
 centre complexes for both partners, cochain involutions
 \(\sigma_{\cA},\sigma_{\cA^!}\), and the anti-equivariant comparison
 \(j_Z\) of Definition~\ref{def:complementarity-complexes}.  The
 restricted map \(\chi^-_{\cA;g,n}\) identifies the negative
 \(\cA\)-summand with the genuine positive \(\cA^!\)-summand, and
 \begin{equation}\label{eq:complementarity-summary}
 H^*(\overline{\mathcal{M}}_{g,n},\,\mathcal{Z}(\cA))
 \;=\;
 Q_{g,n}(\cA) \;\oplus\;
 (\chi^-_{\cA;g,n})^{-1}Q_{g,n}(\cA^!).
 \end{equation}
 This is the $\mathrm{C}_{1a}$ homotopy-eigenspace statement. If the
 strict flat ambient complex is perfect
 \textup{(}for example by the flat perfect locus of
 Theorem~\ref{thm:fiber-center-identification} and
 Lemma~\ref{lem:perfectness-criterion}\textup{)} and the Verdier
 pairing is non-degenerate and anti-invariant, then the
 $\mathrm{C}_{1b}$ perfect-duality statement identifies
 $Q_{g,n}(\cA) \cong Q_{g,n}(\cA^!)^\vee[-d_{g,n}]$.
 The cases \((0,3)\), \((1,1)\), and \((g,0)\) for \(g\geq2\)
 are the canonical genus representatives of
 Convention~\ref{conv:theorem-c-stable-base}.

\item \emph{Shifted-symplectic Lagrangian upgrade
 \textup{(}conditional on the $\mathrm{C}_2$ package\textup{)}.}\;
 For a stable pair \((g,n)\), when the $\mathrm{C}_2$
 shifted-symplectic hypothesis package of
 Definition~\textup{\ref{def:C2-shifted-symplectic-hypothesis-package}}
 is supplied, the complex
 $\mathbf{C}_{g,n}(\cA) = R\Gamma(\overline{\mathcal{M}}_{g,n},
 \mathcal{Z}(\cA))$ carries a $(-d_{g,n})$-shifted symplectic
 structure \textup{(}PTVV
 \cite{PTVV13}\textup{)}, and the summands
 $\mathbf{Q}_{g,n}(\cA)$ and
 $(\chi^-_{\cA;g,n})^{-1}\mathbf{Q}_{g,n}(\cA^!)$ are Lagrangian
 inside~$\mathbf C_{g,n}(\cA)$.  Equivalently, the genuine summands in
 their respective centre complexes satisfy the cross-duality
 $\mathbf{Q}_{g,n}(\cA) \simeq
 \mathbf{Q}_{g,n}(\cA^!)^\vee[-d_{g,n}]$.
...
\end{enumerate}
The conditionality in the perfect-duality part of~(C1) and in~(C2)
is substantive:
perfectness, nondegeneracy, cyclic closure, bracket compatibility,
twisted-tensor acyclicity, and BV/QME compatibility are separate
inputs.  The scalar identity
$\kappa(\cA)+\kappa(\cA^!)=K^\kappa(\cA)$ is their
one-dimensional trace consequence.
The $\mathrm{C}_0$ coderived class exists unconditionally on the
Koszul locus, while the fiber--center identification is the
ordinary-derived realization on the flat perfect locus; the
...
\begin{definition}[Scalar trace of the derived centre;
\ClaimStatusDefinitional]
\label{def:derived-centre-scalar-trace-kappa}
\index{derived centre!scalar trace to kappa@scalar trace to \(\kappa\)}
\index{Theorem C!scalar trace lane}
Assume the strict flat centre-comparison datum of~\(\mathrm{C}_0\) has
been fixed, and fix a Theorem--D trace pair
\((g,n)=(1,1)\) or \((g,n)=(g,0)\) with \(g\geq2\).  For either partner
\(\cB\in\{\cA,\cA^!\}\), let the degree-two diagonal residue class
\[
 \theta_{\cB}^{(2)}
 \in
 H^2\!\left(Z^{\mathrm{der}}_{\mathrm{ch}}(\cB)\right)_{\mathrm{diag}}
\]
be carried to the stable modular centre by a named realization map
\[
 \rho_{\cB;g,n}\colon
 H^2\!\left(Z^{\mathrm{der}}_{\mathrm{ch}}(\cB)\right)_{\mathrm{diag}}
 \longrightarrow
 H^2\!\left(\mathbf C_{g,n}(\cB)\right)_{\mathrm{diag}},
 \qquad
 \vartheta_{\cB;g,n}^{(2)}
 :=\rho_{\cB;g,n}(\theta_{\cB}^{(2)}).
\]
The map \(\rho_{\cB;g,n}\) is part of the strict-flat
fibre--centre realization and is required to intertwine the
represented involutions.  The \emph{scalar
\(\kappa\)-trace} is the scalar projection
\[
 \operatorname{tr}_{\kappa,\cB;g,n}\colon
 H^2\!\left(\mathbf C_{g,n}(\cB)\right)_{\mathrm{diag}}
 \longrightarrow \mathbb C
\]
obtained by applying the diagonal residue trace and then the modular
trace functional of Theorem~\ref{thm:modular-characteristic}.  It is
normalised by
\[
 \operatorname{tr}_{\kappa,\cB;g,n}
   (\vartheta_{\cB;g,n}^{(2)})=\kappa(\cB),
 \qquad
 \operatorname{Tr}_{D,g,n}(\vartheta_{\cB;g,n}^{(2)})
 =\operatorname{tr}_{\kappa,\cB;g,n}
   (\vartheta_{\cB;g,n}^{(2)})\,\lambda_g
 =\kappa(\cB)\lambda_g .
\]
This projection retains the scalar diagonal of the cochain derived
centre.  The open/closed comparison, centre reconstruction, and the
shifted-symplectic BV/QME structure enter through their respective
hypothesis packages.
\end{definition}

\begin{proposition}[Scalar conductor as the C1 trace shadow;
\ClaimStatusConditional]
\label{prop:scalar-conductor-c1-trace-shadow}
\index{scalar conductor!trace shadow of C1}
\index{Theorem C!scalar conductor as C1 shadow}
Let \((\cA,\cA^!)\) be a chiral Koszul pair and let
\((g,n)=(1,1)\) or \((g,0)\) with \(g\geq2\).  Supply the
\(\mathrm{C}_0\) strict-flat centre models, the
\(\mathrm{C}_1\) genuine-partner datum
\((\sigma_{\cA},\sigma_{\cA^!},j_Z,\chi^-_{\cA;g,n})\) of
Definition~\ref{def:complementarity-complexes}, and the scalar traces
of Definition~\ref{def:derived-centre-scalar-trace-kappa}.  Assume the
same symbols denote the induced maps on degree-two cohomology, and
assume the \emph{trace--eigenclass compatibility} equations
\[
 p_{\cA}^{+}\vartheta_{\cA;g,n}^{(2)}
 =\vartheta_{\cA;g,n}^{(2)},
 \qquad
 p_{\cA^!}^{+}\vartheta_{\cA^!;g,n}^{(2)}
 =\vartheta_{\cA^!;g,n}^{(2)},
...
\[
 \widetilde\theta_{\cA^!}^{-}
 :=(\chi^-_{\cA;g,n})^{-1}(\vartheta_{\cA^!;g,n}^{(2)})
 \in H^2(\mathbf Q^-_{g,n}(\cA)),
 \qquad
 \operatorname{tr}^{-}_{\kappa,\cA;g,n}
 :=\operatorname{tr}_{\kappa,\cA^!;g,n}\circ\chi^-_{\cA;g,n}.
\]
Then the scalar Verdier conductor is the scalar trace shadow of the
\(\mathrm{C}_1\) eigenspace decomposition together with Theorem~D:
\[
 K^\kappa(\cA)
 :=
 \operatorname{tr}_{\kappa,\cA;g,n}(\vartheta_{\cA;g,n}^{(2)})
 +
 \operatorname{tr}^{-}_{\kappa,\cA;g,n}
   (\widetilde\theta_{\cA^!}^{-})
 =
 \kappa(\cA)+\kappa(\cA^!).
\]
Equivalently, at genus~\(g\) on the uniform-weight scalar lane,
\[
 \operatorname{Tr}_{D,g,n}(\vartheta_{\cA;g,n}^{(2)})
 +
 \operatorname{Tr}_{D,g,n}(\vartheta_{\cA^!;g,n}^{(2)})
 =
 K^\kappa(\cA)\lambda_g .
\]
The scalar \(K^\kappa(\cA)\) is the one-dimensional trace shadow;
the eigenspace complexes, derived centres, and \(\mathrm{C}_2\)
Lagrangian/BV structure remain the corresponding source objects.
\end{proposition}

\begin{proof}
Trace--eigenclass compatibility places
\(\vartheta_{\cA;g,n}^{(2)}\) in the genuine positive
\(\cA\)-summand and \(\vartheta_{\cA^!;g,n}^{(2)}\) in the genuine
positive \(\cA^!\)-summand.
The map \(\chi^-_{\cA;g,n}\) transports the latter class to the
negative summand of the \(\cA\)-ambient complex, and its transported
trace satisfies
\[
 \operatorname{tr}^{-}_{\kappa,\cA;g,n}
 (\widetilde\theta_{\cA^!}^{-})
 ...
 =\kappa(\cA^!).
\]
The corresponding normalization for \(\cA\) gives \(\kappa(\cA)\).
Theorem~D,
recorded here as Theorem~\ref{thm:modular-characteristic}, then
multiplies each scalar by the top Hodge class \(\lambda_g\).  Adding
the two scalar traces gives the stated formula for \(K^\kappa(\cA)\)
and its genus-\(g\) Hodge projection.  The construction factors through
the displayed scalar traces, so its value is precisely the scalar
shadow of the represented \(\mathrm C_1\) data.
\end{proof}
...
\]
on the scalar diagonal.  Under~$H_D^K$, the genus-$g$ top-Hodge
projection is
\[
...
\cA^i:=C_X(s^{-1}V,s^{-2}R),\qquad
H^\bullet\barB^{\mathrm{ch}}(\cA),\qquad
K_X(\cA):=\mathbb D_{\Ran}\barB^{\mathrm{ch}}(\cA),\qquad
\cA^!,\qquad
\mathcal{Z}(\cA),\qquad
...
the flat perfect locus.  Quadratic recognition is carried by
$H^\bullet(q_\cA)$, Verdier comparison by
$\mathbb D(q_\cA)$ and~$\nu_\cA$, and inversion by
$\Omega\barB(\cA)\to\cA$.
\end{proposition}

\begin{proof}
The displayed objects have different functor signatures. The algebra
$\cA$ is a chiral algebra on~$X$; $\barB^{\mathrm{ch}}(\cA)$ is a
factorization coalgebra; $\cA^i$ is the quadratic presentation
coalgebra; $H^\bullet\barB^{\mathrm{ch}}(\cA)$ is bar cohomology;
$K_X(\cA)$ is the Verdier dual algebra of the full bar coalgebra; and
$\cA^!$ is the chosen Koszul partner supplied by~$\nu_\cA$.
The sheaf $\mathcal{Z}(\cA)$ is the degree-$0$
...
Theorem~H's family-indexed Hochschild support from $H_H(\cA;S)$
therefore live in different categories and are related only through
the displayed comparison functors.
\end{proof}
...
=Q_{g,n_g}(\cA)\oplus
(H^*\chi^-_{\cA;g,n_g})^{-1}Q_{g,n_g}(\cA^!)$.
\end{remark}
...
$Q_{1,1}(\mathcal{H}_\kappa)=\mathbb{C}\cdot\kappa$ is the positive
branch, and the genuine partner
$Q_{1,1}(\mathcal{H}_\kappa^!)=\mathbb{C}\cdot\lambda$ transports to
the negative branch.  Hence
$Q_{1,1}(\mathcal H_\kappa)\oplus
(H^*\chi^-_{\mathcal H_\kappa;1,1})^{-1}
Q_{1,1}(\mathcal H_\kappa^!)=H^0\oplus H^2=\mathbb C^2$.
\end{remark}
...
Let $(\cA, \cA^!)$ be a chiral Koszul pair on~$X$, and fix a stable
pair \((g,n)\).  Represent strict-flat centre complexes for both
partners,
\[
 \mathbf C_{g,n}(\cA)
 =R\Gamma(\overline{\mathcal M}_{g,n},\mathcal Z(\cA)),
 ...
 =R\Gamma(\overline{\mathcal M}_{g,n},\mathcal Z(\cA^!)),
\]
together with cochain involutions \(\sigma_{\cA}\) and
\(\sigma_{\cA^!}\).  The \emph{genuine-partner comparison datum} is
a cochain quasi-isomorphism induced by the derived-centre comparison,
\[
 j_Z:=R\Gamma\!\left(H^0(\iota_Z^{\mathrm{der}})\right)
 \colon \mathbf C_{g,n}(\cA)\xrightarrow{\;\simeq\;}
 \mathbf C_{g,n}(\cA^!),
 \qquad
 \sigma_{\cA^!}j_Z=-j_Z\sigma_{\cA}.
\]
A specified strictification of this identity represents an
anti-equivariant homotopy comparison.  Set
\[
 p_{\cA}^{\pm}:=\tfrac12(\mathrm{id}\pm\sigma_{\cA}),
 \qquad
 p_{\cA^!}^{\pm}:=\tfrac12(\mathrm{id}\pm\sigma_{\cA^!}).
\]
The genuine positive eigensummands and the negative ambient summand are
\begin{align}
 \mathbf Q_{g,n}(\cA)&:=\operatorname{im}(p_{\cA}^{+}),&
 \mathbf Q_{g,n}^{-}(\cA)&:=\operatorname{im}(p_{\cA}^{-}),\\
 \mathbf Q_{g,n}(\cA^!)&:=\operatorname{im}(p_{\cA^!}^{+})
 \subset \mathbf C_{g,n}(\cA^!).
\end{align}
Anti-equivariance gives
\(j_Zp_{\cA}^{-}=p_{\cA^!}^{+}j_Z\), hence the restricted
quasi-isomorphism
\begin{equation}\label{eq:genuine-partner-minus-comparison}
 \chi^-_{\cA;g,n}:=
 j_Z\big|_{\mathbf Q^-_{g,n}(\cA)}
 \colon \mathbf Q^-_{g,n}(\cA)
 \xrightarrow{\;\simeq\;}\mathbf Q_{g,n}(\cA^!).
\end{equation}
Thus the H-level decomposition, with the genuine partner transported
into the \(\cA\)-ambient complex, is
\begin{equation}\label{eq:complementarity-fiber}
 \mathbf C_{g,n}(\cA)
 \simeq \mathbf Q_{g,n}(\cA)\oplus
 (\chi^-_{\cA;g,n})^{-1}\mathbf Q_{g,n}(\cA^!).
\end{equation}
The S-level objects are
\begin{align}\label{eq:shadow-spaces}
 Q_{g,n}(\cA)&:=H^*(\mathbf Q_{g,n}(\cA)),\\
 Q_{g,n}(\cA^!)&:=H^*(\mathbf Q_{g,n}(\cA^!)),
\end{align}
and \(H^*(\chi^-_{\cA;g,n})\) identifies the latter with the
negative eigenspace in
\(H^*(\overline{\mathcal M}_{g,n},\mathcal Z(\cA))\).
Every formula records its ambient object explicitly: the genuine
summand \(\mathbf Q_{g,n}(\cA^!)\) lies in
\(\mathbf C_{g,n}(\cA^!)\), while its realization inside
\(\mathbf C_{g,n}(\cA)\) is
\((\chi^-_{\cA;g,n})^{-1}\mathbf Q_{g,n}(\cA^!)\).
These complexes constitute the \(\mathrm C_1\) centre lane.  Their
scalar traces give the conductor shadow, while an open/closed
comparison supplies a physical bulk interpretation.
\end{definition}

\begin{remark}[H/M/S layers; Convention~\ref{conv:hms-levels}]
The summands and genuine-partner map in
\eqref{eq:complementarity-fiber} are the H-level objects; the bar complexes
...
(Lemma~\ref{lem:involution-splitting}) gives the passage to
cohomological eigenspaces in characteristic~$0$.
\end{remark}
...
\textup{[}Type signature: Open quadrant; strict-flat
centre-local-system presentation; Beilinson level~3;
\(H_{\mathrm C_0}\cup H_{\mathrm C_{1a}}^{\mathrm{partner}}\),
with \(H_{\mathrm C_{1b}}^{\mathrm{pair}}\) and
\(H_{\mathrm C_2}^{\mathrm{BV}}\) attached to their stated clauses.\textup{]}
...
\item \emph{Coderived form.}\;
 ...
 $\barB^{(g)}_{\mathrm{curv}}(\cA)$ determines a class
 $[\barB^{(g)}_{\mathrm{curv}}(\cA)]$ in $D^{\mathrm{co}}$, unique
 up to coacyclic twist. This assertion does not identify ordinary
 cohomology and does not identify the center local system.
 Proposition~\textup{\ref{prop:gauss-manin-uncurving-chain}}
...
\item \emph{Zero-curvature recovery.}\;
 ...
 $\barB^{(g)}_{\mathrm{curv}}(\cA)$.
\end{enumerate}
In particular, the unconditional part of~$\mathrm{C}_0$ is the
existence of the class
$[\barB^{(g)}_{\mathrm{curv}}(\cA)]\in D^{\mathrm{co}}$; the
cohomological concentration and the identification with
$\mathcal Z_{\cA}$ are ordinary-derived statements on the flat
...
This proves \textup{(i)} as a coderived class statement and shows
that the ordinary-derived calculation must be performed on the flat
representative.
...
$\mathrm{C}_{1a}$ (Theorem~\ref{thm:quantum-complementarity-main})
gives the Verdier eigenspace decomposition from the represented
involution; $\mathrm{C}_{1b}$ is the perfect-duality and Lagrangian
clause, and requires perfectness/reflexivity together with the
non-degenerate anti-invariant Verdier pairing. $\mathrm{C}_2$
(Theorem~\ref{thm:shifted-symplectic-complementarity}) is the
conditional bar-side BV / shifted-symplectic upgrade governed by
Definition~\ref{def:C2-shifted-symplectic-hypothesis-package}.
...
\begin{definition}[$\mathrm{C}_2$ shifted-symplectic hypothesis package;
\ClaimStatusDefinitional]
\label{def:C2-shifted-symplectic-hypothesis-package}
\index{Theorem C!C2 shifted-symplectic hypothesis package}
\index{Lagrangian!hypothesis package for C2}
For a chiral Koszul pair $(\cA,\cA^!)$ and a genus $g\geq1$, the
$\mathrm{C}_2$ shifted-symplectic hypothesis package is the following
ten-input datum:
\begin{enumerate}[label=\textup{(C2.\arabic*)}]
\item the $\mathrm{C}_0$ strict flat fiber-center comparison of
Theorem~\ref{thm:fiber-center-identification};
\item the $\mathrm{C}_1$ represented involutions, anti-equivariant
genuine-partner map \(j_Z\), restricted comparison \(\chi^-_{\cA;g,n}\),
and homotopy eigenspace decomposition of
Theorem~\ref{thm:quantum-complementarity-main};
\item uniform-weight perfectness of the strict flat relative bar family,
with finite weight spaces, flat fiber cohomology, and dual regularity
as in Lemma~\ref{lem:perfectness-criterion} and
Proposition~\ref{prop:lagrangian-perfectness};
\item a nondegenerate cyclic Verdier pairing on
$\mathbf{C}_{g,n}(\cA)$ of degree \(-d_{g,n}\), anti-invariant under
the represented involution,
as in Proposition~\ref{prop:lagrangian-eigenspaces};
\item a finite modular envelope for the genus-truncated deformation
objects, compatible with the filtered modular truncations of
Proposition~\ref{prop:modular-deformation-truncation};
\item closedness of the cyclic two-form on each finite window, together
with a vanishing residual de Rham boundary class, so that the PTVV
constant-form construction applies;
\item skew-adjointness of the differential for the cyclic pairing;
\item compatibility of the modular bracket with the cyclic pairing;
\item acyclicity of the twisted tensor kernel used in the bar-chart
transport square of Lemma~\ref{lem:bar-chart-lagrangian-lift};
\item the ambient BV/QME input, namely the config-space BV bracket and
the bracket-compatible Verdier functor of
Theorems~\ref{thm:config-space-bv} and~\ref{thm:bv-functor}, together
with chosen null-homotopies for the anomaly terms on the finite window.
\end{enumerate}
The package is the hypothesis list consumed by the theorem.  The
$\mathrm{C}_2$ conclusion uses the scalar Verdier sum, the
uniform-weight diagonal condition, and the remaining BV-perfectness
and pairing clauses together.
\end{definition}
...
\begin{theorem}[Quantum complementarity: Verdier eigenspaces,
perfect duality, and Lagrangian upgrade; \ClaimStatusConditional]
...
Fix a stable pair \((g,n)\).  Write
$\mathbf{C}_{g,n}(\cA)$, $\mathbf{Q}_{g,n}(\cA)$,
$\mathbf{Q}_{g,n}^{-}(\cA)$, and $\mathbf{Q}_{g,n}(\cA^!)$
for the centre complexes and genuine eigensummands of
Definition~\textup{\ref{def:complementarity-complexes}}.  Assume the
ordinary flat representatives of \textup{(C0)} are available for both
partners and fix the genuine-partner datum
\[
 (\sigma_{\cA},\sigma_{\cA^!},j_Z,\chi^-_{\cA;g,n}),
 \qquad
 \sigma_{\cA^!}j_Z=-j_Z\sigma_{\cA}.
\]
...
\begin{enumerate}[label=\textup{(\roman*)}]
\item \emph{\(\mathrm{C}_{1a}\): homotopy eigenspace decomposition.}
 By Lemma~\textup{\ref{lem:involution-splitting}(a)},
 \begin{equation}\label{eq:complementarity-decomp}
 \mathbf{C}_{g,n}(\cA) \;\simeq\;
 \mathbf{Q}_{g,n}(\cA) \;\oplus\;
 (\chi^-_{\cA;g,n})^{-1}\mathbf{Q}_{g,n}(\cA^!),
 \end{equation}
 functorially in represented Koszul-pair data commuting with both
 involutions and with~\(j_Z\).

\item \emph{\(\mathrm{C}_{1b}\): perfect duality.}
 Assume that $\mathbf{C}_{g,n}(\cA)$ is a perfect
 complex and that its eigensummands are therefore perfect direct
 summands, and assume that Verdier duality induces a
 non-degenerate cochain-level pairing
 $\langle -,- \rangle_{\mathbb{D}}$ on~$\mathbf{C}_{g,n}(\cA)$
 of cohomological degree $-d_{g,n}$, anti-invariant under the
 Verdier involution~$\sigma_{\cA}$. Under these hypotheses, each summand in
 \eqref{eq:complementarity-decomp} is Lagrangian, and
 $\langle -, - \rangle_{\mathbb{D}}$ restricts to a
 perfect pairing
 $\mathbf{Q}_{g,n}(\cA) \otimes \mathbf{Q}_{g,n}(\cA^!)
 \to \mathbb{C}[-d_{g,n}]$,
 so $\mathbf{Q}_{g,n}(\cA) \simeq
 \mathbf{Q}_{g,n}(\cA^!)^\vee[-d_{g,n}]$.

\item \emph{Functoriality.}
 The decomposition is natural in morphisms of represented Koszul-pair
 data commuting with \(\sigma_{\cA}\), \(\sigma_{\cA^!}\), and
 \(j_Z\), and is compatible with conformal weight and cohomological
 degree.
\end{enumerate}

\smallskip\noindent
\textbf{S-level \textup{(}cohomological shadow\textup{)}.}
Setting $\mathcal{H}_{g,n}(\cA) := H^*(\overline{\mathcal{M}}_{g,n},
\mathcal{Z}(\cA))$, Lemma~\textup{\ref{lem:involution-splitting}(b)}
gives:
\begin{equation}\label{eq:verdier-symplectic-pairing}
\mathcal{H}_{g,n}(\cA) \;=\;
Q_{g,n}(\cA) \;\oplus\;
(H^*\chi^-_{\cA;g,n})^{-1}Q_{g,n}(\cA^!),
\end{equation}
where each displayed \(Q\)-space is the positive eigenspace in the
genuine centre complex of the indicated partner; the inverse image is
the negative eigenspace of \(\sigma_{\cA}\).
If the perfectness and pairing hypotheses in the H-level
$\mathrm{C}_{1b}$ clause hold, they are Lagrangians for the induced
pairing on~$\mathcal{H}_{g,n}$, and
...
\begin{proof}
The ordinary flat representatives supplied by~\textup{(C0)} place the
two centre objects in cochain complexes.  Lemma~\ref{lem:involution-splitting}(a)
gives
\[
 \mathbf C_{g,n}(\cA)
 =\mathbf Q_{g,n}(\cA)\oplus\mathbf Q^-_{g,n}(\cA).
\]
Anti-equivariance gives
\(j_Zp^-_{\cA}=p^+_{\cA^!}j_Z\).  Since \(j_Z\) is a
quasi-isomorphism and both source and target split into strict
idempotent summands, its restriction
\(\chi^-_{\cA;g,n}\colon\mathbf Q^-_{g,n}(\cA)\to
\mathbf Q_{g,n}(\cA^!)\) is a quasi-isomorphism.  Substitution gives
\eqref{eq:complementarity-decomp}; applying cohomology gives
\eqref{eq:verdier-symplectic-pairing}.

Assume perfectness/reflexivity of $\mathbf{C}_{g,n}(\cA)$ and its
direct-summand eigenspaces, and assume that the Verdier pairing is
non-degenerate and anti-invariant.
Lemma~\ref{lem:involution-splitting}(c) makes the two summands
isotropic and maximal. Since direct summands of a perfect complex are
perfect, the non-degenerate cross-pairing is a perfect pairing of
derived duals of degree $-d_{g,n}$.  Transport along
\(\chi^-_{\cA;g,n}\) gives the asserted duality with the genuine
\(\cA^!\)-summand. Naturality follows because morphisms of represented
Koszul-pair data commute with both involutions and with~\(j_Z\).
\end{proof}
...
The linear decomposition in Theorem~C uses the stable modular centre
complexes, the two represented involutions, the anti-equivariant
genuine-partner comparison~$j_Z$, and the involution-splitting lemma.
The perfect-duality clause additionally uses Poincar\'e--Verdier
duality on the stable Deligne--Mumford base.  The ambient-level
$D^2=0$ theorem and the bar-intrinsic Maurer--Cartan element
$\Theta_\cA$ belong to the subsequent nonlinear enhancement.  The
scalar curvature~$\kappa(\cA)$ enters after projection through the
Theorem--D trace lane.
\end{remark}
...
Fix a stable pair~$(g,n)$.  \emph{Step~B} (M-level): the homotopy
eigenspace
$\mathbf{Q}_{g,n}(\cA)=\operatorname{fib}(\sigma_\cA-\mathrm{id})$
is modeled by $\operatorname{im}(p_\cA^+)$ on
$C_{g,n}=R\Gamma(\overline{\mathcal{M}}_{g,n},\mathcal{Z}(\cA))$.
The Lagrangian property is $\langle p^+x,p^+y\rangle=0$
(Lemma~\ref{lem:involution-splitting}(c)), after the
perfectness and non-degenerate pairing hypotheses of
$\mathrm{C}_{1b}$ have been supplied.
Taking cohomology gives the S-level decomposition
$\mathcal{H}_{g,n}=Q_{g,n}(\cA)\oplus
(H^*\chi^-_{\cA;g,n})^{-1}Q_{g,n}(\cA^!)$.  Its H-level lift is
canonical relative to the displayed strict flat representative,
centre comparison~$\iota_Z$, and represented involution~$\sigma$,
encoded by $\sigma_\cA$, $\sigma_{\cA^!}$, and~$j_Z$.
Chosen quasi-isomorphism zigzags compare the underlying admissible bar
presentations.  Contractibility of the comparison space for the triples
$(\mathbf C_{g,n}(\cA),\mathbf C_{g,n}(\cA^!),
\sigma_\cA,\sigma_{\cA^!},j_Z)$ remains an open problem.
For the pointed Heisenberg calculation at~$(1,1)$:
$Q_{1,1}=H^0$ and the transported partner branch is~$H^2$.
\end{remark}
...
centre-local-system presentation; Beilinson level~3;
\(H_{\mathrm C_0}\cup H_{\mathrm C_{1a}}^{\mathrm{partner}}
\cup H_{\mathrm C_{1b}}^{\mathrm{pair}}\).]}
For a stable pair \((g,n)\), the complex
$\mathbf{C}_{g,n}(\cA)=R\Gamma(\overline{\mathcal{M}}_{g,n},
\mathcal{Z}(\cA))$ carries a $-d_{g,n}$-shifted symplectic structure
(PTVV~\cite{PTVV13}; Proposition~\ref{prop:ptvv-lagrangian})
under the perfectness and nondegeneracy hypotheses recorded there.
The decomposition
$\mathbf{C}_{g,n}(\cA)\simeq\mathbf{Q}_{g,n}(\cA)\oplus
(\chi^-_{\cA;g,n})^{-1}\mathbf{Q}_{g,n}(\cA^!)$ is a Lagrangian
polarization in that paired regime, with~$\sigma$ acting as an
anti-symplectomorphism.  Its tangent and obstruction complexes are
Serre dual, and passage to cohomology gives the S-level eigenspace
decomposition.
\end{remark}
...
Second, PTVV controls shifted symplectic forms, isotropic structures,
and Lagrangian maps
\textup{(}Definition~\textup{2.8}, Theorem~\textup{2.9}\textup{)}.
The scalar complementarity identity
$\kappa(\cA)+\kappa(\cA^!)=K$ arises from the represented
$\mathrm C_1$ eigenspace decomposition after projection by Theorem~D
and the family computations of
Table~\ref{tab:complementarity-landscape}.

Third, the perfect-duality content of~$\mathrm{C}_{1b}$ is a
polarization statement. Proposition~\ref{prop:ptvv-lagrangian} shows
that $\mathbf{Q}_{g,0}(\cA)$ and
$(\chi^-_{\cA;g,0})^{-1}\mathbf{Q}_{g,0}(\cA^!)$ satisfy the PTVV
Lagrangian conditions, for $g\geq2$, inside the ambient
$(-d_{g,0})$-shifted space $\mathbf{C}_{g,0}(\cA)$.
...
of our fiberwise construction.  A $0$-shifted pairing on the centre
local system, pushed forward along a stable base of dimension
$d_{g,n}$, gives the $(-d_{g,n})$-shifted Verdier pairing of
...
bound one may take $E_r=E_\infty$ for $r>2d_{g,n_g}+1$.
\end{lemma}
...
$\overline{\mathcal{M}}_g=\overline{\mathcal M}_{g,n_g}$ is a smooth
proper Deligne--Mumford stack of dimension $d_{g,n_g}$. For any
...
$Q_{g,n_g}(\mathcal{A})$ is the $+1$ Verdier projection
...
$Q_{g,n_g}(\mathcal{A})$ precisely when the class is represented by the
...
$Q_{g,n_g}(\mathcal{A})$ for the Verdier eigenspace
...
next Leray filtration step. Its image in $Q_{g,n_g}(\mathcal{A})$ is
...
from the genuine Verdier eigenspaces $Q_{g,n_g}(\mathcal{A})$ in
$\mathbf C_{g,n_g}(\mathcal A)$ and $Q_{g,n_g}(\mathcal{A}^!)$ in
$\mathbf C_{g,n_g}(\mathcal A^!)$.
\end{proof}
...
\emph{Setup}: By the Fulton--MacPherson compactification theorem
\textup{(}Theorem~\textup{\ref{thm:FM}}\textup{)}, the boundary
description
\textup{(}Corollary~\textup{\ref{cor:dimension-strata}}\textup{)}, and
functoriality
\textup{(}Theorem~\textup{\ref{thm:FM-functorial}}\textup{)},
$\overline{C}_n(X)$ is constructed by iterated blow-ups along diagonal
strata. The key properties are:
\begin{enumerate}
...
This sign is precisely the Koszul sign convention of the master sign
formula, together with the FM orientation convention
\textup{(}Propositions~\textup{\ref{prop:master-sign}} and
\textup{\ref{prop:orient-fm}}\textup{)}.
...
\textup{[Type signature: Open quadrant; strict-flat Leray and
centre-local-system presentations; Beilinson levels~2 and~3;
\(H_{\mathrm C_0}\cup H_{\mathrm C_{1a}}^{\mathrm{partner}}
\cup H_{\mathrm C_{1b}}^{\mathrm{pair}}\).]}
For each genus $g \geq 1$ and every Koszul dual chiral pair
$(\mathcal{A}, \mathcal{A}^!)$ satisfying the fixed-genus
perfectness hypotheses on the Leray correction objects, Verdier
...
perfectness/reflexivity and non-degenerate anti-invariant pairing
hypotheses of
Theorem~\textup{\ref{thm:quantum-complementarity-main}} hold, then
\[
Q_{g,n_g}(\mathcal{A})
\cong Q_{g,n_g}(\mathcal{A}^!)^\vee[-d_{g,n_g}] .
\]
At the stable genus-zero base \((g,n)=(0,3)\), the same conclusion
uses \(d_{0,3}=0\).  If the centre complex is generated by the unit,
\(\sigma_{\cA}\) fixes that generator, and anti-equivariance makes
\(\sigma_{\cA^!}\) act by \(-1\) on its image, then
\[
 Q_{0,3}(\cA)=H^0(\overline{\mathcal M}_{0,3},Z(\cA)),
 \qquad Q_{0,3}(\cA^!)=0.
\]
For a general coefficient involution, Theorem~C retains its two
coefficient eigensummands on \(\overline{\mathcal M}_{0,3}\).
\end{corollary}
...
passing to $E_\infty$ and summing over total degree, under the
perfectness hypotheses stated above. The final statement is exactly
the $\mathrm{C}_{1b}$ Lagrangian clause of
...
sequences and, after the perfectness/reflexivity and finite-type
pairing hypotheses are imposed, the perfect cross-pairing between the
two Verdier eigenspaces:
\begin{equation}
Q_{g,n_g}(\mathcal{A})
\cong Q_{g,n_g}(\mathcal{A}^!)^\vee[-d_{g,n_g}]
\qquad (g\geq1).
...
$g \geq 1$ carrying the $H^0$ center-comparison datum of
...
denotes the action of $z$ via the Gauss--Manin connection; here
\(\iota_Z=H^0(\iota_Z^{\mathrm{der}})\) is the center-local-system
shadow supplied by Lemma~\textup{\ref{lem:center-isomorphism}}.
\end{theorem}
...
$H^0$ center-comparison and perfectness hypotheses, using three independent ingredients.
...
\emph{Ingredient 2: Verdier duality intertwines center actions through the \(H^0\) comparison.}
For $z \in Z(\mathcal{A})$, the action $\rho(z)$ on the bar complex
$\bar{B}^{(g)}(\mathcal{A})$ is a $\mathcal{D}$-module endomorphism (since $z$ is
central). The datum $\iota_Z=H^0(\iota_Z^{\mathrm{der}})$ identifies the
center local systems in a way compatible with the Verdier--Koszul pairing.
Hence the transpose of the $Z(\mathcal{A})$-action is the
$Z(\mathcal{A}^!)$-action of $\iota_Z(z)$:
...
&& \text{(definition)}
\end{align}
Composing with the $H^0$ center comparison (which sends $\iota_Z(z)$ back to $z$), the
...
The anti-commutativity~\eqref{eq:verdier-ks-anticommute} supplies the
sign compatibility for the eigenspace decomposition.  The represented
involution on $H^*(\overline{\mathcal{M}}_g,Z(\mathcal A))$ has positive
eigenspace $Q_{g,n_g}(\mathcal A)$ and negative eigenspace
$(H^*\chi^-_{\mathcal A;g,n_g})^{-1}Q_{g,n_g}(\mathcal A^!)$.
...
centre-local-system presentation; Beilinson level~3;
\(H_{\mathrm C_0}\cup H_{\mathrm C_{1a}}^{\mathrm{partner}}\).]}
For a chiral Koszul pair $(\mathcal{A}, \mathcal{A}^!)$, supply the
$\mathrm C_0$ strict-flat centre models, the derived-centre comparison,
and a strictification for which Verdier duality on
$\overline C_n(X)$ together with the Koszul pairing
$\mathcal A\otimes\mathcal A^!\to\mathcal O_X$ represents a cochain
involution
\begin{equation}
...
The $E_2$ pages of the two Leray spectral sequences
(Theorem~\ref{thm:ss-quantum}) place the genuine summands
$Q_{g,n_g}(\mathcal A)$ and $Q_{g,n_g}(\mathcal A^!)$ in their own
centre complexes.  The following $H^0$ centre-comparison datum
transports the second summand to
$H^*(\overline{\mathcal M}_g,Z(\mathcal A))$.

\begin{lemma}[Derived-center comparison and its $H^0$ shadow;
\ClaimStatusConditional]
\label{lem:center-isomorphism}
\textup{[Type signature: Open quadrant; derived chiral-centre and
strict-flat centre-local-system presentations; Beilinson level~3;
derived Morita comparison plus the \(H^0\) fibre--centre hypothesis
package.]}
Let $(\mathcal{A},\mathcal{A}^!)$ be a Koszul chiral pair. The
functorial center object controlled by derived Koszul--Morita
equivalence is the chiral Hochschild derived center
\[
 Z^{\mathrm{der}}_{\mathrm{ch}}(\mathcal A)
 =
 \ChirHoch^\bullet(\mathcal A,\mathcal A),
\]
as a brace dg algebra. Suppose the strict flat Koszul package either
supplies, or is induced by, a brace dg-algebra quasi-isomorphism
\[
  ...
  Z^{\mathrm{der}}_{\mathrm{ch}}(\mathcal A^!)
\]
as in Proposition~\textup{\ref{prop:derived-center-morita-criterion}}.
After taking \(H^0\) and applying the ordinary flat fiber--center
identification, this gives an isomorphism of center local systems
\[
  \iota_Z := H^0(\iota_Z^{\mathrm{der}})
  \colon Z(\mathcal{A}) \xrightarrow{\sim} Z(\mathcal{A}^!)
\]
compatible with the Verdier--Koszul pairing. Then the two Leray
$E_2$ pages for $\mathcal{A}$ and $\mathcal{A}^!$ may be read with
coefficients in the same local system. The scalar transpose
\(\kappa_Z\) used in the Kodaira--Spencer comparison is this
\(H^0\)-level map; it is not the full brace dg algebra.
\end{lemma}

\begin{proof}[Proof of Sublemma]
The comparison is a named datum added to the module Koszul package.
The $E_1$ module Koszul equivalence
...
\end{equation}
For $M=\mathcal{A}$ this is the derived endomorphism object of the
regular module. With Morita functoriality for the chiral Hochschild
endomorphism-operad model, this is precisely the derived chiral center
comparison
\[
 Z^{\mathrm{der}}_{\mathrm{ch}}(\mathcal A)
 \simeq
 Z^{\mathrm{der}}_{\mathrm{ch}}(\mathcal A^!)
\]
as a brace dg algebra, hence as an \(E_2\)-algebra after
Deligne--Tamarkin rectification.  Taking \(H^0\) and applying the
ordinary flat fibre--centre comparison produces the centre local
systems used in Theorem~C.  Once the \(H^0\)-shadow
\(\iota_Z=H^0(\iota_Z^{\mathrm{der}})\) is part of the strict flat
package, it transports the coefficients of the \(\mathcal{A}^!\)
Leray sequence to the coefficient system for \(\mathcal{A}\), which
is the assertion.
\end{proof}
...
E_2^{p,d-q}(\mathcal{A})
\end{equation}
(the last isomorphism via the supplied $H^0$ center comparison). This descends to $E_\infty$:
...
$H^0$ center comparison, commutes with the spectral sequence differentials $d_r$
...
\begin{align}
Q_{g,n_g}(\mathcal{A}) &= H^*(\overline{\mathcal{M}}_g, Z(\mathcal{A}))^+\\
(H^*\chi^-_{\mathcal A;g,n_g})^{-1}Q_{g,n_g}(\mathcal{A}^!)
&= H^*(\overline{\mathcal{M}}_g, Z(\mathcal{A}))^-
\end{align}
\end{lemma}
...
\emph{Identification of $V^+$ with $Q_{g,n_g}(\mathcal{A})$.}
By construction, $\sigma$ acts on $E_\infty^{*,*,g}$ by exchanging the contributions
from $\mathcal{A}$ and $\mathcal{A}^!$.  A class
$\alpha\in Q_{g,n_g}(\mathcal A)=H^*(\operatorname{im}p^+_{\mathcal A})$
is obtained by applying~$p^+_{\mathcal A}$ after the Leray-to-centre
comparison of Lemma~\ref{lem:quantum-from-ss}.  Verdier duality maps
its genuine representative to
$Q_{g,n_g}(\mathcal A^!)^\vee[-d_{g,n_g}]$.  The following
$j_*/j_!$ model records the positive sign:
\begin{enumerate}
...
$Q_{g,n_g}(\mathcal{A})$ live on the $q = 0$ row of the $E_2$ page.
Poincar\'e duality sends $H^q$ to $H^{d-q}$ with sign $(-1)^q$;
at $q = 0$ the sign is $(-1)^0 = +1$. Therefore elements of
$Q_{g,n_g}(\mathcal{A})$ have eigenvalue~$+1$.
\end{enumerate}

\emph{Identification of $V^-$ with the transported genuine partner.}
...
elements of $Q_{g,n_g}(\mathcal{A}^!)$ also live on the $q = 0$ row of
\emph{their own} $E_2$ page.

The Verdier involution $\sigma$ on
$\mathcal{H}_g(\mathcal{A})$ identifies
$(H^*\chi^-_{\mathcal A;g,n_g})^{-1}Q_{g,n_g}(\mathcal{A}^!)$
through the $j_! \leftrightarrow j_*$ interchange. Concretely, $\sigma$ sends
...
$Q_{g,n_g}(\mathcal{A})$ and the $-1$-eigenspace with
$(H^*\chi^-_{\mathcal A;g,n_g})^{-1}Q_{g,n_g}(\mathcal{A}^!)$
then follows from the Verdier intertwining
...
centre-local-system presentation; Beilinson level~3;
\(H_{\mathrm C_0}\cup H_{\mathrm C_{1a}}^{\mathrm{partner}}\),
with the deformation interpretation carried by the stated Koszul
comparison.]}
In the decomposition of Lemma~\ref{lem:eigenspace-decomposition-complete}:
\begin{enumerate}
\item Elements of $Q_{g,n_g}(\mathcal{A}) = V^+$ correspond to \emph{obstructions}
to extending $\mathcal{A}$ from genus $g-1$ to genus $g$
\item Elements of
$(H^*\chi^-_{\mathcal A;g,n_g})^{-1}Q_{g,n_g}(\mathcal{A}^!)=V^-$
form the transported genuine obstruction branch of~$\mathcal A^!$;
the Koszul deformation--obstruction comparison identifies this branch
with deformations of~$\mathcal A$ at genus~$g$
\item Interchanging the Koszul pair exchanges the two roles.
\end{enumerate}
\end{lemma}
...
The bar complex $\bar{B}^{(g)}(\mathcal{A}^!)$ computes the genuine
obstruction branch of~$\mathcal A^!$.  The supplied Koszul comparison
identifies the deformation complex of~$\mathcal A$ with this obstruction
complex (Theorem~\ref{thm:chiral-koszul-duality}), and
$H^*\chi^-_{\mathcal A;g,n_g}$ transports the resulting branch to~$V^-$.
\end{proof}

\emph{Eigenspace intersection.}

\begin{lemma}[Complementary eigenspace intersection; \ClaimStatusConditional]
\label{lem:trivial-intersection-complete}
\textup{[Type signature: Open quadrant; strict-flat
centre-local-system presentation; Beilinson level~3;
\(H_{\mathrm C_0}\cup H_{\mathrm C_{1a}}^{\mathrm{partner}}\).]}
The cohomological-shadow eigenspaces form a direct pair:
\begin{equation}
Q_{g,n_g}(\mathcal{A}) \cap
(H^*\chi^-_{\mathcal A;g,n_g})^{-1}Q_{g,n_g}(\mathcal{A}^!) = 0
\end{equation}
\end{lemma}

\begin{proof}[Proof of Lemma~\ref{lem:trivial-intersection-complete}]
By Lemma~\ref{lem:eigenspace-decomposition-complete},
$Q_{g,n_g}(\mathcal{A})=V^+$ and
$(H^*\chi^-_{\mathcal A;g,n_g})^{-1}Q_{g,n_g}(\mathcal{A}^!)=V^-$
are the $+1$ and $-1$ eigenspaces of the involution
...
\emph{Stable genus $0$}: $\overline{\mathcal{M}}_{0,3}=\mathrm{pt}$.
For a one-dimensional unit coefficient complex on which
\(\sigma_{\cA}=+1\), anti-equivariance gives
\(\sigma_{\cA^!}=-1\) on the compared line.  Hence
$Q_{0,3}(\mathcal{A})=\mathbb C$ and
$Q_{0,3}(\mathcal{A}^!)=0$, with dimension check $1+0=1$.
An arbitrary coefficient complex decomposes by its represented
coefficient involution.

\emph{Genus $1$}: $\dim H^*(\overline{\mathcal{M}}_{1,1}) = 2$ ($H^0 \oplus H^2$).
For the Heisenberg algebra:
$Q_{1,1}(\mathcal{H}_\kappa)=\mathbb{C}\cdot\kappa$ is the
positive branch, while
$(H^*\chi^-_{\mathcal H_\kappa;1,1})^{-1}
Q_{1,1}(\mathcal{H}_\kappa^!)=\mathbb{C}\cdot\lambda$ is the negative
branch.  Their dimensions add to~$2$.
...
$\dim Q_{2,0}(\cA)+\dim Q_{2,0}(\cA^!)=8$; the second dimension
equals that of its transported realization under
$H^*\chi^-_{\cA;2,0}$.
...
Q_{g,n_g}(\mathcal{A})\oplus
(H^*\chi^-_{\mathcal A;g,n_g})^{-1}Q_{g,n_g}(\mathcal{A}^!)
\cong H^*(\overline{\mathcal{M}}_g,Z(\mathcal{A}))
\end{equation}
...
supplied $H^0$ center comparison
$\mathcal{Z}(\cA) \cong \mathcal{Z}(\cA^!)$
as the \(H^0\)-shadow of the derived-center brace comparison
...
The decomposition is canonical relative to the chosen strict flat
representative, the supplied map~$\iota_Z$, and the represented
involution~$\sigma$.  Chosen quasi-isomorphism zigzags compare
admissible presentations of the underlying bar object.  A contractible
comparison space for the enhanced triples
$(\mathbf C_g(\cA),\iota_Z,\sigma)$ is an open construction problem.
\end{remark}
...
Let $(\cA, \cA^!)$ be a chiral Koszul pair, let \((g,n)\) be stable,
fix the genuine-partner datum of
Definition~\ref{def:complementarity-complexes}, and let
$V = H^*(\overline{\mathcal{M}}_{g,n}, \mathcal{Z}(\cA))$.
Assume that the strict flat ambient complex is perfect, so that
$V$ is finite-dimensional in each relevant cohomological degree and
the natural double-dual map is an isomorphism. Assume also that the
Koszul pairing identifies
...
\item Verdier duality on the center local system $\mathcal{Z}(\cA)$
 ...
 \]
 of cohomological degree $-d_{g,n}$ \textup{(}the shift coming from
 $\dim_{\mathbb{C}} \overline{\mathcal{M}}_{g,n}=d_{g,n}$\textup{)}.
...
$\overline{\mathcal{M}}_{g,n}$ is a constructible sheaf. Under the
perfectness hypothesis, Verdier duality gives a perfect pairing
$H^k(\overline{\mathcal{M}}_{g,n}, \mathcal{Z}) \otimes
H^{d_{g,n}-k}(\overline{\mathcal{M}}_{g,n}, \mathbb{D}\mathcal{Z})
\to \mathbb{C}$.
The Koszul pairing identifies $\mathbb{D}\mathcal{Z}(\cA) \cong
\mathcal{Z}(\cA^!)$. Transport along the represented
genuine-partner map~$j_Z^{-1}$ returns the second factor to the
$\cA$-ambient complex, so the pairing lands on $V \otimes V$.
Non-degeneracy follows from Verdier
duality for constructible sheaves on the smooth Deligne--Mumford
stack $\overline{\mathcal{M}}_{g,n}$.
...
perfectness/reflexivity hypothesis and the non-degenerate
anti-invariant Verdier pairing of
Proposition~\ref{prop:lagrangian-eigenspaces}, the eigenspaces
$Q_{g,n}(\cA)$ and the transported genuine partner
$(H^*\chi^-_{\cA;g,n})^{-1}Q_{g,n}(\cA^!)$ become complementary
Lagrangians.
...
\textup{[Type signature: Open quadrant; strict-flat bar-chart and
centre-local-system presentations; Beilinson levels~2 and~3;
\(H_{\mathrm C_0}\cup H_{\mathrm C_{1a}}^{\mathrm{partner}}
\cup H_{\mathrm C_{1b}}^{\mathrm{pair}}\) plus the paired chart map
\(\iota_g\).]}
...
\item If $\mathbf{Q}_{g,n_g}(\cA)$ and
 $(\chi^-_{\cA;g,n_g})^{-1}\mathbf{Q}_{g,n_g}(\cA^!)$ are
...
$\mathbf{Q}_{g,n_g}(\cA)$ and
$(\chi^-_{\cA;g,n_g})^{-1}\mathbf{Q}_{g,n_g}(\cA^!)$ pulls back to isotropy of
$L_g^+$ and $L_g^-$. Since $\chi_g$ is a quasi-isomorphism and
$\mathbf{C}_g(\cA)=\mathbf{Q}_{g,n_g}(\cA)\oplus
(\chi^-_{\cA;g,n_g})^{-1}\mathbf{Q}_{g,n_g}(\cA^!)$ is a
...
Assume the $\mathrm{C}_2$ shifted-symplectic hypothesis package of
Definition~\ref{def:C2-shifted-symplectic-hypothesis-package}. In
particular, assume the conditional BV package of
Theorems~\ref{thm:config-space-bv} and~\ref{thm:bv-functor}, and the
cyclic nondegeneracy input: the BV bracket adjoint
\[
...
$Q_{1,1}(\mathcal{H}_\kappa)\oplus
(H^*\chi^-_{\mathcal H_\kappa;1,1})^{-1}
Q_{1,1}(\mathcal{H}_\kappa^!)
...
\item \emph{BV antibracket is $(-1)$-shifted Poisson.}
 ...
 Theorem~\textup{\ref{thm:verdier-bar-cobar}} to the Verdier algebra
 \[
 K_X(\cA)=\mathbb{D}_{\operatorname{Ran}}
 (\barB^{\mathrm{ch}}(\cA)),
 \]
 and then by the chiral Koszul-pair map
 $\nu_{12}\colon K_X(\cA)\xrightarrow{\sim}\cA^!$, the adjoint map
 \[
 x \longmapsto \{x,-\}_{\mathrm{BV}} \colon
 \barB^{\mathrm{ch}}(\cA)
 \longrightarrow
 ...
 \cA^![1]
 \]
 is precisely the cyclic duality quasi-isomorphism assumed above.
 Therefore this Poisson structure is non-degenerate under the stated
 cyclic nondegeneracy package.
...
cohomology groups are $Q_{g,n_g}(\cA)$ and the transported genuine
partner
$(H^*\chi^-_{\cA;g,n_g})^{-1}Q_{g,n_g}(\cA^!)$.
\end{proof}
...
pairing. It supplies the diagonal scalar input for the
\textup{(C2)} shifted-symplectic upgrade on the locus above, provided
the remaining inputs of
Definition~\ref{def:C2-shifted-symplectic-hypothesis-package} are also
supplied. For multi-weight classes with
...
\textup{[Type signature: Open quadrant; complete filtered strict-flat
centre-local-system presentation; Beilinson level~3;
\(H_{\mathrm C_{1a}}^{\mathrm{partner}}\) plus filtration compatibility,
with \(H_{\mathrm C_{1b}}^{\mathrm{pair}}\) for the Lagrangian clause.]}
...
\[
  \mathbf{C}_g^{\leq N}(\cA)
  =
  \mathbf{Q}^{+}_{g,n_g,\leq N}(\cA)
  \oplus
  \mathbf{Q}^{-}_{g,n_g,\leq N}(\cA).
\]
When the genuine-partner map preserves the filtration, it induces
$\chi^-_{\cA;g,n_g,\leq N}\colon
\mathbf Q^-_{g,n_g,\leq N}(\cA)\xrightarrow{\sim}
\mathbf Q_{g,n_g,\leq N}(\cA^!)$.
...
\[
  ...
  =
  \varprojlim_N \mathbf{Q}^{+}_{g,n_g,\leq N}(\cA)
  \oplus
  \varprojlim_N \mathbf{Q}^{-}_{g,n_g,\leq N}(\cA)
\]
...
\textup{[Type signature: Open quadrant; unpointed strict-flat
centre-local-system presentation; Beilinson level~3; \(g\geq2\) and
\(H_{\mathrm C_0}\cup H_{\mathrm C_{1a}}^{\mathrm{partner}}
\cup H_{\mathrm C_{1b}}^{\mathrm{pair}}\).]}
Assume the cochain-level complementarity decomposition, the
perfectness/reflexivity hypothesis, and the non-degenerate Verdier pairing of
Theorem~\textup{\ref{thm:quantum-complementarity-main}}. For a chiral
Koszul pair $(\cA, \cA^!)$ and $g \geq 2$, the cochain
complex
$C_{g,0} := R\Gamma(\overline{\mathcal{M}}_{g,0}, \mathcal{Z}(\cA))$
carries a $(-d_{g,0})$-shifted symplectic structure in the sense of
Pantev--To\"en--Vaqui\'e--Vezzosi~\cite{PTVV13}, and
\(\mathbf{Q}_{g,0}(\cA)\) and
\((\chi^-_{\cA;g,0})^{-1}\mathbf{Q}_{g,0}(\cA^!)\) embed as complementary
Lagrangians in the
PTVV sense.
\end{proposition}
...
Perfectness identifies $V^*$ with the derived dual, and
non-degeneracy of the pairing translates to non-degeneracy of the
$2$-form.
The Verdier pairing on $C_{g,0}$
(Proposition~\ref{prop:lagrangian-eigenspaces}(i)) is a
non-degenerate symmetric pairing of degree $-d_{g,0}$, so $C_{g,0}$
carries a $(-d_{g,0})$-shifted symplectic structure.
...
\end{enumerate}
For $L = \mathbf{Q}_{g,0}(\cA)$:
\begin{itemize}
\item Condition~(a) is
 Proposition~\ref{prop:lagrangian-eigenspaces}(iii):
 $\langle v, w \rangle_{\mathbb{D}} = 0$ for
 $v, w \in \mathbf{Q}_{g,0}(\cA)$.
\item Condition~(b): Since
 $C_{g,0}=\mathbf{Q}_{g,0}(\cA)\oplus
 (\chi^-_{\cA;g,0})^{-1}\mathbf{Q}_{g,0}(\cA^!)$
 (Theorem~\ref{thm:quantum-complementarity-main}), we have
 $C_{g,0}/\mathbf{Q}_{g,0}(\cA)\cong
 (\chi^-_{\cA;g,0})^{-1}\mathbf{Q}_{g,0}(\cA^!)$, and the induced map
 $\mathbf{Q}_{g,0}(\cA)\to
 ((\chi^-_{\cA;g,0})^{-1}\mathbf{Q}_{g,0}(\cA^!))^{\vee}[-d_{g,0}]$ is a
 quasi-isomorphism: the Verdier pairing restricted to
 $\mathbf{Q}_{g,0}(\cA)\otimes
 (\chi^-_{\cA;g,0})^{-1}\mathbf{Q}_{g,0}(\cA^!)$ is perfect of degree $-d_{g,0}$
...
$Q_{g,n_g}(\cA)$ and
$(H^*\chi^-_{\cA;g,n_g})^{-1}Q_{g,n_g}(\cA^!)$ are complementary
Lagrangians; the
...
\textup{[Type signature: Open-to-physical comparison; strict-flat
centre and BRST presentations; Beilinson levels~3--5;
\(H_{\mathrm C_0}\cup H_{\mathrm C_{1a}}^{\mathrm{partner}}\)
plus the displayed BRST trace package.]}
...
eigenspaces
$\mathbf Q_{g,n_g}(\cA)\oplus
(\chi^-_{\cA;g,n_g})^{-1}\mathbf Q_{g,n_g}(\cA^!)$ of
...
centre-local-system presentation; Beilinson level~3;
\(H_{\mathrm C_0}\cup H_{\mathrm C_{1a}}^{\mathrm{partner}}\)
plus mapping-class equivariance.]}
...
$Q_{g,n_g}(\cA)$ and
$(H^*\chi^-_{\cA;g,n_g})^{-1}Q_{g,n_g}(\cA^!)$ by
...
\begin{corollary}[Reconstruction of the genuine partner summand;
\ClaimStatusConditional]
\label{cor:uniqueness-quantum}
\textup{[Type signature: Open quadrant; strict-flat
centre-local-system presentation; Beilinson level~3;
\(H_{\mathrm C_0}\cup H_{\mathrm C_{1a}}^{\mathrm{partner}}\),
with \(H_{\mathrm C_{1b}}^{\mathrm{pair}}\) for the duality formula.]}
For a stable genus base \((g,n_g)\), the quotient by the positive
summand reconstructs the transported genuine partner:
\begin{equation}
(H^*\chi^-_{\mathcal A;g,n_g})^{-1}Q_{g,n_g}(\mathcal A^!)
\cong
H^*(\overline{\mathcal M}_g,Z(\mathcal A))/Q_{g,n_g}(\mathcal A).
\end{equation}
Applying~$H^*\chi^-_{\mathcal A;g,n_g}$ recovers the genuine summand
inside $H^*(\mathbf C_{g,n_g}(\mathcal A^!))$.  Under the perfect
anti-invariant Verdier pairing,
\[
Q_{g,n_g}(\mathcal A)
\cong Q_{g,n_g}(\mathcal A^!)^\vee[-d_{g,n_g}].
...
Directness and exhaustion
(Lemmas~\ref{lem:trivial-intersection-complete}
and~\ref{lem:exhaustion-complete}) give
\begin{equation}
H^*(\overline{\mathcal{M}}_g,Z(\mathcal{A}))
=Q_{g,n_g}(\mathcal{A})\oplus
(H^*\chi^-_{\mathcal A;g,n_g})^{-1}Q_{g,n_g}(\mathcal{A}^!).
\end{equation}
Taking the quotient gives the transported-partner formula.  The map
$H^*\chi^-_{\mathcal A;g,n_g}$ returns it to the genuine partner
centre complex.  Corollary~\ref{cor:quantum-dual-complete} supplies
the shifted cross-duality under~$H_{\mathrm C_{1b}}^{\mathrm{pair}}$.
Constructively, one computes the Leray object~$G_g(\mathcal A)$,
applies the fibre--centre comparison and~$p^+_{\mathcal A}$, and then
takes the displayed quotient.  Examples~\ref{ex:heisenberg-complementarity-explicit}
and~\ref{ex:kac-moody-complementarity-explicit} realize this procedure.
\end{proof}

\begin{corollary}[Vanishing loci of the two summands;
\ClaimStatusConditional]
\label{cor:vanishing-quantum}
\textup{[Type signature: Open quadrant; strict-flat
centre-local-system presentation; Beilinson level~3;
\(H_{\mathrm C_0}\cup H_{\mathrm C_{1a}}^{\mathrm{partner}}\).]}
On the locus $Q_{g,n_g}(\mathcal A)=0$, the transported genuine
partner exhausts the ambient cohomology:
\begin{equation}
(H^*\chi^-_{\mathcal A;g,n_g})^{-1}Q_{g,n_g}(\mathcal{A}^!)
\cong H^*(\overline{\mathcal{M}}_g,Z(\mathcal{A})).
\end{equation}
Simultaneous vanishing of the two genuine summands is equivalent to
\begin{equation}
...
(H^*\chi^-_{\mathcal A;g,n_g})^{-1}Q_{g,n_g}(\mathcal{A}^!).
\end{equation}
Substitution proves the first formula.  Directness and exhaustion make
simultaneous vanishing equivalent to vanishing of the ambient
cohomology.
\end{proof}

\begin{remark}[Examples of vanishing]
At the stable tree-level base \(\overline{\mathcal M}_{0,3}\), a
one-dimensional unit coefficient complex with
\(\sigma_{\cA}=+1\) and anti-equivariant partner comparison gives
$Q_{0,3}(\cA)=\mathbb C$ and $Q_{0,3}(\cA^!)=0$.
The general pointed genus-zero theory retains the coefficient
eigenspaces together with the Beilinson--Drinfeld/Arnold operations.
For TFTs the computed branch satisfies $Q_{g,n_g}(\cA)=0$ on every
stable base.  For $bc$--$\beta\gamma$,
Theorem~\ref{thm:fermion-all-genera} computes cancellation in the
scalar complementarity sum while the individual eigensummands retain
...
\textup{(}Convention~\textup{\ref{conv:regime-tags})}.]}
\textup{[Type signature: Open quadrant; strict-flat
centre-local-system presentation; Beilinson level~3; stable pair
\((g,n)\); \(H_{\mathrm C_0}\cup
H_{\mathrm C_{1a}}^{\mathrm{partner}}\) plus a balanced self-duality
representative.]}

Let $(\cA, \cA^!)$ be a Koszul chiral pair
(Definition~\ref{def:chiral-koszul-pair}), and fix a stable pair
\((g,n)\) in
...
\[
 \mathbf C_{g,n}(\cA)=R\Gamma(\overline{\mathcal M}_{g,n},\mathcal Z(\cA)).
\]
Assume that a self-duality isomorphism
$f\colon \cA\xrightarrow{\sim}\cA^!$ induces a cochain automorphism
$\tau_f$ of $\mathbf C_{g,n}(\cA)$, after the centre comparison, with
...
\;=\; \tfrac{1}{2}\,\dim H^*(\overline{\mathcal{M}}_{g,n}, Z(\cA))
\end{equation}
whenever the displayed cohomology is finite-dimensional.  For an
arbitrary represented self-duality, the trace of the Verdier
involution measures the imbalance:
\begin{equation}\label{eq:self-dual-trace-defect}
\dim Q_{g,n}(\cA)-\dim Q_{g,n}(\cA^!)
=
\operatorname{Tr}\!\left(\sigma\mid
H^*(\mathbf C_{g,n}(\cA))\right).
\end{equation}
\end{theorem}

\begin{proof}
Let $V=H^*(\mathbf C_{g,n}(\cA))$. Theorem~\ref{thm:quantum-complementarity-main}
identifies
\[
Q_{g,n}(\cA)=V^+=\ker(\sigma-\mathrm{id}),\qquad
(H^*\chi^-_{\cA;g,n})^{-1}Q_{g,n}(\cA^!)
=V^-=\ker(\sigma+\mathrm{id}).
\]
If $x\in V^+$, then
...
\[
\dim Q_{g,n}(\cA) + \dim Q_{g,n}(\cA^!)
= \dim H^*(\overline{\mathcal{M}}_{g,n}, Z(\cA))
\]
...
=\dim Q_{g,n}(\cA)-\dim Q_{g,n}(\cA^!).
\]
The equation~\eqref{eq:balanced-self-dual-antiequivariance} makes the
trace zero and is precisely the balanced-representative datum used by
the halving clause.
\end{proof}
...
\textup{[Type signature: Open quadrant; strict-flat
centre-local-system presentation; Beilinson level~3; stable pair
\((g,n)\); Virasoro self-duality plus the balanced-representative
hypothesis package.]}
...
$\mathbf C_{g,n}(\mathrm{Vir}_{13})$ by a balanced automorphism
$\tau_{13}$ satisfying
$\tau_{13}\sigma=-\sigma\tau_{13}$. Then for every stable pair
\((g,n)\) with finite-dimensional cohomology:
\begin{equation}\label{eq:virasoro-quantum-dim}
\dim Q_{g,n}(\mathrm{Vir}_{13})
= \tfrac{1}{2}\,\dim H^*(\overline{\mathcal{M}}_{g,n}).
\end{equation}
Explicitly: $\dim Q_{1,1}(\mathrm{Vir}_{13}) = 1$ and
$\dim Q_{2,0}(\mathrm{Vir}_{13}) = 4$.
The uncurved point $c=0$ belongs to the two-object orbit
$\mathrm{Vir}_0^!=\mathrm{Vir}_{26}$; the balanced fixed point used
by the halving formula is $c=13$.
\end{corollary}
...
\textup{[Type signature: Open quadrant; curved scalar and strict-flat
centre-local-system presentations; Beilinson levels~2, 3, and~5;
scalar uncurving plus the named \(\mathrm C_0/\mathrm C_1\) hypothesis
package for the displayed decomposition.]}
...
Q_{g,n_g}(V_{-h^\vee}(\mathfrak g))
\;\oplus\;
(H^*\chi^-_{V_{-h^\vee}(\mathfrak g);g,n_g})^{-1}
Q_{g,n_g}(V_{-h^\vee}(\mathfrak g)^!)
\;=\;
...
The scalar vanishing governs the scalar diagonal.  The full ambient
decomposition is carried by the \(\mathrm C_0/\mathrm C_1\) package;
the represented involution computes the individual eigenspaces, and a
separate Drinfeld--Sokolov comparison identifies any resulting
$\mathcal W$-algebra realization.
\end{corollary}

\begin{proof}
From Table~\ref{tab:obstruction-summary}, the genus-$g$ signed Hodge
component for $V_k(\mathfrak g)$ is
\[
\operatorname{obs}^{\mathrm{Hdg}}_g(V_k(\mathfrak g))
=(-1)^g\kappa(V_k(\mathfrak g))\lambda_g
=(-1)^g\frac{(k+h^\vee)\dim(\mathfrak{g})}{2h^\vee}\lambda_g
\]
...
invariants packages both $Q_{g,n_g}(\mathcal{A})$ and $Q_{g,n_g}(\mathcal{A}^!)$ into a single
modular form.
\end{itemize}
\end{conjecture}
...
\textup{[Type signature: Open quadrant; pointed strict-flat bar and
centre-local-system presentations; Beilinson levels~2 and~3;
\((g,n)=(1,1)\) and the represented \(\mathrm C_0/\mathrm C_1\)
hypothesis package.]}
...
\emph{Step 1: Compute $Q_{1,1}(\mathcal{H}_\kappa)$}.
...
The \emph{fiberwise} collision operator $d_{\mathrm{fib}}$ is curved;
its curvature is measured by the quasi-periodicity of the propagator
around the $B$-cycle.  The total differential
$d^{(1)}_{\mathrm{total}}$ includes the Gauss--Manin correction and is
square-zero by Theorem~\ref{thm:genus1-d-squared}.
...
Taking cohomology of the total differential first gives the Leray
correction object; the represented fibre--centre comparison and
positive projector then give
\begin{equation}
G_1(\mathcal H_\kappa)
=\operatorname{gr}^{\bullet}_{\mathrm{Leray}}
H^*(\bar B^{(1)}(\mathcal H_\kappa),d^{(1)}_{\mathrm{total}}),
\qquad
Q_{1,1}(\mathcal H_\kappa)
=\operatorname{im}\!\left(
p^+_{\mathcal H_\kappa}\mid H^*(\mathbf C_{1,1}(\mathcal H_\kappa))
\right)
=\mathbb C\cdot\kappa.
\end{equation}

\emph{Step 2: Compute $Q_{1,1}(\mathcal{H}_\kappa^!)$ using complementarity}.
...
The algebra $\mathcal{H}_\kappa^!=\mathrm{Sym}^{\mathrm{ch}}(V^*)$
is generated by the linear dual~$V^*$; the level-inverted Heisenberg
algebra generated by~$V$ is a separate presentation
...
(H^*\chi^-_{\mathcal H_\kappa;1,1})^{-1}
Q_{1,1}(\mathcal{H}_\kappa^!)
=H^*(\overline{\mathcal{M}}_{1,1},Z(\mathcal{H}_\kappa))
/Q_{1,1}(\mathcal{H}_\kappa)
\end{equation}

Since $H^*(\overline{\mathcal{M}}_{1,1}) = \mathbb{C} \oplus \mathbb{C}\lambda$ and
$Q_{1,1}(\mathcal{H}_\kappa) = \mathbb{C} \cdot \kappa$ (which pairs with $H^0 = \mathbb{C}$),
we have:
\begin{equation}
(H^*\chi^-_{\mathcal H_\kappa;1,1})^{-1}
Q_{1,1}(\mathcal{H}_\kappa^!)=\mathbb C\lambda,
\qquad
Q_{1,1}(\mathcal H_\kappa^!)
=H^*\chi^-_{\mathcal H_\kappa;1,1}(\mathbb C\lambda).
\end{equation}

\emph{Interpretation}: $Q_{1,1}(\mathcal{H}_\kappa)=
\mathbb C\cdot\kappa$ records the central-extension branch, while the
genuine partner $Q_{1,1}(\mathcal H_\kappa^!)$ transports to the Hodge
line~$\mathbb C\lambda$.

Together they span:
\begin{equation}
Q_{1,1}(\mathcal{H}_\kappa)\oplus
(H^*\chi^-_{\mathcal H_\kappa;1,1})^{-1}
Q_{1,1}(\mathcal{H}_\kappa^!)=\mathbb{C}\oplus\mathbb{C}
= H^*(\overline{\mathcal{M}}_{1,1})
\end{equation}

\emph{Verification}: the cobar complex of the curved commutative
algebra $\mathrm{CE}(\mathfrak h_{-\kappa})$, followed by the genuine
partner comparison, yields the genus-$1$ Hodge line
$\mathbb C\cdot\lambda$.
\end{example}

\begin{example}[Kac--Moody scalar lane at critical level]
\label{ex:kac-moody-complementarity-explicit}
\textup{[Type signature: Open quadrant; scalar-curvature and
strict-flat centre-local-system presentations; Beilinson levels~2,
3, and~5; critical affine family plus the named
\(\mathrm C_0/\mathrm C_1\) hypothesis package.]}
...
ambient centre complex.  An explicit critical-level bar computation
determines the individual eigenspaces and any quadratic-Casimir or
Hodge-line realization.
...
from the affine algebra by quantum Drinfeld--Sokolov reduction.  A
comparison theorem for the DS functor, with its own hypothesis
package, is the map that can relate these two constructions; Theorem~C
defines~$\cA^!$ through the bar--Verdier lane.
\end{remark}
\end{example}
...
\textup{[Type signature: Open quadrant; pointed strict-flat
centre-local-system presentation; Beilinson level~3;
\((g,n)=(1,1)\), the displayed Koszul-pair comparison, and the
represented \(\mathrm C_0/\mathrm C_1\) hypothesis package.]}
...
\begin{equation}
Q_{1,1}(\beta\gamma) = \mathbb{C} \cdot [\beta\gamma]
\end{equation}
where $[\beta\gamma]$ is the first descendant of the identity operator.

Since $(\beta\gamma)^! \cong \mathcal{F}$ (free fermions):
\begin{equation}
Q_{1,1}((\beta\gamma)^!) = Q_{1,1}(\mathcal{F}) = \mathbb{C} \cdot [\psi \partial\psi]
\end{equation}

By complementarity:
\begin{equation}
Q_{1,1}(\beta\gamma)\oplus
(H^*\chi^-_{\beta\gamma;1,1})^{-1}Q_{1,1}(\mathcal F)
=\mathbb C\cdot[\beta\gamma]\oplus
\mathbb C\cdot[\psi\partial\psi].
\end{equation}
...
The represented \(\mathrm C_1\) projector identifies
$Q_{1,1}(\beta\gamma)$, and the leading $q^0$ coefficient~$1$ gives
the independent dimension check
$\dim Q_{1,1}(\beta\gamma)=1$.
\end{example}
...
\textup{[Type signature: Open quadrant; unpointed strict-flat
centre-local-system presentation; Beilinson level~3;
\((g,n)=(2,0)\) and the represented \(\mathrm C_0/\mathrm C_1\)
hypothesis package.]}
...
\begin{equation}
Q_{2,0}(\mathcal{H}_\kappa)\oplus
(H^*\chi^-_{\mathcal H_\kappa;2,0})^{-1}
Q_{2,0}(\mathcal{H}_\kappa^!)
= H^*(\overline{\mathcal{M}}_2),
\qquad
\dim H^*(\overline{\mathcal{M}}_2) = 8.
\end{equation}
The individual dimensions
$\dim Q_{2,0}(\mathcal{H}_\kappa)$ and $\dim Q_{2,0}(\mathcal{H}_\kappa^!)$
...
On the represented \(\mathrm C_0/\mathrm C_1\) locus, directness and
exhaustion give
$\dim Q_{2,0}(\mathcal H_\kappa)+
\dim Q_{2,0}(\mathcal H_\kappa^!)=8$.
\end{example}
...
The explicit stable-genus computations above are for the abelian
...
represented \(\mathrm C_0/\mathrm C_1\) package gives, for $g\geq2$,
the conditional decomposition
$Q_{g,0}(\cA)\oplus
(H^*\chi^-_{\cA;g,0})^{-1}Q_{g,0}(\cA^!)
=H^*(\mathbf C_{g,0}(\cA))$
(Theorem~\ref{thm:quantum-complementarity-main}).  Explicit
higher-genus bar calculations will determine the individual
dimensions~$\dim Q_{g,0}(\cA)$.

The sharpest test case is $Q_{1,1}(\widehat{\mathfrak{sl}}_{2,k})$
...
We conclude with a practical algorithm for computing $Q_{g,n_g}(\mathcal{A})$ and verifying
complementarity.

\begin{remark}[Computing quantum corrections]\label{rem:quantum-corrections}
\textup{[Type signature: Open quadrant; relative flat-bar and
strict-flat centre-local-system presentations; Beilinson levels~2
and~3; \(H_{\mathrm C_0}\cup
H_{\mathrm C_{1a}}^{\mathrm{partner}}\).]}
Given a chiral algebra $\mathcal{A}$ on a curve $X$ of genus $g$, the quantum correction space $Q_{g,n_g}(\mathcal{A})$ is computed as follows
...
\item Form the Leray correction object
$G_g(\mathcal A)=\operatorname{gr}^{\bullet}_{\mathrm{Leray}}
H^*(\bar B^{(g)}(\mathcal A),\Dg{g})$, apply the \(\mathrm C_0\)
fibre--centre comparison, and project with
$p^+_{\mathcal A}=\tfrac12(\mathrm{id}+\sigma_{\mathcal A})$.  The
result is $Q_{g,n_g}(\mathcal A)$.
\item Verify complementarity inside the \(\mathcal A\)-ambient complex:
$\dim Q_{g,n_g}(\mathcal A)+\dim Q_{g,n_g}(\mathcal A^!)
=\dim H^*(\overline{\mathcal M}_g,\mathcal Z(\mathcal A))$,
where the second dimension is represented by
$(H^*\chi^-_{\mathcal A;g,n_g})^{-1}Q_{g,n_g}(\mathcal A^!)$.
\end{enumerate}
\end{remark}

\begin{example}[Algorithm applied to Heisenberg]
\textup{[Type signature: Open quadrant; pointed relative flat-bar and
strict-flat centre-local-system presentations; Beilinson levels~2
and~3; \((g,n)=(1,1)\) and the represented
\(\mathrm C_0/\mathrm C_1\) hypothesis package.]}
For $\mathcal{H}_\kappa$ at genus 1:
\begin{enumerate}
...
\item The Leray object of
$H^*(\bar B^{(1)}(\mathcal H_\kappa),d^{(1)}_{\mathrm{total}})$
maps through the fibre--centre comparison, and its positive projection
is $Q_{1,1}(\mathcal H_\kappa)=\mathbb C\cdot\kappa$.
\item $\dim Q_{1,1}(\mathcal{H}_\kappa) + \dim Q_{1,1}(\mathcal{H}_\kappa^!) = 1 + 1 = 2 =
...
the formulas of Proposition~\ref{prop:complementarity-landscape}.
The Virasoro row is $N = 2$ in the $\cW_N$ series.}
...
\textup{[Type signature: Open quadrant; strict-flat centre and scalar
trace presentations; Beilinson levels~3 and~5;
\(H_{\mathrm C_0}\cup H_{\mathrm C_{1a}}^{\mathrm{partner}}\)
plus the Theorem~D scalar-trace package.]}
...
\mathbf{C}_g(\cA)
\;\simeq\;
\mathbf{Q}_{g,n_g}(\cA)\oplus
(\chi^-_{\cA;g,n_g})^{-1}\mathbf{Q}_{g,n_g}(\cA^!).
\]
...
after dividing by the universal Hodge factor. Equivalently, on the
strict flat centre-comparison surface, it is the
\(\operatorname{tr}_{\kappa}\)-image of the degree-two diagonal
derived-centre residue class of
Definition~\ref{def:derived-centre-scalar-trace-kappa}; this is a
scalar trace shadow, not a reconstruction of the derived centre or
of a physical bulk object.
...
For Bershadsky--Polyakov, the standard
Fehily--Kawasetsu--Ridout conformal vector gives the exact identity
\[
K^c_{\mathrm{BP}}
=c_{\mathrm{BP}}(k)+c_{\mathrm{BP}}(-k-6)=50.
\]
The four strong generators $J,T,G^+,G^-$ are even.  Consequently the
modular characteristic requires the complete genus-one
nonseparating-curvature calculation for the minimal-DS complex.  Its
status is \ClaimStatusOpen.  A result
$\kappa_{\mathrm{BP}}=c_{\mathrm{BP}}/6$ would imply the conditional
value $K^\kappa_{\mathrm{BP}}=25/3$.  The exact central conductor and
the open modular conductor therefore occupy separate scalar lanes.
\end{remark}
...
$Q_{g,n_g}(\cA)\oplus
(H^*\chi^-_{\cA;g,n_g})^{-1}Q_{g,n_g}(\cA^!)$ is a balanced decomposition
with $\dim Q_{g,n_g}(\cA) = \dim Q_{g,n_g}(\cA^!)$
(Theorem~\ref{thm:self-dual-halving}).
The Virasoro chiral Koszul self-dual point is $c=13$
(Corollary~\ref{cor:virasoro-quantum-dim}).
...
factorization-chain object.  For a quadratic presentation
$\mathcal A^{\mathrm i}=C_X(s^{-1}V,s^{-2}R)$, the comparison
$q_{\mathcal A}\colon\mathcal A^{\mathrm i}\to\barB(\mathcal A)$
identifies its cohomology with bar cohomology on the Koszul locus, and
...
Lemma~\ref{lem:DR-verdier-compat} identifies the upper-right route
with the topological dual of
$\operatorname{DR}(\mathcal M_n)$.  The factorization-chain
comparison identifies this object with
$\mathbb D_{\mathrm{top}}(\int_{\operatorname{Conf}_n(X)}\cA)$.
Lemma~\ref{lem:AF-dual-chiral} identifies the lower route with the
same topological dual.  Naturality of these two comparisons gives the
commutative square. \qedhere
\end{proof}
...
\begin{corollary}[Geometric bar and factorization chains; \ClaimStatusProvedHere]\label{cor:bar-is-fh}
The geometric Ran bar complex $\bar{B}^{\text{geom}}(\mathcal{A})$
models the configuration-space chain presentation of factorization homology:
\[\text{DR}(\bar{B}^{\text{geom}}(\mathcal{A})) \simeq \int_X \mathcal{A}\]
This is a Ran/configuration-space statement over~$X$, not the
one-dimensional ordered-bar statement \(B(A)=\int_{\mathbb R}A\);
for an \(\Eone\)-algebra the line has \(\int_{\mathbb R}A\simeq A\),
whereas the reduced algebraic bar is the interval theory with
augmentation boundary conditions.
\end{corollary}

\begin{proof}
By the compatibility theorem just proved, the de Rham functor intertwines Verdier duality on $\mathcal{D}$-modules with Ayala--Francis duality on factorization algebras. The geometric Ran bar complex is defined as $\bar{B}^{\mathrm{geom}}(\mathcal{A}) = \bigoplus_n \Gamma(\overline{C}_{n+1}(X), \mathcal{A}^{\boxtimes(n+1)} \otimes \Omega^n_{\log})$, which under the de Rham functor becomes the configuration-space chain model for $\int_X \mathcal{A}$: both compute the derived tensor product $\mathcal{A} \otimes^{\mathbb{L}}_{\mathrm{Disk}(X)} \mathrm{pt}$ (Ayala--Francis~\cite{AF15}, Theorem~3.24; for the identification with our geometric bar complex, see Theorem~\ref{thm:fact-homology-quantum}).
\end{proof}
...
\begin{lemma}[Geometric bar as factorization chains; \ClaimStatusProvedHere]\label{lem:bar-as-fact-hom-AF}
The geometric Ran bar construction computes the factorization-chain model:
...
This is the geometric Ran bar construction:
...
\emph{Step 4: Verdier algebra of the bar coalgebra}

Verdier duality on the bar coalgebra gives the algebra
\[
K_X(\cA):=\mathbb{D}_{\text{Ran}(X)}(\bar{B}(\mathcal{A})).
\]
A chiral Koszul pair carries a separate equivalence
$\nu_{12}\colon K_X(\cA)\xrightarrow{\sim}\cA^!$.

\begin{lemma}[Algebra structure on the Verdier dual of the bar;
\ClaimStatusConditional]\label{lem:coalgebra-verdier-AF}
Assume the finite-window dualizability, tensor-exchange, and
completion hypotheses of Theorem~\ref{thm:bar-cobar-verdier}.
Then $K_X(\cA)$ is a factorization algebra whose multiplication is the
Verdier transpose of bar deconcatenation.
\end{lemma}

\begin{proof}[Proof of Lemma]
The bar coalgebra has deconcatenation
\[
 \Delta_B\colon B_X(\cA)\longrightarrow
 B_X(\cA)\widehat\otimes B_X(\cA).
\]
Contravariance and tensor exchange give
\[
 \mathbb D B_X(\cA)\widehat\otimes\mathbb D B_X(\cA)
 \xrightarrow{\ \sim\ }
 \mathbb D(B_X(\cA)\widehat\otimes B_X(\cA))
 \xrightarrow{\ \mathbb D(\Delta_B)\ }
 \mathbb D B_X(\cA).
\]
Coassociativity of $\Delta_B$ gives associativity of this
multiplication, and completion follows from the stated hypotheses.
\end{proof}
...
multi-weight algebras at $g \geq 2$, the full coefficient is the
scalar trace plus the cross-channel summand of
...
map $\mathrm{tr} \colon \HH^{E_1}_\bullet(\int_\gamma\cA) \to
...
$Q_{g,n_g}(\cA)\oplus
(H^*\chi^-_{\cA;g,n_g})^{-1}Q_{g,n_g}(\cA^!)
\cong H^*(\overline{\mathcal{M}}_g,Z(\cA))$ on the represented
\(\mathrm C_0/\mathrm C_1\) locus.  This is the \emph{linear shadow}
of a deeper geometric fact: there is \emph{one} ambient deformation problem,
...
\]
The ambient theorem instead imposes a single deformation
...
\begin{definition}[Ambient complementarity datum; \ClaimStatusDefinitional]
...
\begin{definition}[Complementarity formal moduli problems;
\ClaimStatusDefinitional]
\label{def:complementarity-formal-moduli}
\index{formal moduli problem!complementarity|textbf}
\index{derived intersection!complementarity}
Assume the ambient complementarity datum of
Definition~\ref{def:ambient-complementarity-datum} is represented by
complete filtered dg Lie algebras or cyclic \(L_\infty\)-algebras in
the Lurie--Pridham sense. Define:
\[
\mathcal M_{\cA}:=\mathrm{MC}(T_{\cA}),\qquad
\mathcal M_{\cA^!}:=\mathrm{MC}(T_{\cA^!}),\qquad
\mathcal M_{\mathrm{comp}}(\cA):=\mathrm{MC}(T_{\mathrm{comp}}(\cA)),
\]
where
\[
...
T_{\cA^!}:=\operatorname{fib}(\nabla_{\cA^!}\colon L_{\cA^!}\to K_{\cA})
\]
and \(T_{\mathrm{comp}}(\cA)\) is
Definition~\ref{def:ambient-complementarity-tangent-complex}. The
maps
\[
i_{\cA}\colon \mathcal M_{\cA}\to\mathcal M_{\mathrm{comp}}(\cA),
\qquad
i_{\cA^!}\colon \mathcal M_{\cA^!}\to\mathcal M_{\mathrm{comp}}(\cA)
\]
are induced by the inclusions of one-sided tangent complexes into the
homotopy-fibre tangent complex.

The derived intersection
\[
\mathcal M_{\cA}\times^h_{\mathcal M_{\mathrm{comp}}(\cA)}
\mathcal M_{\cA^!}
\]
is represented, when the comparison has been constructed, by the
Maurer--Cartan functor of the twisted tensor kernel
\[
K^{\mathrm{comp}}_{\tau}(\cA,\cA^!)
:=
\operatorname{fib}\!\left(
L_{\cA}\oplus L_{\cA^!}\xrightarrow{\nabla_{\cA}-\nabla_{\cA^!}}
K_{\cA}\right),
\]
with differential twisted by the universal kernel
\(\tau_{\cA}\). This representation is a theorem only after the
filtered dg Lie model for the homotopy pullback and the kernel
comparison maps are supplied; otherwise it is the missing converse
datum. Acyclicity of \(K^{\mathrm{comp}}_{\tau}(\cA,\cA^!)\) implies
transversality of the two Lagrangian maps, while the converse
requires the same homotopy-pullback model and perfectness hypotheses.
\end{definition}
...
Lagrangian tangent maps. Then the formal moduli problems of
Definition~\ref{def:complementarity-formal-moduli} exist, the ambient
one \(\mathcal{M}_{\mathrm{comp}}(\cA)\) carries a canonical
$(-1)$-shifted symplectic structure, and the one-sided deformation
problems define Lagrangian maps
\[
\mathcal{M}_{\cA}
\longrightarrow
...
the ambient formal moduli problem
$\mathcal{M}_{\mathrm{comp}}(\cA)$ of
Definition~\ref{def:complementarity-formal-moduli}; applying the same
construction to the one-sided tangent dg Lie algebras gives
$\mathcal M_\cA$ and $\mathcal M_{\cA^!}$.
...
This is the polarization form of the PTVV framework.  The
derived-intersection construction belongs to the cotangent-normal-form
lane, while the scalar conductor identity
$\kappa(\cA)+\kappa(\cA^!)=K$ enters after the Verdier eigenspace
decomposition is projected to the scalar lane and the family-specific
computations of Theorem~D are applied.
\end{remark}
...
is the $(-d_{g,n})$-shifted \emph{Verdier-symplectic} upgrade.
Verdier-on-$\overline{\mathcal{M}}_{g,n}$
...
The chiral-to-CY functor of Vol~III is the comparison map relating
these two structures; the present theorem uses the Verdier lane.
\end{remark}
...
delivers the genus-wise Lagrangian decomposition under
$H_{\mathrm C_{1b}}^{\mathrm{pair}}$;
Theorem~\textup{\ref{thm:ambient-complementarity-fmp}}
delivers a $(-1)$-shifted ambient picture under its cyclic formal-moduli
package.  Under the clutching-coherence hypotheses of
Theorem~\ref{thm:lagrangian-complementarity-global-upgrade}, the
$(-d_{g,n})$-shifted Verdier-symplectic modular tangent bundle admits a
\emph{single} global Lagrangian section whose
...
$\mathbf{C}_g(\cA)\simeq\mathbf{Q}_{g,n_g}(\cA)\oplus
(\chi^-_{\cA;g,n_g})^{-1}\mathbf{Q}_{g,n_g}(\cA^!)$ of
Theorem~\ref{thm:quantum-complementarity-main}
...
cohomology of moduli space. This is a typed linear comparison; a
physical bulk factorization algebra belongs to the separate OCA lane.

The linear model is the following for $g\geq2$. The genus-$g$ ambient complex
$\mathbf{C}_{g,0}(\cA) = R\Gamma(\overline{\mathcal{M}}_{g,0},
\mathcal{Z}(\cA))$ is a shifted-symplectic linear space in the sense of
Proposition~\ref{prop:ptvv-lagrangian}, with a
$(-d_{g,0})$-shifted symplectic form.
The two complementary branches $\mathbf{Q}_{g,0}(\cA)$ and
$(\chi^-_{\cA;g,0})^{-1}\mathbf{Q}_{g,0}(\cA^!)$ are Lagrangian
subspaces of this phase space,
...
The theorems in this section use the conditional comparison surfaces
of Theorems~A--D together with finite-rank linear algebra on perfect
dg models.  Explicit Gaussian calculations use
...
sign rule.  Each conclusion below retains the comparison hypotheses
on which its linear model depends.
...
\textup{[Type signature: Open quadrant; coderived factorization and
strict-flat centre-local-system presentations; Beilinson levels~2
and~3; \(H_{\mathrm{VD}}\cup
H_{\mathrm C_{1a}}^{\mathrm{partner}}\).]}
...
\end{equation}
For each stable pair \((g,n)\), the \emph{dual cofiber} is
\begin{equation}\label{eq:holo-comp-cofiber}
\mathbf{H}^{\mathrm{hol}}_{g,n}(\cA)
:=
\cofib\!\bigl(\mathbf{Q}_{g,n}(\cA)
\longrightarrow \mathbf{C}_{g,n}(\cA)\bigr).
\end{equation}
\end{definition}
...
\textup{[Type signature: Open quadrant; coderived factorization and
strict-flat centre-local-system presentations; Beilinson levels~2
and~3; \(H_{\mathrm{VD}}\cup H_{\mathrm{conv}}\cup
H_{\mathrm C_{1a}}^{\mathrm{partner}}\).]}
...
\item \emph{Cofiber identification.}\;
 For every stable pair \((g,n)\),
 $\mathbf{H}^{\mathrm{hol}}_{g,n}(\cA)\simeq
 (\chi^-_{\cA;g,n})^{-1}\mathbf{Q}_{g,n}(\cA^!)$.
\end{enumerate}
In particular, the protected transform $\mathbb{H}_X$ identifies
with the Koszul-dual branch and is a contravariant involution in the
stated coderived category.  Its cofiber clause is the transported
\(\mathrm C_1\) partner; a physical bulk factorization algebra is the
output of the separate OCA comparison.
\end{theorem}
...
gives $\mathbf{C}_{g,n}(\cA)\simeq\mathbf{Q}_{g,n}(\cA)\oplus
(\chi^-_{\cA;g,n})^{-1}\mathbf{Q}_{g,n}(\cA^!)$; the canonical
inclusion of the first summand has cofiber the transported second
summand.
\end{proof}
...
Fix an unpointed stable genus $g \geq 2$ together with the
genuine-partner comparison \(\chi^-_{\cA;g,0}\), and write
$V_g(\cA) := \mathbf{Q}_{g,0}(\cA)$,
$W_g(\cA) :=
(\chi^-_{\cA;g,0})^{-1}\mathbf{Q}_{g,0}(\cA^!)[d_{g,0}]$.
By Theorem~\ref{thm:quantum-complementarity-main},
...
\textup{[Type signature: Open quadrant; unpointed strict-flat
centre-local-system presentation; Beilinson level~3; \(g\geq2\),
\(H_{\mathrm C_{1a}}^{\mathrm{partner}}\cup
H_{\mathrm C_{1b}}^{\mathrm{pair}}\), with balanced protected
self-duality for clause~\textup{(iii)}.]}
Let $\cA$ be a modular Koszul chiral algebra. Then:
\begin{enumerate}[label=\textup{(\roman*)}]
\item \emph{Cotangent decomposition.}\;
 $\mathbf{C}_{g,0}(\cA) \simeq V_g(\cA) \oplus V_g(\cA)^\vee[-d_{g,0}]$.
\item \emph{Transposition.}\;
 $\mathbf{H}^{\mathrm{hol}}_{g,0}(\cA) \simeq V_g(\cA)^\vee[-d_{g,0}]$.
\item \emph{Protected self-duality.}\;
 If a protected self-duality $\mathbb{H}_X(\cA)\simeq\cA$ is
 compatible with the represented partner datum, then
 $V_g(\cA) \simeq V_g(\cA)^\vee[-d_{g,0}]$.
\end{enumerate}
\end{theorem}

\begin{proof}
By complementarity, the transported partner satisfies
$(\chi^-_{\cA;g,0})^{-1}\mathbf{Q}_{g,0}(\cA^!)
\simeq W_g(\cA)[-d_{g,0}]
\simeq V_g(\cA)^\vee[-d_{g,0}]$. Substituting into the direct-sum
...
\(H_{\mathrm C_{1a}}^{\mathrm{partner}}\cup
H_{\mathrm C_{1b}}^{\mathrm{pair}}\), with balanced protected
self-duality for clauses~\textup{(ii)--(iii)}.]}
...
\item \emph{Spectral reciprocity.}\;
 $P_g(\cA^!; u) = u^{d_{g,0}}\, P_g(\cA; u^{-1})$,
 where $P_g(\cA^!;u)$ is computed from the genuine complex
 $\mathbf Q_{g,0}(\cA^!)$.
\item \emph{Protected self-dual palindromicity.}\;
 If the balanced protected self-duality of
 Theorem~\textup{\ref{thm:holo-comp-cotangent-realization}(iii)} is
 supplied, then
 $P_g(\cA; u) = u^{d_{g,0}}\, P_g(\cA; u^{-1})$.
\item \emph{Even-genus Euler vanishing.}\;
 Under the same balanced protected self-duality, if $g$ is even, then
 $\chi(V_g(\cA)) = 0$.
\item \emph{Determinant parity.}\;
 \begin{equation}\label{eq:holo-comp-det-parity}
 \Det\!\bigl(\mathbf{C}_{g,0}(\cA)\bigr)
 \cong
 \begin{cases}
 \C, & g \text{ odd}, \\[4pt]
...
\begin{proof}
The transposition of the genuine partner, transported by
$\chi^-_{\cA;g,0}$, gives
$\mathbf Q_{g,0}(\cA^!)\simeq V_g(\cA)^\vee[-d_{g,0}]$ and hence (i) on
cohomology. Protected self-duality forces palindromicity.
Evaluating at $u = -1$ and using the parity of $d_{g,0}=3g-3$
(odd when $g$ is even) gives (iii). For (iv), duality gives
$\Det(V^\vee) \cong \Det(V)^{-1}$, and a shift by $[-d_{g,0}]$
inverts the determinant line iff $d_{g,0}$ is odd. The direct-sum
...
holds. For the Heisenberg pair, the two-object reciprocity law relates
$V_g(\mathcal{H})$ to the genuine partner complex
$\mathbf Q_{g,0}(\mathcal H^!)$ via the shift by $d_{g,0}$;
...
shadow $\mathfrak{Q}_{\cA}$ and, on the $H_D^K$ surface where
\(\operatorname{ch}_g(\mathfrak O_g^K(\cA))=
(-1)^g\kappa(\cA)\lambda_g\), is controlled by the same data.
...
\textup{[Type signature: Open quadrant; curved scalar and strict-flat
centre-local-system presentations; Beilinson levels~2, 3, and~5;
Maurer--Cartan flatness plus the named \(\mathrm C_0/\mathrm C_1\)
hypothesis package for clause~\textup{(iii)}.]}
...
\item the complementarity splitting
 ...
 is the Verdier eigenspace splitting of the strict-flat ambient
 complex under the C0/C1 hypotheses.  The scalar vanishing supplies
 its level-$5$ coefficient.
\end{enumerate}
Termination of the higher shadow tower is the further condition
$\operatorname{Sh}_r(\cA_c^!)=0$ for all sufficiently large~$r$,
verified by an independent higher-operation computation.
\end{theorem}
...
Verdier involution on $\mathbf C_g(\cA)$; the scalar curvature
calculation supplies its level-$5$ projection.
\end{proof}
...
bar--cobar language, an OCA/BRST comparison may identify the ghost
complex with a realization of the Koszul partner~$\cA^!$ and match
ghost number with cohomological degree. Two distinct identities are at
play:
...
\textup{[Type signature: Open-to-physical comparison; curved scalar
and strict-flat centre/BRST presentations; Beilinson levels~2--5;
Virasoro \(\mathrm C_0/\mathrm C_1\) package plus the Polyakov
BRST comparison datum.]}
...
under the Polyakov BRST comparison datum.  Algebraically,
$\mathrm{Vir}_{26-c}$ is the genuine Koszul partner and
Theorem~\textup{\ref{thm:quantum-complementarity-main}} gives
\[
Q_{g,n_g}(\mathrm{Vir}_c)\oplus
(H^*\chi^-_{\mathrm{Vir}_c;g,n_g})^{-1}
Q_{g,n_g}(\mathrm{Vir}_{26-c})
=H^*(\overline{\mathcal M}_g,\mathcal Z(\mathrm{Vir}_c)).
\]
The BRST comparison promotes this algebraic decomposition to the
matter--Liouville--ghost interpretation and the physical equation
$c_{\mathrm{matter}}+c_{\mathrm{Liouville}}+c_{\mathrm{ghost}}=0$.
\end{proposition}
...
Let the Mukai lattice be
\[
  H^*(K3,\mathbb Z) \simeq II_{4,20},
  \qquad c_+\!\left(II_{4,20}\right)=4,
\]
so the exact lattice arithmetic is $2c_+=8$.  Under the comparison
package
$H_{\mathsf B}=(H_{\mathrm{chart}},H_{\mathrm{KD}},H_{\mathrm{scalar}},
H_{\mathrm{mod}},H_{\mathrm{quant}})$, the proposed chiral normalization is
\[
...
\[
  ...
  \qquad
  K^\kappa_{\mathrm{computed}}=\{0,13,250/3\}.
\]
The open ledger adjoins the BP conditional candidate~$25/3$ and the
Mukai-to-chiral candidate~$8$ with their distinct packages.  If the
additional Vol.~III normalization hypotheses
\[
  \hbar^2 K^{\kappa_{\mathrm{ch}}}=-1,
  \qquad
  \ell=K^{\kappa_{\mathrm{ch}}}
\]
are supplied, substitution gives $\hbar^2=-1/8$ and $\ell=8$.
These are scalar normalization comparisons.  The ambient center
complex, derived chiral center, K3 sigma-model, Mukai--Heisenberg
algebra, Borcherds--Kac--Moody algebra, root-of-unity category, and
Humbert monodromy category each require their own construction maps.
\end{proposition}

\begin{proof}
Mukai's K3 lattice convention gives
$H^*(K3,\mathbb Z)\simeq II_{4,20}$, so the positive rank is
$c_+(II_{4,20})=4$ \cite{Mukai1987}.  This proves the lattice value
$2c_+=8$.  The package $H_{\mathsf B}$ promotes that arithmetic to the
proposed chiral conductor.  The equations
$\hbar^2K^{\kappa_{\mathrm{ch}}}=-1$ and
$\ell=K^{\kappa_{\mathrm{ch}}}$ then give $\hbar^2=-1/8$ and
$\ell=8$ by substitution.  Theorem~\ref{thm:quantum-complementarity-main}
acts on a center local system over $\overline{\mathcal M}_g$ together
with a represented Verdier involution; those data belong to the
construction packages named in the statement.
\end{proof}
