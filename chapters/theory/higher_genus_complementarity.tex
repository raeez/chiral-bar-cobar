...
\lambda_g$ \textup{(UNIFORM-WEIGHT; valid for Kac--Moody,
which has a single conformal weight)}.
...
$\mathrm{obs}_g = \kappa \cdot \lambda_g$ is proved
(UNIFORM-WEIGHT; multi-weight algebras at $g \geq 2$ receive
a cross-channel correction), is controlled by the same data.
