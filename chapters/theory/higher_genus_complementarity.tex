...
% chap:universal-conductor). Defended by \providecommand for portability
% across standalone extraction (AP-FM6/AP-FM36).
...
The constant~$K$ is precisely the universal conductor functor of
Chapter~\ref{chap:universal-conductor}, $K(\cA) = -c_{\mathrm{ghost}}(\BRST(\cA))$;
its Verdier-symmetry on the bar/cobar pair is the conductor-side
incarnation of the Koszul reflection
Theorem~\ref{thm:koszul-reflection}, and
Chapter~\ref{chap:climax-platonic} pairs the conductor identity
$\kappa = -c_{\mathrm{ghost}}(\BRST)$ with the Arnold-pullback identity
$d_{\mathrm{bar}} = \mathrm{KZ}^*(\nabla_{\mathrm{Arnold}})$ as the
two equations of the climax theorem.
...
% HEAL 2026-04-17: Clause (iv) formerly carried ProvedHere tag but
% invoked Vol II prop undefined in Vol I. Downgraded to Conditional per
% cross-volume ProvedHere discipline; clauses (i)-(iii) remain ProvedHere.
...
$\overline{\mathcal{M}}_{g,n}$; clauses~\textup{(i)--(iii)}~\ClaimStatusProvedHere;
...
the modular-bootstrap-to-curved-Dunn bridge proved in Vol~II
...
higher-genus foundations theorem~\textup{\ref{thm:modular-bar}}.

\textbf{(iv)} \emph{Sketch only \textup{(}\ClaimStatusConditional\textup{)};
full proof deferred to Vol~II.}  Genus-wise Lagrangians assemble if and
only if the obstruction cocycle in the curved-Dunn twisting-cochain
...
constructed and proved in Vol~II
Proposition~\ref{V2-prop:modular-bootstrap-to-curved-dunn-bridge};
...
\begin{remark}[Scope: respected; PTVV-for-$\mathrm{CY}_d$ distinguished]
...
The distinction is in the Vol~II registry: Verdier-on-$\overline{\mathcal{M}}_{g,n}$
is an intrinsic pairing built from Serre duality + Verdier self-dual
...
\begin{remark}[Why this is the Platonic form of Theorem~C]
...
delivers a $(-1)$-shifted ambient picture. The Platonic form combines
...
\S\ref{sec:ambient-complementarity-lagrangian}. This is the
``PTVV-on-modular-stack payoff'' announced in the Platonic
Reconstitution manifesto.
\end{remark}
...
\begin{remark}[Higher-genus complementarity and Mukai doubling on B-family]
\label{rem:hgc-B-family}
The classical four-archetype $\mathsf{G}/\mathsf{L}/\mathsf{C}/\mathsf{M}$
landmark ceiling $\kappa + \kappa^! \in \{0, 13, 250/3, 98/3\}$ enlarges on the Lorentzian-lattice-parametric $\mathcal{B}$-family to the canonical five-element bucket $\{0, 8, 13, 250/3, 98/3\}$ via the Mukai-doubling conductor $K^{\kappa_{\mathrm{ch}}}(\mathcal{H}_{\mathrm{Muk}}(K3)) = 2\,c_+(\mathrm{Mukai}(K3)) = 8$. Universal identity $\hbar^2 \cdot K^{\kappa_{\mathrm{ch}}} = -1$ pins the Lusztig $u_\zeta$-specialisation at $\ell = 8$, matching Humbert $H_1$ monodromy order via Bruinier $2002$ Proposition~$5.1$ Heegner Chern-class reciprocity. Three faces of $8$: Mukai, Humbert, Lusztig.
\end{remark}

\begin{remark}[Separating-node factorisation $1/\Phi_{10} \to 1/\eta^{24} \cdot 1/\phi_{10,1}$]
\label{rem:hgc-sepnode}
At $K3 \times E$ the genus-$2$ Siegel character $\mathrm{Ch}(\mathbf{H}_{\Delta_5})(Z) = 1/\Phi_{10}(Z)$ on $\mathbb{H}_2$ factorises at a separating-node degeneration $Z \to \mathrm{diag}(\tau, \rho)$ into $1/\eta(\tau)^{24} \cdot 1/\phi_{10,1}(\rho)$, recovering the genus-$1$ $\eta^{24}$ denominator of the K3 chiral algebra times the Jacobi form of the elliptic direction. $\Delta_5^2 = C \cdot \Phi_{10}$ (Gritsenko--Nikulin $1998$) gives $\kappa_{\mathrm{BKM}}(\mathfrak{g}_{\Delta_5}) = c_\Lambda(0)/2 = \mathrm{wt}(\Delta_5) = 5$.
\end{remark}
