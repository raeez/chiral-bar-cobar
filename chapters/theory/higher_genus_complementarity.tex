...
At genus~$g \geq 1$, the bar differential ceases to square to zero.
The curvature $d_{\mathrm{fib}}^2 = \kappa(\cA) \cdot \omega_g$
deforms the cochain complex, and the deformation-obstruction theory
of this curved complex splits into two halves: a half visible
to~$\cA$ and a complementary half visible to its Koszul
dual~$\cA^!$.  The genus-$g$ quantum corrections
$Q_g(\cA)$ and $Q_g(\cA^!)$ sit as complementary summands inside
$R\Gamma(\overline{\mathcal{M}}_g, \mathcal{Z}(\cA))$---what the
algebra sees as anomaly, the dual sees as deformation, and vice
versa.  This is the content of Theorem~C.
