...
Classical Koszul duality pairs a quadratic algebra~$\cA$ with its
dual~$\cA^!$ via bar-cobar inversion: the two algebras are distinct
objects linked by a derived equivalence.  Over a point, this is a
discrete correspondence---$\cA$ and~$\cA^!$ inhabit separate worlds,
connected only by the functor $\Omega \circ B$.  On an algebraic
curve at genus~$g \geq 1$, a qualitatively new phenomenon emerges.
The genus-$g$ obstructions to extending the bar-cobar adjunction
decompose, via Verdier duality, into two complementary halves:
...
\begin{equation}\label{eq:complementarity-chapter-opening}
  ...
  Q_g(\cA) \;\oplus\; Q_g(\cA^!),
\end{equation}
a Lagrangian decomposition with respect to the Verdier pairing
(Theorem~C).  The unconditional part~(C1) holds on the entire Koszul
locus.  The shifted-symplectic upgrade~(C2), which promotes the
summands to Lagrangian subspaces of a $({-}(3g{-}3))$-shifted
symplectic structure, requires perfectness and nondegeneracy of the
ambient complex.  Within each family, the complementarity constraint
is governed by the discriminant $\Delta = 8\kappa S_4$: the same
invariant that controls shadow depth and $L_\infty$ non-formality
also dictates the interplay between $Q_g(\cA)$ and $Q_g(\cA^!)$.
This splitting of genus-$g$ obstructions into dual Lagrangian halves
has no classical analogue; it is the genuinely new duality phenomenon
that arises from Koszul theory on curves.
...
are Lagrangian subspaces of a $({-}(3g{-}3))$-shifted symplectic
structure (Verdier duality of degree $-(3g{-}3)$).  In this upgraded geometry, the finite-order
...
The chain-level mechanism is the Verdier involution
$\sigma$ exchanging the two eigenspaces; at the scalar level
$\kappa(\cA)+\kappa(\cA^!)=0$ for Kac--Moody and free-field pairs,
while $\kappa(\cA)+\kappa(\cA^!)=\rho\cdot K\neq 0$ for
$\mathcal{W}$-algebras (Remark~\ref{rem:w-algebra-complementarity-sum}).
\end{remark}
...
At the critical level, $\kappa^! = (-(-h^\vee) - h^\vee)\dim\mathfrak{g}/(2h^\vee) = 0$, so the dual is uncurved
...
This confirms the complementarity theorem for the Kac--Moody Koszul pair at critical level.
