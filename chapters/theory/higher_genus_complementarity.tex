...
% HEAL 2026-04-17 (HZ-11): Clause (iv) formerly carried ProvedHere tag but
...
\label{sec:hgc-wave13-DNA}

\begin{remark}[Higher-genus complementarity and Mukai doubling on B-family]
\label{rem:hgc-DNA-B-family}
The higher-genus complementarity $\kappa + \kappa^! \in \{0, 13, 250/3, 98/3\}$ enlarges on the Lorentzian-lattice-parametric $\mathcal{B}$-family to $\{0, 8, 13, 250/3, 98/3\}$ via the Mukai-doubling conductor $K^{\kappa_{\mathrm{ch}}}(\mathcal{H}_{\mathrm{Muk}}(K3)) = 2\,c_+(\mathrm{Mukai}(K3)) = 8$. Universal identity $\hbar^2 \cdot K^{\kappa_{\mathrm{ch}}} = -1$ pins the Lusztig $u_\zeta$-specialisation at $\ell = 8$, matching Humbert $H_1$ monodromy order via Bruinier $2002$ Proposition~$5.1$ Heegner Chern-class reciprocity. Three faces of $8$: Mukai, Humbert, Lusztig.
\end{remark}

\begin{remark}[Separating-node factorisation $1/\Phi_{10} \to 1/\eta^{24} \cdot 1/\phi_{10,1}$]
\label{rem:hgc-DNA-sepnode}
