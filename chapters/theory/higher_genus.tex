...
\emph{Step 4: Quantum terms vanish by the Fay trisecant identity.}

The vanishing of the quantum contribution
$\sum_{k,l} t_k\,t_l\, d_k d_l$ follows from a global argument
that avoids checking the component anticommutations
$\{d_k, d_l\} = 0$ directly.

The total corrected differential $\Dg{g}$ is the bar differential
computed using the \emph{holomorphic} propagator
$h_{ij} = \partial_{z_i}\!\log E(z_i, z_j)$ of the prime form
(Definition~\ref{def:higher-genus-log-forms}).  The holomorphic
cyclic sum
\[
  h_{12}\,h_{23} + h_{23}\,h_{31} + h_{31}\,h_{12}
\]
vanishes identically when wedged with the antisymmetric form
factors $(dz_i - dz_j) \wedge (dz_j - dz_k)$, by the
Fay trisecant identity for the prime form---this is exactly
the content proved genus-by-genus in
Theorem~\ref{thm:quantum-arnold-relations}: the holomorphic
part of the Arnold $3$-form is zero at every genus
(genus~$1$: the $\zeta\zeta + \mathrm{cyc} =
\frac{1}{2}[\wp + \cdots]$ identity;
genus~$\geq 2$: the Fay identity for the Siegel
$\wp$-matrix).  The non-holomorphic Arakelov correction
produces $\mathcal{A}_3^{(g)} = 2\pi i\,\omega_{\mathrm{Ar}}
\neq 0$ only because it departs from the holomorphic
propagator.

Since the holomorphic propagator satisfies the exact Arnold
relation, the bar differential $\Dg{g}$ built from it satisfies
$\Dg{g}^{\,2} = 0$ by the same nine-term verification as the
genus-$0$ case (Theorem~\ref{thm:cobar-d-squared-zero}),
with the Fay identity replacing the classical Arnold relation
at each three-point interaction.

\emph{Component-level consequence.}
From $\Dg{g}^{\,2} = 0$ and the independently-proved identities
$\dzero^2 = 0$ (Step~2) and
$\dzero\, d_k + d_k\,\dzero = 0$ (Step~3), we obtain:
\[
  \sum_{k,l} t_k\,t_l\, d_k d_l
  = \Dg{g}^{\,2} - \dzero^2
    - \sum_k t_k(\dzero\,d_k + d_k\,\dzero)
  = 0.
\]
Since the $A$-period parameters $t_k$ are independently
variable as the complex structure of~$\Sigma_g$ varies
over the period domain, we deduce
$d_k d_l + d_l d_k = 0$ for all~$k, l$.
This is the operator-level manifestation of the Lagrangian
isotropicity $\gamma_k \cdot \gamma_l = 0$ of the $A$-cycle
subspace in $H_1(\Sigma_g, \mathbb{Z})$: the Riemann bilinear
relations connect iterated period integrals of the Cauchy
kernel to the intersection pairing, and restricting to a
Lagrangian subspace forces all anticommutators to vanish.

\emph{(Note: The vanishing of $\Dg{g}^{\,2}$ relies on the
holomorphic Arnold relation (Fay identity) for the prime form,
not on the cohomology of the moduli space $\mathcal{M}_g$.
Recall $H^1(\mathcal{M}_g, \mathbb{Q}) = 0$ for $g \geq 2$ by
Harer's theorem, but this is not the relevant vanishing here.
The corrected differential $\Dg{g}$ is the flat gauge of the
curved connection $\dfib$---the chiral Riemann--Hilbert
correspondence of
Remark~\textup{\ref{rem:curvature-riemann-hilbert}}.)} \qedhere
\end{proof}
