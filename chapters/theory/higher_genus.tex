...
For the Heisenberg algebra with $\operatorname{obs}_g = \kappa \cdot \lambda_g$, we have $(\operatorname{obs}_g)^2 = \kappa^2 \cdot \lambda_g^2$. For $g \leq 2$, Theorem~\ref{thm:obstruction-nilpotent} gives $\lambda_g^2 = 0$ by dimensional vanishing: $\lambda_g^2 \in H^{4g}(\overline{\mathcal{M}}_g)$, which vanishes when $4g > \dim_{\mathbb{R}} \overline{\mathcal{M}}_g = 6g-6$, i.e., when $g < 3$. For $g \geq 3$ the dimensional argument fails; nilpotence in that range is Conjecture~\ref{conj:obstruction-nilpotent-higher}.
\end{example}
