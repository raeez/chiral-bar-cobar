...
What remains open in the $\mathcal{W}$ direction is no longer
higher-genus PBW concentration for finite-type principal families,
but rather the completed/infinite-generator regime
$\mathcal{W}_\infty$
...
$\dim M_h = p(h-2)$ and
$L_0 = h \cdot \mathrm{id}_{M_h}$, together with the
weight-shift identities for $L_{-1},L_0,L_1$.
\end{proof}
...
\item
Theorems~\ref{thm:pbw-allgenera-km}
and~\ref{thm:pbw-allgenera-virasoro}, together with
Theorem~\ref{thm:pbw-allgenera-principal-w},
resolve the higher-genus PBW problem completely for the standard
finite-type interacting families: Kac--Moody, Virasoro, and principal
finite-type $\mathcal{W}$-algebras.  The mechanisms form a strict
hierarchy:
Kac--Moody needs Whitehead plus Killing contraction;
Virasoro needs only the $L_0$ scalar action;
principal finite-type $\mathcal{W}$-algebras add higher-spin terms,
but those terms are strictly weight-raising and therefore do not alter
the invertible $L_0$ diagonal.
...
Kac--Moody, Virasoro, and principal finite-type $\mathcal{W}$.
It may fail for algebras whose OPE structure depends on global curve
data (e.g., elliptic vertex algebras), but for any vertex algebra
defined by local OPE relations, the same principle applies.

\item
Theorems~\ref{thm:pbw-allgenera-km}
and~\ref{thm:pbw-allgenera-virasoro}, together with
Corollaries~\ref{cor:unconditional-allgenera-km}
and~\ref{cor:unconditional-allgenera-virasoro},
and the principal $\mathcal{W}$ upgrades
Theorem~\ref{thm:pbw-allgenera-principal-w}
and Corollary~\ref{cor:unconditional-allgenera-principal-w},
upgrade the interacting conditional claims in Part~II to
unconditional statements at all genera, including complementarity,
genus expansions, and Kodaira--Spencer constructions for higher
$\mathcal{W}_N$.
\end{enumerate}
\end{remark}
