...
For any Koszul chiral algebra $\mathcal{A}$ and \emph{any} genus $g \geq 0$,
...
$\mathcal{H}^1_{\mathrm{dR}} = R^1\pi_*\Omega^{\bullet}_{\mathcal{C}/\overline{\mathcal{M}}_g}(\log D)$
is flat, so
...
Let $\mathcal{A}$ be a Koszul chiral algebra.  The obstruction classes
...
\item Obstruction squares to zero: $(\text{obs}_g)^2 = 0$ for all $g$ (Theorem~\ref{thm:obstruction-nilpotent-all-genera}, via Mumford's relation $c(\mathbb{E})c(\mathbb{E}^\vee) = 1$)
