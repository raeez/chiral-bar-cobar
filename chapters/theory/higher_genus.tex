...
The genus-$g$ bar complex is the stable-graph sum
...
m_3(a \otimes b \otimes c) &= \Res_{D_{12} \cap D_{23}}
  \bigl[\eta_{12} \wedge \eta_{23} \cdot a \otimes b \otimes c\bigr]
  \quad \text{(three trees, one sign each by~\eqref{eq:koszul-tree-sign})} \\
...
For algebras with generators of uniform conformal weight---that is, with
scalar curvature---the factorization is immediate and $\kappa$ is a single
scalar.  (This includes single-generator algebras such as
Heisenberg and Virasoro, and also Kac--Moody algebras where all
...
which identifies it with the eigenspace defined in
...
In summary: the identification of the $+1$-eigenspace with
$Q_g(\mathcal{A})$ and the $-1$-eigenspace with
$Q_g(\mathcal{A}^!)$ follows from the Verdier intertwining
$\mathbb{D}_{\mathrm{Ran}}\, \bar{B}(\mathcal{A}) \simeq
\bar{B}(\mathcal{A}^!)$ (Theorem~\ref{thm:verdier-bar-cobar}),
which at the level of centers gives
$\sigma(\mathcal{Z}(\mathcal{A})) = \mathcal{Z}(\mathcal{A}^!)$,
so that $\sigma$ acts as $+1$ on the $\mathcal{A}$-contribution
(via $j_*$-extension) and as $-1$ on the
$\mathcal{A}^!$-contribution (via $j_!$-extension).
...
Since $U_g$ is a smooth Deligne--Mumford stack, a constructible complex
with universally acyclic stalks is globally acyclic
(proper base change for the structure map $U_g \to \operatorname{Spec} k$,
or equivalently the constancy of fiber cohomology over the smooth locus;
see~\cite[Expos\'e~XVI]{SGA4}).  Hence $C$ is acyclic and $j_g^*\psi_g$
is a quasi-isomorphism on~$U_g$.
\end{proof}
