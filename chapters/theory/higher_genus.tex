...
\bigskip
\noindent
This chapter contains the deepest results of the monograph.
At genus zero, bar-cobar duality is a formal consequence of operadic Koszul theory---the bar and cobar constructions are adjoint functors, and for Koszul algebras the adjunction is an equivalence.
The passage to higher genus reveals something unexpected: the genus tower is not a perturbation of the genus-zero theory but a single Maurer--Cartan deformation, controlled by a universal obstruction class~$\Theta_A$ whose scalar shadow is the genus-universality formula $\mathrm{obs}_g(A) = \kappa(A) \cdot \lambda_g$.
The three main theorems---bar-cobar inversion (Theorem~B, Theorem~\ref{thm:higher-genus-inversion}), deformation-obstruction complementarity (Theorem~C, Theorem~\ref{thm:quantum-complementarity-main}), and the Kodaira--Spencer construction (Theorem~\ref{thm:kodaira-spencer-chiral-complete})---are all proved here.
The central thesis of the monograph---that quantum corrections are the modular completion of Koszul duality---finds its mathematical expression in this chapter.
\bigskip
...
This is established by Theorem~\ref{thm:geometric-equals-operadic-bar} (identifying
geometric and operadic bar constructions), Theorem~\ref{thm:arnold-three}
($d^2 = 0$ via the Arnold relations), and Theorem~\ref{thm:bar-nilpotency-complete}
(acyclicity outside degree~$0$, establishing the Koszul property).
...
$\Omega^{\text{ch}}(\mathcal{A}_2^!) \simeq \mathcal{A}_1$ is proved by
Theorem~\ref{thm:verdier-bar-cobar} (Verdier duality exchanges bar and cobar) and
Theorem~\ref{thm:cobar-d-squared-zero} (cobar nilpotency, giving $d_{\text{cobar}}^2 = 0$).
...
\begin{theorem}[Faber--Pandharipande formula; \ClaimStatusProvedElsewhere]
...
via the Quillen determinant anomaly formula.  The result is:
\begin{equation}
...
the residue extracts the mode $W_0(T) = 0$ (since $T$ is primary of weight~$2$
for the weight-$3$ field $W$), and symmetrically.
...
\end{align}
By the Mumford isomorphism, the first Chern class of the bundle of $h$-differentials satisfies $c_1(\pi_* \omega^{\otimes h}) = (6h^2 - 6h + 1)\lambda_1$ on $\overline{\mathcal{M}}_{1,1}$.  Thus $\lambda_1^{(2)} = 13\lambda_1 = \frac{13}{24}$ and $\lambda_1^{(3)} = 37\lambda_1 = \frac{37}{24}$, giving:
\begin{align}
...
\label{conj:obstruction-nilpotent-higher}% backward-compatible label
...
\lambda_g^{(h_i)}$
where $\lambda_g^{(h)} = c_g(R^0\pi_*\omega_X^{\otimes h})$
(Remark~\ref{rem:multi-generator-obs}).
Mumford's relation $c(\mathbb{E})\,c(\mathbb{E}^\vee) = 1$ applies to
the Hodge bundle $\mathbb{E} = R^0\pi_*\omega_X$ (weight $h = 1$)
via flatness of the Gauss--Manin connection on $H^1_{\mathrm{dR}}$.
For $h \geq 2$, the bundle $E_h = R^0\pi_*\omega_X^{\otimes h}$ has
rank $(2h{-}1)(g{-}1)$ and $H^1(C, \omega^{\otimes h}) = 0$
...
since the products $\lambda_g^{(h)} \cdot \lambda_g^{(h')}$ for
$h, h' \geq 2$ are not a priori zero in
$\mathrm{CH}^{2g}(\overline{\mathcal{M}}_g)$.
\end{remark}
...
reduction: $\kappa(\mathcal{A}^!) = \sigma(\mathfrak{g})\, K - \kappa(\mathcal{A})$, where
$K = c(\mathcal{A}) + c(\mathcal{A}^!)$ is the complementarity constant
(Remark~\ref{rem:vir-vs-km-complementarity}) and
$\sigma(\mathfrak{g}) = \sum_{i=1}^r \frac{1}{m_i + 1}$ is the exponent sum
($m_1, \ldots, m_r$ the exponents of~$\mathfrak{g}$; for $\mathcal{W}_N = \mathcal{W}(\mathfrak{sl}_N)$,
$\sigma = H_N - 1 = \sum_{s=2}^{N} 1/s$).
In particular, $\kappa + \kappa' = 0$ for Kac--Moody,
$\kappa + \kappa' = 13$ for the Virasoro algebra ($\sigma = 1/2$, $K = 26$),
and $\kappa + \kappa' = 250/3$ for $\mathcal{W}_3$ ($\sigma = 5/6$, $K = 100$).
...
$\kappa(\mathcal{A}^!) = \sigma(\mathfrak{g})\, K \neq 0$.
...
$\mathrm{obs}_g = \sum_h \kappa_h \cdot \lambda_g^{(h)}$
where $\lambda_g^{(h)} = c_g(R^0\pi_* \omega_X^h)$.
At genus~$1$, all Hodge-type classes are proportional to $\lambda_1$
(since $H^2(\overline{\mathcal{M}}_{1,1}) \cong \mathbb{C}$), and
$\kappa = \sum_h \kappa_h$ is the total obstruction coefficient.
At genus $g \geq 2$ the individual $\lambda_g^{(h)}$ may differ;
...
$\lambda_g^{(h)}$ against $\psi$-classes.
...
For $\mathcal{W}$-algebras, $\kappa = c \cdot \sigma(\mathfrak{g})$ where
$\sigma(\mathfrak{g}) = \sum 1/(m_i+1)$ is the exponent sum
(Theorem~\ref{thm:wn-obstruction}).  Since $c \mapsto c' = K - c$
under the Feigin--Frenkel involution,
$\kappa' = c' \cdot \sigma = (K-c)\sigma = \sigma K - \kappa$
...
\frac{\kappa(\mathcal{W}^k(\mathfrak{g}))}{c(\mathcal{W}^k(\mathfrak{g}))} = \sigma(\mathfrak{g}) := \sum_{i=1}^r \frac{1}{m_i + 1}
\end{equation}
where $m_1, \ldots, m_r$ are the exponents of $\mathfrak{g}$.  The ratio $\sigma(\mathfrak{g})$ is a root-system invariant independent of the level~$k$.
...
\begin{proof}
From Theorem~\ref{thm:genus-universality}(ii): $\kappa = c \cdot \sigma(\mathfrak{g})$.  Since $\sigma$ depends only on the exponents of $\mathfrak{g}$, it is independent of $k$.
\end{proof}
...
\begin{proof}
By Theorem~\ref{thm:genus-universality}(ii), $\kappa(\mathcal{W}_N^k) = c \cdot \sigma(\mathfrak{sl}_N)$ where $\sigma(\mathfrak{sl}_N) = \sum_{s=2}^N 1/s = H_N - 1$.  Under the Feigin--Frenkel involution $k \mapsto k' = -k - 2N$, $c \mapsto c' = K_N - c$, so $\kappa' = (K_N - c)(H_N - 1)$ and $\kappa + \kappa' = K_N(H_N - 1)$.
\end{proof}
...
\]
in the tautological ring of the moduli space.  The assignment $\mathcal{A} \mapsto \kappa(\mathcal{A})$ is additive under tensor products: $\kappa(\mathcal{A} \otimes \mathcal{B}) = \kappa(\mathcal{A}) + \kappa(\mathcal{B})$ \textup{(}Corollary~\ref{cor:kappa-additivity}\textup{)}.  For Kac--Moody algebras, $\kappa$ further satisfies $\kappa(\mathcal{A}^!) = -\kappa(\mathcal{A})$, making it a group homomorphism from $K_0(\mathrm{KCA}_{\mathrm{KM}})$ to~$\mathbb{Q}$.  For $\mathcal{W}$-algebras the duality relation is affine \textup{(}$\kappa' = \sigma(\mathfrak{g})\,K - \kappa$, where $\sigma(\mathfrak{g}) = \sum 1/(m_i+1)$\textup{)}, so $\kappa$ is additive but does not intertwine Koszul duality with negation.
\end{corollary}
...
$j_*$-extensions.  On the $E_2$ page, elements of $Q_g(\mathcal{A}^!)$
lie on the $q = d$ row (since $\mathcal{A}^!$ contributes the
complementary fiber cohomology degree via Verdier duality).  The
Poincar\'e duality sign at $q = d$ is $(-1)^d$; composed with the
Koszul identification $(\mathcal{A}^!)^! \simeq \mathcal{A}$, which
maps $E_2^{p,d}(\mathcal{A}^!)$ back to $E_2^{p,0}(\mathcal{A})$ with
an additional sign $(-1)^d$, the total eigenvalue is
$(-1)^d \cdot (-1)^d = +1$ on the $q = d$ row \emph{before} Koszul
identification, but the composition $\sigma$ maps $q = d$ to $q = 0$
with the identification picking up a net sign $-1$ from the
anti-commutativity of~$\sigma$ with the Kodaira--Spencer action
(\eqref{eq:verdier-ks-anticommute}).
...
\begin{corollary}[Physical interpretation; \ClaimStatusHeuristic]
...
\begin{corollary}[String theory interpretation; \ClaimStatusHeuristic]
...
see also clause~(D2) of Theorem~\ref{thm:bar-cobar-inversion-qi}.
...
for~(c), at generic level the Casimir $\kappa(\widehat{\fg}_k)$ is nonzero
and the bar spectral sequence at genus~$g$ collapses by the universality
theorem (Theorem~\ref{thm:genus-universality});
for~(d)--(e), the same universality mechanism applies.
