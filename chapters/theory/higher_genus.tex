...
Anomalies appear algebraically as curved $A_\infty$ structures.
...
\begin{theorem}[Cobar resolution {\cite{LV}}; \ClaimStatusProvedElsewhere]
...
At genus $g \geq 3$, modular forms of weight $\leq g(g+1)/2$ appear, with relations from gluing equations along boundary strata of $\overline{\mathcal{M}}_g$.  As observed by Witten, all structures are determined by genus-$0$ data and modular invariance.
\end{remark}
...
The Heisenberg computation (Chapter~\ref{ch:heisenberg-frame}) exhibited genus-$0$ flatness ($\dzero^2 = 0$), genus-$1$ fiberwise curvature ($\dfib^{\,2} = k \cdot \omega_1$), and the holomorphic anomaly via $E_2(\tau)$.  These phenomena are universal.
...
the birth of the modular theory.  The central extension
$\kappa(\cA)$ that appears here is the first characteristic number
of the universal Maurer--Cartan class $\Theta_{\cA}$, and it
controls all higher genera through the universality theorem.

\begin{remark}[Physical origin of genus 1]
\label{princ:genus-1-physical}
The genus expansion is the loop expansion:
$Z = Z_{\text{tree}} + \hbar Z_{\text{1-loop}} + \hbar^2 Z_{\text{2-loop}} + \cdots$.
Genus~$0$ gives tree-level amplitudes, genus~$1$ gives one-loop corrections (where central charges arise), and $g \geq 2$ gives multi-loop corrections.
\end{remark}
...
The bar construction on a single curve produces algebraic data ---
a coalgebra, a differential, a spectral sequence.  When the curve
varies over $\overline{\mathcal{M}}_g$, this algebraic data
assembles into cohomological data on the moduli space.  The
obstruction to extending a chiral algebra across degenerate curves
lands in $H^*(\overline{\mathcal{M}}_g, \mathcal{Z}(\cA))$,
the cohomology with coefficients in the center local system.
Verdier duality endows this space with a shifted-symplectic
pairing, and the deformation and obstruction contributions of a
Koszul pair $(\cA, \cA^!)$ embed as complementary Lagrangians.
This is the content of Theorem~C
...
\item \emph{Non-collapsing spectral sequences}.  For a non-Koszul
  ...
  algebras in the Master Table are Koszul with $E_2$-collapse.

\item \emph{Module insertions}.  The bar complex with module
  insertions $\bar{B}^{(g)}_n(\mathcal{A}; M_1, \ldots, M_n)$
...
\emph{Step~3: Diagonal Ext vanishing.}
Modular Koszulity (axiom~MK3) provides diagonal Ext vanishing
at each genus: $\operatorname{Ext}^{i,j}_{\cA^{(g)}} = 0$ for
$i \neq j$.  This ensures that only total degree~$0$ survives
in the full fiber complex (not just at the associated graded
level), by spectral sequence comparison.

\emph{Step~4: Base change.}
Apply the Leray spectral sequence to the family
$\pi_g\colon \mathcal{C}_g(\cA) \to \overline{\mathcal{M}}_g$.
The fiber-level concentration from Steps~2--3 gives
$R^q\pi_{g*} = 0$ for $q \neq 0$ (cohomology and base change).
The perfectness hypothesis ensures base change applies
uniformly over $\overline{\mathcal{M}}_g$.
The surviving $R^0\pi_{g*}$ is identified with $\mathcal{Z}_{\cA}$
by the bar-center identification at each fiber
(Theorem~\ref{thm:bar-concentration} applied fiberwise).
\end{proof}
...
For a Koszul chiral algebra $\mathcal{A}$ (Definition~\ref{def:chiral-koszul-pair}),
the fiber cohomology sheaf satisfies:
\begin{equation}
...
For the Heisenberg algebra, Verdier duality exchanged the Lie-type bar coalgebra $\mathrm{coLie}^{\mathrm{ch}}(V^*)$ with the Com-type cobar algebra $\mathrm{Sym}^{\mathrm{ch}}(V^*)$ (\S\ref{sec:frame-koszul-dual}).  This section establishes the compatibility of Verdier duality with the Ayala--Francis topological framework at all genera.

\subsection{Three levels of duality}
\label{subsec:three-dualities}

We establish compatibility between Verdier duality (geometric) and Ayala--Francis duality (topological).
...
and are therefore modular Koszul
\textup{(}by Theorems~\textup{\ref{thm:higher-genus-inversion}}
...
For generic affine Kac--Moody algebras $\widehat{\mathfrak{g}}_k$,
generic Virasoro $\mathrm{Vir}_c$, and generic $\mathcal{W}_N$-algebras,
the genus-$g$ PBW spectral sequence degenerates at~$E_2$ for all
$g \geq 1$.
\end{conjecture}

\begin{remark}[Status]\label{rem:mk4-status}
...
standard interacting families.  It holds at genus~$0$ by
Theorem~\ref{thm:km-chiral-koszul} and at the associated graded
level at all genera by the argument above.  The difficulty is that
degeneration of a spectral sequence is not automatic from
degeneration of its associated graded.

\emph{Homotopy template} (Convention~\ref{conv:hms-levels}): Type~I (existence of homotopy object).
The conjecture asserts the existence of a degenerate PBW spectral sequence at all genera, which would provide the missing input for unconditional modular Koszulity.
\end{remark}
...
encoding bar cohomology dimensions
(Theorem~\ref{thm:ds-bar-gf-discriminant}), is a separate
non-scalar invariant: it depends on the full quadratic OPE data,
not on $\kappa(\cA)$ alone.
\end{definition}
...
\item $\Theta_{\cA} \in
  ...
  \bigr)$
  is the \emph{universal Maurer--Cartan class}
...
\smallskip\noindent
The Heisenberg algebra was the atom from which all four main theorems were first visible (\S\ref{sec:frame-modular-package}): a single number $\kappa(\mathcal{H}_k) = k$ controlled the entire genus tower.  This theorem --- Theorem~D --- establishes that universality for all modular Koszul chiral algebras.
