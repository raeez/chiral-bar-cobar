...
The three main theorems of the monograph --- bar-cobar inversion
...
\begin{remark}[Towards a family index theorem]\label{rem:towards-family-index}
\index{index theorem!modular deformation}
Proposition~\ref{prop:grr-bridge} reduces the family index
conjecture (Conjecture~\ref{conj:family-index}) to the following:
construct a perfect complex $\mathcal{D}_{\cA}$ on
$\overline{\mathcal{M}}_g$---the modular deformation complex
of~$\cA$---such that
\begin{equation}\label{eq:family-index-target}
\operatorname{ch}(R\pi_* \mathcal{D}_{\cA}) \cdot
\operatorname{Td}(T^{\mathrm{vir}}_\pi)
\;=\;
\kappa(\cA) \cdot c_g(\mathbb{E}) \cdot \psi^{2g-2}
\quad \text{in } H^*(\overline{\mathcal{M}}_{g,1}).
\end{equation}
The natural candidate is $\mathcal{D}_{\cA} =
\kappa(\cA) \cdot \mathbb{E}$, a twist of the Hodge bundle by
the obstruction coefficient.  Mumford's Grothendieck--Riemann--Roch
calculation~\cite{Mumford83} gives
$\operatorname{ch}_k(\mathbb{E}) =
B_{2k}/(2k \cdot (2k-1)!) \cdot \kappa_{2k-1}/2$ on
$\overline{\mathcal{M}}_g$, which controls the Chern character.
The Faber--Pandharipande integral then extracts the scalar
$\lambda_g^{\mathrm{FP}}$ from this Chern character data.
The precise identification of $\mathcal{D}_{\cA}$ with a functorial
construction from $\cA$ (rather than the ad hoc twist
$\kappa(\cA) \cdot \mathbb{E}$) remains the content of the
family index conjecture.
\end{remark}
...
(proved directly in Theorem~\ref{thm:virasoro-self-duality}; alternatively, $c = 0$ corresponds to the fixed point of the Feigin--Frenkel involution $c \mapsto 26 - c$) with center $Z(\mathrm{Vir}_0)
...
\index{modular characteristic package}
A modular Koszul chiral algebra $\cA$ carries a \emph{modular
characteristic package}: the collection of invariants
$(\kappa(\cA),\, \{F_g(\cA)\}_{g \geq 1},\, \Delta_{\cA}(x))$
consisting of the obstruction coefficient, the genus free energies,
and the bar cohomology discriminant.  These are not independent:
\begin{enumerate}[label=(\roman*)]
\item The genus free energies are determined by $\kappa(\cA)$ via the
  universal generating function
  (Theorem~\ref{thm:universal-generating-function}):
  $\sum_{g \geq 1} F_g(\cA)\, x^{2g} = \kappa(\cA) \cdot ((x/2)/\sin(x/2) - 1)$.
\item The obstruction coefficient is additive under tensor products
  (Corollary~\ref{cor:kappa-additivity}) and anti-symmetric under
  Koszul duality: $\kappa(\cA) + \kappa(\cA^!) = 0$
  (Theorem~\ref{thm:genus-universality}\textup{(ii)}).
\item The discriminant $\Delta_{\cA}(x)$ is invariant under
  Drinfeld--Sokolov reduction
  (Theorem~\ref{thm:ds-bar-gf-discriminant}).
\end{enumerate}
The scalar $\kappa(\cA)$ is the first characteristic number of a
richer universal class $\Theta_{\cA}$
(\S\ref{sec:modular-koszul-programme}); the full non-scalar
structure remains conjectural.
\end{remark}
...
The three main theorems are proved.  Theorem~A
...
These are three aspects of a single object: the modular Koszul chiral
