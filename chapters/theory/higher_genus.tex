...
Curvature $d^2 = \kappa \cdot \omega_1$ & Conformal anomaly \\
...
the higher-genus programme.  In particular, the condition
$\kappa = 0$ is simultaneously the mathematical condition for
the bar complex to be uncurved and the physical condition for
...
For historical background on $A_\infty$ algebras---including Stasheff's original discovery via loop spaces, physical origins in string field theory and deformation quantization, and the operadic unification---see Appendix~\ref{sec:ainfty-historical}.
...
The operation~$m_k$ corresponds to the contribution of Feynman diagrams
whose topology is determined by the boundary stratification
of~$\overline{M}_{0,k+1}$: $m_2$ is tree-level propagation, $m_3$ is the
one-loop correction, and higher~$m_k$ encode higher-loop quantum effects.
The precise correspondence between $A_\infty$ operations and Feynman
diagram topologies is developed in Chapter~\ref{ch:feynman}.
\end{remark}
...
For the Koszul pair $(\mathcal{H}_k, \mathrm{Sym}^{\mathrm{ch}}(V^*))$, we verified this concretely in~\S\ref{sec:frame-inversion}: the bar of one is the dual coalgebra of the other, and the cobar reconstructs the partner.  The following theorem is the general statement --- Theorem~A of this monograph.
...
This is established by combining three results:
Theorem~\ref{thm:geometric-equals-operadic-bar} identifies the geometric bar complex
with the operadic bar construction $B(\mathcal{P}, \mathcal{A}_1)$, so that the
coalgebra structure extracted by residues coincides with the standard operadic
coproduct.  Theorem~\ref{thm:arnold-three} shows $d_{\text{bar}}^2 = 0$ via the
Arnold relations on $H^*(\overline{C}_n)$, so the bar complex is indeed a chain
complex of chiral coalgebras.  Finally, Theorem~\ref{thm:bar-nilpotency-complete}
establishes that the cohomology of the augmented bar complex is concentrated in
degree~$0$---this is the \emph{Koszul property}, and it identifies $H^0$ with the
Koszul dual coalgebra $\mathcal{A}_2^!$, yielding~\eqref{eq:bar-A1-is-A2-dual}.
...
For the Koszul dual coalgebra $\mathcal{A}_2^!$, the cobar reconstruction
$\Omega^{\text{ch}}(\mathcal{A}_2^!) \simeq \mathcal{A}_1$ is proved as follows.
Theorem~\ref{thm:verdier-bar-cobar} (Verdier duality exchanges bar and cobar) provides
a natural quasi-isomorphism
$\Omega^{\text{ch}}(\bar{B}^{\text{ch}}(\mathcal{A}_1)) \simeq \mathcal{A}_1$,
which is the unit of the bar-cobar adjunction.
Theorem~\ref{thm:cobar-d-squared-zero} ensures $d_{\text{cobar}}^2 = 0$, so the
cobar complex is well-defined.  Substituting the identification
$\bar{B}^{\text{ch}}(\mathcal{A}_1) \simeq \mathcal{A}_2^!$ from Step~1 into the
Verdier quasi-isomorphism then gives
$\Omega^{\text{ch}}(\mathcal{A}_2^!) \simeq \mathcal{A}_1$.
...
\Omega^{\text{ch}}(\bar{B}^{\text{ch}}(\mathcal{A}_1))
   ...
   &\simeq \mathcal{A}_1 \quad \text{(by Step 2)}
\end{align*}

This is the bar-cobar inversion: starting from $\mathcal{A}_1$, applying bar-then-cobar
recovers $\mathcal{A}_1$ up to quasi-isomorphism.
...
are quasi-isomorphisms. Concretely, the unit
$\eta \colon \mathcal{A}_1 \to \Omega^{\text{ch}}(\bar{B}^{\text{ch}}(\mathcal{A}_1))$
is a quasi-isomorphism because the Koszul property (diagonal concentration of Ext,
Theorem~\ref{thm:bar-nilpotency-complete}) forces the filtration spectral sequence
computing $H^*(\Omega^{\text{ch}}(\bar{B}^{\text{ch}}(\mathcal{A}_1)))$ to degenerate
at $E_2$ with $E_2 \cong \mathcal{A}_1$.
The counit $\varepsilon \colon \bar{B}^{\text{ch}}(\Omega^{\text{ch}}(\mathcal{C}))
\to \mathcal{C}$ is a quasi-isomorphism by the dual argument applied to the Koszul
dual coalgebra.  Together, bar and cobar are quasi-inverse functors when restricted
...
\item Arnold relations ensure $d^2 = 0$ (Theorem~\ref{thm:arnold-three})
\item Stokes' theorem provides quasi-isomorphism (Theorem~\ref{thm:stokes-config})
\end{itemize}
\end{proof}
...
For a chiral Koszul pair $(\mathcal{A}_1, \mathcal{A}_2)$ on a smooth projective curve~$X$,
their chiral Hochschild cohomologies satisfy Poincar\'e duality:
$$CH^n(\mathcal{A}_1) \simeq CH^{n}(\mathcal{A}_2)^{\vee}
   \otimes \omega_X[2]$$
where $\omega_X$ is the canonical bundle and the $[2]$-shift arises from the
real dimension of~$X$ (complex dimension~$1$).
\end{corollary}

\begin{proof}
The chiral Hochschild complex at bar degree~$n$ is
$CH^n(\mathcal{A}) = \Gamma(\overline{C}_n(X),\, \mathcal{A}^{\boxtimes n})$.
Poincar\'e--Verdier duality on the smooth $2n$-dimensional (real) manifold
$\overline{C}_n(X)$ gives:
\[
H^i(\overline{C}_n(X),\, \mathcal{F})
\;\simeq\; H^{2n-i}(\overline{C}_n(X),\,
   \mathcal{F}^{\vee} \otimes \omega_{\overline{C}_n})^{\vee}.
\]
For a Koszul pair $(\mathcal{A}_1, \mathcal{A}_2)$, Theorem~\ref{thm:bar-cobar-isomorphism-main}
gives $\bar{B}^{\mathrm{ch}}(\mathcal{A}_1) \simeq \mathcal{A}_2^!$ as chiral coalgebras.
Applying Verdier duality (Theorem~\ref{thm:verdier-bar-cobar}) to the bar complex
exchanges $\mathcal{A}_1^{\boxtimes n}$ for $(\mathcal{A}_2^!)^{\boxtimes n}$ with
a $[2]$-shift from each copy of~$\omega_X$.  Summing over bar degree and
identifying $\mathcal{A}_2^! = \mathcal{A}_2$ (using the Koszul involution)
yields the stated duality with the $\omega_X[2]$ twist.
\end{proof}
...
On a genus-$0$ curve, the bar differential satisfies $d^2 = 0$
and configuration spaces are rational.  This is the domain of
classical Koszul duality.  On a curve of genus $g \geq 1$, the
topology changes: $H^1(\Sigma_g) \neq 0$, the propagator depends
on the modular parameter, and the bar differential acquires
curvature.  The passage from genus~$0$ to genus~$g$ is not a
perturbation but a deformation: the full genus tower is controlled
by a single Maurer--Cartan element in the cyclic deformation complex,
and the curvature at each genus is a section of the obstruction
sheaf on $\overline{\mathcal{M}}_g$.  This is the transition from
operadic to modular Koszul duality.
...
At genus~$0$, the Arnold relation $\mathcal{A}_3^{(0)} = 0$ was the mechanism ensuring $d^2 = 0$ in the Heisenberg bar complex (\S\ref{sec:frame-bar-deg2}).  At genus~$1$, the Arnold $3$-form acquires a correction proportional to the volume form; this is the geometric origin of the curvature $d^2 = \kappa \cdot \omega_1$ that we computed in~\S\ref{sec:frame-genus1}.
...
The genus-$2$ computation above reveals the universal pattern that persists at all genera: the holomorphic cyclic sum of propagators vanishes by the Fay trisecant identity (Step~1), the non-holomorphic correction produces the Arakelov $(1,1)$-form (Step~2), and the cross-terms between holomorphic and non-holomorphic pieces cancel by type.  The only genus-dependent input is the period matrix $\Omega \in \mathfrak{H}_g$ and the normalized abelian differentials $\omega_1, \ldots, \omega_g$.  We now verify that this pattern extends to all genera.
\end{remark}
...
The proof of part~(d) uses exactly the same ingredients as
parts~(b) and~(c): the Fay trisecant identity (which holds
at all genera), even parity of the Siegel $\wp$-matrix, and
the universal formula~\eqref{eq:dbar-a-general} for the
$\bar\partial$-derivative of the non-holomorphic correction.
The only genus-dependent feature is the range $1 \leq \alpha, \beta \leq g$
...
At genus~$0$, the Heisenberg bar complex was a genuine cochain complex with $d^2 = 0$ (\S\ref{sec:frame-bar-all}).  At genus~$1$, the curvature $d^2 = k \cdot \omega_1$ appeared (\S\ref{sec:frame-genus1}), and the Eisenstein series $E_2(\tau)$ entered through the holomorphic anomaly (\S\ref{sec:frame-holomorphic-anomaly}).  We now develop these quantum corrections systematically for arbitrary chiral algebras.
...
\begin{remark}[Notation for differentials]
\label{rem:differential-notation}
\index{bar differential!notation}
The superscript~$(k)$ in the decomposition
$d_{\bar{B}} = d^{(0)} + d^{(1)} + \cdots$ of
...
$\bar{B}^{(g)}(\mathcal{A})$ for a single fixed genus~$g$:
\begin{itemize}
\item $d^{(0)}$: fiberwise collision residues (Leray degree~$0$).
...
\end{itemize}
By contrast, $d^{(g)}_{\mathrm{total}}$ (or simply $d^{(g)}$
when no confusion arises) denotes the \emph{total}
bar differential on $\bar{B}^{(g)}(\mathcal{A})$---i.e.\
$d^{(g)}_{\mathrm{total}} = d^{(0)} + d^{(1)} + \cdots$---which
satisfies $(d^{(g)}_{\mathrm{total}})^2 = 0$
(Theorem~\ref{thm:genus1-d-squared} at genus~$1$; the
higher-genus analog follows from the Leray spectral sequence).

Finally, when computing on a fixed curve $\Sigma_g$, the
\emph{fiberwise collision differential}
$d_{\mathrm{fib}} := \sum_{i<j} \operatorname{Res}_{D_{ij}}$
uses the full genus-$g$ propagator and may \emph{not} square to
zero: $(d_{\mathrm{fib}})^2 = \kappa \cdot \omega_g$, with the
curvature valued in the center
(cf.\ Example~\ref{ex:heisenberg-complementarity-explicit}).
This curvature contributes to the Leray spectral sequence
differentials but does not affect the nilpotency of
$d^{(g)}_{\mathrm{total}}$.
\end{remark}
...
At genus~$0$, the bar differential satisfies $d^2 = 0$ exactly,
...
space, and $d^2$ acquires curvature proportional to the central
...
The preceding sections developed the bar complex genus by genus.
We now assemble them into a single spectral sequence whose
...
\begin{equation}
d_g^2(a_m) = \kappa \cdot a_m \otimes \lambda_g
\end{equation}
...
For the Heisenberg algebra, we computed $\mathrm{obs}_g(\mathcal{H}_k) = k \cdot \lambda_g$ and recognized the generating function as the $\hat{A}$-genus (\S\ref{sec:frame-genus-tower}).  The following theorem establishes that this factorization is universal: every Koszul chiral algebra has the same structure, with a single number $\kappa(\cA)$ controlling the entire genus tower.
...
For algebras with a single strong generator (Heisenberg, Virasoro, Kac--Moody),
the factorization holds at the level of cohomology classes.  For multi-generator
algebras such as $\mathcal{W}_3$ (generators $T$ and $W$ of weights $2$ and $3$),
the obstruction is $\mathrm{obs}_g = (c/2)\,\lambda_g^{(T)} + (c/3)\,\lambda_g^{(W)}$
(Theorem~\ref{thm:w3-obstruction}), a sum of contributions from different
generators that are proportional at genus~$1$ but may differ at genus~$g \geq 2$.
The universality formula~\eqref{eq:genus-universality} and the
free energy formula~(iii) hold at the level of
\emph{integrated} invariants (free energies $F_g = \int \mathrm{obs}_g$),
where all $\lambda$-class contributions collapse to a single Faber--Pandharipande
evaluation.
\end{remark}
...
\item Obstruction squares to zero: $(\text{obs}_g)^2 = 0$ for all $g$ and single-generator algebras (Theorem~\ref{thm:obstruction-nilpotent-all-genera}, via Mumford's relation $c(\mathbb{E})c(\mathbb{E}^\vee) = 1$; see Remark~\ref{rem:multi-generator-nilpotence} for the multi-generator case)
...
For the Heisenberg pair $(\mathcal{H}_\kappa, \mathrm{Sym}^{\mathrm{ch}}(V^*))$, we computed in~\S\ref{sec:frame-complementarity} that the central extension of $\mathcal{H}_\kappa$ and the curvature of its dual are complementary components of $H^*(\overline{\mathcal{M}}_1, Z(\mathcal{H}_\kappa))$.  The following theorem --- Theorem~C of this monograph --- establishes this complementarity in full generality, revealing it as a Lagrangian polarization.
...
Set
\[
\mathcal{H}_g(\mathcal{A})
:= H^*(\overline{\mathcal{M}}_g, \mathcal{Z}(\mathcal{A})),
\]
the cohomology of the Deligne--Mumford compactification
$\overline{\mathcal{M}}_g$ with coefficients in the center local system
$\mathcal{Z}(\mathcal{A})$.
Then:
\begin{enumerate}[label=\textup{(\roman*)}]
\item \emph{Shifted-symplectic structure.}
  Verdier duality on $\mathcal{Z}(\mathcal{A})$ induces a
  non-degenerate bilinear pairing
  \begin{equation}\label{eq:verdier-symplectic-pairing}
  \langle -, - \rangle_{\mathbb{D}} \colon
  \mathcal{H}_g(\mathcal{A}) \otimes \mathcal{H}_g(\mathcal{A})
  \to \mathbb{C}
  \end{equation}
  of cohomological degree $-(3g{-}3)$, using the Koszul identification
  $\mathbb{D}\mathcal{Z}(\mathcal{A}) \cong \mathcal{Z}(\mathcal{A})$.

\item \emph{Lagrangian polarization.}
  The Verdier involution $\sigma$ on $\mathcal{H}_g(\mathcal{A})$
  is an anti-involution for $\langle-,-\rangle_{\mathbb{D}}$, and its
  eigenspaces
  \begin{equation}\label{eq:lagrangian-decomp}
  Q_g(\mathcal{A}) = \ker(\sigma - \mathrm{id}), \qquad
  Q_g(\mathcal{A}^!) = \ker(\sigma + \mathrm{id})
  \end{equation}
  are complementary Lagrangians:
  $\langle-,-\rangle_{\mathbb{D}}$ restricts to zero on each, and
  \begin{equation}\label{eq:complementarity-decomp}
  Q_g(\mathcal{A}) \oplus Q_g(\mathcal{A}^!)
  \;\cong\; \mathcal{H}_g(\mathcal{A}).
  \end{equation}
  What one algebra sees as deformation, its dual sees as obstruction,
  because both are opposite polarizations of a single ambient symplectic
  deformation problem.

\item \emph{Perfect duality.}
  The restriction of $\langle-,-\rangle_{\mathbb{D}}$ to
  $Q_g(\mathcal{A}) \otimes Q_g(\mathcal{A}^!) \to \mathbb{C}$
  is a perfect pairing\textup{:}
...
\emph{Step 4: Higher vanishing.}
For $q > 0$: the fiber complex $\bar{B}^*_{\Sigma_g}(\mathcal{A})$ computes
$\mathrm{Ext}^*_{\mathcal{A}}(\mathbb{C}, \mathbb{C})$ via the bar resolution
(Theorem~\ref{thm:bar-nilpotency-complete}).
We claim that the Koszul property implies diagonal Ext concentration:
$\mathrm{Ext}^q_{\mathcal{A}}(\mathbb{C}, \mathbb{C})_n = 0$ for $q \neq n$.
Indeed, the Koszul complex $K = \mathcal{A} \otimes (\mathcal{A}^!)^*$ admits
a filtration by internal degree whose associated graded computes $\mathrm{Ext}$
via the bar resolution.  The acyclicity of the Koszul complex
(Definition~\ref{def:chiral-koszul-pair}, condition~(iii): the augmented Koszul
complex $K \to \mathbb{C}$ is a resolution) implies that $H^q(K_n) = 0$ for
$q \neq n$, since the only non-trivial cohomology arises in the degree
where the augmentation map is non-zero.  This is the chiral analog of the
classical characterization of Koszul algebras via diagonal Ext concentration
(cf.\ Loday--Vallette~\cite{LV12}, Theorem~3.4.4).
The chiral analog follows from the same filtration argument applied to the
chiral bar complex: the filtration by pole order
(Proposition~\ref{prop:pole-decomposition}) provides the analog of the weight
filtration in the classical Koszul complex, and the associated graded coincides
with the classical case by the genus-0 comparison
(Theorem~\ref{thm:geometric-equals-operadic-bar}).
In particular, at each bar degree~$p$ the non-diagonal fiber cohomology vanishes:
\begin{equation}
H^q\bigl(\bar{B}^p_{\Sigma_g}(\mathcal{A}),\, d_{\mathrm{fiber}}\bigr) = 0
\quad \text{for } q \neq p.
\end{equation}
Therefore $\mathcal{H}^q_{\mathrm{fiber}} = 0$ for $q > 0$ when restricted to the
degree-$0$ bar component, confirming that the fiber cohomology sheaf is
concentrated in degree~$0$ as claimed.
\end{proof}
...
\item Compute the \emph{total} differential $d^{(g)}_{\mathrm{total}}$ on $\bar{B}^{(g)}(\mathcal{A})$
...
This differential satisfies $(d^{(g)}_{\mathrm{total}})^2 = 0$.
\item Compute the \emph{fiberwise} collision differential $d_{\mathrm{fib}}$ and verify
that $(d_{\mathrm{fib}})^2 = \kappa \cdot \omega_g \in Z(\mathcal{A}) \otimes H^2(\overline{\mathcal{M}}_g)$;
this curvature measures the obstruction coefficient $\kappa(\mathcal{A})$.
\item Take cohomology of the total differential:
$Q_g(\mathcal{A}) = H^*(\bar{B}^{(g)}(\mathcal{A}),\, d^{(g)}_{\mathrm{total}})$.
...
The residue operator $d_0 = \sum_D \operatorname{Res}_D$ computes residues at collision divisors $D_{ij} = \{z_i = z_j\} \subset \overline{C}_n(\Sigma_g)$, while $d_k = \oint_{\gamma_k}$ integrates a single variable $z_i$ over the cycle $\gamma_k$ on $\Sigma_g$.  Both compositions vanish independently:
\[
d_0 \circ d_k = 0 \quad \text{and} \quad d_k \circ d_0 = 0.
\]
For $d_0 \circ d_k = 0$: after integrating $z_i$ over $\gamma_k$, the result is a form on a configuration space with one fewer variable.  The collision singularity $(z_i - z_j)^{-1}$ is replaced by $\oint_{\gamma_k} (z_i - z_j)^{-1} dz_i$, which is either $0$ (if $z_j \notin \gamma_k$) or $2\pi i$ (if $z_j \in \gamma_k$), in both cases producing a regular function of $z_j$ with no residue to extract.  For $d_k \circ d_0 = 0$: extracting the residue $\operatorname{Res}_{D_{ij}}$ at $z_i = z_j$ eliminates the variable $z_i$ (replacing it by the OPE coefficient at $z_j$); the subsequent period integral $\oint_{\gamma_k} dz_i$ has nothing to act on, since $z_i$ no longer appears.

(Note: The full OPE contains higher-order poles $(z_i - z_j)^{-m}$ for $m \geq 2$, but these contribute to the correction operators $d_k$ with $k \geq 2$ in the expansion $d_g = d_0 + \sum_k t_k d_k$, not to $d_0$ or $d_1$.  The relation $d_0 d_k + d_k d_0 = 0$ established above involves only the residue operator $d_0$ and the period integral $d_k$; the simple-pole analysis suffices because $d_0$ extracts first-order residues by definition.)
...
At genus~$0$, bar-cobar inversion was an algebraic identity: $\Omega(\bar{B}(\mathcal{H}_k)) \xrightarrow{\sim} \mathcal{H}_k$ (\S\ref{sec:frame-inversion}).  At genus~$g \geq 1$, the inversion acquires quantum corrections controlled by $\kappa(\cA) \cdot \lambda_g$, as seen for the Heisenberg genus tower (\S\ref{sec:frame-genus-tower}).  The following theorem --- Theorem~B of this monograph --- gives the general statement.
...
faithfully in the completed coderived category in the sense of
Positselski~\cite{Positselski}, not in the ordinary derived
category.
\end{enumerate}
\end{theorem}
...
& \barB_X^{(g)}(\cA)
  ...
  \arrow[r, phantom, "\scriptstyle d^2 = \kappa \cdot \omega_g"]
& {}
\\
\cA^! \arrow[r, "\barB_X^{(g)}"']
& \barB_X^{(g)}(\cA^!)
...
The preceding theorems converge on a single structural concept that we now
formalize.  A \emph{modular Koszul chiral algebra} is the natural
object for which all three main theorems hold simultaneously.
...
A \emph{modular Koszul chiral algebra} on $X$ is an augmented chiral
...
\item\label{D:dual}
A Koszul dual chiral algebra $\cA^!$ on~$X$ with
$\bar{B}_X(\cA) \simeq \cA^!$ as factorization coalgebras
...
The chiral algebra $\cA$ is Koszul: the bar spectral
...
quasi-isomorphism, and the unit
$\cA^! \xrightarrow{\sim} \bar{B}_X(\Omega_X(\cA^!))$ is likewise.

\item\label{MK:modular}
\emph{(Modular Koszulity.)}
At each genus $g \geq 1$, the genus-$g$ bar complex satisfies
diagonal Ext vanishing:
$\operatorname{Ext}^{i,j}_{\cA^{(g)}}(\Bbbk, \Bbbk) = 0$ for $i \neq j$.

\item\label{MK:complementarity}
\emph{(Complementarity.)}
There is a Verdier involution $\sigma$ on
$H^*(\overline{\mathcal{M}}_g, \mathcal{Z}(\cA))$ with
$Q_g(\cA) = \ker(\sigma - \mathrm{id})$ and
$Q_g(\cA^!) = \ker(\sigma + \mathrm{id})$, giving a direct-sum
decomposition
$Q_g(\cA) \oplus Q_g(\cA^!) = H^*(\overline{\mathcal{M}}_g, \mathcal{Z}(\cA))$.
\end{enumerate}
\end{definition}

\begin{remark}[Redundancy and dependence]\label{rem:mk-axiom-relations}
The axioms are not independent.  Axioms \ref{MK:koszul}
and~\ref{MK:inversion} together imply~\ref{MK:verdier}
(Theorem~\ref{thm:bar-cobar-isomorphism-main} and
Theorem~\ref{thm:verdier-bar-cobar}), and
\ref{MK:complementarity} is a consequence of \ref{MK:verdier}
and~\ref{MK:modular}
(Theorem~\ref{thm:quantum-complementarity-main}).
The definition is stated with all five axioms for clarity:
the axioms at genus~$0$
(\ref{MK:koszul}--\ref{MK:inversion}) carry the algebraic content,
while the axioms at higher genus
(\ref{MK:modular}--\ref{MK:complementarity}) carry the geometric content.
\end{remark}

\begin{proposition}[Standard examples are modular Koszul; \ClaimStatusProvedHere]
\label{prop:standard-examples-modular-koszul}
\index{modular Koszul chiral algebra!examples}
The following chiral algebras are modular Koszul:
\begin{enumerate}[label=\textup{(\alph*)}]
...
\begin{proof}
In each case, we verify the five axioms:

\emph{Axiom \ref{MK:koszul}}: The bar spectral sequence collapses at
$E_2$ for all five families.  For (a)--(b), the bar complex is explicit
(Chapters~\ref{chap:free-fields}--\ref{chap:beta-gamma}); for
(c), this is Theorem~\ref{thm:kw-bar-spectral}; for (d)--(e),
this is Theorem~\ref{thm:bar-cobar-spectral-sequence}.

\emph{Axiom \ref{MK:verdier}}: The Verdier compatibility is
Theorem~\ref{thm:verdier-bar-cobar}, which is proved in full
generality for any Koszul pair.

\emph{Axiom \ref{MK:inversion}}: Bar-cobar inversion at all genera is
Theorem~\ref{thm:higher-genus-inversion}, whose hypotheses are
satisfied for all five families at generic parameters.

\emph{Axiom \ref{MK:modular}}: Modular Koszulity (diagonal Ext vanishing
at genus $g \geq 1$) is verified:
for~(a), the bar complex at genus~$g$ has only double-pole contributions,
which are scalar (Theorem~\ref{thm:heisenberg-higher-genus});
for~(b), the free-field genus expansion is explicit
(\S\ref{sec:free-field-genera});
for~(c)--(e), we give the following argument at generic level.
The PBW filtration on the genus-$g$ bar complex is defined by conformal
weight, identically to genus~$0$.
The genus-$g$ propagator deformation affects the bar differential
only through the curvature term $\kappa \cdot \omega_g$
(Remark~\ref{rem:differential-notation}), which lies in the center
and hence does not alter the PBW-filtered structure of the collision
differential.
At the associated graded level, the $E_1$ page of the PBW spectral
sequence at genus~$g$ is therefore isomorphic (as a bigraded complex)
to the $E_1$ page at genus~$0$.
Since the $E_2$ collapse at genus~$0$ is established by
Theorem~\ref{thm:km-chiral-koszul}, the same collapse holds at
genus~$g$ at the associated graded level, and hence at generic
parameters where the spectral sequence degenerates.
(At special parameters where the spectral sequence may fail to
degenerate, the modular Koszul property remains open;
cf.\ the discussion in \S\ref{sec:koszul-across-genera}.)

\emph{Axiom \ref{MK:complementarity}}: This is
Theorem~\ref{thm:quantum-complementarity-main}, proved for all
Koszul pairs.
\end{proof}

\begin{definition}[Modular characteristic package; \ClaimStatusProvedHere]
\label{def:modular-characteristic-package}
\index{modular characteristic package|textbf}
Let $\cA$ be a modular Koszul chiral algebra
(Definition~\ref{def:modular-koszul-chiral}).  The
\emph{modular characteristic package} of~$\cA$ consists of
the following data:
\begin{enumerate}[label=\textup{(\roman*)}]
...
\item The \emph{spectral discriminant}
  $\Delta_{\cA}(x) = \prod_i (1 - \lambda_i x) \in \mathbb{Z}[x]$,
  encoding the generating function of bar cohomology dimensions
  (Theorem~\ref{thm:ds-bar-gf-discriminant}).
\item The \emph{universal Maurer--Cartan class}
  $\Theta_{\cA}$, whose scalar shadow is $\kappa(\cA)$
  and whose full form is a Maurer--Cartan element of the
  cyclic deformation complex
  (Conjecture~\ref{conj:universal-MC}).
\end{enumerate}
The first three components are established; the fourth is
conjectural in its non-scalar form.  The Heisenberg specialization
appears in Chapter~\ref{ch:heisenberg-frame},
Definition~\ref{def:frame-modular-package}.
\end{definition}

\begin{remark}[The modular characteristic package]\label{rem:modular-characteristic-package}
The modular characteristic package
(Definition~\ref{def:modular-characteristic-package}) specializes to
concrete invariants for each family; see the Master Table
(Table~\ref{tab:master-invariants}).
\end{remark}

\begin{theorem}[Modular characteristic theorem; \ClaimStatusProvedHere]
...
Let $\cA$ be a modular Koszul chiral algebra
(Definition~\ref{def:modular-koszul-chiral}).  The modular
characteristic package
(Definition~\ref{def:modular-characteristic-package}) is
determined, as a scalar system, by a single invariant
...
\item \emph{Duality.}
  Under Koszul duality, $\kappa$ is anti-symmetric:
...
\item \emph{Spectral invariance.}
  The spectral discriminant $\Delta_{\cA}(x)$ is invariant
  under Drinfeld--Sokolov reduction and depends only on the
  quadratic OPE data of the Koszul pair.
\end{enumerate}
Together, $\kappa$ is a group homomorphism from modular Koszul
algebras (under tensor product) to $(\mathbb{C}, +)$,
intertwined with Koszul duality by sign reversal.  It is
the first characteristic number of the conjectural universal
Maurer--Cartan class $\Theta_{\cA}$
(Conjecture~\ref{conj:universal-MC}): the genus-$1$ shadow of a
structure that organizes the entire genus tower.
\end{theorem}
...
Part~(v) is Theorem~\ref{thm:ds-bar-gf-discriminant}.
...
Properties (i)--(iv) of Theorem~\ref{thm:modular-characteristic}
identify $\kappa$ formally as a first Chern class: a group
homomorphism from an abelian group of geometric objects to a ring,
controlling a universal obstruction, and intertwined with duality
by sign reversal.  In the classical setting, $c_1(L) \in H^2(X)$
for a line bundle $L$ satisfies: $c_1(L \otimes L') = c_1(L) +
c_1(L')$ (additivity), $c_1(L^\vee) = -c_1(L)$ (duality), and
$c_1$ controls the obstruction to global sections.  The invariant
$\kappa(\cA)$ is the chiral-algebraic analogue: the modular Koszul
algebra is the ``line bundle,'' the genus tower is the ``obstruction
to global sections,'' and the universal class $\Theta_{\cA}$ is
the ``Chern--Weil representative'' whose scalar trace recovers
$\kappa \cdot \lambda_g$.
\end{remark}
...
\item Affine Kac--Moody algebras at \emph{admissible} levels
  ...
  (\S\ref{sec:admissible-level-reps}).  Axiom~\ref{MK:inversion} fails.
...
package $(\kappa, \{F_g\}, \Delta)$.  What one algebra sees as
