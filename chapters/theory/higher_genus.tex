...
\emph{Candidate~3: Exotic vertex algebras.}
Vertex algebras not of Lie-theoretic origin
(i.e., not obtainable from affine Kac--Moody, DS reduction,
coset, orbifold, or free-field constructions) are rare.
Known examples include:
(a)~the triplet algebras $\mathcal{W}(p)$ (logarithmic
extensions of Virasoro minimal models, one-parameter in~$p$);
(b)~permutation orbifolds $\cA^{\otimes n} / S_n$ (multi-scalar,
not genuinely non-scalar);
(c)~the Mathieu moonshine VOA (a sporadic construction, not
known to admit continuous deformations).
In each case, either the deformation space is at most
one-dimensional or the algebra decomposes as a product.

The absence of genuinely non-scalar examples across all three
classes is strong evidence for a structural principle:
