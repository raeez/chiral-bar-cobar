...
\item[\textup{(d)}] \textup{[\ClaimStatusConjectured]} For $g \geq 3$, $\mathcal{A}_3^{(g)} = 2\pi i \cdot \omega_{\mathrm{Ar}}^{(g)}$, where $\omega_{\mathrm{Ar}}^{(g)}$ is the Arakelov $(1,1)$-form on $\Sigma_g$.
\end{enumerate}
\end{theorem}
...
Part~(d) of Theorem~\ref{thm:quantum-arnold-relations} remains \ClaimStatusConjectured{}
for $g \geq 3$.  The genus-$2$ proof above
generalizes the genus-$1$ strategy by replacing the Weierstrass $\zeta$-function
with the prime form derivative $\partial_z \log E(z,w)$ and using the Fay trisecant
identity in place of the classical identity
$\zeta(u)\zeta(v) + \mathrm{cyc} = \frac{1}{2}[\wp(u) + \wp(v) + \wp(w)]$.
The same strategy applies at genus~$g \geq 3$: the propagator decomposes as
$\eta_{ij}^{(g)} = (h_{ij} + a_{ij})(dz_i - dz_j)$ with
$\bar\partial a_{ij} = \frac{\pi}{2i}\sum_{\alpha,\beta}
\omega_\alpha(z_i)(\operatorname{Im}\Omega)^{-1}_{\alpha\beta}
\bar\omega_\beta(z_j)$, and the expected result is
$\mathcal{A}_3^{(g)} = 2\pi i \cdot \omega_{\mathrm{Ar}}^{(g)}$
where $\omega_{\mathrm{Ar}}^{(g)} = \frac{i}{2}\sum_{\alpha,\beta}
(\operatorname{Im}\Omega)^{-1}_{\alpha\beta}\,
\omega_\alpha \wedge \bar\omega_\beta$.
The remaining ingredient is the vanishing of the holomorphic
cyclic sum via the genus-$g$ Fay identity, which requires
verifying that the symmetric terms in the Fay-type
relation~\eqref{eq:fay-g2} are killed by antisymmetry at
all genera.
\end{remark}
