...
the completed/infinite-generator regime
$\mathcal{W}_\infty$
...
powers (implemented in \texttt{compute/lib/genus1\_pbw\_sl2.py})
confirms that no new representation-theoretic obstruction
appears.  For $\mathfrak{sl}_2^{\otimes 3}$, the diagonal
Casimir eigenspace dimensions are
$\{24{:}7,\;12{:}10,\;4{:}9,\;0{:}1\}$, matching the expected
irreducible decomposition; for $\mathfrak{sl}_2^{\otimes 4}$
they are
$\{40{:}9,\;24{:}21,\;12{:}30,\;4{:}18,\;0{:}3\}$; and for
$\mathfrak{sl}_2^{\otimes 5}$ they are
$\{60{:}11,\;40{:}36,\;24{:}70,\;12{:}75,\;4{:}45,\;0{:}6\}$;
at $\mathfrak{sl}_2^{\otimes 6}$ the same check gives
...
$\operatorname{rank}(d_1)=243$ at $n=6$, and the commutators
$[\operatorname{ad}(x), d_1]$ ($x \in \mathfrak{sl}_2$) and
$[C_2, d_1]$ vanish identically in all four cases.
...
holds unconditionally.
\item The full scalar modular characteristic package
$\{\kappa(\mathrm{Vir}_c),\, \{F_g\}_{g \geq 1},\,
\Delta_{\mathrm{Vir}_c}\}$ is unconditionally defined.
\end{enumerate}
This upgrades
Proposition~\textup{\ref{prop:conditional-modular-koszul}}
...
\item A \emph{finite-dimensional compute scaffold} for Step~1 now exists in
  \texttt{compute/lib/mc2\_cyclic\_linf.py}, with executable dg~Lie identities,
  low-arity cyclic $L_\infty$ brackets, and a first symbolic MC solver pass
  (\texttt{compute/tests/test\_mc2\_cyclic\_linf.py}).
  The same module now includes a first bar-derived seed for affine
