...
and~\ref{conj:universal-theta}, resolving
Conjecture~\ref{conj:master-theta}).
\end{definition}
...
\item under Verdier/Koszul duality, $\Theta_{\cA}$ is sent to
  $\Theta_{\cA^!}$.
\end{enumerate}
This resolves the principal target of Conjecture~\ref{conj:master-theta} (MC2).
\end{theorem}
...
Thus the three open MC2 hypotheses reduce to two: the
cyclic $L_\infty$-algebra construction \textup{(MC2-1)} and the
tautological-line support \textup{(MC2-3)}.
\end{remark}
...
\qquad i = 1, 2,
\]
which has rank~$1$ at all generic $(k_1, k_2)$ (both partials
are nonzero but neither vanishes independently).
The level set $\{(k_1', k_2') : c_{\cA}(k_1', k_2') = c_0\}$
is a one-dimensional curve, and the VOA isomorphism type
depends only on~$c_0$.  For non-GKO embeddings
$\mathfrak{h} \hookrightarrow \mathfrak{g} \oplus \mathfrak{g}'$
...
(Example~\ref{ex:class-s}), which is a one-parameter family
(the level is fixed by the 4D theory; the parameter comes
from varying the theory).
...
\item The vertex algebra $\cA[\mathcal{T}]$ is determined
  ...
  In all computed examples
  (Beem--Rastelli~\cite{beem-et-al}; Bonetti--Meneghelli--Rastelli),
...
\item \emph{$\mathcal{W}$-algebras}
  ...
  scalar saturation holds via the \emph{BRST rigidity} argument
  (Proposition~\ref{prop:nonprincipal-scalar-saturation}).
  The quantum DS~reduction functor produces a unique vertex
  algebra structure at every level $k \neq -h^\vee$, including
  admissible~$k$.  The deformation space remains one-dimensional
  (the central charge direction), so
  $\dim H^2_{\mathrm{cyc}} = 1$.

\item \emph{Simple current extensions} at admissible levels:
  these are the genuine frontier.  Such extensions have primary
  strong generators beyond the currents, and are not
  directly produced by DS reduction.  Neither the trivial
  Kac--Moody argument nor the BRST argument applies.
  Examples include the triplet algebras~$\mathcal{W}(p)$
  (logarithmic extensions of Virasoro minimal models) and
  certain extended affine VOAs.
  In all known instances, the continuous deformation space
  is still one-dimensional, but no uniform proof exists.
\end{itemize}
...
vertex algebras at admissible levels that are obtained by
simple current extension rather than DS reduction.
