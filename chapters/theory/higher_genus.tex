...
The governing question is: how genus-$0$ bar-cobar inversion lifts to all
genera as a single modular deformation with explicit theorem-level control of
curvature, complementarity, and universality.

\begin{remark}[Normative status]\label{rem:higher-genus-normative}
This chapter and Chapter~\ref{chap:concordance} fix the reference
formulation of the modular characteristic theory used throughout the
...
genus-zero theory but the scalar shadow of a single modular
deformation.  At H-level, Conjecture~\ref{conj:master-theta} predicts
that this deformation is controlled by a universal obstruction class
$\Theta_A$, while the theorematic
statement proved in this chapter is the genus-universality formula
$\mathrm{obs}_g(A) = \kappa(A) \cdot \lambda_g$.  In the full
homotopy-native picture the obstruction would live in
$H^1(\overline{\mathcal{M}}_g, \mathbb{E}^\vee)$, and the Hodge class
$\lambda_g$ of the moduli space enters as the modular weight of the
deformation.  Koszul duality at genus $g$ is thus \emph{modular}
Koszul duality: the bar-cobar adjunction extends across genera, the
proved scalar correction is proportional to $\lambda_g$, and the full
Maurer--Cartan package remains part of the MC2 programme.
...
Theorem~\ref{thm:pbw-universal-conformal} resolves MK3
for \emph{every} chiral algebra with a unique weight-$2$ generator,
subsuming the Virasoro and principal $\mathcal{W}$ theorems and
extending to non-principal families (Bershadsky--Polyakov,
hook-type, superconformal).
What remains open is therefore:
genus-$0$ Koszulness~(MK1) and Verdier~(MK2)
for each non-principal family;
the H-level/factorization realization of the
infinite-generator regime $\mathcal{W}_\infty$ beyond the theorematic
completed M-level principal-stage package
\textup{(}Conjecture~\textup{\ref{conj:master-infinite-generator}}\textup{)};
and non-principal orbit duality
...
\begin{theorem}[Universal PBW degeneration for chiral algebras with a
  stress tensor; \ClaimStatusProvedHere]
\label{thm:pbw-universal-conformal}
\index{PBW spectral sequence!universal degeneration|textbf}
\index{modular Koszul chiral algebra!universal criterion|textbf}
\textup{[Regime: curved-central
\textup{(}Convention~\textup{\ref{conv:regime-tags})}.]}
...
\item among all generators, exactly one has conformal
  ...
  $T_{(1)} = L_0$.
\end{enumerate}
Then for every genus~$g \geq 1$, the PBW spectral
sequence for the genus-$g$ bar complex has concentrated
$E_\infty$ page:
\[
E_\infty^{p,q}\bigl(\bar{B}^{(g)}(\cA)\bigr)
\;=\;
E_\infty^{p,q}\bigl(\bar{B}^{(0)}(\cA)\bigr)
\quad \text{for all } g \geq 0.
\]
In particular, $\cA$ satisfies axiom~\textup{MK3}
\textup{(}Definition~\textup{\ref{def:modular-koszul-chiral})}
unconditionally at all genera.
\end{theorem}
...
(\ref{item:pbw-bounded} of
Theorem~\ref{thm:pbw-koszulness-criterion}) yields
$E_\infty(g) = E_\infty(0)$.
\end{proof}
...
\begin{enumerate}[label=\textup{(\roman*)}]
\item
Theorem~\ref{thm:pbw-universal-conformal} subsumes
the Virasoro degeneration
(Theorem~\ref{thm:pbw-allgenera-virasoro})
and the principal finite-type $\mathcal{W}$ degeneration
(Theorem~\ref{thm:pbw-allgenera-principal-w})
as special cases: both satisfy hypotheses~(a)--(b).
...
This resolves the MK3 component of the non-principal orbit
programme
\textup{(}Conjecture~\textup{\ref{conj:w-orbit-duality}}\textup{)}:
PBW concentration is automatic; only genus-$0$ Koszulness
\textup{(MK1)} and Verdier \textup{(MK2)} remain to be
...
finite-dimensional).  The pronilpotent completion
scaffold of \S\ref{sec:modular-koszul-programme}
ensures this.  MK3 is therefore automatic for
$\mathcal{W}_\infty$; the remaining obstacle for
Conjecture~\ref{conj:master-infinite-generator} is
MK1 in the completed setting.
...
and~\ref{thm:pbw-universal-conformal},
together with their corollaries,
upgrade the interacting conditional claims in Part~II to
unconditional statements at all genera, including complementarity,
genus expansions, and Kodaira--Spencer constructions.
\end{enumerate}
\end{remark}
...
\item A \emph{finite-dimensional compute scaffold} for Steps~1--3 now exists in
  ...
  $\mathfrak{sl}_3$, and $\mathfrak{sp}_4$ higher-bracket seeds are all
...
\item A \emph{first completed tensor / clutching surrogate}
  ...
  $\mathfrak{sl}_3$ and $\mathfrak{sp}_4$:
  on the channel $(e_1,e_2,f_{12})$ the residual
  $\eta$-component is $xyz$ for $\mathfrak{sl}_3$ and
  $2xyz$ for $\mathfrak{sp}_4$, with corresponding
  genus-$3$ obstruction outputs $\eta$ and $2\eta$.
  Across $\mathfrak{sl}_2,\mathfrak{sl}_3,\mathfrak{sp}_4$,
...
\item \emph{Geometric tensor / clutching package.}  Form the
  ...
  The open part is the intrinsic pro-nilpotent geometric realization,
  the modular-operadic composition laws, and the boundary/clutching
  compatibility on the geometric complex.
...
\item \emph{One-channel normalization.}  In the simple-Lie case,
  ...
  Proposition~\ref{prop:one-channel-normalization-criterion} then shows
  that once this tautological-line support is in hand, the
  normalization is fixed by one scalar comparison with the already-
  proved shadow $\kappa(\cA)\lambda_g$.  What remains is therefore to
  prove that clutching/trace isolation statement and verify the
  corresponding Verdier/Koszul-duality compatibility.
\end{enumerate}

The clutching part of Step~2 may be stated concretely as: show that
$\Theta_{\cA}$
  ...
  A first boundary-factorized compatibility check now exists at the
  surrogate level; the open part is the full modular-operadic
  compatibility on the geometric complex.
...
via $C^n_{\mathrm{cyc}} \cong \Lambda^{n+1}(\mathfrak{g}^*)$
(Remark~\textup{\ref{rem:mc2-status}}, item~\textup{(vii)}).
...
H^3(\mathfrak{g})$ follows from the pairing-induced
identification $C^n_{\mathrm{cyc}} \cong \Lambda^{n+1}
(\mathfrak{g}^*)$; for simple~$\mathfrak{g}$,
$H^3(\mathfrak{g}) \cong \mathbb{C}$ with generator the
Killing $3$-cocycle (Cartan).
\end{proof}
...
\item identify the surviving one-channel genus-$g$ obstruction with the
  ...
  and then perform one normalized scalar comparison in the simple-Lie
  case
  \textup{(}Propositions~\textup{\ref{prop:tautological-line-support-criterion}}
  and~\textup{\ref{prop:one-channel-normalization-criterion}}\textup{)}.
\end{enumerate}
...
clutching/trace test plus one scalar check.
\end{remark}
