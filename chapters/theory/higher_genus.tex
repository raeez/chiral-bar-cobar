...
The governing question of this chapter is how genus enters the bar-cobar transform internally, as curvature, obstruction data, and modular-operadic Maurer--Cartan structure rather than as an external correction.
\end{remark}

\begin{remark}[H/M/S route; Convention~\ref{conv:hms-levels}]
This chapter is mixed H/M/S. Curved and strict differentials are tracked at M-level, the universal Maurer--Cartan package is promoted to H-level only on the printed loci, and scalar or spectral characteristics are kept at their own S-level or intermediate status.
\end{remark}
...
The genus-$g$ bar complex admits a graph-sum description
...
\end{equation}
summed over stable graphs~$\Gamma$ of type $(g,n)$ with
$\sum_v g_v + b_1(\Gamma) = g$.  The vertices carry local
operations of the chiral algebra; internal edges are
propagators (contractions via the invariant pairing); and
loops encode genus.  This is the Feynman transform
$\mathrm{FT}_{\mathcal{M}od}(\cA)$ of
Definition~\ref{def:feynman-transform}: the modular bar
complex \emph{is} the Feynman integral.
\end{remark}
...
\begin{theorem}[Maximal vs.\ trivial \texorpdfstring{$A_\infty$}{A-infinity}; \ClaimStatusProvedHere]\label{thm:ainfty-com-lie-interchange}
...
This is Koszul duality, not bosonization: the fermion and $\beta\gamma$ system are Koszul dual as chiral algebras (Lie--Com duality), while bosonization is a different operation.
\end{example}
...
\begin{definition}[Genus-\texorpdfstring{$g$}{g} logarithmic forms]
...
At the conceptual level, the modular bar differential has three
geometric components whose nature is more transparent than the
Leray grading:
\begin{enumerate}[label=\textup{(\roman*)}]
\item $d_{\mathrm{int}}$: the internal differential of the
  coefficient chiral algebra, preserving both genus and insertion
  number.  In the Leray decomposition, this is a component of
  $d^{(0)}$.
\item $d_{\mathrm{sep}}$: contraction of \emph{separating} edges
  in the stable-graph description
  (Definition~\ref{def:feynman-transform}).  A separating
  contraction splits a connected stable graph into two
  components of genera $g_1 + g_2 = g$, encoding ordinary gluing of
  two lower-genus pieces.  Physically, this is the factorization
  of a string amplitude into two sub-amplitudes.
\item $d_{\mathrm{ns}}$: contraction of \emph{non-separating}
  edges (loops).  A non-separating contraction decreases the Betti
  number of the graph by one and raises the genus at the vertex
  by one: this is the genus-raising operation.  Physically, it
  corresponds to self-gluing of a surface along a non-trivial cycle.
\end{enumerate}
The nilpotence $d^2 = 0$ follows from
$\partial^2 = 0$ on the boundary stratification of
$\overline{\mathcal{M}}_{g,n}$: each boundary stratum is crossed
an even number of times by the geometric boundary operator.
\end{remark}
...
The genus filtration is not an auxiliary device: it is the
\emph{fundamental organizational parameter} of modular homotopy
theory.  Each genus level carries qualitatively distinct content:
\begin{itemize}
\item $g = 0$ (tree level): the ordinary operadic bar construction,
  governed by the Arnold relation.  Classical Koszul duality.
\item $g = 1$ (one-loop): curvature enters via
  $\dfib^{\,2} = \kappa(\cA) \cdot \omega_1$.
  The central extension (Sugawara, Virasoro) has its geometric
  origin here.
\item $g \geq 2$ (higher loops):
  $H^*(\overline{\mathcal{M}}_g)$ contributes through the
  Feynman transform (Definition~\ref{def:feynman-transform}), with
  Mirzakhani volume asymptotics
  $\operatorname{Vol}(\overline{\mathcal{M}}_g) \sim (2g)!$
  controlling the growth of genus-$g$ bar cohomology.
\end{itemize}
Modular homotopy theory is the homotopy theory whose
fundamental filtration parameter is genus.
\end{remark}
...
Steps~A and~C (H-level homotopy Lagrangian decomposition and
S-level cohomological shadow) are stated directly in the theorem
above.  \emph{Step~B} (M-level) follows.
...
The M-level mapping cone
$\operatorname{Cone}(\sigma - \mathrm{id})[-1]$ is a dg model for
the H-level homotopy fiber
$\operatorname{fib}(\sigma - \mathrm{id})$.  Model independence
(Proposition~\ref{prop:model-independence}) ensures the M-level
Lagrangian property ($\langle p^+x, p^+y\rangle = 0$)
determines the H-level shifted-symplectic Lagrangian structure
up to contractible ambiguity.

\emph{Reduction B$\to$C} (M-level $\to$ S-level):
Taking cohomology of the M-level projector $p^+$ yields the
S-level eigenspace decomposition
$\mathcal{H}_g(\cA) = Q_g(\cA) \oplus Q_g(\cA^!)$
with $\dim Q_g(\cA) = \dim Q_g(\cA^!)$ by perfect duality.
For the Heisenberg atom this extraction gives $Q_1 = H^0$,
$Q_1^! = H^2$.
...
\emph{Reduction B$\to$A} (M-level $\to$ H-level):
The M-level induction is the chain-level shadow of
factorization-homology descent
(Remark~\ref{rem:homotopy-native-b}): the excision property of
factorization homology in the sense of
Ayala--Francis~\cite{AF15} reconstructs the factorization algebra
on a genus-$g$ surface from its restrictions to lower-genus
pieces, and the M-level quasi-isomorphism~$\psi_g$ lifts to the
H-level equivalence by model independence
...
\emph{Reduction B$\to$C} (M-level $\to$ S-level):
The S-level generating functions are extracted from the M-level
dg model via the PBW spectral sequence: its $E_2$ collapse on
the Koszul locus (Lemma~\ref{lem:e2-collapse-higher-genus})
gives
$\dim H^n(\Omega_g \bar{B}_g(\cA)) = \dim E_2^{n,0}(g)$,
and the bar cohomology dimensions are the Taylor coefficients
of the Hilbert series of~$\cA^!$.
\end{remark}
...
\emph{Reduction B$\to$A} (M-level $\to$ H-level):
The M-level curvature $\dfib^{\,2} = \kappa(\cA)\cdot\omega_g$
determines the H-level obstruction class in the tautological ring
via the Grothendieck--Riemann--Roch formula
(Theorem~\ref{thm:family-index}): the Chern character of the
bar family $[\barB^{(g)}(\cA)] \in K_0(\overline{\mathcal{M}}_g)$
maps to $\kappa(\cA) \cdot (\hat{A}(ix) - 1)$ under
$\operatorname{ch}\colon K_0 \to H^*$.
...
\emph{Reduction B$\to$C} (M-level $\to$ S-level):
The S-level free energies are extracted from the M-level curvature
via the Faber--Pandharipande evaluation
$F_g(\cA) = \kappa(\cA) \cdot
\frac{2^{2g-1}-1}{2^{2g-1}} \cdot \frac{|B_{2g}|}{(2g)!}$,
which follows from integrating $\mathrm{obs}_g$ against
$\psi$-classes on~$\overline{\mathcal{M}}_{g,1}$
...
All three inputs have been computationally verified for
$\mathfrak{sl}_2$, $\mathfrak{sl}_3$, $\mathfrak{sp}_4$, and
$\mathfrak{g}_2$, with cyclic $L_\infty$ brackets, Casimir recovery,
$\kappa$-extraction, complementarity, and critical-level vanishing all
confirmed by the module
...
\item For circular quivers (e.g., the $\widehat{A}_1$ necklace
  ...
  but S-duality identifications and Schur-index data are
  consistent with the conjectural picture that the vertex algebra depends on
...
\Theta_\cA
\;\longrightarrow\;
(\barB^{(\bullet)}(\cA),\, \bar{d}^\Theta)
\;\longrightarrow\;
R\pi_{g*}\barB^{(g)}(\cA)
\;\longrightarrow\;
\mathcal{V}_\cA \in K_0(\overline{\mathcal{M}}_g)
\]
separates two objects.
$\Theta_\cA$ is \emph{upstream}: it records
\emph{how} the genus tower is assembled (chain-level
flatness, integrability of the cyclic deformation).
$\mathcal{V}_\cA$ is \emph{downstream}: it records
\emph{what} virtual geometric object remains after
derived pushforward.
In the scalar-saturated case
($\dim H^2_{\mathrm{cyc}} = 1$), $\mathcal{V}_\cA$ is
rank one, $c_1$ carries all information, and
$\Theta_\cA^{\min} = \kappa \cdot \eta \otimes \Lambda$.
Beyond scalar ($\dim H^2_{\mathrm{cyc}} \geq 2$),
$\mathcal{V}_\cA$ is higher-rank,
$c_1$ no longer determines $c_*$ or
$\operatorname{Hol}$, and the non-scalar HPT
construction
(Construction~\ref{constr:non-scalar-hpt}) provides the
upstream data that $\mathcal{V}_\cA$ encodes downstream.
\end{remark}
...
Positselski~\cite{Positselski11} provide the correct categorical
