...
\section{\texorpdfstring{$A_\infty$ Structures and Higher Operations}{A- Structures and Higher Operations}}
...
\subsubsection{\texorpdfstring{The Birth of $A_\infty$: Stasheff's Discovery}{The Birth of A-: Stasheff's Discovery}}
...
\subsection{\texorpdfstring{The Geometric Bar Complex and Its $A_\infty$ Structure}{The Geometric Bar Complex and Its A- Structure}}
...
\subsubsection{\texorpdfstring{Complete $A_\infty$ Structure from Configuration Spaces}{Complete A- Structure from Configuration Spaces}}
...
\begin{theorem}[$A_\infty$ Structure from Bar Complex - Complete; \ClaimStatusProvedHere]\label{thm:bar-ainfty-complete}
The geometric bar complex $\bar{B}^{\text{geom}}(\mathcal{A})$ carries a natural $A_\infty$ structure where:

\textbf{1. Operations from residues:} Each $m_k$ is given by a sum over planar binary trees:
...
\subsubsection{\texorpdfstring{Enhanced $A_\infty$ Structure with Moduli Space Interpretation}{Enhanced A- Structure with Moduli Space Interpretation}}
...
\begin{theorem}[Complete $A_\infty$ Operations via Moduli Spaces; \ClaimStatusProvedHere]\label{thm:ainfty-moduli}
...
\begin{theorem}[Pentagon Identity - Geometric Realization; \ClaimStatusProvedHere]
...
The five associations of four symbols are:
\begin{enumerate}
\item $((ab)c)d$
\item $(a(bc))d$
\item $a((bc)d)$
...
These correspond to the five codimension-1 boundary divisors of $\overline{M}_{0,5}$. The boundary of the 2-dimensional space gives:
...
\subsubsection{\texorpdfstring{Complete $A_\infty$ Structure on Cobar}{Complete A- Structure on Cobar}}

\begin{theorem}[Cobar $A_\infty$ Structure - Complete; \ClaimStatusProvedHere]\label{thm:cobar-ainfty-complete}
...
\textbf{Verification of $n_k$ for $k \geq 3$.}
...
\textbf{$A_\infty$ relations.}
The Stasheff relations $\sum_{r+s+t=n} (-1)^{rs+t}\, n_{r+1+t}(\mathrm{id}^{\otimes r} \otimes n_s \otimes \mathrm{id}^{\otimes t}) = 0$ follow from the codifferential condition on~$\mathcal{C}$.  In the geometric realization, these relations correspond to Stokes' theorem on the boundary strata of $\overline{C}_n(X)$: each boundary face decomposes into a product of lower-dimensional moduli spaces, and the signed sum of boundary contributions vanishes.

\textbf{Duality with bar.}
...
\begin{theorem}[Loss of Structure in Homology; \ClaimStatusProvedHere]
...
\textbf{Row 1: $\eta_{ij} \leftrightarrow \delta_{ij}$.}
...
\textbf{Row 2: Residue $\leftrightarrow$ distribution.}
...
\textbf{Row 3: $\overline{C}_n \leftrightarrow C_n$.}
...
\textbf{Row 4: $m_2 \leftrightarrow \Delta_2$.}
...
\textbf{Row 5: $m_3 \leftrightarrow \Delta_3$.}
...
\subsubsection{\texorpdfstring{How $A_\infty$ Structures Interchange}{How A- Structures Interchange}}
...
\subsubsection{\texorpdfstring{Curved $A_\infty$ Algebras: Central Extensions and Anomalies}{Curved A- Algebras: Central Extensions and Anomalies}}
...
\begin{enumerate}
\item $m_0 \in \mathcal{A}^2$ is the \textbf{curvature element} (a nullary operation producing a degree-2 element)
...
\textbf{Step 1: Filtered bar complex.}
...
\textbf{Step 2: Associated graded comparison.}
...
\textbf{Step 3: Koszul hypothesis gives $\mathrm{gr}$ convergence.}
...
\textbf{Step 4: Filtered quasi-isomorphism.}
...
\textbf{Step 5: Filtered Koszul dual.}
...
\begin{theorem}[Cobar Resolution - Complete; \ClaimStatusProvedElsewhere]
...
\begin{theorem}[Maurer-Cartan = Deformations; \ClaimStatusProvedHere]\label{thm:genus-graded-mc}
...
\begin{theorem}[Yangian from Deformation; \ClaimStatusProvedHere]
...
\begin{theorem}[Deforming Heisenberg; \ClaimStatusProvedHere]
...
\subsubsection{\texorpdfstring{Example: $\beta\gamma$ System Deformation}{Example: System Deformation}}

\begin{theorem}[$\beta\gamma$ Deformations; \ClaimStatusProvedHere]
...
At $t = 1/2$: conformal weights become $h_\beta = 3/2$, $h_\gamma = -1/2$, matching the ghost numbers of the superconformal $\beta\gamma$ system. Note that the fundamental OPE remains $\beta(z)\gamma(w) \sim 1/(z-w)$ (bosonic statistics); only the conformal weights shift.
\end{proof}
...
\begin{theorem}[Higher Associahedron Identity for $m_6$; \ClaimStatusProvedHere]
For six elements, the identity $d^2 = 0$ on $\overline{C}_7(X)$ gives a relation among the $C_5 = 42$ full parenthesizations of 6~elements, governed by the 4-dimensional associahedron~$K_6$.
\end{theorem}

\begin{proof}[Combinatorial Structure]
The $C_5 = 42$ vertices of $K_6$ correspond to the full parenthesizations.  The codimension-1 faces of $K_6$ give the identity: each face is a product of lower associahedra, and the alternating sum of face contributions vanishes by $d^2 = 0$ on the bar complex.
\end{proof}
...
\subsection{\texorpdfstring{Configuration Space on $\Sigma_2$}{Configuration Space on -2}}
...
introduces new terms in the bar differential, giving rise to a \textbf{curved}
...
The $E_4, E_6$ terms are \textbf{two-loop quantum corrections} to the bar complex curvature,
...
The \textbf{pattern}: genus $g$ introduces Siegel modular forms of weight
...
\subsection{\texorpdfstring{Homology and Cohomology of $\Sigma_g$}{Homology and Cohomology of -g}}
...
\begin{theorem}[Properties of Period Matrix; \ClaimStatusProvedElsewhere]
...
\textbf{Component $d^{(0)}$: Collision residues (genus-preserving).}
...
\textbf{Component $d^{(1)}$: Cycle insertions (genus-shifting by 1).}
...
\textbf{Component $d^{(g')}$: Boundary strata contributions.}
...
\item \textbf{Double pole}: Pole of order 2 at $z = 0$
\item \textbf{Expansion}: $\wp(z) = \frac{1}{z^2} + 3G_4(\tau) z^2 + 5G_6(\tau) z^4 + O(z^6)$
...
These are \textbf{modular forms of weight $\frac{1}{2}$} (for $\Gamma_0(4)$ with appropriate multiplier systems).
\end{theorem}
...
\textbf{Components}:
\begin{enumerate}
\item \textbf{Theta part}: $\vartheta_1'/\vartheta_1$ is the elliptic analog of $1/(z_i - z_j)$
\item \textbf{Quasi-periodicity}: Under $z_i \to z_i + \tau$, the form shifts by $-2\pi i\,(dz_i - dz_j)$; this shift is the genus-1 obstruction
\end{enumerate}
\end{definition}
...
\textbf{3. Modular transformation}: Under $\gamma = \bigl(\begin{smallmatrix}a&b\\c&d\end{smallmatrix}\bigr) \in \operatorname{SL}_2(\mathbb{Z})$, the isomorphism $E_\tau \xrightarrow{\sim} E_{\gamma\tau}$ sends $z \mapsto z/(c\tau+d)$.  Then:
\begin{equation}
...
\textbf{Geometric interpretation}: The quasi-periodic shift $-2\pi i\,(dz_i - dz_j)$ under $z_i \to z_i + \tau$ is the obstruction to $\eta_{ij}^{(1)}$ descending to a well-defined form on $C_n(E_\tau)$.  This shift defines the central extension at genus~1 and is the source of the curvature element $\kappa$ in the bar complex.
\end{proof}
...
\textbf{Key observation}: The right side is \textbf{non-zero} and involves all three points symmetrically! This is the quantum correction (cf.\ Totaro, Bezrukavnikov on cohomology of configuration spaces of curves).
...
This is the \textbf{genus-1 quantum correction}: the Arnold relation acquires a $(1,1)$-form correction reflecting the non-trivial topology of $E_\tau$.  The correction is cyclically symmetric in the three points, as required by the symmetry of the left-hand side.
\end{proof}
...
For genus~2: $\Sigma_2$ is parameterized by the period matrix $\Omega \in \mathbb{H}_2$, and the same argument gives the $\mathbb{C}[\Omega,\bar{\Omega}]$-module structure.
\end{proof}
...
\textbf{Identification with moduli space cohomology.}
...
- 16 theta characteristics (10 even + 6 odd)
...
- 64 theta characteristics (36 even + 28 odd)
...
The \textbf{central charge} is the numerical invariant:
\begin{equation}
...
\begin{theorem}[Obstruction Formula - General; \ClaimStatusProvedHere]\label{thm:obstruction-general}
...
\textbf{Step 5: Evaluation of the obstruction class.}
...
evaluations---a deep connection between number theory and quantum geometry.
\end{remark}
...
$d^2 = 0$ on the bar complex (Theorem~\ref{thm:universal-kac-moody-koszul}).
\end{theorem}

\begin{proof}

\textbf{Step 1: Genus-zero curvature.}

By Theorem~\ref{thm:universal-kac-moody-koszul} (Step~2 of the proof), the bar complex
...
\textbf{Step 2: Extension to genus $g$.}
...
\textbf{Step 3: Assembling the formula.}
...
with $d^2 = 0$ on the bar complex at critical level (Theorem~\ref{thm:universal-kac-moody-koszul}),
...
\textbf{Step 5: Verification for $\mathfrak{sl}_2$.}
...
\textbf{Genus 1:} For $W_3$ minimal model $(p,q) = (5,4)$ with $c = \frac{4}{5}$:
\begin{align}
...
\begin{proof}
For $g \leq 2$, nilpotence follows from dimensional vanishing.  The obstruction $\operatorname{obs}_g$ lies in $H^{2g}(\overline{\mathcal{M}}_g, Z(\mathcal{A}))$ (as a $\lambda$-class multiple; see Table~\ref{tab:obstruction-summary}).  Therefore $(\operatorname{obs}_g)^2 \in H^{4g}(\overline{\mathcal{M}}_g, Z(\mathcal{A})^{\otimes 2})$.  Since $\dim_{\mathbb{R}} \overline{\mathcal{M}}_g = 6g - 6$, this group vanishes whenever $4g > 6g - 6$, i.e., $g < 3$.  For $g = 0$: $H^0(\overline{\mathcal{M}}_{0,n})$ is trivially nilpotent (scalars).  For $g = 1$: $(\operatorname{obs}_1)^2 \in H^4(\overline{\mathcal{M}}_1)$, but $\dim_{\mathbb{R}} \overline{\mathcal{M}}_1 = 2$, so $H^4 = 0$.  For $g = 2$: $(\operatorname{obs}_2)^2 \in H^8(\overline{\mathcal{M}}_2)$, but $\dim_{\mathbb{R}} \overline{\mathcal{M}}_2 = 6$, so $H^8 = 0$.
\end{proof}
...
For $g \geq 3$, the dimensional argument fails ($4g \leq 6g - 6$ for $g \geq 3$), and an independent proof is needed.  The expected route is through the tautological ring $R^*(\overline{\mathcal{M}}_g)$: the obstruction is a $\lambda$-class multiple (Remark~\ref{rem:lambda-universality}), and the required identity $\lambda_g^2 = 0$ in $R^{4g}(\overline{\mathcal{M}}_g)$ should follow from the Faber--Pandharipande tautological relations.  We note that the $g \leq 2$ case proved above suffices for all applications in this monograph: the explicit computations in Part~2 use only $g \leq 2$, and the abstract higher-genus theory (Part~1) is stated in terms of the curved $A_\infty$ framework which accommodates non-nilpotent curvature.
\end{remark}
...
\item \textbf{Kac-Moody}: $\text{obs}_g = \frac{(k+h^\vee) \cdot \dim(\mathfrak{g})}{2h^\vee}
\cdot \lambda_g$
\item \textbf{$W_3$}: $\text{obs}_g = c \cdot (\frac{\lambda_g^{(2)}}{2} +
\frac{\lambda_g^{(3)}}{3})$
\end{enumerate}
...
is both natural and fundamental.
...
\begin{remark}[Geometric Perspective: Kontsevich's Construction]
...
\textbf{At genus 1}: $\overline{\mathcal{M}}_{1,1}$ is a one-dimensional Deligne--Mumford stack with tautological class $\lambda = c_1(\mathbb{E})$. The cohomology is:
\begin{equation}
...
\begin{theorem}[Quantum Complementarity - Main Result; \ClaimStatusProvedHere]
...
\textbf{Step 1: Degree-0 identification.}
...
\textbf{Step 3: Constancy over moduli.}
...
\textbf{Verification for Heisenberg.}
...
\textbf{Stage 1: Gauss-Manin connection on chiral homology.}
...
connection (the \textbf{Gauss-Manin connection}):
\begin{equation}
...
\textbf{Stage 2: Center action on moduli cohomology.}
...
\textbf{Stage 3: Anti-commutativity with Verdier duality.}
...
\textit{Ingredient 1: Lie derivative anti-commutes with Verdier duality.}
...
\textit{Ingredient 2: Verdier duality intertwines center actions via the Koszul pairing.}
...
\textit{Ingredient 3: The involution absorbs the transpose.}
...
\textit{Assembly.}  The combined action $\nabla_{\kappa(v)}^z := \rho(z) \circ
...
\textbf{Verification for Heisenberg.}  For $\mathcal{H}_\kappa$ with center
...
\textbf{Step 8: Verdier involution and eigenspace decomposition.}
...
\textbf{Involutivity.}  $\sigma^2 = \mathrm{id}$ follows from
...
\textbf{Identification of $V^+$ with $Q_g(\mathcal{A})$.}
...
\textbf{Identification of $V^-$ with $Q_g(\mathcal{A}^!)$.}
By the same argument with $\mathcal{A}$ and $\mathcal{A}^!$ exchanged:
elements arising from $\bar{B}^{(g)}(\mathcal{A}^!)$ contribute to $V^-$
because the Verdier duality reverses their $j_*/j_!$ extension type.
...
\begin{enumerate}
\item Elements of $Q_g(\mathcal{A}) = V^+$ correspond to \textbf{obstructions}
to extending $\mathcal{A}$ from genus $g-1$ to genus $g$
\item Elements of $Q_g(\mathcal{A}^!) = V^-$ correspond to \textbf{deformations}
...
\textbf{Verification at small genera.}
...
\begin{enumerate}
\item \textbf{Genus 0}: $\overline{\mathcal{M}}_0 = \text{point}$, so
...
(Theorem~\ref{thm:virasoro-self-duality}) with center $Z(\mathrm{Vir}_0)
...
Feigin--Frenkel center (Theorem~\ref{thm:critical-level-structure}).
\end{corollary}
...
\begin{enumerate}
\item \textbf{Universality.}  All Koszul self-dual chiral algebras
...
correspondence.
\item \textbf{Involutive structure.}  For self-dual $\cA$, the
...
\textbf{Genus 1 moduli space}: The cohomology ring of $\overline{\mathcal{M}}_{1,1}$ is generated by $\lambda = c_1(\mathbb{E})$:
\begin{equation}
...
The Koszul dual of the Heisenberg algebra is the curved Chevalley--Eilenberg algebra (cf.\ Theorem~\ref{thm:heisenberg-koszul-dual-early}):
\begin{equation}
...
\caption{Computing Quantum Corrections}
...
\textbf{Explicit form:} For genus $g=1$ (elliptic case), there is one $A$-period $t_1 = \tau$, and:
...
\textbf{Step 4: Quantum terms vanish by intersection properties.}
...
\textit{(Note: The vanishing of $d_g^2$ relies on the Lagrangian property of the $A$-cycle subspace in $H_1(\Sigma_g)$, not on the cohomology of the moduli space $\mathcal{M}_g$. Recall $H^1(\mathcal{M}_g, \mathbb{Q}) = 0$ for $g \geq 2$ by Harer's theorem, but this is not the relevant vanishing here.)}
...
\begin{lemma}[Bar as Factorization Homology - Precise; \ClaimStatusProvedHere]\label{lem:bar-as-fact-hom-AF}
...
Assume:
\begin{enumerate}
\item \textbf{Modular Koszulity:} At each genus $g \geq 1$, the bar complex
$\bar{B}^{(g)}(\mathcal{A})$ satisfies diagonal Ext vanishing:
$\mathrm{Ext}^{i,j}_{\mathcal{A}^{(g)}}(\mathbb{k}, \mathbb{k}) = 0$ for $i \neq j$.
\item \textbf{Convergence:} The genus-graded bar complex
...
of Chapters~\ref{chap:free-fields} and~\ref{chap:kac-moody}.
