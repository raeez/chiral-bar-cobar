...
$g$ period corrections from $H^0(\Sigma_g, \Omega^1)$:
\[
\eta_{ij}^{(g)} = d\log E(z_i, z_j)
+ \sum_{k=1}^{g} \omega_k(z_i)\! \oint_{A_k}\! \omega_k(z_j),
\]
and the curvature generalizes to
$\dfib^{\,2} = \kappa(\cA) \cdot \omega_g$, where $\omega_g$ is the
Arakelov $(1,1)$-form.  The total corrected differential $\Dg{g}$,
which adds period integrals along $A$-cycles, satisfies
$\Dg{g}^{\,2} = 0$
...
\label{rem:higher-genus-normative}%
\label{rem:higher-genus-hms}%
...
counterpart; we present them in parallel:
\begin{center}
\small
\begin{tabular}{ll}
...
Convention~\ref{conv:higher-genus-differentials} encodes the
nilpotence-periodicity correspondence
(Remark~\ref{rem:nilpotence-periodicity}) at the level of
differentials.  The genus-$g$ propagator
$\eta_{ij}^{(g)} = d\log E(z_i,z_j) + \cdots$ is built from the
logarithm of the prime form, which acquires monodromy~$2\pi i$ around
each $B$-cycle of~$\Sigma_g$.  The Arakelov normalization compensates
single-valuedness but transfers the monodromy into the Arnold defect,
yielding the curvature $\dfib^{\,2} = \kappa(\cA)\cdot\omega_g$.

The scalar $\kappa(\cA)$ is therefore the \emph{infinitesimal generator
of the monodromy} of the logarithmic propagator: the residue at the
branch point of the multi-valued function~$\log E$.  The total
corrected differential $\Dg{g}$ incorporates quantum corrections from
all Leray degrees, restoring $\Dg{g}^{\,2} = 0$: this is the
statement that the \emph{full} logarithm is single-valued on the
universal cover of~$\Sigma_g$, with the Gauss--Manin component
$d^{(1)}$ trivializing the monodromy by passage to the covering space.

The equation $\dfib^{\,2} = \kappa\cdot\omega_g$ is the chiral
analogue of the classical Riemann--Hilbert correspondence, which
identifies flat connections ($\nabla^2 = 0$, the nilpotent side) with
local systems (finite monodromy, the periodic side).  Here $\dfib$ is
the connection that fails flatness by the curvature
$\kappa\cdot\omega_g$, and $\Dg{g}$ is the corrected connection that
is genuinely flat.  The passage $\dfib \leadsto \Dg{g}$ is the gauge
transformation that absorbs the curvature into the quantum corrections,
producing the flat connection whose monodromy representation governs
the bar family over~$\overline{\mathcal{M}}_g$.

The three differentials of Convention~\ref{conv:higher-genus-differentials}
thereby correspond to three levels of the nilpotence-periodicity
hierarchy: $\dzero$~is the genus-$0$ logarithm (single-valued,
trivially nilpotent); $\dfib$~is the genus-$g$ logarithm with
monodromy (curved, nilpotence fails by~$\kappa$); and $\Dg{g}$~is
the full logarithm on the universal cover (nilpotent, monodromy
absorbed into quantum corrections).
\end{remark}
...
\begin{theorem}[Higher associahedron identity for $m_5$ {\cite{Sta63}}; \ClaimStatusProvedElsewhere]\label{thm:higher-associahedron-m5}
...
\begin{theorem}[Geometric enhancement of Com-Lie; \ClaimStatusProvedHere]\label{thm:geometric-com-lie-enhancement}
...
\begin{theorem}[Maximal vs.\ trivial $A_\infty$; \ClaimStatusProvedHere]\label{thm:ainfty-com-lie-interchange}
...
\begin{corollary}[Higher associahedron identity for $m_6$; \ClaimStatusProvedHere]\label{cor:higher-associahedron-m6}
...
(Theorem~\ref{thm:explicit-theta}) governs the decomposition
concretely: $\mathrm{C}_0$ detects the scalar trace
$\operatorname{tr}(\Theta_\cA) = \kappa \cdot \lambda_g$
(the Hodge class obstruction at each genus), while
$\mathrm{C}_1$ uses the full matrix data of $\Theta_\cA$ ---
the cyclic $L_\infty$ brackets transferred to the minimal
model --- to split the ambient complex
$\mathbf{C}_g(\cA) = R\Gamma(\overline{\mathcal{M}}_g,
\mathcal{Z}_\cA)$ into Lagrangian halves via the Verdier
involution.  The complementarity identity
$\Theta_\cA + \Theta_{\cA^!} = 0$ is the chain-level
mechanism enforcing $\mathbf{Q}_g(\cA) \oplus
\mathbf{Q}_g(\cA^!) = \mathbf{C}_g(\cA)$.
\end{remark}
...
are quasi-isomorphisms (by the K\"unneth formula for DG modules over a field), yielding the claim.
\end{proof}
...
\begin{remark}[$E_2$ collapse mechanism]\label{rem:e2-collapse-mechanism}
\index{spectral sequence!E2 collapse@$E_2$ collapse}
The $E_2$ collapse of the spectral sequence
$E_1^{p,q}(g) \Rightarrow H^{p+q}(\Omega_g \bar{B}_g(\cA))$
holds for the same structural reason as in genus~$0$: the PBW
filtration is a filtration by chiral algebras, and the Koszulness
hypothesis ensures that the associated graded is a Koszul complex
in the classical sense.  The $d_r$ differential for $r \geq 2$
maps between PBW-graded components separated by $r$ steps, and
the Koszul concentration (all bar cohomology is in bar-degree~$0$
on the associated graded) forces these maps to have zero source
or zero target.  This is
Corollary~\ref{cor:bar-cohomology-koszul-dual}, Step~4, applied
fiberwise over~$\overline{\mathcal{M}}_g$.
\end{remark}
...
Theorem~B requires the Koszulity hypothesis (axiom~MK1 of
Definition~\ref{def:modular-koszul-chiral}), which holds at
generic level but fails at admissible levels
$k = -h^\vee + p/q$ with $p, q \geq 2$ coprime
(Proposition~\ref{prop:admissible-levels-permuted};
\S\ref{sec:admissible-level-reps}).  The failure has three
manifestations, listed in order of increasing structural depth:
\begin{enumerate}[label=\textup{(\roman*)}]
\item \emph{Non-semisimplicity.}
  The category $\mathcal{O}_k$ is non-semisimple: null vectors
  appear in Verma modules, Ext-vanishing fails, and the PBW
  spectral sequence (Corollary~\ref{cor:bar-cohomology-koszul-dual})
  need not degenerate at~$E_2$.  The bar spectral sequence
  for admissible modules is discussed in
  Remark~\ref{rem:admissible-bar-ss}.

\item \emph{Failure of the counit.}
  The counit $\Omega(\bar{B}(\cA)) \to \cA$ is \emph{not} a
  quasi-isomorphism in the ordinary derived category; its failure
  is measured by the non-vanishing of higher $\mathrm{Ext}$ groups
  in the non-semisimple category~$\mathcal{O}_k$.  The universal
  Maurer--Cartan class $\Theta_\cA$
  (equation~\eqref{eq:universal-MC}) is the obstruction:
  $\Theta_\cA \neq 0$ precisely when inversion fails.

\item \emph{Coderived replacement.}
  The correct replacement is the coderived bar-cobar adjunction.
  At the factorization level, this is the Positselski equivalence
  (Theorem~\ref{thm:factorization-positselski}), which gives an
  equivalence on the coderived category \emph{without} requiring
  Koszulness.  At the module level, the comodule-contramodule
  correspondence
  (Theorem~\ref{thm:chiral-co-contra-correspondence}) provides
  the same service: it identifies bar-cobar data with
  $\cA$-modules via the coderived category
  $\mathsf{D}^{\mathrm{co}}_{\mathrm{prov}}(X)$
  (Definition~\ref{def:provisional-coderived}).
\end{enumerate}
In summary: Theorem~B applies on the Koszul locus (generic levels);
off it, one must pass to the coderived category, where bar-cobar
...
The preceding propositions may appear to be mere reformulations of
the M-level proof in the language of factorization homology.
The essential difference is that \emph{the H-level
statement is stronger}: it produces an equivalence of
$\infty$-categories (not just a quasi-isomorphism of chain complexes),
and this additional structure is indispensable in three respects.

\emph{First}, the excision equivalence~\eqref{eq:fh-excision}
is an equivalence of \emph{$\Eone$-algebras}, not merely of
underlying chain complexes.  The M-level proof establishes a
quasi-isomorphism $\psi_g \colon \Omega_g(\Bbar_g(\cA)) \to
\cA_g$ of dg modules; the $\Ainf$-algebra structure on both sides
is \emph{additional data} that must be transferred via the homotopy
transfer theorem (Appendix~\ref{app:homotopy-transfer},
Theorem~\ref{thm:bar-cobar-htt}).  The factorization homology
perspective produces the $\Ainf$-structure automatically, because
excision is an equivalence in the $\infty$-category of
$\Eone$-algebras, and the $\Ainf$-operations are the coherence
data for this equivalence.

\emph{Second}, the Mayer--Vietoris sequence at H-level is
\emph{functorial in the decomposition}: different pants
decompositions of $\Sigma_g$ produce \emph{canonically equivalent}
factorization homology objects, because excision is an
$\infty$-categorical property that does not depend on the choice of
collar.  At M-level, this independence is a non-trivial theorem
requiring separate verification (the compatibility of the extension
lemma with different boundary strata of $\overline{\mathcal{M}}_g$).

\emph{Third}, the coderived category
$\cat{D}^{\mathrm{co}}_\infty(\cat{Fact}_g(X))$, which governs
the off-Koszul regime (Remark~\ref{rem:homotopy-native-b}), is an
$\infty$-categorical construction with \emph{no chain-level
analogue}: the provisional coderived category of
Definition~\ref{def:provisional-coderived} captures only the
homotopy category, not the full $\infty$-categorical data.  The
factorization homology perspective produces the $\infty$-categorical
coderived category directly, via the theory of stable
$\infty$-categories \cite[Chapter~1]{HA}.
\end{remark}

\begin{proposition}[$E_2$-collapse as formality;
\ClaimStatusProvedHere]
...
The H/M/S architecture of this monograph
(Convention~\ref{conv:hms-levels}) assigns semantic levels to the
three viewpoints:
...
\begin{tabular}{@{} c l l @{}}
\toprule
\textbf{Level} & \textbf{Object} & \textbf{Content} \\
\midrule
H & $\displaystyle\int_{\Sigma_g}\cA$ & Factorization homology \\
M & $\Omega_g(\Bbar_g(\cA))$ & Bar-cobar complex \\
S & $\sum_{n \geq 0} \dim H^n \cdot q^n$ & Hilbert series \\
\bottomrule
\end{tabular}
\end{center}
...
\medskip
\noindent
The central claim of this subsection is that the factorization
algebra $\int_{\Sigma_g}\cA$ is not merely a convenient packaging
of the bar-cobar complex, but the \emph{correct mathematical
object}: the M-level construction $\Omega_g(\Bbar_g(\cA))$ is a
\emph{presentation} (model) of the H-level object, and different
presentations (different SDR choices, different pants decompositions,
different orderings of handle attachments) all produce canonically
equivalent H-level data.  The proofs of this chapter work at M-level
because that is where computation is possible, but the
\emph{meaning} of the results---the naturality,
the functoriality, the independence of choices---lives at H-level.

The passage from M to H is not automatic: it requires establishing
that the chain-level constructions are homotopy-invariant, that the
bar-cobar adjunction is a Quillen equivalence (or equivalently,
an adjunction of $\infty$-categories), and that the relevant model
structures exist.  These foundational issues are addressed in
Chapter~\ref{ch:en-koszul-duality}; the present chapter constructs
the M-level data that the H-level theory organizes.
\end{remark}
...
(Remark~\ref{rem:nilpotence-periodicity}), the genus-$0$
diagram is the \emph{single-valued regime}: the categorical
logarithm has no monodromy and the square commutes in the
ordinary derived category.  The genus-$g$ diagram is the
\emph{multi-valued regime}: the curvature $\kappa\cdot\omega_g$ is
the infinitesimal generator of the monodromy of the logarithm
(Remark~\ref{rem:curvature-riemann-hilbert}), and the coderived
category is the analytic continuation that makes the multi-valued
logarithm well-defined.  The complementarity
$\kappa + \kappa' = \kappa_{\mathrm{total}}$ is the
\emph{additivity of the logarithm}: $\log(\cA \otimes \cA^!)
= \log(\cA) + \log(\cA^!)$, with the total monodromy
$\kappa_{\mathrm{total}}$ partitioned between the algebra and
its Koszul dual.
\end{remark}
...
list with three further consequences:
\begin{enumerate}[label=\textup{(MK\arabic*)},start=6]
\item\label{MK:mc-control}
\emph{(Universal MC control.)}
The universal Maurer--Cartan problem for~$\cA$ is posed on
\[
\Defcyc(\cA)\;\widehat{\otimes}\;\Gmod
\]
by Proposition~\ref{prop:geometric-modular-operadic-mc}, and its
...
\bigr)
\]
of Theorem~\ref{thm:universal-theta}.
...
\end{enumerate}
For modular Koszul chiral algebras with simple Lie symmetry, MK6--MK7
are theorematic consequences of Theorem~\ref{thm:mc2-full-resolution};
Lemma~\ref{lem:mk67-from-mc2} packages this identification.  MK8
applies only to $\Eone$-chiral objects and remains programmatic.
On the standard infinite towers, the remaining H-level comparison
problem is exactly Conjecture~\ref{conj:master-infinite-generator}:
construct filtered H-level targets whose finite quotients recover the
theorematic stages and whose coefficient packets satisfy
\[
\mathsf{C}^{\mathrm{res}}_{s,t;u;m,n}(N)
=\mathsf{C}^{\mathrm{DS}}_{s,t;u;m,n}(N)
\quad\text{on }\mathcal{I}_N,
\qquad
\mathsf{K}^{\mathrm{line}}_{a,b}(N)
=\mathsf{K}^{\mathrm{RTT}}_{a,b}(N)
\quad\text{on }\Delta_{a,0}(N).
\]
\end{remark}
...
\begin{theorem}[PBW concentration at all genera for principal finite-type $\mathcal{W}$-algebras; \ClaimStatusProvedHere]
...
The higher-genus PBW problem is now resolved for the standard
finite-type interacting families:
\begin{enumerate}[label=\textup{(\roman*)}]
\item Kac--Moody:
  non-trivial $\mathfrak{g}$-modules die by Whitehead and the
  surviving Killing invariant dies by the level-$k$ contraction
  \textup{(}Theorem~\textup{\ref{thm:pbw-allgenera-km}}\textup{)}.
\item Virasoro:
  the unique weight-$2$ generator gives
  $d_2^{\mathrm{PBW}} = L_0 = h \cdot \mathrm{id}$ on weight~$h$,
  so no further contraction is needed
  \textup{(}Theorem~\textup{\ref{thm:pbw-allgenera-virasoro}}\textup{)}.
\item Principal finite-type $\mathcal{W}$:
  the same $L_0$ diagonal survives, while every higher-spin state
  contributes only strictly above the conformal-weight diagonal, so
  $d_2^{\mathrm{PBW}}$ is block upper triangular with invertible
  diagonal
  \textup{(}Theorem~\textup{\ref{thm:pbw-allgenera-principal-w}}\textup{)}.
\end{enumerate}
The genus-independent input common to all three cases is the
factorization \eqref{eq:enrichment-factorization}: local OPE data acts
on the finite-dimensional modules~$M_h$, while
$H^1(\Sigma_g,\bC)$ enters only as a passive multiplicity factor.

Theorem~\ref{thm:pbw-universal-semisimple} isolates the common
unique-weight-$2$ mechanism on the $E_3$ page for chiral algebras with
a stress tensor.  For the standard finite-type interacting families,
the theorematic MC1 resolution still comes from the family-specific
Virasoro and principal $\mathcal{W}$ proofs built on top of this
mechanism.
What remains open is therefore:
genus-$0$ Koszulness~(MK1) and Verdier~(MK2)
for each non-principal family;
the filtered H-level/factorization realization and
coefficient-identification package for the infinite-generator regime
$\mathcal{W}_\infty$ beyond the theorematic completed M-level
principal-stage package
\textup{(}Conjecture~\textup{\ref{conj:master-infinite-generator}}\textup{)};
and the distinct three-packet non-principal orbit frontier
\textup{(}Conjecture~\textup{\ref{conj:w-orbit-duality}}: dual-orbit
input, orbit-indexed level shift, paired DS seed
transport/globalization\textup{)}.
\end{remark}
...
\begin{corollary}[Unconditional modular Koszulity for principal finite-type $\mathcal{W}$-algebras; \ClaimStatusProvedHere]
...
\label{prop:pbw-universal-conformal}% backward-compatible label
...
\begin{enumerate}[label=\textup{(\roman*)}]
\item
Theorem~\ref{thm:pbw-universal-semisimple}\textup{(a)--(b)} packages
the common unique-weight-$2$ mechanism behind the Virasoro degeneration
(Theorem~\ref{thm:pbw-allgenera-virasoro})
and the principal finite-type $\mathcal{W}$ degeneration
(Theorem~\ref{thm:pbw-allgenera-principal-w}):
both satisfy hypotheses~(a)--(b), and in both families the remaining
no-later-differential and extension analysis is supplied by the
family-specific proofs.
The Kac--Moody theorem
(Theorem~\ref{thm:pbw-allgenera-km})
is \emph{not} a special case of
hypotheses~(a)--(b), because the stress
tensor~$T$ in a Kac--Moody algebra is the Sugawara
construction (a composite field, not a generator);
however, non-principal $\mathcal{W}$-algebras with
semisimple Lie symmetry~$\fg$ that satisfy
hypotheses~(a)--(c) are covered unconditionally by
the semisimple part of the theorem.
The independent Whitehead--Casimir--Killing mechanism of
Theorem~\ref{thm:pbw-allgenera-km} remains the route for
Kac--Moody algebras themselves.

\item
The theorem applies to every non-principal
$\mathcal{W}$-algebra
$\mathcal{W}^k(\mathfrak{g}, f)$, including:
the Bershadsky--Polyakov algebra
$\mathcal{W}^k(\mathfrak{sl}_3, f_{\mathrm{sub}})$
(generators $J$, $G^\pm$, $T$ of weights
$1$, $\tfrac{3}{2}$, $\tfrac{3}{2}$, $2$);
hook-type $\mathcal{W}$-algebras in types~$A$, $D$;
and superconformal algebras $\mathcal{N} = 1, 2$.
In each case, the stress tensor is the unique weight-$2$
generator, and hypotheses~(a)--(b) hold.
When the Lie symmetry of the non-principal
$\mathcal{W}$-algebra is semisimple---as it is for
non-principal $\mathcal{W}^k(\mathfrak{g}, f)$ whenever the
Levi factor of the centralizer~$\mathfrak{g}^f$ is
semisimple---hypothesis~(c) also holds, and the
enrichment-killing is unconditional at all genera by
part~(c).
In the remaining cases, the theorem should be read
as an MK3 criterion at the $E_3$ page, not as a stand-alone resolution
of the non-principal orbit programme
\textup{(}Conjecture~\textup{\ref{conj:w-orbit-duality}}\textup{)}.
What still requires family-specific input for those cases is genus-$0$
Koszulness \textup{(MK1)}, Verdier \textup{(MK2)}, and the exclusion
of later differentials and extension classes beyond the common
$E_3$-page vanishing.

\item
For infinite-generator algebras such as
$\mathcal{W}_\infty$
(one generator $W_s$ for each integer $s \geq 2$),
hypotheses~(a)--(b) hold provided the weight
filtration is complete (each weight-$h$ sector is
finite-dimensional).  In this regime the theorem records only the
finite-stage weight-$2$ mechanism.  Turning that mechanism into a
completed MK3 statement remains part of the inverse-limit MC4 package,
not an automatic consequence of hypotheses~(a)--(b).  The pronilpotent
completion scaffold of \S\ref{sec:modular-koszul-programme} is the
route toward that stronger statement.
...
global curve topology---holds for any chiral algebra
defined by local OPE relations but may fail for algebras
whose structure depends on global curve data
(e.g., elliptic vertex algebras).

\item
Theorems~\ref{thm:pbw-allgenera-km},
\ref{thm:pbw-allgenera-virasoro},
and~\ref{thm:pbw-allgenera-principal-w},
together with their corollaries,
upgrade the interacting conditional claims in Part~II to
unconditional statements at all genera for the named standard
families, including complementarity, genus expansions, and
Kodaira--Spencer constructions.
Theorem~\ref{thm:pbw-universal-semisimple}\textup{(a)--(b)}
records the common unique-weight-$2$ $d_2$ mechanism behind the
Virasoro and principal finite-type $\mathcal{W}$ proofs;
part~\textup{(c)} extends this to an unconditional statement for
all families with semisimple Lie symmetry satisfying
hypotheses~(a)--(c).
\end{enumerate}
\end{remark}
...
\begin{definition}[Full modular characteristic package; \ClaimStatusProvedHere]
...
\begin{remark}[The modular characteristic hierarchy: logarithmic data and their exponentials]
...
of three levels.  In the nilpotence-periodicity framework
(Remark~\ref{rem:nilpotence-periodicity}), the first three are
\emph{logarithmic} (nilpotent, infinitesimal) data at increasing
refinement, while the periodicity profile~$\Pi_{\cA}$ is the
corresponding \emph{exponential} (periodic, global) datum:
\begin{enumerate}[label=\textup{(\arabic*)}]
\item \emph{Scalar logarithmic data} \textup{(proved)}:
  ...
  Determined by a single number~$\kappa(\cA)$---the leading
  coefficient of the categorical logarithm.  Its exponential is
  the scalar modular period~$N$
  (Remark~\ref{rem:periodicity-as-exponential}).
\item \emph{Spectral logarithmic data} \textup{(proved, non-scalar)}:
  $\Delta_{\cA}(x)$, bar cohomology generating functions
  \textup{(}Theorem~\textup{\ref{thm:spectral-characteristic})}.
  Depend on the full quadratic OPE data, not just~$\kappa(\cA)$.
  The discriminant encodes the eigenvalues of the monodromy
  operator on bar cohomology; its exponential is the
  periodicity profile $\Pi_{\cA} = (M_{\cA}, Q_{\cA}, G_{\cA})$
  of Remark~\ref{rem:periodicity-triple}.
\item {\tolerance=9999\emergencystretch=1em
  \emph{Full Maurer--Cartan data} \textup{(proved)}:
  $\Theta_{\cA}$, $\Defcyc(\cA)$, and the ambient
  family $\mathcal{H}_{\cA}$
  \textup{(}Definition~\textup{\ref{def:full-modular-package}},
  Theorems~\textup{\ref{thm:universal-theta}}
  and~\textup{\ref{thm:mc2-full-resolution})}.
  The complete logarithm, whose exponential is the full modular
  automorphy of the bar family over~$\overline{\mathcal{M}}_g$.\par}
\end{enumerate}
The hierarchy resolves the status distinction without weakening
the modular vision: the logarithmic data ($\kappa$, $\Delta$,
$\Theta$) are the primary invariants, and the periodic data
($N$, $\Pi$, modular automorphy) are derived via
exponentiation.  The scalar package specializes to concrete
...
of the universal class~$\Theta_{\cA}$
...
\begin{remark}[$K$-theoretic hierarchy]\label{rem:spectral-characteristic-programme}
\index{spectral discriminant!K-theoretic hierarchy}
The spectral discriminant $\Delta_{\cA}(x)$ captures the
eigenvalue spectrum of the monodromy operator on bar cohomology.
Combined with the scalar invariant $\kappa(\cA)$ from
Theorem~\ref{thm:modular-characteristic}, it fits into a
$K$-theoretic hierarchy.  Setting
$\mathcal{V} = [R\pi_{g*}\bar{B}^{(g)}(\cA)] \in
K_0(\overline{\mathcal{M}}_g)$, we have:
$c_1(\det \mathcal{V})$ recovers~$\kappa$;
$\operatorname{ch}(\mathcal{V})$ recovers~$\Delta_{\cA}$;
and the holonomy representation of~$\mathcal{V}$ recovers the
full periodicity profile~$\Pi_{\cA}$.
See Remark~\ref{rem:structural-saturation} for the structural
hierarchy.
Elevating $\Delta_{\cA}$ to a characteristic class theorem
(asserting that $\Delta_{\cA}$ classifies a natural characteristic
class of~$\mathcal{V}$ beyond the first Chern class) belongs to
the modular characteristic programme
(Definition~\ref{def:full-modular-package}); the present theorem
establishes the invariance of the S-level shadow.
\end{remark}
...
The minimal model identification
$\mathcal{H} \cong \fg \oplus \mathbb{C}\!\cdot\!\eta$ with
$l_n^{\mathrm{tr}} = 0$ for $n \geq 4$ is proved in full for the
Kac--Moody family (Corollary~\ref{cor:km-minimal-linf}) and
extends to $\mathcal{W}$-algebras via DS~functoriality
(Proposition~\ref{prop:ds-package-functoriality}).  For
families not reached by DS reduction from a simple Lie
algebra --- in particular, exotic VOAs with multi-dimensional
$H^2_{\mathrm{cyc}}$ --- the minimal model may differ, and the
explicit formula should be regarded as conditional on the one-dimensional
cyclic cohomology input.  See
Conjecture~\ref{conj:scalar-saturation-universality} for the
universality question at admissible levels.
\end{remark}
...
makes the following conceptual points precise:
\begin{enumerate}[label=\textup{(\roman*)}]
\item \emph{$\kappa$ is a complete invariant at the scalar level.}
  The full genus tower is controlled by a single number: $\kappa$
  determines $\theta_g$ for all~$g$ (through
  $[\theta_g] = \kappa \cdot [\mu] \otimes \lambda_g$).  The
  non-scalar content (the homotopy \emph{type} of $\Defcyc(\cA)$,
  not just its obstruction classes) is invisible to the scalar trace
  but encoded in the chain-level corrections.

\item \emph{The Lie bracket IS the universal class.}
  At chain level, $\theta_g = \kappa \cdot \mu \otimes \lambda_g$
  (modulo exact terms), so the genus-$g$ component of $\Theta$ is
  literally the Lie bracket $[-,-]_{\mathfrak{g}}$ --- the same
  structure that defines the chiral algebra's OPE.  The genus
  tower is the Lie bracket ``integrated over moduli space.''

\item \emph{The MC equation is the Jacobi identity, genus by genus.}
  The vanishing~\eqref{eq:l2-vanishes} of the $l_2$-obstruction at
  all genera is exactly the Jacobi identity
  $[\mu,\mu]_{\mathrm{NR}} = 0$ for $\mathfrak{g}$.  The MC equation
  for the universal class is a genus-parameterized lift of the
  Jacobi identity from algebra to moduli.  The Killing cocycle
  $\phi = \kappa([-,-],-)$ --- the unique obstruction direction ---
  measures the failure of the Lie bracket to be associative,
  controlled by the same 3-form that defines the central extension.

\item \emph{The $\hat{A}$-genus is the character of $\Theta$.}
  The generating function~\eqref{eq:theta-generating-function}
  shows that $\hat{A}(ix) - 1$ is the ``modular character'' of the
  universal class: it counts how $\Theta_{\cA}$ distributes its
  weight across genera.  The appearance of $\hat{A}$ is not
  mysterious --- it is the Todd class of the universal curve,
  pushed forward by GRR.

\item \emph{Anomaly cancellation is $\Theta = 0$.}
  The critical level $k = -h^\vee$ (where $\kappa = 0$) is the
  unique level at which $\Theta_{\cA} = 0$: the bar complex is
  uncurved, the central extension vanishes, and the genus tower
  becomes trivial.  This is the homotopy-algebraic incarnation
  of anomaly cancellation.

\item \emph{Complementarity is $\Theta + \Theta^! = K \cdot \eta
  \otimes \Lambda$.}  The duality constraint $\kappa + \kappa' = K$
  lifts to $\Theta_{\cA} + \Theta_{\cA^!} = K \cdot \eta \otimes
  \Lambda$: what one algebra sees as deformation, its dual sees as
  obstruction, and the sum is a universal constant.

\item \emph{Lattice sector: $\Theta$ is additive.}
  For the rank-$d$ lattice VOA $V_\Lambda = \mathcal{H}_1^{\otimes d}$,
  $\Theta_{V_\Lambda} = d \cdot \eta \otimes \Lambda$ by additivity.
  The lattice grading decomposes $\Theta$ sectorwise, and the
  $E_1$-inversion principle
  \textup{(}Theorem~\textup{\ref{thm:e1-inversion-principle}}\textup{)}
  sends $\Theta_{V_\Lambda} \mapsto \Theta_{V_{\Lambda^*}}$
  \textup{(}dual lattice\textup{)}.
\end{enumerate}
\end{remark}

\begin{remark}[Two-channel decomposition of $\Theta$]
\label{rem:theta-two-channel}
\index{universal Maurer--Cartan class!two-channel decomposition}
The scalar $\kappa(\cA)$ decomposes into OPE channels
...
\]
At the level of $\Theta$, this decomposition is the split of the
genus-$1$ component into the \emph{curvature channel}
(the $k$-dependent piece from double-pole OPE data, proportional
to the Casimir $C_2 = 2h^\vee \cdot \mathrm{id}$) and the
\emph{central extension channel} (the $k$-independent piece from
simple-pole OPE data, equal to $\dim\mathfrak{g}/2$ for all
levels).

The curvature channel vanishes at critical level $k = -h^\vee$;
the central extension channel persists.  Only their sum $\kappa$
appears in $\Theta^{\min}$, but the chain-level representative
$\Theta$ carries both channels as separate coderivation
components of $\theta_1 \in
C^2_{\mathrm{cyc}}(\mathfrak{g},\mathfrak{g})$.
\end{remark}

\begin{remark}[Proof history for MC2]\label{rem:mc2-status}
\index{universal Maurer--Cartan class!proof history}
Theorem~\ref{thm:universal-theta} upgrades the scalar invariant
$\kappa(\cA)$ to a full homotopy-algebraic object organising the
entire genus tower.  The proof assembles three independently
established inputs; the history of their resolution is recorded
below.

\emph{Components of the proof.}
\begin{enumerate}[label=\textup{(\roman*)}]
...
\item The \emph{genus-$0$ cyclic deformation algebra}:
  Proposition~\ref{prop:genus0-cyclic-coderivation} proves that
  cyclic bar coderivations on $\bar{B}(\cA)$ form a cyclic dg~Lie
  algebra with $(-1)$-shifted symplectic pairing on the MC locus;
  Corollary~\ref{cor:km-cyclic-deformation} gives the explicit
  Kac--Moody specialization (adjoint bracket, level-$k$ Killing
  form, Casimir $C_2=2h^\vee\cdot\mathrm{id}$, $\kappa$ recovery).
  \textbf{MC2-1 resolved for KM:}
  Theorem~\ref{thm:mc2-1-km} identifies the coderivation differential
  $[d_{\barB},-]$ on $\Defcyc(\widehat{\mathfrak{g}}_k)$ with the
  cyclic CE~differential, giving
  $H^2(\Defcyc, l_1) \cong H^2_{\mathrm{cyc}}(\mathfrak{g},
  \mathfrak{g}) \cong \mathbb{C}$.
  Corollary~\ref{cor:km-minimal-linf} shows the homotopy-transferred
  minimal model is the Killing extension of
...
\emph{What remains.}
The formal genus-completed framework is now established
(Proposition~\ref{prop:genus-completed-mc-framework}): given a
cyclic $L_\infty$-algebra~$L$, the genus-convolution MC equation
converges and the obstruction theory is genus-stratified.  The
remaining gap is no longer convergence bookkeeping.  By
Proposition~\ref{prop:mc2-reduction-principle}, the live MC2 frontier
is concentrated in three exact packages: an intrinsic cyclic
$L_\infty$ model, a geometric completed tensor / clutching package, and
the one-channel normalization problem in the simple-Lie case.
Theorem~\ref{thm:mc2-conditional-completion} shows that these three
packages are sufficient: if they hold, then $\Theta_{\cA}$ exists
with the required trace, clutching, and Verdier properties.
...
\item \emph{Cyclic structure.}  Equip the bar coderivation
  ...
  \textbf{Resolved for Kac--Moody:}
  Theorem~\ref{thm:mc2-1-km} proves that
  $\Defcyc(\widehat{\mathfrak{g}}_k)$ is a cyclic $L_\infty$-algebra
  with $H^2 \cong \mathbb{C}$, via the identification of the
  coderivation differential with the cyclic CE~differential.

\item \emph{Geometric tensor / clutching package.}  Form the
  ...
  geometric coefficients.  A finite-dimensional genus-convolution
  surrogate is now implemented computationally.
  Proposition~\ref{prop:genus-completed-mc-framework} establishes the
  formal framework: genus convolution yields a well-defined
  $L_\infty$-algebra with convergent, genus-stratified MC equation.
  Proposition~\ref{prop:geometric-modular-operadic-mc} then
  resolves the three open parts---the intrinsic pro-nilpotent
  geometric realization, the modular-operadic composition laws,
  and the boundary/clutching compatibility---via the
  Getzler--Kapranov Feynman transform: the modular graph
  coefficient algebra $\Gmod$ provides the geometric coefficient
  system with clutching morphisms automatic from the dg~Lie
  structure.

\item \emph{One-channel normalization.}  In the simple-Lie case,
  Corollary~\ref{cor:one-dim-obstruction} reduces the genus-$g$
  extension problem to a single obstruction channel valued in the
  geometric coefficient space.  By
  Proposition~\ref{prop:tautological-line-support-criterion}, the
  support problem itself reduces to a precise geometric statement:
  the joint clutching restrictions and normalized trace must isolate
  the tautological line inside the relevant one-channel sector.

  Proposition~\ref{prop:one-channel-verdier-criterion} then reduces the
  Verdier/Koszul side to a precise involution/Lagrangian statement:
  construct the opposite one-channel line inside
  $Q_g(\cA^!)$ so that together with the $\cA$-line it forms a
  $\sigma$-stable Verdier-nondegenerate plane.

  Proposition~\ref{prop:one-channel-ptvv-criterion} then reduces the
  remaining H-level lift to a precise projector-level condition:
  construct perfect one-channel subcomplexes in the $\pm$-eigenspaces
  whose cross Verdier pairing is a quasi-isomorphism in the PTVV sense.

  Proposition~\ref{prop:one-channel-chain-model-criterion} reduces
  that projector-level condition further to explicit chain-model data
  on the MC2 one-channel sector of $\Defcyc(\cA)$:
  one perfect coderivation subcomplex on each side, one perfect
  geometric coefficient complex, and projector-compatible embeddings
  into the $\pm$ sectors.

  Proposition~\ref{prop:one-channel-seed-criterion} reduces that
  chain-model condition further to finite low-bar-length seed data
  inside
  $\operatorname{CoDer}^{\mathrm{cyc}}(\widehat{\barB}_X(\cA))[1]$ and
  $\operatorname{CoDer}^{\mathrm{cyc}}(\widehat{\barB}_X(\cA^!))[1]$:
  finitely many cyclic coderivations on each side whose
  differential-closed spans realize the one-channel sector and whose
  seed-level pairing is chain-level nondegenerate.

  Proposition~\ref{prop:one-channel-minimal-seed-packet-criterion}
  reduces that finite seed problem further to a minimal packet:
  one distinguished degree-$2$ cocycle on each side, finite
  correction packets in bar lengths at most~$3$, and one finite
  chain-level pairing matrix between those packets.

  Proposition~\ref{prop:one-channel-visible-lowarity-packet-criterion}
  then reduces that minimal packet further to the three visible
  low-arity ingredients already present on the MC2 compute surface:
  the simple-pole bracket sector, the normalized double-pole pairing
  matrix, and the Killing $l_3^\eta$ sector.
  Proposition~\ref{prop:one-channel-canonical-transfer-criterion}
  then reduces that visible packet further to a single canonical
  transfer package on the one-channel seed spaces: one cyclic seed,
  one shared generator-seed lift producing the Killing
  $l_3^\eta$ sector, and one functorial normalization splitting off the
  one-channel cocycle line.

  Proposition~\ref{prop:one-channel-transfer-law-criterion} then
  reduces that canonical package further to one explicit root-string
  transfer law on the one-channel seed spaces: a universal seed formula
  for $(O_{2,g},O_{3,g}^\eta)$ together with obstruction-side recovery
  of the seed coefficients and the functorial normalization extracting
  the distinguished simple-pole cocycle line.
  ...
  statement: the one-channel support graph must force an ordered
  root-string chart on the seed spaces, unique up to rescaling of the
  three seed lines.

  Proposition~\ref{prop:one-channel-intrinsic-line-detection-criterion}
  then reduces that chart statement further to an intrinsic
  line-detection problem: the seed, simple-pole, and Killing support
  lines must be canonically picked out by
  ...
  then reduces that intrinsic line-detection problem further to an
  automorphism-rigidity statement: those support lines must be the
  unique invariant lines for the one-channel support automorphism group
  compatible with the bracket, pairing, normalization, and obstruction
  profile.

  Proposition~\ref{prop:one-channel-support-graph-stabilizer-criterion}
  then reduces that rigidity statement further to a finite
  support-graph stabilizer computation: the labeled one-channel support
  graph must have exactly the fixed vertices corresponding to the
  simple-pole, residual-kernel, and Killing support lines.

  Proposition~\ref{prop:one-channel-incidence-orbit-criterion}
  then reduces that stabilizer computation further to an explicit
  root-string incidence-matrix / orbit-count criterion.

  Concretely, the visible one-channel permutation group is determined by
  the simple-pole incidence table, double-pole pairing matrix,
  normalization vector, and genus-$2$/genus-$3$ support indicators.
  It must have exactly the required singleton orbits.
  Proposition~\ref{prop:one-channel-visible-orbit-table-criterion}
  then reduces that bounded permutation computation further to a
  universal three-case orbit table on the visible root-string types:
  it is enough to check the orbit data for
  $m=1,2,3$, equivalently
  $\mathfrak{sl}_3,\mathfrak{sp}_4,\mathfrak{g}_2$.

  Proposition~\ref{prop:one-channel-canonical-universal-orbit-table-criterion}
  then collapses that three-case verification to one canonical
  universal orbit table: each visible root-string type must identify,
  by its labeled incidence/support data, with one fixed table
  independent of~$m$.
  Proposition~\ref{prop:one-channel-universal-invariant-signature-criterion}
  then collapses that fixed-table identification further to the
  minimal invariant-signature packet: the table must already be forced
  by the minimal visible bracket/pairing/normalization/support data on
  the universal seed triple.

  Proposition~\ref{prop:one-channel-signed-seed-character-criterion}
  then collapses that invariant-signature package further to the
  universal signed seed-character law on the ordered seed triple and
  its dual: the full visible invariant packet must be recoverable from
  that character alone.
  Proposition~\ref{prop:one-channel-two-sign-plus-normalization-scalar-criterion}
  then collapses that signed seed-character law further to one reduced
  root-string datum of two signs and one normalization scalar: the
  full signed seed-character must be recoverable from that smaller
  datum alone.
  ...
  sign and one normalization scalar: the genus-$3$ / Killing sign must
  already be forced by the bracket sign together with the fixed
  transfer law.
  Proposition~\ref{prop:one-channel-parity-forcing-criterion}
  then collapses that parity-sign datum further to one
  chart-normalized seed scalar: the bracket-parity sign must already be
  forced by the root-string chart criterion together with the chosen
  normalization convention.

  Proposition~\ref{prop:one-channel-normalization-criterion} finally
  shows that once this support, chart-normalized seed scalar, Verdier,
  and H-level data are in hand, the normalization is fixed by one
  scalar comparison with the already-proved shadow
  $\kappa(\cA)\lambda_g$.

  What remains is therefore to
  prove that clutching/trace isolation statement, construct the
  corresponding one-channel Verdier plane, lift it to a PTVV
  Lagrangian pair compatible with the anti-involution, realize that
  lift by explicit one-channel coderivation subcomplexes in
  $\Defcyc(\cA)$, construct those subcomplexes from finite
  low-bar-length bar-coderivation seeds, compress those seeds to a
  minimal one-cocycle-plus-corrections packet in bar lengths
  at most~$3$, reduce that packet to the visible low-arity
  simple-pole / double-pole / Killing-$l_3^\eta$ data, reduce that
  visible packet further to the canonical transfer package on the
  one-channel seed spaces, and reduce that package further to the
  explicit root-string transfer law on those seed spaces.

  It then remains to
  force the corresponding ordered root-string chart up to seed-line
  rescaling, detect those
  support lines intrinsically, prove that they are the unique invariant
  lines for the one-channel support automorphism group, compute the
  finite stabilizer of the resulting one-channel support graph, reduce
  that to the explicit singleton-orbit computation on the visible
  root-string incidence data, reduce that further to the universal
  three-case orbit table for
  $m=1,2,3$, collapse that further to one canonical universal orbit
  table, reduce that further to one minimal invariant signature
  package, reduce that further to one universal signed seed-character
  law, reduce that further to one universal two-sign plus
  normalization-scalar law, reduce that further to one root-string
  parity sign plus normalization scalar (the parity-sign plus
  normalization-scalar criterion), reduce that further to one
  chart-normalized seed scalar, and then verify one normalized
  scalar comparison.
\end{enumerate}

The clutching part of Step~2 is now resolved
(Proposition~\ref{prop:geometric-modular-operadic-mc}):
the Feynman transform structure on $\Gmod$ ensures that any MC
element $\Theta_{\cA} \in \mathrm{MC}(\Defcyc(\cA) \widehat{\otimes}
\Gmod)$ automatically decomposes via the clutching map at each boundary
divisor $\delta_h \subset \overline{\mathcal{M}}_g$
...
adapted to chiral algebras.  The explicit graph-sum construction is
developed in \S\ref{subsec:chiral-graph-complex}
(Theorem~\ref{thm:cyclic-linf-graph}): the $L_\infty$ brackets $l_n$
on $\Defcyc(\cA)$ are defined as sums over chiral ribbon graphs with
amplitudes given by configuration space integrals, conditional on the
Stokes identity on FM compactifications and the Fay trisecant identity
for the Szeg\H{o} kernel.  At genus~$0$, both hypotheses are
unconditional, recovering the dg Lie structure of
Proposition~\ref{prop:genus0-cyclic-coderivation} and the Killing
$l_3$ of Proposition~\ref{prop:killing-linf-extension}.
...
Hypothesis~\ref{mc2-hyp:completion} \textup{(MC2-2)} conditional
on the existence of $\Defcyc(\cA)$ as a complete cyclic
$L_\infty$-algebra \textup{(}which is the content of
Hypothesis~\textup{\ref{mc2-hyp:cyclic}}, i.e.\ MC2-1\textup{)}.
The key mechanism is the Getzler--Kapranov Feynman transform:
clutching compatibility is not an additional constraint on an MC
element in $\Defcyc(\cA) \widehat{\otimes} \Gmod$---it is
\emph{built into} the dg~Lie structure of $\Gmod$.
Thus, at the time this reduction was established, the three MC2
hypotheses reduced to two: the cyclic $L_\infty$-algebra
construction \textup{(MC2-1)} and the tautological-line support
\textup{(MC2-3)}.  Both are now resolved by
...
\begin{enumerate}[label=\textup{(\arabic*)}]
\item construct the intrinsic cyclic $L_\infty$ model
  $\Defcyc(\cA)$ on bar coderivations;
\item realize the geometric completed tensor / modular-operadic
  clutching package over $\overline{\mathcal{M}}_{g,\bullet}$;
\item identify the surviving one-channel genus-$g$ obstruction with the
  tautological line by a joint clutching/trace isolation statement,
  construct the opposite one-channel Verdier/Koszul line so that both
  sides form a $\sigma$-stable Lagrangian plane, lift that plane to a
  one-channel PTVV / anti-involution package, realize that package by
  explicit one-channel chain models in $\Defcyc(\cA)$, reduce those
  chain models to finite low-bar-length bar-coderivation seeds,
  compress those seeds to a minimal one-cocycle-plus-corrections packet
  in bar lengths at most~$3$, reduce that packet to the visible
  low-arity simple-pole / double-pole / Killing-$l_3^\eta$ data, reduce
  that visible packet to the canonical transfer package, the explicit
  root-string transfer law, the root-string chart forced up to
  seed-line rescaling, intrinsic detection of the one-channel
  support lines, automorphism-rigidity of the one-channel support
  graph, a finite support-graph stabilizer computation, an explicit
  incidence-matrix / orbit-count criterion, a universal visible
  root-string orbit table on the three concrete types, and then perform
  one normalized scalar comparison in the simple-Lie case
  \textup{(}Propositions~\textup{\ref{prop:tautological-line-support-criterion}}
  \textup{\ref{prop:one-channel-verdier-criterion}},
  \textup{\ref{prop:one-channel-ptvv-criterion}},
  \textup{\ref{prop:one-channel-chain-model-criterion}}, and
  \textup{\ref{prop:one-channel-seed-criterion}},
  \textup{\ref{prop:one-channel-minimal-seed-packet-criterion}},
  \textup{\ref{prop:one-channel-visible-lowarity-packet-criterion}},
  \textup{\ref{prop:one-channel-canonical-transfer-criterion}},
  \textup{\ref{prop:one-channel-transfer-law-criterion}},
  \textup{\ref{prop:one-channel-root-string-chart-criterion}},
  \textup{\ref{prop:one-channel-intrinsic-line-detection-criterion}},
  \textup{\ref{prop:one-channel-automorphism-rigidity-criterion}}, and
  \textup{\ref{prop:one-channel-support-graph-stabilizer-criterion}},
  \textup{\ref{prop:one-channel-incidence-orbit-criterion}},
  \textup{\ref{prop:one-channel-visible-orbit-table-criterion}},
  \textup{\ref{prop:one-channel-canonical-universal-orbit-table-criterion}}, and
  \textup{\ref{prop:one-channel-universal-invariant-signature-criterion}}, and
  \textup{\ref{prop:one-channel-signed-seed-character-criterion}}, and
  \textup{\ref{prop:one-channel-two-sign-plus-normalization-scalar-criterion}}, and
  \textup{\ref{prop:one-channel-parity-sign-plus-normalization-scalar-criterion}}, and
  \textup{\ref{prop:one-channel-parity-forcing-criterion}}, and
  \textup{\ref{prop:one-channel-normalization-criterion}}\textup{)}.
\end{enumerate}
This is strictly sharper than the older slogan ``construct
$\Theta_{\cA}$'': convergence, genus bookkeeping, and one-channel
obstruction reduction are already formal once the first two packages
exist, and the last package is itself reduced to a geometric
clutching/trace test, a one-channel Verdier plane, a one-channel PTVV
/ anti-involution lift, an explicit one-channel chain-model
construction, a finite low-bar-length bar-coderivation seed problem, a
minimal one-cocycle-plus-corrections packet in bar lengths at
most~$3$, the visible low-arity simple-pole / double-pole /
Killing-$l_3^\eta$ packet, a canonical transfer package on the
one-channel seed spaces consisting of one cyclic seed, one shared
generator-seed lift, and one functorial normalization of the
simple-pole bracket images, an explicit root-string transfer law on
those seed spaces, a root-string chart forced up to seed-line
rescaling, intrinsic detection of the one-channel support lines,
automorphism-rigidity of that one-channel support graph, a finite
support-graph stabilizer computation, an explicit incidence-matrix /
orbit-count criterion, a universal visible root-string orbit table on
the three concrete types, one canonical universal orbit table
identifying those three cases, one minimal invariant signature packet
  forcing that table, one universal signed seed-character law
  recovering that packet, one universal two-sign plus
  normalization-scalar law recovering that character, one root-string
  parity sign plus normalization scalar (the parity-sign plus
  normalization-scalar criterion) recovering that reduced datum, one
  chart-normalized seed scalar forcing that parity datum, and one
  scalar check.
\end{remark}
...
The scalar invariant $\kappa(\cA)$ is the first
characteristic number of $\Theta_{\cA}$
(Theorem~\ref{thm:universal-theta},
Definition~\ref{def:full-modular-package}): the genus-$1$ shadow
of the non-scalar structure that organizes the entire genus tower.
While Theorem~D$_{\mathrm{scal}}$ establishes the scalar package
independently, the full universal class $\Theta_{\cA}$ is now
constructed by Theorem~\ref{thm:mc2-full-resolution},
confirming that $\kappa$ is indeed the scalar trace of a
homotopy-algebraic object.
...
Corollary~\ref{cor:scalar-saturation} may appear surprising from the
perspective of the invariant ring.  For $\mathfrak{g} =
\mathfrak{sl}_r$ of rank~$r$, the invariant ring
$S^*(\mathfrak{g})^{\mathfrak{g}}$ has $r$ independent generators
(Casimirs of degrees $2, 3, \ldots, r+1$), of which only the
quadratic Casimir~$C_2$ has nonzero trace.  One might expect the
higher Casimirs ($C_3, C_4, \ldots$) to contribute non-scalar
content to~$\Theta_{\cA}$.  This does not occur, for a structural
reason:
\begin{itemize}
\item The MC element lives in degree~$2$ of $\Defcyc(\cA)$.
\item The cyclic cohomology
  $H^2_{\mathrm{cyc}}(\mathfrak{g},\mathfrak{g}) \cong
  H^3(\mathfrak{g}) \cong \mathbb{C}$ is one-dimensional for every
  simple~$\mathfrak{g}$, generated by the Killing $3$-form.
\item The higher Casimirs contribute to
  $H^{2k}_{\mathrm{cyc}}$ for $k \geq 2$
  (e.g., the cubic Casimir of $\mathfrak{sl}_3$ gives
  $H^4_{\mathrm{cyc}} \cong H^5(\mathfrak{sl}_3) \cong \mathbb{C}$),
  but these lie in the wrong degree for the MC equation.
\end{itemize}
The non-scalar characteristic data in the full package
$\mathcal{C}_{\cA} = (\Theta_{\cA}, \kappa, \Delta_{\cA},
\Pi_{\cA}, \mathcal{H}_{\cA})$ is therefore entirely concentrated
in~$\Delta_{\cA}$ (the spectral discriminant, which encodes the
algebraic structure of the bar generating function beyond~$\kappa$)
and~$\Pi_{\cA}$ (the periodicity profile).  The MC
element~$\Theta_{\cA}$, despite being the most
homotopy-theoretically complete object in the package, carries the
\emph{least} refined information: it is fully determined
by~$\kappa(\cA)$.
\end{remark}
...
\begin{proposition}[Scalar saturation for $\mathcal{W}$-algebras;
\ClaimStatusProvedHere]
...
Proposition~\ref{prop:ds-package-functoriality} settles three of
the five package components under DS reduction.  The remaining
items:
\begin{itemize}
\item \emph{Periodicity profile $\Pi_{\cA}$.}
  The $T$-matrix eigenvalues (reciprocals of the $\Delta$-roots)
  are DS-invariant by~(ii).  The sharp modular and geometric periods
  (Future~7) have not been computed explicitly for any
  $\mathcal{W}$-algebra, so their DS transformation is open.
\item \emph{Lattice extensions.}
  Under $\cA \to \cA \otimes V_\Lambda$, additivity gives
  $\kappa(\cA \otimes V_\Lambda)
  = \kappa(\cA) + \operatorname{rank}(\Lambda)$
  (Corollary~\ref{cor:kappa-additivity}).  The discriminant
  $\Delta_{\cA \otimes V_\Lambda}$ involves a product of the
  individual discriminants, but the precise formula for the
  lattice contribution has not been computed.
\end{itemize}
\end{remark}
...
The preceding results establish scalar saturation for a broad class:
\begin{enumerate}[label=\textup{(\roman*)}]
\item Kac--Moody algebras $\widehat{\mathfrak{g}}_k$ with
  simple~$\mathfrak{g}$
  (Corollary~\ref{cor:scalar-saturation});
\item all principal $\mathcal{W}$-algebras
  (Proposition~\ref{prop:w-algebra-scalar-saturation});
\item all non-principal $\mathcal{W}$-algebras
  $\mathcal{W}^k(\mathfrak{g},f)$
  (Proposition~\ref{prop:nonprincipal-scalar-saturation});
\item semi-simple products: for
  $\cA = \cA_1 \otimes \cdots \otimes \cA_r$ with each $\cA_i$ in
  class~(i)--(iii), $\Theta = \sum_i \kappa_i \cdot \eta_i \otimes
  \Lambda$ with independent scalars $\kappa_i$
  (the MC equation decouples).
\end{enumerate}
The structural reason is uniform: every algebra in classes
(i)--(iii) has a one-dimensional deformation space (parameterized
by the level~$k$ or central charge~$c$), giving
$\dim H^2_{\mathrm{cyc}} = 1$.  Class~(iv) is multi-scalar
(determined by a tuple $(\kappa_1, \ldots, \kappa_r)$) but
decoupled.

\emph{Diagonal cosets.}
The GKO coset
$\mathcal{C} = (\widehat{\mathfrak{g}}_{k_1} \otimes
\widehat{\mathfrak{g}}_{k_2}) / \widehat{\mathfrak{g}}_{k_1+k_2}$
depends on two parameters $(k_1, k_2)$, but when the coset algebra
is identified with a $\mathcal{W}$-algebra
(as in Arakawa--Creutzig--Linshaw-type results), it is a
one-parameter family and scalar saturation holds.

\emph{The genuine frontier: coupled multi-parameter families.}
Genuinely non-scalar $\Theta$ --- where the MC equation couples
multiple deformation directions --- requires a vertex algebra
with $\dim H^2_{\mathrm{cyc}} \geq 2$ that is \emph{not} a
product.  No such example is known among vertex algebras of
Lie-theoretic origin on curves; the one-dimensionality of the
deformation space is a structural feature of the Lie-theoretic
construction, not an artefact of the proof method.

We now analyze the three candidate classes in detail.

\emph{Candidate~1: Cosets of different simple factors.}
Consider
$\cA = \mathrm{Com}(\widehat{\mathfrak{h}}_{k_3},\;
\widehat{\mathfrak{g}}_{k_1} \otimes
\widehat{\mathfrak{g}'}_{k_2})$
where $\mathfrak{h} \hookrightarrow \mathfrak{g} \oplus
\mathfrak{g}'$ is the diagonal embedding of a common simple
subalgebra, so that $k_3 = k_1 + k_2$ (current conservation).
A~priori this is a two-parameter family in $(k_1, k_2)$.
However, the conformal embedding constraint forces the
Sugawara energy-momentum tensor to satisfy
$T_{\cA} = T_{\mathfrak{g},k_1} + T_{\mathfrak{g}',k_2}
- T_{\mathfrak{h},k_1+k_2}$,
and the central charge is
$c(\cA) = c(\mathfrak{g},k_1) + c(\mathfrak{g}',k_2)
- c(\mathfrak{h},k_1+k_2)$.
When $\mathfrak{g} = \mathfrak{g}' = \mathfrak{h}$
(the GKO case), the coset $\cA$ is identified with
a $\mathcal{W}$-algebra by the Arakawa--Creutzig--Linshaw
programme~\cite{ACL19}, reducing to a one-parameter family.
For $\mathfrak{g} \neq \mathfrak{g}'$ the situation is more
delicate: the OPE of the coset depends on both $k_1$ and
$k_2$, but the \emph{isomorphism type} (as a vertex algebra
with its conformal vector) is typically constrained to a
one-parameter locus.
The reason is structural: the embedding
$\mathfrak{h} \hookrightarrow \mathfrak{g} \oplus \mathfrak{g}'$
and the coset condition $a_{(n)} v = 0$ for all
$a \in \widehat{\mathfrak{h}}$ impose relations among
OPE coefficients that remove one degree of freedom.
In all computed examples
($\mathfrak{g} = \mathfrak{sl}_2$,
$\mathfrak{g}' = \mathfrak{sl}_3$,
$\mathfrak{h} = \mathfrak{sl}_2$),
the resulting coset is a $\mathcal{W}$-algebra in a
single effective parameter, and scalar saturation holds.

We record the precise obstruction: a genuinely two-parameter
coset would require $\dim H^2_{\mathrm{cyc}}(\cA, \cA) \geq 2$,
which means the coset must admit a continuous deformation that
is \emph{not} induced by varying either ambient level alone.
No such deformation is known.

The reduction is visible at the level of central charges.
For the GKO coset ($\mathfrak{g} = \mathfrak{g}' = \mathfrak{h}$),
the Jacobian $\partial c_{\cA}/\partial(k_1, k_2)$ has
\[
...
\qquad i = 1, 2,
\]
which has rank~$1$ at generic $(k_1, k_2)$ (both partials are
generically nonzero and proportional).
The level set $\{(k_1', k_2') : c_{\cA}(k_1', k_2') = c_0\}$
is a one-dimensional curve.  Combined with the
Arakawa--Creutzig--Linshaw identification of the coset
with a $\mathcal{W}$-algebra (whose isomorphism type,
for fixed $\mathfrak{g}$ and nilpotent~$f$, is determined
by the level), the VOA varies along a one-parameter locus.  For non-GKO embeddings
$\mathfrak{h} \hookrightarrow \mathfrak{g} \oplus \mathfrak{g}'$
with Dynkin indices $j_1, j_2$, the Jacobian is
$\partial c/\partial k_1
= \dim\mathfrak{g}\cdot h^\vee_{\mathfrak{g}} / (k_1 + h^\vee_{\mathfrak{g}})^2
- j_1 \dim\mathfrak{h} \cdot h^\vee_{\mathfrak{h}} / (k_3 + h^\vee_{\mathfrak{h}})^2$
(with $k_3 = j_1 k_1 + j_2 k_2$), and remains rank~$1$ in all
verified instances.

\emph{Candidate~2: VOAs from 4D $\mathcal{N}=2$ gauge theories.}
The Beem--Lemos--Liendo--Peelaers--Rastelli--van~Rees construction
(Construction~\ref{constr:4d2d}) produces a vertex algebra
$\cA[\mathcal{T}_{4d}]$ from each 4D $\mathcal{N}=2$ SCFT.
For class~$\mathcal{S}$ theories of type $A_{n-1}$ on a
Riemann surface~$C$, the result is $\mathcal{W}^{-n}(\mathfrak{sl}_n)$
(Example~\ref{ex:class-s}).  Each such VOA is at a \emph{fixed}
level $k = -n$, and is therefore completely rigid (no continuous
deformation parameters).  Varying $n$ produces a discrete
family of distinct $\mathcal{W}$-algebras.

The most promising candidate for multi-parameter structure
comes from \emph{$\mathcal{N}=2$ quiver gauge theories} with
$r \geq 2$ gauge nodes, each carrying an independent gauge
coupling $\tau_i$.  These theories have a conformal manifold
$\mathcal{M}_{\mathrm{conf}}$ of real dimension $\geq 2r$.
However, the 4d/2d construction is subtle:
\begin{enumerate}[label=\textup{(\alph*)}]
\item The vertex algebra $\cA[\mathcal{T}]$ is determined
  by the \emph{SCFT data} (R-symmetry representations and
  OPE coefficients of Schur operators), which may be
  independent of the exactly marginal couplings $\tau_i$.
  In all computed examples
  (Beem--Rastelli~\cite{beem-et-al}; Bonetti--Meneghelli--Rastelli~\cite{bonetti-meneghelli-rastelli}),
  the vertex algebra structure (OPE
  coefficients as rational functions) depends on at most one
  continuous parameter.
\item For $SU(N)$ SQCD with $N_f = 2N$ flavors, the conformal
  manifold is one-dimensional ($\tau$ alone), and the
  vertex algebra is $\widehat{\mathfrak{sl}}_{N,-N/2}
  \otimes \mathrm{bc}$-system, a single-parameter family.
\item For circular quivers (e.g., the $\widehat{A}_1$ necklace
  with two $SU(N)$ nodes and bifundamental matter), the
  conformal manifold is two-dimensional (two gauge couplings),
  but S-duality identifications and the structure of the
  Schur index suggest that the vertex algebra depends on
  a single effective parameter.
\end{enumerate}
The structural explanation is that exactly marginal couplings
deform conformal blocks and correlation functions but may leave
the \emph{OPE algebra} (the vertex algebra structure constants)
invariant --- a familiar phenomenon in $\mathcal{N}=2$ theories
where the conformal manifold acts trivially on the chiral ring.
Whether this rigidity extends to all Schur operators is
an open problem in 4D physics.

\emph{Candidate~3: Admissible levels and extensions.}
The Whitehead--Kazhdan--Lusztig argument
(Theorem~\ref{thm:cyclic-rigidity-generic}) fails at admissible
levels because the module category is non-semisimple.
A careful analysis of this frontier reveals a three-stratum
structure:
\begin{itemize}
\item \emph{Pure Kac--Moody} at admissible levels:
  $H^2_{\mathrm{cyc,prim}} = 0$ \emph{trivially}, because
  the only strong generators are currents~$J^a$ and there are
  no primary--primary pairs.  The Whitehead decomposition
  (Stage~1 alone) gives $\dim H^2_{\mathrm{cyc}} = 1$ at
  \emph{all} non-critical levels, without invoking
  KL~semisimplicity.

\item \emph{$\mathcal{W}$-algebras}
  $\mathcal{W}^k(\mathfrak{g}, f)$ at admissible levels:
  scalar saturation is proved at \emph{generic} levels
  (Proposition~\ref{prop:nonprincipal-scalar-saturation})
  and expected at admissible levels.
  The quantum DS~reduction functor produces a unique vertex
  algebra structure at every $k \neq -h^\vee$; specializing
  to admissible~$k$ yields unique OPE coefficients
  provided the BRST complex has no extra cohomology.
  No counterexample is known, and the deformation space
  is one-dimensional at all tested levels.

\item \emph{Non-DS constructions} at admissible levels:
  these are the genuine frontier.  Such algebras have primary
  strong generators beyond the currents and are not
  produced by DS reduction.  Neither the trivial
  Kac--Moody argument nor the BRST argument applies.
  Examples include the triplet algebras~$\mathcal{W}(p)$
  (screening-kernel constructions on lattice VOAs,
  logarithmic and $C_2$-cofinite) and
  certain extended affine VOAs.
  In all known instances, the continuous deformation space
  is at most one-dimensional, but no uniform proof exists.
\end{itemize}
Thus the frontier of
Conjecture~\ref{conj:scalar-saturation-universality} is not
``admissible levels'' per se, but rather the specific class of
vertex algebras at admissible levels whose construction is
not governed by the DS functor (screening-kernel algebras,
extended affine VOAs, and related logarithmic structures).

The absence of genuinely non-scalar examples across all three
candidate classes is strong evidence for a structural principle:
\textbf{for all vertex algebras of Lie-theoretic origin, the
universal MC element carries no information beyond the scalar
parameters.  The non-scalar characteristic data of the modular
Koszul object resides entirely in $\Delta_{\cA}$ and
$\Pi_{\cA}$.}
\end{remark}
...
(i)~The Whitehead
decomposition~\eqref{eq:whitehead-decomposition}
holds at \emph{all} non-critical levels, including admissible
levels and integer levels.
At admissible levels, the module category is
non-semisimple (it is a non-semisimple modular tensor
category), so Stage~2 does not apply, and
$H^2_{\mathrm{cyc,prim}}$ is potentially non-zero.
At positive integer levels, the category of integrable
$L_k(\fg)$-modules is a semisimple modular tensor
category (the WZW fusion category), so condition~(c)
holds for any~$\cA$ decomposing into integrable modules.

(ii)~Together with
Corollary~\textup{\ref{cor:effective-quadruple}}, the
theorem establishes the effective quadruple
$\mathcal{C}^{\mathrm{eff}}_{\cA}
= (\kappa, \Delta, \Pi, \mathcal{H})$
for the entire standard Lie-theoretic landscape at
generic level, \emph{without case-by-case verification}:
the Whitehead--Kazhdan--Lusztig argument is uniform.
The case-by-case proofs
(Propositions~\ref{prop:saturation-equivalence}
and~\ref{prop:saturation-functorial}) give complementary
information---in particular, the BRST pullback for
$\mathcal{W}$-algebras covers all levels, not just
generic ones.

(iii)~The decomposition~\eqref{eq:whitehead-decomposition}
reduces the universality conjecture to a single question:
\emph{does $H^2_{\mathrm{cyc,prim}}(\cA) = 0$ persist
at admissible levels?}
At admissible levels, the non-semisimplicity of the
module category is the source of logarithmic
phenomena (indecomposable but non-simple modules).
The conjecture predicts that this non-semisimplicity
does not create new cyclic deformations.
\end{remark}
...
\smallskip\noindent
\emph{(I) Proved instances.}
The conjecture is proved at generic level for \emph{all}
vertex algebras with simple symmetry
(Theorem~\ref{thm:cyclic-rigidity-generic}), and at
\emph{all} levels for the standard Lie-theoretic landscape
(Corollary~\ref{cor:effective-quadruple}):
\begin{enumerate}[label=\textup{(\arabic*)}]
\item Kac--Moody algebras $\widehat{\mathfrak{g}}_k$ with
  simple~$\mathfrak{g}$, arbitrary rank
  (Corollary~\ref{cor:scalar-saturation});
\item principal $\mathcal{W}$-algebras
  $\mathcal{W}_N^k$
  (Proposition~\ref{prop:w-algebra-scalar-saturation});
\item non-principal $\mathcal{W}$-algebras
  $\mathcal{W}^k(\mathfrak{g}, f)$ for any orbit~$f$
  (Proposition~\ref{prop:nonprincipal-scalar-saturation});
\item diagonal cosets identified with $\mathcal{W}$-algebras
  via Arakawa--Creutzig--Linshaw~\cite{ACL19};
\item semi-simple products and tensor products
  (Proposition~\ref{prop:saturation-functorial}(c)).
\end{enumerate}
The saturation equivalence
(Proposition~\ref{prop:saturation-equivalence}) provides
the testable criterion: for any candidate algebra, one
need only verify $\dim H^2_{\mathrm{cyc}} = 1$.

\smallskip\noindent
\emph{(II) The critical test: 4D $\mathcal{N}=2$ quiver VOAs.}
The sharpest potential counterexample arises from
$\mathcal{N}=2$ superconformal field theories with
multi-dimensional conformal manifolds.
Consider a circular quiver with $r \geq 2$ gauge nodes
(e.g., the $\widehat{A}_{r-1}$ necklace quiver with
gauge group $\prod_i SU(N_i)$ and bifundamental matter),
giving $\dim_{\mathbb{C}}\mathcal{M}_{\mathrm{conf}} \geq r$.
The 4d/2d construction
(Construction~\ref{constr:4d2d}) produces a vertex algebra
$\cA[\mathcal{T}]$ that \emph{a priori} could depend on
all~$r$ couplings $\tau_i$.

The conjecture reduces to a precise 4D physics question:
\emph{does the Schur sector OPE algebra
$\cA[\mathcal{T}(\tau_1, \ldots, \tau_r)]$ vary
non-trivially over the conformal manifold?}
In all computed examples ($SU(N)$ SQCD with $N_f = 2N$,
the $\widehat{A}_1$ necklace, class~$\mathcal{S}$ of
type~$A_{n-1}$), the answer is no: the OPE is rigid
and the vertex algebra depends on at most one effective
parameter.

\smallskip\noindent
\emph{(III) Consequences for the characteristic hierarchy.}
The proved quadruple reduction
(Corollary~\ref{cor:effective-quadruple})
already establishes the three-level hierarchy for the
standard landscape:
\begin{enumerate}[label=\textup{(L\arabic*)}]
\item \emph{Scalar logarithmic}:
  $\kappa(\cA) \in \mathbb{C}$ --- the unique scalar invariant
  governing $\Theta_{\cA}$, not merely a trace-level shadow;
\item \emph{Spectral logarithmic}:
  $\Delta_{\cA}(x) = \prod_i (1 - \lambda_i x)$ --- the
  bar generating function discriminant;
\item \emph{Exponential}:
  $\Pi_{\cA} = (M_{\cA}, Q_{\cA}, G_{\cA})$ --- the
  periodicity profile.
\end{enumerate}
In the Chern--Weil analogy
(Remark~\ref{rem:kappa-first-chern}), $\kappa$ is promoted
from a first Chern number (a shadow of the connection) to
the connection itself: the determinant line bundle
$\mathcal{L}_{\cA} = \det R\pi_{g*} \bar{B}^{(g)}(\cA)$
has $c_1 = \kappa \cdot \lambda$, and the connection is
flat in the fiber directions.

\smallskip\noindent
\emph{(IV) Relation to Theorem~D.}
Theorem~D (Theorem~\ref{thm:modular-characteristic})
establishes that $\kappa(\cA)$ is universal, additive,
anti-symmetric, and has $\hat{A}$-genus generating function.
The quadruple reduction strengthens this: $\kappa$ not only
\emph{exists} as an invariant but \emph{determines}
$\Theta_{\cA}$ completely.  For the standard landscape this
is proved; the conjecture asserts this determination is
universal.
\end{remark}
...
Definition~\ref{def:full-modular-package} is now fully constructed
(Theorem~\ref{thm:universal-MC}) and is
scalar-saturated whenever $\dim H^2_{\mathrm{cyc}}(\cA,\cA) = 1$
(Corollary~\ref{cor:scalar-saturation}): at the
minimal-model level, $\Theta_{\cA}^{\min} = \kappa(\cA) \cdot
\eta \otimes \Lambda$.  The Chern--Weil theory is complete: the
determinant line bundle $\mathcal{L}_{\cA}$, its first Chern class
$\kappa \cdot \lambda$, and the GRR genus expansion exhaust the
information in~$\Theta_{\cA}$.
\end{remark}
...
The full modular characteristic package
$\mathcal{C}_{\cA} = (\Theta, \kappa, \Delta, \Pi, \mathcal{H})$
(Definition~\ref{def:full-modular-package}) draws on two
geometrically distinct sources of variation, which explain both
the scalar saturation of~$\Theta_{\cA}$ and the non-scalar content
of~$\Delta_{\cA}$ and~$\Pi_{\cA}$.

\smallskip\noindent
\emph{(a) The virtual bar family.}
The derived pushforward of the genus-$g$ bar complex defines a
...
\end{equation}
Its determinant line
$\mathcal{L}_{\cA} = \det \mathcal{V}_{\cA}$
carries $c_1(\mathcal{L}_{\cA}) = \kappa(\cA) \cdot \lambda$
(Remark~\ref{rem:kappa-first-chern}).  The MC
class~$\Theta_{\cA}$ is a section of a twist of this
determinant line: it lives in
$\operatorname{MC}(\Defcyc(\cA) \widehat{\otimes} \Gmod)$,
where the degree-$2$ deformation cohomology
$H^2_{\mathrm{cyc}}(\cA,\cA) = \mathbb{C}\!\cdot\!\eta$
is the fiber of a \emph{rank-one} object.  Scalar saturation
(Corollary~\ref{cor:scalar-saturation}) is the statement
that a rank-one object carries exactly one characteristic
number: its first Chern class~$\kappa$.

\smallskip\noindent
\emph{(b) Two directions of variation.}
The non-scalar invariants arise from a
different geometric source.
\begin{itemize}
\item \emph{Genus direction}
  (variation over~$\overline{\mathcal{M}}_g$):
  produces the MC class~$\Theta_{\cA}$, the obstruction
  classes~$o_g(\cA)$, and the genus expansion.
  All of this data is controlled by the determinant line
  $\mathcal{L}_{\cA} = \det\mathcal{V}_{\cA}$,
  which is rank~$1$.  Hence
  $\Theta = \kappa \cdot \eta \otimes \Lambda$
  (scalar-saturated).
\item \emph{Level direction}
  (variation of $k \in \mathbb{C} \setminus\{-h^\vee\}$):
  as the level varies, the bar differential deforms and
  the bar cohomology $H^*(\bar{B}(\cA_k))$ acquires a
  \emph{monodromy operator}~$T$ from analytic continuation
  around the critical level.
  The spectral discriminant
  $\Delta_{\cA}(x) = \det(1 - xT)$
  (Remark~\ref{rem:characteristic-hierarchy}(2))
  is the characteristic polynomial of this monodromy,
  encoding \emph{all} eigenvalues of~$T$---not merely
  their sum $\operatorname{tr}(T)$, which would give only the
  scalar invariant.
  The periodicity profile
  $\Pi_{\cA} = (M_{\cA}, Q_{\cA}, G_{\cA})$
  records the multiplicative structure of these eigenvalues
  (Remark~\ref{rem:periodicity-triple}).
\end{itemize}
The genus direction sees
$\det(\mathcal{V}_{\cA})$; the level direction
sees~$\mathcal{V}_{\cA}$ itself.

\smallskip\noindent
\emph{(c) The $K$-theoretic hierarchy.}
The three levels of the characteristic hierarchy
(Remark~\ref{rem:characteristic-hierarchy}) correspond to
three layers of $K$-theoretic data extracted
from~$\mathcal{V}_{\cA}$:
...
\end{tabular}
\end{center}
At level~L1, $\kappa = c_1(\det\mathcal{V})$ is the
\emph{trace} of the monodromy
($\operatorname{tr}(T) = \sum \lambda_i$).
At level~L2,
$\Delta(x) = \det(1-xT) = \sum_k (-1)^k e_k(T)\,x^k$
records the full set of
\emph{elementary symmetric functions} of the eigenvalues,
equivalently, all Chern classes of~$\mathcal{V}$.
At level~L3, the multiplicative orders and
Galois structure of the eigenvalues give~$\Pi$.

\smallskip\noindent
Scalar saturation asserts that L1 determines
the MC class~$\Theta$ completely.
The non-scalar content of the package---the spectral
discriminant~$\Delta$ and the periodicity
profile~$\Pi$---resides at L2 and~L3, encoded in the
higher-rank structure of~$\mathcal{V}_{\cA}$ that the
determinant line does not see.
A line bundle has one Chern class;
a virtual bundle of rank~$r$ has~$r$ independent classes.
The MC element is scalar \emph{because} it inhabits a
line; the package is non-scalar \emph{because}
the virtual bar family has higher rank.
\end{remark}
...
The identification $o_g = \kappa\,\lambda_g$ admits an independent
consistency check through the structure of the tautological ring
$R^*(\overline{\mathcal{M}}_g)$, without appealing to the explicit
bar-curvature formula.  The argument has three legs:

\smallskip\noindent
\emph{(a) Tautologicity.}
Since the Feynman transform
(Theorem~\ref{thm:prism-higher-genus}(ii)) integrates against
logarithmic forms on $\overline{\mathcal{M}}_{g,n}$ built from the
relative dualizing sheaf $\omega_{\pi_g}$, the resulting classes
lie in $R^*(\overline{\mathcal{M}}_g)$
(Mumford~\cite{Mumford83}).  Hence
$o_g \in R^*(\overline{\mathcal{M}}_g) \cap W_g$.

\smallskip\noindent
\emph{(b) Clutching determination.}
The modular-operadic clutching factorization
\eqref{eq:clutching-factorization} determines the restriction of
$o_g$ to every boundary stratum $\delta_h$ of
$\overline{\mathcal{M}}_g$ from lower-genus MC data.  By
induction (base case $g=1$: the Mumford relation $12\lambda =
\delta$ gives $R^1(\overline{\mathcal{M}}_{1,1}) \cong
\mathbb{Q}$, so $o_1 = \kappa\lambda_1$ is forced by the trace),
these restrictions match those of $\kappa\lambda_g$.

\smallskip\noindent
\emph{(c) Pixton separation.}
By Pixton's double-ramification-cycle
relations~\cite{Janda17,PPZ19}, the clutching restrictions together
with the integration pairing $\ell_g$ separate the one-channel
tautological classes: if $\alpha \in R^*(\overline{\mathcal{M}}_g)
\cap W_g$ satisfies $\ell_g(\alpha) = 0$ and $\mathrm{clut}_g(\alpha) = 0$, then $\alpha = 0$.  Combined with~(b) and the trace
$\ell_g(o_g) = \kappa$ (Theorem~\ref{thm:genus-universality}),
Proposition~\ref{prop:tautological-line-support-criterion} gives
$o_g = \kappa\lambda_g$.

\smallskip\noindent
This route provides a structural explanation for \emph{why}
tautological line support must hold: the modular-operadic
boundary structure of $\overline{\mathcal{M}}_g$ is rich enough
to pin down the one-channel class, and the tautological ring
(through Pixton's relations) is the algebraic vehicle that
certifies this determination.
\end{remark}
