...
\item \emph{Minimal-model formula.}
  ...
  \textup{(}Corollary~\textup{\ref{cor:km-minimal-linf}}\textup{)}.
...
\item \emph{MC equation is trivially satisfied.}
  ...
  The obstructions vanish at every arity:
  \begin{itemize}
  \item $l_2^{\mathrm{tr}}(\eta,\eta) = 0$
    \textup{(}$\eta$ is central:
    $[-,-]_{\mathfrak{g}}$ acts on $\mathfrak{g}^{\otimes 2}$,
    not on $\eta$\textup{)};
  \item $l_3^{\mathrm{tr}}(\eta,\eta,\eta) = 0$
    \textup{(}the Killing cocycle $\phi \colon
    \Lambda^3\mathfrak{g} \to \mathbb{C}\!\cdot\!\eta$ accepts
    only $\mathfrak{g}$-inputs\textup{)};
  \item $l_n^{\mathrm{tr}} = 0$ for $n \geq 4$.
  \end{itemize}
...
For every Koszul chiral algebra~$\cA$ with simple Lie
symmetry~$\mathfrak{g}$ of arbitrary rank, the universal MC element
...
$L_\infty$ model has cohomology
$\mathcal{H} \cong \mathfrak{g} \oplus
\mathbb{C}\!\cdot\!\eta$ with $\deg\eta = 2$.
The MC element has degree~$2$, where
$\mathcal{H}^2 = \mathbb{C}\!\cdot\!\eta$ is one-dimensional
--- reflecting the isomorphism
$H^2_{\mathrm{cyc}}(\mathfrak{g},\mathfrak{g})
\cong H^3(\mathfrak{g}) \cong \mathbb{C}$
for every simple~$\mathfrak{g}$
(Proposition~\ref{prop:cyclic-ce-identification}).
Thus $\Theta^{\min} = c \cdot \eta \otimes \Lambda$ for a unique
scalar~$c$, and the trace relation
...
\item \emph{\textup{(}Deformation rigidity\textup{)}}
  ...
  rescaling the level.
\end{enumerate}
\item All four conditions hold unconditionally for every
...
then verify that every simple $\mathfrak{g}$ satisfies~(ii).
...
because all its components do:
$l_2^{\mathrm{tr}}(\eta,\eta) = 0$
(the Lie bracket $[-,-]_{\mathfrak{g}}$ acts on
$\mathfrak{g}^{\otimes 2} \subset (\mathcal{H}^0)^{\otimes 2}$,
not on $(\mathcal{H}^2)^{\otimes 2}$);
$l_3^{\mathrm{tr}}(\eta,\eta,\eta) = 0$ (the Killing cocycle
$\phi \colon \Lambda^3\mathfrak{g} \to \mathbb{C}\!\cdot\!\eta$
accepts only $\mathfrak{g}$-inputs);
$l_n^{\mathrm{tr}} = 0$ for $n \geq 4$
...
\smallskip\noindent
\emph{Unconditional verification on the Koszul locus.}
For simple~$\mathfrak{g}$, condition~(ii) is
$\dim H^2_{\mathrm{cyc}}(\mathfrak{g},\mathfrak{g}) = 1$.
By the cyclic CE identification
(Proposition~\ref{prop:cyclic-ce-identification}),
$H^2_{\mathrm{cyc}}(\mathfrak{g},\mathfrak{g})
\cong H^3(\mathfrak{g})$.
For simple~$\mathfrak{g}$, $H^3(\mathfrak{g}) \cong
\mathbb{C}$ with generator the Killing $3$-form
$\phi(a,b,c) = \kappa([a,b],c)$ (Cartan).
Hence all four conditions hold unconditionally.
\end{proof}
...
scalar-saturated (Corollary~\ref{cor:scalar-saturation}): at the
