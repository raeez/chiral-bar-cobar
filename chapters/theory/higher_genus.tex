...
(Theorem~\ref{conj:universal-MC}) under the trace map; it is
...
On the associated graded, the curved corrections disappear because they
raise the filtration, so only the genus-$0$ collision differential
remains.  By Theorem~\ref{thm:geometric-equals-operadic-bar}, the
resulting complex is the operadic bar complex of
$\operatorname{gr}\mathcal{A}$.  Since $\mathcal{A}$ is Koszul, this
classical bar complex has cohomology concentrated in total degree~$0$
(cf.\ \cite[Theorem~3.4.4]{LV12}).  Therefore the spectral sequence for
$F_\bullet$ has
\[
...
$E_\infty^{p,q} = E_1^{p,q}$.  Hence the full fiber bar complex has no
cohomology outside degree~$0$, and Steps~1--3 identify the surviving
degree-$0$ piece with the center:
\begin{equation}
...
\underline{\mathbb{C}}.
\end{equation}
This is a statement about the total fiber complex, not merely its
bar-degree-$0$ component.
\end{proof}
...
(Theorem~\ref{conj:lagrangian-complementarity}).
\end{remark}
...
\textup{(}Theorem~\textup{\ref{conj:universal-MC}} for the
...
(Theorems~\ref{conj:universal-MC} and~\ref{conj:universal-theta}):
...
Theorem~\ref{conj:universal-MC}; MK8 applies only to $\Eone$-chiral
...
Proposition~\ref{thm:pbw-universal-conformal} isolates the common
...
This theorem does not use Proposition~\ref{thm:pbw-universal-conformal}
...
\begin{proposition}[Unique-weight-$2$ PBW criterion for chiral
  algebras with a stress tensor; \ClaimStatusProvedHere]
\label{thm:pbw-universal-conformal}
...
$E_\infty$ pages agree.  The proposition proves only the common
$E_3$-page vanishing mechanism.
\end{proof}
...
\begin{enumerate}[label=\textup{(\roman*)}]
\item
Proposition~\ref{thm:pbw-universal-conformal} packages
...
(Theorem~\ref{thm:pbw-allgenera-km})
is \emph{not} a special case, because the stress
tensor~$T$ in a Kac--Moody algebra is the Sugawara
construction (a composite field, not a generator);
the Kac--Moody proof requires the independent
Whitehead--Casimir--Killing mechanism of
Theorem~\ref{thm:pbw-allgenera-km}.

\item
The proposition applies formally to every non-principal
$\mathcal{W}$-algebra
...
generator, and hypotheses~(a)--(b) hold.  It should therefore be read
...
What still requires family-specific input is genus-$0$ Koszulness
\textup{(MK1)}, Verdier \textup{(MK2)}, and the exclusion of later
differentials and extension classes beyond the common $E_3$-page
vanishing.
...
finite-dimensional).  In this regime the proposition records only the
...
Kodaira--Spencer constructions.  Proposition~\ref{thm:pbw-universal-conformal}
records the common unique-weight-$2$ $d_2$ mechanism behind the
Virasoro and principal finite-type $\mathcal{W}$ proofs, but it is not
itself the source of unconditional status beyond those family theorems.
\end{enumerate}
\end{remark}
...
(Theorems~\ref{conj:universal-MC}
...
\item {\tolerance=9999\emergencystretch=1em
  ...
  Theorems~\textup{\ref{conj:universal-theta}}
...
\item under Verdier/Koszul duality, $\Theta_{\cA}$ is sent to
  $\Theta_{\cA^!}$.
\end{enumerate}
This is Theorem~\ref{conj:master-theta} (MC2).
\end{theorem}
...
Theorem~\ref{conj:universal-theta} upgrades the scalar invariant
...
Theorem~\ref{conj:universal-theta}.  Then:
\begin{enumerate}[label=\textup{(\roman*)}]
...
$\operatorname{tr}(\Theta_{\cA^!}) = -\kappa(\cA) \cdot \lambda_g$.
In particular, Theorem~\ref{conj:universal-theta}
...
$\Theta_{\cA}$ of
Theorem~\textup{\ref{conj:universal-theta}} exists with the
...
(Theorem~\ref{conj:universal-theta}(i)) forces $c = \kappa(\cA)$.
\end{proof}
...
By Theorem~\ref{conj:universal-theta}(i),
$\operatorname{tr}(\Theta^{\min}) = \kappa(\cA) \cdot \Lambda$,
forcing $c = \kappa(\cA)$.
...
(Theorem~\ref{conj:universal-MC}) and is
...
Theorem~\textup{\ref{conj:universal-theta}} \textup{(MC2)} holds
...
(Theorem~\ref{conj:universal-theta}) is now established by
