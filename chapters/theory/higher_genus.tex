...
\begin{theorem}[Verdier Duality = NAP Duality; \ClaimStatusProvedHere]\label{thm:verdier-NAP}
...
\begin{theorem}[Octahedron Identity for $m_6$; \ClaimStatusProvedHere]
...
\begin{theorem}[Structure of $\overline{\mathcal{M}}_{g,n}$; \ClaimStatusProvedHere]
...
For $\widehat{\mathfrak{g}}_k$, the anti-commutator $[d_0, d_0]$ is the genus-zero
curvature $m_0$ from Step~1, which acts on each of the $\dim(\mathfrak{g})$ current generators.
...
&= \sum_{D} \operatorname{Res}_D \left(\oint_{\gamma_k} \omega\right) + \oint_{\gamma_k} \left(\sum_D \operatorname{Res}_D(\omega)\right)
\end{align*}
where $\gamma_k$ is the $k$-th homology cycle.  Since $d_0 = \sum_D \operatorname{Res}_D$ computes residues
at collision divisors $D \subset \overline{C}_n(X)$, and $d_k = \oint_{\gamma_k}$ integrates
over a cycle on $\Sigma_g$ (acting on a \emph{different} set of variables), these operations
commute by Fubini's theorem: $\operatorname{Res}_D \circ \oint_{\gamma_k} = \oint_{\gamma_k} \circ \operatorname{Res}_D$.
Therefore the two terms are \emph{equal}, giving:
\[
d_0 d_k + d_k d_0 = 2\, d_0 \circ d_k.
\]
The vanishing $d_0 \circ d_k = 0$ then follows from a separate argument:
the cycle $\gamma_k \subset \Sigma_g$ is disjoint from the collision divisors $D \subset \overline{C}_n(X)$
(which involve points colliding on the \emph{same} fiber), so
$\operatorname{Res}_D(\oint_{\gamma_k} \omega) = \oint_{\gamma_k} \operatorname{Res}_D(\omega) = 0$
because the residue along $D$ vanishes when restricted to the cycle $\gamma_k$
(the collision locus and the homology cycle have disjoint support in the relevant directions).
