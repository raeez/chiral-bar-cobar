\chapter{The Fourier seed}\label{ch:fourier-seed}


\index{Fourier transform!non-abelian|textbf}
\index{propagator!logarithmic|textbf}
\index{bar construction!as Fourier transform}

The Fourier seed is where transform language, propagator language, and
kernel language first coincide.  The classical Fourier transform sends
a function to its integral against a kernel; the bar construction does
the same thing, but the kernel is the logarithmic propagator
$\eta_{ij} = d\log(z_i - z_j)$, the ``function'' is the operator
product expansion of a chiral algebra, and the ``integral'' is the
residue along boundary divisors of the Fulton--MacPherson
compactification.  This is not merely a convenient calculation or an
analogy pressed into service.  It is the first place where the volume
teaches the reader to think in correspondences: source, kernel,
target; algebra, coalgebra, transform.  Every higher-genus correction
and every modular shadow that appears in later chapters is a
deformation of this initial transform geometry---the genus-$1$
propagator replaces $d\log$ by $d\log\theta_1$ with an Arakelov
correction, and the curvature $\kappa(\cA)$ measures the failure of
the Arnold relation under that deformation.

What makes this chapter foundational rather than illustrative is the
triple coincidence it establishes.  The \emph{transform} perspective
says: the bar complex is the image of the chiral algebra under a
non-abelian Fourier transform with logarithmic kernel.  The
\emph{propagator} perspective says: the bar differential is built from
the same propagator that governs free-field contractions in conformal
field theory.  The \emph{kernel} perspective says: the Arnold relation
$\eta_{12}\wedge\eta_{23} + \eta_{23}\wedge\eta_{31} +
\eta_{31}\wedge\eta_{12} = 0$ is the single identity that forces
$d^2 = 0$.  These three descriptions are not three approximations to a
deeper truth; they are three projections of a single geometric object,
and their agreement at genus~$0$ is what makes the bar construction
work.  The content of Parts~I and~II is the systematic study of what
happens when one deforms away from this initial point of agreement.

The four evaluations that follow---a single point, two curves of
increasing complexity, and one non-abelian family---are chosen to
display the transform before any general theory is invoked.  The
reader who follows these evaluations will arrive at
Chapter~\ref{chap:bar-cobar} having already seen the bar complex
operate on concrete input, so that the abstract adjunction is a
theorem about something already understood rather than a definition
awaiting examples.

The bar construction is a one-dimensional non-abelian Fourier
transform.  Its kernel is the logarithmic propagator on
configuration spaces; its output is a factorization coalgebra on
$\Ran(X)$.  Four explicit evaluations of increasing
complexity---a point, two curves, and one non-abelian
family---exhibit the transform before the general theory of
Chapter~\ref{chap:bar-cobar}.

% ================================================================
\section{The kernel}\label{sec:fourier-kernel}
% ================================================================

\begin{definition}[Logarithmic propagator]\label{def:fourier-log-propagator}
\index{propagator!logarithmic}
For distinct points $z_1, z_2$ on a smooth algebraic curve~$X$, the
\emph{logarithmic propagator} is
\begin{equation}\label{eq:fourier-log-propagator}
\eta_{ij} \;=\; d\log(z_i - z_j)
\;=\; \frac{dz_i - dz_j}{z_i - z_j}
\;\in\; \Omega^1(\overline{C}_2(X),\, \log D_{ij}).
\end{equation}
\end{definition}

\begin{proposition}[Three properties of the propagator;
  \ClaimStatusProvedHere]
\label{prop:fourier-propagator-properties}
The logarithmic propagator satisfies:
\begin{enumerate}
\item \emph{Simple pole.}
  $\eta_{ij}$ has a simple pole along
  $D_{ij} = \{z_i = z_j\}$ with
  $\Res_{D_{ij}}\eta_{ij} = 1$.
\item \emph{Closedness.}
  $d\eta_{ij} = 0$ on $C_2(X) \setminus D_{ij}$.
\item \emph{Arnold relation.}
  In $\Omega^2(\overline{C}_3(X),\, \log D)$,
  \begin{equation}\label{eq:fourier-arnold}
  \eta_{12} \wedge \eta_{23}
  + \eta_{23} \wedge \eta_{31}
  + \eta_{31} \wedge \eta_{12} = 0.
  \end{equation}
\end{enumerate}
\end{proposition}

\begin{proof}
Properties~(1) and~(2) are immediate: the pole is simple with
residue $\Res_{\epsilon=0}(d\epsilon/\epsilon) = 1$, and
$d\,d\!\log = 0$.

For~(3), set $u_{ij} = z_i - z_j$ and write
$\eta_{ij} = u_{ij}^{-1}\,du_{ij}$ where
$du_{ij} = dz_i - dz_j$.
Each wedge product
$\eta_{ij} \wedge \eta_{jk}
 = u_{ij}^{-1} u_{jk}^{-1}\, du_{ij} \wedge du_{jk}$.
Since $du_{12} + du_{23} + du_{31} = 0$, all three wedge products
share a common $2$-form
$N := du_{12} \wedge du_{23}
= du_{23} \wedge du_{31} = du_{31} \wedge du_{12}$.
(For instance:
$du_{23} \wedge du_{31}
= du_{23} \wedge (-du_{12} - du_{23})
= -du_{23} \wedge du_{12} = du_{12} \wedge du_{23} = N$.)
Then
\begin{align}
\sum_{\mathrm{cyc}} \eta_{12} \wedge \eta_{23}
&= \left[
  \frac{1}{u_{12}\,u_{23}}
  + \frac{1}{u_{23}\,u_{31}}
  + \frac{1}{u_{31}\,u_{12}}
\right] \cdot N
\notag \\
&= \frac{u_{31} + u_{12} + u_{23}}{u_{12}\,u_{23}\,u_{31}}
  \cdot N
= \frac{0}{u_{12}\,u_{23}\,u_{31}} \cdot N = 0.
\label{eq:fourier-arnold-computation}
\end{align}
The numerator vanishes because
$(z_3 - z_1) + (z_1 - z_2) + (z_2 - z_3) = 0$.
\end{proof}

\begin{proposition}[Genus-\texorpdfstring{$1$}{1} propagator; \ClaimStatusProvedHere]
\label{prop:fourier-genus1-propagator}
On the elliptic curve
$E_\tau = \mathbb{C}/(\mathbb{Z} + \tau\mathbb{Z})$, the
propagator acquires a period correction:
\begin{equation}\label{eq:fourier-genus1-propagator}
\eta^{(1)}_{ij} = d\log\theta_1(z_i - z_j \,|\, \tau)
+ \frac{2\pi i}{\operatorname{Im}\tau}\,
  \operatorname{Im}(z_i - z_j)\,d\bar{z}_i,
\end{equation}
where $\theta_1(u|\tau)$ is the Jacobi theta function with simple
zero at $u = 0$.  The correction ensures single-valuedness on
$E_\tau \times E_\tau$ but breaks the Arnold relation:
\begin{equation}\label{eq:fourier-arnold-failure}
\sum_{\mathrm{cyc}} \eta^{(1)}_{12} \wedge \eta^{(1)}_{23}
= \omega_1,
\end{equation}
where
$\omega_1 = \frac{i}{2\operatorname{Im}\tau}\,dz \wedge d\bar{z}$
is the Arakelov $(1,1)$-form on $E_\tau$.
\end{proposition}

\begin{proof}
The holomorphic part
$\partial_{z_i}\!\log\theta_1(z_i - z_j|\tau)\,dz_i$ has the
required simple pole at $z_i = z_j$ (since $\theta_1$ vanishes
simply at the origin) but picks up monodromy
$\partial_u\!\log\theta_1(u + \tau|\tau)
 = \partial_u\!\log\theta_1(u|\tau) - 2\pi i$
around the $B$-cycle.  The correction term
$\frac{2\pi i}{\operatorname{Im}\tau}\operatorname{Im}(z_i - z_j)
\,d\bar{z}_i$
compensates this monodromy, making $\eta^{(1)}_{ij}$ single-valued.

Single-valuedness forces a non-vanishing
$\bar{\partial}$-component.  The Arakelov normalization
(Faltings~\cite{Fal84}) gives
\begin{equation}\label{eq:fourier-arakelov-normalization}
\bar{\partial}_{z_i}\, \eta^{(1)}_{ij}
= \frac{\pi}{\operatorname{Im}\tau}\,d\bar{z}_i.
\end{equation}

In genus~$0$, the propagator is $\bar{\partial}$-closed and
the Arnold relation holds.  In genus~$1$, the non-vanishing
$\bar{\partial}$-component~\eqref{eq:fourier-arakelov-normalization}
produces a residual in the Arnold sum.

The three key steps:
\begin{align}
\sum_{\mathrm{cyc}} \eta^{(1)}_{12} \wedge \eta^{(1)}_{23}
&= \sum_{\mathrm{cyc}} \eta^{(1)}_{12} \wedge
  \bigl(\eta^{\mathrm{hol}}_{23}
  + \eta^{\mathrm{corr}}_{23}\bigr)
\notag \\
&= \text{(holomorphic Arnold defect)}
  + \text{(cross-terms from }
  \bar{\partial}\eta^{(1)} \neq 0\text{)}
\notag \\
&= \omega_1,
\label{eq:fourier-arnold-failure-computation}
\end{align}
where the last equality uses the normalization
$\int_{E_\tau}\omega_1 = 1$: the total defect is constant on
$C_3(E_\tau)$ and is fixed by the Arakelov
normalization~\eqref{eq:fourier-arakelov-normalization}.
The complete computation of the Arnold defect at arbitrary genus
is Theorem~\ref{thm:arnold-higher-genus}.
\end{proof}


% ================================================================
\section{The transform}\label{sec:fourier-transform}
% ================================================================

\begin{definition}[Bar complex at tensor power \texorpdfstring{$n$}{n}]
\label{def:fourier-bar-transform}
For a chiral algebra $\cA$ on a smooth curve $X$, the bar complex
at tensor power $n$ is
\begin{equation}\label{eq:fourier-bar-transform}
\barB_n(\cA) = (s^{-1}\bar{\cA})^{\otimes n}
\otimes \Omega^\bullet(\overline{C}_n(X),\, \log D)
\end{equation}
with differential $d = d_{\mathrm{int}} + d_{\mathrm{res}}
+ d_{\mathrm{dR}}$.  The three components correspond to the internal
differential of $\cA$, residue extraction along collision divisors
$D_{ij} \subset \overline{C}_n(X)$, and the de~Rham differential on
logarithmic forms.  The general construction is developed in
Chapter~\ref{chap:bar-cobar}; here we compute it.
\end{definition}

\subsection*{Computation~(a): $X = \operatorname{Spec} k$,
  $\cA = \Lambda(V)$}

\begin{computation}[Bar on a point]\label{comp:fourier-point}
Let $X = \operatorname{Spec} k$ (a point) and $\cA = \Lambda(V)$
(the exterior algebra on a finite-dimensional vector space $V$).
\begin{enumerate}
\item $\overline{C}_n(\operatorname{Spec} k) = \operatorname{Spec} k$
  for all $n$.
\item $\Omega^\bullet(\overline{C}_n,\, \log D) = k$: no forms, no
  propagator.
\item $d_{\mathrm{dR}} = 0$ and $d_{\mathrm{res}}$ reduces to the
  binary product differential.
\item $\barB(\Lambda(V)) = T^c(s^{-1}\bar{V},\, d_{\mathrm{res}})$,
  the classical bar construction of
  Loday--Vallette~\cite[\S2.2]{LV12}.
\end{enumerate}
\end{computation}

\begin{proposition}[\ClaimStatusProvedHere]
\label{prop:fourier-com-lie-duality}
For $\dim V = 1$, the bar complex has zero differential:
$\barB(\Lambda(V)) \cong \operatorname{Sym}^c(s^{-1}V)$.  The
exterior algebra is the Fourier transform of the symmetric coalgebra.
\end{proposition}

\begin{proof}
The reduced part $\bar{V} = V$ is one-dimensional, so
$(s^{-1}V)^{\otimes n} \cong k$ for all $n$.  The bar differential
$d_{\mathrm{res}}\colon (s^{-1}v)^{\otimes 2} \mapsto
s^{-1}(v \wedge v) = 0$
vanishes because $v \wedge v = 0$ in $\Lambda(V)$.  The cofree
coalgebra on a one-dimensional space is cocommutative, giving
$\operatorname{Sym}^c(s^{-1}V)$.
\end{proof}

\begin{remark}\label{rem:fourier-com-lie-operad}
At the operad level, $\Lambda(V)$ is a free $\mathsf{Com}$-algebra.
Its Koszul dual coalgebra is a cofree $\mathsf{Com}^!$-coalgebra, and
$\mathsf{Com}^! = \mathsf{Lie}$
(Loday--Vallette~\cite[Theorem~7.2.2]{LV12}).  For $\dim V = 1$,
the Lie coalgebra structure is trivial, so the cofree Lie coalgebra
coincides with the symmetric coalgebra.  For $\dim V > 1$, the
nontrivial Lie coalgebra differential encodes the shuffle relations.
\end{remark}

\subsection*{Computation~(b): $X = \mathbb{P}^1$,
  $\cA = \mathcal{H}_k$}

The Heisenberg chiral algebra $\mathcal{H}_k$ at level
$k \in \mathbb{C}^\times$ has a single current $\alpha(z)$ of
conformal weight~$1$ with OPE
\begin{equation}\label{eq:fourier-heisenberg-ope}
\alpha(z)\alpha(w) = \frac{k}{(z-w)^2} + \text{regular}.
\end{equation}
The double pole and the absence of a simple pole are the entire
structure.

\begin{computation}[\texorpdfstring{$n = 1$}{n = 1}]\label{comp:fourier-heisenberg-n1}
The bar complex at tensor power~$1$ is $\barB_1 = s^{-1}\alpha$.
There is no collision divisor, so
$d_{\mathrm{res}} = 0$ and $d_{\mathrm{dR}} = 0$.  The element
$s^{-1}\alpha$ is a cocycle.
\end{computation}

\begin{computation}[\texorpdfstring{$n = 2$}{n = 2}; \ClaimStatusProvedHere]
\label{comp:fourier-heisenberg-n2}
At tensor power~$2$, the bar element is
\begin{equation}\label{eq:fourier-bar2-heisenberg}
\xi_{12} = s^{-1}\alpha(z_1) \otimes s^{-1}\alpha(z_2)
  \otimes \eta_{12}
\;\in\; \barB_2(\mathcal{H}_k).
\end{equation}
The bar differential acts by restricting to the boundary stratum
$D_{12} \subset \overline{C}_2(\mathbb{P}^1)$ and extracting the
OPE data via the Borcherds identity
(cf.~Chapter~\ref{ch:heisenberg-frame},
equation~\eqref{eq:frame-borcherds-extraction}).

\medskip\noindent\textbf{Step~1.}
The OPE $\alpha(z_1)\alpha(z_2) = k/(z_1 - z_2)^2 + \text{reg.}$
has a second-order pole with constant coefficient $c_2 = k$ and no
simple pole ($c_1 = 0$).

\medskip\noindent\textbf{Step~2.}
The boundary pairing on $D_{12}$ pairs the OPE singularity with the
logarithmic test form $\eta_{12}$.  For a second-order pole with
coefficient $c_2$, the Borcherds extraction gives
\begin{equation}\label{eq:fourier-borcherds-extraction}
d_{\mathrm{res}}(\xi_{12})
= \bigl\langle c_2/(z_1 - z_2)^2,\; \eta_{12}
  \bigr\rangle_{D_{12}}
= c_2 = k.
\end{equation}

\medskip\noindent\textbf{Step~3.}
The result is $k \cdot 1 \in \barB_0 = \mathbb{C}$: the
second-order OPE coefficient is extracted into the degree-$0$
component of the bar complex.
\end{computation}

\begin{remark}[Borcherds mechanism]\label{rem:fourier-borcherds}
The extraction~\eqref{eq:fourier-borcherds-extraction} is
\emph{not} the na\"ive contour integral
$\Res_{\epsilon = 0}[k\epsilon^{-2} \cdot d\epsilon/\epsilon]$
(which involves a triple pole and vanishes).  The logarithmic form
$\eta_{12}$ serves as a test form for the boundary pairing on the
Fulton--MacPherson compactification; the Borcherds identity provides
the correct extraction rule.  The detailed mechanism appears in
Chapter~\ref{ch:heisenberg-frame},
\S\ref{sec:frame-bar-deg1}.
\end{remark}

\begin{computation}[\texorpdfstring{$n = 3$}{n = 3}; \ClaimStatusProvedHere]
\label{comp:fourier-heisenberg-n3}
At tensor power~$3$, the bar element is
\[
\xi_{123} = s^{-1}\alpha(z_1) \otimes s^{-1}\alpha(z_2)
  \otimes s^{-1}\alpha(z_3)
  \otimes \eta_{12} \wedge \eta_{23}.
\]
Three boundary strata $D_{12}$, $D_{23}$, $D_{13}$ contribute to
$d_{\mathrm{res}}$, each extracting the second-order OPE coefficient:
\begin{align}
d_{\mathrm{res}}(\xi_{123})
&= k \cdot s^{-1}\alpha(z_3) \otimes
  \eta_{23}\big|_{z_1 = z_2}
\notag \\
&\quad + k \cdot s^{-1}\alpha(z_1) \otimes
  \eta_{12}\big|_{z_2 = z_3}
\notag \\
&\quad + k \cdot s^{-1}\alpha(z_2) \otimes
  \eta_{12}\big|_{z_1 = z_3}.
\label{eq:fourier-dres-n3}
\end{align}
Applying $d_{\mathrm{res}}$ again produces triple collisions.
The Arnold relation~\eqref{eq:fourier-arnold} forces the three-term
sum of triple residues to vanish:
$d_{\mathrm{res}}^2 = 0$ by the
cancellation~\eqref{eq:fourier-arnold-computation}.
\end{computation}

\begin{theorem}[\ClaimStatusProvedHere]
\label{thm:fourier-heisenberg-bar}
\index{Heisenberg!bar complex}
The bar complex of the Heisenberg algebra on $\mathbb{P}^1$ is
\begin{equation}\label{eq:fourier-heisenberg-bar}
\barB(\mathcal{H}_k) \;\cong\;
\operatorname{Sym}^{\mathrm{ch},c}(V^*[-1])
\end{equation}
with induced level $-k$, where $V = \langle\alpha\rangle$ is the
one-dimensional space of currents.
\end{theorem}

\begin{proof}
The bar complex has a single generator $s^{-1}\alpha$ in each tensor
power.  The cofree structure is cocommutative (one generator), giving
the symmetric coalgebra.  The level is $-k$: the bar construction
reverses the sign of the second-order OPE coefficient, as shown by the
extraction $d_{\mathrm{res}}(\xi_{12}) = k \cdot 1$
(Computation~\ref{comp:fourier-heisenberg-n2}).  The full verification
is Theorem~\ref{thm:heisenberg-bar-complete}
in Chapter~\ref{ch:heisenberg-frame}.
\end{proof}

\begin{remark}[Chiral homology verification]
\label{rem:chiral-homology-heisenberg-verification}
\index{chiral homology!Heisenberg verification}
The Beilinson--Drinfeld chiral homology
$H^*_{\mathrm{ch}}(\mathbb{P}^1, \mathcal{H}_k)$ is, by
definition, the cohomology of the de~Rham complex of the
$\mathcal{D}$-module $\Delta_!(\mathcal{H}_k)$ on the Ran space
(\cite[\S4.1]{BD04}).  For $\mathbb{P}^1$ with one current $J$ of
weight~$1$: $H^0_{\mathrm{ch}} = \mathbb{C}$ (vacuum coinvariant,
since $J_{-1}|0\rangle$ generates under the global residue), and
$H^1_{\mathrm{ch}} = \mathbb{C}^3$ (the three CE cocycles
$[J \otimes J \otimes \eta_{12}]$ and its weight-$2$ companions).
These match the bar-complex computation:
$H^0(\bar{B}(\mathcal{H}_k)) = \mathbb{C}$ and
$H^1(\bar{B}(\mathcal{H}_k)) = \mathbb{C}^3$
(Computation~\ref{comp:fourier-heisenberg-n2} and
Theorem~\ref{thm:fourier-heisenberg-bar}).  This confirms the BD
identification $H^*_{\mathrm{ch}} = H^*(\bar{B})$ on the
simplest non-trivial example.
\end{remark}

\begin{remark}[Heisenberg is not self-dual]
\label{rem:fourier-heisenberg-not-selfdual}
$\mathcal{H}_k^! = \operatorname{Sym}^{\mathrm{ch}}(V^*)$, not
$\mathcal{H}_{-k}$.  Self-duality would require the Koszul dual to
be a Heisenberg algebra at the dual level.  Instead,
$\mathcal{H}_k^!$ is the \emph{chiral symmetric algebra}, which is a
free chiral commutative algebra, not a Heisenberg algebra with a
nontrivial OPE.
\end{remark}

\subsection*{Computation~(c): $X = E_\tau$,
  $\cA = \mathcal{H}_k$}

The same elements appear as in Computation~(b), but on the elliptic
curve $E_\tau$.  The propagator $\eta_{12}$ is replaced by the
period-corrected propagator $\eta^{(1)}_{12}$ of
Proposition~\ref{prop:fourier-genus1-propagator}.

\begin{computation}[Heisenberg on \texorpdfstring{$E_\tau$}{E-tau}; \ClaimStatusProvedHere]
\label{comp:fourier-heisenberg-elliptic}
The bar element at tensor power~$2$ on $E_\tau$ is
\[
\xi^{(1)}_{12} = s^{-1}\alpha(z_1) \otimes s^{-1}\alpha(z_2)
  \otimes \eta^{(1)}_{12}.
\]
The fiberwise bar differential $\dfib$ extracts the same OPE
coefficient $k$ via the boundary pairing.  But now
$\dfib^{\,2} \neq 0$: the Arnold
failure~\eqref{eq:fourier-arnold-failure} gives
\begin{equation}\label{eq:fourier-heisenberg-curvature}
\dfib^{\,2}(\xi^{(1)}_{123}) = k \cdot \omega_1.
\end{equation}
The curvature $k \cdot \omega_1$ is the coupling of the second-order
OPE coefficient to the Arakelov form.

The period correction restores nilpotence.  Define the total
differential
\begin{equation}\label{eq:fourier-total-diff-genus1}
\Dg{1} = \dfib + t_1 \otimes \oint_{A_1}
\end{equation}
where $t_1 \in H^1(E_\tau)$ is the generator dual to the $A$-cycle
and $\oint_{A_1}$ integrates the propagator along the $A$-cycle of
$E_\tau$.
\end{computation}

\begin{proposition}[\ClaimStatusProvedHere]
\label{prop:fourier-total-diff-nilpotent}
$\Dg{1}^{\,2} = 0$.
\end{proposition}

\begin{proof}
Expand:
\begin{align}
\Dg{1}^{\,2}
&= \dfib^{\,2}
  + \left(\dfib \circ \textstyle\oint_{A_1}
  + \textstyle\oint_{A_1} \circ \dfib\right) \otimes t_1
  + \left(\textstyle\oint_{A_1}\right)^{\!2} \otimes t_1^2.
\label{eq:fourier-D1-squared}
\end{align}
The last term vanishes because $t_1^2 = 0$ in $H^*(E_\tau)$.  The
first term is $k \cdot \omega_1$
(equation~\eqref{eq:fourier-heisenberg-curvature}).  The middle term
is the commutator
$[\dfib, \oint_{A_1}]$, which equals $-k \cdot \omega_1$ by the
residue theorem: integrating the propagator around the $A$-cycle
picks up the Arakelov contribution.  The three terms cancel:
$k\omega_1 - k\omega_1 + 0 = 0$.
The full computation at arbitrary genus is
Theorem~\ref{thm:quantum-diff-squares-zero}.
\end{proof}

\begin{proposition}[Fourier--Mukai identification;
  \ClaimStatusProvedElsewhere]
\label{prop:fourier-mukai-identification}
\index{Fourier--Mukai transform}
For $k = 1$, the transform $\barB_{E_\tau}(\mathcal{H}_1)$ recovers
the Fourier--Mukai kernel $\mathcal{P}$ on
$\operatorname{Jac}(E_\tau) \times
\widehat{\operatorname{Jac}}(E_\tau)$:
\begin{equation}\label{eq:fourier-mukai-identification}
\barB_{E_\tau}(\mathcal{H}_1) \;\simeq\; \mathcal{P}.
\end{equation}
\end{proposition}

\begin{proof}[Reference]
The identification follows from the abelian case of the bar
construction: the Heisenberg algebra at level~$1$ generates the Fock
sheaf on $\operatorname{Jac}(E_\tau)$, and the bar differential
encodes the Poincar\'e line bundle.  See
Polishchuk~\cite{Pol03} for the Fourier--Mukai formalism on abelian
varieties and Frenkel--Ben-Zvi~\cite{FBZ04} for the vertex algebra
connection.
\end{proof}

\subsection*{Degeneration to the classical Fourier transform}
\label{subsec:fourier-degeneration}
\index{Fourier transform!recovery from bar-cobar}
\index{Fourier--Mukai transform!degeneration to Fourier}
\index{Poincar\'e line bundle!degeneration}

\begin{proposition}[Degeneration of the propagator;
  \ClaimStatusProvedHere]
\label{prop:fourier-propagator-degeneration}
Write $q = e^{2\pi i\tau}$.
The Jacobi triple product
\[
\theta_1(z|\tau)
= 2q^{1/4}\sin(\pi z)
  \prod_{n=1}^{\infty}
  (1-q^n)(1-q^n e^{2\pi iz})(1-q^n e^{-2\pi iz})
\]
gives the propagator on $E_\tau$ by logarithmic differentiation:
\begin{equation}\label{eq:fourier-theta-product}
d_z\log\theta_1(z|\tau)
= \pi\cot(\pi z)\,dz
  + 4\pi\sum_{n=1}^{\infty}
  \frac{q^n}{1-q^n}\sin(2\pi n z)\,dz.
\end{equation}
Three successive degenerations:
\begin{enumerate}
\item \emph{$q \to 0$ \textup{(}$B$-cycle collapse\textup{)}.}
  Every factor $(1 - q^n\,\cdot) \to 1$, and
  \begin{equation}\label{eq:fourier-propagator-degen}
  d_z\log\theta_1(z|\tau)
  \;\xrightarrow{\;q \to 0\;}\;
  \pi\cot(\pi z)\,dz
  = d\log\sin(\pi z).
  \end{equation}
\item \emph{Mittag-Leffler \textup{(}real locus of $\mathbb{C}^*$\textup{)}.}
  On $x \in \mathbb{R}/\mathbb{Z}$,
  \begin{equation}\label{eq:fourier-propagator-real}
  \pi\cot(\pi x)\,dx
  = \operatorname{p.v.}\!\sum_{n=-\infty}^{\infty}
    \frac{dx}{x - n}.
  \end{equation}
\item \emph{Universal cover \textup{(}$\mathbb{R} \to \mathbb{R}/\mathbb{Z}$\textup{)}.}
  Drop the $n \neq 0$ translates:
  \begin{equation}\label{eq:fourier-propagator-R}
  \frac{dx}{x} = d\log x.
  \end{equation}
\end{enumerate}
\end{proposition}

\begin{proof}
Differentiate $\log\theta_1$
term by term using $\frac{d}{dz}\log(1-q^ne^{2\pi iz})
= -\frac{2\pi i\,q^n e^{2\pi iz}}{1-q^n e^{2\pi iz}}$
and combine geometric series
(Whittaker--Watson~\cite[\S21.3]{WW27}).
The Mittag-Leffler expansion
$\pi\cot(\pi x)
= \lim_{N\to\infty}\sum_{n=-N}^{N}(x-n)^{-1}$
follows from the Weierstrass product
$\sin(\pi x) = \pi x\prod_{n=1}^{\infty}(1-x^2/n^2)$
by logarithmic differentiation
(Ahlfors~\cite[\S5.2]{Ahlfors79}).
\end{proof}

\begin{proposition}[Degeneration of the Poincar\'e line bundle;
  \ClaimStatusProvedHere]
\label{prop:fourier-poincare-degeneration}
\index{Poincar\'e line bundle!degeneration to character}
The Poincar\'e line bundle on
$E_\tau \times \widehat{E}_\tau$
\textup{(}$\widehat{E}_\tau \cong E_\tau$ via the principal
polarization\textup{)} has fiber
\begin{equation}\label{eq:fourier-poincare-fiber}
\mathcal{P}_{(z,\,\hat{w})}
= \frac{\theta_1(z - \hat{w}|\tau)\;\theta_1'(0|\tau)}
       {\theta_1(z|\tau)\;\theta_1(\hat{w}|\tau)}
\end{equation}
\textup{(}Mumford~\textup{\cite[\S I.2]{Mum70}}\textup{)}.
Three degenerations:
\begin{enumerate}
\item \emph{$q \to 0$.}
  The triple products cancel:
  \begin{equation}\label{eq:fourier-poincare-degen-Gm}
  \mathcal{P}_{(z,\hat{w})}
  \;\xrightarrow{\;q \to 0\;}\;
  \frac{\sin\pi(z - \hat{w})}
       {\sin(\pi z)\;\sin(\pi\hat{w})}.
  \end{equation}

\item \emph{Real torus.}
  Set $z = x$, $\hat{w} = \xi$ with
  $x,\xi \in \mathbb{R}/\mathbb{Z}$.  Logarithmic differentiation
  of $\sin(\pi u) = \pi u\prod(1-u^2/n^2)$ gives
  \begin{equation}\label{eq:fourier-sine-product}
  \frac{\sin\pi(x - \xi)}{\sin(\pi x)\sin(\pi\xi)}
  = \cot(\pi\xi) - \cot(\pi x)
  = \sum_{n \in \mathbb{Z}}
    \!\left(\frac{1}{\xi - n} - \frac{1}{x - n}\right).
  \end{equation}
  Compute the Fourier coefficients of the right-hand side as a
  distribution in $x$:
  \[
  \int_0^1
    \left(\frac{1}{\xi - n} - \frac{1}{x - n}\right)
    e^{-2\pi i mx}\,dx
  = \frac{e^{-2\pi i m\xi}}{\xi - n}
    \quad (m \neq 0).
  \]
  Summing over $n$ and applying Poisson summation kills every term
  except $m = n$, leaving the $n$-th Fourier coefficient
  $e^{-2\pi in\xi}$.  The Fourier--Mukai integral becomes
  Fourier series:
  \begin{equation}\label{eq:fourier-series-from-FM}
  \int_{\mathbb{R}/\mathbb{Z}} f(x)\,
    \mathcal{P}_{(x,\xi)}\,dx
  = \sum_{n \in \mathbb{Z}} \hat{f}(n)\,e^{2\pi i n\xi}.
  \end{equation}

\item \emph{Universal cover.}
  $\widehat{\mathbb{R}/\mathbb{Z}} = \mathbb{Z}$;
  $\widehat{\mathbb{R}} = \mathbb{R}$.  The characters
  $e^{2\pi inx}$ for $n \in \mathbb{Z}$ extend to
  $e^{2\pi i\xi x}$ for $\xi \in \mathbb{R}$
  \textup{(}Pontryagin~\textup{\cite{Pont66})}, and
  \begin{equation}\label{eq:fourier-integral-from-FM}
  \hat{f}(\xi)
  = \int_{\mathbb{R}} f(x)\,e^{-2\pi i\xi x}\,dx.
  \end{equation}
\end{enumerate}
\end{proposition}

\begin{proof}
The $q \to 0$ limit applies the Jacobi triple product to each
theta factor in~\eqref{eq:fourier-poincare-fiber}
(Polishchuk~\cite[\S16.2]{Pol03}).  The Fourier coefficient
computation uses $\int_0^1 (x-n)^{-1}\,e^{-2\pi imx}\,dx
= -\pi i\,\operatorname{sgn}(m)\,e^{-2\pi imn}$ for
$m \neq 0$ (residue calculus on the strip
$\{0 \le \operatorname{Re} z \le 1\}$).
Pontryagin duality for locally compact abelian groups
(Rudin~\cite[\S1.7]{Rudin62})
gives item~(3).
\end{proof}

\begin{theorem}[Recovery of the Fourier transform;
  \ClaimStatusProvedHere]
\label{thm:fourier-recovery}
\index{Fourier transform!recovery from chiral bar}
\begin{equation}\label{eq:fourier-degeneration-chain}
\begin{tikzcd}[column sep=2.8em, row sep=2.8em]
\barB_{E_\tau}(\mathcal{H}_1)
  \arrow[r, "q \to 0"]
  \arrow[d, "\text{\footnotesize Prop.\
    \ref{prop:fourier-mukai-identification}}"', "\simeq"]
& \barB_{\mathbb{C}^*}(\mathcal{H}_1)
  \arrow[r, "\mathbb{R}\text{-locus}"]
& \barB_{S^1}(\mathcal{H}_1)
  \arrow[r, "\widetilde{\phantom{xx}}"]
& \barB_{\mathbb{R}}(\mathcal{H}_1)
  \arrow[d, "\text{\footnotesize coalgebra exp}",
    "\simeq"']
\\
\mathcal{P}\;\text{on}\;
  E_\tau \times \widehat{E}_\tau
  \arrow[r, "q \to 0"']
& \dfrac{\sin\pi(x{-}\xi)}
        {\sin\pi x\;\sin\pi\xi}
  \arrow[r, "\text{Fourier}"']
& e^{2\pi inx}
  \arrow[r, "n \to \xi"']
& e^{-2\pi i\xi x}
\end{tikzcd}
\end{equation}
\begin{center}
\small
\renewcommand{\arraystretch}{1.4}
\begin{tabular}{llll}
\textbf{Stage}
  & \textbf{Curve}
  & \textbf{Propagator}
  & \textbf{Kernel} \\
\hline
Chiral
  & $E_\tau$
  & $d\log\theta_1(z|\tau)$
  & $\mathcal{P}$ \\
Multiplicative
  & $\mathbb{C}^*$
  & $\pi\cot(\pi z)\,dz$
  & $\sin\pi(x{-}\xi)/(\sin\pi x\;\sin\pi\xi)$ \\
Compact real
  & $S^1$
  & $\operatorname{p.v.}\sum(x{-}n)^{-1}\,dx$
  & $e^{2\pi inx}$ \\
Real line
  & $\mathbb{R}$
  & $dx/x$
  & $e^{-2\pi i\xi x}$
\end{tabular}
\end{center}
The left column degenerates by
Proposition~\textup{\ref{prop:fourier-propagator-degeneration}};
the right column, by
Proposition~\textup{\ref{prop:fourier-poincare-degeneration}}.
The passage from left to right at each stage is the coalgebra
exponential.
\end{theorem}

\begin{proof}
The bar-cobar pairing
$\langle\eta,\delta\rangle = 1$
(Theorem~\ref{thm:bar-cobar-verdier}) at each stage:
\begin{alignat}{2}
&E_\tau\colon\quad
&&\textstyle\int_{\overline{C}_2(E_\tau)}\!
  d\log\theta_1(z|\tau) \wedge \delta(z) = 1,
\label{eq:pairing-Etau}
\\
&\mathbb{C}^*\colon\quad
&&\textstyle\oint \pi\cot(\pi z)\,\delta(z)\,dz = 1,
\label{eq:pairing-Gm}
\\
&\mathbb{R}\colon\quad
&&\textstyle\operatorname{p.v.}\!\int_{\mathbb{R}}
  \frac{\delta(x)}{x}\,dx = 1.
\label{eq:pairing-R}
\end{alignat}
Each of these is the $n = 1$ value.  The coalgebra exponential
produces the full kernel from this seed.  Concretely: the bar complex
$\barB(\mathcal{H}_1)$ is the symmetric coalgebra
$\operatorname{Sym}^c(s^{-1}V^*)$ with $V = \langle\alpha\rangle$
one-dimensional
(Theorem~\ref{thm:fourier-heisenberg-bar}).  Write $\xi = s^{-1}\alpha^*$
for the cogenerator and $x$ for the dual generator of
$\Omega(\barB(\mathcal{H}_1))$.
The comultiplication
$\Delta(\xi^n) = \sum_{p+q=n}\binom{n}{p}\xi^p \otimes \xi^q$
is the binomial coproduct, so the pairing extends multiplicatively:
\begin{equation}\label{eq:pairing-symmetric-powers}
\bigl\langle \xi^n,\, x^m\bigr\rangle
= n!\,\delta_{nm}.
\end{equation}
The generating function of the
pairing is
\begin{equation}\label{eq:pairing-exponential}
\sum_{n=0}^{\infty}
  \frac{\langle\xi^n,\, x^n\rangle}{(n!)^2}\,t^n
= \sum_{n=0}^{\infty}\frac{t^n}{n!}
= e^t.
\end{equation}
Set $t = x\xi$.  On~$\mathbb{R}$, this is $e^{x\xi}$.  On~$E_\tau$,
the same coalgebra exponential applied to the theta-function seed
$\langle d\log\theta_1, \delta\rangle = 1$ produces the Poincar\'e
line bundle $\mathcal{P}_{(z,\hat w)}$: the
$q$-expansion~\eqref{eq:fourier-poincare-fiber} is the multiplicative
lift of the exponential from $\mathbb{Z}$ to
$\mathbb{Z}[\![q]\!]$.
\end{proof}

\begin{remark}[Connection and holonomy]\label{rem:propagator-vs-kernel}
The propagator $d\log(z-w)$ is a flat connection on the trivial line
bundle over $C_2(X)$.  The kernel $e^{x\xi}$ is its holonomy: the
parallel transport operator $\exp\!\bigl(\int_\gamma d\log(z-w)\bigr)$
along a path from $(0,0)$ to $(x,\xi)$ in $\mathbb{R} \times
\widehat{\mathbb{R}}$.  The passage from propagator to kernel is the
passage from a Lie algebra element to its group element via the
exponential map; the coalgebra structure of $\barB(\mathcal{H}_1)$
makes this exponential explicit at each bar degree.
\end{remark}

\begin{remark}[Four incarnations]\label{rem:four-transforms}
\index{Fourier transform!four incarnations}
\begin{center}
\small
\renewcommand{\arraystretch}{1.3}
\begin{tabular}{llll}
\textbf{Transform}
  & \textbf{Domain}
  & \textbf{Kernel}
  & \textbf{Source} \\
\hline
Chiral bar
  & $\cA \mapsto \barB_X(\cA)$
  & $d\log E(z,w)$
  & this monograph \\
Fourier--Mukai
  & $D^b(A) \to D^b(\hat{A})$
  & $\mathcal{P}$
  & Mukai~\cite{Muk81} \\
Fourier series
  & $L^2(S^1) \to \ell^2(\mathbb{Z})$
  & $e^{2\pi inx}$
  & Fourier \\
Fourier integral
  & $L^2(\mathbb{R}) \to L^2(\mathbb{R})$
  & $e^{-2\pi i\xi x}$
  & Fourier
\end{tabular}
\end{center}
Each row is the degeneration of the row above it.
\end{remark}

\subsection*{Computation~(d): $X = \mathbb{P}^1$,
  $\cA = \widehat{\mathfrak{g}}_k$}

The affine Kac--Moody algebra $\widehat{\mathfrak{g}}_k$ at
level $k$ has currents $J^a(z)$ ($a = 1, \ldots, \dim\mathfrak{g}$)
with OPE
\begin{equation}\label{eq:fourier-km-ope}
J^a(z)J^b(w) \sim \frac{k\kappa^{ab}}{(z-w)^2}
+ \frac{f^{ab}{}_c\,J^c(w)}{z-w}
\end{equation}
where $\kappa^{ab}$ is the Killing form and $f^{ab}{}_c$ are the
structure constants of $\mathfrak{g}$.

\begin{computation}[Kac--Moody bar; \ClaimStatusProvedHere]
\label{comp:fourier-km-bar}
At tensor power~$2$, the bar element
$s^{-1}J^a(z_1) \otimes s^{-1}J^b(z_2) \otimes \eta_{12}$ receives
two contributions from the bar differential:
\begin{enumerate}
\item \emph{First-order pole.}
  $\bigl\langle f^{ab}{}_c\, J^c(w)/(z-w),\;
  \eta_{12}\bigr\rangle_{D_{12}}
  = f^{ab}{}_c \cdot s^{-1}J^c$:
  the Lie bracket term.
\item \emph{Second-order pole.}
  $\bigl\langle k\kappa^{ab}/(z-w)^2,\;
  \eta_{12}\bigr\rangle_{D_{12}}
  = k\kappa^{ab} \cdot 1$:
  the Killing term.
\end{enumerate}
The first contribution builds the Chevalley--Eilenberg differential;
the second introduces curvature.
\end{computation}

\begin{theorem}[\ClaimStatusProvedHere]
\label{thm:fourier-km-bar}
\index{Kac--Moody!bar complex}
The bar complex of $\widehat{\mathfrak{g}}_k$ on $\mathbb{P}^1$ is
the curved chiral Chevalley--Eilenberg coalgebra:
\begin{equation}\label{eq:fourier-km-bar}
\barB(\widehat{\mathfrak{g}}_k) \;\cong\;
\hat{C}^\bullet_{\mathrm{ch}}(\mathfrak{g},\, k\kappa).
\end{equation}
The curvature element $m_0 = k\kappa \in \barB_0$ is the Killing
form at level~$k$.  The Koszul dual level is $-k - 2h^\vee$, where
$h^\vee$ is the dual Coxeter number.
\end{theorem}

\begin{proof}
The first-order pole contributes the Lie coalgebra differential
$d_{\mathrm{CE}}\colon s^{-1}\mathfrak{g} \otimes
s^{-1}\mathfrak{g} \to s^{-1}\mathfrak{g}$, giving the
Chevalley--Eilenberg complex $C^\bullet(\mathfrak{g})$.  The
second-order pole contributes the curvature $m_0 = k\kappa$, making
the complex curved: $m_1^2(x) = m_2(m_0, x) - m_2(x, m_0) =
[m_0, x]$ (the commutator, with minus sign).

The level shift $k \mapsto -k - 2h^\vee$ arises from Feigin--Frenkel
duality~\cite{Feigin-Frenkel}: the Koszul dual
$\widehat{\mathfrak{g}}_k^!$ carries level $-k - 2h^\vee$ (not
$-k - h^\vee$; the factor is $2h^\vee$ in the vertex algebra
convention).  See \S\ref{sec:km-koszul-abstract}.
\end{proof}


% ================================================================
\section{Specialization to a point}\label{sec:fourier-specialization}
% ================================================================

\begin{theorem}[Specialization; \ClaimStatusProvedHere]
\label{thm:fourier-specialization}
\index{bar construction!specialization to a point}
Let $X = \operatorname{Spec} k$.  Then:
\begin{enumerate}
\item $\overline{C}_n(\operatorname{Spec} k) = \operatorname{Spec} k$
  for all $n$.
\item $\Omega^\bullet(\overline{C}_n,\, \log D) = k$ (no forms, no
  propagator).
\item $d_{\mathrm{dR}} = 0$ and $d_{\mathrm{res}}$ reduces to the
  binary bracket differential.
\item $\barB_X(\cA)$ reduces to
  $T^c(s^{-1}\bar{\cA},\, d_{\mathrm{bracket}})$, the classical bar
  construction of Loday--Vallette~\cite[\S2.2]{LV12}.
\end{enumerate}
\end{theorem}

\begin{proof}
Items~(1)--(3) are immediate: $\operatorname{Spec} k$ has no
nontrivial configuration geometry.  For~(4), the general
three-component differential
$d = d_{\mathrm{int}} + d_{\mathrm{res}} + d_{\mathrm{dR}}$
collapses to $d_{\mathrm{int}} + d_{\mathrm{res}}$ when
$X = \operatorname{Spec} k$.  The residue component
$d_{\mathrm{res}}$ at $X = \operatorname{Spec} k$ extracts only the
leading OPE coefficient, which on a point is the binary product.
The result is the classical bar construction
$T^c(s^{-1}\bar{\cA},\, d_2)$ with the standard bar differential.
\end{proof}

\begin{equation}\label{eq:fourier-specialization-diagram}
\begin{tikzcd}[column sep=huge, row sep=large]
\mathrm{ChirAlg}(X) \arrow[r, "\barB_X"]
  \arrow[d, "X = \mathrm{pt}"']
& \mathrm{Fact}^c(\Ran\, X) \arrow[d, "X = \mathrm{pt}"] \\
\mathrm{Alg}_k \arrow[r, "\barB"']
& \mathrm{CoAlg}_k
\end{tikzcd}
\end{equation}

\begin{remark}\label{rem:fourier-zero-dim-shadow}
The entire theory of operadic Koszul duality
(Loday--Vallette~\cite{LV12}) is the zero-dimensional shadow of the
chiral bar-cobar theory.  Every classical theorem---bar-cobar
adjunction, Koszul criterion, PBW theorem---lifts to a
genus-sensitive statement on curves, with the configuration space
geometry providing the lift.
\end{remark}


% ================================================================
\section{The four properties}\label{sec:fourier-four-properties}
% ================================================================

\begin{theorem}[The four properties of the Fourier transform;
\ClaimStatusProvedHere]
\label{thm:fourier-four-properties}
The bar construction $\barB_X$, viewed as a non-abelian Fourier
transform, has four structural properties corresponding to
Theorems~A--D of this monograph:
\begin{enumerate}[label=\textup{(\Alph*)}]
\item \emph{Intertwining.}
  $\mathbb{D}_{\Ran} \circ \barB_X \simeq
  \barB_X \circ (-)^!$
  \textup{(}Theorem~\ref{thm:bar-cobar-isomorphism-main}\textup{)}.
  Verdier duality intertwines the bar of $\cA$ with the bar of the
  Koszul dual $\cA^!$.
\item \emph{Inversion.}
  On the Koszul locus,
  $\Omega(\barB(\cA)) \xrightarrow{\;\sim\;} \cA$
  \textup{(}Theorem~\ref{thm:higher-genus-inversion}\textup{)}.
  The cobar construction inverts the transform.
\item \emph{Plancherel.}
  $Q_g(\cA) \oplus Q_g(\cA^!) \simeq
  R\Gamma(\overline{\mathcal{M}}_g,\, Z_{\cA})$
  \textup{(}Theorem~\ref{thm:quantum-complementarity-main}\textup{)}.
  The deformation and obstruction complexes form complementary
  Lagrangians.
\item \emph{Characteristic function.}
  $\kappa(\cA)$ determines the genus expansion; its generating
  function is the $\hat{A}$-genus
  \textup{(}Theorem~\ref{thm:modular-characteristic}\textup{)}.
\end{enumerate}
\end{theorem}

\begin{remark}[Classical Fourier analogues]
\label{rem:fourier-classical-analogues}
\begin{center}
\small
\begin{tabular}{lll}
\textbf{Property}
  & \textbf{Chiral (this monograph)}
  & \textbf{Classical Fourier} \\
\hline
(A) & Verdier intertwining
  & $\widehat{f \cdot g} = \hat{f} * \hat{g}$ \\
(B) & Bar-cobar inversion
  & $\hat{f}^{-1} \circ \hat{f} = \mathrm{id}$ \\
(C) & Lagrangian complementarity
  & $\|f\|^2 = \|\hat{f}\|^2$ (Parseval) \\
(D) & Modular characteristic $\kappa$
  & Characteristic function determines moments
\end{tabular}
\end{center}
\end{remark}

\begin{remark}\label{rem:fourier-scope}
These four properties and the specialization theorem
(\S\ref{sec:fourier-specialization}) contain the entire subject.
Part~1 develops the proofs.  Part~2 computes the transform for each
standard family.
\end{remark}


% ================================================================
\section{The landscape}\label{sec:fourier-landscape}
% ================================================================

\begin{center}
\small
\begin{tabular}{llll}
\textbf{$\cA$}
  & \textbf{$\barB(\cA)$}
  & \textbf{$\kappa(\cA)$}
  & \textbf{$c(\cA)$} \\
\hline
$\mathcal{H}_k$
  & $\operatorname{Sym}^{\mathrm{ch},c}(V^*[-1])$, level $-k$
  & $k$
  & $1$ \\[2pt]
$\widehat{\mathfrak{g}}_k$
  & Curved CE coalgebra, level $-k - 2h^\vee$
  & $\dfrac{k\dim\mathfrak{g}}{k + h^\vee}$
  & $\dfrac{k\dim\mathfrak{g}}{k + h^\vee}$ \\[6pt]
$\mathrm{Vir}_c$
  & Curved CE coalgebra
  & $c/2$
  & $c$ \\[2pt]
$\mathcal{W}_N$
  & Via DS reduction
  & DS formula
  & DS formula
\end{tabular}
\end{center}

\noindent
The detailed computations appear in Part~2
(Chapters~\ref{ch:heisenberg-frame}--\ref{chap:yangians}).


% ================================================================
\section{Genus as deformation variable}\label{sec:fourier-genus}
% ================================================================

\begin{remark}[Ref.\
  Theorem~\ref{thm:mc2-full-resolution}]
\label{rem:fourier-genus-preview}
At genus $g \geq 1$, the bar complex $\barB_g(\cA)$ is a family of
curved cochain complexes over $\overline{\mathcal{M}}_g$.  The proved
invariant $\kappa(\cA)$ (\ClaimStatusProvedHere;
Theorem~\ref{thm:modular-characteristic}) is the scalar shadow of the
universal Maurer--Cartan class
\begin{equation}\label{eq:fourier-theta-preview}
\Theta_{\cA} \;\in\;
\mathrm{MC}\!\left(
  \Defcyc(\cA) \;\hat{\otimes}\;
  R\Gamma(\overline{\mathcal{M}}_{g,\bullet},\, \mathbb{Q})
\right),
\end{equation}
constructed by Theorem~\ref{thm:mc2-full-resolution}
(Theorem~\ref{thm:master-theta}).
On the theorem surface this means that the Fourier transform already
lands in the concrete modular datum of
Theorem~\ref{thm:volume-one-concrete-modular-datum}: its M-level
output is the curved family $(\barB_g(\cA), \Dg{g})$, its S-level
shadow is the characteristic hierarchy, and on the simple-Lie-symmetry
locus its H-level logarithm is~$\Theta_{\cA}$.  What remains frontier
is the coderived continuation beyond the proved loci, not the
existence of this datum.
\end{remark}
