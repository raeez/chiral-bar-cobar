...
zero at $u = 0$. The correction is a $(1,0)$-form with
real-analytic coefficient; it descends the theta quotient to
$E_\tau \times E_\tau$ and satisfies
$\eta^{(1)}_{ji}=\eta^{(1)}_{ij}$. For
\[
\mathcal A^{(1)}_{123}:=
...
\begin{proof}
Set $u=z_i-z_j$ and $du=dz_i-dz_j$. The holomorphic coefficient
$\partial_u\log\theta_1(u|\tau)$ has the required simple pole of
residue~$1$ at $u=0$ (since $\theta_1$ vanishes simply at the
origin) and is $A$-periodic, but it picks up $B$-cycle monodromy
$\partial_u\!\log\theta_1(u + \tau|\tau)
 = \partial_u\!\log\theta_1(u|\tau) - 2\pi i$.
The deficit multiplies the $(1,0)$-form~$du$. Since
$\operatorname{Im}(u+\tau)=\operatorname{Im}u+\operatorname{Im}\tau$,
the coefficient
$2\pi i\,\operatorname{Im}u/\operatorname{Im}\tau$ shifts by
$+2\pi i$ under the same period and multiplies the same~$du$. A
$(0,1)$-form cannot cancel this $(1,0)$ monodromy. Oddness of the
coefficient, together with $d(z_j-z_i)=-du$, gives
$\eta^{(1)}_{ji}=\eta^{(1)}_{ij}$.

Single-valuedness forces a non-holomorphic coefficient, not an
anti-holomorphic one-form. In Arakelov normalization, with
$q_{ij}(z)=z_i-z_j$, the logarithmic current is
\[
d\eta^{(1)}_{ij}
=
2\pi i\,\bigl(\delta_{D_{ij}}-q_{ij}^{*}\omega_\tau\bigr),
\]
so away from the diagonal the smooth term is
$-2\pi i\,q_{ij}^{*}\omega_\tau$ (Faltings~\cite{Fal84}).
...
OPE data via the Borcherds identity, as in the degree-one
Heisenberg-frame extraction computation of
...
$d_{\mathrm{res}}^2 = 0$ by the cancellation computed in the proof of
Proposition~\ref{prop:fourier-propagator-properties}.
\end{computation}
...
$\mathcal{H}_k^! =
(\operatorname{Sym}^{\mathrm{ch}}(V^*[1]),m_0=-k\,\omega)$, not
...
Let $\cA=T_X(V)/(R)$ be an augmented quadratically presented chiral
algebra whose bar pieces are finite-type holonomic
$\mathcal D$-modules on $\Ran(X)$ after the chosen Koszul contraction.
Then the following five objects have different targets:
...
\end{equation}
reconstructs~$\cA$ on the Koszul locus.  Local finite duality
identifies $\mathbb D_{\Ran}(\cA^{\mathrm i})$ with the strict
quadratic algebra~$(\cA^{\mathrm i})^\vee$; the chosen-pair
map~$\nu_\cA$ realizes~$\cA^!$.
\end{proposition}
...
The finite-type holonomicity hypothesis defines Verdier duality on the
Ran bar in the ordinary derived category and produces~$K_X(\cA)$.
Contravariance gives~$\mathbb D(q_\cA)$; formality and the pair package
make~$\nu_\cA$ an equivalence with the strict algebra~$\cA^!$.  The
derived chiral centre has the independent Hochschild target displayed
in~\eqref{eq:fourier-five-objects}.
\end{proof}
...
\begin{proposition}[Poincar\'e connection seed; \ClaimStatusConditional]
...
This is the level-shifting duality of
...
\textup{(}Theorem~\ref{thm:bar-cobar-isomorphism-main}\textup{)}.
 The first object is the homotopy dual algebra.  A chosen Koszul pair
 with formal transferred structure makes~$\nu_\cA$ an equivalence to
 the strict algebra~$\cA^!$.
\item \emph{Inversion.}
...
At the all-weight numerical level, the scalar coefficient is
$F_g^{\mathrm{sc}}(\cA)=\kappa(\cA)\lambda_g^{\mathrm{FP}}$, and the
full coefficient is
