...
The governing question of this chapter is the mathematical role of \emph{The Fourier seed}: which part of the modular bar-cobar construction or its inversion it isolates, and which boundary between construction, resolution, and shadow it clarifies.
\end{remark}
...
\begin{proposition}[Genus-$1$ propagator; \ClaimStatusProvedHere]
...
\begin{definition}[Bar complex at tensor power $n$]
...
(Loday--Vallette~\cite[Theorem~7.2.2]{LV}).  For $\dim V = 1$,
...
\begin{computation}[$n = 1$]\label{comp:fourier-heisenberg-n1}
...
\begin{computation}[$n = 2$; \ClaimStatusProvedHere]
...
\begin{computation}[$n = 3$; \ClaimStatusProvedHere]
...
\begin{computation}[Heisenberg on $E_\tau$; \ClaimStatusProvedHere]
...
duality~\cite{FF92}: the Koszul dual
...
\item $\barB_X(\cA)$ reduces to
  ...
  construction of Loday--Vallette~\cite[\S2.2]{LV}.
\end{enumerate}
\end{theorem}
...
(Loday--Vallette~\cite{LV}) is the zero-dimensional shadow of the
...
\begin{remark}[\ClaimStatusProvedHere; ref.\
