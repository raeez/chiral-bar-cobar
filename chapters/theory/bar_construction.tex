...
tensor coalgebra is the primitive bar object. Its coderivations are
...
The full ordered bar differential is the pullback of one universal flat
connection along the KZ functor:
\[
 \dbar \;=\; \KZ^{*}(\nabla_{\Arn})
\]
...
Theorem~\ref{thm:climax-vol1-platonic} identifies it with a single
universal pullback.
...
\barB_X(\cA)$ is the averaging/coinvariant map; it forgets the
non-symmetric ordered kernel unless an additional acyclicity theorem
is invoked. Throughout this chapter, the unqualified notation
...
The symmetric bar coalgebra computes the dual coalgebra
\[
\cA^i \;:=\; H^*(\barB_X(\cA)).
\]
...
The main geometric operation composes the Poincar\'e residue of the
normal logarithmic form with the OPE-mode projections of the chiral
product:
\[
d_{\text{residue}}\left(\phi_0 \otimes \cdots \otimes \phi_n \otimes \omega\right)
=
\sum_{0 \leq i < j \leq n}
\sum_{m\ge0}
(-1)^{\sigma_{ij}}\,
\left((\phi_i)_{(m)}\phi_j\right)\otimes
\text{(other factors)}
\otimes
\operatorname{Res}^{\mathrm{form}}_{D_{ij}}(\omega).
\]
where:
\begin{itemize}
\item $\mu: \mathcal{A} \otimes \mathcal{A} \to \mathcal{A}$ is the OPE (chiral product), decomposed into modes $( - )_{(m)}( - )$
\item $D_{ij} \subset \overline{C}_{n+1}(\Sigma_g)$ is the divisor where $z_i = z_j$
\item $\operatorname{Res}^{\mathrm{form}}_{D_{ij}}$ is the Poincar\'e residue of the logarithmic form factor along $D_{ij}$ (see \S\ref{sec:residue-calculus} below)
...
\sigma_{ij}
\equiv
\epsilon_{\mathrm{desusp}}(i,j)
+\epsilon_{\mathrm{or}}(D_{ij})
\pmod 2,
\]
where $\epsilon_{\mathrm{desusp}}(i,j)$ is the Koszul parity of the
shuffle on the desuspended factors $s^{-1}\phi_k$ and
$\epsilon_{\mathrm{or}}(D_{ij})$ is the parity of the boundary
orientation of the collision divisor~$D_{ij}$. This convention is the
one used in the deconcatenation coderivation and descends to the
coshuffle convention after taking coinvariants.
...
\begin{convention}[Length filtration]
...
$\{r^{(ij)}(z_{ij}) = \KZ^{*}(\eta_{ij})\}$ is its $r$-matrix-valued
incarnation. The per-family bar differentials are not independent data: every
bar differential is the pullback of the single universal flat
connection $\nabla_{\mathrm{Arnold}}$ along the structure morphism
induced by $\cA \to \End(B^{\mathrm{ord}}(\cA))$. The unification
is Theorem~\ref{thm:climax-vol1-platonic}.
\end{remark}
...
$a_{(1)}b=\kappa(a,b)|0\rangle$; in that restricted lane we write
...
$d_{\mathrm{curvature}}^2=0$ in this lane. Expanding the total identity
...
bicomplex. On the quadratic current lane this reduces to the familiar
...
\begin{theorem}[Ordered, symmetric, and Harrison bars]\label{thm:three-bar-complexes}
...
\item \textup{(Ordered tensor bar.)}
 ...
 This is the Koszul dual cooperad of\/ $\chirAss$ and is the
 primitive object carrying the ordered $R$-matrix tower.
...
\item \textup{(Acyclicity criterion.)}
 ...
 lanes where the non-symmetric ordered kernel is acyclic. This
...
$H^*(\ker\pi_\Sigma)$. On the PBW/Koszul averaging lane this is the
...
\[
\phi_\alpha(z)\phi_\beta(w) \sim \sum_{\gamma} \sum_{r=0}^{N_{\alpha\beta}}
\frac{C^{\gamma,r}_{\alpha\beta} \partial^r\phi_\gamma(w)}{(z-w)^{h_\alpha + h_\beta - h_\gamma - r}}
+ \text{regular}
\]
where the sum is finite (quasi-finite OPE). Write
\[
P_{\alpha\beta}^{\gamma,r}=h_\alpha+h_\beta-h_\gamma-r
\]
for the pole order of the displayed term. If
\[
\omega=\eta_{i_1j_1}\wedge\cdots\wedge\eta_{i_kj_k}
\]
and $(i,j)=(i_s,j_s)$ occurs once, then the $m$-th pole-mode
summand of the residue differential is
\[
\begin{aligned}
d_{(m),ij}&\bigl(
\phi_{\alpha_1}(z_1)\otimes\cdots\otimes
\phi_{\alpha_{N}}(z_N)\otimes \omega\bigr)  \\
&=
(-1)^{s-1+\epsilon_{\mathrm{Koszul}}}
\sum_{\substack{\gamma,r\\P_{\alpha_i\alpha_j}^{\gamma,r}=m+1}}
C^{\gamma,r}_{\alpha_i\alpha_j}\,
\phi_{\alpha_1}\otimes\cdots\otimes
\partial^r\phi_\gamma\otimes\cdots\otimes
\widehat{\phi_{\alpha_i},\phi_{\alpha_j}}\otimes\cdots
\otimes
\eta_{i_1j_1}\wedge\cdots\wedge\widehat{\eta_{ij}}\wedge\cdots
\wedge\eta_{i_kj_k}.
\end{aligned}
\]
Here $N$ is the number of tensor factors. If no factor $\eta_{ij}$ is
present, the form residue along $D_{ij}$ is zero. The case $m=0$ is
the bracket component; the case $m=1$ is the double-pole component;
all $m\ge2$ are higher-pole components.
The Poincar\'e residue removes the logarithmic form factor
$\eta_{ij}$; it does not impose the condition $P=1$ on the OPE.
The condition $P=m+1$ is instead the choice of OPE mode
$a_{(m)}b$ in the chiral product.
\end{theorem}
...
The OPE gives:
\[
\phi_{\alpha_i}(z_i)\phi_{\alpha_j}(z_j) = \sum_{\gamma,r}
\frac{C^{\gamma,r}_{\alpha_i\alpha_j} \partial^r\phi_\gamma(u)}{(\epsilon e^{i\theta})^{h_{\alpha_i} + h_{\alpha_j} - h_\gamma - r}}
+ O(\epsilon^0)
\]
The form residue extracts the coefficient of $d\log\epsilon$ in the
logarithmic form factor:
\[
\operatorname{Res}^{\mathrm{form}}_{D_{ij}}
(d\log\epsilon\wedge\alpha+\beta)=\alpha|_{D_{ij}}.
\]
Independently, the chiral product is decomposed into its mode
projections; the term of pole order $m+1$ is precisely
$(\phi_{\alpha_i})_{(m)}\phi_{\alpha_j}$. Combining these two
operations gives the displayed formula. The sign is the Koszul sign
for moving the normal logarithmic factor to the first position before
taking the Poincar\'e residue, together with the sign from moving the
two desuspended field factors into the collision position.
\end{proof}
...
\begin{remark}[K3 Maurer--Cartan input]
...
Remark~\ref{rem:theta-universal-MC-k3} satisfy
\[
...
separate arithmetic input. Theorem~\ref{thm:bc-unconditional-depth-reduction}
