...
Desuspend it (the shift $s^{-1}$ with $|s^{-1}v| = |v|-1$, lowering
cohomological degree by one), then form the cofree
conilpotent coalgebra $T^c(s^{-1}\bar\cA)$ with deconcatenation coproduct.
...
What follows is the geometric incarnation of this universal object
on algebraic curves.
...
compactification $\FM_n(X)$ extracts their residues; and the
Arnold relation makes $d^2 = 0$ at genus~$0$.
...
$R$-matrix, the KZ associator, and the full Yangian deformation.
The factorization coalgebra $\barB(\cA) = \mathrm{Sym}^c(s^{-1}\bar\cA)$
on the Ran space $\Ran(X)$ (the ind-scheme of nonempty finite subsets of~$X$)
is its $\Sigma_n$-coinvariant quotient, the
input to the universal Maurer--Cartan element
$\Theta_\cA \in \MC(\gAmod)$ of the modular convolution algebra, governing the genus expansion,
the adjunction (Theorem~A), and the shadow obstruction
...
The full bar differential is the pullback of one universal flat
connection along the KZ functor: at the climax of Volume~I one shows
\[
   \dbar \;=\; \KZ^{*}(\nabla_{\Arn})
\]
in the category $\ConnConf_{\mathbb{P}^1}$ of meromorphic flat
connections on configuration spaces with KZ-type coefficients
...
$\KZ(\cA)$ that pulls $\nabla_{\Arn}$ back to $\dbar$ on
$B^{\mathrm{ord}}(\cA)$. The Arnold three-term identity
...
incarnation. The chapter develops the bar differential in its
three-component form because the form is computationally explicit;
the climax statement records that the form is forced by a single
universal pullback.
...
\cA^i \;:=\; H^*(\barB_X(\cA)).
\]
Thus $\barB_X(\cA)$, $\cA^i$, and $\cA^!$ are distinct objects.

When $\cA$ is Koszul, the bar coalgebra has cohomology concentrated
in bar degree~$1$, so $\cA^i$ is concentrated there. The Koszul dual
\emph{algebra}~$\cA^! = ((\cA^i)^\vee)$ is a
...
$\cA^!$ is the graded linear dual of the concentrated dual
coalgebra~$\cA^i$. In the chiral setting, $\cA^!$ is characterized by
...
$\barB_X(\cA)$ & factorization coalgebra
 & bar coalgebra \\[2pt]
$\cA^i = H^*(\barB_X(\cA))$ & factorization coalgebra
 & dual coalgebra \\[2pt]
$\cA^!$ & chiral algebra
 & $= ((\cA^i)^\vee)$; generators $= \mathcal{V}^\vee$,
 relations $= R^\perp$ \\[2pt]
$\mathbb{D}_{\operatorname{Ran}}\barB_X(\cA)$
 & factorization \emph{algebra}
 & $\simeq \cA^!_\infty$ (Theorem~A) \\[2pt]
...
of Costello--Gwilliam~\cite{CG17}.  Promotion to a BD factorization
coalgebra on the Ran space $\Ran(X)$ is a further step: it
requires a $\cD$-module structure compatible with chiral OPE, the
homotopy-Koszul self-duality of~$\SCchtop$, and the
pro-ambient / weight-completed bar on class-M algebras
\textup{(}cf.\ Vol.~II,
\texttt{theory/factorization\_swiss\_cheese.tex},
Construction for the bar complex as factorization coalgebra\textup{)}.
\end{remark}
...
\subsection{Non-abelian Poincar\'e perspective on bar construction}
...
\begin{remark}[Bar differential as pullback of the Arnold connection]
...
Definition~\ref{def:bar-differential-complete} is not assembled by hand:
it is the pullback of one universal flat connection on the open
configuration space $\Conf_n^{\mathrm{ord}}(X)$ along a faithful
structure morphism induced by $\cA$. This is the structural identity
\textup{(}G2\textup{)} of Chapter~\ref{chap:climax-platonic}:
\[
   d_{\mathrm{bar}} \;=\; \KZ^{*}(\nabla_{\mathrm{Arnold}}),
\]
...
\textup{(}Definition in
\S\ref{ssec:climax-platonic-arnold} of the climax chapter\textup{)}, and
the KZ functor of
...
$\End\bigl(B^{\mathrm{ord}}(\cA)\bigr)$. The three components of $d$
then collapse:
\begin{itemize}
...
\item the de~Rham component $d_{\mathrm{form}}$ is $\KZ^{*}$ of
$d$ in $\nabla_{\mathrm{Arnold}}$;
...
$\{r^{(ij)}(z_{ij}) = \KZ^{*}(\eta_{ij})\}$ is its $r$-matrix-valued
incarnation. Thus the per-family bar differentials are not independent
data: every bar differential in this monograph is the pullback of one
universal flat connection along the unique structure morphism induced
by $\cA \to \End(B^{\mathrm{ord}}(\cA))$. The unification is
Theorem~\ref{thm:climax-vol1-platonic}.
\end{remark}

\begin{remark}[The bar coalgebra over $(\chirAss)^!$, not over
$\mathrm{SC}^{\mathrm{ch,top}}$]
\label{rem:bar-not-sc-coalgebra}
%
\index{Swiss-cheese!emerges on derived centre, not on bar}%
The bar coalgebra $B^{\mathrm{ord}}(\cA) = T^c(s^{-1}\bar\cA)$ is a
single coassociative coalgebra over the chiral cooperad $(\chirAss)^!$
with a single degree-$+1$ differential $d_{\mathrm{bar}}$ and the
deconcatenation coproduct. It is \emph{not} a Swiss-cheese coalgebra:
the $\mathrm{SC}^{\mathrm{ch,top}}$ structure of the programme emerges on
...
the universal Swiss-cheese assembly is treated in Vol~II).
\end{remark}
...
$\kappa$-conductor of $\cA$, which by the climax identity~(G3)
...
The decomposition $\mu = d_{\mathrm{bracket}} + d_{\mathrm{curvature}}$
(Proposition~\ref{prop:pole-decomposition}) separates the contributions:
\begin{itemize}
\item $\Delta_k = 1$ (simple pole): the bracket component $d_{\mathrm{bracket}}$ extracts $(\phi_i)_{(0)}\phi_j = C_{ij}^k \phi_k$
\item $\Delta_k = 2$ (double pole): the curvature component $d_{\mathrm{curvature}}$ extracts the invariant pairing $(\phi_i)_{(1)}\phi_j$
...
The two conventions agree up to an overall normalization constant, absorbed into the definition of the pairing.
...
There is a chain of coalgebra inclusions
...
\item \textup{(Harrison/coLie bar.)}
 $\mathrm{Lie}^c(s^{-1}\bar\cA)$:
 the cofree conilpotent coLie coalgebra on
 $s^{-1}\bar\cA$.
 In degree~$n$ the component is the cofree
 $\operatorname{Lie}^c$-coalgebra on $s^{-1}\bar\cA$,
 with $\dim\operatorname{Lie}(n) = (n{-}1)!$ as an
 $S_n$-representation.
 The coproduct is the coLie cobracket.
...
\item \textup{(Tensor/deconcatenation bar.)}
 $T^c(s^{-1}\bar\cA)$:
 ...
 This is the Koszul dual cooperad of\/ $\chirAss$.
\end{enumerate}
The inclusions in~\eqref{eq:three-bar-inclusions} are
strict in each degree\/ $n \ge 2$: coshuffle has\/
$2^n$ terms vs deconcatenation's\/ $n+1$; coLie is
the weight-$1$ Eulerian summand of coshuffle.
...
\item \textup{(PBW decomposition.)}
 ...
 decomposables.
\item \textup{(Symmetrization quasi-isomorphism.)}
 The symmetrization map
 $\operatorname{sym}\colon
 T^c(s^{-1}\bar\cA) \twoheadrightarrow
 \mathrm{Sym}^c(s^{-1}\bar\cA)$
 is a surjective quasi-isomorphism of dg~coalgebras.
 Its kernel is the space of non-symmetric tensors.
\item \textup{(R-matrix descent.)}
 The $R$-matrix tower on\/ $T^c$ descends to the
 shadow obstruction tower on\/ $\mathrm{Sym}^c$
 via\/ $\operatorname{sym}$, and the Harrison
 subcomplex\/ $\mathrm{Lie}^c$ detects the primitives.
 In the notation of\/
 \S\textup{\ref{subsec:e1-as-primitive}}: at degree\/
 $r$, $\operatorname{sym}(r_r) = S_r$ is the
...
$\mathrm{Lie}^c \subset \mathrm{Sym}^c \subset T^c$,
and the inclusions
in~\eqref{eq:three-bar-inclusions} are inherited from
the cooperad inclusions
$\mathrm{Lie}^c \hookrightarrow
\mathrm{Com}^c \hookrightarrow
\mathrm{Ass}^c$
...
\medskip\noindent
Three inputs have now produced one output. The augmentation ideal of an augmented chiral algebra, the Fulton--MacPherson compactification of its configuration spaces, and the logarithmic kernel $\eta_{ij} = d\log(z_i - z_j)$ assemble into the dg factorization coalgebra $\barB_X(\cA)$, with the Arnold relation enforcing $\dzero^2 = 0$ at genus~$0$. The symmetric bar is the $\Sigma_n$-coinvariant projection of the primitive ordered bar $B^{\mathrm{ord}}_X(\cA) = T^c(s^{-1}\bar\cA)$; the coinvariant loses the $R$-matrix's non-symmetric Fourier modes but preserves cohomology. The cobar--bar adjunction (Chapter~\ref{chap:cobar-construction}) inverts this construction on the Koszul locus; the fiberwise curvature $\dfib^{\,2} = \kappa(\cA)\cdot\omega_g$ (Chapter~\ref{chap:higher-genus}) records its failure at higher genus. The characteristic datum $\kappa(\cA)$ is the residue of that failure at the branch point.

\section{Synthesis}
\label{sec:barconstr-supplement}

\begin{remark}[Hall-theoretic bar construction]
\label{rem:barconstr-1}
The bar construction on the Wave~13 K3 chiral bialgebra takes the form
\[
...
Algebra. The bar differential decomposes as
$d_B = d_{\mathrm{Hall}} + d_{\mathrm{Sieg}} + \hbar\, d_{\mathrm{assoc}} + \hbar^2\, d_{\Phi_{10}/\eta^{24}}$
(Vol.~II, \S\ref{sec:bar-cobar-review-supplement}). The Hall
piece $d_{\mathrm{Hall}}$ is what distinguishes Wave~13 from classical
...
Mukai-lattice grading $\Lambda_{\mathrm{Muk}}$ (see C1~heal below).
Let $d_B = d_{\mathrm{Hall}} + d_{\mathrm{Sieg}} + \hbar d_{\mathrm{assoc}}
+ \hbar^2 d_{\Phi_{10}/\eta^{24}}$ be the bar differential of
Remark~\ref{rem:barconstr-1} and let $d_A$ be the BKM differential on
$\mathbf{H}_{\Delta_5}$ (Chevalley--Eilenberg differential of
$\mathfrak{g}_{\Delta_5}$ lifted to its
Etingof--Kazhdan quantisation; Borcherds 1988 \S 4).
\begin{enumerate}[label=\textup{(\alph*)}]
...
\end{enumerate}
The triple $(\mathfrak{C}_{\mathbf{H}_{\Delta_5}},\, D,\, [\cdot,\cdot])$
...
where $d_A^2 = 0$ by Jacobi for $\mathfrak{g}_{\Delta_5}$ extended to its
Etingof--Kazhdan quantisation (Borcherds 1988 \S 4, Theorem 2;
Kac 1990 \S 2.5), $d_B^2 = 0$ by coassociativity of $\bar\Delta$ combined
with the Arnold relation
$\eta_{ij}\wedge\eta_{jk} + \eta_{jk}\wedge\eta_{ki} +
\eta_{ki}\wedge\eta_{ij} = 0$
on $\mathrm{Conf}_n(X)$ (Theorem~\ref{thm:arnold}), and the
...
\begin{remark}[C1 heal: weight-completion for infinite-dimensional $A$]
...
The heal: work in the weight-completed pro-category
...
This is the chain-level / pro-object / weight-completed ambient of
Pattern~236.
\end{remark}

\begin{remark}[Universal Maurer--Cartan element
$\Theta_{\mathbf{H}_{\Delta_5}}$; \ClaimStatusProvedHere]
\label{rem:theta-universal-MC-k3}
The universal obstruction element solving the
Maurer--Cartan equation on $\mathfrak{C}_{\mathbf{H}_{\Delta_5}}$ is
\[
...
sense of Etingof--Kazhdan. The leading
coefficient at $\hbar^3$ is the triple-$\zeta$--Borcherds MZV leg:
\[
  \phi^{(3)}
  \;=\;
...
\]
matching Vol.~III
Theorem~\ref{thm:pentagon-sieg-bor}. The next nontrivial correction at
$\hbar^6$ (the first MC-forced higher coherence) reads
...
representatives of Wave~18. The $\hbar^3$ equation
$D\phi^{(3)} = 0$ is the pentagon equation of
Theorem~\ref{thm:pentagon-sieg-bor}; the $\hbar^4$ equation
$D\phi^{(4)} + \tfrac{1}{2}[\phi^{(3)},\phi^{(3)}]_{\hbar^4} = 0$
is the first genuine super-Jacobi coherence and forces
$\phi^{(4)} = 0$ (by a parity argument: $[\phi^{(3)},\phi^{(3)}]$
lives in the symmetric square and integrates via MZV-shuffle
Brown 2012 \S 3 into $\hbar^6$, not $\hbar^4$). Subsequent MC
equations at $\hbar^{2k}$ are the higher coherence tower of
Drinfeld~1990 Problem 3.16.
\end{remark}

\begin{theorem}[MC verification at $\hbar^3,\hbar^4$;
\ClaimStatusProvedHere]
\label{thm:MC-hbar3-hbar4-k3}
On the weight-completed dGLA
$\mathfrak{C}_{\mathbf{H}_{\Delta_5}}[[\hbar]]$ of
Definition~\ref{def:convolution-dgla-k3} with universal element
$\Theta_{\mathbf{H}_{\Delta_5}}$ of
Remark~\ref{rem:theta-universal-MC-k3}, the Maurer--Cartan equation
\[
...
$D\phi^{(3)} = 0$. The three summands each satisfy
$D\, c = 0$ because $c_{\mathrm{symm}}, c_{\mathrm{timelike}}, c_{\Phi_{10}}$
are the three independent $3$-coboundaries of the
Chevalley--Eilenberg complex
$C^3(\widehat{\mathfrak{t}^{\mathrm{Sieg}}_{2,[2]}\oplus
\mathfrak{n}_+^{\mathrm{imag}}})$ (Enriquez--Gomez-Gonzalez--Maassarani 2022
Theorem~4.3), which map to $d_A$-closed elements under the BKM
quantisation functor by Etingof--Kazhdan naturality.
At $\hbar^4$: $D\phi^{(4)} + \tfrac{1}{2}[\phi^{(3)},\phi^{(3)}]_{\hbar^4}
= 0$. The commutator
$[\phi^{(3)},\phi^{(3)}]$ produces only $\hbar^6$ contributions
(both legs carry $\hbar^3$; $\hbar^3\cdot\hbar^3 = \hbar^6$), so
the $\hbar^4$ component of the commutator is zero, and
$D\phi^{(4)} = 0$ with $\phi^{(4)}=0$ satisfies the equation
trivially. The absence of $\phi^{(4)},\phi^{(5)}$ terms reflects
the Etingof--Kazhdan even-only pentagon cocycle support
(Etingof--Kazhdan 1996 second paper, Proposition 3.2). The first nonzero
higher correction lives at $\hbar^6$ and is verified in
Volume~III Chapter~\ref{ch:k3-chiral-bialgebra-platonic},
Theorem~\ref{thm:kcb-phi-arrow-discipline} and compute module
\texttt{compute/lib/mzv\_shuffle\_borcherds\_phi6\_k3.py}.
\end{proof}

\begin{theorem}[MC verification through weight $\hbar^{12}$;
\ClaimStatusProvedHere]
\label{thm:MC-hbar7-hbar12-k3}
On the weight-completed pro-dGLA
$\mathfrak{C}_{\mathbf{H}_{\Delta_5}}[[\hbar]]$, the universal element
$\Theta_{\mathbf{H}_{\Delta_5}}$ of
Remark~\ref{rem:theta-universal-MC-k3} satisfies the
Maurer--Cartan equation modulo $\hbar^{13}$. The MC tower is supported on
the sub-semigroup $3\mathbb{Z}_{\ge 1}$: $\phi^{(n)} = 0$ for
$n\not\in\{3,6,9,12,\ldots\}$. At $n = 9$ the depth-$3$ MZV
$\zeta(3,3,3)$ enters for the first time via the Brown $2012$ stuffle
$\zeta(3)\cdot\zeta(3,3) = \zeta(3,3,3) + \zeta(3,6) + \zeta(6,3)$; at
$n = 12$ the depth-$4$ MZV $\zeta(3,3,3,3)$ enters via the iterated Brown
shuffle, realising the Brown motivic-Galois horizon at weight $12$.
\end{theorem}

\begin{proof}
The $3\mathbb{Z}$-parity of the non-vanishing $\phi^{(n)}$ follows from
Etingof--Kazhdan $1996$ even-tensor-degree pentagon support
(Proposition $3.2$ loc.\ cit.) together with the Siegel--Borcherds
Manin-pair shift that promotes Drinfeld $1990$ $2\mathbb{Z}$-parity to
$3\mathbb{Z}$-parity through the minimal Humbert-divisor weight of
$(\Phi_{10}/\eta^{24})$ (Bruinier $2002$ Proposition $5.1$; weight $\ge 3$
Heegner-divisor Chern-class reciprocity). Hence $\phi^{(n)} = 0$ for
$n\in\{4,5,7,8,10,11\}$.

At $n\in\{7,8,10,11\}$ every MC bracket-summand
$[\phi^{(i)},\phi^{(j)}]$ with $i+j = n$ and $3\le i\le j\le n-3$
involves a vanishing factor, so the right side is zero and
$\phi^{(n)} = 0$ solves the equation. At $n = 9$ the only contributing
bracket is $[\phi^{(3)},\phi^{(6)}]$, and matching scalar coefficients
via Brown $2012$ motivic stuffle (\S $4.3$ Corollary $4$) yields a
nine-term MZV--Borcherds-leg decomposition including the depth-$3$
MZV $\zeta(3,3,3)$. At $n = 12$ the brackets $[\phi^{(3)},\phi^{(9)}]$
and $\tfrac{1}{2}[\phi^{(6)},\phi^{(6)}]$ contribute; the iterated Brown
shuffle $\zeta(3)\cdot\zeta(3,3,3) = \zeta(3,3,3,3) + \zeta(3,3,6) +
\zeta(3,6,3) + \zeta(6,3,3)$ and the quadruple shuffle
$\zeta(3)^4$ both unfold in the Brown $2012$ depth-$4$ weight-$12$
motivic MZV basis $\{\zeta(3,3,3,3),\zeta(3,3,6),\zeta(3,9),\zeta(6,6),
\zeta(9,3),\zeta(12)\}$. The twelve pentagon coefficients
$c_{12}^{(i)}$ are the EGGM $2022$ $C^3\to\cdots\to C^5$ weight-$12$
representatives, whose consistency under the motivic coaction is Brown
$2012$ \textup{Ann.~Math.}~$175$ Theorem $1$ at weight $12$. The full
per-order verification (stuffle arithmetic, depth filtration,
Borcherds-leg shuffle) is recorded in Volume~III
Theorem~\ref{thm:kcb-MC-hbar7-hbar12} and in the compute modules
\texttt{compute/lib/mzv\_stuffle\_depth3\_phi9\_k3.py} and
\texttt{compute/lib/mzv\_shuffle\_depth4\_phi12\_k3.py}.
\end{proof}
...
$D_n = (n-3)/2$ against the Jacobi-index-$1$ lattice $4NM - \ell^2$
...
polar-support question, and the polar cutoff of
Eichler--Zagier $1985$ supplies an unconditional answer.

\begin{lemma}[Polar support of the K3 elliptic genus; cross-volume
reference]
...
$\Delta = 4N - \ell^2$, with
$C(-1) = 2$, $C(0) = 20$, and
$C(\Delta) = 0$ for all $\Delta < -1$. Proved in Volume~III
Lemma~\ref{lem:polar-support-phi-K3}; primary source
Eichler--Zagier $1985$ \textup{Prog.~Math.}~$55$ Theorem~$9.1$
(index-$m$ weak Jacobi forms vanish in discriminants below $-m^2$, here
$m = 1$).
\end{lemma}

\begin{theorem}[Unconditional depth-reduction of the pentagon tower on
the K3 side]
\label{thm:bc-unconditional-depth-reduction}
\ClaimStatusProvedHere
Consider the pentagon $A_\infty$-tower $\{\phi^{(n)}\}_{n\ge 3}$ of
$\mathbf{H}_{\Delta_5}$ on the $A_\infty$-Humbert regime
(Volume~III
Proposition~\ref{prop:A-infty-structure-constants-humbert}), where
\[
  \phi^{(n)} \;=\; \sum_{\substack{(N, \ell, M)\\ 4NM - \ell^2 = -D_n}}
    c_{\Phi_{10}/\eta^{24}}(N, \ell, M)\,
    [\mathrm{generator}]^{\otimes n},
  \qquad D_n = (n-3)/2.
\]
Admissibility $D_n \bmod 4 \in \{0, 1\}$, equivalently
...
\item $n = 3$: $\phi^{(3)} \ne 0$, the Etingof--Kazhdan pentagon
  cocycle of Theorem~\ref{thm:pentagon-sieg-bor}
  (Volume~III).
...
\begin{proof}
The admissibility congruence $D_n \bmod 4 \in \{0, 1\}$ is the
discriminant form's signature: $4NM - \ell^2 \equiv -\ell^2 \pmod 4$
takes values in $\{0, -1\} \pmod 4$, so $-D_n$ is representable by the
form iff $D_n \in \{0, 1\} \pmod 4$. Combined with the odd-$n$
constraint (else $D_n \notin \mathbb{Z}$), this is $n \equiv 3, 5
\pmod 8$.

Case $n = 3$: $D_3 = 0$, the lattice sum has representative
...
Case $n = 5$: $D_5 = 1$, lattice representative $(0, 1, 0)$ gives
$\Delta = -1$; $C(-1) = 2 \ne 0$; the lattice sum is one-term and
$\phi^{(5)} = 2\,[\mathrm{generator}]^{\otimes 5}$.

Case $n \ge 11$ admissible ($n \in \{11, 13, 19, 21, 27, 29, \ldots\}$):
$D_n \ge 4$, so every lattice representative $(N, \ell, M)$ of
$-D_n$ has $\Delta = -D_n \le -4$. By
...
Case $n$ non-admissible: the congruence $D_n \in \{0, 1\} \pmod 4$
fails, so there exists no $(N, \ell, M)\in\mathbb{Z}_{\ge 0}\times
\mathbb{Z}\times\mathbb{Z}_{\ge 0}$ with $4NM - \ell^2 = -D_n$; the
lattice sum is empty and $\phi^{(n)} = 0$ by vacuity.

The unconditionality statement: Brown $2012$ \textup{Ann.~Math.}~$175$
Theorem~$1$ establishes the motivic-MZV basis through weight $12$, and
Theorem~\ref{thm:MC-hbar7-hbar12-k3} of this chapter uses that input
to close the MC tower in the strict quasi-Hopf regime through
$\hbar^{12}$. Higher weights $n \ge 13$ in the strict regime require
the Zagier--Hoffman conjecture on the motivic-depth filtration (Hoffman
$1997$ \textup{Pac.~J.~Math.}~$194$; Zagier $1994$
\textup{Prog.~Math.}~$120$; unproved beyond Brown's horizon). The
present theorem bypasses this entirely on the $A_\infty$-Humbert regime:
the vanishing of $\phi^{(n)}$ for admissible $n \ge 11$ comes from
Eichler--Zagier $1985$ Theorem~$9.1$, which is unconditional and
structural to the theory of index-$1$ weak Jacobi forms, reaching all
$n$ with $D_n > 1$.
\end{proof}

\begin{remark}[Contrast with Brown's weight-$12$ horizon]
\label{rem:bc-brown-horizon-contrast}
Theorem~\ref{thm:MC-hbar7-hbar12-k3} closes the strict quasi-Hopf
pentagon tower modulo $\hbar^{13}$ through the motivic MZV basis at
weight $12$, whose existence is Brown $2012$
\textup{Ann.~Math.}~$175$ Theorem~$1$. Weights $15$ and beyond
require the depth-$5$ motivic generator $\zeta(3,3,3,3,3)$ and
depend on the Zagier--Hoffman conjectural depth filtration.
Theorem~\ref{thm:bc-unconditional-depth-reduction} is orthogonal: on
the $A_\infty$-Humbert regime the vanishing is polar-cutoff driven
and reaches arbitrarily high $n$, without any motivic-basis input. The
K3-side cross-volume reference is Volume~III
Theorem~\ref{thm:filtration-sharpness} (filtration sharpness), where
the admissibility table through $n \le 30$ is recorded with explicit
Jacobi-lattice representatives $(N, \ell, M)$ and Heegner coefficients
$H_n = C(-D_n)$.
\end{remark}
