...
(Theorem~B), and the shadow obstruction tower $\Theta_\cA^{\leq r}$
...
curvature of the modular bar differential).  The cobar functor inverts
the bar: $\Omega \circ \barB \xrightarrow{\sim} \mathrm{id}$
on the Koszul locus
...
\cA^!_\infty$ (Convention~\ref{conv:bar-coalgebra-identity}).
The curved genus-$g$ deformation, where the seed is
...
\item \textup{(Harrison/coLie bar.)}
  ...
  $s^{-1}\bar\cA$.
  In arity~$n$ the component is the cofree
...
strict in each arity\/ $n \ge 2$: coshuffle has\/
...
\item \textup{(R-matrix descent.)}
  ...
  In the notation of\/
  \S\textup{\ref{subsec:e1-as-primitive}}: at arity\/
  $r$, $\operatorname{sym}(r_r) = S_r$ is the
  $E_\infty$-shadow at arity\/~$r$
  \textup{(}$\kappa$ at\/ $r = 2$;
...
$\mathrm{Lie}^c(s^{-1}\bar\cA)$ in arity~$n$.
...
\otimes \mathcal{O}(*\Delta)$ is the arity-$r$
...
$r_r$ is the arity-$r$ shadow $S_r$ on the
...
In this chapter the ``bar complex''
$\bar{B}^{\mathrm{geom}}(\mathcal{A})$ uses item
\textup{(ii)} of Theorem~\textup{\ref{thm:three-bar-complexes}}:
the coproduct
(Theorems~\ref{thm:bar-coalgebra}--\ref{thm:coassociativity-complete})
is the coshuffle coproduct on
$\mathrm{Sym}^c(s^{-1}\bar{\mathcal{A}})$, matching the
geometric decomposition of $\overline{C}_{n+1}(X)$ into
unordered collision strata.
\end{remark}
