...
The universal MC element $\Theta_\cA \in \MC(\gAmod)$ governs the
entire genus expansion; its construction requires a coalgebraic
input: the factorization coalgebra $\barB(\cA)$ on $\Ran(X)$.
This chapter builds that input.  The bar complex is the tool that
converts the multiplicative OPE of a chiral algebra into a
nilpotent differential, encoding all OPE data as collision residues
on Fulton--MacPherson configuration spaces.  The universal twisting
morphism $\tau_\cA$, the adjunction (Theorem~A), the inversion
(Theorem~B), and the shadow Postnikov tower $\Theta_\cA^{\leq r}$
all pass through $\barB(\cA)$; nothing in the modular theory is
accessible without it.

Concretely, $\barB(\cA)$ is an integral transform over $\FM_n(X)$
whose kernel is the logarithmic 1-form
$\eta_{ij} = d\log(z_i - z_j)$ of \S\ref{sec:the-seed}.  The
output is a factorization coalgebra on $\operatorname{Ran}(X)$ with
$d^2 = 0$ at genus~$0$ (by the Arnold relation) and
$d^2 = \kappa(\cA) \cdot \omega_g$ at genus~$g \geq 1$ (by the
curvature of the modular bar differential).  Verdier duality inverts
the transform: $\Omega \circ \barB \xrightarrow{\sim} \mathrm{id}$
on the Koszul locus
(Theorem~\ref{thm:bar-cobar-inversion-qi}), recovering the algebra
from its nilpotent image (Remark~\ref{rem:nilpotence-periodicity}).
...
logarithm determines the full scalar package on the proved scalar
lane, while the general multi-weight extension remains the open
tautological-purity problem.
...
residue along~$D_{ij}$ extracts only simple poles.  The full bar
...
product~$\mu$.  For algebras whose OPE has no simple pole
