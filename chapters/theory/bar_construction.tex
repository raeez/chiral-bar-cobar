...
$\Sigma_\bullet$-descent datum compatible with the OPE residue
differential, the de~Rham differential, and deconcatenation. In the
pole-free symmetric case this is ordinary equivariant descent. In the
with-poles $\Eone$-chiral case it is the strong-unitary
...
=0 .
\end{equation}
The scalar attached to the same degree-two window is the averaged trace
of the tensor coefficient of~$dz/z$:
...
$V\otimes V$ coefficient of $dz/z$ against the same trace pairing.
...
residue is $k\,J\otimes J\,dz/z$ and the averaged trace is~$k$.
\end{proof}
...
by moving the logarithmic normal form \(d\log(z_i-z_j)\) through the
preceding coefficient factors before applying the Poincare residue.
