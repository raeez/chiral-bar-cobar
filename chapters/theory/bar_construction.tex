...
logarithmic form records the bar degree; the residue is the OPE
coefficient itself.
\end{computation}
...
\emph{unreduced} bar complex. The canonical bar complex used in the
theorems below is the \emph{reduced} bar
$\bar{B}(\mathcal{A}) = T^c(s^{-1}\bar{\mathcal{A}})$ built on the
augmentation ideal.
...
$D\, c = 0$ because $c_{\mathrm{symm}}, c_{\mathrm{timelike}}, c_{\Phi_{10}^{\mathrm{un}}}$
...
$(\Phi_{10}^{\mathrm{un}}/\eta^{24})$ (Bruinier $2002$ Proposition $5.1$; weight $\ge 3$
