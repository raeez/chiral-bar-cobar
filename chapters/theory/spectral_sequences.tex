...
The classical bounded convergence theorem
(Theorem~\ref{thm:classical-convergence}) leaves open the case of
complete but unbounded filtrations. Two refinements close this gap: a
regularity criterion that reduces to first-quadrant vanishing, and a
$\varprojlim^1$-vanishing criterion that handles the completed case
arising from pro-nilpotent Lie algebras.
...
\medskip
The bar differential on a chiral algebra decomposes by genus: the
component $d_k$ raises genus by~$k$. The resulting filtration
separates the classical ($g = 0$) bar cohomology from the quantum
corrections ($g \geq 1$), producing a spectral sequence whose
differentials $d_r$ encode exactly the $r$-loop contributions.
...
\begin{proposition}[Central charge and \texorpdfstring{$d_1$}{d1}; \ClaimStatusConditional]
...
\begin{computation}[Chevalley--Cousin for \texorpdfstring{$\FM_3$}{}]
\label{comp:cc-fm3}
...
The Steenrod algebra $\cA$ is the universal coefficient ring for stable
cohomology operations; its Ext groups detect homotopy groups through a
filtration analogous to the bar filtration on chiral Hochschild cochains.
In the factorization-homology framework of Costello--Gwilliam, the chiral
bar spectral sequence of Theorem~\ref{thm:bar-ss} is the vertex-algebra
analogue of the Adams $E_2$-page, with the residue differential $d_1$
playing the role of the primary Steenrod operations.
\end{example}
