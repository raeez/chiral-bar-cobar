...
\end{equation}
Substituting into the displayed asymptotic for $S_{g-1}(n)$:
\begin{equation}
\label{eq:s-leading}
...
$3617 \mid Z_9$ together pin $Z_g(k)$ to a single polynomial for
...
\section{Arithmetic comparison with the Virasoro residue tower}
\label{sec:zg-arithmetic-duality}

The Verlinde computation above is complete on its own geometric
surface.  It supplies the exact Bernoulli coefficient
\[
 [n^{2(g-2)}]R_{g-2}(n^2)
 =(-1)^g\frac{2^{g-1}B_{2g-2}}{(2g-2)!}.
\]
A comparison with a Virasoro higher-residue sequence requires three
additional data: an ordered residue package
\(\mathsf H_{\mathrm{res}}\), an integral normalization of its scalar
line, and a Kummer comparison
\(\mathsf H_{\mathrm{Kum}}\) relating that line to the integral
Bernoulli coefficient.  These data define the meaning of prime
comparison across the two constructions.

\begin{theorem}[Bernoulli witnesses and the residue comparison problem]
\label{thm:z-g-s-r-arithmetic-duality}
\ClaimStatusConditional
\emph{Type signature:}
\textup{(Open/modular comparison, Verlinde and ordered-residue
presentations, Beilinson levels \(2\leftrightarrow5\),
\(\mathsf H_{\mathrm{res}}+\mathsf H_{\mathrm{Kum}}\)).}
The Verlinde lane has the exact reduced coefficients
\[
 [n^{18}]Z_7(k)=\frac{691}{20432412000},
 \qquad
 [n^{24}]Z_9(k)=\frac{3617}{41682120480000}.
\]
Consequently \(691\) and \(3617\) are the first two displayed
Bernoulli-numerator witnesses on this lane.  If
\(\mathsf H_{\mathrm{res}}+\mathsf H_{\mathrm{Kum}}\) identifies an
integral Virasoro residue coordinate with these genus slots, then its
prime divisibility is obtained by transporting these two primitive
classes through that comparison.

A finite formal Virasoro recurrence may be reduced modulo the same
primes as a separate exact computation.  Equality of that formal
sequence with the geometric residue sequence is part of
\(\mathsf H_{\mathrm{res}}\).
\end{theorem}

\begin{proof}
The two rational coefficients follow from
Theorem~\ref{thm:z-g-leading-coefficient-bernoulli} and
\(B_{12}=-691/2730\), \(B_{16}=-3617/510\), followed by rational
reduction.  The final assertion is functoriality of divisibility under
the integral comparison specified by \(\mathsf H_{\mathrm{Kum}}\).
\end{proof}

\begin{remark}[Geometric provenance]
\label{rem:localization-disjointness}
The Verlinde coefficient is a characteristic number on the moduli
space of flat \(\mathrm{SU}(2)\)-connections.  A Virasoro higher
coefficient is extracted from an ordered chiral residue model.  The
package \(\mathsf H_{\mathrm{Kum}}\) is therefore a correspondence:
it consists of a chain map, an integral lattice on the scalar target,
and compatibility with genus normalization.  Once constructed, it
turns the two proven local computations into a theorem on one common
arithmetic object.
\end{remark}

\begin{remark}[Higher witnesses]
\label{rem:zg-sr-duality-extensions}
Higher Bernoulli numerators are governed by the same exact Verlinde
formula.  Their comparison with Laurent tiers of a residue sequence
requires tierwise integral lattices and reduction maps.  This is the
natural extension of \(\mathsf H_{\mathrm{Kum}}\).
\end{remark}

\begin{remark}[Verification surface]
\label{rem:zg-verify-sr-duality}
The Verlinde side has three independent checks: the cosecant power
sum, Witten's large-level symplectic volume, and interpolation of the
exact polynomial.  The residue side is checked by direct ordered
contraction and by its formal recurrence after
\(\mathsf H_{\mathrm{res}}\) has identified the two sequences.  The
integral comparison is verified prime by prime on primitive
normalizations.
\end{remark}
