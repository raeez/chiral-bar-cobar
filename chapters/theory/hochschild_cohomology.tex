...
topological Hochschild $\mathrm{HH}^*_{\mathrm{top}}$ takes an
$E_1$-algebra in spectra and outputs an $E_2$-Gerstenhaber algebra
\textup{(}Deligne\textup{)}. The categorical Hochschild
$\mathrm{HH}^*_{\mathrm{cat}}$ takes a dg category and outputs an
$E_2$-algebra carrying CY shifted Poisson structure. The geometry
selects which Hochschild: a curve produces $\ChirHoch^*$, a formal
disk over a point produces $\mathrm{HH}^*_{\mathrm{top}}$, a CY dg
category produces $\mathrm{HH}^*_{\mathrm{cat}}$. The present
chapter works exclusively with $\ChirHoch^*$; the other two appear
only in explicit comparison statements
\textup{(}Theorem~\textup{\ref{thm:hochschild-classical-comparison}}
records the spectral sequence relating $\mathrm{HH}^*_{\mathrm{top}}$ at
the fibre to $\ChirHoch^*$ on the curve\textup{)}.
\end{remark}
...
The definitions and sign conventions for classical Hochschild cohomology
$\mathrm{HH}^*$, classical Hochschild homology $\mathrm{HH}_*$, cyclic homology
...
Other chapters that use chiral Hochschild, topological Hochschild, or cyclic invariants
...
(Gel'fand--Fuchs~\cite{FF84}). This is \emph{not} the
...
Instead, the Beilinson--Drinfeld comparison theorem
(\cite{BD04}, Theorem~4.5.2) identifies the chiral Hochschild
