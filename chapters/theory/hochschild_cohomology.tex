...
decoration.  The shadow obstruction tower of
...
Hochschild computation.  For all chirally Koszul algebras,
$\ChirHoch^n(\cA) = 0$ for $n > 2$ (Theorem~H): the Heisenberg has
...
The exterior-polynomial algebra above is the continuous Lie algebra
cohomology, which extends into arbitrarily high degrees.  The chiral
Hochschild cohomology $\ChirHoch^*(\widehat{\mathfrak{g}}_{\critLevel})$
on a curve is bounded: for the universal algebra $V_{\critLevel}(\fg)$,
which is chirally Koszul (Proposition~\ref{prop:pbw-universality}),
Theorem~H gives $\ChirHoch^n = 0$ for $n > 2$.  At critical level
the center $Z(V_{\critLevel}) = \mathfrak{z}(\widehat{\fg})$ is the
Feigin--Frenkel center (a polynomial algebra of rank~$r$), so
$\dim \ChirHoch^0 = \infty$.  The two cohomology theories agree in
low degrees but diverge above degree~$2$ (the de~Rham bound on a
curve truncates the Lie-algebraic computation).
\end{remark}
...
\subsection{W-algebra Hochschild cohomology}

For W-algebras at generic level, Theorem~H gives concentration in $\{0, 1, 2\}$:

\begin{theorem}[W-algebra Hochschild cohomology; \ClaimStatusProvedHere]\label{thm:w-algebra-hochschild}
...
related to the Hochschild-cyclic spectral sequence: the $E_1$ page
...
\smallskip\noindent
${}^\dagger$\,The $E_2^{0,*}$ column records Hochschild \emph{homology}
$\mathrm{HH}_*^{\mathrm{ch}}(\cA)^{S^1}$ (the Tor computation), which
is polynomial for the Virasoro.  This is distinct from Hochschild
\emph{cohomology} $\ChirHoch^*(\cA)$ (the Ext computation), which is
concentrated in $\{0,1,2\}$ by Theorem~H
