...
Theorem~H (Hochschild concentration in degrees $\{0,1,2\}$) applies to
the Wave~13 K3 chiral bialgebra: the chiral Hochschild cohomology
\[
\mathrm{ChirHoch}^\bullet(\mathbf H_{\Delta_5})\bigm|_{\mathrm{Hall}(\mathrm{CoHA}_{K3\times E})}
\]
is concentrated in degrees $0$, $1$, $2$, with degree-$2$ component
classified by
\[
H^2(\mathfrak g_{\Delta_5})^{\mathbb Z/2,\, K(1)} \;\cong\; \mathbb C\cdot \Delta_5
\]
(Etingof-Kazhdan 2007 Part~V). Degree $0$ is the centre
$Z(\mathbf H_{\Delta_5})$ generated by the Casimir + the Igusa cusp
form, degree $1$ is the derivation algebra (outer derivations from
BKM geometry), and degree $2$ is the one-dimensional Koszul class of
$\Delta_5$. The concentration is the Wave~13 realisation of the
general chiral Hochschild concentration theorem, providing a
finite-dimensional handle on the derived-centre algebra. Cross-ref
Vol.~III, Chapter~\ref{ch:k3-chiral-bialgebra-platonic}, Theorem~H.
\end{remark}
