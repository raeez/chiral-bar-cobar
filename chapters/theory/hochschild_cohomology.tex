...
element $\Theta_\cA$ lives: the tangent complex to the MC
...
higher brackets record coherent homotopies governing the
...
shadow of the modular operad's gluing maps, not a homological
decoration.  The periodicity phenomena (Virasoro period~$2$,
$\mathcal{W}_N$ period~$N$) are consequences of the modular structure
of the cyclic complex, and the shadow obstruction tower of
Chapter~\ref{sec:koszul-across-genera} projects onto the cyclic
deformation complex $\mathrm{Def}_{\mathrm{cyc}}^{\mathrm{mod}}(\cA)$
defined here.
\emph{Examples.}  Every family in the standard landscape receives its
Hochschild computation.  The Heisenberg complex is acyclic in positive
degree (Gaussian termination); the Virasoro complex exhibits
$2$-periodic cohomology
(Theorem~\ref{thm:virasoro-hochschild}); W-algebras inherit
$N$-periodicity from free-field resolutions.  These are the
deformation-theoretic counterparts of the shadow archetypes in
Part~II.
\emph{Closing the loop.}  The chiral Hochschild complex is the
...
(Definition~\ref{def:modular-cyclic-deformation-complex}) housing
...
Hochschild cohomology organizes all infinitesimal deformations and
obstructions of a chiral algebra into a single graded object
equipped with a Gerstenhaber bracket.  The chiral enhancement
incorporates OPE data, the geometry of the curve, and the $A_\infty$
structure of the bar complex.
