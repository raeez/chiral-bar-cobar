...
The homotopy perspective is essential here.  Hochschild complexes
...
$\Convinf(\barB(\cA), \cA)$
(Theorem~\ref{thm:operadic-homotopy-convolution}).  Thus this chapter
is not a return to strict cochains but an entrance into the
open/closed and bulk/boundary dictionary, where Koszul duality,
Hochschild theory, and factorization geometry begin to merge.
...
The open/closed dictionary is now sharp.  Two complementary algebraic
objects govern the theory:
\begin{enumerate}[label=\textup{(\alph*)}]
...
by the same argument.  The correct converse is at the level of
convolution formality
(Theorem~\ref{thm:convolution-formality-one-channel}).
\end{remark}

\begin{theorem}[Convolution formality $=$ one-channel;
\ClaimStatusProvedHere]
\label{thm:convolution-formality-one-channel}
\index{formality!convolution algebra}
\index{one-channel!formality equivalence}
...
Then $\gAmod$ is formal as a quantum $L_\infty$-algebra if and
only if $\cA$ is \emph{one-channel}
\textup{(}scalar-saturated\textup{)}:
...
\begin{proof}
\emph{($\Leftarrow$: one-channel implies formality.)}
...
For the full quantum formality, observe that the shadow
...
This gives formality of $\gAmod$ as a quantum $L_\infty$-algebra.

\emph{($\Rightarrow$: formality implies one-channel.)}
Suppose $\phi \colon \gAmod \xrightarrow{\;\sim\;}
H^*(\gAmod)$ is a quantum $L_\infty$ quasi-isomorphism
(formality).  The MC element $\Theta_{\cA} \in \MC(\gAmod)$
transfers to $\phi_*(\Theta_{\cA}) \in \MC(H^*(\gAmod))$.
By PBW concentration
(Theorem~\ref{thm:master-pbw}), the Koszulness of~$\cA$
forces the primitive cyclic deformation cohomology
$H^1_{\mathrm{cyc,prim}}(\Defcyc^{\mathrm{mod}}(\cA))$
to be one-dimensional, spanned by $[\eta \otimes \Lambda]$.
More precisely, algebraic-family rigidity
(Theorem~\ref{thm:algebraic-family-rigidity})
establishes $H^2_{\mathrm{cyc,prim}}(\cA) = 0$
via Whitehead reduction and algebraic semicontinuity,
so the only MC element in $H^*(\gAmod)$ in the primitive
sector is of the form $\kappa' \cdot \eta \otimes \Lambda$
for some scalar~$\kappa'$.
Thus $\phi_*(\Theta_{\cA}) = \kappa' \cdot \eta \otimes \Lambda$.

Now let $\psi \colon H^*(\gAmod) \xrightarrow{\;\sim\;} \gAmod$
be an $L_\infty$ quasi-inverse of~$\phi$.
The composite $\psi_* \circ \phi_*$ acts on MC elements as
gauge equivalence
(by the standard MC--homotopy transfer theorem),
so $\Theta_{\cA}$ is gauge-equivalent to
$\psi_*(\kappa' \cdot \eta \otimes \Lambda)$.
Since $\eta \otimes \Lambda$ is a \emph{strict} cocycle
(it lies in the kernel of~$d$ and of all higher brackets),
$\psi_*(\kappa' \cdot \eta \otimes \Lambda)
= \kappa' \cdot \eta \otimes \Lambda$: the
quasi-isomorphism acts as the identity on strict cocycles
in the one-dimensional primitive cohomology.
Therefore $\Theta_{\cA}$ is gauge-equivalent to
$\kappa' \cdot \eta \otimes \Lambda$.
Finally, by the rigidity of the scalar sector (the
gauge stabilizer of $\eta \otimes \Lambda$ acts trivially
on the primitive component, since
$H^0_{\mathrm{cyc,prim}}(\cA) = 0$ in the relevant
bidegree), gauge equivalence to a scalar MC element
forces equality: $\Theta_{\cA} = \kappa' \cdot
\eta \otimes \Lambda$, and
$\kappa' = \kappa(\cA)$ by
Corollary~\ref{cor:shadow-extraction}.
\end{proof}

\begin{remark}[Convolution formality and scalar saturation]
...
If $\gAmod$ is formal, the inclusion
$H^*(\gAmod) \hookrightarrow \gAmod$ extends to a quantum
$L_\infty$ quasi-isomorphism.  This quasi-isomorphism transports
$\Theta_{\cA}$ to an MC element in the \emph{cohomology}
$H^*(\gAmod)$, where it is determined by its linear data alone.
That linear datum is the scalar $\kappa(\cA)$: the modular
characteristic (Theorem~D) is the projection of $\Theta_{\cA}$
to arity~$2$, genus~$1$
(Corollary~\ref{cor:shadow-extraction}).  Formality forces the
full MC element to be reconstructible from this projection, which is
precisely scalar saturation~\eqref{eq:one-channel-formality}.
...
$\Theta_{\cA}$ from the one-channel line.  In the language of the
...
the non-formal corrections.  If the log Fulton--MacPherson
compactification $\mathrm{FM}_n(X \mid D)$ contributes trivially
from planted-forest strata, then $\gAmod$ is formal and $\cA$
is one-channel.
...
\item \emph{Modular Koszulness} (all genera): MK3 extends PBW
  ...
  in the modular bar complex at $g \geq 1$.

\item \emph{Convolution formality} (structural): the quantum
  $L_\infty$-structure on $\gAmod$ is formal, equivalent to
  scalar saturation by
  Theorem~\ref{thm:convolution-formality-one-channel}.
\end{enumerate}
The hierarchy is
$\text{(c)} \Rightarrow \text{(b)} \Rightarrow \text{(a)}$,
and neither implication reverses: the first gap is the non-scalar
shadow tower (\S\ref{subsec:non-scalar-theta}); the second is the
...
(chromatic filtration, prefundamental Clebsch--Gordan via multiplicity-free $\ell$-weights,
Francis--Gaitsgory completion, Dwyer--Kan on compacts) is a one-slot operation.
The all-types generalization proceeds
one slot at a time; the decomposition property of part~(ii)
explains why this is sufficient.
\end{remark}
