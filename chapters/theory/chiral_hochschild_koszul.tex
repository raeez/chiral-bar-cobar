...
derived critical locus conjecture
(Conjecture~\ref{conj:derived-critical-locus-chk}).
\end{remark}
...
The Chriss--Ginzburg principle organizes this subsection: the modular
...
(Conjecture~\ref{conj:convolution-formality-one-channel}).
\end{remark}

\begin{conjecture}[Convolution formality $=$ one-channel;
...
\begin{remark}[The Chriss--Ginzburg content of convolution formality]
\label{rem:convolution-formality-cg}
\index{Chriss--Ginzburg principle!convolution formality}
This is the central conjecture of the subsection.  Its content:
...
\begin{conjecture}[Bifunctor obstruction decomposition;
\ClaimStatusConjectured]
...
For a modular Koszul algebra~$\cA$:
\begin{enumerate}[label=\textup{(\roman*)}]
...
Conjecture~\ref{conj:operadic-complexity} for the detailed
...
(\S\ref{subsec:hochschild-cochain-spaces}).  When the points are
...
\item $F_2$ is the brace operation
  (Definition~\ref{def:brace-ops}$\,$), encoding the
