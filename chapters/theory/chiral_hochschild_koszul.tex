...
The higher differentials vanish for support reasons: by
\eqref{eq:hoch-fm-e2} the $E_2$-page at filtration~$p$ is
concentrated in the single row $s = p$, and
$d_m \colon E_m^{r,s} \to E_m^{r+m,\,s-m+1}$ changes the row by
$m - 1 \geq 1$ for every $m \geq 2$, so its source or target
vanishes. Thus $E_2(p)=E_\infty(p)$, with no appeal to
formality or weight purity.
...
\item \emph{PBW-Koszulness and the twisted $E_1$-collapse.}\;
 The $d_1$-differential on the collision-depth spectral
 sequence annihilates every Arnold class of positive fibre
 degree. Two inputs enter: the PBW-Koszulness criterion
 \textup{(}Theorem~\textup{\ref{thm:pbw-koszulness-criterion}}\textup{)}
 for the chiral algebra, acting through the bar-concentration
 theorem \textup{(}Theorem~\textup{\ref{thm:bar-concentration}}\textup{)}
 localized at each stratum, and the quadratic Koszulness of
 the braid-arrangement Orlik--Solomon algebra
 \cite{SheltonYuzvinsky1997}, which controls the fibre
 cohomology
 \textup{(}Lemma~\textup{\ref{lem:chiral-homotopy-transport}(i)}\textup{)}.

 At bar degree~$p$, the $E_1$-page carries contributions
 from every collision pattern on the $p+2$ colliding points.
 Each pattern contributes through the Arnold-algebra
 cohomology of its local fibre, coupled to the algebra slots
 by the OPE-residue twist~$d_1$. Positive-fibre-degree
 acyclicity of this residue-twisted complex is the
 stratum-local form of bar concentration
 \textup{(}Conjecture~\textup{\ref{conj:ordered-twisted-tensor-acyclicity}}
 on the ordered side; Theorem~\textup{\ref{thm:bar-concentration}}
 on the Koszul locus after averaging\textup{)}; Orlik--Solomon
 Koszulness of the fibre alone does not produce it
 \textup{(}Lemma~\textup{\ref{lem:chiral-homotopy-transport}(ii)}\textup{)}.
...
\emph{Step~3: twisted collapse of the $E_1$-page through bar
concentration.}\;
We show that the $d_1$-differential kills all $E_1$-classes of
positive fibre degree. The differential is not the internal
differential of the Arnold algebra --- that differential is
zero, and $\cAr_m^{\geq 1}$ admits no contracting homotopy
\textup{(}Lemma~\ref{lem:chiral-homotopy-transport}(ii)\textup{)}
--- but the OPE-residue twist coupling the Arnold classes to
the algebra slots. The object to contract is the
residue-twisted tensor complex of
Conjecture~\ref{conj:ordered-twisted-tensor-acyclicity}, and
the contraction is supplied by bar concentration, localized at
the stratum, as follows.

\emph{\textup{(3a)} The PBW filtration.}\;
The PBW hypothesis
\textup{(}Theorem~\ref{thm:pbw-koszulness-criterion}\textup{)}
filters $\cA$ so that $\operatorname{gr}_F\cA$ is the free
commutative chiral envelope of the generating
$\cD_X$-module; on the associated graded, the OPE-residue
twist degenerates to the classical Koszul differential of the
quadratic datum.

\emph{\textup{(3b)} Associated-graded contraction.}\;
On $\operatorname{gr}_F$, the twisted complex at a cluster of
size~$m$ is the classical Koszul complex of the quadratic
pair --- the Orlik--Solomon algebra against its quadratic
dual, coupled to the free algebra slots --- and is acyclic in
positive fibre degree by
Theorem~\ref{thm:bar-concentration} localized at the stratum.
Quadratic Koszulness of $\cAr_m$ enters here exactly once,
through Priddy acyclicity of the \emph{Koszul complex}
\textup{(}Lemma~\ref{lem:chiral-homotopy-transport}(i)\textup{)},
not through any contraction of $\cAr_m$ itself.

\emph{\textup{(3c)} Convergence of the filtration
comparison.}\;
Acyclicity of $\operatorname{gr}_F$ of the twisted complex
implies acyclicity of the twisted complex itself provided the
PBW filtration spectral sequence converges; in each finite
conformal weight the filtration is finite, so convergence
holds weight by weight. On the ordered side, before
averaging, this chain of reductions is precisely
Conjecture~\ref{conj:ordered-twisted-tensor-acyclicity}; the
symmetric route through Theorem~\ref{thm:bar-concentration}
on the Koszul locus is the one this proposition consumes.
...
survives. The tower collapse is powered by bar concentration
localized at the strata; it is not independent of
...
$\FM_{p+2}(\C)$. The twisted collapse of Step~3 forces
...
Higher differentials vanish for support reasons: by
\eqref{eq:fm-tower-surviving} the $E_2$-page at filtration~$p$
is concentrated in the single row $s = p$, and
$d_m \colon E_m^{r,s} \to E_m^{r+m,\,s-m+1}$ changes the row by
$m-1 \geq 1$ for every $m \geq 2$, so its source or target
vanishes. Therefore $E_2 = E_\infty$, with no appeal to
formality or weight purity.
...
\begin{remark}[PBW, bar concentration, and the FM-tower collapse
are serial, not parallel]
\label{rem:pbw-parallel-consequences}
\index{PBW!serial consequences}
The dependence runs in one direction:
\[
 \text{PBW}
 \;\Longrightarrow\;
 \text{bar concentration
 \textup{(Theorem~\ref{thm:bar-concentration})}}
 \;\Longrightarrow\;
 \text{FM-tower collapse
 \textup{(Proposition~\ref{prop:fm-tower-collapse})}}.
\]
Step~3 of the FM-tower collapse contracts the residue-twisted
tensor complex at each stratum, and that contraction is bar
concentration localized at the stratum; Orlik--Solomon
Koszulness of the fibre controls the fibre cohomology but
cannot supply the contraction
\textup{(}Lemma~\ref{lem:chiral-homotopy-transport}(ii)\textup{)}.
On the ordered side, the stratum-local statement is
Conjecture~\ref{conj:ordered-twisted-tensor-acyclicity} and is
open.
\end{remark}
...
\begin{lemma}[Shelton--Yuzvinsky Koszulness of the pure-braid
Orlik--Solomon algebra: what it furnishes and what it cannot;
...
field.
\item No operator
$h_m \colon \operatorname{OS}(A_{m-1})^{\geq 1} \to
\operatorname{OS}(A_{m-1})^{\geq 1}$
satisfies $d h_m + h_m d = \id - \pi_{0}$. The algebra carries the
zero differential, so the left side vanishes identically, while
$\id - \pi_{0}$ restricts to the identity on the positive-degree
part, which is nonzero for every $m \geq 2$. Koszulness of an
algebra contracts its Koszul complex, never the algebra itself.
\end{enumerate}
\end{lemma}
...
(ii) With $d = 0$ on $\operatorname{OS}(A_{m-1})$, any candidate
homotopy gives $d h_m + h_m d = 0$, while
$(\id - \pi_{0})\big|_{\operatorname{OS}^{\geq 1}} = \id \neq 0$
because $\operatorname{OS}(A_{m-1})^{\geq 1} \neq 0$
\textup{(}Poincar\'e polynomial $\prod_{j=1}^{m-1}(1+jt)$, of
degree $m-1 \geq 1$\textup{)}.
\end{proof}

\begin{conjecture}[Ordered twisted-tensor acyclicity at a
collision stratum]
\label{conj:ordered-twisted-tensor-acyclicity}
\index{twisted tensor complex!ordered chiral}
Let $\cA$ be a chiral Koszul algebra on~$X$ satisfying the PBW
criterion of Theorem~\textup{\ref{thm:pbw-koszulness-criterion}},
and fix a collision stratum of depth profile $(m_1,\dots,m_k)$ in
$\overline{C}^{\mathrm{ord}}_{p+2}(X)$. The $E_{1}$-term of the
collision-depth spectral sequence at the stratum is the
residue-twisted tensor complex
\[
 \Bigl(\textstyle\bigotimes_{i}
 \operatorname{OS}(A_{m_i-1})\Bigr)
 \otimes \bigl(\textup{coefficient system in } \cA\bigr),
 \qquad
 d_{1} = \textup{OPE-residue twist},
\]
and this complex is acyclic in positive fibre degree. The required
input couples PBW-Koszulness of $\operatorname{gr}_F\cA$ to
Orlik--Solomon Koszulness through the twisting cochain: it is the
stratum-local form of
Theorem~\textup{\ref{thm:bar-concentration}}, and it is \emph{not}
implied by Koszulness of the fibre algebras
$\operatorname{OS}(A_{m_i-1})$ alone
\textup{(}Lemma~\textup{\ref{lem:chiral-homotopy-transport}(ii)}\textup{)}.
\end{conjecture}

\begin{remark}[Chain-level versus cohomology-level scope]
\label{rem:sigma-inverse-chain-level}
What Theorem~H consumes at each stratum is the statement that the
$d_{1}$-differential of the collision-depth spectral sequence
annihilates the positive-degree Arnold classes on the
$E_{1}$-page. That statement is \emph{equivalent} to
positive-fibre-degree acyclicity of the residue-twisted tensor
complex of
Conjecture~\ref{conj:ordered-twisted-tensor-acyclicity}; it is not
an output of Shelton--Yuzvinsky Koszulness of the fibre
Orlik--Solomon algebras alone, which contracts the Koszul complex
$\operatorname{OS}\otimes(\operatorname{OS}^{!})^{\vee}$ and is
silent about any complex whose twist couples the Arnold classes to
the algebra slots
\textup{(}Lemma~\ref{lem:chiral-homotopy-transport}(ii)\textup{)}.
\end{remark}

\begin{theorem}[Ordered-bar chiral Hochschild concentration;
...
Theorem~\textup{\ref{thm:pbw-koszulness-criterion}}, and assume
the ordered twisted-tensor acyclicity of
Conjecture~\textup{\ref{conj:ordered-twisted-tensor-acyclicity}}
at every collision stratum. Write
$\ChirHoch^{\mathrm{ord},n}(\cA)$ for the ordered chiral
Hochschild groups computed on
...
\[
 ...
 \textup{for every } n \notin \{0,1,2\}.
\]
The open obligation is exactly
Conjecture~\textup{\ref{conj:ordered-twisted-tensor-acyclicity}}:
collapse of the $E_{1}$-page in positive fibre degree is a
twisted-complex acyclicity statement, the stratum-local form of
Theorem~\textup{\ref{thm:bar-concentration}}, and is not a
corollary of fibrewise Orlik--Solomon Koszulness
\textup{(}Lemma~\textup{\ref{lem:chiral-homotopy-transport}(ii)}\textup{)}.
The symmetric statement $\ChirHoch^{n}(\cA)=0$ for
$n\notin\{0,1,2\}$ follows, given the conjecture, by the
averaging step of
Corollary~\textup{\ref{cor:hochschild-averaging-symmetric}}.
\end{theorem}

\begin{proof}[Proof, conditional on
Conjecture~\textup{\ref{conj:ordered-twisted-tensor-acyclicity}}]
The collision-depth spectral sequence on the ordered FM
compactification has, at each stratum of depth profile
$(m_1,\dots,m_k)$, the $E_{1}$-term displayed in
Conjecture~\ref{conj:ordered-twisted-tensor-acyclicity}: the
fibre is a product of ordered local FM spaces
$\FM^{\mathrm{ord}}_{m_i}(\C)$ with cohomology the pure-braid
Orlik--Solomon algebras $\operatorname{OS}(A_{m_i-1})$ (no
$\Sigma_{m_i}$-coinvariants at any stage), and $d_{1}$ is the
OPE-residue twist coupling the Arnold classes to the algebra
slots. Granting the conjectured positive-fibre-degree acyclicity
stratum by stratum, only the full-collision stratum survives,
contributing the $\cD_X$-Ext amplitude $[0,2]$ on the base curve
as in Steps~4--6 of the proof of
Proposition~\ref{prop:fm-tower-collapse}. Hence
...
controlled by a uniform $[0,2]$ amplitude bound. PBW Koszulness
provides the input that kills these contributions: bar
concentration \textup{(}Theorem~\ref{thm:bar-concentration}\textup{)},
localized at each collision stratum, contracts the residue-twisted
tensor complex that couples the positive Arnold fibre classes to
the algebra slots, and leaves only the curve-level Ext with
amplitude~$[0,2]$ from the single complex dimension of~$X$. The
Arnold algebra itself, having zero differential, admits no
contracting homotopy
\textup{(}Lemma~\ref{lem:chiral-homotopy-transport}(ii)\textup{)};
what its Koszulness supplies is Priddy acyclicity of its Koszul
complex, one input among the two that the twisted contraction
consumes.
