...
\item \emph{Bar-concentration kills the tower.}\;
 On the Koszul locus, the local bar cohomology is
 concentrated in bar-differential degree~$0$
 \textup{(}Theorem~\textup{\ref{thm:bar-concentration}}\textup{)}.
 The iterated OPE residues at each collision stratum are
 computed by the local bar complex on the fiber
 $\FM_m(\C)$, and bar-concentration forces the fiber
 contribution to land entirely in the degree-$0$ piece
 of the fiber cohomology.

 The mechanism is as follows.  At bar degree~$p$, the
 $E_1$-page of the collision-depth spectral sequence has
 contributions from all collision patterns: partial
 collisions where subsets of the $p+2$ points collide on
 intermediate configuration spaces, and the full collision
 to a single point on~$X$.  Each partial collision stratum
 contributes through the Arnold-algebra cohomology of the
 corresponding fiber, coupled by the Arnold differential
 $d_1$ induced by the bar differential.

 Bar-concentration on the Koszul locus forces these
 Arnold-algebra contributions to be exact under~$d_1$:
 the bar complex computes the iterated collision residues,
 and Koszulness guarantees that the bar cohomology is
 concentrated in the lowest filtration degree.  All
 higher-filtration Arnold classes, which record the topology
 of intermediate collision patterns in $\FM_m(\C)$ for
 $m \geq 3$, are killed by the bar differential on the
 $E_1$-page.
...
\emph{Step~3: bar-concentration kills higher fibers.}\;
The bar differential computes iterated OPE residues.  On the
Koszul locus, bar-concentration
(Theorem~\ref{thm:bar-concentration}) forces the
bar cohomology of each fiber to be concentrated in the
lowest degree.  In terms of the collision-depth spectral
sequence, this means the $d_1$-differential
kills all $E_1$-classes coming from the Arnold-algebra
cohomology in degrees $k \geq 1$.
