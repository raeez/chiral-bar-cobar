...
programme; only one is used below. The
...
amplitude~$[0,2]$, so the MC moduli has cohomological
...
amplitude~$[0,2]$ is therefore $E_3$-rigidity at a point
...
Lemma~\textup{\ref{lem:totalization-amplitude}} verifies
that, on the Koszul locus, no such cross-bar-degree pairing
spreads cohomology outside the $\{0, 1, 2\}$ window. The
bookkeeping is supplied by the bar-degree filtration spectral
...
only after one verifies that the bar-degree differential
does not create new cohomology classes in degrees outside
$\{0, 1, 2\}$.
\end{remark}
...
% lem:chiral-quadratic-koszul (line 657) with the
...
quotient points recorded in
Conjecture~\textup{\ref{conj:hochschild-concentration-class-M-non-generic}},
the bigraded chiral
...
is an exclusion from the theorem's domain, not a proof input.
On cohomology:
\[
...
\section{Beilinson--Drinfeld factorization cohomology and
chiral Hochschild: a comparison theorem}
...
The programme carries two a priori distinct Ext-theoretic
cohomologies of a chiral algebra~$\cA$ on a smooth projective
curve~$X$:
\begin{enumerate}[label=\textup{(\roman*)}]
...
factorization homology/cohomology of the unit bimodule, whereas
$\ChirHoch$ and $\mathcal{C}^\bullet_{\mathrm{fact}}(\cA,\cA)$
are the cohomological Ext of the diagonal bimodule.

\begin{theorem}[BD factorization cohomology versus chiral
Hochschild; \ClaimStatusProvedHere]
...
passing through the bar spectral sequence; chain-level
...
\textup{(}Lemma~\textup{\ref{lem:hochschild-shift-computation}},
Proposition~\textup{\ref{prop:fm-tower-collapse}}\textup{)}.
\item \emph{Cohomological isomorphism on the Koszul locus.}\;
Let $\cA$ be a chiral Koszul algebra
...
\qquad (n \in \mathbb{Z})
\]
is an isomorphism for every~$n$. In particular, BD factorization
cohomology of the diagonal bimodule also has universal
cohomological amplitude~$[0,2]$:
$H^n(\mathcal{C}^\bullet_{\mathrm{fact}}(\cA,\cA))=0$
for $n\notin\{0,1,2\}$.
\item \emph{$(\infty,1)$-categorical enhancement.}\;
After passage to the Francis--Gaitsgory
$(\infty,1)$-category $\mathrm{Fact}(X)^{\otimes}$, the comparison
$c_{\cA}$ lifts to a chain-level equivalence
\[
\mathcal{C}^\bullet_{\mathrm{fact}}(\cA,\cA)
\;\simeq\;
RHH_{\mathrm{ch}}(\cA)
...
$\mathrm{D}(\mathrm{Fact}(X))$. The obstruction to
chain-level equivalence in the $1$-categorical model is
exactly the bar-degree filtration spectral sequence; the
$(\infty,1)$-enhancement absorbs this into the totalization.
\item \emph{Class dependence.}\;
The cohomological isomorphism of~\textup{(b)} holds on the generic PBW
lanes of the four archetypes \textup{G/L/C/M}: Heisenberg, affine
Kac--Moody at non-critical level, $\beta\gamma$, and the generic
universal Virasoro/principal $\mathcal W$ lanes. Critical affine level
and minimal/admissible simple quotients are excluded from this proved
clause and remain in the coderived or conjectural scope of
Conjecture~\textup{\ref{conj:hochschild-concentration-class-M-non-generic}}.
\item \emph{Verdier self-duality.}\;
The comparison intertwines Verdier duality on both sides:
\[
\mathbb{D}_{\operatorname{Ran}(X)}\bigl(
\mathcal{C}^\bullet_{\mathrm{fact}}(\cA,\cA)\bigr)
\;\simeq\;
\mathcal{C}^\bullet_{\mathrm{fact}}(\cA^!,\cA^!),
\qquad
H^n(c_{\cA^!}) \circ \mathbb{D}
\;=\;
\mathbb{D}_X \circ H^{2-n}(c_{\cA}),
\]
which, combined with Theorem~\textup{\ref{thm:main-koszul-hoch}},
yields the BD-intrinsic Koszul duality
\[
H^n\bigl(\mathcal{C}^\bullet_{\mathrm{fact}}(\cA,\cA)\bigr)
\;\cong\;
...
an honest compactification, so the two differentials do not agree
...
\emph{Part~(b): cohomological isomorphism on the Koszul locus.}\;
Apply the bar-cobar identification of the diagonal bimodule:
by Theorem~\textup{\ref{thm:hochschild-bar-cobar}},
\[
\ChirHoch^\bullet(\cA)
\simeq
R\mathrm{Hom}_{\mathrm{ChirAlg}}
\bigl(\Omega^{\mathrm{ch}}(\bar{B}^{\mathrm{ch}}(\cA)),\cA\bigr).
\]
By Proposition~\textup{\ref{prop:cobar-fh}} in
Chapter~\textup{\ref{chap:bar-cobar}},
$\Omega^{\mathrm{ch}}(\bar{B}^{\mathrm{ch}}(\cA))
\simeq \int^X\bar{B}^{\mathrm{ch}}(\cA)
\simeq R\mathrm{Hom}_{\operatorname{Fact}(X)}(k,
\bar{B}^{\mathrm{ch}}(\cA))$,
and since $\Omega^{\mathrm{ch}}(\bar{B}^{\mathrm{ch}}(\cA))
\xrightarrow{\sim}\cA$ by the bar-cobar inversion
\textup{(}Theorem~\textup{\ref{thm:bar-cobar-inversion-qi}}\textup{)},
this gives
\[
\ChirHoch^\bullet(\cA)
\;\simeq\;
R\mathrm{Hom}_{\operatorname{Fact}(X)}(\cA,\cA^{\mathrm{diag}})
\;=\;
\mathcal{C}^\bullet_{\mathrm{fact}}(\cA,\cA)
\]
on the Koszul locus, where the chain-level quasi-isomorphism is
mediated by the bar-cobar counit. The cohomology amplitude bound
$\{0,1,2\}$ transfers from
Theorem~\textup{\ref{thm:hochschild-polynomial-growth}} to the
BD side via this isomorphism.

\emph{Part~(c): $(\infty,1)$-enhancement.}\;
In the $(\infty,1)$-category $\mathrm{D}(\mathrm{Fact}(X))$,
the totalization
$RHH_{\mathrm{ch}}(\cA) =
\operatorname{Tot}(\bigoplus_p CH^{p,\bullet}_{\mathrm{ch}}(\cA)[-p])$
is the homotopy colimit over the bar-degree simplicial object, which
is exactly what Francis--Gaitsgory's
$\mathrm{Fact}(X)^{\otimes}$-internal hom computes:
\[
R\mathrm{Hom}_{\mathrm{Fact}(X)^{\otimes}}
(\cA,\cA^{\mathrm{diag}})
\;\simeq\;
\operatorname{Tot}(\mathrm{cosimp}(\cA,\cA^{\mathrm{diag}})).
\]
The cosimplicial object $\mathrm{cosimp}(\cA,\cA^{\mathrm{diag}})$ in
bar degree~$p$ is $\mathrm{Hom}(\cA^{\otimes p},\cA)$ (functions on
the $p$-fold marked diagonal), which is exactly
$CH^{p,\bullet}_{\mathrm{ch}}(\cA)$. The Ran-simplicial boundary
$\partial_{\mathrm{Ran}}$ is absorbed into the totalization:
Lemma~\textup{\ref{lem:totalization-amplitude}} says the
totalization amplitude equals the per-bar-degree amplitude on
the Koszul locus, so the totalization of the Ran-simplicial object
equals the FM totalization up to $(\infty,1)$-equivalence. Hence
$c_{\cA}$ lifts to the $(\infty,1)$-equivalence claimed.
...
and Virasoro $\mathrm{Vir}_c$ at any~$c$
\textup{(}including $c=0$\textup{)}
all satisfy the chiral Koszul hypothesis
\textup{(}Definition~\textup{\ref{def:chiral-koszul-pair}}\textup{)}
on their respective lanes. Part~(b) therefore applies uniformly.
...
$\mathbb{D}_{\operatorname{Ran}(X)}$ acts on
$\operatorname{Fact}(X)$ by $\mathcal{F}\mapsto
R\mathcal{H}om(\mathcal{F},\omega_{\operatorname{Ran}(X)})$,
and Theorem~\textup{\ref{thm:verdier-bar-cobar}} gives
$\mathbb{D}_{\operatorname{Ran}}(\bar{B}^{\mathrm{ch}}(\cA))
\simeq\bar{B}^{\mathrm{ch}}(\cA^!)$
on the bar coalgebras. Apply $R\mathrm{Hom}_{\operatorname{Fact}(X)}$
against $\cA^{\mathrm{diag}}$ on both sides to obtain
\[
...
\mathcal{C}^\bullet_{\mathrm{fact}}(\cA^!,\cA^!).
\]
Combining this with the cohomological isomorphism of~(b) and
Theorem~\textup{\ref{thm:main-koszul-hoch}} yields the stated
BD-intrinsic Koszul reflection.
\end{proof}
...
Per the parity declaration in
CLAUDE.md (\S``How to work''; two lanes: chain-level
chiral, $(\infty,1)$-categorical BD/Francis--Gaitsgory),
Theorem~\textup{\ref{thm:bd-fact-vs-chirhoch}} is deliberately
stated on two levels. Part~(b) is a $1$-categorical cohomological
...
$\mathcal{D}_X$-Ext model is invoked. Part~(c) is the stronger
$(\infty,1)$-categorical chain-level statement, used by the
thick-BD / Ran-derived enhancements in
...
\begin{corollary}[Theorem~H extends to BD factorization cohomology;
\ClaimStatusProvedHere]
\label{cor:thm-H-bd-extension}
\index{Theorem H!BD extension}
On the Koszul locus, the Theorem~H concentration
...
discipline recorded there). Under
Theorem~\textup{\ref{thm:bd-fact-vs-chirhoch}}(c), the $(\infty,1)$-
enhancement $\mathcal{C}^\bullet_{\mathrm{fact}}(\cA,\cA)\simeq
...
This is the structural origin of the Vol~I $\leftrightarrow$ Vol~II
compatibility: the same $(\infty,1)$-equivalence underlies both the
holomorphic factorization bar structure and the topological
coproduct.
\end{remark}
...
\section{Chiral Hochschild and K3 Koszul duality}
\label{sec:chk-supplement}

\begin{remark}[K3 Hochschild bi-grading and CY-$2$ $[2]$-shift]
\label{rem:chk-K3-cy2-shift}
At $\cC = D^b\mathrm{Coh}(K3)$ the chiral Hochschild cohomology is HKR bi-graded
$\mathrm{HH}^\bullet(D^b\mathrm{Coh}(K3)) = \bigoplus_{p+q=\bullet} H^p(K3, \Lambda^q T_{K3})$,
and the bar-cobar Koszul shift equals the CY dimension of the input: $[2]$, not $[3]$.
...
(Bridgeland--Maciocia $2001$ on K3 autoequivalences). The Koszul dual coalgebra
...
\begin{remark}[Hochschild concentration in degrees $\{0,1,2\}$ for $\mathbf H_{\Delta_5}$]
\label{rem:chk-deep-1}
Theorem~H applied to the K3 chiral bialgebra gives
\[
\mathrm{ChirHoch}^k(\mathbf H_{\Delta_5}) \;=\; 0 \quad \text{for } k\notin\{0,1,2\},
\]
with the three surviving degrees:
\begin{itemize}
\item $\mathrm{ChirHoch}^0(\mathbf H_{\Delta_5}) = Z(\mathbf H_{\Delta_5})$ is the chiral centre, generated by the Casimir element plus the Igusa form insertion (two-dimensional on the Borel cusp, one-dimensional in the generic stratum);
\item $\mathrm{ChirHoch}^1(\mathbf H_{\Delta_5}) = \mathrm{Der}^{\mathrm{out}}(\mathbf H_{\Delta_5})$ is the outer-derivation space, spanned by Mukai-vector translations and Siegel-modular transformations;
\item $\mathrm{ChirHoch}^2(\mathbf H_{\Delta_5}) \cong H^2(\mathfrak g_{\Delta_5})^{\mathbb Z/2,K(1)} \cong \mathbb C\cdot \Delta_5$, the one-dimensional Etingof-Kazhdan Part~V super-category class.
\end{itemize}
...
The Hochschild differential decomposes as
\[
...
\S\ref{sec:bar-cobar-review-supplement}). The four pieces live in
...
(the anti-holomorphic Dedekind contribution). Cross-ref Vol.~II.
\end{remark}
...
its universal enveloping chiral algebra. Applied here, it produces
the isomorphism $\mathrm{ChirHoch}^2(\mathbf H_{\Delta_5})^{\mathbb Z/2,K(1)}\cong\mathbb C\cdot\Delta_5$,
showing that $\mathbf H_{\Delta_5}$ is the \emph{unique} chiral
bialgebra on the K3 base (up to gauge equivalence) with this
Hochschild shape. The non-projective
global lift to $\bar{\mathcal A}_2$ requires pinning $\hbar$ to the
