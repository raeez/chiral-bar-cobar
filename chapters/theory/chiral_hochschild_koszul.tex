...
Hochschild cohomology is not merely an auxiliary invariant of a chiral
algebra: it is the receptacle for deformations, centers, and eventually
bulk fields in the physical incarnations of the story.  When the volume
says ``Hochschild,'' it means ``the internal memory of all possible
deformations and couplings.''  This chapter constructs the chiral
Hochschild complex, proves its polynomial growth on the Koszul locus
(Theorem~H), establishes the Koszul duality
$\ChirHoch^n(\cA) \cong \ChirHoch^{2-n}(\cA^!)^\vee \otimes \omega_X$,
and shows how the complementarity theorem
(Theorem~\ref{thm:quantum-complementarity-main}) identifies the
deformation and obstruction packages as complementary Lagrangians.
...
Maurer--Cartan structure; the conjectural all-arity limit
$D\Theta + \tfrac{1}{2}[\Theta,\Theta] = 0$
on $\mathrm{Def}_{\mathrm{cyc}}^{\mathrm{mod}}(\cA)$ would
give the full master equation.
\end{remark}
