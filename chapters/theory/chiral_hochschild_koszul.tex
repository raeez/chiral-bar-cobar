...
\emph{Theorem~H} has two clauses:
\begin{enumerate}[label=\textup{(\alph*)}]
\item \emph{Koszul duality for Hochschild cohomology}
  \textup{(}Theorem~\textup{\ref{thm:main-koszul-hoch})}\textup{:}
  $\ChirHoch^n(\cA) \cong \ChirHoch^{2-n}(\cA^!)^\vee
  \otimes \omega_X$ on the Koszul locus.
\item \emph{Polynomial growth}
  \textup{(}Theorem~\textup{\ref{thm:hochschild-polynomial-growth})}\textup{:}
  $\ChirHoch^n(\cA) = 0$ for $n \notin \{0,1,2\}$, so the
  Hochschild--Hilbert series $P_\cA(t)$ is a polynomial of degree
  at most~$2$, with Koszul-functorial coefficients.
\end{enumerate}
Further constructions: the cyclic deformation complex
$\Defcyc(\cA)$ (Definition~\ref{def:cyclic-deformation-elementary}),
the ambient complementarity potential $S_\cA$
(Definition~\ref{def:complementarity-potential-cotangent}), and the
derived critical locus theorem
(Theorem~\ref{thm:derived-critical-locus-chk}).
\end{remark}

The chiral Hochschild complex is the tangent complex to the
MC moduli $\MC(\gAmod)/\mathrm{gauge}$ at the universal class
$\Theta_\cA$.  Every deformation, every obstruction, every bulk-field
coupling passes through this tangent space; the cyclic deformation
complex $\Defcyc(\cA)$
(Definition~\ref{def:cyclic-deformation-elementary}) is its cyclic
refinement.  Theorem~H says the tangent complex is concentrated
in degrees $\{0, 1, 2\}$ on the Koszul locus (the MC moduli is
smooth of virtual dimension~$2$), and the Koszul duality
$\ChirHoch^n(\cA) \cong \ChirHoch^{2-n}(\cA^!)^\vee \otimes \omega_X$
is the Verdier duality on this tangent space.  The complementarity
...
naturally carry brace and $E_2$ structures; the convolution dg~Lie
...
the strict model of $\Definfmod(\cA)$
(Remark~\ref{rem:modular-cyc-strictification}),
extends the structures built here into the full modular regime.

Two complementary algebraic objects govern the open/closed dictionary:
\begin{enumerate}[label=\textup{(\alph*)}]
...
$C^\bullet_{\mathrm{ch}}(\cA, \cA)$.  The shadow tower
...
\begin{lemma}[Hochschild duality shift; \ClaimStatusProvedHere]
...
curve~$X$ of genus~$g$, the Verdier duality shift in the bigraded
chiral Hochschild complex
\textup{(}Definition~\textup{\ref{def:bigraded-hochschild})}
collapses to a uniform constant.  More precisely, at bar degree~$p$
the three grading contributions are:
\begin{enumerate}[label=\textup{(\roman*)}]
\item \emph{Verdier shift:} $[p+2]$, from
  $\dim_{\mathbb{C}} \overline{C}_{p+2}(X) = p + 2$
  \textup{(}the FM compactification of $p+2$ points on a curve
  has complex dimension $p+2$\textup{)};
\item \emph{Totalization shift:} $[-p]$, from the convention
  $RHH_{\mathrm{ch}}(\cA) := \mathrm{Tot}\bigl(
  \bigoplus_{p \geq 0} CH^{p,\bullet}_{\mathrm{ch}}(\cA)[-p]\bigr)$;
\item \emph{Net shift:} $(p+2) + (-p) = 2$, independent of~$p$.
\end{enumerate}
Koszul concentration
\textup{(}Theorem~\textup{\ref{thm:bar-concentration})}
forces $H^{p,q} = 0$ for $q \neq 0$, so only the diagonal $p = q$
...
uniform~$[2]$.  The $\omega_X$ twist comes from Serre duality on
the underlying curve, yielding the result.
\end{proof}
...
\item \emph{Koszul functoriality.}\;
  ...
  holds unconditionally (the Koszul duality
  $\ChirHoch^n(\cA) \cong \ChirHoch^{2-n}(\cA^!)^\vee \otimes \omega_X$
...
growth bound reflects the finite excess intersection dimension of
...
element $\Theta_{\cA}$
(Theorem~\ref{thm:mc2-bar-intrinsic}), and every structural property
of~$\cA$ is a property of~$\Theta_{\cA}$.  We investigate four
...
\item All higher $\Etwo$-operations \textup{(}braces of
  arity~$\geq 3$, Massey products, $A_\infty$ corrections\textup{)}
...
$\ChirHoch^n(\cA) = 0$ for $n \notin \{0,1,2\}$.
The $\Etwo$-operations of arity~$k$ on $H^*(\ChirHoch(\cA))$
are maps
...
exact: the bar spectral sequence that collapses at~$E_1$ forces the
...
these operations.  This is the standard ``concentration-implies-formality''
argument (cf.\ \cite{LV12}, \S10.3) applied to the three-term
complex $H^0 \oplus H^1 \oplus H^2$.
\end{proof}
...
Gerstenhaber bracket alone.  In particular, the secondary Borcherds
operations $F_n$ from the homotopy chiral algebra structure on
$\barB^{\mathrm{ch}}(\cA)$
(Theorem~\ref{thm:cech-hca}, Pillar~A) are exact for all
$n \geq 3$, and $\Etwo$-formality is an automatic consequence of
the PBW concentration, not an additional hypothesis.

The converse (that $\Etwo$-formality implies Koszulness) is false as
stated.  A non-Koszul algebra whose Hochschild cohomology happens
to be concentrated in three degrees would also be $\Etwo$-formal
by the same argument.  Any stronger converse would have to pass
through the much stronger scalar-orbit formality problem discussed
...
By Theorem~\ref{conj:operadic-complexity-detailed},
...
For the full scalar orbit, observe that the shadow
Postnikov tower terminates at arity~$2$:
...
This unpacks the content of the preceding theorem.

\emph{Forward direction.}\;
If $\Theta_{\cA} = \kappa \cdot \eta \otimes \Lambda$, then the
MC equation $D \Theta + \tfrac{1}{2}[\Theta, \Theta] = 0$
reduces to a \emph{scalar} identity at each $(g,n)$, because
$[\eta \otimes \Lambda, \eta \otimes \Lambda]$ factors through the
metric contraction.  The higher $L_\infty$-brackets $\ell_k$
($k \geq 3$), which are the non-formal corrections from the
modular operad, act trivially on the one-dimensional
subspace~$\bC \cdot \eta \otimes \Lambda$.  Hence
...
one-channelity alone implies formality.  The strongest general
proved input is only the one-channel minimal-model statement
$\Theta_{\cA}^{\min}=\eta\otimes\Gamma_{\cA}$
\textup{(}Corollary~\textup{\ref{cor:scalar-saturation}}\textup{)},
and identifying $\Gamma_{\cA}$ with
$\kappa(\cA)\Lambda$ is the open tautological-purity step outside
the proved scalar lane.

\emph{Relation to the shadow Postnikov tower.}\;
The non-scalar universal classes
of~\S\ref{subsec:non-scalar-theta} measure the deviation of
$\Theta_{\cA}$ from the scalar line.  In the language of the
shadow tower, the cubic shadow~$C$, quartic resonance class~$Q$,
and all higher shadows $\mathrm{Sh}_r$ are the
\emph{obstructions to formality}: they are the higher-order
corrections to $\Theta_{\cA}$ that prevent it from lying on the
scalar line.
...
is proved here.
See Remark~\ref{rem:full-homotopy-why} for the parallel statement
at the homotopy level.
\end{remark}

\begin{remark}[Hierarchy: formality, modular Koszulness, Koszulness]
...
exactly the non-scalar shadow tower
...
slots simultaneously (\cite{RNW19}, Theorem~6.1).  For chirally
...
An element $a \in \mathcal{B}$ lies in the center if and only if $a(z)\alpha(w) \sim 0$ (no singular terms).  Since $\alpha(z)\alpha(w) \sim (z-w)^{-2}$, the only such elements are scalars.  Thus $\ChirHoch^0 = \mathbb{C}$.
...
% D(alpha) = 1 is a non-inner chiral derivation). See AP9.
...
is one-dimensional, spanned by the level deformation $\alpha(z) \alpha(w) \sim \kappa/(z-w)^2 \mapsto (\kappa + \epsilon t)/(z-w)^2$.  Thus $\ChirHoch^2 = \mathbb{C}$.
\end{computation}
...
\begin{remark}[Boson-fermion Hochschild comparison: pole order controls innerness]
...
A chosen strict presentation is the
\emph{cyclic deformation complex} $\Defcyc(\cA)$
used for explicit calculations below.
...
The modular cyclic deformation complex is the ambient home for the
universal Maurer--Cartan element~$\Theta_\cA$.  At the dg~level,
the Lie bracket and differential give a cyclic dg~Lie algebra; by
Theorem~\ref{thm:cyclic-linf-graph}, this extends to a full
\emph{cyclic $L_\infty$-algebra}, and at the modular level
Theorem~\ref{thm:modular-quantum-linfty} further promotes it to a
...
The modular MC problem for~$\cA$ is to find
$\Theta_\cA \in \MC(\Defcyc^{\mathrm{mod}}(\cA))$ satisfying the
full quantum $L_\infty$ Maurer--Cartan equation
\[
...
This is the strict shadow of the full $L_\infty$-MC equation
(Theorem~\ref{thm:modular-homotopy-convolution}).
For the proved families (Kac--Moody at non-critical level,
Virasoro, principal~$\mathcal{W}_N$ at generic level),
...
components; the higher $L_\infty$ brackets $\ell_n^{(g)}$ encode
the full Feynman-amplitude content beyond the leading dg~terms.
\end{remark}

\begin{proposition}[Hochschild as arity-$2$ shadow projection;
\ClaimStatusProvedHere]
\label{prop:hochschild-shadow-projection}
\index{chiral Hochschild cohomology!shadow projection}
\index{shadow algebra!arity-2 projection}

The chiral Hochschild cohomology of a modular Koszul algebra $\cA$ is
the arity-$2$ projection of the shadow algebra:
\[
...
classes are all shadows of the shadow Postnikov tower
$\Theta_\cA^{\leq r}$
...
encode homotopy-coherent deformation data: the $\ell_3$ bracket governs
obstructions to extending first-order deformations, $\ell_4$ governs
secondary obstructions, and so on.  The coderivation model computes the
...
structure, a homotopy Gerstenhaber algebra.  This is the algebraic
source of the bar-cobar adjunction's naturality: the bar functor is an
$E_2$-coalgebra functor, not merely a dg~coalgebra functor.
...
\Definfmod(\cA).
\]
This is the invariant modular deformation object; the formula in
...
In 2D conformal field theory, $\ChirHoch^2(\mathcal{A})$ classifies marginal deformations of the action
$S \to S + \lambda \int_{\Sigma} \phi(z,\bar{z}) d^2z$.
The deformation preserves conformal invariance when $\phi$ has conformal weight $(1,1)$ (marginality), $[\phi] \in \ChirHoch^2$ is a cocycle (preserving the OPE algebra), and the obstruction in $\ChirHoch^3$ vanishes (extending to all orders).
...
\[\ChirHoch^2(\mathcal{B}) = \mathbb{C} \cdot [\kappa].\]
...
\section{Cosimplicial descent and the Hochschild--\v{C}ech comparison}
...
interplay between the Hochschild and \v{C}ech differentials is
...
$U_{i_0\cdots i_p}$, the local Hochschild complex computes
...
\begin{corollary}[HCA structure on local Hochschild cochains;
\ClaimStatusConjectured]
\label{cor:hca-hochschild}
...
\item the full MC element $\Phi_{\ChirHoch}$
  ...
  projections to each arity give the $L_\infty$ relations
...
MC element $\Phi_{\ChirHoch}$: the bracket is the arity-$2$
...
The HCA structure on the Hochschild--\v{C}ech bicomplex is the
...
\begin{theorem}[Derived critical locus; \ClaimStatusConditional]
\label{thm:derived-critical-locus-chk}
...
established tower
$\kappa(\cA) \cdot \lambda_g$.  The true invariant is not
...
$\widehat{\mathfrak{sl}}_2$/Virasoro/$\beta\gamma$ spectral sheet.
The canonical homotopy arity of the subregular hook family
is the canonical
...
(Nishinaka~\cite{Nishinaka25,Nishinaka26}), a vertex algebra
$V(R)$, the enveloping vertex algebra of~$R$.
The Maurer--Cartan shadow tower of~$V(R)$
...
\textup{(}Definition~\textup{\ref{def:cyclically-admissible})},
...
\item \textup{(Modular characteristic from $\lambda$-bracket.)}
  ...
  the arity-$2$ part of the bar complex of $V(R)$, which depends
  ...
  % RECTIFICATION-FLAG (AP1): The supertrace formula below
  % does NOT reproduce kappa(H_k) = k for Heisenberg at level k,
  % nor kappa(V_k(sl_2)) = 3(k+2)/4 for affine sl_2.  The formula
  % needs correction or the normalization conventions need to be
  % specified precisely.  The explicit values in the table below
  % ARE correct (verified by the compute layer); the abstract
  % formula is a placeholder awaiting rigorous derivation.
  Explicitly:
  \begin{center}
...
\item \textup{(Shadow depth from Lie conformal structure.)}
  ...
  \textup{(}Definition~\textup{\ref{def:shadow-depth-class})} of
  ...
  \item Class~M \textup{(}$r_{\max} = \infty$\textup{)}:
    ...
    shadow tower.  Virasoro and $\cW_N$ for $N \geq 2$.
  \end{itemize}
...
arity-$2$ scalar projection of the Maurer--Cartan element
$\Theta_{V(R)}$ in the modular convolution algebra
$\gAmod$
(Definition~\ref{def:modular-convolution-dg-lie}).
At arity~$2$, the bar complex $B(V(R))_2$ involves exactly the
...
$\lambda$-bracket.  No higher-arity vertex algebra data enters
the arity-$2$ bar component.
...
(Chapter~\ref{chap:heisenberg-eisenstein}).
...
(Definition~\ref{def:shadow-depth-class}).
...
$S_3$ via tree-level arity-$3$ Feynman graphs.  The Jacobi
...
complexes.  At arity~$2$, this is the map on self-pairings
...
\section{The graph complex and deformation theory of the shadow tower}
...
The shadow tower of a modular Koszul algebra encodes the finite-order
projections of the universal Maurer--Cartan element~$\Theta_\cA$.
The \emph{deformations} of the shadow tower---the infinitesimal
...
% RECTIFICATION-FLAG: Parts (i) and (iv) rely on the Kontsevich
% formality quasi-isomorphism applied to the E_2 structure on the
% bar complex; the Lie algebra map Phi is described but not
% rigorously constructed.  Part (ii) is Willwacher's theorem
% (ProvedElsewhere).  Part (iii) follows from (i)+(ii) conditionally.
% Downgraded from ProvedHere to Heuristic pending rigorous construction
% of Phi and proof that even-arity shadows are boundaries.
\begin{theorem}[Graph complex controls shadow tower deformations; \ClaimStatusHeuristic]
\label{thm:graph-complex-shadow}
...
\item \textup{(Shadow-to-graph-complex map.)}
  ...
  complex.  Under~$\Phi$, the odd-arity shadow
  ...
  The even-arity shadows $S_{2k}(\cA)$ map to exact forms
...
\item \textup{(Depth classification via truncation.)}
  ...
  \textup{(}Definition~\textup{\ref{def:shadow-depth-class})}
  ...
  arising from odd-arity shadow invariants $S_{2k+1}$ with
  $2k + 1 \leq d$.  For class~G ($d = 2$), no wheel cocycle
  component is activated.  For class~L ($d = 3$) and class~C
  ($d = 4$), only~$\sigma_3$ is activated (the even-arity
...
\item \textup{(Deformation control.)}
  The space of infinitesimal deformations of the shadow tower
  ...
  constraints on which shadow towers can arise from actual
...
computed as follows.  At odd arity $r = 2k+1$:
the shadow $S_{2k+1}$ is an arity-$(2k+1)$ scalar in
$\Defcyc^{\mathrm{mod}}(\cA)$
(Definition~\ref{def:modular-cyclic-deformation-complex}).
...
At even arity $r = 2k$: the wheel graph $W_{2k}$ is
...
$\sigma_3$ (weight~$3$).  Class~C activates $\sigma_3$ only: the shadow tower
terminates at arity~$4$, and the even-arity shadow $S_4$ maps
...
for all $r \geq 5$.  At odd arities, the nonvanishing of
