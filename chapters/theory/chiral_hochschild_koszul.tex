...
The cohomological isomorphism of~\textup{(b)} holds without
class-discrimination on the four archetypes
\textup{G/L/C/M}: all four standard archetypes (Heisenberg,
affine Kac--Moody at non-critical level, $\beta\gamma$,
Virasoro at arbitrary~$c$, including $c=0$) are chiral
Koszul on their respective lanes, so the isomorphism
applies uniformly.
...
\section{Wave 13 DNA perspective: Chiral Hochschild and K3 Koszul duality}
\label{sec:chk-wave13-DNA}

\begin{remark}[K3 Hochschild bi-grading and CY-$2$ $[2]$-shift]
\label{rem:chk-DNA-K3-cy2-shift}
...
\label{sec:chk-wave13-DNA-deep}

\begin{remark}[Hochschild concentration in degrees $\{0,1,2\}$ for $\mathbf H_{\Delta_5}$]
\label{rem:chk-DNA-deep-1}
Theorem~H applied to the Wave~13 K3 chiral bialgebra gives
\[
...
\label{rem:chk-DNA-deep-2}
The Wave~13 Hochschild differential decomposes as
\[
...
\S\ref{sec:bar-cobar-review-wave13-DNA}). The four pieces live in
...
\begin{remark}[Etingof-Kazhdan Part V and uniqueness of Wave 13]
\label{rem:chk-DNA-deep-3}
...
showing that $\mathbf H_{\Delta_5}$ is the \emph{unique} Wave~13 chiral
bialgebra on the K3 base (up to gauge equivalence). The non-projective
global lift to $\bar{\mathcal A}_2$ requires pinning $\hbar$ to the
Lusztig value, which is why Wave~13 converges at $\hbar^2 = -1/8$.
