...
coupling factors through this tangent space; the cyclic deformation
complex $\Defcyc(\cA)$
(Definition~\ref{def:cyclic-deformation-elementary}) is its cyclic
refinement.  Theorem~H concentrates the tangent complex
in degrees $\{0, 1, 2\}$ on the Koszul locus, where the MC moduli is
smooth of virtual dimension~$2$.  The Koszul duality
...
Hochschild level, Koszul duality exchanges deformations and
obstructions:
$\ChirHoch^n(\cA)\cong \ChirHoch^{2-n}(\cA^!)^\vee\otimes\omega_X$
...
Both proofs use the bar-cobar quasi-isomorphism at all genera
...
chiral algebras:
(a)~$\mathfrak{G}_{n,0}$ is finite at each arity~$n$ (a tree on $n + k$ vertices with $n$~external legs has
