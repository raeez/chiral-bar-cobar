...
algebra $\Hom(\barB(\cA), \cA)$ governing deformations is the
strict face of an $L_\infty$ deformation package.  Thus this chapter
...
constructed later in the nonlinear shadow chapters, extends the
structures built here into the full modular regime.
...
carries an HCA structure in which:
\begin{enumerate}[label=\textup{(\roman*)}]
\item the $F_2$ bracket is the brace operation
  (Definition~\ref{def:brace-ops}$\,$);
\item the $F_3$ homotopy controls the compatibility between
  braces and the \v{C}ech differential;
\item the full collection $\{F_n\}$ is an $L_\infty$ enhancement
  of the Gerstenhaber bracket on
  $\ChirHoch^\bullet(\cA)$.
\end{enumerate}
In particular, the Gerstenhaber algebra structure on
$\ChirHoch^\bullet(\cA)$ descends from an $L_\infty$ structure on
the total complex of the bicomplex, with the \v{C}ech direction
contributing only contractible higher operations.
\end{corollary}
