...
\item \emph{Koszul functoriality.}\;
  ...
  \]
  and the duality $P_{\cA}(t) = t^2\,P_{\cA^!}(t^{-1}) \cdot
  (\dim Z(\cA) / \dim Z(\cA^!))$ holds when both centers are
  nonzero.
\end{enumerate}
\end{theorem}

\begin{proof}
Part~(a): on the Koszul locus, the bar complex
$\barB^{\mathrm{ch}}(\cA)$ is concentrated on the diagonal in the
$(p, q)$-bigrading (Theorem~\ref{thm:bar-concentration}).  The
Hochschild spectral sequence
(Theorem~\ref{thm:hochschild-spectral-sequence}) therefore collapses
at~$E_1$, and the range $0 \leq n \leq 2$ is forced by the fact that
the configuration spaces $\overline{C}_{n+2}(X)$ carry the logarithmic
de~Rham complex of the curve~$X$ ($\dim_{\mathbb{C}} X = 1$), so
$H^q(\overline{C}_{n+2}(X), -)$ vanishes for $q > n + 2 - n = 2$.
(In bar degree~$n$, the Koszul concentration gives a single surviving
line, and the sheaf cohomology on~$X$ contributes at most degrees
$0, 1, 2$.)
...
The converse---$\Etwo$-formality implies Koszulness---is false as
...
The forward direction---Koszulness implies
vanishing~\eqref{eq:mc-smoothness}---therefore follows from the
PBW collapse at all genera.  The converse---smoothness implies
...
This is why the MC3 resolution in type~$A$
(Theorem~\ref{thm:mc3-type-a-resolution}) succeeds: each step
(chromatic filtration, prefundamental Clebsch--Gordan,
Efimov completion, Dwyer--Kan on compacts) is a one-slot operation.
The categorical lift to arbitrary type
(Conjecture~\ref{conj:mc3-arbitrary-type}) must also proceed
...
\]
Using the OPE and comparing singularities, we find $D = 0$. Thus $\ChirHoch^1 = 0$.
...
\[
\ChirHoch^n(\mathcal{B}) = (\mathbb{C}, 0, \mathbb{C}) = \ChirHoch^n(\mathcal{F}).
\]
In particular the numerical relation $\ChirHoch^n(\mathcal{B}) \cong \ChirHoch^{2-n}(\mathcal{F})^{\vee}$ holds.
...
\ChirHoch^2 &= \mathbb{C} & & \text{(one marginal deformation: the
  level)},
\end{align}
and these groups satisfy $\ChirHoch^{n+24} \cong \ChirHoch^n$ for all $n$.
...
\[
\ChirHoch^{n+8}(\mathcal{F}, E_{\tau}) \cong \ChirHoch^n(\mathcal{F}, E_{\tau})
\]
...
where $\text{Der}(\mathcal{B})$ is the space of derivations and $\text{Inn}(\mathcal{B})$ the inner derivations.  We show $\ChirHoch^1 = 0$ by proving every derivation is inner.
...
As a graded algebra:
\[\ChirHoch^*(\mathcal{B}) \cong \mathbb{C}[c]/(c^2) \cong \mathbb{C} \oplus \mathbb{C} \cdot c\]
where $c$ is a degree-2 class corresponding to the central charge (with $c^2 = 0$ since $\ChirHoch^n = 0$ for $n \geq 3$).
\end{example}
...
\begin{align*}
\ChirHoch^0(\mathcal{B}) = \mathbb{C} &\xleftrightarrow{\text{dual}} \ChirHoch^2(\mathcal{BG})^* = \mathbb{C}\\
\ChirHoch^1(\mathcal{B}) = 0 &\xleftrightarrow{\text{dual}} \ChirHoch^1(\mathcal{BG})^* = 0\\
\ChirHoch^2(\mathcal{B}) = \mathbb{C} &\xleftrightarrow{\text{dual}} \ChirHoch^0(\mathcal{BG})^* = \mathbb{C}
\end{align*}
...
\begin{theorem}[Derived critical locus; \ClaimStatusProvedHere]
