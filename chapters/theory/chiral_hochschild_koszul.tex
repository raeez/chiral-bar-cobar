...
manuscript; only one is used below. The
chiral Hochschild complex $\ChirHoch^*(\cA)$ defined here is the
tangent complex of the chiral MC moduli on a curve, an
$E_\infty$-chiral input producing an output of cohomological
amplitude~$[0,2]$ on the Koszul locus (Theorem~H below). It is distinct from
$\textup{(a)}$ the topological Hochschild homology $\mathrm{THH}(R)$
of an $E_1$-ring spectrum~$R$, whose output is an $S^1$-equivariant
spectrum; and from \textup{(b)} the categorical Hochschild
...
spectrified interval algebra over a point produces $\mathrm{THH}$,
a CY dg category produces HH$_{\mathrm{cat}}$. Throughout this chapter
...
Theorem~H proves that on the Koszul locus it has cohomological
amplitude~$[0,2]$, so the tangent complex to the MC moduli has
cohomological amplitude~$[0,2]$, and the Koszul duality
...
collapse. No $E_3$ upgrade, CY-$d$ product ambient, or categorical
Hochschild concentration is used in Theorem~H.
...
\item \emph{Chiral Hochschild cochains}
 ...
 acting on the boundary in the local chiral Swiss-cheese category
 (Theorem~\ref{thm:chiral-deligne-tamarkin}). The brace dg algebra
 structure makes $C^\bullet_{\mathrm{ch}}$ the local closed algebra
 over $\cA$ in the curve-level chiral Swiss-cheese sense. The complex
 itself is
...
Koszul locus their common cohomology has amplitude~$[0,2]$
...
resolution; it is not the Koszul dual algebra~$\mathcal{A}^{!}$.
\end{theorem}
...
independently of $p$. Then the totalization
$RHH_{\mathrm{ch}}(\cA)$ has cohomological amplitude~$[0, 2]$,
i.e., $H^n(RHH_{\mathrm{ch}}(\cA)) = 0$ for $n \notin \{0, 1, 2\}$.
\end{lemma}
...
\smallskip
\emph{Convergence.}\;
The filtration $F^p$ is exhaustive
$\bigl(\bigcup_p F^p = RHH_{\mathrm{ch}}(\cA)\bigr)$ and
Hausdorff $\bigl(\bigcap_p F^p = 0\bigr)$, since each total
degree~$n$ has only finitely many bar-degree contributions:
in total degree~$n$, only the bar degrees~$p$ with
$0 \leq p \leq n + ($\,fibre amplitude\,$) = n + 2$ can
contribute (because the per-summand amplitude is $[0, 2]$ and
the totalization shift is~$[-p]$, so contributions appear
only in total degrees $\geq -p + 0$ and $\leq -p + 2$,
i.e., $p \in [-n, 2-n]$ for the contribution to lie in
total degree~$n$; the constraint $p \geq 0$ then gives
$p \in [\max(0,-n), \max(0,2-n)]$, a finite set for each
fixed $n$). Therefore the spectral sequence converges
strongly to $H^*\!\bigl(RHH_{\mathrm{ch}}(\cA)\bigr)$.
...
but nonzero on the totalization, could in principle create
new cohomology classes outside $\{0, 1, 2\}$ from boundaries
that pair across bar degrees. However, the spectral sequence
...
$p + q = 3$ that are nontrivial would require $E_2$-classes
...
would be incomplete: the slogan ``every column has
amplitude $[0,2]$, hence the totalization does'' is true
...
% lem:chiral-homotopy-transport states the chain-level
% contracting homotopy h_m^{\mathrm{ord}} on the ordered chiral
% bar, combining the Fresse chain map \sigma of
% lem:chiral-quadratic-koszul with the
% Shelton--Yuzvinsky contracting homotopy h_m on
% \operatorname{OS}(A_{m-1})^{\geq 1}. This is the statement
% invoked by thm:hochschild-concentration-E1.
...
chiral convolution dGLA. Then $\mathrm{av}$ descends to a
quasi-isomorphism on Hochschild cochains in degrees $n\ge 3$
\textup{(}both sides vanish\textup{)} and is surjective on
cohomology in degrees $0,1,2$, so that
\[
 \ChirHoch^{n}(\cA) \;=\; 0
 \qquad
 \textup{for every } n \notin \{0,1,2\}.
...
Taking $\Sigma_{n}$-coinvariants commutes with the
null-homotopy in these degrees, hence
$\ChirHoch^{n}(\cA) = (\ChirHoch^{\mathrm{ord},n}(\cA))_{\Sigma_{n}}
= 0$ for $n \ge 3$. The degree-$\le 2$ groups are preserved
by averaging because the averaging map is already acyclic
in those degrees on both sides (the Koszul resolution
computes the same $\cD_{X}$-Ext on the curve). The bound
$n<0$ is trivial.
\end{proof}
...
essential way. Outside that hypothesis; for example,
on an $E_{1}$-chiral input without a flat PBW filtration,
such as the chiral Yangian
$Y_{\hbar}(\fg)^{\mathrm{ch}}$; only the ordered bound
...
$E_{\infty}$-completion. Witnessing candidate: the chiral
...
and critical levels for class M; \ClaimStatusConjectured]
...
Theorem~\textup{\ref{thm:hochschild-polynomial-growth}} is
expected to hold only in the pro-object or coderived sense:
there exist discrete parameter points where the Shapovalov
form degenerates and Toda-bracket obstructions may produce
classes in $\ChirHoch^{3}$. A rigorous statement at these
points requires either a coderived reinterpretation
\textup{(}analogous to the coderived scope of
Theorem~\textup{\ref{thm:hochschild-concentration-E1}}
for $E_{1}$ inputs\textup{)} or a case-by-case vanishing
argument invoking the concrete Kac-determinant structure.
...
the critical level $k = -h^{\vee}$ is excluded
unconditionally
\textup{(}Remark~\textup{\ref{rem:critical-level-lie-vs-chirhoch}};
the BD/Feigin--Frenkel comparison yields an unbounded
$\ChirHoch^{*}$\textup{)}.
\end{conjecture}
...
sequence is the mechanism by which bar-concentration leaves only the
curve-level Ext amplitude~$[0,2]$.
\end{remark}
...
Hochschild cohomology by the two-sided diagonal bar resolution
\[
\ChirHoch^\bullet(\cA)
\simeq
...
Hochschild configuration. This gives the
$\Sigma$-coinvariant descent of $(\cA)^!_\infty$ and proves
\eqref{eq:chirhoch-descent}. The bar--cobar counit is used only to
prove that the two-sided bar object resolves the diagonal bimodule;
it does not identify $\Omega\barB(\cA)$ with the Koszul dual algebra.
\end{proof}
...
dual~$\cA^!$ on a smooth projective curve~$X$, satisfying the
...
Lemma~\ref{lem:totalization-amplitude} below, which transports the
...
only the expected indexing of the obstruction theory; it supplies no
...
Naturality in $\cA$ follows because both sides are covariant functors
of the chiral algebra datum and the construction uses only
$\operatorname{Fact}(X)$-functorial data.
...
the $1$-categorical ambient would be false
(Part~(a)).
\end{remark}
...
brace-compatible contraction. A stronger converse would require
...
is the target of obstructions; it may also parametrize unobstructed
families and need not vanish.
\end{proof}
...
recovering Koszulness from MC smoothness would have to prove that
...
See the editorial companion for the full research surface.
\end{remark}
...
first-order deformations of the OPE algebra. A CFT deformation
...
ambient curve-level cohomology above degree~$2$ on the PBW Koszul
locus; it does not make an arbitrary physical marginal operator
integrable to all orders.

\begin{example}[Exactly marginal deformations]
For the free boson, the degree-$2$ chiral Hochschild line is the
radius/level deformation. For Virasoro minimal models on the
Theorem~H lane, vanishing of the degree-$2$ chiral deformation group
expresses rigidity of the chiral OPE algebra. Four-dimensional
...
cofibrancy of this resolution on the Koszul locus. It is not the
Hochschild cochain complex itself and it is not the Koszul dual
algebra~$\mathcal A^!$.
...
The Koszul duality for Hochschild cohomology (Theorem~\ref{thm:main-koszul-hoch}) is realized on examples.
...
\ChirHoch^{2-n}(\cA^!)^\vee\otimes\omega_X$. On the Theorem~H lane,
...
with the weight-$11$ relation obtained by direct graph-complex
...
The numerical computation: for Virasoro at central charge~$c$,
...
$S_{2k+1}(c)$ for generic~$c$ is computed symbolically
...
Koszul hypotheses.
Under $\widetilde{M}_{24}$-equivariance the Serre functor acquires a projective Schur
cocycle of order $6$ in $H^2(M_{24}, \mathrm{U}(1)) \cong \mathbb{Z}/12$
(Bridgeland--Maciocia $2001$ on K3 autoequivalences). The displayed
Koszul dual coalgebra
of the Vol~III K3 chiral Hall--Drinfeld double is
\[
 (\mathbf{H}_{\Delta_5})^! \;\simeq\; V(\mathfrak{g}_{\Delta_5})^{\mathrm{coalg}}[2] \otimes \eta_{\widetilde{M}_{24}}.
\]
It is a Vol~III bridge input; it is not a consequence of Theorem~H.
\end{remark}
...
\begin{remark}[Etingof-Kazhdan Part V and uniqueness of the K3 synthesis]
...
Hall--Drinfeld construction, it supplies the candidate comparison
\[
