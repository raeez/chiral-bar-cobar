...
with generators in arity~$2$ and relations in arity~$3$, and
...
\]
The map $\sigma$ is an isomorphism on the arity-$2$ generator
...
space \textup{(}chiral arity~$2$\textup{)},
$R \subset \cF(E)(3)$ the relation space in free-operad
arity~$3$, and $\gamma$ the structural map.  The chiral Koszul
...
On the arity-$2$ generator piece, $\sigma$ restricts to the
...
to be the arity-$(\leq m)$ truncation of the chiral
...
homotopy $h_m^{\mathrm{ord}}$ of
$\operatorname{OS}(A_{m-1})^{\geq 1}$ onto the ground field.
Lemma~\ref{lem:chiral-quadratic-koszul} transports
$h_m^{\mathrm{ord}}$ through~$\sigma$ onto the ordered
chiral bar side, yielding a fibrewise contracting homotopy
...
divisor~\cite{}. The graph amplitude form $\omega_\Gamma$ is a
...
\cite[§2]{}, the propagator has the expansion
...
configuration with $S$ collapsed to a single point~\cite[§3]{}.
