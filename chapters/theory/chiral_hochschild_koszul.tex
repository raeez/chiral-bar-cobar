...
(Lemma~\ref{lem:arnold-three-point-hochschild}).
...
boundary form. The three-point Arnold relation ensures these contributions
...
\item \emph{PBW-Koszulness routes Arnold collapse without
 ...
 the global bar-concentration theorem. The argument
...
\item \emph{Surviving contribution.}\;
 ...
 collided to a single point of~$X$. After the Arnold--Koszul
 fibre contraction of \textup{(iii)}, this stratum contributes the coefficient
...
\item \emph{Amplitude from the base curve.}\;
 ...
 the Arnold--Koszul fibre contraction eliminates all fiber cohomology above
 degree~$0$.
\end{enumerate}
\end{proposition}
...
invoking the global bar-concentration theorem. The argument
...
the global bar-concentration theorem: the tower collapse is
...
The display in Corollary~\ref{cor:chirhoch-heisenberg} is the
cohomology of the fixed generic fibre~$\mathfrak H_k$.  The formal
deformation parameter ring of the one-parameter Heisenberg family is
instead a completed base such as $\mathbb C[[\kappa]]$ or
$\mathbb C[[k]]$, recording motion of the level.  This base is not the
vector space $\ChirHoch^\bullet(\mathfrak H_k,\mathfrak H_k)$ at a
fixed level; the single class in $\ChirHoch^1$ is the tangent direction
to that family, while the fixed-fibre Poincar\'e polynomial remains
$1+t+t^2$.
\end{remark}
...
deformation each derivation realises. It also records which apparent
parameter motions are \emph{not} elements of
$\ChirHoch^1$: a conformal-weight motion changes the chosen stress
tensor, and a simple-pole charge rescaling is killed by the zero-mode
gauge action. The breakdown across the standard landscape exhibits a
structural distinction between class~$\mathsf G$
\emph{level-deformable} families, class~$\mathsf L$
\emph{current-deformable} families, and simple-pole free-field
families whose contact/shadow parameters live outside the
curve-level Hochschild tangent.
\begin{center}
...
free fermion $\psi$
 & $0$
 & simple-pole Clifford innerness; bilinear rescaling is degree~$2$ \\
$\beta\gamma_\lambda$
 & $0$
 & charge rescaling inner; $\lambda$ changes the conformal vector \\
$V_k(\fg)$ generic
 & $\dim\fg$
...
\textit{(iii) The simple-pole firewall for class $\mathsf C$.}
For the $\beta\gamma$ and $bc$ systems the charge/ghost-number
rescaling is implemented by the zero mode of the current
$J={:}\beta\gamma{:}$ or ${:}bc{:}$, hence it is inner in
$\Der(\cA)/\Inn(\cA)$. The conformal-weight motion
$T\mapsto T+\epsilon\,\partial J$ moves the chosen conformal vector
and the scalar shadow parameter $\lambda$; it does not change the
underlying curve-level chiral product counted by $\ChirHoch^1$.
Thus $\dim\ChirHoch^1(\beta\gamma_\lambda)
=\dim\ChirHoch^1(bc_\lambda)=0$ on the Theorem~H surface. Class
$\mathsf C$ still has shadow depth $r_{\max}=4$: that depth is a
level-$5$ scalar/contact invariant, not a degree-$1$ Hochschild
tangent.
...
$\{\cH_k,\,V_k(\fg),\,\beta\gamma_\lambda,\,bc_\lambda,\,
\Vir_c,\,\cW_N\,(N\geq 3),\,\BP_k\}$ at generic parameter, the
curve-level chiral-product formal moduli space
...
For $\Vir_c$, $\cW_N$ at generic $c$, $\BP_k$ at generic $k$,
$\beta\gamma_\lambda$, $bc_\lambda$, and the free fermion:
$\ChirHoch^{1}(\cA)=0$ \textup{(}chiral rigidity, verified in the
chiral-Hochschild engine for $\cW_N$ at $N=3,\ldots,10$ and by the
simple-pole zero-mode computation for the free-field systems\textup{),}
so the obstruction map has empty domain and the formal moduli is
the reduced point.

For $\cH_k$ and $V_k(\fg)$: each
...
current $J^a$. Each family extends to all orders in $\epsilon$,
so each $\xi_i$ admits a Maurer--Cartan extension
$\Theta(\epsilon)=\xi_i\epsilon+
...
The conformal-weight family $\lambda\mapsto\lambda+\epsilon$ for
$\beta\gamma_\lambda$ and $bc_\lambda$ belongs to the enriched
conformal-vector/shadow atlas, not to this curve-level
chiral-product tangent.
\end{proof}
...
$\mathsf C$ contact & $\beta\gamma_\lambda$, $bc_\lambda$ & $0$ \\
...
\end{tabular}
\end{center}
For class~$\mathsf C$ this table concerns the chiral-product moduli
controlled by $\ChirHoch^1$. The conformal-weight line and the quartic
shadow tower remain part of the enriched free-field/contact atlas.
...
$\HH^*(A_{\mathrm{mode}})$ are finite and concentrated in the
standard Koszul examples, but their cohomological dimensions need not
agree. For the Heisenberg fibre the chiral table gives
$\dim\ChirHoch^\bullet(\cH_k)=3$, while the Weyl-mode algebra has
$\HH^0(A_1)=\bC$ and $\HH^n(A_1)=0$ for $n\ge1$. The genuine
...
The high-degree divergence used in the three-Hochschild comparison is
scalar Gel'fand--Fuchs cohomology, not THH. The \textbf{critical level}
$k=-h^\vee$ is a separate boundary surface: the Feigin--Frenkel centre
...
Deformations of the fermionic structure. The degree-$2$ class rescales
the simple-pole pairing
$b(z)c(w)\sim (1+\epsilon)/(z-w)$. The spin line
$(h_b,h_c)=(\lambda,1-\lambda)$ preserves the same OPE and changes the
chosen conformal vector; it belongs to the conformal/free-field atlas,
not to $\ChirHoch^1$. The product-deformation class gives:
