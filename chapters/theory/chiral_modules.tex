...
The bar-cobar machinery of the preceding chapters operates at the algebra level.  This chapter extends it to modules: for a chiral algebra $\cA$ and an $\cA$-module $\mathcal{M}$, the module bar construction $\barB_\cA(\mathcal{M})$ produces a comodule over the bar coalgebra, and the resulting functor $\Phi$ is an equivalence on appropriate categories.  For the Heisenberg algebra (Chapter~\ref{ch:heisenberg-frame}), this reduces to the classical Fock space duality; for affine Kac--Moody algebras, it produces the Feigin--Frenkel correspondence on Verma modules.
