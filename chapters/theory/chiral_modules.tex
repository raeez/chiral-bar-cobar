...
and derived Ext groups are computed on the bar-comodule side.
Outside this lane, the intrinsic bar-coalgebra/comodule formulation
is the correct object; a stronger module-category equivalence requires
additional hypotheses beyond Theorem~B. Second, the module-level
Verdier intertwining (Theorem~\ref{thm:module-bar-verdier}) exchanges
module bar complexes of a Koszul pair $(\cA, \cA^!)$, yielding
...
Theorem~\ref{thm:e1-module-koszul-duality}. Then $\Phi$ is the module
bar functor implementing the complete/conilpotent comparison
\[
\Phi\colon
...
used here. The module bar functor $\Phi=\barB(\cA;-)$
gives the complete module to conilpotent bar-comodule equivalence, the
quasi-inverse is the module cobar functor~$\Psi$, and the further
rewrite on the graded-dual algebra side is available only when the
finite-type identification $\barB(\cA)^\vee \cong \cA^!$ is justified.
\end{proof}
...
\item On any lane where Proposition~\ref{prop:module-koszul-equivalence}
 applies, the bar-cobar twisting datum exchanges $\cA$-modules with
 bar-comodules; only after finite-type dualization may one rewrite
 the target as a category of $\cA^!$-modules.
\item The dg-shifted Yangian $Y^{\mathrm{dg}}_\cA$ acts on line
 operators through the genus-$0$ convolution algebra. On finite-type
 dualized lanes, its completed module category may be rewritten in
 terms of modules over the graded dual algebra $\cA^!$.
\end{enumerate}
\end{construction}
...
pronilpotent completion of the genus-$0$ convolution algebra. On the
finite-type dualized lanes singled out by
Theorem~\ref{thm:e1-module-koszul-duality}, its completed module
category may be rewritten using the graded dual algebra~$\cA^!$.
\end{proof}
...
\;\xrightarrow{\;\sim\;}
\mathrm{CoMod}^{\mathrm{conil}}_{\barB(\mathcal{A})}
\]
...
hold. When $\barB(\mathcal{A})$ is finite-type graded so that
$\barB(\mathcal{A})^\vee \cong \mathcal{A}^!$, the induced complete
...
identification $\barB(\mathcal{A})^\vee \cong \mathcal{A}^!$ is
available.
\end{proof}
...
compatibility. Braiding reversal is a stronger statement and, on the
current proved surface, is established only on the specific
affine/Yangian Drinfeld--Kohno comparison lanes where KZ monodromy and
...
$M^! = \Phi(M)$ and $N^! = \Phi(N)$ for the corresponding
...
$\barB(\mathcal{A})^\vee \cong \mathcal{A}^!$ is available, one may
...
$\barB(\mathcal A)^\vee \cong \mathcal A^!$ is justified may one
...
$\Eone$ lane. This is the intrinsic theorematic content. Any further
translation into dual-algebra language, into $\mathrm{Tor}$ over
$\mathcal{A}^!$, or into singular-vector bookkeeping on the dual algebra
side, requires the
...
$\mathcal{D}$-module. In the homotopy module-algebra notation of
Convention~\ref{conv:bar-coalgebra-identity},
$\mathbb{D}\bar{B}_n(\cA; \vec{\mathcal{M}})
\simeq \bar{B}_n(\cA^!; \vec{\mathcal{M}}^!)$,
...
\begin{conjecture}[Conditional conformal-block comparison from braided categorical duality; \ClaimStatusConjectured]
...
\textup{(}Proposition~\textup{\ref{conj:koszul-t-structures})}.
Denote by
...
Proposition~\ref{conj:conformal-block-duality} is now explicitly a
conditional categorical comparison, not a theorematic output of the
general bar-cobar package. It is consistent with complementarity
...
comparison under Koszul duality is not proved here in this generality.
\end{remark}
...
Kac--Moody conformal blocks. This is compatible with the expectation
that any semisimple categorical conformal-block comparison, when
available on the affine side, should descend through BRST reduction.
However, no general descended conformal-block duality theorem is proved
here beyond those explicit categorical input surfaces.
\end{remark}
...
Remark~\ref{rem:ds-conformal-block-comparison-scope} is expected to
descend through the BRST reduction on surfaces where the required
categorical hypotheses are known.
...
\begin{conjecture}[Conditional highest-weight Koszul t-structure from bar linearity; \ClaimStatusConjectured]
\label{conj:koszul-t-structures}
\index{t-structure!Koszul duality}
\index{derived category!module Koszul duality}
Let $(\mathcal{A}, \mathcal{A}^!)$ be a Koszul pair of chiral
algebras. Assume the module-category $t$-exactness package of
...
What remains is the internal-degree statement. For a simple object
...
On any highest-weight lane where Proposition~\ref{conj:koszul-t-structures}
...
to tilting modules at the dual level. Such a statement would require a
separate categorical comparison on the affine Drinfeld--Kohno surface,
not merely the lax monoidal bar-comodule theorem.
\end{remark}
...
controlling the block. This shows how the
representation-theoretic data of a non-semisimple module category is classified by the bar complex.
\end{remark}
...
surviving $E_2$ packet is finite and produces the admissible
character formula. In general rank, the corresponding finite
admissible Weyl packet is part of the external
...
Feigin--Frenkel self-dual point ($-(-2)-4 = -2$).
...
\begin{proposition}[\texorpdfstring{$\mathrm{Ext}$}{Ext} complementarity at dual levels; \ClaimStatusConditional]
...
\begin{proposition}[Characters under level-shifting Koszul duality; \ClaimStatusConditional]
...
$\operatorname{ch}(L_k(\lambda)) = \operatorname{ch}(L_{k'}(\lambda))$
holds because the bar complexes at levels $k$ and $k'$ have the same
graded dimensions (Verma module spaces depend only on~$\mathfrak{g}$);
the differentials differ but cancel in the Euler characteristic. This
does \emph{not} imply $\mathcal{O}_k \simeq \mathcal{O}_{k'}$
...
Proposition~\ref{conj:koszul-t-structures}; the general module
bar-comodule package alone does not supply an equivalence of
...
target should be read as a $\barB(\widehat{\mathfrak{sl}}_{2,k})$-comodule.
...
Principal Drinfeld--Sokolov reduction commutes with the
...
The curvature $m_0^{(g)} = \kappa(\cA) \cdot \lambda_g$ \textup{(UNIFORM-WEIGHT)} mixes
...
$\barB(\cA)^\vee \cong \cA^!$ is justified)
...
When $\kappa(\cA) \neq 0$, the curvature $m_0^{(g)} = \kappa \cdot \lambda_g$ \textup{(UNIFORM-WEIGHT)} mixes genus components: individual $M^{(g)}$ are curved ($d^2 = m_0^{(g)} \cdot \mathrm{id}$) and only $M^{\mathrm{total}} = \widehat{\bigoplus}_g \hbar^{2g-2} M^{(g)}$ has $d^2 = 0$. At critical level $\kappa = 0$, the genus-grading becomes strict and classical module theory is recovered as the $g = 0$ summand.
\end{remark}
...
The module theory built here for chiral algebras on the Lorentzian-lattice
B-family acquires a preferred grading under CY-2 Serre duality:
...
\]
Three faces of $8$ coincide: (a) Mukai doubling, (b) order of monodromy
of the holonomic $D$-module $L^{\Delta_5}$ around the Humbert divisor
$H_1\subset\bar{\mathcal A}_2$, (c) Lusztig specialisation $\ell = 8$ of
the Hall--Drinfeld double at $\zeta$ with $\zeta^8 = 1$. Bruinier 2002
Prop.~5.1 Heegner Chern-class reciprocity identifies all three with the
same $\mathbb Z/8$-class in $\mathrm{CH}^1(H_1)$. The universal
identity $\hbar^2 \cdot K^{\kappa_{\mathrm{ch}}} = -1$ on the B-family
then forces $\hbar^2 = -1/8$, which is the precise root-of-unity at
which $\mathbf H_{\Delta_5}$-modules admit a full curved lift.
...
Under $\widetilde M_{24}$-equivariance (Schur cover of $M_{24}$), the
Serre functor on $D^b\mathrm{Coh}(K3)$ acquires a projective cocycle of
order $6$ in
\[
H^2(M_{24},U(1)) \;=\; \mathbb Z/12 \;=\; \mathbb Z/2 \times \mathbb Z/2 \times \mathbb Z/3,
\]
via the Conway--Curtis ATLAS 1985 Schur multiplier. The specific cocycle
on the Serre functor has order $6 = 2\cdot 3$ (it discards one $\mathbb Z/2$
factor). The Gaberdiel--Hohenegger--Volpato 2012 non-Mukai classes
$\{7A, 7B, 11A, 23A, 23B\}$ are precisely the classes on which this
cocycle cannot be trivialised: on chiral modules over $\mathbf H_{\Delta_5}$
they appear as projective representations with genuine cocycle. The
Eguchi--Ooguri--Tachikawa 2011 observation that the elliptic genus of
$K3$ decomposes under $M_{24}$ through umbral shifts $m_\sigma$
(Cheng--Duncan--Harvey 2014) is the trace-level shadow of this cocycle.
\end{remark}

\begin{remark}[Chiral module $\Leftrightarrow$ coherent $K3$-sheaf via $\Phi$]
\label{rem:chiralmod-3}
The Vol~III CY-to-chiral factorisation
$\Phi = \SpCh_{\Sigma_1, C} \circ \PhiFA_2$, specialised along a
genus-one cycle $\Sigma_1 \subset K3$ and restricted to a reference
curve $C$, sends
$D^b\mathrm{Coh}(K3)\to\mathbf H_{\Delta_5}\text{-}\mathrm{mod}^{\mathrm{ch}}$
on the stage-$2$ module side, matching the Mukai pairing (signature
$(4,20)$) against the chiral Hochschild pairing on
$\mathbf H_{\Delta_5}$. The averaging morphism
$\mathrm{av} = \mathrm{Torelli}_{K3}\circ\mathrm{KugaSatake}\circ j_{\mathrm{Kod}}$
descends this to ordinary $K3$-moduli. Under this dictionary, simple
$\mathbf H_{\Delta_5}$-modules correspond to stable $K3$-sheaves with
Mukai vector of discriminant divisible by $8 = K^{\kappa_{\mathrm{ch}}}$.
