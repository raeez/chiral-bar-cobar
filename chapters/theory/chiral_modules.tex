...
$\Theta^{\mathrm{oc}}_\cA = \Theta_\cA + \sum \mu^{M_j}$ packages
the closed-sector element $\Theta_\cA$ with boundary data: the
module terms $\mu^{M_j}$ are the open-sector projections.  This
chapter constructs the module-level bar complex that produces those
open-sector components.  A chiral module over~$\cA$ on a curve~$X$
is a factorization algebra on a pointed manifold $(X, x)$, where
the distinguished point carries module data and the remaining points
carry algebra data.  Passing from algebras to modules introduces
actions, boundary conditions, and defect operators into the
formalism.

The module bar construction
$\barB_\cA(\mathcal{M})$ produces a comodule over the bar coalgebra
$\barB(\cA)$.  The intrinsic comparison lives on the same transferred
and curved bar-coalgebra surface as for algebras: at higher genus one
obtains curved module resolutions in the coderived framework rather
than a blanket ordinary-derived module equivalence.  The manuscript's
proved ordinary module-level Koszul duality is more restrictive: on
the quadratic genus-$0$ $\Eone$ complete/conilpotent lane, bar
resolutions compute Ext groups via the bar dual and induce the module
comparison of Theorem~\ref{thm:e1-module-koszul-duality}.  Outside
that lane, one should keep the intrinsic bar-coalgebra/comodule
formulation, and any stronger module-category equivalence requires
additional hypotheses beyond those assumed for Theorem~B.  In the
...
categorical MC3 results on the Yangian side
\textup{(}categorical Clebsch--Gordan closure plus the generated-core
DK comparison proved for all simple types; post-CG completion open
beyond type~$A$; Corollary~\ref{cor:mc3-all-types}\textup{)} are
...
\simeq \cA^{!}_{\infty,n}(\vec{\mathcal{M}}^!)$ as factorization algebras.
...
$\gcd(p,q) = 1$) that is moreover non-degenerate:
