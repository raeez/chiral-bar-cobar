...
\begin{remark}[Scope]
This theorem is labeled \ClaimStatusConjectured{} because the precise
``deformation'' framework --- a family of complexes $\mathcal{P}_\bullet(t)$
parametrized by a point in a character variety or moduli space --- has not
been rigorously constructed.  The Gauss--Manin connection $\nabla_t$ on
the homology bundle is well-defined when the family is a flat family of
sheaves over a smooth base (Katz--Oda), but the transition from acyclic
to non-acyclic requires passing through a singular fiber where standard
Gauss--Manin theory breaks down.  The expected proof route is through
the theory of irregular connections (Sabbah, Mochizuki) or through
$\mathcal{D}$-module techniques adapted to the chiral setting.
\end{remark}
