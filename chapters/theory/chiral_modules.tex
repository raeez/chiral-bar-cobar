...
A chiral algebra without its modules is a ring without its
representations: the structure is visible only through its shadows on
representation categories.  For a chiral algebra $\cA$ and an
$\cA$-module $\mathcal{M}$, the module bar construction
$\barB_\cA(\mathcal{M})$ produces a comodule over the bar coalgebra,
and the resulting functor~$\Phi$ is an equivalence on appropriate
categories.  For the Heisenberg algebra, this is Fock space duality;
for affine Kac--Moody, the Feigin--Frenkel correspondence on Verma
modules.
