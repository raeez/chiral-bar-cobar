...
Verdier intertwining $\mathbb{D}_{\Ran}\bar{B}(\cA) \simeq \bar{B}(\cA^!)$ gives a Koszul dual identity on conformal blocks:
\[
\mathbb{V}(\mathcal{A};\, \vec{\mathcal{M}};\, X)
\;\cong\;
\mathbb{V}(\mathcal{A}^!;\, \vec{\mathcal{M}}^!;\, X)
\]
for Koszul modules $\mathcal{M}_i$ with duals $\mathcal{M}_i^!$.  This provides a geometric proof that $\dim \mathbb{V}$ is invariant under $k \mapsto -k - 2h^\vee$, distinct from the classical level-rank duality (\cite{Beauville}, \cite{NakanishiTsuchiya}).
\end{remark}
