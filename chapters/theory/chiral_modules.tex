...
\noindent\emph{Governing question.} How does Koszul duality act on
representation categories, not only on algebras?  This chapter promotes
the bar-cobar mechanism to the module level and isolates which parts are
formal from which depend on Koszulity hypotheses.

\begin{remark}[Semantic levels; Convention~\ref{conv:hms-levels}]
\label{rem:chiral-modules-hms}
Module categories and the module bar construction are developed at \textbf{M-level}: explicit pointed configuration spaces, module bar differentials, and representation-theoretic equivalences.  The H-level formulation (modules as factorization algebras on pointed spaces, Swiss-cheese $\infty$-operads) frames the definitions; the proofs are M-level throughout.
\end{remark}
...
Yangians on the evaluation locus.  The full factorization
categorical equivalence $\operatorname{Fact}_{E_1}(Y(\mathfrak{g}))
\simeq \operatorname{Fact}_{E_1}(U_q(\mathfrak{g}))^{\mathrm{op}}$
remains conjectural; see
...
realization remains on the MC4 frontier.
\end{proof}
...
counterpart of the vanishing at level~$k$, as a conjectural
complementarity extension (\ClaimStatusConjectured) of the
complementarity theorem
(Theorem~\ref{thm:quantum-complementarity-main}, which requires the Koszul hypothesis at generic level): what the
...
infinite-generator realization.  Therefore the result is a
...
module-theoretic argument of this section.
\end{remark}
