...
Ayala--Francis~\cite{AF15}---is that a module is a factorization algebra
...
$\Omega_{ij}/(k + h^\vee)$.  Under Koszul duality, the
Casimir~$\Omega$ is exchanged for the dual
Casimir~$\Omega^!$ with level shift $k \mapsto k'$, and the
monodromy representations are intertwined by the tensor
functor~$\Phi$.
\end{proof}
...
We compute the genus-$1$ layer of the genus-graded Verma module
$\mathcal{M}(0)_k$ for $\widehat{\mathfrak{sl}}_2$ at level~$k$.
...
(Definition~\ref{def:bar-complex-modules}).  We now show this
resolution computing Ext groups, producing the first non-trivial
Ext calculation via chiral bar complexes.
...
The theorem assembles three ingredients, all proved earlier in
this chapter:
\begin{itemize}
...
We verify the simplest case of
Theorem~\ref{conj:fusion-bar-cobar}: the Heisenberg algebra
$\mathcal{H}_\kappa$ acting on Fock modules
$F_\lambda$, $F_\mu$.  The result is that the module bar-cobar
functor $\Phi$ of
Theorem~\ref{thm:e1-module-koszul-duality} preserves the
fusion product at genus~$0$.
...
The module Koszul duality functor $\Phi$ maps representations to
representations, and the bar resolution computes the Ext groups
that govern non-semisimple categories.  But a deeper question
remains: what controls the deformations of chiral algebras
\emph{themselves}?  For associative algebras, the answer is
Hochschild cohomology; for chiral algebras, the answer is a
chiral enhancement that incorporates OPE data and the geometry
of the moduli space $\overline{\mathcal{M}}_g$.  The
deformation-obstruction calculus underlying
