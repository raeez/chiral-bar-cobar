...
\end{equation}
\textup{(}replacing $q \to -q$ in the Kac formula, which flips the sign of the denominator $4pq \to -4pq$\textup{)}.  In particular:
\begin{enumerate}[label=\textup{(\roman*)}]
...
Each factor $h_{r,s}(c) = ((pr-qs)^2 - (p-q)^2)/(4pq)$ transforms under $q \to -q$:
the numerator becomes $(pr+qs)^2 - (p+q)^2$ while the denominator becomes $4p(-q) = -4pq$.
Thus $h_{r,s}(26-c) = ((p+q)^2 - (pr+qs)^2)/(4pq)$.
...
= H^*(\mathcal{M}_g, Z(\mathrm{Vir}_c))$ for all such~$c$.
\end{remark}
