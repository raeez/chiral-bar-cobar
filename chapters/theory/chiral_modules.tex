...
$\bar{B}(\cA)$.  The Koszul property descends: if $\cA$ is Koszul, every module $M$
admits a bar resolution whose $E_2$-collapse computes Ext groups.
The resulting module Koszul duality exchanges representation categories
under $k \leftrightarrow -k - 2h^\vee$.

The key idea (Voronov~\cite{Voronov99}, Costello--Gwilliam~\cite{CG17}, Ayala--Francis~\cite{AF15}) is that a module is a factorization algebra on a pointed space, where the marked point carries the module data and the remaining points carry algebra data.
...
constraining conformal block dimensions at level~$k'$ through
the center.  At admissible levels the complementarity gives an
explicit relationship between the conformal block ranks.  At
positive integer level, the bar complex computes the Verlinde
dimensions exactly (Theorem~\ref{thm:verlinde-bar}).
\end{remark}
...
For (iii): the KZB connection arises from the variation
of the propagator $\eta_{ij}$ on~$\overline{C}_2(X)$
...
$\Omega_{ij}/(k + h^\vee)$.  Under Koszul duality,
$\Omega$ is exchanged for the dual
Casimir~$\Omega^!$ with level shift $k \mapsto k'$,
and the monodromy representations are intertwined by~$\Phi$.
\end{proof}
...
\item Under the Kazhdan--Lusztig equivalence
  ...
  tilting module $T_q(\Lambda) \in \mathcal{C}(U_q(\mathfrak{g}))$.
...
We compute the Weyl--Kac character of the vacuum module for
$\widehat{\mathfrak{sl}}_2$ at level~$1$.
...
(Theorem~\ref{thm:e1-module-koszul-duality})
has been developed above; we now apply it to specific modules.
...
In each case, acyclicity of the bar-cobar resolution forces infinite
alternating character sums to collapse to closed product formulas.
When acyclicity fails (critical or admissible levels), the higher
cohomology groups carry the obstruction data
(Theorem~\ref{thm:deformation-acyclicity}).
\end{remark}

\section{Localization and D-modules}\label{sec:localization}

Module Koszul duality has a geometric counterpart: Beilinson--Bernstein localization bridges the bar-cobar framework and the representation theory of flag varieties.
...
We establish the compatibility of Drinfeld--Sokolov reduction
$H^0_{\mathrm{DS}}$ with the bar-cobar framework and module Koszul duality.
...
\begin{proof}
This is an immediate consequence of the monoidal module Koszul duality
theorem (Theorem~\ref{thm:monoidal-module-koszul}), which proves that
...
Theorem~\ref{thm:monoidal-module-koszul}, equation~\eqref{eq:monoidal-bar},
constructs the natural transformation
$\mu_{M_1, M_2}\colon \Phi(M_1 \boxtimes_\cA M_2) \to
\Phi(M_1) \boxtimes_{\cA^{!,c}} \Phi(M_2)$
via the Alexander--Whitney factorization on configuration spaces (Step~1
of that proof), and proves it is a quasi-isomorphism when $M_1, M_2$ are
Koszul modules (Step~2, using the K\"unneth theorem and diagonal
concentration).
...
The theorem assembles three ingredients proved earlier in this chapter:
\begin{itemize}
\item The module-level bar-cobar adjunction
  (Theorem~\ref{thm:e1-module-koszul-duality}), which provides
  $\Phi$ as an exact functor.
\item The monoidal structure on $\Phi$
  (Theorem~\ref{thm:monoidal-module-koszul}), which closes the key gap:
  the bar coalgebra
  coproduct~\eqref{eq:bar-coproduct-fusion} intertwines with the
  fusion tensor product via the Alexander--Whitney factorization.
...
\end{itemize}
The monoidality of $\Phi$ was previously the hardest part of the
Kazhdan--Lusztig programme~\cite{KL93}.  The configuration space
proof via the bar complex resolves this by showing that the residue
...
The deformation-obstruction calculus underlying
Theorem~\ref{thm:quantum-complementarity-main} is the subject of
the next chapter.
