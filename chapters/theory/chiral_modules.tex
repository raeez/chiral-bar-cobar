...
For a Koszul dual pair $(\mathcal{A}, \mathcal{A}^!)$,
...
\]
(see Appendix~\ref{app:existence-criteria}).  When $\mathcal{A}^!$ is finite-type graded, one further obtains $\mathrm{Mod}^{\mathrm{compl}}_{\mathcal{A}} \simeq \mathrm{Mod}^{\mathrm{compl}}_{\mathcal{A}^!}$; without finite-type, the intermediate comodule step is essential.  See \S\ref{sec:complete-calculations} for explicit character computations.
\end{remark}
...
(Proposition~\ref{prop:virasoro-bgg-bar}).  The surviving $E_2 = E_\infty$
...
(Theorem~\ref{thm:ds-koszul-commute}), so the conformal block
