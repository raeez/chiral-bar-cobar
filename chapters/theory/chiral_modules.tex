...
Until this point, the monograph has spoken only about algebras:
chiral algebras, their bar complexes, their Koszul duals, and the
adjunctions and inversions that relate them.  This chapter introduces
the open sector.  A chiral module is where the formalism stops
describing the algebra in isolation and begins describing its actions,
its probes, its boundary conditions.  In the physics language, the
passage from algebras to modules is the passage from the closed string
to the open string, from bulk operators to boundary and defect
operators.  In the mathematical language, it is the passage from
a factorization algebra on a manifold~$M$ to a factorization algebra
on a pointed manifold $(M, x)$, where the distinguished point carries
module data and the remaining points carry algebra data.
...
appropriate homotopy categories.  This is not a parallel story running
alongside the bar-cobar theory of the preceding chapters; it is part
of the same transferred and curved world.  When $\cA$ is chiral
...
$\mathbb{D}_{\Ran}\bar{B}(\cA) \simeq \bar{B}(\cA^!)$
...
The Heisenberg case is exceptional: $d_{\mathrm{cross}} = 0$ because the OPE is purely double-pole.  For non-abelian $\widehat{\fg}_k$, $d_{\mathrm{cross}} \neq 0$ and the bar complex of the fusion product does not split; monoidality requires the full separating-divisor machinery of Theorem~\ref{thm:chain-modular-functor}.
\end{remark}
