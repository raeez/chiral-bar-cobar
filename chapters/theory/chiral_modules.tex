...
$A(\mathrm{Vir}_c) \cong A(\mathrm{Vir}_{26-c})$ follows.  At this
level, $\mathrm{Vir}_{26-c}$ is the proved same-family shadow partner
detected by Zhu invariance; the stronger H-level infinite-generator
...
\begin{remark}[\texorpdfstring{$\mathrm{Ext}$}{Ext} complementarity at dual levels; \ClaimStatusProvedHere]
...
involution, $\bar{B}(\mathrm{Vir}_c)$ is identified with the coalgebra
  model attached to the same-family partner $\mathrm{Vir}_{26-c}$; this
  is the current M-level shadow, not the unresolved H-level
  infinite-generator realization delegated to the exact
...
= H^*(\mathcal{M}_g, Z(\mathrm{Vir}_c))$ for all such~$c$.  Here the
  ...
  the stronger H-level infinite-generator realization lies beyond the
  module-theoretic argument of this section and is delegated to the exact
  $W_\infty$ MC4 package of filtered H-level targets, residue identities,
  and finite detection on $\mathcal{I}_N$, whose first live
  higher-spin stages are already reduced on the theorem surface to the
  stage-$3$ fifteen-coefficient packet and the stage-$4$
...
\textup{(}Arakawa~\textup{\cite{Ara07}}; at generic level
the vanishing also follows from irreducibility of Verma
modules; cf.\
Proposition~\textup{\ref{prop:ds-admissible}(i)}; at generic level
the vanishing follows from irreducibility of Verma modules\textup{)}:
...
\item The fusion product $\boxtimes_k$ on
  ...
  determined by the level shift.  At chain level one expects a
  non-semisimple lift before semisimplification.
