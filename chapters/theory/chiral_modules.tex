...
& \mathrm{Rep}(U_q(\mathfrak{g}))
  \ar[d, "q \mapsto q^{-1}"]
\\
\mathcal{O}_{k'}^{\mathrm{int}}(\widehat{\mathfrak{g}})
  \ar[r, "\Phi_{\mathrm{KL}}", "\sim"']
& \mathrm{Rep}(U_{q^{-1}}(\mathfrak{g}))
\end{tikzcd}
\]
where $q = e^{\pi i/(k + h^\vee)}$ and $k' = -k - 2h^\vee$.  The
...
\mathrm{Rep}(U_q(\mathfrak{g})),
\end{equation}
sending the integrable module $L(\Lambda)$ to the
finite-dimensional irreducible $U_q(\mathfrak{g})$-module
$V_q(\Lambda)$ and identifying the fusion product
$\boxtimes_{\widehat{\mathfrak{g}}_k}$ with the quantum group
tensor product~$\otimes_q$.

The equivalence sends the braiding
\textup{(}monodromy of conformal blocks on $\mathbb{P}^1$\textup{)}
to the $R$-matrix braiding on $\mathrm{Rep}(U_q(\mathfrak{g}))$.
\end{theorem}
...
$L(\Lambda)$ to $V_{q^{-1}}(\Lambda)$); commutativity on
...
\item \emph{Limitation.}
  ...
  the Zhu algebra $A(\mathcal{W}^k(\mathfrak{g}))$ depends on the
  level~$k$ through non-linear relations among generators of different
  weights, and the Koszul dual may have a different Zhu algebra.
\end{enumerate}
\end{remark}
...
$m_1$ is the bar differential, $m_2(a \otimes p) =
