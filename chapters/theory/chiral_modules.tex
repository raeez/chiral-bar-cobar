...
$(\mathcal{M}, \mathcal{M}^!)$ consists of a Koszul $\cA$-module~$\mathcal{M}$
and a Koszul $\cA^!$-module~$\mathcal{M}^!$ such that the module bar
complex satisfies
$H^*(\bar{B}(\cA;\, \mathcal{M})) \cong \mathcal{M}^!$
as a $\bar{B}(\cA)$-comodule; equivalently, the module Koszul
duality functor (Definition~\ref{def:module-koszul-functor}) sends
$\mathcal{M}$ to~$\mathcal{M}^!$ up to quasi-isomorphism.
\end{definition}
