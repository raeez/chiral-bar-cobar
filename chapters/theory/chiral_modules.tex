...
\item For the affine vertex algebra
  ...
  $X_{V_k(\mathfrak{g})} = \{0\}$, which occurs at the
...
The associated variety is \emph{not} preserved by level-shifting Koszul duality: e.g., $X_{V_1(\mathfrak{sl}_2)} = \{0\}$ but $X_{V_{-3}(\mathfrak{sl}_2)} = \mathfrak{sl}_2^*$.  When $X_V = \{0\}$, the bar spectral sequence degenerates at a finite page; for $X_V \neq \{0\}$ it may have non-trivial higher differentials---the passage from Koszul to curved $A_\infty$.
\end{remark}
...
Koszul duality maps rational vertex algebras to maximally non-rational ones.
...
reverses: $L_{-6}(\mathfrak{sl}_2)$ is \emph{not} rational
(its associated variety
$X_{L_{-6}} = \mathcal{N} \neq \{0\}$ by orbit duality,
Proposition~\ref{prop:orbit-duality}),
and the module category has non-trivial extensions.

More precisely, what the rational side sees as semisimplicity
(Ext-vanishing), the dual side sees as non-trivial extensions.
This follows from three ingredients already established in this
chapter:
\begin{enumerate}[label=\textup{(\roman*)}]
...
\item \emph{Associated variety obstruction}
  (Theorem~\ref{thm:arakawa-rationality}):
  $X_{L_{k'}} = \mathcal{N} \neq \{0\}$ implies the module category
  at level~$k'$ is not semisimple, so $\mathrm{Ext}^i \neq 0$ for
  some modules and some $i > 0$.
\end{enumerate}
...
= \frac{k(k+4)}{2(k+2)} \cdot \frac{1}{24}
= \frac{k(k+4)}{48(k+2)}.
\end{equation}
At $k = 1$, this gives $m_0^{(1)} = 5/144$;
at $k = 0$ or $k = -4$, the curvature vanishes.
