...
The bar-cobar adjunction extends from algebras to modules: chiral
modules over~$\cA$ correspond to comodules over~$\barB(\cA)$, and the
bar differential on modules detects extensions via collision residues on
labeled configuration spaces.
\end{remark}
...
$\mathcal{C}(U_q(\mathfrak{g}))$ via the Kazhdan--Lusztig equivalence
(Theorem~\ref{thm:km-quantum-groups}).

The fusion product is computed by integrating over $C_2(X)$ and
taking the limit $z_2 \to z_1$, which is why module categories for
chiral algebras are more structured than for general associative algebras.
\end{remark}
...
The bar resolution of $\mathcal{M} \in \mathrm{Mod}_{\mathcal{A}}$
produces a comodule over $\bar{B}(\mathcal{A}) \simeq \mathcal{A}^{!,c}$,
and the cobar construction inverts this on conilpotent objects; see
\S\ref{sec:complete-calculations} for explicit character computations.
\end{remark}
...
The bar complex computation has a Koszul dual:
\[
...
by the bar-cobar quasi-isomorphism applied termwise.  This gives a
geometric proof that $\dim \mathbb{V}$ is invariant under
$k \mapsto -k - 2h^\vee$, distinct from the classical level-rank duality
...
(Theorem~\ref{thm:quantum-complementarity-main}) gives
\[
Q_g(\widehat{\mathfrak{g}}_k) +
Q_g(\widehat{\mathfrak{g}}_{k'}^!)
= H^*(\mathcal{M}_g, Z(\widehat{\mathfrak{g}}_k)),
\]
constraining conformal block dimensions at level~$k'$
through the center.  At admissible levels the complementarity
gives an explicit relationship between the conformal block
ranks.  At positive integer level, the bar complex computes
the Verlinde dimensions exactly
...
(Theorem~\ref{thm:quantum-complementarity-main}): $Q_g(\mathcal{A})$
and $Q_g(\mathcal{A}^!)$ are exchanged by Verdier duality, and the
...
\textup{[Category: full abelian root-of-unity
$\mathrm{Rep}_{\mathrm{fd}}(U_q(\mathfrak{g}))$.]}
the Koszul t-structure on
$D^b(\mathrm{Rep}(U_{q^{-1}}(\mathfrak{g})))$
corresponds to the t-structure associated with the
canonical basis of $U_{q^{-1}}$---the basis elements
are precisely the images of the simple modules under
bar-cobar duality.
\end{remark}
...
which at a root of unity~$q$ is non-semisimple with infinitely
many indecomposable objects.
Rather, the correct target is the semisimplified quotient of
the tilting category: one takes the full subcategory of tilting
modules and quotients by the tensor ideal of negligible morphisms.
The resulting modular tensor category
$\mathcal{C}(U_q(\mathfrak{g}))$ is semisimple with $|P_+^k|$
simple objects, matching the integrable modules at level~$k$.
For $\mathfrak{sl}_2$ at level~$k$, this gives
...
BGG reciprocity has a transparent
...
For a Koszul algebra, the Koszul resolution is minimal, so
$[P(\lambda) : \mathcal{M}(\mu)] =
\dim \mathrm{Ext}^*_{\mathcal{O}}(\mathcal{L}(\lambda),
\mathcal{L}(\mu))$.
BGG reciprocity then says the bar complex is
self-dual under the Shapovalov form---a consequence of
bar-cobar duality (Theorem~\ref{thm:bar-cobar-isomorphism-main})
combined with self-contragredience of Verma and simple modules.
\end{remark}
...
The associated variety is \emph{not} preserved by level-shifting
Koszul duality.
For $\widehat{\mathfrak{sl}}_2$ at level $k = 1$:
$X_{V_1(\mathfrak{sl}_2)} = \{0\}$, while the Koszul dual at
$k' = -3$: $X_{V_{-3}(\mathfrak{sl}_2)} = \mathfrak{sl}_2^*$
(the full nilpotent cone).  The level shift
$k \leftrightarrow -k - 2h^\vee$ transforms $k = -h^\vee + p/q$
into $k' = -h^\vee - p/q$, which can map rational algebras to
non-rational ones and vice versa.

When $X_V = \{0\}$, the bar spectral sequence
(Theorem~\ref{thm:deformation-acyclicity}) degenerates at a
finite page; for $X_V \neq \{0\}$ it may have non-trivial
higher differentials.  This is the passage from Koszul
(generic level) to curved $A_\infty$ (special level).
\end{remark}
...
The bar complex detects non-semisimplicity:
\begin{enumerate}[label=\textup{(\roman*)}]
\item \emph{Semisimple} ($X_V = \{0\}$, rational):
  bar spectral sequence degenerates at $E_2$; the Verlinde formula
  computes $\dim \mathbb{V}$ exactly.
\item \emph{Non-semisimple} ($X_V = \{0\}$,
  $C_2$-cofinite, not rational): higher differentials encode
  extensions between simples.  The modified Verlinde formula
  (Creutzig--Ridout~\cite{CreutzigRidout2013}) uses pseudo-trace
  functions, computed by the bar complex via the full projective
  resolution.
\item \emph{Wild} ($X_V \neq \{0\}$): the bar spectral
  sequence need not converge; bar-cobar reduces to the curved
  $A_\infty$ regime (\S\ref{sec:deviations-triviality}).
\end{enumerate}
\end{remark}
...
The formula~\eqref{eq:weyl-kac-sl2} exemplifies
Theorem~\ref{thm:weyl-kac-geometric}: an integrable module character
is a ratio of a theta function (Weyl group sum via screening operators
on $\overline{C}_k(X)$) and the eta function (universal bar complex
denominator).  The modular properties---the $S$-transformation
$\tau \mapsto -1/\tau$ exchanging
$L(\Lambda_0) \leftrightarrow L(\Lambda_1)$---follow from the Verlinde
formula but are not directly visible from the bar-cobar framework;
the bar complex produces the character but not the modular structure,
which requires the braided tensor category theory of
...
\label{rem:ext-koszul-dual-level}% backward-compatible label
...
$\operatorname{ch}(L_k(\lambda)) =
\operatorname{ch}(L_{k'}(\lambda))$
has a transparent bar-cobar interpretation: the bar complexes
$\bar{B}^{\mathrm{ch}}(\widehat{\mathfrak{g}}_k, L_k(\lambda))$
and
$\bar{B}^{\mathrm{ch}}(\widehat{\mathfrak{g}}_{k'}, L_{k'}(\lambda))$
have the same \emph{graded dimensions} term by term (since
the underlying vector spaces of the Verma modules depend
only on~$\mathfrak{g}$, not on~$k$).  The difference lies
in the \emph{differentials}: at level~$k$ the bar
differential $d_{\mathrm{bar}}$ depends on the OPE
coefficients, which include the level.  For the character
(Euler characteristic), the differentials cancel in the
alternating sum, leaving only the graded dimensions.
...
same-family partner $c' = 26 - c_{p,q} > 25$ is irrational (in general)
and the partner Virasoro algebra $\mathrm{Vir}_{c'}$ has no minimal
...
independently of rationality, at generic~$c$ where the Koszul property holds (Theorem~\ref{thm:virasoro-chiral-koszul}): the genus-$g$ quantum corrections
satisfy $Q_g(\mathrm{Vir}_c) + Q_g(\mathrm{Vir}_{26-c})
= H^*(\mathcal{M}_g, Z(\mathrm{Vir}_c))$ for all such~$c$.  Here the
  second summand is the contribution of the same-family shadow partner;
  the H-level infinite-generator realization is the MC4 frontier
  (Remark~\ref{rem:virasoro-mc4-scope}).
\end{remark}
...
The singular support computation connects to
Nadler's microlocal sheaf theory.  The category of
...
Example~\ref{ex:ext1-admissible-fractional} illustrates the
general mechanism: for $\widehat{\mathfrak{sl}}_2$ at admissible
level $k = p/q - 2$, the admissible category
$\mathcal{O}^{\mathrm{adm}}_k$ has $(p{-}1)(q{-}1)/2$ simple
objects $L(\lambda_{r,s})$.  The bar spectral sequence
applied to $B_\bullet(\cA) \otimes_\cA L(\lambda_{r,s})$
computes $\mathrm{Ext}^n_\cA(L(\lambda_{r,s}), \mathbb{C})$
at the $E_2$ page, which encodes the BGG resolution of
$L(\lambda_{r,s})$ and determines its character via alternating
sums of Verma characters.  At the dual level
$k' = -k - 4$, the complementary BGG resolution
produces a dual character formula
(Corollary~\ref{cor:singular-vector-symmetry}).
\end{remark}
...
The Heisenberg case is exceptional: $d_{\mathrm{cross}} = 0$
because the OPE is purely double-pole.  For Kac--Moody algebras
$\widehat{\fg}_k$ with $\fg$ non-abelian, the simple-pole
residue $[J^a, -]$ is non-trivial, so
$d_{\mathrm{cross}} \neq 0$ and the bar complex of the fusion
product does not split as a tensor product.  Monoidality in
that setting requires the full separating-divisor machinery of
Theorem~\ref{thm:chain-modular-functor}; see
\S\ref{subsec:km-fusion-monoidality} for the Kac--Moody case
and Theorem~\ref{thm:fusion-bar-cobar} for the general
statement.
\end{remark}

\begin{remark}[Status of module equivalences]%
\label{rem:module-equivalences-summary}
\emph{Proved:}
geometric bar construction for modules
(Theorem~\ref{thm:geometric-bar-module}),
Koszul resolution for modules
(Theorem~\ref{thm:koszul-resolution-module}),
monoidal module Koszul equivalence
(Theorem~\ref{thm:monoidal-module-koszul}),
module $\Eone$-Koszul duality
(Theorem~\ref{thm:e1-module-koszul-duality}),
chain-level modular functor
(Theorem~\ref{thm:chain-modular-functor}).
\emph{Unconditional for Kac--Moody, Virasoro, and principal
finite-type $\mathcal{W}$}
\textup{(}Theorems~\textup{\ref{thm:pbw-allgenera-km}},
\textup{\ref{thm:pbw-allgenera-virasoro}},
and~\textup{\ref{thm:pbw-allgenera-principal-w}}\textup{)}:
interacting-family module Koszulness at generic level.
\emph{Proved:}
fusion-bar-cobar compatibility
(Theorem~\ref{thm:fusion-bar-cobar}).
\emph{Conjectural:}
full factorization-categorical derived equivalence
