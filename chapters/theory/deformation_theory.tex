...
genus~$1$ (Chapter~\ref{ch:heisenberg-frame};
Convention~\ref{conv:higher-genus-differentials}) makes the bar
complex a curved $A_\infty$-algebra rather than a chain complex.
The governing question is: how does this curved fiberwise anomaly feed
a strict deformation-obstruction theory and the Koszul-dual
complementarity package?  This chapter develops that mechanism.
...
Classically, $HH^2$ parametrizes deformations and $HH^3$ carries
obstructions.  For chiral algebras, $\overline{\mathcal{M}}_g$
contributes its own cohomology, splitting the theory into genus-$0$
(algebraic) and higher-genus (modular) components.
...
cyclic model, the geometric completed tensor / clutching package, and
the single surviving obstruction channel in the simple-Lie case.
Propositions~\ref{prop:tautological-line-support-criterion}
and~\ref{prop:one-channel-normalization-criterion} further reduce that
last task to a clutching/trace isolation statement for the
tautological line plus one normalized scalar comparison.
\end{remark}
...
This section separates three different claims.  The unconditional core
is the quantum periodicity input and the structural theorem that
independent periodicity sources combine by lcm.  Explicit modular
periods, sharp geometric periods, and higher-rank closed formulas are
conjectural outside the rank-$1$/minimal-model test cases.  The point is
to prevent the proved exchange theorem from being read as a global
...
The key is that the KL tensor equivalence (Step~2) transports the full
OPE data, not just fusion multiplicities, to quantum group data.
...
\item These morphisms are expressed through quantum Clebsch--Gordan
...
$[n + 2\ell]_q = [n]_q$, so the quantum CG coefficients, and hence the
OPE residues at every pole order, are periodic with the same period as
...
two factors: the OPE residues (periodic by~(iii)) and the
Orlik--Solomon forms $\eta_{ij}$ (independent of conformal weight).

Therefore the bar differential matrices at conformal weight~$h$ and
weight $h + M$ are identical.  Since each block is finite-dimensional
(Step~1), the homology inherits the same periodicity:
...
\begin{evidence}
The lcm structure is unconditional; the conjectural content
is the specific values of $N_{\mathrm{mod}}$ and $N_{\mathrm{geom}}$.
...
$(d_{\mathrm{mod}}, d_{\mathrm{quant}}, d_{\mathrm{geom}})$, where the
three indices record respectively the representation-theoretic weight,
the fusion-theoretic degree, and the configuration-space cohomological
degree.

\emph{Step~1: Individual divisibility.}
Each periodicity source independently constrains the total period:
\begin{enumerate}
\item \emph{Modular.}  The $T$-matrix satisfies
  $T^{N_{\mathrm{mod}}} = \mathrm{Id}$ on the character space, because
  $e^{2\pi i N_{\mathrm{mod}} c/24} = 1$ for
  $N_{\mathrm{mod}} = 24q/\gcd(p,24)$ and
  $cN_{\mathrm{mod}}/24 \in \mathbb{Z}$.  Thus the
  representation-theoretic grading is $N_{\mathrm{mod}}$-periodic.
\item \emph{Quantum.}  The quantum-integer periodicity
  $[n + 2\ell]_q = [n]_q$ forces the fusion multiplicities and OPE
  residues to be $N_{\mathrm{quant}}$-periodic in the
  fusion-theoretic grading.
\item \emph{Geometric.}  The configuration-space factor
  $H^q(\overline{C}_{p+2}(X))$ is controlled by the Hodge bundle
  $\mathbb{E} \to \overline{\mathcal{M}}_g$.  For $g \geq 2$, the
  relation $12\lambda = \kappa_1 + \delta$ and the bound
  $\dim_{\mathbb{C}} \overline{\mathcal{M}}_g = 3g-3$ force
  $\lambda^g = 0$.  The Kodaira--Spencer map sends $\lambda$ to a class
  in $CH^*(\mathcal{A})$, so its order divides $12(2g-2)$ by the same
  Mumford relation together with $\deg(\omega_X) = 2g-2$.
\end{enumerate}
...
\end{itemize}
These grading directions are algebraically independent: the
$T$-eigenvalues depend on conformal weights and central charge, the
quantum dimensions depend on the parameter
$q = e^{\pi i/(k+h^\vee)}$ and the root system, and the geometric
classes depend on $g$ and the tautological ring
$R^*(\overline{\mathcal{M}}_g)$.

\emph{Step~3: The lcm bound.}
The total period is the order of the periodicity in the combined
tridegree.  Since the operators
$\sigma_{\mathrm{mod}}$, $\sigma_{\mathrm{quant}}$, and
$\sigma_{\mathrm{geom}}$ act on independent grading components, the
combined operator is
$\sigma = \sigma_{\mathrm{mod}} \otimes
\sigma_{\mathrm{quant}} \otimes \sigma_{\mathrm{geom}}$, and its order
is the least common multiple of the three individual orders.  Hence
\[
...
\item \emph{Structural stratum.}
  ...
  (Theorem~\ref{thm:periodicity-exchange-koszul}).  Quantum
