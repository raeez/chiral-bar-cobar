...
\[
E_2^{p,q} = H^q(\overline{C}_{p+2}(X)) \otimes H^0(\mathcal{A}^{\otimes(p+2)})
\]
\begin{itemize}
...
Let $(\cA, \cA^!)$ be a Koszul dual pair of chiral algebras on a genus~$g$ curve~$X$. If $\cA$ has period:
\[
N_{\cA} \mid \mathrm{lcm}(N_{\text{mod}},\; N_{\text{quant}},\; N_{\text{geom}})
\]
Then $\cA^!$ has period:
\[
N_{\cA^!} \mid \mathrm{lcm}(N'_{\text{mod}},\; N_{\text{quant}},\; N_{\text{geom}})
\]
...
\emph{Quantum periodicity is preserved.}  The quantum period $N_{\text{quant}}$ depends only on the quantum group $U_q(\mathfrak{g})$ at $q = \exp(2\pi i/(h^\vee + k))$. Since Koszul duality sends level $k$ to $k' = -k - 2h^\vee$ (Feigin--Frenkel), the quantum parameter transforms as $q \mapsto q^{-1}$. Since $[n]_{q^{-1}} = [n]_q$ for all $n$, the periodicity of quantum integers is unchanged.

\emph{Geometric periodicity is unchanged.}  Both $\cA$ and $\cA^!$ live on the same curve $X$, so the contribution from $\mathrm{Jac}(X)$, $\mathrm{Pic}^0(X)$, and $K_X$ is identical.

\emph{Modular periodicity exchanges.}  By the complementarity theorem (Theorem~\ref{thm:kodaira-spencer-chiral-complete} and Corollary~\ref{cor:quantum-dual-complete}), the quantum correction spaces satisfy $\mathcal{Q}_g(\cA) \cong \mathcal{Q}_g(\cA^!)^\vee$. The modular period $N_{\text{mod}}$ is determined by the $T$-action on the space of characters, controlled by $e^{2\pi i c/24}$.  For $\cA$ with $c = p/q$, the dual central charge $c'$ is determined by the level duality $k \mapsto -k - 2h^\vee$ via the Sugawara formula (not by the bosonic string formula $c' = 26 - c$, which applies only in the BRST context).

One can verify the harmonic relation $\frac{1}{N_{\text{mod}}} + \frac{1}{N'_{\text{mod}}} = \frac{1}{12}$ in specific cases (e.g., the Ising model $c = 1/2$, the tricritical Ising model $c = 7/10$), but the general formula for $N'_{\text{mod}}$ in terms of $c'$ and the proof of the harmonic relation remain open.
\end{proof}
