...
$\pi(\psi^*)(z) = \int_{\mathbb{C}} \psi(w) \cdot \frac{dw}{z-w}$,
...
\item \emph{Non-separating boundary differential} $d_{\mathrm{ns}}$.
  For each vertex~$v$ of~$\Gamma$ with $g_v \geq 0$ and
  $\operatorname{val}(v) \geq 2$, the self-gluing map
  $\xi_{v}^{\mathrm{ns}}\colon
  \overline{\mathcal{M}}_{g_v, \operatorname{val}(v)+2}
  \to \overline{\mathcal{M}}_{g_v+1, \operatorname{val}(v)}$
  produces a new graph~$\Gamma_v^+$ with a loop added at
  vertex~$v$ (increasing its genus label by~$1$):
  \begin{equation}\label{eq:d-nonsep}
  d_{\mathrm{ns}} = \sum_{v \in V(\Gamma)}
  \pm\, \xi_v^{\mathrm{ns},*} \circ \Delta_v,
  \end{equation}
  where $\Delta_v$ is the self-sewing coproduct that pairs two
  additional marked points at~$v$ using
  $\langle -, - \rangle_\cA$, and the sign is determined by the
  edge-orientation convention.
  Geometrically: $d_{\mathrm{ns}}$ creates handles (non-separating
  nodes) on the curve at vertex~$v$.
\end{enumerate}
\end{definition}
...
\emph{Diagonal terms.}
$d_{\mathrm{int}}^2$: on each vertex summand, $d_v^2 = \kappa(\cA)
\cdot \omega_{g_v} \cdot \operatorname{id}$.  Since the curvature
is central (a scalar coderivation), $d_v^2$ acts on the tensor
product $\bigotimes_v \cA(g_v, \operatorname{val}(v))$ as
$\sum_v \kappa \cdot \omega_{g_v} \cdot \operatorname{id}$.  But
this sum is the \emph{total} fiberwise curvature $\kappa \cdot
\omega_g$ on the stable-graph summand of total genus~$g$; it
is absorbed into the curved differential
(Convention~\ref{conv:higher-genus-differentials}), not a
failure of nilpotence.
...
($L = 0$ loops).  We now extend to loop-level graphs, obtaining the
full quantum $L_\infty$ algebra.
...
\item For a Koszul algebra, the Hilbert series of $\cA$ and
  $\cA^!$ satisfy the reciprocity
  $h_{\cA}(t) \cdot h_{\cA^!}(-t) = 1$
  \cite[Theorem~3.5.1]{LV12}.  Setting $t = 1$:
  $\operatorname{str}(\operatorname{id}_{\barB})
  = h_{\cA^!}(-1) = 1/h_{\cA}(1)$, and
  on the bar complex (which is a quotient of the cofree
  coalgebra on $\cA^!$) this evaluates to the Euler
  characteristic $\chi(\barB) = 0$ by the alternating-sum
  cancellation forced by the reciprocity.
  Hence $\Delta(m_0^{(g)}) = 0$.
...
\emph{Part~(ii).}
The genus-$g$ component of the MC equation is
$\dzero(\Theta^{(g)}) + \frac{1}{2}\sum_{h=1}^{g-1}
[\Theta^{(h)}, \Theta^{(g-h)}] = 0$,
which is precisely the integrability condition for $\Dg{g}$.
...
On a stable graph of total genus~$g$ with a single vertex
($g_v = g$, the generic case), this gives $\kappa(\cA) \cdot
\omega_g$.  The curvature form $\omega_g$ is the Arnold
defect~\eqref{eq:arnold-defect-hg} on the fixed curve~$\Sigma_g$.
\end{proof}
...
\emph{(L3)}\enspace
The Gauss--Manin connection on the family
$\barB^{(g)}_X(\cA) \to \overline{\mathcal{M}}_g$ is flat
(its curvature is the Kodaira--Spencer class, which acts
nilpotently on cohomology by
Theorem~\ref{thm:kodaira-spencer-chiral-complete}).
The monodromy representation records the action of
$\pi_1(\mathcal{M}_g) = \mathrm{MCG}(\Sigma_g)$ on the bar
cohomology fibers.
\end{proof}
...
\emph{Part~(iv).}
Direct from part~(iii) by taking the genus-completed product
\eqref{eq:completed-bar-total} and applying
Theorem~\ref{thm:total-differential-from-mc}(i) for the nilpotence
$D_{\mathrm{tot}}^2 = 0$.
\end{proof}
