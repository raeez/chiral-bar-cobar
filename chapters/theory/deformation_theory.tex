...
\begin{conjecture}[Modular periodicity of graded dimensions; \ClaimStatusConjectured]
...
The weight-graded pieces of the bar complex decompose: $\bar{B}^{(n)}_{[h]}(\mathcal{A})$ is the subspace of conformal weight~$h$ in bar degree~$n$.  Since $\mathcal{A}$ is rational, each $\bar{B}^{(n)}_{[h]}$ is finite-dimensional (the fusion rules are finite: $\dim \mathrm{Hom}(V_i \otimes V_j, V_k) < \infty$ by Zhu's theorem).  The $T$-eigenvalue condition $T^N = \mathrm{Id}$ gives $e^{2\pi i N(h_i - c/24)} = 1$ for each module~$V_i$, i.e., the conformal weights satisfy $N h_i \in \mathbb{Z} + Nc/24$.  This is a phase condition on the $T$-eigenvalues; it does NOT directly imply that the bar complex chain spaces at weight~$h$ and weight~$h+N$ are isomorphic, since the structure of these spaces depends on fusion coefficients and OPE data, not solely on conformal weights.

To deduce the stated periodicity would require showing that the bar complex in weight~$h$ depends only on the module decomposition data and fusion multiplicities, and that these are eventually $N$-periodic in the weight variable.  For minimal models this follows from the finite representation category (Verlinde formula gives periodic fusion multiplicities).  The general rational case is expected but the deduction from $T^N = \mathrm{Id}$ alone is insufficient.
\end{remark}
...
\begin{conjecture}[Quantum periodicity; \ClaimStatusConjectured]
...
\begin{remark}[Scope]
The logical gap is in Step~4: periodic fusion coefficients do not automatically yield periodic bar differentials.  The bar differential depends on the full OPE data (all pole orders), not only the fusion multiplicities; periodicity of the latter is necessary but not sufficient.  Proved modulo this step.
\end{remark}
...
\begin{conjecture}[Geometric periodicity; \ClaimStatusConjectured]
...
The correct Mumford relation on $\overline{\mathcal{M}}_g$ is
$12\lambda = \kappa_1 + \delta$,
where $\lambda = c_1(\mathbb{E})$, $\kappa_1$ is the first kappa class, and $\delta$ is the boundary divisor class.  On the interior $\mathcal{M}_g$ the boundary class $\delta$ vanishes, but $\kappa_1|_{\mathcal{M}_g} \neq 0$; consequently $c_1(\mathbb{E})|_{\mathcal{M}_g}$ does not vanish.  This means one cannot deduce the periodicity bound $12(2g-2)$ from vanishing of $c_1(\mathbb{E})$ on the interior alone.  A correct proof would require a more refined analysis using the full tautological ring $R^*(\overline{\mathcal{M}}_g)$, accounting for both $\kappa_1$ and $\delta$ contributions via the Kodaira--Spencer map.  The stated bound is supported by all computed examples (Virasoro period~2, affine KM period $2\mathsf{h}$) and by the dimension constraint $c_1(\mathbb{E})^{3g-3+1} = 0$.
\end{remark}

\begin{proof}[Proof sketch (partial)]
...
\emph{Step 1: Tautological constraints.}
The Hodge bundle $\mathbb{E} \to \overline{\mathcal{M}}_g$ satisfies Mumford's formula $12\lambda = \kappa_1 + \delta$ in the Chow ring $\mathrm{CH}^*(\overline{\mathcal{M}}_g)$.  On the compactification, $c_1(\mathbb{E})^{g}$ is the top Chern class of $\mathbb{E}$ (the $\lambda$-class $\lambda_g$), and $c_1(\mathbb{E})^{3g-3+1} = 0$ by the dimension bound $\dim_{\mathbb{C}} \overline{\mathcal{M}}_g = 3g-3$.

\emph{Step 2: Image in Hochschild cohomology.}
The Kodaira--Spencer map sends $c_1(\mathbb{E})$ to a class in $CH^2(\mathcal{A})$ whose cup powers control the genus-$g$ contributions to Hochschild cohomology.  The $\kappa_1$-class and boundary contributions interact through the degeneration formula: boundary contributions factor through lower-genus Hochschild groups, while $\kappa_1$ contributions are controlled by the tautological ring relations.

\emph{Step 3: Partial periodicity.}
The dimension bound gives $\kappa(c_1(\mathbb{E})^{3g-2}) = 0$, providing the weaker constraint $\text{Period}_{\text{geom}} \mid 3g-2$.  The sharper bound $12(2g-2)$ incorporating the Mumford relation requires a complete analysis of the $\kappa_1$-image under the Kodaira--Spencer map, which remains open.  At $g = 0$, $\overline{\mathcal{M}}_{0,n}$ has no Hodge bundle, so there is no geometric periodicity.  At $g = 1$, $c_1(\mathbb{E}) = \frac{1}{12}\delta$ in $\mathrm{CH}^1(\overline{\mathcal{M}}_{1,1})$, and the periodicity from $c_1(\mathbb{E})$ is absorbed into the modular periodicity.
\end{proof}
...
\begin{conjecture}[Complete periodicity classification; \ClaimStatusConjectured]
...
\item $\text{Period} \mid N_{\text{quantum}}$ if the quantum periodicity conjecture holds (see the scope remark following that theorem).
\item $\text{Period} \mid N_{\text{geometric}}$ by the geometric periodicity theorem above.
\end{itemize}

If the three periodicities act on the \emph{same} grading component, the three divisibility constraints combine to give $\text{Period} \mid \gcd(N_{\text{mod}}, N_{\text{quant}}, N_{\text{geom}})$.  In practice, however, the three periodicities act on different components of a multi-graded bar complex (representation-theoretic, fusion-theoretic, and geometric respectively), so the total period need not divide the gcd.  The stated $\text{lcm}$ upper bound is always valid: any common multiple of the three component periods is a period.  Whether the actual period equals the $\text{lcm}$, or is strictly smaller due to interactions between the three sources, depends on the specific algebra and is not determined by the abstract framework alone.
\end{remark}

\begin{remark}[Scope of the classification]\label{rem:periodicity-classification-scope}
The $\operatorname{lcm}$ upper bound is trivially valid (any common multiple of periods is a period), but the precise determination of $\operatorname{Period}(\mathcal{A})$ requires understanding how the three sources interact.  When all three periodicities act on the same grading (as they do for weight-graded Hochschild cohomology), the strongest conclusion from three divisibility constraints $\operatorname{Period} \mid N_i$ is $\operatorname{Period} \mid \gcd(N_1, N_2, N_3)$.  The gap between the $\gcd$ lower bound and the $\operatorname{lcm}$ upper bound measures the extent to which the multi-graded structure of the bar complex separates the three periodicity mechanisms.  Resolving this gap for specific algebras (e.g., minimal models, where all three periods are known) is an open problem.  Furthermore, the geometric periodicity component (Theorem above) is itself conjectural, as the proof requires a complete analysis of the Kodaira--Spencer image under the correct Mumford relation $12\lambda = \kappa_1 + \delta$.
\end{remark}

\subsection{Koszul duality and periodicity interaction}

\begin{conjecture}[Periodicity exchange under Koszul duality; \ClaimStatusConjectured]
\index{periodicity!exchange under duality}
\label{conj:periodicity-exchange-koszul}
