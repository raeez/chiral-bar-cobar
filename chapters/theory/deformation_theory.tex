...
The curvature $d^2 = \kappa \cdot \omega_1$ that appeared at genus~$1$ for the Heisenberg algebra (Chapter~\ref{ch:heisenberg-frame}) signals a deformation-theoretic phenomenon: the bar complex is not a chain complex but a curved $A_\infty$-algebra.  This chapter develops the deformation-obstruction theory that organizes these quantum corrections.

\section{The deformation problem for chiral algebras}

When does a chiral algebra admit deformations, and what obstructs
them?  In classical algebra the answer is clean: $HH^2$ parametrizes
infinitesimal deformations, $HH^3$ carries obstructions, and Koszul
duality interchanges the two.  For chiral algebras the question
acquires a geometric dimension: the moduli space
$\overline{\mathcal{M}}_g$ of the underlying curve contributes its
own cohomology, and the deformation-obstruction theory splits into a
genus-zero algebraic component and a higher-genus modular component.

The complementarity theorem
(Theorem~\ref{thm:quantum-complementarity-main}) gives the precise
answer: $Q_g(\cA)$ and $Q_g(\cA^!)$ are complementary Lagrangians
in a shifted-symplectic ambient space.  This chapter develops the
infinitesimal machinery underlying that result---chiral Hochschild
cohomology, the obstruction calculus, and the curved $A_\infty$
structures that emerge when obstructions are nonzero.
The deformation-obstruction exchange at genus~$0$
--- $HH^2(\cA)$ parametrizes deformations, $HH^3(\cA)$ carries
obstructions, and Koszul duality sends $HH^n(\cA)$ to
$HH^{2-n}(\cA^!)^\vee \otimes \omega_X$ --- is the seed from
which the higher-genus complementarity grows.  At a conceptual
level, the full structure is an instance of the noncommutative
Hodge-to-de Rham degeneration: the Hodge filtration on chiral
Hochschild homology degenerates at $E_1$, and the resulting mixed
structure controls the deformation theory across all genera
...
The free boson is the Heisenberg algebra of Chapter~\ref{ch:heisenberg-frame}, now viewed through the lens of Hochschild cohomology.  Its Hochschild complex $\mathrm{HH}^*(\mathcal{H}_k)$ is computed by the bar-cobar resolution; the boson-fermion Koszul pair reveals how deformations of $\mathcal{H}_k$ are dual to obstructions of the free fermion.
...
Theorem~\ref{thm:modular-periodicity-minimal} proves modular periodicity for
Virasoro minimal models $M(p,q)$, where the finite representation category
($(p-1)(q-1)/2$ simples), explicit Rocha-Caridi character formulas, and the
Verlinde fusion rules provide the necessary control.
Conjecture~\ref{conj:modular-periodicity} extends this to general rational
chiral algebras.  The main obstacle in the general case is that the
$T$-matrix periodicity $T^N = \mathrm{Id}$ on the character space is known
(it follows from the same argument as in Step~1 of the proof above), but
deducing that weight-space dimensions and OPE coefficients are eventually
$N$-periodic requires structural information about the characters that is
available for minimal models (theta functions over eta) but not in full
generality.  The conjecture is expected for all $C_2$-cofinite rational vertex
algebras, where Zhu's finiteness theorem and Huang's modular tensor category
theory apply.  WZW models at positive integer level, whose characters are
given by affine Weyl-Kac formulas (also ratios of theta functions over eta),
provide a natural next case amenable to the same proof strategy.
\end{remark}
...
On a genus $g > 0$ curve, new sources of periodicity arise.  The following theorem establishes a weak bound from the dimension of moduli space; a conjectural sharp bound follows.
...
\begin{theorem}[Complete periodicity classification; \ClaimStatusConjectured]
\label{thm:complete-periodicity-classification}
\index{periodicity!complete classification|textbf}
...
where:
\begin{align}
N_{\text{modular}} &= \frac{24q}{\gcd(p, 24)} \quad \text{(Theorem~\ref{thm:modular-periodicity-minimal} for minimal models; Conjecture~\ref{conj:modular-periodicity} in general)} \\
...
This theorem is tagged \ClaimStatusConjectured{} because two of its three
...
For the Heisenberg algebra, this was verified directly: $\Omega(\bar{B}(\mathcal{H}_k)) \xrightarrow{\sim} \mathcal{H}_k$ (\S\ref{sec:frame-inversion}).  The following general statement shows that the cobar-of-bar is always a free resolution.
