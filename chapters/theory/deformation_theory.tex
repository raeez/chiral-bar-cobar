...
deformation space $Q_g(\cA)$ and the obstruction space
$Q_g(\cA^!)$ as complementary Lagrangians in a shifted-symplectic
ambient space.  This chapter develops the infinitesimal theory
underlying that result: chiral Hochschild cohomology, the
obstruction calculus for chiral algebras, and the curved
$A_\infty$ structures that emerge when obstructions are nonzero.
...
which the higher-genus complementarity grows.
...
Since $\dim_{\mathbb{C}} \overline{C}_{n+2}(X) = n+2$ varies with bar degree~$n$, the Verdier shift on bar degree~$n$ is $n+2$.  Nevertheless, the \emph{totalized} shift is constant.  The mechanism is as follows: the Koszul property ensures diagonal concentration, meaning the bar complex contributes to total Hochschild degree~$d$ only through bar degree~$n = d$.  In bar degree~$d$, the complex lives on $\overline{C}_{d+2}(X)$, and Verdier duality gives:
\[
H^d(\overline{C}_{d+2}(X), \mathcal{M}) \;\cong\; H^{(d+2)-d}(\overline{C}_{d+2}(X), \mathbb{D}\mathcal{M})^\vee \;=\; H^2(\overline{C}_{d+2}(X), \mathbb{D}\mathcal{M})^\vee.
\]
Thus in \emph{every} bar degree, the Verdier shift lands at cohomological degree~$2$ on the configuration space.  The desuspension $s^{-1}$ in the bar construction contributes an additional shift of~$1$, and the Serre duality of the underlying curve~$X$ (one-dimensional) contributes a final shift of~$1$, giving the uniform total shift of~$2$.  The net effect on the total complex is:
\[
HH^n_{\mathrm{chiral}}(\mathcal{A})
...
\begin{theorem}[Modular periodicity of graded dimensions; \ClaimStatusConjectured]
...
\begin{theorem}[Geometric periodicity; \ClaimStatusConjectured]
...
\begin{theorem}[Periodicity exchange under Koszul duality; \ClaimStatusConjectured]
