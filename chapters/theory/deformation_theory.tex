...
The Koszul dual $\mathcal{A}^!$ is the coalgebra produced by the bar
construction: generators are
$\mathcal{V}^* = \mathcal{H}om_{\mathcal{O}_X}(\mathcal{V}, \omega_X)$,
and relations $R^{\perp}$ are orthogonal to $R$ under the residue
pairing.  What is a relation in $\mathcal{A}$ becomes freedom in
$\mathcal{A}^!$.  Saying
$\bar{B}^{\text{ch}}(\mathcal{A}) \simeq \mathcal{A}^!$ verifies that
the bar construction produces the Koszul dual coalgebra.
\end{remark}
...
The twisting morphism $\tau\colon \mathcal{A}^! \to \mathcal{A}$
implements the bar-cobar isomorphism: the MC equation
$\partial \tau + \tau \star \tau = 0$ intertwines coalgebra and
algebra differentials, giving
$\mathcal{A}^!_\tau \simeq \bar{B}^{\text{ch}}(\mathcal{A})$ and
$\mathcal{A}_\tau \simeq \Omega^{\text{ch}}(\mathcal{A}^!)$.
Geometrically,
$\tau(c) = \int_{C_2(X)} \mathrm{ev}^* c \wedge K_{\text{twist}}$
for a universal kernel $K_{\text{twist}}$ on the configuration space.
\end{remark}
...
\begin{remark}[Scope]
The formula~\eqref{eq:period-sum} assumes modular periods are
$24q/\gcd(p,24)$, which depends on
Conjecture~\ref{conj:modular-periodicity}.  The principal obstruction
is that Koszul duality need not preserve rationality, so $N'$ may be
infinite for arbitrary duals.
(Part of the MC2 theorem package; Theorem~\ref{thm:mc2-full-resolution}.)
\end{remark}
...
$\sigma_{\mathrm{geom}}$ is nilpotent ($\sigma^N = 0$), not periodic
($\sigma^N = \mathrm{id}$): the geometric contribution provides a
stabilization threshold, not a periodic automorphism
...
theory~\cite{Kal08}.  Theorem~C (complementarity) is Serre duality in
