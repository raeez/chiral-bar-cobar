...
$T$-matrix periodicity ($T^N = \mathrm{Id}$, Step~1) is proved for minimal models and WZW; the structural framework (Step~3) is sound.  The gap is Step~2: convolution with $p(\cdot) = 1/\eta$ does not preserve eventual periodicity.  The conjecture is posed for all $C_2$-cofinite rational vertex algebras.
\end{remark}

\subsubsection{Examples}

\begin{example}[Period examples]
Ising $(3,4)$: $c=1/2$, $N \mid 48$; Tricritical Ising $(4,5)$: $c=7/10$, $N \mid 240$; Three-state Potts $(5,6)$: $c=4/5$, $N \mid 30$.  For $\widehat{\mathfrak{sl}}_2$: $k=1$ gives $c=1$, $N=24$; $k=2$ gives $c=3/2$, $N=48$.
\end{example}
