...
Exactly marginal CFT deformations correspond to closed Hochschild $2$-cocycles (Seiberg~\cite{Sei88}); the algebraic framework is Gerstenhaber's deformation theory~\cite{Ger63}, extended to chiral algebras via Beilinson--Drinfeld's factorization formalism~\cite{BD04}.
...
Since $\dim_{\mathbb{C}} \overline{C}_{n+2}(X) = n+2$ varies with bar degree~$n$, the Verdier shift on bar degree~$n$ is $n+2$.  Nevertheless, the \emph{totalized} shift is constant.  The two sources of the shift by~$2$ are as follows.  First, the desuspension $s^{-1}$ in the bar construction contributes a shift of~$1$: the bar complex $\bar{B}^{\mathrm{ch}}(\cA) = T^c(s^{-1}\bar{\cA})$ has internal grading shifted by one relative to the Hochschild grading.  Second, Serre duality on the underlying curve~$X$ ($\dim_{\mathbb{C}} X = 1$) contributes a shift of~$1$ via the canonical bundle~$\omega_X$.  The varying Verdier shift $(n+2)$ on the configuration spaces does \emph{not} contribute an additional shift because it is absorbed by the Koszul concentration: the Koszul property ensures that bar degree~$n$ contributes only to Hochschild degree~$n$, and the $\mathcal{D}$-module Verdier duality on $\overline{C}_{n+2}(X)$, combined with the Koszul pairing between~$\cA$ and~$\cA^!$, exchanges the Hom sheaf with the dual Hom sheaf while the $(n+2)$-dependent shift cancels against the identification of bar complexes on matching configuration spaces.  The net effect on the total complex is:
\[
HH^n_{\mathrm{chiral}}(\mathcal{A})
...
\begin{theorem}[Modular periodicity for minimal models; \ClaimStatusProvedHere]
...
\begin{proof}
The proof proceeds in three steps: we establish $T$-matrix periodicity, deduce
...
Theorems~\ref{thm:modular-periodicity-minimal}
and~\ref{thm:modular-periodicity-wzw} prove modular periodicity for
the two principal families of rational chiral algebras: Virasoro
minimal models $M(p,q)$ (via Rocha-Caridi characters) and WZW models
$\widehat{\mathfrak{g}}_k$ at positive integer level (via the
Weyl--Kac character formula).  In both cases the key structural
ingredient is that the characters are ratios of theta functions over
eta functions, giving exact periodicity of Fourier coefficients.
Conjecture~\ref{conj:modular-periodicity} extends this to general
rational chiral algebras.  The main obstacle in the general case is
that the $T$-matrix periodicity $T^N = \mathrm{Id}$ on the character
space is known (it follows from the same argument as in Step~1 of the
proof above), but deducing that weight-space dimensions and OPE
coefficients are eventually $N$-periodic requires structural
information about the characters that may not be available in full
generality.  The conjecture is expected for all $C_2$-cofinite
...
shift.  For minimal models, Theorem~\ref{thm:modular-periodicity-minimal}
...
By Theorem~\ref{thm:modular-periodicity-minimal}, the Virasoro minimal models with
\[
c = 1 - \frac{6(p-q)^2}{pq}
\]
...
\begin{theorem}[Modular periodicity for WZW models; \ClaimStatusProvedHere]
...
\begin{proof}
The proof follows the same three-step strategy as
Theorem~\ref{thm:modular-periodicity-minimal}, using the
...
Theorem~\ref{thm:modular-periodicity-minimal}: since
...
argument of Theorem~\ref{thm:modular-periodicity-minimal},
Step~2, this gives
...
in Step~3 of Theorem~\ref{thm:modular-periodicity-minimal}, with
...
Theorem~\ref{thm:modular-periodicity-wzw} applies to all
simple~$\mathfrak{g}$ at all positive integer levels.  Together with
Theorem~\ref{thm:modular-periodicity-minimal}, modular periodicity
is now established for the two main families of rational chiral
algebras: Virasoro minimal models and WZW models.
...
this is established by Theorem~\ref{thm:modular-periodicity-minimal};
...
where:
\begin{align}
N_{\text{modular}} &= \frac{24q}{\gcd(p, 24)} \quad \text{(Theorems~\ref{thm:modular-periodicity-minimal}, \ref{thm:modular-periodicity-wzw} for minimal/WZW; Conjecture~\ref{conj:modular-periodicity} in general)} \\
...
\item \emph{Unconditional stratum.}
  ...
  (modular by Theorem~\ref{thm:modular-periodicity-minimal},
...
The dual central charge $c'$ is determined by the level duality $k \mapsto k' = -k - 2h^\vee$ via the Sugawara formula.  By the $T$-matrix analysis of Conjecture~\ref{conj:modular-periodicity} (applied to~$\cA^!$), the modular period is $N'_{\text{mod}} = 24q'/\gcd(p',24)$ where $c' = p'/q'$.  Since each periodicity source independently divides the total period, the lcm upper bound follows.
\end{proof}
