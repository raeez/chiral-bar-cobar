...
\[
  ...
  = \operatorname{Res}_{z_2\!=z_3}\!\mu_\epsilon(\mu_\epsilon(a(z_1),b(z_2)),c(z_3)).
\]
...
\]
This does \emph{not} by itself imply periodicity of every OPE coefficient or of the full bar complex: the proposition records only the Type~II quantum input to the periodicity profile $(N_{\mathrm{mod}}, N_{\mathrm{quant}}, N_{\mathrm{geom}})$.
\end{proposition}
...
\begin{evidence}
The unconditional content here is structural rather than numerical.
What is proved is the Type~II quantum input and the formal fact that
commuting periodicity operators on independent grading directions
combine by~$\operatorname{lcm}$.  What remains conjectural in the
displayed formula is the sharp Type~I modular input and the sharp
Type~III geometric input.
...
\emph{Step~1: Inputs at the three strata.}
\begin{enumerate}
...
\item \emph{Quantum.}  The quantum-integer periodicity
...
\item \emph{Geometric (Type~III).}  Theorem~\ref{thm:geometric-periodicity-weak}
  proves only the weak geometric bound
  $\mathrm{Period}_{\mathrm{geom}} \mid (3g-2)$.
  Replacing that weak factor by the sharp quantity
  $12(2g-2)$ is exactly the additional conjectural input of
  Conjecture~\ref{conj:geometric-periodicity}.
\end{enumerate}

\emph{Step~2: Structural $\operatorname{lcm}$ mechanism.}
Suppose one has commuting operators
$\sigma_{\mathrm{mod}}$, $\sigma_{\mathrm{quant}}$,
$\sigma_{\mathrm{geom}}$ acting on the three tridegree directions with
orders dividing the available Type~I/II/III inputs.  Then the combined
operator
\[
\sigma = \sigma_{\mathrm{mod}} \otimes
\sigma_{\mathrm{quant}} \otimes \sigma_{\mathrm{geom}}
\]
acts on the total tridegree, and its order divides the least common
multiple of the three individual orders.  This is the only
unconditional structural statement being used here.

The bookkeeping separation is:
\begin{itemize}
\item the modular input acts on the representation-theoretic component
  via $T$-eigenvalues / conformal weights;
\item the quantum input acts on the fusion-theoretic component via
  quantum dimensions and Clebsch--Gordan data;
\item the geometric input acts on the configuration-space component via
  tautological classes on $\overline{\mathcal{M}}_g$.
\end{itemize}

\emph{Step~3: The conjectural numerical package.}
If the Type~I modular bar-periodicity conjecture supplies the sharp
factor $N_{\mathrm{mod}} = 24q/\gcd(p,24)$ and the Type~III geometric
conjecture supplies the sharp factor $N_{\mathrm{geom}} = 12(2g-2)$,
then the structural $\operatorname{lcm}$ mechanism of Step~2 yields the
displayed conjectural bound
\[
\mathrm{Period}(\mathcal{A}) \mid
\mathrm{lcm}(N_{\mathrm{mod}}, N_{\mathrm{quant}}, N_{\mathrm{geom}}).
\]
If one weakens the Type~I or Type~III inputs, the same argument still
produces the corresponding weaker profile-level upper bound.
\end{evidence}
...
$N_{\text{geom}}$ that are conjectural.  More precisely, only the
$T$-matrix phase order is currently unconditional on the modular side,
and Theorem~\ref{thm:geometric-periodicity-weak} supplies only the weak
geometric bound $\mathrm{Period}_{\mathrm{geom}} \mid (3g-2)$.
...
In the profile formulation, the three periodicity sources
decompose further:
\begin{enumerate}[label=\textup{(\roman*)}]
...
What survives is a \emph{stratified} picture with four levels of
decreasing strength:
\begin{enumerate}[label=\textup{(\alph*)}]
\item \emph{Structural stratum.}
  The lcm mechanism is proved unconditionally
  (Proposition~\ref{thm:periodicity-exchange-koszul}).  Quantum
  periodicity is explicit, and the geometric theory contributes at
  least the weak proved bound of
  Theorem~\ref{thm:geometric-periodicity-weak}; what remains open are
  the sharp formulas for $N_{\mathrm{mod}}$ and the sharp geometric
  term $N_{\mathrm{geom}}$.
...
\item \emph{Outer rational frontier.}
  For general rational chiral algebras, both the modular periodicity
  bound and the sharp geometric periodicity bound remain conjectural
  (Conjectures~\ref{conj:modular-periodicity}
  and~\ref{conj:geometric-periodicity}).  Natural next cases include
  $\mathcal{W}$-algebra minimal models, orbifolds, and cosets.
...
\begin{enumerate}
\item The geometric periodicity component is unchanged:
  $N_{\text{geom}}(\cA) = N_{\text{geom}}(\cA^!)$.
...
$(N_{\text{mod}}, N_{\text{quant}}, N_{\text{geom}})$ component-wise; it
does not assert equality of scalar periods, since the modular component
changes with the dual central charge.  Whenever all three profile
components are defined, the total period of $\cA^!$ divides
$\mathrm{lcm}(N'_{\text{mod}}, N_{\text{quant}}, N_{\text{geom}})$.
\end{proposition}

\begin{proof}
\emph{(1) Geometric periodicity is unchanged.}
...
unconditionally that the geometric component is preserved and that the
dual modular phase bound is determined by the dual central charge.  In
the admissible-level KL/DS regime it also preserves the Type~II quantum
input.  The sum of reciprocal modular periods is given by
