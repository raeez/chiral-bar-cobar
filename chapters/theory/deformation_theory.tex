...
\begin{proposition}[Genus-$0$ cyclic coderivation complex;
...
\begin{proposition}[Killing cocycle $L_\infty$ extension;
...
\begin{corollary}[Minimal cyclic $L_\infty$ model for Kac--Moody;
...
For $n = 4$: the arity-$4$ transferred bracket on
$\mathfrak{g}^{\otimes 4}$ lands in $H^2$; the arity-$4$
homotopy Jacobi identity forces $l_4^{\mathrm{tr}} = 0$
(equivalently: $\delta\phi = 0$, proved in
Proposition~\ref{prop:killing-linf-extension}(i)).
For $n \geq 5$: every surviving term in the transfer formula
involves composing $l_3^{\mathrm{tr}}$ with an $\eta$-input, which
vanishes since $l_3^{\mathrm{tr}}$ is defined only on
$\mathfrak{g}^{\otimes 3}$.
\end{proof}
...
at generic (non-critical) levels.  The remaining two MC2
hypotheses---the geometric completed tensor / clutching package
(Hypothesis~\ref{mc2-hyp:completion}) and the tautological-line
support (Hypothesis~\ref{mc2-hyp:tautological})---remain open.
...
\begin{theorem}[Cyclic $L_\infty$ structure via chiral graph
...
The conjecture (Conjecture~\ref{thm:modular-periodicity-minimal})
...
\label{thm:periodicity-quantum-input}
...
N_{\text{modular}} &= \frac{24q}{\gcd(p, 24)} \quad
  \text{(Conjectures~\ref{thm:modular-periodicity-minimal},
  \ref{thm:modular-periodicity-wzw} for minimal/WZW;
...
\item \emph{Modular (Type~I).}  For the rational families treated
  above, Step~1 of
  Conjectures~\ref{thm:modular-periodicity-minimal}
  and~\ref{thm:modular-periodicity-wzw} proves only the
...
\item \emph{Quantum (Type~II).}  The quantum-integer periodicity
  ...
  KL/DS regime of Proposition~\ref{thm:periodicity-quantum-input}, so
...
\item \emph{Structural stratum.}
  ...
  unconditionally
  (Proposition~\ref{thm:periodicity-exchange-koszul}).  Quantum
...
\item \emph{Principal rational frontier.}
  ...
  (Conjectures~\ref{thm:modular-periodicity-minimal}
  and~\ref{thm:modular-periodicity-wzw}) because the missing
...
Proposition~\ref{thm:periodicity-exchange-koszul} establishes
