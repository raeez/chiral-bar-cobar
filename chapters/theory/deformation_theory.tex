...
The governing question of this chapter is which deformation complexes detect first-order variation of a chiral algebra, and how Koszul duality reorganizes that control data.
\end{remark}

\begin{remark}[H/M/S route; Convention~\ref{conv:hms-levels}]
H/M-level; S-level consequences promoted only where theorem labels do so.
\end{remark}
...
\begin{theorem}[Existence and uniqueness {\cite{LV12}}; \ClaimStatusProvedElsewhere]\label{thm:universal-twisting-existence}
...
The cyclic deformation complex controls tree-level (genus-$0$)
data.  Three levels of homotopy-algebraic structure arise as the
genus filtration is opened:
\begin{enumerate}[label=\textup{(\roman*)}]
\item \emph{Cyclic $L_\infty$ algebra} (tree level, $g = 0$):
  operations $\ell_n$ indexed by rooted trees,
  satisfying cyclic symmetry.  This is the classical operadic
  homotopy theory of $\operatorname{Def}_{\mathrm{cyc}}(\cA)$.
\item \emph{Quantum $L_\infty$ algebra}
  (all genera, $g \geq 0$; Braun--Lazarev~\cite{BL15}):
  operations $m^{(g)}_n$ indexed by stable graphs of
  genus~$g$ and valence~$n$, satisfying the \emph{quantum master
  equation}
  $\sum_{g \geq 0} \hbar^g\, m^{(g)} = 0$.
  This is the loop homotopy algebra of Markl~\cite{Markl06}.
\item \emph{Modular homotopy type}
  (globalized on curves):
  the quantum $L_\infty$ data coupled to
  factorization on $\operatorname{Ran}(X)$ and
  functorial over $\{\overline{\mathcal{M}}_{g,n}\}$
  (Definition~\ref{def:modular-homotopy-theory-intro}).
\end{enumerate}
Lifting from level~(i) to level~(ii) is \emph{obstructed}: the
first obstruction is $\Delta(m_0)$, the BV Laplacian applied to the
genus-$0$ structure, and the lift exists if and only if the algebra
is \emph{unimodular} (the BV operator is exact on $m_0$).  Our
Koszul chiral algebras on the standard landscape
\emph{do} carry full modular homotopy types: this is the content
of the universal Maurer--Cartan class
$\Theta_\cA$
(Theorem~\ref{thm:universal-MC}).  That such types exist is
special, not automatic.
\end{remark}
...
This theorem resolves Hypothesis~\ref{mc2-hyp:cyclic}
for the Kac--Moody family $\widehat{\mathfrak{g}}_k$
at generic (non-critical) levels.  All three MC2
hypotheses are now resolved by
Theorem~\ref{thm:mc2-full-resolution}.
The cyclic dg~Lie algebra structure is stronger than the
$L_\infty$ hypothesis requires: the full coderivation complex
carries a strict bracket (not just homotopy-coherent operations).
The minimal model
(Corollary~\ref{cor:km-minimal-linf}) shows that the
transferred $L_\infty$ structure on~$\mathfrak{g}$
is the Killing extension, which is exactly the finite-dimensional
object verified computationally in
Remark~\ref{rem:mc2-status}, items~(v)--(ix).
\end{remark}
...
The full construction of $\Theta_{\cA}$ is now established
(Theorem~\ref{thm:explicit-theta}): concretely,
$\Theta_{\cA}^{\min} = \kappa(\cA) \cdot \eta \otimes \Lambda$ in
the minimal model, and the chain-level representative
$\theta_g = \kappa \cdot \mu \otimes \lambda_g$ (modulo exact
corrections) recovers exactly the deformation slot identified by the
concrete models above.
The formal genus-completed framework
(Proposition~\ref{prop:genus-completed-mc-framework}) establishes that
the genus-convolution MC equation converges and has genus-stratified
obstructions, and Proposition~\ref{prop:mc2-reduction-principle}
packages the remaining MC2 frontier into three tasks: the intrinsic
cyclic model, the geometric completed tensor / clutching package
(now resolved by Proposition~\ref{prop:geometric-modular-operadic-mc}),
and the single surviving obstruction channel in the simple-Lie case.
...
\ref{prop:one-channel-visible-lowarity-packet-criterion},
\ref{prop:one-channel-canonical-transfer-criterion},
\ref{prop:one-channel-transfer-law-criterion}, and
\ref{prop:one-channel-root-string-chart-criterion}, and
\ref{prop:one-channel-intrinsic-line-detection-criterion}, and
\ref{prop:one-channel-automorphism-rigidity-criterion}, and
\ref{prop:one-channel-support-graph-stabilizer-criterion}, and
\ref{prop:one-channel-incidence-orbit-criterion}, and
\ref{prop:one-channel-visible-orbit-table-criterion}, and
\ref{prop:one-channel-canonical-universal-orbit-table-criterion}, and
\ref{prop:one-channel-universal-invariant-signature-criterion}, and
\ref{prop:one-channel-signed-seed-character-criterion}, and
\ref{prop:one-channel-two-sign-plus-normalization-scalar-criterion}, and
\ref{prop:one-channel-parity-sign-plus-normalization-scalar-criterion}, and
\ref{prop:one-channel-parity-forcing-criterion}, and
\ref{prop:one-channel-normalization-criterion} further reduce that last
task to a clutching/trace isolation statement for the tautological
line, a one-channel Verdier/Lagrangian-plane criterion, a one-channel
PTVV / anti-involution lift criterion, an explicit chain-model
construction on the one-channel sector of~$\Defcyc(\cA)$, a finite
low-bar-length bar-coderivation seed construction inside
$\operatorname{CoDer}^{\mathrm{cyc}}(\widehat{\barB}_X(\cA))[1]$, a
...
those seed spaces with obstruction-side recovery of the seed
coefficients, a root-string chart forced on those seed spaces up to
seed-line rescaling, intrinsic detection of the one-channel support
lines, automorphism-rigidity of the one-channel support graph, a finite
...
in~\cite{Kontsevich03} and~\cite{costello-renormalization}.
...
(c)\enspace
Cyclic symmetry follows from the ribbon structure.
For each tree $\Gamma \in \mathfrak{G}_{n,0}$, the ribbon structure
induces a cyclic ordering of the external legs, and the amplitude
$w_\Gamma$ is cyclically symmetric up to the Koszul sign, because
each internal contraction uses the $\mathrm{ad}$-invariant form
$\langle{-},{-}\rangle_{\cA}$.  The cyclic sum over ribbon
orderings reproduces the signed cyclic
identity~\eqref{eq:cyclic-symmetry-graph}.
\end{proof}
...
\item \emph{Classical limit.}
  When $X = \mathbb{A}^1$ and $\cA$ is the chiral
  envelope of an ordinary associative algebra~$A$, the Szeg\H{o}
  kernel reduces to the Cauchy kernel $dz/(z-w)$, the FM
  compactification $\ConfigSpace{n}$ is the Stasheff
  associahedron, and the graph sum~\eqref{eq:linf-graph-sum}
  reduces to the standard Kontsevich--Soibelman $L_\infty$
  structure on
  $\operatorname{CoDer}(BA)[1]$~\cite{KontsevichSoibelman}.
\item \emph{Costello--Gwilliam framework.}
  In the language of factorization
  algebras~\cite{CG17,CG-vol2}, the graph sum is the
  perturbative quantization of the holomorphic
  Chern--Simons-type theory on~$X$ whose classical moduli problem is
  $\operatorname{MC}(\Defcyc(\cA))$.  The propagator is the chiral
  Green's function, the vertices are the chiral operations, and the
  cyclic pairing is the BV anti-bracket.
  Hypothesis~\ref{hyp:stokes-FM} is the ultraviolet regularity
  statement provided by the FM~compactification
  (cf.~\cite{costello-renormalization}).
\item \emph{Modular-operadic structure.}
  The full graph complex $\bigoplus_L \mathfrak{G}_{n,L}$ has a
  natural modular-operadic structure in the sense of
  Getzler--Kapranov~\cite{GeK98}: composition maps are given by
  gluing graphs along external legs.  The tree-level ($L = 0$)
  part defines the $L_\infty$ brackets; the positive-loop ($L > 0$)
  part encodes quantum corrections and is the algebraic origin of
  the genus tower.  This modular-operadic enhancement is the
  combinatorial backbone of hypothesis~MC2-2 (the geometric
  completed tensor / clutching package of
  Remark~\ref{rem:mc2-status}, Step~2).
\item \emph{Hypothesis status.}
  Both hypotheses are now unconditional.
  Hypothesis~\ref{hyp:stokes-FM} is proved as
  Proposition~\ref{prop:stokes-regularity-FM}, building on the
  classical results of~\cite{Kontsevich03}, \cite{AS94},
  and~\cite{costello-renormalization}; the extension from
  $\mathbb{R}^n$ to chiral algebras on curves uses the same FM
  compactification technology, with the Szeg\H{o} kernel in place
  of the Cauchy kernel.
  Hypothesis~\ref{hyp:fay-propagator} is the standard Fay
  trisecant identity for the Szeg\H{o} kernel on compact Riemann
  surfaces (Proposition~\ref{prop:fay-trisecant}, \cite{Fay73}).
  Theorem~\ref{thm:cyclic-linf-graph} is therefore fully proved
  at all genera.
\end{enumerate}
\end{remark}

\begin{remark}[Physical interpretation]\label{rem:graph-complex-physics}
The graph-sum formula~\eqref{eq:linf-graph-sum} has a direct Feynman
diagram interpretation.  Each chiral graph $\Gamma$ is a tree-level
Feynman diagram for the ``chiral field theory'' on~$X$:
\begin{itemize}
\item External legs = asymptotic states (the deformation inputs
  $f_i$);
\item Internal edges = propagators (the Szeg\H{o} kernel $S(z,w)$,
  playing the role of the Feynman propagator);
\item Internal vertices = interaction vertices (the $A_\infty$
  operations~$m_k$, encoding the OPE data of~$\cA$);
\item Graph amplitude = tree-level scattering amplitude.
\end{itemize}
The homotopy Jacobi identity~\eqref{eq:homotopy-jacobi-graph} is
the Ward identity for this theory: it expresses the vanishing of the
BRST variation of correlation functions, mediated by Stokes' theorem
on the compactified configuration space.

The passage from tree-level ($L = 0$, the $L_\infty$ structure) to
loop-level ($L > 0$, quantum corrections) parallels the standard QFT
perturbation series: the genus-$g$ obstruction $\mathrm{obs}_g(\cA)$
arises from $L = g$ loop diagrams, with the Szeg\H{o} kernel
providing the loop integrand and $\ConfigSpace{k}$ providing the
ultraviolet regulation.  The cyclic pairing ensures the BV quantum
master equation is satisfied order by order in the loop
expansion~\cite{costello-renormalization}.
\end{remark}
...
\index{$\Defmod$|see{modular deformation complex}}
...
complex.  The new feature at $g \geq 1$: the coefficients are
chains on $\overline{\mathcal{M}}_{g,n}$, so stable-graph geometry
enters the governing dg Lie algebra itself.  The genus filtration on
$\operatorname{Def}^{\mathrm{mod}}(\cA)$ is the direct
higher-genus upgrade of ``homotopy algebraic structures $=$
MC elements in the convolution dg Lie algebra.''
\end{remark}
...
The currently accessible families---Kac--Moody at generic level,
Virasoro, principal $\mathcal{W}_N$---all have
$\dim H^2_{\mathrm{cyc}}(\cA,\cA) = 1$ and are scalar-saturated
(Corollary~\ref{cor:scalar-saturation},
Proposition~\ref{prop:saturation-equivalence}).  The obstruction
to non-scalar $\Theta_\cA$ is not the formalism---the HPT
construction~\eqref{eq:hpt-nonscalar-recursion} is
universal---but the concrete realization of chiral algebras with
$\dim H^2_{\mathrm{cyc}} \geq 2$ and nontrivial mixed brackets.
Candidates include: admissible-level Kac--Moody (where
$H^2_{\mathrm{cyc, prim}}$ is open), products of chiral algebras,
and extended algebras with multiple independent central extensions.
This is the frontier of MC2: the proved existence theorem
(Theorem~\ref{thm:mc2-full-resolution}) provides the scaffold;
concrete realization beyond scalar is the live mathematical
problem.
\end{remark}
...
The modular periodicity conjectures below are instances of the
nilpotence-periodicity correspondence
(Remark~\ref{rem:nilpotence-periodicity}).  The scalar characteristic
$\kappa(\cA)$ is the infinitesimal (nilpotent/logarithmic) datum; the
modular period~$N$ is the global (periodic/exponential) datum.  For a
rational chiral algebra with central charge $c = p'/q'$ in lowest terms,
the T-matrix eigenvalues are
$\lambda_{r,s} = \exp\bigl(2\pi i(h_{r,s} - c/24)\bigr)$:
literally the \emph{exponentials} of conformal weights shifted by the
central charge.  The period $N = 24q'/\gcd(p',24)$ is the smallest
positive integer for which $\exp(2\pi i N c/24) = 1$, i.e., for which
$Nc/24 \in \mathbb{Z}$.  Thus the
modular periodicity formula is the condition that the \emph{exponential
of the nilpotent datum returns to identity}: periodicity is
determined by the scalar characteristic via exponentiation.

At the spectral level, the denominator polynomial of the Poincar\'e
series of $CH^*(\cA)$ encodes the eigenvalues of the monodromy
operator on bar cohomology.  The roots of this polynomial are the
exponentials of the spectral data in~$\Delta_{\cA}$, and the period
of bar cohomology is the order of the monodromy---the exponential of
the spectral characteristic.  The periodicity profile
$\Pi(\cA) = (M_{\cA}, Q_{\cA}, G_{\cA})$ of
Remark~\ref{rem:periodicity-triple} is therefore the
\emph{exponential package} corresponding to the \emph{logarithmic
package} $(\kappa(\cA), \Delta_{\cA}, \Theta_{\cA})$.
\end{remark}
...
The argument above claims \emph{eventual} periodicity
($k \gg 0$), but the passage from theta-function coefficient
periodicity to weight-space dimension periodicity is
\emph{obstructed}: the convolution $\dim V_{r,s,[k]}
= \sum_j a_\Theta(k-j)\, p(j)$ with the partition function
$p(j)$ does \emph{not} preserve eventual periodicity,
because $p(j)$ grows sub-exponentially.

\begin{computation}[M(5,8) weight-space dimensions; \ClaimStatusProvedHere]
\label{comp:m58-weight-space-periodicity}
The first test case is $M(5,8)$
(central charge $c = -7/20$, $14$ modules,
$T$-matrix period $N = 480$).
The Rocha-Caridi formula gives $\dim V_{1,1,[k]}
= p(k) - p(k-1)$ for $k < 28$ (first null vector
from $\delta^-(n\!=\!-1) = 28$), and in general:
\[
\dim V_{r,s,[k]}
\;=\;
\sum_{n \in \mathbb{Z}}
\bigl[p(k - \delta^+_n) - p(k - \delta^-_n)\bigr]
\]
where $\delta^+_n = n(qr - ps + pqn)$,
$\delta^-_n = rs + n(qr + ps + pqn)$.
Computed for all $14$ modules through level $k = 80$:
\begin{itemize}
\item Every module dimension sequence is \emph{monotonically
increasing} for $k \geq 3$ (growth rate $\sim p(k)$).
\item For every candidate period $N' \in
\{5, 8, 10, 20, 40, 480\}$ and every module $(r,s)$:
$\dim V_{r,s,[k+N']} > \dim V_{r,s,[k]}$ for all
$k \geq 10$.
\item \textbf{No eventual periodicity} is detected for any
module, at any period dividing $N = 480$ or $pq = 40$.
\end{itemize}
This confirms that Step~2 has a genuine gap: the numerator
coefficient periodicity (which holds by the theta-function
level structure) does not survive convolution with~$1/\eta$.
\end{computation}

The conjecture (Conjecture~\ref{conj:modular-periodicity-minimal})
therefore cannot be established via the three-step strategy
as stated.  The bar \emph{cohomology} periodicity may still
hold through a mechanism that involves the bar differential
(not just the chain dimensions), but this requires
computing OPE structure constants of~$M(5,8)$ and the
resulting bar differential matrices---a significantly harder
computation.
\end{remark}
...
and~\ref{conj:modular-periodicity-wzw} assert modular periodicity for
...
$T$-matrix periodicity.  The conjecture is posed for all $C_2$-cofinite
rational vertex algebras (\ClaimStatusConjectured), where Zhu's finiteness theorem and Huang's
...
the minimal-model and WZW conjectures.
(Part of the MC2 theorem package; Theorem~\ref{thm:mc2-full-resolution}.)
\end{remark}
...
shift.  For minimal models, Conjecture~\ref{conj:modular-periodicity-minimal}
...
By Conjecture~\ref{conj:modular-periodicity-minimal}, the Virasoro minimal models with
\[
c = 1 - \frac{6(p-q)^2}{pq}
\]
...
Conjecture~\ref{conj:modular-periodicity-minimal}, using the
...
Conjecture~\ref{conj:modular-periodicity-minimal}: since
...
argument of Conjecture~\ref{conj:modular-periodicity-minimal},
Step~2, this gives
...
in Step~3 of Conjecture~\ref{conj:modular-periodicity-minimal}, with
...
Conjecture~\ref{conj:modular-periodicity-wzw} covers all
simple~$\mathfrak{g}$ at all positive integer levels.  Together with
Conjecture~\ref{conj:modular-periodicity-minimal}, modular periodicity
...
this is established by Conjecture~\ref{conj:modular-periodicity-minimal};
...
Let $\mathcal{W}^k(\mathfrak{g})$ be the W-algebra at admissible level $k = -h^{\vee} + p/q$ where $h^{\vee}$ is the dual Coxeter number, $p \geq h^\vee$, $q \geq h^\vee$, $\gcd(p,q) = 1$. Then the quantum integers, quantum dimensions, and fusion coefficients (the representation-theoretic combinatorics transported through the KL/DS interface) recur with period dividing
\[
M = 2h^{\vee}pq/\gcd(p,q,h^{\vee}).
\]
This does \emph{not} by itself imply periodicity of every OPE coefficient or of the full bar complex: the proposition records only the Type~II quantum input to the periodicity profile $(N_{\mathrm{mod}}, N_{\mathrm{quant}})$.
\end{proposition}

\begin{remark}[Scope]
This proposition records only the Type~II quantum input to the
periodicity profile.  Upgrading it to periodicity of the full chiral
Hochschild or bar complex would require a separate transport theorem
for all OPE-residue data through the KL/DS interface, and that stronger
statement is not claimed here.
\end{remark}
...
The geometric contribution to bar cohomology from higher genus is
fundamentally different from the modular and quantum sources: it provides
\emph{bounded tautological depth} rather than periodicity.  The
Kodaira--Spencer operator associated to the Hodge class is
\emph{nilpotent}, not periodic, and the geometric direction of the bar
spectral sequence terminates after finitely many steps.
...
\]
Therefore $\sigma_{\mathrm{geom}}$ is nilpotent of index at most
$3g - 2$, giving at most $3g - 3$ nontrivial filtration steps.

For the boundary cases: at $g = 0$, the moduli space $\overline{\mathcal{M}}_{0,n}$ carries no Hodge bundle, so there is no geometric contribution.  At $g = 1$, the relation $c_1(\mathbb{E}) = \frac{1}{12}\delta$ in $\mathrm{CH}^1(\overline{\mathcal{M}}_{1,1})$ shows that the Hodge class contribution is controlled by the boundary divisor and is absorbed into the modular periodicity $N_{\mathrm{mod}}$.
\end{proof}

\begin{remark}[Nilpotent depth vs.\ periodicity]
\label{rem:nilpotent-vs-periodic}
\index{tautological depth!vs.\ periodicity}
The operator $\sigma_{\mathrm{geom}}$ is \emph{nilpotent}
($\sigma^N = 0$), not \emph{periodic} ($\sigma^N = \mathrm{id}$).
These are fundamentally different: a nonzero nilpotent operator
has no finite period.  The geometric contribution to bar cohomology
therefore provides a \emph{stabilization threshold}---a bounded
number of tautological correction layers---rather than a periodic
automorphism.  Geometry naturally gives nilpotence, tautological
truncation, and Hodge-theoretic bounds; it does not give a periodic
operator unless one is explicitly constructed
...
The hypothesis that the Kodaira--Spencer map factors through
$H^*(\mathcal{M}_g)$ holds at the M-level for chiral algebras
on smooth curves that do not require factorization extensions to
nodal curves.  When the bar complex extends to
$\overline{\mathcal{M}}_g$ via the factorization/clutching
structure, boundary contributions from the divisor classes
$\delta_i$ can increase the effective nilpotent depth beyond
$g - 1$ (but never beyond $3g - 2$, by
Theorem~\ref{thm:geometric-periodicity-weak}).  The gap between
the smooth bound~$g - 1$ and the compactified
bound~$3g - 2$ is controlled entirely by the boundary
contributions.
\end{remark}
...
The interior bound $g - 1$ is proved unconditionally in
Theorem~\ref{thm:geometric-depth-smooth}, using the
Graber--Vakil vanishing theorem for the tautological ring of
$\mathcal{M}_g$.  The compactified bound $3g - 2$ is
proved in Theorem~\ref{thm:geometric-periodicity-weak} by
dimension counting on $\overline{\mathcal{M}}_g$.  The
conjecture interpolates between these two bounds by
decomposing the Hodge class via the Mumford relation.

The earlier formulation of this section used the term
``geometric periodicity'' for what is actually a nilpotent
depth bound.  Nilpotency ($\sigma^N = 0$) is fundamentally
different from periodicity ($\sigma^N = \mathrm{id}$), and
no periodic geometric operator has been constructed.  The
mature formulation treats the geometric contribution as
providing \emph{tautological amplitude}---a bounded number
of correction layers indexed by $R^*(\overline{\mathcal{M}}_g)$
---rather than a period.  True periodicity in bar cohomology
arises from the modular (Type~I) and quantum (Type~II)
sources only.

\emph{Homotopy template} (Convention~\ref{conv:hms-levels}): Type~V (periodicity/depth).
The sharp interior bound $g - 1$ is proved; the conjecture
refines the boundary contribution using tautological relations
on $\overline{\mathcal{M}}_g$.
(Part of the MC2 theorem package; Theorem~\ref{thm:mc2-full-resolution}.)
\end{remark}
...
The conjecture is tagged \ClaimStatusConjectured{} because the
modular periodicity bound
(Conjecture~\ref{conj:modular-periodicity}) is conjectural.  The quantum
periodicity component is proved, and the geometric contribution is now
understood as providing nilpotent depth (proved) rather than an
additional periodic source.

The earlier formulation included a ``geometric period''
$N_{\mathrm{geom}}$ as a third factor in the lcm formula.  This was
based on a conflation of nilpotency ($\sigma^N = 0$) with periodicity
($\sigma^N = \mathrm{id}$); the operator $\sigma_{\mathrm{geom}} =
\kappa(\lambda)$ is nilpotent, not periodic.  The corrected
formulation separates the two genuine periodicity sources (modular and
quantum) from the geometric stabilization mechanism.
The lcm formula $\mathrm{Period}(\mathcal{A}) \mid
\mathrm{lcm}(N_{\mathrm{mod}}, N_{\mathrm{quant}})$ is the
correct statement; what was previously attributed to ``geometric
periodicity'' is the threshold beyond which this periodicity holds.
(Part of the MC2 theorem package; Theorem~\ref{thm:mc2-full-resolution}.)
\end{remark}
...
Conjecture~\ref{conj:complete-periodicity-classification} is the
shadow of a richer invariant: the \emph{periodicity profile}
$\Pi(\mathcal{A}) = (M_{\mathcal{A}}, Q_{\mathcal{A}},
G_{\mathcal{A}})$ of Remark~\ref{rem:periodicity-triple}.
In the corrected formulation, the profile decomposes into periodic
and nilpotent contributions:
\begin{enumerate}[label=\textup{(\roman*)}]
...
$e^{2\pi i c/24}$.  This is a genuine periodic automorphism
(conjectural sharp value).
...
to the independent quantum CG coefficient cycles.  This is
a proved periodic structure.
\item The geometric contribution is not periodic but nilpotent:
$G_{\mathcal{A}}$ encodes the tautological depth
(Theorem~\ref{thm:geometric-depth-smooth}) and the Galois
group of the Poincar\'e series denominator.  Its role is to
bound the stabilization threshold, not to contribute a
period to the lcm.
\end{enumerate}
The triple
$\Pi(\mathcal{A})$ retains the structural information (module
decomposition, algebra generators, tautological amplitude)
...
periodicity across all chiral algebras---required a conceptual
correction.  The geometric contribution is not periodic but nilpotent
(Remark~\ref{rem:nilpotent-vs-periodic}), and the mature formulation
reflects this:
\begin{enumerate}[label=\textup{(\alph*)}]
\item \emph{Structural stratum.}
  The lcm mechanism for the two genuine periodic sources is proved
  unconditionally
  (Proposition~\ref{prop:periodicity-exchange-koszul}).  Quantum
  periodicity is explicit.  The geometric theory contributes a
  proved tautological depth bound: nilpotency index $\leq g - 1$
  on smooth moduli (Theorem~\ref{thm:geometric-depth-smooth}),
  $\leq 3g - 2$ on $\overline{\mathcal{M}}_g$
  (Theorem~\ref{thm:geometric-periodicity-weak}).  What remains
  open is the sharp modular formula $N_{\mathrm{mod}}$.
\item \emph{Principal rational frontier.}
  Minimal models and WZW models are the first rational families where
  $T$-matrix periodicity and finite fusion data are explicit, but the
  actual bar-periodicity formulas remain conjectural
  (Conjectures~\ref{conj:modular-periodicity-minimal}
  and~\ref{conj:modular-periodicity-wzw}) because the missing
  Step~2 convolution argument has not been proved.
\item \emph{Outer rational frontier.}
  For general rational chiral algebras, the modular periodicity
  bound remains conjectural
  (Conjecture~\ref{conj:modular-periodicity}).  The geometric
  depth bound is now proved
  (Theorem~\ref{thm:geometric-depth-smooth}).  Natural next cases
  for modular periodicity include $\mathcal{W}$-algebra minimal
  models, orbifolds, and cosets.
\item \emph{Spectral stratum.}
  For all Koszul chiral algebras (not necessarily rational),
  the period---if it exists---divides
  $\mathrm{lcm}(N_{\mathrm{mod}}, N_{\mathrm{quant}})$,
  with each factor determined by the modular
  scalar/spectral data $(\kappa(\cA), \Delta_{\cA})$.
  ...
  The geometric depth controls when this periodicity
  becomes visible.
\end{enumerate}
This stratification replaces the original unified doctrine
with a more honest, yet structurally richer, account that
correctly separates periodic from nilpotent contributions.
...
\begin{remark}[Scope]
This conjecture lies outside the scope of the present monograph: it is a
physical interpretation of Koszul duality, not a mathematical theorem.
The evidence below is heuristic.  A precise formulation would require
specifying the bulk theory as a chiral algebra on a 3-manifold with
boundary (in the sense of Costello--Li), relating $\cA_{\text{bulk}}$
to a factorization algebra on the solid torus, and identifying the
boundary restriction with the Koszul-dual shadow via the bar-cobar
adjunction. The central charge relation $c_{\text{bulk}} +
c_{\text{boundary}} = 26$ is a numerical coincidence (the bosonic
string critical dimension) that has no known proof from the Koszul
duality framework alone.  In the monograph's frontier order, the
theorematic input here is the boundary-side bar-cobar/Hochschild
package; any filtered H-level bulk target lies upstream in MC4, and the
holographic bulk-boundary dictionary itself lies downstream in MC5.

\emph{Homotopy template} (Convention~\ref{conv:hms-levels}): Type~VII (physics-dictionary).
The conjecture treats holographic bulk-boundary duality as a Koszul
template, with $\cA_{\text{bulk}}^! \simeq \cA_{\text{boundary}}$
appearing only as a conjectural shadow after additional physics input.
