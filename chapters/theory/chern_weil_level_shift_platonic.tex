...
%
% Russian-school lossless rectification and structural reconstitution.
...
\begin{definition}[Invariant-form, Casimir, and affine OPE normalisation;
\ClaimStatusDefinitional]
\label{def:chern-weil-normalization-data}
...
\begin{definition}[r-matrix convention labels; \ClaimStatusDefinitional]
...
\label{rem:level-shift-universal}
...
$\rKZ(z) = \OmegaKZ / ((k + \hv) z)$ are distinct as
...
\section{Universal convention bridge}
\label{sec:level-shift-convention-bridge}

The trace-form and KZ normalisations are related by a universal
level-shift comparison.

\begin{corollary}[$r$-matrix convention bridge with explicit $k=0$ check]
\label{cor:level-shift-universal-convention-bridge}
\ClaimStatusProvedHere
Every $r$-matrix presentation for $V_k(\fg)$ in this theory declares
its convention and exhibit the corresponding $k = 0$ check, as follows.
\begin{enumerate}
 \item \emph{Trace-form normalisation.} The $r$-matrix has the
 $k$ numerator prefix:
 ...
 \item \emph{KZ normalisation.} The $r$-matrix has the
 $(k + \hv)$ denominator:
...
\end{enumerate}
A bare expression $r(z) = \Omega/z$ with no level prefix and no
$(k + \hv)$ denominator has no normalisation; by
...
chosen normalisation. The comparison is universal: the level shift
$k \mapsto k + \hv$ supplies the KZ
...
\begin{remark}[Detector for level-prefix omission]
\label{rem:level-prefix-grep-detector}
Once
Corollary~\ref{cor:level-shift-universal-convention-bridge} fixes
the convention discipline, every $r$-matrix write carries either
an explicit $k$ numerator (trace form) or an explicit $(k+\hv)$
denominator (KZ); the level is never implicit. The pattern
$r(z) = \Omega/z$ with no level prefix is therefore a detector for
violations of the discipline: any such write is either an undeclared
free-field identity at $k=1$ or a typesetting omission to be
corrected.
\end{remark}

\section{Independent derivations of the level shift}
\label{sec:level-shift-independent-verification}

\begin{itemize}
 \item Theorem~\ref{thm:level-shift-universality} follows from this
 chapter's conditions (i)--(iii).  Kac's
 \emph{Infinite-Dimensional Lie Algebras}, Chapter~12, derives the
 same shifted parameter from the Sugawara formula.  That derivation
 of the Sugawara denominator uses the quadratic Casimir eigenvalue
 on the adjoint, not the $r$-matrix CYBE.
 \item Proposition~\ref{prop:shift-appears-universally} agrees with
 Di~Francesco--Mathieu--S\'en\'echal, \emph{Conformal Field Theory},
 \S15.3.  Their derivation of
 the KZ connection starts from the Ward identity for conformal blocks,
 rather than from the modular-characteristic calculation.
 \item Theorem~\ref{thm:h-dual-coxeter-coincidence} uses the
 Freudenthal--de~Vries strange formula.  Wendlandt,
 \emph{The restricted Yangian and Yangian doubles}, \S3, obtains the
 corresponding trace-form normalization of the Yangian $r$-matrix
 from Hopf algebra axioms rather than from the strange formula.
 \item In the free-field realization of
 $\widehat{\fsl}_2{}_1$, the central double pole gives the
 trace-form coefficient $\Omega_{\mathrm{tr}}/z$, whereas the
 Sugawara Ward identity gives the KZ coefficient
 $\Omega_{\mathrm{KZ}}/(3z)$.  This is the smallest instance of the
 convention dictionary in
 Theorem~\ref{thm:trace-KZ-convention-bridge}.
\end{itemize}

\section{Trace-form versus KZ presentation: not a hidden Casimir rescaling}
\label{sec:trace-vs-kz-r-matrix-disambiguation}

The candidate identification
$k\Omega_{\mathrm{tr}}/z \stackrel{?}{=} \Omega/((k+\hv)z)$ — the trace-form
$r$-matrix and the KZ $r$-matrix as the same tensor up to a
hidden Casimir rescaling — is false.
Theorem~\ref{thm:level-shift-universality} proves the opposite. The
denominator $k+\hv$ is the universal Sugawara denominator; the
trace-form coefficient $k$ and the KZ coefficient $(k+\hv)^{-1}$
are different extractions from the same affine OPE.
Theorem~\ref{thm:trace-KZ-convention-bridge} bridges the two as a
dictionary on conventions, not as a rational identity in the same
generators.

\section*{The level shift as the universal Sugawara parameter}
...
$r(z)$ remain convention-separated: one reads the central double
pole, the other reads the Sugawara Ward identity. The
critical-level locus $k = -\hv$ is the zero of this universal
denominator, and Feigin--Frenkel localisation is the chart of the
chiral datum adapted to that locus.
Corollary~\ref{cor:level-shift-universal-convention-bridge}
makes the convention discipline universal: every bare $r$-matrix
