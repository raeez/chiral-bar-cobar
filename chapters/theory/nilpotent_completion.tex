...
weight at which Brown's motivic MZV theorem is used here without the
Zagier--Hoffman conjecture};
...
completion, without any recourse to the Zagier--Hoffman
...
unconditional statement for each concrete weight where the
...
of the tower as a chain-level class.
...
K$3$ target comparison invariant and the Fake-Monster invariant at
weight~$12$ are independent observables. The direct compute witness is the
...
\emph{computed at chain level}: earlier analyses reached only
...
establishes the Deligne motivic MZV basis through
...
strict Mittag--Leffler pro-limit is defined through weight~$12$
because each truncated weight still has finite
...
\begin{remark}[Geometric interpretation: target super-MC moduli]
\label{rem:nc-grt1-geometric-interpretation}
The conditional target $\mathrm{GRT}_1^{\mathrm{super}}$-torsor of
...
quantisation and under the obstruction-vanishing hypothesis, a
...
(Deligne~$2013$ Bourbaki~$1054$). Geometrically, Vol.~I sees the
target super-EK comparison space of $\mathfrak g_{\Delta_5}$ as the
moduli of complex structures on super-MC moduli. The passage from this
target moduli problem to the compact Hall source is a separate
Vol.~III recognition problem.
...
Unfolding the target torsor of
...
torsor action on the target super-quasi-Hopf datum later compared with
$\mathbf H_{\Delta_5}$ by the Vol.~III recognition gates.
\cite{Drinfeld1990,Brown2012,Brown2017,Willwacher2015,
 SchnepperWillwacher2017}.
\end{remark}

\begin{remark}[Motivic-Galois geometric interpretation of the target torsor]
...
the KZ associator by the Drinfeld twist action used in
Theorem~\ref{thm:nc-grt1-super-torsor-delta5}.
...
one for type $A_{n\ge 2}$ \textup{(}the $d$-tensor\textup{)}, and zero
for $B_n, C_n, D_n, G_2, F_4, E_6, E_7, E_8$.  By Chevalley's theorem
the primitive degrees are the exponents plus one.  Thus
\[
\deg(E_6)=\{2,5,6,8,9,12\},\qquad
\deg(E_7)=\{2,6,8,10,12,14,18\},
\]
and
\[
\deg(E_8)=\{2,8,12,14,18,20,24,30\}.
\]
The $E_8$ list excludes degree~$3$; equivalently, there is no
$E_8$-invariant homogeneous polynomial of degree~$3$.
...
Mukai enhancement\textup{)} has only its motivic associator coordinate
window unconditional through weight~$12$. Its K$3$-BKM target torsor is
conditional on the imaginary-cone vanishing hypothesis, and its compact
Hall-source recognition is conditional on the Vol.~III source gates
plus cohomology/Heegner comparison. The $K^\kappa = 8$ ceiling is a
target comparison invariant in this chapter, not an unconditional
classification of the K$3$ row through weight~$12$.
\end{remark}
