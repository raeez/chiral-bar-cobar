...
should be read as belonging to the completion regime (Stratum~II),
...
\begin{remark}[Completion-regime proof density]\label{rem:completion-convergence-frontier}
...
$n \gg 0$. This places the result in the completion regime of
...
completion-regime scope of the argument.
\end{theorem}
...
\begin{remark}[Finite-stage compatibility]\label{rem:completed-bar-cobar-frontier}
...
regime of Remark~\ref{rem:existence-regimes}(ii).
\end{remark}
...
\begin{remark}[Essential-image proof density]\label{rem:koszul-dual-characterization-frontier}
The backward direction (Step~3) invokes
Theorem~\ref{thm:completed-bar-cobar}, whose finite-stage compatibility
qualifier is
recorded in Remark~\ref{rem:completed-bar-cobar-frontier}. The
essential-image characterization therefore inherits the same
proof-density qualifier: the result is valid under the convergence
hypotheses of Theorem~\ref{thm:completion-convergence}, but those
hypotheses are part of the completion regime, not the strict
finite-type core.
\end{remark}
