...
\begin{remark}[Wave-$19$ DNA: $c_{12}^{(9)}$ explicit via KZ iterated integral]
\label{rem:nc-c12-explicit}
Wave $18$ (Remark~\ref{rem:nc-phi12}) located $\zeta(3, 3, 3, 3)$
...
but left its value unspecified.  Wave $19$ computes it explicitly.
Under the Drinfeld--Deligne isomorphism
...
\emph{computed on the nose}: Wave-$17$ and prior reached only
...
\begin{remark}[Wave-$19$ DNA: chain/$(\infty,1)$ agreement for $c_{12}^{(9)}$]
...
\begin{remark}[Wave-$19$ DNA: Fake-Monster Fourier-expansion non-interference]
\label{rem:nc-fake-monster-fourier-check}
The Wave-$18$ signature-obstruction verdict
...
\begin{remark}[Wave-$19$ DNA: weight $\ge 13$ conjectural horizon]
...
coefficient is conjectural on Zagier--Hoffman.  Wave $19$ therefore
...
weight~$12$ (item~(III).A of the Wave-$19$ adjudication ledger).
\end{theorem}
...
(real-root part, signature $(2, 1)$, Wave-$19$ item~(I).$4$) this
...
(Wave-$19$ Etingof depth-$4$ DNA; coefficient
$c_{12}^{(9)} = \zeta(3, 3, 3, 3)/12!$ from
Remark~$\ref{rem:nc-c12-explicit}$). The Borcherds leg
$[\Phi_{10}/\eta^{24}]$ of the universal obstruction (Wave-$14$
DNA, Vol.~I Rem.~\ref{rem:nc-phi3-borcherds}) is
...
of the Wave-$19$ synthesis \S$2.1$ (Gritsenko--Nikulin~$1998$
...
universal associator, written in the Wave-$19$ basis as
\[
\phi^{(3)}
\;=\;
...
\]
(Wave-$19$ synthesis \S$3.2$; cf.~Vol~III
Rem.~\ref{rem:qgf-phi3-explicit}), where
$c_{\mathrm{symm}}^{(3)}, c_{\mathrm{KM}}^{(3)},
c_{\Phi_{10}/\eta^{24}}^{(3)}, c_{\mathrm{skew}}^{(3)} \in \mathbb Q$
are the four Wave-$19$ reference coefficients. The generator
