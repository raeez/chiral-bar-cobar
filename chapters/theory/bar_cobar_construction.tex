...
\emph{Setup.} The little disks operad $\mathcal{D}_2$ acts on configuration spaces:
...
Applying $d_{\text{internal}}$ twice, each $c_i$ contributes $d_{\mathcal{C}}^2(c_i) = 0$ (by hypothesis that $(\mathcal{C}, d_{\mathcal{C}})$ is a cochain complex), and cross-terms $d_{\mathcal{C}}(c_i) \otimes d_{\mathcal{C}}(c_j)$ for $i \neq j$ appear with equal signs in both applications, hence cancel:
...
\item \emph{Koszul duality:} $\mathcal{H}_k^! \simeq \mathrm{CE}(\hat{\mathfrak{h}}_k)$ (the Chevalley--Eilenberg DG chiral algebra, a fermionic system)
\item The cobar of $\bar{B}(\mathcal{H}_k)$ gives back $\mathcal{H}_k$ (bar-cobar inversion), but the Koszul dual $\mathrm{CE}(\hat{\mathfrak{h}}_k)$ is a \emph{different type} of algebra (Lie vs.\ commutative); these are distinct statements.
\end{itemize}
