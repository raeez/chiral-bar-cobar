...
are:
\begin{enumerate}[label=\textup{(\roman*)}]
\item mixed-weight orthogonality on the visible generators,
  \[
  \langle W^{(s)}(z)W^{(t)}(0)\rangle=0
  \qquad (s\neq t,\ 2\le s,t\le 4);
  \]
\item diagonal visible normalization,
  \[
  \langle W^{(s)}(z)W^{(s)}(0)\rangle=\frac{c/s}{z^{2s}}
  \qquad (s=2,3,4),
  \]
...
exactly inheritance of these visible pairing statements.
\end{proposition}
...
But the $s=2$ case of \textup{(ii)} is exactly the stress-tensor
normalization
\[
\langle T(z)T(0)\rangle=\frac{c/2}{z^4}.
\]
Therefore, once the visible Ward action is fixed, the only additional
inputs used by the formal reduction lemmas are \textup{(i)} and
\textup{(ii)}.
\end{proof}
...
with the visible pairing package isolated in
Proposition~\ref{prop:winfty-stage4-visible-pairing-gap}; explicitly,
the visible generators $W^{(2)},W^{(3)},W^{(4)}$ satisfy mixed-weight
orthogonality and the diagonal normalization
\[
\langle W^{(s)}(z)W^{(s)}(0)\rangle=\frac{c/s}{z^{2s}}
\qquad (s=2,3,4)
\]
...
mixed-weight orthogonality or the diagonal visible pairings inside that
quotient residue calculus.  Thus the first unconditional standard-tower
...
visible Virasoro Ward action is fixed the missing lemma is exactly
automatic stage-$4$ Ward normalization in the sense of
Definition~\ref{def:winfty-stage4-ward-normalized}.
\end{remark}

\begin{conjecture}[Automatic stage-$4$ Ward normalization from the
quotient-system hypotheses]
...
\item the quotient vacuum functional induces mixed-weight orthogonality
  on the visible generators,
  \[
  \langle W^{(s)}(z)W^{(t)}(0)\rangle = 0
  \qquad (s\neq t,\ 2\le s,t\le 4);
  \]
\item the diagonal visible pairings are normalized by
  \[
  \langle W^{(s)}(z)W^{(s)}(0)\rangle = \frac{c/s}{z^{2s}}
  \qquad (s=2,3,4)
  \]
  ...
  Drinfeld--Sokolov stage.
\end{enumerate}
By Proposition~\ref{prop:winfty-stage4-visible-pairing-gap}, the open
content is exactly the pairing package in \textup{(iii)}--\textup{(iv)}.
...
the mixed-weight orthogonality and diagonal visible pairings on the
stage-$4$ residue calculus.  If they do, the four-channel refinement is
automatic; if they do not, then
...
verified computationally
(\texttt{compute/lib/w4\_stage4\_coefficients.py}, $83$~tests):
\begin{itemize}
