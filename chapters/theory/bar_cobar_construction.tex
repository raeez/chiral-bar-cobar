...
\begin{example}[Heisenberg degree-$1$ bar differential]\label{ex:heisenberg-d-deg1}
...
\begin{example}[Augmentation for $\widehat{\mathfrak{sl}}_2$]
...
\begin{theorem}[$A_\infty$ structure on cobar {\cite{LV}}; \ClaimStatusProvedElsewhere]\label{thm:cobar-ainfty}
...
\begin{remark}[Central extension as genus-$1$ component of $\Theta$]
...
\begin{example}[$W_\infty$: exact MC4 frontier]
...
\begin{proposition}[Stage-$3$ principal packet from the explicit
...
\begin{proposition}[Stage-$4$ residual packet after the theorematic
...
\begin{computation}[Stage-$4$ seed set and residual packet verification]
...
\begin{corollary}[Stage-$4$ top-pole packet after primaryity;
\ClaimStatusProvedHere]
...
\begin{corollary}[Stage-$4$ parity-compressed packet;
\ClaimStatusProvedHere]
...
\begin{corollary}[Stage-$4$ packet as three local OPE blocks;
\ClaimStatusProvedHere]
...
\begin{corollary}[Stage-$4$ frontier as one mixed block and three
...
\begin{corollary}[Stage-$4$ mixed block split by swap parity;
\ClaimStatusProvedHere]
...
\begin{corollary}[Stage-$4$ mixed block as one vanishing channel and a
...
\begin{corollary}[Principal stage-$4$ self-coupling
...
\begin{corollary}[Stage-$4$ principal target packet after theorematic
...
\begin{corollary}[Stage-$4$ \texorpdfstring{$W_\infty$}{W_infty}
...
\texorpdfstring{$W_\infty$}{W_infty} stage-$4$ contraction;
\ClaimStatusProvedHere]
...
\begin{definition}[Stage-$4$ Ward-normalized standard
...
\begin{remark}[What is still missing for an unconditional stage-$4$
...
\begin{proposition}[Stage-$4$ visible mixed-weight orthogonality from
...
\begin{proposition}[Stage-$4$ visible
...
\begin{conjecture}[Stage-$4$ visible weight-$4$ normalization]
...
\begin{conjecture}[Automatic stage-$4$ Ward normalization from the
...
\texorpdfstring{$W_\infty$}{W_infty} stage-$4$ input;
\ClaimStatusProvedHere]
...
\begin{proposition}[Stage-$4$ swap-even residue channel from a visible
...
\begin{corollary}[Stage-$4$ residue packet as three higher-spin
...
\begin{corollary}[Stage-$4$ higher-spin comparison as a
...
\begin{conjecture}[Stage-$4$ visible top-pole Borcherds transport]
...
\begin{corollary}[Equivalent exact forms of the remaining stage-$4$
...
\begin{proposition}[Exact local attack order for the stage-$4$
...
\begin{corollary}[Stage-$5$ entry packet and mixed transport packet;
\ClaimStatusProvedHere]
...
\begin{proposition}[Stage-$5$ entry packet as mixed-entry and
...
\begin{proposition}[Exact reduced tail input at stage~$5$;
\ClaimStatusProvedHere]
...
\begin{proposition}[Exact missing mechanism for the reduced stage-$5$
...
\begin{proposition}[Stage-$5$ mixed transport packet as three
...
\begin{proposition}[Stage-$5$ higher-spin packet by target spin;
\ClaimStatusProvedHere]
...
\begin{corollary}[Stage-$5$ target-$5$ corridor;
\ClaimStatusProvedHere]
...
\begin{corollary}[Residual target-$5$ continuation after the tail
...
\begin{proposition}[Residual target-$5$ continuation as mixed transport
...
\begin{proposition}[Residual target-$5$ continuation as two singleton
...
\begin{proposition}[Stage-$5$ transport ladders ordered by pole profile;
\ClaimStatusProvedHere]
...
\begin{proposition}[Stage-$5$ visible
...
\begin{proposition}[Stage-$5$ target-$5$ pole-$3$ transport singleton
...
\begin{proposition}[Stage-$5$ target-$5$ pole-$4$ transport singleton
...
\begin{corollary}[Stage-$5$ self-return singleton vanishes on the
...
\begin{proposition}[Stage-$5$ reduced tail singleton from a visible
...
\begin{corollary}[Stage-$5$ tail singleton equates neighboring target-$4$
and target-$3$ transport channels on the visible
...
\begin{corollary}[Stage-$5$ target-$5$ corridor contracts to the tail
...
\begin{corollary}[Stage-$5$ target-$5$ corridor carries no new
...
\begin{proposition}[Stage-$5$ target-$4$ pole-$5$ transport singleton
vanishes on a visible \texorpdfstring{$W^{(4)}$}{W4}-pairing locus;
\ClaimStatusProvedHere]
...
\begin{proposition}[Stage-$5$ target-$3$ pole-$5$ transport singleton
...
\begin{proposition}[Stage-$5$ target-$4$/target-$3$ transport channels
...
\begin{corollary}[Stage-$5$ mixed transport frontier carries one
...
\begin{corollary}[Stage-$5$ higher-spin packet reduces to one effective
...
\begin{proposition}[Exact local attack order for the first stage-$5$
...
\begin{conjecture}[Stage-$5$ entry-packet identities]
...
\begin{conjecture}[Stage-$5$ mixed transport identities]
...
\begin{conjecture}[Stage-$5$ transport target-$3$ ladder identities]
...
\begin{conjecture}[Stage-$5$ transport target-$4$ ladder identities]
...
\begin{conjecture}[Stage-$5$ transport target-$5$ ladder identities]
...
\begin{conjecture}[Stage-$5$ target-$5$ transport singleton from
...
\begin{computation}[Full stage-$4$ reduction chain verification]
...
\begin{proposition}[Explicit $W_4$ OPE structure constants;
...
\begin{corollary}[Principal stage-$4$ higher-spin packet from two
...
\begin{computation}[Full $W_4$ OPE coefficient extraction]
...
\begin{theorem}[Twisting by MC elements {\cite{LV}}; \ClaimStatusProvedElsewhere]
...
\begin{corollary}[Bar cohomology computes Ext {\cite{LV}}; \ClaimStatusProvedElsewhere]\label{cor:bar-computes-ext}
...
\begin{example}[$W_\infty$: exact MC4 frontier]\label{ex:winfty-completion-frontier-comprehensive}\label{ex:winfty-no-dual-comprehensive}
...
algebra~\cite{Positselski} replace the ground field~$k$ by
...
This follows Positselski~\cite[\S4.1]{Positselski}.
\end{remark}
...
(Positselski~\cite[\S4.5]{Positselski}).  When $h \neq 0$, the standard
...
Positselski~\cite[\S4.4, proof of Theorem]{Positselski}).
...
Positselski~\cite[\S4.4]{Positselski}, using the injective
...
\begin{definition}[Functors $\Phi_C^{\mathrm{ch}}$ and $\Psi_C^{\mathrm{ch}}$]
...
\begin{lemma}[Key properties of $\Phi_C^{\mathrm{ch}}$ and
$\Psi_C^{\mathrm{ch}}$; \ClaimStatusProvedHere]
...
(cf.\ the example in
Positselski~\cite[\S5.1]{Positselski}), the
...
\begin{proof}
The proof follows Positselski~\cite[\S5.2]{Positselski}, with the
...
Positselski~\cite[\S5.5]{Positselski}, with the key input that any
...
(cf.~\cite[\S5.5]{Positselski}), the equivalence extends to all objects.
\end{proof}
...
(Positselski~\cite[\S4.5]{Positselski}).
...
\cite{Positselski}), the cohomomorphism complex is identified
...
\begin{remark}[Relation to the fundamental theorem $\mathrm{A}_0$]
...
Positselski~\cite{Positselski}.  In this category, a curved complex
...
\begin{proof}
The cobar construction $\Omega_X(\cC) = T(s\,\overline{\cC})$ with
differential $d_{\mathrm{cobar}} = d_{\mathrm{coop}} +
d_{\mathrm{tw}}$ is the cofree algebra on $s\,\overline{\cC}$, with
the differential encoding the coalgebra structure of~$\cC$.
The cofree algebra on a module $V$ is
$\mathbb{R}\mathrm{Hom}(k, T^c(V))$, and for
$V = s\,\overline{\cC}$ supported on $\operatorname{Ran}(X)$, this
...
\begin{remark}[The master diagram at genus~$0$]
