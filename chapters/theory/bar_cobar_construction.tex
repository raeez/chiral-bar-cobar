...
The key point is that purely geometric operations (residues on configuration spaces) recover purely algebraic data (OPE structure constants).
\end{remark}
...
\textbf{Configuration space:} $C_2(\mathbb{P}^1) \subset (\mathbb{P}^1)^2$ has $\dim_\mathbb{C} = 2$.  The $\text{PSL}_2$ action is transitive on ordered pairs of distinct points, so the quotient $C_2(\mathbb{P}^1)/\text{PSL}_2$ is a point.  After fixing the $\text{PSL}_2$ gauge (e.g., $z_2 = \infty$), the residual coordinate is $z_1 \in \mathbb{C}$.
...
\textbf{Configuration space:} $\overline{C}_3(\mathbb{P}^1)$ has $\dim_\mathbb{C} = 3$.  Fixing the $\text{PSL}_2$ gauge (e.g., $z_3 = 0, z_2 = 1, z_1 = \infty$ for the normalization), the quotient $\overline{C}_3(\mathbb{P}^1)/\text{PSL}_2 \cong \overline{\mathcal{M}}_{0,4} \cong \mathbb{P}^1$ has $\dim_\mathbb{C} = 1$.  The $\text{PSL}_2$-reduced configuration has one free cross-ratio parameter.
...
\subsection{Sign Conventions - Complete System}\label{sec:sign-conventions}
...
\textbf{Formulation 2 (Orlik--Solomon presentation):}
...
\textbf{Internal differential $d_{\text{int}}$:} Acts on $\mathcal{A}$-factors. Since $d_{\text{int}}$ acts on each tensor factor independently and $\Delta$ is the deconcatenation coproduct, we have:
$$\Delta \circ d_{\text{int}} = (d_{\text{int}} \otimes \text{id} + \text{id} \otimes d_{\text{int}}) \circ \Delta$$
by the Leibniz rule for coderivations of the tensor coalgebra.
...
\item \textbf{External product:} For disjoint open subsets $U_1, U_2 \subset X$, the identification $C_n(U_1 \sqcup U_2) \cong \bigsqcup_{k+l=n} C_k(U_1) \times C_l(U_2)$ induces a natural isomorphism $\bar{B}_{\mathrm{geom}}(\mathcal{A}|_{U_1 \sqcup U_2}) \cong \bigoplus_{k+l=n} \bar{B}_k(\mathcal{A}|_{U_1}) \otimes \bar{B}_l(\mathcal{A}|_{U_2})$, compatible with boundary stratifications.
...
The $A_\infty$ relations follow from the boundary stratification of moduli spaces (see \cite{LodayVallette}, Chapter~9):
\[
\partial \overline{M}_{0,k+1} = \bigcup_{I \sqcup J = [k+1], |I|,|J| \geq 2} \overline{M}_{0,|I|+1} \times \overline{M}_{0,|J|+1}
\]
Each boundary stratum contributes a composition $m_i \circ m_j$ to $\partial m_k$; the boundary $\partial^2 = 0$ identity yields the $A_\infty$ relations.
\end{proof}
...
("strict") versus when does it hold only up to homotopy?}
...
Here $d_{dR}^2 = 0$ \textbf{strict} because:
...
\begin{equation}
d_{\text{bar}}^2 = 0 \quad \textbf{strictly}
\end{equation}
\end{theorem}
...
\begin{equation}
d_{\text{bar}}^2 = 0 + 0 + 0 + 0 + 0 + 0 + 0 = 0 \quad \textbf{strictly}
\end{equation}
...
\textbf{Consequence:} Again $d_{\text{bar}}^2 = 0$ strict.
...
\begin{remark}[Why This Matters]\label{rem:why-strict-matters}
The distinction between strict and homotopy nilpotence has significant consequences:

\textbf{For computations:}
\begin{itemize}
\item Strict $\Rightarrow$ can compute cohomology directly
\item Homotopy only $\Rightarrow$ need spectral sequences that may not degenerate
\end{itemize}

\textbf{For convergence:}
\begin{itemize}
\item Strict $\Rightarrow$ bar-cobar adjunction works without completion (for quadratic algebras)
\item Homotopy only $\Rightarrow$ must complete, convergence issues
\end{itemize}

\textbf{For physics:}
\begin{itemize}
\item Strict $\Rightarrow$ quantum corrections are controlled by central charges
...
Grothendieck would observe that the strict nilpotence is a consequence of \textbf{functoriality}:
...
\subsection{Conclusion: Resolution of Strict vs Homotopy}
...
\begin{equation}
d_{\text{bar}}^2 = 0 \quad \textbf{strictly (not merely up to homotopy)}
\end{equation}
...
\begin{itemize}
\item \textbf{Underlying space}: $\mathrm{Sym}^{\mathrm{ch}}(V^*)$ as graded chiral algebra (commutative, \emph{not} Heisenberg)
\item \textbf{Differential}: $d = 0$ (since $\mathfrak{h}$ is abelian)
