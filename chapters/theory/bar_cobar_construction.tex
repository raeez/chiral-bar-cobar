...
\emph{General case.}
The three sign sources factor through the parity homomorphism $\epsilon\colon \mathrm{Aut}(T) \to \{+1,-1\}$ for each rooted tree~$T$ labeling a bar complex element.  Koszul signs are determined by the permutation of tensor factors (parity $(-1)^{|\phi_i||\phi_j|}$), orientation signs by the ordering of configuration space coordinates ($(-1)^{\operatorname{sgn}(\sigma)}$ for a coordinate permutation), and operadic signs by the operad composition structure ($(-1)^{p}$ where $p$ counts transpositions of odd elements).  The compatibility relation $\epsilon_{\mathrm{Koszul}} \cdot \epsilon_{\mathrm{orient}} \cdot \epsilon_{\mathrm{operad}} = +1$ holds because the combined sign is the determinant of the induced action on the desuspension $s^{-1}\bar{\mathcal{A}}$: the desuspension shifts each generator degree by $-1$, so transposing two desuspended elements $s^{-1}\phi_i$ and $s^{-1}\phi_j$ produces the sign $(-1)^{(|\phi_i|-1)(|\phi_j|-1)}$, which absorbs the operadic sign $(-1)^{|\phi_i|+|\phi_j|}$ from the operad composition and the orientation sign $(-1)^1$ from the coordinate swap.  An explicit check at arity~2 (the case above) confirms this, and the general case follows by induction on tree depth.
\end{proof}
...
have no Koszul dual at all (e.g., $\mathcal{W}_\infty$).
...
\begin{lemma}[Adjunction]\label{lem:chiral-co-contra-adjunction}
