...
\begin{enumerate}
\item \emph{Perfect pairing:} Non-degenerate in both arguments (by the holonomicity standing assumption: each graded piece is a holonomic $\mathcal{D}$-module on $\overline{C}_n(X)$, hence has finite-dimensional stalks, and Verdier duality for holonomic $\mathcal{D}$-modules is an involution~\cite{KashiwaraSchapira})
...
At the level of $\mathcal{D}$-modules on $\overline{C}_n(X)$, the bar and cobar differentials are flat connections (the bar complex is a complex of holonomic $\mathcal{D}$-modules by the standing assumption).  Verdier duality is an exact contravariant involution on $D^b_{\mathrm{hol}}(\mathcal{D}_{\overline{C}_n(X)})$ that sends a flat connection $\nabla$ to the adjoint connection $\nabla^\dagger$ (see~\cite{KashiwaraSchapira}, Chapter~4).  In particular, $\langle \nabla(\omega), K \rangle = -\langle \omega, \nabla^\dagger(K) \rangle$ is the defining property of the adjoint.  Since $d_{\mathrm{bar}}$ is the flat connection on the bar complex and $d_{\mathrm{cobar}} = d_{\mathrm{bar}}^\dagger$ under Verdier duality, the differential compatibility
$$\langle d_{\text{bar}}\omega, K \rangle = -\langle \omega, d_{\text{cobar}}K \rangle$$
holds by the functoriality of Verdier duality on holonomic $\mathcal{D}$-modules, without requiring a component-by-component Stokes argument.
...
\begin{remark}[Criteria for chiral Koszul pairs --- updated recollection]\label{def:koszul-criteria}
...
realization problems remain on the MC4/MC5 frontier.  By
...
residue-side checks
$\mathsf{C}^{\mathrm{res}}_{4,4;2;0,6}=2$ and
$\mathsf{C}^{\mathrm{res}}_{3,4;2;0,5}=0$, not another stabilization
theorem.
\end{corollary}
...
\begin{proof}
Let $A:=W^{(s)}$ and $B:=W^{(t)}$ with $s<t$.  By Step~2 of
Theorem~\ref{thm:wn-obstruction}, the mixed two-point function
\[
\langle A(z)B(0)\rangle
\]
vanishes identically because $A$ and $B$ have distinct conformal
weights.  Since both fields are primary, the Virasoro Ward identity
therefore gives
\[
\langle T(x)A(z)B(0)\rangle = 0.
\]

Now extract the $T$-channel from the mixed OPE.  By
Proposition~\ref{prop:winfty-ds-primary-top-pole}, the undifferentiated
primary target $T=W^{(2)}$ can occur only at the top pole order
$s+t-2$, with coefficient
\[
\mathsf{C}^{\mathrm{DS}}_{s,t;2;0,\,s+t-2}(N).
\]
All lower-pole $T$-descendants have conformal weight $>2$, so their
two-point functions with $T$ vanish by weight mismatch.  Thus the
coefficient of $x^{-4}z^{-(s+t-2)}$ in the three-point function
$\langle T(x)A(z)B(0)\rangle$ is exactly
\[
\frac{c}{2}\,\mathsf{C}^{\mathrm{DS}}_{s,t;2;0,\,s+t-2}(N),
\]
because $\langle T(x)T(0)\rangle = \frac{c/2}{x^4}$ in the same
normalization.  The three-point function vanishes identically and
...
N_s:=\frac{c}{s},
\]
so that Theorem~\ref{thm:wn-obstruction} gives
\[
\langle A(z)A(0)\rangle=N_s z^{-2s}.
\]
Because $A$ is Virasoro-primary of weight $s$, the Ward identity gives
\[
...
By Proposition~\ref{prop:winfty-ds-primary-top-pole}, the
undifferentiated stress tensor can occur in the self-OPE only at the
top pole $2s-2$, with coefficient
\[
\mathsf{C}^{\mathrm{DS}}_{s,s;2;0,\,2s-2}(N).
\]
Pairing that term with $T(x)$ contributes
\[
\mathsf{C}^{\mathrm{DS}}_{s,s;2;0,\,2s-2}(N)\cdot \frac{c/2}{x^4}
\,z^{-2s+2},
\]
because $\langle T(x)T(0)\rangle=\frac{c/2}{x^4}$.  Matching the
$x^{-4}z^{-2s+2}$ coefficient gives
\[
\mathsf{C}^{\mathrm{DS}}_{s,s;2;0,\,2s-2}(N)\cdot \frac{c}{2}=c.
\]
...
\begin{corollary}[Stage-$4$ frontier after theorematic mixed
...
residual principal stage-$4$ comparison decomposes as:
\begin{enumerate}[label=\textup{(\roman*)}]
\item a self-coupling sector consisting of the two free scalar
  coefficients
  \[
  c_{334},\qquad c_{444};
  \]
\item a mixed swap-even channel given by the coefficient
  \[
  \mathsf{C}_{3,4;3;0,4};
  \]
\item a mixed swap-odd channel given by the coefficient
  \[
  \mathsf{C}_{3,4;4;0,3};
  \]
\item a normalization check in the self-coupling $W^{(4)}$-$W^{(4)}$
  stress-tensor channel
  \[
  \mathsf{C}^{\mathrm{res}}_{4,4;2;0,6}=2;
  \]
\item a vanishing check in the mixed $W^{(2)}$ target channel
  \[
  \mathsf{C}^{\mathrm{res}}_{3,4;2;0,5}=0.
  \]
\end{enumerate}
Equivalently, the only potentially nonzero unfixed principal stage-$4$
data are four coefficients:
\[
...
\]
to the universal value `2`, so that channel becomes a residue-side
normalization check rather than free principal data.  Combining the
three reductions gives the displayed decomposition.
\end{proof}
...
\item at stage~$4$, after removing the theorematic Virasoro and
  ...
  the four free coefficient channels
  \[
  c_{334},\qquad c_{444},\qquad
  \mathsf{C}_{3,4;3;0,4},\qquad \mathsf{C}_{3,4;4;0,3},
  \]
...
verified computationally
(\texttt{compute/lib/w4\_stage4\_coefficients.py}, $72$~tests):
\begin{itemize}
...
\item \emph{Virasoro elimination}
  ...
  reduce to $4$~free coefficients $+$ $2$~residue-side checks.
\end{itemize}
...
the four free stage-$4$ coefficients of
...
\end{align}
The two residue-side predictions of
Corollary~\textup{\ref{cor:winfty-ds-stage4-five-plus-zero}} are
confirmed:
\begin{equation}
\mathsf{C}^{\mathrm{DS}}_{4,4;2;0,6} = 2,
\qquad
\mathsf{C}^{\mathrm{DS}}_{3,4;2;0,5} = 0.
\label{eq:w4-predictions-verified}
\end{equation}
\end{proposition}
...
\item unitarity non-negativity ($c_{334}^2 \geq 0$, $c_{444}^2 \geq 0$)
  ...
  for $4 \leq N \leq 19$;
\item both falsifiable predictions
...
completed framework of Theorem~\ref{conj:en-koszul-duality}.
...
surface to the stage-$3$ fifteen-coefficient packet and the stage-$4$
six-coefficient packet organized as three local OPE blocks, sharpened
further to the four channels
$c_{334}$, $c_{444}$,
$\mathsf{C}_{3,4;3;0,4}$, $\mathsf{C}_{3,4;4;0,3}$ together with the
...
\item \emph{Finite-dimensionality from holonomicity.}
  ...
  stratum ensures finite-dimensional graded pieces, which is
...
The analogy with power series suggests that the convergence radius of
the categorical logarithm should be computable from the input data.
