...
\subsection{\texorpdfstring{Proof that $d^2 = 0$ - Complete Nine-Term Verification}{Proof that d2 = 0 - Complete Nine-Term Verification}}\label{sec:bar-nilpotency-nine-terms-complete}\label{sec:bar-nilpotency}
...
For $\{d_2, d_3\} = 0$: this is Stokes' theorem on the compactified configuration space $\overline{C}_{n+1}(X)$, viewed as a manifold with corners. The residue operation $d_2$ restricts to boundary components, and $d_3 = d_{\text{dR}}$. Their anticommutation is equivalent to $\partial^2 = 0$ for the topological boundary operator, with signs fixed by Convention~\ref{conv:orientations-enhanced}.
\end{proof}
...
\begin{theorem}[Arnold Relations - Three Formulations; \ClaimStatusProvedHere]\label{thm:arnold-three}
...
\textbf{Remark:} This proof shows that the Arnold relations are a consequence of $\partial^2 = 0$ for configuration spaces.
\end{proof}
...
\begin{corollary}[Cohomology of Configuration Spaces; \ClaimStatusProvedHere]\label{cor:cohomology-config}
...
\begin{proof}
This follows from the theorem of Arnol'd, Cohen, Brieskorn, and others. The generators are the classes $[\eta_{ij}]$ (in degree 1, since $\eta_{ij} = d\log(z_i - z_j)$ are 1-forms), and the relations are precisely the Arnold relations.
...
\begin{computation}[Degree 1 - General Structure; \ClaimStatusProvedHere]\label{comp:deg1-general}
In degree 1:
...
\end{itemize}
\textbf{Warning:} One must \emph{not} multiply the OPE pole by the propagator $\eta_{12} = d\epsilon/\epsilon$ and take the combined residue; the logarithmic form $\eta_{12}$ accounts for the \emph{degree} in the bar complex, not an additional pole (see Theorem~\ref{thm:betagamma-bar-complex} for the careful calculation).
\end{computation}
...
\textbf{Key observation:} These are the same set of ordered triples, in different notation.
...
\begin{proof}
See Theorem~\ref{thm:bar-nilpotency-complete} for the complete nine-term verification.
\end{proof}
...
\begin{definition}[Factorization Differential - CORRECTED with Signs]\label{def:diff-fact}
   ...
   \begin{lemma}[Orientation Convention - RIGOROUS; \ClaimStatusProvedHere]\label{lem:orientation}
...
\textbf{Step 1} (Geometric realization of $\mathcal{P}^{!,c}$).
...
\textbf{Step 2} (Identification of differentials).
...
by integrating the logarithmic form $\omega$ over the fiber of the
forgetful map
$\overline{C}_{n+1}(X) \to X^{n+1}$.
This integration is a chain map by Stokes's theorem
(boundary terms give the residue differential), and it is a
quasi-isomorphism because the fibers of the forgetful map are
contractible (the FM compactification resolves the singularities of
the diagonal without changing the homotopy type of the
complement).
\end{proof}

We now provide the precise residue formula with complete justification:
 
\begin{theorem}[Residue Formula - Complete; \ClaimStatusProvedHere]\label{thm:residue-formula}
...
\begin{theorem}[Uniqueness and Functoriality - Complete; \ClaimStatusProvedHere]\label{thm:bar-uniqueness-functoriality}
...
\textbf{Physical intuition (Witten):}
\begin{itemize}
...
\begin{theorem}[Schwartz Kernel Theorem for Cobar; \ClaimStatusProvedHere]\label{thm:schwartz-kernel-cobar}
...
\begin{proof}
\textbf{Step 1: Identification of the underlying graded object.}
...
\textbf{Step 2: Differential components.}
...
\begin{theorem}[Cobar Differential - Geometric; \ClaimStatusProvedHere]\label{thm:cobar-diff-geom}
...
Arnold-Orlik-Solomon relations from the bar complex (Section~\ref{sec:bar-nilpotency}). The key is that 
...
\textbf{Physical interpretation:} This is the \emph{Koszul duality} of free fermions (not to be confused with the boson-fermion correspondence/bosonization, which identifies $\mathcal{F} \cong V_{\mathbb{Z}}$ as a lattice vertex algebra).
...
of the classical $\text{Sym}(V) \leftrightarrow \Lambda(V^*)$ Koszul duality (see Section~\ref{sec:betagamma-koszul-dual} for the complete bar-cobar computation).
...
\textbf{Example:} For $K_1 = \frac{1}{z_1 - w}$, $K_2 = \frac{1}{w - z_2}$, using partial fractions:
\begin{align*}
...
Under hypothesis (i), the Koszulity condition ensures that $E_1$ degenerates:
...
\subsection{Summary: Enhanced Cobar Construction}

\begin{remark}[Complete Cobar Enhancement]
This patch completes the enhanced treatment of the geometric cobar construction, 
parallel to Section~\ref{sec:bar-nilpotency}'s treatment of the bar construction. We have established:
...
- Fermion-boson Koszul duality as cobar phenomenon
...
\textbf{Step 2: Čech-de Rham comparison.}
...
\begin{proof}
\textbf{Step 1: Topological structure.}
...
\textbf{Step 2: Prescribed singularities.}
...
\textbf{Step 3: Continuity of $d_\Omega$.}
...
\begin{lemma}[Regularization; \ClaimStatusProvedHere]
...
\subsection{\texorpdfstring{Cobar $A_\infty$ Structure}{Cobar A- Structure}}

\begin{theorem}[$A_\infty$ Structure on Cobar; \ClaimStatusProvedHere]\label{thm:cobar-ainfty}
...
\begin{theorem}[Curved Maurer-Cartan; \ClaimStatusProvedHere]\label{thm:curved-mc-cobar}
...
(ii) The curvature $m_0$ is the genus-0 obstruction to $d^2 = 0$; for a chiral algebra with central charge $c$, $m_0 \propto c$ (Theorem~\ref{thm:universal-kac-moody-koszul}), which is the conformal anomaly.
...
\textbf{Step 1: Genus-1 bar complex in low degrees.}
...
\textbf{Step 2: The genus-1 cocycle.}
...
\]
The first term is the genus-0 collision residue.  The second term is new at genus~1: using $d\eta_{12}^{(1)} = 2\pi i\, \omega_\tau$ (the normalized holomorphic 1-form on $E_\tau$), we get
\[
...
\textbf{Step 3: Cobar complex computation.}
...
\begin{proof}[Proof sketch]
The deformation theory of chiral algebras on families of curves gives a long exact sequence
(see Chapter~\ref{chap:higher-genus} for the full development):
\[
0 \to H^1(\Sigma_g, \mathcal{A}) \to \text{Ext}_{\Sigma_g}(\mathcal{A}) \to H^2(\mathcal{M}_g, \mathbb{C}) \to \text{Obs}_g(\mathcal{A}) \to 0
\]

The obstruction lives in $H^2(\mathcal{M}_g, \mathrm{End}(\mathcal{A}))$ with local system coefficients. For specific classes of chiral algebras:
\begin{enumerate}
\item For the matter-ghost BRST system in bosonic string theory, the obstruction vanishes when $c_{\text{matter}} = 26$ (cancellation with ghost central charge $c_{\text{ghost}} = -26$)
\item For a general chiral algebra $\mathcal{A}$, the obstruction depends on the cohomology class $[\omega_g] \in H^2(\mathcal{M}_g, Z(\mathcal{A}))$, where $Z(\mathcal{A})$ is the center
\item Free theories (e.g., Heisenberg at any level, free fermions with spin structure) extend to all genera without central charge restrictions
\end{enumerate}
\end{proof}
...
\begin{theorem}[BRST Cohomology Realization; \ClaimStatusProvedHere]\label{thm:brst-cohomology}
...
\begin{theorem}[Anomaly Cancellation for Matter-Ghost Systems; \ClaimStatusProvedHere]\label{thm:anomaly-cancellation}
...
\begin{enumerate}
\item \textbf{Central Charge Constraint:} The \emph{BRST} differential of the combined matter-ghost system satisfies $d_{\text{BRST}}^2 = 0$ if and only if $c_{\text{matter}} = 26$ (bosonic) or $c_{\text{matter}} = 15$ (superstring).

\item \textbf{Modular Invariance:} The bar complex of the total system transforms covariantly under $\operatorname{SL}_2(\mathbb{Z})$ if and only if the anomaly polynomial vanishes.

\item \textbf{Geometric Interpretation:} The anomaly corresponds to the obstruction to extending the bar complex of the total system to higher genus.
\end{enumerate}
...
\textbf{Step 1: Individual Nilpotency.} On any chiral algebra, the bar differential satisfies $d_{\text{bar}}^2 = 0$ by Theorem~\ref{thm:bar-nilpotency-complete}.

\textbf{Step 2: BRST Coupling.} The total BRST differential $d_{\text{BRST}} = d_{\text{matter}} + d_{\text{ghost}} + d_{\text{coupling}}$ includes a coupling term from the matter-ghost interaction. The cross-terms yield:
$$d_{\text{BRST}}^2 = \frac{c_{\text{matter}} + c_{\text{ghost}}}{24} \cdot \chi(\overline{C}_n(X))$$
where $\chi$ is the Euler characteristic.

\textbf{Step 3: Cancellation.} The anomaly vanishes precisely when $c_{\text{matter}} + c_{\text{ghost}} = 0$. Since $c_{\text{ghost}} = -26$ (bosonic) or $c_{\text{ghost}} = -15$ (superstring), this forces $c_{\text{matter}} = 26$ or $c_{\text{matter}} = 15$ respectively.
\end{proof}
...
\item \textbf{Koszul duality:} $\mathcal{H}_k^! \simeq \mathrm{CE}(\hat{\mathfrak{h}}_k)$ (the Chevalley--Eilenberg DG chiral algebra, a fermionic system)
\item \textbf{Key point:} The cobar of $\bar{B}(\mathcal{H}_k)$ gives back $\mathcal{H}_k$ (bar-cobar inversion), but the Koszul dual $\mathrm{CE}(\hat{\mathfrak{h}}_k)$ is a \emph{different type} of algebra (Lie vs.\ commutative); these are distinct statements.
\end{itemize}
...
\textit{(Note: One must use the local system $\mathcal{Z}(\mathcal{A})$, not the trivial local system.  By Harer's theorem, $H^1(\mathcal{M}_g, \mathbb{Q}) = 0$ for $g \geq 2$, so the deformation space would be trivially zero with trivial coefficients.)}
\end{proof}
...
\textbf{Genus 1:} The moduli space $\mathcal{M}_1$ has $H^1(\mathcal{M}_1, \mathbb{Q}) = 0$ (as for all $\mathcal{M}_g$), but $\dim \mathcal{M}_1 = 1$.  Quantum corrections enter through the \emph{partition function}, which depends on the modulus $\tau \in \mathfrak{h}/\mathrm{SL}_2(\mathbb{Z})$:
...
\textbf{Genus 2:} $\dim \mathcal{M}_2 = 3$ and $H^2(\mathcal{M}_2, \mathbb{Q}) = \mathbb{Q} \cdot \lambda_1$ (generated by the Hodge class). For $\widehat{\mathfrak{sl}}_2$ at
...
\begin{enumerate}[label=(\roman*)]
\item The \textbf{curvature} of a curved $A_\infty$ algebra is the element $m_0 := \mu_0(1) \in A^2$ (degree $+2$ in our cohomological convention).
\item The \textbf{failure of $\mu_1$ to square to zero} is controlled by curvature: $\mu_1^2(a) = \mu_2(m_0, a) - \mu_2(a, m_0)$ for all $a \in A$. The total bar differential $d_{\mathrm{bar}}$ always satisfies $d_{\mathrm{bar}}^2 = 0$; the curvature manifests in the \emph{internal} $A_\infty$ structure, not in the nilpotence of $d_{\mathrm{bar}}$.
\item In the physics literature, \textbf{backreaction} refers to the same phenomenon: the classical BRST operator $Q$ (identified with $\mu_1$) is deformed by quantum corrections (anomalies, central extensions) so that $Q^2 \neq 0$, with the obstruction measured by the curvature class.
\item The curvature $m_0$ defines a class $[m_0] \in H^2(A, \mu_1)$. A curved $A_\infty$ algebra is \textbf{strictifiable} (equivalent to one with $\mu_1^2 = 0$) if and only if $m_0$ is central in the sense of Theorem~\ref{thm:curvature-central} below and the curvature class is gauge-trivial in the completed filtration.
\end{enumerate}
\end{remark}
...
operations are no longer finite on the nose and convergence requires completion,
...
By the $\mathfrak{sl}_2$ specialization of W-algebra Koszul duality (Theorem~\ref{thm:w-algebra-koszul-main}), the completed Koszul dual of Virasoro with central charge $c$ is:
\begin{equation}
\text{Vir}_c^! \cong \text{Vir}_{26-c}
\end{equation}
Indeed, $\mathrm{Vir}_c = \mathcal{W}^k(\mathfrak{sl}_2)$ with $h^\vee = 2$, so the dual level $k' = -k-4$ satisfies $c(k) + c(k') = 26$ (Theorem~\ref{thm:w-algebra-koszul-main}(iii)). The physical interpretation is matter-ghost duality in bosonic string theory: a matter system at central charge $c$ couples to a ghost system at $c_{\text{ghost}} = 26 - c$.

\textbf{Consistency check}: For $c=26$, this predicts $\text{Vir}_{26}^! \cong \text{Vir}_0$, which is compatible with the BRST cohomology of the bosonic string at zero ghost number.
\end{example}
...
\section{\texorpdfstring{Curved $A_\infty$ Structures: On-Nose vs.\ Homotopy Nilpotence}{Curved A-infinity Structures: On-Nose vs Homotopy Nilpotence}}
...
\subsubsection{\texorpdfstring{Regime I: Strict Differential ($d^2 = 0$ on the nose)}{Regime I: Strict Differential (d2 = 0 on the nose)}}
...
\textbf{No homotopy is involved.}
\end{example}

\subsubsection{\texorpdfstring{Regime II: Curved Differential ($d^2 = \mu_0 \cdot \text{id}$, central curvature)}{Regime II: Curved Differential (d2 = -0 id, central curvature)}}
...
\begin{theorem}[Centrality Implies On-Nose Nilpotence; \ClaimStatusProvedHere]\label{thm:central-implies-strict}
...
\textbf{Terms 1+5+6 combined: $d_{\text{internal}}^2 + d_{\text{internal}} d_{\text{correction}} + d_{\text{correction}} d_{\text{internal}} = 0$}
...
\end{aligned}
\end{equation}
By the curved $A_\infty$ relation~\eqref{eq:curved-m1-squared}: $m_1^2(a_i) = m_2(\mu_0, a_i) - m_2(a_i, \mu_0)$. Since $\mu_0$ is a \textbf{cycle} ($m_1(\mu_0) = 0$), the combined contribution is:
...
\textbf{central curvature}, so we obtain strict nilpotence.  See
...
and Theorem~\ref{thm:central-implies-strict} yields $d_{\mathrm{bar}}^2=0$ on the nose.
...
\textbf{We do not need this level of generality for the chiral algebras studied in this monograph,} since their curvature elements are central (see Remark~\ref{rem:voa-central-curvature} below).
...
\item \textbf{Central:} $K$ is central by definition (it is the generator of the one-dimensional center of $\widehat{\mathfrak{g}}_k$)
\end{itemize}
...
\textbf{Step 1: From connection to bar element.}
...
\textbf{Step 2: MC equation $=$ flatness.}
...
the bar differential satisfies $d_0^2 = 0$ on the nose.
\end{theorem}
...
$d_{\mathrm{full}}^2 = 0$ on the nose.
\end{theorem}
...
\medskip
\textbf{Step 1: The modular operad boundary relation.}
...
\medskip
\textbf{Step 2: Genus-$0$ nilpotence ($d_0^2 = 0$).}
...
\medskip
\textbf{Step 3: Cross-terms ($d_0 d_g + d_g d_0 = 0$).}
...
\medskip
\textbf{Step 4: Pure higher-genus terms ($\sum_{g_1 + g_2 = g} d_{g_1} d_{g_2} = 0$).}
...
\end{itemize}
$d_{\text{bar}}^2 = 0$ ON THE NOSE (Theorem~\ref{thm:genus-induction-strict}) \\
\hline
...
\textbf{Good news:} Vertex operator algebras (with the standard vacuum axiom) and
chiral algebras arising from unitary CFT have \textbf{central curvature}
(Remark~\ref{rem:voa-central-curvature}), so we are in the on-nose regime.
\end{remark}
...
The key point is that $d^2 = 0$ holds \emph{strictly} at each genus (not merely
up to $\hbar$-corrections), so each $Z_g$ is an honest finite-dimensional vector
space, not a formal deformation.  This is the on-nose nilpotence established in
Section~\ref{sec:on-nose-vs-homotopy}.
\end{proof}
...
This is our on-nose nilpotence $d^2 = 0$. The Arnold relations are the
...
The pattern established by these five cases motivates the general theorem.
...
\begin{theorem}[Functoriality of Bar Construction - Grothendieck Style; \ClaimStatusProvedHere]\label{thm:bar-functorial-grothendieck}
...
\textbf{$d^2 = 0$ from the adjunction.}
...
conformal field theories whose anomalies are central, at genus~$0$:
\begin{equation}
...
This holds because:
\begin{enumerate}
\item The curvature $\mu_0$ is proportional to the vacuum $\mathbf{1}$
\item The vacuum axiom forces $\mathbf{1} \in Z(\mathcal{A})$, hence $\mu_0 \in Z(\mathcal{A})$
      (Remark~\ref{rem:voa-central-curvature})
\item Arnold relations ensure residue nilpotence ($d_{\mathrm{residue}}^2 = 0$)
\item The Leibniz rule ensures $d_{\mathrm{internal}}$ and $d_{\mathrm{residue}}$ anticommute
\end{enumerate}

\textbf{All genera:} Strict nilpotence holds at all genera for chiral algebras
with central curvature (Theorem~\ref{thm:genus-induction-strict}), via the
modular operad boundary relations on $\overline{\mathcal{M}}_{g,n}$.

\textbf{Scope:} This covers all standard examples (Heisenberg,
Kac--Moody, Virasoro, $W$-algebras, free field systems). For non-unital chiral
algebras or theories with non-central anomalies, $d^2 = 0$ may hold only up
to homotopy.

\textbf{Practical consequence:} We can compute Koszul duals directly
using the bar construction at any genus, without needing to resolve homotopy
coherence issues or invoke $\infty$-categorical machinery.

\textbf{Physical interpretation:} In 2D CFT, associativity of the OPE reflects locality of the
field theory, and quantum corrections (central charges) modify the overall normalization without
breaking this locality.
}}
\end{center}
...
$$\mathcal{H}_k^! = \text{CE}(\mathfrak{h}_{-k}) = V^{\text{CE}}(\mathfrak{h}_{-k}) \quad \text{(Chevalley-Eilenberg DG chiral algebra at negated level)}$$
...
\item \textbf{Curvature}: $m_0 = -k \cdot c$ (level negates; $h^\vee = 0$ for Heisenberg)
...
\hline
\textbf{Algebra} & \textbf{OPE Pole} & \textbf{Bar Differential} & \textbf{Effect} \\
\hline
...
\textbf{No completion is needed.} The bar complex is conilpotent: finite-dimensional at each degree 
...
where $\Lambda(L \cdot L)$ is a composite operator, not a single generator.
...
\textbf{Completion is essential.} The bar construction must be completed:
...
\item \textbf{Separated}: $\bigcap_{k=0}^\infty F^k\mathcal{C} = 0$
...
\begin{proposition}[Filtered $\Rightarrow$ Curved; \ClaimStatusProvedHere]\label{prop:filtered-to-curved}
...
\item Its Koszul dual $\text{CE}(\mathfrak{h}_{-k})$ describes the \emph{ghost system} for gauging the $U(1)$ symmetry
...
\textbf{No completion is needed.} The bar complex is finite-dimensional at each degree and 
...
\textbf{Completion is essential.} Without completion, the bar complex does not converge and the
...
\textbf{Completion is essential.} Without it, the bar construction does not converge.
\end{example}
...
\item \textbf{Growth bound:}
$\limsup_{n\to\infty}\dim(\bar{B}^n(\mathcal{A}))^{1/n}<\infty$.
\item \textbf{Filtration compatibility:}
...
\begin{enumerate}[label=(D\arabic*)]
\item \textbf{Morphism of chiral algebras.}
...
\item \textbf{Genuswise quasi-isomorphism.}
The genus-$g$ components $\psi_g$ are quasi-isomorphisms by
Theorem~\ref{thm:genus-graded-convergence}, item (2), together with the explicit
higher-genus inversion package referenced there (Theorem~\ref{thm:higher-genus-inversion}).

\item \textbf{Full genus-graded convergence and quasi-isomorphism.}
...
\item \textbf{Spectral sequence statement.}
...
\textbf{Chain complex quasi-isomorphism:} A chain map $\psi: C^\bullet \to D^\bullet$ inducing an isomorphism $H^\bullet(\psi): H^\bullet(C) \xrightarrow{\cong} H^\bullet(D)$ on cohomology. This is the standard notion; a ``homology isomorphism'' and a ``quasi-isomorphism'' are the \emph{same thing} at the chain complex level.

\textbf{Chiral algebra quasi-isomorphism:} An $A_\infty$ (or chiral algebra) morphism $\psi: \mathcal{A} \to \mathcal{B}$ whose linear component $\psi_1: \mathcal{A} \to \mathcal{B}$ is a chain complex quasi-isomorphism. Such a map preserves the full $A_\infty$ structure up to coherent homotopy.
...
\textbf{Example where the distinction is visible:}
...
\textbf{Growth estimate:} By Mirzakhani's results, Weil--Petersson volumes satisfy
...
\textbf{$\hbar$-adic completion:} For rigorous formal computations, work in:
