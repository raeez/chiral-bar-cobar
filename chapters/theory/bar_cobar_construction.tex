...
The Heisenberg bar differential vanishes at degree~$1$ (Example~\ref{ex:heisenberg-d-deg1}): $d_{\text{res}}(J \otimes J \otimes \eta_{12}) = 0$ because the double pole produces a triple-pole residue.  The class $[J \otimes J \otimes \eta_{12}] \in H^1(\bar{B}^{\bullet}(\mathcal{H}))$ is non-trivial; the central charge $k$ appears only at genus~$1$.
\end{example}
...
The cobar construction is the Verdier dual of the bar: where the bar uses logarithmic forms on $\overline{C}_n(X)$ with residue differential, the cobar uses distributional sections on $C_n(X)$ with delta-function insertions.
...
The constructions of the preceding sections --- bar, cobar, their
Verdier pairing, and the genus-$1$ curvature --- are now assembled
into a single statement.  For a chiral Koszul pair $(\cA_1,
\cA_2)$, the bar of one is the Koszul dual coalgebra of the other,
and the Verdier identification on $\operatorname{Ran}(X)$ provides
the equivalence.  This is the \emph{content} of Theorem~A
(Theorem~\ref{thm:bar-cobar-isomorphism-main}): not merely that bar
and cobar are quasi-inverse, but that their quasi-inverseness is
governed by Verdier duality on configuration spaces.  The Koszul
property --- diagonal Ext vanishing in the (bar degree, weight)
bigrading --- is the condition under which this Verdier equivalence
descends from the coderived category to the ordinary derived
category.
...
The bar-cobar theory developed so far assumes quadratic
presentations.  But the most interesting chiral algebras ---
$\mathcal{W}$-algebras, non-simply-laced Kac--Moody algebras,
the Virasoro algebra at irrational central charge --- are not
quadratic, and the question of when (and in what sense) Koszul
duality survives is both technically and conceptually central.
Four increasingly general classes arise, forming a hierarchy
\begin{equation}
...
A morphism of chiral CDG-coalgebras
$(C, d_C, h_C) \to (D, d_D, h_D)$ is a pair
$(f, a)$ where $f\colon C \to D$ is a morphism of
graded chiral coalgebras and $a\colon C \to \omega_X$
...
With the coderived and contraderived categories in place, the bar-cobar adjunction can be inverted: the cobar of the bar recovers the original algebra up to quasi-isomorphism.
