...
\emph{The $-1$-eigenspace is $Q_g(\mathcal{A}^!)$.}
By Lemma~\ref{lem:deformation-space}, the deformation space
$\mathrm{Def}_g(\mathcal{A}^!)$ embeds in
$H^*(\overline{\mathcal{M}}_g, \mathcal{Z}(\mathcal{A}))$.
Since Koszul duality negates the Verdier involution
($\sigma_{\mathcal{A}^!} = -\sigma_{\mathcal{A}}$ on centers,
by Theorem~\ref{thm:verdier-bar-cobar}), deformation classes
lie in the $-1$-eigenspace:
$Q_g(\mathcal{A}^!) = \ker(\sigma^* + \mathrm{id})$.
...
completed framework of Conjecture~\ref{conj:en-koszul-duality}.
...
A morphism of chiral CDG-coalgebras $(C, d_C, h_C) \to (D, d_D, h_D)$
is a pair $(f, a)$ where $f\colon C \to D$ is a morphism of graded
chiral coalgebras and $a\colon C \to \omega_X$ is a homogeneous
linear function of degree~$1$ such that
$d_D(f(c)) = f(d_C(c)) + f(a \ast c) - (-1)^{|c|}f(c \ast a)$ and
$h_D(f(c)) = h_C(c) + a(d_C(c)) + a^2(c)$ for all $c \in C$.
Composition of morphisms is defined by $(g, b) \circ (f, a) = (g \circ f, b \circ f + a)$.
...
\emph{Step~1: Definition of derived functors.}
By Proposition~\ref{prop:chiral-inj-proj-resolutions}(c), the
contraderived category $D^{\mathrm{ctr}}(C\text{-}\mathrm{contra}^{\mathrm{ch}})$
is equivalent to the homotopy category of CDG-contramodules with
projective underlying graded contramodule, and the coderived category
$D^{\mathrm{co}}(C\text{-}\mathrm{comod}^{\mathrm{ch}})$ is equivalent
to the homotopy category of CDG-comodules with injective underlying
graded comodule.  The left derived functor
$\mathbb{L}\Phi_C^{\mathrm{ch}}$ is obtained by restricting
$\Phi_C^{\mathrm{ch}}$ to projective contramodules (and composing
with the localization), and $\mathbb{R}\Psi_C^{\mathrm{ch}}$ by
restricting $\Psi_C^{\mathrm{ch}}$ to injective comodules.
