...
curve~$X$ is a non-abelian integral transform.  Its input is the
chiral algebra (encoded by OPE singularities); its kernel is the
propagator $\eta_{ij}$; its output is a factorization coalgebra on
$\operatorname{Ran}(X)$.  Verdier duality inverts the transform:
the cobar construction $\Omega(\barB(\cA))$ reconstructs~$\cA$ from
its transform, just as the inverse Fourier transform reconstructs a
function from its spectrum.  For the Heisenberg algebra, this
...
At this stage, $d^2 = 0$ exactly (Arnold relations), and
the diagram commutes strictly.  At genus~$g \geq 1$
(Chapter~\ref{chap:higher-genus}), the square acquires
curvature~$\kappa(\cA) \cdot \omega_1$, and strict commutativity
...
associahedra to the geometry of the Deligne--Mumford compactification.
