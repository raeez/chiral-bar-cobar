...
The bar construction is the point where three mathematical
traditions converge.  In algebra, bar-cobar is an adjunction on
dg-categories whose unit and counit measure Koszulity.  In geometry,
the Fulton--MacPherson compactification $\overline{C}_n(X)$ encodes
all collision patterns of $n$ points on a curve, and its boundary
divisors carry the same combinatorics as the faces of associahedra.
In analysis, the logarithmic form $\eta_{ij} = d\log(z_i - z_j)$ is
the unique propagator satisfying conformal invariance, well-defined
residues, and the Arnold relations that force $d^2 = 0$.

The synthesis is this: the bar differential on a chiral algebra $\cA$
\emph{is} the residue map on $\overline{C}_n(X)$.  Collision of
points $z_i \to z_j$ extracts the OPE coefficient; the Arnold
relation $\eta_{ij} \wedge \eta_{jk} + \eta_{jk} \wedge \eta_{ki}
+ \eta_{ki} \wedge \eta_{ij} = 0$ translates into $d^2 = 0$; and
Verdier duality on $\operatorname{Ran}(X)$ exchanges bar and cobar.
For Koszul algebras the adjunction becomes an equivalence ---
Theorem~A (Theorem~\ref{thm:bar-cobar-isomorphism-main}).
Every subsequent result in this monograph --- the genus tower
(Chapter~\ref{chap:higher-genus}), the Lagrangian complementarity
of quantum corrections
(Theorem~\ref{thm:quantum-complementarity-main}), the discriminant
and $\hat{A}$-genus invariants of Part~II --- rests on the
...
This is the Fulton--MacPherson compactification (see Chapter~\ref{chap:config-spaces}). It parametrizes $(n+1)$ points on $\Sigma_g$, with smooth compactification encoding collision patterns.
...
\begin{itemize}
\item Borcherds identity for the chiral product ($d_{\text{residue}}^2 = 0$); the Lie-bracket Jacobi identity alone is insufficient---the full OPE associativity is required (Proposition~\ref{prop:pole-decomposition})
...
\begin{remark}[What we have proved]\label{rem:d-squared-synthesis}
The identity $d^2 = 0$ is the engine driving every subsequent construction in this monograph.  It ensures that the bar complex $\bar{B}^{\mathrm{ch}}(\cA)$ is a genuine cochain complex, and hence that bar cohomology, the bar-cobar adjunction, and the spectral sequences that follow are all well-defined.  The reader will recall that for the Heisenberg algebra (Chapter~\ref{ch:heisenberg-frame}, \S\ref{sec:frame-bar-deg2}), the identity $d^2 = 0$ reduced to the Arnold relation $\eta_{12} \wedge \eta_{23} + \eta_{23} \wedge \eta_{31} + \eta_{31} \wedge \eta_{12} = 0$, which we verified by direct computation.  The general proof required verifying the vanishing of six cross-terms, each drawing on a different mathematical mechanism---the Borcherds identity (algebra), Stokes' theorem (topology), and the residue exact sequence (analysis).  That these mechanisms conspire to produce $d^2 = 0$ is not coincidental; it reflects the factorization structure of the compactification $\overline{C}_n(X)$, as we now explain.
\end{remark}
...
The following low-degree computations serve two purposes: they verify the general theory by explicit calculation, and they reveal the pattern by which the bar differential extracts OPE data from the chiral algebra.  At degree~$1$, the differential recovers the residue of the OPE (the Lie bracket); at degree~$2$, it produces the curvature (the double-pole coefficient).  This pattern---simple poles give brackets, double poles give curvature---generalizes to all degrees and underlies the pole decomposition of Proposition~\ref{prop:pole-decomposition}.
...
&= (a_0 \otimes a_1 \otimes a_2 \otimes \omega_{012})
    \otimes (1 \otimes \omega_{\emptyset}) \\
&\quad + (a_0 \otimes a_1 \otimes \omega_{01})
    \otimes (a_2 \otimes \omega_2) \\
&\quad + (a_0 \otimes a_2 \otimes \omega_{02})
    \otimes (a_1 \otimes \omega_1) \\
&\quad + (a_0 \otimes \omega_0)
    \otimes (a_1 \otimes a_2 \otimes \omega_{12}) \\
&\quad + \text{(other terms)}
\end{align*}
where the summands correspond to partitions $I \sqcup J = \{0,1,2\}$ with $0 \in I$.
...
The bar construction extracts coalgebraic data from a chiral
algebra by taking residues at collision divisors.  But residues are
only half of a duality: on any smooth compactification, a
logarithmic form and a distributional current pair to a number
via integration.  The cobar construction is the other half.
Where the bar sees logarithmic forms on the compactified
configuration space $\overline{C}_n(X)$, the cobar sees
distributional sections on the open $C_n(X)$.  Where the bar
differential is a residue, the cobar differential is an insertion
of a delta function at a diagonal.  The two are related by Verdier
duality --- and it is this duality, not the individual
constructions, that carries the structural content.  Koszul duality
for chiral algebras is the statement that Verdier duality on
$\operatorname{Ran}(X)$ restricts to an equivalence between bar
and cobar.
...
We arrive at the structural heart of the chapter.  The bar and
cobar constructions were built from different analytic data ---
logarithmic forms vs.\ distributions, compactified vs.\ open
configuration spaces --- but Verdier duality reveals them as two
presentations of a single object.  The perfect pairing below is
not merely a computational tool; it is the genus-$0$ manifestation
of the Ran-space duality $\mathbb{D}_{\operatorname{Ran}}
\bar{B}_X(\cA) \simeq \bar{B}_X(\cA^!)$ that will organize the
entire monograph.
...
For the Heisenberg algebra, the bar construction produced the coLie coalgebra $\mathrm{coLie}^{\mathrm{ch}}(V^*)$ and the cobar produced $\mathrm{Sym}^{\mathrm{ch}}(V^*)$ (\S\ref{sec:frame-koszul-dual}): Verdier duality exchanged Lie-type and Com-type.  The following theorem is the general mechanism.
...
The curvature $m_0 = -k \cdot \omega$ that appeared in the Heisenberg Koszul dual (\S\ref{sec:frame-koszul-dual}) is the simplest instance of the following construction.
...
We develop this concretely: an intuitive picture via Feynman
diagrams, explicit chain-level formulas on the torus, and formal
computations through degree~$5$.
...
\end{corollary}
\begin{proof}
Immediate from the bar-cobar adjunction (Theorem~\ref{thm:bar-cobar-adjunction}): $\mathcal{C}$ is in the essential image of $\bar{B}^{\mathrm{geom}}$ if and only if the counit $\bar{B}(\Omega(\mathcal{C})) \to \mathcal{C}$ is a quasi-isomorphism, which holds precisely when $\Omega(\mathcal{C})$ has strict (not homotopy) chiral algebra structure.
\end{proof}
...
We establish the result through five lemmas below.  The complete proof,
synthesizing these via the Verdier involution~$\sigma$ and the genus
spectral sequence, is given in
...
bar complex carries curvature $h = m_0^{(g)}$ satisfying $d_g^2 = h \ast (-)$.
...
and the class $[d_g^2] = [m_0^{(g)} \ast (-)]$ is the obstruction to
...
\begin{proof}
The bar differential at genus~$g$ is
$d_g = \sum_{D \subset \partial \overline{C}_n(\Sigma_g)} \mathrm{Res}_D$,
constructed from logarithmic propagators
$\omega_{ij} = d\log(z_i - z_j)$ on configuration spaces
(Definition~\ref{def:bar-differential-complete}).  We compute $d_g^2$ at the
cochain level.

\emph{Step~1: Double residue as fiber $2$-form.}
For $\phi \in \bar{B}^n_g(\mathcal{A})$, the composition $d_g^2(\phi)$
...
$[d_g^2] = [m_0^{(g)} \ast (-)]$ lies in the $\sigma$-invariant part
...
$d_g^2 = [\mu_0, -] \otimes \int_{\mathcal{M}_g} \omega_g^2$ follows
from expanding
$(d_0 + \mu_0 \otimes \omega_g)^2
= d_0^2 + d_0(\mu_0 \otimes \omega_g)
+ (\mu_0 \otimes \omega_g)d_0 + \mu_0^2 \otimes \omega_g^2$
and using $d_0^2 = 0$, $d_0 \mu_0 = 0$ (Maurer--Cartan), and
...
Recall that for the Heisenberg algebra on an elliptic curve (\S\ref{sec:frame-genus1}), the curvature $d^2 = k \cdot \omega_1$ was a \emph{scalar}, hence automatically central.  The following theorem shows that centrality of the curvature is the general mechanism ensuring that the bar complex remains a genuine cochain complex ($d^2 = 0$ strictly) even in the curved setting.
...
DG chiral coalgebra ($d^2 = 0$); at genus~$g \geq 1$ with
$\kappa(\cA) \neq 0$, it is a \emph{curved} DG chiral coalgebra
($d^2 = m_0^{(g)} \neq 0$).  In the curved case, the ordinary
derived category is trivial
(Proposition~\ref{prop:curved-bar-acyclicity}), and a richer
homological algebra is needed.

This section develops the chiral analogues of Positselski's
coalgebra homological algebra~\cite{Positselski}, culminating in the
\emph{chiral comodule-contramodule correspondence}
(Theorem~\ref{thm:chiral-co-contra-correspondence}) and the
\emph{full derived module equivalence}
(Theorem~\ref{thm:full-derived-module-equiv-proved}).  The strategy
is to replace Positselski's ground field~$k$ by the sheaf of
differential operators $\mathcal{D}_X$, the ordinary tensor product
by the chiral tensor product $\chirtensor$, and to verify that all
exactness properties are inherited from the geometry of
...
\begin{remark}[Morphisms of chiral CDG-coalgebras]
...
We now apply the chiral comodule-contramodule correspondence to
prove the derived module category equivalence at all genera.
...
differential satisfying $d^2 = m_0^{(g)} \ast (-)$.  This is
...
For the Heisenberg algebra, we verified directly that $\Omega^{\mathrm{ch}}(\bar{B}^{\mathrm{ch}}(\mathcal{H}_k)) \xrightarrow{\sim} \mathcal{H}_k$ (\S\ref{sec:frame-inversion}): the cobar of the bar recovered the original algebra.  The following theorem establishes this inversion for all Koszul chiral algebras.
...
We conclude this chapter by placing the explicit chain-level
constructions into the $\infty$-categorical framework of factorization
algebras on $\operatorname{Ran}(X)$.  This provides the correct
conceptual home for the bar-cobar adjunction and clarifies the role of
Verdier duality.
...
$d^2 = \kappa \cdot \omega_1 \cdot \mathrm{id}$ rather than $d^2 = 0$.
