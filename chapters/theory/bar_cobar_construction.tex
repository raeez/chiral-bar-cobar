...
\begin{theorem}[BRST cohomology realization; \ClaimStatusHeuristic]\label{thm:brst-cohomology}
...
\begin{theorem}[Anomaly cancellation for matter-ghost systems; \ClaimStatusHeuristic]\label{thm:anomaly-cancellation}
...
\begin{proof}[Proof sketch (full proof in \S\textup{\ref{sec:complementarity-theorem}})]
We outline the argument; the complete proof via the Verdier involution
$\sigma$ and its eigenspace decomposition is given in
...
higher genus (Theorem~\ref{conj:positselski-chiral}) requires
...
$\pi_P\colon \mathrm{Hom}^{\mathrm{ch}}(C, P) \to P$ satisfying the
contraassociativity and contraunit axioms) endowed with a
...
Corollary~\ref{thm:derived-equivalence-koszul}.  Off this locus, the
