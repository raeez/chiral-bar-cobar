...
\texorpdfstring{$W_\infty$}{W_infty}; \ClaimStatusConjectured]
...
\end{enumerate}
Thus the live $\mathcal{W}_\infty$ frontier is not further stabilization
of the standard tower.  It is the construction of the filtered
factorization target itself.
\end{conjecture}
...
$W^{(3)}W^{(4)}$, then match the mixed transport of that same new
generator through $W^{(3)}W^{(5)}$ and $W^{(4)}W^{(5)}$, and only then move to
