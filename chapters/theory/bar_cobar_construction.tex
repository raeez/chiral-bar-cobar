...
Assuming the additional stage-$4$ Ward-normalized input, apply
Proposition~\ref{prop:winfty-formal-mixed-virasoro-zero} to the
stage-$4$ residue calculus to obtain
\[
\mathsf{C}^{\mathrm{res}}_{3,4;2;0,5}(4)=0,
\]
and applying
...
acts on them by the standard Ward identity.  Then the only additional
inputs from the normalized residue two-point/Ward package actually used
to deduce
\[
\mathsf{C}^{\mathrm{res}}_{3,4;2;0,5}(4)=0,
\qquad
\mathsf{C}^{\mathrm{res}}_{4,4;2;0,6}(4)=2
\]
is the diagonal visible normalization
\[
\langle W^{(s)}(z)W^{(s)}(0)\rangle=\frac{c/s}{z^{2s}}
\qquad (s=2,3,4),
\]
...
exactly inheritance of this diagonal normalization.
\end{proposition}
...
\mathsf{C}^{\mathrm{res}}_{4,4;2;0,6}(4)=2
\]
to the normalized visible two-point functions, again with the same
stress-tensor normalization.

But the $s=2$ case of the displayed diagonal normalization is exactly
the stress-tensor
normalization
\[
\langle T(z)T(0)\rangle=\frac{c/2}{z^4}.
\]
Therefore, once the visible Ward action is fixed, the only additional
input used by the formal reduction lemmas is the displayed diagonal
normalization.
\end{proof}
...
visible generators are then mixed-weight orthogonal as well, so the
full visible pairing package required by
...
diagonal visible pairings inside that
quotient residue calculus.  Mixed-weight orthogonality is already
forced by the Virasoro Ward identity, by
...
\begin{conjecture}[Stage-$4$ visible diagonal normalization]
...
\item the Virasoro field $T:=W^{(2)}$ acts on these visible generators
  by the standard stage-$4$ Virasoro Ward identity.
\end{enumerate}
Then the diagonal visible pairings are normalized by
\[
\langle W^{(s)}(z)W^{(s)}(0)\rangle = \frac{c/s}{z^{2s}}
\qquad (s=2,3,4)
\]
with the same nonzero central charge $c$ as on the principal
Drinfeld--Sokolov stage.
\end{conjecture}
...
the visible diagonal normalization of
...
diagonal normalization is forced as well, then
...
packet $\mathcal{J}_5^{\mathrm{entry}}$, whose source pairs are already
isolated as singletons.  The remaining six-channel packet
