...
The three-step bar differential $d = d_{\mathrm{internal}} + d_{\mathrm{residue}} + d_{\mathrm{de\,Rham}}$ that we built by hand for the Heisenberg algebra in Chapter~\ref{ch:heisenberg-frame} now receives its general formulation. For an arbitrary chiral algebra $\cA$ on a smooth curve $X$, the bar complex $\barB(\cA)$ is a cosheaf on the Ran space of $X$ whose sections on $\overline{C}_n(X)$ are computed by the same residue-extraction procedure, applied to the full OPE data of $\cA$.

\noindent\emph{Governing question.} How much of bar-cobar duality is a
strict genus-$0$ theorem and where does genuinely modular curvature
enter?  This chapter proves the strict genus-$0$ bar/cobar machine and
its Verdier compatibility; the curved genus-$g$ deformation is deferred
to Chapter~\ref{chap:higher-genus}.
...
The bar and cobar functors are constructed at \textbf{M-level} as explicit dg objects on configuration spaces, with differentials decomposed into internal, residue, and de~Rham components.  The main theorem (Theorem~\ref{thm:bar-cobar-isomorphism-main}) is \emph{stated} at H-level (adjoint equivalence in factorization $\infty$-categories) and \emph{proved} at M-level.  Bar cohomology dimensions and Hilbert series are S-level shadows.
\end{remark}
...
\begin{conjecture}[Cobar elements = on-shell propagators; \ClaimStatusHeuristic]\label{conj:cobar-physical}
...
The conjecture identifies cobar complex elements with QFT on-shell propagators and the $A_\infty$ operations with Feynman rules at genus zero.
\end{remark}
...
\begin{remark}[Criteria for Koszul pairs --- recollection]\label{def:koszul-criteria}
\label{def:koszul-chiral}
\index{Koszul duality!chiral}
\index{Koszul property}
Recall (Definition~\ref{def:chiral-koszul-pair}) that two chiral algebras $(\mathcal{A}_1, \mathcal{A}_2)$ form a \emph{chiral Koszul pair} if and only if:
\begin{enumerate}
...
\begin{example}[$W_\infty$: completion frontier]
...
\emph{The precise frontier statement} is an inverse-limit bar theorem:
one seeks a separated complete weight filtration with finite-type
truncations $W_{\infty,\le N}$ and a completed bar object
\[
\widehat{\bar B}(W_\infty) := \varprojlim_N \bar B(W_{\infty,\le N})
\]
whose cobar recovers the completed algebra.

\emph{Obstructions still to be overcome}: 
\begin{enumerate}
\item The bar complex $\bar{B}(W_\infty)$ is infinitely generated in each degree
\item no theorem in the manuscript yet shows that the completed object
  $\widehat{\bar{B}}(W_\infty)$ carries the full M-level bar-cobar package
  beyond the standard principal-stage tower
\item for the standard principal-stage tower, conformal-weight cutoff now
  makes the finite-stage stabilization formal
  (Corollary~\ref{cor:winfty-weight-cutoff}), and
  Proposition~\ref{prop:inverse-limit-differential-continuity}
  upgrades the completed differential to a continuous map for the
  inverse-limit topology; Corollary~\ref{cor:winfty-standard-mc4-package}
  therefore supplies the standard completed M-level package
\end{enumerate}

\emph{Interpretation.} $W_\infty$ sits at the boundary of the present
finite-type theory.  It should be read as a completion frontier, not as
...
reduces the standard $W_\infty$ tower to the completed M-level package
of Corollary~\ref{cor:winfty-standard-mc4-package}.
\end{corollary}
...
Thus the standard principal-stage tower realizes the full M-level
completed bar-cobar package for $W_\infty$; the remaining frontier is
...
construct such a filtered H-level target, not another stabilization
theorem.
\end{corollary}
...
which remains the model completion frontier).
...
encoded by the Schwarzian derivative.  In the present completed
formalism, one tracks the same-family partner determined by the
central-charge involution $c \leftrightarrow 26-c$
...
while the stronger infinite-generator realization remains open.
...
\subsubsection{Class IV: general (completion frontier)}

\begin{example}[$W_\infty$: completion frontier]\label{ex:winfty-completion-frontier-comprehensive}\label{ex:winfty-no-dual-comprehensive}
...
No completed duality theorem is currently proved: the generating space $V$ is
infinite-dimensional, and the manuscript does not yet establish a completion of
the bar construction $\bar{B}(W_\infty)$ with the continuity and convergence
properties needed for bar-cobar inversion.
...
completed M-level bar-cobar package.  The remaining problem is not to
construct a first completed bar object, but to compare that standard
completion with stronger H-level factorization-theoretic or physical
realizations of $W_\infty$.

\emph{Physical interpretation.}
$W_\infty$ describes non-local interactions in 2D gravity.  The open problem is
to construct the correct completed dual description, not to prove that no such
...
Quadratic algebras correspond to free field theories (Heisenberg $\leftrightarrow$ free boson, Kac--Moody $\leftrightarrow$ WZW).  Curved algebras correspond to interacting theories with anomalies (Virasoro, with central charge $c$ measuring quantum breaking of scale invariance).  Filtered algebras correspond to theories with composite operators ($W_3$ and Toda field theory).  General algebras correspond to non-local theories ($W_\infty$), where a completed local-to-global field-theoretic realization remains open.
...
\item Is the generating space infinite-dimensional? $\Rightarrow$ treat as a completion frontier and first construct the completed dual object
\end{enumerate}
