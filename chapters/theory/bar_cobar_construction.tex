...
\emph{Current status}: the monograph does not prove a completed Koszul duality
theorem for $W_\infty$.
...
\]
whose differential is continuous and whose cobar recovers the completed
algebra.
...
\item no theorem in the manuscript yet shows that the completed object
  $\widehat{\bar{B}}(W_\infty)$ carries the continuity and coalgebra structure
  needed for bar-cobar formalism
\item no convergence theorem is yet available for the corresponding completed
  cobar operations
\end{enumerate}
...
The open problem is therefore specific: prove
Conjecture~\ref{conj:inverse-limit-completed-bar-cobar} for the
inverse system of finite-type truncations, rather than search for a
new finite-type presentation of $W_\infty$.
