...
Since $\bar{B}(\mathrm{Vir})$ is not conilpotent (Example~\ref{ex:virasoro-not-conilpotent}), one forms the completed bar complex $\widehat{\bar{B}}(\mathrm{Vir}) = \varprojlim_k \bar{B}(\mathrm{Vir}) / I^k$ and takes continuous linear dual.  Writing $\mathrm{Vir}_c = \mathcal{W}^k(\mathfrak{sl}_2)$ with $h^\vee = 2$, level-shifting duality (Corollary~\ref{cor:level-shifting-part1}) gives $k' = -k - 2h^\vee = -k-4$, so
\begin{equation}
...
\begin{proof}
The quotient-system data package precisely the inputs of Definition~\ref{def:winfty-principal-stage-compatible}; the final claim follows from Corollary~\ref{cor:winfty-hlevel-comparison-criterion}.
\end{proof}
...
\begin{proof}
The BD equivalence and the chiral-envelope functor produce compatible quotients recovering the principal stages; Proposition~\ref{prop:winfty-quotient-system-criterion} applies.
\end{proof}
...
\begin{proof}
Conditions~(1)--(3) give the same strong generators, OPE relations, and bar operations as $W_N$; the standard reconstruction principle identifies the quotient with the principal stage.
\end{proof}
...
\]
and suppose the coefficients agree with the principal
Drinfeld--Sokolov data, translation descendants generate all local
relations, and the induced bar operations match the finite-stage package.
Then Proposition~\ref{prop:winfty-ds-coefficient-criterion} applies
and the quotient identifies with $W_N$.
\end{proposition}

\begin{proof}
The three conditions are exactly the inputs of Proposition~\ref{prop:winfty-ds-coefficient-criterion}.
\end{proof}
...
Let $\mathsf{C}^{\mathrm{res}}_{s,t;u;m,n}(N)$ and
$\mathsf{C}^{\mathrm{DS}}_{s,t;u;m,n}(N)$ denote the residue and
principal DS coefficients.  Assume the quotient OPE and bar operations
are entirely computed by the residue coefficients, and
$\mathsf{C}^{\mathrm{res}}_{s,t;u;m,n}(N)
= \mathsf{C}^{\mathrm{DS}}_{s,t;u;m,n}(N)$
for every admissible tuple.
Then Proposition~\ref{prop:winfty-ds-local-coefficient-criterion} applies
and the quotient identifies with $W_N$.
\end{proposition}

\begin{proof}
The residue-coefficient equality and bar matching are exactly the inputs of Proposition~\ref{prop:winfty-ds-local-coefficient-criterion}.
\end{proof}
...
% Singleton blocks $(3,4)$ and $(5,5)$ are smallest; new transport content is in $(3,5)$ and $(4,5)$.
...
Define $\mathcal{J}_5^{\mathrm{entry}}:=\mathcal{J}_5^{(3,4)}\sqcup\mathcal{J}_5^{(5,5)}$ and $\mathcal{J}_5^{\mathrm{tr}}:=\mathcal{J}_5^{(3,5)}\sqcup\mathcal{J}_5^{(4,5)}$.
Then $\mathcal{J}_5^{\mathrm{hs}}=\mathcal{J}_5^{\mathrm{entry}}\sqcup\mathcal{J}_5^{\mathrm{tr}}$
with $|\mathcal{J}_5^{\mathrm{entry}}|=2$ and $|\mathcal{J}_5^{\mathrm{tr}}|=6$.
\end{corollary}
...
Define $\mathcal{J}_5^{\mathrm{ent,mix}}:=\bigl\{(3,4;5;0,2)\bigr\}$ and
$\mathcal{J}_5^{\mathrm{ent,self}}:=\bigl\{(5,5;4;0,6)\bigr\}$.
Then $\mathcal{J}_5^{\mathrm{entry}}=\mathcal{J}_5^{\mathrm{ent,mix}}\sqcup\mathcal{J}_5^{\mathrm{ent,self}}$.
\end{proposition}

\begin{proposition}[Exact reduced tail input at stage~\texorpdfstring{$5$}{5};
\ClaimStatusProvedHere]
\label{prop:winfty-stage5-reduced-tail-singleton}
The reduced tail block $\mathcal{J}_5^{\mathrm{tail,red}}:=\bigl\{(3,4;5;0,2)\bigr\}$ is the unique tail entry surviving top-pole/parity reduction in $\mathcal{J}_5^{\mathrm{red}}$.
\end{proposition}
...
Once the stage-$4$ packet is fixed, the reduced tail problem is matching the $W^{(5)}$-projection in the top pole of $W^{(3)}(z)W^{(4)}(w)$:
...
\begin{proof}
Regroup $\mathcal{J}_5^{\mathrm{tr}}$ by target spin.
\end{proof}
...
\mathcal{J}_5^{u=3}
:=
\bigl\{(3,5;3;0,5),\,(4,5;3;0,6)\bigr\},\quad
\mathcal{J}_5^{u=4}
:=
\bigl\{(3,5;4;0,4),\,(4,5;4;0,5),\,(5,5;4;0,6)\bigr\},\\
\mathcal{J}_5^{u=5}
:=
\bigl\{(3,4;5;0,2),\,(3,5;5;0,3),\,(4,5;5;0,4)\bigr\}.
\end{gather*}
Then
$\mathcal{J}_5^{\mathrm{hs}}
=
...
\sqcup
\mathcal{J}_5^{u=5}$,
with cardinalities $2$, $3$, $3$.
\end{proposition}

\begin{proof}
Regroup $\mathcal{J}_5^{\mathrm{hs}}$ by target spin.
\end{proof}

The target-$5$ corridor, residual continuation, and singleton decompositions
follow by straightforward set-theoretic regrouping.
...
(4,5;5;0,4)
\bigr\}$,
supported at consecutive pole orders $2,3,4$.
\end{corollary}
...
After matching the tail singleton,
...
Under the hypotheses of
Corollary~\ref{cor:winfty-stage5-target5-residual}, the residual
target-$5$ corridor is the comparison of $W^{(5)}$ inside
...
\begin{proof}
These are the top-pole $W^{(5)}$-coefficients in
$\mathcal{J}_5^{\mathrm{tr},5}$.
\end{proof}
...
The three fixed-target ladders of
Proposition~\ref{prop:winfty-stage5-transport-target-ladders}
have pole-order profiles
$\mathcal{J}_5^{\mathrm{tr},5}:\{3,4\}$,
$\mathcal{J}_5^{\mathrm{tr},4}:\{4,5\}$,
$\mathcal{J}_5^{\mathrm{tr},3}:\{5,6\}$.
In particular, $\mathcal{J}_5^{\mathrm{tr},5}$ is supported at the two smallest pole orders.
\end{proposition}
...
\begin{proof}
The conjunction of the two transport vanishings and the tail cross-target relation is exactly the content of the two sub-conjectures.
\end{proof}
...
\begin{proof}
The conjunction of the three vanishings and the residual cross-target relation is exactly the content of the two sub-conjectures.
\end{proof}
