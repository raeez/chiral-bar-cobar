...
Then the bar complex satisfies the sub-exponential growth
condition~\textup{(ii)} of
Conjecture~\textup{\ref{conj:factorization-finiteness-criterion}}
for \emph{every} bigrading $\barB^{p,q}(\cA)$ with
$\dim \barB^{p,q} < \infty$ compatible with the conformal grading.
\end{proposition}
...
\;\leq\;
\bigl(\max_{1 \leq k \leq h} \dim \cA_k\bigr)^n
\binom{h-1}{n-1}.
\]
For positive-energy algebras, the character has
modular-type asymptotics:
$\dim \cA_n \leq C\, e^{\alpha \sqrt{n}}$ for constants
$C, \alpha > 0$ depending on the effective central
charge~$c_{\mathrm{eff}}$ (Hardy--Ramanujan--Rademacher).
Any bigrading compatible with the conformal grading satisfies
$\dim E_1^{0,p} \leq \sum_{n \leq N_0} \dim \barB^n_{[p]}$
for bounded bar degree, giving
$\log(\dim E_1^{0,p}) \leq O(\sqrt{p})$, which is $o(p)$.
Hence $\dim E_1^{0,p} < c^p$ for every $c > 1$ and
$p \gg 0$.
\end{proof}
...
Conjecture~\textup{\ref{conj:factorization-finiteness-criterion}}
reduces to condition~\textup{(i)} alone for the entire standard
landscape.  The residual content is the geometric assertion that
sectorwise spectral sequence degeneration~\textup{(i)} on the
bar complex lifts to compact generation of the factorization
category $\mathrm{Fact}(X;\, \cA)$ on\/ $\Ran(X)$.
\end{corollary}
