...
and the Koszulness of the underlying chiral operad.  Bar and cobar are
adjoint projections of a single incidence correspondence on
$\overline{\Conf}_n(X)$: bar records collision strata, cobar
reassembles the algebra from collision data.  On the Koszul locus
(Definition~\ref{def:koszul-locus}),
this round trip loses no information.
...
This identification uses finite homological dimension.
For a chirally Koszul algebra~$\cA$, the bar spectral sequence
collapses at~$E_2$ with cohomology concentrated on the diagonal
(Theorem~\ref{thm:pbw-koszulness-criterion}).  The bar coalgebra
$\bar{B}^{\mathrm{ch}}(\cA)$ therefore has graded pieces of
finite total dimension in each conformal weight
(by the standing hypothesis of finite-dimensional conformal
weight spaces on~$\cA$), and the Koszul resolution provides a
finite-length resolution of any finitely generated module.
Under these conditions, Positselski's criterion
\cite[\S4.5, Theorem~4.5.1]{Positselski11} identifies the
contraderived and ordinary bounded derived categories.
...
The map $\psi$ is the counit of the bar-cobar adjunction
(the right adjoint of the unit map of Theorem~\ref{thm:bar-cobar-adjunction}); therefore it is canonical and respects
...
The full differential on $\Omega(\bar{B}(\mathcal{A}))$ decomposes as $d = d_{\bar{B}} + d_{\text{cobar}}$, where $d_{\bar{B}} = d_{\text{internal}} + d_{\text{bar}}$ is the lift of the bar complex differential to each cobar tensor factor (preserving cobar arity~$p$), and $d_{\text{cobar}}$ is the comultiplication-induced part (raising cobar arity by~$1$).  On the associated graded $\text{Gr}^p = F^p/F^{p+1}$, only the \emph{filtration-preserving} component survives: $d_{\text{cobar}}$ maps $F^p$ into $F^{p+1}$ and so vanishes on $\text{Gr}^p$.  Thus $d_{\text{gr}} = d_{\bar{B}}$, which is the $d_0$ differential of the spectral sequence (cf.\ Theorem~\ref{thm:bar-cobar-spectral-sequence}).
\end{proof}
...
is the bar complex differential $d_{\bar{B}} = d_{\text{internal}} + d_{\text{bar}}$, lifted to each cobar tensor factor.  This is the filtration-preserving component of $d$: both $d_{\text{internal}}$ (from the differential on~$\mathcal{A}$) and $d_{\text{bar}}$ (from the multiplication of~$\mathcal{A}$, encoding collisions) preserve cobar arity.
...
is induced by $d_{\text{cobar}}$ (the comultiplication on $\bar{B}(\mathcal{A})$, which increases cobar arity by~$1$).
...
(Theorem~\ref{thm:bar-cobar-inversion-qi}) gives the counit
$\Omega_\kappa(B_\kappa(\cA)) \xrightarrow{\sim} \cA$
is a quasi-isomorphism.  If $f\colon \cA \to \cA'$ satisfies
$B_\kappa(f) \simeq 0$, then
$f \simeq \varepsilon_{\cA'} \circ \Omega_\kappa(B_\kappa(f))
\simeq 0$, so $B_\kappa$ is conservative.
...
finite-dimensional chain complex (by finite-dimensional conformal
weight spaces).  For a $B_\kappa$-split cosimplicial diagram,
splittings pass to these fiber complexes, so the totalization
commutes with the global sections functor.
...
i_v^!\,\barBch_{|v|}(\cA)$,
where each factor is a lower-arity bar component restricted to
...
\begin{theorem}[Lagrangian characterization of chiral Koszulness;
\ClaimStatusConditional]\label{conj:lagrangian-koszulness}%
...
Proposition~\ref{prop:lagrangian-perfectness} verifies all three
sub-hypotheses for the standard landscape at non-critical,
non-degenerate levels: perfectness follows from finite-dimensional
weight spaces~(P1) and nondegenerate invariant form~(P2);
non-degeneracy of the shifted-symplectic pairing follows from
the tangent-level theorem plus perfectness; finiteness of the
modular envelope is immediate from~(P1).
...
Let $\cA$ satisfy the standing hypotheses and additionally:
...
and vanishes for $m \ll 0$.  The cyclic deformation complex
$L_\cA = \Defcyc(\cA)$ at weight~$n$ is built from
finite tensor products of the~$\cA_m$ (weighted by
stable-graph combinatorics at each genus~$g$ and arity~$r$,
of which finitely many contribute at weight~$n$).
Hence $\dim L_\cA|_n < \infty$.  By~(P\ref{item:perf-dual}),
the same holds for $L_{\cA^!}|_n$.
The kernel object
$K_\cA = (\cA^! \widehat\otimes_\tau \cA)[1]$ at weight~$n$ is
$\bigoplus_{p+q=n}(\cA^!_p \otimes \cA_q)[1]$, a finite direct
sum of finite-dimensional spaces.

\smallskip\noindent
\emph{Step~2: tangent-level nondegeneracy.}
By Theorem~\ref{thm:ambient-complementarity-tangent}(i), the
degree~$-1$ pairing on the ambient complementarity tangent complex
$T_{\mathrm{comp}}(\cA)$ is nondegenerate.  This is proved
unconditionally: the pairing descends from the cyclic structure on
$L_\cA \oplus K_\cA \oplus L_{\cA^!}$ by passage to the homotopy
fiber, and skew-adjointness of the differential guarantees
compatibility.  The nondegeneracy at the tangent level
means the pairing induces an injection from $T_{\mathrm{comp}}$
into its graded dual at each weight.

\smallskip\noindent
\emph{Step~3: perfectness.}
A nondegenerate pairing on a finite-dimensional chain complex
over a field is automatically perfect: an injective linear map
between equidimensional finite-dimensional spaces is an
isomorphism.  Steps~1 and~2 together give:
at each weight level~$n$, the pairing map
$(L_\cA \oplus K_\cA \oplus L_{\cA^!})|_n \to
(L_\cA \oplus K_\cA \oplus L_{\cA^!})^\vee[-1]|_n$
is an isomorphism of finite-dimensional vector spaces.
The pro-complete pairing is the inverse limit of these
weight-level isomorphisms, hence perfect.
\end{proof}
...
\begin{proof}
By Proposition~\ref{prop:lagrangian-perfectness}, the cyclic
pairing is perfect under~(P1)--(P3).
Theorem~\ref{thm:ambient-complementarity-tangent}(i) gives a
canonical nondegenerate degree~$-1$ pairing on
$T_{\mathrm{comp}}(\cA)$ (proved unconditionally).
By the Lurie--Pridham correspondence, the complete filtered dg~Lie
algebra $\Defcyc^{\mathrm{mod}}(\cA)$ integrates to a formal
moduli problem $\cM_{\mathrm{comp}}(\cA)$; the perfect cyclic
pairing induces a $(-1)$-shifted symplectic structure via the
PTVV/Kontsevich--Pridham principle, so
Theorem~\ref{thm:ambient-complementarity-fmp} applies with the
perfectness hypothesis now verified.

The equivalence then follows from the proof of
Theorem~\ref{conj:lagrangian-koszulness}:
\begin{itemize}
\item \emph{Forward.}
  On the Koszul locus, the twisted tensor product
  $K_\tau(\cA, \cA^!)$ is acyclic, so the complementarity
  splitting (Theorem~C) gives a direct-sum decomposition
  $T_{\mathrm{comp}} \simeq T_\cA \oplus T_{\cA^!}$.
  The tangent spaces are isotropic
  (Theorem~\ref{thm:ambient-complementarity-tangent}(ii));
  the complementarity splitting makes them half-dimensional,
  hence Lagrangian.
\item \emph{Converse.}
  Transverse Lagrangians in a $(-1)$-shifted symplectic formal
  moduli problem have discrete derived intersection (PTVV).
  The derived intersection
  $\cM_\cA \times^h_{\cM_{\mathrm{comp}}} \cM_{\cA^!}$
  computes the twisted tensor product $K_\tau(\cA, \cA^!)$;
  discreteness is acyclicity, which is Koszulness.
\end{itemize}

The standard-landscape verification: hypotheses (P1)--(P3)
are satisfied by every listed family.  The invariant form is the
Killing form (nondegenerate at $k \neq -h^\vee$ for Kac--Moody),
the Shapovalov form (nondegenerate at generic~$c$ for Virasoro),
or the canonical pairing for free fields.  Dual regularity~(P3)
holds because Koszul duality preserves finite-dimensionality of
weight spaces and the dual pairing inherits nondegeneracy.
\end{proof}
...
and null vectors appear---a satisfying coincidence, since
both phenomena reflect the breakdown of the PBW structure.
\end{remark}
...
and Conjectures~\ref{conj:d-module-purity-koszulness}--\ref{conj:lagrangian-koszulness}
...
$\mathbb{D}_{\operatorname{Ran}}\barBch(\cA) \simeq \cA^!_\infty$ (underlying complex $\simeq \barBch(\cA^!)$;
...
$\mathbb{D}_{\mathrm{Ran}}\barB_X(\cA) \simeq \cA^!_\infty$ (underlying complex $\simeq \barB_X(\cA^!)$)
...
Theorem~\ref{thm:modular-characteristic}(iii)), self-duality
$\cA \simeq \cA^!$ forces $\kappa(\cA) = 0$ and hence $F_g(\cA) = 0$
for all $g \geq 1$: the scalar logarithm of such a self-dual chiral
algebra vanishes identically.  For $\mathcal{W}$-algebras, the
complementarity sum $\kappa + \kappa'$ is a nonzero constant
(e.g.\ $\kappa + \kappa' = 13$ for Virasoro), so self-duality at
$c = 13$ gives $\kappa(\mathrm{Vir}_{13}) = 13/2 \neq 0$: the
Virasoro self-dual point is \emph{not} at critical level.
