...
($\mathcal{W}_\infty$, $\mathcal{W}_{1+\infty}$) mark the frontier
where the MC4 completion programme is still under construction.
The regime hierarchy established in
Convention~\ref{conv:regime-tags} organizes these cases; the
sections that follow develop each regime and prove inversion in the
first three.
...
require bar-cobar inversion as input.  The regime hierarchy
established in
Convention~\ref{conv:regime-tags} connects forward to Ring~3:
the curved-central and filtered-complete regimes govern the
$W$-algebra tower (MC4, closed) and the Yangian axis (MC3,
proved in type~A).
...
and programmatic (infinite generators, MC4 completion target).
...
\item Is the generating space infinite-dimensional? $\Rightarrow$ treat as an MC4 completion target: first construct the filtered H-level target, then identify its finite quotients by explicit coefficient identities and finite detection
\end{enumerate}
...
\begin{remark}[Parallel with the modular convolution algebra]
\label{rem:cech-modular-parallel}
The algebra $\mathfrak{g}^{\check{C}}(\cA,\mathcal{U})$ is the
\v{C}ech analogue of the modular convolution dg~Lie algebra
$\mathfrak{g}^{\mathrm{mod}}_\cA$
(Definition~\ref{def:modular-convolution-dg-lie}).  Both encode
all algebraic operations on a resolution of~$\cA$ as components
of a single Maurer--Cartan element.  The key difference:
$\mathfrak{g}^{\mathrm{mod}}_\cA$ is indexed by stable graphs
(all genera), while $\mathfrak{g}^{\check{C}}$ is indexed by the
simplicial combinatorics of the cover (all \v{C}ech degrees).
\end{remark}
...
(Theorem~\ref{thm:kappa-universality}) and
...
(Theorem~\ref{thm:kappa-universality}), while $L_2$ is the
