...
develop each regime in turn, with the infinite-type case treated as
...
lie beyond the present theorem (e.g., $\mathcal{W}_\infty$,
whose standard tower is covered while the remaining problem is the
...
Since $\mathfrak{h}$ is abelian, the chiral Koszul dual is
$\mathrm{Sym}^{\mathrm{ch}}(V^*)$ with $d = 0$ and curvature
$m_0 \propto -k \cdot \mathbf{1}$ (the level negates under Koszul
duality; $h^\vee = 0$ for the abelian Lie algebra). This is a
\emph{commutative} chiral algebra with curvature, distinct from the
negated-level Heisenberg algebra $\mathcal{H}_{-k}$. The bar differential
vanishes (Example~\ref{ex:ope-to-residue}), so
$H^*(\bar{B}(\mathcal{H}_k)) \simeq \bar{B}(\mathcal{H}_k)$ with CE
cooperad structure.
...
\emph{Bar complex and Koszul-dual lane.}
...
Applying the \emph{cobar} functor to this coalgebra recovers the
\emph{original} algebra:
$\Omegach(\bar{B}^{\mathrm{ch}}(\widehat{\mathfrak{g}}_k))
\simeq \widehat{\mathfrak{g}}_k$ (bar-cobar inversion,
Theorem~\ref{thm:bar-cobar-inversion-qi}). The Verdier-Koszul lane is
separate:
\[
\mathbb{D}_{\Ran}\bar{B}^{\mathrm{ch}}(\widehat{\mathfrak{g}}_k)
\simeq (\widehat{\mathfrak{g}}_k^!)^{\mathrm{co}}_\infty
\]
as a factorization coalgebra, and applying the completed cobar functor
gives the Koszul dual algebra
...
The Verdier-Koszul lane is governed by the central-charge involution
...
where $\Lambda(L \cdot L)$ is a composite operator. The algebra is
filtered. It has no finite-dimensional generating space $V$: composite
operators like $(L \cdot L)$ appear at all levels, and the filtration is
infinite-dimensional at each level. The completed Verdier-dual coalgebra
lane is
\[W_3^{!,\mathrm{co}} = \widehat{\mathrm{CoW}_3}
...
effectively curved; the filtration introduces no independent composite
generators.
...
\emph{Completed curved differential.}
\[\widehat{d}(T \otimes T) = \text{Res}(T(z)T(w)) + c \cdot \mathbf{1}\]

The completed object is a curved CDG bar coalgebra with
$\widehat d^{\,2}=[c\cdot\mathbf 1,-]$. A square-zero differential appears
only after passing to the completed totalized model used on the collapse
surface; outside that setting the correct ambient category is coderived.

\emph{Cohomology.}
\[H^*(\widehat{\bar{B}}(\text{Vir})) = \widehat{U(\text{Vir})}^*_{-c}\]
is the completed dual universal enveloping algebra.

Completion is essential. Without completion, the bar complex does not
converge and the Verdier-Koszul lane has no algebraic realization.
\end{example}
...
the bar object is coacyclic on the higher-genus coderived surface
...
treated here the curved bar objects are coacyclic in the second-kind
sense, while $D^{\mathrm{co}}$ retains nontrivial comodule
information.
\end{remark}
...
\textup{(}Theorem~\textup{\ref{thm:bar-convergence}}, growth bound plus
filtration compatibility\textup{)}. Chain-level and $(\infty,1)$ are
...
\textup{(}Theorem~\ref{thm:bar-convergence}, growth bound (2) plus
filtration compatibility (3)\textup{)}, and Positselski 2011 \S A.5
...
\begin{itemize}
 ...
 \item[(H3b)] \emph{Bar-degree-piece finiteness, raw ambient.}
 $\dim \bar B^n_X(A) < \infty$ for each $n \geq 0$, together
 with the growth bound and filtration-compatibility of
 ...
 \end{itemize}
\end{itemize}
Conilpotency of $\bar{B}_X(A)$ is automatic under (H1)+(H2) by the
cofree-conilpotent left adjoint to the forgetful functor (Vallette 2014
\S 3); the Koszul condition is \emph{not} a hypothesis but a property of
the output, defining the locus $\mathrm{Kosz}(X)$ on which the coderived
equivalence becomes a chain-level qi. The bifurcation (H3a)/(H3b)
determines \emph{which bar complex} carries the chain-level qi: raw
$\bar B_X$ for class-$\mathsf{L}$ under (H3b), weight-completed
$\widehat{\bar B}_X$ for class-$\mathsf{M}$ under (H3a); the
$(\infty,1)$-categorical statement of the theorem is insensitive to
this split because both ambients map into the same stable presentable
$\infty$-category $\mathrm{CoAlg}^{\mathrm{fact,conil,co}}_X$ under the
weight-completion functor, whose underlying $\infty$-functor is an
equivalence on the essential image of (H3a)+(H3b).
...
on every Fulton--MacPherson stratum, and that the completed cobar
construction converges on its Verdier dual. Define the coalgebraic
Verdier dual
\[
 A^{!,\mathrm{co}}_\infty := D_{\Ran}\bar B_X(A)
\]
...
Riemann surface~$X$. Assume either that
$\bar{B}^{\mathrm{ch}}(\mathcal{A})$ is a strict conilpotent
finite-window bar coalgebra, or that $\mathcal{A}$ is complete with
respect to its augmentation ideal and the bar coalgebra lies in the
weight-completed filtered pro-conilpotent essential image of
Theorem~\ref{thm:chiral-positselski-weight-completed}. In the raw lane
the graded bar pieces are finite-dimensional. In the class-$\mathsf M$
lane the finite-dimensional objects are the strict weight-window
stages, not the raw direct-sum bar complex. The bar-cobar counit
\[
...
\textup{(}conditional filtered pro-conilpotent scope\textup{)}.}
 ...
 $D^{\mathrm{co}}(\barB\text{-}\mathrm{CoFact})$ inside that
...
central-charge package $K^c$ and from the legacy ghost package
...
curved-central, filtered-complete, and infinite-type.
\end{remark}
...
\item \textbf{Derived chiral centre}:
 ...
 Hochschild cochain algebra of~$\cA$. It is the bulk/centre object,
...
$(a)$.
The anti-isomorphism of lattices is immediate because larger divisors
produce smaller quotient ideals.
\end{proof}
...
cohomology, not idempotently removable summands.
\end{corollary}

\begin{proof}
Let $q$ be a common irreducible factor of $g$ and $p_\cA/g$.
Then the $q$-primary piece of $\mathcal{C}_g(p_\cA)$ lies inside the
Jordan filtration of Theorem~\ref{thm:primary-jordan-filtration-inv}
with exponent at least~$2$.
That theorem identifies the corresponding primary summand with
$k[t]/(q^m)$ and, when $q=t-\lambda$ splits, exhibits a single Jordan
block for $T_{p_\cA}-\lambda$.
Such a primary block is canonical as a submodule, but it cannot split
idempotently into semisimple eigenspaces because the nilpotent Jordan
operator is nonzero.
Hence repeated roots of the discriminant encode genuine extension data,
not formal summands.
\end{proof}
...
\subsection{Geometric divisor-core lifts}
...
The following conjectures are the geometric lifts of the divisor-core
calculus.
...
\section{Bar-cobar inversion at K3 and the Universal Phi-trace}
...
\begin{remark}[Universal $\Phi$-trace: bar-cobar side intertwined with Borcherds pushforward]
...
\mathbf H_{\Delta_5},
\]
is a quasi-isomorphism on the strict Koszul locus and becomes a
\emph{strict isomorphism} at the Lusztig specialisation
...
obstruction $K^{\kappa_{\mathrm{ch}}} = 8$, eliminating the homotopy
coherence needed away from Lusztig. See Theorem~\ref{thm:bar-cobar-platonic}.
\end{remark}

\begin{remark}[Strict isomorphism in the derived Hall category]
\label{rem:bcai-deep-2}
In the derived Hall category
$D^{\mathrm{Hall}}(\mathrm{CoHA}_{K3\times E})$, the bar-cobar inversion
$\varepsilon_{\mathbf H_{\Delta_5}}$ is strict after passage to the
derived category of Hall complexes and restriction to the Lusztig
locus $\hbar^2=-1/8$. In that ambient, inversion identifies
$\mathbf H_{\Delta_5}$ as a Koszul algebra object in the derived Hall
category. The relevant comparison rests on
