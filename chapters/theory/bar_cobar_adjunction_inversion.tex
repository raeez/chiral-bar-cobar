...
curve $X$ with finite-dimensional graded pieces. Assume moreover that
...
Let $X$ be a smooth projective curve over $\mathbb{C}$.  The pair
...
$\mathrm{Kosz}(X) \subset \mathrm{Alg}^{\mathrm{fact, aug, comp}}_X$, to a
symmetric-monoidal equivalence between the relevant ordinary
\textup{(}non-coderived\textup{)} subcategories, with chain-level
quasi-isomorphisms for the restricted unit and counit.
\end{theorem}

\begin{remark}[Foundational scope: ordering, lane, and open-problem
dependency]\label{rem:bar-cobar-platonic-foundational-scope}
...
\textup{(}$E_1$ primacy: the ordered lane is constructed first; the symmetric lane is obtained by averaging\textup{)}. The counit
$\varepsilon$ of
...
right adjoint and right adjoints preserve the $\Sigma_n$-fixed points
that define $\mathrm{av}$. Concretely, for each $n$ the comparison
$\barB^{\mathrm{ord}}_n \twoheadrightarrow \barB^{\Sigma}_n$ is the
quotient by the Arnold relation generated by
$\eta_{ij}\wedge\eta_{jk}+\eta_{jk}\wedge\eta_{ki}+\eta_{ki}\wedge\eta_{ij}=0$;
inversion on the ordered side descends to inversion on the symmetric
...
\bar B^{\mathrm{ch}}(\mathrm{Vir}_{c,\leq N})$ rather than on raw
$\mathrm{Ch}(\mathrm{Vect})$\textup{)}, and the $\beta\gamma$ witness of
...
The five named open problems of the preface localise on specific
clauses of Theorem~\ref{thm:bar-cobar-platonic}:
\textup{(i)}~chain-level topologization for affine Kac--Moody
extends clause~(4) on the non-critical Koszul lane, conditional on the
coherence equation $[m,G]=\partial_z$; \textup{(ii)}~general
topologization extends (i) to $\mathrm{Vir}_c$ and $\cW_N$ on the
class-$\mathsf M$ stratum, pending the bulk BRST complex;
\textup{(iii)}~MC5 class $\mathsf M$ at chain level is conditional on
Theorem~\textup{\ref{thm:mc5-class-m-chain-level-pro-ambient}} in the
pro-ambient; a lift to the bounded direct-sum ambient is not available,
compatible with clause~(2) \textup{(}coderived off-Koszul
lane\textup{)}; \textup{(iv)}~Koszul freeness is the converse of
clause~(1) \textup{(}D-module purity $\Rightarrow$ Koszul\textup{)};
\textup{(v)}~the Baxter constraint is an open refinement of the
Koszul locus $\mathrm{Kosz}(X)$ on the $\mathcal O_{\mathrm{poly}}$
stratum. None of the five open problems falsifies a proved clause of
Theorem~\ref{thm:bar-cobar-platonic}; each extends one clause to a
larger ambient or a wider locus. A negative resolution of any of them
would sharpen the theorem's scope, not collapse it: the Heisenberg,
Kac--Moody at non-critical level, Virasoro at generic $c$, and
$\beta\gamma$ witnesses are chain-level strict on the Koszul locus
independent of (i)--(v).
\end{remark}

\begin{remark}[Class-$\mathsf{L}$ versus class-$\mathsf{M}$ ambient
split for T1c]\label{rem:bar-cobar-platonic-class-M-ambient}
The Koszul-locus chain-level clause of
Theorem~\ref{thm:bar-cobar-platonic} \textup{(}call it T1c, reserving
T1a for the backbone $(\infty,1)$-adjunction and T1b for the Verdier
intertwining of
Corollary~\ref{cor:A-shriek-constructed}\textup{)} is a statement
about two ambients at once, parametrised by the
hypothesis (H3a) vs (H3b) of the preceding list.
\begin{itemize}
...
adjoint. The $(\infty,1)$-categorical statement T1a of
Theorem~\ref{thm:bar-cobar-platonic} is stated \emph{after} this
weight-completion functor is applied; T1c is stated at chain level
and therefore splits along (H3a) vs (H3b). The two lanes agree on
class-$\mathsf{L}$ \textup{(}where weight completion is the identity
up to qi\textup{)} and are honestly distinct on class-$\mathsf{M}$
\textup{(}where raw $\bar B_X$ does not admit a chain-level
contracting homotopy, but $\widehat{\bar B}_X$ does\textup{)}. The
...
completion on the centre side and does not affect this (H3a)/(H3b)
...
verified weight-completed coderived surface off the strict lane. The
...
\paragraph{Four animating morphisms.}
Theorem~\ref{thm:bar-cobar-platonic} is animated by exactly four canonical
morphisms:
\begin{itemize}
 ...
 \item $K \colon A \to \Omega \bar{B}(A) \simeq A$ \emph{(Koszul
       reflection):} the involutive symmetry.
...
\begin{corollary}[Constructed Koszul dual]
\label{cor:A-shriek-constructed}
\ClaimStatusProvedHere
Define $A^!_\infty := \Omega_X(\bar{B}_X(A))^\vee$.  Then $A^!_\infty$ is
a constructed object, not an unconstructed symbol, and the Verdier
intertwining
$D_{\Ran} \bar{B}_X(A) \simeq A^!_\infty$
is automatic from Theorem~\ref{thm:bar-cobar-platonic} and Verdier duality
on $D^b_c(\bar{C}_k(X))$.  This closes the long-standing $A^!_\infty$-symbol
gap.
\end{corollary}

\paragraph{Open frontiers as named conjectures.}
Four obstruction classes lie just beyond Theorem~\ref{thm:bar-cobar-platonic}
and are named explicitly to keep the scope of the platonic statement clean.
...
\textup{(}This is Theorem~B of the programme; in the bar-cobar
reconstruction theorem of Theorem~\ref{thm:koszul-reflection}, it is the
reconstruction clause~\ref{KR-ii} \textup{(}chain-level
...
locus\textup{)}, written in concrete chain-level form for the
Heisenberg-rooted inversion lineage.\textup{)}
...
\textup{(}conditional filtered pro-conilpotent scope\textup{)}.}
 ...
 lies in the verified filtered pro-conilpotent essential image and has
...
verified filtered pro-conilpotent essential image and have
finite-dimensional graded bar pieces. Its proof is independent of
...
$\varkappa = 98/3$; in Trinity normalisation $K := c + c^! = 26$ for
Virasoro and $K = 196$ for BP, related by
$\varkappa = \varrho_{\cA}\cdot K$), a quasi-isomorphism of bar
coalgebras forces
...
$\hbar$-adic completion.  Analytic convergence for non-formal
$\hbar$ is conditional on the corresponding all-genus growth bound for
the moduli-space integrals.
...
\item \emph{Genus series:}
 \[\psi = \sum_{g=0}^\infty \hbar^{2g-2} \psi_g\]
defines an element of the formal $\hbar$-adic completion.  If the
genus-$g$ norms of the corrected counits satisfy an exponential bound
\(\|\psi_g\| \leq C R^{-g}\), then the same series converges
analytically for \(|\hbar|^2 < R\).
\end{enumerate}
\end{theorem}
...
\begin{proposition}[Counit is quasi-isomorphism; \ClaimStatusConditional]\label{prop:counit-qi}
...
\begin{proof}
Verdier duality identifies the counit
$\epsilon_{\mathcal{C}}\colon
\bar{B}^{\mathrm{ch}}(\Omega^{\mathrm{ch}}(\mathcal{C}))\to\mathcal{C}$
with the dual of the unit map on the dual algebra side
(Theorem~\ref{thm:verdier-bar-cobar}).
The unit is a quasi-isomorphism by Theorem~\ref{thm:geom-unit}.
The same Verdier-duality theorem shows that duality on the relevant
Fulton--MacPherson complexes preserves quasi-isomorphisms.
Applying Verdier duality to the unit therefore yields that the counit
...
object on $\operatorname{Ran}(X)$. The assembly is not a
reformulation but an \emph{identification}: the bar construction
\emph{is} factorization homology, and the bar-cobar adjunction
\emph{is} the adjunction of factorization (co)homology functors.
...
rather than removable bookkeeping.
\end{proof}
...
The bar-cobar inversion $\Omega(B(\cA)) \simeq \cA$ is \emph{inversion}, not Koszul
duality. At $\cA = \mathcal{H}_{\mathrm{Muk}}$ the Mukai-Heisenberg
K3 chiral algebra, inversion recovers the $E_2$-chiral output of $\Phi_2$ on its own
bar complex. Koszul duality via Verdier gives the coalgebra
...
The bar-cobar Koszul conductor $K(\cA) = -c_{\mathrm{ghost}}(\mathrm{BRST}(\cA)) =
\mathrm{tr}_{\mathfrak{Z}(\cA)}(\mathfrak{K}_{\cA})$ on the chiral side intertwines via
the Borcherds singular-theta pushforward with the Vol~III BKM weight
...
\[
\Omega_X\bigl(B_{\mathrm{ch}}(\mathbf H_{\Delta_5})\bigr)\ \xrightarrow{\;\eta\;}\ \mathbf H_{\Delta_5},
\]
...
statement is the completed coderived or $A_\infty$ repair, not a raw
...
coherence needed away from Lusztig. Cross-ref Vol.~I, Theorem~A
(chiral bar-cobar inversion).
\end{remark}
...
$\eta$ becomes a strict isomorphism after the following two passes:
(i) passage to the derived category of Hall complexes (no quasi-isomorphism
subtleties); (ii) restriction to the Lusztig locus $\hbar^2 = -1/8$
(no curvature non-triviality). Combined, the inversion
identifies $\mathbf H_{\Delta_5}$ as a \emph{Koszul algebra} in the
derived Hall category, the first-known example of a genuinely
non-abelian chiral Koszul algebra on a CY-$2$ base. Primary-lit:
Schiffmann-Vasserot 2012, Davison 2017, Kontsevich-Soibelman 2011.
\end{remark}
