...
the square-zero bar-cobar spectral sequence only on the Koszul surface.
...
strict Koszul surface is clause~\ref{KR-ii}
\textup{(}$\Omega^{\mathrm{ch}}\bar B^{\mathrm{ch}}(\cA)\to\cA$ at
chain level on $\Kosz(X)$\textup{)}, the coderived off-Koszul surface is
clause~\ref{KR-iii}
...
The ordered $E_1$ surface is constructed first; the symmetric surface is
...
first obstruction; the finite-mode family
\[
\xi_N=\sum_{\lvert k\rvert\leq N}
s^{-1}L_{-k}\otimes s^{-1}L_k
\]
records the direct-sum failure: each $\xi_N$ is finite, but the
coordinatewise limit has one nonzero coordinate for every Virasoro
mode and therefore lives only in the weight-completed product
ambient; Example~\ref{ex:virasoro-not-conilpotent} is the local
model\textup{)}, so the direct-sum bar differential fails the
Mittag-Leffler condition on the bar-degree filtration and no
...
surfaces agree on class-$\mathsf{L}$, where weight completion is the
...
proved weight-completed coderived surface off the strict surface. The
Verdier-dualized Koszul-duality functor is different:
\[
 K_X(A):=\mathbb{D}_{\Ran}\bar B_X(A).
\]
...
extra contents of (H3b) in the raw surface and of the finite-window
...
\begin{definition}[Finite-window bar-cobar essential image; \ClaimStatusDefinitional]
...
\item $\bar B_X(A)=T^c(s^{-1}\bar A)$, with the chiral twisting
 ...
 coalgebra.
\item On the strict square-zero Koszul surface,
 \[
 A^{\mathrm i}:=H^\bullet(\bar B_X(A))
 \]
 is the Koszul dual coalgebra. In a curved higher-genus surface this
...
\item Under constructibility, finite-dimensional-stratum, and completed
 ...
 the completed conilpotent coalgebra side, the Verdier-Koszul dual component is
	 \[
...
\begin{corollary}[Constructed Verdier-Koszul dual component]
...
\]
This is the constructed dual component. It is not the bar-cobar inversion
...
into the four corollaries (strict Koszul surface, coderived off-Koszul surface,
spectral-sequence collapse, Quillen equivalence) traditionally bundled as
the four-clause inversion theorem below.

\subsection{Statement of the main result}

\begin{theorem}[Bar-cobar inversion: strict on the Koszul locus, coderived off it;
\ClaimStatusConditional]\label{thm:bar-cobar-inversion-qi}
\textup{[Regime: quadratic on the strict surface; curved-central and
filtered-complete on the coderived surface
...
\begin{cases}
\bar B_X(\mathcal A),&
\text{on the raw finite-window surface},\\
\widehat{\bar B}_X(\mathcal A)=
\varprojlim_N\bar B_X(\mathcal A_{\le N}),&
\text{on the completed pro-conilpotent surface}.
\end{cases}
\]
In the raw surface the graded bar pieces are finite-dimensional and the
finite-support comparison holds. In the class-$\mathsf M$ surface the
...
\]
where $\Omega_X^{\mathrm{cont}}=\Omega_X$ on the raw surface and denotes
the continuous completed cobar functor on the pro-conilpotent surface.
Its strongest proved form is:
\begin{enumerate}
\item \emph{Strict genus-\(0\) FTM surface.}
 For the genus-\(0\) chiral twisting datum
 \[
 (\mathcal A,C_{\mathcal A,0},\tau_{\mathcal A,0},F_\bullet),
 \]
 the following four conditions are equivalent:
 \begin{enumerate}[label=\textup{(\alph*)}]
 \item \(\tau_{\mathcal A,0}\) is a chiral Koszul morphism
  \textup{(}Definition~\ref{def:chiral-koszul-morphism}\textup{)};
 \item the genus-\(0\) counit in the chosen ambient
	 \[
	 \psi_0 \colon
 ...
 \item the genus-\(0\) unit
 \[
 \eta_0\colon C_{\mathcal A,0}
 \longrightarrow \bar B^{\mathrm{ch}}_0(\mathcal A)
 \]
 is a weak equivalence of conilpotent complete factorization
 coalgebras;
 \item the twisted tensor products
 \(K_{\tau_{\mathcal A,0}}^L(\mathcal A,C_{\mathcal A,0})\)
 and
 \(K_{\tau_{\mathcal A,0}}^R(C_{\mathcal A,0},\mathcal A)\)
 are acyclic.
 \end{enumerate}
 Thus \(\mathcal A \in \operatorname{Kosz}(X)\)
 \textup{(}Definition~\ref{def:koszul-chiral-algebra}\textup{)}
 is the algebra-level name for condition~\textup{(b)} on the raw
 genus-\(0\) bar surface, while the theorem records the equivalent
 twisting-morphism, unit, and twisted-tensor formulations as the
 non-tautological FTM package.
 If these genus-\(0\) equivalent conditions hold and, in addition,
 $\mathcal{A}$ lies on the higher-genus Koszul locus
...
\item \emph{Coderived off-Koszul surface
...
\item \emph{Coderived bar-degree filtration.}
 ...
 On the square-zero Koszul surface this reduces to
...
asserts the coderived statement of~\textup{(2)}, and isolates in~\textup{(4)}
the extra input needed to recover an ordinary quasi-isomorphism there.
\end{theorem}
...
\item \emph{Strict genus-\(0\) FTM surface.}
Apply the fundamental theorem of chiral twisting morphisms
\textup{(}Theorem~\ref{thm:fundamental-twisting-morphisms}\textup{)}
to the genus-\(0\) twisting datum
\((\mathcal A,C_{\mathcal A,0},\tau_{\mathcal A,0},F_\bullet)\).
The theorem gives the equivalence of the four displayed conditions:
\ref{ftm:koszul} is condition~\textup{(a)},
\ref{ftm:counit} is condition~\textup{(b)},
\ref{ftm:unit} is condition~\textup{(c)}, and
\ref{ftm:acyclic} is condition~\textup{(d)}. In particular, the
genus-\(0\) counit quasi-isomorphism is not proved by reusing
Definition~\ref{def:koszul-chiral-algebra}; it is one face of the
FTM equivalence package. The strict standard examples are checked by
...
on the quadratic regime\textup{)}. This recovers clause~\textup{(1)}.
...
\item \emph{Precise genus-$0$ locus.}
 ...
 Remark~\ref{rem:equivalent-formulations-koszul}\textup{)}. On a
 chosen chiral twisting datum this is one member of the four-way FTM
 package of
 Theorem~\ref{thm:fundamental-twisting-morphisms}: chiral Koszul
 morphism, counit quasi-isomorphism, unit weak equivalence, and
 acyclicity of the two twisted tensor products.
...
\item \emph{Candidate off-locus families.}
...
associated variety in the sense of Arakawa is conjectured to jump from
 the full nilpotent cone to the closure of a proper orbit; at that
same boundary conjecturally new off-diagonal bar classes appear and the
...
the four-way FTM package attached to genus-$0$ Koszulity
(axiom~\ref{MK:koszul}): chiral Koszul morphism, counit
quasi-isomorphism, unit weak equivalence, and acyclicity of the two
twisted tensor products are equivalent by the fundamental theorem of
...
bar-dual component. Outside that surface, one must keep the intrinsic
...
one is conditional on the shifted-symplectic hypothesis package, and
one ($\mathcal{D}$-module purity) remains partially proved
...
strict Koszul-surface hypotheses of Theorems~\textup{\ref{thm:koszul-reflection}}
...
Koszul surface, Theorem~\ref{thm:pbw-koszulness-criterion} gives PBW
diagonal concentration, and Theorem~\ref{thm:spectral-sequence-collapse}
turns it into $E_2$-collapse. On a curved or completed surface, the same
...
(Theorem~\ref{conj:ambient-complementarity};
...
complex it is, and then the Koszul-dual component. Three constructions must
remain separate.
\begin{itemize}
...
\item \textbf{Verdier dual}:
 ...
 coalgebraic Verdier dual component. When the completed cobar construction
...
\item Affine KM bar coalgebra $\to$
 ...
 with the same scalar characteristic as the level-reflected affine
 algebra.
...
this coalgebraic memory into the Verdier-dual coalgebraic component; the
...
\cA^{!,\mathrm{co}}_\infty$ as a coalgebraic Verdier-dual surface, and
...
\cA^{!,\mathrm{co}}_\infty$ is the Verdier-dual coalgebra component of
...
The analogy with power series provides the following heuristic
conjecture: the convergence radius of the categorical logarithm is
computable from the input data.
...
\item \textbf{Convergence radius from OPE poles.} The Koszul locus is
conjecturally characterized by the singularity structure of the OPE: the
...
\begin{proposition}[Sub-exponential growth under character bounds;
\ClaimStatusConditional]%
...
Assume, in addition, that its conformal character has
Cardy--Hardy--Ramanujan growth:
\[
\dim \cA_n \leq C e^{\alpha\sqrt n}
\qquad (n \gg 0)
\]
for some constants $C,\alpha>0$, and that the $p$-grading in
$E_1^{0,p}$ is linearly compatible with conformal weight: every
summand contributing to $E_1^{0,p}$ has conformal weight at most
$Ap+B$ for fixed constants $A,B$.
Then for every such bigrading on the bar complex
$\barB^{p,q}(\cA)$
the growth bound
\[
\dim E_1^{0,p} < c^p
...
\end{equation}
By the character-growth hypothesis,
$a_k := \dim \cA_k$ satisfies
$a_k \leq C\, e^{\alpha \sqrt{k}}$ for constants $C, \alpha > 0$
(Cardy--Hardy--Ramanujan: $\alpha =
\pi\sqrt{2c_{\mathrm{eff}}/3}$ in the usual modular character
case).
...
= O(\sqrt{h})$,
which is $o(h)$. Since $E_1^{0,p}$ is supported in conformal
weights $h\leq Ap+B$, its dimension is bounded by the sum of the
total conformal-weight dimensions for $h\leq Ap+B$; the logarithm
of this sum is $O(\sqrt p)+O(\log p)=o(p)$. Thus
$\dim E_1^{0,p}<c^p$ for every $c>1$ and all sufficiently large~$p$.
\end{proof}

\begin{corollary}[Reduction under character bounds;
\ClaimStatusConditional]\label{cor:finiteness-criterion-reduction}
\index{factorization bar-cobar!finiteness criterion!reduction}
Under the hypotheses of
Proposition~\textup{\ref{prop:subexponential-growth-automatic}},
Conjecture~\textup{\ref{conj:factorization-finiteness-criterion}}
reduces to the spectral sequence degeneration
condition~\textup{(i)} alone. The residual content is a geometric assertion:
...
\end{itemize}
These are not five separate theories. They are five windows onto a
single object.
...
comparison hypotheses it gives the coalgebraic Koszul-dual component
\[
