...
is a quasi-isomorphism on the Koszul locus.  The cobar functor
$\Omegach$ recovers the algebra from its bar coalgebra: on the
Koszul locus (Definition~\ref{def:koszul-locus}), the round trip
through $\barB$ and $\Omega$ loses no information.  The quasi-isomorphism is not a
formal consequence of the adjunction; it requires the geometry of
the Fulton--MacPherson compactification and the Koszulness of the
underlying chiral operad.

Off the Koszul locus, inversion persists in progressively more
structured forms: curved-central algebras via $I$-adic completion,
filtered-complete algebras via coderived categories
...
\;\xrightarrow{\;\sim\;}\; \cA
\]
is a quasi-isomorphism of chiral algebras, genus by genus and in
...
induced by the bar-cobar adjunction is a quasi-isomorphism of \emph{chiral algebras}: a morphism whose linear component induces an isomorphism on cohomology and whose higher components intertwine all $A_\infty$ operations up to coherent homotopy \textup{(}Remark~\textup{\ref{rem:qi-vs-homology-iso})}.
...
\begin{theorem}[Conditional Lagrangian characterization of chiral Koszulness;
...
self-duality $\mathrm{Vir}_c \simeq \mathrm{Vir}_{26-c}$ at
$c = 13$ gives $\kappa(\mathrm{Vir}_{13}) = 13/2 \neq 0$: the
Virasoro Koszul self-dual point is \emph{not} at critical level.
