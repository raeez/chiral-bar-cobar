...
Theorem~A gives the adjunction $\Omegach \dashv \barBch$; Theorem~B
proves that the counit $\psi \colon \Omegach(\barBch(\cA)) \to \cA$
is a quasi-isomorphism on the Koszul locus.  Inversion is the
statement that the bar complex $\barBch(\cA)$ determines~$\cA$: the
cobar functor $\Omegach$ recovers the algebra from its bar coalgebra.
This is not a formal
consequence of the adjunction; it requires the geometry of the
Fulton--MacPherson compactification and the Koszulness of the
underlying chiral operad.  On the Koszul locus
(Definition~\ref{def:koszul-locus}), the round trip through
$\barB$ and $\Omega$ loses no information.
...
(MC3, proved for all simple types via multiplicity-free $\ell$-weights,
...
duality is an equivalence.  We give five characterizations
...
\item Affine Kac--Moody coalgebra $\to$ Kac--Moody at Feigin--Frenkel dual level
...
cancellation at $c = 26$ ($\kappa = 0$) removes this scalar
...
\item \textbf{MC3}: what is the convergence domain? (all-types
categorical CG closure plus the DK comparison on the
evaluation-generated core; remaining post-CG completion packets open
beyond type~$A$, Corollary~\ref{cor:mc3-all-types})
...
\label{prop:cech-bar-intertwining}% alias for backward compatibility
