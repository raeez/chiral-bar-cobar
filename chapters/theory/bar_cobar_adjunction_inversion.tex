...
The key insight is that the passage from local data
(sections on affine opens) to global data (cohomology on~$X$)
is \emph{not} a mere chain-level computation: it carries algebraic
structure through the Moore complex functor, and this structure
is precisely an $L_\infty$ enhancement of the Lie bracket in
Beilinson--Drinfeld's pseudo-tensor category.
...
\subsection{The Moore complex and HCA structure}
\label{subsec:moore-hca}
...
cohomology $H^\bullet(X,\cA)$.  The theorem of
Malikov--Schechtman enriches this complex with higher operations:
...
$\mathcal{U}$ an affine open cover.  Then the Moore complex
$M\check{C}^\bullet(\mathcal{U};\cA)$ carries the structure of a
homotopy Lie algebra in the pseudo-tensor category of
$\cD_X$-modules---that is, a homotopy chiral algebra.

Concretely, there exist multilinear operations
\[
F_n\colon
  \mathrm{Lie}(n) \otimes \mathcal{Z}(n)
  \otimes (M\check{C})^{\otimes n}
  \longrightarrow M\check{C},
  \qquad n \geq 1,
\]
where $\mathcal{Z}(n)$ is the Eilenberg--Zilber chain complex
$\operatorname{Tot}\operatorname{Hom}_{\Delta(k)}(Z, Z^{\otimes n})$,
satisfying:
\begin{enumerate}[label=\textup{(\roman*)}]
\item $F_1$ is the identity (the differential $d_M$ on $M\check{C}$);
\item $F_2$ is the binary Lie bracket on
  $M\check{C}$, transferred from the chiral product;
\item the collection $\{F_n\}_{n \geq 1}$ defines a pseudo-tensor
  functor from the Lie Eilenberg--Zilber operad
  $\mathcal{Y}(n) = \mathcal{Z}(n) \otimes_k \mathrm{Lie}(n)$
  to the endomorphism operad of $M\check{C}$ in the pseudo-tensor
  category;
\item the $L_\infty$ relations hold: for each arity~$n$, the
  $A_\infty$/$L_\infty$ Stasheff identity is satisfied up to
  homotopies controlled by $F_{n+1}$.
\end{enumerate}
\end{theorem}
...
and elements
$a_{m_1} \in (M\check{C})^{m_1}, \ldots,
 a_{m_n} \in (M\check{C})^{m_n}$,
\[
...
\subsection{\v{C}ech--bar comparison}
...
same object---the chiral homology of $\cA$ on~$X$---and their
algebraic enhancements must be compatible.
...
There exists a quasi-isomorphism of $L_\infty$ algebras
(in the pseudo-tensor category)
\[
\Phi\colon
  \bigl(M\check{C}^\bullet(\mathcal{U};\cA),\; \{F_n\}\bigr)
  \xrightarrow{\;\sim\;}
  \bigl(\barBgeom(\cA)|_{\mathrm{Ran}(X)},\;
    \{m_n\}\bigr)
\]
that on cohomology induces the canonical isomorphism
$\check{H}^\bullet(X,\cA) \cong H_\bullet^{\mathrm{ch}}(X,\cA)$
between \v{C}ech cohomology and chiral homology.

The map $\Phi$ is constructed by the standard comparison
between \v{C}ech and de~Rham/Dolbeault resolutions, enhanced to
an $L_\infty$ morphism by homological perturbation.
\end{proposition}

\begin{remark}[\v{C}ech--bar dictionary]
\label{rem:cech-bar-dictionary}
The comparison yields a concrete dictionary:
\begin{center}
\renewcommand{\arraystretch}{1.2}
\begin{tabular}{ll}
\textbf{\v{C}ech side} & \textbf{Bar side} \\
\hline
Sections on $U_{i_0\cdots i_p}$
  & Factorization data on $\mathrm{Ran}_p(X)$ \\
\v{C}ech differential $d_{\check{C}}$
  & Bar differential $d_B$ \\
Moore normalization
  & Conilpotent filtration \\
Eilenberg--Zilber operad $\mathcal{Y}(n)$
  & Associahedra / $\mathrm{FCom}$ (genus~$0$) \\
HCA operations $F_n$
  & $A_\infty$ operations $m_n$ \\
Cosimplicial identities
  & $\partial^2 = 0$ on $\overline{\mathcal{M}}_{0,n}$ \\
\end{tabular}
\end{center}
...
$\Phi$ intertwines these combinatorics.
\end{remark}

\begin{example}[\v{C}ech HCA for the Heisenberg algebra on $\mathbb{P}^1$]
\label{ex:cech-hca-heisenberg}
Take $X = \mathbb{P}^1$ with the standard cover
$U_0 = \mathbb{A}^1 = \operatorname{Spec}k[z]$ and
$U_\infty = \mathbb{P}^1 \setminus\{0\} = \operatorname{Spec}k[z^{-1}]$,
so $U_0 \cap U_\infty = \mathbb{G}_m = \operatorname{Spec}k[z,z^{-1}]$.
The \v{C}ech complex for the Heisenberg sheaf
$\cH_k$ is the two-term complex
\[
...
Moore normalization gives $(M\check{C})^0 = \cH_k(U_0)$ and
$(M\check{C})^1 = \cH_k(\mathbb{G}_m)/\cH_k(U_\infty)$.

The binary bracket $F_2$ on $M\check{C}$ is the restriction of
the Heisenberg chiral bracket: for
$J_n = z^n\,dz \in \cH_k(U_0)$,
\[
F_2(J_m, J_n) = k\,m\,\delta_{m+n,0}\cdot\mathbf{1}.
\]
Since the Heisenberg bracket is abelian (only the central
extension is nonzero), $F_3 = 0$ and all higher $F_n$ vanish:
the \v{C}ech HCA is \emph{strict} for the Heisenberg algebra,
matching the strictness of the bar $A_\infty$ structure
(\S\ref{sec:explicit-low-degree}).
...
\begin{example}[\v{C}ech HCA for $\widehat{\mathfrak{sl}}_2$ and the Jacobiator]
\label{ex:cech-hca-sl2}
For $\cA = \widehat{\mathfrak{sl}}_2$ at level~$k$ on $\mathbb{P}^1$
with the same cover, the binary bracket $F_2$ encodes the affine
OPE.  The key difference from the Heisenberg case is that $F_2$ is
\emph{not} abelian: the Lie bracket $[e,f] = h$ produces a
nonzero Jacobiator
\[
J(e_m, f_n, h_p)
= [e_m,[f_n,h_p]] + [f_n,[h_p,e_m]] + [h_p,[e_m,f_n]]
\]
on the \v{C}ech complex, which is nullhomotopic by the
Malikov--Schechtman theorem: there exists
$J'\colon (M\check{C})^{\otimes 3} \to M\check{C}[-1]$
with $d_M J' + J' d_M = J$.  This homotopy $J'$ is
precisely the ternary operation $F_3$, which corresponds
under the \v{C}ech--bar comparison to the $A_\infty$ operation
$m_3$ on the bar complex (cf.\
