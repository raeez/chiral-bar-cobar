...
From the Chriss--Ginzburg perspective that organizes this monograph
(\S\ref{sec:categorical-logarithm},
Chapter~\ref{chap:higher-genus}), the HCA operations $\{F_n\}$
should not be presented as a sequence of maps subject to
\emph{ad hoc} compatibilities.  They are the \emph{components} of
a single Maurer--Cartan element in an appropriate convolution
dg~Lie algebra---the \v{C}ech convolution algebra
$\mathfrak{g}^{\check{C}}(\cA,\mathcal{U})$.  The key insight is
...
cohomology $H^\bullet(X,\cA)$.  Following the Chriss--Ginzburg
principle (Proposition~\ref{prop:chriss-ginzburg-structure}), we
...
\begin{remark}[Chriss--Ginzburg principle: \v{C}ech and modular convolution algebras]
\label{rem:cech-modular-parallel}
The algebra $\mathfrak{g}^{\check{C}}(\cA,\mathcal{U})$ is the
\v{C}ech incarnation of the Chriss--Ginzburg principle: every algebraic
structure in this monograph is a Maurer--Cartan element in a
convolution dg~Lie algebra.  Its modular counterpart is
...
Chriss--Ginzburg formulation makes their compatibility a statement
...
\begin{remark}[\v{C}ech--bar Chriss--Ginzburg dictionary]
...
tables of Chapter~\ref{chap:bar-tables}: the Betti numbers
