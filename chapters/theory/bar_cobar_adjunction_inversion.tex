...
Since $\mathfrak{h}$ is abelian, the chiral Koszul dual is $\mathrm{Sym}^{\mathrm{ch}}(V^*)$ with $d = 0$ and curvature $m_0 \propto -k \cdot \mathbf{1}$ (the level negates under Koszul duality; $h^\vee = 0$ for the abelian Lie algebra).  This is a \emph{commutative} chiral algebra with curvature, not the Heisenberg at negated level (AP33: $\mathcal{H}_k^! = \mathrm{Sym}^{\mathrm{ch}}(V^*) \neq \mathcal{H}_{-k}$).  The bar differential vanishes (Example~\ref{ex:ope-to-residue}), so $H^*(\bar{B}(\mathcal{H}_k)) \simeq \bar{B}(\mathcal{H}_k)$ with CE cooperad structure.
...
$\widehat{\mathcal{C}}$, producing three distinct objects
\textup{(}AP25\textup{)}:
\begin{itemize}
