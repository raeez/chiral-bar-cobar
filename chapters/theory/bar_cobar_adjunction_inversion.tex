...
\begin{proof}
Conservativity: $B_\kappa$ detects quasi-isomorphisms on the
Koszul locus by bar-cobar inversion
(Theorem~\ref{thm:bar-cobar-inversion-qi}).
Totalization preservation: the log FM compactification has contractible
strata (Mok, \cite{Mok25}); totalizations are computed fiber-by-fiber,
each acyclic.  These two inputs satisfy the hypotheses of the
Barr--Beck--Lurie theorem (\cite[Theorem~4.7.3.5]{HA}), yielding
the comparison equivalence~$\Phi$ on the Koszul locus.  Conversely,
if~$\Phi$ is an equivalence then $B_\kappa$ is conservative, which
forces bar-cobar inversion, hence Koszulness.
\end{proof}
