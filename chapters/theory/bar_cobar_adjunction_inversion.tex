...
\begin{itemize}
 ...
 \item[(H3)] \emph{Finite-dimensional graded bar pieces.} Each conformal
       weight space of $\bar{B}_X(A)$ is finite-dimensional (required for
       ordinary acyclicity to follow from coacyclicity, Positselski 2011
       \S A.5).
\end{itemize}
...
equivalence becomes a chain-level qi.
...
equivalence to chain-level quasi-isomorphism on $\mathrm{Kosz}(X)$ follows
from the $E_2$-collapse of the bar--cobar spectral sequence
\textup{(}Theorem~\ref{thm:spectral-sequence-collapse}\textup{)} plus
Positselski 2011 \S A.5 covariantly-weighted to identify coacyclicity with
acyclicity under (H3).
...
\section{Wave 13 DNA perspective: bar-cobar inversion at K3 and the Universal Phi-Trace}
\label{sec:bcinv-wave13-DNA}

\begin{remark}[Bar-cobar inversion: $\Omega(B(\cA)) \simeq \cA$ at K3]
\label{rem:bcinv-DNA-inversion-K3}
...
\label{sec:bcai-wave13-DNA-deep}

\begin{remark}[Bar-cobar inversion at $\hbar^2 = -1/8$]
\label{rem:bcai-DNA-deep-1}
The bar-cobar inversion map for the Wave~13 K3 chiral bialgebra,
\[
...
(no curvature non-triviality). Combined, the Wave~13 inversion
