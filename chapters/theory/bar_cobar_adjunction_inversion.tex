...
hold on the finite-type conilpotent bar-coalgebra surface. They do not
...
that witness for the class-$\mathsf{M}$ strand
...
Chapter~\ref{chap:chiral-center-theorem}.
...
\textup{(iii)}~MC5 class $\mathsf M$ at chain level is
Theorem~\textup{\ref{thm:mc5-class-m-chain-level-pro-ambient}} in the
pro-ambient; the lift to the bounded direct-sum ambient is the open
problem, compatible with clause~(2) \textup{(}coderived off-Koszul
