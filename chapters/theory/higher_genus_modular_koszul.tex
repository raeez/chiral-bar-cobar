...
\begin{remark}[Theorem~D as integrability condition]%
\label{rem:theorem-d-integrability}%
\index{modular characteristic theorem!as integrability condition}%
Theorem~D$_{\mathrm{scal}}$ is the \emph{zeroth level} of a
hierarchy of modular invariants
(Remark~\ref{rem:four-levels}).  The genus-$g$ bar complex
$\barB^{(g)}(\cA)$ is a \emph{curved} coalgebra:
$d_{\mathrm{fib}}^2 = \kappa \cdot \omega_g \neq 0$
(Definition~\ref{def:scalar-modular-package}).  In the
terminology of Positselski~\cite{Positselski11},
this is a CDG-coalgebra---the coalgebra analogue of a
matrix factorization---and its correct homological algebra
is the \emph{coderived category}
$\mathsf{D}^{\mathrm{co}}(\barB^{(g)}\text{-}\mathsf{comod})$
(\S\ref{subsec:coderived-ran}).

The period-corrected differential $\Dg{g}$ with $\Dg{g}^{\,2}=0$
(Convention~\ref{conv:higher-genus-differentials}) restores
nilpotency, and its cohomology
$H^*(\barB^{(g)}(\cA), \Dg{g})$---the derived fiber of the
factorization homology bundle $\int_{\Sigma_g}\!\cA$---is a
\emph{genuinely algebra-dependent} genus-$g$ invariant: for
$\widehat{\mathfrak{sl}}_{2,k}$ at integrable level,
$H^0$ at genus~$1$ recovers the space of conformal blocks
of dimension $k{+}1$ (the Verlinde number), while $\kappa = 3(k{+}2)/4$
is a single scalar that cannot distinguish levels with the same
central charge from those with different representation theories.

Thus the $\hat{A}$-genus generating
function~\eqref{eq:modular-char-gf} is the \emph{first Chern
class} of the factorization homology bundle: its Euler
characteristic, not the bundle itself.  Theorem~D guarantees the
integrability (absorbability of the curvature by tautological
period corrections) that makes the richer invariants at
Levels~$1$--$3$ well-defined.  The separately proved spectral
discriminant~$\Delta_{\cA}$
(Theorem~\ref{thm:spectral-characteristic})
and universal MC class~$\Theta_{\cA}$
(Theorem~\ref{thm:universal-theta})
begin the ascent from Level~$0$ to the full modular homotopy type.
See Conjecture~\ref{conj:categorical-modular-kd}.
\end{remark}
