...
d \;+\; [\mathfrak{K}_\cA, -],
\end{equation}
where $d$ is the internal differential on
$\widehat{\mathfrak{g}}^{\mathrm{mod,log}}_\cA$ and
$[\mathfrak{K}_\cA, -]$ is the adjoint action of the
primitive kernel via the Lie bracket of the convolution algebra.
Then $D_{\mathfrak{K}}^2 = 0$, because
$D_{\mathfrak{K}}^2(x) = [d\mathfrak{K}_\cA +
\tfrac{1}{2}[\mathfrak{K}_\cA, \mathfrak{K}_\cA],\, x] = 0$
by the primitive master equation~\eqref{eq:primitive-master-equation}
and the Jacobi identity.

The connection is filtration-preserving: since
$\mathfrak{K}_\cA \in F^1\widehat{\mathfrak{g}}^{\mathrm{mod,log}}_\cA$
(all primitive components have Euler characteristic $\geq 1$),
the adjoint action respects the pronilpotent filtration
(Theorem~\ref{thm:stable-graph-pronilpotent-completion}).
The parallel transport of $D_{\mathfrak{K}}$ along a path in
...
The second term $2\Delta\,t^2$ is the \emph{interaction
correction}.
...
+ \alpha^2 t^2$.  Adding $2\Delta\,t^2 = 16\kappa S_4\,t^2$
gives $Q_L(t)$.
\end{proof}
