...
on $\Bmod^{\log}(\cA)$: define $\mathrm{wt}(\Gamma) :=
...
Let $\Theta_\cA\in
\operatorname{MC}(\Definfmod_{\log}(\cA;\mathfrak{U}))$.  The
...
$(\gAmod, d_{\Theta^{(0)}})$.
The bar-intrinsic representative is canonical.
\end{proof}
...
\begin{remark}[Family activation pattern]
\label{rem:genus2-shell-activation}
\index{genus-two shells!activation}
The shell activation pattern diagnoses depth:
\[
\begin{array}{c|ccc|c}
& \Theta^{(2)}_{\mathrm{loop}^2}
& \Theta^{(2)}_{\mathrm{sep}\circ\mathrm{loop}}
...
\end{array}
\]
Gaussian and contact algebras have no separating-clutching shell
(the cubic shadow vanishes on bar cohomology); the planted-forest
shell is activated only when $\ell_4^{(0)}|_{H^*} \neq 0$ (contact
and mixed classes).
\end{remark}

\subsection{The renormalized comparison at genus $g \geq 2$}
\label{subsec:renormalized-comparison}
\index{Costello renormalization|textbf}
\index{genus!renormalized comparison}

The algebraic bar complex~$\barB^{(g)}(\cA)$ at genus~$g \geq 2$ is
fully determined by the inductive construction of
Theorem~\ref{thm:inductive-genus-determination}.
The question MC5 asks at genus~$g \geq 2$ is whether the
\emph{analytic} genus-$g$ amplitudes---defined by configuration-space
integrals on~$\Sigma_g$---agree with these algebraic data.
...
The amplitude~\eqref{eq:genus-g-amplitude} converges absolutely
for every stable graph~$\Gamma$ with finitely many external
insertions of bounded conformal weight.
\end{proposition}

\begin{proof}
On the compact surface~$\Sigma_g$, the Green's function
$G^g(z,w)$ has a logarithmic singularity:
$|G^g(z,w)| \leq C\,|\log|z-w||$ in local coordinates.
A product of~$k$ Green's functions satisfies
$\prod_{e} |G^g| \leq C^k \prod_e |\log|z_{s(e)} - z_{t(e)}||$.
In two real dimensions, $|\log|z||^k$ is integrable near $z = 0$
for all~$k$.  The Fulton--MacPherson compactification
$\operatorname{FM}_n(\Sigma_g)$ is a smooth manifold with corners;
on each boundary stratum, the integrand has at worst poly-logarithmic
singularities.  Poly-logarithmic singularities are integrable in
dimension~$2$, so each graph amplitude converges.
\end{proof}

\begin{remark}[No ultraviolet renormalization is needed]
\label{rem:no-uv-renormalization}
\index{renormalization!ultraviolet}
In contrast to quantum field theories in dimension~$\geq 3$,
the graph amplitudes on a Riemann surface require no counterterm
subtraction.  The superficial degree of divergence of a
subgraph~$\gamma$ with $b_1(\gamma)$ loops is
$d(\gamma) = 2b_1(\gamma)$ in two real dimensions, but
the singularity is logarithmic (not power-law), so
$d(\gamma) = 0$ effectively.  All counterterms vanish.
The Costello--Gwilliam renormalization
framework~\cite{CG17,costello-renormalization} applied to
a chiral algebra on a curve produces the \emph{same}
graph amplitudes as the unregularized integrals: the heat-kernel
parametrix $P_\varepsilon^g(z,w)$ converges to
$G^g(z,w)$ as $\varepsilon \to 0$ with no subtraction needed.
\end{remark}
...
\begin{theorem}[Analytic-algebraic comparison; \ClaimStatusProvedHere]
\label{thm:analytic-algebraic-comparison}
\index{comparison theorem!analytic-algebraic}
For a chiral algebra~$\cA$ on a smooth projective curve~$X$ of
genus~$g \geq 2$:
\begin{enumerate}[label=\textup{(\roman*)}]
\item the analytic differential
  $D_{\mathrm{an}}^{(g)}$ is well-defined
  \textup{(}Proposition~\textup{\ref{prop:2d-convergence}}\textup{)},
\item $(D_{\mathrm{an}}^{(g)})^2 = \kappa(\cA) \cdot \omega_g$
  by the Fay trisecant identity on~$\Sigma_g$ and the
...
\item $D_{\mathrm{an}}^{(g)}$ agrees with the algebraic
  bar differential~$D_{\mathrm{alg}}^{(g)}$ on the algebraic
  subspace $\cA^{\otimes n}_{\mathrm{alg}} \subset
  \cA^{\otimes n}_{\mathrm{an}}$,
  ...
  \end{equation}
\item the genus-$g$ shadow invariants extracted from
  $D_{\mathrm{an}}^{(g)}$ coincide with those from
  $D_{\mathrm{alg}}^{(g)}$:
  \[
  \kappa_{\mathrm{an}}(\cA) = \kappa_{\mathrm{alg}}(\cA),
  \qquad
  Q_{g,\mathrm{an}}(\cA) = Q_{g,\mathrm{alg}}(\cA).
  \]
\end{enumerate}
\end{theorem}

\begin{proof}
Part~(i) is Proposition~\ref{prop:2d-convergence}.
Part~(ii): the identity $(D_{\mathrm{an}}^{(g)})^2 =
\kappa \cdot \omega_g$ is a chain-level identity on
$\operatorname{FM}_n^{\log}(\Sigma_g \mathbin{|} D)$.
The argument is identical to the algebraic case
(Theorem~\ref{thm:ambient-d-squared-zero}): the square
$D^2$ decomposes into contributions from codimension-two
faces of the log-FM boundary, which cancel in pairs by
the normal-crossings transversality of Mok's compactification.
The residual curvature $\kappa \cdot \omega_g$ survives from
the prime-form monodromy, exactly as at genus~$1$.

Part~(iii): the algebraic bar differential extracts OPE residues
at collision divisors of~$\operatorname{FM}_n(X)$.
The analytic differential integrates the Green's function
$G^g(z,w)$ over the same configuration space.
Near a collision divisor $D_{\{i,j\}}$, the Green's function
has the expansion
$G^g(z_i, z_j) = -\frac{1}{2\pi}\log|z_i - z_j|^2
+ \text{smooth}$,
and the logarithmic-form residue extracts exactly the coefficient
of~$d\log(z_i - z_j)$, which is the OPE singular part.
The smooth remainder contributes to the genus-$g$ period corrections.
Since both differentials extract the same OPE residues and the
same period corrections, they agree on~$\cA_{\mathrm{alg}}$.

Part~(iv) follows from~(iii): the shadow invariants
$\kappa,\, Q_g$ are projections of~$\Theta^{(g)}$, which
agree on the algebraic subspace.
\end{proof}

\begin{remark}[What remains for MC5]
\label{rem:mc5-remaining}
\index{MC5!remaining obstacles}
The comparison theorem establishes that the algebraic and analytic
genus-$g$ bar differentials agree on $\cA_{\mathrm{alg}}$ for
all~$g$.  The extension from $\cA_{\mathrm{alg}}$ to the sewing
envelope~$\cA^{\mathrm{sew}}$
(Definition~\ref{def:sewing-envelope}) requires the
HS-sewing condition (Definition~\ref{def:hs-sewing}):
$\sum_{a,b,c} q^{a+b+c}\,\|m^c_{a,b}\|_{\mathrm{HS}}^2 < \infty$.
When this holds, the analytic bar differential extends continuously
to~$\cA^{\mathrm{sew}}$, and the coderived shadow
$Q_g^{\mathrm{an}}(\cA)$
(Definition~\ref{def:analytic-shadow}) coincides with the
algebraic shadow $Q_g^{\mathrm{alg}}(\cA)$ on the dense algebraic
core.

The obstruction to completing MC5 is therefore:
\emph{not} ultraviolet renormalization (which is vacuous in
dimension~$2$), but rather the convergence of mode sums when
sewing amplitudes along boundary circles---precisely the
HS-sewing condition.  For the Heisenberg algebra
(Conjecture~\ref{conj:heisenberg-sewing}) and lattice algebras
(Conjecture~\ref{conj:lattice-sewing}), this is the first target.
\end{remark}
