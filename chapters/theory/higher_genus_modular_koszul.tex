...
hypotheses~\textup{(a)--(b)} of
Theorem~\textup{\ref{thm:pbw-universal-semisimple}}
\textup{(}the entire standard landscape\textup{)},
...
\item \emph{Universality.}
  \begin{enumerate}[label=\textup{(\alph*)}]
  \item \emph{Cohomological\textup{:}} For algebras with generators
  of uniform conformal weight \textup{(}Heisenberg, Virasoro,
  affine Kac--Moody\textup{)}, the genus-$g$ obstruction class
  factors as
  $\mathrm{obs}_g(\cA) = \kappa(\cA) \cdot \lambda_g$
  in $H^{2g}(\overline{\mathcal{M}}_g)$ for all $g \geq 1$.
  \item \emph{Free-energy\textup{:}} For single-generator algebras
  \textup{(}Heisenberg, Virasoro, affine Kac--Moody\textup{)},
  the genus-$g$ free energy
  $F_g(\cA) = \kappa(\cA) \cdot
  \frac{2^{2g-1}-1}{2^{2g-1}} \cdot
  \frac{|B_{2g}|}{(2g)!}$
  at all genera $g \geq 1$.
  For all modular Koszul algebras, the same formula holds
  unconditionally at $g = 1$.
  \end{enumerate}
  For multi-generator algebras \textup{(}$\mathcal{W}_N$ with
  $N \geq 3$\textup{)} at $g \geq 2$, the cohomological
  factorization
  $\mathrm{obs}_g = \kappa \cdot \lambda_g$ is conjectural
  \textup{(}Open
  Problem~\textup{\ref{op:multi-generator-universality})}.
  The free-energy factorization $F_g = \kappa \cdot
  [\text{Bernoulli}]$ at $g \geq 2$ depends on whether
  the individual Hodge-type integrals
  $\int \lambda_g^{(j)}$ coincide
  \textup{(}Remark~\textup{\ref{rem:multi-generator-obs})}.
...
intertwined with Koszul duality by the complementarity
relation $\kappa(\cA) + \kappa(\cA^!) = K(\mathfrak{g})$
\textup{(}sign reversal for Kac--Moody; affine shift for
$\mathcal{W}$-algebras\textup{)}.
\end{theorem}

\begin{proof}
Part~(i)(a) is Theorem~\ref{thm:genus-universality} for
uniform-weight generators.
Part~(i)(b): for single-generator algebras, the same argument
as~(i)(a) applies.  For multi-generator algebras ($\mathcal{W}_N$,
$N \geq 3$), the cohomological factorization may split into
several Hodge-type contributions $\sum_j \kappa_j \lambda_g^{(j)}$
(Remark~\ref{rem:multi-generator-obs}).  At genus~$1$, the
single-$\lambda_1$ evaluation gives $F_1 = \kappa \cdot
\lambda_1^{\mathrm{FP}}$ unconditionally by
Theorem~\ref{thm:genus-universality}(iii).  At genus $g \geq 2$,
the identification $\lambda_g^{(j)} = \lambda_g$ for each~$j$
\textup{(}i.e., the multi-generator Hodge components all coincide
with the standard Mumford class\textup{)} is
Open Problem~\ref{op:multi-generator-universality};
conditional on this identification,
...
are needed: the centrality hypothesis of
Theorem~\ref{thm:genus-induction-strict} is satisfied automatically
by the locality axiom
(Proposition~\ref{prop:curvature-centrality-general}).
Every finite-order shadow
...
\item \emph{$\Theta_\cA$ lies in the completed algebra.}
  ...
  Crucially, the element $\Theta_\cA$ starts at genus $g = 1$,
  hence it lies in the augmentation ideal
...
$\operatorname{tr}(\Theta_{\cA^!}) = \kappa(\cA^!) \cdot \lambda_g$
\textup{(}$= -\kappa(\cA) \cdot \lambda_g$ for Kac--Moody;
$= (K(\mathfrak{g}) - \kappa(\cA)) \cdot \lambda_g$ for
$\mathcal{W}$-algebras\textup{)}.
...
\item \emph{The complementarity constant is an algebraic identity
  ...
  satisfies $c(t) + c(-t) = 2(N{-}1)(2N^2{+}2N{+}1)$ because
  the odd-degree terms in~$t$ cancel.  Since
...
\item \emph{Affine Kac--Moody.}  For $V^k(\fg)$ with
  ...
  admissible parameter values on the universal family.
...
Moreover, $D = d_{\mathrm{int}} + [\tau,-] + d_{\mathrm{sew}}
...
\end{tikzpicture}
\caption{The shadow Postnikov tower.
...
\begin{theorem}[Recursive existence and shadow tower convergence;
\ClaimStatusProvedHere]
\label{thm:recursive-existence}
\index{recursive existence!proved}
\index{shadow Postnikov tower!convergence}
...
precisely the shadow Postnikov tower
(Definition~\ref{def:shadow-postnikov-tower}).
\end{proof}
...
by construction.  Crucially, each genus-$g$ component
...
giving an infinite shadow tower.
...
the shadow tower geometry, not in the sense of a Riemannian or
...
$r_{\max} \leq 3$.
Figure~\textup{\ref{fig:genus2-stable-graphs}} illustrates
...
% ---- A: Smooth genus-2 ----
\node[gv] (A) at (0,3.4) {$2$};
\node[font=\scriptsize, below=2pt] at (A.south) {$\mathrm{A}$};
%
% ---- B: delta_irr (genus-1, one self-loop) ----
\node[gv] (B) at (1.5,3.4) {$1$};
\draw[sloop] (B) to[out=50,in=130,looseness=6] (B);
\node[font=\scriptsize, below=2pt] at (B.south) {$\mathrm{B}:\;\delta_{\mathrm{irr}}$};
%
% ---- C: delta_1 (two genus-1, one bridge) ----
\node[gv] (C1) at (2.2,2.0) {$1$};
\node[gv] (C2) at (3.0,2.0) {$1$};
\draw[pedge] (C1) -- (C2);
\node[font=\scriptsize, below=2pt] at ($(C1)!0.5!(C2) - (0,0.27)$) {$\mathrm{C}:\;\delta_1$};
%
...
\node[font=\tiny, below=14pt, text=black!60] at (D.south) {wt $S_4$};
%
...
\node[font=\tiny, below=11pt, text=black!60] at ($(E1)!0.5!(E2) - (0,0.27)$) {wt $S_3 \cdot \kappa$};
%
...
\node[font=\tiny, below=11pt, text=black!60] at ($(F1)!0.5!(F2) - (0,0.15)$) {wt $S_3^2$};
%
...
\node[font=\tiny, below=11pt, text=black!60] at ($(G1)!0.5!(G2) - (0,0.27)$) {wt $S_3^2$};
%
...
\node[font=\footnotesize, text width=9.2cm, align=center] at (4,0.7)
  {The MC relation at $(2,0)$:\;
   ...
   \;=\; 0$};
%
% ---- Legend: vertex convention ----
\node[font=\tiny, text=black!50, text width=4.5cm, align=left]
  at (1.5,0.0)
  {$\bullet$ = genus $0$;\; circled $g$ = genus $g$;\;
   $\cdot\!\!$--- = propagator $P_e$};
\end{tikzpicture}
...
free energy $F_2 = \kappa \cdot 7/(10 \cdot 24)$.
...
\item
  ...
  At genus~$3$, both~$S_4$ and~$S_5$ enter:
...
(\texttt{pixton\_shadow\_bridge.py}).
\end{proof}
...
shadow Postnikov tower (Theorem~\ref{thm:recursive-existence})
