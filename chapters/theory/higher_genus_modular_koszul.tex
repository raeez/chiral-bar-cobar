...
family: Heisenberg, free fermions, $\beta\gamma$-$bc$,
...
Definition~\ref{def:modular-koszul-chiral}
collapses to a single axiom for the standard landscape:
...
\subsection{The locality principle and the platonic form}
...
\item Theorem~D \textup{(}modular characteristic
  ...
  function\textup{)} and Theorem~H
  \textup{(}Hochschild--Koszul
  duality\textup{)} hold unconditionally.
\end{enumerate}
...
\item The shadow metric $Q_L$, shadow connection
  ...
  $\Delta(\cA) + \Delta(\cA^!) =
  6960/[(5c{+}22)(152{-}5c)]$
  \textup{(Theorem~\ref{thm:shadow-connection})} are
...
This is not a deficiency of the definition.  It is
a theorem about the structure of chiral algebras:
\emph{locality of the OPE implies locality of bar
cohomology}.  Any proposed ``genus-$g$ refinement''
of chiral Koszulness that operates at the level of
bar cohomology will collapse to genus~$0$ by the
same mechanism.
...
\emph{platonic form} of a modular Koszul chiral
...
\item the shadow Postnikov tower $\Theta_\cA^{\leq r}$
...
$R_4 = 4(\alpha^2 + 2\Delta)/(2\kappa)$ encode the cubic and quartic inputs.
The left-hand side is \emph{quadratic in~$U'$}: this is the precise sense in
which the shadow tower is algebraic of degree~$2$.  All terms at $t^r$ for
...
The shadow tower on~$L$, computed from the full multi-dimensional
...
polynomial and the shadow tower terminates finitely;
...
\item \emph{Algebraic continuation.}\enspace
  When $\rho > 1$, the shadow tower is the divergent
...
$C^{W^{(s)}}_{W^{(s)}W^{(s)}} = 0$.
Indeed, for $\cW_4$ the explicit structure constant
...
\noindent
The T-line growth rate dominates the W-line at
all values of~$c$
(ratio $\rho_T/\rho_W^{\mathrm{aut}} \sim c(5c{+}22)$),
and the $T$-channel coupling corrections cannot
close this gap.  The algebra-level growth rate is
$\rho(\cW_3) = \rho_T$
...
\textup{(}Koszul conductor
$K_N = 2(N{-}1)(2N^2{+}2N{+}1)$\textup{)}, the
...
depends only on the Virasoro data $(\kappa_T, \alpha_T, S_4^T) = (c/2, 2, 10/(c(5c{+}22)))$
and is therefore $N$-independent.}
\end{center}
...
\item If $\rho > 1$: $\varepsilon^c_s$ diverges for all~$s$.
  ...
  $\bar{t}_0$\textup{)}, so the shadow tower is the
...
when the tree-level shadow tower diverges
...
fast-growing shadow tower.

The precise relationship: the shadow tower with
...
\index{shadow Postnikov tower!Hamilton--Jacobi equation|textbf}
...
\item the shadow tower $U(x)$ as the \emph{Hamilton principal
  function}: its graph $\{(x, \nabla U(x))\}$ is a
  Lagrangian submanifold of $T^*(V)$.
\end{itemize}
The shadow Postnikov tower is the Hamilton principal
...
trajectory traces a shadow tower parametrised by the OPE data.
...
\ClaimStatusProvedHere
\index{shadow Postnikov tower!finite determination|textbf}
\index{shadow tower!finite determination|textbf}
\index{finite determination!shadow tower}
The full shadow Postnikov tower on any
...
\item By PBW propagation
  \textup{(}Theorem~\textup{\ref{thm:pbw-propagation}}\textup{)},
  the five main theorems hold at all genera.
\end{enumerate}
\end{theorem}
...
modulo the Lie conformal bracket
relations~\cite{Nish26,Vic25}
...
bigrading (the partition bound of
Theorem~\ref{thm:km-chiral-koszul}).
The five main theorems follow
from the three-tier architecture
(Theorem~\ref{thm:three-tier-architecture}).
\end{proof}
...
interactions at generic coupling.  Then the boundary
chiral algebra $\cA_\mathcal{T}$ is chirally Koszul.
\end{corollary}
...
\item \emph{Simple quotients at admissible levels.}
  ...
  may fail free strong generation when null vectors
  enter the bar-relevant range.  The precise criterion
  is the Kac--Shapovalov determinant
  \textup{(}Theorem~\textup{\ref{thm:kac-shapovalov-koszulness}}\textup{)}:
  $L_k(\fg)$ is chirally Koszul iff
  $\det G_h(k) \neq 0$ for all conformal weights~$h$
  in the bar-relevant range.
\item \emph{Coset and orbifold algebras.}
  $\operatorname{Com}(V_k(\fh), V_l(\fg))$ and
...
For class~$M$ (Virasoro, $\mathcal{W}_N$), the infinite shadow tower
