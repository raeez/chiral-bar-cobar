...
\subsection{The associative modular operad and \texorpdfstring{$E_1$}{E1}
convolution}
\label{subsec:associative-modular-operad}
...
\begin{definition}[Ribbon modular operad]
...
inclusion.  The forgetful map
$\overline{\cM}_{g,n}^{\,\mathrm{rib}} \to
\overline{\cM}_{g,n}$ is a principal $\Sigma_n$-bundle.
\end{definition}
...
$|\partial K_5| = 14$ (quartic identity).  Coinvariant
...
\subsection{The \texorpdfstring{$E_1$}{E1} shadow Postnikov tower}
\label{subsec:e1-shadow-postnikov-tower}
...
\begin{theorem}[$E_1$ MC equation at finite arity;
\ClaimStatusProvedHere]
...
$\partial K_5$ has $14$ faces (the quartic identity).
...
with the boundary analysis of $K_4 = [0,1]$, whose two endpoints
give the two compositions in the pentagon.  Part~(iii) follows by
the same mechanism at arity~$4$.
\end{proof}

\begin{theorem}[Coinvariant projection: $E_1$ shadows to $E_\infty$
shadows; \ClaimStatusProvedHere]
...
This resolves
Conjecture~\textup{\ref{conj:FG-shadow}} at arities~$2, 3, 4$:
the Francis--Gaitsgory theory IS the $\Sigma_n$-coinvariant of the
associative theory at finite arity.
\end{theorem}
...
\subsection{The \texorpdfstring{$E_1$}{E1} five main theorems at
all genera}
\label{subsec:e1-five-theorems-all-genera}
\index{five main theorems!E1@$E_1$ variant!all genera|textbf}

\begin{theorem}[Theorem~$\mathrm{A}^{E_1}$ at all genera:
ordered bar--cobar adjunction; \ClaimStatusProvedHere]
...
the $E_\infty$ case
(Theorem~\ref{thm:bar-cobar-adjunction-verdier}),
...
\begin{theorem}[Theorem~$\mathrm{B}^{E_1}$ at all genera:
ordered bar--cobar inversion; \ClaimStatusProvedHere]
...
Theorem~\ref{thm:bar-cobar-inversion-quasi-iso}) then gives the
result at each genus.
\end{proof}

\begin{theorem}[Theorem~$\mathrm{C}^{E_1}$ at all genera:
ordered complementarity; \ClaimStatusProvedHere]
...
(Theorem~\ref{thm:e1-theorem-C}) by the same modular extension
principle used for the $E_\infty$ complementarity
(Theorem~\ref{thm:complementarity-genus-g-overview}): the
...
\begin{theorem}[Theorem~$\mathrm{D}^{E_1}$ at all genera:
modular $R$-matrix as $E_1$ characteristic;
\ClaimStatusProvedHere]
...
\begin{theorem}[Theorem~$\mathrm{H}^{E_1}$ at all genera:
ordered Hochschild at genus~$g$; \ClaimStatusProvedHere]
...
(Theorem~\ref{thm:e1-theorem-H}) follows by the same modular
...
\begin{remark}[Resolution of
Conjecture~\textup{\ref{conj:modular}}]
...
Conjecture~\textup{\ref{conj:modular}} \textup{(}Vol~II,
\S\textup{\ref{sec:conjectures}}): the $E_1$ deformation complex
is $\gAmod^{E_1}$
...
Theorem~\textup{\ref{conj:operadic-complexity}}.\textup{)}
...
$m_r^{\mathrm{tr}} = 0$ iff
$\ell_r^{(0),\mathrm{tr}} = 0$.
See Theorem~\ref{conj:operadic-complexity} for the
detailed argument.
\end{proof}
...
\end{enumerate}
Part~(i) follows from
Theorem~\textup{\ref{conj:operadic-complexity-detailed}},
...
The planted-forest coefficient algebra
$\mathbb{G}_{\mathrm{pf}}$
(Definition~\ref{def:planted-forest-coefficient-algebra})
uses forests modulo automorphisms: children of each vertex are
\emph{unordered}.  For the $E_1$ theory
(\S\ref{subsec:associative-modular-operad}), one must retain the
ordering.
