...
Let $\cA$ be a modular Koszul chiral algebra.  The primitive flat
connection $D_{\mathfrak{K}}$ descends to a flat connection
...
\item The flat connection $D_{\mathfrak{K}}$ preserves the
  cohomological filtration and descends to
  $\nabla_\cA := d + [K_{0,2}^\cA, -]|_{H^\bullet}$
  on the $E_2$-page.  The higher primitive corollas
  $K_{0,n}^\cA$ for $n \geq 3$ contribute to the
  higher-order corrections of $\nabla_\cA$ via transferred
...
(ii)~The primitive flat connection
$D_{\mathfrak{K}} = d + [\mathfrak{K}_\cA, -]$ acts on
the bar complex $\Barch^{(g)}(\cA)$ over
$\overline{\mathcal{M}}_g$.  Since $\mathfrak{K}_\cA$ has
strictly positive Euler characteristic,
$[\mathfrak{K}_\cA, -]$ shifts the filtration by at least~$1$.
On the $E_2$-page (which equals the $E_\infty$-page by
Koszulness), the leading term of $D_{\mathfrak{K}}$ is
$d + [K_{0,2}^\cA, -]$, which is a genuine connection on
$H^\bullet(\Barch^{(g)})$.  The higher terms contribute
order-by-order via the transferred
...
restriction of $D_{\mathfrak{K}}$ to
...
$\delta_i$ is computed by restricting $D_{\mathfrak{K}}$
to a formal neighborhood of $\delta_i$.  The restriction of
$\mathfrak{K}_\cA$ to $\delta_i$ factorizes via the clutching
