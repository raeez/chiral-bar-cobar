...
\begin{remark}[From OPE to conformal blocks]
...
the primitive master equation.
The genus-$g$ free energy is the Feynman amplitude sum
$F_g = \kappa(\cA) \cdot \lambda_g^{\mathrm{FP}}$
(Theorem~\ref{thm:conformal-block-reconstruction}).
\end{remark}
...
$\mathbf{G}$ & $2$
  & $S_{\mathrm{eff}}(\cA) = \kappa \cdot S_{\mathrm{Polyakov}}$.
    The Polyakov formula captures the \emph{full} shadow tower.
...
\index{shadow Postnikov tower!effective action}
The shadow tower determines a decomposition of the effective
action:
...
\begin{proof}
The shadow Postnikov tower gives
...
\index{shadow Postnikov tower!Polyakov dictionary}
\begin{center}
\small
\renewcommand{\arraystretch}{1.25}
\begin{tabular}{@{}ll@{}}
\toprule
\emph{Polyakov programme} & \emph{Shadow tower} \\
\midrule
Polyakov formula
...
Ghost sector
  & Koszul dual $\cA^!$
    \textup{(Thm~\ref{thm:bar-cobar-inversion})} \\
BPZ equations
  & Shadow connection
    $\nabla^{\mathrm{sh}}$
...
\end{center}
\noindent
The shadow tower is the algebraic completion of the Polyakov
...
\'Etale base change preserves simple normal crossings:
if $Y \to X$ is \'etale and $D \subset X$ is snc, then
$Y \times_X D \subset Y$ is snc (\'etale-local property
of normal crossings; cf.\ \cite[IV.2.3.2]{EGA}).
Applying this to the fibre product
$\operatorname{FM}_n^{\mathrm{ord}} =
\operatorname{Conf}_n^{\mathrm{ord}} \times_{\operatorname{Conf}_n}
\operatorname{FM}_n$ gives part~(i).
