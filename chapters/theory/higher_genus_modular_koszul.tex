...
level direction, but not yet proved to be a function of
$\kappa(\cA)$ alone at higher genus: explicitly
...
\begin{theorem}[Conditional multi-generator universality]
\label{thm:multi-generator-universality}
\ClaimStatusConditional
\index{multi-generator obstruction!universality theorem|textbf}%
\index{obstruction coefficient!multi-generator universality}%
\index{Faber--Pandharipande formula!multi-generator}%
Let $\cA$ be a modular Koszul chiral algebra whose
genus-completed minimal-model MC element satisfies the
strong scalar ansatz
\begin{equation}\label{eq:multigen-strong-scalar-ansatz}
\Theta_{\cA}^{\min}
\;=\;
\kappa(\cA) \cdot \eta \otimes \Lambda,
\qquad
\Lambda = \sum_{g \geq 1}\lambda_g,
\end{equation}
with strong generators of conformal weights $h_1, \ldots, h_r$.
Then
\begin{equation}\label{eq:multi-generator-universality}
\mathrm{obs}_g(\cA) \;=\; \kappa(\cA) \cdot \lambda_g
\qquad \text{in } H^{2g}(\overline{\mathcal{M}}_g)
\end{equation}
for all $g \geq 1$, where
$\kappa(\cA) = \sum_{i=1}^r \kappa_{h_i}(\cA)$ is the total
modular characteristic.
In particular, the free energy satisfies
$F_g(\cA) = \kappa(\cA) \cdot \lambda_g^{\mathrm{FP}}$.
This hypothesis is proved on the uniform-weight scalar lane and
remains open in general for multi-weight families.
\end{theorem}

\begin{proof}
The obstruction class $\mathrm{obs}_g$ is the $(g,0)$-component
of the minimal-model MC element, extracted by the normalized
cyclic trace $\operatorname{tr}(\eta) = 1$:
\[
\mathrm{obs}_g(\cA)
\;=\;
\operatorname{tr}\!\bigl(\Theta_{\cA}^{\min,(g,0)}\bigr)
\;=\;
\kappa(\cA)\,\operatorname{tr}(\eta)\,\lambda_g
\;=\;
\kappa(\cA) \cdot \lambda_g.
\]
The conformal weights $h_1, \ldots, h_r$ enter only through
the partial modular characteristics
$\kappa_{h_i}(\cA)$, whose sum is~$\kappa(\cA)$.
\end{proof}

\begin{remark}[Scope and hypotheses]%
\label{rem:multi-gen-scope}%
Theorem~\ref{thm:multi-generator-universality} requires the
strong scalar ansatz~\eqref{eq:multigen-strong-scalar-ansatz}.
What is proved for algebraic families by
Theorem~\ref{thm:algebraic-family-rigidity} is only the weaker
level-direction concentration
$\Theta_{\cA}^{\min} = \eta \otimes \Gamma_{\cA}$.
The Virasoro algebra ($r=1$) and Kac--Moody algebras
are special cases; the theorem extends to principal
$\mathcal{W}$-algebras $\mathcal{W}_N$ with $r = N-1$
generators of weights $2, 3, \ldots, N$ if the additional
tautological-purity statement
$\Gamma_{\cA} = \kappa(\cA)\Lambda$ can be established.
That additional step is precisely the content of
