...
proved at all genera only on the uniform-weight lane; see
Theorem~\ref{thm:genus-universality},
...
shadow tower and by the spectral/periodic invariants.
\end{remark}
...
uniform-weight lane; the non-scalar shadow tower
...
\index{gauge invariance!of genus element|textbf}%
Let\/ $\cA$ be a modular Koszul chiral algebra on the standard
Lie-theoretic landscape, with strong generators of conformal
weights $h_1, \ldots, h_r$, and write
\[
\Theta_{\cA}^{\min}
\;=\;
\eta \otimes \Gamma_{\cA},
\qquad
\Gamma_{\cA} \in \widehat{\Gmod},
\]
for the one-channel minimal-model class furnished by
Theorem~\ref{thm:algebraic-family-rigidity}.  Assume in addition
that the tautological coefficient satisfies the scalar-purity
identity
\[
\Gamma_{\cA}
\;=\;
...
$\kappa(\cA) = \sum_{i=1}^r \kappa_{h_i}(\cA)$
is the total modular characteristic.
The free energy satisfies\/
$F_g(\cA) = \kappa(\cA) \cdot \lambda_g^{\mathrm{FP}}$
for all\/ $g \geq 1$.
At genus~$1$, the cohomological and free-energy identities hold
unconditionally by Theorem~\ref{thm:genus-universality}.
\end{theorem}

\begin{proof}
The displayed identity for $\Gamma_{\cA}$ gives
\[
\Theta_{\cA}^{\min,(g,0)}
\;=\;
\kappa(\cA)\,\eta \otimes \lambda_g
\]
for every\/ $g \geq 1$.  Taking cyclic trace yields
\[
\mathrm{obs}_g(\cA)
\;=\;
...
\kappa(\cA)\,\lambda_g.
\]
The free-energy formula follows by the same integration used on
the uniform-weight scalar lane:
$F_g(\cA)=\int_{\overline{\mathcal{M}}_g}
\mathrm{obs}_g(\cA)
=\kappa(\cA)\lambda_g^{\mathrm{FP}}$.
\end{proof}

\begin{remark}[What the rigidity argument does and does not prove]%
\label{rem:multi-gen-independence}%
The ingredients that suggest multi-generator universality are
pairwise independent.
Step~(A), proved in
Theorem~\ref{thm:algebraic-family-rigidity}, gives only the
one-channel factorization
$\Theta_{\cA}^{\min}=\eta\otimes\Gamma_{\cA}$.
Step~(B), the parity vanishing of the Kuranishi map on the odd
class $s^{-1}\eta$, shows that this one-dimensional cyclic
direction is unobstructed; it does \emph{not} identify the
tautological coefficient~$\Gamma_{\cA}$.
Step~(C), the Heisenberg calculation, calibrates the special
scalar solution
$\Gamma_{\mathcal{H}_\kappa}=\kappa\Lambda$ on the uniform-weight
lane.

What remains open is the tautological-purity step
\[
\Gamma_{\cA}
\stackrel{?}{=}
\kappa(\cA)\Lambda
\]
for general multi-weight families.
The decomposition theorem below isolates a shadow correction
$\Delta_g(\cA)$, but its integrated vanishing is obtained only
\emph{after} imposing the scalar free-energy formula at genus~$g$.
It should therefore be read as a conditional consequence of the
scalar hypothesis, not as a proof of the stronger class identity
$\Gamma_{\cA}=\kappa(\cA)\Lambda$.
That missing identification is exactly
...
itself identify $\Gamma_{\cA}$ with $\kappa(\cA)\Lambda$ or
resolve Open Problem~\textup{\ref{op:multi-generator-universality}}.
At genus~$2$\textup{:}
