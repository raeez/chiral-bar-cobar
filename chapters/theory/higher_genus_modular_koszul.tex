...
(shadows are $\sigma$-antisymmetric because $\kappa$ is linear
in $k + h^\vee$).
...
\item \emph{Perturbative layer \textup{(}arities $2$--$3$\textup{)}.}
  ...
  \lambda_g^{\mathrm{FP}},
  \qquad
  \Delta^{(3,0)}\big|_T = -K_\fg \cdot \varrho(\fg),
  \end{equation}
...
\begin{proof}
(i) On the affine primary line,
$\kappa = td/(2h^\vee)$ and the Killing $3$-cocycle
$\omega(a,b,c) = \kappa([a,b], c)$ are both proportional to
$t = k + h^\vee$.  Since $\sigma$ sends $t \mapsto -t$:
$\sigma(\operatorname{Sh}_r) = -\operatorname{Sh}_r$ at every arity,
giving $\Delta^{(r,g)} = 0$.
(ii) Write $K_\fg := c_{\cW} + c_{\cW}^!$ for the Verdier
complementarity constant of~$\cW(\fg)$
(Computation~\ref{comp:ff-thooft-involution}).
For type~$A$: $K_{\mathfrak{sl}_N} = 2(N-1) + 4(N^2\!-1)N = N^2(2N-2)+\cdots$,
verified $K_{\mathfrak{sl}_2} = 26$, $K_{\mathfrak{sl}_3} = 100$
(matching $c_{\mathrm{Vir}} + c_{\mathrm{Vir}}' = 26$ and
$c_{\cW_3} + c_{\cW_3}' = 100$).
The formula $K_\fg = 2r + 4dh^\vee$ and the identification
$K_\fg\cdot\varrho(\fg) = \kappa_{\cW} + \kappa_{\cW}^!$
are verified for all type~$A$ ($\mathfrak{sl}_2$ through
$\mathfrak{sl}_6$); for types~$B$, $C$, $D$, $G$, $F$, $E$
the formula requires explicit verification of the Verdier
complement at the $\cW$-algebra level,
which is ongoing (Remark~\ref{rem:ds-tower-non-a}).
...
$K_\fg = c_{\cW} + c_{\cW}^!$.  For type~$A$, the formula
$K_{\mathfrak{sl}_N} = 2(N\!-\!1) + 4(N^2\!-\!1)N$
is verified computationally ($N = 2,\ldots,6$) and follows from
the explicit $\cW_N$ central charge formula
$c + c' = 2(N\!-\!1)\bigl(1 + N(N\!+\!1)\bigr)$.
For types~$B$, $C$, $D$: the $\cW$-algebra Verdier complement
involves the Kac--Roan--Wakimoto central charge and the
non-principal DS structure; the formula $K = 2r + 4dh^\vee$
should be treated as conjectural outside type~$A$ until
the Verdier complement for non-simply-laced $\cW$-algebras
is established.
\end{remark}
...
The \emph{shadow metric on~$L$} is the quadratic form
