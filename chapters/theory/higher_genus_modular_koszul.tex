...
one-dimensional primary subspace with coordinate~$x$, curvature
$\kappa \neq 0$, cubic coefficient~$\alpha$, and total quartic
coefficient~$S_4$.  Define the \emph{critical discriminant}
\begin{equation}\label{eq:critical-discriminant}
  \Delta \;:=\; 8\,\kappa\,S_4.
\end{equation}
Then every shadow coefficient $S_r$ \textup{(}$r \geq 4$\textup{)}
factors as
\begin{equation}\label{eq:universal-factorization}
  S_r \;=\; \Delta \cdot R_r(\alpha, \kappa),
\end{equation}
where $R_r \in \mathbb{Q}(\alpha, \kappa)$ is determined
recursively by
\begin{equation}\label{eq:single-line-inversion}
  S_r
  \;=\;
  -\frac{P}{2r}
  \sum_{\substack{j + k = r + 2 \\ 2 \leq j \leq k}}
  c_{jk}\,jk\,S_j\,S_k,
  \qquad P = \frac{2}{\kappa},\quad
  c_{jk} = \begin{cases} 1 & j < k \\ 1/2 & j = k,\end{cases}
\end{equation}
and is nonvanishing for generic $(\alpha, \kappa)$.
The first values:
...
\begin{array}{rclrcl}
  ...
  \\[4pt]
  R_6 &=& \displaystyle
  \frac{-(8\kappa S_4 - 45\alpha^2)}{48\,\kappa^3},
  &\quad
  R_7 &=& \displaystyle
  \frac{9\alpha\,(8\kappa S_4 - 21\alpha^2)}{56\,\kappa^4}.
\end{array}
\end{equation}
Consequently, on any one-dimensional primary line,
$r_{\max} \in \{2,\, 3,\, \infty\}$.
\end{theorem}

\begin{proof}
The recursion~\eqref{eq:single-line-inversion} is the
projection of the all-arity master equation
(Proposition~\ref{prop:master-equation-from-mc}) onto~$L$,
using $\{S_j x^j, S_k x^k\}_H = jk\,P\,S_j S_k\,x^{j+k-2}$
and the eigenvalue identity
$\nabla_H(x^r) = (\kappa x)(2/\kappa)(r x^{r-1}) = 2r\,x^r$.

\smallskip\noindent
\emph{Universal factorization.}\enspace
\emph{Base} ($r = 4$): the only pair is $(3,3)$, giving
$S_4 = -\tfrac{1}{2} \cdot 9P\alpha^2/(2 \cdot 4) + Q
= -9\alpha^2/(8\kappa) + Q = \Delta/(8\kappa)$
where $Q = S_4 - S_4^{\mathrm{der}}$ is the direct contact
contribution.
\emph{Induction} ($r \geq 5$): every summand in~\eqref{eq:single-line-inversion} is
$S_j S_k$ with $j + k = r + 2$, hence $\max(j,k) \geq 4$.
By hypothesis $S_j = \Delta \cdot R_j$ for every $j \geq 4$,
so $S_j S_k = \Delta \cdot (\cdots)$ and
$S_r = \Delta \cdot R_r$.

\smallskip\noindent
\emph{Three regimes.}\enspace
\textbf{(1)}~$\Delta = 0$: immediate from~\eqref{eq:universal-factorization}.
\textbf{(2)}~$\alpha = 0$, $S_4 \neq 0$: odd
shadows vanish by parity---if $r$ is odd then $j + k = r + 2$
is odd, so one of $j, k$ is odd, and the odd factor vanishes by
induction (base: $S_3 = \alpha = 0$).  Even shadows persist:
$S_6 = -4S_4^2/(3\kappa) \neq 0$, and the
bracket $\{S_4 x^4,\,S_{2n} x^{2n}\}_H$ propagates
nonvanishing inductively.
\textbf{(3)}~$\alpha \neq 0$, $\Delta \neq 0$:
$R_5 = -3\alpha/(10\kappa^2) \neq 0$, and
for $r \geq 6$ the quotient $R_r$ has
numerator of degree $r - 3$ in~$(\alpha, S_4)$ with distinct
leading $\alpha$-power at each~$r$, so the vanishing loci of
different $R_r$ intersect in codimension $\geq 2$; generically
all $S_r \neq 0$.
\end{proof}

\begin{example}[Virasoro verification]
\label{ex:virasoro-dichotomy-verification}
For $\mathrm{Vir}_c$: $\kappa = c/2$, $\alpha = 2$,
$S_4 = 10/[c(5c{+}22)]$.  The critical discriminant
$\Delta = 8 \cdot \tfrac{c}{2} \cdot \tfrac{10}{c(5c+22)}
= \tfrac{40}{5c+22}$
is nonzero for $c \neq -22/5$, confirming
class~$\mathbf{M}$.
The factorization $S_r = \Delta \cdot R_r$ is verified
through $r = 11$ by $32$ symbolic tests
\textup{(}\texttt{test\_single\_line\_dichotomy.py}\textup{)}.
\end{example}

\begin{remark}[Stratum separation and the contact class]
...
The dichotomy restricts the achievable finite depths on a
single primary line to $r_{\max} \in \{2, 3\}$.  The contact
class $\mathbf{C}$ ($r_{\max} = 4$) requires a second
mechanism: \emph{stratum separation}.
...
lives in a charged sector $\Defcyc^{(q)}$, $q \neq 0$.
The H-Poisson self-bracket maps
$\Defcyc^{(q)} \otimes \Defcyc^{(q)} \to \Defcyc^{(2q)}$,
but $\dim \Defcyc^{(2q)} = 0$ for rank-one systems:
there are not enough generators to carry charge-$2q$
cyclic cochains.  The quartic pump does not activate.
This is rank-one rigidity
(Theorem~\ref{thm:betagamma-rank-one-rigidity})
as stratum separation.

The complete landscape constraint:
on a line, $r_{\max} \in \{2, 3, \infty\}$;
off a line via stratum separation, $r_{\max} = 4$.
The interval $5 \leq r_{\max} < \infty$ requires charged
strata $\Defcyc^{(q)}, \Defcyc^{(2q)}, \ldots,
\Defcyc^{(nq)}$ with $\dim \Defcyc^{(nq)} = 0$ for
$n$ large enough but $\dim \Defcyc^{(mq)} > 0$ for
$m < n$---a rank-$n$ rigidity not realised by any
standard family.
\end{remark}
