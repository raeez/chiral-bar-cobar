...
$\Theta_\cA \in \MC(\gAmod)$, the shadow Postnikov tower,
...
\begin{theorem}[Descent to the conformal block connection;
\ClaimStatusProvedHere]
\label{thm:primitive-flat-descent}
\index{conformal block connection!from primitive kernel|textbf}
...
$H^\bullet(\Barch^{(g)}(\cA))$, viewed as a local system over
...
\item The Koszul property implies $E_2$-degeneration of the
  ...
  so $H^\bullet(\Barch^{(g)}(\cA))$ is a locally free sheaf
...
the bar complex $\Barch^{(g)}(\cA)$ over
...
For $\cA = \widehat{\mathfrak{g}}_k$, this is the KZ connection
by Theorem~\ref{thm:shadow-connection-kz}.
...
\begin{theorem}[Conformal block reconstruction from the
primitive kernel; \ClaimStatusProvedHere]
\label{thm:conformal-block-reconstruction}
\index{conformal block!reconstruction from primitive kernel|textbf}
\index{primitive kernel!conformal block reconstruction}
...
\item The generating function is determined by the primitive
  ...
  \end{equation}
  where $\kappa(\cA) = \operatorname{tr}_{\mathrm{cyc}}(K_{1,1}^\cA)$
  is the cyclic trace of the genus-one primitive corolla.
\item The first Chern class of the determinant line bundle
  $L_\cA := \det R\pi_{g*}\Barch^{(g)}(\cA)$ is
  \begin{equation}
  \label{eq:det-line-from-kernel}
  c_1(L_\cA)
  \;=\;
  \operatorname{tr}_{\mathrm{cyc}}(K_{1,1}^\cA)
  \cdot \lambda,
  \end{equation}
...
\item At integrable levels, $H^0$ of the flat bundle
  ...
  KZ/Hitchin connection.
\end{enumerate}
\end{theorem}

\begin{proof}
(i)~The cyclic trace of $\Theta_\cA^{(g)}$ is the genus-$g$
obstruction $\operatorname{obs}_g(\cA) = \kappa(\cA) \cdot
\lambda_g$ by Theorem~D
...
is the conformal block space by
Remark~\ref{rem:theorem-d-integrability} and the
genus-$0$ identification with the KZ connection
(Theorem~\ref{thm:shadow-connection-kz}).
\end{proof}

\begin{theorem}[Perturbative quantization of the chiral OPE;
\ClaimStatusProvedHere]
\label{thm:deformation-quantization-ope}
\index{deformation quantization!chiral OPE|textbf}
\index{Feynman calculus!from primitive kernel|textbf}
...
\item \emph{Classical field theory.}
  ...
  of $\cA$.
\item \emph{One-loop quantization.}
  The genus-one correction $K_{1,1}^\cA$ is the one-loop
  Feynman integral: the propagator $P_\cA$
  \textup{(}Theorem~\textup{\ref{thm:modular-propagator-existence})}
  contracts the BV self-sewing of the cubic vertex:
  \begin{equation}
  ...
  \end{equation}
  This is a single tadpole diagram: one cubic vertex
  $K_{0,3}^\cA$ with one pair of legs closed into a loop by
  the BV contraction $\Delta_{\mathrm{ns}}$, and the resulting
  cocycle contracted with the propagator~$P_\cA$.
  Its cyclic trace is the modular characteristic:
  \begin{equation}
  \label{eq:kappa-propagator-formula}
  \kappa(\cA)
  \;=\;
  -\operatorname{tr}_{\mathrm{cyc}}\bigl(
  P_\cA \circ \Delta_{\mathrm{ns}}(K_{0,3}^\cA)
...
(ii)~At genus~$1$, the separating bracket $[K_{0,a}, K_{0,b}]$
requires $a, b \geq 3$ by stability, giving total arity $\geq 6$;
no such term contributes at arity~$1$.  The equation reduces to
$dK_{1,1} + \Delta_{\mathrm{ns}}(K_{0,3}) = 0$, whose unique
solution modulo gauge is
$K_{1,1} = -P_\cA \circ \Delta_{\mathrm{ns}}(K_{0,3})$.
The formula~\eqref{eq:kappa-propagator-formula} follows by
applying $\operatorname{tr}_{\mathrm{cyc}}$.

(iii)~The genus-two equation is the projection of
$d\mathfrak{K} + \mathfrak{K}\star\mathfrak{K} = 0$ to
...
The pre-Lie product $\star$ encodes both the tree-level
composition and the loop contraction in a single operation.
Its antisymmetrization $[a,b] = a \star b - b \star a$
is the Lie bracket (tree-level interaction); its symmetric part
$a \star b + b \star a$ is the self-sewing
$\Delta_{\mathrm{ns}}$ (loop production).  In the genus-$2$
shell, $[K_1,K_1]$ vanishes by antisymmetry for rank-one
families, but $\Delta_{\mathrm{ns}}(K_1)$ does not: the loop
channel survives when the tree channel is trivial.
At the $E_1$ level
(Remark~\ref{rem:e1-primitive-kernel}),
$K_{0,2}^{\cA,E_1} = r(z)$ is the classical $r$-matrix, and
the CYBE is the $E_\infty$ shadow of the $E_1$ pre-Lie
equation at arity~$3$: the passage
$\star_{E_1} \to [-,-]$ is the passage from quantum group
to classical limit.
\end{remark}

\begin{remark}[The circle closes]
\label{rem:circle-closes}
\index{primitive kernel!the closed circle}
The primitive kernel $\mathfrak{K}_\cA$ closes the circle
from chiral algebra to conformal field theory:
\[
\cA
\;\xrightarrow{\;\text{Ch}_\infty\text{-model}\;}
A^{\mathrm{ch}}_\infty
\;\xrightarrow{\;\text{primitive restriction}\;}
\mathfrak{K}_\cA
...
\]
Every step is controlled by the equation
$d\mathfrak{K}_\cA + \mathfrak{K}_\cA \star
\mathfrak{K}_\cA = 0$:
\begin{itemize}
\item the equation itself is flatness ($D_\Theta^2 = 0$);
\item the genus-$0$ restriction is Arnold/factorization coherence;
\item the genus-$1$ trace is the modular characteristic
  $\kappa(\cA)$;
\item the boundary monodromy is the shell recursion;
\item the generating function is the $\widehat{A}$-genus;
\item the determinant line is $L_\cA$ with
  $c_1 = \kappa \cdot \lambda$;
\item the global flat sections are conformal blocks.
\end{itemize}
The scalar package (Theorems~A--D), the spectral package
($\Delta_\cA$, $\delta_\cA$), the branch BV action
($S_\cA^{\mathrm{br}}$), and the conformal block reconstruction
($\nabla_\cA$) are successive extractions from one object:
the primitive kernel $\mathfrak{K}_\cA$ in
$\operatorname{Prim}^{\log}_{\mathrm{mod}}(\cA)$.
\end{remark}

\begin{conjecture}[Conformal block half-form]
\label{conj:conformal-block-half-form}
\ClaimStatusConjectured
\index{conformal block!half-form conjecture}
\index{metaplectic half-density!conformal block identification}
...
$L_\cA^{1/2}$ of the conformal block line bundle:
$\delta_\cA^2 = \Delta_\cA = c_\bullet(V_\cA)$,
where $V_\cA$ is the virtual bar family in
$K_0(\overline{\mathcal{M}}_g)$.  The branch BV master action
...
\begin{remark}[The converse at all arities]
\label{rem:operadic-complexity-converse}
\index{operadic complexity!converse direction}
...
injective on cyclic cochains (HPL tree formula), so:
...
one-dimensional primary subspace with coordinate~$x$, shadow data
$\mathrm{Sh}_r|_L = S_r\,x^r$, curvature
$S_2 = \kappa/2$, and cubic coefficient
$S_3 =: \alpha$.  The \emph{shadow metric on~$L$} is the
quadratic form
\begin{equation}\label{eq:shadow-metric}
  ...
  4\kappa\alpha\,t
  \;+\;
  \bigl(\alpha^2 + 16\kappa^2 S_4\bigr)\,t^2
  \;\in\; k(c)[t].
\end{equation}
...
Set $P = 2/\kappa$ and define the \emph{weighted shadow
generating function}
\[
  H(t)
  \;:=\;
  \sum_{r \geq 2} r\,S_r\,t^r.
\]
...
\begin{equation}\label{eq:riccati-algebraic-relation}
  ...
  t^4\,Q_L(t).
\end{equation}
The shadow generating function is therefore algebraic
of degree~$2$ over $k(c)[t]$:
...
Then $a_0 = 2S_2 = \kappa$, $a_1 = 3\alpha$,
...
at $m = 0$, $a_0^2 = \kappa^2 = (2\kappa)^2/4$;
at $m = 1$, $2a_0 a_1 = 6\kappa\alpha$;
at $m = 2$,
$a_1^2 + 2a_0 a_2 = 9\alpha^2 + 8\kappa S_4$.
These match the coefficients of $Q_L(t)/4$ at orders~$0$,
$1$, $2$ (the factor~$4$ arises from
$H^2 = t^4 F^2 = t^4 Q_L$,
so $F^2 = Q_L$ and $4F^2 = 4Q_L$---one verifies the
identification directly from the explicit
formula~\eqref{eq:shadow-metric}).
...
The first term $(2\kappa + \alpha\,t)^2$ is the
\emph{Gaussian envelope}: a perfect square, the shadow metric of
the truncated tower with $S_r = 0$ for $r \geq 4$.
The second term $2\kappa\Delta\,t^2 = 16\kappa^2 S_4\,t^2$
...
\begin{proof}
$(2\kappa + \alpha t)^2 = 4\kappa^2 + 4\kappa\alpha\,t
+ \alpha^2 t^2$.  Adding $2\kappa\Delta\,t^2 = 16\kappa^2 S_4\,t^2$
gives $Q_L(t)$.
\end{proof}
...
On a primary line~$L$, the shadow tower has intrinsic OPE data at arities $2$, $3$, $4$ only.  All shadow coefficients $S_r$ for $r \geq 5$ are determined by the MC recursion~\eqref{eq:single-line-inversion} from the three initial values $(\kappa, \alpha, S_4)$.  Equivalently, the shadow metric $Q_L$ is determined by three parameters, and its square root $\sqrt{Q_L}$ generates all higher shadows without further OPE input.
\end{proposition}

\begin{proof}
Immediate from the closed form: $Q_L(t) = (2\kappa)^2 + 4\kappa\alpha\,t + (\alpha^2 + 16\kappa^2 S_4)\,t^2$ depends on exactly $(\kappa, \alpha, S_4)$, and Theorem~\ref{thm:riccati-algebraicity} gives $S_r = [t^{r-2}]\sqrt{Q_L} / r$ for all $r \geq 2$.
\end{proof}

\begin{remark}[Inherited vs intrinsic quartic]
\label{rem:inherited-vs-intrinsic}
\index{shadow tower!inherited vs intrinsic}
The MC recursion from $(\kappa, \alpha)$ alone (without quartic input) produces a ``free shadow tower'' with $S_4^{\mathrm{free}} = -9\alpha^2/(8\kappa)$ (the inherited quartic from the cubic self-bracket).  For the Virasoro: $S_4^{\mathrm{free}} = -9/(2c)$, while $S_4^{\mathrm{actual}} = 10/[c(5c{+}22)]$.  The difference $S_4^{\mathrm{actual}} - S_4^{\mathrm{free}} = (45c{+}218)/[2c(5c{+}22)]$ is the \emph{intrinsic quartic}: the OPE contribution not captured by the cubic alone.  For affine algebras, this intrinsic quartic exactly cancels the inherited one (Jacobi identity), giving $\Delta = 0$.
\end{remark}

\begin{proposition}[Alternating signs]
\label{prop:alternating-signs}
\ClaimStatusProvedHere
\index{shadow tower!sign pattern}
For $c > 0$ and $r \geq 4$: $\operatorname{sgn}(S_r(c)) = (-1)^r$.  The signs alternate because the interaction correction $2\kappa\Delta\,t^2 > 0$ in the Gaussian decomposition gives a binomial expansion with alternating contributions.  The sign pattern exhibits a phase drift at arity $r \approx 2\pi/|\pi - \arg(t_0)|$, where $t_0$ is the nearest complex zero of~$Q_L$, causing occasional sign breaks at high arity.
\end{proposition}
\begin{proof}
From $\sqrt{Q_L} = (2\kappa + \alpha t)\sqrt{1 + 2\kappa\Delta\,t^2/(2\kappa+\alpha t)^2}$ and the binomial expansion $\sqrt{1+u} = 1 + u/2 - u^2/8 + \dotsb$ with $u > 0$, the even-order corrections are negative, giving $(-1)^{n}$ for the $n$-th correction to $[t^{n}]\sqrt{Q_L}$.  Tracking through $S_r = [t^{r-2}]\sqrt{Q_L}/r$ yields $(-1)^r$ for $r \geq 4$.
\end{proof}
...
\item \emph{$\Delta = 0$, $\alpha \neq 0$
  ...
  $Q_L = (2\kappa + \alpha\,t)^2$, a perfect square.
  $H(t) = t^2(2\kappa + \alpha\,t)$.
...
When $\Delta = 0$: $Q_L = (2\kappa + \alpha t)^2$
is a perfect square, so $\sqrt{Q_L} = 2\kappa + \alpha t$
is polynomial and $H(t) = t^2(2\kappa + \alpha t)$
terminates.  $\alpha = 0$ gives $r_{\max} = 2$;
$\alpha \neq 0$ gives $r_{\max} = 3$.

When $\Delta \neq 0$: the
discriminant of $Q_L$ as a polynomial in~$t$ is
$(4\kappa\alpha)^2 - 4 \cdot (2\kappa)^2
\cdot (\alpha^2 + 16\kappa^2 S_4) = -2(2\kappa)^2\Delta
\neq 0$,
...
$Q_L = 4\kappa^2 + 16\kappa^2 S_4\,t^2$ is even in~$t$,
so $\sqrt{Q_L}$ is even, and $H(t)$ has only even powers,
giving $S_{2n+1} = 0$.

Full pump: $R_5 = -3\alpha/(10\kappa^2) \neq 0$;
...
\begin{remark}[Hitchin interpretation]
\label{rem:hitchin-interpretation}
\index{Hitchin system!shadow spectral curve}
\index{spectral curve!Hitchin interpretation}
The spectral curve $\Sigma_L$ is the spectral data of a
rank-$1$ Hitchin system on the shadow line~$L$.
The Higgs field is the recursion operator
$S_r \mapsto S_{r+1}$ whose characteristic polynomial
is $H^2 - t^4 Q_L(t) = 0$.
The Hitchin base is the parameter space
$(\kappa, \alpha, S_4)$; the Hitchin fibre over the
Gaussian locus $\Delta = 0$ is abelian (nodal spectral
curve, Prym trivial); over the interacting locus
$\Delta \neq 0$ it is non-abelian (smooth spectral
curve).  The shadow connection
$\nabla^{\mathrm{sh}}$ of
Theorem~\ref{thm:shadow-connection} is the oper
associated to this Hitchin datum.
\end{remark}
...
\item \emph{Monodromy.}\enspace
  ...
  factors through $\mathbb{Z}/2$; the generator acts
  by the \emph{Koszul sign}~$(-1)$.
...
(Theorem~\ref{thm:propagator-variance}), which is the
...
The shadow connection~$\nabla^{\mathrm{sh}}$ is the Gauss--Manin connection of the family $\{F_t := \sqrt{Q_L(t)}\}_{t \in L}$: the unique flat connection for which $\sqrt{Q_L}$ is a horizontal section.  The Picard--Fuchs equation $Q_L f'' + \tfrac{1}{2}Q_L' f' = 0$ is the ODE satisfied by flat sections.  This identifies the shadow tower with a classical object: the period integral of a family of algebraic curves parameterized by the arity variable~$t$.
\end{remark}
...
For $\mathcal{W}_N$ ($N \geq 3$), the $T$-line data equals the Virasoro data (autonomous, Remark~\ref{rem:w3-multi-channel-tower}); the full rank-$(N{-}1)$ shadow metric is multi-channel with non-autonomy controlled by the propagator variance (Theorem~\ref{thm:propagator-variance}).
\end{example}

\begin{remark}[Affine shadow metric]
\label{rem:affine-shadow-metric}
\index{shadow metric!affine Kac--Moody}
For affine Kac--Moody $\hat{\fg}_k$: the quartic obstruction $o_4 = \frac{1}{2}\{\mathfrak{C},\mathfrak{C}\}_H$ vanishes by the Jacobi identity (Theorem~\ref{thm:shadow-archetype-classification}(ii)), giving $S_4 = 0$ and $\Delta = 0$.  The shadow metric $Q_L = (2\kappa + \alpha t)^2$ is a \emph{perfect square}: $\sqrt{Q_L}$ is polynomial and the tower terminates at $r_{\max} = 3$.  The shadow connection is trivial ($\omega = Q'/(2Q) = \alpha/(2\kappa+\alpha t)$, regular everywhere), consistent with the finite depth.
\end{remark}
...
\item \emph{Curvature-proportionality criterion.}
  ...
  \end{equation}
  At such \emph{enhanced symmetry loci}, the multi-channel
  shadow metric on the diagonal reduces to the single-channel
  shadow metric: $Q_{\mathrm{diag}} = Q_{\mathrm{eff}}$, and
  the diagonal shadow tower is autonomous with algebraic
  generating function
...
\]
Each pair of channels contributes independently to the non-autonomy.  Enhanced symmetry ($\delta_{\mathrm{mix}} = 0$) requires \emph{every} pairwise difference $f_i/\kappa_i - f_j/\kappa_j$ to vanish simultaneously.
\end{remark}
...
deformation.  Then the shadow tower restricted to~$L_T$ equals
the Virasoro shadow tower at central charge $c(\cA)$:
\[
...
\[
  Q_{\mathrm{Vir}}(t)
  \;=\;
  c^2 + 4c\,t + \alpha(c)\,t^2,
  \qquad
  \alpha(c) \;=\; 4 + \frac{160}{c(5c{+}22)}\cdot\frac{c^2}{4}
  \;=\; 36 + \frac{80}{5c{+}22},
\]
and the Gaussian decomposition gives
$Q_{\mathrm{Vir}} = (c + 2t)^2 + 80t^2/(5c{+}22)$.
The shadow generating function
(Theorem~\ref{thm:riccati-algebraicity}) is
\[
  H_{\mathrm{Vir}}(t,c)
  \;=\;
  t^2\sqrt{c^2 + 4c\,t + \alpha(c)\,t^2}.
\]
...
$R_4 = 1/(4c)$,
$R_5 = -6/(5c^2)$,
reproducing the shadow tower of
\S\ref{sec:mixed-cubic-quartic-shadows}
through arity~$11$.
\end{example}

\begin{remark}[Radius of convergence]
\label{rem:shadow-radius}
\index{shadow tower!radius of convergence}
The shadow generating function $H(t) = t^2\sqrt{Q_L(t)}$ converges for $|t| < R$, where $R$ is the distance from $t = 0$ to the nearest zero of~$Q_L$ in the complex $t$-plane.  For general shadow metric data $(\kappa, \alpha, \Delta)$:
\[
  R \;=\; \frac{2\kappa}{\sqrt{\alpha^2 + 2\kappa\Delta}}.
\]
For $\mathrm{Vir}_c$: $R(c) = c\sqrt{(5c{+}22)/(180c{+}872)}$; at the self-dual point $c = 13$, $R \approx 2.14$ with complex branch points at argument $\approx 171^\circ$ (near-alternating signs).  In the semiclassical limit $c \to \infty$: $R \to \infty$ (the tower converges everywhere).  At the Lee--Yang point $c \to -22/5$: $R \to 0$ (the tower diverges).
\end{remark}
...
At the enhanced symmetry loci $P_{\mathcal{W}_3}(c) = 0$,
...
\]
The enhanced symmetry loci $P = 0$ correspond to
...
Theorem~\ref{thm:propagator-variance}(iii)) rather than from
...
This section applies the Chriss--Ginzburg principle to the
tautological ring of $\overline{\cM}_{g,n}$.
The principle states: every algebraic structure is a Maurer--Cartan
element, a projection thereof, or a cohomological invariant of the
associated deformation complex.  Here the single MC element is
$\Theta_\cA \in \operatorname{MC}(\gAmod)$
(Theorem~\ref{thm:mc2-bar-intrinsic}), and every result below---the
shadow CohFT and its axioms, the tautological relations,
WDVV, the Mumford $\lambda$-class relations, the $R$-matrix
identification, Eynard--Orantin recursion, and the Virasoro/$W$-constraints---is
a projection of $\Theta_\cA$ or a formal consequence of $D_\cA^2 = 0$.
...
containing the strong generators, and let
$\eta = \langle{-},{-}\rangle|_V$.  Define
...
\begin{proof}
The three CohFT axioms are three faces of $D^2 = 0$.
The differential~$D$
(eq.~\eqref{eq:D-five-components}) has clutching components
$d_{\mathrm{sew}}$ (separating) and $\hbar\Delta$ (non-separating);
$D^2 = 0$ on $\operatorname{FM}_n^{\mathrm{log}}(X \mathbin{|} D)$
(Theorem~\ref{thm:ambient-d-squared-zero}) forces the CohFT axioms
as projections of the MC equation to boundary strata.  We verify each
axiom in turn.
...
\smallskip\noindent
(ii)~A stable graph~$\Gamma$ with a
separating edge~$e$ decomposes as $\Gamma = \Gamma_1 \cup_e \Gamma_2$.  The amplitude factors:
...
P_e(\psi_{e^+}, \psi_{e^-})
\cdot
W_{\Gamma_2}(v_{S^c}, e^\alpha)$,
where the dual-basis summation implements
contraction via $\eta^{-1}$.  The graph
sum~\eqref{eq:phi-composite} restricted to the boundary stratum
$\xi_{\mathrm{sep}}(\overline{\cM}_{g_1,|S|+1} \times
\overline{\cM}_{g_2,|S^c|+1})$ collects precisely these graphs.
This is the projection of
$\tfrac{1}{2}[\Theta_\cA, \Theta_\cA]$ to the separating boundary.

\smallskip\noindent
(iii)~A non-separating edge~$e$ in~$\Gamma$ produces, upon removal, a
graph~$\Gamma'$ of type $(g-1, n+2)$.  The amplitude is
$\sum_\alpha W_{\Gamma'}(v_1, \ldots, v_n, e_\alpha, e^\alpha) \cdot
P_e(\psi)$.  Summing over all such graphs gives the restriction to
$\xi_{\mathrm{nsep}}(\overline{\cM}_{g-1,n+2})$.
This is the projection of $\hbar\Delta(\Theta_\cA)$ to the
...
(Definition~\ref{def:shadow-archetype-classification})
...
\item
  ...
  contribute, but the cyclic Jacobi identity kills
  their cohomology classes---$\delta_{\mathrm{pf}} = 0$
  in~$R^*(\overline{\mathcal{M}}_{g,n+1})$.
\item
...
\smallskip\noindent
(iii)~The Givental--Teleman
reconstruction~\cite{Givental01,Teleman12}
writes a semisimple CohFT as
$\hat{R} \cdot \eta$ via
stable-graph summation with $R$-matrix insertions at half-edges
and $\eta^{-1}/(\psi_{e^+}+\psi_{e^-})$ at edges.
The shadow CohFT~\eqref{eq:shadow-cohft} has vertex weights
$\ell_{|v|}^{(g(v))}$ and propagator $P_e(\psi_{e^+}, \psi_{e^-})$.
When $(V, \eta, \ell_3^{(0)})$ is semisimple,
$P_e = \eta^{-1}/(\psi_+ + \psi_-) \cdot
R_\cA(\psi_+) \cdot R_\cA(\psi_-)$
identifies
$\Omega_{g,n}^{\cA}$ with
$\hat{R}_\cA \cdot \eta$.
\end{proof}
...
The Eynard--Orantin recursion for Koszul
algebras
\textup{(}Conjecture~\textup{\ref{conj:EO-recursion}(b)}\textup{)}
is the scalar shadow of~\eqref{eq:mc-tautological-relation}
under the Chern--Weil transform:
...
\begin{proof}
The proof of
Conjecture~\ref{conj:EO-recursion}(b)
(\S\ref{subsec:recursion}) decomposes the graph sum by
...
establish the shadow Postnikov tower
...
platonic package cannot be formed for $\mathcal{W}_{1+\infty}$.
\end{proof}
...
\item \emph{Modular characteristic.}
  ...
  \kappa(\cA) + \kappa(\cA')$.
  Anti-symmetry: $\kappa(\cA^!) = -\kappa(\cA)$
  by the Verdier duality relation.
...
\smallskip\noindent
\emph{(D).}
Universality and anti-symmetry are properties~(i) and~(iv)
of Theorem~\ref{thm:mc2-bar-intrinsic}; additivity is
Proposition~\ref{prop:independent-sum-factorization};
the generating function is the exponential of the additive
invariant composed with the Hodge-class identity
$\sum_g \lambda_g \hbar^g = \exp(\sum_g \kappa_g \hbar^g)$.
Full detail: Theorem~\ref{thm:modular-characteristic}.

\smallskip\noindent
\emph{(H).}
The Hochschild spectral sequence degenerates at~$E_2$ for
Koszul algebras by the shadow tower concentration at arity~$2$
...
\item \emph{Formality criterion.}
  The modular $L_\infty$-algebra $\Convinf(\cA)$ is
  formal if and only if $\cA$ is one-channel
  \textup{(}$\dim H^2_{\mathrm{cyc}}(\cA,\cA) = 1$\textup{)}.
  In this case the transferred operations satisfy
  $l_n^{\mathrm{tr}} = 0$ for $n \geq 4$
  \textup{(}Corollary~\textup{\ref{cor:km-minimal-linf}}\textup{)},
  and the MC moduli is the affine line
...
\item \emph{Drinfeld--Sokolov formality.}
  ...
  algebras.  The BRST complex
  $C^{\mathrm{BRST}}_f(\cA)$ is formal as a dg
  algebra: its $A_\infty$-operations $m_n^{\mathrm{tr}}$
  vanish for $n \geq 3$ at generic~$k$, by the
  $L_\infty$-formality of the associated convolution
  algebra.
...
\begin{proof}
\emph{(i).}
One-channel forces $H^2_{\mathrm{cyc}} = \C \cdot \eta$,
so the minimal model has $l_2 = [-,-]_\fg$,
$l_3 = \phi$ (Killing cocycle), $l_n = 0$ ($n \geq 4$)
by Corollary~\ref{cor:km-minimal-linf}.
Transfer to cohomology sends the dg~Lie algebra to its
minimal $L_\infty$-model; formality is the vanishing of
...
\smallskip\noindent
\emph{(iii).}
At generic~$k$ the BRST differential has trivial higher
cohomology; the spectral sequence computing
$H^*_{Q_{\mathrm{DS}}}$ degenerates at~$E_1$.
Formality of the BRST complex follows from formality of
$\Convinf(\cA)$ at the principal embedding, since
the DS functor is an $L_\infty$-morphism on
...
%%-------------------------------------------------------------
%% 6. The platonic package as universal structure
%%-------------------------------------------------------------

\subsubsection{The platonic package and the modular envelope}

\begin{theorem}[Recovery of the platonic package from
$\Theta_\cA$]
...
\times_{\operatorname{Conf}_n(X \setminus D)}
\operatorname{FM}_n(X \mathbin{|} D),
\]
the fibre product of the ordered configuration space with
Mok's log-FM compactification
\textup{(\cite[Theorem~3.3.1]{Mok25})} along the forgetful
map $\operatorname{Conf}_n^{\mathrm{ord}} \to
\operatorname{Conf}_n$.
\begin{enumerate}[label=\textup{(\roman*)}]
...
\begin{proof}
Part~(i): the fibre product of a smooth variety with simple
normal crossings boundary along an \'etale map inherits the
normal crossings property.  The forgetful map
$\operatorname{Conf}_n^{\mathrm{ord}} \to
\operatorname{Conf}_n$ is a $\Sigma_n$-torsor,
hence \'etale; its extension to the blowup locus is
a local isomorphism on each stratum.
