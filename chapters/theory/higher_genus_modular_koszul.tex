...
The primitive master equation $d\mathfrak{K}_\cA +
\mathfrak{K}_\cA \star \mathfrak{K}_\cA = 0$ is a
\emph{flatness equation}.  The operator
$D_{\mathfrak{K}} := d + \operatorname{ad}(\mathfrak{K}_\cA)$
on the completed modular convolution algebra satisfies
$D_{\mathfrak{K}}^2 = 0$: this is the content of
$(Q_\cA^{\mathrm{mod}})^2 = 0$
(Theorem~\ref{thm:primitive-to-global-reconstruction}).
So the primitive kernel determines a flat connection on
the modular deformation complex, and its parallel transport
encodes the sewing of conformal data across degenerations of
the curve.
...
A^{\mathrm{ch}}_\infty
\;\xrightarrow{\;\text{cofree restriction}\;}
\mathfrak{K}_\cA
...
\item the genus-$1$ trace is the modular characteristic $\kappa(\cA)$;
...
\begin{theorem}[Deformation quantization of the chiral OPE;
\ClaimStatusProvedHere]
...
$d\mathfrak{K}_\cA + \mathfrak{K}_\cA \star
\mathfrak{K}_\cA = 0$
is a deformation quantization of the chiral OPE in the
...
\item \emph{One-loop quantization.}
  $K_{1,1}^\cA$ is uniquely determined by
  $\mathfrak{K}_\cA^{(0)}$ and the propagator~$P_\cA$
  via $dK_{1,1}^\cA + \Delta_{\mathrm{ns}}(K_{0,3}^\cA) = 0$.
  Its cyclic trace is the modular characteristic:
  $\kappa(\cA) = \operatorname{tr}_{\mathrm{cyc}}(K_{1,1}^\cA)$.
\item \emph{All-loop quantization.}
  $K_{g,n}^\cA$ for $g \geq 2$ and $R_\rho^\cA$ are uniquely
  determined by $\mathfrak{K}_\cA^{(0)}$ and~$P_\cA$
  ...
  is the unique quantum master solution.
\end{enumerate}
Conformal blocks on all Riemann surfaces are determined by the
OPE on the formal disk.
\end{theorem}

\begin{proof}
(i)~Genus-zero projection of the primitive equation.
(ii)~At genus~$1$, stability forces both bracketed corollas
to have $n \geq 3$, so no separating term appears; the equation
reduces to $dK_{1,1} + \Delta_{\mathrm{ns}}(K_{0,3}) = 0$, solved
by $K_{1,1} = -P_\cA \circ \Delta_{\mathrm{ns}}(K_{0,3})$.
(iii)~Theorem~\ref{thm:recursive-existence}; gauge equivalence
by Theorem~\ref{thm:shadow-homotopy-invariance}.
\end{proof}

\begin{remark}[The pre-Lie product and quantization]
\label{rem:pre-lie-quantization}
\index{pre-Lie product!and quantization}
The pre-Lie product~$\star$ is strictly richer than
$[a,b] = a \star b - b \star a$: the symmetric part
encodes self-sewing data that the Lie bracket loses.
In the genus-$2$ shell, $[K_1,K_1]$ vanishes by antisymmetry
for rank-one families, but $\Delta_{\mathrm{ns}}(K_1)$ does not.
At the $E_1$ level, $K_{0,2}^{\cA,E_1} = r(z)$ is the classical
$r$-matrix; the CYBE is the $E_\infty$ shadow of the $E_1$
...
\multicolumn{5}{l}{\emph{Higher-rank lattice VOAs
...
\begin{example}[Standard families and the shadow metric]
...
\end{center}
\noindent
The $\beta\gamma$ system has $r_{\max} = 4$, outside
$\{2, 3, \infty\}$: stratum separation, not the
...
The shadow tautological classes
(Definition~\ref{def:shadow-tautological-classes}) are projections of
a single MC element.  Every
relation among them is a projection of the
MC equation, and the system $\{\tau_{g,n}(\cA)\}_{g,n}$ carries a
cohomological field theory structure whose axioms are formal
consequences of $D_\cA^2 = 0$.
...
\smallskip
The three properties are three faces of $D^2 = 0$: the differential~$D$
(eq.~\eqref{eq:D-five-components}) has clutching components
$d_{\mathrm{sew}}$ (separating) and $\hbar\Delta$ (non-separating).
The MC equation projected to each boundary stratum gives the axioms.
The CohFT structure is a formal consequence of
...
The MC equation, projected to the $(g,n)$-component and evaluated via~\eqref{eq:tautological-evaluation-map},
...
\item \emph{The $R$-matrix.}
  The $R$-matrix of the shadow CohFT
  is the complementarity propagator:
...
$\operatorname{Der}(\cH) \hookrightarrow
T^{\mathrm{mod}}_{\Theta_\cH}
...
recover all known tautological relations?
\end{remark}
