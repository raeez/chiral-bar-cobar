...
\item the \emph{recursive existence theorem}
  ...
  projections.
\end{enumerate}
Three structure theorems govern the shadow tower and constitute the
...
\item \emph{Formality}
  (Theorem~\ref{thm:shadow-formality-identification}).  The shadow
  tower is the $L_\infty$~formality obstruction tower: $\kappa$,
  $\mathfrak{C}$, $\mathfrak{Q}$ are the obstructions to formality at
  arities $2$, $3$, $4$, and the four-class partition
  $\mathsf{G}/\mathsf{L}/\mathsf{C}/\mathsf{M}$ is precisely the
...
$k = -h^\vee$, the curvature $\kappa$ vanishes, the shadow tower
collapses to its zeroth term, and the content migrates to bar
...
The Heisenberg algebra was the atom from which all four main theorems
were first visible (\S\ref{sec:frame-modular-package}): a single
number $\kappa(\mathcal{H}_k) = k$ controlled the entire scalar genus
tower.  The following theorem establishes that universality for all
modular Koszul chiral algebras.  (The spectral discriminant~$\Delta_\cA$,
periodicity profile~$\Pi_\cA$, and full MC
completion~$\Theta_\cA$ require data beyond~$\kappa$;
...
\item \emph{Generating function.}
  For single-generator algebras \textup{(}and for multi-generator
  algebras subject to the factorization condition
  of~\textup{(i)(b)} above\textup{)}, the genus free energies
  assemble as
...
\item
  ...
  through $35$~stable graphs ($4$ one-vertex, $12$ two-vertex,
  $18$ three-vertex, $1$ four-vertex),
  with $\ell_2^{(1)} = S_3\kappa/24 - S_3^2/2$
...
Let $\cA$ be a chirally Koszul algebra with shadow tower
...
\item \emph{Rank-$2$ $\cW_3$ planted-forest
  \textup{(}eq.~\textup{\eqref{eq:w3-planted-forest-genus2}}\textup{)}.}
  On $V = \{T, W\}$ with $S_3(T,T,T) = 2$,
  $S_3(T,W,W) = 1$, $\kappa_W = 0$ ($\mathbb{Z}_2$~symmetry):
  $\delta_{\mathrm{pf}}^{(2,0)}(\cW_3) = (932 - 7c^2)/(48c^3)$.
...
matching of the genus-$0$ shadow tower data with the $A_{N-1}$
