...
for modular Koszul algebras.  We give a homotopy-native version
...
$\Defcyc^{\mathrm{mod}}(\cA)$, so the shadow Postnikov tower
...
(Theorem~\ref{thm:genus-universality}).
Off the scalar lane, this identity is proved at genus~$1$
...
This is because the strict model $\Convstr$ has $\ell_n = 0$ for
...
\item \emph{Formality.}
  ...
  The shadow Postnikov tower measures the
...
dg~Lie algebra (no higher operations); the shadow tower
...
identification
$F_g = \kappa\cdot\lambda_g^{\mathrm{FP}}$ remains conditional
on~\textup{\ref{op:multi-generator-universality}}.
...
\begin{remark}[The shadow tower as $L_\infty$-formality
...
the shadow tower with the formality obstruction tower for
...
$n \geq 3$.  The shadow Postnikov tower measures the
...
The full shadow tower is the interleaving of the classical
...
\begin{equation}\label{eq:shadow-gf-closed-form}
  ...
  t^2\sqrt{Q_L(t)}.
\end{equation}
The entire shadow tower on~$L$ is determined by three
...
non-polynomial character of the shadow tower.
\end{proof}
...
is the \emph{first obstruction to the shadow tower being
...
The MC recursion from $(\kappa, \alpha)$ alone (without quartic input) produces a ``free shadow tower'' with $S_4^{\mathrm{free}} = -9\alpha^2/(16\kappa)$ (the inherited quartic from the cubic self-bracket; see Proposition~\ref{prop:hankel-extraction}(iv)).  For the Virasoro: $S_4^{\mathrm{free}} = -9/(2c)$, while $S_4^{\mathrm{actual}} = 10/[c(5c{+}22)]$.  The difference $S_4^{\mathrm{actual}} - S_4^{\mathrm{free}} = (45c{+}218)/[2c(5c{+}22)]$ is the \emph{intrinsic quartic}: the OPE contribution not captured by the cubic alone.  For affine algebras, this intrinsic quartic exactly cancels the inherited one (Jacobi identity), giving $\Delta = 0$.
\end{remark}
...
The shadow connection~$\nabla^{\mathrm{sh}}$ is the Gauss--Manin connection of the family $\{F_t := \sqrt{Q_L(t)}\}_{t \in L}$: the unique flat connection for which $\sqrt{Q_L}$ is a horizontal section.  The Picard--Fuchs equation $2\,Q_L\,f'' + Q_L'\,f' - Q_L''\,f = 0$ is the ODE satisfied by flat sections (obtained by differentiating the first-order relation $2Q_L f' - Q_L' f = 0$).  This identifies the shadow tower with a classical object: the period integral of a family of algebraic curves parameterized by the arity variable~$t$.
\end{remark}
...
The shadow tower on the T-line converges
...
$\Delta = 0$, so the shadow tower terminates and $\rho = 0$
...
(Definition~\ref{def:weight-filtration-tower})
controls the shadow Postnikov tower
...
(Computation~\ref{comp:w3-genus2-multichannel}) exhibits a
cross-channel correction beyond the scalar $\kappa$-universal
term.  The six genus-$2$ stable graphs, summed over
$2^e$ channel assignments per graph, yield
\[
F_2^{\mathrm{full}}(\cW_3)
= \underbrace{\frac{7c}{6912}}_{\kappa \cdot \lambda_2^{\mathrm{FP}}}
\;+\; \underbrace{\frac{c + 120}{16c}}_{\delta F_2}.
\]
The cross-channel correction $\delta F_2 = (c{+}120)/(16c)$
is an explicit instance of the multi-generator phenomenon
flagged in op:multi-generator-universality: the scalar formula
$F_g = \kappa \cdot \lambda_g^{\mathrm{FP}}$ receives
corrections from mixed-channel graph amplitudes at genus~$\geq 2$.
This is the first such correction computed in closed form.
The ratio $\delta F_2/F_2^{\mathrm{diag}} = 432(c{+}120)/(7c^2)$
vanishes as $c \to \infty$ (the semi-classical limit), but
diverges as $c \to 0$ (the strongly coupled regime).
\end{remark}
...
the shadow tower terminates at arity~$3$: the cubic shadow
