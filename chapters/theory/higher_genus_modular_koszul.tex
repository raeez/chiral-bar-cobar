...
\begin{remark}[Relation to classical enveloping algebras]
\label{rem:platonic-adjunction-classical}
\index{enveloping algebra!universal property}
Theorem~\ref{thm:platonic-adjunction} is the chiral/modular
lift of the classical $U(\fg) \dashv \mathrm{Prim}$ adjunction
between Lie algebras and cocommutative Hopf algebras.  In the
classical setting, the Milnor--Moore theorem identifies the
primitive elements of a cocommutative Hopf algebra with a Lie
algebra, and the universal enveloping algebra is the left
adjoint.  Here, the Lie algebra is replaced by a cyclically
admissible Lie conformal algebra, the Hopf algebra by a cyclic
factorization algebra, and the universal envelope by the modular
factorization envelope $U_X^{\mathrm{mod}}$.  The modular
extension --- absent classically --- is the genuinely new
ingredient: it equips the envelope with the shadow Postnikov tower
$\Theta_L$ of
