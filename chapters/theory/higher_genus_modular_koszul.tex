...
\subsubsection*{The bar-intrinsic construction}

The universal MC class admits a direct construction from the
genus-completed bar differential that requires no conditional
hypotheses, no simple Lie symmetry, and no one-channel
assumption.  The key observation is that the ``missing'' universal
class was never missing: it is the positive-genus part of the
genus-completed bar differential, which already exists by the
modular operad structure (Theorem~\ref{thm:bar-modular-operad}).
...
(Theorem~\ref{conj:differential-square-zero}).
\end{remark}
