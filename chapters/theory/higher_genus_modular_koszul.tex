...
underlies the tower.  The key structural insight is that bar-cobar
...
\ClaimStatusProvedHere]
\label{rem:coefficient-algebras-well-defined}
...
\begin{proof}
The proof decomposes into diagonal and cross-terms.
\begin{enumerate}[label=\textup{(\roman*)}]
...
\item The cross-terms cancel pairwise by the master identity
  \[
    ...
    = 0,
  \]
  which follows from the factorization of the modular operad boundary
  into separating and non-separating degenerations
  (Theorem~\ref{thm:prism-higher-genus}).  The genus-loop term
  $\hbar\Delta$ accounts for the non-separating degeneration
  contribution.  \qedhere
\end{enumerate}
\end{proof}
...
\begin{remark}[Log-geometric evidence for
Conjecture~\ref{conj:differential-square-zero}]
\label{rem:log-evidence-d-squared}
\index{differential!ambient square-zero!log-geometric evidence}
...
skeleton of $\operatorname{FM}_n^{\mathrm{log}}$.  The functorial
transfer principle of Robert-Nicoud--Wierstra
\cite[Theorem~5.1, Proposition~5.5]{RNW19} then implies: if the
tropicalisation map induces a quasi-isomorphism
...
\end{equation}
on chain complexes, then $D_{\cA}^2 = 0$ at the ambient level would
follow from $\partial^2 = 0$ on
$\operatorname{FM}_n^{\mathrm{log}}$ by one-slot functoriality of
$\operatorname{Conv}$.  The hypothesis~\eqref{eq:log-fm-qiso-hypothesis}
is plausible---the tropical skeleton captures the combinatorial type
of every boundary stratum---but not yet established.  Proving it
would reduce Conjecture~\ref{conj:differential-square-zero} to the
topological statement that $\operatorname{FM}_n^{\mathrm{log}}$ has
no cohomology invisible to the tropical skeleton.
\end{remark}
