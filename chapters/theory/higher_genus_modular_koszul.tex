...
(Theorem~\ref{thm:bar-modular-operad}(iii)).  Expanding:
\[
0 = D_\cA^2
= (\dzero + \Theta_\cA)^2
...
$\Theta_\cA \mapsto \Theta_{\cA^!}$.  As a consistency
check, the scalar trace transforms correctly:
...
Theorem~\ref{thm:ambient-d-squared-zero} depends critically on
\cite[Theorem~3.3.1]{Mok25}: the log Fulton--MacPherson
compactification $\operatorname{FM}_n^{\mathrm{log}}(X|D)$ is a
smooth compactification with simple normal-crossings boundary.
This is an external result, not proved in this monograph.
...
triviality.  All five main theorems (A--D, H) and the shadow
Postnikov tower (Theorem~\ref{thm:recursive-existence}) depend
only on the convolution-level result.  The ambient-level $D^2 = 0$
is used for the planted-forest corrections and the depth
filtration (Definition~\ref{def:vol1-rigid-planted-forest-depth-filtration}), which provide finer
structural information but are not logically required for the
main theorems.
\end{remark}
