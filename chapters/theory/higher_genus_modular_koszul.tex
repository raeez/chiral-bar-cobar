...
The shadow Postnikov tower $\Theta_{\cA}^{\leq r}$ is the primary
mathematical object of Part~I beyond the linear regime.  At each finite
order~$r$, the tower is constructive and proved: the modular
characteristic $\kappa(\cA)$ at arity~2, the cubic shadow~$C$ at
arity~3, the quartic resonance class~$Q$ at arity~4, with obstruction
classes $o_{r+1}(\cA)$ living in the modular cyclic deformation
complex~$\mathrm{Def}_{\mathrm{cyc}}^{\mathrm{mod}}(\cA)$.  The full
unrestricted Maurer--Cartan element
$\Theta_{\cA} \in \mathrm{MC}(\gAmod)$ is the conjectural all-arity
limit of this tower---it is the target, not the starting point.

The present section develops the genus-graded Koszul duality that
underlies the tower.  The modular convolution dg~Lie
algebra~$\gAmod$ is the strict model of a homotopy-invariant
$L_\infty$-algebra $\Definfmod(\cA)$
(\S\ref{subsec:two-level-convention}): the strict model suffices
for all MC computations, while the $L_\infty$ structure controls
functoriality under quasi-isomorphic replacements and different
choices of contracting homotopy.
The key structural insight is that bar-cobar
inversion (Theorem~B) extends from genus~0 to all genera, producing a
compatible system of Koszul dualities indexed by the boundary
stratification of~$\overline{\mathcal{M}}_{g,n}$.  At each
genus~$g$, the bar complex $\barB^{(g)}(\cA)$ carries a curved
differential whose curvature is controlled by~$\kappa(\cA)$; the
cobar functor inverts it whenever the Koszul property (diagonal Ext
vanishing) holds.  In the three-ring architecture, this section
provides the algebraic scaffolding for Ring~2: the weight filtration
on~$\gAmod$ controls the extension tower, and each finite truncation
$\Theta_{\cA}^{\leq r}$ is a graph sum through arity~$r$ that does
not require the full all-genera modular envelope.
...
\item the \emph{all-arity master equation}
  ...
  arity-$r$ component;
\item the \emph{recursive existence conjecture}
  (Conjecture~\ref{conj:recursive-existence}): the full
  inverse-limit element
  $\Theta_\cA = \varprojlim_r \Theta_\cA^{\leq r}$ exists for
  every modular Koszul algebra equipped with a cyclic slice,
  stable propagator, and planted-forest residue model.
\end{enumerate}
...
(3)~shadow tower / conjectural full MC: $\Theta_{\cA}^{\leq r}$, $\Defcyc(\cA)$
...
the all-arity convergence of this tower is conjectural.
\end{remark}
...
The genuine open problem consists of non-DS constructions at admissible
...
order.  The all-arity convergence of the tower remains conjectural.
...
The conjectural all-arity limit of the tower is a
...
$d_{\mathrm{sew}}$ misses.  The conjecture that $D^2 = 0$ at the
ambient level (Conjecture~\ref{conj:differential-square-zero}) is
...
conjectural all-arity limit
$\Theta_{\cA} = \varprojlim_r \Theta_{\cA}^{\leq r}$
is the target of Conjecture~\ref{conj:recursive-existence}.
\end{definition}
...
$\Theta_\cA = \varprojlim_r \Theta_\cA^{\leq r}$ is the
\emph{conjectural} horizon
\textup{(}Conjecture~\textup{\ref{conj:recursive-existence})}.
...
\begin{remark}[Scope: scalar saturation vs.\ full all-arity limit]
...
Maurer--Cartan element $\Theta_\cA = \varprojlim_r \Theta_\cA^{\leq r}$
is the \emph{conjectural} limit of the shadow Postnikov tower.
Each finite truncation $\Theta_\cA^{\leq r}$ is proved
constructively:
$\Theta^{\leq 2} = \kappa$,
$\Theta^{\leq 3} = \kappa + \mathfrak{C}$,
$\Theta^{\leq 4} = \kappa + \mathfrak{C} + \mathfrak{Q}$.
The convergence of the full tower
\textup{(}Conjecture~\textup{\ref{conj:recursive-existence})}
remains open for non-one-channel algebras.

The MC2 gauntlet
\textup{(}Theorem~\textup{\ref{thm:mc2-full-resolution})} remains
the verification that the one-channel and tautological-line
hypotheses hold for all standard families; the present theorem
distils its content to three lines under those hypotheses.
\end{remark}
...
The \emph{conjectural} full modular lift of $\cA$ is the statement
that the all-arity limit $\Theta_{\cA}$
(Conjecture~\ref{conj:recursive-existence}) satisfies
\begin{equation}
...
\emph{conjectural} consequences of the existence of the
limit $\Theta_\cA = \varprojlim_r \Theta_\cA^{\leq r}$
...
clutching, and is the primary reason
$D_\cA^2 = 0$ at the ambient level remains conjectural
(Conjecture~\ref{conj:differential-square-zero}).
\end{remark}
...
\item The cross-terms are \emph{expected} to cancel by the master identity
  \[
    ...
    = 0,
  \]
  contingent on the planted-forest coefficient algebra faithfully capturing
  all codimension-${\geq}2$ boundary relations of $\overline{\mathcal{M}}_{g,n}$
  (see Conjecture~\ref{conj:differential-square-zero} and Remark~\ref{rem:log-evidence-d-squared}).
\end{enumerate}
\end{proof}
...
\begin{conjecture}[Square-zero: ambient level]
...
the differential $D_{\cA}$ satisfies $D_{\cA}^2 = 0$.

The distinction from Theorem~\textup{\ref{thm:convolution-d-squared-zero}}
is that $\mathfrak{g}^{\mathrm{amb}}_{\cA}$ includes the
cross-polarized $\cA/\cA^!$ structure and the planted-forest
coefficient algebra $\mathbb{G}_{\mathrm{pf}}$.  The additional
content is that the Fulton--MacPherson residue model faithfully
captures all codimension-${\geq}2$ boundary relations needed
for the ambient differential.  At the pure convolution level
\textup{(}where $\mathbb{G}_{\mathrm{pf}}$ is omitted and
cross-polarization is not imposed\textup{)}, $D^2 = 0$ is
already proved.
\end{conjecture}

\begin{hypothesis}[Tropical faithfulness]
\label{hyp:tropical-faithfulness}
\ClaimStatusConjectured
\index{tropical faithfulness hypothesis|textbf}
The tropicalisation map of
Mok~\cite[Theorem~3.3.1]{Mok25} induces a quasi-isomorphism on
chain complexes:
...
\end{equation}
Equivalently: the logarithmic Fulton--MacPherson space
$\operatorname{FM}_n^{\mathrm{log}}(X \mathbin{|} D)$ has no
cohomology invisible to its tropical skeleton.
\end{hypothesis}

\begin{proposition}[Tropical reduction of ambient $D^2 = 0$;
\ClaimStatusProvedHere]
\label{prop:tropical-reduction-d-squared}
\index{differential!ambient square-zero!tropical reduction}
Hypothesis~\textup{\ref{hyp:tropical-faithfulness}} implies
Conjecture~\textup{\ref{conj:differential-square-zero}}.
\end{proposition}

\begin{proof}
The logarithmic Fulton--MacPherson space
$\operatorname{FM}_n^{\mathrm{log}}(X \mathbin{|} D)$ of
Mok~\cite{Mok25} is a smooth compactification with normal-crossings
boundary \cite[Theorem~3.3.1(1)]{Mok25}, so $\partial^2 = 0$ on its
chain complex is automatic.  By
Proposition~\ref{prop:planted-forest-tropical}, the planted-forest
coefficient algebra $\mathbb{G}_{\mathrm{pf}}$ is the tropical
skeleton of $\operatorname{FM}_n^{\mathrm{log}}$.  Under
Hypothesis~\ref{hyp:tropical-faithfulness}, the tropicalisation
$\operatorname{trop}_*$ is a quasi-isomorphism.  The functorial
transfer principle of Robert-Nicoud--Wierstra
\cite[Theorem~5.1, Proposition~5.5]{RNW19} then gives: the induced
map on convolution $L_\infty$-algebras
\[
\operatorname{trop}^*\colon
\Convinf\bigl(C_*^{\mathrm{trop}}(\mathbb{G}_{\mathrm{pf}}),\,
  \operatorname{End}_{\cA}\bigr)
\;\xrightarrow{\;\sim\;}\;
\Convinf\bigl(C_*(\operatorname{FM}_n^{\mathrm{log}}),\,
  \operatorname{End}_{\cA}\bigr)
\]
is an $L_\infty$-quasi-isomorphism (one-slot functoriality in the
coalgebra variable, with the algebra slot
$\operatorname{End}_{\cA}$ held fixed).  Since $\partial^2 = 0$ on
$\operatorname{FM}_n^{\mathrm{log}}$ implies $D^2 = 0$ on the
right-hand side \textup{(}by the same argument as
Theorem~\textup{\ref{thm:convolution-d-squared-zero})}, and the
$L_\infty$-quasi-isomorphism preserves the differential, $D^2 = 0$
transfers to the left-hand side, which is the ambient algebra
$\mathfrak{g}^{\mathrm{amb}}_{\cA}$.
\end{proof}
