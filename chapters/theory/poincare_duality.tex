...
The bar construction of Chapter~\ref{chap:bar-cobar} produces a coalgebra $\bar{B}(\cA)$ from any chiral algebra $\cA$ --- the Koszul dual \emph{coalgebra}.  Verdier duality on configuration spaces then relates this coalgebra to the bar coalgebra of a second algebra~$\cA^!$: $\mathbb{D}_{\Ran}\bar{B}(\cA) \simeq \bar{B}(\cA^!)$.  The Koszul dual \emph{algebra}~$\cA^!$ is thereby determined intrinsically, without presupposing a definition from classical Koszul theory.  The Heisenberg specialization of Chapter~\ref{ch:heisenberg-frame} is the prototypical case.\label{rem:poincare-duality-hms}
