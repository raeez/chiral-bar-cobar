...
This chapter globalizes the local engine.  Up to this point the bar
construction $\barB(\cA)$ has been built pointwise: fix a finite set of
marked points, form the bar complex on $\overline{C}_n(X)$, and compute.
Non-abelian Poincar\'e duality replaces the pointwise viewpoint with a
global one, passing from individual configuration spaces to the Ran
space $\Ran(X)$ and its factorization structure.  The passage is not
merely organizational.  On $\Ran(X)$, Verdier duality becomes a
\emph{functor} $\mathbb{D}_{\Ran}$ that intertwines bar coalgebras:
$\mathbb{D}_{\Ran}\barB(\cA) \simeq \barB(\cA^!)$.  The dual algebra
$\cA^!$ is thereby \emph{constructed}, not postulated, and the classical
Koszul relation $R^\perp$ appears as the cohomological shadow of a
chain-level equivalence.  Strictness is again a shadow: the quadratic
criterion $R_1 \perp R_2$ captures the first-order data of a derived
enhancement that persists for non-quadratic, curved, and higher-genus
algebras.

The homotopy upgrade is essential for two reasons.  First, the
factorization homology $\int_M \cA$ is computed as a homotopy colimit
over Weiss covers, and the resulting object is a derived invariant that
detects quasi-isomorphisms invisible to the strict bar complex.  Second,
at higher genus the bar differential acquires curvature
$(\dfib)^2 = \kappa(\cA)\cdot\omega_g$
(Chapter~\ref{chap:higher-genus}), and the Verdier intertwining
$\mathbb{D}_{\Ran}\barB(\cA) \simeq \barB(\cA^!)$ must be upgraded to a
statement about \emph{curved} factorization coalgebras.  The modular
$\Linf$~convolution algebra $\Convinf(\barB(\cA), \cA)$ is the natural
home for this upgrade: its Maurer--Cartan elements encode the full tower
of Koszul duality data, with the strict Verdier pairing of logarithmic
forms against distributional currents (equation~\eqref{eq:log-dist-pairing-pd}
below) as its chain-level realization.  The shadow Postnikov tower of
Chapter~\ref{chap:deformation-theory} is a finite-order projection of
this Maurer--Cartan structure, and the ambient complementarity of
Theorem~C (Chapter~\ref{chap:higher-genus}) is its genus-summed
incarnation.

The Heisenberg algebra (Chapter~\ref{ch:heisenberg-frame}) remains the
prototypical test case: there, Verdier duality reduces to the pairing
between $d\log(z_1 - z_2)$ and $\delta(z_1 - z_2)$, and the full
NAP machinery collapses to a single residue.  At the opposite extreme,
the Virasoro algebra requires the complete depth of the factorization
structure, and the distributional currents on $C_n(X)$ carry the
information of all OPE channels simultaneously.  The loop closes:
the intrinsic construction of $\cA^!$ via NAP duality feeds back into
the bar--cobar inversion of
Chapter~\ref{chap:bar-cobar-adjunction}, completing the passage from
pointwise computations to a genuinely homotopy-theoretic equivalence on
$\Ran(X)$.

Three objects govern chiral Koszul duality, and the relationship among them is the subject of this chapter:
\begin{center}
\begin{tabular}{r@{\;:\quad}l}
$\bar{B}(\cA)$ & the bar construction---a dg factorization \emph{coalgebra} on $\Ran(X)$, \\
$\cA^!_{\mathrm{coal}}$ & the Koszul dual \emph{coalgebra}---defined classically via $R^\perp$, \\
$\cA^!$ & the Koszul dual \emph{algebra}---the linear dual of $\cA^!_{\mathrm{coal}}$ when it exists.
\end{tabular}
\end{center}
The identity $\bar{B}(\cA) = \cA^!_{\mathrm{coal}}$ is the starting point: the bar construction does not \emph{compute} the dual, it \emph{is} the dual.  The classical object $\cA^!_{\mathrm{coal}}$ is merely the cohomological shadow $H^*(\bar{B}(\cA))$, visible only when $\cA$ is Koszul.  In general, $\bar{B}(\cA)$ retains bar cohomology in all degrees, not just the cogenerators in bar degree~$1$; it is the derived enhancement, and the correct object for homotopy-theoretic purposes.

Verdier duality on $\Ran(X)$ intertwines bar coalgebras: $\mathbb{D}_{\Ran}\bar{B}(\cA) \simeq \bar{B}(\cA^!)$.  The algebra $\cA^!$ is thereby determined intrinsically, without presupposing a definition from classical Koszul theory.  The Heisenberg specialization of Chapter~\ref{ch:heisenberg-frame} is the prototypical case.\label{rem:poincare-duality-hms}
...
Configuration spaces carry a natural duality that resolves the circularity.  On $\overline{C}_k(X)$, two dual sheaf complexes live naturally; one builds the bar construction, the other the cobar.
...
The general theory of Verdier duality for constructible complexes on smooth varieties~\cite{KS90} gives perfectness.
\end{proof}
...
No bar construction, orthogonal relations, or quadraticity
hypothesis appears in Construction~\ref{const:A-dual-intrinsic}.
...
Here $\bar{B}^{\mathrm{ch}}(\mathcal{A})$ is the geometric bar complex (Definition~\ref{def:geometric-bar}) and $\mathcal{A}^! = \mathbb{D}_{\mathrm{Ran}}(\mathcal{A})$ is the Verdier dual chiral coalgebra (Construction~\ref{const:A-dual-intrinsic}).
...
\emph{Step~4: Cobar.}
By the finite-dimensionality hypothesis imposed in the completed
setting, $\dim \mathrm{CH}^n(\mathcal{A}) < \infty$, so the cobar
