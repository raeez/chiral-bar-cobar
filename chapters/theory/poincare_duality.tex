\chapter{Non-abelian Poincaré duality and the construction of Koszul dual coalgebras}
\label{chap:NAP-koszul-derivation}

\index{Poincare duality!chiral|textbf}
\index{Verdier duality!bar construction computes dual coalgebra}
...
$Z(\cA) \xrightarrow{\sim} Z(\Omegach_X(\Bbarch_X(\cA)))$ at every
chirally Koszul datum, unconditional from
Theorem~\ref{thm:koszul-reflection} via the chain-level
$K^{2}\simeq\mathrm{id}$ on $\Kosz(X)$.
\textsc{C1}: the Lagrangian-eigenspace decomposition of the
quantum class $Q_g(\cA) \oplus Q_g(\cA^!) \simeq H^*(\overline{\cM}_g,
Z(\cA))$ at every genus $g \geq 1$ on the Koszul locus, unconditional
under perfectness of the bar--cobar normal complex (the perfectness
hypothesis is verified family-by-family on the standard landscape).
...
constructs the $\cA^!$-side of that decomposition. The Verdier
dual $\mathbb{D}_{\Ran}\barB(\cA)$ is the codomain side of the
Koszul Reflection $K = \Bbarch_X$
(Theorem~\ref{thm:koszul-reflection}); the involutivity
$K^{2}\simeq\mathrm{id}$ on $\Kosz(X)$ is exactly the chain-level
identification of $\Omegach_X(\Bbarch_X(\cA))$ with $\cA$, with
$\cA^!_\infty$ playing the role of the Verdier-dualised image of
the bar.
...
object is $\cA^!_\infty$.

\section{The circularity problem}

\begin{problem}[Independent construction of Koszul dual]\label{prob:independent-dual}
The statement $\bar{B}^{\text{ch}}(\mathcal{A}_1) \simeq \mathcal{A}_2^!$ is circular as it stands: the dual $\mathcal{A}_2^!$ is ``defined'' as the Koszul dual coalgebra, but no independent construction of it as a chiral coalgebra exists, and for non-quadratic algebras the classical criterion $R_1 \perp R_2$ does not apply.
...
distributional currents on the open configuration space
(more precisely, distributional sections of the dualizing sheaf
on $\overline{C}_k(X)$ supported along the boundary strata).
The delta current $\delta_{ij} = \delta(z_i - z_j)\, dz_i \wedge d\bar{z}_j$,
supported on the diagonal $\{z_i = z_j\}$ in the ambient
space~$X^k$, is the simplest example; the distributional
insertions $\prod_{e \in E(T)} \delta_e$ are the integrands of
the cobar construction (see Remark~\ref{rem:analytic-framework}
for the precise sheaf-theoretic formulation).
...
Since $\mathcal{A}$ is graded by conformal weight with finite-dimensional graded pieces, $\mathcal{A}^!$ inherits a dual weight grading. The coproduct on $\cA^!$ is dual to the product on~$\cA$: it maps a linear functional on $\cA^{\otimes 1}$ to functionals on $\cA^{\otimes 2}$, strictly increasing the number of tensor factors. The iterated coproduct $\Delta^{(N)}$ therefore vanishes on elements of degree~$\leq N$.

This perspective is confirmed \emph{a posteriori} by Theorem~\ref{thm:bar-computes-dual}, which identifies $\cA^{\scriptstyle \text{\normalfont !\textasciigrave}} \simeq \bar{B}^{\mathrm{ch}}(\mathcal{A})$: the bar complex is filtered by bar degree and the deconcatenation coproduct strictly increases the number of tensor factors, so $\Delta^{(N)} = 0$ on bar degree $\leq N$.
\end{proof}

\section{\texorpdfstring{The bar construction computes the dual coalgebra $\cA^{\scriptstyle \text{\normalfont !\textasciigrave}}$}{The bar construction computes the dual coalgebra}}

\begin{theorem}[Bar construction = Verdier dual coalgebra via NAP; \ClaimStatusProvedHere]\label{thm:bar-computes-dual}
\index{Verdier duality!bar computes dual coalgebra|textbf}
...
Here $\bar{B}^{\mathrm{ch}}(\mathcal{A})$ is the geometric bar complex (Definition~\ref{def:geometric-bar}) and $\cA^{\scriptstyle \text{\normalfont !\textasciigrave}}$ is the Verdier dual chiral coalgebra (Construction~\ref{const:A-dual-intrinsic}). The Koszul dual \emph{algebra} is $\cA^! = (\cA^{\scriptstyle \text{\normalfont !\textasciigrave}})^\vee$.
...
$\mathcal{A}$ is Koszul (Definition~\ref{def:chiral-koszul-pair}),
...
$\bar{B}^{\mathrm{ch}}(\cH_k) \simeq \cH_k^{\scriptstyle \text{\normalfont !\textasciigrave}}$:
the bar complex, built from logarithmic forms on $\overline{C}_n(X)$,
is quasi-isomorphic to the Verdier dual coalgebra whose
linear dual is the Koszul dual algebra
$\cH_k^! = \mathrm{Sym}^{\mathrm{ch}}(V^*)$.
Since $\cH_k$ is Koszul, the bar cohomology is concentrated in
bar degree~$1$: $H^*(\bar{B}(\cH_k)) \cong \mathrm{Sym}^{\mathrm{ch}}(V^*)$
with graded character $\prod_{n \geq 1}(1-q^n)^{-1}$
...
completed counit
$\Omega^{\mathrm{ch}}(\widehat{\cA^!}) \to \cA$ is a
quasi-isomorphism.
\end{proof}
...
is the quasi-primary composite of weight 4. It does not lie in~$W_3$ itself but appears in $\bar{B}^{\mathrm{ch}}(W_3)$, with OPE coefficient $\frac{16}{22+5c}$ in the $W$--$W$ channel
...
\section{Synthesis}
\label{sec:pd-wave13-DNA}

\begin{remark}[Mukai pairing as Poincar\'e duality]
\label{rem:pd-DNA-1}
The Mukai pairing on $\Lambda_{\mathrm{Muk}} = II_{4,20}$ is the Wave~13
Poincar\'e duality pairing on $\mathbf H_{\Delta_5}$: signature $(4, 20)$
with $c_+ = 4$ and $c_- = 20$, Mukai doubling $K^{\kappa_{\mathrm{ch}}} = 2c_+ = 8$,
and chirality $+1$. The Poincar\'e-duality dimension $24 = c_+ + c_-$
equals the rank of the Mukai lattice, matching the $24$ Kodaira $I_1$
fibres on $E^{\mathrm{nod,sm}}_{24}$ in the Wave~13 bi-based Ran pair.
The duality is implemented by the $\mathrm{SO}(4,20;\mathbb Z)$-action
on the K3 moduli \cite{Mukai1987,Borcherds1998}. Cross-ref Vol.~III
Chapter~\ref{ch:k3-chiral-bialgebra-platonic}.
\end{remark}

\begin{remark}[Three faces of Poincar\'e duality at K3]
\label{rem:pd-DNA-2}
The Wave~13 Poincar\'e duality at K3 has three faces:
(i)~Mukai pairing on $D^b\mathrm{Coh}(K3)$ (CY side);
(ii)~Chiral Hochschild pairing on $\mathbf H_{\Delta_5}$ (chiral side);
(iii)~Siegel-modular $\Phi_{10}$-denominator cusp-form pairing on
$\bar{\mathcal A}_2$ (modular-automorphic side).
The universal identity $\hbar^2\cdot K^{\kappa_{\mathrm{ch}}} = -1$ is
the statement that all three faces agree at the Lusztig specialisation
$\hbar^2 = -1/8$ \cite{Lusztig1990,PasolZagier2013}. Cross-ref
Chapter~\ref{ch:k3-chiral-bialgebra-platonic}.
\end{remark}

\begin{remark}[Wave 15 quantum-determinant duality]
\label{rem:pd-DNA-3-wave15}
The chiral-Hochschild face~(ii) of
Remark~\ref{rem:pd-DNA-2} refines as follows under Wave~15. On the
...
the Molev-style quantum determinant on the rank-$24$
...
Siegel-modular $\Delta_5$-pairing is therefore the Poincar\'e dual, at
the quantum determinant level, of face~(ii)'s chiral-Hochschild
pairing: the two faces coincide up to the rational Yang prefactor
$C(u)$. Cross-ref Vol.~III
Chapter~\ref{ch:k3-chiral-bialgebra-platonic}
Theorem~\ref{thm:w15-quantum-determinant}.
\end{remark}

\begin{remark}[Wave 15 BKM vertical vs.~Mukai horizontal duality]
\label{rem:pd-DNA-4-wave15}
The Mukai pairing rank-$24$ of Remark~\ref{rem:pd-DNA-1} is the
...
Wave~15 (Theorem~\ref{thm:w15-chevalley-bkm-generators}) is the
\emph{vertical} duality datum on the hyperbolic core
$\mathrm{II}_{2,1}\subset\mathrm{II}_{3,19}$
\cite{GritsenkoNikulin1997,GritsenkoNikulin1998}. The apparent
rank discrepancy $24\neq 3$ is the Mukai-lattice rank (horizontal)
vs.~the BKM-hyperbolic rank (vertical). The averaging morphism of
Vol.~III Chapter~\ref{ch:k3-chiral-bialgebra-platonic}'s
Definition~\ref{def:k3-chiral-bialgebra} intertwines the two via the
composite
$\mathrm{II}_{3,19}\hookrightarrow\mathrm{II}_{4,20}\to
\mathrm{Mukai}(K3)$, realising Poincar\'e duality as a commutative
triangle linking the three faces (Mukai, chiral-Hochschild,
Siegel-modular).
\end{remark}

\begin{remark}[$N$-parametric refinement of the three-faces triangle]
\label{rem:pd-DNA-5-wave15-N-parametric}
The $N$-parametric family
$\{\mathbf H^{(N)}_{\Delta_5}\}_{N\ge 2}$ of class-$\mathcal S$
K3 chiral bialgebras
(Vol.~I Proposition~\ref{prop:thqg-occ-CD-ANm1-24}) refines the
three-face triangle of Remark~\ref{rem:pd-DNA-2} to an
...
\item \emph{Mukai face.} At $N\ge 2$, the Maulik--Okounkov
  ...
  acquires an $\mathfrak{sl}_N$-refinement via the
  $\mathrm{SU}(N)^{24}$ flavour action on the $24$ punctures;
  the horizontal rank upgrades from $24$ at $N=2$ to
  $(N^2-1)\cdot 24 + 22(N-1)(N-2)(N+2)/2 + \ldots$ via the
  class-$\mathcal S$ counts of
  Vol.~I Proposition~\ref{prop:thqg-occ-CD-ANm1-24}.
\item \emph{Chiral-Hochschild face.} The $N$-parametric Hochschild
...
\item \emph{Siegel-modular face.} The paramodular cusp form
  $\Delta_5$ at $N = 2$ lifts to a weight-$k_N$ Siegel form on
  $\mathrm{Sp}_{2r(N)}(\mathbb Z)$ at $N \ge 3$, with
  $k_N = \tfrac{1}{2}(N+3)$ from the Borcherds rank formula
  (Vol.~I Remark~\ref{rem:thqg-occ-paramodular-lift-N}); the
  existence of this lift as a closed automorphic form at
  $N \ge 3$ is conjectural.
\end{enumerate}
The $N$-parametric Lusztig specialisation
...
Remark~\ref{rem:pd-DNA-2}: the bridge identity
...
against the rank-$N$ BKM core, not a new $\hbar$).
\end{remark}
