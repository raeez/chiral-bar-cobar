...
The governing question of this chapter is how Verdier and non-abelian Poincar\'e duality construct the Koszul dual cooperad intrinsically, rather than by defining duality through itself.
\end{remark}
...
\index{Verdier duality!bar computes dual}

The bar construction of Chapter~\ref{chap:bar-cobar} produces a coalgebra $\bar{B}(\cA)$ from any chiral algebra $\cA$. Verdier duality on configuration spaces identifies this coalgebra with the Koszul dual $\cA^!$ intrinsically, without presupposing a definition from classical Koszul theory. The Heisenberg specialization of Chapter~\ref{ch:heisenberg-frame} is the prototypical case.\label{rem:poincare-duality-hms}
...
The dual $\mathcal{A}_2^!$ is typically ``defined'' as the Koszul dual cooperad---but no independent construction as a chiral coalgebra has been given, no first-principles proof that the bar construction computes it exists, and for non-quadratic cases the classical orthogonality criterion $R_1 \perp R_2$ does not apply.
...
\begin{remark}[NAP duality]\label{prin:NAP-solution}
...
Construction~\ref{const:A-dual-intrinsic} defines the Koszul dual $\mathcal{A}^!$ intrinsically via Verdier duality on factorization homology, with no reference to bar constructions or orthogonal relations. Theorem~\ref{thm:bar-computes-dual} then identifies $\bar{B}^{\text{ch}}(\mathcal{A}) \xrightarrow{\sim} \mathcal{A}^!$, and for Koszul pairs $(\mathcal{A}_1, \mathcal{A}_2)$, Theorem~\ref{thm:symmetric-koszul} gives the symmetric statement
