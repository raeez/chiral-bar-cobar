...
third object, already an algebra:
\begin{equation}\label{eq:homotopy-koszul-dual-def}
K_X(\cA):=\mathbb{D}_{\Ran}\barB_X(\cA).
\end{equation}
For a quadratic presentation, the coalgebra map
$q_\cA\colon\cA^{\mathrm i}\to\barB_X(\cA)$ gives the algebra map
\[
\mathbb D(q_\cA)\colon K_X(\cA)\longrightarrow
\mathbb D_{\Ran}(\cA^{\mathrm i}).
\]
On the finite Koszul locus, local finite duality identifies the target
with $(\cA^{\mathrm i})^\vee$.  A chosen Koszul partner carries a
separate map $\nu_\cA\colon K_X(\cA)\to\cA^!$, an equivalence under
the pair package.  Here $\cA^{\mathrm i}$ is the quadratic coalgebra;
when $q_\cA$ is a quasi-isomorphism it computes
$H^\bullet(\barB_X(\cA))$.
The cobar counit
$\Omegach_X\barB_X(\cA)\xrightarrow{\sim}\cA$ gives bar--cobar
reconstruction of the original algebra.
...
exchanges $!$-tensor and $*$-tensor. Hence it carries the bar
coalgebra to the algebra whose operations are the Verdier transposes
of the bar operations:
\begin{center}
\begin{tabular}{r@{\;$\longleftrightarrow$\;}l}
bar coproduct $\Delta_{\barB}$ & product $\mu$ of $K_X(\cA)$, \\
bar counit $\varepsilon_{\barB}$ & unit of $K_X(\cA)$, \\
bar differential $d_{\barB}$ & differential of $K_X(\cA)$, \\
...
$\cA^{\mathrm i}=C_X(s^{-1}V,s^{-2}R)$ & quadratic coalgebra of a chosen
 presentation, with $q_\cA\colon\cA^{\mathrm i}\to\barB_X(\cA)$, \\
$K_X(\cA)=\mathbb{D}_{\Ran}\barB_X(\cA)$ &
 Verdier homotopy Koszul-dual algebra, \\
$\nu_\cA\colon K_X(\cA)\to\cA^!$ & chosen-pair algebra map, \\
$\cA^! = (\cA^{\mathrm i})^\vee$ & Verdier or continuous linear dual algebra,
...
\end{tabular}
\end{center}
The genus-$g$ obstruction theory uses the Verdier algebra
$K_X(\cA)$ and, after~$\nu_\cA$, its chosen strict partner~$\cA^!$.
The cochain-level
...
The bar coalgebra of $\mathcal{A}_1$ first enters Verdier duality.
The resulting algebra and its chosen-pair comparison are
\[
H^\bullet\bar{B}^{\mathrm{ch}}(\mathcal{A}_1)
\simeq \mathcal{A}_1^i
\quad\text{and}\quad
K_X(\mathcal A_1):=\mathbb{D}_{\Ran}
\bar{B}^{\mathrm{ch}}(\mathcal{A}_1)
\xrightarrow{\ \nu_{12}\ }\mathcal{A}_2\simeq\mathcal{A}_1^!
\]
...
duals; these hypotheses define the continuous Verdier algebra.
...
$\mathcal{A}^!$ denotes the chosen target of the pair map from the
Verdier algebra. The formula is a theorem under these compactness,
...
level-$k$ pairing that identifies
...
\begin{construction}[Verdier algebra and chosen Koszul partner]\label{const:A-dual-intrinsic}
...
Define the Verdier homotopy Koszul-dual algebra by
\[
K_X(\mathcal A):=
\mathbb{D}_{\Ran}\barB_X^{\mathrm{ch}}(\mathcal{A}).
\]
The $n$-ary component of $K_X(\mathcal A)$ is
represented by distributional sections on $C_n(X)$:
\[
(K_X(\mathcal A))_n
\simeq
...
\right).
\]
The Verdier transposes of the bar coproduct, counit, and differential
give the product, unit, and differential of $K_X(\mathcal A)$.

Choose a quadratic presentation
$\mathcal A=T_X(V)/(R)$ and form its quadratic coalgebra
\[
\mathcal{A}^{\mathrm i}:=C_X(s^{-1}V,s^{-2}R),
\qquad
q_{\mathcal A}\colon\mathcal A^{\mathrm i}\longrightarrow
\barB_X^{\mathrm{ch}}(\mathcal A).
\]
On the finite Koszul locus, $q_{\mathcal A}$ is a quasi-isomorphism and
the locally finite dual
\[
\mathcal{A}^! := (\mathcal{A}^{\mathrm i})^\vee
\]
is the strict Koszul-dual algebra.  The quadratic comparison
has Verdier transpose
\[
\mathbb D(q_{\mathcal A})\colon K_X(\mathcal A)\longrightarrow
\mathbb D_{\Ran}(\mathcal A^{\mathrm i}).
\]
A chosen Koszul pair supplies
$\nu_{\mathcal A}\colon K_X(\mathcal A)\to\mathcal A^!$; formality and
local finite duality make this map an equivalence.
\end{construction}

\begin{remark}\label{rem:intrinsic-nature}
Construction~\ref{const:A-dual-intrinsic} forms the Verdier algebra
from~$\cA$ alone. The chain coalgebra is $\barB_X(\cA)$. A subsequent
quadratic presentation supplies
$\cA^{\mathrm i}$ and $q_\cA$. The algebra is
$K_X(\cA)=\mathbb{D}_{\Ran}\barB_X(\cA)$, with chosen strict target
$\cA^!=(\cA^{\mathrm i})^\vee$ through~$\nu_\cA$ when finite Koszul formality
holds.
\end{remark}

\subsection{Verification of coalgebra axioms}

\begin{theorem}[Bar coalgebra and Verdier algebra; \ClaimStatusProvedHere]\label{thm:coalgebra-via-NAP}
...
\[
K_X(\mathcal A)=\mathbb{D}_{\Ran}\barB_X^{\mathrm{ch}}(\mathcal{A}),
\]
is a factorization algebra; its multiplication, unit, and differential
are the Verdier transposes of these coalgebra operations.
\end{theorem}
...
dual in the completed case, transposes these coalgebra identities into
the factorization-algebra identities for $K_X(\mathcal A)$.
\end{proof}

\section{The bar coalgebra, its quadratic model, and its Verdier algebra}

\begin{theorem}[Bar coalgebra, quadratic model, and Verdier algebra; \ClaimStatusProvedHere]\label{thm:bar-computes-dual}
...
with a quadratic presentation
$\mathcal A=T_X(V)/(R)$ and quadratic coalgebra
$\mathcal A^{\mathrm i}=C_X(s^{-1}V,s^{-2}R)$,
valued in bounded holonomic $\mathcal{D}$-modules in the
Verdier-dualizable ambient. Assume that the configuration-space
...
\item The canonical quadratic comparison
 $q_{\mathcal A}\colon\mathcal A^{\mathrm i}\to
 \bar{B}^{\mathrm{ch}}(\mathcal A)$ is a quasi-isomorphism precisely
 on the quadratic Koszul locus.
\item Its Verdier algebra
 $K_X(\mathcal A):=\mathbb{D}_{\Ran}\bar{B}^{\mathrm{ch}}(\mathcal{A})$
 is equipped with the perfect finite-type
 evaluation pairing, respectively the continuous evaluation pairing,
 \[
 \bar{B}^{\mathrm{ch}}(\mathcal{A})\otimes
 K_X(\mathcal A) \longrightarrow \omega_{\Ran}.
 \]
\item Verdier duality reverses the quadratic comparison to the
 algebra map
 \[
 \mathbb D(q_{\mathcal A})\colon K_X(\mathcal A)
 \longrightarrow\mathbb D_{\Ran}(\mathcal A^{\mathrm i}).
 \]
 On the finite Koszul locus this is a quasi-isomorphism.  A chosen
 Koszul pair supplies
 \[
 \nu_{\mathcal A}\colon K_X(\mathcal A)\longrightarrow
 \mathcal A^!:=(\mathcal A^{\mathrm i})^\vee,
 \]
 and the pair package makes~$\nu_{\mathcal A}$ an equivalence.
\end{enumerate}
\end{theorem}
...
\end{align*}
so the dual object is the Verdier algebra $K_X(\mathcal A)$ of
Construction~\ref{const:A-dual-intrinsic}.
The coalgebra coproduct on $\barB^{\mathrm{ch}}(\mathcal{A})$
dualizes to the multiplication on $K_X(\mathcal A)$; the bar counit
dualizes to its unit.

In the finite-type case Verdier duality gives, with the dimension
shifts fixed by Theorem~\ref{thm:verdier-config},
\[
H^\bullet(K_X(\mathcal A))
\cong
...
the inverse limit.  The map $q_{\mathcal A}$ gives the asserted
quadratic comparison, and contravariance gives
$\mathbb D(q_{\mathcal A})$.  Local finite duality identifies
$\mathbb D_{\Ran}(\mathcal A^{\mathrm i})$ with
$(\mathcal A^{\mathrm i})^\vee$; the chosen pair map~$\nu_{\mathcal A}$
records the further identification with~$\mathcal A^!$.
\end{proof}

\begin{example}[Heisenberg specialization]\label{ex:bar-dual-heisenberg}
For $\cA = \cH_k$, Theorem~\ref{thm:bar-computes-dual} gives
$q_{\cH_k}\colon\cH_k^{\mathrm i}\xrightarrow{\sim}
\bar{B}^{\mathrm{ch}}(\cH_k)$ and the Verdier algebra
$K_X(\cH_k)=\mathbb{D}_{\Ran}\bar{B}^{\mathrm{ch}}(\cH_k)$.
The bar complex is built from
logarithmic forms on $\overline{C}_n(X)$; its Verdier dual is the
algebra represented by distributional currents on $C_n(X)$.  The
Heisenberg pair map is
\[
...
\[(K_X(\mathcal A))^{(0)} = \mathbb{D}(\mathcal{A}) \otimes \omega_X\]
...
part of the chiral Koszul-pair datum is the pair of algebra maps
\begin{align}
\nu_{12}\colon K_X(\mathcal A_1)
=\mathbb{D}_{\Ran}\bar{B}^{\mathrm{ch}}(\mathcal{A}_1)
&\xrightarrow{\sim} \mathcal{A}_2,\\
\nu_{21}\colon K_X(\mathcal A_2)
=\mathbb{D}_{\Ran}\bar{B}^{\mathrm{ch}}(\mathcal{A}_2)
&\xrightarrow{\sim} \mathcal{A}_1.
\end{align}
Equivalently, on the finite Koszul locus,
\[
q_i\colon\mathcal{A}_i^{\mathrm i}
\xrightarrow{\sim}\bar{B}^{\mathrm{ch}}(\mathcal{A}_i),
\qquad
\mathcal{A}_i^!=(\mathcal{A}_i^{\mathrm i})^\vee,
\]
and the notation $\mathcal{A}_2\simeq\mathcal{A}_1^!$,
$\mathcal{A}_1\simeq\mathcal{A}_2^!$ names the targets of the pair
maps.
...
\begin{proof}
Theorem~\ref{thm:bar-computes-dual} gives the coalgebra
$\bar{B}^{\mathrm{ch}}(\mathcal{A})$ and its Verdier algebra
$K_X(\mathcal A)=\mathbb{D}_{\Ran}\bar{B}^{\mathrm{ch}}(\mathcal{A})$.
Definition~\ref{def:chiral-koszul-pair}
requires twisting morphisms, filtrations, Koszulness, and precisely
the Verdier-compatible maps from these algebras to the opposite
member of the pair. Applying factorization homology gives the
displayed NAP equivalence after the
...
pair with Verdier-dualizable bar components and locally finite
quadratic models, then:
\begin{align}
q_1\colon\mathcal A_1^{\mathrm i}
&\xrightarrow{\sim}\bar{B}^{\mathrm{ch}}(\mathcal{A}_1),\\
q_2\colon\mathcal A_2^{\mathrm i}
&\xrightarrow{\sim}\bar{B}^{\mathrm{ch}}(\mathcal{A}_2),\\
\nu_{12}\colon K_X(\mathcal A_1)
&\xrightarrow{\sim}\mathcal{A}_2\simeq\mathcal{A}_1^!,\\
\nu_{21}\colon K_X(\mathcal A_2)
&\xrightarrow{\sim}\mathcal{A}_1\simeq\mathcal{A}_2^!,\\
\Omega^{\mathrm{ch}}\bar{B}^{\mathrm{ch}}(\mathcal{A}_i)
&\simeq \mathcal{A}_i \qquad (i=1,2).
\end{align}

Thus $\Omega^{\mathrm{ch}}\bar{B}^{\mathrm{ch}}$ reconstructs each
member, while $K_X=\mathbb D_{\Ran}\bar B^{\mathrm{ch}}$ and the two
maps~$\nu_{ij}$ realize duality between the members. In the completed
case the Verdier algebras use continuous duals.
\end{theorem}

\begin{proof}
The first two maps are the quadratic Koszul comparisons. The two
maps~$\nu_{ij}$ are exactly the Verdier compatibility data of
...
\[
\nu_k\colon K_X(\cH_k)=\mathbb{D}_{\Ran}
\bar{B}^{\mathrm{ch}}(\cH_k)
\xrightarrow{\sim} \mathrm{Sym}^{\mathrm{ch}}(V^*)
\]
and
...
The bar complex has infinitely many bar-length components.  Its
cohomology therefore lives naturally in a completed conilpotent
category, and its continuous Verdier dual is a complete algebra.
...
\begin{definition}[Completed Verdier algebra and coalgebra model]\label{def:completed-dual}
For a chiral algebra $\mathcal{A}$, define its completed Verdier
algebra by
\[
\widehat K_X(\mathcal A):=
\mathbb{D}^{\mathrm{cont}}_{\Ran}
\widehat{\bar{B}}^{\mathrm{ch}}(\mathcal{A}).
\]
When a complete quadratic or minimal coalgebra model
$\widehat{\mathcal A^{\mathrm i}}$ is supplied, its comparison and
Verdier transpose are
\[
\widehat q_{\mathcal A}\colon\widehat{\mathcal A^{\mathrm i}}
\longrightarrow\widehat{\bar B}^{\mathrm{ch}}(\mathcal A),
\qquad
\mathbb D(\widehat q_{\mathcal A})\colon\widehat K_X(\mathcal A)
\longrightarrow\mathbb D^{\mathrm{cont}}_{\Ran}
(\widehat{\mathcal A^{\mathrm i}}).
\]
The continuous dual
$(\widehat{\mathcal A^{\mathrm i}})^\vee_{\mathrm{cont}}$ becomes a
chosen strict partner through a separate pair map.
...
coalgebra. Its continuous Verdier dual is the completed homotopy
dual algebra:
\[
\mathbb{D}^{\mathrm{cont}}_{\Ran}
\widehat{\bar{B}}^{\mathrm{ch}}(\mathcal{A})
=\widehat K_X(\mathcal A).
\]
If a model~$\widehat{\mathcal A^{\mathrm i}}$ and a filtered
quasi-isomorphism~$\widehat q_{\mathcal A}$ are supplied, then
$\mathbb D(\widehat q_{\mathcal A})$ is a quasi-isomorphism of complete
algebras.  A chosen completed Koszul pair further supplies
$\widehat\nu_{\mathcal A}\colon\widehat K_X(\mathcal A)\to
(\widehat{\mathcal A^{\mathrm i}})^\vee_{\mathrm{cont}}$.
...
recovers $\mathcal{A}$ when the completion map
...
its domain consists of the stated complete inverse systems.
...
the graded pieces.  Hypothesis~(4) supplies the Mittag--Leffler
condition and gives
...
$H^\bullet\widehat K_X(\mathcal A)\cong
H^\bullet(\widehat{\bar B}^{\mathrm{ch}}(\mathcal A))^\vee_{\mathrm{cont}}$.
A supplied map~$\widehat q_{\mathcal A}$ dualizes to the
complete algebra map of the statement.
...
\emph{Completed coalgebra model, first associated-graded window.}
Assume a complete coalgebra model
$\widehat{W_3^{\mathrm i}}$ and a filtered quasi-isomorphism
$\widehat q_{W_3}\colon\widehat{W_3^{\mathrm i}}\to
\widehat{\bar B}^{\mathrm{ch}}(W_3)$ as in
Definition~\ref{def:completed-dual}.  Its first associated-graded
window is
\[
\operatorname{gr}^{(1)}\widehat{W_3^{\mathrm i}}
=\operatorname{coFree}(L^*,W^*,\Lambda^*,
...
\[
\widehat{W_3^!}:=(\widehat{W_3^{\mathrm i}})^\vee_{\mathrm{cont}}.
\]
The chain-level comparison is
\[
\mathbb D(\widehat q_{W_3})\colon
\widehat K_X(W_3)\xrightarrow{\sim}
\mathbb D^{\mathrm{cont}}_{\Ran}(\widehat{W_3^{\mathrm i}}),
\]
followed by the chosen-pair map to~$\widehat{W_3^!}$.
...
\[\Omega^{\text{ch}}(\widehat{W_3^{\mathrm i}})
\simeq \widehat{W_3}\]
...
Construction~\ref{const:A-dual-intrinsic} defines the Verdier
homotopy dual algebra
$K_X(\cA)=\mathbb{D}_{\Ran}\barB_X^{\mathrm{ch}}(\cA)$.
For a quadratic presentation,
Theorem~\ref{thm:bar-computes-dual} supplies
$q_\cA\colon\cA^{\mathrm i}\to\barB_X^{\mathrm{ch}}(\cA)$ and
\[
\mathbb D(q_\cA)\colon K_X(\cA)\longrightarrow
\mathbb D_{\Ran}(\cA^{\mathrm i}).
\]
Local finite duality identifies the target with
$\cA^!=(\cA^{\mathrm i})^\vee$ on the finite Koszul locus.
...
extends this to non-quadratic algebras after the separated
...
\item \emph{Mukai face.} At $N\ge 2$, the Maulik--Okounkov
 ...
 is conjectured to acquire an $\mathfrak{sl}_N$-refinement through the
