...
\end{tabular}
\end{center}
The fundamental insight is that $\bar{B}(\cA) = \cA^!_{\mathrm{coal}}$: the bar construction does not \emph{compute} the dual, it \emph{is} the dual.  The classical object $\cA^!_{\mathrm{coal}}$ is merely the cohomological shadow $H^*(\bar{B}(\cA))$, visible only when $\cA$ is Koszul.  In general, $\bar{B}(\cA)$ retains all higher bar cohomology $H^i(\bar{B}(\cA))$ for $i \geq 1$, not just the concentrated piece $H^0 \cong \cA^!_{\mathrm{coal}}$; it is the derived enhancement, and the correct object for homotopy-theoretic purposes.
...
\emph{The circularity.}
The dual $\mathcal{A}_2^!$ is typically ``defined'' as the Koszul dual cooperad---but no independent construction as a chiral coalgebra has been given, no first-principles proof that the bar construction defines a dg coalgebra that \emph{is} this dual exists, and for non-quadratic cases the classical orthogonality criterion $R_1 \perp R_2$ does not apply.

\emph{What is needed.}
An intrinsic definition of $\mathcal{A}_2^!$ using only $\mathcal{A}_2$; a proof that $\mathcal{A}_2^!$ is inherently a coalgebra; a geometric identification of $\bar{B}^{\text{ch}}(\mathcal{A}_1)$ with this dual; and an extension to non-quadratic and curved cases via nilpotent completion.
\end{problem}
...
On $\overline{C}_k(X)$, two dual sheaf complexes live naturally.
...
\begin{theorem}[Verdier duality for configuration spaces; \ClaimStatusProvedElsewhere{} \cite{KashiwaraSchapira}]\label{thm:verdier-config}
...
The general theory of Verdier duality for constructible complexes on smooth varieties~\cite{KashiwaraSchapira} gives perfectness.
\end{proof}
...
\cite{KashiwaraSchapira}.  See Theorem~\ref{thm:cobar-distributional-model}
...
\emph{Step 1: Configuration space valued factorization algebra.}
...
Construction~\ref{const:A-dual-intrinsic} uses only configuration space geometry, Verdier duality, and factorization --- no bar construction, orthogonal relations, or quadraticity.
\end{remark}
...
\emph{Geometric interpretation.} The completion sums over all collision patterns:
...
\begin{remark}[How this chapter is used later]\label{rem:nap-usage}
The NAP duality results serve three later roles:
\begin{enumerate}
\item Theorem~\ref{thm:main-NAP-resolution} supplies the intrinsic
  definition of Koszul dual used by
  Theorem~A$_2$ (Theorem~\ref{thm:bar-cobar-isomorphism-main}).
\item Theorem~\ref{thm:verdier-bar-cobar} provides the Verdier
  compatibility input for
  Theorem~C (Theorem~\ref{thm:quantum-complementarity-main}).
\item The explicit NAP computations of
  Chapter~\ref{chap:NAP-computations} give independent
  computational checks for Heisenberg, Kac--Moody, and
  $\beta\gamma$/bc Koszul pairs.
\end{enumerate}
\end{remark}
