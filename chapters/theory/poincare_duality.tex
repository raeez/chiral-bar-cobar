...
The Verdier duality that exchanged the Heisenberg algebra and its Koszul dual in Chapter~\ref{ch:heisenberg-frame} is a special case of a general mechanism on Fulton--MacPherson spaces. This chapter establishes the Verdier framework for chiral bar complexes.

\section{Introduction}

The bar construction of Chapter~\ref{chap:bar-cobar} produces a
coalgebra $\bar{B}(\cA)$ from any chiral algebra $\cA$.  But why
should this coalgebra be the Koszul dual?  The answer is Verdier
duality: non-abelian Poincar\'e duality on configuration spaces
identifies $\bar{B}(\cA)$ with $\cA^!$ \emph{intrinsically},
without presupposing a definition of $\cA^!$ from classical Koszul
theory.  This chapter resolves the circularity and establishes the
geometric foundation: the bar construction computes the Koszul dual
because Verdier duality on $\operatorname{Ran}(X)$ identifies the
two.
...
\item We have not given an \emph{independent construction} of $\mathcal{A}_2^!$ as a chiral coalgebra
\item We have not \emph{proven from first principles} that the bar construction actually computes this dual
\item For non-quadratic cases, the classical orthogonality criterion $R_1 \perp R_2$ does not apply
\end{itemize}
...
Non-abelian Poincaré duality resolves this circularity:

In quantum field theory (Witten), correlation functions satisfy:
...
Mathematically (Grothendieck), for a factorization algebra $\mathcal{A}$ on a manifold $M$:
...
\item $\mathcal{A}^!$ is the \emph{Koszul dual}, defined via this duality
\end{itemize}
...
This duality \emph{defines} $\mathcal{A}^!$ intrinsically.
\end{remark}
...
\emph{Observation \textup{(Serre)}.} Boundary divisors $D \subset \overline{C}_k(X)$ are in bijection with:
\begin{itemize}
...
\subsection{Construction of $\mathcal{A}^!$ via Verdier duality}
...
\begin{remark}[Intrinsic nature of the construction]\label{rem:intrinsic-nature}
...
For $\mathcal{H}_k$, the bar produced the Lie-type coalgebra and the cobar produced the Com-type algebra (\S\ref{sec:frame-koszul-dual}): bar and Verdier dual coincided.  The following theorem establishes this identification in general via non-abelian Poincar\'e duality.
...
\subsection{Koszul pairs via NAP}

\begin{proposition}[Chiral Koszul pair via NAP]\label{def:koszul-pair-NAP}
...
\emph{Step~4: Cobar.}
By formal smoothness (hypothesis~(3)),
$\dim \mathrm{CH}^n(\mathcal{A}) < \infty$
(Theorem~\ref{thm:existence-koszul}(2)), so the cobar
...
This is not a definition but a theorem, derived from the
NAP identity:
$$\int_X \mathcal{A}_1 \simeq \mathbb{D}\left(\int_{-X} \mathcal{A}_2\right)$$
\end{theorem}

\begin{proof}
The intrinsic Koszul dual $\mathcal{A}^!$ is defined via
Verdier duality on factorization homology
(Definition~\ref{def:geom-cobar-intrinsic}):
$\mathcal{A}^! := \mathbb{D}(\int_{-X} \mathcal{A})$,
using only the factorization structure and Verdier duality,
with no reference to bar constructions.  The identification
$\bar{B}^{\mathrm{ch}}(\mathcal{A}) \xrightarrow{\sim}
\mathcal{A}^!$ is Theorem~\ref{thm:verdier-bar-cobar},
whose proof proceeds by comparing the geometric bar complex
with the Verdier dual via the configuration space model of
factorization homology
(Theorem~\ref{thm:bar-NAP-homology}).  The Koszul pair
statement follows by applying this identification to both
$\mathcal{A}_1$ and $\mathcal{A}_2$ and using the NAP
identity (Theorem~\ref{thm:verdier-NAP}).
\end{proof}

\begin{remark}[Three viewpoints]\label{rem:three-unified}
The NAP construction admits three readings: physical (chiral algebras as local operators, Koszul duality as S-duality, bar-cobar as loop expansion), geometric (configuration spaces as moduli of field insertions, residues as OPE extraction, Verdier duality as compact/open support exchange), and categorical (factorization algebras as sheaves on the Ran space, NAP duality as orientation reversal, Koszul duality as the essential image of NAP).
\end{remark}

\begin{remark}[Quantum extensions]\label{rem:looking-ahead}
