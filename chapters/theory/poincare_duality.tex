...
\noindent\emph{Governing question.} Can the Koszul dual be constructed
intrinsically from geometric duality, rather than postulated from
classical orthogonality data?  This chapter resolves that circularity:
it upgrades Verdier duality from a comparison tool to the defining
construction of the dual cooperad in the chiral setting.

\begin{remark}[Semantic levels; Convention~\ref{conv:hms-levels}]
\label{rem:poincare-duality-hms}
Verdier duality on FM compactifications is realized at \textbf{M-level}: the exchange of log forms (bar) with distributional currents (cobar) is an explicit chain-level construction.  The H-level statement---that $\mathbb{D}_{\operatorname{Ran}}$ intertwines bar and cobar as an equivalence of factorization (co)algebras---is referenced from the abstract framework of Ayala--Francis~\cite{AF15}.
\end{remark}
...
\begin{proposition}[Chiral Koszul pair via NAP; \ClaimStatusProvedHere]\label{def:koszul-pair-NAP}
