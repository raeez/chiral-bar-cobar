...
Two chiral algebras $(\mathcal{A}_1, \mathcal{A}_2)$ on $X$ form a \emph{chiral Koszul pair} if there exist quasi-isomorphisms of chiral coalgebras:
\begin{align}
...
\emph{Step 4: Cobar reconstruction.}
...
\begin{proof}
The intrinsic Koszul dual $\mathcal{A}^!$ is defined via Verdier duality on factorization homology (Definition~\ref{def:geom-cobar-intrinsic}): $\mathcal{A}^! := \mathbb{D}(\int_{-X} \mathcal{A})$, using only the factorization structure and Verdier duality, with no reference to bar constructions.  The identification $\bar{B}^{\mathrm{ch}}(\mathcal{A}) \xrightarrow{\sim} \mathcal{A}^!$ is Theorem~\ref{thm:verdier-bar-cobar}, whose proof proceeds by comparing the geometric bar complex with the Verdier dual via the configuration space model of factorization homology (Theorem~\ref{thm:bar-NAP-homology}).  The Koszul pair statement follows by applying this identification to both $\mathcal{A}_1$ and $\mathcal{A}_2$ and using the NAP identity (Theorem~\ref{thm:verdier-NAP}).
\end{proof}
