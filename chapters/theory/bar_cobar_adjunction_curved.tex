...
(quadratic, curved-central, filtered-complete, infinite-type) and the
$I$-adic completion machinery.  The MC4 completion packet for the
standard $W_\infty$ tower (Proposition~\ref{prop:winfty-mc4-frontier-package}).
\end{remark}

This is where the algebraic engine first closes.  Chapters~\ref{ch:algebraic-foundations}
and~\ref{chap:cobar-construction} built the bar functor~$\barBch$ and the
cobar functor~$\Omegach$ separately; here they meet as an adjoint pair
on the $\Ran$-space of a smooth curve~$X$.
Theorem~\ref{thm:bar-cobar-adjunction} (``Theorem~A'') establishes
the adjunction $\Omegach \dashv \barBch$ together with its Verdier
intertwining, and the next part of this chapter
(Theorem~\ref{thm:bar-cobar-inversion-qi}, ``Theorem~B'') inverts it:
on the Koszul locus the counit
$\psi \colon \Omegach(\barBch(\cA)) \xrightarrow{\;\sim\;} \cA$
is a quasi-isomorphism.  The two theorems should be read as a single
statement: $\barBch$ and $\Omegach$ are adjoint views of the same
incidence geometry on the Fulton--MacPherson
compactification~$\overline{\Conf}_n(X)$, and inversion is the
geometric fact that this incidence correspondence is birational on the
Koszul locus.
...
\item \emph{Programmatic} (infinite generators):
  MC4 completion target ($\Walg_\infty$, $\Walg_{1+\infty}$).
\end{enumerate}
...
throughout Parts~II--III, whereas $W_\infty$ marks the locus where a
...
This is the M/S-level completed shadow; the stronger H-level realization via an infinite-generator dual is the MC4 completion problem (Example~\ref{ex:winfty-completion-frontier}).
\end{example}

\begin{example}[\texorpdfstring{$W_\infty$}{W-infinity}: MC4 completion target]
\label{ex:winfty-completion-frontier}

The standard principal-stage completed M-level bar-cobar package for $W_\infty = \varprojlim_N W_N$ is proved (Corollary~\ref{cor:winfty-standard-mc4-package}).  The remaining MC4 problem is to construct a factorization target (Proposition~\ref{prop:winfty-factorization-package}) and prove the residue identities
\[
...
\emph{This is the theorem-shaped form of the MC4 completion problem:
...
MC4 packet was the six-entry block of
Proposition~\ref{prop:winfty-mc4-frontier-package}, now
...
(Theorem~\ref{thm:bar-cobar-inversion}); it is a quasi-isomorphism on
the Koszul locus.
\end{proof}
...
(\bigoplus_{n \geq 1} \mathfrak{p}^\vee_{(n)}'
\oplus \bigoplus_{n \geq 1} \mathfrak{p}^\vee_{(n)}'')$.
...
\begin{remark}[Position in the architecture]
\label{rem:casimir-transgression-position}
The reconstruction/duality separation (Theorem~\ref{thm:reconstruction-vs-duality}) sits at the foundation of the algebraic engine: it clarifies that Theorem~A (adjunction) and Theorem~B (inversion) are about
$R_X$, while the Koszul pair structure of Chapter~\ref{chap:koszul-pair} is about~$K_X$.
The coupled bar complex, with its mixed-boundary spectral sequence
(Theorem~\ref{thm:mixed-boundary-sseq}), extends the engine to
tensor products.  The Casimir transgression
(Theorem~\ref{thm:sugawara-casimir-transgression}) identifies the
first genuinely coupled obstruction, and the quadric rigidity
(Theorem~\ref{thm:casimir-quadric-rigidity}) shows it is the
\emph{only} obstruction in the minimal sector.  The conceptual
upshot: genuine higher $A_\infty$ corrections in coupled chiral
bar complexes arise from \emph{singular} coupling---the failure
of the transgression classes to form a regular sequence---not from
coupling per~se.
\end{remark}
