...
(Theorem~\ref{thm:verdier-bar-cobar}).  In summary:
\begin{center}
...
$\cA^!$ (an algebra): it first dualises the coalgebra to an
algebra, then applies cobar to extract the underlying chiral
algebra.  The crucial point: cobar applied to the Verdier dual
(an algebra) gives~$\cA^!$; cobar applied to the bar coalgebra
directly gives~$\cA$ itself.
\end{remark}
