...
\label{sec:bccurved-wave13-DNA}

\begin{remark}[CY-$2$ curved bar-cobar at K3]
\label{rem:bccurved-DNA-K3-cy2}
...
\label{rem:bccurved-DNA-K3xE-hall-drinfeld}
...
element of
Remark~\ref{rem:bcac-DNA-deep-1},
\[
 w^{\mathrm{Wave13}}
...
Remark~\ref{rem:bcac-DNA-deep-1} from its Wave-13 ambient
status.
\end{remark}

\begin{remark}[Curvature perturbation on $H_1$: $\mathbb{Z}/8$-band]
\label{rem:bcac-wave18-DNA-2}
...
\label{thm:bcac-wave18-trichotomy}
...
(Remark~\ref{rem:bcac-wave18-DNA-1}).
...
Remark~\ref{rem:bcac-wave18-DNA-3} to repair inversion at the
$A_\infty$ level
...
(Remark~\ref{rem:bcac-DNA-deep-1}) is uniform on $U$ because the
Mittag--Leffler condition holds uniformly on $U$
(Lemma~\ref{lem:cclimax-wave18-H1-failure} shows it fails only at
$H_1$, Lemma~\ref{lem:cclimax-wave18-H4-failure} only at $H_4$,
and the three-loop coboundary of
Lemma~\ref{lem:cclimax-wave18-H3-failure} is non-zero only at $H_3$;
...
(Remark~\ref{rem:bcac-wave18-DNA-3}).
...
\label{rem:bcac-wave18-DNA-5}
\index{universal trace identity!W18 chain-level footing}
The W18 trichotomy
(Theorem~\ref{thm:bcac-wave18-trichotomy}) provides the
