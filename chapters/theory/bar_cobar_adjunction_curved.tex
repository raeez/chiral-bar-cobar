...
\end{equation}
which is the $\Sigma_n$-coinvariant projection at each arity.
...
\begin{enumerate}[label=\textup{(\roman*)}]
\item \emph{The $R$-matrix.}  The genus-$0$, arity-$2$ component
  ...
  \textup{(}equation~\eqref{eq:intro-kappa-from-r}\textup{)}: the
...
ambiguities that prevent reconstruction.  The four depth classes
