...
The Ran bar--cobar theorem gives the counit
\[
 \psi_\cA\colon
 \Omegach\barBch_X(\cA)\xrightarrow{\ \sim\ }\cA
\]
in its pro-nilpotent ambient.  Theorem~B adds a quadratic coalgebra
\(\cA^{\mathrm i}\), the map
\(q_\cA\colon\cA^{\mathrm i}\to\barBch_X(\cA)\), and the acyclicity
criterion for its twisted tensor products.  Verdier duality and the
formation of~\(\cA^!\) are separate operations with their own
targets.

Filtered and curved presentations include Virasoro,
\(\mathcal W_N\), and \(\mathcal W_\infty\).  Their structure maps
satisfy
\(\mu_0\neq0\) and
\(\mu_1^2=[\mu_0,-]_{\mu_2}\).
The homotopy-invariant convolution algebra
\(\Convinf(\barBch(\cA),\cA)\) carries the twisting
Maurer--Cartan element.  A curvature-killing MC twist or a
square-zero total differential produces an ordinary chain
comparison.  The second-kind coderived presentation instead
requires a coacyclic cone and detects equivalence on comodules over
the completed CDG-coalgebra.

The precise ambient content of Theorem~A
(Theorem~\ref{thm:bar-cobar-adjunction}) is enhanced bar--cobar
equivalence for the reduced associative operad in the pro-nilpotent
Ran category.  It carries the unit
\[
 \eta_\cC\colon\cC\longrightarrow\barBch\Omegach(\cC)
\]
and the displayed counit~\(\psi_\cA\).  The package
\(H_{\mathrm{fact}}\) restricts these maps to the selected
factorization subcategories.  The separate Verdier package forms the
algebra
\[
 K_X(\cA):=\mathbb D_{\Ran}\barBch_X(\cA).
\]
A quadratic comparison identifies this algebra with the continuous
realization of \((\cA^{\mathrm i})^\vee\); a pairwise Verdier package
identifies it with a chosen dual partner.  The completed coderived
localization
\(D^{\co}(\barBch\text{-}\mathrm{CoFact})\)
carries the corresponding coacyclic-cone formulation.
...
\item \emph{Curved-central} ($d_0^{\,2} \neq 0$,
 ...
 model; the raw fiberwise operator is curved, with scalar diagonal
 curvature $\kappa \cdot \omega_g$ (affine algebras, Virasoro);
\item \emph{Filtered-complete} (non-quadratic OPE):
 $I$-adic completion required, work in the weight-completed
 coderived category
 ($\Walg_N$ at finite~$N$);
\item \emph{Infinite-complete} (infinite generators):
...
pro-complete) that determines which completions are needed. The
second part
...
\begin{remark}[The ordered bar before averaging]
\label{rem:bar-ordered-primacy}
\index{ordered bar complex!before averaging}
...
The level-$2$ ordered object before averaging is the \emph{ordered} bar complex
...
\end{equation}
which is the untwisted characteristic-zero Reynolds representative of
the $\Sigma_n$-coinvariant projection at each degree. With nontrivial
braiding, this line is replaced by the $L_R$-twisted derived completed
coinvariant descent of the universal conductor. On the
PBW/Koszul $E_\infty$ surface the untwisted map is a quasi-isomorphism
of chain complexes.  Its source carries deconcatenation, while its
target carries the coshuffle coproduct; a coalgebra comparison between
these structures is additional data.  The ordered-to-symmetric map
therefore exposes three comparison problems:
\begin{enumerate}[label=\textup{(\roman*)}]
\item \emph{The collision kernel.} The genus-$0$, degree-$2$ component
 of the ordered MC element
 $\Theta_\cA^{\Eone}$ is the collision kernel
 $r_\cA^{\mathrm{coll}}(z)
 \in \End(V^{\otimes 2}) \otimes \cO(*\Delta)$.  A classical
 $r$-matrix designation follows from a separately constructed CYBE
 identity in a specified representation.  Homotopy coinvariants retain
 the equivariant endomorphism data; a normalized trace
 $\tau_\cA\colon\End_\cA(2)_{h\Sigma_2}\to\C$ gives the scalar
 $\kappa(\cA)=\tau_\cA(r_\cA^{\mathrm{coll}})$ for abelian algebras
 (for non-abelian Kac--Moody,
 $\kappa = \tau_\cA(r_\cA^{\mathrm{coll}}) + \dim(\fg)/2$;
 see equation~\eqref{eq:intro-kappa-from-r}): the
 scalar is the trace realization of the operator-valued kernel.
\item \emph{The braided monoidal category.} The deconcatenation
 ...
 $\Factord(\cA)$.  A Yang--Baxter operator together with factorization
 descent supplies the braiding, and the symmetric descent records its
 homotopy-coinvariant realization.
...
For genus $g \geq 1$, the curved fiberwise bar model carries a
curvature element $m_0^{(g)}$. Its scalar diagonal projection is
\[
 \operatorname{tr}_{\mathrm{diag}}\!\bigl(m_0^{(g)}\bigr)
 = \kappa(\mathcal{A}) \cdot \omega_g,
 \qquad
 \omega_g = c_1(\lambda) \in H^2(\overline{\mathcal{M}}_g),
\]
where $\lambda = \det \mathbb{E}$ is the Hodge line bundle on the moduli space. The raw fiberwise curved identity is
\[
...
gravity dictionary:
\begin{enumerate}[label=(\roman*)]
\item $m_0^{(g)} \neq 0$ if and only if the curved genus-$g$ package is nontrivial; on the scalar diagonal this is detected by $\kappa(\mathcal{A}) \neq 0$. This is the precise sense in which gravity is turned on. The scalar diagonal of $m_0^{(g)}$ is the cosmological-constant term of the genus-$g$ curved $A_\infty$ model. For the standard families $m_0^{(g)}$ is central, so the internal commutator formula $m_1^2 = [m_0,-]$ may vanish even when the scalar diagonal curvature is nonzero.
\item $\kappa(\mathcal{A})$ is the coefficient of the Hodge class in the scalar diagonal projection of $m_0^{(g)}$, hence the strength of the genus-$g$ gravitational backreaction in that projection. In that dictionary, $\kappa(\mathcal{A})$ is the Newton coupling. It is family-specific: $\kappa(\widehat{\mathfrak{g}}_k) = (k+h^\vee)\dim\mathfrak{g}/(2h^\vee)$ for affine Kac--Moody, $\kappa(\mathrm{Vir}_c) = c/2$ for Virasoro, and $\kappa(\mathcal{B}^k) = c/6$ for Bershadsky--Polyakov.
\item The critical gravitational coupling is the fixed point of the duality involution on the scalar projection. When the scalar complementarity constant is written as $\kappa(\mathcal{A}) + \kappa(\mathcal{A}^!) = K_\kappa(\mathcal{A})$, the fixed point is $\kappa_{\mathrm{crit}} = K_\kappa(\mathcal{A})/2$. This constant is family-specific. For affine Kac--Moody, Heisenberg, and free-field families one has $K_\kappa = 0$, so $\kappa_{\mathrm{crit}} = 0$. For Virasoro one has $K_\kappa = 13$, so $\kappa_{\mathrm{crit}} = 13/2$, equivalently $c = 13$. For the Bershadsky--Polyakov family the manuscript uses the conditional normalisation $\kappa(\mathcal{B}^k) = c/6$ together with the central-charge conductor $c(\mathcal{B}^k) + c((\mathcal{B}^k)^!) = 50$; hence the self-dual central charge is $c = 25$, and the induced scalar critical coupling is $\kappa_{\mathrm{crit}} = 25/6$.
\end{enumerate}
\end{remark}
...
positive-energy families considered here, the genus-$g$ curvature has
scalar diagonal projection
$\operatorname{tr}_{\mathrm{diag}}(\mu_0)=\kappa(\cA)\cdot\omega_g$;
the corresponding vacuum component is central in the binary bar operation
...
\begin{definition}[I-adic topology; \ClaimStatusDefinitional]
...
\begin{theorem}[State-space direct sum and weight completion;
\ClaimStatusProvedHere]
\label{thm:completion-necessity}
Let \(V_c=\bigoplus_{h\geq0}(V_c)_h\) be the universal Virasoro
vacuum module, and form the state-space bar object graded by total
bar weight
\[
 \bar B_\oplus(V_c)
 =\bigoplus_{w\geq0}\bar B(V_c)_w,
 \qquad
 \bar B_\Pi(V_c)
 =\prod_{w\geq0}\bar B(V_c)_w .
\]
For \(k\geq2\), put
\[
 v_k=L_{-k}\mathbf1\in(V_c)_k,
 \qquad
 e_k=s^{-1}v_k\otimes s^{-1}v_k
 \in\bar B^2(V_c)_{2k+2}.
\]
Then
\[
 \xi=(e_2,e_3,\ldots)
 \in \bar B_\Pi(V_c)\setminus\bar B_\oplus(V_c).
\]
Thus a specified finite-window comparison series whose weight
components are the \(e_k\) has a value in the product completion and
has infinite weight support.
\end{theorem}

\begin{proof}
The state \(v_k\) has conformal weight \(k\), and the twofold bar word
\(e_k\) has total weight \(2k+2\).  These weights are pairwise
distinct.  The direct sum consists of finite-weight-support families,
whereas \(\xi\) has a nonzero component in every weight \(2k+2\).
\end{proof}

\begin{remark}[Scope of the state-space calculation;
\ClaimStatusOpen]\label{rem:completion-necessity-scope}
Theorem~\ref{thm:completion-necessity} proves a statement about the
underlying weight-graded vector space.  An identification of the
vectors \(e_k\) with the components of a bar--cobar homotopy requires
an explicit finite-window contraction.  Given a raw chiral counit
\[
 \varepsilon_A^{\mathrm{raw}}\colon
 \Omega_X^{\mathrm{raw}}B_X^{\mathrm{raw}}(A)\longrightarrow A,
\]
its ordinary-derived status is the acyclicity problem for
\(\operatorname{Cone}(\varepsilon_A^{\mathrm{raw}})\).
For class~\(\mathsf M\), this cone calculation remains open here.
The completed comparison uses a strict inverse system and the
continuity, localization, and second-kind hypotheses
\textup{(CP1)--(CP3)} of
...
Definition~\ref{def:strict-ml-completed-chiral-coalgebra}.  Assume
also the second-kind localization compatibility
\textup{(CP3)} of
Definition~\ref{def:completed-chiral-positselski-package}. Then the
completed cone of
\[
...
The assertion concerns the comparison cone.  The underlying curved
bar object \(\bar B^{(g)}(\mathcal A)\) retains its own coderived
class, and ordinary contractibility after forgetting curvature and
filtration is a separate statement.
\end{proposition}

\begin{proof}
At genus \(g\geq1\), the raw curved identity is
\[
 m_1^{(g)\,2}(a)=[m_0^{(g)},a]_{m_2},
 \qquad
 \operatorname{tr}_{\mathrm{diag}}(m_0^{(g)})
 =\kappa(\mathcal A)\omega_g ,
\]
while the corrected total differential satisfies
\(\Dg{g}^{\,2}=0\)
(Theorem~\ref{thm:genus-universality};
Convention~\ref{conv:higher-genus-differentials}).  For the standard
families, \(m_0^{(g)}\) is central for the binary operation, so the
commutator vanishes.  The curved datum remains visible in the
second-kind CDG totalization and the completed factorization
structure.
...
generate coacyclic objects commute with the inverse limit by
\textup{(CP3)}. Therefore
\[
...
The cone tower is the carrier of the argument.  Coacyclicity of its
limit follows from the finite-stage coacyclicity, strict
Mittag--Leffler passage, and \textup{(CP3)}.  A contracting homotopy
for the bar object itself would be an additional construction.
\end{proof}

\begin{remark}[Coderived meaning of the comparison cone]
\label{rem:positselski-acyclicity}
At genus $g \geq 1$ with $\kappa \neq 0$, coacyclicity of
$\operatorname{Cone}(\widehat\epsilon)$ makes the bar--cobar
comparison map invertible in the coderived category.  The raw curved
bar coalgebra remains the base object for the associated comodule
homological algebra.  Its ambient category is Positselski's coderived
category
$D^{\mathrm{co}}(\bar{B}^{(g)}\text{-}\mathrm{CoMod})$~\cite[\S\S3--4]{Positselski11},
and Theorem~\ref{thm:positselski-chiral} uses the corresponding
...
Let \(C=\bar B^{\mathrm{ch}}(\cA)\) with curvature
\(h=m_0^{(g)}\).  After a specified de Rham realization, the ordinary
CDG-coalgebra \(C_{\mathrm{dR}}\) carries Positselski's
fixed-coalgebra equivalence
\[
 D^{\mathrm{co}}(C_{\mathrm{dR}}\text{-}\mathrm{comod})
 \simeq
 D^{\mathrm{ctr}}(C_{\mathrm{dR}}\text{-}\mathrm{contra})
\]
for arbitrary \(C_{\mathrm{dR}}\).  In the uncurved conilpotent case,
Positselski's Corollary~4.4 identifies the coderived and ordinary
derived comodule categories.

A chiral statement requires functors preserving the chiral
coaction, contraaction, factorization maps, and the second-kind
acyclic subcategories.  A completed chiral statement further requires
the strict transition maps, localization comparison, and Milnor
vanishing in \textup{(CP1)--(CP3)}.  Positive-energy grading supplies
finite-dimensional weight pieces.  Compact generation and passage
through the completed Verdier quotients are additional hypotheses.
The worked examples in this chapter establish selected finite-window
and continuity inputs; their full completed chiral equivalences retain
...
This theorem is imported and treated as \ClaimStatusConditional after
the filtered-to-curved reduction is imposed. The
...
\begin{theorem}[When filtered reduces to curved; \ClaimStatusConditional]
...
\begin{definition}[Conilpotent coalgebra; \ClaimStatusDefinitional]
...
\begin{theorem}[Algebraic bar--cobar resolution and fixed-coalgebra
correspondence; \ClaimStatusProvedElsewhere]
...
Let \(k\) be a field.
\begin{enumerate}[label=\textup{(\roman*)}]
\item For every augmented dg \(k\)-algebra \(A\), the bar coalgebra
 \(B(A)\) is conilpotent and the counit
 \[
  \varepsilon_A\colon\Omega B(A)\xrightarrow{\sim}A
 \]
 is a quasi-isomorphism.
\item For every conilpotent coaugmented dg \(k\)-coalgebra \(C\), the
 unit
 \[
  \eta_C\colon C\xrightarrow{\sim}B\Omega(C)
 \]
 is a quasi-isomorphism.
\item For every CDG \(k\)-coalgebra \(D\), Positselski's adjoint
 functors induce
 \[
  D^{\mathrm{co}}(D\text{-}\mathrm{comod})
  \simeq
  D^{\mathrm{ctr}}(D\text{-}\mathrm{contra}).
 \]
\end{enumerate}
The first two assertions are
Loday--Vallette~\cite[Corollary~2.3.4]{LV12}; the third is
Positselski~\cite[Theorem~5.2]{Positselski11}.  Their natural ambient
uses algebraic tensor products over \(k\), and the underlying graded
pieces may have arbitrary dimension.
\end{theorem}

\begin{proof}
For \(B(A)=T^c(s^{-1}\bar A)\) in the manuscript's cohomological
convention, deconcatenation makes every bar word
conilpotent.  Loday--Vallette's fundamental theorem of twisting
morphisms applied to the universal twisting morphisms gives the
counit and unit quasi-isomorphisms.  Positselski constructs the
co--contra adjunction for a fixed CDG-coalgebra and identifies its
unit and counit after localization by the coacyclic and
contraacyclic subcategories.

In the desuspension convention used in this manuscript, the ordinary
cobar differential on a generator is
\begin{equation}\label{eq:cobar-diff-generators}
 ...
 -\sum_{(c)}(-1)^{|c'|}
  (s^{-1}c')\otimes(s^{-1}c''),
 \qquad
 \bar\Delta(c)=\sum_{(c)}c'\otimes c'' .
\end{equation}
The internal differential and coassociativity give
\(d_\Omega^2=0\); algebraic tensor products make every displayed
coproduct a finite sum.
\end{proof}

\begin{remark}[Chiral realization and completion;
\ClaimStatusConditional]
\label{rem:conilpotency-chiral-completion}
The enhanced chiral maps
\[
 \Omega_XB_X(A)\longrightarrow A,
 \qquad
 C\longrightarrow B_X\Omega_X(C)
\]
require the Ran/factorization comparison package of Theorem~A.  A
strict weight-completed chain model additionally requires transition
maps commuting with the differential and cooperations, the Milnor
condition for the relevant cone towers, and compatibility with the
second-kind localizations.  These are precisely the additional
assertions \textup{(CP1)--(CP3)} in
Definition~\ref{def:completed-chiral-positselski-package}.  A worked
chiral family must construct these data separately from the ordinary
algebraic theorems above.
\end{remark}

\begin{example}[Heisenberg: conilpotent]
\label{ex:heisenberg-conilpotent-complete}
The ordinary bar coalgebra of the augmented Heisenberg algebra is
\[
 B(\mathcal H_\kappa)=T^c(s\overline{\mathcal H}_\kappa).
\]
For a word of bar length \(n\), the \(n\)-fold reduced
deconcatenation vanishes.  Hence this coalgebra is conilpotent and
Loday--Vallette's algebraic counit
\(\Omega B(\mathcal H_\kappa)\to\mathcal H_\kappa\) is a
quasi-isomorphism.  A chiral Ran realization carries the additional
hypotheses of Theorem~A.
\end{example}

\begin{example}[Virasoro state-space completion]
\label{ex:virasoro-not-conilpotent}
For the universal vacuum module \(V_c\), the states
\(v_k=L_{-k}\mathbf1\), \(k\geq2\), have conformal weight \(k\).
The twofold bar words
\[
 e_k=s^{-1}v_k\otimes s^{-1}v_k
\]
have total weights \(2k+2\).  Their family
\(\xi=(e_2,e_3,\ldots)\) belongs to the product completion
\(\prod_w\bar B(V_c)_w\) and has infinite weight support.  The
algebraic bar coalgebra remains conilpotent on every finite word.
An actual bar--cobar homotopy with these components would therefore
live in the completed Hom complex.  Construction of such a homotopy,
and the ordinary-derived status of the raw chiral counit, are the
separate open problems recorded in
Remark~\ref{rem:completion-necessity-scope}.
\end{example}
...
The presentation used in this example is the weightwise product
completion of Example~\ref{ex:virasoro-not-conilpotent}, followed by
continuous linear duality.
...
Example~\ref{ex:virasoro-koszul-dual} has a rank-$1$ primary-state
shadow. On the highest-weight line of a primary module, the
path-ordered exponential of the Virasoro collision residue becomes a
scalar Ward factor and closes into an elementary function.

\begin{computation}[Primary-state Virasoro Ward factor;
\ClaimStatusProvedHere]
\label{comp:virasoro-spectral-r-matrix}
\index{Virasoro!primary-state Ward factor|textbf}
\index{Ward identity!Virasoro primary-state factor}
For the Virasoro algebra at central charge $c$ acting on a
primary state of conformal weight $h$, the scalar primary-state
Ward factor is
\begin{equation}
...
\[
 ...
 R_6 \;=\; -\frac{c^{3}}{384}.
\]
After the leading factor $z^{2h}$ is removed, only even powers of
$1/z$ appear.
\end{computation}
...
\[
 R^{\mathrm{prim}}_{h,c}(z)
 \;=\; \mathcal{P}\exp\!\left(\oint\;\sum_{n\geq 0}
 r_{n}\,z^{-n-1}\,dz\right)
\]
...
\[
 R^{\mathrm{prim}}_{h,c}(z)\,\lvert h\rangle
...
$\exp(-c/(4z^{2}))$. Expanding the single monomial
$-c/(4z^2)$ gives powers $z^{-2k}$ and no odd inverse powers,
yielding \eqref{eq:virasoro-r-matrix-closed}.
\end{proof}

\begin{remark}[Coefficient expansion and scope]
\label{rem:virasoro-r-matrix-paths}
The scalar primary-state factor
$R^{\mathrm{prim}}_{h,c}(z)=z^{2h}\,\exp(-c/(4z^{2}))$
has the formal expansion
\[
 R^{\mathrm{prim}}_{h,c}(z)
 =z^{2h}\sum_{m\geq 0}\frac{1}{m!}
 \left(-\frac{c}{4}\right)^m z^{-2m}.
\]
Thus the coefficient of $z^{2h-2m}$ is
$(-c/4)^m/m!$, while every odd inverse-power coefficient vanishes.
This conclusion follows directly from the closed form and is valid
formally for every $h$ and $c$; it does not by itself identify the
corresponding operator on the full descendant space or its action on
conformal blocks.
The $R_{2} = -c/4$ first correction is the genus-$0$
primary-line footprint of the quartic Virasoro pole. It is one scalar
projection of the data from which the infinite primitive-cumulant series
$\Delta_{\mathrm{Vir}}(t) = t^{3} + 2t^{5} + \cdots$ of
Example~\ref{ex:virasoro-koszul-dual} descends via the shadow
obstruction tower. It is not the full completed spectral
$R$-operator.
\end{remark}
...
pro-complete regime: each finite stage~$W_N$ is principal finite
...
\begin{definition}[Strong completion tower; \ClaimStatusDefinitional]
...
\begin{proposition}[Positive-energy vector spaces and a strict
finite-stage tower; \ClaimStatusConditional]
\label{prop:standard-strong-filtration}
\index{bar complex!weight truncations for standard families|textbf}
Let
\[
 \cA=\bigoplus_{h\geq0}\cA_h,
 \qquad \dim\cA_h<\infty ,
\]
be a positive-energy chiral algebra.  The vector-space truncations
\[
 V_{\leq N}:=\bigoplus_{0\leq h\leq N}\cA_h
\]
form a surjective inverse system of finite-dimensional vector spaces,
and
\[
 \varprojlim_N V_{\leq N}
 \cong\prod_{h\geq0}\cA_h .
\]

A strict finite-stage chiral tower consists of additional data
\[
 \bigl(\cA_{\langle N\rangle},
 p_{N+1,N}\colon\cA_{\langle N+1\rangle}
 \twoheadrightarrow\cA_{\langle N\rangle},
 \theta_N\bigr)_{N\geq0},
\]
where the \(\cA_{\langle N\rangle}\) are finite-type curved chiral
\(A_\infty\)-algebras, the maps \(p_{N+1,N}\) are strict, and
\(\theta_N\) identifies their underlying low-weight vector spaces
with \(V_{\leq N}\).  Assume these identifications are compatible and
the induced bar maps
\[
 C_{N+1}:=\bar B(\cA_{\langle N+1\rangle})
 \longrightarrow
 C_N:=\bar B(\cA_{\langle N\rangle})
\]
restrict to isomorphisms
\[
 F_{\leq w}C_{N+1}\xrightarrow{\sim}F_{\leq w}C_N
 \qquad (N\geq w).
\]
Then \(\{C_N\}_N\) satisfies the low-weight stabilization hypothesis
of Proposition~\ref{prop:mc4-weight-cutoff}.

For affine Kac--Moody, Virasoro, principal \(\mathcal W\)-algebras,
and positive-definite lattice algebras, positive energy proves the
first vector-space statement.  Construction of the strict chiral
tower and its low-weight bar isomorphisms remains a separate
family-level obligation.
\end{proposition}

\begin{proof}
Finite dimensionality of \(V_{\leq N}\) follows by summing the
finitely many spaces \(\cA_h\), and coordinate truncation gives
\(\varprojlim_NV_{\leq N}\cong\prod_{h\geq0}\cA_h\).
The final conclusion is exactly the assumed low-weight isomorphism
for the induced bar tower.
\end{proof}
...
\]
for a strict tower whose finite stages
\(\cA_{\langle N\rangle}\) lie in the proved
bar-cobar regime. Assume:
\begin{enumerate}
\item each transition map \(C_{N+1}\twoheadrightarrow C_N\) and each
 algebra transition
 \(\cA_{\langle N+1\rangle}\twoheadrightarrow
 \cA_{\langle N\rangle}\) is degreewise surjective;
...
\item the finite-stage bar-cobar quasi-isomorphisms in the
 ...
 $\epsilon_N \colon \Omega(C_N) \xrightarrow{\sim}
 \cA_{\langle N\rangle}$ are
 compatible with the tower maps.
\item the continuous cobar object is the strict inverse limit
 \[
  \Omega^{\mathrm{cont}}(\widehat C)
  :=\varprojlim_N\Omega(C_N),
 \]
 its differential is continuous, and the cone tower
 \(\{\operatorname{Cone}(\epsilon_N)\}_N\) is Mittag--Leffler in every
 cohomological degree.  In the curved CDG setting, assume also
 \textup{(CP3)}.
\end{enumerate}
Then:
\begin{enumerate}
...
\item the induced map
 \[
 \widehat{\epsilon}\colon
 \Omega^{\mathrm{cont}}(\widehat{C}) \longrightarrow
 \varprojlim_N \cA_{\langle N\rangle}
 \]
is a quasi-isomorphism when the completed differential is square-zero;
in the curved CDG setting the same map has coacyclic cone in the
specified second-kind category.
\end{enumerate}
Thus MC4 for a concrete tower is the construction of the strict
transition maps, continuous cobar limit, cone-level
Mittag--Leffler passage, and the relevant square-zero or second-kind
structure.
\end{proposition}
...
\]
Assumptions~\textup{(1)}--\textup{(2)} kill the derived limit term
...
$\widehat{\epsilon}\colon
\Omega^{\mathrm{cont}}(\widehat{C})\to
\varprojlim_N\cA_{\langle N\rangle}$.
The finite-stage maps are quasi-isomorphisms in the square-zero total
model.  Apply the Milnor sequence directly to their cone tower;
Assumption~\textup{(4)} removes its derived-limit term.  Hence
$\widehat{\epsilon}$ induces an isomorphism on cohomology. The
square-zero hypothesis makes the continuous cobar differential an
ordinary differential, so
$\widehat{\epsilon}$ is a quasi-isomorphism. In the curved CDG
setting, \textup{(CP3)} carries the finite-stage coacyclic cones to a
coacyclic inverse-limit cone.
\end{proof}

% \label{rem:mc4-concrete-tests} removed (echo of Proposition~\ref{prop:mc4-reduction-principle})
...
\item every compatible system of finite-stage bar-cobar
 ...
 \varprojlim_N \cA_{\langle N\rangle},
 \]
 in the square-zero setting; the curved CDG setting gives the
 coacyclic-cone statement of the reduction principle.
\end{enumerate}
...
\varprojlim_N \cA_{\langle N\rangle},
\]
in the square-zero setting; the curved CDG setting gives a
coacyclic-cone statement.  Thus eventual surjectivity on
finite-dimensional weight slices supplies the cohomological
...
Yangian towers is now reduced to: \emph{prove coefficient stabilization
...
\begin{proof}
Primary generator agreement propagates to all descendants by translation (assumption~(3)), and strong generation (assumption~(1)) plus bar recovery (assumption~(4)) identify the full coefficient family, proving Proposition~\ref{prop:winfty-ds-residue-identity-criterion}.
\end{proof}
...
\begin{computation}[Stage-\texorpdfstring{$4$}{4} seed set and residual packet]
\label{comp:winfty-stage4-seed-check}
\index{MC4!W-infinity stage-4 check}
The finite sets in
Proposition~\ref{prop:winfty-ds-stage4-residual-packet} have the
following cardinalities and decomposition:
\begin{itemize}
...
\item Universal $T$-coupling pattern:
 $\mathsf{C}_{s,s;2;0,2s-2} = 2$ for $s = 2, 3$;
 the $s=4$ identity remains the conditional stage-$4$ target.
\end{itemize}
\end{computation}
...
\begin{proof}
The principal Drinfeld--Sokolov calculation
(Proposition~\ref{prop:w4-ds-ope-explicit} and
Computation~\ref{comp:w4-ds-ope-extraction}) gives all stage-$4$ OPE
...
pairing package leaves the transport identity as the open mathematical condition
...
point is the explicit open mathematical condition in
...
\texorpdfstring{$(3,4)$}{(3,4)} block coefficient comparison]
\label{comp:winfty-stage5-block34-coefficient}
The finite comparison in
Proposition~\ref{prop:winfty-stage5-block-34} consists of the following
three expressions:
\begin{enumerate}[label=\textup{(\roman*)}]
...
\]
\end{enumerate}
At (c=10,24,26,123,\tfrac12), respectively, the displayed rational
square is
\[
 \frac{28000}{12903},\quad
 \frac{16818480}{1521197},\quad
 \frac{110047392}{8854375},\quad
 \frac{5396816880}{110031689},\quad 0.
\]
These five specializations record a bounded exact calculation; they do
not replace the coefficient identification in the preceding proof.
\end{computation}
...
\begin{computation}[Full stage-\texorpdfstring{$4$}{4} reduction chain]
...
Proposition~\ref{prop:winfty-mc4-frontier-package} is the following
...
\item \emph{Primaryity}
 ...
 eliminating $22$ sub-leading entries; the seven surviving elements
 are listed in that corollary;
\item \emph{Parity}
...
\end{itemize}
The full chain is therefore $29 \to 7 \to 6 = 4 + 2$; each arrow is
the restriction defined by the cited corollary.
\end{computation}
...
Drinfeld--Sokolov data. That side is already explicit. The remaining
stage-$4$ obligation is the six bar-side identities
\[
\mathsf{C}^{\mathrm{res}}_{s,t;u;m,n}(4)
=
\mathsf{C}^{\mathrm{DS}}_{s,t;u;m,n}(4)
...
\begin{computation}[Finite \texorpdfstring{$W_4$}{W4} OPE specializations]
\label{comp:w4-ds-ope-extraction}
\index{MC4!W4 OPE coefficient extraction}
Substitution in the rational functions of
Proposition~\ref{prop:w4-ds-ope-explicit} gives the following bounded
consequences:
\begin{itemize}
\item all four squared structure constants are defined by direct
 substitution at the regular levels $k = 0, 1, 2, -2, -3$, using
 $c(k)=3-60(k+3)^2/(k+4)$;
\item inter-coefficient ratios
 $\mathsf{C}_{3,4;3;0,4}^2/c_{334}^2 = 9/16$ and
 $\mathsf{C}_{3,4;4;0,3}^2/c_{334}^2 = 5/7$ hold identically wherever
 $c_{334}\neq0$ and the displayed rational functions are regular;
\item Feigin--Frenkel duality $k \leftrightarrow -k-2h^\vee$
 gives $c(k) + c(-k-8) = 246$ by direct simplification;
\item unitarity non-negativity ($c_{334}^2 \geq 0$, $c_{444}^2 \geq 0$)
 holds after substitution at the $\mathcal{W}(N, N{+}1)$ minimal models
 for $4 \leq N \leq 19$;
\item both proved principal Virasoro-target values
 $\mathsf{C}^{\mathrm{DS}}_{4,4;2;0,6} = 2$ and
 $\mathsf{C}^{\mathrm{DS}}_{3,4;2;0,5} = 0$ are the remaining two
 entries of the six-channel packet.
\end{itemize}
The listed levels and minimal models are a finite specialization of the
rational identities, not an extension beyond the conditional status of
Proposition~\ref{prop:w4-ds-ope-explicit}.
\end{computation}
...
$m_1^2=[m_0^{(g)},-]$ with scalar diagonal
$\kappa\cdot\omega_g$. Ordinary
...
II.\;Curved & $d_0^{\,2} \neq 0$;\; finitely generated
 & $D_{\mathrm{tot}}^2 = 0$;\; scalar diagonal $\kappa \cdot \omega_g$
...
$\overline{\mathcal{M}}_g$ acquires geometric curvature with scalar
diagonal $\kappa(\mathcal{A}) \cdot \omega_g$;
...
\item \emph{Geometric:}\; the scalar diagonal of $m_0^{(g)}$ is $\kappa(\mathcal{A})\cdot\omega_g$ whenever $\kappa(\mathcal{A}) \neq 0$
...
(Regime~III). The fourth level (pro-complete) lies beyond the
...
\ClaimStatusConditional]\label{thm:central-implies-strict}
...
included in this conclusion: it may still carry a nonzero curvature
element whose scalar diagonal is $\kappa(\cA)\omega_g$ when
$\kappa(\cA)\neq0$.
\end{theorem}
...
\begin{definition}[Non-quadratic Verdier--Koszul algebra via curved
completion; \ClaimStatusDefinitional]
\label{def:nonquadratic-curved-verdier-koszul-dual}
\index{Koszul dual!non-quadratic curved completion|textbf}
\index{curved A-infinity@curved $A_\infty$!non-quadratic Koszul dual}
Let \(A\) be a non-quadratic chiral algebra whose completed
second-kind bar coalgebra
\(\widehat{\bar B}^{\mathrm{II}}_X(A)\) is formed in the
conformal-weight or Kazhdan filtration.  Its non-quadratic
Verdier--Koszul algebra is
\[
K_X^{\mathrm{II}}(A)
:=
\mathbb D_{\Ran}\widehat{\bar B}^{\mathrm{II}}_X(A).
\]
The operations \(\{m_r\}_{r\ge0}\) are the continuous Verdier
transposes of the length components of the bar coderivation and
coalgebra structure; in particular, \(m_0\) is dual to the curvature
functional.  The symbol \(A^!\) denotes a strict quadratic model once
the quadratic comparison, local finite duality, and a formality or
same-family strictification theorem have been supplied.
\end{definition}

\begin{proposition}[Curved completed Koszul duality and the
critical-level boundary; \ClaimStatusConditional]
\label{prop:curved-completed-koszul-critical-boundary}
\index{curved A-infinity@curved $A_\infty$!Koszul duality}
\index{critical level!curved Koszul boundary}
Let $A$ be one of the non-quadratic standard families treated in this
chapter, for instance $\mathrm{Vir}_c$ at generic central charge or a
principal $\mathcal W_N$-algebra at generic non-critical level, in the
completed conformal-weight/Kazhdan filtration of
Theorem~\ref{thm:completed-bar-cobar-strong}. Assume the finite-window
PBW condition, dualizability of every finite window, completeness and
separatedness of the filtration, centrality of the curvature, and the
Mittag--Leffler condition governing continuous Verdier duality. Then
the object
\[
K_X^{\mathrm{II}}(A)
=
\mathbb D_{\Ran}\widehat{\bar B}^{\mathrm{II}}_X(A)
\]
of Definition~\ref{def:nonquadratic-curved-verdier-koszul-dual}
is a completed curved \(A_\infty\) algebra for these families.  A
formality theorem that contracts the higher operations supplies its
strict quadratic realization. Its operations
\[
\{m_r\}_{r\ge0},\qquad m_0\in K_X^{\mathrm{II}}(A)^2,
\]
satisfy
\[
m_1(m_0)=0,\qquad
...
\]
If $m_0$ is central, the internal commutator vanishes; the raw
fiberwise higher-genus CDG operator may still have scalar diagonal
curvature $\kappa(A)\omega_g$, so ordinary cohomology is recovered
only after the MC twist or period-corrected total differential of
Theorem~\ref{thm:quantum-diff-squares-zero}.

For the Heisenberg algebra, $m_0=k\omega_{\mathrm{cen}}$ is central by
the one-dimensional central extension, and the curved dual is the
commutative chiral algebra
$(\mathrm{Sym}^{\mathrm{ch}}(V^*[1]),m_0=-k\omega_{\mathrm{cen}})$ of
Theorem~\ref{thm:heisenberg-curved-structure}. For affine
Kac--Moody at generic non-critical level,
$m_0$ is proportional to the central current $K$ (equivalently to the
modular scalar
$\kappa(\widehat{\fg}_k)=\dim(\fg)(k+h^\vee)/(2h^\vee)$ on the
higher-genus shadow), hence it is central in the PBW presentation.
At critical level \(k=-h^\vee\), the invariant form used by
Sugawara/KZ normalization degenerates and the Feigin--Frenkel centre
enters.  The resulting completed curved object is recorded in its
\(A_\infty\) type in this chapter.
\end{proposition}

\begin{proof}
The completed bar coalgebra is filtered by conformal weight or
Kazhdan degree.  Finite-window dualizability and the
Mittag--Leffler hypothesis identify its continuous Verdier dual with
the completed inverse limit of the finite-window dual algebras; in
particular, the associated $\varprojlim^1$ term vanishes.  The bar
coderivation and cooperations transpose continuously to the operations
$m_r$, including the curvature $m_0$. The $n=0$ and $n=1$ curved
$A_\infty$ identities are precisely
$m_1(m_0)=0$ and
$m_1^2(a)=m_2(m_0,a)-m_2(a,m_0)$, as in
Theorem~\ref{thm:central-implies-strict}. The Heisenberg and generic
affine centrality statements are the central-extension computations:
$\omega_{\mathrm{cen}}$ and $K$ commute with every generator.  At
$k=-h^\vee$, the Feigin--Frenkel centre supplies the completed
critical-level ambient, whose comparison with the generic PBW/KZ lane
is an additional family datum.
\end{proof}
...
At genus~$1$ the raw fiberwise operator is curved and has scalar
diagonal curvature
\[
...
Getzler--Kapranov~\textup{\cite{GetzlerKapranov98}}. Thus the
genus-$0$ restriction is cyclic, while the positive-genus object is
wheeled/modular because non-separating self-gluings are part of
\(\mathsf{F}\mathrm{Com}\). Concretely:
\begin{enumerate}[label=\textup{(\roman*)}]
...
chiral operad in the conditional factorization ambient package
$H_{\Fact}(X)$ of Proposition~\ref{prop:fg-ambient-properties}; no
published GR17 model theorem is used for this lift. In that ambient:
\begin{enumerate}[label=\textup{(\roman*)}]
\item Weak equivalences are maps whose cobar construction is a
 quasi-isomorphism in the finite-type square-zero setting,
 respectively maps detected by coderived comodules in the curved
 setting.
...
The required chiral ambient is the conditional factorization package
$H_{\Fact}(X)$ of Proposition~\ref{prop:fg-ambient-properties}, as
used in Theorem~$A^{\infty,2}$
(Theorem~\ref{thm:A-infinity-2}). Transporting the operadic bar-cobar
theorem through that package gives the displayed homotopy equivalence.
...
factorization ambient of Theorem~$A^{\infty,2}$. Under that hypothesis
package, on the
...
$\infty$-categories. The chiral version uses the conditional
factorization ambient package $H_{\Fact}(X)$ before taking this
underlying $\infty$-category; without that package this is only the
chain-operadic rectification statement. This is the target for the DK-$5$
categorical lift
...
rectification; Proposition~\ref{prop:fg-ambient-properties} and
Theorem~\ref{thm:A-infinity-2} for the conditional chiral
factorization ambient.
\end{proof}
...
ordinary derived category. The curved CDG setting represents the same
comparison by a coacyclic cone in Positselski's second-kind category
of curved comodules~\textup{\cite{GLZ22,Positselski11}}; the ordinary
chain quasi-isomorphism belongs to the square-zero setting.
\end{theorem}

Theorem~\ref{thm:central-implies-strict} provides the geometric
realization of the GLZ algebraic result.

\begin{theorem}[Associative bar--cobar on the chiral Ran space;
\ClaimStatusConditional]
\label{thm:fg-factorization-bar-cobar}
\textup{[Type signature: Open quadrant; enhanced associative
Ran/factorization presentation; Beilinson level $1\leftrightarrow2$;
pro-nilpotent Francis--Gaitsgory ambient; hypothesis packages
$H_{\Fact}(X)+H_{\mathrm{fact}}+H_{\mathrm{conv}}+
H_{\mathrm{fact}}^{R}+H_{\mathrm{VD}}$.]}
Let $X$ be a smooth curve over a characteristic-zero field and set
\[
 \mathcal C_X:=\bigl(D(\Ran X),\otimes^{\mathrm{ch}}\bigr).
\]
The associative bar--cobar comparison has three typed forms.
\begin{enumerate}[label=\textup{(\roman*)}]
\item \emph{Ambient Ran equivalence; \ClaimStatusProvedElsewhere.}
The proof of Francis--Gaitsgory's Theorem~5.1.1 makes
$\mathcal C_X$ pro-nilpotent.  Proposition~4.1.2 of
\cite{FrancisGaitsgory2012}, applied to the reduced associative
operad, therefore gives mutually inverse enhanced functors
\[
 ...
 (\mathcal C_X)
 \mathbin{:}
 \operatorname{Cobar}^{\mathrm{enh}}_{\operatorname{Bar}(\mathsf{Ass})}.
\]
After adjoining units, the algebra-side counit is an equivalence
\[
 \varepsilon_A\colon
 \operatorname{Cobar}^{\mathrm{enh}}
 \operatorname{Bar}^{\mathrm{enh}}(A)
 \xrightarrow{\ \sim\ } A .
\]

\item \emph{Associative factorization restriction;
\ClaimStatusConditional.}
Assume the model package $H_{\Fact}(X)$ of
Proposition~\ref{prop:fg-ambient-properties} and the closure package
$H_{\mathrm{fact}}$.  The enhanced functors preserve the chosen
associative factorization subcategories, and the counit becomes
\[
 \varepsilon_{\mathcal F}\colon
 \Omegach_X\barBch_X(\mathcal F)
 \xrightarrow{\ \sim\ }\mathcal F .
\]
Francis--Gaitsgory's Theorem~5.2.1 supplies the corresponding
factorization restriction for the Lie/commutative Chevalley
equivalence.  The package $H_{\mathrm{fact}}$ supplies the associative
closure used in this volume.

\item \emph{Fulton--MacPherson and Verdier refinements;
\ClaimStatusConditional.}
The package $H_{\mathrm{fact}}^{R}$ extends the ordered-to-symmetric
comparison through the Fulton--MacPherson collision strata.  On the
dualizable domain $H_{\mathrm{VD}}$, Verdier duality produces the
separate algebra
\[
 K_X(\mathcal F):=\mathbb D_{\Ran}\barBch_X(\mathcal F),
 \qquad
 \barBch_X(\mathcal F)
 \xrightarrow{\ \sim\ }
 \mathbb D_{\Ran}K_X(\mathcal F).
\]
The counit $\varepsilon_{\mathcal F}$ is the reconstruction map;
$K_X(\mathcal F)$ is the Verdier--Koszul output.
\end{enumerate}
\end{theorem}

Definition~\ref{def:geometric-bar} gives a chain representative of the
enhanced bar under $H_{\Fact}(X)+H_{\mathrm{conv}}$.  The package
$H_{\mathrm{fact}}$ identifies its disjoint-locus factorization maps,
and $H_{\mathrm{fact}}^{R}$ carries the differential through the
Fulton--MacPherson collision strata.  Thus the geometric bar
coderivation represents the enhanced associative bar on this stated
hypothesis package.
...
is the field-theoretic form of~\eqref{eq:mc-equation}.  A chosen
factorization comparison sends the effective action to the chiral
twisting class; its vacuum-scalar anomaly maps to central curvature.
The MC identity then equips the completed twisted object with its
square-zero total differential.

\subsection{Computational corollaries}

\begin{corollary}[Bar cohomology computes Ext;
 \ClaimStatusConditional]\label{cor:bar-computes-ext}
Under the hypothesis package of Theorem~A, for a chiral algebra
$\mathcal{A}$ with central curvature and a square-zero total bar differential,
...
\begin{corollary}[Quadratic coalgebra and Verdier algebra;
\ClaimStatusConditional]\label{cor:koszul-dual-cooperad}
Let $\mathcal A$ carry a connected quadratic presentation, its
canonical twisting morphism
$\tau_{\mathrm i}\colon\mathcal A^{\mathrm i}\to\mathcal A$, the
comparison package
$H_{\mathrm{CL}}(\mathcal A,\mathcal A^{\mathrm i},\tau_{\mathrm i})$,
and the Verdier package $H_{\mathrm{VD}}$.  Then the canonical map
\[
 q_\cA\colon\cA^{\mathrm i}\xrightarrow{\sim}\barB_X(\cA)
\]
is a quasi-isomorphism of coalgebras.  Verdier duality gives the
algebra map
\[
...
\]
Local finite duality identifies the target with the continuous
realization of the quadratic-dual algebra
$\cA^!=(\cA^{\mathrm i})^\vee$.
\end{corollary}
...
fiberwise CDG operator. This is the strict nilpotence input used in the
...
The bar complex of a chiral algebra carries two typed operations.
The first applies the cobar
...
$K_X(\cA):=\mathbb{D}_{\Ran}\barBch(\cA)$. This is
\emph{duality}: a chosen Koszul-pair map
$\nu_\cA\colon K_X(\cA)\to\cA^!$ reaches the partner algebra.
The distinction between reconstruction and duality is
...
$\cA=T_X(V)/(R)$, the typed objects and comparison maps are
\[
\cA,\qquad
\barBch(\cA),\qquad
\cA^i=C_X(s^{-1}V,s^{-2}R),\qquad
H^\bullet\barBch(\cA),\qquad
K_X(\cA)=\mathbb D_{\Ran}\barBch(\cA),\qquad
\cA^!,\qquad
Z^{\mathrm{der}}_{\mathrm{ch}}(\cA)
\]
with
\[
q_\cA\colon\cA^i\longrightarrow\barBch(\cA),\qquad
\mathbb D(q_\cA)\colon K_X(\cA)\longrightarrow\mathbb D_{\Ran}(\cA^i),
\qquad
\nu_\cA\colon K_X(\cA)\longrightarrow\cA^!.
\]
This type separation governs the whole bar--cobar theory.
...
factorization \emph{algebra}; its multiplication is dual to the bar
coproduct.  Under the Verdier package, write
\[
 K_X(\cA)=\mathbb D_{\Ran}\barBch_X(\cA).
\]
A finite chiral Koszul pair supplies a comparison
$\nu_{\cA,\cA^!}\colon K_X(\cA)\xrightarrow{\sim}\cA^!$.
For a quadratic presentation, the dual of
$q_\cA\colon\cA^{\mathrm i}\to\barBch_X(\cA)$ gives the algebra map
$\mathbb D(q_\cA)\colon K_X(\cA)\to(\cA^{\mathrm i})^\vee$.
The bar of~$\cA^!$ is a separate coalgebra formed from the dual
algebra:
\begin{center}
...
$\cA^{\mathrm i}=C_X(s^{-1}V,s^{-2}R)$ & coalgebra
 & quadratic presentation and $q_\cA\colon\cA^{\mathrm i}\to\barBch(\cA)$ \\
$H^\bullet(\barBch(\cA))$ & graded coalgebra
 & bar cohomology, compared by $H^\bullet(q_\cA)$ under $H_{\mathrm{CL}}$ \\
$K_X(\cA)=\mathbb D_{\Ran}\barBch(\cA)$ & algebra
 & Ran--Verdier algebra \\
$\cA^!$ & algebra
 & chosen Koszul partner reached by $\nu_\cA\colon K_X(\cA)\to\cA^!$
 \\
...
has target~$\cA$.  The duality functor
$K_X=\mathbb D_{\Ran}\circ\barBch_X$ has target the
Verdier--bar algebra.  The finite pair comparison identifies this
algebra with the chosen strict partner.  The derived centre
$Z^{\mathrm{der}}_{\mathrm{ch}}(\cA)$ is
$C^\bullet_{\mathrm{ch}}(\cA,\cA)$, the Hochschild closed-sector
algebra acting on~$\cA$.  Thus reconstruction, Verdier duality,
quadratic compression, and Hochschild formation have four explicit
targets.
\end{remark}

\begin{theorem}[Pairwise Verdier recognition for finite chiral Koszul pairs;
\ClaimStatusConditional]
...
finite-type and convergence hypotheses.  A pairwise Verdier package
consists of chiral twisting morphisms in both directions, the two
acyclic twisted tensor products, and compatible algebra maps
\[
 \nu_{\cA\cB}\colon K_X(\cA)\xrightarrow{\sim}\cB,
 \qquad
 \nu_{\cB\cA}\colon K_X(\cB)\xrightarrow{\sim}\cA,
\]
together with evaluation--coevaluation homotopies satisfying the two
zig--zag identities.  This package makes $(\cA,\cB)$ a finite chiral
Koszul pair.  The displayed maps represent the return from each
Verdier--bar algebra to its chosen partner.
\end{theorem}

\begin{proof}
The twisting-morphism fundamental theorem turns the two acyclic
twisted tensor products into the corresponding bar comparisons.
Verdier duality transports those coalgebra comparisons to the maps
$\nu_{\cA\cB}$ and $\nu_{\cB\cA}$.  The stated homotopies give the
triangle identities for the represented pair.
\end{proof}
...
$A_\infty$-deformation supplies the homotopy $h^{(3)}|_{H_3}$ of
Remark~\ref{rem:bcac-3}, whose cone is coacyclic in the
...
(Remark~\ref{rem:bcac-deep-1}) is uniform on $U$ conditional on the
nearby-cycle, deformation-class, and descent data required by Open
Lemmas~\ref{lem:cclimax-H1-failure},
\ref{lem:cclimax-H3-failure}, and~\ref{lem:cclimax-H4-failure}.
When those data identify the three exceptional supports with
$H_1,H_3,H_4$, the Mittag--Leffler condition holds uniformly on their
complement and the strict quasi-isomorphism is defined there.
Statement~(b) is the weight-completed Theorem~B of
...
$\kappa_{\mathrm{BKM}}(\Phi_N)=c_N(0)/2$ together with the separate
chiral conductor $K^{\kappa_{\mathrm{ch}}}(A_X)$ of
Equation~\eqref{eq:trace-identity-platonic}: the comparison is
...
provide the strict-open and coderived-wall model on which the Vol.~III
...
in three ambient-qualified settings on the leading three walls
...
$\widehat{\mathcal{O}}_{H_n}$ by the defining CDG identity. The
$V$-filtration is
...
Assume additionally the filtered automorphic-to-bar comparison required
by Open Theorem~\ref{thm:tbsp-lim1-is-fourier-coefficient}; thus the
Chern/residue class of the chosen automorphic object maps to the
extension class of the inverse system attached to the filtered
comparison cone.
...
Bruinier lift $\phi_{-2,1}$.  The displayed identity is the conditional
output of the comparison required by Open
...
Step 2 (identification with Fourier coefficient).  The comparison
assumed in the theorem, whose construction is the content of Open
Theorem~\ref{thm:tbsp-lim1-is-fourier-coefficient}, carries this class
to $c_{-2,1}(n,r_n)\cdot[H_n]^\vee$. Under the de Rham
...
Step 4 (three leading walls).  If the direct normalized
Jacobi-coefficient calculation gives
$c_{-2,1}(1,1)=-2$, $c_{-2,1}(3,1)=-8$, $c_{-2,1}(4,0)=2$
(Remark~\ref{rem:tbsp-leading-three-walls-first-coefficients} in
Chapter~\ref{ch:theorem-B-scope-platonic}), the conditional formula
specializes to the three displayed coefficients.  Their interpretation
as nearby-cycle, deformation, and descent classes requires the three
comparison packages formulated in Open
...
\index{global inversion theorem!curved setting}
Let
...
the generic fibre identifies both presentations
...
\begin{proof}
The open presentation is a strict chain-level
quasi-isomorphism, while each wall presentation is a completed coderived
...
the restriction of the weight-completed presentation to the generic fibre
...
chain-level presentation on $U^{\mathrm{adm}}$. On
...
\begin{remark}[Comparison routes for
Theorem~\ref{thm:bcac-global-cech-descent}]
\label{rem:bcac-cech-descent-independent-checks}
Four routes enter the comparison: (1) the leading-wall nearby-cycle,
deformation, and descent constructions formulated in Open
Lemmas~\ref{lem:cclimax-H1-failure}--\ref{lem:cclimax-H4-failure}
...
Chapter~\ref{ch:theorem-B-scope-platonic}) clause~(iv), which is
this theorem stated in chiral bar ambient (here stated in curved
CDG-coalgebra ambient); (3) Chern-class identification with
$\kappa_{\mathrm{BKM}}(\Phi_N)=c_N(0)/2$ and the chiral conductor
on the K3-fibered Class-A locus (Vol.~III trace identity);
(4) limiting-case
