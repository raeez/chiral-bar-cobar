...
\item \emph{Completed Koszul duality.}
 ...
 completion towers whose finite quotients lie in the theorematic
...
\end{enumerate}
The construction is Theorem~\ref{thm:winfty-factorization-kd}: the factorization
...
Proposition~\ref{prop:winfty-mc4-frontier-package} is closed by
...
the base stage of the rigidity induction.
...
\emph{Stage-$5$ specialization.} Setting $N=4$ in the inductive step,
...
This modifies $d_0$ but the period-corrected genus-$1$ differential
still satisfies $d_1^2 = 0$ strictly because $k$ is central and the
geometric correction is included.

\emph{Explicit check at genus $1$.}
\begin{align}
...
on the open Koszul locus of the Humbert stratification of
$\overline{\mathcal{A}_2}$: there the curvature cancels exactly and
the curved adjunction is strict. On each Humbert wall a distinct
failure mode obstructs strict chain-level inversion; the repaired
