...
the pair $K = \barBch$, $K^{-1} = \Omegach$ is a symmetric-monoidal
adjoint equivalence with $K^2 \simeq \mathrm{id}$ on the Koszul
locus \textup{(}clause~\ref{KR-ii}\textup{)}, and a coderived
involution $K^{2}\simeq\mathrm{id}$ in
$D^{\co}(\barBch\text{-}\mathrm{CoFact})$ off it
\textup{(}clause~\ref{KR-iii}\textup{)}. The strict adjunction of
this chapter is the Koszul reflection's $K\dashv K^{-1}$ unit; the
inversion clause that the second half of the chapter proves is its
$K^{2}\simeq\mathrm{id}$ involutivity restricted to chain-level qi.
...
\end{enumerate}
In particular, $\mu_1$ fails to be a differential precisely when $\mu_0$ is \emph{not} central with respect to $\mu_2$. For every standard chiral algebra (Heisenberg, Kac--Moody, Virasoro, W-algebras, lattice), the genus-$g$ curvature is $\mu_0 = \kappa(\cA)\cdot\omega_g\cdot\mathbf{1}$: a scalar multiple of the vacuum vector, which is central in the binary bar operation $\mu_2$, so $[\mu_0, -]_{\mu_2} = 0$ automatically. The curvature therefore enters not through $\mu_2$ but through higher operations ($\mu_3, \mu_4, \ldots$) and through the fiberwise Hodge bundle structure on $\overline{\cM}_{g,n}$.
\end{theorem}
...
Here $d^2 = 0$ holds \emph{strictly} at each genus (not up
to $\hbar$-corrections), so each $Z_g$ is an honest finite-dimensional vector
...
conformal field theories whose anomalies are central, we have $d_{\text{bar}}^2 = 0$ strictly at all genera. The ingredients are: (1) the curvature $\mu_0$ is proportional to the vacuum $\mathbf{1}$, hence central by the vacuum axiom (Remark~\ref{rem:voa-central-curvature}); (2) on the affine genus-$0$ screen, Arnold relations ensure $d_{\mathrm{residue}}^2 = 0$, while on $\mathbb{P}^1$ and for $g\geq1$ this residue-square computation also uses the projective and period relations of Proposition~\ref{prop:arnold-higher-genus}; (3) the Leibniz rule gives the anticommutation of $d_{\mathrm{internal}}$ and $d_{\mathrm{residue}}$; (4) the modular operad boundary relations on $\overline{\mathcal{M}}_{g,n}$ extend this to all genera (Theorem~\ref{thm:genus-induction-strict}).
...
$E_2$-chiral output of $\Phi_2$, with shift $[2]$ and $\widetilde{M}_{24}$ projective Schur
cocycle of order $6$. The curvature term comes from the Mukai pairing's signature
...
At $K3 \times E$ ($d = 3$) the curved bar-cobar output is the K3 chiral
Hall--Drinfeld double
$\mathcal{D}_\hbar(\mathcal{Y}^{\mathrm{Hall}}_\hbar(\mathrm{CoHA}_{K3 \times E}))$
(Vol~III Wave~13) with Schiffmann--Vasserot shuffle presentation attached to the Manin
...
Classification invariant $H^2(\mathfrak{g}_{\Delta_5})^{\mathbb{Z}/2, K(1)} \cong \mathbb{C} \cdot \Delta_5$:
the Igusa cusp form IS the gauge-class invariant.
\end{remark}
...
The curved bar-cobar adjunction on the Wave~13 K3 chiral bialgebra
is controlled by the curvature element
\[
w^{\mathrm{Wave13}} \;=\; \hbar^2\cdot K^{\kappa_{\mathrm{ch}}}\cdot \mathbf{1}_{\mathbf H_{\Delta_5}} \;=\; -\mathbf{1}_{\mathbf H_{\Delta_5}}\quad\text{at $\hbar^2 = -1/8$},
\]
so that the curved bar $B^w(\mathbf H_{\Delta_5})$ admits a strict
quasi-isomorphism
$\Omega^w(B^w(\mathbf H_{\Delta_5}))\simeq\mathbf H_{\Delta_5}$
as curved coalgebras-algebras. Off the Lusztig locus the curvature
is non-trivial and the adjunction is genuinely curved, requiring
pro-object / weight-completed framework. Cross-ref AP142 and Vol.~I
Pattern 269.
\end{remark}

\begin{remark}[Curved bar in the Hall subcategory]
\label{rem:bcac-deep-2}
The curved bar $B^w(\mathbf H_{\Delta_5})$ is constructed in the Hall
subcategory of $\mathrm{CoHA}_{K3\times E}$ with differential
\[
d_{B^w} \;=\; d_{\mathrm{Hall}} + d_{\mathrm{Sieg}} + \hbar\, d_{\mathrm{assoc}} + \hbar^2\, d_{\Phi_{10}/\eta^{24}} + w^{\mathrm{Wave13}}\cdot\mathrm{Id},
\]
where the last piece is the curvature contribution. The closedness
$(d_{B^w})^2 = 0$ \emph{requires} $\hbar^2\cdot K^{\kappa_{\mathrm{ch}}} = -1$,
reproducing the universal identity as the curvature-cancellation
condition. Primary-lit: Positselski 2011 (curved bar),
...
The curvature element $w^{\mathrm{Wave13}} = -1$ at Lusztig reflects
...
This chapter has established curved bar-cobar at the
CDG-coalgebra level (Theorems \ref{thm:chiral-positselski-weight-completed}
and relatives), with explicit curvature $w^{\mathrm{Wave13}}$ for
the K3 chiral bialgebra $\mathbf{H}_{\Delta_5}$. The Wave~18
addition is the chain-level statement of curved bar-cobar inversion
across the Humbert stratification of $\overline{\mathcal{A}_2}$:
on the Koszul locus the curvature cancels exactly and the curved
adjunction is strict; on each Humbert wall a distinct failure
mode obstructs strict chain-level inversion, with the
$A_\infty$-deformation providing the minimal repair at $H_3$.
...
$(B^w(\mathbf{H}_{\Delta_5}), d_{B^w}, w^{\mathrm{Wave13}})$
...
quasi-isomorphism, not merely a coderived equivalence. This is the
chain-level upgrade of
Remark~\ref{rem:bcac-deep-1} from its Wave-13 ambient
status.
\end{remark}
...
$w^{\mathrm{Wave13}}$ picks up an additional Bruinier-Chern
...
$w^{(k)} = \zeta^k\cdot w^{\mathrm{Wave13}}$, so the curved bar
differential acquires the $\mathbb{Z}/8$-twist
$d_{B^w}|_{H_1} = \bigoplus_k (d + \zeta^k\mathrm{Id}\cdot
w^{\mathrm{Wave13}})$. Each eigenspace piece is bounded; the
...
weight-completed analogue survives.
\end{remark}
...
\]
the Wave-17 corrected value (replacing the naive $176256$ with the
literature-anchored $-8$; \cite{Costello2013,CostelloGaiotto2018,
...
\[
 ...
 \;=\; d_{\mathrm{Hall}} + d_{\mathrm{Sieg}}
       ...
       + w^{\mathrm{Wave13}}\cdot\mathrm{Id},
\]
...
bar-cobar pair is strict on the $A_\infty$ level but requires the
$h^{(3)}$ homotopy as extra data.
\end{remark}
...
$\mathcal{G}_{H_4}$ only at the pro-object level; the
...
\begin{theorem}[W18 Beilinson: curved bar-cobar trichotomy on
$\overline{\mathcal{A}_2}$]
\label{thm:bcac-trichotomy}
\index{curved bar-cobar adjunction!W18 trichotomy}
\index{bar-cobar inversion!global chain-level}
The curved bar-cobar adjunction $(\Omega^w\dashv B^w)$ on the K3
chiral bialgebra $\mathbf{H}_{\Delta_5}$ satisfies the three
...
$w^{\mathrm{Wave13}} = -1$ cancelling against the identity piece
...
Remark~\ref{rem:bcac-3} to repair inversion at the
$A_\infty$ level
...
This theorem is the curved-bar-cobar translation of
Theorem~\ref{thm:climax-W18-global-inversion}; the two differ
only in ambient (curved CDG-coalgebra here, chiral bar complex in
the climax chapter). Statement~(a): the curvature cancellation
$w^{\mathrm{Wave13}} + \mathrm{Id} = 0$ at $\hbar^2 = -1/8$,
...
\begin{remark}[W18 relation to the Vol.~III
$\kappa_{\mathrm{BKM}} = c_N(0)/2$ identity]
\label{rem:bcac-5}
\index{universal trace identity!W18 chain-level footing}
The W18 trichotomy
(Theorem~\ref{thm:bcac-trichotomy}) provides the
chain-level footing for the universal trace identity
...
complex $B^w(A_X)$ satisfies the W18 trichotomy, and the
...
faithfully. The chapter's Wave-13--14 DNA (establishing the
curvature cancellation at the Lusztig point) and Wave-18
(establishing the Humbert stratification) therefore together
provide the chain-level scaffold on which the Vol.~III
identification $\kappa_{\mathrm{BKM}} = c_N(0)/2$ of
Chapter~\ref{ch:k3-chiral-bialgebra-platonic} rests.
\end{remark}
...
$\{H_1,H_3,H_4\}$. The purpose of this section is to make the
chain-level mechanism uniform across \emph{every} admissible
Humbert divisor
$H_n\subset\overline{\mathcal{A}_2}$ with $n\equiv 0,3\pmod 4$, and
to write down the explicit extension/obstruction formula governing
the curved adjunction on the formal neighbourhood of $H_n$.
...
Let $(B^w(A),d_{B^w},w^{\mathrm{Wave13}})$ denote the curved bar
...
\[
 ...
 \mathrm{gr}^V_\bullet\, w^{\mathrm{Wave13}}\big|_{H_n}
 \Bigr)
\]
...
$\mathrm{gr}^V_0 w^{\mathrm{Wave13}}|_{H_n}$ takes the value
\[
 \mathrm{gr}^V_0 w^{\mathrm{Wave13}}\big|_{H_n}
 \;=\;
...
\[
 ...
 \;+\;
 [\mathrm{gr}^V_\bullet w^{\mathrm{Wave13}}|_{H_n},\,-]
 \;=\;
 0
\]
...
\begin{proof}
The curved bar coalgebra $(B^w(A),d_{B^w},w^{\mathrm{Wave13}})$
satisfies $d_{B^w}^2+[w^{\mathrm{Wave13}},-]=0$ on
$\widehat{\mathcal{O}}_{H_n}$
(Remark~\ref{rem:bcac-deep-1}). The $V$-filtration is
preserved by both $d_{B^w}$ and the action of $w^{\mathrm{Wave13}}$
(by
...
$(B^w(A),d_{B^w},w^{\mathrm{Wave13}})|_{\widehat{U}_{H_n}}$ denote
...
\[
 ...
 (-8)\cdot c_{\Phi_{10}/\eta^{24}}|_{H_3},
\]
where the coefficient $-8$ is now \emph{explicitly} identified as
the $(3,1)$-Fourier coefficient of $\phi_{-2,1}$. This upgrades
the Wave-17 Costello correction
\cite{Costello2013,CostelloGaiotto2018} from a literature-anchored
numerical value to a chain-level Fourier-coefficient reading:
...
curved bar-cobar quasi-isomorphism
\[
 \Omega^w(B^w(A))
 \;\simeq\;
 A
...
with curvature cancellation $w^{\mathrm{Wave13}}+\mathrm{Id}=0$
...
\begin{proof}
Lane assembly: each lane is a strict quasi-isomorphism in its
ambient by
Theorem~\ref{thm:bcac-trichotomy} (for $U^{\mathrm{adm}}$
and $H_3$) and
Theorem~\ref{thm:bcac-extension-obstruction-across-Hn} (for
general admissible $H_n$). \v Cech cocycle condition on double
...
\begin{remark}[Independent verification of
Theorem~\ref{thm:bcac-global-cech-descent}]
\label{rem:bcac-cech-descent-independent-verification}
Four verification paths: (1) direct computation at each leading
