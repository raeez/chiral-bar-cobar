...
Chapters~\ref{ch:algebraic-foundations} and~\ref{chap:cobar-construction}
built the bar functor~$\barBch$ and the cobar functor~$\Omegach$
separately; they are the two projections of a single Maurer--Cartan
element: the universal twisting morphism
$\tau_\cA \in \MC\bigl(\operatorname{Conv}(\barBch(\cA), \cA)\bigr)$.
Theorem~\ref{thm:bar-cobar-adjunction} (``Theorem~A'') establishes
the representability
$\operatorname{Tw}(\cC, \cA) = \MC(\operatorname{Conv}(\cC, \cA))$,
giving the adjunction $\Omegach \dashv \barBch$ with Verdier
intertwining; Theorem~\ref{thm:bar-cobar-inversion-qi} (``Theorem~B'')
is the statement that~$\tau_\cA$ is \emph{acyclic}: the twisted tensor
...
$\mathbb{D}_{\Ran}\, \barBch(\cA) \simeq \cA^!_\infty$ (underlying complex $\simeq \barBch(\cA^!)$).
...
of $d^2=0$ is not a pathology; it is the algebraic trace of the
genus-$1$ boundary $\delta_0 \in \partial\overline{\mathcal{M}}_{1,1}$,
the nodal degeneration that introduces curvature into every genus
tower.
...
\textup{(}$h \neq 0$\textup{)}.}
The ordinary derived category is trivial
(Proposition~\ref{prop:curved-bar-acyclicity}), but the
coderived--contraderived equivalence
(Theorem~\ref{thm:positselski-chiral-proved}) is nontrivial.
The Positselski equivalence
$D^{\mathrm{co}}(\cA\text{-}\mathrm{CoMod}) \simeq
D^{\mathrm{ctr}}(\cA^!\text{-}\mathrm{ContraMod})$
...
duality between contramodules and complete modules, and compact
generation for the triangulated comparison functor.
...
$\operatorname{wt}(a)$.  The key observation: the bar
...
\begin{theorem}[Modular operad structure of the bar complex;
  \ClaimStatusProvedHere]\label{thm:bar-modular-operad}%
\index{bar complex!modular operad structure}%
...
Theorem~\ref{thm:bar-modular-operad} makes the modular operad structure,
...
identification
$\mathbb{D}_{\Ran}\barBch_X(\cA) \simeq (\cA^\sharp)_\infty$ (underlying complex $\simeq \barBch_X(\cA^\sharp)$).
