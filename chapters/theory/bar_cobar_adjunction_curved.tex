...
This is where the algebraic engine closes into a single object.
...
separately; here they are revealed as the two projections of a single
Maurer--Cartan element: the universal twisting morphism
...
Together, the two movements close the algebraic engine: once the
adjunction and its inversion are in hand, the entire genus-$0$
bar complex is a functor, and the passage to higher genus
(Chapter~\ref{chap:higher-genus}) amounts to deforming the
differential by curvature.  In the three-ring architecture, Theorem~A
is the capstone of Ring~1: it synthesizes
Chapters~\ref{ch:algebraic-foundations}--\ref{chap:cobar-construction}
into a single adjunction, and the curved extension reaches into
Ring~2 by providing the formal framework in which the curvature
$\kappa(\cA) \cdot \omega_g$ and the shadow Postnikov tower
$\Theta_\cA^{\leq r}$ operate.
...
\item \emph{Completed Koszul duality.}
  ...
  regime.  In summary: the completed twisting morphism
...
identification
$\mathbb{D}_{\Ran}\barBch_X(\cA) \simeq \barBch_X(\cA^\sharp)$.
...
Chapter~\ref{chap:koszul-pair} is about~$K_X$.
...
is an instance of the Chriss--Ginzburg principle: the coupled
obstruction is a Maurer--Cartan element in the mixed-color
convolution algebra, and its finite-order structure (the Casimir
transgression) is a projection of that MC element to the
quadratic stratum.
\end{remark}
