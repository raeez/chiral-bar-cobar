...
\emph{Theorem~A}: the bar-cobar adjunction $\Omegach \dashv \barBch$
on $\Ran(X)$ with Verdier intertwining.  The four-regime hierarchy
(quadratic, curved-central, filtered-complete, programmatic) and the
$I$-adic completion machinery.  The chapter culminates in two
structural results: the strong completion-tower theorem
(Theorem~\ref{thm:completed-bar-cobar-strong}), which resolves MC4
by identifying the single axiom (the strong filtration condition
$\mu_r(F^{i_1},\dots,F^{i_r}) \subset F^{i_1+\cdots+i_r}$) that
makes completed bar-cobar a quasi-isomorphism; and the
reconstruction-vs-duality theorem
(Theorem~\ref{thm:reconstruction-vs-duality}), which separates
the two natural functors $R_X \neq K_X$ on the bar complex and
thereby codifies the four-object distinction
($\cA$, $\barBch(\cA)$, $\cA^i$, $\cA^!$)
on which the entire monograph depends.
\end{remark}

The MC element $\Theta_\cA \in \MC(\gAmod)$ is constructed from the
bar coalgebra $\barBch(\cA)$ and the cobar functor $\Omegach$; this
chapter establishes the adjunction that makes the construction
possible.  Theorem~A (Theorem~\ref{thm:bar-cobar-adjunction})
proves that the space of twisting morphisms
$\operatorname{Tw}(\cC, \cA) = \MC(\operatorname{Conv}(\cC, \cA))$
is representable by the bar-cobar adjunction
$\Omegach \dashv \barBch$, with Verdier intertwining.  The
universal twisting morphism
$\tau_\cA \in \MC\bigl(\operatorname{Conv}(\barBch(\cA), \cA)\bigr)$
is the genus-$0$ seed of the full MC element: the arity-$2$
projection of $\Theta_\cA$ is the modular characteristic
$\kappa(\cA)$, and the twisting morphism $\tau_\cA$ is the
genus-$0$ binary component.  Theorem~B
(Theorem~\ref{thm:bar-cobar-inversion-qi}) is the statement
...
The homotopy upgrade is essential from the start.  Not every chiral
...
larger algebra.  The passage from strict to homotopy convolution is
the reason bar-cobar preserves quasi-isomorphisms: the functor is a
...
The chapter proceeds in two movements.  The first (this file)
...
Postnikov tower $\Theta_\cA^{\leq r}$ operate.
...
the curvature element encoding the central extension.  This failure
of $d^2=0$ is the algebraic trace of the
genus-$1$ boundary $\delta_0 \in \partial\overline{\mathcal{M}}_{1,1}$:
the nodal degeneration introduces curvature into every genus
tower.
...
\emph{finite} sum (Lemma~\ref{lem:arity-cutoff}).  Virasoro admits a
completed same-family shadow used throughout Parts~II--III;
...
\end{equation}
since $c(k) + c(k') = 2\operatorname{rank}(\mathfrak{sl}_2) + 4h^\vee\dim\mathfrak{sl}_2 = 2 + 24 = 26$ by Theorem~\ref{thm:central-charge-complementarity}(b) (the $\mathcal{W}$-algebra central-charge sum; this is the Virasoro-family formula, not the affine KM formula $c + c' = 2\dim\mathfrak{g}$).  Physically, this is matter--ghost duality: a matter system at central charge~$c$ couples to ghosts at $c_{\mathrm{ghost}} = 26 - c$.  In particular, $\mathrm{Vir}_{26}^! \cong \mathrm{Vir}_0$, consistent with bosonic string BRST cohomology at ghost number zero.

This is the M/S-level dynamic shadow.  Kinematically, the completed dual has infinitely many primitive cumulants ($\Delta_{\mathrm{Vir}}(t) = t^3 + 2t^5 + \cdots$; Definition~\ref{def:primitive-defect-series}).  The MC4 structural framework is proved (Theorem~\ref{thm:completed-bar-cobar-strong}); the residual H-level target identification is the subject of Example~\ref{ex:winfty-completion-frontier}.
\end{example}
...
(Lemma~\ref{lem:arity-cutoff}), so the passage to the completion
is automatic.
...
towers.  When the strong-filtration axiom holds, the arity cutoff
\textup{(}Lemma~\textup{\ref{lem:arity-cutoff})} makes each
...
By Lemma~\ref{lem:arity-cutoff}, modulo $F^{N+1}$ only arities
...
weight quotient only finitely many arities survive and all matrices are
...
was Conjecture~\ref{conj:winfty-stage4-visible-diagonal-normalization},
...
Same argument as
Conjecture~\ref{conj:winfty-stage4-ward-inheritance}: the hypotheses
...
\item the packet-level stage-$5$ conditions
  \textup{(}\ref{conj:winfty-stage5-higher-spin-identities},
  \ref{conj:winfty-stage5-entry-identities},
  \ref{conj:winfty-stage5-transport-identities},
  \ref{conj:winfty-stage5-transport-target-3},
  \ref{conj:winfty-stage5-transport-target-4},
  \ref{conj:winfty-stage5-block-34}--\ref{conj:winfty-stage5-block-45}\textup{)}
...
\item the target-$5$ ladder conditions
  \textup{(}\ref{conj:winfty-stage5-transport-target-5},
  \ref{conj:winfty-stage5-transport-target5-35},
  \ref{conj:winfty-stage5-transport-target5-45},
  \ref{conj:winfty-stage5-block-55}\textup{)} become automatic.
\end{enumerate}
...
\item the mixed-entry block
  \textup{(}\ref{conj:winfty-stage5-block-34}\textup{)} carries defect
  \[
  \mathsf{C}^{\mathrm{res}}_{3,4;5;0,2}(5)
  -
  \mathsf{C}^{\mathrm{DS}}_{3,4;5;0,2}(5)
...
\item the representative target-$4$ class consists of the singleton
  comparison \textup{(}\ref{conj:winfty-stage5-one-coefficient-comparison}\textup{)},
  the target-$4$ ladder \textup{(}\ref{conj:winfty-stage5-transport-target-4}\textup{)}, and the
  \texorpdfstring{$(3,5)$}{(3,5)} block
  \textup{(}\ref{conj:winfty-stage5-block-35}\textup{)}, with representative
  defect exactly \(D_5\);
\item the target-$3$ class consists of the target-$3$ ladder
  \textup{(}\ref{conj:winfty-stage5-transport-target-3}\textup{)} and the
  \texorpdfstring{$(4,5)$}{(4,5)} block
  \textup{(}\ref{conj:winfty-stage5-block-45}\textup{)}, with representative
  defect
  \[
  \mathsf{C}^{\mathrm{res}}_{4,5;3;0,6}(5)
  -
...
\item the target-$5$ ladder
  ...
  \ref{conj:winfty-stage5-transport-target5-35},
  \ref{conj:winfty-stage5-transport-target5-45},
  \ref{conj:winfty-stage5-block-55}\textup{)}
...
moreover that at each stage~$N$ the stage-$N$ quotient of the
...
\]
In particular, the stage-$4$ six-defect block, the stage-$5$ conjecture
network
\textup{(}Conjectures~\textup{\ref{conj:winfty-stage5-principal-target5-no-new-independent-data}}%
--\textup{\ref{conj:winfty-stage5-principal-one-coefficient-normal-form}},
\textup{\ref{conj:winfty-stage5-one-coefficient-comparison}}\textup{)},
...
In particular, Conjectures~\ref{conj:winfty-stage4-visible-borcherds-transport},
\ref{conj:winfty-stage4-ward-inheritance},
and~\ref{conj:winfty-stage4-visible-diagonal-normalization} are
resolved.
...
Conjectures~\ref{conj:winfty-stage5-principal-target5-no-new-independent-data},
\ref{conj:winfty-stage5-principal-target5-transport-vanishing},
\ref{conj:winfty-stage5-principal-tail-cross-target-reduction},
\ref{conj:winfty-stage5-principal-residual-front-one-coefficient},
\ref{conj:winfty-stage5-principal-one-coefficient-normal-form},
and~\ref{conj:winfty-stage5-one-coefficient-comparison}, together with
all downstream block, ladder, and packet conjectures in
...
Here $d_{dR}^2 = 0$ \emph{strictly} because on a function $f$:
\[d_{dR}^2 f
= d_{dR}\Bigl(\sum_i \frac{\partial f}{\partial x^i}\, dx^i\Bigr)
= \sum_{i,j} \frac{\partial^2 f}{\partial x^j \partial x^i}\, dx^j \wedge dx^i
= 0,\]
since the mixed partials are symmetric in $i,j$ while $dx^j \wedge dx^i$ is antisymmetric.  The Leibniz rule extends this to all forms.
...
However, the \emph{curvature} $\mu_0 \propto \mathbf{1}$ is still central, so:
\[d_{\text{bar}}^2 = 0 \quad \text{strictly}\]
...
\emph{Curvature.}
The curvature element is proportional to central elements:
$\mu_0 \propto \mathbf{1}$ (with possible higher-filtration
central corrections from the non-linear OPE).
All contributions lie in the center $Z(\mathcal{W}_3)$, so:
\[d_{\text{bar}}^2 = 0 \quad \text{strictly}\]
...
the modular operad axioms for $\{\overline{\mathcal{M}}_{g,n}\}$.  We now
crystallize this dependence into an explicit algebraic statement: the bar
...
Francis--Gaitsgory~\cite{FG12} prove that factorization algebras satisfy
bar-cobar inversion (the cobar of the bar recovers the original algebra).
Their result:
...
Combined with our explicit bar construction (Definition~\ref{def:geometric-bar}), this identifies our geometric bar differential with the standard one.
...
The second part of this section develops the coupled bar complex.
...
Postnikov tower, every modular characteristic, and every
