...
\item \emph{Quadratic} ($\mu_0 = 0$): strict $d^2 = 0$,
  no completion needed (Heisenberg, free fields);
\item \emph{Curved-central} ($\mu_0 \neq 0$, $[\mu_0, -] =
  \kappa \cdot \mathrm{id}$): completed bar-cobar, curvature
  $\kappa$ is scalar (affine algebras at generic level);
\item \emph{Filtered-complete} ($\mu_0$ non-central):
  $I$-adic completion required, work in coderived category
  (Virasoro, $\Walg_N$ at finite~$N$);
...
\end{enumerate}
\emph{This resolves Conjecture~6.7.20, the MC4 completion problem.
For $W_\infty$ the finite stages are $W_N$; for Yangian towers
they are RTT stages.  The single strong-filtration axiom ensures
that the MC equation
$\partial(\widehat\tau) + \widehat\tau \star \widehat\tau = 0$
converges in the completed convolution algebra: the arity cutoff
\textup{(}Lemma~\textup{\ref{lem:arity-cutoff})} makes each
component a finite sum.}
\end{theorem}
...
Theorem~\ref{thm:completed-bar-cobar-strong}.  Assume:
\begin{enumerate}
...
If no MC element exists, the curvature is an honest obstruction to a bar-side
differential in the classical sense.
\end{remark}

\subsection{Obstruction hierarchy}
\label{sec:curved-summary}

The three regimes are distinguished by the structure of the curvature element
and the resulting bar-cobar technology.

\begin{center}
\renewcommand{\arraystretch}{1.3}
\begin{tabular}{llll}
\textbf{Regime} & \textbf{Curvature} & \textbf{Bar differential} & \textbf{Example} \\
\hline
I.\;Quadratic & $\mu_0 = 0$ & $d_{\mathrm{bar}}^2 = 0$ on the nose & $\mathcal{H}_k$ \\
II.\;Curved-central & $\mu_0 \in Z(A) \setminus \{0\}$
  & $m_1^2 = [\mu_0, -] = 0$;\; $d_{\mathrm{fib}}^2 = \kappa \cdot \omega_g$
  & $\mathrm{Vir}_c$ \\
III.\;Filtered & $\mu_0 \in Z(A)$, non-quadratic OPE
...
\noindent
Regime~I admits the classical bar-cobar adjunction
(Theorem~\ref{thm:bar-cobar-adjunction}).  Regime~II preserves
$d_{\mathrm{bar}}^2 = 0$ on the total bar complex but acquires
geometric curvature $\kappa(\mathcal{A}) \cdot \omega_g$ on fibers
over $\overline{\mathcal{M}}_g$; the corrected differential
$D^{(g)}$ of Theorem~\ref{thm:quantum-diff-squares-zero} absorbs
this.  Regime~III requires the completed bar complex
$\widehat{\overline{B}}(A)$ and the strong filtration axiom
