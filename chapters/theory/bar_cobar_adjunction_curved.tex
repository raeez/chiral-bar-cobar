...
\begin{proposition}[Coacyclicity of curved bar complexes in the
...
second-kind totalizations. At genus $g \geq 1$, the curved bar object
$\bar{B}^{(g)}(\mathcal{A})$ is then
\emph{coacyclic} in Positselski's coderived sense: its totalization
lies in the thick subcategory generated by curved short-exact
totalizations and closed under the direct sums admitted by the
weight-completed ambient. No claim is made here that the underlying
ordinary cochain complex is contractible after forgetting curvature
and filtration.
\end{proposition}
...
The standard Positselski argument~\cite[Remark~3.6]{Positselski11}
is a second-kind statement: for a curved CDG-coalgebra
$(C,d,h)$ with nonzero curvature, the relevant bar totalization is
coacyclic in the coderived localization once the curved
CDG-coalgebra and its exact second-kind ambient are fixed. Under the
hypothesis of the proposition
$C=\bar{B}^{(g)}(\mathcal{A})$ and $h=m_0^{(g)}\neq 0$. Thus the
bar object becomes zero in the coacyclic quotient while the
coderived comodule category over it remains the correct homotopy
theory. This is precisely the reason ordinary cohomological
invariants are not the right higher-genus invariant.
\end{proof}
...
weight-completed/pro object is required. The historical label of this
example is retained for cross-references, but the corrected obstruction
is finite-type completion, not deconcatenation nonconilpotency of
$L_0$.
\end{example}
...
coacyclicity hypotheses have been verified. Then:
\begin{enumerate}
...
\item \emph{Dual acyclicity in the square-zero total model.}
 ...
 its cone is coacyclic in the verified second-kind category.
...
Proposition~\ref{prop:curved-bar-acyclicity} are verified.
...
The curved bar-cobar adjunction on the K3 chiral bialgebra
...
pro-object / weight-completed framework.
\end{remark}
...
and relatives), with explicit curvature $w^{\mathrm{chir}}$ for
the K3 chiral bialgebra $\mathbf{H}_{\Delta_5}$. The chain-level
...
\]
the Costello-corrected value (replacing the naive $176256$ with the
