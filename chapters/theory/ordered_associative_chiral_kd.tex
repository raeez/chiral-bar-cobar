...
the two structural obstructions, one from the OPE pole in $T(u) T(u)$
and one from the Hopf axiom $m \circ (S \otimes \id) \circ \Delta = \eta \varepsilon$
incompatible with locality, are established in this chapter. We
therefore reserve the name ``chiral bialgebra equivalence'' for this
statement; the phrase ``chiral quantum group'' continues to denote
the underlying structural package, with the understanding that no
Hopf antipode exists at the vertex-algebraic level.
\end{remark}
...
\item \emph{Argument~$B$ \textup{(}JKL vertex bialgebra
  ...
  trivial parent: a non-trivial construction is required on the
...
\]
The classical (non-chiral) statement is Molev's antisymmetriser
theorem~\textup{\cite{Molev07}}, Theorem~$1.11.2$: the $N$-fold
antisymmetriser $A_N = \frac{1}{N!}\sum_{\sigma\in S_N}
...
$T_i(v)$ by construction of the antisymmetriser.
