...
plus unitarity $R(z)\,R^{op}(-z)=\id$; automatic for rational
Yangians and generic-$q$ trigonometric $U_q(\widehat{\fg})$, vacuous
when $R(z)=\tau$) is the symmetrisation map
$\Barord(A) \to \BarSig(A)$ that relates the ordered bar to the
symmetric bar of Theorem~A.
\end{remark}
...
\begin{remark}[DNA: the universal curvature invariant
...
On the B-family of the CY-$2$ input \textup{(}K3 chiral Hall--Drinfeld
double, Volume~III\textup{)}, the modulus takes the value
$K^{\kappa_{\mathrm{ch}}} = 8$,
and this common value admits three genuinely independent verification
paths through Bruinier reciprocity for the Igusa form on the
...
\begin{remark}[DNA: Hall--Drinfeld double as fourth taxonomic slot beyond
Yangian]
\label{rem:oackd-1}
...
\[
D_\hbar\bigl(Y_\hbar^{\mathrm{Hall}}(\mathrm{CoHA}_{K3\times E})\bigr),
\]
built from the Schiffmann--Vasserot $2012$ CoHA shuffle presentation and
its Davison $2017$ CY-$3$ extension. The Cartan lives inside a Manin
...
triple, so the Drinfeld double construction admits no quasi-Hopf
lift and no RTT presentation on a real root ladder. The chiral
...
slot; the Kac--Moody frontier of Remark~\ref{rem:km-frontier} widens,
beyond root-multiplicity breakdown, into the BKM regime where the
Cartan signature itself obstructs the classical double.
\end{remark}
...
Section~\ref{sec:r-matrix-descent-vol1} on the K3 chiral Hall--Drinfeld
double $\cA = \mathbf{H}_{\Delta_5}$. The chain-level ordered Koszul
dual $\cA^!_{\mathrm{line}}$ produced by Verdier duality
...
\begin{remark}[DNA: abelian at Lie, non-abelian at vertex]
\label{rem:oackd-2}
...
\begin{remark}[DNA: CY-$2$ $\lbrack 2\rbrack$-shift and the
...
of the Hall-side chiral algebra acquires a cohomological shift that is
...
\begin{remark}[DNA: Igusa form as the gauge invariant of the
chiral Hall--Drinfeld double]
\label{rem:oackd-4}
...
classification. For the chiral Hall--Drinfeld double of
...
one-dimensional, with canonical generator the weight-$10$
Igusa cusp form $\Delta_5$ (Igusa's Siegel genus-two cusp form of
weight~$10$). The classification statement
\[
H^2\bigl(\fg_{\Delta_5}\bigr)^{\ZZ/2,\,K(1)}
\;\cong\;
\CC\cdot\Delta_5
...
of the chiral Hall--Drinfeld double. Concretely, $\Delta_5$ plays the
...
\item \textup{($R$-matrix.)}
 ...
 classical limit is
 % r-matrix with level prefix Psi; Psi=0 -> r=0. Verified.
...
Verified computationally at spins $2$--$6$ in the
...
\begin{remark}[Scope of the primary cross-term]
...
established in this volume; the pattern is a frontier item.
\end{remark}
...
% level prefix Psi mandatory; Psi=0 -> R = uI_4 (trivial). Verified.
...
\item \textup{(Yang's rational $R$-matrix.)}
 ...
 % level prefix Psi mandatory; Psi=0 -> R = uI (trivial). Verified.
 R(u)
 = u\, I_{N^2} + \Psi\, P
...
\]
% level prefix Psi; Psi=0 -> R = u I (trivial). Verified.
...
\item \textbf{Shift pattern is rigid.} The reversed-shift variant
 ...
 \texttt{Fraction} verification). The same is true of the plain
...
supertrace of the CoHA-side Hall complex on $Y$ are intertwined by
this stage-$2$ shadow:
\[
...
$1994$\textup{)}, and is established at standalone scope only
...
Identity \eqref{eq:bernard-zeta-identity} itself is verified
...
\eqref{eq:kzb} does NOT satisfy the ordinary classical Yang--Baxter
...
is established as a theorem body in Vol~I; the scope here is
