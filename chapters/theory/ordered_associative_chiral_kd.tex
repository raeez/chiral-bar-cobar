...
\noindent
The symmetric bar complex $\BarchSig(A)$ retains none of this.
...
dependence entirely: across every family in the chiral landscape
(affine KM, Heisenberg, $\beta\gamma$, Virasoro, $\mathcal{W}_N$, and
the BKM/K3 slot above), the normalised curvature modulus is universal,
\[
\hbar^{2}\,K^{\kappa_{\mathrm{ch}}}(A)
\;=\;
-1.
\]
This is a cross-family landscape invariant: it says that the bar and
cobar channels of the chain-level chiral Koszul duality are in exact
reciprocal balance once $\hbar$ is installed as the universal unit of
curvature. The identity is sharper than the family-by-family
complementarity values of Theorem~C (Volume~I): those values
...
of a super-Jacobi collapse (computed on $\widehat{\mathfrak{sl}_2}$
...
\begin{remark}[Tests]
Two computations suffice to promote the conjecture from heuristic to
...
companion object to the K3 Yangian under the Vol~III CY-to-chiral
...
(Chapter reference in Volume~III), is not a Yangian at all, but the
compact Hall--Drinfeld double
\[
D_\hbar\!\left(Y_{\hbar,\mathrm{crit}}^{\mathrm{Hall},+}(K3\times E)\right),
\]
formed from the realised positive critical CoHA only after the compact
passage datum has supplied completion, monoidal vanishing cycles,
compact support, the Hall pairing, and the negative half. The finite
Rees hCS--Hall comparison gives bounded Rees Hall classes and comparison
maps; it is not this compact CoHA and not the double. The Cartan lives
inside a Manin
...
$U_{q_1,q_2}(\widehat{\widehat{\mathfrak{gl}}}_1)$. None is correct
on the nose. The Koszul-duality structure of the ordered bar complex
selects a fourth object, and the selection is forced by the same
gauge-invariant calculation that identifies the Igusa form
$\Delta_5$.
...
Run the bar--cobar construction of
Section~\ref{sec:r-matrix-descent-vol1} on the compact Hall--Drinfeld
double $\cA = \mathbf{H}_{\Delta_5}$, when the compact passage datum of
Remark~\ref{rem:oackd-1} has been supplied. The finite Rees
hCS--Hall maps alone give only a bounded positive-half target and do
not construct $\cA$. The chain-level ordered Koszul dual
$\cA^!_{\mathrm{line}}$ produced by Verdier duality
\textup{(}Theorem~\textup{\ref{thm:opposite}}\textup{)} is, on the
...
the unique value at which the ordered bar--cobar roundtrip closes
...
Taken together, $\mathbf{Q}^{\mathrm{K3}}_{\hbar}$ at
$\hbar^{2} = -1/8$ with Arthur parameter $\psi_{\Delta_{10}}$
is the chiral quantum group undergirding the K3 BKM. The
identification is chain-level and primary-source-backed.
\end{remark}
...
\cite[\S6]{Lorgat26I} for the type~$A$ construction.
\end{remark}
...
Theorem~\texttt{thm:complete-strictification}), yielding a
...
$1994$\textup{)}, and is established only in the standalone reference
