...
complementarity values of Theorem~D (Volume~I): those values
$\kappa+\kappa^{!}\in\{0,13,250/3,98/3\}$
record the family-dependent
additive offset of $\kappa$, while $\hbar^{2}\,K^{\kappa_{\mathrm{ch}}}$
...
companion object to the K3 Yangian of the CY-to-chiral functor
...
\begin{remark}[DNA: the universal $\Phi$-trace identity]
\label{rem:oackd-6}
...
\[
\Phi \;\colon\;
D^b_{\mathrm{CY}\text{-}2}(Y\times E)
\longrightarrow
\mathrm{ChirAlg}_E
\]
be the CY-to-chiral functor of Part~\ref{part:v1-frontier} (established
in Volume~III). Then the chiral supertrace of the identity on the
