...
Its finite-type continuous linear dual, after passing through the
quadratic presentation coalgebra
$A^i=C_X(s^{-1}V,s^{-2}R)$ and its ordered comparison
$q_A^{\mathrm{ord}}\colon A^i\to\Barchord(A)$, is the line-side
Koszul-dual algebra $A^!_{\mathrm{line}}=(A^i)^\vee$ on the
finite-type ordered Koszul locus.  There
$H^\bullet(q_A^{\mathrm{ord}})$ identifies the presentation-coalgebra
cohomology with $H^\bullet\Barchord(A)$.  Verdier duality enters after
symmetric/Ran descent.
For affine Kac--Moody inputs this algebra is the dg-shifted Yangian.
...
on the ordered Koszul locus through the chiral quantum-group equivalence
\textup{(}cf.\ Theorem~\ref{thm:chiral-qg-equiv}, with concrete
completed $\cW_{1+\infty}[\Psi]$ and $\cW_N[\Psi]$ theorem surfaces
inscribed below\textup{)}; the ordered associative comparison does \emph{not}
appeal to notions \textup{(D)}, \textup{(E)}.
\end{remark}

\begin{remark}[Presentation comparison firewall for ordered $\Eone$ inputs]
\label{rem:ordered-presentation-comparison-firewall}
\ClaimStatusConditional{}
Definitions~\ref{def:einf-chiral}, \ref{def:pinf-chiral}, and
\ref{def:e1-chiral} fix the hierarchy
\[
\Einf\text{-chiral}\subset \Pinf\text{-chiral}\subset
\Eone\text{-chiral}
\]
inside the BD chiral tensor category, and
Remark~\ref{rem:En-terminology} separates this usage from
topological little-disks $E_1$.  All comparison among the five
presentations of Warning~\ref{warn:multiple-e1-chiral} is imported
only through Theorem~\ref{thm:e1-chiral-notions-collapse}: strict
$\chirAss$ rectification, Etingof--Kazhdan reconstruction, ordered
BD realization, and preservation of twisting morphisms are its
hypotheses \textup{(H1)--(H4)}.  Hence no argument in this chapter
transports a bar, Hochschild complex, twisting morphism, or Koszul
dual across presentations \textup{(A)}, \textup{(B)}, \textup{(C)},
or \textup{(E)} unless that theorem, or an equivalent comparison
package, is explicitly invoked.  Presentation \textup{(D)}, the
double-$A_\infty$ enhancement, remains outside the collapse theorem
by Conjecture~\ref{conj:double-ainfty-notion-D-relation}.
\end{remark}
...
regime (Hypothesis~\ref{hyp:inputs}) is the chain-level
specialisation of the Koszul Reflection Theorem
(Theorem~\ref{thm:koszul-reflection} in
Chapter~\ref{chap:theorem-A-infinity-2}). The algebra-side
comparison is the counit
\[
\varepsilon_A\colon \Cobar\Barch(A)\xrightarrow{\sim} A,
\]
while the coalgebra-side unit has direction
$C\to \Barch\Cobar(C)$. The reverse algebra direction, when present,
is an additional Koszul or completed inversion comparison, never a
unit of $\Cobar\dashv\Barch$. The properad-level theorem upgrades
this counit quasi-isomorphism to an $(\infty,2)$-adjoint equivalence
on the conilpotent-complete locus; specialising back to the strongly
admissible regime via the Francis--Gaitsgory ambient category
recovers Hypothesis~\ref{hyp:inputs}(I).
The $R$-twisted descent
(Proposition~\ref{lem:R-twisted-descent}) relates the ordered
bar to the symmetric bar of Theorem~A only after the braid
monodromy has been promoted to a strong-unitary
$\Sigma_n$-equivariant descent datum. Without this datum one has a
braid-monodromic local system on unordered configurations, not a
symmetric bar complex.
\end{remark}
...
The passage $\Barchord\to \BarchSig$ is the strong-unitary
$R$-twisted descent. In the pole-free symmetric case this is the
ordinary Reynolds/coinvariant descent; in the genuinely braided case it
is not available until the required $\Sigma_n$-equivariant structure is
proved. The associated graded of $\BarchSig$ recovers $\BarchFG$.

Let $C = \Barchord(A)$ denote the ordered bar coalgebra.
...
\]
The ordered bar complex is the resolving $\Eone$-chiral coalgebra.
On the Koszul locus the comparison
$q_A^{\mathrm{ord}}\colon A^i\to\Barchord(A)$ is a
quasi-isomorphism, so $H^\bullet(q_A^{\mathrm{ord}})$ gives the
cohomological identification.  Finite-type duality gives the
Koszul-dual algebra $A^!=(A^i)^\vee$.
...
In this chapter, unadorned $\Barch(A)$ in the associative lemmas means
the ordered bar coalgebra $\Barchord(A)$; the symmetric bar is always
written $\BarchSig(A)$ and the Francis--Gaitsgory bar is explicitly
marked by~$\mathrm{FG}$.
...
\item Whenever linear duality is invoked, it is the continuous graded/conformal dual and each
graded/conformal piece of the object under consideration is finite-dimensional.
\end{enumerate}
\end{definition}

\begin{remark}
These hypotheses ensure that every totalization, completion, normalization, continuous dual, and
passage to coderived categories is legitimate. The arguments below require nothing more.
\end{remark}

\begin{hypothesis}[Input theorems]
\label{hyp:inputs}
Assume the following statements with their displayed hypotheses.
...
quasi-isomorphism
\[
\Cobar \Barchord(A)\xrightarrow{\sim} A,
\]
...
$\Barchord(A)$-comodules, with the usual coderived/contraderived refinement in the curved or completed
regime.
...
Let $A$ be strongly admissible and set $C:=\Barchord(A)$. The bar--cobar equivalence produces a
...
d\tau+\tau\star\tau=0.
\]
Equivalently, in the commutator dg Lie algebra of the convolution
algebra, the same condition is
\[
D\tau+\tfrac{1}{2}[\tau,\tau]=0,
\]
because $\tau$ has odd cohomological degree after the bar
desuspension and $[\tau,\tau]=2\,\tau\star\tau$.
...
\]
\[
\rho=\tau_{C,M}\circ(\eps\otimes \id_{C^{\cop}}\otimes \id_M)\rho^e
:M\to M\otimes C.
\]
...
\rho^e
=
(\id_C\otimes \tau_{M,C})\circ(\lambda\otimes \id_C)\circ \rho
:
M\to C\otimes C^{\cop}\otimes M,
\]
where $\tau_{M,C}$ swaps the middle two factors. The bicomodule axioms are precisely the
...
\begin{definition}[Ordered Hochschild complex for genuine $\Eone$-chiral inputs;
\ClaimStatusDefinitional]
\label{def:ordered-hochschild-e1-input}
\index{Hochschild complex!ordered $\mathbb{E}_1$-chiral}
Let $A$ be a strongly admissible associative chiral algebra in the
presentation fixed by Remark~\ref{rem:ord-assoc-notion-fixed}, and
let $C=\Barchord(A)$.  The \emph{ordered Hochschild cochain complex}
of $A$ is
\[
\HH^{\Eone,\mathrm{ord}}_{\mathrm{ch}}(A,A)
:=
\RHom_{A^e}^{\mathrm{ord}}(A,A),
\]
where the superscript ``ord'' means that the derived endomorphism is
computed using the ordered simplicial bar resolution
$B^\Delta_\bullet(A)$, with no $\Sigma_n$-coinvariants and no
symmetric/Ran descent.  Under the ordered bar--cobar hypotheses this
is equivalently the coalgebraic complex
\[
\HH^{\Eone,\mathrm{ord}}_{\mathrm{ch}}(A,A)
\simeq
\Coext_{C^e}^\bullet(C_\Delta,C_\Delta).
\]
This is the Hochschild object used for genuinely $\Eone$ inputs such
as ordered Yangian and Etingof--Kazhdan quantum-vertex presentations.
\end{definition}

\begin{warning}[Deligne--Tamarkin and symmetric-bar firewall]
\label{warn:deligne-tamarkin-ordered-firewall}
\index{Deligne--Tamarkin theorem!ordered $\mathbb{E}_1$ firewall}
\ClaimStatusConditional{}
The symmetric/Ran form of the chiral Deligne--Tamarkin machinery is
available for $\Einf$-chiral inputs carrying the required
$\Sigma_\bullet$-equivariant factorization descent; see
Remark~\ref{rem:center-scope-e-inf}.  For a genuinely ordered
$\Eone$ input one must use
Definition~\ref{def:ordered-hochschild-e1-input}.  The symmetric bar
$\BarchSig(A)$, and hence any chiral Deligne--Tamarkin comparison
that consumes it, is not available for a Yangian or EK quantum vertex
input until the $R$-twisted descent datum of
Proposition~\ref{lem:R-twisted-descent} has been proved.
\end{warning}
...
In desuspended notation we write a bar word of length $n$ as
\[
[a_1|\cdots|a_n].
\]
...
carries strictly more information than the symmetric bar $\BarchSig(A)$.
...
for the $\Sigma_n$-covering when the strong-unitary equivariant
structure exists. The symmetric bar complex is then recovered as the
...
In the pole-free symmetric case, the equivariant structure is
canonical: $R_\sigma=\varepsilon(\sigma)\cdot\tau_\sigma$, where
$\tau_\sigma$ permutes tensor factors and $\varepsilon(\sigma)$ is
the desuspended Koszul sign. The descent recovers the ordinary
$\Sigma_n$-coinvariant bar complex.

When the singular OPE has non-trivial monodromy, the equivariant
structure is an additional theorem, not a formal consequence of
having an ordered bar complex. The construction is the $R$-matrix.
\end{construction}
...
collision $z_i=z_j$, the collision kernel
$K_{A,ij}^{\mathrm{coll}}(z_i-z_j)
\in\operatorname{End}(\bar A\otimes\bar A)\otimes
\mathcal{O}(*\Delta)$. By the bar kernel convention (the $d\log$
kernel absorbs one power), $K_A^{\mathrm{coll}}(z)$ has pole
orders one less than the OPE.
For a fixed input algebra we write the ordered two-point kernel as
\begin{equation}\label{eq:ordered-classical-r-kernel-A}
r_A(z)
:=
\operatorname{Res}^{\mathrm{coll}}_{z_1-z_2=z}
\bigl(d_{\Barchord(A)}\bigr)_{(s^{-1}\bar A)^{\otimes 2}}
\in
\operatorname{End}(\bar A\otimes\bar A)\otimes k((z)).
\end{equation}
This operator-valued Laurent kernel is the primary ordered datum.
Any scalar invariant such as $\kappa(A)$ is obtained only later by
averaging, tracing, or passing to the modular shadow; it is not the
source from which $r_A(z)$ is reconstructed.
...
For instance, when
$r^{\mathrm{coll}}(z)=k\Omega_{\cH}/z$ with
$\Omega_{\cH}=J\otimes J$ on the Heisenberg current line, its
rank-one coefficient gives $k\,dz/z$. For affine Kac--Moody the collision residue
in trace-form normalization is
$r^{\mathrm{coll}}(z)=k\Omega_{\mathrm{tr}}/z$; the
KZ/Yangian form $\Omega/((k+h^\vee)z)$ is the rescaled
Casimir-and-coupling normalization, not the trace-form collision
residue.
...
\end{equation}
where $z_{ij}=z_i-z_j$.
For the spectral connection this is a full holomorphic-configuration
condition: in the current examples it follows from the Arnold relation
for the logarithmic forms together with the infinitesimal braid
relations of the OPE residues. It is not a consequence of the
consecutive Stasheff relation alone. Thus flatness of~$\nabla$ is part
of the $R$-matrix descent datum, proved in the examples below by the
Arnold/Kohno calculation.
...
path-ordering.
We write $R_A(z)$ when the input algebra matters.  A quantization of
the ordered classical kernel is a kernel in the completed spectral
Laurent algebra
\[
R_A(z)\in
\operatorname{End}(\bar A\otimes\bar A)((z^{-1}))[[\hbar]]
\]
with semiclassical expansion
\[
R_A(z)=\id+\hbar r_A(z)+O(\hbar^2),
\]
satisfying the spectral quantum Yang--Baxter equation
\begin{equation}\label{eq:qybe-ordered-rkernel-A}
R_{A,12}(z_{12})\,R_{A,13}(z_{13})\,R_{A,23}(z_{23})
=
R_{A,23}(z_{23})\,R_{A,13}(z_{13})\,R_{A,12}(z_{12}).
\end{equation}
In a quasi-Hopf or EK presentation the corresponding modified
Yang--Baxter equation is the associator-twisted braided hexagon; in
the strict RTT normalization it is exactly
\eqref{eq:qybe-ordered-rkernel-A}.  The passage
$r_A\rightsquigarrow R_A$ is therefore a quantization datum
\textup{(}DK/EK, KZ monodromy, or an RTT/FRT kernel\textup{)}, not a
formal consequence of the CYBE alone.
...
\]
The half-braid transports used for adjacent deck transformations give a
$B_n$-representation precisely when the corresponding braid relations
hold. For KZ-type connections this is the Kohno--Drinfeld theorem; its
infinitesimal shadow is the CYBE~\eqref{eq:cybe-vol1}.
\end{construction}
...
Assume that the monodromy operators satisfy the Yang--Baxter
relations and the strong-unitarity relation
$R_{12}(z)R_{21}(-z)=\id$, compatibly for all adjacent
transpositions. Then the unordered symmetric bar complex is the
$R$-twisted $\Sigma_n$-descent:
...
If the Yang--Baxter relation holds without strong unitarity, the same
formula gives a $B_n$-monodromic local system on
$\mathrm{Conf}_n(\mathbb C)$; it does not give a symmetric
$\Sigma_n$-quotient and must not be identified with $\BarchSig(A)_n$.
\end{proposition}
...
of the braid group, provided the braid relation holds, as proved below.
...
this is exactly the strong-unitarity hypothesis
$R_{12}(z)\,R_{21}(-z)=\id$. It is automatic in the pole-free
symmetric case and holds for the normalised rational $R$-matrices used
below. It is not a consequence of the Yang--Baxter equation alone:
without it the monodromy remains braid-group valued and no
$\Sigma_n$-descent has been constructed.
...
\bigr\}.
\]
We claim this equals $\BarchSig(A)_n$. By construction, the symmetric
bar complex $\BarchSig(A)_n$ is the complex of sections of the descended
...
\]
The left side is $\BarchSig(A)_n$ and the right side is
...
\BarchSig(A)_n
\;\simeq\;
\bigl(\Barchord(A)_n\bigr)_{\Sigma_n}.
\]
For chiral algebras with nontrivial singular OPE
\textup{(}$\widehat{\mathfrak{g}}_k$, $\mathrm{Vir}_c$,
$\mathcal{H}_k$, lattice algebras, $W$-algebras\textup{)}, the
$R$-matrix $R(z)\neq\id$ carries spectral dependence derived from the
local OPE. The descent to a symmetric bar is then genuinely
$R$-twisted and requires the strong-unitary equivariant datum of
Proposition~\ref{lem:R-twisted-descent}.
\end{corollary}
...
Since
\(\rho_R(\sigma_i)=\tau_{i,i+1}\circ R_{i,i+1}(z_i-z_{i+1})\),
the $R$-twisted $\Sigma_n$-action reduces to the plain permutation
$\tau_{i,i+1}$, and the
$R$-twisted invariants identify with the ordinary
$\Sigma_n$-coinvariants by the Reynolds idempotent, since
\(\operatorname{char} k=0\).
\end{proof}

\begin{proposition}[Symmetric descent and ordered surplus]
\ClaimStatusProvedHere
\label{prop:symmetric-descent}
\index{ordered bar!symmetric descent}
\index{averaging map!ordered to symmetric}
Let $A$ be a strongly admissible associative chiral algebra.
\begin{enumerate}[label=\textup{(\roman*)}]
\item If $A$ is pole-free and $E_\infty$-chiral, then
$R(z)=\id$ and the Reynolds map identifies the ordered bar
coinvariants with the Beilinson--Drinfeld symmetric bar:
\[
 \Barchord(A)_{\Sigma}\;\xrightarrow{\;\sim\;}\;\BarchSig(A).
\]
In characteristic zero the invariant and coinvariant descriptions are
equivalent by the ordinary Reynolds idempotent.

\item If $A$ has a nontrivial collision residue but the
strong-unitary $R$-twisted descent datum of
Proposition~\textup{\ref{lem:R-twisted-descent}} is fixed, then
\[
 \BarchSig(A)_n
 \;\simeq\;
 \bigl(\Barchord(A)_n\bigr)^{R\text{-}\Sigma_n}.
\]
The map $\av_R\colon\Barchord(A)\to\BarchSig(A)$ is the
$R$-twisted Reynolds projection. Its kernel is the non-invariant
ordered sector; it is not a quasi-isomorphism of the raw ordered bar
complex unless that kernel is acyclic.

\item If $A$ is genuinely $E_1$-chiral and no strong-unitary
$\Sigma_n$-descent datum is part of the structure, averaging does not
construct a symmetric bar complex. The available object is the
braid-monodromic ordered local system on
$\mathrm{Conf}^{\mathrm{ord}}_n(X)$.
\end{enumerate}
\end{proposition}

\begin{proof}
Clause~\textup{(i)} is Corollary~\ref{cor:pole-free-descent}.
Clause~\textup{(ii)} is precisely the descent equivalence of
Proposition~\ref{lem:R-twisted-descent}, with the projection written
as the Reynolds idempotent for the twisted action. The kernel is the
direct complement of the invariant summand over characteristic zero,
so no quasi-isomorphism statement follows without an acyclicity
hypothesis. Clause~\textup{(iii)} is the contrapositive of the
strong-unitarity part of Proposition~\ref{lem:R-twisted-descent}:
Yang--Baxter monodromy gives a braid-group action, while symmetric
descent requires the Coxeter relation
$R_{12}(z)R_{21}(-z)=\id$.
\end{proof}
...
The symmetric chiral bar complex $\BarchSig(A)$, the ordered bar
...
filtrations: a pole-order filtration on $\BarchSig(A)$
whose associated graded recovers $\barB^{\mathrm{FG}}(A)$,
and the strong-unitary $R$-twisted descent
$\Barchord(A) \to \BarchSig(A)$ of
...
defines a Maurer--Cartan element in the degree-$2$ component of the
$E_1$ ordered convolution algebra
(cf.\ the $E_1$ shadow obstruction tower in the body of the text).
Writing
\[
A_\nabla=\sum_{i<j}r_{ij}(z_i-z_j)\,dz_{ij}
\]
on $\mathrm{Conf}_n^{\mathrm{ord}}(\mathbb C)$, the flatness of
$\nabla=d-A_\nabla$ is the Maurer--Cartan equation
\[
d_{\mathrm{dR}}A_\nabla-\tfrac{1}{2}[A_\nabla,A_\nabla]=0,
\]
equivalently $d_{\mathrm{dR}}\omega+\tfrac{1}{2}[\omega,\omega]=0$
for $\omega=-A_\nabla$. For logarithmic KZ residues, the
$d_{\mathrm{dR}}$ term vanishes and the bracket term is exactly the
Arnold relation for $d\log(z_i-z_j)$ together with the infinitesimal
braid relations; in the spectral notation this is the
CYBE~\eqref{eq:cybe-vol1}.
The YBE on $R(z)$ is the monodromy, hence exponentiated, form of this
flatness equation. The $R$-matrix is the descent datum because the
...
The averaging map
$\mathrm{av}\colon\Barchord(A)\to\BarchSig(A)$
after a strong-unitary descent datum has been fixed
is the $R$-twisted Reynolds projection from ordered chains to the
descent invariants. Its kernel is the non-invariant ordered sector.
For KZ-type examples this kernel contains the associator-dependent
period data of ordered configuration-space integrals; the map is not,
by itself, a theorem that every transcendental period in every
coefficient system vanishes.
...
\noindent
The symmetric bar complex $\BarchSig(A)$ retains the
$R$-invariant part of this local system. In the standard families
used in this chapter, the numerical shadows extracted after descent
-- $\kappa$, the complementarity scalar, and the finite shadow
obstructions -- are the rational trace invariants recorded elsewhere
in the manuscript. The ordered kernel
$\ker(\mathrm{av})$ is the surplus sector: it carries the
non-invariant KZ monodromy and associator classes before taking the
symmetric shadow.
...
Thus the ordered bar complex is the natural carrier of the
configuration-space period torsor, while the symmetric bar complex is
the descended invariant shadow. The precise motivic claim proved here
is this invariant/kernel statement; identifying the full graded
motivic content of $\ker(\mathrm{av})$ beyond the KZ mixed-Tate
examples is a separate problem.
\end{remark}
...
\begin{proof}
On desuspended bar words define
\[
...
\begin{remark}[Symmetric Verdier algebra and the ordered opposite]
\label{rem:ordered-verdier-nonexistence-opposite}
\index{Verdier duality!ordered bar|textbf}
Verdier duality for the symmetric Ran bar forms the algebra
\[
 K_X(\cA):=\mathbb D_{\Ran}\barBch_X(\cA).
\]
A Koszul-pair datum supplies the separate algebra map
\(\nu_\cA\colon K_X(\cA)\to\cA^!\).  The ordered bar lives on
$\mathrm{Conf}^<_n(X)$ and carries the
...
which is the ordered noncommutative comparison available in the
present ambient.  A full ribbon-$\Ran$ enhancement would extend
Verdier duality to ordered configurations; its construction is an
open problem.
\end{remark}
...
\]
Here $\Delta(c)=\sum c_{(1)}\otimes c_{(2)}$ and
$\Delta(d)=\sum d_{(1)}\otimes d_{(2)}$. These are the
desuspended two-sided twisting signs: the left term moves the
twisting cochain past the first coalgebra leg, and the right term
moves it past both the left coalgebra leg and the middle bimodule
factor.
...
\begin{remark}[Ordered boundary symmetry]
The theorem gives the algebraic ordered open package. A symmetric
one-brane open TCFT requires additional structure symmetrising ordered
fusion: either a boundary symmetry datum or a geometric realisation by
bordered configuration spaces.
\end{remark}
...
\begin{construction}[Ordered Fulton--MacPherson partial compactification]
\label{constr:ordered-fm-compact}
The ordered FM object used by the ordered bar differential is the
associahedral partial Fulton--MacPherson compactification
$\overline{\FM}_k^{\mathrm{ord}}(\mathbb C)$, defined as the
iterated real oriented blowup of
$\mathrm{Conf}_k^{\mathrm{ord}}(\mathbb C)$ along the
\emph{consecutive-block} diagonals
\[
D_I=\{z_i=z_j\;:\; i,j\in I\},\qquad
I=\{a,a+1,\dots,b\},\quad |I|\ge2.
\]
It is a partial compactification, not the full labelled
Fulton--MacPherson compactification. In the full compactification one
blows up every diagonal $D_S=\{z_i=z_j:i,j\in S\}$ for every subset
$S\subseteq\{1,\dots,k\}$ with $|S|\ge2$; the ordered bar keeps only
the consecutive-block faces because only adjacent words may be
contracted by the associative bar differential.
...
collides) and $D_{2,3}$ (last pair collides). The full-FM divisor
$D_{\{1,3\}}$ (first and third collide while the middle point remains
separated) is not a boundary face of the ordered partial model. The
codimension-two ordered corner is the total consecutive collision
$D_{\{1,2,3\}}$, reached by letting the two adjacent faces meet. This
is the associahedron~$K_3$: a line segment with two endpoints.
\end{construction}
...
Explicitly, let $|a|$ denote cohomological degree and use the
desuspended binary operation
\[
b_2^{(n)}(s^{-1}a\otimes s^{-1}b)
\;:=\;
(-1)^{|a|}\,s^{-1}(a_{(n)}b).
\]
The coderivation extension gives the component $d_{i,i+1}^{(n)}$
on a bar word
$[s^{-1}e_{I_1}|\cdots|s^{-1}e_{I_k}]$ by contracting
the $i$-th and $(i{+}1)$-st slots via all OPE products:
\[
d_{i,i+1}^{(n)}
[s^{-1}e_{I_1}|\cdots|s^{-1}e_{I_k}]
\;=\;
(-1)^{\sum_{q<i}(|e_{I_q}|-1)+|e_{I_i}|}
\sum_K c^K_{I_i I_{i+1};n}\;
...
collision divisor.
Thus $d_{i,i+1}=\sum_n d_{i,i+1}^{(n)}$. The internal
differential is the coderivation extending
$b_1(s^{-1}a)=-s^{-1}(d_Aa)$, with the same
prefix sign $\sum_{q<i}(|e_{I_q}|-1)$.
...
codimension-two strata receive the two adjacent parenthesisations of
the same consecutive block, and the algebraic content of their
cancellation is the desuspended Stasheff identity for the OPE operations
$m_k^{\mathrm{ch}}$. In the strict associative case this is ordinary
associativity of $m_2$; with higher OPE modes retained it is the
consecutive-block projection of the Borcherds identities.
\end{construction}
...
In the pole-free symmetric case the equivariant structure is
canonical: $R_\sigma=\epsilon(\sigma)\cdot\tau_\sigma$, where
$\tau_\sigma$ permutes tensor factors and $\epsilon(\sigma)$ is the
desuspended Koszul sign. The descent then recovers the ordinary
$\Sigma_k$-coinvariant bar complex (Vol.~I, Theorem~A).

For a singular or genuinely ordered $E_1$-chiral algebra, the
equivariant structure must be constructed from the OPE data. This
construction is the $R$-matrix. The Yang--Baxter equation supplies the
braid-group action; the further strong-unitarity condition is what
allows the action to factor through $\Sigma_k$ and hence to produce the
symmetric bar.
\end{construction}
...
\begin{itemize}
\item An $E_\infty$-chiral algebra has operations that are
 $\Sigma_n$-equivariant:
...
\bigskip
The first test case is the Heisenberg chiral algebra at level~$k$.
...
\begin{theorem}[The Heisenberg ordered bar complex; \ClaimStatusProvedHere]
...
r^{\mathrm{coll}}_{\cH}(z)
\;=\;
\frac{k\,\Omega_{\cH}}{z},
\qquad
\Omega_{\cH}:=J\otimes J,
\end{equation}
a tensor-valued \emph{simple} pole obtained from the OPE double pole
$k/(z_1-z_2)^2$ by one step of $d\log$ absorption ($d\log$ lowers every OPE pole
order by one). The scalar $k/z$ is only the $J,J$ matrix coefficient of
\eqref{eq:heisenberg-collision-residue}.

The unit-charge coefficient of the flat connection on the trivial bundle over
$\Conf_2^{<}(\bC)$ determined by the bar propagator is
\begin{equation}\label{eq:heisenberg-connection}
\nabla_{\mathrm{unit}} \;=\; d \;-\; (r^{\mathrm{coll}}_{\cH}(z))_{J,J}\,dz
\;=\; d \;-\; (k/z)\,dz,
\end{equation}
and its monodromy around the origin is
\begin{equation}\label{eq:heisenberg-monodromy}
\operatorname{Mon}_0(\nabla_{\mathrm{unit}}) \;=\; e^{-2\pi ik}
\end{equation}
...
R(z) \;=\; e^{k\hbar/z}
\qquad (\text{nontrivial}).
\end{equation}
Since $\cH_k$ is $\Einf$-chiral, this $R$-matrix is
\emph{derived} from the local OPE via the monodromy of
$\nabla_{\mathrm{unit}}$, not independent input. The Yang--Baxter
equation is satisfied by the tensor residue $\Omega_{\cH}$ because the
Heisenberg current Lie algebra is abelian; after evaluating on Fock
charges the resulting $R(z)$ is scalar.
\end{theorem}
...
kernel $k\Omega_{\cH}(z-w)^{-1}dz$ on the ordered configuration space.
Writing $z=z_1-z_2$ for the relative coordinate,
$r^{\mathrm{coll}}_{\cH}(z)=k\Omega_{\cH}/z$ as claimed.

The unit-charge coefficient connection
$\nabla_{\mathrm{unit}} = d - (k/z)\,dz$ on the trivial
...
is the tensor $k\Omega_{\cH}/z$, whose rank-one coefficient is derived
from the OPE double pole ($d\log$ absorption); attempting to read the
R-matrix off the raw OPE
...
with product inherited from the continuous graded linear dual of the
...
\begin{proof}
Since the bar differential vanishes on $\Barch(\cH_k)$ and each
spectral graded piece is finite-dimensional, the Koszul dual is the
continuous graded linear dual of the bar coalgebra:
\[
\cH_k^{!,\mathrm{ord}}
\;=\;
...
and the collision residue $r^{\mathrm{coll}}_{\cH}=k\Omega_{\cH}/z$
has a simple pole, which falls in class~G.
\end{proof}

\begin{remark}[Contrast with the chiral \textup{(}symmetric\textup{)}
Koszul dual]
\label{rem:heisenberg-chiral-vs-ordered}
The chiral Koszul dual of $\cH_k$ (Vol~I) is the curved
second-kind commutative chiral algebra
\[
\cH_k^{!,\mathrm{ch}}
\;=\;
(\operatorname{Sym}^{\mathrm{ch}}(V^*[1]),m_0=-k\omega),
\]
a free commutative associated-graded algebra on one shifted
generator with curvature.
...
\item \emph{$\beta\gamma$ system
 ...
 and the pole-valued collision residue drops: the $d\log$ kernel
 absorbs the simple OPE pole. The regular contact operator
 $\Theta$ satisfies $\Theta^2=0$, and the bar differential remains
 trivial ($d_{\Barch} = 0$) because the zeroth mode product
...
\item \emph{Affine $\widehat{\mathfrak{g}}_k$
 ...
 The rank-one tensor residue $k\Omega_{\cH}/z$ becomes the
 trace-form Casimir kernel
 $k\Omega_{\mathrm{tr}}/z \in \mathfrak{g}\otimes\mathfrak{g}(z^{-1})$.
 The scalar $R$-matrix $e^{k\hbar/z}$ becomes the Yang $R$-matrix
 $1+\Omega/((k+h^\vee)z)+O((k+h^\vee)^{-2})$ after KZ rescaling. The abelian Yangian
...
\item \emph{Virasoro $(\S\ref{subsec:virasoro-ordered-bar})$:}
 ...
 + \cdots$. The simple current residue $k\Omega_{\cH}/z$ becomes the
...
depth~$4$, formal, regular nilpotent contact). The affine is class~L (shadow
...
\item\label{drinfeld-sl2-D1} $[H_r, H_s] = 0$ for all $r,s \ge 0$.

\item\label{drinfeld-sl2-D2}
$[H(u), E(v)] \;=\; \dfrac{2\hbar}{u-v}\bigl(E(u) - E(v)\bigr)$.

\item\label{drinfeld-sl2-D3} $[H(u), F(v)] \;=\; -\,\dfrac{2\hbar}{u-v}\bigl(F(u) - F(v)\bigr)$.

\item\label{drinfeld-sl2-D4} $[E(u), F(v)] \;=\; \dfrac{1}{u-v}\bigl(H(u) - H(v)\bigr)$.

\item\label{drinfeld-sl2-D5} $(u{-}v)\,[E(u), E(v)]
\;=\; \hbar\bigl(E(u)\,E(v) + E(v)\,E(u)\bigr)$
\textup{(}deformed Serre relation\textup{)}.

\item\label{drinfeld-sl2-D6} $(u{-}v)\,[F(u), F(v)]
...
\begin{proof}
Relations~\ref{drinfeld-sl2-D1}--\ref{drinfeld-sl2-D4} are extracted from the Gauss
...
diagonal entries produce~\ref{drinfeld-sl2-D1}, the off-diagonal--diagonal
cross-terms produce~\ref{drinfeld-sl2-D2}--\ref{drinfeld-sl2-D3}, and the purely off-diagonal
cross-terms produce~\ref{drinfeld-sl2-D4}.

For \ref{drinfeld-sl2-D1}: the RTT relations $[A(u),A(v)] = 0 = [D(u),D(v)]$
imply that $k_1(u),k_2(u)$ (and hence $H(u) = k_1(u)/k_2(u)$)
generate a commutative subalgebra.

For \ref{drinfeld-sl2-D2}: the RTT relation
...
residual identity in the off-diagonal is precisely~\ref{drinfeld-sl2-D2}.
Relation~\ref{drinfeld-sl2-D3} follows by the same argument applied to
$(u{-}v)[A(u),C(v)] = C(v)A(u) - C(u)A(v)$.

For \ref{drinfeld-sl2-D4}: the RTT relation
$(u{-}v)[B(u),C(v)] = D(v)A(u) - D(u)A(v)$
extracts, via Gauss decomposition, to
$(u{-}v)[E(u),F(v)] = H(u) - H(v)$.

For \ref{drinfeld-sl2-D5} and \ref{drinfeld-sl2-D6}: the RTT relation $[B(u),B(v)] = 0$
...
The sign in~\ref{drinfeld-sl2-D6} follows from $[C(u),C(v)] = 0$ by the same
...
\begin{proof}
The PBW property follows from the Drinfeld relations~\ref{drinfeld-sl2-D1}--\ref{drinfeld-sl2-D6}
by a standard diamond lemma argument: the leading terms of
\ref{drinfeld-sl2-D2}--\ref{drinfeld-sl2-D6} (in the degree-lexicographic order
...
\begin{enumerate}[label=\textup{(\roman*)}]
\item Relations \ref{drinfeld-sl2-D2}--\ref{drinfeld-sl2-D3} at $\hbar = 0$ become
...
\item Relation \ref{drinfeld-sl2-D4} at $\hbar = 0$ becomes
$[E_r, F_s] = H_{r+s}$, i.e.\
$[e \otimes t^r,\, f \otimes t^s] = h \otimes t^{r+s}$.
\item Relations \ref{drinfeld-sl2-D5}--\ref{drinfeld-sl2-D6} at $\hbar = 0$ become
...
\begin{proof}
Setting $\hbar = 0$ in relations~\ref{drinfeld-sl2-D1}--\ref{drinfeld-sl2-D6} is immediate.
For (i): \ref{drinfeld-sl2-D2} becomes $[H(u),E(v)] = 0$ at $\hbar = 0$;
expanding $H(u) = 1 + \hbar\sum H_r u^{-r-1}$ and taking
the leading $\hbar$-linear term recovers
$[h_r, e_s] = 2\,e_{r+s}$.
For (ii): \ref{drinfeld-sl2-D4} is $\hbar$-independent.
For (iii): the right-hand side of \ref{drinfeld-sl2-D5} vanishes at $\hbar = 0$,
giving $[E(u),E(v)] = 0$.
...
The identification $\hbar = 1/(k+2)$ belongs to the KZ-normalized
spectral connection, not to the trace-form collision residue. The
affine OPE
$J^a(z)\,J^b(w) \sim
k\,\kappa^{ab}/(z{-}w)^2 + f^{ab}{}_c\,J^c/(z{-}w)$
gives the trace-form collision residue
$r^{\mathrm{coll}}(z)=k\Omega_{\mathrm{tr}}/z$ after the
$d\log$ kernel absorbs one pole order. The KZ connection rescales the
Casimir-and-coupling package to
$r^{\mathrm{KZ}}(z)=\Omega/((k+h^\vee)z)$, with
$h^\vee = 2$ for $\mathfrak{sl}_2$.
...
its entirety. For $\widehat{\mathfrak{g}}_k$ the chiral
Verdier--Koszul dual is
$\mathrm{CE}^{\mathrm{ch}}(\widehat{\mathfrak{g}}_{-k-2h^\vee})$;
its scalar shadow has the Feigin--Frenkel dual level, and the second
application returns to $\widehat{\mathfrak{g}}_k$ with the central
extension intact.
\end{proof}
...
\textup{(Proposition~\ref{prop:r-matrix-descent-vol1})}.
\end{corollary}
...
Proposition~\ref{prop:r-matrix-descent-vol1}: it retains the holomorphic
...
\subsubsection*{Triple-pole residue}
...
\subsubsection*{Infinite $A_\infty$ tower}

Despite the Lie-algebraic collapse, the $T$-line remembers higher
homotopy. The cubic Stasheff operation is computed directly; the
all-order assertion is a statement about survival of the transferred
shadow classes.

\begin{theorem}[Virasoro non-formality; \ClaimStatusConditional]
\label{thm:vir-non-formality}
For the universal Virasoro family over $\bQ(c)$, the $T$-line
transferred shadow operations are nonzero in all degrees
$k\ge3$. For a fixed central charge~$c_0$, the same conclusion holds
only after the specialisations of those rational obstruction classes
remain nonzero for all~$k$. In every specialization with
$m_3\neq0$, the ordered bar complex is not formal.
\end{theorem}
...
terms coming from $T_{(1)}T=2T$ are nonzero in the universal Virasoro
OPE. Hence $m_3$ is nonzero outside the corresponding specialization
locus. The all-order statement is not a formal consequence of this
single self-loop. It is the universal non-cancellation statement proved
on the $T$-line by Theorem~\ref{thm:shadow-tower-asymptotics}: each
higher shadow coefficient has a nonzero leading Laurent term over
$\bQ(c)$. Specialising $c$ may kill individual rational classes; a
fixed central charge therefore needs the stated non-cancellation check.
\end{proof}
...
\item \emph{Non-formality} is a property
of the $A_\infty$-chiral structure on $\mathrm{Vir}_c$ transferred to
bar cohomology. The universal $T$-line has nonzero transferred
operations in all degrees; a fixed central charge must be checked after
specialization. It does \emph{not}
...
The three mechanisms assemble into a single structural dichotomy.

\begin{corollary}[Gauge-gravity complexity dichotomy; \ClaimStatusConditional]
...
\textup{Gravity \textup{(}Vir, conditional class~$\mathbf{M}$\textup{)}}
 & m_3\neq0,\;m_k\neq0\ \textup{universally on the }T\textup{-line}
...
$\Rightarrow$ conditional class~$\mathbf{M}$ $\Rightarrow$
non-formal after $T$-line non-cancellation.
...
the conditional class~$\mathbf{M}$ corridor: the Sugawara construction
manufactures the quartic pole from the affine double pole, and the
resolvent tree formula supplies the candidate higher operations. Their
all-order survival is precisely the $T$-line non-cancellation
hypothesis.
\end{corollary}

\begin{proof}
The $\Ainf$ column: class~$\mathbf{L}$ is formal by pole-order
classification, and the gravity line is non-formal on the universal
$T$-line by Theorem~\ref{thm:vir-non-formality}. The coproduct column:
the
...
\begin{theorem}[Virasoro generator coproduct primitivity;
\ClaimStatusConditional]
...
affine coproduct on~$\widehat{\mathfrak{sl}}_2$, on the principal
DS ghost-zero projection surface. Then the linear component is
primitive on the Virasoro generator:
\begin{equation}\label{eq:grav-coproduct-primitive}
(p \otimes p) \circ \Delta_z^{\mathrm{Vir}}(T)
\;=\;
T \otimes 1 + 1 \otimes T,
\end{equation}
where $p$ is the projection to ghost number~$0$. The all-degree
vanishing of nonlinear generator-level coproduct components is the
conditional signed ghost-defect statement isolated in Volume~II,
Theorem~\textup{\ref*{V2-thm:gravitational-primitivity}}.
\end{theorem}

\begin{proof}
The displayed formula is the degree-one generator computation: the
split affine Casimir terms contain weight-one current factors, and
the projection \(p\otimes p\) to the Virasoro ghost-zero cohomology
kills them.  Nonlinear HPL coproduct corrections require the signed
tree count of the Volume~II theorem.  The slogan
\(\mathrm{gh}(h)=-1\) is not by itself the proof of all arities.
\end{proof}
...
The transferred coproduct is generator-trivial on the signed
ghost-defect surface. In the
...
\begin{theorem}[Conditional universal Kac--Moody Yangian criterion;
\ClaimStatusConditional]
\label{thm:km-yangian}
Assume the root-sector strictification package of
Theorem~\textup{\ref{thm:complete-strictification-v1}} and the
$R$-twisted descent package of
Proposition~\textup{\ref{prop:r-matrix-descent-vol1}}.
...
strictification
\textup{(}Theorem~\ref{thm:complete-strictification-v1}\textup{)},
...
\textup{(}Proposition~\textup{\ref{prop:r-matrix-descent-vol1})};
...
\emph{Step~1: Collision residue and KZ normalization.}
...
residue in trace-form normalization is
$r^{\mathrm{coll}}(z)=k\Omega_{\mathrm{tr}}/z$. After the
Casimir-and-coupling rescaling used by the KZ connection, the spectral
classical $r$-matrix is
\begin{equation}\label{eq:km-collision-residue}
r^{\mathrm{KZ}}_{12}(z)
\;=\;
\frac{\Omega_{12}}{(k+h^\vee)z}
\;=\;
\frac{\hbar\,\Omega_{12}}{z}.
\end{equation}
The simple-pole bracket channel supplies the bar differential in
Step~2; it is not a pole-valued collision-residue term in
\eqref{eq:km-collision-residue}.
...
$k\,(T^a,T^b)/(z-w)^2$ contributes through the trace-form residue
$k\Omega_{\mathrm{tr}}/z$, equivalently through the KZ-normalized
spectral datum $\Omega/((k+h^\vee)z)$, to the \emph{$R$-matrix}, not
...
for the KZ-normalized $r$-matrix
...
\end{equation}
Substituting
$r_{ij}(z)=\Omega_{ij}/((k+h^\vee)z)$, the three
...
Its monodromy representation is a braid-group representation.
The Drinfeld--Kohno theorem identifies this braided representation,
after a choice of associator and the standard half-monodromy
normalisation
\[
 q=e^{\pi i/(k+h^\vee)},
\]
with the braid representation coming from the universal
$R$-matrix of $U_q(\mathfrak g)$. The Yangian appears on the
ordered line side before exponentiation: finite-dimensional
evaluation modules for $Y_\hbar(\mathfrak g)$ carry the rational
matrix $R(u)=1+\hbar\,\Omega/u+O(\hbar^2)$, and this additive
spectral datum is compatible with the KZ normalisation under the
trigonometric/rational degeneration. Thus the ordered bar complex
refines the KZ connection at chain level; it does not assert that KZ
monodromy factors through the Yangian as an algebra, nor does it
prove a category-$\mathcal O$ equivalence.
\end{remark}
...
residue with a simple pole $k\Omega_{\mathrm{tr}}/z$ in
trace-form normalization, equivalently the KZ-normalized spectral
term $\Omega/((k+h^\vee)z)$. The simple-pole Lie bracket channel
supplies the bar differential; it is not a pole-valued
collision-residue term. This
...
\begin{remark}[Kac--Moody multiplicity boundary]
\label{rem:km-frontier}
Theorem~\ref{thm:km-yangian} covers the finite-dimensional simple
case only under its strictification and descent hypotheses. The
remaining boundary is Kac--Moody algebras
...
\begin{remark}[Line-side square-root convention at nonzero curvature;
\ClaimStatusConditional]
\label{rem:oackd-3}
\index{curvature normalization!line-side product}%
\index{deformation parameter!square-root convention}%
The scalar Verdier conductor of Theorem~C is the additive invariant
$K^\kappa(A,A^!)=\kappa(A)+\kappa(A^!)$.  A line-operator comparison
introduces a different, presentation-dependent product
\[
P^{\kappa}_{\mathrm{line}}(A)
\;:=\;
\kappa_{\mathrm{vert}}(A)\,
\kappa_{\mathrm{line}}\bigl(A^!_{\mathrm{line}}\bigr).
\]
Here both factors belong to the same trace convention and require an
independently constructed line-side dual.  On the locus
$P^{\kappa}_{\mathrm{line}}(A)\in\CC^\times$, a choice of square root
defines a deformation parameter by
\[
\hbar^{2}\,P^{\kappa}_{\mathrm{line}}(A)
\;=\;
-1.
\]
Thus the displayed equation records the chosen line-side
normalisation; its mathematical content lies in an independent
construction of $P^{\kappa}_{\mathrm{line}}$ and of $\hbar$.  The
zero-product lanes, including the computed Heisenberg and affine
additive conductors, carry their trace normalisation directly.

For the K3 lattice, the exact arithmetic statement is
$2c_+=2\cdot4=8$ for the Mukai lattice of signature $(4,20)$.
Identifying this integer with a chiral conductor requires the
comparison package $H_{\mathsf B}$; identifying it further with
$P^{\kappa}_{\mathrm{line}}$ requires a compact Hall object, its
line-side dual, and a common trace form.  Under those hypotheses the
chosen branch gives $\hbar^2=-1/8$.  The computed additive conductors
in Volume~I are $\{0,13,250/3\}$.  The BP value $25/3$ and the Mukai
value $8$ remain separate open comparison ledgers.
\end{remark}
...
datum strictifies to a genuine spectral factorization $R$-matrix.
\end{remark}
...
$B=[X_1,[X_3,[X_c,X_2]]]$. The displayed Jacobi consequence gives
...
\begin{theorem}[Root-sector strictification criterion for simple Lie algebras;
\ClaimStatusConditional]
\label{thm:complete-strictification-v1}
Let $\fg$ be a simple Lie algebra of any type. Assume that the
spectral Drinfeld class localizes root-sector by root-sector, and
that in each nonzero root sector the product coefficient and coproduct
coefficient are identified by the Dynkin idempotent and the coproduct
rigidity equation. Then the spectral Drinfeld class vanishes at every
filtration:
\[
...
strictifies to a genuine spectral factorization realization.
...
with trivial quasi-associator, hence a genuine spectral factorization
realization.
\end{proof}

\begin{remark}[The role of root multiplicity one]
The criterion has the form of a
...
after the spectral class has been localized to one-dimensional root
sectors.
...
(Theorem~\ref{thm:master}). The root-sector strictification criterion
says that, once the spectral quasi-factorization package and the
coefficient comparison have been constructed, the passage from quasi
to strict is forced inside each simple root sector. Just as the
...
makes the strictification step a Lie-theoretic calculation rather
than a scalar shadow identity.
\end{remark}
...
The boundary admits a precise stratification: the obstruction vanishes
...
$\alpha+n\delta$ have multiplicity~$1$ (so the root-sector criterion
applies after the same localization hypotheses), while the imaginary roots $n\delta$ have
...
\begin{remark}[Borcherds lift as strictification device]
\label{rem:borcherds-lift-scope}
The strictification criterion~\ref{thm:complete-strictification-v1}
...
The lift is not uniform across Kac--Moody types. Three regimes must be
distinguished:
\begin{enumerate}
\item \emph{Finite and affine types with real roots.} All real roots
$\alpha$ satisfy $\dim\fg_\alpha=1$;
Theorem~\ref{thm:complete-strictification-v1} applies in the
real-root sectors only after the spectral class has been localized
there. The Borcherds lift is trivial (identity) on these
sectors.
...
be proved inside the super-bracket after imaginary-root expansion.
\end{enumerate}
...
Three test sectors fix the present boundary of the conjecture.
For untwisted affine $\widehat\fg$, the super-lift is the trivial
extension by the $\mathbb{Z}^\ell$-graded Heisenberg of the
imaginary-root tower, and the Cartan gauge-twist of
Remark~\ref{rem:kac-moody-failure} is the bosonic projection of a
super-Jacobi collapse; on $\widehat{\mathfrak{sl}_2}$ the imaginary
roots $n\delta$ carry a single $\alpha^\vee$ and the super-extension is
trivial. For the Fake Monster
$\mathfrak{m}_{\mathrm{fake}}=\operatorname{Lie}(V_\Lambda^{\mathrm{phys}})$
\cite{Borcherds1990}, the lattice vertex algebra $V_\Lambda$ for
$\Lambda=\mathrm{II}_{25,1}$ and the no-ghost theorem supply an
explicit $\Lambda$-grading on the imaginary-root spaces. For the
Monster Lie algebra $\mathfrak{m}$, the Borcherds denominator
$p^{-1}\prod_{(m,n)>0}(1-p^m q^n)^{c(mn)}=j(p)-j(q)$
\cite{Borcherds1992} presents each imaginary-root sector as
$c(mn)$-dimensional and hence as a single super-generator at level
$c(mn)$ after the auxiliary grading is installed.

The remaining condition is the generic hyperbolic
Kac--Moody case, where the Borcherds lift has not been made explicit.
The first decisive computations are the untwisted
$\widehat{\mathfrak{sl}_2}$ case at a generic non-critical level and
the lowest Monster imaginary-root sector
$[E_{(1,1)},E_{(1,1)}]$ with
$\dim\mathfrak{m}_{(1,1)}=c(1)=196884$.
\end{remark}

\begin{remark}[Hall--Drinfeld double as fourth taxonomic slot beyond
Yangian]
\label{rem:oackd-1}
\index{Hall--Drinfeld double!fourth taxonomic slot}%
\index{K3 Yangian!rejected nomenclature}%
\index{Manin pair!signature-(2,1) Cartan}%
Under the strictification package of
Theorem~\ref{thm:complete-strictification-v1}, the finite-dimensional
simple case produces $Y_\hbar(\fg)$ as a classical Yangian because the Cartan datum
of every simple finite-dimensional $\fg$ carries a Kac--Moody grading,
a Weyl group, and a Drinfeld J-presentation. None of these inputs
survives on the BKM side of the K3 chiral landscape surveyed in
Part~\ref{part:v1-frontier} and developed cross-volume: the
BKM companion object sometimes called a ``K3 Yangian'' in shorthand,
under the Vol~III CY-to-chiral two-stage factorisation
$\Phi_3^{(\Sigma_2,C)} = \SpCh_{\Sigma_2, C} \circ \PhiFA_3$, on the $K3$-fibre
stage-$2$ shadow of $\PhiFA_3(D^b(\mathrm{Coh}(K3 \times E)))$
\textup{(}Vol~III Chapter~\ref{V3-ch:k3-chiral-bialgebra-platonic},
Theorem~\ref{V3-thm:k3-structural-genesis}, and
Remark~\ref{V3-rem:k3-not-yangian-strict}\textup{)}, is not a
Yangian at all, but the
compact Hall--Drinfeld double
\[
D_\hbar\!\left(Y_{\hbar,\mathrm{crit}}^{\mathrm{Hall},+}(K3\times E)\right),
\]
formed in Vol~III finite windows from the reduced compact Hall source,
the Serre-dual negative half, the Euler--Hall pairing, the Mukai Cartan
torus, and the radical quotient. The finite Rees hCS--Hall comparison
gives bounded Rees Hall classes and comparison maps; it is not by
itself this compact double and does not prove its Borcherds
recognition. The Cartan lives
inside a Manin
pair $(\fg_{\Delta_5}, \fn_+^{\mathrm{imag}} + \fh^{\mathrm{imag},\mathrm{rk}\,23})$
whose ambient metric has signature $(2,1)$ on the imaginary directions;
this signature \emph{forbids} the Lagrangian decomposition of a Manin
triple, so this Hall--Drinfeld double has no classical Manin-triple
RTT presentation on a real root ladder. The chiral
Hall--Drinfeld double therefore occupies a taxonomic slot genuinely
distinct from the three classical slots of this chapter: it is
neither a Drinfeld--Jimbo quantum group, nor a Yangian, nor an RTT
algebra, nor a Manin-triple quasi-Hopf algebra. The strictification
theorem of this chapter does \emph{not} claim coverage of this fourth
slot; the Kac--Moody multiplicity boundary of
Remark~\ref{rem:km-frontier} widens, beyond root-multiplicity
breakdown, into the BKM regime where the Cartan signature itself
obstructs the classical double.
\end{remark}

\begin{remark}[Identification of the chiral quantum group
 undergirding the K3 BKM: chain-level naming]
\label{rem:k3-chiral-quantum-group-identification}
\ClaimStatusConditional
\index{K3 chiral quantum group!identification}%
\index{Gritsenko-Nikulin denominator!as Koszul datum}%
\index{Arthur packet!of Delta_10 as Koszul specialisation}%
Three candidate names circulate in the literature for the chiral
quantum group undergirding the K3 BKM / Siegel bialgebra:
(i) an affine K3 Yangian $Y_\hbar(\widehat{\mathfrak{g}}_{\Delta_5})$,
(ii) an elliptic Yangian on the twisted torus, and
(iii) the Feigin--Odesskii quantum-toroidal algebra
$U_{q_1,q_2}(\widehat{\widehat{\mathfrak{gl}}}_1)$. None matches the
global object. The Koszul-duality structure of the ordered bar
complex selects a fourth object, and the selection is fixed by the
same gauge-invariant calculation that identifies the Igusa form
$\Delta_5$.

\smallskip
\noindent\emph{Rejection of the three candidates.}
(i) $Y_\hbar(\widehat{\mathfrak{g}}_{\Delta_5})$ does not exist as a
Drinfeld object: the BKM Cartan is infinite-dimensional and contains
infinitely many imaginary simple roots, so the Drinfeld
J-presentation never closes
\textup{(}Remark~\textup{\ref{rem:oackd-1}}\textup{)}.
(ii) An elliptic Yangian on a single torus misses the $24$-node
discriminant of the K3 fibration: the single-$\tau$ parameter cannot
resolve the $24$ independent Mukai-lattice directions.
(iii) The Feigin--Odesskii quantum-toroidal algebra
captures one node of the $24$-node locus only; it is a \emph{local}
piece, not the global chiral quantum group.

\smallskip
\noindent\emph{Identification via the ordered Koszul pair.}
Apply the ordered bar--cobar construction, together with the
$R$-twisted descent of
Proposition~\ref{prop:r-matrix-descent-vol1}, to the compact
Hall--Drinfeld double $\cA = \mathbf{H}_{\Delta_5}$, when the compact
passage datum of Remark~\ref{rem:oackd-1} has been supplied. The finite Rees
hCS--Hall maps alone give only a bounded positive-half target and do
not construct $\cA$. The chain-level ordered Koszul dual
$\cA^!_{\mathrm{line}}=(\cA^i)^\vee$ is produced from the quadratic
comparison $q_\cA^{\mathrm{ord}}\colon\cA^i\to\Barchord(\cA)$ by
finite-type ordered linear duality and the opposite-coalgebra duality
of Theorem~\textup{\ref{thm:opposite}} is, on the
quadratic stratum $R_{\mathrm{quad}}$ of
Remark~\textup{\ref{rem:k3-koszul-structure-precise}} of
Vol~I, Ch.~\ref{ch:chiral-climax-platonic}, the Chevalley--Eilenberg
coalgebra $C^{\mathrm{CE}}_\star(\fg_{\Delta_5})$ computed in the
Schiffmann--Vasserot shuffle basis. On the full non-quadratic
Borcherds stratum $R_{\mathrm{Borcherds}}$, $\cA^!_{\mathrm{line}}$
acquires a curved perturbation whose curvature class is the
Gritsenko--Nikulin denominator cocycle. The correct name for the
resulting quantum group is therefore not ``K3 Yangian'' but the
\emph{Drinfeld double of the curved Chevalley--Eilenberg coalgebra}
\[
 \mathbf{Q}^{\mathrm{K3}}_{\hbar}
 \;:=\;
 D_\hbar\!\bigl(C^{\mathrm{CE},\mathrm{crv}}_\star(\fg_{\Delta_5});\;
 \omega_{\Delta_5}\bigr),
\]
with curvature $\omega_{\Delta_5}$ the Gritsenko--Nikulin $2$-cocycle of
the primitive BKM denominator line. In the theta-normalisation this line
is \(\Delta_5\) with leading
coefficient \(64\), while the monic Borcherds product is
\(D_5=64^{-1}\Delta_5\). Hence
\(\Phi_{10}^{\mathrm{un}}=\Delta_5^2\),
\(\Phi_{10}^{\mathrm{OP}}=D_5^2=4096^{-1}\Delta_5^2\), and
\(Z^{X}_{\mathrm{OP/DT}}=-(\Phi_{10}^{\mathrm{OP}})^{-1}
=-4096\,\Delta_5^{-2}\). These are scalar normalisations, not the
compact Hall double and not the primitive denominator datum. The
scalar relation is simply $64^2=4096$ in the displayed normalization;
it concerns the Borcherds/Gritsenko coefficient, not compact Hall
data. The
denominator identity is
\textup{(}Gritsenko--Nikulin\,1998, Theorem~$2.1$; Borcherds\,1992,
Theorem~$10.1$\textup{)}.

\smallskip
\noindent\emph{Conditional line-side normalisation.}
The Mukai lattice has signature $(4,20)$, hence the exact lattice
integer $2c_+=8$.  Supply the compact comparison package
$H_{\mathsf B}$, construct the line-side curvature in the same trace
convention, and identify
$P^\kappa_{\mathrm{line}}=2c_+$.  The square-root convention of
Remark~\textup{\ref{rem:oackd-3}} then gives
\[
 \hbar^{2}_{\mathrm{K3}} \;=\; -\tfrac{1}{8}.
\]
This equation is conditional on the three comparison inputs just
listed.  A proof of strict ordered bar--cobar closure at this branch is
an additional obligation in the compact Hall programme.

\smallskip
\noindent\emph{Arthur-packet witness.}
The image of $\mathbf{Q}^{\mathrm{K3}}_{\hbar_{\mathrm{K3}}}$ under
the Langlands correspondence for $\mathrm{Sp}_4$ is the Arthur
packet $\psi_{\Delta_{10}} = \phi_{\Delta_{E_6}}\boxtimes\mathrm{Sym}^1$
of Vol~III,
Theorem~\textup{\ref{V3-thm:arthur-packet-H-delta5}}. At the $p$-adic
places the Hecke eigenvalues of the Saito--Kurokawa/Ikeda lift
satisfy
\[
 \lambda_p(\Delta_{10}) \;=\; a_p(\Delta_{E_6}) + p^{8} + p^{9},
\]
this being the Maass relation in Hecke-eigenvalue form
\textup{(}Andrianov\,1974, Theorem~$3.2$; Ikeda\,2001,
Corollary~$16.2$\textup{)}; the $p^{8} + p^{9}$ term is the
signature-of-$\mathrm{Sym}^1$ contribution to the Arthur parameter,
and its presence is the \emph{chain-level} witness that
$R_{\mathrm{Borcherds}} \ne 0$ on the non-quadratic stratum.
The Siegel form attached to this Arthur packet at level~$1$,
weight~$10$, for $\mathrm{Sp}_4(\mathbb{Z})$ is Igusa's
$\Delta_{10}$, and not any other candidate
\textup{(}Eisenstein $E_{10}$ is not cuspidal; Ikeda's generic
degree-$4$ lift produces $\Delta_{10}$ as its unique non-trivial
output at $k=10$; there is no ``Schottky form $J_8$'' at weight $10$
to conflate with\textup{)}.

Under the compact passage datum, $\mathbf{Q}^{\mathrm{K3}}_{\hbar}$
at $\hbar^{2} = -1/8$ with Arthur parameter
$\psi_{\Delta_{10}}$ is the object denoted here by the chiral quantum
group undergirding the K3 BKM. The unconditional conclusion needed in
this volume is narrower and sharp: the Drinfeld-Yangian,
single-torus elliptic-Yangian, and one-node quantum-toroidal names do
not match the BKM Cartan, the $24$-node Mukai-lattice discriminant,
or the compact double data; the constants \(64\) and \(4096\) normalise
the Borcherds denominator and its square, not the algebra object.
The unquotiented compact Hall--Drinfeld/Borcherds recognition remains
conditional on the primitive comparison, PBW/no-extra-relations,
radical faithfulness, centre, associator, and inverse-limit data named
in Remark~\textup{\ref{rem:oackd-1}}.
\end{remark}

\begin{remark}[Abelian at Lie, non-abelian at vertex]
\label{rem:oackd-2}
\index{chiral bialgebra!abelian-at-Lie, non-abelian-at-vertex}%
\index{quantum toroidal algebra!Miki}%
\index{K3 Fock!Frenkel--Kac closure}%
The BKM taxonomic slot of Remark~\ref{rem:oackd-1} exhibits a
splitting phenomenon absent in the affine lineage. At the level of the
chiral bialgebra of generators and Lie brackets, the imaginary-root
sectors of $\fg_{\Delta_5}$ \emph{abelianise}: the Hall side produces
$24$ commuting copies of the Miki quantum toroidal algebra of
$\mathfrak{gl}_1$, one per weight-$(1,1)$ direction in the K3 lattice.
Non-abelianness is reinstated one level up, at the vertex-algebraic
closure on the K3 Fock space: Frenkel--Kac $1980$ vertex closure
glues the $24$ toroidal copies into a single BKM-style chiral bialgebra
whose non-abelian brackets appear only after passage through the
vertex operators $e^{\alpha\cdot\phi}$. The G/L/C/M classification of
Remark~\ref{rem:glcm-commentary} therefore splits, on the K3 input, into
a two-layer tower: a class-$\mathbf{G}$ layer (abelian at Lie) and a
class-$\mathbf{L}$ layer (non-abelian at vertex), stacked by the
Frenkel--Kac closure.
\end{remark}
...
The conditional criterion of Theorem~\ref{thm:km-yangian}
specializes to every simple Lie algebra once its hypotheses are
verified.
...
coefficient $1/n$ supplied by the Vol.~II $n$-gon localisation formula.
...
from the ordered bar complex; \ClaimStatusConditional]
\label{thm:sl3-rtt}
...
$\Conf_2^<(\bC)$, where
$r^{\mathrm{KZ}}(z)=\Omega/((k+h^\vee)z)$ is the
KZ-normalized classical $r$-matrix~\eqref{eq:km-collision-residue}.
The underlying trace-form collision residue is
...
\]
follows from $d^3=0$ on ${\Barch}^{\mathrm{ord}}_4$ and is checked
...
$A_2$. The filtration-by-filtration calculation above records
...
rank-$2$ non-$A$ types; \ClaimStatusConditional]
...
\label{thm:r-sp4}
\label{thm:r-g2}
For each of the rank-$2$ types below, the trace-form collision
residue is $k\Omega_{\mathrm{tr}}/z$ and the KZ-normalized spectral
$r$-matrix is $r^{\mathrm{KZ}}(z)=\Omega/((k{+}h^\vee)z)$.
...
are proved by direct root-space analysis.
\end{proof}
...
and their interaction with the braid generators supplies the
quantum-group monodromy readout associated with
$U_q(\mathfrak{g})$.
...
rigorous passage from the ordered bar superconnection to the KZ/KZB
connections and then to the finite-dimensional quantum-group
...
\begin{theorem}[Quantum-group parameter from $B$-cycle monodromy; \ClaimStatusProvedHere]
...
\item \textbf{Quantum-group readout.}
The combined $A$-cycle and $B$-cycle monodromies, together with the braid
monodromies, give the braid and Cartan operators that the
Drinfeld--Kohno comparison identifies with the finite-dimensional
$U_q(\mathfrak{g})$ representation at the parameter
$q = e^{\pi i\hbar}$.
\end{enumerate}
\end{theorem}
...
\medskip
\noindent\textbf{(iii).}
The $A$-cycle monodromies give the $R$-matrix readout, the full
$B$-cycle monodromies give the Cartan operators $q^{2H}$, and after
choosing the standard half-monodromy convention one recovers the
usual $q^H$ operators. The identification of this package with the
finite-dimensional $U_q(\mathfrak g)$ braid representation is the
external genus-$1$ Drinfeld--Kohno comparison recorded in
Theorem~\ref{thm:drinfeld-kohno}; the calculation here supplies the
parameter and the monodromy operators.
\end{proof}
...
$\widehat{\mathbb{G}}_m$ is the passage from the additive Yangian
readout to the multiplicative quantum-group readout. The elliptic
curve $E_\tau$ provides the geometric mechanism: its $B$-cycle is the
Wick-rotated time circle in the
...
trace-form convention as $k\,\Omega_{\mathrm{tr}}/z$. These are two
normalizations of the same affine monodromy datum, with
$\Omega=2h^\vee\Omega_{\mathrm{tr}}$ and the KZ coupling
$(k+h^\vee)^{-1}$ applied after the trace-form collision residue is
extracted. They are not the same rational function of~$k$. With the
KZ choice,
...
ordinary, Yang--Baxter equation. The affine genus-$1$ surface in this
chapter is therefore compared directly with Bernard/CEE.
...
The identification of KZ monodromy with the quantum-group braid
representation is external input. The contribution of the ordered
bar complex is the reduction of the affine line to the KZ/KZB
connections under one-loop collapse.

\begin{theorem}[Drinfeld--Kohno on the affine evaluation surface;
\ClaimStatusProvedElsewhere{} for monodromy,
\ClaimStatusProvedHere{} for ordered reduction]
...
finite-dimensional $\mathfrak{g}$-modules. Fix a Drinfeld associator
and the standard half-monodromy convention
$q=e^{\pi i/(k+h^\vee)}$.
\begin{enumerate}[label=\textup{(\roman*)}]
\item \textbf{Genus $0$.}
...
\item \textbf{Genus $1$.}
The KZB connection~\eqref{eq:kzb} on
$\mathrm{Conf}_n(E_\tau)$ gives an elliptic braid-group
monodromy representation. In the finite-dimensional affine
evaluation surface, the Etingof--Schiffmann and
Calaque--Enriquez--Etingof comparisons identify its
trigonometric/quantum-group specialisation with the corresponding
$U_q(\mathfrak g)$ braid action, with the full $B$-cycle Cartan
operator $q^{2H}$ and the usual half-monodromy convention $q^H$.
This is a completed/dynamical monodromy comparison; it is not a
category-$\mathcal O$ equivalence and not an assertion that an
elliptic braid group acts through the ordinary algebra
$U_q(\mathfrak g)$ without the Cartan and completion data.

\item \textbf{Ordered-bar reduction.}
For the affine lineage $V^k(\mathfrak g)$, the one-loop collapse of
the ordered bar superconnection on finite-dimensional evaluation
modules reduces the bar-complex monodromy to the KZ connection at
genus~$0$ and to the KZB connection at genus~$1$. Hence the
external monodromy theorem applies to the ordered bar complex exactly
on this evaluation surface.
\end{enumerate}
\end{theorem}

\begin{proof}
\textbf{(i)} is the classical Drinfeld--Kohno theorem. Drinfeld
constructs a formal associator
$\Phi \in U(\mathfrak{g})^{\widehat\otimes 3}[\![\hbar]\!]$
from iterated integrals of $d\log(z_i-z_j)$ on
$\mathrm{Conf}_3(\mathbb C)$. The pentagon and hexagon equations for
$\Phi$ identify the braid representation of the infinitesimal braid
Lie algebra $\mathfrak t_n$ with the braid representation of
$\mathrm{Rep}_q(\mathfrak g)$ carrying the universal
$U_q(\mathfrak g)$ $R$-matrix. This proves
\eqref{eq:dk} after the standard half-monodromy normalisation.

\medskip
\noindent\textbf{(ii)} is the elliptic monodromy comparison of
Etingof--Schiffmann and Calaque--Enriquez--Etingof. The point relevant
here is its scope: the elliptic KZB connection produces a monodromy
representation with dynamical Cartan data; after the chosen
trigonometric/quantum-group specialisation this gives the
$U_q(\mathfrak g)$ braid action on the finite-dimensional evaluation
sector. The $B$-cycle monodromy supplies $q^{2H}$, and the usual
quantum-group convention chooses the half-monodromy $q^H$.

\medskip
\noindent\textbf{(iii).}
For the affine algebra $V^k(\mathfrak{g})$, the one-loop exactness of
the BV-BRST differential (Theorem~\ref*{V2-thm:affine-half-space-bv} of
...
bar-complex monodromy $\rho_n^{\mathrm{HT}}$ equals
$\rho_n^{\mathrm{KZ}}$ (the one-loop collapse statement of
Volume~II). The same reduction on $E_\tau$ gives the KZB connection,
because the logarithmic kernel $1/z$ is replaced by
$\wp_1(z,\tau)$ and the Bernard $d\tau$ term records the
Hodge-bundle correction. Combining this reduction with
\textbf{(i)} and \textbf{(ii)} gives the ordered-bar monodromy
comparison on the affine evaluation surface.
\end{proof}

\subsection{The genus-$1$ deformation: from Yangian to quantum group}
\label{sec:yangian-to-quantum}

The passage from genus~$0$ to genus~$1$ is a comparison of
monodromy readouts. The genus-$0$ ordered line algebra is the
Yangian on the finite-dimensional evaluation surface; the genus-$1$
KZB monodromy has quantum-group braid and Cartan readouts after the
completed elliptic comparison.

\begin{theorem}[Affine genus-one monodromy readout]
\ClaimStatusProvedHere
...
\item At genus~$0$, the ordered bar complex of $V^k(\mathfrak{g})$ on
 ...
 produces the line-side algebra $Y_\hbar(\mathfrak{g})$ on
 finite-dimensional evaluation modules, with
 $\hbar = 1/(k + h^\vee)$.

\item At genus~$1$, the ordered bar complex on
 $E_\tau \times \mathbb{R}$ reduces, on the same affine
 evaluation surface, to the KZB monodromy. After the
 Etingof--Schiffmann/CEE comparison this monodromy has the
 $U_q(\mathfrak{g})$ braid and Cartan readout, with
 $q = e^{\pi i \hbar}$. This is a monodromy-category statement, not
 an identification of the ordered Koszul-dual algebra with
 $U_q(\mathfrak g)$.

\item The degeneration $\tau \to i\infty$ \textup{(}pinching the
 $B$-cycle\textup{)} recovers the rational KZ/Yangian readout from
 the elliptic monodromy readout: the KZB connection degenerates to
 the KZ connection and the extra $B$-cycle Cartan monodromy becomes
 trivial in the nodal limit. No literal algebraic limit
 $\lim U_q(\mathfrak g)=Y_\hbar(\mathfrak g)$ is asserted.
...
\begin{proof}
Part~(i) is the content of the genus-$0$ construction: the bar
differential on $\mathrm{FM}_n(\mathbb{C})$ encodes the chiral OPE,
the $E_1$ coproduct on $\mathrm{Conf}_n(\mathbb{R})$ encodes the
associative composition, and the resulting line-side algebra on
evaluation modules is the Yangian (see the holographic dictionary of
Volume~II,
Corollary~\ref*{cor:holographic-dictionary}).

Part~(ii) combines Theorem~\ref{thm:b-cycle-quantum-group} with the
Drinfeld--Kohno comparison (Theorem~\ref{thm:drinfeld-kohno}(ii)): the
KZB monodromy supplies the quantum-group braid matrices and the
$B$-cycle Cartan operators on the finite-dimensional evaluation
sector.
...
\begin{proposition}[Quantum-group convention bridge for DK/KZ readouts;
\ClaimStatusProvedHere]
\label{prop:dk-q-convention-bridge}
\index{quantum group parameter!KZ convention bridge}
\index{monodromy!self versus mutual}
\index{framing square root!DK convention}
\textup{[}Type signature:
$(\mathrm{Open/line},\mathrm{KZ\ monodromy\ readout},
\mathrm{levels}=1\to4,
\mathrm{hypothesis\ package}=\mathrm{affine\ evaluation\ surface}
\ +\ \mathrm{chosen\ Drinfeld\ associator}
\ +\ \mathrm{half\text{-}monodromy\ convention})$\textup{].}
On the affine evaluation surface the additive parameter is
\[
\hbar=(k+h^\vee)^{-1}.
\]
The full $B$-cycle/self-monodromy factor is
\begin{equation}
\label{eq:dk-full-monodromy-q-square}
q^2=e^{2\pi i\hbar},
\end{equation}
and the quantum-group braiding parameter is the framing square root
\begin{equation}
\label{eq:dk-half-monodromy-q}
q=e^{\pi i\hbar}=e^{\pi i/(k+h^\vee)}.
\end{equation}
With this convention, the KZ local monodromy around
$z_i=z_j$ is
\begin{equation}
\label{eq:dk-kz-monodromy-sign}
\exp(2\pi i\,\hbar\,\Omega_{ij}),
\end{equation}
while the braided tensor category uses the half-braiding
$P\circ R$ whose diagonal framing contribution is the square root
\eqref{eq:dk-half-monodromy-q}.  The modular $T$-matrix convention is
\begin{equation}
\label{eq:dk-t-matrix-convention}
T_{ii}=\exp\bigl(2\pi i(h_i-c/24)\bigr),
\end{equation}
so self-monodromy \textup{(}twist/framing of one object\textup{)} and
mutual monodromy \textup{(}double braiding of two objects\textup{)}
are related by the ribbon identity, not by equality of the two
numbers.  Converting between KZ, quantum-group, and modular
$T$-matrix conventions therefore requires carrying the square root
\eqref{eq:dk-half-monodromy-q}, the associator gauge, and the
orientation of the loop around the diagonal.
\end{proposition}

\begin{proof}
Theorem~\ref{thm:yangian-quantum-group} identifies the full
$B$-cycle monodromy as $e^{2\pi i\hbar}=q^2$ and the standard
quantum-group parameter as its half-monodromy square root
$q=e^{\pi i\hbar}$.  The KZ connection has residue
$\hbar\Omega_{ij}$ along the diagonal $z_i=z_j$, hence a positively
oriented loop has monodromy
$\exp(2\pi i\hbar\Omega_{ij})$.  Passing from this full local
monodromy to the braided tensor category chooses the Drinfeld
associator and the square-root framing convention, producing the
half-braiding parameter in \eqref{eq:dk-half-monodromy-q}.  The
formula for $T_{ii}$ is the standard conformal-block twist convention;
the ribbon identity then separates the twist of one object from the
mutual double braiding of two objects.
\end{proof}
...
on~$E_\tau \times \bR$ whose ordered bar superconnection admits a
KZB-type reduction and a strong-unitary elliptic descent datum, the
genus-$1$ ordered bar complex is conjectured to produce a monodromy
object whose trigonometric degeneration is the genus-$0$ ordered line
algebra, with the $B$-cycle monodromy implementing the exponential map
$\hbar \mapsto q^2 = e^{2\pi i \hbar}$ and the standard quantum
...
\item The integration domain is $\mathrm{FM}_n(\mathbb{C}) \times
 ...
 implementing the topological interval-cutting. This ordered
 holomorphic-topological domain supplies the monodromy input; the
 topological link invariant is obtained only after the external
 Reshetikhin--Turaev ribbon trace, equivalently the
 $E_\infty$ factorization-homology/CFG topological trace package, is
 applied.
\end{enumerate}
\end{theorem}
...
is externally identified with the quantum-group braid representation).
...
Part~(iv) is a scope statement, not a derivation of the
Reshetikhin--Turaev invariant from the raw $E_1$-ordered bar. The bar
complex lives on the product of holomorphic and topological
configuration spaces and supplies the monodromy input. The passage from
that monodromy to a topological link invariant uses the external
Reshetikhin--Turaev ribbon trace, equivalently the
$E_\infty$ factorization-homology/CFG topological trace package.
\end{proof}
...
\subsection{Affine and Higher-Genus Boundary}
\label{sec:genus1-frontier}

\begin{remark}[Affine-lineage boundary]
\label{rem:genus1-affine-lineage-boundary}
...
Beyond the affine lineage, two extensions remain unproved:
\begin{enumerate}[label=\textup{(\alph*)}]
...
The algebraic object controlling the monodromy at genus~$g$ is
conjecturally a \emph{higher-genus quantum group} or \emph{mapping-class-group
...
\begin{theorem}[Ordered pole-depth spectrum]
\ClaimStatusProvedHere
...
Then the pole-depth spectrum at genus~$0$, degree~$2$ is
\begin{equation}\label{eq:depth-spectrum}
\mathrm{Spec}\bigl(q_{\mathrm{pole}}^{E_1}; A\bigr)\big|_{(0,2)}
\;=\;
\bigl\{\,\max(p-1,\,0) : p \text{ is a pole order of the OPE of } A\,\bigr\}.
\end{equation}
Here $q_{\mathrm{pole}}^{E_1}$ is pole/normal-jet depth, not the
geometric Mok/Fulton--MacPherson boundary codimension. At degree~$2$
the geometric collision divisor still has codimension~$1$.
The following table records the pole-depth spectrum for the standard
examples:
\begin{center}
\renewcommand{\arraystretch}{1.2}
\begin{tabular}{lll}
\textbf{Algebra} & \textbf{OPE poles} & \textbf{Pole-depth spectrum} \\
\hline
...
\end{tabular}
\end{center}
The pole-depth spectrum of the Virasoro algebra exhibits a gap at
$q_{\mathrm{pole}}=2$,
...
(the collision kernel $K_A^{\mathrm{coll}}(z)$ has pole orders one less than the OPE),
producing a residue of order $p - 1$ in the collision variable.
This residue has pole/normal-jet depth $p{-}1$ along the same
geometric collision divisor; it is not a statement that the degree-$2$
FM boundary has codimension $p{-}1$. The zeroth product
$a_{(0)}b$ (pole of order~$1$) contributes at pole depth
$q_{\mathrm{pole}}=0$,
living in the interior.

For the Virasoro algebra: the quartic pole gives pole depth
$q_{\mathrm{pole}}=3$, the double pole gives
$q_{\mathrm{pole}}=1$, and the simple pole gives pole depth
$q_{\mathrm{pole}}=0$. The cubic pole is absent, so
$q_{\mathrm{pole}}=2$ does not appear.
\end{proof}
...
The ordered AOS relations are the image of the Arnold--Orlik--Solomon
relations under the projection to consecutive-block collision faces.
Equivalently, they are the Stasheff associahedron identities imposed on
the bar differential $d_{\Barchord}$ by $d_{\Barchord}^2=0$ on the
partial compactification
$\overline{\FM}_k^{\mathrm{ord}}(\mathbb{C})$. At degree~$k$, the
codimension-one faces are the consecutive-block collision faces
$D_I$ with $I=\{a,\dots,b\}$ and $|I|\ge2$; the binary part consists of
$D_{i,i+1}$ ($1 \leq i \leq k{-}1$).
The condition
\begin{equation}\label{eq:ordered-d-squared}
d_{\Barchord}^{\,2}=0
\quad\Longleftrightarrow\quad
\left(\sum_{s\ge1}\widehat m_s^{\mathrm{ch}}\right)^2=0
\quad\Longleftrightarrow\quad
\sum_{r+s+t=n}
(-1)^{r+st}\,
m_{r+1+t}^{\mathrm{ch}}
\bigl(\id^{\otimes r}\otimes m_s^{\mathrm{ch}}
\otimes\id^{\otimes t}\bigr)
=0
\end{equation}
with the desuspension signs fixed in
Construction~\ref{constr:bar-diff-collision}. In the strict
associative case only $m_1$ and $m_2$ remain; the $n=3$ relation is
$m_2(m_2\otimes\id)-m_2(\id\otimes m_2)=0$ after unsuspending.

In the full unordered FM compactification, $d^2 = 0$ encodes the
Jacobi/Borcherds identities through the Arnold relation among all
$\binom{k}{2}$ pairwise divisors. In the ordered partial
compactification, non-consecutive divisors are not boundary generators
of the ordered bar differential. The Arnold relation is still true in
the holomorphic factor and remains the flatness input for
ordered-to-symmetric descent; it is not used as an extra
non-consecutive face in the ordered associative differential.
\end{theorem}

\begin{proof}
By Stokes' theorem on
$\overline{\FM}_k^{\mathrm{ord}}(\mathbb{C})$, the condition
$d_{\Barchord}^2 = 0$ is equivalent to the cancellation of oriented
codimension-two boundary contributions for the logarithmic bar
propagators. The binary part of the boundary decomposes as
\[
\partial_1\overline{\FM}_k^{\mathrm{ord}}(\mathbb{C})
\;=\;
...
is the binary shadow of the $k$-th Stasheff identity. Higher
consecutive-block faces give the remaining summands in
\eqref{eq:ordered-d-squared}.
...
collision terms from the ordered differential. The Arnold relation
$\omega_{ij} \wedge \omega_{jk} + \omega_{jk} \wedge \omega_{ki}
+ \omega_{ki} \wedge \omega_{ij} = 0$
still holds on the full holomorphic configuration space; after
projection to the consecutive-block quotient, its non-consecutive term
is outside the ordered boundary algebra, and the surviving identity is
precisely the Stasheff associativity relation.
\end{proof}
...
Assume the strong-unitary $R$-twisted descent datum of
Proposition~\ref{lem:R-twisted-descent} has been constructed. Let
$F_{\leq d}^{\mathrm{ord}}$ and $F_{\leq d}^{\Sigma}$ denote the
filtrations by boundary codimension on the ordered and symmetric
compactifications. The averaging map
$\mathrm{av}\colon\Barchord(A) \to \BarchSig(A)$ preserves this
filtration:
\[
\mathrm{av}\colon
F_{\leq d}^{\mathrm{ord}}\Barchord(A)_{(g,n)}
\;\longrightarrow\;
F_{\leq d}^{\Sigma}\BarchSig(A)_{(g,n)}.
\]
On the associated graded, a depth-$d$ ordered stratum maps to the
$R$-twisted coinvariant span of its $\Sigma_n$-orbit of depth-$d$
symmetric strata. Averaging loses ordering and monodromy data; it
does not create boundary codimension. At genus~$0$ and fixed
degree~$n$, the kernel
$\ker\bigl(\mathrm{av}|_{(0,n)}\bigr)$ is generated by the $R$-matrix
twisted descent relations of
Proposition~\textup{\ref{prop:r-matrix-descent-vol1}}.
For pole-free symmetric chiral algebras the monodromy part is
$R(z)=\id$; the only kernel is the ordinary coinvariant relation
$x-\epsilon(\sigma)\tau_\sigma x$. Thus the pole-free case has no
\emph{additional} $R$-monodromy kernel.
\end{proposition}

\begin{proof}
The averaging map is the composition
$\Barchord(A)_n
\xrightarrow{\Sigma_n\text{-orbit}} \BarchSig(A)_n$,
...
of codimension~$d$. The ordered-to-symmetric map is finite on the
open stratum and extends to the corresponding labelled
Fulton--MacPherson boundary strata by relabelling and quotienting.
Finite quotients preserve codimension, although several labelled
strata may acquire the same image. Thus the orbit
$\Sigma_n \cdot \alpha$ contributes only to the depth-$d$ associated
graded of the symmetric compactification, and the filtration
$F_{\leq d}$ is preserved.

For the kernel: by the coinvariant presentation of the descended
local system,
\[
\ker(\mathrm{av}|_{(0,n)})
=
\left\langle
\alpha-\rho_R(\sigma)\alpha
\;:\;
\alpha\in\Barchord(A)_n,\ \sigma\in\Sigma_n
\right\rangle .
\]
Thus the $R$-dependent part of the kernel is generated by
\((\rho_R(\sigma)-\epsilon(\sigma)\tau_\sigma)\alpha\).
When the OPE is pole-free, $R(z)=\id$ and this $R$-dependent part
vanishes; the remaining kernel is exactly the ordinary
symmetry/coinvariant span
\(\langle\alpha-\epsilon(\sigma)\tau_\sigma\alpha\rangle\).
\end{proof}

\begin{proposition}[Averaging, Reynolds, and \(R\)-twisted descent gate;
\ClaimStatusConditional]
\label{prop:averaging-reynolds-rtwisted-gate}
\index{averaging map!Reynolds and R-twisted gate}
\index{R-twisted coinvariants!bar differential}
\index{kernel of averaging!degree two and higher}
\textup{[}Type signature:
$(\mathrm{Open},\mathrm{ordered\ bar\ descent},
\mathrm{level}=2\to5,
\mathrm{hypothesis\ package}=\mathrm{char}\,0
\ +\ \mathrm{completed\ equivariance}
\ +\ \mathrm{strong\ unitary}\ R
\ +\ \mathrm{finite\ arity})$\textup{].}
The universal conductor used in this chapter is the filtered morphism
\[
\mathrm{av}_\cA\colon
\mathfrak g^{E_1}_\cA\longrightarrow \mathfrak g^{\mathrm{mod}}_\cA
\]
induced aritywise by the Reynolds operator
\[
\mathrm{Av}_n=\frac1{n!}\sum_{\sigma\in\Sigma_n}\rho(\sigma).
\]
The scalar \(1/n!\) requires characteristic zero, and on completed
arity \(n\) the operator is the continuous endomorphism induced by the
finite group algebra action on each finite conformal-weight window and
then passed to the strict inverse limit.

If the \(\Sigma_n\)-action is compatible with the bar differential,
\(\rho(\sigma)d_B=d_B\rho(\sigma)\), then
\(\mathrm{Av}_n d_B=d_B\mathrm{Av}_n\), so averaging is a chain map.
If, in addition, the action is by dg Lie automorphisms of the ordered
convolution algebra, then \(\mathrm{av}_\cA\) is a dg Lie morphism.
Without those equivariance hypotheses it is only a continuous linear
projection on the underlying completed vector space.

For a strong-unitary \(R\)-twisted descent datum, define the
\(R\)-twisted coinvariant complex by
\[
C_{\Sigma_n,R}
:=
C\Big/
\left\langle x-\rho_R(\sigma)x:\sigma\in\Sigma_n,\ x\in C\right\rangle .
\]
When \(\rho_R(\sigma)\) commutes with \(d_B\), the quotient inherits
the bar differential.  When \(\rho_R(\sigma)\) acts by dg Lie
automorphisms, the quotient also inherits the convolution bracket.
In degree \(2\) the kernel of \(\mathrm{av}\) is the ordered
antisymmetric residue, equivalently the ordered \(r\)-matrix datum:
\[
\ker(\mathrm{av}_2)=\bigwedge\nolimits^2 V
\]
on a finite generator window \(V\).  In higher degree, the nontrivial
isotypic summands of the \(\Sigma_n\)-action carry the braid and
associator data; Proposition~\ref{prop:ker-av-schur-weyl} gives their
bare tensor-power dimensions, while passage to chiral-centre
cohomology is a separate theorem or conjecture in each family.
\end{proposition}
...
first non-formal input in the transferred HPL model. The
Stasheff identities organise the compatible higher operations, but
the all-arity Virasoro tower uses the wheel-residue survival
hypothesis; it is not a formal induction from the first ternary
operation alone. The gap at depth~$2$ in the Virasoro
...
$\mathrm{av}\colon\Barchord(A) \to \BarchSig(A)$, when the
strong-unitary descent datum exists, is a motivic projection in the
following precise sense: the ordered bar
...
keeps the $R$-twisted coinvariant combinations. The scalar symmetric
invariants used in this chapter (curvature~$\kappa$,
complementarity class~$C$, shadow tower~$Q$) are rational residues,
periods of the trivial motive~$\mathbb{Q}(0)$; the ordered
iterated-integral classes carrying non-trivial mixed Tate periods
are not seen by these scalar shadows.
\end{enumerate}

\noindent
The ordered surplus $\ker(\mathrm{av})$ contains the iterated-integral
classes discarded by the symmetric projection. The tridegree
decomposition stratifies this surplus by boundary depth, and this
depth filtration is compatible with the motivic weight filtration in
the mixed Tate configuration-space cases. This is the precise sense in
which the ordered bar complex is arithmetically richer than the
symmetric bar complex: it remembers multiple-zeta periods that the
scalar symmetric shadows forget.
\end{remark}

\begin{remark}[Drinfeld centre and the quantum group category]
\label{rem:drinfeld-centre-qg}
The centre appearing in the Hochschild component of this chapter is the
derived chiral centre
$Z^{\mathrm{der}}_{\mathrm{ch}}(A)$, computed by chiral Hochschild
cochains under the hypotheses of
Theorems~\ref{thm:HH-coHH-homology} and
\ref{thm:HH-coHH-cohomology}. The Drinfeld centre of a monoidal line
category is a different categorical object. For the affine
$\mathfrak{sl}_2$ evaluation surface, the conjectural comparison is
between the completed Drinfeld centre of the line category and the
corresponding quantum-group representation category, after the
Kazhdan--Lusztig completion and the half-monodromy convention
$q=e^{\pi i/(k+2)}$. That comparison is not proved here and is
not an algebraic statement about
$Z^{\mathrm{der}}_{\mathrm{ch}}(A)$. The type~$A$ construction is the
DK-5 comparison theorem; see \cite[\S6]{Lorgat26I}.
\end{remark}
...
This gives the following conditional structural picture for $\ker(\mathrm{av})$
at genus~$1$.
...
$c_1=a_{(1)}b$ is the conformal pairing. We use the raw
$\lambda$-bracket convention here; it is shifted by one pole index
from the $d\log$-absorbed collision-residue convention.
...
\toprule
\textbf{Algebra} & $c_0$ & \textbf{Genus-one class} & \textbf{Mechanism} \\
\midrule
...
\begin{question}[Central genus-$1$ entanglement]
...
is needed. For $c_0\ne 0$, the conjectural answer is no: the
$G_2$ quasi-modularity anomaly
...
$a(z)\,a'(w) \sim 1/(z{-}w)$.
\begin{enumerate}[label=\textup{(\alph*)}]
\item \emph{Collision residue.} The $d\log$ kernel absorbs the
simple OPE pole completely, so the pole-valued binary collision
residue is
\[
r^{\mathrm{coll}}_{\mathcal F}(z)=0.
\]
The regular contact operator retained by the ordered bar transfer is
...
\item \emph{Contact transport.} The collision-residue connection has
trivial monodromy, $R^{\mathrm{coll}}=\id$. If the regular contact
operator is retained as a constant nilpotent transport, its
exponential is
\begin{equation}\label{eq:bg-rmatrix}
R \;=\; \exp(2\pi i\,\Theta) \;=\; \id + 2\pi i\,\Theta,
\end{equation}
terminating at linear order because $\Theta^2 = 0$; this is contact
transport, not a pole-residue $r$-matrix.
...
every raw simple-pole contraction $\pm 1$ lies in $\bC\cdot 1$,
which the augmentation kills, and the pole-valued residue is zero
because the simple pole is absorbed by the $d\log$ kernel. Contact
transport terminates at linear order because $\Theta^2=0$. Koszul dual: the antisymmetric
...
series; the free-field collision-residue monodromy is trivial, while
the regular contact transport terminates at linear order. This
dichotomy reflects the composite-closed support-packet classification:
the Heisenberg packet is binary and scalar (class~G), whereas the
$\beta\gamma$ generator pole is simple but its closed contact envelope
is finite and nilpotent (class~C).
\end{remark}
...
satisfies, under the exactness, PBW-preservation, and BRST-lift
hypotheses of Theorem~\ref{thm:ordered-associative-ds-principal},
...
\begin{proof}
Under the stated hypotheses, $Q_{\mathrm{DS}}$ lifts to the bar
coalgebra and is compatible with the PBW filtration. The resulting
spectral sequence has
$E_1=H^\bullet(\Barch(\mathcal{M}_{\mathrm{Wak}}),d_{\Barch})$ and
$d_1=Q_{\mathrm{DS}}$. Its convergence is part of the hypothesis
package, and its abutment is the reduced bar model.
...
$\cH_k$ & $e^{k\hbar/z}$ \textup{(scalar)}
 ...
 & $\checkmark$ \\[2pt]
$\beta\gamma$ & $\id$ \textup{(contact: }$\id + 2\pi i\,\Theta$\textup{)}
 & $4$ & $\mathbf{C}$
 & $\bigwedge^*(\beta^*,\gamma^*)$
 & $\checkmark$ \\[2pt]
$bc$ & $\id$ \textup{(contact: }$\id + 2\pi i\,\Theta_{bc}$\textup{)}
 & $4$ & $\mathbf{C}$
 & $\mathrm{Sym}^*(b^*,c^*)$
 & $\checkmark$ \\[2pt]
$\widehat{\fg}_k$ & $\exp(\Omega/((k+h^\vee)z))$
...
\begin{proof}
The differential follows from the ordered lattice OPE and the ordered
bar construction: adjacent collisions in the ordered bar complex use the
...
Substituting this into the general transfer-matrix action
\[
T^a(u)\cdot v
=
v+\frac{\hbar}{u-a}\sum_\alpha \rho(J^\alpha)v\otimes J_\alpha
\]
yields
...
\begin{theorem}[Line category as finite-dimensional modules;
\ClaimStatusProvedHere]
...
\textup{(}Vol.~II comparison theorem
\ref*{thm:Koszul_dual_Yangian}\textup{)}.
...
\begin{remark}[Finite-dimensional surface, category $\mathcal O$, and RTT]
\label{rem:line-category-fd-category-o-rtt}
\index{category O@category $\mathcal O$!not identified by line-category theorem}
\index{RTT presentation!finite-dimensional surface}
Theorem~\ref{thm:line-category} is a finite-dimensional
evaluation-surface statement. It identifies the line category with
modules over the ordered Koszul dual after the finite-type continuous
dual has been taken; for affine inputs this gives
$Y_\hbar(\mathfrak g)\text{-}\mathrm{mod}^{\mathrm{fd}}$.
It does not identify the BGG/category-$\mathcal O$ category, the
Kazhdan--Lusztig WZW category, or a completed RTT representation
category. The RTT presentation is an algebra presentation of the same
Yangian on this surface, and Drinfeld polynomial classification applies
to finite-dimensional irreducible Yangian modules. Passing to
category~$\mathcal O$, admissible affine modules, or completions is
extra representation-theoretic input, not a consequence of the
ordered bar--cobar counit.
\end{remark}
...
The ordered bar complex $\Barchord(A)$ carries two layers of data:
...
$\Barchord(A)$ retains the ordering as auxiliary order data;
the descent to the symmetric bar $B^\Sigma(A)$ is the
strong-unitary $R$-matrix-twisted quotient of
Proposition~\ref*{sec:r-matrix-descent}.
...
$\Barchord(A)$ has the following invariants, where\/ $r^{\mathrm{coll}}(z)$
denotes the collision residue
\textup{(}the OPE kernel after the $d\log$ bar propagator
absorbs one pole order\textup{)}, $\Omega_{\cH}=J\otimes J$ on the
rank-one Heisenberg current window,
...
$\widehat{\mathfrak{g}}_k$\; ($\mathfrak{sl}_2$)
 & $\mathbf{L}$ & $2$
 & $k\Omega_{\mathrm{tr}}/z$\rlap{\textsuperscript{$\ddagger$}}
 & $u\,I + \hbar\,P$
 & $3$
...
$\beta\gamma$
 & $\mathbf{C}$ & $1$
 & $0$
 & $\id$ \textup{(contact: }$I + 2\pi i\,\Theta$\textup{)}
 & $4$
 & $bc$
 & yes \\[3pt]
$bc$
 & $\mathbf{C}$ & $1$
 & $0$
 & $\id$ \textup{(contact: }$I + 2\pi i\,\Theta_{bc}$\textup{)}
 & $4$
 & $\beta\gamma$
 & yes \\[3pt]
$\mathrm{Vir}_c$
...
\textsuperscript{$\ddagger$}The KZ/Yangian normalization of the same
affine monodromy datum is
$r^{\mathrm{KZ}}(z)=\Omega/((k+h^\vee)z)$.
\end{theorem}
...
double-pole $k/(z{-}w)^2$ gives
$r^{\mathrm{coll}}_{\cH}=k\Omega_{\cH}/z$, whose rank-one coefficient
is $k/z$.
For affine $\widehat{\mathfrak{g}}_k$: the trace-form Casimir
$\Omega_{\mathrm{tr}} = \sum_a t^a \otimes t_a$ produces
$r^{\mathrm{coll}} = k\Omega_{\mathrm{tr}}/z$; the KZ/Yangian
normalization rewrites the same monodromy datum as
$\Omega/((k+h^\vee)z)$ with $\Omega=2h^\vee\Omega_{\mathrm{tr}}$.
For $\beta\gamma$/$bc$: the simple-pole OPE $1/(z{-}w)$ is absorbed
completely, so $r^{\mathrm{coll}}=0$; the nilpotent operator
$\Theta$ is a regular contact class, not a pole-valued collision
residue.
...
$R(z) = \exp(k\hbar/z)$
(Volume~II, Corollary~\ref*{V2-cor:rosetta-heisenberg-projections}).
...
For $\beta\gamma$/$bc$: the collision-residue monodromy is the identity.
If the regular contact class $\Theta$ is retained as a constant
nilpotent transport, $\exp(2\pi i\,\Theta)=I+2\pi i\,\Theta$ because
$\Theta^2=0$.
...
For affine $\widehat{\mathfrak{g}}_k$: under the ordered PBW,
completion, and Drinfeld--Kohno comparison hypotheses, the ordered
line-operator companion is the Yangian $Y_\hbar(\mathfrak{g})$
(Volume~II, Theorem~\ref*{thm:Koszul_dual_Yangian}); this is not a
bare identification with the strict symmetric chiral Koszul dual.
For Virasoro and $\mathcal{W}_N$: the ordered bar-dual lies on the
completed infinite-generator surface; under the Volume~II
$\mathsf{E}_1$ hypotheses its finite windows are represented by the
dg-shifted Yangian $Y^{\mathrm{dg}}$.
...
\begin{remark}[CY-$2$ $\lbrack 2\rbrack$-shift and the
$\widetilde{M}_{24}$ anomaly cocycle on the K3 input]
\label{rem:oackd-5}
\index{CY-$2$ shift!$[2]$ on K3 input}%
\index{$\widetilde{M}_{24}$!'t Hooft anomaly}%
\index{anomaly cocycle!$H^3(M_{24},U(1))$}%
The landscape Theorem~\ref{thm:e1-ordered-bar-landscape} covers the
four standard chain-level classes $\mathbf{G}/\mathbf{L}/\mathbf{C}/\mathbf{M}$ of
genus-zero affine input. The K3 input of
Remark~\ref{rem:oackd-1} sits off the table because its carrier
is a Calabi--Yau surface rather than a curve, and two separate
refinements must be installed before the ordered bar complex can be
assembled. First, the cohomology of $K3\times E$ carries a
$[2]$-shift relative to the curve case: the CoHA inputs
$\mathrm{CoHA}_{K3\times E}$ sit in the CY-$2$ derived category
$D^b_{\mathrm{CY}\text{-}2}(K3\times E)$, so the shuffle presentation
of the realised positive Hall object acquires a cohomological shift that is
invisible on the curve lineage of this chapter. The $[2]$-shift
feeds directly into the degree assignment of the ordered bar complex:
each CoHA generator contributes two additional units of cohomological
degree, so the shadow depth $r_{\max}$ of the K3 input is shifted by
$+2$ relative to the naive affine reading. Second, the automorphic
symmetry acting on the K3 Fock lattice is not $M_{24}$ but the
projective $\widetilde{M}_{24}$ cover induced by the $H^3$ anomaly
cocycle (generalised-Mathieu-moonshine 't~Hooft anomaly,
Gaberdiel--Persson--Ronellenfitsch--Volpato 2013)
\[
[\omega]
\;\in\;
H^3\bigl(M_{24},\,U(1)\bigr)
\;\supseteq\;
\ZZ/12,
\]
and the relevant cocycle $[\omega]$ has order exactly $6$ in this
cyclic subgroup. The factor-of-$6$ is the same order that governs the
sixfold symmetry of the Igusa form $\Delta_5$ under the Fricke
involution of Remark~\ref{rem:oackd-4}, and the two are related
by the standard comparison between anomaly-twisted and untwisted
sectors of the $M_{24}$-action on the K3 Fock space. Both refinements lie outside the
reach of the landscape table above but are forced on the K3/BKM
extension; the fourth taxonomic slot of
Remark~\ref{rem:oackd-1} acquires this double structural input.
\end{remark}
...
\item Class~$\mathbf{C}$ \textup{(}Contact\textup{)}:
 ...
 The pole-valued collision-residue monodromy is the identity; the
 regular contact transport is a unipotent perturbation of the identity
 ($\Theta^2 = 0$ forces $R_{\mathrm{contact}} = I + 2\pi i\,\Theta$).
...
The affine ordered datum and the principal DS reduction land in the
Virasoro/W shadow tower. This does not turn the ordinary Drinfeld
Yangian $Y_\hbar(\mathfrak g)$ into a Yangian of~$\mathrm{Vir}_c$.
A dg-shifted Virasoro/W object, if used, is a separate ordered
derived construction with its own generators and differential.
...
\index{CoHA!positive half and $\cW_{1+\infty}$ envelope}
The $\cW$-algebra $\cW_{1+\infty}[\Psi]$
(Schiffmann--Vasserot, Maulik--Okounkov) is a class~$\mathbf{M}$
algebra whose Hall origin passes through three typed steps: the
Jordan-quiver CoHA is the positive half
\(Y^+(\widehat{\mathfrak{gl}}_1)\), the Drinfeld double gives the full
affine Yangian, and the Miura/vacuum evaluation gives the
\(\cW_{1+\infty}[\Psi]\) vertex algebra. This chain provides all three
structures of the chiral bialgebra equivalence
...
descent map only after the strong-unitary $R$-twisted
$\Sigma_n$-datum has been fixed
(Proposition~\ref{prop:symmetric-descent}(ii)). The descended
invariant complex is the Beilinson--Drinfeld symmetric bar. The raw
ordered complex may still contain a non-invariant kernel; no
quasi-isomorphism is asserted unless that kernel is acyclic. In the
pole-free case this reduces to the ordinary Reynolds/coinvariant
descent of Proposition~\ref{prop:symmetric-descent}(i).

For genuinely $\Eone$-chiral algebras (tier~(c): the Yangian
$Y_\hbar(\fg)$, Etingof--Kazhdan quantum vertex algebras),
there is no symmetric bar without extra descent data. The
$R$-matrix is independent input, the factorisation $\cD$-module
$\cF_n^{\mathrm{ord}}$ is not automatically $\Sigma_n$-equivariant,
and the ordered theory retains braid-monodromic data. In KZ-type
examples the first associator class
$\Phi_{\mathrm{KZ}} \in \exp(\hat{\mathfrak{t}}_3)$ occurs at
three points and carries $\mathrm{GRT}_1$-dependent information; a
chosen symmetric descent projects to the invariant shadow and
forgets the non-invariant representative. The Yangian evaluation
surface is therefore a first test case for genuinely ordered data, not
a proof that all ordered kernels are nonzero in every degree.
\end{remark}
...
\section{The chiral bialgebra equivalence on the ordered Koszul locus}
...
is a single object from which three readouts are recovered, after a
Drinfeld associator and an $R$-twisted descent gauge have been fixed: the
vertex $R$-matrix $S(z)$ (regularised KZ holonomy of the pole-valued
collision residue, together with the regular contact class in
class~$\mathbf{C}$), the chiral $A_\infty$-structure maps
$m_k^{\mathrm{ch}}$ (higher associativity from boundary strata of
associahedra), and the chiral coproduct $\Delta^{\mathrm{ch}}$
(coalgebra data from deconcatenation transported through the
bar--cobar counit). Without these fixed gauges, the ordered bar
determines a braid-monodromic local system, not a symmetric chiral
bialgebra structure.
...
\begin{definition}[Ordered Koszul locus; \ClaimStatusDefinitional]
\label{def:ordered-koszul-locus}
\index{Koszul locus!ordered $\Eone$-chiral|textbf}
\index{ordered bar-cobar counit|textbf}
Let \(\operatorname{ChirAlg}^{\Eone,\mathrm{ft,aug}}_X\)
denote the chosen finite-type augmented presentation of
\(\Eone\)-chiral algebras on~\(X\). The \emph{ordered Koszul
locus}
\[
\operatorname{Kosz}^{\mathrm{ord}}_{\Eone}(X)
\;\subset\;
\operatorname{ChirAlg}^{\Eone,\mathrm{ft,aug}}_X
\]
is the full subcategory of those \(\cA\) for which the ordered
bar-cobar counit
\[
\epsilon^{\mathrm{ord}}_{\cA}\colon
\Omega(\barB^{\mathrm{ord}}(\cA))
\longrightarrow
\cA
\]
is a quasi-isomorphism in the chosen complete \(\Eone\)-chiral
category. This is the ordered condition used in this section.
It is not the symmetric Koszul locus
\(\operatorname{Kosz}(X)\) of
Definition~\textup{\ref{def:koszul-locus}}, whose bar object is the
\(\Sigma_\bullet\)-equivariant Ran bar. If an \(\Einf\)-chiral algebra
lies in \(\operatorname{Kosz}(X)\) and the ordered forgetful
presentation preserves the bar-cobar counit, then its ordered
underlying \(\Eone\)-chiral algebra lies in
\(\operatorname{Kosz}^{\mathrm{ord}}_{\Eone}(X)\); genuinely
\(\Eone\)-chiral inputs must satisfy the ordered counit condition
directly.
\end{definition}

\begin{theorem}[Chiral bialgebra equivalence on the ordered Koszul locus]
\label{thm:chiral-qg-equiv}
\ClaimStatusConditional
\index{chiral bialgebra!three equivalent descriptions}
Let $\cA$ be a finite-type strongly admissible $\Eone$-chiral
algebra on the ordered Koszul locus
\(\operatorname{Kosz}^{\mathrm{ord}}_{\Eone}(X)\). Fix a Drinfeld associator
$\Phi \in \exp(\hat{\mathfrak{t}}_3)$ and a strong-unitary
$R$-twisted descent datum as in
Proposition~\textup{\ref{lem:R-twisted-descent}}. Then the following
three compatible structures are equivalent descriptions of the same
...
\textup{(}the chiral Drinfeld formula\textup{)}. The statement is an
ordered Koszul-locus statement: the ordered bar--cobar counit
$\Omega(\barB^{\mathrm{ord}}(\cA))\to\cA$ and the chosen
associator/descent gauges are part of the data.
\end{theorem}
...
(the ordered Koszul-locus condition of
Definition~\ref{def:ordered-koszul-locus}).
...
$S(z) = \sigma \circ \Delta^{\mathrm{ch}}
\circ (\Delta^{\mathrm{ch},\mathrm{op}})^{-1}$
defines the vertex $R$-matrix once the Drinfeld--Kohno or
Etingof--Kazhdan quantization datum has been supplied: source Lie
bialgebra, \(\hbar\)-adic/pro-nilpotent topology, associator, and
QUE/quasi-Hopf target.  The
pentagon for~$\Phi$ gives the braid coherence; the QYBE is the
hexagon consequence for the induced braiding.  Coassociativity alone
recovers the classical CYBE at the infinitesimal residue level and
does not quantize it.
\end{proof}
...
canonical one. The invariant actually used below is narrower: for the
verified $\mathfrak{sl}_2$ and affine Kac--Moody checks, the
bar-side $\mathrm{H}^0$ shadow \textup{(}$\kappa$, the shadow tower,
and the cohomology class of the ordered Maurer--Cartan datum
\textup{)} is unchanged after replacing $\Phi$ by a
$\mathrm{GRT}_1(\bQ)$-transport. For a general simple $\fg$ this
associator-independence is a comparison hypothesis in the ordered
Koszul/descent package, not a formal consequence of the
$\mathrm{GRT}_1$ action. It does not prove
$\mathrm{GRT}_1$-triviality of categorical modular data, the
Drinfeld centre, the line category, or any root-of-unity BKM
$S$-matrix. At the cochain level, different associator
representatives produce coproducts related by the supplied gauge
transformation inside the chosen descent datum; the representatives
are homotopic in that datum but are not equal. The statement of
Theorem~\textup{\ref{thm:chiral-qg-equiv}} is therefore canonical
only after passing to the specified $\mathrm{H}^0$ bar-shadow quotient
and remains non-canonical up to $\mathrm{GRT}_1(\bQ)$ at the
cochain/coproduct level. Throughout the chapter a stronger
chain-level claim requires fixing a specific associator class.
\end{remark}

\begin{remark}[Automorphic normalization and the compact Hall
recognition problem; Open]
\label{rem:oackd-4}
\index{Igusa form!compact Hall recognition problem}%
\index{automorphic kernel!Siegel genus two}%
The established automorphic input is the Borcherds product
normalisation.  In theta normalisation the weight-$5$ form
$\Delta_5$ has leading coefficient $64$; the monic product is
...
\]
The coefficient $64$ and the equality $64^2=4096$ establish only the
scalar automorphic normalization.

The compact Hall statement requires a separately defined deformation
complex for the completed Hall--Drinfeld double, together with the
Fricke action, orientation data, and a comparison map from its
cohomology to the Borcherds-product line.  The recognition target is a
one-dimensional image generated by $\Delta_5$.  Establishing that
target, its injectivity range, and its relation to the Mukai integer
$2c_+=8$ is an open theorem obligation.  This formulation keeps the
automorphic denominator, the Hall object, and the chiral conductor as
three typed layers joined by explicit comparison maps.
\end{remark}
...
T^c(s^{-1}\bar\cA)$, together with the chosen associator and
strong-unitary descent gauge of
Theorem~\textup{\ref{thm:chiral-qg-equiv}}, is the universal
chain-level datum: the vertex
$R$-matrix is the regularised holonomy of the degree-$2$
KZ-normalized collision datum
$r^{\mathrm{KZ}}(z)$ \textup{(}and, in class~$\mathbf{C}$, of the
regular contact operator left after the pole-valued residue
vanishes\textup{)},
the $A_\infty$-maps are the bar differential components
$d_{\barB}|_{\text{degree } k}$, and the chiral coproduct is
the cobar-dual of the deconcatenation. No additional operation on
$\cA$ is needed; without the associator/descent gauge the same bar
coalgebra determines only the ordered braid-monodromic object.
\end{corollary}

\begin{proof}
Theorem~\ref{thm:chiral-qg-equiv} provides three readouts of the same
ordered bar coalgebra.
Arrow $\alpha$ identifies the degree-$2$ KZ-normalized collision
datum, after regularised holonomy and the chosen descent gauge, with
the vertex $R$-matrix.
...
$\barB^{\mathrm{ord}}(\cA)$ once the associator and descent gauge have
been fixed, no further product or coproduct on $\cA$ is required.
\end{proof}
...
% ================================================================
\begin{example}[Etingof--Kazhdan quantum VOA as a chiral bialgebra]
\label{ex:ek-quantum-voa-chiral-bialgebra}
...
simultaneously on a single algebra satisfying all axioms.
The ordered chiral center carries the full quantum $R$-matrix
$S(z)$, together with the classical $r$-matrix
$r^{\mathrm{KZ}}(z)=\Omega/((k+h^\vee)z)$), and the kernel
...
\begin{theorem}[$\cW_{1+\infty}\lbrack\Psi\rbrack$ as a chiral quantum
group]
\label{thm:w-infty-chiral-qg}
\ClaimStatusConditional
In its completed positive-energy/Yangian presentation, the algebra
$\cW_{1+\infty}[\Psi]$ carries a chiral quantum group datum in the sense of
Theorem~\textup{\ref{thm:chiral-qg-equiv}}: an $R$-matrix,
a chiral coproduct, and an $\Ainf$ structure, all explicit
and obtained from the Jordan-quiver CoHA only after the positive-half
and double/evaluation arrows are crossed:
\[
\cH_{\mathrm{Jor}}\cong Y^+(\widehat{\mathfrak{gl}}_1)
\hookrightarrow
D(\cH_{\mathrm{Jor}})\cong Y(\widehat{\mathfrak{gl}}_1)
\longrightarrow U(\cW_{1+\infty}[\Psi]).
\]
The CoHA itself is the positive half, not
\(\cW_{1+\infty}\). The raw direct-sum class~$\mathsf M$
bar complex is not the ambient of this theorem. The precise content is as follows.
\begin{enumerate}[label=\textup{(\Roman*)}]
\item \textup{($R$-matrix.)}
 The Maulik--Okounkov $R$-matrix~\cite{MO19} of quantum
 toroidal $\mathfrak{gl}_1$, restricted to the positive-half CoHA, is
 \emph{scalar}:
 ...
 classical limit is
 % r-matrix with level prefix Psi; Psi=0 -> r=0.
...
\end{enumerate}
With this $R$-matrix and the fixed associator/descent gauge,
Theorem~\textup{\ref{thm:chiral-qg-equiv}} identifies the three
structures as the same ordered-bar datum on the ordered Koszul locus. The OPE
compatibility axiom~\eqref{eq:ope-compat} is satisfied at
all spins simultaneously by the coderivation property on the
completed ordered Koszul-locus bar complex and, on the positive-half CoHA
comparison surface, by the JKL vertex bialgebra theorem.
\end{theorem}
...
$\cH_{\mathrm{Jor}} \cong Y^+(\widehat{\mathfrak{gl}}_1)$
by matching generators and relations. This is the positive half only;
the \(\cW_{1+\infty}\) vertex algebra appears after Drinfeld doubling
and Miura/vacuum evaluation. The critical
...
as an identity of vertex operators. This is stronger than
an identity in the underlying associative algebra. We give two independent
proofs that this holds at all spins simultaneously.

\smallskip
\noindent\emph{Argument~A \textup{(}coderivation on the
ordered Koszul locus\textup{)}.}
The algebra $\cW_{1+\infty}[\Psi]$ at generic~$\Psi$ lies
on the ordered Koszul locus
\textup{(}Definition~\textup{\ref{def:ordered-koszul-locus}}:
...
\begin{equation}\label{eq:coderivation-w-infty}
 ...
 =
 (d_{\Barord}\otimes\id+\id\otimes d_{\Barord})\circ\Delta .
\end{equation}
...
coassociativity and the counit axiom. On the ordered Koszul locus,
...
The structures (I)--(III) are the three readouts of
Theorem~\ref{thm:chiral-qg-equiv}: the $R$-matrix is
given by step~5, the coproduct by steps~4 and~6, and the
$\Ainf$ structure is extracted, after fixing the same gauge, from
...
\leftrightarrow \textup{(III)}$ has a second comparison
surface for algebras arising as critical CoHAs of quivers with
...
complex, one exhibits the CoHA-side $R$-matrix and vertex
coproduct on a single geometric object and then asks which
double, completion, and vertex-algebra evaluation identify that
object with the required full Yangian or $\cW$-algebra target.
...
double~\cite{YZ18a}. This establishes the CoHA-side
instance of $\textup{(I)}$ and $\textup{(III)}$: the
$R$-matrix and the coproduct coexist on $\cH_{Q,W}$ and
satisfy the quasi-triangularity relation
$R(z)\, \Delta^{\mathrm{ch}} = \Delta^{\mathrm{ch},\op}\,
R(z)$. It is not, by itself, an identification with a full
Yangian or finite $\cW_N$ vertex algebra.
...
\smallskip
\noindent\textbf{Step~3: the comparison triangle on the CoHA surface.}
By the JKL vertex bialgebra theorem, the CoHA
simultaneously carries the CoHA analogues of (I), (II), and
(III). To identify this with the triangle of
Theorem~\ref{thm:chiral-qg-equiv}, one must add the
Drinfeld-double, completion, and vertex-algebra evaluation maps
that carry the positive-half Hall object to the chosen target.
On the Koszul
locus:
\begin{enumerate}[label=\textup{(\roman*)}]
\item \emph{(I) determines (III):} given an $R$-matrix
 ...
 when the Drinfeld reconstruction hypotheses are imposed.
...
$\alpha \circ \gamma \circ \beta = \id$ then follows on the
target surface from the uniqueness of each vertex, not from the
positive-half CoHA statement alone.
...
\textup{(III)}$ is checked on the geometric CoHA
construction (stable envelopes for the $R$-matrix,
Levi restriction for the coproduct, JKL for the vertex
bialgebra axiom), before any passage through the ordered bar complex
...
bar-cobar adjunction on the ordered Koszul locus. The functor
...
\smallskip
\noindent\textbf{CoHA domain.}
The CoHA proof applies to associative Hall algebras
arising as critical CoHAs of quivers with potential; the chiral object
appears only after the comparison to a vertex-algebra target.
...
structure on $\cW_{1+\infty}[\Psi]$ only after the
Drinfeld double and Miura/vacuum evaluation
...
Yangian~\cite{JKL26} on that positive-half comparison
surface. This is a quiver-origin statement; it is not the
rank-$N$ Jordan-quiver substitute for the
$\mathfrak{gl}_N$ positive half. Beyond quiver-origin
algebras, and before the double/completion/evaluation arrows
are supplied, the CoHA argument does not directly apply; the
bar-cobar proof of Theorem~\ref{thm:chiral-qg-equiv} remains
the primary route for general $\Eone$-chiral algebras on the
...
\begin{remark}[GRZ comparison; \ClaimStatusConditional]
\label{rem:grz-comparison}
Theorem~\ref{thm:w-infty-chiral-qg} has three comparison surfaces
in the type-$A$ literature. The cited type-$A$ results are external;
the identification with the ordered $\cW_{1+\infty}$ chiral quantum
group of this manuscript is conditional on the hypothesis package of
Theorem~\ref{thm:w-infty-chiral-qg}.
...
For quiver-origin algebras there is a second comparison route.
...
coproduct is the positive-half comparison surface. Identifying it
with the ordered $\Eone$-chiral coproduct on a full Yangian or
finite $\cW_N$ target still requires the Drinfeld double,
completion, and Miura/Drinfeld--Sokolov evaluation hypotheses.
...
\smallskip
\noindent\emph{Spectral-parameter firewall.}
This convention does not license erasure of spectral parameters
from associative Yangians or from topological Drinfeld-centre
language. The associative Yangian $Y(\mathfrak g)$ has evaluation modules
$V(u)$, and the universal $R$-matrix on
$V(u)\otimes W(v)$ gives the rational matrix $R_{V,W}(u-v)$
\cite[Ch.~12]{ChariPressley94}. What is independent of the
collision coordinate is the primitive topological bar
deconcatenation coproduct before the evaluation-module shift is
chosen. The chiral bar differential is different: it is
meromorphic in the Ran collision coordinate, and this
meromorphic dependence is the datum carried by the ordered
factorisation reconstruction.
\end{remark}

\begin{remark}[RTT, CoHA, and coproduct compatibility]
...
The construction in step~6 closes this gap by the coderivation
property on the ordered Koszul-locus bar complex; the JKL vertex
bialgebra theorem gives the matching comparison on the positive-half
CoHA surface.

\smallskip
\noindent\emph{(2) Truncation to $\cW_N$.}
Theorem~\ref{thm:glN-chiral-qg} resolves this question as a
conditional Koszul-locus statement: for the truncations
$\cW_N$ at finite $N \geq 2$, the
...
is constructed by the coderivation argument
(argument~A). The CoHA route (argument~B) gives an independent
comparison only after a quiver-origin positive-half CoHA has
been identified with the required $Y^+$, then transported
through the Drinfeld double, completion, and
Miura/Drinfeld--Sokolov evaluation. JKL's general-quiver vertex
bialgebra theorem does not by itself produce a rank-$N$
Jordan-quiver identification with
$Y^+(\widehat{\mathfrak{gl}}_N)$.

\smallskip
\noindent\emph{(3) Antipode and Drinfeld double.}
The Schiffmann--Mellit--Minets--Vasserot
isomorphism~\cite{SMMV23} identifies the Jordan-quiver CoHA with the
positive half of the affine Yangian,
$\cH_{\mathrm{Jor}} \cong Y^+(\widehat{\mathfrak{gl}}_1)$; the
\(\cW_{1+\infty}[\Psi]\) fields appear only after Drinfeld doubling and
Miura/vacuum evaluation.
The Drinfeld double
...
\begin{lemma}[Miura triangularity under the Drinfeld coproduct]
\label{lem:miura-triangularity-under-Delta}
\ClaimStatusProvedHere
Work in the Open quadrant, in the ordered associative
chiral/Yangian presentation, in the Beilinson level
$4 \to 5$ operator-to-coproduct lane.  Assume the generic
$\Psi$ package consisting of the
Proch\'azka--Rap\v{c}\'ak triangular Miura basis
\[
 \psi_n
 \;=\;
 W_n + P_n(J,W_2,\ldots,W_{n-1})
 \qquad (P_n \in F^{W}_{\leq n-1})
\]
and the Drinfeld spectral coproduct
$\Delta_z(T(u)) = T(u)\otimes T(u-z)$.  Let
$F^{W}_{\leq p}$ be the normally ordered filtration by
largest primary $W$-spin appearing in a monomial, with
$J=W_1$ and the unit in $F^{W}_{\leq0}$.  Then
\[
 \Delta_z(F^{W}_{\leq p})
 \;\subset\;
 F^{W}_{\leq p}\,\widehat{\otimes}\,F^{W}_{\leq p}.
\]
Consequently, for $s\geq3$, every lower Miura monomial of
$W$-spin $\leq s-2$ in the inversion of $\psi_s$ has
spectral coproduct with no component in
$J\otimes W_{s-1}$ or $W_{s-1}\otimes J$.
\end{lemma}

\begin{proof}
The triangular Miura theorem gives the associated-graded
identity $\operatorname{gr}^{W}\psi_n=W_n$.  Its inverse is
therefore also triangular:
$W_n=\psi_n+Q_n(\psi_1,\ldots,\psi_{n-1})$, with
$Q_n$ of $W$-spin at most $n-1$.  The Drinfeld formula
for the $u^{-n}$ coefficient,
\[
 \Delta_z(\psi_n)
 =
 \psi_n\otimes1
 + \sum_{k=0}^{n-1}\sum_{m=1}^{n-k}
   \binom{n-k-1}{m-1}z^{\,n-k-m}\,
   \psi_k\otimes\psi_m,
\]
has tensor indices $k,m\leq n$.  After triangular expansion
from the $\psi$-basis to the primary basis, each tensor factor
has $W$-spin at most $n$.  Hence
$\Delta_z(\psi_n)\in
F^{W}_{\leq n}\widehat{\otimes}F^{W}_{\leq n}$, and the
same statement for $W_n$ follows from the triangular inverse
and multiplicativity of~$\Delta_z$.

The filtration is defined by the largest primary spin in a
normal monomial.  Multiplication by powers of
$J=W_1$ does not increase that largest spin, and normal-ordering
contractions with $J$ produce derivatives or lower composites,
not a new primary $W_r$ with larger $r$.  Thus, for a
representative lower sector
$M_{s,k}={:}J^kW_{s-k}{:}$ with $k\geq2$,
\[
 \Delta_z(M_{s,k})
 \in
 \sum_{a=0}^{k}
 ...
 {:}J^{k-a}F^{W}_{\leq s-k}{:},
\]
up to derivative and lower normal-ordering terms in the same
filtration.  Since $s-k\leq s-2$, no tensor factor can contain
the target primary $W_{s-1}$.

The first nontrivial displayed case is spin~$3$.  The composite
term in Proposition~\ref{prop:spin3-miura-coprod} is
\[
 \frac{1-\Psi}{2\Psi^2}
 \bigl(J\otimes{:}J^2{:}+{:}J^2{:}\otimes J\bigr),
\]
which has $W$-spin $1$ on both sides and therefore cannot
contribute to the $J\otimes T$ or $T\otimes J$ channel.
At spin~$4$, the lower representatives
${:}J^2T{:}$ and ${:}TT{:}$ remain in
$F^{W}_{\leq2}$ after applying~$\Delta_z$, so they cannot
produce $J\otimes W_3$.  At spins~$5$ and~$6$, the maximal
lower primary spins are respectively $3$ and~$4$, below the
targets $W_4$ and $W_5$.  This proves the required
non-contribution of all lower sectors.
\end{proof}
...
\begin{proof}
The case $s=2$ is Lemma~\ref{lem:coprod-T-miura}.  For
$s\geq3$ the spin-uniform argument has three steps.
...
\item From the Miura sectors with $k \geq 2$
 ...
 By Lemma~\ref{lem:miura-triangularity-under-Delta},
 the tensor support of these sectors is contained in
 $W$-spin $\leq s - 2$ on each side after applying
 $\Delta_z$.  They therefore contribute zero to the
...
is tautological, the single-slot Miura factor $1/\Psi$ is
...
Expanding $\prod_i(\sigma_i+J_i/\Psi)$ in the rational function
field $\bQ(\Psi)$ gives the same coefficient at spins $2\leq s\leq6$;
the proof above is spin-uniform and does not rely on this finite range.
\end{proof}

\begin{remark}[Primary cross-term only]
\label{rem:miura-primary-vs-composite-scope-monograph}
Theorem~\textup{\ref{thm:miura-cross-universality-monograph}}
asserts the universal coefficient $(\Psi - 1)/\Psi$ only on the
\emph{primary} cross-term $J \otimes W_{s-1} + W_{s-1} \otimes J$.
The composite cross-terms, for example
$J \otimes {:}J \cdot W_{s-2}{:}$,
${:}J^2{:} \otimes W_{s-2}$, and the higher $J$-insertion sectors,
carry spin-dependent coefficients. At spin~$3$ the unique
composite correction
$(1 - \Psi)/(2\Psi^2)$ on $J \otimes {:}J^2{:} + {:}J^2{:} \otimes J$
is established in
Proposition~\textup{\ref{prop:spin3-miura-coprod}} above.
For $s \geq 4$ no closed form for the composite corrections is
established in this volume.
\end{remark}

\begin{remark}[Attribution for the triangular Miura basis]
\label{rem:miura-triangularity-attribution-monograph}
The identification used in Step~$1$ of the proof,
namely that the $(s-1)$-slot primary block of the single-$J$
Miura sector equals $W_{s-1}$ modulo lower-spin composites,
is the general-$s$ form of the triangular change of basis
$\psi_n = W_n + P_n(J, W_2, \ldots, W_{n-1})$ of
Proch\'azka--Rap\v{c}\'ak~\cite{PR19}; see also
Step~$3$ of the proof of
Theorem~\textup{\ref{thm:w-infty-chiral-qg}} for the statement
at the level of the filtered isomorphism
$Y(\widehat{\mathfrak{gl}}_1) \cong U(\cW_{1+\infty}[\Psi])$.
The triangular structure is explicit through $\psi_3$ in this
volume; the general-$s$ statement is cited from~\cite{PR19}
rather than independently derived here.  Lemma~
\ref{lem:miura-triangularity-under-Delta} separates this imported
triangular basis from the filtration consequence used in the
coproduct proof: once the triangular basis and Drinfeld coproduct
are assumed, preservation of $W$-spin under~$\Delta_z$ is proved in
this volume.
\end{remark}
...
\textup{(}no compact curves\textup{)}, this identifies the scalar
constant-map sector in the MNOP normalisation; it is not a chain-level
or physical Gromov--Witten partition function without the corresponding
comparison datum.
\end{remark}
...
% level prefix Psi mandatory; Psi=0 -> R = uI_4 (trivial).
...
follows by direct $4 \times 4$ multiplication.
...
via Theorem~\ref{thm:miura-cross-universality-monograph}\textup{)} does
...
\textup{(}Theorem~\ref{thm:miura-cross-universality-monograph}\textup{)}.
...
therefore requires either the Koszul-locus bar-cobar
construction of Argument~$A$ or an external positive-half CoHA
comparison. Jindal--Kaubrys--Latyntsev supply the vertex
bialgebra axiom on critical CoHAs~\textup{\cite{JKL26}}; it
becomes input for $\mathcal{W}_N$ only after a type-$A$
positive-half CoHA is identified with the appropriate
$Y^+(\widehat{\mathfrak{gl}}_N)$ and then transported through
Drinfeld doubling, completion, and
Miura/Drinfeld--Sokolov evaluation. The
Schiffmann--Vasserot Jordan-quiver identification gives
$\cH_{\mathrm{Jor}}\cong Y^+(\widehat{\mathfrak{gl}}_1)$; it
is not a rank-$N$ substitute.
...
\begin{remark}[Structural mirror: the Miura cross-term and the
Feigin--Frenkel screening obstruction share a single
cohomological invariant]
\label{rem:miura-ff-screening-structural-mirror}
The coefficient $(\Psi - 1)/\Psi$ appears twice in this chapter,
once as the universal cross-term prefactor of
Theorem~\ref{thm:miura-cross-universality-monograph} and once as
the residue class of the chiral $1$-cocycle $R_i(z)$ in
Proposition~\ref{prop:ff-screening-coproduct-obstruction}. The
two derivations are formally independent. The first assembles
from the Drinfeld binomial $\binom{s-2}{s-2} = 1$ and the Miura
inversion coefficient $-1/\Psi$; the second from the
exponential-field contraction weight $\alpha_i^2 \log z
= (2/\Psi)\log z$ and its regularised residue on the Heisenberg
tensor square. Both outputs are $1 - 1/\Psi$. The equality is
not a normalisation accident.

In both derivations the primitive contribution $+1$ records the
Heisenberg parent: the trivial chiral coproduct
$\Delta_z^{\mathfrak{h}}(J_i) = J_i \otimes 1 + 1 \otimes J_i$
in the screening picture, the Drinfeld $z^0$ monomial
$\psi_1 \otimes \psi_{s-1}$ in the Miura picture. In both the
subtraction $-1/\Psi$ records the Miura nonlinearity: the single
$J/\Psi$ slot in $\Lambda_i = \sigma_i + J_i/\Psi$ on one side,
the simple-root length $\alpha_i^2 = 2/\Psi$ on the other. The
two presentations access the same deformation of the Heisenberg
parent, read off along complementary coordinates of the
$\mathcal{W}_N$ moduli of chiral coproducts.

A structural statement follows.

\begin{proposition}[Miura--Feigin--Frenkel structural identification]
\label{prop:miura-ff-screening-identification}
\ClaimStatusConditional
Let $\mathfrak{h}$ be the rank-$(N-1)$ Heisenberg vertex algebra
with simple-root screening charges
$Q_{\alpha_1}, \ldots, Q_{\alpha_{N-1}}$ normalised by
$\alpha_i^2 = 2/\Psi$, and let $\mathcal{W}_N
= \bigcap_i \ker Q_{\alpha_i}$ be the Feigin--Frenkel
presentation. For each spin $s \geq 2$ and each simple root
$\alpha_i$, the universal cross-term coefficient of
Theorem~\textup{\ref{thm:miura-cross-universality-monograph}}
on the generator $W_s \in \mathcal{W}_N$ and the residue class
of the chiral $1$-cocycle $R_i(z) \in H^1_{\mathrm{ch}}(\mathfrak{h},
\mathfrak{h} \otimes \mathfrak{h}[z, z^{-1}])$ of
Proposition~\textup{\ref{prop:ff-screening-coproduct-obstruction}}
are equal in $\mathbb{Q}(\Psi)$:
\begin{equation}\label{eq:miura-ff-identification}
 \bigl[\text{cross-term coefficient on } J \otimes W_{s-1}
 + W_{s-1} \otimes J\bigr]
 \;=\;
 \bigl[R_i(z)\bigr]_{\text{primary}}
 \;=\;
 \frac{\Psi - 1}{\Psi}.
\end{equation}
Both rational functions extract the primitive $+1$ from the
Heisenberg parent and the subtraction $-1/\Psi$ from the Miura,
respectively screening, nonlinearity.
\end{proposition}

\begin{proof}
Theorem~\ref{thm:miura-cross-universality-monograph} gives the
left-hand coefficient as $1 - 1/\Psi$. For the right-hand
coefficient, the proof of
Proposition~\ref{prop:ff-screening-coproduct-obstruction}
computes the spectral-residue prefactor of $R_i(z)$ by
contour-integrating the cross-term $z^{\alpha_i^2} - 1
= z^{2/\Psi} - 1$ against the normal-ordered exponential tensor
product. The leading coefficient of the $z \to 0$ expansion of
$z^{2/\Psi} - 1$ is $(2/\Psi)\log z$; after the contour
integration extracts the simple-pole residue and the
normal-ordering correction reduces the universal factor
$2/\Psi$ to its additive complement, the residue class is
represented by $(\Psi - 1)/\Psi$ on the joint screening kernel.
Equality follows.

Step~1 of Theorem~\ref{thm:miura-cross-universality-monograph}
and the exponential-contraction computation in
Proposition~\ref{prop:ff-screening-coproduct-obstruction} are
both arithmetic consequences of the same input, the simple-root
length $\alpha_i^2 = 2/\Psi$: the Miura slot $J_i/\Psi$ in
$\Lambda_i = \sigma_i + J_i/\Psi$ is the semiclassical shadow
of the quantum screening $e^{\alpha_i \cdot \varphi}$, and the
single-slot factor $1/\Psi$ appearing in both places is the same datum
read along two complementary coordinates.
\end{proof}

The identification yields a sharp falsification test. Any future
construction of a chiral coproduct on $\mathcal{W}_N$ descending
from a Heisenberg-type parent with screening charges of length
$\alpha^2 = \kappa$ must produce, on the primary cross-term
$J \otimes W_{s-1} + W_{s-1} \otimes J$, the coefficient
$1 - \kappa/2$ in the Miura normalisation $\Psi = 2/\kappa$. A
deviation from $1 - 1/\Psi$ at any spin $s \geq 2$ invalidates
the parent, the screening, or the descent, whichever fails
first. Direct expansion of the Miura product gives the stated
coefficient through $s = 6$; the Feigin--Frenkel side is
established symbolically for all $N \geq 2$ in the proof of
Proposition~\ref{prop:ff-screening-coproduct-obstruction}.

Two consequences follow. First, the universality at
$N \geq 3$ is not a coincidence between an algebraic
construction and a geometric obstruction: the Heisenberg parent
has exactly one $\Psi$-deformation of its trivial coproduct, of
rank one in $H^1_{\mathrm{ch}}(\mathfrak{h}, \mathfrak{h}
\otimes \mathfrak{h}[z, z^{-1}])$ at the simple-root-length
locus, and the Drinfeld coproduct of
$Y(\widehat{\mathfrak{gl}}_N)$ is the unique representative of
the class compatible with the Miura factorisation. Second, the
obstruction structure is $s$-independent: the rank of
$H^1_{\mathrm{ch}}$ in the target cohomology does not grow with
spin, which is the cohomological face of the universality
asserted by
Theorem~\ref{thm:miura-cross-universality-monograph}.
\end{remark}

\begin{theorem}[$Y(\widehat{\mathfrak{gl}}_N)$ chiral
bialgebra for $N \geq 2$: bar-cobar construction and
conditional CoHA comparison]
\label{thm:glN-chiral-qg}
\ClaimStatusConditional
For each $N \geq 2$ and generic $\Psi$, subject to the
Koszul-locus and fixed-gauge hypotheses of
Theorem~\textup{\ref{thm:chiral-qg-equiv}}, the truncation
...
OPE compatibility.
At $N = 2$ the rank-two Yangian, RTT, quantum determinant, and
Virasoro restriction are computed explicitly in
Example~\ref{ex:gl2-chiral-qg}. At $N \geq 3$ the Drinfeld
...
Theorem~\ref{thm:miura-cross-universality-monograph}. The
construction used below is the ordered bar-cobar construction
on the ordered Koszul locus. The CoHA route gives a second comparison
only under an additional positive-half package: a type-$A$
critical CoHA with the Jindal--Kaubrys--Latyntsev vertex
coproduct~\textup{\cite{JKL26}}, an identification with the
required $Y^+$, Drinfeld double and completion, and
Miura/Drinfeld--Sokolov evaluation to $\cW_N$. The
Schiffmann--Vasserot Jordan-quiver theorem supplies the
$\mathfrak{gl}_1$ positive half~\textup{\cite{SV13}}, not a
rank-$N$ Jordan-quiver proof. The Hopf-antipode lift is a proved negative
...
\item \textup{(Yang's rational $R$-matrix.)}
 ...
 % level prefix Psi mandatory; Psi=0 -> R = uI (trivial).
 R(u)
 = u\, I_{N^2} + \Psi\, P
...
\textup{(}computed numerically at $N = 2, 3$\textup{)}.
...
on~$\CC^N$ \textup{(}computed numerically at
 $N = 2, 3$\textup{)}.
...
$\Ainf$\textup{)} are the three gauge-fixed readouts of the
ordered-bar datum in
Theorem~\textup{\ref{thm:chiral-qg-equiv}}.
The OPE compatibility axiom~\eqref{eq:ope-compat}
is constructed at all spins $1, \ldots, N$ by the
Koszul-locus bar-cobar argument, with the CoHA route serving
as a conditional comparison surface:
\begin{enumerate}[label=\textup{(\alph*)}]
\item \emph{Argument~$A$ \textup{(}coderivation on the
 ordered Koszul locus\textup{)}.}
 At generic~$\Psi$, the algebra $\cW_N[\Psi]$ lies on the
 ordered Koszul locus
 \textup{(}Definition~\textup{\ref{def:ordered-koszul-locus}}\textup{)}.
The bar-cobar adjunction
...
compatibility~\eqref{eq:ope-compat} at all spins
 ...
 Uniqueness on the ordered Koszul locus identifies this bar-cobar
 coproduct with the Drinfeld
 coproduct~\eqref{eq:glN-drinfeld-coprod}.
\item \emph{Argument~$B$ \textup{(}conditional CoHA comparison\textup{)}.}
Suppose a type-$A$ positive-half critical CoHA has been
identified with the required
$Y^+(\widehat{\mathfrak{gl}}_N)$ and that the Drinfeld double,
completion, and Miura/Drinfeld--Sokolov evaluation maps are fixed.
On that comparison surface, the vertex bialgebra axiom of
Jindal--Kaubrys--Latyntsev~\textup{\cite{JKL26}} transports to the
positive half and is compatible with the Drinfeld coproduct on the
full Yangian. This is a conditional comparison, not an unconditional
rank-$N$ Jordan-quiver proof. It explains the non-Heisenberg input
at $N \geq 3$: a Feigin--Frenkel
screening alternative (realising $\cW_N$ as an intersection
 ...
 \textup{(}Theorem~\textup{\ref{thm:miura-cross-universality-monograph}}\textup{)},
 ...
 intersection. The CoHA route supplies such a construction only
 after the positive-half identification and double/evaluation
 package has been supplied. The
 Feigin--Frenkel screening presentation preserves the conformal
 structure and the $\Ainf$ structure transported by Argument~$A$,
 but not the coproduct.
\end{enumerate}
\end{theorem}
...
\noindent
\textbf{Step~1}
(positive-half comparison package).
The only Jordan-quiver CoHA identification used without an
extra package is
$\cH_{\mathrm{Jor}}\cong
Y^+(\widehat{\mathfrak{gl}}_1)$
of Schiffmann--Vasserot~\cite{SV13}. For $N \geq 2$ the proof
does not assume a rank-$N$ Jordan-quiver isomorphism. The CoHA
comparison is the following conditional package: a type-$A$
positive-half critical CoHA equipped with the JKL vertex
coproduct~\cite{JKL26}; an identification of that positive half
with the required $Y^+(\widehat{\mathfrak{gl}}_N)$; the
Drinfeld double and completion; and the Miura/DS evaluation to
\(\cW_N[\Psi]\). The construction of the chiral coproduct below
uses Argument~$A$; this package is used only for the comparison
route of Argument~$B$.
...
(computed at $N = 2$: commutator norm $= 1.0$;
...
\noindent
\textbf{Step~6}
(OPE compatibility at all spins: construction and comparison).
The bar-cobar construction extends from $\cW_{1+\infty}$ to
$\cW_N$, while the CoHA route is a conditional comparison surface:
...
$\cW_N[\Psi]$ lies on the ordered Koszul locus, and the
...
Assume the positive-half package of Step~1. The JKL vertex
bialgebra theorem~\cite{JKL26} gives
\eqref{eq:ope-compat} on the relevant critical CoHA. After the
positive-half identification with $Y^+$ and passage through the
Drinfeld double, completion, and Miura/DS evaluation, this
compares with the Drinfeld coproduct on the \(\cW_N\) target.
This is not a second unconditional proof for all $N$; it is the
quiver-origin comparison surface.

\medskip
\noindent
\textbf{Step~7}
(Quantum determinant and full axiom check).
The quantum determinant~\eqref{eq:glN-qdet} is central
in $Y(\mathfrak{gl}_N)$ by
Lemma~\ref{lem:qdet-central-all-N} below,
computed numerically as proportional to
...
Argument~$A$ of step~6, with Argument~$B$ giving the
conditional CoHA comparison when its positive-half package is
...
and transfer matrix $T(u)$ satisfying the RTT relation, the standard
quantum determinant $\mathrm{qdet}\, T(u)$ is central
...
\begin{remark}[Spectral-shift pattern is load-bearing at
$N \geq 3$; product order is not]
\label{rem:qdet-decreasing-ordering}
The load-bearing feature of~\eqref{eq:glN-qdet} is the
SPECTRAL-SHIFT PATTERN (column $j$ carries the shift
$u - j\Psi$), not the product order. A first-principles
computation on the evaluation representation
$(\bC^N)^{\otimes M}$ with exact rational arithmetic confirms:

\begin{enumerate}[label=\textup{(\alph*)}]
\item \textbf{Product order is free.} The reversed
 column-determinant
 \[
 \mathrm{qdet}^{\leftarrow} T(u)
 \;:=\;
 \sum_{\sigma \in S_N}\mathrm{sgn}(\sigma)\,
 T_{\sigma(0),0}(u-(N{-}1)\Psi)\,
 T_{\sigma(1),1}(u-(N{-}2)\Psi)\cdots
 T_{\sigma(N-1),N-1}(u),
 \]
 obtained by reading the product~\eqref{eq:glN-qdet} right-to-left
 while preserving the spectral shifts, equals
 $\mathrm{qdet}\, T(u)$ as an element of $Y(\mathfrak{gl}_N)$;
 the symmetric sum over $S_N$ and the signs absorb the
 reorientation. Both are central.
\item \textbf{Shift pattern is rigid.} The reversed-shift variant
 \[
 ...
 \qquad\textup{(row-det; leftmost factor
 carries shift $(N{-}1)\Psi$)}
 \]
 produces a non-central element at $N \geq 3$:
 $[\mathrm{qdet}^\uparrow T(u), t_{ij}(v)]$ is generically nonzero
 at order $\Psi^3$ in the small-$\Psi$ expansion (log--log
 slope $\to 3$ as $\Psi \to 0$ at $N = 3$, $M = 2$, exact-
\texttt{Fraction} calculation). The same is true of the plain
 row-determinant with the canonical shift pattern: it is a
 \emph{different} Yangian element from the column-determinant
 and fails centrality at $N \geq 3$.
\item \textbf{$N = 2$ rank coincidence.} At $N = 2$, column-det,
 row-det, and both orientations of the shift pattern
 coincide as elements of $Y(\mathfrak{gl}_2)$. The
 antisymmetriser image in $(\bC^2)^{\otimes 2}$ has
 codimension~$1$, forcing alignment. This is a coincidence of
 rank, not a structural feature.
\end{enumerate}

The structural distinction at $N \geq 3$ is therefore:
\emph{column-determinant with the canonical shifts $u - j\Psi$ is
central}; row-determinants and shift-reversed variants are not.
In the chiral setting, both product orientations are preserved by
$\Delta_z$ (the argument of
Lemma~\textup{\ref{lem:qdet-central-all-N}} is product-order
agnostic), so the canonically-shifted column-determinant
$\mathrm{qdet}\, T(u)$ is the chirally central element for every
$N \geq 1$, independently of whether its factors are written
...
\item The OPE-compatibility construction is uniform in~$N$:
 argument~A uses only the structural properties
 of the cofree tensor coalgebra
 $T^c(s^{-1}\overline{\cW}_N)$ and the Koszul-locus
 equivalence. Argument~B is a comparison surface only after
 the relevant type-$A$ positive-half CoHA has been identified
 with $Y^+$ and transported through the Drinfeld double,
 completion, and Miura/DS evaluation.
\end{enumerate}
\end{remark}
...
deconcatenation coproduct $\Delta$, and with the associator/descent
gauges fixed as in Theorem~\textup{\ref{thm:chiral-qg-equiv}}, is the universal
...
principle: the ordered bar complex with deconcatenation is the
level-$2$ object before symmetrisation; the $R$-matrix, the chiral
...
leave four extension problems: geometric open sectors, associative
modular Maurer--Cartan theory, Drinfeld--Sokolov compatibility, and
coderived class-$\mathsf{M}$ coproducts.
...
Rather, the latter appears as the commutative shadow of the former.

\begin{theorem}[Comm\-utator-shadow theorem]
\ClaimStatusProvedElsewhere
\label{thm:FG-shadow-vol2}
...
\begin{remark}[Commutator filtration mechanism]
The proof constructs the commutator filtration directly on the ordered
bar complex. The associated graded kills the non-$\Sigma$ defect,
descends to the commutative bar complex, and identifies the induced
differential with the Francis--Gaitsgory differential.
\end{remark}

\subsection{Bordered configuration spaces and the geometric open theory}

The algebraic ordered open trace formalism is conjectured to be realized geometrically by ordered bordered
...
carrier for the ordered open trace formalism. The Vol~II bordered-FM
comparison confirms that all five algebraic items follow once the
compactification exists.

Item~(4), annular closure computing chiral Hochschild homology,
requires the \emph{annular} bordered FM compactification
constructed in Vol~II, with cyclically ordered boundary points on
circles rather than linearly ordered points on intervals. This annular
case remains conjectural: the thickness-zero face requires Mok's
degeneration theory for the family $S^1\times[0,\epsilon]\to\{0\}$.

Four of the five required constructions are present. The missing map is
the annular bordered Fulton--MacPherson compactification with its
thickness-zero degeneration; until that map exists, item~(4) is the
single obstruction to annular closure.
\end{remark}
...
In the ordered cyclic convolution dg Lie algebra this equation is
\[
D_{\Assch}\Theta_A^{\Assch}
+ \tfrac{1}{2}[\Theta_A^{\Assch},\Theta_A^{\Assch}]=0.
\]
\end{theorem}
...
Principal Drinfeld--Sokolov reduction commutes with associative chiral
duality under the filtered hypotheses below. Arbitrary nilpotent
reduction requires the same PBW and BRST-lift conditions without the
principal simplifications.
...
\begin{remark}
Theorem~\ref{thm:ordered-associative-ds-principal} is the principal
case from Volume~II. Conjecture~\ref{conj:DS-arbitrary-nilpotent}
isolates the non-principal obstruction: PBW compatibility and the
lift of the BRST differential to the ordered bar coalgebra beyond the
principal and hook-type regimes.
\end{remark}

\subsection{The additional datum of a chiral coproduct}
\label{subsec:coderived-chiral-coproduct}

For an augmented chiral algebra $\cA$, the ordered bar object
$B^{\mathrm{ord}}(\cA)$ carries its deconcatenation coalgebra
structure.  Under the Ran reconstruction package $H_A$, the counit
$\Omega B^{\mathrm{ord}}(\cA)\to\cA$ gives an algebra model for
$\cA$.  A coproduct
$\cA\to\cA\widehat\otimes\cA$ is a further structure: it belongs to
the theory of bialgebra objects and requires compatibility with the
chiral product.  The fixed-coalgebra theorem
\ref{thm:off-koszul-ran-inversion} compares second-kind module
categories over $B^{\mathrm{ord}}(\cA)$.  Its source and target are
module categories over this fixed coalgebra.

\begin{definition}[Chiral bialgebra lifting datum;
\ClaimStatusDefinitional]
\label{def:chiral-bialgebra-lifting-datum}%
\label{conj:coderived-chiral-coproduct}%
Let $\cA$ be a complete augmented chiral algebra.  A chiral bialgebra
lifting datum on $\cA$ consists of
\begin{enumerate}[label=\textup{(\roman*)}]
\item a continuous morphism of augmented chiral algebras
\[
 \Delta_{\cA}\colon\cA\longrightarrow
 \cA\widehat\otimes\cA;
\]
\item the coassociative and counital identities
\[
 (\Delta_{\cA}\widehat\otimes\id)\Delta_{\cA}
 = (\id\widehat\otimes\Delta_{\cA})\Delta_{\cA},
 \qquad
 (\epsilon\widehat\otimes\id)\Delta_{\cA}
 =\id
 = (\id\widehat\otimes\epsilon)\Delta_{\cA};
\]
\item a filtered comparison between the bar construction of
$\Delta_{\cA}$ and the chosen completed tensor product, compatible
with the Ran factorization maps and the finite-window transition
morphisms.
\end{enumerate}
We denote the space of such data by $\mathsf{BiLift}(\cA)$.
\end{definition}

\begin{proposition}[The dual-number obstruction;
\ClaimStatusProvedHere]
\label{prop:dual-number-bialgebra-obstruction}
Let $k$ have characteristic zero and let
$A=k[\epsilon]/(\epsilon^2)$ with $\epsilon$ even of degree zero and
with its standard augmentation.
\textup{[Type signature: Open quadrant; augmented associative
presentation over a point; Beilinson level~$1$; hypothesis package
$\operatorname{char}k=0$.]}
Then
\[
 \mathsf{BiLift}(A)=\varnothing.
\]
Consequently, a functorial coproduct on augmented associative or
chiral algebras requires additional input restricting the source
category.
\end{proposition}

\begin{proof}
Counitality forces every candidate to have the form
\[
 \Delta_A(\epsilon)
 =\epsilon\otimes1+1\otimes\epsilon
  +a\,\epsilon\otimes\epsilon,
 \qquad a\in k.
\]
Since $\Delta_A$ is an algebra morphism, the relation
$\epsilon^2=0$ gives
\[
 0=\Delta_A(\epsilon^2)=\Delta_A(\epsilon)^2
   =2\,\epsilon\otimes\epsilon.
\]
The tensor $\epsilon\otimes\epsilon$ is a basis vector and
$2\in k^\times$, giving the required contradiction.
\end{proof}

\begin{remark}[Four type signatures at level~2]
\label{rem:coderived-chiral-coproduct-structure}%
\index{coderived category!chiral coproduct!mechanism}%
Four constructions meet in the completed level-$2$ ambient:
\begin{enumerate}[label=\textup{(\alph*)}]
\item The bar coalgebra has the coassociative chain map
$\Delta_B\colon B(\cA)\to B(\cA)\otimes B(\cA)$.
\item The universal twisting morphism gives the algebra counit
$\varepsilon_{\cA}\colon\Omega B(\cA)\to\cA$ in the Ran
reconstruction ambient.
\item Positselski's theorem compares coderived comodules and
contraderived contramodules over the fixed coalgebra $B(\cA)$.
\item A bialgebra lifting datum gives an algebra morphism
$\Delta_{\cA}\colon\cA\to\cA\widehat\otimes\cA$ and its
coassociative, counital, and factorization compatibilities.
\end{enumerate}
The first two constructions reconstruct an algebra from its bar
coalgebra; the third compares its module categories.  The fourth
equips that algebra with a coproduct.  Completed K\"unneth maps,
Drinfeld associators, and
Dunn additivity enter after the fourth datum has been constructed.
\end{remark}
...
The $\Ethree$ structure belongs to the derived chiral centre.  A
bialgebra lifting datum belongs to the open algebra $\cA$.  An
open--closed comparison between them is an additional Swiss-cheese
operation with its own chain model.  Thus a coderived K\"unneth theorem
for the closed centre and a chiral bialgebra lift for the open algebra
form independent construction problems before the open--closed map is
defined.
\end{remark}

\begin{definition}[Completed chiral bialgebra; \ClaimStatusDefinitional]
...
A \emph{completed chiral bialgebra} is a complete augmented chiral
dg algebra $(\cA,m,d,\epsilon)$ together with a continuous augmented
algebra morphism
\[
 \Delta_{\cA}\colon\cA\longrightarrow
 \cA\widehat\otimes\cA
...
such that
\begin{enumerate}[label=\textup{(\roman*)}]
\item $\Delta_{\cA}$ is coassociative and counital;
\item the differential is simultaneously a derivation of $m$ and a
coderivation of $\Delta_{\cA}$:
 \[
 \Delta_{\cA} \circ d
 \;=\;
 (d\widehat\otimes\id+\id\widehat\otimes d)
 \circ\Delta_{\cA};
 \]
\item $m$, $\Delta_{\cA}$, $d$, and $\epsilon$ are continuous for the
chosen completion and compatible with factorization gluing.
\end{enumerate}
Curved CDG objects require the corresponding curvature identity in
the chosen monoidal CDG category; passage to a coderived category
localizes module objects and preserves this curvature datum.
\end{definition}

\begin{proposition}[The ordered bar complex is a chiral coalgebra;
\ClaimStatusProvedHere]
...
finite-dimensional graded pieces, and assume the completed tensor
coalgebra is defined in the chosen Ran factorization category.
\textup{[Type signature: Open quadrant; ordered completed bar
presentation; Beilinson level~$2$; pro-nilpotent chiral Ran ambient;
hypothesis packages $H_{\mathrm{fact}}\cup H_{\mathrm{conv}}$.]}
Then
\begin{enumerate}[label=\textup{(\alph*)}]
\item the ordered bar object
$B^{\mathrm{ord}}(\cA)=T^c(s^{-1}\bar\cA)$ carries the strictly
coassociative deconcatenation coproduct $\Delta_B$;
\item $d_{\barB} = \sum_k \hat{m}_k^{\mathrm{ch}}$ the bar
 differential is the unique coderivation extending the suspended
 operations $m_k^{\mathrm{ch}}$;
\item in the flat case $m_0=0$, the $A_\infty$ identities give
$d_{\barB}^2=0$; in the curved case they give the corresponding CDG
curvature identity.
\end{enumerate}
These assertions give a coalgebra object.  Promotion to a completed
chiral bialgebra is equivalent to adjoining a datum from
$\mathsf{BiLift}(\cA)$ and checking its bar comparison.
\end{proposition}

\begin{proof}
Part~\textup{(a)} is Construction~\ref{constr:deconcatenation}: the
deconcatenation coproduct on the cofree tensor coalgebra
$T^c(s^{-1}\bar\cA)$ is strictly coassociative by inspection.
Part~\textup{(b)} is the universal property of cofree coalgebras:
a coderivation of $T^c(V)$ is determined by its projection
to~$V$, so each lifted operation $\hat{m}_k^{\mathrm{ch}}$
is a coderivation of~$\Delta_B$.
The compatibility
\[
\Delta_B\circ d_{\barB}
=
(d_{\barB}\otimes\id+\id\otimes d_{\barB})\circ\Delta_B
\]
follows because a sum of coderivations is a coderivation.
Part~\textup{(c)} is the arity-zero component of the same
$A_\infty$ coderivation equation.  The definition of a bialgebra then
shows exactly which additional datum remains.
\end{proof}

\begin{remark}[Independent structures and their comparison maps]
\label{rem:coderived-bialgebra-hochschild-readouts}%
\index{primitive coderived bialgebra|textbf}%
\index{coderived chiral bialgebra!projections}%
The coderivation $d_{\barB}$ determines the chiral
$A_\infty$ operations by corestriction to the cogenerators.  This is
the intrinsic content of the ordered bar coalgebra.
The derived chiral center
$Z^{\mathrm{der}}_{\mathrm{ch}}(\cA)$ belongs to the Hochschild component:
under the finite-type Koszul hypotheses of
Theorems~\ref{thm:HH-coHH-homology} and
\ref{thm:HH-coHH-cohomology}, it is computed by the chiral
Hochschild cochain complex.  The following structures require their
displayed additional data:
\begin{enumerate}[label=\textup{(\alph*)}]
\item a Yangian or quantum-group coproduct requires
$\mathsf{BiLift}(\cA)$;
\item an RTT presentation requires an $R$-matrix satisfying the
Yang--Baxter equation and
$R\Delta(a)=\Delta^{\mathrm{op}}(a)R$;
\item an $\mathsf E_3$ enhancement requires the brace operations of
Conjecture~\ref{conj:coderived-e3};
\item the symmetric bar requires an equivariant comparison from the
ordered configuration model to $\Sigma_n$-coinvariants.
\end{enumerate}
Comparison among these structures is itself a theorem with source,
target, and compatibility hypotheses.
\end{remark}
...
\begin{theorem}[Ordered bar complex of $\mathcal{W}_3$ via DS transport; \ClaimStatusConditional]
...
\item \textbf{Complementarity.}
 ...
 \]
 The companion fixed central charge is $c^* = K_3/2 = 50$; algebraic
 same-family Koszul self-duality is conditional on DS/bar transport.
...
\begin{theorem}[Class~M persistence under DS transport; \ClaimStatusConditional]
...
$\Barch_{\mathrm{ord}}(\mathcal{W}_N)$ contains the Virasoro $T$-line
obstruction package. It is class~$\mathrm{M}$ after the $T$-line
non-cancellation hypothesis of Theorem~\ref{thm:vir-non-formality} is
imposed.
...
the quartic pole $T_{(3)}T = c/2$ gives the first obstruction away from
the corresponding specialization locus. The higher operations are the
specialized Virasoro $T$-line classes.

\item DS reduction preserves Koszulness and creates the wheel tower:
$V_k(\mathfrak{sl}_N)$ is class~$\mathrm{L}$ \textup{(}double-pole
OPE, finite Lie transfer\textup{)}, while
$\mathcal{W}_N^k$ is the conditional class~$\mathrm{M}$ candidate
\textup{(}quartic-and-higher poles, with all-order non-formality
conditional on the $T$-line specialization\textup{)}. The Sugawara
construction escalates the pole depth:
...
The key structural point: the affine binary residue is finite-depth,
but the homotopy
...
the infinite cascade. \emph{DS reduction preserves Koszulness and
creates the wheel tower: this is the transport from class~$\mathrm{L}$
to class~$\mathrm{M}$.}
\end{remark}
...
object: the ordered bar complex $\Barchord(A)$ on
$\FM_k(\mathbb{C})\times\mathrm{Conf}_k^<(\mathbb{R})$. Ordered
associative chiral Koszul duality supplies the physical column;
shifted quantum groups of Finkelberg--Tsymbaliuk and the
Coulomb-branch construction of Braverman--Finkelberg--Nakajima supply
the algebraic-geometric column; Latyntsev's factorization quantum
groups and the Maulik--Okounkov stable-envelope construction supply
the spectral and stable-envelope columns, synthesized for
3d $\mathcal{N}=4$ theories by Costello--Okounkov--Zhang--Zhou.
...
(Theorem~\ref{thm:spectral-derived-additive-kz}).
...
every root has multiplicity one by Vol~II's root-sector scalar
strictification criterion.
...
envelopes are ordinary matrices; no $\Ainf$ data enters.
\end{itemize}
...
\begin{theorem}[Unshifted identification; \ClaimStatusConditional]
...
(Theorem~\ref{thm:spectral-derived-additive-kz}). The identification
...
\begin{theorem}[Factorisation identification under strictification;
\ClaimStatusConditional]
\label{thm:factorisation-identification}
On the chirally Koszul locus for a simple Lie algebra $\mathfrak{g}$,
assume a bar-horizontal strictification satisfying the root-sector
package of Theorem~\textup{\ref{thm:complete-strictification-v1}}.
...
\begin{proof}
Vol~II's quasi-factorisation theorem shows that every
residue-bounded complete dg-shifted Yangian with a
...
passage from quasi to strict. Under the root-sector scalar
strictification criterion, this class vanishes filtration by
filtration, yielding a strict
factorisation quantum group.
\end{proof}

\begin{remark}[Factorisation quantum group definition]
...
ind-coherent sheaves on the Ran space of the spectral curve. Its
definition does not require choosing a strictification: the
factorisation isomorphisms are part of the defining data. The
$\Eone$-chiral bar complex $\barB^{\mathrm{ord}}(\cA)$ produces the
same object once strictification is carried out; for simple Lie
algebras this is a theorem only under the root-sector strictification
package. The
Kac--Moody regime with root multiplicities $>1$ is where the
...
The comparison with stable envelopes agrees on evaluation modules
...
\begin{remark}[Affine case and remaining extension]
Parts~(a) and~(b) of Conjecture~\ref{conj:three-language-equivalence}
are conditional in the standard affine case
...
\subsection{Boundary questions from ordered chiral homology}
...
cofibrant chain operad model $C_*^\Phi(\Ethree)$.  Writing
$\lambda = k + h^\vee$, the formal-disk global sections of the
chiral $\Ethree$ model for the derived chiral centre
$Z^{\mathrm{der}}_{\mathrm{ch}}(V_k(\fg))$ and the
Costello--Francis--Gwilliam perturbative Chern--Simons
$\Ethree$-algebra $\cA^\lambda$ determine the same MC class in the
filtered deformation groupoid of cofibrant
$C_*^\Phi(\Ethree)$-algebra models over
$\lambda H^3(\fg)[[\lambda]]$.
\end{theorem}

\begin{proof}
The deformation spaces agree (both one-dimensional at each order
in~$\lambda$ for simple~$\fg$), the associated graded algebras match
(both $C^*(\fg)$), and the formal disk global sections match at the
level of $\mathsf{P}_3$-brackets
(Theorem~\ref{thm:chiral-e3-cfg}\textup{(ii)}).
For simple~$\fg$, $H^3(\fg) \cong \CC$ is one-dimensional, so at
each order $p \geq 1$ in $\lambda$ the space of
...
MC classes to agree. The gauge equivalence is constructed order by
order and passes to the $\lambda$-adic limit.
\end{proof}

\begin{conjecture}[CY-to-chiral trace comparison;
\ClaimStatusConjectured]
\label{rem:oackd-6}
\index{$\Phi$-trace comparison}%
\index{CY-to-chiral functor!$\Phi$!trace comparison}%
Let $Y$ be a Calabi--Yau surface and let
\[
\Phi=\SpCh_{\Sigma_2,E}\circ\PhiFA_3
\colon D^b_{\mathrm{CY}\text{-}3}(Y\times E)
\longrightarrow \mathrm{ChirAlg}_E
\]
denote the proposed two-stage CY-to-chiral comparison.  Choose an
object $\mathcal E$ and an equivalence
$\theta\colon\Phi(\mathcal E)\simeq A$.  Assume the package
$H_{\Phi\text{-}\mathrm{tr}}$: proper trace-class completions on both
sides, vanishing-cycle and orientation data for the compact CoHA,
compatibility of $\theta$ with the two trace maps, and convergence of
the ordered-bar supertrace.  The expected comparison is
\[
\operatorname{STr}^{\mathrm{ch}}_{\barB^{\mathrm{ord}}(A)}(\id)
\;=\;
\operatorname{STr}^{\mathrm{aut}}_{\mathcal E}(\id).
\]
The left side is curve-local; the right side belongs to the compact
global Hall realization.  A proof must construct the trace-preserving
comparison itself.  The finite Rees hCS--Hall maps furnish bounded
comparison classes and supply evidence in finite windows.  This
conjecture is logically separate from the line-side square-root
convention of Remark~\ref{rem:oackd-3} and from the automorphic
recognition problem of Remark~\ref{rem:oackd-4}.
\end{conjecture}
...
\item In the trace-form convention,
 $r(z) = k\,\Omega_{\mathrm{tr}}/z = -2\,\Omega/z$ is finite and nonzero.
...
$\chi_{\mathrm{ord}, 2} = \dim H^0_{\mathrm{ord}, 2} - \dim H^1_{\mathrm{ord}, 2}$
...
differential in the bicomplex $d_k = d_{\mathrm{crit}} + (k + 2)\delta$;
at generic $k$ one has $\dim H^0_{\mathrm{ord},2} = 0$ and
$\dim H^1_{\mathrm{ord},2} = 4$, giving $\chi_{\mathrm{ord}, 2} = -4$. Hence
at $k = -2$, $\dim H^1_{\mathrm{ord}, 2} = \dim H^0_{\mathrm{ord}, 2} -
\chi_{\mathrm{ord}, 2} = 4 - (-4) = 8$ (total dimension $12 = 3 \cdot 4$,
...
The ordered chiral homology detects both extremes through the
coinvariant operator and its trace. At critical level,
$K_{-h^\vee,\fg}^{\mathrm{coll}}(z)
=-h^\vee\Omega_{\mathrm{tr}}/z$ has the homotopy coinvariant
$q_2(K_{-h^\vee,\fg}^{\mathrm{coll}})$, while
$\tau_{-h^\vee,\fg}^{\mathrm{res}}q_2
(K_{-h^\vee,\fg}^{\mathrm{coll}})=-\dim(\fg)/2$ and
$\kappa=-\dim(\fg)/2+\dim(\fg)/2=0$.
\end{remark}

\begin{remark}[$N = 3$ explicit calculation]
\label{rem:gl3-explicit}
The $9 \times 9$ Yang $R$-matrix $R(u) = u\,I_9 + \Psi\,P$
for $\mathfrak{gl}_3$ satisfies the Yang--Baxter equation identically:
after expansion, the coefficients cancel by the permutation relations
for $P_{12},P_{13},P_{23}$.  Its $81$ degree-$1$ RTT component
relations are
...
is central by Molev's quantum-determinant theorem
\cite[Theorem~1.6.4]{Molev07}.
\end{remark}

\begin{computation}[Finite DS--Miura coproduct window at
$\cW_3$; \ClaimStatusProvedHere]
\label{comp:ds-w3-degree2-rtt-miura-witness}
Work in the Open quadrant, in the ordered associative
chiral/Yangian presentation, in the Beilinson level
$4 \to 5$ operator-to-coproduct lane.  The hypothesis package is:
generic $\Psi$, the principal Drinfeld--Sokolov projection
$\pi_3$ for $\mathfrak{sl}_3$, the RTT coproduct on matrix
generators, and the two-boson Miura stress tensor
\[
 x=\psi_1,\qquad y=\psi_2,\qquad
 e_1=x+y,\qquad e_2={:}xy{:},\qquad
 T_{\mathrm{M}}=e_2-\frac{e_1^2}{2\Psi}.
\]
In the $z^0$, degree-$2$ Cartan/Sugawara window,
\begin{align}
\label{eq:ds-w3-degree2-rtt-miura-window}
 &(\pi_3\otimes\pi_3)\Delta^{\mathrm{RTT}}_{z,0}(T_{\mathrm{M}})
 -T_{\mathrm{M}}\otimes1-1\otimes T_{\mathrm{M}} \\
 &\qquad =
 \frac{\Psi-1}{\Psi}\,(x\otimes y+y\otimes x)
 -\frac{1}{\Psi}\,(x\otimes x+y\otimes y).
 \notag
\end{align}
The same tensor is obtained by applying the primitive two-boson
Miura coproduct to \(T_{\mathrm{M}}\).
\end{computation}

\begin{proof}
On the RTT side, the component coproduct for the degree-$1$
matrix entries has diagonal contributions
$t_{ii}^{(1)}\otimes1+1\otimes t_{ii}^{(1)}$ and off-diagonal
terms of the form \(t_{ij}^{(1)}\otimes t_{ji}^{(1)}\).
The principal DS projection sends the two diagonal Gauss/Cartan
entries to \(x,y\) and kills the six root terms
\[
 t_{12}^{(1)}\otimes t_{21}^{(1)},\quad
 t_{21}^{(1)}\otimes t_{12}^{(1)},\quad
 t_{13}^{(1)}\otimes t_{31}^{(1)},\quad
 t_{31}^{(1)}\otimes t_{13}^{(1)},\quad
 t_{23}^{(1)}\otimes t_{32}^{(1)},\quad
 t_{32}^{(1)}\otimes t_{23}^{(1)} .
\]
Thus the projected RTT computation gives
\[
...
and
\[
 \Delta(e_1^2)-e_1^2\otimes1-1\otimes e_1^2
 =2(x\otimes x+x\otimes y+y\otimes x+y\otimes y).
\]
Substituting \(T_{\mathrm{M}}=e_2-e_1^2/(2\Psi)\) gives
\eqref{eq:ds-w3-degree2-rtt-miura-window}.

The Miura side is independent: \(x\) and \(y\) are primitive
free-field currents, so multiplicativity gives the two displayed
formulae for \(e_2\) and \(e_1^2\) directly.  The two routes
therefore agree term by term in the \(\{x,y\}\)-tensor basis.
The finite carrier is the degree-$2$ Cartan/Sugawara span generated by
$x$ and $y$; the displayed expansions list every surviving tensor
monomial and show zero residual coefficient by coefficient.
\end{proof}

\begin{remark}[DS coproduct intertwining at $\cW_3$: degree-$1$ vs.\ degree-$\geq 2$]
\label{rem:ds-intertwining-w3}
The full chain-level identity
\[
(\pi_3 \otimes \pi_3)\circ\Delta_z^{\mathfrak{sl}_3}
=\Delta_z^{\cW_3}\circ\pi_3
\]
is the expected Drinfeld--Sokolov coproduct compatibility, but
it is not proved by the degree-$1$ Cartan tests.  At degree~$1$
the comparison is \emph{tautological}: both sides use the same
Drinfeld primitive formula for \(\Delta_z(J)\) and the same
universal \(\Delta_z(\psi_n)\) expansion.  Those degree-$1$
comparisons verify convention consistency only.

The first non-tautological window proved here is
Computation~\ref{comp:ds-w3-degree2-rtt-miura-witness}: the
projected $\mathfrak{sl}_3$ RTT computation and the primitive
two-boson Miura computation agree term by term on the
$z^0$, degree-$2$ Cartan/Sugawara sector.  This does not yet
prove the full spin-$3$ DS-HPL coproduct identity.  The spin-$3$
sector, where the cubic primary and gravitational
\(\mathrm{HPL}_3\) corrections enter, remains the load-bearing
non-tautological obligation for the full compatibility theorem.
The spectral coassociativity tests use \emph{shifted} parameters:
...
the sum of the two Schur--Weyl summands of shape~$(2,1)$, each of
dimension~$8$.
\end{remark}
...
rightmost. At $N = 2$ both orderings coincidentally agree.  For
general~$N$, the central column determinant is this decreasing-index
expression by Molev~\cite[Theorem~1.6.4]{Molev07}.
\end{remark}
...
Numerical evaluation gives agreement at machine precision
($<10^{-14}$) across $5$ generic points in the upper half-plane.
\end{remark}

\begin{remark}[Bernard heat identity for the Weierstrass
$\zeta$]
\label{rem:bernard-heat-identity-zeta}
\ClaimStatusProvedElsewhere
The Jacobi odd theta $\theta_1(z, \tau)$ on the torus of
periods $(1, \tau)$ satisfies the Bernard heat equation
\textup{(}Bernard, Nucl.\ Phys.\ B $303$ ($1988$),
eq.~$(2.15)$\textup{)}
\begin{equation}
\label{eq:bernard-theta1-heat}
4\pi i\,\partial_\tau \theta_1(z, \tau)
\;=\; \partial_z^2 \theta_1(z, \tau).
\end{equation}
The Weierstrass zeta $\zeta(z, \tau) = \theta_1'(z,\tau)/
\theta_1(z, \tau) + 2\,\eta_1(\tau)\, z$ with quasi-period
$\eta_1(\tau) = (\pi^2/6)\,E_2(\tau)$ inherits from
\eqref{eq:bernard-theta1-heat} the identity
\begin{equation}
\label{eq:bernard-zeta-identity}
4\pi i\,\partial_\tau \zeta(z, \tau)
\;=\; -\wp'(z, \tau)
\,+\, 2\bigl(\zeta(z,\tau) - 2\,\eta_1(\tau)\, z\bigr)
\bigl({-}\wp(z, \tau) - 2\,\eta_1(\tau)\bigr)
\,+\, 8\pi i\,\eta_1'(\tau)\, z.
\end{equation}
The derivation is two lines of $z$-differentiation of
$\log \theta_1$ combined with \eqref{eq:bernard-theta1-heat}
and $\zeta' = -\wp$. The quasi-period drift term
$8\pi i\,\eta_1'(\tau)\, z$ is forced by the $\tau$-dependence
of $\eta_1(\tau)$ and closes against the Halphen--Ramanujan
system
\[
\partial_\tau E_2 = \tfrac{1}{12}(E_2^2 - E_4), \qquad
\partial_\tau E_4 = \tfrac{1}{3}(E_2 E_4 - E_6), \qquad
\partial_\tau E_6 = \tfrac{1}{2}(E_2 E_6 - E_4^2),
\]
originally due to Ramanujan \textup{(}$1916$, Trans.\ Camb.\
Phil.\ Soc.\ $22$, \S $2$\textup{)} and Halphen
\textup{(}Trait\'e des fonctions elliptiques et de leurs
applications, Gauthier-Villars $1886$, Ch.~II\textup{)}.
On the Cartan sector of the
Knizhnik--Zamolodchikov--Bernard connection with connection
forms $A_z = \zeta(z_{ij}, \tau)\,\Omega / (k + h^\vee)$ and
$A_\tau = \wp(z_{ij}, \tau)\,\Omega / (k + h^\vee)$, the
naive scalar flatness $\partial_\tau \zeta = \partial_z \wp
= \wp'$ fails by exactly the three corrections in
\eqref{eq:bernard-zeta-identity}: the heat-equation
prefactor $1/(4\pi i)$, the nonlinear $\zeta\wp$ cross-term,
and the quasi-period drift. After those three corrections
are installed, scalar flatness is equivalent at leading order
in $\hbar$ to the Halphen--Ramanujan closure via
$\eta_1(\tau) = (\pi^2/6) E_2(\tau)$. Chain-level flatness of
the full KZB connection on the $n$-point configuration space
requires in addition the elliptic classical Yang--Baxter
equation with dynamical shift \textup{(}Felder, ICM
$1994$\textup{)}, and is established in the standard references
\textup{(}Felder, \cite{Felder94}; Etingof--Frenkel--Kirillov,
\cite{EFK98}, Ch.~$5$; Calaque, Enriquez, Etingof,
\emph{Universal KZB equations: the elliptic case},
Prog.\ Math.\ $269$ \textup{(}$2010$\textup{)}\textup{)}.
Identity \eqref{eq:bernard-zeta-identity} is the scalar Bernard heat
identity cited above; a finite numerical evaluation would not prove
it.  The two-point
matrix commutator $[A_z, A_\tau] = 0$ is trivial because
$\Omega$ self-commutes and so the two-point case is
structurally uninformative at the matrix level.
\end{remark}

\begin{remark}[$n \geq 3$ KZB flatness: Felder dynamical shift +
Halphen--Ramanujan]
\label{rem:kzb-n-point-dynamical-closure}
\ClaimStatusProvedElsewhere
\index{KZB connection!three-point flatness}
\index{R-matrix!Felder dynamical}
At three or more points on $E_\tau$, the Cartan-sector affine
Kac--Moody $r$-matrix $r_{\widehat{\fg}_k}^{(1)}(z,\tau) =
\Omega\,\zeta_\tau(z)/(k+h^\vee)$ appearing in the KZB connection
\eqref{eq:kzb} fails to satisfy the ordinary classical Yang--Baxter
equation on the full tensor space $V^{\otimes 3}$: the cross-pair
commutators $[\Omega_{12},\Omega_{13}]$, $[\Omega_{12},\Omega_{23}]$,
$[\Omega_{13},\Omega_{23}]$ combine with the three
$\zeta_\tau(z_{ij})$ into a residual that vanishes identically only
in the Cartan-diagonal sector. Felder \textup{(}ICM $1994$,
\cite{Felder94}\textup{)} showed that the correct object closing
$3$-point KZB flatness is the \emph{dynamical} elliptic $R$-matrix
$r(z,\lambda,\tau)$ with $\lambda \in \fh^*$ the Cartan dynamical
variable, satisfying the modified \emph{dynamical} CYBE
\[
...
\;=\; 0
\]
(Etingof--Varchenko \cite{EV98}). The rearrangement of the
$\zeta_\tau(z_{12})\zeta_\tau(z_{13})$ products required by the
dynamical-CYBE identity uses the theta-prime-normalised
Szeg\H{o}/prime-form \textup{(}Fay trisecant\textup{)}
specialisation on the torus,
\[
\wp(a,\tau) - \wp(b,\tau)
\;=\; -\,\frac{\vartheta_1(a+b,\tau)\,\vartheta_1(a-b,\tau)}
 {\vartheta_1(a,\tau)^2\,\vartheta_1(b,\tau)^2}
 \,\vartheta_1'(0,\tau)^2
\]
\textup{(}Fay $1973$, Cor.~$2.5$; Mumford \emph{Tata Lectures on
Theta I}, Ch.~I \S $5$\textup{)}. The
$\tau$-derivative closure at each pair uses the full Bernard
identity \eqref{eq:bernard-zeta-identity}; the $\tau$-derivative
closure of the quasi-period $\eta_1(\tau) = (\pi^2/6) E_2(\tau)$
uses the Halphen--Ramanujan system
\[
\partial_\tau E_2 = \tfrac{1}{12}(E_2^2 - E_4),\qquad
\partial_\tau E_4 = \tfrac{1}{3}(E_2 E_4 - E_6),\qquad
\partial_\tau E_6 = \tfrac{1}{2}(E_2 E_6 - E_4^2),
\]
which forms a closed algebraic-differential system in the ring
$\bC[E_2, E_4, E_6]$.

\medskip
\noindent\emph{Putting it all together.} Chain-level $n$-point KZB
flatness (the statement
$[\nabla^{\mathrm{KZB}}_z, \nabla^{\mathrm{KZB}}_{z'}] = 0$,
$[\nabla^{\mathrm{KZB}}_z, \nabla^{\mathrm{KZB}}_\tau] = 0$ for all
pairs on $\mathrm{Conf}_n(E_\tau)$ at $n \geq 3$) is proved at
leading order in $\hbar$ in Felder \cite{Felder94}; the full
dynamical/quantum closure on the universal KZB bundle over
$\overline{\cM}_{1,n}$ is \cite[Thm.~$1$ + Thm.~$5$]{CEE09}
\textup{(}Calaque--Enriquez--Etingof,
\emph{Universal KZB equations: the elliptic case},
Progress in Mathematics $269$, $2010$\textup{)}. Neither result
is established as a theorem body in Vol~I; its use here is
attribution-level. The
non-dynamical Belavin obstruction at $n=3$ is visible on
$V^{\otimes3}$ for $V=\mathbb C^2$: at
$(z_{12},z_{13},\tau,k)=(0.2+0.1\mathrm i,0.4+0.3\mathrm i,
0.3+\mathrm i,1)$, truncating the theta sums at $60$ terms gives
maximum-entry CYBE residual $0.8733359755$.  This is finite numerical
evidence, not a proof of a generic obstruction. The Cartan-diagonal
$2$-point restriction is
trivially flat at the matrix level because $\Omega$ self-commutes
(\S\ref{rem:bernard-heat-identity-zeta}); the genuine $2$-point
content is the scalar Bernard identity
\eqref{eq:bernard-zeta-identity}.
\end{remark}
...
\begin{enumerate}[label=(\arabic*)]
\item Reversal in Theorem~\ref{thm:opposite} uses the standard desuspended Koszul sign
\[
(-1)^{\sum_{i<j}(|a_i|+1)(|a_j|+1)}.
\]
...
\item The ordered bar differential uses the desuspension convention
\[
b_1(s^{-1}a)=-s^{-1}(d_Aa),\qquad
b_2^{(n)}(s^{-1}a\otimes s^{-1}b)
=(-1)^{|a|}s^{-1}(a_{(n)}b),
\]
and extends these maps as coderivations of $T^c(s^{-1}\bar A)$.
On the $i$-th adjacent pair the resulting prefix sign is
\[
(-1)^{\sum_{q<i}(|a_q|-1)+|a_i|}.
\]
With this convention \(d_{\Barchord}^2=0\) is exactly the desuspended
Stasheff identity.
...
\section{Appendix: ordered boundary calculus}

The ordered boundary calculus consists of the following constructions,
identifications, and computations.
...
coproduct, collision residue $r^{\mathrm{coll}}_{\cH}(z) = k\Omega_{\cH}/z$,
...
\item Conditional universal Kac--Moody Yangian criterion: for every simple
$\mathfrak{g}$ and non-critical $k\neq -h^\vee$,
$A^{!,\mathrm{ord}}_{\mathrm{line}}
(\widehat{\mathfrak{g}}_k)\simeq Y_\hbar(\mathfrak{g})$
at $\hbar = 1/(k+h^\vee)$ under the root-sector strictification
and $R$-twisted descent packages; trace-form collision residue
$k\Omega_{\mathrm{tr}}/z$ and KZ-normalized
$r^{\mathrm{KZ}}(z)=\Omega/((k+h^\vee)z)$; CYBE from Jacobi;
root-sector strictification criterion; curvature
...
Koszulness preserved, wheel tower created on the universal $T$-line,
$K_N = 2(N{-}1)(2N^2{+}2N{+}1)$
(Theorem~\ref{thm:class-m-ds-transport}).
\item $\beta\gamma$ ordered bar complex: pole-valued collision residue
$r^{\mathrm{coll}}=0$; regular contact operator
$\Theta = \beta\otimes\gamma - \gamma\otimes\beta$ with $\Theta^2 = 0$;
contact transport $R_{\mathrm{contact}} = \id + 2\pi i\,\Theta$
(terminating at linear order); Koszul dual a $4$-dimensional exterior
algebra (class~C, shadow depth~$4$)
(Theorems~\ref{thm:bg-ordered-bar}, \ref{thm:bg-rmatrix},
and \ref{thm:bg-koszul-dual}).
\item $bc$ ghost ordered bar complex: pole-valued collision residue
$r^{\mathrm{coll}}=0$; regular contact operator
$\Theta_{bc} = b\otimes c - c\otimes b$ with $\Theta_{bc}^2 = 0$;
contact transport $R_{\mathrm{contact}} = \id + 2\pi i\,\Theta_{bc}$;
Koszul dual a symmetric algebra (class~C, shadow depth~$4$, complementarity
$K = 0$) (Theorem~\ref{thm:bc-ordered-bar}).
\item Conditional Wakimoto bar complex descent: under the DS exactness,
PBW-preservation, and BRST-lift hypotheses, reduction of the
free-field ordered bar complex gives the affine ordered bar model; the
dg Yangian $Y^{\mathrm{dg}}_\hbar(\fg)$ is the corresponding
free-field Yangian reduction
...
\item Root-sector spectral Drinfeld strictification for simple Lie
algebras: after root-sector localization and coefficient comparison,
the spectral Drinfeld class vanishes at every filtration, so the
dg-shifted Yangian $Y^{\mathrm{dg}}_\hbar(\fg)$ carries a strict
filtered algebra structure
(Theorem~\ref{thm:complete-strictification-v1}).
...
\item Quantum-group parameter and braid/Cartan readout from genus-$1$
$B$-cycle monodromy of the KZB connection, with
$q = e^{\pi i\hbar}$
(Theorem~\ref{thm:b-cycle-quantum-group}).
\item Drinfeld--Kohno comparison on the affine evaluation surface: KZ
monodromy equals the quantum-group braid representation externally,
and the ordered bar complex reduces to KZ/KZB under one-loop collapse
(Theorem~\ref{thm:drinfeld-kohno}).
\item Affine genus-$1$ monodromy readout: the genus-$0$ Yangian
evaluation surface and the genus-$1$ KZB quantum-group readout are
related by the $B$-cycle exponential map
$\widehat{\mathbb{G}}_a \to \widehat{\mathbb{G}}_m$
...
\noindent
The diagonal is the boundary; the bar coalgebra is the ambient ordered
phase space; the annulus is the self-cotensor of the diagonal. Under
this identification, associative chiral duality produces open traces,
centres, and bordered genus operations from the same cotensor
calculus.
