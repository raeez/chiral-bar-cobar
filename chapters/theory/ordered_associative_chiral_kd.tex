...
= (26 - c)/2$.  At $c = 26$: $\kappa = 0$.  At $c = 13$:
$\mathrm{Vir}_{13}^! = \mathrm{Vir}_{13}$ is self-dual, so the
genus tower $F_g(Y^{\mathrm{dg}}(\mathrm{Vir}_{13})) =
-F_g(Y^{\mathrm{dg}}(\mathrm{Vir}_{13}))$ by the Koszul duality
anti-symmetry of the tower, giving $F_g = 0$.
\end{proof}
...
The ordered bar--cobar equivalence produces $A$ from $\Barch(A)$ in a
single step.  The question addressed here is what happens when one applies
the ordered bar construction \emph{twice}: starting from an $\Einf$-chiral
algebra $A$ (for instance $\hat\mathfrak{g}_k$), one first takes the
symmetric bar $\barB^{\Sigma}(A)$, then dualises to get the (ordered)
line-operator algebra $A^!_{\mathrm{line}}$, and finally applies the
ordered bar again to form $\Barch^{\mathrm{ord}}(A^!_{\mathrm{line}})$.
The result is a \emph{loss}: the open-colour double bar forgets the
central extension.  Recovery of the full algebra requires both colours
of $\mathrm{SC}^{\mathrm{ch,top}}$.

\subsection{The open-colour double bar for $\mathfrak{sl}_2$}

\begin{proposition}[Open-colour double bar of $\hat{\mathfrak{sl}}_2$]
\ClaimStatusProvedHere
\label{prop:double-bar-sl2}
Let $A = \hat{\mathfrak{sl}}_{2,k}$ at non-critical level $k\neq -2$,
and let $Y_\hbar(\mathfrak{sl}_2)$ with $\hbar = 1/(k{+}2)$ be its
ordered Koszul dual.  The ordered bar complex
$\Barch^{\mathrm{ord}}(Y_\hbar(\mathfrak{sl}_2))$ on $\mathbb{A}^1_u$
has cohomology
\[
H^\bullet\bigl(\Barch^{\mathrm{ord}}(Y_\hbar(\mathfrak{sl}_2))\bigr)
\;\simeq\;
U(\mathfrak{sl}_2[t]),
\]
the universal enveloping algebra of the current algebra
$\mathfrak{sl}_2[t]$ --- \textbf{without} the central extension
$k\,n\,\delta_{n+m,0}$.
\end{proposition}

\begin{proof}
We verify the claim degree by degree.

\smallskip\noindent\emph{Degree $1$.}
The Yangian generators $E_n, F_n, H_n$ of $Y_\hbar(\mathfrak{sl}_2)$
map to current modes $J^a_n$ under the bar differential.  This is the
standard identification of degree-one bar elements with generators.

\smallskip\noindent\emph{Degree $2$.}
The spectral OPE of the Yangian has only \emph{simple} poles in the
spectral parameter:
\[
[E(u), F(v)] \;\sim\; \frac{\hbar}{u - v}\cdot(\ldots).
\]
By the $d\log$ absorption principle (the bar differential uses the
propagator $d\log(u_i - u_j)$, which absorbs one power of the pole),
the bar differential extracts only the zeroth product.  Hence
\[
...
s^{-1}H_{n+m}.
\]
No central term $k\,n\,\delta_{n+m,0}$ appears: such a term would
require a \emph{double} pole in the spectral OPE, which the Yangian
does not have.  The central extension of $\hat\mathfrak{g}_k$ came from
a double pole $k\,\delta^{ab}/(z-w)^2$ in the \emph{holomorphic} OPE;
in passing to the Yangian (ordered Koszul dual), this double pole
becomes the deformation parameter $\hbar = 1/(k{+}h^\vee)$, not a
double spectral pole.

For the $E$--$E$ component: the Yangian OPE has its pole at
$u - v = \hbar$ (shifted off the collision diagonal), so
\[
d_{\mathrm{res}}(s^{-1}E \otimes s^{-1}E) = 0,
\]
correctly reproducing $[e,e] = 0$ in $\mathfrak{sl}_2[t]$.

\smallskip\noindent\emph{Degree $3$.}
The condition $d^2 = 0$ at degree $3$ is equivalent to the classical
Yang--Baxter equation for $r(u) = P/u$ (where $P$ is the permutation
operator), which holds by the Jacobi identity for $\mathfrak{sl}_2$.
\end{proof}

\subsection{Why the central extension is lost}
...
double Koszul dual
$(A^!_{\mathrm{line}})^!_{\mathrm{line}}$ recovers
$U(\mathfrak{g}[t])$, not $\hat\mathfrak{g}_k$.
\end{theorem}

\begin{proof}
The argument has three steps.

\smallskip\noindent\emph{Step 1: the level enters as a deformation
parameter.}
The level $k$ of $\hat\mathfrak{g}_k$ enters the ordered Koszul dual
$Y_\hbar(\mathfrak{g})$ as $\hbar = 1/(k + h^\vee)$.  This is a
\emph{scalar} parameter of the Yangian, not a structural feature of
its spectral OPE.

\smallskip\noindent\emph{Step 2: the spectral OPE has only simple
poles.}
The double pole $k\,\delta^{ab}/(z-w)^2$ in the holomorphic OPE of
$\hat\mathfrak{g}_k$ is the source of the central term
$k\,n\,\delta_{n+m,0}$ in the mode algebra.  Under the passage
$\hat\mathfrak{g}_k \leadsto Y_\hbar(\mathfrak{g})$, this double pole
does not survive as a double pole in the spectral variable: the
Yangian $R$-matrix $R(u) = 1 + \hbar\,r(u) + O(\hbar^2)$ has its
singularity at $u = 0$ with a \emph{simple} pole $r(u) = P/u$.

\smallskip\noindent\emph{Step 3: the ordered bar sees only simple
poles.}
The ordered bar differential of $Y_\hbar(\mathfrak{g})$ uses the
propagator $d\log(u_i - u_j)$ on the ordered configuration space
$\mathrm{Conf}_n^{<}(\mathbb{A}^1_u)$.  By the $d\log$ absorption
principle, the bar differential extracts the residue at the collision
$u_i = u_j$, which for a simple pole gives the zeroth product
$a_{(0)}b$.  A double pole would give a first-order derivative term
--- but the spectral OPE has no double pole.  Hence the bar
differential recovers only the Lie bracket of $\mathfrak{g}[t]$,
not the central extension.
\end{proof}

\subsection{The full $\mathrm{SC}^{\mathrm{ch,top}}$ double Koszul
duality recovers the central extension}
...
$(A^!)^! \simeq A$ recovers $A$ completely, including all central
extensions and higher-pole data.
\end{theorem}

\begin{proof}
The full $\mathrm{SC}^{\mathrm{ch,top}}$ Koszul duality operates on
\emph{both} colours simultaneously:
\begin{itemize}[nosep]
\item the \emph{closed colour} (symmetric bar
$\barB^{\Sigma}(A)$ on $\mathrm{Ran}(X)/\Sigma_n$, dualised by the
Verdier/Ran functor $D_{\mathrm{Ran}}$) carries the holomorphic OPE
data, including the double pole $k/(z-w)^2$ that encodes the central
extension;
\item the \emph{open colour} (ordered bar
$\Barch^{\mathrm{ord}}(A)$ on $\mathrm{Conf}_n^{<}(\mathbb{R})$,
dualised by linear duality) carries the topological/spectral data.
\end{itemize}
The Verdier/Ran duality $D_{\mathrm{Ran}}$ on the closed colour
preserves the full pole structure of the holomorphic OPE, so the
double pole survives.  The bar--cobar involution
$\Omega(\Barch(\Omega(\Barch(A)))) \simeq A$ (from
Hypothesis~\ref{hyp:inputs}(I), applied twice) then recovers $A$
in its entirety.

For $A = \hat\mathfrak{g}_k$: the closed-colour Verdier dual is
$\hat\mathfrak{g}_{-k-2h^\vee}$ (the Feigin--Frenkel dual level),
and applying the full $\mathrm{SC}^{\mathrm{ch,top}}$ duality again
returns to $\hat\mathfrak{g}_k$, with the central extension intact.
\end{proof}

\subsection{Moral: the two colours carry non-redundant information}
...
open-colour (ordered/topological) and closed-colour
(symmetric/holomorphic) bar complexes carry genuinely non-redundant
information.  Neither colour alone suffices to reconstruct $A$ from
$A^!$ by iterating Koszul duality.
\end{corollary}

\begin{proof}
The open-colour double bar $(A^!_{\mathrm{line}})^!_{\mathrm{line}}$
loses the central extension
(Theorem~\ref{thm:central-extension-invisible}), while the full
two-colour double Koszul duality $(A^!)^! \simeq A$ recovers it
(Theorem~\ref{thm:two-colour-double-kd}).  The difference is
precisely the closed-colour data: the double pole in the holomorphic
OPE, transported through Verdier/Ran duality.  Hence the closed
colour carries information invisible to the open colour.

Conversely, the open colour carries the $R$-matrix monodromy datum for
the ordered-to-unordered descent (\S\ref{sec:r-matrix-descent}),
which is not determined by the closed-colour symmetric bar alone
(cf.\ Proposition~\ref{prop:r-matrix-descent}).  Hence neither colour
is redundant.
\end{proof}
...
The double-bar computation for $\hat\mathfrak{sl}_{2,k}$ provides a
clean illustration of the two-colour architecture of
$\mathrm{SC}^{\mathrm{ch,top}}$:
\[
\begin{array}{c|c|c}
& \text{Closed (holomorphic)} & \text{Open (topological)} \\
\hline
\text{Pole order in OPE} & \text{double: } k/(z-w)^2
  & \text{simple: } \hbar/(u-v) \\
\text{Bar propagator} & d\log(z_i - z_j)
  & d\log(u_i - u_j) \\
\text{Extracted datum} & \text{central extension}
  & \text{Lie bracket of } \mathfrak{g}[t] \\
\text{Duality functor} & D_{\mathrm{Ran}} \text{ (Verdier)}
  & \text{linear} \\
\text{Double KD output} & \hat\mathfrak{g}_k \text{ (full)}
  & U(\mathfrak{g}[t]) \text{ (no central ext.)}
\end{array}
\]
The central extension is a closed-colour datum.  The $R$-matrix
monodromy is an open-colour datum.  The full
$\mathrm{SC}^{\mathrm{ch,top}}$ Koszul duality assembles both.
\end{remark}
...
\begin{conjecture}[Comm\-utator-shadow theorem]
