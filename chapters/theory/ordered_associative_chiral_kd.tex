...
presentation\textup{)}; the ordered associative comparison does \emph{not}
...
For instance, when $r^{\mathrm{coll}}(z)=k/z$ (Heisenberg),
this gives $k\,dz/z$. For affine Kac--Moody the collision residue
...
\item \emph{Affine $\widehat{\mathfrak{g}}_k$
 ...
 $k\Omega_{\mathrm{tr}}/z \in \End(\mathfrak{g}\otimes\mathfrak{g})$.
...
algebra on the Koszul locus. Fix a Drinfeld associator
...
\textup{(}the chiral Drinfeld formula\textup{)}. The statement is a
Koszul-locus statement: the bar--cobar counit
...
\(\cW_{1+\infty}\). The precise content is as follows.
\begin{enumerate}[label=\textup{(\Roman*)}]
\item \textup{($R$-matrix.)}
...
structures as the same Koszul-locus ordered-bar datum. The OPE
...
and the JKL vertex bialgebra theorem on the positive-half CoHA.
\end{theorem}
...
these three structures \emph{determine} each other, one checks
the uniqueness directions. On the Koszul
locus:
\begin{enumerate}[label=\textup{(\roman*)}]
...
rather than independently derived here.
\end{remark}
...
coefficient $1/\Psi$ appearing in both places is the same datum
...
Theorem~\ref{thm:miura-cross-universality-monograph}. At $N \geq 3$
...
The positive-half critical CoHA of the Jordan quiver at dimension
...
root of $\widehat{\mathfrak{gl}}_N$) satisfies the vertex
 ...
 Schiffmann--Vasserot identification
$\cH_{\mathrm{Jor},N} \cong Y^+(\widehat{\mathfrak{gl}}_N)$
transports this to the positive half; the Drinfeld double supplies
the full Yangian coproduct used on the \(\cW_N\) side. This gives a
second proof at all spins simultaneously. This argument supplies the
non-Heisenberg input at $N \geq 3$: a Feigin--Frenkel
...
\noindent
\textbf{Step~1}
(positive-half CoHA identification; Schiffmann--Vasserot~\cite{SV13}).
The critical CoHA of the framed Jordan quiver at
rank~$N$ satisfies
$\cH_{\mathrm{Jor},N} \cong Y^+(\widehat{\mathfrak{gl}}_N)$.
This is a positive-half statement; the full Yangian and the
\(\cW_N\)-vertex algebra enter only after the Drinfeld double and
Miura/DS evaluation. The $N = 1$ case is
...
gives the following conjectural coderived extension of the chiral quantum group
equivalence.
...
(\S\ref{subsec:chiral-e3-cfg-comparison}).
...
$\cW_3$ OPE after $\pi_3$-reduction; the independent computation agrees
