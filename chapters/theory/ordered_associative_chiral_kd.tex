...
as the Steinberg variety presents the Hecke algebra: the associative
dual already remembers the open boundary formalism.
...
$A_\infty$-corrections $m_k$ at arity $k\ge3$
...
$k/(z_1-z_2)^2$ by one step of $d\log$~absorption$ lowers every OPE pole
order by one).
...
\section{The Drinfeld presentation of $Y_z^\hbar(\mathfrak{sl}_2)$}
...
any arity; the mechanism is ghost-number filtration, not a
cancellation.
...
HPL tree correction to the coproduct at arity~$r \ge 2$ passes through
...
The vanishing is not an accident of low arity. The ghost-number
...
The statement holds at all arities simultaneously.
\end{proof}
...
complicated: the full $\Ainf$ tower is nonzero at all arities
...
identity $\kappa(\widehat{\mathfrak{g}}_k) + \kappa(Y_z^\hbar(\mathfrak{g})) = 0$
...
(cf.\ Remark~\ref{rem:chromatic-shadow-correspondence}).
\end{remark}
...
$Y_z^\hbar(\fg)$, Etingof--Kazhdan quantum vertex algebras),
...
vertex $R$-matrix $S(z)$ (braiding data from the arity-$2$ collision
...
m_j^{\mathrm{ch}} = 0$.
The inverse $\alpha^{-1}$ recovers $S(z)$ as the arity-$2$
KZ holonomy.
...
\bigskip
The Rosetta stone table and the chiral quantum group equivalence
...
and the chiral quantum group equivalence
(Theorem~\ref{thm:chiral-qg-equiv}) open five further problems.

\begin{conjecture}[$\mathsf{E}_3$ identification for affine Kac--Moody]
\label{conj:e3-identification-km}
\ClaimStatusConjectured
\index{E3 algebra@$\Ethree$-algebra!identification conjecture}
The derived chiral centre
$Z^{\mathrm{der}}_{\mathrm{ch}}(V_k(\fg))$ and the
Costello--Francis--Gwilliam perturbative Chern--Simons
$\Ethree$-algebra $\cA^\lambda$ are isomorphic as
$\Ethree$-algebras over the formal deformation space
$\hbar H^3(\fg)[[\hbar]]$, for simple~$\fg$ at non-critical
level $k \neq -h^\vee$.
The deformation spaces agree (both one-dimensional at each order
in~$\hbar$) and the formal disk global sections match at the level
of $\mathsf{P}_3$-brackets; what is missing is a chain-level
comparison as families, not fiber by fiber.
\end{conjecture}
...
via a period-corrected differential whose arity-$2$ coefficients
