...
$\Barch^{\mathrm{ord}}(A)$ its ordered bar complex.
The \emph{ordered tridegree} of a bar element
$\alpha \in \Barch^{\mathrm{ord}}(A)$ is the triple
$(g,n,d)^{E_1}$ where:
\begin{enumerate}[label=\textup{(\roman*)}]
...
differential $d_{\Barch^{\mathrm{ord}}}$ by the condition $d^2 = 0$
...
$\mathrm{av}\colon\Barch^{\mathrm{ord}}(A) \to \Barch(A)$ is a
