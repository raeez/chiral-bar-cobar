...
$R(z)=e^{k\hbar/z}$
(Theorem~\ref{thm:heisenberg-rmatrix}, AP41).
...
$k/(z_1-z_2)^2$ by one step of $d\log$~absorption (AP19:
the bar kernel $d\log(z_1-z_2)$ lowers every OPE pole
order by one).
...
\begin{remark}[Non-triviality: AP41]
...
$k/z$ is derived from the OPE double pole by AP19 ($d\log$
absorption); attempting to read the R-matrix off the raw OPE
without applying the AP19 step gives the incorrect answer
...
\begin{remark}[What non-formality is not: AP14 and AP41]
...
of the Swiss-cheese structure on $\mathrm{Vir}_c$ transferred to
bar cohomology: it says the Swiss-cheese operations
$m_k^{\mathrm{SC}}$ are not formal (AP14).  It does \emph{not}
...
freeness.  Koszulness (bar concentration) and Swiss-cheese
formality (vanishing of transferred higher operations) are
different properties.  Virasoro is Koszul but Swiss-cheese
non-formal.
\item \emph{$E_1$ versus $E_\infty$} is the question of locality.
$\mathrm{Vir}_c$ is and remains $\Einf$-chiral: local, with
$\Sigma_n$-equivariant factorization (AP35--AP47).  The ordered
...
$d\log$ absorption step (AP19/AP41) sets the correct pole
structure.
\end{enumerate}
\end{remark}
...
(cf.\ AP24 and Theorem~D of Volume~I).
...
rather than $a_{(0)}b$ \textup{(}AP19: the $d\log$ kernel
absorbs one pole order\textup{)}.  The two conventions are
...
anomaly into the Hodge curvature of the closed colour.
...
\item \emph{Quasi-modular component} (new at genus~$1$):
  ...
  linearly with $\kappa(\mathcal{A})$ (UNIFORM-WEIGHT; unconditional
  at genus~$1$).

\item \emph{Interaction}: the two components are not independent.
  The genus-$1$ free energy
  $F_1 = \kappa(\mathcal{A})/24$ (UNIFORM-WEIGHT)
  measures the leading Hodge class $\lambda_1$ on
...
enters as the scalar by which this cocycle acts on the closed colour:
...
collision-residue convention of AP19\textup{)}.
