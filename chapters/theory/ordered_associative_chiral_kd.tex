...
\begin{conjecture}[Miura cross-term universality]
\label{conj:miura-cross-universality-monograph}
\ClaimStatusConjectured
For all $s \geq 2$, the primary cross-term in
$\Delta_z(W_s)$ has coefficient $(\Psi - 1)/\Psi$ on
$J \otimes W_{s-1} + W_{s-1} \otimes J$.
The mechanism is spin-independent:
$\binom{s-2}{s-2} = 1$ from the Drinfeld formula minus
$1/\Psi$ from the Miura coefficient of ${:}J \cdot
W_{s-1}{:}$ in $\psi_s = W_s + (1/\Psi){:}J \cdot
W_{s-1}{:} + \ldots$.
Verified at spins $2$, $3$, $4$
\textup{(}$51 + 67 + 24$ compute tests\textup{)}.
\end{conjecture}
...
d-log absorption to $2N - 1$\textup{)} and
$4N - 4$ Stokes rays. $\cW_2$: $4$ rays.
...
$Z^{\mathrm{der}}_{\mathrm{ch}}(\cA)$ is the Hochschild
...
Part~(ii): the Casimir eigenvalues on the fundamental
representation are $+1/2$ and $-3/2$; multiplied by
$k = -2$ these give $-1$ and $+3$, both integers, so
$\exp(2\pi i \cdot \text{eigenvalue}) = 1$.
Part~(iii): trivial monodromy gives $H^0 = V \otimes V$
(all flat sections are global), and
...
\begin{remark}[DS coproduct intertwining at $\cW_3$]
...
= \Delta_z^{\cW_3} \circ \pi_3$ at degree~$1$,
verified with $57$ compute tests. The spectral
coassociativity uses \emph{shifted} parameters:
...
central \textup{(}$74$ compute tests, FM33\textup{)}.
\end{remark}
...
across $5$ generic points in the upper half-plane (FM34).
\end{remark}
