...
$\widetilde{M}_{24}$ Schur cocycle on the K3 input]
\label{rem:oackd-5}
\index{CY-$2$ shift!$[2]$ on K3 input}%
\index{$\widetilde{M}_{24}$!Schur multiplier}%
\index{Schur cocycle!$H^2(M_{24},U(1))$}%
...
symmetry acting on the K3 Fock lattice is not $M_{24}$ but its
non-split central extension $\widetilde{M}_{24}$ classified by a
Schur cocycle
\[
[\omega]
\;\in\;
H^2\bigl(M_{24},\,U(1)\bigr)
\;\cong\;
\ZZ/12,
\]
and the relevant cocycle $[\omega]$ has order exactly $6$ in this
cyclic group. The factor-of-$6$ is the same order that governs the
sixfold symmetry of the Igusa form $\Delta_5$ under the Fricke
involution of Remark~\ref{rem:oackd-4}, and the two are related
by the standard comparison between projective and linear representations
of $M_{24}$ on the K3 Fock space. Both refinements lie outside the
