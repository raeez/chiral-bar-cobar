...
scalar \(64\) and its square are checked, with this normalisation, by
\texttt{igusa-cusp-form/compute/verify\_square\_root.py} and
\texttt{calabi-yau-quantum-groups/compute/lib/borcherds\_denominator\_phi10\_engine.py};
those computations check the Borcherds/Gritsenko coefficient
normalisation, not compact Hall data. The
...
\]
The coefficient $64$ and its square are checked, in this convention, by
\texttt{igusa-cusp-form/compute/verify\_square\_root.py} and
\texttt{calabi-yau-quantum-groups/compute/lib/borcherds\_denominator\_phi10\_engine.py};
they establish the scalar automorphic face.
...
The identity has been computed at spins $2$--$6$ in the
rational function field $\bQ(\Psi)$ via
\texttt{miura\_universality\_proof\_engine.py} \textup{(}symbolic
channel-level abstraction $\prod_i(\sigma_i + J_i/\Psi)$,
spin-uniform by construction\textup{)}; full-algebraic coproduct
cross-checked at $s \in \{2,3,4\}$ by
\texttt{miura\_coproduct\_universal\_engine.py}.
\end{proof}
...
(Molev~\cite{Molev07}, Theorem~1.11.2; computed
...
first. The engine
\texttt{miura\_universality\_proof\_engine.py} verifies the
Miura side through $s = 6$; the Feigin--Frenkel side is
...
$81$ RTT component relations at degree~$1$:
...
\texttt{ds\_w3\_degree2\_rtt\_miura\_witness} in
\texttt{ds\_coproduct\_intertwining\_engine.py}; the decorated
test \texttt{test\_ds\_w3\_degree2\_rtt\_miura\_hz\_iv}
checks the RTT source terms, the killed off-diagonal list, and
the primitive Miura expansion without calling the shared
\(\Delta_z(\psi_n)\) formula.
\end{proof}
...
universal \(\Delta_z(\psi_n)\) expansion.  Those tests verify
convention consistency only.
...
Identity \eqref{eq:bernard-zeta-identity} itself follows
numerically at machine precision in the companion engine
\texttt{verify\_bernard\_heat\_identity\_zeta} across five
generic points in the upper half-plane; the two-point
...
non-dynamical Belavin obstruction at $n \geq 3$ is witnessed
numerically by the xfail test \texttt{test\_cybe\_generic\_points}
at \texttt{compute/tests/test\_theorem\_genus1\_seven\_faces\_engine.py}
(residual $\sim 1$ at generic $(z_{12}, z_{13})$; not a numerical
precision issue). The Cartan-diagonal $2$-point restriction is
