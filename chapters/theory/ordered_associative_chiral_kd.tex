...
Koszul-dual coalgebra $A^i=H^\bullet\Barchord(A)$, is the
line-side Koszul-dual algebra $A^!_{\mathrm{line}}=(A^i)^\vee$;
Verdier duality enters only after symmetric/Ran descent.
...
On the Koszul locus its cohomology is the Koszul-dual coalgebra
$A^i$, and the Koszul-dual algebra is $A^!=(A^i)^\vee$.
...
\begin{remark}[Universal curvature invariant
$\hbar^{2}\,K^{\kappa_{\mathrm{ch}}} = -1$]
\label{rem:oackd-3}
\index{curvature invariant!$K^{\kappa_{\mathrm{ch}}}$}%
\index{Bruinier reciprocity!three faces}%
\index{universal $\hbar^{2} K^{\kappa_{\mathrm{ch}}} = -1$}%
The KM curvature identity
\eqref{eq:km-yangian-curvature},
$\kappa(Y_\hbar(\fg)) = -\dim(\fg)\cdot(k+h^\vee)/(2h^\vee)$,
hides a sharper invariant that survives the passage to the BKM slot
of Remark~\ref{rem:oackd-1}. Define the
\emph{chiral curvature modulus}
\[
K^{\kappa_{\mathrm{ch}}}(A)
\;:=\;
\kappa(A)\cdot\kappa\bigl(A^!_{\mathrm{line}}\bigr),
\]
i.e.\ the product of the chiral curvature on the vertex side with
the chiral curvature on the line-side Koszul dual, read in the
trace-form convention fixed by Remark~\ref{rem:ord-assoc-notion-fixed}.
On the B-family of the CY-$2$ input \textup{(}the compact
Hall--Drinfeld double after the compact passage datum of
Remark~\ref{rem:oackd-1}\textup{)}, the modulus takes the value
$K^{\kappa_{\mathrm{ch}}} = 8$,
and this common value admits three genuinely independent derivations
through Bruinier reciprocity for the Igusa form on the
imaginary-root directions (automorphic face), through the lattice
Borcherds denominator \cite{Borcherds1992} specialised to
$\mathrm{II}_{3,19}$ (vertex face), and through the
Schiffmann--Vasserot shuffle presentation of the CoHA
(spectral face). For the standard slots whose line-side deformation
parameter is constructed in this chapter, the rescaling
$\kappa \mapsto \hbar\,\kappa$ that converts the trace-form curvature
into the deformation-parameter curvature removes the family
dependence:
\[
\hbar^{2}\,K^{\kappa_{\mathrm{ch}}}(A)
\;=\;
-1.
\]
For affine KM and Heisenberg this is the substitution
$\hbar=(k+h^\vee)^{-1}$ or $\hbar=k^{-1}$ into the trace-form
curvature. For $\beta\gamma$, Virasoro, and $\mathcal W_N$ it is the
same calculation after the contact or DS-transport normalisation
already recorded in their ordered-bar computations. For the BKM/K3
slot it is conditional on the compact Hall--Drinfeld passage datum and
on the value $K^{\kappa_{\mathrm{ch}}}=8$ above. No assertion is made
here for an arbitrary modular Koszul algebra before both the
line-side curvature and the deformation parameter have been
constructed.

This is the scoped cross-family landscape invariant: the bar and
cobar channels of the chain-level chiral Koszul duality are in
reciprocal balance once $\hbar$ is fixed by the corresponding
line-side normalisation. The identity is sharper than the
family-by-family complementarity values of Theorem~C (Volume~I): those values
$\kappa+\kappa^{!}\in\{0,8,13,250/3,98/3\}$ on the canonical
five-archetype $\mathsf{G}/\mathsf{L}/\mathsf{C}/\mathsf{M}/\mathsf{B}$
landmark ceiling (the classical four-archetype subset
$\{0,13,250/3,98/3\}$ restricts to $\mathsf{G}/\mathsf{L}/\mathsf{C}/\mathsf{M}$)
record the family-dependent additive offset of $\kappa$, while
$\hbar^{2}\,K^{\kappa_{\mathrm{ch}}}$
records the family-independent multiplicative pairing. The two
invariants are orthogonal witnesses of the same underlying duality
structure.
\end{remark}
...
$\cA^!_{\mathrm{line}}=(H^\bullet\Barchord(\cA))^\vee$ produced by
...
\smallskip
\noindent\emph{Specialisation via Theorem~C.}
The $\hbar$-parameter is fixed by the universal curvature identity
$\hbar^{2}\,K^{\kappa_{\mathrm{ch}}}=-1$ of
Remark~\textup{\ref{rem:oackd-3}}. For the K3 BKM slot
$K^{\kappa_{\mathrm{ch}}}=8$ \textup{(}Bruinier\,2002,
Proposition~$5.1$; Vol~I,
Remark~\textup{\ref{rem:climax-k3-face}}\textup{)}, so
\[
 \hbar^{2}_{\mathrm{K3}} \;=\; -\tfrac{1}{8}.
\]
The specialisation $\hbar^{2} = -1/8$ is not a free parameter: it is
the unique value at which the ordered bar--cobar composite closes
strictly on the ordered Koszul locus
$\overline{\mathcal{A}_2}\setminus H_1$ of
Remark~\textup{\ref{rem:k3-koszul-structure-precise}}.
...
dichotomy reflects the pole-order classification: double pole with
scalar residue (class~G) versus simple pole whose pole residue drops
but whose regular contact class is nilpotent (class~C).
\end{remark}
...
equivalence is associator-independent (checked for $\mathfrak{sl}_2$
...
\begin{remark}[Igusa form as the gauge invariant of the
compact Hall--Drinfeld double]
\label{rem:oackd-4}
\index{Igusa form!as gauge invariant}%
\index{$H^2(\fg_{\Delta_5})^{\ZZ/2,K(1)}$}%
\index{automorphic kernel!Siegel genus two}%
The Drinfeld associator torsor $\mathrm{GRT}_1(\bQ)$ of
Remark~\ref{rem:chiral-qg-grt1-torsor} witnesses the gauge ambiguity
on the affine side; its BKM counterpart admits a sharper
classification. For the compact Hall--Drinfeld double of
Remark~\ref{rem:oackd-1}, attached to the signature-$(2,1)$
Manin pair $(\fg_{\Delta_5}, \fn_+^{\mathrm{imag}} +
\fh^{\mathrm{imag},\mathrm{rk}\,23})$, the relevant
$H^2$ is not the deformation-cohomology
$H^2(\fg,\fg\otimes\fg)$ of the spectral Drinfeld obstruction
(Definition~\ref{def:spectral-drinfeld-class-oackd}) but the
equivariant cohomology
\[
H^2\bigl(\fg_{\Delta_5}\bigr)^{\ZZ/2,\,K(1)}
\]
computed with respect to the $\ZZ/2$-Fricke involution and the
$K(1)$-local refinement of the K3 Fock lattice. This group is
one-dimensional, with canonical generator the weight-$5$ Maass-spin
Igusa square root \(\Delta_5\), satisfying
\(\Phi_{10}^{\mathrm{un}}=\Delta_5^2\). The primitive BKM denominator
line is the
\(\Delta_5\)-line; in monic Borcherds normalisation
\(D_5=64^{-1}\Delta_5\). The OP square is
\(\Phi_{10}^{\mathrm{OP}}=D_5^2=4096^{-1}\Delta_5^2\), and the scalar
branch is
\(Z^X_{\mathrm{OP/DT}}=-(\Phi_{10}^{\mathrm{OP}})^{-1}
=-4096\,\Delta_5^{-2}\). These are orientation-forgetful
normalisations and do not supply the Hall double, the compact CoHA, or
the gauge class. The scalar \(64\) and its square are checked, with this
normalisation, by
\texttt{igusa-cusp-form/compute/verify\_square\_root.py} and
\texttt{calabi-yau-quantum-groups/compute/lib/borcherds\_denominator\_phi10\_engine.py};
those computations check the Borcherds/Gritsenko coefficient
normalisation, not compact Hall data. The classification statement
\[
H^2\bigl(\fg_{\Delta_5}\bigr)^{\ZZ/2,\,K(1)}
\;\cong\;
\CC\cdot\Delta_5
\]
identifies $\Delta_5$ as the unique (up to scalar) gauge invariant
of the compact Hall--Drinfeld double. Concretely, $\Delta_5$ plays the
role that the Drinfeld associator $\Phi$ plays for simple $\fg$:
where $\Phi$ is a cochain representative of the trivial class in
$H^2(\fg,\fg\otimes\fg)$ (since the class vanishes for every simple
$\fg$ by Theorem~\ref{thm:km-yangian}, Step~$4$), here the class does
\emph{not} vanish and its nonzero value is recorded by a single
automorphic form. The Bruinier-reciprocity identity of
Remark~\ref{rem:oackd-3} rests on this classification: the three
faces of $K^{\kappa_{\mathrm{ch}}}=8$ are three representations of the
same one-dimensional cohomology class as $\Delta_5$ specialised through
three different integral transforms.
\end{remark}
...
is established in
Proposition~\textup{\ref{prop:spin3-coproduct}} above.
...
(Theorem~\ref{thm:chiral-qg-equiv}) is restricted to the ordered Koszul
...
is a quasi-isomorphism only on the ordered Koszul locus. For raw
direct-sum class~$\mathsf{M}$ bar complexes, without a strict
completed/Yangian comparison package, the bar-cobar counit is
\emph{not} a chain-level quasi-isomorphism but \emph{is} an
isomorphism in the coderived category $D^{\mathrm{co}}$
(Theorem~\ref{thm:off-koszul-ran-inversion}). This does not
contradict the completed positive-energy/Yangian surfaces of
Theorems~\ref{thm:w-infty-chiral-qg} and
\ref{thm:glN-chiral-qg}: those theorems work in a completed
strict presentation, not in the raw direct-sum bar ambient.
The deconcatenation coproduct, being a purely combinatorial
structure on the cofree tensor coalgebra, survives intact and
has a coderived image. This gives the following conjectural
coderived extension of the chiral quantum group equivalence.
...
finite-dimensional graded/conformal pieces, so that continuous
duals and completed tensor products are defined, of arbitrary
shadow class.
\begin{enumerate}[label=\textup{(\roman*)}]
...
\item \textup{(Class-by-class agreement.)}
 ...
 Theorem~\textup{\ref{thm:chiral-qg-equiv}}.
 For raw direct-sum class~$\mathsf{M}$ inputs with no strict
 completed/Yangian comparison package, $\Delta^{\mathrm{ch,co}}$
 is the canonical coproduct produced by deconcatenation and equips
 the input with a coderived chiral quantum group structure. When
 a weight-completed or Yangian positive-energy package supplies a
 strict completed coproduct, its image in $D^{\mathrm{co}}$ agrees
 with $\Delta^{\mathrm{ch,co}}$.
\end{enumerate}
\end{conjecture}
...
It separates three ambients. The raw direct-sum class~$\mathsf M$
bar complex has no ordinary chain-level bar-cobar inverse in
$\Ch(\Vect)$. Completed positive-energy/Yangian presentations can
carry strict completed coproducts, as in
Theorems~\ref{thm:w-infty-chiral-qg} and
\ref{thm:glN-chiral-qg}. The coderived coproduct is the universal
off-locus construction and is required to be the image of those
strict completed coproducts whenever such a completed presentation
exists.
...
the CDG-comodule $(C, d_C)$ represents a well-defined object
...
Let $\cA$ be a complete augmented chiral algebra with
finite-dimensional graded/conformal pieces. The ordered bar complex
...
\item the coderived acyclicity condition holds by
 ...
 and Theorem~\textup{\ref{thm:bv-bar-coderived-vol1}}: the
...
For raw direct-sum class~$\mathsf{M}$ bar complexes without a
strict completed/Yangian comparison package, the coderived category
is essential: $d_{\barB}^2 \neq 0$ at chains, but $d_{\barB}^2$ is
coacyclic. In a completed/Yangian package this proposition records
the coderived shadow of the stricter completed chain-level structure,
not a denial of that structure.
\end{proposition}
...
(Theorem~\ref{thm:bv-bar-coderived-vol1}) to absorb the
...
\begin{remark}[Primitive coderived bialgebra and Hochschild readouts]
...
the single level-$2$ ordered object from which
...
Hochschild cochain complex. The $\mathsf{E}_3$ enhancement is the
additional coderived assertion of
Conjecture~\ref{conj:coderived-e3}; it is not a consequence of the
bar--cobar counit alone.
...
\item $\barB^{\mathrm{ord}} \mapsto \barB^{\Sigma}$ via
 ...
 $(\kappa, C, Q, \ldots)$.
\end{enumerate}
The coderivation identity
\[
\Delta\circ d_{\barB}
=
(d_{\barB}\otimes\id+\id\otimes d_{\barB})\circ\Delta
\]
is the universal compatibility from which all specific compatibilities
are derived by functoriality.
\end{remark}
...
\begin{remark}[Universal $\Phi$-trace identity]
\label{rem:oackd-6}
\index{$\Phi$-trace identity!universal}%
\index{CY-to-chiral functor!$\Phi$!trace identity}%
\index{automorphic supertrace!$\Phi$-intertwining}%
Theorem~\ref{thm:e3-identification-km} identifies the derived chiral
centre of $V_k(\fg)$ with a Chern--Simons $\Ethree$-algebra for
every simple $\fg$. The companion intertwining theorem on the
CY-to-chiral side, Volume~III, couples this identification to an
automorphic invariant and closes the following diagram into a
\emph{single cross-volume trace equation}. For every input $(Y,A)$
where $Y$ is a Calabi--Yau surface (K3, abelian surface) and
$A = V_k(\fg)$ is an affine chiral algebra living on the
holomorphic direction of $Y\times E$, let
\[
\Phi \;=\; \SpCh_{\Sigma_2, E} \circ \PhiFA_3
\;\colon\;
D^b_{\mathrm{CY}\text{-}3}(Y\times E)
\longrightarrow
\mathrm{ChirAlg}_E
\]
be the CY-to-chiral two-stage factorisation of
Part~\ref{part:v1-frontier}: stage~$1$ produces the canonical
$E_3$-holomorphic factorisation algebra
$\PhiFA_3(D^b_{\mathrm{CY}\text{-}3}(Y \times E))$ on the CY$_3$
variety $Y\times E$ (pinned by Kontsevich--Tamarkin $E_3$-formality
and Costello--Gwilliam--Li locality); stage~$2$ specialises along
the $Y$-fibre $\Sigma_2 = p_Y^{-1}(\mathrm{pt}) \subset Y \times E$
and restricts to the reference curve $E$
(Volume~III Theorem~\textup{phi-two-stage-factorisation};
established in Volume~III).
Then the chiral supertrace of the identity on the
ordered bar complex $\barB^{\mathrm{ord}}(A)$ and the automorphic
supertrace of the realised positive critical CoHA on $Y$ are
intertwined by this stage-$2$ shadow:
\[
\operatorname{STr}^{\mathrm{ch}}_{\barB^{\mathrm{ord}}(A)}(\id)
\;=\;
\operatorname{STr}^{\mathrm{aut}}_{\Phi^{-1}(A)}(\id).
\]
Finite Rees hCS--Hall maps provide the bounded comparison classes
entering this trace only before completion; the displayed equality is a
compact-CoHA statement and requires the vanishing-cycle realisation
and orientation data named in Remark~\ref{rem:oackd-1}.
This is the \emph{universal $\Phi$-trace identity}: its left-hand side
is a curve-local invariant computed from the collision-residue
$r$-matrix of Theorem~\ref{thm:e1-ordered-bar-landscape}, and its
right-hand side is a global automorphic invariant computed from the
Igusa-form gauge class of Remark~\ref{rem:oackd-4}. The equality
is both stronger than the family-by-family chiral/dual curvature
identities of Theorem~D (Volume~I) and dual, in a specific sense, to
the universal curvature modulus
$\hbar^{2}\,K^{\kappa_{\mathrm{ch}}}=-1$ of
Remark~\ref{rem:oackd-3}: the modulus identity is
multiplicative and curve-local, the $\Phi$-trace identity is additive
and cross-volume. Read together, the two identities pin down the
chain-level Koszul reflection at the level of secondary invariants,
one per side of the adjunction. At the locus where $\Phi$ is invertible
(e.g.\ affine Kac--Moody with rank $23$ K3 input), the identity
reduces to the $\Ethree$-centre identification of
Theorem~\ref{thm:e3-identification-km}; at the BKM locus
(Remark~\ref{rem:oackd-1}), the identity becomes the authoritative
statement, replacing the no-longer-applicable Yangian strictification.
\end{remark}
...
dynamical-CYBE identity uses the Szeg\H{o} three-term
\textup{(}Fay trisecant\textup{)} specialisation on the torus,
\[
\wp(a,\tau) - \wp(b,\tau)
