...
algebras. The remaining boundary is Kac--Moody algebras
...
The remaining proof obligation is the generic hyperbolic
...
The strictification mechanism of Theorem~\ref{thm:complete-strictification-v1}
produces $Y_\hbar(\fg)$ as a classical Yangian because the Cartan datum
...
$\Phi_3 = \SpCh_{\Sigma_2, C} \circ \PhiFA_3$, on the $K3$-fibre
...
\begin{theorem}[Drinfeld--Kohno on the affine evaluation lane;
...
evaluation lane, the Etingof--Schiffmann and
...
comparison on the affine evaluation lane.
\end{proof}
...
Yangian on the finite-dimensional evaluation lane; the genus-$1$
...
\item At genus~$1$, the ordered bar complex on
 ...
 evaluation lane, to the KZB monodromy. After the
...
\begin{proof}
Part~(i) is the content of the genus-$0$ programme: the bar
...
The centre appearing in the Hochschild lane of this chapter is the
...
$\mathfrak{sl}_2$ evaluation lane one expects, after the
Kazhdan--Lusztig completion and the half-monodromy convention
$q=e^{\pi i/(k+2)}$, a comparison between the completed Drinfeld
centre of the line category and the corresponding quantum-group
representation category. That comparison is not proved here and is
not an algebraic statement about
$Z^{\mathrm{der}}_{\mathrm{ch}}(A)$. The type~$A$ construction is the
DK-5 programme; see \cite[\S6]{Lorgat26I}.
\end{remark}
...
\begin{remark}[Finite-dimensional lane, category $\mathcal O$, and RTT]
\label{rem:line-category-fd-category-o-rtt}
\index{category O@category $\mathcal O$!not identified by line-category theorem}
\index{RTT presentation!finite-dimensional lane}
Theorem~\ref{thm:line-category} is a finite-dimensional
evaluation-lane statement. It identifies the line category with
...
Yangian on this lane, and Drinfeld polynomial classification applies
...
lane is therefore a first test case for genuinely ordered data, not
...
Two consequences for the programme. First, the universality at
...
$Z^{\mathrm{der}}_{\mathrm{ch}}(\cA)$ belongs to the Hochschild lane:
...
one forces this class to vanish at every filtration level by the
root-sector scalar strictification criterion, yielding a strict
...
same object once strictification is carried out, and for simple Lie
algebras the strictification is forced by root multiplicity one. The
...
at $\hbar = 1/(k+h^\vee)$; trace-form collision residue
$k\Omega_{\mathrm{tr}}/z$ and KZ-normalized
$r^{\mathrm{KZ}}(z)=\Omega/((k+h^\vee)z)$; CYBE from Jacobi; complete
...
\item Drinfeld--Kohno comparison on the affine evaluation lane: KZ
...
evaluation lane and the genus-$1$ KZB quantum-group readout are
