...
of a chiral algebra~$\cA$ on an algebraic curve~$X$
carries a deconcatenation coproduct, an $R$-matrix, and
Yangian structure; the symmetric bar
$B^\Sigma(\cA)$ on~$\operatorname{Ran}(X)$ is its
$\Sigma_n$-coinvariant shadow.
A universal Maurer--Cartan element
$\Theta_\cA \in \gAmod$ satisfies
$D\Theta_\cA + \tfrac{1}{2}[\Theta_\cA,\Theta_\cA] = 0$
in the modular convolution $\Linf$-algebra, and
its leading scalar projection under the averaging map
$\operatorname{av}\colon \mathfrak{g}^{E_1}_\cA
\to \gAmod$ is the modular
characteristic~$\kappa(\cA)$: family-specific
($\kappa^{\mathrm{KM}} = \dim(\mathfrak{g})(k+h^\vee)/(2h^\vee)$,
$\kappa^{\mathrm{Vir}} = c/2$,
$\kappa^{\mathrm{Heis}} = k$),
with complementarity $\kappa + \kappa' = K$
where $K = 0$ (Kac--Moody), $K = 13$ (Virasoro).

Five theorems extract the $\Sigma_n$-invariant content of
$\Theta_\cA$:
...
center local system under a shifted symplectic pairing,
with the scalar identity
$\kappa(\cA) + \kappa(\cA^!) = K(\cA)$
following from the eigenspace structure;
\textup{(D)}~the genus expansion
$F_g = \kappa(\cA)\cdot\lambda_g^{\mathrm{FP}}$
\textup{(UNIFORM-WEIGHT)}, with cross-channel corrections
$\delta F_g^{\mathrm{cross}}$ for multi-weight families at
$g \geq 2$;
\textup{(H)}~chiral Hochschild cohomology concentrated in
degrees~$\{0,1,2\}$ with polynomial Hilbert series.

The shadow obstruction tower of~$\Theta_\cA$ classifies
chiral algebras into four depth classes:
G~($r_{\max} = 2$, Heisenberg),
L~($r_{\max} = 3$, affine Kac--Moody),
C~($r_{\max} = 4$, $\beta\gamma$),
M~($r_{\max} = \infty$, Virasoro and $W_N$).
Ten unconditional equivalences, one conditional
(Lagrangian), and one forward implication ($D$-module
purity) characterize chiral Koszulness.
Programme milestones MC1--MC4 are proved; MC5 is proved
analytically (HS-sewing at all genera) and in the coderived
category, with the genuswise chain-level BV/BRST/bar
identification conjectural.
The entire standard landscape is computed and verified by
$120{,}000$+ tests.
\end{abstract}
...
linear leading term of the categorical logarithm: even this arity-$2$
...
% AP76: c(Y_{1,1,1}) = 0 (sigma = h1+h2+h3 = 0); kappa = Psi channel-by-channel.
...
% Shadow class inherited from seed X; twist sectors from arity 4.
...
% W(p) at p>=2: C_2-cofinite, non-rational, 4 generators (AP79).
% Koszulness OPEN (AP67: strong != free strong generation).
% --------------------------------------------------
