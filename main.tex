...
\noindent
The ordered bar complex~$B^{\mathrm{ord}}(\cA)$ of a chiral
algebra~$\cA$ on an algebraic curve~$X$ is a coalgebra over
the tensor cooperad~$T^c$ carrying an $r$-matrix, a Yangian,
and braided quantum group structure; the symmetric
bar~$B^\Sigma(\cA)$ on~$\operatorname{Ran}(X)$ is its
$\Sigma_n$-coinvariant projection.
Five theorems govern the theory:
...
for uniform-weight algebras, with cross-channel corrections
for multi-weight families;
\textup{(H)}~polynomial chiral Hochschild cohomology in
degrees~$\{0,1,2\}$.
A universal Maurer--Cartan element~$\Theta_\cA$ in the
modular convolution $\Linf$-algebra~$\gAmod$, whose
leading scalar shadow is~$\kappa(\cA) = \operatorname{av}(r(z))$,
produces a shadow obstruction tower classifying Koszul
algebras into four depth classes
(G, L, C, M with $r_{\max} = 2, 3, 4, \infty$).
...
All five programme milestones MC1--5 are proved.
The entire standard landscape is computed.
\end{abstract}
...
\ifannalsedition
  ...
  \phantomsection\label{thm:affine-half-space-bv}%        % Vol II affine_half_space_bv.tex
